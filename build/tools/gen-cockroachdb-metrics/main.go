...
// gen-cockroachdb-metrics generates the cockroachdb_metrics.go file for OpenTelemetry
// integration with Datadog metrics.
//
...
//
// 2. cockroachdb_metrics_base.yaml - extra_metrics section
//   - Contains metrics that are actively emitted by CRDB but NOT documented in metrics.yaml
//   - These are typically dynamically generated metrics based on runtime conditions
...
//
//	a) Downloads the latest Datadog metric mapping rules from the DataDog integrations repository
//	b) Reads and parses all three metric sources (metrics.yaml + base YAML with extra + legacy)
//	c) Processes metrics.yaml and extra_metrics, converting them to Prometheus format and applying
...
type BaseMappingsYAML struct {
	ExtraMetrics  []MetricInfo      `yaml:"extra_metrics"`
...
const goTemplate = `// DO NOT EDIT THIS FILE MANUALLY
//
...
const datadogMetricsURL = "https://raw.githubusercontent.com/DataDog/integrations-core/f37c7837771cf0df7771d2017fd05d55eb521994/cockroachdb/datadog_checks/cockroachdb/metrics.py"

// downloadDatadogMappings downloads the Datadog metrics mapping from their repository
func downloadDatadogMappings(ctx context.Context) (map[string]string, error) {
	client := &http.Client{Timeout: 30 * time.Second}
	req, err := http.NewRequestWithContext(ctx, "GET", datadogMetricsURL, nil)
	if err != nil {
	...
	if err != nil {
		return nil, fmt.Errorf("downloading Datadog mappings: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != 200 {
		return nil, fmt.Errorf("HTTP %d when downloading Datadog mappings", resp.StatusCode)
	}

	return parseDatadogMappings(resp.Body)
}
...
// with any extra metrics that aren't documented in metrics.yaml
func collectAllCRDBMetrics(yamlOutput *YAMLOutput, extraMetrics []MetricInfo) []MetricInfo {
...
func main() {
	if len(os.Args) < 3 {
		fmt.Fprintf(os.Stderr, "Usage: %s <metrics.yaml> <output.go> [base.yaml]\n", os.Args[0])
		os.Exit(1)
	}
	...
	datadogMappings, err := downloadDatadogMappings(ctx)
	if err != nil {
	...
	allMetrics := collectAllCRDBMetrics(yamlOutput, baseMetrics.ExtraMetrics)
	...
	if !strings.Contains(outputPath, "-exec-") && !strings.Contains(outputPath, "for tool") {
		...
		fmt.Printf("  - %d from metrics.yaml\n", len(allMetrics)-len(baseMetrics.ExtraMetrics))
		fmt.Printf("  - %d extra metrics\n", len(baseMetrics.ExtraMetrics))
