...
#      - Processed through Datadog mapping rules
#
# cockroachdb_metrics is a mapping of CockroachDB metric names to cockroachdb
...
func main() {
	if len(os.Args) < 4 {
		fmt.Fprintf(os.Stderr, "Usage: %s <metrics.yaml> <output.yaml> <datadog-mappings.py> [base.yaml]\n", os.Args[0])
		os.Exit(1)
	}
