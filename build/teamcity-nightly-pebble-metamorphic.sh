#!/usr/bin/env bash
#
# This script is run by the Pebble Nightly Metamorphic - AWS TeamCity build
# configuration.
...
GOOS=linux GOARCH=amd64 go build -o stress.linux github.com/cockroachdb/stress
export STRESS_BIN=stress.linux

# Build the Pebble metamorphic package.
...
GOOS=linux GOARCH=amd64 go test -o metamorphic.test.linux -c ./vendor/github.com/cockroachdb/pebble/internal/metamorphic
export METAMORPHIC_TEST_BIN=metamorphic.test.linux

# NB: We specify "true" for the --cockroach and --workload binaries to prevent
# roachtest from complaining (and failing) when it can't find them. The pebble
# metamorphic roachtests don't actually use either cockroach or workload.
exit_status=0
if ! timeout -s INT $((1000*60)) bin/roachtest run \
  ...
  --teamcity \
  --cpu-quota=384 \
  tag:pebble "pebble/metamorphic"; then
  exit_status=$?
fi
