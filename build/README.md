...
- `cd` into `vendor`, and ...
    ...
    + Run `git add .`.  Note: NOTE: Check `git status` after staging the changes as content may have 
  been missed because of `.gitignore`. `git add -f PATH_TO_FILE` can be used to add git ignored files.
...
update the master branch in vendored to point at the git SHA currently referenced in cockroachdb/cockroach.
