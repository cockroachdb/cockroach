...
# Write per-ticket version-check results to a file under the mounted
# /artifacts dir (host: $root/artifacts). The host wrapper folds this into
# $GITHUB_STEP_SUMMARY after the container exits so a version.txt/ticket
# mismatch shows up in the job summary, not just the logs.
args+=(--summary-file /artifacts/pick-sha-summary.md)
