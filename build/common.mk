...
# necessary. For an example, see build/cmd/crdb-build-release, which sets these
...
# variables directly from the command line. We want the source of truth to be
# either GOFLAGS or TAGS.
override USE_RACE := $(findstring -race,$(GOFLAGS))
override USE_MSAN := $(findstring -msan,$(GOFLAGS))
override USE_STDMALLOC := $(findstring stdmalloc,$(TAGS))
override USE_DEADLOCK  := $(findstring deadlock,$(TAGS))
...
# already run on TeamCity.
USE_ROCKSDB_ASSERTIONS := $(USE_RACE)
...
JEMALLOC_DIR := $(BUILD_DIR)/jemalloc$(if $(USE_MSAN),_msan)
PROTOBUF_DIR := $(BUILD_DIR)/protobuf$(if $(USE_MSAN),_msan)
ROCKSDB_DIR  := $(BUILD_DIR)/rocksdb$(if $(USE_STDMALLOC),_stdmalloc)$(if $(USE_ROCKSDB_ASSERTIONS),_assert)
SNAPPY_DIR   := $(BUILD_DIR)/snappy$(if $(USE_MSAN),_msan)
...
# multiple configurations at once, while still allowing the use of `go build`
# and `go test` for the configuration most recently built with Make.
#
...
# build encounters a given `arch_vendor_os_abi` target triple. The Go toolchain
# does not automatically set target-triple build tags, so these files are only
# compiled when building with Make.
...
$(ROCKSDB_DIR)/Makefile: $(C_DEPS_DIR)/rocksdb-rebuild $(ROCKSDB_SRC_DIR)/.extracted | libsnappy $(if $(USE_STDMALLOC),,libjemalloc)
	rm -rf $(ROCKSDB_DIR)
	mkdir -p $(ROCKSDB_DIR)
	@# NOTE: If you change the CMake flags below, bump the version in
	...
	cd $(ROCKSDB_DIR) && cmake $(CMAKE_FLAGS) $(ROCKSDB_SRC_DIR) \
	  ...
	  $(if $(USE_ROCKSDB_ASSERTIONS),,-DCMAKE_CXX_FLAGS=-DNDEBUG)
