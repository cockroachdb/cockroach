...
# Variables to be control executable names. These can be overridden in the
...
override ORG_ROOT  := $(REPO_ROOT)/..
override PKG_ROOT  := $(REPO_ROOT)/pkg
override UI_ROOT   := $(PKG_ROOT)/ui

# Ensure we we have an unambiguous in GOPATH.
export GOPATH := $(realpath $(ORG_ROOT)/../../..)
#                                     ^   ^  ^~ GOPATH
#                                     |  |~ GOPATH/src
#                                     |~ GOPATH/src/github.com

# Prefer tools from $GOPATH/bin and our local node modules over those elsewhere
# on the path. This ensures that we get the versions that we go install or yarn
# install.
