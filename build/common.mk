...
override STDMALLOC_TAG := $(if $(findstring stdmalloc,$(TAGS)),_msan)
override DEADLOCK_TAG := $(if $(findstring deadlock,$(TAGS)),_deadlock)
...
ROCKSDB_DIR  := $(BUILD_DIR)/rocksdb$(STDMALLOC_TAG)$(ROCKSDB_ASSERTION_TAG)
...
NATIVE_TAG := $(subst -,_,$(TARGET_TRIPLE))$(RACE_TAG)$(MSAN_TAG)$(STDMALLOC_TAG)$(DEADLOCK_TAG)
...
# invoked with.
#
# Suffixed flags files (e.g. zcgo_flags_{NATIVE_TAG}.go) have the build
# constraint `NATIVE_TAG` and are built the first time a Make-driven build
# encounters a particular NATIVE_TAG. This build tag is not set when invoking
# the Go toolchain directly, so these files are only compiled when building with
# Make.
