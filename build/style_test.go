...
func TestStyle(t *testing.T) {
	...
	t.Run("TestForbiddenImports", func(t *testing.T) {
		...
		settingsPkg := `github.com/cockroachdb/cockroach/pkg/settings`
		if err := stream.ForEach(stream.Sequence(
			filter,
			stream.Sort(),
			stream.Uniq(),
			stream.GrepNot(`cockroach/pkg/cmd/`),
			stream.Grep(`^`+settingsPkg+`: | (github\.com/golang/protobuf/proto|github\.com/satori/go\.uuid|log|path|context|syscall)$`),
			...
			case strings.HasPrefix(s, settingsPkg+": github.com/cockroachdb/cockroach"):
				if !strings.HasSuffix(s, "testutils") && !strings.HasSuffix(s, "syncutil") {
					t.Errorf("%s <- please don't add dependencies to settings pkg", s)
				}
			}
		}); err != nil {
			t.Error(err)
