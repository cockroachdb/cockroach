	...
	t.Run("TestMegacheck", func(t *testing.T) {
		...
		for checker, ignores := range map[lint.Checker][]lint.Ignore{
			...
			unused.NewLintChecker(unusedChecker): {
				...
				&lint.GlobIgnore{Pattern: "github.com/cockroachdb/cockroach/pkg/sql/irgen/parser/irgen.go", Checks: []string{"U1000"}},
				...
				// IR templates.
				&lint.GlobIgnore{Pattern: "github.com/cockroachdb/cockroach/pkg/sql/ir/base/*.tmpl.go", Checks: []string{"U1000"}},
			},
		} {
			t.Run(checker.Name(), func(t *testing.T) {
				...
				for _, p := range linter.Lint(lprog) {
					if !noCopyRe.MatchString(p.Text) {
						t.Errorf("%s: %s", p.Position, p.Text)
					}
				}
			})
		}
