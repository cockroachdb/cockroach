...
docker build --no-cache --pull --platform "linux/amd64" --tag="${gcr_tag_fips}" \
  --build-arg additional_packages="openssl crypto-policies-scripts" --build-arg fips_enable=1 "build/deploy-${platform_name}"
