...
# release-25.2 still ships publish-provisional-artifacts; the rename to
# publish-artifacts and the `release` subcommand split happened post-25.2.
...
if [[ $platform == "linux-amd64" || $platform == "linux-arm64" || $platform == "linux-amd64-fips" ]]; then
  ...
  if [[ $platform == "linux-amd64-fips" ]]; then
    ...
    # release-25.2's build/deploy/Dockerfile gates FIPS-specific package
    # installation on the fips_enabled build-arg. The post-25.2 Dockerfile
    # consolidated this so the arg is no longer needed; on release-25.2 we
