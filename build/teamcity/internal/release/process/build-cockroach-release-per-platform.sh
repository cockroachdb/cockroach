...
# release-24.1 still ships publish-provisional-artifacts; the rename to
...
if [[ $platform == "linux-amd64" || $platform == "linux-arm64" || $platform == "linux-amd64-fips" ]]; then
  ...
  if [[ $platform == "linux-amd64-fips" ]]; then
    ...
    # release-24.1's build/deploy/Dockerfile gates FIPS-specific package
    # installation on the fips_enabled build-arg. The post-25.x Dockerfile
    # consolidated this so the arg is no longer needed; on release-24.1 we
