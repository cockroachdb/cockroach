...
#   USE_COMPARE_CACHE: whether to use the cache for the compare revision (default: true)
...
if gsutil ls "$baseline_cache_path" &>/dev/null; then
  ...
  echo "Baseline cache found. Using it for comparison."
  unset "sha_arr[-1]"
fi
...
# Log into gcloud again (previous credentials were reset by the docker container)
log_into_gcloud
...
./bin/roachprod-microbench run "$ROACHPROD_CLUSTER" \
  ...
  ${sha_arr[1]:+--binaries baseline="${sha_arr[1]}"} \
...
# These should exists if the benchmarks were run successfully.
