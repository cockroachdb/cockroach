...
if [ "$#" -eq 0 ]; then
  ...
  echo ""
  echo "Note: if ROACHTEST_PREBUILT_BINARIES=1, it only checks that the binaries"
  echo "      or libraries for the component are present"
  exit 1
fi
...
bazel_args=()
# Array of build products. Each item has format "src:dest"; src is relative to
...
case "$component" in
  cockroach)
    ...
    products=("pkg/cmd/cockroach/cockroach_/cockroach:bin/cockroach.$os-$arch")
    ;;
  cockroach-ea)
    ...
    products=("pkg/cmd/cockroach-short/cockroach-short_/cockroach-short:bin/cockroach-ea.$os-$arch")
    ;;
  workload)
    # Workload binary.
    bazel_args=(--config force_build_cdeps //pkg/cmd/workload)
    products=("pkg/cmd/workload/workload_/workload:bin/workload.$os-$arch")
    ;;
  libgeos)
    # Geos libs.
  ...
  roachtest)
    ...
    products=("pkg/cmd/roachtest/roachtest_/roachtest:bin/roachtest.$os-$arch")
    ;;
  *)
...
if [ ${ROACHTEST_PREBUILT_BINARIES:-0} -eq 1 ]; then
  echo "Checking $os/$arch/$component..."
  for product in "${products[@]}"; do
    dst=${product#*:}
    if [ ! -f "$dst" ]; then
      echo "Error: could not find pre-built $dst"
      exit 1
    fi
  done
  exit 0
