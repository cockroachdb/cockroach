...
if [ $exit_status = 0 ]; then
  gsutil cp -r $ARTIFACTS_DIR/corpus gs://cockroach-corpus/corpus-$TC_BUILD_BRANCH
fi
...
# If validation passes its safe to update the copy in storage.
if [ $exit_status = 0 ]; then
  gsutil cp -r $ARTIFACTS_DIR/corpus-mixed gs://cockroach-corpus/corpus-mixed-$TC_BUILD_BRANCH
fi
