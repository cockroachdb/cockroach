...
CERTS_DIR=
URL=
LISTEN_ADDR_PORT="127.0.0.1:26375"
LISTEN_ADDR=
LISTEN_PORT=
...
parse_command_line() {
  ...
  if [[ -n $INSECURE ]] && [[ -n $CERTS_DIR ]]; then
    echo >&2 "error: cannot use certs in insecure mode" && exit 1
  fi

  if [[ -z $INSECURE ]] && [[ -z $CERTS_DIR ]]; then
      echo >&2 "using insecure mode by default" && INSECURE=true
  fi

  LISTEN_ADDR_REGEX="([^:]+)(:)?([^[:space:]]+)?"
  if [[ $LISTEN_ADDR_PORT =~ $LISTEN_ADDR_REGEX ]]; then
...
start_init_node() {
  ...
  # If not, add the `--certs-dir=CERTS_DIR` to init_env_query.
  if [[ -n $INSECURE ]]; then
...
setup_db() {
  ...
  local init_env_query=( -e "GRANT ALL ON DATABASE $COCKROACH_DATABASE TO $COCKROACH_USER" \
...
create_defaultdb() {
  ...
  db_already_exists="$(run_sql_query -e "SELECT 1 FROM [SHOW DATABASES] WHERE database_name='$COCKROACH_DATABASE'")"
  ...
  if [ "$(echo -n "$db_already_exists" | grep -c '^')" -eq 1 ]; then
...
create_default_user() {
  ...
  user_already_exists="$(run_sql_query -e "SELECT 1 FROM [SHOW ROLES] WHERE username='$COCKROACH_USER'")"
  # If there's no user with the given name, dbAlreadyExists should have only 1 row.
  # Otherwise, it should have at lease 2 rows.
  if [ "$(echo -n "$user_already_exists" | grep -c '^')" -eq 1 ]; then
  ...
  case $mode in
    ...
    *)
      if [[ -n $CERTS_DIR ]] && [[ -z $URL ]]; then
        ...
        check https://www.cockroachlabs.com/docs/stable/connection-parameters#connect-using-a-url" && exit 1
      fi
