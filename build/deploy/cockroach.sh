...
LISTEN_ADDR_PORT="127.0.0.1:26275"
LISTEN_ADDR=
...
setup_certs_dir() {
  ...
  if ! [[ $(find $CERTS_DIR -type f -name "node\.*") ]]; then
...
create_defaultdb() {
  ...
  # If there's no database with the given name, db_already_exists should have only 1 row.
...
create_default_user() {
  ...
  # If there's no user with the given name, user_already_exists should have only 1 row.
  # Otherwise, it should have at lease 2 rows.
