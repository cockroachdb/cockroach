...
var (
	suffix  = flag.String("suffix", "", "SUFFIX to pass to the Makefile")
	goflags = flag.String("goflags", "", "GOFLAGS to pass to the Makefile")
	tags    = flag.String("tags", "", "TAGS to pass to the Makefile")
)
...
func main() {
	...
	if flag.NArg() < 2 {
		log.Fatalf(
			"usage: %s [-suffix SUFFIX] [-goflags GOFLAGS] [-tags TAGS] RELEASE-CONFIG [MAKE-GOALS...]",
			os.Args[0],
		)
	}
	...
	if *goflags != "" {
		args = append(args, "GOFLAGS="+*goflags)
	}
	if *tags != "" {
