...
time make STATIC=1 build GOFLAGS=-race SUFFIX=.race
...
time make STATIC=1 testbuild PKG=./... TAGS=acceptance GOFLAGS=-race SUFFIX=.race

# We don't check all test binaries, but one from each invocation.
check_static cockroach.race
check_static cli/cli.test.race

strip -S cockroach.race
