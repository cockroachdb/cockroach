...
verify_docker_image(){
  ...
  if [[ $fips_build == true ]]; then
    # release-24.3's cockroach binary doesn't emit a "FIPS enabled: true"
    # line (that requires the crypto/fips140 check in pkg/build/info.go
    # introduced post-25.x). Fall back to the original detection: the
    # FIPS Go toolchain leaves "fips" in the Go version string, and the
    # FIPS docker image has an OpenSSL build that reports FIPS_VERSION.
