...
def stringer(src, typ, name, linecomment = False):
    native.genrule(
        name = name,
        ...
        cmd = """
         ...
         $(location @com_github_cockroachdb_tools//cmd/stringer:stringer) -output=$@ -type={typ} {linecomment} $<
        """.format(
            typ = typ,
            linecomment = "--linecomment" if linecomment else "",
        ),
        tools = [
            "@go_sdk//:bin/go",
