...
"${build_dir}"/builder.sh make gotestdashi

mkdir -p artifacts

"${build_dir}"/builder.sh make check 2>&1 | tee artifacts/check.log | go-test-teamcity

"${build_dir}"/builder.sh go generate ./pkg/...
"${build_dir}"/builder.sh /bin/bash -c '! git status --porcelain | read || (git status; git diff -a 1>&2; exit 1)'
...
if grep "make generate" pkg/ui/ui.go; then
    "${build_dir}"/builder.sh make -C pkg/ui
fi
