...
// SQLRunner wraps a *gosql.DB connection and provides convenience functions to
// run SQL statements and fail the test on any errors.
...
func (sr *SQLRunner) Exec(query string, args ...interface{}) gosql.Result {
	...
	if err != nil {
		sr.T.Fatalf("error executing '%s': %s", query, err)
	}
	return r
}
...
func (sr *SQLRunner) ExecRowsAffected(expRowsAffected int, query string, args ...interface{}) {
	...
	if numRows != int64(expRowsAffected) {
		sr.T.Fatalf("expected %d affected rows, got %d on '%s'", expRowsAffected, numRows, query)
	}
}
