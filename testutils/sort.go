...
func (ss structSorter) Less(i, j int) bool {
	...
	if v1.Kind() != reflect.Struct {
		if v1.Kind() != reflect.Ptr || v1.Elem().Kind() != reflect.Struct {
			panic(fmt.Sprintf("element %d is not a struct or pointer to a struct", i))
		}
		v1 = v1.Elem()
	}
	if v2.Kind() != reflect.Struct {
		if v2.Kind() != reflect.Ptr || v2.Elem().Kind() != reflect.Struct {
			panic(fmt.Sprintf("element %d is not a struct or pointer to a struct", j))
		}
		v2 = v2.Elem()
	}
