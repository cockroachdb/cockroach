...
- Direct merges using GitHub's "big green button" are avoided.  Instead, we use
  ...
  form `bors r=<reviewer>`.  For instance, if Ben gave you the LGTM, you'd
  write `bors r=bdarnell`.  If you're merging an external contributor's PR,
  just write `bors r+` to mark yourself as the reviewer.  Craig (our Bors bot)
