...
func (ds *DistSender) sendChunk(ctx context.Context, ba roachpb.BatchRequest) (*roachpb.BatchResponse, *roachpb.Error, bool) {
	...
	for {
		...
		if needAnother {
			...
			for i, union := range ba.Requests {
				...
				if !ok {
					...
					// have already been saturated at this stage but we have
		...
		// been masked out as saturated, while a saturated unbounded request
		// could have been the reason for needing the next descriptor; we now
		// have rs.Key=KeyMax.
