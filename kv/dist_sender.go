...
// request.
var errNo1PCTxn = roachpb.NewError(errors.New("cannot send 1PC txn to multiple ranges"))
...
func (ds *DistSender) sendBatch(ctx context.Context, ba *roachpb.BatchRequest) (*roachpb.BatchResponse, *roachpb.Error) {
	...
	for {
		...
		for r := retry.StartWithCtx(ctx, ds.rpcRetryOptions); r.Next(); {
			...
			if !includesFrontOfCurSpan(isReverse, desc, rs) {
				log.Tracef(ctx, "meta descriptor lookup for %s got %s", rs, desc)
		...
		if pErr := ds.deduceEarlyExitError(ctx); pErr != nil {
		...
		if ba.MaxSpanRequestKeys == 0 && ds.allocAsyncSender() && needAnother {
			...
			// Clone the txn to preserve the sequence number for the async call.
			if ba.Txn != nil {
				txnClone := ba.Txn.Clone()
				ba.Txn = &txnClone
			}
...
// sendPartialBatch sends the supplied batch to the first range in the
// specified span (or last if the request is a reverse op). The send
// occurs in a retry loop which looks up the first descriptor and then
// trims the batch to fit (i.e. only the requests which fall into the
// range of the first descriptor are included, and requests which span
// ranges are trimmed to fit within the first range's bounds).
func (ds *DistSender) sendPartialBatch(
	...
	for r := retry.StartWithCtx(ctx, ds.rpcRetryOptions); r.Next(); {
		...
		if desc == nil {
			log.Trace(ctx, "meta descriptor re-lookup")
		...
		case *roachpb.SendError:
			...
			log.Trace(ctx, "evicting range descriptor on send error")
			desc = nil
			continue
...
func (ds *DistSender) deduceEarlyExitError(ctx context.Context) *roachpb.Error {
	select {
