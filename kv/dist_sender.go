...
func (ds *DistSender) Send(ctx context.Context, ba roachpb.BatchRequest) (*roachpb.BatchResponse, *roachpb.Error) {
	...
	if len(parts) > 1 && ba.MaxScanResults != 0 {
		...
		panic("batch with same-direction scans needs splitting")
	}
	for len(parts) > 0 {
		part := parts[0]
		ba.Requests = part
...
func (ds *DistSender) sendChunk(ctx context.Context, ba roachpb.BatchRequest) (*roachpb.BatchResponse, *roachpb.Error, bool) {
	...
	// If MaxScanResults is set we have to keep track of how many results we accumulate and adjust
	// the limit for each request and possibly stop early.
	var batchLimit bool
	var remScanResults int64
	if ba.MaxScanResults != 0 {
		batchLimit = true
		remScanResults = ba.MaxScanResults
	}
	...
	for {
		...
		for r := retry.Start(ds.rpcRetryOptions); r.Next(); {
			...
			curReply, pErr = func() (*roachpb.BatchResponse, *roachpb.Error) {
				...
				if batchLimit {
					truncBA.MaxScanResults = remScanResults
				}
		...
		if batchLimit {
			...
			if numResults > remScanResults {
				panic(fmt.Sprintf("received %d results, limit was %d", numResults, remScanResults))
			}
			remScanResults -= numResults
			if remScanResults == 0 {
