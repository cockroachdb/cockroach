	...
	for {
		...
		for r := retry.Start(ds.rpcRetryOptions); r.Next(); {
			...
			case *roachpb.RangeKeyMismatchError:
				// Range descriptor might be out of date - evict it. If we have
				// new range descriptors, insert them instead as long as they
				// are different from the last descriptor to avoid endless loops.
