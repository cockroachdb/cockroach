		...
		if ba.MaxSpanRequestKeys > 0 {
			...
			if ba.MaxSpanRequestKeys == 0 {
				...
				for i, req := range ba.Requests {
					...
					case *roachpb.DeleteRangeRequest:
						// Add the start key of the DeleteRangeRequest as the
						// ResumeKey to indicate that this range hasn't been
						// deleted.
						reply = &roachpb.DeleteRangeResponse{ResumeKey: req.GetInner().(*roachpb.DeleteRangeRequest).Key}

					case *roachpb.BeginTransactionRequest:
						reply = &roachpb.BeginTransactionResponse{}

					case *roachpb.EndTransactionRequest:
						reply = &roachpb.EndTransactionResponse{}
