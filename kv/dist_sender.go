...
func (ds *DistSender) Send(ctx context.Context, ba roachpb.BatchRequest) (*roachpb.BatchResponse, *roachpb.Error) {
	...
	if ba.MaxScanResults != 0 {
		...
		for _, req := range ba.Requests {
			...
			default:
				return nil, roachpb.NewErrorf("batch with limit contains %T request", t)
			}
		}
		if fwd && rev {
	...
	for {
		...
		if needAnother {
			...
			for i, union := range ba.Requests {
				args := union.GetInner()
				// Skip some request types. BeginTransactionRequest is added
				// before a scan request that writes like DeleteRangeRequest().
				switch args.(type) {
				case *roachpb.NoopRequest,
