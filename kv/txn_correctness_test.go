...
func TestEnumerateIsolations(t *testing.T) {
	...
	enum := enumerateIsolations(3, bothIsolations)
	if !reflect.DeepEqual(enum, expIsolations) {
	...
	enum = enumerateIsolations(3, onlySerializable)
	if !reflect.DeepEqual(enum, expDegenerate) {
