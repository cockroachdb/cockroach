...
// enumerateHistoriesAfterRety returns a slice enumerating all
...
func enumerateHistoriesAfterRetry(err *retryError, h []*cmd) [][]*cmd {
	...
	// Now, introduce a series of histories which start with a command
	// from each of the histories other than the one which experienced
	// the retry.
	//
	// Note: to avoid the potential for endless restarts, alternate
	// histories returned with the enumeration will always include one
	// command from a transaction other than the one implicated by the
	// retry error. It would be better to not make this exception and
	// provide some other mechanism to notice and short circuit endless
	// retries.
	var results [][]*cmd
	for i := range cmds {
		// Excepting the txn which experienced retry, enumerate all
		// histories starting with first remaining command in each
		// txn history.
		if i != err.txnIdx && len(cmds[i]) > 0 {
			cp := append([][]*cmd(nil), cmds...)
			cp[i] = append([]*cmd(nil), cp[i][1:]...)
			histories := enumerateHistories(cp, false)
			for _, h := range histories {
				results = append(results, append([]*cmd{cmds[i][0]}, h...))
			}
		}
	}
	// If no results were added, it means the all histories were
	// played out except for the retrying txn's.
	if len(results) == 0 {
		results = [][]*cmd{cmds[err.txnIdx]}
	}
...
func TestEnumerateHistoriesAfterRetry(t *testing.T) {
	...
	for i, c := range testCases {
		// Take the case where we must retry on the 17th history for txn1 at "W(B,A)".
...
type verifier struct {
	...
	samplePct  float64 // percentage of histories to sample
}
...
func (hv *historyVerifier) runHistoryWithRetry(priorities []int32,
	...
	if err := hv.runHistory(priorities, isolations, cmds, db, t); err != nil {
		...
		// Randomly subsample 5% of histories for speed.
...
func (hv *historyVerifier) runHistory(priorities []int32,
	...
	if log.V(1) {
		log.Infof("attempting iso=%d pri=%d history=%s", isolations, priorities, plannedStr)
	}
