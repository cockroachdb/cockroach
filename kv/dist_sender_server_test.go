...
func checkScanResults(t *testing.T, results []client.Result, expResults [][]string, bound int) {
	...
	if count != bound {
		t.Errorf("count = %d, bound %d", count, bound)
	}
}
...
func TestMultiRangeBoundedBatchReverse(t *testing.T) {
	defer leaktest.AfterTest(t)()
	s, _, _ := serverutils.StartServer(t, base.TestServerArgs{})
	defer s.Stopper().Stop()
