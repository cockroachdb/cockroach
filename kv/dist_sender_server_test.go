...
func checkDelRangeResultsReturnKeys(t *testing.T, results []client.Result, expResults [][]string) {
	if len(expResults) != len(results) {
		t.Fatalf("only got %d results, wanted %d", len(expResults), len(results))
	}
	for i, res := range results {
...
func checkDelRangeResults(t *testing.T, results []client.Result, expResults [][]string) {
	...
	for i, res := range results {
		if len(expResults[i]) > 0 {
			// expKey is the next key to resume from.
			expKey := roachpb.Key(expResults[i][len(expResults[i])-1]).Next()
			if key := res.ResumeKey; !key.Equal(expKey) {
				t.Errorf("%s: expected key %d to be %q; got %q", errInfo(), i, expKey, key)
			}
		} else if res.ResumeKey != nil {
			t.Errorf("%s: expected key %d to be nil; got %q", errInfo(), i, res.ResumeKey)
		}
	}
}
...
func TestMultiRangeBatchBoundedDelRange(t *testing.T) {
	...
	runBatchWithDelRange := func(returnKeys bool) []client.Result {
		b := db.NewBatch()
		b.DelRange("aaa", "dd", 3, returnKeys)
		b.DelRange("a", "z", 2, returnKeys)
		b.DelRange("cc", "ff", 3, returnKeys)
		if err := db.Run(b); err != nil {
			t.Fatal(err)
		}
		return b.Results
	...
	// Set returnKeys to true
	returnKeys := true
	populateDB()
	results := runBatchWithDelRange(returnKeys)
	checkDelRangeResults(t, results, expectedResults)
	// check returned keys.
	checkDelRangeResultsReturnKeys(t, results, expectedResults)

	// Set returnKeys to false.
	returnKeys = false
	populateDB()
	results = runBatchWithDelRange(returnKeys)
	checkDelRangeResults(t, results, expectedResults)
	for _, res := range results {
		if len(res.Keys) != 0 {
			t.Fatalf("keys returned when not requested: %v", res.Keys)
		}
	}
}
...
func TestMultiRangeBoundedBatchWithDelRange(t *testing.T) {
	...
	for bound := 1; bound <= 20; bound++ {
		...
		// Pick a key bound larger than the number of keys in the range.
		b.DelRange("g", "h", 5, true)
		if err := db.Run(b); err != nil {
			t.Fatal(err)
		}
		...
		checkDelRangeResultsReturnKeys(t, b.Results, expResults)
		// Check whether ResumeKey has been set correctly.
		for i, res := range b.Results {
			...
			switch i {
			case 0:
				if bound > maxBounds[0] {
					if key != nil {
						t.Errorf("%s: expected key %d to be nil; got %q", errInfo(), i, key)
					}
			...
			case 2:
				if bound <= maxBounds[1] || bound > maxBounds[2] {
					if key != nil {
						t.Errorf("%s: expected key %d to be nil; got %q", errInfo(), i, key)
					}
				} else if !key.Equal(expKey) {
...
func TestMultiRangeBoundedBatchDelRangeOverlappingKeys(t *testing.T) {
	...
	for bound := 1; bound <= 20; bound++ {
		...
		b.DelRange("a", "b3", 0, true)
		b.DelRange("b", "f", 3, true)
		b.DelRange("c", "g", 0, true)
		b.DelRange("f1a", "g", 1, true)
		if err := db.Run(b); err != nil {
			t.Fatal(err)
		}
		...
		checkDelRangeResultsReturnKeys(t, b.Results, expResults)
		// Check whether ResumeKey has been set correctly. These checks are a
		// bit complex because different batch requests get acted on depending
		// on range boundaries.
		for i, res := range b.Results {
			key := res.ResumeKey
			var expKey roachpb.Key
			...
			case 0:
				if bound > 5 {
					if key != nil {
						t.Errorf("%s: expected key %d, %d to be nil; got %q", errInfo(), i, bound, key)
					}
				} else if !key.Equal(expKey) {
					t.Errorf("%s: expected key %d, %d to be %q; got %q", errInfo(), i, bound, expKey, key)
				}

			case 1:
				// Adjustments because request 0 and 1 work on range "b"
				if bound == 4 || bound == 5 {
					expKey = roachpb.Key("b")
				}
				if bound >= 4 {
					if !key.Equal(expKey) {
						t.Errorf("%s: expected key %d, %d to be %q; got %q", errInfo(), i, bound, expKey, key)
			...
			case 2:
				// Adjustments because request 1 and 2 work on range "c"
				if bound == 7 || bound == 8 {
					expKey = roachpb.Key("c")
				}
				if bound < 7 || bound > 12 {
					if key != nil {
						t.Errorf("%s: expected key %d, %d to be nil; got %q", errInfo(), i, bound, key)
			...
			case 3:
				// Adjustments because request 2 and 3 work on range "f"
				if bound > 9 && bound < 13 {
					expKey = roachpb.Key("f1a")
					if !key.Equal(expKey) {
						t.Errorf("%s: expected key %d, %d to be %q; got %q", errInfo(), i, bound, expKey, key)
					}
				} else if key != nil {
