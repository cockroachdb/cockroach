	...
	for bound := 1; bound <= 20; bound++ {
		...
		for i, res := range b.Results {
			...
			if rem <= 0 {
				...
				if key, expKey := string(res.ResumeSpan.Key), spans[i][1]; key != expKey {
					t.Errorf("%s: expected resume key %d to be %q; got %q", errInfo(), i, expKey, key)
				}
			} else {
				if key, expKey := string(res.ResumeSpan.Key), string(roachpb.Key(expResults[i][rem-1]).Next()); key != expKey {
					t.Errorf("%s: expected resume key %d to be %q; got %q", errInfo(), i, expKey, key)
				}
			}
			rem -= len(res.Rows)
	...
	for bound := 1; bound <= 20; bound++ {
		...
		for i, res := range b.Results {
			...
			if rem <= 0 {
				...
				if key, expKey := string(res.ResumeSpan.EndKey), spans[i][0]; key != expKey {
					t.Errorf("%s: expected resume key %d to be %q; got %q", errInfo(), i, expKey, key)
				}
			} else {
				if key, expKey := string(res.ResumeSpan.EndKey), expResults[i][rem-1]; key != expKey {
					t.Errorf("%s: expected resume key %d to be %q; got %q", errInfo(), i, expKey, key)
				}
			}
			rem -= len(res.Rows)
