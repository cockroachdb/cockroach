...
func sendOne(opts SendOptions, rpcContext *rpc.Context, client batchClient, done chan batchCall) {
	...
	go func() {
		...
		for state, err := c.State(); state != grpc.Ready; state, err = c.WaitForStateChange(ctx, state) {
			if err != nil {
				// TODO(tschottdorf): the way this is used suggests that it
				// may be better if these weren't retriable - they are
				// returned when the connection fails, i.e. for example when a
				// node is down or the network fails. Retrying on such errors
				// keeps the caller waiting for a long time and without a
				// positive outlook.
