...
func send(opts SendOptions, replicas ReplicaSlice,
	...
	sendNextTimer := time.NewTimer(opts.SendNextTimeout)
	sendNextTimerRead := false
	for {
		if !sendNextTimer.Reset(opts.SendNextTimeout) && !sendNextTimerRead {
			<-sendNextTimer.C
		}
		sendNextTimerRead = false
		select {
