...
func (so SendOptions) contextWithTimeout() context.Context {
	if so.Timeout != 0 {
		ctx, _ := context.WithTimeout(so.Context, so.Timeout)
		return ctx
	}
	return so.Context
}
...
func send(opts SendOptions, replicas ReplicaSlice,
	...
	sendOneFn(opts.contextWithTimeout(), rpcContext, orderedClients[0], done)
...
func sendOne(ctx context.Context, rpcContext *rpc.Context, client batchClient, done chan batchCall) {
