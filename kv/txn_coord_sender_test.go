...
func TestTxnCoordSenderAddRequest(t *testing.T) {
	...
	txnMeta, ok := s.Sender.txns[*txn.Proto.ID]
	if !ok {
	...
	txnMeta = s.Sender.txns[*txn.Proto.ID]
...
func TestTxnCoordSenderAddIntentOnError(t *testing.T) {
	...
	s.Sender.Lock()
	intentSpans := s.Sender.txns[*txn.Proto.ID].intentSpans()
...
func TestTxnCoordSenderGC(t *testing.T) {
	...
	if err := util.IsTrueWithin(func() bool {
		// Locking the TxnCoordSender to prevent a data race.
		s.Sender.Lock()
		_, ok := s.Sender.txns[*txn.Proto.ID]
		s.Sender.Unlock()
		return !ok
	}, 50*time.Millisecond); err != nil {
...
func TestTxnCoordSenderReleaseTxnMeta(t *testing.T) {
	...
	if _, ok := s.Sender.txns[*txn.Proto.ID]; ok {
