...
func TestTxnAbandonCount(t *testing.T) {
	...
	if pErr := db.Txn(func(txn *client.Txn) *roachpb.Error {
		...
		return nil
	}); !testutils.IsPError(pErr, "writing transaction is gone") {
...
func TestTxnReadAfterAbandon(t *testing.T) {
	...
	if !getRan {
		t.Fatalf("txn failed before Get call")
	} else if getErr == nil {
		t.Fatalf("Get succeeded on abandoned txn")
	} else if !testutils.IsPError(getErr, "writing transaction is gone") {
		t.Fatalf("unexpected error from Get on abandoned txn: %s", getErr)
	}
	if pErr == nil {
