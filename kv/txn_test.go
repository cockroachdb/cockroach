...
func TestTxnDBBasics(t *testing.T) {
	...
	for _, commit := range []bool{true, false} {
		...
		err := s.KV.RunTransaction(txnOpts, func(txn *client.Txn) error {
			...
			if err := s.KV.Run(client.GetCall(key, gr)); err != nil {
				return err
			}
			if gr.Value != nil {
		...
		gr := &proto.GetResponse{}
		err = s.KV.Run(client.GetCall(key, gr))
		if commit {
...
func TestTxnTimestampRegression(t *testing.T) {
	...
	err := s.KV.RunTransaction(txnOpts, func(txn *client.Txn) error {
		...
		if err := s.KV.Run(client.GetCall(keyA, nil)); err != nil {
			return err
		}

		// Now, read again outside of txn to warmup timestamp cache with higher timestamp.
		if err := s.KV.Run(client.GetCall(keyB, nil)); err != nil {
			return err
		}
...
func TestTxnLongDelayBetweenWritesWithConcurrentRead(t *testing.T) {
	...
	err := s.KV.RunTransaction(txnBOpts, func(txn *client.Txn) error {
		...
		if err := txn.Run(client.GetCall(keyB, gr1)); err != nil {
			return err
		}
		// Notify txnA put(b).
		ch <- struct{}{}
		...
		<-ch
		// get(b) again.
		if err := txn.Run(client.GetCall(keyB, gr2)); err != nil {
			return err
		}
...
func TestTxnRepeatGetWithRangeSplit(t *testing.T) {
	...
	go func() {
		err := s.KV.RunTransaction(txnAOpts, func(txn *client.Txn) error {
			...
			if err := txn.Run(client.PutCall(keyC, []byte("value2"), nil)); err != nil {
				return err
			}
			return nil
		})
	...
	err := s.KV.RunTransaction(txnBOpts, func(txn *client.Txn) error {
		...
		if err := txn.Run(client.GetCall(keyC, gr1)); err != nil {
			return err
		}
		...
		<-ch
		// Get(c) again.
		if err := txn.Run(client.GetCall(keyC, gr2)); err != nil {
			return err
		}
