...
func TestClockUpdateOnResponse(t *testing.T) {
	...
	doCheck := func(testFn rpcSendFn, fakeTime roachpb.Timestamp) {
		...
		if err != nil && err.String() != "boom" {
			t.Fatal(err)
		}
		newTime := ds.clock.Now()
		if newTime.Less(fakeTime) {
			t.Fatalf("clock was not advanced to %s (found %s instead)", fakeTime, newTime)
		}
	...
	doCheck(
		func(_ SendOptions, _ ReplicaSlice,
			args roachpb.BatchRequest, _ *rpc.Context) (*roachpb.BatchResponse, error) {
			rb := args.CreateReply()
			rb.Now = fakeTime
			return rb, nil
	...
	doCheck(
		func(_ SendOptions, _ ReplicaSlice,
			args roachpb.BatchRequest, _ *rpc.Context) (*roachpb.BatchResponse, error) {
			rb := args.CreateReply()
			rb.Error = roachpb.NewError(errors.New("boom"))
