...
func TestRangeCacheDetectSplit(t *testing.T) {
	...
	pauseLookupResumeAndAsserts := func(key string, expected int, evictToken evictionToken) {
		var wg sync.WaitGroup
		db.pauseRangeLookups()
		for i := 0; i < 3; i++ {
			wg.Add(1)
			go func() {
				...
				doLookupWithToken(t, db.cache, fmt.Sprintf("%s%d", key, i), evictToken, false)
				wg.Done()
			}()
		}
		time.Sleep(10 * time.Millisecond)
	...
	pauseLookupResumeAndAsserts("az", 3, evictToken)
