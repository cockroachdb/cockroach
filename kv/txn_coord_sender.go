...
func (tc *TxnCoordSender) Send(ctx context.Context, ba roachpb.BatchRequest) (*roachpb.BatchResponse, *roachpb.Error) {
	...
	if ba.Txn != nil {
		...
		txnID := *ba.Txn.ID
		if pErr := tc.maybeRejectClient(ctx, &ba); pErr != nil {
			return nil, pErr
		}
...
func (tc *TxnCoordSender) maybeRejectClient(ctx context.Context,
	ba *roachpb.BatchRequest) *roachpb.Error {
...
func (tc *TxnCoordSender) heartbeat(ctx context.Context, txnID uuid.UUID) bool {
	...
	// transactions (and more up-to-date information about whether they have to
	// restart), but in particular makes sure that they notice when they've
	// been aborted (in which case we'll give them an error on their next
	// request).
	tc.Lock()
