...
func (tc *TxnCoordSender) unregisterTxnLocked(txnID uuid.UUID) (
	...
	onePC = txnMeta.txn.OnePhaseCommit

	txnMeta.keys.Clear()

	delete(tc.txns, txnID)

	return duration, restarts, status, onePC
}
...
func (tc *TxnCoordSender) heartbeatLoop(ctx context.Context, txnID uuid.UUID) {
	...
	defer func() {
		tc.Lock()
		duration, restarts, status, onePC := tc.unregisterTxnLocked(txnID)
		tc.Unlock()
		tc.updateStats(duration, int64(restarts), status, onePC)
	}()
