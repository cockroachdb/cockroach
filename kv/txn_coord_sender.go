...
func (tc *TxnCoordSender) updateState(ctx context.Context, ba roachpb.BatchRequest, br *roachpb.BatchResponse, pErr *roachpb.Error) *roachpb.Error {
	...
	if pErr == nil && newTxn.Isolation == roachpb.SERIALIZABLE &&
		...
		log.Warningf("doing it %s vs %s", newTxn.OrigTimestamp, newTxn.Timestamp)
	...
	case *roachpb.ReadWithinUncertaintyIntervalError:
		...
		// interval, we simply move past the last observed timestamp from the
		// node.
		newTxn.Timestamp.Forward(newTxn.GetObservedTimestamp(pErr.OriginNode).Add(0, 1))
