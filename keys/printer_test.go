...
func TestPrettyPrint(t *testing.T) {
	...
	testCases := []struct {
		...
		{MakeKey(MakeTablePrefix(42),
			roachpb.RKey(encoding.EncodeFloat(nil, float64(233.221112), encoding.Ascending))),
			"/Table/42/233.221112"},
		{MakeKey(MakeTablePrefix(42),
			roachpb.RKey(encoding.EncodeVarint(nil, 1222, encoding.Ascending)),
			roachpb.RKey(encoding.EncodeString(nil, "handsome man", encoding.Ascending))),
			`/Table/42/1222/"handsome man"`},
		{MakeKey(MakeTablePrefix(42),
			roachpb.RKey(encoding.EncodeBytes(nil, []byte{1, 2, 8, 255}, encoding.Ascending))),
			`/Table/42/"\x01\x02\b\xff"`},
		{MakeKey(MakeTablePrefix(42),
			roachpb.RKey(encoding.EncodeBytes(nil, []byte{1, 2, 8, 255}, encoding.Ascending)),
			roachpb.RKey("bar")), `/Table/42/"\x01\x02\b\xff"/"bar"`},
		{MakeKey(MakeTablePrefix(42),
			roachpb.RKey(encoding.EncodeNull(nil, encoding.Ascending))), "/Table/42/NULL"},
		{MakeKey(MakeTablePrefix(42),
			roachpb.RKey(encoding.EncodeNotNull(nil, encoding.Ascending))), "/Table/42/#"},
		{MakeKey(MakeTablePrefix(42),
			roachpb.RKey(encoding.EncodeTime(nil, tm, encoding.Ascending))),
			"/Table/42/Sat Mar  7 11:06:39 UTC 2015"},

		// others
		{MakeKey([]byte("")), "/Min"},
		{MakeKey(MakeTablePrefix(42), roachpb.RKey([]byte{0x21, 'a', 0x00, 0x02})), "/Table/42/<util/encoding/encoding.go:518: unknown escape>"},
	}
