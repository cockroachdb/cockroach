...
type dictEntry struct {
	...
	// ugly-print the relevant prefix of the input, returning the remainder and
	// the key corresponding to the consumed prefix of 'input', or an error.
	upFunc func(input string) (string, roachpb.Key, error)
}
...
var (
	...
	keyDict = []struct {
		...
		{name: "/Local", start: localPrefix, end: LocalMax, entries: []dictEntry{
			{name: "/Store", prefix: roachpb.Key(localStorePrefix),
				ppFunc: localStoreKeyPrint, upFunc: localStoreKeyParse},
			{name: "/RangeID", prefix: roachpb.Key(LocalRangeIDPrefix),
				ppFunc: localRangeIDKeyPrint, upFunc: localRangeIDKeyParse},
		...
		{name: "/Meta1", start: Meta1Prefix, end: Meta1KeyMax, entries: []dictEntry{
			{name: "", prefix: Meta1Prefix, ppFunc: print,
				upFunc: func(input string) (string, roachpb.Key, error) {
					unq, err := strconv.Unquote(input[1:])
					if err != nil {
						panic(err)
					}
					if len(unq) == 0 {
						return "", Meta1Prefix, nil
					}
		...
		{name: "/Meta2", start: Meta2Prefix, end: Meta2KeyMax, entries: []dictEntry{
			{name: "", prefix: Meta2Prefix, ppFunc: print,
				upFunc: func(input string) (string, roachpb.Key, error) {
					...
					if len(unq) == 0 {
						return "", Meta2Prefix, nil
					}
					return "", RangeMetaKey(Addr(roachpb.Key(unq))), nil
				},
			}},
		},
	...
	rangeIDSuffixDict = []struct {
		...
		suffix []byte
		ppFunc func(key roachpb.Key) string
		upFunc func(rangeID roachpb.RangeID, input string) (string, roachpb.Key)
	}{
		{name: "SequenceCache", suffix: LocalSequenceCacheSuffix, ppFunc: sequenceCacheKeyPrint, upFunc: sequenceCacheKeyParse},
...
func localStoreKeyParse(input string) (remainder string, output roachpb.Key, _ error) {
...
func mustUnshiftSlash(in string) string {
	slash, out := mustUnshift(in)
	if slash != "/" {
		panic("expected /: " + in)
	}
	return out
...
func mustUnshift(in string) (first, remainder string) {
	if len(in) == 0 {
...
func localRangeIDKeyParse(input string) (remainder string, key roachpb.Key, _ error) {
	var rangeID int64
	var err error
	input = mustUnshiftSlash(input)
	if endPos := strings.Index(input, "/"); endPos > 0 {
		...
		return "", nil, errors.New("illegal RangeID")
	}
	input = mustUnshiftSlash(input)
	var infix string
	infix, input = mustUnshift(input)
	var replicated bool
	switch {
	...
	case bytes.Equal(localRangeIDReplicatedInfix, []byte(infix)):
		replicated = true
	default:
		return "", nil, fmt.Errorf("invalid infix")
	}
	...
	for _, s := range rangeIDSuffixDict {
		if strings.HasPrefix(input, s.name) {
			...
			if s.upFunc != nil {
				remainder, key = s.upFunc(roachpb.RangeID(rangeID), input)
				return
			}
	...
	return "", nil, &errUglifyUnsupported{errors.New("TODO")}
}
...
func sequenceCacheKeyParse(rangeID roachpb.RangeID, input string) (string, roachpb.Key) {
	var err error
	input = mustUnshiftSlash(input)
	_, input = mustUnshift(input[:len(input)-1])
	id, err := uuid.FromString(input)
	if err != nil || len(input) != 36 {
		if err == nil {
		...
		if entries != nil {
			for _, v := range entries {
				if strings.HasPrefix(input, v.name) {
					...
					remainder, key, err := v.upFunc(input)
					if err != nil {
						return mkErr(err)
					}
					input = remainder
