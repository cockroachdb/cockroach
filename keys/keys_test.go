...
func TestKeyAddress(t *testing.T) {
	testCases := []struct {
		...
		{TransactionKey(TransactionKey(roachpb.Key("doubleBaz"), uuid.NewV4()), uuid.NewV4()), roachpb.RKey("doubleBaz")},
		{nil, nil},
	}
	for i, test := range testCases {
		keyAddr, err := Addr(test.key)
		if err != nil {
			t.Errorf("%d: %v", i, err)
		}
		if !keyAddr.Equal(test.expAddress) {
