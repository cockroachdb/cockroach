	...
	for regexp, keyList := range testCases {
		for _, key := range keyList {
			if addr, err := Addr(key); err == nil {
				t.Errorf("%d: expected addressing key %q to throw error, but it returned address %q",
					i, key, addr)
			} else if !testutils.IsError(err, regexp) {
				t.Errorf("%d: expected addressing key %q to throw error matching %s, but got error %s",
					i, key, regexp, err)
			}
		}
	}
