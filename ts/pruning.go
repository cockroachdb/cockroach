...
// PruneTimeSeries will prune old data for any time series found in the supplied
// key range. This method is supplied with both an Engine and a KV client.
//
...
func findTimeSeries(
	...
	// Search for unique time series names in the engine, constrained to the
	// supplied start/end range.
	...
	for iter.Seek(next); iter.Valid() && iter.Less(end); iter.Seek(next) {
		...
		// If the found key is not a time series key, this means that we are at
		// the end of time series data.
		if !IsDataKey(foundKey) {
			break
		}

		// Extract the name and resolution from the discovered key.
		name, _, r, _, err := DecodeDataKey(foundKey)
		if err != nil {
			return nil, err
		}
		...
		next = engine.MakeMVCCMetadataKey(MakeDataKeySeriesPrefix(name, r).PrefixEnd())
	}
