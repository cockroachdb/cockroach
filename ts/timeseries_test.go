...
func TestNegativeMax(t *testing.T) {
	...
	// regression: The negative number was only improperly set if a sample with
	// an explicitly set negative max value is merged (it did not occur when
	// individual samples were merged, because a sample with count=1 does not
	// have an explicitly set max). Therefore, it could only happen if a sample
	// is merged twice.
