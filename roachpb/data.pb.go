...
type ModifiedSpanTrigger struct {
	...
	// span is set to a single key when node's update their liveness records
	// with heartbeats to extend the expiration timestamp. For range lease
	// updates, the span is set to the entire node liveness key range.
