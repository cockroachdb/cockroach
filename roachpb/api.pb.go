...
const (
	CHECKSUM_SUCCESS ChecksumStatus = 0
	// The requested checksumming version was not available.
	CHECKSUM_BAD_VERSION ChecksumStatus = 1
	// An error occurred while computing the checksum.
	CHECKSUM_COMPUTATION_ERROR ChecksumStatus = 2
	// The checksum didn't match the checksum from the range leader.
	CHECKSUM_MISMATCH ChecksumStatus = 3
	// The computed checksum has already been GCed.
	CHECKSUM_GCED ChecksumStatus = 4
)

var ChecksumStatus_name = map[int32]string{
	0: "CHECKSUM_SUCCESS",
	1: "CHECKSUM_BAD_VERSION",
	2: "CHECKSUM_COMPUTATION_ERROR",
	3: "CHECKSUM_MISMATCH",
	4: "CHECKSUM_GCED",
}
var ChecksumStatus_value = map[string]int32{
	"CHECKSUM_SUCCESS":           0,
	"CHECKSUM_BAD_VERSION":       1,
	"CHECKSUM_COMPUTATION_ERROR": 2,
	"CHECKSUM_MISMATCH":          3,
	"CHECKSUM_GCED":              4,
}
...
func (x ChecksumStatus) String() string {
	return proto.EnumName(ChecksumStatus_name, int32(x))
}
func (x *ChecksumStatus) UnmarshalJSON(data []byte) error {
	value, err := proto.UnmarshalJSONEnum(ChecksumStatus_value, data, "ChecksumStatus")
	if err != nil {
		return err
	}
	*x = ChecksumStatus(value)
	return nil
}
...
// 1. A checksum couldn't be computed on one of the range leaders.
// 2. An error from ComputeChecksum or VerifyChecksum for one of the ranges.
...
type ComputeChecksumRequest struct {
	...
	ChecksumID ChecksumID `protobuf:"bytes,3,opt,name=checksum_id,customtype=ChecksumID" json:"checksum_id"`
}
...
// A status returned in the response can only be CHECKSUM_SUCCESS
// and CHECKSUM_BAD_VERSION.
type ComputeChecksumResponse struct {
	ResponseHeader `protobuf:"bytes,1,opt,name=header,embedded=header" json:"header"`
	Status         ChecksumStatus `protobuf:"varint,2,opt,name=status,enum=cockroach.roachpb.ChecksumStatus" json:"status"`
}
...
type VerifyChecksumRequest struct {
	...
	// The verify is allowed to panic if the checksum doesn't match
	// the computed checksum.
	CanPanic bool `protobuf:"varint,5,opt,name=can_panic" json:"can_panic"`
}
...
func init() {
	...
	proto.RegisterEnum("cockroach.roachpb.ChecksumStatus", ChecksumStatus_name, ChecksumStatus_value)
}
	...
	for iNdEx < l {
		...
		case 2:
			...
			for shift := uint(0); ; shift += 7 {
				...
				b := data[iNdEx]
				iNdEx++
				m.Status |= (ChecksumStatus(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
...
func (m *VerifyChecksumRequest) Unmarshal(data []byte) error {
	...
	for iNdEx < l {
		...
		case 5:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field CanPanic", wireType)
			}
			var v int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
