...
// appendZeros is equivalent to append(appendTo, make([]byte, n)), but without
// the extra allocation.
func appendZeros(appendTo []byte, n int) []byte {
	if len(appendTo)+n <= cap(appendTo) {
		return appendTo[:len(appendTo)+n]
	}
	newSize := len(appendTo) * 2
	if len(appendTo)+n > newSize {
		newSize = len(appendTo) + n
	}
	ret := make([]byte, newSize)
	copy(ret, appendTo)
	return ret[:len(appendTo)+n]
}
...
func EncodeDecimalValue(appendTo []byte, d *inf.Dec, delimited bool) []byte {
	if delimited {
		...
		// varint.
		i := len(appendTo)
		appendTo = appendZeros(appendTo, binary.MaxVarintLen64)
		appendTo = encoding.EncodeNonsortingDecimal(appendTo, d)
		decLen := len(appendTo) - i - binary.MaxVarintLen64
		decOffset := i + binary.MaxVarintLen64
		varintLen := binary.PutVarint(appendTo[i:decOffset], int64(decLen))
		copy(appendTo[i+varintLen:i+varintLen+decLen], appendTo[decOffset:decOffset+decLen])
		return appendTo[:i+varintLen+decLen]
	}
...
func DecodeBytesValue(b []byte) ([]byte, []byte, error) {
	...
	return nil, nil, fmt.Errorf("value type is not %s or %s: %s", ValueType_BYTES, ValueType_DELIMITED_BYTES, tag)
}
...
func getVarintLen(b []byte) (int, int, error) {
	i, n := binary.Varint(b)
	if n <= 0 {
