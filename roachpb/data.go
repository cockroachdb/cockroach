...
func (t *Transaction) Update(o *Transaction) {
	...
	if len(o.MaxTimestamps) > 0 {
		if t.MaxTimestamps == nil {
			t.MaxTimestamps = make(map[NodeID]Timestamp)
		}
		for k, v := range o.MaxTimestamps {
			t.MaxTimestamps[k] = v
		}
	}
...
// the lowest timestamp prevails. Calls with timestamps above t.MaxTimestamp
// have no effect.
func (t *Transaction) UpdateMaxTimestamp(nodeID NodeID, maxTS Timestamp) {
	if !maxTS.Less(t.MaxTimestamp) {
		return
	}
	if t.MaxTimestamps == nil {
		t.MaxTimestamps = make(map[NodeID]Timestamp)
	}
	if ts, ok := t.MaxTimestamps[nodeID]; !ok || maxTS.Less(ts) {
		t.MaxTimestamps[nodeID] = maxTS
	}
}
