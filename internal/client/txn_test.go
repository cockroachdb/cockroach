...
func TestTransactionKeyNotChangedInRestart(t *testing.T) {
	...
	db := NewDB(newTestSender(nil, func(ba roachpb.BatchRequest) (*roachpb.BatchResponse, *roachpb.Error) {
		...
		// In the frist try, the transaction key is the key of the first write command. Before the
		...
		case expectedTries:
			expectedKey = txnKey
		default:
			t.Fatalf("No. %d try is being made. At most %d tries can be made", tries, expectedTries)
		}
	...
	if tries != expectedTries {
		t.Errorf("expected try count %d, got %d", expectedTries, tries)
	}
}
