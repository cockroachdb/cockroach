...
Determine `COCKROACH_RANDOM_SEED` and other environment variables from the
beginning of `cockroach.log`:
```bash
grep -C5 "COCKROACH_RANDOM_SEED" cockroach.log
```
...
```bash
COCKROACH_RANDOM_SEED=<seed> <other env vars> ./cockroach demo --multitenant=false --nodes=9 --insecure --set=errexit=false --no-example-database --format=tsv -f <full-sql-log-file>
```
