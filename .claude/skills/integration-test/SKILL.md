...
description: Guidelines for setting up and running integration tests with CockroachDB test servers, including when to use them and how to use sqlutils.
---

# Integration Test Guidelines

Integration tests use real database servers. They're **SLOW** to spin up (can add seconds per test case), so use them only when necessary.
...
**Always prefer** `sqlutils.MakeSQLRunner` over raw database connections.
...
- `QueryRow` - **Preferred** when reading data to use later in test logic (IDs, counts, values). Preserves datatypes and enables clean scanning into typed variables.
- `QueryStr` - Use when you need all results as strings for manipulation or comparison.
- `CheckQueryResults` - Use for validating exact query output matches expected strings.
...
**Bad:**
```go
// DON'T DO THIS - spinning up server for each case is SLOW
for _, tc := range tests {
    t.Run(tc.name, func(t *testing.T) {
        s := serverutils.StartServerOnly(t, base.TestServerArgs{}) // SLOW!
        defer s.Stopper().Stop(context.Background())
        // ...
    })
}
...
for _, tc := range tests {
    t.Run(tc.name, func(t *testing.T) {
        ...
        defer sqlDB.Exec(t, fmt.Sprintf("DROP TABLE %s", tableName))
        // Run test case
    })
}
```
...
**Key principles:**
- Only use integration tests when necessary (prefer unit tests)
- Spin up server ONCE, reuse for all test cases
- Always use `sqlutils.MakeSQLRunner`
- Each test case must still be independent
- Use unique table names or cleanup between cases for independence

**Common operations:**
- `sqlDB.Exec(t, sql)` - Execute statement
- `sqlDB.QueryRow(t, sql).Scan(&var)` - Query single value to use in test logic (preferred)
- `sqlDB.QueryStr(t, sql)` - Query and return results as strings
- `sqlDB.CheckQueryResults(t, sql, expected)` - Query and verify exact output
