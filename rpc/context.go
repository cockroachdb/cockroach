...
func NewContext(baseCtx *base.Context, hlcClock *hlc.Clock, stopper *stop.Stopper) *Context {
	...
	ctx.dialCtx, ctx.dialCancel = context.WithCancel(context.Background())
	ctx.Stopper = stopper
...
func (ctx *Context) removeConnLocked(key string, conn *grpc.ClientConn) {
	if log.V(1) {
		log.Infof(ctx.dialCtx, "closing %s", key)
	}
	if conn != nil {
		if err := conn.Close(); err != nil && !grpcutil.IsClosedConnection(err) {
			if log.V(1) {
				log.Errorf(ctx.dialCtx, "failed to close client connection: %s", err)
			}
		}
	}
...
func (ctx *Context) GRPCDial(target string, opts ...grpc.DialOption) (*grpc.ClientConn, error) {
	...
	meta.Do(func() {
		...
		if log.V(1) {
			log.Infof(ctx.dialCtx, "dialing %s", target)
		}
		meta.conn, meta.err = grpc.DialWithContext(ctx.dialCtx, target, dialOpts...)
		if meta.err == nil {
			if ctx.Stopper.RunTask(func() {
				ctx.Stopper.RunWorker(func() {
					err := ctx.runHeartbeat(meta.conn, target)
					if err != nil && !grpcutil.IsClosedConnection(err) {
						log.Error(ctx.dialCtx, err)
					}
