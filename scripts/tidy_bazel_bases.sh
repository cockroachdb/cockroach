...
if [ ${#KNOWN_OUTPUT_ROOTS[@]} -eq 0 ] 2>/dev/null; then
...
# remove_base removes an output base. Bazel marks some directories read-only
# (rm needs directories to be writable to remove their contents), so we chmod
# first. Users with --experimental_writable_outputs skip the expensive chmod:
# we probe a Go stdlib dir as an O(1) indicator since it is always present in
# CRDB builds and reflects the writable_outputs setting.
remove_base() {
  local skip_chmod=false
  for d in "$1"/execroot/*/bazel-out/*/bin/external/io_bazel_rules_go/stdlib_/pkg; do
    if [ -d "$d" ] && [ -w "$d" ]; then
      skip_chmod=true
    fi
    break
  ...
  if ! $skip_chmod; then
    nice chmod -R u+w -- "$1" 2>/dev/null || true
  fi
  rm -rf -- "$1"
}
