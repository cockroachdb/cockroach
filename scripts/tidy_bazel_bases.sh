...
# (rm needs directories to be writable to remove their contents). We use the
# same approach as bazel clean --expunge_async: find only non-writable
# directories and chmod just those, rather than a blanket chmod -R.
remove_base() {
...
dedupe_base_contents() {
  ...
  # parent directory. Fix non-writable directories the same way as remove_base.
  for base in "${bases[@]}"; do
    nice find "$base" -type d ! -perm -u=rwx -exec chmod -f u=rwx {} + 2>/dev/null || true
  done
