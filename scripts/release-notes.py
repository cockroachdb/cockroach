...
# Analyzing releases.csv and versions.csv for production release/first alpha
...
if first_alpha:
    with open(versions_file, 'a', encoding='utf-8') as wv:
        ...
        version_writer.writerow([current_version, 'N/A', 'N/A', 'N/A', docker_image])

with open(releases_file, 'a', encoding='utf-8') as wr:
    release_writer = csv.writer(wr, delimiter=',', lineterminator='\n',)
    release_writer.writerow([current_release, current_version, release_date_iso, 'false', no_windows, "Production" if prod_release else "Testing", no_source, go_version, until_commit])
...
rn_file = os.path.join(docsdir, "releases", current_release + ".md")

f = open(rn_file, "a")

if not hideheader:
    print("---", file=f)
    print("title: What's New in", current_release, file=f)
    print("toc: true", file=f)
    print("summary: Additions and changes in CockroachDB version", current_release, "since version", previous_release, file=f)
    print("docs_area: releases", file=f)
    print("---", file=f)
    print(file=f)
    print("## " + release_date_pretty, file=f)
    print(file=f)

# Print the release notes sign-up and Downloads section.

if prod_release:
    print("""- For a comprehensive summary of features in """ + current_version + ", see the [" + current_version + " GA release notes](" + current_version + """.0.html).
- To upgrade to """ + current_version + ", see [Upgrade to CockroachDB " + current_version + "](../" + current_version + """/upgrade-cockroach-version.html).
""", file=f)

if not hidedownloads:
    print("""Get future release notes emailed to you:

{% include_cached marketo.html %}""", file=f)
    print(file=f)

    print("""### Downloads

<div><div id="os-tabs" class="filters clearfix">
    <a href="https://binaries.cockroachdb.com/cockroach-""" + current_release + """.linux-amd64.tgz"><button id="linux" class="filter-button" data-scope="linux" data-eventcategory="linux-binary-release-notes">Linux</button></a>
    <a href="https://binaries.cockroachdb.com/cockroach-""" + current_release + """.darwin-10.9-amd64.tgz"><button id="mac" class="filter-button" data-scope="mac" data-eventcategory="mac-binary-release-notes">Mac</button></a>
    <a href="https://binaries.cockroachdb.com/cockroach-""" + current_release + """.windows-6.2-amd64.zip"><button id="windows" class="filter-button" data-scope="windows" data-eventcategory="windows-binary-release-notes">Windows</button></a>
    <a target="_blank" rel="noopener" href="https://github.com/cockroachdb/cockroach/releases/tag/""" + current_release + """"><button id="source" class="filter-button" data-scope="source" data-eventcategory="source-release-notes">Source</button></a
</div></div>
...
~~~shell
$ docker pull """ + docker_image + ":" + current_release + """
~~~
""", file=f)
    print()
...
if len(missing_release_notes) > 0:
    print("#### Changes without release note annotation", file=f)
    print(file=f)
    for item in missing_release_notes:
        authors = ', '.join(str(x) for x in sorted(item['authors']))
        print("- [%(pr)s][%(pr)s] [%(sha)s][%(sha)s] %(title)s" % item, "(%s)" % authors, file=f)
        seenshas.add(item['sha'])
        seenprs.add(item['pr'])
    print(file=f)

# Print the Doc Updates section.
print("### Doc updates", file=f)
print(file=f)
print("{% comment %}Docs team: Please add these manually.{% endcomment %}", file=f)
print(file=f)

# Print the Contributors section.
print("### Contributors", file=f)
print(file=f)
print("This release includes %d merged PR%s by %s author%s." %
      (len(allprs), len(allprs) != 1 and "s" or "",
       len(individual_authors), (len(individual_authors) != 1 and "s" or "")), file=f)

ext_contributors = individual_authors - crdb_folk

notified_authors = sorted(set(ext_contributors) | set(firsttime_contributors))
if len(notified_authors) > 0:
    print("We would like to thank the following contributors from the CockroachDB community:", file=f)
    print(file=f)
    for person in notified_authors:
        ...
        if person in firsttime_contributors:
            ...
            print(" (first-time contributor%s)" % annot, end='', file=f)
        print(file=f)
print(file=f)

# Print the per-author contribution list.
if not hidepercontributor:
    print("### PRs merged by contributors", file=f)
    print(file=f)
    if not hideshas:
    ...
    for group in allgroups:
        ...
        print("- %s:" % ', '.join(a.name for a in sorted(al)), file=f)
        for item in items:
            print(fmt % item, end='', file=f)
            if not hideshas:
                seenshas.add(item['sha'])
            seenprs.add(item['pr'])
            ...
            if ncommits > 1:
                print(" (", end='', file=f)
                print("%d commits" % ncommits, end='', file=f)
                print(")", end='', file=f)
            print(file=f)
        print(file=f)
    print(file=f)
...
for pr in sorted(seenprs):
    print("[%s]: https://github.com/cockroachdb/cockroach/pull/%s" % (pr, pr[1:]), file=f)
for sha in sorted(seenshas):
    print("[%s]: https://github.com/cockroachdb/cockroach/commit/%s" % (sha, sha), file=f)

print("New release note file created: " + rn_file)
