...
case "${cmd}" in
    ...
    retry gcloud compute ssh "${USER}@${INSTANCE}" --command=true

    gcloud compute scp --recurse "build/bootstrap" "${INSTANCE}:bootstrap"
    gcloud compute ssh "${USER}@${INSTANCE}" --ssh-flag="-A" --command="./bootstrap/bootstrap-debian.sh"

    if [[ "$COCKROACH_DEV_LICENSE" ]]; then
      gcloud compute ssh "${USER}@${INSTANCE}" --command="echo COCKROACH_DEV_LICENSE=$COCKROACH_DEV_LICENSE >> ~/.bashrc_bootstrap"
    fi

    # Install automatic shutdown after ten minutes of operation without a
    # logged in user. To disable this, `sudo touch /.active`.
    gcloud compute ssh "${USER}@${INSTANCE}" --command="sudo cp bootstrap/autoshutdown.cron.sh /root/; echo '* * * * * /root/autoshutdown.cron.sh 10' | sudo crontab -i -"
    ...
    retry gcloud compute ssh "${USER}@${INSTANCE}" --command=true || true
    # SSH into the node, since that's probably why we started it.
    gcloud compute ssh "${USER}@${INSTANCE}" --ssh-flag="-A" "$@"
    ;;
    stop)
    ...
    read -r -a arr <<< "$(gcloud compute ssh "${USER}@${INSTANCE}" --dry-run)"
    host="${arr[-1]}"
    unset 'arr[${#arr[@]}-1]'
    ...
    unison "$host" "ssh://${USER}@${INSTANCE}.${CLOUDSDK_COMPUTE_ZONE}.${CLOUDSDK_CORE_PROJECT}/$worker" \
