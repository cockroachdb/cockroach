...
case "${cmd}" in
    ...
    # Also, we need to create a symlink name `~/.ssh/id_rsa` to the key file for the `git` command to work
    ...
    gcloud compute ssh "${NAME}" --command="ln -s ~/.ssh/$(basename ${SSH_KEY_FILE_PATH}) ~/.ssh/id_rsa"
    ...
    # Previously we copied the ssh keys to the gceworker for cloning the managed-service repo
    # Now we need to remove the ssh keys 
    gcloud compute ssh "${NAME}" --command="rm -rf ~/.ssh"
