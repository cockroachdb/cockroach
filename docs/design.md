...
RFC](https://github.com/cockroachdb/cockroach/blob/master/docs/RFCS/distributed_sql.md)
...
the *logical pla* to a *physical plan*; to express what distribution and
...
- `AGGREGATOR` is the one that does "aggregation" in the SQL sense. It groups
  ...
  aggregagated values (i.e. it can use even values that are not ultimately
  outputted).
