...
Now that the lexical analyser knows about all our keywords, we need to teach the
...
Finally, we need to add a production rule for our statment.  Somewhere below the
...
That's it!  Now our parser will recognize the new statement type.  Let's give it
a try.  First, we need to regenerate the file `sql.go`:
...
Make a new file for our statment type: `pkg/sql/parser/frobnicate.go`.  In it,
...
Rebuild the project (don't forget to regenerate the parser) and try again:
...
constrained.  The `application_name` setting can be any aribitrary string, but
