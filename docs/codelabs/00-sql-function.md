...
*Original author: Peter Mattis <<peter@cockroachlabs.com>>*
...
Notice that this a map from `string` to a slice of `Builtin`. The slice is used
to distinguish the "overloads" for a given function. For example, `abs` has an
overload for each numeric type (`float`, `decimal`, and `int`). The type system
takes care of selecting the correct version of a function given the name and the
argument types. To add a new built-in we just have to decide on a name and some
functionality.
