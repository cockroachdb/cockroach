...
to provide stable identifiers that map tenant IDs.
...
- we will want users to distinguish their "application" tenant
  ...
  for operation experience and in documentation to explain
...
Each tenant SQL server will be instantiated upon first use.
...
executor" interface [2] will be available.
...
[2] or its future replacement: the SQL teams are busy designing the
...
- when we implement the pivot migration mentioned above, we'll want to
  ...
  rewiring the which SQL server is connected to which network
  listener.
...
- it will detect which tenant the connection should be routed to:
  ...
  - otherwise, if a TLS client cert is provided in the TLS handshake
    and there is a single *tenant scope* included in the TLS cert [3],
...
- once the tenant is selected, the `net.Conn` object is passed to
  `(*pgwire.Server).serveConn()` as usual.

[3] tenant scopes inside TLS client certs are a feature introduced in
...
For this we propose to introduce the following new SQL syntax:
