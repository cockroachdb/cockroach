...
All timestamps discussions in this document are references to database
timestamps.
...
* `lease.expiration = L + expiration_offset` rounded to microseconds (DTimestamp).
...
We subtract `1us` from the transaction time to protect us from precision loss due to rounding to create a DTimestamp from a database timestamp.
...
this is always true.
3. The transaction commits before the timestamp `hlc.Timestamp(lease.expiration)`
is handed over to a version increment transaction on the cluster.
...
Is the use of timestamp `T` still valid? For a read only transaction it is
certainly true that it can continue on indefinitely with timestamp `T`.
However a transaction that writes can write inconsistent data past the
expiration time. For example, consider a lease acquisition before an index
is added to a table. A transaction uses a timestamp when the index was absent
and attempts to INSERT a row without an index entry for a row in the table
after the lease expires. If it so happens that the index backfill is complete
and the index is added, the index will become inconsistent. The transaction
must commit before the backfill starts!
...
current timestamp on the node that receives the end-transaction and
...
* `lease.expiration = E + expiration_offset` rounded to microseconds (DTimestamp).
* `INSERT INTO system.lease VALUES (<desc.ID>, <desc.version>, <nodeID>, <lease.expiration>)`
* release old lease.
