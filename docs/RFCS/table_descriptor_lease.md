...
Note that while these are all constraints on the timestamp used by a
transaction using a table lease, the transaction itself might not be
...
* For a transaction that writes, it has to write within the read timestamp
cache window to be considered valid.
