...
Note that while most transactions use cached table descriptors provided
by leases, it is perfectly valid to ignore the cached table descriptors
and read the table descriptors directly from the store at the database
timestamp of the transaction. This is done for instance in the implementation
of commands that use `AS OF SYSTEM TIME`
...
loss due to rounding while creating a expiration time of type DTimestamp
from a database timestamp.
...
Note that these are all constraints on the timestamp used by a
transaction using a table lease and have no bearing on when in real
time the transaction executes.
* For a read only transaction it is certainly true that it can continue on
indefinitely into the future using timestamp `T`.
* For a transaction that writes on the other hand, it has to write within
the read timestamp cache window to be considered valid.

We will move to a model where we assign a transaction timestamp at the
start of a transaction or at least before we get to acquiring leases,
allowing us to enforce the timestamp constraints at the time we acquire
a lease. If we can't, we will need to configure a transaction with a
`min-start-time` and `deadline` to allow delayed constraints enforcement.
