...
Note that while most transactions use leased table descriptors, it is
perfectly valid for a transaction to ignore the use of leased table
descriptors and read the table descriptors directly from the store at the
database timestamp of the transaction. This is done for instance in
the implementation of commands that use `AS OF SYSTEM TIME`. Leases
can only be acquired on the last two versions of a table descriptor,
Therefore, a transaction using a timestamp that predates the last two
versions is disallowed from using a lease and must read the table descriptor
directly from the store. We currently support such transactions explicitly
through specifying `AS OF SYSTEM TIME`.

# Detailed design

The design maintains one invariant:
* There can be valid leases on at most the 2 latest versions of a table in
  the cluster at any time. Leases are usually granted on the latest version.
...
A transaction with timestamp `T` using a lease acquired at timestamp `L`
is valid iff all these three conditions are met:
1. `T >= ModificationTime` of the descriptor in the lease to prevent the
use of a descriptor "from the future". Since `L >= ModificationTime` any
timestamp assigned after the lease is guaranteed to be greater than the
`ModificationTime`
...
this is always true.
3. `T < hlc.Timestamp(lease.expiration)` making it valid because the actual
stored table version at timestamp `T` is guaranteed to at most be off by 1.

A transaction that doesn't meet these constraints is restarted.
We will assign a transaction timestamp at the start of a transaction
allowing us to enforce the timestamp constraints at the time we acquire
a lease.

Instead of restarting a transaction when its timestamp is behind the
lease timestamp: `T < ModificationTime`, we can acquire a lease on a previous
version if it's legal.
 
* First read the table descriptor `desc` for the new lease at timestamp
`ModificationTime - 1ns`.

Using a lease acquiring transaction at timestamp `P` do the following:
* `SELECT descriptor FROM system.descriptor WHERE id = <descID>`
* check that the version of the descriptor read is `desc.version + 1`
...
read all the data in thetable, and (via the timestamp cache) prevent any
