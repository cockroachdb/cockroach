...
the implementation of commands that use `AS OF SYSTEM TIME`. In particular,
a lease defines a validity window of timestamps that a transaction using
it must fall within. A transaction with a timestamp falling out of the
validity window of the last two versions of a table descriptor is forced
to read the table descriptor directly from the store.
