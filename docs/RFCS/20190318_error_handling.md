...
Note: [PR
#37121](https://github.com/cockroachdb/cockroach/pull/37121) contains code
...
However the masked error becomes visible when printin the error via `%+v`
...
Note also that the full message is never included in telemetry report
...
See the [reference section on module
`pgcode`](#pgcode-PostgreSQL-error-codes) below for more details.
...
- `assert`, `issuelink`, `hintdetail`, `telemetrykey`, `pgcode`, `sqlutil` provide feature parity with the original `pgerror` package.
  Note however that PostreSQL-specific behavior is encapsulated in packages `pgcode` and `sqlutil`, and the other sub-packages
...
| `pgcode`        | [PostgreSQL error codes](#pgcode-PostgreSQL-error-codes)                                                                                      |
...
| `sqlutil`       | [Convenience API for SQL](#sqlutil-Convenience-API-for-SQL) using the other wrappers                                                          |
...
type can implement, instead of this need to "register" error types?”
...
- [Handling error type/package migrations with markers](#Handling-error-type-package-migrations-with-markers)
...
- A function `ErrorMark()` that either extracts an explicit marker
  introduced via `Mark()`, or provides meaningful markers to arbitrary
  Go error objects (including the base Go error type).
...
The `markers` package ensures two things:

- that the result of applying `ErrorMark()` is preserved across the
  network, even when applied on intermediate nodes on a received error
  when the origin type is not known locally.

- that the equivalence according to `Is()` is preserved across the
  network, including on the intermediate nodes that do not know how to
  decode the error types (this directly follows from the preservation
  of error markers).
...
The current prototype implementation combines at least the following
...
(Note: there is a hazard when packages or types are renamed, see the next section for details.)
...
Another alternative, initially proposed by Bob, was to reduce the
applicability of `errors.If` to *only* errors that have received an
explicit mark, for example a UUID. This way the marks are independent
from the type, the message, the wrappers etc. This was deemed too
restrictive as it would make it impossible to distinguish/recognize
standard errors produced by 3rd party packages, for example
`context.DeadlineExceeded` from inside gRPC.

#### Handling error type/package migrations with markers

Consider the following scenario:

- in v1 of the code, a sentinel error with a custom type is defined in package `foo`.
  The sentinel is called `foo.errSentinel` (and so is its auto-generated marker).
- in v2 of the code, the package `foo` is renamed to `bar`. This changes
  the auto-generated marker to `bar.errSentinel`.
- two networked nodes running v1 and v2 exchange error objects.

How can we ensure that v2 can recognize errors coming from v1 and vice-versa?

Without additional care there are two problematic cases:

- v1 constructs an error, sends it to v2. v2 sees an error with marker
  `foo.errSentinel` come in, which is different from its locally known
  sentinel marker `bar.errSentinel`.
  A naive observation here is that "v2 knows the history" and could
  remember all past markers, and thus handle a special case for the
  known past `foo.` prefix. However...
- what if ... v2 constructs an error, sends it to v1. v1 sees an error
  with marker `bar.errSentinel` come in which it knows nothing about.

What to do in that case? The solution is as follows: *when/if* this
situation ever arises in the future, extend the `EncodeError()`
function in the *new* code to rewrite the marker of errors to their
original value, and extend `DecodeError()` to recognize the "old" markers.

This can be done using a registry API similar to the encoder/decoder
registration already in place. The old code needs not be modified, and
thus the approach remains forward-compatible.
...
// ErrorMark computes a marker for the given error.
// If the error carries an explicit mark (e.g. via Mark() below)
// that is returned, otherwise an identifying string is computed for
// the given error.
...
also used by [`pgcode`](#pgcode-PostgreSQL-error-codes) to automatically derive the "internal error" pg
code.
...
- an "unimplemented error" leaf error type which indicates a feature
  ...
  error but client code should use the [enhanced `Unimplemented`
  constructors from
  `errutil`](#errutil-Convenience-and-compatibility-API) instead.
  Unimplemented errors can be subsequently tested with the
  `IsUnimplementedError` / `HasUnimplementedError` predicates.  This
  type is also used by [`pgcode`](#pgcode-PostgreSQL-error-codes) to auto-generate pg error codes.
...
// This should not be used directly, consider `errutil` instead.
...
The `hintdetail` package generalizes the concepts of "hints" and "error details" defined
by the PostgreSQL error packet:
...
### `pgcode`: PostgreSQL error codes

An example implementation of `pgcode` is provided here:
https://github.com/knz/cockroach/tree/20190425-rfc-exp/pkg/errors/experiment/pgcode

With an API summary here:
https://github.com/knz/cockroach/tree/20190425-rfc-exp/pkg/errors/experiment/pgcode_api.go

Table of contents:

- [Overview](#pgcode-Overview)
- [API summary](#pgcode-API-summary)
...
Two example implementations for the callback arguments to
`GetPGCode()` are also provided; these implementations mirror the
current logic implemented by the `pgerror` package in CockroachDB. In
particular the provided `SimpleComputeDefaultCode()` knows how to
produce `FeatureNotSupportedError` for [unimplemented errors](#issuelink-Issue-tracker-references-and-unimplemented-errors) and
`InternalError` for [assertion failures](#assert-Assertion-failures).
...
| `Unimplemented`                    | `UnimplementedError` + `WithStack`                                                               |
| `Unimplementedf`                   | `UnimplementedError` + `WithStack` + `WithSafeDetails`                                           |
| `UnimplementedWithDepthf`          | `UnimplementedError` + `WithStackDepth` + `WithSafeDetails`                                      |
| `UnimplementedWithIssue`           | `UnimplementedError` + `WithStack` + `WithTelemetryKey`                                          |
| `UnimplementedWithIssuef`          | `UnimplementedError` + `WithStack` + `WithTelemetryKey` + `WithSafeDetails`                      |
| `UnimplementedWithIssueHint`       | `UnimplementedError` + `WithStack` + `WithTelemetryKey` + `WithHint`                             |
| `UnimplementedWithIssueDetail`     | `UnimplementedError` + `WithStack` + `WithTelemetryKey`                                          |
| `UnimplementedWithIssueDetailf`    | `UnimplementedError` + `WithStack` + `WithTelemetryKey` + `WithSafeDetails`                      |
...
### `sqlutil`: Convenience API for SQL


An example implementation of `sqlutil` is provided here:
https://github.com/knz/cockroach/tree/20190425-rfc-exp/pkg/errors/experiment/errutil

Table of contents:

- [Overview](#sqlutil-Overview)
- [API summary](#sqlutil-API-summary)

#### `sqlutil`: Overview

This package extends
[`errutil`](#errutil-Convenience-and-compatibility-API) above with
additional constructors that embed a PostgreSQL error code annotation.

This logic is segregated in a separate package so that client code can
opt into `errutil` even when they are not producing errors for SQL
clients.

`sqlutil` also provides `DangerousStatementf` used inside CockroachDB
when a user attempts to use a sensitive statement and
`sql_safe_updates` is set.
...
func WrapWithCodeDepthf(depth int, err error, code string, format string, args ...interface{}) error
```
