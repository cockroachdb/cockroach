...
Conversely, client code can add additional wrapper types.
...
- add a new wrapper type `withLogTags{cause error, tags ...}`.
...
- alternatively (or complementarily), make the `WithLogTags()`
  constructort copy the log tags into a layer of PostgreSQL detail
...
- add a new wrapper type `withHTTPCode{cause error, code int}`.
...
| `withHTTPCode` | wrapper with a [HTTP result code](#Example-HTTP-error-codes) | `WithHTTPCode`        |
...
- non-encodable leafs are transformed to `fundamental` + `withMark`, then
  transformed into protobuf-encodable messages. On the receiving side,
  the protobuf messages are converted back into `fundamental`/`withMark`.
- non-encodable wrappers are transformed to `unknownWrapper`, then
  transformed into protobuf-encodable messages. On the receiving side,
  the protobuf messages are converted back into `unknownWrapper`.
...
message AnyErrorContainer {
  oneof detail {
    // wrapped_error is one of the error wrappers provided by the library.
    EncodedErrorWrapper wrapped_error = 1;
    
    // roachpb_error is used when encoding a roachpb error.
    roachpb.ErrorDetail roachpb_error = 2;
    
    // pg_error is used when encoding 19.1 and earlier pgerror.Error.
    pgerror.Error pg_error = 3 [(gogoproto.customname) = "PGError"];

    // Additional encodable errors can be listed here.
    // Alternatively, we can consider using proto3's Any type.
...
message ErrorWrapperPayload {
  oneof payload {
    ...
    string hint = 3;
    string detail = 4;
    string telemetry_key = 5;
    pgerror.Error.Source source = 6;
    pgerror.SafeDetailPayload safe_detail = 7;
    string explicit_mark = 8;
    string unknown_wrapper_marker = 9;
    string barrier_error_code = 10;
  }
}
```

Additionally, the `distsql.Error` type is extended with a new field
`full_error` with type `errors.AnyErrorContainer`. See the next
