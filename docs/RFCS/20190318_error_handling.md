...
To help with this, the library provides *error markers* used to extend
...
created automatically using package path, type name and error message
of the provided error, and thus automatically support most of the
standard errors (e.g. `io.EOF`, `context.DeadlineExceeded`, etc).

Additionally, certain error objects may provide a client-customizable
error mark (see the next sub-section).
...
concatenated across [direct causes](#Decorating-existing-errors),
...
fields](#Additional-PostgreSQL-error-fields) across [direct
causes](#Decorating-existing-errors), using a depth-first traversal,
then *de-duplicating* the hints before concatenating the result.
...
is collected through both [direct causes](#Decorating-existing-errors) and
["secondary" error objects](#Caturing-secondary-errors-for-troubleshooting).
...
- [How the markers work](#How-the-markers-work)
...
The *mark* of an error is an opaque string that identifies an error
*object* (i.e. not just its type).
...
See how to use these facilities in the guide section [Error
equivalence and markers](#Error-equivalence-and-markers).
...
Two local error objects behave sanely wrt `Is()` and `Mark()`.
...
By default, the mark of an object is computed (via `ErrorMark()`) using
the fully qualified type name of the error object and its message.

Because `errbase` guarantees the preservation of the
message and type name through the network, even when the actual Go
type is not known on a receiving node, this is sufficient to preserve
`Is()` equivalence across the network even for unknown types.

To support the `Mark()` override, a new wrapper type `withMark` is
introduced with a simple string payload (the explicit mark). This type
is preserved exactly across the network so its effects are preserved
too.
...
after an error change domains. This is the reason why [error barriers](#barriers-Error-barriers) are not sufficient for this use
case.
...
The reason why the wrappde is called "candidate" is because the algorithm
