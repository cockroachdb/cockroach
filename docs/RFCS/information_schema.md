...
querying `information_schema` over using `SHOW` statments, see MySQL's
...
This RFC proposed [above](#standards-considerations) that we could add relevant extension
columns to our `information_schema` tables, as long as they do not conflict with other columns,
