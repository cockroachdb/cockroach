...
We say that two SQL statements _"independent"_ if their execution could be safely
...
It is easy to reason that SELECT statements and other SQL statements that are strictly
...
is easier to reason about by implementors and users. Tracking dependencies at the
...
take forms like SERIAL columns, which cause the ordering of mutations to different rows
in a table, even without an associated filter, to be dependent.
...
It is not completely clear at this point how batched statements will interact with retryable
errors, but we do not believe there will need to be any special handling for this case.
