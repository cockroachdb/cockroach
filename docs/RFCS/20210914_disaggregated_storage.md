...
across nodes when needed. The shared storage is cheaper per byte, and
the fact that it can be shared across nodes allows for cheap
...
availability SLO is lower than block storage currently employed in
CockroachDB deployments. There are various ways to deal with it:
...
- Local-store and shared-store: The current LSM implementation is said
  ...
  These operations are sufficient for how a LSM produces and consumes sstable files.
...
[here](https://github.com/cockroachdb/pebble/blob/master/docs/range_deletions.md). Which
means we can cleanly consider the semantics of the data in an sstable
as purely a function of the key-span-constraint plus file contents
(which is not true for local sstables, where the file bounds in the
manifest must also be utilized).
...
use when accepting writes. So [M, N) should be large enough to last a
long time duration. [0, M) is used for manufacturing seqnums for
foreign shared sstables.
...
- N2 imports the shared sstables for which it has acquired
  ...
  foreign sstables. Additionally, it takes the data returned in the
...
help control costs.  We consider two kinds of caching.
...
- Which node is responsible for caching it: We embed the node number
  of the original creator of the shared sstable in the sstable
  ...
  is used to decide who is responsible for the file.
