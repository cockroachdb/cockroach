...
column based on the set of columns **in the index**.

## Benchmarks 

 The following benchmarks were all run on a 5 node GCE cluster with `n1-standard-8`
 machines.

 We benchmarked the sequential kv workload against the following four schemas:
...
(4) Primary index on `k` and a secondary index on `v`.
```sql
CREATE TABLE kv (
    k INT8 NOT NULL,
    v BYTES NOT NULL,
...
Now, we demonstrate write throughput on the newly-added `kvbench` benchmark, which is a
`tpcc-bench` style benchmark that binary searches for the maximum throughput that can be
sustained while maintaining an average latency less than a provided threshold. We ran
`kvbench` with the aforementioned threshold value configured to `10.0ms`.
...
(6 rows)
```
This means that if the shard value is plumbed down from the client side, performance
...
- **Syntax for changing the number of shards**

Do we want to create new syntax for this? How does this tie in with Solon and Rohan's work
with regards to being able to change primary keys?
...
place, since load-based splitting should take care.