...
There are two main components to the implementation. Both of them will use the dependency walking algorithm described below. The first place that this will be used is in `TablesNeededForFKs` will will have to be expanded. Currently, `TablesNeededForFKs` only fetches the table IDs that need to be checked when an update, delete or insert operation occurs. With the addition of cascading referential constraint, this is no longer sufficient. Instead of just fetching IDs, this function will now also support optionally retrieving the table descriptors themselves. Without the actual table descriptors, it would not be possible to walk the dependency graph and thus impossible to know if there may be yet another cascading action. This will be done iteratively and should be relatively quick. Care will be taken to not slow down non-cascading examples as this codepath is used with all basic operations.

The second location that will require a fairly large change is in the `fkInsertHelper`, `fkDeleteHelper` and `fkUpdateHelper`. These will have to be augmented to be able to perform cascading operations prior to checking that the foreign key constraints are still being held. A new function called `cascadeAll` will be added. In `rowwriter.go`, the `DeleteRow`, `UpdateRow` and `InsertRow` functions will all have to be updated to call this new `cascadeAll` function before calling `checkAll`. This way, all the cascading deletes and updates will happen prior to the foreign key checks, but those checks will still happen after the cascading actions are complete. Note that most cascading actions will launch new row deleters and updaters, so this is indeed recursive and caution must be taken that we limit the depth of that recursion. While it would be possible to convert this to an iterative instead of recursive function, this will require a lot of code refactoring and it breaks the separation of concerns that currently exists between `fk.go` and `rowwriter.go`.

One tradeoff to using cascading actions is that they will always be somewhat slow. This is due to the fact that it is not possible to know beforehand if a cascade is actually required. An index row fetch must be performed on all referenced tables to see if the value being updated/deleted exists. For example, a self-referencing table with an `ON DELETE CASCADE` will have to perform a row fetch for each row being deleted; then again for each row that is cascading the delete; then yet again for each of those new cascading deletes, etc.

That being said, there are plenty of opportunities to speed up the performance of the cascades. Here are a few quick ones:

- Instead of creating a new row deleter or updater with each cascade, and the row fetcher that accompanies them, use a cache of them instead. This will help significantly with self-referencing tables.
- Collect all the foreign key constraint checks and instead of performing them after each cascade operation, save them for after the final cascade has completed, de-duplicate them, and then perform all the validations in a single batch. This has the advantage of also preparing for deferred foreign key constraint checks.
