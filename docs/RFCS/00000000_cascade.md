...
Even without deferrable constraints, it is possible to create a cycle using an `UPDATE`. All row should be deleted.
...
A new `Cascader` struct will be added that will coordinate all cascading operations. The `Cascader` will also act as a cache for `RowDeleters`, `RowUpdaters` and `MultiRowFetchers`. It will contain the following:
...
- a map from table IDs to `MultiRowFetchers` - to be used with `RowDeleters` - there should be at most one per table accessed during the cascade that requires deletes
...
- a map from table IDs to `MultiRowFetchers` - to be used with `RowUpdaters` - there should be at most one per table accessed during the cascade that requires updates
- a double map from table IDs to index IDs to `MultiRowFetchers` - to be used when looking for foreign key references that need to be cascaded - there should be at most one per table per foreign key index accessed during the cascade
...
1. the number rows that have been deleted
1. the number rows that have been updated

By caching all of the `RowDeleters`, `RowUpdaters` and `MultiRowFetchers`, the limiting factor for cascades should not be memory but the current transaction intent limit. However, testing will be required for both very broad (e.g. 1,000,000 tables all referencing one) and very long (e.g. 10,000,000 self-referential rows in a giant linked list) setups.
...
The algorithm to determine which tables are affected by any operation is a graph walk. Each table can be seen as a node in the graph and each foreign key relationship as an edge. What complicates this walk is that the different actions for both `ON UPDATE` and `ON DELETE` determine different stopping conditions. This algorithm will be used twice in practice. The first time to determine which table descriptors need to be fetched, and then again when performing the actual operations. The difference between the two, is that when fetching the table descriptors, one must assume that any relationships may be affected, while when actually performing the operations, only a subset of those tables will actually require changes. The main goal of this algorithm is twofold. First to ensure no table is unaccounted for that could potentially produce an orphaned reference. And secondly, to never fetch any table descriptors which are not part of the operation and have no chance to create orphaned rows.
...
1. While the queue is not empty:
   1. Pop a table and the its changed rows off of the queue.
   1. For each on delete and on update action:
      ...
      1. If any do exist, fetch the required columns needed for deleting or updating the rows that have been affected.
      1. If one isn't cached, create a new `RowDeleter`/`RowUpdater` specifically without creating a new cascader.
      1. Add each row that has been affected to the map of table IDs to rows to check later for referential integrity violations.
      1. Call `DeleteRow`/`UpdateRow` for each change (without triggering another cascade call).
      1. Enqueue the deleted/updated rows and table ID.
1. Call each table's `RowDeleter` and `RowUpdater` foreign key constraint checker for all rows that have been updated to ensure no orphans were created.

Cycle detection is done by performing all delete and update operations in a completed batch and by delaying the checking for foreign key constraints violations. If there is a cycle, the first delete or update, which would leave an orphaned row in isolation, is performed blindly, as are all subsequent cascading operations. Once all the operations are complete, all referential integrity constraints that were touched are then checked. This has the disadvantage that a long running cascade operation that ultimately fails will leave around a large number of intents due to the failed transaction. But it does leverage our transaction itself to store these changes and does not require large amounts of memory.

If it's found that there is a need to limit the number of cascading operations, this can be added easily by adding counters to this function. Testing will be done to ensure that if a limit is required, a reasonable one is added.
...
It might also be useful to create a new type of test that specifically address testing these new relationships. This would be in addition to logic tests and use a new DSL. If needed, this will be added after the basic functionality described in the RFC is already added.
