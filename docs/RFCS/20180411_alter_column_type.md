...
- [Reference-level explanation](#reference-level-explanation)
    ...
    - [Rationale and Alternatives](#rationale-and-alternatives)
        - [Why new keywords?](#why-new-keywords)
...
same manner as supporting colums that are the target of a foreign-key
constraint.
...
The `ALTER TABLE` command is first run with the `SHADOW` option, which
...
`ALTER TABLE` command, replacing `SHADOW` with `COMPLETE` to allow the
...
```
-- Example: Convert a STRING to the 2.0 INET type.

-- 1) Start the conversion, using SHADOW to tell crdb that we want
-- explicit control over when the schema change becomes visible.
-- This command will block until the backfill / conversion is complete.
...
-- Once the above command has returned, we're ready to perform the swap.
-- Any SQL callers will continue to see a STRING column. 
...
-- Any reads that happen-after this command will see the new type, while
-- any reads that happen-before this command will see the old type.

-- 4) Restart new version of the application that expects the INET type.
```

Electing not to use the `SHADOW` option is equivalent to running a
`SHADOW` followed immediately by a `COMPLETE`. We will refer to this as
"auto-complete" mode.
...
* A backfill process is distributed across all nodes in the cluster
  ...
  column and shadow indexes. Ideally, this backfill process will be able
  to bulk-load sstables in such a manner as to preserve any intervening
  writes to KV keys that resulted from updates made while the backfill
  is running.
...
* Validated adds a background job, related plumbing, and makes the
  two-phase change (`SHADOW` and `COMPLETE`) actually do something.
...
An alternate approach to controlling the schema change could be to make
the `ALTER TABLE` command transactional. That is, one could execute the
...
Using transactions are not the right way to control a two-phase schema
change for the following reasons:

* PostgreSQL
  [locks the table](https://www.postgresql.org/docs/current/static/sql-altertable.html)
  when executing a schema change. This is something that's antithetical
  to Cockroach's approach, so we're never going to match pg's behavior
  100%.
* It is conceivable that for a large data set, the amount of time it
  takes to perform a validation or a backfill would exceed the amount of
  time that a single transaction could (or should) be held open for.
* The schema change initiated within a transaction is necessarily going
  to have observable side-effects for writers in order to make a
  cast-type backfill work well in the face of non-trivial amounts of
  existing data to convert. Either we disallow writes to the affected
  column in other transactions or impose the proposed write-duplication
  behavior (with additional value constraints).

The new keywords are optional, and if a caller doesn't use `ALTER TABLE
... SHADOW`, the command will block until the schema change is visible.
This should satisfy most simple use cases.

## Unresolved questions

* Can we easily merge bulk and ad-hoc writes with `AddSSTable` or
  similar? We want to avoid the following scenario:
