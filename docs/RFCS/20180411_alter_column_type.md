...
existing columns, without resorting to a copy-and-rename approach.
These kinds of statements are often used by ORM or other SQL tools or by
users during their prototyping phase.
...
progress.  Changes to primary-key columns are explicitly out of scope
for this RFC.

# Motivation

Schema evolution is an important part of any database with a
non-trivial lifetime.  While it is possible to effectively change a
column's type using a copy-and-rename approach, users expect to be able
to perform in-place changes to a column's type.  Users should also have
an easy way to take advantage of new features in CockroachDB, such as
the `INET` or `JSONB` types.
...
type conversions is described later [TODO] in this RFC.
...
become visible, allowing application downtime to be minimized.
...
completed, the user may issue the same `ALTER TABLE` command, replacing
`DEFERRED` with `COMPLETE` to allow the type change to become visible
once the job has completed.  The `ABORT` option allows an ongoing
type-conversion operation to be canceled.
...
not be supported under this RFC.  It could be possible to support
foreign keys in the future by cascading the type change, but this would
greatly increase the amount of coordination required to effect the total
schema change.
...
the user until it is swapped in.
...
* Nodes that write to the source column will synchronously apply the
  requested type change and duplicate the write to the shadow column
  and indexes.
...
* The table metadata is updated, changing the column name and indexes
  to point at the shadow data, making the original column or indexes
  non-live.
...
In the case where a column-type conversion (e.g. `STRING -> UUID`) is
in process and a user attempts to insert data that is not compatible
with the target type, the `INSERT` should fail with an error message
such as `column user_id is being converted from STRING to UUID.
The value 'foobar' cannot be converted to UUID.`
...
added to avoid IO starvation.  Non-trivial conversions will be
able to operate on a range-by-range basis, which can be checkpointed.

## Allowable conversions

TODO: Big table showing data-conversion matrix, including classification
of trivial, validated, cast, or assisted.
...
The reason for calling out the various cases of
type conversions is that they lend themselves to a phased implementation
approach:
