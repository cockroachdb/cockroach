...
From an end-user perspective, the new HTTP sink is differentiated from the 
...
In conjunction with our current security efforts, HTTP sink will only support
the `https` protocol.
...
The HTTP sink should be configurable through a few different TLS-related query 
params which are already implemented from the Kafka sink, including:

* `ca_cert` - supply a custom base 64-encoded CA cert for server verification
...
HTTP sink, and will likely be added in the future. It will be configured using 
...
Note: the `key_in_value` option is automatically used here to embed the key in 
the message, similar to the cloud storage sink, since webhook payloads don't 
have the key/value structure that Kafka does.
...
Internally, The HTTP sink struct might look something like this:
...
Global ordering will be ensured by flushing prior to emitting resolved 
timestamps, so to enable per-key ordering, we can send several rows on 
different primary keys to the sink at the same time, using different concurrent 
worker goroutines. This is also allowed by the Go http client, 
as it is [goroutine-safe](https://golang.org/src/net/http/client.go#L33).

Efficient per-key ordering can be accomplished by using `N` channels (some odd 
number), each sending requests to a corresponding worker goroutine which will 
perform the POSTs. Each row emitted will be assigned to a goroutine according 
to a hash of its key modulo `N`,  ensuring per-key ordering by assigning rows 
with the same key to the same worker goroutine. 

```
func (s *httpSink) EmitRow (...) error {
    ...
    index = hash(primary key) % N // assign worker via hash
    flushChans[index] <- row // send to worker via channel
    ...
}
```

On the flush side, the number of messages being processed by workers will be
tracked using a counter `inflight`. Flush will wait until `inflight` reaches
zero, then return.
...
5. Roachtests (end-to-end and verify ordering guarantees)
6. Improper options (wrong type, options missing, incompatible options),
   continuation of `TestChangefeedErrors()` in `changefeed_test.go`
7. Jobs table properly redacts credentials
