...
## Optimization: resume tokens

To avoid unnecessary catch-up on already received values, a straightforward
optimization is possible for the common case in which a stream disconnects
because of a replication or lease change (but not a split). Namely,

1. each `Value()` event contains a `resume_token`, which is opaque to the caller
   but totally ordered. It is really a Raft log index of the Raft log entry
...
1. if the Replica serving the new stream has the log index corresponding to
   `resume_token+1` available (i.e. if it hasn't been truncated away), it skips
   the catch-up phase by sending `StreamState` first, and then replays events
   straight from the Raft log. Otherwise, it behaves as if no `resume_token` was
   specified.
1. Whenever the caller receives `StreamState` as the first event on a
   reconnected stream, it knows that it won't be receiving any duplicate values
   (callers can either deduplicate by keeping state or drop intermediate results
   on reconnection; when `resume_token` catch up works frequently, it is better
   to do the latter). If instead it receives an event, it needs to be prepared
