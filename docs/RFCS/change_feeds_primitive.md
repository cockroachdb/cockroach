...
- `SteadyState`: lets the client know that initial catch-up
...
- `Checkpoint(startKey, endKey, timestamp)`: no more events will be emitted for
  `[startKey, endKey)` at timestamps below the event's. This event will only be
  received after the `SteadyState` notification.
