...
- a `SteadyState` notification that lets the client know that initial catch-up
...
- `[startKey, endKey)` was closed at `timestamp`, i.e. no more events will be
  emitted for `[startKey, endKey)` at timestamps below the event's. This event
  will only be received after the `SteadyState` notification.


## Stream semantics

A stream guarantees exactly-once delivery while it is active, but in the event
of a reconnect, the caller must handle duplicates as it will reconnect using the
latest known close notification as its new base timestamp:
...
and that, once a close notification for a timestamp `ts1` has been received, no
events for lower timestamps may be emitted, i.e. the following would be illegal:
...
should ignore close notifications that have no effect (due to having received a
"newer" close notification before).
...
close notification. Duplication of values streamed to clients after its
reconnect can be reduced by strategically closing out a timestamp just before
