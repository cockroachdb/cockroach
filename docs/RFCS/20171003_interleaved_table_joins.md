...
3. the join columns must be exactly the interleave prefix (see future work on [\[3b\]
   Prefix joins](#3b-strict-prefix-joins) and [\[3c\] Subset
...
2. Create a new processor `InterleavedReaderJoiner` and copy most of the logic
   ...
   to do 2 passes for [prefix joins](#3b-prefix-joins) or any
...
are ordered because of the primary key join. This is however may not be the case with
[subset joins](#3c-subset-joins).
...
Prefix joins involves joining on a strict prefix of the primary key of
