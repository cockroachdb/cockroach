...
The Processors implementation phase will be carried out first.
...
for interleaved tables. We currently do no support interleave schema changes
...
1. Currently, one cannot selectively scan all parent rows without scanning
   ...
   - One suggestion by @jordanlewis is to maintain a secondary index that can
     ...
     - The planner will be need to know when to use this secondary index and
       an index join to perfrom these types of scans.
...
pattern e-merges here:
  ...
  entire hierarchy. The bases for the following approaches revolves around the
...
2. For every new `(pk1, pk2)` prefix:
  - `NextRow()` and store parent/child row in their respective buffers until we
    see a new `(pk1, pk2)` prefix
...
3. Scan through the the current `(pk1, pk2)` span again
   - Join each encountered child row with the cached parent rows and emits
...
groked the implications of theta joins for interleaved tables, so I'll leave it
open for now.
