...
both the number of scans&mdash;interleaved parent and child tables are
scanned simultaneously instead of in two separate scans&mdash;and the volume of
inter-node gRPC traffic for the join&mdash;rows routed by [the hash on their
join
...
1. The Planning implementation phase will introduce changes to the logical plan
   ...
   planning](#2-logical-planning--physical-planning).
    ...
    [\[2\] Logical planning > physical planning](#2-logical-planning--physical-planning).
    - Distributed execution engine will physically plan an `InterleaveReader`
      and a `MergeJoiner` processor (along with their routers and flows) when a
      `joinNode` with interleaved join annotations is encountered.
2. The Processors implementation phase will introduce a new processor, the
   ...
   gaurantee we can take advantage of):
    - The `InterleaveReader` will scan the interleaved hierarchy (scoped to
      the relevant key span if there is a filter on the primary key). It can
      be configured with the `Table` and `IndexDescriptor`s of the parent and child rows
      such that it produces `N` output streams for each group of rows. `N = 2` if we
      want to retrieve a parent table's and its child table's rows.
...
1. A new `MultiplexedRowFetcher` that permits `n` `TableDescriptor`s,
   ...
   - Instead of trying to refactor `RowFetcher` and introducing additional
     overhead, it is prudent to separate this `MultiplexedRowFetcher` for
     `InterleaveReader`.
...
[required to prevent
deadlocks](https://github.com/cockroachdb/cockroach/issues/17097). This happens
when `MergeJoiner` tries to [batch all rows from both sides with for the current join column values](https://github.com/cockroachdb/cockroach/blob/70c96175bb5b60bf6d531ab3bcebd2bb723d0bc7/pkg/sql/distsqlrun/stream_merger.go#L37#L45). In the case of a join
on the entire interleave prefix, this is fine since there is at most 1 parent row
for 0+ child rows, thus we would be buffering at most 1 child row. In the case
where we do a [prefix join](3b-prefix-joins), parent rows are separated by
blocks of 0+ child rows, thus we will need to buffer the child rows in order to
retrieve all the parent rows for a given batch.
...
unique primary parent key, which is hard to imagine as a common query. A multi-table
join between parent-child-grandchild is conceivable (see [Multi-table joins](#4-multi-table-joins)).
...
interleaved hierarchy and an `N-input` `InterleaveJoiner` that performs a
merge join on `N` tables.
