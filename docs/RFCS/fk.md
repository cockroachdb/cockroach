...
those columns highly desireable. While some databases encourage but do not
require indexes on both referenced and referencing columns, making them
mandatory makes the implementation easier to reason about, as their presence can
thus be assumed.
...
implementation simpler, since it can assume their presence, while requiring they
be explictly created by operators, avoids any surprising or unexpected changes.
