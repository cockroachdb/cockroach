...
// specific FK relation. For example: if we are deleting rows from a child
// table, after deleting the rows from the child table this interface will be
// used to build the corresponding deletion in the parent table.
type CascadeBuilder interface {
	// Build constructs a cascading query in the parent table. The input is
	// scanned using WithScan with the given WithID; oldValues and newValues
