...
container.
In addition, we include a reserved 0 byte at the beginning of a JSON document
for future versioning considerations (such as the possibility of splitting
JSONB documents up across multiple kv entries).
...
TODO(justin): We could use 2 bits for this, but Postgres uses 3, I think
this is just for simplicity and because 29 bits for length is still plenty,
but I'm not completely sure. [Relevant part of PG](https://github.com/postgres/postgres/blob/90627cf98a8e7d0531789391fd798c9bfcc3bc1a/src/include/utils/jsonb.h#L203-L212).
...
- What should be done about `$STRIDE`, should it be used as written here, or
  one of the alternatives outlined in the Postgres comment (fixed value, or
  specified in the container header).
- What value should be used for `$STRIDE`? This will have to be the result of
