...
any field not required by an index is stashed in the `payload` column, which
stores a protobuf that can be evolved per the standard protobuf
...
message JobPayload {
    ...
    uint64 finished = 5;
    uint64 modified = 6;
    repeated uint32 descriptor_id = 7;
...
Next up are three fields to track the timing of the job. The `started` field
tracks when the job switches from `pending` to `running`, and the `finished`
field tracks when the job switches from `running` to its final state of
`succeeded` or `failed`. The `modified` field is updated whenever the job is
updated and can be used to detect when a job has stalled.
...
UPDATE system.jobs
    SET status = 'running',
        payload = {...payload, node_id = 1, started = now(), fraction_completed = 0.0}
    WHERE id = ?
```
...
```sql
UPDATE system.jobs
    SET payload = {...payload, fraction_completed = 0.2442}
    WHERE id = ?
```
