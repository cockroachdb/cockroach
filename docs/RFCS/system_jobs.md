...
The admin UI job status page is expected to display jobs filtered by their
status and ordered by their creation time. To make this query efficient, the
table has a secondary index on `status`. Since `unique_rowid()` puts the
timestamp first, sorting by ID is close enough to sorting by creation time.
...
```sql
UPDATE system.jobs
    SET payload = {...payload, fraction_completed = 0.2442, modified = now()}
    WHERE id = ?
```
...
func maybeAcquireAbandonedJob() (int, JobPayload) {
    ...
    for _, job := range jobs {
        ...
        if payload.lease.expires.Before(time.Now()) {
            payload.lease = &JobLease{NodeID: NODE-ID, Expires: time.Now().Add(time.Minute)}
            if db.Exec(
...
UPDATE system.jobs
    SET status = 'failed',
        payload = {...payload, error = 's3.aws.amazon.com: host unreachable', modified = now()}
    WHERE id = ?
```
