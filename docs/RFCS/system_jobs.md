...
pending schema changes if the gateway node dies, the same is not true for
backups and restores. If the gateway node for a backup or restore job dies, the
...
The `fractionCompleted` field, as expected, starts as `NULL` and is periodically
updated from 0.0 to 1.0 while the job is `running`. Jobs in the `succeeded`
state will always have a `fractionCompleted` of 1, while jobs in the `failed`
state may have any `fractionCompleted` value. This value is stored as a float to
avoid arbitrarily.
...
```sql
SELECT status, fractionCompleted FROM system.jobs WHERE id = ?;
```
...
fractionCompleted = ...` queries to update the progress of running jobs is
somewhat unsatisfying. One wishes to be able to conjure the `fractionCompleted`
...
for a specific job ID (`SELECT status, fractionCompleted WHERE jobId = ?`)
...
- Is `FLOAT` the right choice of data type for the `fractionCompleted` column?
  `DECIMAL(5, 2)` was rejected since it's slower and we don't need the exactness
  it guarantees. `INT` was rejected since it requires us to commit to a fixed
  denominator (e.g. 100 or 1000) for the fraction.
