...
|-------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `QUERY_TIMESERIES(TimeseriesSpans...)`    | Run a timeseries query over the timeseries with the prefixes included in the capability.                                                                                               |
...
| `CHANGE_SYSTEM_TENANT`                    | Set the `SystemTenantID` config key, to configure which tenant is authoritative for `settings`, `tenants` etc. [1]                                                                     |

[1] See section below [Deciding which tenant is responsible for hosting cluster configuration](#deciding-which-tenant-is-responsible-for-hosting-cluster-configuration).
