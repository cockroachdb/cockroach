...
- By default, users accessing the DB Console for the application
  ...
  however an option must exist to revoke this access: in CC Dedicated,
  certain storage-level metrics are not actionable by end-users and
...
registry (as explained below), leavint hose from the 1st registry unchanged.
