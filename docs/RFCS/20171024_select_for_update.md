...
and it would also get us closer to feature parity with Postgres. The easiest way to implement
this in CockroachDB would be by setting row-level "dummy" intents, but another option would be
to set span-level locks.
...
This proposal will implement the semantics that users expect for `FOR UPDATE`: `SELECT ... FOR UPDATE` will lay
intents on all of the rows specified by the `SELECT` until the transaction is committed or aborted.
There will be cases when additional intents are written as well due to the mechanism for setting
intents in CockroachDB, but that will not change the semantics. It could affect performance, though, if
other transactions are blocked due to the additional intents.
...
waiting on intents. Using `FOR UPDATE` will result in laying intents on the rows touched by the `SELECT`
...
will lay intents on the rows of all employees named John Smith at the beginning of the transaction,
...
`SELECT * FROM employees WHERE name like '%Smith' FOR UPDATE;`, CockroachDB will set intents on
...
Another difference relates to transaction priorities. In CockroachDB, if there is a conflict and the
second transaction has a higher priority, the first transaction will be pushed out of the way --
even if it has laid down intents already. This would apply to transactions with `SELECT ... FOR UPDATE`,
just as it would for any other transaction.
...
basic functionality in which clients use `FOR UPDATE` to lay intents on the rows touched by the query.
...
as expected. If `FOR UPDATE` only occurs in a subquery, the rows locked are those touched by the subquery.
For example, `SELECT * FROM employees e, (SELECT * FROM departments FOR UPDATE) d WHERE e.did = d.id`
would only lock rows in the departments table. If the `FOR UPDATE` occurs on the outer `SELECT`, however,
all rows touched by the query will be locked.
...
be precise with setting intents in CockroachDB, since intents are set at the KV layer, and
...
are unchanged if extra intents are set.
...
values that were just read (that's not the most efficient way to do things, but it's
...
There was some discussion about whether we really needed new API calls, but the consensus was
that making it possible to write on `Scan` requests would make debugging a nightmare.

Another change we need to make in the SQL layer applies to the case when a `SELECT FOR UPDATE`
query would only scan a secondary index and not touch the primary key (PK) index. In this case,
we need to implicitly modify the query plan to add a join with the PK index so that intents
are always laid on the PK index. This ensures that the `SELECT FOR UPDATE` query will prevent
...
There are also drawbacks to customers who choose to use this feature. Since we will
likely set intents on more rows than returned by the `SELECT`, customers may see more performance
degradation than expected due to contention, regardless of the isolation level. Postgres also
recommends against using this feature in `SERIALIZABLE` mode, since it is not necessary for correctness,
and can degrade performance by causing disk accesses to set locks. Additionally, although `FOR UPDATE`
can prevent deadlocks if used judiciously (as described above), it can also cause deadlocks if not every transaction
sets intents in the same order. Since `FOR UPDATE` results in transactions setting more intents
for a longer period of time, the chance of collision is higher. The solution
is probably to discourage customers from using `FOR UPDATE` in`SERIALIZABLE` mode unless
they have good reasons to do so.

It's also not clear that this feature is something we should promote for `SNAPSHOT` mode.
...
However, there are a couple of alternative implementations worth considering, specifically 
...
it would avoid the issues of lock precision and poor performance that exist in the proposed solution
of using row-level intents. Furthermore, it would successfully prevent write skew in
...
- Should we stick with row-level intents or implement range-level intents?
- Should we be concerned about locking precision (i.e., set intents on exactly the 
