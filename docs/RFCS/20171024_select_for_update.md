...
It is possible to prevent this this scenario from happening in `SNAPSHOT` mode by using `SELECT ... FOR UPDATE`.
...
transactions will abort after waiting a certain amount of time to acquire a lock. In either case, the
...
(intents in CockroachDB) up front. For example, the above schedule would change to the following:
...
Acquiring all locks (or write intents) up front allows the database to lock rows in a consistent order (even if they are updated
in a different order), thus preventing deadlocks. 
...
can in fact guarantee a consistent ordering of write intents. If `FOR UPDATE` is supported
later in DistSQL, ordering will be more difficult to guarantee.
...
degradation than expected due to contention. Postgres also recommends against using
this feature in `SERIALIZABLE` mode. Although `FOR UPDATE` can prevent deadlocks if used
judiciously (as described above), it can also cause deadlocks if not every transaction
...
However, another alternative is to set an intent on an entire Range if the `SELECT` statement
