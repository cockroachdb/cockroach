...
* `drain-leases` mode: all range leases are transferred away from the node, new
  ...
  draining node will also reject any transfers made by nodes that are not yet
  aware of its state.
...
After the lease is transferred, each replica will wait for its command queue to
drain.
...
* Moving ranges to another node if we're draining the node for decommissioning
  or rejecting a shut down if not doing so would cause Raft groups to drop
  below quorum. This is considered outside the scope of this RFC.
