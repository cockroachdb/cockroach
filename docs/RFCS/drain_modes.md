...
* `drain-leases` mode: all range leases are transferred away from the node, new
  ...
  draining node will reject any range transfers made by nodes that are not yet
  aware of its state.
...
Additionally, both `pgwire.Server` and `server.adminServer` will not create new
sessions once in `draining` mode.
...
context is used to send RPCs, these RPCs will be canceled, resulting in the
propagation of errors back to the Executor and the client. Additionally,
...
The session registry will be polled for the session count up to a timeout of one
second to check for the completion of sessions. Some sessions might keep going
indefinitely despite a cancelled context.
...
to delete all leases from `system.lease` with a `refcount` of 0.
...
into account in `Allocator.TransferLeaseTarget` and `Allocator.AllocateTarget`,
resulting in no leases or ranges being transferred to a node that is known to be
draining. Transfers to a draining node could still happen before the
gossiped `StoreDescriptor` has reached the source of the transfer. A draining
node will decline a snapshot in `HandleSnapshot` if its store is in
`drain-leases` mode. In the case of lease transfers, since they are proposed and
applied as raft commands, there is no way for one replica to stop this process.

Any leases that a node's replicas currently hold will be transferred away using
`AdminTransferLease(target)` where target will be found using
`Allocator.TransferLeaseTarget`. A best effort will be made by the draining
store to avoid unavailability by waiting for 2s for the gossip of the
`StoreDescriptor` to propagate, after which any remaining leases will be
transferred away.
...
* Don't offer a `drainMaxWait` timeout to ongoing transactions. The idea of the
  ...
  forcibly close these connections. It might also make sense to offer users to
...
* Move ranges to another node if we're draining the node for decommissioning or
  ...
  quorum.
* Change the lease transfer mechanism to make it possible for replicas to reject
  lease transfers.
