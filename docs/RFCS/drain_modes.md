...
A `draining` field will be added to `v3conn`. Open pgwire v3 connections will be
kept around in the `pgwire.Server` as a linked list. When entering drain mode,
each `v3conn.draining` will be set to `true`. This will enable special behavior
before and after reading messages from the client. If both `v3conn.draining` is
set to `true` and `v3conn.session.TxnState.State` is `NoTxn`, the `v3conn` will
exit the read loop, send an [appropriate error code](#note-on-closing-sql-connections),
and close the connection, thereby denying the reading and execution of
statements that aren't part of a transaction.
...
clients):
* Blocked on the read loop with no active transaction.
* Blocked on the read loop with an active transaction.
* Not blocked on the read loop.

The first class will be taken care of by a first pass that checks for no active
transaction (`v3conn.session.TxnState.State == NoTxn`) and closing the
connection if so.
...
not block on the read loop and will instead exit. Once `drainMaxWait` has
elapsed, the context of any active session (note that this includes any
non-pgwire sessions) found in the session registry
([#10317](https://github.com/cockroachdb/cockroach/pull/10317/)) will be
canceled. Since a derived context is used to send RPCs, these RPCs will be
canceled. Additionally, plumbing will have to be added to important local work
done on nodes so that this context cancellation leads to the interruption of
this work.
...
`AdminTransferLease(target)` where target will be the first `StoreID` found in
`replica.mu.state.Desc` not equal to the `StoreID` currently draining.
