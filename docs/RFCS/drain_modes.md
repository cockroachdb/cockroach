...
which if any lease's `refcount` is decremented to 0, it will be deleted from
`system.lease`.

If sessions are still ongoing after this point, a log message would warn of this
fact.
...
gossiped `StoreDescriptor` has reached the source of the transfer.

A draining node will decline a snapshot in `HandleSnapshot` if its store is in
`drain-leases` mode.

In the case of lease transfers, since they are proposed and applied as raft
commands, there is no way for the recipient to reject a lease. However, any
leases received while draining will be immediately transferred away. A
`draining` field will be added to the `TransferLeaseRequest` when draining that
will notify the transferrer that it should update its version of the
`StoreDescriptor` that it has for the draining node.
...
store to avoid unavailability by waiting for 10s for the gossip of the
...
draining node will wait for its replica's command queues to drain up to a
