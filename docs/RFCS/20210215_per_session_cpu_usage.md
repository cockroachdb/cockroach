...
(NB: this is just a proof of concept. We could have created a
function `GetInternalTaskGroupMetric(TaskGroup, string)` instead which
retrieves the scheduler ticks when passed the `schedticks` label as argument;
this is the API pattern promoted by [Go 1.16's new `metrics` package](https://tip.golang.org/pkg/runtime/metrics/).)
...
- we add a new field `lastsched` in the `g` struct.
- we add a new field `cputime` in the `taskGroupCtx`
- upon every scheduling event:
  - we take `lastsched` from the current/past `g` (the one that is being scheduled away from)
  - we measure the current time with `nanotime()` or `walltime()`.
  - we add the delta between current time and `lastsched` to `taskGroupCtx.cputime`
  - we store the current time in `lastsched` of the new `g` (the one that is being scheduled into).
