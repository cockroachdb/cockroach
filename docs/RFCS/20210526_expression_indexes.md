...
In order to satisfy these requirements, a new boolean `Invisible` field will be
added to column descriptors. An invisible column:
...
4. _Can_ be referenced in queries.*

*We intentionally allow references to invisible columns in queries. Preventing
queries from referencing these columns would add significant complexity to
column resolution within the optimizer for little benefit. If a user discovers
the name of an invisible column and references it in a query, the query will
execute without adverse effects. The biggest risk is that a user writes
application code that depends on an invisible column, which later breaks when
the invisible columns is automatically dropped by the system (e.g., when an
expression index is dropped - see "Dropping and expression index").

To be clear, the invisibility of a column only affects the logic of statements
that show table information, like `SHOW CREATE TABLE`, and DDL statements, like
`CREATE INDEX` or `ALTER TABLE`. A column's invisibility status does not
propagate to the optimizer or execution engine.
...
When an expression index is created, a _hidden, invisible virtual column_ is
...
conflicts when there is more than one expression index column in the table. The
column is hidden so that it does not appear in `*` expansion.
...
When an index is dropped, any invisible indexed columns are dropped. This is
safe because it is guaranteed that an invisible column is not referenced
elsewhere in the table's descriptor.

The invisibility of a column allows us to differentiate between a hidden virtual
column created for an expression based index (which we want to drop) and a
hidden virtual column created manually by a user (which we do not want to drop).
...
-- Dropping expr_idx drops the invisible crdb_internal_idx_expr column.
DROP INDEX expr_idx;
...
When displaying an expression index to a user, any invisible virtual computed
columns in the index are displayed as the expression they represent, rather than
being displayed as their column name.

The invisibility of a column is again used to differentiate between a virtual
...
Multiple alternatives were explored, including some where the `Invisible` column
field were not necessary. Ultimately, these designs either failed to meet the
primary requirements, or did not closely match the behavior of Postgres.
