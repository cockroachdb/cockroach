...
A second table, `crdb_internal.ranges_cached` has the same schema; but the data
it returns is from the range and lease holder caches. Specifically: `replicas`
is from the range cache, `lease_holder` is from the lease holder cache. Only
...
other cache has multiple corresponding spans.
