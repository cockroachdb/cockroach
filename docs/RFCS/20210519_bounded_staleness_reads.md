...
bounded staleness reads, the query timestamp is chosen dynamically and the least
stale timestamp that can be served locally is used.

# Technical design

The design of bounded staleness reads will be broadly similar to their design in
Google's Spanner (see [prior art](#google-spanner)).
...
requires knowledge of which rows will be read, so the timestamp selection cannot
be performed before the SQL gateway has seen the entire transaction.
...
At a high-level, the KV client will expose a new `kv.BoundedStalenessSender`
interface, exposed on a `kv.DB`, for upper levels to interact with bounded
staleness reads. This interface will provide two methods.

First, it will embed the `kv.Sender` interface, so it will expose a `Send`
method which accepts a non-transactional, read-only `BatchRequest` with a
`min_timestamp_bound` set in its `Header`. This `Sender` implementation will
issue the `BatchRequest` while providing the [bounded staleness
guarantee](#guarantees).

Second, it will provide access to a `BoundedStalenessNegotiator` instance,
which is capable of determining the `local_resolved_timestamp` for a set
of spans.

Together, the `BoundedStalenessSender` will looks like:
```go
// BoundedStalenessSender is a specialized Sender that is capable of
// orchestrating bounded-staleness reads, given non-transactional, read-only
// BatchRequests which set a min_timestamp_bound in their Header.
type BoundedStalenessSender interface {
    Sender
    Negotiator() BoundedStalenessNegotiator
}
...
To support the `BoundedStalenessSender` implementation, we will make three
changes to the KV Protobuf API.
...
### Implementation of BoundedStalenessSender

The `BoundedStalenessSender` is responsible for orchestrating bounded-staleness
reads, given non-transactional, read-only `BatchRequests`. It does so by breaking
`BatchRequests` apart into two phases - _negotiation_ and _execution_.
...
`min_timestamp_bound` itself to perform the scan.
...
issues the request through its wrapped sender.

#### BatchRequest preconditions

A BatchRequest will be rejected by the `BoundedStalenessSender` unless it meets
all the following conditions:
- `Header.Timestamp` is not empty XOR
...
2. it provides stronger guarantees around minimizing staleness during bounded
   ...
   to provide cheaply. Additionally, this appears to be exactly the (undocumented)
   behavior in [Google's Cloud Spanner](#google-spanner) when using bounded staleness
   timestamp bounds.
...
To allow the `BoundedStalenessSender` to optimistically hit the server-side
...
minimizing staleness](#minimizing-staleness). It also appears to be exactly the
(undocumented) behavior in [Google's Cloud Spanner](#google-spanner) when using
bounded staleness timestamp bounds and issuing reads across multiple "splits".
...
policy so that even without an asynchronous refresh mechanism, we can still
place some kind of staleness bound (for lack of a better word) on resolved
...
`BoundedStalenessSender.Send` will look something like:
```
...
-- set batch timestamp to local_resolved_timestamp
- else
-- set batch timestamp to min_timestamp_bound
...
This section discusses the implementation of bounded staleness reads from the
perspective of the SQL layer, which uses the `kv.BoundedStalenessSender` to
navigate the process of evaluating a bounded staleness read.
...
staleness parallels to:
- the `default_transaction_use_follower_reads` session variable / connection param
- `SESSION CHARACTERISTICS AS TRANSACTION AS OF SYSTEM TIME follower_read_timestamp()`

### SQL Execution

SQL execution is in charge of using the `kv.BoundedStalenessSender` to execute a
bounded staleness read. It does so in one of two ways. Either it passes the
entire statement's batch of KV gets/scans through the `Sender` all at once along
...
for negotiating the query timestamp, which enables the [server-side negotiation
fast-path](#server-side-negotiation-fast-path). This will be the only approach
supported in step 1 and step 2 of the implementation.

Once we reach step 3, we will add support for multi-scan queries. This requires
SQL Execution to use the `BoundedStalenessNegotiator` and the
`min_timestamp_bound` to negotiate the query timestamp ahead of evaluation. The
query timestamp can then be attached to each `BatchRequest` that is later handed
to the `BoundedStalenessSender`. Since the query timestamp has already been
negotiated, the BatchRequests will have empty `min_timestamp_bound` fields.
...
bounded staleness trasanction. There has been some interesting recent discussion
[on Slack](https://cockroachlabs.slack.com/archives/C876WUAFM/p1621680234002600?thread_ts=1621680187.002500&cid=C876WUAFM)
about using NOTICE messages to inform clients of transaction timestamps.
...
transformation rule, to start we will err on the side of allowing fewer queries
rather than more queries. If there is a need to support more queries, these
checks can easily be relaxed later. If any of these checks fail, the optbuilder
...
index joins. Similar to TryPlaceholderFastPath, we can also add an explicit
check in the execbuilder that the ScanExpr scans at most one row.
...
descriptors. There is precedent for this — it appears to be what [Google's Cloud
Spanner](#google-spanner) does. However, this does lead to a period of
...
staleness reads as well. These have the benefit of being more explicit and
parallel the timestamp bound functions in Spanner.
...
with a valid enterprise license.
...
simply does not seem to be a motivating use case for stale reads.
