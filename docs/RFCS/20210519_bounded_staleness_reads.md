...
bound, to read from local replicas while minimizing data staleness. They
...
replica - it does not have to be local to the region, but avoids the use of
the leaseholder.
...
For `nearest_only` reads, there is a chance of schema unavailability if the
`system.descriptor` table's is not available nearby and we don't have a lease
on the table - see [Table Schemas](#table-schemas).
...
unavailability because it places a floor on staleness. This can similarly
occur if we have no lease on the table descriptor and we do not
have a copy of the system.descriptors table nearby.
