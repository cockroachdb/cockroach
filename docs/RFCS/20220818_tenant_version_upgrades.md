...
will simplify conversations around upgrades and increase
...
- it's trivially possible to violate invariant D, because we do not implement
  [the version interlock described above](#enforcing-the-invariants-during-upgrades-in-single-tenancy) in secondary tenants.
...
interlock](#enforcing-the-invariants-during-upgrades-in-single-tenancy)
