...
- on the storage cluster:
  ...
  B) storage SQL+KV layers must observe max 1 version difference for SLV.
...
- on each tenant:
  ...
  D) all SQL servers for tenants must observe max 1 version difference for TLV.
...
E) the storage SLV must be greater or equal to all the tenants TLVs.
   ...
   be observed in a tenant after the storage cluster have reached X already (e.g.
...
F) any attempt to upgrade the TLV is blocked (with an upper bound) by
   ...
   (i.e. if a user tries to upgrade their tenant from 20.1 to 20.2, but
...
- it's trivially possible to violate invariant D, because we do not implement
  ...
  feature sets (or even use system tables incorrectly).
...
- we are missing a guardrail against violations of invariants E, F and
  ...
  This is tracked here: https://github.com/cockroachdb/cockroach/issues/66606
