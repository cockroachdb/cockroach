...
## 2. How does the optimizer support this feature?
...
### Unique constraint check will be needed
  - `INSERT [ON CONFLICT DO NOTHING | DO UPDATE SET | ON CONSTRAINT | DISTINCT ON]`
  - `UPSERT`, `UPDATE`
...
An alternative would be to do something with the `computeScanCost` under `pkg/sql/opt/xform/coster.go`.

## 3. Test Cases
By adding a new scan flag in `ScanPrivate`, you will be able to see when the invisible index feature will be disabled under `OptBuilder` testdata.

The invisible index test cases need to exercise these dimensions:
a. `invisible-index-normal`: Check normal invisible indexes that do not involve any FK or unique constraint checks.
- Select
...
b. Invisible index feature is disabled during any FK checks.
- `invisible-index-fkchecks-parent`: When a parent table performs DELETE or UPDATE, FK check on the child table is needed.
- Invisible indexes created on the child table FK columns should still be used.
- Tests with different FK actions including ON DELETE DO NOTHING, ON DELETE CASCADE, ON DELETE SET DEFAULT, ON DELETE SET NULL, and same for ON UPDATE.
- `invisible-index-fkchecks-child`: When a child table performs INSERT, UPSERT, or UPDATE, FK check on the parent table is needed.
- Tests with different insert actions with INSERT ON CONFLICT, DO NOTHING, DO UPDATE SET, ON CONSTRAINT.

c. `invisible-index-unique`: Invisible index feature is disabled during Unique constraint checks.
- Test different insert actions with an invisible unique index: INSERT ON CONFLICT, DO NOTHING, DO UPDATE SET, ON CONSTRAINT.
