...
order of several gigabytes per week) which quickly loses utility as it ages. The
amount of data is a function of the number of nodes and the number of time
series; as more nodes are added (or if later versions of cockroach add
additional time series), the rate of new time series data increases linearly. As
weeks go on, this data can represent a large share (or in the case of idle
...
consume all available disk space, even if no external data is written!
...
1. Find for the first time series key in the range (scan for [ts prefix]). 
2. Deconstruct the key to retrieve its name.
3. Run the rollup operation on all keys in the range: 
...
2. The "rollup" algorithm will be implementing, generating low-resolution data
...
common occurence, it could result in a "trail" of ranges with no data, which
