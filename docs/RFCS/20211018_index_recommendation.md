...
*W* contains the top *x* DML statements ordered by execution count times the statement latency, where *x* must be high enough to ensure that the sum of _statement*execution_count_ is beyond some threshold.
...
- Connect this flow the `EXPLAIN` output, showing the final recommended candidate set.
