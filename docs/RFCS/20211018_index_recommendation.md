...
There are three proposed interfaces, with the first two being mutually exclusive. 

1. When the user wants index recommendations, they run a SQL statement indicating so. Index recommendations will be an additional option in the `EXPLAIN` syntax.
   Using the collected SQL statements, run the “index recommendation” algorithm that takes the SQL statements as input and outputs index recommendations that the user can be prompted to accept or reject.
...
2. There is a new built-in function for index recommendations that takes in no parameters. It generates index recommendations for the current database. Similar to above, the user can view the recommendations, and then accept or reject them.
...
*W* contains the top *x* statements ordered by execution count, where *x* must be high enough to include many query-type statements.
If this is not possible, we return an error to the user stating that there isn't enough information to recommend indexes.
Otherwise, we filter the statements to only include queries and determine each query's corresponding index candidate set. 
...
## Determining the Candidate Set for a Single Query
There are a large number of possible indexes for a given query *(Q)* that uses one or more tables, so we choose a candidate set. This method has been adapted from two papers, cited at the end of the RFC.
Choose *Q's* candidate set as follows:
- Separate attributes that appear in *Q* into 5 categories:
  ...
  - **USED**: Attributes that are referenced anywhere in the query that are not in the above categories.
- Note that in order to access this information, we need to parse the query string and build the canonical expression so that `tree.UnresolvedName` types are resolved.
...
- Inject these indexes as *hypothetical indexes* into the schema and optimize the single query (more information about hypothetical indexes in the following section).
Take every index that was used in the optimal plan and put it in this query's candidate set.
...
This is dependent on the plan chosen by the optimizer. 
...
For each query's candidate set, determine the optimizer cost of *W* if that index subset were to be applied. Choose the query candidate sets with the lowest *Cost(W, X)*.
We must then check for index overlap and remove similar/identical indexes to avoid redundancy.
...
We run the algorithm for each statement in *W*, for each query's candidate set, which is O(cardinality of *W*, squared) time.
...
Another way of mitigating this is by ensuring the sample workload is a meaningful *sample* which is not too large. We do this by limiting the size of the workload when we fetch it. 
...
## References
https://baozhifeng.net/papers/cikm20-IndexRec.pdf
http://www.cs.toronto.edu/~alan/papers/icde00.pdf
