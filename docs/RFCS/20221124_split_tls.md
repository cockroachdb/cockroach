...
symlink of the file named `node.crt` to `server.crt` in the target
...
- In particular:
  - `ca-client.crt` becomes mandatory. There's a one-time symlink of
    `ca.crt` to `ca-client.crt` if `ca-client.crt` does not exist already.
  - `server.crt` becomes mandatory. We also have a one-time symlink from
    `node.crt`.
...
| `server.crt`      | Server cert presented to incoming SQL/RPC non-node clients.                                                                                                        | N/A                              | mandatory (NEW, see explanation below) |
| `ca-server.crt`   | CA signing cert embedded in server response to incoming SQL/RPC non-node clients.                                                                                  | N/A                              | optional                               |
| `ca-client.crt`   | CA used to verify peer certs in incoming non-node client conns.                                                                                                    | N/A                              | mandatory (NEW, see explanation below) |
...
We are changing this to mandate 4 certs per CockroachDB nodes:
...
- `ca-client.crt` (NEW) will be used to authenticate non-node RPC/SQL
  ...
  Note that `ca-client.crt` is needed *even when SQL is configured to
...
auto-created via a symlink from `node.crt`, and `ca-client.crt` will
be auto-created via a symlink from `ca.crt`, for convenience.
...
Additionally, `ca-client.crt` becomes mandatory in each node's certs
