...
In any case, we will change CockroachDB to **stop using `node.crt` to
serve SQL/HTTP/RPC for clients**. For convenience for users who are
upgrading from a previous version, we will offer a one-time automatic
copy of the file named `node.crt` to `server.crt` in the target
...
If the `ui.crt` doesn’t exist, the HTTPS endpoint will fallback to the
new `server.crt`, allowing database operators who don’t need a
separation of those interfaces to use the same certificate for all
three.
...
- In particular:
  - `ca-client.crt` becomes mandatory. There's a one-time copy of
    ca.crt to ca-client.crt if ca-client.crt does not exist already.
  - `server.crt` becomes mandatory. We also have a one-time copy from
    node.crt.
  - `ca-server.crt` is optional. It's possible to use the system trust
    store to validate server.crt.
...
This means that the minimum number of certificates required for a cluster would be:
...
- `ca-clients.crt` (for RPC/SQL client certificate verification)
...
Previously, it was only necessary to configure 1 `node.crt` for each
CockroachDB node, then client certs for RPC/SQL clients as necessary
(with a minimum of 1 `client.root.crt`).

We are changing this to mandate 2 certs per CockroachDB nodes:

- `node.crt` (same as previous) will now be used exclusively for node-node connections.
- `server.crt` (NEW) will be used to present to RPC/SQL clients when they connect.

For clusters migrated from a previous versions, `server.crt` will be
auto-created by copying it from `node.crt`. However, from that point
onwards, automation to renew certs will need to update `server.crt`
explicitly.
...
Additionally, `ca-clients.crt` becomes mandatory in each node's certs
