...
## Locality + Tags vs Tags

We want to maximize the diversity of node locations. However, we don't
neccessarily want to maximize the diversity of all tags. There's no reason by
default to try and have a node set of `[ssd, mem, hdd]`, but there is a good
reason to have `[rack-1, rack-2, rack3]`.

### Locality + Tags

This can either be addressed by having a separate locality flag
`--locality=us/west/us-west-1/12`, which we then try to maximize diversity of.
The ZoneConfig would have a locality constraint field which would let users
limit to whatever level they want.

Once consideration is how to match those tags. You can either attempt to do full
path matches `us/.*`, or just match them using the standard constraint system.
When using the constraint system you could do `[us]` or `[us-west-1, eu-east-2]`.

### Tags

The same type of diversity can be implemented with the above system with a
positive constraint `rack-.*`. This would try to maximize the diversity of the
matched tags, while also being able to limit to certain areas.
