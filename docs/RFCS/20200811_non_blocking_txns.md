...
- non-blocking transactions support a replication scheme over Ranges that they
  operate on which allows all followers in these Ranges to server **consistent**
...
look for other solutions to this problem, such as [duplicated indexes](https://www.cockroachlabs.com/docs/stable/pology-duplicate-indexes.html)
to avoid WAN hops on foreign key checks.

Another hurdle is that the staleness they permit requires buy-in, so accessing
them from SQL [requires application-level changes](https://github.com/cockroachdb/cockroach/blob/master/docs/RFCS/181227_follower_reads_implementation.md).
...
at, there is a strong desire to support [mixed timestamp statements](https://github.com/cockroachdb/cockroach/sues/35712),
...
(up to 100ms), it is safe to say that atomic clocks and TrueTime are not
...
The RFC then introduces the term "consistent read Range", which is a Range in
which any transaction that writes to it is turned into a non-blocking
transaction, if it is not one already. In doing so, these consistent read Ranges
are able to propagate [closed timestamps](20180603_follower_reads.md) in the
future of present time. The effect of this is that all replicas in a consistent
read Range are expected to be able to serve transactionally-consistent reads at
the present time. In this sense, all followers and all learners (non-voting
followers) in a consistent read Range implicitly behave as "consistent read
replicas".

In the reference-level explanation, we will explore how non-blocking
transactions are implemented, how their implementation combines with consistent
read Ranges to provide consistent read replicas, how they interact with other
standard read-only and read-write transactions, and how they interact with each
other.

For now, it is sufficient to say that "non-blocking transactions" are hidden
from DML statements. Instead, users interact with "consistent read Ranges" by
choosing which key ranges should consist of consistent read Range. These Ranges
...
A better solution to this problem would be to use consistent read Ranges for
these tables. This would reduce operational burden, reduce write amplification,
avoid blocking on contention, and increase availability.
...
- Conflicting non-stale readers ignore future writes until they enter their
  ...
  offset = 30ms).
- consistent read Range leaseholders close out timestamps in the future of present
  time.
- consistent read Range followers receive these future-time closed timestamps and
...
perform perform a remote network hop (or several). Depending on the location of
the conflicting write's transaction record, this network hop may be over the WAN
and may cost 100's of ms.
...
Examples:
```
Forward({synth, 5, 10}, {synth, 6, 5}) = {synth, 6, 5}
Forward({real,  5, 10}, {synth, 6, 5}) = {synth, 6, 5}
Forward({synth, 5, 10}, {real,  6, 5}) = {real,  6, 5}
Forward({real,   5, 10}, {real, 6, 5}) = {real,  6, 5}

Forward({synth, 6, 5}, {synth, 6, 5}) = {synth, 6, 5}
Forward({real,  6, 5}, {synth, 6, 5}) = {real,  6, 5}
Forward({synth, 6, 5}, {real,  6, 5}) = {real,  6, 5}
Forward({real,  6, 5}, {real,  6, 5}) = {real,  6, 5}
```
...
HAZARD! txn3 just saw stale read. It was triggered by txn2, which had observed
        the value written by txn1, but it did not observe the value.
```
...
This RFC proposes that "consistent read Ranges" use a separate closed timestamp
...
users directly. Instead, it exposes "consistent read Ranges" to users. It was
also mentioned earlier that a "consistent read Range" will have a closed
...
Any standard transaction that writes to a "consistent read Range" will naturally
...
to other conflicting read-write transactions seems undesirable, it is no more
undesirable in practice than read-write transactions that touch global data an
...
proposal](https://github.com/cockroachdb/cockroach/pull/39758).
...
* Introduce consistent read Range concept
  ...
  * Add to zone configs
* Route present-time reads to followers in consistent read Ranges
...
would monitor active intents on consistent read Ranges and push them and their
