...
```
{
  ...
  "sub":       "1b1569d1-7e9c-486e-1e6a-dfac2b72d8df",
...
| `sub`      | Payload  | string | The subject/user identifier (the CRDB Cloud user ID). Returned only if the scope `openid`  is included in a request.   |
...
be. ID tokens for a given user will always have the same `sub` claim (which specifies the user's
CockroachDB Cloud user ID), so the `sub` claim will be used to reliably identity users.
...
The token will be passed to the server (Base64 encoded) via the connection string
