...
**Make changes to the authentication method selection in order to enable clients to authenticate 
with JWTs**

In order to ensure users can use SSO authentication with the default HBA configuration 
(which the server uses to choose an authentication protocol), a message of intent will be passed 
to the server through the pgurl and will function as signal that the client will pass a JWT. 
The server will process this request prior to making an authentication method selection 
using the HBA config. We are considering supporting one or a few of the following options 
regarding how the message will be included in the pgurl:
* in the hostname passed through SNI (e.g. `postgres://username@jwt.<server>/...`)
* an options payload (e.g. `postgres://....?--options=jwt=true`)
* a suffix/prefix in the database name (e.g. `postgres://.../<dbname>:jwt/`)
* a suffix/prefix in the username (e.g. `postgres://<username>:jwt@....`)

After receiving the request, the server will select the JWT authentication protocol and request a 
cleartext form of authentication with the expectation that it will be a JWT.

**Update SQL authentication code to validate access tokens**

Once the server requests a plaintext password (expecting the form to be a JWT) and the client 
sends the JWT, the server proceeds to verify the token. If the token is valid, authentication 
proceeds as if a correct password were provided.
...
The signing key will rotate periodically, and in the case of an emergency, could be rotated 
immediately. The rotation period will be around once a year. With Intrusion, CC Console will push 
new public keys to CRDB cloud instances. CC Console will sign tokens using 
...
**Create token credential management for CC-hosted service accounts**
...
lifetime for these credentials is 30 days. This UI will be authorized using standard CC Console 
session cookies and will enable the user to:
...
A new API `GenerateTokensService` will generate an access token in exchange for the user's service
token. Service accounts will directly send requests using this API.
...
Request parameters:
* `service_token` the token passed by the CC-rooted service account in exchange for an access token.
...
infrastructure as the identity provider or can use CC. If the customer elects to used
