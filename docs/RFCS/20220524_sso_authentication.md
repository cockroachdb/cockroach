...
| Service            | Used to retrieve an access token.                                         <br/>                                                                 | Service Accounts (rooted in customer's IdP) | signed | many  | long-lived   | 30 days    |
| ID                 | Contains information about the user. Needed to follow the OIDC protocol, which we wish to adhere to for easier integration with DB Console SSO. | All                                         | signed | many  | short-lived  | 60 minutes |
...
token-based authentication including `jwt`, `jwt-password`, `cert-jwt`, and `cert-jwt-password`, 
with the latter being the default rule for users. 
When a client connects, the client may encounter one of these rules, which means that the HBA configuration
...
There will be a heuristic used to distinguish between passwords and JWTs. There will be 
detection of three parts separated by dots (.) within the cleartext. If it 
is detected, the text will be treated as a JWT; otherwise, it will be treated as a password.
...
so that console admins create . The lifetime for these credentials 
is 30 days. This UI 
...
The approach outlined here fully suppors self-hosted customers. If the customer elects to used 
CC as the credential issuer, their instances would need to trust CC. For customers that do not 
want to trust an external entity, we could provide a software microservice they can run in their 
infrastructure that converts their SSO login operations into credentials that CRDB can understand. 


Customers can use their own infrastructure as the identity provider or CC if they wish. 
Customers can use their own credential issuer if they wish as well; we can ensure that the 
expected issuer they can set 
the signing key in 
the same way CC does. 
