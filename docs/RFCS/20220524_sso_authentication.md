...
CC Console's signing key (i.e. private key) will rotate on a periodic basis, and in the case of an 
emergency, could be rotated immediately. An initial validation key (i.e. public key) for CRDB 
will be baked into the image. There will always be more than one validation key available during 
token validation. If CRDB attempts to use expired verification keys to verify the 
signature on a token, CRDB will reject the token. However, the cluster will dynamically refresh its
current key by updating its configuration when (1) the given token has an unknown key ID 
specified in the `kid` Claim and additionally (2) the key has not been recently updated. Assuming 
these conditions are satisfied, a new key will be acquired which will become the
current key, and the old current key will become the previous key, which will be kept around. 
When verifying a token, the server will attempt to use the current key followed by the previous key.
...
There will be slightly different flows depending on if the service account is rooted in (1) the 
