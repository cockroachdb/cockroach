...
emergency, could be rotated immediately. An initial validation key set (i.e. public keys) for CRDB 
...
signature on a token, CRDB will reject the token. CC Console will push new public keys to CRDB 
cloud clusters when the public/private key set rotates. There will also be an option to override default keys with keys of choice.
