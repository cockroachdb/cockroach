...
| `sys_id` | Payload  | string | The identity used to map to the user's SQL username.                          |
...
[OAuth 2.0 Authorization Code Flow](https://oauth.net/2/device-flow/);
...
5. CC Console responds with the token credentials and they are [passed via the
   connection string](#update-sql-authentication-flow-to-identify-and-validate-access-tokens).
...
CC Console will derive a identity from the user's email address, which will be generated 
earlier on at CC user creation time and will be included on the CC user object. When token 
credentials are minted, the identity will be included the credentials and will be mapped to a 
database username via the `server.identity_map.configuration` cluster setting.
Eventually, we will want to support the ability to directly specify database usernames if 
the customer 
wishes, 
but it is not a priority considering that the customer has presumably created these users already.

When an access token is presented to CRDB, token validation will include a check to make sure that
the audience field matches what is expected. The audience field consist of the cluster ID, so that 
...
token requests. It generates the authorization code and either
(1) redirects the user back to the redirect URI along with the code or (2) displays the 
authorization code for the user to copy and paste into the CLI, depending on whether the CLI was
...
The token will be passed to the server via the connection string as an option in the `options`
field  (e.g. `postgres://....?--options=jwt=<jwt>`). The credential can be safely passed in this
way because the TLS handshake occurs before the
pg status parameters are sent by the client.
...
CC Console will sign tokens using
[RS256 (RSA Digital Signature Algorithm with SHA-256)](https://ldapwiki.com/wiki/RS256).
The signing key will rotate once a year, and in the case of an emergency, could be rotated
immediately. With Intrusion, CC Console will push new public keys to CRDB cloud instances.
...
1. CC customers who want to use own infrastructure as the credential issuer.
...
forward it SQL pod which would handle authentication.
