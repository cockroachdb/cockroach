...
from the user's email. Eventually, we will want to make it so that SSO users are not be able to 
modify the permissions associated with this username. To ensure that, the usernames will start with 
a prefix `sso_` in order to protect that namespace from permissions modification and ensure that the 
permissions come only from CC. Eventually, we will also want to add the ability for customers to 
