...
failure). Then, for each missing migration, the migration coordinator runs its
work function and writes a record of the completion to kv entry
...
migration. It has also been suggested that we [use a form of version leases for
this](https://github.com/cockroachdb/cockroach/issues/10212).
