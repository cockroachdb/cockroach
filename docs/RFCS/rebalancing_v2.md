...
outdated information (gossiped `StoreDescriptor`s that are up to a minute old).
...
overloaded with replicas, its descriptor will take up to a minute plus the time to propagate the
descriptor through gossip.
...
- User latency. Rebalancing should never affect query throughput. Measured using latencies of user
  queries.
...
replica appears within that time, it is no longer reserved.
...
1. Too full in terms free disk space (this includes all reserved replica spots)
1. Overloaded (once we define what that term specifically means)
1. Too many current reservation requests
...
reservation is decided in the `ReserveReplica` RPC and not part of `AddReplica`.
...
- `Status` A enum value that indicates if a replica has been reserved or not. Usually this will
  be either a `reserved` or `not reserved`.
...
considered for new allocations for a short period of time (perhaps 1 second).

On a `reserved` response, as long as the requesting replica is still the raft leader, the replica
will issue a `replicaChangeRequest` to add the chosen store.
...
- Don't try to rebalance any other replicas on a store until the previous *ChangeReplicas* call has
  finished adding the replica to the new store. Currently, multiple *ChangeReplicas* requests can be
  simultaneously, by limiting the number of concurrent requests, we can limit the frequency of rebalances.
...
We would decrease the interval for gossiping `StoreDescriptor`s from 60 seconds to 10 seconds and
...
use the latest gossiped `StoreDescriptor`s and its knowledge of the replica allocation decisions
...
suppose that a cluster is storing 1 TiB of de-duplicated data. That is a minimum of 16,384 ranges
each with an assocaited `RangeDescriptor`. Requiring the scanning, sorting and This could easily be a performance problem.
