...
1. Too many current reservation requests (throttling factor will be determined experimentally)
...
- `RangeID` of the requesting range. Consdider repurposing a `ReplicaDescriptor` here.
...
paritally considered a forced gossip update.)
...
On a `reserved` response, as long as the requesting replica is still the range leader, the replica
...
- Don't try to rebalance any other replicas on a store until the previous *ChangeReplicas* call has
  ...
  issued simultaneously, by limiting the number of concurrent requests, we can limit the frequency
  of rebalances.
...
This could be achived using either of two different mechanisms: decreasing the
interval for gossiping `StoreDescriptor`s from 60 seconds to 10 seconds or by
writting the descriptors to a system keyspace and retrieve them, possibly
using an inconsistant read, every 10 seconds. Recall that `StoreDescriptor`
updates are frequent and the allocation leader is the only node making
rebalancing decisions. So, the allocation leader could use the latest gossiped
`StoreDescriptor`s and its knowledge of the replica allocation decisions made
since the last `StoreDescriptor` gossip to derive the current state of replica
...
suppose that a cluster is storing 100 TiB of de-duplicated data. That is a minimum of 16,384,000
ranges each with an associated `RangeDescriptor`. Requiring the scanning, sorting and decision
making based on this large of a collection could be a performance problem.
...
- To make decisions for all ranges, the leader must iterate over the full set of `RangeDescriptors`,
  which could be large and distributed. This may become a performance issue.
