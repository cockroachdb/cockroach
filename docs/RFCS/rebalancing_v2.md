...
- [Detailed Design](#detailed-design)
  ...
  - [Protos: add a timestamp to *StoreDescriptor* and *reservedBytes* to *StoreCapacity*](#protos-add-a-timestamp-to-storedescriptor-and-reservedbytes-to-storecapacity)
...
avoid the excessive thrashing of replicas that results from making rebalancing decisions based on
outdated information (gossiped `StoreDescriptors` that are up to a minute old).
...
allocator and intelligently increasing the frequency at which we gossip `StoreDescriptors`.
These modifications will significantly reduce the time required to rebalance small-to-medium-sized
clusters while avoiding the waste of resources caused by excessive thrashing of replicas.
...
converge on a desirable distribution of replicas with a reasonable number of replica allocations.
...
overloaded with replicas, it will take up to a minute plus the time to propagate the descriptor
through gossip.
...
Rebalancing is a topic of ongoing research and investigation and to this end, in this section, the
goals for this second version of rebalancing are presented.  In the section that immediately
follows it, a collection of future goals are presented for potential post V2 work.
...
- Minimizes thrashing.
  ...
  halt due to the number replicas being moved between nodes.

- Works well in clusters with up to 32 nodes.

  The choice of 32 nodes here matches our OKRs. This limit is to make testing more tractable.

- Works well in a dynamic cluster. A dynamic cluster is one is which nodes can be added and removed
  arbitrarily.
...
- User latency. Rebalancing should never affect query throughput. Measured using queries per second.
...
`yes`, the space for said replica is reserved for a predetermined amount of time. If no replica
appears within that time, it is no longer reserved.
...
When it replies with a `no`, it also includes possible reasons as to why for debugging purposes.
These reasons can include:

1. Too Full (this includes all reserved replica spots)
1. Overloaded (once we define what that term specifically means)
1. Too many requests

Any other error, including networking errors can be considered a `no` response for the purposes of
allocation. When a `no` is received, that response is cached in the store pool until the next
`StoreDescriptor` update. We avoid issuing further `ReserveReplica` calls to that store until the
next `StoreDescriptor` update.
...
`replicaChangeRequest` for a specific rangeID. If the replica for the range is not added within the
reservation timeframe, the reservation is removed and the space becomes free again.

If a `replicaChangeRequest` arrives and there is no reservation, the store will still create the
new replica and this will not cancel any pre-existing reservations. The gating of when to allow a
new reservation is decided in the `ReserveReplica` RPC and not part of the `replicaChangeRequest`.

Optionally, the ability to control the amount of currently available space on a store can be used
...
## Protos: add a timestamp to *StoreDescriptor* and *reservedBytes* to *StoreCapacity*
...
response being a `yes` or a `no`, it will also return updated `StoreDescriptors` for all the stores
on the node. These updated descriptors will be used to update the cached `StoreDescriptor` in the
`StorePool` of the node calling `ReserveReplica`. Any subsequent calls to the allocator will have a
fresher sense of the cluster.

By adding a `reservedSpace` value to capacity, it allows more insight into how the total capacity
of a store is used and be able to make better decisions around it.
...
`StoreDescriptor` for the store in which a reservation is requested.

The request will include:

- `StoreID` of the store in which to reserve the replica space
- `RangeID` of the requesting range
- All other parameters that are required by the allocator, such as required attributes.

The response will include:

- `StoreDescriptors` The most up to date, freshly generated store descriptors for all stores in
  the node. Note that there may be a requirement to limit the number of times `engine.Capacity` is
  called as this is doing a physical walk of the hard drive.
- `Reserved` A boolean value that indicates if a replica has been reserved or not.
...
Typically the response will be a yes. A response of no will only occur when the store is being
overloaded or is close to being overly full. Even when a reservation has been made, there is no
guarantee that the store calling will still fill the reservation. It only means that that space
has been reserved.
...
On a no response, add a note that the store refused and not consider it for allocation for a short
period of time (perhaps 1 second).

On a yes response, if the replica is still the raft leader, the replica will issue a
`replicaChangeRequest` to add the chosen store.
...
- Make the gossiping of `StoreDescriptors` event driven. Anytime a snapshot is applied or a
  replica is garbage collected. If no event occurs, gossip the `storeDescriptor` every
  `gossipStoresInterval`.

  **_This has no foreseable drawbacks and will be included in v2._**

- Decrease the `gossipStoresInterval` to 10 seconds so `StoreDescriptors` are fresher.

  This adds a lot of chun to the gossiped descriptors so the increased network traffic might
  outweigh the benefits of faster rebalancing.

- Move from using gossiped `StoreDescriptors` (updated every 60 seconds) to stored `StoreStatuses`
...
- Don't try to rebalance until the previous *ChangeReplicas* call has finished adding the replica
  to the new store. *ChangeReplicas* does not currently block, but by adding one, we can limit the
  frequency of rebalances.

This limits each store to performing a single *ChangeReplica* at a time. It would limit thrashing
but also greatly increase the time it takes to reach equilibrium.
...
We would decrease the interval for gossiping `StoreDescriptors` from 60 seconds to 10 seconds and
also enable event driven gossiping. Recall that `StoreDescriptor` updates are frequent and the
allocation leader is the only node making rebalancing decisions. So, the allocation leader could
use the latest gossiped `StoreDescriptors` and its knowledge of the replica allocation decisions
made since the last `StoreDescriptor` gossip to derive the current state of replica allocation in
the cluster.
...
The *StoreDescriptor*s are discussed in the previous section. The bolded function warrants specific
attention: it needs to retrieve a potentially large number of range descriptors. For example,
suppose that a cluster is storing 1 TB of de-duplicated data. That is nearly 1.6 million ranges
and, therefore, `RangeDescriptors`. This could easily be a performance problem.
...
- Less thrashing, since the allocator will know not to overload a new node. Distributed,
  independently acting allocators can make decisions that run counter to the others’ decisions.
...
- To make decisions for all ranges, the leader must iterate over the full set of `RangeDescriptors`,
  which could be large and distributed. This could easily become a performance issue.
...
- More internal work needs to be done to support a centralized system.  Be it via an election or
  using the raft leader of a specific key.

***Verdict***

The main issue that causes the thrashing and overloading of stores is lack of current information.
Centralized allocation doesn’t fix that issue; it merely shifts allocation burden to a single node.
Also, the single allocator would have to routinely iterate over the full set of `RangeDescriptors`
for the cluster, likely posing a major performance challenge.
...
rebalancing that adds an affinity for collocating replicas on the same set of stores.  This can be
...
- Much more subtle than copysets.
