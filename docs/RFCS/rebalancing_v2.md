...
is a new type of RPC in which it adresses a store and not a node or range.
...
by writing the descriptors to a system keyspace and retrieving them, possibly using inconsistant
...
A big part of that is the lack of knowledge about allocation decisions that are occuring while
making other decision. A centralized allocator would fix those issues. However, there are
implementation and performance issues that may arrise from moving to a central allocator. Be it the
potential requirement to iterate over a set of `RangeDescriptors`, dealing with performance
concerns of having all reblalncing decicsions made in an expidant manner, or cases in which the
centrailized allocator itself is faulty in some way, make the centralized solution less appealing.
...
- There is a chance that the cluster gets into a less desireable state if not done carefully.
