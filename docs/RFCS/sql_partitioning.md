...
partitioned by one set of columns. Secondary indexes on partitioned tables will
required being explicitly declared as partitioned or unpartitioned.
...
partitioning changes, that is, changing the assignment of some value, say "us" require a individually rewriting all keys in the index.
...
this seamless for users required maintaining the denormalized evaluation of the
...
this could likely be treated as immutable to avoid needing to lease them (all
actual mutable schema information would still live on the real table descriptor).
...
In the prefix encoding, since the table primary key is forced to be `[region,
userid]` to maintain the prefix property, the narrowest bound possible is a full
table scan.

In the Partition IDs proposalt here is more flexibility in the construction of
the table primary key. If it’s reordered to `[user_id, region]`, then this
SELECT will be one tight-bounded lookup per partition e.g. `/51/1/71/*,
/52/1/73/*, ...`.
