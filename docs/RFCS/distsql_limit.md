...
degradation in many cases:
 - DistSQL processors (a processor is roughly one of potentially multiple
   ...
   always read a second batch even
   if we don't need to.
...
read from those ranges first. In widely distributed clusters, we can always
choose among the nodes that are close to the gateway.
...
away, and we set up operations on more nodes than we have to.
