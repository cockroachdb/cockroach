...
WITH RECURSIVE map AS (
  SELECT
    ...
    system.sql_statement_stats.aggregated_ts,
  ...
  WHERE fingerprint_id = $1
    ...
    AND aggregated_ts < $3
  ORDER BY
    (system.sql_statement_stats.aggregated_ts, system.sql_statement_stats.fingerprint_id)
),
reduce AS (
  (
    SELECT
...
* Instead of deleting the oldest stats entries from the system table in the
  ...
  oldest aggregation interval. This is because for any given transaction
...
* We want to have a better policy to retain more statistically significant
  ...
  (TODO: we should probably find the name of this algo)
