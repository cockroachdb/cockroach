...
1. Since the amount of statistics data we collected is limited to one-hour
   period, operators have no way to compare the current statistics to the
   historical statistics in order to understand how has the performance of
   queries changed.
...
1. **Usability**: currently, users have access to only the node-local SQL
  ...
  Console.  Wit persistent SQL statistics, cluster-level SQL statistics are now
  available as system tables. Users will be able to run more complex SQL
  queries on the statistics tables directly through SQL shell.
...
CREATE TABLE system.sql_statement_stats (
    ...
    */
    ...
    PRIMARY KEY (last_run, fingerprint_id, plan_hash, app_name) 
      USING HASH WITH BUCKET_COUNT = 8,
    INDEX (fingerprint_id, last_run, plan_hash, app_name, node_id)
);
...
CREATE TABLE system.sql_transaction_stats (
    ...
    */
    ...
    PRIMARY KEY (last_run, fingerprint_id, app_name) 
      USING HASH WITH BUCKET_COUNT = 8,
    INDEX (fingerprint_id, last_run, node_id, app_name)
);
```

The first two columns of the primary keys for both tables contain
`last_run` time stamp and `fingerprint_id`. The primary key utilizes hash-sharding
...
We also have an index for `(fingerprint_id, last_run, node_id, app_name)`. This
...
WITH RECURSIVE map AS (
  SELECT
    LEAD(last_run, 1)
      OVER (ORDER BY (last_run, fingerprint_id)) AS next_last_run,
    LEAD(fingerprint_id, 1)
      OVER (ORDER BY (last_run, fingerprint_id)) AS next_fingerprint_id,
    system.sql_statement_stats.last_run,
  ...
  WHERE fingerprint_id = $1
    ...
    AND last_run < $3
  ORDER BY
    (system.sql_statement_stats.last_run, system.sql_statement_stats.fingerprint_id)
),
reduce AS (
  (
    SELECT
    ...
    FROM
      map
    ORDER BY
      (map.last_run, map.fingerprint_id)
    LIMIT 1
  )
UNION ALL
  (
...
SELECT * FROM reduce ORDER BY (last_run, fingerprint_id) DESC LIMIT 1;
