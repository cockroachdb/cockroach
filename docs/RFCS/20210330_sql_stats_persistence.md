...
1. Since statement and transaction statistics are stored in memory, to
   ...
   1. Due to the reliance, post-processing of the SQL statistics (e.g. sorting,
   filtering) are currently implemented within the DB Console. As we move to
   implement displaying and comparing historical statistics, solely relying on
   the DB Console to perform slicing-n-dicing of the statistics data is not
   scalable.
   1. Also because currently we implement post-processing of the SQL statistics
   in the DB Console, users lack the ability to view and analyze SQL statistics
   within the SQL shell. This results in poor UX.
...
Additionally, we can implement a system view to allow users to access this
up-to-date statistics within the SQL shell.
...
CREATE TABLE system.sql_statement_stats (
    ...
    */
    ...
    PRIMARY KEY (first_executed_at, fingerprint_id, plan_hash, app_name) 
      USING HASH WITH BUCKET_COUNT = 8,
    INDEX (fingerprint_id, first_executed_at, plan_hash, app_name, node_id)
);
...
CREATE TABLE system.sql_transaction_stats (
    ...
    */
    ...
    PRIMARY KEY (first_executed_at, fingerprint_id, app_name) 
      USING HASH WITH BUCKET_COUNT = 8,
    INDEX (fingerprint_id, first_executed_at, node_id, app_name)
);
```

The first two columns of the primary keys for both tables contain
`first_executed_at` timestamp and `fingerprint_id`. The primary key utilizes hash-sharding
with 8 buckets. There are two reasons for this design:
1. Using hash-sharded primary key avoids writing contentions since `first_executed_at`
...
We also have an index for `(fingerprint_id, first_executed_at, node_id, app_name)`. This
...
WHERE fingerprint_id = $1
  AND first_executed_at < $2
  AND first_executed_at > $3
ORDER BY
  first_executed_at;
```
...
WITH RECURSIVE map AS (
  SELECT
    LEAD(first_executed_at, 1)
      OVER (ORDER BY (first_executed_at, fingerprint_id)) AS next_first_executed_at,
    LEAD(fingerprint_id, 1)
      OVER (ORDER BY (first_executed_at, fingerprint_id)) AS next_fingerprint_id,
    system.sql_statement_stats.first_executed_at,
  ...
  WHERE fingerprint_id = $1
    ...
    AND first_executed_at < $3
  ORDER BY
    (system.sql_statement_stats.first_executed_at, system.sql_statement_stats.fingerprint_id)
),
reduce AS (
  (
    SELECT
      map.next_first_executed_at,
      map.next_fingerprint_id,
      map.first_executed_at,
      map.fingerprint_id,
      map.mean,
      map.squared_diff,
      map.count
    FROM
      map
    ORDER BY
      (map.first_executed_at, map.fingerprint_id)
    LIMIT 1
  )
UNION ALL
  (
    ...
    JOIN
      reduce
    ON
      map.first_executed_at = reduce.next_ts AND
      map.fingerprint_id = reduce.next_fingerprint_id
    WHERE
      map.first_executed_at IS NOT NULL OR
...
SELECT * FROM reduce ORDER BY (first_executed_at, fingerprint_id) DESC LIMIT 1;
...
FROM system.sql_statement_stats
WHERE first_executed_at > (current_timestamp() - MOD(EXTRACT(EPOCH FROM current_timestamp())::INT, 5 * 60)::INTERVAL)
...
1. Update the existing statistics entry with newly combined statistics.
1. If such a fingerprint does not exist, we insert the statistics into the
   system table.
...
* Write path: if we lose ability to write to system table, that means the
  ...
  fingerprint. We will also be tracking number of statistics we discard using
  the same counter that was described in the earlier section.
