...
```sql
ALTER TABLE <table> <select_statement>
ALTER INDEX <table>@index <select_statement>
```
...
### 3. `ALTER TABLE/INDEX RELOCATE` ###

The `RELOCATE` statements can be used to relocate specific ranges to specific
stores. This is very low-level functionality and is intended to be used
...
TBD: hide this behind session or env var?

```sql
ALTER TABLE <table> RELOCATE <select_statement>
ALTER INDEX <table>@<index> RELOCATE <select_statement>
```

`RELOCATE` takes a select statement with the following result schema:
...
ALTER TABLE t RELOCATE VALUES (ARRAY[1], 1, 2, 3)

-- Move the range containing /t/primary/1/2 to stores 5,6,7 (with 5 as lease owner):
ALTER TABLE t RELOCATE VALUES (ARRAY[5,6,7], 1, 2)

-- Move even k1s to store 1, odd k1s to store 2:
ALTER TABLE t RELOCATE SELECT ARRAY[1+i%2], i FROM GENERATE_SERIES(1, 10) AS g(i)
```

### TODO? ###

Do we want `SHOW KV TABLE/INDEX` statements that return information about the
ranges in a table/index (potentially between two keys)?
