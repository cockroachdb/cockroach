...
for existent queries to finish”. CockroachDB expects all connections to be
closed by clients before the end of `drain_wait`. After `drain_wait` ends and
`query_wait` starts, the server shuts down all connections that are not with
...
1. From the start of `drain_wait`, the health checkpoint /health?ready=1 returns
  ...
  message, it is expected to stop routing new connections to this draining node.
  But if during `drain_wait`, the user tries to connect to this node without a
  load balancer, they still can. (Which means the
...
4. If all connections are idle or there are no connections during
...
# Technical Design
([A draft PR can be found here.][draft pr])
...
period, and the connection pools are expected to recycle idle connections.**
If at a certain timestamp all connections are closed, the server performs an
early exit and proceeds to `lease_transfer_wait`. Note that if a connection is
pulled by a user and waiting for query input, this connection is not considered
idle, and will not be recycled until the user puts it back in the pool. The
timeout can be set to be infinitely long. Clients can notice from the log that
the existing connections that are not executing queries will be closed.
...
connectionWait = settings.RegisterDurationSetting(
    ...
    If all connections are closed by the client, the server will continue with
    the rest of the shutdown process before the end of connection_wait.”,
    0*time.Second,
).WithPublic()
...
- **Notify customers of the connection status**:
  ...
  - **CLI prompt**: if user start the draining the node using CLI (e.g. "ctrl-c"),
    ...
    no new connections are allowed now, connections will all be closed within
    1m20s`.
  - **Log**: print the timestamp of the start and end of each draining
    ...
    connection pools or load balancer if they receive an error message. Also,
    for better troubleshooting after applying this new draining process, the log
    should provide the monitoring of current alive connections / unfinished
...
`--infinite-connection-wait` boolean flag. If `----infinite-connection-wait` is
set true, the value set for `server.shutdown.connection_wait` will be ignored.
The `----infinite-connection-wait` should be a "hidden" setting which is not
going to be documented in cockroachDB public docs and won't be shown as an option in
`cockroach node drain --help`. I.e. we want to avoid the user from applying
an infinitely long `connection_wait`, and leave it for the technical support
(e.g. CC customer) team to use it for troubleshooting.
For cluster settings, we set an upper limit (e.g. 1 hour) for the duration
...
- `connection_wait`: No new connections are allowed. Existing connections
  still work, but those without queries in-flight will be given a timeout to be
  recycled by connection pool or closed by user. Idle connections whose lifetime
  is longer than the connection pool’s maximum lifetime will be recycled.
  Connections not returned to the pool won’t be recycled. If all connections
  are closed, it does an early exit before the timeout, and proceed to
  `lease_transfer_wait`.
- `query_wait`: Any connections without open queries in-flight will be closed.
  ...
  and proceed to `lease_transfer_wait`. Otherwise, the server waits till
  `query_wait` times out, and then force into the `lease_transfer_wait` period.
...
- How does this work in a multi-tenant deployment?
  ...
  started with `mt-start-sql`, with which this design will be compatible with
  the multi-tenant scenario. The draining process with multi-tenancy should not
  differ much from servers started via `start` or `start-single-node`, since in
  the[multi-tenant architecture][multi-tenant architecture], each tenant has
...
# Docs changes
  ...
  - How to tune the length of each waiting period when encounter errors in
    draining.
      ...
      - `server.shutdown.connection_wait` should be longer than the maximum life
        time of a connection as specified in the connection pool.
      - `server.shutdown.query_wait` is more based on user's own preference --
        ...
        exit early once all queries are finished.
...
provides the most similar functionality as
...
[PR 74412]: https://github.com/cockroachdb/cockroach/issues/74412
