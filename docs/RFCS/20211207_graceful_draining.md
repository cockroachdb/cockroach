...
A test with the [HirakiCP][HirakiCP] 
...
I.e. **clients are not encouraged to start new queries within this period, and 
the connection pools are expected to recycle idle connections.** If at a certain
timestamp all connections are closed, the server performs an early exit and 
proceeds to `lease_transfer_wait`. Note that if a connection is pulled by a user 
and waiting for query input, this connection is not considered idle, and will 
...
- `connection_wait`: No new connections are allowed, and existing connections 
  still work will be given a timeout to be recycled by connection pool or closed
  by user. Idle connections whose lifetime is longer than the connection pool’s 
  maximum lifetime will be recycled. Connections not returned to the pool won’t 
  be recycled. If all connections are closed, it does an early exit before 
