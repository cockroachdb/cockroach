...
The downsides of this workaround are:
  ...
  1. The `server.shutdown.drain_wait` has to be adjusted based on clients’
     configuration of the connection pooling and load balancer.
...
timeout is finite. Clients can notice from the log that
...
The default duration length of `connection_wait` is set to be 600 second.
This is based on concerns from both the customer and the cockroach support team
(the operators). For the customers, to avoid the existing issue, they would
expect a long `connection_wait` to wait till all SQL connections are closed.
But from the operator side, a node restarts should completes within a reasonable
time. Therefore, we set 10 mins (600 seconds) as the default because taking
2-10 mins to drain a node is whithin customers' expectation, it is (possibly)
accpetable for the operators as well.
(TODO: talk to the field team for validation.)
...
this in consideration, we propose to pause allowing an infinite
...
- `query_wait` (unchanged): Any SQL connections without open queries in-flight
  ...
  before the timeout, and proceed to `lease_transfer_wait`. Otherwise, the
...
SQL connection before the `connection_wait` timeout). And also the early exit in
