...
and some of the remaining features are discussed in Future Work.
...
descriptor (see the [Descriptors](#descriptors) section below). With that, UDF
resolution is as:
...
5. User should have privileges on all the objects referenced in the function
   body at definition time. Privilege may change before the next execution of
   the function which sucks.
...
message SchemaDescriptor {
  ...
  message Function {
    ...
    repeated FunctionOverload overloads = 1 [(gogoproto.nullable) = false];
  }
...
To make OID function reference parsing easier in expression SQL string, we
propose a new SQL token `OID_FUNC_REF` in the SQL parser. Which essentially
represents a string in the format of `@Fxxx` where `xxx` is an OID integer.
Current SQL token scanner needs to be extended to recognize the two character
string `@F` and scan an oid number after, then set token id to `OID_FUNC_REF`.
We also need a new implementation of the `tree.FunctionReference` interface,
which essentially is just an OID,  to represent resolvable function OIDs. This
would be the value type of the new `OID_FUNC_REF` token.
