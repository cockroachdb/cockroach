...
be a built-in type, a composite type, or a reference to the type of table's
column (with the syntax `table_name.column_name%TYPE`). **We will initially
support only built-in types and user-defined enums.** Argument modes include the
default `IN`, `INOUT`, `OUT`, and `VARIADIC`. The "out" variants mark an
"argument" as a return parameter and cannot be used when specifying a `RETURNS`
type. `VARIADIC` allows defining a function with variable arity. **We will
initially support only the `IN` argument mode.** `DEFAULT` values for arguments
can be specified for `IN` and `INOUT` arguments. **We will not initially support
...
returned. **We will initially support returning built-in types and user-defined
enums, `SETOF`, and `VOID`.** We will be able to support `TABLE` once we are able
to support user defined record types
(tracked by [issue 27792](https://github.com/cockroachdb/cockroach/issues/27792)).

The function body can reference arguments by name, or by their ordinal in the
function definition with, like `$1`, like in a prepared statement. If the return
type of the function is not `VOID`, the last statement of a UDF must be a
...
`UPSERT`, `DELETE`), but this is contingent on fixing [a known issue that also
affects CTEs with
mutations](https://github.com/cockroachdb/cockroach/issues/70731). We will not
initially support other statements including schema changes, setting cluster and
session settings, or creating table statistics.**
...
`[ EXTERNAL ] SECURITY INVOKER \| [ EXTERNAL ] SECURITY DEFINER`      | No               | Indicates the privileges to run the function with.
`PARALLEL [ UNSAFE \| RESTRICTED \| SAFE ]`                           | No               | Indicates whether or not the function is safe to execute in parallel.
`COST execution_cost`                                                 | No               | The estimated execution cost of a function that aids in query optimization. Functions that are inlined (see the [Optimization and Execution](#optimization-and-execution) section) will not need this estimate. Before we can support this, we'll need to decide a unit that makes sense to users.
...
`sql_body`                                                            | No               | Allows writing the function body as an expression rather than a string constant. It behaves differently to the string constant form. It only is allowed for SQL functions, it is parsed when the function is defined rather then when the function is executed, and dependencies of objects referenced in the function are tracked.
...
3. Always return the best match even if there’s a compatible match earlier in the search path.
   ...
   Candidates in pg_catalog: None
   Resolution result: sc2.f // It’s the better match even sc1.f is compatible.
   ```
...
name to a list of overload signatures (argument type OIDs, a return type OID
won’t hurt as well) plus UDF descriptor ids will be stored in the schema
descriptor (see the [Descriptors](#descriptors) section below). Schema descriptor
versions need to be bumped when adding/removing/renaming UDFs to make sure future
function resolution works properly with the new UDF set. With that, UDF resolution
is as:
...
`tree.FunctionDefinition` and hyrdate any user defined types. This means that a
...
1. Any object(table, type, UDF and etc.) referenced in the UDF function body
   ...
   the first pass of UDF, will be used to block renaming or altering type the
   referenced objects.
2. Usage of a function in tables/views will be tracked. It will be tracked at
   ...
   This is important to not violate constraints and correctness of index.
...
a string in the format of `@Fxxx` where `xxx` is an OID integer. The current
SQL token scanner needs to be extended to recognize the two character string
`@F` and scan an oid number after, then set token id to `OID_FUNC_REF`. We
also need a new implementation of the `tree.FunctionReference` interface, which
essentially is just an OID,  to represent resolvable function OIDs. This would
be the value type of the new `OID_FUNC_REF` token.
...
and sequences. Calling out views and sequences because UDF is kinda a
combination of the two:

1. Similar to view, UDF contains queries in the function body which need to be
rewritten to reflect new db names (target database being restored into).
2. Similar sequence, UDF will be referenced by other objects by id, and
...
Restoring is more interesting because we need to carefully:

1. make sure objects on the dependency graph exist
2. make sure UDF descriptor ids are properly rewritten
3. make sure queries in UDF function body are properly rewritten with target
database name
4. make sure UDF OIDs referenced by tables/views are properly rewritten
...
expression will contain a planning callback function that returns an optimized
`exec.Plan` for each statement in the UDF when given the argument values.
Similarly, a relational routine will be converted into a `execPlan` that
contains a planning callback function for planning each statement in the UDF
based on argument values.

When a UDF is evaluated, the planning callback function will be called for each
statement in the UDF. The callback function will replace argument references
with constant values and then optimize the relational expression before
constructing an `exec.Plan`. The `exec.Plan` for each statement will be executed
sequentially, and the results of the last statement will be returned. This
approach is similar to the pattern used for the execution of apply-joins.
...
Supporting recursive CTEs will require changes to query planning and execution.
In particular, we'll need a way to reference a routine expression in the query
plan to avoid building an expression tree with infinite depth.
...
point for optimizing these constructs into relational expressions.
