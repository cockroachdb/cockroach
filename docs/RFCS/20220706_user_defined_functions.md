...
to the type of a table's column (with the syntax `table_name.column_name%TYPE`).
...
apply-joins, precludes the distribution of UDFs in DistSQL processors. A UDF
`planNode` cannot be distributed because the planning closure exists in memory
on the gateway node. However, inlined UDFs (see [Inlining](#inlining)) can be
distributed as long as the operators they are transformed into are
distributable.
