...
The proposed implementation is as follows:
 ...
 - On the system tenant we set up rangefeed on `system.tenant_settings` and
   keeps all the changed settings in memory. This can be started only on
   multitenant configurations (e.g. the first time a tenant connects via the
   connector). This code has the responsibility of implementing the connector
   API above to keep all tenants up to date. We continue to set up the rangefeed
   on the `system.settings` table to maintain the system tenant settings.
