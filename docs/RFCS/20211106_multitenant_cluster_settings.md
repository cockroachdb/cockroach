...
`tenant-rw` settings):
 ...
 - `ALTER TENANT ALL SET CLUSTER SETTING <setting> = <value>`
   - Sets the value seen by all tenants, except those that have a specific
     value for that tenant (set with `ALTER TENANT <id>`). For `tenant-rw`,
     this value will override any setting from the tenant's side, until the
     cluster setting is reset.
 ...
 - `SHOW CLUSTER SETTING <setting> FOR TENANT <tenant>`
...
The proposed implementation is as follows:
 ...
 - We update the semantics of the existing `system.settings` table:
    - for the system tenant, this table is now unused.
    - for other tenants, this table stores only values for `tenant-rw` settings.
      Any table rows for other types of variables are ignored (in the case that
      the tenant manually inserts data into the table).

 - We add a new `system.tenant_settings` (TODO: `master_settings`,
   `tenant_setting_overrides`?) table with following schema:
   ```
   ...
   ```
   This table is only used on the system tenant. All settings for the
   system tenant (including `system`) are stored in `tenant_id=1`. All-tenant
   values are stored in `tenant_id=0`.
 ...
 - We modify the tenant connector APIs to allow "listening" for updates to
   ...
   a precedent for a similar subsystem.

 - On the system tenant we set up rangefeed on `system.tenant_settings` and
   keeps all the changed settings in memory. This code has the responsibility of
   maintaining the system tenant's set of settings up to date, as well
   implementing the connector API above to keep all tenants up to date.

 - On non-system tenants we continue to set up a rangefeed on the tenant's
...
In mixed-cluster operation, the system tenant continues to listen to
`system.settings` and `SET CLUSTER SETTING` continues to write to that table.
To migrate a cluster to the new version, we copy all settings from the system
tenant's `system.settings` to `system.tenant_settings` (with `tenant_id=1`).
...
There are three possibilities in terms of the system table changes:
 ...
 - a) Add a new `system.tenant_settings` table (as described above).
    ...
    - Con: non-trivial mixed-version and upgrade logic.
