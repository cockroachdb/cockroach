...
values (`DInt`, `DString`, `DInterval`, etc.), increasing to 18 when
#18171 merges, and possibly increasing further as we extend pg
...
| `type Datum interface { Expr; ... }` | `type Datum int` | |
...
- Slight deviation from 100-level data structure textbooks. (Although
  the new propsoed pattern is not really unknown to 200- or 300-level
  compiler courses).
...
- rewrite CockroachDB's SQL layer in a different programming language
  ...
  alternative is serious but a longer-term endeavour. Also even if the
  generated code would be overall much more heap-efficient with tree
  rewrites, it would still require algorithms to traverse trees even
