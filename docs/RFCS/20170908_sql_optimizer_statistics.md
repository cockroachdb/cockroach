...
timestamp cache in order to avoid disturbing foreground traffic. [TBD:
What is the refresh period? Ideally we'd want to refresh hotter
tables/ranges faster, though that leads down a road of significant
...
table size. Consider using a MaxDiff histogram instead of an
equi-depth histogram. A MaxDiff histogram provides much tighter error
bounds.]
...
* The centralized stats tables provide a near failure mode. If the
  stats tables are unavailable we would proceed without the stats but
  ...
  failure mode is not new to the stats tables: most of the system
  metadata tables have th esame concern. Storing the stats for a table
...
* While executing a query we might discover that the expectations we
  ...
  statistics? See also [self-tuning
