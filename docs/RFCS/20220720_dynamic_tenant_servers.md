- Feature Name: Starting tenant SQL servers dynamically
- Status: draft
...
This change is meant for use both for:
...
- SQL pods in CockroachCloud Serverless, as a way to separate the step
  ...
  This will generally helps towards lowering our initial connection
...
Regardless of whether the tenant server is run inside a separate SQL
pod (`cockroach mt start-sql`) or in-process with the KV code
(`cockroach start`), the key point is that it starts dynamically,
after the process has been running for a while already.

This is a key building block of an passive->active failover in
...
- to power an active/passive online replication failover, so that a
  ...
  We would also like to orchestrate this failover without necessitating
  starting (or restarting) a `cockroach` process. Restarting
  process is a heavyweight and error-prone operation. By instantiating
  the new SQL server dynamically, we can verify beforehand
  that the process is pre-started successfully, that its TLS/networking
  config is valid, that its logging works, etc.
...
predefined name for clusters that have not been migrated yet.
...
Additionally, there will be a cap on the maximum simultaneous number
of tenant SQL servers running. By default, that cap will be 2, with 1
slot always reserved for the system/admin tenant. An additional
license check will be required to increase that limit past 2.
