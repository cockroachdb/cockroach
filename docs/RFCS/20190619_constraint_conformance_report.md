...
1. If one has exactly two localities, and a range only has replicas in one, should it be counted as under-diversified?
1. Should the SQL statements that now create job also become synchronous (i.e. only return to the client once the corresponding job is done)? A la schema changes.
