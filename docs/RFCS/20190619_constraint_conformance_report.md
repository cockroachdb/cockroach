...
-- object is the name of the zone/table/partition/index that this report refers to.
object string,
...
A background process will populate this once per minute or so. The table will
expose, per zone/partition (and perhaps per table/index too) the following:
...
replicas in dc=west)
```
1. How much data violates each constraint.
E.g. 
```
...
````
1. How much data violates the leaseholder preferences
...
scan all of the meta2 range descriptors together with all the zone configs. For
each descriptor, it’ll use logic factored out of the allocator for deciding what
constraints the range is violating (if any). For each constraint, it’ll
aggregate the (sum of sizes of) ranges violating that constraint. Same for lease
preference and replication factor.
...
under-replication, constraint conformance lease preference conformance, a node
...
could also generate it on demand, but then we'd only have velocity information
when we compute it a 2nd time in some close succession.
