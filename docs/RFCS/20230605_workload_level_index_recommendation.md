...
`pkg.sql.idxrecommendations.idx_recommendations_cache.ShouldGenerateIndexRecommendation()`. Meanwhile, it will also 
clear those index recommendations which was not used for more than one day in method
`pkg.sql.idxrecommendations.idx_recommendations_cache.clearOldIdxRecommendations()`. Thus, all the index recommendations 
we select from the table `system.statement_statistics` are those which are run more frequently and are crucial for the 
performance.
...
potential point: inputs users need to specify, including the budget

### Insight delivered to users
Since we have the statistics for the frequency about the queries, so that we can deliver the "usefulness" for each 
index, lik, the number of query types can be optimized, the number of sql statements can be optimized (some type of 
queries may be executed many times). In addition, we can also deliver the space cost for all the index recommendations.
...
Nevertheless, we only select some internal nodes to build indexes so that not everything in the tree is covered, meaning 
that some dropped indexes are not covered by the indexes selected to create. Since the space and time budget is 
confined, we can also take dropping indexes into account, the cost function can be used to calculate the space released 
by dropping one index. Following the idea of the trie tree, we can add all the indexes to drop to the tree. When we 
decide to merge two indexes, there may exist some indexes to drop along the paths to their LCA, when we compute the 
space to save, we can minus those space occupied by the indexes to drop since we can't cover them once we merge these 
two indexes. Thus, our merging process will be more comprehensive.
...
`[/StoreColumns]`. Suppose there are 3 tuples inserted to the table `t`: `{1, 2, 3}, {4, 5, 6}, {7, 8, 9}` and i is the 
primary key. If we create the following index:
```sql
