...
index `(i) storing (j)`. Otherwise, let us consider the indexes as follows:
```sql
...
be the smallest index) in the j's subtree (under t, not the leaf node). In our example, there is only one leaf node j, 
thus i will be stored there. To sum up, for the index 1-6, we will build indexes `(i, j)` and `(j, k) storing (i)`.
...
index, like, the number of query types can be optimized, the number of sql statements can be optimized (some type of 
queries may be executed many times). In addition, we can also deliver the space cost for all the index recommendations.

For the command line interface, we show all the index recommendations in the descending order of the number of sql 
statements one can optimize. In the future, we can deliver more information, e.g. all the fingerprints one index can 
optimize and the trie tree to represent the relationship among indexes.
