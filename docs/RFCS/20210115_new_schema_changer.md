...
Note: This section is still in progress. An outline follows of topics we need to cover.

### Schema changer and `connExecutor` state machine interactions in the general case

- `extraTxnState` stores a set of nodes representing the current targets and their states, accumulated from all the DDL statements issued so far
- For each DDL statement issued in the transaction, the schema changer plans and then executes a part of the schema change; note that these steps basically coincide with statement planning and execution
  - Planning:
    - Building targets: Take initial nodes and the DDL statement as input, and output a new set of nodes that incorporate new or modified targets resulting from the DDL statement
    - Constructing stages: Determine the stages and ops to be executed as part of the statement execution phase
      - For example, dropped columns should immediately become non-public; it's undesirable to have them be writable for the remainder of the transaction
      - Usually this will be a single stage, but for tables created earlier in the same transaction we'll be able to generate multiple stages, since no concurrent transactions see the table
  - Execution: Execute the ops, and accordingly update the targets' states (i.e., the nodes) on `extraTxnState`, which will become the initial nodes for the next DDL statement
- At commit time, we plan and execute for the pre-commit phase, with the input nodes stored in `extraTxnState` being the targets and states accumulated from the entire transaction's worth of DDL statements
  - For example, this is when we make new indexes and columns write-only
  - This is "planning" and "execution" from the point of view of the schema changer, but isn't tied to a statement
- Afterward, the resulting targets and states after pre-commit execution are marshalled into the job record, so that execution can finish in the job

### Constructing graphs

- Op edges and dependency edges are specified declaratively, and encode the progression of states in a schema change
  - Op rules generate successive op edges for a target
  - Dependency rules (effectively) generate dependency edges between pairs of nodes
- Ops that must occur in the same stage are represented by dependency edges in both directions between a pair of nodes (e.g., a new primary index replacing the former primary index)

### Constructing stages

- A sequence of stages of a graph is a partition of all its op edges, such that the sequencing of stages is compatible with the ordering constraints imposed by the dependency edges, and all ops within a stage have the same type (descriptor mutation, backfill, validation)
  - Specifically, no dependency edge can point from a later stage to an earlier stage
- Stages are constructed greedily: From the states produced as a result of the previous stage, we take all successive op edges directed from those states for each target which do not cause a dependency edge to be violated
  - This works because once a constraint implied by a dep edge is satisfied, it will be satisfied for all later states of the depended-on target

### Reverting schema changes

- Schema changes being reverted are planned by reversing the directions in each target, with a possible extra step to queue a GC job to clean up indexes
- Some failures are "expected" (e.g., failure to add constraints), but in general we'll have to assume that other failures can also happen
- Schema changes that need to drop indexes (including column changes, which need to drop the prior primary index) have a point of no return past which reverting is not possible
- In our current async job framework:
  - We assume schema changes can be canceled in "most" states (but not past a certain point when dropping indexes since the job will be marked non-cancelable)
  - We probably also have to assume that other job failures can happen in any state, without a perfect comprehensive list of retriable errors
  - Reverting will work similarly to how it works now: we run them in the `OnFailOrCancel` hook

### Limitations on concurrent schema changes

- New invariant: schema changes cannot be queued (in the mutations list) until previous ones finish
  - "Queuing a mutation" is properly thought of as partially executing a schema change, and we'd like to avoid this
  - We need to wait and effectively poll the descriptor until the next schema change can run
  - In general we need child transactions
  - For single-statement implicit transactions, it suffices to just keep restarting until there are no queued mutations
- Locking vs. non-locking descriptor updates
  - Some descriptor updates, like adding foreign key or type backreferences on "another" table, are considered "non-locking" and can interleave arbitrarily with other schema changes on that table

### Compatibility with old schema changer

- New schema changes are version-gated
- We need both forward and backward compatibility
- New schema changes need to wait for either new or old schema changes to finish
- Old schema changes need to wait for new schema changes to finish
- Polling and waiting can be done for single-statement implicit transactions by simply restarting until the condition is met; otherwise we need child transactions in the general case

### Testing

- We can test components with datadriven tests:
  - DDL statements to targets
  - DDL statements to stages/ops
- Having discrete stages for execution means we can insert testing knobs and test effect of concurrent transactions in specific states
