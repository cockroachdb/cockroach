...
1. There should be no overlap of nodes between copysets for scatter width of 1
and at most 1 overlapping node for scatter width of > 1.
...
It's possible to have an algorithm which tries to re-use the previous copysets
to minimize data movement across the cluster at the cost of reduced diversity,
but in this RFC we will not cover that as it's not a part of the MVP of this 
proposal.
...
Other nodes will be periodically reading the persisted copysets and using it for
re-balancing.
...
5. Balance score difference: Balance score is the normalized utilization of a 
  ...
  with a low balance score are preferred.
...
copysets with significantly lower load. 
...
[academic literature](https://web.stanford.edu/~skatti/pubs/usenix13-copysets.pdf)).
2. In the above copyset allocation algorithm, addition / removal or node crashes
can result in a lot of data movement. We can later tweak the used algorithm of 
copyset generation to minimize data movement (potentially at the cost of 
diversity). In scenarios where the above events are rare, this problem does not
have much impact.
...
Copyset allocation on addition / removal / node crashes can take into account
previous copysets in order to minimize data movement. This would make the
allocation more complex and we may not be able to optimize for diversity.

For the first cut of copysets this optimization will not be considered.
