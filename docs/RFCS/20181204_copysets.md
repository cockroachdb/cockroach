...
Copysets will initially be an opt-in feature and implemented for a scatter
width of `replication_factor - 1` (eventually it will be extended to support 
higher scatter width).  
...
2. Assign previously existing copysets to the same copyset id they belonged to
...
#### Swaps
Store swaps are made between a source copyset and a target copyset if
1. The source copyset has diversity < replication factor
2. Target locality either
   1. Has two stores in the target locality being swapped.
   2. Has diversity > replication factor.  
By diversity above we mean the number of localities in a copyset.

This guarantees that the diversity of the source copyset increases while
the diversity of the target copyset does not fall below replication factor.
