...
2. Starts with an allocation very similar to previously used copysets.
3. Adds the newly added stores to empty spots in each copyset.
...
stores. Copysets will be re-generated if the store list has been stable
and not changed for 3 ticks (each tick has a 10s interval).
