...
This active query field will be a map from `planNode` to `QueryMeta`s. We already have
`parallelizeQueue` of a map type with `planNode` as a key, but because of the way
...
- Start timestamp
- A boolean denoting whether the query is using distsql.
- Eventually: any contexts/object references useful for cancellation 
...
+---------+----------+------------+--------------------------------+------------------+------------------+------------------+
| node_id | username | start      | query                          | client_address   | application_name | using_dist_sql   |
+---------+----------+------------+--------------------------------+------------------+------------------+------------------+
| 1       |  root    | 1234567894 | SELECT * FROM users;           | 192.168.1.5:8080 | test             | true             |
| 1       |  root    | 1234567891 | INSERT INTO test VALUES (...); | 192.168.1.5:8080 | test             | false            |
+---------+----------+------------+--------------------------------+------------------+------------------+------------------+
(1 row)
