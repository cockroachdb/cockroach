...
Since CockroachDB does not support all the objects listed, the matrix below does a comparison to show how the default privileges in Postgres (for the PUBLIC role) will map to CockroachDB.
...
1. In 21.2, continue supporting the syntax for incompatible privileges, however under the hood we will "translate" the incompatible privilege to the corresponding default privilege.
  ...
  - Example, `GRANT SELECT on DATABASE db TO user` will under the hood perform `ALTER DEFAULT PRIVILEGES GRANT SELECT ON TABLES TO user` in database db. Thus any tables created by `user` in the database `db` will have `SELECT` privileges for `user`.
...
In both cases, the long running migration will upgrade the privilege descriptors in the same way. Any incompatible privileges will be removed from the privilege descriptor and added to the default privilege descriptor such that "inheritance" (privileges for new objects) are equivalent pre and post migration.
