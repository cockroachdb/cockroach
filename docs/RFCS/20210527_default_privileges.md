...
The proposal to work around this is having a "reserved" role that effectively acts as `ALL` and where we translate GRANTS over to for the purpose of migration.
One would not be able to directly do `ALTER DEFAULT PRIVIELGES FOR ALL` as this is explicitly not allowed in the syntax but for
v21.2, we would translate `GRANT SELECT ON DATABASE d TO foo` to `ALTER DEFAULT PRIVILEGES FOR ALL GRANT SELECT ON TABLES TO foo`. 
...
Users would not be able to perform a `ALTER DEFAULT PRIVILEGES REVOKE FOR ALL`, to mitigate this, a function should be exposed to fully reset default privileges for the `ALL` role in 22.1.
In 22.1, once the syntax is deprecating for granting invalid privileges, there will be no way to change default privileges for the `ALL` role.
