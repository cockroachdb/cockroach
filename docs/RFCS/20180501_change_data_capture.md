...
the current value of each watched row is emitted. After this "initial scan",
updates to watched rows are emitted. The `AS OF SYSTEM TIME <timestamp>` syntax
can be used to start a new changefeed that skips the initial scan and only emits
all changes after the user-given timestamp.
...
Changes to a row are always emitted in mvcc timestamp order and rows are sharded
between Kafka partitions by the row’s primary key. This allows simple cases to
be easy and low-latency (after the initial catch-up and in the absence of
rebalancing, perhaps single digit milliseconds). It also means Kafka’s log
compaction feature will work as expected.
...
Tables with more than one [column family] will require some buffering and are
more expensive in a changefeed. If a row is modified more than once in the same
transaction, only the last will be emitted.
...
Each emitted record contains the timestamp of the transaction that updated the
row. This can be used with the `timestamp_closures` option, which causes the
changefeed to periodically emit timestamp close notifications on every kafka
partition. The timestamp included in this record is a guarantee that every
record emitted to that partition afterward will have a higher timestamp.
...
```sql
CREATE CHANGEFEED FOR TABLE tweets EMIT TO 'kafka://host:port' WITH <...>
CREATE CHANGEFEED FOR TABLE tweets VALUES FROM (1) TO (2) EMIT TO <...>
CREATE CHANGEFEED FOR TABLE tweets PARTITION north_america EMIT TO <...>
CREATE CHANGEFEED FOR DATABASE db EMIT TO <...>
```

- `<sink>` is a URI that contains all the configuration information needed to
  connect to a given sink. Any necessary configuration is done through query
...
<jobid>`. Then the following `ALTER CHANGEFEED <jobid>`  commands may be used to
change the configuration. After any alterations, the changefeed must be unpaused
with `RESUME JOB <jobid>` to restart the changefeed.
...
Similarly, a counterpart to `SHOW JOBS` is introduced.


# Reference-level explanation

Each changefeed is structured as a single coordinator which sets up a
long-running [DistSQL] flow with some new processors described below.

Each changefeed is given an entry in the [system jobs], which provides this
fault-resistent coordinator. This works by a time based lease, which must be
continually extended. If it is not, a new coordinator is started.

TODO: Some failure scenarios result in a split-brain coordinator, which other
jobs handle by being idempotent. Changefeeds could emit only if the node's time
is in `[previous job lease, current job lease + max offset)` but this doesn't
account for something getting gummed up in the network. Kafka (and SQL)
transactions would work, but the overhead is unfortunate and it's preferable to
...
timestamp)` pairs is passed back to the job coordinator via DistSQL metadata.
(TODO: Is the metadata the right place to do this?).
...
If `initial_scan` is set, then the current value of every row at `timestamp` is
emitted. `ChangeAggregatorProcessor` then proceeds to set up ChangeFeeds.
...
and ChangeAggregator can immediately emit them as rows. Because tables with
multiple column families have more than one key per row, they always require
this buffering, even after `StreamState`.

After `StreamState`, the ChangeFeed will also begin emitting timestamp close
notifications, which are messages that indicate that no kvs with a lower or
equal timestamp than the one in the close notification will be emitted. These
are passed on by ChangeAggregator to be used by downstream consumers.
...
partitions. TODO: I think this is how everyone else does it but should double
check.

TODO: If we had an option to shard by range ID, we could push close
notifications more aggressively (though splits and merges would cause reshards).
Naiad likely wants this, do any customers?
