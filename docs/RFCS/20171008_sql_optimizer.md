...
logical node is represented by one or more equivalent logical or
physical nodes. For example, one equivalancy group might contain both
"a JOIN b" and "b JOIN a" which are logically equivalent in relational
algebra. During Search, but nodes might get enumerated in order to
determine in which order to perform the join. The memo structure
avoids the combinatorial explosion of trees be naively enumerating the
variants by having each node in the tree point to child equivalency
groups rather than directly to child nodes.
