...
are a common class of failures, in CRDB, they are distinct from lock contention.  
These failures, which are `TransactionRetryError`s with reason `RETRY_SERIALIZABLE`, 
