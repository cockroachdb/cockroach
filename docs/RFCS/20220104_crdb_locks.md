...
2. **Only transactions hold locks.**  While non-transactional requests can wait 
for locks, only transactional requests can obtain them.  This means that 
non-transactional requests will only show up as waiters.
...
far, with `UPDATE item SET i_name = 'Lamp' WHERE i_id = 124;` as the first 
statement.  While other transactions that attempt to obtain the lock on this 
key will have waiting queries visible in `cluster_queries`, we will not be able 
to show that this `UPDATE` statement was the one that caused its transaction to 
obtain the lock.
...
“blocked” by other transactions).  This also applies to readers, with the 
exception of `GetForUpdate`/`ScanForUpdate` requests (which acquire 
unreplicated locks).
...
* Incorporating [how long a lock has been held](https://github.com/cockroachdb/cockroach/issues/67619).
