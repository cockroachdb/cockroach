...
Note that not all schema change operations have asynchronous schema change
steps. For example, column renames occur completely in one descriptor version
step. One challenge for users, however, is determining which schema changes
are exposed to the hazards of asynchronous failure (and thus might be rolled
back).

Another problem here is the way constraint addition interacts with
concurrent writes. In general, we tend to begin enforcing the constraint
prior to validation. This violates the principle of atomicity.
...
we know that no transactions are in version 1. Note that physical layout here
refers to the set of columns which exist in encoded rows in public indexes.
...
and populates its target by synthesizing SSTs using the `bulkAdder`. Prior to
reading data to be written, the backfill scans the entire table to bump the
timestamp cache above the timestamp of SSTs being added.
...
* Lock the descriptor in question using a side key (the descriptor coordination key).
   ...
   * The schema change statement in depth:
      * Ensure that the user transaction is able to drop the leases to the old
        table descriptor so that we don’t prevent version bumps.
          * It's totally fine if we've already performed reads or writes because the
            descriptor coordination key prevents the descriptor from changing
            underneath the user transaction and so we know that all previous writes
            are valid.
      ...
      * Why is this backfill different from all other backfills?
          ...
          * What should it do when it encounters an intent from the user transaction?
            ...
            * What does it mean to re-issue previous writes to tables with new indexes?
              * Note that previous write here refers to intents of the user transaction
                performing the schema change. It does not refer to intents due to other
                transactions.
...
* Should jobs be used in any way for dealing with backfills and validations
  underneath a user transaction?
    ...
    * One pro would be observability, though the observability mechanism of jobs
      is a big hammer.
    * We could easily hook up the progress to the `SHOW QUERIES` progress added
      in 20.1
