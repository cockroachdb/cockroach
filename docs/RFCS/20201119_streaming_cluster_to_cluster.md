...
Cluster-to-cluster replication or "streaming backup/restore" is an often requested feature, particularly for primary/secondary deployments. In such deployments the secondary would provide an option of **rapid failover** (RTO) compared to waiting for a full RESTORE, as well as **minimal data loss** (RPO), compared to taking periodic backups.
...
Rolling back to the low-water mark time with the direct ingestion approach is potentially a function of how far behind one stream is. Notably, this means that the **RTO will be a function of the RPO**. However, this makes observability into how far behind each stream all the more critical so that the operator can ensure that the streams don’t fall too far behind.


##### Tradeoff: Node Failure Relisancy
...
- RPO: how far back is the latest ingested timestamp? This is the timestamp we would rollback to on cut-over.
