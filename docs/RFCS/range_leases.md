...
Expiration-based range leases are verified when applying the raft
command by checking the command timestamp against the lease
expiration. Epoch-based range leases cannot be independently verified
by each Raft applier, as they rely on state which may or may not be
...
has the information necessary to verify validity.
