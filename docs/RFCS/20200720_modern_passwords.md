...
- Cockroach Issue: [#50757](https://github.com/cockroachdb/cockroach/issues/50757)  [#42519](https://github.com/cockroachdb/cockroach/issues/42519)
...
2. the server stores a **server-side secret** that represents the password.
   ...
   The server-side secret is typically another string than
...
PosgreSQL supports the following combinations of storage/authn methods:

| PosgreSQL `password_encryption` | Storage method                                 | Supported authentication methods |
|---------------------------------|------------------------------------------------|----------------------------------|
...
- Storage methods:
  ...
  - CockroachDB's `bcrypt-crdb` storage method also offers OK security.
...
- Authn methods:
  ...
  - `scram-sha-256` offers protection against credential stuffing and
...
implemented in CockroachDB up to and until v20.1. This method was not
...
- some user accounts are already created in v20.1, with method `bcrypt-crdb`
...
- server detects method `bcrypt-crdb` from cluster setting; selects salt and computes Bcrypt-10 hash
...
- client sends `CREATE USER foo WITH PASSWORD '$2a$10$fGgWYzxv4UTXVTNzQTHEa.kX3pMNNE.mxxoSk1ZTF9MPZlLOkHxbK' ENCRYPTION 'bcrypt-crdb`
- server verifies that `$2a$10$...` has the right format given the `bcrypt-crdb` method
