...
- [Stale LeafTxn errors](#Stale-LeafTxn-errors)
- [Zombie LeafTxns](#Zombie-LeafTxns)
- [Zombie RootTxns](#Zombie-RootTxns)
...
coordination. They also contain additional local state.
...
state, but none of them fully absorb the error to turn it into a
non-error (Nathan: can you confirm this?).
...
of interceptors, it then separates between two groups of errors:

- *unrecoverable* errors, which cause the TCS (and,
  together with it, the `client.Txn`) to become "trashed".
  An example such error is attempting to read data using
  a historical timestamp that has already been garbage collected.

  When this occurs the TCS changes its `txnState` to
...
- *recoverable* errors, such as retry errors, in which case:
  the TCS does three things:
    1. on the way out, some of its interceptors may update their
       internal state to prepare for a retry.
    2. the TCS upgrades its internal `roachpb.Transaction`
       object to prepare it for a new round of client-side retry,
       bumping its epoch,
    3. TCS then propagates the error to its client.

  Example sequence diagram in the case of a recoverable error:

  ![](20191014_savepoints/erecovery.png)

  (Technically there's an additional path where the TCS will "refresh
  the txn"—i.e. bump its commit ts—without bumping the epoch, in the
  case where there were no writes completed yet, but conceptually the
  situation is similar.)

An important aspect of this error recovery, which will become more
noteworthy below, is that the txn object stored inside the
TCS may become different "on the way out" (back to
...
If there is one or more LeafTxn existing concurrently with the RootTxn
and an error happens, we have a wide array of combinations to deal
with:

- combining errors:
  - error in LeafTxn, but not in RootTxn
  - error in RootTxn but not in LeafTxn
  - error in both
- error types:
  - recoverable in LeafTxn, not in RootTxn
  - recoverable in RootTxn, not in LeafTxn
  - recoverable in both
  - unrecoverable in both

Instead of casing each combination separately, the following rules are
currently applied:

1. LeafTxn TCSs never handle errors themselves.
   When they encounter an error, they become
   trashed and propagate the error to be dealt with by SQL and the RootTxn.
2. The state of LeafTxns is repatriated back via TxnCoordMeta
   into the gateway's RootTxn when distsql execution completes,
   regardless of whether an error occurs or not.
3. Error handling in SQL and the RootTxn occurs after the state
   of LeafTxns has been repatriated.

Note that even if an `error` object appears in step 1, it may very
well be that the state of the txn object (the `roachpb.Transaction`,
not the TCS) is still fine and not in error. The converse
is not true: if a hard txn error was found whereby the txn object
becomes unusable, there will also be an `error` object.

In step 2 the LeafTxn's TCS's state is repatriated and
merged with the RootTxn. Regardless of the presence of an `error`
object, the internal error state of the txn object is merged: if there
was a hard txn error on either side, there will be a hard txn error on
the merged txn.

Otherwise, in step 3 if there was a recoverable error the RootTxn
will take over and try its best to do error recovery in the manner
discussed above.

The astute reader will remark something unwieldy in this logic: *what
happens if a RootTxn TCS becomes aware of an error from a LeafTxn TCS
after it dealt with a retry error of its own?*

#### Stale LeafTxn errors
...
This is where we're reminded of [the last paragraph of the section
above](#Error-handling-in-TxnCoordSender): the TCS updates the txn
object "under the hood" and violates a desirable property: that the
txn object does not change while there are LeafTxn still active.
Today, it does change on the error path if LeafTxns are active!

To handle this, there is some complexity in the augment code to ensure
that the state is not merged if the epoch does not match. (We suspect
there are bugs lurking in there.)

It's also possible for the RootTxn to refresh while the LeafTxn
doesn't, then the refreshed RootTxn to be augmented with state from a
non-refreshed LeafTxn. This happens to work because the state that's
been repatriated is valid across refreshes. (Nathan, can you confirm?)

**Why this matters in this RFC:** savepoint rollbacks/releases must wait for all LeafTxns
to complete so that there are no LeafTxns alive across savepoints.

#### Zombie LeafTxns

Consider the following sequence:

![](20191014_savepoints/zombie.png)

In this situation, at t1 the RootTxn is forked into a LeafTxn on another
node to start parallel processing in a distributed query.

Then at t2 an unrecoverable error is encountered in the RootTxn. The txn
object becomes invalid for further use.

However the distributed execution is not immediately informed of this,
and then at t3 after t2 issues some KV operations via the LeafTxn
which, at that point, is unaware of the error status.

What to do then? Today the situation is not fully satisfactory:

- if the error is so hard that the persistent txn record
  is trashed, it's possible for a KV op in the LeafTxn to
  notice that and abort early. However this may not happen.
- concurrently, the error is propagated from the RootTxn
  back to SQL on the gateway, and causes (asynchronous) context cancellation
  of the currently running distributed query. We expect
  that the cancellation propagates through the flows and cause the
  processors to abort - eventually.
- in the meantime, it is definitely possible for KV requests to make
  it down to storage through all the layers and perform operations on
  behalf of the txn that's already dead from the perspective of SQL -
  in other words, KV operations on behalf of a zombie txn.

Why this happens to be fine today:

- the results of KV reads, if any will be ignored by the SQL gateway.
- LeafTxns only perform reads and so cannot lay write intents
  on behalf of a zombie txn.

**Why this matters in this RFC:** savepoint rollbacks must wait for all
LeafTxns to be terminated to ensure no LeafTxn can issue KV ops on behalf
of a txn state that's been partially rolled back already.

#### Zombie RootTxns

Same as previous section, inverting the two roles: the LeafTxn
encounters the unrecoverable error first, but the RootTxn is not aware
of this error yet and continues to issue KV batches on behalf of a SQL
txn that's now dead.

Here the situation is particularly bad: it's possible for the RootTxn
to issue KV writes and new write intents on behalf of a low-level txn
that's already aborted and possibly already being cleaned up. There's
(so far I can see as of this writing) no logic to prevent that inside
TCS or lower levels.

Why this happens to be fine today: I have no idea? (Andrei please help)

**Why this matters in this RFC:** savepoint rollbacks/releases must
detect when a txn is bad and prevent the savepoint operation from
proceeding (and produce an error instead).
...
It's interesting to consider how this happens to work with LeafTxns.
...
Just one inconsistency remains, and is the cause of the [current
halloween
problem](https://github.com/cockroachdb/cockroach/issues/28842): all
the KV reads issued _via the RootTxn_ interleaved with writes will use
the incrementing seqnum. This is incorrect because a SQL mutations
should not see its own writes.
...
4) a *SQL savepoint token* is a copy of the seqnum captured
   by the last call to `Step()`.

5) a SQL savepoint release is a no-op with regards to TCS.
...
2) let "soft retry" error recovery logic (txn refreshes and
   auto-pushes) remain as-is, and do not let them mark
   the TCS as "in error" (it currently does not become in-error
   on retry errors, so this needs no changes).

   The main change from the section above decouples the seqnum
   ...
   *The new behavior is that it becomes possible to recover from
   a retry error other than WriteTooOld using a SQL savepoint rollback.*

3) in a first implementation phase, let the WriteTooOld deferred
   error handling as-is.

   This means that it will not be possible to recover from a
   WriteTooOld error using a SQL savepoint rollack. This is the cause
   of the main [known limitation reported in the guide-level section
   above](#Relationship with client-side retries): a full client-side
   retry will sometimes need to restart from the beginning of the
   overall SQL txn.
...
- `GetSavepoint() (SavepointToken, error)` method on TCS and
  ...
  1) new `GetSavepoint()` method on TCS.
     ...
     Initially, `type SavepointToken = enginepb.TxnSeq`
