...
The addition of SQL savepoints is enabled by a recent changes to the
...
If an error occurs "under" a savepoint, it is possible to recover
...
A corollary of this restriction is that a retry error requires clients
to unwind the entire savepoint hierarchy.

There is a glimmer of home for an implementation that is able to
