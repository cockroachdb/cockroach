...
- the `search_path` is now required to become non-empty.

  This is aligned with pg: pg spells out "the default `search_path`
  include `public` and `pg_catalog`.  If `search_path` does not
  *explicitly* contain `pg_catalog`, the database will operate as if
  `search_path` includes `pg_catalog` in the last position." (this
  thus implies that if the client clears `search_path`, the database
  will operate as if it's not empty).

  Prior to this RFC we already had some code to make it appear *as if*
  `pg_catalog` was there even if not explicitly in `search_path`, but
  now the changes to algorithm A1 really require `search_path` to be
  non-empty so we need to make clearer checks for it.
