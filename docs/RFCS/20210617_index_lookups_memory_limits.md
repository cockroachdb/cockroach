...
order, to sometimes cache responses to short-circuit repeated lookups, and to
sometimes sort requests in key order to take advantage of low-level Pebble
...
joiner cannot produce out-of-order rows. So, the `Streamer` gets configured with
...
type Streamer interface {
	...
	// SetCachingHint can be used to hint the aggressiveness of the caching
	...
	// Batch versions enqueuing multiple requests at the same time. There are
	// performance advantages to using the batch interface - it helps the Streamer
	// generate bigger single-range batches which, in turn, helps with request
	// de-duping - particularly important when the results cache is cold.
...
// ResultMemoryToken represents a handled to a Result's memory tracking. The
...
func (j *joiner) Run() {
	  ...
	  for {
	  	...
	  	// ordinal will be used to associate responses to requests. 
	  	//TODO: We use Get, although we'd like to use BatchGet. See Unresolved
	  	// Question #2.
...
eventually clears the buffer. There's no limit on this buffer's resources; in
other words, there's no mechanism for throwing away buffered results and
re-doing work in order to stay within some budget. On the flip side, the
`joinReader's` buffer knows how to spill to disk. In contrast, the `Streamer's`
buffer allows clearing and re-doing of work (in a more ordered fashion) when
...
In order to facilitate the partial clearing of the buffer when under memory pressure,
each shared buffered result will also be annotated with the earliest position in
the buffer that references it. With this information, the buffer clearing
process could work as follow: say that the `Streamer` wants to clear 1MB worth
of entries from the buffer. Say that the buffer looks as follows:
```
|  | 1| <empty>
|  | 2| res a  (800 KB)
|  | 3| res b  (1 KB)
...
|  | 6| res a  (800 MB)
|
↓ buffer positions, from results to be returned first (lower) to results to be
  returned later (higher). Each result is annotated with its memory size, but
  node that there's duplicate results shared by multiple buffer slots.
```

The algorithm is as follows: we want to discard higher entries before we discard
earlier entries. As such, we start traversing the buffer from the highest
non-empty position downwards, and we keep clearing entries until we've freed up
enough memory. A result that isn't shared between multiple entries can be
released immediately, and the respective buffer entry can be cleared. The case when
our cursor gets to a shared result is interesting. For example, in the example above,
when we get to position 6, we can't immediately decide whether to drop `res a`;
the answer will ultimately depend on whether we'll need to clear the result at
position 2 or not. So, we move the cursor away from position 6 without clearing
anything, but we remember the fact that, if we end up clearing position 2, we'll
also need to clear position 5. This information about deferred potential actions
can be accumulated into a max-heap, indexed by position number with the values
being the list of other positions that get cleared at the same time. So, at the
moment when we move the cursor from position 6 to 5, our heap will consist of
the entry `2->[6]`. When we move from 5 to 4, out heap will be `2->[5,6]`. When
we get to position 3, we'll clear it and free up 1KB. Then, when we get to 2, we
free up 800MB and clear three entries: 2,5,6.

Every time a result is discarded, the request that produced it is automatically
re-enqueued for execution. This means that requests need to be kept around while
their results are buffered. Note that a re-enqueued request will not be executed
immediately, because of the logic about prioritizing earlier requests (see the
[Avoiding wasted work](#avoiding-wasted-work) section); the reason why a
buffered entry is getting discarded in the first place is because there's not
enough budget for requests at lower positions so, as budget frees up, those
requests will take priority.
...
batching interface that takes multiple requests at once (`Batch{Get,Scan}`).
That interface makes it easy to group the requests into `BatchRequests`. The
batching is done at range-level (using a best-effort `RangeIterator`). We'll
call one of these batches of requests that go to a single range a "single-range
batch".
...
is configured to return a single row no matter the size.
...
For its part, the client is responsable of destroying its reference when it's
...
It's an open question whether the `Streamer's` buffer should spill to disk in
order to avoid wasting results (so, should the `Streamer` also have a disk
budget?).
...
rows that it does anyway (to handled re-ordered results); the `red` lookup
result is thus buffered (like all the other results) in a disk-backed
container and used after the result for row 2 was emitted.

We propose to keep this de-duping behavior in the `Streamer`. The de-duping will
happen at the level of a single-range batch. This means that there will be
opportunities for de-duping that the `Streamer` will miss: if a request is
in-flight, other requests will not be de-duped against it. This is for
simplicity, and also to limit coordination required for different requests - if
you've got 1000 requests in-flight, you don't want to look through all of them
for every newly-arriving request.

In the `joinReader`, the de-duping is limited to requests within the same input
chunk: if the 1st chunk has 1000 lookups for key "a", they'll all get
coallesced, but then if the 2nd chunk wants key "a" again, that'll be a 2nd
lookup. The `Streamer's` deduplication, by itself, has the same issue. The
`Streamer` will hopefully do away with the joiner's notion of an input chunk,
but it still operates in batches of input rows. These batches are supposed to be
smaller (with the possible exception of the first one), because the `Streamer`
is supposed to have requests and responses streaming in and out of it. To make
up for the difference, we propose that the `Streamer` gets a response cache
that's not tied to these batches. This would be in addition to the de-duping,
and in addition to the buffering that the `Streamer` does for a different
purpose. A cached response short-circuits a new request for it. Details in the
next section.

Note that in case a duplicate request arrives while (one of the possibly-many)
duplicates are buffered, the request will be de-duped. In this sense, the
contents of the buffer behave as if they are cached.

### Result caching

In order to avoid repeated identical lookups, the `Streamer` will get a results
cache. Frequently accessed (or perhaps frequently and recently accessed) results
will be kept in this cache, memory permitting. In case the `Streamer` is
buffering (i.e. in `InOrder` mode), there's some overlap between the cache and
the buffer that we'll discuss below.
...
dropped, similarly to buffered rows can be dropped.
...
The joiner does another interesting thing: it usually [sorts lookups in key
order](https://github.com/cockroachdb/cockroach/blob/960817027ca772055c60f31fa0a5a0b6b75eb0ef/pkg/sql/rowexec/joinreader.go#L671-L694),
at the level of an input chunk, before sending them out. It does so foremost
because it has to - if it wants to use byte limits on its lookups, they need to
be sorted because of limitations of the `fetcher stack -> DistSender` (the whole
reason for this RFC). But secondarily, it gets some low-level benefits because
of this sorting because, at the level of a `BatchRequest`, the underlying
`pebble.Iterators` are reused and there are multiple seek optimizations inside
Pebble that can benefit if seeks are in sorted order.

To retain these optimizations in the cases where we usually can, we propose to
sort requests in key order at the level of a single-range request when operating
in `OutOfOrder` mode. This brings this optimization to the right level - the
level of a single range; sorting across ranges is not meaningful. In the
`InOrder` mode, sorting the requests would cause the results of a single-range
request to be delivered out of order in case the memory reservation for that
request is not sufficient, which would be a difficulty.
...
being PK values. (Is the uniqueness true for inverted JSON indexes?). Similarly, de-duping is inhibited.
...
caching). Being `OutOfOrder`, the `Streamer` 
...
2. The `Streamer` offers a batch-oriented request interface (`BatchGet`), and a
lookup-by-lookup one (`Get`). The pseudo-code uses `Get()`, although there are
benefits to using `BatchGet` (you get de-duping of lookups at the level of a
batch). The `joinReader` is not a vectorized processor; it uses the `RowSource`
interface for reading its input-side. `RowSource` returns one row at a time and
is a blocking interface. So, how is the `joiner` supposed to build a batch of
input rows in order to use `Streamer.BatchGet()`? The way in which it currently
does it is not great - the `joinReader` decided how big of a batch it wants to
accumulate and then blocks until it fills up that batch. What we'd want instead
is for the `joinReader` to read its input until it blocks and, once blocked,
pass do lookups on the accumulated batch. Should we add a non-blocking version
of `RowSource.Next()`?

3. Should the `Streamer` disable the `DistSender's` transparent splitting of
...
4. Should the `Streamer's` buffer spill to disk (i.e. have a disk budget in
addition to a memory budget)? Similarly, should the cache spill to disk? The
point of both the buffer and the cache is to use memory in order to save on
future work. In the case of the buffer, the future work is guarantteed (i.e. if
we drop parts of the buffer because we're out of memory, we'll have to redo
lookups that we've already done). In the case of the cache, we're just guessing
that there's future work (i.e. based on historical key access frequency).
