...
ordering guarantees than the batch-oriented API but, on he flip side, it would
...
It's worth noted that the batching described here and the budgets assigned to
...
will use less than `TargetBytes`, and so we can immediately release `actual size - TargetBytes` 
...
within the current reorder window. Say that a single-range batch contains
requests `{1,2, 1000}` and the reorder window consists of requests `1->100`
(where a lookup number identifies the order in which the responses need to be
delivered to the client. There's two choices:
...
the reorder window (e.g. 2 in out example). So, we don't expect responses for
...
I think the `Streamer` should offer pipelininig, although not necessarily from
...
requests in order to decide that granularity of shared spans for buffering
...
read its input until it blocks and, once blocked, pass do lookups on the
