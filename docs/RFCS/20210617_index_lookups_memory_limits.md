...
in particular for probing requests sent by the index and lookup joiners. These
joiners work by scanning, say, the left side of the join, building batches of
10k keys, and then probing the left side with 10k keys at a time. These probing
batches of 10k `Scans` or `Gets` tend to divide randomly across many ranges in
the cluster. In the case of the index joiners, each key from the left side
corresponds to one row on the right side. In the case of more general lookup
joins, each key can correspond to many rows. Evaluating them with a high degree
of parallelism (for example, parallelizing across ranges as the `DistSender`
does), is crucial for `JOIN` performance.

As things stand today, the joiner is forced to choose between setting memory
limits on the probing batches (thus protecting the joiner node from OOMs) and
...
parallelism: if each probe is guaranteed to return at most one row, then it
...
As a consequence, we've been known to OOM whenever the non-limited 10k rows
prove to be too big to hold in memory at once. And we've also been known to
execute the memory-limited lookups too slowly (e.g. [in our TPC-E
implementation](https://github.com/cockroachdb/cockroach/issues/54680#issuecomment-858776769).
The limit that SQL does implicitly use is the 10k rows per batch. Were it not
for the memory considerations, this number would perhaps be much higher (or, in
fact, this kind of batching should not exist at all, as we'll argue below).

Besides the memory limit aspect, the KV API doesn't seem to suite the joiners
(and perhaps other processors) well from another perspective: by forcing the
joiners into this one-BatchRequest-at-a-time execution model, the progress of a
joiner gets needlessly blocked by slow requests. The current model is: send a
batch a 10k lookups, wait for all of them to finish, then do the processing on
the up-to-10k returned rows (or perhaps more rows if it's not a index-join),
then scan the next 10k rows on the left side, then send the next 10k lookups,
etc. To understand whether this execution model is right or not, we should
analyze two cases:

1. The joiner wants to produce results in order (i.e. in the order of the rows
   on the scan-side of the join). In this case, it seems that the fact that each
   batch of lookups acts as a barrier (and, moreover, that the slowest
   range-sub-batch of lookups within the batch acts as a barrier) is OK, since
...
3. A sub-case of 2) that wants barriers even less is when there's no ordering
   requirement on the joiner and also there's a row limit. In this case, we can
...
This RFC discusses how to run a join with a fixed memory budget. It doesn't go
beyond that in thinking about how to make the join's budget dynamic, reactive to
...
a `ResumeSpan` in the middle of the results of a `Scan`. So, if a `Scan` were to
be split into two sub-scans (for two ranges), and the first one would return a
`ResumeSpan`, then the results of the second one would probably need to be
thrown away (or, at least, it's not clear what to do with them). Of course, at
the moment we don't have this problem since a limited `Scan` is never split into
concurrently-executing sub-scans, but we'll return to this issue.


#  A stream-oriented API

This RFC is proposing the introduction of a new library/API for the benefit of
the lookup joiner - the `Streamer`.
...
when the joiner needs to produce its joined rows in order (i.e. in scan-side
...
to the client (i.e. the joiner) in order. The joiner is expected to never do
buffer much on its side(*).
...
type Streamer interface {
	...
	// The more memory it has, the more it'll parallelize the requests.
	SetMemoryBudget(bytes int64)
	// SetOperationMode controls the order in which results are delivered to the client.
	...
	// Result blocks until one result is available. If the operation mode is
	...
	// The Results() channel will be closed once all in-flight requests finish.
	Done()
}
...
func (j *joiner) Run() {
	  scanSideBudget := 10MB
	  probeSideBudget := 10MB
	  scanSideAcc := BoundAccount(limit=scanSideBudget)
	  // scanSideRows will maintain the scan-side rows that haven't yet been
	  // joined with their probe-side counterparts. Rows enter the map in the scan
	  // loop below, and exit it async, as the joiner has produced the joined
	  // result rows.
	  // Map from an ordinal to the row.
	  scanSideRows := make(map[int]roachpb.KeyValue)
	  
	  var prober Streamer
	  prober.SetOperationMode(<InOrder/OutOfOrder corresponding to the joiners ordering>)
	  
	  prober.SetMemoryBudget(probeSideBudget)
	  probeDone := j.processProbeResults(prober)
	  resumeKey := <scan-side lower bound>
	  scanEnd := <scan-side upper bound>
	  var ordinal int
	  for {
	  	// Read a chunk of rows from the scan side. Note that we're not using a row limit,
	  	// only a memory limit. 
	  	// But first, wait to have some scan-side budget. 
	  	for {
	  		scanBudget := scanSideBudget - scanSideAcc.Size()
	  		if scanBudget > 10KB {
	  			break
	  		}
	  		// Synchronize with processProbeResultsAsync and wait for enough 
	  		// scan-side budget to be released.
	  		scanSideAcc.Wait()
	  	}
	  	resp := db.Scan(roachpb.ScanRequest(resumeKey,scanEnd), TargetBytes=scanSideBudget)
	  	// Account for all the rows we just got. Each row will be un-accounted 
	  	done := resp.ResumeSpan.Empty()
	  	if !done {
	  		resumeKey := resp.ResumeSpan.Key	
	  	}
	  	// Submit the probe requests.
	  	for r := range resp.Rows {
	  		scanSideAcc.Grow(<size of r>)
	  		ordinal++
	  		scanSideRows[ordinal] = r
	  		// We pass in the ordinal into the prober so that we can retrieve         	  
	  		// the scan-side row when the probe-side response is received.
	  		prober.Get(<join key from r>, ordinal)
	  	}
	  ...
	  prober.Done()
	  // Wait for the processing goroutine.
	  <-probeDone
}

func (j *joiner) processProbeResultsAsync(
  prober *Streamer, scanSideRows map[int]roacpb.KeyValue, scanSideAcc BoundAccount,
) <-chan struct{} {
	  ...
	  go func() {
    		...
    		for {
    			...
    			scanSideRow := scanSideRows[res.Baggage.(int)]
    			// <do the actual join between scanSideRow and res.Resp and output the row>
    			// <release memory from scanSideAcc corresponding to scanSideRow> 
    		}       
    }
    return done
}
...
2. Don't send too many `BatchRequests` at once. The `Streamer` will maintain an
   ...
   the estimate will be per probe-side `Scan`). This estimate will dictate how
...
line key is slow to be retrieved at some point, the joiner's scan-side loop will
still push requests for as long as the scan-side and the `Streamer`-side budgets
allow. You can imagine that, if rows in some tables are small, millions of
out-of-order results can be buffered while the head-of-line request is blocked.
To make this increasingly true, we can imagine adding the ability to
spill-to-disk in the `Streamer`, to basically increase its memory budget with a
disk budget.
...
The `Streamer` will constantly maintain around *B/P* requests in-flight. The
requests are batched, so the actual number of batches will be smaller; a batch
weighs according to the number of requests in it (*n*). Each batch consumes some
budget - *n\*P*. This budget constitutes its `TargetBytes`. When a response is
received for a batch, we can see if the `TargetBytes` estimate was over or
under. Generally, responses will use less than `TargetBytes`, and so we can
immediately release `actual size - TargetBytes` back to the `Streamer's` budget.
The bulk of the response's bytes cannot be released immediately; they're only
released once the respective results have been delivered to the client (which
can be delayed, according to the ordering setting). The estimate *P* is updated
as responses come in - after the initial responses, *P* will be tracking the
...
`OutOfOrder` case, if there's no budget left. That's when the `Streamer` is
forced to throw results away. Assume that there's no budget left, there's a
front of the line of *n* requests, and there's a buffer of *m* responses. We'll
throw away as many of these *m* as necessary to free up *n\*P* bytes. Responses
will be thrown out in reverse order. In other words, buffered results will start
being discarded as soon as it appears that there's not enough budget for all the
requests in front of them. This is not the only option; at the extreme, as long
as there's budget for one row, we could read one row at a time (the first one),
deliver it to the client, read the next row, etc., without throwing away
buffered results.
...
TODO: Is it a reasonable assumption that we'll plan lookup-joins for such queries? Or should I expect a lateral join, perhaps?
