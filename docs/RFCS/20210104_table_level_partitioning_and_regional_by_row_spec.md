...
partition) couple with the fact that we want to optionally hide the partitioning
...
row-level regional tables are table-leveled partitioned, we take the opportunity
...
created automatically for unique constraints on such tables.

- **Index-level implicit columns.** These are partitioning columns that are part of
...
crdb_region out of star expansion by default so that MR doesn’t break apps doing
`SELECT *`.
...
3. Introduce table-level partitioning: a set of column that partition all of the
table's indexes.
...
to fail. What to do? Should we strive to pretend that the PK is the one the user
indicated, or should we expose `crdb_region` more?
...
1. Don’t require partitioning columns to be explicitly included in an index.
  ...
  it:`INDEX idx_implicit (x) PARTITION BY LIST(region) …`.  
  ...
  partitioning is removed. So, `ALTER INDEX idx_implicit PARTITION BY NONE` would
  result in an index on `(x)`. Not so for `idx_explicit`.
2. Don’t require unique constraints (and, by extension, the PK) to include all
partitioning columns. This builds on the previous bullet, and we’d now get ahead
of MySQL and PG (PG lists it as an implementation limitation).
  ...
  Open question: when a partitioned unique index is defined as `UNIQUE INDEX (x)
  PARTITION BY LIST(region)...`, should we allow foreign keys to reference it as
  `(region,id)` (besides referencing it simply as `(id)`)? Doing so would save
  a fan-out query for validating the FK, but opens the question about what
  happens when the partitioning is removed from the index (and the column
  removed from the index). One is allowed to declare both indexes, thus
  allowing both single and double-column foreign keys, but it seems kinda
  wasteful.
3. The primary key would similarly not need to specify the implicit columns. For
the table above, the key can be specified as `PRIMARY KEY (id)`. The differences
...
indexes prefix, and in any other order than the partitioning order. So, the
...
need to be at the front, in the partitioning order.
...
tables”, in contrast with other table that may have different “partitioned
indexes”.
...
partitioning?
- What's the name of the column is created auto-magically by `CREATE TABLE..
  SET LOCALITY REGIONAL BY ROW`. `region` or `crdb_region`?
