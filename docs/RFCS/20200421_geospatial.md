...
geometries). SQL Datums can be either of these types, as can SQL table
columns.
...
* Geometry - for planar functionality compliant with the OGC spec. For
  ...
  geometries, which is lacking in twpayne/go-geom.
...
these geometry types "shapes". We are planning to support the
following:
...
default arguments. This will look like the following:
...
PostGIS representation to GEOS is slow.
...
* Users will have to specify the path of their GEOS install when
  ...
  * We should update our installation instruction page to include
    ...
    cockroach, or include it in the tarball and include instructions
    from the installation page to copy it into the correct
    location). This is similar to PostGIS where users would require
    extra steps to perform Geospatial operations.
...
which require calculations over a spheroid are done using
[GeographicLib](https://geographiclib.sourceforge.io/) (X/MIT
license).
...
plane. This is heavily inbuilt into the SQL type system (see SQL
...
* We will allow modifications from ONLY admin CRDB users (a departure
  ...
  being an extension), which will similarly come preloaded with a
  default set of spatial_ref_sys entries.
* We can also optionally look to "lock" in default SRIDs, preventing
  deletion entirely of "default" SRIDs (especially 4326 and 0).
* It will not be available for lookup in the public schema by default,
  unless we decide to persue what is described in "Availability on the
  Public Schema".
...
available as if it were available on the public schema. To resolve
this, we will add this system table as one of the resolvable table
...
installation of sqlite) and as such we may decide to also ship this as
a library which requires dlopen/dlsym as well.
...
improvements. We will prioritize improvements based on customer
