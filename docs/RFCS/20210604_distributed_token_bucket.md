...
type TokenBucketRequest struct {
  // Unique identifier for the operation; used to detect duplicate requests.
  OperationID util.UUID

  // Amount of requested tokens.
  RequestedTokens float64

  // Previous shares value for the node (equal to NewShares in the previous
  // request).
  // 0 if the node just started up.
  PreviousShares float64

  // Timestamp returned by the previous operation; unset if the node just
  // started up.
...
type TokenBucketResponse struct {
  ...
  // Used to pass back in the next request.
