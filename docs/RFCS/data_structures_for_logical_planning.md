...
The readers are encouraged to read these prior work RFCs, in
particular the RFC on logical planning by Peter, and make their own
experiments in code to accompany their understanding of the concepts
...
- the parser would transform an expression into an AST, like before.
  ...
  out of scope. This is out of scope - it will be treated separately
...
inside a class have the same semantics.* In particular, they share
the same properties.
...
comparisons. For this we need the new m-expression type `(and z)`
appears. `z` is an *integer set*, which we can conveniently encode as
a bitmap.
...
always apply the rule `(and (and x) y) = (and x (and y)) = (and (union
...
The properties part on each class encode the following:

- the expression's functional type (used to make sense of the bytes in
  memory).
- its other *logical properties*.

Additionally, we'll want later during optimizations to differentiate
m-expressions that are functionally equivalent but use different
strategies. For this we'll use *physical properties*, some of which
will separate classes (and thus be class properties) and some of which
will be specific to individual m-expressions.
...
Nevertheless, the following logical properties are likely to be
included no matter what, for every relational expression:

- its *result columns* as an integer set (bitmap)
- its *presentation order* as an integer list
- its *column labels* as a label list
- its *needed columns* as an integer set (bitmap), which are the
  columns outside of its definition needed by (scalar) expressions
  inside, non-empty when a (sub)query is correlated.
- (later, during optimizations) its *used columns* which must be
  fetched from KV and decoded in memory, as an integer set (bitmap)
  subset of the result columns

The result columns, presentation order and column labels correspond to
the data structure `ResultColumn`(s) in the current CockroachDB
code. The "needed columns" is not yet used in CockroachDB and must be
introduced to support correlated subqueries. The used columns set
corresponds to the `neededColumns` in the current CockroachDB code.

Some properties considered for scalar expressions:

- its needed columns, like above, to determine correlation.
- whether the expression is *constant* and can be pre-evaluated
  at the start (or before) execution. This is trivially true of
  literal m-expressions, but will also be true of non-correlated
  subqueries.
- whether the expression is *monotonic* in its operand,
  and thus able to preserve ordering information in some way.

Some example physical properties that are also considered,
based on the work by Radu in
[#17439](https://github.com/cockroachdb/cockroach/pull/17439),
[#18508](https://github.com/cockroachdb/cockroach/pull/18508), and
[#19097](https://github.com/cockroachdb/cockroach/pull/19097):

- *known orderings*, as a set of lists (arrays) of memo positions (classes)
- *key column set*, as an integer set (bitmap) of memo positions (classes)
- *column equivalency groups*, as an integer set (bitmap) of memo
  positions (classes)
