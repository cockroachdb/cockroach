...
ALTER TABLE tbl SET (expire_after = '5 minutes', ttl_select_batch_size=200);
...
```sql
ALTER TABLE tbl RESET (expire_after)
```
...
a blocking feature for the first iteration of TTL. This would involve adding
an additional column on the TTL table:

```
crdb_internal_expiration_override TIMESTAMPTZ DEFAULT NULL
```

Which would be used as the TTL value by the deletion job if the column value for
that row was not NULL. To make for an "infinite" expiration time, we would set
the TIMESTAMPTZ value as a very high value.
...
* Speed up the deletion by using indexes if one was created on the TTL column
  for the table.
* Users can specify their own column to use as TTL instead.
