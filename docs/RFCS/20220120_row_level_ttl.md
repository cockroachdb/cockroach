...
We will add a `ttl_expiration_expression` option which adds an expression to use
as the value for TTL expression, which replaces the TTL expiration condition.
This expiration must type resolve to a TIMESTAMPTZ (which can be NULL).
