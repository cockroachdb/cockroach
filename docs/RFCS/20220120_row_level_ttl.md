...
CREATE TABLE tbl (
   id INT PRIMARY KEY,
   text TEXT
) TTL -- if an interval is not specified, default to 30 days.
```

Users can add additional options to control their TTL (see full list of options
below) using a `(key=value, ...)` style syntax:
...
```sql
ALTER TABLE tbl SET TTL; -- introduce a TTL with default expiration.
...
ALTER TABLE tbl RESET TTL (deletion_cron) -- restore default option for the TTL
```
...
The DROP will drop the TTL column if it the column was not overriden by
the user with `expiration_column`.
...
`select_batch_size` | How many rows to fetch from the range that have expired at a given time. Defaults to 500. Must be at least `1`.
`delete_batch_size` | How many rows to delete at a time. Defaults to 100. Must be at least `1`.
`range_concurrency` | How many concurrent ranges are being worked on at a time. Defaults to `cpu_core_count`. Must be at least `1`.
...
`job_cron` | Frequency the job runs, specified using the CRON syntax.
...
## Introspection
The TTL definition for the table will appear in `SHOW CREATE TABLE`.

We will also introduce a `crdb_internal.table_ttl_options` table, which displays
the `(key=value, ...)` options within the TTL options, as well as extending
`crdb_internal.tables` to include a `ttl` column which is NULL if there is no
TTL or the interval if there is.
...
progress, which can be monitored using metrics graphs on the DB console:
...
An alternative SQL syntax for TTL would be to use the existing storage_parameter
syntax (`WITH (key = value)`) PostgreSQL provides. The main argument for this is
that we are not adding extra PG syntax.

The syntax would look as follows:

```sql
-- adding the TTL using CREATE
CREATE TABLE ... WITH (ttl_expire_after = <value>)
-- adding the TTL using ALTER
ALTER TABLE ... SET (ttl_expiration_column = 'blah')
-- configuring the TTL
CREATE TABLE ... WITH (ttl_expiration_column = <column>, ttl_expire_after = <interval>)
-- dropping the TTL
ALTER TABLE ... RESET (ttl_expire_after)
```

The main hangup for this is the surprise that occurs that setting a
storage_parameter introduces schema changes in the background, which is not the
case using the storage_parameter syntax in PG (the storage_parameters in PostgreSQL
today affect already necessary operations and does not mechanically change the
table). The feeling is that this is a high level enough change affecting query
semantics that it needs its own special clause.
...
## Open Questions
* Whether to use the alternative TTL syntax.
