...
This makes it possible for customers to willingly share log files and
other trace-related debugging data with CRL when CRL does not
otherwise have access to a running cluster due to the confidentiality
of the information stored therein, or confidentiality of the network
environment. It also opens the path to an “anonymous” mode for
`cockroach zip`.
...
are potentially PII-laden or confidential inside logs (and possibly
traces), and automatically erase the annotated parts when extracting
the data.
...
- [Reference-level explanation](#reference-level-explanation)
  - [Detailed design](#detailed-design)
    ...
    - [Discussion: usage of `log.Safe` vs `SafeMessager`](#discussion-usage-of-logsafe-vs-safemessager)
    - [Discussion: `SafeMessager` vs `SafeFormatter`](#discussion-safemessager-vs-safeformatter)
...
- every argument passed to `log.Infof` etc that is *not* enclosed in
  `log.Safe()` / `errors.Safe()` and does not implement the
  `errors.SafeMessager` interface is then enclosed by unicode markers
  "‹" and "›" the logging/tracing message.
  ...
  purpose of Sentry/telemetry reporting. We are reusing this contract
  here.)
...
### Discussion: usage of `log.Safe` vs `SafeMessager`

The proposal above is agnostic of how safe information is marked; both
the following are possible:

```go
log.Infof("hello %s", log.Safe("world"))
```

```go
log.Infof("hello %s", myWorldType{})
...
func (myWorldType) SafeMessage() string { return "world" }
```

Yet, the latter is strictly better and this may even cause us to
slowly deprecate `log.Safe` / `errors.Safe` entirely.

The problem is that nothing prevents the definition of the type/value
passed as argument to the log call from being changed far from the
logging site to start including sensitive information, without anyone
noticing.
...
The `SafeMessager` interface is less broken. How a value/struct turns
into a telemetry-safe report is exactly spelled out in the
implementation of the `SafeMessage()` method, and thus any alteration
there will clearly invoke scrutiny during review.

We will probably introduce a linter that rejects `log.Safe` unless it is
applied to a literal constant.

### Discussion: `SafeMessager` vs `SafeFormatter`

The current / original `SafeMessager` is defined by a function returning a Go string:
...
The problem with this interface is that it forces a string allocation
for every logging value, and then a string copy to the output buffer
in the logging function. These allocation costs, as experience has
shown, are detrimental to performance.

This problem has existed in multiple other areas of CockroachDB in the
past, and the same solution has been applied every time: evolve the
interface from "function retruning `string`" to "function taking a `Writer` as argument":
...
This evolution will be considered as part of the implementation of
this RFC.
...
- implements `SafeMessager` (nb: `log.Safe` also implements
  `SafeMessager`, so that's implicitly included here)
...
```
thatfile.go:123:  unsafeVar does not implement SafeMessager, use log.Unsafe(unsafeVar) instead
```
...
`SafeMessager` for their type (once and for all, which is more
