...
See [#16361](https://github.com/cockroachdb/cockroach/pull/16361) for more
discussion on this.
...
after' performance numbers (at the very least as a sanity check).</br>
...
application of the truncated entries have actually been persisted.  Given we
expect the `ReplicaState` to be stored in the first engine let's consider the
case where we've truncated a set of log entries and the corresponding
`TruncatedState`, to be stored on the first engine, is _not_ synchronized to
...
**NB**: RocksDB by default uses faster `fdatasync()` to sync files. We'll need
to use `fsync()` instead in filesystems like ext3 where you can lose files
after a reboot, this can be done so by setting `Options::use_fsync` to `true`
(for the `WriteBatch`).
...
None that are immediately obvious. We'll definitely have to pay attention to
how the separate RocksDB instances interact with one another.
...
in non-volatile memory instead of disk (for etcd/raft)<sup>[6]</sup>. Given
