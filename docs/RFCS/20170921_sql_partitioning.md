...
ALTER TABLE roachblog.articles PARTITION BY RANGE (published) (
  PARTITION archived VALUES < (now() - '30d'),
  PARTITION recent VALUES < MAXVALUE
);
```
...
Partition scheme:
  ...
  PARTITION LIKE <table-name>@<index-name> ( <col-names> );

List partition:
  PARTITION <partition-name> VALUES IN ( <expr>... )
...
CREATE TABLE roachmart.users (
  ...
  PARTITION australia VALUES ('AU', 'NZ') PARTITION BY RANGE (last_seen) (
    PARTITION australia_archived VALUES < (now() - '30d'),
    PARTITION australia_recent VALUES < MAXVALUE,
  ),
  PARTITION north_america VALUES ('CA', 'MX', 'US') PARTITION BY RANGE (last_seen) (
    PARTITION north_america_archived VALUES < (now() - '30d'),
    PARTITION north_america_recent VALUES < MAXVALUE
  ),
  ...
);
```
...
message PartitioningDescriptor {
  ...
  optional ForeignKeyReference reference = 4;
  repeated ForeignKeyReference referenced_by = 5;
}
```

The `reference` and `referenced_by` fields deserve additional explanation: they
track forward and reverse dependencies for indices that reference another
index's partitioning via `PARTITION LIKE`. While partition schemes can usually
be modified at will, partition schemes that are referenced by another index must
maintain the same number of columns while the reference exists. (If necessary,
we can loosen this restriction in the future.)
