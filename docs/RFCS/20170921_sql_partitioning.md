...
TODO: We’ll almost certainly want syntax to copy an index’s partitioning from a
table (and to keep it in sync with any changes to the table’s partitioning).
Perhaps:

```sql
CREATE INDEX ON roachmart.orders (user_country, part_id)
PARTITION LIKE roachmart.users (user_country);
```
...
Note that no zone config is necessary for the `archived` partition, as those
articles are free to be stored on SSDs provided there’s enough space. The zone
config for the `recent` partition ensures that the `recent` articles will never
be migrated to HDDs if the SSDs run low on space.
...
puts an effective limit on the number of partitions allowed on a table. Some
back of the envelope math suggests 60 bytes per partition, which is 64MB / 60B ~
1 million. Other partitioning implementations have limits less than 100,000, so
...
configs to target a partition, it needs to be on its own range. This could be
triggered either immediately after a `PARTITION BY` clause is updated, or lazily
when a zone config is added for the partition.

The former may create extra ranges if a table is partitioned, repartitioned, and
only then given zone configs (though this problem goes away when we support
range merges). The latter is more complex to implement and would violate a
user’s expectation of partitions living on separate ranges. We’ve decided on
`PARTITION BY` as the trigger.
...
even create the index on `hash(id)` (which preserves uniqueness but does not
provide fast lookups). Second, every secondary index increases the cost of
writes. For example, an unpartitioned, unique index on `roachmart.users` would
