...
but would bring back all the pain of manual sharding that CockroachDB was
...
ALTER TABLE roachmart.users PARTITION BY LIST (country) (
  PARTITION australia ('AU', 'NZ'),
  PARTITION north_america ('CA', 'MX', 'US'),
  PARTITION scandinavia ('DK', 'NO', 'SE'),
  PARTITION default (DEFAULT)
);
```
...
$ ./cockroach zone set roachmart.users.australia -f australia.zone.yml
...
$ ./cockroach zone set roachblog.articles.archived -f archived.zone.yml
```
...
Subpartitioning allow partitioning along several axes simultaneously. For
example, suppose RoachMart wants to age out users who haven’t logged in in
several months to slower hardware, while continuing to partition by country for
improved latency. With subpartitioning, this might be expressed like so:
...
CREATE TABLE roachmart.users (
  ...
  PARTITION australia ('AU', 'NZ') (
    SUBPARTITION australia_archived VALUES LESS THAN (now() - '30d'),
    SUBPARTITION australia_recent VALUES LESS THAN (MAXVALUE)
  ),
  PARTITION north_america ('CA', 'MX', 'US') (
    SUBPARTITION north_america_archived VALUES LESS THAN (now() - '30d'),
    SUBPARTITION north_america_recent VALUES LESS THAN (MAXVALUE)
  ),
  ...
);
```
...
so this is mostly a syntax and table descriptor extension.
...
as with e.g. a list partition that consists of non-adjacent elements.
...
One case requires special care to handle. Suppose `roachblog` and the `archived`
partition of `roachblog.articles` have custom zone configs, but the
`roachblog.articles` table itself and its `recent` partition do *not* have
custom zone configs. The `system.zones` table will necessarily have an entry for
`roachblog.articles` to store the custom zone config for the `archived`
partition, but that entry will have an otherwise empty `ZoneConfig`. This must
not be taken to mean that the `recent` partition has an empty zone config, but
that the zone config for the database, `roachblog`, applies. Distinguishing
between a table zone config override that is intentionally empty and the absence
of a table zone config entirely requires the addition of a flag, `table_active`,
to the `ZoneConfig` proto. When this flag is false, the table in question does
not have an active `ZoneConfig`.

The downside of this scheme is that the zone configs for all partitions of a
...
PostgreSQL 10 (which is as of yet unreleased), so we shouldn’t have
...
a collection of tables, which each have their own indexes, etc. It does not
allow for a global index across all partitions of a table and the global table
namespace is polluted with each partition.
