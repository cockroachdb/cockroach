...
As described below in the #scheduled-job-execution section below, the execution of the
schedule job happens under transaction.  As such, it is safe to attempt to change schedule
job settings even as the job is currently running.  Those changes will take effect
as soon as the schedule job change transaction commits.

If the cluster is down, the scheduled job system does not run; but we will restart
running scheduled jobs as soon as the nodes start up.  The initial implementation will
execute any schedule which was supposed to have run in the past.  The future versions
will allow for some flexibility to allow skipping of the "stale" scheduled runs
(see #scheduled-job-protocol-messages section below) 
