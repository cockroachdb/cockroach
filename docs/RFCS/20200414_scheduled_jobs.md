...
### `system.scheduled_job_runs` Table

Each run of the schedule is recorded in the `system.scheduled_job_runs` table:

```sql
CREATE TABLE system.scheduled_job_runs (
  job_id        INT NOT NULL PRIMARY KEY REFERENCES system.jobs (id) ON DELETE CASCADE,
  schedule_id   INT NOT NULL,
  job_completed BOOL DEFAULT false,
  INDEX (schedule_id),
)
```

The `job_id` and `schedule_id` columns establish schedule to job relation.
The `job_completed` is used in order to implement efficient polling,
...
#### Alternative: Modify `system.jobs` Table

As an alternative to the `system.scheduled_job_runs` table we could modify
`system.jobs` table instead to add schedule id column along with an
...
The benefit of the `system.scheduled_job_runs` table is that it decouples
scheduled jobs from `system.jobs`.  

On the flip side, using job runs table has few downsides:
  * job runs table is a bit less efficient (a bit of data duplication)
  * make some SQL statements (such as `SHOW JOBS`) which may want to 
display schedule related information a bit more complex.
  * would require a bit more complexity in the code to deal with the
    fact that `system.jobs` record may be deleted (garbage collected)
    asynchronously

Nonetheless, the benefit of the loosely couple system seem to outweigh
the downsides.
...
about the status of the completed jobs.  This is accomplished by
periodically polling `system.scheduled_job_runs` table:

```sql
SELECT S.*, J.id, J.status
FROM system.scheduled_jobs S, system.scheduled_job_runs R, system.jobs J
WHERE 
  R.schedule_id = S.schedule_id AND 
  NOT R.job_completed AND 
  R.job_id = J.id AND 
  J.status IN ('failed', 'succeeded', 'cancelled')
```

For each completed job run, the daemon will perform the following
actions under a single transaction:
   * Handle failed runs based on the schedule configuration (i.e. pause the schedule)
   * Invoke `NotifyJobTermination` to handle job completion.
   * Update `job_completed` to `true`
