...
ID be the highest ID + 1.
  - #### Pros:
    ...
    - Currently, OID is int64 and a column can be added easily during long running migration:
    `alter table t1 add column y oid default oid(unique_rowid());`
  - #### Cons:
    - To atomically add a NOT NULL column during long running migration, we have to come up with another function to 
    generate the ID here instead of `gen_random_uuid`. This will most likely be less efficient.
    - Due to [#41904](https://github.com/cockroachdb/cockroach/issues/41904), OID might be converted to int32 to match
    postgres and this means `unique_rowid()` can no longer be used as it generates int64 values. So without resolving 
