...
external function calls, and wrap them with a `scrub.Error` struct
...
During local engine execution we will generate the the necessary check
plans, then execute the plans in sequence on the distSQL execution
engine. These plans will be the ones such that will scan the data in the
distributed fashion and the only data leaving the nodes are any error
results. This strategy is similar to schema change index backfilling.
...
As an alternative for section 3, during query planning we can transform
