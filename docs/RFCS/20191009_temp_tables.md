...
- `SHOW TABLES` will show all PTs and only the TTs created by the session.
- `SHOW SCHEMAS` will show the temporary schema belonging to the session, if one was created. Again, this
...
#### SHOW TABLES
`SHOW TABLES` is a view that uses `pg_namespace` and `pg_class` to show tables under the supplied schema.
Ensuring this works correctly is equivalent to ensuring the underlying `pg_catalog` tables are
correctly populated.
...
- Name resolution for tables changes to (ParentDatabaseID, SchemaID, Table Name) -> TableDescriptorID.
...
- For all existing tables, the schemaID field is prefilled with 29 to scope them under `public`.
