...
number of round trips needed to serve KV requests.
...
Each leaseholder replica will maintain a map from `NodeID` to the number of
requests received from that node. Ideally, the request counts from each node
would decay exponentially over time. If this proves too difficult to implement
efficiently (e.g. in a 1000 node cluster with requests coming to each range from
each node), we could opt for swapping out the map with a new map periodically,
such as when we examine it to decide whether to transfer the replica's lease.
...
BatchRequest. The latter would likely be a better estimate of load, so we can
try that first.
...
determine when this could become a problem.
...
requests are coming from a single locality, it would be ideal from a performance
...
throughput is worse than it would be if the leases were properly balanced.
...
* Extending (or forking) `allocsim` to be able to set up different locality
  configurations with the new latency knob.
* Extending (or forking) `allocsim` to send load against the specific
  nodes/localities and measure the resulting throughput and latency.

The output of the new tool will enable us to better understand how different
...
without a testing tool like this.
...
* As mentioned above, it would be very beneficial if we came up with some way of
  * measuring the true load on each
  node, either as a collection of measurements or as some combination of CPU
  utilization, memory pressure, and disk I/O utilization. This would make
  improving locality and balancing load significantly easier and more effective.

* It's possible that this work will actually be harmful in certain cluster
  configurations. For example, if a range has two replicas in different parts of
  Europe and only a single replica in Australia, then it'd typically be better
  for the lease to be in Europe even if most of its requests are coming from
...
* As mentioned above, we could track the origin of each request by sending along
  the locality labels along with each request rather than just the source node
  ID, but that would add a lot of duplicated info along with every request.
...
* We may find it more effective to measure something other than number of
  ...
  are things that we could potentially experiment with using the new testing
  tool to see if they provide better balance.
