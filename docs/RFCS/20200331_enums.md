...
- Cockroach Issue: #TODO (rohany): Fill this out.
...
of the system. In particlar, we need to introduce a way of storing 
...
The SQL Features team is working on building a schema leasing system.
Using this leasing system, we can cache enum descriptors in memory
and invalidate entries when the lease is invalidated. 

Enums are scoped within a database, and are created within a schema.
...
* The name can change.
* The resident schema can change.
...
reads a newly written enum from disk and is unsure how to decode it. (TODO rohany: maybe 
write down an explicit example of how this could happen?).

In order to avoid these situations, we propose to apply a similar strategy
as the strategy for online schema changes. As a reminder, when we add a new
...
progression to adding new elements to an enum type. When a new value is added
to an enum, it is instead placed into a "read only" state. After all nodes
agree on the "read only" state, the new enum value is promoted into the 
set of writeable values in the enum. This process ensures that all nodes know
...
An additional strategy is to represent enum values with this byte array and an integer.
...
an increment/decrement of an integer rather than creating a new and possibly longer byte array.
