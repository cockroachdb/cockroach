...
of the enum's values to their physical representations. These descriptors
...
and cache invalidation of of type descriptors. Until the leasing system
...
compliant operations. Importantly, the OIDs cannot overlap with conflict
...
values. However, the first algorithm could be adapted to avoid this "worst case"
by not bisecting the range at the min and max key, but instead by just
adding/subtracting a small constant from the boundary key. This would allow
for more insertions at the ends of the range without adding more bytes.
...
`TypeDescriptor` that backs the type. A potential option was to avoid using
...
A new `Datum` `DEnum` will be introduced to hold to represent values of the
enums at runtime. A `DEnum` will store the physical representation of the
enum as well as the ID of the enum's `TypeDescriptor`.
