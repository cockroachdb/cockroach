...
This is achieved through the introduction of a new transaction status `STAGED` which can be written in parallel with the final batch.
...
The code first checks whether the batch is eligible for parallel commit. Initially, this means that the batch

- has no commit trigger,
- has no `MaxSpanRequestKeys` set,
- contains no ranged request with a limit,
- contains no self-overlap, and
- is not reverse.

These criteria were chosen to cover all cases relevant in practice:

- commit triggers are used for internal operations that are not overly latency-sensitive, and the restriction can be lifted easily by storing the commit trigger in the staged record and carrying it over into the committed one.
- adding limit handling to the status resolution process is complicated and the only request that would profit from it is a limited `DeleteRange`, which is unlikely to be issued on commit.
- self-overlap complicates checking the promised writes as we have to compute which sequence number to expect. This too can be lifted by computing the expected set of promised writes before compiling the staged record. However, it may need a reworking of our sequence numbers which are currently assigned too late to make this process straightforward.
- we have no reverse range writing requests, and thus creating a reverse ranged promised writes check is pointless.
...
When it is eligible, a copy of the `EndTransaction` request is created, with the status changed to `STAGED`, and the `PromisedWrites` field is populated from the other requests in the batch:
...
```
  ...
  // are not reverse, contain only non-overlapping requests, and have
  // no limits or commit triggers set. All of these restrictions can
  // be lifted in the future at the expense of additional complexity.
  repeated message PromisedWrite {
    bytes key = 1;
    bytes end_key = 2; // empty for point write
    int32 seq = 3;
  } promised_writes = 17;
```
...
In the happy case, all requests come back successfully. Errors (from above the routing layer) are propagated back to the client, with the client restarting or aborting the transaction as desired (to avoid forcing concurrent transactions into the status resolution process).
...
At the heart of the process is trying to prevent an intent of the transaction to be laid down (which is only possible if it isn't already there) at at the provisional commit timestamp. To achieve this efficiently, we introduce a new point read request, `PreventIntentRequest` and an analogous ranged version. This request populates the timestamp cache (as any read does) and returns whether there is an intent at the given key for the specified transaction and timestamp. It also returns whether  there was an intent of the transaction *above* the expected timestamp. This suggests that the transaction has restarted, and should instruct the caller to check the transaction record for a new update (since status resolution isn't kicked off unless a transaction looks abandoned, this may not be worth it in practice).
