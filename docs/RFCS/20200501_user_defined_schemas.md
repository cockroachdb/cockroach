...
- Cockroach Issue:
  [#26443](https://github.com/cockroachdb/cockroach/pulls/26443)
...
* In the release where user-defined schemas are introduced, all existing tables
  ...
  existing cross-database references are preserved.
    ...
    * New tables and enums created in `public` should probably go into the
      unupgraded `public` schema, to avoid complications from creating a new
      `public` schema and having two `public` schemas existing simultaneously.
