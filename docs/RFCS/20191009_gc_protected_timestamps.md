...
provide the specified semantics. We'll then detail how internal KV clients such
...
Instead we provide a weaker, more flexible guarantee which meets the present requirements
and admits a simpler architecture. In particular the solution presented here
requires only a constant number of transactional writes to protect a timestamp.
Verification in this proposal will need to perform `O(ranges)`
requests (but not `O(ranges)` writes).
...
* The GC timestamp must be newer the lease start time.
...
To do this we augment the volatile state on the leaseholder to include a minimum
timestamp at which the leaseholder is permitted to have read the protected
timestamp state in order to run GC. The leaseholder also stores the timestamp at
which it last attempted to run a GC on the leaseholder and it does so prior to
sending any `GCRequest`s. Just modifying the in-memory state of the leaseholder
is safe because we also added a condition that GC can only be run if the start
time of the lease is before the last read from the protected timestamp state. In
this way we ensure that if the lease were to move to a new node that the
protected timestamp state would be read again after that lease started before
running GC. If the same node were to restart it too would satisfy the safety
property as it would have to read the protected timestamp state after restarting
before running GC.
...
Note that this does not solve the problem of keeping data alive between
incremental ba
...
by a record. After a record has been verified it can be marked as verified so
