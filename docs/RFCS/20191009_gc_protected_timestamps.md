...
use of `CREATE TABLE AS`. These jobs can fail or be canceled. In
...
## Basic terminology
  ...
  * GC Threshold: When a range runs garbage collection it sets, in the
    ...
    This timestamp is called the GC Threshold.
  ...
  * Expiration: a row (henceforth a single MVCC version of a key) is said to
    ...
    `Now()` less the leaseholder's current view of its GC TTL. An "expired" row
    may still be read so long as its timestamp remains above the GC Threshold.
    Conceptually the expired time refers to the latest time which could possibly
    be set as the GC Threshold. This term does not use any new concepts but is
    being defined here to describe semantic properties later.
  ...
  * Protected Timestamp Record: the basic unit in the `protectedts` subsystem is
    ...
    (open question about partial coverings, say Range contains [a, c) and [a, b)
    is protected, will [b,c) get collected?). 
  ...
  * Protection Mode: each record has a protection mode. There are two 
    ...
    exactly this metadata looks like is an open question).
...
staleness of a Replica's current view of its GC TTL, though in practice it
is generally not more than a few seconds stale. Furthermore, we may not have
...
for of timestamps for requests. The GC threshold is updated during the GC
...
responsible for ensuring that records are eventually cleaned up.
...
which any data in the spans of the record have expired then the data in those
spans will be protected until that record is removed.
```
...
runtime. This leaves us with the revert-range case when using `CREATE TABLE AS
SELECT`. In this case we can augment the AddSSTable request to ensure that if
...
not GC anything in any range which has a protected timestamp.
...
* The records themselves are stored in a system table which has a well-known ID.
  ...
  * The primary key of the table is a UUID. The other `bytes` row contains
...
roughly every minute. For each key it would generate over 1000 rows of history. In order to perform incremental backup we'd only need to keep 2.
