...
UUID associated with each run of a check can be used to look up failures
...
|------------------|----------|-------------------------------------------------------------------------------------------------------------------------------------------|
| job_uuid         | `UUID`     | A UUID for the execution of a check. The job_uuid can be used to later lookup failures from the system table.                            |
...
| pkey_id          | `STRING`   | Primary key ID of the row with the error, cast to a string. If no primary key is involved, or retrievable, will be `NULL`.              |
...
## `SCRUB TABLE` and `SCRUB DATABASE` commands

The `SCRUB TABLE` and `SCRUB DATABASE` statements run a check on the
...
| CONSTRAINTS | `SCRUB CONSTRAINTS` | Force constraint validation checks on all SQL constraints in the table(s) or database(s).    |
...
```sql
SCRUB CONSTRAINTS (<table>|<constraint>)...
```

Rows returned by `SCRUB CONSTRAINT` will always have `constraint_name`
and `columns` present. `columns` will list all of the columns involved
in the constraint. If the constraint is `SCRUB`, then the list of
columns will be empty. `check_failure` types associated with `SCRUB
...
Rows returned by `SCRUB INDEX` will always have `constraint_name` and
...
`check_failure` types associated with `SCRUB INDEX` checks are as
follows:
...
Also, if the `check_failure` type provided requires `allow_data_loss`
when it hasn't been provided, an error will be thrown.

```sql
SCRUB TABLE mytable WITH PHYSICAL, REPAIR(encoding)
error: "\'allow_data_loss' is required when attempting to repair error type that may cause data loss: encoding"
```
...
2) Does this check involve an element that can be referenced that isn't
   ...
   If so, the check should exist as its own sub-command which is able
...
- What SQL queries will be available? What kind of options do we want to
  provide? How do we split them, or do we allow multiple ways to do the same checks? e.g. `CHECK TABLE table WITH PHYSICAL` vs
...
- Do these check queries run in the foreground, or in the background as
  a job? Can they be paused/resumed/canceled?
