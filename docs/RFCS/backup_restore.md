...
with backing it up. It flushes the RocksDB memtables into L0 sstables and grabs
a snapshot. If the node doesn't have the entire keyrange because a split has
happened in the interim, the rpc is failed and the coordinator retries with more
fine-grained segments. Alternately, the backup RPC could continue, and do a
...
end time. A running hash of key and value bytes is kept. When finished, the full
