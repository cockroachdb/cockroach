...
Any durable datastore is expected to have the ability to save a snapphot of data
...
Using sstables as the storage format allows us to use RocksDB's [AddFile]
method, which links a well-formed sstable directly into the LSM tree. These
files are easy to output from backup and this bypasses quite a bit of
computation. It could potentially be a huge performance win. However, there are
many details to work out, including validation of the files and keeping the
`MVCCStats` updated. It's possible that work done to support this could also be
used for snapshots.
...
backup's progress is written to [the `system.jobs` table](./system_jobs.md) and
...
the end time is streamed to the backup's configured basepath. The iteration must
either go through the Store at the right level for the Store's read/push/resolve
loop to take effect or implement such a loop itself. A running hash of key and
...
and thus avoids the write amplification, is described in the [Bulk
Import RFC](https://github.com/cockroachdb/cockroach/pull/15895).
...
`JobLease` with an expiration time of `now() + n`, where `n` is yet to be
decided but is likely to be on the order of a few minutes. (For context, schema
changes currently use five minute leases.) As long as the coordinating node is
alive, it will extend its lease approximately every `n / 2` minutes.
...
expired. With five minute leases, this is extremely unlikely, but still
possible—consider a network request that hangs for several minutes before
completing, for example. Thus, it must be safe for two nodes to perform the work
of the backup or restore simultaneously, but it need not be efficient.
...
`Import`. The space requirements of this approach are unfortunate: O(n)
in the number of ranges in the restore. For a 2TB restore with 2<sup>16</sup>
ranges and 16-byte split keys, this would require storing several _megabytes_ of
progress information. Storing this progress information in the `system.jobs`
table would require rewriting this several-megabyte blob on every checkpoint, so
we'd have to introduce a a separate SQL table, e.g. `CREATE TABLE
...
While the backup is in progress, the coordinator will stream
`BackupDescriptor_File` messages to a `PROGRESS` file in the backup directory.
To abstract away the details of "streaming uploads", we'll expose these
additional methods on the `ExportStorage` interface:

```go
type ExportStorage interface {
  // ...
  AppendFile(ctx context.Context, basename string, content io.ReadSeeker) error
  FlushFile(ctx context.Context, basename string)
}
```

Only `FlushFile` will guarantee that the data has been persisted to remote
storage; the coordinator will call `AppendFile` whenever an `ExportRequest`
returns, and `FlushFile` whenever job progress is updated (currently, every
second or every 5%).
...
preferable since they enable the use faster [AddFile] method.
...
slow. Bulk load, however, will generate any secondary indexes.
