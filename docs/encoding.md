...
for how CRDB encodes primitive values
...
covers the first format version, which predates column families,
interleaving, and composite encoding.
...
encoding is [prefix-free](https://en.wikipedia.org/wiki/Prefix_code).
...
introduces a [new `DECIMAL`
encoding](https://github.com/cockroachdb/cockroach/issues/13384#issuecomment-277120394),
since the old decoder will be retained for backward compatibility.

The format of the remaining bytes depends on the field type. The details
(in
[pkg/util/encoding/encoding.go](https://github.com/cockroachdb/cockroach/blob/master/pkg/util/encoding/encoding.go))
are irrelevant here except that, for primary key fields, these bytes
have the following order property. Consider a particular primary key
column and let enc be the mathematical function that maps SQL data in
that column to bytes.
...
([pkg/util/encoding/encoding.go](https://github.com/cockroachdb/cockroach/blob/master/pkg/util/encoding/encoding.go)),
which emits an unsigned integer with a variable-length encoding. The low
four bits of the integer contain the datum encoding type. The rest
contain the column ID difference. As an alternative for datum encoding
types greater than 14, `encodeValueTag` sets the low four bits to
`SentinelType` (15) and emits the actual datum encoding type next.
...
Before column families, non-sentinel KV values contained exactly one
datum, whose encoding was indicated by the one-byte value type (see
`MarshalColumnValue` in
[pkg/sql/sqlbase/table.go](https://github.com/cockroachdb/cockroach/blob/master/pkg/sql/sqlbase/table.go)).
Unlike the `TUPLE` encoding, this encoding did not need to be
prefix-free, which was a boon for strings. CRDB still constructs
non-sentinel KV values this way for single-column column families when
the ID of that column equals the `DefaultColumnID` of the column family
([pkg/sql/sqlbase/structured.proto](https://github.com/cockroachdb/cockroach/blob/master/pkg/sql/sqlbase/structured.proto)).
    ...
    /Table/51/1/1/0/1489427290.811792567,0 : 0xB244BD870A3505348D0F4272
           ...
           Table ID                            Checksum| |||
              ...
              Index ID                                 Value type (TUPLE)
                | |                                      |||
                id (primary key)                         Column ID difference
                  |                                       ||
    ...
    /Table/51/1/1/1/1/1489427290.811792567,0 : 0x30C8FBD403416C696365
           ...
           Table ID |                            Checksum| |
              ...
              Index ID                                   Value type (BYTES)
                | | |                                      |
                id (primary key)                           Datum encoding ('Alice')
                  | |
                  Column family ID (f1)
                    |
...
keys is defined by the [Unicode Collation
Algorithm](http://unicode.org/reports/tr10/), which may not even have
[an efficient partial
inverse](http://stackoverflow.com/q/23609457/2144669).

When collated strings and [(soon)
decimals](https://github.com/cockroachdb/cockroach/issues/13384#issuecomment-277120394)
...
By default, tables occupy disjoint KV key spans. CRDB users can request
that a table instead be interleaved with another table, which improves
the efficiency of joining them.

One table, the parent, must have a primary key that is a prefix (not
necessarily proper) of the other table, the child. To encode a KV key
for a row in the child table, encode it as if it were a row in the
parent table, then append the interleaving sentinel
...
indexes are termed secondary indexes. All indexes have an ID that is
...
secondary indexes constrain the table data not to have two rows that
agree on the indexed columns.
...
[pkg/sql/sqlbase/table.go](https://github.com/cockroachdb/cockroach/blob/master/pkg/sql/sqlbase/table.go).
Each row gives rise to one KV pair per secondary index, whose KV key has
...
list of indexed columns. The encoding differences are analogous to
