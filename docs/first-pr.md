...
You shouldn't merge yourself, instead, request our build bot Craig to do it for
you.  Craig is a [Bors merge bot], whose only job is to be gatekeeper to the
repository.  When you're ready to merge, add a comment on the PR of the form
`bors r=<reviewer>`, replacing `<reviewer>` with the person who gave you the
LGTM.  Once approved, your PR will be batched up with other PRs approved around
the same time.  Craig will try to build them as though they were merged to the
target branch, and if successful, will merge them.  This limits your
exposure by ensuring you don't accidentally merge a commit with an obviously
broken build or merge skew.
