...
- The remaining components are used adequatly by the statement types
...
- Executor: pgwire queues input SQL queries and COPY data packets to
  ...
  then translates the results to re response packets towards the
  client.
...
- constructs a memoized tree of "relational expression nodes".
- the resulting memoized tree *is* the logical plan.
...
**Role:** prepare the distribution of query execution and then
