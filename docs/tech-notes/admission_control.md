...
KV work concurrency slots are adjusted by monitoring the runnable
goroutine count, and additively decreasing or increasing the total
slots, when the runnable count is too high or too low. The adjustment
also takes into account current usage of slots. The exact heuristic
can be found in `admission.kvSlotAdjuster`. It monitors the runnable
count at 1ms intervals. The motivation for this high frequency is that
sudden shifts in CPU/IO ratio or lock cotention can cause the current
slot count to be inadequate, while leaving the CPU underutilized,
