...
- GRPC propagates cancel and deadline contexts across the wire.  Deadline
  ...
  sending a message GRPC does a [select](https://github.com/cockroachdb/vendored/blob/ed53f102e86b51c228b61d3ad54f1428982fe75e/google.golang.org/grpc/internal/transport/transport.go#L178) on the response and also listens
  on the Done channel for the context passed it by the client. This in turn will
  close the GRPC stream which will send an RST message the server which will
  cause the server side to be [canceled](https://github.com/cockroachdb/vendored/blob/ed53f102e86b51c228b61d3ad54f1428982fe75e/google.golang.org/grpc/internal/transport/http2_server.go#L1231).
  See also TestGRPCDeadlinePropagation and TestTenantStatementTimeoutAdmissionQueueCancelation for test that rely on this
  behavior.
