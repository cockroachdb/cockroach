...
6. If any of the `InitPut` commands fail, revert to the slow path. Create a new
   ...
   transaction. If this transaction fails due to an `InitPut` failure, that
   indicates an actual uniqueness violation or other error and the schema
   change is reversed.
...
Further, let's say the schema change flips into write/delete mode at time
`tWrite`, where `tScan > tWrite > 0`. Recall `tScan` is a fixed timestamp
chosen for the backfill's historical reads.
