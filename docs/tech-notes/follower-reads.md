...
A more extended description of CT follows, but let's start with how
CT can be used to implement follower reads, which is straightforward.
...
- **Closed timestamp**: the timestamp, before which reads may be
  ...
  index. This value will be in the past by at least a constant target
  duration. This value is transmitted with coalesced Raft heartbeats,
  for an entire store, on behalf of all ranges for which it is the
  leader / leaseholder.
...
- **Unquiesced ranges**: a set of range IDs which were previously
  ...
  the closed timsestamp in conjunction with explicit min log indexes
...
returned. The MPT is also accessed when sending CTs with Raft
...
- Return nothing. If the range is quiesced, then the store-wide CT can
  ...
  the range in unquiesced, the last-received per-range CT must be used
  in conjunction with the last min log index.
...
reset. Each heartbeat for a range updates the replica's `r.mu.ct`
struct, which keeps track of the closed timestamp, min log index pair.

When a read is serviced by a follower, that replica first checks if
the last-received CT for the range covers the read timestamp. If it
does, and the min log index has been committed, the read is
...
epoch, the *store-wide* CT can be used as long as the min log index
has been committed.
...
| Lose / regain leaseholdership | Range quiesced | If the L/LH loses the lease, the liveness epoch will have been incremented, preventing the use of new store-wide CT with old min log index. |
