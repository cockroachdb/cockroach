...
Author: Sumeer Bhola, Irfan Sharif
...
IO admission control<sup>[27]</sup> ensures that we only admit as much write
work as we're able to compact out of L0. This is to keep the LSM
intact<sup>[1]</sup> with respect to read-amplification, and is done so by
...
size<sup>[2]</sup> when admitting work. This same IO token scheme is used to
...
provides strict prioritization<sup>[3]</sup> and intra-tenant
fairness<sup>[4]</sup>, only admitting elastic work (like index backfills) when
...
raft we would simply deduct IO tokens without waiting; see <sup>[5]</sup> and
<sup>[22]</sup>. The idea here was that since we have little queueing ability
...
on that store to keep the LSM intact<sup>[6]</sup>.
...
- C2. For stores that observe a fair amount of follower write traffic, we're unable
  ...
  writes<sup>[5]</sup>, so nothing shaping it to observed L0 compaction
  ...
  latter as recently as <sup>[25]</sup>.
...
IO tokens below raft avoiding the hazards listed in <sup>[5]</sup>, while
...
<sup>[5]</sup>. To this end we introduce a non-blocking admission
interface<sup>[21]</sup>, used exclusively for below-raft IO work. This is
...
- Before doing so, we enqueue a "virtual" work item in IO work queue for the
  ...
  - The work item also includes the node ID where the proposal originated, and
    ...
    replica below-raft.
- We proceed write the log entry to stable storage and tick the raft state
...
- When work gets logically admitted, we record into the outbox the log position
  ...
  - We use `<log position, priority>` to indicate that all proposals of the given
    ...
    deducting tokens. This is discussed further below.
...
writes<sup>[7]</sup>. Users can elect to also use this flow control for all
regular writes<sup>[8]</sup> in cases where it makes sense<sup>[26]</sup> (with
...
- C2 and C3 are partially addressed by assuming regular write bandwidth is
  ...
  bandwidth/admission queueing/tenancy<sup>[9]</sup> into account in the
  allocator<sup>[10]</sup>.

See <sup>[7]</sup> and <sup>[11]</sup> for more discussion on the matter,
...
See `pkg/.../kvserver/flow_control_*.go`<sup>[16]</sup> for where we address all
the interactions above, and `I1-I13` in
`pkg/.../kvflowcontrol/doc.go`<sup>[23]</sup> for detailed commentary on each.
...
learnings here. The interactions listed above are intended to be exhaustive, 
...
we were able to claw back most performance regressions. We expect minimal
impact, certainly when only using replication admission control only for elastic
work where request rates are quite a bit lower.
...
- `kvadmission.flow_control.enabled` is the top-level switch that enables any of
  this machinery. When toggled, any work waiting for flow tokens is insta-admitted.
...
- `kvadmission.flow_controller.{elastic,regular}_tokens_per_stream` controls how
  ...
  `8MiB/200ms = 40MiB/s` of write bandwidth to a given replica. It also affects
...
grafana dashboard internally<sup>[28]</sup> and a subset of them surfaced in the
'Overload' dashboard in the UI. They're more easily interpreted on grafana but
we'll list them here anyway:
...
local fast path, and the number of nodes we're pending delivery to.
...
`/inspectz/kvflowhandles?ranges=<rangeid>,<rangeid>` (the query parameter is
optional). We've not found a need to use them directly, instead mostly using the
virtual tables above. The data returned looks like so:
...
- If `store=s1` and `store=s2` both independently issue regular follower writes
  on behalf of the same `tenant=t1` to `store=s3`, we make no fairness
  guarantees between the write bandwidth `store=s1` and `store=s2` are able to
  drive. After the initial burst, the aggregate bandwidth across the two origins
  will be equal to the IO admission rate of the shared receiver `store=s3`.
...
- Follower pausing (which abruptly stalls replication traffic to stores reaching
  ...
  <sup>[29]</sup>. We're deliberately reducing some availabililty despite no node
  ...
  levels of IO overload.
- Applying admission control only for bulk writes<sup>[30]</sup>. Since
...
See <sup>[14]</14> for a few other thought experiments. One of them is anchored
around "remote prediction".

## Future work

- Using elastic write flow control during raft log catchup. See <sup>[31]</sup>.
- Removing the per-replica quota pool. See <sup>[32]</sup>.
...
- Reduce fan-in by copysets.
- Talk about fan-in effect. Quiz based on different scenarios.
- ref to _index_backfill.go test
...
`pkg/kv/kvserver/kvflowcontrol`<sup>[15]</sup>, with integration code under
`pkg/kv/kvserver/flow_control_*.go`<sup>[16]</sup>. We're building on top of
library components in `pkg/util/admission`<sup>[17]</sup>. For the individual PRs building
up to this work, see <sup>[20]</sup>
...
See:
- <sup>[14]</sup> for an earlier working document,
- <sup>[12]</sup> and <sup>[13]</sup> for a high-level eng-biweekly presentation for this work, 
- <sup>[18]</sup> and <sup>[19]</sup> for a synchronous walkthrough of this work for/with
  knowledgeable reviewers (it's more lower-level+interesting than the
  eng-biweekly presentation),
- <sup>[7]</sup> and <sup>[11]</sup> for discussions on only using flow control for elastic work.
...
[5]: It's not possible to simply wait for IO tokens below raft. At this level
     we're simply told to append entries to the log (MsgApps) in order, and
     once we do, we tick the raft state machine, which triggers
     acknowledgements (MsgAppResps) to wherever the entries originated. 
...
[6]: We also recorded this bypassed work to keep the underlying linear
     models<sup>[2]</sup> accurate, but that's a minor detail. 
