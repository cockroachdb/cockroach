...
ranged intents and exclusive keyspan locks.
...
all, because the range tombstones' end bound is exclusive.
...
return nothing, while `Get(b)` would return `b5`. Again, more on this later.
...
All other KV APIs, such as `ClearRange` and `RevertRange`, also take MVCC range
tombstones into account as appropriate. See the KV API documentation for details.
...
be affected by the mutation (e.g. by fragmenting them). Two `UseRangeTombstone`
range deletions with abutting key spans will therefore be forced to serialize
with each other.
...
* `HasPointAndRange()`: whether the current position has a point and/or
  range key. At least one of these will be `true` for a valid iterator.
...
An `MVCCScan` with `Tombstones` across this span would emit synthetic MVCC point
tombstones at `a@4`, `b@4`, `c@4`, and `d@4`.
...
`MVCCScan` across `[bar-foo)` would emit synthetic MVCC point tombstones at
`bar@4` and `bar@2`, because this is the truncated start bound of the range
tombstones, but these synthetic point tombstones would not be visible to a scan
across `[abc-def)` which would instead see a synthetic tombstone at `abc@4`. The
same would happen if the CRDB range was split or merged at the key `bar`.

Additionally, an `MVCCGet` will emit synthetic point tombstones for the key even
...
* `RangeKeyBytes`: 48 (3 * 2 * 2 for encoded key bounds `a`, `b`, `b`, `c`,
  `c`, `d`, 4 * 9 for encoded timestamps `@1`, `@2`, `@1`, `@2`).
* `RangeValCount`: 4 (`[a-b)@1`, `[b-c)@2`, `[b-c)@1`, `[c-d)@2`).
...
historical versions also contribute encoded timestamp bytes, and so on. And if
we introduce non-tombstone range keys then liveness is determined in exactly the
same way as for point keys: if the top-most range key is not a tombstone then
that range key is considered live, and the previous range key becomes garbage at
its timestamp.

## MVCC Garbage Collection

During garbage collection, MVCC range tombstones are mostly equivalent to MVCC
point tombstones: keys covered by them will become eligible for GC when they are
no longer visible above the GC threshold, and the tombstones themselves become
eligible for GC when they fall below the GC threshold.
...
value when new keys are written above them. The message key span will be
truncated to the rangefeed subscription bounds as necessary.
