...
synchronized during the [transfer of power](#transfer-of-power).
...
range that coordinates a split becomes the LHS of the split. Maintaining this
symmetry means that a range's start key never changes during its lifetime,
while its end key may change arbitrarily in response to splits and merges.
...
mechanisms kick in here (write intents, the timestamp cache, the spanlatch
manager, etc.).
...
responsibilities:
  ...
  1. It ensures the end transaction request knows which intents it can resolve
     ...
     replica. If the intent was resolved asynchronously, we could attempt to
     load both the post-merge subsuming replica and the subsumed replica, which
     would overlap and crash the node.

  2. It adjusts the LHS's MVCCStats to incorporate the subsumed range's
     MVCCstats.

  3. It attaches a [merge payload to the replicated proposal result][pd-flag].
...
bumped to time 3. But because _P_ and _Q_ have separate spanlatch managers, no
...
queue, as no other commands can possibly execute in parallel with a command that
...
the [merge transaction](#merge-transaction) section. After all, applying a
widening snapshot is simply the bulk version of applying a merge command
directly. The details are again too complicated to go into here, but you can
begin your own exploration by starting with this call to
...
load that _Q_ is currently experiencing. The merge queue must send a
`RangeStats` RPC to collect this information from _Q_'s leaseholder, because
there is no guarantee that the current store is _Q_'s leaseholder—or that the
current store even has a replica of _Q_ at all.

To prevent unnecessary RPC chatter, the merge queue uses several heuristics to
avoid sending `RangeStats` requests when it seems like the merge is unlikely to
be permitted. For example, if it determines that _P_ and _Q_ store different
...
implementation of the freeze ab(uses) the spanlatch manager, to flush out
