...
| `max-file-size` | the approximate maximum size of individual files generated by this sink. If zero, there is no maximum size. |
| `max-group-size` | the approximate maximum combined size of all files to be preserved for this sink. An asynchronous garbage collection removes files that cause the file set to grow beyond this specified size. If zero, old files are not removed. |
| `buffered-writes` | specifies whether to buffer log entries. Setting this to false flushes log writes upon every entry. |
