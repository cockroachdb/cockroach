...
### `delete_statement_hints`

An event of type `delete_statement_hints` is recorded when a command to delete hints is
issued via information_schema.crdb_delete_statement_hints.


| Field | Description | Sensitive |
|--|--|--|
| `DeleteCount` | The number of hints being deleted. | no |
| `StatementFingerprints` | The target statement fingerprints for which inline hints are being deleted. | no |
| `HintIDs` | The hint IDs of the to-delete statement hint. | yes |
