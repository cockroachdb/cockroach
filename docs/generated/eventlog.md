...
### `schema_snapshot_descriptor`

An event of type `schema_snapshot_descriptor` is an event for schema telemetry, whose purpose is
...
so instead they're broken down into SchemaSnapshotDescriptor events which
...
is a set of SchemaSnapshotDescriptor messages sharing the same SnapshotID.


| Field | Description | Sensitive |
|--|--|--|
| `SnapshotID` | SnapshotID is the unique identifier of this snapshot. | no |
| `NumRecords` | NumRecords is how many SchemaSnapshotDescriptor events are in the snapshot. | no |
