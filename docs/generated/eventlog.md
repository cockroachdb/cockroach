...
An event of type `telemetry_event` is the SQL query event logged to the telemetry channel. It
...
| Field | Description | Sensitive |
|--|--|--|
| `EffectiveSampleRate` | The effective sampling rate of the telemetry event. When query load is low, the sampling rate is 1.0, indicating all queries are being sampled/represented. When the query load exceeds a designated threshold, probabilistic sampling is introduced to reduce logging load. Consequently, the effective sampling rate decreases, indicating the probability of the event being sampled/represented. The complement of the effective sampling rate (1 - effective sampling rate) is the probability of a telemetry event not being represented in the logs. | no |
