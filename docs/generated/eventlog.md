...
| Field | Description | Sensitive |
|--|--|--|
| `RecoveryType` | RecoveryType is the type of recovery described by this event. | no |
...
| `Options` | Options contain all of the options specified by the user in the BACKUP or RESTORE statement. For options that are accompanied by a value, only those with non-empty values will be present. | no |
