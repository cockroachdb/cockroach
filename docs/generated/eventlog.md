...
An event of type `delete_inline_hints` is recorded when a command to delete inline-hints
is issued via information_schema.crdb_delete_inline_hints or
crdb_internal.delete_hints.


| Field | Description | Sensitive |
|--|--|--|
| `StatementFingerprint` | The target statement fingerprint for which inline hints are being deleted. | no |
| `DonorSQL` | The donor SQL used to filter statement hints for deletion. | no |
| `ID` | The ID of the to-delete statement hint. | no |
