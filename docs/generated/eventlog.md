...
| `CaCert` | (Kafka Query Param) The base64-encoded ca_cert file | no |
...
| `SaslMechanism` | (Kafka Query Param) The SASL mechanism (ex: SASL-SCRAM-SHA-256, SASL-PLAIN) | yes |
...
| `FileSize` | (CloudStorage Query Param) A custom maximum file size for files before they are flushed | yes |
...
| `Resolved` | (Changefeed Option) The interval at which resolved timestamps are emitted | yes |
| `Envelope` | (Changefeed Option) Either key_only to emit only keys or wrapped for both key and value | yes |
| `Cursor` | (Changefeed Option) Whether a cursor timestamp has been specified to begin emitting events from | no |
| `Format` | (Changefeed Option) The data format, either JSON or Avro, that the changefeed emits | yes |
...
| `Compression` | (Changefeed Option) The compression format being used (ex: gzip) | yes |
| `ProtectDataFromGcOnPause` | (Changefeed Option) Whether data to resume a changefeed is protected from GC when paused | no |
| `SchemaChangeEvents` | (Changefeed Option) The type of schema events that trigger the change policy | yes |
| `SchemaChangePolicy` | (Changefeed Option) The behavior to trigger from a schema change event (ex: backfill) | yes |
| `Scan` | Whether the initial_scan or no_initial_scan options have been specified | yes |
...
| `Timestamp` | The timestamp of the event. Expressed as nanoseconds since the Unix epoch. | no |
| `EventType` | The type of the event. | no |
| `ChangefeedType` | The type of changefeed (ex: Enterprise) | yes |
| `CrdbVersion` | The CRDB version of the node that triggered the event | yes |
| `NodeID` | The ID of the node that triggered the event | no |
| `InstanceID` | The Instance ID of the running SQL server | no |
| `ClusterID` | The ID of the cluster containing the node that triggered the event | yes |
| `TenantID` | The ID of the tenant that triggered the event | no |
| `OrganizationID` | The ID of the organization linked to the cluster | yes |
...
| `Severity` | String value for the severity of the event | yes |
| `SinkType` | The scheme of sink being emitted to (ex: kafka, nodelocal, webhook-https) | yes |
| `JobID` | The ID of the job that triggered the event | no |
