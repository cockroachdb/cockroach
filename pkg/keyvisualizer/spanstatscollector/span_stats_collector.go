...
// discardStaleUpdates eagerly discards updates that will never be applied.
// An update should be discarded when there are other updates in the past
// that are more recent. discardStaleUpdates is not safe for concurrent use, and assumes
// that its callers acquire a lock on the boundaryUpdateQueue.
func (s *SpanStatsCollector) discardStaleUpdates() {
	for {

		if len(s.mu.boundaryUpdateQueue) < 2 {
			break
		}

		t1 := s.mu.boundaryUpdateQueue[0].scheduledTime
		t2 := s.mu.boundaryUpdateQueue[1].scheduledTime

		if t1.Before(t2) && t2.Before(timeutil.Now()) {
			// The boundary update at index 0 will never be applied,
			// so it can be proactively discarded.
			s.mu.boundaryUpdateQueue = s.mu.boundaryUpdateQueue[1:]
		} else {
			// The update at boundary index 0 should be applied,
			// so no more proactive discarding is needed.
			break
		}
	}
}
