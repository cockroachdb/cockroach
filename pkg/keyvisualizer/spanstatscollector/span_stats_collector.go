...
func discardStaleUpdates(updates []boundaryUpdate) []boundaryUpdate {
	for {
...
func (s *SpanStatsCollector) SaveBoundaries(boundaries []roachpb.Span, scheduledTime time.Time) {
	...
	s.mu.boundaryUpdateQueue = discardStaleUpdates(s.mu.boundaryUpdateQueue)
}
...
func (s *SpanStatsCollector) reset() {
	...
	if timeutil.Now().After(nextUpdate.scheduledTime) {
