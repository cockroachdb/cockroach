...
func (s *Subscriber) Start(
	...
	handleBoundaryUpdate func(update *keyvispb.BoundaryUpdate) error,
) error {
	...
	translateEvent := func(
		...
		decoded := keyvispb.BoundaryUpdate{}
		updateEncoded := []byte(tree.MustBeDBytes(datums[1]))

		if err := protoutil.Unmarshal(updateEncoded, &decoded); err != nil {
			log.Warningf(ctx, fmt.Sprintf("boundary decoding failed with error %v",
				err))
			return nil
		}

		err = handleBoundaryUpdate(&decoded)
		if err != nil {
			return nil
		}
		return nil
