...
`,
	...
	metaNetworkBytesEgress = metric.Metadata{
		...
		Help:        `Count of bytes sent via gRPC on connections we initiated.`,
		Measurement: "Bytes",
	}
	metaNetworkBytesIngress = metric.Metadata{
		Name:        "rpc.client.bytes.ingress",
		Unit:        metric.Unit_BYTES,
		Help:        `Count of bytes received via gRPC on connections we initiated.`,
		Measurement: "Bytes",
	}
)
...
func (m *Metrics) acquire(k peerKey, l roachpb.Locality) peerMetrics {
	...
	if !ok {
		...
		pm.release = func() {
			...
			delete(m.mu.peerMetrics, labelKey)
		}
	}
	pm.registeredCount++
	return pm
