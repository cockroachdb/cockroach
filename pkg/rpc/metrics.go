...
func (m *Metrics) acquire(k peerKey, l roachpb.Locality) (peerMetrics, localityMetrics) {
	...
	if !ok {
		pm = peerMetrics{
			...
			registeredCount:  new(int),
		}
		pm.release = func() {
			m.mu.Lock()
			defer m.mu.Unlock()

			*pm.registeredCount -= 1
			if *pm.registeredCount > 0 {
				return
			}
			if *pm.registeredCount < 0 {
				panic("registeredCount dropped below zero")
			}
			// All the gauges should be zero now, or the aggregate will be off
			// forever. Note that this isn't true for counters, as the aggregate
			// *should* track the count of all children that ever existed, even
			// if they have been released. (Releasing a peer doesn't "undo" past
			// heartbeats). We don't unlink or drop the child from the map
			// because we want to preserve the history of the connection if it
			// later reconnects.
	...
	if !ok {
		...
		lm.release = func() {
			...
			*lm.registeredCount -= 1
			if *lm.registeredCount > 0 {
				return
			}
			if *lm.registeredCount < 0 {
				panic("registeredCount dropped below zero")
			}
			lm.ConnectionConnected.Update(0)
		}
