...
// requirement will remains; if no connection existed yet,
...
func (ctx *Context) GRPCDialNode(target string, remoteNodeID roachpb.NodeID) *Connection {
	connKey := connKey{target, remoteNodeID}
	value, ok := ctx.conns.Load(connKey)
	if !ok {
		value, _ = ctx.conns.LoadOrStore(connKey, newConnectionToNodeID(ctx.Stopper, remoteNodeID))
	}
...
func (ctx *Context) runHeartbeat(
	...
	for {
		...
		// We re-mint the PingRequest to pick up any asynchronous update to clusterID and nodeID.
