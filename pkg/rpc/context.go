...
func (rpcCtx *Context) makeDialCtx(
	...
	// We want this to time out after the DialTimeout. We set the DialTimeout in
	// dialOptsNetwork but that doesn't always fire.
	ctx, _ := context.WithDeadline(dialCtx, timeutil.Now().Add(base.DialTimeout))
	return ctx
}
