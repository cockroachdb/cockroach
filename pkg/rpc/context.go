...
func NewServerWithInterceptor(
	...
	RegisterHeartbeatServer(s, &HeartbeatService{
		...
		disablePeerNameVerification:           ctx.disablePeerNameVerification,
...
func NewContext(
	...
	ctx := &Context{
		...
		disablePeerNameVerification: baseCtx.DisablePeerNameVerification,
	}
	...
	for {
		...
		if err := ctx.Stopper.RunTaskWithErr(ctx.masterCtx, "rpc heartbeat", func(goCtx context.Context) error {
			...
			if err == nil {
				...
				err = errors.Wrap(
					checkClusterName(
						ctx.disablePeerNameVerification || response.DisablePeerNameVerification,
						ctx.clusterName, response.ClusterName),
