...
func (rpcCtx *Context) GRPCDialRaw(target string) (*grpc.ClientConn, <-chan struct{}, error) {
	dialCtx := rpcCtx.makeDialCtx(target, 0, DefaultClass)
...
func (rpcCtx *Context) GRPCUnvalidatedDial(target string) *Connection {
	dialCtx := rpcCtx.makeDialCtx(target, 0, SystemClass)
...
func (rpcCtx *Context) GRPCDialNode(
	...
	dialCtx := rpcCtx.makeDialCtx(target, remoteNodeID, class)
	if remoteNodeID == 0 && !rpcCtx.TestingAllowNamedRPCToAnonymousServer {
		log.Fatalf(dialCtx, "%v", errors.AssertionFailedf("invalid node ID 0 in GRPCDialNode()"))
	}
			...
			if err := rpcCtx.Stopper.RunAsyncTask(
				dialCtx, "rpc.Context: grpc heartbeat", func(dialCtx context.Context) {
					...
					if err != nil && !grpcutil.IsClosedConnection(err) &&
						...
						log.Health.Errorf(dialCtx, "removing connection to %s due to error: %v", target, err)
					}
