...
func (rpcCtx *Context) removeConn(keys ...connKey) {
...
func (rpcCtx *Context) grpcDialRaw(
	...
	if backoff > minConnectionTimeout {
		...
		// max backoff.
		//
		// Note that MaxBackoff is kind of useless in our use case anyway, since
		// we don't let gRPC redial at all, see onlyOnceDialer. So all backing
		// off has to happen on our end.
...
func (rpcCtx *Context) GRPCDialPod(
	ctx context.Context, target string, remoteInstanceID base.SQLInstanceID, class ConnectionClass,
) *Connection {
...
func (rpcCtx *Context) grpcDialNodeInternal(
	...
	newConn := newConnectionToNodeID(rpcCtx.Stopper, remoteNodeID)
	newConn.class = class // TODO(tbg): put in constructor
	var loaded bool
	value, loaded = rpcCtx.conns.LoadOrStore(thisConnKeys[0], newConn)
	if loaded {
		// Someone else won the race and now there's a cached connection.
		// We just throw away newConn; it doesn't need any cleanup.
		return value.(*Connection)
	}
	if remoteNodeID != 0 {
		// If the first connection established to a target address is
	...
	// dialing and health checking of this connection. Until that reports back,
	// any other callers will block on `c.initialHeartbeatDone` in Connect().
	if err := rpcCtx.Stopper.RunAsyncTask(
		...
		"rpc.Context: heartbeat", func(ctx context.Context) {
			...
			defer rpcCtx.removeConn(thisConnKeys...)

			// Run the heartbeat; this will block until the connection breaks for
			// whatever reason.
			err := rpcCtx.runHeartbeat(ctx, newConn, target)
		...
		newConn.err.Store(errDialRejected)
		close(newConn.initialHeartbeatDone)
		rpcCtx.removeConn(thisConnKeys...)
	}
