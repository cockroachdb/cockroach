	...
	go func() {
		...
		handler := func(srv interface{}, stream grpc.ServerStream) error {
			adapter := muxRangeFeedServerAdapter{
				localStream: localStream{ctx: stream.Context()},
		...
		// This call is blocking.
		err := a.serverStreamInterceptors.run(a.server, responsePipe, muxRangefeedStreamInfo, handler)
		if err == nil {
			err = io.EOF
		}
		responsePipe.errC <- err
	}()
