...
func newTestContext(storageClusterID uuid.UUID, clock *hlc.Clock, stopper *stop.Stopper) *Context {
...
func TestHeartbeatCB(t *testing.T) {
	...
	testutils.RunTrueAndFalse(t, "compression", func(t *testing.T, compression bool) {
		...
		clock := hlc.NewClockWithTimeSource(hlc.NewManualClock(20), time.Nanosecond /* maxOffset */)
...
func TestInternalServerAddress(t *testing.T) {
	...
	clock := hlc.NewClockWithTimeSource(hlc.NewManualClock(1), time.Nanosecond /* maxOffset */)
...
func TestOffsetMeasurement(t *testing.T) {
	...
	serverClock := hlc.NewClockWithTimeSource(hlc.NewManualClock(20), time.Nanosecond /* maxOffset */)
	...
	clientClock := hlc.NewClockWithTimeSource(clientAdvancing, time.Nanosecond /* maxOffset */)
...
var _ hlc.NowSource = &AdvancingClock{}
...
func TestRemoteOffsetUnhealthy(t *testing.T) {
	...
	for i := range nodeCtxs {
		clock := hlc.NewClockWithTimeSource(hlc.NewManualClock(start.Add(nodeCtxs[i].offset).UnixNano()), maxOffset /* maxOffset */)
...
func TestClusterNameMismatch(t *testing.T) {
	...
	for i, c := range testData {
		t.Run(fmt.Sprintf("%d", i), func(t *testing.T) {
			...
			clientCtx := newTestContext(serverCtx.StorageClusterID.Get(), clock, stopper)
...
func BenchmarkGRPCDial(b *testing.B) {
	...
	clock := hlc.NewClockWithSystemTimeSource(250 * time.Millisecond /* maxOffset */)
