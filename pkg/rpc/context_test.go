...
func TestTestingKnobs(t *testing.T) {
	...
	for i := 0; i < numSysUnary; i++ {
		...
		if err := sysConn.Invoke(context.TODO(), unaryMethod, &ba, &br); err != nil {
			t.Fatal(err)
		}
	}
	...
	for i := 0; i < numDefStream; i++ {
		...
		if err := cs.SendMsg(&roachpb.BatchRequest{}); err != nil {
			t.Fatal(err)
		}
		var br roachpb.BatchResponse
		if err := cs.RecvMsg(&br); err != nil {
			t.Fatal(err)
		}
	...
	for call, num := range exp {
		if seen[call] != num {
			t.Fatalf("expected %d instances of %v in %v, got %d",
				num, call, seen, seen[call])
		}
	}
}
