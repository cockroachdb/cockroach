...
func TestGRPCDialClass(t *testing.T) {
	...
	require.Nil(t, err, "expected successful connection")
	require.NotEqual(t, sysConn1, defConn1, "expected connections dialed with "+
	...
	require.Equal(t, def1, def2, "expected connections dialed with the same "+
		"class to the same target to be the same")
	sys2 := clientCtx.GRPCDialNodeClass(remoteAddr, 1, SystemClass)
	require.Equal(t, sys1, sys2, "expected connections dialed with the same "+
		"class to the same target to be the same")
