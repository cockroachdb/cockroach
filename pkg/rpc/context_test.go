...
func newRegisteredServer(
	...
	require.NoError(t, stopper.RunAsyncTask(context.Background(), "serve", func(context.Context) {
		closeReason := s.Serve(&tracker)
		log.Warningf(context.Background(), "Closed with reason %v", closeReason)
	}))
	require.Nil(t, err)
...
func TestHeartbeatDialback(t *testing.T) {
	...
	// Initiate connection from node 1 to node 2 which will create a dialback connection back to 1.
	// This will be a blocking connection with no Dialback
	{
	...
	//Now connect back in the opposite direction. This should not dialback since we are already connected.
	{
		conn, err := ctx1.GRPCDialNode(remoteAddr2, 2, DefaultClass).Connect(ctx)
		defer func() {
			_ = conn.Close() // nolint:grpcconnclose
	...
	{
		// Make sure we get blocking requests in both directions now.
		pingReq := <-pingChan2
		require.Equal(t, PingRequest_NON_BLOCKING, pingReq.NeedsDialback)
	}
	{
		pingReq := <-pingChan1
		require.Equal(t, PingRequest_NON_BLOCKING, pingReq.NeedsDialback)
	...
	// Verify we are fully healthy in both directions.
	require.Equal(t, nil, ctx1.ConnHealth(remoteAddr2, 2, DefaultClass))
	require.Equal(t, nil, ctx2.ConnHealth(remoteAddr1, 1, DefaultClass))

	// Now shut down listener 1 and the connection that node1 -> node2.
	// Verify the reverse connection will also close.
	_ = ln2.Close()
	...
	for {
		...
		case <-time.After(1 * time.Second):
			log.Info(ctx, "No more pings")
			require.ErrorAs(t, ctx1.ConnHealth(remoteAddr2, 2, DefaultClass), &ErrNoConnection)
			require.ErrorAs(t, ctx2.ConnHealth(remoteAddr1, 1, DefaultClass), &ErrNoConnection)
			return
		}
	}
}
