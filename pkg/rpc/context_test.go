...
func TestRemoteOffsetUnhealthy(t *testing.T) {
	...
	for i, clientNodeContext := range nodeCtxs {
		for j, serverNodeContext := range nodeCtxs {
			...
			if _, err := clientNodeContext.ctx.GRPCDial(serverNodeContext.ctx.Addr).Connect(context.TODO()); err != nil {
				t.Fatal(err)
			}
		}
	}
...
func TestClusterIDMismatch(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		wg.Add(1)
		go func() {
			_, err := clientCtx.GRPCDial(remoteAddr).Connect(context.TODO())
...
func BenchmarkGRPCDial(b *testing.B) {
	...
	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
			_, err := ctx.GRPCDial(remoteAddr).Connect(context.TODO())
			if err != nil {
				b.Fatal(err)
			}
		}
