...
func TestClusterNameMismatch(t *testing.T) {
	...
	testData := []struct {
		...
		{"a", true, "", false, ``},
		{"", true, "a", false, ``},
		{"a", true, "b", false, `local cluster name "b" does not match peer cluster name "a"`},
		{"a", false, "", true, ``},
		{"", false, "a", true, ``},
		{"a", false, "b", true, `local cluster name "b" does not match peer cluster name "a"`},
		{"a", true, "", true, ``},
		{"", true, "a", true, ``},
		{"a", true, "b", true, `local cluster name "b" does not match peer cluster name "a"`},
	}
	...
	for i, c := range testData {
		t.Run(fmt.Sprintf("%d", i), func(t *testing.T) {
			...
			serverCtx.disablePeerNameVerification = c.serverDisablePeerCheck
			...
			RegisterHeartbeatServer(s, &HeartbeatService{
				...
				disablePeerNameVerification: serverCtx.disablePeerNameVerification,
			})
			...
			clientCtx.disablePeerNameVerification = c.clientDisablePeerCheck
