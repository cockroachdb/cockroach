...
var systemClassKeyPrefixes = [][]byte{
	[]byte(keys.NodeLivenessPrefix),
}
...
func ConnectionClassForKey(key roachpb.RKey) ConnectionClass {
	// An empty RKey will address range 1 and will occur in its range descriptor
	if len(key) == 0 ||
		// Directly check equality for the meta1 prefix.
		key[0] == keys.Meta1Prefix[0] {
		return SystemClass
	}
