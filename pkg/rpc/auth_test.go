...
func TestAuthenticateTenant(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{systemID: stid, ous: []string{"foo"}, commonName: "other",
			expErr: `need root or node client cert to perform RPCs on tenant system \(cert valid for "other" on all tenants\)`},
		...
		{systemID: stid, ous: nil, commonName: "root", tenantScope: 10,
			expErr: `need root or node client cert to perform RPCs on tenant system \(cert valid for "root" on tenant 10\)`},
