		...
		"/cockroach.roachpb.Internal/Batch": {
			{
				...
				expErr: `requested key span /Max not fully contained in tenant keypace /Tenant/1{0-1}`,
			},
			{
				req: &roachpb.BatchRequest{Requests: makeReqs(
					makeReq("a", "b"),
				)},
				expErr: `requested key span {a-b} not fully contained in tenant keypace /Tenant/1{0-1}`,
			},
			{
				req: &roachpb.BatchRequest{Requests: makeReqs(
					makeReq(prefix(5, "a"), prefix(5, "b")),
				)},
				expErr: `requested key span /Tenant/5"{a"-b"} not fully contained in tenant keypace /Tenant/1{0-1}`,
			},
			{
				req: &roachpb.BatchRequest{Requests: makeReqs(
			...
			},
			{
				...
				expErr: `requested key span /Tenant/50"{a"-b"} not fully contained in tenant keypace /Tenant/1{0-1}`,
			},
			{
				req: &roachpb.BatchRequest{Requests: makeReqs(
					makeReq("a", "b"),
				...
				expErr: `requested key span {a-/Tenant/5"b"} not fully contained in tenant keypace /Tenant/1{0-1}`,
			},
			{
				req: &roachpb.BatchRequest{Requests: makeReqs(
					makeReq(prefix(5, "a"), prefix(5, "b")),
				...
				expErr: `requested key span /Tenant/{5"a"-10"b"} not fully contained in tenant keypace /Tenant/1{0-1}`,
			},
			{
				req: &roachpb.BatchRequest{Requests: makeReqs(
					makeReq("a", prefix(10, "b")),
				)},
				expErr: `requested key span {a-/Tenant/10"b"} not fully contained in tenant keypace /Tenant/1{0-1}`,
			},
			{
				req: &roachpb.BatchRequest{Requests: makeReqs(
					makeReq(prefix(10, "a"), prefix(20, "b")),
				)},
				expErr: `requested key span /Tenant/{10"a"-20"b"} not fully contained in tenant keypace /Tenant/1{0-1}`,
			},
		},
		"/cockroach.roachpb.Internal/RangeLookup": {
			{
				req:    &roachpb.RangeLookupRequest{},
				expErr: `requested key /Min not fully contained in tenant keypace /Tenant/1{0-1}`,
			},
			{
				req:    &roachpb.RangeLookupRequest{Key: roachpb.RKey("a")},
				expErr: `requested key "a" not fully contained in tenant keypace /Tenant/1{0-1}`,
			},
			{
				req:    &roachpb.RangeLookupRequest{Key: roachpb.RKey(prefix(5, "a"))},
				expErr: `requested key /Tenant/5"a" not fully contained in tenant keypace /Tenant/1{0-1}`,
			},
			{
			...
			},
			{
				...
				expErr: `requested key /Tenant/50"a" not fully contained in tenant keypace /Tenant/1{0-1}`,
			},
		},
		"/cockroach.roachpb.Internal/RangeFeed": {
			{
				req:    &roachpb.RangeFeedRequest{},
				expErr: `requested key span /Min not fully contained in tenant keypace /Tenant/1{0-1}`,
			},
			{
