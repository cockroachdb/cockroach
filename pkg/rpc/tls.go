...
func (sctx serverParameters) AdvertiseAddr() string {
...
func (sctx serverParameters) SQLAdvertiseAddr() string {
	if sctx.sqlAdvertiseAddr == nil {
		panic(errors.AssertionFailedf("programming error: cannot use SQL server address in client command"))
	}
	if *sctx.sqlAdvertiseAddr == "" {
		panic(errors.AssertionFailedf("programming error: cannot use the SQL advertised address before the server has started listening on the network"))
	}
...
type SecurityContext struct {
	...
	// username is only used when connecting to another RPC instance.
	// For CockroachDB node processes, this is security.NodeUser.
	username security.SQLUsername

	// serverCfg is only used on server processes.
	serverCfg serverParameters
...
func MakeSecurityContext(
	cfg SecurityConfig, tlsSettings security.TLSSettings, tenID roachpb.TenantID,
) SecurityContext {
	switch scfg := cfg.(type) {
	case serverSecurityConfig:
		return makeSecurityContextInternal(scfg.CommonConfig, security.NodeUserName(), scfg.serverParameters, tlsSettings, tenID)
	case ClientSecurityConfig:
		return makeSecurityContextInternal(scfg.CommonConfig, scfg.User, serverParameters{}, tlsSettings, tenID)
	default:
		panic(errors.AssertionFailedf("programming error: unknown config type %T", cfg))
	}
}

func makeSecurityContextInternal(
	cfg CommonConfig,
	username security.SQLUsername,
	serverCfg serverParameters,
	...
	return SecurityContext{
		CertsLocator: security.MakeCertsLocator(cfg.CertsDir),
...
func (ctx *SecurityContext) GetCertificateManager() (*security.CertificateManager, error) {
	ctx.lazy.certificateManager.Do(func() {
		...
		ctx.lazy.certificateManager.cm, ctx.lazy.certificateManager.err =
			security.NewCertificateManager(ctx.cfg.CertsDir, ctx, opts...)

		if ctx.lazy.certificateManager.err == nil && !ctx.cfg.Insecure {
...
func (ctx *SecurityContext) GetUIServerTLSConfig() (*tls.Config, error) {
	// Early out.
	if ctx.cfg.Insecure || ctx.serverCfg.disableTLSForHTTP {
		return nil, nil
	}
