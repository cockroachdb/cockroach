...
// config. When called on a tenant instance, will return the
// server TLS config for the node, instead of the DB Console config.
// This choice was made to keep configuration simpler on the tenant, and
// due to the fact that tenant HTTPS access is currently mediated
// through a proxy.
//
