...
func (ctx *SecurityContext) getClientCertPaths(user string) (string, string, error) {
	if user == security.NodeUser {
		return ctx.NodeCertPath(), ctx.NodeKeyPath(), nil
	}
	return ctx.ClientCertPath(user), ctx.ClientKeyPath(user), nil
}
