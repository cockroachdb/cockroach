...
type Dialer struct {
	...
	defaultBreakers syncutil.IntMap // map[roachpb.NodeID]*wrappedBreaker
	systemBreakers  syncutil.IntMap // map[roachpb.NodeID]*wrappedBreaker
}
...
func (n *Dialer) getBreaker(nodeID roachpb.NodeID, class rpc.ConnectionClass) *wrappedBreaker {
	var breakers *syncutil.IntMap
	switch class {
	case rpc.DefaultClass:
		breakers = &n.defaultBreakers
	case rpc.SystemClass:
		breakers = &n.systemBreakers
	default:
		panic(fmt.Errorf("unknown rpc.ConnectionClass %d", class))
	}
