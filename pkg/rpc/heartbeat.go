...
func checkClusterName(disablePeerNameVerification bool, clusterName string, peerName string) error {
	if disablePeerNameVerification && (clusterName == "" || peerName == "") {
		// A cluster uses --disable-peer-name-verification to gradually introduce
		// a cluster name where none was known before.
		// In that case, we tolerate whyen either side doesn't know the cluster
		// name yet.
		// However, if both sides know a cluster name, we still check it below.
		return nil
	}
...
func (hs *HeartbeatService) Ping(ctx context.Context, args *PingRequest) (*PingResponse, error) {
	...
	return &PingResponse{
		...
		DisablePeerNameVerification: hs.disablePeerNameVerification,
	}, nil
}
