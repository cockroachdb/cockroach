...
// HeartbeatStreamServer interface in addition to the HeartbeatServer interface.
// Instead of providing a request-response model, the service reads on its input
// stream and periodically sends on its output stream with its latest ping
// response. This means that even if the service stops receiving requests, it
// will continue to send responses.
type HeartbeatStreamService struct {
	HeartbeatService
	interval time.Duration
}

func (hss *HeartbeatStreamService) PingStream(stream HeartbeatStream_PingStreamServer) error {
	ctx := stream.Context()
...
type lockedPingStreamClient struct {
	HeartbeatStream_PingStreamClient
...
func (c *lockedPingStreamClient) Send(req *PingRequest) error {
	...
	return c.HeartbeatStream_PingStreamClient.Send(req)
}
