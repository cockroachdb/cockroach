...
func TestStatsHandlerBasic(t *testing.T) {
	...
	expResults := map[string]*Stats{
		"10.10.1.3:26257": &Stats{},
		"10.10.1.4:26257": &Stats{},
	}
	var sh StatsHandler
	...
	expResults["10.10.1.3:26257"].incoming += 10
	...
	cStats1.HandleRPC(ctx, &stats.InHeader{WireLength: 17})
	cStats1.HandleRPC(ctx, &stats.InPayload{WireLength: 19})
	cStats1.HandleRPC(ctx, &stats.InTrailer{WireLength: 23})
	expResults["10.10.1.3:26257"].incoming += 59

	cStats2 := sh.newClient("10.10.1.4:26257")
	cStats2.HandleRPC(ctx, &stats.OutPayload{WireLength: 29})
	cStats2.HandleRPC(ctx, &stats.OutTrailer{WireLength: 31})
	expResults["10.10.1.4:26257"].outgoing += 60
...
func TestStatsHandlerWithHeartbeats(t *testing.T) {
	...
	testutils.SucceedsSoon(t, func() error {
		...
		// Verify that server stats mirror client stats.
		if s, c := serverVal.(*Stats).incoming, clientVal.(*Stats).outgoing; s != c {
			return fmt.Errorf("expected server.incoming == client.outgoing; got %d, %d", s, c)
		}
		if s, c := serverVal.(*Stats).outgoing, clientVal.(*Stats).incoming; s != c {
