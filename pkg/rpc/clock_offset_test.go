...
func TestStatsFuncs(t *testing.T) {
	...
	floatWithinReasonableTolerance := func(t *testing.T, expected, actual float64) {
		...
		withinTolerance := cmp.Equal(expected, actual, cmpopts.EquateApprox(0, tolerance))
		if !withinTolerance {
			t.Errorf("values outside tolerance %f (expected) - %f (actual) = %f", expected, actual, tolerance)
		}
	}
