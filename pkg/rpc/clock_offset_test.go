...
func TestVerifyClockOffset(t *testing.T) {
	...
	clock := hlc.NewClockWithTimeSource(hlc.NewManualClock(123), 50*time.Nanosecond /* maxOffset */)
	monitor := newRemoteClockMonitor(clock, time.Hour, 0)
...
func TestClockOffsetMetrics(t *testing.T) {
	...
	clock := hlc.NewClockWithTimeSource(hlc.NewManualClock(123), 20*time.Nanosecond /* maxOffset */)
	monitor := newRemoteClockMonitor(clock, time.Hour, 0)
