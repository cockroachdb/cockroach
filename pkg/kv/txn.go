		...
		if err != nil {
			if errors.HasType(err, (*kvpb.UnhandledRetryableError)(nil)) {
				...
				if !txn.IsRetryableErrMeantForTxn(*t) {
					...
					// We might get a TransactionRetryWithProtoRefreshError if the closure
					// ran another transaction internally and let the error propagate
					// upwards.
...
func (txn *Txn) IsRetryableErrMeantForTxn(
	...
	return retryErr.TxnID == txn.mu.ID
}
