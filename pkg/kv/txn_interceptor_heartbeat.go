...
type txnHeartbeat struct {
	...
	mu struct {
		...
		// EndTransaction can be elided - we want to allow multiple rollback
		// attempts to be sent and the first one stops the heartbeat loop.
...
func (h *txnHeartbeat) SendLocked(
	...
	if haveEndTxn {
		...
		needEndTxn := h.mu.everSentBeginTxn
