...
type txnHeartbeat struct {
	...
	// everSentBeginTxnToServerLocked is a callback that says whether a
	// BeginTransactionRequest (of possibly multiple) was ever sent to the server.
	// This is hooked up to the txnLockGatekeeped, which is the ultimate source of
	// truth: it covers the cases where any interceptor rejected the batch.
	// If a BeginTxn was ever sent, then an EndTransaction needs to eventually be
	// sent and cannot be elided.
	...
	everSentBeginTxnToServerLocked func() bool

	// mu contains state protected by the TxnCoordSender's mutex.
	mu struct {
		sync.Locker

		// hbStartNanos is the wall time at which the heartbeat is started. This is
		// around the time the client sent the first write.
		hbStartNanos int64
...
func (h *txnHeartbeat) init(
	...
	h.everSentBeginTxnToServerLocked = everSentBeginTxnToServerLocked
...
func (h *txnHeartbeat) SendLocked(
	...
	if needBeginTxn {
		...
		// Start the heartbeat loop. Perhaps surprisingly, this needs to be done
		// even if the batch also has an EndTransaction. Although on batch success
		// the heartbeat loop will be stopped right away, on retriable errors we
		// need the heartbeat loop: the intents and txn record should be kept in
		// place just like for non-1PC txns.
		//
		// Note that we don't start the heartbeat loop if the loop is already
		// running. That can happen because of send BeginTransaction again after
		// retriable errors.
		if h.mu.txnEnd == nil {
	...
	etReq, haveEndTxn := ba.Requests[lastIndex].GetInner().(*roachpb.EndTransactionRequest)
	var elideEndTxn bool
	var swallowETError bool
	if haveEndTxn {
		// Are we writing now or have we written in the past?
		needEndTxn := haveTxnWrite || h.everSentBeginTxnToServerLocked()
		elideEndTxn = !needEndTxn
		if elideEndTxn {
			...
			if h.mu.needBeginTxn {
				...
				etReq.Commit = false
				// We swallows errors from this rollback. In particular, we might see
				// TransactionStatusError is the transaction record was never written.
				swallowETError = true
			}
		}
	}
	...
	if pErr != nil {
		// Maybe swallow errors from the final EndTransaction.
		if swallowETError && pErr.Index != nil && pErr.Index.Index == lastIndex {
			pErr = nil
			if br == nil {
				br = ba.CreateReply()
				txn := ba.Txn.Clone()
			...
			numResponses := len(br.Responses)
			if numResponses == 0 || br.Responses[numResponses-1].GetEndTransaction() == nil {
	...
	if br.Txn.Status == roachpb.COMMITTED && needBeginTxn {
		h.metrics.Commits1PC.Inc(1)
	}
...
func (h *txnHeartbeat) startHeartbeatLoopLocked(ctx context.Context) error {
	...
	h.mu.hbStartNanos = h.clock.PhysicalNow()
...
func (h *txnHeartbeat) heartbeatLoop(ctx context.Context) {
	...
	defer func() {
		...
		duration, restarts, status := h.finalTxnStatsLocked()
		h.mu.Unlock()
		h.updateStats(duration, restarts, status)
	}()
...
func (h *txnHeartbeat) finalTxnStatsLocked() (duration, restarts int64, status roachpb.TransactionStatus) {
	duration = h.clock.PhysicalNow() - h.mu.hbStartNanos
	restarts = int64(h.mu.txn.Epoch)
	status = h.mu.txn.Status
	return duration, restarts, status
}

// updateStats updates transaction metrics after the transaction finishes.
func (h *txnHeartbeat) updateStats(duration, restarts int64, status roachpb.TransactionStatus) {
	h.metrics.Durations.RecordValue(duration)
	h.metrics.Restarts.RecordValue(restarts)
	switch status {
	case roachpb.ABORTED:
		h.metrics.Aborts.Inc(1)
