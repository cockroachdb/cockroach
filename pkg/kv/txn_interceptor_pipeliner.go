...
type txnPipeliner struct {
	...
	// in this epoch and the lock spans from all locking reads that have been
	// performed in this epoch. Additionally, the span set contains all locks
	// held at the end of prior epochs. All of the transaction's in-flight
	// writes are morally in this set as well, but they are not stored here to
	// avoid duplication.
	//
