...
//    evaluate in.
//
// The interceptor queries all outstanding writes before committing a
// transaction by tacking on a QueryIntent request for each one to the front of
// an EndTransaction(Commit=true) requests. The result of this is that the
...
// outstanding writes without resolving any of them, and the more of these
// writes there are, the more chance querying one of them gets delayed and
// delays the overall transaction.
//
...
// would be in the txnPipeliner's best interest to resolve outstanding writes as
// early as possible, even if no other overlapping requests force their
// resolution. The approaches are:
...
//    complete until after an EndTransaction synchronously needed to resolve them
//    anyway.
// 2. to address the issue of an unbounded number of background goroutines
//    resolving writes in approach 1, a single background goroutine could be run
//    that repeatedly loops over all outstanding writes and attempts to resolve
...
//    reason as approach 1: most of it's QueryIntent requests will be useless
...
//    request and be notified immediately after it "replication" phase completes.
//    This would allow txnPipeliner to resolve outstanding writes immediately after
...
// outstandingWrite represents a point write at a given sequence that has
// been proposed into Raft but has not been confirmed to have succeeded.
// outstandingWrites are "resolved" when they are proven to have resulted
// in replicated intents by a successful QueryIntent request.
...
func (tp *txnPipeliner) SendLocked(
	...
	br = tp.updateOutstandingWrites(ba, br)
	return br, nil
}
...
func (tp *txnPipeliner) chainToOutstandingWrites(ba roachpb.BatchRequest) roachpb.BatchRequest {
	...
	for i, ru := range oldReqs {
		if tp.outstandingWritesLen() == 0 && !asyncConsensus {
			// If there are no outstanding writes and async consensus is
			// disallowed, short-circuit immediately.
			break
		}
		...
		if tp.outstandingWritesLen() > 0 {
			...
			itemIter := func(item btree.Item) bool {
				...
				// so fork it before modifying it. Truncating the capacity will
				// force all future appends to re-allocate a new slice.
				if !forked {
			...
			if !roachpb.IsTransactional(req) {
				...
				// pipeline by chaining on to all outstanding writes.
	...
	// Set the batches AsyncConsensus flag based on whether AsyncConsensus is
...
func (tp *txnPipeliner) updateOutstandingWrites(
	...
	// tree. This will turn maybeRemoveResolvedWriteLocked into a quick no-op.
	...
	for i, ru := range ba.Requests {
		...
		if qiReq, ok := req.(*roachpb.QueryIntentRequest); ok {
			// Resolve any outstanding writes that were proven to exist.
			if resp.(*roachpb.QueryIntentResponse).FoundIntent {
				tp.maybeRemoveResolvedWriteLocked(qiReq.Key, qiReq.Txn.Sequence)
			}
		} else {
...
func (tp *txnPipeliner) adjustError(
	...
	if ime, ok := pErr.GetDetail().(*roachpb.IntentMissingError); ok {
		fmt.Println("HERERERE NATHAN", pErr.GetTxn(), ime)
		log.VEventf(ctx, 2, "transforming intent missing error into retry: %v", ime)
		return roachpb.NewErrorWithTxn(
			roachpb.NewTransactionRetryError(roachpb.RETRY_ASYNC_WRITE_FAILURE), pErr.GetTxn(),
		)
	}
...
// maybeRemoveResolvedWriteLocked attempts to remove an outstanding write that
// was proven to have succeeded. The method will be a no-op if the write was
// already resolved. Care is taken not to accidentally resolve a write to the
// same key but at a later epoch or sequence number.
func (tp *txnPipeliner) maybeRemoveResolvedWriteLocked(key roachpb.Key, seq int32) {
	tp.tmpOW1.Key = key
	item := tp.outstandingWrites.Get(&tp.tmpOW1)
	if item == nil {
		// The write was already resolved or the txn epoch was incremented.
		return
	}

	w := item.(*outstandingWrite)
	if seq < w.Sequence {
		// The sequence might have change, which means that a new write was
	...
	tp.outstandingWrites.Delete(item)
}
