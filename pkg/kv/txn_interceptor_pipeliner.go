...
// before committing. These async writes are referred to as "in-fight writes"
...
type txnPipeliner struct {
	...
	// full precision. Instead, the tracking can be an overestimate and should
	// be thought of as an upper-bound on the influence that the transaction has
	// had. The set contains all keys spans that the transaction will need to
	// eventually clean up upon its completion.
...
func (tp *txnPipeliner) epochBumpedLocked() {
	...
	// longer need to be tracked precicely, but we don't want to forget about
...
// haveWrites returns whether the interceptor is tracking any writes.
...
// condensableSpanSet is a set of key spans that is compressable in order to
...
func (s *condensableSpanSet) maybeCondense(ctx context.Context, ri *RangeIterator, maxBytes int64) {
	...
	for _, bucket := range buckets {
		...
		for _, s := range bucket.spans[1:] {
			cs = cs.Combine(s)
			if !cs.Valid() {
				log.VEventf(ctx, 2, "failed to condense intent spans: "+
