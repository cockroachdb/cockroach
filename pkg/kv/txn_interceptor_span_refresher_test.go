...
// assigns the CanCommitAtHigherTimestamp flag on EndTransaction requests.
func TestTxnSpanRefresherAssignsNoRefreshSpans(t *testing.T) {
	...
	// Send an EndTransaction request. Should set CanCommitAtHigherTimestamp flag.
	var ba roachpb.BatchRequest
	ba.Header = roachpb.Header{Txn: &txn}
	...
	mockSender.MockSend(func(ba roachpb.BatchRequest) (*roachpb.BatchResponse, *roachpb.Error) {
		...
		require.True(t, ba.Requests[0].GetEndTransaction().CanCommitAtHigherTimestamp)
	...
	// Send another EndTransaction request. Should NOT set CanCommitAtHigherTimestamp flag.
	ba.Requests = nil
	ba.Add(&roachpb.EndTransactionRequest{})
	...
	mockSender.MockSend(func(ba roachpb.BatchRequest) (*roachpb.BatchResponse, *roachpb.Error) {
		require.Len(t, ba.Requests, 1)
		require.IsType(t, &roachpb.EndTransactionRequest{}, ba.Requests[0].GetInner())
		require.False(t, ba.Requests[0].GetEndTransaction().CanCommitAtHigherTimestamp)
	...
	// Send another EndTransaction request. Still should NOT set CanCommitAtHigherTimestamp flag.
	ba.Requests = nil
	ba.Add(&roachpb.EndTransactionRequest{})
