...
func TestSetTxn(t *testing.T) {
	...
	txn := roachpb.MakeTransaction("test", roachpb.Key("a"), isolation.Serializable, 1, hlc.Timestamp{}, 0, 99, 0, false)
	e.SetTxn(&txn)
...
func TestErrorRedaction(t *testing.T) {
	...
	t.Run("uncertainty-restart", func(t *testing.T) {
		...
		txn := roachpb.MakeTransaction("foo", roachpb.Key("bar"), isolation.Serializable, 1, hlc.Timestamp{WallTime: 1}, 1, 99, 0, false)
		txn.ID = uuid.Nil
		txn.Priority = 1234
