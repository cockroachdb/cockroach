...
func NewPebbleCorruptionError(info *pebble.DataCorruptionInfo) *PebbleCorruptionError {
...
func (e *PebbleCorruptionError) SafeFormatError(p errors.Printer) (next error) {
	p.Printf("pebble corruption error on path:%s, remote:%t, extra message: %s",
		e.Path, e.IsRemote, e.ExtraMsg)
	return nil
}
