...
func TestTxnPipelinerSavepoints(t *testing.T) {
	...
	// of the savepoint too, so we'll check that the savepoint still retains the
	// earlier seq num.
	...
	// Check that the savepoint has the correct writes.
	{
		...
		require.Equal(t,
			[]inFlightWrite{
				inFlightWrite{roachpb.SequencedWrite{roachpb.Key("a"), 10}},
				inFlightWrite{roachpb.SequencedWrite{roachpb.Key("b"), 11}},
				inFlightWrite{roachpb.SequencedWrite{roachpb.Key("c"), 12}},
			},
			savepointWrites)
	}
	...
	expectedIfWrites := []inFlightWrite{
		{roachpb.SequencedWrite{roachpb.Key("b"), 11}},
		{roachpb.SequencedWrite{roachpb.Key("c"), 12}},
	}
	{
		var savepointWrites []inFlightWrite
		s.ifWrites.Ascend(func(i btree.Item) bool {
	...
	// Check that the footprint was updated correctly. In addition to the "a" which it had before,
	// it will also have "d" because it's not part of the savepoint.
	require.Equal(t,
		[]roachpb.Span{
			{Key: roachpb.Key("a")},
			{Key: roachpb.Key("d")},
