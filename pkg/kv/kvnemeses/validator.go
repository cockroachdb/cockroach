...
//
// TODO(dan): Validate currently ignores read-only transactions, but we should
// be able to construct a validity timespan for each read in the transaction and
// fail if there isn't some timestamp that overlaps all of them.
func Validate(steps []Step, kvs *Engine) []error {
	...
	// needs to see all txn usage before the txn commit or rollback. Stepper
	// currently only emits ClosureTxnOperations, so it currently doesn't matter
	// which order we process these.
	//
