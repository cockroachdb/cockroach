...
// OpP is a key used to configure the relative proporition of various options.
type OpP string
...
type Stepper struct {
	...
	// allSplits is the set of every key that has been split at, regardless of
	// whether it has been merged since.
	allSplits map[string]struct{}
}
...
func MakeStepper(config StepperConfig) *Stepper {
	return &Stepper{
		...
		allSplits:     make(map[string]struct{}),
	}
}
...
// RandStep returns a single randomly generated next operation to execute. The
// previously completed step should be handed back as an argument, which allows
// later steps to depend on the knowledge that it has been finished.
//
// RandStep is not concurrency safe.
func (s *Stepper) RandStep(rng *rand.Rand, prev *Step) Step {
	if prev != nil {
		switch t := prev.Op.GetValue().(type) {
		case *MergeOperation:
			// Assume that this split actually got merged, even though we may have
			// handed out a concurrent split for the same key.
			delete(s.currentSplits, string(t.Key))
		}
	}
	...
	case OpPPutMissing:
		return step(s.randPutMissing(rng, nil /* usedKeys */))
	case OpPPutExisting:
		return step(s.randPutExisting(rng, nil /* usedKeys */))
	case OpPBatch:
		return step(s.randBatch(rng, make(map[string]struct{})))
	case OpPClosureTxn:
		return step(s.randTxnOp(rng))
	case OpPSplit:
		return step(s.randSplit(rng))
	case OpPMerge:
		return step(s.randMerge(rng))
	default:
...
func (s *Stepper) randMerge(rng *rand.Rand) Operation {
	...
	key := randMapKey(rng, s.currentSplits, nil)
	return merge(key)
}
...
func randKey(rng *rand.Rand) string {
	...
	key := stepperSpan.Key[:len(stepperSpan.Key):len(stepperSpan.Key)]
