...
// Engine is an in-memory partial reference implemention of engine.ReadWriter.
// It is intentionally coded as simply as possible so that tests may use it to
...
// such value exists, the RawBytes is nil.
...
func (e *Engine) Put(key engine.MVCCKey, value []byte) {
	e.kvs.ReplaceOrInsert(btreeItem(engine.MVCCKeyValue{Key: key, Value: value}))
}

// DebugPrint returns the entire contents of this Engine as a string for use in
// debugging.
func (e *Engine) DebugPrint() string {
	...
	e.kvs.Ascend(func(item btree.Item) bool {
		...
		fmt.Fprintf(&buf, "%s %s -> %s",
			kv.Key.Key, kv.Key.Timestamp, roachpb.Value{RawBytes: kv.Value}.PrettyPrint())
		return true
	})
	return buf.String()
}
