...
func TestValidate(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			steps:    []Step{step(withResult(put(`a`, `v1`), nil))},
			kvs:      nil,
			expected: []string{`missing write: "a"->v1`},
		},
		{
		...
		},
		{
			...
			steps:    []Step{step(withResult(put(`a`, `v1`), retryableError))},
			kvs:      kvs(kv(`a`, 1, `v1`)),
			expected: []string{`write from failed put: "a"->v1`},
		},
		{
		...
		},
		{
			...
			kvs:      kvs(kv(`b`, 1, `v2`)),
			expected: []string{`committed txn missing write: "a":0,0-><nil> "b":0.000000001,0->v2`},
		},
		{
			...
			kvs:      kvs(kv(`a`, 1, `v1`)),
			expected: []string{`committed txn missing write: "a":0.000000001,0->v1 "b":0,0-><nil>`},
		},
		{
		...
		},
		{
			...
			kvs:      kvs(kv(`a`, 1, `v1`)),
			expected: []string{`rolled back txn had writes: "a":0.000000001,0->v1`},
		},
		{
		...
		// TODO
		// - write to the same key twice in a txn
	}
