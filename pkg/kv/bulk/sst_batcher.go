...
type SSTBatcher struct {
	...
	// lastRangeSpan is used to tell if a send went to same range as prior send.
	lastRangeSpan roachpb.Span
	lastRangeFull bool
...
func (b *SSTBatcher) AddMVCCKey(ctx context.Context, key storage.MVCCKey, value []byte) error {
	...
	// If we're about to add more data to the same range that previously told us
	// its remaining capacity was less than the size of file we last sent it, we
	// should split off a new range before we proceed to add this key.
	if b.lastRangeFull {
		if b.lastRangeSpan.ContainsKey(key.Key) {
			log.VEventf(ctx, 2, "%s batcher splitting full range %s before adding next key %s", b.name, b.lastRangeSpan, key)
			if splitAt, err := keys.EnsureSafeSplitKey(key.Key); err != nil {
				log.Warningf(ctx, "%v", err)
			} else {
				expire := b.db.Clock().Now().Add(15*time.Minute.Nanoseconds(), 0)
				reply, err := b.db.SplitAndScatter(ctx, splitAt, expire)
				if err != nil {
					log.Warningf(ctx, "%s failed to split and scatter during ingest: %+v", b.name, err)
				}
				...
				if reply.ScatteredStats != nil {
					...
					if moved > 0 {
						// This is somewhat unexpected since we are sending data to the same
						// range that previously replied it is full; sending sequential keys
						// to the same range generally suggests we're doing sorted ingestion
						// where we expect the scattered RHS to be empty.
						log.VEventf(ctx, 1, "%s batcher splitting moved %s while scattering span %s",
							b.name, moved, reply.ScatteredSpan)
					}
				}
				b.lastRangeSpan = reply.ScatteredSpan
			}
		}
		b.lastRangeFull = false
	}
...
func (b *SSTBatcher) doFlush(ctx context.Context, reason int) error {
	...
	if reason == sizeFlush {
		...
		if !b.initialSplitDone && b.flushCounts.total == 1 {
	...
	b.lastRangeSpan = rangeSpan
	if rangeSpan.Valid() {
		b.flushKey = rangeSpan.EndKey
		b.lastRangeFull = size > rangeAvailable
	} else {
		b.lastRangeFull = false
	}
			...
			for i := 0; i < maxAddSSTableRetries; i++ {
				...
				if err == nil {
					...
					if maxRangeSpan.EndKey.Compare(rangeSpan.EndKey) < 0 {
						...
						maxRangeRemaining = sz(rangeAvailable)
					}
					return nil
				}
