...
var debugDropSSTOnFlush = settings.RegisterBoolSetting(
	settings.ApplicationLevel,
	"bulkio.ingest.debug_drop_sst_on_flush.enabled",
	"if set, the SSTBatcher will simply SSTs instead of flushing them (destroys data; for performance debugging experiments only)",
	false,
	settings.WithUnsafe,
)
