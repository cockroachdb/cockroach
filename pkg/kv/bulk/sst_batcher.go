...
func (b *batch) reset(ctx context.Context, settings *cluster.Settings) {
	b.sstWriter.Close()
...
func MakeSSTBatcher(
	...
	b := &SSTBatcher{
		name:                   name,
		db:                     db,
		adder:                  newSSTAdder(db, settings, disallowShadowingBelow, admissionpb.BulkNormalPri),
		settings:               settings,
...
func MakeStreamSSTBatcher(
	...
	b := &SSTBatcher{
		...
		adder:     newSSTAdder(db, settings, hlc.Timestamp{}, admissionpb.BulkNormalPri),
		settings:  settings,
		ingestAll: true,
		mem:       mem,
		limiter:   sendLimiter,
