...
func TestIngestAll(t *testing.T) {
	...
	keyMaker := func(name string) func(i int, ts int) storage.MVCCKey {
		return func(i int, ts int) storage.MVCCKey {
			return storageutils.PointKey(fmt.Sprintf("bulk-test-%s-%04d", name, i+1), ts)
		}
	}
	...
	t.Run("ingest all does not add exact matches to SST", func(t *testing.T) {
		b, err := bulk.MakeStreamSSTBatcher(ctx, kvDB, s.ClusterSettings(), mem.MakeBoundAccount(), reqs)
		require.NoError(t, err)
		k := keyMaker(t.Name())
		...
		startKey := k(0, tsStart).Key
		endKey := k(keyCount+1, tsStart).Key
		expectRevisionCount(startKey, endKey, keyCount)
	})
	t.Run("ingest all does not error on key/value matches", func(t *testing.T) {
		b, err := bulk.MakeStreamSSTBatcher(ctx, kvDB, s.ClusterSettings(), mem.MakeBoundAccount(), reqs)
		require.NoError(t, err)
		k := keyMaker(t.Name())
		for i := 0; i < keyCount; i++ {
		...
		startKey := k(0, tsStart).Key
		endKey := k(keyCount+1, tsStart).Key
		expectRevisionCount(startKey, endKey, keyCount*2)
	})
}
