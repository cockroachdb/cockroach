...
func makeTestData(num int) (kvs []kvPair, totalSize sz) {
	...
	for i := range kvs {
		...
		kvs[i].key = alloc[:randutil.RandIntInRange(r, 2, 30)]
...
func TestCompressed(t *testing.T) {
	...
	compressed, err := compressBuffer(b)
	require.NoError(t, err)
	checkIterable(t, src, compressed)
}

func TestSortedBufferSingleCompressed(t *testing.T) {
	defer leaktest.AfterTest(t)()

	src, totalSize := makeTestData(10)
