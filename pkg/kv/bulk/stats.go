...
func (s ingestionPerformanceStats) LogFlushes(ctx context.Context, name, action string, bufSize sz) {
	log.Infof(ctx,
		"%s adder %s; flushed into %s %d times, %d due to buffer size (%s); flushing chunked into %d files (%d for ranges, %d for sst size, +%d after split-retries)",
		name,
		redact.Safe(action),
		action,
		s.span,
