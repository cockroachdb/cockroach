...
func TestSplitSST(t *testing.T) {
	...
	sstBytes, start, end := storageutils.MakeSST(t, s.ClusterSettings(), kvs)
	...
	for _, tc := range []testCase{
		{
			...
			errorContains: "original sst must be larger than split key",
		},
		{
			name:          "middle",
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			left, right, err := createSplitSSTable(ctx, start, tc.splitPoint, iter, s.ClusterSettings())
			if tc.errorContains != "" {
