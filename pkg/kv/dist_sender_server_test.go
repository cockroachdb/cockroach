...
type checkOptions struct {
	...
	// exhausted (ResumeDetails.Reason == RANGE_BOUNDARY) as opposed to the scan
	...
	for i, res := range results {
		...
		if opt.mode == Strict {
			if len(res.Rows) != len(expResults[i]) {
				t.Fatalf("%s: scan %d (%s): expected %d rows, got %d",
					errInfo(), i, spans[i], len(expResults[i]), len(res.Rows))
			}
		}
...
func checkResumeSpanScanResults(
	...
	for i, res := range results {
		...
		if res.ResumeDetails == nil {
			t.Fatalf("%s: scan %d (%s): no resume details. resume span: %+v",
				errInfo(), i, spans[i], res.ResumeSpan)
		}
		...
		if opt.nextRangeStart != "" {
			if res.ResumeDetails.Reason != roachpb.RANGE_BOUNDARY {
				t.Fatalf("%s: scan %d (%s): expected resume reason RANGE_BOUNDARY but got: %s",
					errInfo(), i, spans[i], res.ResumeDetails.Reason)
			}
			...
			if res.ResumeDetails.Reason != roachpb.KEY_LIMIT {
				t.Fatalf("%s: scan %d (%s): expected resume reason RANGE_BOUNDARY but got: %s",
					errInfo(), i, spans[i], res.ResumeDetails.Reason)
			}
			if rowLen == 0 {
...
func TestStopAtRangeBoundary(t *testing.T) {
	...
	type iterationExpRes struct {
		...
		// range boundary was hit (i.e. ResumeDetails.Reason == RANGE_BOUNDARY) and
	...
	testCases := []struct {
		...
		// No limit, no minResults.
		{
			...
			iterResults: []iterationExpRes{
				...
				// scanning [d,e). Notice that [c,d) is expected to be skipped.
				{
					expResults: map[int][]string{
						3: {},
					},
