...
func TestTxnCoordSenderRetries(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			// This test sends a 1PC batch with Put+EndTransaction.
		...
		},
		{
			...
			// an updated timestamp, but this time the EndTransaction puts the
			// transaction in the STAGING state instead of COMMITTED because there had
			// been previous write in a different batch.
