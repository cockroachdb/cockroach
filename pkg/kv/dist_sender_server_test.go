...
func TestTxnCoordSenderRetries(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			txnCoordRetry: true, // will succeed because no partial success
		},
		{
		...
		{
			name: "multi range batch with uncertainty interval error and ambiguous partial success",
			beforeTxnStart: func(ctx context.Context, db *client.DB) error {
				return db.Put(ctx, "c", "value")
			},
			retryable: func(ctx context.Context, txn *client.Txn) error {
			...
			filter:        newUncertaintyFilter(roachpb.Key([]byte("c"))),
			txnCoordRetry: false, // client-side retry required as this will be an ambiguous partial success
		},
		{
			...
			filter:        newUncertaintyFilter(roachpb.Key([]byte("c"))),
			txnCoordRetry: true, // can restart at higher timestamp despite partial success because read-only
		},
		{
			...
			filter:        newUncertaintyFilter(roachpb.Key([]byte("c"))),
			txnCoordRetry: false, // can't restart because of partial success and write batch
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Verify auto retry metric.
			autoRetries := metrics.AutoRetries.Count() - lastAutoRetries
			if tc.txnCoordRetry && autoRetries != 1 {
				t.Errorf("expected one auto retry; got %d", autoRetries)
			}
		})
	}
}
