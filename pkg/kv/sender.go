...
type TxnSender interface {
	...
	// up to its provisional commit timestamp. In the case that the two are
	// already the same, it is a no-op. The reason one might want to do that is
	// to ensure that a transaction can commit without experiencing another push.
	//
	...
	ForceRefreshTransaction(ctx context.Context) error
}
