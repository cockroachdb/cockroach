...
func (sr *txnSpanRefresher) initializeFromTxn(*roachpb.Transaction) {}
...
func (sr *txnSpanRefresher) initializeLeaf(tis *roachpb.LeafTxnInputState) {
	sr.refreshInvalid = tis.RefreshInvalid
}
...
func (sr *txnSpanRefresher) populateLeafFinalState(tfs *roachpb.LeafTxnFinalState) {
	...
	if !sr.refreshInvalid {
		...
		tfs.RefreshWrites = append([]roachpb.Span(nil), sr.refreshWrites...)
	}
}
...
func (sr *txnSpanRefresher) importLeafFinalState(tfs *roachpb.LeafTxnFinalState) {
	...
	if tfs.RefreshInvalid {
		...
		if tfs.RefreshWrites != nil {
			sr.refreshWrites, _ = roachpb.MergeSpans(
				append(append([]roachpb.Span(nil), sr.refreshWrites...), tfs.RefreshWrites...),
			)
		}
	}
