...
func (b *Batch) PutBytes(bs BulkSource[[]byte]) {
	...
	i := 0
	bsi := bs.Iter()
	b.bulkRequest(numKeys, func() (kvpb.Request, int) {
		pr := &reqs[i]
		i++
		k, v := bsi.Next()
		pr.Key = k
		...
		return pr, len(k) + len(pr.Value.RawBytes)
	})
}
...
// Empty keys are allowed and will be ignored.
func (b *Batch) CPutValues(bs BulkSource[roachpb.Value]) {
	numKeys := bs.Len()
	reqs := make([]kvpb.ConditionalPutRequest, numKeys)
	i := 0
	bsi := bs.Iter()
...
func (b *Batch) bulkRequest(numKeys int, requestFactory func() (req kvpb.Request, kvSize int)) {
	n := len(b.reqs)
	b.growReqs(numKeys)
	newReqs := b.reqs[n:]
