...
func (b *Batch) get(key interface{}, strengthType kvpb.KeyLockingStrengthType) {
	k, err := marshalKey(key)
	if err != nil {
		b.initResult(0, 1, notRaw, err)
		return
	}
	b.appendReqs(kvpb.NewGet(k, strengthType))
...
func (b *Batch) scan(s, e interface{}, isReverse bool, strengthType kvpb.KeyLockingStrengthType) {
	...
	if !isReverse {
		b.appendReqs(kvpb.NewScan(begin, end, strengthType))
	} else {
		b.appendReqs(kvpb.NewReverseScan(begin, end, strengthType))
	}
