...
func (b *Batch) PutBytes(keys []roachpb.Key, values [][]byte) {
	...
	reqs := make([]roachpb.PutRequest, numKeys)
	b.bulkRequest(keys, numKeys, func(keyIdx, reqIdx int) (roachpb.Request, int) {
		pr := &reqs[reqIdx]
		k := keys[keyIdx]
		pr.Key = k
...
func (b *Batch) CPutTuples(keys []roachpb.Key, values [][]byte) {
	...
	reqs := make([]roachpb.ConditionalPutRequest, numKeys)
	b.bulkRequest(keys, numKeys, func(keyIdx, reqIdx int) (roachpb.Request, int) {
		pr := &reqs[reqIdx]
		k := keys[keyIdx]
		pr.Key = k
...
func (b *Batch) InitPutBytes(keys []roachpb.Key, values [][]byte) {
	...
	reqs := make([]roachpb.InitPutRequest, numKeys)
	b.bulkRequest(keys, numKeys, func(keyIdx, reqIdx int) (roachpb.Request, int) {
		pr := &reqs[reqIdx]
		k := keys[keyIdx]
		pr.Key = k
...
func (b *Batch) bulkRequest(
	keys []roachpb.Key,
	numKeys int,
	requestFactory func(keyIdx, reqIdx int) (req roachpb.Request, valueSize int),
) {
	n := len(b.reqs)
	b.growReqs(numKeys)
	newReqs := b.reqs[n:]
