...
func Validate(steps []Step, kvs *Engine) []error {
	...
	for _, kv := range v.kvByValue {
		extraKVs = append(extraKVs, &observedWrite{KV: kv})
	}
	if len(extraKVs) > 0 {
...
// Validator collects these grouped by txn, then computed the time window that
...
func (v *validator) processOp(txnID *string, op Operation) {
	...
	case *PutOperation:
		if txnID == nil {
			...
			v.observedOpsByTxn[*txnID] = append(v.observedOpsByTxn[*txnID], &observedWrite{KV: kv})
		}
	case *SplitOperation:
		v.failIfError(op, t.Result)
	case *MergeOperation:
...
func (v *validator) checkCommittedTxn(atomicType string, txnObservations []observedOp) {
	...
	for idx := len(txnObservations) - 1; idx >= 0; idx-- {
		...
		case *observedWrite:
			if _, ok := lastWriteIdxByKey[string(o.KV.Key.Key)]; !ok {
				lastWriteIdxByKey[string(o.KV.Key.Key)] = idx
			}
			batch.Delete(storage.EncodeKey(o.KV.Key), nil)
		}
	}
	...
	for idx, observation := range txnObservations {
		...
		case *observedWrite:
			var key = o.KV.Key
			if lastWriteIdx := lastWriteIdxByKey[string(o.KV.Key.Key)]; idx != lastWriteIdx {
				if !o.KV.Key.Timestamp.IsEmpty() {
					failure = `committed txn overwritten key had write`
				}
				key.Timestamp = txnObservations[lastWriteIdx].(*observedWrite).KV.Key.Timestamp
			}
			batch.Set(storage.EncodeKey(key), o.KV.Value, nil)
		case *observedRead:
	...
	valid := timeSpan{Start: hlc.Timestamp{}, End: hlc.MaxTimestamp}
	for idx, observation := range txnObservations {
		if failure != `` {
			break
		}
		...
		switch o := observation.(type) {
		case *observedWrite:
			isLastWriteForKey := idx == lastWriteIdxByKey[string(o.KV.Key.Key)]
			...
			opValid = timeSpan{Start: o.KV.Key.Timestamp, End: o.KV.Key.Timestamp.Next()}
		case *observedRead:
			opValid = o.Valid
		default:
			panic(errors.AssertionFailedf(`unknown observedOp: %T %s`, observation, observation))
...
func (v *validator) checkAmbiguousTxn(atomicType string, txnObservations []observedOp) {
	...
	if !hadWrite {
		...
		// but the uncommited case doesn't and this seems to work.
...
func validReadTime(
	...
	for ; iter.Valid(); iter.Next() {
		...
		if mustGetStringValue(iter.Value()) == mustGetStringValue(value) {
			valid = append(valid, timeSpan{Start: mvccKey.Timestamp, End: end})
		}
		end = mvccKey.Timestamp
	}
	if len(value) == 0 {
		valid = append(valid, timeSpan{Start: hlc.Timestamp{}, End: end})
	}
...
func printObserved(observedOps ...observedOp) string {
	...
	for _, observed := range observedOps {
		...
		case *observedWrite:
			ts := `missing`
			if !o.KV.Key.Timestamp.IsEmpty() {
				ts = o.KV.Key.Timestamp.String()
			}
			fmt.Fprintf(&buf, "[w]%s:%s->%s", o.KV.Key.Key, ts, mustGetStringValue(o.KV.Value))
		case *observedRead:
			var end string
			...
			fmt.Fprintf(&buf, "[r]%s:[%s,%s)->%s",
				o.Key, o.Valid.Start, end, mustGetStringValue(o.Value.RawBytes))
		default:
			panic(errors.AssertionFailedf(`unknown observedOp: %T %s`, observed, observed))
		}
	}
