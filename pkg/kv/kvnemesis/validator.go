...
// hasn't yet been matched to a Delete operation from tombstonesForKey,
// returning the MVCCKey along with a `true` boolean value if there are
// tombstones remaining, or returning the empty key and a `false` boolean value.
func (v *validator) getNextDeleteForKey(key string) (storage.MVCCKey, bool) {
	if v.committedDeletesForKey[key] > len(v.tombstonesForKey[key]) {
		return storage.MVCCKey{}, false
	}

	for ts, used := range v.tombstonesForKey[key] {
		if !used {
