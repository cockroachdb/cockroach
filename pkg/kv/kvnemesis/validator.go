...
func (v *validator) processOp(txnID *string, op Operation) {
	...
	case *ChangeReplicasOperation:
		...
		if t.Result.Type == ResultType_Error {
			...
			ignore = kvserver.IsRetriableReplicationChangeError(errors.DecodeError(ctx, *t.Result.Err))
		}
		if !ignore {
			if resultIsError(t.Result, `received invalid ChangeReplicasTrigger .* to remove self \(leaseholder\)`) {
				// Removing the leaseholder is invalid for technical reasons, but
				// Generator intentionally does not try to avoid this so that this edge
				// case is exercised.
			} else {
				v.failIfError(op, t.Result)
			}
