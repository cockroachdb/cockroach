...
func (v *validator) checkAtomic(atomicType string, result Result, ops ...Operation) {
	...
	if result.Type != ResultType_Error {
		...
		execTimestamp := result.OptionalTimestamp
		v.checkAtomicCommitted(`committed `+atomicType, observations, execTimestamp)
	...
	for _, observation := range txnObservations {
		...
		case *observedWrite:
			...
			if o.Materialized {
				...
				maybeExecTS.Forward(o.Timestamp) // just in case o.Timestamp is zero
				break
			}
