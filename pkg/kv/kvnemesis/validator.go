...
func (v *validator) checkCommittedTxn(
	...
	txnIsAmbiguous := strings.Contains(atomicType, "ambiguous")
	...
	for idx, observation := range txnObservations {
		...
		case *observedWrite:
			...
			if o.isDelete() && (len(txnObservations) == 1 || txnIsAmbiguous) {
				...
				// This also applies for delete operations in ambiguous transactions.
				continue
			}
