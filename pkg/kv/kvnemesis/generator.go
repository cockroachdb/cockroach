...
type ChangeReplicasConfig struct {
	...
	// AtomicSwapReplica adds 1 non-voting replica and removes 1 non-voting
...
func (g *generator) RandStep(rng *rand.Rand) Step {
	...
	if numReplicas < g.Config.NumNodes {
		...
		addOpGen(&allowed, addVoterFn, g.Config.Ops.ChangeReplicas.AddReplica)
	...
	if numReplicas == g.Config.NumReplicas && numReplicas < g.Config.NumNodes {
		...
		addOpGen(&allowed, atomicSwapVoterFn, g.Config.Ops.ChangeReplicas.AtomicSwapReplica)
		if numNonVoters > 0 {
	...
	if numReplicas > g.Config.NumReplicas {
		...
		addOpGen(&allowed, removeVoterFn, g.Config.Ops.ChangeReplicas.RemoveReplica)
		if numNonVoters > 0 {
