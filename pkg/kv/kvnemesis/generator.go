...
func randAddSSTable(g *generator, rng *rand.Rand) Operation {
	...
	ranges := make([]string, 1, len(g.currentSplits)+1) // first range starts at ""
	for key := range g.currentSplits {
	...
	rangeStart = ranges[rangeIdx]
	if rangeIdx+1 < len(ranges) {
	...
	// Generate keys first, to write them in order and without duplicates.
	//
	...
	curKeys := make([]string, 0, len(g.keys)/len(ranges))
	...
	sstSpan := roachpb.Span{
		...
		EndKey: roachpb.Key(sstKeys[len(sstKeys)-1]).Next(),
	}
