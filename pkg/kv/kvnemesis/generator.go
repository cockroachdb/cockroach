...
func (g *generator) registerClosureTxnOps(allowed *[]opGen, c *ClosureTxnConfig) {
	...
	addOpGen(allowed,
		makeClosureTxn(Commit, SSI, g.Config.BatchConfig, &c.TxnClientOps, &c.TxnBatchOps, nil /* commitInBatch*/, &c.SavepointOps), c.CommitSerializable)
	addOpGen(allowed,
		makeClosureTxn(Commit, SI, g.Config.BatchConfig, &c.TxnClientOps, &c.TxnBatchOps, nil /* commitInBatch*/, &c.SavepointOps), c.CommitSnapshot)
	addOpGen(allowed,
		makeClosureTxn(Commit, RC, g.Config.BatchConfig, &c.TxnClientOps, &c.TxnBatchOps, nil /* commitInBatch*/, &c.SavepointOps), c.CommitReadCommitted)

	addOpGen(allowed,
		makeClosureTxn(Rollback, SSI, g.Config.BatchConfig, &c.TxnClientOps, &c.TxnBatchOps, nil /* commitInBatch*/, &c.SavepointOps), c.RollbackSerializable)
	addOpGen(allowed,
		makeClosureTxn(Rollback, SI, g.Config.BatchConfig, &c.TxnClientOps, &c.TxnBatchOps, nil /* commitInBatch*/, &c.SavepointOps), c.RollbackSnapshot)
	addOpGen(allowed,
		makeClosureTxn(Rollback, RC, g.Config.BatchConfig, &c.TxnClientOps, &c.TxnBatchOps, nil /* commitInBatch*/, &c.SavepointOps), c.RollbackReadCommitted)

	addOpGen(allowed,
		makeClosureTxn(Commit, SSI, g.Config.BatchConfig, &c.TxnClientOps, &c.TxnBatchOps, &c.CommitBatchOps, &c.SavepointOps), c.CommitSerializableInBatch)
	addOpGen(allowed,
		makeClosureTxn(Commit, SI, g.Config.BatchConfig, &c.TxnClientOps, &c.TxnBatchOps, &c.CommitBatchOps, &c.SavepointOps), c.CommitSnapshotInBatch)
	addOpGen(allowed,
		makeClosureTxn(Commit, RC, g.Config.BatchConfig, &c.TxnClientOps, &c.TxnBatchOps, &c.CommitBatchOps, &c.SavepointOps), c.CommitReadCommittedInBatch)
}
...
func makeClosureTxn(
	...
	return func(g *generator, rng *rand.Rand) Operation {
		...
		if batchConfig.RandomUserPriority {
			op.ClosureTxn.UserPriority = randomUserPriority(rng)
		}
		op.ClosureTxn.BufferedWrites = rng.Float64() < batchConfig.BufferedWritesProb
