...
func RunNemesis(
	...
	for _, f := range failures {
		...
		canBeIgnoredSafety := mode == Safety && (exceptReplicaUnavailable(f) || exceptAmbiguous(f) ||
			exceptContextCanceled(f) || exceptSQLCanceled(f))
		if !canBeIgnored && !canBeIgnoredSafety {
