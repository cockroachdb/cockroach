...
func TestKVNemesisMultiNode_BufferedWrites(t *testing.T) {
	...
	testKVNemesisImpl(t, kvnemesisTestCfg{
		...
		testSettings: func(ctx context.Context, st *cluster.Settings) {
			// TODO(ssd): We keep buffered writes default off for txns we don't
			// generate explicitly because buffered writes currently doesn't play
			// nicely with the columnar batch format that might get used during
			// metamorphic builds. These can get triggered during zone configuration
			// operations. We could alternatively disable zone update operations
			// below.
			//
		...
		testGeneratorConfig: func(c *GeneratorConfig) {
			// TODO(ssd): Remove once the txnWriteBuffer handles
			// savepoint rollbacks correctly.
			c.Ops.ClosureTxn.SavepointOps = SavepointConfig{}
		},
	})
}
