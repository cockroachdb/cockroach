...
func TestKVNemesisMultiNode_Faults_Safety(t *testing.T) {
	...
	testKVNemesisImpl(t, kvnemesisTestCfg{
		...
		testGeneratorConfig: func(cfg *GeneratorConfig) {
			...
			// Lease transfers should be ok if we tweak the range lookup function in
			// the generator.
			cfg.Ops.ChangeLease = ChangeLeaseConfig{}
			// Applying zone configs and cluster settings is brittle in the presence
			// of network partitions (e.g. ambiguous errors). But it can probably be
			// handled by the test.
...
func setAndVerifyZoneConfigs(
	t testing.TB, ctx context.Context, tc *testcluster.TestCluster, sqlRunner *sqlutils.SQLRunner,
