...
func TestKVNemesisMultiNode_Faults_Safety(t *testing.T) {
	...
	testKVNemesisImpl(t, kvnemesisTestCfg{
		...
		testGeneratorConfig: func(cfg *GeneratorConfig) {
			...
			// TODO(mira): applying zone configs stalls the test occasionally. The
			// issue is that in conn.QueryContext we stop propagating the ctx and
			// just check if it was canceled after conn.query; but in conn.query we
			// end up hanging waiting for a response.
...
func TestKVNemesisMultiNode_Faults_Liveness(t *testing.T) {
	...
	testKVNemesisImpl(t, kvnemesisTestCfg{
		...
		testGeneratorConfig: func(cfg *GeneratorConfig) {
			...
			// Leader lease transfers use expiration leases temporarily. A network
			// partition racing with the transfer can lead to a leader-leaseholder
			// split and permanent unavailability.
