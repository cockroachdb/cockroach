...
func TestValidate(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			steps: []Step{
				step(withDeleteRangeResult(delRange(`a`, `c`, false))),
				step(withResult(put(`a`, `v1`), nil)),
			},
			kvs:      kvs(kv(`a`, 1, `v1`)),
			expected: nil,
		},
		{
			...
			steps: []Step{
				step(withDeleteRangeResult(delRange(`a`, `c`, false), roachpb.Key(`a`))),
				step(withResult(put(`a`, `v1`), nil)),
			},
			kvs: kvs(kv(`a`, 1, `v1`)),
			expected: []string{
		...
		},
		{
			...
			steps: []Step{
				step(withResult(put(`a`, `v1`), nil)),
				step(withResult(withTimestamp(closureTxn(ClosureTxnType_Commit,
					withDeleteRangeResult(delRange(`a`, `c`, false), roachpb.Key(`a`)),
				), 2), nil)),
			},
			kvs:      kvs(kv(`a`, 1, `v1`), tombstone(`a`, 2)),
			expected: nil,
		},
		{
			...
			steps: []Step{
				step(withResult(put(`a`, `v1`), nil)),
				step(withResult(withTimestamp(closureTxn(ClosureTxnType_Commit,
					withDeleteRangeResult(delRange(`a`, `c`, false)),
				), 2), nil)),
			},
			kvs: kvs(kv(`a`, 1, `v1`), tombstone(`a`, 2)),
			expected: []string{
		...
		},
		{
			...
			steps: []Step{
				...
				step(withResult(put(`c`, `v3`), nil)),
				step(withResult(withTimestamp(closureTxn(ClosureTxnType_Commit,
					withDeleteRangeResult(delRange(`a`, `c`, false), roachpb.Key(`a`), roachpb.Key(`b`)),
				), 4), nil)),
		...
		},
		{
			...
			steps: []Step{
				...
				step(withResult(withTimestamp(closureTxn(ClosureTxnType_Commit,
					withDeleteRangeResult(delRange(`a`, `c`, false), roachpb.Key(`a`), roachpb.Key(`d`)),
				), 3), nil)),
			},
