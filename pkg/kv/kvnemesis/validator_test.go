...
func stepWithTxnTimestamp(op Operation, ts int) Step {
	txnOp := op.GetValue().(*ClosureTxnOperation)
...
func TestValidate(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expected: []string{
				`committed txn non-atomic timestamps: [w]"a":0.000000001,0-><nil> [w]"b":0.000000002,0-><nil>`,
			},
		},
		{
		...
		},
		{
			...
			expected: []string{`extra writes: [w]"a":0.000000002,0-><nil>`},
		},
		{
		...
		},
		{
			...
			expected: []string{
				`ambiguous txn non-atomic timestamps: [w]"a":0.000000001,0->v1 [w]"b":0.000000002,0-><nil>`,
			},
		},
		{
		...
		},
		{
			...
			steps: []Step{
				stepWithTxnTimestamp(withResult(closureTxn(ClosureTxnType_Commit,
					withResult(put(`a`, `v1`), nil),
				), nil), 1),
			},
			kvs:      kvs(kv(`a`, 1, `v1`)),
		...
		{
			name: "one transactional delete with write on another key after delete (without transaction timestamp)",
			steps: []Step{
				// NB: this Delete comes first in operation order, but the write is delayed.
				step(withResult(del(`a`), nil)),
				step(withResult(closureTxn(ClosureTxnType_Commit,
			...
			expected: []string{
				`committed txn non-atomic timestamps: ` +
					`[w]"b":0.000000002,0->v1 [w]"a":0.000000003,0-><nil>`,
			},
		},
		{
		...
		{
			name: "two transactional deletes with out of order commit times (without transaction timestamp)",
			steps: []Step{
				step(withResult(del(`a`), nil)),
				step(withResult(del(`b`), nil)),
				step(withResult(closureTxn(ClosureTxnType_Commit,
			...
			expected: []string{
				`committed txn non-atomic timestamps: ` +
					`[w]"a":0.000000002,0-><nil> [w]"b":0.000000003,0-><nil>`,
			},
		},
		{
