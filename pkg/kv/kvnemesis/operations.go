...
func (op SavepointOperation) format(w *strings.Builder, fctx formatCtx) {
	fmt.Fprintf(w, `%s.CreateSavepoint(ctx, %d)`, fctx.receiver, int(op.SeqNum))
	op.Result.format(w)
}

func (op SavepointReleaseOperation) format(w *strings.Builder, fctx formatCtx) {
	fmt.Fprintf(w, `%s.ReleaseSavepoint(ctx, %d)`, fctx.receiver, int(op.SeqNum))
	op.Result.format(w)
}

func (op SavepointRollbackOperation) format(w *strings.Builder, fctx formatCtx) {
	fmt.Fprintf(w, `%s.RollbackSavepoint(ctx, %d)`, fctx.receiver, int(op.SeqNum))
	op.Result.format(w)
}
