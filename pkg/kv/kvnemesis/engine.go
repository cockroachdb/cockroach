...
// Delete writes a tombstone value for a given key/timestamp.  It is
...
func (e *Engine) DeleteRange(key, endKey roachpb.Key, ts hlc.Timestamp) {
	...
	for iter.SeekGE(seekKey); iter.Valid(); iter.Next() {
		...
		if mvccKey.Key.Compare(key) < 0 || endKey.Compare(mvccKey.Key) <= 0 {
			break
		}
	...
	// go through keys and write a tombstone for each one (all at the given ts)
	for keyToDelete := range keysToDelete {
		mvccKeyToDelete := storage.MVCCKey{Key: roachpb.Key(keyToDelete), Timestamp: ts}
		if err := e.kvs.Set(storage.EncodeKey(mvccKeyToDelete), nil, nil); err != nil {
			panic(err)
		}
	}
}
