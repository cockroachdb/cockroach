...
func TestTxnCoordSenderCondenseIntentSpans(t *testing.T) {
	...
	testCases := []struct {
		...
		{span: g0Tog1, expKeys: []roachpb.Span{fTog1Closed, cToEClosed, a, b}, expKeysSize: 8},
		// Add a key in the middle of a span, which will get merged on commit.
		{span: c, expKeys: []roachpb.Span{fTog1Closed, cToEClosed, a, b, c}, expKeysSize: 9},
	}
	splits := []roachpb.Span{
	...
	txnCoord.maxTxnIntentsBytes = 10 /* 10 bytes and it will condense */
	defer s.Stop()
