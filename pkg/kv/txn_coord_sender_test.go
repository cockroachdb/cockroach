...
func TestTxnMultipleCoord(t *testing.T) {
	...
	tfs := txn2.GetLeafTxnFinalState(ctx)
	txn.UpdateRootWithLeafFinalState(ctx, &tfs)
...
func TestUpdateRoootWithLeafFinalStateInAbortedTxn(t *testing.T) {
	...
	finalState := leafTxn.GetLeafTxnFinalState(ctx)
	finalState.Txn.Status = roachpb.ABORTED
	txn.UpdateRootWithLeafFinalState(ctx, &finalState)
