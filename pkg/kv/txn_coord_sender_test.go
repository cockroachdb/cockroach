	...
	for _, test := range testCases {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.expNewTransaction {
				// Check that the timestamp has been reset. If the transaction reset code
				// changes, this should simply assert that the timestamp was advanced.
				zeroTS := hlc.Timestamp{}
				if txn.Proto().Timestamp != zeroTS || txn.Proto().OrigTimestamp != zeroTS {
					t.Fatalf("expected txn with no timestamp, but got: %s", txn.Proto())
				}
			}
	...
	testCases := []struct {
	...
	}{
		{
			errFn: func(txn roachpb.Transaction) *roachpb.Error {
				const nodeID = 1
				// ReadWithUncertaintyErrors need a clock to have been recorded on the
				// origin.
