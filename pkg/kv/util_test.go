...
func TestMarshalKey(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			input:    func() *roachpb.Key { k := roachpb.Key("test"); return &k }(),
		...
		},
		{
			...
			input:    func() *roachpb.RKey { k := roachpb.RKey("test"); return &k }(),
			expected: roachpb.Key("test"),
		},
		{
...
func TestMarshalValue(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			input: nil,
			validator: func(t *testing.T, v roachpb.Value) {
				require.Equal(t, 0, len(v.RawBytes))
			},
		},
		{
			name:  "bool true",
		...
		{
			name:  "apd.Decimal",
			input: func() apd.Decimal { d, _, _ := apd.NewFromString("123.45"); return *d }(),
			validator: func(t *testing.T, v roachpb.Value) {
				d, err := v.GetDecimal()
				require.NoError(t, err)
				expected, _, _ := apd.NewFromString("123.45")
				require.Equal(t, 0, d.Cmp(expected))
			},
		},
		{
			name:  "roachpb.Key",
		...
		},
		{
			...
			validator: func(t *testing.T, v roachpb.Value) {
				tm, err := v.GetTime()
				require.NoError(t, err)
				require.Equal(t, time.Unix(1234567890, 0).UTC(), tm)
			},
		},
		{
		...
		},
		{
			...
			input:   struct{ Field int }{Field: 42},
			wantErr: true,
		},
	}
...
func TestMarshalValuePanics(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)
	...
	testCases := []struct {
	...
	}{
		{
			...
			expectedPanicMsg: "unexpected type roachpb.Value",
		},
		{
			name: "roachpb.Value pointer with invalid header",
			input: func() *roachpb.Value {
				v := &roachpb.Value{RawBytes: []byte{1, 2, 3}} // length 3 < headerSize (5)
				return v
			}(),
			expectedPanicMsg: "invalid header size: 3",
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			_, _ = marshalValue(tc.input)

			require.NotEmpty(t, panicMsg, "expected panic but none occurred")
			require.Contains(t, panicMsg, tc.expectedPanicMsg)
		})
	}
}
