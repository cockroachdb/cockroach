...
func TestShouldUseLooselyCoupledTruncation(t *testing.T) {
	...
	tests := []struct {
		...
		{settingEnabled: false, enginesSeparated: true, raftExpectedFirstIndex: 0, wantPanic: true},
		...
		{settingEnabled: true, enginesSeparated: true, raftExpectedFirstIndex: 0, wantPanic: true},
	...
	for _, tc := range tests {
		t.Run("", func(t *testing.T) {
			...
			call := func() bool {
				return shouldUseLooselyCoupledTruncation(
					ctx, &st.SV, tc.raftExpectedFirstIndex, tc.enginesSeparated,
				)
			}
			if tc.wantPanic {
				require.Panics(t, func() { _ = call() })
			} else {
				require.Equal(t, tc.wantLoose, call())
			}
		})
	}
}
