...
func TestAdminScatterWithDrainingNodes(t *testing.T) {
	...
	// Verify that the lease is on the non-draining node.
	var drainingStore, nonDrainingStore *kvserver.Store
	tc.Server(drainingServerIdx).GetStores().(*kvserver.Stores).VisitStores(
	...
	_, pErr := kv.SendWrapped(
		ctx, drainingStore.TestSender(), adminScatterArgs(scratchKey, true /* randomizeLeases */),
	)
	require.Regexp(t, "lease held by.*n1,s1", pErr.GoError())
	...
	tc.Server(0).GetStores().(*kvserver.Stores).VisitStores(
		func(s *kvserver.Store) error {
			nonDrainingStore = s
			return nil
		},
