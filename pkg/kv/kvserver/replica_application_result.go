	...
	if pErr != nil {
		...
		case kvserverbase.ProposalRejectionIllegalLeaseIndex:
			...
			if pErr != nil {
				...
				// TODO(tbg): this assertion can mis-fire if we artificially inject
				// invalid LAIs during proposal, see the explanatory comment above. If,
				// in the current app batch, we had two identical copies of an entry
				// (which maps to a local proposal), and the LAI was stale, then both
				// entries would be local. The first could succeed to repropose, which,
				// if the propBuf was full, would immediately insert into the proposals
				// map. Normally, when we then apply the second entry, it would be
				// superseded and not hit the assertion. But, if we injected a stale LAI
				// during this last reproposal, we could "accidentally" assign the same
				// LAI again. The second entry would then try to repropose again, which
				// is fine, but it could bump into the closed timestamp, get an error,
				// enter this branch, and then trip the assertion.
				// We should be able to avoid pathologies such as this one if in
				// `retrieveLocalProposals` we attached the proposal (and removed from
				// the map) to the first copy of the current non-superseded entry. This
				// avoids situations in which multiple entries deal with the same proposal
				// within the context of an replicaAppBatch.
				// It might be even better to remove proposals from the map not at the
				// beginning of application, but at the end: a proposal would leave the
				// map only once, never to return again.
				r.mu.RLock()
