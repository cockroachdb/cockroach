...
func (r *Replica) evalAndPropose(
	...
	ch, abandon, _ /* id */, pErr := r.evalAndProposeInner(ctx, ba, g, st, lul, tok)
	return ch, abandon, pErr
}

// evalAndProposeInner is like evalAndPropose, except it also returns the
// propal's ID, to help tests.
func (r *Replica) evalAndProposeInner(
	ctx context.Context,
	ba *roachpb.BatchRequest,
	g *concurrency.Guard,
	st kvserverpb.LeaseStatus,
