...
func (r *Replica) tick(
	...
	{
		outcome, lhRepID, lhProg := r.maybeTransferRaftLeadershipToLeaseholderLocked(ctx, leaseStatus)
...
// maybeTransferRaftState tracks the state machine for Raft leadership transfer
...
// be emitted.
type maybeTransferRaftState struct {
...
func (s *maybeTransferRaftState) update(
	...
	if outcome != s.outcome {
		...
		if s.outcome.isBlocked() && outcome == raftLeaderTransferOK {
			...
			logResolved = elapsed >= time.Minute
		}
		s.outcome = outcome
...
func (r *Replica) maybeTransferRaftLeadershipToLeaseholderLocked(
	...
	lhProgress := r.mu.internalRaftGroup.ReplicaProgress(lhReplicaID)
	...
	outcome := shouldTransferRaftLeadershipToLeaseholderLocked(
		raftStatus, lhProgress, leaseStatus,
	...
	return outcome, lhReplicaID, lhProgress
}
