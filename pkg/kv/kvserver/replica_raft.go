...
func (r *Replica) propose(
	...
	if crt := p.command.ReplicatedEvalResult.ChangeReplicas; crt != nil {
		...
		// 1) Simple (old style) where there is a reconfiguration turning a voter into a LEARNER /
		// NON-VOTER. 2) Through an intermediate joint configuration, where the replica remains in
		// the descriptor, but as VOTER_{OUTGOING, DEMOTING}. When leaving the JOINT config (a second
		// Raft operation), the removed replica transitions a LEARNER / NON-VOTER.
		//
		// In case (1) the lease needs to be transferred out before a removal is proposed (cooperative
		// transfer). The code below permits leaseholder removal only if entering a joint configuration
		// (option 2 above) in which the leaseholder is (any kind of) voter. In this case, the lease is
		// transferred to a different voter (potentially incoming) in maybeLeaveAtomicChangeReplicas
		// right before we exit the joint configuration.
		//
		// When the leaseholder is replaced by a new replica, transferring the lease in the joint config
		// allows transferring directly from old to new, since both are active in the joint config,
		// without going through a third node or adding the new node before transferring, which might
		// reduce fault tolerance. For example, consider v1 in region1 (leaseholder), v2 in region2 and
		// v3 in region3. We want to relocate v1 to a new node v4 in region1. We add v4 as LEARNER.
		// At this point we can't transfer the lease to v4, so we could transfer it to v2 first, but
		// this is likely to hurt application performance. We could instead add v4 as VOTER first,
