...
func (r *Replica) stepRaftGroupRaftMuLocked(req *kvserverpb.RaftMessageRequest) error {
	...
	err := r.withRaftGroup(func(raftGroup *raft.RawNode) (bool, error) {
		if mark := req.TraceMark; mark != nil {
			r.mu.raftTracer.MaybeRegister(raft.LogMark{Term: mark.Term, Index: mark.Index})
		}
...
func (r *Replica) sendRaftMessage(
	...
	r.mu.RLock()
	mark, trace := r.mu.raftTracer.MaybeTrace(msg)
	...
	*req = kvserverpb.RaftMessageRequest{
	...
	if trace {
		req.TraceMark = &kvserverpb.LogMark{Term: mark.Term, Index: mark.Index}
	}
