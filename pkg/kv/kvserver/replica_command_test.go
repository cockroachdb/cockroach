...
func TestSynthesizeTargetsByChangeType(t *testing.T) {
	...
	mk := func(targets []int32) []roachpb.ReplicationTarget {
		if len(targets) == 0 {
			return nil
		}
		res := make([]roachpb.ReplicationTarget, len(targets))
		for i, t := range targets {
			res[i] = roachpb.ReplicationTarget{
	...
	tests := []testCase{
		...
		},
		{
			...
			changes: []roachpb.ReplicationChange{
				...
				{ChangeType: roachpb.ADD_VOTER, Target: roachpb.ReplicationTarget{NodeID: 2, StoreID: 2}},
				{ChangeType: roachpb.REMOVE_NON_VOTER, Target: roachpb.ReplicationTarget{NodeID: 2, StoreID: 2}},
				{ChangeType: roachpb.REMOVE_VOTER, Target: roachpb.ReplicationTarget{NodeID: 4, StoreID: 4}},
			},
			expPromotions:    []int32{2},
			expDemotions:     []int32{1},
		...
		},
		{
			...
			changes: []roachpb.ReplicationChange{
				...
				{ChangeType: roachpb.ADD_VOTER, Target: roachpb.ReplicationTarget{NodeID: 3, StoreID: 3}},
				{ChangeType: roachpb.ADD_VOTER, Target: roachpb.ReplicationTarget{NodeID: 4, StoreID: 4}},
				{ChangeType: roachpb.ADD_NON_VOTER, Target: roachpb.ReplicationTarget{NodeID: 5, StoreID: 5}},
				{ChangeType: roachpb.REMOVE_NON_VOTER, Target: roachpb.ReplicationTarget{NodeID: 6, StoreID: 6}},
			},
			expPromotions:        []int32{2, 3},
			expDemotions:         []int32{1},
			expVoterAdditions:    []int32{4},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			require.Equal(t, result.nonVoterPromotions, mk(test.expPromotions))
			require.Equal(t, result.voterDemotions, mk(test.expDemotions))
			require.Equal(t, result.voterAdditions, mk(test.expVoterAdditions))
			require.Equal(t, result.voterRemovals, mk(test.expVoterRemovals))
			require.Equal(t, result.nonVoterAdditions, mk(test.expNonVoterAdditions))
			require.Equal(t, result.nonVoterRemovals, mk(test.expNonVoterRemovals))
		})
	}
}
