...
// migrateLockTable runs a migration of all interlevaed intents in the provided
...
func (r *Replica) migrateLockTable(
	ctx context.Context, req *roachpb.MigrateLockTableRequest,
	res *roachpb.MigrateLockTableResponse,
) error {
	if res.NoIntents {
		// This range is guaranteed to not contain any intents.
		return nil
	}
	snap := batcheval.GetAndRemoveLockTableSnapshot(res.SnapshotId)
