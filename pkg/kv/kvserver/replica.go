...
type Replica struct {
	...
	mu struct {
		...
		// system config is successfully gossipped or when the Replica loses the
		// lease. It is read when handling a MaybeGossipSystemConfigIfHaveFailure
		// local result trigger. That trigger is are set when an EndTransaction
		// with an ABORTED status is evaluated on a range containing the system
		// config span.
		//
		...
		// gossipped. This degradation is due to the fact that schema changes, after
		...
		// being gossipped and attempting to gossip again.
