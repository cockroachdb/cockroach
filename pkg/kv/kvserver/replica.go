...
//
// We iterate manually with GetSpanConfigForKey rather than using
// ForEachOverlappingSpanConfig because the latter is on
// spanconfig.Store, not on spanconfig.StoreReader / KVSubscriber.
func entireSpanExcludedFromBackup(
	...
	// Iterate over all span configs covering the request span, verifying
	// that each one has ExcludeDataFromBackup set.
	key := roachpb.RKey(sp.Key)
	endKey := roachpb.RKey(sp.EndKey)
	for key.Less(endKey) {
		conf, confSpan, err := confReader.GetSpanConfigForKey(ctx, key)
		if err != nil {
		...
		// Advance past this span config's bounds.
		nextKey := roachpb.RKey(confSpan.EndKey)
		if !key.Less(nextKey) {
			// GetSpanConfigForKey returns a zero confSpan when the key
			// falls outside all btree entries and the fallback config
			// is used (see spanconfigstore.getSpanConfigForKeyRLocked).
			// Guard against that to avoid an infinite loop.
			log.KvExec.Warningf(
				ctx, "ExcludeDataFromBackup: span config at %s did not advance past key %s", confSpan, key,
			)
			return false, nil
		}
		key = nextKey
