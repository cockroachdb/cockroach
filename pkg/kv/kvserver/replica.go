...
func (r *Replica) maybeWatchForMergeLocked(ctx context.Context) (bool, error) {
	...
	err = r.store.stopper.RunAsyncTask(taskCtx, "wait-for-merge", func(ctx context.Context) {
		...
		if shouldQueueForGC {
			...
			// into the GC queue is via RaftGroupDeletedError responses from Raft
			// traffic. If the Raft group has quiesced, those responses may never
			// arrive. Queue directly so the GC queue can process this replica (or
			// place it in purgatory if the left neighbor isn't ready yet).
