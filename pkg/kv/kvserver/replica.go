...
// This setting controls whether requests are rejected based on the GC threshold
// and the current GC TTL (true) or just based on the GC threshold (false).
var strictGCEnforcement = settings.RegisterBoolSetting(
	"kv.gc_ttl.strict_enforcement",
...
func (r *Replica) getImpliedGCThresholdRLocked(
	...
	if c.readAt.Less(st.Lease.Start) {
		return threshold
	}
