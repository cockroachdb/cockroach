...
func (r *Replica) State(ctx context.Context) kvserverpb.RangeInfo {
	...
	ri.TruncatedState = (protoutil.Clone(&logStorage.shMu.raftTruncState)).(*kvserverpb.RaftTruncatedState)

	ri.LastIndex = logStorage.shMu.lastIndexNotDurable
	ri.NumPending = uint64(r.numPendingProposalsRLocked())
	ri.RaftLogSize = logStorage.shMu.raftLogSize
	ri.RaftLogSizeTrusted = logStorage.shMu.raftLogSizeTrusted
...
func (r *Replica) assertStateRaftMuLockedReplicaMuRLocked(
	...
	} else if loaded, err := r.asLogStorage().raftMu.stateLoader.LoadRaftTruncatedState(ctx, reader); err != nil {
