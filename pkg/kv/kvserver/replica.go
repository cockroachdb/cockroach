...
// MinimumValidObservedTimestamp returns the minimum timestamp that can be used
// as a local limit when evaluating a request. See the comment on
// uncertainty.ComputeInterval for an explanation of cases where observed
...
// range merge).
func (r *Replica) MinimumValidObservedTimestamp() (timestamp hlc.ClockTimestamp) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	return r.mu.minValidObservedTimestamp
}
