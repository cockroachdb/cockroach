...
func (r *Replica) RefreshPolicy(latencies map[roachpb.NodeID]time.Duration) {
	policy := func() ctpb.RangeClosedTimestampPolicy {
		...
		// leaseholder-to-peer replica using the provided map and sets an
		// appropriate policy bucket which will control how far in the future
		// timestamps should be closed for global reads. Skip peers for which we
		// don't have latency information.
