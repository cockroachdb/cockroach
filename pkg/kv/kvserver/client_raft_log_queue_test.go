...
// TestRaftLogQueueSideloaded verifies that the raft log queue correctly
// truncates the raft log when it has sideloaded entries.
func TestRaftLogQueueSideloaded(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)
		...
		ServerArgs: base.TestServerArgs{
			Knobs: base.TestingKnobs{
				Store: &kvserver.StoreTestingKnobs{
					...
					AfterRaftLogSyncThrottle: func(id storage.FullReplicaID) { applyThrottle.get()(id) },
					TestingProposalFilter: func(args kvserverbase.ProposalFilterArgs) *kvpb.Error {
						return propFilter.get()(args)
					},
					TestingPostSideEffectsFilter: func(args kvserverbase.ApplyFilterArgs) (int, *kvpb.Error) {
			...
			RaftConfig: base.RaftConfig{
				RaftTickInterval:  time.Hour, // disable elections
	...
	//
	// TODO(pavelkalinnikov): group proposal lifecycle callbacks into a convenient
