...
func TestCrashWhileTruncatingSideloadedEntries(t *testing.T) {
	...
	tc := testcluster.StartTestCluster(t, 3, base.TestClusterArgs{
		...
		ServerArgs: base.TestServerArgs{
			...
			RaftConfig: base.RaftConfig{
				...
				RaftTickInterval:   24 * time.Hour, // disable elections
	...
	// TODO(pavelkalinnikov): the emulation here is not entirely correct. We need
	// to "hang" the node's connectivity first, so that durability events do not
	// leak from this node after SetIgnoreSyncs(true).
	tc.StopServer(1)
