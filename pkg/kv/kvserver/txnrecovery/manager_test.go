		...
		t.Run(fmt.Sprintf("strength:%s", str), func(t *testing.T) {
			testutils.RunTrueAndFalse(t, "prevent", func(t *testing.T, prevent bool) {
				...
				mockSender = kv.SenderFunc(func(
					...
					assert.Equal(t, str, ba.Requests[1].GetInner().(*kvpb.QueryIntentRequest).Strength)
					assert.Equal(t, str, ba.Requests[2].GetInner().(*kvpb.QueryIntentRequest).Strength)
					assert.Equal(
						t, enginepb.TxnSeq(11), ba.Requests[1].GetInner().(*kvpb.QueryIntentRequest).Txn.Sequence,
					)
					assert.Equal(
						t, enginepb.TxnSeq(12), ba.Requests[2].GetInner().(*kvpb.QueryIntentRequest).Txn.Sequence,
					)

					assert.Equal(t, []enginepb.IgnoredSeqNumRange{{Start: 1, End: 10}},
						ba.Requests[1].GetInner().(*kvpb.QueryIntentRequest).IgnoredSeqNums)
					assert.Equal(t, []enginepb.IgnoredSeqNumRange{{Start: 1, End: 10}},
						ba.Requests[2].GetInner().(*kvpb.QueryIntentRequest).IgnoredSeqNums)
