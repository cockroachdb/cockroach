...
func TestRaftFortificationEnabledForRangeID(t *testing.T) {
	...
	t.Run("fracEnabled=0.0", func(t *testing.T) {
		for i := range 10_000 {
			require.False(t, kvserver.TestingRaftFortificationEnabledForRangeID(0.0, roachpb.RangeID(i)))
		}
	})
	...
	t.Run("fracEnabled=1.0", func(t *testing.T) {
		for i := range 10_000 {
			require.True(t, kvserver.TestingRaftFortificationEnabledForRangeID(1.0, roachpb.RangeID(i)))
		}
	})
	...
	for _, tc := range testCases {
		...
		t.Run(name, func(t *testing.T) {
			require.Equal(t, tc.exp, kvserver.TestingRaftFortificationEnabledForRangeID(tc.fracEnabled, tc.rangeID))
		})
	}
	...
	for _, fracEnabled := range []float64{0.1, 0.01, 0.001} {
		t.Run(fmt.Sprintf("fracEnabled=%f", fracEnabled), func(t *testing.T) {
			...
			const total = 100_000
			for i := range total {
				if kvserver.TestingRaftFortificationEnabledForRangeID(fracEnabled, roachpb.RangeID(i)) {
					enabled++
				}
			}
	...
	t.Run("fracEnabled=invalid", func(t *testing.T) {
		require.Panics(t, func() {
			kvserver.TestingRaftFortificationEnabledForRangeID(-0.1, 1)
		})
		require.Panics(t, func() {
			kvserver.TestingRaftFortificationEnabledForRangeID(1.1, 1)
		})
	})
}
...
func TestRaftFortificationDisabledForExpirationBasedLeases(t *testing.T) {
	...
	testutils.RunTrueAndFalse(t, "useExpirationBasedLeases", func(t *testing.T, useExpirationBasedLeases bool) {
		...
		for _, test := range testCases {
			...
			if useExpirationBasedLeases {
				require.False(t, repl.TestingSupportFromEnabled())
			} else {
				require.Equal(t, test.exp, repl.TestingSupportFromEnabled())
			}
		}
	})
}
...
func BenchmarkRaftFortificationEnabledForRangeID(b *testing.B) {
	for i := 0; i < b.N; i++ {
		_ = kvserver.TestingRaftFortificationEnabledForRangeID(0.5, roachpb.RangeID(i))
	}
}
