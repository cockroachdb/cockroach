...
func TestLeaseTransferAfterStoreLivenessSupportWithdrawn(t *testing.T) {
	...
	// n1 is the preffered leaseholder, but it should only do so if it doesn't
	// treat n1 as suspect. Currently, n1 is not treated as suspect if support
	// has been recently withdrawn from it.
	...
	// Wait for the lease to transfer back to n1.
	// This demonstrates the bug: the lease should NOT transfer back immediately
	// because n1 was recently suspect (had support withdrawn).
	//
