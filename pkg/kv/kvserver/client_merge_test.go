...
func TestStoreRangeMergeRaftSnapshot(t *testing.T) {
	...
	var receivingEng engine.Engine
	...
	storeCfg.TestingKnobs.BeforeSnapshotSSTIngestion = func(
		...
		// should be in the SST.
		{
			...
			if err := sst.ClearRange(engine.MakeMVCCMetadataKey(roachpb.Key("a")), engine.MakeMVCCMetadataKey(roachpb.Key("c").Next())); err != nil {
				return err
			}

			// Insert keys "a", "b" and "c" with a timestamp that matches the
			// sender's and has a value of 1.
			createValFunc := func(key string, val int64) (engine.MVCCKey, roachpb.Value) {
				k := roachpb.Key(key)
				mvccKey := engine.MVCCKey{Key: k, Timestamp: tsMap[string(k)]}
				var v roachpb.Value
				v.SetInt(val)
				v.InitChecksum(k)
				return mvccKey, v
		...
		if err := engine.ClearRangeWithHeuristic(receivingEng, &sst, roachpb.Key("d0"), roachpb.Key("d9").Next()); err != nil {
			return err
		}
