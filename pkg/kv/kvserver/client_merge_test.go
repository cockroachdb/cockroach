...
func mergeCheckingTimestampCaches(
	...
	if !throughSnapshot {
		...
		var truncIndex roachpb.RaftIndex
		...
		restoredLeaseholderFuncs.dropReq = func(req *kvserverpb.RaftMessageRequest) bool {
			...
			return req.Message.Type == raftpb.MsgApp && roachpb.RaftIndex(req.Message.Index) < truncIndex
		}
		...
		filterMu.mergeCommitFilter = func() {
			...
			min := func(a, b roachpb.RaftIndex) roachpb.RaftIndex {
				if a < b {
					return a
				}
				return b
			}
			minLastIndex := roachpb.RaftIndex(math.MaxUint64)
			for _, r := range lhsRepls {
