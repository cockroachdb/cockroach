...
func TestStoreRangeMergeTimestampCacheCausality(t *testing.T) {
	...
	tc.WaitForValues(t, rhsKey, []int64{1, 1, 1})
...
func TestMergeQueueSeesNonVoters(t *testing.T) {
	...
	// NB: The setup code  subtest places a single voter replica on (n1,s1) for
	// both left and right range.
	tests := []test{
		{
			...
			expectedRightVoters:    []int{0, 2, 3, 4},
	...
	for _, subtest := range tests {
		t.Run(subtest.name, func(t *testing.T) {
			...
			for _, id := range subtest.leftVoters {
				tc.AddVotersOrFatal(t, leftDesc.StartKey.AsRawKey(), tc.Target(id))
				leftDesc = tc.LookupRangeOrFatal(t, leftDesc.StartKey.AsRawKey())
			}
			for _, id := range subtest.rightVoters {
			...
			voterTargets, nonVoterTargets := kvserver.GetTargetsToCollocateRHSForMerge(leftDesc.Replicas(),
				rightDesc.Replicas())
