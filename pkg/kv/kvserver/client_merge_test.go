...
func TestHistoricalReadsAfterSubsume(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			go func() {
				errCh <- sendTxn(store, rhsDesc, test.queryTsFunc(freezeStart), maxOffset,
...
func TestStoreBlockTransferLeaseRequestAfterSubsumption(t *testing.T) {
	...
	go func() {
		...
		errCh <- sendTxn(store, rhsDesc, freezeStart.Prev(), maxOffset, args)
	}()
		...
		for {
			*desc, err = tc.AddReplicas(desc.StartKey.AsRawKey(), tc.Target(1))
			if kv.IsExpectedRelocateError(err) {
				log.Infof(ctx, "ChangeReplicas: received error %s, retrying", err)
				// Retry.
				continue
			}
			break
