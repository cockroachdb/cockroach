...
// rlocked. It also optionally takes a optional hint: if sufficient is not
...
func (r *Replica) maxClosedRLocked(
	...
	// Consider timestamp closed through the side-transport. Such a timestamp can be in two places:
	// - r.mu.sideTransportClosedTimestamp
	// - in the ClosedTimestampReceiver
	sideTransportClosed, sideTransportClosedLAI := r.getSideTransportClosedTimestampRLocked()
	if appliedLAI >= sideTransportClosedLAI {
		sideTransportClosed = r.mu.sideTransportClosedTimestamp
	}
	...
	if !lease.Empty() && r.store.cfg.ClosedTimestampReceiver != nil {
		...
		// If otherSideTransportClosed ends up winning, we return it so that the
		// caller copies it into the replica so that future calls with `sufficient`
		// set don't need to go to the ClosedTimestampReceiver for a while.
