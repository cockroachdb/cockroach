...
//
// An execution/run is a sequence of contiguous StepID starting from 0. A valid
// run completes all specified StepIDs sequentially and exactly once.
//
...
// latter case, there must be a way to clarify the steps and remove deadlocks.
...
// indefinitely if this step has been completed before. For each step ID, there
// must be at most one invocation of Step.
...
// before. The caller must ensure that, for any step ID that is gated through
// StepIf, the RW function returns true exactly once.
//
