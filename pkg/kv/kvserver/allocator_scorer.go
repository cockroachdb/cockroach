...
func candidateListForRemoval(
	...
	candidateStoreList := makeStoreList(removalCandidateStores)
	for i := range candidates {
		...
		candidates[i].balanceScore = options.balanceScore(
			candidateStoreList, candidates[i].store.Capacity,
		)
...
func (eqClass equivalenceClass) rebalanceTargetsToMinimizeQPSDelta(
	...
	rebalanceCandidates := eqClass.candidateSL.stores
	...
	coldestCandidate := getStoreWithMinQPS(storeQPSMap, rebalanceCandidates)
	...
	existingQPSWithoutRepl := math.Max(existingQPS-perReplicaQPS, 0)
	qpsDifference := existingQPSWithoutRepl - coldestCandidate.Capacity.QueriesPerSecond
	if qpsDifference < minQPSDifferenceForReplicaTransfers {
		declineReason = fmt.Sprintf(
			"s%d qps %.2f is too close to the coldest candidate s%d with %.2f qps",
			existingStore.StoreID,
			existingQPSWithoutRepl,
	...
	storeQPSMap[existingStore.StoreID] = existingQPSWithoutRepl
	...
	// `coldestCandidate`.
	if storeQPSMap[existingStore.StoreID]+maxQPSReplicaTransferOvershoot < storeQPSMap[coldestCandidate.StoreID] {
		declineReason = fmt.Sprintf(
	...
	declineReason = fmt.Sprintf("qps delta between the stores (%0.2f) is as low as it can be", currentQPSDelta)
	return roachpb.StoreDescriptor{}, roachpb.StoreDescriptor{}, false /* shouldRebalance */, declineReason
}
