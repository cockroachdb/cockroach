...
func allocateCandidates(
	...
	for _, s := range sl.stores {
		...
		candidates = append(candidates, candidate{
			...
			rangeCount:     int(impliedStoreRangeCount(ctx, sl, s)),
		})
	}
...
func rebalanceCandidates(
	...
	for _, existing := range existingStores {
		...
		comparableStores = append(comparableStores, comparableStoreList{
			...
			sl:         allStores.makeStoreList(bestStores),
	...
	for _, comparable := range comparableStores {
		...
		for _, existingDesc := range comparable.existing {
			...
			existing.rangeCount = int(impliedStoreRangeCount(ctx, comparable.sl, existing.store))
			existingCandidates = append(existingCandidates, existing)
		}
		...
		for _, cand := range comparable.candidates {
			...
			cand.rangeCount = int(impliedStoreRangeCount(ctx, comparable.sl, s))
...
func shouldRebalance(
	...
	rc := impliedStoreRangeCount(ctx, sl, store)
	...
	if float64(rc) > sl.candidateRanges.mean {
		...
		for _, desc := range sl.stores {
			descRangeCount := impliedStoreRangeCount(ctx, sl, desc)
...
// impliedStoreRangeCount computes the rangeCount to use for a given store.
func impliedStoreRangeCount(ctx context.Context, sl StoreList, s roachpb.StoreDescriptor) int32 {
...
func balanceScore(
	...
	rangeCount := float64(impliedStoreRangeCount(ctx, sl, s))
...
func rebalanceFromConvergesOnMean(
	...
	return rebalanceConvergesOnMean(ctx, sl, s, impliedStoreRangeCount(ctx, sl, s)-1)
}
...
func rebalanceConvergesOnMean(
	...
	return convergesOnMean(float64(impliedStoreRangeCount(ctx, sl, s)), float64(newRangeCount), sl.candidateRanges.mean)
}
