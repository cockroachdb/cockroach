...
type scorerOptions interface {
	...
	// referred to by `sd` based on whether moving a replica away from this store
	// would converge its stats towards the mean (relative to the equivalence
	// class `eqClass`). If moving the replica away from `sd` would not converge
	// its stats towards the mean, a high convergence score is assigned, which
	// would make it less likely for us to pick this store's replica to move away.
	rebalanceFromConvergesScore(eqClass equivalenceClass, sd roachpb.StoreDescriptor) int
	...
	// referred to by `sd` will converge its stats towards the mean (relative to
	// the equivalence class `eqClass`). This makes it more likely for us to pick
	// this store as the rebalance target.
	rebalanceToConvergesScore(eqClass equivalenceClass, sd roachpb.StoreDescriptor) int
	// removalConvergesScore is almost identical to `rebalanceFromConvergesScore`
	// (both deal with computing a converges score for existing stores that might
	// relinquish a replica). The difference is that `removalConvergesScore`
	// operates over the set of existing replicas with equivalent diversity scores
	// (`removalCandidates`) whereas `rebalanceFromConvergesScore` operates over
	// the entire equivalence class.
	removalConvergesScore(removalCandidates []roachpb.StoreDescriptor, sd roachpb.StoreDescriptor) int
}
...
// `sd` will _not_ converge its range count towards the mean of stores in the
// equivalence class `eqClass`. When we're considering whether to rebalance a
// replica away from a store or not, we want to give it a "boost" (i.e. make it
// a less likely candidate for removal) if it doesn't further our goal to
// converge range count towards the mean.
func (o rangeCountScorerOptions) rebalanceFromConvergesScore(
	eqClass equivalenceClass, sd roachpb.StoreDescriptor,
) int {
	if !rebalanceConvergesRangeCountOnMean(eqClass.candidateSL, sd.Capacity, sd.Capacity.RangeCount-1) {
		return 1
	}
	return 0
}
...
// converge its range count towards the mean of all the stores inside `eqClass`.
...
// removalConvergesScore implemented by `rangeCountScorerOptions` is identical
// to `rebalanceFromConvergesScore`.
func (o rangeCountScorerOptions) removalConvergesScore(
	removalCandidateStores []roachpb.StoreDescriptor, sd roachpb.StoreDescriptor,
) int {
	if !rebalanceConvergesRangeCountOnMean(
		makeStoreList(removalCandidateStores), sd.Capacity, sd.Capacity.RangeCount-1,
	) {
		return 1
	}
	return 0
...
type qpsScorerOptions struct {
	...
	// QPS-based rebalancing assumes that every replica of a range receives the
...
func (o qpsScorerOptions) shouldRebalanceBasedOnThresholds(
	...
	addTarget, removeTarget, shouldRebalance, declineReason := eqClass.rebalanceTargetsToMinimizeQPSDelta(
		o.qpsPerReplica, o.qpsRebalanceThreshold,
	)
	if shouldRebalance {
		log.VEventf(
			ctx, 3, "should-rebalance(qps): rebalancing replica with qps %0.2f on s%d (%0.2f) to s%d (%0.2f)",
			o.qpsPerReplica, removeTarget.StoreID, removeTarget.Capacity.QueriesPerSecond,
			addTarget.StoreID, addTarget.Capacity.QueriesPerSecond,
		)
	} else {
		log.VEventf(
			ctx, 5, "not rebalancing away from s%d: %s", eqClass.existing.StoreID, declineReason,
		)
	}
...
// rebalanceFromConvergesScore returns a score of -1 if `sd` needs to be
// rebalanced away in order to minimize the QPS delta between the stores in the
// equivalence class `eqClass`.
func (o qpsScorerOptions) rebalanceFromConvergesScore(
	eqClass equivalenceClass, sd roachpb.StoreDescriptor,
) int {
	_, remove, shouldRebalance, _ := eqClass.rebalanceTargetsToMinimizeQPSDelta(
		o.qpsPerReplica,
		o.qpsRebalanceThreshold,
	)
	if shouldRebalance && remove.StoreID == sd.StoreID {
		return -1
	}
	return 0
}

// rebalanceToConvergesScore returns a score of 1 if `sd` needs to be rebalanced
// to in order to minimize the QPS delta between the stores in the equivalence
// class `eqClass`
func (o qpsScorerOptions) rebalanceToConvergesScore(
	eqClass equivalenceClass, sd roachpb.StoreDescriptor,
) int {
	add, _, shouldRebalance, _ := eqClass.rebalanceTargetsToMinimizeQPSDelta(
		o.qpsPerReplica,
		o.qpsRebalanceThreshold,
	)
	if shouldRebalance && add.StoreID == sd.StoreID {
		return 1
	}
	return 0
}

// removalConvergesScore returns a score of -1 `sd` is the hottest store (based
// on QPS) among the stores inside `removalCandidateStores`.
func (o qpsScorerOptions) removalConvergesScore(
	removalCandidateStores []roachpb.StoreDescriptor, sd roachpb.StoreDescriptor,
) int {
	maxQPS := float64(-1)
	storeWithMaxQPS := roachpb.StoreDescriptor{}
	for _, store := range removalCandidateStores {
		// NB: In the case that there are multiple stores with `maxQPS`, we return
		// -1 for all of them. This will make it such that we randomly pick one of
		// these stores for removal. See candidateList.selectBad().
		if store.Capacity.QueriesPerSecond >= maxQPS {
			maxQPS = store.Capacity.QueriesPerSecond
			storeWithMaxQPS = store
		}
	}
	// if `sd` is the the hottest store, assign a negative convergesScore. This
	// makes it such that our heuristics will pick it over other stores for
	// removal.
	if storeWithMaxQPS.StoreID == sd.StoreID {
		return -1
	}
	return 0
}
...
func candidateListForRemoval(
	...
	for i := range candidates {
		...
		candidates[i].convergesScore = options.removalConvergesScore(
			removalCandidateStores, candidates[i].store,
		)
...
const (
	// minQPSDifferenceForLeaseTransfers is the minimum QPS difference that the
	// store rebalancer would care to reconcile (via lease transfers) between any
	// two stores that hold a replica for a range.
	minQPSDifferenceForLeaseTransfers = 2 * minQPSThresholdDifference
	// minQPSDifferenceForReplicaTransfers is the minimum QPS difference that the
	// store rebalancer would care to reconcile (via replica rebalancing) between
	// any stores in an equivalence class.
	minQPSDifferenceForReplicaTransfers = 2 * minQPSThresholdDifference
	...
	// minQPS{Lease,Replica}TransferOvershoot qps).
	maxQPSLeaseTransferOvershoot   = 500
	maxQPSReplicaTransferOvershoot = 500
)

// rebalanceTargetsToMinimizeQPSDelta computes a set of rebalance targets (i.e.
// an existing store that needs to be removed in favor of a new candidate store
// from the equivalence class) such that executing the rebalance decision would
// minimize the QPS range between all the stores in the equivalence class.
func (eqClass equivalenceClass) rebalanceTargetsToMinimizeQPSDelta(
	perReplicaQPS, qpsRebalanceThreshold float64,
) (addTarget, removeTarget roachpb.StoreDescriptor, shouldRebalance bool, declineReason string) {
	storeQPSMap := make(map[roachpb.StoreID]float64, len(eqClass.candidateSL.stores)+1)

	for _, store := range eqClass.candidateSL.stores {
		storeQPSMap[store.StoreID] = store.Capacity.QueriesPerSecond
	}
	storeQPSMap[eqClass.existing.StoreID] = eqClass.existing.Capacity.QueriesPerSecond

	allStoresInClass := append(eqClass.candidateSL.stores, eqClass.existing)
	currentQPSDelta := getStoreQPSDelta(storeQPSMap, allStoresInClass)

	// If moving the replica from the existing store to the coldest candidate
	// brings the delta between these stores down, then `existingStore` could
	// potentially be rebalanced away.
	coldestCandidate := getStoreWithMinQPS(storeQPSMap, eqClass.candidateSL.stores)
	existingStore := eqClass.existing

	// Only proceed if the QPS difference between `existingStore` and
	// `coldestCandidate` (not accounting for the replica under consideration) is
	// higher than `minQPSDifferenceForReplicaTransfers`.
	existingQPS := storeQPSMap[existingStore.StoreID]
	...
	existingQPSIgnoringRepl := math.Max(existingQPS-perReplicaQPS, 0)

	diffIgnoringRepl := existingQPSIgnoringRepl - coldestCandidate.Capacity.QueriesPerSecond
	if diffIgnoringRepl < minQPSDifferenceForReplicaTransfers {
		declineReason = fmt.Sprintf(
			"s%d qps %.2f is too close to the coldest candidate s%d with %.2f qps",
			existingStore.StoreID,
			existingQPSIgnoringRepl,
			coldestCandidate.StoreID,
			coldestCandidate.Capacity.QueriesPerSecond,
		)
		return roachpb.StoreDescriptor{}, roachpb.StoreDescriptor{}, false /* shouldRebalance */, declineReason
	}

	// Only proceed with rebalancing iff `existingStore` is overfull relative to
	// the equivalence class.
	allStoresList := makeStoreList(allStoresInClass)
	mean := allStoresList.candidateQueriesPerSecond.mean
	overfullThreshold := mean + math.Max(mean*qpsRebalanceThreshold, minQPSThresholdDifference)
	if existingQPS < overfullThreshold {
		declineReason = fmt.Sprintf(
			"s%d qps %.2f is below the overfull threshold %.2f",
			existingStore.StoreID, existingQPS, overfullThreshold,
		)
		return roachpb.StoreDescriptor{}, roachpb.StoreDescriptor{}, false /* shouldRebalance */, declineReason
	}

	// Simulate the coldest candidate's QPS after it receives a replica for the
	// range.
	storeQPSMap[coldestCandidate.StoreID] += perReplicaQPS
	// Simulate the hottest existing store's QPS after it sheds the replica away.
	storeQPSMap[existingStore.StoreID] = existingQPSIgnoringRepl
	coldestCandidateWithRepl := storeQPSMap[coldestCandidate.StoreID]

	// Only proceed if coldestCandidate doesn't become significantly hotter than
	// the existingStore after this rebalance. This means we're going to avoid
	// rebalances that switch the relative dispositions of the `existingStore` and
	// `coldestCandidate`.
	if storeQPSMap[existingStore.StoreID]+maxQPSReplicaTransferOvershoot < coldestCandidateWithRepl {
		declineReason = fmt.Sprintf(
			"coldest candidate s%d (%.2f -> %.2f qps) will become hotter than s%d (%.2f -> %.2f qps)",
			coldestCandidate.StoreID,
			coldestCandidate.Capacity.QueriesPerSecond,
			storeQPSMap[coldestCandidate.StoreID],
			existingStore.StoreID,
			existingQPS,
			existingQPSIgnoringRepl,
		)
	...
	newQPSDelta := getStoreQPSDelta(storeQPSMap, allStoresInClass)

	// NB: We proceed with a rebalance even if `currentQPSDelta` is exactly equal
	// to `newQPSDelta`. Consider the following example:
	// perReplicaQPS: 10qps
	// existingQPS: 100qps
	...
	if currentQPSDelta < newQPSDelta {
		panic(fmt.Sprintf(
			"programming error: projected QPS delta higher than current delta;"+
				" existing: %0.2f qps, coldest candidate: %0.2f qps, replica: %0.2f qps",
			existingQPS, coldestCandidate.Capacity.QueriesPerSecond, perReplicaQPS,
		))
	}

	return coldestCandidate, existingStore, true /* shouldRebalance */, ""
}
...
func rankedCandidateListForRebalancing(
	...
	for _, comparable := range equivalenceClasses {
		...
		if !existing.valid {
			...
			convergesScore := options.rebalanceFromConvergesScore(comparable, existing.store)
...
// getStoreWithMinQPS returns the `StoreDescriptor` that belongs to the store
// serving the lowest QPS among all the `candidate` stores.
func getStoreWithMinQPS(
	storeQPSMap map[roachpb.StoreID]float64, candidates []roachpb.StoreDescriptor,
) roachpb.StoreDescriptor {
	minCandidateQPS := math.MaxFloat64
	var candidateWithMin roachpb.StoreDescriptor
	for _, store := range candidates {
		candidateQPS, ok := storeQPSMap[store.StoreID]
		if !ok {
			continue
		}
		if minCandidateQPS > candidateQPS {
			minCandidateQPS = candidateQPS
			candidateWithMin = store
		}
	}
	return candidateWithMin
}

// getStoreQPSDelta computes the QPS delta between the coldest and the hottest
// store in `stores`.
func getStoreQPSDelta(
	storeQPSMap map[roachpb.StoreID]float64, stores []roachpb.StoreDescriptor,
) float64 {
	maxCandidateQPS := float64(0)
	minCandidateQPS := math.MaxFloat64
	for _, repl := range stores {
