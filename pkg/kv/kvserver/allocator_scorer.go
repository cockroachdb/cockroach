...
func (c byScoreAndID) Less(i, j int) bool {
	if scoresAlmostEqual(c[i].diversityScore, c[j].diversityScore) &&
		...
		c[i].l0SubLevels < c[j].l0SubLevels &&
		c[i].valid == c[j].valid {
...
func rankedCandidateListForAllocation(
	...
	for _, s := range candidateStores.stores {
		...
		if !maxCapacityCheck(s) || !readAmpIsHealthy(ctx, options.storeHealthOptions, s, candidateStores.candidateL0Sublevels.mean) {
			continue
		}
...
func rankedCandidateListForRebalancing(
	...
	for _, store := range allStores.stores {
		for _, repl := range existingReplicasForType {
			...
			unhealthyReadAmp := !rebalanceToReadAmpIsHealthy(
				ctx,
				options.getStoreHealthOptions(),
				store,
				allStores.candidateL0Sublevels.mean,
			)
			if !valid {
			...
			existingStores[store.StoreID] = candidate{
				...
				necessary:      necessary,
				fullDisk:       fullDisk,
				l0SubLevels:    int(store.Capacity.L0Sublevels),
				highReadAmp:    unhealthyReadAmp,
	...
	for _, existing := range existingStores {
		...
		for _, store := range allStores.stores {
			...
			readAmpOk := rebalanceToReadAmpIsHealthy(
				ctx,
				options.getStoreHealthOptions(),
				store,
				allStores.candidateL0Sublevels.mean,
	...
	for _, comparable := range equivalenceClasses {
		...
		for _, cand := range comparable.candidates {
			...
			cand.highReadAmp = !rebalanceToReadAmpIsHealthy(
				ctx,
				options.getStoreHealthOptions(),
				s,
				allStores.candidateL0Sublevels.mean,
...
func readAmpIsHealthy(
	ctx context.Context, options *storeHealthOptions, store roachpb.StoreDescriptor, avg float64,
) bool {
...
func rebalanceToReadAmpIsHealthy(
	ctx context.Context, options *storeHealthOptions, store roachpb.StoreDescriptor, avg float64,
) bool {
	if options.enforcementLevel == storeHealthNoAction ||
		store.Capacity.L0Sublevels < options.l0SublevelThreshold {
