...
type replicaForTruncator interface {
	...
	stageTruncatedState(_ context.Context, _ pendingTruncation)
...
func (t *raftLogTruncator) tryEnactTruncations(
	...
	pendingTruncs.iterateLocked(func(index int, trunc pendingTruncation) {
		...
		r.stageTruncatedState(ctx, trunc)
	})
	...
	// Truncation done. Need to finalize updating the Replica state. This requires
	// iterating over all the enacted entries.
	pendingTruncs.iterateLocked(func(index int, trunc pendingTruncation) {
		if index > enactIndex {
			return
		}
