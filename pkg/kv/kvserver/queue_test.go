...
type testQueueImpl struct {
	...
	noop          bool  // if noop enabled, process will return false
}
...
func TestBaseQueueNoop(t *testing.T) {
	...
	testutils.SucceedsSoon(t, func() error {
		if pc := testQueue.getProcessed(); pc != 1 {
			t.Errorf("expected 1 processed range; got %d", pc)
		}
		if v := bq.successes.Count(); v != 1 {
			t.Errorf("expected 1 processed replica; got %d", v)
		}
		return nil
	})
