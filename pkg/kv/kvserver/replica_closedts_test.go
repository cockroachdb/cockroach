	...
	for _, s := range tc.Servers {
		...
		g.Go(func() error {
			...
			for ; atomic.LoadInt32(&done) == 0; sleep() {
				...
				// Issue a transactional scan over the keys at the resolved timestamp.
				// Use an error wait policy so that we'll hear an error under conditions
				// that would otherwise cause us to block on an intent.
