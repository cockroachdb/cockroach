...
func TestClosedTimestampPolicyRefreshOnSetSpanConfig(t *testing.T) {
	...
	closedts.RangeClosedTimestampPolicyRefreshInterval.Override(ctx, &st.SV, 1*time.Millisecond)
...
// closed timestamp policy refresh interval is correctly applied to the node
// liveness range. That is, even if we try to set the node liveness range to
// have global reads, the closed timestamp policy should use
// LAG_BY_CLUSTER_SETTING. Read more in replica.closedTimestampPolicyRLocked.
