		...
		Knobs: base.TestingKnobs{
			Store: &kvserver.StoreTestingKnobs{
				TestingRequestFilter: func(ctx context.Context, ba *roachpb.BatchRequest) *roachpb.Error {
					// We'll recognized a GetRequest and a PushRequest, check that the
					...
					case getReq != nil:
						...
						if !isTenantRequest || tenID != tenantID {
							err = errors.Newf("expected Get to run as the expected tenant, but it isn't. tenant request: %t, tenantID: %d",
								isTenantRequest, tenID)
						}
						getErr <- err
						return nil
					case pushReq != nil:
