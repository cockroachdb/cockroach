...
func testSideloadingSideloadedStorage(t *testing.T, eng storage.Engine) {
	...
	file := func(index roachpb.RaftIndex, term roachpb.RaftTerm) []byte { // take uint64 for convenience
	...
	payloads := []roachpb.RaftIndex{3, 5, 7, 9, 10}
	...
	pastPayloads := append([]roachpb.RaftIndex{81}, payloads...)
	...
	for _, term := range []roachpb.RaftTerm{lowTerm, highTerm} {
	...
	// with.
	for index := roachpb.RaftIndex(1); index < 5; index++ {
		if index == 3 {
			continue
		}
	...
	for _, term := range []roachpb.RaftTerm{9, 11} {
