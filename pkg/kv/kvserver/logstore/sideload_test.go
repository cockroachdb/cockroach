...
func TestMkdirAllAndSyncParentsErrors(t *testing.T) {
	...
	t.Run("not-a-directory", func(t *testing.T) {
		vfs := vfs.NewMem()
		require.NoError(t, mkdirAllAndSyncParents(vfs, "/a", os.ModePerm))

		// Write a file, and try to trick mkdir into thinking that it's a directory.
		f, err := vfs.Create("/a/file", fs.UnspecifiedWriteCategory)
		require.NoError(t, err)
		require.NoError(t, f.Close())

		for _, path := range []string{"/a/file", "/a/file/sub"} {
			require.ErrorContains(t, mkdirAllAndSyncParents(vfs, path, os.ModePerm), "not a directory")
		}
	})
}
