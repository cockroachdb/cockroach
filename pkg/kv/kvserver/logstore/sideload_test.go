...
func testSideloadingSideloadedStorage(t *testing.T, eng storage.Engine) {
	...
	func() {
		...
		// Ensture HasEntries doesn't find anything.
		found, err := ss.HasEntries(ctx, 0, 10000)
		require.NoError(t, err)
		...
		assertExists(true)
		// Verify the HasEntries semantics.
		for _, check := range []struct {
			...
			found, err := ss.HasEntries(ctx, check.from, check.to)
			require.NoError(t, err)
