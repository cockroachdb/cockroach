...
func (ss *DiskSideloadStorage) Put(
	ctx context.Context, index roachpb.RaftIndex, term roachpb.RaftTerm, contents []byte,
) error {
...
func (ss *DiskSideloadStorage) Get(
	ctx context.Context, index roachpb.RaftIndex, term roachpb.RaftTerm,
) ([]byte, error) {
	filename := ss.filename(ctx, index, term)
...
func (ss *DiskSideloadStorage) TruncateTo(
	ctx context.Context, firstIndex roachpb.RaftIndex,
...
func (ss *DiskSideloadStorage) possiblyTruncateTo(
	ctx context.Context, from roachpb.RaftIndex, to roachpb.RaftIndex, doTruncate bool,
) (bytesFreed, bytesRetained int64, _ error) {
	deletedAll := true
	if err := ss.forEach(ctx, func(index roachpb.RaftIndex, filename string) error {
		if index >= to {
			size, err := ss.fileSize(filename)
			if err != nil {
				return err
...
func (ss *DiskSideloadStorage) BytesIfTruncatedFromTo(
	ctx context.Context, from roachpb.RaftIndex, to roachpb.RaftIndex,
) (freed, retained int64, _ error) {
	return ss.possiblyTruncateTo(ctx, from, to, false /* doTruncate */)
}

func (ss *DiskSideloadStorage) forEach(
	ctx context.Context, visit func(index roachpb.RaftIndex, filename string) error,
) error {
	...
	for _, match := range matches {
		...
		if err := visit(roachpb.RaftIndex(logIdx), match); err != nil {
...
func (ss *DiskSideloadStorage) String() string {
	...
	var count int
	if err := ss.forEach(context.Background(), func(_ roachpb.RaftIndex, filename string) error {
		count++
