...
type SideloadStorage interface {
	...
	Put(_ context.Context, index roachpb.RaftIndex, term roachpb.RaftTerm, contents []byte) error
	// Load the file at the given index and term. Return errSideloadedFileNotFound when no
	// such file is present.
	Get(_ context.Context, index roachpb.RaftIndex, term roachpb.RaftTerm) ([]byte, error)
	...
	Purge(_ context.Context, index roachpb.RaftIndex, term roachpb.RaftTerm) (int64, error)
	...
	TruncateTo(_ context.Context, index roachpb.RaftIndex) (freed, retained int64, _ error)
	...
	BytesIfTruncatedFromTo(_ context.Context, from roachpb.RaftIndex, to roachpb.RaftIndex) (freed, retained int64, _ error)
	...
	Filename(_ context.Context, index roachpb.RaftIndex, term roachpb.RaftTerm) (string, error)
}
...
func MaybeSideloadEntries(
	...
	for i := range input {
		...
		if err := sideloaded.Put(ctx, roachpb.RaftIndex(outputEnt.Index), roachpb.RaftTerm(outputEnt.Term), dataToSideload); err != nil { // TODO could verify checksum here
			return nil, 0, 0, 0, err
		}
...
func MaybeInlineSideloadedRaftCommand(
	...
	cachedSingleton, _, _, _ := entryCache.Scan(
		nil, rangeID, roachpb.RaftIndex(ent.Index), roachpb.RaftIndex(ent.Index+1), 1<<20,
	)
	...
	sideloadedData, err := sideloaded.Get(ctx, roachpb.RaftIndex(ent.Index), roachpb.RaftTerm(ent.Term))
	if err != nil {
