...
func TestRaftStorageWrites(t *testing.T) {
	...
	testutils.RunTrueAndFalse(t, "single_delete", func(t *testing.T, useSingleDelete bool) {
		defer func(prev bool) { UseRaftLogSingleDelete = prev }(UseRaftLogSingleDelete)
		...
		// SingleClearUnversioned not updating MVCCStats. truncate resets
		// expectedDrift to 0 because the test recalibrates state.ByteSize
		// from stats() there.
		liveSizes := map[kvpb.RaftIndex]int64{}
		var expectedDrift int64
		...
		write := func(name string, hs raftpb.HardState, entries []raftpb.Entry) {
			...
			if useSingleDelete {
				...
				// Update liveSizes to reflect the post-call log: drop trimmed
				// trailing indices, then record (or overwrite) the new
				// entries.
				for i := newLast + 1; i <= state.LastIndex; i++ {
					delete(liveSizes, i)
				}
