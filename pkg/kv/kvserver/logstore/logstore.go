...
func logAppend(
	...
	if util.RaceEnabled {
		...
		defer la.Close()
		if err := assertNoLiveRaftLogEntriesAfter(ctx, la, prefixBuf, prev.LastIndex); err != nil {
			return RaftState{}, err
		}
	}
	...
	// PUTs for the same key. If we are not using single delete we rely on
	// MVCCBlindPut to overwrite those entries.
	// We will later delete (newLast, prev.LastIndex] so keys are deleted
	// in order.
	//
	// In all cases, we need to subtract [firstNew, prev.LastIndex] stats as
	// these entries will be deleted/overwritten.
	//
	...
	if overlapping {
		...
		if useSingleDelete {
			if err := ClearRangeSizeKnown(
				w, prefixBuf, newFirst-1, newLast, math.MaxInt, true,
			); err != nil {
				return RaftState{}, err
			}
		}
	...
	if overlapping {
		// Truncate the log suffix.
		if err := ClearRangeSizeKnown(
			w, prefixBuf, newLast, prev.LastIndex, math.MaxInt, useSingleDelete,
		); err != nil {
			return RaftState{}, err
		}
	}
