...
func logAppend(
	...
	// [newFirst, prev.lastIndex] here if we are using single delete. This is
	// because single delete relies on not having double PUTs for the same key.
	// If we are not using single delete we rely on MVCCBlindPut to overwrite
	// those entries, and we will later delete (newLast, prev.LastIndex] so
	// keys are deleted in order.
	//
