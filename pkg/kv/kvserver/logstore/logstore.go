...
// use Pebble's SingleDelete for raft log entries deletes or regular point
// deletes.
...
// parseRaftLogSingleDeleteMode decodes the value of the
// COCKROACH_RAFT_LOG_SINGLE_DELETE env var. It accepts "true"/"false"
// (case-insensitive) and treats anything else (including the empty string and
// the literal "default") as the default mode.
func parseRaftLogSingleDeleteMode(s string) raftLogSingleDeleteMode {
...
// parsed once at init.
var raftLogSingleDeleteEnv = parseRaftLogSingleDeleteMode(
	envutil.EnvOrDefaultString("COCKROACH_RAFT_LOG_SINGLE_DELETE", "default"))
...
// It is safe to use SingleDelete with separate engines because we are
// guaranteed an empty raft log history. This is because running CRDB with
// separate engines has two possible paths: completely new cluster, and
// migrating a cluster that uses a single engine to separate engines. Both cases
// have an empty raft log history, and therefore, SingleDelete is safe.
