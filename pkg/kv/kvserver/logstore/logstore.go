...
func (s *LogStore) storeEntriesAndCommitBatch(
	...
	overwriting := false
	if len(m.Entries) > 0 {
		firstPurge := roachpb.RaftIndex(m.Entries[0].Index) // first new entry written
	...
	if overwriting {
		...
		// just above).
		firstPurge := roachpb.RaftIndex(m.Entries[0].Index) // first new entry written
		purgeTerm := roachpb.RaftTerm(m.Entries[0].Term - 1)
...
func logAppend(
	...
	for i := range entries {
		ent := &entries[i]
		key := keys.RaftLogKeyFromPrefix(raftLogPrefix, roachpb.RaftIndex(ent.Index))
		...
		var err error
		if roachpb.RaftIndex(ent.Index) > prev.LastIndex {
	...
	newLastIndex := roachpb.RaftIndex(entries[len(entries)-1].Index)
	...
	return RaftState{
		...
		LastTerm:  roachpb.RaftTerm(entries[len(entries)-1].Term),
...
func LoadTerm(
...
) (roachpb.RaftTerm, error) {
	entry, found := eCache.Get(rangeID, index)
	if found {
		return roachpb.RaftTerm(entry.Term), nil
	}
	...
	if found {
		...
		if got, want := roachpb.RaftIndex(entry.Index), index; got != want {
...
func LoadEntries(
	...
	if roachpb.RaftIndex(len(ents)) == hi-lo || exceededMaxBytes {
		return ents, nil
	}
	...
	scanFunc := func(ent raftpb.Entry) error {
		...
		if roachpb.RaftIndex(ent.Index) != expectedIndex {
	...
	if roachpb.RaftIndex(len(ents)) == hi-lo {
		return ents, nil
	}
