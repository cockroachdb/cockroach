...
type replicaFlowControlIntegrationImpl struct {
	...
	lastKnownDesc       *roachpb.RangeDescriptor
	disconnectedStreams map[roachpb.ReplicaID]kvflowcontrol.Stream
}

func newFlowControlIntegrationImpl(
	r *Replica, handleFactory kvflowcontrol.HandleFactory,
) *replicaFlowControlIntegrationImpl {
	return &replicaFlowControlIntegrationImpl{
		replicaForFlowControl: (*replicaFlowControl)(r),
...
func (f *replicaFlowControlIntegrationImpl) onBecameLeader(ctx context.Context) {
	...
	f.lastKnownDesc = f.replicaForFlowControl.getDescriptor()
...
func (f *replicaFlowControlIntegrationImpl) onDescChanged(ctx context.Context) {
	...
	addedReplicas, removedReplicas := f.lastKnownDesc.Replicas().Difference(
	...
	if len(addedReplicas) > 0 || len(removedReplicas) > 0 {
		log.VInfof(ctx, 1, "desc changed from %s to %s: added=%s removed=%s",
			f.lastKnownDesc, f.replicaForFlowControl.getDescriptor(), addedReplicas, removedReplicas,
		)
	}
	f.lastKnownDesc = f.replicaForFlowControl.getDescriptor()
}
...
func (f *replicaFlowControlIntegrationImpl) onFollowersPaused(ctx context.Context) {
	...
	for replID := range f.replicaForFlowControl.pausedFollowers() {
		repl, ok := f.lastKnownDesc.GetReplicaDescriptorByID(replID)
		if !ok {
			continue
		}
		if repl.ReplicaID == f.replicaForFlowControl.getReplicaID() {
	...
	f.disconnectStreams(ctx, toDisconnect)
	f.tryReconnect(ctx)
}
...
func (f *replicaFlowControlIntegrationImpl) onProposalQuotaUpdated(ctx context.Context) {
	...
	for _, repl := range f.lastKnownDesc.Replicas().Descriptors() {
		if f.replicaForFlowControl.isFollowerActive(ctx, repl.ReplicaID) {
...
// replicas that we're still probing to figure out its last index, or replicas
// that are pending raft snapshots because the leader has truncated away entries
// higher than its last position.
func (f *replicaFlowControlIntegrationImpl) notActivelyReplicatingTo() []roachpb.ReplicaID {
	...
	f.replicaForFlowControl.withReplicaProgress(func(replID roachpb.ReplicaID, progress rafttracker.Progress) {
		...
		_, ok := f.lastKnownDesc.GetReplicaDescriptorByID(replID)
		if !ok {
			return
		}
...
func (f *replicaFlowControlIntegrationImpl) disconnectStreams(
	ctx context.Context, toDisconnect []roachpb.ReplicaDescriptor,
) {
	for _, repl := range toDisconnect {
		...
		f.innerHandle.DisconnectStream(ctx, stream)
		f.disconnectedStreams[repl.ReplicaID] = stream
		log.VInfof(ctx, 1, "tracked disconnected stream: %s", stream)
	}
}
...
func (f *replicaFlowControlIntegrationImpl) tryReconnect(ctx context.Context) {
	...
	for replID, stream := range f.disconnectedStreams {
		...
		if _, ok := f.lastKnownDesc.GetReplicaDescriptorByID(replID); !ok {
			log.Fatalf(ctx, "%s: tracking %s in disconnected streams despite it not being in descriptor: %s",
				f.replicaForFlowControl.getReplicaID(), replID, f.lastKnownDesc)
		}
		if !f.replicaForFlowControl.isFollowerActive(ctx, replID) {
		...
		// See I2, I3, I3a, I4 from kvflowcontrol/doc.go. Replica is active on a
		// live node (I2), not paused (I3), and is being actively replicated to
		// through log entries (I3a, I4). Re-connect so we can start deducting
		// tokens for it.
		f.innerHandle.ConnectStream(ctx, appliedLogPosition, stream)
