	...
	datadriven.Walk(t, "testdata/replica_lifecycle", func(t *testing.T, path string) {
		...
		datadriven.RunTest(t, path, func(t *testing.T, d *datadriven.TestData) string {
			...
			case "append-raft-entries":
				...
				lastIndex := rs.replica.ts.Index
				rs.replica.ts.Index += kvpb.RaftIndex(numEntries)
				...
				for i := 0; i < numEntries; i++ {
					...
					require.NoError(t, storage.MVCCBlindPutProto(
						...
						&raftpb.Entry{Index: uint64(entryIndex), Term: uint64(term)},
			...
			case "create-range-data":
				rangeID := dd.ScanArg[roachpb.RangeID](t, d, "range-id")
				numUserKeys := dd.ScanArgOr[int](t, d, "num-user-keys", 0)
				numSystemKeys := dd.ScanArgOr[int](t, d, "num-system-keys", 0)
				numLockTableKeys := dd.ScanArgOr[int](t, d, "num-lock-table-keys", 0)
				...
				for i := 0; i < numUserKeys; i++ {
					key := append(rs.desc.StartKey.AsRawKey(), byte(i))
				...
				for i := 0; i < numSystemKeys; i++ {
					key := keys.TransactionKey(
						append(rs.desc.StartKey.AsRawKey(), byte(i)), uuid.NamespaceDNS,
					)
					require.NoError(t, batch.PutMVCC(
						storage.MVCCKey{Key: key, Timestamp: ts}, storage.MVCCValue{},
					))
				...
				for i := 0; i < numLockTableKeys; i++ {
					...
					ek, _ := storage.LockTableKey{
						Key: key, Strength: lock.Intent, TxnUUID: uuid.UUID{},
					}.ToEngineKey(nil)
