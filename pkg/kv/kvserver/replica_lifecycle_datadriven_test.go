	...
	datadriven.Walk(t, "testdata/replica_lifecycle", func(t *testing.T, path string) {
		...
		datadriven.RunTest(t, path, func(t *testing.T, d *datadriven.TestData) string {
			...
			case "set-lease":
				...
				require.NotNil(
					t, targetReplica,
					"replica with NodeID %d not found in range descriptor", replicaNodeID,
				)
				if leaseType == "" {
...
func (tc *testCtx) updatePostSplitRangeState(
	...
	originalRangeState, ok := tc.ranges[roachpb.RangeID(lhsRangeID)]
	require.True(t, ok, "original range state not found for RangeID %d", lhsRangeID)
	// The range ID should not change for LHS since it's the same range.
	require.Equal(t, lhsRangeID, int(split.LeftDesc.RangeID))
	// Update LHS by just updating the descriptor.
	originalRangeState.desc = split.LeftDesc
	tc.ranges[split.LeftDesc.RangeID] = originalRangeState
	...
	rhsLease, err := rhsSl.LoadLease(ctx, reader)
	require.NoError(t, err)
	rhsRangeState.lease = rhsLease
	rhsGCThreshold, err := rhsSl.LoadGCThreshold(ctx, reader)
	require.NoError(t, err)
	require.NotNil(t, rhsGCThreshold)
	rhsRangeState.gcThreshold = *rhsGCThreshold
	rhsGCHint, err := rhsSl.LoadGCHint(ctx, reader)
	require.NoError(t, err)
	require.NotNil(t, rhsGCHint)
	rhsRangeState.gcHint = *rhsGCHint
	rhsVersion, err := rhsSl.LoadVersion(ctx, reader)
	require.NoError(t, err)
