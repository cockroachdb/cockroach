...
func (msstw *multiSSTWriter) initSST(ctx context.Context) error {
	...
	if msstw.currSpan < len(msstw.localKeySpans) || (!msstw.skipClearForMVCCSpan && msstw.currSpan <= len(msstw.localKeySpans)) {
		...
		// to the fragmenter; we don't need to keep re-adding it as the fragmenter
		// will take care of splits.
		...
		// and use that here.
		ikey := pebble.MakeInternalKey(startKey, 0, pebble.InternalKeyKindRangeKeyDelete)
		s := rangekey.Span{Start: startKey, End: endKey, Keys: []rangekey.Key{{Trailer: ikey.Trailer}}}
...
func (msstw *multiSSTWriter) PutRangeKey(
	...
	// TODO(bilal): Use pebble.MakeTrailer instead of this, once that method is exported.
...
func (kvSS *kvBatchSnapshotStrategy) Receive(
	...
	msstw, err := newMultiSSTWriter(ctx, kvSS.st, kvSS.scratch, localRanges, mvccRange, kvSS.sstChunkSize, skipClearForMVCCSpan)
	if err != nil {
		return noSnap, err
	}
	defer msstw.Close()
