...
func (kvSS *kvBatchSnapshotStrategy) Receive(
	...
	// (with a StopSharedReplicate = true), we will have to switch to adding a
	// rangedel for that span. Since multiSSTWriter acts on an opaque slice of
	// keyRanges, we just tell it to add a rangedel for the last span. To avoid
	// bugs, assert on the last span in keyRanges actually being equal to the user
	// key span.
	if doExcise {
...
func (kvSS *kvBatchSnapshotStrategy) Send(
	...
	flushBatch := func() error {
		if err := kvSS.sendBatch(ctx, stream, b, ssts, stopSharedReplicate, timingTag); err != nil {
			return err
		}
		bLen := int64(b.Len())
		bytesSent += bLen
...
func (kvSS *kvBatchSnapshotStrategy) sendBatch(
	...
	res := stream.Send(&kvserverpb.SnapshotRequest{
		KVBatch:             batch.Repr(),
		SharedTables:        ssts,
		StopSharedReplicate: stopSharedReplicate,
	})
	timerTag.stop("send")
	return res
}
