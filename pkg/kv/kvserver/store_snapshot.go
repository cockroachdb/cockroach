...
func (msstw *multiSSTWriter) initSST(ctx context.Context) error {
	...
	if msstw.currSpan < len(msstw.localKeySpans) || (!msstw.skipRangeDelForMVCCSpan && msstw.currSpan <= len(msstw.localKeySpans)) {
		startKey := storage.EngineKey{Key: msstw.currentSpan().Key}.Encode()
		endKey := storage.EngineKey{Key: msstw.currentSpan().EndKey}.Encode()
		s := rangekey.Span{Start: startKey, End: endKey}
		// TODO(bilal): Export pebble's MakeTrailer so we don't have to create an internal key.
		// Or better yet, add a rangekey.MakeRangeKeyDelete() that returns a rangekey.Span{}.
		ikey := pebble.MakeInternalKey(s.Start, 0, pebble.InternalKeyKindRangeKeyDelete)
		s.Keys = append(s.Keys, rangekey.Key{Trailer: ikey.Trailer})
...
func (msstw *multiSSTWriter) PutRangeKey(
	...
	startIKey := pebble.MakeInternalKey(startKey, 0, pebble.InternalKeyKindRangeKeySet)
	msstw.rangeKeyFrag.Add(rangekey.Span{
		Start: startKey,
		End:   endKey,
		Keys:  []rangekey.Key{{Trailer: startIKey.Trailer, Suffix: suffix, Value: value}},
	})
	return nil
}
...
func (kvSS *kvBatchSnapshotStrategy) Receive(
	...
	msstw, err := newMultiSSTWriter(ctx, kvSS.st, kvSS.scratch, localRanges, mvccRange, kvSS.sstChunkSize, skipRangeDelForMVCCSpan)
	if err != nil {
		return noSnap, err
	}
	defer msstw.Close()
