...
func TestReplicaRangefeedPushesTransactions(t *testing.T) {
	...
	ctx := context.Background()
	tc, db, _, repls := setupClusterForClosedTimestampTesting(ctx, t, testingTargetDuration, defaultClusterArgs)
...
func TestReplicaRangefeedNudgeSlowClosedTimestamp(t *testing.T) {
	...
	ctx := context.Background()
	tc, db, desc, repls := setupClusterForClosedTimestampTesting(ctx, t, testingTargetDuration, defaultClusterArgs)
	defer tc.Stopper().Stop(ctx)
