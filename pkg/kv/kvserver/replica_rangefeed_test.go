...
func TestRangefeedCheckpointsRecoverFromLeaseExpiration(t *testing.T) {
	...
	expireLease := func(t *testing.T, newLeaseFails bool) {
		ctx := context.Background()
				...
				Store: &kvserver.StoreTestingKnobs{
					TestingRequestFilter: func(ctx context.Context, ba roachpb.BatchRequest) *roachpb.Error {
						...
						if ba.IsSingleLeaseInfoRequest() {
							...
							if newLeaseFails {
								if timeoutSimulated != 1 {
									mockTimeout := contextutil.MockTimeoutError()
									timeoutSimulated = 1
	...
	expireLease(t, false)
	t.Run("retriable", func(t *testing.T) { expireLease(t, true) })
}
