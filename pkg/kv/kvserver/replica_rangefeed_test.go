...
func TestReplicaRangefeedErrors(t *testing.T) {
	...
	t.Run("range key mismatch", func(t *testing.T) {
		knobs := base.TestingKnobs{
			Store: &kvserver.StoreTestingKnobs{
				// Use an span config override to check that we get a key mismatch error
		...
		// Split the range so that the RHS actually *is* a system range, while the
		// LHS is not. The LHS should still error that the key is outside the span.
		tc.SplitRangeOrFatal(t, startKey)
