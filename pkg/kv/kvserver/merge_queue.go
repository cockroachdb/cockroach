...
func newMergeQueue(store *Store, db *kv.DB) *mergeQueue {
	...
	mq.SetDisabled(!kvserverbase.MergeQueueEnabled.Get(&store.cfg.Settings.SV))
	kvserverbase.MergeQueueEnabled.SetOnChange(&store.cfg.Settings.SV, func(ctx context.Context) {
		mq.SetDisabled(!kvserverbase.MergeQueueEnabled.Get(&store.cfg.Settings.SV))
	})
	return mq
}
