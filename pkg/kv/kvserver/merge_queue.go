...
func (mq *mergeQueue) process(
	ctx context.Context, lhsRepl *Replica, confReader spanconfig.StoreReader, priority float64,
	...
	if testingAggressiveConsistencyChecks {
		if _, err := mq.store.consistencyQueue.process(ctx, lhsRepl, confReader, priority); err != nil {
			log.Dev.Warningf(ctx, "%v", err)
		}
	}
