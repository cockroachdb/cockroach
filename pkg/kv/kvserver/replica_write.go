	...
	for {
		...
		case propResult := <-ch:
			...
			if ba.Requests[0].GetMigrate() != nil && propResult.Err == nil {
				...
				// follower that's not in `desc` (but will be in it soon)? This
				// seems to be in the realm of what's possible in theory: we'll
				// only ever proactively send a snap when the peer is a learner,
				// but an old snapshot that precedes maxLeaseIndex may be in
				// flight for no good reason and once that peer is added to desc
				// (after this code has done its work) and receives said zombie
				// snap, it'll be initialized without having caught up past the
				// migration. What's the safeguard needed here? I'm not sure. If
				// we force a log truncation right after this proposal, would
				// that force all to-be replicas to first catch up? Does that
				// provide the invariants we want? Can we do something using the
				// migrations infrastructure to prevent all historical snapshots
