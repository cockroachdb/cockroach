		...
		testTxnExecute := func(t *testing.T, ctx context.Context, spec testTxnSpec, txn *kv.Txn, txnKey roachpb.Key) error {
			...
			case "push":
				...
				go func() {
					...
					pusherProto := roachpb.MakeTransaction("pusher", nil /* baseKey */, isolation.Serializable, roachpb.MaxUserPriority, now.ToTimestamp(), srv.Clock().MaxOffset().Nanoseconds(), int32(srv.SQLInstanceID()), 0, false)
