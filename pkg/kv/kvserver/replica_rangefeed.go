	...
	for _, op := range ops.Ops {
		...
		prevValRes, err := storage.MVCCGet(
			ctx, prevReader, key, ts, storage.MVCCGetOptions{
				Tombstones: true, Inconsistent: true, ReadCategory: storage.RangefeedReadCategory},
		)
		if err != nil {
...
func (r *Replica) handleLogicalOpLogRaftMuLocked(
	...
	for _, op := range ops.Ops {
		...
		*valPtr = val.RawBytes
		...
		// If OmitInRangefeeds is set and this op is a write, propagate
		// the OmitInRangefeeds flag from the MVCCValueHeader to the
		// MVCCCommitIntentOp. It will be used later, in the registry's
		// PublishToOverlapping, to potentially filter this event out of the
		// rangefeed of any registrations that opted into filtering.
