...
func BenchmarkIntentResolution(b *testing.B) {
	...
	pendingWriter := func(t testing.TB, txn *kv.Txn, makeKey func(i int) roachpb.Key) {
		batchSize := batchSize
	...
	rolledBackWriter := func(t testing.TB, txn *kv.Txn, makeKey func(i int) roachpb.Key) {
		pendingWriter(t, txn, makeKey)
		require.NoError(t, txn.Rollback(ctx))
	}
	abortedWriter := func(t testing.TB, txn *kv.Txn, makeKey func(i int) roachpb.Key) {
		pendingWriter(t, txn, makeKey)
	...
	highPriorityScan := func(t testing.TB, ctx context.Context, kvDB *kv.DB, span roachpb.Span) {
		...
		// txnCache and thus this test would end up pushing the conflicting 10k
		// times rather than 1 times.
		//
		// What isn't clear to me is, what is the common case here? It seems like
		// _most_ reads will have an uncertainty interval.
	...
	type testCase struct {
		name   string
		writer func(testing.TB, *kv.Txn, func(i int) roachpb.Key)
	...
	testCases := []testCase{
		...
		// These test cases should be more or less the same as the writer=ABORTED
		// cases, but here we abort the transaction using a PushTxn(PUSH_ABORT)
		// rather than a EndTxn(commit=false).
		{
	...
	for _, tc := range testCases {
		b.Run(tc.name, func(b *testing.B) {
			...
			for b.Loop() {
				...
				pendingWriter(b, txn1, makeKey)

				collectAndFinish := func() tracingpb.Recording { return tracingpb.Recording{} }
				if doDebug {
					tracer := s.TracerI().(*tracing.Tracer)
					ctx, collectAndFinish = tracing.ContextWithRecordingSpan(ctx, tracer, "test")
					defer collectAndFinish()
				}

				b.StartTimer()
				highPriorityScan(b, ctx, kvDB, testRange)
				b.StopTimer()
				if doDebug {
					b.Logf("TRACE: %s", collectAndFinish())
				}
