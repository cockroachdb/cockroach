...
type mockNodeLiveness struct {
	syncutil.Mutex
	defaultNodeStatus nodelivenesspb.NodeLivenessStatus
	nodes             map[roachpb.NodeID]nodelivenesspb.NodeLivenessStatus
}

func newMockNodeLiveness(defaultNodeStatus nodelivenesspb.NodeLivenessStatus) *mockNodeLiveness {
...
func (m *mockNodeLiveness) setNodeStatus(
	nodeID roachpb.NodeID, status nodelivenesspb.NodeLivenessStatus,
) {
	m.Lock()
	defer m.Unlock()
...
func TestStorePoolGossipUpdate(t *testing.T) {
	...
	stopper, g, _, sp, _ := createTestStorePool(
		...
		nodelivenesspb.NodeLivenessStatus_DEAD)
...
func TestStorePoolGetStoreList(t *testing.T) {
	...
	for i := 1; i <= 7; i++ {
		mnl.setNodeStatus(roachpb.NodeID(i), nodelivenesspb.NodeLivenessStatus_LIVE)
	}

	// Set deadStore as dead.
	mnl.setNodeStatus(deadStore.Node.NodeID, nodelivenesspb.NodeLivenessStatus_DEAD)
	sp.detailsMu.Lock()
...
func TestStorePoolFindDeadReplicas(t *testing.T) {
	...
	mnl.setNodeStatus(4, nodelivenesspb.NodeLivenessStatus_DEAD)
	mnl.setNodeStatus(5, nodelivenesspb.NodeLivenessStatus_DEAD)
	...
	mnl.setNodeStatus(4, nodelivenesspb.NodeLivenessStatus_UNAVAILABLE)
...
func TestStorePoolDecommissioningReplicas(t *testing.T) {
	...
	mnl.setNodeStatus(4, nodelivenesspb.NodeLivenessStatus_DECOMMISSIONING)
	// Mark node 5 as dead.
	mnl.setNodeStatus(5, nodelivenesspb.NodeLivenessStatus_DEAD)
...
func TestNodeLivenessLivenessStatus(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		expected nodelivenesspb.NodeLivenessStatus
	}{
		// Valid status.
		{
			liveness: nodelivenesspb.Liveness{
			...
			expected: nodelivenesspb.NodeLivenessStatus_LIVE,
		},
		{
			liveness: nodelivenesspb.Liveness{
				NodeID: 1,
		...
		// Expired status.
		{
			...
			expected: nodelivenesspb.NodeLivenessStatus_UNAVAILABLE,
		},
		// Expired status.
		{
			liveness: nodelivenesspb.Liveness{
		...
		// Dead status.
		{
			...
			expected: nodelivenesspb.NodeLivenessStatus_DEAD,
		},
		// Decommissioning.
		{
			liveness: nodelivenesspb.Liveness{
				...
				Membership: nodelivenesspb.MembershipStatus_DECOMMISSIONING,
				Draining:   false,
			},
			expected: nodelivenesspb.NodeLivenessStatus_DECOMMISSIONING,
		},
		// Decommissioning + expired.
		{
			...
			expected: nodelivenesspb.NodeLivenessStatus_DECOMMISSIONED,
		},
		// Decommissioned + live.
		{
			liveness: nodelivenesspb.Liveness{
				...
				Membership: nodelivenesspb.MembershipStatus_DECOMMISSIONED,
				Draining:   false,
			},
