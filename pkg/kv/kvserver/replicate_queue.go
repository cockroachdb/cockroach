...
func (rq *replicateQueue) addOrReplaceVoters(
	...
	if found {
		...
		ops = []roachpb.ReplicationChange{{ChangeType: roachpb.ADD_VOTER, Target: newReplica}}
	}
	...
	if removeIdx < 0 {
		...
		// not perform a demotion here since we know that the `removeReplica` is
		// either dead or decommissioning. This means that after this allocation is
		// executed, we could be one non-voter short. This will be handled by the
		// replicateQueue's next attempt at this range.
		ops = append(ops,
...
func (rq *replicateQueue) considerRebalance(
	...
	if !rq.store.TestingKnobs().DisableReplicaRebalancing {
		...
		if !ok {
			...
			log.VEventf(ctx, 1, "rebalancing %s %+v to %+v: %s",
				rebalanceTargetType, addTarget, rangeRaftProgress(repl.RaftStatus(), existingVoters))
...
func (rq *replicateQueue) replicationChangesForRebalance(
	...
	case voterTarget:
		...
		if found && rdesc.GetType() == roachpb.NON_VOTER {
			...
			// the `voter_constraints`, it is "fine" to make this swap since:
			//
			...
			log.Errorf(ctx, "programming error: store being rebalanced to(%s) already has a"+
				" voting replica", addTarget.StoreID)
		} else {
