...
func (rq *replicateQueue) processOneChange(
	...
	case AllocatorReplaceDeadVoter:
		...
		removeIdx := getRemoveIdx(voterReplicas, deadVoterReplicas)
		if removeIdx < 0 {
	...
	case AllocatorReplaceDeadNonVoter:
		...
		removeIdx := getRemoveIdx(nonVoterReplicas, deadNonVoterReplicas)
		if removeIdx < 0 {
			return false, errors.AssertionFailedf(
	...
	case AllocatorReplaceDecommissioningVoter:
		...
		removeIdx := getRemoveIdx(voterReplicas, decommissioningVoterReplicas)
		if removeIdx < 0 {
			return false, errors.AssertionFailedf(
	...
	case AllocatorReplaceDecommissioningNonVoter:
		...
		removeIdx := getRemoveIdx(nonVoterReplicas, decommissioningNonVoterReplicas)
		if removeIdx < 0 {
			return false, errors.AssertionFailedf(
...
func getRemoveIdx(repls, deadOrDecommissioningRepls []roachpb.ReplicaDescriptor) (removeIdx int) {
	...
	for i, rDesc := range repls {
		if rDesc.StoreID == deadOrDecommissioningRepls[0].StoreID {
			removeIdx = i
			break
		}
	}
...
func (rq *replicateQueue) removeDecommissioning(
	...
	switch targetType {
	case voterTarget:
		decommissioningReplicas = rq.allocator.storePool.decommissioningReplicas(desc.Replicas().VoterDescriptors())
		removeOp = roachpb.REMOVE_VOTER
	case nonVoterTarget:
		decommissioningReplicas = rq.allocator.storePool.decommissioningReplicas(desc.Replicas().NonVoterDescriptors())
