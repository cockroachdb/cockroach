...
func (rq *replicateQueue) considerRebalance(
	...
	// We require the lease in order to process replicas, so repl.store.StoreID()
	// corresponds to the lease-holder's store ID.
	transferred, err := rq.shedLease(
		ctx,
		repl,
		desc,
	...
	case transferErr:
		return false, err
	case noTransferDryRun, noSuitableTarget:
		return false, nil
	default:
		panic("unreachable")
	}
