...
func (rq *replicateQueue) shouldQueue(
	...
	hasExpirationLease := leaseStatus.Lease.Type() == roachpb.LeaseExpiration
	if !leaseStatus.IsValid() && !hasExpirationLease {
	...
	if hasExpirationLease != repl.shouldUseExpirationLeaseRLocked() {
		// The lease type is incorrect, switch it to the correct type. Typically
		// when changing kv.expiration_leases_only.enabled.
...
func (rq *replicateQueue) PlanOneChange(
	...
	// NB: switches both to and from expiration leases.
	hasExpirationLease := leaseStatus.Lease.Type() == roachpb.LeaseExpiration
	if hasExpirationLease != repl.shouldUseExpirationLeaseRLocked() {
		_ = repl.requestLeaseLocked(ctx, leaseStatus)
	}
