...
func (rq *replicateQueue) shouldQueue(
	...
	if action == allocatorimpl.AllocatorNoop {
		log.Allocator.VInfof(ctx, 2, "no action to take")
		return false, 0
	} else if action != allocatorimpl.AllocatorConsiderRebalance {
		log.Allocator.VInfof(ctx, 2, "repair needed (%s), enqueuing", action)
	...
	if !rq.store.TestingKnobs().DisableReplicaRebalancing {
		...
		if ok {
			log.Allocator.VInfof(ctx, 2, "rebalance target found for voter, enqueuing")
			return true, 0
		}
		...
		if ok {
			log.Allocator.VInfof(ctx, 2, "rebalance target found for non-voter, enqueuing")
			return true, 0
		}
		log.Allocator.VInfof(ctx, 2, "no rebalance target found, not enqueuing")
	}
	...
	if status.IsValid() &&
		...
		log.Allocator.VInfof(ctx, 2, "lease transfer needed, enqueuing")
		return true, 0
	}
	if !status.IsValid() {
		...
		// lease.
		log.Allocator.VInfof(ctx, 2, "invalid lease, enqueuing")
		return true, 0
	}
	...
	for r := retry.StartWithCtx(ctx, retryOpts); r.Next(); {
		...
		if testingAggressiveConsistencyChecks {
			if _, err := rq.store.consistencyQueue.process(ctx, repl, confReader); err != nil {
				log.Allocator.Warningf(ctx, "%v", err)
			}
		}
		...
		if requeue {
			log.Allocator.VInfof(ctx, 1, "re-processing")
...
func (rq *replicateQueue) processOneChange(
	...
	log.Allocator.VInfof(ctx, 1, "next replica action: %s", action)
...
func (rq *replicateQueue) addOrReplaceVoters(
	...
	if removeIdx < 0 {
		log.Allocator.VInfof(ctx, 1, "adding voter %+v: %s",
		...
		log.Allocator.VInfof(ctx, 1, "replacing voter %s with %+v: %s",
...
func (rq *replicateQueue) addOrReplaceNonVoters(
	...
	if removeIdx < 0 {
		log.Allocator.VInfof(ctx, 1, "adding non-voter %+v: %s",
		...
		log.Allocator.VInfof(ctx, 1, "replacing non-voter %s with %+v: %s",
...
func (rq *replicateQueue) findRemoveVoter(
	...
	for r := retry.StartWithCtx(ctx, retryOpts); r.Next() && timeutil.Now().Before(deadline); {
		...
		log.Allocator.VInfof(ctx, 3, "filtered unremovable replicas from %v to get %v as candidates for removal: %s",
...
func (rq *replicateQueue) removeVoter(
	...
	log.Allocator.VInfof(ctx, 1, "removing voting replica %+v due to over-replication: %s",
...
func (rq *replicateQueue) removeNonVoter(
	...
	log.Allocator.VInfof(ctx, 1, "removing non-voting replica %+v due to over-replication: %s",
...
func (rq *replicateQueue) removeDecommissioning(
	...
	if len(decommissioningReplicas) == 0 {
		log.Allocator.VInfof(ctx, 1, "range of %[1]ss %[2]s was identified as having decommissioning %[1]ss, "+
	...
	log.Allocator.VInfof(ctx, 1, "removing decommissioning %s %+v from store", targetType, decommissioningReplica)
	target := roachpb.ReplicationTarget{
...
func (rq *replicateQueue) removeDead(
	...
	log.Allocator.VInfof(ctx, 1, "removing dead %s %+v from store", targetType, deadReplica)
	target := roachpb.ReplicationTarget{
...
func (rq *replicateQueue) considerRebalance(
	...
	if !rq.store.TestingKnobs().DisableReplicaRebalancing {
		...
		if !ok {
			...
			log.Allocator.VInfof(ctx, 1, "no suitable rebalance target for voters")
		...
		if !ok {
			log.Allocator.VInfof(ctx, 1, "no suitable rebalance target for non-voters")
		} else if !lhRemovalAllowed {
			if done, err := rq.maybeTransferLeaseAway(
				ctx, repl, removeTarget.StoreID, dryRun, canTransferLeaseFrom,
			); err != nil {
				log.Allocator.VInfof(ctx, 1, "want to remove self, but failed to transfer lease away: %s", err)
				ok = false
			} else if done {
				// Lease is now elsewhere, so we're not in charge any more.
				return false, nil
...
func replicationChangesForRebalance(
	...
	if rebalanceTargetType == allocatorimpl.VoterTarget && numExistingVoters == 1 {
		...
		log.Allocator.VInfof(ctx, 1, "can't swap replica due to lease; falling back to add")
...
func (rq *replicateQueue) shedLease(
	...
	if opts.DryRun {
		log.Allocator.VInfof(ctx, 1, "transferring lease to s%d", target.StoreID)
...
func (rq *replicateQueue) TransferLease(
	...
	log.Allocator.VInfof(ctx, 1, "transferring lease to s%d", target)
