...
func (b *replicaAppBatch) stageTrivialReplicatedEvalResult(
	...
	if cts := cmd.raftCmd.ClosedTimestamp; cts != nil && !cts.IsEmpty() {
		...
		b.closedTimestampSetter.cmd = cmd
...
func (b *replicaAppBatch) assertNoCmdClosedTimestampRegression(cmd *replicatedCmd) error {
	...
	if newClosed != nil && !newClosed.IsEmpty() && newClosed.Less(existingClosed) {
		...
		var prevReq string
		var prevLeaseIdx ctpb.LAI
		prevCmd := b.closedTimestampSetter.cmd
		if prevCmd != nil {
			prevLeaseIdx = ctpb.LAI(prevCmd.leaseIndex)
			if prevCmd.IsLocal() && prevCmd.proposal.Request.IsIntentWrite() {
				prevReq = prevCmd.proposal.Request.String()
			} else {
		...
		return errors.AssertionFailedf(
			...
			prevReq, b.closedTimestampSetter.lease, prevLeaseIdx, b.entries)
	}
	return nil
}
...
func (b *replicaAppBatch) assertNoWriteBelowClosedTimestamp(cmd *replicatedCmd) error {
	if !b.state.MigratedToRaftClosedTimestamp() || !raftClosedTimestampAssertionsEnabled() {
		return nil
	}
	if !cmd.raftCmd.ReplicatedEvalResult.IsIntentWrite() {
		return nil
	}
