...
func (b *replicaAppBatch) Stage(
	...
	if cmd.Index() == 0 {
		return nil, kvserverbase.NewNonDeterministicFailureErrorf("processRaftCommand requires a non-zero index")
	}
	if idx, applied := cmd.Index(), b.state.RaftAppliedIndex; idx != applied+1 {
		// If we have an out of order index, there's corruption. No sense in
		// trying to update anything or running the command. Simply return.
		return nil, kvserverbase.NewNonDeterministicFailureErrorf("applied index jumped from %d to %d", applied, idx)
	}
	if log.V(4) {
	...
	if splitMergeUnlock, err := b.r.maybeAcquireSplitMergeLock(ctx, cmd.Cmd); err != nil {
		if cmd.Cmd.ReplicatedEvalResult.Split != nil {
			err = kvserverbase.NonDeterministicFailureErrorWrapf(err, "unable to acquire split lock")
		} else {
			err = kvserverbase.NonDeterministicFailureErrorWrapf(err, "unable to acquire merge lock")
		}
		return nil, err
...
func (b *replicaAppBatch) stageWriteBatch(ctx context.Context, cmd *replicatedCmd) error {
	...
	if err := b.batch.ApplyBatchRepr(wb.Data, false); err != nil {
		return kvserverbase.NonDeterministicFailureErrorWrapf(err, "unable to apply WriteBatch")
	}
	return nil
}
...
func (b *replicaAppBatch) runPreApplyTriggersAfterStagingWriteBatch(
	...
	if merge := res.Merge; merge != nil {
		...
		if err != nil {
			return kvserverbase.NonDeterministicFailureErrorWrapf(err, "unable to get replica for merge")
		}
		...
		if err := rhsRepl.preDestroyRaftMuLocked(
			...
			return kvserverbase.NonDeterministicFailureErrorWrapf(err, "unable to destroy replica before merge")
		}
	...
	if res.State != nil && res.State.TruncatedState != nil {
		...
		if apply {
			if apply, err = handleTruncatedStateBelowRaftPreApply(
				...
				return kvserverbase.NonDeterministicFailureErrorWrapf(err, "unable to handle truncated state")
			}
		} else {
	...
	if change := res.ChangeReplicas; change != nil &&
		...
		if err := b.r.preDestroyRaftMuLocked(
			...
			return kvserverbase.NonDeterministicFailureErrorWrapf(err, "unable to destroy replica before removal")
		}
	}
...
func (b *replicaAppBatch) ApplyToStateMachine(ctx context.Context) error {
	...
	if err := b.batch.Commit(sync); err != nil {
		return kvserverbase.NonDeterministicFailureErrorWrapf(err, "unable to commit Raft entry batch")
	}
	b.batch.Close()
	b.batch = nil
...
func (b *replicaAppBatch) assertNoWriteBelowClosedTimestamp(cmd *replicatedCmd) error {
	...
	if !wts.IsEmpty() && wts.LessEq(b.state.RaftClosedTimestamp) {
		...
		return kvserverbase.NonDeterministicFailureErrorWrapf(errors.AssertionFailedf(
...
func (sm *replicaStateMachine) ApplySideEffects(
	...
	if err := sm.maybeApplyConfChange(ctx, cmd); err != nil {
		return nil, kvserverbase.NonDeterministicFailureErrorWrapf(err, "unable to apply conf change")
	}
