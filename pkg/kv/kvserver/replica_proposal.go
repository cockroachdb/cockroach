...
func (r *Replica) computeChecksumPostApply(ctx context.Context, cc kvserverpb.ComputeChecksum) {
	...
	limiter := quotapool.NewRateLimiter(
		"ConsistencyQueue",
		quotapool.Limit(consistencyCheckRate.Get(&r.store.ClusterSettings().SV)),
		consistencyCheckRate.Get(&r.store.ClusterSettings().SV)*consistencyCheckRateBurstFactor,
