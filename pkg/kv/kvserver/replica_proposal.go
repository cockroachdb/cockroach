...
func (r *Replica) leasePostApplyLocked(
	...
	if leaseChangingHands && iAmTheLeaseHolder && hasExpirationLease && r.ownsValidLeaseRLocked(ctx, now) {
		if r.requiresExpiringLeaseRLocked() {
			...
			// requires it (i.e. a critical system range like the liveness or meta
			// ranges), notify the lease renewer worker that we want it to keep
			// proactively renewing the lease before it expires. We don't eagerly
			// renew other expiration leases, only the ones for critical system
			// ranges, because a more sophisticated scheduler is needed to handle
			// large numbers of expiration leases.
