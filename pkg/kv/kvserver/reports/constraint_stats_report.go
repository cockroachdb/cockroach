...
// of voters are not specified e.g. equal to number of replicas implicitly.
var replicaConstraintsAllVoters = constraintCheckPolicy{
	predicates: map[zonepb.Constraint_Type][]replicaPredicate{
		zonepb.Constraint_REQUIRED: {
			isIncomingAndNonVoter, isOutgoingAndNonVoter,
		},
...
// number of voters are explicitly specified.
...
func (v *constraintConformanceVisitor) visitSameZone(
	...
	if v.prevNumVoters != 0 {
		v.countRange(ctx, r, v.prevZoneKey, Constraint, replicaConstraintsWithNonVoters,
			v.prevConstraints)
	} else {
		v.countRange(ctx, r, v.prevZoneKey, Constraint, replicaConstraintsAllVoters, v.prevConstraints)
	}
	v.countRange(ctx, r, v.prevZoneKey, VoterConstraint, voterConstraints, v.prevVoterConstraints)
}
...
func isIncomingAndNonVoter(r roachpb.ReplicaDescriptor) bool {
	return isIncoming(r) || isNonVoter(r)
}

func isOutgoingAndNonVoter(r roachpb.ReplicaDescriptor) bool {
	return isOutgoing(r) || isNonVoter(r)
}
...
func isVoter(r roachpb.ReplicaDescriptor) bool {
	return isOutgoing(r) || isIncoming(r)
}
...
func isOutgoing(r roachpb.ReplicaDescriptor) bool {
	switch r.Type {
