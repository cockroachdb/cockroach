...
type rangeConstraints struct {
	...
	// If any of subsets of replicas generated by predicates are not satisfied
	// we are in violation.
	exclusionPredicates []replicaPredicate
}
				...
				constraints = []rangeConstraints{
					{
						...
						exclusionPredicates: []replicaPredicate{allReplicas},
					},
				}
			} else {
				constraints = []rangeConstraints{
					...
					{
						cType:               VoterConstraint,
						conjunctions:        zone.VoterConstraints,
						inclusionPredicates: []replicaPredicate{incomingConsensus, outgoingConsensus},
						exclusionPredicates: []replicaPredicate{voterPredicate},
					},
				}
			}
			zKey = key
...
func incomingConsensus(r roachpb.ReplicaDescriptor) bool {
	switch r.GetType() {
...
func outgoingConsensus(r roachpb.ReplicaDescriptor) bool {
	switch r.GetType() {
...
func voterPredicate(r roachpb.ReplicaDescriptor) bool {
	return outgoingConsensus(r) || incomingConsensus(r)
}

func allReplicas(_ roachpb.ReplicaDescriptor) bool {
	return true
}
...
func (v *constraintConformanceVisitor) countRange(
	...
	storesFromPredicates := func(ps []replicaPredicate) [][]roachpb.StoreDescriptor {
		...
		for i, p := range ps {
			...
			storeVariants[i] = make([]roachpb.StoreDescriptor, len(rds))
			for j, r := range rds {
				storeVariants[i][j] = v.storeResolver(r.StoreID)
			}
		}
	...
	for _, c := range constraints {
		inclusionStores := storesFromPredicates(c.inclusionPredicates)
		exclusionStores := storesFromPredicates(c.exclusionPredicates)
		violated := getViolations(ctx, inclusionStores, exclusionStores, c.conjunctions)
...
func getViolations(
	...
	prohibitedStoreDescs [][]roachpb.StoreDescriptor,
	...
	for _, conjunction := range constraintConjunctions {
		requiredReplicasToMatch := int(conjunction.NumReplicas)
		for _, c := range conjunction.Constraints {
			storesToCheck := requiredStoreDescs
			if c.Type == zonepb.Constraint_PROHIBITED {
				storesToCheck = prohibitedStoreDescs
			}
			...
			// raft consensus. Validating them together will cause us to have too many
			// voter replicas since those are disjoint sets.
			for _, stores := range storesToCheck {
				if requiredReplicasToMatch == 0 {
					requiredReplicasToMatch = len(stores)
				}
				if !constraintSatisfied(c, requiredReplicasToMatch, stores) {
