...
// TestRangeFeedWithExcise tests that a RangeFeed can be created just before
// excising a range and that the RangeFeed will stop when an excise command is issued.
//
// Test setup can be visualized in the following diagram:
/*
             a                d                g                z
...
func TestRangeFeedWithExcise(t *testing.T) {
	...
	waitForCheckpoint := func(rfErr chan error, stream *testStream) {
		require.Eventually(t, func() bool {
			if len(rfErr) > 0 {
				require.Fail(t, "unexpected rangefeed error", "%v", <-rfErr)
			}
	...
	require.Equal(t, 0, len(rfErr2))
	require.Equal(t, 0, len(rfErr3))
	...
	waitForExciseError := func(rfErr chan error) {
		testutils.SucceedsSoon(t, func() error {
			...
			require.Equal(t, 1, len(rfErr))
			require.Regexp(t, "Replica applied ExciseRequest", <-rfErr)
			return nil
		})
	}
