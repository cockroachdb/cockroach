...
var rangefeedRestartAvailableFraction = settings.RegisterFloatSetting(
	...
	"minimum available fraction of rangefeed catchup quota that allows processor "+
...
func (rr *rangefeedRestarter) startJob(ctx context.Context, toScheduler bool, stopper *stop.Stopper) {
...
func (rr *rangefeedRestarter) run(
	...
	log.VInfof(ctx, 2, "attempting to update processors to new type of %t", needScheduled)
	...
	refreshConfig := func() error {
		...
		minAvailableCatchups := float64(concurrentCatchupLimit) * minAvailableQuotaFraction
		if minAvailableCatchups > float64(concurrentCatchupLimit) {
			return errors.New("processor restarts disabled")
		}
		minCatchups = uint64(math.Round(minAvailableCatchups))
		return nil
	}
	...
	waitRestartQuota := func() error {
		...
		for {
			...
			case <-wait.Ch():
				wait.MarkRead()
				if avail := rr.catchupLimiter.Available(); avail >= minCatchups {
					return nil
				}
			case <-rr.cfgChangedC:
				if err := refreshConfig(); err != nil {
	...
	ids := rr.findRangesWithProcType(!needScheduled)
