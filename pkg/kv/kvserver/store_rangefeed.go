...
// - minimum quota - delay restarts if too many other rangefeeds are performing
//   catchup scans
...
func (rr *rangefeedRestarter) run(
	...
	refreshConfig := func() error {
		...
		minAvailableCatchups = uint64(math.Round(
			float64(concurrentCatchupLimit) * minAvailableQuotaFraction))
		return nil
	}
