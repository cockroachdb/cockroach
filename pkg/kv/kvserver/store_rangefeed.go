...
// within a quant of time, and a suggested deadline for completing this work. It
// assumes that work can be done at constant speed and uniformly fill the
// remaining time between now and the target.
//
...
func rangeFeedUpdaterPace(
	now, target time.Time, quant time.Duration, workLeft int,
) (todo int, by time.Time) {
	timeLeft := target.Sub(now)
	if workLeft <= 0 || timeLeft <= 0 { // ran out of work or time
		return workLeft, now
	} else if timeLeft <= quant { // time is running out
		return workLeft, target
	}
	// Otherwise, we have workLeft >= 1, and at least a full quant of time. Assume
	// we can complete work at uniform speed.
	todo = int(float64(workLeft) * quant.Seconds() / timeLeft.Seconds())
	by = now.Add(quant)
