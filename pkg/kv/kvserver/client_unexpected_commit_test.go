...
type interceptingTransport struct {
	kvcoord.Transport
	intercept func(context.Context, roachpb.BatchRequest) (*roachpb.BatchResponse, error)
}
	...
	getInterceptingTransportFactory := func(nID roachpb.NodeID) kvcoord.TransportFactory {
		return func(options kvcoord.SendOptions, dialer *nodedialer.Dialer, slice kvcoord.ReplicaSlice) (kvcoord.Transport, error) {
			...
			interceptor := &interceptingTransport{
				Transport: transport,
				intercept: func(ctx context.Context, ba roachpb.BatchRequest) (*roachpb.BatchResponse, error) {
					fromNodeID := nID
					...
					if txnName == "txn1" || txnName == "txn2" {
						if jamChan != nil {
							jammed = true
							fmt.Fprintf(&tags, "[jammed] ")
						} else if toNodeID == blockedNodeID {
							blocked = true
							fmt.Fprintf(&tags, "[blocked] ")
						...
						t.Logf("%sbatchReq={%s}, meta={key=%s ts=%s seq=%d}", tags.String(), ba,
