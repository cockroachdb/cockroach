...
func TestTransactionUnexpectedlyCommitted(t *testing.T) {
	...
	txn1MetaCh := make(chan *enginepb.TxnMeta, 1)
			...
			interceptor := &interceptingTransport{
				...
				intercept: func(ctx context.Context, ba *kvpb.BatchRequest) (*kvpb.BatchResponse, error) {
					...
					if ((txnName == "txn1" || txnName == "txn2") && !ba.IsSingleHeartbeatTxnRequest()) ||
						...
						(ba.IsSinglePushTxnRequest() && ba.Requests[0].GetPushTxn().PusherTxn.Name == "txn2") {
						opID++
						if jamChan != nil {
							jammed = true
					...
					if txnName == "txn1" && jammed {
						// Start intent query loop to wait on txn1's writes.
						txn1MetaCh <- &ba.Txn.TxnMeta
					}
	...
	go func() {
		defer wg.Done()
		// Wait until txn1's TxnMeta is available.
		var txnMeta *enginepb.TxnMeta
		select {
		case txnMeta = <-txn1MetaCh:
		case <-tc.Stopper().ShouldQuiesce():
			t.Logf("txn1 querier quiescing...")
		}

		if txnMeta == nil {
			t.Errorf("cannot check txn1's writes, txnMeta is nil")
			return
		}

		dbg("starting txn1 record/intent query loop: %v", txnMeta)
		startTime := time.Now()
		tCtx := context.Background()
		if err := testutils.SucceedsSoonError(func() error {
			meta := *txnMeta
			meta.Sequence = enginepb.TxnSeq(0)
			var b kv.Batch
			b.Header.Timestamp = txnMeta.WriteTimestamp
			...
			if err := db.Run(tCtx, &b); err != nil {
				return err
			}

			queryTxnResp := b.RawResponse().Responses[0].GetInner().(*kvpb.QueryTxnResponse)
			if !queryTxnResp.TxnRecordExists {
				return errors.New("no txn1 record found")
			}

			queryIntentResp := b.RawResponse().Responses[1].GetInner().(*kvpb.QueryIntentResponse)
			if !queryIntentResp.FoundIntent {
				return errors.Newf("no txn1 intent found on %s", key2)
			}

			dbg("found txn1 record & intent in %s, ifwrites: %v", time.Since(startTime),
				queryTxnResp.QueriedTxn.InFlightWrites)
			return nil
		}); err != nil {
			t.Errorf("failed to get a valid record/intent for txn1")
			return
		}

		// Allow txn2 to start once txn1's write on key2 is in-flight but durable.
		close(txn2Ready)
	}()
