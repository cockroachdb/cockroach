...
func (r *Replica) loadUninit(ctx context.Context, desc *roachpb.RangeDescriptor) error {
	// FIXME: Leaving any of the following fields nil leads to panics in some
	// tests. The safest would be to initialize ReplicaState to a reasonable
	// default (same as stateLoader.Load on an empty storage would return). In
	// contrast, revisiting nil checking everywhere seems too much work.
	...
	// FIXME: Only here to satisfy the assert below.
	r.mu.state.GCHint = &roachpb.GCHint{}
	// FIXME: r.mu.state.Version?
