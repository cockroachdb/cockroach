...
func (r *Replica) initRaftMuLockedReplicaMuLocked(
	...
	if r.shMu.state.Lease.Sequence > 0 {
		if waitForPrevLeaseToExpire {
			...
			// This is important so that we don't serve a write request with the new
...
// so by sleeping until Clock().Now() is past the previous lease expiration.
// This works for expiration-based leases, and leader-leases but only
// best-effort for epoch-based leases since the liveness record might not be
// found in cache after the restart.
