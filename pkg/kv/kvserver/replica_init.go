...
func newInitializedReplica(store *Store, loaded kvstorage.LoadedReplicaState) (*Replica, error) {
	r := newReplica(store, loaded.ReplState.Desc.RangeID, loaded.ReplicaID)
	r.raftMu.Lock()
...
func newUninitializedReplica(
	...
	r := newReplica(store, rangeID, replicaID)
	r.raftMu.Lock()
	defer r.raftMu.Unlock()
	r.mu.Lock()
	defer r.mu.Unlock()
...
// newReplica creates a new, bare replica without a Raft group. Use either
// newInitializedReplica() or newUninitializedReplica() instead, which will
// include an appropriate Raft group.
func newReplica(store *Store, rangeID roachpb.RangeID, replicaID roachpb.ReplicaID) *Replica {
