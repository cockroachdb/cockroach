...
func (m *latchManagerImpl) Acquire(ctx context.Context, req Request) (latchGuard, *Error) {
	lg, err := m.m.Acquire(ctx, req.LatchSpans, req.PoisonPolicy, req.BaFormatter)
	if err != nil {
...
func (m *latchManagerImpl) AcquireOptimistic(req Request) latchGuard {
	lg := m.m.AcquireOptimistic(req.LatchSpans, req.PoisonPolicy, req.BaFormatter)
	return lg
}
...
func (m *latchManagerImpl) WaitFor(
	ctx context.Context, ss *spanset.SpanSet, pp poison.Policy, baFormatter redact.SafeFormatter,
) *Error {
	err := m.m.WaitFor(ctx, ss, pp, baFormatter)
	if err != nil {
