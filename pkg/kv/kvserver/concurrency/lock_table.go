...
//
// TODO(XXX): Let's rename this to keyLocks (or locksState?). I can tack on a
// commit once we're happy with the name.
type lockState struct {
	...
	// wait if the lock is held. The converse, however, is not true.

	// holders is a list of *txnLocks. Lock information on a particular key is
	// tracked per-transaction; every element in this list represents a lock on
	// acquired by different transactions.
	//
	// This list may be empty, in which case the current key is unlocked.
	holders list.List
	// heldByMap is used to lookup txnLocks in the holders list using transaction
...
type txnLocks struct {
	...
	// with an older epoch).
	//
	// TODO(XXX): Now that we expect this to always be set, should this no longer
	// be a pointer to the txnMeta?
...
// newTxnLocks constructs and returns a new txnLocks.
func newTxnLocks(txn *enginepb.TxnMeta, clock *hlc.Clock) *txnLocks {
	tl := &txnLocks{}
...
func (tl *txnLocks) isHeldReplicated() bool {
	return !tl.replicatedInfo.ts.IsEmpty()
}

// Returns information about the lock's holder.
//
// REQUIRES: l.mu is locked.
func (tl *txnLocks) getLockHolder() (*enginepb.TxnMeta, hlc.Timestamp) {
	assert(
	...
	if tl.isHeldReplicated() && tl.isHeldUnreplicated() {
		minTS := tl.unreplicatedInfo.ts
		minTS.Backward(tl.replicatedInfo.ts)
		return tl.txn, minTS
	}

	// Else, we return whichever one the lock is held at.
	if tl.isHeldUnreplicated() {
		return tl.txn, tl.unreplicatedInfo.ts
	}
	return tl.txn, tl.replicatedInfo.ts
}
...
//
// REQUIRES: l.mu is locked.
func (tl *txnLocks) isHeldUnreplicated() bool {
...
//
// REQUIRES: l.mu is locked.
func (tl *txnLocks) getLockMode() lock.Mode {
...
func (tl *txnLocks) isIdempotentLockAcquisition(acq *roachpb.LockAcquisition) bool {
...
//
// REQUIRES: l.mu to be locked.
func (tl *txnLocks) reacquireLock(acq *roachpb.LockAcquisition) error {
...
func (l *lockState) safeFormat(sb *redact.StringBuilder, txnStatusCache *txnStatusCache) {
	...
	if l.isLocked() {
		...
		// subsequent patch.
		tl := l.holders.Front().Value.(*txnLocks)
...
func (l *lockState) informActiveWaiters() {
	...
	if l.distinguishedWaiter == nil ||
		...
		assert(
			l.distinguishedWaiter == nil || !l.isLocked(), fmt.Sprintf(
...
func (l *lockState) claimantTxn() (_ *enginepb.TxnMeta, held bool) {
	if l.isLocked() {
		return l.holders.Front().Value.(*txnLocks).txn, true
	}
	if l.queuedWriters.Len() == 0 {
...
// held with durability replicated. If so, the transaction that holds the
...
func (l *lockState) lockHeldDuration(now time.Time) time.Duration {
	...
	for e := l.holders.Front(); e != nil; e = e.Next() {
		tl := e.Value.(*txnLocks)
		if minStartTS.Equal(time.Time{}) || tl.startTime.Before(minStartTS) {
...
//
// REQUIRES: l.mu to be locked.
func (l *lockState) lockAcquiredOrDiscovered(tl *txnLocks) {
	_, found := l.heldBy[tl.txn.ID]
	assert(!found, "lock was already being tracked for this key")
	l.holders.PushBack(tl)
	l.heldBy[tl.txn.ID] = l.holders.Back()
}
...
func (l *lockState) conflictsWithLockHolders(g *lockTableGuardImpl) bool {
	...
	for e := l.holders.Front(); e != nil; e = e.Next() {
		...
		assert(
			!g.isSameTxn(lockHolderTxn) || g.curStrength() > l.holders.Front().Value.(*txnLocks).getLockMode().Strength,
...
func (l *lockState) acquireLock(acq *roachpb.LockAcquisition, clock *hlc.Clock) error {
	...
	if l.isLockedBy(acq.Txn.ID) {
		...
		assert(found, "multiple lock holders on a single key aren't supported yet")
		tl := e.Value.(*txnLocks)
...
func (l *lockState) tryUpdateLockLocked(up roachpb.LockUpdate) (heldByTxn, gc bool) {
	...
	if up.Status.IsFinalized() {
		...
		if !l.isLocked() {
			// We only call lockIsFree if the key is no longer locked as a result of
			// this lock update.
			gc = l.lockIsFree()
		}
		return true, gc
...
func (l *lockState) tryFreeLockOnReplicatedAcquire() bool {
	...
	//
	// TODO(arul): This tryFreeLockOnReplicatedAcquire structure may be a bit too
	// gung-ho for shared locks. Imagine the case where a there are 3
	// (unreplicated) shared locks on a key. One of the transactions (txn 1) decides to
	// acquire a replicated shared lock. We say that we can get rid of this lock
	// tracking because now there's a replicated version of the lock. However, it
	// only exists till txn 1 is pending. If txn 1 is finalized before the other
	// two transactions, they will lose their shared lock protection as a result
	// of us dropping this lock here. Unreplicated locks are best effort and all,
	// but maybe we should be trying harder?
	l.clearAllLockHolders()
	gc := l.lockIsFree()
	if !gc {
