...
func (kl *keyLocks) tryUpdateLockLocked(
	...
	if up.Status.IsFinalized() {
		...
		if !kl.isLocked() {
			...
			// stronger might be able to do so.
			//
...
func (kl *keyLocks) recomputeWaitQueues(st *cluster.Settings) {
	...
	belongsToOnlyLockHolderOrDoesNotConflictsWithStrongestMode := func(g *lockTableGuardImpl) bool {
		return (lockHolderTxn != nil && g.isSameTxn(lockHolderTxn)) ||
			!lock.Conflicts(g.curLockMode(), strongestMode, &st.SV)
	}
	...
	for e := kl.waitingReaders.Front(); e != nil; {
		...
		curr := e
		e = e.Next()
		if belongsToOnlyLockHolderOrDoesNotConflictsWithStrongestMode(reader) {
