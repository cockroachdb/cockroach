...
func (kl *keyLocks) maybeDisallowLockPromotion(
	...
	if (reAcquisitionStr == lock.Intent || reAcquisitionStr == lock.Exclusive) && held == lock.Shared {
		return errors.Newf("lock promotion from %s to %s is not allowed", held, reAcquisitionStr)
	}
	return nil
}
