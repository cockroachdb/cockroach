		...
		for e := kl.holders.Front(); e != nil; e = e.Next() {
			...
			if holderTxn.ID != acq.Txn.ID &&
				...
				return errors.AssertionFailedf(
					"lock acquisition by transaction %s with strength %s incompatible with an already held lock by %s with strength %s",
					acq.Txn.ID, acq.Strength, holder.txn.ID, holder.getLockMode().Strength,
				)
			}
		}
	}
...
func (kl *keyLocks) discoveredLock(
	...
	if kl.isLockedBy(foundLock.Txn.ID) {
		...
		for e := kl.holders.Front(); e != nil; e = e.Next() {
			...
			holderTxn, _ := holder.getLockHolder()
			// Holder belongs to a different transaction ...
			if holderTxn.ID != foundLock.Txn.ID &&
				// ... which conflicts with the discovered lock.
				lock.Conflicts(holder.getLockMode(), m, &g.lt.settings.SV) {
				return errors.AssertionFailedf(
					"discovered lock by transaction %s with strength %s incompatible with an already held lock by %s with strength %s",
					foundLock.Txn.ID, foundLock.Strength, holder.txn.ID, holder.getLockMode().Strength,
				)
			}
		}
