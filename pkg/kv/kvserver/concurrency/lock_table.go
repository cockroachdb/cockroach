...
const (
	...
	//
	// NB: The main motivation of the waitSelf state is to avoid certain deadlocks
	// by potentially re-ordering the lock's wait queue. Typically, the waitSelf
	// state arises when there are >= 1 locking requests from other transactions
	// between two requests from the same transaction waiting in a lock's wait
	// queue. So something like:
	//
	// key a: unlocked
	// wait-queue: [(r1,txn1,inactive), (r2,txn2,active), ... (r3,txn1,active)]
	//
	// Without any re-ordering, this is a guaranteed deadlock, as r2 from txn2 is waiting
	// on txn1, and r3 from txn1 is waiting on txn2. We're able to avoid this deadlock
...
func (kl *keyLocks) informActiveWaiters() {
	...
	for e := kl.waitingReaders.Front(); e != nil; e = e.Next() {
		state := waitForState
		g := e.Value
		state.txn, state.held = kl.claimantTxn(g)
...
// claimantTxn returns the transaction that the lock table deems to have claimed
// the key, which in turn causes the supplied request to actively wait.
...
//
// REQUIRES: kl.mu to be locked.
func (kl *keyLocks) claimantTxn(g *lockTableGuardImpl) (_ *enginepb.TxnMeta, held bool) {
	if kl.isLocked() {
...
func (kl *keyLocks) tryUpdateLockLocked(
	...
	if up.Status.IsFinalized() {
		...
		if !kl.isLocked() {
			...
			// transaction that were waiting to promote their lock to something stronger
			// are now able to do so. Let them through.
			//
...
func (kl *keyLocks) verify(st *cluster.Settings) error {
	...
	for e := kl.waitingReaders.Front(); e != nil; e = e.Next() {
		claimantTxn, _ := kl.claimantTxn(e.Value)
		e.Value.mu.Lock()
	...
	for e := kl.queuedLockingRequests.Front(); e != nil; e = e.Next() {
		...
		claimantTxn, _ := kl.claimantTxn(e.Value.guard)
