...
// nonLockingReadersBlockingTS returns a timestamp such that any non-locking
// readers above this timestamp should block on this lock. If non-locking
// readers should not block on this lock, hlc.MaxTimestamp is returned.
//
// REQUIRES: kl.mu to be locked.
// TODO(XXX): having trouble with the name here.
func (tl *txnLock) nonLockingReadersBlockingTS() hlc.Timestamp {
	ts := hlc.MaxTimestamp
...
func (tl *txnLock) reacquireLock(acq *roachpb.LockAcquisition) error {
	...
	//
	// However, a lock's timestamp as reported by nonLockingReadersBlockingTS can
	// regress if it is acquired at a lower timestamp and a different durability
	// than it was previously held with. This is necessary to support because the
	// hard constraint which we must uphold here that the lockHolderInfo for a
...
func (kl *keyLocks) acquireLock(
	...
	if kl.isLockedBy(acq.Txn.ID) {
		...
		tl := e.Value
		beforeTs := tl.nonLockingReadersBlockingTS()
		err := tl.reacquireLock(acq)
		if err != nil {
			return err
		}
		afterTs := tl.nonLockingReadersBlockingTS()
