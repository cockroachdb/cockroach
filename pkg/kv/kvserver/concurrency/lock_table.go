...
func (t *lockTableImpl) AcquireLock(
	...
	if !iter.Valid() {
		if durability == lock.Replicated {
			...
			// running into the maxLocks limit is very crude, and involves removing
			// all locks from the data-structure. Treating the data-structure as a
			// bounded cache with eviction would be better.
			tree.mu.Unlock()
			return nil
		}
		l = &lockState{id: tree.nextLockSeqNum(), key: key, ss: ss}
