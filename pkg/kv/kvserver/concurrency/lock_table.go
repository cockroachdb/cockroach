...
func (g *lockTableGuardImpl) IsKeyLocked(key roachpb.Key) (bool, *enginepb.TxnMeta) {
...
func (t *lockTableImpl) ScanAndEnqueue(req Request, guard lockTableGuard) lockTableGuard {
	...
	if g.waitPolicy == lock.WaitPolicy_SkipLocked {
		...
		// calls into IsKeyLocked before adding keys to its result set to determine
		// which keys it should skip.
		return g
	}
