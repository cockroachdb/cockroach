...
// markLockIneligbleForExport marks any lock held by the same transaction on the
...
//
// Acquires kl.mu.
func (kl *keyLocks) markLockIneligbleForExport(acq *roachpb.LockAcquisition) {
...
func (t *lockTableImpl) MarkIneligibleForExport(acq *roachpb.LockAcquisition) error {
	...
	iter.FirstOverlap(&keyLocks{key: acq.Key})
	if iter.Valid() {
		iter.Cur().markLockIneligbleForExport(acq)
	}
	return nil
}
