	...
	datadriven.Walk(t, "testdata/concurrency_manager", func(t *testing.T, path string) {
		...
		datadriven.RunTest(t, path, func(t *testing.T, d *datadriven.TestData) string {
			...
			case "debug-latch-manager":
				...
				output := []string{
					fmt.Sprintf("write count: %d", metrics.GlobalWriteCount+metrics.LocalWriteCount),
					fmt.Sprintf(" read count: %d", metrics.GlobalReadCount+metrics.LocalReadCount),
				}
...
func (c *cluster) reset() error {
	...
	if metrics.LocalReadCount+metrics.LocalWriteCount+metrics.GlobalReadCount+metrics.GlobalWriteCount > 0 {
