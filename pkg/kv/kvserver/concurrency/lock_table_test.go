...
func TestLockTableConcurrentRequests(t *testing.T) {
	...
	numRequests := 1000
	numActiveTxns := 8
	priTxnalReq := 0.9 // 10% of requests will be non-transactional
	priCreateNewTxn := 0.75
	priDupAccessWithWeakerStr := 0.5
	priOnlyRead := 0.5
	testLockTableConcurrentRequests(
		t, numActiveTxns, numRequests, priTxnalReq, priCreateNewTxn,
		priDupAccessWithWeakerStr, priOnlyRead,
	)
}
...
func testLockTableConcurrentRequests(
	...
	for i := 0; i < numRequests; i++ {
		...
		for i := 0; i < numKeys; i++ {
			...
			if dupAccess {
				// TOOD(XXX): we could do something similar to strongerOrEqualStrengths
				// and create a weakerStrengths function, but it feels overkill if we're
				// just using it for this test.
