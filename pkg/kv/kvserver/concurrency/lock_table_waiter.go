...
func (w *lockTableWaiterImpl) pushLockTxn(
	...
	case lock.WaitPolicy_Block:
		// The wait policy signifies that the request wants to wait until the
	...
	case lock.WaitPolicy_Error:
		// The wait policy signifies that the request wants to raise an error
		...
		log.VEventf(ctx, 3, "pushing txn %s to check if abandoned", ws.txn.ID.Short())
	...
	if err != nil {
		// If the push failed due to a TransactionPushError then we must
		// have been using a PUSH_TOUCH, otherwise it would have blocked
		// until completion (except when DontRetryPushTxnFailures is set).
		// Transform the error into a WriteIntentError.
		if _, ok := err.GetDetail().(*roachpb.TransactionPushError); ok && pushType == roachpb.PUSH_TOUCH {
