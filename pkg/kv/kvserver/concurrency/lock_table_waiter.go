	...
	for {
		...
		case <-newStateC:
			...
			case waitFor, waitForDistinguished:
				if req.WaitPolicy == lock.WaitPolicy_Error {
					...
					// immediately without waiting. If the conflict is a lock, or if the
					// waiter has priority, push the lock holder's transaction.
					// Otherwise, an active concurrent transaction is being sequenced
					// through the lock table and has claimed the lock; raise an error
					// immediately.
					if state.held || canPushWithPriority(req, state) {
		...
		case <-timerC:
			...
			pushNoWait := func(ctx context.Context) *Error {
				// Resolve the conflict without waiting. If the conflict is a lock, or
				// if the waiter has priority, push the lock holder's transaction.
				// Otherwise, the conflict is an active claimant transaction; raise an
				// error immediately.
				if timerWaitingState.held || canPushWithPriority(req, timerWaitingState) {
