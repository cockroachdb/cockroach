...
// readTimestamp returns the maximum timestamp at which the request conflicts
...
// locks acquired by other transactions above this timestamp are ignored.
func (r *Request) readTimestamp() hlc.Timestamp {
	ts := r.Timestamp
	if r.Txn != nil {
...
// writeTimestamp returns the timestamp at which the request acquires locks when
// performing mutations. All writes performed by the requests must take place at
// or above this timestamp.
func (r *Request) writeTimestamp() hlc.Timestamp {
	ts := r.Timestamp
	if r.Txn != nil {
