...
// UnreplicatedLockReliabilityUpgrade controls whether the replica will attempt
// to keep unreplicated locks during various node operations.
var UnreplicatedLockReliabilityUpgrade = settings.RegisterBoolSetting(
	settings.SystemOnly,
	"kv.lock_table.unreplicated_lock_reliability_upgrade.enabled",
...
func (m *managerImpl) OnRangeSplit(endKey roachpb.Key) []roachpb.LockAcquisition {
	if UnreplicatedLockReliabilityUpgrade.Get(&m.st.SV) {
		lockToMove := m.lt.ClearRHS(endKey)
		m.twq.ClearRHS(endKey)
		return lockToMove
	} else {
		// TODO(ssd): We could call ClearRHS here but ignore the response. But for now
		// we leave the old behaviour unchanged.
		const disable = false
		m.lt.Clear(disable)
		m.twq.Clear(disable)
		return nil
