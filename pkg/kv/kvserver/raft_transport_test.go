...
func (rttc *raftTransportTestContext) AddNode(nodeID roachpb.NodeID) *kvserver.RaftTransport {
	transport, addr := rttc.AddNodeWithoutGossip(
		...
		kvflowdispatch.NewDummyDispatch(), nil,
	)
...
func TestRaftTransportCircuitBreaker(t *testing.T) {
	...
	_, serverAddr := rttc.AddNodeWithoutGossip(serverReplica.NodeID, util.TestAddr, rttc.stopper, kvflowdispatch.NewDummyDispatch(), nil)
	serverChannel := rttc.ListenStore(serverReplica.NodeID, serverReplica.StoreID)
...
func TestReopenConnection(t *testing.T) {
	...
	serverTransport, serverAddr :=
		rttc.AddNodeWithoutGossip(serverReplica.NodeID, util.TestAddr, serverStopper, kvflowdispatch.NewDummyDispatch(), nil)
	rttc.GossipNode(serverReplica.NodeID, serverAddr)
	...
	rttc.AddNodeWithoutGossip(replacementReplica.NodeID, serverAddr, rttc.stopper, kvflowdispatch.NewDummyDispatch(), nil)
