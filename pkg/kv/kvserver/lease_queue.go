...
// replica to be removed from a range or moving from a non-preferrred
...
func (lq *leaseQueue) process(
	...
	if tokenErr := repl.allocatorToken.Acquire(ctx, lq.name, func(ctx context.Context) {
		lq.maybeAdd(ctx, repl, lq.store.Clock().NowAsClockTimestamp())
	}); tokenErr != nil {
...
func (lq *leaseQueue) canTransferLeaseFrom(
	...
	if !repl.OwnsValidLease(ctx, lq.store.cfg.Clock.NowAsClockTimestamp()) {
