...
func (n *controllerImpl) AdmitKVWork(
	...
	if ba.IsWrite() && !ba.IsSingleHeartbeatTxnRequest() {
		if !bypassAdmission &&
			...
			n.settings.Version.IsActive(ctx, clusterversion.V23_1_UseACRaftEntryEntryEncodings) {
...
func (n *controllerImpl) AdmitRaftEntry(
	...
	if log.V(1) {
		log.Infof(ctx, "decoded raft admission meta below-raft: pri=%s create-time=%d proposer=n%s receiver=[n%s,s%s] tenant=t%d tokens=%d sideloaded=%t raft-entry=%d/%d",
