...
func (n *controllerImpl) AdmitKVWork(
	...
	if admissionEnabled {
		...
		//   same machine.
		// - We do the same internally submitted low priority reads in general
		//   (notably, for KV work done on the behalf of row-level TTL reads).
		//   Everything admissionpb.UserLowPri and above uses the slots
		//   mechanism.
