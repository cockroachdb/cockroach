...
// PruneDisconnectedStoresInterval determines the frequency at which we prune
// the set of disconnected stores in the RaftTransport layer.
var PruneDisconnectedStoresInterval = settings.RegisterDurationSetting(
	settings.SystemOnly,
	"kvadmission.raft_transport.prune_disconnected_stores_interval",
	"the interval at which the raft transport prunes its disconnected stores; set to 0 to disable the mechanism",
	time.Second,
	settings.NonNegativeDuration,
)
