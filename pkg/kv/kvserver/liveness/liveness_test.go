...
func TestNodeLivenessLivenessStatus(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			// TODO: Add additional testing around the lastNodeUpdate.
			// Pretend that the node descriptor has been recently updated so we don't
			// fail due to that.
			nl.cache.mu.lastNodeUpdate = make(map[roachpb.NodeID]UpdateInfo)
			nl.cache.mu.lastNodeUpdate[1] = UpdateInfo{lastUpdateTime: now}

			nv := nl.convertToNodeVitality(tc.liveness)
			ls := nv.LivenessStatus()
			if ls != tc.expected {
				t.Errorf("liveness status was %s, expected %s for %+v at %v", ls, tc.expected, tc.liveness, now)
			}
		})
	}
}
