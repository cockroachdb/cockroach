...
func isErrRetryLiveness(err error) bool {
	return errors.HasType(err, (*roachpb.TransactionStatusError)(nil)) ||
...
func (nl *NodeLiveness) CreateLivenessRecord(ctx context.Context, nodeID roachpb.NodeID) error {
	...
	return errors.Newf("failed to create liveness record for node %d", nodeID)
}
...
func (nl *NodeLiveness) updateLivenessAttempt(
	...
	if err := nl.db.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
		...
		} else if isErrRetryLiveness(err) {
