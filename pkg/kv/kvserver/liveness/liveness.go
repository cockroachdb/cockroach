...
// nodes could still be leaseholders for ranges until drained, so this should
// not be used when the caller needs to be able to contact leaseholders
// directly. Returns false if the node is not in the local liveness table.
