...
func NewNodeLiveness(opts NodeLivenessOptions) *NodeLiveness {
	nl := &NodeLiveness{
		...
		clock:                 opts.Clock,
		livenessStorage:       livenessStorage{db: opts.DB},
		gossip:                opts.Gossip,
...
func (nl *NodeLiveness) CreateLivenessRecord(ctx context.Context, nodeID roachpb.NodeID) error {
	return nl.livenessStorage.create(ctx, nodeID)
}
...
func (nl *NodeLiveness) GetLivenessesFromKV(ctx context.Context) ([]livenesspb.Liveness, error) {
	records, err := nl.livenessStorage.scan(ctx)
	if err != nil {
		return nil, err
	}
...
func (nl *NodeLiveness) getLivenessRecordFromKV(
	...
	livenessRec, err := nl.livenessStorage.get(ctx, nodeID)
	if err == nil {
...
// node's IsLive() state changes to true. This must be called before Start, or
// it will wait until start is called to return.
...
func (nl *NodeLiveness) updateLivenessAttempt(
	...
	return nl.livenessStorage.update(ctx, update, handleCondFailed)
}
