...
type cache struct {
	...
	mu                    struct {
		syncutil.RWMutex
		// Stores timestamps of StoreDescriptor updates in Gossip
		nodeDescMap map[roachpb.NodeID]hlc.Timestamp
		// Stores liveness records read from Gossip
		nodes map[roachpb.NodeID]Record
	}
}

// start will register with gossip and update the cache when there are updates
// to the liveness or store descriptors.
func (c *cache) start(notifyLivenessChanged func(livenesspb.Liveness, livenesspb.Liveness)) {
	c.notifyLivenessChanged = notifyLivenessChanged
	c.mu.nodes = make(map[roachpb.NodeID]Record)
	c.mu.nodeDescMap = make(map[roachpb.NodeID]hlc.Timestamp)
...
// myNodeID returns the ID for this node according to Gossip. This will be 0
// until the node has joined the cluster.
func (c *cache) myNodeID() roachpb.NodeID {
...
func (c *cache) storeGossipUpdate(_ string, content roachpb.Value) {
	...
	c.mu.Lock()
	c.mu.nodeDescMap[nodeID] = c.clock.Now()
	c.mu.Unlock()
}
...
func (c *cache) Self() (_ Record, ok bool) {
	c.mu.RLock()
	defer c.mu.RUnlock()
	return c.getLivenessLocked(c.myNodeID())
}
