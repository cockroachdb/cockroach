...
// livenessStorage is the subset of liveness that deals with reading and writing the
// liveness records to kv. All calls to modify liveness are centrialized here.
type livenessStorage struct {
	db *kv.DB
}
...
// previous data.
func (ls livenessStorage) get(ctx context.Context, nodeID roachpb.NodeID) (Record, error) {
...
func (ls livenessStorage) update(
...
// an error instead.
func (ls livenessStorage) create(ctx context.Context, nodeID roachpb.NodeID) error {
...
func (ls livenessStorage) scan(ctx context.Context) ([]Record, error) {
