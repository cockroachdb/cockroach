...
func replicaIsSuspect(repl *Replica) bool {
	...
	case raft.StateFollower:
		...
		if !ok || !repl.store.cfg.NodeLiveness.GetNodeVitalityFromCache(leadDesc.NodeID).IsLive(livenesspb.ReplicaProgress) {
			return true
		}
	...
	case raft.StateLeader:
		if !repl.Desc().Replicas().CanMakeProgress(func(d roachpb.ReplicaDescriptor) bool {
			return !repl.store.cfg.NodeLiveness.GetNodeVitalityFromCache(d.NodeID).IsLive(livenesspb.ReplicaProgress)
		}) {
			return true
		}
	}
