...
const (
	...
	// is suspected to be removed should be processed by the queue. See
	// replicaIsSuspect() for details on what makes a replica suspect.
	ReplicaGCQueueSuspectTimeout = 5 * time.Second
)
...
func replicaGCShouldQueueImpl(
	...
	shouldQ := lastActivity.Add(timeout.Nanoseconds(), 0).Less(now) &&
		lastCheck.Add(timeout.Nanoseconds(), 0).Less(now)
	if !shouldQ {
		return false, 0
	}
