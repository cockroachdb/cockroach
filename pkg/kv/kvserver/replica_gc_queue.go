...
const (
	...
	// waiting for their left neighbor to be GC'd first.
	replicaGCPurgatoryCheckInterval = 1 * time.Second
)
...
func (e *replicaGCPurgatoryError) Error() string         { return e.err.Error() }
func (e *replicaGCPurgatoryError) PurgatoryErrorMarker() {}
		...
		if leftRepl != nil {
			...
			if leftReplyDesc := &rs[0]; !leftDesc.Equal(leftReplyDesc) {
				...
				return false, &replicaGCPurgatoryError{
					err: errors.Errorf("left neighbor %s not up-to-date with meta descriptor %s",
						leftDesc, leftReplyDesc),
				}
			}
		}
