...
func NewMVCCGCQueue(
	...
	mgcq.BaseQueue = kvqueue.NewBaseQueue(
		"mvccGC", mgcq, store,
		kvqueue.Config{
			...
			ProcessTimeoutFunc: func(st *cluster.Settings, _ kvqueue.Replica) time.Duration {
				timeout := mvccGCQueueTimeout
				if d := kvqueue.QueueGuaranteedProcessingTimeBudget.Get(&st.SV); d > timeout {
					timeout = d
				}
				return timeout
			},
			Successes:       metrics.MVCCGCQueueSuccesses,
			Failures:        metrics.MVCCGCQueueFailures,
			Pending:         metrics.MVCCGCQueuePending,
			ProcessingNanos: metrics.MVCCGCQueueProcessingNanos,
		},
		knobs,
	)
	return mgcq
...
func (mgcq *MvccGCQueue) ShouldQueue(
	...
	r := mgcq.makeMVCCGCQueueScore(ctx, repl, gcTimestamp, lastGC, conf.TTL(), canAdvanceGCThreshold)
	return r.ShouldQueue, r.FinalScore
}

func (mgcq *MvccGCQueue) makeMVCCGCQueueScore(
	ctx context.Context,
	...
	if mgcq.Knobs.DisableLastProcessedCheck {
		lastGC = hlc.Timestamp{}
	}
	...
	r := MakeMVCCGCQueueScoreImpl(
		ctx, int64(repl.GetRangeID()), now, ms, gcTTL, lastGC, canAdvanceGCThreshold,
	)
	return r
}
...
func (mgcq *MvccGCQueue) Process(
	...
	if err := repl.SetQueueLastProcessed(ctx, mgcq.Name(), mgcq.Clock.Now()); err != nil {
		log.VErrEventf(ctx, 2, "failed to update last processed time: %v", err)
	}

	snap := mgcq.Store.Engine().NewSnapshot()
	defer snap.Close()

	intentAgeThreshold := gc.IntentAgeThreshold.Get(&mgcq.Settings.SV)
	maxIntentsPerCleanupBatch := gc.MaxIntentsPerCleanupBatch.Get(&mgcq.Settings.SV)
	maxIntentKeyBytesPerCleanupBatch := gc.MaxIntentKeyBytesPerCleanupBatch.Get(&mgcq.Settings.SV)
	...
	info, err := gc.Run(ctx, desc, snap, gcTimestamp, newThreshold,
		...
		&replicaGCer{
			repl:                repl,
			admissionController: mgcq.Store.KVAdmissionController(),
			storeID:             mgcq.Store.StoreID(),
			clock:               mgcq.Clock,
		},
		func(ctx context.Context, intents []roachpb.Intent) error {
			intentCount, err := mgcq.Store.IntentResolver().
				CleanupIntents(ctx, intents, gcTimestamp, roachpb.PUSH_TOUCH)
			if err == nil {
				mgcq.metrics.GCResolveSuccess.Inc(int64(intentCount))
			} else {
				mgcq.metrics.GCResolveFailed.Inc(int64(intentCount))
			}
			return err
		},
		func(ctx context.Context, txn *roachpb.Transaction) error {
			err := mgcq.Store.IntentResolver().
				CleanupTxnIntentsOnGCAsync(ctx, repl.GetRangeID(), txn, gcTimestamp,
					func(pushed, succeeded bool) {
						if pushed {
							mgcq.metrics.GCPushTxn.Inc(1)
						}
						if succeeded {
							mgcq.metrics.GCResolveSuccess.Inc(int64(len(txn.LockSpans)))
						} else {
							mgcq.metrics.GCTxnIntentsResolveFailed.Inc(int64(len(txn.LockSpans)))
						}
					})
	...
	log.Eventf(ctx, "GC score after GC: %s", mgcq.makeMVCCGCQueueScore(
		ctx, repl, mgcq.Clock.Now(), lastGC, conf.TTL(), canAdvanceGCThreshold))
	updateStoreMetricsWithGCInfo(mgcq.metrics, info)
	return true, nil
}

func updateStoreMetricsWithGCInfo(metrics *storemetrics.StoreMetrics, info gc.Info) {
