...
func TestReplicateQueueUpReplicateOddVoters(t *testing.T) {
	...
	if n := store.TestingReplicateQueuePurgatoryLength(); expected != n {
...
func TestReplicateQueueMetrics(t *testing.T) { // XXX: Move
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)
	skip.UnderRace(t, "takes a long time or times out under race")
...
func TestTransferLeaseToLaggingNode(t *testing.T) {
	...
	remoteStore.TestingTransport().Listen(
		remoteStoreID,
	...
	testutils.SucceedsSoon(t, func() error {
		...
		transferred, err := leaseStore.TestingFindTargetAndTransferLease(
