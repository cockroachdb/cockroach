...
type ReplicateQueue struct {
	*kvqueue.BaseQueue
	Metrics           ReplicateQueueMetrics   // XXX: Shouldn't be exported
	Allocator         allocatorimpl.Allocator // XXX: Shouldn't be exported
...
func NewReplicateQueue(
	...
	metrics.Registry.AddMetricStruct(&rq.Metrics)
	rq.BaseQueue = kvqueue.NewBaseQueue(
		...
		kvqueue.Config{
			...
			Successes:          metrics.ReplicateQueueSuccesses,
			Failures:           metrics.ReplicateQueueFailures,
			Pending:            metrics.ReplicateQueuePending,
			ProcessingNanos:    metrics.ReplicateQueueProcessingNanos,
			Purgatory:          metrics.ReplicateQueuePurgatory,
		},
		knobs,
	)
	...
	if g := store.Gossip(); g != nil { // gossip is nil for some unittests
	...
	if nl := store.NodeLiveness(); nl != nil { // node liveness is nil for some unittests
...
func (rq *ReplicateQueue) ShouldQueue(
	...
	if !rq.Knobs.DisableReplicaRebalancing {
...
func (rq *ReplicateQueue) Process(
	ctx context.Context, repl kvqueue.Replica, confReader spanconfig.StoreReader,
...
func (rq *ReplicateQueue) ProcessOneChange(
	...
	canTransferLeaseFrom func(ctx context.Context, repl kvqueue.Replica) bool,
	scatter, dryRun bool,
	...
	voterReplicas := desc.Replicas().VoterDescriptors()
	nonVoterReplicas := desc.Replicas().NonVoterDescriptors()
	liveVoterReplicas, deadVoterReplicas := rq.Store.StorePool().LiveAndDeadReplicas(
		voterReplicas, true, /* includeSuspectAndDrainingStores */
	)
	liveNonVoterReplicas, deadNonVoterReplicas := rq.Store.StorePool().LiveAndDeadReplicas(
	...
	// unavailability; see:
	//
	//   _ = kvserver.execChangeReplicasTxn
	...
	case allocatorimpl.AllocatorReplaceDecommissioningVoter:
		decommissioningVoterReplicas := rq.Store.StorePool().DecommissioningReplicas(voterReplicas)
	...
	case allocatorimpl.AllocatorReplaceDecommissioningNonVoter:
		decommissioningNonVoterReplicas := rq.Store.StorePool().DecommissioningReplicas(nonVoterReplicas)
...
func (rq *ReplicateQueue) addOrReplaceVoters(
	...
	if removeIdx >= 0 {
		...
		lhRemovalAllowed := rq.Settings.Version.IsActive(ctx,
	...
	lhBeingRemoved := removeIdx >= 0 && existingVoters[removeIdx].StoreID == rq.Store.StoreID()
	...
	clusterNodes := rq.Store.StorePool().ClusterNodeCount()
...
func (rq *ReplicateQueue) findRemoveVoter(
	...
	for r := retry.StartWithCtx(ctx, retryOpts); r.Next() && timeutil.Now().Before(deadline); {
		...
		if raftStatus == nil || raftStatus.RaftState != raft.StateLeader {
			...
			return roachpb.ReplicationTarget{}, "", &kvqueue.BenignError{errors.Errorf("not raft leader while range needs removal")}
		}
	...
	if len(candidates) == 0 {
		...
		return roachpb.ReplicationTarget{}, "", &kvqueue.BenignError{
			errors.Errorf(
...
func (rq *ReplicateQueue) maybeTransferLeaseAway(
	...
	if removeStoreID != rq.Store.StoreID() {
		return false, nil
	}
...
func (rq *ReplicateQueue) removeDecommissioning(
	...
	switch targetType {
	case allocatorimpl.VoterTarget:
		decommissioningReplicas = rq.Store.StorePool().DecommissioningReplicas(
...
func (rq *ReplicateQueue) removeLearner(
	ctx context.Context, repl kvqueue.Replica, dryRun bool,
) (requeue bool, _ error) {
	desc := repl.Desc()
...
func (rq *ReplicateQueue) considerRebalance(
	...
	if !rq.Knobs.DisableReplicaRebalancing {
		...
		lhRemovalAllowed := addTarget != (roachpb.ReplicationTarget{}) &&
			rq.Settings.Version.IsActive(ctx, clusterversion.EnableLeaseHolderRemoval)
		lhBeingRemoved := removeTarget.StoreID == rq.Store.StoreID()
...
func (rq *ReplicateQueue) TransferLease(
	ctx context.Context, repl kvqueue.Replica, target roachpb.ReplicaDescriptor, rangeQPS float64,
) error {
	...
	rq.Store.StorePool().UpdateLocalStoresAfterLeaseTransfer(
		rq.Store.StoreID(), target.StoreID, rangeQPS)
	return nil
}
...
func (rq *ReplicateQueue) changeReplicas(
	...
	for _, chg := range chgs {
		rq.Store.StorePool().UpdateLocalStoreAfterRebalance(
...
func (rq *ReplicateQueue) canTransferLeaseFrom(ctx context.Context, repl kvqueue.Replica) bool {
	...
	if lastLeaseTransfer := rq.lastLeaseTransfer.Load(); lastLeaseTransfer != nil {
		minInterval := MinLeaseTransferInterval.Get(&rq.Settings.SV)
...
func RangeUsageInfoForRepl(repl kvqueue.Replica) allocator.RangeUsageInfo {
