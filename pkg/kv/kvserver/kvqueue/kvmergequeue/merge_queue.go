...
type MergeQueue struct {
	*kvqueue.BaseQueue
	...
	// spanConfigsDisabled captures whether the span configs infrastructure is
	// disabled.
...
func NewMergeQueue(
	...
	mq := &MergeQueue{
		...
		spanConfigsDisabled: spanConfigsDisabled,
	}
	mq.BaseQueue = kvqueue.NewBaseQueue(
		"merge", mq, store,
		kvqueue.Config{
			...
			// factor.
			ProcessTimeoutFunc:   kvqueue.MakeRateLimitedTimeoutFunc(kvqueue.RebalanceSnapshotRate),
			NeedsLease:           true,
			NeedsSystemConfig:    true,
			AcceptsUnsplitRanges: false,
			Successes:            metrics.MergeQueueSuccesses,
			Failures:             metrics.MergeQueueFailures,
			Pending:              metrics.MergeQueuePending,
			ProcessingNanos:      metrics.MergeQueueProcessingNanos,
			Purgatory:            metrics.MergeQueuePurgatory,
		},
		knobs,
	)
	return mq
...
func (mq *MergeQueue) enabled() bool {
	if !mq.spanConfigsDisabled {
		if mq.Store.SpanConfigSubscriber().LastUpdated().IsEmpty() {
	...
	return kvserverbase.MergeQueueEnabled.Get(&mq.Settings.SV)
}
...
var _ kvqueue.PurgatoryError = rangeMergePurgatoryError{}
...
func (mq *MergeQueue) Process(
	ctx context.Context, lhsRepl kvqueue.Replica, confReader spanconfig.StoreReader,
	...
	now := mq.Clock.NowAsClockTimestamp()
	...
	shouldSplit, _ := kvsplitqueue.ShouldSplitRange(ctx, mergedDesc, mergedStats,
	...
	if kvqueue.TestingAggressiveConsistencyChecks {
	...
	if mergedQPS != 0 {
		lhsRepl.LoadBasedSplitter().RecordMax(mq.Clock.PhysicalTime(), mergedQPS)
	}
	return true, nil
}

func (mq *MergeQueue) Timer(time.Duration) time.Duration {
	return MergeQueueInterval.Get(&mq.Settings.SV)
}
