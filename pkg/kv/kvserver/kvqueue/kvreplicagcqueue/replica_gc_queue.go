// CopyrighmergedTombstoneReplicaIDt 2015 The Cockroach Authors.
//
...
func NewReplicaGCQueue(
	...
	metrics.Registry.AddMetricStruct(&rgcq.metrics)
	rgcq.BaseQueue = kvqueue.NewBaseQueue(
		...
		kvqueue.Config{
			...
			Successes:                metrics.ReplicaGCQueueSuccesses,
			Failures:                 metrics.ReplicaGCQueueFailures,
			Pending:                  metrics.ReplicaGCQueuePending,
			ProcessingNanos:          metrics.ReplicaGCQueueProcessingNanos,
		}, knobs,
	)
	return rgcq
}
...
func (rgcq *ReplicaGCQueue) ShouldQueue(
	...
	if _, currentMember := repl.Desc().GetReplicaDescriptor(rgcq.Store.StoreID()); !currentMember {
	...
	isSuspect := rgcq.replicaIsSuspect(repl)

	return replicaGCShouldQueueImpl(now.ToTimestamp(), lastCheck, isSuspect)
}

func (rgcq *ReplicaGCQueue) replicaIsSuspect(repl kvqueue.Replica) bool {
	...
	replDesc, ok := repl.Desc().GetReplicaDescriptor(rgcq.Store.StoreID())
	if !ok {
		return true
	}
	...
	if rgcq.Store.NodeLiveness() == nil {
		return false
	}
	...
	if raftStatus == nil {
		liveness, ok := rgcq.Store.NodeLiveness().Self()
		return ok && !liveness.Membership.Active()
	}

	livenessMap := rgcq.Store.NodeLiveness().GetIsLiveMap()
...
func (rgcq *ReplicaGCQueue) Process(
	...
	currentDesc, currentMember := replyDesc.GetReplicaDescriptor(rgcq.Store.StoreID())
	...
	if sameRange && currentMember {
		...
		if err := repl.SetLastReplicaGCTimestamp(ctx, rgcq.Clock.Now()); err != nil {
			return false, err
		}
		...
		// knows that.
		if err := rgcq.Store.RemoveReplica(ctx, repl, nextReplicaID, kvqueue.RemoveOptions{
			DestroyData: true,
		}); err != nil {
			return false, err
		}
		...
		leftRepl := rgcq.Store.LookupPrecedingReplica(desc.StartKey)
		if leftRepl != nil {
		...
		if err := rgcq.Store.RemoveReplica(ctx, repl, MergedTombstoneReplicaID, kvqueue.RemoveOptions{
			DestroyData: true,
		}); err != nil {
			return false, err
		}
