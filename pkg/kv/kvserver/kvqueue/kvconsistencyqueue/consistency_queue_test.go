...
package kvconsistencyqueue_test
...
func TestConsistencyQueueRequiresLive(t *testing.T) {
	...
	if shouldQ, priority := kvconsistencyqueue.ShouldQueueImpl(
		context.Background(), clock.NowAsClockTimestamp(), kvconsistencyqueue.ShouldQueueData{
...
func testConsistencyQueueRecomputeStatsImpl(t *testing.T, hadEstimates bool) {
	...
	knobs.QueueKnobs.ConsistencyQueueResultHook = func(resp roachpb.CheckConsistencyResponse) {
