...
// XXX: Go through all exported symbols and fields and de-stutter. Also see what
...
func defaultProcessTimeoutFunc(cs *cluster.Settings, _ Replica) time.Duration {
...
func MakeRateLimitedTimeoutFunc(rateSetting *settings.ByteSizeSetting) QueueProcessTimeoutFunc {
	return func(cs *cluster.Settings, repl Replica) time.Duration {
...
// PurgatoryError indicates a replica processing failure which indicates
...
// Replica is the subset of *Replica required for interacting with queues.
//
...
type Replica interface {
	...
	IsInitialized() bool
	IsDestroyed() (kvserverbase.DestroyReason, error)
	...
	GetQueueLastProcessed(ctx context.Context, queue string) (hlc.Timestamp, error)
	SetQueueLastProcessed(ctx context.Context, queue string, timestamp hlc.Timestamp) error
	CheckConsistency(ctx context.Context, args roachpb.CheckConsistencyRequest) (roachpb.CheckConsistencyResponse, *roachpb.Error)
	GetMVCCStats() enginepb.MVCCStats
	GetMinBytes() int64
	GetMaxSplitQPS() (float64, bool)
	SplitByLoadEnabled() bool
	SplitByLoadQPSThreshold() float64
	GetMaxBytes() int64
	ShouldBackpressureWrites() bool
	MaybeLeaveAtomicChangeReplicasAndRemoveLearners(
	...
	MarkPendingGC(readAt, newThreshold hlc.Timestamp) error
	RaftStatus() *raft.Status
	GetLastReplicaGCTimestamp(ctx context.Context) (hlc.Timestamp, error)
	SetLastReplicaGCTimestamp(ctx context.Context, timestamp hlc.Timestamp) error
	...
	QueriesPerSecond() (float64, time.Duration)
	...
	ReportSnapshotStatus(ctx context.Context, to roachpb.ReplicaID, snapErr error)
	GetAndGCSnapshotLogTruncationConstraints(
		now time.Time, recipientStore roachpb.StoreID,
	) (minSnapIndex uint64)
	Send(context.Context, roachpb.BatchRequest) (*roachpb.BatchResponse, *roachpb.Error)
	String() string
	NewTruncateDecision(ctx context.Context) (TruncateDecision, error)
	RaftLock()
	RaftUnlock()
	SideloadedRaftMuLocked() replicasideload.SideloadStorage
	UpdateRaftLogSize(size int64)
	...
	CheckLeaseRespectsPreferences(ctx context.Context) (bool, error)
	ChangeReplicasImpl(
		ctx context.Context,
		desc *roachpb.RangeDescriptor,
		priority kvserverpb.SnapshotRequest_Priority,
...
type Store interface { // XXX:
	StoreID() roachpb.StoreID
	GetKVQueueReplica(id roachpb.RangeID) (Replica, error)
	GetConfReader(ctx context.Context) (spanconfig.StoreReader, error)
	ReplicaCount() int
	Engine() storage.Engine
	NodeLiveness() *liveness.NodeLiveness
	SpanConfigSubscriber() spanconfig.KVSubscriber
	ConsistencyQueue() QueueImpl
	DB() *kv.DB
	...
	LookupPrecedingReplica(key roachpb.RKey) Replica
...
// Replica.
type QueueProcessTimeoutFunc func(*cluster.Settings, Replica) time.Duration

type Config struct {
	log.AmbientContext // XXX: Fill in
	Clock              *hlc.Clock
...
type BaseQueue struct {
	...
	name       string
	getReplica func(roachpb.RangeID) (Replica, error)
...
// priority replica.
func NewBaseQueue(
	name string, impl QueueImpl, store Store, cfg Config, knobs *TestingKnobs,
) *BaseQueue {
	...
	bq := BaseQueue{
		...
		getReplica: func(id roachpb.RangeID) (Replica, error) {
			repl, err := store.GetKVQueueReplica(id)
			if repl == nil || err != nil {
...
// Disabled returns whether the queue disabled.
...
func (h baseQueueHelper) MaybeAdd(ctx context.Context, repl Replica, now hlc.ClockTimestamp) {
...
type QueueHelper interface {
	MaybeAdd(ctx context.Context, repl Replica, now hlc.ClockTimestamp)
...
func (bq *BaseQueue) Async(
	...
	if err := bq.Config.Stopper.RunAsyncTaskEx(bgCtx,
		stop.TaskOpts{
			TaskName:   opName,
...
func (bq *BaseQueue) MaybeAddAsync(ctx context.Context, repl Replica, now hlc.ClockTimestamp) {
...
func (bq *BaseQueue) MaybeAdd(ctx context.Context, repl Replica, now hlc.ClockTimestamp) {
	...
	should, priority := bq.impl.ShouldQueue(ctx, now, repl, confReader)
	if !should {
		return
	}
...
func (bq *BaseQueue) processReplica(ctx context.Context, repl Replica) error {
	...
	return contextutil.RunWithTimeout(ctx, fmt.Sprintf("%s queue Process replica %d", bq.name, repl.GetRangeID()),
		bq.ProcessTimeoutFunc(bq.Settings, repl), func(ctx context.Context) error {
			...
			if reason, err := repl.IsDestroyed(); err != nil {
				if !bq.Config.ProcessDestroyedReplicas || reason == kvserverbase.DestroyReasonRemoved {
			...
			// it may not be and ShouldQueue will be passed a nil realRepl. These tests
			// know what they're getting into so that's fine.
			processed, err := bq.impl.Process(ctx, repl, confReader)
			if err != nil {
				return err
			}
			if processed {
...
type BenignError struct { // XXX: Create constructor for this
	Err error
}

func (be *BenignError) Error() string { return be.Err.Error() }
func (be *BenignError) Cause() error  { return be.Err }
...
func (bq *BaseQueue) finishProcessingReplica(
	...
	if requeue {
		bq.MaybeAdd(ctx, repl, bq.Clock.NowAsClockTimestamp())
	}
}
...
func (bq *BaseQueue) TestingAdd(ctx context.Context, repl Replica, priority float64) (bool, error) {
	return bq.addInternal(ctx, repl.Desc(), repl.ReplicaID(), priority)
}

func (bq *BaseQueue) TestingOverrideGetReplica(fn func(id roachpb.RangeID) (Replica, error)) {
	bq.mu.Lock()
	bq.getReplica = fn
	bq.mu.Unlock()
}
