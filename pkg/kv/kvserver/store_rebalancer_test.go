...
func TestChooseRangeToRebalanceAcrossHeterogeneousZones(t *testing.T) {
	...
	allReplicasInHotRegion := []zonepb.ConstraintsConjunction{
		constraint("a", 3),
	}
	...
	testCases := []struct {
		name                          string
		voters, nonVoters             []roachpb.StoreID
		voterConstraints, constraints []zonepb.ConstraintsConjunction
		...
		{
			name:   "primary region with highest QPS, region survival, one voter on sub-optimal node",
			voters: []roachpb.StoreID{2, 3, 4, 8, 9},
		...
		},
		{
			...
			// the least hot region. Furthermore, we'd expect the voter on node 4
			// (hottest node in region B) to move to node 6 (least hot node in region
			// B).
			expRebalancedVoters: []roachpb.StoreID{9, 2, 6, 8, 3},
		},
	}
