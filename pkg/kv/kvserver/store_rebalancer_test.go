...
func (t *testRebalanceObjectiveProvider) Objective(_ context.Context) LBRebalancingObjective {
	return t.objective
}
...
func TestChooseLeaseToTransfer(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run("", withQPSCPU(t, objectiveProvider, func(t *testing.T) {
			lbRebalanceDimension := objectiveProvider.Objective(ctx).ToDimension()
...
func TestChooseRangeToRebalanceRandom(t *testing.T) {
	...
	for i := 0; i < numIterations; i++ {
		t.Run(fmt.Sprintf("%d", i+1), withQPSCPU(t, objectiveProvider, func(t *testing.T) {
			...
			lbRebalanceDimension := sr.RebalanceObjective(ctx).ToDimension()
			loadRanges(
				rr, s, []testRange{
			...
			if r, o := meanLoad(rebalancedVoterStores), meanLoad(voterStores); load.Greater(load.Sub(r, o), epsilon, objectiveProvider.Objective(ctx).ToDimension()) {
			...
			if load.Greater(load.Sub(newMean, previousMean), epsilon, objectiveProvider.Objective(ctx).ToDimension()) {
