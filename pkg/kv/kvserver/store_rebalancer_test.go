...
func TestStoreRebalancerReadAmpCheck(t *testing.T) {
	...
	tests := []testCase{
		{
			name: "ignore read amp on allocation when storeHealthDisabled enforcement",
			// NB: All stores have high read amp, this should be ignored when disabled.
		...
		},
		{
			...
			// NB: All stores have high read amp, this should be ignored when log.
			stores: noLocalityHighReadAmpStores,
			conf:   emptySpanConfig(),
			expectedTargets: []roachpb.ReplicationTarget{
				{NodeID: 4, StoreID: 4}, {NodeID: 3, StoreID: 3}, {NodeID: 5, StoreID: 5},
			},
			enforcement: storeHealthLog,
		},
		{
			name: "don't allocate to stores when all have highReadAmp and storeHealthRebalanceOnly enforcement",
			// NB: All stores have high read amp, no rebalancing can occur when rebalance is enforced.
			stores:         noLocalityHighReadAmpStores,
			conf:           emptySpanConfig(),
			expectNoAction: true,
		...
		{
			name: "do not rebalance to stores when all have highReadAmp and storeHealthAllocate enforcement",
			// NB: All stores have high read amp, no rebalancing can be occur when allocate is enforced.
			stores:         noLocalityHighReadAmpStores,
			conf:           emptySpanConfig(),
			expectNoAction: true,
		...
		},
		{
			...
			// NB: Store 4, 5 have high read amp, they should not be rebalance targets.
		...
		{
			name: "rebalance should ignore stores with high read amp when storeHealthAllocate enforcement",
			...
			// transferred its lease to 2, however the logic will not aim to
			// promote rebalancing off high read amp - only avoid allocation to
			// high read amp.
			stores: noLocalityAscendingReadAmpStores,
			conf:   emptySpanConfig(),
			expectedTargets: []roachpb.ReplicationTarget{
