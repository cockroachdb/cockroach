...
func DecodeRaftCommand(data []byte) (RaftCommandEncodingVersion, CmdIDKey, []byte) {
	if len(data) == 0 {
		// An empty command.
		return RaftVersionStandard, "", nil
	}
	v := RaftCommandEncodingVersion(data[0])
	if v != RaftVersionStandard && v != RaftVersionSideloaded {
		panic(fmt.Sprintf("unknown command encoding version %v", data[0]))
	}
	return v, CmdIDKey(data[1 : 1+RaftCommandIDLen]), data[1+RaftCommandIDLen:]
}
