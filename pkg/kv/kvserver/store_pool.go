...
//
// XXX: These enums only exist across a thin API boundary (.status() below), and
// probably can be collapsed away. Either in this PR or another.
const (
	_ storeStatus = iota
...
func (sd *storeDetail) status(
	now time.Time, threshold time.Duration, nodeLiveness *liveness.NodeLiveness, suspectDuration time.Duration,
) storeStatus {
	...
	//
	l, ok := nodeLiveness.GetLiveness(sd.desc.Node.NodeID)
	if !ok {
...
// with gossip.
// XXX: Go through callers in tests, and update them to provide a mock liveness
// instance instead, or a thin liveness interface instead. Probably the latter.
func NewStorePool(
...
func (sp *StorePool) String() string {
	...
	for _, id := range ids {
		...
		status := detail.status(now, timeUntilStoreDead, sp.nodeLiveness, timeAfterStoreSuspect)
...
func (sp *StorePool) decommissioningReplicas(
	...
	for _, repl := range repls {
		...
		switch detail.status(now, timeUntilStoreDead, sp.nodeLiveness, timeAfterStoreSuspect) {
...
func (sp *StorePool) storeStatus(storeID roachpb.StoreID) (storeStatus, error) {
	...
	timeUntilStoreDead := TimeUntilStoreDead.Get(&sp.st.SV)
	timeAfterStoreSuspect := TimeAfterStoreSuspect.Get(&sp.st.SV)
	return sd.status(now, timeUntilStoreDead, sp.nodeLiveness, timeAfterStoreSuspect), nil
}
...
func (sp *StorePool) getStoreListFromIDsLocked(
	...
	for _, storeID := range storeIDs {
		...
		switch s := detail.status(now, timeUntilStoreDead, sp.nodeLiveness, timeAfterStoreSuspect); s {
