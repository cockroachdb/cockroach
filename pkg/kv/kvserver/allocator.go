...
func (a *Allocator) allocateTargetFromList(
	...
	candidates := rankedCandidateListForAllocation(
		...
		// for either 1 voter or 1 non-voter, we want the voter to be able to
		// displace the non-voter.
		existingReplicaSet,
...
func (a Allocator) simulateRemoveTarget(
	...
	case voterTarget:
		...
		log.VEventf(ctx, 3, "simulating which voter would be removed after adding s%d", targetStore)
	...
	case nonVoterTarget:
		...
		defer func() {
			a.storePool.updateLocalStoreAfterRebalance(
				targetStore,
				rangeUsageInfo,
				roachpb.REMOVE_NON_VOTER,
			)
		}()
		log.VEventf(ctx, 3, "simulating which non-voter would be removed after adding s%d", targetStore)
		return a.RemoveNonVoter(ctx, zone, candidates, existingVoters, existingNonVoters)
	default:
		log.Fatalf(ctx, "unknown targetReplicaType: %s", t)
	}

	// Unreachable.
	return roachpb.ReplicaDescriptor{}, "", nil
}
...
func (a Allocator) rebalanceTarget(
	...
	replicaSetForDiversityCalc := getReplicaSetForDiversityCalc(targetType, existingVoters, existingReplicas)
	switch t := targetType; t {
	case voterTarget:
...
//
// 1. We do not consider stores that have voters as valid candidates for
// rebalancing.
// 2. Diversity score calculation for non-voters is relative to all existing
