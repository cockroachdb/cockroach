...
func (a *Allocator) allocateTargetFromList(
	...
	var constraintsCheckFn constraintsCheckFn
	var replicaSetForDiversityCalc []roachpb.ReplicaDescriptor
	switch t := targetType; t {
	case voterTarget:
		constraintsCheckFn = voterConstraintsCheckerForAllocation(
	...
	case nonVoterTarget:
		constraintsCheckFn = nonVoterConstraintsCheckerForAllocation(analyzedOverallConstraints)
...
func (a Allocator) removeTarget(
	...
	case voterTarget:
		...
		// replicas.
		constraintsCheckFn = voterConstraintsCheckerForRemoval(
			analyzedOverallConstraints,
			analyzedVoterConstraints,
		)
		...
		replicaSetForDiversityCalc = existingVoters
	case nonVoterTarget:
		constraintsCheckFn = nonVoterConstraintsCheckerForRemoval(analyzedOverallConstraints)
		replicaSetForDiversityCalc = existingReplicas
	default:
		log.Fatalf(ctx, "unsupported targetReplicaType: %v", t)
	}
...
// as AllocateVoter(), except the chosen target must follow some additional
...
// by AllocateVoter() doesn't fit balancing criteria is perfectly fine, as
