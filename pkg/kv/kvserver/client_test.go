...
type multiTestContext struct {
	...
	nodeLivenesses []*nodeliveness.NodeLiveness
}
...
func (m *multiTestContext) Start(t testing.TB, numStores int) {
	...
	m.nodeLivenesses = make([]*nodeliveness.NodeLiveness, numStores)
...
func (m *multiTestContext) populateStorePool(
	idx int, cfg kvserver.StoreConfig, nodeLiveness *nodeliveness.NodeLiveness,
) {
...
func (m *multiTestContext) addStore(idx int) {
	...
	m.nodeLivenesses[idx] = nodeliveness.NewNodeLiveness(nodeliveness.NodeLivenessOptions{
	...
	m.nodeLivenesses[idx].Start(ctx,
		nodeliveness.NodeLivenessStartOptions{
			Stopper: stopper,
...
func (m *multiTestContext) restartStoreWithoutHeartbeat(i int) {
	...
	nlActive, nlRenewal := cfg.NodeLivenessDurations()
	m.nodeLivenesses[i] = nodeliveness.NewNodeLiveness(nodeliveness.NodeLivenessOptions{
	...
	m.mu.Unlock()
	cfg.NodeLiveness.Start(ctx, nodeliveness.NodeLivenessStartOptions{
		Stopper: stopper,
...
func (m *multiTestContext) heartbeatLiveness(ctx context.Context, store int) error {
	...
	for r := retry.StartWithCtx(ctx, retry.Options{MaxRetries: 5}); r.Next(); {
		if err = nl.Heartbeat(ctx, l); !errors.Is(err, nodeliveness.ErrEpochIncremented) {
			break
		}
	}
	return err
