...
func (sr *StoreRebalancer) rebalanceStore(ctx context.Context, rctx *RebalanceContext) {
	...
	//     replica will have have the lease transferred to it.
	...
	// balancing targets again.
	// TODO(kvoli): Consider retrying rebalancing, or changing our thresholds
	// when we were unsuccessful in meeting our goal. This would introduce a
	// behavior change.
	_ = sr.RebalanceStoreOucome(ctx, rctx)
}
...
// exceeds the threshold and NoRebalanceTarget if the threshold is not
// exceeded.
...
// RebalanceStoreOucome logs the outcome of rebalancing the store, returning
// true if successful and false if unsuccessful w.r.t the local stores QPS and
// threshold.
func (sr *StoreRebalancer) RebalanceStoreOucome(ctx context.Context, rctx *RebalanceContext) bool {
	// We failed rebalancing below the max threshold, failing our goal, QPS >
	// max threshold. Log the failure and return false.
	if rctx.LocalDesc.Capacity.QueriesPerSecond > rctx.QPSMaxThreshold {
		log.Infof(ctx,
	...
	// fulfilling our goal, QPS <= max threshold. Log the success and return
	// true.
	log.Infof(ctx,
...
// still exceeds the threshold and NoRebalanceTarget if the threshold is not
// exceeded.
...
func (sr *StoreRebalancer) chooseLeaseToTransfer(
	...
	for {
		...
		if !candidateReplica.OwnsValidLease(ctx, now) {
			log.VEventf(ctx, 3, "store doesn't own the lease for r%d", candidateReplica.GetRangeID())
			continue
		}
...
func (sr *StoreRebalancer) chooseRangeToRebalance(
	...
	if len(rctx.rebalanceCandidates) == 0 && len(rctx.hottestRanges) >= 0 {
		...
		// hottest ranges by the preceeding function call to this. Some
		// tests, that only call this function without transferring from
		// lease rebalancing first, won't populate rebalanceCandidates, so
		// instead we check if any hottestRanges exist and append them if
		// so.
		rctx.rebalanceCandidates = append(rctx.rebalanceCandidates, rctx.hottestRanges...)
	}
	...
	for {
		...
		if !foundRebalance {
			...
			log.VEventf(ctx, 3, "could not find rebalance opportunities for r%d", candidateReplica.GetRangeID())
			continue
		}
