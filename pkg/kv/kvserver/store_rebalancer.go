...
// TODO(kvoli): Make it such that CPU rebalancing doesn't work if there is more
// than 1 store with CPU=-1.

// RebalanceDimension returns the dimension the store rebalancer is balancing.
func (sr *StoreRebalancer) RebalanceObjective(ctx context.Context) LBRebalancingObjective {
	return sr.objectiveProvider.Objective(ctx)
}
...
func (sr *StoreRebalancer) Start(ctx context.Context, stopper *stop.Stopper) {
	...
	_ = stopper.RunAsyncTask(ctx, "store-rebalancer", func(ctx context.Context) {
		...
		for {
			...
			objective := sr.RebalanceObjective(ctx)
