	...
	for {
		...
		*hottestRanges = (*hottestRanges)[1:]
		...
		targetVoterRepls, targetNonVoterRepls := sr.getRebalanceCandidatesBasedOnQPS(
			ctx, replWithStats, localDesc, storeMap, storeList, minQPS, maxQPS,
		)
		...
		desiredNonVoters := GetNeededNonVoters(desiredVoters, int(zone.GetNumNonVoters()), clusterNodes)
		...
		if len(targetVoterRepls) < desiredVoters {
			log.VEventf(ctx, 3, "couldn't find enough voter rebalance targets for r%d (%d/%d)",
				rangeDesc.RangeID, len(targetVoterRepls), desiredVoters)
			continue
		}
		if len(targetNonVoterRepls) < desiredNonVoters {
			log.VEventf(ctx, 3, "couldn't find enough non-voter rebalance targets for r%d (%d/%d)",
				rangeDesc.RangeID, len(targetNonVoterRepls), desiredNonVoters)
			continue
		}
		...
		currentVoters := rangeDesc.Replicas().VoterDescriptors()
		currentNonVoters := rangeDesc.Replicas().NonVoterDescriptors()
		curVoterDiversity := rangeDiversityScore(
			sr.rq.allocator.storePool.getLocalitiesByStore(currentVoters),
		)
		newVoterDiversity := rangeDiversityScore(
			sr.rq.allocator.storePool.getLocalitiesByStore(targetVoterRepls),
		)
		if newVoterDiversity < curVoterDiversity {
			log.VEventf(
				ctx,
				3,
				"new voting replica diversity %.2f for r%d worse than current diversity %.2f; not rebalancing",
				newVoterDiversity,
				rangeDesc.RangeID,
				curVoterDiversity,
			)
		...
		allCurrentRepls := append(currentVoters, currentNonVoters...)
		allTargetRepls := append(targetVoterRepls, targetNonVoterRepls...)
		curReplicaDiversity := rangeDiversityScore(
			sr.rq.allocator.storePool.getLocalitiesByStore(allCurrentRepls),
		)
		newReplicaDiversity := rangeDiversityScore(
			sr.rq.allocator.storePool.getLocalitiesByStore(allTargetRepls),
		)
		if newReplicaDiversity < curReplicaDiversity {
			log.VEventf(ctx, 3,
				"new replica diversity %.2f for r%d worse than current diversity %.2f; not rebalancing",
				newReplicaDiversity, rangeDesc.RangeID, curReplicaDiversity)
			continue
		}
...
//
// NB: `localDesc` is expected to be the leaseholder of the range being operated
// on.
func (sr *StoreRebalancer) getRebalanceCandidatesBasedOnQPS(
...
) (targetVoters, targetNonVoters []roachpb.ReplicaDescriptor) {
	rangeDesc, zone := replWithStats.repl.DescAndZone()
	clusterNodes := sr.rq.allocator.storePool.ClusterNodeCount()
	...
	pickReplsToKeep := func(
		currentRepls []roachpb.ReplicaDescriptor, targetType targetReplicaType,
	) (targetRepls []roachpb.ReplicaDescriptor) {
		shouldExclude := func(repl roachpb.ReplicaDescriptor) bool {
			if targetType == nonVoterTarget {
				// If we're selecting the non-voter targets we want to keep, make sure
				// to disregard all voters. This is because a previous call to
				// `pickRemainingRepls` may have picked a store with a current non-voter
				// to rebalance to.
				for _, excluded := range targetVoters {
		...
		// Check the existing replicas, keeping around those that aren't
		// overloaded.
		for i := range currentRepls {
			if shouldExclude(currentRepls[i]) || currentRepls[i].StoreID == localDesc.StoreID {
				continue
			}
			...
			storeDesc, ok := storeMap[currentRepls[i].StoreID]
			if !ok || storeDesc.Capacity.QueriesPerSecond < maxQPS {
				if log.V(3) {
					...
					log.VEventf(
						ctx,
						3,
						"keeping r%d/%d on s%d%s",
						rangeDesc.RangeID,
						currentRepls[i].ReplicaID,
						currentRepls[i].StoreID,
						reason,
					)
				}
				targetRepls = append(targetRepls, roachpb.ReplicaDescriptor{
					NodeID:  currentRepls[i].NodeID,
					StoreID: currentRepls[i].StoreID,
				})
			}
		}
		return targetRepls
	...
	pickRemainingRepls := func(
		targetRepls *[]roachpb.ReplicaDescriptor, numDesiredRepls int, targetType targetReplicaType,
	) {
		options := sr.rq.allocator.scorerOptions()
		options.qpsRebalanceThreshold = qpsRebalanceThreshold.Get(&sr.st.SV)

		for len(*targetRepls) < numDesiredRepls {
			...
			if target == nil {
				log.VEventf(ctx, 3, "no rebalance %ss found to replace the current store for r%d",
					targetType, rangeDesc.RangeID)
				break
			}
	...
	targetVoters = pickReplsToKeep(currentVoters, voterTarget)
	pickRemainingRepls(&targetVoters, numDesiredVoters, voterTarget)
	// NB: `targetVoters` may contain replicas that are part of
	// `currentNonVoters`, so we make sure that we don't keep those replicas
	// around in `targetNonVoters`.
	targetNonVoters = pickReplsToKeep(currentNonVoters, nonVoterTarget)
	pickRemainingRepls(&targetNonVoters, numDesiredNonVoters, nonVoterTarget)

	return targetVoters, targetNonVoters
}
