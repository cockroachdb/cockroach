...
type RebalanceContext struct {
	...
	loadDimension                      state.LoadDimension
	maxThresholds                      state.Load
...
func (sr *StoreRebalancer) scorerOptions(ctx context.Context) *allocatorimpl.LoadScorerOptions {
	...
	return &allocatorimpl.LoadScorerOptions{
		...
		LoadDims:                     []state.LoadDimension{lbDimension},
...
func (sr *StoreRebalancer) PostLeaseRebalance(
	...
	rctx.LocalDesc.Capacity.QueriesPerSecond -= candidateReplica.RangeUsageInfo().QueriesPerSecond
	if otherDesc := rctx.storeMap[target.StoreID]; otherDesc != nil {
		otherDesc.Capacity.LeaseCount++
		otherDesc.Capacity.QueriesPerSecond += candidateReplica.RangeUsageInfo().QueriesPerSecond
	}
}
	...
	for i := range voterTargets {
		if storeDesc := rctx.storeMap[voterTargets[i].StoreID]; storeDesc != nil {
			...
			if i == 0 {
				...
				storeDesc.Capacity.QueriesPerSecond += candidateReplica.RangeUsageInfo().QueriesPerSecond
			}
		}
	}
}
...
func (sr *StoreRebalancer) chooseRangeToRebalance(
	...
	// log.Infof(ctx, "rctx %+v", *rctx)

	for {
		var candidateReplica CandidateReplica
		if len(rctx.rebalanceCandidates) == 0 {
			log.KvDistribution.Infof(ctx, "ran out of options")
			return candidateReplica, nil, nil
		}

		log.Infof(ctx, "options len %d", len(rctx.rebalanceCandidates))
		...
		log.Infof(ctx, "considering candidate %+v", candidateReplica.RangeUsageInfo())
