...
func splitPreApply(
	...
	// and rightRepl is either nil (though the replica may still be in the process
	// of being created concurrently), or an uninitialized replica with a higher
	// ReplicaID. Its raftMu is not locked.
	//
	// Note that in any case, the RHS, if exists, is uninitialized. Such replicas
	// may only have non-empty RaftReplicaID and RaftHardState keys in storage.
	...
	if rightRepl == nil || rightRepl.isNewerThanSplit(&split) {
		// We're in the rare case where we know that the RHS has been removed and
		// re-added with a higher replica ID (and then maybe removed again).
		//
		...
		// because we might have already voted at a higher term. In general this
		// shouldn't happen because we add learners and then promote them only after
		// they apply a snapshot, but we're going to be extra careful in case future
		// versions of cockroach somehow promote replicas without ensuring that a
		// snapshot has been received.
		//
		// NB: the rightRepl == nil condition is flaky, in a sense that the RHS
		// replica can be created concurrently here. But we only use it for an
		// assertion, so this is not critical at the moment.
		if rightRepl != nil {
