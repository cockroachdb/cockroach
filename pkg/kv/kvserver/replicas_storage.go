...
// TODO(sumeer):
// Implementation:
// x Initial interface implementation without applying to state machine in
//   Init.
// - Unit tests and randomized tests, including engine restarts that lose
//   some state (using vfs.NewStrictMem).
// - Improved implementation: concurrency control; complete Init impl etc.
// - Ensure range keys are being properly handled, i.e., they need to be
//   cleared.
// - Optimize implementation: there are places the implementation simply used
//   range tombstones instead of the heuristic of writing point deletes if the
//   number of points is small.
// - Benchmarks comparing single and two engine implementations: this is
//   already done in
...
// This is discovered by looking at the RangeTombstone.
//
...
// TODO(sumeer): create issue for this, since we can do this for 23.1 since
// RaftAppliedIndexTerm was introduced in 22.1.
//
// ============================================================================
// Replica Deletion:
//
...
//     Range-ID local keys -- this iteration can be accomplished by seeking using
...
type RangeStorage interface {
	...
	// been done. This is breaking the abstraction, in that the caller has
	// changed the raft log and removed sideloaded entries without going through
	// RangeStorage, so we should see if we can do better here. The current use
	// cases are:
	...
	//   supposed to only touch the state machine.
	...
	IngestRangeSnapshot(
		...
		sstScratch *SSTSnapshotStorageScratch) error
...
type sideloadedStorageConstructor func(
	rangeID roachpb.RangeID, replicaID roachpb.ReplicaID) SideloadStorage
...
func MakeSingleEngineReplicasStorage(
	...
	reps.mu.replicasMap = make(map[roachpb.RangeID]*rangeStorageImpl)
	return reps
}

// rangeStorageImpl is an implementation of RangeStorage.
type rangeStorageImpl struct {
	id   FullReplicaID
	reps *replicasStorageImpl
	mu   struct {
		syncutil.RWMutex
		state ReplicaState
		// span is only populated for InitializedStateMachine. span.EndKey changes
		// when the replica is split or merged. span.Key never changes.
		span             roachpb.RSpan
		lastSyncedCommit uint64
		lastCommit       uint64
	}
...
var _ RangeStorage = &rangeStorageImpl{}

func makeRangeStorageImpl(
	id FullReplicaID, reps *replicasStorageImpl, ssConstructor sideloadedStorageConstructor,
) *rangeStorageImpl {
	return &rangeStorageImpl{
		id:   id,
		reps: reps,
		rsl:  stateloader.Make(id.RangeID),
		ss:   ssConstructor(id.RangeID, id.ReplicaID),
	}
}

func (r *rangeStorageImpl) FullReplicaID() FullReplicaID {
	return r.id
}

func (r *rangeStorageImpl) State() ReplicaState {
	r.mu.RLock()
	defer r.mu.RUnlock()
	return r.mu.state
}

func (r *rangeStorageImpl) CurrentRaftEntriesRange(
	ctx context.Context,
) (lo uint64, hi uint64, err error) {
	r.mu.RLock()
	defer r.mu.RUnlock()

	reader := r.reps.eng.NewReadOnly(storage.StandardDurability)
	defer reader.Close()
	truncState, err := r.rsl.LoadRaftTruncatedState(ctx, reader)
	if err != nil {
		return 0, 0, err
	}
	lastRaftIndex, hasEntry, err := r.rsl.LoadLastIndexWithLogEntry(ctx, reader)
	if err != nil {
		return 0, 0, err
	}
	if !hasEntry {
	...
	if truncState.Index >= lastRaftIndex {
		// Found entry that is expected to be truncated.
		return 0, 0, errors.Errorf(
			"truncState.Index %d >= lastRaftIndex %d", truncState.Index, lastRaftIndex)
	}
	return truncState.Index + 1, lastRaftIndex + 1, nil
}

func (r *rangeStorageImpl) GetHardState(ctx context.Context) (raftpb.HardState, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()

	reader := r.reps.eng.NewReadOnly(storage.GuaranteedDurability)
	defer reader.Close()
	return r.rsl.LoadHardState(ctx, reader)
}

func (r *rangeStorageImpl) TruncatedRaftLog(index uint64) {
	// Noop since rangeStorageImpl does not have in-memory truncation state.
}

func (r *rangeStorageImpl) DoRaftMutation(ctx context.Context, rBatch RaftMutationBatch) error {
	r.mu.Lock()
	defer r.mu.Unlock()

	var sideloadedDeleteFunc func() error
	if rBatch.Lo < rBatch.Hi {
		if r.mu.state != InitializedStateMachine {
			return errors.Errorf("cannot add raft log entries to a !InitializedStateMachine replica ")
		}
		if !rBatch.MustSync {
			return errors.Errorf("must not append raft log entries without syncing")
		}

		var err error
		sideloadedDeleteFunc, err = func() (func() error, error) {
			batch := rBatch.Batch()
			lastRaftIndex, hasEntry, err := r.rsl.LoadLastIndexWithLogEntry(ctx, batch)
			if err != nil {
				return nil, err
			}
			// TODO(sumeer):
			// - stats delta needs to be plumbed back.
			// - Replica.append in replica_raftstorage.go has slightly different logic
			//   where instead of doing MVCCDelete for parts that it is over-writing,
			//   and then doing a write, it does a MVCCPut. And does a MVCCBlindPut for
			//   parts that it is not over-writing. If we had log entries [5,11) and we
			//   are adding [5,8) we can do such logic for [5,8), but we also need to
			//   delete [9,11).
			var ms enginepb.MVCCStats
			var sdFunc func() error
			if hasEntry {
				// Clear [hi, lastRaftIndex]. If there are existing entries in [lo,
				// hi) they are already being overwritten in the batch. Though we will
				// need to compute stats corresponding to their deletion.
				for i := rBatch.Hi; i <= lastRaftIndex; i++ {
					if _, err := storage.MVCCDelete(
						ctx, batch, &ms, r.rsl.RaftLogKey(i), hlc.Timestamp{}, hlc.ClockTimestamp{},
						nil /* txn */); err != nil {
						return nil, err
					}
				}
				sdFunc = func() error {
					for i := rBatch.Lo; i <= lastRaftIndex; i++ {
						if _, err := r.ss.Purge(ctx, i, rBatch.Term-1); err != nil && !errors.Is(err, errSideloadedFileNotFound) {
							return err
						}
					}
					return nil
	...
	if rBatch.HardState != nil {
		if rBatch.HardState.Commit < r.mu.lastCommit {
			return errors.Errorf("Commit value must not regress")
		}
	}
	// Commit the batch.
	if err := rBatch.Commit(rBatch.MustSync); err != nil {
		return err
	}
	if rBatch.HardState != nil {
		r.mu.lastCommit = rBatch.HardState.Commit
		if rBatch.MustSync {
			r.mu.lastSyncedCommit = rBatch.HardState.Commit
		}
	}
	if sideloadedDeleteFunc != nil {
		return sideloadedDeleteFunc()
	}
	return nil
}

func unionSortedSpans(a []roachpb.RSpan, b []roachpb.RSpan) []roachpb.RSpan {
	var result []roachpb.RSpan
	addToResult := func(x roachpb.RSpan) {
		n := len(result)
		if n > 0 && result[n-1].EndKey.Compare(x.Key) >= 0 {
			// Overlap or adjacent, so merge.
			if result[n-1].EndKey.Compare(x.EndKey) >= 0 {
				// Nothing to merge.
				return
			}
			result[n-1].EndKey = x.EndKey
			return
		}
		result = append(result, x)
	}
	for i, j, n, m := 0, 0, len(a), len(b); i < n || j < m; {
		if i < n && (j == m || a[i].Key.Compare(b[j].Key) <= 0) {
			addToResult(a[i])
			i++
		} else {
			addToResult(b[j])
...
func (r *rangeStorageImpl) IngestRangeSnapshot(
	ctx context.Context,
	span roachpb.RSpan,
	...
	r.reps.mu.Lock()
	defer r.reps.mu.Unlock()

	reader := r.reps.eng.NewReadOnly(storage.StandardDurability)
	defer reader.Close()
	lastRaftIndex, _, err := r.rsl.LoadLastIndexWithLogEntry(ctx, reader)
	if err != nil {
		return err
	}
	if lastRaftIndex > raftAppliedIndex {
		// etcd/raft is expected to maintain this invariant, so this error should
		// not occur in practice.
		return errors.Errorf("snapshot with applied index %d < index of latest raft entry %d",
			raftAppliedIndex, lastRaftIndex)
	}
	subsumedRanges, extraRangeSpansToDelete, err :=
		r.reps.checkNoAdditionalOverlap(span, r.id.RangeID, subsumedRangeIDs)
	if err != nil {
		return err
	}
	if r.mu.state == InitializedStateMachine {
		// We may be moving past splits with this ingest, so there can be
		// additional spans to clear (fixes #73462).
		additionalRangeSpansToDelete := diffSpans(r.mu.span, span)
		extraRangeSpansToDelete = unionSortedSpans(extraRangeSpansToDelete, additionalRangeSpansToDelete)
	}

	// Add ssts to clear range local and global keys for these spans.
	for i := range extraRangeSpansToDelete {
		ranges := keyRangesForStateMachineExceptRangeIDKeys(extraRangeSpansToDelete[i])
		for j := range ranges {
			path, err := makeSSTToDeleteRange(ctx, ranges[j], sstScratch, r.reps.st)
			if err != nil {
				return err
			}
			if len(path) > 0 {
	...
	// Add ssts that clear replicated range-ID keys for the subsumed ranges and write
	// RangeTombstone.
	for i := range subsumedRangeIDs {
		keyRange := rditer.MakeRangeIDLocalKeySpan(subsumedRangeIDs[i], true)
		path, err := makeSSTToDeleteRange(ctx, keyRange, sstScratch, r.reps.st)
		if err != nil {
			return err
		}
		if len(path) > 0 {
		...
		path, err = makeSSTForRangeTombstoneForMergedRange(ctx, subsumedRangeIDs[i], sstScratch, r.reps.st)
		if err != nil {
			return err
		}
		sstPaths = append(sstPaths, path)
	...
	// Ingest the ssts.
	if err := r.reps.eng.IngestExternalFiles(ctx, sstPaths); err != nil {
		return err
	}
	batch := r.reps.eng.NewUnindexedBatch(false)
	defer batch.Close()
	var sideloadedDeleteFuncs []func() error
	// Step C3 for this range.
	sideloadedDeleteFunc, err := makeRaftConsistentWithStateMachineHelper(
		ctx, batch, r.rsl, r.ss)
	if err != nil {
		return err
	}
	if sideloadedDeleteFunc != nil {
		sideloadedDeleteFuncs = append(sideloadedDeleteFuncs, sideloadedDeleteFunc)
	}
	// Step D2 for all subsumed ranges.
	for i := range subsumedRanges {
		sideloadedDeleteFunc, err := clearRaftStateHelper(
			ctx, batch, subsumedRanges[i].rsl, subsumedRanges[i].ss)
		if err != nil {
			return err
		}
		if sideloadedDeleteFunc != nil {
	...
	// Commit the batch containing raft changes for steps C3 and D2. Neither
	// step needs syncing, so the batch is not synced.
	if err := batch.Commit(false); err != nil {
		err = errors.Wrapf(err, "failed to do steps C3 and D2")
		log.Fatalf(ctx, "%v", err)
		return err
	}
	// Update the in-memory data-structures.
	for i := range subsumedRanges {
		if err := r.reps.removeFromMaps(subsumedRanges[i]); err != nil {
			log.Fatalf(ctx, "%v", err)
			return err
		}
		subsumedRanges[i].mu.Lock()
		subsumedRanges[i].mu.state = DeletedReplica
		subsumedRanges[i].mu.Unlock()
	}
	// Remove the sideloaded files.
	for i := range sideloadedDeleteFuncs {
		if err := sideloadedDeleteFuncs[i](); err != nil {
			return err
		}
	}
	r.mu.Lock()
	...
	if r.mu.state == UninitializedStateMachine {
		r.mu.state = InitializedStateMachine
		if err := r.reps.addInitializedRangeToMap(r); err != nil {
			log.Fatalf(ctx, "%v", err)
		}
	}
	r.mu.Unlock()
...
func diffSpans(x roachpb.RSpan, y roachpb.RSpan) []roachpb.RSpan {
	var spans []roachpb.RSpan
	if x.Key.Compare(y.Key) < 0 {
		endKey := y.Key
		noOverlap := false
		if endKey.Compare(x.EndKey) > 0 {
			noOverlap = true
			endKey = x.EndKey
		}
	...
	if x.EndKey.Compare(y.EndKey) > 0 {
		startKey := y.EndKey
		if startKey.Compare(x.Key) < 0 {
			startKey = x.Key
		}
		spans = append(spans, roachpb.RSpan{
			Key:    startKey,
...
func (r *rangeStorageImpl) ApplyCommittedUsingIngest(
	ctx context.Context, sstPaths []string, highestRaftIndex uint64,
) error {
	r.mu.Lock()
	defer r.mu.Unlock()

	if r.mu.state != InitializedStateMachine {
		return errors.Errorf("cannot apply to state machine for a !InitializedStateMachine replica ")
	}
	if highestRaftIndex > r.mu.lastCommit {
		return errors.Errorf(
			"trying to apply at raft index %d while highest committed index is %d",
			highestRaftIndex, r.mu.lastCommit)
	}
	if highestRaftIndex > r.mu.lastSyncedCommit {
		if err := storage.WriteSyncNoop(r.reps.eng); err != nil {
			return err
		}
		r.mu.lastSyncedCommit = highestRaftIndex
	}
	// TODO(sumeer): caller probably wants IngestOperationStats, so change
	// interface and call IngestExternalFilesWithStats.
	return r.reps.eng.IngestExternalFiles(ctx, sstPaths)
}

func (r *rangeStorageImpl) ApplyCommittedBatch(smBatch MutationBatch) error {
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	return smBatch.Commit(false)
}

func (r *rangeStorageImpl) SyncStateMachine(ctx context.Context) error {
	return storage.WriteSyncNoop(r.reps.eng)
}

// replicasStorageImpl is an implementation of ReplicasStorage.
//
// It assumes that the caller of methods on this implementation is not trying
// to concurrently write (or concurrently read-write) to the same Range-ID or
// roachpb.RSpans (including the case where spans that are being extended or
// shortened could partially overlap).
// TODO(sumeer):
//   - revisit this concurrency control assumption, since this is just a
//     temporary assumption to get all the logic done without worrying about
//     fine-grained locking.
//   - the code below reads rangeStorageImpl.mu.{span,state} without any locking
//     of rangeStorageImpl.mu, since those mutations also hold
//     replicasStorageImpl.mu. Revisit this scheme.
type replicasStorageImpl struct {
	nodeID        roachpb.NodeID
	storeID       roachpb.StoreID
	eng           storage.Engine
	ssConstructor sideloadedStorageConstructor
	...
	mu            struct {
		syncutil.RWMutex
		replicasMap map[roachpb.RangeID]*rangeStorageImpl
		// The subset of replicas in replicasMap that are in state
		// InitializedStateMachine are in replicasSpans in span order. The spans
		// do not overlap.
		// TODO(sumeer): switch to using a btree.
		replicasSpans []*rangeStorageImpl
	}
}

var _ ReplicasStorage = &replicasStorageImpl{}
...
func decodeProtoFromValue(ts hlc.Timestamp, rawValue []byte, msg protoutil.Message) error {
	if ts.IsEmpty() {
		var meta enginepb.MVCCMetadata
		err := protoutil.Unmarshal(rawValue, &meta)
		if err != nil {
			return err
		}
		rawValue = meta.RawBytes
	} else {
...
func (reps *replicasStorageImpl) Init(ctx context.Context) error {
	reader := reps.eng.NewReadOnly(storage.StandardDurability)
	defer reader.Close()

	// raftRangeIDs represents the set of ranges R_r, that have raft state.
	// replicatedRangeIDs represents the set of ranges that have a state
	// machine.
	var raftRangeIDs, replicatedRangeIDs []roachpb.RangeID
	err := func() error {
		nextRangeID := roachpb.RangeID(0)
		// Since the raft state and state machine state are in the same engine,
		// and the unreplicated and replicated range-ID keys for the same range are
		// adjacent to each other, we do a single coordinated iteration.
		//
		// Unreplicated range-ID local keys come after the replicated ones, so we
		// set the upper bound after the unreplicated HardState.
		iter := reader.NewMVCCIterator(storage.MVCCKeyIterKind, storage.IterOptions{
			UpperBound: keys.RaftTruncatedStateKey(math.MaxInt64)})
		defer iter.Close()
		done := false
		for !done {
			seekKey := keys.RangeAppliedStateKey(nextRangeID)
			iter.SeekGE(storage.MVCCKey{Key: seekKey})
			for {
				...
				rangeID, infix, suffix, _, err := keys.DecodeRangeIDKey(iter.UnsafeKey().Key)
				if err != nil {
					return err
				}
				if bytes.Equal(infix, keys.LocalRangeIDReplicatedInfix) {
					replicatedRangeIDs = append(replicatedRangeIDs, rangeID)
					nextRangeID = rangeID
					seekKey = keys.RaftHardStateKey(nextRangeID)
					iter.SeekGE(storage.MVCCKey{Key: seekKey})
					continue
				} else {
					// Unreplicated key
					if bytes.Equal(keys.LocalRangeTombstoneSuffix, suffix) {
						nextRangeID = rangeID
						seekKey = keys.RaftHardStateKey(nextRangeID)
						iter.SeekGE(storage.MVCCKey{Key: seekKey})
						continue
					}
					raftRangeIDs = append(raftRangeIDs, rangeID)
	...
	err = func() error {
		lower := keys.RangeDescriptorKey(roachpb.RKeyMin)
		upper := keys.RangeDescriptorKey(roachpb.RKeyMax)
		iter := reader.NewMVCCIterator(storage.MVCCKeyAndIntentsIterKind, storage.IterOptions{
			UpperBound: upper})
		defer iter.Close()
		iter.SeekGE(storage.MVCCKey{Key: lower})
		valid, err := iter.Valid()
		for valid && err == nil {
			var suffix roachpb.Key
			key := iter.UnsafeKey()
			...
			if bytes.Equal(suffix, keys.LocalRangeDescriptorSuffix) {
				if !key.IsValue() {
					savedKey := iter.Key()
					// Next one should be provisional value.
					iter.Next()
					if valid, err = iter.Valid(); err != nil {
						return err
					}
					if !valid || iter.UnsafeKey().Key.Equal(savedKey.Key) {
						return errors.Errorf("RangeDescriptor intent with no provisional value")
					}
					// Next one should be a committed range descriptor
					iter.Next()
					if valid, err = iter.Valid(); err != nil {
						return err
					}
					if !valid || iter.UnsafeKey().Key.Equal(savedKey.Key) {
						return errors.Errorf("no committed RangeDescriptor")
					}
				}
				// Committed RangeDescriptor.
				var desc roachpb.RangeDescriptor
				if err := decodeProtoFromValue(iter.UnsafeKey().Timestamp, iter.UnsafeValue(), &desc); err != nil {
					return err
				}
				rInfo := rangeInfo{
					id: FullReplicaID{
						RangeID: desc.GetRangeID(),
					},
					span: desc.RSpan(),
				}
				found := false
				for i := range desc.InternalReplicas {
					if desc.InternalReplicas[i].NodeID == reps.nodeID &&
						desc.InternalReplicas[i].StoreID == reps.storeID {
						found = true
						rInfo.id.ReplicaID = desc.InternalReplicas[i].ReplicaID
						break
					}
				}
				if !found {
					return errors.Errorf("stored RangeDescriptor does not mention replica for this store")
				}
				rangeInfos = append(rangeInfos, rInfo)
				iter.SeekGE(storage.MVCCKey{Key: keys.RangeDescriptorKey(rInfo.span.EndKey)})
			} else {
				// Step until come to a RangeDescriptor.
				iter.Next()
			}
			valid, err = iter.Valid() //lint:ignore SA4006 this value of err is never used
		}
		if err != nil {
			return err
		}
	...
	// Validate that rangeInfos and replicatedRangeIDs contain the same
	// RangeIDs. These are the initialized ranges.
	sort.Slice(rangeInfos, func(i, j int) bool {
		return rangeInfos[i].id.RangeID < rangeInfos[j].id.RangeID
	})
	{
		i := 0
		for n, m := len(rangeInfos), len(replicatedRangeIDs); i < n || i < m; i++ {
			if i < n && (i >= m || rangeInfos[i].id.RangeID < replicatedRangeIDs[i]) {
				return errors.Errorf("rangeID %d has RangeDescriptor but no replicated range-id keys",
					rangeInfos[i].id.RangeID)
			} else if i < m && (i >= n || rangeInfos[i].id.RangeID > replicatedRangeIDs[i]) {
				return errors.Errorf("rangeID %d has no RangeDescriptor but has replicated range-id key",
					rangeInfos[i].id.RangeID)
			}
		}
	}

	// Validate that the set R_s - R_r is empty, i.e., R_s is a subset of R_r.
	// And compute R_r - R_s. These replicas are either in state
	// UninitializedStateMachine or RecoveryDeletingReplica.
	var rangeIDsWithNoStateMachine []roachpb.RangeID
	{
		i := 0
		j := 0
		for n, m := len(rangeInfos), len(raftRangeIDs); i < n || j < m; {
			if i < n && (j >= m || rangeInfos[i].id.RangeID < raftRangeIDs[j]) {
				return errors.Errorf("rangeID %d has RangeDescriptor but no raft state",
					rangeInfos[i].id.RangeID)
			} else if j < m && (i >= n || rangeInfos[i].id.RangeID > raftRangeIDs[j]) {
				rangeIDsWithNoStateMachine = append(rangeIDsWithNoStateMachine, raftRangeIDs[j])
				j++
			} else {
				i++
				j++
	...
	// Remove RecoveryDeletingReplica replicas by transitioning them to
	// DeletedReplica by executing D2. These are distinguishable from
	// UninitializedStateMachine since RaftTruncatedState.{Index,Term} are
	// guaranteed to exist and have values > 0.
	if n := len(rangeIDsWithNoStateMachine); n > 0 {
		err := func() error {
			iter := reader.NewMVCCIterator(storage.MVCCKeyIterKind, storage.IterOptions{
				UpperBound: encoding.BytesNext(keys.RaftTruncatedStateKey(
					rangeIDsWithNoStateMachine[n-1])),
			})
			defer iter.Close()
			for i := 0; i < n; i++ {
				rangeID := rangeIDsWithNoStateMachine[i]
				rsl := stateloader.Make(rangeID)
				foundTombstone, rangeTombstone, err := getRangeTombstone(ctx, reader, rsl)
				if err != nil {
					return err
				}
				replicaID, foundReplicaID, err := rsl.LoadRaftReplicaID(ctx, reader)
				if err != nil {
					return err
				}
				if !foundReplicaID {
					return errors.Errorf("did not find RaftReplicaID for range %v with raft state",
						rangeID)
				}
				var truncState roachpb.RaftTruncatedState
				foundTruncState, err := storage.MVCCGetProto(
					ctx, reader, rsl.RaftTruncatedStateKey(), hlc.Timestamp{}, &truncState, storage.MVCCGetOptions{})
				if err != nil {
					return err
				}
				// The second condition is sufficient.
				if (foundTruncState && truncState.Index > 0 && truncState.Term > 0) ||
					(foundTombstone && replicaID.ReplicaID < rangeTombstone.NextReplicaID) {
					if err := clearRaftState(ctx, reps.eng, stateloader.Make(rangeID),
						reps.ssConstructor(rangeID, replicaID.ReplicaID)); err != nil {
						return err
					}
					continue
				}
				// Not deleted.
				rsi := makeRangeStorageImpl(FullReplicaID{
					RangeID:   rangeID,
					ReplicaID: replicaID.ReplicaID,
				}, reps, reps.ssConstructor)
				rsi.mu.state = UninitializedStateMachine
				reps.mu.replicasMap[rangeID] = rsi
			}
			return nil
		}()
		if err != nil {
	...
	// InitializedStateMachine replicas: check that the spans do not overlap.
	sort.Slice(rangeInfos, func(i, j int) bool {
		return rangeInfos[i].span.Key.Less(rangeInfos[j].span.Key)
	})
	for i := 1; i < len(rangeInfos); i++ {
		if rangeInfos[i-1].span.EndKey.Compare(rangeInfos[i].span.Key) > 0 {
			// Overlapping spans.
			return errors.Errorf("ranges %v and %v have overlapping spans %v, %v",
				rangeInfos[i-1].id.RangeID, rangeInfos[i].id.RangeID, rangeInfos[i-1].span,
				rangeInfos[i].span)
		}
	}
	// Handle RecoveryInconsistentReplicas: This is a Replica that mostly looks
	// like to be in state InitializedStateMachine, but has suffered regression
	// in durable state such that the state machine has advanced past
	// HardState.Commit, or a snapshot has been applied and all raft log entries
	// are < RaftAppliedIndex, i.e., it violates RaftAndStateConsistency
	// invariants. More severely, it can also violate StateConsistency
	// invariants by having durably ingested SSTables but not yet updated the
	// RaftAppliedIndex to reflect that state machine change. Restores all the
	// invariants needed by an InitializedStateMachine replica, by fixing the
	// raft log to be consistent with the state machine, and re-applying log
	// entries up to HardState.Commit (except for log entries that indicate a
	// split or merge -- see below).
	for i := 0; i < len(rangeInfos); i++ {
		rsi := makeRangeStorageImpl(rangeInfos[i].id, reps, reps.ssConstructor)
		rsi.mu.state = InitializedStateMachine
		rsi.mu.span = rangeInfos[i].span
		reps.mu.replicasMap[rsi.id.RangeID] = rsi
		reps.mu.replicasSpans = append(reps.mu.replicasSpans, rsi)
		err := func(rsi *rangeStorageImpl) error {
			batch := reps.eng.NewUnindexedBatch(false)
			defer batch.Close()
			hs, err := rsi.rsl.LoadHardState(ctx, batch)
			if err != nil {
				return err
			}
			as, err := rsi.rsl.LoadRangeAppliedState(ctx, batch)
			if err != nil {
				return err
			}

			// If RangeAppliedState.RaftAppliedIndex > HardState.Commit (NB:
			// HardState must exist), execute the following atomically:
			// -  If there are no log entries or all log entries are <
			//   RaftAppliedIndex: remove all log entries and set
			//   RaftTruncatedState.{Index,Term} equal to
			//   RangeAppliedState.{RaftAppliedIndex,RaftAppliedIndexTerm}.
			//  - Set HardState.Commit to RaftAppliedIndex.
			//  These steps handle (a) crash in the middle of replica creation
			//  (doing step C3), and (b) regression of HardState.Commit under normal
			//  replica operation. The RaftAndStateConsistency invariant now holds.
			if hs.Commit >= as.RaftAppliedIndex {
				rsi.mu.lastCommit = hs.Commit
				return nil
			}
			// INVARIANT: hs.Commit < as.RaftAppliedIndex
			hs.Commit = as.RaftAppliedIndex
			rsi.mu.lastCommit = hs.Commit
			if err := rsi.rsl.SetHardState(ctx, batch, hs); err != nil {
				return err
			}
			truncState, err := rsi.rsl.LoadRaftTruncatedState(ctx, batch)
			if err != nil {
				return err
			}
			lastRaftIndex, hasEntry, err := rsi.rsl.LoadLastIndexWithLogEntry(ctx, batch)
			if err != nil {
				return err
			}
			if !hasEntry || lastRaftIndex < as.RaftAppliedIndex {
				if hasEntry {
					// Delete all raft log entries. Callee is going to again load
					// lastRaftIndex, which is wasteful.
					sideloadedDeleteFunc, err := clearRaftLogEntries(ctx, batch, rsi.rsl, truncState, rsi.ss)
					if err != nil {
						return err
					}
					// The log truncation is not durable, but a crash now will ensure that Init
					// will truncate the log. So we don't need the sideloaded entries even if
					// the log entries come back.
					if err := sideloadedDeleteFunc(); err != nil {
						return err
					}
				}
				truncState.Index = as.RaftAppliedIndex
				truncState.Term = as.RaftAppliedIndexTerm
				if err := rsi.rsl.SetRaftTruncatedState(ctx, batch, &truncState); err != nil {
					return err
				}
			}
			if err := batch.Commit(false); err != nil {
		...
		// apply log entries, including those that remove this replica, until one
		// encounters a log entry that is performing a split or merge.
		// TODO(sumeer): Following is just rough sketching of the code based on
		// copy-pasting etc., which I did for my understanding. It doesn't really
		// represent what we really want, which is the refactor outlined in
		// https://github.com/cockroachdb/cockroach/issues/75729

		// var sideloaded SideloadStorage
		// sideloaded, err =
		// 	newDiskSideloadStorage(nil /*TODO*/, rsi.id.RangeID, rsi.id.ReplicaID,
		// 		"" /*TODO*/, nil, reps.eng)
		err = func() error {
			batch := reps.eng.NewUnindexedBatch(false)
			defer batch.Close()
			...
			if hs.Commit < as.RaftAppliedIndex {
				// Should have fixed this earlier in Init.
				panic("")
			}
			/*
				// hs.Commit > as.RaftAppliedIndex. Need to apply entries.
				for i := as.RaftAppliedIndex + 1; i <= hs.Commit; i++ {
					// Load log entry i
					committedEntries[]
					raftpb.Entry
				}
				...
				for ; ; iter.Next() {
					...
					if sniffSideloadedRaftCommand(ent.Data) {
						newEnt, err := maybeInlineSideloadedRaftCommand(
							ctx, rsi.id.RangeID, ent, sideloaded, nil)
						if err != nil {
							return err
						}
						if newEnt != nil {
					...
					// If we want to do size accounting, which we don't
					size += uint64(ent.Size())

					// Now we need to use replicaDecoder.DecodeAndBind. No decode is sufficient since
					// don't need to retrieve local proposals
					cmd := &replicatedCmd{}
					if err := cmd.decode(ctx, &ent); err != nil {
						return err
					}
					// See Task.applyOneBatch
					cmd.IsTrivial()
					// See replicaStateMachine. It has a Replica which we don't.
					// Directly use a replicaAppBatch? That also has a Replica and a replicaStateMachine.
					batch = reps.eng.NewBatch()
					defer batch.Close()
					// Look at replicaAppBatch.Stage that converts to a CheckedCommand.
					if cmd.ent.Index == 0 {
						return makeNonDeterministicFailure("processRaftCommand requires a non-zero index")
					}
					if idx, applied := cmd.ent.Index, b.state.RaftAppliedIndex; idx != applied+1 {
						// If we have an out of order index, there's corruption. No sense in
						// trying to update anything or running the command. Simply return.
						return makeNonDeterministicFailure("applied index jumped from %d to %d", applied, idx)
					}
					// Replica.shouldApplyCommand and checkForcedErr. Where do we get a *kvserverpb.ReplicaState
					// checkForcedErr is stateless -- it does not need a Replica.
					// We can use StateLoader.Load to load a ReplicaState. How will we keep it up to date when
					// applying?**

					if cmd.raftCmd.ReplicatedEvalResult.Split != nil || cmd.raftCmd.ReplicatedEvalResult.Merge != nil {
						// Stop here.
					}
					// replicaAppBatch.Stage should not be copied. We need to refactor it.
					// replicaAppBatch.ApplyToStateMachine

					// Also see replicaStateMachine.handleNonTrivialReplicatedEvalResult
					// for code that updates the lease etc. that is needed to have the latest
					// ReplicaState. It is called by replicaStateMachine.ApplySideEffects.
			*/
			return nil
		}()
		if err != nil {
...
func (reps *replicasStorageImpl) CurrentRanges() []ReplicaInfo {
	reps.mu.RLock()
	defer reps.mu.RUnlock()
	replicas := make([]ReplicaInfo, len(reps.mu.replicasMap))
	i := 0
	for _, r := range reps.mu.replicasMap {
		replicas[i].FullReplicaID = r.id
		replicas[i].State = r.mu.state
		i++
	}
	return replicas
}

func (reps *replicasStorageImpl) GetRangeTombstone(
	ctx context.Context, rangeID roachpb.RangeID,
) (nextReplicaID roachpb.ReplicaID, err error) {
	reader := reps.eng.NewReadOnly(storage.StandardDurability)
	defer reader.Close()
	_, tombstone, err := getRangeTombstone(ctx, reader, stateloader.Make(rangeID))
	if err != nil {
		return roachpb.ReplicaID(0), err
	}
	return tombstone.NextReplicaID, nil
}

func (reps *replicasStorageImpl) GetHandle(rr FullReplicaID) (RangeStorage, error) {
	reps.mu.RLock()
	defer reps.mu.RUnlock()
	v := reps.mu.replicasMap[rr.RangeID]
	if v.id.ReplicaID != rr.ReplicaID {
		v = nil
	}
	return v, nil
}

func (reps *replicasStorageImpl) CreateUninitializedRange(
	ctx context.Context, rr FullReplicaID,
) (RangeStorage, error) {
	// Initialize rangeStorageImpl and install if it does not already exist.
	r := makeRangeStorageImpl(rr, reps, reps.ssConstructor)
	rsl := r.rsl
	r.mu.state = UninitializedStateMachine
	reps.mu.Lock()
	v := reps.mu.replicasMap[rr.RangeID]
	var err error
	if v != nil {
		r = nil
		err = errors.Errorf("range %v already exists", rr.RangeID)
	} else {
		reps.mu.replicasMap[rr.RangeID] = r
	}
	reps.mu.Unlock()
	if err != nil {
		return nil, err
	}
	removeFromMapDueToErr := func() {
		reps.mu.Lock()
		defer reps.mu.Unlock()
		v := reps.mu.replicasMap[rr.RangeID]
		if v == r {
			delete(reps.mu.replicasMap, rr.RangeID)
		}
		r = nil
	}
	reader := reps.eng.NewReadOnly(storage.StandardDurability)
	...
	if err != nil {
		removeFromMapDueToErr()
		return nil, err
	}
	if !raft.IsEmptyHardState(hs) {
		removeFromMapDueToErr()
		return nil, errors.Errorf("HardState for range %v already exists", rr.RangeID)
	}
	// Ensure RangeTombstone does not block this replica.
	found, tombstone, err := getRangeTombstone(ctx, reader, rsl)
	if err != nil {
		removeFromMapDueToErr()
		return nil, err
	}
	if found && rr.ReplicaID < tombstone.NextReplicaID {
		removeFromMapDueToErr()
		return nil, &roachpb.RaftGroupDeletedError{}
	}
	// Write HardState.
	if err := createHardStateAndReplicaID(ctx, reps.eng, rr, rsl); err != nil {
		removeFromMapDueToErr()
		return nil, err
	}
	return r, err
...
func (reps *replicasStorageImpl) SplitReplica(
	ctx context.Context,
	r RangeStorage,
	rhsRR FullReplicaID,
	...
	reps.mu.Lock()
	defer reps.mu.Unlock()
	rs := r.(*rangeStorageImpl)
	rsInMap := reps.mu.replicasMap[rs.id.RangeID]
	if rs != rsInMap {
		return nil, errors.Errorf(
			"provided RangeStorage is not current for this range %v", rs.id.RangeID)
	}
	if rs.mu.state != InitializedStateMachine {
		return nil, errors.Errorf(
			"range %v with replica %v being split is not initialized", rs.id.RangeID, rs.id.ReplicaID)
	}
	reader := reps.eng.NewReadOnly(storage.StandardDurability)
	defer reader.Close()
	rhsRSL := stateloader.Make(rhsRR.RangeID)
	_, tombstone, err := getRangeTombstone(ctx, reader, rhsRSL)
	if err != nil {
		return nil, err
	}
	var hs raftpb.HardState
	foundHS, err := storage.MVCCGetProto(ctx, reader, rhsRSL.RaftHardStateKey(),
		hlc.Timestamp{}, &hs, storage.MVCCGetOptions{})
	if err != nil {
		return nil, err
	}
	var rhsRS *rangeStorageImpl
	doStepC3 := false
	if tombstone.NextReplicaID > rhsRR.ReplicaID {
		// The replica has been removed.
		var ok bool
		rhsRS, ok = reps.mu.replicasMap[rhsRR.RangeID]
		if foundHS {
			// Case A11: RHS has been added back with a later ReplicaID
			// We must have a rangeStorageImpl with state UninitializedStateMachine,
			// that we need to return.
			if !ok {
				return nil, errors.Errorf(
					"range %v with replica with HardState is unknown to ReplicasStorage",
					rhsRR.RangeID)
			}
		} else {
			// Case A12: The RHS range ID must not exist on this store.
			// We must not have any rangeStorageImpl and we will return nil.
			if ok {
				return nil, errors.Errorf(
					"range %v with replica %v without HardState is known to ReplicasStorage",
					rhsRS.id.RangeID, rhsRS.id.ReplicaID)
			}
		}
		if rhsRS != nil {
			if rhsRS.mu.state != UninitializedStateMachine {
				return nil, errors.Errorf("RHS range %v with replica %v is already initialized",
					rhsRS.id.RangeID, rhsRS.id.ReplicaID)
			}
		}
		// NB: The engine does not contain the RSH range-id keys, but the smBatch
		// does, because it was setting up the RHS of the split. So by removing
		// both the range keys and the range-id keys for the RHS, we are
		// effectively removing the former from the engine and the latter from the
		// smBatch.
		if err := clearRanges(
			smBatch.Batch(), replicatedKeySpansForStateMachine(rhsRR.RangeID, rhsSpan)); err != nil {
			return nil, err
		}
	} else {
		// The replica has not been removed.
		// Do step C1 if needed.
		if !foundHS {
			// Durably create the HardState.
			if err = createHardStateAndReplicaID(ctx, reps.eng, rhsRR, rhsRSL); err != nil {
				// This is a retryable error (i.e., we don't need to crash).
				return nil, err
			}
		}
		doStepC3 = true
	}
	// Commit smBatch. It needs to sync in both the case when it is C2 or D1.
	if err := smBatch.Commit(true); err != nil {
		// This is a retryable error (i.e., we don't need to crash), since the
		// only mutation we may have already committed is creating the empty
		// HardState.
		return nil, err
	}
	rs.mu.span.EndKey = rhsSpan.Key
	if doStepC3 {
		rhsRS = makeRangeStorageImpl(rhsRR, reps, reps.ssConstructor)
		if err := makeRaftConsistentWithStateMachine(ctx, reps.eng, rhsRSL, rhsRS.ss); err != nil {
			err := errors.Wrapf(err, "failing because step C3 of creation has failed")
			log.Fatalf(ctx, "%v", err)
			return nil, err
		}
		rhsRS.mu.state = InitializedStateMachine
		rhsRS.mu.span = rhsSpan
		reps.mu.replicasMap[rhsRS.id.RangeID] = rhsRS
		if err := reps.addInitializedRangeToMap(rhsRS); err != nil {
			log.Fatalf(ctx, "%v", err)
		}
	}
	return rhsRS, nil
...
func (reps *replicasStorageImpl) MergeReplicas(
	ctx context.Context, lhsRS RangeStorage, rhsRS RangeStorage, smBatch MutationBatch,
) error {
	reps.mu.Lock()
	defer reps.mu.Unlock()

	lRS := lhsRS.(*rangeStorageImpl)
	rRS := rhsRS.(*rangeStorageImpl)
	if lRS.mu.state != InitializedStateMachine || rRS.mu.state != InitializedStateMachine {
		return errors.Errorf("ranges %v and %v being merged are not initialized",
			lRS.id.RangeID, rRS.id.RangeID)
	}
	batch := smBatch.Batch()
	if err := clearRanges(batch, []roachpb.Span{
		rditer.MakeRangeIDLocalKeySpan(rRS.id.RangeID, true)}); err != nil {
		return err
	}
	// The RangeTombstoneKey, which is an unreplicated range-id local key that
	// is in the state machine, needs to be adjusted.
	err := storage.MVCCBlindPutProto(ctx, batch, nil, rRS.rsl.RangeTombstoneKey(),
		hlc.Timestamp{}, hlc.ClockTimestamp{},
		&roachpb.RangeTombstone{NextReplicaID: mergedTombstoneReplicaID}, nil)
	if err != nil {
		return err
	}
	// Sync is set to true since this is step D1 for the RHS.
	if err := smBatch.Commit(true); err != nil {
		return err
	}
	// Do step D2.
	if err := clearRaftState(ctx, reps.eng, rRS.rsl, rRS.ss); err != nil {
		err = errors.Wrap(err, "failing because D2 for RHS of a merge failed")
		log.Fatalf(ctx, "%v", err)
		return err
	}
	if err := reps.removeFromMaps(rRS); err != nil {
		log.Fatalf(ctx, "%v", err)
		return err
	}
	rRS.mu.Lock()
	rRS.mu.state = DeletedReplica
	rRS.mu.Unlock()
	lRS.mu.Lock()
	lRS.mu.span.EndKey = rRS.mu.span.EndKey
	lRS.mu.Unlock()
	return nil
}

func (reps *replicasStorageImpl) DiscardReplica(
	ctx context.Context, r RangeStorage, nextReplicaID roachpb.ReplicaID,
) error {
	reps.mu.Lock()
	defer reps.mu.Unlock()

	// The replica is either in UninitializedStateMachine or
	// InitializedStateMachine state.
	rs := r.(*rangeStorageImpl)
	// Step D1
	{
		batch := reps.eng.NewUnindexedBatch(true)
		defer batch.Close()
		if rs.mu.state == InitializedStateMachine {
			if err := clearRanges(batch, replicatedKeySpansForStateMachine(rs.id.RangeID, rs.mu.span)); err != nil {
				return err
			}
		}
		err := storage.MVCCBlindPutProto(ctx, batch, nil, rs.rsl.RangeTombstoneKey(),
			hlc.Timestamp{}, hlc.ClockTimestamp{},
			&roachpb.RangeTombstone{NextReplicaID: nextReplicaID}, nil)
		if err != nil {
			return err
		}
	...
	// Step D2
	if err := clearRaftState(ctx, rs.reps.eng, rs.rsl, rs.ss); err != nil {
		err = errors.Wrap(err, "failing because D2 for RHS of a merge failed")
		log.Fatalf(ctx, "%v", err)
		return err
	}
	if err := reps.removeFromMaps(rs); err != nil {
		log.Fatalf(ctx, "%v", err)
		return err
	}
	rs.mu.Lock()
	rs.mu.state = DeletedReplica
	rs.mu.Unlock()
	return nil
}

func (reps *replicasStorageImpl) addInitializedRangeToMap(rs *rangeStorageImpl) error {
	n := len(reps.mu.replicasSpans)
	i := sort.Search(n, func(i int) bool {
		return reps.mu.replicasSpans[i].mu.span.Key.Compare(rs.mu.span.Key) > 0
	})
	if i > 0 {
		if reps.mu.replicasSpans[i-1].mu.span.EndKey.Compare(rs.mu.span.Key) > 0 {
			return errors.Errorf("overlapping spans %s, %s",
				reps.mu.replicasSpans[i-1].mu.span.String(), rs.mu.span.String())
		}
	}
	if i < n {
		if rs.mu.span.EndKey.Compare(reps.mu.replicasSpans[i].mu.span.Key) > 0 {
			return errors.Errorf("overlapping spans %s, %s",
				rs.mu.span.String(), reps.mu.replicasSpans[i].mu.span.String())
		}
	}
	if i == n { // nil or empty slice or after last element
		reps.mu.replicasSpans = append(reps.mu.replicasSpans, rs)
		return nil
	}
	reps.mu.replicasSpans = append(reps.mu.replicasSpans[:i+1], reps.mu.replicasSpans[i:]...)
	reps.mu.replicasSpans[i] = rs
	return nil
}

// Removes uninitialized or initialized range from the maps.
func (reps *replicasStorageImpl) removeFromMaps(rs *rangeStorageImpl) error {
	delete(reps.mu.replicasMap, rs.id.RangeID)
	if rs.mu.state == InitializedStateMachine {
		n := len(reps.mu.replicasSpans)
		i := sort.Search(n, func(i int) bool {
			return reps.mu.replicasSpans[i].mu.span.Key.Compare(rs.mu.span.Key) >= 0
		})
		if i == n || reps.mu.replicasSpans[i].mu.span.Key.Compare(rs.mu.span.Key) != 0 {
			return errors.Errorf("initialized range not found in map %s", rs.mu.span.String())
		}
		if i == n-1 {
			reps.mu.replicasSpans = reps.mu.replicasSpans[:n-1]
		} else if i == 0 {
			reps.mu.replicasSpans = reps.mu.replicasSpans[1:]
		} else {
			reps.mu.replicasSpans = append(reps.mu.replicasSpans[:i], reps.mu.replicasSpans[i+1:]...)
		}
	}
	return nil
}
...
func getRangeTombstone(
	ctx context.Context, reader storage.Reader, rsl stateloader.StateLoader,
) (bool, roachpb.RangeTombstone, error) {
	rsl.RangeTombstoneKey()
	tombstoneKey := rsl.RangeTombstoneKey()
	var tombstone roachpb.RangeTombstone
	if ok, err := storage.MVCCGetProto(
		ctx, reader, tombstoneKey, hlc.Timestamp{}, &tombstone, storage.MVCCGetOptions{},
	); !ok || err != nil {
		return false, roachpb.RangeTombstone{}, err
	}
	return true, tombstone, nil
}

func replicatedKeySpansForStateMachine(rangeID roachpb.RangeID, span roachpb.RSpan) []roachpb.Span {
	ranges := keyRangesForStateMachineExceptRangeIDKeys(span)
	ranges = append(ranges, rditer.MakeRangeIDLocalKeySpan(rangeID, true))
	return ranges
}

func keyRangesForStateMachineExceptRangeIDKeys(span roachpb.RSpan) []roachpb.Span {
	// TODO(sumeer): remove this hack that relies on knowing that the rditer
	// function only looks at StartKey and EndKey. We should refactor the rditer
	// functions.
	descriptor := &roachpb.RangeDescriptor{StartKey: span.Key, EndKey: span.EndKey}
	return rditer.MakeReplicatedKeySpansExceptRangeID(descriptor)
}

func clearRanges(writer storage.Writer, keyRanges []roachpb.Span) error {
	// TODO(sumeer): add option to use ClearRangeWithHeuristic. We will need a
	// ReadWriter, so do this after resolving consistency questions and whether
	// Batch can be read from.
	// TODO(sumeer): some callers need this to also clear range keys.
	for _, keyRange := range keyRanges {
		if err := writer.ClearRawRange(
			keyRange.Key, keyRange.EndKey, true, false); err != nil {
			return err
		}
	}
	return nil
...
// Performs step D2 when D2 does not need to be atomic with some other change,
// so this function can create and commit the batch.
func clearRaftState(
	ctx context.Context, eng storage.Engine, rsl stateloader.StateLoader, ss SideloadStorage,
) error {
	batch := eng.NewUnindexedBatch(false /* writeOnly */)
	defer batch.Close()
	sideloadedDeleteFunc, err := clearRaftStateHelper(ctx, batch, rsl, ss)
	if err != nil {
		return err
	}
	if err := batch.Commit(false); err != nil {
...
// Performs step D2 with the given batch.
func clearRaftStateHelper(
	ctx context.Context, batch storage.ReadWriter, rsl stateloader.StateLoader, ss SideloadStorage,
) (sideloadedDeleteFunc func() error, err error) {
	if err := batch.ClearUnversioned(rsl.RaftHardStateKey()); err != nil {
		return nil, err
	}
	...
	sideloadedDeleteFunc, err = clearRaftLogEntries(ctx, batch, rsl, truncState, ss)
	if err != nil {
		return nil, err
	}
	if err := batch.ClearUnversioned(rsl.RaftReplicaIDKey()); err != nil {
		return nil, err
	}
	if err := batch.ClearUnversioned(rsl.RaftTruncatedStateKey()); err != nil {
		return nil, err
	}
	if err := batch.ClearUnversioned(rsl.RangeLastReplicaGCTimestampKey()); err != nil {
		return nil, err
	}
	return sideloadedDeleteFunc, nil
}

func createHardStateAndReplicaID(
	ctx context.Context, eng storage.Engine, rid FullReplicaID, rsl stateloader.StateLoader,
) error {
	batch := eng.NewUnindexedBatch(true /* writeOnly */)
	defer batch.Close()
	if err := rsl.SetHardState(ctx, batch, raftpb.HardState{}); err != nil {
		return err
	}
	if err := rsl.SetRaftReplicaID(ctx, batch, rid.ReplicaID); err != nil {
		return err
	}
	if err := batch.Commit(true); err != nil {
		return err
...
// Used for step C3, where we don't need to perform it atomically with
// something else, so the construction of a batch, and its commit can be done
// by this function.
func makeRaftConsistentWithStateMachine(
	ctx context.Context, eng storage.Engine, rsl stateloader.StateLoader, ss SideloadStorage,
) error {
	batch := eng.NewUnindexedBatch(false)
	defer batch.Close()
	sideloadedDeleteFunc, err := makeRaftConsistentWithStateMachineHelper(ctx, batch, rsl, ss)
	if err != nil {
		return err
	}
	if err := batch.Commit(false); err != nil {
...
// This helper is used when we want to discard all raft log entries because we
// are applying a snapshot or creating the RHS during a split. And we need to
// move HardState.Commit forward. That is, this is step C3. It is not what we
// want to do in ReplicasStorage.Init.
func makeRaftConsistentWithStateMachineHelper(
	ctx context.Context, batch storage.ReadWriter, rsl stateloader.StateLoader, ss SideloadStorage,
) (sideloadedDeleteFunc func() error, err error) {
	...
	as, err := rsl.LoadRangeAppliedState(ctx, batch)
	if err != nil {
		return nil, err
	}
	sideloadedDeleteFunc, err = clearRaftLogEntries(ctx, batch, rsl, truncState, ss)
	...
	truncState.Term = as.RaftAppliedIndexTerm
	if hs.Commit > as.RaftAppliedIndex {
		return nil, errors.Errorf("cannot decrease HardState.Commit from %d to %d",
			hs.Commit, as.RaftAppliedIndex)
	}
	hs.Commit = as.RaftAppliedIndex
	if err := rsl.SetHardState(ctx, batch, hs); err != nil {
		return nil, err
	}
	if err := rsl.SetRaftTruncatedState(ctx, batch, &truncState); err != nil {
		return nil, err
	}
	return sideloadedDeleteFunc, err
}

// clearRaftLogEntries clears entries in (truncState.Index, lastRaftIndex],
// where lastRaftIndex will be computed by the callee.
func clearRaftLogEntries(
	ctx context.Context,
	...
	lo := truncState.Index + 1
	lastRaftIndex, hasEntry, err := rsl.LoadLastIndexWithLogEntry(ctx, readWriter)
	if err != nil {
		return nil, err
	}
	// TODO(sumeer):
	...
	var ms enginepb.MVCCStats
	if hasEntry {
		// Clear [lo, lastRaftIndex].
		for i := lo; i <= lastRaftIndex; i++ {
			if _, err := storage.MVCCDelete(
				ctx, readWriter, &ms, rsl.RaftLogKey(i), hlc.Timestamp{}, hlc.ClockTimestamp{},
				nil /* txn */); err != nil {
				return nil, err
			}
		}
		sideloadedDeleteFunc = func() error {
			_, _, err := ss.TruncateTo(ctx, lastRaftIndex+1)
			return err
		}
	}
	return sideloadedDeleteFunc, nil
...
// Checks that span only overlaps with ranges mentioned in subsumedRangeIDs.
// Returns the rangeStorageImpls corresponding to those subsumed ranges, and
// the sorted spans in those ranges that need to be deleted.
func (reps *replicasStorageImpl) checkNoAdditionalOverlap(
	span roachpb.RSpan, rangeID roachpb.RangeID, subsumedRangeIDs []roachpb.RangeID,
) (ranges []*rangeStorageImpl, extraRangeSpansToDelete []roachpb.RSpan, err error) {
	// Find index of first span whose EndKey overlaps.
	n := len(reps.mu.replicasSpans)
	i := sort.Search(n, func(i int) bool {
		return reps.mu.replicasSpans[i].mu.span.EndKey.Compare(span.Key) > 0
	})
	for ; i < n; i++ {
		subsumedSpan := reps.mu.replicasSpans[i].mu.span
		if subsumedSpan.Key.Compare(span.EndKey) >= 0 {
			break
		}
		if reps.mu.replicasSpans[i].id.RangeID == rangeID {
			continue
		}
		found := false
		for _, id := range subsumedRangeIDs {
			if id == reps.mu.replicasSpans[i].id.RangeID {
				found = true
				ranges = append(ranges, reps.mu.replicasSpans[i])
				break
			}
		}
		if subsumedSpan.Key.Compare(span.Key) < 0 {
			extraRangeSpansToDelete = append(extraRangeSpansToDelete, roachpb.RSpan{
				Key: subsumedSpan.Key, EndKey: span.Key})
		}
		if span.EndKey.Compare(subsumedSpan.EndKey) < 0 {
			extraRangeSpansToDelete = append(extraRangeSpansToDelete, roachpb.RSpan{
				Key: span.EndKey, EndKey: subsumedSpan.EndKey})
		}
		if !found {
			return nil, nil, errors.Errorf("additional range with rangeID %d is being subsumed",
				reps.mu.replicasSpans[i].id.RangeID)
		}
	}
	if len(subsumedRangeIDs) != len(ranges) {
		return nil, nil, errors.Errorf("did not find all subsumed replicas")
	}
	return ranges, extraRangeSpansToDelete, nil
}

func makeSSTToDeleteRange(
	ctx context.Context,
	keyRange roachpb.Span,
	sstScratch *SSTSnapshotStorageScratch,
	st *cluster.Settings,
	...
	return sstWriterToFile(ctx, sstWriter, sstFile, sstScratch)
}

func makeSSTForRangeTombstoneForMergedRange(
	ctx context.Context,
	rangeID roachpb.RangeID,
	sstScratch *SSTSnapshotStorageScratch,
	st *cluster.Settings,
	...
	sstWriter := storage.MakeIngestionSSTWriter(ctx, st, sstFile)
	defer sstWriter.Close()
	err = storage.MVCCBlindPutProto(ctx, &sstWriter, nil, keys.RangeTombstoneKey(rangeID),
		hlc.Timestamp{}, hlc.ClockTimestamp{},
		&roachpb.RangeTombstone{NextReplicaID: mergedTombstoneReplicaID}, nil)
	if err != nil {
		return "", err
...
func sstWriterToFile(
	...
	if sstWriter.DataSize > 0 {
		var path string
		if path, err = sstScratch.WriteSSTAndReturnPath(ctx, sstFile.Data()); err != nil {
			return "", err
		}
		return path, nil
	}
