...
func (s *Store) HandleRaftUncoalescedRequest(
	...
	// NB: add a buffer for a few extra messages, to allow heartbeats getting
	// through even if MsgApp quota is maxed out by the sender.
	q, _ := s.raftRecvQueues.LoadOrCreate(req.RangeID, s.cfg.RaftMaxInflightMsgs+10)
