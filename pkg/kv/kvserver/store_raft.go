...
// "other" messages in the code. Other messages are always dequeued when Drain
// is called from the raft scheduler (after which the caller processes the
// drained messages).
//
// MsgApp messages may not be drained if the MsgApp processing is being paced
// to avoid store write stalls. MsgApp draining can be arbitrary delayed, but
...
type raftReceiveQueue struct {
	...
	mu struct { // not to be locked directly
		...
		// The total size in bytes of the queued MsgApp RaftMessageRequests. This
		// field is read by the storeRaftDequeuePacer.
		sizeMsgApps int64
		...
		// fields in the raftReceiveQueue. One of these is sizeMsgApps which is
		// read by storeRaftDequeuePacer and modified by raftReceiveQueue (also
		...
		// set to 0 when the MsgApps are drained.
		//
		// INVARIANTS:
		//
		// pacerDequeueDecision == raftDequeueOtherMsgs <=> pacerDequeueAccountingBytes == 0
		//
		// pacerDequeueDecision == raftDequeueOtherMsgs <=>
		//   the raftReceiveQueue is enqueued in the pacer
		//
		// The first invariant is maintained by ensuring that any transition from
		// raftDequeueAllMsgs => raftDequeueOtherMsgs only happens when the
		// enqueued msgAppInfos are transitioning from non-empty to empty.
		//
		// The transition from raftDequeueOtherMsgs => raftDequeueAllMsgs happens
		// in two situations, (a) when the raftReceiveQueue decides it needs to
		// force the transition because it has MsgApps that cannot wait, (b) when
		// the store is healthy enough (decided by the storeRaftDequeuePacer) for
		// them to be dequeued.
		//
	...
	maxLen int
	// Memory accounting for the queued MsgApp requests.
	...
	*infos = append(*infos, raftRequestInfo{
	...
	if req.LowPriorityOverride {
		if wasEmpty {
			...
			q.dequeuePacer.UpdateDecisionQLocked(q)
		}
		// Else use the current decision.
	} else {
		if q.mu.pacerDequeueDecision == raftDequeueOtherMsgs && n > 0 {
			// Adding non low-pri-override MsgApp entries. We need to dequeue.
			q.dequeuePacer.ForceTransitionToDequeueAllQLocked(q)
		}
		// Else use the current decision.
	}
...
type engineForDequeuePacer interface {
	// TryWaitForMemTableStallHeadroom return true if the number of engine
...
// raftReceiveQueues. Roughly, it needs to perform a number of tasks (a) queue
// raftReceiveQueues that need to be paced, (b) block in the storage.Engine
// waiting for an allowedBurst, (c) track the burst granted by the engine and
// how it is distributed to some of the waiting raftReceiveQueues, and when
// those raftReceiveQueues have used those grants, (d) enqueue granted
// raftReceiveQueues in the raftScheduler. Additionally, we don't always want
// to run in the mode of blocking in the storage.Engine and distributing
// bursts among raftReceiveQueues, since waiting for those bursts to be
// consumed can slow down raftReceiveQueue processing needlessly when the
// Engine is fully healthy.
//
...
//
// - UpdateDecisionQLocked: to ask the pacer to update the pacerDequeueDecision.
//
...
// UpdateDecisionQLocked is called by a raftReceiveQueue to request an update
// to its pacerDequeueDecision.
//
// REQUIRES: q.mu.pacerDequeueDecision == raftDequeueAllMsgs and no MsgApps
// queued.
func (w *storeRaftDequeuePacer) UpdateDecisionQLocked(q *raftReceiveQueue) {
	if w == nil {
		// For tests.
		return
	}
	...
	if len(w.mu.waiting) == 0 {
		now := crtime.NowMono()
		haveHeadroom := true
		if now.Sub(w.mu.lastHeadroomAvailableSampleTime) >= 10*time.Millisecond &&
...
// ForceTransitionToDequeueAllQLocked is called by a raftReceiveQueue to force
// transition to raftDequeueAllMsgs.
//
// REQUIRES: q.mu.pacerDequeueDecision == raftDequeueOtherMsgs.
func (w *storeRaftDequeuePacer) ForceTransitionToDequeueAllQLocked(q *raftReceiveQueue) {
	w.mu.Lock()
	defer w.mu.Unlock()
	_, ok := w.mu.waiting[q]
	if !ok {
	...
	dequeueBurst := func(allowedBurst int64) {
		...
		for !burstConsumed {
			...
			for _, q := range queueBuf {
				...
				q.mu.pacerDequeueAccountingBytes = q.mu.sizeMsgApps
				q.mu.pacerDequeueDecision = raftDequeueAllMsgs
				w.mu.Lock()
				delete(w.mu.waiting, q)
				w.mu.burstBytesToDrain += q.mu.pacerDequeueAccountingBytes
...
func (w *storeRaftDequeuePacer) TryLog(ctx context.Context) {
	// Gauge metric on queue length and ... No, just have a 1 min log statement
	// on queue length bytes granted but not yet returned.
	w.mu.Lock()
	defer w.mu.Unlock()
	if len(w.mu.waiting) == 0 && w.mu.burstBytesToDrain == 0 {
		return
