...
func (s *Store) nodeIsLiveCallback(l livenesspb.Liveness) {
	...
	// NB: The liveness map is currently only used by epoch based leases
	// currently. Technically, we could avoid this as well if we knew that no
	// epoch based leases were in the system. However, to prevent surprises were
	// this map to be used elsewhere in the future, we update it before the check
	// below; this isn't a big deal, as the callback is expensive because of the
	// iteration over all replicas on the store, not this update.
	s.updateLivenessMap()
