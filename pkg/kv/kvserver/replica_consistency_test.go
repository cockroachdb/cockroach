...
func TestGetChecksumNotSuccessfulExitConditions(t *testing.T) {
	...
	rc = tc.repl.getReplicaChecksum(id)
	resp, err = tc.repl.getChecksum(ctx, id)
	if !testutils.IsError(err, "checksum computation did not start") { // FIXME
		t.Fatal(err)
	}
	require.Nil(t, resp.Checksum)
	...
	id = uuid.FastMakeV4()
	rc = tc.repl.getReplicaChecksum(id)
	go require.NoError(t, rc.start(ctx, func() {})) // FIXME
	resp, err = tc.repl.getChecksum(ctx, id)
	...
	resp, err = tc.repl.getChecksum(ctx, uuid.FastMakeV4())
