	...
	datadriven.Walk(t, "testdata/truncated_state", func(t *testing.T, path string) {
		...
		datadriven.RunTest(t, path, func(t *testing.T, d *datadriven.TestData) string {
			...
			case "handle":
				...
				apply, err := handleTruncatedStateBelowRaftPreApply(ctx, &prevTruncatedState, suggestedTruncatedState, loader, eng)
				if err != nil {
					return err.Error()
				}
				...
				if apply {
					prevTruncatedState.Index = suggestedTruncatedState.Index
					prevTruncatedState.Term = suggestedTruncatedState.Term
				}
