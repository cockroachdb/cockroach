	...
	datadriven.Walk(t, datapathutils.TestDataPath(t, "truncated_state"), func(t *testing.T, path string) {
		...
		datadriven.RunTest(t, path, func(t *testing.T, d *datadriven.TestData) string {
			...
			case "prev":
				...
				prevTruncatedState.Index = roachpb.RaftIndex(v)
				d.ScanArgs(t, "term", &v)
				prevTruncatedState.Term = roachpb.RaftTerm(v)
				return ""
			...
			case "put":
				...
				truncState := &roachpb.RaftTruncatedState{
					Index: roachpb.RaftIndex(index),
					Term:  roachpb.RaftTerm(term),
				}
