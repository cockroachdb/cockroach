...
type ClusterAdminClient interface {
	...
	// known cluster node.
	// If visitor returns error marked with errMarkRetry then node would be dialed
	// again and revisited until visitor succeeds or retry attempts are exhausted.
	// In latter case cause of retryVisit is returned.
