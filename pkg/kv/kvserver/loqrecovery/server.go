...
func (s Server) Verify(
	...
	for _, plannedID := range req.DecommissionedNodeIDs {
		...
		if vitality := nl.GetNodeVitalityFromCache(plannedID); vitality.IsValid() {
			decomStatus[plannedID] = vitality.MembershipStatus()
		}
	}
	...
	isNodeLive := func(rd roachpb.ReplicaDescriptor) bool {
		...
		return nl.GetNodeVitalityFromCache(rd.NodeID).IsAlive()
	}
