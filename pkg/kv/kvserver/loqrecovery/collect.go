...
func CollectRemoteReplicaInfo(
	...
	var replInfo []loqrecoverypb.ReplicaInfo
	...
	for {
		...
		if r := info.GetReplicaInfo(); r != nil {
			if currentNode != r.NodeID {
				restartIndex = len(replInfo)
				currentNode = r.NodeID
			}
			replInfo = append(replInfo, *r)
			...
			if s.NodeID == currentNode {
				replInfo = replInfo[:restartIndex]
			}
		}
	}
	return loqrecoverypb.ClusterReplicaInfo{
			LocalInfo:   []loqrecoverypb.NodeReplicaInfo{{Replicas: replInfo}},
...
func CollectStoresReplicaInfo(
	...
	for _, reader := range stores {
		...
		if err := collectReplicasFromStore(ctx, reader, ident.StoreID, ident.NodeID,
