...
func CollectReplicaInfo(
	...
	for _, reader := range stores {
		...
		if err = kvserver.IterateRangeDescriptorsFromDisk(ctx, reader, func(desc roachpb.RangeDescriptor) error {
			...
			// the position after applied (inclusive) and read until the end of the
			// log. We also look on potentially uncommitted entries as we have no way
			// to determine their outcome, and they will become committed as soon as
			// the replica is designated as a survivor.
			...
			replicaData := loqrecoverypb.ReplicaInfo{
				...
				HasUncommittedDescriptors: len(rangeUpdates) > 0,
...
// and searches for changes to range descriptor. Changes are identified by
// commit trigger content which is extracted either from EntryNormal where
// change updates key range info (split/merge) or from EntryConfChange*
