	...
	func() {
		for _, status := range r.UnavailableRanges.Ranges {
			if status.RangeID == d.RangeID {
				require.Equal(t, loqrecoverypb.RangeHealth_LossOfQuorum.String(), status.FailureType.String())
				return
			}
		}
...
func TestRetrieveApplyStatus(t *testing.T) {
	...
	for _, id := range planDetails.UpdatedNodes {
		...
		require.NoError(t, lReg.Reopen(int(id.NodeID-1)), "failed to reopen listen socket")
