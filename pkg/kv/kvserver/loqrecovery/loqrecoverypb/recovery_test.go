...
func TestClusterInfoMergeSameCollectorVersions(t *testing.T) {
	v1 := roachpb.Version{Major: 23, Minor: 1}
	uuid1 := uuid.FastMakeV4()
	info1 := ClusterReplicaInfo{
		ClusterID:        uuid1.String(),
		CockroachVersion: v1,
	...
	require.NoError(t, info1.Merge(info3), "should be able to merge info from the same binary")
}

func TestClusterInfoMergeRejectDifferentCollectorVersions(t *testing.T) {
	v1 := roachpb.Version{Major: 23, Minor: 1}
	v2 := roachpb.Version{Major: 23, Minor: 2}
	uuid1 := uuid.FastMakeV4()
	info1 := ClusterReplicaInfo{
		ClusterID:        uuid1.String(),
		CockroachVersion: v1,
	...
	require.Error(t, info1.Merge(info3), "reject merging of info from different binaries")
}
...
func TestClusterInfoInitializeByMerge(t *testing.T) {
	...
	require.Equal(t, empty.CockroachVersion, v1, "merge should update empty info fields")
}
