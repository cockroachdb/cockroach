...
func requireContainsDescriptor(
	...
	t.Helper()
	for _, v := range seq {
		if value.ChangeType == v.ChangeType {
			switch value.ChangeType {
			case loqrecoverypb.DescriptorChangeType_ReplicaChange:
				if value.Desc.RangeID == v.Desc.RangeID &&
					value.Desc.Replicas().String() == v.Desc.Replicas().String() {
					return
				}
			case loqrecoverypb.DescriptorChangeType_Merge, loqrecoverypb.DescriptorChangeType_Split:
				if value.OtherDesc.RangeID == v.OtherDesc.RangeID &&
					bytes.Equal(value.Desc.StartKey, v.Desc.StartKey) &&
					bytes.Equal(value.Desc.EndKey, v.Desc.EndKey) &&
					bytes.Equal(value.OtherDesc.StartKey, v.OtherDesc.StartKey) &&
					bytes.Equal(value.OtherDesc.EndKey, v.OtherDesc.EndKey) {
					return
				}
			}
		}
	}
	t.Errorf("descriptor change sequence %v doesn't contain %v", seq, value)
	t.FailNow()
}
