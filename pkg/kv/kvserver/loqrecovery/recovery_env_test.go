...
type testReplicaInfo struct {
	...
	// Raft state.
	RangeAppliedIndex  roachpb.RaftIndex             `yaml:"RangeAppliedIndex"`
	RaftCommittedIndex roachpb.RaftIndex             `yaml:"RaftCommittedIndex"`
	...
	for _, replica := range replicaData {
		...
		for i, entry := range raftLog {
			...
			if err := eng.PutUnversioned(keys.RaftLogKey(replica.RangeID,
				roachpb.RaftIndex(uint64(i)+hardState.Commit+1)), value); err != nil {
...
func buildReplicaDescriptorFromTestData(
	...
	for i, u := range replica.DescriptorUpdates {
		entry := raftLogFromPendingDescriptorUpdate(t, replica, u, desc, roachpb.RaftIndex(i))
