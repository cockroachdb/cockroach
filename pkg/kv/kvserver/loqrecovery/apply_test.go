...
func TestApplyVerifiesVersion(t *testing.T) {
	...
	t.Run("apply plan version is newer than cluster", func(t *testing.T) {
		...
		assertPlanError(t, plan, "version is too new")
	})

	t.Run("apply plan version too lower than min supported", func(t *testing.T) {
		...
		assertPlanError(t, plan, "version is too old")
	})
}
