...
) (loqrecoverypb.PlanApplicationResult, error) {
	var result loqrecoverypb.PlanApplicationResult
	_, err := storage.MVCCGetProto(ctx, reader, keys.StoreLossOfQuorumRecoveryStatusKey(),
		hlc.Timestamp{}, &result, storage.MVCCGetOptions{})
	if err != nil {
		log.Error(ctx, "failed to write loss of quorum recovery plan application status")
		return loqrecoverypb.PlanApplicationResult{}, err
	}
	return result, nil
}
