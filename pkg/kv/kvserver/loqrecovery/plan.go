...
type PlanningReport struct {
	...
	// KeyspaceProblems contains any keyspace coverage problems
	KeyspaceProblems []PlanningProblem
}
...
func (p PlanningReport) Error() error {
	if len(p.KeyspaceProblems) > 0 {
		return &KeyspaceError{p.KeyspaceProblems}
	}
	return nil
}
...
func PlanReplicas(
	...
	report.KeyspaceProblems = problems
...
func checkKeyspaceCovering(replicas []rankedReplicas) ([]PlanningProblem, error) {
	sort.Slice(replicas, func(i, j int) bool {
...
func checkDescriptor(rankedDescriptors rankedReplicas) (problems []PlanningProblem) {
