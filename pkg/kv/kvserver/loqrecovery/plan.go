...
// the designated surviving replica to avoid any potential replicaID conflicts in
...
func PlanReplicas(
	...
	// proposedSurvivors contain decisions for all ranges in keyspace. it contains ranges
	...
	for _, rangeReplicas := range replicasByRangeID {
		proposedSurvivors = append(proposedSurvivors, pickPotentialSurvivingReplica(rangeReplicas))
	}
...
// pickPotentialSurvivingReplica given a slice of replicas for the range from all live stores,
// pick one to survive recovery. if progress can be made, still pick one replica so that it
...
func checkKeyspaceCovering(replicas []rankedReplicas) error {
	...
	for _, desc := range replicas {
		// We need to take special care of the case where the survivor replica is outgoing voter.
		// It cannot be designated, as the only replication change it could make is to turn itself
		// into a learner, at which point the range is completely messed up.
		// If it is not a stale replica of some sorts, then that would be a gap in keyspace coverage.
		r, err := desc.survivor().Replica()
		if err != nil {
			return err
		}
		...
		switch {
		case desc.startKey().Less(prevDesc.endKey()):
			start := keyMax(desc.startKey(), prevDesc.startKey())
			end := keyMin(desc.endKey(), prevDesc.endKey())
		...
		// We want to advance previous range details only when new range will advance upper
		// bound. This is not always the case as theoretically ranges could be "nested" or
		// range could be an earlier version encompassing LHS and RHS parts.
		if prevDesc.endKey().Less(desc.endKey()) {
			prevDesc = desc
		}
	}
...
func makeReplicaUpdateIfNeeded(
	...
	if err != nil {
		// We don't expect invalid replicas reaching this stage because we will err out on earlier
		// stages. This is covered by invalid input tests and if we ended up here that means tests
		// are not run, or code changed sufficiently and both checks and tests were lost.
...
func makeReplicaUpdateReport(
	...
	// Replicas that belonged to unavailable nodes based on surviving range descriptor.
