...
func PlanReplicas(
	...
	for _, p := range proposedSurvivors {
		...
		if ok {
			...
			report.PlannedUpdates = append(report.PlannedUpdates, makeReplicaUpdateReport(p, u))
...
func makeReplicaUpdateIfNeeded(
	...
	if err != nil {
		log.Fatalf(ctx, "replica descriptor doesn't contain replica for its own store: %s",
			p.survivor().Desc)
	}
...
func makeReplicaUpdateReport(
	p rankedReplicas, update loqrecoverypb.ReplicaUpdate,
) ReplicaUpdateReport {
	oldReplica, _ := p.survivor().Replica()
