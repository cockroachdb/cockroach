...
type PlanningReport struct {
	...
	// KeyspaceAnomalies contains any keyspace coverage anomalies
	KeyspaceAnomalies []KeyspaceAnomaly
}
...
func (p PlanningReport) Error() error {
	if len(p.KeyspaceAnomalies) > 0 {
		return &KeyspaceError{p.KeyspaceAnomalies}
	}
	return nil
}
...
// error is returned in case of unrecoverable error in the collected data that
// prevents creation of any sane plan or correctable user error.
func PlanReplicas(
	...
	anomalies, err := checkKeyspaceCovering(proposedSurvivors)
	if err != nil {
		return loqrecoverypb.ReplicaUpdatePlan{}, PlanningReport{}, err
	}
	...
	for _, p := range proposedSurvivors {
		...
		if ok {
			anomalies = append(anomalies, checkDescriptor(p)...)
			plan = append(plan, u)
	...
	sort.Slice(anomalies, func(i, j int) bool {
		return anomalies[i].Span().Key.Compare(anomalies[j].Span().Key) < 0
	})
	report.KeyspaceAnomalies = anomalies
...
func checkKeyspaceCovering(replicas []rankedReplicas) ([]KeyspaceAnomaly, error) {
	sort.Slice(replicas, func(i, j int) bool {
	...
	for _, rankedDescriptors := range replicas {
		...
		case rankedDescriptors.startKey().Less(prevDesc.endKey()):
			...
			anomalies = append(anomalies, keyspaceOverlapAnomaly{
		...
		case prevDesc.endKey().Less(rankedDescriptors.startKey()):
			anomalies = append(anomalies, keyspaceGapAnomaly{
				span: roachpb.Span{
...
func checkDescriptor(rankedDescriptors rankedReplicas) (anomalies []KeyspaceAnomaly) {
	...
	for _, change := range rankedDescriptors.survivor().RaftLogDescriptorChanges {
		...
		case loqrecoverypb.DescriptorChangeType_Split:
			anomalies = append(anomalies, rangeSplitAnomaly{
		...
		case loqrecoverypb.DescriptorChangeType_Merge:
			anomalies = append(anomalies, rangeMergeAnomaly{
				rangeID:    rankedDescriptors.rangeID(),
				span:       rankedDescriptors.span(),
				rHSRangeID: change.OtherDesc.RangeID,
				rHSRangeSpan: roachpb.Span{
		...
		case loqrecoverypb.DescriptorChangeType_ReplicaChange:
			...
			if !ok {
				anomalies = append(anomalies, rangeReplicaRemovalAnomaly{
					rangeID: rankedDescriptors.rangeID(),
					span:    rankedDescriptors.span(),
				})
			}
