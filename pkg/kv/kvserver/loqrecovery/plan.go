...
type PlanningReport struct {
	...
	// cluster
	TotalReplicas int
	// HealthyReplicas is the number of replicas that belong to ranges that didn't lose
	// quorum.
	HealthyReplicas int
	...
	if !p.winner.Desc.Replicas().CanMakeProgress(func(rep roachpb.ReplicaDescriptor) bool {
		...
		return loqrecoverypb.ReplicaUpdate{
			...
			NewReplica: &roachpb.ReplicaDescriptor{
				...
				ReplicaID: nextReplicaID,
			},
			NextReplicaID: nextReplicaID + 1,
		}, true
	}
...
func makeReplicaUpdateReport(
	...
	oldReplicaID, _ := p.winner.ReplicaID()
	...
	return ReplicaUpdateReport{
		...
		ReplicaID:                  p.winner.Desc.NextReplicaID,
		StoreID:                    p.winner.StoreID,
