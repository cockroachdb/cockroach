...
import (
	...
	"github.com/cockroachdb/cockroach/pkg/kv/kvserver/loqrecovery/loqrecoverypb/loqutil"
...
func PlanReplicas(
	...
	for _, rangeDesc := range replicas {
		...
		update := loqrecoverypb.ReplicaUpdate{
			...
			StartKey: loqutil.RecoveryKey(desc.StartKey),
...
func PrepareUpdateReplicas(
...
) (map[roachpb.StoreID]storage.Batch, PrepareStoreReport, error) {
	...
	for _, update := range plan.Updates {
		...
		replicaReport, err := prepareReplicaUpdate(ctx, store, batch, update)
		if err != nil {
			return nil, PrepareStoreReport{}, errors.Wrapf(err,
...
func prepareReplicaUpdate(
	...
	if value == nil {
		return PrepareReplicaReport{}, errors.Errorf("failed to find a range descriptor for range %v", key)
	}
	if err != nil {
	...
	if desc.RangeID != update.RangeID {
		return PrepareReplicaReport{}, errors.Errorf("unexpected range ID at key: expected r%d but found r%d", update.RangeID, desc.RangeID)
	}
...
func ApplyRecoveryChanges(
	ctx context.Context, batches map[roachpb.StoreID]storage.Batch,
) (ApplyUpdateReport, error) {
	...
	for id, batch := range batches {
		...
		if !batch.Empty() {
			if err := batch.Commit(true); err != nil {
				...
				const errMsgF = "failed to update store s%d: %v"
				log.Errorf(ctx, errMsgF, id, err)
				updateErrors = append(updateErrors, fmt.Sprintf(errMsgF, id, err))
				failed = true
			} else {
