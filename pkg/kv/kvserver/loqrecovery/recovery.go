...
func (l *DefaultLogger) String() string {
	var messages string
	if len(l.messages) > 0 {
		messages = fmt.Sprintf("Info:\n  %s\n", strings.Join(l.messages, "\n  "))
	}
	var warnings string
	if len(l.warnings) > 0 {
		warnings = fmt.Sprintf("Warnings:\n  %s\n", strings.Join(l.warnings, "\n  "))
	}
...
func CollectReplicaInfo(
...
) (loqrecoverypb.Replicas, error) {
	if len(stores) == 0 {
		return loqrecoverypb.Replicas{}, errors.New("no stores were provided for info collection")
	}

	var replicas []loqrecoverypb.LocalRangeInfo
	for _, db := range stores {
		storeIdent, err := kvserver.ReadStoreIdent(ctx, db)
		if err != nil {
			return loqrecoverypb.Replicas{}, err
		}
	...
	return loqrecoverypb.Replicas{Replicas: replicas}, nil
}
...
func PlanReplicas(
	ranges []loqrecoverypb.Replicas, deadStores []roachpb.StoreID, log Logger,
) (loqrecoverypb.Updates, error) {
	var replicas []loqrecoverypb.LocalRangeInfo
	for _, storeRanges := range ranges {
		replicas = append(replicas, storeRanges.Replicas...)
	}
	availableStoreIDs, missingStoreIDs, err := validateReplicaSets(replicas, deadStores)
	if err != nil {
		return loqrecoverypb.Updates{}, err
	}
	log.Infof("found available stores: %s", joinStoreIDs(availableStoreIDs))
	if len(deadStores) == 0 {
		// If dead stores were not explicitly requested, warn about deduced dead stores
		// to confirm before saving plan.
		log.Warnf("found dead stores: %s and no dead stores were provided explicitly", joinStoreIDs(missingStoreIDs))
	}
	...
	for _, rangeDesc := range replicas {
		...
		if rangeDesc.StoreID != maxLiveVoter {
			log.Infof("not designated survivor, skipping: %s", &desc)
			continue
		}
		...
		if desc.Replicas().CanMakeProgress(func(rep roachpb.ReplicaDescriptor) bool {
			...
			log.Infof("replica has not lost quorum, skipping: %s", &desc)
			continue
		}
		...
		log.Infof("replica has lost quorum, recovering: %s -> %s", &desc, &update)
		plan = append(plan, update)
	}
	return loqrecoverypb.Updates{Updates: plan}, nil
}
...
func validateReplicaSets(
	replicas []loqrecoverypb.LocalRangeInfo, deadStores []roachpb.StoreID,
...
func MarshalAndWriteResults(results interface{}, writer io.Writer) error {
	jsonpb := protoutil.JSONPb{Indent: "  "}
	// We want to serialize non Message values as slices, but TestProtoMarshal won't let us.
	marshalFn := jsonpb.Marshal
	var out []byte
	var err error
	// If we are trying to write an empty go slice, it would be represented by nil.
	// That would produce 'nil' as a result where we want to have '[]'.
	if results == nil {
		results = make([]interface{}, 0)
	}
	if out, err = marshalFn(results); err != nil {
...
func prepareReplicaUpdate(
	...
	if len(desc.InternalReplicas) == 1 &&
		...
		log.Warnf("replica is already updated: %s", desc)
		return nil
	}
	...
	if intent != nil {
		...
		log.Infof("aborting intent: %s (txn %s)", key, intent.Txn.ID)
	...
	log.Infof("updating replica for r%d:%s", desc.RangeID, newDesc.StartKey)
	return nil
}
	...
	for id, batch := range batches {
		...
		if !batch.Empty() {
			if err := batch.Commit(true); err != nil {
				...
				log.Warnf("failed to update store s%d: %v", id, err)
				failed = true
			} else {
				log.Infof("updated store s%d", id)
			}
		}
	}
	if failed {
