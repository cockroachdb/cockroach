...
func TestPublishRangeLogEvents(t *testing.T) {
	...
	for _, td := range []struct {
		...
		expectSuccess bool
		expectReason  kvserverpb.RangeLogEventType
	}{
		{
			name:             "success",
			rangeID:          7,
			...
			returnedRowCount: 1,
			expectSuccess:    true,
			expectReason:     kvserverpb.RangeLogEventType_unsafe_quorum_recovery,
		},
		{
			name:             "sql error",
			rangeID:          7,
		...
		},
		{
			...
			returnedRowCount: 1,
			expectSuccess:    true,
			expectReason:     kvserverpb.RangeLogEventType_add_voter,
		},
	} {
		t.Run(td.name, func(t *testing.T) {
			...
			err := UpdateRangeLogWithRecovery(ctx, execFn, event, td.useLegacyEvent)
			if td.expectSuccess {
			...
			require.Equal(t, td.expectReason.String(), actualArgs[3], "incorrect RangeLog event type")
		})
	}
}
