...
func setupPartitionedRangeWithHandlers(
	...
	for i := range tc.Servers {
		...
		targetStore, err := tc.Servers[partitionedNodeIdx].GetStores().(*kvserver.Stores).
			GetStore(tc.Servers[partitionedNodeIdx].GetFirstStoreID())
		if err != nil {
			return nil, err
		}
		pr.addStore(targetStore.StoreID())
			...
			ListenMessages(store.StoreID(), &storeliveness.UnreliableHandler{
				...
				UnreliableHandlerFuncs: storeliveness.UnreliableHandlerFuncs{
					DropStoreLivenessMsg: func(msg *storelivenesspb.Message) bool {
						...
						if drop {
							log.Infof(context.Background(), "dropping msg %s from store %d: to %d",
								msg.Type, msg.From.StoreID, msg.To.StoreID)
						} else {
							log.Infof(context.Background(), "allowing msg %s from store %d: to %d",
								msg.Type, msg.From.StoreID, msg.To.StoreID)
						}
						return drop
					},
