...
func (r *Replica) ForwardSideTransportClosedTimestamp(
	ctx context.Context, closed hlc.Timestamp, lai roachpb.LeaseSequence,
) {
...
func (st *sidetransportAccess) forward(
	ctx context.Context, closed hlc.Timestamp, lai roachpb.LeaseSequence, knownApplied bool,
) closedTimestamp {
	st.mu.Lock()
	defer st.mu.Unlock()
...
func (st *sidetransportAccess) get(
	...
	appliedLAI roachpb.LeaseSequence,
