...
func (r *Replica) getTargetByPolicy(
	...
	// If the range's policy is not found in targetByPolicy, it falls back to
	...
	// sequence: 1. Side transport prepares a policy map before cluster upgrade is
	// complete. 2. Cluster upgrade completes. 3. Policy refresher sees the
	// upgrade and quickly updates replica policies to use latency-based policies.
