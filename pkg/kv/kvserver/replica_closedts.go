...
func (r *Replica) BumpSideTransportClosed(
	...
	// If the range has been subsumed we can only close up to the subsumption
	// time.
	if freezeStart := r.getFreezeStartRLocked(); !freezeStart.IsEmpty() {
		if freezeStart.Less(target) {
			return false, 0, 0
		}
	}
