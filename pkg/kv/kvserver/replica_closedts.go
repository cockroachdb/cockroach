...
func (r *Replica) ForwardSideTransportClosedTimestamp(
	ctx context.Context, closed hlc.Timestamp, lai enginepb.LeaseSequence,
) {
...
func (st *sidetransportAccess) forward(
	ctx context.Context, closed hlc.Timestamp, lai enginepb.LeaseSequence, knownApplied bool,
) closedTimestamp {
	st.mu.Lock()
	defer st.mu.Unlock()
...
func (st *sidetransportAccess) get(
	...
	appliedLAI enginepb.LeaseSequence,
