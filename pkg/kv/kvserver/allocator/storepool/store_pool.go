...
// is considered dead in StoreLiveness or not. If the store is considered dead,
// true is returned. On the other hand, if the store is considered live, or if
// no attempt was made by the store to get support, false is returned.
//
...
type StoreLivenessFunc func(nid roachpb.NodeID, sid roachpb.StoreID) (isDead bool, _ hlc.ClockTimestamp)
...
func (sd *StoreDetailMu) status(
	...
	// Also check whether the store is considered dead in StoreLiveness.
	isDead, supportWithdrawnTS := sl(sd.Desc.Node.NodeID, sd.Desc.StoreID)
	if isDead {
