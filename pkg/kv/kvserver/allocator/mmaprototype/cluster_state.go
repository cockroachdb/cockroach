...
type storeState struct {
	...
	adjusted struct {
		...
		//
		// LIFECYCLE / STALENESS INVARIANT.
		// topKRanges is rebuilt only by processStoreLeaseholderMsgInternal: a
		// StoreLeaseholderMsg from local store L recomputes topKRanges[L] across
		// every storeState from scratch. Pending mutations to rs.replicas and
		// ss.adjusted.replicas (via applyReplicaChange / undoReplicaChange) and
		// enactment / GC of pending changes do NOT update topK. So between
		// consecutive msgs from L, topKRanges[L] entries can become stale in two
		// ways:
		...
		// rebalanceReplicasFromShedingStore) tolerating mismatches by skipping
		// affected entries. Self-corrects on L's next leaseholder msg.
		//
...
// rebalanceReplicasFromShedingStore). Any code path that mutates rs.replicas
