...
func (prc PendingRangeChange) StringForTesting() string {
	...
	sortedIndices := make([]int, len(prc.pendingReplicaChanges))
	for i := range prc.pendingReplicaChanges {
		sortedIndices[i] = i
	}
	slices.SortFunc(sortedIndices, func(i, j int) int {
		return cmp.Compare(
			prc.pendingReplicaChanges[i].target.StoreID, prc.pendingReplicaChanges[j].target.StoreID)
	})
	for _, index := range sortedIndices {
		c := prc.pendingReplicaChanges[index]
...
func (prc PendingRangeChange) ReplicationChanges() kvpb.ReplicationChanges {
	...
	for _, c := range prc.pendingReplicaChanges {
		...
		case Unknown:
			panic("unknown change type")
		case AddLease, RemoveLease:
			panic("lease changes are not a change replicas")
		}
