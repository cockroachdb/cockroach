...
type rangeState struct {
	...
	// corresponding stores. There must be exactly one replica marked as the
	// leaseholder in this slice.
	//
	...
	//
	// The modeling of intermediate states is not perfect. Say in the above
	// example s3 was the local leaseholder. The decision is removing both the
	// leaseholder and replica from s3, and adding a replica and leaseholder to
	// s4. This will be modeled with two pending changes, one that removes the
	// replica and leaseholder from s3, and another that adds the replica and
	// leaseholder to s4. An intermediate state that can be observed is {s1, s2,
	// s3, s4} with the lease still at s3. But the pending change for adding s4
	// includes both that it has a replica, and it has the lease, so we will not
	// mark it done, and keep pretending that the whole change is pending. Since
	// lease transfers are fast, we accept this imperfect modeling fidelity.
	//
...
func (cs *clusterState) gcPendingChanges(now time.Time) {
	...
	for _, pendingChange := range cs.pendingChanges {
		...
		if pendingChange.startTime.Add(pendingChangeGCDuration).Before(now) {
	...
	if len(replicaChanges) > 0 {
		...
		for _, rmChange := range removeChangeIds {
			cs.maybeUndoPendingChange(rmChange, true)
		}
	}
}
...
// maybeUndoPendingChange attempts to reverse the change with ID cid. If
// requireFound is true, it panics if the change is not found, else is a noop.
// If the change is marked with no-rollback, it is not undone.
func (cs *clusterState) maybeUndoPendingChange(cid ChangeID, requireFound bool) {
	change, ok := cs.pendingChanges[cid]
	if !ok {
		if requireFound {
			panic(fmt.Sprintf("change %v not found %v", cid, printMapPendingChanges(cs.pendingChanges)))
	...
	if rs.pendingChangeNoRollback {
		// One cannot undo changes once no-rollback is true. These will need to be
		// discarded when processing the RangeMsg.
		return
	}
...
//
// REQUIRES: all the changes are to the same range.
