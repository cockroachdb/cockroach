...
func (prc PendingRangeChange) isPureTransferLease() bool {
	...
	for _, c := range prc.pendingReplicaChanges {
		switch c.replicaChangeType() {
		// Any changes to the replica type or replicaID are not lease
		// transfers, since they require a replication change.
		case AddReplica, RemoveReplica, ChangeReplica:
			return false
		}
