...
func TestLoadSummaryForDimension(t *testing.T) {
	...
	testCases := []struct {
		...
		//
		// WriteBandwidth (UnknownCapacity, significance floor = 128 kib).
		// Tests the denominator clamp: denom = max(meanLoad, 128 kib).
		// With meanLoad = 64 kib (< floor), denom = 128 kib.
		// Fraction thresholds: 5% (loadNoChange) ≈ 6.4 kib,
		//                     10% (overloadSlow) ≈ 12.8 kib.
		//
		{
			// Just below the 5% boundary (loadNoChange requires >= 5%):
			// fractionAbove = (6 kib) / 128 kib ≈ 0.047 → loadNormal.
		...
		{
			// At the 5% boundary:
			// fractionAbove = (7 kib) / 128 kib ≈ 0.055 → loadNoChange.
		...
		},
		{
			...
			// fractionAbove = (13 kib + 1) / 128 kib > 0.1 → overloadSlow.
			name:     "WB above 10pct overloadSlow",
			dim:      WriteBandwidth,
			load:     64*kib + 13*kib + 1,
			capacity: UnknownCapacity,
			meanLoad: 64 * kib,
		...
		},
		{
			...
			// fractionAbove = -(13 kib + 1) / 128 kib < -0.1 → loadLow.
		...
		},
		{
			...
			// 5 kib / 128 kib ≈ 0.039 → loadNormal.
