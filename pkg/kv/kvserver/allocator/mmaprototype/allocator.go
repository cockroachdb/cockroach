...
type Allocator interface {
	...
	// ownership of all state inside change is handed off to the callee, and the
	// caller must never mutate PendingRangeChange. If ok is true, the change
	// was registered, and the caller should subsequently use the same change in
	// a subsequent call to AdjustPendingChangeDisposition when the changes are
