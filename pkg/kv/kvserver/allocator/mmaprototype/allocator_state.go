...
func sortTargetCandidateSetAndPick(
	...
	var formatCandidatesLog = func(b *strings.Builder, candidates []candidateInfo) bool {
		...
		for _, c := range candidates {
			if overloadedDim != NumLoadDimensions {
				fmt.Fprintf(b, " s%v(SLS:%v, overloadedDim:%v)", c.StoreID, c.sls, c.dimSummary[overloadedDim])
			} else {
	...
	if formatCandidatesLog(&b, cands.candidates) {
		log.KvDistribution.VEventf(ctx, 2,
			"sortTargetCandidateSetAndPick: candidates:%s, overloadedDim:%s", b.String(), overloadedDim)
	}
