...
// known, it is derived as 5% of capacity. For dimensions with unknown capacity,
// a per-dimension fallback constant is used.
func significanceFloor(dim LoadDimension, capacity LoadValue) LoadValue {
	if capacity != UnknownCapacity {
		return LoadValue(float64(capacity) * 0.05)
	}
	...
	switch dim {
	case WriteBandwidth:
		return 128 << 10 // 128 KiB/s
	case CPURate:
		return 100_000_000 // 100ms/s = 5% of 2 vCPU
	case ByteSize:
...
func loadSummaryForDimension(
	...
	// For example, 5 KiB/s above the mean is 50% at a 10 KiB/s mean, enough
	// to trigger overloadSlow and cause unnecessary replica moves. Clamping
	// the denominator to 128 KiB/s for WriteBandwidth makes that same 5 KiB/s
	// only ~4%. At high loads (mean > floor), the clamp has no effect.
