...
func loadSummaryForDimension(
	...
	//
	// The capacity may be UnknownCapacity. For resources with a known capacity
	// we compare utilization (load/capacity) to the mean utilization across
	// stores. This avoids balancing on absolute usage in heterogeneous clusters
	// (e.g. different CPU core counts or disk sizes). For resources without a
	// known capacity (WriteBandwidth), we fall back to comparing absolute load
	// to the mean load.
