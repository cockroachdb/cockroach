...
func newRebalanceEnv(
	...
	re := &rebalanceEnv{
		...
		fractionPendingIncreaseOrDecreaseThreshold: maxFractionPendingThreshold,
...
func (re *rebalanceEnv) rebalanceStores(
	...
	for _, store := range sheddingStores {
		if re.rangeMoveCount >= re.maxRangeMoveCount {
			log.KvDistribution.VEventf(ctx, 2, "reached max range move count %d, stopping further rebalancing", re.maxRangeMoveCount)
			break
		}
	...
	if ss.StoreID == localStoreID {
		if store.dimSummary[CPURate] >= overloadSlow {
			...
			if hitLimit := re.rebalanceLeasesFromLocalStoreID(ctx, ss, store, localStoreID); hitLimit {
				...
				log.KvDistribution.VEventf(ctx, 2, "skipping replica transfers for s%d to try more leases next time", ss.StoreID)
				return
			}
		} else {
