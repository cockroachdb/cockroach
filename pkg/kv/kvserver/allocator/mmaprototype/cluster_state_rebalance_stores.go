...
func (re *rebalanceEnv) rebalanceStores(
	...
	re.passObs.finishRebalancingPass(consideredAllOverloadedStores)
	return re.changes
}
...
func (re *rebalanceEnv) rebalanceStore(
	...
	re.passObs.storeOverloaded(ss.StoreID, withinLeaseSheddingGracePeriod, ignoreLevel)
	defer func() {
	...
	re.rebalanceReplicas(ctx, store, ss, localStoreID, ignoreLevel)
}
