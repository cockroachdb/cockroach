...
type rebalancingPassMetricsAndLogger struct {
	...
	// Per-pass state (reset each pass, used to update m and log).
	...
	// Overloaded stores skipped during the pass (due to max range move limit
	// reached). Unordered during collection; sorted by store ID before logging.
	...
	// Stores with shedding failures. Unordered during collection; sorted by
	// failure count (descending) before logging.
	failedSummaries []storePassSummary
	// Stores with shedding successes. Unordered during collection; sorted by
	// store ID before logging.
...
// storePassSummary aggregates shedding outcomes for a single overloaded store.
// Computed in finishRebalancingPass().
type storePassSummary struct {
	...
	// this overloaded store, used in failure logging to show the primary failure
	// reason.
