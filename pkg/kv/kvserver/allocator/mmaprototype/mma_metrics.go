...
func (g *rebalancingPassMetricsAndLogger) computePassSummary(buf *redact.StringBuilder) {
	...
	// the stores that had shedding success or failure. Each store is counted at
	// most once based on whether it had any shedding success.
	...
	if len(g.failedSummaries) > 0 {
		...
		logStores(buf, g.failedSummaries, func(store storePassSummary) redact.RedactableString {
			numRanges := store.numShedFailures + store.numShedSuccesses
			buf := redact.StringBuilder{}
			buf.Printf("{s%v, total: %d", store.storeID, redact.SafeInt(numRanges))
