...
const (
	// LeaseDispositionOK dennotes that the store is in principle willing to
	// receive leases.
...
func (ss *Status) assert() error {
	...
	d := ss.Disposition
	if d.Lease < 0 || d.Lease >= leaseDispositionCount {
		return errors.AssertionFailedf("invalid lease disposition %d", d.Lease)
	}
	if d.Replica < 0 || d.Replica >= replicaDispositionCount {
