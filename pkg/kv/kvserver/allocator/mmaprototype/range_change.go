...
// IsTransferLease returns true if the range change is a transfer lease
// operation.
func (rc *ExternalRangeChange) IsTransferLease() bool {
	if len(rc.Changes) != 2 {
		return false
	}
...
func (c ExternalReplicaChange) isOnlyLeaseChange() bool {
	return c.ChangeType == AddLease || c.ChangeType == RemoveLease
}
