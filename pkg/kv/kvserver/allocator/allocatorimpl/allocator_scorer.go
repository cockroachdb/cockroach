...
const (
	...
	// DefaultIOOverloadThreshold is the IO overload score of a store
	// descriptor, that when greater than this value and in excess of the
	// average IO overload of comparable candidates - will have the action
	// defined by StoreHealthEnforcement taken. This value does not affect
	// the allocator in deciding to remove replicas from it's store, only
...
const (
	// StoreHealthNoAction wil not exclude stores from being considered as
	// targets for any action regardless of the store IO overload..
	StoreHealthNoAction StoreHealthEnforcement = iota
	// StoreHealthLogOnly will not exclude stores from being considered as
	// targets for any action regarldess of the store IO overload. When a store
	// exceeds IOOverloadThreshold, an event is logged.
	StoreHealthLogOnly
	// StoreHealthBlockRebalanceTo excludes stores from being being considered as
	// targets for rebalance actions if they exceed (a)
	...
	// StoreHealthBlockAll excludes stores from being considered as a target for
	// allocation and rebalancing actions if they exceed (a)
	// kv.allocator.io_overload_threshold and (b) the mean IO overload among
...
// before considered unhealthy. Once considered unhealthy, the action taken
// will be dictated by IOOverloadThresholdEnforce cluster setting defined
// below. The rationale for using
var IOOverloadThreshold = settings.RegisterFloatSetting(
	...
	"the maximum store io overload before the enforcement defined by "+
		"`kv.allocator.io_overload_threshold_enforce` is taken",
	DefaultIOOverloadThreshold,
)

// IOOverloadThresholdEnforce is the level of enforcement taken upon candidate
// stores when their IO overload exceeds the threshold defined in
// IOOverloadThresold. Under disabled and log enforcement, no action is taken
...
func (c candidate) compare(o candidate) float64 {
	...
	// canidate with the lower IO overload score.
...
func rankedCandidateListForAllocation(
	...
	for _, s := range candidateStores.Stores {
		...
		if !allocator.MaxCapacityCheck(s) || !options.getStoreHealthOptions().storeIsHealthy(
			ctx,
			s,
			candidateStores.CandidateIOOverload.Mean,
		) {
			continue
		}
	...
	for _, comparable := range equivalenceClasses {
		...
		for _, cand := range comparable.candidates {
			...
			cand.ioOverloaded = !options.getStoreHealthOptions().rebalanceToStoreIsHealthy(
				...
				comparable.candidateSL.CandidateIOOverload.Mean,
			)
...
func (o StoreHealthOptions) storeIsHealthy(
	...
	if ioOverloadScore < avg*IOOverloadWaterMark {
		log.KvDistribution.VEventf(ctx, 5, "s%d, allocate check io overload %.2f exceeds threshold %.2f, but below average: %.2f, action enabled %d",
			...
			o.IOThreshold, avg, o.EnforcementLevel)
		return true
	}
...
func (o StoreHealthOptions) rebalanceToStoreIsHealthy(
	...
	if ioOverloadScore < avg*IOOverloadWaterMark {
		log.KvDistribution.VEventf(ctx, 5,
			...
			store.StoreID, ioOverloadScore, o.IOThreshold,
			avg*IOOverloadWaterMark, o.EnforcementLevel)
		return true
	}
...
func (o StoreHealthOptions) excludeUnhealthy(sl storepool.StoreList) storepool.StoreList {
	var filteredDescs []roachpb.StoreDescriptor

	avg := sl.CandidateIOOverload.Mean
	watermark := avg * IOOverloadWaterMark
	for _, store := range sl.Stores {
		score, _ := store.Capacity.IOThreshold.Score()
		if score >= o.IOThreshold && score >= watermark {
			continue
		}
		filteredDescs = append(filteredDescs, store)
	}
	return storepool.MakeStoreList(filteredDescs)
}
