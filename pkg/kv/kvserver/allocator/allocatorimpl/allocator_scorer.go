...
type LoadScorerOptions struct {
	...
	LoadThreshold, MinLoadThreshold state.Load
	...
	MinRequiredRebalanceLoadDiff state.Load
...
func bestStoreToMinimizeLoadDelta(
	...
	storeLoadMap := make(map[roachpb.StoreID]state.Load, len(candidates)+1)
	...
	existingLoadIgnoringRepl := existingLoad.Sub(replLoadValue).Max(state.StaticDimensionContainer{})
	...
	if currentLoadDelta < newLoadDelta {
		panic(
			fmt.Sprintf(
				...
				state.LoadDimensionNames[dimension],
			),
		)
	}
