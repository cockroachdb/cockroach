...
func (o QPSScorerOptions) shouldRebalanceBasedOnThresholds(
	...
	case existingNotOverfull:
		...
		log.Allocator.VInfof(ctx, 4, "existing store s%d is not overfull", eqClass.existing.StoreID)
	...
	case missingStatsForExistingStore:
		...
		log.Allocator.VInfof(ctx, 4, "missing QPS stats for s%d", eqClass.existing.StoreID)
	...
	default:
		log.Allocator.Fatalf(ctx, "unknown reason to decline rebalance: %v", declineReason)
	}
...
func rankedCandidateListForRebalancing(
		...
		for _, repl := range existingReplicasForType {
			...
			if !valid {
				if !needRebalanceFrom {
					log.Allocator.VInfof(ctx, 2, "s%d: should-rebalance(invalid): locality:%q",
			...
			if fullDisk {
				if !needRebalanceFrom {
					log.Allocator.VInfof(ctx, 2, "s%d: should-rebalance(full-disk): capacity:%q",
	...
	for _, comparable := range equivalenceClasses {
		...
		if !ok {
			log.Allocator.Errorf(ctx, "BUG: missing candidate struct for existing store %+v; stores: %+v",
		...
		log.Allocator.VInfof(ctx, 5, "rebalance candidates #%d: %s\nexisting replicas: %s",
...
func (o StoreHealthOptions) readAmpIsHealthy(
	...
	if float64(store.Capacity.L0Sublevels) < avg*l0SubLevelWaterMark {
		log.Allocator.Infof(ctx, "s%d, allocate check l0 sublevels %d exceeds threshold %d, but below average: %f, action enabled %d",
			store.StoreID, store.Capacity.L0Sublevels,
			o.L0SublevelThreshold, avg, o.EnforcementLevel)
		return true
	}

	log.Allocator.Infof(ctx, "s%d, allocate check l0 sublevels %d exceeds threshold %d, above average: %f, action enabled %d",
		store.StoreID, store.Capacity.L0Sublevels,
		o.L0SublevelThreshold, avg, o.EnforcementLevel)
...
func (o StoreHealthOptions) rebalanceToReadAmpIsHealthy(
	...
	if float64(store.Capacity.L0Sublevels) < avg*l0SubLevelWaterMark {
		log.Allocator.Infof(ctx, "s%d, allocate check l0 sublevels %d exceeds threshold %d, but below average watermark: %f, action enabled %d",
			store.StoreID, store.Capacity.L0Sublevels,
			o.L0SublevelThreshold, avg*l0SubLevelWaterMark, o.EnforcementLevel)
		return true
	}

	log.Allocator.Infof(ctx, "s%d, allocate check l0 sublevels %d exceeds threshold %d, above average watermark: %f, action enabled %d",
		store.StoreID, store.Capacity.L0Sublevels,
		o.L0SublevelThreshold, avg*l0SubLevelWaterMark, o.EnforcementLevel)
