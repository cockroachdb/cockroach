...
func TestAllocatorRhadAmpCheck(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)
...
func TestAllocatorTransferLeaseTargetHealth(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)
	ctx := context.Background()
	...
	testCases := []struct {
		...
		{
			desc: "ignore io overload on transfer target when all have io overload",
			// NB: All replicas have the same, high IO overload (+10
			// threshold), none are above the watermark, select the lowest
			// lease count. Store 1 has less leases than store 3.
			stores:           threeStoresHighIOOverloadAscRangeCount,
		...
		{
			desc: "ignore io overload on transfer target when all have io overload",
			// NB: All replicas have the same, high IO overload (+1
			// threshold), none are above the watermark, select the lowest
			// lease count. Store 3 has the least leases.
			stores:           allStoresHighIOOverload,
			existingVoters:   replicas(1, 2, 3),
			leaseholder:      1,
			excludeLeaseRepl: false,
			// Either 2 or 3 could be selected since they are similar and we
			// use power of 2 choices.
			expectedTarget: 2,
		},
		{
			desc:             "don't shed lease when high io overload",
			stores:           threeStoresHighIOOverloadAscRangeCount,
			existingVoters:   replicas(1, 4, 5),
			leaseholder:      1,
			excludeLeaseRepl: false,
			expectedTarget:   0,
		},
	...
	for _, tc := range testCases {
		...
		t.Run("", func(t *testing.T) {
			target := a.TransferLeaseTarget(
				...
				false, /* alwaysAllowDecisionWithoutStats */
				allocator.TransferLeaseOptions{
					ExcludeLeaseRepl:       tc.excludeLeaseRepl,
					CheckCandidateFullness: true,
				},
			)
			require.Equal(t, tc.expectedTarget, target.StoreID)
		})
	}
}
...
func TestAllocatorLeasePreferencesMultipleStoresPerLocality(t *testing.T) {
	...
	testCases := []struct {
		...
		{1, replicas(1, 2, 3), preferEast, 0, 2 /* 2 */},
		{3, replicas(1, 3, 5), preferEast, 1 /* 1 */, 1},
...
func TestAllocatorRebalanceStoreHealthCheck(t *testing.T) {
	...
	for i, test := range tests {
		t.Run(fmt.Sprintf("%d_%s", i+1, test.name), func(t *testing.T) {
			...
			options.StoreHealthOptions = StoreHealthOptions{EnforcementLevel: test.enforcement, IOThreshold: 1}
			add, remove, _, ok := a.RebalanceVoter(
				ctx,
				sp,
				test.conf,
