...
func TestAllocatorTransferLeaseTargetDraining(t *testing.T) {
	...
	a := MakeAllocator(st, MakeAllocatorSync(sp, st), true /* deterministic */, func(id roachpb.NodeID) (time.Duration, bool) {
		return 0, true
	}, nil)
	defer stopper.Stop(ctx)
...
func TestAllocatorComputeActionNoStorePool(t *testing.T) {
	...
	a := MakeAllocator(nil, MakeAllocatorSync(sp, st), false, nil, nil)
