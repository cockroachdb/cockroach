...
var LeaseRebalanceThreshold = settings.RegisterFloatSetting(
	...
	"minimum fraction of a store’s lease count away from the average lease count "+
		"across all stores before it is considered for lease-transfers away from the "+
		"store",
	0.05,
).WithPublic()
...
func (a Allocator) shouldTransferLeaseForLeaseCountConvergence(
	...
	overfullLeaseThreshold := int32(math.Ceil(sl.CandidateLeases.Mean * (1 + LeaseRebalanceThreshold.Get(&a.st.SV))))
	...
	if float64(source.Capacity.LeaseCount) > sl.CandidateLeases.Mean {
		underfullLeaseThreshold := int32(math.Ceil(sl.CandidateLeases.Mean * (1 - LeaseRebalanceThreshold.Get(&a.st.SV))))
