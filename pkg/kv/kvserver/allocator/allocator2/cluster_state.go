...
func (crl *storeChangeRateLimiter) updateForRebalancePass(h *storeEnactedHistory, now time.Time) {
	...
	if len(h.changes) == 0 {
		...
		// where a significant number of nodes will only have follower replicas.
		// We can add a cluster setting effective_fraction_allocators, that
		// defaults to 1.0 that can be adjusted by the operator to the fraction of
		// nodes that can do allocation.
		h.allowChanges = true
		return
	}
	h.allowChanges = true
