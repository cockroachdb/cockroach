...
func (mm *meansMemo) getMeans(expr constraintsDisj) *meansForStoreSet {
	...
	if n < 1 {
		log.Infof(context.Background(), "expr=%+v mean-stores=%v mean-load=%v summaries=%v", expr, means.stores, means.storeLoad, means.storeSummaries)
