...
// This relationship is defined based on the expressions, and not based on
...
// Does some structural normalization even when returning an error. See the
// under-specified voter constraint examples in the datadriven test -- we
// sometimes see these in production settings, and we want to fix ones that we
// can, and raise an error for users to fix their configs.
func doStructuralNormalization(conf *normalizedSpanConfig) (*normalizedSpanConfig, error) {
	...
	// replicas, we add both with 1 voter replica each. This "load-balancing" of
	// the satisfaction is why we need the outer for loop below.
	for {
		added := false
	...
	for i := range conf.voterConstraints {
		...
		if i == emptyVoterConstraintIndex {
			// Let's try to satisfy it now, since we skipped it earlier.
			remaining := neededReplicas - actualReplicas
			if remaining > 0 && emptyConstraintIndex >= 0 {
				...
				if remainingSatisfiable > 0 {
					...
					voterConstraints[i].numReplicas += count
