...
func (r RangeUsageInfo) TransferImpact() load.Load {
	...
	dims[load.CPUTime] = r.ReqCPUNanosPerSecond
	return dims
}

// Load returns a load dimension representation of the range usage.
func (r RangeUsageInfo) RebalanceImpact() load.Load {
	dims := load.Vector{}
	dims[load.CPUTime] = r.RaftCPUNanosPerSecond
	return dims
}
...
func (r RangeUsageInfo) Load() load.Load {
	dims := load.Vector{}
	dims[load.Queries] = r.QueriesPerSecond
	dims[load.CPUTime] = r.RaftCPUNanosPerSecond + r.ReqCPUNanosPerSecond
	return dims
}
