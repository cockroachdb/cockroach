...
func parseNodeLoadMsg(t *testing.T, in string) nodeLoadMsg {
	...
	for _, part := range strings.Fields(lines[0]) {
		...
		case "load-time":
			msg.loadTime = testingBaseTime.Add(time.Second * time.Duration(parseInt(t, parts[1])))
		default:
			t.Fatalf("Unknown argument: %s", parts[0])
		}
	}
...
func parseStoreLeaseholderMsg(t *testing.T, in string) storeLeaseholderMsg {
	...
	lines := strings.Split(in, "\n")
	strings.TrimPrefix(lines[0], "store-leaseholder-msg")
	for _, field := range strings.Fields(lines[0]) {
		...
		case "store-id":
			msg.StoreID = roachpb.StoreID(parseInt(t, parts[1]))
		default:
			t.Fatalf("unknown argument: parts=%v field=%v lines[0]=%v", parts[0], field, lines[0]) // TODO(kvoli): remove this
		}
	}
...
func printPendingChanges(changes []*pendingReplicaChange) string {
	sortedChanges := make([]*pendingReplicaChange, 0, len(changes))
	sort.Slice(changes, func(i, j int) bool {
		return changes[i].startTime.Before(changes[j].startTime)
	})
	var buf strings.Builder
	fmt.Fprintf(&buf, "pending(%d)", len(sortedChanges))
...
// TODO(kvoli): Clean this up, it is leftover from the earlier testing.
		...
		func(t *testing.T, path string) {
			...
			datadriven.RunTest(t, path, func(t *testing.T, d *datadriven.TestData) string {
				...
				case "set-store-membership":
					...
					var storeMembership storeMembership
					switch storeMembershipString {
					case "member":
						storeMembership = storeMembershipMember
					case "removing":
						storeMembership = storeMembershipRemoving
					case "removed":
						storeMembership = storeMembershipRemoved
					}
					cs.setStoreMembership(roachpb.StoreID(storeID), storeMembership)
				...
				case "make-pending-changes":
					...
					for _, next := range lines {
						...
						case "transfer-lease":
							...
							rState.rangeID = rangeID
							transferChanges := makeLeaseTransferChanges(rState, load, add, remove)
							changes = append(changes, transferChanges[:]...)
						case "add-replica":
							add, _, replType, load := parseChangeAddRemove(t, v)
							addChanges := makeAddReplicaChange(rangeID, load, add, replType)
						...
						case "remove-replica":
							...
							changes = append(changes, makeRemoveReplicaChange(rangeID, load, removeRepl))
						case "rebalance-replica":
							add, remove, _, load := parseChangeAddRemove(t, v)
							rebalanceChanges := makeRebalanceReplicaChanges(rState, load, add, remove)
				...
				case "tick":
					...
					return fmt.Sprintf("clock=%ds", ts.Now().UnixNano()/int64(time.Second))
