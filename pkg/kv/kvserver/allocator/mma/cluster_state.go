...
func (rt replicaIDAndType) SafeFormat(w redact.SafePrinter, _ rune) {
	...
	w.Printf(" type=%v leaseholder=%v", rt.replicaType.replicaType, rt.isLeaseholder)
}
...
func makeLeaseTransferChanges(
	rState *rangeState, rLoad rangeLoad, addStoreID, removeStoreID roachpb.StoreID,
) [2]replicaChange {
...
func makeRebalanceReplicaChanges(
	...
	addReplicaChange := makeAddReplicaChange(rState.rangeID, rLoad, addStoreID, remove.replicaType.replicaType)
	removeReplicaChange := makeRemoveReplicaChange(rState.rangeID, rLoad, remove)
...
type storeState struct {
	...
	adjusted struct {
		...
		//
		// XXX: Changes when something is never going to happen (gc duration 5
		// mins), reject pending change (enacting module). Or a change is
		// tentativelyMade.
...
// XXX: We GC pending changes on processNodeLoadMsg.
// - This implies that pendingChanges are only
// XXX: We update pending changes as enacted vs not enacted on
// processStoreLeaseholderMsg.
func (cs *clusterState) processNodeLoadMsg(msg *nodeLoadMsg) {
	...
	for _, storeMsg := range msg.stores {
		...
		// (should) already contain(s) the effect(s).
		for _, change := range ss.computePendingChangesReflectedInLatestLoad(now) {
...
func (cs *clusterState) processStoreLeaseholderMsg(msg *storeLeaseholderMsg) {
	...
	for _, rangeMsg := range msg.ranges {
		...
		// replicas. The pending changes which are not enacted in the range message
		// are handled and added back below.
		...
		if err != nil {
			// TODO(kvoli): Decide if we want to bubble this error up, or log a
...
// as pending. The application updates the adjusted load, tracked pending
// changes and changeID to reflect the pending application.
...
func (cs *clusterState) undoPendingChange(cid changeID) {
	...
	if !ok {
		// TODO(kvoli): This is a panic but we could just log a warning or let this
		// happen, revisit.
