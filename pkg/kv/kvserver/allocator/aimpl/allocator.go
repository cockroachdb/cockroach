...
// not have enough capacity for the extra load. We can add a retry parameter
// that is increased whenever a store that needs to shed load is unable to
// shed anything, or a range that is under-replicated is unable to add a
// replica. And use that retry parameter to broaden the stores we can possibly
// pick from pl.
