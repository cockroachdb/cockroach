...
// snapshotStorageData contains all the data needed for snapshot application that requires
// storage engine interaction to generate.
type snapshotStorageData struct {
	// state is the replica state loaded from storage.
	state kvserverpb.ReplicaState
	// priorReadSum is the read summary loaded from storage.
	priorReadSum *rspb.ReadSummary
	// unreplicatedSST contains the SST for unreplicated state.
	unreplicatedSST []byte
	// clearedSpans are the spans that will be cleared during snapshot application.
	clearedSpans []roachpb.Span
	// subsumedSSTs contains SSTs for clearing subsumed replicas.
	subsumedSSTs [][]byte
}

// prepareSnapshotData prepares all storage-related data needed for snapshot application.
// This includes loading state from disk and creating SSTs for clearing state.
func prepareSnapshotData(
	ctx context.Context,
	eng storage.Engine,
...
) (*snapshotStorageData, error) {
	// Load replica state.
	state, err := sl.Load(ctx, eng, desc)
	if err != nil {
		return nil, errors.Wrap(err, "failed to load replica state")
	}

	// Load prior read summary.
	prioReadSum, err := readsummary.Load(ctx, eng, desc.RangeID)
	if err != nil {
		return nil, errors.Wrap(err, "failed to read prior read summary")
	}

	// Create unreplicated SST.
	unreplicatedSSTFile, clearedSpan, err := writeUnreplicatedSST(
		ctx, storage.FullReplicaID{RangeID: desc.RangeID, ReplicaID: replicaID}, st, truncState, hs, &sl.StateLoader)
	if err != nil {
		return nil, err
	}
	clearedSpans := []roachpb.Span{clearedSpan}

	// Create SSTs for subsumed replicas.
	var subsumedSSTs [][]byte
	if len(subsumedDescs) > 0 {
		writeSST := func(ctx context.Context, data []byte) error {
			subsumedSSTs = append(subsumedSSTs, data)
			return nil
		}

		subsumedClearedSpans, err := clearSubsumedReplicaDiskData(
			ctx, st, eng, writeSST, desc, subsumedDescs, mergedTombstoneReplicaID)
		if err != nil {
			return nil, err
		}
	...
	return &snapshotStorageData{
		state:           state,
		priorReadSum:    prioReadSum,
		unreplicatedSST: unreplicatedSSTFile.Data(),
		clearedSpans:    clearedSpans,
		subsumedSSTs:    subsumedSSTs,
	}, nil
}
