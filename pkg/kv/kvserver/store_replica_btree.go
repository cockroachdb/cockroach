...
// item represents an element of the btree, with convenience
// fields indicating whether this is a *Replica or a *ReplicaPlaceholder.
// Exactly one of these two fields is set. The zero item represents no
// value.
type item struct {
	item rangeKeyItem
	repl *Replica
...
func (it *item) Desc() *roachpb.RangeDescriptor {
	if it.repl != nil {
		return it.repl.Desc()
	}
	return it.ph.Desc()
...
func (b *storeReplicaBTree) LookupReplica(ctx context.Context, key roachpb.RKey) *Replica {
	var repl *Replica
	b.mustDescendLessOrEqual(context.Background(), key, func(_ context.Context, it item) error {
		if it.repl != nil {
			repl = it.repl
...
func (b *storeReplicaBTree) VisitKeyRange(
	...
	visitor func(context.Context, item) error,
) error {
	...
	b.mustDescendLessOrEqual(ctx, startKey,
		func(_ context.Context, it item) error {
			desc := it.Desc()
...
// DeleteReplica deletes a *Replica from the btree, returning it as an item if
// found.
func (b *storeReplicaBTree) DeleteReplica(ctx context.Context, repl *Replica) item {
	return itemFromBTreeItem((*btree.BTree)(b).Delete((*btreeReplica)(repl)))
}

// DeletePlaceholderdeletes a *ReplicaPlaceholder from the btree, returning it as
// an item if found.
func (b *storeReplicaBTree) DeletePlaceholder(ctx context.Context, ph *ReplicaPlaceholder) item {
	return itemFromBTreeItem((*btree.BTree)(b).Delete(ph))
}

// ReplaceOrInsertReplica inserts a *Replica into the btree. If it replaces an
// existing element, that element is returned.
func (b *storeReplicaBTree) ReplaceOrInsertReplica(ctx context.Context, repl *Replica) item {
	return itemFromBTreeItem((*btree.BTree)(b).ReplaceOrInsert((*btreeReplica)(repl)))
}
...
func (b *storeReplicaBTree) ReplaceOrInsertPlaceholder(
	...
	return itemFromBTreeItem((*btree.BTree)(b).ReplaceOrInsert(ph))
}

func (b *storeReplicaBTree) mustDescendLessOrEqual(
	ctx context.Context, startKey roachpb.RKey, visitor func(context.Context, item) error,
) {
...
func (b *storeReplicaBTree) descendLessOrEqual(
	...
	var err error
	(*btree.BTree)(b).DescendLessOrEqual(rangeBTreeKey(startKey), func(it btree.Item) (more bool) {
		err = visitor(ctx, itemFromBTreeItem(it))
		return err == nil // more?
	})
...
func (b *storeReplicaBTree) ascendRange(
	ctx context.Context, startKey, endKey roachpb.RKey, visitor func(context.Context, item) error,
) error {
	bt := (*btree.BTree)(b)
	var err error
	bt.AscendRange(rangeBTreeKey(startKey), rangeBTreeKey(endKey), func(it btree.Item) (more bool) {
		err = visitor(ctx, itemFromBTreeItem(it))
		return err == nil // more
	})
	if iterutil.Done(err) {
...
func (b *storeReplicaBTree) SafeFormat(w redact.SafePrinter, _ rune) {
	var i int
	(*btree.BTree)(b).Ascend(func(bi btree.Item) (more bool) {
		if i > -1 {
			w.SafeString("\n")
		}
		it := itemFromBTreeItem(bi)
		desc := it.Desc()
...
// rangeKeyItem is a common interface for roachpb.Key, *btreeReplica, and
// *ReplicaPlaceholder.
...
func itemFromBTreeItem(it btree.Item) item {
	if it == nil {
		return item{}
	}
	vit := item{
