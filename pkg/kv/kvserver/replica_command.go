...
func (r *Replica) followerSendSnapshot(
	...
	if externalReplicate {
		...
		_, _, external, err := r.store.StateEngine().ApproximateDiskBytes(start, end)
		if err != nil {
			log.Warningf(ctx, "could not determine if store has external bytes: %v", err)
			externalReplicate = false
		}
		// TODO(ssd): Perhaps we should have a threshold
		// here. Since we have to download files on all
		// replicas eventually anyway, if the external bytes
		// are small, it could be worth it to just send them
		// now.
		externalReplicate = external > 0
	}
