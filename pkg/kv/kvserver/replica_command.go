...
func (r *Replica) changeReplicasImpl(
	...
	if adds := targets.voterAdditions; len(adds) > 0 {
		...
		var err error
		desc, err = r.addAndInitializeRaftLearners(
			ctx,
			desc,
			priority,
			reason,
			details,
			targets.voterAdditions,
			roachpb.LEARNER,
		)
		if err != nil {
			log.Infof(
				ctx,
				"could not successfully add and upreplicate LEARNER replica(s) on %s, rolling back: %v",
				adds,
				err,
			)
			if fn := r.store.cfg.TestingKnobs.VoterAddSkipLearnerRollback; fn != nil && fn() {
				return nil, err
			}
	...
	if adds := targets.nonVoterAdditions; len(adds) > 0 {
		...
		// care about only dealing with replicas that are almost caught up with the
		...
		// https://github.com/cockroachdb/cockroach/issues/63199 for details.
		desc, err = r.addAndInitializeRaftLearners(
			ctx,
			desc,
			priority,
			reason,
			details,
			targets.nonVoterAdditions,
			roachpb.NON_VOTER,
		)
		if err != nil {
			// NB: If we get an error here because we somehow failed to send a
			// snapshot to the newly added NON_VOTER, we don't care about rolling it
			// back because we expect it will get picked up by the raft snapshot queue
			// soon and upreplicate that way.
			r.store.raftSnapshotQueue.MaybeAddAsync(ctx, r, r.store.Clock().NowAsClockTimestamp())
			return nil, err
		}
	}
...
// addAndInitializeRaftLearners adds etcd learners (LEARNERs or NON_VOTERs) to
// the given replication targets and synchronously sends them an initial
// snapshot to upreplicate.
func (r *Replica) addAndInitializeRaftLearners(
	ctx context.Context,
	desc *roachpb.RangeDescriptor,
	priority SnapshotRequest_Priority,
	reason kvserverpb.RangeLogEventReason,
