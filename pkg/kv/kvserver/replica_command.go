...
// GetTargetsToCollocateRHSForMerge computes the stores which the voters and
...
func GetTargetsToCollocateRHSForMerge(
	existingLeftRepls, existingRightRepls roachpb.ReplicaSet,
) (voterTargets, nonVoterTargets []roachpb.ReplicationTarget) {
	numVoters, numNonVoters := len(existingLeftRepls.VoterDescriptors()), len(existingLeftRepls.NonVoterDescriptors())
	notInRight := func(desc roachpb.ReplicaDescriptor) bool {
		return !existingRightRepls.Contains(desc)
	}

	// Sets of replicas that exist on the LHS but not on the RHS
	leftMinusRight := existingLeftRepls.Filter(notInRight)
	...
	finalRightVoters := existingRightRepls.Voters().Filter(existingLeftRepls.Contains).DeepCopy()
	finalRightNonVoters := existingRightRepls.NonVoters().Filter(existingLeftRepls.Contains).DeepCopy()

	for len(finalRightVoters.Descriptors()) != numVoters {
		...
		if len(leftMinusRightVoters) != 0 {
			...
			panic(`programming error: unexpectedly ran out of valid stores to relocate RHS voters to`)
		}
	}

	for len(finalRightNonVoters.Descriptors()) != numNonVoters {
		...
		if len(leftMinusRightNonVoters) != 0 {
			...
			panic(`programming error: unexpectedly ran out of valid stores to relocate RHS non-voters to`)
		}
	}

	notCollocated := len(leftMinusRightVoters) != 0 || len(leftMinusRightNonVoters) != 0
	if notCollocated {
		panic(fmt.Sprintf(`programming error: expected zero non-collocated replicas; found voters: %s, non-voters: %s`,
			leftMinusRightVoters, leftMinusRightNonVoters))
	}

	return finalRightVoters.ReplicationTargets(), finalRightNonVoters.ReplicationTargets()
}
...
func (s *Store) AdminRelocateRange(
	...
	if len(voterTargets) > 0 {
		_, err = s.relocateReplicas(ctx, rangeDesc, voterTargets, true)
		if err != nil {
			return err
		}
	}
	if len(nonVoterTargets) > 0 {
		rangeDesc, err = s.relocateReplicas(ctx, rangeDesc, nonVoterTargets, false)
		if err != nil {
			return err
		}
	}
...
func (s *Store) relocateReplicas(
	...
	for {
		for re := retry.StartWithCtx(ctx, retry.Options{MaxBackoff: 5 * time.Second}); re.Next(); {
			...
			ops, leaseTarget, err := s.relocateOne(ctx, &rangeDesc, targets, areVoters)
			if err != nil {
				return rangeDesc, err
			}
			if areVoters && leaseTarget != nil {
...
func (s *Store) relocateOne(
	...
	var rangeReplicas []roachpb.ReplicaDescriptor
	var addOp, removeOp roachpb.ReplicaChangeType
	if isVoter {
		rangeReplicas = desc.Replicas().VoterDescriptors()
		addOp, removeOp = roachpb.ADD_VOTER, roachpb.REMOVE_VOTER
	} else {
		rangeReplicas = desc.Replicas().NonVoterDescriptors()
		addOp, removeOp = roachpb.ADD_NON_VOTER, roachpb.REMOVE_NON_VOTER
	}
	...
	existingRepls := roachpb.MakeReplicaSet(rangeReplicas).ReplicationTargets()
	targetsToAdd := leftMinusRight(targets, existingRepls)    // targets - existingRepls
	targetsToRemove := leftMinusRight(existingRepls, targets) // existingRepls - targets
	...
	if len(targetsToRemove) > 0 {
		...
		targetStore, _, err := s.allocator.RemoveTarget(ctx, zone, targetsToRemove, rangeReplicas)
		if err != nil {
			return nil, nil, errors.Wrapf(err, "unable to select removal target from %v; current replicas %v",
				targetsToRemove, rangeReplicas)
		}
...
// leftMinusRight returns the set of replication targets that in `left` but not
// in `right` (i.e. left - right).
func leftMinusRight(left, right []roachpb.ReplicationTarget) (diff []roachpb.ReplicaDescriptor) {
	for _, t := range left {
		found := false
