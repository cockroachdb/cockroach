...
func GetTargetsToCollocateRHSForMerge(
...
) (voterTargets, nonVoterTargets []roachpb.ReplicationTarget, err error) {
	...
	// We loop through set of non-collocated replicas and figure out a suitable
	// configuration to relocate RHS's replicas to. At the end of these two loops,
	// we will have exhausted `leftMinusRight`.
