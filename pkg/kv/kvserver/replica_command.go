...
func splitTxnAttempt(
	...
	antiDriftDeltaStats enginepb.MVCCStats,
) error {
	...
	b.AddRawRequest(&kvpb.EndTxnRequest{
		...
		InternalCommitTrigger: &roachpb.InternalCommitTrigger{
			SplitTrigger: &roachpb.SplitTrigger{
				...
				AntiDriftDeltaStats:   antiDriftDeltaStats,
			},
		},
	})
...
func (r *Replica) adminSplitWithDescriptor(
	...
	// Compute the stats for the range and a delta wrt the stored stats. Applying
	// this delta to the on-disk stats in splitTrigger will prevent estimates from
	// compounding across splits.
	totalStats, err := rditer.ComputeStatsForRange(
		ctx, desc, r.store.TODOEngine(), r.store.Clock().NowAsClockTimestamp().WallTime)
	if err != nil {
		return reply, errors.Wrapf(err, "unable to compute total pre-split stats for range")
	}
	antiDriftDelta := totalStats
	antiDriftDelta.Subtract(r.GetMVCCStats())

	// We need the LHS user-only stats separately to estimate the total post-split
	// LHS stats by combining these stats with the non-user stats computed in
	// splitTrigger.
	...
	if err := r.store.DB().Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
		return splitTxnAttempt(ctx, r.store, txn, leftDesc, rightDesc, desc, reason, userOnlyLeftStats, antiDriftDelta)
	}); err != nil {
