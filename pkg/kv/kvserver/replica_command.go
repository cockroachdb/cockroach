...
func (r *Replica) adminSplitWithDescriptor(
	...
	// As a result of exiting the joint config, we might no longer be the leaseholder
	/*if !r.OwnsValidLease(ctx, r.store.Clock().NowAsClockTimestamp()) {
		return reply, &benignError{errors.Errorf(
			"unable to complete - leaseholder moved.")}
	}*/
...
func (r *Replica) changeReplicasImpl(
	...
	// As a result of exiting the joint config, we might no longer be the leaseholder
	/*if !r.OwnsValidLease(ctx, r.store.Clock().NowAsClockTimestamp()) {
		return nil, errors.Errorf("unable to complete - leaseholder moved.")
	}*/
...
func (r *Replica) maybeLeaveAtomicChangeReplicas(
	...
	if beingRemoved {
		...
		if target == (roachpb.ReplicaDescriptor{}) {
			...
			/*log.VEventf(ctx, 5,"Leaseholder transfer to %v complete.", target.String())
			return nil, nil*/
		}
	}
...
func (r *Replica) AdminRelocateRange(
	...
	newDesc, err := r.maybeLeaveAtomicChangeReplicasAndRemoveLearners(ctx, &rangeDesc)
	if err != nil {
