...
func (r *Replica) sendSnapshotUsingDelegate(
	...
	for n, sender := range senders {
		...
		if retErr == nil {
			...
			log.Warningf(ctx, "attempt %d: delegate snapshot %+v request failed %v", n, delegateRequest, err)
		}
	}
	return
}
...
func (r *Replica) validateSnapshotDelegationRequest(
	...
	// applied index is lower than or equal to the truncated state on the
	...
	if replIdx <= req.FirstIndex {
		log.Infof(
			ctx, "sender: %v is not fit to send snapshot;"+" sender first index: %v, "+
				"coordinator first index: %v", req.DelegatedSender, replIdx, req.FirstIndex,
		)
