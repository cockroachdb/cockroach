...
func GetTargetsToCollocateRHSForMerge(
	...
	for len(finalRightVoters.Descriptors()) < numVoters && notCollocated() {
		...
		if len(leftMinusRightVoters) != 0 {
			...
			panic(fmt.Sprintf("programming error: unexpectedly ran out of valid stores to relocate RHS"+
				" voters to; LHS: %s, RHS: %s", leftRepls.Descriptors(), rightRepls.Descriptors()))
		}
	}

	for len(finalRightNonVoters.Descriptors()) < numNonVoters && notCollocated() {
