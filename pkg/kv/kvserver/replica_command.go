...
func (r *Replica) changeReplicasImpl(
	...
	desc, err = execExplicitPromotionsAndDemotions(ctx, r.store, desc, reason, details,
		targets.voterDemotions, targets.nonVoterPromotions)
	if err != nil {
		return nil, err
	}
...
// execExplicitPromotionsAndDemotions performs promotions of non-voting replicas
// to voting replicas, and likewise, demotions of voting replicas to non-voting
// replicas. If both these types of operations are being applied to a (voter,
// non-voter) pair, it will execute an atomic swap using joint consensus.
//
// This method returns the resulting range descriptor after these promotions or
// demotions are handled.
//
// NB: Promotions and demotions of LEARNER replicas are handled implicitly
// during the addition or removal of voting replicas and are not handled by this
// method.
func execExplicitPromotionsAndDemotions(
	ctx context.Context,
	s *Store,
	desc *roachpb.RangeDescriptor,
	reason kvserverpb.RangeLogEventReason,
