...
func (s *Store) relocateOne(
	...
	if len(args.targetsToAdd()) > 0 {
		...
		if args.targetType == voterTarget {
			...
			promotionTargetIdx := -1
			for i, nonVoter := range existingNonVoters {
				if nonVoter.StoreID == additionTarget.StoreID {
					promotionTargetIdx = i
					canPromoteNonVoter = true
					break
				}
			}
			if promotionTargetIdx >= 0 {
				// If we're about to execute a promotion of a non-voter then we want
				// that non-voter to be gone from `existingNonVoters`.
				existingNonVoters[promotionTargetIdx] = existingNonVoters[len(existingNonVoters)-1]
