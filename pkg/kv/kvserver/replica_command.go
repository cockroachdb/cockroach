...
func (r *Replica) atomicReplicationChange(
	...
	for i := 0; ; i++ {
		...
		// Protect against waiting too long and holding up the replication queue.
		if i > 0 && waited > 10*time.Second {
			return nil, errors.Newf("waited for %s and replication hasn't caught up with descriptor update", waited)
		}
