...
func execChangeReplicasTxn(
	...
	if err := args.db.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
		...
		if chgs.leaveJoint() && !desc.Replicas().InAtomicReplicationChange() {
			...
			//
			// NB: We return the `referenceDesc` and not the `desc` we just read from
			// KV, as it is possible that we raced with another `ChangeReplicas`
			// operation that has mutated the RangeDescriptor. In such a scenario, we
			// want all future `check`s inside `execChangeReplicasImpl` to fail.
			returnDesc = referenceDesc
			return nil
		}
