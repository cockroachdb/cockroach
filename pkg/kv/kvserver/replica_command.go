...
func (r *Replica) adminSplitWithDescriptor(
	...
	var splitKey roachpb.RKey
	{
		...
		if len(args.SplitKey) == 0 {
			...
			// the args.SplitKey, provided that it is a table key and there is more
			// than one safe split key in the range.
			if findNextSafeKey {
				...
				// the range. The 2nd key, after args.SplitKey is returned as a result.
				targetSize := int64(0)
				foundSplitKey, err = storage.MVCCFindSplitKey(
