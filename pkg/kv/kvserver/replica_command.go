...
func (r *Replica) followerSendSnapshot(
	...
	cleanup, err := r.store.reserveSendSnapshot(ctx, req, r.GetMVCCStats().Total())
	if err != nil {
		return err
	}
	defer cleanup()
	...
	header := kvserverpb.SnapshotRequest_Header{
		...
		RaftMessageRequest: kvserverpb.RaftMessageRequest{
			...
			Message: raftpb.Message{
				Type:     raftpb.MsgSnap,
				To:       uint64(req.Header.CoordinatorReplica.ReplicaID),
				From:     uint64(req.Header.RecipientReplica.ReplicaID),
