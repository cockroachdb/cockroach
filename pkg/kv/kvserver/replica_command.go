...
//    Learner replicas receive the log, but do not have voting rights. If new
//    voter replicas are being added, we first add "ephemeral" learners, catch
//    them up, and then promote them to voters. This is important for the
//    continued availability of the range throughout the replication change.
//    Learners are added (and removed) one by one due to a technicality
...
func maybeLeaveAtomicChangeReplicasAndRemoveEphemeralLearners(
	...
	for _, target := range targets {
		...
		desc, err = execChangeReplicasTxn(
			...
			[]internalReplicationChange{{target: target, typ: internalChangeTypeRemoveEphemeralLearner}},
		)
		if err != nil {
...
func (r *Replica) atomicReplicationChange(
	...
	for _, target := range chgs.VoterRemovals() {
		typ := internalChangeTypeRemoveEphemeralLearner
...
const (
	...
	// NB: can't remove multiple ephemeral learners at once (need to remove at least one
	// voter with them), see:
	// https://github.com/cockroachdb/cockroach/pull/40268
...
func prepareChangeReplicasTrigger(
	...
	if !chgs.leaveJoint() {
		...
		for _, chg := range chgs {
			...
			case internalChangeTypeRemoveEphemeralLearner:
