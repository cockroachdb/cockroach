...
func (r *Replica) followerSendSnapshot(
	...
	if sendThreshold > 0 {
		...
		ctx, sp = tracing.EnsureChildSpan(ctx, r.store.cfg.Tracer(),
			"overarching span for snapshots activities", tracing.WithRecording(tracingpb.RecordingVerbose))
		...
		defer func() {
			...
			if sendThreshold > 0 && sendDur > sendThreshold {
				...
				log.Infof(ctx, "%s took %s, exceeding threshold of %s:\n%s",
					"snapshot", sendDur, sendThreshold, sp.GetRecording(tracingpb.RecordingVerbose))
			}
			sp.Finish()
		}()
	}
