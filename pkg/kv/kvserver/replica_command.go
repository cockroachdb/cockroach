...
func (r *Replica) AdminMerge(
	...
	runMergeTxn := func(txn *kv.Txn) error {
		...
		// the descriptor early (i.e. on the read instead of the write) helps
		// prevent multiple concurrent Range merges from thrashing. Thrashing is
		// especially detrimental for Range merges because any restart results
		// in an abort (see the retry loop below), so thrashing can result in
		// indefinite livelock.
		//
