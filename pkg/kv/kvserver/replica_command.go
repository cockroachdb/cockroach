...
func (r *Replica) AdminMerge(
	...
	runMergeTxn := func(txn *kv.Txn) error {
		...
		if err := r.store.logMerge(ctx, txn, updatedLeftDesc, rightDesc, true); err != nil {
			return err
		}
	...
	if err := args.db.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
...

		{
			...
			err = recordRangeEventsInLog(
				ctx, txn, true /* added */, crt.Added(), crt.Desc, reason, details, true, args.logChange,
			)
			if err != nil {
				return err
			}
			err = recordRangeEventsInLog(
				ctx, txn, false /* added */, crt.Removed(), crt.Desc, reason, details, true, args.logChange,
			)
			if err != nil {
				return err
			}
