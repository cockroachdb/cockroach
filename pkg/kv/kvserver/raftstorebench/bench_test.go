...
// write amplification.
// It is not a traditional Go benchmark in that it does not respect the `b.N`
// parameter: instead, the number of operations is hard-coded in the testdata
// file. It is still useful to use the Go benchmark harness because this makes
// it straightforward to compare results between different configurations using
// benchstat.
//
// BenchmarkRaftStore skips itself unless invoked with test.benchtime=1x. This
// implies that it is not exercised during CI or nightly benchmark runs - an
// intentional choice, since the full suite takes many hours to run and requires
// hundreds of GBs of disk space.
func BenchmarkRaftStore(t *testing.B) {
	defer leaktest.AfterTest(t)()
	var benchTime string
	require.NoError(t, sniffarg.DoEnv("test.benchtime", &benchTime))
	if benchTime != "1x" {
		t.Skipf("not a traditional benchmark; use -test.benchtime=1x to run full suite")
	}
...
func maybeReportMetrics(t interface{}, res Result) {
	if b, ok := t.(*testing.B); ok {
		walkStruct(res, func(value interface{}, path ...string) {
			var f float64
			switch v := value.(type) {
			case float64:
				f = v
			...
			b.ReportMetric(f, strings.Join(path, "."))
		})
	}
}
