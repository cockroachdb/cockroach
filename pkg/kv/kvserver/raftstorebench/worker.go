...
func (w *worker) getDurabilityCallbackCount() int64 {
	return w.durabilityCallbackCount.Load()
}
	...
	for i := r.truncatedLogIndex + 1; i <= truncIndex; i++ {
		if w.o.cfg.SingleDel {
			if err := raftBatch.SingleClearEngineKey(
				storage.EngineKey{Key: r.rangeIDPrefixBuf.RaftLogKey(kvpb.RaftIndex(i))}); err != nil {
				return err
			}
		} else {
			if err := raftBatch.ClearUnversioned(r.rangeIDPrefixBuf.RaftLogKey(kvpb.RaftIndex(i)), storage.ClearOptions{}); err != nil {
				return err
			}
		}
	}
