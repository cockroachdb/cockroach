...
func Run(t T, cfg Config) Result {
	...
	for i := 0; i < cfg.NumWorkers; i++ {
		wg.Add(1)
		w := &worker{t: t, s: s, o: o, rng: rand.New(rand.NewSource(int64(i)))}
	...
	o.smEng.RegisterFlushCompletedCallback(func() {
		for i := 0; i < len(workers); i++ {
			n := workers[i].durabilityCallbackCount.Add(1)
			if workers[i].s.ops.Load() >= cfg.NumWrites {
	...
	goodputMBsec := float64(1e3*payloadBytes) / float64(1+duration) // 1e6/1e-9=1e3
...
func yamlEncode(t T, obj interface{}) string {
