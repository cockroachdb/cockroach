...
// walkStruct recursively walks through the fields of a struct and invokes the callback function.
func walkStruct(
	value interface{}, callback func(value interface{}, path ...string), path ...string,
) {
	...
	for i := 0; i < v.NumField(); i++ {
		...
		if field.Kind() == reflect.Struct {
			...
			walkStruct(field.Interface(), callback, currentPath...)
		} else if field.Kind() == reflect.Ptr && !field.IsNil() && field.Elem().Kind() == reflect.Struct {
			// Handle pointers to structs
			walkStruct(field.Elem().Interface(), callback, currentPath...)
		} else if field.CanInterface() {
			// Invoke the callback for primitive fields
			callback(field.Interface(), currentPath...)
		}
	}
}
