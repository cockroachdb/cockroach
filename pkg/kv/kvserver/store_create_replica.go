...
func (s *Store) tryGetOrCreateReplica(
	...
	if err := func() error {
		...
		//   allow us to tighten invariants. Additionally, knowing the
		//   ReplicaID for an unitialized range will allow a node to contact
		//   the raft group, and if the ReplicaID is stale, will allow the node to
		//   remove the HardState and RaftReplicaID. See
