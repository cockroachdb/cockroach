...
func TestEntries(t *testing.T) {
	...
	testutils.RunTrueAndFalse(t, "loosely-coupled", func(t *testing.T, looselyCoupled bool) {
		...
		populateLogs := func(from, to int) []roachpb.RaftIndex {
			var newIndexes []roachpb.RaftIndex
			for i := from; i < to; i++ {
		...
		for i, tc := range []struct {
			...
			if len(ents) != tc.expResultCount {
				...
				expHitLimit := roachpb.RaftIndex(ents[len(ents)-1].Index) < tc.hi-1
...
func TestReplicaRetryRaftProposal(t *testing.T) {
	...
	var wrongLeaseIndex roachpb.LeaseAppliedIndex // populated below

	tc.repl.mu.Lock()
	tc.repl.mu.proposalBuf.testing.leaseIndexFilter = func(p *ProposalData) (indexOverride roachpb.LeaseAppliedIndex) {
		if v := p.ctx.Value(magicKey{}); v != nil {
...
func TestReplicaRefreshMultiple(t *testing.T) {
	...
	assigned := false
	repl.mu.proposalBuf.testing.leaseIndexFilter = func(p *ProposalData) roachpb.LeaseAppliedIndex {
...
func TestReplicaReproposalWithNewLeaseIndexError(t *testing.T) {
	...
	tc.repl.mu.Lock()
	tc.repl.mu.proposalBuf.testing.leaseIndexFilter = func(p *ProposalData) roachpb.LeaseAppliedIndex {
		if v := p.ctx.Value(magicKey{}); v != nil {
			...
			case 1:
				...
				wrongLeaseIndex := roachpb.LeaseAppliedIndex(1)
				return wrongLeaseIndex
			default:
...
func TestFailureToProcessCommandClearsLocalResult(t *testing.T) {
	...
	r := tc.repl
	r.mu.Lock()
	r.mu.proposalBuf.testing.leaseIndexFilter = func(p *ProposalData) roachpb.LeaseAppliedIndex {
...
func (q *testQuiescer) raftLastIndexRLocked() roachpb.RaftIndex {
	return q.lastIndex
}
