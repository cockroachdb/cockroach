...
// TestReplicaRangeBoundsChecking tests two behaviors:
// (1): Upon a Range split, where the RHS has not yet been
...
//      the store still has the correct range locally, it can avoid
//      an additional KV rpc hop bubbling the error - by instead 
//      server side retrying the request with updated Range information
//      in the BatchRequest. We expect no error to be returned here,
