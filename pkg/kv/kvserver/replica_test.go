...
func TestReplicaAsyncIntentResolutionOn1PC(t *testing.T) {
	...
	testutils.RunTrueAndFalse(t, "external", func(t *testing.T, external bool) {
		...
		require.NoError(t, err)
		successfulOnePCBefore := store.Metrics().Successful1PC.Count()
		failedOnePCBefore := store.Metrics().Failed1PC.Count()
		...
		successfulOnePCAfter := store.Metrics().Successful1PC.Count()
		failedOnePCAfter := store.Metrics().Failed1PC.Count()
