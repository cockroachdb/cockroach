...
func TestReplicaRetryRaftProposal(t *testing.T) {
	...
	var wrongLeaseIndex roachpb.LeaseSequence // populated below

	tc.repl.mu.Lock()
	tc.repl.mu.proposalBuf.testing.leaseIndexFilter = func(p *ProposalData) (indexOverride roachpb.LeaseSequence) {
		if v := p.ctx.Value(magicKey{}); v != nil {
...
func TestReplicaRefreshMultiple(t *testing.T) {
	...
	assigned := false
	repl.mu.proposalBuf.testing.leaseIndexFilter = func(p *ProposalData) roachpb.LeaseSequence {
...
func TestReplicaReproposalWithNewLeaseIndexError(t *testing.T) {
	...
	tc.repl.mu.Lock()
	tc.repl.mu.proposalBuf.testing.leaseIndexFilter = func(p *ProposalData) roachpb.LeaseSequence {
		if v := p.ctx.Value(magicKey{}); v != nil {
			...
			case 1:
				...
				wrongLeaseIndex := roachpb.LeaseSequence(1)
				return wrongLeaseIndex
			default:
...
func TestFailureToProcessCommandClearsLocalResult(t *testing.T) {
	...
	r := tc.repl
	r.mu.Lock()
	r.mu.proposalBuf.testing.leaseIndexFilter = func(p *ProposalData) roachpb.LeaseSequence {
