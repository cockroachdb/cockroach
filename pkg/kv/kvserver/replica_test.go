...
// TestOverlapsStateEngineKeys verifies that the function
// overlapsStateEngineKeys() successfully catches any overlap with
// stateEngine keys.
func TestOverlapsStateEngineKeys(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)
	...
	testCases := []struct {
		...
		{span: s(keys.RangeGCThresholdKey(1), keys.RangeVersionKey(1))},
		{span: s(keys.RangeTombstoneKey(1).Next(), keys.RaftReplicaIDKey(1))},
		{span: s(keys.RaftReplicaIDKey(1).Next(), keys.LocalRangeIDPrefix.AsRawKey().PrefixEnd())},
		...
		{span: s(keys.RangeLeaseKey(1), nil)},
		...
		{span: s(nil, keys.LocalRangeIDPrefix.AsRawKey().Next())},
		{span: s(nil, keys.RaftLogKey(1, 1))},
		{span: s(nil, keys.RangeLeaseKey(1))},
	...
	for _, tc := range testCases {
		t.Run("", func(t *testing.T) {
			err := overlapsStateEngineKeys(spanset.TrickySpan(tc.span))
			require.Equal(t, tc.notOk, err != nil, tc.span)
		})
	}
}
