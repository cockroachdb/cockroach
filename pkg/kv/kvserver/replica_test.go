...
func TestReplicaRefreshMultiple(t *testing.T) {
	...
	proposal, pErr := repl.requestToProposal(ctx, incCmdID, &ba, allSpansGuard(), repl.CurrentLeaseStatus(ctx), uncertainty.Interval{})
	if pErr != nil {
		t.Fatal(pErr)
	}
...
func TestReplicaEvaluationNotTxnMutation(t *testing.T) {
	...
	batch, _, _, _, pErr := tc.repl.evaluateWriteBatch(ctx, makeIDKey(), &ba, allSpansGuard(), uncertainty.Interval{})
	defer batch.Close()
	if pErr != nil {
		t.Fatal(pErr)
	}
...
func TestContainsEstimatesClampProposal(t *testing.T) {
	...
	someRequestToProposal := func(tc *testContext, ctx context.Context) *ProposalData {
		...
		ba.Add(&req)
		proposal, err := tc.repl.requestToProposal(ctx, cmdIDKey, &ba, allSpansGuard(), tc.repl.CurrentLeaseStatus(ctx), uncertainty.Interval{})
		if err != nil {
			t.Error(err)
		}
		return proposal
