...
func TestReplicaGossipConfigsOnLease(t *testing.T) {
	...
	key := keys.SystemTenantKeyGen.TablePrefix(keys.MaxSystemConfigDescID)
...
func TestReplicaNoGossipConfig(t *testing.T) {
	...
	key := keys.SystemTenantKeyGen.TablePrefix(keys.MaxReservedDescID)
...
func TestReplicaTelemetryCounterForPushesDueToClosedTimestamp(t *testing.T) {
	defer leaktest.AfterTest(t)()
	ctx := context.Background()
	keyA := append(keys.SystemTenantKeyGen.TablePrefix(math.MaxUint32), 'a')
