			...
			datadriven.RunTest(
				t, path, func(t *testing.T, d *datadriven.TestData) string {
					...
					case "send-heartbeats":
						...
						heartbeats, err := rs.getHeartbeatsToSend(now, livenessInterval)
						return fmt.Sprintf("heartbeats:\n%s", printMsgs(heartbeats))

					case "handle-heartbeats":
						msgs := parseMsgs(t, d, rs.storeID)
						responses, err := ss.handleHeartbeats(msgs)
						if err != nil {
							d.Fatalf(t, "error handling heartbeats: %v", err)
						}
						return fmt.Sprintf("responses:\n%s", printMsgs(responses))

					case "handle-heartbeat-responses":
						msgs := parseMsgs(t, d, rs.storeID)
						err := rs.handleHeartbeatResponses(msgs)
						if err != nil {
							d.Fatalf(t, "error handling heartbeat responses: %v", err)
						}
						return ""

					case "withdraw-support":
						now := parseTimestamp(t, d, "now")
						err := ss.withdrawSupport(hlc.ClockTimestamp(now))
						if err != nil {
							d.Fatalf(t, "error withdrawing support: %v", err)
						}
						return ""
					...
					case "restart":
						...
						err := rs.incrementMaxEpoch()
						if err != nil {
							d.Fatalf(t, "error incrementing MaxEpoch: %v", err)
						}
						return ""
