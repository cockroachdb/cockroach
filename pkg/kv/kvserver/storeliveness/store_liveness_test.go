		...
		t, datapathutils.TestDataPath(t), func(t *testing.T, path string) {
			...
			datadriven.RunTest(
				t, path, func(t *testing.T, d *datadriven.TestData) string {
					...
					case "restart":
						clear(rs.requesterState.supportFrom)
						rs.requesterState.meta.Reset()
						clear(ss.supporterState.supportFor)
						ss.supporterState.meta.Reset()
						if err := onRestart(ctx, rs, ss, engine); err != nil {
							t.Errorf("persisting data while restarting failed: %v", err)
						}
						return ""
...
func onRestart(
	...
	if err := ss.load(ctx, engine); err != nil {
		return err
	}
	if err := rs.load(ctx, engine); err != nil {
		return err
	}
	rsfu := rs.checkOutUpdate()
