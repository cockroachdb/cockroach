			...
			datadriven.RunTest(
				t, path, func(t *testing.T, d *datadriven.TestData) string {
					...
					case "send-heartbeats":
						...
						update := rs.checkOutUpdate()
						heartbeats := update.getHeartbeatsToSend(storeID, now, livenessInterval)
						rs.checkInUpdate(update)
					...
					case "handle-messages":
						...
						rsUpdate := rs.checkOutUpdate()
						ssUpdate := ss.checkOutUpdate()
						for _, msg := range msgs {
							switch msg.Type {
							case slpb.MsgHeartbeat:
								responses = append(responses, ssUpdate.handleHeartbeat(msg))
							case slpb.MsgHeartbeatResp:
								rsUpdate.handleHeartbeatResponse(msg)
							default:
								log.Errorf(context.Background(), "unexpected message type: %v", msg.Type)
							}
						}
						rs.checkInUpdate(rsUpdate)
						ss.checkInUpdate(ssUpdate)
						if len(responses) > 0 {
					...
					case "withdraw-support":
						now := parseTimestamp(t, d, "now")
						update := ss.checkOutUpdate()
						update.withdrawSupport(hlc.ClockTimestamp(now))
						ss.checkInUpdate(update)
						return ""
