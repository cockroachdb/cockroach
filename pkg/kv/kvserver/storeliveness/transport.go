...
func NewTransport(
	...
	// Start background goroutine to act as the transport sender coordinator.
	// It is responsible for instructing the sendQueues to send their messages.
	...
	UseHeartbeatCoordinatorEnabled.SetOnChange(&settings.SV, func(ctx context.Context) {
		...
		//
		// By signaling q.sendMessages here, we unblock the goroutines, allowing
		// them to complete their current iteration and re-read the setting on the
		// next iteration. They will then see the coordinator is disabled and adjust
		// their select to wait on the messages channel (directMessages = q.messages)
		// instead, enabling direct message processing.
		//
		...
		if !UseHeartbeatCoordinatorEnabled.Get(&settings.SV) {
			t.queues.Range(func(nodeID roachpb.NodeID, q *sendQueue) bool {
				select {
...
func (t *Transport) processQueue(
	...
	for {
		...
		// heartbeat sends. Once the smearing has battle-tested, remove this and
		// default to using the smeared heartbeat sends approach (no more fallback).
		useHeartbeatCoordinator := UseHeartbeatCoordinatorEnabled.Get(&t.settings.SV)
		...
		case msg := <-directMessages:
			// Direct message received - add it to batch, then proceed to batching logic below.
