...
func NewTransport(
	...
	HeartbeatSmearingEnabled.SetOnChange(&settings.SV, func(ctx context.Context) {
		// When the smearing is disabled (true → false), we must wake up all
		...
		// would be nil when smearing was enabled). Since support_manager stops
		...
		// currently waiting on q.sendMessages from when smearing was enabled),
		// allowing them to complete their current iteration and re-read the setting
		// on the next iteration. They will then see the smearing is disabled and
		// adjust their select to wait on the directSend signal channel instead,
		// enabling direct message processing.
		//
		// When the smearing is enabled (false → true), we don't signal because
		// messages should accumulate and wait for the smearing sender to call
