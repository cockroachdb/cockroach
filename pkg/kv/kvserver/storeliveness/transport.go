...
func (t *Transport) startProcessNewQueue(
	...
	worker := func(ctx context.Context) {
		q, existingQueue := t.getQueue(toNodeID)
		if !existingQueue {
			log.Liveness.Fatalf(ctx, "queue for n%d does not exist", toNodeID)
		}
		defer cleanup()
		...
		if err != nil {
			log.Liveness.Warningf(ctx, "creating stream client for node %d failed: %s", toNodeID, err)
			return
		}

		if err = t.processQueue(q, stream); err != nil {
			log.Liveness.Warningf(ctx, "processing outgoing queue to node %d failed: %s:", toNodeID, err)
		}
	}
