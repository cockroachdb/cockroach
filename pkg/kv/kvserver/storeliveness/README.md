...
or not is expressed in terms of pair-wise _support_ between stores.
...
a few seconds into the future. See `storelivenesspb/requester_state` for 
...
`storelivenesspb/supporter_state` for implementation details.
...
one-way transition. See `storelivenesspb/supporter_state` for implementation 
details.
...
`storelivenesspb/service.go` for details on why each individual field needs to 
be persisted or not. Notably, support received from other stores is not 
...
`storelivenesspb/service.go` for details on how each persisted field is used.
...
- **Message loss and re-ordering**: Lost heartbeats are simply resend on the next
...
(see `MessageBatch.Now` in `storelivenesspb/service.go`).
...
liveness over to the store liveness layer. See `raft/fortify.go` for more
details on the interaction between store liveness and Raft.
