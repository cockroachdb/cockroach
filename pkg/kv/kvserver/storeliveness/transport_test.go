...
func TestTransportHeartbeatSmearingBatching(t *testing.T) {
	...
	node2Message := slpb.Message{Type: slpb.MsgHeartbeat, From: node1Sender, To: node2Receiver}
	node3Message := slpb.Message{Type: slpb.MsgHeartbeat, From: node1Sender, To: node3Receiver}
	...
	// Now trigger SendAllEnqueuedMessages - heartbeat smearing should batch and send.
	tt.transports[node1Sender.NodeID].SendAllEnqueuedMessages(ctx)
...
func TestTransportSmearingLogicalBehaviour(t *testing.T) {
	...
	HeartbeatCoordinatorRefresh.Override(ctx, &st.SV, 250*time.Millisecond)
	HeartbeatCoordinatorSmear.Override(ctx, &st.SV, 50*time.Millisecond)
	...
	// Wait for all messages using SucceedsSoon pattern.
	for i := 0; i < numReceivers; i++ {
		idx := i
	...
	// are delivered in a tight window.
	HeartbeatCoordinatorRefresh.Override(ctx, &st.SV, 0)
	...
	receiveTimes2 := make([]time.Time, numReceivers)
	tt.transports[sender.NodeID].SendAllEnqueuedMessages(ctx)
	...
	for i := 0; i < numReceivers; i++ {
		...
		testutils.SucceedsSoon(t, func() error {
			...
			case received := <-handlers[idx].messages:
				require.NotNil(t, received.msg)
				receiveTimes2[idx] = received.receivedAt
				return nil
			default:
				return errors.Newf("message to receiver %d not yet delivered", idx)
			}
	...
	sort.Slice(receiveTimes2, func(i, j int) bool {
		return receiveTimes2[i].Before(receiveTimes2[j])
	})

	timeSpan2 := receiveTimes2[len(receiveTimes2)-1].Sub(receiveTimes2[0])
	require.Less(t, timeSpan2, 50*time.Millisecond,
...
func TestTransportClusterSettingToggle(t *testing.T) {
	...
	// Start with heartbeat smearing enabled (messages wait for
	...
	// directSendSignal=nil, the OnChange callback should wake it up so it can
	...
	// Send a message with signal to ensure processQueue completes at least
	// one more iteration and picks up the new setting value.
	msg := slpb.Message{Type: slpb.MsgHeartbeat, From: sender, To: receiver}
	require.True(t, tt.transports[sender.NodeID].EnqueueMessage(ctx, msg))
	tt.transports[sender.NodeID].SendAllEnqueuedMessages(ctx) // can't signal directly with 0 msgs
	testutils.SucceedsSoon(t, func() error {
		select {
		...
		return errors.New("waiting for message")
	})
	// Note: this message is a test test-only synchronization mechanism.
	// In production, `support_manager.go` will call `SendAllEnqueuedMessages`
	// when the setting changes.

	// Wait for the queue to be fully drained and processQueue to return to
	// waiting state. We verify this by checking that the send queue is empty.
	testutils.SucceedsSoon(t, func() error {
		if tt.transports[sender.NodeID].metrics.SendQueueSize.Value() != 0 {
			return errors.New("queue not empty yet")
		}
		return nil
	})
