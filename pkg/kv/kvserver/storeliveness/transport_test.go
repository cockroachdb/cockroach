...
// TestTransportSmearing verifies that heartbeat smearing spreads signals across
...
func TestTransportSmearing(t *testing.T) {
	...
	// Trigger SendAllEnqueuedMessages - heartbeat smearing should spread signals over time.
	start := timeutil.Now()
	tt.transports[sender.NodeID].SendAllEnqueuedMessages(ctx)
	...
	require.Greater(t, timeSpan, 5*time.Millisecond,
		"smearing should spread signals over time")

	// But not too long (should complete reasonably quickly).
	require.Less(t, timeSpan, 100*time.Millisecond,
		"smearing should complete in reasonable time")
