...
func ensureAllToAllSupport(t *testing.T, tc *testcluster.TestCluster) {
	checkSupportFn := func(
		...
		// we request support-from first to start the heartbeats.
...
func TestStoreLivenessRestart(t *testing.T) {
	...
	checkSupportFn := func(
		...
		// support from the remote node.
		if remoteID.NodeID == stoppedNode && localID.NodeID != stoppedNode {
			checkSupportFrom(t, localSM, remoteID, false /* supportExpected */)
		}
		// If the local store is the stopped one, the remote store should withdraw
		// support for the local node.
		if localID.NodeID == stoppedNode && remoteID.NodeID != stoppedNode {
			checkSupportFor(t, remoteSM, localID, false /* supportExpected */)
		}
		// All other pairs of stores should have support for/from each other.
		if localID.NodeID != stoppedNode && remoteID.NodeID != stoppedNode {
			checkSupportFrom(t, localSM, remoteID, true /* supportExpected */)
			checkSupportFor(t, remoteSM, localID, true /* supportExpected */)
		}
...
func TestStoreLivenessDiskStall(t *testing.T) {
	...
	checkSupportFn := func(
		...
		// If the remote store is the stalled one, the local store should lose
		// support from the remote node.
		if remoteID == stalledStore && localID != stalledStore {
			checkSupportFrom(t, localSM, remoteID, false /* supportExpected */)
		}
		// If the local store is the stalled one, the remote store should withdraw
		// support for the local node.
		if localID == stalledStore && remoteID != stalledStore {
			checkSupportFor(t, remoteSM, localID, false /* supportExpected */)
		}
		// All other pairs of stores should have support for/from each other.
		if localID != stalledStore && remoteID != stalledStore {
