...
// recentlyQueriedState encodes all the possible values of recentlyQueried. It
// includes three values to make sure stores are not marked as idle prematurely.
// A store transitions from active to inactive every IdleSupportFromInterval and
// back to active upon being queried in a SupportFrom call. If another
// IdleSupportFromInterval expires after a store was marked as inactive, it will
// be marked as idle and will not be sent heartbeats until it transitions to
// active again.
type recentlyQueriedState byte

const (
	// active indicates that the store has been queried in a SupportFrom call
	// recently (within IdleSupportFromInterval).
	active recentlyQueriedState = iota
