...
func logSupportFromChange(ctx context.Context, ss slpb.SupportState, ssNew slpb.SupportState) {
	if ss.Epoch == ssNew.Epoch {
		if ss.Expiration.IsEmpty() {
			log.Infof(ctx, "received support from %v", supportChangeStr(ss, ssNew))
		} else {
			log.VInfof(ctx, 3, "extended support from %v", supportChangeStr(ss, ssNew))
		}
	} else {
		assert(ss.Epoch < ssNew.Epoch, "epoch regressed")
		log.Infof(ctx, "lost support from %v", supportChangeStr(ss, ssNew))
	}
}
...
func supportChangeStr(current slpb.SupportState, previous slpb.SupportState) string {
	return fmt.Sprintf("store %v; current = %+v, previous = %+v", current.Target, current, previous)
}
