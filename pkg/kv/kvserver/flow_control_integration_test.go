...
func TestFlowControlBasicV2(t *testing.T) {
	...
	testutils.RunTrueAndFalse(t, "apply_to_all", func(t *testing.T, applyToAll bool) {
		testutils.RunValues(t, "v2_enabled_when_leader_level", []kvflowcontrol.V2EnabledWhenLeaderLevel{
			...
			testutils.RunTrueAndFalse(t, "always-enqueue", func(t *testing.T, alwaysEnqueue bool) {
				...
				h.init(applyToElasticOrAll(applyToAll))
				defer h.close(makeV2FlowTestFileName(v2EnabledWhenLeaderLevel, applyToAll, "basic"))
...
func TestFlowControlRangeSplitMergeV2(t *testing.T) {
	...
	testutils.RunTrueAndFalse(t, "apply_to_all", func(t *testing.T, applyToAll bool) {
		testutils.RunValues(t, "v2_enabled_when_leader_level", []kvflowcontrol.V2EnabledWhenLeaderLevel{
			...
			h := newFlowControlTestHelperV2(t, tc, v2EnabledWhenLeaderLevel)
			h.init(applyToElasticOrAll(applyToAll))
			defer h.close(makeV2FlowTestFileName(v2EnabledWhenLeaderLevel, applyToAll, "split_merge"))
...
`)
			...
			h.log("sending 2MiB put request to post-split LHS")
			h.put(ctx, k, 2<<20 /* 2MiB */, pri)
			h.log("sent 2MiB put request to post-split LHS")

			h.log("sending 3MiB put request to post-split RHS")
			h.put(ctx, roachpb.Key(right.StartKey), 3<<20 /* 3MiB */, pri)
			h.log("sent 3MiB put request to post-split RHS")
...
`, "range_id", "stream_count")
			...
			h.log("sending 4MiB put request to post-merge range")
			h.put(ctx, roachpb.Key(merged.StartKey), 4<<20 /* 4MiB */, pri)
			h.log("sent 4MiB put request to post-merged range")
...
func TestFlowControlBlockedAdmissionV2(t *testing.T) {
	...
	testutils.RunTrueAndFalse(t, "apply_to_all", func(t *testing.T, applyToAll bool) {
		testutils.RunValues(t, "v2_enabled_when_leader_level", []kvflowcontrol.V2EnabledWhenLeaderLevel{
			...
			h := newFlowControlTestHelperV2(t, tc, v2EnabledWhenLeaderLevel)
			h.init(applyToElasticOrAll(applyToAll))
			defer h.close(makeV2FlowTestFileName(v2EnabledWhenLeaderLevel, applyToAll, "blocked_admission"))
...
func TestFlowControlAdmissionPostSplitMergeV2(t *testing.T) {
	...
	testutils.RunTrueAndFalse(t, "apply_to_all", func(t *testing.T, applyToAll bool) {
		testutils.RunValues(t, "v2_enabled_when_leader_level", []kvflowcontrol.V2EnabledWhenLeaderLevel{
			...
			h := newFlowControlTestHelperV2(t, tc, v2EnabledWhenLeaderLevel)
			h.init(applyToElasticOrAll(applyToAll))
			defer h.close(makeV2FlowTestFileName(v2EnabledWhenLeaderLevel, applyToAll, "admission_post_split_merge"))
			...
			h.log("sending put request to pre-split range")
			h.put(ctx, k, 1<<20 /* 1MiB */, pri)
			h.put(ctx, k.Next(), 1<<20 /* 1MiB */, pri)
			h.log("sent put request to pre-split range")
...
func TestFlowControlCrashedNodeV2(t *testing.T) {
	...
	testutils.RunTrueAndFalse(t, "apply_to_all", func(t *testing.T, applyToAll bool) {
		testutils.RunValues(t, "v2_enabled_when_leader_level", []kvflowcontrol.V2EnabledWhenLeaderLevel{
			...
			h := newFlowControlTestHelperV2(t, tc, v2EnabledWhenLeaderLevel)
			h.init(applyToElasticOrAll(applyToAll))
			defer h.close(makeV2FlowTestFileName(v2EnabledWhenLeaderLevel, applyToAll, "crashed_node"))
...
func TestFlowControlRaftSnapshotV2(t *testing.T) {
	...
	testutils.RunTrueAndFalse(t, "apply_to_all", func(t *testing.T, applyToAll bool) {
		testutils.RunValues(t, "v2_enabled_when_leader_level", []kvflowcontrol.V2EnabledWhenLeaderLevel{
			...
			h := newFlowControlTestHelperV2(t, tc, v2EnabledWhenLeaderLevel)
			h.init(applyToElasticOrAll(applyToAll))
			defer h.close(makeV2FlowTestFileName(v2EnabledWhenLeaderLevel, applyToAll, "raft_snapshot"))
			...
			repl := store.LookupReplica(roachpb.RKey(k))
			require.NotNil(t, repl)
			h.enableVerboseRaftMsgLoggingForRange(repl.RangeID)
			h.waitForConnectedStreams(ctx, repl.RangeID, 5, 0 /* serverIdx */)
...
func TestFlowControlRaftMembershipV2(t *testing.T) {
	...
	testutils.RunTrueAndFalse(t, "apply_to_all", func(t *testing.T, applyToAll bool) {
		testutils.RunValues(t, "v2_enabled_when_leader_level", []kvflowcontrol.V2EnabledWhenLeaderLevel{
			...
			h := newFlowControlTestHelperV2(t, tc, v2EnabledWhenLeaderLevel)
			h.init(applyToElasticOrAll(applyToAll))
			defer h.close(makeV2FlowTestFileName(v2EnabledWhenLeaderLevel, applyToAll, "raft_membership"))
	...
	testutils.RunTrueAndFalse(t, "apply_to_all", func(t *testing.T, applyToAll bool) {
		testutils.RunValues(t, "v2_enabled_when_leader_level", []kvflowcontrol.V2EnabledWhenLeaderLevel{
			...
			testutils.RunTrueAndFalse(t, "transfer-lease-first", func(t *testing.T, transferLeaseFirst bool) {
				...
				// first.
				defer h.close(makeV2FlowTestFileName(v2EnabledWhenLeaderLevel, applyToAll, "raft_membership_remove_self"))
...
func TestFlowControlClassPrioritizationV2(t *testing.T) {
	...
	testutils.RunTrueAndFalse(t, "apply_to_all", func(t *testing.T, applyToAll bool) {
		testutils.RunValues(t, "v2_enabled_when_leader_level", []kvflowcontrol.V2EnabledWhenLeaderLevel{
			...
			h := newFlowControlTestHelperV2(t, tc, v2EnabledWhenLeaderLevel)
			h.init(applyToElasticOrAll(applyToAll))
			defer h.close(makeV2FlowTestFileName(v2EnabledWhenLeaderLevel, applyToAll, "class_prioritization"))
...
func TestFlowControlQuiescedRangeV2(t *testing.T) {
	...
	testutils.RunTrueAndFalse(t, "apply_to_all", func(t *testing.T, applyToAll bool) {
		testutils.RunValues(t, "v2_enabled_when_leader_level", []kvflowcontrol.V2EnabledWhenLeaderLevel{
			...
			h := newFlowControlTestHelperV2(t, tc, v2EnabledWhenLeaderLevel)
			h.init(applyToElasticOrAll(applyToAll))
			defer h.close(makeV2FlowTestFileName(v2EnabledWhenLeaderLevel, applyToAll, "quiesced_range"))
...
func TestFlowControlUnquiescedRangeV2(t *testing.T) {
	...
	testutils.RunTrueAndFalse(t, "apply_to_all", func(t *testing.T, applyToAll bool) {
		testutils.RunValues(t, "v2_enabled_when_leader_level", []kvflowcontrol.V2EnabledWhenLeaderLevel{
			...
			h := newFlowControlTestHelperV2(t, tc, v2EnabledWhenLeaderLevel)
			h.init(applyToElasticOrAll(applyToAll))
			defer h.close(makeV2FlowTestFileName(v2EnabledWhenLeaderLevel, applyToAll, "unquiesced_range"))
...
func TestFlowControlTransferLeaseV2(t *testing.T) {
	...
	testutils.RunTrueAndFalse(t, "apply_to_all", func(t *testing.T, applyToAll bool) {
		testutils.RunValues(t, "v2_enabled_when_leader_level", []kvflowcontrol.V2EnabledWhenLeaderLevel{
			...
			h := newFlowControlTestHelperV2(t, tc, v2EnabledWhenLeaderLevel)
			h.init(applyToElasticOrAll(applyToAll))
			defer h.close(makeV2FlowTestFileName(v2EnabledWhenLeaderLevel, applyToAll, "transfer_lease"))
...
func TestFlowControlLeaderNotLeaseholderV2(t *testing.T) {
	...
	testutils.RunTrueAndFalse(t, "apply_to_all", func(t *testing.T, applyToAll bool) {
		testutils.RunValues(t, "v2_enabled_when_leader_level", []kvflowcontrol.V2EnabledWhenLeaderLevel{
			...
			h := newFlowControlTestHelperV2(t, tc, v2EnabledWhenLeaderLevel)
			h.init(applyToElasticOrAll(applyToAll))
			defer h.close(makeV2FlowTestFileName(v2EnabledWhenLeaderLevel, applyToAll, "leader_not_leaseholder"))
...
func TestFlowControlGranterAdmitOneByOneV2(t *testing.T) {
	...
	testutils.RunTrueAndFalse(t, "apply_to_all", func(t *testing.T, applyToAll bool) {
		testutils.RunValues(t, "v2_enabled_when_leader_level", []kvflowcontrol.V2EnabledWhenLeaderLevel{
			...
			h := newFlowControlTestHelperV2(t, tc, v2EnabledWhenLeaderLevel)
			h.init(applyToElasticOrAll(applyToAll))
			defer h.close(makeV2FlowTestFileName(v2EnabledWhenLeaderLevel, applyToAll, "granter_admit_one_by_one"))
...
func applyToElasticOrAll(all bool) kvflowcontrol.ModeT {
	if all {
		return kvflowcontrol.ApplyToAll
	}
	return kvflowcontrol.ApplyToElastic
}
...
// makeV2FlowTestFileName is a utility function which returns an updated
// filename for the testdata file based on the v2EnabledWhenLeaderLevel.
func makeV2FlowTestFileName(
	v2EnabledWhenLeaderLevel kvflowcontrol.V2EnabledWhenLeaderLevel, applyToAll bool, filename string,
) string {
	var enabledPart, modePart string
	...
	if applyToAll {
		modePart = "_apply_to_all"
	} else {
		modePart = "_apply_to_elastic"
	}
	return filename + enabledPart + modePart
}
...
func BenchmarkFlowControlV2Basic(b *testing.B) {
	...
	testutils.RunTrueAndFalse(b, "apply_to_all", func(b *testing.B, applyToAll bool) {
