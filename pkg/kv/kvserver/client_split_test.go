...
func TestUnsplittableRange(t *testing.T) {
	...
	testutils.SucceedsSoon(t, func() error {
		...
		// exit the split queue purgatory. We do this in a loop, as we have to
		// wait for the protected timestamp subsystem to release a timestamp at
		// which we get to actually remove the data.
		//
		// TODO(lunevalex): this test should probably use a TestCluster which offers
		// a SQLConn which we can use to properly set the cluster settings. The below
		// is a hack, we shouldn't call Override in fully running systems.
		protectedts.PollInterval.Override(&serv.ClusterSettings().SV, 300*time.Millisecond)
		...
		select {
		case splitQueuePurgatoryChan <- timeutil.Now():
		default:
		}
...
func TestStoreCapacityAfterSplit(t *testing.T) {
	...
	testutils.SucceedsSoon(t, func() error {
		...
		if !repl.OwnsValidLease(ctx, tc.Servers[1].Clock().Now()) {
