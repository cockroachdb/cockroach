...
func TestStoreSplitAbortSpan(t *testing.T) {
	...
	// exact timestamp because of unpredictable logical ticks. We are using a live
	// clock so need to a dd a reasonable buffer in tsFresh for this request to
	// execute to avoid flakiness.
	tsFresh := hlc.Timestamp{WallTime: s.Clock().Now().WallTime - thresh + int64(5*time.Second)}
