...
func TestStoreRangeSplitIdempotency(t *testing.T) {
	...
	ms, err := kvstorage.MakeLoader(originalRepl.RangeID).LoadMVCCStats(ctx, store.TODOEngine())
	if err != nil {
		t.Fatal(err)
	}
	...
	left, err := kvstorage.MakeLoader(originalRepl.RangeID).LoadMVCCStats(ctx, store.TODOEngine())
	if err != nil {
		t.Fatal(err)
	}
	lKeyBytes, lValBytes := left.KeyBytes, left.ValBytes
	right, err := kvstorage.MakeLoader(newRng.RangeID).LoadMVCCStats(ctx, store.TODOEngine())
	if err != nil {
		t.Fatal(err)
	}
...
func TestStoreRangeSplitMergeStats(t *testing.T) {
	...
	ms, err := kvstorage.MakeLoader(repl.RangeID).LoadMVCCStats(ctx, snap)
	require.NoError(t, err)
	...
	snap = store.TODOEngine().NewSnapshot()
	defer snap.Close()
	msLeft, err := kvstorage.MakeLoader(repl.RangeID).LoadMVCCStats(ctx, snap)
	require.NoError(t, err)
	replRight := store.LookupReplica(splitKey)
	msRight, err := kvstorage.MakeLoader(replRight.RangeID).LoadMVCCStats(ctx, snap)
	require.NoError(t, err)
	...
	msMerged, err := kvstorage.MakeLoader(repl.RangeID).LoadMVCCStats(ctx, snap)
	require.NoError(t, err)
		...
		testutils.RunTrueAndFalse(t, "recompute", func(t *testing.T, recompute bool) {
			testutils.RunTrueAndFalse(t, "maxCount", func(t *testing.T, maxCount bool) {
				testutils.RunTrueAndFalse(t, "maxBytes", func(t *testing.T, maxBytes bool) {
					...
					snap := store.TODOEngine().NewSnapshot()
					defer snap.Close()
					lhsStats, err := kvstorage.MakeLoader(lhsRepl.RangeID).LoadMVCCStats(ctx, snap)
					require.NoError(t, err)
					rhsRepl := store.LookupReplica(splitKeyAddr)
					rhsStats, err := kvstorage.MakeLoader(rhsRepl.RangeID).LoadMVCCStats(ctx, snap)
					require.NoError(t, err)
					...
					snap = store.TODOEngine().NewSnapshot()
					defer snap.Close()
					lhs1Stats, err := kvstorage.MakeLoader(lhsRepl.RangeID).LoadMVCCStats(ctx, snap)
					require.NoError(t, err)
					lhs2Repl := store.LookupReplica(splitKeyLeftAddr)
					lhs2Stats, err := kvstorage.MakeLoader(lhs2Repl.RangeID).LoadMVCCStats(ctx, snap)
					require.NoError(t, err)
					rhs1Stats, err := kvstorage.MakeLoader(rhsRepl.RangeID).LoadMVCCStats(ctx, snap)
					require.NoError(t, err)
					rhs2Repl := store.LookupReplica(splitKeyRightAddr)
					rhs2Stats, err := kvstorage.MakeLoader(rhs2Repl.RangeID).LoadMVCCStats(ctx, snap)
					require.NoError(t, err)
...
func fillRange(
	...
	var key []byte
	for {
		ms, err := kvstorage.MakeLoader(rangeID).LoadMVCCStats(context.Background(), store.TODOEngine())
		if err != nil {
			t.Fatal(err)
		}
		keyBytes, valBytes := ms.KeyBytes, ms.ValBytes
...
func TestSplitWithExternalFilesFastStats(t *testing.T) {
	...
	for _, fastStats := range []bool{true, false} {
		...
		t.Run(testName, func(t *testing.T) {
			...
			originalRepl := store.LookupReplica(roachpb.RKey(splitKey))
			require.NotNil(t, originalRepl)
			origStats, err := kvstorage.MakeLoader(originalRepl.RangeID).LoadMVCCStats(ctx, store.TODOEngine())
			require.NoError(t, err)
