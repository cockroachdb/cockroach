...
func TestSpanConfigUpdateAppearsOnReplica(t *testing.T) {
	...
	spanconfig.EnabledSetting.Override(ctx, &st.SV, true)
...
func TestSpanConfigUpdateInducesSplit(t *testing.T) {
	...
	leftSpan, rightSpan := desc.RSpan().AsRawSpanWithNoLocals().SplitOnKey(desc.StartKey.AsRawKey().Next().Next())
	...
	testutils.SucceedsSoon(t, func() error {
		...
		if rightRange.RangeID == scratchRng.RangeID {
