...
func TestStoreRangeSplitAtIllegalKeys(t *testing.T) {
	...
	for _, key := range []roachpb.Key{
		...
		keys.SystemTenantKeyGen.TablePrefix(10 /* system descriptor ID */),
	} {
...
func TestStoreRangeSplitInsideRow(t *testing.T) {
	...
	tableKey := roachpb.RKey(keys.SystemTenantKeyGen.TablePrefix(keys.MinUserDescID))
...
func TestStoreRangeSplitStats(t *testing.T) {
	...
	keyPrefix := keys.SystemTenantKeyGen.TablePrefix(keys.MinUserDescID)
...
func TestStoreEmptyRangeSnapshotSize(t *testing.T) {
	...
	// no user data.
	splitKey := keys.SystemTenantKeyGen.TablePrefix(keys.MinUserDescID)
...
func TestStoreZoneUpdateAndRangeSplit(t *testing.T) {
	...
	tableBoundary := keys.SystemTenantKeyGen.TablePrefix(descID)
	...
	testutils.SucceedsSoon(t, func() error {
		repl := store.LookupReplica(roachpb.RKey(keys.SystemTenantKeyGen.TablePrefix(descID + 1)))
		rngDesc := repl.Desc()
...
func TestStoreRangeSplitWithMaxBytesUpdate(t *testing.T) {
	...
	testutils.SucceedsSoon(t, func() error {
		newRng := store.LookupReplica(roachpb.RKey(keys.SystemTenantKeyGen.TablePrefix(descID)))
...
func TestStoreRangeSystemSplits(t *testing.T) {
	...
	if err := store.DB().Txn(context.TODO(), func(ctx context.Context, txn *kv.Txn) error {
		...
		descTablePrefix := keys.SystemTenantKeyGen.TablePrefix(keys.DescriptorTableID)
	...
	verifySplitsAtTablePrefixes := func() {
		...
		for i := uint32(keys.MaxSystemConfigDescID + 1); i <= maxID; i++ {
			expKeys = append(expKeys,
				testutils.MakeKey(keys.Meta2Prefix, keys.SystemTenantKeyGen.TablePrefix(i)),
			)
		}
		for i := keys.MinUserDescID; i <= userTableMax; i++ {
			if _, ok := exceptions[i]; !ok {
				expKeys = append(expKeys,
					testutils.MakeKey(keys.Meta2Prefix, keys.SystemTenantKeyGen.TablePrefix(uint32(i))),
				)
			}
		}
...
func TestUnsplittableRange(t *testing.T) {
	...
	tableKey := roachpb.RKey(keys.SystemTenantKeyGen.TablePrefix(keys.UITableID))
...
func TestRangeLookupAfterMeta2Split(t *testing.T) {
	...
	splitReq := adminSplitArgs(keys.SystemTenantKeyGen.TablePrefix(tableID - 3 /* 48 */))
	if _, pErr := kv.SendWrapped(ctx, s.DB().NonTransactionalSender(), splitReq); pErr != nil {
		t.Fatal(pErr)
	}

	metaKey := keys.RangeMetaKey(roachpb.RKey(keys.SystemTenantKeyGen.TablePrefix(tableID))).AsRawKey()
	...
	testutils.RunTrueAndFalse(t, "reverse", func(t *testing.T, rev bool) {
		...
		scanStart := keys.SystemTenantKeyGen.TablePrefix(tableID - 2) // 49
