...
func TestStoreSplitAbortSpan(t *testing.T) {
	...
	txn := func(key roachpb.Key, ts hlc.Timestamp) *roachpb.Transaction {
		txn := roachpb.MakeTransaction("test", key, 0, 0, ts, 0, int32(s.SQLInstanceID()), 0, false)
		return &txn
	}
...
func TestQueryLocksAcrossRanges(t *testing.T) {
	...
	txn3Proto := roachpb.MakeTransaction("waiter", nil /* baseKey */, isolation.Serializable, roachpb.NormalUserPriority, s.Clock().NowAsClockTimestamp().ToTimestamp(), s.Clock().MaxOffset().Nanoseconds(), int32(s.SQLInstanceID()), 0, false)
	txn3ID := txn3Proto.ID
...
func TestStoreRangeSplitIdempotency(t *testing.T) {
	...
	// contains the key.
	txn := roachpb.MakeTransaction("test", []byte("c"), isolation.Serializable, 10, store.Clock().Now(), 0, int32(s.SQLInstanceID()), 0, false)
...
func TestRangeLookupAsyncResolveIntent(t *testing.T) {
	...
	txn := roachpb.MakeTransaction("test", key2, isolation.Serializable, 1, store.Clock().Now(), store.Clock().MaxOffset().Nanoseconds(), int32(s.SQLInstanceID()), 0, false)
