...
func TestLBSplitUnsafeKeys(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			// /2 -> error
			// We search *after* the given split key, in this case we won't find a key
			// because there are no keys after 2.
			splitKey:     es(2),
	...
	for _, tc := range testCases {
		...
		t.Run(fmt.Sprintf("%s%v -> %s", makeTestKey(1, tc.splitKey), tc.existingKeys, expectStr), func(t *testing.T) {
			...
			// Force a table scan to resolve the descrptor.
			tdb.QueryRow(t, "SELECT count(*) FROM t")
			...
			if tc.expErrStr != "" {
				...
				require.ErrorContains(t, processErr, tc.expErrStr)
				require.Equal(t, prevEndKey, repl.Desc().EndKey.AsRawKey())
			} else {
				// Otherwise, assert that the new range end key matches the expected
				// end key.
				require.NoError(t, processErr)
				require.Equal(t, makeTestKey(tableID, tc.expSplitKey), repl.Desc().EndKey.AsRawKey())
			}
		})
	}
}
