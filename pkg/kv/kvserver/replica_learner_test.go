...
func TestAddReplicaViaDelegateSnapshot(t *testing.T) {
	...
	shouldDelegateSnapshots := int64(1)
				...
				Knobs: base.TestingKnobs{
					Store: &kvserver.StoreTestingKnobs{
						DelegateSnapshots: func() bool {
							t.Helper()
							if a := atomic.LoadInt64(&shouldDelegateSnapshots); a == 0 {
								// TODO(amy): make this return the replica id of new sender
								return false
							}
							return true
						},
	...
	// Add a follower replica that will actually send the delegated snapshot.
	tc.AddVotersOrFatal(t, scratch, tc.Target(1))

	desc := tc.LookupRangeOrFatal(t, scratch)
	require.Len(t, desc.Replicas().VoterDescriptors(), 2)
	t.Logf("DEBUG: new sender added %v:", desc.Replicas().VoterDescriptors())
	// Try to add a voter to node 2, and the new follower should send the snapshot

	_, err := tc.AddVoters(scratch, tc.Target(2))
	if err != nil {
		return
	}

	_, err = tc.AddVoters(scratch, tc.Target(3))
	if err != nil {
		return
	}
	desc = tc.LookupRangeOrFatal(t, scratch)
	require.Len(t, desc.Replicas().VoterDescriptors(), 4)
	//nodes := 4
	...
	//		defer wg.Done()
	//		_, err := tc.AddVoters(scratch, tc.Target(i))
	//		if err != nil {
	//			return
	//		}
	//		errCh <- err
	...
	//wg.Wait()
	//desc = tc.LookupRangeOrFatal(t, scratch)
	//require.Len(t, desc.Replicas().VoterDescriptors(), nodes)
	//t.Logf("DEBUG: new voter successfully added %v:", desc.Replicas().Descriptors())
	// expect that the delegated follower sends the snapshot to the new voterStores
