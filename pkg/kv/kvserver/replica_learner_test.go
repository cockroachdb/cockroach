...
func TestAddReplicaWithReceiverThrottling(t *testing.T) {
	...
	// Add a voter to the range and expect it to block on blockIncomingSnapshots.
	g.GoCtx(
		func(ctx context.Context) error {
			...
			_, err = tc.Servers[0].DB().AdminChangeReplicas(
				ctx, scratch, desc, roachpb.MakeReplicationChanges(roachpb.ADD_VOTER, tc.Target(2)),
			)
	...
	// Wait for the goroutines to finish and assert the error is as expected.
	const msgRE = `descriptor changed`
