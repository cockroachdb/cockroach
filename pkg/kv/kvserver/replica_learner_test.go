...
func TestAddRemoveNonVotingReplicasBasic(t *testing.T) {
	...
	g.Go(func() error {
		_, err := tc.AddNonVoters(scratchStartKey, true, tc.Target(1))
		return err
	})
	require.NoError(t, g.Wait())
...
func TestLearnerSnapshotFailsRollback(t *testing.T) {
	...
	runTest := func(t *testing.T, replicaType roachpb.ReplicaType) {
		...
		case roachpb.NON_VOTER:
			_, err = tc.AddNonVoters(scratchStartKey, true /* waitForInit */, tc.Target(1))
		default:
...
// meant to test that a non-voting replica that is initialized via an `INITIAL`
// snapshot during its addition is not ignored by the raft snapshot queue for
// future snapshots.
func TestNonVoterCatchesUpViaRaftSnapshotQueue(t *testing.T) {
	...
	_, err := tc.AddNonVoters(scratchStartKey, false, tc.Target(1))
	atomic.StoreInt64(&skipInitialSnapshot, 0)
	require.NoError(t, err)

	scratchDesc := tc.LookupRangeOrFatal(t, scratchStartKey)
	require.Lenf(
		t,
		scratchDesc.Replicas().NonVoterDescriptors(),
		1,
		"expected to find 1 non-voter; found %v",
		scratchDesc.Replicas().NonVoterDescriptors(),
	)
