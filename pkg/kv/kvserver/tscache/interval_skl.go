...
func makeRatchetingTime(ts hlc.Timestamp) ratchetingTime {
	...
	// the flag, the timestamp without the flag has a larger ratchetingTime
	// value.
	if rt&1 == 1 {
		rt++
	}
	if !ts.IsFlagSet(hlc.TimestampFlag_SYNTHETIC) {
		rt |= 1
	}
