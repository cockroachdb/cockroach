...
//     transport/store request intercepting on n1, but can be overridden.
//
...
//     returned as normal.
//
//   - jam-req token=<string> [txn=<txn-name>] [src-node=<int>] [dst-node=<int>] [range=<int>] [method=<string>]
...
//
//   - jam-resp token=<string> [txn=<txn-name>] [src-node=<int>] [dst-node=<int>] [range=<int>] [method=<string>]
...
//
//   - block-req token=<string> [txn=<txn-name>] [src-node=<int>] [dst-node=<int>] [range=<int>] [method=<string>]
...
//
//   - block-resp token=<string> [txn=<txn-name>] [src-node=<int>] [dst-node=<int>] [range=<int>] [method=<string>]
...
//
// Store Request Filter Specific Interception commands:
// (Note: store request filter interceptors are released using the same
// commands as above. Also note that these intercept requests at the store filter,
// so they may only pause/jam/block requests, and cannot hold back responses.)
//
//   - store-pause-req token=<string> [txn=<txn-name>] [dst-node=<int>] [range=<int>] [method=<string>]
//     Pause all requests matching the provided filter criteria at the
//     store filter. Paused requests are held until the named token is
//     released, allowing the request to pass the filter once done.
//
//   - store-jam-req token=<string> [txn=<txn-name>] [dst-node=<int>] [range=<int>] [method=<string>]
//     Jam all requests matching the provided filter criteria at the
//     store filter. Jammed requests are held until the named token is
//     released, returning an error to block the request at the filter once done.
//
//   - store-block-req token=<string> [txn=<txn-name>] [dst-node=<int>] [range=<int>] [method=<string>]
//     Pause all requests matching the provided filter criteria at the
//     store filter. Paused requests are held before until the named token is
//     released, allowing the request to pass the filter once done.
//
// Debug commands:
//
	...
	datadriven.Walk(t, datapathutils.TestDataPath(t, "intercepted_transactions"), func(t *testing.T, path string) {
		...
		datadriven.RunTest(t, path, func(t *testing.T, d *datadriven.TestData) string {
			...
			case "pause-req":
				...
				err := tt.addInterceptor(d.Cmd, tok, filter, interceptTransport, onRequest, returnResponse, false)
				if err != nil {
			...
			case "pause-resp":
				tok, filter := tt.parseInterceptArgs(d)
				tt.trace("adding interceptor \"%s-%s\" where %s", d.Cmd, tok, filter)
				err := tt.addInterceptor(d.Cmd, tok, filter, interceptTransport, onResponse, returnResponse, false)
				if err != nil {
					d.Fatalf(t, "error intercepting: %v", err)
				}
				return ""
			case "jam-req":
				tok, filter := tt.parseInterceptArgs(d)
				tt.trace("adding interceptor \"%s-%s\" where %s", d.Cmd, tok, filter)
				err := tt.addInterceptor(d.Cmd, tok, filter, interceptTransport, onRequest, returnError, false)
				if err != nil {
					d.Fatalf(t, "error intercepting: %v", err)
				}
				return ""
			case "jam-resp":
				tok, filter := tt.parseInterceptArgs(d)
				tt.trace("adding interceptor \"%s-%s\" where %s", d.Cmd, tok, filter)
				err := tt.addInterceptor(d.Cmd, tok, filter, interceptTransport, onResponse, returnError, false)
				if err != nil {
					d.Fatalf(t, "error intercepting: %v", err)
				}
				return ""
			case "block-req":
				tok, filter := tt.parseInterceptArgs(d)
				tt.trace("adding interceptor \"%s-%s\" where %s", d.Cmd, tok, filter)
				err := tt.addInterceptor(d.Cmd, tok, filter, interceptTransport, onRequest, returnError, true)
				if err != nil {
					d.Fatalf(t, "error intercepting: %v", err)
				}
				return ""
			case "block-resp":
				tok, filter := tt.parseInterceptArgs(d)
				tt.trace("adding interceptor \"%s-%s\" where %s", d.Cmd, tok, filter)
				err := tt.addInterceptor(d.Cmd, tok, filter, interceptTransport, onResponse, returnError, true)
				if err != nil {
					d.Fatalf(t, "error intercepting: %v", err)
				}
				return ""
			...
			case "chain":
				...
				return ""
			case "store-pause-req":
				tok, filter := tt.parseInterceptArgs(d)
				tt.trace("adding store interceptor \"%s-%s\" where %s", d.Cmd, tok, filter)
				err := tt.addInterceptor(d.Cmd, tok, filter, interceptStore, onRequest, returnResponse, false)
				if err != nil {
					d.Fatalf(t, "error intercepting: %v", err)
				}
				return ""
			case "store-jam-req":
				tok, filter := tt.parseInterceptArgs(d)
				tt.trace("adding store interceptor \"%s-%s\" where %s", d.Cmd, tok, filter)
				err := tt.addInterceptor(d.Cmd, tok, filter, interceptStore, onRequest, returnError, false)
				if err != nil {
					d.Fatalf(t, "error intercepting: %v", err)
				}
				return ""
			case "store-block-req":
				tok, filter := tt.parseInterceptArgs(d)
				tt.trace("adding store interceptor \"%s-%s\" where %s", d.Cmd, tok, filter)
				err := tt.addInterceptor(d.Cmd, tok, filter, interceptStore, onRequest, returnError, true)
				if err != nil {
					d.Fatalf(t, "error intercepting: %v", err)
				}
				return ""
...
type interceptedTransactionsTest struct {
	*testing.T
	...
	storeReqInterceptors      sync.Map // map[InterceptToken]requestIntercept
}
	...
	for idx := 0; idx < numNodes; idx++ {
		...
		if _, ok := tt.interceptedNodeIDs[svrToNodeID(idx)]; ok {
			...
			args.Knobs.Store = &kvserver.StoreTestingKnobs{
				TestingRequestFilter: tt.createInterceptingStoreFilter(svrToNodeID(idx)),
			}
		}
...
func (tt *interceptedTransactionsTest) addInterceptor(
	...
	transportOrStore interceptorType,
	...
	if reqOrResp == onRequest && transportOrStore == interceptTransport {
		targetCollection = &tt.transportReqInterceptors
	} else if reqOrResp == onRequest && transportOrStore == interceptStore {
		targetCollection = &tt.storeReqInterceptors
	} else if reqOrResp == onResponse && transportOrStore == interceptTransport {
		targetCollection = &tt.transportRespInterceptors
	} else if reqOrResp == onResponse && transportOrStore == interceptStore {
		return errors.Errorf("store response interceptors not supported")
	} else {
...
func (tb *interceptTagBuilder) addTypeTag(ba *kvpb.BatchRequest, srcNodeID roachpb.NodeID, target roachpb.ReplicaDescriptor, onTransport bool) {
	if onTransport {
		fmt.Fprintf(tb, "n%d->n%d:r%d/%d", srcNodeID, target.NodeID, ba.RangeID, target.ReplicaID)
	} else {
		fmt.Fprintf(tb, "n%ds%d:r%d/%d", target.NodeID, target.StoreID, ba.RangeID, target.ReplicaID)
	}
}
...
func (tb *interceptTagBuilder) Build(
	...
	tb.addTypeTag(ba, srcNodeID, target, onTransport)
}
	...
	return func(options kvcoord.SendOptions, dialer *nodedialer.Dialer, slice kvcoord.ReplicaSlice) (kvcoord.Transport, error) {
		...
		interceptor := &InterceptingTransport{
			...
			intercept: func(ctx context.Context, ba *kvpb.BatchRequest) (*kvpb.BatchResponse, error) {
				...
				tb.Build(reqITok, reqI, respITok, respI, ba, srcNodeID, transport.NextReplica(), true)
...
// createInterceptingStoreFilter, similar to the intercepting transport factory,
// returns a store filter that can be used to pause/jam/block requests at the
// point of a request's entry to a store, controlled by the logic in the
// data-driven test. As this is not a transport and is simply a filter, it can
// only delay (or block) requests, not responses.
func (tt *interceptedTransactionsTest) createInterceptingStoreFilter(srcNodeID roachpb.NodeID) kvserverbase.ReplicaRequestFilter {
	tt.trace("enabled interception at store on n%d", srcNodeID)
	return func(ctx context.Context, ba *kvpb.BatchRequest) *kvpb.Error {
		reqITok, reqI := getFirstMatchingInterceptor(&tt.storeReqInterceptors, ba, srcNodeID, ba.Replica)

		// For the store filter, only log if being intercepted to avoid duplication.
		isObserved := reqI != nil

		var tb interceptTagBuilder
		tb.Build(reqITok, reqI, "", nil, ba, srcNodeID, ba.Replica, false)
		...
		if isObserved {
			...
			tt.log("%s (store) batchReq={%s} %s", tb.String(), ba, txnSuffix)
		}
		...
		if reqI != nil {
			...
			if reqI.onRelease == returnError {
				return kvpb.NewErrorf("request intercepted at n%ds%d", ba.Replica.NodeID, ba.Replica.StoreID)
			}
		}
...
const (
	interceptTransport interceptorType = iota
	interceptStore
)
