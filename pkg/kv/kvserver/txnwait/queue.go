...
func CanPushWithPriority(
	...
	case kvpb.PUSH_TIMESTAMP:
		...
		// write skew but the pusher transaction does (and expects other to), let
		// the PUSH_TIMESTAMP proceed as long as the pusher has at least the same
		// priority. Otherwise, if neither transaction tolerates write skew, let the
