...
// ClearRHS remvoes anyone waiting on a key greater or equal to the given end key.
//
// This method is invoked as part of range split handling.
func (q *Queue) ClearRHS(endKey roachpb.Key) []roachpb.LockAcquisition {
	q.mu.Lock()
	...
	for uuid, pt := range q.mu.txns {
		if roachpb.Key(pt.getTxn().Key).Less(endKey) {
			continue
		}
