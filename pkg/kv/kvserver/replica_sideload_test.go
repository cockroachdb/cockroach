...
func TestRaftSSTableSideloadingSnapshot(t *testing.T) {
	...
	inlinedEntry := func() raftpb.Entry {
		os, err := tc.repl.GetSnapshot(ctx, SnapshotRequest_VOTER_RAFT, tc.store.StoreID())
		if err != nil {
			t.Fatal(err)
		}
		defer os.Close()
	...
	func() {
		failingOS, err := tc.repl.GetSnapshot(ctx, SnapshotRequest_VOTER_RAFT, tc.store.StoreID())
		if err != nil {
			t.Fatal(err)
		}
