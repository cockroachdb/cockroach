...
func verifyStorageStats(t *testing.T, s *kvserver.Store) {
	...
	// TODO(jackson): Adjust the test to ensure there are non-L6 files, and
	// assert that RdbBloomFilter{PrefixChecked,PrefixUseful} are nonzero. Since
	// filters are not consulted for L6 files, TestStoreMetrics is flaky if we
	// assert that the bloom filter is consulted.
	m := s.Metrics()
	testcases := []struct {
		gauge *metric.Gauge
		min   int64
