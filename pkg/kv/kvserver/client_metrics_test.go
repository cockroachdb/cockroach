...
func TestStoreMaxBehindNanosOnlyTracksEpochBasedLeases(t *testing.T) {
	...
	testutils.SucceedsSoon(t, func() error {
		...
		behind := timeutil.Since(timeutil.Unix(0, l.Start.WallTime))
		for i := 0; i < tc.NumServers(); i++ {
			...
			const behindMultiple = 5 // ensure the max behind value is much larger
			if maxBehind*behindMultiple > behind {
