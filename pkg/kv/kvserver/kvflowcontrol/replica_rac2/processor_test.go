	...
	datadriven.RunTest(t, datapathutils.TestDataPath(t, "processor"),
		func(t *testing.T, d *datadriven.TestData) string {
			...
			case "set-enabled-level":
				enabledLevel := parseEnabledLevel(t, d)
				var basicState RaftNodeBasicState
				if r.raftNode != nil {
					basicState = RaftNodeBasicState{
				...
				p.SetEnabledWhenLeaderRaftMuLocked(ctx, enabledLevel, basicState)
				return builderStr()
			...
			case "handle-raft-ready-and-admit":
				...
				p.HandleRaftReadyRaftMuLocked(ctx, basicState, event)
				fmt.Fprintf(&b, ".....\n")
				if len(event.Entries) > 0 {
