...
func (f *testRangeControllerFactory) New(state rangeControllerInitState) rac2.RangeController {
	fmt.Fprintf(f.b, " RangeControllerFactory.New(%+v)\n", state)
...
func createEntry(t *testing.T, info entryInfo) raftpb.Entry {
	...
	// this as needing 4 bytes, i.e., we are not making a serious effort to
	// respect info.length.
	require.LessOrEqual(t, 4, paddingLen)
	cmd := kvserverpb.RaftCommand{
		WriteBatch: &kvserverpb.WriteBatch{Data: make([]byte, paddingLen-4)}}
	cmdBuf, err := cmd.Marshal()
	require.NoError(t, err)
