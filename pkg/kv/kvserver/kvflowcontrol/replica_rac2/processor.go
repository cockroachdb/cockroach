...
// RaftInterfaceForMsgApp is temporarily separate from the various other Raft
// interfaces, and will eventually be merged.
type RaftInterfaceForMsgApp interface {
	// RaftMsgAppModeLocked returns the current RaftMsgAppMode.
	RaftMsgAppModeLocked() RaftMsgAppMode

	// SwitchToPullModeLocked must only be called when in MsgAppPush mode, and
	// switches to MsgAppPull mode. Any buffered MsgApps that were generated in
	// Step are discarded -- note that these are MsgApps generated by a Step
	// that happened after the last Ready, since the latter includes MsgApps
	// that have been sent, i.e., the discarded MsgApps for replica i are for
	// indices >= Next(i).
	//
	// The Processor when processing a Ready in push mode will do all the
	// processing using RangeController. Then call SwitchToPullModeLocked. In
	// the next Ready, which will not include MsgApps, the Processor will tell
	// RangeController that it is in pull mode.
	SwitchToPullModeLocked()

	// SwitchToPushModeLocked must only be called when in MsgAppPull, and switches
	// to MsgAppPush mode. The callee will immediately generate and buffer some
	// MsgApps. It is the callers responsibility to schedule Ready processing, so
	// that these can be processed.
	//
	// The Processor when processing a Ready in pull mode will do all the
	// processing under RangeController. The RangeController may cause Next(i)
	// to advance. Then the Processor will call SwitchToPushModeLocked. The next
	// Ready may include MsgApps, and the Processor will tell RangeController
	// that it is now in push mode.
	SwitchToPushModeLocked()

	// MakeMsgAppLocked is used to construct a MsgApp for entries in [start,
	// end) and must only be called in MsgAppPush mode.
	//
	// REQUIRES:
	...
	MakeMsgAppLocked(replicaID roachpb.ReplicaID, start, end uint64, maxSize int64) (raftpb.Message, error)
}
