...
// queued messages) will always return true if there wre any messages that
...
type Processor interface {
	...
	// OnDescChangedLocked provides a possibly updated RangeDescriptor. Both
	// Replica mu and raftMu are held.
	//
...
// the entries provided to RangeController in the next call to
...
func (p *processorImpl) ProcessPiggybackedAdmittedAtLeaderRaftMuLocked(ctx context.Context) bool {
	...
	for k, m := range p.mu.leader.enqueuedPiggybackedResponses {
		if roachpb.ReplicaID(m.To) != p.opts.ReplicaID {
