...
type processorImpl struct {
	...
	mu struct {
		...
		leader struct {
			...
			// rcRef mirrors the rc pointer, and is get the RangeController reference
			// without extra synchronization. Updated together with rc under raftMu.
			rcRef atomic.Value
...
func (p *processorImpl) AdmitForEval(
	...
	rc := p.mu.leader.rcRef.Load().(rac2.RangeController)
	if rc == nil {
		p.opts.EvalWaitMetrics.OnWaiting(workClass)
		p.opts.EvalWaitMetrics.OnBypassed(workClass, 0 /* duration */)
		return false, nil
