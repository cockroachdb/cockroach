...
// isLeaderUsingV2Locked returns true if the current leader uses the V2
// protocol.
//
// NB: the result of this method does not change while raftMu is held.
func (p *processorImpl) isLeaderUsingV2Locked() bool {
...
func (p *processorImpl) AdmitRaftEntriesRaftMuLocked(ctx context.Context, e rac2.RaftEvent) bool {
	destroyed, usingV2 := func() (bool, bool) {
		p.mu.Lock()
		defer p.mu.Unlock()
		return p.mu.destroyed, p.isLeaderUsingV2Locked()
	}()
