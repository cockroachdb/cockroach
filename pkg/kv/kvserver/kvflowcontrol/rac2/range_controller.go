...
func (rc *rangeController) updateReplicaSet(ctx context.Context, replicas ReplicaSet) {
	rc.replicaSet = replicas
}
...
func (rc *rangeController) updateVoterSets() {
	...
	for _, r := range rc.replicaSet {
		...
		vsfw := voterStateForWaiters{
			...
			// is initialized here instead and also check that the elastic eval
			// tokens are positive.
			isStateReplicate: rs.connectedState == replicate,
