...
type RangeController interface {
	...
	//
	// Requires replica.mu and replica.raftMu to be held.
...
//
// Locking: as noted in replicaSendStream, replicaSendStream.mu is ordered
// after Replica.mu. So avoid acquiring Replica.mu in the implementation of
// RaftInterface (to prevent deadlock), or very carefully reason through the
// consequences.
//
...
type RaftInterface interface {
	...
	// SendPingLocked sends a MsgApp ping to the given raft peer if there wasn't
	// a recent MsgApp to this peer. The message is added to raft's message
	...
	SendPingLocked(roachpb.ReplicaID) bool
}
...
func (rc *rangeController) MaybeSendPingsLocked() {
	...
	for id, state := range rc.replicaMap {
		...
		if s := state.sendStream; s != nil && s.shouldPing() {
			rc.opts.RaftInterface.SendPingLocked(id)
		}
	}
}
