...
func constructRaftEventForReplica(
	...
	if createSendStream {
		if next > raftEventAppendState.rewoundNextRaftIndex {
			// NB: will never happen in push mode, except for leader (which is
			// always in push mode).
			//
...
// whether it has send tokens or should be force flushing.
...
// First-pass decision making on what the stream is doing, or should do.
// Called for all replicas when in pull mode.
