...
// minCap is the smallest capacity. The choice of 32 is arbitrary.
const minCap = 32
...
func (cb *CircularBuffer[T]) Push(a T) {
	...
	if needed > cap {
		...
		if cap > minCap*2 && cap > 3*cb.maxObservedLen && cap/2 > needed {
			// Shrink.
			size := cap / 2
			cb.reallocate(size)
		}
...
func (cb *CircularBuffer[T]) Pop(num int) {
	...
	if num == 0 {
		// It is possible cb.cap is 0, so returning here avoids doing an incorrect
		// <something> % cb.cap.
		return
	}
	cb.len -= num
