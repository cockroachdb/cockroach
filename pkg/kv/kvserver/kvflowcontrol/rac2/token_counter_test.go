        ...
        typ := "eval"
				...
				for _, line := range strings.Split(d.Input, "\n") {
					...
					require.GreaterOrEqual(t, len(parts), 2, "expected form '"+
						...
						"delta={+,-}<size> "+
						"[flags={normal,force_flush,prevention,disconnect}]",
					)

					var delta kvflowcontrol.Tokens
					var wc admissionpb.WorkClass
					var flags TokenAdjustFlags
					...
					// Parse [flags={normal,force_flush,prevention,disconnect}]
					if len(parts) >= 3 {
						parts[2] = strings.TrimSpace(parts[2])
						if strings.HasPrefix(parts[2], "flags=") {
							parts[2] = strings.TrimPrefix(parts[2], "flags=")
							switch parts[2] {
							case "normal":
								flags = AdjNormal
							case "force_flush":
								flags = AdjForceFlush
							case "prevention":
								flags = AdjPreventionSendQueue
							case "disconnect":
								flags = AdjDisconnect
							default:
								t.Fatalf("unknown flags: %s", parts[2])
							}
						}
					}
					...
					counter.adjust(ctx, wc, delta, flags)
...
func (h adjustment) String() string {
	...
	return fmt.Sprintf("%8s %7s %-11s  %8s | %8s%s",
