	...
	for {
		...
		if w.empty() {
			...
			case <-w.stopper.ShouldQuiesce():
				return
			case <-w.timer.Ch():
				log.Infof(ctx, "timerCh")
				// The queue has been empty for watcherIdleCloseDuration, so stop the
				// watcher.
				w.timer.MarkRead()
				w.timer.Stop()
				w.markStopped()
...
func (w *sendStreamTokenWatcher) markStopped() {
	w.mu.Lock()
	defer w.mu.Unlock()

	clear(w.mu.queueItems)
	w.mu.started = false
}
