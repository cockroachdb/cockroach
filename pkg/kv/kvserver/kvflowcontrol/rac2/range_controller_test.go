...
func (s *testingRCState) init(t *testing.T, ctx context.Context) {
	...
	s.probeToCloseScheduler = &testingProbeToCloseTimerScheduler{}
	...
	datadriven.Walk(t, datapathutils.TestDataPath(t, "range_controller"), func(t *testing.T, path string) {
		...
		datadriven.RunTest(t, path, func(t *testing.T, d *datadriven.TestData) string {
			...
			case "tick":
				...
				// We send an empty raft event in order to trigger any potential state
				// changes.
				for _, testRC := range state.ranges {
