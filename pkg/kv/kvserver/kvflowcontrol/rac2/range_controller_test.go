	...
	datadriven.Walk(t, datapathutils.TestDataPath(t, "range_controller"), func(t *testing.T, path string) {
		...
		datadriven.RunTest(t, path, func(t *testing.T, d *datadriven.TestData) string {
			...
			case "metrics":
				...
				case "send_queue":
					...
					for _, rcState := range state.ranges {
						count, bytes := rcState.rc.SendStreamStatsRaftMuLocked().SumSendQueues()
						sizeCount += count
						sizeBytes += bytes
					}
			...
			case "send_stream_stats":
				var (
					rangeID int
					// Default to using the basic stats.
					basic = true
				)
				d.ScanArgs(t, "range_id", &rangeID)
				d.MaybeScanArgs(t, "basic", &basic)

				r := state.ranges[roachpb.RangeID(rangeID)]
				var stats RangeSendStreamStats
				if basic {
					stats = r.rc.BasicSendStreamStats()
				} else {
