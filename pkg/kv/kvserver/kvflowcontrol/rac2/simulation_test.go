		...
		datadriven.RunTest(t, path, func(t *testing.T, d *datadriven.TestData) string {
			...
			case "init":
				...
				for _, line := range strings.Split(d.Input, "\n") {
					...
					sim.state.getOrInitRange(makeSingleVoterTestingRange(
				...
				for _, line := range strings.Split(d.Input, "\n") {
					...
					for i := range parts {
						...
						case strings.HasPrefix(parts[i], "handle="):
							...
							tl.handle = sim.state.ranges[roachpb.RangeID(rangeID)]
			...
			case "snapshots":
				...
				handle := sim.state.ranges[roachpb.RangeID(rangeID)]
...
func (s *simulator) init(t *testing.T, ctx context.Context) {
	...
	s.registry.AddMetricStruct(s.state.streamTokenProvider.Metrics())
	s.tsdb.Register(s.state.evalMetrics)
	s.tsdb.Register(s.state.streamTokenProvider.Metrics())
}
...
func (s *simulator) timeline(tl timeline) {
	...
	if tl.handle == nil {
		...
		s.ticker = append(s.ticker, &streamsTicker{
			t:           s.state.t,
			evalTC:      s.state.streamTokenProvider.Eval(tl.stream),
			sendTC:      s.state.streamTokenProvider.Send(tl.stream),
...
func (s *simulator) simulate(ctx context.Context, tend time.Time) {
	...
	for {
		...
		if t.UnixNano()%metricTick.Nanoseconds() == 0 {
			s.state.streamTokenProvider.UpdateMetricGauges()
			s.tsdb.Scrape(ctx)
		}
...
func (rt *rangeTicker) tick(ctx context.Context, t time.Time) {
	...
	if rt.delta >= 0 { // return tokens
		rt.handle.testingReturnTokens(ctx, rt.pri, rt.delta, rt.stream)
		return
	}
