...
type deltaStats struct {
	...
	// force-flushing, and we do wait for inflight tokens in that case. So we
	// don't bother with additional metrics that we are unlikely to care about.
...
// kvflowcontrol.Stream. When the TokenType is send tokens, it also hold flow
...
type tokenCounter struct {
	...
	mu struct {
		...
		// Inflight index is only initialized when this is a send tokenCounter.
...
func newTokenCounter(
	...
	t.mu.counters[RegularWC] = makeTokenCounterPerWorkClass(
		RegularWC, limit.regular, now)
	t.mu.counters[ElasticWC] = makeTokenCounterPerWorkClass(
		ElasticWC, limit.elastic, now)
