...
const (
	...
	AdjForceFlush
	// AdjPreventionSendQueue is set when not enough (regular|elastic) send
	// tokens are available but being deducted anyway (negative balance) to
	// prevent a delay in quorum by forming a send queue.
	AdjPreventionSendQueue
)

func (a TokenAdjustFlags) String() string {
	return redact.StringWithoutMarkers(a)
}

// SafeFormat implements the redact.SafeFormatter interface.
func (a TokenAdjustFlags) SafeFormat(w redact.SafePrinter, _ rune) {
	switch a {
	case AdjNormal:
	case AdjDisconnect:
		w.Print("disconnect")
	case AdjForceFlush:
		w.Print("force_flush")
	case AdjPreventionSendQueue:
		w.Print("prevention")
	default:
		panic("unknown token_adjust_flags")
	}
}
