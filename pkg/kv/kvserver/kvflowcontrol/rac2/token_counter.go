...
type TokenWaitingHandle interface {
	...
	// (such as when waiting before eval). The tokens granted will be less than
	// or equal to the requested tokens and deducted from the token counter.
	// haveTokens is true iff there are tokens available after this grant. When
	// the tokens argument is zero, granted will be zero, and haveTokens
	// represents whether there were positive tokens. If the caller is
