...
const regular, elastic = RegularWC, ElasticWC
...
const (
	...
	// stream when using MsgAppPull mode. Currently, the RangeControllers only
	// attempt to respect this when force-flushing a replicaSendStream, since
	// the typical production configuration of this value (64MiB) is larger than
	// the typical production configuration of the shared regular token pool
	// (16MiB), so attempting to respect this when doing non-force-flush sends
	// is unnecessary. One could consider respecting this when not allowed to
	// form a send-queue, but (a) that case doesn't result in a burst, (b) we
	// don't want to delay quorum by creating a send-queue where there was none,
	// so we don't respect this limit.
...
type SendQueueMetrics struct {
	...
	// tokens if useful. I've never found this existing
