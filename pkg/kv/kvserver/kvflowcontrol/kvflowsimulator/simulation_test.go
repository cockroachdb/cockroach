...
//     ....
//     t=[<duration>,<duration>) class={regular,elastic} \
//     handle=<string> adjust={+,-}<bytes>/s rate=<int>/s \
//     log-position=<int>/<int> [deduction-delay=<duration>]                 (B)
//     ....
...
//     deducts tokens from all connected streams. Token deductions/returns from
//     a handle are tied to specific raft log positions starting from the one
//     specified.
//
...
func (h *replicaHandle) returnTokens(
	...
	h.deductionTracker[stream].Iter(ctx,
		func(priority admissionpb.WorkPriority, tokens kvflowcontrol.Tokens, position kvflowcontrolpb.RaftLogPosition) bool {
			if priority != pri {
				return true
			}
	...
	if !returnUpTo.Equal(kvflowcontrolpb.RaftLogPosition{}) {
		...
		h.deductionTracker[stream].Untrack(ctx, pri, returnUpTo,
			func(kvflowcontrol.Tokens, kvflowcontrolpb.RaftLogPosition) {
				// Untrack from simulator's deduction tracker.
			},
		)
	}
}
...
func (ht *handleTicker) tick(ctx context.Context, t time.Time) {
	...
	admitted, signaled, admit := ht.replicaHandle.handle.TestingNonBlockingAdmit(ht.pri)
	if admitted {
		// Request admitted; proceed with token deductions.
		if ht.deductionDelay == 0 {
			ht.replicaHandle.deductTokens(ctx, ht.pri, -ht.delta)
...
func (ht *handleOpTicker) tick(ctx context.Context, t time.Time) {
	...
	case "connect":
		...
		ht.replicaHandle.deductionTracker[ht.stream] = kvflowtokentracker.New()
	case "snapshot":
