...
func (rl *limiter) Wait(ctx context.Context, reqInfo tenantcostmodel.RequestInfo) error {
	unlimited := rl.authorizer.IsNotRateLimited(ctx, rl.tenantID)
	if !unlimited {
...
// Releaes cleans up resources reserved for this limiter.
func (rl *limiter) Release() {
	rl.metrics.unlink()
	rl.qp.Close("released")
}

// Releaes is part of the Limiter interface.
...
// Aqcuire is part of quotapool.Request.
