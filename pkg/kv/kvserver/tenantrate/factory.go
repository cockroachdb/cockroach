...
func NewLimiterFactory(st *cluster.Settings, knobs *TestingKnobs) *LimiterFactory {
	...
	for _, setOnChange := range []func(*settings.Values, func()){
...
func (rl *LimiterFactory) Release(lim *Limiter) {
	rl.mu.Lock()
	defer rl.mu.Unlock()
	rcLim, ok := rl.mu.tenants[lim.tenantID]
	if !ok {
		panic(errors.AssertionFailedf("expected to find entry for tenant %v", lim.tenantID))
	}
	if &rcLim.lim != lim {
		panic(errors.AssertionFailedf("two limiters exist for tenant %v", lim.tenantID))
	}
	if rcLim.refCount--; rcLim.refCount == 0 {
		lim.metrics.destroy()
		delete(rl.mu.tenants, lim.tenantID)
	}
}
