...
func (rl *LimiterFactory) GetTenant(
	...
	if !ok {
		...
		log.Infof(
			ctx, "tenant %s rate limiter initialized (rate: %g RU/s; burst: %g RU; exempt: %t)",
			tenantID, rl.mu.config.Rate, rl.mu.config.Burst, rl.authorizer.IsExemptFromRateLimiting(ctx, tenantID),
		)
	}
	rcLim.refCount++
	return &rcLim.lim
