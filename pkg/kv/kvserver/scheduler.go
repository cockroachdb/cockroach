...
type raftScheduleState struct {
	...
	// scheduling or processing is slow. It is limited by maxSchedulerTicks, so
	// that the cost of processing all the ticks doesn't grow uncontrollably. If
	// ticks consistently reaches maxSchedulerTicks, the node/range is too slow,
	// and it is safer to not deliver all the ticks as it may cause a cascading
	// effect (the range events take longer and longer to process).
...
func (s *raftScheduler) enqueue1Locked(
	...
	if newState.ticks > maxSchedulerTicks {
		newState.ticks = maxSchedulerTicks
	}
