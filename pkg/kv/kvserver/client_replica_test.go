...
func TestLeaderlessWatcherErrorRefreshedOnUnavailabilityTransition(t *testing.T) {
	...
	// The leaderlessWatcher starts off as unavailable.
	require.False(t, repl.LeaderlessWatcher.IsUnavailable())
	// Let it know it's leaderless.
	repl.LeaderlessWatcher.TestingRefreshUnavailableState(ctx, raft.None, manual.Now(), st)
	...
	// available again.
	repl.LeaderlessWatcher.TestingRefreshUnavailableState(ctx, 1, manual.Now(), st)
	require.False(t, repl.LeaderlessWatcher.IsUnavailable())
