...
func TestTxnReadWithinUncertaintyIntervalAfterLeaseTransfer(t *testing.T) {
	...
	testutils.SucceedsSoon(t, func() error {
		...
		if tc.Servers[0].Clock().Now().WallTime != tc.Servers[1].Clock().Now().WallTime {
			return errors.Errorf("clocks are not synchronized: n1's clock: %+v, n2's clock: %+v",
				tc.Servers[0].Clock().Now(), tc.Servers[1].Clock().Now())
		}
		return nil
	})

	t.Logf("all clocks synchronized: n1's clock: %+v, n2's clock: %+v", tc.Servers[0].Clock().Now(), tc.Servers[1].Clock().Now())
	...
	manuals[0].Increment(100)
	t.Logf("advanced n1's clock by 100ns: n1's clock: %+v, n2's clock: %+v", tc.Servers[0].Clock().Now(), tc.Servers[1].Clock().Now())
	...
	t.Logf("validating the clocks before test assertions: n1's clock: %+v, n2's clock: %+v", tc.Servers[0].Clock().Now(), tc.Servers[1].Clock().Now())
