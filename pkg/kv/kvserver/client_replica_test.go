...
func TestLeaseTransferReplicatesLocks(t *testing.T) {
	...
	ctx := context.Background()
	st := cluster.MakeClusterSettings()
	concurrency.UnreplicatedLockReliabilityUpgrade.Override(ctx, &st.SV, true)
	tc := testcluster.StartTestCluster(t, 3, base.TestClusterArgs{
		ServerArgs: base.TestServerArgs{
			Settings: st,
		},
	...
	// Move lease to to store 2.

	<-txn2Started

	t.Log("transfering lease from s1 -> s2")
	require.NoError(t, tc.TransferRangeLease(desc, tc.Target(1)))
	// TODO(ssd): I'm not sure on the best way to coordinate this test. We _never_
	// expect to hit the t.Error in txn2 above, no matter how long we wait.
...
func TestMergeReplicatesLocks(t *testing.T) {
	...
	require.NoError(t, log.SetVModule("cmd_merge=2"))
	...
	scratch := tc.ScratchRange(t)
	splitKey := append(scratch[:len(scratch):len(scratch)], uuid.MakeV4().String()...)
	k1 := splitKey.Next()
