...
func testTxnReadWithinUncertaintyInterval(t *testing.T, observedTS bool, readOp string) {
	...
	txn := roachpb.MakeTransaction("test", key, isolation.Serializable, 1, now, maxOffset, int32(s.SQLInstanceID()), 0, false)
...
func testTxnReadWithinUncertaintyIntervalAfterIntentResolution(
	...
	require.NotZero(t, maxOffset)
	writerTxn := roachpb.MakeTransaction("test_writer", keyA, isolation.Serializable, 1, clocks[0].Now(), maxOffset, int32(tc.Servers[0].NodeID()), 0, false)
	...
	readerTxn := roachpb.MakeTransaction("test_reader", keyA, isolation.Serializable, 1, writerTxn.MinTimestamp, maxOffset, int32(tc.Servers[1].NodeID()), 0, false)
...
func TestTxnReadWithinUncertaintyIntervalAfterLeaseTransfer(t *testing.T) {
	...
	require.NotZero(t, maxOffset)
	txn := roachpb.MakeTransaction("test", keyB, isolation.Serializable, 1, now, maxOffset, int32(tc.Servers[1].SQLInstanceID()), 0, false)
	require.True(t, txn.ReadTimestamp.Less(txn.GlobalUncertaintyLimit))
	require.Len(t, txn.ObservedTimestamps, 0)
...
func TestTxnReadWithinUncertaintyIntervalAfterRangeMerge(t *testing.T) {
	...
	run := func(t *testing.T, alignLeaseholders bool, alsoSplit bool) {
		...
		txn := roachpb.MakeTransaction("txn1", keyA, isolation.Serializable, 1, now, maxOffset, instanceId, 0, false)
		txn2 := roachpb.MakeTransaction("txn2", keyA, isolation.Serializable, 1, now, maxOffset, instanceId, 0, false)
...
func TestRangeLocalUncertaintyLimitAfterNewLease(t *testing.T) {
	...
	txn := roachpb.MakeTransaction("test", keyA, isolation.Serializable, 1, tc.Servers[1].Clock().Now(), tc.Servers[1].Clock().MaxOffset().Nanoseconds(), int32(tc.Servers[1].SQLInstanceID()), 0, false)
