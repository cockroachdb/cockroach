...
func TestRangeLocalUncertaintyLimitAfterRangeMerge(t *testing.T) {
	...
	// writing. Sometimes clocks on 2 and 3 move ahead by an additional 1000ns as
	// a result of the lease transfers. This is why we push clocks by 2000, to
	// make sure the test is catching the correct condition.
	...
	if _, err := kv.SendWrapped(ctx, tc.Servers[2].DistSender(), putArgs(keyC, []byte("value"))); err != nil {
		t.Fatal(err)
	}
	...
	// Finally, try and read the key, this will return an uncertainty error, it
