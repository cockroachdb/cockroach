...
func TestLossQuorumCauseLeaderWatcherToSignalUnavailable(t *testing.T) {
	...
	sendTimedPutRequest := func(repl *kvserver.Replica, timeout time.Duration) (*kvpb.Error, error) {
	...
	// unavailable.
	pErr, ctxErr := sendTimedPutRequest(repl, 2*time.Second)
	...
	// At this point, requests should succeed. We need to try both replicas to
	// avoid NotLeaseHolderErrors.
	testutils.SucceedsSoon(t, func() error {
		...
		// At this point we know that the request failed. Return the error.
		if ctxErr != nil {
			return ctxErr
		}
		return pErr.GoError()
