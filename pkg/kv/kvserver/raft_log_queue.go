...
func computeTruncateDecision(input truncateDecisionInput) truncateDecision {
	...
	//         NewFirstIndex <= CommitIndex (unless CommitIndex == 10)       (3)
	//
	...
	//     the last known CommitIndex. We make an exception for when
	//     `CommitIndex == 10`, i.e. CommitIndex is the default initial log
	//     index (and FirstIndex == CommitIndex+1)
	//
	...
	empty := input.FirstIndex == input.LastIndex+1
	valid := empty || (decision.NewFirstIndex >= input.FirstIndex) &&
		(decision.NewFirstIndex <= input.LastIndex) &&
		(decision.NewFirstIndex <= decision.CommitIndex ||
			 decision.CommitIndex == 10 && decision.NewFirstIndex == decision.CommitIndex+1)
	if !valid {
