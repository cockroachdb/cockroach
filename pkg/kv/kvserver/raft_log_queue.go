...
func newTruncateDecision(ctx context.Context, r *Replica) (truncateDecision, error) {
	...
	r.mu.RLock()
	ls := r.asLogStorage()
	raftLogSize := r.pendingLogTruncations.computePostTruncLogSize(ls.shMu.raftLogSize)
	...
	lastIndex := ls.shMu.lastIndexNotDurable
	...
	logSizeTrusted := ls.shMu.raftLogSizeTrusted
