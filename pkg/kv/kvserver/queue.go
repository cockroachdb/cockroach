...
func (bq *baseQueue) processLoop(stopper *stop.Stopper) {
	...
	if err := stopper.RunAsyncTaskEx(ctx,
		stop.TaskOpts{TaskName: "queue-loop", SpanOpt: stop.Sterile},
		func(ctx context.Context) {
			defer done()
...
func (bq *baseQueue) addToPurgatoryLocked(
	...
	_ = stopper.RunAsyncTaskEx(workerCtx, stop.TaskOpts{TaskName: bq.name + ".purgatory", SpanOpt: stop.Sterile}, func(ctx context.Context) {
