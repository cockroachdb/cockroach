...
func (bq *baseQueue) maybeAdd(ctx context.Context, repl replicaInQueue, now hlc.ClockTimestamp) {
	...
	stopped := func() bool {
		bq.mu.Lock()
		defer bq.mu.Unlock()
		return bq.mu.stopped || bq.mu.disabled
	}()
...
func (bq *baseQueue) processReplicasInPurgatory(
	...
	func() {
		...
		ranges := func() []*replicaItem {
			bq.mu.Lock()
			defer bq.mu.Unlock()
			rs := make([]*replicaItem, 0, len(bq.mu.purgatory))
			for rangeID := range bq.mu.purgatory {
