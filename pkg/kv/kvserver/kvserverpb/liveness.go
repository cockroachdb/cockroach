...
// the valid state transitions for Membership are as follows:
...
func ValidateLivenessTransition(old, new Liveness) error {
	...
	if new.Membership.Decommissioning() && !old.Membership.Active() {
		err := fmt.Sprintf("can only decommission an active node; n%d found to be %s",
			new.NodeID, old.Membership.String())
		return status.Error(codes.FailedPrecondition, err)
	}
