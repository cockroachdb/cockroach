...
func Build(st Settings, nl NodeLiveness, i Input) (Output, error) {
	...
	case roachpb.LeaseEpoch:
		epoch, l, err := leaseEpoch(nl, i, nextLeaseType)
		if err != nil {
...
func leaseExpiration(st Settings, i Input, nextType roachpb.LeaseType) *hlc.Timestamp {
	...
	exp.Forward(i.PrevLease.GetExpiration())
	return &exp
}

func leaseEpoch(
	nl NodeLiveness, i Input, nextType roachpb.LeaseType,
) (int64, *livenesspb.Liveness, error) {
	if nextType != roachpb.LeaseEpoch {
		panic("leaseEpoch called for non-epoch lease")
	}
	l, ok := nl.GetLiveness(i.NextLeaseHolder.NodeID)
	if !ok || l.Epoch == 0 {
		return 0, nil, errors.Wrapf(liveness.ErrRecordCacheMiss,
			"couldn't request lease for %+v", i.NextLeaseHolder)
	}
	return l.Epoch, &l.Liveness, nil
}
...
func nodeLivenessManipulation(
	...
	if expToEpochPromo && nextLeaseLiveness.Expiration.ToTimestamp().Less(i.PrevLease.GetExpiration()) {
		return NodeLivenessManipulation{Heartbeat: nextLeaseLiveness}
	}
	...
	if i.PrevLease.Type() == roachpb.LeaseEpoch && i.PrevLeaseExpired {
		...
		if i.Extension() {
			...
			// lease and prevent it from ever becoming valid again.
...
func validateStart(i Input, nextLease roachpb.Lease) error {
	...
	if i.Extension() || i.Transfer() {
		...
		if nextLease.Start.ToTimestamp().Less(i.PrevLease.GetExpiration()) {
