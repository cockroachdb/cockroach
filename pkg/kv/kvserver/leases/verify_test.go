...
func TestVerify(t *testing.T) {
	...
	makeRaftStatus := func(lead roachpb.ReplicaID, state raft.StateType) *raft.Status {
		...
		status.ID = uint64(repl1.ReplicaID)
		status.Lead = uint64(lead)
	...
	t.Run("transfer", func(t *testing.T) {
		...
		defaultLeaderInput := func(targetState rafttracker.StateType, targetMatch kvpb.RaftIndex) VerifyInput {
			...
			in.RaftStatus.Progress = map[uint64]rafttracker.Progress{
				uint64(repl1.ReplicaID): {State: rafttracker.StateReplicate, Match: uint64(in.RaftFirstIndex)},
			}
			if targetState != noTargetState {
				in.RaftStatus.Progress[uint64(repl2.ReplicaID)] = rafttracker.Progress{
