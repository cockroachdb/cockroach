...
func TestStoresAddStore(t *testing.T) {
	...
	ls := newStores(testutils.MakeAmbientCtx(), hlc.NewClock(hlc.UnixNano, time.Nanosecond))
	store := Store{
...
func TestStoresGetReplicaForRangeID(t *testing.T) {
	...
	ls := newStores(testutils.MakeAmbientCtx(), clock)
	numStores := 10
...
func createStores(count int, t *testing.T) (*hlc.ManualClock, []*Store, *Stores, *stop.Stopper) {
	...
	ls := newStores(testutils.MakeAmbientCtx(), cfg.Clock)
...
func TestStoresGossipStorage(t *testing.T) {
	...
	ls2 := newStores(testutils.MakeAmbientCtx(), ls.clock)
...
func TestStoresGossipStorageReadLatest(t *testing.T) {
	...
	ls3 := newStores(testutils.MakeAmbientCtx(), ls.clock)
...
func TestClusterVersionWriteSynthesize(t *testing.T) {
	...
	makeStores := func() *Stores {
		ls := NewStores(testutils.MakeAmbientCtx(), stores[0].Clock())
		return ls
	}
