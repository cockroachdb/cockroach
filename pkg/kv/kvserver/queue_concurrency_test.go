...
func TestBaseQueueConcurrent(t *testing.T) {
	...
	impl := fakeQueueImpl{
		pr: func(context.Context, *Replica, spanconfig.QueueReader) (bool, error) {
			n := rand.Intn(4)
			if n == 0 {
				return true, nil
			} else if n == 1 {
...
type fakeQueueImpl struct {
	pr func(context.Context, *Replica, spanconfig.QueueReader) (processed bool, err error)
}

func (fakeQueueImpl) shouldQueue(
	context.Context, hlc.ClockTimestamp, *Replica, spanconfig.QueueReader,
) (shouldQueue bool, priority float64) {
	return rand.Intn(5) != 0, 1.0
}

func (fq fakeQueueImpl) process(
	ctx context.Context, repl *Replica, confReader spanconfig.QueueReader,
) (bool, error) {
