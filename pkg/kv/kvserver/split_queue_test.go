...
func TestSplitQueueShouldQueue(t *testing.T) {
	...
	testCases := []struct {
		...
		{roachpb.RKey(keys.SystemTenantKeyGen.TablePrefix(2001)), roachpb.RKeyMax, 0, 64 << 20, true, 1},
		// Already split at largest ID, no load.
		{roachpb.RKey(keys.SystemTenantKeyGen.TablePrefix(2002)), roachpb.RKeyMax, 0, 32 << 20, false, 0},
		...
		{roachpb.RKey(keys.SystemTenantKeyGen.TablePrefix(2000)), roachpb.RKeyMax, 32<<20 + 1, 32 << 20, true, 2},
		// Split needed at table boundary, but no zone config, no load.
		{roachpb.RKey(keys.SystemTenantKeyGen.TablePrefix(2001)), roachpb.RKeyMax, 32<<20 + 1, 64 << 20, true, 1},
	}
