...
// in files under testdata/flow_control/*. Every TestFlowControlFileName maps to
// testdata/flow_control/file_name -- running TestFlowControl* will match
// everything. It's instructive to run these tests with the following vmodule,
// as of 04/23:
//
...
`)
		...
		//
		//   [T1,n1] admitted request (pri=high-pri stream=t1/s1 tokens=+16 MiB wait-duration=7.791µs mode=apply_to_all)
		//   [T1,n1] admitted request (pri=high-pri stream=t1/s2 tokens=+16 MiB wait-duration=1.125µs mode=apply_to_all)
		//   [T1,n1] admitted request (pri=high-pri stream=t1/s3 tokens=+16 MiB wait-duration=1.042µs mode=apply_to_all)
		//
		...
		//
		//   [T1,n1,s1,r63/1:/{Table/Max-Max}] encoded raft admission meta: pri=high-pri create-time=1681169932186349000 proposer=n1
		//   [T1,n1,s1,r63/1:/{Table/Max-Max},raft] bound index/log terms for proposal entry: 6/20 EntryNormal <omitted>
		//   [T1,n1,s1,r63/1:/{Table/Max-Max}] tracking +1.0 MiB flow control tokens for pri=high-pri stream=t1/s1 pos=log-position=6/20
		//   [T1,n1,s1,r63/1:/{Table/Max-Max}] adjusted flow tokens (pri=high-pri stream=t1/s1 delta=-1.0 MiB): regular=+15 MiB elastic=+7.0 MiB
		//   [T1,n1,s1,r63/1:/{Table/Max-Max}] tracking +1.0 MiB flow control tokens for pri=high-pri stream=t1/s2 pos=log-position=6/20
		//   [T1,n1,s1,r63/1:/{Table/Max-Max}] adjusted flow tokens (pri=high-pri stream=t1/s2 delta=-1.0 MiB): regular=+15 MiB elastic=+7.0 MiB
		//   [T1,n1,s1,r63/1:/{Table/Max-Max}] tracking +1.0 MiB flow control tokens for pri=high-pri stream=t1/s3 pos=log-position=6/20
		//   [T1,n1,s1,r63/1:/{Table/Max-Max}] adjusted flow tokens (pri=high-pri stream=t1/s3 delta=-1.0 MiB): regular=+15 MiB elastic=+7.0 MiB
		//
		...
		// tokens.
		//
		//   [T1,n1,s1,r63/1:/{Table/Max-Max},raft] decoded raft admission meta below-raft: pri=high-pri create-time=1681169932186349000 proposer=n1 receiver=[n1,s1] tenant=t1 tokens=1048725 sideloaded=false raft-entry=6/20
		//   Fast-path:
		//  	[T1,n1,s1,r63/1:/{Table/Max-Max},raft] fast-path: admitting t1 pri=high-pri r63 origin=n1 log-position=6/20 ingested=false
		//   Async-path:
		//   	[T1,n1,s1,r63/1:/{Table/Max-Max},raft] async-path: len(waiting-work)=1: enqueued t1 pri=high-pri r63 origin=n1 log-position=6/20 ingested=false
		//      [-] async-path: len(waiting-work)=0 dequeued t1 pri=high-pri r63 origin=n1 log-position=6/20 ingested=false
		//   [-] dispatching admitted-entries (r63 s1 pri=high-pri up-to-log-position=6/20) to n1
		//   [-] released +1.0 MiB flow control tokens for 1 out of 1 tracked deductions for pri=high-pri stream=t1/s1, up to log-position=6/20; 0 tracked deduction(s) remain
		//   [-] adjusted flow tokens (pri=high-pri stream=t1/s1 delta=+1.0 MiB): regular=+16 MiB elastic=+8.0 MiB
		//
		...
		//
		//   [-] [raft] r63 Raft message 1->3 MsgApp Term:6 Log:6/19 Commit:19 Entries:[6/20 EntryNormal <omitted>]
		//   [-] [raft] r63 Raft message 1->2 MsgApp Term:6 Log:6/19 Commit:19 Entries:[6/20 EntryNormal <omitted>]
		//   [T1,n3,s3,r63/3:/{Table/Max-Max},raft] decoded raft admission meta below-raft: pri=high-pri create-time=1681169932186349000 proposer=n1 receiver=[n3,s3] tenant=t1 tokens=1048725 sideloaded=false raft-entry=6/20
		//   [T1,n2,s2,r63/2:/{Table/Max-Max},raft] decoded raft admission meta below-raft: pri=high-pri create-time=1681169932186349000 proposer=n1 receiver=[n2,s2] tenant=t1 tokens=1048725 sideloaded=false raft-entry=6/20
		//   [T1,n3,s3,r63/3:/{Table/Max-Max},raft] fast-path: admitting t1 pri=high-pri r63 origin=n1 log-position=6/20 ingested=false
		//   [T1,n2,s2,r63/2:/{Table/Max-Max},raft] fast-path: admitting t1 pri=high-pri r63 origin=n1 log-position=6/20 ingested=false
		//   [-] dispatching admitted-entries (r63 s3 pri=high-pri up-to-log-position=6/20) to n1
		//   [-] dispatching admitted-entries (r63 s2 pri=high-pri up-to-log-position=6/20) to n1
		//
		// 5. On MsgAppResps (or really any raft messages bound for n1), we piggy
		//    back these token dispatches. n1, on hearing about them, returns relevant
		//    tokens back to the stream.
		//
		//    [T1,n2] informing n1 of below-raft admission (r63 s2 pri=high-pri up-to-log-position=6/20): 1 out of 1 dispatches
		//    [-] [raft] r63 Raft message 2->1 MsgAppResp Term:6 Log:0/20
		//    [-] released +1.0 MiB flow control tokens for 1 out of 1 tracked deductions for pri=high-pri stream=t1/s2, up to log-position=6/20; 0 tracked deduction(s) remain
		//    [-] adjusted flow tokens (pri=high-pri stream=t1/s2 delta=+1.0 MiB): regular=+16 MiB elastic=+8.0 MiB
		//    [T1,n3] informing n1 of below-raft admission (r63 s3 pri=high-pri up-to-log-position=6/20): 1 out of 1 dispatches
		//    [-] [raft] r63 Raft message 3->1 MsgAppResp Term:6 Log:0/20
		//    [-] released +1.0 MiB flow control tokens for 1 out of 1 tracked deductions for pri=high-pri stream=t1/s3, up to log-position=6/20; 0 tracked deduction(s) remain
		//    [-] adjusted flow tokens (pri=high-pri stream=t1/s3 delta=+1.0 MiB): regular=+16 MiB elastic=+8.0 MiB
	})
}
...
`)
	...
	// [T1,n1,s1,r63/1:/{Table/Max-Max},*kvpb.AdminSplitRequest] initiating a split of this range at key "\xfa\x00" [r64] (manual)
	// [T1,n1,s1,r64/1:{\xfa\x00-/Max},raft] connected to stream: t1/s1
	// [T1,n1,s1,r64/1:{\xfa\x00-/Max},raft] connected to stream: t1/s3
	// [T1,n1,s1,r64/1:{\xfa\x00-/Max},raft] connected to stream: t1/s2
...
`, "range_id", "stream_count")
	...
	// [T1,n1,s1,r63/1:{/Table/Max-\xfa\x00},*kvpb.AdminMergeRequest] initiating a merge of r64:{\xfa\x00-/Max} [(n1,s1):1, (n2,s2):2, (n3,s3):3, next=4, gen=6, sticky=9223372036.854775807,2147483647] into this range (manual)
	// [T1,n1,s1,r63/1:{/Table/Max-\xfa\x00},raft] removing replica r64/1
	// [T1,n2,s2,r63/2:{/Table/Max-\xfa\x00},raft] removing replica r64/2
	// [T1,n3,s3,r63/3:{/Table/Max-\xfa\x00},raft] removing replica r64/3
	// [T1,n1,s1,r64/1:{\xfa\x00-/Max},raft] disconnected stream: t1/s1
	// [T1,n1,s1,r64/1:{\xfa\x00-/Max},raft] disconnected stream: t1/s2
	// [T1,n1,s1,r64/1:{\xfa\x00-/Max},raft] disconnected stream: t1/s3
...
-- no s3 since it was removed, s4 and s4 should have 2MiB and 1MiB
-- respectively.
`)
	h.query(n1, `
  SELECT range_id, store_id, crdb_internal.humanize_bytes(total_tracked_tokens::INT8)
...
func TestFlowControlRaftMembershipRemoveSelf(t *testing.T) {
	...
	h := newFlowTokenTestHelper(t, tc)
	h.init()
	defer h.close("raft_membership_remove_self")
...
`)
	...
	h.comment(`-- (Removing current raft leader on n1 from raft group.)`)
	testutils.SucceedsSoon(t, func() error {
		// Relocate range from n1 -> n4.
		err = tc.Servers[2].DB().
			AdminRelocateRange(
				context.Background(), desc.StartKey.AsRawKey(),
				tc.Targets(1, 2, 3), nil, false)
		if err != nil {
			return err
		}
		...
		if leaseHolder.Equal(tc.Target(0)) {
			return errors.Errorf("expected leaseholder to not be on n0")
		}
		return nil
	})
	h.waitForAllTokensReturned(ctx, 3)
...
`)
	...
	h.comment(`-- (Transferring range lease and raft leadership to n2.)`)
...
-- leadership. No deducted tokens are returned.
`)
	h.query(n1, `
  SELECT name, crdb_internal.humanize_bytes(value::INT8)
    FROM crdb_internal.node_metrics
...
`)
	...
	h.comment(`
-- All flow tokens are returned back to where the raft leader is.
`)
	h.query(n1, `
  SELECT name, crdb_internal.humanize_bytes(value::INT8)
    FROM crdb_internal.node_metrics
...
type flowTokenTestHelper struct {
	t   *testing.T
...
func newFlowTokenTestHelper(t *testing.T, tc *testcluster.TestCluster) *flowTokenTestHelper {
	rng, _ := randutil.NewPseudoRand()
	buf := &strings.Builder{}
	return &flowTokenTestHelper{
		t:   t,
		tc:  tc,
		buf: buf,
		rng: rng,
...
func (h *flowTokenTestHelper) init() {
...
func (h *flowTokenTestHelper) waitForAllTokensReturned(ctx context.Context, expStreamCount int) {
	testutils.SucceedsSoon(h.t, func() error {
		return h.checkAllTokensReturned(ctx, expStreamCount)
	})
}

func (h *flowTokenTestHelper) checkAllTokensReturned(
...
func (h *flowTokenTestHelper) waitForConnectedStreams(
...
func (h *flowTokenTestHelper) waitForTotalTrackedTokens(
...
func (h *flowTokenTestHelper) comment(comment string) {
	if h.buf.Len() > 0 {
		h.buf.WriteString("\n\n")
	}
...
func (h *flowTokenTestHelper) query(runner *sqlutils.SQLRunner, sql string, headers ...string) {
...
func (h *flowTokenTestHelper) put(
...
func (h *flowTokenTestHelper) close(filename string) {
	echotest.Require(h.t, h.buf.String(), datapathutils.TestDataPath(h.t, "flow_control", filename))
}
