...
func TestGossipAfterAbortOfSystemConfigTransactionAfterFailureDueToIntents(t *testing.T) {
	...
	require.NoError(t, txA.Put(ctx, keys.SystemTenantKeyGen.DescMetadataKey(1000), "foo"))

	require.NoError(t, txB.SetSystemConfigTrigger())
	require.NoError(t, txB.Put(ctx, keys.SystemTenantKeyGen.DescMetadataKey(2000), "bar"))
