	...
	for index, union := range baReqs {
		...
		if baHeader.TargetBytes > 0 {
			if h.ResumeReason == roachpb.RESUME_BYTE_LIMIT {
				...
				baHeader.TargetBytesRequireNextBytes = true
				baHeader.TargetBytes = -1
			}
		}

		// Once a response contains a ResumeSpan we don't want to do further work to
		// determine additional ResumeNextBytes, since only the first ResumeSpan and
		// its ResumeNextBytes is of interest.
		if h.ResumeSpan != nil {
			baHeader.TargetBytesRequireNextBytes = false
		}
	}
