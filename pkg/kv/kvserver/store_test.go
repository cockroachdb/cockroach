...
// and a resume span.
//
// Requests that evaluate via the optimistic evaluation path will usually bump
// the tscache over just the keys that they ended up reading, whereas requests
// that evaluate via the pessimistic path will usually bump the tscache over the
// entire span of the request.
func TestStoreScanResumeTSCache(t *testing.T) {
	...
	cfg := TestStoreConfig(hlc.NewClock(manualClock, time.Nanosecond) /* maxOffset */)
	...
	// retried under the pessimistic path. We disable this behavior here via this
	// testing knob to avoid flakiness.
	cfg.TestingKnobs.RetainEvalKindOnConcurrencyRetry = true
	...
	// Scan the span using 2 Get requests at t3 with max keys and verify the
	// expected resume spans. The one Get request that is evaluated should be
	// accounted for in the timestamp cache, but the second request which is not
	...
	ba.Header = h
	ba.Add(getArgsString("a"), getArgsString("b"))
	br, pErr := store.TestSender().Send(ctx, ba)
	require.Nil(t, pErr)
	...
	// Verify the timestamp cache has been set for "a" but not for "b".
	rTS, _ = store.tsCache.GetMax(roachpb.Key("a"), nil)
