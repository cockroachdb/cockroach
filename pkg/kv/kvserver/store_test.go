...
// TestReserveSnapshotQueueTimeoutAvoidsStarvation verifies that the snapshot
// reservation queue applies a tighter queueing timeout than overall operation
// timeout to incoming snapshot requests, which enables it to avoid starvation
// even with its FIFO queueing policy under high concurrency.
func TestReserveSnapshotQueueTimeoutAvoidsStarvation(t *testing.T) {
	...
	s := tc.store
	snapshotQueueTimeoutFraction.Override(ctx, &s.ClusterSettings().SV, timeoutFrac)
		...
		g.Go(func() error {
			for atomic.LoadInt64(&done) == 0 {
				if err := func() error {
					...
					defer cancel()
					cleanup, err := s.reserveSnapshot(ctx, &SnapshotRequest_Header{RangeSize: 1})
					if err != nil {
					...
					// reactive and terminate the sleep as soon as the ctx is cancelled,
