...
func (s *Store) Send(
	...
	// Avoid mutating the original request, instead creating an immutable BatchRequest copy on
	// each retry.
	baRetry := ba

	// Run a retry loop on retriable RangeKeyMismatchErrors, where the requested RangeID does
	// not match any Range. The reuqest is retriable when the correct Range, corresponding to the
	// request key span also exists entirely on this store, on a different Replica then requested.
	retryOptions := retry.Options{
		InitialBackoff: time.Nanosecond,
		MaxBackoff:     256 * time.Millisecond,
		Multiplier:     2,
	}
	for retrier := retry.StartWithCtx(ctx, retryOptions); retrier.Next(); {
		// Get range and add command to the range for execution.
		repl, err := s.GetReplica(baRetry.RangeID)
		if err != nil {
			return nil, roachpb.NewError(err)
		}
		...
		case *roachpb.RangeKeyMismatchError:
			...
			rSpan, err := keys.Range(baRetry.Requests)
			if err != nil {
				return nil, roachpb.NewError(err)
			}
			...
			for _, ri := range ris {
				...
				candidateRanges = append(candidateRanges, ri)
				...
				if ri.Desc.RSpan().ContainsKeyRange(rSpan.Key, rSpan.EndKey) {
					...
					// Refresh the request and populate the updated request information.
					baRetry = ba
					baRetry.RangeID = ri.Desc.RangeID
					baRetry.ClientRangeInfo.ClosedTimestampPolicy = ri.ClosedTimestampPolicy
					baRetry.ClientRangeInfo.DescriptorGeneration = ri.Desc.Generation
					baRetry.ClientRangeInfo.LeaseSequence = ri.Lease.Sequence
				}
			}

			// if we are able to retry, iterate retry loop.
			if isRetriableRangeKeyMismatch {
				continue
			}

			// Update the RangeInfos that will be returned to the client, to invalidate
			// the client's RangeCache.
			t.AppendRangeInfo(ctx, candidateRanges...)
