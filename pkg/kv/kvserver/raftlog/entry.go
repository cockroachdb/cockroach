...
type Entry struct {
	raftpb.Entry // must be pre-populated when calling Load
...
// MakeEntry populates an Entry from the provided raftpb.Entry.
func MakeEntry(ent raftpb.Entry) (Entry, error) {
	e := Entry{Entry: ent}
...
// MakeEntryFromRawValue populates an Entry from a raw value, i.e. from bytes
// as stored on a storage.Engine.
func MakeEntryFromRawValue(b []byte) (Entry, error) {
	var meta enginepb.MVCCMetadata
	if err := protoutil.Unmarshal(b, &meta); err != nil {
		return Entry{}, errors.Wrap(err, "decoding raft log MVCCMetadata")
	}

	var e Entry

	if err := storage.MakeValue(meta).GetProto(&e.Entry); err != nil {
		return Entry{}, errors.Wrap(err, "unmarshalling raft Entry")
	}
