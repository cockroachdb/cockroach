...
func ents(inds ...uint64) []raftpb.Entry {
	...
	for _, ind := range inds {
		cmd := kvserverpb.RaftCommand{
			MaxLeaseIndex: roachpb.LeaseAppliedIndex(ind), // just to have something nontrivial in here
		}
...
func (it *modelIter) SeekGE(lo roachpb.RaftIndex) (bool, error) {
	for {
		if it.idx >= len(it.ents) || (it.hi > 0 && roachpb.RaftIndex(it.ents[it.idx].Index) >= it.hi) {
			return false, nil
		}
		if ind := roachpb.RaftIndex(it.ents[it.idx].Index); ind >= lo && (it.hi == 0 || ind < it.hi) {
...
func (it *modelIter) Next() (bool, error) {
	...
	if it.hi > 0 && roachpb.RaftIndex(it.ents[it.idx].Index) >= it.hi {
		return false, nil
	}
	err := it.check()
	return err == nil, err
...
func TestIteratorEmptyLog(t *testing.T) {
	...
	for _, hi := range []roachpb.RaftIndex{0, 1} {
...
func TestIterator(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		indToName := func(ind roachpb.RaftIndex) string {
		...
		for _, ent := range tc.ents {
			inds = append(inds, indToName(roachpb.RaftIndex(ent.Index)))
		}
		if len(inds) == 0 {
			inds = []string{"empty"}
		}
		t.Run("log="+strings.Join(inds, ","), func(t *testing.T) {
			...
			for _, ent := range tc.ents {
				...
				require.NoError(t, err)
				require.NoError(t, eng.PutUnversioned(keys.RaftLogKey(rangeID, roachpb.RaftIndex(ent.Index)), metaB))
			}
			...
			var fi roachpb.RaftIndex // firstIndex
			var li roachpb.RaftIndex // lastIndex
			if n := len(tc.ents); n > 0 {
				fi = roachpb.RaftIndex(tc.ents[0].Index)
				li = roachpb.RaftIndex(tc.ents[n-1].Index)
			} else {
...
type iter interface {
	SeekGE(idx roachpb.RaftIndex) (bool, error)
	Next() (bool, error)
	Entry() raftpb.Entry
}

func consumeIter(it iter, lo roachpb.RaftIndex) ([]uint64, error) {
	var sl []uint64
