...
const (
	...
	// empty or whose first byte matches entryEncodingStandardWithACPrefixByte. The
	...
	// EntryEncodingStandardWithoutAC is like
	// EntryEncodingStandardWithAC but without below-raft admission
	// metadata.
...
// UsesReplicationAdmissionControl returns true if the encoding is
// EntryEncoding{Standard,Sideloaded}WithAC.
func (enc EntryEncoding) UsesReplicationAdmissionControl() bool {
...
// raftpb.Entry.Data. Assumes an EntryEncoding{Standard,Sideloaded}WithAC
// encoding.
func DecodeRaftAdmissionMeta(data []byte) (kvserverpb.RaftAdmissionMeta, error) {
	prefix := data[0]
	...
	// up slightly.
	var raftAdmissionMeta kvserverpb.RaftAdmissionMeta
	if err := protoutil.Unmarshal(data[1+RaftCommandIDLen:], &raftAdmissionMeta); err != nil {
		return kvserverpb.RaftAdmissionMeta{}, err
	}
