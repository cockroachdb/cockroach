...
// direction. The algorithms for each is slightly different. First, we present
// the algorithms for each, and then we talk about the differences (and the
// reasons for them).
//
// -----------------------------------------------------------------------------
...
// reaches the hard upper bound constaint (i.e. constrMaxReached is true).
// constrMaxReached is the first item whose start key is greater than or equal
// to the search range's end key.
// 2. The reverse overlaping scan can directly "jump" to the item before
// constrMaxReached.
...
// item's start key. Otherwise, constrMaxReached would be true, and we'd
// terminate the scan. So, the item must overlap with the search range, and we
// can skip key comparisons.
// [2] After it's jumped to the item right before constrMaxReached.
//
...
// will ever see.
// 2. The number of children at an inter-node is one more than the number of
...
// last child for inter-nodes, and the last item for leaf-nodes. These are "off
...
func (i *iterator) findPrevOverlap(item *latch) {
	for {
		...
		if i.n == i.o.constrMaxN && i.pos > i.o.constrMaxPos {
			...
			// can't just position the iterator to i.o.constrMaxPos - 1; if
		...
		// Iterate across node.
		if i.pos >= 0 && i.o.constrMaxReached {
			if i.o.inConstrMin {
				// Fast-path to avoid span comparison. i.o.inConstrMin
