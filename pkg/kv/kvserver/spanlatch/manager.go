...
func (lg *Guard) maybeGetTxnMeta() *enginepb.TxnMeta {
	if lg.ba == nil || lg.ba.Txn == nil {
		return nil
	}
	return &lg.ba.Txn.TxnMeta
}
...
func (m *Manager) waitForSignal(
	...
	if sp != nil || m.latchWaitDurations != nil {
		...
		defer m.logWaitTime(sp, tBegin, wait, held)
	}
...
func (m *Manager) logWaitTime(sp *tracing.Span, startTime crtime.Mono, wait, held *latch) {
