...
func (m *Manager) wait(ctx context.Context, lg *Guard, snap snapshot) error {
	// Register latch wait state for ASH sampling.
	var workloadID uint64
	var appNameID uint64
	var gatewayNodeID roachpb.NodeID
	if lg.ba != nil {
		workloadID = lg.ba.Header.WorkloadId
		appNameID = lg.ba.Header.AppNameID
		gatewayNodeID = lg.ba.Header.SQLGatewayNodeID
	}
	clearWorkState := ash.SetWorkStateWithAppName(workloadID, ash.WORK_LOCK, "LatchWait", appNameID, gatewayNodeID)
