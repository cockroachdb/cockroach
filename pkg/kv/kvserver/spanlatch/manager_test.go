...
func TestLatchManagerPoison(t *testing.T) {
	...
	var m Manager
	chga1, _ := m.MustAcquireChExt(ctx, spans("a", "", write, zeroTS), poison.Policy_Wait)
	chga2, errCh2 := m.MustAcquireChExt(ctx, spans("a", "", write, zeroTS), poison.Policy_Error)
	chga3, errCh3 := m.MustAcquireChExt(ctx, spans("a", "", write, zeroTS), poison.Policy_Error)
	chga4, _ := m.MustAcquireChExt(ctx, spans("a", "", write, zeroTS), poison.Policy_Wait)
	chga5, errCh5 := m.MustAcquireChExt(ctx, spans("a", "", write, zeroTS), poison.Policy_Error)

	ga1 := testLatchSucceeds(t, chga1)

	// chga5 blocks on chga4 blocks on chga3 blocks on chga2 blocks on chga1.
	testLatchBlocks(t, chga5)
	testLatchBlocks(t, chga4)
	testLatchBlocks(t, chga3)
	testLatchBlocks(t, chga2)
	...
	testLatchPoisons(t, errCh2)
	testLatchPoisons(t, errCh3)
	testLatchPoisons(t, errCh5)

	// Release ga1, which allows ga4 to sequence. At that point, we can check
	// directly that it is poisoned.
	m.Release(ga1)
	ga4 := testLatchSucceeds(t, chga4)
	require.True(t, ga4.poison.sig.signaled())
}
