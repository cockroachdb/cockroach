...
type StoreGossip struct {
	...
	gossiper GossipInfoAdder
	// descriptorGetter is used for getting an up to date or cached store
	// descriptor to gossip.
	descriptorGetter StoreDescriptorGetter
}

// StoreGossipTestingKnobs... TODO(kvoli):
...
func DefaultStoreGossipTestingKnobs() StoreGossipTestingKnobs {
	return StoreGossipTestingKnobs{
		GossipWhenCapacityDeltaExceedsFraction: defaultGossipWhenCapacityDeltaExceedsFraction,
		StoreGossipIntercept:                   nil,
		DisableLeaseCapacityGossip:             false,
		AsyncDisabled:                          false,
	}
}
...
func NewStoreGossip(
	gossiper GossipInfoAdder, descGetter StoreDescriptorGetter, testingKnobs StoreGossipTestingKnobs,
) *StoreGossip {
	return &StoreGossip{
		cachedCapacity:   &cachedCapacity{},
		gossiper:         gossiper,
...
// StoreDescriptorGetter provides a method to access the store descriptor.
type StoreDescriptorGetter interface {
	// Descriptor returns a StoreDescriptor including current store
	// capacity information.
	Descriptor(ctx context.Context, cached bool) (*roachpb.StoreDescriptor, error)
}

// GossipInfoAdder provides a method to add a message to gossip.
type GossipInfoAdder interface {
...
func (s *StoreGossip) shouldGossipOnCapacityDelta() (should bool, reason string) {
	...
	updateForRangeCount, deltaRangeCount := deltaExceedsThreshold(
		...
		gossipWhenRangeCountDeltaExceeds, s.knobs.GossipWhenCapacityDeltaExceedsFraction)
	...
	updateForLeaseCount, deltaLeaseCount := deltaExceedsThreshold(
		...
		gossipWhenLeaseCountDeltaExceeds, s.knobs.GossipWhenCapacityDeltaExceedsFraction)
	s.cachedCapacity.Unlock()
...
func deltaExceedsThreshold(
	old, new, requiredMinDelta, requiredDeltaFraction float64,
) (exceeds bool, delta float64) {
	delta = new - old
	deltaAbsolute := math.Abs(new - old)
	deltaFraction := 10e9
