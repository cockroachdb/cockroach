...
func TestSystemSpanConfigProtectionPoliciesApplyAfterGC(t *testing.T) {
	...
	args.ServerArgs.DisableDefaultTestTenant = true
	...
	_, err := sqlDB.Exec("SET CLUSTER SETTING kv.closed_timestamp.target_duration = '100ms'")
	require.NoError(t, err)
	...
	testutils.SucceedsSoon(t, func() error {
		if repl.SpanConfig().GCPolicy.TTLSeconds != 1 {
...
SELECT count(*)
  FROM (
	...
	mkRecordAndProtect(hlc.Timestamp{WallTime: protectedTime}, clusterTarget)

	// Wait for the protection policy to apply to foo's range.
	testutils.SucceedsSoon(t, func() error {
		if len(repl.SpanConfig().GCPolicy.ProtectionPolicies) == 0 {
			return errors.New("waiting for span config to apply")
		}
	...
	testutils.SucceedsSoon(t, func() error {
		...
		if !threshold.Equal(ptsTime.Prev()) {
			if ptsTime.Prev().Less(threshold) {
				t.Fatal("range GC threshold should never pass protected timestamp")
			}
