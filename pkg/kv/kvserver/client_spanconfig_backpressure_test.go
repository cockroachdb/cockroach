...
func TestSpanConfigUpdatesBlockedByRangeSizeBackpressureOnDefaultRanges(t *testing.T) {
	...
	const (
		...
		numWrites             = 64 << 10
		// 65,536 writes, this was calculated by (64 MiB / 2 KiB) * 2.
		// (2 KiB is the size of the spanconfig record `spanConfig2Kib`).
		// See func exceedsMultipleOfSplitSize in
		// /pkg/kv/kvserver/replica_metrics.go for the logic.
	...
	store, err := tc.GetStores().(*kvserver.Stores).GetStore(
		tc.GetFirstStoreID())
	require.NoError(t, err)

	waitForSpanConfig := func(t *testing.T, tc serverutils.TestServerInterface,
		tablePrefix roachpb.Key, exp int64) {
		testutils.SucceedsSoon(t, func() error {
			...
			if log.V(1) {
				log.Dev.Infof(ctx,
					"RangeMaxBytes for tablePrefix %s: %d\n", tablePrefix,
					conf.RangeMaxBytes)
			}
	...
	// This is a big spanconfig with all fields set to maximum int64 and int32
	// values. This is done to have a spanconfig that is large enough to trigger
	// backpressure without having to write a million records.
	// Having this be 2 KiB gives us (64 << 20 / 2 << 10) * 2 = 65,536 writes.
	// See fn exceedsMultipleOfSplitSize in /pkg/kv/kvserver/replica_metrics.go
	// for the logic.
	...
	log.Dev.Infof(ctx,
		"Targeting span_configurations table at key: %s (table ID %d)\n",
		spanConfigTablePrefix, keys.SpanConfigurationsTableID)

	log.Dev.Infof(ctx,
		"Configuring span_configurations table with custom zone settings...\n")
	...
	testutils.SucceedsSoon(t, func() error {
		...
		if conf.RangeMaxBytes != overloadMaxRangeBytes {
			return fmt.Errorf("expected RangeMaxBytes %d, got %d",
				overloadMaxRangeBytes, conf.RangeMaxBytes)
		}
		return nil
	})
	...
	if repl != nil {
		...
		if err != nil {
			...
			log.Dev.Infof(
				ctx, `Current range config - RangeMaxBytes: %d bytes (%d MiB), 
	...
	log.Dev.Infof(ctx,
		...
		spanConfigTablePrefix, keys.SpanConfigurationsTableID)
	log.Dev.Infof(ctx,
		"Direct KV writes to span_configurations table range %d times...\n",
		numWrites)
	...
	for i = 0; i < numWrites; i++ {
		...
		err = kvaccessor.UpdateSpanConfigRecords(ctx,
			nil, []spanconfig.Record{testRecord},
			hlc.MinTimestamp, hlc.MaxTimestamp)
		if log.V(1) {
			log.Dev.Infof(ctx,
				"KVAccessor write %d/%d: target=%q\n", i+1,
				numWrites, testTargetKey)
		}
		if err != nil {
			log.Dev.Infof(ctx,
				`ERROR! BREAKING OUT OF LOOP, 
				numWrites successful: %d, error: %+v\n`, i, err)
			break
		}
	}
	...
	if repl != nil {
		stats := repl.GetMVCCStats()
		log.Dev.Infof(ctx, `Range size after all writes: %d bytes 
			(KeyCount: %d, LiveBytes: %d)\n`, stats.Total(), stats.KeyCount,
			stats.LiveBytes)
	}
	...
	smallSpancofnRecordWriteErr := kvaccessor.UpdateSpanConfigRecords(ctx,
		[]spanconfig.Target{testTarget},
		[]spanconfig.Record{smallSpancofnRecord},
		hlc.MinTimestamp, hlc.MaxTimestamp)
	if smallSpancofnRecordWriteErr != nil {
		log.Dev.Infof(ctx, "ERROR: smallSpancofnRecord write failed: %v\n",
			smallSpancofnRecordWriteErr)
	}
	require.NoError(t, smallSpancofnRecordWriteErr,
		`Expected smallSpancofnRecord write to succeed (spanconfigs 
		should bypass backpressure)`)
	log.Dev.Infof(ctx, `SUCCESS: smallSpancofnRecord write succeeded as expected 
		(spanconfigs bypassed backpressure)\n`)
