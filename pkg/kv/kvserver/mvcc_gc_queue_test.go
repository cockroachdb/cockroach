...
func TestMVCCGCQueueGroupsRangeDeletions(t *testing.T) {
	...
	store, clock := createTestStore(ctx, t, testStoreOpts{createSystemRanges: false}, stopper)
	...
	gcQueue.leaseholderCheck = func(ctx context.Context, replica *Replica, timeout time.Duration,
	) error {
		return leaseError
	}
	...
	leaseError = errors.New("not lease holder")
	gcQueue.postProcessScheduled(ctx, r1, deleteRangePriority)
	qr, _ = gcQueue.pop()
