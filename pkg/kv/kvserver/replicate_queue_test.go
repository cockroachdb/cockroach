...
func TestReplicateQueueRebalanceMultiStore(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			// ranges in the cluster which brings us to 68 leases (ranges) total. Each
			// store should have around 68/8=8.5 leases. With a leasesThreshold of 70%
			// the test expects floor(8.5*0.7)=5 leases per store. The allocator
			// should allow up to ceil(8.5)=9 leases per store, meaning, that in the
			// worst case 7 stores can have 9 leases each, not more, which leaves us
			// with 68-7*9=5 leases on the 8th store, which is exactly what the test
			// expects, and therefore should not be flaky.
			// Note that without setting LeaseRebalanceThreshold and
			// LeaseRebalanceThresholdMin above, we would need more than 100 ranges
			// per store, which will make this test much slower, probably more than
			// 5-10 minutes, where currently it should succeed within a minute
			// normally.
