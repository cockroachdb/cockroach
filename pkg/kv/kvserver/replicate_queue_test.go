...
func TestReplicateQueueDownReplicate(t *testing.T) {
	...
	// all "system ranges". When the ScratchRange(), it also starts up with 5
	// replicas. Since it's not a system range, its default zone config asks for
	// 3x replication, and the replication queue will down-replicate it.
