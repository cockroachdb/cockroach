...
func (b *ringBuf) add(ents []raftpb.Entry) (addedBytes, addedEntries int32) {
	if it := last(b); it.valid(b) && roachpb.RaftIndex(ents[0].Index) > it.index(b)+1 {
	...
	before, after, ok := computeExtension(b, roachpb.RaftIndex(ents[0].Index), roachpb.RaftIndex(ents[len(ents)-1].Index))
	if !ok {
		return
	}
	...
	if before == 0 && after != b.len { // skip unchanged prefix
		it, _ = iterateFrom(b, roachpb.RaftIndex(ents[0].Index)) // safe by construction
	}
...
func (b *ringBuf) truncateFrom(lo roachpb.RaftIndex) (removedBytes, removedEntries int32) {
	if b.len == 0 {
		return
	}
...
func (b *ringBuf) clearTo(hi roachpb.RaftIndex) (removedBytes, removedEntries int32) {
...
func (b *ringBuf) get(index roachpb.RaftIndex) (e raftpb.Entry, ok bool) {
...
func (b *ringBuf) scan(
	ents []raftpb.Entry, lo roachpb.RaftIndex, hi roachpb.RaftIndex, maxBytes uint64,
...
func computeExtension(b *ringBuf, lo, hi roachpb.RaftIndex) (before, after int, ok bool) {
	if b.len == 0 {
...
func iterateFrom(b *ringBuf, index roachpb.RaftIndex) (_ iterator, ok bool) {
	if b.len == 0 {
		return -1, false
	}
...
func (it iterator) index(b *ringBuf) roachpb.RaftIndex {
	return roachpb.RaftIndex(b.buf[it].Index)
}
