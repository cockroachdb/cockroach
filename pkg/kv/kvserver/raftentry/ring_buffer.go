...
func (b *ringBuf) add(ents []raftpb.Entry) (addedBytes, addedEntries int32) {
	if it := last(b); it.valid(b) && enginepb.RaftIndex(ents[0].Index) > it.index(b)+1 {
	...
	before, after, ok := computeExtension(b, enginepb.RaftIndex(ents[0].Index), enginepb.RaftIndex(ents[len(ents)-1].Index))
	if !ok {
		return
	}
	...
	if before == 0 && after != b.len { // skip unchanged prefix
		it, _ = iterateFrom(b, enginepb.RaftIndex(ents[0].Index)) // safe by construction
	}
...
func (b *ringBuf) truncateFrom(lo enginepb.RaftIndex) (removedBytes, removedEntries int32) {
	if b.len == 0 {
		return
	}
...
func (b *ringBuf) clearTo(hi enginepb.RaftIndex) (removedBytes, removedEntries int32) {
...
func (b *ringBuf) get(index enginepb.RaftIndex) (e raftpb.Entry, ok bool) {
...
func (b *ringBuf) scan(
	ents []raftpb.Entry, lo enginepb.RaftIndex, hi enginepb.RaftIndex, maxBytes uint64,
...
func computeExtension(b *ringBuf, lo, hi enginepb.RaftIndex) (before, after int, ok bool) {
	if b.len == 0 {
...
func iterateFrom(b *ringBuf, index enginepb.RaftIndex) (_ iterator, ok bool) {
	if b.len == 0 {
		return -1, false
	}
...
func (it iterator) index(b *ringBuf) enginepb.RaftIndex {
	return enginepb.RaftIndex(b.buf[it].Index)
}
