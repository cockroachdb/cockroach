...
func newEntry(index enginepb.RaftIndex, size int) raftpb.Entry {
...
func newEntries(lo, hi enginepb.RaftIndex, size int) []raftpb.Entry {
...
func addEntries(c *Cache, rangeID roachpb.RangeID, lo, hi enginepb.RaftIndex) []raftpb.Entry {
...
func verifyGet(
	...
	expNextIndex enginepb.RaftIndex,
	allowEviction bool,
) {
	t.Helper()
	...
	for _, e := range ents {
		found, ok := c.Get(rangeID, enginepb.RaftIndex(e.Index))
		if !ok {
			if allowEviction {
				break
			}
...
func requireEqual(t *testing.T, c *Cache, rangeID roachpb.RangeID, idxs ...enginepb.RaftIndex) {
	t.Helper()
...
func TestEntryCache(t *testing.T) {
	...
	verify := func(rangeID roachpb.RangeID, lo, hi enginepb.RaftIndex, ents []raftpb.Entry, expNextIndex enginepb.RaftIndex) {
		t.Helper()
...
func TestConcurrentEvictions(t *testing.T) {
	...
	doRead := func(r roachpb.RangeID) {
		...
		lo := enginepb.RaftIndex(ents[offset].Index)
		hi := lo + enginepb.RaftIndex(length)
		wg.Add(1)
		go func() {
	...
	doWrite := func(r roachpb.RangeID) {
		...
		if offset >= 0 && offset < len(ents) {
			...
			hi := lo + uint64(length)
			toAdd = newEntries(enginepb.RaftIndex(lo), enginepb.RaftIndex(hi), 9)
	...
	for r, data := range rangeData {
		...
		c.Clear(r, enginepb.RaftIndex(data[len(data)-1].Index+1))
	}
...
func TestConcurrentUpdates(t *testing.T) {
	...
	for _, clearMethod := range []struct {
		...
		{"drop", func() { c.Drop(r1) }},
		{"clear", func() { c.Clear(r1, enginepb.RaftIndex(ents[len(ents)-1].Index+1)) }},
	} {
...
func TestConcurrentAddGetAndEviction(t *testing.T) {
	...
	doAddAndGetToRange := func(rangeID roachpb.RangeID) {
		...
		doAction(func() { c.Get(rangeID, enginepb.RaftIndex(ents[0].Index)) })
	}
...
func BenchmarkEntryCache(b *testing.B) {
	...
	for i := range ents {
		ents[i] = newEntry(enginepb.RaftIndex(i+1), 9)
	}
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		b.StopTimer()
		...
		_, _, _, _ = c.Scan(nil, rangeID, 0, enginepb.RaftIndex(len(ents)-10), noLimit)
		c.Clear(rangeID, enginepb.RaftIndex(len(ents)-10))
	}
}
