...
func eq(t *testing.T, b *ringBuf, idxs ...enginepb.RaftIndex) {
	t.Helper()
	var sl []enginepb.RaftIndex
	it := first(b)
	for it.valid(b) {
		idx := it.index(b)
		sl = append(sl, idx)
		...
		ent, ok := b.get(idx)
		require.True(t, ok)
		require.Equal(t, idx, enginepb.RaftIndex(ent.Index))
	}
