...
type rangeCache interface {
	...
	truncateFrom(lo roachpb.RaftIndex) (bytesRemoved, entriesRemoved int32)
	clearTo(hi roachpb.RaftIndex) (bytesRemoved, entriesRemoved int32)
	get(index roachpb.RaftIndex) (raftpb.Entry, bool)
	scan(ents []raftpb.Entry, lo, hi roachpb.RaftIndex, maxBytes uint64) (
		_ []raftpb.Entry, bytes uint64, nextIdx roachpb.RaftIndex, exceededMaxBytes bool)
}
...
func (c *Cache) Add(id roachpb.RangeID, ents []raftpb.Entry, truncate bool) {
	...
	if truncate {
		...
		truncIdx := roachpb.RaftIndex(ents[0].Index)
...
func (c *Cache) Clear(id roachpb.RangeID, hi roachpb.RaftIndex) {
	c.mu.Lock()
	p := c.getPartLocked(id, false /* create */, false /* recordUse */)
	if p == nil {
		c.mu.Unlock()
...
func (c *Cache) Get(id roachpb.RangeID, idx roachpb.RaftIndex) (e raftpb.Entry, ok bool) {
...
// entries.
func (c *Cache) Scan(
	ents []raftpb.Entry, id roachpb.RangeID, lo, hi roachpb.RaftIndex, maxBytes uint64,
) (_ []raftpb.Entry, bytes uint64, nextIdx roachpb.RaftIndex, exceededMaxBytes bool) {
	c.metrics.Accesses.Inc(1)
	c.mu.Lock()
	p := c.getPartLocked(id, false /* create */, true /* recordUse */)
