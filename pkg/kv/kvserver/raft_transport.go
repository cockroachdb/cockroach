...
type RaftTransport struct {
	...
	// also maintain kvflowControlMu.connectedNodes. So writing to this field is
	// done while holding kvflowControl.mu.
	...
	kvflowControl struct {
		...
		// protocol, and can be experimented with using TestFlowTokenTransport
		...
		//   uses of connectedStores below and how the storesForFlowControl
		...
		//     connected.
		//
		// TODO(irfansharif): Do we need the background
		// startPruningDisconnectedStores logic below? For every stream that's
		// established, aren't we guaranteed to observe a disconnection event at
		// the very end? So the concern around concurrent stream establishments
		// shouldn't matter. Even for decommissioned nodes -- once we've
		// disconnected all streams to it, won't we have just cleared it out?
		//
		...
		mu struct {
			syncutil.Mutex
			localStoreIDs                 []roachpb.StoreID
			connectedStores               RaftTransportConnectedStores // as seen by the server-side of RaftMessageBatch
			connectedNodes                RaftTransportConnectedNodes  // as seen by the client-side of RaftMessageBatch
			setAdditionalStoreIDs         atomic.Bool                  // can be accessed without holding mutex
			startedPruningConnectedStores atomic.Bool                  // can be accessed without holding mutex
		}
		dispatchReader       kvflowcontrol.DispatchReader
		storesForFlowControl StoresFlowControlIntegration
	}
...
func NewDummyRaftTransport(st *cluster.Settings, tracer *tracing.Tracer) *RaftTransport {
	...
	return NewRaftTransport(log.MakeTestingAmbientContext(tracer), st, tracer,
		...
		NoopRaftTransportConnectedStores{}, NoopRaftTransportConnectedNodes{},
		nil,
	)
}
...
func NewRaftTransport(
	...
	storesForFlowControl StoresFlowControlIntegration,
	connectedStores RaftTransportConnectedStores,
	connectedNodes RaftTransportConnectedNodes,
	knobs *RaftTransportTestingKnobs,
) *RaftTransport {
	if knobs == nil {
		knobs = &RaftTransportTestingKnobs{}
	...
	t.kvflowControl.storesForFlowControl = storesForFlowControl
	t.kvflowControl.mu.connectedStores = connectedStores
	t.kvflowControl.mu.connectedNodes = connectedNodes
	t.initMetrics()
	if grpcServer != nil {
...
func (t *RaftTransport) Start(ctx context.Context) error {
	if err := t.startPruningDisconnectedStores(ctx); err != nil {
		return errors.Wrapf(err, "failed to start pruning connected stores")
	}
...
func (t *RaftTransport) SetAdditionalStoreIDs(storeIDs []roachpb.StoreID) {
	...
	t.kvflowControl.mu.setAdditionalStoreIDs.Store(true)
}
...
func (t *RaftTransport) handleRaftRequest(
	...
	for i := range req.AdmittedRaftLogEntries {
		...
		handle, found := t.kvflowControl.storesForFlowControl.Lookup(admittedEntries.RangeID)
		if found {
...
func (t *RaftTransport) RaftMessageBatch(stream MultiRaft_RaftMessageBatchServer) (lastErr error) {
	...
	defer func() {
		...
		t.kvflowControl.mu.Lock()
		t.kvflowControl.mu.connectedStores.MarkDisconnected(storeIDs)
		t.kvflowControl.mu.Unlock()
		t.kvflowControl.storesForFlowControl.OnRaftTransportDisconnected(ctx, storeIDs...)
		...
		stop.TaskOpts{
			...
			errCh <- func() error {
				...
				for {
					...
					t.kvflowControl.mu.Lock()
					t.kvflowControl.mu.connectedStores.MarkConnected(storeIDs, timeutil.Now())
					t.kvflowControl.mu.Unlock()
...
func (t *RaftTransport) processQueue(
	...
	maybeAnnotateWithAdmittedRaftLogEntries := func(
		...
		if log.V(2) && flowTokenDispatchCount > 0 {
			for i := range req.AdmittedRaftLogEntries {
				admittedEntries := req.AdmittedRaftLogEntries[i]
				log.Infof(ctx, "informing n%s of below-raft admission (r%d s%s pri=%s up-to-%s): %d out of %d dispatches",
	...
	maybeAnnotateWithStoreIDs := func(batch *kvserverpb.RaftMessageRequestBatch) {
		shouldSendAdditionalStoreIDs := t.kvflowControl.mu.setAdditionalStoreIDs.Load() && !sentAdditionalStoreIDs
...
func (t *RaftTransport) getQueue(
	...
	if !ok {
		...
		t.kvflowControl.mu.connectedNodes.MarkConnected(nodeID, class)
		t.kvflowControl.mu.Unlock()
	}
...
func (t *RaftTransport) startProcessNewQueue(
	...
	worker := func(ctx context.Context) {
		...
		defer func() {
			...
			t.kvflowControl.mu.connectedNodes.MarkDisconnected(toNodeID, class)
			t.kvflowControl.mu.Unlock()
		}()
...
func (t *RaftTransport) startPruningDisconnectedStores(ctx context.Context) error {
	if t.kvflowControl.mu.startedPruningConnectedStores.CompareAndSwap(false, true) {
		ctx = t.AnnotateCtx(ctx)
		if err := t.stopper.RunAsyncTask(
			ctx,
			"kvserver.RaftTransport: prune disconnected stores",
			func(ctx context.Context) {
				settingChangeCh := make(chan struct{}, 1)
				kvadmission.PruneDisconnectedStoresInterval.SetOnChange(
				...
				for {
					interval := kvadmission.PruneDisconnectedStoresInterval.Get(&t.st.SV)
					if interval > 0 {
...
func (t *RaftTransport) dropFlowTokensForDisconnectedNodes() {
	...
	for _, nodeID := range t.kvflowControl.dispatchReader.PendingDispatch() {
		if t.kvflowControl.mu.connectedNodes.IsConnected(nodeID) {
...
func (t *RaftTransport) pruneDisconnectedStores(ctx context.Context) {
	t.kvflowControl.mu.Lock()
	toDisconnect := t.kvflowControl.mu.connectedStores.ShouldDisconnect(
		timeutil.Now(), kvadmission.ConnectedStoreExpiration.Get(&t.st.SV))
	t.kvflowControl.mu.connectedStores.MarkDisconnected(toDisconnect)
	t.kvflowControl.mu.Unlock()
	t.kvflowControl.storesForFlowControl.OnRaftTransportDisconnected(ctx, toDisconnect...)
}
...
type RaftTransportTestingKnobs struct {
	...
	// idle, identified by the remote node ID.
