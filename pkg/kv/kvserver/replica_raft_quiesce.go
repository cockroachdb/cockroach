...
func shouldReplicaQuiesce(
	...
	// expiration-based leases. The lease may be invalid, but in that case we
	// wouldn't be able to quiesce anyway (see leaseholder condition below).
	if lease, _ := q.getLeaseRLocked(); lease.Type() == roachpb.LeaseExpiration {
