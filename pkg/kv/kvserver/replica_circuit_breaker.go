...
func (br *replicaCircuitBreaker) asyncProbe(report func(error), done func()) {
	...
	if err := br.stopper.RunAsyncTask(bgCtx, "replica-probe", func(ctx context.Context) {
		...
		// Poison any inflight latches.
		//
		// TODO(during review): discuss what happens if a request sneaks in between
		// the next line and sendProbe. I think the answer is just that we will
		// start a new probe with each call to refreshProposalsLocked that
		// reproposes entries, so there is something that will catch any stragglers.
		// We need to discuss if that mechanism is OK since it means we'll roughly
		// have a probe per tripped Replica in-flight most of the time, so in the
		// worst case as many probes as Replicas.
