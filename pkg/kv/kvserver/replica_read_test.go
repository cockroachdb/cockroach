...
func TestVirtualizedIntentResolution(t *testing.T) {
	...
	for _, c := range testCases {
		...
		if !c.readGUL.IsEmpty() {
			...
			// TODO(mira): How does this read txn get this older observation? It's not
			// while pushing, because the ClockWhilePending observation is taken
			// before the push, and we need something smaller than ClockWhilePending.
			readTxn.UpdateObservedTimestamp(roachpb.NodeID(1), ts300.UnsafeToClockTimestamp())
			ba.Txn = readTxn
		}
		gArgs := getArgs(k)
		ba.Add(&gArgs)
