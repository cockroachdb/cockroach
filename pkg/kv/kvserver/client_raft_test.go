...
// TestRaftPreVote tests that Raft PreVote works properly, including the time
// dependence only enabled via CheckQuorum. Specifically, a replica that's
// partitioned away from the leader (or restarted) should not be able to call an
// election, even if it's still up-to-date on the log. This should be prevented
// because followers should not grant prevotes if they've heard from a leader in
// the past election timeout.
//
	...
	testutils.RunTrueAndFalse(t, "partial", func(t *testing.T, partial bool) {
		testutils.RunTrueAndFalse(t, "quiesce", func(t *testing.T, quiesce bool) {
			testutils.RunTrueAndFalse(t, "expLease", func(t *testing.T, expLease bool) {
				...
				// The candidate shouldn't change state for 3 seconds.
