...
func waitForTruncationForTesting(
	t *testing.T, r *kvserver.Replica, newFirstIndex roachpb.RaftIndex,
) {
	testutils.SucceedsSoon(t, func() error {
...
func TestReplicaGCRace(t *testing.T) {
	...
	testutils.SucceedsSoon(t, func() error {
		...
		for i := range hbReq.Heartbeats {
			hbReq.Heartbeats[i].Term = roachpb.RaftTerm(status.Term)
			hbReq.Heartbeats[i].Commit = roachpb.RaftIndex(progress.Match)
		}
		return nil
	})
