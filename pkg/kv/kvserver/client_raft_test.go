	...
	testutils.RunTrueAndFalse(t, "partial", func(t *testing.T, partial bool) {
		testutils.RunTrueAndFalse(t, "quiesce", func(t *testing.T, quiesce bool) {
			testutils.RunTrueAndFalse(t, "expLease", func(t *testing.T, expLease bool) {
				...
				// range to stabilize such that all replicas have the same last and
				// commit index for a second -- similarly to the quiesce checks.
				if quiesce {
					...
					require.Eventually(t, func() bool {
						...
						if status.Commit != lastIndex {
							t.Logf("n1 commit index %d not at last index %d, waiting", status.Commit, lastIndex)
							return false
						}
