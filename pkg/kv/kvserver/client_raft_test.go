...
func TestStoreRecoverFromEngine(t *testing.T) {
	...
	get := func(store *kvserver.Store, key roachpb.Key) int64 {
		args := getArgs(key)
		resp, err := kv.SendWrappedWith(context.Background(), store.TestSender(), roachpb.Header{}, args)
		if err != nil {
			t.Fatal(err)
		}
	...
	if _, err := kv.SendWrappedWith(context.Background(), store.TestSender(), roachpb.Header{}, incArgs); err != nil {
		t.Fatal(err)
	}
...
func TestStoreRecoverWithErrors(t *testing.T) {
	...
	tc.StopServer(0)
	tc.AddAndStartServer(t, stickySpecTestServerArgs)
	recoveredStore := tc.GetFirstStoreFromServer(t, 1)

	// Issue a no-op write to lazily initialize raft on the range.
	keyB := roachpb.Key("b")
	incArgs = incrementArgs(keyB, 0)
	if _, err := kv.SendWrapped(context.Background(), recoveredStore.TestSender(), incArgs); err != nil {
		t.Fatal(err)
	}
...
func TestReplicateRange(t *testing.T) {
	...
	if ok, err := storage.MVCCGetProto(context.Background(), store.Engine(), key,
	...
	testutils.SucceedsSoon(t, func() error {
		...
		for _, key := range []roachpb.RKey{meta2, meta1} {
			...
			if ok, err := storage.MVCCGetProto(context.Background(), store.Engine(), key.AsRawKey(),
	...
	testutils.SucceedsSoon(t, func() error {
		...
		if reply, err := kv.SendWrappedWith(context.Background(), tc.GetFirstStoreFromServer(t, 1).TestSender(), roachpb.Header{
...
func TestRestoreReplicas(t *testing.T) {
	...
	if _, err := kv.SendWrapped(context.Background(),
	...
	for i := 2; i < len(tc.Servers); i++ {
		if _, pErr := kv.SendWrapped(context.Background(), tc.GetFirstStoreFromServer(t, i).TestSender(), incArgs); pErr != nil {
			failures++
	...
	testutils.SucceedsSoon(t, func() error {
		...
		if reply, err := kv.SendWrappedWith(context.Background(), followerStore.TestSender(), roachpb.Header{
			ReadConsistency: roachpb.INCONSISTENT,
		}, getArgs); err != nil {
			return errors.Errorf("failed to read data: %s", err)
	...
	validate(tc.GetFirstStoreFromServer(t, 2))
	validate(tc.GetFirstStoreFromServer(t, 3))
}
...
func TestRaftLogSizeAfterTruncation(t *testing.T) {
	...
	if _, err := kv.SendWrapped(
		context.Background(), store.TestSender(), incArgs); err != nil {
		t.Fatal(err)
	}
	...
	truncArgs := truncateLogArgs(index+1, repl.GetRangeID())
	if _, err := kv.SendWrapped(
		context.Background(), store.TestSender(), truncArgs); err != nil {
		t.Fatal(err)
	}
...
func TestSnapshotAfterTruncation(t *testing.T) {
	...
	for _, changeTerm := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			tc.WaitForValues(t, key, []int64{incAB, incAB})
			...
			if changeTerm {
				...
				tc.AddAndStartServer(t, stickyServerArgs[stoppedStore])
				stoppedStore = len(tc.Servers) - 1
				testutils.SucceedsSoon(t, func() error {
					hasLeader := false
					term := uint64(0)
					for i := 3; i < len(tc.Servers); i++ {
...
func TestConcurrentRaftSnapshots(t *testing.T) {
	...
	tc.AddAndStartServer(t, stickyServerArgs[1])
	...
	testutils.RunTrueAndFalse(t, "proposeOnFollower", func(t *testing.T, proposeOnFollower bool) {
		...
		for _, s := range []int{3, 4} {
			if tc.ServerStopped(s) {
				tc.AddAndStartServer(t, stickyServerArgs[s])
			}
		}
...
func TestProgressWithDownNode(t *testing.T) {
	...
	tc.WaitForValues(t, key, []int64{16, 16})
...
func TestReportUnreachableHeartbeats(t *testing.T) {
	...
	if _, err := kv.SendWrapped(
		context.Background(), tc.GetFirstStoreFromServer(t, 0).TestSender(),
...
func TestReplicateAfterSplit(t *testing.T) {
	...
	if _, err := kv.SendWrappedWith(context.Background(), store0.TestSender(),
...
func TestRaftRemoveRace(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		if ok, err := storage.MVCCGetProto(
			context.Background(), tc.GetFirstStoreFromServer(t, 2).Engine(), tombstoneKey,
...
func TestReplicateRemovedNodeDisruptiveElection(t *testing.T) {
	...
	if _, err := kv.SendWrapped(context.Background(), tc.Servers[1].DistSender(), incArgs); err != nil {
		t.Fatal(err)
	}
...
func TestReplicaTooOldGC(t *testing.T) {
	...
	tc.AddAndStartServer(t, stickyServerArgs[3])
	...
	tc.GetFirstStoreFromServer(t, 4).EnqueueRaftUpdateCheck(desc.RangeID)

	testutils.SucceedsSoon(t, func() error {
		replica, err := tc.GetFirstStoreFromServer(t, 4).GetReplica(desc.RangeID)
		if err != nil {
			if errors.HasType(err, (*roachpb.RangeNotFoundError)(nil)) {
				return nil
			}
...
func TestReplicateReAddAfterDown(t *testing.T) {
	...
	tc.AddAndStartServer(t, stickyServerArgs[downedStoreIdx])
...
func TestInitRaftGroupOnRequest(t *testing.T) {
	...
	tc.StopServer(0)
	tc.StopServer(1)
	tc.AddAndStartServer(t, stickyServerArgs[0])
	tc.AddAndStartServer(t, stickyServerArgs[1])

	followerIdx := int(leaseHolder.StoreID)%numServers + numServers
	...
	kv.SendWrapped(context.Background(),
...
func TestDefaultConnectionDisruptionDoesNotInterfereWithSystemTraffic(t *testing.T) {
	...
	for i := range tc.Servers {
		tc.GetFirstStoreFromServer(t, i).SetReplicateQueueActive(true)
	}
