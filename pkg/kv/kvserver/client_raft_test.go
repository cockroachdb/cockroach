	...
	testutils.RunValues(t, "lease-type", roachpb.TestingAllLeaseTypes(),
		func(t *testing.T, leaseType roachpb.LeaseType) {
			...
			for _, s := range tc.Servers {
				client, err := kvserver.DialPerReplicaClient(s.NodeDialer().(*nodedialer.Dialer), ctx, s.NodeID())
				if err != nil {
					t.Fatal(err)
				}
...
func TestStoreWaitForReplicaInit(t *testing.T) {
	...
	client, err := kvserver.DialPerReplicaClient(tc.Servers[0].NodeDialer().(*nodedialer.Dialer), ctx, store.Ident.NodeID)
	if err != nil {
		t.Fatal(err)
	}
