...
func TestQuotaPool(t *testing.T) {
	...
	// Otherwise the quota gets prematurely released into the pool. We can use RaftAppliedIndex
	// instead of proposalQuotaBaseIndex because it's strictly ahead or the same.
	desc := leaderRepl.Desc()
	status := leaderRepl.RaftStatus()
	testutils.SucceedsSoon(t, func() error {
		var err error
		leaderRepl.WithRaftGroupProgress(func(id uint64, _ raft.ProgressType, progress tracker.Progress) {
			minIndex := status.Applied
			_, ok := desc.GetReplicaDescriptorByID(roachpb.ReplicaID(id))
			if !ok {
				err = errors.Errorf("Replica %d not found", id)
