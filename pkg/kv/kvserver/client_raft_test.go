...
func TestQuotaPool(t *testing.T) {
	...
	// Check that the follower will not be ignored by updateProposalQuotaRaftMuLocked
	// and the quota prematurely released into the pool. We can use RaftAppliedIndex
	// instead of ProposalQuotaBaseIndex because it's strictly ahead or the same.
	testutils.SucceedsSoon(t, func() error {
		var err error
		leaderRepl.WithProgress(func(state kvserverpb.ReplicaState, id uint64, _ raft.ProgressType, progress tracker.Progress) {
			minIndex := state.RaftAppliedIndex
			_, ok := state.Desc.GetReplicaDescriptorByID(roachpb.ReplicaID(id))
			if !ok {
