...
// maybeWakeUpReplicaMuLocked acquires r.store.quiescence and invokes
// maybeWakeUpReplicaMuLockedQuiescenceMuLocked.
func (r *Replica) maybeWakeUpReplicaMuLocked() {
	r.store.quiescence.Lock()
	defer r.store.quiescence.Unlock()
	r.maybeWakeUpReplicaMuLockedQuiescenceMuLocked()
}

// maybeWakeUpQuiescenceMuLocked acquires r.mu and invokes
// maybeWakeUpReplicaMuLockedQuiescenceMuLocked.
func (r *Replica) maybeWakeUpQuiescenceMuLocked() {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.maybeWakeUpReplicaMuLockedQuiescenceMuLocked()
}

// maybeWakeUpReplicaMuLockedQuiescenceMuLocked marks a follower as awake.
func (r *Replica) maybeWakeUpReplicaMuLockedQuiescenceMuLocked() {
	r.mu.AssertHeld()
	r.store.quiescence.AssertHeld()
	if !r.mu.asleep {
		return
	}
