...
// Tracker itself does not know anything about closed timestamps).
//
...
func (t *Tracker) Track(ctx context.Context, wts int64) Token {
	...
	// request, and continuously draining them and creating new buckets.
	...
	//  				|		 | 			|____|
	//        	|		 |      b2 t2: 20
	//  				|		 |                  <- req2 ts 15 joins b2, extending it downwards
	//  				|____|
	//  				b1 ts: 10
	//  													      <- req3 ts 5 joins b1, extending it downwards
	//
	// This is discussing the case where we have two buckets initialized (b1@ts=10
	// and b2@ts=20), and describes what bucket requests at different timestamps
	// join. Our goal is to maximize the Tracker's lower bound (i.e. its
	// conservative approximation about the lowest timestamp at which a request
	...
	_, t1 := createIfNotInitialized(b1, wts)
...
func (t *Tracker) Untrack(ctx context.Context, tok Token) {
	if tok.untracked {
		log.Fatalf(ctx, "untracking already-untracked request")
	}
	tok.untracked = true
	b := tok.b
...
//
// LowerBound cannot be called concurrently with other operations.
...
//
// b's timestamp is returned in either case. As the bucket's timestamp can be
// lowered at any time by other requests, this returned timestamp can only be
...
// un-initialize it.
func createIfNotInitialized(b *bucket, ts int64) (bool, int64) {
