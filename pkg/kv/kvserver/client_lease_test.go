	...
	testutils.RunValues(t, "lease-type", roachpb.EpochAndLeaderLeaseType(),
		func(t *testing.T, leaseType roachpb.LeaseType) {
			...
			if leaseType == roachpb.LeaseEpoch {
				...
				// Make sure that store liveness heartbeat has started.
				store, err := tc.Servers[0].GetStores().(*kvserver.Stores).
					GetStore(tc.Servers[0].GetFirstStoreID())
				require.NoError(t, err)
				s1Ident := slpb.StoreIdent{NodeID: roachpb.NodeID(1), StoreID: roachpb.StoreID(1)}

				testutils.SucceedsSoon(t, func() error {
					_, ts = store.TestingStoreLivenessSupportManager().SupportFrom(s1Ident)
					if ts.IsEmpty() {
						return errors.New("store liveness hasn't started yet")
					}
					return nil
				})
				...
				dropStoreLivenessHeartbeatsFrom(t, tc.Servers[0], desc, []roachpb.ReplicaID{1}, &partitioned)
			}
			...
			var curLease kvserverpb.LeaseStatus
			testutils.SucceedsSoon(t, func() error {
				curLease = repl.CurrentLeaseStatus(ctx)
				if curLease.Lease.Type() != leaseType {
					return errors.Errorf("lease %v not upgraded", curLease)
				}
				return nil
			})
			...
			// assertion fails.
			require.True(t, expLease.Expiration().LessEq(curLease.Expiration()))
		})
}
