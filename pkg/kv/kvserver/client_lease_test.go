	...
	testutils.RunValues(t, "lease-type", roachpb.EpochAndLeaderLeaseType(),
		func(t *testing.T, leaseType roachpb.LeaseType) {
			...
			if leaseType == roachpb.LeaseEpoch {
				...
				// Make sure that store liveness heartbeat has started. This ensures
				// that the supportFrom() is not 0.
				ts = tc.EnsureStoreLivenessHeartbeatsToSelfHaveStarted(t, 0 /* nodeIdx */)
