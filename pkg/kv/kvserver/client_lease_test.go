...
func TestTransferLeaseFailureDuringJointConfig(t *testing.T) {
	...
	// but all other replicas think n1 is the leaseholder. As long as n1 is
	// alive, it is the only one that can get the lease. It will retry and
	// be able to do that ,thanks to the fix in #83686.
	desc, err = tc.LookupRange(scratchStartKey)
	require.NoError(t, err)
