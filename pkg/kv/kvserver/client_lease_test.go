...
func TestLeaseholderLocalRelocate(t *testing.T) {
	...
	testutils.SucceedsSoon(t, func() error {
		...
		err = tc.Servers[2].DB().
			AdminRelocateRange(
				// TODO(shralex): update the order to 0, 1,
				// 3 and pass the new parameter following Aayush's PR merge.
				context.Background(), rhsDesc.StartKey.AsRawKey(), tc.Targets(3, 1, 0),
				nil)
		if err != nil {
			require.Regexp(t, "Removing leaseholder not allowed since it isn't the Raft leader", err)
			return err
		}
		leaseHolder, err = tc.FindRangeLeaseHolder(rhsDesc, nil)
		require.NoError(t, err)
		log.Infof(ctx, "Leaseholder: %s", leaseHolder)
	...
	// TODO(shralex): this is blocked on changing relocateReplicas to not transfer the lease to first
	// element in the list. Currently, this may cause redundant lease transfers.
	// https://github.com/cockroachdb/cockroach/pull/75676
	// Needs to be stress tested
	//repl := tc.GetFirstStoreFromServer(t, 3).LookupReplica(roachpb.RKey(rhsDesc.StartKey.
	//AsRawKey()))
...
func TestLeaseholderLocalRelocateNonPreferred(t *testing.T) {
	...
	// We start with having the range under test on (1,2,3). 1 and 2 are in the preferred region.
	// Manually move lease to server 3 (out of preference).
	tc.TransferRangeLeaseOrFatal(t, rhsDesc, tc.Target(2))

	// Check that the lease is on 3
	leaseHolder, err := tc.FindRangeLeaseHolder(rhsDesc, nil)
	require.NoError(t, err)
	require.Equal(t, tc.Target(2), leaseHolder)
	...
	testutils.SucceedsSoon(t, func() error {
		...
		err = tc.Servers[2].DB().
			AdminRelocateRange(
				context.Background(), rhsDesc.StartKey.AsRawKey(), tc.Targets(0, 1, 3),
				nil)
		if err != nil {
			require.Regexp(t, "Removing leaseholder not allowed since it isn't the Raft leader", err)
			return err
	...
	// Make sure lease moved to 0 or 1 (they are in a preferred location), not 4
	require.True(t,
	...
	// Needs to be stress tested
	// https://github.com/cockroachdb/cockroach/pull/75676
	//repl := tc.GetFirstStoreFromServer(t, 0).LookupReplica(roachpb.RKey(rhsDesc.StartKey. AsRawKey()))
	// history := repl.GetLeaseHistory()
