...
// RangeAppliedState describes the applied state of a range in the state engine,
...
// AppliedStateLoader loads the applied state for a range from the state engine.
// It is called during WAG replay to determine whether each WAG node needs to be
// applied.
//
// TODO(mira): This is a callback type rather than a direct dependency on
// kvstorage.StateLoader to avoid a circular import: kvstorage imports wag (to
// call Replay), so wag cannot import kvstorage. The caller wires this up to the
// StateLoader.
...
func checkAddrInvariants(event wagpb.Addr, state RangeAppliedState) {
	...
	if event.ReplicaID == 0 {
		panic(errors.AssertionFailedf(
			"WAG event for r%d has zero ReplicaID", event.RangeID,
		))
	}
	// Assertion 2.
...
// the event against the state engine's current position for this range. See
...
// Assertion 2 (see checkAddrInvariants) guarantees state.Tombstone <=
// state.ReplicaID when a replica exists, giving four regions:
//
//	[0, state.Tombstone)             → destroyed; skip
//	[state.Tombstone, state.Replica) → stale; skip
...
func raftCatchUp(event wagpb.Addr, eventType wagpb.EventType) kvpb.RaftIndex {
	switch eventType {
...
// to the state engine. It checks each event in the node and returns the replay
...
func NodeShouldApply(
	...
	for i, event := range node.Events {
		...
		if i == 0 {
			...
			panic(errors.AssertionFailedf(
				"WAG node events disagree on whether to apply: "+
					"event[0]=%s (apply=%t), event[%d]=%s (apply=%t)",
				node.Events[0].Addr, result.Apply, i, event.Addr, apply,
			))
		}
		if catchUp := raftCatchUp(event.Addr, event.Type); catchUp > 0 && apply {
...
// nodes to the state engine. It is called during store startup, before the
// store goes online.
func Replay(
	ctx context.Context,
...
// applyMutation applies a WAG node's mutation to the state engine. It handles
...
func applyMutation(ctx context.Context, stateEngine storage.Engine, m wagpb.Mutation) error {
	...
	if m.Ingestion != nil {
		...
		// IngestAndExciseFiles call on the state engine.
