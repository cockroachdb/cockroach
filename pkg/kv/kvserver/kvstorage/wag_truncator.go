...
type WAGTruncator struct {
	...
	// initialized. We only expect to see gaps in WAG indices below this index
	// due to server crashes. However, after startup, we do not skip gaps when
...
func (t *WAGTruncator) DurabilityAdvancedCallback() {
	// NB: logically, lastTruncatedWAGIndex > seq isn't possible because the
