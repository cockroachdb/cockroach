...
var wagSuffixRetentionCount = settings.RegisterIntSetting(
	settings.SystemOnly,
	"kv.wag.truncator_suffix_retention.count",
...
func (t *WAGTruncator) maybeAdvanceAllowedIndex() {
	...
	// If retain is larger than the number of WAG nodes, we set the allowed index
	// to truncIndex which prevents us from deleting any nodes.
	var suffixBound uint64
	if seqIdx > retain {
		suffixBound = seqIdx - retain
	}
	newAllowed := max(t.truncIndex.Load(), suffixBound)
	// The allowedIndex could get advanced by the age-timer and we want to make
	// sure that we do not regress it.
	t.allowedIndex = max(t.allowedIndex, newAllowed)
}
