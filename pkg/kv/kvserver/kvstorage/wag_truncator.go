...
func (t *WAGTruncator) clearReplicaRaftLogAndSideloaded(
	...
	// We want to delete all raft log entries <= lastIndex. Since Raft log
	...
	if lo, err := firstRaftLogIndex(ctx, raft.RO, prefixBuf, hi); err != nil {
		return errors.Wrapf(err, "finding first raft log index for r%d", rangeID)
	} else if err := logstore.ClearRangeSizeKnown(
		raft.WO, prefixBuf,
		lo-1, lastIndex, // if not found, lo-1 == lastIndex which is a no-op
...
// that exists, or hi if no such entry was found.
