...
func DestroyReplica(
	...
	return destroyReplicaImpl(ctx, reader, writer, info, next, false /* sortedKeys */)
}
...
//
// The sortedKeys flag, if true, forces the writes to be generated in the sorted
// order of keys. This is to support feeding the writes from this function to
// SSTables, in the snapshot ingestion path.
//
...
func SubsumeReplica(
	...
	}, destroyReplicaImpl(ctx, reader, writer, info, MergedTombstoneReplicaID, sortedKeys)
}
