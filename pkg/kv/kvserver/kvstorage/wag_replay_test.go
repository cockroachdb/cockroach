...
func simpleNewBatch(stateRW StateRW) NewReplayBatchFn {
	return func(ctx context.Context, rangeID roachpb.RangeID) (ReplayBatch, error) {
		sl := MakeStateLoader(rangeID)
...
func TestReplayWAG(t *testing.T) {
	...
	t.Run("empty WAG", func(t *testing.T) {
		...
		noReplay := func(context.Context, roachpb.RangeID) (ReplayBatch, error) {
	...
	t.Run("unapplied nodes are applied", func(t *testing.T) {
		...
		// Establish the replica in the state engine as initialized at index 9.
		...
		writeRaftLogEntries(t, eng.LogEngine(), 1, 10, 20)

		// WAG node at index 15: catch-up replays entries 10-15.
		...
		// Second WAG node at index 20: catch-up replays entries 16-20.
		// The first node should be skipped on this replay.
		writeWAGNode(t, &bf, wagpb.Addr{RangeID: 1, ReplicaID: 1, Index: 20}, wagpb.EventApply, "key2", "val2")
