...
func WriteInitialRangeState(
	...
	// Stage WAG events recording the replica's creation and initialization.
	// We stage these events separately, as both of them correspond to the same
	// range, and a WAG node is only allowed to have a single Event per-range.
	id := roachpb.FullReplicaID{RangeID: desc.RangeID, ReplicaID: replicaID}
	if err := w.FlushNodeWithoutMutation(
		raftWO, wagpb.MakeAddr(id, 0), wagpb.EventCreate,
	); err != nil {
		return err
	}
