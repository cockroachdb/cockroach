...
func checkCanInitializeEngine(ctx context.Context, eng Engines) error {
	...
	if err := IterEmptyEngine(ctx, eng.LogEngine(), check); err != nil {
		return err
	}
	if eng.Separated() {
		return IterEmptyEngine(ctx, eng.StateEngine(), check)
	}
	return nil
}

// IterEmptyEngine iterates the Engine that is expected to be empty. Some
