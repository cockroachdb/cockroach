...
func TestMaybeAdvanceAllowedIndex(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{retain: 0, seq: 10, wantAllowedIndex: 10},
		{retain: 3, seq: 10, wantAllowedIndex: 7},
		{retain: 10, seq: 10, wantAllowedIndex: 0},
		// allowedIndex gets set to at least truncIndex.
		{retain: 10, truncIndex: 5, seq: 10, wantAllowedIndex: 5},
		{retain: 100, seq: 10, wantAllowedIndex: 0},
		// allowedIndex doesn't regress.
		{retain: 100, allowedIndex: 9, seq: 10, wantAllowedIndex: 9},
	} {
		t.Run("", func(t *testing.T) {
			st := cluster.MakeTestingClusterSettings()
