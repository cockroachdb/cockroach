...
func TestFirstRaftLogIndex(t *testing.T) {
	...
	tests := []struct {
		...
		{entries: nil, hi: 100, wantIdx: 100},
		{entries: []kvpb.RaftIndex{}, hi: 0, wantIdx: 0},
		{entries: []kvpb.RaftIndex{}, hi: 100, wantIdx: 100},
		{entries: []kvpb.RaftIndex{15}, hi: 0, wantIdx: 0},
		{entries: []kvpb.RaftIndex{15}, hi: 14, wantIdx: 14},
		{entries: []kvpb.RaftIndex{15}, hi: 15, wantIdx: 15},
		{entries: []kvpb.RaftIndex{15}, hi: 16, wantIdx: 15},
		{entries: []kvpb.RaftIndex{15, 16}, hi: 15, wantIdx: 15},
		{entries: []kvpb.RaftIndex{15, 16}, hi: 100, wantIdx: 15},
	}
	for _, tc := range tests {
		t.Run("", func(t *testing.T) {
			...
			idx, err := firstRaftLogIndex(ctx, e.LogEngine(), prefixBuf, tc.hi)
			require.NoError(t, err)
