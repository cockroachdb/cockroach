...
// the given replica. Each node contains an EventInit event with its raft index
// set to the WAG index as a simplification. Advances the WAG sequence number
// accordingly.
func (e *testEngines) writeWAGNodesAt(t *testing.T, wagIndices []uint64, r roachpb.FullReplicaID) {
	t.Helper()
	for _, idx := range wagIndices {
		event := wagpb.Event{
			Addr: wagpb.MakeAddr(r, kvpb.RaftIndex(idx)),
			Type: wagpb.EventInit,
		}
		require.NoError(t, wag.Write(
			e.LogEngine(), idx, wagpb.Node{Events: []wagpb.Event{event}},
		))
	}
...
func TestTruncateAppliedNodes(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			wagIndices: []uint64{}, initIndex: 0,
			wantRemaining: nil, wantTruncIndex: 0,
		},
		{
			wagIndices: []uint64{1, 2, 3, 4}, initIndex: 0,
			wantRemaining: nil, wantTruncIndex: 4,
		},
		{
			wagIndices: []uint64{1, 2, 3, 4}, initIndex: 2,
			wantRemaining: nil, wantTruncIndex: 4,
		},
		{
			wagIndices: []uint64{1, 2, 3, 4}, initIndex: 4,
			wantRemaining: nil, wantTruncIndex: 4,
		},
		{
			wagIndices: []uint64{2, 5, 6, 7, 10}, initIndex: 0,
			wantRemaining: []uint64{2, 5, 6, 7, 10}, wantTruncIndex: 0,
		},
		{
			wagIndices: []uint64{2, 5, 6, 7, 10}, initIndex: 2,
			wantRemaining: []uint64{5, 6, 7, 10}, wantTruncIndex: 2,
		},
		{
			wagIndices: []uint64{2, 5, 6, 7, 10}, initIndex: 5,
			wantRemaining: []uint64{10}, wantTruncIndex: 7,
		},
		{
			wagIndices: []uint64{2, 5, 6, 7, 10}, initIndex: 7,
			wantRemaining: []uint64{10}, wantTruncIndex: 7,
		},
		{
			wagIndices: []uint64{2, 5, 6, 7, 10}, initIndex: 10,
			wantRemaining: nil, wantTruncIndex: 10,
		},
	} {
		t.Run("", func(t *testing.T) {
			e := makeTestEngines()
