...
// truncateWAGNodes repeatedly calls truncateAppliedNode until no more nodes can
// be deleted.
func (e *testEngines) truncateWAGNodes(t *testing.T) {
	...
	for {
		...
		ok, err := truncateAppliedNode(context.Background(), WrapRaft(e.LogEngine()), stateReader)
