...
//
// exists indicates whether the replica already exists in the state machine
// (i.e. mark.Is(r) matched in canApply).
func assertValidTransition(event wagpb.Event, state persistedRangeState, exists bool) {
	if !exists {
		// Nonexistent: only EventCreate is valid.
		assert(event.Type == wagpb.EventCreate, "first event for new replica is not EventCreate", event, state)
	} else if state.appliedIndex == 0 {
		// Uninitialized: only EventInit and EventDestroy (at index 0) are valid.
		assert(
			event.Type == wagpb.EventInit ||
				(event.Type == wagpb.EventDestroy && event.Addr.Index == 0),
			"invalid forward transition from uninitialized replica", event, state,
		)
	} else {
		// Initialized: EventInit is not a valid transition.
		assert(event.Type != wagpb.EventInit, "EventInit on initialized replica", event, state)
		assert(event.Addr.Index >= state.appliedIndex, "event index below applied index", event, state)
	}
}
