...
// bulkIOWriteLimit is defined here because it is used by BulkIOWriteRate Limiter.
...
// bulkReadIOLimit is defined here because it is used by BulkReadIORate TokenBucket.
var bulkReadIOLimit = settings.RegisterByteSizeSetting(
	settings.SystemOnly,
...
// TTL Scans) that can execute concurrently for a store.
...
func NewStore(
	...
	rateLimit := bulkReadIOLimit.Get(&cfg.Settings.SV)
	s.limiters.BulkReadIORate.Init(tokenbucket.TokensPerSecond(rateLimit), tokenbucket.Tokens(rateLimit))
	bulkReadIOLimit.SetOnChange(&cfg.Settings.SV, func(ctx context.Context) {
		rateLimit := bulkReadIOLimit.Get(&cfg.Settings.SV)
		s.limiters.BulkReadIORate.Lock()
		defer s.limiters.BulkReadIORate.Unlock()
		s.limiters.BulkReadIORate.UpdateConfig(tokenbucket.TokensPerSecond(rateLimit), tokenbucket.Tokens(rateLimit))
		s.limiters.BulkReadIORate.Reset()
	})
	s.limiters.ConcurrentBulkReadRequests = limit.MakeConcurrentRequestLimiter(
		"bulkReadRequestLimiter", int(bulkReadRequestsLimit.Get(&cfg.Settings.SV)),
	)
	bulkReadRequestsLimit.SetOnChange(&cfg.Settings.SV, func(ctx context.Context) {
		s.limiters.ConcurrentBulkReadRequests.SetLimit(int(bulkReadRequestsLimit.Get(&cfg.Settings.SV)))
	})
