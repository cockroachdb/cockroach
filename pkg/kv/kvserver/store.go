...
func (s *Store) SetDraining(drain bool, reporter func(int, string)) {
	...
	ctx, cancelFn := contextutil.WithCancelReason(baseCtx)
	defer cancelFn(errors.New("drain finished"))
	s.stopper.RunAsyncTask(
		ctx, "drain monitor", func(ctx context.Context) {
			select {
			case <-s.stopper.ShouldStop():
				cancelFn(errors.New("interrupted by stopper"))
			case <-ctx.Done():
				// This is when the SetDrain function completes on its own and
				// calls cancelFn via the defer above.
			}
		})
