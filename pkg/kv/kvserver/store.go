...
usually corresponding to a physical device. It also houses a collection of
subsystems that, in broad terms, perform maintenance of each Replica on the
...
INVARIANT: the set of Ranges (as determined by, e.g. a transactionally
consistent scan of the meta index ranges) always exactly covers the addressable keyspace
roachpb.KeyMin (inclusive) to roachpb.KeyMax (exclusive).

Ranges

Each Replica is part of a Range, i.e. corresponds to what other systems would
call a shard. A Range is a consensus group backed by Raft, i.e. each Replica
is a state machine backed by a replicated log of commands. In CockroachDB,
Ranges own a contiguous chunk of addressable keyspace, where the word
"addressable" is a fine-print that interested readers can learn about in
keys.Addr; in short the Replica data is logically contiguous, but not
contiguous when viewed as Engine kv pairs.

Considerable complexity is incurred by the features of dynamic re-sharding
and relocation, i.e. range splits, range merges, and range relocation (also
called replication changes, or, in the case of lateral movement,
rebalancing). Each of these interact heavily with the Range a consensus group
(of which each Replica is a member). All of these intricacies are described
at a high level in this comment.
...
To understand this invariant, it's best to try to make do without it and fail.
Assume that a descriptor change committed, but that the intent will only get
resolved asynchronously.

- since the transaction committed, its update to the meta ranges becomes
visible and the cluster will start routing requests according to the new
descriptor. However, the Replicas may yet have to learn that the new
descriptor is committed. This is not a correctness problem, but it is
undesirable. The invariant avoids this, since the first intent to be resolved
will be the RangeDescriptor on the leaseholder (during EndTxn application),
and this will causally precede resolution of auxiliary intents since those
will only be resolved after the leaseholder has applied the EndTxn.

- each Replica (and its surrounding Store) needs to run a considerable amount
of logic to update its state when switching to the new descriptor, and now
this logic has to be triggered by the intent resolution. This is a lot less
convenient than using instead the roachpb.EndTxnRequest as a direct trigger
due to the need to somehow pass the data through the intent resolution
process.

Thanks to the invariant, a Replica can switch to the new RangeDescriptor when
it applies the corresponding EndTxnRequest.

TODO(tbg): is that it? I thought there was more. Seems that even the raft
config change stuff doesn't "really" get more involved if we didn't have
the invariant.

TODO: write something somewhere and link raft way of doing things and this: -
https://github.com/etcd-io/etcd/issues/7625#issuecomment-489232411 Also make
sure the comment clearly has the invariants we need for apply-time activation.

INVARIANT: A Store never contains two Replicas from the same Range, nor do
the key ranges for any two of its Replicas overlap. (Note that there
is no requirement that these Replicas come from a consistent set of Ranges).
...
directly from [a-z) to [a,c)+[c,z). For a similar example, consider accepting
a new Replica [b,d) when a Replica [a,c) is already present, which is similarly
not allowed. To understand how such situations could arise, consider that
a series of changes to the RangeDescriptor is observed asynchronously by
followers. This means that a follower may have Replica [a-z) while any number
of splits and replication changes are already known to the leaseholder, and
the new Ranges created in the process may be attempting to add a Replica to
the slow follower.

With the invariant, we effectively allow looking up a unique (if it exists)
Replica for any given key, and ensure that no two Replicas on a Store operate
on shared keyspace (as seen by the storage.Engine). Refer to the Replica
Lifecycle diagram below for details on how this invariant is upheld.
...
Range). The Store's RaftTransport receives Raft messages from Replicas
residing on other Stores and routes them to the appropriate Replicas via
...
RaftTransport.SendAsync method. This is relatively straightforward unless
when Ranges are being reconfigured, which is where understanding the Replica
Lifecycle becomes important and upholding the Store's invariants becomes more
complex.

A first phenomenon to understand is that of uninitialized Replicas. Such a
Replica corresponds to a State Machine that yet has to apply any entries,
and in particular has no notion of an active RangeDescriptor yet. This state
...
on the RangeDescriptors. The fact that uninitialized Replicas can be removed has
to be taken into account by splits as well; the split trigger may find that the
right-hand side uninitialized Replica has already been removed, in which case
the right half of the split has to be discarded (see acquireSplitLock and
splitPostApply).
