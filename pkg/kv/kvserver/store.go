...
// SeparatedEngine is a stepping stone for separating the raft log. It allows
// callers of NewStore to pass in "two engines" masquerading as one. This is not
// the intended end state, where it will most likely make sense to pass in two
// engines. But for prototyping an experimentation, this approach avoids large
// amounts of refactoring.
...
func NewStore(
	...
	logEng := eng
	if se, ok := logEng.(SeparatedEngine); ok {
		logEng = se.LogEngine()
	}
	s := &Store{
