...
var SnapshotSendLimit = settings.RegisterIntSetting(
	...
	"number of concurrent snapshot sent request can be generated by store",
	2,
	settings.PositiveInt,
)
...
func NewStore(
	...
	s.snapshotSendQueue = multiqueue.NewMultiQueue(uint(SnapshotSendLimit.Get(&cfg.Settings.SV)))
	SnapshotSendLimit.SetOnChange(&cfg.Settings.SV, func(ctx context.Context) {
		s.snapshotSendQueue.UpdateConcurrencyLimit(uint(SnapshotSendLimit.Get(&cfg.Settings.SV)))
	})
