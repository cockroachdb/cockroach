			...
			if err := s.stopper.RunAsyncTaskEx(
				...
				func(ctx context.Context) {
					...
					if transferStatus != transferOK {
						format := "failed to transfer lease %s for range %s when draining: %s"
						var outcome string
						if err != nil {
							outcome = err.Error()
						} else {
							outcome = transferStatus.String()
						}
						if verbose {
							log.Dev.Infof(ctx, format, drainingLeaseStatus.Lease, desc, outcome)
						} else {
							log.VErrEventf(ctx, 1, format, drainingLeaseStatus.Lease, desc, outcome)
						}
					}
				}); err != nil {
				if verbose || log.V(1) {
