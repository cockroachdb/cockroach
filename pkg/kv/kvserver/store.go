...
func (s *Store) ComputeMetricsPeriodically(
	...
	if prevMetrics != nil {
		...
		// between the two points in time. However, since the prometheus.Histogram
		// does not expose any of the data it collects the current metrics need to
		// be written to a Metric.
		prevFsync := prevMetrics.WALFsyncLatency
		windowFysncLatency := &prometheusgo.Metric{}
		err := m.LogWriter.FsyncLatency.Write(windowFysncLatency)
		if err != nil {
			return m, err
		}
		windowFysncLatency = subtractPrometheusMetrics(windowFysncLatency, prevFsync)

		s.metrics.FsyncLatency.Update(m.LogWriter.FsyncLatency, windowFysncLatency.Histogram)
	}
