...
// visited in key order. store.mu is held during the visiting. Placeholders are
// not visited.
//
...
func (s *Store) visitReplicasByKey(
	...
	visitor func(context.Context, *Replica) error, // can return iterutil.IterationDone
) error {
	s.mu.RLock()
	defer s.mu.RUnlock()
	return s.mu.replicasByKey.VisitKeyRange(ctx, startKey, endKey, order, func(ctx context.Context, it item) error {
		if it.repl != nil {
...
// getOverlappingKeyRangeLocked returns an item from Store.mu.replicasByKey
// overlapping the given descriptor (or nil if no such keyRange exists).
func (s *Store) getOverlappingKeyRangeLocked(rngDesc *roachpb.RangeDescriptor) item {
	var it item
	if err := s.mu.replicasByKey.VisitKeyRange(
		context.Background(), rngDesc.StartKey, rngDesc.EndKey, AscendingKeyOrder,
		func(ctx context.Context, iit item) error {
			it = iit
			return iterutil.StopIteration()
		}); err != nil {
