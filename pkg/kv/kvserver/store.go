...
func (s *Store) startRangefeedUpdater(ctx context.Context) {
	_ /* err */ = s.stopper.RunAsyncTaskEx(ctx, stop.TaskOpts{
		...
		for {
			...
			// Otherwise, we either successfully completed the run, or were
			// interrupted by a config change. Loop around, and start a new run.
		}
	})
}
