...
func (s *Store) startRangefeedUpdater(ctx context.Context) {
	_ /* err */ = s.stopper.RunAsyncTaskEx(ctx, stop.TaskOpts{
		...
		//
		// The refreshInterval defines the target interval between delivering closed
		// timestamp updates for a single range. This is the target time in which a
		// single pass should complete. The smearInterval defines the target
		// interval between wake-ups of this loop.
		//
		...
		// - We smear across replicas at the granularity of smearInterval (e.g. 1ms)
		//   to avoid putting this goroutine to sleep for excessively short periods
		//   of time.
		// - We prioritize staying within the target refresh interval. So if there
		//   are delays in processing, the work goes with no sleeps.
		for {
			// Configuration may have changed between runs, load it unconditionally.
			refreshInterval, smearInterval, err := conf.wait(ctx)
			if err != nil { // context canceled
				return
			}

			work := updateRangeIDs()
			sched := newRangeFeedUpdaterSched(timeutil.Now(), refreshInterval, smearInterval, len(work))
			for confChanged := false; !confChanged; {
				// The last iteration may return 0 items to do, but it's fine - we need
				// to wait until targetTime before looping around.
				...
				if now.Before(targetTime) {
					...
					case t := <-timer.C:
						timer.Read = true
						now = t
					case <-conf.changed: // be responsive to config changes
						confChanged = true
					case <-ctx.Done():
						return
					}
				...
				sched.tick(now)
				if len(work) == 0 {
					// NB: we have waited until at least refreshInterval has passed, so
					// it's safe to loop around.
					break
				}
			}
		}
