...
func (s *Store) Capacity(ctx context.Context, useCached bool) (roachpb.StoreCapacity, error) {
	...
	newStoreReplicaVisitor(s).Visit(func(r *Replica) bool {
		...
		totalQueriesPerSecond += stats.QueriesPerSecond
		totalWritesPerSecond += stats.WriteKeysPerSecond
		writesPerReplica = append(writesPerReplica, stats.WriteKeysPerSecond)
		cr := candidateReplica{
			Replica: r,
