...
func (s *Store) PurgeOutdatedReplicas(ctx context.Context, version roachpb.Version) error {
	...
	s.VisitReplicas(func(repl *Replica) (wantMore bool) {
		...
		if err != nil {
			g.GoCtx("acquire quota to purge outdated replicas", func(ctx context.Context) error {
				return err
			})
			return false
		}

		g.GoCtx("purge outdated replicas from GCQueue", func(ctx context.Context) error {
			defer alloc.Release()
