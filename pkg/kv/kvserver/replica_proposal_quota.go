...
func (r *Replica) updateProposalQuotaRaftMuLocked(
	...
	if r.mu.leaderID != lastLeaderID {
		if r.replicaID == r.mu.leaderID {
			...
			r.mu.proposalQuotaBaseIndex = roachpb.RaftIndex(status.Applied)
	...
	// caught up.
	commitIndex := roachpb.RaftIndex(status.Commit)
	...
	minIndex := roachpb.RaftIndex(status.Applied)
	...
	r.mu.internalRaftGroup.WithProgress(func(id uint64, _ raft.ProgressType, progress tracker.Progress) {
		...
		if roachpb.RaftIndex(progress.Match) < r.mu.proposalQuotaBaseIndex {
			return
		}
		...
		if progress.Match > 0 && roachpb.RaftIndex(progress.Match) < minIndex {
			minIndex = roachpb.RaftIndex(progress.Match)
		}
		...
		if rep.ReplicaID == r.mu.lastReplicaAdded && roachpb.RaftIndex(progress.Match) >= commitIndex {
	...
	// index.
	releasableIndex := r.mu.proposalQuotaBaseIndex + roachpb.RaftIndex(len(r.mu.quotaReleaseQueue))
	if releasableIndex != roachpb.RaftIndex(status.Applied) {
