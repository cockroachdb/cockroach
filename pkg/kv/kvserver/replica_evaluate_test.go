...
func writeABCDEFWith(t *testing.T, eng storage.Engine, ts hlc.Timestamp, txn *roachpb.Transaction) {
	for _, k := range []string{"a", "b", "c", "d", "e", "f"} {
		require.NoError(t, storage.MVCCPut(context.Background(), eng, nil, roachpb.Key(k), ts, hlc.ClockTimestamp{}, roachpb.MakeValueFromString("value-"+k), storage.NoLogicalReplication, txn))
	}
}
