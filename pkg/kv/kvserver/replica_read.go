...
func (r *Replica) canDropLatchesBeforeEval(
	...
	targetBytesPerLockConflict := storage.TargetBytesPerLockConflictError.Get(&r.store.cfg.Settings.SV)
	...
	for _, req := range ba.Requests {
		...
		needsIntentInterleavingForThisRequest, err := storage.ScanConflictingIntentsForDroppingLatchesEarly(
			ctx, rw, txnID, ba.Header.Timestamp, start, end, &intents, maxLockConflicts, targetBytesPerLockConflict,
		)
		if err != nil {
