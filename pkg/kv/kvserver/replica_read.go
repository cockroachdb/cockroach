...
func (r *Replica) executeReadOnlyBatchWithServersideRefreshes(
	...
	// locally. Local requests (typically DistSQL, though we may not have
	// instrumented the source as SQL in all cases, so some may be flowing in as
	// OTHER), do memory accounting before issuing the request. Even though the
	// accounting for the first request in fetcher is small, subsequent ones use
	// the size of the response for subsequent requests (see
	// https://github.com/cockroachdb/cockroach/pull/52496). So in the interest
	// of not overcounting, we bypass such accounting for local requests. This
	// scheme could be tightened.
	...
	if rootMonitor != nil {
		...
		monitor.Start(ctx, rootMonitor, mon.BoundAccount{})
