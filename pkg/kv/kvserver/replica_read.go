...
func (r *Replica) executeReadOnlyBatch(
	...
	if g != nil {
		...
		_ = r.updateTimestampCacheAndDropLatches(ctx, g, ba, br, pErr, st)
	}
...
func (r *Replica) canDropLatchesBeforeEval(
	...
	disabled := r.store.cfg.TestingKnobs.DisableDroppingLatchesBeforeEval ||
		!allowDroppingLatchesBeforeEval.Get(&r.store.cfg.Settings.SV)
	if disabled || !canReadOnlyRequestDropLatchesBeforeEval(ba, g) {
	...
	for _, req := range ba.Requests {
		...
		cumIntentBytes, needsIntentInterleavingForThisRequest, err = scanConflictingIntents(
			ctx, rw, ba.Txn, ba.Header.Timestamp, start, end, &intents, maxIntents, cumIntentBytes, ba.TargetBytes,
		)
		if err != nil {
		...
		if ba.TargetBytes != 0 && cumIntentBytes >= ba.TargetBytes {
			break
		}
	}
	if len(intents) > 0 {
