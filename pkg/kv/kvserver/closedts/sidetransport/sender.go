...
type Sender struct {
	...
	// avgMaxNetWorkLatency tracks the average maximum network latency between the
	// sender and any of its follower nodes. It uses an exponentially weighted
	// moving average with an effective window size of 30, meaning recent data
	// points influence the average more while the older data gradually fade.
	// Thread safe.
	//
