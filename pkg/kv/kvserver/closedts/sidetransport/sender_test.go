...
func newMockSideTransportDRPCServerWithOpts(
	...
	drpcImpl := (*drpcMockReceiver)(receiver)
	...
	stopper.OnQuiesce(func() { _ = lis.Close() })
	go func() {
		_ = srv.Serve(ctx, drpcL)
	}()

	return server, nil
}

type sideTransportDRPCServerForReceiver struct {
	lis net.Listener
	srv *drpcserver.Server
}

func (s *sideTransportDRPCServerForReceiver) Close() {
	_ = s.lis.Close()
}

func (s *sideTransportDRPCServerForReceiver) addr() net.Addr {
	return s.lis.Addr()
}

func newMockSideTransportDRPCServerForReceiver(
	ctx context.Context, stopper *stop.Stopper, receiver *Receiver,
) (*sideTransportDRPCServerForReceiver, error) {
	lis, err := net.Listen("tcp", "localhost:")
	if err != nil {
		return nil, err
	}
	...
	if err := ctpb.DRPCRegisterSideTransport(mux, receiver.AsDRPCServer()); err != nil {
		return nil, err
	}

	server := &sideTransportDRPCServerForReceiver{
		lis: lis,
		srv: srv,
	}
	stopper.OnQuiesce(func() { _ = lis.Close() })
...
// Test that the stopper quiescence interrupts a gRPC stream.Send.
func TestRPCConnUnblocksOnStopper(t *testing.T) {
	...
	s, stopper := newMockSender(newRPCConnFactory(
		newMockSideTransportClientFactory(dialer, false /* useDRPC */),
...
func TestSenderReceiverIntegration(t *testing.T) {
	...
	testutils.RunTrueAndFalse(t, "useDRPC", func(t *testing.T, useDRPC bool) {
		...
		for i := 0; i < numNodes; i++ {
			...
			if useDRPC {
				srv, err := newMockSideTransportDRPCServerForReceiver(
