...
func TestSenderReceiverIntegration(t *testing.T) {
	...
	<-receivers[1].testingKnobs[1].onRecvErr
	// Check that the other Receiver is still receiving updates.
	<-receivers[2].testingKnobs[1].onMsg

	// We need to consume the error on the receiver that's still open, to allow it
	// to close. Note that we don't consume the error from receivers[0] (i.e. Node
	// 1) and yet that Receiver doesn't leak goroutine, which proves that nobody
	// connected to it (a node doesn't connect to itself).
	go func() {
		<-receivers[2].testingKnobs[1].onRecvErr
	}()
}
