...
func (m *mockReplica) BumpSideTransportClosed(
	...
	_ [roachpb.MAX_CLOSED_TIMESTAMP_POLICY]hlc.Timestamp,
...
func expGroupUpdates(s *Sender, now hlc.ClockTimestamp) []ctpb.Update_GroupUpdate {
	...
	return []ctpb.Update_GroupUpdate{
		{roachpb.LAG_BY_CLUSTER_SETTING, targetForPolicy(roachpb.LAG_BY_CLUSTER_SETTING)},
		{roachpb.LEAD_FOR_GLOBAL_READS, targetForPolicy(roachpb.LEAD_FOR_GLOBAL_READS)},
	}
}
...
func TestSenderBasic(t *testing.T) {
	...
	require.Equal(t, map[roachpb.RangeID]trackedRange{
		15: trackedRange{lai: 5, policy: roachpb.LAG_BY_CLUSTER_SETTING},
	}, s.trackedMu.tracked)
