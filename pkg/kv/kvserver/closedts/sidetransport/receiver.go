...
func (r *incomingStream) handleRemovals(ctx context.Context, removed []roachpb.RangeID) {
	...
	// We're going to remove in two phases, because of locking concerns.
	// Phase 1: Take the *read* lock, go through the removed ranges and call into
	// each Replica, telling it to hold on locally to the the info we're about to
	...
	// (=> deadlock).
	// Phase 2: Take the write lock and remove the state.
