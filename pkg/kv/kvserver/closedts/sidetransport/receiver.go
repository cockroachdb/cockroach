...
func NewReceiver(nodeID *base.NodeIDContainer, stop *stop.Stopper, stores Stores) *Receiver {
	r := &Receiver{
		stop:   stop,
		stores: stores,
	}
...
func newClosedTimestampStreamReceiver(s *Receiver, stores Stores) *incomingStream {
	r := &incomingStream{
		server: s,
		stores: stores,
	}
...
func (r *incomingStream) processUpdate(ctx context.Context, msg *ctpb.Update) {
	...
	// Handle the removed tracked. In order to not lose closed ts info, before we
