...
func TestTargetForPolicy(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			sideTransportCloseInterval: millis(200),
			rangePolicy:                roachpb.LEAD_FOR_GLOBAL_READS,
			observedMaxNetworkRTT:      millis(700),
			leadTargetAutoTune:         true,
			expClosedTSTarget: now.
				Add((maxClockOffset +
					millis(1070) /* raftTransportPropTime */ +
					millis(25) /* bufferTime */).Nanoseconds(), 0),
		},
		{
			name:                       "kv.closed_timestamp.lead_for_global_reads_auto_tune with side transport",
			sideTransportCloseInterval: millis(500),
			rangePolicy:                roachpb.LEAD_FOR_GLOBAL_READS,
			observedMaxNetworkRTT:      millis(200),
			leadTargetAutoTune:         true,
			expClosedTSTarget: now.
				Add((maxClockOffset +
					millis(600) /* sideTransportPropTime */ +
					millis(25) /* bufferTime */).Nanoseconds(), 0),
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			target := TargetForPolicy(
				...
				tc.sideTransportCloseInterval, /*observedSideTransportLatency*/
				tc.observedMaxNetworkRTT,      /*sideTransportCloseInterval*/
