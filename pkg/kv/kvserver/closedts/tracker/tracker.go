...
//
// Track and LowerBound can be called concurrently. Untrack cannot be called
...
type Tracker interface {
	...
	// down.
	//
	// LowerBound can be called concurrently with Track (and it may or may not see
	// the racing Track), but not with Untrack.
	//
	// Synthetic timestamps: The Tracker doesn't necessarily track synthetic /
	// physical timestamps precisely; the only gurantee implemententations need to
	...
	LowerBound(_ context.Context) hlc.Timestamp
}
