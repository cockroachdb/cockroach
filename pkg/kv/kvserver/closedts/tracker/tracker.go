...
// An accurate implementation would hold all tracked timestamps in a min-heap,
// but that costs memory and can't be implemented in a lock-free manner (at
// least not by this author). This Tracker generally doesn't know exactly what
// the lowest tracked timestamp is; it just knows a lower-bound on it.
// Concretely, the tracker doesn't maintain information on all tracked
...
//
// While `ts` is tracked, LowerBound() will return values less or equal to `ts`.
//
// Track can be called concurrently with other Track calls.
func (t *Tracker) Track(ctx context.Context, ts hlc.Timestamp) Token {
	...
	// timestamp will join the bucket before the read lock is released). This
	// assures that we're not leaking buckets. The timestamp creating the bucket
	// doesn't necessarily join the bucket: a lower timestamp might
	// race to join the newly-created bucket, lowering its timestamp in the
	// process, and the bucket-creating timestamp might end up creating or otherwise
	// joining the second bucket.
	...
	createIfNotInitialized(ctx, b2, wts)
...
// Untrack removes a tiemstamp from the tracked set - the timestamp that created
...
// LowerBound returns a conservative estimate of the lower bound of the tracked set of timestamps
// If the tracked set is currently empty, an empty timestamp is returned.
//
// LowerBound called concurrently with Track, but not with Untrack.
func (t *Tracker) LowerBound() hlc.Timestamp {
...
// join adds a timestmap to the bucket and returns a token, to be used for
// removing the timestamp from the bucket (through Tracker.Untrack()).
...
// ts is <= ts, and then adds a timestamp to the bucket. Like join, it returns
// a token to be used for removing the timestamp from the bucket.
...
// Token represents the information identifying a tracked timestamp, tying a
// Track() to an Untrack().
type Token struct {
...
// be lowered at any time by others, this returned timestamp can only be used as
// an upper bound.
//
...
// un-initialize it.
func createIfNotInitialized(ctx context.Context, b *bucket, ts int64) int64 {
	if t, initialized := b.timestamp(); initialized {
		return t
	}
	// Initialize b.
	...
	ts, initialized := b.timestamp()
	if !initialized {
		log.Fatalf(ctx, "bucket unexpectedly not initialized")
	}
	return ts
}
