...
func TestLockfreeTrackerRandomStress(t *testing.T) {
	...
	for i := 0; i < numProducers; i++ {
		p := makeRequestProducer(
			...
			tr, &rs)
		g.GoCtx("run request producer", func(ctx context.Context) error {
			p.run(ctx)
			return nil
		})
	}
	...
	g.GoCtx("run request consumer", func(ctx context.Context) error {
		c.run(ctx)
		return nil
	})

	checker := makeTrackerChecker(stop, &mu, tr, &rs)
	g.GoCtx("run tracker checker", checker.run)
