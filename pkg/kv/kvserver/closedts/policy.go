...
func getNetworkRTT(policy ctpb.LatencyBasedRangeClosedTimestampPolicy) time.Duration {
	switch {
	...
	case policy <= ctpb.LEAD_FOR_GLOBAL_READS_LATENCY_GREATER_THAN_300MS:
		bucket := int(policy) - int(ctpb.LEAD_FOR_GLOBAL_READS_WITH_NO_LOCALITY)
		return time.Duration(bucket) * ClosedTimestampPolicyLatencyInterval
	default:
...
func TargetForPolicy(
	...
	case policy >= ctpb.LEAD_FOR_GLOBAL_READS_WITH_NO_LOCALITY &&
		...
		networkRTT := getNetworkRTT(policy)
		leadTimeAtSender = targetForPolicy(networkRTT, maxClockOffset, leadTargetOverride, sideTransportCloseInterval)
