...
func TestPolicyRefresher(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			latencies:       map[roachpb.NodeID]time.Duration{1: 100 * time.Millisecond},
		...
		},
		{
			...
			latencies:       map[roachpb.NodeID]time.Duration{1: 50 * time.Millisecond},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			closedts.LeadForGlobalReadsAutoTuneEnabled.Override(ctx, &st.SV, tc.autoTuneEnabled)
			...
			for _, replica := range tc.replicas {
				...
				expectedLatency, ok := tc.latencies[replica.nodeID]
				if tc.autoTuneEnabled && tc.version25_2 && ok {
					require.Equal(t, expectedLatency, got,
						"expected replica %d to have latency %v", replica.nodeID, expectedLatency)
				} else {
					require.Zero(t, got,
