...
func FindBucketBasedOnNetworkRTT(networkRTT time.Duration) ctpb.RangeClosedTimestampPolicy {
	// If maxLatency is negative (i.e. no peer latency is provided), It returns
	...
	// It divides RTT by policy interval, adds 1 for zero-based indexing, and
	// offsets by the base policy enum value.
