...
func TestGC(t *testing.T) {
	for _, d := range []struct {
		name string
		data string
		// TODO(oleg): Remove once we have all MVCC stats for ranges in place.
		// Currently live key stats are not updated correctly when doing delete
		// with range tombstone.
		ignoreLiveStats bool
	}{
		...
		{name: "delete_range_data", data: deleteRangeData, ignoreLiveStats: true},
		{name: "delete_range_data_newer", data: deleteRangeDataWithNewerValues, ignoreLiveStats: true},
		{name: "delete_range_multiple_points", data: deleteRangeMultipleValues, ignoreLiveStats: true},
		{name: "delete_range_with_intents", data: deleteRangeDataWithIntents, ignoreLiveStats: true},
	} {
		t.Run(d.name, func(t *testing.T) {
			runTest(t, d.data, d.ignoreLiveStats)
		})
	}
}

func runTest(t *testing.T, data string, ignoreLiveStats bool) {
	ctx := context.Background()
	...
	if ignoreLiveStats {
		stats.LiveCount = expectedStats.LiveCount
		stats.LiveBytes = expectedStats.LiveBytes
	}
...
func (d tableData) fullDistribution() dataDistribution {
	...
	return func() (storage.MVCCKeyValue, storage.MVCCRangeKey, *roachpb.Transaction, bool) {
		...
		return items[0].value, items[0].rangeKey, items[0].txn, true
	}
}
...
func engineData(t *testing.T, r storage.Reader, desc roachpb.RangeDescriptor) []tableCell {
	...
	for {
		...
		if r {
			...
			// TODO(oleg): should we panic if there's a value there?
...
func formatTable(data []tableCell, prefix roachpb.Key) []string {
	...
	lsLen := len(fmt.Sprintf("%d", tsSeq[0]))
