...
func TestIntentCleanupBatching(t *testing.T) {
	...
	info, err := Run(ctx, &desc, snap, nowTs, nowTs,
		RunOptions{IntentAgeThreshold: intentAgeThreshold, MaxIntentCleanupBatch: batchSize}, policy,
...
func (r *testResolver) assertInvariants(t *testing.T, opts intentBatcherOptions) {
	...
	for i, batch := range *r {
		...
		if opts.maxIntentCleanupBatch > 0 {
			require.LessOrEqual(t, int64(len(batch)), opts.maxIntentCleanupBatch,
				fmt.Sprintf("Batch size exceeded in batch %d/%d", i+1, len(*r)))
		}
		// Last key could overspill over limit, but that's ok.
		if opts.maxIntentCleanupBytes > 0 {
			require.Less(t, int64(totalKeyBytes-len(batch[len(batch)-1].Key)), opts.maxIntentCleanupBytes,
				fmt.Sprintf("Byte limit was exceeded for more than the last key in batch %d/%d", i+1, len(*r)))
		}
		if opts.maxTxnsPerCleanupBatch > 0 {
			require.LessOrEqual(t, int64(len(txnMap)), opts.maxTxnsPerCleanupBatch,
		...
		require.True(t, i == len(*r)-1 ||
			int64(len(batch)) == opts.maxIntentCleanupBatch ||
			int64(totalKeyBytes) >= opts.maxIntentCleanupBytes ||
			int64(len(txnMap)) == opts.maxTxnsPerCleanupBatch,
				...
				for _, txnCount := range []int64{0, 1, 10, 100, 1000} {
					t.Run(fmt.Sprintf("batch:%d bytes:%d txns:%d txn_intents:%s", batchSize, byteCount, txnCount, s.name), func(t *testing.T) {
						opts := intentBatcherOptions{
							maxIntentCleanupBatch:  batchSize,
							maxIntentCleanupBytes:  byteCount,
							maxTxnsPerCleanupBatch: txnCount,
						}

						resolver := testResolver{}
						batcher := intentBatcher{}

						batcher.init(resolver.resolveBatch, opts)
						for _, intent := range s.intents {
							require.NoError(t, batcher.addIntentToBatch(ctx, intent.key, intent.meta))
						}
						require.NoError(t, batcher.resolvePendingIntents(ctx))
