...
// TestNewVsInvariants verifies GC behaviour using randomized data distribution.
// To do that, it
// 1) generates data using random data distribution and fills up engine
// 2) runs GC and collects gc keys to be removed
// 3) applies keys to engine to remove old data
// 4) compares state of engine post gc with data from initial engine filtered by
//    gc predicates
// This approach allows us to use simple checks and ensures internal GC
// optimizations doesn't break GC logic.
// If failures are found, they could be used gc_test.go:TestGC, where key
// sequence could be added directly to cover additional cases.
