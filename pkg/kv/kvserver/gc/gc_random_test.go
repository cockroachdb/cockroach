...
func BenchmarkRun(b *testing.B) {
	...
	runGC := func(eng storage.Engine, old bool, spec randomRunGCTestSpec) (Info, error) {
		...
		if spec.inentAgeSec > 0 {
			intentThreshold = time.Duration(spec.inentAgeSec) * time.Second
		}
...
func assertGCInvariants(
	...
	pointExpectations := getPointExpectations(t, pointIt, gcThreshold, intentThreshold, &expInfo)
