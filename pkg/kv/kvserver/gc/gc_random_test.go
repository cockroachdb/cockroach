...
func TestRunNewVsOld(t *testing.T) {
	...
	for _, tc := range []randomRunGCTestSpec{
		...
		t.Run(fmt.Sprintf("%v@%v,ttlSec=%v", tc.ds, tc.now, tc.ttlSec), func(t *testing.T) {
			...
			t.Logf("running with random seed %d", seed)
...
func BenchmarkRun(b *testing.B) {
	...
	for _, old := range []bool{true, false} {
		b.Run(fmt.Sprintf("old=%v", old), func(b *testing.B) {
			rng, seed := randutil.NewTestRand()
			b.Logf("running with random seed %d", seed)
...
func TestNewVsInvariants(t *testing.T) {
	...
	for _, tc := range []randomRunGCTestSpec{
		...
		t.Run(fmt.Sprintf("%v@%v,ttl=%vsec", tc.ds, tc.now, tc.ttlSec), func(t *testing.T) {
			...
			sortedDistribution(tc.ds.dist(N, rng)).setupTest(t, eng, *tc.ds.desc())
			...
			require.NoError(t,
				storage.MVCCGarbageCollect(ctx, eng, nil, gcer.pointKeys(), gcThreshold))
			for _, i := range gcer.intents {
				...
				_, err := storage.MVCCResolveWriteIntent(ctx, eng, nil, l)
			...
			require.NoError(t,
				storage.MVCCGarbageCollectRanges(ctx, eng, nil, rangeKeys, gcThreshold))
...
func getExpectationsGenerator(
	...
	return func() (storage.MVCCKeyValue, bool) {
		for {
			...
			if i < len(history) {
				// TODO(oleg): once range keys are deleted, count any history as removed
				removedKeys := false
				for ; i < len(history); i++ {
					if !history[i].isRangeDel {
...
func rangeFragmentsFromIt(t *testing.T, it storage.MVCCIterator) [][]storage.MVCCRangeKey {
	var result [][]storage.MVCCRangeKey
	for {
		ok, err := it.Valid()
		...
		if r {
			fragments := make([]storage.MVCCRangeKey, len(it.RangeKeys()))
			for i, r := range it.RangeKeys() {
				fragments[i] = r.RangeKey.Clone()
			}
...
func filterRangeFragments(
	fragments [][]storage.MVCCRangeKey, gcThreshold hlc.Timestamp, expInfo *Info,
) [][]storage.MVCCRangeKey {
	var result [][]storage.MVCCRangeKey
	for _, stack := range fragments {
		var newStack []storage.MVCCRangeKey
		for i, r := range stack {
			if r.Timestamp.LessEq(gcThreshold) {
				// Update expectations:
				// On lowest range timestamp bump range counter.
				if i == len(stack)-1 {
					expInfo.NumRangesAffected++
				}
				// If all fragments are deleted then keys bytes are accounted for.
				if i == 0 {
					expInfo.AffectedVersionRangeKeyBytes += int64(len(r.StartKey) + len(r.EndKey))
				}
				// Count timestamps for all versions of range keys.
				expInfo.AffectedVersionRangeKeyBytes += storage.MVCCVersionTimestampSize
			} else {
...
func mergeRanges(fragments [][]storage.MVCCRangeKey) []storage.MVCCRangeKey {
	var result []storage.MVCCRangeKey
	var partialRangeKeys []storage.MVCCRangeKey
	var lastEnd roachpb.Key
	for _, stack := range fragments {
		start, end := stack[0].StartKey, stack[0].EndKey
		if lastEnd.Equal(start) {
			...
			for i < len(stack) && j < len(partialRangeKeys) {
				switch stack[i].Timestamp.Compare(partialRangeKeys[j].Timestamp) {
				case 1:
			...
			partialRangeKeys = make([]storage.MVCCRangeKey, 0, len(stack))
...
type fakeGCer struct {
	...
	gcRangeKeys []roachpb.GCRequest_GCRangeKey
	threshold   Threshold
...
func (f *fakeGCer) GC(
	...
	f.gcRangeKeys = append(f.gcRangeKeys, rangeKeys...)
	return nil
}
