...
func processReplicatedKeyRange(
	...
	return rditer.IterateMVCCReplicaKeySpans(desc, snap, rditer.IterateOptions{
		...
		b := gcKeyBatcher{
			...
			gcer:                      gcer,
			state:                     batcherCheckpoint{prevWasNewest: true},
...
// checkpoint time and the last processed key, as we only put checkpoints on
// complete batches.
...
func (c *batchingState) isEmpty() bool {
...
func (c *batchingState) merge(other batchingState) {
	c.gcBatchCounters.merge(other.gcBatchCounters)
	c.lastProcessedKey = other.lastProcessedKey
}

func (c *batchingState) clear() {
	*c = batchingState{}
}
...
type batcherCheckpoint struct {
	currentBatch  batchingState
	lastKey       batchingState
	prevWasNewest bool
}

func (cp *batcherCheckpoint) isEmpty() bool {
	return cp.currentBatch.isEmpty() && cp.lastKey.isEmpty()
}

func (cp *batcherCheckpoint) mergeLastKey() {
	cp.currentBatch.merge(cp.lastKey)
	cp.lastKey.clear()
}

func (cp *batcherCheckpoint) keySize() int64 {
	return cp.currentBatch.keyBytes + cp.lastKey.keyBytes
}

func (cp *batcherCheckpoint) clear() {
	cp.lastKey.clear()
...
// clear range, current state is saved into pointKeysBatch checkpoint.
...
// - restore state from pointKeysBatch and send point key batch, rewind to the
...
type gcKeyBatcher struct {
	...
	pointKeysBatch batcherCheckpoint
...
) (rewindKey storage.MVCCKey, counterUpdate gcBatchCounters, err error) {
	b.clearRangeGate.foundLiveData()

	// Not tracking range deletion yet.
	if b.pointKeysBatch.isEmpty() {
		...
		if isMeta {
			...
			return storage.MVCCKey{}, gcBatchCounters{}, nil
		}
...
func (b *gcKeyBatcher) flushPointsBatch(
	...
	b.state.mergeLastKey()
	counterUpdate = b.state.currentBatch.gcBatchCounters
	b.state.clear()
...
func (b *gcKeyBatcher) maybeFlushPendingRange(
	...
	if b.sequentialKeysFound < b.deleteRangeMinKeys || b.state.currentBatch.keysAffected == 0 {
		...
		b.state = b.pointKeysBatch
		b.pointKeysBatch.clear()
