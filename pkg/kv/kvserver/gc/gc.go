...
func processReplicatedKeyRange(
	...
	excludeUserKeySpan := false
	{
		...
		if err := storage.CanGCEntireRange(ctx, snap, start, end, threshold); err == nil {
			if err = gcer.GC(ctx, nil, nil, roachpb.GCRequest_GCClearRangeKey{
				...
				log.Warningf(ctx, "failed to perform GC clear range operation on range %s: %s", desc.String(),
					err)
	...
	for ; ; it.step() {
		...
		if shouldSendBatch {
			if err := gcer.GC(ctx, batchGCKeys, nil, roachpb.GCRequest_GCClearRangeKey{}); err != nil {
				if errors.Is(err, ctx.Err()) {
					return false, err
				}
...
func (b *rangeKeyBatcher) flushPendingFragments(ctx context.Context) error {
	if pendingCount := len(b.pending); pendingCount > 0 {
		...
		b.pendingSize = 0
		return b.gcer.GC(ctx, nil, toSend, roachpb.GCRequest_GCClearRangeKey{})
	}
	return nil
}
...
func (b *batchingInlineGCer) Flush(ctx context.Context) {
	err := b.gcer.GC(ctx, b.gcKeys, nil, roachpb.GCRequest_GCClearRangeKey{})
	b.gcKeys = nil
	b.size = 0
	if err != nil {
		b.onErr(err)
