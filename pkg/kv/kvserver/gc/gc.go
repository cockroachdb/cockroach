...
func processReplicatedLocks(
	...
	// We want to find/resolve replicated locks in over both local and global
	// keys.
	var ltSpans []roachpb.Span
	globalLTStart, _ := keys.LockTableSingleKey(roachpb.Key(desc.StartKey), nil)
	globalLTEnd, _ := keys.LockTableSingleKey(roachpb.Key(desc.EndKey), nil)
	localLTStart, _ := keys.LockTableSingleKey(keys.MakeRangeKeyPrefix(desc.StartKey), nil)
	localLTEnd, _ := keys.LockTableSingleKey(keys.MakeRangeKeyPrefix(desc.EndKey), nil)
	ltSpans = append(ltSpans, roachpb.Span{
		Key:    globalLTStart,
		EndKey: globalLTEnd,
	}, roachpb.Span{
		Key:    localLTStart,
