...
func (r *Replica) Send(
	...
	br, writeBytes, pErr := r.SendWithWriteBytes(ctx, ba, kvadmission.AdmissionInfo{})
	writeBytes.Release()
	return br, pErr
}

// SendWithWriteBytes is the implementation of Send with an additional
// *StoreWriteBytes return value. The admissionInfo parameter carries
// admission control state that is plumbed to evalAndPropose.
