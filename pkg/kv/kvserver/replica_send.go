...
func (r *Replica) maybeProxyRequest(
	...
	if !kvpb.ProxyBatchRequest.Get(&r.store.cfg.Settings.SV) {
	...
	// client has up-to-date information with us. If the server has stale
	// information we could choose to still proxy this request. In practice it
	// likely doesn't matter as the client will just retry once.
	if cri.DescriptorGeneration != ri.Desc.Generation ||
		cri.LeaseSequence != ri.Lease.Sequence {
		log.VEventf(ctx, 1, "stale descriptor don't proxy %v != %+v", ri, ba.Header.ClientRangeInfo)
		return br, pErr
	}
	...
	if ba.GatewayNodeID == r.NodeID() {
		log.VEventf(ctx, 1, "we are already the client, don't proxy to n%d", ba.GatewayNodeID)
		return br, pErr
	}
