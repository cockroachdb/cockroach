...
func (r *Replica) executeBatchWithConcurrencyRetries(
	...
	if ba.IsSingleRequest() {
		if mlt := ba.Requests[0].GetMigrateLockTable(); mlt != nil {
			// Special case: The lock table migration request needs to wait for
			// existing write latches on its entire request span, to ensure that
			// any existing request evaluation that could have written an interleaved
			// intent is complete. As adding a latch for the entire span could stall
