...
func (is Server) CompactEngineSpan(
	...
	err := is.execStoreCommand(ctx, req.StoreRequestHeader,
		func(ctx context.Context, s *Store) error {
			return s.StateEngine().CompactRange(req.Span.Key, req.Span.EndKey)
		})
	return resp, err
}
...
func (is Server) SetCompactionConcurrency(
	...
	err := is.execStoreCommand(ctx, req.StoreRequestHeader,
		func(ctx context.Context, s *Store) error {
			prevConcurrency := s.StateEngine().SetCompactionConcurrency(req.CompactionConcurrency)

			// Wait for cancellation, and once cancelled, reset the compaction concurrency.
			<-ctx.Done()
			s.StateEngine().SetCompactionConcurrency(prevConcurrency)
			return nil
		})
	return resp, err
}
