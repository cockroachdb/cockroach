...
func (r *Replica) shouldRenewLeaseRLocked(st kvserverpb.LeaseStatus) bool {
	...
	// TODO(erikgrinaker): requiresExpiringLeaseRLocked() incurs a key comparison,
	// which is too expensive here. Do the key comparison only when the range
	// descriptor changes.
	isExpirationLease := st.Lease.Type() == roachpb.LeaseExpiration
	shouldUseExpirationLease := onlyExpirationLeases.Get(&r.store.cfg.Settings.SV) ||
