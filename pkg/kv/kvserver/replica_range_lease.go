...
func (p *pendingLeaseRequest) requestLeaseAsync(
	...
	// Lease requests always bypass the circuit breaker (i.e. will prefer to get
	// stuck on an unavailable range rather than failing fast). This enables the
	// caller to chose between either behavior for themselves: if they too want to
	// bypass the circuit breaker, they simply don't check for the circuit breaker
	// while waiting for their lease handle. If they want to fail-fast, they do.
	// If the lease instead adopted the caller's preference, we'd have to handle
	// the case of multiple preferences joining onto one lease request, which is
	// more difficult.
