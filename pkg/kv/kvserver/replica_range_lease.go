...
func (r *Replica) leaseViolatesPreferences(ctx context.Context) bool {
	if !r.OwnsValidLease(ctx, r.store.cfg.Clock.NowAsClockTimestamp()) {
		log.Infof(ctx, "replica %s is not the leaseholder, cannot check if lease respects preferences", r)
		return false
	}
	storeDesc, err := r.store.Descriptor(ctx, false /* useCached */)
	if err != nil {
		log.Infof(ctx, "Unable to load the descriptor %v: cannot check if lease respects preference", err)
		return false
	}
	conf := r.SpanConfig()
