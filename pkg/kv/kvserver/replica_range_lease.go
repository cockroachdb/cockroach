...
func (p *pendingLeaseRequest) requestLease(
	...
	if reqLease.Type() == roachpb.LeaseEpoch && status.State == kvserverpb.LeaseState_EXPIRED &&
		...
		if err != nil {
			...
			// particular, if it was ours but we failed to reacquire it (e.g. because
			// our heartbeat failed due to a stalled disk) then we don't want
			// DistSender to retry us.
