...
func (p *pendingLeaseRequest) requestLeaseAsync(
	...
	err := p.repl.store.Stopper().RunAsyncTaskEx(
		...
		func(ctx context.Context) {
			...
			// lease. We give it the same timeout as a regular heartbeat, since lease
			// acquisition will sometimes do a synchronous heartbeat.
			err := contextutil.RunWithTimeout(ctx, taskName, nlRenewal, func(ctx context.Context) error {
