...
func maybeBumpReadTimestampToWriteTimestamp(
	...
	if batcheval.CanForwardCommitTimestampWithoutRefresh(ba.Txn, etArg) &&
		...
		return tryBumpBatchTimestamp(ctx, ba, ba.Txn.WriteTimestamp, spans)
	}
	return false
}
...
func tryBumpBatchTimestamp(
	ctx context.Context, ba *roachpb.BatchRequest, ts hlc.Timestamp, spans *spanset.SpanSet,
) bool {
	if spans.MaxProtectedTimestamp().Less(ts) {
