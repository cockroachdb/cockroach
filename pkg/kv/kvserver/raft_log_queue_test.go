...
func TestShouldTruncate(t *testing.T) {
	...
	testCases := []struct {
		truncatableIndexes roachpb.RaftIndex
		raftLogSize        int64
		expected           bool
	}{
...
func TestComputeTruncateDecision(t *testing.T) {
	...
	testCases := []struct {
		commit          roachpb.RaftIndex
		progress        []uint64
		raftLogSize     int64
		firstIndex      roachpb.RaftIndex
		lastIndex       roachpb.RaftIndex
		pendingSnapshot roachpb.RaftIndex
		exp             string
	}{
		{
			// Nothing to truncate.
...
func TestComputeTruncateDecisionProgressStatusProbe(t *testing.T) {
	...
	testutils.RunTrueAndFalse(t, "tooLarge", func(t *testing.T, tooLarge bool) {
		testutils.RunTrueAndFalse(t, "active", func(t *testing.T, active bool) {
			...
			progress := []roachpb.RaftIndex{100, 200, 300, 400, 500}
			lastIndex := roachpb.RaftIndex(500)
			status.Commit = 300
...
func TestNewTruncateDecision(t *testing.T) {
	...
	getIndexes := func() (roachpb.RaftIndex, int, roachpb.RaftIndex, error) {
	...
	// indexes updating.
	var cFirst, cOldest roachpb.RaftIndex
...
func TestSnapshotLogTruncationConstraints(t *testing.T) {
	...
	assertMin := func(exp roachpb.RaftIndex, now time.Time) {
		t.Helper()
...
func waitForTruncationForTesting(
	t *testing.T, r *Replica, newFirstIndex roachpb.RaftIndex, looselyCoupled bool,
) {
	testutils.SucceedsSoon(t, func() error {
		if looselyCoupled {
			// Flush the engine to advance durability, which triggers truncation.
