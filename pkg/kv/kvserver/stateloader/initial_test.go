...
func TestSynthesizeHardState(t *testing.T) {
	...
	testCases := []struct {
		...
		{OldHS: &tHS, RaftAppliedIndex: roachpb.RaftIndex(tHS.Commit - 1), Err: "can't decrease HardState.Commit"},
		{OldHS: &tHS, RaftAppliedIndex: roachpb.RaftIndex(tHS.Commit), NewHS: tHS},
		{OldHS: &tHS, RaftAppliedIndex: roachpb.RaftIndex(tHS.Commit + 1), NewHS: raftpb.HardState{Term: tHS.Term, Vote: 3, Commit: tHS.Commit + 1}},
		// Higher Term is picked up, but vote isn't carried over when the term
		// changes.
		{OldHS: &tHS, RaftAppliedIndex: roachpb.RaftIndex(tHS.Commit), TruncTerm: 11, NewHS: raftpb.HardState{Term: 11, Vote: 0, Commit: tHS.Commit}},
	}
