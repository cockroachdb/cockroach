...
func (rsl StateLoader) Load(
	...
	if s.GCRangeHint, err = rsl.LoadGCHint(ctx, reader); err != nil {
		return kvserverpb.ReplicaState{}, err
	}
...
func (rsl StateLoader) Save(
	...
	if err := rsl.SetGCHint(ctx, readWriter, ms, state.GCRangeHint); err != nil {
		return enginepb.MVCCStats{}, err
	}
...
func (rsl StateLoader) LoadGCHint(
	ctx context.Context, reader storage.Reader,
) (*roachpb.GCRangeHint, error) {
	var h roachpb.GCRangeHint
...
func (rsl StateLoader) SetGCHint(
	...
	readWriter storage.ReadWriter,
	ms *enginepb.MVCCStats,
	hint *roachpb.GCRangeHint,
) error {
	if hint == nil {
		return errors.New("cannot persist nil GCRangeHint")
	}
