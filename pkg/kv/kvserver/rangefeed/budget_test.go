...
func TestFeedBudget(t *testing.T) {
	makeBudgetWithSize := func(size int64) (*FeedBudget, *mon.BytesMonitor, *mon.BoundAccount) {
		...
		f := NewFeedBudget(&b, NewMetrics())
		return f, m, &b
	}
	ctx := context.Background()

	t.Run("allocate one", func(t *testing.T) {
		f, _, b := makeBudgetWithSize(40)
