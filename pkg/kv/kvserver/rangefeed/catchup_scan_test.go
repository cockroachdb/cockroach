...
func TestCatchupScanWithDiffAndTBI(t *testing.T) {
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			err := iter.CatchupScan(storage.MakeMVCCMetadataKey(tc.startKey),
				storage.MakeMVCCMetadataKey(tc.endKey), tc.ts, true, func(e *roachpb.RangeFeedEvent) error {
			...
			err = iter2.CatchupScan(storage.MakeMVCCMetadataKey(tc.startKey),
				storage.MakeMVCCMetadataKey(tc.endKey), tc.ts, true, func(e *roachpb.RangeFeedEvent) error {
