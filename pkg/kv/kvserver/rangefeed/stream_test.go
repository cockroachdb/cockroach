...
func ev(id int64, key string) *kvpb.MuxRangeFeedEvent {
...
func (s *testMuxSink) setError(err error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *testMuxSink) expectSoonAtMost(t *testing.T, evs ...*kvpb.MuxRangeFeedEvent) {
	t.Helper()
	checkPrefix := func() error {
		...
		for _, event := range s.mu.data {
			...
			if j >= len(evs) {
				return errors.Newf("unexpected event %s in excess of expected %d, history: %s", event, j,
					s.mu.data)
			}
			require.Equal(t, evs[j], event, "unexpected value %s at index %d, history: %s", event, j,
				s.mu.data)
			j++
		}
		return nil
...
func (s *testSink) ExpectSoon(t *testing.T, keys ...string) {
	t.Helper()
	testutils.SucceedsSoon(t, func() error {
		s.mu.Lock()
		defer s.mu.Unlock()
		if exp, actual := len(keys), len(s.mu.data); exp != actual {
			return errors.Newf("incorrect number of keys: expected %d, found %d", exp, actual)
		}
		for i, key := range keys {
			require.Equal(t, feedEvent(key), s.mu.data[i], "wrong value at index %d", i)
		}
		return nil
	})
}
...
func (s *testSink) ExpectSoonAtMost(t *testing.T, keys ...string) {
	t.Helper()
	checkPrefix := func() error {
		s.mu.Lock()
		defer s.mu.Unlock()
		...
		for i, data := range s.mu.data {
			require.Equal(t, feedEvent(keys[i]), data, "wrong value at index %d", i)
		}
		return nil
	}
	testutils.SucceedsSoon(t, checkPrefix)
...
func testFeedBudget(t *testing.T) (
	*FeedBudget, func(size int64) *SharedBudgetAllocation, func(),
) {
...
func TestBufferedStreamSend(t *testing.T) {
	...
	s := NewSingleBufferedStream(sink.ctx, sink, func() {
		close(drainDone)
	})
...
func TestBufferedMuxedStreamSend(t *testing.T) {
	...
	require.True(t, s.Send(feedEvent("a"), alloc(10)))
	require.True(t, s.Send(feedEvent("b"), alloc(10)))
	require.True(t, s.Send(feedEvent("c"), alloc(10)))
	procRelease()

	sink.expectSoon(t, ev(3, "a"), ev(3, "b"), ev(3, "c"))
}
...
func TestBufferedMuxedStreamError(t *testing.T) {
	...
	sink.expectErrorSoon(t, s1.streamID,
		kvpb.NewError(kvpb.NewRangeFeedRetryError(kvpb.RangeFeedRetryError_REASON_REPLICA_REMOVED)))
	waitStreamNotified(t, drain1Done)
	sink.expectHas(t, ev(s2.streamID, "b"), ev(s2.streamID, "c"), ev(s2.streamID, "e"))
	sink.expectHasNo(t, ev(s1.streamID, "d"))
	require.Zero(t, budget.mu.memBudget.Used(), "memory budget wasn't released")
}
