...
func TestUnbufferedSenderDisconnect(t *testing.T) {
	...
	t.Run("nil handling", func(t *testing.T) {
		...
		ubs.SendErrorWithoutBlocking(makeMuxRangefeedErrorEvent(streamID, rangeID,
			kvpb.NewError(nil)))
	...
	t.Run("send rangefeed completion error concurrently", func(t *testing.T) {
		...
		for _, muxError := range testRangefeedCompletionErrors {
			...
			go func(streamID int64, rangeID roachpb.RangeID, err error) {
				defer wg.Done()
				ubs.SendErrorWithoutBlocking(makeMuxRangefeedErrorEvent(streamID, rangeID, kvpb.NewError(err)))
...
// TestUnbufferedSenderConcurrentSend tests that UnbufferedSender.Send is thread-safe.
