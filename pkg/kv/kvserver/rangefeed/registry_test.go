...
func newTestRegistration(
	...
	//m := mon.NewUnlimitedMonitor(context.Background(), "test-memmon", mon.MemoryResource, nil, nil, 0,
	//	nil)
...
func TestRegistrationBasic(t *testing.T) {
	...
	noCatchupReg.publish(ev1, nil)
	noCatchupReg.publish(ev2, nil)
	...
	catchupReg.publish(ev1, nil)
	catchupReg.publish(ev2, nil)
	...
	disconnectReg.publish(ev1, nil)
	disconnectReg.publish(ev2, nil)
	...
	disconnectEarlyReg.publish(ev1, nil)
	disconnectEarlyReg.publish(ev2, nil)
...
func TestRegistryBasic(t *testing.T) {
	...
	require.NotPanics(t, func() { reg.PublishToOverlapping(spAB, ev1, nil) })
	...
	reg.PublishToOverlapping(spAB, ev1, nil)
	reg.PublishToOverlapping(spBC, ev2, nil)
	reg.PublishToOverlapping(spCD, ev3, nil)
	reg.PublishToOverlapping(spAC, ev4, nil)
	...
	reg.PublishToOverlapping(spAB, ev4, nil)
	reg.PublishToOverlapping(spBC, ev3, nil)
	reg.PublishToOverlapping(spCD, ev2, nil)
	reg.PublishToOverlapping(spAC, ev1, nil)
	require.NoError(t, reg.waitForCaughtUp(all))
...
func TestRegistryPublishAssertsPopulatedInformation(t *testing.T) {
	...
	require.Panics(t, func() { reg.PublishToOverlapping(spAB, ev, nil) })
	require.Panics(t, func() { reg.PublishToOverlapping(spCD, ev, nil) })
	require.NoError(t, reg.waitForCaughtUp(all))
	...
	require.NotPanics(t, func() { reg.PublishToOverlapping(spAB, ev, nil) })
	require.NotPanics(t, func() { reg.PublishToOverlapping(spCD, ev, nil) })
	require.NoError(t, reg.waitForCaughtUp(all))
...
func TestRegistryPublishBeneathStartTimestamp(t *testing.T) {
	...
	reg.PublishToOverlapping(spAB, ev, nil)
	require.NoError(t, reg.waitForCaughtUp(all))
