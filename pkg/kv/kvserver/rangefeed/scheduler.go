...
// enqueueStartTime returns now in nanos or 0. 0 means event will have no start
...
func (ss *schedulerShard) enqueueStartTime() int64 {
	var now int64
...
func (ss *schedulerShard) enqueueN(ids []int64, evt processorEventType) int {
	...
	// For bulk insertions we ignore sampling frequency and just use one sample
	// per chunk for simplicity.
	now := timeutil.Now().UnixNano()
	ss.Lock()
	var count int
	for i, id := range ids {
...
func (ss *schedulerShard) processEvents(ctx context.Context) {
	for {
		...
		if newStatus == Queued {
			...
			ss.queue.pushBack(queueEntry{id: entry.id, startTime: ss.enqueueStartTime()})
