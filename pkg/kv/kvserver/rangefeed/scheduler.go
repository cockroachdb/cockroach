...
func NewScheduler(cfg SchedulerConfig) *Scheduler {
	...
	for i := 0; i < numShards; i++ {
		...
		s.shards = append(s.shards,
			newSchedulerShard(shardWorkers, bulkChunkSize, cfg.Metrics, histogramFrequency))
	}
...
func newSchedulerShard(
	numWorkers, bulkChunkSize int, metrics *SchedulerMetrics, histogramFrequency int64,
) *schedulerShard {
	ss := &schedulerShard{
...
func (ss *schedulerShard) enqueueLocked(entry queueEntry, evt processorEventType) bool {
	...
	if pending == 0 {
		// Only keep timestamp on entries every HistogramFrequency times.
		if ss.nextLatencyCheck <= 1 {
			ss.nextLatencyCheck = ss.histogramFrequency
		} else {
			entry.startTime = 0
		}
		ss.nextLatencyCheck--
...
func (ss *schedulerShard) processEvents(ctx context.Context) {
	for {
		...
		if newStatus == Queued {
			...
			// Create some hacky start time if original event had it. We place it
			// in between of now and when original event was sent as a wrong estimate.
			newTime := int64(0)
			if entry.startTime > 0 {
				newTime = (timeutil.Now().UnixNano() - entry.startTime)/2
			}
			ss.queue.pushBack(queueEntry{id: entry.id, startTime: newTime})
