...
// shard count and the range's priority. Priority processors are assigned to the
...
type Scheduler struct {
	...
	// separate shards to reduce contention at high worker counts. Allocation is
	// modulo processors, with shard 0 reserved for priority processors.
...
// which should be used to send notifications to the callback. If priority is
// true, the range is allocated to a separate priority shard with 2 dedicated
// workers (intended for a small number of system ranges). Returns error if
// Scheduler is stopped.
func (s *Scheduler) Register(f Callback, priority bool) (int64, error) {
	id := s.nextID.Add(1)
	if priority {
...
func (cs *ClientScheduler) Register(cb Callback, priority bool) error {
	...
	var err error
	cs.id, err = cs.s.Register(cb, priority)
	return err
}
