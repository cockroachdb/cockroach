...
func TestBufferedSenderWithSendBufferedError(t *testing.T) {
	...
	t.Run("basic operation", func(t *testing.T) {
		...
		sm.AddStream(int64(streamID), cancel)
		require.Equal(t, int64(0), getLen())
		sm.SendBufferedError(makeMuxRangefeedErrorEvent(int64(streamID), 1, kvpb.NewError(nil)))
...
func TestBufferedSenderOnStop(t *testing.T) {
	...
	for i := 0; i < 100000; i++ {
		...
		if randBool || streamIdStart == streamIdEnd {
			_, cancel := context.WithCancel(context.Background())
			sm.AddStream(streamIdEnd, cancel)
			streamIdEnd++
		} else {
			sm.SendBufferedError(makeMuxRangefeedErrorEvent(streamIdStart, 1, kvpb.NewError(nil)))
			streamIdStart++
		}
	}
	require.NoError(t, bs.waitForEmptyBuffer(ctx))
