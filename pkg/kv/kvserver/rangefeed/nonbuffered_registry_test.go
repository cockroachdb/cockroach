...
type syncBufferedStream struct {
	ctx context.Context
	mu  struct {
		syncutil.Mutex
...
var _ BufferedStream = (*syncBufferedStream)(nil)

func (s *syncBufferedStream) Context() context.Context {
	return s.ctx
}

func (s *syncBufferedStream) Send(event *kvpb.RangeFeedEvent, _ *SharedBudgetAllocation) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.mu.events = append(s.mu.events, event)
}

func (s *syncBufferedStream) SendUnbuffered(event *kvpb.RangeFeedEvent) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.mu.events = append(s.mu.events, event)
	return nil
...
func (s *syncBufferedStream) SendError(err *kvpb.Error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.mu.error == nil {
		s.mu.error = err
...
func (s *syncBufferedStream) events() []*kvpb.RangeFeedEvent {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.mu.events
}

func (s *syncBufferedStream) assertError(t *testing.T) {
	t.Helper()
	testutils.SucceedsSoon(t, func() error {
		s.mu.Lock()
		defer s.mu.Unlock()
		if s.mu.error == nil {
			return errors.New("stream didn't finish with error")
		}
		return nil
	})
}

func (s *syncBufferedStream) blockSend() (unblock func()) {
	s.mu.Lock()
	return s.mu.Unlock
}
...
func newNonBufferedTestRegistration(
	...
	catchupSize int,
	withDiff bool,
) (*nonBufferedRegistration, <-chan struct{}, *syncBufferedStream) {
	s := &syncBufferedStream{}
	drainedC := make(chan struct{}, 1)
	reg := newNonBufferedRegistration(span, ts, withDiff, NewMetrics())
	reg.connect(s, catchupSize, makeCatchUpIterator(catchup, span, ts))
