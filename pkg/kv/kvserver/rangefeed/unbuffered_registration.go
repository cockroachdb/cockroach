...
func (ubr *unbufferedRegistration) publishCatchUpBuffer(ctx context.Context) error {
	publish := func(underLock bool) error {
		// When not under lock, we use the unbuffered sender until we have sent a
		// non-empty checkpoint or until we've sent a full buffer worth of events.
		// The goal here is to inform the stream of its successful catch-up scan
		// promptly. The checkpoint is typically the first event in the buffer.
		//
		// When under lock we always use the buffered sender to avoid blocking for
		// too long.
		shouldSendBuffered := underLock
		...
		for {
			...
			case e := <-ubr.mu.catchUpBuf:
				...
				if shouldSendBuffered {
					...
					unbufferedSendCount++
					if e.event.Checkpoint != nil && !e.event.Checkpoint.ResolvedTS.IsEmpty() {
						foundCheckpoint = true
						shouldSendBuffered = true
					}
					if unbufferedSendCount > cap(ubr.mu.catchUpBuf) {
						if !foundCheckpoint {
							log.KvDistribution.Warningf(ctx, "no non-empty checkpoint found before transitioning to buffered sender")
						}
						shouldSendBuffered = true
					}
