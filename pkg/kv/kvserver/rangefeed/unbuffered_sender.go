...
//		                                                  			          ┌─────────────▼────────────┴──────────────┐
//		 MuxRangefeedEvent ──────────────────────────────────────────────►│            Node.MuxRangeFeed            │◄─────────────────────────────────────── MuxRangefeedEvent ────── MuxRangefeedEvent with kvpb.RangeFeedError
//		       │                         		                   	          └─────────────────┬───▲───────────────────┘                                                 │               (client: rangefeedMuxer.restartActiveRangeFeed)
//	         │                                                           Sender.AddStream │   │LockedMuxStream.Send(*kvpb.MuxRangefeedEvent)                        │                                        ▲
//	         │                                           		                 ┌────────────▼───┴──────────┐                                                          │                                        │
...
//		       │                                         	  	                     └────────┬─────────┘                         │                                     │                                        │
//		       │                                        		                                │                                   │                                     │                                        │
//	   BufferedSender ◄───── BufferedSender         BufferedSender                ┌───────▼─────────┐                BufferedSender            UnbufferedSender   UnbufferedSender                           │
//	        Run            	  SendBuffered          RegisterRangefeedCleanUp      │ Store.Rangefeed │      RegisterRangefeedCleanUp          SendUnbuffered     SendErrorWithoutBlocking ───────────► UnbufferedSender.run
//	         ▲           	         ▲                       ▲         		          └───────┬─────────┘                         ▲                                     ▲                                        ▲
//	         │           	         │                       │         		                  │                                   │                                     │                                        │
//	         │           	         │                       │         		         ┌────────▼──────────┐                        │                                     │                                        │
//	         │           	         │                       │         		         │ Replica.Rangefeed │                        │                                     │                                        │
...
//	         │           	         │                       │                       ┌──────▼───────┐                           │                                     │                                        │
//	         └──────────────────── └────────────────────── └───────────────────────│ Registration ├───────────────────────────┘                                     │                                        │
//	                    	         │               ScheduledProcessor.Register     └──────┬───────┘  ScheduledProcessor.Register                                    │                                        │
//	                    	         │                                		                  │                                                                         │                                        │
//	                    	      Stream.Send                                 		          └─────────────────────────────────────────────────────────────────────────┘────────────────────────────────────────┘
//	                        Stream.Disconnect                                			                                                                                              Stream.Send             Stream.Disconnect
//
...
// - For error events, SendErrorWithoutBlocking is non-blocking and ensures
// 1) stream context is canceled
// 2) exactly one error is sent back to the client on behalf of the stream
// 3) metrics updates.
// It makes sure SendErrorWithoutBlocking is non-blocking by delegating the
...
type UnbufferedSender struct {
	...
	// reg is used to send events to the underlying grpc server stream. Note
	// that streamReg.Send is blocking.
	reg struct {
		syncutil.Map[int64, context.CancelFunc]
...
func NewUnbufferedSender(
	...
	ubs.reg.metrics = metrics
	return ubs
}

// SendErrorWithoutBlocking 1. Sends a mux rangefeed completion error to the
...
// should return, allowing clients to know rangefeed completions.
//
// Note that this function can be called by the processor worker while holding
// raftMu, so it is important that this function doesn't block IO. Caller needs
// to make sure this is called only with non-nil error events. Important to be
// thread-safe.
func (ubs *UnbufferedSender) SendErrorWithoutBlocking(ev *kvpb.MuxRangeFeedEvent) {
	if ev.Error == nil {
		log.Errorf(context.Background(), "SendWithoutBlocking called with non-error event")
	}

	if cancel, ok := ubs.reg.LoadAndDelete(ev.StreamID); ok {
		// Fine to skip nil checking here since that would be a programming error.
		(*cancel)()
		ubs.reg.metrics.UpdateMetricsOnRangefeedDisconnect()
		ubs.appendMuxError(ev)
	}
}

// SendUnbuffered blocks until the event is sent to the underlying grpc stream.
// If this function returns an error, caller must ensure that no further events
// are sent from rangefeed.Stream to avoid potential event loss. (NB: While
// subsequent Send should also return an error if one is encountered. But let's
// play safe.) Important to be thread-safe.
...
func (ubs *UnbufferedSender) Error() chan error {
	if ubs.errCh == nil {
		log.Fatalf(context.Background(), "streamRegistry.Error called before streamRegistry.Start")
	}
	return ubs.errCh
}

// Stop cancels the streamRegistry.run task and waits for it to complete. It
...
func (ubs *UnbufferedSender) Stop() {
	...
	// cancellation, decrement metrics here since node level will cancel all
	// streams context. SendErrorWithoutBlocking will be called again by
	// rangefeed.Stream. No errors will be sent but metrics cleanup will still
	// happen during SendErrorWithoutBlocking.
}

// AddStream registers a rangefeed.Stream with UnbufferedSender. It remains
// active until SendErrorWithoutBlocking is called with the same streamID.
...
func (ubs *UnbufferedSender) AddStream(streamID int64, cancel context.CancelFunc) {
	if _, loaded := ubs.reg.LoadOrStore(streamID, &cancel); loaded {
		log.Fatalf(context.Background(), "stream %d already exists", streamID)
	}
	ubs.reg.metrics.UpdateMetricsOnRangefeedConnect()
}
