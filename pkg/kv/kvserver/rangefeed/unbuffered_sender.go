...
// sendBuffered 1. Sends a mux rangefeed completion error to the client without
// blocking. It does so by delegating the responsibility of sending mux error to
// UnbufferedSender.run 2. Disconnects the stream with ev.StreamID. Safe to call
// repeatedly for the same stream, but subsequent errors are ignored.
//
...
// sendIsThreadSafe is a no-op declaration method. sendUnbuffered wraps the
// lockedMuxStream which ensures thread safety. sendBuffered is also thread-safe
// by design.
func (ubs *UnbufferedSender) sendIsThreadSafe() {}
