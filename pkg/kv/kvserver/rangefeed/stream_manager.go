...
// |  MuxRangeFeed   +-------------->|   SteamManager  +----------------->|   Registration  |
// |                 |               |                 |  Send/SendError  |                 |
// |                 |               |                 |<-----------------+                 |
// +-----------------+               +-----------------+                  +-------------+---+
//                                                                            ^         |
...
type sender interface {
	...
	// directly. Note that this call may block.
	...
	// stream. It should be unblocking.
...
func (sm *StreamManager) DisconnectStream(streamID int64, err *kvpb.Error) {
	if err == nil {
		log.Fatalf(context.Background(),
			"unexpected: DisconnectStream called with non-error event")
		return
	}
	sm.streams.Lock()
	defer sm.streams.Unlock()
...
func (sm *StreamManager) AddStream(streamID int64, d Disconnector) {
	...
	defer sm.streams.Unlock()
	if d.IsDisconnected() {
		// If the stream is already disconnected, we don't add it to the stream. An
