	...
	filter := runRequest(p, func(ctx context.Context, p *ScheduledProcessor) *Filter {
		...
		if ubr, ok := r.(*unbufferedRegistration); ok {
			bufferedStream.RegisterRangefeedCleanUp(func() {
				...
				// disconnect happens from the rangefeed side, this is no-op. But if
				// this is called from streamMuxer.disconnectAll, this disconnects
				// everything properly. runOutputLoop is responsible for draining
				// catchUpBuf.
