...
func (rts *resolvedTimestamp) recompute() bool {
	...
	if txn := rts.intentQ.Oldest(); txn != nil {
		if txn.timestamp.LessEq(rts.resolvedTS) {
			panic(fmt.Sprintf("unresolved txn below resolved timestamp: %s <=%s",
				txn.timestamp, rts.resolvedTS))
		}
		// txn.timestamp cannot be resolved, so we need a lower timestamp.
		// txn.timestamp.Prev is dangerous to use, potentially having
		// Logical=MaxInt32. So, we do a FloorPrev, but that might be too much - it
		// might takes us down below the resolved timestamp (although that'd be
		// unexpected; it should only happen if we've closed logical timestamps,
		// which we currently don't do). So, we further Forward to the existing
		// resolvedTS.
