...
func generateOneLogicalOpEvent(
	...
	expectedMemUsage += eventOverhead + mem + mvccLogicalOp
	expectedFutureMemUsage += futureMem
...
func generateRandomizedEventAndSend(
	rand *rand.Rand,
) (ev event, expectedMemUsage int64, expectedFutureMemUsage int64, randomlyChosenEvent string) {
	// Opt out sst event since it requires testing.T to call
	// storageutils.MakeSST(t, st, data.kvs)
	typesOfEvents := []string{"logicalsOps", "ct", "initRTS", "sync", "sst"}
	randomlyChosenEvent = typesOfEvents[rand.Intn(len(typesOfEvents))]
	data := generateRandomTestData(rand)
	switch randomlyChosenEvent {
	case "logicalsOps":
		ev, expectedMemUsage, expectedFutureMemUsage = generateLogicalOpEvents(data)
	case "ct":
	...
	return ev, expectedMemUsage, expectedFutureMemUsage, randomlyChosenEvent
}
...
func TestBasicEventSizeCalculation(t *testing.T) {
	...
	t.Run("write_value event", func(t *testing.T) {
		ev, futureEvent, expectedMemUsage, expectedFutureMemUsage := generateOneLogicalOpEvent("write_value", data)
		curr, future := ev.currAndFutureMemUsage()
	...
	t.Run("delete_range event", func(t *testing.T) {
		ev, futureEvent, expectedMemUsage, expectedFutureMemUsage := generateOneLogicalOpEvent("delete_range", data)
		curr, future := ev.currAndFutureMemUsage()
		require.Equal(t, expectedMemUsage, curr)
		require.Equal(t, expectedFutureMemUsage, future)
		require.Equal(t, EventMemUsage(&ev), max(expectedMemUsage, expectedFutureMemUsage))
	...
	t.Run("write_intent event", func(t *testing.T) {
		ev, futureEvent, expectedMemUsage, expectedFutureMemUsage := generateOneLogicalOpEvent("write_intent", data)
		curr, future := ev.currAndFutureMemUsage()
		require.Equal(t, expectedMemUsage, curr)
		require.Equal(t, expectedFutureMemUsage, future)
	...
	t.Run("update_intent event", func(t *testing.T) {
		ev, futureEvent, expectedMemUsage, expectedFutureMemUsage := generateOneLogicalOpEvent("update_intent", data)
		curr, future := ev.currAndFutureMemUsage()
		require.Equal(t, expectedMemUsage, curr)
		require.Equal(t, expectedFutureMemUsage, future)
		require.Equal(t, EventMemUsage(&ev), max(expectedMemUsage, expectedFutureMemUsage))
	...
	t.Run("commit_intent event", func(t *testing.T) {
		ev, futureEvent, expectedMemUsage, expectedFutureMemUsage := generateOneLogicalOpEvent("commit_intent", data)
		curr, future := ev.currAndFutureMemUsage()
		require.Equal(t, expectedMemUsage, curr)
		require.Equal(t, expectedFutureMemUsage, future)
		require.Equal(t, EventMemUsage(&ev), max(expectedMemUsage, expectedFutureMemUsage))
	...
	t.Run("abort_intent event", func(t *testing.T) {
		ev, futureEvent, expectedMemUsage, expectedFutureMemUsage := generateOneLogicalOpEvent("abort_intent", data)
		curr, future := ev.currAndFutureMemUsage()
		require.Equal(t, expectedMemUsage, curr)
		require.Equal(t, expectedFutureMemUsage, future)
		require.Equal(t, EventMemUsage(&ev), max(expectedMemUsage, expectedFutureMemUsage))
	...
	t.Run("abort_txn event", func(t *testing.T) {
		ev, futureEvent, expectedMemUsage, expectedFutureMemUsage := generateOneLogicalOpEvent("abort_txn", data)
		curr, future := ev.currAndFutureMemUsage()
		require.Equal(t, expectedMemUsage, curr)
		require.Equal(t, expectedFutureMemUsage, future)
		require.Equal(t, EventMemUsage(&ev), max(expectedMemUsage, expectedFutureMemUsage))
...
func BenchmarkMemoryAccounting(b *testing.B) {
	b.Run("memory_calculation", func(b *testing.B) {
		...
		expectedRes := []res{}
		for i := 0; i < 20; i++ {
			ev, mem, futureMem, chosenEvent := generateRandomizedEventAndSend(rand)
		...
		for i := 0; i < 20; i++ {
			...
			diff += int(expectedRes[i].currMemUsages - expectedRes[i].futureMemUsages)

			if expectedRes[i].currMemUsages < expectedRes[i].futureMemUsages {
				if expectedRes[i].chosenEvent != "initRTS" && expectedRes[i].chosenEvent != "ct" {
					b.Logf("currMemUsage %d < futureMemUsage: %d\n",
						expectedRes[i].currMemUsages, expectedRes[i].futureMemUsages)
				}
			} else {
				b.Logf("futureMemUsage %d < currMemUsage: %d\n",
					expectedRes[i].futureMemUsages, expectedRes[i].currMemUsages)
			}
		}
