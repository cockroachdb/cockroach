...
func TestProcessorBasic(t *testing.T) {
	...
	r1Stream.Cancel()
	require.NotNil(t, future.MakeAwaitableFuture(&r1Done).Get())

	// Stop the processor with an error.
	pErr := kvpb.NewErrorf("stop err")
	p.StopWithErr(pErr)
	require.NotNil(t, future.MakeAwaitableFuture(&r2Done).Get())
...
func TestProcessorSlowConsumer(t *testing.T) {
	...
	require.Equal(t, newErrBufferCapacityExceeded().GoError(), future.MakeAwaitableFuture(&r1Done).Get())
...
func BenchmarkProcessorWithBudget(b *testing.B) {
	...
	if p.reg.Len() == 0 {
		require.NoError(b, future.MakeAwaitableFuture(&r1Done).Get())
	}
}
