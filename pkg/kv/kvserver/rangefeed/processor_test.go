...
func (h *processorTestHelper) triggerTxnPush() {
	if h.scheduler != nil {
		_ = h.scheduler.Schedule(PushTxn)
	}
}
...
func newTestProcessor(t tHelper, opts ...option) (Processor, *processorTestHelper, *stop.Stopper) {
	...
	if cfg.useScheduler {
		sch := NewScheduler(SchedulerConfig{Name: "test-scheduler", Workers: 1})
		_ = sch.Start(stopper)
