...
func TestProcessorBasic(t *testing.T) {
	...
	r1Stream.Cancel()
	require.NotNil(t, r1Done.Get())

	// Stop the processor with an error.
	pErr := kvpb.NewErrorf("stop err")
	p.StopWithErr(pErr)
	require.NoError(t, r2Done.Get())
	require.NotNil(t, r2Done.Get())
...
func TestProcessorSlowConsumer(t *testing.T) {
	...
	require.Equal(t, newErrBufferCapacityExceeded().GoError(), r1Done.Get())
...
func BenchmarkProcessorWithBudget(b *testing.B) {
	...
	if p.reg.Len() == 0 {
		err := future.MakeAwaitableFuture(&r1Done).Get()
		pErr, err := r1Done.Get()
