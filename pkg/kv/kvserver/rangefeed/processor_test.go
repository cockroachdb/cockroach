...
func TestProcessorSlowConsumer(t *testing.T) {
	defer leaktest.AfterTest(t)()
	testutils.RunValues(t, "feed type", []rangefeedTestType{legacyProcessor, scheduledProcessorWithBufferedReg},
	...
	testutils.RunValues(t, "feed type", testTypes, func(t *testing.T, rt rangefeedTestType) {
		...
		for i := 0; i < trials; i++ {
			...
			go func() {
				...
				runtime.Gosched()
				s := newTestStream()
				p.Register(ctx, h.span, hlc.Timestamp{}, nil, /* catchUpIter */
					false /* withDiff */, false /* withFiltering */, false, /* withOmitRemote */
					h.toBufferedStreamIfNeeded(s), func() {})
			}()
			go func() {
				defer wg.Done()
				runtime.Gosched()
...
func (c *consumer) AddRegistration(r registration, cleanup func(registration) bool) {
}
...
func TestProcessorBackpressure(t *testing.T) {
	...
	ok, _ := p.Register(ctx, span, hlc.MinTimestamp, nil, /* catchUpIter */
