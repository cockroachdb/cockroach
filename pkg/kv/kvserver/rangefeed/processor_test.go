...
func TestProcessorBasic(t *testing.T) {
	...
	r1Done := future.MakePromise[*roachpb.Error]()
	...
	r2Done := future.MakePromise[*roachpb.Error]()
	...
	r3OK, _ := p.Register(
		...
		r3Stream,
		func() {},
		future.MakePromise[*roachpb.Error](),
	)
...
func TestNilProcessor(t *testing.T) {
	...
	require.Panics(t, func() {
		p.Register(roachpb.RSpan{}, hlc.Timestamp{}, nil, false, nil,
			func() {}, future.MakePromise[*roachpb.Error](),
		)
	})
}
...
func TestProcessorSlowConsumer(t *testing.T) {
	...
	pErr, err := r1Done.Get()
	require.NoError(t, err)
	require.Equal(t, newErrBufferCapacityExceeded().GoError(), pErr.GoError())
...
func TestProcessorMemoryBudgetExceeded(t *testing.T) {
	...
	v, err := r1Done.Get()
	require.NoError(t, err)
	require.Equal(t, newErrBufferCapacityExceeded().GoError(), v.GoError())
	...
	go func() {
		...
		for firstIdx := range firstC {
			...
			p.Register(p.Span, hlc.Timestamp{}, nil, false,
				s, func() {}, future.MakePromise[*roachpb.Error]())
			regDone <- struct{}{}
		}
	}()
	wg.Wait()
...
func notifyWhenDone(f future.Future[*roachpb.Error]) chan *roachpb.Error {
	ch := make(chan *roachpb.Error, 1)
	f.WhenReady(func(pErr *roachpb.Error, err error) {
		if err != nil {
...
func BenchmarkProcessorWithBudget(b *testing.B) {
	...
	if p.reg.Len() == 0 {
		...
		require.NoError(b, pErr.GoError())
	}
}
