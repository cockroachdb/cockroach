...
func TestUnbufferedRegOnConcurrentDisconnect(t *testing.T) {
	...
	for id := int64(0); id < 50; id++ {
		...
		go func(id int64) {
			...
			stream := sm.NewStream(id, r1, done)
	...
	for id := int64(0); id < 50; id++ {
		...
		go func(id int64) {
			defer wg.Done()
			sm.SendBufferedError(makeMuxRangefeedErrorEvent(id, r1, kvpb.NewError(nil)))
		}(id)
	}
	wg.Wait()
...
func TestUnbufferedRegOnDisconnect(t *testing.T) {
	...
	p.Register(ctx, h.span, startTs, makeCatchUpIterator(catchUpIter, span, startTs), /* catchUpIter */
		true /* withDiff */, false /* withFiltering */, false /* withOmitRemote */, sink)
