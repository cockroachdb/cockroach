...
func TestUnbufferedRegOnConcurrentDisconnect(t *testing.T) {
	...
	defer func() {
		...
		require.NoError(t, testutils.SucceedsWithinError(func() error {
			...
			return errors.Newf("expected 0, got %d registrations", p.Len())
		}, 100*time.Second))
	}()
	...
	for id := int64(0); id < 50; id++ {
		...
		go func(id int64) {
			defer wg.Done()
			_, d, _ := p.Register(ctx, h.span, hlc.Timestamp{}, nil, /* catchUpIter */
	...
	wg.Wait()
	testutils.SucceedsSoon(t, func() error {
		if testRangefeedCounter.get() == 0 {
			return nil
		}
		return errors.Newf("expected 0, got %d", testRangefeedCounter.get())
	})
}
...
func TestUnbufferedRegOnDisconnect(t *testing.T) {
	...
	_, d, _ := p.Register(ctx, h.span, startTs, makeCatchUpIterator(catchUpIter, span, startTs), /* catchUpIter */
...
func TestUnbufferedRegistration(t *testing.T) {
	...
	t.Run("catch up scan + publish updates correctness testing", func(t *testing.T) {
		...
		testutils.SucceedsSoon(t, func() error {
			s.mu.Lock()
			defer s.mu.Unlock()
			if len(s.mu.events) == capOfBuf+len(catchUpEvents) {
				require.Equal(t, []*kvpb.RangeFeedEvent{ev1, ev2, ev3, ev4, ev5}, s.mu.events[len(catchUpEvents):])
				return nil
			}
			return errors.Newf("expected %d events, got %d", capOfBuf+len(catchUpEvents), len(s.mu.events))
		})
	})
}
