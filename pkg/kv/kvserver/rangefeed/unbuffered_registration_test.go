...
// TestStreamMuxerWithUnbufferedRegOnConcurrentDisconnect tests that StreamMuxer
// can handle concurrent stream disconnects.
func TestStreamMuxerWithUnbufferedRegOnConcurrentDisconnect(t *testing.T) {
...
// TestStreamMuxerWithUnbufferedRegOnDisconnect tests that StreamMuxer can
// handle disconnects properly and send events in the correct order.
func TestStreamMuxerWithUnbufferedRegOnDisconnect(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)
	...
	p.Register(h.span, startTs, makeCatchUpIterator(catchUpIter, span, startTs), /* catchUpIter */
		...
		NewTestPerRangeEventSink(ctx, s1, r1, muxer, unbuffered), func() {})
