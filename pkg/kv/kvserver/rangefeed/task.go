...
func (a *txnPushAttempt) pushOldTxns(ctx context.Context) error {
	...
	for i, txn := range pushedTxns {
		...
		case roachpb.COMMITTED:
			...
			// the resolution.
			txnIntents := intentsInSpan(txn, a.p.Span.AsRawSpanWithNoLocals())
		...
		case roachpb.ABORTED:
			...
			// the intents within the processor's range as an optimization to help
			// others and to prevent any rangefeed reconnections from needing to push
			// the same txn. If we aborted the txn, then it won't have its LockSpans
...
// intentsInSpan returns LockUpdates for the provided transaction's LockSpans
