...
// DisableTxnPushBarrier is an escape hatch to disable the txn push barrier
// command in case it causes unexpected problems. This can result in violations
// of the rangefeed checkpoint guarantee, emitting premature checkpoints before
// all writes below it have been emitted.
//
// TODO(erikgrinaker): remove this once we're confident it doesn't cause issues.
var DisableTxnPushBarrier = envutil.EnvOrDefaultBool(
	"COCKROACH_RANGEFEED_DISABLE_TXN_PUSH_BARRIER", false)
...
func (a *txnPushAttempt) pushOldTxns(ctx context.Context) error {
	...
	if anyAmbiguousAbort && !DisableTxnPushBarrier {
