	...
	for {
		...
		if hasRange {
			if rangeBounds := i.RangeBounds(); !rangeBounds.Key.Equal(rangeKeysStart) {
				// Clear scratch buffer, we'll populate it only when we need to. (This optimizes
				// for the case in which there are few range tombstones visible)
				rangeKeysStart = rangeKeysStart[:0]
				...
				for j := len(versions) - 1; j >= 0; j-- {
					if !i.startTime.Less(versions[j].Timestamp) {
						// This range tombstone isn't visible by this catch-up scan.
						continue
					}

					if len(rangeKeysStart) == 0 {
