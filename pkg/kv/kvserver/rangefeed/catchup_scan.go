...
func (i *CatchUpIterator) CatchUpScan(outputFn outputEventFn, withDiff bool) error {
	...
	for {
		...
		// Next(Key)IgnoringTime() call moved onto an MVCC range tombstone outside
		// of the range bounds. In this case, HasPointAndRange() will return
		// false,false and we step forward.
