	...
	case streamActive:
		if bs.queueMu.perStreamCapacity > 0 && status.queueItems == bs.queueMu.perStreamCapacity {
			if ev.Error != nil {
				...
				//
				// TODO(review): We could alternatively put the error event onto the
				// queue right here and mark the stream as overflowed rather than have
				// this three state approach. The stream would still send us another
				// copy of it. My current thinking is that despite the complexity there
				// are two nice reasons for this three state approach:
				//
				// (1) It allows us to have the assertion above that the very next event
				// we see is the error, confirming our understanding of the system and
				//
				// (2) It means that we don't run the OnError() callback for this stream
				// until the registration initiates it. This allows us to put logic in
				// that on error handler that can assume the registration is
				// disconnected by the time it is being called.
