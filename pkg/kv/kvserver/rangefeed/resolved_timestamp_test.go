...
func TestClosedTimestampLogicalPark(t *testing.T) {
	defer leaktest.AfterTest(t)()
	...
	require.True(t, fwd)
	require.Equal(t, hlc.Timestamp{WallTime: 10, Logical: 2}, rts.Get())
	...
	// Set a new closed timestamp. Resolved timestamp advances, but only up to
	// just below txn1.
	fwd = rts.ForwardClosedTS(hlc.Timestamp{WallTime: 10, Logical: 6})
	require.True(t, fwd)
	require.Equal(t, hlc.Timestamp{WallTime: 10, Logical: 3}, rts.Get())

	// Abort txn1. Resolved timestamp advances.
	fwd = rts.ConsumeLogicalOp(abortTxnOp(txn1))
	require.True(t, fwd)
	require.Equal(t, hlc.Timestamp{WallTime: 10, Logical: 6}, rts.Get())

	// Create an intent at 11.0. Resolved timestamp doesn't advance. This tests
	// the case where the closed timestamp has a logical part, and an intent is in
	// the next wall tick; this used to cause an issue because of the rounding logic.
	txn2 := uuid.MakeV4()
	fwd = rts.ConsumeLogicalOp(writeIntentOp(txn2, hlc.Timestamp{WallTime: 11}))
	require.False(t, fwd)
	require.Equal(t, hlc.Timestamp{WallTime: 10, Logical: 6}, rts.Get())
}
