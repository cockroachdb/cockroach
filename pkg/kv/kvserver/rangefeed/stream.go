...
func NewSingleFeedStream(
	ctx context.Context, done chan *kvpb.Error, stream kvpb.RangeFeedEventSink,
) *SingleFeedStream {
...
func (s *SingleFeedStream) Error(err *kvpb.Error) {
	...
	s.stop.Do(func() {
		...
		case s.err <- err:
		default:
			// We don't want to block here just in case receiver gave up already.
			// TODO(oleg): to make this more robust we should check on <-ctx.Done()
			// but this is currently breaking tests that cancel feed and then
			// expect error to be posted.
		}
	})
}
...
func NewSingleBufferedStream(
	ctx context.Context,
	output kvpb.RangeFeedEventSink,
	capacity int,
	blockWhenFull bool,
	done func(),
	...
	return &SingleBufferedStream{
		ctx:           ctx,
		buffer:        make(chan *sharedEvent, capacity),
...
func (s *SingleBufferedStream) Send(event *kvpb.RangeFeedEvent, alloc *SharedBudgetAllocation) {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	default:
		if s.blockWhenFull {
			s.mu.Unlock()
			// In tests, we defer to blocking indefinitely to measure throughput.
			// We must also unlock to avoid interfering with Done()
			s.buffer <- e
			s.mu.Lock()
			return
		}
