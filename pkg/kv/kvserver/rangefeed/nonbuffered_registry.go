...
func newNonBufferedRegistration(
	...
	var catchUpBuf chan *sharedEvent
	if catchUpIter == nil {
		close(catchupDrained)
	} else {
		catchUpBuf = make(chan *sharedEvent, catchUpBufferSize)
	}
	r := &nonBufferedRegistration{
		...
		metrics:          metrics,
		stream:           stream,
		catchUpDrained:   catchupDrained,
	}
	r.mu.catchUpBuf = catchUpBuf
	return r
}
...
func (r *nonBufferedRegistration) setupCatchupScan(ctx context.Context) (
...
func (r *nonBufferedRegistration) publish(
	...
	// If true then we've overflown here, but we still need to wait for callback
	// to fire that registration is drained.
...
// abortAndDisconnect disposes of registration that didn't start yet.
...
func (r *nonBufferedRegistration) abortAndDisconnect(pErr *kvpb.Error) {
	if r.catchUpIter != nil {
		r.catchUpIter.Close()
		close(r.catchUpDrained)
	}
...
func (r *nonBufferedRegistration) disconnect(pErr *kvpb.Error) {
	// We don't have proper context for cleanup here and it needs to be passed
	// through all involved parties (e.g. node, replica, processor).
	r.cleanup()
...
func (r *nonBufferedRegistration) cleanup() {
	defer func() {
		// If we cancelled catch up context, then we need to wait for it to finish
