...
func TestClosedTimestampInactiveAfterSubsumption(t *testing.T) {
	...
	t.Run("without lease transfer", func(t *testing.T) {
		...
		postSubsumptionTimestamp := getCurrentMaxClosed(t, tc, leaseholder, rightDesc).ClosedTimestamp
		if postSubsumptionTimestamp.IsEmpty() {
			t.Fatal("could not find a closed timestamp update for RHS leaseholder")
		}
		postSubsumptionTimestamp = postSubsumptionTimestamp.Next()
	...
	t.Run("with intervening lease transfer", func(t *testing.T) {
		...
		// We now have the RHS in its subsumed state.
		freezeStartTimestamp := (<-blockMergeTrigger).(hlc.Timestamp)
		leaseholder := getCurrentLeaseholder(t, tc, rightDesc)
		...
		case entry := <-closedTimestampCh:
			...
			t.Fatalf("saw a closed timestamp update for the subsumed RHS range after a lease transfer: %+v",
				entry)
		case <-time.After(testingTargetDuration):
		}
		rightReplFollowers := getFollowerReplicas(ctx, t, tc, rightDesc, leaseholder)
...
func suspendMergeTrigger(
	suspendMergeTrigger chan<- interface{}, finishMergeTxn <-chan struct{},
	...
	return func(_ context.Context, ba roachpb.BatchRequest) *roachpb.Error {
		for _, req := range ba.Requests {
			if et := req.GetEndTxn(); et != nil && et.InternalCommitTrigger.GetMergeTrigger() != nil {
				...
				suspendMergeTrigger <- et.InternalCommitTrigger.MergeTrigger.FreezeStart
				<-finishMergeTxn
			}
		}
		return nil
