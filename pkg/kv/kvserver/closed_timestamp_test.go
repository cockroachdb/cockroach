...
func TestClosedTimestampInactiveAfterSubsumption(t *testing.T) {
	...
	t.Run("without lease transfer", func(t *testing.T) {
		...
		st := mergeFilterState{blockMergeTrigger: blockMergeTrigger, finishMergeTxn: finishMergeTxn}
		...
		g.Go(func() error {
			return beginMergeTxn(tc, leftDesc)
		})
		defer func() {
		...
		case <-time.After(30 * time.Second):
			t.Fatal("failed to receive closed timestamp update")
		}
	...
	t.Run("with intervening lease transfer", func(t *testing.T) {
		...
		case storeID := <-leaseAcquisitionCh:
			require.Equal(t, storeID, newRightLeaseholder.StoreID(),
				"expected store %d to try to acquire the lease; got a request from store %d instead",
				newRightLeaseholder.StoreID(), storeID)
		case <-time.After(30 * time.Second):
			t.Fatal("failed to receive lease acquisition request")
		}
		...
		case <-closedTimestampCh:
		case <-time.After(30 * time.Second):
			t.Fatal("failed to receive initial closed timestamp update after lease transfer")
		}
...
func beginMergeTxn(tc serverutils.TestClusterInterface, leftDesc roachpb.RangeDescriptor) error {
	ctx := context.Background()
