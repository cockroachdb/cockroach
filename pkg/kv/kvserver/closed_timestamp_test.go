...
func setupClusterForClosedTsTesting(
	...
	tc, desc := setupTestClusterWithDummyRange(t, clusterArgs, dbName, numNodes, func(db *gosql.DB) error {
		return enableFollowerReadsForTesting(db, targetDuration, closeFraction)
	})
...
CREATE DATABASE %[1]s;
CREATE TABLE %[1]s.kv (id INT PRIMARY KEY, value STRING);
...
`, dbName)); err != nil {
	...
	if clusterArgs.ReplicationMode != base.ReplicationManual {
		testutils.SucceedsSoon(t, func() error {
			if err := db0.QueryRow(
				fmt.Sprintf(`SELECT range_id, start_key, array_length(replicas, 1) FROM crdb_internal.ranges WHERE table_name = 'kv' AND database_name = '%s'`, dbName),
