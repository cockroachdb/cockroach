...
func TestClosedTimestampCanServe(t *testing.T) {
	...
	testutils.RunTrueAndFalse(t, "withNonVoters", func(t *testing.T, withNonVoters bool) {
		...
		// that same timestamp.
		{
			...
			txn := roachpb.MakeTransaction("testwrite", r.Key, isolation.Serializable, roachpb.NormalUserPriority, ts, 100, int32(tc.Server(0).SQLInstanceID()), 0, false)
			baWrite.Txn = &txn
			baWrite.Add(r)
...
func TestClosedTimestampCantServeWithConflictingIntent(t *testing.T) {
	...
	txn := roachpb.MakeTransaction("txn", txnKey, 0, 0, tc.Server(0).Clock().Now(), 0, int32(tc.Server(0).SQLInstanceID()), 0, false)
...
func makeTxnReadBatchForDesc(desc roachpb.RangeDescriptor, ts hlc.Timestamp) *kvpb.BatchRequest {
	txn := roachpb.MakeTransaction("txn", nil, 0, 0, ts, 0, 0, 0, false)
