...
// the {dbname}.{tableName} table and performs splits on the table's range such
// that the 2 resulting ranges contain exactly one of the rows each.
...
// setupClusterForClosedTsTesting creates a test cluster that is prepared
...
// 0, a range descriptor for the range used by the table `{dbName}.kv`. It is
// the caller's responsibility to Stop the Stopper on the returned test cluster
// when done.
func setupClusterForClosedTsTesting(ctx context.Context, t *testing.T, targetDuration time.Duration, closeFraction float64, clusterArgs base.TestClusterArgs, dbName, tableName string) (tc serverutils.TestClusterInterface, db0 *gosql.DB, kvTableDesc roachpb.RangeDescriptor) {
...
type postSetupCallback func(db *gosql.DB) error

// setupTestClusterWithDummyRange creates a TestCluster with an empty table
// called `{dbname}.kv` and returns a handle to the range descriptor
