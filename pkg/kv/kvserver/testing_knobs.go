...
type StoreTestingKnobs struct {
	...
	// AfterRaftLogSyncThrottle is invoked after completion of a synced write to
	...
	//
	// If async log writes are disabled, blocks the entire store's raft processing
	// while also holding raftMu for the replica. Not recommended to use this way.
	AfterRaftLogSyncThrottle func(storage.FullReplicaID)
	...
	// TestingPostSideEffectsFilter is called after a command is applied to state
	// machine, and its side effects are applied to memory. Called on each replica
	// that applies the command.
	//
	// NB: not all fields are passed in to this callback, see the implementation.
	TestingPostSideEffectsFilter kvserverbase.ReplicaApplyFilter
	...
	// when possible, and configured so by the cluster settings. If false, some
	// asynchronous log writes can be randomly made synchronous in tests. Should
	// be set to true by tests that specifically need the asynchronous log writes
	// behaviour.
