...
func TestCalcRangeCounterLeaseHolder(t *testing.T) {
	...
	testcases := []struct {
		...
		{"leaseholder is counter", 30, leaseStatus, []roachpb.NodeID{1, 2, 3, 4}, true},
		{"invalid leaseholder is counter", 30, leaseStatusInvalid, []roachpb.NodeID{1, 2, 3, 4}, true},
		{"non-leaseholder is not counter", 10, leaseStatus, []roachpb.NodeID{1, 2, 3, 4}, false},
		{"non-leaseholder not counter with invalid lease", 10, leaseStatusInvalid, []roachpb.NodeID{1, 2, 3, 4}, false},
		{"unavailable leaseholder is not counter", 30, leaseStatus, []roachpb.NodeID{1, 2, 4}, false},
		{"first is counter with unavailable leaseholder", 10, leaseStatus, []roachpb.NodeID{1, 2, 4}, true},
		{"other is not counter with unavailable leaseholder", 20, leaseStatus, []roachpb.NodeID{1, 2, 4}, false},
		{"non-voter can be counter", 20, leaseStatus, []roachpb.NodeID{2, 4}, true},
	}
