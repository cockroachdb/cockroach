...
func addLockTableSpans(spans *SpanSet) *SpanSet {
	...
	spans.Iterate(func(sa SpanAccess, _ SpanScope, span Span) {
		// Some commands (e.g. TransferLease) declare access to the entire key
		// space, or very large chunks of it. Even though this could be unsafe, we
		// allow this by ignoring spans that contain the entire lock table, because
