...
type ReadWriterBatch struct {
	spanSetReader
	spanSetWriteBatch
}

var _ storage.ReadWriter = ReadWriterBatch{}

// Close implements storage.ReadWriter (resolves ambiguity between Reader.Close
// and WriteBatch.Close).
func (rwb ReadWriterBatch) Close() {
	rwb.wb.Close()
}

func makeSpanSetReadWriterBatch(
	rw storage.ReadWriter, batch storage.WriteBatch, spans *SpanSet,
) ReadWriterBatch {
	spans = addLockTableSpans(spans)
	return ReadWriterBatch{
		spanSetReader: spanSetReader{r: rw, spans: spans, spansOnly: true},
...
func makeSpanSetReadWriterBatchAt(
	rw storage.ReadWriter, batch storage.WriteBatch, spans *SpanSet, ts hlc.Timestamp,
) ReadWriterBatch {
	spans = addLockTableSpans(spans)
	return ReadWriterBatch{
		spanSetReader: spanSetReader{r: rw, spans: spans, ts: ts},
		spanSetWriteBatch: spanSetWriteBatch{
			spanSetWriter: spanSetWriter{w: batch, spans: spans, ts: ts},
			wb:            batch,
		},
	}
}
...
func (s spanSetBatch) shallowCopy() *spanSetBatch {
	...
	b.spanSetWriteBatch.spanSetWriter.spans = b.spanSetWriteBatch.spanSetWriter.spans.ShallowCopy()
...
func NewBatch(b storage.Batch, spans *SpanSet) storage.Batch {
	return &spanSetBatch{
		ReadWriterBatch: makeSpanSetReadWriterBatch(b, b, spans),
		b:               b,
		spans:           spans,
		spansOnly:       true,
	}
...
func NewBatchAt(b storage.Batch, spans *SpanSet, ts hlc.Timestamp) storage.Batch {
	return &spanSetBatch{
		ReadWriterBatch: makeSpanSetReadWriterBatchAt(b, b, spans, ts),
		b:               b,
		spans:           spans,
		ts:              ts,
	}
...
func DisableReadWriterAssertions(rw storage.ReadWriter) storage.ReadWriter {
	...
	case ReadWriterBatch:
		return DisableReadWriterAssertions(v.spanSetWriteBatch.w.(storage.ReadWriter))
	case *spanSetBatch:
...
func DisableUndeclaredSpanAssertions(rw storage.ReadWriter) storage.ReadWriter {
	...
	case *spanSetBatch:
		...
		newSnapSetBatch.spanSetWriteBatch.spanSetWriter.spans.DisableUndeclaredAccessAssertions()
...
func DisableForbiddenSpanAssertions(rw storage.ReadWriter) storage.ReadWriter {
	...
	case *spanSetBatch:
		...
		newSnapSetBatch.spanSetWriteBatch.spanSetWriter.spans.DisableForbiddenSpansAssertions()
		return newSnapSetBatch
