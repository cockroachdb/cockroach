...
// QueryResolvedTimestampMaxEncounteredIntents configures the maximum number of
...
var QueryResolvedTimestampMaxEncounteredIntents = settings.RegisterIntSetting(
	"kv.query_resolved_timestamp.max_encountered_intents",
	"maximum number of intents returned for async intent cleanup by a single QueryResolvedTimestamp request",
	1024)
...
func QueryResolvedTimestamp(
	...
	maxEncounteredIntents := QueryResolvedTimestampMaxEncounteredIntents.Get(&st.SV)
...
func computeMinIntentTimestamp(
	...
	for iter.SeekGE(storage.MakeMVCCMetadataKey(span.Key)); ; iter.NextKey() {
		...
		if meta.Txn != nil {
			...
			if oldEnough && len(encountered) < maxEncounteredIntents {
				encountered = append(encountered, roachpb.MakeIntent(meta.Txn, iter.Key().Key))
			}
		}
	}
