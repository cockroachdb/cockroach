...
func TestEvalAddSSTable(t *testing.T) {
	...
	testcases := map[string]struct {
		...
		sst            []sstutil.KV
		reqTS          int64
		toReqTS        int64 // UpdateToRequestTimestamp with given SST timestamp
		...
		"UpdateToRequestTimestamp rewrites timestamp": {
			reqTS:          10,
			toReqTS:        1,
		...
		"UpdateToRequestTimestamp writes tombstones unless race": { // unfortunately, for performance
			reqTS:          10,
			toReqTS:        1,
			sst:            []sstutil.KV{{"a", 1, ""}},
		...
		"UpdateToRequestTimestamp rejects incorrect SST timestamp": {
			reqTS:   10,
			toReqTS: 1,
		...
		"UpdateToRequestTimestamp rejects incorrect 0 SST timestamp": {
			reqTS:   10,
			toReqTS: 1,
		...
		"UpdateToRequestTimestamp writes below and replaces": {
			reqTS:          5,
			toReqTS:        1,
		...
		"UpdateToRequestTimestamp returns WriteIntentError for intents": {
			reqTS:     10,
			toReqTS:   1,
		...
		"UpdateToRequestTimestamp errors with DisallowConflicts below existing": {
			reqTS:      5,
			toReqTS:    10,
			noConflict: true,
			data:       []sstutil.KV{{"a", 5, "a5"}, {"b", 7, "b7"}},
		...
		"UpdateToRequestTimestamp succeeds with DisallowConflicts above existing": {
			reqTS:      8,
			toReqTS:    1,
			noConflict: true,
			data:       []sstutil.KV{{"a", 5, "a5"}, {"b", 7, "b7"}},
		...
		"UpdateToRequestTimestamp errors with DisallowShadowing below existing": {
			reqTS:     5,
			toReqTS:   10,
			noShadow:  true,
			data:      []sstutil.KV{{"a", 5, "a5"}, {"b", 7, "b7"}},
		...
		"UpdateToRequestTimestamp errors with DisallowShadowing above existing": {
			reqTS:     8,
			toReqTS:   1,
			noShadow:  true,
			data:      []sstutil.KV{{"a", 5, "a5"}, {"b", 7, "b7"}},
		...
		"UpdateToRequestTimestamp succeeds with DisallowShadowing above tombstones": {
			reqTS:    8,
			toReqTS:  1,
			noShadow: true,
		...
		"UpdateToRequestTimestamp succeeds with DisallowShadowing and idempotent writes": {
			reqTS:    5,
			toReqTS:  1,
			noShadow: true,
		...
		"UpdateToRequestTimestamp errors with DisallowShadowingBelow equal value above existing below limit": {
			reqTS:         7,
			toReqTS:       10,
			noShadowBelow: 5,
		...
		"UpdateToRequestTimestamp errors with DisallowShadowingBelow errors above existing above limit": {
			reqTS:         7,
			toReqTS:       10,
			noShadowBelow: 5,
		...
		"UpdateToRequestTimestamp allows DisallowShadowingBelow equal value above existing above limit": {
			reqTS:         7,
			toReqTS:       10,
			noShadowBelow: 5,
			data:          []sstutil.KV{{"a", 6, "a6"}},
				...
				t.Run(name, func(t *testing.T) {
					...
					result, err := batcheval.EvalAddSSTable(ctx, engine, batcheval.CommandArgs{
						...
						Args: &roachpb.AddSSTableRequest{
							...
							UpdateToRequestTimestamp: hlc.Timestamp{WallTime: tc.toReqTS},
...
func TestEvalAddSSTableRangefeed(t *testing.T) {
	...
	testcases := map[string]struct {
		sst                   []sstutil.KV
		toReqTS               int64 // UpdateToRequestTimestamp
		...
		"UpdateToRequestTimestamp alone": {
			sst:                   []sstutil.KV{{"a", 1, "a1"}},
			toReqTS:               1,
		...
		"IngestAsWrites and UpdateToRequestTimestamp": {
			sst:                   []sstutil.KV{{"a", 1, "a1"}, {"b", 1, "b1"}},
			asWrites:              true,
			toReqTS:               1,
			expectHistoryMutation: false,
...
// TestAddSSTableUpdateToRequestTimestampRespectsTSCache checks that AddSSTable
// with UpdateToRequestTimestamp respects the timestamp cache.
func TestAddSSTableUpdateToRequestTimestampRespectsTSCache(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)
	...
	sstReq := &roachpb.AddSSTableRequest{
		...
		Data:                     sst,
		MVCCStats:                sstutil.ComputeStats(t, sst),
		UpdateToRequestTimestamp: hlc.Timestamp{WallTime: 1},
	}
	ba := roachpb.BatchRequest{
...
// TestAddSSTableUpdateToRequestTimestampRespectsClosedTS checks that AddSSTable
// with UpdateToRequestTimestamp respects the closed timestamp.
func TestAddSSTableUpdateToRequestTimestampRespectsClosedTS(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)
