...
func TestAddSSTableBlindWriteBelow(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)
	...
	// Write and read foo.
	txn := db.NewTxn(ctx, "txn")
	require.NoError(t, txn.Put(ctx, "foo", "txn"))
	require.NoError(t, txn.Commit(ctx))
	txnTS := txn.CommitTimestamp()

	// Add an SST writing to foo below the previous write.
	sst, start, end := makeSST(t, txnTS, map[string]string{"foo": "sst"})
	stats := sstStats(t, sst)
	ba := roachpb.BatchRequest{
		Header: roachpb.Header{
			Timestamp: txnTS.Prev(),
		},
	}
	ba.Add(&roachpb.AddSSTableRequest{
		RequestHeader: roachpb.RequestHeader{
	...
	// Reading back foo should get the value from the txn, because the tscache
	// does not apply to writes.
	kv, err := db.Get(ctx, "foo")
	require.NoError(t, err)
	require.Equal(t, "txn", string(kv.ValueBytes()))
}

func TestAddSSTableBlindRespectsTSCache(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)
	...
	// Reading back foo should get the value from the SST. Even though it was
	// submitted below the txn write, the tsCache pushes it forward because of the
	// write.
	kv, err := db.Get(ctx, "foo")
	require.NoError(t, err)
