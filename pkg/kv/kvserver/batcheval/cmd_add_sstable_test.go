...
func TestEvalAddSSTable(t *testing.T) {
	...
	testcases := map[string]struct {
		...
		noConflicts    bool  // DisallowConflicts
		noShadowing    bool  // DisallowShadowing
		expect         []mvccKV
	...
	testutils.RunTrueAndFalse(t, "IngestAsWrites", func(t *testing.T, ingestAsWrites bool) {
		for name, tc := range testcases {
			t.Run(name, func(t *testing.T) {
				...
				dir := t.TempDir()
				engine, err := storage.Open(ctx, storage.Filesystem(filepath.Join(dir, "pebble")))
				...
				result, err := batcheval.EvalAddSSTable(ctx, engine, batcheval.CommandArgs{
					...
					Args: &roachpb.AddSSTableRequest{
						...
						DisallowConflicts:       tc.noConflicts,
						DisallowShadowing:       tc.noShadowing,
				...
				if ingestAsWrites {
					...
					sstPath := filepath.Join(t.TempDir(), "sst")
					require.NoError(t, ioutil.WriteFile(sstPath, result.Replicated.AddSSTable.Data, 0755))
				...
				for {
					...
					if iter.Key().IsValue() {
						...
						require.NoError(t, meta.Unmarshal(iter.UnsafeValue()))
...
func runTestDBAddSSTable(
...

	{
		...
		err := db.AddSSTable(
			ctx, "d", "e", sst, allowConflicts, allowShadowing, nilStats, ingestAsSST, noTS, writeAtSST)
		require.Error(t, err)
		require.Contains(t, err.Error(), "not in request range")

		// Key is after the range in the request span.
		err = db.AddSSTable(
			ctx, "a", "b", sst, allowConflicts, allowShadowing, nilStats, ingestAsSST, noTS, writeAtSST)
		require.Error(t, err)
		require.Contains(t, err.Error(), "not in request range")
		...
		require.NoError(t, db.AddSSTable(
			ingestCtx, start, end, sst, allowConflicts, allowShadowing, nilStats, ingestAsSST, noTS, writeAtSST))
		trace := collect().String()
	...
	// the value returned by Get.
	{
		...
		require.NoError(t, db.AddSSTable(
			ctx, start, end, sst, allowConflicts, allowShadowing, nilStats, ingestAsSST, noTS, writeAtSST))
		r, err := db.Get(ctx, "bb")
		require.NoError(t, err)
		require.Equal(t, []byte("1"), r.ValueBytes())
		if store != nil {
	...
	// ... and doing the same thing but via write-batch works the same.
	{
		...
		for i := 0; i < 2; i++ {
			...
			require.NoError(t, db.AddSSTable(
				ingestCtx, start, end, sst, allowConflicts, allowShadowing, nilStats, ingestAsWrites, noTS, writeAtSST))
			trace := collect().String()
			require.Contains(t, trace, "evaluating AddSSTable")
	...
	// Invalid key/value entry checksum.
	{
		...
		err := db.AddSSTable(
			ctx, "b", "c", sstFile.Data(), allowConflicts, allowShadowing, nilStats, ingestAsSST, noTS, writeAtSST)
		require.Error(t, err)
...
func TestAddSSTableMVCCStats(t *testing.T) {
	...
	require.NoError(t, engine.WriteFile(path, sst))
	require.NoError(t, engine.IngestExternalFiles(ctx, []string{path}))
