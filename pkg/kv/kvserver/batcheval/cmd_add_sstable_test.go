		...
		testutils.RunValues(t, "RewriteConcurrency", []interface{}{0, 8}, func(t *testing.T, c interface{}) {
			for name, tc := range testcases {
				t.Run(name, func(t *testing.T) {
					...
					sst, start, end := sstutil.MakeSST(t, ctx, st, tc.sst)
...
func runTestDBAddSSTable(
	...
	{
		sst, start, end := sstutil.MakeSST(t, ctx, cs, []sstutil.KV{{"bb", 2, "1"}})
	...
	{
		sst, start, end := sstutil.MakeSST(t, ctx, cs, []sstutil.KV{{"bb", 1, "2"}})
		require.NoError(t, db.AddSSTable(
	...
	{
		sst, start, end := sstutil.MakeSST(t, ctx, cs, []sstutil.KV{{"bc", 1, "3"}})
	...
	{
		sst, start, end := sstutil.MakeSST(t, ctx, cs, []sstutil.KV{{"bd", 1, "3"}})
...
func TestAddSSTableMVCCStats(t *testing.T) {
	...
	sst, start, end := sstutil.MakeSST(t, ctx, st, []sstutil.KV{
	...
	sst, start, end = sstutil.MakeSST(t, ctx, st, []sstutil.KV{{"zzzzzzz", ts.WallTime, "zzz"}})
	cArgs = batcheval.CommandArgs{
		EvalCtx: evalCtx,
...
func TestAddSSTableMVCCStatsDisallowShadowing(t *testing.T) {
	...
	sst, start, end := sstutil.MakeSST(t, ctx, st, kvs)
...
func TestAddSSTableIntentResolution(t *testing.T) {
	...
	sst, start, end := sstutil.MakeSST(t, ctx, s.ClusterSettings(), []sstutil.KV{
		{"a", 1, "1"},
		{"b", 1, "2"},
		{"c", 1, "3"},
	})
...
func TestAddSSTableSSTTimestampToRequestTimestampRespectsTSCache(t *testing.T) {
	...
	sst, start, end := sstutil.MakeSST(t, ctx, s.ClusterSettings(), []sstutil.KV{{"key", 1, "sst"}})
...
func TestAddSSTableSSTTimestampToRequestTimestampRespectsClosedTS(t *testing.T) {
	...
	reqTS := closedTS.Prev()
	sst, start, end := sstutil.MakeSST(t, ctx, store.ClusterSettings(), []sstutil.KV{{"key", 1, "sst"}})
	sstReq := &roachpb.AddSSTableRequest{
		RequestHeader:                  roachpb.RequestHeader{Key: start, EndKey: end},
		Data:                           sst,
		MVCCStats:                      sstutil.ComputeStats(t, sst),
