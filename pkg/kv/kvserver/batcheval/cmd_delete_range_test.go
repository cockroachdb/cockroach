...
func TestDeleteRangeTombstone(t *testing.T) {
	...
	writeInitialData := func(t *testing.T, ctx context.Context, rw storage.ReadWriter) {
		...
		txn := roachpb.MakeTransaction("test", nil /* baseKey */, isolation.Serializable, roachpb.NormalUserPriority, hlc.Timestamp{WallTime: 5e9}, 0, 0, 0, false)
		...
		t.Run(name, func(t *testing.T) {
			for _, runWithPredicates := range []bool{false, true} {
				...
				t.Run(fmt.Sprintf("Predicates=%v", runWithPredicates), func(t *testing.T) {
					...
					if tc.txn {
						txn := roachpb.MakeTransaction("txn", nil, isolation.Serializable, roachpb.NormalUserPriority, rangeKey.Timestamp, 0, 0, 0, false)
						h.Txn = &txn
					}
