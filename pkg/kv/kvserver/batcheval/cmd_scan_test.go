...
func testScanReverseScanInner(
	...
	for _, k := range []roachpb.Key{k1, k2} {
		err := storage.MVCCPut(ctx, eng, nil, k, ts, hlc.ClockTimestamp{}, roachpb.MakeValueFromString("value-"+string(k)), storage.NoLogicalReplication, nil)
...
func TestScanReverseScanWholeRows(t *testing.T) {
	...
	for r := 0; r < 2; r++ {
		for cf := uint32(0); cf < 3; cf++ {
			...
			err := storage.MVCCPut(ctx, eng, nil, key, ts, hlc.ClockTimestamp{}, roachpb.MakeValueFromString("value"), storage.NoLogicalReplication, nil)
			require.NoError(t, err)
