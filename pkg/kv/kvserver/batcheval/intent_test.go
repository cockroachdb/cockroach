	...
	for _, c := range testCases {
		t.Run(c.name, func(t *testing.T) {
			...
			testutils.RunTrueAndFalse(t, "deletion intent", func(t *testing.T, delete bool) {
				...
				var err error
				if delete {
					_, err = storage.MVCCDelete(ctx, db, nil, key, ts, hlc.ClockTimestamp{}, storage.NoLogicalReplication, &txn)
				} else {
					err = storage.MVCCPut(ctx, db, nil, key, ts, hlc.ClockTimestamp{}, val, storage.NoLogicalReplication, &txn)
				}
