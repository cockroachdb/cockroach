...
func init() {
	migrationRegistry[cv.ByKey(cv.TruncatedAndRangeAppliedStateMigration)] = truncatedAndAppliedStateMigration
}
...
func Migrate(
	...
	fn, ok := migrationRegistry[args.TargetVersion]
	if !ok {
		return result.Result{}, errors.Newf("migration for %s not found", args.TargetVersion)
	}
