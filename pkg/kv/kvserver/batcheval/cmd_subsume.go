...
// non-empty Result. Both are necessary for SubsumeRequest to be replicated
// via Raft. The write is limited to updating the RangeForceFlushKey with a
// value corresponding to the index of the raft entry of the Subsume. Since
// this write is to a replicated Range-ID local key (and because it is done
// non-transactionally -- see kvserver.AdminMerge), it will not be rolled back
// if the distributed transaction doing the merge aborts. This is ok since the
// RangeForceFlushKey only provides a directive to the replication flow
// control v2 (RACv2) machinery to force-flush up to a particular Raft index.
// Additionally, safety relies on SubsumeRequest only writing to a Range-ID
// local key:
//
