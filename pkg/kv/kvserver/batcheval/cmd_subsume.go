...
func Subsume(
	...
	// If requested, we write out any unreplicated locks as replicated locks.
	// Note that in
	stats := cArgs.EvalCtx.GetMVCCStats()
	if args.PreserveUnreplicatedLocks {
		acquisitions := cArgs.EvalCtx.GetConcurrencyManager().OnRangeLeaseTransferEval()
	...
	// NB: This will get overwritten via RepopulateSubsumeResponse if
	...
	pd.Local.RepopulateSubsumeResponse = args.PreserveUnreplicatedLocks
	return pd, nil
}
