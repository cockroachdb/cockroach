...
func TestMergeGCRangeBoundaries(t *testing.T) {
	...
	preKey := func(k string) roachpb.Key {
		l, _ := storage.RangeTombstonePeekBounds(key(k), key(k+"zzzzzzz"), nil, nil)
		return l
	}
	postKey := func(k string) roachpb.Key {
		_, r := storage.RangeTombstonePeekBounds(key(""), key(k), nil, nil)
		return r
	}
	...
	for _, d := range []struct {
		...
		t.Run(d.name, func(t *testing.T) {
			spans := storage.MakeLookupBoundariesForGCRanges(d.rangeStart, d.rangeEnd, d.rangeKeys)
