...
func EvalAddSSTable(
	...
	if checkConflicts {
		...
		//
		// Additionally, if DisallowShadowing is set, it will not write above
		// existing/visible values (but will write above tombstones).
		statsDelta, err = storage.CheckSSTConflicts(
			ctx, sst, readWriter, start, end, args.DisallowShadowing, maxIntents)
		if err != nil {
		...
		intents, err := storage.ScanIntents(ctx, readWriter, start.Key, end.Key, maxIntents)
		if err != nil {
