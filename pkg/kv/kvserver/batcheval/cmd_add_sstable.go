...
func EvalAddSSTable(
	...
	if util.RaceEnabled {
		if err := assertSSTContents(sst, args.UpdateToRequestTimestamp, args.MVCCStats); err != nil {
	...
	if args.UpdateToRequestTimestamp.IsSet() && (h.Timestamp != args.UpdateToRequestTimestamp ||
		...
		sst, err = storage.UpdateSSTTimestamps(sst, args.UpdateToRequestTimestamp, h.Timestamp, conc)
		if err != nil {
	...
	if args.UpdateToRequestTimestamp.IsEmpty() {
	...
	if args.IngestAsWrites {
		...
		for {
			...
			if args.UpdateToRequestTimestamp.IsSet() {
	...
	return result.Result{
		Replicated: kvserverpb.ReplicatedEvalResult{
			AddSSTable: &kvserverpb.ReplicatedEvalResult_AddSSTable{
				...
				AtWriteTimestamp: args.UpdateToRequestTimestamp.IsSet(),
			},
			MVCCHistoryMutation: mvccHistoryMutation,
		},
	}, nil
...
func assertSSTContents(sst []byte, updateTimestamp hlc.Timestamp, stats *enginepb.MVCCStats) error {
	...
	for {
		...
		if updateTimestamp.IsSet() && key.Timestamp != updateTimestamp {
			return errors.AssertionFailedf("SST has unexpected timestamp %s (expected %s) for key %s",
				key.Timestamp, updateTimestamp, key.Key)
		}
		iter.Next()
	}
