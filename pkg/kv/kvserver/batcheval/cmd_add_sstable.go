...
func declareKeysAddSSTable(
	...
	// versions or write below existing versions and intents.
	//
	...
	// if the intent was aborted the request may succeed, but if it was committed
	// the request will fail. This still violates MVCC (it may write a key in the
	// past whose absence has already been observed by a reader) and the closed
	// timestamp (it may write a key below it).
	//
	...
	// declare lock spans over [Key,EndKey) in order to synchronize with
	// concurrent transactions and resolve any intents (via WriteIntentError).
	// This is particularly relevant for IMPORT INTO, which imports into an
	// offline table that may contain unresolved intents from previously aborted
	// transactions.
	//
