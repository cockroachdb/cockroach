...
func EvalAddSSTable(
	...
	// there is a range key in the incoming sst.
	//
	// TODO(msbutler): In the first iteration, this function will also throw an
	// error in the existing keyspace contains a rangekey, but this will be fixed.
	//
	...
	// of the range, it ensures a healthy allocator that makes good
	...
	if args.ComputeStatsDiff {
		...
		stats, err = storage.ComputeSSTStatsDiff(
			ctx, sst, readWriter, h.Now.WallTime, start, end)
		if err != nil {
			return result.Result{}, err
		}
