...
func RequestLease(
	...
	// We don't allow replicas of type LEARNER_EPHEMERAL to be leaseholders.
	// There's no reason this wouldn't work in principle, but it seems
	// inadvisable. In particular, learners can't become raft leaders, so we
	// wouldn't be able to co-locate the leaseholder + raft leader, which is going
	// to affect tail latencies. Ephemeral learner replicas are only used for a
	// short time in replica addition, so it's not worth working out the edge
	// cases.
	//
	// If this check is removed at some point, the filtering of ephemeral learners
	// on the sending side would have to be removed as well.
