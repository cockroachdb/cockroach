...
func rangeTombstoneReadBounds(
	...
	// We need to peek to the left for another range key that touches our start
	// key, so we need to extend the left-hand bound to cover it. However, we
	// can't know how long its end key is and we can't generate an infinitely long
	// key for an optimally tight bound. It seems fair to assume that most keys
	// are less than 8192 bytes, or at least that they have a fairly unique
	// 8192-byte prefix, so this bound shouldn't cover very many existing keys.
