...
func DeleteRange(
	...
	if args.UpdateDeleteRangeGCHint && !args.UseRangeTombstone {
	...
	if args.UseRangeTombstone {
		...
		maybeUpdateGCHint := func(res *result.Result) error {
			if !args.UpdateDeleteRangeGCHint || !cArgs.EvalCtx.ClusterSettings().Version.IsActive(ctx,
			...
			if !hint.UpdateLatestDeleteRangeTimestamp(h.Timestamp) {
				return nil
			}
