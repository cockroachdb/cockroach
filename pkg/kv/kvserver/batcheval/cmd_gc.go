...
func declareKeysGC(
	...
	if gcr.RangeKeys != nil {
		// When GC-ing MVCC range key tombstones or individual range keys, we need
		// to serialize with all range writes(deletes) that overlap the MVCC range
		// tombstone, as well as the immediate left/right neighboring keys. This is
		// because a range key write has non-local effects, i.e. it can fragment or
		// merge other range keys at other timestamps and at its boundaries, and
		// this has a non-commutative effect on MVCC stats -- if someone writes a
		// new range key while we're GCing one below, the stats would come out wrong.
		...
		// threshold).
		// To achieve this, we use special latch key that will prevent any range
		// write operations until GC request is done. All other range tombstone
		// operations must obtain a read latch on this key.
