...
func declareKeysGC(
	...
	// threshold). Currently, we will serialize with both readers and writers.
	//
	// TODO(oleg, aayush): These latches are very broad and will be disruptive to
	// read and write operations despite only accessing "stale" data. We need to
	// serialize with writers as explained above, but we can potentially work
	// around serializing with readers above the GC threshold by declaring these
	// read-write latches at hlc.MaxTimestamp which would not serialize with any
	// readers.
