...
func TestCmdRevertRange(t *testing.T) {
	...
	for i := 0; i < keyCount; i++ {
		...
		if err := storage.MVCCPut(ctx, eng, &stats, key, baseTime.Add(int64(i%10), 0), hlc.ClockTimestamp{}, value, storage.NoLogicalReplication, nil); err != nil {
			t.Fatal(err)
		}
	}
	...
	for i := 5; i < keyCount+5; i++ {
		...
		if err := storage.MVCCPut(ctx, eng, &stats, key, tsA.Add(int64(i%5), 1), hlc.ClockTimestamp{}, value, storage.NoLogicalReplication, nil); err != nil {
			t.Fatal(err)
		}
	}
	...
	for i := 7; i < keyCount+7; i++ {
		...
		if err := storage.MVCCPut(ctx, eng, &stats, key, tsB.Add(1, int32(i%5)), hlc.ClockTimestamp{}, value, storage.NoLogicalReplication, nil); err != nil {
			t.Fatal(err)
		}
	}
	...
	if err := storage.MVCCPut(ctx, eng, &stats, []byte("0012"), tsC, hlc.ClockTimestamp{}, roachpb.MakeValueFromBytes([]byte("i")), storage.NoLogicalReplication, &txn); err != nil {
		t.Fatal(err)
	}
	...
	for i := 7; i < keyCount+7; i += 2 {
		...
		var value roachpb.Value
		value.SetString(fmt.Sprintf("%d-rev-b", i))
		if err := storage.MVCCPut(ctx, eng, &stats, key, tsC.Add(10, int32(i%5)), hlc.ClockTimestamp{}, value, storage.NoLogicalReplication, nil); err != nil {
