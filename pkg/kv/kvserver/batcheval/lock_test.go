...
func TestTxnBoundReplicatedLockTableView(t *testing.T) {
	...
	txn1LTView := newTransactionBoundReplicatedLockTableView(engine, &txn1)
	txn2LTView := newTransactionBoundReplicatedLockTableView(engine, &txn2)
...
func TestRequestBoundLockTableView(t *testing.T) {
	...
	testutils.RunTrueAndFalse(t, "replicatedLockConflicts", func(t *testing.T, replicatedLockConflicts bool) {
		...
		// Non-locking request.
		ltView := newRequestBoundLockTableView(m, mockTxnBoundReplicatedLTV, lock.None)
		...
		ltView = newRequestBoundLockTableView(m, mockTxnBoundReplicatedLTV, lock.Shared)
		locked, _, err = ltView.IsKeyLockedByConflictingTxn(ctx, keyA)
		require.NoError(t, err)
		...
		ltView = newRequestBoundLockTableView(m, mockTxnBoundReplicatedLTV, lock.Exclusive)
