...
func evalExport(
	...
	// An ExportRequest to a prior range might have already buffered some SSTs in
	// its response that we must account for when setting the `targetSize` for the
	// SST generated by this ExportRequest.
	targetSize := uint64(args.TargetFileSize)
	if targetSize > uint64(h.TargetBytes) {
		targetSize = uint64(h.TargetBytes)
	}
	...
	//
	// TODO(adityamaru): Remove in 23.2.
	if allowedOverage == 0 {
	...
	for start := args.Key; start != nil; {
		...
		if h.TargetBytes > 0 {
			...
			if reply.NumBytes >= h.TargetBytes {
				...
				// Adjust the target size of the next SST that we want to create to fit
				// in the number of bytes we have left to fill in the ExportResponse.
				remainingBytes := uint64(h.TargetBytes - reply.NumBytes)
				if targetSize > remainingBytes {
					targetSize = remainingBytes
					maxSize = targetSize + uint64(args.MaxAllowedFileSizeOverage)
				}
			}
		}
	}
