...
func EndTxn(
	...
	log.VEventf(
		ctx, 2, "checking to see if transaction record has already been written for txn: %s", h.Txn,
	)
	...
	if reply.Txn.Status == roachpb.COMMITTED {
		if len(releasedReplLocks) != 0 {
			...
			log.VEventf(
				ctx, 2, "committed transaction released local replicated shared/exclusive replicated locks",
			)
		}
