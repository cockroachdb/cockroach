	...
	if err != nil {
		...
		case roachpb.ABORTED:
			if !args.Commit {
				...
				resolvedLocks, externalLocks, numBytes, err := resolveLocalLocks(ctx, desc, readWriter, ms, args, reply.Txn, cArgs.EvalCtx, cArgs.Header.TargetBytes)
				if err != nil {
					return result.Result{}, err
				}
...
func resolveLocalLocks(
	...
	evalCtx EvalContext,
	targetBytes int64,
) (resolvedLocks []roachpb.LockUpdate, externalLocks []roachpb.Span, numBytes int64, _ error) {
	...
	numKeys, numBytes, _, err := storage.MVCCPagination(ctx, resolveAllowance, targetBytes, done, f)
	if err != nil {
	...
	return resolvedLocks, externalLocks, numBytes, nil
}
