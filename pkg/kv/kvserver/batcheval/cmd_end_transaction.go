...
func EndTxn(
	...
	if args.Commit {
		...
		if reply.Txn.Status == roachpb.STAGING {
			...
			// erroneously committing it during the transaction recovery that is about
			// to follow.
			var txnForError roachpb.Transaction
			...
			err := kvpb.NewIndeterminateCommitError(txnForError)
