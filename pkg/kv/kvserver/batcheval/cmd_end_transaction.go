...
func RunCommitTrigger(
	...
	maybeWrapReplicaCorruptionError := func(err error) error {
		if err == nil {
	...
	if ct.GetSplitTrigger() != nil {
		...
		if err != nil {
			return result.Result{}, maybeWrapReplicaCorruptionError(
				errors.Wrap(err, "unable to load lease"),
			)
		}
		gcThreshold, err := sl.LoadGCThreshold(ctx, batch)
		if err != nil {
			return result.Result{}, maybeWrapReplicaCorruptionError(
				errors.Wrap(err, "unable to load GCThreshold"),
			)
		}
		gcHint, err := sl.LoadGCHint(ctx, batch)
		if err != nil {
			return result.Result{}, maybeWrapReplicaCorruptionError(
				errors.Wrap(err, "unable to load GCHint"),
			)
		}
		replicaVersion, err := sl.LoadVersion(ctx, batch)
		if err != nil {
			return result.Result{}, maybeWrapReplicaCorruptionError(
				errors.Wrap(err, "unable to load replica version"),
			)
		}
		...
		if err != nil {
			return result.Result{}, maybeWrapReplicaCorruptionError(err)
		}
		*ms = newMS
		return res, nil
	}
