...
func TestIsEndTxnTriggeringRetryError(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(name, func(t *testing.T) {
			txn := roachpb.MakeTransaction("test", nil, tt.txnIsoLevel, 0, hlc.Timestamp{WallTime: 10}, 0, 1, 0, false)
	...
	testutils.RunTrueAndFalse(t, "commitTrigger", func(t *testing.T, commitTrigger bool) {
		for _, cfg := range []struct {
			...
			t.Run(cfg.name, func(t *testing.T) {
				...
				txn := roachpb.MakeTransaction("test", desc.StartKey.AsRawKey(), 0, 0, now, 0, 1, 0, false)
