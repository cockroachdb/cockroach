...
// A LoadEvent represent a kv operation such as a read or write.
type LoadEvent struct {
}

// A WorkloadGenerator generates workload where each op contains: key,
...
// A ConfigEvent can be, for example, adding or removing a node in a region.
...
// A Range spans keys greater or equal to MinKey and smaller than the MinKey
// of the next range.
type Range struct {
	MinKey string
}
...
// A RangeMap (unlike a regular map) can return the Range responsible for a key.
...
// A Replica represents a replica of a range.
type Replica struct {
	spanConf *roachpb.SpanConfig
	desc     *roachpb.RangeDescriptor
}

// A Store simulates a store within a node.
type Store struct {
	replicas  map[int]*Replica
	allocator kvserver.Allocator
}

// A Node represents a node within the cluster.
// TODO: add regions.
type Node struct {
...
// The State offers two ways to access replicas, one is by looking up the node,
...
func RunAllocator(
	...
	desc *roachpb.RangeDescriptor,
	tick time.Time,
) (bool, kvserver.AllocatorAction, float64) {
