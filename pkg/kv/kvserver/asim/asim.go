...
func (rm *RangeMap) AddRange(minKey string) *Range {
	...
	rm.ranges.AscendGreaterOrEqual(r, func(i btree.Item) bool {
		...
		if successorRange, ok := i.(*Range); ok {
	...
	rm.ranges.DescendLessOrEqual(r, func(i btree.Item) bool {
		// The case where the min key already exists cannot occur here, as we a
		// panic will have fired in the above iteration.
		if predecessorRange, ok := i.(*Range); ok {
...
func (s *State) AddReplica(r *Range, node int) int {
	if nodeImpl, ok := s.Nodes[node]; ok {
		// Initially we assume that there is at most one store per node.
		...
		if existing, ok := store.Replicas[int(r.Desc.RangeID)]; ok {
			panic(fmt.Sprintf(
				...
				r.Desc.RangeID.String(),
				node, existing.replDesc.ReplicaID.String(),
			))
		}
	...
	// The node doesn't exist, we are unable to add a replica to a store on that node.
