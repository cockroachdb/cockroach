...
func NewSimulator(
	...
	exchange := state.NewFixedDelayExhange(
		settings.Start.Add(-settings.StateExchangeInterval),
		settings.StateExchangeInterval,
		settings.StateExchangeDelay,
	)
	exchange.Put(
		settings.Start.Add(-settings.StateExchangeInterval),
		initialState.StoreDescriptors()...,
	)
	...
	for _, store := range stores {
		...
		for _, rangeID := range state.LoadSplitterFor(storeID).ClearSplitKeys() {
			if r, ok := state.LeaseHolderReplica(rangeID); ok {
				log.Infof(ctx, "enqueuing lb split candidate +%v", rangeID)
				s.sqs[storeID].MaybeAdd(ctx, r, state)
			} else {
				log.Infof(ctx, "can't enqueu split candidate +%v", rangeID)
			}
		}
