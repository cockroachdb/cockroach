...
func NewMetricsTracker(listeners ...StoreMetricsListener) *MetricsTracker {
	return &MetricsTracker{
		storeListeners: listeners,
	}
}

func (mt *MetricsTracker) Tick(ctx context.Context, tick time.Time, s state.State) {
	...
	for storeID, u := range usage.StoreUsage {
		sm := StoreMetrics{
			...
			QPS:                int64(state.Capacity(s, storeID).QueriesPerSecond),
