...
func RangesInfoWithDistribution(
	...
	for rngIdx := 0; rngIdx < numRanges; rngIdx++ {
		...
		for replCandidateIdx := 0; replCandidateIdx < rf; replCandidateIdx++ {
			...
			rangeInfo.Replicas[replCandidateIdx] = storeID
			if targetLeaseCount[storeID] > targetLeaseCount[rangeInfo.Replicas[maxLeaseRequestedIdx]] {
				maxLeaseRequestedIdx = replCandidateIdx
			}
		}

		lhStore := rangeInfo.Replicas[maxLeaseRequestedIdx]
		targetLeaseCount[lhStore]--
		rangeInfo.Leaseholder = lhStore
