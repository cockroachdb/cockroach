...
func (s *state) addReplica(rangeID RangeID, storeID StoreID) (*replica, bool) {
	...
	if !s.ValidTarget(rangeID, storeID, true /* addTarget */) {
		return nil, false
	}
...
// ValidTarget returns whether adding/removing a replica for the Range with ID
// RangeID from/to the Store with ID StoreID is valid.
func (s *state) ValidTarget(rangeID RangeID, storeID StoreID, add bool) bool {
	...
	// If checking a valid add target, then a replica must not already
	// exist on the store. If checking a valid remove target, then a
	// replica must exist on the store.
	replica, ok := s.stores[storeID].replicas[rangeID]
	if add == ok {
		return false
	}
...
func (s *state) removeReplica(rangeID RangeID, storeID StoreID) bool {
	...
	if !s.ValidTarget(rangeID, storeID, false /* addTarget */) {
		return false
	}
...
func (s *state) SplitRange(splitKey Key) (Range, Range, bool) {
	...
	// create replicas on the same stores for the RHS. We first check  whether
	// all of the replicas can be added before proceeding.
	for storeID := range predecessorRange.replicas {
		if !s.ValidTarget(rangeID, storeID, true /* add replica */) {
			// When a replica cannot be added, instead return before committing
...
func (s *state) NodeLivenessFn() storepool.NodeLivenessFunc {
	nodeLivenessFn := func(nid roachpb.NodeID, now time.Time, timeUntilStoreDead time.Duration) livenesspb.NodeLivenessStatus {
		...
		// are deal when simulating partitions, crashes etc.
