...
func (s *state) Stores() []Store {
	stores := make([]Store, len(s.stores))
	i := 0
	for storeID := range s.stores {
		stores[i] = s.stores[storeID]
		i++
	}
...
func (s *state) Replicas(storeID StoreID) []Replica {
	...
	repls := make(replicaList, len(store.replicas))
	i := 0
...
func (r *rng) Replicas() []Replica {
	replicas := make(replicaList, len(r.replicas))
	i := 0
	for storeID := range r.replicas {
		replicas[i] = r.replicas[storeID]
		i++
	}
	sort.Sort(replicas)
	return replicas
