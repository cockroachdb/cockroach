...
func (co CapacityOverride) String() string {
	return fmt.Sprintf(
		"capacity=%d, available=%d, used=%d, logical_bytes=%d, range_count=%d, lease_count=%d, queries_per_sec=%.2f, writes_per_sec=%.2f, cpu_per_sec=%.2f, l0_sublevels=%d, io_threhold=%s",
		co.Capacity,
		co.Available,
		co.Used,
		co.LogicalBytes,
