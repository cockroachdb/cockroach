...
func promoDemo(
	...
	if removeOk, revertRemove := removeReplica(s, rangeID, storeID, current); !removeOk {
		rollback = append(rollback, revertRemove)
		return
	}
	if addOk, revertAdd := addReplica(s, rangeID, storeID, next); !addOk {
...
func (rc *ReplicaChange) Apply(s State) {
	...
	if lhBeingRemoved {
		if len(targets.NonVoterPromotions) > 0 {
			candidateLH := StoreID(targets.NonVoterPromotions[0].StoreID)
			nextLH = candidateLH
		}
		if len(targets.VoterAdditions) > 0 {
			candidateLH := StoreID(targets.VoterAdditions[0].StoreID)
			nextLH = candidateLH
		}
	...
	// or promoted first before removing any previous voters. This is doen so
	...
	for _, nonVoterPromotion := range targets.NonVoterPromotions {
		if ok, revert := promoDemo(
			s, rangeID, StoreID(nonVoterPromotion.StoreID), roachpb.NON_VOTER, roachpb.VOTER_FULL); !ok {
			rollback = append(rollback, revert...)
			return
		}
	}
	for _, voterAddition := range targets.VoterAdditions {
		if ok, revert := addReplica(
			s, rangeID, StoreID(voterAddition.StoreID), roachpb.VOTER_FULL); !ok {
			rollback = append(rollback, revert)
			return
		}
	}
	...
	for _, voterDemotion := range targets.VoterDemotions {
		if ok, revert := promoDemo(
			s, rangeID, StoreID(voterDemotion.StoreID), roachpb.VOTER_FULL, roachpb.NON_VOTER); !ok {
			rollback = append(rollback, revert...)
			return
		}
	}
	for _, voterRemoval := range targets.VoterRemovals {
		if ok, revert := removeReplica(
			s, rangeID, StoreID(voterRemoval.StoreID), roachpb.VOTER_FULL); !ok {
			rollback = append(rollback, revert)
			return
		}
	}
	for _, nonVoterAddition := range targets.NonVoterAdditions {
		if ok, revert := addReplica(
			s, rangeID, StoreID(nonVoterAddition.StoreID), roachpb.NON_VOTER); !ok {
			rollback = append(rollback, revert)
			return
		}
	}
	for _, nonVoterRemoval := range targets.NonVoterRemovals {
		if ok, revert := removeReplica(
			s, rangeID, StoreID(nonVoterRemoval.StoreID), roachpb.NON_VOTER); !ok {
			rollback = append(rollback, revert)
			return
		}
	}
...
func (rc *ReplicaChange) Target() StoreID {
	...
	if len(voterAdditions) > 1 || len(nonVoterAdditions) > 1 ||
		(len(voterAdditions) >= 1 && len(nonVoterAdditions) >= 1) {
		panic(fmt.Sprintf(
