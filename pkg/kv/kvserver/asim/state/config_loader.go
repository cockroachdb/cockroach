...
func NewZone(name string, nodeCount int, storesPerNode int) Zone {
	if storesPerNode < 1 {
		panic(fmt.Sprintf("storesPerNode cannot be less than one but received %v", storesPerNode))
	}
	return Zone{
		Name:          name,
		NodeCount:     nodeCount,
	...
	for _, r := range c.Regions {
		...
		for _, z := range r.Zones {
			...
			for i := 0; i < z.NodeCount; i++ {
				...
				if storesRequired < 1 {
					panic(fmt.Sprintf("storesPerNode cannot be less than one but received %v", storesRequired))
				}
