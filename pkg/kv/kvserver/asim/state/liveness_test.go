...
func TestLivenessAggregation(t *testing.T) {
	clock := &ManualSimClock{nanos: time.Date(2022, 03, 21, 11, 0, 0, 0, time.UTC).UnixNano()}
	hlcClock := hlc.NewClockForTesting(clock)

	// Create a node with 3 stores.
	m := StatusTracker{
		clock: hlcClock,
		storeStatusMap: map[StoreID]StoreStatus{
			1: {Liveness: LivenessLive},
			2: {Liveness: LivenessLive},
			3: {Liveness: LivenessLive},
		},
		nodeStatusMap: map[NodeID]NodeStatus{1: {Membership: livenesspb.MembershipStatus_ACTIVE}},
		storeToNode:   map[StoreID]NodeID{1: 1, 2: 1, 3: 1},
	}

	// All stores live => node live.
	require.Equal(t, LivenessLive, m.worstLivenessForStoresOnNode(1))

	// One store unavailable => node unavailable.
	m.storeStatusMap[2] = StoreStatus{Liveness: LivenessUnavailable}
	require.Equal(t, LivenessUnavailable, m.worstLivenessForStoresOnNode(1))

	// One store dead => node dead.
	m.storeStatusMap[3] = StoreStatus{Liveness: LivenessDead}
	require.Equal(t, LivenessDead, m.worstLivenessForStoresOnNode(1))
}
