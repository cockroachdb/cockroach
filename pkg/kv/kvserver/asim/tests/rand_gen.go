...
// generateSurvivalGoalsEvents schedules two mutation with assertion events. The
// first mutation event occurs at the start time, followed by an assertion at
// durationToAssert. Immediately after, the second mutation with assertion event
// occurs. Both mutation events enforce two hardcoded zone configuration.
...
func generateRandomSurvivalGoalsEvents(
	...
	for delay < duration {
		...
		zoneConfig, convertErr := sql.TestConvertRegionToZoneConfig(rc)
		if convertErr != nil {
