		...
		datadriven.Walk(t, dir, func(t *testing.T, path string) {
			...
			datadriven.RunTest(t, path, func(t *testing.T, d *datadriven.TestData) string {
				...
				case "eval":
					...
					// TODO(tbg): there's tension between having subtests per config (for
					// filtering and individual failure reporting), running them concurrently
					// (for speed), and combining their output into a single testdata file.
					// t.Parallel() subtests don't start until the parent yields, which
					// deadlocks if we block waiting for results to combine. A clean-ish fix
					// is to split each config into its own testdata file, sharing common
					// setup via an `include` directive (see TestClusterState for an example).
					for _, mv := range cfgs {
