...
//     Set the liveness for a single store.
//
...
//     Set status signals for a node. When liveness is specified, it sets the
//     liveness for all stores on that node. Membership and draining are always
//     per-node. Defaults: membership=active, draining=false.
//
