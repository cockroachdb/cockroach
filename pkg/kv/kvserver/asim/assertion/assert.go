...
func (ca ConformanceAssertion) Assert(
	...
	leaseViolatingPrefs, leaseLessPref := leasePreferenceReport(ctx, h)
	buf := strings.Builder{}
	holds = true
	...
	violatingLeases, lessPrefLeases := len(leaseViolatingPrefs), len(leaseLessPref)
	...
	if ca.LessPreferredLeases != ConformanceAssertionSentinel &&
		...
		maybeInitHolds()
		buf.WriteString(PrintSpanConfigConformanceList(
			"less preferred preferences", leaseLessPref))
	}
