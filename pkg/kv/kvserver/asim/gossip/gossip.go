...
type storeGossip struct {
	prevMap            map[state.StoreID]roachpb.StoreDescriptor
...
func (g *storeGossip) Gossip(tick time.Time, s state.State, stores ...state.StoreID) {
	...
	for _, desc := range descs {
		...
		// gossiped.
		g.prevMap[state.StoreID(desc.StoreID)] = desc
	}
...
func (g *storeGossip) shouldGossipOnCapacityChange(s state.State, storeID state.StoreID) bool {
	// NB: Variadic fn to get the store descriptor with ID storeID.
	desc := s.StoreDescriptors(storeID)[0]
	prev, cur := g.prevMap[storeID], desc
	rangeDelta := math.Abs(float64(prev.Capacity.RangeCount - cur.Capacity.RangeCount))
	leaseDelta := math.Abs(float64(prev.Capacity.LeaseCount - cur.Capacity.LeaseCount))

	rangeThreshold := math.Ceil(math.Min(float64(prev.Capacity.RangeCount)*
		kvserver.DefaultGossipWhenCapacityDeltaExceedsFraction, kvserver.GossipWhenRangeCountDeltaExceeds))
	leaseThreshold := math.Ceil(math.Min(float64(prev.Capacity.LeaseCount)*
		kvserver.DefaultGossipWhenCapacityDeltaExceedsFraction, kvserver.GossipWhenLeaseCountDeltaExceeds))
