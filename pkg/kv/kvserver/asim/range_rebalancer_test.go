...
func TestAdminLeaseTransfer(t *testing.T) {

	start := state.TestingStartTime()
	leaseDelayFn := func() time.Duration {
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			rangeRebalancer := NewRangeRebalancer(changer, allocatorimpl.Allocator{}, s, nil, start)
			replicaLeaseMovers := make(map[state.RangeID]ReplicaLeaseMover)
			for i := 2; i <= tc.ranges+1; i++ {
				rangeID := state.RangeID(i)
				s.TransferLease(state.RangeID(i), 1)
				replicaLeaseMovers[rangeID] = ReplicaLeaseMover{
					changer: changer,
					state:   s,
					rangeID: rangeID,
					tick:    start,
			...
			for i, tick := range tc.ticks {
				...
				rangeRebalancer.Tick(state.OffsetTick(start, tick))
				for _, mover := range replicaLeaseMovers {
					mover.tick = state.OffsetTick(start, tick)
				}

				for _, transfers := range tc.transfers[tick] {
					rangeID, target := transfers[0], transfers[1]
					err := rangeRebalancer.TransferLease(ctx, replicaLeaseMovers[state.RangeID(rangeID)], 0, roachpb.StoreID(target), 0)
...
func TestRelocateRange(t *testing.T) {
	...
	testDelayFn := func(rangeSize int64, add bool) time.Duration {
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			rangeRebalancer := NewRangeRebalancer(changer, allocator, s, testDelayFn, start)
			...
			pendingRelocationKeys := []state.Key{}
			for _, tick := range tc.ticks {
				...
				for rangeID, relocation := range relocations {
					...
					pendingRelocationKeys = append(pendingRelocationKeys, rangeStartKey)
					require.NoError(t, rangeRebalancer.RelocateRange(
						ctx,
				...
				// Tick the range rebalancer again - this is to handle the case
				// where we have just put in new relocations that should be
				// ticked immediately. This pattern won't follow in the actual
				// simulation loop.
				rangeRebalancer.Tick(state.OffsetTick(start, tick))
			...
			for _, key := range pendingRelocationKeys {
				done, err := rangeRebalancer.Check(key)
				require.True(t, done)
				require.NoError(t, err)
			}
