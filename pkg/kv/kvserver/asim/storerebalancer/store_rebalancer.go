...
// if it passes the should rebalance store check.
...
func (src *storeRebalancerControl) phaseLeaseRebalancing(
	...
	// No more rebalancing should be attempted, move to eplogue and clear the
	// rebalancer state.
	src.phaseEpilogue(ctx, tick)
}
...
func (src *storeRebalancerControl) phaseRangeRebalancing(
	...
	_ = src.sr.RebalanceStoreOucome(ctx, src.rebalancerState.rctx)
	src.phaseEpilogue(ctx, tick)
}
