...
type ReplicaLoadStats struct {
	...
	// RaftCPUNanosPerSecond is the range's time spent on-processor averaged
	// per second for raft.
	RaftCPUNanosPerSecond float64
	// RequestCPUNanosPerSecond is the range's time spent on-processor averaged
	// per second for requests.
