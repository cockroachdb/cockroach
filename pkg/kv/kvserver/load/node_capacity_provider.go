...
func (m *runtimeLoadMonitor) recordCPUUsage(ctx context.Context) error {
	...
	if err != nil {
		return errors.AssertionFailedf("failed to get cpu usage: %v", err)
	}
	// Convert milliseconds to nanoseconds.
	totalUsageNanos := float64(userTimeMillis*1e6 + sysTimeMillis*1e6)
	if m.mu.lastTotalUsageNanos > totalUsageNanos {
		return errors.AssertionFailedf("programming error: last cpu usage is larger than current: %v > %v",
