...
// this peer.
func (t *traceValue) sawMsgStorageAppendResp() bool {
	t.mu.Lock()
	defer t.mu.Unlock()
...
// storeEntry stores the TracedEntry without a tracing context and returns the
// newly created traceValue.
func (r *RaftTracer) storeEntry(te kvserverpb.TracedEntry) *traceValue {
...
// storeEntryWithTracing stores the TracedEntry along with a tracing context and
// returns the newly created traceValue.
func (r *RaftTracer) storeEntryWithTracing(
	propCtx context.Context, span *tracing.Span, te kvserverpb.TracedEntry,
) *traceValue {
	tv := traceValue{traced: te}
	tv.ctx = logtags.AddTag(r.ctx, "id", redact.Safe(tv.String()))
...
func (r *RaftTracer) traceIfPast(m raftpb.Message) {
	...
	r.m.Range(func(index kvpb.RaftIndex, t *traceValue) bool {
		...
		case raftpb.MsgStorageAppendResp:
			if kvpb.RaftIndex(m.Index) >= index && t.sawMsgStorageAppendResp() {
				t.logf(4,
					"%s->%s %v Log:%d/%d",
					peer(m.From),
					peer(m.To),
