...
func (r *RaftTracer) Register(baseCtx withContext, ent raftpb.Entry) {
	...
	log.VEventfDepth(ctx, 1, 3, "registering entry (i%d) for tracing with id %s", ent.Index, prettyTraceID(ctx))
	log.VEventfDepth(baseCtx.Context(), 1, 3, "registering entry (i%d) for tracing with id %s", ent.Index, prettyTraceID(ctx))
