...
func tracingID(traceID tracingpb.TraceID, spanID tracingpb.SpanID) string {
	// Balance uniqueness with readability by only keeping the lower 16 bits of
	// the trace and span.
...
func (r *RaftTracer) RegisterRemote(
	...
	if r.numRegistered.Load() > maxTraces {
		return
	}
	...
	t.logf(1, "trace index (i%d) from remote %s", index, tracingID(traceID, remoteSpanID))
	r.m.Store(index, &t)
}
