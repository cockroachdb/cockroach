...
func (r *RaftTracer) storeEntry(te kvserverpb.TracedEntry, baseCtx withContext) *traceValue {
	tv := traceValue{
		traced:                te,
		baseCtx:               baseCtx,
		appRespSent:           make(map[raftpb.PeerID]bool),
		storageAppendRespSent: make(map[raftpb.PeerID]bool),
	}
...
func (r *RaftTracer) traceIfPast(m raftpb.Message) {
	...
	r.m.Range(func(index kvpb.RaftIndex, t *traceValue) bool {
		...
		case raftpb.MsgAppResp:
			if kvpb.RaftIndex(m.Index) >= index && !t.appRespSent[m.From] {
				t.appRespSent[m.From] = true
				t.logf(4,
		...
		case raftpb.MsgStorageAppendResp:
			if kvpb.RaftIndex(m.Index) >= index && !t.storageAppendRespSent[m.From] {
				t.storageAppendRespSent[m.From] = true
				t.logf(4,
					"%s->%s %v Log:%d/%d",
					peer(m.From),
					peer(m.To),
