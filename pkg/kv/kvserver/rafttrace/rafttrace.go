...
type traceValue struct {
	...
	ctx context.Context
	// propCtx is the underlying proposal context which we additionally log to.
	// We can't store the underlying context directly because it can be modified
	// by the proposal buffer once it is returned to the user.
	propCtx withContext

	mu struct {
		syncutil.Mutex
		// seenMsgAppResp and seenMsgStorageAppendResp track whether range
		// messages have already been logged to log at most once. This limits
		// the log from growing too large at a small risk of missing some
		// messages in the case of dropped messages or reproposals.
		seenMsgAppResp, seenMsgStorageAppendResp map[raftpb.PeerID]bool
	}
}
...
func (tv *traceValue) logf(depth int, format string, args ...interface{}) {
	...
	if tv.propCtx != nil {
		if ctx := tv.propCtx.Context(); ctx != nil {
			log.VEventfDepth(ctx, depth+1, 3, format, args...)
		}
	}
}

// sawMsgAppResp returns true on the first MsgAppResp for this peer.
func (t *traceValue) sawMsgAppResp(p raftpb.PeerID) bool {
	t.mu.Lock()
	defer t.mu.Unlock()
...
// sawMsgStorageAppendResp returns true on the first MsgStorageAppendResp for
// this peer.
func (t *traceValue) sawMsgStorageAppendResp(p raftpb.PeerID) bool {
	t.mu.Lock()
	defer t.mu.Unlock()
	if t.mu.seenMsgStorageAppendResp[p] {
		return false
	}
	t.mu.seenMsgStorageAppendResp[p] = true
	return true
}
...
type RaftTracer struct {
	...
	// numRegisteredReplica to MaxInt64 when we close the tracer to prevent new
	// registrations.
...
// replica.
func NewRaftTracer(
	ctx context.Context, st *cluster.Settings, numRegisteredStore *atomic.Int64,
) *RaftTracer {
	return &RaftTracer{ctx: ctx, st: st, numRegisteredStore: numRegisteredStore}
}
...
// the number .  This method will return true if we can add one to the number
// registered for both the store and replica, otherwise it will return false.
func (r *RaftTracer) reserveSpace() bool {
	...
	// previous call to reserveAll. If this happens flush all our current traces
	// and don't register this request. Note that when this happens we also
	// wont't log this request.
	...
	if numAllowed == 0 {
		// This occurs if the setting has changed. If this happens additionally
		// flush all current traces.
		return false
	}
	...
	if numRegisteredReplica > numRegisteredStore {
		panic(fmt.Sprintf("numRegisteredReplica(%d) > numRegisteredStore(%d)", numRegisteredReplica, numRegisteredStore))
	}
	if numRegisteredReplica > numAllowed {
		panic(fmt.Sprintf("numRegisteredReplica(%d) > numAllowed(%d)", numRegisteredReplica, numAllowed))
	}
...
// storeEntry stores the trace and returns the newly created value.
func (r *RaftTracer) storeEntry(te kvserverpb.TracedEntry, propCtx withContext) *traceValue {
	tv := traceValue{
		traced:  te,
		propCtx: propCtx,
	}
	...
	tv.mu.seenMsgStorageAppendResp = make(map[raftpb.PeerID]bool)
...
// MaybeRegister is called on an entry that is about to be proposed. This will
// begin logging all subsequent updates to this entry. It will return true if
// the registration succeeded.
func (r *RaftTracer) MaybeRegister(propCtx withContext, ent raftpb.Entry) bool {
	// Only register if there is a trace in the context and it is set to verbose
	// logging.
	span := tracing.SpanFromContext(propCtx.Context())
	...
	if ent.Index == 0 {
		log.VEventf(propCtx.Context(), 2, "skip registering raft proposal without index: %v", ent)
		return false
	}
	...
	if !r.reserveSpace() {
		log.VEvent(propCtx.Context(), 2, "too many active raft traces, skipping")
		return false
	}

	// Grab the trace and span id now because the propCtx.Context may be
	// nil'ed later.
	tv := r.storeEntry(
...
// MaybeTrace logs the message if it is covered by a trace.
...
// removeEntry remove this entry and decrements the registered counters at the
// replica and store level.
...
func (r *RaftTracer) Close() {
	r.m.Range(func(index kvpb.RaftIndex, t *traceValue) bool {
		r.removeEntry(index)
		t.logf(2, "cleanup log index %d during FlushAll", index)
		return true
	})
	// Prevent any new registrations.
	r.numRegisteredReplica.Store(math.MaxInt64)
}
...
// only allow MsgAppResp and MsgStorageAppendResp to be logged once per peer.
func (r *RaftTracer) traceIfPast(m raftpb.Message) {
	...
	r.m.Range(func(index kvpb.RaftIndex, t *traceValue) bool {
		...
		case raftpb.MsgAppResp:
			if kvpb.RaftIndex(m.Index) >= index && t.sawMsgAppResp(m.From) {
				t.logf(4,
		...
		case raftpb.MsgStorageAppendResp:
			if kvpb.RaftIndex(m.Index) >= index && t.sawMsgStorageAppendResp(m.From) {
				t.logf(4,
					"%s->%s %v Log:%d/%d",
					peer(m.From),
					peer(m.To),
