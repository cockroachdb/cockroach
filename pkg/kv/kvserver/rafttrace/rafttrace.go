...
type traceValue struct {
	...
	mu struct {
		...
		// propSpan is the span connected to the propCtx.
...
type RaftTracer struct {
	...
	// registrations.
	//
	// TODO(baptist/pavelkalinnikov): Look at optimizing to avoid the need for
	// this to be an atomic. It likely doesn't need to be atomic since the
	// callers should be holding Replica.raftMu and/or Replica.mu.
...
func (r *RaftTracer) reserveSpace() bool {
	...
	if buildutil.CrdbTestBuild {
		if numRegisteredReplica > numRegisteredStore {
			log.Fatalf(r.ctx, "numRegisteredReplica(%d) > numRegisteredStore(%d)", numRegisteredReplica, numRegisteredStore)
		}
		if numRegisteredReplica > numAllowed {
			log.Fatalf(r.ctx, "numRegisteredReplica(%d) > numAllowed(%d)", numRegisteredReplica, numAllowed)
		}
	}
...
func (r *RaftTracer) storeEntry(
	te kvserverpb.TracedEntry, propCtx context.Context, span *tracing.Span,
) *traceValue {
	tv := traceValue{
		traced: te,
	}
...
func (r *RaftTracer) MaybeRegister(ctx context.Context, ent raftpb.Entry) bool {
	...
	if ent.Index == 0 {
		log.VEventf(ctx, 2, "skip registering raft proposal without index: %v", ent)
		return false
	}
...
func (r *RaftTracer) removeEntry(index kvpb.RaftIndex) {
	...
	if tv.mu.propSpan != nil {
		...
		// Don't allow tracing to this context anymore.
		tv.mu.propCtx = nil
	}
...
func (r *RaftTracer) Close() {
	// Prevent any new registrations before clearing the entries. This is done
	// before we iterate over the current entries to prevent new entries from
	// being added. Note that there could be a race between another caller
	// calling Register and us closing the tracer, however we won't allow any
	// new registrations to come through after this. Note that we set this to
	// MaxInt32 instead of MaxInt64 to avoid a rare race where another thread is
	// in the middle of `reserveSpace` and calls `Add(1)` after we set this to
	// max.
