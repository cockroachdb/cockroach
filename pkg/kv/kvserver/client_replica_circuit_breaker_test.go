...
func TestReplicaCircuitBreaker_LeaseholderTripped(t *testing.T) {
	...
	require.EqualValues(t, 1, s1.Metrics().ReplicaCircuitBreakerCumTripped.Value())
	require.Zero(t, s2.Metrics().ReplicaCircuitBreakerCurTripped.Value())
	require.Zero(t, s2.Metrics().ReplicaCircuitBreakerCumTripped.Value())
	...
	require.EqualValues(t, 2, s1.Metrics().ReplicaCircuitBreakerCumTripped.Value())
