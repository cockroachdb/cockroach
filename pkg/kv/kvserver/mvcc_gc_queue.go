...
// mvccGCQueueFullyEnableAC controls whether admission control is enabled for the MVCC GC queue.
var mvccGCQueueFullyEnableAC = settings.RegisterBoolSetting(
	settings.SystemOnly,
	"kv.mvcc_gc.queue_kv_admission_control.enabled",
	"when true, MVCC GC queue operations are subject to admission control",
	true,
)
