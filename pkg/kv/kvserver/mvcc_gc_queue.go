...
func (mgcq *mvccGCQueue) process(
	...
	if repl.store.cfg.SharedStorageEnabled || storage.UseEFOS.Get(&repl.ClusterSettings().SV) {
		...
		if err := efos.WaitForFileOnly(ctx); err != nil {
			efos.Close()
			log.VEventf(ctx, 1, "not gc'ing replica %v due to error grabbing file-only snapshot: %v", repl, err)
			return false, errors.Wrap(err, "error when waiting for file-only snapshot")
		}
		snap = efos
	} else {
