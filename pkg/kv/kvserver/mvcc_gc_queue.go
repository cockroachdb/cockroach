...
func newMVCCGCQueue(store *Store) *mvccGCQueue {
	...
	mgcq.SetDisabled(!kvserverbase.MVCCGCQueueEnabled.Get(&store.cfg.Settings.SV))
	kvserverbase.MVCCGCQueueEnabled.SetOnChange(&store.cfg.Settings.SV, func(ctx context.Context) {
		mgcq.SetDisabled(!kvserverbase.MVCCGCQueueEnabled.Get(&store.cfg.Settings.SV))
	})
	return mgcq
}
