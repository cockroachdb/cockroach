...
type testProposer struct {
	...
	// If not nil, this can be a testRaftProposer used to mock the raft group
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expRejection {
				p.onRejectProposalWithRedirectLocked = func(_ *ProposalData, redirectTo roachpb.ReplicaID) {
					rejected = append(rejected, redirectTo)
				}
			} else {
			...
			if tc.expRejection {
				require.ElementsMatch(t, rejected, []roachpb.ReplicaID{roachpb.ReplicaID(tc.leader)})
			}
		})
	}
}
