...
func TestProposalBufferClosedTimestamp(t *testing.T) {
	...
	// Each test will propose one command and check for the expected closed timestamp
	// being written to the proposal by the propBuf.
	for _, tc := range []struct {
		name string
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			tracker.lowerBound = tc.trackerLowerBound
			var b propBuf
