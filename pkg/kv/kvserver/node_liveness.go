...
func (nl *NodeLiveness) setMembershipStatusInternal(
	...
	if oldLivenessRec.Liveness == (kvserverpb.Liveness{}) {
		// Liveness record didn't previously exist, so we create one.
		//
		// ASK(reviewers): Why is it possible for a liveness record to not exist
		// previously? This code feels a bit hairy because of it. Can we
		// generally make it not so?
		newLiveness = kvserverpb.Liveness{
			NodeID: nodeID,
			Epoch:  1,
		}
...
func (nl *NodeLiveness) GetIsLiveMap() IsLiveMap {
	...
	for nID, l := range nl.mu.nodes {
		isLive := l.IsLive(now)
		if !isLive && l.Membership.Decommissioning() || l.Membership.Decommissioned() {
