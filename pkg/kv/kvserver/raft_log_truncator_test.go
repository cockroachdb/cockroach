...
func (r *replicaTruncatorTest) printEngine(t *testing.T, eng storage.Engine) {
	...
	// OnlyReadGuaranteedDurable for printing in this test, since we flush above
	// whenever writing RaftAppliedIndex.
	...
	datadriven.RunTest(t, testutils.TestDataPath(t, "raft_log_truncator"),
		func(t *testing.T, d *datadriven.TestData) string {
			...
			case "add-pending-truncation":
				...
				printTruncatorState(&buf, &truncator)
				return flushAndReset()
			...
			case "add-replica-to-truncator":
				...
				truncator.mu.Lock()
				truncator.mu.ranges[scanRangeID(t, d)] = struct{}{}
				truncator.mu.Unlock()
				printTruncatorState(&buf, &truncator)
				return flushAndReset()
			...
			case "durability-advanced":
				truncator.mu.Lock()
				n := len(truncator.mu.ranges)
				truncator.mu.Unlock()
				truncator.durabilityAdvanced(context.Background())
				if n > 1 {
					// Non-deterministic map iteration order. We don't need the details
					// for every durability advanced test case since we will also print
					// the replica states.
					buf.Reset()
					fmt.Fprintf(&buf, "<elided details>\n")
				}
				printTruncatorState(&buf, &truncator)
				return flushAndReset()
...
func printTruncatorState(buf *strings.Builder, truncator *raftLogTruncator) {
	truncator.mu.Lock()
	defer truncator.mu.Unlock()
	ranges := make([]roachpb.RangeID, 0, len(truncator.mu.ranges))
	for id := range truncator.mu.ranges {
