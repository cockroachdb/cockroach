...
func (r *replicaTruncatorTest) sideloadedBytesIfTruncatedFromTo(
	_ context.Context, from, to roachpb.RaftIndex,
) (freed int64, _ error) {
...
func (r *replicaTruncatorTest) writeRaftStateToEngine(
	t *testing.T, eng storage.Engine, truncIndex roachpb.RaftIndex, lastLogEntry roachpb.RaftIndex,
) {
...
func (r *replicaTruncatorTest) writeRaftAppliedIndex(
	t *testing.T, eng storage.Engine, raftAppliedIndex roachpb.RaftIndex, flush bool,
) {
	...
	datadriven.RunTest(t, datapathutils.TestDataPath(t, "raft_log_truncator"),
		func(t *testing.T, d *datadriven.TestData) string {
			...
			case "create-replica":
				...
				r.truncState.Index = roachpb.RaftIndex(truncIndex)
				r.writeRaftStateToEngine(t, eng, roachpb.RaftIndex(truncIndex), roachpb.RaftIndex(lastLogEntry))
			...
			case "add-pending-truncation":
				...
				truncator.addPendingTruncation(context.Background(), r,
					roachpb.RaftTruncatedState{Index: roachpb.RaftIndex(truncIndex)}, roachpb.RaftIndex(firstIndex), int64(deltaBytes))
			...
			case "write-raft-applied-index":
				...
				store.replicas[rangeID].writeRaftAppliedIndex(t, eng, roachpb.RaftIndex(raftAppliedIndex), !noFlush)
				return flushAndReset()
