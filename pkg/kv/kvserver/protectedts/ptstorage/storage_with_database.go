...
func (s *storageWithDatabase) GetMetadata(
	...
	if txn == nil {
		err = s.ief.TxnWithExecutor(ctx, s.db, nil /* sessionData */, func(ctx context.Context, txn *kv.Txn, ie sqlutil.InternalExecutor) error {
			md, err = s.s.GetMetadata(ctx, txn, ie)
			return err
		})
		return md, err
	}
...
func (s *storageWithDatabase) GetState(
	...
	if txn == nil {
		err = s.ief.TxnWithExecutor(ctx, s.db, nil /* sessionData */, func(ctx context.Context, txn *kv.Txn, ie sqlutil.InternalExecutor) (err error) {
			state, err = s.s.GetState(ctx, txn, ie)
			return err
		})
		return state, err
	}
