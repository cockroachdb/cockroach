...
func TestMetrics(t *testing.T) {
	...
	timeStart1 := 1000
	for i := 0; i < 20; i++ {
		d.Record(context.Background(), ms(timeStart1), 1, func() roachpb.Span {
			return roachpb.Span{Key: keys.SystemSQLCodec.TablePrefix(uint32(0))}
		})
	}
	for i := 1; i <= 1000; i++ {
		d.Record(context.Background(), ms(timeStart1+i*100), 1, func() roachpb.Span {
			return roachpb.Span{Key: keys.SystemSQLCodec.TablePrefix(uint32(0))}
		})
	}

	assert.Equal(t, d.loadSplitterMetrics.PopularKeyCount.Count(), int64(2))
	assert.Equal(t, d.loadSplitterMetrics.NoSplitKeyCount.Count(), int64(2))

	// No split key, not popular key
	timeStart2 := 110000
	for i := 0; i < 20; i++ {
		d.Record(context.Background(), ms(timeStart2), 1, func() roachpb.Span {
			return roachpb.Span{Key: keys.SystemSQLCodec.TablePrefix(uint32(0))}
		})
	}
	for i := 1; i <= 1000; i++ {
		d.Record(context.Background(), ms(timeStart2+i*100), 1, func() roachpb.Span {
			return roachpb.Span{Key: keys.SystemSQLCodec.TablePrefix(uint32(i))}
		})
	}

	assert.Equal(t, d.loadSplitterMetrics.PopularKeyCount.Count(), int64(2))
	assert.Equal(t, d.loadSplitterMetrics.NoSplitKeyCount.Count(), int64(4))
}
