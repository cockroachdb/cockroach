...
func runTest(
	reqGen requestGenerator, recordFn func(span roachpb.Span, weight int), keyFn func() roachpb.Key,
) result {
	requests, weightedKeys, totalWeight := reqGen.generate()
	optimalKey, optimalLeftWeight, optimalRightWeight := getOptimalKey(weightedKeys, totalWeight)
	key, leftWeight, rightWeight, recordExecutionTime, keyExecutionTime, noKeyFound :=
		getKey(recordFn, keyFn, requests, weightedKeys)
...
func resultTable(configs []requestConfig, rr []repeatedResult, showTiming bool) string {
...
func runTestRepeated(settings *lbsTestSettings) repeatedResult {
	...
	for i := 0; i < settings.iterations; i++ {
		...
		if settings.deciderConfig != nil {
			...
			now := startTime
			numRequests := requestGen.numRequests
		...
		ret := runTest(requestGen, recordFn, keyFn)
...
//
//   - "eval" seed=<int> iterations=<int> cartesian=<bool> seed is the seed to
//     instantiate the current finder/decider and request generators with,
//     iterations is the number of iterations to repeatedly run before
//     synthesizing a result. The cartesian evaluates a list of configurations
//     with every different distribution and a few maximum values for span
//     length, start key and weight.
//
	...
	datadriven.Walk(t, dir, func(t *testing.T, path string) {
		...
		datadriven.RunTest(t, path, func(t *testing.T, d *datadriven.TestData) string {
			...
			case "eval":
				...
				repeatedResults := make([]repeatedResult, len(requestConfigs))
				for i, r := range requestConfigs {
				...
				return resultTable(requestConfigs, repeatedResults, showTiming)
			default:
