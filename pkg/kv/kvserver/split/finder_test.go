...
func TestSplitFinderKey(t *testing.T) {
	...
	for i := 0; i < splitKeySampleSize; i++ {
		tempSample := sample{
			key:       keys.SystemTenantKeyGen.TablePrefix(uint32(ReservoirKeyOffset + i)),
			left:      0,
			right:     0,
			contained: 0,
		}
	...
	midSample := sample{
		key:       keys.SystemTenantKeyGen.TablePrefix(uint32(ReservoirKeyOffset + splitKeySampleSize/2)),
		left:      splitKeyMinCounter,
		right:     splitKeyMinCounter,
		contained: 0,
	}
	...
	testCases := []struct {
		...
		{uniformReservoir, keys.SystemTenantKeyGen.TablePrefix(ReservoirKeyOffset)},
		// Testing a non-uniform reservoir.
		{nonUniformReservoir, keys.SystemTenantKeyGen.TablePrefix(ReservoirKeyOffset + splitKeySampleSize/2)},
		...
		{multipleHotKeysReservoir, keys.SystemTenantKeyGen.TablePrefix(ReservoirKeyOffset + 1)},
		// Test a spanning reservoir. Splitting will be bad here. Should avoid it.
		{spanningReservoir, nil},
		// Test that splits happen between two heavy spans.
		{multipleSpanReservoir, keys.SystemTenantKeyGen.TablePrefix(ReservoirKeyOffset + splitKeySampleSize/2)},
	}
...
func TestSplitFinderRecorder(t *testing.T) {
	...
	basicSpan := roachpb.Span{
		Key:    keys.SystemTenantKeyGen.TablePrefix(ReservoirKeyOffset),
		EndKey: keys.SystemTenantKeyGen.TablePrefix(ReservoirKeyOffset + 1),
	}
	...
	replacementSpan := roachpb.Span{
		Key:    keys.SystemTenantKeyGen.TablePrefix(ReservoirKeyOffset + splitKeySampleSize),
		EndKey: keys.SystemTenantKeyGen.TablePrefix(ReservoirKeyOffset + splitKeySampleSize + 1),
	}
	...
	spanningSpan := roachpb.Span{
		Key:    keys.SystemTenantKeyGen.TablePrefix(ReservoirKeyOffset - 1),
		EndKey: keys.SystemTenantKeyGen.TablePrefix(ReservoirKeyOffset + splitKeySampleSize + 1),
	}
