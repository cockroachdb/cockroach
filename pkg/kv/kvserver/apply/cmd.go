...
// as the receiver, thus forcing that to be the first param. The caller didn't
...
func forEachCmdIter(
	ctx context.Context,
	iter CommandIterator,
	fn func(Command, context.Context) error) error {
	defer iter.Close()
	for iter.Valid() {
...
func forEachCheckedCmdIter(
	ctx context.Context,
	iter CheckedCommandIterator,
	fn func(CheckedCommand, context.Context) error,
) error {
	defer iter.Close()
	for iter.Valid() {
