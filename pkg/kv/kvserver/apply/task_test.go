...
func (c *cmd) Index() roachpb.RaftIndex { return c.index }
...
type testStateMachine struct {
	batches            [][]roachpb.RaftIndex
	applied            []roachpb.RaftIndex
	appliedSideEffects []roachpb.RaftIndex
	batchOpen          bool
}
...
type testDecoder struct {
	nonTrivial            map[roachpb.RaftIndex]bool
	nonLocal              map[roachpb.RaftIndex]bool
	shouldReject          map[roachpb.RaftIndex]bool
	shouldThrowErrRemoved map[roachpb.RaftIndex]bool
...
func newTestDecoder() *testDecoder {
	return &testDecoder{
		nonTrivial:            make(map[roachpb.RaftIndex]bool),
		nonLocal:              make(map[roachpb.RaftIndex]bool),
		shouldReject:          make(map[roachpb.RaftIndex]bool),
		shouldThrowErrRemoved: make(map[roachpb.RaftIndex]bool),
	}
}
...
func (d *testDecoder) DecodeAndBind(_ context.Context, ents []raftpb.Entry) (bool, error) {
	...
	for i, ent := range ents {
		idx := roachpb.RaftIndex(ent.Index)
		cmd := &cmd{
			index:                 idx,
...
func TestApplyCommittedEntries(t *testing.T) {
	...
	exp := testStateMachine{
		batches:            [][]roachpb.RaftIndex{{1, 2}, {3}, {4}, {5}, {6}},
		applied:            []roachpb.RaftIndex{1, 2, 3, 4, 5, 6},
		appliedSideEffects: []roachpb.RaftIndex{1, 2, 3, 4, 5, 6},
	}
...
func TestApplyCommittedEntriesWithBatchSize(t *testing.T) {
	...
	// Assert that all commands were applied in the correct batches.
	exp := testStateMachine{
		batches:            [][]roachpb.RaftIndex{{1, 2}, {3}, {4}, {5, 6}, {7}},
		applied:            []roachpb.RaftIndex{1, 2, 3, 4, 5, 6, 7},
		appliedSideEffects: []roachpb.RaftIndex{1, 2, 3, 4, 5, 6, 7},
	}
	require.Equal(t, exp, *sm)
...
func TestApplyCommittedEntriesWithErr(t *testing.T) {
	...
	exp := testStateMachine{
		batches:            [][]roachpb.RaftIndex{{1, 2}, {3}},
		applied:            []roachpb.RaftIndex{1, 2, 3},
		appliedSideEffects: []roachpb.RaftIndex{1, 2, 3},
	}
	require.Equal(t, exp, *sm)
