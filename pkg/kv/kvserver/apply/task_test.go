...
func (c *cmd) Index() enginepb.RaftIndex { return c.index }
...
type testStateMachine struct {
	batches            [][]enginepb.RaftIndex
	applied            []enginepb.RaftIndex
	appliedSideEffects []enginepb.RaftIndex
	batchOpen          bool
}
...
type testDecoder struct {
	nonTrivial            map[enginepb.RaftIndex]bool
	nonLocal              map[enginepb.RaftIndex]bool
	shouldReject          map[enginepb.RaftIndex]bool
	shouldThrowErrRemoved map[enginepb.RaftIndex]bool
...
func newTestDecoder() *testDecoder {
	return &testDecoder{
		nonTrivial:            make(map[enginepb.RaftIndex]bool),
		nonLocal:              make(map[enginepb.RaftIndex]bool),
		shouldReject:          make(map[enginepb.RaftIndex]bool),
		shouldThrowErrRemoved: make(map[enginepb.RaftIndex]bool),
	}
}
...
func (d *testDecoder) DecodeAndBind(_ context.Context, ents []raftpb.Entry) (bool, error) {
	...
	for i, ent := range ents {
		idx := enginepb.RaftIndex(ent.Index)
		cmd := &cmd{
			index:                 idx,
...
func TestApplyCommittedEntries(t *testing.T) {
	...
	exp := testStateMachine{
		batches:            [][]enginepb.RaftIndex{{1, 2}, {3}, {4}, {5}, {6}},
		applied:            []enginepb.RaftIndex{1, 2, 3, 4, 5, 6},
		appliedSideEffects: []enginepb.RaftIndex{1, 2, 3, 4, 5, 6},
	}
...
func TestApplyCommittedEntriesWithBatchSize(t *testing.T) {
	...
	// Assert that all commands were applied in the correct batches.
	exp := testStateMachine{
		batches:            [][]enginepb.RaftIndex{{1, 2}, {3}, {4}, {5, 6}, {7}},
		applied:            []enginepb.RaftIndex{1, 2, 3, 4, 5, 6, 7},
		appliedSideEffects: []enginepb.RaftIndex{1, 2, 3, 4, 5, 6, 7},
	}
	require.Equal(t, exp, *sm)
...
func TestApplyCommittedEntriesWithErr(t *testing.T) {
	...
	exp := testStateMachine{
		batches:            [][]enginepb.RaftIndex{{1, 2}, {3}},
		applied:            []enginepb.RaftIndex{1, 2, 3},
		appliedSideEffects: []enginepb.RaftIndex{1, 2, 3},
	}
	require.Equal(t, exp, *sm)
