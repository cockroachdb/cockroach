...
// Apply records command application, and asserts validity.
//
...
func (a *Asserter) Apply(
	...
	if ac, ok := a.appliedCmds[rangeID][cmdID]; !ok {
		...
		msg += fmt.Sprintf("index %d term %d (%s): %x", entry.Index, entry.Term, entry.Type, entry.Data)
		panic(msg)
	}
}
...
// applied index (which must be the case if the snapshot is correct). We can't
// verify this, but we assume so in order to assert invariants that depend on
// it being true (the consistency checker will also check it).
//
// TODO(erikgrinaker): actually verify that the snapshot contains the expected
// commands, if this is even possible at a reasonable cost.
func (a *Asserter) ApplySnapshot(
	...
	// Assert and record the applied index. Snapshots should not regress it
	// either.
	if appliedIndex := a.appliedIndex[rangeID][replicaID]; index <= appliedIndex {
		panic(fmt.Sprintf("snapshot applied index regression for r%d/%d: %d -> %d",
			rangeID, replicaID, appliedIndex, index))
	}
	...
	//
	// A command must have applied on at least one replica before it can be sent
	// in a snapshot, so all relevant commands are already present in the map.
