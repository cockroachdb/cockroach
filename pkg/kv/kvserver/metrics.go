...
non-durable I/O (except for replication configuration changes). Note that
a "batch" in this context is really a sub-batch of the batch received for
application during raft ready handling. The 'raft.process.applycommitted.latency'
histogram may be more suitable in most cases.
`,
		Measurement: "Latency",
		Unit:        metric.Unit_NANOSECONDS,
	}
