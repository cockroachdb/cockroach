...
var (
	...
	metaRangeSnapShotCrossZoneSentBytes = metric.Metadata{
		...
		region tier keys are not configured. This count increases for each snapshot
		sent between different zones within the same region. However, if the region
		tiers are not configured in any nodes, this count may also include snapshot
		data sent between zones in different regions. Ensuring consistent
	...
	metaRangeSnapShotCrossZoneRcvdBytes = metric.Metadata{
		...
		region tier keys are is not configured. This count increases for each
		snapshot received between different zones within the same region. However,
		if the region tiers are not configured in any nodes, this count may also
		include snapshot data received between zones in different regions. Ensuring
		consistent configuration of region and zone tiers across nodes helps to
		accurately monitor the data transmitted.`,
		Measurement: "Bytes",
		Unit:        metric.Unit_BYTES,
	}
