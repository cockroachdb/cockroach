...
// getReplicaChecksum returns replicaChecksum tracker for the given ID. The
// caller must call delReplicaChecksum when finished working on this tracker.
...
// delReplicaChecksum removes the given replicaChecksum from the state.
func (r *Replica) delReplicaChecksum(id uuid.UUID, rc *replicaChecksum) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *Replica) getChecksum(ctx context.Context, id uuid.UUID) (CollectChecksumResponse, error) {
	now := timeutil.Now()
	c := r.getReplicaChecksum(id)
	defer r.delReplicaChecksum(id, c)
...
func (r *Replica) computeChecksumPostApply(
	...
	// NB: all exit paths must call delReplicaChecksum.
	c := r.getReplicaChecksum(cc.ChecksumID)
	defer func() {
		if err != nil {
			close(c.started) // send nothing to signal that the task failed to start
			r.delReplicaChecksum(cc.ChecksumID, c)
		}
	}()
	...
	if err := stopper.RunAsyncTaskEx(taskCtx, stop.TaskOpts{
		...
		defer taskCancel()
		defer snap.Close()
		defer r.delReplicaChecksum(cc.ChecksumID, c)
