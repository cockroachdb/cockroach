...
// How long to keep consistency checker checksums in-memory for collection. A
// minute is generous. Typically a long-poll waits for the result of the
// computation, so it's almost immediately collected. However, the consistency
// checker synchronously collects the first replica's checksum before all
// others, so if the first one is slow the checksum may not be collected right
// away.
