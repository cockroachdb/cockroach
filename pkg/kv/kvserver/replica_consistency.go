...
func (r *Replica) RunConsistencyCheck(
	...
	results := make([]ConsistencyCheckResult, len(replicas))

	ctx, cancel := context.WithCancel(ctx)
	defer cancel() // cancel all the outstanding requests on early exiting
	...
	for result := range resultCh {
		...
		if result.Replica.IsSame(localReplica) {
			...
			// the outstanding requests, and wait for all the async tasks from above
			// to terminate.
