...
type propBuf struct {
	...
	assignedLAI roachpb.LeaseSequence
	...
	testing struct {
		...
		leaseIndexFilter func(*ProposalData) roachpb.LeaseSequence
...
) (roachpb.LeaseSequence, hlc.Timestamp, error) {
...
func (b *propBuf) marshallLAIAndClosedTimestampToProposalLocked(
	ctx context.Context, p *ProposalData, lai roachpb.LeaseSequence, closedTimestamp hlc.Timestamp,
) error {
	buf := &b.scratchFooter
...
func (b *propBuf) forwardAssignedLAILocked(v roachpb.LeaseSequence) {
	if b.assignedLAI < v {
		b.assignedLAI = v
	}
}
...
func (b *propBuf) OnLeaseChangeLocked(
	leaseOwned bool, appliedClosedTS hlc.Timestamp, appliedLAI roachpb.LeaseSequence,
) {
	if leaseOwned {
...
func (rp *replicaProposer) firstIndex() roachpb.RaftIndex {
	return (*Replica)(rp).raftFirstIndexRLocked()
}

func (rp *replicaProposer) leaseAppliedIndex() roachpb.LeaseSequence {
