...
func proposeBatch(
	...
	// lifecycle. We trace at most one entry per batch, so pick the last one as
	// is the last one to be completed.
	lastIndex := len(ents) - 1
	p.registerForTracing(props[lastIndex], ents[lastIndex])
	return nil
}
