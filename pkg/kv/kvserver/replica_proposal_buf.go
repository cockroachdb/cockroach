...
type proposer interface {
	...
	// consequence of the rejection is that the request that caused the lease
	// acquisition attempt is tried on the leader, at which point it should result
	// in a lease acquisition attempt by that node (or, perhaps by then the leader
	// will have already gotten a lease and the request can be serviced directly).
	rejectProposalWithRedirectLocked(ctx context.Context, prop *ProposalData, redirectTo roachpb.ReplicaID)
}
...
func (b *propBuf) FlushLockedWithRaftGroup(
	...
	if raftGroup != nil {
		...
		if leaderKnown {
			...
			if leader == b.p.replicaID() {
				...
				log.Fatalf(ctx,
					"inconsistent Raft state: StateFollower while the current replica is also the lead: %d",
					leader)
			}
		}
	}
	...
	for i, p := range buf {
		...
		if follower && p.Request.IsLeaseRequest() {
			if leaderKnown {
...
func (rp *replicaProposer) rejectProposalWithRedirectLocked(
	...
	if err != nil {
		...
		log.Warningf(ctx, "replica not present in range desc %s; ignoring", rangeDesc)
	}
	...
	prop.finishApplication(ctx, proposalResult{
		Err: roachpb.NewError(newNotLeaseHolderError(speculativeLease, replicaDesc.StoreID, rangeDesc)),
	})
}
