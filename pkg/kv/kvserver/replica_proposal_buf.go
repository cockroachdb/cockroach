...
func (b *propBuf) FlushLockedWithRaftGroup(
	...
	for i, p := range buf {
		...
		if !reproposal {
			lai, closedTimestamp, err := b.allocateClosedTimestampAndLAILocked(ctx, p, closedTSTarget)
			if err != nil {
				firstErr = err
				continue
			}
			err = b.marshallClosedTimestampAndLAIToProposalLocked(ctx, p, lai, closedTimestamp)
			if err != nil {
				firstErr = err
				continue
			}
...
// marshallClosedTimestampAndLAIToProposalLocked modifies p.encodedCommand,
// adding the lai and closed timestamp.
func (b *propBuf) marshallClosedTimestampAndLAIToProposalLocked(
	ctx context.Context, p *ProposalData, lai uint64, closedTimestamp hlc.Timestamp,
) error {
	buf := &b.scratchFooter
	// Reset the buffer, as a safety measure.
	*buf = kvserverpb.RaftCommandFooter{}
	...
	if log.ExpensiveLogEnabled(ctx, 4) {
		log.VEventf(ctx, 4, "attaching closed timestamp %s to proposal %x",
			b.assignedClosedTimestamp, p.idKey)
	}
...
// assignClosedTimestampAndLAILocked computes a LAI and closed timestamp to be
...
func (b *propBuf) allocateClosedTimestampAndLAILocked(
	...
	if b.testing.dontCloseTimestamps {
		return b.assignedLAI, hlc.Timestamp{}, nil
	}
	...
	return b.assignedLAI, closedTSTarget, nil
}
