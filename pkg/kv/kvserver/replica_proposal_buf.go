...
type proposer interface {
	...
	// raftAppliedClosedTimestamp returns whether the range has switched to the
	// Raft-based closed timestamp transport.
...
func (b *propBuf) assignClosedTimestampToProposal(
	...
	if p.Request.IsLeaseRequest() {
		...
		oldLease := p.leaseStatus.Lease.Sequence
		leaseExtension := newLease.Sequence == oldLease
		if leaseExtension {
			return nil
		}
	...
	b.FwdClosedTimestampLocked(closedTSTarget)
...
func (b *propBuf) OnLeaseChangeLocked(leaseOwned bool, closedTS hlc.Timestamp) {
	if leaseOwned {
		b.FwdClosedTimestampLocked(closedTS)
	} else {
...
// FwdClosedTimestamp forwards the closed timestamp tracked by the propBuf.
func (b *propBuf) FwdClosedTimestampLocked(closedTS hlc.Timestamp) {
