...
func (s *snapWriteBuilder) clearSubsumedReplicaDiskData(ctx context.Context) error {
	...
	// clearResidualDiskDataOnReceivingNarrowerSnapshot, not here.
...
func (s *snapWriteBuilder) clearResidualDiskDataOnReceivingNarrowerSnapshot(
	...
	if len(s.subsumedDescs) != 0 {
		// NB: s.subsumedDescs are non-overlapping and sorted by start key. Pick the
		// last one to determine whether the snapshot is narrowing the keyspace or
		// not.
