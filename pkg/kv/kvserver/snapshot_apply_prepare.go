...
func (s *snapWriteBuilder) prepareSnapApply(ctx context.Context) error {
	...
	err = s.clearSplitReplicaDiskData(ctx)
	if err != nil {
		return err
	}
...
//
// clearSplitReplicaDiskState is a no-op if there is no split, i.e, the right
// most descriptor's EndKey indicates that it's narrower than the snapshot's
// descriptor.
func (s *snapWriteBuilder) clearSplitReplicaDiskData(ctx context.Context) error {
	...
	for _, span := range rditer.Select(0, rditer.SelectOpts{
		...
		s.cleared = append(s.cleared, roachpb.Span{Key: span.Key, EndKey: span.EndKey})
	}
