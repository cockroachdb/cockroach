...
func (r *replicaRaftStorage) Entries(lo, hi uint64, maxBytes uint64) ([]raftpb.Entry, error) {
	return r.TypedEntries(enginepb.RaftIndex(lo), enginepb.RaftIndex(hi), maxBytes)
}

func (r *replicaRaftStorage) TypedEntries(
	lo, hi enginepb.RaftIndex, maxBytes uint64,
) ([]raftpb.Entry, error) {
	ctx := r.AnnotateCtx(context.TODO())
...
func (r *replicaRaftStorage) Term(i uint64) (uint64, error) {
	term, err := r.TypedTerm(enginepb.RaftIndex(i))
	return uint64(term), err
}

// TypedTerm requires that r.mu is held for writing because it requires exclusive
// access to r.mu.stateLoader.
func (r *replicaRaftStorage) TypedTerm(i enginepb.RaftIndex) (enginepb.RaftTerm, error) {
...
func (r *Replica) raftTermLocked(i enginepb.RaftIndex) (enginepb.RaftTerm, error) {
	return (*replicaRaftStorage)(r).TypedTerm(i)
}

// GetTerm returns the term of the given index in the raft log. It requires that
// r.mu is not held.
func (r *Replica) GetTerm(i enginepb.RaftIndex) (enginepb.RaftTerm, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *Replica) raftLastIndexRLocked() enginepb.RaftIndex {
...
func (r *Replica) raftFirstIndexRLocked() enginepb.RaftIndex {
...
func (r *Replica) GetLeaseAppliedIndex() enginepb.LeaseSequence {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
type IncomingSnapshot struct {
	...
	raftAppliedIndex enginepb.RaftIndex // logging only
}
...
func writeUnreplicatedSST(
	...
	if err := sl.SetRaftTruncatedState(
		...
		&roachpb.RaftTruncatedState{
			Index: enginepb.RaftIndex(meta.Index),
			Term:  enginepb.RaftTerm(meta.Term),
		},
	); err != nil {
