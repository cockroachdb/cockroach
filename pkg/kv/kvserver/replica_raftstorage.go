...
func (r *Replica) applySnapshotRaftMuLocked(
	...
	applyAsBatch := len(inSnap.externalSSTs)+len(inSnap.sharedSSTs) == 0 && /* simple */
		!inSnap.SSTStorageScratch.HasBlobFiles() && /* no blob files */
	...
	if applyAsBatch {
		...
		sstPaths := inSnap.SSTStorageScratch.SSTPaths()
		if err := sw.eng.StateEngine().IngestLocalFilesToWriter(
			ctx, sstPaths, inSnap.clearedSpans, sw.applyAsBatch(),
		); err != nil {
			return errors.Wrapf(err, "while converting to batch %s", sstPaths)
		}
	}
	...
	if fn := r.store.cfg.TestingKnobs.BeforeSnapshotSSTIngestion; fn != nil {
		if err := fn(inSnap, inSnap.SSTStorageScratch.SSTPaths()); err != nil {
			return err
		}
	}
	...
	ingestStats, err := sw.commit(ctx, snapIngestion{
		localSSTs:  inSnap.SSTStorageScratch.SSTs(),
