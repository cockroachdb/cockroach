...
func (r *Replica) applySnapshot(
	...
	// Prepare all storage-related data.
	...
	// Convert subsumedRepls to subsumedDescs
	...
	storageData, err := prepareSnapshotData(
		ctx,
		r.store.TODOEngine(),
		desc,
		r.replicaID,
	...
	// Add the unreplicated SST to the snapshot's SSTs.
	if err := inSnap.SSTStorageScratch.WriteSST(ctx, storageData.unreplicatedSST); err != nil {
		return err
	}

	// Add all subsumed SSTs to the snapshot's SSTs.
	for _, sst := range storageData.subsumedSSTs {
		if err := inSnap.SSTStorageScratch.WriteSST(ctx, sst); err != nil {
			return err
		}
	}
	...
	if len(inSnap.externalSSTs) > 0 || len(inSnap.sharedSSTs) > 0 {
		...
		if inSnap.SSTSize > snapshotIngestAsWriteThreshold.Get(&r.ClusterSettings().SV) {
			...
			err := r.store.TODOEngine().ConvertFilesToBatchAndCommit(
				ctx, inSnap.SSTStorageScratch.SSTs(), storageData.clearedSpans)
			if err != nil {
	...
	if uint64(storageData.state.RaftAppliedIndex) != nonemptySnap.Metadata.Index {
		log.Fatalf(ctx, "snapshot RaftAppliedIndex %d doesn't match its metadata index %d",
			storageData.state.RaftAppliedIndex, nonemptySnap.Metadata.Index)
	}
	if uint64(storageData.state.RaftAppliedIndexTerm) != nonemptySnap.Metadata.Term {
		log.Fatalf(ctx, "snapshot RaftAppliedIndexTerm %d doesn't match its metadata term %d",
			storageData.state.RaftAppliedIndexTerm, nonemptySnap.Metadata.Term)
	}
	...
	r.store.metrics.addMVCCStats(ctx, r.tenantMetricsRef, *storageData.state.Stats)
	...
	r.shMu.state = storageData.state
	...
	// in the snapshot.
	r.leasePostApplyLocked(ctx, lastKnownLease, storageData.state.Lease /* newLease */, storageData.priorReadSum, allowLeaseJump)
	...
	if len(subsumedRepls) > 0 && storageData.state.Lease.Replica.ReplicaID == r.replicaID && storageData.priorReadSum != nil {
		applyReadSummaryToTimestampCache(ctx, r.store.tsCache, r.descRLocked(), *storageData.priorReadSum)
	}
