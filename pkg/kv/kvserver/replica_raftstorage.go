...
type OutgoingSnapshot struct {
	...
	sharedBackings []objstorage.SharedObjectBackingHandle
	onClose        func()
}
...
func (r *Replica) applySnapshot(
	...
	defer func(start time.Time) {
		...
		if len(inSnap.sharedSSTs) > 0 {
			logDetails.Printf(" shared=%d sharedSize=%d", len(inSnap.sharedSSTs), inSnap.SharedSize)
		}
		if inSnap.doExcise {
	...
	if inSnap.doExcise {
		...
		if ingestStats, err =
			r.store.TODOEngine().IngestAndExciseExternalFiles(ctx, inSnap.SSTStorageScratch.SSTs(), inSnap.sharedSSTs, exciseSpan); err != nil {
