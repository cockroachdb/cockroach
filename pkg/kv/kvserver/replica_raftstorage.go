...
func (r *Replica) applySnapshot(
	...
	defer func(start time.Time) {
		logDetails := redact.Sprintf("total=%0.0fms", timeutil.Now().Sub(start).Seconds()*1000)
		if len(subsumedRepls) > 0 {
			logDetails += redact.Sprintf(" subsumedReplicas=%d@%0.0fms",
				len(subsumedRepls), stats.subsumedReplicas.Sub(start).Seconds()*1000)
		}
		logDetails += redact.Sprintf(" ingestion=%d@%0.0fms", len(inSnap.SSTStorageScratch.SSTs()),
