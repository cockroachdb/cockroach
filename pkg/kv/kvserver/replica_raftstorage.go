...
func (r *replicaRaftStorage) Entries(lo, hi uint64, maxBytes uint64) ([]raftpb.Entry, error) {
	return r.TypedEntries(roachpb.RaftIndex(lo), roachpb.RaftIndex(hi), maxBytes)
}

func (r *replicaRaftStorage) TypedEntries(
	lo, hi roachpb.RaftIndex, maxBytes uint64,
) ([]raftpb.Entry, error) {
	ctx := r.AnnotateCtx(context.TODO())
...
func (r *replicaRaftStorage) Term(i uint64) (uint64, error) {
	term, err := r.TypedTerm(roachpb.RaftIndex(i))
	return uint64(term), err
}

// TypedTerm requires that r.mu is held for writing because it requires exclusive
// access to r.mu.stateLoader.
func (r *replicaRaftStorage) TypedTerm(i roachpb.RaftIndex) (roachpb.RaftTerm, error) {
...
func (r *Replica) raftTermLocked(i roachpb.RaftIndex) (roachpb.RaftTerm, error) {
	return (*replicaRaftStorage)(r).TypedTerm(i)
}

// GetTerm returns the term of the given index in the raft log. It requires that
// r.mu is not held.
func (r *Replica) GetTerm(i roachpb.RaftIndex) (roachpb.RaftTerm, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *Replica) raftLastIndexRLocked() roachpb.RaftIndex {
...
func (r *Replica) raftFirstIndexRLocked() roachpb.RaftIndex {
...
func (r *Replica) GetLeaseAppliedIndex() roachpb.LeaseAppliedIndex {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
type IncomingSnapshot struct {
	...
	raftAppliedIndex roachpb.RaftIndex // logging only
}
...
func writeUnreplicatedSST(
	...
	if err := sl.SetRaftTruncatedState(
		...
		&roachpb.RaftTruncatedState{
			Index: roachpb.RaftIndex(meta.Index),
			Term:  roachpb.RaftTerm(meta.Term),
		},
	); err != nil {
