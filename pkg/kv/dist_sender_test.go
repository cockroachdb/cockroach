...
func TestErrorIndexAlignment(t *testing.T) {
	...
	for i, tc := range testCases {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			...
			aPS, ok := pErr.GetDetail().(*roachpb.PartialSuccessError)
			if a, e := ok, tc.expectedPartialSuccess; a != e {
				t.Fatalf("expected partial success %t; got %t", e, a)
			}
			if ok {
				pErr = aPS.Wrapped
			}
