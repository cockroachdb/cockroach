...
func makeGossip(t *testing.T, stopper *stop.Stopper) (*gossip.Gossip, *hlc.Clock) {
	...
	const nodeID = 1
	g := gossip.NewTest(nodeID, rpcContext, server, stopper, metric.NewRegistry(), config.DefaultZoneConfig())
...
func TestGetFirstRangeDescriptor(t *testing.T) {
	...
	n := simulation.NewNetwork(stopper, 3, true, config.DefaultZoneConfig())
