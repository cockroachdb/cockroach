...
func (tc *TxnCoordSender) maybeCondenseIntentSpans(
	...
	// Iterate over spans using a range iterator and add each to a
	// bucket keyed by range ID. Local keys are kept in a new slice
	// and not added to buckets.
	buckets := []*spanBucket{}
	...
	for _, s := range spans {
		...
		for ri.Seek(ctx, roachpb.RKey(s.Key), Ascending); ri.Valid(); ri.Next(ctx) {
	...
	for _, bucket := range buckets {
		...
		for _, s := range bucket.spans[1:] {
			newCS := cs.Combine(s)
			if !newCS.Valid() {
				return nil, errors.Errorf("combining span %s with %s yielded invalid result", cs, s)
			}
		}
...
func (tc *TxnCoordSender) updateState(
	...
	if txnMeta != nil || pErr == nil || ambiguousErr || newTxn.Writing {
		...
		// read key bytes.
		if ba.Txn.Isolation == enginepb.SERIALIZABLE && txnMeta != nil && !txnMeta.readKeysExceeded {
			ba.ReadSpanIterate(br, func(key, endKey roachpb.Key) {
				readKeys = append(readKeys, roachpb.Span{Key: key, EndKey: endKey})
			})
		}
		...
		for _, s := range readKeys {
			...
			if readKeysSize > tc.maxTxnReadsBytes {
				// Note that txnMeta is guaranteed non-nil here since readKeys is non-empty.
				txnMeta.readKeys = []roachpb.Span{}
				txnMeta.readKeysExceeded = true
				break
			}
		}
	}
...
func (tc *TxnCoordSender) tryUpdateTxnReads(
	...
	for _, s := range txnMeta.readKeys {
		updateReadBa.Add(&roachpb.UpdateReadRequest{Span: s})
	}
