...
func (tcf *TxnCoordSenderFactory) TransactionalSender(
	...
	tcs.interceptorAlloc.txnMetrics.init(
		&tcs.mu.Mutex,
		&tcs.mu.txn,
		tcs.clock,
		&tcs.metrics,
	...
	tcs.interceptorStack = [...]txnInterceptor{
		...
		// The seq num allocator is the last one so that it sees all requests
		// inserted by other interceptors. Note though that it skips assigning seq
		// nums to certain types of requests (e.g. heartbeats).
