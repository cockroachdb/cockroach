...
func (tc *TxnCoordSender) updateState(
	...
	if pErr == nil {
		...
		if pErr.TransactionRestart != roachpb.TransactionRestart_NONE {
			...
			if !ok {
				...
				log.Errorf(ctx, "Expected conversion to InternalRetryableTxnError "+
					"to succeed, but it didn't. The conversion returned: %s", goErr)
		...
		if retryErr != nil {
			...
			// Pass a HandledRetryableError up to the next layer.
			pErr = roachpb.NewError(
				roachpb.NewHandledRetryableError(retryErr.Error(), retryErr.TxnID, newTxn))
		} else {
