...
// maxTxnUpdatesBytes is a threshold in bytes for update spans stored
// on the coordinator during the lifetime of a transaction. Updates
// are used for SERIALIZABLE transactions to avoid client restarts.
var maxTxnUpdatesBytes = settings.RegisterIntSetting(
	"kv.transaction.max_updates_bytes",
	"maximum number of bytes used to track updates in serializable transactions",
	256*1000,
)
...
type TxnCoordSender struct {
	mu struct {
		...
		// updatesSizeBytes is the total size in bytes of the spans
		// encountered during this transaction that need to be updated to
		// avoid serializable restart.
		updatesSizeBytes int64
...
func (tc *TxnCoordSender) GetMeta() roachpb.TxnCoordMeta {
	...
	if !tc.mu.meta.UpdatesExceeded {
		meta.UpdateReads = append([]roachpb.Span(nil), tc.mu.meta.UpdateReads...)
		meta.UpdateWrites = append([]roachpb.Span(nil), tc.mu.meta.UpdateWrites...)
	}
	return meta
}
...
func (tc *TxnCoordSender) AugmentMeta(meta roachpb.TxnCoordMeta) {
	...
	if meta.UpdatesExceeded {
		tc.mu.meta.UpdatesExceeded = true
		tc.mu.meta.UpdateReads = nil
		tc.mu.meta.UpdateWrites = nil
	} else if !tc.mu.meta.UpdatesExceeded {
		tc.mu.meta.UpdateReads, _ = roachpb.MergeSpans(
			append(append([]roachpb.Span(nil), tc.mu.meta.UpdateReads...), meta.UpdateReads...),
		)
		tc.mu.meta.UpdateWrites, _ = roachpb.MergeSpans(
			append(append([]roachpb.Span(nil), tc.mu.meta.UpdateWrites...), meta.UpdateWrites...),
		)
	}
	...
	// Recompute the size of the updates.
	tc.mu.updatesSizeBytes = 0
	for _, u := range tc.mu.meta.UpdateReads {
		tc.mu.updatesSizeBytes += int64(len(u.Key) + len(u.EndKey))
	}
	for _, u := range tc.mu.meta.UpdateWrites {
		tc.mu.updatesSizeBytes += int64(len(u.Key) + len(u.EndKey))
	}
}
...
func (tc *TxnCoordSender) Send(
	...
	if ba.Txn != nil {
		...
		if hasET && log.V(1) {
			for _, intent := range et.IntentSpans {
				log.VEventf(ctx, 2, "intent: [%s,%s)", intent.Key, intent.EndKey)
			}
		}
	}
	...
	var br *roachpb.BatchResponse
	{
		...
		// Iterate over and aggregate update spans in the requests,
		...
		if pErr == nil && ba.Txn != nil && ba.Txn.Isolation == enginepb.SERIALIZABLE {
			tc.mu.Lock()
			if !tc.mu.meta.UpdatesExceeded {
				if !tc.appendUpdatesLocked(ctx, ba, br) {
					// The updates are out of date, return a generic client-side retry error.
					pErr = roachpb.NewErrorWithTxn(
						roachpb.NewTransactionRetryError(roachpb.RETRY_SERIALIZABLE), &tc.mu.meta.Txn,
					)
				} else if tc.mu.updatesSizeBytes > maxTxnUpdatesBytes.Get(&tc.st.SV) {
					// Verify and enforce the size in bytes of all read-only spans
					// doesn't exceed the max threshold.
					log.VEventf(ctx, 2, "updates size exceeded; clearing")
					tc.mu.meta.UpdateReads = nil
					tc.mu.meta.UpdateWrites = nil
					tc.mu.meta.UpdatesExceeded = true
					tc.mu.updatesSizeBytes = 0
...
//    if the spans collected during the transaction can be updated,
...
func (tc *TxnCoordSender) maybeRetrySend(
	...
	// If partial success is ambiguous, we can't attempt a retry without
	// potentially succeeding at the same conditional put or increment
	// request twice; return the wrapped error instead.
	if aPSErr, ok := pErr.GetDetail().(*roachpb.AmbiguousPartialSuccessError); ok {
		log.VEventf(ctx, 2, "got ambiguous partial success; cannot retry %s (pErr=%s)", ba, aPSErr.Wrapped)
	...
	// only places where we have all of the update spans. If this is a leaf, as
	// in a distributed sql flow, we need to propagate the error to the root for
	// an epoch restart.
	canRetry, retryTxn := roachpb.CanTransactionRetryAtSafeTimestamp(ctx, pErr)
	if !canRetry || tc.typ == client.LeafTxn {
		return nil, pErr
	}

	// If the batch was partially successful (unambiguously so), collect
	// update spans for the successful portion, and only retry the
	// remainder. The canonical case is a batch split between everything
	// up to but not including the EndTransaction. Requests up to the
	// EndTransaction succeed, but the EndTransaction fails with a
	// retryable error. We want to retry only the EndTransaction.
	ba.UpdateTxn(retryTxn)
	retryBa := *ba
	if br != nil {
		doneBa := *ba
		doneBa.Requests = ba.Requests[:len(br.Responses)]
		log.VEventf(ctx, 2, "collecting update spans after partial batch execution of %s", doneBa)
		tc.mu.Lock()
		if !tc.appendUpdatesLocked(ctx, doneBa, br) {
			tc.mu.Unlock()
			return nil, pErr
		}
		tc.mu.meta.Txn.SafeTimestamp.Forward(retryTxn.SafeTimestamp)
		tc.mu.Unlock()
		retryBa.Requests = ba.Requests[len(br.Responses):]
	}

	log.VEventf(ctx, 2, "retrying %s at safe timestamp %s because of %s",
		retryBa, retryTxn.SafeTimestamp, pErr)
	...
	// newBa.Txn.SafeTimestamp to the current timestamp. Submit the
	// batch again.
	retryBr, retryErr := tc.wrapped.Send(ctx, retryBa)
	if retryErr != nil {
		log.VEventf(ctx, 2, "failed to avoid txn restart: %s", retryErr)
		return nil, pErr
	}
	log.VEventf(ctx, 2, "retry of %s successful @%s", retryBa, retryBa.Txn.Timestamp)
...
// appendUpdatesLocked appends update spans from the supplied batch
// request, qualified by the batch response where appropriate. Returns
// whether the batch transaction's safe timestamp is greater or equal
// to the max safe timestamp used so far with this sender.
//
// The batch safe timestamp and the max safe timestamp for the sender
// can get out of step because the txn coord sender can be used
// concurrently (i.e. when using the "RETURNING NOTHING" syntax). What
// we don't want is to append updates which are already too old
// compared to the max safe timestamp that's already in use with this
// sender. In that case the caller should return an error for
// client-side retry.
func (tc *TxnCoordSender) appendUpdatesLocked(
	ctx context.Context, ba roachpb.BatchRequest, br *roachpb.BatchResponse,
) bool {
	origTS := ba.Txn.OrigTimestamp
	origTS.Forward(ba.Txn.SafeTimestamp)
	if origTS.Less(tc.mu.meta.Txn.SafeTimestamp) {
		log.VEventf(ctx, 2, "txn orig timestamp %s < sender safe timestamp %s",
			origTS, tc.mu.meta.Txn.SafeTimestamp)
		return false
	}
	ba.UpdateSpanIterate(br, func(span roachpb.Span, write bool) {
		log.VEventf(ctx, 2, "[%s] update: %s write=%t", ba.Txn.ID.Short(), span, write)
		if write {
			tc.mu.meta.UpdateWrites = append(tc.mu.meta.UpdateWrites, span)
		} else {
			tc.mu.meta.UpdateReads = append(tc.mu.meta.UpdateReads, span)
		}
		tc.mu.updatesSizeBytes += int64(len(span.Key) + len(span.EndKey))
	})
	return true
}

// tryUpdatingTxnSpans sends UpdateSpan commands to all spans read
// during the transaction to ensure that no writes were written more
// recently than the original transaction timestamp. All implicated
// timestamp caches are updated with the final transaction
...
func (tc *TxnCoordSender) tryUpdatingTxnSpans(
	ctx context.Context, updateTxn *roachpb.Transaction,
) bool {
	tc.mu.Lock()
	updateReads := tc.mu.meta.UpdateReads
	updateWrites := tc.mu.meta.UpdateWrites
	updatesExceeded := tc.mu.meta.UpdatesExceeded
	tc.mu.Unlock()

	if updatesExceeded {
		log.VEvent(ctx, 2, "can't update txn spans because updates exceeded")
		return false
	} else if len(updateReads) == 0 && len(updateWrites) == 0 {
		log.VEvent(ctx, 2, "there are no txn spans to update")
		return true
	}

	// Update all spans (merge first).
	updateSpanBa := roachpb.BatchRequest{}
	updateSpanBa.Txn = updateTxn
	addUpdates := func(updates []roachpb.Span, write bool) {
		for _, u := range updates {
			var req roachpb.Request
			if len(u.EndKey) == 0 {
				req = &roachpb.UpdateRequest{
					Span:  u,
					Write: write,
				}
			} else {
				req = &roachpb.UpdateRangeRequest{
					Span:  u,
					Write: write,
				}
			}
			updateSpanBa.Add(req)
			log.VEventf(ctx, 2, "updating span %s @%s - @%s to avoid serializable restart",
				req.Header(), updateTxn.OrigTimestamp, updateTxn.Timestamp)
		}
	}
	addUpdates(updateReads, false)
	addUpdates(updateWrites, true)
	if _, batchErr := tc.wrapped.Send(ctx, updateSpanBa); batchErr != nil {
		log.VEventf(ctx, 2, "failed to update txn spans (%s); propagating original retry error", batchErr)
		return false
	}
...
func (tc *TxnCoordSender) heartbeat(ctx context.Context) bool {
	...
	if pErr != nil {
		log.VEventf(ctx, 2, "heartbeat failed to %s: %s", txn, pErr)
		if errTxn := pErr.GetTxn(); errTxn != nil {
			tc.mu.Lock()
			tc.mu.meta.Txn.Update(errTxn)
			tc.mu.Unlock()
...
func (tc *TxnCoordSender) updateState(
	...
	if pErr == nil {
		...
		if pErr.TransactionRestart != roachpb.TransactionRestart_NONE &&
			...
			tc.mu.Lock()
			log.VEventf(ctx, 2, "clearing state on retry: %s", pErr)
			tc.mu.meta.CommandCount = 0
			tc.mu.meta.UpdateReads = nil
			tc.mu.meta.UpdateWrites = nil
			tc.mu.meta.UpdatesExceeded = false
			tc.mu.updatesSizeBytes = 0
			tc.mu.Unlock()
