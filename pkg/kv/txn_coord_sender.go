...
type txnInterceptor interface {
	...
	// loadTxn populates the interceptor's initial state from a given
	// Transaction.
	//
	// This is called once, both for root and leaf txns.
	loadTxn(*roachpb.Transaction)

	// initializeLeaf initializes the interceptor from an input payload
	// coming from a RootTxn.
	//
...
func newRootTxnCoordSender(
	...
	tcs.initCommonInterceptors(tcf, client.RootTxn, riGen)
...
func (tc *TxnCoordSender) initCommonInterceptors(
	tcf *TxnCoordSenderFactory, typ client.TxnType, riGen RangeIteratorGen,
) {
...
func newLeafTxnCoordSender(
	...
	tcs.initCommonInterceptors(tcf, client.LeafTxn, riGen)
...
func (tc *TxnCoordSender) SetUserPriority(pri roachpb.UserPriority) error {
	...
	if tc.mu.active && pri != tc.mu.userPriority {
		return errors.WithStack(errCannotChangePriority)
	}
