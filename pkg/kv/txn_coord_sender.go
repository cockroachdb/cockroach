...
func (tc *TxnCoordSender) updateState(
	...
	if pErr == nil && ba.Txn.IsSerializable() {
		...
		if !tc.mu.meta.RefreshValid && (newTxn.WriteTooOld || newTxn.OrigTimestamp != newTxn.Timestamp) {
			pErr = roachpb.NewErrorWithTxn(
				errors.New("transaction is too large to complete; try splitting into pieces"), &tc.mu.meta.Txn,
			)
		}
	}
