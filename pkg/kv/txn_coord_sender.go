...
type TxnMetrics struct {
	...
	RestartsSnapshot     *metric.Counter
...
var (
	...
	metaRestartsSnapshot = metric.Metadata{
		Name: "txn.restarts.snapshot",
		Help: "Number of restarts due to a DelRange, forwarded commit timestamp, and isolation=SNAPSHOT"}
...
func MakeTxnMetrics(histogramWindow time.Duration) TxnMetrics {
	return TxnMetrics{
		...
		RestartsSnapshot:     metric.NewCounter(metaRestartsSnapshot),
...
func (tc *TxnCoordSender) updateState(
	...
	case *roachpb.TransactionRetryError:
		...
		case roachpb.RETRY_SNAPSHOT:
			tc.metrics.RestartsSnapshot.Inc(1)
