...
const (
	// ready indicates the transaction is ready to start or has started
	// and is pending.
	ready txnCoordState = iota
...
// record, to keep it live. Note that this capability is optional, and
// intended for use only from the top level transaction coordinator,
// in the event that multiple coordinators are active (i.e. in a
// distributed SQL flow).
type TxnCoordSender struct {
	mu struct {
		...
		txnEnd chan struct{}
		// state indicates the state of the transaction coordinator.
		state txnCoordState
	}
...
// New implements the TxnCoordSenderFactory interface. It returns a new
// TxnCoordSender instance with transaction type set to rootOrLeaf.
func (tcf *TxnCoordSenderFactory) New(rootOrLeaf client.TxnType) client.TxnSender {
	return &TxnCoordSender{
		rootOrLeaf:            rootOrLeaf,
		TxnCoordSenderFactory: tcf,
	}
}

// GetMeta implements the client.TxnSender interface. It
// returns the txn coord sender metadata.
func (tc *TxnCoordSender) GetMeta() roachpb.TxnCoordMeta {
	tc.mu.Lock()
	defer tc.mu.Unlock()
	// Copy the values so they're immutable for the caller.
	return roachpb.TxnCoordMeta{
		Txn:     tc.mu.meta.Txn.Clone(),
		Intents: append([]roachpb.Span(nil), tc.mu.meta.Intents...),
	}
}

// AugmentMeta implements the client.TxnSender interface. It
// combines the state of other, distributed TxnCoordSenders into this
// sender. Note that if the provided meta is for a different txn,
// this is a noop.
func (tc *TxnCoordSender) AugmentMeta(meta roachpb.TxnCoordMeta) {
	...
	newIntents := append(tc.mu.meta.Intents, meta.Intents...)
	tc.mu.meta.Intents, _ = roachpb.MergeSpans(newIntents)
}
...
func (tc *TxnCoordSender) Send(
	...
	if ba.Txn != nil {
		...
		if pErr := func() *roachpb.Error {
			...
			if tc.rootOrLeaf == client.LeafTxn {
...
// goroutine signaled to clean up the transaction gracefully.
...
func (tc *TxnCoordSender) updateState(
	...
	if pErr == nil || ambiguousErr || newTxn.Writing {
		...
		if int64(len(intents)) > maxIntents.Get(&tc.st.SV) {
			...
			return roachpb.NewErrorf("transaction is too large to commit: %d intents", len(intents))
		}

		// Initialize the first update time and maybe start the heartbeat.
		if tc.mu.firstUpdateNanos == 0 {
			...
			if _, isEnding := ba.GetArg(roachpb.EndTransaction); pErr != nil || !isEnding {
				...
				if tc.rootOrLeaf == client.RootTxn {
					if err := tc.stopper.RunAsyncTask(
