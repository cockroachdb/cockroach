...
type TxnCoordSender struct {
	...
	*TxnCoordSenderFactory
	...
	interceptorStack [5]txnInterceptor
...
type txnLockGatekeeper struct {
	...
	mu      struct {
		sync.Locker // shared with TxnCoordSender

		// everSentBeginTxn is set when the first (of possible multiple)
		// BeginTransaction request is sent. This means that an EndTransaction needs
		// to eventually be sent and cannot be elided.
...
func (tcf *TxnCoordSenderFactory) TransactionalSender(
	...
	everSentBeginTxn := func() bool {
		return tcs.interceptorAlloc.txnLockGatekeeper.mu.everSentBeginTxn
	}
	...
	tcs.interceptorAlloc.txnLockGatekeeper.mu.Locker = &tcs.mu
...
func (tc *TxnCoordSender) GetMeta() roachpb.TxnCoordMeta {
	...
	meta.CommandCount = tc.interceptorAlloc.txnSeqNumAllocator.commandCount
...
func (tc *TxnCoordSender) Send(
	...
	if !tc.mu.active {
		// !!! is this use of ba.UserPriority ok? The doc says ba.UserPriority is
		// ignored if a txn is specified.
		user := roachpb.MakePriority(ba.UserPriority)
		if tc.mu.txn.Priority < user {
			tc.mu.txn.Priority = user
		}
		tc.mu.active = true
	}
	...
	if req, ok := ba.GetArg(roachpb.EndTransaction); ok {
		...
		if etReq.Commit {
			if pErr == nil {
				...
				tc.sleepForLineariazableMaybe(ctx, br, startNs)
			}
		} else {
...
// If the linearizable flag is set, we want to make sure that all the clocks in
// the system are past the commit timestamp of the transaction.  This is
// guaranteed if either:
...
func (tc *TxnCoordSender) sleepForLineariazableMaybe(
...
// BumpEpochAfterConcurrentRetryError is part of the client.TxnSender interface.
func (tc *TxnCoordSender) BumpEpochAfterConcurrentRetryError(ctx context.Context) {
	tc.mu.Lock()
	defer tc.mu.Unlock()

	if tc.mu.txnState == txnFinalized {
		log.Fatalf(ctx, "BumpEpochAfterConcurrentRetryError called on finalized txn: %s", tc.mu.txn)
	}
