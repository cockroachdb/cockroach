...
func TestNodeIDAndObservedTimestamps(t *testing.T) {
	...
	for i, test := range directCases {
		t.Run(fmt.Sprintf("direct-txn-%d", i), func(t *testing.T) {
			...
			kvTxn := roachpb.MakeTransaction("unnamed", nil, isolation.Serializable, roachpb.NormalUserPriority, now.ToTimestamp(), db.Clock().MaxOffset().Nanoseconds(), int32(test.nodeID), 0, false)
