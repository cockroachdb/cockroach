...
// clients. If found and not currently pending or already retried
// once, it is moved to the front, even if it's already been invoked.
func (gt *grpcTransport) MoveToFront(replica roachpb.ReplicaDescriptor) {
	...
	for i, client := range gt.orderedClients {
		if client.args.Replica == replica {
			...
			if client.pending || client.retried {
				return
			}
