...
func TestRangeFeedMock(t *testing.T) {
	...
	t.Run("changefeed retries", func(t *testing.T) {
		...
		mc := mockClient{
			...
			rangefeed: func(
				ctx context.Context, spans []roachpb.Span, startFrom hlc.Timestamp, withDiff bool, eventC chan<- *kvclient.RangeFeedEnvelope,
			) error {
				assert.False(t, withDiff) // it was not set
				sendEvent := func(ts hlc.Timestamp) {
					eventC <- &kvclient.RangeFeedEnvelope{
				...
				case iteration == firstPartialCheckpoint:
					assert.Equal(t, startFrom, initialTS)
					eventC <- &kvclient.RangeFeedEnvelope{RangeFeedEvent: &roachpb.RangeFeedEvent{
...
func TestBackoffOnRangefeedFailure(t *testing.T) {
	...
	db.EXPECT().RangeFeed(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).
		Do(func(context.Context, []roachpb.Span, hlc.Timestamp, bool, chan<- *kvclient.RangeFeedEnvelope) {
			cancel()
		}).
		Return(nil)
