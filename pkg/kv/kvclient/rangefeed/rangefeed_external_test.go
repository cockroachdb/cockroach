...
func TestWithOnSSTable(t *testing.T) {
	...
	sstC := make(chan *kvclient.RangeFeedEnvelope)
	...
	r, err := f.RangeFeed(ctx, "test", spans, db.Clock().Now(),
		...
		rangefeed.WithOnSSTable(func(ctx context.Context, sst *roachpb.RangeFeedSSTable, registeredSpan roachpb.Span) {
			select {
			case sstC <- &kvclient.RangeFeedEnvelope{
	...
	var sstEnvelope *kvclient.RangeFeedEnvelope
	select {
	case sstEnvelope = <-sstC:
	case <-time.After(3 * time.Second):
		require.Fail(t, "timed out waiting for SST event")
	}

	require.Equal(t, roachpb.Span{Key: sstStart, EndKey: sstEnd}, sstEnvelope.SST.Span)
	require.Equal(t, now, sstEnvelope.SST.WriteTS)
	require.Equal(t, sstKVs, storageutils.ScanSST(t, sstEnvelope.SST.Data))
	require.Equal(t, spans[0], *sstEnvelope.RegisteredSpan)
}
