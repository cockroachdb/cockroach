...
func TestRangeFeedMetadata(t *testing.T) {
	...
	testutils.RunValues(t, "feed_type", procTypes, func(t *testing.T, s feedProcessorType) {
		...
		spanMinKey := ts.Codec().TablePrefix(maxTableID)

		// Create a rangefeed that listens to key space above all the system tables, i.e. empty key space.
		...
		initialScanDone := make(chan struct{})
		r, err := f.RangeFeed(ctx, "test", []roachpb.Span{fullSpan}, db.Clock().Now(),
			func(ctx context.Context, value *kvpb.RangeFeedValue) {
			},
			rangefeed.WithInitialScan(func(ctx context.Context) {
				close(initialScanDone)
		...
		sql.Exec(t, `INSERT INTO foo (key) SELECT * FROM generate_series(1, 100)`)
		{
			...
			meta := <-metadata
			require.Equal(t, fullSpan.Key, meta.Span.Key)
			require.Equal(t, fullSpan.EndKey, meta.Span.EndKey)
			require.Equal(t, false, meta.FromManualSplit)
			fmt.Printf("\t new range key span %s-%s; manual split %t \n", meta.Span.Key, meta.Span.EndKey, meta.FromManualSplit)
		}
		...
		for {
			...
			if !meta.Span.EndKey.Equal(fullSpan.EndKey) {
				require.Equal(t, fullSpan.Key, meta.Span.Key)
				break
			}
		}
		{
			...
			require.Equal(t, true, meta.FromManualSplit)
			require.NotEqual(t, fullSpan.Key, meta.Span.Key)
			require.Equal(t, fullSpan.EndKey, meta.Span.EndKey)
		}
	})
}
