...
func TestRangeFeedIntentResolutionRace(t *testing.T) {
	...
	tc := testcluster.StartTestCluster(t, 3, base.TestClusterArgs{
		ReplicationMode: base.ReplicationManual,
		ServerArgs: base.TestServerArgs{
			DefaultTestTenant: base.TODOTestTenantDisabled, // TODO: fix shared test tenant
			Settings:          st,
			Knobs: base.TestingKnobs{
				Store: &kvserver.StoreTestingKnobs{
	...
	_, desc, err := n1.StorageLayer().SplitRange(prefix)
	require.NoError(t, err)
	tc.AddVotersOrFatal(t, prefix, tc.Targets(1, 2)...)
