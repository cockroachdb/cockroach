...
// StartFromFrontier is like Start but allows passing a frontier to reflect any
// previous progress. The rangefeed takes ownership of the passed frontier until
// it is closed; the caller must not interact with it until Close returns. The
// caller remains responsible for releasing the frontier thereafter however.
	...
	if f.withInitialScan {
		...
		for _, sp := range f.spans {
			if _, err := frontier.Forward(sp, f.initialTimestamp); err != nil {
				f.onUnrecoverableError(ctx, err)
				return
			}
		}
	}
