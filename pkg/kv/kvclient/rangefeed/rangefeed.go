...
func (f *RangeFeed) run(ctx context.Context, frontier *span.Frontier) {
	...
	eventCh := make(chan *kvclient.RangeFeedEnvelope)
...
func (f *RangeFeed) processEvents(
	ctx context.Context, frontier *span.Frontier, eventCh <-chan *kvclient.RangeFeedEnvelope,
) error {
	for {
		select {
		case ev := <-eventCh:
			...
			case ev.SST != nil:
				...
				if ev.RegisteredSpan == nil {
					return errors.AssertionFailedf(
						"received rangefeed SSTable does not have registered span set")
				}
