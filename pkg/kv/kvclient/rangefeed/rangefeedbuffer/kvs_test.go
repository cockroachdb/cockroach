...
// TestMergeKVs tests the logic of MergeKVsKeepingDeletions and the
// logic of to extract KVs from a slice of events.
func TestMergeKVsKeepingDeletions(t *testing.T) {
	type row struct {
		key   string
		ts    int64
		value string
	...
	for _, tc := range []testCase{
		...
		require.Equal(t, toKVs(merged), MergeKVsKeepingDeletions(toKVs(a), toKVs(b)))

		// Exercise the conversions out of RangeFeedValue.
		require.Equal(t, toKVs(merged), MergeKVsKeepingDeletions(
			toKVsThroughBuffer(t, a), toKVsThroughBuffer(t, b),
		))
	}
}
