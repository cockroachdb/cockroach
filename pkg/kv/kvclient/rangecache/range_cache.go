...
type EvictionToken struct {
	...
	// The lease can be speculative which means some follower replica knew who the
	// Raft leader was, but did not have any valid lease record and also did not
	// attempt to become the Leaseholder. The leader is the most likely replica to
	// become the next Leaseholder for this range.
	lease *roachpb.Lease
