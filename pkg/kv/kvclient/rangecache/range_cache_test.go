...
func TestRangeCacheSyncTokenAndMaybeUpdateCache(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			testFn: func(t *testing.T, cache *RangeCache) {
				// Check that trying to update the descriptor with something fresher than
				// what is on the token but stale in comparison to what's contained in the
				// cache behaves correctly. Specifically, the (freshest) descriptor from the
				// cache should be on the token.
				cache.Insert(ctx, roachpb.RangeInfo{
					Desc:                  desc1,
					Lease:                 roachpb.Lease{},
					ClosedTimestampPolicy: roachpb.LEAD_FOR_GLOBAL_READS,
