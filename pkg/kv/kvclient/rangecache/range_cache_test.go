...
func TestRangeCacheEntryAge(t *testing.T) {
	db := initTestDescriptorDB(t)
	ctx := context.Background()

	// We should set the value of this delta to be reasonable because there
	// might be thread preemptions or timer delays that cause our timestamps to
	// be slightly different from the ones inside range cache entry.
	expectedDelta := 20 * time.Millisecond

	// We should lock the thread to reduce the chances of scheduling messing
	// with timing-based assertions.
	runtime.LockOSThread()
	defer runtime.UnlockOSThread()

	// Test whether a token reflects the age of the created range cache entry.
	token, err := db.cache.LookupWithEvictionToken(ctx, roachpb.RKey("a"), EvictionToken{}, false)
	require.NoError(t, err)
	require.True(t, token.Valid())
	require.LessOrEqual(t, token.Age(), expectedDelta)

	// Test whether this created token's age progresses linearly with time in
	// the absence of evictions.
	minDur, maxDur := 150*time.Millisecond, 500*time.Millisecond
	expectedAge := minDur + time.Duration(rand.Int())%(maxDur-minDur)
	time.Sleep(expectedAge)
	difference := token.Age() - expectedAge
	require.LessOrEqual(t, difference.Abs(), expectedDelta)

	// Test whether ExtendTTL refreshes the age of a created token.
	token.ExtendTTL()
	require.LessOrEqual(t, token.Age(), expectedDelta)
}
