...
func initTestDescriptorDB(t *testing.T) *testDescriptorDB {
	...
	db.cache = NewRangeCache(st, db, staticSize(2<<10), db.stopper, log.AmbientContext{Tracer: st.Tracer})
	return db
}
...
func TestLookupByKeyMin(t *testing.T) {
	...
	cache := NewRangeCache(st, nil, staticSize(2<<10), stopper, log.AmbientContext{Tracer: st.Tracer})
...
func TestRangeCacheClearOlderOverlapping(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run("", func(t *testing.T) {
			...
			stopper := stop.NewStopper()
			defer stopper.Stop(ctx)
			cache := NewRangeCache(st, nil /* db */, staticSize(2<<10), stopper, log.AmbientContext{Tracer: st.Tracer})
