...
func clearOlderOverlapping(
	...
	ent := newCacheEntry(cacheEntryData{desc: *desc}, 0)
...
func TestRangeCacheClearOverlapping(t *testing.T) {
	...
	cache := NewRangeCache(st, nil, staticSize(2<<10), stopper)
	cache.addEntryLocked(newCacheEntry(cacheEntryData{desc: *defDesc}, 0))
	...
	cache.addEntryLocked(newCacheEntry(cacheEntryData{desc: minToBDesc}, 0))
	...
	cache.addEntryLocked(newCacheEntry(cacheEntryData{desc: bToMaxDesc}, 0))
	...
	cache.addEntryLocked(newCacheEntry(cacheEntryData{desc: defDescCpy}, 0))
	...
	cache.addEntryLocked(newCacheEntry(cacheEntryData{desc: bToCDesc}, 0))
...
func TestRangeCacheClearOlderOverlapping(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run("", func(t *testing.T) {
			...
			newEntry := newCacheEntry(cacheEntryData{desc: tc.clearDesc}, 0)
...
func TestRangeCacheClearOverlappingMeta(t *testing.T) {
	...
	func() {
		...
		cache.clearOlderOverlapping(ctx, newCacheEntry(cacheEntryData{desc: metaSplitDesc}, 0))
	}()
}
...
func TestRangeCacheEntryMaybeUpdate(t *testing.T) {
	...
	e := newCacheEntry(cacheEntryData{
		desc:  desc,
		lease: roachpb.Lease{},
	}, 0)
	...
	updated, updatedLease, e := e.maybeUpdate(ctx, &l, &desc, 0)
	...
	updated, updatedLease, e = e.maybeUpdate(ctx, &l, &staleDesc, 0)
	...
	updated, updatedLease, e = e.maybeUpdate(ctx, &l, &desc2, 0)
	require.True(t, updated)
	require.False(t, updatedLease)
	require.Equal(t, l, e.lease)
	...
	updated, updatedLease, e = e.maybeUpdate(ctx, &l, &desc3, 0)
	require.True(t, updated)
	require.False(t, updatedLease)
...
func TestRangeCacheEntryOverrides(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			a, b := newCacheEntry(tc.a, 0), newCacheEntry(tc.b, 0)
			require.Equal(t, tc.exp, b.overrides(a))
		})
	}
}

func TestRangeCacheEntryLeaseholderContactDuration(t *testing.T) {
	db := initTestDescriptorDB(t)
	ctx := context.Background()

	clock := hlc.NewHybridManualClock()
	db.cache.TestingSetManualClock(clock)
	clock.Pause()

	// Check that a newly fetched token's leaseholder contacted duration is zero.
	token, err := db.cache.LookupWithEvictionToken(ctx, roachpb.RKey("a"), EvictionToken{}, false)
	require.NoError(t, err)
	require.True(t, token.Valid())
	require.Zero(t, token.SinceLeaseholderContacted())

	// Check that the leaseholder contacted duration progresses with time.
	minDur, maxDur := 150*time.Millisecond, 500*time.Millisecond
	expectedAge := minDur + time.Duration(rand.Int())%(maxDur-minDur)
	clock.Increment(expectedAge.Nanoseconds())
	require.Equal(t, token.SinceLeaseholderContacted(), expectedAge)

	// Test whether leaseholder contacted duration is correctly updated.
	token.MarkLeaseholderContacted()
	require.Zero(t, token.SinceLeaseholderContacted())
}

func TestRangeCacheEntryLeaseholderRandomization(t *testing.T) {
	db := initTestDescriptorDB(t)
	ctx := context.Background()

	// Insert a range descriptor cache entry with valid replicas.
	db.cache.Insert(ctx, roachpb.RangeInfo{
		Desc: roachpb.RangeDescriptor{
			StartKey: roachpb.RKey("a"),
			EndKey:   roachpb.RKey("b"),
			InternalReplicas: []roachpb.ReplicaDescriptor{
	...
	require.NoError(t, err)
	require.True(t, token.Valid())
	require.False(t, token.Lease().Empty())

	// Check that the leaseholder is correctly randomized.
	previousLease := token.Lease()
	token.RandomizeLeaseholder(ctx)
	require.NotEqual(t, token.Lease(), previousLease)
}
