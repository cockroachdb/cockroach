...
type avgResponseEstimator struct {
	...
	// numNonResumeResponses tracks the number of "non-resume" responses that
	// have been received so far. A "resume" response is a continuation for the
	// ScanResponse that didn't contain all data within a single BatchRequest.
	// In other words, responses to the same originally-enqueued request are
	// only counted once in this number (although if the request spans multiple
	// ranges, each single-range request is tracked separately here).
	numNonResumeResponses float64
}
...
// ScanRequests independently (rather than the whole multi-range original one).
...
func (e *avgResponseEstimator) getAvgResponseSize() int64 {
	...
	// number of "non-resume" responses we've seen so far. This allows us to
	// handle partial ScanResponses that need to be resumed across
	// BatchRequests without double-counting them in the denominator of the
	// average computation (which would lead to artificially low estimate, see
	// #103586 for an example).
	//
	...
	return int64(e.responseBytes / e.numNonResumeResponses * e.avgResponseSizeMultiple)
}
...
// BatchResponse. numNonResumeResponses indicates the number of "non-resume"
// responses in that BatchResponse.
func (e *avgResponseEstimator) update(responseBytes int64, numNonResumeResponses int) {
	e.responseBytes += float64(responseBytes)
	e.numNonResumeResponses += float64(numNonResumeResponses)
}
