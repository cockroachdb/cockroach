...
func TestStreamerCorrectlyDiscardsResponses(t *testing.T) {
	...
	// scenarios. All of these values allow for all initial requests to be
	// issued asynchronously, but only for some of the responses to be
	// "accepted" by the budget.
	for _, workmem := range []int{
		4 * initialAvgResponseSize * numRows / 3,
		3 * initialAvgResponseSize * numRows / 2,
		5 * initialAvgResponseSize * numRows / 3,
	} {
