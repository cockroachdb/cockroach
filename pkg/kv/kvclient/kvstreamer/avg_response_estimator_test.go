...
func TestAvgResponseEstimator(t *testing.T) {
	...
	e.update(firstResponseSize, 1 /* numNonResumeResponses */)
	...
	for i := 0; i < 100; i++ {
		e.update(smallResponseSize*5, 5 /* numNonResumeResponses */)
	}
	...
	for i := 0; i < 10; i++ {
		e.update(largeResponseSize*1000, 1000 /* numNonResumeResponses */)
	}
	// The estimate should now be pretty close to the size of a single response
...
func TestAvgResponseSizeForPartialResponses(t *testing.T) {
	...
	for ; rowsLeft > 0; batchRequestsCount++ {
		...
		// All responses except for the first one are considered "resume"
		// responses.
		var numNonResumeResponses int
		if batchRequestsCount == 0 {
			numNonResumeResponses = 1
		}
		e.update(rowsReceived*rowSize, numNonResumeResponses)
	}
