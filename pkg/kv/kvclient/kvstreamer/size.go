...
func init() {
	reverseScanRequestOverhead := int64(unsafe.Sizeof(kvpb.RequestUnion_Scan{}) +
		unsafe.Sizeof(kvpb.ScanRequest{}))
...
// getRequestSize calculates the footprint of a Get request, including the
// overhead. key and endKey are the keys from the span of the request header
// (we choose to avoid taking in a roachpb.Span in order to reduce allocation).
func getRequestSize(key, endKey roachpb.Key) int64 {
	return getRequestOverhead + int64(cap(key)) + int64(cap(endKey))
}
...
func requestsMemUsage(reqs []kvpb.RequestUnion) (memUsage int64) {
	for _, r := range reqs {
		...
		case kvpb.Get:
			memUsage += getRequestSize(h.Key, h.EndKey)
