...
type singleRangeBatch struct {
	...
	// Scan(b, c), Scan(c, e), Scan(e, f), and each will get included into a
	// separate singleRangeBatch with the same 'positions' value of 0. However,
	// the subRequestIdx values will be [0], [1], and [2], respectively - thus,
	// we'll be able to order the responses to these requests (which might come
	// back in any order) correctly.
	//
