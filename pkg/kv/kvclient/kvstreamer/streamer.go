...
type Result struct {
	...
	// In InOrder mode a Result can only satisfy multiple consecutive requests.
	EnqueueKeys []int
...
// never split across different BatchResponses when TargetBytes limitBytes is
// exceeded.
...
//
// limitBytes determines the maximum amount of memory this Streamer is allowed to use
...
// return an error if that footprint exceeds the Streamer's limitBytes. As a rule of
// thumb, that footprint should be on the order of MBs, and not tens of MBs.
//
...
func (s *Streamer) Enqueue(
	...
	ri := kvcoord.NewRangeIterator(s.distSender)
...
// results will do. Zero-length result slice is returned once all enqueued
...
// Cancel cancels all in-flight operations and discards all buffered results if
// operating in InOrder mode. It blocks until all goroutines created by the
// Streamer exit.
...
func (w *workerCoordinator) mainLoop(ctx context.Context) {
	...
	for {
		...
		shouldExit := w.issueRequestsForAsyncProcessing(ctx, requestsToServe, avgResponseSize)
		if shouldExit {
			return
		}
	}
...
func (w *workerCoordinator) issueRequestsForAsyncProcessing(
	...
	for numRequestsIssued < len(requestsToServe) {
		...
		availableBudget := w.s.budget.limitBytes - w.s.budget.mu.used
...
func (w *workerCoordinator) performRequestAsync(
	...
	if buildutil.CrdbTestBuild {
		if targetBytesReservationDenied && !headOfLine {
			panic("targetBytesReservationDenied is true when headOfLine is false")
		}
	}
	w.s.waitGroup.Add(1)
	atomic.AddInt32(&w.s.atomics.numRequestsInFlight, 1)
	if err := w.s.stopper.RunAsyncTaskEx(
		...
		func(ctx context.Context) {
			...
			// Since we have already reserved at least 10KiB when creating the
			// budget, we always have some memory reserved on this node ("at
			// source").
				...
				case *roachpb.GetRequest:
					...
					if get.ResumeSpan != nil {
						...
						result := Result{
							...
							// unique.
							EnqueueKeys: []int{w.s.enqueueKeys[req.positions[i]]},
							MemoryTok: &resultMemoryToken{
								toRelease: toRelease,
								budget:    w.s.budget,
							},
