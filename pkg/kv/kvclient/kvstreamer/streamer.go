...
type Result struct {
	...
	// memoryTok describes the memory reservation of this Result that need to be
	// released back to the budget when the Result is Release()'d.
	memoryTok struct {
		budget    *budget
		toRelease int64
	}
...
// Release needs to be called by the recipient of the Result once it's not
// referencing this Result any more. If this was the last (or only) reference to
// this Result, the memory used by this Result is made available in the
// Streamer's budget.
//
