...
func (w *workerCoordinator) performRequestAsync(
	...
	if err := w.s.stopper.RunAsyncTaskEx(
		...
		func(ctx context.Context) {
			...
			var numIncompleteGets, numIncompleteScans int
			...
			if resumeReqsMemUsage > 0 {
				// This addendum is the first step of requestsMemUsage() and
				// we've already added the size of each resume request above.
				//
				// If resumeReqsMemUsage is currently 0, then there are no
				// ResumeSpans.
				resumeReqsMemUsage += requestUnionOverhead * int64(numIncompleteRequests)
			}
			...
			// We have to allocate the new slice for requests, but we can reuse
			// the positions slice.
			resumeReq.reqs = make([]roachpb.RequestUnion, numIncompleteRequests)
			// numIncompleteRequests will never exceed the number of requests in
			// req.
