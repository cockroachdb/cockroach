...
func (r Result) Release(ctx context.Context) {
	if s := r.memoryTok.streamer; s != nil {
		// Note that it is important to decrement numUnreleasedResults first,
		// before releasing the memory - this ensures that the workerCoordinator
		// doesn't wait forever for the budget in case this is the last Result
		// to be released.
		s.mu.Lock()
		s.mu.numUnreleasedResults--
		s.mu.Unlock()
		s.budget.release(ctx, r.memoryTok.toRelease)
	}
}
...
func (w *workerCoordinator) mainLoop(ctx context.Context) {
	...
	for {
		...
		//
		// We have to be this tricky with acquiring the Streamer's mutex and
		// not releasing it in order to not miss the signal on hasWork condition
		// variable.
		w.s.mu.Lock()
		...
		if len(requestsToServe) == 0 {
			// We're still holding the Streamer's mutex.
			w.s.mu.hasWork.Wait()
			w.s.mu.Unlock()
...
// getRequestsLocked returns all currently enqueued requests to be served. It
// assumes that the Streamer's mutex is being held.
//
...
func (w *workerCoordinator) performRequestAsync(
	...
	if err := w.s.stopper.RunAsyncTaskEx(
		...
		func(ctx context.Context) {
			...
			// First, we have to reconcile the memory budget. We have to do it
			// separately from processing the results because we want to reuse
			// the same slices from req for the ResumeSpans, if any are present.
			// However, if we were to do that before performing the memory
			// accounting, we would corrupt the original request in case when
			// headOfLine is false, memory reservation is denied, and we want to
			// put req back to be processed.
			//
			// Note that we cannot use Size() methods that are automatically
			// generated by the protobuf library because they account for things
			// differently from how the memory usage is accounted for by the KV
			// layer for the purposes of tracking TargetBytes limit.
			...
			// resumeReqsMemUsage tracks the memory usage of the requests for
			// the ResumeSpans.
			var resumeReqsMemUsage int64
			...
			if resumeReqsMemUsage > 0 {
				// Note that we'll be reusing the slices, and req.reqs will not
				// be reallocated (it won't shrink nor will it grow because the
				// number of ResumeSpans won't exceed the number of original
				// requests). This addendum is the first step of
				// requestsMemUsage() and we've already added the size of each
				// resume request above.
				//
				// If resumeReqsMemUsage is currently 0, then there are no
				// ResumeSpans, so req will not be referenced anymore, and we
				// don't include the footprint of req.reqs into the memory
				// usage.
				resumeReqsMemUsage += requestUnionOverhead * int64(cap(req.reqs))
			}
			...
			// We've already reconciled the budget with the actual reservation
			// for the requests with the ResumeSpans.
			...
			for i, resp := range br.Responses {
				...
				case *roachpb.ScanRequest:
					...
					if scan.ResumeSpan != nil {
						...
						// request according to the ResumeSpan and include it
						// into the batch again.
						origRequest.SetSpan(*scan.ResumeSpan)
						resumeReq.reqs = append(resumeReq.reqs, origReq)
						resumeReq.positions = append(resumeReq.positions, req.positions[i])
					}
				}
...
func requestsMemUsage(reqs []roachpb.RequestUnion) int64 {
	...
	// No need to account for elements past len(reqs) because those have been
	// zeroed out and we have already accounted for RequestUnion object above.
	for _, r := range reqs {
...
func scanResponseSize(scan *roachpb.ScanResponse) int64 {
	...
	for _, row := range scan.Rows {
		size += len(row.Key) + len(row.Value.RawBytes)
	}
	for _, batchResponse := range scan.BatchResponses {
		size += len(batchResponse)
	}
	return int64(size)
}
