...
func (w *workerCoordinator) performRequestAsync(
	...
	if err := w.s.stopper.RunAsyncTaskEx(
		...
		func(ctx context.Context) {
			...
			// the ResumeSpans. Note that we'll be reusing the slices, and
			// req.reqs will not be reallocated (it won't shrink nor will it
			// grow because the number of ResumeSpans won't exceed the number of
			// original requests). This calculation is the first step of
			// requestsMemUsage and we'll be adding the size of each resume
			// request as we go.
