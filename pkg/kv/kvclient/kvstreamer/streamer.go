...
//  s.Cancel()
//
// The Streamer builds on top the BatchRequest API provided by the DistSender
...
// thumb though, the footprint of reqs should be on the order of MBs, and not
// tens of MBs.
//
...
func (s *Streamer) Enqueue(
	...
	// calls to Enqueue are not allowed until previous requests have been fully
	// processed.
...
// Result.EnqueueKeys to understand which request the result corresponds to. For
...
func (w *workerCoordinator) issueRequestsForAsyncProcessing(
	...
	for numRequestsIssued < len(requestsToServe) && !budgetIsExhausted {
		...
		if err := w.s.budget.consumeLocked(ctx, targetBytes, allowDebt); err != nil {
			...
			// that the root memory pool has been exhausted, so we'll exit to be
			// safe (in order not to OOM the node).
			w.s.setError(err)
			return true
		}
...
func (w *workerCoordinator) performRequestAsync(
	...
	if err := w.s.stopper.RunAsyncTaskEx(
		...
		func(ctx context.Context) {
			...
			respOverestimate := targetBytes - actualMemoryReservation
