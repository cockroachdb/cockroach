...
func (tc *TxnCoordSender) CreateSavepoint(ctx context.Context) (kv.SavepointToken, error) {
	...
	//
	// We also increment the write sequence if any writes are buffered. For
	// SQL-generated transactions, this additional check is unnecessary since we
	// won't ever have buffered writes wihout also at least 1 locked row. However,
	// for other types of KV transactions such as those generated by KVNemesis we
	// may have blind Put ro Delete requests that would be erroneously rolled back
	// if we didn't increment the sequence here.
