...
type ClientTestingKnobs struct {
	...
	// to the first replica in the transport list when the policy is LEASEHOLDER.
	// The leaseholder may still be the first replica it sends the request to if
	// it is also the closed replica. Requests that are targeted for the
	// leaseholder will instead be proxied to it. This parameter is typically not
	// set for tests and instead is controlled by
