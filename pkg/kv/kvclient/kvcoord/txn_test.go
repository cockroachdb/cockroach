...
// TestRetrySerializableBumpsToNow verifies that transaction is restarted on now rather than
// closed timestamp to give it enough time to retry.
...
func TestRetrySerializableBumpsToNow(t *testing.T) {
	...
	bumpTsCache := func(delay time.Duration) {
		s.Manual.Increment(delay.Nanoseconds())
		// We make an unrelated put which advances ts cache as a side effect
	...
	require.NoError(t, s.DB.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
		...
		bumpTsCache(delay)
		attempt++
		// Doing scan to populate txn read span
		_, _ = txn.Scan(ctx, roachpb.Key("a"), roachpb.Key("p"), 1000)
