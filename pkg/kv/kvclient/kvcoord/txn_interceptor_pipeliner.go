...
func (tp *txnPipeliner) SendLocked(
	...
	// request; even if the check passes, we might end up over budget.
	...
	condenseOpt := condenseLocksIfOverBudget
	if rejectOverBudget {
		// If we were configured to reject transaction when they go over budget, we
		// don't want to condense the lock spans (even if it turns out that we did
		// go over budget). The point of the rejection setting is to avoid
		// condensing because of the possible performance cliff when doing so. As
		// such, if we did go over budget despite the earlier pre-emptive check,
		// then we stay over budget. Further requests will be rejected if they
		// attempt to take more locks.
		condenseOpt = neverCondenseLocks
	}
	tp.updateLockTracking(ctx, ba, br, maxBytes, condenseOpt)
	if pErr != nil {
...
//
// We can only estimate what what spans this request would end up locking
...
func (tp *txnPipeliner) maybeRejectOverBudget(ba roachpb.BatchRequest, maxBytes int64) error {
	...
	// want to permit commits, since the damage in taking too much memory has
	// already been done.
	if !ba.IsLocking() {
		return nil
	}
...
type overBudgetOpt bool

const (
	condenseLocksIfOverBudget overBudgetOpt = false
	neverCondenseLocks                      = true
)
...
// over, the behavior depends on the overBudgetOpt - we either compress the
// spans with loss of fidelity (which can be a significant performance problem
// at commit/rollback time) or we don't compress (and we stay over budget).
//
...
func (tp *txnPipeliner) updateLockTracking(
	...
	if tp.lockFootprint.Bytes() <= locksBudget {
		return
	}
	...
	if overBudgetOpt == condenseLocksIfOverBudget {
...
func encodeLockSpansOverBudgetError(
	...
	details = &errorspb.StringsPayload{
		Details: []string{
			strconv.Itoa(int(t.lockSpansBytes)), strconv.Itoa(int(t.limitBytes)), t.baSummary, t.txnDetails,
		},
	}
