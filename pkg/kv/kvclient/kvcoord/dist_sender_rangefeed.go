...
var catchupScanConcurrency = settings.RegisterIntSetting(
	...
	"number of concurrent catchup scans that can execute concurrently; 0 disables",
	8,
...
func (ds *DistSender) RangeFeed(
	...
	catchupLimit := maxConcurrentCatchupScans(&ds.st.SV)
	catchupSem := limit.MakeConcurrentRequestLimiter("distSenderCatchupLimit", catchupLimit)
