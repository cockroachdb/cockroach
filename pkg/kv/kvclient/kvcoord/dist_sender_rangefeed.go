...
// and returns activeRangeFeed along with the cleanup function.
...
// connection; this may involve instructing higher-level functions to transient
...
func handleRangefeedError(ctx context.Context, err error) (rangefeedErrorInfo, error) {
	if err == nil {
		return rangefeedErrorInfo{restart: true}, nil
	}
	...
	case errors.HasType(err, (*kvpb.StoreNotFoundError)(nil)) ||
		...
		// transient.
		return rangefeedErrorInfo{transient: true}, nil
	...
	case errors.HasType(err, (*kvpb.RangeKeyMismatchError)(nil)):
		return rangefeedErrorInfo{evict: true, restart: true}, nil
