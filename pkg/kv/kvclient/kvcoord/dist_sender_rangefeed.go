...
func (ds *DistSender) ForEachActiveRangeFeed(fn ActiveRangeFeedIterFn) (iterErr error) {
	...
	partialRangeFeed := func(active *activeRangeFeed) (p PartialRangeFeed) {
		active.Lock()
		defer active.Unlock()
...
func newActiveRangeFeed(
	...
	active.release = func() {
		...
		active.Lock()
		defer active.Unlock()
		if active.Resolved.IsSet() {
			// Range reached post-catchup phase.
			metrics.RangefeedPostCatchupRanges.Dec(1)
		}
	}
...
func handleRangefeedError(
	...
	case errors.HasType(err, (*kvpb.RangeFeedRetryError)(nil)):
		...
		case kvpb.RangeFeedRetryError_REASON_REPLICA_REMOVED,
			...
			metrics.RangefeedRetryErrors[t.Reason].Inc(1)
			return rangefeedErrorInfo{}, nil
			...
			case *kvpb.RangeFeedCheckpoint:
				if t.Span.Contains(args.Span) {
					...
					if !t.ResolvedTS.IsEmpty() && active.catchupRes != nil {
						active.releaseCatchupScan()
						metrics.RangefeedPostCatchupRanges.Inc(1)
					}
