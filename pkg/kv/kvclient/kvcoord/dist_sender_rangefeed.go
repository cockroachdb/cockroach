...
func (a *activeRangeFeed) onRangeEvent(nodeID roachpb.NodeID, event *roachpb.RangeFeedEvent) {
	a.Lock()
	defer a.Unlock()
	if event.Val != nil {
...
func (ds *DistSender) partialRangeFeed(
	...
	active.PartialRangeFeed = PartialRangeFeed{
		Span:    span,
		StartTS: ts,
	}
...
type onRangeEventCb func(nodeID roachpb.NodeID, event *roachpb.RangeFeedEvent)
...
func (ds *DistSender) singleRangeFeed(
	...
	for {
		...
		for {
			...
			onRangeEvent(args.Replica.NodeID, event)
