...
// A "kill switch" to disable streaming rangefeed if severe issues discovered with new implementation.
var enableStreamingRangefeed = envutil.EnvOrDefaultBool("COCKROACH_ENABLE_STREAMING_RANGEFEED", true)
...
func (ds *DistSender) RangeFeed(
	...
	if ds.st.Version.IsActive(ctx, clusterversion.RangefeedUseOneStreamPerNode) &&
		enableStreamingRangefeed {
		return ds.startStreamingRangeFeed(ctx, spans, startFrom, withDiff, rr, eventCh)
	}
...
func newActiveRangeFeed(
	rr *rangeFeedRegistry, span roachpb.Span, startTS hlc.Timestamp,
) *activeRangeFeed {
	a := &activeRangeFeed{}
	a.mu.PartialRangeFeed.Span = span
	a.mu.StartTS = startTS
...
func (a *activeRangeFeed) setRangeID(rangeID roachpb.RangeID) {
	a.mu.Lock()
	defer a.mu.Unlock()
	a.mu.RangeID = rangeID
}
...
func (ds *DistSender) singleRangeFeed(
	...
	active.setRangeID(desc.RangeID)
