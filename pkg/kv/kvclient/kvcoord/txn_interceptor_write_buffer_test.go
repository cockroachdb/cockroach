...
func TestTxnWriteBufferLockingReadsTransformations(t *testing.T) {
	...
	validateScan := func(str lock.Strength, dur lock.Durability, value *roachpb.Value) func(t *testing.T, ba *kvpb.BatchRequest) *kvpb.BatchResponse {
		return func(t *testing.T, ba *kvpb.BatchRequest) *kvpb.BatchResponse {
			require.Len(t, ba.Requests, 1)
			scanReq := ba.Requests[0].GetScan()
			require.NotNil(t, scanReq)
	...
	testCases := []testCase{
		...
		},
		{
			...
			validateTransformedRequestAndGenerateResponse: validateScan(lock.Exclusive, lock.Unreplicated, &valueA),
			validateFlushedRequest:                        expectGetWithEndTxn(lock.Exclusive, lock.Replicated),
		},
		{
			...
			validateTransformedRequestAndGenerateResponse: validateScan(lock.Shared, lock.Unreplicated, &valueA),
			validateFlushedRequest:                        expectGetWithEndTxn(lock.Shared, lock.Replicated),
		},
		{
