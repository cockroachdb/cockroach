...
func TestTxnWriteBufferElidesUnnecessaryLockingRequests(t *testing.T) {
	...
	type lockingRequests struct {
		...
		// requiresValue, when true, indicates that this request requires.
		requiresValue bool
		...
		// is different from requiresValue in that requests that aren't fully
		// covered must
		isFullyCovered bool
		...
		for _, secondReq := range reqs {
			t.Run(fmt.Sprintf("%s followed by %s", firstReq.name, secondReq.name), func(t *testing.T) {
				if !firstReq.buffersLock {
					t.Skipf("%s does not buffer its lock", firstReq.name)
				}
				if secondReq.requiresValue && !firstReq.buffersValue {
					t.Skipf("%s requires a value but %s does not buffer its response", firstReq.name, secondReq.name)
				}
