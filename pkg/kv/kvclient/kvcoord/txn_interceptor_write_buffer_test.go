...
func TestTxnWriteBufferLockingGetFlushing(t *testing.T) {
	...
	const (
		sharedLockingGet    = "shared-lock"
		exclusiveLockingGet = "exclusive-lock"
		put                 = "put"
	...
	testCases := []testCase{
		...
		{
			ops: []string{
				sharedLockingGet,    // This is elided because a write exists and no savepoint requires it.
				exclusiveLockingGet, // This is elided because a write exists and no savepoint requires it.
				put,
			},
			midTxn:             true,
			validateFinalBatch: expectSingleWrite,
		...
		{
			ops: []string{
				sharedLockingGet,    // This is elided because a write is also below savepoint.
				exclusiveLockingGet, // This is elided because a write is also below savepoint.
				put,
				createSavepoint,
			},
			midTxn:             true,
		...
		{
			ops: []string{
				sharedLockingGet, // This is elided because a stronger lock is also below the savepoint.
				exclusiveLockingGet,
				createSavepoint,
			},
			midTxn:             true,
		...
		},
		{
			ops: []string{
				createSavepoint,
				put,
				sharedLockingGet, // This is elided because write is at a lower sequence.
			},
			midTxn:             true,
			validateFinalBatch: expectSingleWrite,
		},
		...
		// Rollback special cases
		{
			ops: []string{
				createSavepoint,
				sharedLockingGet, // This should be rolled back.
				rollbackSavepoint,
			},
			validateFinalBatch: func(t *testing.T, ba *kvpb.BatchRequest) {
				require.Len(t, ba.Requests, 1)
	...
	addGet := func(ba *kvpb.BatchRequest, str lock.Strength, seq enginepb.TxnSeq) {
		ba.Add(&kvpb.GetRequest{
			KeyLockingStrength:   str,
			KeyLockingDurability: lock.Replicated,
			RequestHeader: kvpb.RequestHeader{
	...
	for _, tc := range testCases {
		...
		t.Run(name, func(t *testing.T) {
			...
			for _, op := range tc.ops {
				...
				switch op {
				case sharedLockingGet:
					ba := &kvpb.BatchRequest{Header: kvpb.Header{Txn: &txn}}
					addGet(ba, lock.Shared, txn.Sequence)
					br, pErr := twb.SendLocked(ctx, ba)
					require.NotNil(t, br)
					require.Nil(t, pErr)
				case exclusiveLockingGet:
					ba := &kvpb.BatchRequest{Header: kvpb.Header{Txn: &txn}}
					addGet(ba, lock.Exclusive, txn.Sequence)
					br, pErr := twb.SendLocked(ctx, ba)
					require.NotNil(t, br)
					require.Nil(t, pErr)
				case put:
