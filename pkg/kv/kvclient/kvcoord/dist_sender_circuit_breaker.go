...
func (r *ReplicaCircuitBreaker) launchProbe(report func(error), done func()) {
	...
	err := r.d.stopper.RunAsyncTask(ctx, name, func(ctx context.Context) {
		...
		for {
			...
			// requests come in.
			probeInterval := CircuitBreakerProbeInterval.Get(&r.d.settings.SV).Nanoseconds()
			if r.lastRequest.Load() < timeutil.Now().UnixNano()-cbProbeIdleIntervals*probeInterval {
				return
			}
		}
	})
