...
// unfortunately not quite as simply), it returns updated information (besides
...
// passed-in lease was compatible with the descriptor and more recent that the
// cached lease).
	...
	resC, leader := rdc.lookupRequests.DoChan(requestKey, func() (interface{}, error) {
		...
		if err := rdc.stopper.RunTaskWithErr(ctx, "rangecache: range lookup", func(ctx context.Context) error {
			...
			for i := 0; i < len(preRs); i++ {
				newEntries[i+1] = &kvbase.RangeCacheEntry{Desc: preRs[i]}
			}
