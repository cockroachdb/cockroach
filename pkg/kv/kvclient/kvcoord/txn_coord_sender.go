...
// refreshAncestor on its immediate parent after committing. The logic to
// invoke this behavior lives both in the txnSpanRefresher and in the fast-path
// of the txnCoordSender for read-only transactions. This logic is important
// for two reasons:
//
...
//     error.
//
// Note that if the commitCommitTimestamp is equal to the parent's current
...
func (tc *TxnCoordSender) commitReadOnlyTxnLocked(
	...
	return roachpb.NewError(tc.pushParentUponCommitLocked(ctx))
}
...
func (tc *TxnCoordSender) updateStateLocked(
	...
	if pErr == nil {
		// If this response corresponds to committing the transaction, make sure
		// to push the parent.
		justCommitted := tc.mu.txn.Status != roachpb.COMMITTED &&
