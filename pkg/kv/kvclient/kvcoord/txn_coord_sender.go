...
func (tc *TxnCoordSender) updateStateLocked(
	...
	if errTxn := pErr.GetTxn(); errTxn != nil {
		// Most errors cause the transaction to not accept further request, but some
		// errors are safe to allow continuing (in particular ConditionFailedError).
		// In particular, SQL can recover by rolling back to a savepoint.
