...
func (b *blockRecvRangeFeedClient) Recv() (*roachpb.RangeFeedEvent, error) {
	if !b.blocked {
		...
		<-ctx.Done()
		b.blocked = true
		b.reason = contextutil.GetCancelReason(ctx)
		return nil, ctx.Err()
	}
...
func TestRestartsStuckRangeFeeds(t *testing.T) {
	...
	tc := testcluster.StartTestCluster(t, 1, base.TestClusterArgs{
		ServerArgs: base.TestServerArgs{
			Knobs: base.TestingKnobs{
				KVClient: &kvcoord.ClientTestingKnobs{
					TransportFactory: makeTransportFactory(true, nil, wrapRfClient),
				},
			},
		},
	})
	...
	sqlDB.ExecMultiple(t,
		`SET CLUSTER SETTING kv.rangefeed.enabled = true`,
		`SET CLUSTER SETTING kv.rangefeed.range_liveness_threshold='500ms'`,
		`CREATE TABLE foo (key INT PRIMARY KEY)`,
	...
	require.Regexp(t, "something", blockingClient.reason)
}
