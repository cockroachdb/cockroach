...
func TestRangeFeedMetricsManagement(t *testing.T) {
	...
	testutils.RunTrueAndFalse(t, "mux", func(t *testing.T, useMux bool) {
		...
		defer func() {
			...
			require.True(t, numRangesToRetry <= metrics.RangefeedErrorCatchup.Count())
			require.True(t, numRangesToRetry <= metrics.RangefeedRestartRanges.Count())
		...
		var numRetried atomic.Int32
		var numCatchupBlocked atomic.Int32
		skipSet := struct {
			syncutil.Mutex
