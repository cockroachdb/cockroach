...
func (c *testRangefeedClient) RangeFeedStream(
	...
	return c.InternalClient.RangeFeedStream(ctx, opts...)
}
...
func (c *countConnectionsTransport) NextInternalClient(
	...
	tc := &testRangefeedClient{
		...
		rfStreamEnabled: c.rfStreamEnabled,
		count:           countFn,
	}
	return ctx, tc, nil
}
...
func rangeFeed(
	...
	withCancel := func(err error) error {
		cancel()
		onError(err)
		return err
	}
	...
	g.GoCtx(func(ctx context.Context) (err error) {
		return withCancel(ds.RangeFeed(ctx, []roachpb.Span{sp}, startFrom, false, events))
	})
	g.GoCtx(func(ctx context.Context) error {
		for {
			select {
			case <-ctx.Done():
				return withCancel(ctx.Err())
			case ev := <-events:
				onValue(ev)
			}
		}
...
// rangeFeedOnError returns a error channel and an onError handler for rangefeed call.
func rangeFeedOnError() (chan error, func(err error)) {
	errCh := make(chan error, 1)
	return errCh, func(err error) {
		select {
		case errCh <- err:
		default:
		}
...
func TestBiDirectionalRangefeedNotUsedUntilUpgradeFinalilzed(t *testing.T) {
	...
	runRangeFeed := func(tc *testcluster.TestCluster) {
		...
		errCh, onError := rangeFeedOnError()
		closeFeed := rangeFeed(ts.DistSenderI(), fooSpan, startTime, onValue, onError)
		select {
		case <-allSeen:
	...
	t.Run("rangefeed-stream-disabled-via-environment", func(t *testing.T) {
		defer kvcoord.TestingSetEnableStreamingRangefeed(false)()
...
func TestBiDirectionRangeFeedStreamConnectsToNodeOnce(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)
