	...
	for _, test := range testCases {
		t.Run(fmt.Sprintf("%t-%d", test.useProxy, test.numServers),
			func(t *testing.T) {
				...
				kvpb.ProxyBatchRequest.Override(ctx, &st.SV, test.useProxy)
							...
							perNode[i] = base.TestServerArgs{
								...
								Knobs: base.TestingKnobs{
									Server: &server.TestingKnobs{
										...
										ContextTestingKnobs:       p.CreateTestingKnobs(roachpb.NodeID(i+1), test.partition),
									},
								},
							}
						}
				...
				if rangeFeedsEnabled {
					...
					for i := 0; i < numServers; i++ {
						...
						w := rangefeedcache.NewWatcher(
							...
							func(ctx context.Context, value *kvpb.RangeFeedValue) rangefeedbuffer.Event { return value },
