...
func (m *rangefeedMuxer) shutdownWithError(terminalErr error) error {
	...
	for _, p := range m.mu.producers {
		// Signal producer termination if needed.  We must take care not to
		// close the channel twice since this producer may have already been notified
		// via propagateStreamTerminationErrorToConsumers.
		if p.termErr == nil {
