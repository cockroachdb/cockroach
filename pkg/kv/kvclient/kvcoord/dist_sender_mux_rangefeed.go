...
// existing (e.g. because of node shutdown), and an attempt to start rangefeed on that node.
...
func (m *nodeMuxer) clearActive(key rangeStateKey) error {
	...
	return errors.AssertionFailedf("expected to find active range feed %v, found none", key)
}
...
func (m *nodeMuxer) receive(ctx context.Context) error {
	for {
		...
		if err := m.clearActive(
			rangeStateKey{streamID: event.StreamID, rangeID: event.RangeID},
		); err != nil {
			return err
		}
