...
func (m *rangefeedMuxer) startMuxRangeFeed(
	...
	req.StreamID = atomic.AddInt64(&m.nextStreamID, 1)
...
func (m *rangefeedMuxer) establishMuxConnection(
	...
	if !found {
		ms = &muxClientState{initCtx: makeTerminationContext(), doneCtx: makeTerminationContext()}
...
func (m *rangefeedMuxer) startNodeMuxRangeFeed(
	...
	ms.client, ms.initCtx.err = client.MuxRangeFeed(ctx)
	close(ms.initCtx.done)

	defer close(ms.doneCtx.done)
	if ms.initCtx.err == nil {
		ms.doneCtx.err = m.receiveEventsFromNode(ms)
	}
...
func makeTerminationContext() terminationContext {
	return terminationContext{done: make(chan struct{})}
}

func (tc *terminationContext) Done() <-chan struct{} {
	return tc.done
}
func (tc *terminationContext) Err() error {
	return tc.err
}
