...
type rangefeedMuxer struct {
	...
	// muxClient is a nodeID -> *establishedMuxNodeConn
...
// muxNodeConn represents MuxRangeFeed RPC established a node.
type muxNodeConn struct {
...
// connOrError is a tuple of node connection or an error that occurred while
...
func (m *rangefeedMuxer) startSingleRangeFeed(
	...
	// Start a transient loop for sending the batch to the range.
...
func (m *rangefeedMuxer) establishMuxConnection(
	...
	var muxClient *future.Future[connOrError]

	{
		ptr, exists := m.muxClients.Load(int64(nodeID))
		if !exists {
			ptr, exists = m.muxClients.LoadOrStore(int64(nodeID), unsafe.Pointer(future.MakePromise[connOrError]()))
		}
		muxClient = (*future.Future[connOrError])(ptr)
		if !exists {
...
func (m *rangefeedMuxer) startNodeMuxRangeFeed(
	...
	if recvErr := m.receiveEventsFromNode(ctx, nodeID, stuckWatcher, nodeConn); recvErr != nil {
		...
		return nodeConn.closeWithRestart(func(_ int64, a *activeMuxRangeFeed) error {
...
func (m *rangefeedMuxer) receiveEventsFromNode(
	...
	for {
		if err := stuckWatcher.do(func() (err error) {
			event, err = conn.receiver.Recv()
			return err
		}); err != nil {
			return err
		}

		active := conn.lookupStream(event.StreamID)
		...
		case *kvpb.RangeFeedError:
			...
			conn.streams.Delete(event.StreamID)
			if _, err := handleRangefeedError(ctx, t.Error.GoError()); err != nil {
				// If this is an error we cannot recover from, terminate the rangefeed.
				return err
			}
...
func (c *muxNodeConn) startRangeFeed(
...
func (c *muxNodeConn) lookupStream(streamID int64) *activeMuxRangeFeed {
...
func (c *muxNodeConn) closeWithRestart(
...
func (c *muxNodeConn) eachStream(fn func(streamID int64, a *activeMuxRangeFeed) error) error {
