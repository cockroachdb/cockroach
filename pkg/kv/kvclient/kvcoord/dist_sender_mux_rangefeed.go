...
func (m *rangefeedMuxer) startNodeMuxRangeFeed(
	...
	if recvErr := m.receiveEventsFromNode(ctx, mux, stuckWatcher, &ms); recvErr != nil {
		...
		m.restartActiveRangeFeeds(recvErr, toRestart...)
	}
...
func (m *rangefeedMuxer) receiveEventsFromNode(
	...
	for {
		...
		case *kvpb.RangeFeedError:
			...
			m.restartActiveRangeFeeds(t.Error.GoError(), active)
			continue
		}
		...
		if timeutil.Now().Before(nextStuckCheck) {
			if threshold := stuckThreshold(); threshold > 0 {
				m.restartActiveRangeFeeds(errRestartStuckRange, ms.purgeStuckStreams(threshold)...)
			}
			nextStuckCheck = timeutil.Now().Add(stuckCheckFreq())
		}
	}
...
func (m *rangefeedMuxer) restartActiveRangeFeeds(reason error, toRestart ...*activeMuxRangeFeed) {
	// Restart rangefeed on another goroutine.  There are two reasons for doing this
	...
	//   2. The context passed to this function may be canceled if
...
func (c *muxStream) close() []*activeMuxRangeFeed {
	c.mu.Lock()
	c.mu.closed = true
	toRestart := func() []*activeMuxRangeFeed {
		active := make([]*activeMuxRangeFeed, 0, len(c.mu.streams))
