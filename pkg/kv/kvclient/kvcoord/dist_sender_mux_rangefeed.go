...
// muxer is responsible for coordination and management of mux rangefeeds.
// muxer caches MuxRangeFeed stream per node, and executes each range feed request
...
func newMuxer(g ctxgroup.Group) *muxer {
	m := &muxer{
...
var _ rangeFeedEventProducer = (*channelRangeFeedEventProducer)(nil)

// startMuxRangeFeed begins the execution of rangefeed for the specified RangeFeedRequest.
func (m *muxer) startMuxRangeFeed(
	...
	if !found {
		var err error
		stream, err = rpc.InternalClient.MuxRangeFeed(rpc.ctx)
		if err != nil {
			return nil, err
		}
...
func (m *muxer) demux(ctx context.Context) error {
	for {
		select {
		case <-ctx.Done():
			return ctx.Err()
		case e := <-m.eventCh:
			...
			if !found {
				return m.shutdownWithError(errors.AssertionFailedf(
					"expected to find consumer for node %d, streamID=%d", e.StreamID),
				)
			}
...
func (m *muxer) receiveEventsFromNode(
...
func (m *muxer) propagateStreamTerminationErrorToConsumers(
...
// shutdownWithError terminates this muxer with a terminal error (usually an assertion failure).
// It's a bit unwieldy way to propagate this error to the caller, but as soon as  one of consumers
// notices this terminal error, the context should be cancelled.
// Returns the terminal error passed in.
func (m *muxer) shutdownWithError(terminalErr error) error {
