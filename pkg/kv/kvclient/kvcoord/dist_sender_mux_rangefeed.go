...
type streamState struct {
	...
	mu struct {
		syncutil.Mutex
		activeFeeds map[roachpb.RangeID]rangeState
	}
}

func (ss *streamState) getRangeState(rangeID roachpb.RangeID) rangeState {
	ss.mu.Lock()
	defer ss.mu.Unlock()
	return ss.mu.activeFeeds[rangeID]
}

func (ss *streamState) clearActive(active *activeRangeFeed, rangeID roachpb.RangeID) {
	ss.mu.Lock()
	defer ss.mu.Unlock()
	delete(ss.mu.activeFeeds, rangeID)
	active.release()
}
...
func (ss *streamState) markActive(
	...
	if ss.mu.activeFeeds == nil {
		ss.mu.activeFeeds = make(map[roachpb.RangeID]rangeState)
	}

	rangeID := token.Desc().RangeID
	span := rSpan.AsRawSpanWithNoLocals()

	delete(ss.mu.activeFeeds, rangeID) // markActive can be called multiple times due to retries
	active := newActiveRangeFeed(rr, span, startFrom)
	active.setNodeID(nodeID)
	active.setRangeID(rangeID)

	ss.mu.activeFeeds[rangeID] = rangeState{
...
func (rfs *rfStream) startRangeFeedStream(
	...
	for r := retry.StartWithCtx(ctx, rfs.ds.rpcRetryOptions); r.Next(); {
		...
		rangeID := token.Desc().RangeID
		ss.markActive(rfs.rr, replica.NodeID, token, rs, startFrom)
...
func (rfs *rfStream) rfStreamEventProcessor(ctx context.Context, ss *streamState) error {
	for {
		...
		if eventErr == nil {
			...
			rs = ss.getRangeState(event.RangeID)
			if rs.activeRangeFeed == nil {
				return errors.AssertionFailedf(
					"unexpected state: expected to find active range feed for %d, found none",
					event.RangeID)
			}
		}
		...
		ss.clearActive(rs.activeRangeFeed, event.RangeID)
