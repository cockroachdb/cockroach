...
var bufferedWritesScanTransformEnabled = settings.RegisterBoolSetting(
	settings.ApplicationLevel,
	"kv.transactions.write_buffering.transformations.scans.enabled",
...
func (twb *txnWriteBuffer) estimateSize(ba *kvpb.BatchRequest, transformScans bool) int64 {
	...
	for _, ru := range ba.Requests {
		...
		case *kvpb.ScanRequest:
			...
			// about the size of the scan start boundary.
			//
			// Then we try to protect from large buffer overflows by transforming the
			// batch's MaxSpanRequestKeys later.
...
func (twb *txnWriteBuffer) applyTransformations(
	...
	for i, ru := range ba.Requests {
		...
		case *kvpb.ReverseScanRequest:
			// Regardless of whether the reverse scans overlaps with any writes in the
	...
	if hasTransformedLockingScan {
		twb.mutateBatchMaxSpanRequestKeys(ctx, baRemote, transformedLockingScanKeySizeEstimate)
	}

	return baRemote, rr, nil
}

// mutateBatchMaxSpanRequestKeys limits MaxSpanRequestKeys to protect from the
...
// the same batch.
func (twb *txnWriteBuffer) mutateBatchMaxSpanRequestKeys(
	...
	// requests in a transformed batch.
	for _, ru := range ba.Requests {
		req := ru.GetInner()
		switch req.(type) {
	...
	if ba.MaxSpanRequestKeys != targetKeys {
...
func (rr requestRecord) toResp(
	...
	case *kvpb.ScanRequest:
		...
		if rr.transformed {
			// TODO(ssd): We do two passes over the ScanResponse to avoid mutating the
			// buffer while also iterating over the buffer below.
			respIter := newScanRespIter(origReq, resp)
			for ; respIter.valid(); respIter.next() {
				// TODO(ssd): Do we need to clone here?
				twb.addDurableLockedReadToBuffer(respIter.peekKey().Clone(), &bufferedDurableLockAcquisition{
					str: req.KeyLockingStrength,
					seq: req.Sequence,
					ts:  txn.ReadTimestamp,
				})
