...
type txnWriteBuffer struct {
	...
	// conditional puts. Indended only for tests.
...
// Get's response. The Get is sent to KV and the Put is buffered locally. The
// Put is only buffered, and eventually flushed to KV, if the condition
// evaluates successfully. Otherwise, a ConditionFailedError is returned.
//
