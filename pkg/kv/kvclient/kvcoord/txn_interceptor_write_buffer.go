...
func (bw *bufferedWrite) toRequest() kvpb.RequestUnion {
	...
	// However, when flushing to KV, we often only need to flush the most recent
	// write (read: the one with the highest sequence number). As we store values
	// in increasing sequence number order, this should be the last value in the
	// slice.
	return bw.vals[len(bw.vals)-1].toRequestUnion(bw.key)
}

// toAllRevisionRequests returns requests for all revisions of all keys in the
// buffer. When the buffer is flushed before the end of a transaction, all
// revisions must be written to storage to ensure that a future rollback is
// properly handled.
