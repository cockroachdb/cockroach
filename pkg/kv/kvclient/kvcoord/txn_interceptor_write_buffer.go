...
func (twb *txnWriteBuffer) SendLocked(
	...
	if err := twb.validateBatchRequest(ba); err != nil {
...
// batchOptionSupported returns an error if the batch is unsupported
// by the txnWriteBuffer.
func (twb *txnWriteBuffer) validateBatchRequest(ba *kvpb.BatchRequest) error {
	...
	return twb.validateBatchRequests(ba)
}

// validateBatchRequests returns an error if any of the requests in the batch
// are unsupported by the txnWriteBuffer.
func (twb *txnWriteBuffer) validateBatchRequests(ba *kvpb.BatchRequest) error {
