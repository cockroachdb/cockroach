...
func TestErrorIndexOnRangeSplit(t *testing.T) {
	...
	// We'll send a BatchRequest touching the original two ranges, which it so
	...
	var injectErr atomic.Bool
	injectErr.Store(true)
	transportFn := func(_ context.Context, ba *kvpb.BatchRequest) (*kvpb.BatchResponse, error) {
		...
		if injectErr.Load() {
			...
			// If this is the batch with 2 requests, then inject an error to
			// simulate a stale range cache.
			switchToSplitDesc()
			injectErr.Store(false)
		...
		// When we're not injecting errors, we'll receive a few batches, all
		// with 1 Get. Find the one targeting keyB and simulate it encountering
		// an error (that could occur for InitPuts).
