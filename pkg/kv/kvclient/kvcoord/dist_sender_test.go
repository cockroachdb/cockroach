...
func benchDistSender(b *testing.B, rf int, numRange int, conc bool) {
	...
	// Create a singleton range for each key.
	rddb := MockRangeDescriptorDB(func(key roachpb.RKey, reverse bool) (
		[]roachpb.RangeDescriptor, []roachpb.RangeDescriptor, error,
	) {
		var desc = roachpb.RangeDescriptor{
	...
	transportFactory := adaptSimpleTransport(
		func(ctx context.Context, args *kvpb.BatchRequest) (*kvpb.BatchResponse, error) {
			...
			correctNode := int(rangeId) % rf
			key := roachpb.RKey(fmt.Sprintf("%d", rangeId))
			reply := args.CreateReply()
			if correctNode != int(nodeId) {
				rangeDesc, _, _ := rddb.RangeLookup(ctx, key, kvpb.CONSISTENT, false)
				lease := &roachpb.Lease{
					Sequence: 1,
					Replica:  rangeDesc[0].InternalReplicas[correctNode],
				}
	...
	cfg := DistSenderConfig{
		...
		NodeIDGetter:      func() roachpb.NodeID { return 1 },
		Stopper:           stopper,
		TransportFactory:  transportFactory,
		...
		LatencyFunc: func(id roachpb.NodeID) (time.Duration, bool) {
			// Randomize the latency so there is some time spent sorting.
			return time.Duration(id * 7919 % 10), true
		},
		Settings: st,
	}
	ds := NewDistSender(cfg)
	...
	for i := 0; i < numRange; i++ {
		get := kvpb.NewGet(roachpb.Key(fmt.Sprintf("%d", i%numRange)))
	...
	b.ResetTimer()
	// This loop will cycle through all the ranges and request a key from each of
	// them. It runs multiple goroutines which are each running concurrently to
	// also test any locking contention.
	numProcs := 1
	if conc {
	...
	for i := 0; i < numProcs; i++ {
		go func() {
			defer wg.Done()
			// Divide iterations by numProcs to get accurate time per operation.
