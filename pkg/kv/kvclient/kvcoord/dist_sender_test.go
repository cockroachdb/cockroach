...
func (mdb MockRangeDescriptorDB) withMetaRecursion(rdc *rngcache.RangeCache) MockRangeDescriptorDB {
	return func(key roachpb.RKey, useReverseScan bool) (rs, preRs []roachpb.RangeDescriptor, err error) {
		...
		if !metaKey.Equal(roachpb.RKeyMin) {
			_, err := rdc.LookupWithEvictionToken(context.Background(), metaKey, rngcache.EvictionToken{}, useReverseScan)
			if err != nil {
				return nil, nil, err
			}
		}
...
func (ds *DistSender) withMetaRecursion() *DistSender {
	ds.rangeCache.SetDB(ds.rangeCache.DB().(MockRangeDescriptorDB).withMetaRecursion(ds.rangeCache))
	return ds
}
...
func TestEvictOnFirstRangeGossip(t *testing.T) {
	...
	call := func() {
		if _, err := ds.rangeCache.LookupWithEvictionToken(
			context.Background(), rAnyKey, rngcache.EvictionToken{}, false,
		); err != nil {
			t.Fatal(err)
		}
	}
...
func TestSendToReplicasSkipsStaleReplicas(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			rc := rngcache.NewRangeDescriptorCache(st, nil /* db */, getRangeDescCacheSize, stopper)
