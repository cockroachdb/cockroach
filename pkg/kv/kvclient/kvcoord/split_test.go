...
func TestSplitPredicates(t *testing.T) {
	...
	require.NoError(t, s.DB.AdminSplit(ctx, roachpb.Key("b"), expire, roachpb.AdminSplitRequestIngestion))
	require.NoError(t, s.DB.AdminSplit(ctx, roachpb.Key("g"), expire, roachpb.AdminSplitRequestIngestion))
	// c is below split key f, and is in [b, g).
	require.NoError(t, s.DB.AdminSplit(ctx, roachpb.Key("f"), expire, roachpb.AdminSplitRequestIngestion, roachpb.Key("c")))
	// e is above split key d, and is in [b, f).
	require.NoError(t, s.DB.AdminSplit(ctx, roachpb.Key("d"), expire, roachpb.AdminSplitRequestIngestion, roachpb.Key("e")))
	// b is above split key c, and is in [b, d) although just barely.
	require.NoError(t, s.DB.AdminSplit(ctx, roachpb.Key("c"), expire, roachpb.AdminSplitRequestIngestion, roachpb.Key("b")))

	// Setup another known span [g, n) and test rejections with it.
	require.NoError(t, s.DB.AdminSplit(ctx, roachpb.Key("n"), expire, roachpb.AdminSplitRequestIngestion))

	// Reject split at h that wanted b to be in range [g, n).
	require.Error(t, s.DB.AdminSplit(ctx, roachpb.Key("h"), expire, roachpb.AdminSplitRequestIngestion, roachpb.Key("b")))
	// Reject split at h that wanted i, j, and z to be in range [g, n).
	require.Error(t, s.DB.AdminSplit(ctx, roachpb.Key("h"), expire, roachpb.AdminSplitRequestIngestion, roachpb.Key("i"), roachpb.Key("j"), roachpb.Key("z")))
	// Reject split at h that wanted i, j, and n to be in range [g, n).
	require.Error(t, s.DB.AdminSplit(ctx, roachpb.Key("h"), expire, roachpb.AdminSplitRequestIngestion, roachpb.Key("i"), roachpb.Key("j"), roachpb.Key("n")))
	// Reject split at h that wanted i, n and j to be in range [g, n).
	require.Error(t, s.DB.AdminSplit(ctx, roachpb.Key("h"), expire, roachpb.AdminSplitRequestIngestion, roachpb.Key("i"), roachpb.Key("n"), roachpb.Key("j")))

	// Allow split at h that wanted i, k and j to be in range [g, n).
	require.NoError(t, s.DB.AdminSplit(ctx, roachpb.Key("h"), expire, roachpb.AdminSplitRequestIngestion, roachpb.Key("i"), roachpb.Key("k"), roachpb.Key("j")))
}
