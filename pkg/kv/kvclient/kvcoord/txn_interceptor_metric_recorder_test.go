...
func TestTxnMetricRecorder(t *testing.T) {
	...
	check := func(t *testing.T, tm *txnMetricRecorder, m metrics) {
		...
		assert.Equal(t, m.duration != 0, tm.metrics.Durations.TotalSum() != 0, "TxnMetrics.Durations")
		assert.Equal(t, m.restarts != 0, tm.metrics.Restarts.TotalSum() != 0, "TxnMetrics.Restarts")
	}
