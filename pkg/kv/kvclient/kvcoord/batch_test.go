...
func BenchmarkTruncateNext(b *testing.B) {
	...
	for _, numRequests := range []int{128, 16384} {
		for _, numRanges := range []int{4, 64} {
			for _, keyLength := range []int{1, 32} {
				splitPoints := make([][]byte, numRanges-1)
				for i := range splitPoints {
					splitPoints[i] = randomKey(keyLength)
				}
				for i := range splitPoints {
				...
				for _, requestType := range []string{"get", "scan"} {
					b.Run(fmt.Sprintf(
						"reqs=%d/ranges=%d/keyLength=%d/type=%s",
						numRequests, numRanges, keyLength, requestType,
					), func(b *testing.B) {
						...
						case "scan":
							for i := 0; i < numRequests; i++ {
								...
								startKey := randomKey(keyLength)
								endKey := randomKey(keyLength)
