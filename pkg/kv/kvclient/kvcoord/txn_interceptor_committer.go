...
func (tc *txnCommitter) SendLocked(
	...
	// Assert that, given that we're in the STAGING state, the WriteTooOld flag
	// isn't set. If the wto flag were set, it'd be a problem since the refresher
	// interceptor would attempt to retry the request, which is problematic after
	// a txn is implicitly committed (which it now is since the EndTxn batch
	// succeeded). The WriteTooOld flag shouldn't be set, however, even if the
	// batch was split by the DistSender and some of the sub-batches returned with
	// this flag set: since needTxnRetryAfterStaging() above didn't force a retry,
	// we know that, even if some sub-batches were pushed, the EndTxn sub-batch
	// was pushed higher than all the others and it also succeeded in performing a
	// server-side refresh - so it couldn't return the wto flag. Having the
	...
	if br.Txn.WriteTooOld {
		log.Fatalf(ctx, "unexpected WriteTooOld set on STAGING txn: %s", br.Txn)
	}
	if !br.Txn.ReadTimestamp.Equal(br.Txn.WriteTimestamp) {
		log.Fatalf(ctx, "unexpected diverged timestamps on STAGING txn: %s", br.Txn)
	}
...
func assertTxnCleanWhenCommitted(ctx context.Context, txn *roachpb.Transaction) {
	if txn.WriteTooOld {
		log.Fatalf(ctx, "unexpected WriteTooOld set on COMMITTED txn: %s", txn)
	}
	if !txn.ReadTimestamp.Equal(txn.WriteTimestamp) {
		log.Fatalf(ctx, "unexpected diverged timestamps on COMMITTED txn: %s", txn)
	}
}
