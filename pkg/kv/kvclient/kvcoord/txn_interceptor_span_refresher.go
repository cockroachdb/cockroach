...
type txnSpanRefresher struct {
	...
	// memory budget was exceeded). When set, refreshFootprint is empty.
	// This is only set when importing read spans from a 19.2 leaf. It can be
	// removed completely in 20.2. Although maybe we should still be using it when
	...
	// failed to refresh after having lost fidelity in their reads tracking
	// because they exceed the MaxRefreshSpanBytes.
...
func (sr *txnSpanRefresher) SendLocked(
	...
	if !sr.refreshInvalid {
		...
		// TODO(andrei): We should remember what size we got after condensing, and
		// if it's still really large, we shouldn't condense again and again on
		// every request.
		if condensed := sr.refreshFootprint.maybeCondense(
			ctx, sr.riGen, MaxTxnRefreshSpansBytes.Get(&sr.st.SV),
		); condensed {
...
func (sr *txnSpanRefresher) importLeafFinalState(
	...
	if tfs.RefreshInvalid {
		...
		sr.refreshFootprint.insertMultiple(tfs.RefreshSpans)
...
func (sr *txnSpanRefresher) rollbackToSavepointLocked(ctx context.Context, s savepoint) {
	sr.refreshFootprint.clear()
	sr.refreshFootprint.insertMultiple(s.refreshSpans)
