...
func newRetryErrorOnFailedPreemptiveRefresh(txn *roachpb.Transaction, err *kvpb.Error) *kvpb.Error {
	...
	if err != nil {
		if refreshErr, ok := err.GetDetail().(*kvpb.RefreshFailedError); ok {
			if refreshErr.ConflictingTxn != nil {
				conflictingTxn = refreshErr.ConflictingTxn
			}
			msg.Printf(" due to %s", refreshErr)
		} else if wiErr, ok := err.GetDetail().(*kvpb.WriteIntentError); ok {
			if len(wiErr.Intents) > 0 {
				conflictingTxn = &wiErr.Intents[0].Txn
			}
			msg.Printf(" due to %s", wiErr)
		} else {
			msg.Printf(" - unknown error: %s", err)
		}
	}
...
func (sr *txnSpanRefresher) tryRefreshTxnSpans(
	...
	// encountered a committed value) or a WriteIntentError (if it encountered an
