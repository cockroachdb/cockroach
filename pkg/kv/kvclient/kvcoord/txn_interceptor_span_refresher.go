...
func (sr *txnSpanRefresher) sendLockedWithRefreshAttempts(
	...
	if pErr == nil && br.Txn.WriteTooOld {
		...
		pErr = kvpb.NewErrorWithTxn(kvpb.NewTransactionRetryError(kvpb.RETRY_WRITE_TOO_OLD, "WriteTooOld flag converted to WriteTooOldError", nil), bumpedTxn)
		br = nil
	}
	if pErr != nil {
...
func newRetryErrorOnFailedPreemptiveRefresh(
	...
	if refreshErr != nil {
		if refreshErr, ok := refreshErr.GetDetail().(*kvpb.RefreshFailedError); ok {
			...
			// TODO(aadityas): consider adding txn ID for conflicting -- 	conflictingTxn.ID.Short()
			msg.Printf(" due to a conflict: %s on key %s", refreshErr.FailureReason(), refreshErr.Key)
		} else {
			msg.Printf(" - unknown error: %s", refreshErr)
		}
	}
	retryErr := kvpb.NewTransactionRetryError(reason, msg.RedactableString(), conflictingTxn)
