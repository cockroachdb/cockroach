...
// startStreamingRangeFeed starts rangefeed using streaming RangeFeedStream RPC.
func (ds *DistSender) startStreamingRangeFeed(
	ctx context.Context,
	spans []roachpb.Span,
...
type streamState struct {
	...
	stream roachpb.Internal_RangeFeedStreamClient
...
func (rfs *rfStream) startRangeFeedStream(
	...
	for r := retry.StartWithCtx(ctx, rfs.ds.rpcRetryOptions); r.Next(); {
		...
		if err != nil {
			if IsSendError(err) {
				// Evict and re-resolve routing info.
				if err := evictAndResolve(); err != nil && !rangecache.IsRangeLookupErrorRetryable(err) {
					return err
				}
				continue
			}
...
func (rfs *rfStream) lookupOrCreateStream(
	...
	stream, err := client.RangeFeedStream(clientCtx)
	if err != nil {
		return nil, roachpb.ReplicaDescriptor{}, false, err
	}
...
func (rfs *rfStream) restartAllActive(ctx context.Context, ss *streamState) error {
	...
	for _, active := range ss.mu.activeFeeds {
		if err := rfs.restartRange(ctx, active.rSpan, active.restartFrom(), active.token); err != nil {
			return err
		}
	}
...
func (rfs *rfStream) restartRange(
	ctx context.Context, rs roachpb.RSpan, restartFrom hlc.Timestamp, token rangecache.EvictionToken,
) error {
	// Evict token, and re-resolve the span
	token.Evict(ctx)
...
func (rfs *rfStream) rfStreamEventProcessor(ctx context.Context, ss *streamState) error {
	for {
		...
		if eventErr == nil {
			...
			rs.onRangeEvent(event)

			select {
			case rfs.eventsCh <- event:
			case <-ctx.Done():
				return ctx.Err()
			}
			continue
		...
		case restartRange:
			if err := rfs.restartRange(ctx, rs.rSpan, restartFrom, rs.token); err != nil {
				return err
			}
		default:
