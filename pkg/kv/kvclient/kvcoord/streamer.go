...
// consume draws bytes from the available budget. An error is returned iff the
// root pool budget is used up such that the budget's limit cannot be fully
// reserved.
// TODO: think more whether we want the caller to know whether the reservation
// was truncated.
...
func (w *workerCoordinator) performRequestAsync(
	...
	if err := w.s.stopper.RunAsyncTaskEx(
		...
		func(ctx context.Context) {
			...
			// non-empty responses. Resume spans, if any, are handled
			// separately.
			targetBytesReservation := targetBytes
			if targetBytesReservationDenied {
				targetBytesReservation = 0
			}
			if actualMemoryReservation < targetBytesReservation {
				w.s.budget.release(targetBytesReservation - actualMemoryReservation)
			} else if actualMemoryReservation > targetBytesReservation {
				if err := w.s.budget.consume(ctx, actualMemoryReservation-targetBytesReservation); err != nil {
