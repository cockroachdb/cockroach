...
func TestTxnCoordSenderHeartbeat(t *testing.T) {
	...
	if err := s.DB.AdminSplit(ctx, splitKey, hlc.MaxTimestamp, roachpb.AdminSplitRequest_Ingestion); err != nil {
		t.Fatal(err)
	}
...
func TestDB_PrepareForRetryAfterHeartbeatFailure(t *testing.T) {
	...
	keyC := roachpb.Key("c")
	splitKey := roachpb.Key("b")
	require.NoError(t, s.DB.AdminSplit(ctx, splitKey, hlc.MaxTimestamp, roachpb.AdminSplitRequest_Ingestion))
...
func TestWTOBitTerminatedOnErrorResponses(t *testing.T) {
	...
	require.NoError(t, db.AdminSplit(ctx, keyB, hlc.MaxTimestamp, roachpb.AdminSplitRequest_Ingestion))
