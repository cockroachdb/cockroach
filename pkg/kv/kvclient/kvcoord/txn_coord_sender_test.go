...
// those cases the state of the handle ends up at txnRetryableError.
...
func TestDB_PrepareForRetryAfterHeartbeatFailure(t *testing.T) {
	...
	testutils.SucceedsSoon(t, func() error {
		// Calling GetLeafTxnFinalState() in order to trigger
		// maybeRejectClientLocked(), which will poison the handle if there is a
		// heartbeat failure.
		// Note that we can't call something like txn.Get() here because if we do so
		// before the heartbeat failure, then maybeRejectClientLocked() will return
		// nil and we will fail in Send(), and then the txn will be poisoned. This
		// is the common path and this test tries to verify that the poisoning
		// happens due to a heartbeat failure. On the other hand
		// GetLeafTxnFinalState() calls maybeRejectClientLocked() without trying a
		// Send().
		_, err := tc.GetLeafTxnFinalState(ctx, kv.OnlyPending)
		if err == nil {
