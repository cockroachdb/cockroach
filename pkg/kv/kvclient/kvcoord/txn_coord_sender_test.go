...
// TestTxnForceRefresh verifies that TxnCoordSender's ForceRefresh method
// works as expected.
func TestTxnForceRefresh(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)
	...
	var cases = []testCase{
		...
		},
		{
			...
			run: func(
				...
				// Now a refresh should be a no-op.
				require.NoError(t, txn.ForceRefresh(ctx))
			},
		},
	}
