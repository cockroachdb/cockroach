...
func (m *DistSenderKVInterceptorMetrics) ensureNetworkPathMetrics(
...
func makeLocalityLabelValues(baseLocality, fromLocality, toLocality roachpb.Locality) []string {
	...
	length := int(math.Min(
		float64(len(baseLocality.Tiers)),
		math.Min(float64(len(fromLocality.Tiers)), float64(len(toLocality.Tiers))),
	))
	for i := 0; i < length; i++ {
