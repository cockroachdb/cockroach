...
func (ds *DistSender) divideAndSendParallelCommit(
	...
	var qiIsReverse bool
	if isReverse {
		// Avoid checking whether the QueryIntent-only batch is reversed unless
		// the original batch is too.
	...
	if isReverse {
		// Avoid checking whether the remaining batch is reversed unless the
		// original batch is too.
		isReverse = ba.IsReverse()
	}
