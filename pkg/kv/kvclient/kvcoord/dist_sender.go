...
// acceptable values for the consistency argument are STALE_READ_UNCOMMITTED
// or READ_UNCOMMITTED. These consistency levels are used because we want the
// scan to return intents as well as committed values. The reason for this is
// that it's not clear whether the intent or the previous value points to the
// correct location of the Range. It gets even more complicated when there are
// split-related intents or a txn record co-located with a replica involved in
// the split. Since we cannot know the correct answer, we look up both the
...
func (ds *DistSender) RangeLookup(
	...
	switch rc {
	case roachpb.STALE_READ_UNCOMMITTED, roachpb.READ_UNCOMMITTED:
	default:
