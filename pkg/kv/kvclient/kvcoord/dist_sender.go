...
func (ds *DistSender) deduceRetryEarlyExitError(ctx context.Context, err error) error {
	...
	case <-ds.rpcRetryOptions.Closer:
		return errors.Wrap(kvpb.NewAmbiguousResultError(err), "aborted in DistSender (node unavailable)")
	case <-ctx.Done():
		return errors.Wrap(kvpb.NewAmbiguousResultError(err), "aborted in DistSender (context canceled)")
	default:
	}
	return nil
}
