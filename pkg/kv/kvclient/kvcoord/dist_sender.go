...
type DistSenderConfig struct {
	...
	FirstRangeProvider FirstRangeProvider
	RangeDescriptorDB  rngcache.RangeDescriptorDB
...
func NewDistSender(cfg DistSenderConfig) *DistSender {
	...
	var rdb rngcache.RangeDescriptorDB
	...
	ds.rangeCache = rngcache.NewRangeDescriptorCache(ds.st, rdb, getRangeDescCacheSize, cfg.RPCContext.Stopper)
...
func (ds *DistSender) RangeDescriptorCache() *rngcache.RangeCache {
...
func (ds *DistSender) getRoutingInfo(
	ctx context.Context, descKey roachpb.RKey, evictToken rngcache.EvictionToken, useReverseScan bool,
) (rngcache.EvictionToken, error) {
	returnToken, err := ds.rangeCache.LookupWithEvictionToken(
		ctx, descKey, evictToken, useReverseScan,
	)
	if err != nil {
		return rngcache.EvictionToken{}, err
	}
...
func (ds *DistSender) sendPartialBatchAsync(
	...
	ba roachpb.BatchRequest,
	rs roachpb.RSpan,
	routing rngcache.EvictionToken,
	withCommit bool,
	batchIdx int,
...
func (ds *DistSender) sendPartialBatch(
	...
	var prevTok rngcache.EvictionToken
	for r := retry.StartWithCtx(ctx, ds.rpcRetryOptions); r.Next(); {
		...
		if routing.Empty() {
			...
			if err != nil {
				...
				pErr = roachpb.NewError(err)
				if !rngcache.IsRangeLookupErrorRetryable(err) {
					return response{pErr: roachpb.NewError(err)}
				}
				continue
			}
...
func (ds *DistSender) sendToReplicas(
	ctx context.Context, ba roachpb.BatchRequest, routing rngcache.EvictionToken, withCommit bool,
...
func skipStaleReplicas(
	transport Transport, routing rngcache.EvictionToken, ambiguousError error, lastErr error,
) error {
