	...
	for ; ri.Valid(); ri.Seek(ctx, seekKey, scanDir) {
		...
		if canParallelize && !lastRange && !ds.disableParallelBatches &&
			...
			if mightStopEarly {
				...
				// do this after checking the limits, so that they take precedence -- if
				// none of the responses have a resume span but the limits were exceeded
				// above then we must have exhausted the range and so the generated
