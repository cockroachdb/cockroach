...
// only send to a follower if the leaseholder is unavailable.  If this flag is
// set to false, then it will route the request using the default sorting logic.
// This means that most requests will not go to the leaseholder first, and
// instead be sent to the leaseholder through a follower using a proxy request.
// This setting is only intended for testing to stress proxy behavior.
