...
func TestingSetEnableStreamingRangefeed(enabled bool) func() {
	old := enableStreamingRangefeed
	enableStreamingRangefeed = enabled
	return func() {
		enableStreamingRangefeed = old
	}
}
...
func TestDistSenderRangeFeedRetryOnTransportErrors(t *testing.T) {
	...
	for _, useStreamingRangefeed := range []bool{false, true} {
		enableStreamingRangefeed = useStreamingRangefeed
		...
		for _, spec := range []struct {
			...
			t.Run(fmt.Sprintf("streaming=%t/%s", useStreamingRangefeed, spec.errorCode),
				func(t *testing.T) {
					...
					if spec.expectRetry {
						...
						if useStreamingRangefeed {
							stream := roachpb.NewMockInternal_RangeFeedStreamClient(ctrl)
							stream.EXPECT().Send(gomock.Any()).Return(nil)
							stream.EXPECT().Recv().Do(cancel).Return(nil, io.EOF)
							client.EXPECT().RangeFeedStream(gomock.Any()).Return(stream, nil)
						} else {
