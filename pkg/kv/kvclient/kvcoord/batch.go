...
func (h *BatchTruncationHelper) truncateAsc(rs roachpb.RSpan) ([]kvpb.RequestUnion, []int, error) {
	headers := h.headers[h.startIdx:]
	positions := h.positions[h.startIdx:]
	requests := h.requests[h.startIdx:]
	isRange := h.isRange[h.startIdx:]
	...
	for i := range positions {
		...
		if !isRange[i] {
			...
			// processed".
			truncReqs = append(truncReqs, requests[i])
