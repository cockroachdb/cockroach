...
// corresponding update to the Proxy's cached NodeDescriptor set. It contains
// similar logic to Gossip.updateNodeAddress.
func (p *Proxy) updateNodeAddress(ctx context.Context, key string, content roachpb.Value) {
	...
	if desc.NodeID == 0 || desc.Address.IsEmpty() {
		nodeID, err := gossip.NodeIDFromKey(key, gossip.KeyNodeIDPrefix)
		if err != nil {
			log.Errorf(ctx, "unable to update node address for removed node: %s", err)
			return
		}
		delete(p.nodeDescs, nodeID)
	} else {
