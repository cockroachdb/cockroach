...
type ProberOpts struct {
	...
	// This option is optional & should be left unset in production.  It is public
	// for use from integration tests. It configures the production planner.
	// See plan.go for detailed docs.
...
var (
	...
	metaReadProbeFailureDistribution = metric.Metadata{
		...
		// Normalized location in rangespace ([0, 1]).
		// O implies query to first range is failing.
		// 1 implies query to last range is failing.
		// No good unit for this so metric.Unit_UNSET.
...
func (p *Prober) probe(ctx context.Context, db db) {
	...
	// TODO(josh): What we set this timeout to likely needs more thought.
