			...
			Store: &kvserver.StoreTestingKnobs{
				TestingRequestFilter: func(i context.Context, ba roachpb.BatchRequest) *roachpb.Error {
					...
					if !dbIsAvailable {
						for _, ru := range ba.Requests {
							key := ru.GetInner().Header().Key
							// This exclusion keep planning working. An above test breaks
...
func TestPlannerMakesPlansCoveringAllRanges(t *testing.T) {
    ...
    rangeIDToTimesWouldBeProbed[int64(plan.RangeID)]++
		...
		for i := int64(1); i <= numRanges; i++ {
			// Expect all ranges to eventually be included in the plan exactly one
			// time.
...
func initTestProber(t * testing.T, knobs base.TestingKnobs) (serverutils.TestServerInterface, *gosql.DB, *kvprober.Prober, func()) {
	...
	p := kvprober.NewProber(kvprober.ProberOpts{
		...
		PlanNProbesAtATime: 10, // given small test cluster, this better exercises the planning logic
	})
