...
// Step is a decision on what range to readProbe next, including info needed by
...
// planner abstracts deciding on ranges to readProbe. It is public to integration
...
// of steps. A Step is a decision on what range to readProbe next.
//
...
// of the meta2Planner.
func newMeta2Planner(db *kv.DB, settings *cluster.Settings, happyInterval func() time.Duration) *meta2Planner {
	return &meta2Planner{
		db:       db,
		settings: settings,
...
func meta2KVsToPlanImpl(kvs []kv.KeyValue) ([]Step, error) {
	...
	for i, kv := range kvs {
		...
		if rangeDesc.RangeID == 1 {
			plans[i].Key = keys.RangeProbeKey(rangeDesc.StartKey.Next())
		} else {
