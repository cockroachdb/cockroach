...
// TestTxnCommitterAttachesTxnKey tests that the txnCommitter attaches the
// pipelined writes that have yet to be proven and the new writes that are
// part of the same batch as an EndTransaction request to the EndTransaction
// request as "in-flight" writes.
func TestTxnCommitterAttachesInFlightWrites(t *testing.T) {
	defer leaktest.AfterTest(t)()
	ctx := context.Background()
	tc, mockSender := makeMockTxnCommitter()
	defer tc.stopper.Stop(ctx)
	...
	// Attach IntentSpans to each EndTransaction request to avoid the elided
	// EndTransaction optimization. These intents correspond to the writes
	// performed in the test.
	intents := []roachpb.Span{{Key: keyA}, {Key: keyB}}

	// Verify that the QueryIntent and the Put are both attached as in-flight
	// writes to the committing EndTransaction request when expected.
	var ba roachpb.BatchRequest
	ba.Header = roachpb.Header{Txn: &txn}
	...
	etArgs := roachpb.EndTransactionRequest{Commit: true, IntentSpans: intents}
	qiArgs.Txn.Sequence = 1
	putArgs.Sequence = 2
	etArgs.Sequence = 3
	ba.Add(&putArgs, &qiArgs, &etArgs)
	...
	mockSender.MockSend(func(ba roachpb.BatchRequest) (*roachpb.BatchResponse, *roachpb.Error) {
		...
		require.Equal(t, 0, len(et.InFlightWrites))
	...
	mockSender.MockSend(func(ba roachpb.BatchRequest) (*roachpb.BatchResponse, *roachpb.Error) {
		...
		et := ba.Requests[2].GetInner().(*roachpb.EndTransactionRequest)
		require.True(t, et.Commit)
		require.Equal(t, 2, len(et.InFlightWrites))
		require.Equal(t, roachpb.Key(keyA), et.IntentSpans[et.InFlightWrites[1]].Key)
		require.Equal(t, roachpb.Key(keyB), et.IntentSpans[et.InFlightWrites[2]].Key)
		et.InFlightWrites = nil // reset so variable can be reused
	...
	ba.Add(&delRngArgs, &qiArgs, &etArgs)
...
func TestTxnCommitterAsyncExplicitCommitTask(t *testing.T) {
	...
	mockSender.MockSend(func(ba roachpb.BatchRequest) (*roachpb.BatchResponse, *roachpb.Error) {
		...
		require.True(t, et.Commit)
		require.Equal(t, 1, len(et.InFlightWrites))
		require.Equal(t, roachpb.Key(keyA), et.IntentSpans[et.InFlightWrites[1]].Key)
