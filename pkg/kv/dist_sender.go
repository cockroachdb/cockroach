...
func (ds *DistSender) divideAndSendBatchToRanges(
	...
	var resumeReason roachpb.ResumeDetails_ResumeReason
	defer func() {
		if r := recover(); r != nil {
			...
			if mightStopEarly {
				...
				if ba.MaxSpanRequestKeys > 0 {
					...
					if ba.MaxSpanRequestKeys == 0 {
						...
						resumeReason = roachpb.KEY_LIMIT
						return
					}
				}
				...
				if stopAtRangeBoundary &&
					...
					couldHaveSkippedResponses = true
					resumeReason = roachpb.RANGE_BOUNDARY
					return
				}
			}
		}
...
func fillSkippedResponses(
	...
	resumeReason roachpb.ResumeDetails_ResumeReason,
) {
	...
	for i, resp := range br.Responses {
		...
		hdr.ResumeDetails = &roachpb.ResumeDetails{
			Reason: resumeReason,
		}
		origSpan := req.Header()
		if isReverse {
