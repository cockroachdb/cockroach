...
func (ds *DistSender) divideAndSendParallelCommit(
	...
	// This effectively creates a split point between the two groups of
	// requests.
	//
	//  Before:    [put qi put del qi qi et]
	//  After:     [put qi put del et qi qi]
	//  Separated: [put qi put del et] [qi qi]
	//
	// NOTE: the non-pre-commit QueryIntent's must remain where they are in
	// the batch. These ensure that the transaction always reads its writes.
	// These will introduce pipeline stalls and undo most of the benefit of
	// this method, but luckily they are rare in practice.
