...
var rangeLookupPrefetchCount = settings.RegisterIntSetting(
	"kv.range_descriptor_cache.lookup_prefetch_count",
	"maximum number of range descriptors to prefetch during range lookups",
	8,
)
...
func (ds *DistSender) RangeLookup(
	...
	return client.LookupRange(ctx, ds, key.AsRawKey(), roachpb.INCONSISTENT,
		rangeLookupPrefetchCount.Get(&ds.st.SV), useReverseScan)
}

// rangeLookupCompat implements the rangeDescriptorDBCompat interface. The
...
func (ds *DistSender) rangeLookupCompat(
	...
	return client.LookupRangeCompat(ctx, sender, key.AsRawKey(), roachpb.INCONSISTENT,
		rangeLookupPrefetchCount.Get(&ds.st.SV), useReverseScan)
}
