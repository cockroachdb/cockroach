...
func TestRestartDueToAbortedParentTransactionDetectedByAncestor(t *testing.T) {
	...
	indexKeyPrefix, err := rowenc.EncodeTableKey(
	...
	err = interceptedTxn.ChildTxn(ctx, func(_ context.Context, childTxn *kv.Txn) error {
		...
		require.Truef(t, errors.HasType(err, (*roachpb.AncestorAbortedError)(nil)), "%T", err)
		return err
	})
	if retry := (*roachpb.TransactionRetryWithProtoRefreshError)(nil); !errors.As(err, &retry) {
		t.Fatalf("expected %T, got %T",
			(*roachpb.TransactionRetryWithProtoRefreshError)(nil), err)
	} else {
