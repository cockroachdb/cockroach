...
func TestRollbackAfterAmbiguousCommit(t *testing.T) {
	...
	for _, testCase := range testCases {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			// DistSender to return a joint ambiguous and context cancelled
			// error. The transaction will be committed, through.
			...
			cancelCommit()
			commitErr := <-commitCh
			require.ErrorContains(t, commitErr, "result is ambiguous")
