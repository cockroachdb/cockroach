...
// insertRangeDescriptorsLocks is like InsertRangeDescriptors, but it assumes
...
func (rdc *RangeDescriptorCache) insertRangeDescriptorsLocked(
	...
	for i := range rs {
		...
		notPresent, err := rdc.clearOverlappingCachedRangeDescriptors(ctx, &rs[i])
		if err != nil || !notPresent {
			return err
		}
