...
type rangeDescriptorDBCompat interface {
	...
	// rangeLookupCompat takes a key to look up descriptors for and a descriptor
	...
	//
	// Implementations of rangeLookupCompat do not need to properly handle meta2
...
func (rdc *RangeDescriptorCache) performRangeLookup(
	...
	if !rdc.st.Version.IsActive(cluster.VersionMeta2Splits) {
		return rdc.performRangeLookupCompat(ctx, key, useReverseScan)
	}
...
func (rdc *RangeDescriptorCache) performRangeLookupCompat(
	...
	// Will panic if not provided a rangeDescriptorDBCompat.
	compatDB := rdc.db.(rangeDescriptorDBCompat)
	return compatDB.rangeLookupCompat(ctx, key, desc, useReverseScan)
}
