...
func TestTargetRegistryWithGossipTargetDiscovery(t *testing.T) {
	// test setup
	skip.UnderShort(t) // This test takes >10s to run.
	...
	pgURL, _ := pgUrl(tc.Server(0).ServingSQLAddr(), pgurl.TransportNone())
	...
	r.pollingInterval = 20 * time.Millisecond
	r.Start(ctx)
