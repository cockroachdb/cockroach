...
// pull data from the respective target. The leasing state is stored in the sync
// CRDB cluster.
//
...
// session and heartbeats it throughout the node's lifetime. A session is used
...
//     // Another session stole the lease. Watch that other session or otherwise
//     // periodically try to re-acquire the lease.
//   }
...
// is, say, 0.4s, then we're guaranteed to have exlusive access to the resource
// for (1s - 0.4s = 0.6s).
//
...
// 2. s1 becomes unable to heartbeat, for whatever reason, and its Expiration time passes
// 3.	l1.Expiration passes
// 4. l2, err := s2.lease(ctx, 42 /* targetID */)
// 5. s1 is able to heartbeat again
// 6. s1.Lease() returns ???
//
// We want s1.Lease to return ErrLeasedByAnother in step 6 (since there's another conflicting
// lease - l2). The deletion of s1 that happens in step 4 cause the heartbeat
// in step 5 to notice that the session record is missing, and to recreate it
// with an incremented epoch.
//
// Note that, if there was no step 4 and nobody stole any of s1's leases will s1
// was invalid, then, once s1 becomes valid again, all of its leases are again
// valid. That's a good thing: if a session expired briefly but nobody takes a
// conflicting lease, then we can allow all of the session's leases to be used
// again (in fact, it's possible that nobody ever realized that the session was
// temporarily expired). However, if other session steals *any* leases, then the
// expired session will be forced to renew *all* its leases when it becomes
// valid again.
//
