...
func TestLeasing(t *testing.T) {
	...
	mgr1 := NewSession(uuid.MakeV4(), pool, clock, stop)
	require.NoError(t, mgr1.Start())
	mgr2 := NewSession(uuid.MakeV4(), pool, clock, stop)
	require.NoError(t, mgr2.Start())

	// First session succeeds in taking a lease.
	l1, err := mgr1.Lease(ctx, 42)
	require.NoError(t, err)
	...
	_, err = mgr2.Lease(ctx, 42)
	require.Equal(t, ErrLeasedByAnother, err)
	// But it can take a lease on another instance.
	l2, err := mgr2.Lease(ctx, 43)
	require.NoError(t, err)

	// The lease expires.
	// Stop the sessions' heartbeat, to not race with the checks below.
	mgr1.testingKnobs.heartbeatSem = make(chan struct{}, 1)
	mgr2.testingKnobs.heartbeatSem = make(chan struct{}, 1)
	// Acquire the semaphores, so heartbeats are now blocked.
	mgr1.testingKnobs.heartbeatSem <- struct{}{}
	mgr2.testingKnobs.heartbeatSem <- struct{}{}
	clock.MustAdvanceTo(l1.ValidityEnd.Add(time.Nanosecond))

	// mgr1 will attempt to acquire the lease again. We'll verify that it succeeds,
	...
	mgr1.testingKnobs.onLeaseAcquisitionBlockedOnHeartbeatOnce = waitForLeaseAcqBlocked
	...
	<-mgr1.testingKnobs.heartbeatSem
	...
	<-mgr2.testingKnobs.heartbeatSem
	_, err = mgr2.Lease(ctx, 42)
	require.Equal(t, ErrLeasedByAnother, err)

	// Check that mgr2 becomes valid, at which point we can call Lease(43) again.
	// We'll check that mgr2.Lease(43) doesn't block on heartbeats.
	require.True(t, mgr2.waitForValid(ctx))
	c := make(chan (struct{}))
	mgr2.testingKnobs.onLeaseAcquisitionBlockedOnHeartbeatOnce = c
	l22, err := mgr2.Lease(ctx, 43)
	require.NoError(t, err)
	require.Equal(t, l2.Epoch, l22.Epoch)
	mgr2.testingKnobs.onLeaseAcquisitionBlockedOnHeartbeatOnce = nil

	// The lease expires. We stop mgr1's heartbeats, and have session2 steal
	// the lease.

	// Block heartbeats for mgr1.
	mgr1.testingKnobs.heartbeatSem <- struct{}{}
	clock.MustAdvanceTo(l1.Expiration.Add(time.Nanosecond))
	// session2 successfully steals the lease.
	_, err = mgr2.Lease(ctx, 42)
	require.NoError(t, err)
	// Unblock mg1 and check that mgr1 cannot take the lease.
	<-mgr1.testingKnobs.heartbeatSem
	_, err = mgr1.Lease(ctx, 42)
	require.Equal(t, ErrLeasedByAnother, err)

	// Verify that mgr1 has incremented its epoch. It was forced to do so because
	// the lease stealing above deleted the session record with the old epoch.
	require.Equal(t, l1.Epoch+1, mgr1.epoch())
}
			...
			testutils.RunTrueAndFalse(t, "conflicting lease", func(t *testing.T, conflictingLease bool) {
				...
				for _, recreateTime := range recreationTimes {
					t.Run(fmt.Sprintf("recreateTime:%d", recreateTime), func(t *testing.T) {
						...
						require.True(t, s1.waitForValid(ctx))
