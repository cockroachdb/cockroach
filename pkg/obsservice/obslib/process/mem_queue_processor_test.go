...
func TestMemQueueProcessor(t *testing.T) {
	...
	t.Run("drains elements from queue after quiesce", func(t *testing.T) {
		...
		// We want the processor to stall until after quiesce. This allows us to verify that the queue is drained,
		// even after the
		processor.wg.Add(1)
		qProcessor, err := NewMemQueueProcessor[string](q, processor)
		require.NoError(t, err)
		...
		stopper.Quiesce(ctx)
		testutils.SucceedsSoon(t, func() error {
			if processor.timesCalled == 6 && q.Len() == 0 {
				return nil
			}
