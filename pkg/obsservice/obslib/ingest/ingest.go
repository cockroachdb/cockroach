...
type EventIngester struct {
	// Knobs groups testing facilities.
	Knobs struct {
		// OnFirstConnection, if set, is called when the first SubscribeToEvents
		// call succeeds.
		OnFirstConnection func(ctx context.Context)
	}
}
...
func (e *EventIngester) ingestEvents(ctx context.Context, addr string, db *pgxpool.Pool) {
	...
	if fn := e.Knobs.OnFirstConnection; fn != nil {
		fn(ctx)
	}
