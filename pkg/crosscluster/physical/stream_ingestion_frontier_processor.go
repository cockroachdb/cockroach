...
func newStreamIngestionFrontierProcessor(
	...
	sf.metrics = flowCtx.Cfg.JobRegistry.MetricsStruct().
	...
	job, err := flowCtx.Cfg.JobRegistry.LoadClaimedJob(ctx, jobspb.JobID(spec.JobID))
	details := job.Details().(jobspb.StreamIngestionDetails)
	if err != nil {
		return nil, err
	}
	sf.clusterMetrics.labels, err = resolveClusterMetricsLabels(
		ctx, flowCtx.Cfg.DB, flowCtx.Cfg.Settings, details,
	)
	if err != nil {
		return nil, errors.Wrap(err, "resolving cluster metrics labels")
	}
...
func (sf *streamIngestionFrontier) maybeUpdateProgress() error {
	...
	unixPersistedReplicatedTime := sf.persistedReplicatedTime.GoTime().Unix()
	sf.clusterMetrics.ReplicatedTimeSeconds.Update(sf.clusterMetrics.labels, unixPersistedReplicatedTime)
	sf.metrics.ReplicatedTimeSeconds.Update(unixPersistedReplicatedTime)
	return nil
}
