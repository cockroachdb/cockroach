...
// resolveClusterMetricsLabels resolves the labels for a stream ingestion job's
// cluster metrics. The source tenant name is taken directly from the job
// details. The destination tenant name is looked up from system.tenants by ID.
func resolveClusterMetricsLabels(
	ctx context.Context,
	db isql.DB,
	settings *cluster.Settings,
	details jobspb.StreamIngestionDetails,
) (map[string]string, error) {
	var destName roachpb.TenantName
	if err := db.Txn(ctx, func(ctx context.Context, txn isql.Txn) error {
		info, err := sql.GetTenantRecordByID(ctx, txn, details.DestinationTenantID, settings)
		if err != nil {
			return err
		}
		destName = info.Name
	...
	return map[string]string{
		streamingSourceTenantLabelName:      string(details.SourceTenantName),
		streamingDestinationTenantLabelName: string(destName),
	}, nil
}
