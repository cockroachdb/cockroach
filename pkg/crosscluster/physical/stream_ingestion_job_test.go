...
func TestPhysicalReplicationStreamOfSystemTenant(t *testing.T) {
	...
	testutils.SucceedsWithin(t, func() error {
		...
		rows := c.DestTenantSQL.Query(
			t, `SELECT id, claim_instance_id FROM system.jobs WHERE job_type = 'AUTO SPAN CONFIG RECONCILIATION' AND status = 'running'`,
		)
		defer rows.Close()
		if !rows.Next() {
			return errors.New("no running span config reconciliation job found")
		}
		if err := rows.Scan(&spanConfigJobID, &claimID); err != nil {
			return err
		}
		if claimID == 0 {
			// Span config job is waiting to be claimed, nudge the adoption queue to
			// speed it up.
			registry.TestingNudgeAdoptionQueue()
			return errors.New("span config reconciliation job has no claim session ID")
		}
