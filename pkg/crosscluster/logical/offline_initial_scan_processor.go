...
func (o *offlineInitialScanProcessor) Start(ctx context.Context) {
	ctx = logtags.AddTag(ctx, "job", o.spec.JobID)
	ctx = logtags.AddTag(ctx, "src-node", o.spec.PartitionSpec.PartitionID)
	ctx = logtags.AddTag(ctx, "proc", o.ProcessorID)
	...
	if err != nil {
		o.MoveToDrainingAndLogError(errors.Wrapf(err, "creating client for partition spec %q from %q", token, redactedAddr))
		return
	}
	...
	if err != nil {
		o.MoveToDrainingAndLogError(errors.Wrapf(err, "subscribing to partition from %s", redactedAddr))
		return
	}
