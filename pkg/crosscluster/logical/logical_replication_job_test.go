...
func TestTombstoneUpdate(t *testing.T) {
	...
	//
	// Replicating the bug requires three databases ('a', 'b', 'c').
	//
	// Here's the timeline of events:
	//
	// 1. row is inserted into 'a', 'b', and 'c'
	// 2. row is deleted from 'b'
	// 3. row is updated in 'c'
	// 4. row is deleted in 'a'
	//
	// 5. Replication is started from 'a' -> 'b' and allowed to catch up to now.
	// 6. Replication is started from 'c' -> 'b' and allowed to catch up to now.
	//
	// In this example, the last delete is the LWW winner. But if tombstones are not updated, then the update in 'c' will be
	// replicated to 'b' and the last delete will be a no-op.
	...
	urlA, urlB, _ := PGURLs[0].String(), PGURLs[1].String(), PGURLs[2].String()
	...
	// 5. Replicate the delete from 'a' -> 'b'
	...
	dst.QueryRow(t, "CREATE LOGICAL REPLICATION STREAM FROM TABLE tab ON $1 INTO TABLE tab WITH MODE = VALIDATED, CURSOR = $2",
		urlA, start.AsOfSystemTime()).Scan(&jobIDSrcA)
	WaitUntilReplicatedTime(t, s.Clock().Now(), dst, jobIDSrcA)

	// 6. Replicate the update from 'c' -> 'b'
	var jobIDSrcB jobspb.JobID
	dst.QueryRow(t, "CREATE LOGICAL REPLICATION STREAM FROM TABLE tab ON $1 INTO TABLE tab WITH MODE = VALIDATED, CURSOR = $2",
		urlB, start.AsOfSystemTime()).Scan(&jobIDSrcB)
