...
type MergeFeed struct {
	...
	// targetBatchKVs is the number of KV entries that trigger a flush. We may
	// flush a smaller batch than targetBatchKVs if we see a checkpoint and we
	// may flush a larger batch to avoid breaking up a transaction.
	targetBatchKVs int
...
// transaction).
//
// There are a few behaviors the merge feed assumes are provided by the input
// subscriptions:
// 1. The input subscriptions must emit events in time order.
// 2. When the input subscription emits a checkpoint, the checkpoint has every
// span at the same time and it covers the entire frontier tracked by the
// subscription.
// 3. If there is no data that is ready to be sent, the last event emitted is a
// checkpoint. We always need to read one event past the event we are emitting
// and having a checkpoint trail data ensures the peeked event we hold on
// to during these idle periods is a checkpoint.
func NewMergeFeed(
...
func (m *MergeFeed) mergeLoop(ctx context.Context) error {
	...
	for h.len() != 0 {
		...
		if !entry.isKV() {
			// We flush if the next entry in the heap is a checkpoint because a
			// checkpoint may indicate the input stream is now idle and we don't want
			// to wait for data that may never come.
		...
		// Drain all KVs at the current timestamp from this entry before
		// pushing it back on the heap to avoid unnecessary heap operations.
		peekTS := entry.peekTS
		closed := false
		for entry.isKV() && entry.peekTS == peekTS {
