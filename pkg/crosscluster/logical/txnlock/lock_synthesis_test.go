...
func TestDeriveLocks(t *testing.T) {
	...
	testCases := []lockSynthesisTestCase{
		...
		},
		{
			...
			readOverlapCount: 1, // UC(shared@test.com)
		},
		{
		...
		},
		{
			...
			txn1: txnCase{
				...
				// 3 PK + UC(val-a,val-b,val-c) = 6 write; UC(val-d) = 1 read.
				writeLockCount: 6,
				readLockCount:  1,
				order:          []int{2, 1, 0},
			},
		...
		},
		{
			...
			txn1: txnCase{
				...
				// PK + UC(old: 10,20) = 2 write; UC(new: 30,40) = 1 read.
				writeLockCount: 2,
				readLockCount:  1,
				order:          []int{0},
			},
		...
		},
		{
			...
			txn1: txnCase{
				...
				// 2 PK + UC(10,20) = 3 write; UC(30,40) = 1 read.
				writeLockCount: 3,
				readLockCount:  1,
				order:          []int{1, 0},
			},
		...
		},
		{
			...
			txn2: txnCase{
				...
				// PK + UC(write,10) + FK(write,10,ref) + FK(write,20,ref)
				writeLockCount: 4,
				readLockCount:  1,
				order:          []int{0},
			},
		...
		},
		{
			...
			txn1: txnCase{
				...
				// PK + UC(write,10) + FK(write,10,ref_col) +
				// FK(write,20,ref_col) = 4 write; UC(read,20) = 1 read.
		...
		{
			// Within a single txn, a child starts referencing a parent's
			// ref_col value that the parent is simultaneously changing away
			// from. The parent update must be applied before the child
			// update so the referenced value exists when the child needs it.
			name: "fk_child_refs_value_parent_is_changing",
			txn1: txnCase{
				events: []streampb.StreamEvent_KV{
					...
					childEB.UpdateEvent(txnTime,
						...
						fkChildRow(1, 10, 5, 0)),
					// Parent changes ref_col from 10 to 20.
					parentEB.UpdateEvent(txnTime,
						fkParentRow(2, 10, 0),
						fkParentRow(2, 20, 0)),
				},
		...
		},
		{
			...
			txn1: txnCase{
				...
				// Delete: PK + UC delete + 2 FK parent inserts = 4
				// Update: PK + 2 FK child inserts (reads) = 1
				// Insert: PK  + 2 FK parent inserts = 3
				writeLockCount: 8,
				// UC insert
				readLockCount: 1,
				order:         []int{2, 1, 0},
		...
		},
		{
			...
			txn1: txnCase{
				...
				// child: PK
				// parent: PK + FK(id=10) + FK(ref_col=50) = 3
				writeLockCount: 4,
