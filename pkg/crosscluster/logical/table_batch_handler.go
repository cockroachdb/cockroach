...
func (t *tableHandler) attemptBatch(
	...
	if len(batch) == 1 && isLwwLoser(err) {
		return tableBatchStats{kvLwwLosers: 1}, nil
	}
	if err != nil {
		return tableBatchStats{}, err
	}
...
func (t *tableHandler) refreshPrevRows(
	...
	for i, event := range batch {
		...
		if found {
			if !refreshed.logicalTimestamp.Less(event.originTimestamp.Next()) {
				// Skip the row because it is a lww loser.
				// TODO(jeffswenson): update this logic when its time to handle
				// ties.
				stats.refreshLwwLosers++
				continue
			}
			revfreshedEvent.prevRow = refreshed.row
		}
		refreshedBatch = append(refreshedBatch, revfreshedEvent)
	}
