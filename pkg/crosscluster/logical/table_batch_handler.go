...
func (t *tableHandler) handleDecodedBatch(
	...
	log.Dev.Infof(ctx, "JS failed initial apply attempt %d: %+v", batch[0].dstDescID, err)
	...
	if err != nil {
		log.Dev.Infof(ctx, "JS error applying batch to table %d: %+v", batch[0].dstDescID, err)
