...
// TODO(jeffswenson): write a test for this
func (t *TxnDecoder) DecodeTxn(transaction []streampb.StreamEvent_KV) (Transaction, error) {
	...
	for _, event := range transaction {
		decoded, _, err := t.decoder.decodeEvent(context.Background(), event)
		if err != nil {
			return Transaction{}, err
		}
		if decoded.RowTimestamp != result.Timestamp {
			return Transaction{}, errors.Errorf("inconsistent timestamps in transaction: got %s, expected %s",
