...
func createLogicalReplicationStreamPlanHook(
	...
	fn := func(ctx context.Context, resultsCh chan<- tree.Datums) (retErr error) {
		...
		if options.ParentID != 0 {
			row, err := p.ExecCfg().InternalDB.Executor().QueryRow(ctx, "check-parent-job", nil, parentQuery, fmt.Sprintf("%d", options.ParentID))
			if err != nil {
				return err
			}
			if row == nil {
			} else if jobID := int(*row[0].(*tree.DInt)); jobID != 0 {
