...
type tombstoneUpdater struct {
	...
	leased struct {
		// descriptoris a leased descriptor. Callers should use getDeleter to ensure
		// the lease is valid for the current transaction.
...
func (tu *tombstoneUpdater) addToBatch(
	...
	return deleter.UpdateTombstone(
		ctx,
		batch,
		tu.scratch,
...
func (tu *tombstoneUpdater) getDeleter(ctx context.Context, txn *kv.Txn) (row.Deleter, error) {
	...
	if tu.leased.descriptor == nil || !tu.leased.descriptor.Expiration(ctx).After(timestamp) {
