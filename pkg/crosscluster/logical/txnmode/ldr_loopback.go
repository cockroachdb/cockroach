...
// lookupOrCreate returns the backchannel for the given flow and applier,
// creating it if it does not yet exist. This is safe to call from both the
// applier and dep resolver processors regardless of startup order.
func (m *ldrLoopbackMap) lookupOrCreate(
	flowCtx *execinfra.FlowCtx, applierID ldrdecoder.ApplierID,
) *ldrLoopbackChannels {
	m.Lock()
	defer m.Unlock()
	key := ldrLoopbackKey{flowID: flowCtx.ID, applierID: applierID}
	if chs, ok := m.channels[key]; ok {
		return chs
	}
...
// cleanup removes and closes the backchannel for the given flow and applier.
// It is idempotent.
func (m *ldrLoopbackMap) cleanup(flowCtx *execinfra.FlowCtx, applierID ldrdecoder.ApplierID) {
	m.Lock()
	defer m.Unlock()
	key := ldrLoopbackKey{flowID: flowCtx.ID, applierID: applierID}
	if chs, ok := m.channels[key]; ok {
