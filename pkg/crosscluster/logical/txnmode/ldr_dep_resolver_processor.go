...
func (p *ldrDepResolverProcessor) handleRequest(row rowenc.EncDatumRow) error {
	...
	case *txnpb.LDRDepResolverEvent_Wait:
		for _, txn := range e.Wait.WaitTxns {
			resolved, resolvedTime := p.tracker.MaybeAddWaiter(txn, e.Wait.WaitingID)
			if resolved {
				if err := p.sendUpdate(e.Wait.WaitingID, txnapply.DependencyUpdate{
					TxnID:        txn,
	...
	case *txnpb.LDRDepResolverEvent_WaitHorizon:
		...
		if resolved {
			return p.sendUpdate(e.WaitHorizon.WaitingID, txnapply.DependencyUpdate{
				TxnID:        ldrdecoder.TxnID{ApplierID: e.WaitHorizon.DependID},
				ResolvedTime: resolvedTime,
			})
		}
	case *txnpb.LDRDepResolverEvent_Ready:
		updates := p.tracker.Ready(e.Ready.ReadyTxn, e.Ready.ResolvedTime)
		for applierID, update := range updates {
			if err := p.sendUpdate(applierID, update); err != nil {
				return err
			}
		}
	case *txnpb.LDRDepResolverEvent_ForwardUpdate:
		return p.sendUpdate(p.spec.ApplierID, txnapply.DependencyUpdate{
			TxnID:        e.ForwardUpdate.TxnID,
			ResolvedTime: e.ForwardUpdate.ResolvedTime,
		})
	default:
		return errors.Newf("unknown dep resolver event type: %T", ev.Event)
	}
	return nil
}
...
func decodeDepResolverEvent(row rowenc.EncDatumRow) (txnpb.LDRDepResolverEvent, error) {
	if len(row) != 2 {
		return txnpb.LDRDepResolverEvent{}, errors.Newf(
			"expected 2 columns, got %d", len(row))
	}
	if err := row[1].EnsureDecoded(types.Bytes, nil); err != nil {
		return txnpb.LDRDepResolverEvent{}, err
	}
	payloadBytes := []byte(*row[1].Datum.(*tree.DBytes))

	var ev txnpb.LDRDepResolverEvent
	if err := protoutil.Unmarshal(payloadBytes, &ev); err != nil {
		return txnpb.LDRDepResolverEvent{}, errors.Wrap(
...
// close cleans up the loopback backchannel and closes the processor. It is
// idempotent.
func (p *ldrDepResolverProcessor) close() []execinfrapb.ProducerMetadata {
	if p.Closed {
		return nil
	}
...
func (p *ldrDepResolverProcessor) ConsumerClosed() {
	p.close()
}
