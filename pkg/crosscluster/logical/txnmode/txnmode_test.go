...
func TestTxnModeUniqueConstraintUpdate(t *testing.T) {
	...
	var jobID jobspb.JobID
	destDB.QueryRow(t,
		"CREATE LOGICAL REPLICATION STREAM FROM TABLES (test_table) ON $1 INTO TABLES (test_table) WITH MODE = 'transaction'",
		sourceURL.String(),
	).Scan(&jobID)
	...
	// Verify initial data was replicated
	...
	// Wait for replication to catch up
	now = s.Clock().Now()
	sourceDB.Exec(t, "INSERT INTO test_table (uuid, unique_value) VALUES (gen_random_uuid(), 8888)")
	ldrtestutils.WaitUntilReplicatedTime(t, now, destDB, jobID)
