...
func (p *ldrCoordinatorProcessor) hydrateApplierID(
	...
	if len(txn.WriteSet) > 0 {
		...
		besteffort.Warning(ctx, "node-router", func(ctx context.Context) error {
			var err error
...
func (p *ldrCoordinatorProcessor) encodeScheduledTxn(
	...
	evt := txnpb.LDRApplierEvent{
		Event: &txnpb.LDRApplierEvent_ScheduledTxn{
			ScheduledTxn: &txnpb.EncodedScheduledTransaction{
...
func (p *ldrCoordinatorProcessor) encodeCheckpoint(
	...
	evt := txnpb.LDRApplierEvent{
		Event: &txnpb.LDRApplierEvent_Checkpoint{
			Checkpoint: &txnpb.EncodedCheckpoint{
				Timestamp: ts,
			},
		},
	}
