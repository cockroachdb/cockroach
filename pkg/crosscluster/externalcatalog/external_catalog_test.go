...
func TestExtractIngestExternalCatalog(t *testing.T) {
	...
	require.ErrorContains(t, IngestExternalCatalog(ctx, &execCfg2, sqlUser, sadCatalog, parentID, schemaID, false), "invalid foreign key backreference")
	require.NoError(t, IngestExternalCatalog(ctx, &execCfg2, sqlUser, ingestableCatalog, parentID, schemaID, false))
