...
func TestRangeStats(t *testing.T) {
	...
	type testCase struct {
		...
		fraction       float32
		expectedStatus jobspb.ReplicationStatus
		expectedMsg    string
	...
	testCases := []testCase{
		{
			...
			fraction:       1,
			expectedStatus: jobspb.Replicating,
			expectedMsg:    "",
			inputStats: map[int32]*streampb.StreamEvent_RangeStats{
				1: {RangeCount: 5},
				2: {RangeCount: 3},
		...
		},
		{
			...
			fraction:       0.4,
			expectedStatus: jobspb.InitialScan,
		...
		},
		{
			...
			fraction:       0.8,
			expectedStatus: jobspb.CatchupScan,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			total, fraction, status, msg := r.RollupStats()
			...
			total, fraction, status, msg = rInitializing.RollupStats()
			require.Equal(t, total, streampb.StreamEvent_RangeStats{})
			require.Equal(t, float32(0), fraction)
			require.Equal(t, jobspb.InitializingReplication, status)
