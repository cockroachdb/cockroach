...
func reloadURI(ctx context.Context, id jobspb.JobID, execCfg *sql.ExecutorConfig) (string, error) {
	reloadedJob, err := execCfg.JobRegistry.LoadJob(ctx, id)
	if err != nil {
		return "", err
	}
	newDetails := reloadedJob.Details().(jobspb.LogicalReplicationDetails)
	return resolveURI(ctx, execCfg, newDetails.SourceClusterConnUri)
}
