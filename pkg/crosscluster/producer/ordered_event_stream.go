...
type RangefeedHandler interface {
	...
	// events up to timestamp into the handler's batch; the handler should flush
...
func (h *OrderedStreamHandler) handleFrontier(ctx context.Context, timestamp hlc.Timestamp) error {
	...
	for !iterExhausted {
		events, iterExhausted, err = h.buffer.GetEventsFromDisk(ctx, timestamp)
		if err != nil {
			return err
		}
...
func (h *OrderedStreamHandler) onInitialScanDone(ctx context.Context) {
	h.handler.setErr(h.handleFrontier(ctx, hlc.MaxTimestamp))
