...
type EngineMetrics struct {
	...
	SQLExecLatencyCurrent metric.IHistogram
	...
	SQLServiceLatencyCurrent metric.IHistogram
	...
	if automaticRetryCount == 0 {
		...
		if shouldIncludeInLatencyMetrics {
			...
			if ex.state.isHistorical.Load() {
				...
				m.SQLExecLatencyCurrent.RecordValue(runLatRaw.Nanoseconds())
				m.SQLServiceLatencyCurrent.RecordValue(svcLatRaw.Nanoseconds())
			}
		}
	}
}
