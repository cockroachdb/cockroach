...
func (p *phaseTimes) getServiceLatency() time.Duration {
	...
	// `sessionQueryServiced` is unset, because that happens in the outermost
	// wrapper. To prevent negative values for the case mentioned above, we have
	// this second possible way of calculating the service latency by relying on
	// the plannerEndExecStmt phase. It's worth noting that the plannerEndExecStmt
