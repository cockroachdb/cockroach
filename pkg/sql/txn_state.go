...
func (ts *txnState) resetForNewSQLTxn(
	...
	// Create a span for the new txn. The span is always WithRealSpan to support the
	...
	// doesn't current support the WithRealSpan option.
	if parentSp := tracing.SpanFromContext(connCtx); parentSp != nil {
		...
		sp = tranCtx.tracer.StartRootSpan(
			opName, logtags.FromContext(connCtx), true /* forceRealSpan */)
	}
