...
type advanceInfo struct {
	...
	// results stream.
	//
	// The current implementation ignores this signal and only flushes results
	// when the client asks for it or when the buffer overflows.
	flush bool
...
// and act on it.
func (ts *txnState2) setAdvanceInfo(
	code advanceCode, flushOpt flushOpt, rewCap rewindCapability, ev txnEvent,
) {
