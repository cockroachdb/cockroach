	...
	txnID = func() (txnID uuid.UUID) {
		...
		if txn == nil {
			...
			if bufferedWritesEnabled {
				ts.setBufferedWritesEnabledLocked(true /* enabled */)
			}
		} else {
...
func (ts *txnState) setBufferedWritesEnabled(enabled bool) {
	ts.mu.Lock()
	defer ts.mu.Unlock()
	ts.setBufferedWritesEnabledLocked(enabled)
}

func (ts *txnState) setBufferedWritesEnabledLocked(enabled bool) {
	ts.mu.txn.SetBufferedWritesEnabled(enabled)
}
