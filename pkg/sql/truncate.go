...
func (t *truncateNode) startExec(params runParams) error {
	...
	for len(toTraverse) > 0 {
		...
		maybeEnqueue := func(ref sqlbase.ForeignKeyReference, msg string) error {
			...
			otherName, err := p.getQualifiedTableName(ctx, other.TableDescriptor)
			if err != nil {
				return err
			}
...
func (p *planner) truncateTable(
	...
	cpy := *tableDesc.TableDescriptor
	newTableDesc := sqlbase.MutableTableDescriptor{TableDescriptor: &cpy}
	...
	zoneKey, nameKey, _ := GetKeysForTableDescriptor(tableDesc.TableDescriptor)
	b := &client.Batch{}
