...
func (p *planner) copySplitPointsToNewIndexes(
	...
	if preservedSplitsMultiple := int(PreservedSplitCountMultiple.Get(p.execCfg.SV())); preservedSplitsMultiple > 0 {
		row, err := p.EvalContext().InternalExecutor.QueryRow(
			ctx, "count-active-nodes", p.txn, "SELECT COUNT(*) FROM crdb_internal.kv_node_status")
		if err != nil || row == nil {
			return err
		}
		...
		log.Infof(ctx, "making %d splits (%d * %d)", nSplits, preservedSplitsMultiple, nNodes)
		...
		step := float64(len(ranges)) / float64(nSplits)
		...
		for i := 0; i < nSplits; i++ {
			...
			log.Errorf(ctx, "sending split request for key %s with expiration time %s", newStartKey,
