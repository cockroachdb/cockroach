...
func (p *planner) truncateTable(ctx context.Context, id sqlbase.ID, jobDesc string) error {
	...
	oldIndexes := make([]sqlbase.IndexDescriptor, len(tableDesc.Indexes)+1)
	oldIndexes[0] = *protoutil.Clone(&tableDesc.PrimaryIndex).(*sqlbase.IndexDescriptor)
	for i := range tableDesc.Indexes {
		oldIndexes[i+1] = *protoutil.Clone(&tableDesc.Indexes[i]).(*sqlbase.IndexDescriptor)
	}

	// Reset all of the index IDs.
	tableDesc.PrimaryIndex.ID = sqlbase.IndexID(0)
	for i := range tableDesc.Indexes {
		tableDesc.Indexes[i].ID = sqlbase.IndexID(0)
	}
	...
	indexIDMapping := make(map[sqlbase.IndexID]sqlbase.IndexID, len(oldIndexes))
	...
	if err := p.writeSchemaChange(ctx, tableDesc, sqlbase.InvalidMutationID, jobDesc); err != nil {
		return err
	}
	...
	var droppedInterleaves []sqlbase.IndexDescriptor
	...
	if _, err := p.ExecCfg().JobRegistry.CreateJobWithTxn(ctx, record, p.txn); err != nil {
		return err
	}
...
func ClearTableDataInChunks(
	...
	for rowIdx, done := 0, false; !done; rowIdx += chunkSize {
		...
		if err := db.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
			rd := row.MakeDeleter(codec, sqlbase.NewImmutableTableDescriptor(*tableDesc), nil)
...
func (p *planner) reassignInterleaveIndexReferences(
	...
	truncatedID sqlbase.ID,
	indexIDMapping map[sqlbase.IndexID]sqlbase.IndexID,
) error {
	for _, table := range tables {
		...
		if changed {
			if err := p.writeSchemaChange(ctx, table, sqlbase.InvalidMutationID, "updating reference for truncated table"); err != nil {
				return err
			}
		}
	}
