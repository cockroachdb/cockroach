	...
	for i, m := range desc.AllMutations() {
		if idx := m.AsIndex(); idx != nil {
			...
			if col.Dropped() && col.GetType().UserDefined() {
				return unimplemented.Newf(
					"TRUNCATE concurrent with ongoing schema change",
					"cannot perform TRUNCATE on %q which has a column with a "+
						"user-defined type being dropped", desc.GetName())
			}
			...
			case descpb.ConstraintToUpdate_FOREIGN_KEY:
				return unimplemented.Newf(
					"TRUNCATE concurrent with ongoing schema change",
					"cannot perform TRUNCATE on %q which has an ongoing foreign "+
						"key constraint change", desc.GetName())
			default:
				return errors.AssertionFailedf("unknown check constraint type %v on mutation %d in %v", ct, i, desc)
			}
			...
			return errors.AssertionFailedf("unknown mutation of type %T for mutation %d in %v", m, i, desc)
		}
	}
	return nil
}
