...
func reassignComment(
	...
	for _, column := range oldTableDesc.Columns {
		colNum, err := oldTableDesc.FindColumnNumberByName(tree.Name(column.Name))
		if err != nil {
			return err
		}

		err = reassignColumnComment(ctx, p, oldTableDesc.ID, newID, colNum)
		if err != nil {
			return err
		}
	}
...
func reassignColumnComment(
	ctx context.Context, p *planner, oldID sqlbase.ID, newID sqlbase.ID, columnNum int,
) error {
	comment, err := p.ExtendedEvalContext().ExecCfg.InternalExecutor.QueryRow(
		ctx,
