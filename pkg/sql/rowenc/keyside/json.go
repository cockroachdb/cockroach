...
func decodeJSONKey(buf []byte, dir encoding.Direction) (json.JSON, []byte, error) {
	...
	} else if typ == encoding.JSONString {
		var str string
		buf, str, err = encoding.DecodeUnsafeStringAscending(buf, nil)
		if err != nil {
			panic("could not decode the JSON string")
		}
		buf = buf[1:] //Removing the terminator
		jsonVal, err = json.MakeJSON(str)
	} else if typ == encoding.JSONStringDesc {
		var str string
...
func decodeJSONArray(buf []byte, dir encoding.Direction) (json.JSON, []byte, error) {
	...
	var err error
	buf, length, err := encoding.DecodeJSONArrayOrJSONObjectLength(buf, dir)

	// Allocating it len(buf) bytes since in the worst case, a JSON array
	// could consist of only primitive JSON values which take up only one
	// byte during their encodings (taken up by their respective markers)
	...
	for {
		...
		if encoding.IsJSONKeyDone(buf, dir) {
			buf = buf[1:]
			return jsonArray.Build(), buf, err
		}
...
func decodeJSONObject(buf []byte, dir encoding.Direction) (json.JSON, []byte, error) {
	...
	for {
		...
		if encoding.IsJSONKeyDone(buf, dir) {
			...
			buf = buf[1:]
			return jsonObject.Build(), buf, err
		}
