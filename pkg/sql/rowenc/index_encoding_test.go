...
func TestEncodeTrigramInvertedIndexSpans(t *testing.T) {
	...
	runTest := func(indexedValue, value string, searchType trigramSearchType,
		...
		chunks, typedExpr := makeTrigramBinOp(t, indexedValue, value, searchType)
		invertedExpr, err := EncodeTrigramSpans(chunks, searchType != similar)
		require.NoError(t, err)
	...
	for i := 0; i < 100; i++ {
		...
		for _, searchType := range []trigramSearchType{like, eq, similar} {
			chunks, expr := makeTrigramBinOp(t, left, right, searchType)
			...
			for _, chunk := range chunks {
				rTrigrams := trigram.MakeTrigrams(chunk, false /*pad*/)
			...
			unique := true
			for _, chunk := range chunks {
				trigrams := trigram.MakeTrigrams(chunk, false /* pad */)
				if len(trigrams) > 1 {
					// Unique is always false if a chunk has more than 1 trigram.
					unique = false
				}
				if len(trigrams) >= 1 {
					if valid {
						// Unique is false if there's more than 1 trigram.
						unique = false
					}
					valid = true
				}
			...
			if !valid {
				_, err := EncodeTrigramSpans(chunks, true /* allMusMatch */)
				require.Error(t, err)
				continue
			}
...
func makeTrigramBinOp(
...
) (chunks []string, typedExpr tree.TypedExpr) {
	chunks = []string{value}
	var expr tree.Expr
	var err error
	switch searchType {
	case like, eq:
		...
		require.NoError(t, err)
	case similar:
		expr, err = parser.ParseExpr(fmt.Sprintf("'%s' %% '%s'", indexedValue, value))
		require.NoError(t, err)
		chunks = []string{value}
	default:
...
func TestEncodeTrigramInvertedIndexSpansError(t *testing.T) {
	...
	for _, tc := range tcs {
		_, err := EncodeTrigramSpans(tc.chunks, true /* allMustMatch */)
		require.Error(t, err)
		_, err = EncodeTrigramSpans(tc.chunks, false /* allMustMatch */)
		require.Error(t, err)
	}
}
