...
func TestEncodeOverlapsArrayInvertedIndexSpans(t *testing.T) {
	testCases := []struct {
		...
		{`{1, 2}`, `{1}`, true, true},
		{`{2}`, `{1, 2}`, true, true},
		{`{2, NULL}`, `{1, 2}`, true, true},
		{`{1, 2}`, `{1, 2}`, true, true},
		{`{1, 3}`, `{1, 2}`, true, true},
		{`{2}`, `{2, 2}`, true, true},
		{`{1, 2}`, `{1, 2, 1}`, true, true},
		{`{1, 1, 2, 3}`, `{1, 2, 1}`, true, true},
		{`{1, 2, 4}`, `{1, 2, 3}`, true, true},
		{`{2}`, `{2, NULL}`, true, true},
		{`{2, 3}`, `{2, NULL}`, true, true},
		{`{1, NULL}`, `{1, 2, NULL}`, true, true},
		...
		{`{4, 3}`, `{2, 1}`, true, false},
		{`{5}`, `{1, 2, 1}`, true, false},
		{`{NULL, 3}`, `{1, 2, 1}`, true, false},
		{`{NULL}`, `{NULL}`, false, false},
		{`{NULL}`, `{1, NULL}`, true, false},
		{`{1,NULL}`, `{NULL}`, false, false},
		{`{2, NULL}`, `{1, NULL}`, true, false},
	}
	...
	runTest := func(indexedValue, value tree.Datum, expected, ok bool) {
		keys, err := EncodeInvertedIndexTableKeys(indexedValue, nil, descpb.LatestNonPrimaryIndexDescriptorVersion)
		require.NoError(t, err)
	...
	for _, c := range testCases {
		indexedValue, value := parseArray(c.indexedValue), parseArray(c.value)
		runTest(indexedValue, value, c.expected, c.ok)
	}
	...
	for i := 0; i < 100; i++ {
		...
		// Now check that we get the same result with the inverted index spans.
		runTest(left, right, bool(*overlaps), ok)
	}
}
