...
func makeTableDescForTest(test indexKeyTest) (catalog.TableDescriptor, catalog.TableColMap) {
	...
	tableDesc := descpb.TableDescriptor{
		...
		PrimaryIndex: descpb.IndexDescriptor{
			...
			KeyColumnDirections: make([]fetchpb.IndexColumn_Direction, len(primaryColumnIDs)),
		},
		Indexes: []descpb.IndexDescriptor{{
			...
			Unique:              true,
			KeyColumnDirections: make([]fetchpb.IndexColumn_Direction, len(secondaryColumnIDs)),
			Type:                secondaryType,
		}},
	}
...
func ExtractIndexKey(
	...
	dirs = make([]fetchpb.IndexColumn_Direction, index.NumKeySuffixColumns())
	for i := 0; i < index.NumKeySuffixColumns(); i++ {
		// Implicit columns are always encoded Ascending.
		dirs[i] = fetchpb.IndexColumn_ASC
	}
	extraKey := key
	if index.IsUnique() {
