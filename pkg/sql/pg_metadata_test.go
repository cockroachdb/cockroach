...
const (
	...
	informationSchemaNewTableInserationText = `// forEachSchema iterates over the physical and virtual schemas.`
...
var codeFixers = map[string]schemaCodeFixer{
	...
	"information_schema": {
		...
		textForNewTableInsertion: informationSchemaNewTableInserationText,
...
func (scf schemaCodeFixer) fixVtable(
	...
	rewriteFile(fileName, func(input *os.File, output outputFile) {
		...
		for reader.Scan() {
			...
			// fixing columns is only compatible with postgres an pg_catalog.
