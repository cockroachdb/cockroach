	...
	rewriteFile(pgCatalogGo, func(input *os.File, output outputFile) {
		...
		for reader.Scan() {
			...
			case tableDefsDeclaration:
				printBeforeTerminalString(reader, output, tableDefsCloseBracket, tableDefinitionText)
			case allTableNamesDeclaration:
				printBeforeTerminalString(reader, output, allTableNamesClose, allTableNamesText)
			}
		}
	})
}
...
func TestPGCatalog(t *testing.T) {
	...
	if *addMissingTables {
		notImplemented := diffs.getUnimplementedTables(pgTables)
		fixConstants(t, notImplemented)
		fixVtable(t, notImplemented)
		fixPgCatalogGo(notImplemented)
	}
}
