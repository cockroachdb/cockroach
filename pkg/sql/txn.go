...
func (p *planner) setTransactionModes(modes parser.TransactionModes) error {
	...
	if err := p.setReadOnly(modes.ReadOnly); err != nil {
		return err
	}
	return nil
}
...
func (p *planner) setReadOnly(readOnly parser.ReadOnly) error {
	switch readOnly {
	case parser.UnspecifiedReadOnly:
		return nil
	case parser.Only:
	...
	case parser.Write:
		return nil
	default:
		return errors.Errorf("unknown read mode: %s", readOnly)
	}
}
