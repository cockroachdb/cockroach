...
// RelocateLease moves leases to specific stores.
// (`ALTER TABLE/INDEX ... EXPERIMENTAL_RELOCATE LEASE ...` statement)
// Privileges: INSERT on table.
func (p *planner) RelocateLease(ctx context.Context, n *tree.RelocateLease) (planNode, error) {
	tableDesc, index, err := p.getTableAndIndex(ctx, n.Table, n.Index, privilege.INSERT)
	if err != nil {
		return nil, err
	}
	...
	cols := planColumns(rows)
	if len(cols) < 2 {
		return nil, errors.Errorf("less than two columns in EXPERIMENTAL_RELOCATE LEASE data")
	}
	if len(cols) > len(index.ColumnIDs)+1 {
		return nil, errors.Errorf("too many columns in EXPERIMENTAL_RELOCATE LEASE data")
	}
	for i := range cols {
		if !cols[i].Typ.Equivalent(desiredTypes[i]) {
			...
			return nil, errors.Errorf(
				"EXPERIMENTAL_RELOCATE LEASE data column %d (%s) must be of type %s, not type %s",
				i+1, colName, desiredTypes[i], cols[i].Typ,
			)
		}
	}

	return &relocateLeaseNode{
		tableDesc: tableDesc,
		index:     index,
		rows:      rows,
		run: relocateRun{
...
func (n *relocateLeaseNode) Next(params runParams) (bool, error) {
	// Each Next call relocates one lease (corresponding to one row from n.rows).
	// TODO(a-robinson): perform multiple relocations in parallel.

	if ok, err := n.rows.Next(params); err != nil || !ok {
		return ok, err
	}

	// First column is the target leaseholder store; the rest of the columns
	// indicate the table/index row.
	data := n.rows.Values()

	if !data[0].ResolvedType().Equivalent(types.Int) {
		return false, errors.Errorf(
			"expected int in the first EXPERIMENTAL_RELOCATE LEASE data column; got %s",
			data[0].ResolvedType(),
		)
	}
	storeID := roachpb.StoreID(tree.MustBeDInt(data[0]))
	if storeID <= 0 {
		return false, errors.Errorf("invalid target leaseholder store ID %d for EXPERIMENTAL_RELOCATE LEASE", storeID)
	}
	...
	// TODO(XXX): Get the lastRangeStartKey some other way! Preferably from the result of the transfer lease request.
	rangeDesc, err := lookupRangeDescriptor(params.ctx, params.extendedEvalCtx.ExecCfg.DB, rowKey)
	if err != nil {
		return false, errors.Wrap(err, "error looking up range descriptor")
	}
	...
	if err := params.p.ExecCfg().DB.AdminTransferLease(params.ctx, rowKey, storeID); err != nil {
		return false, err
	}
...
func (n *relocateLeaseNode) Values() tree.Datums {
	return tree.Datums{
		tree.NewDBytes(tree.DBytes(n.run.lastRangeStartKey)),
		tree.NewDString(keys.PrettyPrint(sqlbase.IndexKeyValDirs(n.index), n.run.lastRangeStartKey)),
	}
...
func (n *relocateLeaseNode) Close(ctx context.Context) {
	n.rows.Close(ctx)
}
