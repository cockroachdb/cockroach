...
func TestUpdateRollingQueryCounts(t *testing.T) {
	...
	for _, tc := range testData {
		...
		circIdx := freshMetrics.RollingQueryCounts.NextIndex(freshMetrics.RollingQueryCounts.EndPointer())
		for i := 0; i < len(freshMetrics.RollingQueryCounts.GetQueryCounts()); i++ {
			queryCount := freshMetrics.GetQueryCount(circIdx)
			require.Equal(t, tc.expectedQueryCounts[i], queryCount.Count())
			circIdx = freshMetrics.RollingQueryCounts.NextIndex(circIdx)
		}
	}
}
