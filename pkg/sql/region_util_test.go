...
func TestZoneConfigFromTableLocalityConfig(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			desc: "4-region global table with zone survival on primary region",
			localityConfig: descpb.TableDescriptor_LocalityConfig{
			...
			expected: &zonepb.ZoneConfig{
				...
				Constraints: []zonepb.ConstraintsConjunction{
					{NumReplicas: 4, Constraints: []zonepb.Constraint{{Type: zonepb.Constraint_REQUIRED, Key: "region", Value: "region_b"}}},
				},
			},
		},
		{
			desc: "4-region global table with regional survival on primary region",
			localityConfig: descpb.TableDescriptor_LocalityConfig{
				Locality: &descpb.TableDescriptor_LocalityConfig_RegionalByTable_{
					RegionalByTable: &descpb.TableDescriptor_LocalityConfig_RegionalByTable{
						Region: nil,
		...
		{
			desc: "4-region global table with zone survival on non primary region",
			localityConfig: descpb.TableDescriptor_LocalityConfig{
				Locality: &descpb.TableDescriptor_LocalityConfig_RegionalByTable_{
					RegionalByTable: &descpb.TableDescriptor_LocalityConfig_RegionalByTable{
		...
		{
			desc: "4-region global table with regional survival on non primary region",
			localityConfig: descpb.TableDescriptor_LocalityConfig{
				Locality: &descpb.TableDescriptor_LocalityConfig_RegionalByTable_{
					RegionalByTable: &descpb.TableDescriptor_LocalityConfig_RegionalByTable{
						Region: protoRegion("region_c"),
