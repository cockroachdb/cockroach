...
// which aims to reduce I/O by avoiding the retrieval of column data
...
// values, but really this ought to be optimized away to re-use the
// column data from the index scanNode instead.
//
// TODO(radu/knz): optimize setNeededColumns() further to reuse the
// indexed column values instead of forcing a re-read with the table
// scanNode.
//
...
type indexJoinNode struct {
	...
	// valNeededIndex is the set of columns for which a value is
	// required in the index scanNode. This contains all the values
	// needed for the PK of the table scanNode.
	valNeededIndex []bool
...
func (p *planner) makeIndexJoin(
	...
	// valNeededIndex defined here will serve both as the valNeededIndex
	...
	// are provided by the index scanNode. Since valNeededIndex only
