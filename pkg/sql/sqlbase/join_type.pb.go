...
const (
	...
	// A left semi join returns the rows from the left side that have at least a
	// match (as per equality columns and ON condition).
	JoinType_LEFT_SEMI JoinType = 4
	// A left anti join is an "inverted" semi join: it returns the rows from the
	// left side that don't have a match (as per equality columns and ON
	// condition).
	...
	// rows that have the same values on the equality columns, only as many as
	// those are removed as there are matches on the right side.
	//
	// For example: if we have equality between the first column on each side:
	//
	//       Left    Right    Result
	//       1       1        1
	//       1       2        2
