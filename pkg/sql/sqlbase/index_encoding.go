...
func EncodePartialIndexKey(
	...
	if len(index.Interleave.Ancestors) > 0 {
		for i, ancestor := range index.Interleave.Ancestors {
			...
			var n bool
			key, n, err = EncodeKeyColumns(colIDs[:length], dirs[:length], colMap, values, key)
			if err != nil {
	...
	var n bool
	key, n, err = EncodeKeyColumns(colIDs, dirs, colMap, values, key)
	containsNull = containsNull || n
	return key, containsNull, err
}
...
// EncodeKeyColumns is a version of EncodePartialIndexKey that takes ColumnIDs and
// directions explicitly. WARNING: unlike EncodePartialIndexKey, EncodeKeyColumns
// appends directly to keyPrefix.
func EncodeKeyColumns(
	columnIDs []ColumnID,
...
// key part only contains part of the datum's information.
...
// This is used when decoding key prefixes.
...
func EncodeSecondaryIndex(
	...
	extraKey, _, err := EncodeKeyColumns(secondaryIndex.ExtraColumnIDs, nil,
		colMap, values, nil)
	if err != nil {
		return []IndexEntry{}, err
	}
...
// TestingMakePrimaryIndexKey creates a key prefix that corresponds to a table row
