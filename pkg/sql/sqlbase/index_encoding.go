...
func NeededColumnFamilyIDs(
	...
	// compositeNeeded tracks if any composite columns are needed. This is
	// relevant for secondary indexes, which store all composite encodings in
	// family 0.
	compositeNeeded := false
	...
	for _, columnID := range index.CompositeColumnIDs {
		...
		compositeNeeded = compositeNeeded || neededCols.Contains(colIdxMap[columnID])
	}
	...
	for i := range table.Families {
		...
		if family.ID == 0 {
			...
			family0 = family
			if index.EncodingType == SecondaryIndexEncoding && compositeNeeded {
				// Secondary indexes store all composite encodings in family 0.
				needed = true
			}
			nullable = false
		}
	...
	// Note that this is only the case if family 0 was not already added to
	// neededFamilyIDs.
