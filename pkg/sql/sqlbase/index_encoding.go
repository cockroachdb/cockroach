...
func DecodeIndexKeyWithoutTableIDIndexIDPrefix(
	...
	if len(index.Interleave.Ancestors) > 0 {
		for i, ancestor := range index.Interleave.Ancestors {
			...
			key, _, err = DecodeKeyVals(types[:length], vals[:length], colDirs[:length], key)
			if err != nil {
				return nil, false, false, err
			}
...
func EncodeSecondaryIndex(
	...
	for _, key := range secondaryKeys {
		...
		// TODO (rohany): also add if there is a mixed cluster version less than the version I want.
		//  Its unclear how to plumb a cluster.Settings object all the way down here, or if that
		//  really makes sense in terms of abstraction. Any suggestions?
		...
		if len(tableDesc.Families) == 1 || secondaryIndex.Type == IndexDescriptor_INVERTED {
