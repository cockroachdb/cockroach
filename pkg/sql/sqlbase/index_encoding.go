...
// AppendSpansAfterSpanSplitIntoSeparateFamilies splits a span representing a
// single row point lookup into separate disjoint spans that request only the
// particular column families from neededFamilies instead of requesting all the
// families. It is up to the client to ensure the requested span represents a
// single row lookup and that the span splitting is appropriate (see
// CanSplitSpanIntoSeparateFamilies).
//
// The function accepts a slice of spans to append to.
func AppendSpansAfterSpanSplitIntoSeparateFamilies(
