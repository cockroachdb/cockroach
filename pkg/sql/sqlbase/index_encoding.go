...
func EncodePrimaryIndex(
	...
	isIndexed := func(col ColumnID) bool {
		_, ok := indexedColumns[col]
		return ok
	}
	isComposite := func(col ColumnID) bool {
		cdatum, ok := values[colMap[col]].(tree.CompositeDatum)
		if ok && cdatum.IsComposite() {
			return true
		}
		return false
	}
