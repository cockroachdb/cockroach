...
func MakeSpanFromEncDatums(
	...
	if complete && index.Unique {
		...
		//
		// Note that currently only primary indexes can contain interleaved tables,
		// so this condition is broader than necessary in case one day we permit
		// interleaving into unique indexes. Note also that we could precisely only
		// emit an interleaved sentinel if this index does in fact have
		// interleaves - we choose not to do that to make testing simpler and traces
		// and spans more consistent.
