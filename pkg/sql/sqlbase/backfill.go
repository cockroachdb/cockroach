...
type MutationFilter func(DescriptorMutation) bool

// ColumnMutationFilter is a filter that allows mutations that add or drop
// columns.
func ColumnMutationFilter(m DescriptorMutation) bool {
	return m.GetColumn() != nil && (m.Direction == DescriptorMutation_ADD || m.Direction == DescriptorMutation_DROP)
}

// IndexMutationFilter is a filter that allows mutations that add indexes.
func IndexMutationFilter(m DescriptorMutation) bool {
	return m.GetIndex() != nil && m.Direction == DescriptorMutation_ADD
}
...
// ColumnBackfiller is capable of running a column backfill.
...
func (cb *ColumnBackfiller) Init(evalCtx *tree.EvalContext, desc TableDescriptor) error {
	cb.evalCtx = evalCtx
	var dropped []ColumnDescriptor
	...
	defaultExprs, err := MakeDefaultExprs(
	...
	computedExprs, err := MakeComputedExprs(cb.added, &desc,
...
// RunColumnBackfillChunk runs column backfill for all updateCols.
func (cb *ColumnBackfiller) RunColumnBackfillChunk(
	...
	// makeRowUpdater.
	requestedCols := make([]ColumnDescriptor, 0, len(tableDesc.Columns)+len(cb.added))
	...
	for i := int64(0); i < chunkSize; i++ {
		...
		for j, e := range cb.updateExprs {
			...
			if err != nil {
				return roachpb.Key{}, NewInvalidSchemaDefinitionError(err)
			}
			if j < len(cb.added) && !cb.added[j].Nullable && val == tree.DNull {
				return roachpb.Key{}, NewNonNullViolationError(cb.added[j].Name)
			}
			updateValues[j] = val
		}
		...
		if _, err := ru.UpdateRow(
			ctx, b, oldValues, updateValues, CheckFKs, false, /* traceKV */
		); err != nil {
			return roachpb.Key{}, err
		}
	}
...
func ConvertBackfillError(ctx context.Context, tableDesc *TableDescriptor, b *client.Batch) error {
	...
	desc := protoutil.Clone(tableDesc).(*TableDescriptor)
	...
	return ConvertBatchError(ctx, desc, b)
}
...
func (ib *IndexBackfiller) Init(desc TableDescriptor) error {
	numCols := len(desc.Columns)
	cols := desc.Columns
	if len(desc.Mutations) > 0 {
		cols = make([]ColumnDescriptor, 0, numCols+len(desc.Mutations))
	...
	ib.types = make([]ColumnType, len(cols))
	for i := range cols {
		ib.types[i] = cols[i].Type
	}

	ib.colIdxMap = make(map[ColumnID]int, len(cols))
	for i, c := range cols {
		ib.colIdxMap[c.ID] = i
	}
...
// BuildIndexEntriesChunk reads a chunk of rows from a table and builds the
// added indexes.
func (ib *IndexBackfiller) BuildIndexEntriesChunk(
	...
	for i := int64(0); i < chunkSize; i++ {
		...
		if err := EncDatumRowToDatums(ib.types, ib.rowVals, encRow, &ib.alloc); err != nil {
			return nil, nil, err
		}
...
// RunIndexBackfillChunk runs index backfill for all added indexes.
