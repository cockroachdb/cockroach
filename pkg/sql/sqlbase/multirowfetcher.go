...
type MultiRowFetcher struct {
	...
	// allEquivSignatures is a set used for checking if an equivalence
	// signature belongs to any table or table's ancestor. It stores the
	// string representation of every table's and every table's ancestors'
	// signature.
	allEquivSignatures map[string]bool
...
func (mrf *MultiRowFetcher) Init(
	...
	mrf.alloc = alloc
	mrf.allEquivSignatures = make(map[string]bool, len(tables))
	...
	for _, tableArgs := range tables {
		...
		for _, sig := range equivSignatures {
			mrf.allEquivSignatures[string(sig)] = true
		}
...
func (mrf *MultiRowFetcher) ReadIndexKey(key roachpb.Key) (remaining []byte, ok bool, err error) {
	...
	// signature in order.
	keyEquivSignature, key, match, err := IndexKeyEquivSignature(key, mrf.allEquivSignatures)
	if err != nil {
	...
	for {
		if bytes.Equal(keyEquivSignature, mrf.tables[idx].equivSignature) {
