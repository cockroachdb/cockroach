...
func (mrf *MultiRowFetcher) Init(
	...
	for _, tableArgs := range tables {
		...
		var err error
		if table.equivSignature, err = TableEquivSignature(table.desc, table.index); err != nil {
			return err
		}
...
func (mrf *MultiRowFetcher) ReadIndexKey(key roachpb.Key) (remaining []byte, ok bool, err error) {
	...
	keyEquivSignature, colVals, err := IndexKeyEquivSignature(key)
	if err != nil {
	...
	// when processing the index key in one go.
	if colVals, err = DecodeKeyVals(
		mrf.currentTable.keyVals,
		mrf.currentTable.indexColumnDirs,
		colVals,
	); err != nil {
	...
	// The size of the signature and the colVals slice is exactly
	// the bytes we processed from the front key.
	// We return the rest as required.
	return key[len(keyEquivSignature)+len(colVals):], true, nil
}
