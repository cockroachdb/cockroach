...
type tableInfo struct {
	...
	// Column types must be associated with each table (whereas we can use
	// a general []EncDatum buffer for keyVals and extraVals).
...
type MultiRowFetcher struct {
	...
	keyValsBuf     []EncDatum  // backing slice for keyVals (for more than  one table)
	extraVals      EncDatumRow // the extra column values for unique indexes
	extraValsBuf   EncDatumRow // backing row for extraVals (for more than one table)
...
func (mrf *MultiRowFetcher) Init(
	...
	// Keep track of the maximum number of index columns and extra values
	// for a given table to initialize mrf.keyVals and mrf.extraVals,
	// respectively.
	var maxIndexCols, maxExtraVals int
	for tableIdx, tableArgs := range tables {
		...
		for _, sig := range equivSignatures {
			// Map each table's signature and the table's ancestors' signatures
			// to the table's index in tables.
			// Note ancestor signatures will be overwritten by
			// later tables but the table's full signature will
			// be mapped to its own tableInfo in tables.
		...
		if len(indexColumnIDs) > maxIndexCols {
			maxIndexCols = len(indexColumnIDs)
		}
		...
		if hasExtraCols(&table) {
			...
			if len(table.index.ExtraColumnIDs) > maxExtraVals {
				maxExtraVals = len(table.index.ExtraColumnIDs)
			}
		}
	...
	// Initialize the index key value and extra value buffers with
	// the maximum possible sizes for each.
	mrf.keyValsBuf = make([]EncDatum, maxIndexCols)
	mrf.keyVals = mrf.keyValsBuf
	// There will be extra values if and only if there was a unique secondary index.
	if maxExtraVals > 0 {
		mrf.extraValsBuf = make([]EncDatum, maxExtraVals)
		mrf.extraVals = mrf.extraValsBuf
	}
...
func (mrf *MultiRowFetcher) ReadIndexKey(key roachpb.Key) (remaining []byte, ok bool, err error) {
	...
	// key now contains the bytes in the key that are not part of the
	// signature in order.
	keyEquivSignature, key, match, err := IndexKeyEquivSignature(key, mrf.allEquivSignatures, mrf.keySigBuf, mrf.keyRestBuf)
	if err != nil {
		return nil, false, err
	}
	// IndexKeyEquivSignature short-circuited which means part of the index
	// key's signature did not match any of mrf.allEquivSignatures.
	if !match {
		return nil, false, nil
	}

	tableIdx, found := mrf.allEquivSignatures[string(keyEquivSignature)]
	// keyEquivSignature should always exist at this point; otherwise,
	// match would have been false.
	if !found {
		panic("full index key equivalence signature cannot be found")
	}

	mrf.currentTable = &mrf.tables[tableIdx]
	// Restrict mrf.keyVals to the relevant index key indices of the
	// current table from the buffer.
	mrf.keyVals = mrf.keyValsBuf[:len(mrf.currentTable.keyValTypes)]
...
func (mrf *MultiRowFetcher) processKV(
	...
	if mrf.traceKV {
		prettyKey = fmt.Sprintf(
			...
			table.desc.Name,
			table.index.Name,
			mrf.prettyEncDatums(table.keyValTypes, mrf.keyVals),
		)
	}
	...
	if !table.isSecondaryIndex && len(mrf.keyRemainingBytes) > 0 {
		...
		if hasExtraCols(table) {
			...
			var err error
			// Restrict mrf.extraVals to the relevant extra values
			// indices of the current table from the buffer.
			mrf.extraVals = mrf.extraValsBuf[:len(table.extraTypes)]
			...
			for i, id := range table.index.ExtraColumnIDs {
				if table.neededCols.Contains(int(id)) {
					table.row[table.colIdxMap[id]] = mrf.extraVals[i]
				}
			}
			if mrf.traceKV {
				prettyValue = mrf.prettyEncDatums(table.extraTypes, mrf.extraVals)
			}
		}

		if debugRowFetch {
			if hasExtraCols(table) {
				log.Infof(ctx, "Scan %s -> %s", kv.Key, mrf.prettyEncDatums(table.extraTypes, mrf.extraVals))
			} else {
...
func (mrf *MultiRowFetcher) NextRowDecoded(
	...
	for i, encDatum := range resp.Row {
		if encDatum.IsUnset() {
			mrf.currentTable.decodedRow[i] = parser.DNull
			continue
		}
		if err := encDatum.EnsureDecoded(&mrf.currentTable.cols[i].Type, mrf.alloc); err != nil {
			return DecodedRowResponse{}, err
		}
		mrf.currentTable.decodedRow[i] = encDatum.Datum
	}
