...
// LessDatums compares two datum rows according to a column ordering. Returns true
// if lhs sorts before rhs, false otherwise.
func LessDatums(
	ordering ColumnOrdering, invertSort bool, evalCtx *tree.EvalContext, lhs, rhs tree.Datums,
) bool {
	for _, c := range ordering {
		if invertSort != (c.Direction == encoding.Ascending) {
			if tree.IsLowerOrdered(evalCtx, lhs[c.ColIdx], rhs[c.ColIdx]) {
				return true
			}
		} else {
			if tree.IsLowerOrdered(evalCtx, rhs[c.ColIdx], lhs[c.ColIdx]) {
				return true
			}
		}
	}
