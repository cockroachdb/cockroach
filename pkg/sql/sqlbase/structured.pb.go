...
type TableDescriptor struct {
	...
	// A list of dropped name mappings.
	//
	// When this is observed by a schema changer, the dropped name map entries
	// are purged from the cluster wide name caches by incrementing the version
	// for every descriptor id and ensuring that there are no leases on prior
	// versions of the descriptors. This field is then cleared with the version
	// of the descriptor incremented.
	//
	// A table descriptor with a dropped_name_map entry with the same name
	// as the descriptor cannot be leased. This is to enforce the name can only
	// name one descriptor invariant. This also results in a period of
	// unavailability of the name.
	//
	DroppedNameMap []TableDescriptor_NameMap `protobuf:"bytes,21,rep,name=dropped_name_map,json=droppedNameMap" json:"dropped_name_map"`
...
func (m *TableDescriptor) GetDroppedNameMap() []TableDescriptor_NameMap {
	if m != nil {
		return m.DroppedNameMap
	}
	return nil
}
...
// which makes reassigning a name complicated. In particular, since a
// name cannot be withdrawn across a cluster in a transaction at
// timestamp T, three kinds of inconsistencies can occur:
//
...
// can be used by transaction at timestamps >= T.
// 3. A name foo is reassigned from one table to another at T, and the name
// foo can be used to reference two different tables at timestamps >= T.
//
// To support all three inconsistencies as naturally as possible, we
// introduce a new invariant: a name can only name one descriptor at a
// timestamp. This invariant is true for inconsistencies 1 and 2, but
// not for 3.
//
// Mechanics:
//
// The transaction at T is followed by another transaction at timestamp S,
// which when committed guarantees that an old cached name reference
// has been purged from the cluster. Consider a transaction at T in which
// name B is dropped, name C is created, and name D is reassigned. name C
// is viable as soon as the transaction commits. When transaction at S
// commits, the name B is no more, and C is viable through its new assignment.
// The interval from T to S is called the name drain interval.
// Note: The term reassign is used to describe a name reassignment
// within a transaction as well as an assignment of a yet to drain name
// (dropped in a prior transaction).
//
// This transaction at S runs through the schema changer, with the SQL
// transaction that initiated transaction at T completing only once
// transaction at S is committed. So effectively the SQL transaction once
// it returns can be followed by client transactions that do not observe
// the old references.
//
...
// the partial effect of a committed transaction. For instance during the
// drain interval a transaction will see the correct assignment for C,
// and the old assignments for B and D.
//
// In order for transaction at S to act properly each name maintains
// a prior owner, which the schema changer can lookup
// to check if the prior usage of the name has been purged.
// The name maintains this through the table descriptor it currently
// references, which is the descriptor about to own the name (D), or the
// descriptor last owning the name (when there is no current descriptor
// with the name; case B). Specifically, the system.namespace table entry
// for a name will reference the descriptor holding a prior owner
// of the name. There can be multiple prior owners, since multiple names
// are allowed to reference a single descriptor. The schema changer
// will push a new version of all prior owner descriptors in the list
// to speed up purging prior usage of the name.
//
// Note: There can be a period of time during the drain interval when
// the database is unable to use a name that was reassigned. The user
// will see an error saying the name is undergoing reassignment.
//
// Name map: {parent_id, table_name} -> id
type TableDescriptor_NameMap struct {
	...
	Name     string `protobuf:"bytes,2,opt,name=name" json:"name"`
	// Previous owner of the above name mapping. For backwards compatibility
	// if this is missing it's the same value as the id of the descriptor.
	ID ID `protobuf:"varint,3,opt,name=id,casttype=ID" json:"id"`
}

func (m *TableDescriptor_NameMap) Reset()         { *m = TableDescriptor_NameMap{} }
func (m *TableDescriptor_NameMap) String() string { return proto.CompactTextString(m) }
func (*TableDescriptor_NameMap) ProtoMessage()    {}
func (*TableDescriptor_NameMap) Descriptor() ([]byte, []int) {
...
func init() {
	...
	proto.RegisterType((*TableDescriptor_NameMap)(nil), "cockroach.sql.sqlbase.TableDescriptor.NameMap")
...
func (m *TableDescriptor) MarshalTo(dAtA []byte) (int, error) {
	...
	if len(m.DroppedNameMap) > 0 {
		for _, msg := range m.DroppedNameMap {
			dAtA[i] = 0xaa
			i++
			dAtA[i] = 0x1
			i++
...
func (m *TableDescriptor_NameMap) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalTo(dAtA)
	if err != nil {
...
func (m *TableDescriptor_NameMap) MarshalTo(dAtA []byte) (int, error) {
	var i int
	_ = i
	var l int
	_ = l
...
func (m *TableDescriptor) Size() (n int) {
	...
	if len(m.DroppedNameMap) > 0 {
		for _, e := range m.DroppedNameMap {
			l = e.Size()
			n += 2 + l + sovStructured(uint64(l))
		}
	}
...
func (m *TableDescriptor_NameMap) Size() (n int) {
	var l int
	_ = l
	n += 1 + sovStructured(uint64(m.ParentID))
	l = len(m.Name)
...
func (m *TableDescriptor) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 21:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field DroppedNameMap", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.DroppedNameMap = append(m.DroppedNameMap, TableDescriptor_NameMap{})
			if err := m.DroppedNameMap[len(m.DroppedNameMap)-1].Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 22:
...
func (m *TableDescriptor_NameMap) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: NameMap: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: NameMap: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 0 {
...
var fileDescriptorStructured = []byte{
	...
	,
}
