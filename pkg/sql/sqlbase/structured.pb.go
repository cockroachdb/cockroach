...
type TableDescriptor struct {
	...
	// The GC deadline is set when a table is truncated or dropped,
	// based on the current time and the GC TTL for the table. It's
	// the timestamp after which the table's contents will be cleared
	// from the cluster. In nanoseconds since the epoch.
	GCDeadline int64 `protobuf:"varint,29,opt,name=gc_deadline,json=gcDeadline" json:"gc_deadline"`
}
...
func (m *TableDescriptor) GetGCDeadline() int64 {
	if m != nil {
		return m.GCDeadline
	}
	return 0
...
//
// To support some of these inconsistencies as naturally as possible, we
// introduce a new invariant: a name can name only a single descriptor
// at a timestamp. This invariant is trivially true for inconsistencies
// 1 and 2, but not 3 (the name references two tables), so we're not
// not going to allow inconsistency 3.
//
// Mechanics of name reuse:
//
// The transaction at T is followed by a time period when names no longer
// in use are drained from the system. Once the old name is drained from
// the system another transaction at timestamp S is executed to release
// the name for future use. The interval from T to S is called the name
// drain interval and it allows a name to be drained across the cluster
// before it can be used for another table.
//
// Consider a transaction at T in which name B is dropped, a new name C is
// created, and name D is reassigned. Name C is viable as soon as the
// transaction commits. Reassignment of name D is just not permitted.
// When the transaction at S commits, the name B is released for reuse.
//
// The transaction at S runs through the schema changer, with the SQL
...
// a prior owner, which the schema changer can lookup to check if the
// prior usage of the name has been drained. The name maintains this
// in the table descriptor it currently references, which is the
// descriptor last owning the name.
//
// Specifically, the system.namespace table entry for a name will
// reference the descriptor holding a prior owner of the name requiring
// draining. There can be a period of time during the drain interval when
// the database is unable to use a name that was dropped. The user
// will see an error saying the name is being drained.
//
...
func (m *TableDescriptor) MarshalTo(dAtA []byte) (int, error) {
	...
	dAtA[i] = 0x1
	i++
	i = encodeVarintStructured(dAtA, i, uint64(m.GCDeadline))
	return i, nil
}
...
func (m *TableDescriptor) Size() (n int) {
	...
	n += 2 + sovStructured(uint64(m.GCDeadline))
	return n
}
...
func (m *TableDescriptor) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 29:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field GCDeadline", wireType)
			}
			m.GCDeadline = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.GCDeadline |= (int64(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
...
var fileDescriptorStructured = []byte{
	...
	,
}
