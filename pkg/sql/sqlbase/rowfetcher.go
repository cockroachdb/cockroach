...
// whether or not the kv was from a isNewSpan, and any errors that may have
// occurred.
func (rf *RowFetcher) nextKV(
	...
	if len(rf.kvs) != 0 {
		kv = rf.kvs[0]
		rf.kvs = rf.kvs[1:]
		newSpan = rf.isNewSpan
		rf.isNewSpan = false
		return true, kv, newSpan, nil
	}
	ok, rf.kvs, rf.isNewSpan, err = rf.kvFetcher.nextBatch(ctx)
	if err != nil {
...
func (rf *RowFetcher) NextKey(ctx context.Context) (rowDone bool, err error) {
	...
	for {
		var isNewSpan bool
		ok, rf.kv, isNewSpan, err = rf.nextKV(ctx)
		if err != nil {
			return false, err
		}
		rf.kvEnd = !ok
		...
		case rf.rowReadyTable != rf.currentTable:
			...
			rowDone = true
		case isNewSpan:
			// If the kvFetcher reports that a isNewSpan was fetched, then the last
