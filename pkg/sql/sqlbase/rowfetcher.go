...
func (rf *RowFetcher) processValueSingle(
	...
	if ok && (debugStrings || rf.valNeededForCol[idx]) {
		...
		// TODO(arjun): How do we do so? What does UnmarshalColumnValue do that
		// DecodeTableValue does not?
...
func (rf *RowFetcher) processValueTuple(
	...
	for len(tupleBytes) > 0 {
		...
		lastColID = colID
		idx, ok := rf.colIdxMap[colID]
		// TODO(arjun): Now that rowFetcher generates EncDatums instead of Datums,
		// we no longer need valNeededForCol. Remove it entirely from RowFetcher.
		// Doing so requires fixing some logic tests that rely on it.
