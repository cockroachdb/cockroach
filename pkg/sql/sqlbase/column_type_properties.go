...
//
func DatumTypeToColumnType(ptyp types.T) (catpb.ColumnType, error) {
	var ctyp catpb.ColumnType
	switch t := ptyp.(type) {
	case types.TCollatedString:
		ctyp.SemanticType = catpb.ColumnType_COLLATEDSTRING
		ctyp.Locale = &t.Locale
	case types.TArray:
		ctyp.SemanticType = catpb.ColumnType_ARRAY
		contents, err := catpb.DatumTypeToColumnSemanticType(t.Typ)
		if err != nil {
			return catpb.ColumnType{}, err
		}
	...
	case types.TTuple:
		ctyp.SemanticType = catpb.ColumnType_TUPLE
		ctyp.TupleContents = make([]catpb.ColumnType, len(t.Types))
		...
		return ctyp, nil
	default:
		semanticType, err := catpb.DatumTypeToColumnSemanticType(ptyp)
		if err != nil {
			return catpb.ColumnType{}, err
		}
...
func PopulateTypeAttrs(base catpb.ColumnType, typ coltypes.T) (catpb.ColumnType, error) {
	switch t := typ.(type) {
	case *coltypes.TBitArray:
		if t.Width > math.MaxInt32 {
			return catpb.ColumnType{}, fmt.Errorf("bit width too large: %d", t.Width)
		}
		base.Width = int32(t.Width)
		if t.Variable {
			base.VisibleType = catpb.ColumnType_VARBIT
		}
	...
	case *coltypes.TInt:
		...
		switch t.Width {
		case 16:
			base.VisibleType = catpb.ColumnType_SMALLINT
		case 64:
			base.VisibleType = catpb.ColumnType_BIGINT
		case 32:
			base.VisibleType = catpb.ColumnType_INTEGER
		}

	case *coltypes.TFloat:
		base.VisibleType = catpb.ColumnType_NONE
		if t.Short {
			base.VisibleType = catpb.ColumnType_REAL
		}
	...
	case *coltypes.TDecimal:
		...
		case base.Precision == 0 && base.Width > 0:
			...
			return catpb.ColumnType{}, errors.New("invalid NUMERIC precision 0")
		case base.Precision < base.Width:
			return catpb.ColumnType{}, fmt.Errorf("NUMERIC scale %d must be between 0 and precision %d",
	...
	case *coltypes.TVector:
		...
		default:
			return catpb.ColumnType{}, errors.Errorf("vectors of type %s are unsupported", t.ParamType)
		}
	...
	case *coltypes.TUUID:
	default:
		return catpb.ColumnType{}, errors.Errorf("unexpected type %T", t)
	}
	return base, nil
}
...
func coltypeStringVariantToVisibleType(c coltypes.TStringVariant) catpb.ColumnType_VisibleType {
	switch c {
	case coltypes.TStringVariantVARCHAR:
		return catpb.ColumnType_VARCHAR
	case coltypes.TStringVariantCHAR:
		return catpb.ColumnType_CHAR
	case coltypes.TStringVariantQCHAR:
		return catpb.ColumnType_QCHAR
	default:
		return catpb.ColumnType_NONE
	}
}

// ColumnTypesToDatumTypes converts a slice of ColumnTypes to a slice of
// datum types.
func ColumnTypesToDatumTypes(colTypes []catpb.ColumnType) []types.T {
...
func LimitValueWidth(
	typ catpb.ColumnType, inVal tree.Datum, name *string,
) (outVal tree.Datum, err error) {
	switch typ.SemanticType {
	case catpb.ColumnType_STRING, catpb.ColumnType_COLLATEDSTRING:
		var sv string
...
func CheckDatumTypeFitsColumnType(
	col catpb.ColumnDescriptor, typ types.T, pmap *tree.PlaceholderInfo,
) error {
