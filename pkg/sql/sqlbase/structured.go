...
// GetColumnMutationCapabilities returns:
// 1. Whether the column is a mutation
// 2. if so, is it in state DELETE_AND_WRITE_ONLY
func (desc *MutableTableDescriptor) GetColumnMutationCapabilities(id ColumnID) (bool, bool) {
	for _, mutation := range desc.Mutations {
...
// AddCheckAddMutation adds an add check constraint mutation to desc.Mutations.
// If moveInitialState is specified, the initial state of the mutation is
// DELETE_AND_WRITE_ONLY.
func (desc *MutableTableDescriptor) AddCheckAddMutation(
	c TableDescriptor_CheckConstraint, moveInitialState bool,
) {
	desc.addCheckMutation(c, DescriptorMutation_ADD)
	if moveInitialState {
		// If all columns are write-only or public, the constraint can start immediately in
		// write-only and can (and will) also be immediately enforced.
		desc.Mutations[len(desc.Mutations)-1].State = DescriptorMutation_DELETE_AND_WRITE_ONLY
	}
}
