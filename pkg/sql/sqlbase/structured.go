...
func (m *Descriptor) Table(ts hlc.Timestamp) *TableDescriptor {
	t := m.GetTable()
	if t != nil {
		t.maybeSetModificationTimeFromMVCCTimestamp(ts)
	}
	return t
}
...
func (desc *TableDescriptor) maybeSetModificationTimeFromMVCCTimestamp(ts hlc.Timestamp) {
	...
	if desc.ModificationTime.IsEmpty() && ts.IsEmpty() {
		log.Fatalf(context.TODO(), "read table descriptor without Modification time"+
			"with zero MVCC timestamp")
	} else if desc.ModificationTime.IsEmpty() {
		desc.ModificationTime = ts
	} else if !ts.IsEmpty() && ts.Less(desc.ModificationTime) {
		log.Fatalf(context.TODO(), "read table descriptor which has a modification "+
			"time after its MVCC timestamp: has %v, expected less than %v %+v",
