...
func (desc *MutableTableDescriptor) RenameColumnDescriptor(
	...
	// Mutate copy of existing descriptor.
	copy := *column
	colID := copy.ID
	copy.Name = newColName
	desc.UpdateColumnDescriptor(&copy)
	for i := range desc.Families {
...
// UpdateColumnDescriptor updates an existing column descriptor. The caller is
// responsible for passing in a column descriptor that will not be mutated or
// overwritten (it will be saved in the mutation descriptor).
...
// AddColumnMutation adds a column mutation to desc.Mutations. The caller is
// responsible for making sure the column descriptor is immutable, because it
// will be saved in the descriptor mutation.
func (desc *MutableTableDescriptor) AddColumnMutation(
	c *ColumnDescriptor, direction DescriptorMutation_Direction,
) {
	// Save copy of column since it may change or be overwritten.
