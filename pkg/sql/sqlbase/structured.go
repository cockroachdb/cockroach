...
// table indexes are well-formed. Checks include validating the table-level
// uniqueness of all partition names, validating that the encoded tuples match
// the corresponding column types, and that range partitions are stored sorted
// by upper bound.
func (desc *TableDescriptor) validatePartitioning() error {
	partitionNames := make(map[string]struct{})

	a := &DatumAlloc{}
	translateValueEncodingToKey := func(
		valueEncBuf []byte, colIDs []ColumnID,
	...
	check = func(idxDesc *IndexDescriptor, partDesc *PartitioningDescriptor, colOffset int) error {
		...
		if len(partDesc.List) > 0 {
			...
			for _, p := range partDesc.List {
				...
				for _, valueEncBuf := range p.Values {
					datums, keyEncBuf, err := translateValueEncodingToKey(valueEncBuf, colIDs)
					if err != nil {
		...
		if len(partDesc.Range) > 0 {
			...
			for _, p := range partDesc.Range {
				...
				// indexed.
				datums, keyEncBuf, err := translateValueEncodingToKey(p.UpperBound, colIDs)
				if err != nil {
					return fmt.Errorf("decoding %s: %v", p.Name, err)
				}
