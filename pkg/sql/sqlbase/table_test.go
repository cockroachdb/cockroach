...
func TestTableEquivSignatures(t *testing.T) {
	hierarchy := map[string]*interleaveInfo{
		"t1": &interleaveInfo{
			tableID: 50,
			children: map[string]*interleaveInfo{
				"t2": &interleaveInfo{
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for i, table := range tc.tables {
				...
				if keyEquivSigs[i], restKey, _, err = IndexKeyEquivSignature(primaryKey, nil /*validAncesetorSignatures*/); err != nil {
					t.Fatal(err)
				}
				if len(keyEquivSigs[i])+len(restKey) != len(primaryKey) {
					t.Fatalf(
						"signature and rest of key slices do not add up to length of original key: expected %d, actual %d",
						len(primaryKey),
						len(keyEquivSigs[i])+len(restKey),
					)
				}
