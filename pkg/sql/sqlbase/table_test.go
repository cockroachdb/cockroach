...
func TestIndexKeyEquivSignature(t *testing.T) {
	...
	for _, tc := range createEquivTCs(hierarchy) {
		t.Run(tc.name, func(t *testing.T) {
			...
			keyEquivSig, restKey, valid, tableIdx, err := IndexKeyEquivSignature(primaryKey, validEquivSigs, keySigBuf, keyRestBuf)
			if err != nil {
				t.Fatal(err)
			}
			if !valid {
			...
			if validEquivSigs[string(keyEquivSig)] != tableIdx {
				t.Fatalf("unexpected table index returned from IndexKeyEquivSignature.\nexpected: %d\nactual: %d", validEquivSigs[string(keyEquivSig)], tableIdx)
			}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for i, table := range tc.tables {
				...
				// Extract out the index key's equivalence signature.
				validEquivSigs := make(map[string]int)
				for i, sig := range tempEquivSigs {
					validEquivSigs[string(sig)] = i
				}
				if keyEquivSigs[i], _, _, _, err = IndexKeyEquivSignature(primaryKey, validEquivSigs, nil /*keySigBuf*/, nil /*keyRestBuf*/); err != nil {
					t.Fatal(err)
				}
			}
