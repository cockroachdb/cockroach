...
func TestIndexKeyEquivSignature(t *testing.T) {
	...
	for _, tc := range createEquivTCs(hierarchy) {
		t.Run(tc.name, func(t *testing.T) {
			...
			keyEquivSig, restKey, valid, err := IndexKeyEquivSignature(primaryKey, validEquivSigs, keySigBuf, keyRestBuf)
			if err != nil {
				t.Fatal(err)
			}
			if !valid {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for i, table := range tc.tables {
				...
				if keyEquivSigs[i], _, _, err = IndexKeyEquivSignature(primaryKey, validEquivSigs, nil /*keySigBuf*/, nil /*keyRestBuf*/); err != nil {
					t.Fatal(err)
				}
			}
