...
// invoking TableEquivSignature on the table-index itself.
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for i, table := range tc.tables {
				...
				if keyEquivSigs[i], _, err = IndexKeyEquivSignature(primaryKey); err != nil {
					t.Fatal(err)
				}
				if tableEquivSigs[i], err = TableEquivSignature(&desc, &desc.PrimaryIndex); err != nil {
					t.Fatal(err)
				}
			}
