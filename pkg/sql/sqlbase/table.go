...
// column values of ancestors at the beginning. It requires two []byte buffers:
// one for 'signature' and one for the 'rest' of the key.
...
func IndexKeyEquivSignature(
	key []byte, validEquivSignatures map[string]int, signature []byte, rest []byte,
	...
	for {
		...
		if _, found := validEquivSignatures[string(signature)]; !found {
