...
func TightenEndKey(
	...
	numNonInterleavedCols := len(index.ColumnIDs) - totalSharedPrefixLen(index)

	if len(keyTokens) <= nIndexTokens-numNonInterleavedCols {
		// Case 1: parent key:
		//    /1 --> /1
	...
	// Always >= 0 from the above if condition.
	// lastCol refers to the last index column that the last key token
	// (i.e. the column's value) refers to in the index.
	lastColIdx := len(index.ColumnIDs) - (nIndexTokens - len(keyTokens)) - 1

	if nextColIdx := lastColIdx + 1; nextColIdx < len(index.ColumnIDs) {
		// We want to check if the next column is NULLable and has
		// direction DESC.
		// If we have a table with a two-column index (PK1, PK2 DESC)
		// we can't tighten
		//	/5 --> /4/#
		// for NULLable DESC columns since the above tightening will
		// omit the keys with prefix
		//    /4/NULL
		// where NULL is an encodedNullDesc.
		nextColDesc, err := table.FindColumnByID(index.ColumnIDs[nextColIdx])
		if err != nil {
			return roachpb.Key{}, err
		}
		if nextColDesc.Nullable && index.ColumnDirections[nextColIdx] == IndexDescriptor_DESC {
			// We'll still need to PrefixEnd this if it's still
			// inclusive.
			if inclusive {
				end = end.PrefixEnd()
			}
	...
	// len(keyTokens) > nIndexTokens - <# of non-interleaved index columns>
	// This means there is at least one key token that refers to a non-shared
	// column of this index.
	if len(keyTokens) <= nIndexTokens {
		// Case 2: child key

		// The last key token is a non-interleaved index column.
...
func totalSharedPrefixLen(index *IndexDescriptor) (sharedLen int) {
	for _, ancs := range index.Interleave.Ancestors {
		sharedLen += int(ancs.SharedPrefixLen)
	}
