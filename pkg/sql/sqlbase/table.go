...
// index key, the byte slice of the rest of the key with the
// column values of ancestors at the beginning, and the table index from
// validEquivSignatures.
// validEquivSignatures: a map containing signatures of valid ancestors and
// of the table which the key belongs to as keys, and the table indexes the
// signatures is associated with.
// It also requires two []byte buffers: one for 'signature' (keySigBuf) and one
// for the 'rest' of the key (keyRestBuf).
...
// IndexKeyEquivSignature will return false if an ancestor table's or the key's
// equivalence signature is not found in validEquivSignatures.
// validEquivSignatures must not be nil.
...
// indexKeyEquivSignatureInternal will short-circuit and return false once
...
func IndexKeyEquivSignature(
	key []byte, validEquivSignatures map[string]int, keySigBuf []byte, keyRestBuf []byte,
) (signature []byte, rest []byte, success bool, tableIdx int, rr error) {
	signature = signature[:0]
	rest = rest[:0]
	var recentTableIdx int
	var isSentinel bool
	for {
		...
		var found bool
		if recentTableIdx, found = validEquivSignatures[string(signature)]; !found {
		...
		if isSentinel {
			...
			// descendant(s).
			// We insert an interleave sentinel and continue
			// extracting the next descendant's IDs.
	...
	return signature, rest, true, recentTableIdx, nil
}
