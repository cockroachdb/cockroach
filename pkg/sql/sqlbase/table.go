...
// index key as well as the byte slice of the rest of the key with the
// column values of ancestors at the beginning.
...
// If validAncestorSignatures is not nil, then IndexKeyEquivSignature will
// return false if an ancestor's equivalence signature is not found in
// validAncestorSignatures.
// For example, suppose the given key is
//    /<t2 table id>/<t2 index id>/<val t2>/#/<t3 table id>/<t3 table id>/<val t3>
// and valid AncestorSignatures contains
...
// is processed since t2 does not correspond to any of the specified ancestors.
func IndexKeyEquivSignature(
	key []byte, validAncestorSignatures map[string]bool,
) (signature []byte, rest []byte, success bool, err error) {
	var isSentinel bool
	signature = make([]byte, 0, len(key))
	rest = make([]byte, 0, len(key))
	for {
		...
		for {
			// The current ancestor is not one of the specified ancestors.
			// We can short-circuit and return false.
			if validAncestorSignatures != nil && !validAncestorSignatures[string(signature)] {
	...
	return signature, rest, true, nil
}
