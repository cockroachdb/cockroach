...
func TestRowFetcherSidebandDeletesAndTimestamp(t *testing.T) {
	...
	type rowWithSideband struct {
		PrimaryKey []string
		Deleted    bool
		Timestamp  string
	}
	kvsToRows := func(kvs []roachpb.KeyValue) []rowWithSideband {
		for _, kv := range kvs {
		...
		for {
			...
			row := rowWithSideband{
				Deleted:   rf.RowLastDeleted(),
				Timestamp: tree.TimestampToDecimal(rf.RowLastModifiedTimestamp()).String(),
			}
	...
	if actual, expected := kvsToRows(slurpUserDataKVs(t, store.Engine())), []rowWithSideband{
