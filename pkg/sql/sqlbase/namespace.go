...
// existing entries.
// In 20.1, the older system.namespace table is marked deprecated. It is replaced
// by a new system.namespace table that has an additional parentSchemaID column,
// which allows support for additional physical schemas. The new namespace table
// is also created outside the system config range, so it is no longer gossiped.
//
...
// TODO(whomever): The fallback semantics would no longer be required in 20.2.
...
// In a mixed version cluster, the entry only exists in the older system.namespace.
