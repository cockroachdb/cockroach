...
func (c *CheckHelper) Init(
	...
	exprStrings := make([]string, 0, len(tableDesc.Checks)+len(tableDesc.Mutations))
	for _, ck := range tableDesc.Checks {
		exprStrings = append(exprStrings, ck.Expr)
	}
	for _, m := range tableDesc.Mutations {
		if ck := m.GetCheck(); ck != nil && m.State == DescriptorMutation_DELETE_AND_WRITE_ONLY {
			exprStrings = append(exprStrings, ck.Expr)
		}
	}
	...
	c.sourceInfo = NewSourceInfoForSingleTable(
		*tn, ResultColumnsFromColDescs(tableDesc.AllNonDropColumns()),
	)
