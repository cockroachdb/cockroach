...
func (ri *RowInserter) InsertRow(
	...
	ri.valueBuf, err = prepareInsertOrUpdateBatch(ctx, b,
		...
		&ri.key, &ri.value, ri.valueBuf, putFn, delFn, overwrite, traceKV)
	if err != nil {
		return err
	}
...
// - putFn, delFn are the functions that can append commands to the batch.
...
func prepareInsertOrUpdateBatch(
	...
	delFn func(ctx context.Context, b putter, key *roachpb.Key, traceKV bool),
...
func (ru *RowUpdater) UpdateRow(
	...
	ru.valueBuf, err = prepareInsertOrUpdateBatch(ctx, b,
		...
		&ru.key, &ru.value, ru.valueBuf, insertPutFn, insertDelFn, true /* overwrite */, traceKV)
	if err != nil {
		return ru.newValues, nil
	}
