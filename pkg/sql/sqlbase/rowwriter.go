...
func MakeRowUpdater(
	...
	if primaryKeyColChange {
		...
		if ru.rd, err = MakeRowDeleter(txn, sc, tableDesc, fkTables, tableDesc.Columns, SkipFKs); err != nil {
			return RowUpdater{}, err
		}
		ru.FetchCols = ru.rd.FetchCols
		ru.FetchColIDtoRowIndex = ColIDtoRowIndexFromCols(ru.FetchCols)
		if ru.ri, err = MakeRowInserter(txn, sc, tableDesc, fkTables, tableDesc.Columns, SkipFKs); err != nil {
			return RowUpdater{}, err
		}
	} else {
...
func MakeRowDeleter(
	...
	if checkFKs {
		var err error
		if rd.Fks, err = makeFKDeleteHelper(txn, sc, *tableDesc, fkTables, fetchColIDtoRowIndex); err != nil {
			return RowDeleter{}, err
		}
	}
