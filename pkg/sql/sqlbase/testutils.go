...
func RandCreateTableWithInterleave(
	...
	if interleaveInto != nil {
		for i := range interleaveInto.Defs {
			if cd, ok := interleaveInto.Defs[i].(*tree.ColumnTableDef); ok {
				interleaveIntoColumnDefs[cd.Name] = cd
			}
			if uc, ok := interleaveInto.Defs[i].(*tree.UniqueConstraintTableDef); ok && uc.PrimaryKey {
				interleaveIntoPK = uc
			}
		}
	}
	...
	if interleaveIntoPK != nil && len(interleaveIntoPK.Columns) > 0 {
		...
		for i := range interleaveIntoPK.Columns[:prefixLength] {
			fmt.Println(interleaveIntoPK.Columns[i].Column, interleaveIntoColumnDefs)
		...
		pk := &tree.UniqueConstraintTableDef{
			...
			IndexTableDef: tree.IndexTableDef{
				Columns: interleaveIntoPK.Columns[:prefixLength],
			},
		}
