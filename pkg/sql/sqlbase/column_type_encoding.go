...
// [arrayMarker, elementMarker, enc(a), elementMarker, enc(b), terminator]
// We include this element markers to handle the following case.
// x, y = ARRAY[1], ARRAY[1, 2]. x is less than y, so we need the shorter
// array to sort first. The terminator is chosen to be less than the
// elementMarker so that this result is achieved. Note that we cannot
// include the encoding length or number of elements in the array at the
// front of the encoding, as this would affect the sorting of differently
// sizes arrays. The effect of this is that peeking/skipping over the
// encoded array is not as efficient as it could be.
func encodeArrayKey(b []byte, array *tree.DArray, dir encoding.Direction) ([]byte, error) {
	...
	for _, elem := range array.Array {
		b = encoding.EncodeArrayKeyElementMarker(b, dir)
		b, err = EncodeTableKey(b, elem, dir)
		if err != nil {
			return nil, err
		}
	}
	b = encoding.EncodeArrayKeyTerminator(b, dir)
	return b, nil
}
...
func decodeArrayKey(
	...
	for {
		...
		var more bool
		buf, more, err = encoding.ConsumeNextArrayElementMarker(buf, dir)
		if err != nil {
			return nil, nil, err
		}
		if !more {
