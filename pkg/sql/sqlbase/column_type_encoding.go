...
func EncodeTableValue(
	...
	case *tree.DGeometry:
		return encoding.EncodeGeoValue(appendTo, uint32(colID), t.Geometry), nil
...
func decodeUntaggedDatum(a *DatumAlloc, t *types.T, buf []byte) (tree.Datum, []byte, error) {
	...
	case types.GeometryFamily:
		...
		return a.NewDGeometry(tree.DGeometry{Geometry: data}), b, nil
	case types.GeographyFamily:
		b, data, err := encoding.DecodeUntaggedGeoValue(buf)
		if err != nil {
			return nil, b, err
		}
		return a.NewDGeography(tree.DGeography{Geometry: data}), b, nil
	case types.TimestampFamily:
...
func UnmarshalColumnValue(a *DatumAlloc, typ *types.T, value roachpb.Value) (tree.Datum, error) {
	...
	case types.GeometryFamily:
		...
		return a.NewDGeometry(tree.DGeometry{Geometry: v}), nil
	case types.GeographyFamily:
		v, err := value.GetGeo()
		if err != nil {
			return nil, err
		}
		return a.NewDGeography(tree.DGeography{Geometry: v}), nil
	case types.TimestampFamily:
		v, err := value.GetTime()
		if err != nil {
			return nil, err
...
func encodeArrayElement(b []byte, d tree.Datum) ([]byte, error) {
	...
	case *tree.DGeometry:
		return encoding.EncodeUntaggedGeoValue(b, t.Geometry), nil
	case *tree.DTimestamp:
