...
`); err != nil {
	...
	expected := []string{
		...
		fmt.Sprintf("Put /Table/%d/7/2/0 -> /BYTES/0x0a030a1302", tableID),
		fmt.Sprintf("Put /Table/%d/7/2/2/1 -> /BYTES/0x0a030a3306", tableID),
		fmt.Sprintf("Put /Table/%d/7/2/4/1 -> /BYTES/0x0a02010c", tableID),

		// Index 6 is currently being added, and it should have no writes because
		// it is in the BACKFILLING state at this point.
		// Index 8 (an additional unique index on the old primary key) is not
		// created yet.
		// Index 9, index 8's temporary index, is not create yet.
	}
	...
	expected = []string{
		...
		fmt.Sprintf("Put (delete) /Table/%d/7/2/0", tableID),
		fmt.Sprintf("Put (delete) /Table/%d/7/2/2/1", tableID),
		fmt.Sprintf("Put (delete) /Table/%d/7/2/3/1", tableID),
		fmt.Sprintf("Put (delete) /Table/%d/7/2/4/1", tableID),
	}
	require.Equal(t, expected, scanToArray(rows))
	...
	expected = []string{
		...
		fmt.Sprintf("Put /Table/%d/7/3/0 -> /BYTES/0x0a030a1302", tableID),
		fmt.Sprintf("Put /Table/%d/7/3/2/1 -> /BYTES/0x0a030a3306", tableID),
		fmt.Sprintf("Put /Table/%d/7/3/4/1 -> /BYTES/0x0a02010c", tableID),
	}
	require.Equal(t, expected, scanToArray(rows))
	...
	expected = []string{
		...
		fmt.Sprintf("Put /Table/%d/7/3/0 -> /BYTES/0x0a030a1302", tableID),
		fmt.Sprintf("Put /Table/%d/7/3/3/1 -> /BYTES/0x0a02010a", tableID),
	}
	require.Equal(t, expected, scanToArray(rows))
