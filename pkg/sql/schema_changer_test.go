...
`); err != nil {
	...
	index := protoutil.Clone(&tableDesc.Indexes[0]).(*descpb.IndexDescriptor)
	index.Name = "bar"
...
`); err != nil {
	...
	upTableVersion = func() {
		...
		if err := descs.Txn(ctx, s.ClusterSettings(), leaseMgr, ie, kvDB, func(
			...
			table, err := descriptors.GetMutableTableVersionByID(ctx, id, txn)
			if err != nil {
				return err
			}
			return descriptors.WriteDesc(ctx, false /* kvTrace */, table, txn)
		}); err != nil {
			t.Error(err)
		}
