...
func TestCancelSchemaChange(t *testing.T) {
	...
	if _, err := db.Exec(`ALTER TABLE t.test ADD COLUMN x DECIMAL DEFAULT (DECIMAL '1.4')`); !testutils.IsError(err, "cannot update progress on canceled job") {
		t.Fatalf("unexpected %v", err)
	}
	if _, err := db.Exec(`CREATE INDEX ON t.test (v)`); !testutils.IsError(err, "cannot update progress on canceled job") {
		t.Fatalf("unexpected %v", err)
	}
