...
`); err != nil {
	...
	tableDesc := desctestutils.TestingGetMutableExistingTableDescriptor(kvDB, keys.SystemSQLCodec, version, "t", "test")
	...
	for _, direction := range []descpb.DescriptorMutation_Direction{
		...
		for i := 0; i < 2; i++ {
			...
			tableDesc = desctestutils.TestingGetMutableExistingTableDescriptor(
				kvDB, keys.SystemSQLCodec, version, "t", "test")
			newVersion = tableDesc.Version
			if newVersion != expectedVersion {
				t.Fatalf("bad version; e = %d, v = %d", expectedVersion, newVersion)
			}
...
`); err != nil {
	...
	mTest := makeMutationTest(t, kvDB, sqlDB, tableDesc, version)
...
`); err != nil {
	...
	tableDesc := desctestutils.TestingGetMutableExistingTableDescriptor(
		kvDB, keys.SystemSQLCodec, version, "d", "t")
...
`); err != nil {
	...
	newTableDesc := desctestutils.TestingGetPublicTableDescriptor(kvDB, keys.SystemSQLCodec, version, "t", "test")
	...
	fkTableDesc := desctestutils.TestingGetPublicTableDescriptor(kvDB, keys.SystemSQLCodec, version, "t", "pi")
