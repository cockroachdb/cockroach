...
`); err != nil {
	...
	// Downgrade table to a time when there were no column families.
	tableDesc := sqlbase.GetTableDescriptor(kvDB, "t", "test")
	tableDesc.Families = nil
	tableDesc.NextFamilyID = 0
	tableDesc.FormatVersion = sqlbase.BaseFormatVersion
	if err := kvDB.Put(
		context.TODO(),
		sqlbase.MakeDescMetadataKey(tableDesc.ID),
		sqlbase.WrapDescriptor(tableDesc),
	...
	// Convert table data to sentinel values
	kvs, err := kvDB.Scan(
		context.TODO(),
	...
	for ; rows.Next(); count++ {
		...
		if count == setKey {
			if v := *val.(*interface{}); v != nil {
				...
				case int64:
					if setVal != value {
						t.Errorf("value = %d, expected %d", value, setVal)
					}

				default:
					t.Errorf("error input of type %T", value)
				}
			} else {
