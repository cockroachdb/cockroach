...
func getTableKeyCount(ctx context.Context, kvDB *kv.DB) (int, error) {
	tableDesc := sqlbase.GetTableDescriptor(kvDB, "t", "test")
	tablePrefix := keys.SystemTenantKeyGen.TablePrefix(uint32(tableDesc.ID))
...
`); err != nil {
	...
	startKey := keys.SystemTenantKeyGen.TablePrefix(uint32(tableDesc.ID))
	kvs, err := kvDB.Scan(
		ctx,
		startKey,
		startKey.PrefixEnd(),
...
`); err != nil {
	...
	tablePrefix = keys.SystemTenantKeyGen.TablePrefix(uint32(fkTableDesc.ID))
	tableEnd = tablePrefix.PrefixEnd()
	if kvs, err := kvDB.Scan(ctx, tablePrefix, tableEnd, 0); err != nil {
		t.Fatal(err)
		...
		DistSQL: &execinfra.TestingKnobs{
			RunAfterBackfillChunk: func() {
				...
				if count == 2 {
					...
					key := keys.SystemTenantKeyGen.IndexPrefix(uint32(tableDesc.ID), uint32(tableDesc.NextIndexID))
