...
func (p *planner) makeEqualityPredicate(
	...
	if len(leftColNames) != len(rightColNames) {
		return nil, nil, fmt.Errorf("left columns' length %q doesn't match right columns' length %q in EqualityPredicate",
...
// makeUsingyPredicate constructs a joinPredicate object for joins with
// a USING clause.
