...
func TestMakeStatement_sqlcommenter(t *testing.T) {
	...
	expectedTags := []sqlcommenter.Tags{
			...
			testutils.RunTrueAndFalse(t, "withComments", func(t *testing.T, withComments bool) {
				testutils.RunTrueAndFalse(t, "preparedStatement", func(t *testing.T, preparedStatement bool) {
					for _, query := range tc.queries {
						var withCommentsOption parser.ParseOption = func(*parser.ParseOptions) {}
						if withComments {
							withCommentsOption = parser.WithComments()
						}
						stmts, err := p.Parse(query, withCommentsOption)
						...
						if withComments {
							require.Equal(t, tc.expectedTags, stmt.SqlCommenterTags)
						} else {
