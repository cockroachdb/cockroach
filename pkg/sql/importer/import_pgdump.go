...
func bufferDDLPostgresStatement(
	...
	case *tree.Grant:
		...
		for _, table := range stmt.Targets.Tables.TablePatterns {
			...
			tableName, ok := normalizedTable.(*tree.TableName)
			if !ok {
				return errors.New("cannot get the table name of grant target")
			}
			if tableName.ExplicitCatalog {
				if err := checkCatalogName(tableName.CatalogName, tree.Name(handler.dumpDatabaseName)); err != nil {
					return err
				}
				tableName.CatalogName = TempImportPGDumpDBName
			}
		}
		handler.BufferedDDLStmts = append(handler.BufferedDDLStmts, formatStatement(stmt))
	case error:
