...
const (
	// TempImportPGDumpDBName is the default name of the temp database for newly
	// created schema from DDL stmts in the dump file.
	TempImportPGDumpDBName = "crdb_temp_pgdump_import"
)

// createTempImportDatabase creates a temporary database where we will create
// all the PGDUMP objects during the import.
func createTempImportDatabase(ctx context.Context, p sql.JobExecContext) error {
	id, err := p.ExecCfg().DescIDGenerator.GenerateUniqueDescID(ctx)
	if err != nil {
		return err
	}
	tempDBDesc := dbdesc.NewInitial(id, TempImportPGDumpDBName, username.NodeUserName())
	return p.ExecCfg().InternalExecutorFactory.DescsTxn(ctx, p.ExecCfg().DB, func(
		...
		b.CPut(catalogkeys.MakeDatabaseNameKey(p.ExecCfg().Codec, TempImportPGDumpDBName), tempDBDesc.GetID(), nil)
		return txn.Run(ctx, b)
	})
}

// PostgresDDLHandler stores DDL stmts and the temp db to store the new schemas.
...
func formatStatement(n tree.Statement) string {
	f := tree.NewFmtCtx(
...
func rewriteStatementTableName(n tree.Statement, dumpDatabaseName string) (string, error) {
	var err error
	f := tree.NewFmtCtx(
		// TODO(adityamaru): should this be serializable?
		tree.FmtParsable,
		tree.FmtReformatTableNames(func(fmtctx *tree.FmtCtx, tn *tree.TableName) {
			...
			if tn.CatalogName != "" {
				...
				tn.CatalogName = TempImportPGDumpDBName
			}
		}),
	)
	if err != nil {
...
func bufferDDLPostgresStatement(
	...
	case *tree.CreateRole:
		handler.BufferedDDLStmts = append(handler.BufferedDDLStmts, formatStatement(stmt))
	...
	case *tree.CreateSchema:
		...
		if stmt.Schema.ExplicitCatalog {
			...
			stmt.Schema.CatalogName = TempImportPGDumpDBName
		}
		handler.BufferedDDLStmts = append(handler.BufferedDDLStmts, formatStatement(stmt))
	case *tree.CreateTable:
		...
		s, err := rewriteStatementTableName(stmt, handler.dumpDatabaseName)
		if err != nil {
			return err
		}
	...
	case *tree.AlterTable:
		...
		if stmt.Table.HasExplicitCatalog() {
			...
			stmt.Table.Parts[2] = TempImportPGDumpDBName
		}
			...
			case *tree.AlterTableAddConstraint:
				...
				case *tree.ForeignKeyConstraintTableDef:
					...
					if con.Table.ExplicitCatalog {
						con.Table.CatalogName = TempImportPGDumpDBName
					}
				default:
	...
	case *tree.Insert, *tree.CopyFrom, *tree.Delete:
	...
	case *tree.Grant:
		for i, db := range stmt.Targets.Databases {
			if err := checkCatalogName(db, tree.Name(handler.dumpDatabaseName)); err != nil {
				return err
			}
			stmt.Targets.Databases[i] = TempImportPGDumpDBName
		}
		for _, schema := range stmt.Targets.Schemas {
			if schema.ExplicitCatalog {
				...
				schema.CatalogName = TempImportPGDumpDBName
			}
		}
		for _, table := range stmt.Targets.Tables.TablePatterns {
			explicitCatalog, ok := table.GetExplicitCatalog()
			if !ok {
				return errors.Newf("cannot determine if table %q has an explicit catalog", table)
			}
			if explicitCatalog != "" {
				if err := checkCatalogName(explicitCatalog, tree.Name(handler.dumpDatabaseName)); err != nil {
					return err
				}
				if ok := table.SetExplicitCatalog(TempImportPGDumpDBName); !ok {
					return errors.Newf("cannot format the catalog name of table %q to %q", table, TempImportPGDumpDBName)
				}
			}
		}
		handler.BufferedDDLStmts = append(handler.BufferedDDLStmts, formatStatement(stmt))
...
func ParseDDLStatementsFromDumpFile(
	...
	for {
		...
		if err := bufferDDLPostgresStatement(ctx, evalCtx, stmt.(tree.Statement), p, parentID, &handler); err != nil {
			return handler, err
		}
	}
	return handler, nil
...
func runDDLStmtsFromDumpFile(
	...
	for _, stmt := range stmts {
		_, err := ie.ExecEx(ctx, "import-pgdump-ddl", txn,
			sessiondata.InternalExecutorOverride{User: username.NodeUserName(), Database: TempImportPGDumpDBName}, stmt)
		if err != nil {
...
// MoveObjectsInTempDatabaseToState is to move the db and the objects in it to
// a certain state.
func MoveObjectsInTempDatabaseToState(
	ctx context.Context, state descpb.DescriptorState, txn *kv.Txn, descsCol *descs.Collection,
) (*dbdesc.Mutable, []*tabledesc.Mutable, []*schemadesc.Mutable, error) {
	importedTables := make([]*tabledesc.Mutable, 0)
	importedSchemas := make([]*schemadesc.Mutable, 0)

	dbDescs, err := descsCol.GetMutableDatabaseByName(
		ctx,
		txn,
		TempImportPGDumpDBName,
	...
	tableDescs, err := descsCol.GetAllTableDescriptorsInDatabase(ctx, txn, dbDescs)
	if err != nil {
		return nil, nil, nil, err
	}
	...
	schemaDescs, err := descsCol.GetAllSchemaDescriptorsInDatabase(ctx, txn, dbDescs)
	if err != nil {
		return nil, nil, nil, err
	}
	...
	dbDescs.State = state
	if err := descsCol.WriteDesc(ctx, false /* kvTrace */, dbDescs, txn); err != nil {
		return nil, nil, nil, err
	}

	return dbDescs, importedTables, importedSchemas, err
}
...
func ProcessDDLStatements(
	...
	dbDescs *dbdesc.Mutable,
	tableDescs []*tabledesc.Mutable,
	...
	if err := createTempImportDatabase(ctx, p); err != nil {
		return nil, nil, nil, errors.Wrap(err, "creating temporary import database")
	}
	...
	if err != nil {
		return nil, nil, nil, errors.Wrap(err, "parsing DDL statements from dump file")
	}
	...
	if err := p.ExecCfg().InternalExecutorFactory.DescsTxnWithExecutor(ctx, p.ExecCfg().DB, nil /* sessionData */, func(
		...
		// OFFLINE state.
		dbDescs, tableDescs, schemaDescs, err = MoveObjectsInTempDatabaseToState(ctx, descpb.DescriptorState_OFFLINE, txn, descsCol)
		if err != nil {
	...
	return dbDescs, tableDescs, schemaDescs, nil
}
