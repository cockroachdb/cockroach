...
// mapDateTime maps the time.Time object returned by goAvro to the proper CRL datum type.
func mapDateTime(x interface{}, targetT *types.T) (tree.Datum, error) {
	t := x.(time.Time)
...
func nativeToDatum(
	...
	case float64:
		d = tree.NewDFloat(tree.DFloat(v))
	case time.Time:
		return mapDateTime(x, targetT)
	case time.Duration:
		// goAvro returns avro cols of logical type time as time.duration
		dU := x.(time.Duration) / time.Microsecond
