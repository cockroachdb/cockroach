...
func newParquetRowProducer(
	...
	//
	// detectListColumn recognizes the 3-level structure and records the
	// group node name in listInfo.columnName; parquetColumnName() then
	// returns that name for LIST leaves and falls back to the leaf name for
	// flat columns. That mapping has to exist before any code compares leaf
	// names against user-supplied column names — which is why this loop
	// runs ahead of determineColumnsToRead and validateAndBuildColumnMapping.
	...
	for _, colIdx := range p.columnsToRead {
		...
		if err != nil {
			return errors.Wrapf(err, "reading batch from column %q (index %d)",
				parquetColumnName(parquetSchema, colIdx, p.columnMetadata), colIdx)
		}
...
func determineColumnsToRead(
	...
	for parquetColIdx := range parquetSchema.NumColumns() {
		...
		if maxDefLevel > 1 && !columnMetadata[parquetColIdx].isList {
