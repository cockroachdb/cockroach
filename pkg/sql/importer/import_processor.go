...
func newReadImportDataProcessor(
	...
	if UseDistributedMergeForImport.Get(&flowCtx.Cfg.Settings.SV) {
...
var UseDistributedMergeForImport = settings.RegisterBoolSetting(settings.ApplicationLevel,
	"bulkio.import.distributed_merge.mode",
...
func ingestKvs(
	...
	if spec.UseDistributedMerge {
		rowURI := fmt.Sprintf("nodelocal://%d/import/%d/%s/", flowCtx.Cfg.NodeID.SQLInstanceID(),
		...
		indexURI := fmt.Sprintf("nodelocal://%d/import/%d/%s/", flowCtx.Cfg.NodeID.SQLInstanceID(), spec.JobID,
			indexAdderName)
