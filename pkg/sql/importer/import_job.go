...
func (r *importResumer) Resume(ctx context.Context, execCtx interface{}) error {
	...
	case ImportRowCountValidationAsync, ImportRowCountValidationSync:
		...
		if err := p.ExecCfg().InternalDB.DescsTxn(ctx, func(
			...
			if creationVersion := r.job.Payload().CreationClusterVersion; !details.Table.WasEmpty && creationVersion.Less(clusterversion.V26_2_AddInspectRowCountCheck.Version()) {
...
// return an updated details with the initial row count.
