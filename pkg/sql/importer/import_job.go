...
func (r *importResumer) OnFailOrCancel(
	...
	switch {
	case !details.PrepareComplete || details.Walltime == 0:
	...
	case tbl.WasEmpty:
		...
		if err != nil {
			return errors.Wrap(err, "truncating empty table to roll back revert")
		}
	default:
...
func (r *importResumer) markOnline(
	...
	return sql.DescsTxn(ctx, cfg, func(ctx context.Context, txn isql.Txn, collection *descs.Collection) error {
		// Bring the IMPORT INTO table back online
		b := txn.KV().NewBatch()
		intoDesc, err := collection.MutableByID(txn.KV()).Table(ctx, id)
		if err != nil {
			return err
		}
		intoDesc.SetPublic()
		...
		const kvTrace = false
		if err := collection.WriteDescToBatch(ctx, kvTrace, intoDesc, b); err != nil {
			return err
		}
