...
func TestProcessorEncountersUncertaintyError(t *testing.T) {
	...
	// We are going to EXPORT a table with 10 rows, with rows 1..5 located on node 0 and
	// nodes 6..10 on node 1.
	var (
	...
	t.Run("after result rows are emitted returns error", func(t *testing.T) {
		...
		// Node 1's read is unblocked and returns a RWUI error which eventually makes it to the gateway.
		// Node 0's file write is unblocked, at which point it should see a draining
		// output and the RWUI error should be returned to the user.
	...
	t.Run("before result rows are emitted retries", func(t *testing.T) {
		...
		// Node 0 is blocked writing the first file to external storage until the RWUI makes it to the gateway.
		// Node 0's file write is unblocked, at which point it should see a draining
		// output and the RWUI error should be returned and then retried.
		// On retry, no RWUI is returned, so the export succeeds.
		...
		// Reads are trapped but not blocked, so node 1 should immediately return an RWUI. Node 0 will
		// be blocked waiting on allowOneWrite.
		atomic.StoreInt64(&trapRead, 1)
		unblockRead()
