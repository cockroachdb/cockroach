	...
	if len(virtComputedCols) != 0 {
		...
		for i, col := range requestedCols {
			if col.IsVirtual() {
				...
				if noDistSQLCauses := checkExprForDistSQL(virtComputedExprs[virtIdx], &distSQLVisitor); noDistSQLCauses != 0 {
					return nil, errors.Newf("cannot be executed with distsql: %s", noDistSQLCauses)
				}
