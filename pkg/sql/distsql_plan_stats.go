...
func (dsp *DistSQLPlanner) createPartialStatsPlan(
	...
	for _, t := range tableStats {
		...
		if len(t.ColumnIDs) == 1 && column.GetID() == t.ColumnIDs[0] && t.PartialPredicate == "" &&
			...
			if colinfo.ColumnTypeIsOnlyInvertedIndexable(column.GetType()) || colinfo.ColumnTypeIsInvertedIndexable(column.GetType()) && t.HistogramData.ColumnType.Family() == types.BytesFamily {
