...
func (dsp *DistSQLPlanner) createPartialStatsPlan(
	...
	for _, t := range tableStats {
		...
		if len(t.ColumnIDs) == 1 && column.GetID() == t.ColumnIDs[0] && t.PartialPredicate == "" &&
			...
			if colinfo.ColumnTypeIsOnlyInvertedIndexable(column.GetType()) {
				return nil, pgerror.Newf(pgcode.ObjectNotInPrerequisiteState, "the latest full statistic histogram for column %s is an inverted index", column.GetName())
			}
			if colinfo.ColumnTypeIsInvertedIndexable(column.GetType()) && t.HistogramData.ColumnType.Family() == types.BytesFamily {
				return nil, pgerror.Newf(pgcode.ObjectNotInPrerequisiteState, "the latest full statistic histogram for column %s is a trigram index histogram", column.GetName())
			}
			stat = t
