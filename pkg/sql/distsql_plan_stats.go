...
func (dsp *DistSQLPlanner) createPartialStatsPlan(
	...
	for _, t := range tableStats {
		if len(t.ColumnIDs) == 1 && column.GetID() == t.ColumnIDs[0] && t.PartialPredicate == "" && !t.IsForecast {
	...
	var lowerBoundSpan constraint.Span
	lowerBoundSpan, err = constructLowerBoundSpan(planCtx, histogram)
	if err != nil {
		return nil, err
	}
...
func constructLowerBoundSpan(
	...
	if lowerBound.Compare(planCtx.EvalContext(), tree.DNull) == 0 {
		return constraint.Span{}, pgerror.Newf(pgcode.ObjectNotInPrerequisiteState, "only NULL values exist in the index, so partial stats cannot be collected")
	}

	var lbSpan constraint.Span
	lbSpan.Init(constraint.EmptyKey, constraint.IncludeBoundary, constraint.MakeKey(lowerBound), constraint.IncludeBoundary)
	return lbSpan, nil
}
