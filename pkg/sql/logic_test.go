...
var (
	...
	generateTestfiles = flag.Bool(
		"generate-testfiles", false,
...
func (t *logicTest) emit(line string) {
	if *generateTestfiles {
		t.generateTestBuf.WriteString(line)
		t.generateTestBuf.WriteString("\n")
	}
}
...
func (t *logicTest) processTestFile(path string, config testClusterConfig) error {
	...
	if *generateTestfiles && !t.t.Failed() {
	...
	if *generateTestfiles {
		...
		if query.checkResults {
			...
			if reflect.DeepEqual(query.expectedResults, results) {
				...
				// Calculate the length of each column so we can nicely align things.
				var colLengths []int
				for _, resultLine := range resultLines {
					for c, value := range resultLine {
						l := len(value)
						if len(colLengths) == c {
							colLengths = append(colLengths, l)
						} else if colLengths[c] < l {
							colLengths[c] = l
						}
					}
				}
				...
				for _, resultLine := range resultLines {
					...
					for c, value := range resultLine {
						...
						// Pad for alignment, unless there is nothing left to print out.
						align := false
						for i := c + 1; i < len(resultLine); i++ {
							if resultLine[i] != "" {
								align = true
								break
							}
						...
						if align {
							for i := len(value); i < colLengths[c]+2; i++ {
								line.WriteByte(' ')
							}
						}
					}
