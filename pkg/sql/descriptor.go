...
// TestingInjectFixedDescIDOffset makes descriptor ID generation for test-defined SQL objects
// agnostic of the number of objects created prior to the call.
func TestingInjectFixedDescIDOffset(ctx context.Context, exec *InternalExecutor) error {
	if exec.s.cfg.Codec.ForSystemTenant() &&
		!exec.s.cfg.Settings.Version.IsActive(ctx, clusterversion.V23_1DescIDSequenceForSystemTenant) {
		// The descriptor ID sequence in the system tenant did not exist at that version.
		return nil
	}

	// The name of the descriptor ID sequence.
	const descIDseqFQN = catconstants.SystemDatabaseName + "." + catconstants.PublicSchemaName + "." + catconstants.DescIDSequenceTableName

	_, err := exec.ExecEx(ctx, "fix-descID-offset", nil, /* txn */
		// Need node identity because the desc ID sequence is owned by 'node'.
		sessiondata.NodeUserSessionDataOverride,
		// We need to use WHERE last_value < $2 because this code can
		// execute on each node of a multi-node TestCluster, and we only
		// want to run it once.
		fmt.Sprintf(`SELECT setval($1, $2) FROM %s WHERE last_value < $2`, descIDseqFQN),
		descIDseqFQN,
		TestingFixedDescIDOffset)
	return err
}

// TestingFixedDescIDOffset is a fixed base offset for descriptor IDs
// generated by test servers.
//
// We need a value such that, for many IDs generated sequentially
// from it, we preserve the nice property that
// MakeTablePrefix(ID).PrefixEnd() == MakeTablePrefix(ID+1).
//
// This is true of all values between 1 and 108, but e.g not true
// of 109, 511, 1023.
//
// So for example, 1000 would be a poor offset choice because as
// soon as 23 SQL objects are defined, we'd get artifacts in test
// expectations (e.g. "/PrefixEnd" appears in key string
// representations).
//
// 10000 is better because the first value after 10000 that
// behaves non-regularly is 10239, so we get 239 IDs that behave
// peacefully.
const TestingFixedDescIDOffset = 10000
