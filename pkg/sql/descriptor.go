...
func (p *planner) createDatabase(
	...
	initializationConfig, err := p.maybeInitializeMultiRegionMetadata(
		ctx,
	...
	desc := dbdesc.NewInitial(
		...
		p.SessionData().User(),
		dbdesc.MaybeWithDatabaseRegionConfig(initializationConfig),
	)
	...
	// descriptor.
	if err := p.maybeInitializeMultiRegionDatabase(ctx, desc, initializationConfig); err != nil {
		return nil, true, err
	}
...
func (p *planner) maybeInitializeMultiRegionMetadata(
	...
	initializationConfig, err := InitializeMultiRegionMetadataCCL(
		ctx,
		p.EvalContext(),
		p.ExecCfg(),
		liveRegions,
	...
	if err := multiregion.ValidateInitializationRegionConfig(initializationConfig); err != nil {
		return nil, err
	}
