...
func (n *alterDatabasePrimaryRegionNode) switchPrimaryRegion(params runParams) error {
	...
	prevRegionConfig, err := SynthesizeRegionConfig(params.ctx, params.p.txn, &n.desc.Immutable, params.p.Descriptors())
	if err != nil {
		return err
	}
	found := false
	...
	updatedRegionConfig, err := SynthesizeRegionConfig(
		params.ctx, params.p.txn, &n.desc.Immutable, params.p.Descriptors(),
	)
	if err != nil {
		return err
	}
...
func (n *alterDatabasePrimaryRegionNode) setInitialPrimaryRegion(params runParams) error {
	...
	initializationConfig, err := params.p.maybeInitializeMultiRegionMetadata(
		params.ctx,
	...
	if err := n.desc.SetInitialMultiRegionConfig(initializationConfig); err != nil {
		return err
	}
	...
	if err := addDefaultLocalityConfigToAllTables(
		...
		params.p,
		&n.desc.Immutable,
		initializationConfig.RegionEnumID(),
	); err != nil {
		return err
	}
	...
	return params.p.maybeInitializeMultiRegionDatabase(params.ctx, n.desc, initializationConfig)
}
...
func (n *alterDatabaseSurvivalGoalNode) startExec(params runParams) error {
	...
	regionConfig, err := SynthesizeRegionConfig(params.ctx, params.p.txn, &n.desc.Immutable, params.p.Descriptors())
	if err != nil {
		return err
	}
	if err := regionConfig.EnsureSurvivalGoalIsSatisfiable(); err != nil {
		return err
	}
