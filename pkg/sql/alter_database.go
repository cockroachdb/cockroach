...
func (p *planner) AlterDatabaseDropRegion(
	...
	// Ensure survivability goal can still be satisfied after a region is removed
	// from the database.
	regionConfig, err := SynthesizeRegionConfig(ctx, p.txn, dbDesc.ID, p.Descriptors())
	if err != nil {
		return nil, err
	}
	if err := multiregion.CanSatisfySurvivalGoalAfterRegionDrop(regionConfig); err != nil {
		return nil, err
	}
