...
func (n *alterDatabaseAddSuperRegion) startExec(params runParams) error {
	...
	return params.p.prepareAddOrAlterSuperRegion(params.ctx, n.desc, typeDesc, n.n.Regions, n.n.SuperRegionName, tree.AsStringWithFQNames(n.n, params.Ann()))
}
...
func (n *alterDatabaseDropSuperRegion) startExec(params runParams) error {
	...
	found := removeSuperRegion(typeDesc.RegionConfig, n.n.SuperRegionName)
...
func (n *alterDatabaseAlterSuperRegion) startExec(params runParams) error {
	...
	for _, superRegion := range superRegions {
		if superRegion.SuperRegionName == n.n.SuperRegionName.String() {
			found = true
		}
	}
	...
	removeSuperRegion(typeDesc.RegionConfig, n.n.SuperRegionName)
...
func (p *planner) prepareAddOrAlterSuperRegion(
	ctx context.Context,
	desc *dbdesc.Mutable,
	...
	if err := multiregion.CanSatisfySurvivalGoal(desc.RegionConfig.SurvivalGoal, len(regionList)); err != nil {
		return errors.Wrapf(err, "super region %s only has %d regions", superRegionName, len(regionList))
	}
