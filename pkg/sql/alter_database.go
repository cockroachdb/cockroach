...
func (n *alterDatabasePrimaryRegionNode) switchPrimaryRegion(params runParams) error {
	...
	*params.extendedEvalCtx.validateDbZoneConfig = true
	...
	// zone configs with tighter replica constraints (2+2+1 instead of 1+1+1)
	// that must be updated when the primary region changes.
	hasExplicitRegionFailureZoneConfigs := len(updatedRegionConfig.Regions()) == 3 &&
		updatedRegionConfig.SurvivalGoal() == descpb.SurvivalGoal_REGION_FAILURE
	if hasExplicitRegionFailureZoneConfigs {
		params.p.BufferClientNotice(params.ctx, pgnotice.Newf("all REGIONAL BY TABLE zone configurations in the primary region have been updated to reflect the new primary region"))
	}
	...
	if isNewPrimaryRegionMemberOfASuperRegion || isOldPrimaryRegionMemberOfASuperRegion || hasExplicitRegionFailureZoneConfigs {
		opts = WithOnlyRegionalTablesAndGlobalTables
	}

	if updatedRegionConfig.IsPlacementRestricted() || isNewPrimaryRegionMemberOfASuperRegion || isOldPrimaryRegionMemberOfASuperRegion || hasExplicitRegionFailureZoneConfigs {
