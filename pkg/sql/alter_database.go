...
func (n *alterDatabaseDropRegionNode) startExec(params runParams) error {
	...
	if !found {
		return errors.WithHint(
			errors.Newf(
				"attempting to drop region %s which has not yet been added to database %s", n.n.Region,
			),
			"Only regions which have been previously added to a database can be dropped.")
	}
...
func addDefaultLocalityConfigToAllTables(
	...
	if err := forEachTableDesc(ctx, p, desc, hideVirtual,
		func(immutable *dbdesc.Immutable, _ string, desc catalog.TableDescriptor) error {
			...
			if err := p.setTableLocalityToRegionalByTable(
...
func (n *alterDatabasePrimaryRegionNode) setInitialPrimaryRegion(params runParams) error {
	...
	if err := addDefaultLocalityConfigToAllTables(params.ctx, params.p,
		&n.desc.Immutable, regionConfig.RegionEnumID); err != nil {
		return err
	}
