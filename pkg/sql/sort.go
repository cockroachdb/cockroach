		...
		case parser.OrderByIndex:
			...
			for k, colName := range idxDesc.ColumnNames {
				newOrderBy = append(newOrderBy, &parser.Order{
					...
					Direction: chooseDirection(o.Direction, idxDesc.ColumnDirections[k]),
				})
			}
...
// choseDirection translates the specified IndexDescriptor_Direction
// into a parser.Direction. If the wantedDir is DESCENDING, the idxDir
// is inverted.
func chooseDirection(
	wantedDir parser.Direction, idxDir sqlbase.IndexDescriptor_Direction,
) parser.Direction {
	if wantedDir == parser.Ascending || wantedDir == parser.DefaultDirection {
		if idxDir == sqlbase.IndexDescriptor_ASC {
