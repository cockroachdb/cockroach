...
func NewStructuredEventProcessor(
	execCfg *sql.ExecutorConfig, stopper *stop.Stopper,
) StructuredEventProcessor {
...
func (s *StructuredEventProcessor) RegisterEventType(
	...
	logstream.RegisterProcessor(ctx, s.stopper, meta, s)
}
...
func (s *StructuredEventProcessor) Process(ctx context.Context, event any) error {
	...
	if err := timeutil.RunWithTimeout(context.TODO(), "record-events", time.Second, func(ctx context.Context) error {
		return s.execCfg.InternalDB.Txn(ctx, func(ctx context.Context, txn isql.Txn) error {
			_, err := txn.ExecEx(ctx, "insert-event-log", txn.KV(), sessiondata.NodeUserSessionDataOverride, query, args...)
			return err
		})
	}); err != nil {
