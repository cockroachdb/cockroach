...
func TestPrepareDeclare(t *testing.T) {
	...
	stmt, err := db.Prepare("DECLARE foo CURSOR FOR VALUES (1), (2)")
	...
	stmt, err = db.Prepare("FETCH 2 foo")
	require.NoError(t, err)
	r, err := stmt.Query()
	require.NoError(t, err)
	var actual int
	...
	// When this limitation is lifted, the rest of this test shoudl be uncommented.
	_, err = db.Exec("DECLARE foo CURSOR FOR SELECT 1 WHERE $1", true)
	require.Contains(t, err.Error(), "could not determine data type of placeholder")

	//stmt, err = db.Prepare("FETCH 1 foo")
	//require.NoError(t, err)
	//r, err = stmt.Query()
	//require.NoError(t, err)
	//require.NoError(t, r.Scan(&actual))
