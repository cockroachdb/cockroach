...
func initColsForScan(
	...
	for _, wc := range colCfg.wantedColumns {
		...
		var err error
		if sqlbase.IsColIDSystemColumn(desc.TableDesc(), sqlbase.ColumnID(wc)) {
			// If some system columns are requested from the table, then retrieve
			// the descriptors here.
			c, err = sqlbase.GetSystemColumnDescriptorFromID(desc.TableDesc(), sqlbase.ColumnID(wc))
			if err != nil {
				return nil, err
			}
		} else {
