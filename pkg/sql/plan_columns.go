...
func getPlanColumns(plan planNode, mut bool) sqlbase.ResultColumns {
	...
	case *explainDebugNode:
		return n.get(mut, debugColumns)
	case *explainDistSQLNode:
		return n.get(mut, explainDistSQLColumns)
	case *relocateNode:
		return n.get(mut, relocateNodeColumns)
	case *scatterNode:
		return n.get(mut, scatterNodeColumns)
	case *showRangesNode:
		return n.get(mut, showRangesColumns)
	case *showFingerprintsNode:
		return n.get(mut, showFingerprintsColumns)
	case *splitNode:
		return n.get(mut, splitNodeColumns)
...
func (c *optColumnsSlot) get(mut bool, cols sqlbase.ResultColumns) sqlbase.ResultColumns {
	if c.columns != nil {
		return c.columns
	}
	if !mut {
