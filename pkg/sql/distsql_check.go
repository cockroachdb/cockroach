...
// distSQLProhibitedCause is an enum of all reasons for why a query couldn't be
// distributed.
//
// It is separate from distSQLProhibitedCauses so that we can generate the
// stringer for each cause.
type distSQLProhibitedCause uint64

//go:generate stringer -type=distSQLProhibitedCause

const (
	funcDistSQLBlocklist distSQLProhibitedCause = 1 << iota
	routineProhibited
	oidProhibited
	castToOidProhibited
	...
	numDistSQLProhibitedCauses
)

// distSQLProhibitedCauses is a bit-map describing all reasons for why a query
// couldn't be distributed.
type distSQLProhibitedCauses uint64

func (c *distSQLProhibitedCauses) addSingle(cause distSQLProhibitedCause) {
	*c |= distSQLProhibitedCauses(cause)
}

func (c *distSQLProhibitedCauses) addMultiple(causes distSQLProhibitedCauses) {
	*c |= causes
}

func (c distSQLProhibitedCauses) String() string {
	if c == 0 {
		return ""
	}
	var sb strings.Builder
	var comma string
	for cause := distSQLProhibitedCause(1); cause < numDistSQLProhibitedCauses; cause <<= 1 {
		if c&distSQLProhibitedCauses(cause) != 0 {
...
//
// Note that in order to find all problematic elements, this visitor does _not_
// short-circuit and always fully walks the tree.
type distSQLExprCheckVisitor struct {
	causes distSQLProhibitedCauses
}
...
func (v *distSQLExprCheckVisitor) VisitPre(expr tree.Expr) (recurse bool, newExpr tree.Expr) {
	...
	case *tree.FuncExpr:
		if t.IsDistSQLBlocklist() {
			v.causes.addSingle(funcDistSQLBlocklist)
		}
	case *tree.RoutineExpr:
		v.causes.addSingle(routineProhibited)
	case *tree.DOid:
		v.causes.addSingle(oidProhibited)
	case *tree.Subquery:
	...
	case *tree.CastExpr:
		...
		if typ, ok := tree.GetStaticallyKnownType(t.Type); ok {
			...
			case types.OidFamily:
				v.causes.addSingle(castToOidProhibited)
			}
		}
	case *tree.DArray:
...
func checkExprForDistSQL(
	expr tree.Expr, distSQLVisitor *distSQLExprCheckVisitor,
) distSQLProhibitedCauses {
	if expr == nil {
		return 0
	}
	distSQLVisitor.causes = 0
	tree.WalkExprConst(distSQLVisitor, expr)
	return distSQLVisitor.causes
}
...
// cannotDistribute and distSQLProhibitedCauses if the plan subtree is not
// distributable. The causes don't indicate complete failure - it's instead the
// reason that this plan couldn't be distributed.
//
// Note that in order to find all causes that prohibit DistSQL, this method does
// _not_ short-circuit and always fully walks the tree.
func checkSupportForPlanNode(
...
) (retRec distRecommendation, retCauses distSQLProhibitedCauses) {
	defer func() {
		// In order to simplify the method, the recommendation returned might be
		// "incomplete" - without fully accounting for no-DistSQL causes, so we
		// reconcile that in the defer unconditionally if at least one cause is
		// present.
		if retCauses != 0 {
	...
	case *createStatsNode:
		if n.runAsJob {
			return cannotDistribute, distSQLProhibitedCauses(unsupportedPlanNode)
		}
	...
	case *filterNode:
		rec, causes := checkSupportForPlanNode(ctx, n.input, distSQLVisitor, sd, txnHasBufferedWrites)
		causes.addMultiple(checkExprForDistSQL(n.filter, distSQLVisitor))
		return rec, causes
	...
	case *groupNode:
		...
		for _, agg := range n.funcs {
			if agg.distsqlBlocklist {
				causes.addSingle(aggDistSQLBlocklist)
			}
		}
		...
		return rec.compose(aggRec), causes
	...
	case *indexJoinNode:
		...
		if n.fetch.lockingStrength != descpb.ScanLockingStrength_FOR_NONE {
			...
			causes.addSingle(rowLevelLockingProhibited)
		}
		if txnHasBufferedWrites && n.fetch.requiresMVCCDecoding() {
			// TODO(#144166): relax this.
			causes.addSingle(systemColumnsAndBufferedWritesProhibited)
		}
		return rec, causes

	case *invertedFilterNode:
		rec, causes := checkSupportForPlanNode(ctx, n.input, distSQLVisitor, sd, txnHasBufferedWrites)
		causes.addMultiple(checkExprForDistSQL(n.preFiltererExpr, distSQLVisitor))
		...
		if n.expression.Left != nil || n.expression.Right != nil {
			causes.addSingle(invertedFilterProhibited)
		}
		...
		return rec.compose(shouldDistribute), causes
	...
	case *invertedJoinNode:
		...
		causes.addMultiple(checkExprForDistSQL(n.onExpr, distSQLVisitor))
	...
	case *joinNode:
		rec, causes := checkSupportForPlanNode(ctx, n.left, distSQLVisitor, sd, txnHasBufferedWrites)
		causes.addMultiple(checkExprForDistSQL(n.pred.onCond, distSQLVisitor))
		recRight, causesRight := checkSupportForPlanNode(ctx, n.right, distSQLVisitor, sd, txnHasBufferedWrites)
		causes.addMultiple(causesRight)
	...
	case *lookupJoinNode:
		...
		if n.remoteLookupExpr != nil || n.remoteOnlyLookups {
			...
			causes.addSingle(localityOptimizedOpProhibited)
		}
		if n.fetch.lockingStrength != descpb.ScanLockingStrength_FOR_NONE {
		...
		for _, expr := range []tree.TypedExpr{
			...
			causes.addMultiple(checkExprForDistSQL(expr, distSQLVisitor))
		}
		return rec.compose(canDistribute), causes

	case *ordinalityNode:
		_, causes := checkSupportForPlanNode(ctx, n.input, distSQLVisitor, sd, txnHasBufferedWrites)
		// WITH ORDINALITY never gets distributed so that the gateway node can
		// always number each row in order.
		causes.addSingle(ordinalityProhibited)
		return cannotDistribute, causes

	case *projectSetNode:
		rec, causes := checkSupportForPlanNode(ctx, n.input, distSQLVisitor, sd, txnHasBufferedWrites)
		for i := range n.exprs {
			causes.addMultiple(checkExprForDistSQL(n.exprs[i], distSQLVisitor))
		}
		return rec, causes
	...
	case *renderNode:
		...
		for _, e := range n.render {
			causes.addMultiple(checkExprForDistSQL(e, distSQLVisitor))
		}
		return rec, causes

	case *scanNode:
		rec, causes := canDistribute, distSQLProhibitedCauses(0)
	...
	case *sortNode:
		...
		return rec.compose(sortRec), causes
	...
	case *topKNode:
		...
		return rec.compose(topKRec), causes
	...
	case *unionNode:
		...
		recRight, causesRight := checkSupportForPlanNode(ctx, n.right, distSQLVisitor, sd, txnHasBufferedWrites)
		causes.addMultiple(causesRight)
		return rec.compose(recRight), causes
	...
	case *valuesNode:
		...
		if !n.specifiedInQuery {
			...
			causes.addSingle(valuesNodeProhibited)
		}
	...
	case *vectorMutationSearchNode:
		...
		_, causes := checkSupportForPlanNode(ctx, n.input, distSQLVisitor, sd, txnHasBufferedWrites)
		causes.addSingle(vectorSearchProhibited)
		return cannotDistribute, causes
	...
	case *vectorSearchNode:
		...
		// TODO(yuzefovich): if we start distributing plans with these nodes,
		// we'll need to ensure to collect LeafTxnFinalInfo metadata.
		return cannotDistribute, distSQLProhibitedCauses(vectorSearchProhibited)
	...
	case *windowNode:
		...
		return rec.compose(windowRec), causes
	...
	case *zigzagJoinNode:
		...
		rec, causes := shouldDistribute, distSQLProhibitedCauses(0)
		...
		causes.addMultiple(checkExprForDistSQL(n.onCond, distSQLVisitor))
		return rec, causes
	...
	default:
		...
		case 1:
			...
			_, causes := checkSupportForPlanNode(ctx, input, distSQLVisitor, sd, txnHasBufferedWrites)
			causes.addSingle(unsupportedPlanNode)
			return cannotDistribute, causes
		default:
