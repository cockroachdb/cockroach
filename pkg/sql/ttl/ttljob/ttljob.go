...
func (t *rowLevelTTLResumer) Resume(ctx context.Context, execCtx interface{}) (retErr error) {
	...
	replanChecker, cancelReplanner := sql.PhysicalPlanChangeChecker(
		...
		replanDecider(&t.consecutiveReplanDecisions, func() int64 { return replanStabilityWindow.Get(&execCfg.Settings.SV) }, func() float64 { return replanThreshold.Get(&execCfg.Settings.SV) }),
...
func replanDecider(
	...
	return func(ctx context.Context, oldPlan, newPlan *sql.PhysicalPlan) bool {
		...
		// If stability window is 1, replan immediately (current behavior)
		// Otherwise, require consecutive decisions to meet the window threshold
