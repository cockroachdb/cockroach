...
//
// In mixed-version clusters, TTL processors fall back to direct job table updates
// if any node in the cluster does not support coordinator-based progress reporting.
// In that case, no processors will emit progress metadata, so this callback will
// never be invoked.
