...
func (t rowLevelTTLResumer) Resume(ctx context.Context, execCtx interface{}) error {
	...
	if err := func() error {
		...
		for _, r := range ranges {
			...
			r.startPK, err = keyToDatums(rangeDesc.StartKey, p.ExecCfg().Codec, pkTypes, pkDirs, &alloc)
			if err != nil {
				return err
			}
			r.endPK, err = keyToDatums(rangeDesc.EndKey, p.ExecCfg().Codec, pkTypes, pkDirs, &alloc)
			if err != nil {
				return err
			}
			ch <- r
...
func getRangeConcurrency(sv *settings.Values, ttl descpb.TableDescriptor_RowLevelTTL) int {
