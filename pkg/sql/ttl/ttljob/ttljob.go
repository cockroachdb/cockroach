...
func (t rowLevelTTLResumer) Resume(ctx context.Context, execCtx interface{}) error {
	...
	for _, spanPartition := range spanPartitions {
		ttlSpec := &execinfrapb.TTLSpec{
			...
			LabelMetrics:                   rowLevelTTL.LabelMetrics,
			ChangeTableVersionDuringDelete: knobs.ChangeTableDescriptorVersionDuringDelete,
			PreSelectDeleteStatement:       knobs.PreSelectDeleteStatement,
		}
	...
	metadataCallbackWriter := sql.NewMetadataCallbackWriter(rowResultWriter, nil)
