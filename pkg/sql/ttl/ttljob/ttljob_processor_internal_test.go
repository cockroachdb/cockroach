...
func (m *mockTTLWorker) startAsyncWork(
	...
	return func() error { return nil }, m.workersDoneCh, func(error) {}, m.setupError
}
...
func (m *metadataCache) Push(
	...
	if meta != nil {
		m.mu.Lock()
		defer m.mu.Unlock()
		m.mu.bufferedMeta = append(m.mu.bufferedMeta, *meta)
	}
...
func (m *metadataCache) getLastError() error {
	m.mu.Lock()
	defer m.mu.Unlock()
	if len(m.mu.bufferedMeta) == 0 {
		return nil
	}
	return m.mu.bufferedMeta[len(m.mu.bufferedMeta)-1].Err
}

func (m *metadataCache) getMetadataCacheSize() int {
	m.mu.Lock()
	defer m.mu.Unlock()
	return len(m.mu.bufferedMeta)
}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			testutils.SucceedsSoon(t, func() error {
				if mock.output.getMetadataCacheSize() == 0 {
					return errors.New("no metadata emitted")
				}
				return nil
			})
			if tc.workerSetupError != nil {
				require.Equal(t, 1, mock.output.getMetadataCacheSize())
