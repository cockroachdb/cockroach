...
type ttlProcessorWorker interface {
	startAsyncWork(ctx context.Context) (completeCallback func() error, workersDoneCh chan error, cancel context.CancelFunc, retErr error)
	...
	for {
		...
		case <-tick.C:
			...
			if status := output.Push(nil, &execinfrapb.ProducerMetadata{}); status != execinfra.NeedMoreRows {
				cancelFunc() // Stop the async work
			}
...
func (t *ttlProcessor) startAsyncWork(
	...
	defer func() {
		if retErr != nil {
			cancel() // cancel all work on early error
		}
	}()
	...
	group.GoCtx(func(ctx context.Context) error {
		...
		for i, span := range ttlSpec.Spans {
			select {
			case <-ctx.Done():
				log.VInfof(ctx, 2 /* level */, "stopping TTL processor bounds producer: %s", ctx.Err())
				return errors.Wrapf(ctx.Err(), "stopping TTL processor bounds producer")
			default:
				// continue work
			}
...
func (t *ttlProcessor) runTTLOnQueryBounds(
	...
	for {
		...
		select {
		case <-ctx.Done():
			log.VInfof(ctx, 2 /* level */, "stopping TTL processor on query bounds: %s", ctx.Err())
			return spanRowCount, errors.Wrapf(ctx.Err(), "stopping TTL processor on query bounds")
		default:
			// continue work
		}
