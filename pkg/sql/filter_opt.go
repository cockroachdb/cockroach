...
func (p *planner) addRenderFilter(
	...
	if !isFilterTrue(extraFilter) {
		...
		//
		//   SELECT * FROM (SELECT a + 2 * b AS c FROM t) WHERE c > 123
		// by
...
func (p *planner) addJoinFilter(
	...
	if mergedBoundary > 0 && !isFilterTrue(extraFilter) {
		...
		noMergedVars := func(expr parser.VariableExpr) (bool, parser.Expr) {
			if iv, ok := expr.(*parser.IndexedVar); ok && iv.Idx >= mergedBoundary {
				return true, expr
			}
			return false, expr
		}
		...
		leftFilter := exprConvertVars(perhapsUsingMerged,
			func(expr parser.VariableExpr) (bool, parser.Expr) {
				if iv, ok := expr.(*parser.IndexedVar); ok && iv.Idx < mergedBoundary {
					newIv := n.pred.iVarHelper.IndexedVar(mergedBoundary + n.pred.leftEqualityIndices[iv.Idx])
					return true, newIv
				}
				return true, expr
			})
		rightFilter := exprConvertVars(perhapsUsingMerged,
			func(expr parser.VariableExpr) (bool, parser.Expr) {
				if iv, ok := expr.(*parser.IndexedVar); ok && iv.Idx < mergedBoundary {
					newIv := n.pred.iVarHelper.IndexedVar(sourceBoundary + n.pred.rightEqualityIndices[iv.Idx])
					return true, newIv
				}
				return true, expr
			})
	...
	propagate := func(pred parser.TypedExpr, offset int, side *planDataSource) error {
...
func shiftConj(
	ivarHelper parser.IndexedVarHelper, filter parser.TypedExpr, offset int,
) parser.TypedExpr {
	if isFilterTrue(filter) {
		return nil
	}
	...
	return exprConvertVars(filter, shiftFunc)
}
