...
// ByDisplayName is a map of display bame -> kind value. It is populated by init.
var ByDisplayName map[KindDisplayName]Kind

// ByInternalKey is a map of internal key -> kind value. It is populated by init.
...
func init() {
	...
	for kind := ALL; kind <= largestKind; kind++ {
		...
		// It should also be possible to look up a privilege
		// using its internal key.
