...
// either in ByInternalKey or in ByDisplayName map (or both), depending on the
// origin. If an entry is not found, it is either ignored or an error is raised
// (also depending on the origin).
func ListFromStrings(strs []string, origin Origin) (List, error) {
	...
	for i, s := range strs {
		...
		case OriginFromUserInput:
			...
			if !ok {
				// Perhaps the privilege has been renamed, and the user still
				// references the original name - we allow that for
