...
// ListFromStrings takes a list of internal storage keys and attempts
// to build a list of Kind. We convert each string to uppercase and
// search for it in the ByName map. If an entry is not found in
// ByName, it is either ignored or reports an error depending on the
// purpose.
...
func getByKey(key string) (Kind, bool) {
	k, ok := ByInternalKey[KindInternalKey(key)]
	return k, ok
}

func getByName(name string) (Kind, bool) {
	k, ok := ByDisplayName[KindDisplayName(name)]
	return k, ok
}
