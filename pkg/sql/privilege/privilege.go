...
func (pl List) ListToACL(grantOptions List, objectType ObjectType) string {
	...
	for _, privilege := range privileges {
		...
		//	chars = append(chars, "c")
		//}
		if aclChar, ok := privToACL[privilege]; ok {
			chars = append(chars, aclChar)
			if grantOptions.Contains(privilege) {
				charToGrantOption[aclChar] = true
			}
		}
	}

	sort.Strings(chars)
	starredChars := make([]string, len(privileges)+len(grantOptions))
	for _, aclChar := range chars {
		starredChars = append(starredChars, aclChar)
		if _, ok := charToGrantOption[aclChar]; ok {
			starredChars = append(starredChars, "*")
		}
	}
	return strings.Join(starredChars, "")
}
