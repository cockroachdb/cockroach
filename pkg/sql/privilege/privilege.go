...
var (
	AllPrivileges           = List{ALL, CREATE, CREATE, DROP, GRANT, SELECT, INSERT, DELETE, UPDATE, USAGE, ZONECONFIG}
...
func ValidatePrivileges(privileges List, objectType ObjectType) error {
	...
	switch objectType {
	case Database, Table:
		validPrivs = DBSchemaTablePrivileges
	case Type:
		validPrivs = TypePrivileges
	default:
		panic(fmt.Sprintf("invalid object type to grant on %s", objectType))
	}
