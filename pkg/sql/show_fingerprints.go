...
func (n *showFingerprintsNode) Next(params runParams) (bool, error) {
	...
	// Skip partial and inverted indexes. Experimental fingerprint uses a
	// query that forces the use of an index and that is incompatible with
	// partial and inverted indexes.
	if index.IsPartial() || index.GetType() == idxtype.INVERTED {
		n.run.rowIdx++
