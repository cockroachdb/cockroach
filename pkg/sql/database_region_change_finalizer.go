...
func (r *databaseRegionChangeFinalizer) finalize(ctx context.Context) error {
	if err := r.updateDatabaseZoneConfig(ctx); err != nil {
		return err
	}
	return r.repartitionRegionalByRowTables(ctx)
}
...
// region values.
func (r *databaseRegionChangeFinalizer) updateDatabaseZoneConfig(ctx context.Context) error {
	regionConfig, err := SynthesizeRegionConfig(ctx, r.txn, r.dbID, r.localPlanner.Descriptors())
	if err != nil {
		return err
	}
...
func (r *databaseRegionChangeFinalizer) repartitionRegionalByRowTables(ctx context.Context) error {
	...
	for _, tableDesc := range r.regionalByRowTables {
		// Since we hydrated the columns with the old enum, and now that the enum moved
		// all transitioned, we have to re-hydrate all the enums.
	...
	r.waitForLeaseIDs = repartitionedTableIDs
...
func (r *databaseRegionChangeFinalizer) waitToUpdateLeases(
	...
	for _, tbID := range r.waitForLeaseIDs {
