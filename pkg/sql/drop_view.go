...
func (p *planner) DropView(ctx context.Context, n *tree.DropView) (planNode, error) {
	...
	for _, toDel := range td {
		...
		for _, ref := range droppedDesc.DependedOnBy {
			...
			if err := p.canRemoveDependent(ctx, droppedDesc, ref, n.DropBehavior); err != nil {
				return nil, err
			}
		}
	}
...
func (p *planner) canRemoveDependentGeneric(
	ctx context.Context,
	typeName string,
	objName string,
	parentID descpb.ID,
...
func (p *planner) canRemoveDependent(
	...
	return p.canRemoveDependentGeneric(ctx, string(from.DescriptorType()), from.Name, from.ParentID, ref, behavior)
}
...
func (p *planner) canRemoveDependentViewGeneric(
	...
	for _, ref := range viewDesc.DependedOnBy {
		if err := p.canRemoveDependent(ctx, viewDesc, ref, behavior); err != nil {
			return err
		}
	}
	return nil
