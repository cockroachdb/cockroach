...
func (p *planner) canRemoveDependentView(
	...
	return p.canRemoveDependentViewGeneric(ctx, string(from.TypeName()), from.Name, from.ParentID, ref, behavior)
}
	...
	if behavior == tree.DropCascade {
		...
		for _, ref := range dependedOnBy {
			dependentDesc, err := p.getViewDescForCascade(
				ctx, string(viewDesc.TypeName()), viewDesc.Name, viewDesc.ParentID, ref.ID, behavior,
			)
			if err != nil {
				return cascadeDroppedViews, err
			}
