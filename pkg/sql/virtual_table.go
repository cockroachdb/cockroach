...
type rowPusher interface {
	// pushRow pushes the input row to the receiver of the generator. It does
	// mutate the input row. It will block until the the data has been received
	// and more data has been requested.
...
// The worker may assume that rows passes into addRow will be copied before
// being mutated, so it may reuse the underlying space across calls to addRow.
func setupGenerator(
