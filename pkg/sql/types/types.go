...
func (m *GeoMetadata) SQLString() string {
	...
	if m.SRID != 0 {
		figureName := strings.ToLower(m.Figure.String())
		if m.Figure == geopb.Figure_Unset {
			figureName = "geometry"
		}
		return fmt.Sprintf("(%s,%d)", figureName, m.SRID)
	} else if m.Figure != geopb.Figure_Unset {
		return fmt.Sprintf("(%s)", m.Figure)
	}
	return ""
}
