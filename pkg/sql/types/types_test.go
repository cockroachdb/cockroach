...
func TestTypes(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			Geography,
			MakeScalar(GeographyFamily, oid_extension.T_geography, 0, 0, emptyLocale),
		},
		{
			&T{
				InternalType: InternalType{
					...
					GeoMetadata: &GeoMetadata{
						SRID:   4325,
						Figure: geopb.Figure_MultiPoint,
					},
				},
			},
			MakeGeography(geopb.Figure_MultiPoint, 4325),
		},
		...
		{
			Geometry,
			MakeScalar(GeometryFamily, oid_extension.T_geometry, 0, 0, emptyLocale),
		},
		{
			&T{
				InternalType: InternalType{
			...
			MakeGeometry(geopb.Figure_MultiPoint, 4325),
		},
