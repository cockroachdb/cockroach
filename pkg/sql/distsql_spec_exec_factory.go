...
func (e *distSQLSpecExecFactory) ConstructSort(
	input exec.Node, ordering exec.OutputOrdering, alreadyOrderedPrefix int, estimatedInputRowCount uint64,
) (exec.Node, error) {
	physPlan, plan := getPhysPlan(input)
...
func (e *distSQLSpecExecFactory) ConstructTopK(
	input exec.Node, k int64, ordering exec.OutputOrdering, alreadyOrderedPrefix int, estimatedInputRowCount uint64,
) (exec.Node, error) {
	physPlan, plan := getPhysPlan(input)
	if k <= 0 {
