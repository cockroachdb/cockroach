...
// setReqOrdering is a helper method that takes care of updating MergeOrdering
// field of the physical plan. If there is already an ordering set on the plan
// and the new ordering is different from the present one, the method merges
// all streams on the gateway node.
func (e *distSQLSpecExecFactory) setReqOrdering(
	physPlan *PhysicalPlan, reqOrdering exec.OutputOrdering,
) {
	oldMergeOrdering := physPlan.MergeOrdering
	newMergeOrdering := e.dsp.convertOrdering(ReqOrdering(reqOrdering), physPlan.PlanToStreamColMap)
	if len(oldMergeOrdering.Columns) > 0 && !oldMergeOrdering.Equal(newMergeOrdering) {
		// Merge orderings are different, so we need to make sure that the old
		// ordering is satisfied which we can do by merging all streams on the
		// gateway. Ideally, this should only be necessary when adding a
		// top-level projection (i.e. the "final" projection).
		physPlan.EnsureSingleStreamOnGateway()
	}
	physPlan.SetMergeOrdering(newMergeOrdering)
}
...
func (e *distSQLSpecExecFactory) ConstructFilter(
	...
	e.setReqOrdering(physPlan, reqOrdering)
	return plan, nil
}
