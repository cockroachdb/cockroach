...
func createWindowPlanState(n *windowNode, evalCtx *tree.EvalContext, plan *physicalPlan) *windowPlanState {
...
func createWindowerSpec_Func(funcStr string) (distsqlrun.WindowerSpec_Func, error) {
...
func (s *windowPlanState) createWindowFnSpec(funcInProgress *windowFuncHolder) (distsqlrun.WindowerSpec_WindowFn, sqlbase.ColumnType, error) {
	...
	funcSpec, err := createWindowerSpec_Func(funcStr)
	if err != nil {
	...
	if s.n.run.windowFrames[funcInProgress.funcIdx] != nil {
		...
		frameSpec, err := distsqlplan.ConvertToSpec(s.n.run.windowFrames[funcInProgress.funcIdx], s.evalCtx)
		if err != nil {
			return distsqlrun.WindowerSpec_WindowFn{}, outputType, err
		}
...
func (s *windowPlanState) addRenderingIfNecessary() error {
	...
	s.plan.AddRendering(renderExprs, s.evalCtx, s.plan.planToStreamColMap, renderTypes)
