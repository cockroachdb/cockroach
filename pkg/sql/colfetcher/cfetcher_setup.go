...
func populateTableArgs(
	...
	if neededColumns.Len() != len(cols) {
		...
		for idx, ok := neededColumns.Next(0); ok; idx, ok = neededColumns.Next(idx + 1) {
			cols[neededColIdx] = cols[idx]
			idxMap[idx] = neededColIdx
			neededColIdx++
		}
		cols = cols[:neededColIdx]
...
func getNeededColumns(post *execinfrapb.PostProcessSpec, numColumns int) util.FastIntSet {
	...
	if !post.Projection && len(post.RenderExprs) == 0 {
		...
		for _, expr := range post.RenderExprs {
			...
			for _, needed := range ivars {
				neededColumns.Add(int(needed))
			}
...
// ivarSeen must be a bool slice of length end with all values set to false.
func findIVarsInRange(expr tree.Expr, start int, end int, ivarSeen []bool) []uint32 {
	var res []uint32
	if start >= end {
		return res
	}
	visitor := ivarExpressionVisitor{ivarSeen: ivarSeen}
	_, _ = tree.WalkExpr(visitor, expr)
	for i := start; i < end; i++ {
