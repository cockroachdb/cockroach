...
// accessible columns. post will be adjusted automatically.
//
...
func populateTableArgs(
	...
	if !index.Primary() {
		...
		if colIDs.Len() < len(cols)-numSystemCols {
			...
			cols = cols[:colIdx]
			if err := remapPostProcessSpec(post, idxMap, helper, evalCtx, args.typs); err != nil {
				return nil, nil, err
			}
		}
	}
...
func keepOnlyNeededColumns(
	...
	if !traceKV && len(neededColumns) < len(tableArgs.cols) {
		...
		if err := remapPostProcessSpec(post, idxMap, helper, evalCtx, tableArgs.typs); err != nil {
			return err
		}
...
// mapping of idxMap was applied. These will only be used post.RenderExprs is
// not nil.
func remapPostProcessSpec(
	...
	// TODO(yuzefovich): since we're modifying the spec directly, the flow
	// diagrams will now refer to the remapped ordinals. However, fixing this is
	// not easy - planCtx.saveFlows (where we generate the diagram) is called on
	// the shallow copy of the spec and after the flow has been setup (i.e.
	// after this method runs). We're destructively modifying post.OutputColumns
	// or post.RenderExprs without any simple way to restore the state. Even if
	// we store the original copy side-by-side within PostProcessSpec, the
	// shallow copy used by planCtx.saveFlows will still refer to the old
	// modified slice.
	if post.Projection {
		for i, colIdx := range post.OutputColumns {
		...
		for i := range post.RenderExprs {
			...
			post.RenderExprs[i].LocalExpr, err = helper.ProcessExpr(post.RenderExprs[i], evalCtx, typsBeforeRemapping)
			if err != nil {
				return err
			}
			post.RenderExprs[i].LocalExpr = physicalplan.RemapIVarsInTypedExpr(post.RenderExprs[i].LocalExpr, idxMap)
		}
	}
	return nil
}
