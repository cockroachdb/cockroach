...
func TestScanLimitKVPairsRead(t *testing.T) {
	...
	sqlDB.Exec(t, `CREATE TABLE t_multi_cf (
	...
	// There should be 15 KVs read to satisfy the limit (5*numFamilies) plus 1
	// extra read to determine that the last row is finished.
	require.Equal(t, "16", kvPairsCount,
		"expected exactly 16 KV pairs to be read for LIMIT 5 with multiple column families",
	)
}
