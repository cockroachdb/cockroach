...
type ColIndexJoin struct {
	...
	mem struct {
		...
		// varSizeColIndices is the set of indices of the columns that do not have
		// a fixed size for each row. Currently this includes decimal, datum and
		// bytes-like types.
		varSizeColIndices util.FastIntSet
	}
	...
	for {
		...
		case indexJoinConstructingSpans:
			...
			for ; ; s.batch = s.Input.Next() {
				...
				if n := s.batch.Length(); n == 0 || endIdx < n {
					// Reached the memory limit or the end of the input.
					s.startIdx = endIdx
					break
				}
				// Process the next input batch.
				s.startIdx = 0
			}
...
func (s *ColIndexJoin) getRowSize(idx int) int64 {
	...
	for i, ok := s.mem.varSizeColIndices.Next(0); ok; i, ok = s.mem.varSizeColIndices.Next(i + 1) {
		vec := s.batch.ColVec(i)
		switch vec.CanonicalTypeFamily() {
		case types.DecimalFamily:
			rowSize += int64(tree.SizeOfDecimal(&vec.Decimal()[idx]))
		case types.BytesFamily:
			rowSize += vec.Bytes().ElemSize(idx)
		case types.JsonFamily:
			rowSize += vec.JSON().ElemSize(idx)
		case typeconv.DatumVecCanonicalTypeFamily:
			rowSize += int64(vec.Datum().Get(idx).(tree.Datum).Size()) + memsize.DatumOverhead
		}
	}
	return rowSize
}
...
func (s *ColIndexJoin) prepareMemLimit(inputTypes []*types.T) {
	...
	s.mem.constRowSize += int64(rowenc.EncDatumOverhead) * int64(len(inputTypes))
	for i, t := range inputTypes {
		...
		case
			...
			s.mem.constRowSize += colmem.GetFixedSizeTypeSize(t)
		case
			types.DecimalFamily,
			types.BytesFamily,
			types.JsonFamily,
			typeconv.DatumVecCanonicalTypeFamily:
			s.mem.varSizeColIndices.Add(i)
		default:
