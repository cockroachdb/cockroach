...
type ColIndexJoin struct {
	...
	// expectedFetchRowCount is the number of rows that are expected to be
	// fetched for the current lookup. It is equal to the number of input rows
	// that have been consumed to create the spans for a lookup. It is used to
	// make assertions that prevent returning incorrect results due to index
	// corruption.
	expectedFetchRowCount int64
...
func (s *ColIndexJoin) Next() coldata.Batch {
	for {
		...
		case indexJoinConstructingSpans:
			...
			// exactly one output row per input row. This is this true most of
			// the time, except when the locking wait policy is SKIP LOCKED.
			s.cf.setEstimatedRowCount(uint64(rowCount))
			s.expectedFetchRowCount = rowCount
		...
		case indexJoinScanning:
			...
			if s.lockingWaitPolicy == descpb.ScanLockingWaitPolicy_SKIP_LOCKED {
				// If the locking wait policy is SKIP LOCKED, then the number of
				// fetched rows may be less than the number of input rows, but
				// never greater.
				if n := int64(n); n > s.expectedFetchRowCount {
					colexecerror.InternalError(errors.AssertionFailedf(
						"expected to fetch no more than %d rows, found %d", s.expectedFetchRowCount, n,
					))
				}
			} else {
				// If the locking wait policy is not SKIP LOCKED, then the
				// number of fetched rows must be equal to the number of input
				// rows.
				if n := int64(n); n != s.expectedFetchRowCount {
					colexecerror.InternalError(errors.AssertionFailedf(
						"expected to fetch %d rows, found %d", s.expectedFetchRowCount, n,
					))
				}
			}
			s.mu.Lock()
