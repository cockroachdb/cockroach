...
type ColIndexJoin struct {
	...
	spanAssembler colexecjoin.ColSpanAssembler
...
var _ colexecop.Operator = &ColIndexJoin{}
	...
	for {
		...
		case indexJoinConstructingSpans:
			...
			if !s.maintainOrdering {
				...
				//   fetcher to limit the number of results per batch. It's safe to
				//   reorder the spans here because we already restore the original
				//   order of the output during the output collection phase.
				...
				//   rows are output unchanged, in the retrieval order, so it is not
				//   safe to do this when maintainOrdering is true (the ordering to be
				//   maintained may be different than the ordering in the index).
				sort.Sort(spans)
			}
...
func (s *ColIndexJoin) DrainMeta() []execinfrapb.ProducerMetadata {
	...
	if !s.flowCtx.Local {
		if s.nodeID != 0 && s.misplannedRanges != nil {
			trailingMeta = append(trailingMeta, execinfrapb.ProducerMetadata{Ranges: s.misplannedRanges})
		}
	}
...
func NewColIndexJoin(
	...
	tableArgs.InitCols(table, spec.Visibility, spec.HasSystemColumns, nil /* virtualColumn */)

	fetcher := cFetcherPool.Get().(*cFetcher)
	fetcher.estimatedRowCount = 0
	if err := fetcher.Init(
		flowCtx.Codec(), allocator, execinfra.GetWorkMemLimit(flowCtx), false, /* false */
		spec.LockingStrength, spec.LockingWaitPolicy, tableArgs,
	); err != nil {
		return nil, err
	}

	spanAssembler := colexecjoin.NewColSpanAssembler(
		flowCtx.Codec(), allocator, table, index, inputTypes, neededColumns, defaultBatchSizeLimit)
...
// defaultBatchSizeLimit is the limit on the total number of bytes that can be
// allocated in a given batch for the span keys.
const defaultBatchSizeLimit = 4 << 20 /* 4 MB */
