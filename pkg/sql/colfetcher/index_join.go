...
// such as selection vectors or byte offsets.
func (s *ColIndexJoin) getBatchSize() int64 {
	n := s.batch.Length()
	batchSize := s.mem.constRowSize * int64(n)
	if s.mem.hasVarSizeCols {
		for i := range s.mem.byteLikeCols {
			for j := 0; j < n; j++ {
				batchSize += adjustMemEstimate(s.mem.byteLikeCols[i].ElemSize(j))
			}
		}
		for i := range s.mem.decimalCols {
			for j := 0; j < n; j++ {
				batchSize += adjustMemEstimate(int64(tree.SizeOfDecimal(&s.mem.decimalCols[i][j])))
			}
		}
		for i := range s.mem.datumCols {
			for j := 0; j < n; j++ {
				memEstimate := int64(s.mem.datumCols[i].Get(j).(tree.Datum).Size())
				memEstimate += memsize.DatumOverhead
				batchSize += adjustMemEstimate(memEstimate)
			}
		}
	}
	return batchSize
