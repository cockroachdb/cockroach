...
func NewColIndexJoin(
	...
	if useStreamer {
		...
		op.streamerInfo.Streamer = kvstreamer.NewStreamer(
			flowCtx.Cfg.DistSender,
			flowCtx.Stopper(),
			flowCtx.Txn,
			flowCtx.EvalCtx.Settings,
			row.GetWaitPolicy(spec.LockingWaitPolicy),
			op.streamerInfo.budgetLimit,
			op.streamerInfo.budgetAcc,
		)
		op.streamerInfo.Streamer.Init(
