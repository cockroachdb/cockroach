...
func (rf *cFetcher) Init(
	...
	for i := range tableArgs.Cols {
		...
		table.colIdxMap.ords = append(table.colIdxMap.ords, tableArgs.ColIdxMap.GetDefault(int(id)))
	}
	...
	*table = cTableInfo{
	...
	for i := range tableArgs.Cols {
		...
		idx := tableArgs.ColIdxMap.GetDefault(int(col))
	...
	if cHasExtraCols(table) {
		...
		for i, id := range table.index.ExtraColumnIDs {
			idx := tableArgs.ColIdxMap.GetDefault(int(id))
