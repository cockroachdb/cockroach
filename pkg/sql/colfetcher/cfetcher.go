...
type cFetcher struct {
	...
	machine struct {
		...
		// hint for the rows remaining to be returned.
		limitHint int
...
func (rf *cFetcher) resetBatch(timestampOutputIdx, tableOidOutputIdx int) {
	...
	if rf.estimatedRowCount == 0 && rf.machine.limitHint != 0 {
		...
		// initial batch of capacity 1 which is desired behavior.
		//
		...
		case stateFinalizeRow:
			...
			if rf.machine.rowIdx == rf.machine.batch.Capacity() ||
				...
				if rf.machine.limitHint >= rf.machine.rowIdx {
					...
					// rows to be fetched.
					//
					// Note that limitHint will never become negative because of
