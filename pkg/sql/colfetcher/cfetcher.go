...
// returned in columnar format as a coldata.Batch. The batch contains one Vec
// per table column, regardless of the index used; columns that are not needed
// (as per neededCols) are filled with nulls. The Batch should not be modified
// and is only valid until the next call. When there are no more rows, the
// Batch.Length is 0.
