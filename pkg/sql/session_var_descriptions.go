...
var sessionVarDescriptions = map[string]string{
	...
	"canary_stats_mode":                                               "When sql.stats.canary_fraction > 0, controls which table statistics are used for query planning: 'on' always uses the newest (canary) stats, 'off' always uses the second newest (stable) stats, and 'auto' selects probabilistically based on the canary fraction. Has no effect when sql.stats.canary_fraction is 0.",
