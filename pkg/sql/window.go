...
func (p *planner) constructWindowDefinitions(
	...
	for _, windowFn := range n.funcs {
		...
		// Copying the frame since we might be modifying it below.
		if windowDef.Frame != nil {
			n.run.windowFrame = windowDef.Frame.Copy()
		}
	...
	for windowIdx, windowFn := range n.funcs {
		...
		for _, partition := range partitions {
			...
			if n.run.windowFrame != nil {
				frame.Bounds = n.run.windowFrame.Bounds
				frame.Mode = n.run.windowFrame.Mode
