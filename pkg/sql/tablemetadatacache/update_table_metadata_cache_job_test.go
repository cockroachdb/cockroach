...
func TestUpdateTableMetadataCacheAutomaticUpdates(t *testing.T) {
	...
	t.Run("AutomaticUpdatesEnabled", func(t *testing.T) {
		conn.Exec(t, `SET CLUSTER SETTING tablemetadatacache.automatic_updates.enabled = true`)
		conn.Exec(t, `SET CLUSTER SETTING tablemetadatacache.data_valid_duration = '50ms'`)
		...
		conn.Exec(t, `RESET CLUSTER SETTING tablemetadatacache.automatic_updates.enabled`)
	...
	t.Run("AutomaticUpdatesDisabled", func(t *testing.T) {
		conn.Exec(t, `SET CLUSTER SETTING tablemetadatacache.automatic_updates.enabled = f`)
