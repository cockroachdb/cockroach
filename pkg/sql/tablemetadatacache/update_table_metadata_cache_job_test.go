...
func TestUpdateTableMetadataCacheJobRunsOnRPCTrigger(t *testing.T) {
	...
	//tc.ApplicationLayer(0).SQLConn(t)
...
WHERE job_type = 'UPDATE TABLE METADATA CACHE' AND claim_instance_id IS NOT NULL`)
		if !row.Next() {
	...
	testutils.SucceedsSoon(t, func() error {
		// Check num runs is 2.
		var count int
		rowCount := conn.QueryRow(t, "SELECT num_runs FROM system.jobs WHERE job_type = 'UPDATE TABLE METADATA CACHE'")
		rowCount.Scan(&count)
		if count != 2 {
