...
`,
		...
		for ; ok; ok, err = it.Next(ctx) {
			...
			ti.batchRows = append(ti.batchRows, tableMetadataIterRow{
		...
		if len(ti.batchRows) > 0 {
			break
		}
	}

	return len(ti.batchRows) > 0, nil
}
