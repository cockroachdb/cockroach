...
func TestDeletePreservingIndexEncoding(t *testing.T) {
	...
	getRevisionsForTest := func(setupSQL, schemaChangeSQL, dataSQL string, deletePreservingEncoding bool) ([]storageccl.VersionedValues, []byte, error) {
...
type WrappedVersionedValues struct {
	Key    roachpb.Key
	Values []roachpb.IndexValueWrapper
}

func compareRevisionHistories(
	expectedHistory []storageccl.VersionedValues,
	expectedPrefixLength int,
	deletePreservingHistory []storageccl.VersionedValues,
...
func decodeVersionedValues(
	revisions []storageccl.VersionedValues, deletePreserving bool,
) ([]WrappedVersionedValues, error) {
	wrappedVersionedValues := make([]WrappedVersionedValues, len(revisions))

	for i, revision := range revisions {
		wrappedValues := make([]roachpb.IndexValueWrapper, len(revision.Values))

		for j, value := range revision.Values {
			var wrappedValue *roachpb.IndexValueWrapper
			var err error
			...
			if deletePreserving {
				...
				if len(value.RawBytes) == 0 {
					wrappedValue = &roachpb.IndexValueWrapper{
						Value:   nil,
						Deleted: true,
					}
				} else {
