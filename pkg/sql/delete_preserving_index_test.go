...
func TestDeletePreservingIndexEncoding(t *testing.T) {
	...
	getRevisionsForTest := func(setupSQL, schemaChangeSQL, dataSQL string, deletePreservingEncoding bool) ([]bulk.VersionedValues, []byte, error) {
		...
		revisions, err := bulk.GetAllRevisions(context.Background(), kvDB, prefix, prefixEnd, now, end)
		if err != nil {
			return nil, nil, err
		}
...
func compareRevisionHistories(
	expectedHistory []bulk.VersionedValues,
	expectedPrefixLength int,
	deletePreservingHistory []bulk.VersionedValues,
...
func decodeVersionedValues(
	revisions []bulk.VersionedValues, deletePreserving bool,
