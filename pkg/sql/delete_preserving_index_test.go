...
func TestDeletePreservingIndexEncoding(t *testing.T) {
	...
	getRevisionsForTest := func(setupSQL, schemaChangeSQL, dataSQL string, deletePreservingEncoding bool) ([]client.VersionedValues, []byte, error) {
		...
		revisions, err := client.GetAllRevisions(context.Background(), kvDB, prefix, prefixEnd, now, end)
		if err != nil {
			return nil, nil, err
		}
...
func compareRevisionHistories(
	expectedHistory []client.VersionedValues,
	expectedPrefixLength int,
	deletePreservingHistory []client.VersionedValues,
...
func decodeVersionedValues(
	revisions []client.VersionedValues, deletePreserving bool,
