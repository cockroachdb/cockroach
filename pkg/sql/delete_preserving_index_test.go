...
func TestDeletePreservingIndexEncoding(t *testing.T) {
	...
	getRevisionsForTest := func(setupSQL, schemaChangeSQL, dataSQL string, deletePreservingEncoding bool) ([]backfill.VersionedValues, []byte, error) {
		...
		revisions, err := backfill.GetAllRevisions(context.Background(), kvDB, prefix, prefixEnd, now, end)
		if err != nil {
			return nil, nil, err
		}
...
type WrappedVersionedValues struct {
	Key    roachpb.Key
	Values []descpb.IndexValueWrapper
}

func compareRevisionHistories(
	expectedHistory []backfill.VersionedValues,
	expectedPrefixLength int,
	deletePreservingHistory []backfill.VersionedValues,
...
func decodeVersionedValues(
	revisions []backfill.VersionedValues, deletePreserving bool,
) ([]WrappedVersionedValues, error) {
	wrappedVersionedValues := make([]WrappedVersionedValues, len(revisions))

	for i, revision := range revisions {
		wrappedValues := make([]descpb.IndexValueWrapper, len(revision.Values))

		for j, value := range revision.Values {
			var wrappedValue *descpb.IndexValueWrapper
			var err error
			...
			if deletePreserving {
				...
				if len(value.RawBytes) == 0 {
					wrappedValue = &descpb.IndexValueWrapper{
						Value:   nil,
						Deleted: true,
					}
				} else {
