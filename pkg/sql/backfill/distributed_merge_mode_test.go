...
// secIdx describes a secondary index for test construction.
type secIdx struct {
...
// column IDs and directions. The inverted flag marks an inverted index.
...
func TestIsBackfillDataSorted(t *testing.T) {
	...
	tests := []struct {
		...
		pkIDs:    ids(1, 2),
		pkDirs:   dirs(asc, asc),
		destIdx:  secIdx{colIDs: ids(1), dirs: dirs(asc), inverted: true},
		expected: false,
	}, {
