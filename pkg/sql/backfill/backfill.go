...
type IndexBackfiller struct {
	...
	// backfilled.
	indexesToEncode []sqlbase.IndexDescriptor
}
...
func (ib *IndexBackfiller) InitForLocalUse(
	...
	predicateRefColIDs.ForEach(func(col sqlbase.ColumnID) {
...
func (ib *IndexBackfiller) InitForDistributedUse(
	...
	evalCtx := flowCtx.NewEvalCtx()
	var predicates map[sqlbase.IndexID]tree.TypedExpr
...
func (ib *IndexBackfiller) initCols(desc *sqlbase.ImmutableTableDescriptor) {
	...
	if len(desc.Mutations) > 0 {
		ib.cols = make([]sqlbase.ColumnDescriptor, 0, len(desc.Columns)+len(desc.Mutations))
...
func (ib *IndexBackfiller) init(
	evalCtx *tree.EvalContext,
	predicateExprs map[sqlbase.IndexID]tree.TypedExpr,
	...
	if len(ib.predicates) > 0 {
		ib.indexesToEncode = make([]sqlbase.IndexDescriptor, 0, len(ib.added))
	}
