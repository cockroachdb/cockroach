...
func (vih *VectorIndexHelper) ReEncodeVector(
	...
	key, err := vecencoding.ExtractKey(keyBytes, vih.numPrefixCols)
	if err != nil {
	...
	vec, suffix, err := vecencoding.DecodeUnquantizedVector(valueBytes)
	if err != nil {
		return &rowenc.IndexEntry{}, err
	}
	...
	vecencoding.EncodeValueSide(buf, quantizedVector.UnsafeBytes(), suffix)
