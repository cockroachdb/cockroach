...
func CheckMutationFilter(m sqlbase.DescriptorMutation) bool {
	return m.GetCheck() != nil && m.Direction == sqlbase.DescriptorMutation_ADD
}
...
func (cb *CheckBackfiller) Init(evalCtx *tree.EvalContext, desc sqlbase.TableDescriptor) error {
	cb.evalCtx = evalCtx
	...
	if typed, err := sqlbase.MakeCheckExprs(
		cb.added,
