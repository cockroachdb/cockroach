...
type VectorIndexHelper struct {
	...
	// familyIDs contains the family IDs needed to fetch the required columns for the merge operation.
...
func NewVectorIndexHelper(
	...
	// Initialize familyIDs using span.MakeSplitter for the merge fetch spec
	var neededColOrdinals intsets.Fast
	for _, colID := range columnsToFetch {
	...
	splitter := span.MakeSplitter(desc, desc.GetPrimaryIndex(), neededColOrdinals)
	vih.familyIDs = splitter.FamilyIDs()
...
func (vih *VectorIndexHelper) MergeVector(
	...
	spanToScan, containsNull, err := builder.SpanFromEncDatums(encRow)
	if err != nil {
		return nil, false, err
	}
	if containsNull {
	...
	// Use span.Splitter to only fetch the needed column families
	splitter := span.MakeSplitterWithFamilyIDs(
		len(vih.mergeFetchSpec.KeyAndSuffixColumns),
		vih.familyIDs,
	)
	
	var tmpSpans []roachpb.Span
	tmpSpans = splitter.MaybeSplitSpanIntoSeparateFamilies(tmpSpans, spanToScan, len(encRow), containsNull)
	...
	err = fetcher.StartScan(ctx, tmpSpans, nil /* spanIDs */, rowinfra.GetDefaultBatchBytesLimit(false /* forceProductionValue */), 1 /* rowLimit */)
	if err != nil {
		return nil, false, err
	}
