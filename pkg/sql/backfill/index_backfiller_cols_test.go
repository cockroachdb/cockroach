...
func TestInitIndexesAllowList(t *testing.T) {
	...
	t.Run("nil allowList", func(t *testing.T) {
		...
		ib := &IndexBackfiller{}
		err := ib.initIndexes(context.Background(), keys.SystemSQLCodec, desc, nil /* allowList */, 0 /* sourceIndexID */, nil /* vecIndexMgr */)
	...
	t.Run("non-nil allowList", func(t *testing.T) {
		ib := &IndexBackfiller{}
		err := ib.initIndexes(context.Background(), keys.SystemSQLCodec, desc, []catid.IndexID{3} /* allowList */, 0 /* sourceIndexID */, nil /* vecIndexMgr */)
		require.NoError(t, err)
