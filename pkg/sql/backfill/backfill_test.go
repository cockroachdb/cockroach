...
func TestConcurrentOperationsDuringVectorIndexCreation(t *testing.T) {
	...
	_, err := db.ExecContext(ctx, `UPDATE vectors SET vec = '[10, 11, 12]' WHERE id = 1`)
	require.Error(t, err)
	require.Contains(t, err.Error(), "pq: cannot write to vector index on vectors while index is being built")

	_, err = db.ExecContext(ctx, `INSERT INTO vectors (id, vec) VALUES (4, '[13, 14, 15]')`)
	require.Error(t, err)
	require.Contains(t, err.Error(), "pq: cannot write to vector index on vectors while index is being built")
