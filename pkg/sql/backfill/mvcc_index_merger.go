...
func (ibm *IndexBackfillMerger) constructMergeBatch(
	...
	for i := range rb.Results {
		...
		if rb.Results[i].Rows[0].Value == nil {
			return nil, 0, 0, errors.Errorf("expected value to be present in temp index for key=%s", rb.Results[i].Rows[0].Key)
		}
		...
		if err := ibm.muBoundAccount.boundAccount.Grow(ctx, rowMem); err != nil {
			return nil, 0, 0, errors.Errorf("failed to allocate space to read latest keys from temp index")
		}
	...
	for i := range rb.Results {
		...
		if deleted {
			...
			if err := ibm.muBoundAccount.boundAccount.Grow(ctx, int64(len(mergedEntry.Key))); err != nil {
				ibm.muBoundAccount.Unlock()
				return nil, 0, 0, errors.Errorf("failed to allocate space to merge delete from temp index")
			}
			...
			if err := ibm.muBoundAccount.boundAccount.Grow(ctx, int64(len(mergedEntry.Key)+len(mergedEntry.Value.RawBytes))); err != nil {
				ibm.muBoundAccount.Unlock()
				return nil, 0, 0, errors.Errorf("failed to allocate space to merge put from temp index")
			}
