...
func (ibm *IndexBackfillMerger) Run(ctx context.Context) {
	...
	mainCtxGroup := ctxgroup.WithContext(ctx)
	mainCtxGroup.GoCtx(func(ctx context.Context) error {
	...
	workerCtxGroup := ctxgroup.WithContext(ctx)
	workerCtxGroup.GoCtx(func(ctx context.Context) error {
		defer close(mergeCh)
	...
	mainCtxGroup.GoCtx(func(ctx context.Context) error {
		...
		return workerCtxGroup.Wait()
	})

	var err error
	go func() {
		defer close(progCh)
		err = mainCtxGroup.Wait()
	}()
