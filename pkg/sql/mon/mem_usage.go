...
// StartUnlimited starts the monitor in "detached" mode without
// a pool and without a maximum budget.
func (mm *MemoryMonitor) StartUnlimited(
	name string, curCount *metric.Counter, maxHist *metric.Histogram,
) {
	mm.Start(name, curCount, maxHist, nil, MakeStandaloneBudget(math.MaxInt64), 1)
}
