...
func TestMemoryAllocations(t *testing.T) {
	...
	for _, max := range maxs {
		pool.Start("test-pool", nil, nil, nil, MakeStandaloneBudget(max), 1)
		...
		for _, hf := range hysteresisFactors {
			...
			for _, pb := range preBudgets {
				...
				for _, pa := range poolAllocSizes {
					...
					// parameters.
					m = MemoryMonitor{}
					m.Start("test-mon", nil, nil, &pool, MakeStandaloneBudget(pb), pa)
...
func TestMemoryAccount(t *testing.T) {
	...
	m := MemoryMonitor{}
	pool := MemoryMonitor{}
	pool.Start("test-pool", nil, nil, nil, MakeStandaloneBudget(100), 1)
	m.Start("test-mon", nil, nil, &pool, BoundAccount{}, 1)
