...
func TestMemoryAllocations(t *testing.T) {
	...
	checkInvariants := func() {
		...
		avail := m.mu.curBudget.curAllocated + m.reservedBytes
		if sum > avail {
			t.Errorf("total account sum %d greater than total monitor budget %d", sum, avail)
			fail = true
		}
		if pool.mu.curAllocated > pool.reservedBytes {
			t.Errorf("pool cur %d exceeds max %d", pool.mu.curAllocated, pool.reservedBytes)
			fail = true
		}
	...
	if log.V(2) {
		...
		reportAndCheck = func(extraFmt string, extras ...interface{}) {
			fmt.Printf("%5d %5d %5d %5d ", m.mu.curAllocated, m.mu.curBudget.curAllocated, m.reservedBytes, pool.mu.curAllocated)
			for accI := range accs {
	...
	for _, max := range maxs {
		pool.StartMonitor("test-pool", nil, nil, nil, max, 1)
		...
		for _, hf := range hysteresisFactors {
			...
			for _, pb := range preBudgets {
				...
				for _, pa := range poolAllocSizes {
					...
					// parameters.
					m = MemoryMonitor{}
					m.StartMonitor("test-mon", nil, nil, &pool, pb, pa)
...
func TestMemoryAccount(t *testing.T) {
	...
	m := MemoryMonitor{}
	pool := MemoryMonitor{}
	pool.StartMonitor("test-pool", nil, nil, nil, 100, 1)
	m.StartMonitor("test-mon", nil, nil, &pool, 0, 1)
