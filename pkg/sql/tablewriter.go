...
func (ti *tableInserter) spans() (reads, writes roachpb.Spans) {
	return collectTableWriterSpans(&ti.ri.Helper, ti.ri.Fks)
}
...
func (tu *tableUpdater) spans() (reads, writes roachpb.Spans) {
	return collectTableWriterSpans(&tu.ru.Helper, tu.ru.Fks)
}
...
func (tu *tableUpserter) spans() (reads, writes roachpb.Spans) {
	return collectTableWriterSpans(&tu.ri.Helper, tu.ri.Fks)
}
...
func (td *tableDeleter) spans() (reads, writes roachpb.Spans) {
	return collectTableWriterSpans(&td.rd.Helper, td.rd.Fks)
}

func collectTableWriterSpans(rows, fks sqlbase.SpanCollector) (reads, writes roachpb.Spans) {
	rowsReads, rowsWrites := rows.CollectSpans()
	fkReads, fkWrites := fks.CollectSpans()
	if len(fkWrites) > 0 {
		panic(fmt.Sprintf("unexpected foreign key span writes: %v", fkWrites))
	}
	return append(rowsReads, fkReads...), rowsWrites
}
