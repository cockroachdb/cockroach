...
func (tb *tableWriterBase) init(
	...
	tb.txn = txn
	if tb.txn.AdmissionHeader().CreateTime == 0 {
		log.Warningf(evalCtx.Context, "tableWriterBase has uninitialized AdmissionHeader")
	}
	tb.desc = tableDesc
	tb.b = txn.NewBatch()
...
func (tb *tableWriterBase) flushAndStartNewBatch(ctx context.Context) error {
	...
	if responseAdmissionQ != nil {
		...
		log.Warningf(ctx, "tableWriterBase does not have a admission.WorkQueue for the response")
	}
