...
func (c *ConnectionStateMachine) Init(ctx context.Context, results ClientComm) error {
	...
	// TODO(jeffswenson): can I poison the session if this is called? Or make a metemorphic
	// option to ensure it doesn't matter.
	executor := c.config.server.newConnExecutor(
		...
		stmtBuf,
		results,
		// TODO(jeffswenson): get the internal memory metrics here
		c.config.metrics,
