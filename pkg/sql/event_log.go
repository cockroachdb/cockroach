...
func insertEventRecords(
	...
	if !logToSystemTable || txn == nil {
		for _, entry := range entries {
			log.SEventLoggerNoPersist.StructuredEvent(ctx, entry, log.WithWriteToTable(logToSystemTable))
		}
	} else {
		txn.KV().AddCommitTrigger(func(ctx context.Context) {
			for _, entry := range entries {
				logger := log.NewSEventLogger(execCfg.Codec.TenantID.ToUint64())
				logger.StructuredEvent(ctx, entry, log.WithWriteToTable(logToSystemTable))
			}
		})
	}
	return nil
