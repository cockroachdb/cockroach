...
func (p *planner) logEvent(
	...
	return logEventInternalForSQLStatements(ctx, p.extendedEvalCtx.ExecCfg, p.txn, ts, sqlInstanceID, descID, user, stmt, event)
}
...
// log.ClusterEvent(). In that case, the caller is responsible for
// calling log.ClusterEvent() directly.
//
