...
func TestConcurrentUpsertAddDropVirtualComputedColumn(t *testing.T) {
	...
	testCases := []struct {
		...
		// Upsert case.
		{
			...
			returningBDelta: 0,
			alterStmt:       `ALTER TABLE d.t ADD COLUMN positive_a BOOL AS (a > 0) VIRTUAL`,
			testQuery:       `SELECT count(*) FROM d.t@primary WHERE positive_a IS NULL OR NOT positive_a`,
			result:          []string{"0"},
			colAValue:       1,
		},
		{
			...
			updateStmt:      `UPSERT INTO d.t VALUES ($1, $2) RETURNING a,b`,
			returningBDelta: 0,
			alterStmt:       `ALTER TABLE d.t DROP COLUMN positive_a`,
		...
		// Update cases.
		{
			...
			returningBDelta: -1,
			alterStmt:       `ALTER TABLE d.t ADD COLUMN positive_b_plus_2 BOOL AS (b+2 > 0) VIRTUAL`,
			testQuery:       `SELECT * FROM d.t@primary ORDER BY 1,2 LIMIT 1`,
			result:          []string{"1", "0", "true"},
			colAValue:       1,
		},
		{
			name:            "update drop",
			...
			returningBDelta: -2,
			alterStmt:       `ALTER TABLE d.t DROP COLUMN positive_b_plus_2`,
			testQuery:       `SELECT * FROM d.t@primary ORDER BY 1,2 LIMIT 1`,
			result:          []string{"1", "-1"},
			colAValue:       1,
		},
		...
		{
			name:            "delete add",
			updateStmt:      `DELETE FROM d.t WHERE a = $1 AND b = $2 RETURNING a, b`,
			returningBDelta: 0,
			alterStmt:       `ALTER TABLE d.t ADD COLUMN positive_a BOOL AS (a > 0) VIRTUAL`,
			testQuery:       `SELECT count(*) FROM d.t@primary WHERE positive_a IS NULL OR NOT positive_a`,
			result:          []string{"0"},
