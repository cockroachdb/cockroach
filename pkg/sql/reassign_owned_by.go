...
func (n *reassignOwnedByNode) startExec(params runParams) error {
	...
	for _, oldRole := range n.n.OldRoles {
		...
		for _, dbID := range lCtx.dbIDs {
			if params.p.IsOwner(lCtx.dbDescs[dbID], oldRole) {
				if err := n.reassignDatabaseOwner(lCtx.dbDescs[dbID], oldRole, params); err != nil {
					return err
				}
			}
		}
		for _, schemaID := range lCtx.schemaIDs {
			if params.p.IsOwner(lCtx.schemaDescs[schemaID], oldRole) {
				if err := n.reassignSchemaOwner(lCtx.schemaDescs[schemaID], oldRole, params); err != nil {
					return err
				}
			}
		}
		for _, tbID := range lCtx.tbIDs {
			if params.p.IsOwner(lCtx.tbDescs[tbID], oldRole) {
				if err := n.reassignTableOwner(lCtx.tbDescs[tbID], oldRole, params); err != nil {
					return err
				}
			}
		}
		for _, typID := range lCtx.typIDs {
			if params.p.IsOwner(lCtx.typDescs[typID], oldRole) && (lCtx.typDescs[typID].Kind != descpb.TypeDescriptor_ALIAS) {
				if err := n.reassignTypeOwner(lCtx.typDescs[typID], oldRole, params); err != nil {
					return err
				}
			}
		}
