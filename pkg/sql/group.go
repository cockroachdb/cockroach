...
func (p *planner) groupBy(
	...
	if isAggregate := p.trans.IsAggregate(n, p.session.SearchPath); !isAggregate {
		return nil, nil, nil
	}
	...
	if n.Having != nil {
		if p.trans.WindowFuncInExpr(n.Having.Expr) {
	...
	case *parser.FuncExpr:
		if agg := t.GetAggregateConstructor(); agg != nil {
			...
			case 1:
				...
				if err := v.planner.trans.AssertNoAggregationOrWindowing(
					argExpr,
