...
func (p *planner) groupBy(
	...
	if isAggregate := p.txCtx.IsAggregate(n, p.evalCtx.SessData.SearchPath); !isAggregate {
		return nil, nil, nil
	}
	...
	for i, expr := range n.GroupBy {
		...
		if err := p.txCtx.AssertNoAggregationOrWindowing(
			expr, "GROUP BY", p.evalCtx.SessData.SearchPath,
		); err != nil {
			return nil, nil, err
		}
	}
	...
	group.desiredOrdering = group.desiredAggregateOrdering(&p.evalCtx.EvalContext)
...
func (n *groupNode) Next(params runParams) (bool, error) {
	...
	for !n.run.populated {
		...
		for _, f := range n.funcs {
			...
			if err := f.add(params.ctx, &params.evalCtx.EvalContext, bucket, value); err != nil {
				return false, err
			}
		}
		scratch = bucket[:0]
	...
	for i, f := range n.funcs {
		...
		if !ok {
			...
			// (like COUNT).
			aggregateFunc = f.create(&params.evalCtx.EvalContext)
		}
		var err error
...
func (v *extractAggregatesVisitor) addAggregation(f *aggregateFuncHolder) *tree.IndexedVar {
	...
	colName, err := getRenderColName(
		v.planner.evalCtx.SessData.SearchPath, tree.SelectExpr{Expr: f.expr}, &v.preRender.ivarHelper)
	if err != nil {
		...
		if agg := t.GetAggregateConstructor(); agg != nil {
			...
			case 1:
				...
				if err := v.planner.txCtx.AssertNoAggregationOrWindowing(
					...
					v.planner.evalCtx.SessData.SearchPath,
				); err != nil {
					v.err = err
					return false, expr
				}
			...
			if t.Filter != nil {
				...
				if err := v.planner.txCtx.AssertNoAggregationOrWindowing(
					filterExpr, "FILTER", v.planner.evalCtx.SessData.SearchPath,
				); err != nil {
					v.err = err
					return false, expr
				}
