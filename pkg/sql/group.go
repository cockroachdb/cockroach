...
func (n *groupNode) Next(params runParams) (bool, error) {
	...
	for i, f := range n.funcs {
		...
		if !ok {
			...
			// (like COUNT).
			aggregateFunc = f.create(params.EvalContext(), nil)
		}
		var err error
			...
			if len(t.Exprs) == 0 {
				...
				if len(t.Exprs) > 1 {
					...
					for i := 1; i < len(t.Exprs); i++ {
						if _, ok := t.Exprs[i].(tree.VariableExpr); ok {
							v.err = pgerror.UnimplementedWithIssueError(10495, "aggregate functions with multiple non-constant expressions are not supported")
							return false, expr
						}
						var err error
						...
						if err != nil {
							v.err = pgerror.NewErrorf(pgerror.CodeInternalError,
								"Can't evaluate %s", t.Exprs[i].String())
							return false, expr
						}
					}
				}
