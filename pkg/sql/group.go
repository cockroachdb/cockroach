...
//
// The bottom of the complex is always a groupNode which initially wraps the given
// renderNode (but it can change later for an equivalent planNode if the
// renderNode is optimized out).
//
// On top of the groupNode there is normally a renderNode which renders
// post-aggregation expressions. If HAVING is used, there is also a filterNode
// in-between them.
//
// The top planNode and bottom groupNode the node complex are returned; if there
...
func (p *planner) groupBy(
	...
	if typedHaving != nil {
		havingNode = &filterNode{
			p:      r.planner,
			source: planDataSource{plan: top, info: &dataSourceInfo{}},
		}
		top = havingNode
		...
		havingNode.source.info.sourceColumns = group.columns
	...
	postRender := &renderNode{
		planner: r.planner,
		source:  planDataSource{plan: top},
	}
	top = postRender
	...
	postRender.source.info = &dataSourceInfo{sourceColumns: group.columns}
...
// them to the preRender renderNode. It returns new expression where
// arguments to aggregate functions, as well as expressions that also appear in a GROUP
// BY, are replaced with IndexedVars suitable for a node that sits on top of the
// groupNode - a renderNode or a filterNode (for HAVING).
