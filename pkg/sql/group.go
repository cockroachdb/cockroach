...
func (p *planner) groupBy(
	...
	group.desiredOrdering = group.desiredAggregateOrdering(&p.extendedEvalCtx.EvalContext)
...
func (n *groupNode) Next(params runParams) (bool, error) {
	...
	for !n.run.populated {
		...
		for _, f := range n.funcs {
			...
			if err := f.add(params.ctx, &params.extendedEvalCtx.EvalContext, bucket, value); err != nil {
				return false, err
			}
		}
		scratch = bucket[:0]
	...
	for i, f := range n.funcs {
		...
		if !ok {
			...
			// (like COUNT).
			aggregateFunc = f.create(&params.extendedEvalCtx.EvalContext)
		}
		var err error
