...
const (
	// Packed is used when the generated bytes are intended to be "close"
	// together in the generated key space.
	Packed ByteSpacing = iota
	// Spread is used when the generated bytes are intended to be evenly
...
// byteBetweenPacked generates a byte value between hi and lo. Additionally,
...
// byteBetweenSpread returns a byte value between hi and lo.
...
func genEvenlySpacedHelper(result [][]byte, lo, hi int, bot, top []byte) {
	...
	mid := lo + (hi-lo)/2
	midBytes := GenByteStringBetween(bot, top, Spread)
	result[mid] = midBytes
