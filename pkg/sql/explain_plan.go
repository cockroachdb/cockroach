...
func observePlan(
	...
	for i := range subqueryPlans {
		if _, err := observer.enterNode(ctx, "subquery", nil); err != nil && returnError {
			return err
		}
		...
		if err := observer.leaveNode("subquery", nil); err != nil && returnError {
			return err
		}
	}
	...
	for i := range postqueryPlans {
		if _, err := observer.enterNode(ctx, "postquery", nil); err != nil && returnError {
			return err
		}
		...
		if err := observer.leaveNode("postquery", nil); err != nil && returnError {
			return err
		}
	}
