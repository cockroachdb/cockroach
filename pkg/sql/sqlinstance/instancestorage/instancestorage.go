...
type Storage struct {
	...
	TestingKnobs struct {
		...
		// ShouldEncodeLocalityAddressesFn, if set, overrides the cluster
		// version check for whether to encode the locality_addresses column.
...
func (s *Storage) CreateNodeInstance(
	...
	return s.createInstanceRow(ctx, session, rpcAddr, sqlAddr, locality, binaryVersion, nodeID, localityAddresses)
}
...
func (s *Storage) CreateInstance(
	...
	return s.createInstanceRow(ctx, session, rpcAddr, sqlAddr, locality, binaryVersion, noNodeID, localityAddresses)
}
...
func (s *Storage) shouldEncodeLocalityAddresses(ctx context.Context, txn *kv.Txn) (bool, error) {
	...
	guard, err := s.settingsWatch.MakeVersionGuard(
		ctx, txn, clusterversion.V26_3_SQLInstancesAddLocalityAddresses)
	if err != nil {
		return false, err
	}
	return guard.IsActive(clusterversion.V26_3_SQLInstancesAddLocalityAddresses), nil
}
