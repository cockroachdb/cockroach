...
func (s *Storage) reclaimRegion(ctx context.Context, region []byte) error {
	...
	instances, err := s.getInstanceRows(ctx, region, s.db)
	if err != nil {
		return err
	}
...
func (s *Storage) getInstanceRows(
	...
	const fetchAllRows = 0
	rows, err := db.Scan(ctx, start, start.PrefixEnd(), fetchAllRows)
	if err != nil {
		return nil, err
	}
...
func (s *Storage) generateAvailableInstanceRows(
	...
	_, _, err := s.reclaimGroup.Do("allocate-instance-ids", func() (interface{}, error) {
		err := s.db.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
			instances, err := s.getInstanceRows(ctx /*global*/, nil, txn)
			if err != nil {
				return err
			}
