...
func (s *Storage) CreateInstance(
	...
	err := s.db.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
		...
		// Iterate over all current instances in the system.sql_instances table to initialize the instance
		// ID. If any existing instance ID does not have a live SQL session associated it, we can reuse it.
		...
		for _, instance := range rows {
			...
			if len(instance.addr) == 0 {
				instanceID = instance.instanceID
				break
			}
			sessionAlive, _ := s.slReader.IsAlive(ctx, instance.sessionID)
			if !sessionAlive {
			...
			if instance.instanceID >= instanceID {
				// Set available instance ID to be 1 + max(active instance IDs)
