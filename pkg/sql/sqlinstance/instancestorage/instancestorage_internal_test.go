...
func TestReclaimAndGenerateInstanceRows(t *testing.T) {
	...
	t.Run("with some preallocated", func(t *testing.T) {
		...
		for _, i := range []int{0, 1} {
			require.NoError(t, storage.CreateInstanceDataForTest(ctx, region, instanceIDs[i], "", "", sqlliveness.SessionID([]byte{}), sessionExpiry, roachpb.Locality{}, roachpb.Version{}, true, false, nil))
		}
...
func claim(
	...
	require.NoError(t, storage.CreateInstanceDataForTest(ctx, region, instanceID, rpcAddr, sqlAddr, sessionID, sessionExpiration, roachpb.Locality{}, roachpb.Version{}, true, false, nil))
}
