...
func TestStorage(t *testing.T) {
	...
	setup := func(t *testing.T) (
		*stop.Stopper, *instancestorage.Storage, *slstorage.FakeStorage, *hlc.Clock,
		...
		storage.TestingKnobs.ShouldEncodeLocalityAddressListFn = func() (bool, error) {
	...
	t.Run("create-instance-get-instance", func(t *testing.T) {
		...
		localityAddresses := getTestLocalityAddressList()

		{
			...
			require.Equal(t, localityAddresses, instance.LocalityAddressList)
		}
	})
	...
	t.Run("release-instance-get-all-instances", func(t *testing.T) {
		...
		createInstance := func(t *testing.T, instance sqlinstance.InstanceInfo) {
			...
			session := &sqllivenesstestutils.FakeSession{SessionID: instance.SessionID,
				...
				ExpTS:   sessionExpiry}
			created, err := storage.CreateInstance(ctx, session, instance.InstanceRPCAddr, instance.InstanceSQLAddr, instance.Locality, instance.BinaryVersion, instance.LocalityAddressList)
			require.NoError(t, err)
		...
		equalInstance := func(t *testing.T, expect sqlinstance.InstanceInfo, actual sqlinstance.InstanceInfo) {
			...
			require.Equal(t, expect.LocalityAddressList, actual.LocalityAddressList)
		}
...
func getTestLocalityAddressList() []roachpb.LocalityAddress {
...
func TestSQLAccess(t *testing.T) {
	...
	localityAddressList := getTestLocalityAddressList()
	session := makeSession()
	session.StartTS = clock.Now()
	session.ExpTS = session.StartTS.Add(expiration.Nanoseconds(), 0)
	instance, err := storage.CreateInstance(ctx, session, "rpcAddr", "sqlAddr", locality, binaryVersion, localityAddressList)
	require.NoError(t, err)

	// Query the table through SQL and verify the query completes successfully.
	rows := tDB.Query(t, fmt.Sprintf("SELECT id, addr, sql_addr, session_id, locality, locality_address_list FROM \"%s\".sql_instances", dbName))
	defer rows.Close()
	...
	var parsedLocalityAddressList gosql.NullString
	if !assert.True(t, rows.Next()) {
		require.NoError(t, rows.Err())
		require.NoError(t, rows.Close())
	}
	err = rows.Scan(&parsedInstanceID, &parsedAddr, &parsedSqlAddr, &parsedSessionID, &parsedLocality, &parsedLocalityAddressList)
	require.NoError(t, err)
	...
	require.Equal(t, instance.LocalityAddressList, localityAddressList)
	...
	for rows.Next() {
		...
		require.Empty(t, parsedLocalityAddressList.String)
		i++
	}
	require.NoError(t, rows.Err())
}
...
func TestReclaimLoop(t *testing.T) {
	...
	localityAddressLists := [][]roachpb.LocalityAddress{getTestLocalityAddressList(), getTestLocalityAddressList()}

	for i, id := range instanceIDs {
		require.NoError(t, slStorage.Insert(ctx, sessionIDs[i].ID(), sessionExpiry))
		require.NoError(t, storage.CreateInstanceDataForTest(ctx, region, id, rpcAddresses[i], sqlAddresses[i], sessionIDs[i].ID(), sessionExpiry, localities[i], binaryVersions[i], false, true, localityAddressLists[i]))
	}
	...
	for i, instance := range instances {
		...
		case 0, 1:
			...
			require.Equal(t, localityAddressLists[i], instance.LocalityAddressList)
		default:
			require.Empty(t, instance.InstanceRPCAddr)
			require.Empty(t, instance.InstanceSQLAddr)
			require.Empty(t, instance.SessionID)
			require.Empty(t, instance.Locality)
			require.Empty(t, instance.BinaryVersion)
			require.Empty(t, instance.LocalityAddressList)
		}
	}
}
