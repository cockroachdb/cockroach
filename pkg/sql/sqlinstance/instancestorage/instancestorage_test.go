...
func TestStorage(t *testing.T) {
	...
	t.Run("create-instance-get-instance", func(t *testing.T) {
		...
		const addr = "addr"
		{
			instanceID, err := storage.CreateInstance(ctx, sessionID, addr)
	...
	t.Run("release-instance-get-all-instances", func(t *testing.T) {
		...
		sessionIDs := [...]sqlliveness.SessionID{"session1", "session2", "session3"}
		{
			for index, addr := range addresses {
				instanceID, err := storage.CreateInstance(ctx, sessionIDs[index], addr)
		...
		// Release an instance and verify all instances
		// are returned including the released instance.
		{
			...
			for index, instance := range instances {
				require.Equal(t, instanceIDs[index], instance.InstanceID())
				if index == 0 {
					// Verify released instance has empty session and address fields
					require.Equal(t, len(instance.SessionID().String()), 0)
					require.Equal(t, len(instance.InstanceAddr()), 0)
				} else {
					require.Equal(t, sessionIDs[index].String(), instance.SessionID().String())
					require.Equal(t, addresses[index], instance.InstanceAddr())
				}
		...
		// Verify released instance ID gets reused.
		{
			...
			newAddr := "addr4"
			instanceID, err = storage.CreateInstance(ctx, newSessionID, newAddr)
			require.NoError(t, err)
	...
	var (
		...
		checkGetInstance = func(t *testing.T, i base.SQLInstanceID) {
			...
			if _, free := state.freeInstances[i]; free {
				require.Equal(t, 0, len(instanceInfo.InstanceAddr()))
				require.Equal(t, 0, len(instanceInfo.SessionID()))
			} else {
