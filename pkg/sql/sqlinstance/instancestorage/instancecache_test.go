...
func TestMigrationCache(t *testing.T) {
	...
	t.Run("setting_changed_before_initialization", func(t *testing.T) {
		...
		settings.Version.SetActiveVersion(ctx, makeVersion(clusterversion.V23_1_SystemRbrReadNew))
	...
	t.Run("setting_changed_after_initialization", func(t *testing.T) {
		...
		settings := cluster.MakeClusterSettings()
		settings.Version.SetActiveVersion(ctx, makeVersion(clusterversion.V22_2))
		...
		settings.Version.SetActiveVersion(ctx, makeVersion(clusterversion.V23_1_SystemRbrDualWrite))
		waitForIdle(t, stopper)
		requireCache(t, oldCache, cache)
