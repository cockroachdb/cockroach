...
func TestInstanceProvider(t *testing.T) {
	...
	// Set heartbeat duration to a high enough value so that we
	// can check for the session expiry callback mechanism in this
	// test.
	//slinstance.DefaultHeartBeat.Override(ctx, &settings.SV, 2*expiration)
