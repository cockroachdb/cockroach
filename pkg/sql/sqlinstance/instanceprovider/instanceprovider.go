...
func (p *provider) Instance(ctx context.Context) (_ base.SQLInstanceID, err error) {
	...
	p.mu.initOnce.Do(func() {
		...
		session, e := p.mu.session.Session(ctx)
		if e != nil {
			err = e
			return
		}
		instanceID, e := p.storage.CreateInstance(ctx, session.ID(), p.mu.instanceAddr)
		if e != nil {
			err = e
			return
		}
	...
	log.Ops.Infof(ctx, "created SQL instance %d", p.mu.instanceInfo.InstanceID())
