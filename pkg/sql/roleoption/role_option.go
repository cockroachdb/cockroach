...
var toSQLStmts = map[Option][]string{
	CREATEROLE: {`DELETE FROM system.role_options WHERE username = $1 AND option = 'NOCREATEROLE'`,
		`UPSERT INTO system.role_options (username, option) VALUES ($1, 'CREATEROLE')`},
	NOCREATEROLE: {`DELETE FROM system.role_options WHERE username = $1 AND option = 'CREATEROLE'`,
		`UPSERT INTO system.role_options (username, option) VALUES ($1, 'NOCREATEROLE')`},
	LOGIN: {`DELETE FROM system.role_options WHERE username = $1 AND option = 'NOLOGIN'`,
		`UPSERT INTO system.role_options (username, option) VALUES ($1, 'LOGIN')`},
	NOLOGIN: {`DELETE FROM system.role_options WHERE username = $1 AND option = 'LOGIN'`,
		`UPSERT INTO system.role_options (username, option) VALUES ($1, 'NOLOGIN')`},
...
func (rol List) GetSQLStmts() (map[string]func() (bool, string, error), error) {
	...
	for _, ro := range rol {
		...
		optionStmts := toSQLStmts[ro.Option]
		for _, stmt := range optionStmts {
			if ro.HasValue {
				stmts[stmt] = ro.Value
			} else {
				stmts[stmt] = nil
