...
var toSQLStmts = map[Option]string{
	CREATEROLE:             `UPSERT INTO system.role_options (username, option, id) VALUES ($1, 'CREATEROLE', $2)`,
	NOCREATEROLE:           `DELETE FROM system.role_options WHERE username = $1 AND id = $2 AND option = 'CREATEROLE'`,
	LOGIN:                  `DELETE FROM system.role_options WHERE username = $1 AND id = $2 AND option = 'NOLOGIN'`,
	NOLOGIN:                `UPSERT INTO system.role_options (username, option, id) VALUES ($1, 'NOLOGIN', $2)`,
	VALIDUNTIL:             `UPSERT INTO system.role_options (username, option, value, id) VALUES ($1, 'VALID UNTIL', $3::timestamptz::string, $2)`,
	CONTROLJOB:             `UPSERT INTO system.role_options (username, option, id) VALUES ($1, 'CONTROLJOB', $2)`,
	NOCONTROLJOB:           `DELETE FROM system.role_options WHERE username = $1 AND id = $2 AND option = 'CONTROLJOB'`,
	CONTROLCHANGEFEED:      `UPSERT INTO system.role_options (username, option, id) VALUES ($1, 'CONTROLCHANGEFEED', $2)`,
	NOCONTROLCHANGEFEED:    `DELETE FROM system.role_options WHERE username = $1 AND id = $2 AND option = 'CONTROLCHANGEFEED'`,
	CREATEDB:               `UPSERT INTO system.role_options (username, option, id) VALUES ($1, 'CREATEDB', $2)`,
	NOCREATEDB:             `DELETE FROM system.role_options WHERE username = $1 AND id = $2 AND option = 'CREATEDB'`,
	CREATELOGIN:            `UPSERT INTO system.role_options (username, option, id) VALUES ($1, 'CREATELOGIN', $2)`,
	NOCREATELOGIN:          `DELETE FROM system.role_options WHERE username = $1 AND id = $2 AND option = 'CREATELOGIN'`,
	VIEWACTIVITY:           `UPSERT INTO system.role_options (username, option, id) VALUES ($1, 'VIEWACTIVITY', $2)`,
	NOVIEWACTIVITY:         `DELETE FROM system.role_options WHERE username = $1 AND id = $2 AND option = 'VIEWACTIVITY'`,
	CANCELQUERY:            `UPSERT INTO system.role_options (username, option, id) VALUES ($1, 'CANCELQUERY', $2)`,
	NOCANCELQUERY:          `DELETE FROM system.role_options WHERE username = $1 AND id = $2 AND option = 'CANCELQUERY'`,
	MODIFYCLUSTERSETTING:   `UPSERT INTO system.role_options (username, option, id) VALUES ($1, 'MODIFYCLUSTERSETTING', $2)`,
	NOMODIFYCLUSTERSETTING: `DELETE FROM system.role_options WHERE username = $1 AND id = $2 AND option = 'MODIFYCLUSTERSETTING'`,
	SQLLOGIN:               `DELETE FROM system.role_options WHERE username = $1 AND id = $2 AND option = 'NOSQLLOGIN'`,
	NOSQLLOGIN:             `UPSERT INTO system.role_options (username, option, id) VALUES ($1, 'NOSQLLOGIN', $2)`,
	VIEWACTIVITYREDACTED:   `UPSERT INTO system.role_options (username, option, id) VALUES ($1, 'VIEWACTIVITYREDACTED', $2)`,
	NOVIEWACTIVITYREDACTED: `DELETE FROM system.role_options WHERE username = $1 AND id = $2 AND option = 'VIEWACTIVITYREDACTED'`,
	VIEWCLUSTERSETTING:     `UPSERT INTO system.role_options (username, option, id) VALUES ($1, 'VIEWCLUSTERSETTING', $2)`,
	NOVIEWCLUSTERSETTING:   `DELETE FROM system.role_options WHERE username = $1 AND id = $2 AND option = 'VIEWCLUSTERSETTING'`,
}
