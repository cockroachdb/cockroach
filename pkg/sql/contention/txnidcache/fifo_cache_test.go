	...
	datadriven.Walk(t, testutils.TestDataPath(t), func(t *testing.T, path string) {
		datadriven.RunTest(t, path, func(t *testing.T, d *datadriven.TestData) string {
			...
			case "newInputBlock":
				...
				var result []string
				result = append(result, fmt.Sprintf("blockSize: %d", actualSize))
				for blockOffset := 0; blockOffset < blockSize; blockOffset++ {
					if !input[blockOffset].Valid() {
						break
					}
					result = append(result,
						fmt.Sprintf("txnID: %s, txnFingerprintID: %d",
							input[blockOffset].TxnID.String(), input[blockOffset].TxnFingerprintID))
				}
			...
			case "show":
				...
				for cur := cache.mu.eviction.head; cur != nil; cur = cur.next {
					blockName, ok := blockToNameMap[cur.block]
					require.True(t, ok, "expected to found an inserted block, but it's missing")
					...
					for blockOffset := 0; blockOffset < blockSize; blockOffset++ {
						...
						actualBlockSize++
						txnIDs = append(txnIDs, cur.block[blockOffset].TxnID)
					}

					result = append(result,
						fmt.Sprintf("blockIdx: %d, blockName: %s, blockSize: %d",
							blockIdx, blockName, actualBlockSize))

					blockIdx++
				}

				for _, txnID := range txnIDs {
					txnFingerprintIDs, ok := cache.mu.data[txnID]
					require.True(t, ok, "expected to find txn fingerprint ID "+
						"for txnID %s, but it was not found", txnID.String())

					result = append(result, fmt.Sprintf("txnID: %s, txnFingerprintID: %d",
						txnID.String(), txnFingerprintIDs))
				}
