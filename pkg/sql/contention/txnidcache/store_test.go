...
func TestShardedStore(t *testing.T) {
	...
	inputs, expectedMaps := generateMultipleBlocks()
	for i := range inputs {
...
func generateMultipleBlocks() (
	blocks []messageBlock,
	expectedMaps []map[uuid.UUID]roachpb.TransactionFingerprintID,
) {
	// Generate a random block count from the half open interval [1, 21).
	testBlockCount := rand.Intn(20) + 1
	blocks = make([]messageBlock, 0, testBlockCount)
	expectedMaps = make([]map[uuid.UUID]roachpb.TransactionFingerprintID, 0, testBlockCount)
