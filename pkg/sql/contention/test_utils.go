...
type fakeStatusServerCluster map[int32]*fakeStatusServer

func newFakeStatusServerCluster() fakeStatusServerCluster {
	return make(fakeStatusServerCluster)
}

func (f fakeStatusServerCluster) getStatusServer(nodeID int32) *fakeStatusServer {
	statusServer, ok := f[nodeID]
	if !ok {
		statusServer = newFakeStatusServer()
		f[nodeID] = statusServer
	}
	return statusServer
}
...
func (f fakeStatusServerCluster) txnIDResolution(
	...
	return f.getStatusServer(req.CoordinatorNodeID).txnIDResolution(ctx, req)
}

func (f fakeStatusServerCluster) setTxnIDEntry(
	coordinatorNodeID int32, txnID uuid.UUID, txnFingerprintID roachpb.TransactionFingerprintID,
) {
	f.getStatusServer(coordinatorNodeID).setTxnIDEntry(txnID, txnFingerprintID)
}

func (f fakeStatusServerCluster) setStatusServerError(coordinatorNodeID int32, err error) {
