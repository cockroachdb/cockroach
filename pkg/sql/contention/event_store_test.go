...
func TestEventStore(t *testing.T) {
	...
	input, expected := generateUnresolvedContentionEventsFromTestData(t, testCases)
...
func BenchmarkEventStoreIntake(b *testing.B) {
	...
	st := cluster.MakeTestingClusterSettings()
	// TxnIDResolutionInterval.Override(ctx, &st.SV, time.Millisecond*300)
	statusServer := newFakeStatusServerCluster()
	...
	run := func(b *testing.B, store *eventStore, numOfConcurrentWriter int) {
		...
		for writerIdx := 0; writerIdx < numOfConcurrentWriter; writerIdx++ {
			...
			go func(writerIdx int) {
				...
				inputIdx := numOfOps * writerIdx
				for i := 0; i < numOfOps; i++ {
					store.addEvent(input[inputIdx+i])
				}
			}(writerIdx)
		}
...
func randomlyGenerateTestCases(testSize int, numOfCoordinator int) []testData {
