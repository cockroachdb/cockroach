...
func TestLogResolvedEvents(t *testing.T) {
	...
	fingerprintIDToHex := func(id uint64) redact.SafeString {
		return redact.SafeString("\\x" + sqlstatsutil.EncodeStmtFingerprintIDToString(appstatspb.StmtFingerprintID(id)))
	}
	txnFingerprintIDToHex := func(id uint64) redact.SafeString {
		return redact.SafeString("\\x" + sqlstatsutil.EncodeTxnFingerprintIDToString(appstatspb.TransactionFingerprintID(id)))
	}
