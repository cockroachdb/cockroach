...
func TestLogResolvedEvents(t *testing.T) {
	...
	for _, log := range contentionLogs {
		...
		if log.Duration == 300000000 {
			require.Equal(t, 123, int(log.WaitingStmtFingerprintId))
			require.Equal(t, 456, int(log.WaitingTxnFingerprintId))
			require.Equal(t, 789, int(log.BlockingTxnFingerprintId))
		...
		if log.Duration == 50000000 {
			require.Equal(t, 321, int(log.WaitingStmtFingerprintId))
			require.Equal(t, 456, int(log.WaitingTxnFingerprintId))
			require.Equal(t, 789, int(log.BlockingTxnFingerprintId))
			require.Equal(t, "‹test-key-1›", log.ContendedKey)
		}
		if log.Duration == 20000000 {
			require.Equal(t, 123, int(log.WaitingStmtFingerprintId))
			require.Equal(t, 456, int(log.WaitingTxnFingerprintId))
			require.Equal(t, 888, int(log.BlockingTxnFingerprintId))
			require.Equal(t, "‹test-key-1›", log.ContendedKey)
		}
	}
}
