...
func TestResolver(t *testing.T) {
	...
	t.Run("normal_resolution", func(t *testing.T) {
		...
		input, expected := generateUnresolvedContentionEventsFromTestCases(t, tcs)
	...
	t.Run("retry_after_encountering_provisional_value", func(t *testing.T) {
		...
		coordinatorID, err := strconv.Atoi(newTxn.coordinatorNodeID)
		require.NoError(t, err)
		input[0].Event.TxnMeta.CoordinatorNodeID = int32(coordinatorID)
		expected[1].Event.TxnMeta.ID = newTxn.TxnID
		expected[1].Event.TxnMeta.CoordinatorNodeID = int32(coordinatorID)
	...
	t.Run("handle_transient_rpc_failure", func(t *testing.T) {
		...
		input2, expected2 := generateUnresolvedContentionEventsFromTestCases(t, tcs)
