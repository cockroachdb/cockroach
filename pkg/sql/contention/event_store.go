...
type contentionKey struct {
	waitingStmtFingerprintID uint64
	waitingTxnFingerprintID  uint64
	blockingTxnFingerprintID uint64
	contendedKey             string
}
...
func logResolvedEvents(ctx context.Context, events []contentionpb.ExtendedContentionEvent) {
	...
	for i := range events {
		...
		key := contentionKey{
			waitingStmtFingerprintID: uint64(event.WaitingStmtFingerprintID),
			waitingTxnFingerprintID:  uint64(event.WaitingTxnFingerprintID),
			blockingTxnFingerprintID: uint64(event.BlockingTxnFingerprintID),
	...
	for key, duration := range eventsAggregated {
		...
		event := &eventpb.AggregatedContentionInfo{
			WaitingStmtFingerprintId: key.waitingStmtFingerprintID,
			WaitingTxnFingerprintId:  key.waitingTxnFingerprintID,
			BlockingTxnFingerprintId: key.blockingTxnFingerprintID,
