...
func logResolvedEvents(ctx context.Context, events []contentionpb.ExtendedContentionEvent) {
	...
	for key, duration := range eventsAggregated {
		...
		event := &eventpb.AggregatedContentionInfo{
			WaitingStmtFingerprintId: redact.SafeString("\\x" + sqlstatsutil.EncodeStmtFingerprintIDToString(key.waitingStmtFingerprintID)),
			WaitingTxnFingerprintId:  redact.SafeString("\\x" + sqlstatsutil.EncodeTxnFingerprintIDToString(key.waitingTxnFingerprintID)),
			BlockingTxnFingerprintId: redact.SafeString("\\x" + sqlstatsutil.EncodeTxnFingerprintIDToString(key.blockingTxnFingerprintID)),
			ContendedKey:             redact.RedactableString(key.contendedKey),
