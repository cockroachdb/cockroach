...
func logResolvedEvents(
	...
	for _, event := range events {
		...
		if agg, ok := aggregated[key]; ok {
			...
			// First time seeing this key combination - decode the key.
			var decodedInfo *keydecoder.DecodedKeyInfo
			if db != nil {
				keyToDecode, _, _ := keys.DecodeTenantPrefix(event.BlockingEvent.Key)
				decodedInfo = decodeKey(ctx, db, codec, keyToDecode)
			}
...
// decodeKey decodes a SQL key into human-readable information.
// Returns nil if the key cannot be decoded.
func decodeKey(
	ctx context.Context, db descs.DB, codec keys.SQLCodec, key roachpb.Key,
) *keydecoder.DecodedKeyInfo {
	var result *keydecoder.DecodedKeyInfo
	err := db.DescsTxn(ctx, func(ctx context.Context, txn descs.Txn) error {
		...
		result, err = keydecoder.DecodeKey(ctx, codec, txn.Descriptors(), txn.KV(), nil, key)
		return err
	})
	if err != nil {
		if log.V(2) {
