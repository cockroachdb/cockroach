...
func NewMetrics() Metrics {
	return Metrics{
		...
		ResolverRetries: metric.NewCounter(metric.Metadata{
			...
			Help:        "Number of times the txn id resolution is retried",
			Measurement: "Retry count",
			Unit:        metric.Unit_COUNT,
		}),
		ResolverFailed: metric.NewCounter(metric.Metadata{
			Name:        "sql.contention.resolver.failed_resolution",
			Help:        "Number of failed transaction ID resolution",
			Measurement: "Failed txn ID resolution count",
			Unit:        metric.Unit_COUNT,
		}),
	}
}
