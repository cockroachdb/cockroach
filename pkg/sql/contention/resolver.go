...
const (
	...
	// scenarios:
	// 1. the txnID entry is permanently lost. This can happen due to TxnID cache
	//    eviction (very not ideal situation) or data corruption (hopefully it
	//    never happens).
	// 2. the txnID is in the concurrentWriteBuffer of the remote node and
	...
	for {
		...
		for _, event := range currentBatch {
			if txnFingerprintID, ok := resolvedTxnIDs[event.Event.TxnMeta.ID]; ok {
				...
				// TODO(azhng): wip: what happen if the txnID runs so long that
				//  the provisional entry gets evicted?
				//  we can handle this in the txnID Cache by not allowing evicting old
				//  entries.
				...
				// TODO(azhng): wip: plumb down the metrics here for discard.
				_ = s.maybeRequeueEventForRetryLocked(event, retryBudgetForMissingResult)
			}
		}
