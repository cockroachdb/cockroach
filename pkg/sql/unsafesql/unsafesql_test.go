	...
	t.Run("returns the right response with the right session data", func(t *testing.T) {
		for _, test := range []struct {
			...
			t.Run(fmt.Sprintf("%t", test), func(t *testing.T) {
				...
				query := redact.Sprint("test query")
				err := unsafesql.CheckInternalsAccess(ctx, &sessiondata.SessionData{
...
func TestAccessCheckServer(t *testing.T) {
	...
	spy := logtestutils.NewStructuredLogSpy[eventpb.UnsafeInternalsAccess](
		t, []logpb.Channel{logpb.Channel_SQL_EXEC}, nil, logtestutils.FromLogEntry,
	)
