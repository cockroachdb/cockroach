...
func (n *alterTableSetLocalityNode) alterTableLocalityNonRegionalByRowToRegionalByRow(
	...
	if err != nil {
		// Return the error if we get anything other than "undefined column" back
		// and we were actually looking for a column.
		if !(sqlerrors.IsUndefinedColumnError(err) && mayNeedImplicitCRDBRegionCol) {
			return err
		}
