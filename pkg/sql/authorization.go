...
func MemberOfWithAdminOption(
	...
	// If `txn` is in a retry, do not launch the singleflight to populate the
	// cache because it can potentially cause a deadlock (see
	// TestConcurrentGrants/concurrent-GRANTs-high-priority for a repro) and
	// instead just issue a read using `txn` to `system.role_members` table to
	// populate the cache.
	if txn.KV().Epoch() > 0 {
		return resolveMemberOfWithAdminOption(ctx, member, txn, useSingleQueryForRoleMembershipCache.Get(execCfg.SV()))
	}
