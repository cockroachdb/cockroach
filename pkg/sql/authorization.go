...
func (p *planner) HasPrivilege(
	...
	if d, ok := privilegeObject.(catalog.Descriptor); ok && catalog.IsSystemDescriptor(d) {
		...
		if err := CheckUnsafeInternalsAccess(ctx, actx, sd, p.stmt); err != nil {
			return false, err
		}
	}
...
// CheckUnsafeInternalsAccess checks whether a querier is allowed to
// access unsafe internals like the system database or crdb_internal
// objects.
func CheckUnsafeInternalsAccess(
