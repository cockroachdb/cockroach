...
func (p *planner) HasPrivilege(
	...
	if p.objectIsUnsafe(ctx, privilegeObject) {
		allowUnsafeOverride := p.ExecCfg().EvalContextTestingKnobs.AllowInternalAccess
		if err := unsafesql.CheckInternalsAccess(ctx, p.SessionData(), p.stmt.AST, p.extendedEvalCtx.Annotations, &p.ExecCfg().Settings.SV, allowUnsafeOverride); err != nil {
			return false, err
		}
	}
