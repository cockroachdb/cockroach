...
func (p *planner) CheckPrivilege(
	...
	return CheckPrivilegeForUser(p.evalCtx.SessData.User, descriptor, privilege)
}
...
func (p *planner) CheckAnyPrivilege(descriptor sqlbase.DescriptorProto) error {
	...
	if descriptor.GetPrivileges().AnyPrivilege(p.evalCtx.SessData.User) {
		return nil
	}

	return fmt.Errorf("user %s has no privileges on %s %s",
		p.evalCtx.SessData.User, descriptor.TypeName(), descriptor.GetName())
}

// RequireSuperUser implements the AuthorizationAccessor interface.
func (p *planner) RequireSuperUser(action string) error {
	if p.evalCtx.SessData.User != security.RootUser && p.evalCtx.SessData.User != security.NodeUser {
