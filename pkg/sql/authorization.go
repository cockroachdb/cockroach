...
func (p *planner) canCreateOnSchema(
	...
	resolvedSchema, err := resolver.ResolveSchemaDescByID(ctx, p.Txn(), p.ExecCfg().Codec, schemaID)
	if err != nil {
		return err
	}
	...
	case sqlbase.SchemaTemporary, sqlbase.SchemaVirtual:
		scName, err := resolver.ResolveSchemaNameByID(ctx, p.Txn(), p.ExecCfg().Codec, dbID, schemaID)
		if err != nil {
			return err
		}
		return pgerror.Newf(pgcode.InsufficientPrivilege,
			"cannot CREATE on schema %s", scName)
