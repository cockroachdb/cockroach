...
func makeAllRelationsVirtualTableWithDescriptorIDIndex(
	...
	isIncludingCompositeTypes := populateFromType != nil
				...
				populate: func(ctx context.Context, unwrappedConstraint tree.Datum, p *planner, db catalog.DatabaseDescriptor,
					...
					if err != nil {
						if errors.Is(err, catalog.ErrDescriptorNotFound) ||
							...
							// the catalog table being populated, we should convert the user-defined OID into a
							// descriptor ID.
							if isIncludingCompositeTypes {
								maybeId := catid.UserDefinedOIDToID(oid.Oid(id))
								// We are for sure not looking at a user-defined OID
								if maybeId == catid.InvalidDescID {
									return !IsMaybeHashedOid(oid.Oid(id)), nil
								}
								id = maybeId
								desc, err = p.byIDGetterBuilder().WithoutNonPublic().Get().Desc(ctx, id)
								if err != nil {
									// This is under the assumption that the OIDs for user-defined "things" are not hashed.
									return false, err
								}
							} else {
					...
					// If the descriptor is not a table or a type, then we have a complete result
					...
					// the ID being queried is *not* a table or a type.
					var tableDesc catalog.TableDescriptor
					var typeDesc catalog.TypeDescriptor
					switch desc.DescriptorType() {
					case catalog.Table:
						tableDesc = desc.(catalog.TableDescriptor)
						return populateVirtualIndexForTable(ctx, p, db, tableDesc, addRow, populateFromTable)
					case catalog.Type:
						if !isIncludingCompositeTypes {
							return true, nil
						}
						typeDesc = desc.(catalog.TypeDescriptor)
						return populateVirtualIndexForType(ctx, p, db, typeDesc, addRow, populateFromType)
					default:
						return true, nil
					}
				},
