		...
		for name, builtins := range parser.Builtins {
			...
			for _, builtin := range builtins {
				...
				if builtin.ReturnType != nil {
					oid, ok := DatumToOid(builtin.ReturnType)
					if !ok {
						return errors.Errorf("could not find referenced return type %v on builtin %s",
							builtin.ReturnType, name)
					}
				...
				for i, argType := range argTypes.Types() {
					dArgType, ok := DatumToOid(argType)
					if !ok {
						return errors.Errorf("could not find referenced argument type %v on builtin %s",
							argType, name)
					}
				...
				case parser.VariadicType:
					...
					oid, ok := DatumToOid(argType)
					if !ok {
						return errors.Errorf("could not find referenced variadic type %v on builtin %s",
							argType, name)
					}
