...
`,
	populate: func(ctx context.Context, p *planner, prefix *DatabaseDescriptor, addRow func(...tree.Datum) error) error {
		h := makeOidHasher()
		// TODO(knz): this can be optimized by having an alternate
		// forEachTableDesc which does not iterate over virtual schemas --
		// virtual schemas cannot contain indexes.
		return forEachTableDesc(ctx, p, prefix, func(db *sqlbase.DatabaseDescriptor, scName string, table *sqlbase.TableDescriptor) error {
...
`,
	populate: func(ctx context.Context, p *planner, prefix *DatabaseDescriptor, addRow func(...tree.Datum) error) error {
		...
		return forEachTableDesc(ctx, p, prefix, func(db *sqlbase.DatabaseDescriptor, scName string, desc *sqlbase.TableDescriptor) error {
			if !desc.IsView() {
				return nil
			}
