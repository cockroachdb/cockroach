				...
				if !ok || len(privs.Users) == 0 {
					...
					case tree.Tables:
						if !defaultPrivilegesForRole.IsExplicitRole() ||
							catprivilege.GetRoleHasAllPrivilegesOnTargetObject(&defaultPrivilegesForRole, tree.Tables) {
							continue
						}
					case tree.Sequences:
						if !defaultPrivilegesForRole.IsExplicitRole() ||
							catprivilege.GetRoleHasAllPrivilegesOnTargetObject(&defaultPrivilegesForRole, tree.Sequences) {
							continue
						}
					case tree.Schemas:
						if !defaultPrivilegesForRole.IsExplicitRole() ||
							catprivilege.GetRoleHasAllPrivilegesOnTargetObject(&defaultPrivilegesForRole, tree.Schemas) {
							continue
						}
					case tree.Types:
						if (!defaultPrivilegesForRole.IsExplicitRole() ||
