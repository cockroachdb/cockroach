	...
	indexes: []virtualIndex{
		{
			...
			populate: func(ctx context.Context, constraint tree.Datum, p *planner, db *sqlbase.ImmutableDatabaseDescriptor,
				...
				usrT, ok := lCtx.typDescs[sqlbase.ID(types.UserDefinedTypeOIDToID(ooid))]
				if ok {
					usrTyp, err := usrT.MakeTypesT(
						tree.NewUnqualifiedTypeName(tree.Name(usrT.GetName())),
						p.makeTypeLookupFn(ctx),
					)
					if err != nil {
						return false, err
					...
					if err := addPGTypeRow(h, nspOid, usrTyp, addRow); err != nil {
						return false, err
					}
					return true, nil
				}
