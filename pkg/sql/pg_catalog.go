...
https://www.postgresql.org/docs/13/catalog-pg-default-acl.html`,
	...
	populate: func(ctx context.Context, p *planner, dbContext catalog.DatabaseDescriptor, addRow func(...tree.Datum) error) error {
		...
		privilegeWithID := func(schemaID descpb.ID) func(defaultPrivilegesForRole catpb.DefaultPrivilegesForRole) error {
			...
			return func(defaultPrivilegesForRole catpb.DefaultPrivilegesForRole) error {
				...
				for _, objectType := range objectTypes {
					...
					if defaultPrivilegesForRole.IsExplicitRole() {
						...
						default:
							if len(privs.Users) == 0 && schemaID != descpb.ID(0) {
								continue
							}
						}

						if objectType == privilege.Types || objectType == privilege.Routines {
							if publicHasUsage {
							...
							} else if len(privs.Users) == 0 && schemaID != descpb.ID(0) {
								continue
							}
						}
					}
		...
		err := dbContext.ForEachSchema(func(id descpb.ID, name string) error {
			...
			err = schemaDescriptor.GetDefaultPrivilegeDescriptor().ForEachDefaultPrivilegeForRole(privilegeWithID(id))
		...
		return dbContext.GetDefaultPrivilegeDescriptor().ForEachDefaultPrivilegeForRole(privilegeWithID(0))
	},
}
