		...
		indexes: []virtualIndex{
			{
				...
				populate: func(ctx context.Context, constraint tree.Datum, p *planner, db *dbdesc.ImmutableDatabaseDescriptor,
					...
					scName, err := resolver.ResolveSchemaNameByID(ctx, p.txn, p.ExecCfg().Codec, db.GetID(), table.GetParentSchemaID())
					if err != nil {
						return false, err
					}
					if err := populateFromTable(ctx, p, h, db, scName, table, scResolver,
						addRow); err != nil {
						return false, err
					}
					return true, nil
