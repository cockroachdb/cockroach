...
func addPgProcBuiltinRow(nspOid *tree.DOid, addRow func(...tree.Datum) error) error {
...
https://www.postgresql.org/docs/9.5/catalog-pg-proc.html`,
	...
	populate: func(ctx context.Context, p *planner, dbContext catalog.DatabaseDescriptor, addRow func(...tree.Datum) error) error {
		...
		err := forEachDatabaseDesc(ctx, p, dbContext, false, /* requiresPrivileges */
			func(db catalog.DatabaseDescriptor) error {
				nspOid := tree.NewDOid(catconstants.PgCatalogID)
				err := addPgProcBuiltinRow(nspOid, addRow)
				if err != nil {
					return err
				}
				return nil
	...
	indexes: []virtualIndex{
		{
			...
			populate: func(ctx context.Context, unwrappedConstraint tree.Datum, p *planner, dbContext catalog.DatabaseDescriptor,
				...
				_, overload, err := p.ResolveFunctionByOID(ctx, ooid)
				if err != nil {
				...
				if overload.IsUDF {
					...
					err := addPgProcBuiltinRow(tree.NewDOid(catconstants.PgCatalogID),
						addRow)
					if err != nil {
						return false, err
					}
