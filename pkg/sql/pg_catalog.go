...
func populateTableConstraints(
	...
	for conName, con := range conInfo {
		...
		case descpb.ConstraintTypeUnique:
			...
			} else if con.UniqueConstraint != nil {
				oid = h.UniqueWithoutIndexConstraintOid(
					db.GetID(), scName, table.GetID(), con.UniqueConstraint,
				)
				f.WriteString("UNIQUE WITHOUT INDEX (")
				colNames, err := table.NamesForColumnIDs(con.UniqueConstraint.ColumnIDs)
				if err != nil {
					return err
				}
...
func (h oidHasher) writeUniqueConstraint(uc *descpb.UniqueConstraint) {
...
func (h oidHasher) UniqueWithoutIndexConstraintOid(
	dbID descpb.ID, scName string, tableID descpb.ID, uc *descpb.UniqueConstraint,
) *tree.DOid {
