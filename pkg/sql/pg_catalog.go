...
var pgCatalogAttrDefTable = makeAllRelationsPGCatalogTable(
...
CREATE TABLE pg_catalog.pg_attrdef (
  ...
  INDEX(adrelid)
	...
	virtualMany, false, /* containsIndexOids */
...
var pgCatalogAttributeTable = makeAllRelationsPGCatalogTable(
...
CREATE TABLE pg_catalog.pg_attribute (
  ...
  INDEX(attrelid)
	...
	virtualMany, true, /* containsIndexOids */
	func(ctx context.Context, p *planner, h oidHasher, db *DatabaseDescriptor, scName string,
		table *sqlbase.TableDescriptor,
		lookup simpleSchemaResolver,
		addRow func(...tree.Datum) error) error {
...
var pgCatalogClassTable = makeAllRelationsPGCatalogTable(
...
// gotcha to look out for is that some tables do not have entries in
// system.descriptor, such as other virtual tables.
func makeAllRelationsPGCatalogTable(
	comment string,
	schemaDef string,
	virtualOpts virtualOpts,
	indexesAreNotComplete bool,
		...
		indexes: []virtualIndex{
			{
				...
				populate: func(ctx context.Context, constraint tree.Datum, p *planner, db *DatabaseDescriptor,
					...
					// Don't include tables that aren't in the database context or that
					// aren't visible
					if table.Desc.ParentID != db.ID || table.Desc.Dropped() ||
...
var pgCatalogConstraintTable = makeAllRelationsPGCatalogTable(
