...
func populateDomainConstraints(
	...
	for _, ck := range d.CheckConstraints {
		conoid := h.DomainConstraintOid(typOid, ck.Name)
		consrc := tree.NewDString(fmt.Sprintf("(%s)", ck.Expr))
		condef := tree.NewDString(fmt.Sprintf("CHECK ((%s))", ck.Expr))
		if err := addRow(
			conoid,               // oid
	...
	if d.NotNull {
		conoid := h.DomainConstraintOid(typOid, d.NotNullConstraintName)
...
func (h oidHasher) DomainConstraintOid(typeOid oid.Oid, constraintName string) *tree.DOid {
	h.writeTypeTag(domainConstraintTypeTag)
	h.writeUInt32(uint32(typeOid))
	h.writeStr(constraintName)
	return h.getOid()
}
