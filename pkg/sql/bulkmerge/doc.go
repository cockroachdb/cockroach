...
// backfill and IMPORT operations. It replaces the gateway-based merge
// approach (see pkg/sql/backfill.IndexBackfillMerger) with a
// MapReduce-inspired DistSQL flow that distributes merge work across all
// SQL instances in the cluster.
//
// # Motivation
//
// The traditional (gateway) merge approach works well when the backfill
...
//
// The distributed merge pipeline and the gateway merge
// (pkg/sql/backfill.IndexBackfillMerger) are alternative implementations
// of the same logical step. The choice is made by the cluster setting
// bulkio.index_backfill.distributed_merge.mode:
//
//   - "disabled" (default): Always use the gateway merge.
//   - "enabled": Use distributed merge when the backfill data is
//     unsorted; fall back to gateway merge when sorted (the sorted-data
//     optimization checks IsBackfillDataSorted).
//   - "legacy": Intended to enable only for the legacy schema changer,
//     but the legacy schema changer's distributed merge execution was
//     never implemented. The mode value is accepted and stored in job
//     details, but has no practical effect.
