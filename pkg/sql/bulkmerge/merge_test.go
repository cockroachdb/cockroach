...
func testMergeProcessors(
	...
	require.Eventually(t, func() bool {
		...
		return err == nil && len(sqlInstanceIDs) == expectedInstanceCount
	}, 30*time.Second, 100*time.Millisecond, "timed out waiting for %d nodes to be ready for DistSQL", expectedInstanceCount)
	defer cleanup()
	...
	require.Equalf(t, expectedInstanceCount, len(foundInstances),
		"expected %d instances but found instances: %v",
		expectedInstanceCount, foundInstances)
}
