...
func comparePrivileges(
	...
	computePrivilegeChanges := func(prev, cur *descpb.UserPrivileges, username string) (granted, revoked []string) {
		...
		if curPrivileges, ok := curUserMap[username]; ok {
			if prev.Privileges != curPrivileges.Privileges {
				...
				for _, priv := range privilege.ListFromBitField(curPrivileges.Privileges, objectType).SortedNames() {
	...
	for i := range prevUserPrivileges {
		...
		granted, revoked := computePrivilegeChanges(prev, cur, username)
