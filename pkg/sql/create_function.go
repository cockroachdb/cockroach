...
func (n *createFunctionNode) createNewFunction(
	...
	argTypes := make([]*types.T, len(udfDesc.Params))
	for i, arg := range udfDesc.Params {
		argTypes[i] = arg.Type
	}
	scDesc.AddFunction(
		udfDesc.GetName(),
...
func (n *createFunctionNode) getMutableFuncDesc(
	...
	for i, param := range n.cf.Params {
		if param.Name != "" {
			if _, ok := argNameSeen[param.Name]; ok {
...
func makeFunctionArg(
	...
	pbParam.Type, err = tree.ResolveType(ctx, arg.Type, typeResolver)
	if err != nil {
		return descpb.FunctionDescriptor_Parameter{}, err
	}
