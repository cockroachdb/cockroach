...
type PartitionedQueue interface {
	...
	Enqueue(ctx context.Context, partitionIdx int, batch coldata.Batch)
	...
	Dequeue(ctx context.Context, partitionIdx int, batch coldata.Batch)
	...
	CloseAllOpenWriteFileDescriptors(ctx context.Context)
	...
	CloseAllOpenReadFileDescriptors()
	...
	CloseInactiveReadPartitions(ctx context.Context)
...
func (p *PartitionedDiskQueue) closeWritePartition(
	...
	p.partitions[idx].Enqueue(ctx, coldata.ZeroBatch)
...
func (p *PartitionedDiskQueue) closeReadPartition(idx int) {
...
func (p *PartitionedDiskQueue) Enqueue(ctx context.Context, partitionIdx int, batch coldata.Batch) {
	var err error
	defer func() {
		if err != nil {
	...
	if !ok {
		...
		if p.strategy == PartitionerStrategyCloseOnNewPartition && p.lastEnqueuedPartitionIdx != -1 {
			...
			if !found {
				...
				err = errors.AssertionFailedf("PartitionerStrategyCloseOnNewPartition unable to find last Enqueued partition")
				return
			}
			if p.partitions[idxToClose].state == partitionStateWriting {
				...
				p.closeWritePartition(ctx, idxToClose, retainFD)
	...
	if state := p.partitions[idx].state; state != partitionStateWriting {
		if state == partitionStatePermanentlyClosed {
			err = errors.AssertionFailedf("partition at index %d permanently closed, cannot Enqueue", partitionIdx)
			return
		}
		err = errors.AssertionFailedf("Enqueue illegally called after Dequeue or CloseAllOpenWriteFileDescriptors")
		return
	}
	p.lastEnqueuedPartitionIdx = partitionIdx
	p.partitions[idx].Enqueue(ctx, batch)
}
...
func (p *PartitionedDiskQueue) Dequeue(ctx context.Context, partitionIdx int, batch coldata.Batch) {
	idx, ok := p.partitionIdxToIndex[partitionIdx]
	if !ok {
		batch.SetLength(0)
		return
	...
	case partitionStateWriting:
		...
		p.closeWritePartition(ctx, idx, retainFD)
	...
	case partitionStatePermanentlyClosed:
		colexecerror.InternalError(errors.AssertionFailedf("partition at index %d permanently closed, cannot Dequeue", partitionIdx))
	default:
		colexecerror.InternalError(errors.AssertionFailedf("unhandled state %d", state))
	}
	notEmpty := p.partitions[idx].Dequeue(ctx, batch)
	if batch.Length() == 0 {
		// Queue is finished, release the acquired file descriptor.
		p.closeReadPartition(idx)
	}
	if !notEmpty {
...
func (p *PartitionedDiskQueue) CloseAllOpenWriteFileDescriptors(ctx context.Context) {
	for i, q := range p.partitions {
		if q.state != partitionStateWriting {
			continue
		}
		// closeWritePartition will Release the file descriptor.
		p.closeWritePartition(ctx, i, releaseFD)
	}
}
...
func (p *PartitionedDiskQueue) CloseInactiveReadPartitions(ctx context.Context) {
	for i, q := range p.partitions {
...
func (p *PartitionedDiskQueue) Close(ctx context.Context) {
	for i, q := range p.partitions {
