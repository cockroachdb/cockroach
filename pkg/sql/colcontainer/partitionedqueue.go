...
// still be read from, but never written to again. Note that releaseFD should
...
func (p *PartitionedDiskQueue) closeReadPartition(idx int) error {
	if p.partitions[idx].state != partitionStateReading {
		panic(fmt.Sprintf("illegal state change from %d to partitionStateClosedForReading, only partitionStateReading allowed", p.partitions[idx].state))
	}
	...
	if !ok {
		...
		if p.strategy == PartitionerStrategyCloseOnNewPartition && p.lastEnqueuedPartitionIdx != -1 {
			...
			if p.partitions[idxToClose].state == partitionStateWriting {
				...
				if err := p.closeWritePartition(idxToClose, false); err != nil {
					return err
				}
...
func (p *PartitionedDiskQueue) Dequeue(
	...
	case partitionStateWriting:
		...
		if err := p.closeWritePartition(idx, false); err != nil {
			return err
		}
	...
	if !notEmpty {
		...
		panic("DiskQueue unexpectedly returned that more data will be added")
	}
	return nil
}
...
func (p *PartitionedDiskQueue) CloseAllOpenWriteFileDescriptors() error {
	for i, q := range p.partitions {
		...
		if err := p.closeWritePartition(i, true); err != nil {
			return err
		}
	}
	return nil
