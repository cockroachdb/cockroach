...
func (d *diskQueue) Dequeue(ctx context.Context, b coldata.Batch) (bool, error) {
	...
	if dataToRead, err := d.maybeInitDeserializer(ctx); err != nil {
		...
		if d.deserializerState.curBatch == 0 {
			...
			for i := range vecs {
				// WHen we deserialize a new memory region, we create allocate a new
				// bitmap for the batch which deserializer will write to. This is due to
				...
				// converter to directly overwrite null bitmap of each column. Doing
				// this avoids reallocating a new memory columns every time we perform a
				// read from the file.
