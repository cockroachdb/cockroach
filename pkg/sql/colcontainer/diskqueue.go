...
// diskQueueState is a simple state that records the current state of the disk
// queue. Used to assert that an invalid state transition doesn't happen when
// a DiskQueue is in DiskQueueCacheMode{ClearAnd}ReuseCache.
...
// DiskQueuCacheMode comment as to how cfg.BufferSizeBytes is divided in each
// mode.
...
const (
	...
	// coldata.ZeroBatch is Enqueued, the cache will be release to the GC.
