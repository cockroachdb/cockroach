...
func (d *diskQueue) querySpilled(cfg DiskQueueCfg) {
	d.diskSpillingMetricsHelper.mu.Lock()
	defer d.diskSpillingMetricsHelper.mu.Unlock()
	if !d.diskSpillingMetricsHelper.mu.querySpilled {
		d.diskSpillingMetricsHelper.mu.querySpilled = true
		cfg.DistSQLMetrics.QueriesSpilled.Inc(1)
	}
}
