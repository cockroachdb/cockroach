...
func (d *diskQueue) Dequeue(ctx context.Context, b coldata.Batch) (bool, error) {
	...
	if dataToRead, err := d.maybeInitDeserializer(ctx); err != nil {
		...
		if d.deserializerState.curBatch == 0 {
			...
			for i := range vecs {
				// When we deserialize a new memory region, we allocate a new bitmap for
				// the batch which deserializer will write to. If we naively allow the
				// arrow batch converter to directly overwrite null bitmap of each
				// column, it could lead to memory corruption. Doing this avoids
