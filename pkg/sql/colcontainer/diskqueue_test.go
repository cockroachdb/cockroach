...
func TestDiskQueue(t *testing.T) {
	...
	for _, diskQueueReadMode := range []colcontainer.DiskQueueReadMode{colcontainer.DiskQueueReadModeDefault, colcontainer.DiskQueueReadModeRewindable} {
		for _, bufferSizeBytes := range []int{0, 16<<10 + rng.Intn(1<<20) /* 16 KiB up to 1 MiB */} {
			for _, maxFileSizeBytes := range []int{10 << 10 /* 10 KiB */, 1<<20 + rng.Intn(64<<20) /* 1 MiB up to 64 MiB */} {
				...
				if diskQueueReadMode == colcontainer.DiskQueueReadModeRewindable {
					dequeuedProbabilityBeforeAllEnqueuesAreDone = 0
					prefix, suffix = "Rewindable/", ""
				}
				t.Run(fmt.Sprintf("%sDiskQueueCacheMode=%d/AlwaysCompress=%t%s",
					prefix, diskQueueCacheMode, alwaysCompress, suffix), func(t *testing.T) {
					// Create random input.
					...
					queueCfg.ReadMode = diskQueueReadMode
					...
					numReadIterations := 1
					if diskQueueReadMode == colcontainer.DiskQueueReadModeRewindable {
						numReadIterations = 2 + rng.Intn(2)
					}
