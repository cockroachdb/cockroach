...
func (p *planner) EvalRoutineExpr(
	...
	if expr.TriggerFunc {
		...
		if limit := int(p.SessionData().TriggerDepthLimit); triggerDepth > limit {
			telemetry.Inc(sqltelemetry.TriggerDepthLimitReached)
			err = pgerror.Newf(pgcode.TriggeredActionException, "trigger depth limit (%d) reached", limit)
			return nil, err
		}
