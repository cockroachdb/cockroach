...
func (g *routineGenerator) startInternal(ctx context.Context, txn *kv.Txn) (err error) {
	...
	err = g.expr.ForEachPlan(ctx, ef, g.args, func(plan tree.RoutinePlan, isFinalPlan bool) error {
		...
		// Run the plan.
		return runPlanInsidePlan(ctx, g.p.RunParams(ctx), plan.(*planComponents), w, g)
	})
	if err != nil {
...
func (g *routineGenerator) handleException(ctx context.Context, err error) error {
	...
	if !g.p.Txn().IsOpen() {
		// It is not safe to catch an error unless the transaction is open.
		// TODO(drewk): some retryable errors allow the transaction to be rolled
	...
	for i, code := range exceptionHandler.Codes {
		if code == caughtCode {
			spErr := g.p.Txn().RollbackToSavepoint(ctx, blockState.SavepointTok)
			if spErr == nil {
