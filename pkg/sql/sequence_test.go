		...
		},
		{
           ...
           x INT UNIQUE,
			  ...
			  `)
				...
				// an underlying sequence named `demo_t_y_seq` for the y column of the
				// demo_t table. Such creation applies the sequence option (
				// START 2 INCREMENT 2 CACHE 5) in the statement.
				// The cache of demo_t_y_seq starts out empty. When the cache is empty,
				...
				// db:
				//  s: 10
				checkIntValue(t, 0, "SELECT nextval('demo_t_y_seq')", 2)
				checkIntValue(t, anySession(), "SELECT last_value FROM demo_t_y_seq", 10)
				...
				//  s: 10
				for sequenceNumber := 4; sequenceNumber <= 10; sequenceNumber += 2 {
					checkIntValue(t, 0, "SELECT nextval('demo_t_y_seq')", sequenceNumber)
				}
				checkIntValue(t, anySession(), "SELECT last_value FROM demo_t_y_seq", 10)
				...
				// db:
				//  s: 20
				checkIntValue(t, 0, "SELECT nextval('demo_t_y_seq')", 12)
				checkIntValue(t, anySession(), "SELECT last_value FROM demo_t_y_seq", 20)
