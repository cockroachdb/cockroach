...
func TestCachedSequences(t *testing.T) {
	...
	execStmt := func(statement string) {
		sqlSessions[anySession()].Exec(t, statement)
	}

	checkIntValue := func(session int, statement string, value int) {
		...
		// Test a cached sequences in a single session.
		{
			...
			test: func(t *testing.T) {
			  ...
			  `)
				...
				// db:
				//  s: 10
				checkIntValue(0, "SELECT nextval('s')", 2)
				checkIntValue(anySession(), "SELECT last_value FROM s", 10)
				...
				for sequenceNumber := 4; sequenceNumber <= 10; sequenceNumber += 2 {
					checkIntValue(0, "SELECT nextval('s')", sequenceNumber)
				}
				checkIntValue(anySession(), "SELECT last_value FROM s", 10)
				...
				// db:
				//  s: 20
				checkIntValue(0, "SELECT nextval('s')", 12)
				checkIntValue(anySession(), "SELECT last_value FROM s", 20)
		...
		// Test multiple cached sequences using multiple sessions.
		{
			...
			test: func(t *testing.T) {
			  ...
			  `)
				...
				// db:
				//  s1: 10
				checkIntValue(0, "SELECT nextval('s1')", 2)
				checkIntValue(anySession(), "SELECT last_value FROM s1", 10)
				...
				//  s1: 10
				//  s2: 12
				checkIntValue(0, "SELECT nextval('s2')", 3)
				checkIntValue(anySession(), "SELECT last_value FROM s2", 12)
				...
				//  s1: 20
				//  s2: 12
				checkIntValue(1, "SELECT nextval('s1')", 12)
				checkIntValue(anySession(), "SELECT last_value FROM s1", 20)
				...
				//  s1: 20
				//  s2: 24
				checkIntValue(1, "SELECT nextval('s2')", 15)
				checkIntValue(anySession(), "SELECT last_value FROM s2", 24)
				...
				//  s1: 30
				//  s2: 24
				checkIntValue(2, "SELECT nextval('s1')", 22)
				checkIntValue(anySession(), "SELECT last_value FROM s1", 30)
				...
				//  s1: 30
				//  s2: 36
				checkIntValue(2, "SELECT nextval('s2')", 27)
				checkIntValue(anySession(), "SELECT last_value FROM s2", 36)
				...
				emptyCache := func(session, start, finish, inc int, seq string) {
					for sequenceNumber := start; sequenceNumber <= finish; sequenceNumber += inc {
						checkIntValue(session, fmt.Sprintf("SELECT nextval('%s')", seq), sequenceNumber)
					}
					wg.Done()
				}
				wg.Add(3)
				...
				execStmt("DROP SEQUENCE s1")
				execStmt("DROP SEQUENCE s2")
			},
		},
	}
