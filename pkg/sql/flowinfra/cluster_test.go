...
func runTestClusterFlow(
	...
	txnProto := roachpb.MakeTransaction("cluster-test", nil, isolation.Serializable, roachpb.NormalUserPriority, now.ToTimestamp(), 0, int32(servers[0].SQLInstanceID()), 0, false)
...
func TestLimitedBufferingDeadlock(t *testing.T) {
	...
	txnProto := roachpb.MakeTransaction("deadlock-test", nil, isolation.Serializable, roachpb.NormalUserPriority, now.ToTimestamp(), 0, int32(tc.Server(0).SQLInstanceID()), 0, false)
		...
		b.Run(fmt.Sprintf("n%d", numNodes), func(b *testing.B) {
			for _, numRows := range []int{1, 100, 10000} {
				b.Run(fmt.Sprintf("r%d", numRows), func(b *testing.B) {
					...
					now := tc.Server(0).Clock().NowAsClockTimestamp()
					txnProto := roachpb.MakeTransaction("cluster-test", nil, isolation.Serializable, roachpb.NormalUserPriority, now.ToTimestamp(), 0, int32(tc.Server(0).SQLInstanceID()), 0, false)
					txn := kv.NewTxnFromProto(
						context.Background(), tc.Server(0).DB(), tc.Server(0).NodeID(),
						now, kv.RootTxn, &txnProto)
					leafInputState, err := txn.GetLeafTxnInputState(context.Background())
