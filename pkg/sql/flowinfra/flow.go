...
func (f *FlowBase) Resume(recv execinfra.RowReceiver) {
	...
	f.outputs[0] = recv
	log.VEventf(f.Ctx, 1, "resuming %T in the flow's goroutine", f.processors[0])
...
func (f *FlowBase) Run(ctx context.Context, noWait bool) {
	...
	f.Ctx = ctx
	log.VEventf(f.Ctx, 1, "running %T in the flow's goroutine", headProc)
	headProc.Run(f.Ctx, headOutput)
}
