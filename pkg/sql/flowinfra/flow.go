...
type Flow interface {
	...
	// goroutines are done.
	//
	// It is assumed that syncFlowConsumer is set, so all errors encountered
...
type FlowBase struct {
	...
	// syncFlowConsumer is a special execinfra.RowReceiver which, instead of
...
func (f *FlowBase) Run(ctx context.Context, doneFn func()) {
	...
	if len(f.processors) == 0 {
		f.syncFlowConsumer.Push(nil /* row */, &execinfrapb.ProducerMetadata{Err: errors.AssertionFailedf("no processors in flow")})
