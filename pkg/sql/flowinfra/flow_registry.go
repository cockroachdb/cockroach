...
func (s *InboundStreamInfo) cancelLocked() {
	s.mu.canceled = true
	s.finishLocked()
}
...
// connected yet in this flow, by canceling and finishing them. The method
// returns the list of RowReceivers corresponding to the streams that were
// canceled. The caller is expected to send those RowReceivers a cancellation
// message - this method can't do it because sending those messages shouldn't
// happen under the flow registry's lock.
