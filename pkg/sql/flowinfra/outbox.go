...
type Outbox struct {
	...
	// used when gracefully shutting down the tree rooted in this Outbox.
	outboxCtxCancel context.CancelFunc

	// err is only used for testing. It can be written from two different
	// goroutines - from the main goroutine as well as the watchdog, but the
	// latter will always write first, so the race is not possible and, thus,
	// there is no explicit synchronization on this field.
	err error
...
func (m *Outbox) mainLoop(ctx context.Context) (retErr error) {
	var rowChannelClosed bool
	defer func() {
		if !rowChannelClosed {
			// No matter what happens, we need to make sure we close our
			// RowChannel, since writers could be writing to it as soon as we
			// are started.
	...
	waitCh, drainCh, err := m.startWatchdogGoroutine(ctx)
	defer func() {
		// First, ensure that writers don't push any more data since it will
		// just be ignored.
		m.RowChannel.ConsumerClosed()
		rowChannelClosed = true
		// Next, make sure to always close the stream if it is still usable (if
		// not, then the field is set to nil).
		...
		// Block until the watchdog goroutine exits (if it hasn't already).
		<-waitCh
	}()
	if err != nil {
		return err
...
//
// This goroutine will tear down the flow if non-io.EOF error is received -
// without it, a producer goroutine might spin doing work forever after a
// connection is closed, since it wouldn't notice a closed connection until it
// tried to Send over that connection.
//
...
// tree of operators rooted in this outbox.
//
// It returns a channel that will be closed once the new goroutine exits as well
// as another channel which will be signaled when draining is requested. The
// caller is responsible to not exit before the "wait" channel is closed in
// order to not leak resources.
func (m *Outbox) startWatchdogGoroutine(
	...
	if err := m.flowCtx.Cfg.Stopper.RunAsyncTask(ctx, "watchdog", func(ctx context.Context) {
		for {
			...
			if err != nil {
				if err != io.EOF {
					...
					//
					// The main goroutine will access the err field only after
					// this goroutine exits, so it's safe to write without mutex
					// protection.
					m.err = err
				}
			...
			case signal.Handshake != nil:
				log.VEventf(ctx, 2, "Outbox received handshake: %v", signal.Handshake)
		...
		close(waitCh)
		return waitCh, drainCh, err
	}
	return waitCh, drainCh, nil
}

func (m *Outbox) run(ctx context.Context, wg *sync.WaitGroup) {
	mainLoopErr := m.mainLoop(ctx)
	if m.err == nil {
		m.err = mainLoopErr
	}
	if wg != nil {
