...
// txnStats holds per-transaction statistics.
type txnStats struct {
	roachpb.TransactionStatistics
}
...
func (a *appStats) getStatsForTxn(txnID string) *txnStats {
	a.Lock()
...
func (s *sqlStats) getStatsForApplication(appName string) *appStats {
	...
	a := &appStats{st: s.st, stmts: make(map[stmtKey]*stmtStats), txns: make(map[string]*txnStats)}
	s.apps[appName] = a
	return a
}
...
func (s *sqlStats) resetStats(ctx context.Context) {
	...
	for appName, a := range s.apps {
		...
		a.txns = make(map[string]*txnStats, len(a.txns)/2)
		a.Unlock()
	}
