	...
	for k, v := range statMap {
		...
		if created {
			...
			if latestErr := a.acc.Grow(ctx, estimatedAllocBytes); latestErr != nil {
				...
				// generated in this function is out of memory errors. Also since we do
				// not abort after encountering such errors, combining many same errors
				// is not helpful.
				err = latestErr
				delete(a.stmts, k)
				a.Unlock()
...
func (s *sqlStats) init(ctx context.Context, parentMon *mon.BytesMonitor) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.mu.mon.Start(ctx, parentMon, mon.BoundAccount{})
}
