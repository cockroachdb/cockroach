...
func TestInternalDBWithOverrides(t *testing.T) {
	...
	row, err := idb1.Executor().QueryRow(ctx, "test", nil, "show default_int_size")
	require.NoError(t, err)
	assert.Equal(t, "'0'", row[0].String())
	...
	row, err = idb2.Executor().QueryRow(ctx, "test", nil, "show default_int_size")
	require.NoError(t, err)
	assert.Equal(t, "'2'", row[0].String())

	row, err = idb2.Executor().QueryRow(ctx, "test", nil, "show session_authorization")
	require.NoError(t, err)
	assert.Equal(t, "'wowowo'", row[0].String())

	row, err = idb2.Executor().QueryRow(ctx, "test", nil, "show distsql")
	require.NoError(t, err)
	assert.Equal(t, "'off'", row[0].String())
}
