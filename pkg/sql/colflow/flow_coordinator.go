...
func (f *batchFlowCoordinator) run(ctx context.Context) {
	...
	for status := execinfra.NeedMoreRows; status == execinfra.NeedMoreRows; {
		err := f.next()
		if err != nil {
	...
	if span != nil {
		...
		if meta := execinfra.GetTraceDataAsMetadata(span); meta != nil {
			if f.output.PushBatch(nil /* batch */, meta) == execinfra.ConsumerClosed {
				return
			}
		}
	}
	...
	for i := range drainedMeta {
		...
		if f.output.PushBatch(nil /* batch */, &drainedMeta[i]) == execinfra.ConsumerClosed {
			return
		}
	}
}
