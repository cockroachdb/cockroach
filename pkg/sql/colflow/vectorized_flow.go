...
func (s *vectorizedFlowCreator) setupOutput(
	...
	case execinfrapb.StreamEndpointSpec_SYNC_RESPONSE:
		...
		if err := typesMatch(columnTypes, opOutputTypes); err != nil {
			return err
		}
				...
				for k := range outputSpec.Input {
					for l := range outputSpec.Input[k].Streams {
						...
						if inputStream.StreamID == outputStream.StreamID {
							if err := typesMatch(outputSpec.Input[k].ColumnTypes, opOutputTypes); err != nil {
								return nil, err
							}
						}
...
// typesMatch checks whether expected logical types match with actual physical
// types and returns an error if not.
func typesMatch(expected []types.T, actual []coltypes.T) error {
