...
func newVectorizedFlowCreator(
	...
	creator.releasable = append(creator.releasable, creator.exprHelper)
	return creator
}
	...
	if vecErr := colexecerror.CatchVectorizedRuntimeError(func() {
		...
		for len(queue) > 0 {
			...
			if result != nil {
				...
				s.releasable = append(s.releasable, result)
			}
			if err != nil {
...
func ConvertToVecTree(
	...
	defer func() {
		...
		// Note that we cannot Release all of the objects in creator.releasable
