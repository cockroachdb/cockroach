...
func (s *vectorizedFlowCreator) setupRouter(
	...
	router, outputs, err := colexec.NewHashRouter(colexec.NewAllocator(), input, outputTyps, hashCols, len(output.Streams))
	if err != nil {
		return err
	}
	...
	if len(inputStreamOps) > 1 {
		...
		if input.Type == execinfrapb.InputSyncSpec_ORDERED {
			...
			if opt == flowinfra.FuseAggressively {
				op, err = colexec.NewSerialUnorderedSynchronizer(colexec.NewAllocator(), inputStreamOps, typs)
			} else {
				op, err = colexec.NewParallelUnorderedSynchronizer(colexec.NewAllocator(), inputStreamOps, typs, s.waitGroup)
