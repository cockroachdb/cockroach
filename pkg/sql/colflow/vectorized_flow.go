...
func (s *vectorizedFlowCreator) wrapWithNetworkVectorizedStatsCollector(
	inbox *colrpc.Inbox, id int32, idTagKey string,
	...
	nvsc := colexec.NewNetworkVectorizedStatsCollector(op, ioReader, id, idTagKey, inputWatch, networkReader)
...
type remoteComponentCreator interface {
	...
	newInbox(ctx context.Context, allocator *colmem.Allocator, typs []*types.T, streamID execinfrapb.StreamID, latency int64) (*colrpc.Inbox, error)
}
...
func (vectorizedRemoteComponentCreator) newInbox(
	...
	return colrpc.NewInbox(ctx, allocator, typs, streamID, latency)
}
	...
	for _, inputStream := range input.Streams {
		...
		case execinfrapb.StreamEndpointSpec_REMOTE:
			...
			inbox, err := s.remoteComponentCreator.newInbox(
				ctx, colmem.NewAllocator(ctx, s.newStreamingMemAccount(flowCtx), factory), input.ColumnTypes, inputStream.StreamID, latency,
			)
			if err != nil {
				return nil, nil, nil, err
			}
