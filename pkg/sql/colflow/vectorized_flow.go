...
func (f *vectorizedFlow) Setup(
	...
	if vectorizeBatchSize := f.FlowCtx.EvalCtx.SessionData.VectorizeBatchSize; vectorizeBatchSize != 0 {
		if err := coldata.SetBatchSize(int(vectorizeBatchSize)); err != nil {
			return ctx, err
		}
	} else {
		coldata.ResetBatchSize()
	}
