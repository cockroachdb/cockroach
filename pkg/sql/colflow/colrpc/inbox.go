...
func (i *Inbox) Init(ctx context.Context) {
	...
	if err := func() error {
		...
		case <-i.Ctx.Done():
			...
			// we send nil error to the stream handler so that it exits
			// gracefully as well.
			i.errCh <- nil
			// However, we still return the context cancellation error so that
			// this goroutine didn't proceed further.
			return i.Ctx.Err()
		}
