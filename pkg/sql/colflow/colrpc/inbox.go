...
func (i *Inbox) Next() coldata.Batch {
	...
	for {
		...
		// Do admission control before deserialization.
		if i.admissionQ != nil {
			if _, err := i.admissionQ.Admit(i.Ctx, i.admissionInfo); err != nil {
				// TODO: err includes context cancellation while waiting for
				// admission. Is it ok to pass such an error to ExpectedError.
				colexecerror.ExpectedError(err)
			}
		}
