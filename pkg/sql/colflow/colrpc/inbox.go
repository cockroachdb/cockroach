		...
		if len(m.Data.Metadata) != 0 {
			...
			for _, rpm := range m.Data.Metadata {
				...
				if meta.Err != nil && receivedErr == nil {
					...
					// Perform the memory accounting for the LeafTxnFinalState
					// metadata since it might be of non-trivial size and we're
					// holding on to it until we're drained.
					if ltfs := meta.LeafTxnFinalState; ltfs != nil {
						memUsage := roachpb.Spans(ltfs.RefreshSpans).MemUsage()
						i.allocator.AdjustMemoryUsage(memUsage)
					}
				}
			}
			if receivedErr != nil {
...
func (i *Inbox) DrainMeta() []execinfrapb.ProducerMetadata {
	...
	defer i.allocator.ReleaseMemory(i.allocator.Used())
