...
type Inbox struct {
	...
	streamCh chan streamAndFirstMsg
...
func NewInbox(
	...
	i := &Inbox{
		...
		allocator:                allocator,
		streamID:                 streamID,
		streamCh:                 make(chan streamAndFirstMsg, 1),
...
func (i *Inbox) RunWithStream(
	streamCtx context.Context, stream flowStreamServer, firstMsg *execinfrapb.ProducerMessage,
) error {
	...
	i.streamCh <- streamAndFirstMsg{stream: stream, firstMsg: firstMsg}
...
func (i *Inbox) Init(ctx context.Context) {
	...
	if err := func() error {
		...
		select {
		case streamAndFirstMsg := <-i.streamCh:
			i.stream = streamAndFirstMsg.stream
			if streamAndFirstMsg.firstMsg.Header != nil {
				i.producer = streamAndFirstMsg.firstMsg.Header.Producer
			}
