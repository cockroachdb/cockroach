...
func TestInboxCancellation(t *testing.T) {
	...
	t.Run("ReaderWaitingForStreamHandler", func(t *testing.T) {
		inbox, err := NewInbox(context.Background(), testAllocator, typs, execinfrapb.StreamID(0), 0 /* latency */)
...
func TestInboxNextPanicDoesntLeakGoroutines(t *testing.T) {
	...
	inbox, err := NewInbox(context.Background(), testAllocator, []*types.T{types.Int}, execinfrapb.StreamID(0), 0 /* latency */)
	require.NoError(t, err)
...
func TestInboxTimeout(t *testing.T) {
	...
	inbox, err := NewInbox(ctx, testAllocator, []*types.T{types.Int}, execinfrapb.StreamID(0), 0 /* latency */)
	require.NoError(t, err)
		...
		for _, runNextGoroutine := range []bool{false, true} {
			...
			for _, runRunWithStreamGoroutine := range []bool{false, true} {
				...
				t.Run(fmt.Sprintf(
					...
					inbox, err := NewInbox(context.Background(), colmem.NewAllocator(inboxCtx, &inboxMemAccount, coldata.StandardColumnFactory), typs, execinfrapb.StreamID(0), 0 /* latency */)
					require.NoError(t, err)
