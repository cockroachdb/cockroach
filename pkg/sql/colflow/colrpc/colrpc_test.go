...
func TestOutboxInbox(t *testing.T) {
	...
	t.Run(fmt.Sprintf("cancellationScenario=%s", cancellationScenarioName), func(t *testing.T) {
		...
		input := coldatatestutils.NewRandomDataOp(
			colexecbase.NewAllocator(ctx, &inputMemAcc), rng, args,
		)

		outboxMemAcc := testMemMonitor.MakeBoundAccount()
		defer outboxMemAcc.Close(ctx)
		outbox, err := NewOutbox(colexecbase.NewAllocator(ctx, &outboxMemAcc), input, typs, nil /* metadataSource */, nil /* toClose */)
		require.NoError(t, err)

		inboxMemAcc := testMemMonitor.MakeBoundAccount()
		defer inboxMemAcc.Close(ctx)
		inbox, err := NewInbox(
			colexecbase.NewAllocator(ctx, &inboxMemAcc), typs, execinfrapb.StreamID(0),
		)
		require.NoError(t, err)
		...
		inputBatches := colexec.NewDeselectorOp(
			colexecbase.NewAllocator(ctx, &deselectorMemAcc), inputBuffer, typs,
		)
		inputBatches.Init()
...
func TestOutboxInboxMetadataPropagation(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			outboxMemAcc := testMemMonitor.MakeBoundAccount()
			defer outboxMemAcc.Close(ctx)
			outbox, err := NewOutbox(colexecbase.NewAllocator(ctx, &outboxMemAcc), input, typs, []execinfrapb.MetadataSource{
			...
			defer inboxMemAcc.Close(ctx)
			inbox, err := NewInbox(
				colexecbase.NewAllocator(ctx, &inboxMemAcc),
...
func BenchmarkOutboxInbox(b *testing.B) {
	...
	outboxMemAcc := testMemMonitor.MakeBoundAccount()
	defer outboxMemAcc.Close(ctx)
	outbox, err := NewOutbox(colexecbase.NewAllocator(ctx, &outboxMemAcc), input, typs, nil /* metadataSources */, nil /* toClose */)
	require.NoError(b, err)
