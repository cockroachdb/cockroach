...
// testAllocator is a colexec.Allocator with an unlimited budget for use
// in tests.
...
func BenchmarkOutboxInbox(b *testing.B) {
	...
	batch, err := testAllocator.NewMemBatch(typs)
	require.NoError(b, err)
...
func TestOutboxStreamIDPropagation(t *testing.T) {
	...
	input := &colexec.CallbackOperator{NextCb: func(ctx context.Context) coldata.Batch {
		b, err := testAllocator.NewMemBatchWithSize(typs, 0)
		require.NoError(t, err)
		b.SetLength(0)
