...
type callbackRemoteComponentCreator struct {
	newOutboxFn func(*colexec.Allocator, colexec.Operator, []colphystypes.T, []execinfrapb.MetadataSource) (*colrpc.Outbox, error)
	newInboxFn  func(allocator *colexec.Allocator, typs []colphystypes.T, streamID execinfrapb.StreamID) (*colrpc.Inbox, error)
}
...
func TestDrainOnlyInputDAG(t *testing.T) {
	...
	inboxToNumInputTypes := make(map[*colrpc.Inbox][]colphystypes.T)
	outboxCreated := false
