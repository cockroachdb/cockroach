...
func TestVectorizedFlowTempDirectory(t *testing.T) {
	...
	checkDirs := func(t *testing.T, i int) {
		t.Helper()
		dirs, err := ngn.ListDir(baseDirName)
		require.NoError(t, err)
		require.Equal(t, i, len(dirs), "expected %d directories but found %d: %s", i, len(dirs), dirs)
	}
	...
	t.Run("LazilyCreated", func(t *testing.T) {
		...
		creator.diskQueueCfg.OnNewDiskQueue()

		// We should now have one directory, the flow's temporary storage directory.
		checkDirs(t, 1)

		// Another operator calling OnNewDiskQueue again should not create a new
		// directory
		creator.diskQueueCfg.OnNewDiskQueue()
		checkDirs(t, 1)
	...
	t.Run("DirCreationHandlesUnsetIDCollisions", func(t *testing.T) {
		...
		checkDirs(t, 0)
		creator1.diskQueueCfg.OnNewDiskQueue()
		checkDirs(t, 1)
		...
		checkDirs(t, 1)
		creator2.diskQueueCfg.OnNewDiskQueue()
