...
func TestDrainingAfterRemoteError(t *testing.T) {
	...
	rows, err := conn.Query("EXPLAIN (VEC) SELECT sum(length(v)) FROM small INNER LOOKUP JOIN large ON small.k = large.k GROUP BY large.k;")
	...
	sqlDB.ExpectErr(t, ".*test-disk.*", "SELECT sum(length(v)) FROM small INNER LOOKUP JOIN large ON small.k = large.k GROUP BY large.k")
