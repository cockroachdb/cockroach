	...
	for run := 0; run < 10; run++ {
		for _, shutdownOperation := range shutdownScenarios {
			t.Run(fmt.Sprintf("shutdownScenario=%s", shutdownOperation.string), func(t *testing.T) {
				...
				hashRouter, hashRouterOutputs, err := colexec.NewHashRouter(testAllocator, hashRouterInput, typs, []int{0}, numHashRouterOutputs)
				require.NoError(t, err)
				...
				synchronizer, err := colexec.NewParallelUnorderedSynchronizer(testAllocator, synchronizerInputs, typs, &wg)
				require.NoError(t, err)
