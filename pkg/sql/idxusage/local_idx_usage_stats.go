...
// batchInsert inserts otherStats into s without taking on write lock.
...
func (s *LocalIndexUsageStats) getStatsForTableID(
	...
	// Upgrading the lock to from a read-lock into a write-lock.
	// Then subsequently we call s.getStatsForTableIDLocked(). That function
	// will check again whether the given roachpb.TableID exists in our map.
	// This is necessary to prevent race condition.
	s.mu.RUnlock()
	s.mu.Lock()
	defer s.mu.Unlock()
