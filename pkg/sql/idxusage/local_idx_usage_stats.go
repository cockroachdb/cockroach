...
// Start implements the idxusage.Provider interface.
func (is *LocalIndexUsageStats) Start(ctx context.Context, stopper *stop.Stopper) {
	is.startStatsIngestionLoop(ctx, stopper)
}

// RecordRead implements the idxusage.Provider interface.
func (is *LocalIndexUsageStats) RecordRead(ctx context.Context, key roachpb.IndexUsageKey) {
	is.record(ctx, indexUse{
		key:      key,
		usageTyp: readOp,
	})
}

func (is *LocalIndexUsageStats) record(ctx context.Context, payload indexUse) {
	select {
...
// GetIndexUsageStats implements the idxusage.Provider interface.
func (is *LocalIndexUsageStats) GetIndexUsageStats(
	key roachpb.IndexUsageKey,
) roachpb.IndexUsageStatistics {
	is.mu.RLock()
	defer is.mu.RUnlock()
...
// IterateIndexUsageStats the idxusage.Provider interface.
func (is *LocalIndexUsageStats) IterateIndexUsageStats(
...
func (is *LocalIndexUsageStats) clear() {
	is.mu.Lock()
	defer is.mu.Unlock()
...
func (is *LocalIndexUsageStats) insertIndexUsage(idxUse *indexUse) {
	tableStats := is.getStatsForTableID(idxUse.key.TableID, true /* createIfNotExists */)
	...
	// TODO(azhng): include TotalRowsRead/TotalRowsWritten field once it is plumbed
	//  into the SQL engine.
	case readOp:
...
func (is *LocalIndexUsageStats) getStatsForTableID(
...
func (t *tableIndexStats) iterateIndexStats(
	orderedIndexID bool, curIterVal, max *uint64, visitor StatsVisitor,
...
func (is *LocalIndexUsageStats) startStatsIngestionLoop(
