...
func TestIndexUsageStatisticsSubsystem(t *testing.T) {
	...
	subsystem := NewLocalIndexUsageStats(&Config{
		ChannelSize: 10,
	...
	t.Run("iterator", func(t *testing.T) {
		actualEntryCount := 0
		err := subsystem.IterateIndexUsageStats(IteratorOptions{}, func(key *roachpb.IndexUsageKey, value *roachpb.IndexUsageStatistics) error {
			actualEntryCount++
	...
	t.Run("iterator with options", func(t *testing.T) {
		...
		maxEntry := uint64(1)
		err := subsystem.IterateIndexUsageStats(IteratorOptions{
			SortedTableID: true,
			SortedIndexID: true,
			Max:           &maxEntry,
	...
	t.Run("clear", func(t *testing.T) {
		...
		subsystem.clear()
		err := subsystem.IterateIndexUsageStats(IteratorOptions{}, func(_ *roachpb.IndexUsageKey, _ *roachpb.IndexUsageStatistics) error {
			actualEntryCount++
			return nil
		})
		require.NoError(t, err)
