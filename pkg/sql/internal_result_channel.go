...
// newAsyncIEResultChannel returns a ieResultChannel which does not attempt to
...
// newSyncIEResultChannel returns an syncIEResultChannel which synchronizes the
// writer with the reader.
func newSyncIEResultChannel() *syncIEResultChannel {
...
func (i *syncIEResultChannel) addResult(ctx context.Context, result ieIteratorResult) error {
	...
	case <-ctx.Done():
		return ctx.Err()
	case <-i.doneCh:
		// TODO(ajwerner): Pick a less ridiculous error.
