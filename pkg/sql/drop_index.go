...
func (p *planner) dropIndexByName(
	...
	indexHasReplacementCandidate := func(fkColIDs sqlbase.ColumnIDs, isOrigin bool) bool {
		foundReplacement := false
		for _, index := range remainingIndexes {
			if (isOrigin && index.IsValidOriginIndex(fkColIDs)) || (!isOrigin && index.IsValidReferencedIndex(fkColIDs)) {
