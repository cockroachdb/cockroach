	...
	for _, index := range n.idxNames {
		...
		if shardColName != "" {
			...
			if shardColDesc.IsVirtual() || n.n.DropBehavior == tree.DropCascade {
				...
				params.p.BufferClientNotice(
					ctx,
					pgnotice.Newf("The accompanying shard column %q is a physical column and dropping "+
						"it can be expensive, so, we skipped dropping it. Issue another `DROP INDEX %v` statement if you want "+
						"to drop it.", shardColName, shardColName),
				)
			}
		}
