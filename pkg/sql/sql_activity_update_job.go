...
// give an even number. The top 500 are likely the same for several columns
// ,so it should still give 3 days of history.
...
func (u *SqlActivityUpdater) TransferStatsToActivity(ctx context.Context) error {
	...
	stmtRowCount, txnRowCount, totalStmtClusterExecCount, totalTxnClusterExecCount, err := u.getExecutionCount(ctx, aggTs)
	if err != nil {
		return err
	}
...
func (u *SqlActivityUpdater) transferTopStats(
	...
	// cpu_sql_nanos, contention_time, p99_latency and insert into
...
`,
	...
	// Select the top 500 for execution_count, service_latency,
	// cpu_sql_nanos, contention_time, p99_latency, and any statement
	// that was in the top 500 transactions and insert into
	// statement_activity table.
	_, err = u.db.Executor().ExecEx(ctx,
...
// getExecutionCount is used to get the row counts of both the
// system.statement_statistics and system.transaction_statistics.
// It also gets the total execution count for the specified aggregated
// timestamp.
func (u *SqlActivityUpdater) getExecutionCount(
