...
func (j *sqlActivityUpdateJob) Resume(ctx context.Context, execCtxI interface{}) (jobErr error) {
	...
	intervalLock := syncutil.Mutex{}
	// Update the sleep interval if the config time changes.
	// This avoids needing to wait for the original sleep time
	// to pass to for the loop to get the new value.
	flushInterval.SetOnChange(&settings.SV,
		func(ctx context.Context) {
			sleepInterval := flushInterval.Get(&settings.SV)
			intervalLock.Lock()
