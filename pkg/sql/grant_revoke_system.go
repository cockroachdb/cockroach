...
func (p *planner) SynthesizePrivilegeDescriptor(
	...
	if desc.IsUncommittedVersion() {
		return p.synthesizePrivilegeDescriptorFromSystemPrivilegesTable(ctx, privilegeObjectName, privilegeObjectPath, privilegeObjectType)
	}
	...
	privDesc, err := p.synthesizePrivilegeDescriptorFromSystemPrivilegesTable(ctx, privilegeObjectName, privilegeObjectPath, privilegeObjectType)
	if err != nil {
		return nil, err
	}
