...
func (n *changeNonDescriptorBackedPrivilegesNode) makeSystemPrivilegeObject(
	...
	case privilege.VirtualTable:
		...
		for _, tableTarget := range n.targets.Tables.TablePatterns {
			...
			for _, name := range tableNames {
				...
				privDesc, err := p.SynthesizePrivilegeDescriptor(
					ctx, syntheticprivilege.CreateVirtualTablePrivilegePath(name.Schema(), name.Table()),
					privilege.VirtualTable)
				if err != nil {
					return nil, err
				}
				if privDesc == nil {
	...
	case privilege.ExternalConnection:
		...
		for _, externalConnectionName := range n.targets.ExternalConnections {
			...
			privDesc, err := p.SynthesizePrivilegeDescriptor(
				ctx, syntheticprivilege.CreateExternalConnectionPrivilegePath(string(externalConnectionName)),
