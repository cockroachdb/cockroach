...
func GetUserHashedPassword(username string, session *Session, executor *Executor) ([]byte, error) {
	normalizedUsername := sqlbase.NormalizeString(username)
	if session == nil || executor == nil {
		return nil, errors.Errorf("error looking up user %s", normalizedUsername)
	}
	...
	placeholderInfo.SetValue("1", parser.NewDString(normalizedUsername))
	...
	if len(r.ResultList) != 1 || r.ResultList[0].Err != nil {
		...
		return nil, errors.Errorf("user %s does not exist", normalizedUsername)
	}
