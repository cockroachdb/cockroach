...
func checkBlockedSchemaChange(
	...
	if err := jobutils.VerifySystemJob(t, runner, 4, jobspb.TypeSchemaChange, expStatus, jobs.Record{
	...
	if !hadJobInOldVersion(schemaChangeType) {
		...
		rows := runner.QueryStr(t, "SELECT * FROM [SHOW JOBS] WHERE job_type = 'SCHEMA CHANGE' AND description <> 'updating privileges'")
...
func testSchemaChangeMigrations(
	...
	checkBlockedSchemaChange(t, runner, shouldCancel, schemaChange, schemaChangeType)

	// Start the migrations.
	log.Info(ctx, "starting job migration")
	migMgr := tc.Server(0).MigrationManager().(*sqlmigrations.Manager)
	if err := migMgr.StartSchemaChangeJobMigration(ctx, true); err != nil {
		t.Fatal(err)
	}
	...
	//runner.Exec(t, "SHOW JOBS WHEN COMPLETE SELECT job_id FROM [SHOW JOBS] WHERE job_type = 'SCHEMA CHANGE' AND description <> 'updating privileges'")
	// Wait until there are no more running schema changes.
	log.Info(ctx, "waiting for new schema change jobs to complete")
	runner.CheckQueryResultsRetry(t, "SELECT * FROM [SHOW JOBS] WHERE job_type = 'SCHEMA CHANGE' AND description <> 'updating privileges' AND NOT (status = 'succeeded' OR status = 'canceled')", [][]string{})
...
func setupServerAndStartSchemaChange(
	...
	setupTestingKnobs(t, &kvDB, blockState, schemaChangeType, shouldCancel, &registry, &params, &runner, &blockSchemaChanges, blockCh, migrationDoneCh)
	...
	runner.CheckQueryResultsRetry(t, "SELECT count(*) FROM [SHOW JOBS] WHERE job_type = 'SCHEMA CHANGE' AND description <> 'updating privileges' AND NOT (status = 'succeeded' OR status = 'canceled')", [][]string{{"0"}})
...
// updateJob is a helper which modifies the details and progress of a given
// schema change job.
func updateJob(
	job *jobs.Job,
	updateDetailsFn func(details *jobspb.SchemaChangeDetails),
	updateProgressFn func(progress *jobspb.Progress),
) error {
	return job.Update(context.TODO(), func(txn *kv.Txn, md jobs.JobMetadata, ju *jobs.JobUpdater) error {
		details := job.Details().(jobspb.SchemaChangeDetails)
		updateDetailsFn(&details)
...
// it's 20.1 to it's 19.2 representation.
func migrateJobToOldFormat(
	...
	if err := kvDB.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
		...
		return updateJob(
			job,
			func(details *jobspb.SchemaChangeDetails) {
			...
			func(progress *jobspb.Progress) {
				...
				//	progress.RunningStatus = string(sql.RunningStatusWaitingGC)
				//}
	...
	// TODO(pbardea): Populate drop time and job ID in GC Mutations when we
	// start testing jobs that are waiting for GC.
...
func setupTestingKnobs(
	...
	blockFn := func(jobID int64) error {
		...
		if doneReverseMigration {
			...
			migrateJobToOldFormat(t, runner, kvDB, registry, jobID, schemaChangeType)
			migratedCount++
		}
	...
	if shouldCancel {
		if _, ok := runsBackfill[schemaChangeType]; ok {
			...
			// TODO(pbardea): Nothing should reach here.
...
func verifySchemaChangeJobRan(
	...
	if didCancel {
		...
		if schemaChangeType == AddConstraint && blockState == AfterBackfill {
			// TODO(pbardea): Broken case - add constraints when migrating
			// aren't marked as canceled.
			expStatus = jobs.StatusReverting
		}
	}
	if schemaChangeType == CreateTable {
...
// Test cases are split out into separate tests so that they race be tested
// against stressrace in a reasonable amount of time.

var jobIntervalMutex = syncutil.Mutex{}

func atomicSetTestJobsAdoptInterval() func() {
	const testingJobsAdoptionInverval = 100 * time.Millisecond
	jobIntervalMutex.Lock()
	old := jobs.DefaultAdoptInterval
	jobs.DefaultAdoptInterval = testingJobsAdoptionInverval
	jobIntervalMutex.Unlock()
	return func() {
		jobIntervalMutex.Lock()
		defer jobIntervalMutex.Unlock()
...
func TestMigrateCreateTable(t *testing.T) {
	...
	// TODO(pbardea): While stressing, got stock after 80 runs (30s).
	defer leaktest.AfterTest(t)()
	...
	for _, blockState := range blockStates {
		...
		t.Run(getTestName(schemaChangeType, blockState, shouldCancel), func(t *testing.T) {
			defer atomicSetTestJobsAdoptInterval()()
