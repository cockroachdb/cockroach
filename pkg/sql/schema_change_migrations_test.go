...
func checkBlockedSchemaChange(
	...
	if err := jobutils.VerifySystemJob(t, runner, 0, jobspb.TypeSchemaChange, expStatus, jobs.Record{
		Description:   testCase.schemaChange,
		Username:      security.RootUser,
		DescriptorIDs: getTableIDsUnderTest(testCase.schemaChangeType),
	}); err != nil {
		t.Fatal(err)
	}

	if !hadJobInOldVersion(testCase.schemaChangeType) {
...
func testSchemaChangeMigrations(t *testing.T, testCase migrationTestCase) {
	...
	atomic.StoreInt32(&shouldBlockMigration, 1)
...
func verifySchemaChangeJobRan(
	...
	switch testCase.schemaChangeType {
	case AddColumn:
		if didCancel {
			expected = [][]string{{"1", "2"}}
		} else {
