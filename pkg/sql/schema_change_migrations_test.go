...
func checkBlockedSchemaChange(
	...
	if blockState == WaitingForGC {
		...
		runner.CheckQueryResultsRetry(t,
			"SELECT count(*) FROM [SHOW JOBS] WHERE job_type = 'SCHEMA CHANGE' AND "+
				"description <> 'updating privileges' AND status = 'succeeded'",
			[][]string{{"1"}},
		)
		rows := runner.QueryStr(
			t,
			"SELECT * FROM [SHOW JOBS] WHERE job_type = 'SCHEMA CHANGE' AND "+
				"description <> 'updating privileges' AND status = 'succeeded'",
		)
...
func testSchemaChangeMigrations(
	...
	runner.CheckQueryResultsRetry(t, "SELECT * FROM [SHOW JOBS] WHERE (job_type = 'SCHEMA CHANGE' OR job_type = 'SCHEMA CHANGE GC')  AND description <> 'updating privileges' AND NOT (status = 'succeeded' OR status = 'canceled')", [][]string{})
...
func migrateGCJobToOldFormat(
	...
	if err := kvDB.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
		...
		return job.WithTxn(txn).Update(ctx, func(txn *kv.Txn, md jobs.JobMetadata, ju *jobs.JobUpdater) error {
			...
			md.Payload.Lease = nil
			md.Payload.Description = strings.Trim(md.Payload.Description, "GC for ")
			md.Payload.Details = jobspb.WrapPayloadDetails(details)
			md.Progress = &progress
			ju.UpdatePayload(md.Payload)
			ju.UpdateProgress(md.Progress)
	...
	case DropIndex:
		...
		// For indexes being GC'ed, re-populate fields in GCMutations.
		if schemaChangeType == DropIndex {
			if l := len(tableDesc.GCMutations); l != 1 {
				t.Fatalf("expected exactly 1 index in GCMutations but found %d", l)
			}
	...
	default:
		t.Fatalf("invalid schema change type: %d", schemaChangeType)
	}
}
...
func setupTestingKnobs(
	...
	blockFn := func(jobID int64) error {
		...
		if doneReverseMigration {
			...
			if blockState == WaitingForGC {
				migrateGCJobToOldFormat(t, kvDB, registry, jobID, schemaChangeType)
			} else {
				migrateJobToOldFormat(t, kvDB, registry, jobID, schemaChangeType)
			}
			migratedCount++
		}
