...
// queryStrIdx represents the index of a query string within a collection of
// query strings.
type queryStrIdx int
...
// allows one reader and one writer. The writer adds statements to the buffer
// using push(). The reader reads one statement at a time using getCurStmt().
// The buffer internally maintains a cursor representing the reader's position.
// The reader has to manually move the cursor using advanceOneStmt(),
// seekToNextQueryStr() and rewind().
//
// In practice, the writer is a module responsible for communicating with a SQL
// client (i.e.  pgwire) and the reader is a Session.
//
// Push() can be called concurrently with GetCurStmt().
//
...
// push adds a query string to the end of the buffer. If a getCurStmt() call was
...
// getCurStmt returns the statement currently indicated by the cursor. Besides
...
func (buf *stmtBuf) getCurStmt(ctx context.Context) (parser.Statement, cursorPosition) {
	buf.mu.Lock()
	defer buf.mu.Unlock()
	for {
...
// error.
func (buf *stmtBuf) translateQueryStrPosLocked(pos queryStrPos) (queryStrIdx, error) {
	if pos < buf.mu.startPos {
		return 0, errors.Errorf(
			"position %d no longer in buffer (buffer starting at %d)",
			pos, buf.mu.startPos)
	}
	return queryStrIdx(pos - buf.mu.startPos), nil
}
...
// advanceOneStmt advances the cursor one statement over. The statement over
// which the cursor will be positioned when this returns may not be in the
// buffer yet.
func (buf *stmtBuf) advanceOneStmt(ctx context.Context) {
	buf.mu.Lock()
	defer buf.mu.Unlock()
...
type clientComm interface {
	...
	// client. The returned clientLock can be queried to see what results may have
	// been already delivered to the client and to discard results that haven't
	// been delivered.
	//
	// clientLock.Close() needs to be called on the returned lock once results
