...
func refreshTables(
...
) (expired bool, duration time.Duration, err error) {
	...
	for _, tableID := range tableIDs {
		...
		var err error
		if tableHasExpiredElem, newTimerDuration, err = updateStatusForGCElements(
			ctx,
			execCfg,
			tableID,
...
func getUpdatedTables(
	...
	if zoneCfgModified {
		...
		return getAllRemainingTables(details, progress), nil
	}
