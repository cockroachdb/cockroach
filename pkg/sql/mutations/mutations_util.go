...
//
// If overrideMaxBatchSize is true, then the "production" value will be returned
// regardless of whether the build is metamorphic or not. This should only be
// used by tests the output of which differs if maxBatchSize is randomized.
func MaxBatchSize(overrideMaxBatchSize bool) int {
	if overrideMaxBatchSize {
