...
func checkInspectPrivilegeForJob(ctx context.Context, p *planner, jobID jobspb.JobID) error {
	row, err := p.InternalSQLTxn().QueryRow(ctx, "check-inspect-job-privilege", p.txn,
		`SELECT ji.value as payload, j.status as status FROM system.jobs j JOIN system.job_info ji ON j.id = ji.job_id WHERE j.id = $1 AND j.job_type = 'INSPECT' AND j.status = 'succeeded' AND ji.info_key = 'legacy_payload'`, jobID)
	if err != nil {
		return err
	}
	if row == nil {
		return errors.Newf("job %d not found or is not a successfully completed INSPECT job", jobID)
	}
