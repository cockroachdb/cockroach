...
func TestShowFingerprintsPartialAndInvertedIndex(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)
	...
	// Get fingerprints for all indexes
	rows := sqlDB.QueryStr(t, `SHOW EXPERIMENTAL_FINGERPRINTS FROM TABLE d.t`)
	require.Len(t, rows, 2, "expected only primary and b_idx to be fingerprinted")
