...
// or a save point helper. This isn't as important as the internal executor
// because statement failures like unique constraint validation failure will
// poision the txn like a normal pgwire connection.
...
func (i *InternalSession) readResults(ctx context.Context) ([]tree.Datums, int, error) {
	...
	// Consume every event that was pushed into conneciton state machine before
	// we read the results.
	for {
