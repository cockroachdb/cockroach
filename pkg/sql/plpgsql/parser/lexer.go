...
func (l *lexer) parseOne(sql string) (tree.Statement, error) {
	sqlStmt, err := parser.ParseOneWithNumAnnotations(sql, l.numAnnotations)
	if err != nil {
		return nil, err
	}
...
func (l *lexer) ParseExpr(sqlStr string) (plpgsqltree.Expr, error) {
	...
	exprs, numAnnotations, err := parser.ParseExprsWithNumAnnotations([]string{sqlStr}, l.numAnnotations)
	if err != nil {
		return nil, err
	}
	if len(exprs) != 1 {
