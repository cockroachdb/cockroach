...
func TestParseLineNumbers(t *testing.T) {
	...
	// Test with whitespace-only lines (spaces and tabs) between statements.
	t.Run("whitespace_only_lines", func(t *testing.T) {
		input := "\nBEGIN\n" +
			"  \t \n" + // line 3: spaces and tab
			"\t\t\n" + // line 4: tabs only
			"  IF x <= 0 THEN\n" + // line 5
			"   \n" + // line 6: spaces only
			"\t\n" + // line 7: tab only
		...
		ifStmt, ok := stmt.AST.Body[0].(*plpgsqltree.If)
		require.True(t, ok, "expected If, got %T", stmt.AST.Body[0])
		require.Equal(t, 5, ifStmt.GetLineNo(), "IF should be on line 5 (after whitespace-only lines)")

		raiseStmt, ok := ifStmt.ThenBody[0].(*plpgsqltree.Raise)
		require.True(t, ok, "expected Raise, got %T", ifStmt.ThenBody[0])
		require.Equal(t, 8, raiseStmt.GetLineNo(), "RAISE should be on line 8 (after whitespace-only lines)")
	})
