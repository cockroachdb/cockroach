...
// TestHumanReadableStmtTag verifies that HumanReadableStmtTag converts
// internal statement tags to PostgreSQL-compatible human-readable names.
func TestHumanReadableStmtTag(t *testing.T) {
	testCases := []struct {
		tag      string
		expected string
	}{
		...
		{"stmt_if", "IF"},
		{"stmt_case", "CASE"},
		{"stmt_simple_loop", "LOOP"},
		{"stmt_while", "WHILE"},
		{"stmt_for_int_loop", "FOR with integer loop variable"},
		{"stmt_exit", "EXIT"},
		{"stmt_continue", "CONTINUE"},
		{"stmt_return", "RETURN"},
		{"stmt_return_next", "RETURN NEXT"},
		{"stmt_return_query", "RETURN QUERY"},
		{"stmt_raise", "RAISE"},
		{"stmt_assert", "ASSERT"},
		{"stmt_exec_sql", "SQL statement"},
		{"stmt_dyn_exec", "EXECUTE"},
		{"stmt_call", "CALL"},
		{"stmt_get_diag", "GET DIAGNOSTICS"},
		{"stmt_open", "OPEN"},
		{"stmt_fetch", "FETCH"},
		{"stmt_close", "CLOSE"},
		{"stmt_null", "NULL"},
	...
	for _, tc := range testCases {
		t.Run(tc.tag, func(t *testing.T) {
			result := plpgsqltree.HumanReadableStmtTag(tc.tag)
