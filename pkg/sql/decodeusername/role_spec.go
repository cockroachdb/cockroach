...
func FromRoleSpec(
	sessionData *sessiondata.SessionData, purpose username.UsernamePurpose, r tree.RoleSpec,
...
func FromRoleSpecList(
	sessionData *sessiondata.SessionData, purpose username.UsernamePurpose, l tree.RoleSpecList,
) ([]username.SQLUsername, error) {
...
func FromNameList(l tree.NameList) ([]username.SQLUsername, error) {
	targetRoles := make([]username.SQLUsername, len(l))
	for i, role := range l {
		user, err := username.MakeSQLUsernameFromUserInput(string(role), username.UsernameValidation)
		if err != nil {
			return nil, err
		}
		targetRoles[i] = user
