...
func (t *TelemetryLoggingMetrics) ExpSmoothQPS() int64 {
	...
	currIdx := t.RollingQueryCounts.EndPointer()
	startIdx := t.RollingQueryCounts.NextIndex(currIdx)
	t.ResetMovingQPS()
	...
	for i := t.MovingQPSLength() - 1; i >= 0; i-- {
		qpsVal := float64(t.GetMovingQPS(i))
		...
		if i == t.MovingQPSLength()-1 {
			avgQPSVal := totalQPSVal / t.MovingQPSLength()
