...
type aggregatorPlanningInfo struct {
	...
	// getInputMergeOrdering returns MergeOrdering property of the input plan.
...
func (dsp *DistSQLPlanner) planAggregators(
	...
	if !multiStage && allDistinct {
		...
		ordering := info.getInputMergeOrdering().Columns
...
// createPhysPlanForValuesNode creates a physical plan based on a valuesNode.
func (dsp *DistSQLPlanner) createPhysPlanForValuesNode(
...
func (dsp *DistSQLPlanner) createPhysPlanForPlanNode(
	...
	case *valuesNode:
		plan, err = dsp.createPhysPlanForValuesNode(planCtx, n)
