...
const (
	// shouldNotDistribute indicates that a plan can't be distributed to other
	// nodes.
...
// error if the plan subtree is not distributable.
...
type planningCtx struct {
	...
	// firstNode is set to true until we've finished planning the root of the
	// planNode tree. It's used to keep track of whether it's valid to run that
	// root node in a special fast path mode.
	firstNode bool
}
...
func (dsp *DistSQLPlanner) wrapPlan(planCtx *planningCtx, n planNode) (physicalPlan, error) {
	...
	*/
	...
	useFastPath := planCtx.firstNode && planCtx.stmtType == tree.RowsAffected
