...
func (dsp *DistSQLPlanner) createTableReaders(
	...
	spec, post, err := initTableReaderSpec(n, &planCtx.extendedEvalCtx.EvalContext)
	if err != nil {
			...
			for i, e := range aggregations {
				...
				if info.FinalRendering == nil {
					...
					renderExprs[i] = distsqlplan.MakeExpression(
						h.IndexedVar(mappedIdx), &planCtx.extendedEvalCtx.EvalContext,
						nil /* indexVarMap */)
				} else {
					...
					renderExprs[i] = distsqlplan.MakeExpression(
						expr, &planCtx.extendedEvalCtx.EvalContext,
						nil /* indexVarMap */)
				}
...
ColLoop:
	...
	post := distsqlrun.PostProcessSpec{
		Filter: distsqlplan.MakeExpression(
			n.table.filter, &planCtx.extendedEvalCtx.EvalContext, nil /* indexVarMap */),
		Projection:    true,
...
func (dsp *DistSQLPlanner) createPlanForJoin(
	...
	onExpr := remapOnExpr(&planCtx.extendedEvalCtx.EvalContext, n, leftPlan, rightPlan)
...
func (dsp *DistSQLPlanner) createPlanForNode(
	...
	case *renderNode:
		...
		dsp.selectRenders(&plan, n, &planCtx.extendedEvalCtx.EvalContext)
		return plan, nil
	...
	case *filterNode:
		...
		plan.AddFilter(n.filter, &planCtx.extendedEvalCtx.EvalContext, plan.planToStreamColMap)
	...
	case *limitNode:
		...
		if err := n.evalLimit(&planCtx.extendedEvalCtx.EvalContext); err != nil {
			return physicalPlan{}, err
		}
