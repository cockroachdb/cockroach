...
func (dsp *distSQLPlanner) addAggregators(
	...
	if !multiStage && anyDistinct && allDistinct {
		...
		orderedColumns := make([]uint32, len(ordering))
		...
		var intermediateTypes []sqlbase.ColumnType
		for _, agg := range aggregations {
			_, t, err := distsqlrun.GetAggregateInfo(agg.Func, p.ResultTypes[agg.ColIdx])
			if err != nil {
				return err
			}
			intermediateTypes = append(intermediateTypes, t)
		}

		if len(p.ResultRouters) > 1 {
			// Add distinct processors local to each existing current result processor.
			p.AddNoGroupingStage(distinctSpec, distsqlrun.PostProcessSpec{}, intermediateTypes, p.MergeOrdering)

			// TODO(arjun): We could distribute this final stage by hash.
			p.AddSingleGroupStage(node, distinctSpec, distsqlrun.PostProcessSpec{}, intermediateTypes)
		} else {
			p.AddNoGroupingStage(distinctSpec, distsqlrun.PostProcessSpec{}, intermediateTypes, p.MergeOrdering)
		}
	}
