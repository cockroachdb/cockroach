...
func (dsp *distSQLPlanner) addAggregators(
	...
	if !multiStage {
		...
		if needRender {
			...
			// We need to map our reduced set of final aggregations
			// and their respective indices in finalAggs as the
			// aggregation's output columns.
			// Note that before reduction, we had a 1-1 mapping
			// between finalAggs and aggregations.
			outputColumns := make([]uint32, len(aggregations))
			copy(outputColumns, finalIdxMap)
			finalAggsPost.OutputColumns = outputColumns
