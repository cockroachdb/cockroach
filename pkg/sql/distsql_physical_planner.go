...
func (dsp *DistSQLPlanner) createTableReaders(
	planCtx *PlanningCtx, n *scanNode, overrideResultColumns []sqlbase.ColumnID, batchSize int64,
) (PhysicalPlan, error) {

	scanNodeToTableOrdinalMap := getScanNodeToTableOrdinalMap(n)
	spec, post, err := initTableReaderSpec(n, planCtx, scanNodeToTableOrdinalMap, batchSize)
	if err != nil {
...
func (dsp *DistSQLPlanner) createPlanForIndexJoin(
	...
	plan, err := dsp.createTableReaders(
		planCtx, n.index, n.index.desc.PrimaryIndex.ColumnIDs, row.DefaultKVBatchSize,
	)
	if err != nil {
		return PhysicalPlan{}, err
	}
...
func (dsp *DistSQLPlanner) createPlanForNode(
	...
	switch n := node.(type) {
	case *scanNode:
		plan, err = dsp.createTableReaders(planCtx, n, nil, row.DefaultKVBatchSize)
