...
func (dsp *distSQLPlanner) partitionSpans(
	...
	for _, span := range spans {
		...
		// lastKey maintains the EndKey of the last span piece.
		lastKey := rspan.Key
		for it.Seek(ctx, span, kv.Ascending); ; it.Next(ctx) {
			...
			if lastKey.Less(desc.StartKey) || !lastKey.Less(desc.EndKey) {
				// This range must contain the last range's EndKey.
				log.Fatalf(
					ctx, "next range doesn't cover last end key: %#v %v",
					splits, roachpb.RSpan{Key: desc.StartKey, EndKey: desc.EndKey},
				)
			}
