...
func (dsp *DistSQLPlanner) createPlanForProjectSet(
	...
	for i := range projectSetSpec.GeneratedColumns {
		plan.planToStreamColMap = append(plan.planToStreamColMap, len(plan.ResultTypes)+i)
	}

	plan.SetMergeOrdering(dsp.convertOrdering(planPhysicalProps(n), plan.planToStreamColMap))
