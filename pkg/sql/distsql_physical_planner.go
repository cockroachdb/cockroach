...
func (dsp *DistSQLPlanner) createPlanForVectorSearch(
	...
	// VectorSearch operators fetch the primary key columns.
	primaryIndex := n.table.GetPrimaryIndex()
	fetchCols := make([]descpb.ColumnID, primaryIndex.NumKeyColumns())
	for i := range primaryIndex.NumKeyColumns() {
		fetchCols[i] = primaryIndex.GetKeyColumnID(i)
	}
	if err := rowenc.InitIndexFetchSpec(
		&spec.FetchSpec,
		planCtx.ExtendedEvalCtx.Codec,
