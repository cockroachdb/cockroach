...
func (dsp *DistSQLPlanner) createPlanForJoin(
	...
	if isLookupJoin {
		...
		if len(rightEqCols) > len(lookupJoinScan.index.ColumnIDs) {
			...
			return physicalPlan{}, errors.Errorf("%d equality columns specified, expecting a maximum of %d",
				len(rightEqCols), len(lookupJoinScan.index.ColumnIDs))
		}
	...
	if forceLookupJoin && !isLookupJoin {
		log.Warningf(planCtx.ctx, "lookup join was forced, but could not be used")
	}
	...
	if isLookupJoin {
		lookupExpr := shiftExprCols(
			planCtx.EvalContext(), lookupJoinScan.filter, rightPlan.planToStreamColMap, leftPlan.planToStreamColMap,
		)
		...
		core.JoinReader = &distsqlrun.JoinReaderSpec{
			...
			IndexIdx:      uint32(lookupJoinScan.index.ID) - 1,
			LookupColumns: lookupCols,
			RightSpans:    lookupJoinScan.spans,
			IndexMap:      indexMap,
			OnExpr:        onExpr,
		}
