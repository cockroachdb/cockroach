...
func maybeMoveSingleFlowToGateway(planCtx *PlanningCtx, plan *PhysicalPlan, rowCount int64) {
	if !planCtx.isLocal && planCtx.ExtendedEvalCtx.SessionData().DistSQLMode != sessiondatapb.DistSQLAlways {
		...
		// don't have an estimate (rowCount is 0) or the data cardinality is
		// reduced significantly (the reduction ratio exceeds 10), we will keep
		// the plan as is.
...
func (dsp *DistSQLPlanner) FinalizePlan(planCtx *PlanningCtx, plan *PhysicalPlan) {
	dsp.finalizePlanWithRowCount(planCtx, plan, 0 /* rowCount */)
}
...
// - rowCount is the estimated number of rows that the plan outputs. Use 0 if
