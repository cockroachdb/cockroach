...
type windowFuncsSorter struct {
	funcs []*windowFuncHolder
}

func (w *windowFuncsSorter) Len() int      { return len(w.funcs) }
func (w *windowFuncsSorter) Swap(i, j int) { w.funcs[i], w.funcs[j] = w.funcs[j], w.funcs[i] }
func (w *windowFuncsSorter) Less(i, j int) bool {
	return w.funcs[i].argIdxStart < w.funcs[j].argIdxStart
}
...
// We also adjust argIdxStart for already processed window functions to correctly
// point at the output column of that window function during all stages.
//
...
func (dsp *DistSQLPlanner) createPlanForWindow(
	...
	// Sort window functions by their ardIdxStart.
	sort.Sort(&windowFuncsSorter{funcs: n.funcs})
	...
	for numWindowFuncProcessed < len(n.funcs) {
		...
		for i, fInProgress := range samePartitionFuncs {
			if fInProgress.argCount != 1 {
				...
				for _, f := range n.funcs[samePartitionFuncsIdx[i]+1:] {
	...
	if needRendering {
		...
		windowFuncToOrdinalMap := make(map[*windowFuncHolder]int)
		for _, windowFn := range n.funcs {
			// We maintained argIdxStart of all window functions
			// to point at the output of respective windowFn
			// throughout all stages of windower processors.
			windowFuncToOrdinalMap[windowFn] = windowFn.argIdxStart
		}

		renderExprs := make([]tree.TypedExpr, len(n.windowRender))
		visitor := replaceWindowFuncsVisitor{
			windowFuncToOrdinalMap: windowFuncToOrdinalMap,
