...
const (
	...
	// SpanPartitionReason_MIXED_PROCESS_GATEWAY_ON_ERROR is reported in a
	// mixed-process cluster when there is an error and so we default to the
	// gateway node
	SpanPartitionReason_MIXED_PROCESS_GATEWAY_ON_ERROR SpanPartitionReason = 1
	// SpanPartitionReason_MIXED_PROCESS_GATEWAY_TARGET_UNHEALTHY is reported in a
	// mixed-process cluster when the target node is unhealthy and so we default
	// to the gateway node
	SpanPartitionReason_MIXED_PROCESS_GATEWAY_TARGET_UNHEALTHY SpanPartitionReason = 2
	// SpanPartitionReason_MIXED_PROCESS_TARGET_HEALTHY is reported in a
	// mixed-process cluster when the target node is healthy.
	SpanPartitionReason_MIXED_PROCESS_TARGET_HEALTHY SpanPartitionReason = 3
	...
	SpanPartitionReason_GATEWAY_NO_HEALTHY_INSTANCES SpanPartitionReason = 4
	// SpanPartitionReason_GATEWAY_ON_ERROR is reported when there is an error and
	// so we default to the gateway node.
	SpanPartitionReason_GATEWAY_ON_ERROR SpanPartitionReason = 5
	...
	SpanPartitionReason_CLOSEST_LOCALITY_MATCH SpanPartitionReason = 6
	...
	SpanPartitionReason_GATEWAY_NO_LOCALITY_MATCH SpanPartitionReason = 7
	...
	SpanPartitionReason_LOCALITY_AWARE_RANDOM SpanPartitionReason = 8
	...
	SpanPartitionReason_ROUND_ROBIN SpanPartitionReason = 9
	...
	SpanPartitionReason_GOSSIP_GATEWAY_TARGET_UNHEALTHY SpanPartitionReason = 10
	// SpanPartitionReason_GOSSIP_TARGET_HEALTHY is reported when the
	// target node retrieved via gossip is deemed healthy.
	SpanPartitionReason_GOSSIP_TARGET_HEALTHY SpanPartitionReason = 11
)
...
func (r SpanPartitionReason) String() string {
	...
	case SpanPartitionReason_UNSPECIFIED:
		return ""
	case SpanPartitionReason_MIXED_PROCESS_GATEWAY_ON_ERROR:
		return "mixed-process-gateway-on-error"
	case SpanPartitionReason_MIXED_PROCESS_GATEWAY_TARGET_UNHEALTHY:
		return "mixed-process-gateway-target-unhealthy"
	case SpanPartitionReason_MIXED_PROCESS_TARGET_HEALTHY:
		return "mixed-process-target-healthy"
		...
		*ignoreMisplannedRanges = *ignoreMisplannedRanges || ignore
		...
		if log.ExpensiveLogEnabled(ctx, 2) {
			log.VEventf(ctx, 2, "partition span: %s, instance ID: %d, reason: %s",
				span.String(), sqlInstanceID, reason.String())
		}
...
func (dsp *DistSQLPlanner) healthySQLInstanceIDForKVNodeHostedInstanceResolver(
	...
	if err != nil {
		...
		return dsp.alwaysUseGatewayWithReason(SpanPartitionReason_MIXED_PROCESS_GATEWAY_ON_ERROR)
	}
	...
	return func(nodeID roachpb.NodeID) (base.SQLInstanceID, SpanPartitionReason) {
		...
		if _, ok := healthyNodes[sqlInstance]; ok {
			return sqlInstance, SpanPartitionReason_MIXED_PROCESS_TARGET_HEALTHY
		}
		log.Warningf(ctx, "not planning on node %d", sqlInstance)
		return dsp.gatewaySQLInstanceID, SpanPartitionReason_MIXED_PROCESS_GATEWAY_TARGET_UNHEALTHY
	}
}
