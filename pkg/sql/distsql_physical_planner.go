		...
		resolver := func(nodeID roachpb.NodeID) (base.SQLInstanceID, SpanPartitionReason) {
			...
			if planCtx.strictFiltering {
				...
				if !matched {
					...
					// this plan should return an error. In strict mode, the the caller of
					// the instance resolver will error if the partition reason is not
					// CLOSEST_LOCALITY_MATCH or TARGET HEALTHY. So, to ensure the plan
					// fails, return a different reason-- it doesnt matter which one.
