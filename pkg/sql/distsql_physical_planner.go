...
func (dsp *distSQLPlanner) CheckSupport(tree planNode) (shouldRunDist bool, notSuppErr error) {
	...
	case *selectNode:
		...
		shouldRunDist, err := dsp.CheckSupport(n.source.plan)
		if err != nil {
			return false, err
		}
		// If we have to sort, distribute the query.
		shouldRunDist = shouldRunDist || (n.top.sort != nil && n.top.sort.needSort)
		return shouldRunDist, nil
	...
	case *joinNode:
		...
		shouldRunDist = shouldRunDistLeft || shouldRunDistRight
		// If we can do a hash join, we should distribute.
		shouldRunDist = shouldRunDist || len(n.pred.leftEqualityIndices) > 0
		return shouldRunDist, nil
...
func (dsp *distSQLPlanner) createPlanForJoin(
	...
	for bucket := 0; bucket < len(nodes); bucket++ {
		...
		dsp.mergeResultStreams(&p, leftRouters, bucket, distsql.Ordering{}, pIdx, 0)
		// Connect right routers to the processor's second input.
		dsp.mergeResultStreams(&p, rightRouters, bucket, distsql.Ordering{}, pIdx, 1)
