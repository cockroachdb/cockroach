...
func (l *DistLoader) LoadCSV(
	...
	for i, input := range from {
		...
		// TODO(mjibson): fill in args with CSV specs (sample size, table desc, etc.)
		args := []byte(input)
		rps := distsqlrun.RegisteredProcessorSpec{
			Name: "csv",
			Args: args,
		}
		...
		proc := distsqlplan.Processor{
			Node: node.NodeID,
			Spec: distsqlrun.ProcessorSpec{
				Core:    distsqlrun.ProcessorCoreUnion{Registered: &rps},
				Output:  []distsqlrun.OutputRouterSpec{{Type: distsqlrun.OutputRouterSpec_PASS_THROUGH}},
				StageID: stageID,
			},
		}
