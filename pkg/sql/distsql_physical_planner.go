...
type DistSQLPlanner struct {
	...
	// for the same stage of the fully local physical plan.
...
type PlanningCtx struct {
	...
	// onFlowCleanup contains non-nil functions that after the local flow
	// finished running and is being cleaned up. It allows us to release the
	// resources that are acquired during the physical planning and are being
	// hold onto throughout the whole flow lifecycle.
	onFlowCleanup []func()
}
...
// local flow finished running. This needs to be called only after the physical
...
func (dsp *DistSQLPlanner) planTableReaders(
	...
	if planCtx.isLocal {
		...
		// significantly worse, so we prohibit it. This is the case because if
		// we have a local region hit, we would still execute all lookups into
		// the remote regions and would block until all come back in the
		// row-based flow.
		...
		if len(info.reqOrdering) == 0 &&
			...
			if len(spanPartitions) > 1 {
				// We're touching ranges that have leaseholders on multiple
				// nodes, so it'd be beneficial to parallelize such a scan.
				// Determine the desired concurrency.
				const maxConcurrency = 64
				actualConcurrency := len(spanPartitions)
				if actualConcurrency > maxConcurrency {
					actualConcurrency = maxConcurrency
				}
				// alloc will be non-nil only if actualConcurrency remains above
				// 1.
				...
				if actualConcurrency > 1 {
					...
					//
					// Now we might need to merge some span partitions together.
					// We will keep first actualConcurrency partitions and will
					// append into them all "extra" span partitions in an
					// alternating fashion.
