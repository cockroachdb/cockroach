...
func (dsp *DistSQLPlanner) createPlanForSetOp(
	...
	// Even if the left and right planToStreamColMaps are mismatched,
	// they will have the same length, otherwise the logical plan would
	// not have been constructed.
	planCols := make([]int, 0, len(leftPlan.planToStreamColMap))
	streamCols := make([]uint32, 0, len(leftPlan.planToStreamColMap))
	for planCol, streamCol := range leftPlan.planToStreamColMap {
		if streamCol < 0 {
			continue
		}
	...
	var leftDistinctOrd, rightDistinctOrd distsqlrun.Ordering
	var leftDistinctProps, rightDistinctProps physicalProps
	var distinctSpecs [2]distsqlrun.ProcessorCoreUnion

	if !n.all {
		leftDistinctProps = leftProps.project(planCols)
		rightDistinctProps = rightProps.project(planCols)

		leftDistinctOrd = distsqlrun.ConvertToMappedSpecOrdering(
			leftDistinctProps.ordering, leftPlan.planToStreamColMap,
		)
		rightDistinctOrd = distsqlrun.ConvertToMappedSpecOrdering(
			rightDistinctProps.ordering, rightPlan.planToStreamColMap,
		)

		distinctOrds := []distsqlrun.Ordering{leftDistinctOrd, rightDistinctOrd}
	...
	if n.unionType != tree.UnionOp {
		...
		var newOrdering sqlbase.ColumnOrdering
		if n.all {
			newOrdering = computeMergeJoinOrdering(
				leftProps, rightProps, planCols, planCols,
			)
		} else {
			newOrdering = computeMergeJoinOrdering(
				leftDistinctProps, rightDistinctProps, planCols, planCols,
			)
		}
