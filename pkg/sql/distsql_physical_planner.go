...
type PlanningCtx struct {
	...
	// Nodes contains info for all NodeIDs that are referenced by any PhysicalPlan
	// we generate with this context.
...
func (dsp *DistSQLPlanner) CheckNodeHealthAndVersion(
	...
	if status, ok := planCtx.Nodes[nodeID]; ok {
		return status
	}
...
func (dsp *DistSQLPlanner) NewPlanningCtx(
	...
	planCtx.Nodes = make(map[roachpb.NodeID]NodeStatus)
	planCtx.Nodes[dsp.nodeDesc.NodeID] = NodeOK
	return planCtx
}
