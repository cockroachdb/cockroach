...
// post-processing step to the last stage.
func addFilter(p *physicalPlan, expr parser.TypedExpr, columnMap []int) {
	post := p.getLastStagePost()

	// The filter expression variables (after remapping via the columnMap) refer
	// to output columns of this processor. The filter in a processor's
	// PostProcessSpec refers to variables before any output projection; so if
	// there is an output projection, we have to take it into account and generate
	// a composite map.  For example:
	//
	//  TableReader // table columns A,B,C,D
	//  OutputColumns:       0, 2  //  A, C
	//
	//  Filter:              IndexVar(0) < IndexVar(1)  //  A < C
	//  columnMap:           0, 1  // identity
	//  compositeMap:        0, 2
	//  Remapped expression: IndexVar(0) < IndexVar(2)

	var compositeMap []int
	if len(post.OutputColumns) == 0 {
		compositeMap = columnMap
	} else {
		compositeMap = make([]int, len(columnMap))
		for i, col := range columnMap {
			if col == -1 {
				compositeMap[i] = -1
			} else {
