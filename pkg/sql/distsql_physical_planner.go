...
func (dsp *DistSQLPlanner) NewPlanningCtx(
	...
	planCtx := &PlanningCtx{
		...
		isLocal:         !distribute,
		planner:         planner,
		// Make sure to release the physical infrastructure after the physical
		// planning is done. Note that onFlowCleanup might not be called in some
		// cases (when DistSQLPlanner.Run is not called), but that is ok since
		// on the main query path it will get called.
