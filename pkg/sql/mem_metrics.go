...
func MakeMemMetrics(endpoint string, histogramWindow time.Duration) MemoryMetrics {
	...
	return MemoryMetrics{
		...
		TxnMaxBytesHist:      metric.NewHistogram(MetaMemMaxTxnBytes, histogramWindow, metric.MemoryUsage16MBBuckets),
		TxnCurBytesCount:     metric.NewGauge(MetaMemTxnCurBytes),
		SessionMaxBytesHist:  metric.NewHistogram(MetaMemMaxSessionBytes, histogramWindow, metric.MemoryUsage16MBBuckets),
