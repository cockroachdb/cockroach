...
func (p *planner) ShowTenant(ctx context.Context, n *tree.ShowTenant) (planNode, error) {
	if err := canControlReplication(ctx, p); err != nil {
		return nil, err
	}
...
func canControlReplication(ctx context.Context, p AuthorizationAccessor) error {
	...
	return p.CheckPrivilege(ctx, syntheticprivilege.GlobalPrivilegeObject, privilege.CONTROLREPLICATION)
}
