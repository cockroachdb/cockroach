...
func (n *showTenantNode) initReplicationStats(
	params runParams, tenantName roachpb.TenantName, job *jobs.Job,
) error {
...
func (n *showTenantNode) startExec(params runParams) error {
	...
	if jobId == 0 {
		...
		if n.withReplication {
			return errors.Newf("tenant %q is not a replication target", tenantName)
		}
	...
	case descpb.TenantInfo_ADD:
		...
		if err := n.initReplicationStats(params, tenantName, job); err != nil {
			return err
		}
