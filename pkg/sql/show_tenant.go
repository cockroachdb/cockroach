...
func (n *showTenantNode) getTenantValues(
	...
	values.tenantInfo = tenantRecord
	jobId := values.tenantInfo.TenantReplicationJobID
	if jobId == 0 {
		// No replication job, this is a non-replicating tenant.
		if n.withReplication {
			return nil, errors.Newf("tenant %q is not a replicated tenant", tenantName)
		}
	...
	case descpb.TenantInfo_ADD:
		...
		case jobs.StatusPending, jobs.StatusRunning, jobs.StatusPauseRequested:
			if values.replicationInfo == nil || values.replicationInfo.ReplicationLagInfo == nil {
				// Still no lag info which means we never recorded progress, and
				// replication did not complete the initial scan yet.
				values.tenantStatus = initReplication
			} else {
				progress := values.replicationInfo.IngestionProgress
				if progress != nil && !progress.CutoverTime.IsEmpty() {
					values.tenantStatus = cuttingOver
				} else {
					values.tenantStatus = replicating
				}
			}
		case jobs.StatusPaused:
			values.tenantStatus = replicationPaused
		default:
			values.tenantStatus = tenantStatus(fmt.Sprintf(string(replicationUnknownFormat), job.Status()))
		}
	case descpb.TenantInfo_ACTIVE, descpb.TenantInfo_DROP:
		values.tenantStatus = tenantStatus(values.tenantInfo.State.String())
	default:
		status := fmt.Sprintf("tenant status: %s", values.tenantInfo.State)
		values.tenantStatus = tenantStatus(fmt.Sprintf(string(replicationUnknownFormat), status))
	}
	return &values, nil
}

func (n *showTenantNode) Next(params runParams) (bool, error) {
	if n.row >= len(n.names) {
		return false, nil
	}
	values, err := n.getTenantValues(params, n.names[n.row])
	if err != nil {
		return false, err
	}
	n.values = values
