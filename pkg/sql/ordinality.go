...
func (p *planner) wrapOrdinality(ds planDataSource) planDataSource {
	...
	entry, _ := ds.info.sourceAliases[parser.TableName{}]
	entry = append(entry, len(res.columns)-1)
	ds.info.sourceAliases[parser.TableName{}] = entry

	res.ordering = src.Ordering()
...
func (o *ordinalityNode) expandPlan() error {
	...
	// We are going to "optimize" the ordering. First retrieve the
	// ordering of the source again, since it might have changed from
	// construction time.
	origOrdering := o.source.Ordering()

	fmt.Fprintf(os.Stderr, "WAAIA %s\n", origOrdering.AsString(o.columns))
