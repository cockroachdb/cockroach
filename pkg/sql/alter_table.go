...
func (p *planner) checkSchemaChangeIsAllowed(
	...
	case *tree.AlterTable:
		for _, cmd := range stmt.Cmds {
			switch cmd.(type) {
			case *tree.AlterTableDropStored, *tree.AlterTableRenameColumn,
				*tree.AlterTableRenameConstraint, *tree.AlterTableValidateConstraint,
	...
	if !tree.IsSetOrResetSchemaLocked(n) &&
		(!p.extendedEvalCtx.TxnIsSingleStmt || !p.extendedEvalCtx.TxnImplicit) {
		unsupportedCmd = true
	}
	...
	p.extendedEvalCtx.ResetSchemaLockedOnCommit = desc.IsSchemaLocked() &&
		!tree.IsSetOrResetSchemaLocked(n)
