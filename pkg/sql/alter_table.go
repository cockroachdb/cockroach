	...
	for i, cmd := range n.n.Cmds {
		...
		case *tree.AlterTableDropColumn:
			...
			for _, check := range n.tableDesc.Checks {
				if used, err := check.UsesColumn(n.tableDesc.TableDescriptor, col.ID); err != nil {
					return err
				} else if !used {
		...
		case *tree.AlterTableValidateConstraint:
			...
			case sqlbase.ConstraintTypeCheck:
				...
				if err := params.p.validateCheckExpr(
					params.ctx, ck.Expr, &n.n.Table, n.tableDesc.TableDescriptor,
				); err != nil {
					return err
				}
			...
			case sqlbase.ConstraintTypeFK:
				...
				if err := params.p.validateForeignKey(params.ctx, n.tableDesc.TableDescriptor, idx); err != nil {
					return err
				}
		...
		case *tree.AlterTablePartitionBy:
			...
			err = deleteRemovedPartitionZoneConfigs(
				...
				n.tableDesc.TableDescriptor, &n.tableDesc.PrimaryIndex, &n.tableDesc.PrimaryIndex.Partitioning,
		...
		case *tree.AlterTableSetAudit:
			var err error
			descriptorChanged, err = params.p.setAuditMode(params.ctx, n.tableDesc.TableDescriptor, t.Mode)
			if err != nil {
				return err
			}
		...
		case *tree.AlterTableInjectStats:
			...
			if err := injectTableStats(params, n.tableDesc.TableDescriptor, sd); err != nil {
				return err
			}
