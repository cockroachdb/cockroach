...
func (n *alterTableNode) startExec(params runParams) error {
	...
	for i, cmd := range n.n.Cmds {
		...
		case *tree.AlterTableAlterPrimaryKey:
			...
			// TODO (rohany,solongordon): This should be changed to see if any of the InboundFK's depend on indexes
			//  that are being rewritten.
			if len(n.tableDesc.InboundFKs) > 0 {
				return errors.New(
			...
			if len(n.tableDesc.PrimaryIndex.ColumnNames) == 1 && n.tableDesc.PrimaryIndex.ColumnNames[0] != "rowid" {
				...
				// TODO (rohany): what if this name clashes with an existing index name?
				oldPrimaryIndexCopy.Name = "old_primary_key"
				oldPrimaryIndexCopy.StoreColumnIDs = nil
				oldPrimaryIndexCopy.StoreColumnNames = nil
				if err := addIndexWithSpecificPrimaryKey(n.tableDesc, oldPrimaryIndexCopy, newPrimaryIndexDesc); err != nil {
					return err
				}
			}
			...
			for _, idx := range indexesToRewrite {
				...
				if err := addIndexWithSpecificPrimaryKey(n.tableDesc, newIndex, newPrimaryIndexDesc); err != nil {
					return err
				}
			...
			// N.B. We don't schedule index deletions here because the existing indexes need to be visibile to the user
...
// addIndexWithSpecificPrimaryKey adds an index mutation into the given table descriptor, but sets up
// the index with ExtraColumnIDs from the given index, rather than the table's primary key.
func addIndexWithSpecificPrimaryKey(
