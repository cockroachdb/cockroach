...
func (n *alterTableNode) Start(ctx context.Context) error {
	...
	for _, cmd := range n.n.Cmds {
		...
		case *parser.AlterTableAddColumn:
			...
			_, err = n.tableDesc.FindColumnByName(d.Name)
			if err == sqlbase.ErrColumnBeingDropped {
		...
		case *parser.AlterTableDropColumn:
			col, err := n.tableDesc.FindColumnByName(t.Column)
			if err != nil {
				if t.IfExists {
					// Noop.
					continue
				...
				if err == sqlbase.ErrColumnBeingDropped {
					return fmt.Errorf("column %q does not exist", t.Column)
				}
				return err
			}
		...
		case parser.ColumnMutationCmd:
			// Column mutations
			col, err := n.tableDesc.FindColumnByName(t.GetColumn())
			if err != nil {
				if err == sqlbase.ErrColumnBeingDropped {
