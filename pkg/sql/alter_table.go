	...
	for i := 0; i < len(n.n.Cmds); i++ {
		...
		case *tree.AlterTableAddConstraint:
			...
			case *tree.UniqueConstraintTableDef:
				if d.PrimaryKey {
					// If a primary key was requested to be added, insert an AlterPrimaryKey statement
					// to be processed after this constraint addition. However, only succeed if the primary key
					// has been dropped before this command.
					foundDropPK := false
					...
					if !foundDropPK {
						return errors.Errorf(
							"cannot add primary key to table %s without preceding drop of primary key", n.tableDesc.Name)
					}
					alterPK := &tree.AlterTableAlterPrimaryKey{
						Columns:    d.Columns,
						Interleave: d.Interleave,
					}
					// Slice n.n.Cmds to capacity i+1 to avoid the addition of alterPK overwriting other commands in the slice.
					n.n.Cmds = append(append(n.n.Cmds[:i+1:i+1], alterPK), n.n.Cmds[i+1:]...)
					continue
				}
