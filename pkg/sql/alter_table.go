...
func (n *alterTableNode) startExec(params runParams) error {
	...
	for i, cmd := range n.n.Cmds {
		...
		case *tree.AlterTableAddColumn:
			...
			if d.HasDefaultExpr() {
				changedSeqDescs, err := maybeAddSequenceDependencies(params.ctx, params.p, n.tableDesc, col, expr, make(map[sqlbase.ID]*sqlbase.MutableTableDescriptor))
				if err != nil {
					return err
				}
...
func applyColumnMutation(
	...
	case *tree.AlterTableSetDefault:
		...
		if t.Default == nil {
			...
			changedSeqDescs, err := maybeAddSequenceDependencies(params.ctx, params.p, tableDesc, col, expr, make(map[sqlbase.ID]*sqlbase.MutableTableDescriptor))
			if err != nil {
				return err
			}
			for _, changedSeqDesc := range changedSeqDescs {
