...
func (n *alterTableNode) startExec(params runParams) error {
	...
	for i, cmd := range n.n.Cmds {
		...
		case *tree.AlterTableDropColumn:
			...
			columnNum, err := n.tableDesc.FindColumnNumberByName(t.Column)
			if err != nil {
				return err
			}
			if err := params.p.removeColumnComment(params.ctx, n.tableDesc.ID, columnNum); err != nil {
				return err
			}
...
func (p *planner) removeColumnComment(
	ctx context.Context, tableID sqlbase.ID, columnNum int,
) error {
