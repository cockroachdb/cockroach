	...
	for i := 0; i < len(n.n.Cmds); i++ {
		...
		case *tree.AlterTableAddConstraint:
			...
			case *tree.UniqueConstraintTableDef:
				if d.PrimaryKey {
					// If a primary key was requested to be added, insert an AlterPrimaryKey
					// statement to be processed after this constraint addition. However,
					// only succeed if the primary key has been dropped before this command.
					foundDropPK := false
					currentMutationID := n.tableDesc.ClusterVersion.NextMutationID
					for _, mut := range n.tableDesc.Mutations {
						if mut.MutationID == currentMutationID && mut.GetDropPrimaryKey() != nil {
							foundDropPK = true
						}
					}
					if !foundDropPK {
						return errors.Errorf("multiple primary keys for table %s are not allowed", n.tableDesc.Name)
					}
					...
					// Move n.n.Cmds into a new array to avoid overwriting existing commands.
					newCmds := make(tree.AlterTableCmds, 0, len(n.n.Cmds)+1)
					newCmds = append(newCmds, n.n.Cmds[:i+1]...)
					newCmds = append(newCmds, alterPK)
					n.n.Cmds = append(newCmds, n.n.Cmds[i+1:]...)
					continue
				}
		...
		case *tree.AlterTableAlterPrimaryKey:
			...
			// If we find that a DROP PRIMARY KEY was requested previously in this transaction,
			// then we are not going to rebuild the primary key as a secondary index.
			foundDropPK := false
			for _, mut := range n.tableDesc.Mutations {
				if mut.MutationID == currentMutationID && mut.GetDropPrimaryKey() != nil {
					foundDropPK = true
			...
			if !foundDropPK && !n.tableDesc.IsPrimaryIndexDefaultRowID() {
