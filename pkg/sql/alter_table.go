	...
	for i, cmd := range n.n.Cmds {
		...
		case *tree.AlterTableAddColumn:
			...
			if !n.tableDesc.IsInterleaved() {
				n.tableDesc.AddingColumns = append(n.tableDesc.AddingColumns, *col)
				if err := n.tableDesc.AllocateIDs(); err != nil {
					return err
				}
				if err := params.p.MakePkSwap(params.ctx, n.tableDesc); err != nil {
					return err
				}
			} else {
				// Cannot add column using primary key swap if the table is interleaved.
...
// MakePkSwap makes a copy of the current PrimaryIndex as a SecondaryIndex
// ready to be queued up for an AddIndexMutation and made into the
// PrimaryIndex via a PrimaryKeySwap.
func (p *planner) MakePkSwap(
	ctx context.Context,
	tableDesc *MutableTableDescriptor,
) error {
	// Adding via PK swap.
	pkCopy := protoutil.Clone(&tableDesc.PrimaryIndex).(*sqlbase.IndexDescriptor)
	...
	//Set new PK's ID to 0 so AllocateID sets it later.
	pkCopy.ID = 0

	pkCopy.Name = name
	pkCopy.ColumnIDs = tableDesc.PrimaryIndex.ColumnIDs
	pkCopy.ColumnNames = tableDesc.PrimaryIndex.ColumnNames
	pkCopy.ColumnDirections = tableDesc.PrimaryIndex.ColumnDirections
	pkCopy.CreatedExplicitly = true
	pkCopy.EncodingType = sqlbase.PrimaryIndexEncoding
	pkCopy.Version = sqlbase.SecondaryIndexFamilyFormatVersion

	pkCopy.StoreColumnNames, pkCopy.StoreColumnIDs = nil, nil
	for _, c := range tableDesc.Columns {
		containsCol := false
		for _, colID := range pkCopy.ColumnIDs {
			if colID == c.ID {
				containsCol = true
				break
			}
		...
		if !containsCol {
			pkCopy.StoreColumnIDs = append(pkCopy.StoreColumnIDs, c.ID)
			pkCopy.StoreColumnNames = append(pkCopy.StoreColumnNames, c.Name)
		}
	}
	...
	if shouldCopyPrimaryKey(tableDesc, pkCopy) {
		...
		if err := addIndexMutationWithSpecificPrimaryKey(tableDesc, oldPrimaryIndexCopy, pkCopy); err != nil {
			return err
		}
	}
	...
	for i := range tableDesc.Indexes {
		...
		if idx.ID != pkCopy.ID && shouldRewriteIndex(idx) {
	...
	for _, idx := range indexesToRewrite {
		...
		if err := addIndexMutationWithSpecificPrimaryKey(tableDesc, newIndex, pkCopy); err != nil {
			return err
		}
	...
	if err := tableDesc.AddIndexMutation(pkCopy, sqlbase.DescriptorMutation_ADD); err != nil {
		return err
	}
