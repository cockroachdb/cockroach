...
func pushTableStats(
	...
	columnsUsed := make([][]descpb.ColumnID, len(jsonStats))

	// Insert each statistic using retention logic.
	for i := range jsonStats {
		s := &jsonStats[i]
		// Validation check that all the stats in one PUSH STATISTICS attempt
		// should belong to one stats collection, so they should be all auto or all manual,
		// and should be of the same creation timestamp.
		statsAuto := s.Name == jobspb.AutoStatsName || s.Name == jobspb.AutoPartialStatsName
		if i == 0 {
			isAuto = statsAuto
		} else {
			if isAuto != statsAuto {
				return errors.Newf("expect auto=%t, got auto=%t", isAuto, statsAuto)
			}
		}
