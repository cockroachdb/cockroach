...
func pushTableStats(
	...
	if shouldDeleteOtherStats && len(columnsUsed) > 0 {
		...
		columnIDsPlaceholders, placeHolderVals, keepTimeIdx, err := stats.GetPlaceholderValsFromColumnIDs(desc.TableDesc().ID, columnsUsed, keepTime)
		if err != nil {
			return err
		}
		return params.ExecCfg().InternalDB.DescsTxn(ctx, func(ctx context.Context, txn descs.Txn) error {
