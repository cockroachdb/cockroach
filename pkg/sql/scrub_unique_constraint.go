...
func (o *sqlUniqueConstraintCheckOperation) Start(params runParams) error {
	...
	//   (SELECT b, c FROM db.t GROUP BY b, c [WHERE partial index predicate]
	...
	predicate := ""
	if o.predicate != "" {
		predicate = fmt.Sprintf("WHERE %s", o.predicate)
	}
	sel := fmt.Sprintf(`SELECT %[1]s 
FROM %[2]s AS tbl1 JOIN 
(SELECT %[3]s FROM %[4]s %[5]s GROUP BY %[6]s HAVING COUNT(*) > 1) AS tbl2 
ON %[7]s
%[8]s `,
		strings.Join(pCols, ","),        // 1
		tableName,                       // 2
		strings.Join(keyCols, ","),      // 3
		tableName,                       // 4
		predicate,                       // 5
		strings.Join(keyCols, ","),      // 6
		strings.Join(matchers, " AND "), // 7
		asOf,                            // 8
	)
