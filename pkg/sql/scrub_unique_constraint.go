...
// sqlUniqueConstraintCheckOperation during local execution.
type sqlUniqueConstraintCheckRun struct {
	started  bool
	rows     []tree.Datums
	rowIndex int
}
...
func newSQLUniqueConstraintCheckOperation(
	...
	if constraint.UniqueWithoutIndexConstraint != nil {
		...
		if constraint.Index.Partitioning.NumColumns > 0 {
			op.cols = op.cols[constraint.Index.Partitioning.NumColumns:]
		}
...
func (o *sqlUniqueConstraintCheckOperation) Start(params runParams) error {
	...
	//   (SELECT b, c FROM db.t GROUP BY b, c HAVING COUNT(*) > 1) as tbl2
...
FROM %[2]s.%[3]s AS tbl1 JOIN 
(SELECT %[4]s FROM %[2]s.%[3]s GROUP BY %[4]s HAVING COUNT(*) > 1) AS tbl2 
ON %[5]s
%[6]s `,
		strings.Join(pCols, ","),
		o.tableName.Catalog(),
