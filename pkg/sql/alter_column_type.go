...
func alterColumnTypeGeneral(
	...
	if Using != nil {
		...
		replacedExpr, _, err := replaceVars(tableDesc, Using)
		if err != nil {
			return err
		}

		_, err = sqlbase.SanitizeVarFreeExpr(
			ctx, replacedExpr, toType, "ALTER COLUMN TYPE USING EXPRESSION", &params.p.semaCtx, true, /* allowImpure */
		)
		if err != nil {
			return err
		}
		...
		// Currently, if USING EXPRESION is used we always set the inverse to NULL.
		// We may support user provided inverses in the future.
		nullExpr := tree.Serialize(tree.DNull)
		inverseExpr = &nullExpr
	} else {
