...
const (
	...
	hashAggregatorSizeOfBucketsItem = 32
)
...
func (ag *hashAggregator) emitRow() (
	...
	*execinfrapb.ProducerMetadata,
	...
	if len(ag.bucketsIter) == 0 {
		...
		// We have accounted for ag.bucketsLenGrowThreshold / 2 items in
		// 'buckets' map (except for the case when that number if below our
		// initial length estimate - then, we have accounted for any items).
		accountedForMapItems := ag.bucketsLenGrowThreshold / 2
		if accountedForMapItems >= hashAggregatorBucketsInitialLen {
			ag.bucketsAcc.Shrink(ag.Ctx, int64(accountedForMapItems)*hashAggregatorSizeOfBucketsItem)
		}
		// Note that, for simplicity, we're ignoring the overhead of the slice of
		// strings.
...
func (ag *hashAggregator) accumulateRow(row sqlbase.EncDatumRow) error {
	...
	if !ok {
		...
		if len(ag.buckets) == ag.bucketsLenGrowThreshold {
			if err := ag.bucketsAcc.Grow(ag.Ctx, int64(ag.bucketsLenGrowThreshold)*hashAggregatorSizeOfBucketsItem); err != nil {
				return err
			}
