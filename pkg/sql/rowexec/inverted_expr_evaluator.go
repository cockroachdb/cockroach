...
type spansAndSetIndex struct {
	spans    []invertedexpr.InvertedSpan
	setIndex int
}
...
type invertedExprEvaluator struct {
	expr    *invertedexpr.SpanExpression
...
func newInvertedExprEvaluator(expr *invertedexpr.SpanExpression) *invertedExprEvaluator {
	eval := &invertedExprEvaluator{expr: expr}
	eval.setExpr = eval.initSetExpr(expr)
	return eval
}

func (ev *invertedExprEvaluator) initSetExpr(expr *invertedexpr.SpanExpression) *setExpression {
	// Assign it an index even if FactoredUnionSpans is empty, since we will
	// need it when evaluating.
	i := len(ev.sets)
	ev.sets = append(ev.sets, nil)
	sx := &setExpression{op: expr.Operator, unionSet: i}
	...
	if expr.Left != nil {
		sx.left = ev.initSetExpr(expr.Left.(*invertedexpr.SpanExpression))
	}
	if expr.Right != nil {
		sx.right = ev.initSetExpr(expr.Right.(*invertedexpr.SpanExpression))
	}
	return sx
}
...
func (ev *invertedExprEvaluator) evaluateSetExpr(sx *setExpression) setContainer {
	...
	return unionSetContainers(ev.sets[sx.unionSet], childrenSet)
}
...
// invertedValRoutingInfo contains the list of exprAndSetIndex pairs that
// needs rows from the inverted index span. A []invertedSpanRoutingInfo with
...
type invertedSpanRoutingInfo struct {
	span                invertedexpr.InvertedSpan
	exprAndSetIndexList []exprAndSetIndex
}

// batchedInvertedExprEvaluator is for evaluating one or more expressions.
...
// permitted, and is just a placeholder that will result in a nil []KeyIndex
// in evaluate(). getSpans() must be called before calls to addIndexRow() even
// if the result of getSpans() is not needed -- it builds the fragmentedSpans
// used for routing the added rows.
type batchedInvertedExprEvaluator struct {
	exprs []*invertedexpr.SpanExpression
...
// unioned since any row in that span needs to be routed to all these
...
func (b *batchedInvertedExprEvaluator) getSpans() []invertedexpr.InvertedSpan {
	...
	for i, expr := range b.exprs {
		...
		b.exprEvals[i] = newInvertedExprEvaluator(expr)
		exprSpans := b.exprEvals[i].getSpansAndSetIndex()
		for _, spans := range exprSpans {
			for _, span := range spans.spans {
				// TODO(sumeer): code is much simpler with [start, start\x00) instead of
				// [start, start]. Consider making this change in InvertedSpan itself.
				spanCopy := span
				if spanCopy.End == nil {
					spanCopy.End = roachpb.BytesNext(spanCopy.Start)
				}
	...
	var coveringSpans []invertedexpr.InvertedSpan
