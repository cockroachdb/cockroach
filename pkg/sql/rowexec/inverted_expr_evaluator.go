...
// the ? operator checks existence of a top-level key in the JSON value.
//
...
// getSpansAndOperandIndexes returns the spans and corresponding operand
// indexes for the expression it needs to execute. The spans are not in sorted
// order and can be overlapping. It also sets up internal state of the
// invertedExprEvaluator to prepare for calls to addIndexRow.
...
type exprAndOperand struct {
	...
	exprNum int
	// An index into batchedInvertedExprEvaluator.exprEvals[exprNum].operandSets.
	operandIndex int
}
...
// TODO(sumeer): add a high-level description of how
// batchedInvertedExprEvaluator works so details like the following have more
// context.
