...
func (sp *bulkRowWriter) work(ctx context.Context) error {
	...
	if sp.EvalCtx.Txn == nil {
		panic("sp.EvalCtx.Txn nil in bulk_row_writer.go")
	}
	semaCtx := sp.flowCtx.TypeResolverFactory.NewSemaContext(sp.EvalCtx.Txn)
	conv, err := row.NewDatumRowConverter(ctx,
		sp.tableDesc, nil /* targetColNames */, sp.EvalCtx, semaCtx, kvCh, nil /* seqChunkProvider */)
	if err != nil {
		return err
	}
