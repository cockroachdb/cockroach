...
func newJoinReader(
	...
	jr.initJoinReaderStrategy(flowCtx, jr.desc.ColumnTypesWithMutations(returnMutations), spec.MaintainOrdering)
...
func (jr *joinReader) initJoinReaderStrategy(
	flowCtx *execinfra.FlowCtx, typs []*types.T, maintainOrdering bool,
) {
	if !maintainOrdering {
...
func (jr *joinReader) readInput() (joinReaderState, *execinfrapb.ProducerMetadata) {
	...
	for k := range jr.keyToInputRowIndices {
		delete(jr.keyToInputRowIndices, k)
	}
	...
	jr.strategy.processLookupRows(jr.scratchInputRows)
	...
	for i, inputRow := range jr.scratchInputRows {
...
func (jr *joinReader) performLookup() (joinReaderState, *execinfrapb.ProducerMetadata) {
	...
	for {
		...
		if nextState, err := jr.strategy.processLookedUpRow(jr.Ctx, lookedUpRow, jr.keyToInputRowIndices[string(key)]); err != nil {
			jr.MoveToDraining(err)
			return jrStateUnknown, jr.DrainHelper()
