...
func newJoinReader(
	...
	if readerType != indexJoinReaderType {
		...
		jr.groupingState = &inputBatchGroupingState{noGrouping: !spec.LeftJoinWithPairedJoiner}
	}
	var err error
...
func (jr *joinReader) readInput() (
	...
	*execinfrapb.ProducerMetadata,
	...
	if jr.groupingState != nil && !jr.groupingState.noGrouping && len(jr.scratchInputRows) > 0 {
...
// - noGrouping is true: It is expected that for each input row in
//   a batch, addContinuationValForRow(false) will be called.
// - noGrouping is false: The join is functioning in a manner where
...
func (ib *inputBatchGroupingState) lastGroupMatched() bool {
	if ib.noGrouping || len(ib.groupMatched) == 0 {
		return false
	}
...
func (ib *inputBatchGroupingState) isUnmatched(rowIndex int) bool {
	...
	if !ib.noGrouping && groupNum == len(ib.groupMatched)-1 {
