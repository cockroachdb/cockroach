...
type joinReader struct {
	...
	// maintainOrdering controls whether the joinReader should maintain the
	// ordering of the input stream. This is applicable to both lookup joins and
	// index joins. Maintaining ordering with lookup joins is expensive. With
	// index joins, not maintaining ordering allows for optimizations at lower
	// layers.
...
func newJoinReader(
	...
	jr := &joinReader{
		...
		// choose parallelism when we know that the number of requests in a batch
		// has a relation to the number of results in the batch: in case of
		// indexJoinReaderType, we know that there's exactly one lookup row for each
		// input row. Similarly, in case of spec.LookupColumnsAreKey, we know that
		// there's at most one lookup row per input row. In other cases, we use
		// limits.
