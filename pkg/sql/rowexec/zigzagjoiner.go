...
func newZigzagJoiner(
	...
	leftColumnTypes := catalog.ColumnTypes(tables[0].PublicColumnsNew())
	rightColumnTypes := catalog.ColumnTypes(tables[1].PublicColumnsNew())
	...
	for i := 0; i < z.numTables; i++ {
		...
		colOffset += len(z.infos[i].table.PublicColumnsNew())
	}
	z.side = 0
	return z, nil
}
...
func (z *zigzagJoiner) setupInfo(
	...
	columnTypes := catalog.ColumnTypes(info.table.PublicColumnsNew())
	colIdxMap := catalog.ColumnIDToOrdinalMap(info.table.PublicColumnsNew())
	...
	maxCol := colOffset + len(info.table.PublicColumnsNew())
	...
	_, _, err := initRowFetcher(
		...
		info.table,
		int(indexOrdinal),
		catalog.ColumnIDToOrdinalMap(info.table.PublicColumnsNew()),
		false, /* reverse */
		neededCols,
		false, /* check */
		flowCtx.EvalCtx.Mon,
...
func (zi *zigzagJoinerInfo) eqColTypes() []*types.T {
	...
	colTypes := catalog.ColumnTypes(zi.table.PublicColumnsNew())
...
func (zi *zigzagJoinerInfo) eqOrdering() (colinfo.ColumnOrdering, error) {
	...
	for i := range zi.eqColumns {
		colID := zi.table.PublicColumnsNew()[zi.eqColumns[i]].GetID()
