...
func newFiltererProcessor(
	...
	resolver := flowCtx.TypeResolverFactory.NewTypeResolver(f.EvalCtx.Txn)
	if err := resolver.HydrateTypeSlice(f.EvalCtx.Context, types); err != nil {
		return nil, err
	}
	...
	if err := f.filter.Init(spec.Filter, types, &semaCtx, f.EvalCtx); err != nil {
		return nil, err
	}
