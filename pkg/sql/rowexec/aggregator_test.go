...
func BenchmarkAggregation(b *testing.B) {
	...
	for _, aggFunc := range aggFuncs {
		b.Run(aggFunc.String(), func(b *testing.B) {
			...
			input := execinfra.NewRepeatableRowSource(sql.OneIntCol, randgen.MakeIntRows(numRows, numCols))
...
func benchmarkAggregationWithGrouping(b *testing.B, numOrderedCols int) {
	...
	for _, aggFunc := range aggFuncs {
		b.Run(aggFunc.String(), func(b *testing.B) {
			...
			post := &execinfrapb.PostProcessSpec{}
			disposer := &rowDisposer{}
			input := execinfra.NewRepeatableRowSource(sql.ThreeIntCols, makeGroupedIntRows(groupSize, numCols, groupedCols[:]))
