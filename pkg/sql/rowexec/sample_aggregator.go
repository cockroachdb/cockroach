		...
		for _, si := range s.sketches {
			...
			if si.spec.PartialPredicate == "" {
				...
				if canaryEnabled {
					...
					// picked the stable path for stats selection, we will need to
					// reuse the stale stats.
					...
					if err := stats.MarkDelayDelete(ctx, txn, s.tableID, columnIDs); err != nil {
						return errors.Wrapf(err, "failed to marks rows for delayed deletion for table %d columns %v", s.tableID, columnIDs)
					}
				} else {
