	...
	if err := s.FlowCtx.Cfg.DB.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
		for _, si := range s.sketches {
			...
			if si.spec.GenerateHistogram {
				...
				lowerBound := tree.DNull
				if si.spec.LowerBound != "" {
					lbExpr, err := parser.ParseExpr(si.spec.LowerBound)
					if err != nil {
						return err
					}
...
func (s *sampleAggregator) generateHistogram(
	...
	if lowerBound != tree.DNull {
		var lowerValues tree.Datums
		var upperValues tree.Datums
		for _, val := range values {
			if c := val.Compare(evalCtx, lowerBound); c == -1 {
				lowerValues = append(lowerValues, val)
			} else {
				upperValues = append(upperValues, val)
			}
		}

		h, buckets, err := stats.ConstructExtremesHistogram(evalCtx, colType, int64(values.Len()), lowerValues, upperValues, numRows, distinctCount, maxBuckets)
		_ = buckets
		return h, err
	}
	...
	h, _, err := stats.EquiDepthHistogram(evalCtx, colType, values, numRows, distinctCount, maxBuckets, false /* partial */)
	return h, err
}
