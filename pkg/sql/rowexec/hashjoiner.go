...
func newHashJoiner(
	...
	h := &hashJoiner{
		...
		disableTempStorage: disableTempStorage,
		hashTableEmpty:     true,
	}
	...
	if !h.disableTempStorage {
		...
		hmrc := rowcontainer.MakeHashMemRowContainer(
			flowCtx.EvalCtx, h.MemMonitor, h.rightSource.OutputTypes(), h.eqCols[rightSide],
		)
		h.hashTable = &hmrc
	}
...
func (h *hashJoiner) build() (hashJoinerState, rowenc.EncDatumRow, *execinfrapb.ProducerMetadata) {
	for {
		...
		if row == nil {
			...
			// to short-circuit.
			if h.hashTableEmpty && h.joinType.IsEmptyOutputWhenRightIsEmpty() {
				h.MoveToDraining(nil /* err */)
				return hjStateUnknown, nil, h.DrainHelper()
			}
			// If hashTable is in-memory, pre-reserve the memory needed to mark.
			if rc, ok := h.hashTable.(*rowcontainer.HashMemRowContainer); ok {
				// h.hashTable is hashMemRowContainer and not a disk backed one,
				// so h.disableTempStorage is true and we cannot spill to disk,
				// so we simply will return an error if it occurs.
				err = rc.ReserveMarkMemoryMaybe(h.Ctx)
			} else if hdbrc, ok := h.hashTable.(*rowcontainer.HashDiskBackedRowContainer); ok {
				err = hdbrc.ReserveMarkMemoryMaybe(h.Ctx)
			} else {
				panic("unexpected type of hashTable in hashJoiner")
			}
			if err != nil {
				h.MoveToDraining(err)
				return hjStateUnknown, nil, h.DrainHelper()
