...
;`)
	...
	b.Run("LeftSemiJoin", func(b *testing.B) {
		runSemiAntiJoinBench(b, setup, func(b *testing.B, r *sqlutils.SQLRunner) {
			for i := 0; i < b.N; i++ {
				r.Exec(b, fmt.Sprintf(`SELECT * FROM ltable WHERE EXISTS (SELECT 1 FROM rtable WHERE rtable.v = ltable.v + %d)`, i))
			}
		})
	})
}
