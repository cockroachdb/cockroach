...
func (ij *invertedJoiner) generateSpans(invertedSpans []invertedSpan) (roachpb.Spans, error) {
	...
	for _, span := range invertedSpans {
		...
		// sqlbase.MakeSpanFromEncDatums when given a prefix of the index constructs
		// the start key, and then the end key using startKey.PrefixEnd(). If the
		// value being encoded has a fixed length encoding, this is tight. But for a
		// string this seems loose (the successor of "ab" is not "ac" -- there can be
		// many strings after "ab" and before "ac"). It seems one could do something
		// more accurate with knowledge of the Datum being encoded. The bug we have
		// is that MakeSingleInvertedValSpan() in the invertedexpr package, that we
		// also use in the test, does roachpb.BytesNext(). For a fixed length encoded
		// datum, like int, this is incorrect. I am inclined to remove
		// MakeSingleInvertedValSpan and have the callers, who are already encoding
		// the value in the inverted column, do a type dependent encoding.
		//
