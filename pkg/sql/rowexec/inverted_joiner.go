...
func newInvertedJoiner(
	...
	if ij.datumsToInvertedExpr == nil {
		...
		if ij.canPreFilter {
			ij.batchedExprEval.canPreFilter = ij.canPreFilter
			ij.batchedExprEval.preFilterer = ij.datumsToInvertedExpr
		}
	}
...
func (ij *invertedJoiner) Next() (rowenc.EncDatumRow, *execinfrapb.ProducerMetadata) {
	...
	preFilterCount, preFilterMatchCount := ij.datumsToInvertedExpr.PreFilterStats()
	// TODO: remove this temporary debugging aid.
	log.Errorf(ij.Ctx, "prefilter: total: %d, match: %d", preFilterCount, preFilterMatchCount)
...
func (ij *invertedJoiner) readInput() (invertedJoinerState, *execinfrapb.ProducerMetadata) {
	...
	for len(ij.inputRows) < ij.batchSize {
		...
		if expr == nil {
			...
			if ij.canPreFilter {
				preFilterState, err := ij.datumsToInvertedExpr.GetPreFilterStateForLastConvert(ij.Ctx)
				if err != nil {
					ij.MoveToDraining(err)
					return ijStateUnknown, ij.DrainHelper()
				}
...
func (ij *invertedJoiner) performScan() (invertedJoinerState, *execinfrapb.ProducerMetadata) {
	...
	for {
		...
		if shouldAdd {
			...
			if err = ij.batchedExprEval.addIndexRow(encInvertedVal, rowIdx); err != nil {
				ij.MoveToDraining(err)
				return ijStateUnknown, ij.DrainHelper()
			}
		}
