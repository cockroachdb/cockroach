...
func newInvertedJoiner(
	...
	if execinfra.SettingUseTempStorageJoins.Get(&st.SV) && false {
...
func (ij *invertedJoiner) performLookup() (invertedJoinerState, *execinfrapb.ProducerMetadata) {
	// nCols := len(ij.lookupCols)

	// isJoinTypePartialJoin := ij.joinType == sqlbase.LeftSemiJoin || ij.joinType == sqlbase.LeftAntiJoin
