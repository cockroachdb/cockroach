...
type invertedJoiner struct {
	...
	// storedRow represents a entry retrieved from the index. It includes the
	// non-inverted prefix columns in the order defined by the index descriptor
	// and any remaining primary key columns. These columns are stored because
	// they are emitted from the join. It does not include the inverted column
	// because its value cannot be constructed from an inverted index entry.
	//
	...
	// during prefiltering, and (3) stored in the storedRows row container for
	// de-duplicating by the PK.
	//
	// It is reused to reduce allocations.
	storedRow rowenc.EncDatumRow

	// storedRowTypes is a list of the types of each column in storedRow.
	storedRowTypes []*types.T

	// storedRowToTableRowMap is used to convert a tableRow to storedRow, and
	// vice versa. Two-way conversion is possible with a single map because we
	// iterate over all entries when converting.
	storedRowToTableRowMap []int
	...
	// The container for the stored rows retrieved from the index. For
	...
	// adding to storedRows -- this is more efficient since multiple expressions
	// may be using the same spans from the index. Note that De-duplicating by
	// the entire stored row, which includes non-inverted prefix columns for
	// multi-column inverted indexes, is equivalent to de-duplicating by the PK
	// because the all table columns are functionally dependent on the PK.
	storedRows *rowcontainer.DiskBackedNumberedRowContainer
...
func newInvertedJoiner(
	...
	// Initialize tableRow, storedRow, storedRowTypes, and
	// storedRowToTableRowMap, a mapping from storedRow column ordinal to tableRow column ordinals.
	...
	ij.storedRow = make(rowenc.EncDatumRow, len(indexColumnIDs)-1)
	ij.storedRowTypes = make([]*types.T, len(ij.storedRow))
	ij.storedRowToTableRowMap = make([]int, len(ij.storedRow))
	storedRowIdx := 0
	for _, colID := range indexColumnIDs {
		...
		ij.storedRowToTableRowMap[storedRowIdx] = tableRowIdx
		ij.storedRowTypes[storedRowIdx] = ij.desc.Columns[tableRowIdx].Type
		storedRowIdx++
	}
	...
	// Initialize memory monitors and row container for stored rows.
	...
	ij.storedRows = rowcontainer.NewDiskBackedNumberedRowContainer(
		true, /* deDup */
		ij.storedRowTypes,
		ij.EvalCtx,
		...
		if len(ij.prefixEqualityCols) > 0 {
			if expr == nil {
				...
				for prefixIdx, colIdx := range ij.prefixEqualityCols {
					ij.storedRow[prefixIdx] = row[colIdx]
				}
				...
				prefixKey, _, _, err := rowenc.MakeKeyFromEncDatums(
					ij.storedRow[:len(ij.prefixEqualityCols)],
					ij.storedRowTypes[:len(ij.prefixEqualityCols)],
...
func (ij *invertedJoiner) performScan() (invertedJoinerState, *execinfrapb.ProducerMetadata) {
	...
	for {
		...
		ij.transformToStoredRow(scannedRow)
		...
		if shouldAdd {
			rowIdx, err := ij.storedRows.AddRow(ij.Ctx, ij.storedRow)
			if err != nil {
				ij.MoveToDraining(err)
				return ijStateUnknown, ij.DrainHelper()
			}
	...
	ij.storedRows.SetupForRead(ij.Ctx, ij.joinedRowIdx)
...
func (ij *invertedJoiner) emitRow() (
	...
	*execinfrapb.ProducerMetadata,
	...
	if ij.emitCursor.inputRowIdx >= len(ij.joinedRowIdx) {
		...
		if err := ij.storedRows.UnsafeReset(ij.Ctx); err != nil {
			ij.MoveToDraining(err)
	...
	indexedRow, err := ij.storedRows.GetRow(ij.Ctx, joinedRowIdx, false /* skip */)
	if err != nil {
		ij.MoveToDraining(err)
		return ijStateUnknown, nil, ij.DrainHelper()
	}
	...
	skipRemaining := func() error {
		for ; ij.emitCursor.outputRowIdx < len(ij.joinedRowIdx[ij.emitCursor.inputRowIdx]); ij.emitCursor.outputRowIdx++ {
			...
			if _, err := ij.storedRows.GetRow(ij.Ctx, idx, true /* skip */); err != nil {
				return err
			}
		}
		return nil
...
func (ij *invertedJoiner) transformToStoredRow(row rowenc.EncDatumRow) {
	for keyIdx, rowIdx := range ij.storedRowToTableRowMap {
		ij.storedRow[keyIdx] = row[rowIdx]
	}
}

func (ij *invertedJoiner) transformToTableRow(storedRow rowenc.EncDatumRow) {
	for keyIdx, rowIdx := range ij.storedRowToTableRowMap {
		ij.tableRow[rowIdx] = storedRow[keyIdx]
	}
}
...
func (ij *invertedJoiner) close() {
	if ij.InternalClose() {
		...
		if ij.storedRows != nil {
			ij.storedRows.Close(ij.Ctx)
		}
