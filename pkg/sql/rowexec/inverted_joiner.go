...
type invertedJoiner struct {
	...
	// during prefiltering, and (3) adding to the storedRows row container.
	//
	...
	// in the batch of expressions, it is done once when adding to storedRows --
...
func (ij *invertedJoiner) performScan() (invertedJoinerState, *execinfrapb.ProducerMetadata) {
	...
	for {
		...
		if shouldAdd {
			rowIdx, err := ij.storedRows.AddRow(ij.Ctx, ij.indexRow)
			if err != nil {
				ij.MoveToDraining(err)
				return ijStateUnknown, ij.DrainHelper()
			}
