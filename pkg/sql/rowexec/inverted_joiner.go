...
type invertedJoiner struct {
	...
	// tableRowToKeyRowMap. The map is used to transform to the keyRow, and add
	// to the row container, which de-duplicates the primary keys. The index
	// assigned by the container is the keyIndex in the addIndexRow() call
	// mentioned earlier.
	...
	// retrieved from the inverted index. Instead of doing such ded-duplication
...
func newInvertedJoiner(
	...
	// TODO(sumeer): should this be visible columns or something else?
	...
	// TODO(sumeer): is the length of the following the same as
	// VisibleColumns()?
...
func (ij *invertedJoiner) emitRow() (
	...
	*execinfrapb.ProducerMetadata,
	...
	skipRemaining := func() {
		for i := ij.emitCursor.outputRowIdx; i < len(ij.joinedRowIdx[ij.emitCursor.inputRowIdx]); i++ {
			idx := ij.joinedRowIdx[ij.emitCursor.inputRowIdx][ij.emitCursor.outputRowIdx]
			ij.keyRows.GetRow(ij.Ctx, idx, true /* skip */)
		}
	}
	if renderedRow != nil {
