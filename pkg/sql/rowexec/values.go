...
type valuesProcessor struct {
	...
	// len(data) is zero. numRows is modified as rows are emitted.
	numRows uint64
...
func newValuesProcessor(
	...
	if len(spec.Columns) > 0 && uint64(len(spec.RawBytes)) != spec.NumRows {
		panic(errors.AssertionFailedf(
			"malformed ValuesCoreSpec: rows %d != NumRows %d", len(spec.RawBytes), spec.NumRows,
		))
	}
	v := &valuesProcessor{
	...
	for v.State == execinfra.StateRunning {
		...
		if len(v.typs) != 0 {
			for i, typ := range v.typs {
				var err error
				v.rowBuf[i], v.data[0], err = rowenc.EncDatumFromBuffer(
					typ, descpb.DatumEncoding_VALUE, v.data[0],
				)
				if err != nil {
			...
			if len(v.data[0]) != 0 {
				panic(errors.AssertionFailedf(
					"malformed ValuesCoreSpec row: %x, numRows %d", v.data[0], v.numRows,
				))
			}
			v.data = v.data[1:]
		}
