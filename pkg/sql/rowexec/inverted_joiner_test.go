...
func TestInvertedJoiner(t *testing.T) {
	...
	type testCase struct {
		...
		expected                      string
		outputColumnsWithContinuation []uint32
	...
	testCases := []testCase{
		{
			...
			outputColumnsWithContinuation: []uint32{0, 1, 4},
	...
	for _, c := range testCases {
		for _, outputGroupContinuation := range []bool{false, true} {
			if outputGroupContinuation && len(c.outputColumnsWithContinuation) == 0 {
				continue
			}
			t.Run(fmt.Sprintf("%s/cont=%t", c.description, outputGroupContinuation), func(t *testing.T) {
				...
				if outputGroupContinuation {
					post.OutputColumns = c.outputColumnsWithContinuation
				} else {
