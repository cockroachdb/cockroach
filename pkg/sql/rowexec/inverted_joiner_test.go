...
// The table has 99 rows, where each row is the row index 1..100 (primark
// key), followed by an array and a JSON column. Each array has two elements,
...
//
// Join expression: The left side of the join is simply integers. This is
// not realistic since the left side should be the same type as the
// corresponding column on the right side. But the DatumToInvertedExpr
// hook allows us to convert any left value into a SpanExpression for
// the join, so we utilize that to simplify the test. The SpanExpressions
// are defined below.
const numRows = 99

// For each integer d provided by the left it construct an intersection of
// d/10 and d%10. As mentioned earlier, the array inverted index is position
// agnostic, so say d = 5, which means we are looking for rows on the right
// that have both 5/10 and 5%10 as array elements. This set, {0, 5} is
// satisfied by two right rows: the row index 5 (obviously), since it used
// the same transformation to generate its array. And row index 50, since
// 50%10 = 0, 50/10 = 5.
...
// This is similar to the arrayIntersectionExpr, but for JSON. Since the keys
// "c1" and "c2" distinguish the application of the / and % operator here
// (unlike the array case), a left integer d will only match a right row with
// row index d.
...
// For each integer d provided by the left it constructs a union of {"c1": d/10} and
// {"c2": d%10}. So if d = 5, we will find all right rows containing {"c1": 0} or
// {"c2": 5}, which is {1..9, 15, 25, 35, ..., 95}.
...
func TestInvertedJoiner(t *testing.T) {
	...
	testCases := []testCase{
		...
		{
			// This is similar to the "array intersection" case, and uses the same input,
			// but additionally filters to only include right rows with index > 20. So
			// the output is a subset. Note that the input 20 has its joined rows completely
			// eliminated. We use the same OnExpr for the LeftOuterJoin, LeftSemiJoin and
			// LeftAntiJoin cases below.
		...
		{
			// JSON intersection. The left and right rows have the same index
			// as described in the comment in jsonIntersectionExpr. There is no
			// row 101 in the right so 101 is not output.
			...
			input: [][]tree.Datum{
				{tree.NewDInt(tree.DInt(5))}, {tree.NewDInt(tree.DInt(42))}, {tree.NewDInt(tree.DInt(101))},
		...
		},
		{
			...
			// 5, 42 are (c1, 0), (c1, 4), (c2, 2), (c2, 5). (c1, 0) causes the
			// [5 1] [5 2] ... [5 9] to be output first. Since 45 was indexed also under
			// (c1, 4), it gets read next and is assigned a smaller number in the row
			// container than the rows read for (c2, 5) (which are also used by left input
			// 5. Which is why [5 45] precedes [5 15] ... (the set operators output the
			// result in the order of the numbering in the row container).
		...
		{
			// JSON union with LeftSemiJoin. Everything on the left is output.
