...
func TestInvertedJoiner(t *testing.T) {
	...
	type testCase struct {
		...
		outputTypes []*types.T
		// The output columns for the case without continuation.
	...
	testCases := []testCase{
		...
		},
		{
			...
			indexIdx:    dbiIndex,
			// (5, 50) will match row 5.
			// (20, 0) will match no rows because row 20 has d=200.
			// (42, 420) will match any rows that have array elements {2, 0} and
			// d=420, which is only row 42.
			input: [][]tree.Datum{
		...
		{
			// JSON union; two-column index. See the comment in jsonUnionExpr
			// that describes what is matched by each input row.
		...
		for _, outputGroupContinuation := range []bool{false, true} {
			...
			t.Run(fmt.Sprintf("%s/cont=%t", c.description, outputGroupContinuation), func(t *testing.T) {
				...
				if outputGroupContinuation {
					...
					// The number of columns is the number of left columns + 4
					// columns from the right (a, b, c, d) + 1 continuation
					// column.
