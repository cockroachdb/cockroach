...
// The table has 99 rows, where each row is the row index 1..100 (primary
...
// For each integer d provided by the left side, this constructs an
// intersection of d/10 and d%10. As mentioned earlier, the array inverted
// index is position agnostic. So say d = 5, which means we are looking for
// rows on the right side that have both 5/10 and 5%10 as array elements. This
// set, {0, 5} is satisfied by two right side rows: the row index 5
// (obviously), since it used the same transformation to generate its array.
// And row index 50, since 50%10 = 0, 50/10 = 5.
...
// (unlike the array case), a left side integer d will only match a right side
// row with row index d.
...
// For each integer d provided by the left side, this constructs a union of
// {"c1": d/10} and {"c2": d%10}. So if d = 5, we will find all right side
// rows containing {"c1": 0} or {"c2": 5}, which is
// {1..9, 15, 25, 35, ..., 95}.
