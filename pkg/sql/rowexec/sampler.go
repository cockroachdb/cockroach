...
func (s *sketchInfo) addRow(
	...
	isNull := true
	*buf = (*buf)[:0]
	// We might allocate tree.Datums when hashing the row, so we'll track those
	// allocations and account for them. Note that even though we're losing the
	...
	s.sketch.Insert(*buf)
	return unlimitedMemAcc.Grow(ctx, int64(newMemUsage))
}
