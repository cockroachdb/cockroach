...
func (s *sorterBase) init(
	...
	memMonitor := execinfra.NewLimitedMonitor(ctx, flowCtx.EvalCtx.Mon, flowCtx, redact.Sprint("%s-limited", processorName))
	...
	s.diskMonitor = execinfra.NewMonitor(ctx, flowCtx.DiskMonitor, redact.Sprint("%s-disk", processorName))
