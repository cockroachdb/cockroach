...
//
// There's 3 implementations of joinReaderStrategy:
...
// fetched, it is handed to the joinReaderNoOrderingStrategy, which will match
// it against all the corresponding input rows, and immediately emit (1, red),
// (4, red). Then the joinReader will handed in blue, for which the
...
// buffered until all joined rows are produced. Then the joinReader will handed
// in blue, for which the joinReaderNoOrderingStrategy produces (2, blue) and
// (3, blue). Now that all output rows are buffered, they are re-ordered
// according to the input order and emitted.
//
