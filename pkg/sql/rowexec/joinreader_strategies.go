...
// rows to the joinReaderStrategy.processLookupRow() which (usually) holds on to
// them and returns key spans to be looked up, then the joinReader iteratively
// looks up those spans and passes the resulting looked-up rows to
...
//   index and the table's PK. This guy is the simplest and the most efficient
...
type joinReaderStrategy interface {
	...
	// processLookupRows consumes the rows the joinReader has buffered returns the
	// lookup spans.
...
// - the input side has rows 1, 2, 3, 4.
// - the lookup side has rows red, blue.
// - the join needs to produce the pairs (1, red), (2, blue), (3, blue),
//  (4, red), in any order.
//
// Say the joinReader looks up rows in order: red, then blue. Once red is
// fetched, it is handed to joinReaderNoOrderingStrategy.processLookedUpRow(),
// which will match it against all the corresponding input rows, and immediately
// emit (1, red), (4, red). Then the joinReader will be handed in blue, for
// which the joinReaderNoOrderingStrategy will emit (2, blue) and (3, blue).
// Notice that the rows were produced in an order different from the input order
// but, on the flip side, there was no buffering of the looked-up rows. See
...
//
// Say the joinReader looks up rows in order: red, then blue. Once red is
// fetched, it is handed to joinReaderNoOrderingStrategy.processLookedUpRow(), which will match
// it against all the corresponding input rows, producing (1, red), (4, red).
// These two rows are not emitted because that would violate the input ordering
// (well, (1, red) could be emitted, but we're not smart enough). So, they are
// buffered until all joined rows are produced. Then the joinReader will hand in
// blue, for which the joinReaderNoOrderingStrategy produces (2, blue) and (3,
// blue). Now that all output rows are buffered, they are re-ordered according
// to the input order and emitted.
//
