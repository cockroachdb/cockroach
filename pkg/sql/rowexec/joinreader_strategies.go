...
type joinReaderOrderingStrategy struct {
	*joinerBase
	...
	emitCursor struct {
		...
		outputRowIdx int
		// unbufferedRow, if non-nil, contains a looked-up row that matches the
...
func (s *joinReaderOrderingStrategy) processLookedUpRow(
	...
	for _, inputRowIdx := range matchingInputRowIndices {
		if !s.isPartialJoin {
			if inputRowIdx == 0 {
				...
				s.emitCursor.unbufferedRow = row
				continue
			}
	...
	if s.emitCursor.unbufferedRow != nil {
...
func (s *joinReaderOrderingStrategy) nextRowToEmit(
	...
	if s.emitCursor.unbufferedRow == nil && s.emitCursor.outputRowIdx >= len(lookedUpRows) {
	...
	if s.emitCursor.unbufferedRow != nil {
		...
		defer func() { s.emitCursor.unbufferedRow = nil }()
	} else {
	...
	if s.emitCursor.unbufferedRow != nil {
		lookedUpRow = s.emitCursor.unbufferedRow
	} else {
