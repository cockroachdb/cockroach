...
func TestSetContainerUnion(t *testing.T) {
	require.Equal(t, setToString(nil), setToString(unionSetContainers(nil, nil)))
	require.Equal(t, setToString([]KeyIndex{5}), setToString(unionSetContainers(
		[]KeyIndex{5}, nil)))
	require.Equal(t, setToString([]KeyIndex{2, 5, 12}), setToString(unionSetContainers(
		[]KeyIndex{5}, []KeyIndex{2, 12})))
	require.Equal(t, setToString([]KeyIndex{2, 5, 12}), setToString(unionSetContainers(
		[]KeyIndex{2, 5}, []KeyIndex{12})))
	require.Equal(t, setToString([]KeyIndex{2, 5, 12}), setToString(unionSetContainers(
		[]KeyIndex{2, 12}, []KeyIndex{2, 5})))
}

func TestSetContainerIntersection(t *testing.T) {
	require.Equal(t, setToString(nil), setToString(intersectSetContainers(nil, nil)))
	require.Equal(t, setToString(nil), setToString(intersectSetContainers(
		[]KeyIndex{5}, nil)))
	require.Equal(t, setToString(nil), setToString(intersectSetContainers(
		[]KeyIndex{5}, []KeyIndex{2, 12})))
	require.Equal(t, setToString([]KeyIndex{2, 17, 30}), setToString(intersectSetContainers(
		[]KeyIndex{2, 5, 17, 25, 30}, []KeyIndex{2, 12, 13, 17, 23, 30})))
}
...
func TestInvertedExpressionEvaluator(t *testing.T) {
	...
	batchBoth := &batchedInvertedExprEvaluator{
		exprs: []*invertedexpr.SpanExpressionProto{&protoUnion, &protoIntersection},
	}
	batchBoth.getSpans()
	for _, elem := range indexRows {
