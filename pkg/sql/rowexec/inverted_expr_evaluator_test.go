...
func TestInvertedExpressionEvaluator(t *testing.T) {
	...
	for _, elem := range indexRows {
		...
		err = batchEvalUnion.addIndexRow(invertedexpr.EncInvertedVal(elem.key), elem.index)
		require.NoError(t, err)
		add, err = batchEvalIntersection.prepareAddIndexRow(invertedexpr.EncInvertedVal(elem.key))
		require.NoError(t, err)
		require.Equal(t, true, add)
		err = batchEvalIntersection.addIndexRow(invertedexpr.EncInvertedVal(elem.key), elem.index)
		require.NoError(t, err)
	}
	...
	for _, elem := range indexRows {
		...
		require.NoError(t, err)
		require.Equal(t, true, add)
		err = batchBoth.addIndexRow(invertedexpr.EncInvertedVal(elem.key), elem.index)
		require.NoError(t, err)
	}
...
func TestInvertedExpressionEvaluatorPreFilter(t *testing.T) {
	...
	feedIndexRows := func(indexRows []keyAndIndex, expectedAdd bool) {
		for _, elem := range indexRows {
			...
			if add {
				err = batchEval.addIndexRow(invertedexpr.EncInvertedVal(elem.key), elem.index)
			}
			require.NoError(t, err)
		}
