...
func (is *indexScan) Next() (rowenc.EncDatumRow, *execinfrapb.ProducerMetadata) {
	...
	result := rowenc.EncDatumRow{
		...
		rowenc.DatumToEncDatum(types.String, tree.NewDString(is.scanResponse.Rows[is.rowIdx].Value.Timestamp.String())),
	}
	is.rowIdx++
...
func newIndexScan(
	...
	err := tmpScan.Init(ctx, tmpScan, post, []*types.T{types.Bytes, types.Bytes, types.String}, flowCtx, processorID, output, nil, execinfra.ProcStateOpts{})
	return tmpScan, err
}
