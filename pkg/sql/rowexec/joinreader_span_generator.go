...
func (g *multiSpanGenerator) fillInIndexColInfos(expr tree.TypedExpr) error {
	...
	case *tree.ComparisonExpr:
		...
		if inequality {
			...
			if colInfo != nil {
				i, ok := colInfo.(multiSpanGeneratorInequalityColInfo)
				if !ok {
					panic(errors.AssertionFailedf("unexpected colInfo type"))
				}
				inequalityInfo = &i
			} else {
				inequalityInfo = &multiSpanGeneratorInequalityColInfo{}
			}
