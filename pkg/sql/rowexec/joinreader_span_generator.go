...
// sizeOfMapEntry is a guess on how much space (in bytes) each item added to the
// keyToInputRowIndices map of each span generator takes up in the map (i.e. it
// is memory internal to the map, orthogonal to "key-value" pair that we're
// adding to the map).
const sizeOfMapEntry = 64
...
func (s spanRowIndices) memUsage() int64 {
	...
	for i := range s_cap {
		size += s_cap[i].span.MemUsage()
		size += memsize.IntSliceOverhead + memsize.Int*int64(cap(s_cap[i].rowIndices))
	}
	return size
}
