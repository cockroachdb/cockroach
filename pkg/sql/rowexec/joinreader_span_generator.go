...
type spanIDHelper struct {
	...
	unique bool
	// singleRowInput is true when the input only has a single row. In this case
	// we don't need spanKeyToSpanID map.
	singleRowInput bool
...
// must contain exactly one row. In both cases, most of the overhead of other
...
func (h *spanIDHelper) addInputRowIdxForSpan(
	...
	if h.unique || h.singleRowInput {
		...
		// single row, then the span is always new. The returned span ID is
		// never used, so its value can be anything.
		return 0, true
	}
...
func (g *defaultSpanGenerator) generateSpans(
	...
	g.spanIDHelper.reset(g.uniqueRows, singleRowInput)
