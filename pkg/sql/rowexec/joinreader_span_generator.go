...
func (g *multiSpanGenerator) init(
	...
	return g.lookupExprHasVarInequality(exprHelper.Expr), nil
}
...
func (g *multiSpanGenerator) lookupExprHasVarInequality(lookupExpr tree.TypedExpr) bool {
	switch t := lookupExpr.(type) {
	case *tree.AndExpr:
		return g.lookupExprHasVarInequality(t.Left.(tree.TypedExpr)) ||
			g.lookupExprHasVarInequality(t.Right.(tree.TypedExpr))
