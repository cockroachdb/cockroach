...
func (g *localityOptimizedSpanGenerator) getMatchingRowIndices(key roachpb.Key) ([]int, error) {
	// Note that we are consciously ignoring the error here because it indicates
	// that the key isn't contained by any "local" spans and we have to check
	// the remote span generator.
	if res, _ := g.localSpanGen.getMatchingRowIndices(key); len(res) > 0 {
		return res, nil
	}
