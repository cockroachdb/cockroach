...
type joinReaderSpanGenerator interface {
	...
	// lookupColsCount returns the number of index columns used as the key for each
	// lookup.
	lookupColsCount() int
}
...
func (g *multiSpanGenerator) lookupColsCount() int {
...
func (g *multiSpanGenerator) init(
	...
	spansCount := 1
	var indexOrdsSeen util.FastIntSet
	...
	var fillInIndexColInfos func(expr tree.TypedExpr) error
	fillInIndexColInfos = func(expr tree.TypedExpr) error {
		switch t := expr.(type) {
		case *tree.AndExpr:
			if err := fillInIndexColInfos(t.Left.(tree.TypedExpr)); err != nil {
				return err
			}
			return fillInIndexColInfos(t.Right.(tree.TypedExpr))
		...
		case *tree.ComparisonExpr:
			...
			getInfo := func(typedExpr tree.TypedExpr) error {
				...
				case *tree.IndexedVar:
					...
					if t.Idx >= numInputCols {
						tabOrd = t.Idx - numInputCols
					} else {
			...
			idxOrd, ok := tableOrdToIndexOrd.Get(tabOrd)
			if !ok {
	...
	if err := fillInIndexColInfos(exprHelper.Expr); err != nil {
		return err
	}

	lookupColsCount := len(g.indexColInfos)
	if indexOrdsSeen.Len() != lookupColsCount {
		return errors.AssertionFailedf("columns in the join condition do not form a prefix on the index")
	}
	...
	//   [ 'west'  -  2  - ]
	//
	g.indexKeyRows = make([]rowenc.EncDatumRow, 1, spansCount)
	...
	g.indexKeySpans = make(roachpb.Spans, 0, spansCount)
	return nil
}
