...
// TODO(tpr): We should probably be doing this at compile time, see #65773
...
func (g *multiSpanGenerator) generateSpans(rows []rowenc.EncDatumRow) (roachpb.Spans, error) {
	...
	for i, inputRow := range rows {
		...
		for j := range generatedSpans {
			...
			inputRowIndices := g.keyToInputRowIndices[string(generatedSpan.Key)]
			if inputRowIndices == nil {
				// Inequality range spans don't work with
				// MaybeSplitSpanIntoSeparateFamilies. The issue is that we don't know
				// what the prefix len is, if its a LessThan operation the start key
				// prefixLen will be len(g.indexColInfos) - 1. Since
				// MaybeSplitSpanIntoSeparateFamilies is an optimization this should be
				// okay.
				// RFC: is this true?  Should I figure out a way to pass in the correct
				// prefixLen instead of this?
				if g.inequalityColIdx != -1 {
