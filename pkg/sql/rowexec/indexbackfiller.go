...
func (ib *indexBackfiller) buildIndexEntryBatch(
	...
	br := batchRetry{
		...
		resetForNextAttempt: func(ctx context.Context) {
...
func (b *batchRetry) buildBatchWithRetry(ctx context.Context, db isql.DB) error {
	r := retry.StartWithCtx(ctx, b.retryOpts)
	for {
		if err := db.Txn(ctx, b.builder); err != nil {
