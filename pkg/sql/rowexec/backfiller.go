...
func (b *backfiller) mainLoop(ctx context.Context) (roachpb.Spans, error) {
	...
	updateRunThresholdBytes := rowinfra.BytesLimit(b.spec.UpdateRunThresholdBytes)
	start := timeutil.Now()
	totalChunks := 0
	totalSpans := 0
	...
	for i := range b.spec.Spans {
		...
		for todo.Key != nil {
			...
			var err error
			todo.Key, err = b.chunks.runChunk(ctx, todo, chunkSize, updateRunThresholdBytes, b.spec.ReadAsOf)
			if err != nil {
				return nil, err
			}
			chunks++
