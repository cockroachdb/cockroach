...
`); err != nil {
	...
	colIDtoRowIndex.Set(tableDesc.TableDesc().Columns[0].ID, 0)
	colIDtoRowIndex.Set(tableDesc.TableDesc().Columns[1].ID, 1)
...
`); err != nil {
	...
	colIDtoRowIndex.Set(tableDesc.TableDesc().Columns[0].ID, 0)
	colIDtoRowIndex.Set(tableDesc.TableDesc().Columns[1].ID, 1)
	colIDtoRowIndex.Set(tableDesc.TableDesc().Columns[2].ID, 2)
...
`); err != nil {
	...
	valueBuf, err := rowenc.EncodeTableValue(
		[]byte(nil), tableDesc.TableDesc().Columns[1].ID, values[1], []byte(nil))
	if err != nil {
		t.Fatalf("unexpected error: %s", err)
	}
