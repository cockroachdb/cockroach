...
`); err != nil {
	...
	values := []parser.Datum{parser.NewDInt(10), parser.NewDInt(2)}
	...
	// constraint.
	values = []parser.Datum{parser.NewDInt(10), parser.NewDInt(0)}
...
`); err != nil {
	...
	// Create the primary index key for the child k/v.
	values := []parser.Datum{parser.NewDInt(11), parser.NewDInt(1337), parser.NewDInt(300)}
	primaryIndexKeyPrefix := sqlbase.MakeIndexKeyPrefix(tableDesc, tableDesc.PrimaryIndex.ID)
	primaryIndexKey, _, err := sqlbase.EncodeIndexKey(
		tableDesc, &tableDesc.PrimaryIndex, colIDtoRowIndex, values, primaryIndexKeyPrefix)
	if err != nil {
	...
	// constraint.
	valueBuf, err := sqlbase.EncodeTableValue(
		[]byte(nil), tableDesc.Columns[1].ID, parser.NewDInt(1337), []byte(nil))
	if err != nil {
		t.Fatalf("unexpected error: %s", err)
	}
	colIDDiff := tableDesc.Columns[2].ID - tableDesc.Columns[1].ID
	valueBuf, err = sqlbase.EncodeTableValue(
		valueBuf, colIDDiff, parser.DNull, []byte(nil))
	if err != nil {
		t.Fatalf("unexpected error: %s", err)
	}
	var value roachpb.Value
