...
`); err != nil {
	...
	colIDtoRowIndex.Set(tableDesc.PublicColumnsNew()[0].GetID(), 0)
	colIDtoRowIndex.Set(tableDesc.PublicColumnsNew()[1].GetID(), 1)
...
`); err != nil {
	...
	colIDtoRowIndex.Set(tableDesc.PublicColumnsNew()[0].GetID(), 0)
	colIDtoRowIndex.Set(tableDesc.PublicColumnsNew()[1].GetID(), 1)
	colIDtoRowIndex.Set(tableDesc.PublicColumnsNew()[2].GetID(), 2)
...
`); err != nil {
	...
	valueBuf, err := rowenc.EncodeTableValue(
		[]byte(nil), tableDesc.PublicColumnsNew()[1].GetID(), values[1], []byte(nil))
	if err != nil {
		t.Fatalf("unexpected error: %s", err)
	}
