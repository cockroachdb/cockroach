...
func TestDeadlockDetection(t *testing.T) {
	...
	sql.Exec(t, "CREATE TABLE test1 (id int, age int)")
	sql.Exec(t, "CREATE TABLE test2 (id int, age int)")
	sql.Exec(t, "INSERT INTO test1 (id, age) VALUES (1, 0)")
	sql.Exec(t, "INSERT INTO test2 (id, age) VALUES (1, 0)")
	...
	for _, tc := range []struct {
		...
		sql.Exec(t, "ALTER ROLE ALL SET deadlock_timeout = '123'")
		...
		sql.Exec(t, "SET tracing = off")

		// We should find traces of txn push after the expected timeout.
		row := sql.QueryRow(t, fmt.Sprintf(
			"SELECT count(*) FROM [SHOW TRACE FOR SESSION] WHERE message like '%s'",
