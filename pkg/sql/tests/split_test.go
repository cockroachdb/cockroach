...
func TestSplitOnTableBoundaries(t *testing.T) {
	...
	_, err = sqlDB.Exec(`CREATE TABLE test.test (k INT PRIMARY KEY, v INT)`)
	require.NoError(t, err)

	tableID := sqlutils.QueryTableID(t, sqlDB, "test", "public", "test")
	// Choose the encoding codec based on whether we started the tenant.
	...
	expectedSplit := fmt.Sprintf("%s", codec.TablePrefix(tableID))

	// Wait for the split queue to create the split point.
	testutils.SucceedsSoon(t, func() error {
		...
		var startKey string
		require.NoError(t, sqlDB.QueryRow("SELECT start_key FROM [SHOW RANGES FROM DATABASE test]").Scan(&startKey), err)
