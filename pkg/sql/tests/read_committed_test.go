...
func TestReadCommittedVolatileUDF(t *testing.T) {
	...
	_, err = sqlDB.Exec(`CREATE TABLE kv (k TEXT PRIMARY KEY, v INT);`)
	require.NoError(t, err)
	_, err = sqlDB.Exec(`INSERT INTO kv VALUES ('a', 1);`)
	require.NoError(t, err)
	...
	g.GoCtx(func(ctx context.Context) error {
		// The READ COMMITTED transaction should retry the statement, but should
		// still be using a read timestamp that cannot see row 'b'.
		var udfResult1, udfResult2 int
		if err := txReadCommitted.QueryRow(`SELECT f(), f()`).Scan(&udfResult1, &udfResult2); err != nil {
			return err
		}
		if udfResult1 != 3 {
			return errors.Newf("expected first invocation result to be 3; got %d", udfResult1)
		}
		if udfResult2 != 3 {
			return errors.Newf("expected second invocation result to be 3; got %d", udfResult2)
		}
	...
	require.Equal(t, int64(1), readCommittedRetryCount.Load())
}
