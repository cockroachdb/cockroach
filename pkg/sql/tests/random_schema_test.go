...
func TestCreateRandomSchema(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		row := db.QueryRow(fmt.Sprintf("SHOW CREATE TABLE %s", tab.Table.String()))

		var tabName, tabStmt, secondTabStmt string
		if err := row.Scan(&tabName, &tabStmt); err != nil {
			t.Fatal(err)
		}
		...
		row = db.QueryRow(fmt.Sprintf("SHOW CREATE TABLE %s", tabName))

		if err := row.Scan(&tabName, &secondTabStmt); err != nil {
			t.Fatal(err)
		}
