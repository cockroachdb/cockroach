...
// TestAllowRoleMembershipsToChangeDuringTransactionWithUpsert ensures that
// when the corresponding session variable is set for all sessions using cached
// role memberships, performing new grants do not need to wait for open
// transactions to complete.
func TestAllowRoleMembershipsToChangeDuringTransactionWithUpsert(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)
	...
	t.Run("normal path waits", func(t *testing.T) {
		fooTx, err := fooDB.BeginTx(ctx, nil)
		require.NoError(t, err)
		_, err = fooTx.Exec("UPSERT INTO t VALUES (777)")
		require.NoError(t, err)
		errCh := make(chan error, 1)
		go func() {
			_, err := sqlDB.Exec("GRANT bar TO baz;")
	...
	t.Run("session variable prevents waiting", func(t *testing.T) {
		...
		fooTx, err := fooConn.BeginTx(ctx, nil)
		require.NoError(t, err)
		_, err = fooTx.Exec("UPSERT INTO t VALUES (77)")
		require.NoError(t, err)
		conn, err := sqlDB.Conn(ctx)
		require.NoError(t, err)
		defer func() { _ = conn.Close() }()
