...
// This is a narrower and tweaked version of BenchmarkKV/Update/SQL. This will
// eventually be merged back into BenchmarkKV above, but we are keeping it
// separate for now since changing BenchmarkKV reasonably for all values of
// rows is tricky due to the optimization that switches scans for a large set
// specified by the SQL IN operator to full table scans that are not bounded
// by the smallest, largest key in the set.
...
// and do two significant pieces of work:
// - A read-only batch with 100 ScanRequests (this should eventually be
//   optimised by SQL to 100 GetRequests
...
// this with -benchtime 5000x which increases the amount of data in the engine
// and keeps the engine size consistent across runs.
func BenchmarkKVAndStorageUsingSQL(b *testing.B) {
	...
	for _, maxVersions := range []int{2, 4, 8} {
		b.Run(fmt.Sprintf("versions=%d", maxVersions), func(b *testing.B) {
			...
			// so that we have a more reasonable comparison. Otherwise the benchmark
			// with maxVersions=2 creates more keys, resulting in more files in the
			// engine, which makes it slower.
			rowsToInit := b.N * rowsToUpdate
			// rowsToInit := (b.N/numUpdatesToRow + 1) * rowsToUpdate
