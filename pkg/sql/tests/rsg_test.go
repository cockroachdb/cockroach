...
func init() {
	ignoredRegex = regexp.MustCompile(strings.Join(ignoredErrorPatterns, "|"))
}
...
func TestRandomSyntaxSQLSmith(t *testing.T) {
	...
	testRandomSyntax(t, true, func(ctx context.Context, db *verifyFormatDB, r *rsg.RSG) error {
		...
		s := smither.Generate()
		err := db.exec(ctx, s)
		if crasher, ok := err.(crasher); ok {
			if err := db.exec(ctx, "USE defaultdb"); err != nil {
				t.Fatalf("couldn't reconnect to db after crasher: %v", crasher)
			}
			fmt.Printf("CRASHER:\n%s\ncaused by: %s\nserver stacktrace:\n%s\n\n", s, crasher.Error(), crasher.detail)
			return crasher
		}
		if err == nil {
			return nil
