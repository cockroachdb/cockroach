...
func (db *verifyFormatDB) exec(ctx context.Context, sql string) error {
	...
	case <-time.After(*flagRSGExecTimeout):
		db.mu.Lock()
		defer db.mu.Unlock()
		return errors.Errorf("timeout: %q. currently executing: %v", sql, db.mu.active)
	}
}
...
func testRandomSyntax(
	t *testing.T, setup func(context.Context, *verifyFormatDB) error, fn func(context.Context, *verifyFormatDB, *rsg.RSG) error,
) {
	if *flagRSGTime == 0 {
	...
	ctx, _ = context.WithTimeout(ctx, *flagRSGTime)
