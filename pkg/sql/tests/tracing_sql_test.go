...
func TestSetTraceSpansVerbosityBuiltin(t *testing.T) {
	...
	r.CheckQueryResults(
		t,
		"SELECT * FROM crdb_internal.set_verbosity_of_spans_in_trace(0, true)",
		[][]string{{`false`}},
	)
	...
	// Toggle the trace's verbosity to true and confirm all spans are verbose.
	traceID := root.GetTraceID()
	query := fmt.Sprintf(
		"SELECT * FROM crdb_internal.set_verbosity_of_spans_in_trace(%d, true)",
		traceID,
	)
	r.CheckQueryResults(
		t,
	...
	// Toggle the trace's verbosity to false and confirm all spans are not verbose.
	query = fmt.Sprintf(
		"SELECT * FROM crdb_internal.set_verbosity_of_spans_in_trace(%d, false)",
		traceID,
	)
	r.CheckQueryResults(
		t,
