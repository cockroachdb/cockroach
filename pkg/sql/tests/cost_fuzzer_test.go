...
func TestCostFuzzer(t *testing.T) {
	...
	smither, err := sqlsmith.NewSmither(conn, rng,
		sqlsmith.DisableMutations(), sqlsmith.DisableImpureFuncs())
	if err != nil {
		t.Fatal(err)
	}
	defer smither.Close()
...
%s
%s`,
				diff, stmt, seed, unperturbedExplain, perturbedExplain, setup, unperturbedRows, perturbedRows)
		}
	}
}
