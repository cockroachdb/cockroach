...
// TestCatalogGeneration validates that catalog generation information is
// properly invalidated in the face of modifications for prepared queries.
func TestCatalogGeneration(t *testing.T) {
	...
	validateColumnsOnQuery := func(exp int) {
		...
		require.Equal(t, val, exp)
	}
	// Sanity: Ensure that sc3.t1 is picked in the original prepare.
	validateColumnsOnQuery(1)
	// Set up a new t1 in sc2, which should be picked earlier in our search_path.
