...
func (ie *InternalExecutor) newConnExecutorWithTxn(
	...
	postSetupFn := func(ex *connExecutor) {
		if ie.extraTxnState != nil {
			...
			ex.extraTxnState.jobsInfo = ie.extraTxnState.jobsInfo
...
func (ie *InternalExecutor) releaseSchemaChangeJobRecords() {
	ie.extraTxnState.jobsInfo.releaseJobsToRecreate()
}
...
func (ief *InternalDB) newInternalExecutorWithTxn(
	...
	commitTxnFunc := func(ctx context.Context) error {
		...
		return ie.s.cfg.JobRegistry.Run(ctx, ie.extraTxnState.jobsInfo.jobsCreated)
	}
