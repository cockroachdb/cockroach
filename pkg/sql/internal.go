...
// specified in the SessionArgs. Every other parameter in the
// SessionArgs is ignored.
func (ie *InternalExecutor) QueryWithUser(
	...
	return ie.queryInternal(ctx, opName, txn, internalExecFixedUserSession, sargs, stmt, qargs...)
}
...
func (ie *InternalExecutor) Exec(
	...
	res, err := ie.execInternal(
		ctx, opName, txn, internalExecRootSession, nil /* sargs */, stmt, qargs...,
	)
	if err != nil {
		return 0, err
	}
...
func (ie *InternalExecutor) ExecWithUser(
	...
	res, err := ie.execInternal(ctx, opName, txn, internalExecFixedUserSession, sargs, stmt, qargs...)
	if err != nil {
		return 0, err
	}
	return res.rowsAffected, res.err
...
// NewInternalSessionUserArgs creates a SessionArgs with a custom
// user name, suitable for use with internalExecUseFixedUserSession.
func NewInternalSessionUserArgs(user string) *SessionArgs {
	return &SessionArgs{User: user}
}
...
func (ie *internalExecutorImpl) execInternal(
	...
	case internalExecFixedUserSession:
		if sargs == nil {
			log.Fatal(ctx, "programming error: mode fixed user with nil sargs")
		}
