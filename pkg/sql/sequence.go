...
func (p *planner) IncrementSequence(ctx context.Context, seqName *tree.TableName) (int64, error) {
	...
	if seqOpts.Virtual {
		...
		seqValueKey := keys.TODOTenantKeyGen.SequenceKey(uint32(descriptor.ID))
...
func MakeSequenceKeyVal(
	...
	seqValueKey := keys.TODOTenantKeyGen.SequenceKey(uint32(sequence.ID))
...
func (p *planner) GetSequenceValue(
	...
	keyValue, err := p.txn.Get(ctx, keys.TODOTenantKeyGen.SequenceKey(uint32(desc.ID)))
	if err != nil {
		return 0, err
	}
