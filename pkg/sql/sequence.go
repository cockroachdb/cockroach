...
func (p *planner) incrementSequenceUsingCache(
	...
	if cacheSize == 1 {
		...
		// If cache size option is 1 (default, not cached), and node cache size option is greater than 1 (not default, cached), then use node-level cache
		if seqOpts.CacheSize == 1 && seqOpts.NodeCacheSize > 1 {
			val, err = p.GetSequenceCacheNode().NextValue(uint32(sequenceID), uint32(descriptor.GetVersion()), fetchNextValues)
		} else {
