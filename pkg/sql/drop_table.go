...
func (p *planner) removeFKForBackReference(
	...
	jobDesc := fmt.Sprintf("removing inbound FK %s from table %s(%d), referenced by table %s(%d)",
		ref.Name, thisTableName.FQString(), tableDesc.GetID(), originTableName.FQString(), originTableDesc.GetID())
...
func (p *planner) removeFKBackReference(
	...
	jobDesc := fmt.Sprintf("removing outbound FK %s from table %s(%d), referencing to table %s(%d)",
		ref.Name, thisTableName.FQString(), tableDesc.GetID(), referencedTableName.FQString(), referencedTableDesc.GetID())
