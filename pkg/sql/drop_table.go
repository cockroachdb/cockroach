...
func (p *planner) initiateDropTable(ctx context.Context, tableDesc *sqlbase.TableDescriptor) error {
	...
	// ClearRange will only work if the entire hierarchy of interleaved
	// tables are dropped at once, as with ON DELETE CASCADE where the
	// top-level "root" table is dropped.
