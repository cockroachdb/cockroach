...
func (p *planner) markTableMutationJobsSuccessful(
	...
	for _, mj := range tableDesc.MutationJobs {
		...
		if record, exists := p.ExtendedEvalContext().JobsInfo.uniqueToCreate[tableDesc.ID]; exists && record.JobID == jobID {
			delete(p.ExtendedEvalContext().JobsInfo.uniqueToCreate, tableDesc.ID)
			continue
		}
