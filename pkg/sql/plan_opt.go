	...
	if opc.useCache {
		...
		if ok && cachedData.Metadata != nil {
			...
			if !pm.TypeHints.Identical(p.semaCtx.Placeholders.TypeHints) {
				...
				isStale, err := cachedData.Memo.IsStale(ctx, p.EvalContext(), opc.catalog, false /* ignoreCanaryStats */)
				if err != nil {
					return 0, err
				}
				if !isStale {
...
func (p *planner) makeOptimizerPlan(ctx context.Context) error {
	...
	// We set the UseCanaryStats of eval.Context here, before the optimization context
	// is reset below. This is because the memo is initialzed in the opc.reset(ctx), so we
	// need to ensure the canary stats decision is made before that.
