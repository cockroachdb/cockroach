...
func (p *planner) makeOptimizerPlan(ctx context.Context) error {
	...
	if mode := p.SessionData().ExperimentalDistSQLPlanningMode; mode != sessiondata.ExperimentalDistSQLPlanningOff {
		bld = execbuilder.New(newDistSQLSpecExecFactory(p, false /* forceLocalPlanning */), execMemo, &opc.catalog, root, p.EvalContext())
		plan, err = bld.Build()
		if err != nil {
		...
		if err == nil {
			...
			if m.isPartiallyDistributed() && p.SessionData().PartiallyDistributedPlansDisabled {
				...
				// execFactory.
				bld = execbuilder.New(newDistSQLSpecExecFactory(p, true /* forceLocalPlanning */), execMemo, &opc.catalog, root, p.EvalContext())
				plan, err = bld.Build()
			}
		}
		if err != nil {
