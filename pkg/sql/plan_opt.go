...
// a canary window. This avoids random cache flip-flops and preserves a
// clean stable baseline: only stable executions participate in memo
// reuse (query cache or prepared statements). Write-side cache skipping
// is handled by DisableMemoReuse in the optbuilder.
...
func (opc *optPlanningCtx) fetchPreparedMemo(ctx context.Context) (_ *memo.Memo, err error) {
	...
	if validMemo != nil {
		if opc.skipMemoReuseForCanaryExec(validMemo) {
			opc.allowMemoReuse = false
			opc.useCache = false
		} else {
...
func (opc *optPlanningCtx) buildExecMemo(ctx context.Context) (_ *memo.Memo, _ error) {
	...
	if opc.useCache {
		...
		if ok && opc.skipMemoReuseForCanaryExec(cachedData.Memo) {
			ok = false
			opc.useCache = false
		}
		if ok {
