...
func (opc *optPlanningCtx) buildReusableMemo(
	...
	if !f.Memo().HasPlaceholders() && !f.FoldingControl().PreventedStableFold() {
		...
		opc.flags.Set(planFlagReoptimized)
		return opc.optimizer.DetachMemo(ctx), false, nil
	}
...
func (opc *optPlanningCtx) buildExecMemo(ctx context.Context) (_ *memo.Memo, _ error) {
	...
	// placeholders.
	//
	// We're not concerned with generic plans here because they only benefit
	// queries with placeholders. Therefore, any queries that will benefit from
	// generic plans will already be handled by the prepared statement paths
	// before this.
