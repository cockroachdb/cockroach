...
func TestMakeConstraints(t *testing.T) {
	...
	testData := []struct {
		...
		{`c = true`, `c`, `[c = true]`},
		{`c = false`, `c`, `[c = false]`},
		{`c != true`, `c`, `[c IS DISTINCT FROM NULL, c <= false]`},
		{`c != false`, `c`, `[c >= true]`},
		{`NOT c`, `c`, `[c IS DISTINCT FROM NULL, c <= false]`},
		{`c IS TRUE`, `c`, `[c = true]`},
		{`c IS NOT TRUE`, `c`, `[c IS DISTINCT FROM NULL, c <= false] OR [c IS NOT DISTINCT FROM NULL]`},
		{`c IS FALSE`, `c`, `[c = false]`},
		{`c IS NOT FALSE`, `c`, `[c >= true] OR [c IS NOT DISTINCT FROM NULL]`},
		{`c IS NOT DISTINCT FROM TRUE`, `c`, `[c = true]`},
		{`c IS DISTINCT FROM TRUE`, `c`, `[c IS DISTINCT FROM NULL, c <= false] OR [c IS NOT DISTINCT FROM NULL]`},
		{`c IS NOT DISTINCT FROM FALSE`, `c`, `[c = false]`},
		{`c IS DISTINCT FROM FALSE`, `c`, `[c >= true] OR [c IS NOT DISTINCT FROM NULL]`},

		{`a = 1`, `b`, ``},
		{`a = 1`, `a`, `[a = 1]`},
		{`a != 1`, `a`, `[a IS DISTINCT FROM NULL]`},
		{`a > 1`, `a`, `[a >= 2]`},
		{`a >= 1`, `a`, `[a >= 1]`},
		{`a < 1`, `a`, `[a IS DISTINCT FROM NULL, a <= 0]`},
		{`a <= 1`, `a`, `[a IS DISTINCT FROM NULL, a <= 1]`},
		...
		{`a = 1 AND b != 1`, `a,b`, `[a = 1, b IS DISTINCT FROM NULL]`},
		{`a = 1 AND b > 1`, `a,b`, `[a = 1, b >= 2]`},
		{`a = 1 AND b >= 1`, `a,b`, `[a = 1, b >= 1]`},
		{`a = 1 AND b < 1`, `a,b`, `[a = 1, b IS DISTINCT FROM NULL, b <= 0]`},
		{`a = 1 AND b <= 1`, `a,b`, `[a = 1, b IS DISTINCT FROM NULL, b <= 1]`},

		{`a != 1 AND b = 1`, `a,b`, `[a IS DISTINCT FROM NULL]`},
		{`a != 1 AND b != 1`, `a,b`, `[a IS DISTINCT FROM NULL]`},
		{`a != 1 AND b > 1`, `a,b`, `[a IS DISTINCT FROM NULL]`},
		{`a != 1 AND b >= 1`, `a,b`, `[a IS DISTINCT FROM NULL]`},
		{`a != 1 AND b < 1`, `a,b`, `[a IS DISTINCT FROM NULL]`},
		...
		{`a > 1 AND b != 1`, `a,b`, `[a >= 2, b IS DISTINCT FROM NULL]`},
		...
		{`a < 1 AND b = 1`, `a,b`, `[a IS DISTINCT FROM NULL, a <= 0, b = 1]`},
		{`a < 1 AND b != 1`, `a,b`, `[a IS DISTINCT FROM NULL, a <= 0]`},
		{`a < 1 AND b > 1`, `a,b`, `[a IS DISTINCT FROM NULL, a <= 0]`},
		{`a < 1 AND b >= 1`, `a,b`, `[a IS DISTINCT FROM NULL, a <= 0]`},
		{`a < 1 AND b < 1`, `a,b`, `[a IS DISTINCT FROM NULL, a <= 0, b <= 0]`},
		{`a < 1 AND b <= 1`, `a,b`, `[a IS DISTINCT FROM NULL, a <= 0, b <= 1]`},

		{`a <= 1 AND b = 1`, `a,b`, `[a IS DISTINCT FROM NULL, a <= 1, b = 1]`},
		{`a <= 1 AND b != 1`, `a,b`, `[a IS DISTINCT FROM NULL, a <= 1]`},
		{`a <= 1 AND b > 1`, `a,b`, `[a IS DISTINCT FROM NULL, a <= 1]`},
		{`a <= 1 AND b >= 1`, `a,b`, `[a IS DISTINCT FROM NULL, a <= 1]`},
		{`a <= 1 AND b < 1`, `a,b`, `[a IS DISTINCT FROM NULL, a <= 1, b <= 0]`},
		{`a <= 1 AND b <= 1`, `a,b`, `[a IS DISTINCT FROM NULL, a <= 1, b <= 1]`},

		{`a IN (1) AND b = 1`, `a,b`, `[a IN (1), b = 1]`},
		{`a IN (1) AND b != 1`, `a,b`, `[a IN (1), b IS DISTINCT FROM NULL]`},
		{`a IN (1) AND b > 1`, `a,b`, `[a IN (1), b >= 2]`},
		{`a IN (1) AND b >= 1`, `a,b`, `[a IN (1), b >= 1]`},
		{`a IN (1) AND b < 1`, `a,b`, `[a IN (1), b IS DISTINCT FROM NULL, b <= 0]`},
		{`a IN (1) AND b <= 1`, `a,b`, `[a IN (1), b IS DISTINCT FROM NULL, b <= 1]`},
		...
		{`a IS NULL`, `a`, `[a IS NOT DISTINCT FROM NULL]`},
		{`a IS NOT NULL`, `a`, `[a IS DISTINCT FROM NULL]`},
		{`a IS NOT DISTINCT FROM NULL`, `a`, `[a IS NOT DISTINCT FROM NULL]`},
		{`a IS DISTINCT FROM NULL`, `a`, `[a IS DISTINCT FROM NULL]`},

		{`a = 1 OR a = 3`, `a`, `[a IN (1, 3)]`},
		{`a <= 1 OR a >= 8`, `a`, `[a IS DISTINCT FROM NULL, a <= 1] OR [a >= 8]`},
		{`a < 1 OR a > 2`, `a`, `[a IS DISTINCT FROM NULL, a <= 0] OR [a >= 3]`},
		{`a < 1 OR a = 3 OR a > 5`, `a`, `[a IS DISTINCT FROM NULL, a <= 0] OR [a = 3] OR [a >= 6]`},
		...
		{`a < 2 OR (a > 5 AND b > 2)`, `a`,
			`[a IS DISTINCT FROM NULL, a <= 1] OR [a >= 6]`},
		{`a < 2 OR (a > 5 AND b > 2)`, `b`, ``},
		{`a < 2 OR (a > 5 AND b > 2)`, `a,b`,
			`[a IS DISTINCT FROM NULL, a <= 1] OR [a >= 6, b >= 3]`},
