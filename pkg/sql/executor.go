	...
	for {
		...
		if autoCommit {
			if err == nil {
				...
				// Copy the status out of the Proto under lock. Making decisions on this
				// later is not thread-safe, but the commutativity property of
				// transactions assure that reasoning about the situation is
				// straightforward.
				status := txnState.mu.txn.Status()
				switch status {
				case roachpb.ABORTED:
					// TODO(andrei): Until 7881 is fixed.
					log.Errorf(session.Ctx(), "#7881: the statement we just ran didn't generate an error "+
