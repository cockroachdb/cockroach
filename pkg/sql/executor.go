...
func (e *Executor) execStmtsInCurrentTxn(
	...
	for i, stmt := range stmts {
		...
		queryMeta := session.addActiveQuery(stmt)
		defer session.removeActiveQuery(queryMeta)
		...
		if _, ok := stmt.(*parser.ShowTransactionStatus); ok {
			...
			case Open:
				res, err = e.execStmtInOpenTxn(
					...
					avoidCachedDescriptors, automaticRetryCount, queryMeta)
...
func (e *Executor) execStmt(
	...
	if useDistSQL {
		planner.queryMeta.Lock()
		planner.queryMeta.usingDistSQL = true
		planner.queryMeta.Unlock()
