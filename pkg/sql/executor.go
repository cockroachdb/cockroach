...
func (e *Executor) ExecuteStatementParsed(
	...
	defer func() {
		if r := recover(); r != nil {
			// On a panic, prepend the executed SQL.
			panic(fmt.Errorf("%s: %s", stmt.Statement.String(), r))
		}
	}()
...
func (e *Executor) execStmtInOpenTxn(
	...
	case *parser.Execute:
		...
		results := e.ExecuteStatementParsed(session, prepared, &parser.PlaceholderInfo{Values: qArgs, Types: prepared.SQLTypes})
		if results.Empty {
			return Result{}, nil
		}
