...
func (e *Executor) execStmtsInCurrentTxn(
	...
	for i, stmt := range stmts {
		...
		queryHandle := session.addActiveQuery(stmt)
		defer session.removeActiveQuery(queryHandle)
		...
		if _, ok := stmt.(*parser.ShowTransactionStatus); ok {
			...
			case Open:
				res, err = e.execStmtInOpenTxn(
					...
					avoidCachedDescriptors, automaticRetryCount, queryHandle)
...
func (e *Executor) execStmt(
	...
	if useDistSQL {
		session.setQueryExecutionMode(planner.queryHandle, true /* usingDistSQL */)
