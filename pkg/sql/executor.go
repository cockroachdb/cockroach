...
// execPlan executes the plan for the statement and returns the plan's result.
// The caller closes result.Rows (even in error cases).
func (e *Executor) execPlan(
	stmt parser.Statement, planMaker *planner, plan planNode, automaticRetryCount int,
) (Result, error) {
...
func (e *Executor) execStmt(
	...
	return e.execPlan(stmt, planMaker, plan, automaticRetryCount)
}
...
func (e *Executor) execStmtPipelined(stmt parser.Statement, planMaker *planner) (Result, error) {
	...
	session.pipelineQueue.Add(&plan, func() error {
		defer plan.Close(session.Ctx())
		result, err := e.execPlan(stmt, planMaker, plan, 0 /* automaticRetryCount */)
