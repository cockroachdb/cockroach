...
func (e *Executor) execRequest(
	...
	if err != nil {
		...
		return session.ResultWriter.SetError(err)
	}
	...
	for len(stmts) > 0 {
		...
		if !inTxn {
			...
			if _, isBegin := stmts[0].AST.(*parser.BeginTransaction); !isBegin {
				...
				if err != nil {
					return resultWriter.SetError(err)
				}
				if protoTS != nil {
		...
		_, isWireFailureErr := err.(WireFailureError)
		if isWireFailureErr {
		...
		if err != nil {
			...
			if err := resultWriter.SetError(convertToErrWithPGCode(err)); err != nil {
				txn.CleanupOnError(session.Ctx(), err)
				return err
			}
		}
		...
		if !txnState.TxnIsOpen() {
			...
			// found).
			txnState.schemaChangers.execSchemaChanges(session.Ctx(), e, session, session.ResultWriter)
		}
...
func (e *Executor) execStmtsInCurrentTxn(
	...
	for i, stmt := range stmts {
		...
		if _, ok := stmt.AST.(*parser.ShowTransactionStatus); ok {
			err = runShowTransactionState(session, session.ResultWriter)
		} else {
			switch txnState.State() {
			case Open, FirstBatch:
				err = e.execStmtInOpenTxn(
					session, stmt, pinfo, txnPrefix && (i == 0), /* firstInTxn */
					avoidCachedDescriptors, automaticRetryCount, session.ResultWriter)
			case Aborted, RestartWait:
				err = e.execStmtInAbortedTxn(session, stmt, session.ResultWriter)
			case CommitWait:
				err = e.execStmtInCommitWaitTxn(session, stmt, session.ResultWriter)
			default:
...
func runShowTransactionState(session *Session, resultWriter StatementResultWriter) error {
	resultWriter.BeginResult((*parser.Show)(nil))
...
func (e *Executor) execStmtInAbortedTxn(
	session *Session, stmt Statement, resultWriter StatementResultWriter,
) error {
	txnState := &session.TxnState
	...
	case *parser.CommitTransaction, *parser.RollbackTransaction:
		if txnState.State() == RestartWait {
			return rollbackSQLTransaction(txnState, resultWriter)
		}
		...
		txnState.resetStateAndTxn(NoTxn)
		resultWriter.BeginResult((*parser.RollbackTransaction)(nil))
		return resultWriter.EndResult()
...
func (e *Executor) execStmtInCommitWaitTxn(
	...
	case *parser.CommitTransaction, *parser.RollbackTransaction:
		// Reset the state to allow new transactions to start.
		txnState.resetStateAndTxn(NoTxn)
		resultWriter.BeginResult((*parser.CommitTransaction)(nil))
		return resultWriter.EndResult()
	default:
...
//  using cached descriptors.
// automaticRetryCount: increases with each retry; 0 for the first attempt.
// resultWriter: the writer to send results to.
func (e *Executor) execStmtInOpenTxn(
	...
	defer func() {
		if err != nil {
			...
			!canStayInFirstBatchState(stmt, session.ResultWriter) {
	...
	if runInParallel {
		...
		err = e.execStmtInParallel(stmt, p, resultWriter)
	} else {
		p.autoCommit = txnState.implicitTxn && !e.cfg.TestingKnobs.DisableAutoCommit
		err = e.execStmt(stmt, p, automaticRetryCount, resultWriter)
	...
	tResult := &traceResult{tag: resultWriter.GetPGTag(), count: -1}
	switch resultWriter.GetStatementType() {
	case parser.RowsAffected, parser.Rows:
		tResult.count = resultWriter.GetRowsAffected()
	}
...
func rollbackSQLTransaction(txnState *txnState, resultWriter StatementResultWriter) error {
...
func commitSQLTransaction(
	txnState *txnState, commitType commitType, resultWriter StatementResultWriter,
) error {
	if !txnState.TxnIsOpen() {
...
// runs it.
func (e *Executor) execDistSQL(planner *planner, tree planNode, resultWriter StatementResultWriter) error {
	...
	if resultWriter.GetStatementType() == parser.RowsAffected {
...
func (e *Executor) execClassic(
	planner *planner, plan planNode, resultWriter StatementResultWriter,
) error {
	ctx := planner.stmt.queryMeta.ctx
...
func setupWriter(stmt Statement, plan planNode, resultWriter StatementResultWriter) error {
	stmtAst := stmt.AST
	resultWriter.BeginResult(stmtAst)
	if stmtAst.StatementType() == parser.Rows {
		columns := planColumns(plan)
		resultWriter.SetColumns(columns)
...
func (e *Executor) execStmt(
	stmt Statement, planner *planner, automaticRetryCount int, resultWriter StatementResultWriter,
) error {
...
func (e *Executor) execStmtInParallel(
	stmt Statement, planner *planner, resultWriter StatementResultWriter,
) error {
	session := planner.session
	ctx := session.Ctx()
	...
	session.parallelizeQueue.Add(ctx, plan, func(plan planNode) error {
		bufferedWriter := newBufferedWriter(session)
...
func canStayInFirstBatchState(stmt Statement, resultWriter GroupResultWriter) bool {
	if !resultWriter.CanAutomaticallyRetry() {
		return false
	}
