...
func (e *Executor) execStmtInOpenTxn(
	...
	defer func() {
		...
		// statements we have:
		// - Some statements explicitly decide what state transition they want. For
		// them, explicitStateTransition is set and transition is filled in, and so
		// we obey their instructions.
		...
		if explicitStateTransition {
			if err != nil {
				panic(fmt.Sprintf("unexpected return err when explicitStateTransition is set: %s", err))
			}
			err = transition.err
			if transition.transitionDependentOnErrType {
				if err == nil {
