...
type DistSQLPlannerTestingKnobs struct {
	...
	// of gossip for figuring out a node's DistSQL version. The callback is
	// supposed to populate res, or it can return an error to simulate gossip not
	// having any info for the node. If the test wants to simulate the node
