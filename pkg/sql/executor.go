...
// Currently the only error returned from execParsed are communication errors
// if we're using v3 as our result writer or memory errors if we're using
// buffered writer.
func (e *Executor) execParsed(
	...
	for len(stmts) > 0 {
		...
		// If we encountered a isWireFailureErr then the connection had an issue so
		// we return early and close the connection.
		...
		if err != nil {
			...
			if wireFailureErr := resultWriter.SetError(convertToErrWithPGCode(err)); wireFailureErr != nil {
				txn.CleanupOnError(session.Ctx(), wireFailureErr)
				return wireFailureErr
			}
		}
		...
		if !txnState.TxnIsOpen() {
			...
			// found).
			err := txnState.schemaChangers.execSchemaChanges(session.Ctx(), e, session, groupResultWriter.NewStatementResultWriter())
			if err != nil {
				return err
			}
		}
...
func (e *Executor) execStmtsInCurrentTxn(
	...
	for i, stmt := range stmts {
		...
		if _, ok := stmt.AST.(*parser.ShowTransactionStatus); ok {
			...
			case Open, FirstBatch:
				err = e.execStmtInOpenTxn(
					...
					avoidCachedDescriptors, automaticRetryCount, groupResultWriter)
		...
		if filter := e.cfg.TestingKnobs.StatementFilter; filter != nil {
			filter(session.Ctx(), stmt.String(), session.ResultWriter)
		}
		if err != nil {
	...
	case *parser.RollbackToSavepoint, *parser.Savepoint:
		...
		if txnState.State() == RestartWait {
			...
			txnState.resetForNewSQLTxn(
				...
				curTs /* sqlTimestamp */, curIso /* isolation */, curPri /* priority */, groupResultWriter)
		}
...
func (e *Executor) execStmtInOpenTxn(
	...
	defer func() {
		if err != nil {
			...
			!canStayInFirstBatchState(stmt, groupResultWriter) {
...
func (e *Executor) execDistSQL(
	planner *planner, tree planNode, rowResultWriter RowResultWriter,
) error {
...
func (e *Executor) execClassic(
	planner *planner, plan planNode, rowResultWriter RowResultWriter,
) error {
	ctx := planner.session.Ctx()
...
func canStayInFirstBatchState(stmt Statement, groupResultWriter GroupResultWriter) bool {
	if !groupResultWriter.CanAutomaticallyRetry() {
		return false
	}
