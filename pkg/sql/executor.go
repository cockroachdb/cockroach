...
func (e *Executor) execStmtInAbortedTxn(
	...
	case *parser.RollbackToSavepoint, *parser.Savepoint:
		...
		// TODO(andrei): should we return some result here? We're not doing anything
		// with res, we're not even closing it, which is against the contract.
...
func rollbackSQLTransaction(txnState *txnState, res StatementResult) error {
	...
	if err != nil {
		...
		// TODO(andrei): I think we need to write to res here just like below.
		txnState.resetStateAndTxn(NoTxn)
		return err
	}
