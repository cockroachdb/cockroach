...
type ExecutorTestingKnobs struct {
	...
	// skipCommit should be returned true if the Executor should skip doing the
	// txn.Commit() call. If an error is returned, that error will be considered
	// the result of txn.Commit(). If an error is returned, skipCommit has to be
	// true.
	//
			...
			if err == nil {
				...
				if !txn.IsCommitted() {
					...
					if e.cfg.TestingKnobs.BeforeAutoCommit != nil {
						skipCommit, err = e.cfg.TestingKnobs.BeforeAutoCommit(session.Ctx(), stmtsToExec[0].String())
					}
					if !skipCommit {
