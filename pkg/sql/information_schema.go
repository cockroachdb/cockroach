...
func forEachTableDescWithTableLookupInternalFromDescriptors(
	...
	for _, tbID := range lCtx.tbIDs {
		...
		// The system database still uses a synthetic public schema. Otherwise, we
		// lookup the schema normally.
		if !dbDesc.HasPublicSchemaWithDescriptor() && table.GetParentSchemaID() == keys.SystemPublicSchemaID {
		...
		if err != nil {
			...
			// missing temporary schema name. Temporary schemas do have namespace
