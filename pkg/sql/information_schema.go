	...
	indexes: []virtualIndex{
		{
			populate: func(ctx context.Context, constraint tree.Datum, p *planner, db *DatabaseDescriptor,
				...
				desc, err := ResolveExistingObject(ctx, p, tree.NewTableName(tree.Name(db.Name), tree.Name(name)),
				...
				schema, ok := schemas[desc.ID]
				if !ok {
					schema = ""
				}
				return addTablesTableRow(addRow)(db, schema, desc.TableDesc())
			},
		},
	},
}
