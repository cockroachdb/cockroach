...
func forEachTableDescWithTableLookupInternalFromDescriptors(
	...
	if virtualOpts == virtualMany || virtualOpts == virtualCurrentDB {
		...
		iterate := func(dbDesc catalog.DatabaseDescriptor) error {
			for _, virtSchemaEntry := range vEntries {
	...
	for _, tbID := range lCtx.tbIDs {
		...
		if err != nil {
			...
			// missing temporary schema name. The only schema which does not have a
			// descriptor is the temporary schema. Temporary schemas do, however, have
			// namespace entries. The below code will go and lookup schema names from
			// the namespace table if needed to qualify the name of a temporary table.
