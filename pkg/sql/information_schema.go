...
https://www.postgresql.org/docs/9.5/infoschema-columns.html`,
	...
	populate: func(ctx context.Context, p *planner, dbContext catalog.DatabaseDescriptor, addRow func(...tree.Datum) error) error {
		colCommentType := []catalogkeys.CommentType{catalogkeys.ColumnCommentType}
		...
		return forEachTableDesc(ctx, p, dbContext, virtualMany, func(
			...
			columnComments, err := p.Descriptors().LoadComments(ctx, p.Txn(), colCommentType, table.GetID())
			if err != nil {
				return err
			}
			columnComments.ForEachCommentOnDescriptor(
				table.GetID(), func(key catalogkeys.CommentKey, cmt string) error {
