...
func forEachSchema(
	...
	schemaNames, err := getSchemaNames(ctx, p, db, false)
	if err != nil {
		return err
	}
...
func forEachDatabaseDesc(
	...
	if dbContext == nil {
		allDbDescs, err := p.Descriptors().GetAllDatabaseDescriptors(ctx, p.txn, false)
		if err != nil {
			return err
		}
		dbDescs = allDbDescs
		...
		fetchedDbDesc, err := catalogkv.GetDatabaseDescriptorsFromIDs(
			ctx, p.txn, p.ExecCfg().Codec, []descpb.ID{dbContext.GetID()}, false,
		)
		if err != nil {
...
func forEachTypeDesc(
	...
	descs, err := p.Descriptors().GetAllDescriptors(ctx, p.txn, true)
	if err != nil {
		return err
	}
	schemaNames, err := getSchemaNames(ctx, p, dbContext, false)
	if err != nil {
		return err
	}
