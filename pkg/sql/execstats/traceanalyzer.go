...
type QueryLevelStats struct {
	...
	ContentionEvent       []roachpb.ContentionEvent
}
...
func (s *QueryLevelStats) Accumulate(other QueryLevelStats) {
	...
	if len(other.ContentionEvent) > 0 {
		s.ContentionEvent = append(s.ContentionEvent, other.ContentionEvent...)
	}
}
...
func (a *TraceAnalyzer) ProcessStats() error {
	...
	for _, stats := range a.processorStats {
		...
		if len(stats.KV.ContentionEvents) > 0 {
	...
	a.queryLevelStats.ContentionEvent = allContentionEvents
