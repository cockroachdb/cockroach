...
type NodeLevelStats struct {
	...
	NetworkHopsGroupedByNode      map[roachpb.NodeID]int64
}
...
func (a *TraceAnalyzer) ProcessStats() error {
	...
	a.nodeLevelStats = NodeLevelStats{
		...
		NetworkHopsGroupedByNode:      make(map[roachpb.NodeID]int64),
	}
	var errs error
	...
	for _, stats := range a.streamStats {
		...
		hops, err := getNetworkHopsFromComponentsStats(stats.stats)
		if err != nil {
			errs = errors.CombineErrors(errs, errors.Wrap(err, "error calculating network hops"))
		} else {
			a.nodeLevelStats.NetworkHopsGroupedByNode[stats.originNodeID] += hops
		}
	}
	...
	for _, networkHops := range a.nodeLevelStats.NetworkHopsGroupedByNode {
		a.queryLevelStats.NetworkHops += networkHops
	}
	return errs
}
...
func getNetworkHopsFromComponentsStats(v *execinfrapb.ComponentStats) (int64, error) {
	...
	if v.NetRx.MessagesReceived.HasValue() {
		if v.NetTx.MessagesSent.HasValue() {
			return 0, errors.Errorf("could not get network hops; both MessagesReceived and MessagesSent are set")
		}
	...
	return 0, errors.Errorf("could not get network hops; neither MessagesReceived and MessagesSent is set")
}
