...
// ProcessNodeLevelStats calculates all the stats for the trace and stores them in TraceAnalyzer.
// ProcessNodeLevelStats tries to calculate as many stats as possible. If errors occur
// while calculating stats, ProcessNodeLevelStats returns the combined errors to the caller
// but continues calculating other stats.
func (a *TraceAnalyzer) ProcessNodeLevelStats() error {
	...
	for _, stats := range a.streamStats {
		...
		if err != nil {
			...
			networkBytesSentGroupedByNode[stats.originNodeID] += bytes
		}
		...
		if stats.stats.FlowStats.MaxMemUsage.HasValue() {
			if memUsage := int64(stats.stats.FlowStats.MaxMemUsage.Value()); memUsage > maxMemoryUsageGroupedByNode[stats.originNodeID] {
				maxMemoryUsageGroupedByNode[stats.originNodeID] = memUsage
			}
		}
	}
	...
	for _, stats := range a.flowStats {
		...
		for _, v := range stats.stats {
			if v.FlowStats.MaxMemUsage.HasValue() {
				if memUsage := int64(v.FlowStats.MaxMemUsage.Value()); memUsage > maxMemoryUsageGroupedByNode[stats.nodeID] {
					maxMemoryUsageGroupedByNode[stats.nodeID] = memUsage
				}
			}
		}
	}
	...
	a.nodeLevelStats = NodeLevelStats{
		NetworkBytesSentGroupedByNode: networkBytesSentGroupedByNode,
		MaxMemoryUsageGroupedByNode:   maxMemoryUsageGroupedByNode,
	}
...
func GetQueryLevelStats(
	...
	for _, metadata := range flowMetadata {
		...
		if err := analyzer.ProcessNodeLevelStats(); err != nil {
		...
		for _, maxMemUsage := range nodeLevelStats.MaxMemoryUsageGroupedByNode {
			if maxMemUsage > queryMaxMemUsage {
				queryMaxMemUsage = maxMemUsage
			}
		}
	}
