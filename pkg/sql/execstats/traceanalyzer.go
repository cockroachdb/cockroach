...
// processNodeLevelStats calculates all the node level stats for the trace and stores them
// in TraceAnalyzer. processNodeLevelStats tries to calculate as many stats as possible. If
// errors occur while calculating stats, processNodeLevelStats returns the combined errors to
// the caller but continues calculating other stats.
...
// processQueryLevelStats calculates all the stats for the trace and stores them in TraceAnalyzer.
func (a *TraceAnalyzer) processQueryLevelStats() {
...
// If errors occur while calculating stats, ProcessStats returns the combined errors to
// the caller but continues calculating other stats.
func (a *TraceAnalyzer) ProcessStats() error {
	err := a.processNodeLevelStats()
	a.processQueryLevelStats()
	return err
}
