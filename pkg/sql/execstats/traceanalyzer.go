...
func (a *TraceAnalyzer) ProcessStats() {
	...
	// Max memory and max disk usages are grouped by node, so we need a couple
	// of maps.
	...
	for instanceID, stats := range a.flowStats {
		...
		if idx, found := slices.BinarySearch(s.SQLInstanceIDs, int32(instanceID)); !found {
			s.SQLInstanceIDs = slices.Insert(s.SQLInstanceIDs, idx, int32(instanceID))
		}
		for _, v := range stats.stats {
			// Avoid duplicates and empty string.
			if v.Component.Region != "" {
				if idx, found := slices.BinarySearch(s.Regions, v.Component.Region); !found {
					s.Regions = slices.Insert(s.Regions, idx, v.Component.Region)
				}
			}

			if v.FlowStats.MaxMemUsage.HasValue() {
				if memUsage := int64(v.FlowStats.MaxMemUsage.Value()); memUsage > maxMemoryUsageGroupedByNode[instanceID] {
					maxMemoryUsageGroupedByNode[instanceID] = memUsage
				}
			}
			if v.FlowStats.MaxDiskUsage.HasValue() {
				if diskUsage := int64(v.FlowStats.MaxDiskUsage.Value()); diskUsage > maxDiskUsageGroupedByNode[instanceID] {
					maxDiskUsageGroupedByNode[instanceID] = diskUsage
				}
			}
	...
	// Finalize max memory and max disk usages.
	for _, maxMemUsage := range maxMemoryUsageGroupedByNode {
		if maxMemUsage > a.queryLevelStats.MaxMemUsage {
			a.queryLevelStats.MaxMemUsage = maxMemUsage
		}
	}
	for _, maxDiskUsage := range maxDiskUsageGroupedByNode {
