...
func (s *OperatorStats) formatStats(fn func(suffix string, value interface{})) {
	...
	if s.NetRx.RowsReceived.HasValue() {
		fn("network rows received", s.NetRx.RowsReceived.Value())
	}
	...
	// Network Tx stats.
	if s.NetTx.RowsSent.HasValue() {
		fn("network rows sent", s.NetTx.RowsSent.Value())
	}
	...
	// KV stats.
	if s.KV.KVTime != 0 {
		// TODO(radu): should we rename this to "KV Time" which is more specific?
		fn("IO time", s.KV.KVTime.Round(time.Microsecond))
	}
	if s.KV.RowsRead.HasValue() {
		fn("KV rows read", s.KV.RowsRead.Value())
	}
...
// collected.
func (s *OperatorStats) MakeDeterministic() {
