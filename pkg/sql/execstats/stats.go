...
type ContentionEventsListener struct {
	...
	// This is used to distinguish "self" latch wait time (e.g. for QueryIntent)
	// from "contention" latch wait time.
	txnID uuid.UUID
}
...
func (c *ContentionEventsListener) Notify(event tracing.Structured) tracing.EventConsumptionStatus {
	...
	case *kvpb.ContentionEvent:
		atomic.AddInt64(&c.cumulativeContentionTime, int64(t.Duration))
	case *kvpb.LatchWaitEvent:
		if t.TxnID == nil || c.txnID != *t.TxnID {
			atomic.AddInt64(&c.cumulativeContentionTime, int64(t.Duration))
		}
	default:
