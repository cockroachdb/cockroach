...
func (r *stmtDiagnosticsRequestRegistry) GetStatementDiagnostics(
	ctx context.Context, statementDiagnosticsID int,
) (*StmtDiagnostics, error) {
	var err error
	...
	return r.db.Txn(ctx, func(ctx context.Context, txn *client.Txn) error {
		...
		if json, err := traceToJSON(trace); err != nil {
			row, err := r.ie.QueryRowEx(ctx, "stmt-diag-insert-trace", txn,
				...
				req.queryFingerprint, stmt, collectedAt, err.Error())
			if err != nil {
				return err
			}
			...
			row, err := r.ie.QueryRowEx(ctx, "stmt-diag-insert-trace", txn,
				...
				"INSERT INTO system.statement_diagnostics "+
					...
					"VALUES ($1, $2, $3, $4) RETURNING id",
				req.queryFingerprint, stmt, collectedAt, json)
			if err != nil {
				return err
			}
			traceID = int(*row[0].(*tree.DInt))
