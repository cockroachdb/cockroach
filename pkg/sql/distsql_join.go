...
func (dsp *DistSQLPlanner) tryCreatePlanForInterleavedJoin(
	...
	for _, partitions := range [][]spanPartition{ancsPartitions, descPartitions} {
		for _, part := range partitions {
			if _, ok := nodeIDs[part.node]; !ok {
				nodeIDs[part.node] = true
			}
		}
	}
	...
	plan.ResultRouters = make([]distsqlplan.ProcessorIdx, len(nodeIDs))
...
// potentially remapped to other parent join spans.
//
	...
	for _, childPart := range childSpans {
		for _, childSpan := range childPart.spans {
			...
			for len(spansLeft) > 0 {
				...
				spansLeft[0], spansLeft[len(spansLeft)-1] = spansLeft[len(spansLeft)-1], spansLeft[0]
