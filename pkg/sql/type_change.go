...
func (t *typeSchemaChanger) canRemoveEnumValue(
	...
	for _, ID := range typeDesc.ReferencingDescriptorIDs {
		...
		if desc.DescriptorType() == catalog.Table {
			...
			if err != nil {
				return errors.Wrapf(err,
					"unable to find referencing table for enum value %q", member.LogicalRepresentation)
			}
			err = t.canRemoveEnumValueFromTable(ctx, typeDesc, txn, member, descsCol, tblDesc, ID)
			if err != nil {
				return err
			}
		} else if desc.DescriptorType() == catalog.Function {
			udfDesc, err := descGetter.Function(ctx, ID)
			if err != nil {
				return errors.Wrapf(err,
					"unable to find referencing function for enum value %q", member.LogicalRepresentation)
			}
			err = t.canRemoveEnumValueFromUDF(typeDesc, member, udfDesc)
			if err != nil {
				return err
			}
		} else {
...
func (t *typeSchemaChanger) canRemoveEnumValueFromArrayUsages(
	...
	for i := 0; i < arrayTypeDesc.NumReferencingDescriptors(); i++ {
		...
		if !ok {
			return errors.AssertionFailedf("expected relation type, got %q while checking for enum usage", desc.DescriptorType())
		}
