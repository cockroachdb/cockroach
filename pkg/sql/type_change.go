...
func (t *typeSchemaChanger) exec(ctx context.Context) error {
	...
	if (typeDesc.Kind == descpb.TypeDescriptor_ENUM ||
		...
		run := func(ctx context.Context, txn *kv.Txn, descsCol *descs.Collection) error {
			...
			filterEnumMembers(typeDesc, func(member *descpb.TypeDescriptor_EnumMember) bool {
...
// filterEnumMembers modifies the supplied typeDesc by removing all enum members
// as dictated by shouldRemove.
func filterEnumMembers(
...
func (t *typeSchemaChanger) canRemoveEnumValue(
	...
	for _, ID := range typeDesc.ReferencingDescriptorIDs {
		...
		// the type whose value is being removed, the notable exception being
		// Regional by Table multi-region tables. In this case, no valid query is
		// constructed and there's nothing to execute. The validation for them is
