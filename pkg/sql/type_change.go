...
func (t *typeSchemaChanger) exec(ctx context.Context) error {
	...
	if (typeDesc.GetKind() == descpb.TypeDescriptor_ENUM ||
		...
		// situation causes a panic, and the query fails. To address this issue,
		// and only in the DROP REGION case, we repartition the tables first,
		// and drop the value from the enum in a separate transaction. Note that
		// we must refresh the table descriptors before we proceed to the drop
		// enum portion, so that we ensure that any concurrent queries see the
		// descriptor updates in the correct order.
