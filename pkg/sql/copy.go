...
func newCopyMachine(
	...
	c := &copyMachine{
		...
		session: s,
		txnOpt:  txnOpt,
		// The planner will be reset before use.
		p: planner{},
	}
...
func (c *copyMachine) run(ctx context.Context) error {
	...
	for {
		...
		cont, err := func() (cont bool, retErr error) {
			cleanup := c.preparePlanner(ctx)
			defer func() {
				if err := cleanup(); err != nil {
					if retErr == nil {
			...
			case pgwirebase.ClientMsgCopyDone:
				...
				if c.buf.Len() > 0 {
					if err := c.addRow(ctx, c.buf.Bytes(), c.p.EvalContext()); err != nil {
						return false, err
					}
				}
				if err := c.processCopyData(
			...
			case pgwirebase.ClientMsgCopyFail:
				return false, fmt.Errorf("client canceled COPY")
			...
			default:
				return false, pgwirebase.NewUnrecognizedMsgTypeErr(typ)
			}
			panic("not reached")
		}()
		if err != nil {
...
func (c *copyMachine) processCopyData(
	...
	for c.buf.Len() > 0 {
		...
		if err := c.addRow(ctx, line, evalCtx); err != nil {
			return err
		}
	}
...
// the planner (before preparePlanner is called again).
func (c *copyMachine) preparePlanner(ctx context.Context) func() error {
	txn := c.txnOpt.txn
	txnTs := c.txnOpt.txnTimestamp
	stmtTs := c.txnOpt.stmtTimestamp
	autoCommit := false
	if txn == nil {
		txn = client.NewTxn(c.session.execCfg.DB, c.session.execCfg.NodeID.Get())
...
func (c *copyMachine) insertRows(ctx context.Context) error {
