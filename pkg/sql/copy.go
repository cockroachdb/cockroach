...
func newCopyMachine(
	...
	// If there are no column specifiers, we are using every column, and may have
	// to populate each index.
	if c.p.SessionData().CopyExpectAndIgnoreNotVisibleColumns && len(n.Columns) == 0 {
...
func (c *copyMachine) readBinaryData(ctx context.Context, final bool) (brk bool, err error) {
	if len(c.expectedHiddenColumnIdxs) > 0 {
		return false, pgerror.Newf(
			pgcode.FeatureNotSupported,
			"copy_expect_and_ignore_not_visible_columns not supported in binary mode",
		)
	}
