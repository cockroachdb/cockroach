...
func TestJobLogger(t *testing.T) {
	...
	t.Run("valid job lifecycles succeed", func(t *testing.T) {
		...
		woodyExpectation := jobutils.JobExpectation{
			Job:    woodyJob,
		...
		jobutils.VerifyJobRecord(t, db, sql.JobStatusPending, woodyExpectation)
		if err := woodyLogger.Started(ctx); err != nil {
			t.Fatal(err)
		}
		jobutils.VerifyJobRecord(t, db, sql.JobStatusRunning, woodyExpectation)
		woodyLogger.Succeeded(ctx)
		jobutils.VerifyJobRecord(t, db, sql.JobStatusSucceeded, woodyExpectation)
		...
		buzzExpectation := jobutils.JobExpectation{
			Offset: 1,
			Job:    buzzJob,
		...
		jobutils.VerifyJobRecord(t, db, sql.JobStatusPending, buzzExpectation)
		if err := buzzLogger.Started(ctx); err != nil {
			t.Fatal(err)
		}
		jobutils.VerifyJobRecord(t, db, sql.JobStatusRunning, buzzExpectation)
		buzzLogger.Failed(ctx, errors.New("Buzz Lightyear can't fly"))
		jobutils.VerifyJobRecord(t, db, sql.JobStatusFailed, buzzExpectation)
		...
		sidExpectation := jobutils.JobExpectation{
			Offset: 2,
			Job:    sidJob,
			Type:   sql.JobTypeRestore,
			Before: timeutil.Now(),
		...
		jobutils.VerifyJobRecord(t, db, sql.JobStatusPending, sidExpectation)
		sidLogger.Failed(ctx, errors.New("Sid is a total failure"))
		jobutils.VerifyJobRecord(t, db, sql.JobStatusFailed, sidExpectation)
