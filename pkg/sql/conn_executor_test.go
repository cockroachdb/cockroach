...
INSERT INTO t1 SELECT i FROM generate_series(1, 100000) AS g(i);
ALTER TABLE t1 ADD COLUMN b INT DEFAULT 1`,
	...
	for _, tc := range testCases {
		...
		if tc.expectNonTrivialSchemaChangeTime && postCommitJobsInterval.AsFloat64() < 0.3 {
			t.Fatalf(
				"expected schema changes to take longer than 0.3 seconds, took: %v",
