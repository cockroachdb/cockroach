...
func TestTransactionDeadline(t *testing.T) {
	...
	`); err != nil {
		...
		mu.Lock()
		require.True(t, session.Expiration().EqOrdering(mu.txnDeadline))
		mu.Unlock()
	})
	...
	`); err != nil {
		...
		mu.Lock()
		require.True(t, mu.txnDeadline.Less(session.Expiration()))
		mu.Unlock()
	})
	...
	`); err != nil {
		...
		require.True(t, session.Expiration().Less(mu.txnDeadline))
		mu.Unlock()
	})
}
