...
// - In a multi-tenant environment, the transaction deadline should use the
//   min(sqlliveness.Session expiry, lease descriptor expiration) while
...
func TestTransactionDeadline(t *testing.T) {
	...
	ctx := context.Background()
	var txnDeadline hlc.Timestamp
	var txnID string
	var sqlLiveness slprovider.TestLiveness
	...
	checkTransactionDeadlineFilter := func(_ context.Context, ba roachpb.BatchRequest) *roachpb.Error {
		...
		if args, ok := ba.GetArg(roachpb.EndTxn); ok {
			...
			txnDeadline = *et.Deadline
		}
		return nil
	}

	// Use the BeforeStart transaction hook to inject a test SQLLiveness instance.
	injectSQLLiveness := func(cfg *sql.ExecutorConfig) {
		cfg.SQLLiveness = sqlLiveness
	}
	...
	setupSQLLiveness := func(sessionDuration time.Duration) {
		...
		sqlLiveness = slprovider.NewTestSQLLiveness(tc.Stopper(), clock, settings)
		sqlLiveness.Start(ctx)
	}
	s := tc.Server(0)
	// Set up a dummy SQLLiveness instance for bootstrapping the test environment.
	setupSQLLiveness(40 * time.Second)
	_, sqlConn := serverutils.StartTenant(t, s,
		base.TestTenantArgs{
			TenantID: serverutils.TestTenantID(),
			TestingKnobs: base.TestingKnobs{
				SQLExecutor: &sql.ExecutorTestingKnobs{
					BeforeStart: injectSQLLiveness,
				},
			},
		})
	defer sqlConn.Close()
	...
	t.Run("session_expiry_overrides_lease_deadline", func(t *testing.T) {
		...
		setupSQLLiveness(sessionDuration)
	...
	`); err != nil {
		...
		require.True(t, session.Expiration().EqOrdering(txnDeadline))
	})
	...
	`); err != nil {
		...
		require.True(t, txnDeadline.Less(session.Expiration()))
	})
	...
	`); err != nil {
		...
		require.True(t, session.Expiration().Less(txnDeadline))
	})
}
...
func getTxnID(t *testing.T, tx *gosql.Tx) string {
	t.Helper()
	var txnID string
	txIDRows, err := tx.Query(`SELECT id FROM crdb_internal.node_transactions 
WHERE session_id = (SELECT * FROM [show session_id]);`)
	if err != nil {
		t.Fatal(err)
	}
	defer txIDRows.Close()
	txIDRows.Next()
	txIDRows.Scan(&txnID)
	return txnID
}
