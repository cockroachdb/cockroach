...
func TestUpdateRollingQueryCounts(t *testing.T) {
	...
	testData := []struct {
		...
		// Test case for no updates.
		{
			...
			numUpdates:                0,
			expectedQueryCountLength:  0,
			expectedLargestQueryCount: 0,
		},
		// Test case for interval size of 1.
		{
			intervalLength:            1,
			numUpdates:                1,
			expectedQueryCountLength:  1,
			expectedLargestQueryCount: 1,
		},
		...
		// Test case for number of updates less than size of interval.
		{
			intervalLength:            defaultIntervalLength,
			numUpdates:                5,
			expectedQueryCountLength:  5,
			expectedLargestQueryCount: 5,
		},
		// Test case for number of updates 1 more than size of interval
		{
			intervalLength:            defaultIntervalLength,
			numUpdates:                11,
			expectedQueryCountLength:  defaultIntervalLength,
			expectedLargestQueryCount: 11,
		},
		{
			intervalLength:            defaultIntervalLength,
			numUpdates:                20,
			expectedQueryCountLength:  defaultIntervalLength,
			expectedLargestQueryCount: 20,
		},
	}
	...
	for _, tc := range testData {
		...
		if tc.numUpdates > 0 {
			...
			for idx := freshMetrics.RollingQueryCounts.End; idx != freshMetrics.RollingQueryCounts.Start; idx = freshMetrics.RollingQueryCounts.PrevIndex(idx) {
				currQC := freshMetrics.RollingQueryCounts.QueryCounts[idx]
				require.Equal(t, currQC.Count(), expectedQueryCount)
				expectedQueryCount--

				// Check if the current index is not equal to the starting pointer.
				...
				// At start pointer, do a final query count check, then exit loop based on loop conditional.
				if idx == freshMetrics.RollingQueryCounts.Start {
					finalQC := freshMetrics.RollingQueryCounts.QueryCounts[idx]
					require.Equal(t, finalQC.Count(), expectedQueryCount)
				}
			}
		}
	}
