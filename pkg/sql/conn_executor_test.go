...
func TestUpdateRollingQueryCounts(t *testing.T) {
	...
	for _, tc := range testData {
		freshMetrics := sql.NewTelemetryLoggingMetrics(defaultAlpha, int64(intervalLength))
		...
		if tc.numUpdates <= intervalLength {
			require.Equal(t, queryCountLength, tc.numUpdates)
		} else {
			require.Equal(t, queryCountLength, intervalLength)
		}
