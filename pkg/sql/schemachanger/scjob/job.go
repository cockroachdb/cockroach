	...
	for i, s := range sc.Stages {
		if err := descs.Txn(ctx, settings, lm, ie, db, func(ctx context.Context, txn *kv.Txn, descriptors *descs.Collection) error {
			...
			if err := scexec.NewExecutor(
				...
				execCtx.ExecCfg().InternalExecutor, execCtx.ExecCfg().Settings, &execCtx.ExtendedEvalContext().EvalContext, sql.ValidateForwardIndexes, sql.ValidateInvertedIndexes).ExecuteOps(ctx, s.Ops, scexec.TestingKnobMetadata{
