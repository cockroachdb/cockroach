...
func loadProgressesAndMaybePerformInitialScan(
	...
	{
		didScan, err := scanDestinationIndexes(ctx, deps, progresses, tables, tracker)
		if err != nil {
			return nil, err
		}
		if didScan {
...
func scanDestinationIndexes(
	...
	for i := range progresses {
		...
		didScan = true
		if err := tracker.SetBackfillProgress(ctx, progresses[i]); err != nil {
			return false, err
		}
	}
	return didScan, nil
...
func runBackfills(
	...
	stop := deps.PeriodicProgressWriter().StartPeriodicUpdates(ctx, tracker)
	defer func() { _ = stop() }()
