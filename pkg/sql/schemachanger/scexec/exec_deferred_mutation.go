...
// interface.
func (s *deferredState) EnqueueEvent(
	id descpb.ID,
	metadata scpb.TargetMetadata,
	details eventpb.CommonSQLEventDetails,
	event logpb.EventPayload,
) error {
	existing := s.eventsByStatement[metadata.StatementID]
	if s.eventsByStatement == nil {
		s.eventsByStatement = make(map[uint32][]eventPayload)
	}
	s.eventsByStatement[metadata.StatementID] = append(existing, eventPayload{
		id:             id,
		event:          event,
