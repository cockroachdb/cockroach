...
//go:generate mockgen  -prog_only github.com/cockroachdb/cockroach/pkg/sql/schemachanger/scexec Catalog,Dependencies,Backfiller,BackfillTracker,IndexSpanSplitter,PeriodicBackfillProgressFlusher
...
func TestExecBackfill(t *testing.T) {
	...
	testCases := []testCase{
		{name: "simple", f: func(t *testing.T, tdb *sqlutils.SQLRunner) {
			...
			require.NoError(t, scexec.ExecuteStage(ctx, deps, scop.MakeOps(
				&scop.BackfillIndex{
					TableID:       tab.GetID(),
					SourceIndexID: 1,
					IndexID:       2,
