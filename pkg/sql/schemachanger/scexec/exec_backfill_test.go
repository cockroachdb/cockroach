...
func TestMergeTimestampSkew(t *testing.T) {
	...
	db0 := tc.ApplicationLayer(0).SQLConn(t, serverutils.DBName("defaultdb"))
	r0 := sqlutils.MakeSQLRunner(db0)
	...
	db1 := tc.ApplicationLayer(1).SQLConn(t, serverutils.DBName("defaultdb"))
	...
	t.Run("create_non_unique_index", func(t *testing.T) {
		...
		ingestFn = func() error {
			...
			additionalInserts++
			r1.Exec(t, fmt.Sprintf("INSERT INTO t_idx(n) VALUES (%d)", keyVal))
			return nil
		}
		...
		grp.GoCtx(func(ctx context.Context) error {
			r0.Exec(t, "CREATE INDEX i1 ON t_idx (n)")
