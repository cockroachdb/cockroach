...
func run(
	...
	case "decompose":
		...
		desc := getDescriptor(ctx, t, fields[0], execCfg, tdb)
		m := make(map[scpb.Element]scpb.Status)
		backRefs := scdecomp.WalkDescriptor(desc, func(status scpb.Status, element scpb.Element) {
			m[element] = status
		})
...
func getDescriptor(
	...
	idInt, err := strconv.Atoi(results[0][0])
	require.NoError(t, err)
	id := descpb.ID(idInt)
	require.NoError(t, sql.DescsTxn(ctx, execCfg, func(ctx context.Context, txn *kv.Txn, col *descs.Collection) (err error) {
		desc, err = col.GetImmutableDescriptorByID(ctx, txn, id, tree.CommonLookupFlags{
			Required:       true,
