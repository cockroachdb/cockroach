...
func (w *walkCtx) walkZoneConfigOptions(config zonepb.ZoneConfig) []scpb.ZoneConfigOption {
	...
	for optionName, optionEntry := range zoneconfig.SupportedZoneConfigOptions {
		if optionEntry.Getter != nil {
			dValue, err := optionEntry.Getter(&config)
			if err != nil {
				panic(err)
			}
