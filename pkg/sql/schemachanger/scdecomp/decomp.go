...
func (w *walkCtx) walkForeignKeyConstraint(
	...
	w.ev(scpb.Status_PUBLIC, &scpb.ForeignKeyConstraint{
		...
		OnUpdateAction:          descpb.ForeignKeyReferenceActionType[c.OnUpdate()],
		OnDeleteAction:          descpb.ForeignKeyReferenceActionType[c.OnDelete()],
		CompositeKeyMatchMethod: descpb.ForeignKeyReferenceMatchValue[c.Match()],
	})
	w.ev(scpb.Status_PUBLIC, &scpb.ConstraintWithoutIndexName{
		TableID:      tbl.GetID(),
		ConstraintID: c.GetConstraintID(),
