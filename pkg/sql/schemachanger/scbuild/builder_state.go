...
func (b *builderState) WrapFunctionBody(
	...
	if err := refProvider.ForEachTableReference(func(tblID descpb.ID, refs []descpb.TableDescriptor_Reference) error {
		var isView bool
		var isSequence bool
		elts := b.QueryByID(tblID)
		if _, _, v := scpb.FindView(elts); v != nil {
			isView = true
		} else if _, _, s := scpb.FindSequence(elts); s != nil {
			isSequence = true
		}
		for _, ref := range refs {
			if isView {
				...
				fnBody.UsesSequenceIDs = append(fnBody.UsesSequenceIDs, tblID)
			} else {
