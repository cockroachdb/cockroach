...
func (b *builderState) ensureDescriptor(id catid.DescID) {
	...
	// Name prefix and namespace lookups.
	{
		...
		if scID := c.desc.GetParentSchemaID(); scID != catid.InvalidDescID {
			parentSchema = b.readDescriptor(scID)
		...
		if parent == nil && parentSchema == nil {
			...
			// map.
			db := c.desc.(catalog.DatabaseDescriptor)
			childSchemas := b.cr.MustGetSchemasForDatabase(b.ctx, db)
			for schemaID, schemaName := range childSchemas {
				...
				if strings.HasPrefix(schemaName, catconstants.PgTempSchemaName) {
					b.tempSchemas[schemaID] = schemadesc.NewTemporarySchema(schemaName, schemaID, db.GetID())
				}
			}
		}
		if parent != nil && parentSchema == nil {
