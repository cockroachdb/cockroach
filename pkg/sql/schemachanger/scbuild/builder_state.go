...
func (b *builderState) nextIndexID(id catid.DescID) (ret catid.IndexID) {
	...
	scpb.ForEachPrimaryIndex(
		b.QueryByID(id),
		func(_ scpb.Status, _ scpb.TargetStatus, index *scpb.PrimaryIndex) {
			if index.IndexID >= ret {
				ret = index.IndexID + 1
			}
		},
	)
	scpb.ForEachTemporaryIndex(
		b.QueryByID(id),
		func(_ scpb.Status, _ scpb.TargetStatus, index *scpb.TemporaryIndex) {
			if index.IndexID >= ret {
				ret = index.IndexID + 1
			}
		},
	...
	scpb.ForEachSecondaryIndex(
		b.QueryByID(id),
		func(_ scpb.Status, _ scpb.TargetStatus, index *scpb.SecondaryIndex) {
...
func (b *builderState) ResolveConstraint(
	...
	scpb.ForEachConstraintWithoutIndexName(elts, func(status scpb.Status, _ scpb.TargetStatus,
		e *scpb.ConstraintWithoutIndexName) {
	...
	if constraintID == 0 {
		...
		if indexID != 0 {
			scpb.ForEachPrimaryIndex(elts, func(_ scpb.Status, _ scpb.TargetStatus,
				e *scpb.PrimaryIndex) {
				if e.TableID == relationID && e.IndexID == indexID {
					constraintID = e.ConstraintID
				}
			})
			scpb.ForEachSecondaryIndex(elts, func(_ scpb.Status, _ scpb.TargetStatus,
				e *scpb.SecondaryIndex) {
