...
func alterTableAddColumn(
	...
	if toType.UserDefined() {
		panic(scbuildctx.NotImplementedErrorf(t, "user defined type in column"))
	}

	if d.IsSerial {
		panic(scbuildctx.NotImplementedErrorf(t.ColumnDef, "contains serial data type"))
	}
	...
	if d.Unique.IsUnique {
		panic(scbuildctx.NotImplementedErrorf(t.ColumnDef, "contains unique constraint"))
	}
	cdd, err := tabledesc.MakeColumnDefDescs(ctx, d, b.SemaCtx(), b.EvalCtx(ctx))
	onErrPanic(err)
	...
	_ = cdd.ForEachTypedExpr(func(expr tree.TypedExpr) error {
		maybeAddSequenceReferenceDependencies(ctx, b, table.GetID(), col, expr)
		return nil
	})
	...
	if toType.UserDefined() {
		...
		onErrPanic(err)
		typeDesc := b.MustReadType(ctx, typeID)
...
func maybeAddSequenceReferenceDependencies(
	...
	for _, seqIdentifier := range seqIdentifiers {
		...
		if seqIdentifier.IsByID() {
			seq = b.MustReadTable(ctx, descpb.ID(seqIdentifier.SeqID))
		} else {
...
func addOrUpdatePrimaryIndexTargetsForAddColumn(
	b scbuildctx.BuildCtx, table catalog.TableDescriptor, colID descpb.ColumnID, colName string,
