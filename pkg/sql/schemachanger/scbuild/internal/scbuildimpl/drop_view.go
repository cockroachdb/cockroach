...
func DropView(ctx context.Context, b scbuildctx.BuildCtx, n *tree.DropView) {
	for _, name := range n.Names {
		_, view := b.ResolveView(ctx, name.ToUnresolvedObjectName(), scbuildctx.ResolveParams{
		...
		dropView(ctx, b, view, n.DropBehavior)
...
func dropView(
	...
	// Any elements added with c will be children of this view.
	{
		...
		onErrPanic(c.AuthorizationAccessor().CheckPrivilege(ctx, view, privilege.DROP))
	...
	// Use the c BuildCtx again.
	{
		...
		removeTypeBackRefDeps(ctx, c, view)
		...
		_ = view.ForeachDependedOnBy(func(dep *descpb.TableDescriptor_Reference) error {
			...
			if behavior != tree.DropCascade {
				name, err := c.CatalogReader().GetQualifiedTableNameByID(ctx, int64(view.GetID()), tree.ResolveRequireViewDesc)
				onErrPanic(err)
				depViewName, err := c.CatalogReader().GetQualifiedTableNameByID(ctx, int64(dep.ID), tree.ResolveRequireViewDesc)
				onErrPanic(err)
