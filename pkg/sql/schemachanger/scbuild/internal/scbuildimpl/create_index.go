...
func CreateIndex(ctx context.Context, b scbuildctx.BuildCtx, n *tree.CreateIndex) {
	_, rel, idx := b.ResolveIndex(ctx, n.Table.ToUnresolvedObjectName(), n.Name, scbuildctx.ResolveParams{
	...
	for _, columnNode := range n.Columns {
		...
		if b.HasNode(func(status scpb.Status, dir scpb.Target_Direction, elem scpb.Element) bool {
			...
			panic(scbuildctx.NotImplementedErrorf(n, "column was added in the current transaction"))
		}
		if columnNode.Expr != nil {
			...
			alterTableAddColumn(ctx, b, rel, addCol, &n.Table)
	...
	if n.Sharded != nil {
		...
		buckets, err := tabledesc.EvalShardBucketCount(ctx, b.SemaCtx(), b.EvalCtx(ctx), n.Sharded.ShardBuckets)
		if err != nil {
			panic(err)
		}
