...
func DropType(ctx context.Context, b scbuildctx.BuildCtx, n *tree.DropType) {
	if n.DropBehavior == tree.DropCascade {
		panic(unimplemented.NewWithIssue(51480, "DROP TYPE CASCADE is not yet supported"))
	}
	for _, name := range n.Names {
		_, typ := b.ResolveType(ctx, name, scbuildctx.ResolveParams{
		...
		dropType(ctx, b, typ, n.DropBehavior)
...
func dropType(
	...
	canDrop := func(desc catalog.TypeDescriptor) {
		...
		if desc.NumReferencingDescriptors() > 0 && behavior != tree.DropCascade {
			...
			for i := 0; i < desc.NumReferencingDescriptors(); i++ {
				...
				name, err := b.CatalogReader().GetQualifiedTableNameByID(ctx, int64(id), tree.ResolveAnyTableKind)
				if err != nil {
	...
	arrayType := b.MustReadType(ctx, typ.GetArrayTypeID())
