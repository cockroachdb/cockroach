...
func isSupportedColumnTableDef(col *tree.ColumnTableDef) bool {
	...
	// An explicit constraint name (e.g. CONSTRAINT pk PRIMARY KEY) lands in
	// Unique.ConstraintName for both UNIQUE and PRIMARY KEY column qualifiers.
	if col.Unique.ConstraintName != "" {
		return false
	}
	...
	// Storage parameters on an inline primary key (e.g. PRIMARY KEY WITH
	// (fillfactor=50)) are not yet handled.
...
func CreateTable(b BuildCtx, n *tree.CreateTable) {
	...
	existing := b.ResolveRelation(n.Table.ToUnresolvedObjectName(), ResolveParams{
		...
		RequiredPrivilege:   0, // collision probe; matches legacy (no priv check)
		WithOffline:         true,
		ResolveTypes:        true,
	})
