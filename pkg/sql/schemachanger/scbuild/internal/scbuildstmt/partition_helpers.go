...
func configureIndexDescForNewIndexPartitioning(
	...
	if partitionByIndex.ContainsPartitioningClause() || partitionAllBy != nil {
		...
		if partitionAllBy == nil {
			...
			partitionBy, err = partitionByFromTableDesc(b, tableID)
			if err != nil {
		...
		if partitionBy != nil {
			...
			updateIndexPartitioning(&indexDesc, false, newImplicitCols, newPartitioning)
		}
	}
...
func partitionByFromTableDesc(b BuildCtx, tableID catid.DescID) (*tree.PartitionBy, error) {
	idx := b.QueryByID(tableID).FilterPrimaryIndex().MustGetOneElement()
	partitioning := mustRetrievePartitioningFromIndexPartitioning(b, tableID, idx.IndexID)
	return partitionByFromTableDescImpl(b, tableID, idx.IndexID, partitioning, 0)
}
...
func createPartitioning(
	...
	if partBy == nil {
		if idxPart.PartitioningDescriptor.NumImplicitColumns > 0 {
	...
	oldKeyColumnNames, _, _ := getSortedColumnNamesInIndexByKind(b, tableID, indexID)
	oldNumImplicitColumns := int(idxPart.PartitioningDescriptor.NumImplicitColumns)
