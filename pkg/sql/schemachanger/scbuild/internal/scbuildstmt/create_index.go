...
func CreateIndex(b BuildCtx, n *tree.CreateIndex) {
	...
	if idxSpec.secondary.IsUnique {
		idxSpec.secondary.ConstraintID = b.NextTableConstraintID(idxSpec.secondary.TableID, false /* useTempID */)
	}
	keyIdx := 0
	keySuffixIdx := 0
	keyStoredIdx := 0
...
func nextRelationIndexID(b BuildCtx, relation scpb.Element) catid.IndexID {
	...
	case *scpb.Table:
		return b.NextTableIndexID(t.TableID, false /* useTempID */)
	case *scpb.View:
...
func maybeCreateAndAddShardCol(
	...
	shardColCkConstraintID := b.NextTableConstraintID(tbl.TableID, false /* useTempID */)
