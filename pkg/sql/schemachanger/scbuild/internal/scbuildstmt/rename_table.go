...
func RenameTable(b BuildCtx, n *tree.RenameTable) {
	...
	checkNameBasedDependencies(b, elts, targetDescriptorID, objectName)
	...
	// Check for name conflicts with the new fully qualified name.
	...
	b.LogEventForExistingPayload(targetElement, renameEvent)
...
func checkNameBasedDependencies(
	b BuildCtx, elts ElementResultSet, descriptorID catid.DescID, objectName tree.TableName,
) {
	backRefs := b.BackReferences(descriptorID)
	backRefs.ForEach(func(current scpb.Status, target scpb.TargetStatus, e scpb.Element) {
		switch elem := e.(type) {
		case *scpb.View:
			// Views depend on other objects by name, so the rename needs to be
			// blocked.
			viewElts := b.QueryByID(elem.ViewID)
		...
		case *scpb.FunctionName:
			panic(sqlerrors.NewDependentBlocksOpError("rename", "relation", objectName.String(), "function", elem.Name))
		}
	})
}
