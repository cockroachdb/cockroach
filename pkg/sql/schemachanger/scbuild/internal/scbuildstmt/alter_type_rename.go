...
func alterTypeRename(
	...
	newArrayName := findFreeNameInSchema(b, currentNS.DatabaseID, currentNS.SchemaID, "_"+newName)
...
func checkTypeNameConflicts(b BuildCtx, newName string, currentNS *scpb.Namespace) {
	tn := tree.MakeTableNameFromPrefix(
		qualifiedSchemaPrefix(b, currentNS.DatabaseID, currentNS.SchemaID),
		tree.Name(newName),
	)
...
func findFreeNameInSchema(b BuildCtx, dbID, schemaID catid.DescID, candidateName string) string {
	prefix := qualifiedSchemaPrefix(b, dbID, schemaID)
	for {
...
// qualifiedSchemaPrefix builds an ObjectNamePrefix from database and schema IDs.
func qualifiedSchemaPrefix(b BuildCtx, dbID, schemaID catid.DescID) tree.ObjectNamePrefix {
	dbNS := b.QueryByID(dbID).FilterNamespace().MustGetOneElement()
	scNS := b.QueryByID(schemaID).FilterNamespace().MustGetOneElement()
	return tree.ObjectNamePrefix{
		CatalogName:     tree.Name(dbNS.Name),
		SchemaName:      tree.Name(scNS.Name),
