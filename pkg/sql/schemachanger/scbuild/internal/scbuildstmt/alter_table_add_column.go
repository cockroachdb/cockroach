...
func alterTableAddColumn(
	...
	if idx := cdd.PrimaryKeyOrUniqueIndexDescriptor; idx != nil {
		...
		idx.ID = b.NextTableIndexID(tbl.TableID, false /* useTempID */)
		{
...
func addColumn(b BuildCtx, spec addColumnSpec, n tree.NodeFormatter) (backing *scpb.PrimaryIndex) {
	...
	addColumnIgnoringNotNull := func(
		...
		if spec.colType.IsVirtual {
			old, _, _, final := getAllPrimaryIndexesSortedBySourcing(b, spec.tbl.TableID)
			if final != nil {
				return final
			} else {
				return old
		...
		oldSpec, inter1Spec, inter2Spec, finalSpec := ensureFourNonNilPrimaryIndexes(b, spec.tbl.TableID)
		if spec.def == nil && spec.colType.ComputeExpr == nil {
			...
			addStoredColumnToPrimaryIndexTargeting(b, spec.tbl.TableID, oldSpec.primary, spec.col, scpb.ToPublic)
		}
		addStoredColumnToPrimaryIndexTargeting(b, spec.tbl.TableID, inter1Spec.primary, spec.col, scpb.Transient)
		addStoredColumnToPrimaryIndexTargeting(b, spec.tbl.TableID, inter2Spec.primary, spec.col, scpb.Transient)
		addStoredColumnToPrimaryIndexTargeting(b, spec.tbl.TableID, finalSpec.primary, spec.col, scpb.ToPublic)
		return finalSpec.primary
	}
...
func addSecondaryIndexTargetsForAddColumn(
	...
	if desc.Unique {
		index.ConstraintID = b.NextTableConstraintID(tbl.TableID, false /* useTempID */)
	}
