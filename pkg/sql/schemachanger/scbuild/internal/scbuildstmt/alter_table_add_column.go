...
func alterTableAddColumn(
	...
	var colDefaultExpression *scpb.Expression
	if d.IsSerial {
		d, colDefaultExpression = alterTableAddColumnSerial(b, d, tn)
	}
	...
	if desc.HasDefault() {
		if colDefaultExpression == nil {
			colDefaultExpression = b.WrapExpression(tbl.TableID, cdd.DefaultExpr)
		} else {
			// Otherwise, a sequence was created above and assigned.
			b.Add(&scpb.SequenceOwner{
				SequenceID: colDefaultExpression.UsesSequenceIDs[0],
				TableID:    tbl.TableID,
				ColumnID:   cdd.ID,
			})
		}
		spec.def = &scpb.ColumnDefaultExpression{
			TableID:    tbl.TableID,
			ColumnID:   spec.col.ColumnID,
			Expression: *colDefaultExpression,
		}
...
func alterTableAddColumnSerial(
	...
	case sessiondatapb.SerialUsesVirtualSequences:
		d.Type = types.Int
		seqOptions = append(seqOptions, tree.SequenceOption{
			Name: tree.SeqOptVirtual,
		})
	case sessiondatapb.SerialUsesCachedSQLSequences:
		cacheValue := sqlclustersettings.CachedSequencesCacheSizeSetting.Get(&b.ClusterSettings().SV)
		seqOptions = append(seqOptions, tree.SequenceOption{
