...
func addSecondaryIndexTargetsForAddColumn(
	...
	err := configureIndexDescForNewIndexPartitioning(b, tbl.TableID, &spec, nil /* partitionByIndex */)
	if err != nil {
		panic(err)
	}
	...
	scpb.ForEachIndexPartitioning(
		...
		func(current scpb.Status, target scpb.TargetStatus, e *scpb.IndexPartitioning) {
			if e.IndexID == newPrimaryIdx.IndexID && spec.partitioning == nil {
				spec.partitioning = &scpb.IndexPartitioning{
					TableID:                tbl.TableID,
					IndexID:                index.IndexID,
					PartitioningDescriptor: *protoutil.Clone(&e.PartitioningDescriptor).(*catpb.PartitioningDescriptor),
				}
			}
		},
	)
