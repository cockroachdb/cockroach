...
type supportedStatement struct {
	...
	// altStatements are concrete statements that correspond to each of the
	// tags in statementTags, if statementTags has more than one element.
	altStatements []tree.Statement
...
var supportedStatements = map[reflect.Type]supportedStatement{
	...
	reflect.TypeOf((*tree.DropRoutine)(nil)): {
		fn:            DropFunction,
		statementTags: []string{tree.DropFunctionTag, tree.DropProcedureTag},
		altStatements: []tree.Statement{&tree.DropRoutine{}, &tree.DropRoutine{Procedure: true}},
		on:            true,
		checks:        isV231Active,
	},
	reflect.TypeOf((*tree.CreateRoutine)(nil)): {
		fn:            CreateFunction,
		statementTags: []string{tree.CreateFunctionTag, tree.CreateProcedureTag},
		altStatements: []tree.Statement{&tree.CreateRoutine{}, &tree.CreateRoutine{IsProcedure: true}},
		on:            true,
		checks:        isV231Active,
	},
...
func init() {
	...
	for statementType, statementEntry := range supportedStatements {
		...
		// Validate that the length of altStatements matches statementTags if
		// statementTags has more than one element.
		if tagLen := len(statementEntry.statementTags); tagLen > 1 &&
			tagLen != len(statementEntry.altStatements) {
			panic(errors.AssertionFailedf(
				"%v entry statementTags length does not match altStatements",
				statementType,
			))
		}
