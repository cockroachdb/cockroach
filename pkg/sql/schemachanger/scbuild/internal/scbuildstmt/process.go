...
func Process(b BuildCtx, initialNode tree.Statement) {
	// Clone the initial tree for the builder, which will inject
	// names inside the tree.
	n := b.SetStatementOnce(initialNode)
	...
	fn.Call(in)
	// Validate that any annotations injected by the builder are sane, all names
	// should be fully resolved if we didn't hit any errors.
	b.ValidateAnnotations()
	// Finalize, the event log state for the current statement.
	b.FinalizeEventLogState(n, b.GetAnnotations())
}
