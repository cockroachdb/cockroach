...
// supportedAlterStatements tracks alter table operations fully supported by
...
var supportedAlterStatements = map[reflect.Type]supportedStatement{
	reflect.TypeOf((*tree.AlterTableAddColumn)(nil)): {alterTableAddColumn, false},
}

func init() {
	// Sanity: All function signatures for supported alter statements take in
	// a BuildCtx, TableDescriptor, AlterTableCmd, and TableName.
	for statementType, statementEntry := range supportedAlterStatements {
...
func alterTableCmd(
	...
	info, ok := supportedAlterStatements[reflect.TypeOf(cmd)]
	if !ok {
