...
func checkConstraintNameConflicts(b BuildCtx, tableID catid.DescID, newName tree.Name) {
	...
	scpb.ForEachConstraintWithoutIndexName(tableElems, func(
		current scpb.Status, target scpb.TargetStatus, e *scpb.ConstraintWithoutIndexName,
	) {
		if tree.Name(e.Name) == newName && target == scpb.ToPublic {
	...
	scpb.ForEachIndexName(tableElems, func(
		...
		if e.Name == string(newName) && target == scpb.ToPublic {
			...
			tableElems.ForEach(func(_ scpb.Status, _ scpb.TargetStatus, elem scpb.Element) {
				switch idx := elem.(type) {
				case *scpb.SecondaryIndex:
					if idx.IndexID == e.IndexID && idx.ConstraintID != 0 {
						isConstraintIndex = true
					}
				case *scpb.PrimaryIndex:
					if idx.IndexID == e.IndexID {
						isConstraintIndex = true
					}
				}
			})
