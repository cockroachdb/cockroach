...
func alterTableAlterPrimaryKey(
	...
	if isNewPrimaryKeySameAsOldPrimaryKey(b, tbl, t) {
		b.EvalCtx().ClientNoticeSender.BufferClientNotice(b,
			pgnotice.Newf("new primary key is the same as the old primary key; nothing needs to be done!"))
		return
	}
	...
	dropOldPrimaryIndex(b, tbl.TableID, oldPrimaryIndexElem.IndexID)

	// Construct and add elements for the new primary index.
	newPrimaryIndexElem := addNewPrimaryIndex(b, tn, tbl, t, oldPrimaryIndexElem)
...
// by searching for any element that is in ABSENT (resp. PUBLIC) and targeting
// PUBLIC (resp. ABSENT).
func fallBackIfConcurrentSchemaChange(b BuildCtx, tableID catid.DescID) {
	b.QueryByID(tableID).ForEachElementStatus(func(current scpb.Status, target scpb.TargetStatus, e scpb.Element) {
		if (current == scpb.Status_PUBLIC && target == scpb.ToAbsent) ||
			(current == scpb.Status_ABSENT && target == scpb.ToPublic) {
			_, _, ns := scpb.FindNamespace(b.QueryByID(tableID))
			if ns == nil {
				panic(fmt.Sprintf("programming error: resolving table %v does not give a Namespace element", tableID))
			}
...
func mustRetrievePrimaryIndexElement(b BuildCtx, tableID catid.DescID) (res *scpb.PrimaryIndex) {
	scpb.ForEachPrimaryIndex(b.QueryByID(tableID), func(
		...
		//               will not be true when there are DDL stmts (e.g.
		//               ADD/DROP COLUMN) before this `ALTER PRIMARY KEY`.
		if current == scpb.Status_PUBLIC {
			res = e
		}
	})
	if res == nil {
		panic(fmt.Sprintf("programming error: resovling table %v does not give a PrimaryIndex element", tableID))
	}
	return res
}

func mustRetrievePrimaryIndexNameElem(
	b BuildCtx, tableID catid.DescID, primaryIndexID catid.IndexID,
) (res *scpb.IndexName) {
	scpb.ForEachIndexName(b.QueryByID(tableID), func(_ scpb.Status, _ scpb.TargetStatus, e *scpb.IndexName) {
...
func mustRetrieveColumnElem(
	...
	if column == nil {
		panic(fmt.Sprintf("programming error: cannot find a Column element for column ID %v", columnID))
	}
	return column
}
...
func mustRetrieveColumnNameElem(
	...
	scpb.ForEachColumnName(b.QueryByID(tableID), func(current scpb.Status, target scpb.TargetStatus, e *scpb.ColumnName) {
		if e.ColumnID == columnID {
			columnName = e
		}
	})
	if columnName == nil {
		panic(fmt.Sprintf("programming error: cannot find a ColumnName element for column ID %v", columnID))
	}
	return columnName
}
...
func mustRetrieveColumnTypeElem(
	...
	scpb.ForEachColumnType(b.QueryByID(tableID), func(current scpb.Status, target scpb.TargetStatus, e *scpb.ColumnType) {
		if e.ColumnID == columnID {
			columnType = e
		}
	})
	if columnType == nil {
		panic(fmt.Sprintf("programming error: cannot find a ColumnType element for column ID %v", columnID))
	}
	return columnType
}
...
func mustRetrieveIndexElement(
	...
	if indexElem == nil {
		panic(fmt.Sprintf("programming error: cannot find an index with ID %v from table %v", indexID, tableID))
	}
	return indexElem
}
...
func mustRetrieveKeyIndexColumns(
	...
	if indexColumns == nil {
		panic(fmt.Sprintf("programming error: cannot find any KEY index columns in index %v from table %v",
			indexID, tableID))
	}
	return indexColumns
}
...
//     5. index name;
// It returns the constructed new primary index element.
func addNewPrimaryIndex(
	...
	addIndexColumnsForNewPrimaryIndexAndTempIndex(b, tn, tbl, t, newPrimaryIndex.IndexID, tempIndex.IndexID)
	addIndexNameForNewPrimaryIndex(b, tn, tbl, t, newPrimaryIndex.IndexID)
...
func addIndexColumnsForNewPrimaryIndexAndTempIndex(
	...
	allNonSystemColumnsName2ID := getAllNonSystemColumnsName2ID(b, tbl.TableID)
	...
	// no SUFFIX columns
	// what's left (all columns - KEY columns) are STORED columns, skipping virtual columns.
	storedIndexColumnsForNewPrimaryIndex := make([]*scpb.IndexColumn, 0)
	storedIndexColumnsForTempIndex := make([]*scpb.IndexColumn, 0)
	for colName, colID := range allNonSystemColumnsName2ID {
		if !mustRetrieveColumnTypeElem(b, tbl.TableID, colID).IsVirtual {
			storedIndexColumnsForNewPrimaryIndex = append(storedIndexColumnsForNewPrimaryIndex, &scpb.IndexColumn{
				TableID:       tbl.TableID,
				IndexID:       newPrimaryIndexID,
				ColumnID:      colID,
				OrdinalInKind: 0, // unpopulated yet
				Kind:          scpb.IndexColumn_STORED,
			})
			storedIndexColumnsForTempIndex = append(storedIndexColumnsForTempIndex, &scpb.IndexColumn{
				TableID:       tbl.TableID,
				IndexID:       temporaryIndexIDForNewPrimarIndex,
				ColumnID:      colID,
				OrdinalInKind: 0, // unpopulated yet
	...
	// and both slices have the same length.
	...
	if len(storedIndexColumnsForNewPrimaryIndex) != len(storedIndexColumnsForTempIndex) {
		panic(fmt.Sprintf("internal error: new primary index and its temp index do not have the same number of" +
			"IndexColumn elements."))
	}

	// Populate `OrdinalInKind` in those stored index column sorted by their column ID.
	sort.Slice(storedIndexColumnsForNewPrimaryIndex, func(i, j int) bool {
		return storedIndexColumnsForNewPrimaryIndex[i].ColumnID < storedIndexColumnsForNewPrimaryIndex[j].ColumnID
	})
	sort.Slice(storedIndexColumnsForTempIndex, func(i, j int) bool {
		return storedIndexColumnsForTempIndex[i].ColumnID < storedIndexColumnsForTempIndex[j].ColumnID
	})
	for i := range storedIndexColumnsForNewPrimaryIndex {
		storedIndexColumnsForNewPrimaryIndex[i].OrdinalInKind = uint32(i)
		storedIndexColumnsForTempIndex[i].OrdinalInKind = uint32(i)
	}

	// Finally, add those index columns
	for i := range storedIndexColumnsForNewPrimaryIndex {
		b.Add(storedIndexColumnsForNewPrimaryIndex[i])
		b.Add(storedIndexColumnsForTempIndex[i])
	}
}
...
func isPrimaryIndexDefaultRowID(
	...
	// Sanity check: input `indexID` should really be index of
	// a primary index.
	...
	// that one column should have name 'rowid' or prefixed by 'rowid'.
...
// getAllNonSystemColumnsName2ID constructs a name to ID mapping
// for all non-system columns.
func getAllNonSystemColumnsName2ID(
