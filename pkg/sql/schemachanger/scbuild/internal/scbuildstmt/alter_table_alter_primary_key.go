	...
	if newPrimaryIndexElem != nil {
		...
		newPrimaryIndexElem = createNewPrimaryIndex(b, tbl, oldPrimaryIndexElem, func(
			...
			for i, col := range t.Columns {
				if colID, exist := allColumnsName2IDMapping[string(col.Column)]; !exist {
					...
					ic := &scpb.IndexColumn{
						...
						Direction:     col.Direction.ToIndexColumnDirection(),
					}
					b.Add(ic)
...
func isNewPrimaryKeySameAsOldPrimaryKey(
	...
	for i, col := range t.Columns {
		...
		if (oldPrimaryIndexKeyColumns[i].ColumnID != colElem.ColumnID) ||
			oldPrimaryIndexKeyColumns[i].Direction != col.Direction.ToIndexColumnDirection() {
			return false
		}
	}
...
func fallBackIfDescColInRowLevelTTLTables(
	...
	for _, col := range t.Columns {
		if col.Direction.ToIndexColumnDirection() != catpb.IndexColumn_ASC {
...
func addIndexColumnsForNewUniqueSecondaryIndexAndTempIndex(
	...
	for i, col := range t.Columns {
		if colID, exist := allColumnsName2IDMapping[string(col.Column)]; !exist {
			...
			newPrimaryIndexKeyColumnDirs[i] = col.Direction.ToIndexColumnDirection()
		}
	}
...
// getAllColumnsName2IDMapping constructs a name to ID mapping
// for all non-system columns.
func getAllColumnsName2IDMapping(b BuildCtx, tableID catid.DescID) (res map[string]catid.ColumnID) {
