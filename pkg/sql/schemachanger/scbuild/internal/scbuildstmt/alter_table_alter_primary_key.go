...
func alterPrimaryKey(b BuildCtx, tn *tree.TableName, tbl *scpb.Table, t alterPrimaryKeySpec) {
	...
	oldSpec, _, inter2Spec, finalSpec := ensureFourNonNilPrimaryIndexes(b, tbl.TableID)
	if !haveSameIndexColsByKind(b, tbl.TableID, oldSpec.primary.IndexID, finalSpec.primary.IndexID, scpb.IndexColumn_KEY) {
	...
	if t.Name != "" && string(t.Name) != finalSpec.name.Name {
		checkIfConstraintNameAlreadyExists(b, tbl, t)
		finalSpec.name.Name = string(t.Name)
	}

	// Set up shard column and sharding descriptor, if applicable.
	setupSharding(b, tbl, t, inter2Spec.primary, finalSpec.primary)

	// Alter index columns from `inter2` and `final`.
	alterPKInPrimaryIndexAndItsTemp(b, tn, tbl.TableID, inter2Spec.primary, t, false /* isIndexFinal */)
	alterPKInPrimaryIndexAndItsTemp(b, tn, tbl.TableID, finalSpec.primary, t, true /* isIndexFinal */)

	b.LogEventForExistingTarget(finalSpec.primary)

	// Recreate all secondary indexes.
	recreateAllSecondaryIndexes(b, tbl, finalSpec.primary, inter2Spec.primary)

	// Drop the rowid column, if applicable.
	rowidToDrop := getPrimaryIndexDefaultRowIDColumn(b, tbl.TableID, oldSpec.primary.IndexID)
	...
	maybeAddUniqueIndexForOldPrimaryKey(b, tn, tbl, t, oldSpec.primary, finalSpec.primary, rowidToDrop)

	// Drop the old shard column, if the old PK is hash-sharded.
	// This behavior is added in V23.1 and gated.
	oldShardColToDrop := getprimaryIndexShardColumn(b, tbl.TableID, oldSpec.primary.IndexID)
