...
func recreateAllSecondaryIndexes(
	...
	scpb.ForEachSecondaryIndex(publicTableElts, func(currentStatus scpb.Status, _ scpb.TargetStatus, idx *scpb.SecondaryIndex) {
		...
		// new primary key. If the original index is non-public, then we have a
		// transaction that is swapping primary keys and creating an index concurrently.
		// So the source index ID will never come into existence (this should be
		// safe on mixed version clusters).
