...
func CreateFunction(b BuildCtx, n *tree.CreateFunction) {
	...
	createPrivilegesFromDefaultPrivileges(b, dbElts, scElts, fnID, privilege.Functions)
...
func validateFunctionRelationReferences(
	...
	if err := refProvider.ForEachTableReference(func(tbl catalog.TableDescriptor, refs []descpb.TableDescriptor_Reference) error {
		_, _, namespace := scpb.FindNamespace(b.QueryByID(tbl.GetID()))
