...
func dropDependentForeignKey(
	...
	if fkOriginTableID == tbl.TableID {
		constraintName := constraintElems.Filter(publicTargetFilter).FilterConstraintWithoutIndexName().MustGetOneElement()
		alterTableDropConstraint(b, tn, tbl, stmt, &tree.AlterTableDropConstraint{
			IfExists:     false,
			Constraint:   tree.Name(constraintName.Name),
			DropBehavior: behavior,
	...
	if behavior != tree.DropCascade {
		originName := b.QueryByID(fkOriginTableID).FilterNamespace().MustGetOneElement()
		constraintName := constraintElems.Filter(publicTargetFilter).FilterConstraintWithoutIndexName().MustGetOneElement()
