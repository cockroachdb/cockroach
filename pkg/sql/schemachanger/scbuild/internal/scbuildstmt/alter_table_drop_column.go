		...
		case *scpb.View:
			if behavior != tree.DropCascade {
				...
				if nsDep.DatabaseID != ns.DatabaseID || nsDep.SchemaID != ns.SchemaID {
					panic(sqlerrors.NewDependingObjBlocksOpError("drop", "column", cn.Name, "view", qualifiedName(b, e.ViewID)))
				}
				panic(sqlerrors.NewDependingObjBlocksOpError("drop", "column", cn.Name, "view", nsDep.Name))
			}
