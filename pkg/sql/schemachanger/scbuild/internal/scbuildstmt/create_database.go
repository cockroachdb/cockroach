...
func CreateDatabase(b BuildCtx, n *tree.CreateDatabase) {
	...
	if err := checkCanAlterToOwner(b, n, owner); err != nil {
		panic(err)
	}
...
func addCreateDatabaseElements(
	...
	b.Add(&scpb.DatabaseRoleSetting{
...
// checkCanAlterToOwner ensures that current user can alter the ownership
...
func checkCanAlterToOwner(b BuildCtx, n *tree.CreateDatabase, newOwner username.SQLUsername) error {
	// Ensure `newOwner` exists.
	roleExists, err := b.RoleExists(b, newOwner)
	if err != nil {
		return err
	}
	if !roleExists {
...
// fallBackIfMultiRegion falls back if
...
func fallBackIfMultiRegion(b BuildCtx, n *tree.CreateDatabase) {
	// This recover is a hacky way to pass one of the logic test where we attempt
	// to create a database as of system time follower_read_timestamp() soon after
	// a node has started. follower_read_timestamp() is set to be ~4.2 seconds in
	// the past, so if we attempt to create a database within 4.2 seconds after
	// the node starts, we will not be able to see the `system` database when we
	// attempt to access it below (to determine whether `system` database is
	// multi-regional) and an error "referenced descriptor ID 1: descriptor not
	// found" will be returned. The legacy schema changer, however, has code
	// slightly rearranged such that it will hit error "cannot execute CREATE
	// DATABASE in a read-only transaction". We therefore attempt to recover such
	// a "referenced descriptor ID 1: descriptor not found" panic and re-panic
	// with NotImplementedError, so the legacy schema changer will do its work and
	// give us the expected error output. For any other panic errors, we simply
	// re-panic with the same error.
	defer func() {
		if r := recover(); r != nil {
			err := r.(error)
			if errors.Is(err, catalog.ErrDescriptorNotFound) &&
				strings.Contains(err.Error(), "referenced descriptor ID 1") {
...
// canCreateDatabase returns nil if current user is a
// superuser (root, node, or an admin) or has CREATEDB role option.
func canCreateDatabase(b BuildCtx) error {
	hasCreateDB, err := b.HasRoleOption(b, roleoption.CREATEDB)
	if err != nil {
		return err
	}
	if !hasCreateDB {
