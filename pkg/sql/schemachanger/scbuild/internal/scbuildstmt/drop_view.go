...
func maybePanicOnDependentView(b BuildCtx, ns *scpb.Namespace, backrefs ElementResultSet) {
	...
	if nsDep.DatabaseID != ns.DatabaseID {
		panic(sqlerrors.NewDependingObjBlocksOpError("drop", "relation", ns.Name, "view", qualifiedName(b, depView.ViewID)))
	}
	panic(sqlerrors.NewDependingObjBlocksOpError("drop", "relation", ns.Name, "view", nsDep.Name))
}
