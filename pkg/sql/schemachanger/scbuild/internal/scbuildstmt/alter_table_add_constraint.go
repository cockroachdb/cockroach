...
func alterTableAddForeignKey(
	...
	//    database as the originTable (i.e. tbl).
	//    We only allow cross database FK references if session variable `allowCrossDatabaseFKsSetting = true`.
	...
	b.Add(&scpb.ForeignKeyConstraint{
		...
		OnUpdateAction:          fkDef.Actions.Update,
		OnDeleteAction:          fkDef.Actions.Delete,
		CompositeKeyMatchMethod: fkDef.Match,
		IndexIDForValidation:    getIndexIDForValidationForConstraint(b, tbl.TableID),
	})
	b.Add(&scpb.ConstraintWithoutIndexName{
		TableID:      tbl.TableID,
...
func retrieveColumnDefaultExpressionElem(
...
) (columnDefaultExpression *scpb.ColumnDefaultExpression) {
	...
	return columnDefaultExpression
}
