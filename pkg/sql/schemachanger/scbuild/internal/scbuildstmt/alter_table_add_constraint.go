...
func alterTableAddForeignKey(
	...
	if originalTableNamespaceElem.DatabaseID != referencedTableNamespaceElem.DatabaseID {
		panic(pgerror.New(pgcode.FeatureNotSupported, "foreign key references cross databases is a "+
			"deprecated feature and no longer supported."))
	}
	...
	b.Add(&scpb.ForeignKeyConstraint{
		...
		OnUpdateAction:          descpb.BuildEquivFKAction(fkDef.Actions.Update).AsScpb,
		OnDeleteAction:          descpb.BuildEquivFKAction(fkDef.Actions.Delete).AsScpb,
		CompositeKeyMatchMethod: descpb.BuildEquivFKMatchMethod(fkDef.Match).AsScpb,
		IndexIDForValidation:    getIndexIDForValidationForConstraint(b, tbl.TableID),
	})
	b.Add(&scpb.ConstraintWithoutIndexName{
		TableID:      tbl.TableID,
