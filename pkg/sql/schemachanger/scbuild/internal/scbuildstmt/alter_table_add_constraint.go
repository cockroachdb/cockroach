...
func alterTableAddForeignKey(
	...
	if b.QueryByID(referencedTableID).FilterRowLevelTTL() != nil {
		b.EvalCtx().ClientNoticeSender.BufferClientNotice(
			b,
			pgnotice.Newf(`
Table %s has row level TTL enabled. This will make TTL deletion jobs more expensive as dependent rows
will need to be updated as well. To improve performance of the TTL job, consider setting
the value of ttl_delete_batch_size to a reasonable number.`, referencedTableNamespaceElem.Name))
	}
