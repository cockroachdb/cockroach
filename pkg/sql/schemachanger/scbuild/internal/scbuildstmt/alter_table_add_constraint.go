...
func alterTableAddConstraint(
	...
	case *tree.UniqueConstraintTableDef:
		if !d.PrimaryKey || t.ValidationBehavior != tree.ValidationDefault {
			panicBecauseGuardRailsIsViolated()
		}
	...
	case *tree.CheckConstraintTableDef:
		...
		alterTableAddConstraintCheck(b, tn, tbl, t)
	default:
		panicBecauseGuardRailsIsViolated()
	}
}

// panicBecauseGuardRailsIsViolated is used when previously passed guardrail, `extraChecks`, is now violated.
func panicBecauseGuardRailsIsViolated() {
	panic(errors.AssertionFailedf("previously passed guardrail `extraChecks` for ALTER TABLE is now violated."))
}
...
// alterTableAddConstraintCheck contains logic for building ALTER TABLE ... ADD CONSTRAINT ... CHECK.
// It assumes `t` is such a command.
func alterTableAddConstraintCheck(
	b BuildCtx, tn *tree.TableName, tbl *scpb.Table, t *tree.AlterTableAddConstraint,
) {
...
func validateConstraintNameIsNotUsed(
	...
	var isInUse bool
	scpb.ForEachConstraintName(b.QueryByID(tbl.TableID), func(
		_ scpb.Status, _ scpb.TargetStatus, e *scpb.ConstraintName,
	) {
...
func getNonDropResultColumns(b BuildCtx, tableID catid.DescID) (ret colinfo.ResultColumns) {
	for _, colID := range getNonDropColumnIDs(b, tableID) {
		colElem := mustRetrieveColumnElem(b, tableID, colID)
		ret = append(ret, colinfo.ResultColumn{
			Name:           mustRetrieveColumnNameElem(b, tableID, colID).Name,
			Typ:            mustRetrieveColumnTypeElem(b, tableID, colID).Type,
			Hidden:         colElem.IsHidden,
			TableID:        tableID,
			PGAttributeNum: uint32(colElem.PgAttributeNum),
		})
	}
...
func constraintNameInUse(b BuildCtx, tableID catid.DescID, name string) (ret bool) {
	scpb.ForEachConstraintName(b.QueryByID(tableID).Filter(publicTargetFilter), func(
		_ scpb.Status, _ scpb.TargetStatus, e *scpb.ConstraintName,
	) {
		if e.Name == name {
			ret = true
