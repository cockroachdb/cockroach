...
func maybeCleanupSchemaLocked(b BuildCtx, id catid.DescID) {
	...
	// being modified. If none exist then the schema_locked element will be
	// deleted.
	keepSchemaLocked := !elts.Filter(validTargetFilter).Filter(func(current scpb.Status, target scpb.TargetStatus, e scpb.Element) bool {
		switch e.(type) {
	...
	if !keepSchemaLocked {
		b.Drop(elts.FilterTableSchemaLocked().MustGetOneElement())
	}
}
