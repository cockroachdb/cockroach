	...
	b.QueryByID(tableID).ForEach(func(current scpb.Status, target scpb.TargetStatus, e scpb.Element) {
		...
		case *scpb.SecondaryIndex:
			...
			if e.RecreateTargetIndexID == old && !excludes.Contains(e.IndexID) {
				e.RecreateTargetIndexID = remaining
			}
...
func (pic *primaryIndexChain) deflate(b BuildCtx) {
	...
	for _, redundant := range redundants {
		...
		// primary indexes).
		nextIndexID := redundant.SourceIndexID()
		if nextIndexID == pic.oldSpec.primary.IndexID {
			// Otherwise, we need to select the next valid index in the chain.
			specs := pic.allPrimaryIndexSpecs(func(spec *indexSpec) bool {
				return !redundantIDs[spec] && spec != &pic.oldSpec
			})
			if len(specs) > 0 {
				nextIndexID = specs[0].indexID()
			}
		}
		updateElementsToDependOnNewFromOld(b, tableID,
			redundant.indexID(), redundant.SourceIndexID(), nextIndexID, catid.IndexSet{} /* excludes */)
