...
// getNonDropColumnIDs returns all non-drop column IDs in a table in ascending order.
func getNonDropColumnIDs(b BuildCtx, tableID catid.DescID) (ret []catid.ColumnID) {
	scpb.ForEachColumn(b.QueryByID(tableID).Filter(publicTargetFilter), func(
		_ scpb.Status, _ scpb.TargetStatus, e *scpb.Column,
	) {
		ret = append(ret, e.ColumnID)
	})
...
// schemaexpr.ExtractColumnIDsInExpr but this function can also extract columns
// added in the same transaction (e.g. for `ADD COLUMN j INT CHECK (j > 0);`,
// schemaexpr.ExtractColumnIDsInExpr will err with "column j does not exist", but
