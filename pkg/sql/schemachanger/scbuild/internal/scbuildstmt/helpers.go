...
func addASwapInIndexByCloningFromSource(
	...
	inSpec, inTempSpec = makeSwapIndexSpec(b, outSpec, out.IndexID, inColumns, true /* inUseTempIDs */)
	if isInFinal {
		inSpec.apply(b.Add)
	} else {
...
func (pic *primaryIndexChain) inflate(b BuildCtx) {
	...
	// oldSpec is always initialized and whenever an inflation happens, we mark
	// the old primary index as dropped.
	pic.oldSpec.apply(b.Drop)

	if pic.inter1Spec.primary == nil {
		pic.inter1Spec, pic.inter1TempSpec = addASwapInIndexByCloningFromSource(b, tableID,
			pic.oldSpec.primary, false /* isInFinal */)
		updateElementsToDependOnNewFromOld(b, tableID,
			pic.oldSpec.primary.IndexID, pic.inter1Spec.primary.IndexID,
			catid.MakeIndexIDSet(pic.inter1Spec.primary.IndexID, pic.inter1Spec.primary.TemporaryIndexID))
	}
	if pic.inter2Spec.primary == nil {
		pic.inter2Spec, pic.inter2TempSpec = addASwapInIndexByCloningFromSource(b, tableID,
			pic.inter1Spec.primary, false /* isInFinal */)
		updateElementsToDependOnNewFromOld(b, tableID,
			pic.inter1Spec.primary.IndexID, pic.inter2Spec.primary.IndexID,
			catid.MakeIndexIDSet(pic.inter2Spec.primary.IndexID, pic.inter2Spec.primary.TemporaryIndexID))
	}
	if pic.finalSpec.primary == nil {
		pic.finalSpec, pic.finalTempSpec = addASwapInIndexByCloningFromSource(b, tableID,
			pic.inter2Spec.primary, true /* isInFinal */)
		updateElementsToDependOnNewFromOld(b, tableID,
			pic.inter2Spec.primary.IndexID, pic.finalSpec.primary.IndexID,
			catid.MakeIndexIDSet(pic.finalSpec.primary.IndexID, pic.finalSpec.primary.TemporaryIndexID))
	}
...
func nonNilIndexSpecSelector(spec *indexSpec) bool {
	return spec != nil
}

func primaryIndexSpecSelector(spec *indexSpec) bool {
	return spec.primary != nil
}

func temporaryIndexSpecSelector(spec *indexSpec) bool {
	return spec.temporary != nil
}

// viewOfChain gives a view of the chain of index specs that satisfies all selectors.
func (pic *primaryIndexChain) viewOfChain(selectors ...func(*indexSpec) bool) (ret []*indexSpec) {
...
// forEachIndexSpec iterates over each spec in the chain and apply `f` on it
// if this spec satisfies all the selectors.
func (pic *primaryIndexChain) forEachIndexSpec(
	f func(spec *indexSpec), selectors ...func(*indexSpec) bool,
) {
	for _, spec := range []*indexSpec{&pic.oldSpec, &pic.inter1Spec, &pic.inter1TempSpec,
		&pic.inter2Spec, &pic.inter2TempSpec, &pic.finalSpec, &pic.finalTempSpec} {
		satisfied := true
...
func (pic *primaryIndexChain) ensureSortedBySourcing() {
	...
	// as source.
	nonNilPrimaryIndexSpecs := pic.viewOfChain(nonNilIndexSpecSelector, primaryIndexSpecSelector)
...
func (pic *primaryIndexChain) mustGetIndexSpecByID(id catid.IndexID) *indexSpec {
	for _, spec := range pic.viewOfChain() {
		if spec.indexID() == id {
			return spec
		}
	}
...
func (pic *primaryIndexChain) clearIndexSpecOfID(id catid.IndexID) {
	switch id {
	case pic.oldSpec.indexID():
		pic.oldSpec = indexSpec{}
	case pic.inter1Spec.indexID():
		pic.inter1Spec = indexSpec{}
	case pic.inter1TempSpec.indexID():
		pic.inter1TempSpec = indexSpec{}
	case pic.inter2Spec.indexID():
		pic.inter2Spec = indexSpec{}
	case pic.inter2TempSpec.indexID():
		pic.inter2TempSpec = indexSpec{}
	case pic.finalSpec.indexID():
		pic.finalSpec = indexSpec{}
	case pic.finalTempSpec.indexID():
		pic.finalTempSpec = indexSpec{}
	}
}
