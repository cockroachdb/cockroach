...
func CreateTrigger(b BuildCtx, n *tree.CreateTrigger) {
	...
	if n.When != nil {
		...
		if len(when.ReferencedColumnIDs) > 0 {
			panic(errors.AssertionFailedf("unexpected column references in WHEN clause"))
		}
...
func fullyQualifyPLpgSQLRelNames(stmt string) string {
	ast, err := plpgsqlparser.Parse(stmt)
	if err != nil {
		panic(err)
	}
	fmtCtx := tree.NewFmtCtx(tree.FmtSerializable)
	fmtCtx.FormatNode(ast.AST)
