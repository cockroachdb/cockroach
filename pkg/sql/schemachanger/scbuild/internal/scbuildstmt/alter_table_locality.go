...
func isTargetLocalitySameAsCurrent(
	currentLocality *tree.Locality, targetLocality *tree.Locality,
) bool {
	if currentLocality.LocalityLevel != targetLocality.LocalityLevel {
		return false
	}
	if currentLocality.LocalityLevel == tree.LocalityLevelTable &&
		currentLocality.TableRegion != targetLocality.TableRegion {
		return false
	}
	if currentLocality.LocalityLevel == tree.LocalityLevelRow &&
		currentLocality.RegionalByRowColumn != targetLocality.RegionalByRowColumn {
		return false
	}
	return true
}
