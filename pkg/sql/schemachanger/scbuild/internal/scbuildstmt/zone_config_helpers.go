...
func applyZoneConfig(
...
) (*zonepb.ZoneConfig, uint32, catid.IndexID, error) {
	...
	if err := completeZone.Validate(); err != nil {
		return nil, 0, 0, pgerror.Wrap(err, pgcode.CheckViolation, "could not validate zone config")
	}
	...
	return partialZone, seqNum, indexID, nil
}
...
func retrievePartialZoneConfig(
	...
	case idxObj:
		...
		// Get the zone configuration of the table.
		var tblZoneConfigElem *scpb.TableZoneConfig
		tblZoneElems := b.QueryByID(targetID).FilterTableZoneConfig()
		tblZoneElems.ForEach(func(current scpb.Status, target scpb.TargetStatus, e *scpb.TableZoneConfig) {
			// We want to get the most recent change that has not applied yet. For transactions, this will
		...
		if tblZoneConfigElem != nil {
			...
			panic(errors.Newf("error during idx zone config elem lookup: no zone config elem "+
				"for table [%d] found", targetID))
		}
		...
		if idxZoneConfigElem != nil {
			parentZC := tblZoneConfigElem.ZoneConfig
			parentZC.Subzones = idxZoneConfigElem.Subzones
			parentZC.SubzoneSpans = idxZoneConfigElem.SubzoneSpans
			partialZone = parentZC
			zc = parentZC
		}
	default:
...
func generateSubzoneSpans(
	...
	idxPartElems := b.QueryByID(tableID).FilterIndexPartitioning()
	idxPartElems.ForEach(func(current scpb.Status, target scpb.TargetStatus, e *scpb.IndexPartitioning) {
		...
		idxColElems := b.QueryByID(tableID).FilterIndexColumn()
		var idxCol *scpb.IndexColumn
		idxColElems.ForEach(func(current scpb.Status, target scpb.TargetStatus, ce *scpb.IndexColumn) {
			if ce.IndexID == e.IndexID {
				idxCol = ce
			}
		})
		...
		indexPartitionCoverings, err := indexCoveringsForPartitioning(
			b, a, tableID, idxCol, partition, subzoneIndexByPartition, emptyPrefix)
		if err != nil {
			panic(err)
		}
...
func decodePartitionTuple(
	...
	// TODO
	// []fetchpb.IndexFetchSpec_KeyColumn is the type i need returned from keyAndSuffixCols
	// we have this info in a certain index element i believe
	indexCol := fetchpb.IndexFetchSpec_Column{
		ColumnID:      index.ColumnID,
		Name:          "", // can get name by doing b.Query
		Type:          nil,
		IsNonNullable: false,
	}
