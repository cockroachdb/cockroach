...
// evaluateZoneOptions gets the input option map ready for use.
...
func validateZoneAttrsAndLocalities(b BuildCtx, currentZone, newZone *zonepb.ZoneConfig) error {
	...
	if b.Codec().ForSystemTenant() {
		...
		return zonepb.ValidateZoneAttrsAndLocalitiesForSystemTenant(b, ss.ListNodesInternal, currentZone, newZone)
	}
