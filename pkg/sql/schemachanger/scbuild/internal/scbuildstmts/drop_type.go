...
func DropType(ctx context.Context, b BuildCtx, n *tree.DropType) {
	if n.DropBehavior == tree.DropCascade {
		panic(unimplemented.NewWithIssue(51480, "DROP TYPE CASCADE is not yet supported"))
	}
	for _, name := range n.Names {
		_, typ := b.ResolveType(ctx, name, ResolveParams{
...
func dropType(
	ctx context.Context, b BuildCtx, typ catalog.TypeDescriptor, behavior tree.DropBehavior,
) {
	switch typ.GetKind() {
