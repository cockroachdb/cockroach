...
func DropDatabase(ctx context.Context, b BuildCtx, n *tree.DropDatabase) {
	db := b.ResolveDatabase(ctx, n.Name, ResolveParams{
	...
	dropIDs := catalog.DescriptorIDSet{}
	{
		...
		doSchema := func(schema catalog.SchemaDescriptor) {
			...
			nodeAdded, schemaDroppedIDs := dropSchema(ctx, c, db, schema, tree.DropCascade)
		...
		for _, schemaID := range schemaIDs.Ordered() {
			schema := c.MustReadSchema(ctx, schemaID)
			if schema.Dropped() {
				continue
			}
