...
func (g *Graph) AddDepEdge(
	...
	if _, ok := g.nodeDepEdgesFrom[de.from]; !ok {
		g.nodeDepEdgesFrom[de.from] = btree.New(2)
	}
	g.nodeDepEdgesFrom[de.from].ReplaceOrInsert(&edgeTreeEntry{
		g:     g,
		edge:  de,
		order: fromTo,
	})
	if _, ok := g.nodeDepEdgesTo[de.to]; !ok {
		g.nodeDepEdgesTo[de.to] = btree.New(2)
	}
	g.nodeDepEdgesTo[de.to].ReplaceOrInsert(&edgeTreeEntry{
		g:     g,
		edge:  de,
		order: toFrom,
	})
