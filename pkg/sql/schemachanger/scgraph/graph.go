...
func (g *Graph) AddOpEdge(t *scpb.Target, from, to scpb.State, op scop.Op) {
	g.AddOpEdgeSlice(t, from, to, []scop.Op{op})
}

// AddOpEdgeSlice adds an op edge connecting the nodes for two states of a target.
func (g *Graph) AddOpEdgeSlice(t *scpb.Target, from, to scpb.State, op []scop.Op) {
	oe := &OpEdge{
