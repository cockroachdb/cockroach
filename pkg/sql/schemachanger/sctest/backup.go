...
func backupSuccessPrepare(
	...
	backupsToTake := make(map[string]struct{})
	...
	knobs := &scexec.TestingKnobs{
		...
		OuterLoop:
			...
			backupArgs.backups[key] = backupInfo{
				url:            url,
				isPostBackfill: isBackupPostBackfill,
			}
			backupsToTake[backupStmt] = struct{}{}
			return nil
		},
	}
	...
	// Take all of the required backups using the timestamps gathered.
	for backupStmt := range backupsToTake {
		tdb.Exec(t, backupStmt)
	}
