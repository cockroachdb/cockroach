...
func backupRollbacks(t *testing.T, factory TestServerFactory, cs CumulativeTestCaseSpec) {
	...
	runfn := func(s serverutils.TestServerInterface, db *gosql.DB) {
		...
		jobID := tdb.QueryRow(t, "SELECT MAX(job_id) FROM [SHOW JOBS]")
		var maxJobID int64
		jobID.Scan(&maxJobID)
...
func exerciseBackupRestore(
	...
	for _, rc := range allRestoreCases {
		t.Run(string(rc.restoreFlavor), func(t *testing.T) {
			...
			// Store the highest job finish time, so we know which jobs are relevant.
			tdb.Exec(t, fmt.Sprintf("USE system"))
			jobID := tdb.QueryRow(t, "SELECT MAX(job_id) FROM [SHOW JOBS]")
			var maxJobID int64
			jobID.Scan(&maxJobID)
