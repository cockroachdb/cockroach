...
// restored correctly. Note that the factory needs to provide a cluster
// with CCL BACKUP/RESTORE functionality enabled.
func Backup(t *testing.T, path string, factory TestServerFactory) {
	cumulativeTestForEachPostCommitStage(t, path, factory, func(t *testing.T, cs CumulativeTestCaseSpec) {
		t.Run("pre_successful_stage_exec", func(t *testing.T) {
			backupDuringSuccess(t, factory, cs)
		})
		if cs.Phase == scop.PostCommitPhase {
			t.Run("post_failed_stage_exec", func(t *testing.T) {
				backupsDuringRollback(t, factory, cs)
			})
		}
	})
}

// BackupMixedVersion is like Backup but in a mixed-version cluster which
// is upgraded prior to restoring the backups.
func BackupMixedVersion(t *testing.T, path string, factory TestServerFactory) {
	Backup(t, path, factory.WithMixedVersion())
}

func backupDuringSuccess(t *testing.T, factory TestServerFactory, cs CumulativeTestCaseSpec) {
...
func backupsDuringRollback(t *testing.T, factory TestServerFactory, cs CumulativeTestCaseSpec) {
	ctx := context.Background()
...
func exerciseBackupRestore(
	...
	for _, rc := range allRestoreCases {
		t.Run(string(rc.restoreFlavor), func(t *testing.T) {
			...
			const vq = `SELECT * FROM crdb_internal.invalid_objects WHERE database_name != 'backups'`
			tdb.CheckQueryResults(t, vq, [][]string{})
