...
func BackupSuccess(t *testing.T, path string, factory TestServerFactory) {
	...
	var backupInfo backupSuccessTestArgs
	defer func() {
		if backupInfo.server.Stopper != nil {
			defer backupInfo.server.Stopper(t)
		}
	}()
	...
	cumulativeTestForEachPostCommitStage(t, path, factory, func(t *testing.T, spec CumulativeTestSpec, dbName string) {
		backupInfo = backupSuccessPrepare(t, factory, spec, dbName)
	}, func(t *testing.T, cs CumulativeTestCaseSpec) {
		backupSuccess(t, backupInfo, cs)
	})
}
