...
type rollbackStageTracker struct {
	...
	targetOrdinal               int
	numInjectedFailures         uint32
	numCheckedExplainInRollback uint32
}
		...
		BeforeStage: func(p scplan.Plan, stageIdx int) error {
			...
			action := func() stageAction {
				...
				a := stageAction{
					...
					numInjectedFailures:         scenario.numInjectedFailures,
					numCheckedExplainInRollback: scenario.numCheckedExplainInRollback,
				}
				if a.active && a.numInjectedFailures == 0 {
					s := p.Stages[stageIdx]
					if s.Phase == scenario.targetPhase && s.Ordinal == a.targetOrdinal {
						scenario.numInjectedFailures++
						a.inject = true
					}
				}
				if a.active && scenario.numInjectedFailures > 0 &&
					a.numCheckedExplainInRollback == 0 {
					scenario.numCheckedExplainInRollback++
				}
				return a
			}()
			...
			if action.numInjectedFailures > 0 {
				...
				if action.numCheckedExplainInRollback > 0 {
					return nil
				}
...
func rollbackCase(t *testing.T, args rollbackPrepData, cs CumulativeTestCaseSpec) {
	...
	func() {
		...
		args.scenario.numInjectedFailures = 0
		args.scenario.numCheckedExplainInRollback = 0
	}()
	defer func() {
		args.scenario.Lock()
		defer args.scenario.Unlock()
	...
	// Collect results.
	var numInjectedFailures, numCheckedExplainInRollback uint32
	func() {
		args.scenario.Lock()
		defer args.scenario.Unlock()
		numInjectedFailures = args.scenario.numInjectedFailures
		numCheckedExplainInRollback = args.scenario.numCheckedExplainInRollback
	}()

	if numInjectedFailures == 0 {
		require.NoError(t, err)
	} else {
		require.Regexp(t, fmt.Sprintf("boom %d", cs.StageOrdinal), err)
		require.NotZero(t, numCheckedExplainInRollback)
...
func pausePrepare(
	...
	knobs := &scexec.TestingKnobs{
		BeforeStage: func(p scplan.Plan, stageIdx int) error {
			...
			if scenario.numInjectedFailures > 0 {
				return nil
			}
...
func pauseCase(t *testing.T, args pausePrepData, cs CumulativeTestCaseSpec) {
	...
	var numInjected uint32
	func() {
		args.scenario.Lock()
		defer args.scenario.Unlock()
		numInjected = args.scenario.numInjectedFailures
	}()
	require.Equal(t, uint32(1), numInjected)
}
