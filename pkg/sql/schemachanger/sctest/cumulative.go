...
func Backup(t *testing.T, dir string, newCluster NewClusterFunc) {
	...
	testBackupRestoreCase = func(
		...
		for i := 0; !done; i++ {
			...
			s := <-stageChan
			t.Logf("stage %d/%d in %v (rollback=%v)",
				s.stageIdx+1, len(s.p.Stages), s.p.Stages[s.stageIdx].Phase, s.p.InRollback)
			shouldFail := ord == i &&
			...
			ids := screl.GetDescIDs(s.p.TargetState)
			var exists bool
			if dbName, exists = maybeGetDatabaseForIDs(t, tdb, ids); !exists {
				close(s.resume)
				continue
			}
		...
		for i, b := range backups {
			...
			restoreDB := fmt.Sprintf(`restored%d`, i)
			tdb.Exec(t, "SET experimental_use_new_schema_changer = 'off'")
			tdb.Exec(t, fmt.Sprintf(
				"RESTORE DATABASE %s FROM LATEST IN '%s' WITH new_db_name = '%s'",
				dbName, b.url, restoreDB))
			waitForSchemaChangesToFinish(t, tdb)
			tdb.Exec(t, fmt.Sprintf("USE %q", restoreDB))
			strs := tdb.QueryStr(t, `
SELECT crdb_internal.pb_to_json('desc', descriptor)
  FROM system.descriptor
 WHERE id
       IN (
            SELECT id
              FROM system.namespace
             WHERE "parentID" = (SELECT id FROM system.namespace WHERE name = $1)
        )`, dbName)
			afterRestore := tdb.QueryStr(t, fetchDescriptorStateQuery)
			t.Log(sqlutils.MatrixToStr(strs))
			if b.isRollback {
				require.Equal(t, before, afterRestore)
			} else {
				require.Equal(t, after, afterRestore)
			}
			tdb.Exec(t, fmt.Sprintf("DROP DATABASE %q CASCADE", restoreDB))
			tdb.Exec(t, fmt.Sprintf("USE %q", dbName))
		}
	}
	cumulativeTest(t, dir, testFunc)
 ...
 WHERE id
       IN (
            SELECT DISTINCT "parentID"
              FROM system.namespace
             WHERE id IN (SELECT * FROM ROWS FROM (unnest($1::INT[])))
	...
	// hanging when a regression is introduced.
	{
		...
		go func() {
			...
			c <- crdb.Execute(func() (err error) {
				...
				defer func() {
					if err != nil {
						errors.WithSecondaryError(err, tx.Rollback())
					} else {
						err = tx.Commit()
					}
				}()
