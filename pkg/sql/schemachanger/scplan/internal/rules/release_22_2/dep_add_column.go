	...
	registerDepRule(
		...
		func(from, to common.NodeVars) rel.Clauses {
			return rel.Clauses{
				...
				to.Type((*scpb.Column)(nil)),
				common.JoinOnColumnID(from, to, "table-id", "col-id"),
				common.StatusesToPublicOrTransient(from, scpb.Status_PUBLIC, to, scpb.Status_WRITE_ONLY),
			}
		},
	)
}
	...
	registerDepRule(
		...
		func(from, to common.NodeVars) rel.Clauses {
			...
			return rel.Clauses{
				...
				common.ToPublicOrTransient(from, to),
				status.In(scpb.Status_WRITE_ONLY, scpb.Status_PUBLIC),
				status.Entities(screl.CurrentStatus, from.Node, to.Node),
				common.FilterElements("SmallerColumnIDFirst", from, to, func(from, to *scpb.Column) bool {
