...
func TestRuleAssertions(t *testing.T) {
	for _, fn := range []func(e scpb.Element) error{
		...
		t.Run(shortName, func(t *testing.T) {
			_ = common.ForEachElementInActiveVersion(func(e scpb.Element) error {
				e = nonNilElement(e)
				if err := fn(e); err != nil {
					t.Errorf("%T: %+v", e, err)
				}
...
// Assert that common.IsColumnDependent covers all dependent elements of a column
// element.
func checkIsColumnDependent(e scpb.Element) error {
	...
	if common.IsColumnDependent(e) {
		if err != nil {
			return errors.New("verifies common.IsColumnDependent but doesn't have ColumnID attr")
		}
	} else if err == nil {
		return errors.New("has ColumnID attr but doesn't verify common.IsColumnDependent")
	}
	return nil
}

// Assert that common.IsIndexDependent covers all dependent elements of an index
// element.
func checkIsIndexDependent(e scpb.Element) error {
	// Exclude indexes themselves.
	if common.IsIndex(e) || common.IsSupportedNonIndexBackedConstraint(e) {
		return nil
	}
	...
	if common.IsIndexDependent(e) {
		if err != nil {
			return errors.New("verifies common.IsIndexDependent but doesn't have IndexID attr")
		}
	} else if err == nil {
		return errors.New("has IndexID attr but doesn't verify common.IsIndexDependent")
	}
	return nil
}
...
func checkIsConstraintDependent(e scpb.Element) error {
	...
	if common.IsConstraintDependent(e) {
		if err != nil {
			return errors.New("verifies common.IsConstraintDependent but doesn't have ConstraintID attr")
		}
	} else if err == nil {
		return errors.New("has ConstraintID attr but doesn't verify common.IsConstraintDependent")
	}
	return nil
}
