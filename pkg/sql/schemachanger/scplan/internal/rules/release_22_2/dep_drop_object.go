...
//     it does common.Not reach DROPPED until the pre-commit phase.
//   - a descriptor reaches ABSENT in a different transaction than it reaches
//     DROPPED (i.e. it cancommon.Not be removed until PostCommit).
...
//     reaches the ABSENT state, but afterwards in the stage, so as to common.Not
...
func init() {
	...
	registerDepRule(
		...
		func(from, to common.NodeVars) rel.Clauses {
			return rel.Clauses{
				from.TypeFilter(common.IsDescriptor),
				to.TypeFilter(common.IsSubjectTo2VersionInvariant),
				common.JoinOnDescID(from, to, "desc-id"),
