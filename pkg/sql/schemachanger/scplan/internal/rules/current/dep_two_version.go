...
func init() {
	...
	clausesForTwoVersionEdge := func(
		...
		if len(prePrevStatuses) > 0 {
			clauses = append(clauses,
				common.GetNotJoinOnNodeWithStatusIn(prePrevStatuses)(from.Target),
			)
		}
		return clauses
	}
	...
	_ = common.ForEachElement(func(el scpb.Element) error {
		if !common.IsSubjectTo2VersionInvariant(el) {
			return nil
		}
