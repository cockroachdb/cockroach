...
func init() {
	...
	// Sequences are tacked separately from other relations, so we need
	// another rule for them.
	registerDepRuleForDrop("trigger references cleaned before referenced sequences",
		scgraph.SameStagePrecedence,
		"trigger dependency", "sequence",
		scpb.Status_ABSENT, scpb.Status_DROPPED,
		func(from, to NodeVars) rel.Clauses {
			return rel.Clauses{
				from.Type((*scpb.TriggerDeps)(nil)),
				...
				to.El.AttrEqVar(screl.DescID, "desc-id"),
				from.El.AttrContainsVar(screl.ReferencedSequenceIDs, "desc-id"),
			}
		})
}
