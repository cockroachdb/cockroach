	...
	registerDepRule(
		...
		func(from, to NodeVars) rel.Clauses {
			return rel.Clauses{
				...
				to.TypeFilter(rulesVersionKey, isNonIndexBackedComplexConstraint, Not(isNonIndexBackedCrossDescriptorConstraint)),
				JoinOnDescID(from, to, "desc-id"),
				StatusesToAbsent(from, scpb.Status_DROPPED, to, scpb.Status_VALIDATED),
			}
		},
...
func init() {
	registerDepRule(
		...
		func(from, to NodeVars) rel.Clauses {
			return rel.Clauses{
				from.TypeFilter(rulesVersionKey, isNonIndexBackedComplexConstraint, isNonIndexBackedCrossDescriptorConstraint),
				to.TypeFilter(rulesVersionKey, isDescriptor),
				JoinReferencedDescID(from, to, "desc-id"),
				StatusesToAbsent(from, scpb.Status_ABSENT, to, scpb.Status_DROPPED),
			}
