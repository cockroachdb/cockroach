...
func init() {
	...
	// new enough version.
	registerDepRule(
		"secondary index being recreated after a primary key swap",
		scgraph.Precedence,
		"old-index", "new-index",
		func(from, to NodeVars) rel.Clauses {
			// Detect a potential secondary index recreation because of a ALTER
			// PRIMARY KEY, and require that the old index should be validated,
			// before the new index can be made public (i.e. they are swapped
			// an atomic manner).
	...
	registerDepRule(
		...
		func(from, to NodeVars) rel.Clauses {
			// Detect a potential secondary index recreation because of a ALTER
			// PRIMARY KEY, and require that the new index should be validated,
