...
// JoinDeleteOnlyColToPublicDep is a helper that associates a delete-only column
// ('from') with its public column-dependent ('to').
func JoinDeleteOnlyColToPublicDep(from, to NodeVars, tableIDVar, columnIDVar rel.Var) rel.Clauses {
	return rel.Clauses{
		JoinOnColumnID(from, to, tableIDVar, columnIDVar),
