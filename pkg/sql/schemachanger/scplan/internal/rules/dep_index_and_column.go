	...
	registerDepRule(
		...
		func(from, to nodeVars) rel.Clauses {
			...
			return rel.Clauses{
				...
				joinOnColumnID(ic, to, relationID, columnID),
				depEdgeToPublic(from, scpb.Status_PUBLIC, to, scpb.Status_PUBLIC),
			}
		},
	)
	...
	registerDepRule(
		...
		func(from, to nodeVars) rel.Clauses {
			...
			return rel.Clauses{
				...
				columnInPrimaryIndexSwap(ic, from, relationID, columnID, "index-id"),
				joinOnColumnID(ic, to, relationID, columnID),
				depEdgeToAbsent(from, scpb.Status_WRITE_ONLY, to, scpb.Status_WRITE_ONLY),
			}
		})
	registerDepRule(
		...
		func(from, to nodeVars) rel.Clauses {
			...
			return rel.Clauses{
				...
				depEdgeToPublic(from, scpb.Status_DELETE_ONLY, to, scpb.Status_BACKFILL_ONLY),
			}
		},
	)
	...
	registerDepRule("index-column added to index after index exists",
		...
		func(from, to nodeVars) rel.Clauses {
			return rel.Clauses{
				...
				to.Type((*scpb.IndexColumn)(nil)),
				joinOnIndexID(from, to, "table-id", "index-id"),
				depEdgeToPublic(from, scpb.Status_BACKFILL_ONLY, to, scpb.Status_PUBLIC),
			}
		})
	...
	registerDepRule("index-column added to index before index is backfilled",
		...
		func(from, to nodeVars) rel.Clauses {
			return rel.Clauses{
				...
				to.Type((*scpb.PrimaryIndex)(nil), (*scpb.SecondaryIndex)(nil)),
				joinOnIndexID(from, to, "table-id", "index-id"),
				depEdgeToPublic(from, scpb.Status_PUBLIC, to, scpb.Status_BACKFILLED),
			}
		})
