...
// change.
//
// Stages also contain their corresponding targets and statuses before and after
// the execution of the ops in the stage, reflecting the fact that any set of
// ops can be thought of as a transition from a set of target statuses to another.
type Stage struct {
...
func MakePlan(initialState []*scpb.Node, params Params) (_ Plan, err error) {
	defer func() {
		if r := recover(); r != nil {
			rAsErr, ok := r.(error)
			if !ok {
	...
	stages := buildStages(initialState, g, params)
	return Plan{
		Params:       params,
