...
func collapseStages(stages []Stage) (collapsedStages []Stage) {
	...
	for _, s := range stages[1:] {
		if isCollapsible(acc, s) {
			acc.After = s.After
			acc.Revertible = acc.Revertible && s.Revertible
			if s.Ops != nil {
				acc.Ops = s.Ops
			} else if acc.Ops == nil {
				panic("missing ops")
			...
			collapsedStages = append(collapsedStages, acc)
			acc = s
		}
	}
	collapsedStages = append(collapsedStages, acc)
...
// isCollapsible returns true iff s can be collapsed into acc.
