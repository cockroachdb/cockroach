...
var (
	...
	opsArrayType         = reflect.TypeOf(([]scop.Op)(nil)).Elem()
...
func buildSchemaChangeOpGenFunc(e scpb.Element, forward, backwards targetOpRules) opGenFunc {
	...
	opArrayType := reflect.FuncOf(
		[]reflect.Type{tTyp},
		[]reflect.Type{opsArrayType},
		false, /* variadic */
	)
	for s, rules := range forward {
		for i, rule := range rules {
			...
			if ot := reflect.TypeOf(rule.op); ot != opType || ot != opArrayType {
				panic(errors.Errorf("invalid ops with signature %v != (%v || %v) %p %p for (%T, %s)[%d]", ot, opType, opArrayType, ot, opsType, e, s, i))
			}
		}
	}
		...
		for {
			...
			for _, rule := range rules {
				...
				if op, ok := out[0].Interface().(scop.Op); ok {
					builder.AddOpEdge(t, cur, rule.nextState, op)
				} else if opArray, ok := out[0].Interface().([]scop.Op); ok {
					builder.AddOpEdgeSlice(t, cur, rule.nextState, opArray)
				}
