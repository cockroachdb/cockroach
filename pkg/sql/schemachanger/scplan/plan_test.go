...
func marshalDeps(t *testing.T, plan *scplan.Plan) string {
	...
	plan.Graph.ForEachNode(func(n *scpb.Node) error {
		return plan.Graph.ForEachDepEdgeFrom(n, func(de *scgraph.DepEdge) error {
			...
			stages += fmt.Sprintf("%s%s:%s->%s%s:%s\n",
				scpb.ElementIDToString(scpb.ElementType(de.To().Element())), toAttr, de.To().State,
				scpb.ElementIDToString(scpb.ElementType(de.From().Element())), fromAttr, de.From().State)
			return nil
		})
	})
	return stages
