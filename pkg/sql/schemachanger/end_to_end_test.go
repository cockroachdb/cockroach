...
func execNewSchemaChangerInMemory(
	...
	{ // Run new schema changer with in-memory testing dependencies.
		...
		// Run statement and pre-commit phases.
		for _, phase := range []scop.Phase{scop.StatementPhase, scop.PreCommitPhase} {
			deps.WithTxn(phase, func(s *sctestdeps.TestState) {
				state, err = scrun.RunSchemaChangesInTxn(ctx, s, state)
				require.NoError(t, err, "error in %s", phase)
			})
		}

		// Mock creating the schema change job.
		jobID, err := scrun.CreateSchemaChangeJob(ctx, deps, state)
	...
	// any side effects outside of the test state. We therefore need to execute
...
func marshalCatalog(ctx context.Context, t *testing.T, s *sctestdeps.TestState) string {
	...
	for _, desc := range nonDropped {
		...
		// Marshall to YAML in a roundabout way.
		var buf bytes.Buffer
		require.NoError(t, (&jsonpb.Marshaler{EmitDefaults: false}).Marshal(&buf, desc.DescriptorProto()))
