...
func TestSchemaChangerSideEffects(t *testing.T) {
	...
	datadriven.Walk(t, filepath.Join("testdata"), func(t *testing.T, path string) {
		...
		// Disable the declarative schema changer in the test cluster.
		// This isn't strictly-speaking necessary, although doing so might help
		...
		datadriven.RunTest(t, path, func(t *testing.T, d *datadriven.TestData) string {
			...
			// Always keep test cluster in sync.
			defer func() {
			...
			case "test":
				...
				// The test dependencies do not hold any reference to the test cluster,
				// the test cluster is only used by the constructor to populate the
				// catalog state. Executing the statement therefore does not modify
				// the test cluster in any way.
