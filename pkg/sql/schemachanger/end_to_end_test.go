		...
		datadriven.RunTest(t, path, func(t *testing.T, d *datadriven.TestData) string {
			...
			case "test":
				...
				t.Run(strings.Join(lines, "; "), func(t *testing.T) {
					for i, n := 0, countPostCommitStages(ctx, t, setup, stmts); i < n; i++ {
						if !t.Run(fmt.Sprintf("rollback stage %d of %d", i+1, n), func(t *testing.T) {
...
// countPostCommitStages runs the test statements to count the number of stages
// in the post-commit phase.
func countPostCommitStages(
	...
	beforeStage := func(p scplan.Plan, stageIdx int) error {
		if p.Params.ExecutionPhase == scop.PostCommitPhase {
			atomic.StoreUint32(&n, uint32(len(p.Stages)))
			return nil
		}
		return nil
	}
...
func execRolledBackStatements(
	...
	setup []parser.Statement,
	stmts []parser.Statement,
	beforeStageCallback func(p scplan.Plan, stageIdx int) error,
		...
		ServerArgs: base.TestServerArgs{
			Knobs: base.TestingKnobs{
				SQLDeclarativeSchemaChanger: &scrun.TestingKnobs{
					BeforeStage: beforeStageCallback,
				},
			},
		},
	})
