...
//    be an equality or inequality from an entity-attribute to a constant (1)
//    or to a referenced entity-attribute (2) or a comparison between
//    entities along all attributes (3).
//
//    Imagine we have entities e1 and e2 and attributes a1 and a2 where a1 is
//    a string and a2 is a number. The following would be a valid query:
//
//     - e1[a1] = e2[a2]     // (1)
//     - e2[a1] = foo        // (2)
//     - e2[a2] = 1          // (2)
...
func (dd *DataDriven) iterateCmd(t *testing.T, d *datadriven.TestData) string {
	...
	var got []string
	_ = db.Iterate(where, eav.IteratorFunc(func(e eav.Entity) error {
		got = append(got, dd.entities[e.(*entity)])
		return nil
	}))
	sort.Strings(got)
	return fmt.Sprintf("%s", strings.Join(got, ","))
}
...
func (dd *DataDriven) evalCmd(t *testing.T, d *datadriven.TestData) string {
	...
	var got []string
	_ = q.Evaluate(db, func(r eavquery.Result) error {
		var row []string
