...
type SchemaChangeJobExecutionDependencies interface {
	// WithTxn is a wrapper for opening and committing a transaction around the
	// execution of the callback. After committing the transaction, the job
	// registry should be notified to adopt jobs.
	WithTxn(ctx context.Context, fn func(ctx context.Context, txndeps SchemaChangeJobTxnDependencies) error) error
