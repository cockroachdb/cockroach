...
func RunSchemaChangesInJob(
	...
	if len(sc.Stages) == 0 {
		...
		return deps.WithTxn(ctx, func(ctx context.Context, td SchemaChangeJobTxnDependencies) error {
	...
	for i, stage := range sc.Stages {
		...
		if err := deps.WithTxn(ctx, func(ctx context.Context, td SchemaChangeJobTxnDependencies) error {
