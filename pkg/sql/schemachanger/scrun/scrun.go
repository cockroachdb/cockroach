...
func executeStage(
	...
	defer scerrors.HandleErrorOrPanic(ctx, &err,
		"executing declarative schema change %s (rollback=%v)",
		redact.Safe(stage), redact.Safe(p.InRollback),
	)()
...
func makeState(
	...
	defer scerrors.HandleErrorOrPanic(ctx, &err,
		"rebuilding declarative schema change state from descriptors %v", descriptorIDs,
	)()
