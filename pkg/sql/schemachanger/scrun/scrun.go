...
func RunSchemaChangesInJob(
	...
	state, err := makeState(ctx, settings, deps, jobDetails.Descriptors, rollback)
	if err != nil {
...
func makeState(
	...
	if err := deps.WithTxnInJob(ctx, func(ctx context.Context, txnDeps scexec.Dependencies) error {
		...
		for _, id := range descriptors {
			...
			cs := desc.GetDeclarativeSchemaChangeState()
			...
			for rank, stmt := range cs.RelevantStatements {
				if existing, ok := stmts[rank]; ok {
					if existing.Statement != stmt.Statement {
						return errors.AssertionFailedf(
							"job %d: statement %q does not match %q for rank %d",
							desc.GetDeclarativeSchemaChangeJobID(),
							existing.Statement,
							stmt.Statement,
							rank,
						)
