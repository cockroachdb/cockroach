...
func convertToJobMergeProgress(
	...
	for _, mp := range progresses {
		ret = append(ret, jobspb.MergeProgress{
			TableID:        mp.TableID,
			SourceIndexIDs: mp.SourceIndexIDs,
			DestIndexIDs:   mp.DestIndexIDs,
			CompletedSpans: addTenantPrefixToSpans(codec, mp.CompletedSpans),
		})
	}
	return ret, nil
}
...
func convertFromJobMergeProgress(
	...
	for _, mp := range progresses {
		ret = append(ret, scexec.MergeProgress{
			Merge: scexec.Merge{
				TableID:        mp.TableID,
				SourceIndexIDs: mp.SourceIndexIDs,
				DestIndexIDs:   mp.DestIndexIDs,
...
func newMergeProgress(codec keys.SQLCodec, mp scexec.MergeProgress) *mergeProgress {
	minIndexID := mp.SourceIndexIDs[0]
	maxIndexID := minIndexID
	for _, id := range mp.SourceIndexIDs[1:] {
		if id < minIndexID {
			minIndexID = id
		}
		if id > maxIndexID {
	...
	minIndexPrefix := codec.IndexPrefix(uint32(mp.TableID), uint32(minIndexID))
	maxIndexPrefix := codec.IndexPrefix(uint32(mp.TableID), uint32(maxIndexID))
	indexSpan := roachpb.Span{
		Key:    minIndexPrefix,
		EndKey: maxIndexPrefix.PrefixEnd(),
	}
...
type mergeProgress struct {
	...
	// totalSpan represents the tightest span that includes all the spans of the
	// temporary indexes being merged from.
	// This is used to give a (very poor) approximation of progress.
	totalSpan roachpb.Span
}
...
func indexIDSet(ids ...descpb.IndexID) (ret util.FastIntSet) {
	for _, id := range ids {
		ret.Add(int(id))
	}
	return ret
