...
func (s *TestState) logSideEffectf(fmtstr string, args ...interface{}) {
	s.sideEffectLog = append(s.sideEffectLog, fmt.Sprintf(fmtstr, args...))
}
...
// SideEffectLog returns a text representation of all side effects that have
// been logged so far.
func (s *TestState) SideEffectLog() string {
	return strings.Join(s.sideEffectLog, "\n")
}
