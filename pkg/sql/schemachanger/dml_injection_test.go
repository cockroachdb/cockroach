...
func (po phaseOrdinal) backfillDeleteRow(operationPO phaseOrdinal, isAfter bool) []string {
	return []string{
		po.String(),
		operationPO.String(),
...
func (po phaseOrdinal) backfillUpdateRow(operationPO phaseOrdinal, isAfter bool) []string {
	return []string{
		po.String(),
		operationPO.String(),
...
func TestAlterTableDMLInjection(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.desc, tc.capture(func(t *testing.T, tc testCase) {
			...
			// Seperate rows are updated and modified within the back fill.
			var currentPOForBackFill atomic.Value
			currentPOForBackFill.Store(phaseOrdinal{})
			var backfillBeforeDone, backfillAfterDone atomic.Bool
					...
					Knobs: base.TestingKnobs{
						...
						SQLDeclarativeSchemaChanger: &scexec.TestingKnobs{
							BeforeStage: func(p scplan.Plan, stageIdx int) error {
								...
								backfillAfterDone.Store(false)
								backfillBeforeDone.Store(false)
			...
			backfillCallback = func(isAfter bool) {
				...
				backfillDone := false
				if isAfter {
					backfillDone = backfillAfterDone.Swap(true)
				} else {
					backfillDone = backfillBeforeDone.Swap(true)
				}
				if !backfillDone {
					for j := poIdx; j < len(poMap); j++ {
						sqlDB.ExecWithMessage(t, errorMessage, insert, toAnySlice(currentPO.backfillUpdateRow(poSlice[j], isAfter))...)
						sqlDB.ExecWithMessage(t, errorMessage, insert, toAnySlice(currentPO.backfillDeleteRow(poSlice[j], isAfter))...)
					}
				}
