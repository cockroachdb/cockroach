...
func (es *entitySet) internString(s string) uintptr {
	id, exists := es.strings[s]
	if !exists {
		id = len(es.stringIntern)
		es.stringIntern = append(es.stringIntern, s)
		es.strings[s] = id
	}
	return uintptr(id)
}
...
func (es *entitySet) makeInlineValue(attr ordinal, vv reflect.Value) (uintptr, error) {
	...
	*/
	switch {
	case attr == es.schema.typeOrdinal:
		...
		ti, ok := es.schema.entityTypeSchemas[typVal]
		if !ok {
			return 0, errors.Errorf(
	...
	case typ.Kind() == reflect.String:
		return es.internString(vv.String()), nil
	default:
		return 0, errors.Errorf(
