...
func (idx indexSpec) matchesPredicate(v *values) bool {
	if idx.where.attrs.empty() {
		return true
	}
	matches := true
	idx.where.attrs.forEach(func(a ordinal) (wantMore bool) {
		av, aOk := idx.where.get(a)
		bv, bOk := v.get(a)
