...
var SkipBackfillSplitsForSmallTables = settings.RegisterBoolSetting(
	...
	"when enabled, index backfill operations skip creating range splits "+
		...
		"avoiding range count bloat on clusters running many schema changes "+
		"on small tables",
	true,
...
func (is *indexSplitAndScatter) MaybeSplitIndexSpansForPartitioning(
	...
	if estimated := is.estimatedTableSize(ctx, tableDesc); estimated < is.rangeMaxBytesForTable(tableDesc) {
		log.Dev.Infof(ctx,
			"skipping hash shard pre-split for tableId=%d index=%d: estimated table size %d bytes fits in one range",
			tableDesc.GetID(), idx.GetID(), estimated)
		return nil
	}
	const backfillSplitExpiration = time.Hour
