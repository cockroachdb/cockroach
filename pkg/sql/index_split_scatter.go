...
func (is *indexSplitAndScatter) MaybeSplitIndexSpans(
	...
	for _, idx := range table.ActiveIndexes() {
		if idx.GetID() != indexToBackfill.GetID() &&
			...
			log.Infof(ctx, "making %d split points in index=%d sampled from (table=%d index=%d)",
				nSplits, indexToBackfill.GetID(), tableID, copySplitsFromIndexID)
			break
		}
	}
	...
	if len(splitPoints) == 0 {
		...
		return splitAndScatter(ctx, is.db, splitKey, expirationTime)
	}
	...
	for i := 0; i < nSplits; i++ {
		...
		log.Infof(ctx, "MaybeSplitIndexSpans sending split request for key %s", sp)
