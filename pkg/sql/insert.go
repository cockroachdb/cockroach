...
func (p *planner) Insert(
	...
	insertCols, computedCols, computeExprs, err :=
		ProcessComputedColumns(ctx, insertCols, tn, desc, &p.txCtx, p.EvalContext())
	if err != nil {
		return nil, err
	}
		...
		if values != nil {
			if len(values.Tuples) > 0 {
				...
				if numExprs > maxInsertIdx {
					return nil, cannotWriteToComputedColError(insertCols[maxInsertIdx])
				}
				arityChecked = true
			}
...
func GenerateInsertRow(
	...
	rowVals tree.Datums,
	rowContainerForComputedVals *RowIndexedVarContainer,
) (tree.Datums, error) {
