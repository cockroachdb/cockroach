...
func (p *planner) Insert(
	...
	cols, defaultExprs, err :=
		sqlbase.ProcessDefaultColumns(cols, en.tableDesc, &p.txCtx, &p.evalCtx.EvalContext)
	if err != nil {
		return nil, err
	}
...
func (n *insertNode) internalNext(params runParams) (bool, error) {
	...
	if err := n.checkHelper.check(&params.evalCtx.EvalContext); err != nil {
		return false, err
	}
