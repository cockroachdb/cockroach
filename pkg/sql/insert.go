...
func (r *insertRun) processSourceRow(params runParams, rowVals tree.Datums) error {
	...
	// Keep track of the vector index partitions to update, as well as their
	// centroids. This information is passed to tableInserter.row below.
	...
	if n := len(r.ti.tableDesc().VectorIndexes()); n > 0 {
		...
		putCentroids := vectorIndexValStart[n : n*2]
		vh.InitForPut(putPartitionKeys, putCentroids, r.ti.tableDesc())
	}
