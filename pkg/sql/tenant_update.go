...
func (p *planner) setTenantService(
	...
	for {
		lastMode, err := stepTenantServiceState(ctx,
...
func stepTenantServiceState(
	...
	if mode == mtinfopb.ServiceModeStopping {
		...
		defer cancel()
		for re := retry.StartWithCtx(waitCtx, retryOpts); re.Next(); {
			resp, err := statusSrv.TenantServiceStatus(waitCtx, &serverpb.TenantServiceStatusRequest{TenantID: inInfo.ID})
			if err != nil {
				return mode, errors.Wrap(err, "failed while waiting for tenant service status")
			}
			if len(resp.ErrorsByNodeID) > 0 {
				for _, errStr := range resp.ErrorsByNodeID {
					return mode, errors.Newf("failed to poll service status on all nodes (first error): %s", errStr)
				}
			}
			allStopped := true
