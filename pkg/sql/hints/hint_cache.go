...
func (c *StatementHintsCache) onUpdate(
	...
	if update.Type == rangefeedcache.CompleteUpdate || len(update.Events) == 0 {
		// A complete update or an empty incremental update indicate that no more
		// events will be received below the update's timestamp.
		defer c.frontier.Store(update.Timestamp.WallTime)
	}
