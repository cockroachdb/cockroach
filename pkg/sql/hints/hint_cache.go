...
type StatementHintsCache struct {
	mu struct {
		...
		// hintedHashes maps from every 64-bit fingerprint hash in the system table
		// to the number of associated hints. This map allows StatementHintsCache to
		// quickly determine whether there are any hints for a given statement,
		// minimizing the overhead for non-hinted statements.
...
// TestingLookupHash returns the number of hints for a given hash. Returns 0 if
// the hash is not present in hintedHashes.
func (c *StatementHintsCache) TestingLookupHash(hash int64) int {
	c.mu.Lock()
	defer c.mu.Unlock()
	if c.mu.hintedHashes == nil {
		return 0
	}
	return c.mu.hintedHashes[hash]
}
