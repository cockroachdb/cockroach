...
func TestHintCacheBasic(t *testing.T) {
	...
	// Decrease the closed timestamp interval to speed up refreshes.
...
func TestHintCacheLRU(t *testing.T) {
	...
	initialReads := hc.TestingNumTableReads()
	...
	require.Equal(t, initialReads+1, hc.TestingNumTableReads())

	require.NotNil(t, hc.MaybeGetStatementHints(ctx, fingerprints[1]))
	require.Equal(t, initialReads+2, hc.TestingNumTableReads())
	...
	require.NotNil(t, hc.MaybeGetStatementHints(ctx, fingerprints[2]))
	require.NotNil(t, hc.MaybeGetStatementHints(ctx, fingerprints[2]))
	require.Equal(t, initialReads+6, hc.TestingNumTableReads())
}
