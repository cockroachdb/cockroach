...
// filter on them. Return the rowID, fingerprint and hint protobuf of all the
// rows that got deleted.
func DeleteHintFromDB(
	ctx context.Context, txn isql.Txn, rowID int64, fingerprint string,
) (rowIDs []int64, fingerprints []string, hints []hintpb.StatementHintUnion, err error) {
	...
	for _, row := range rows {
		...
		hintBytes, ok := row[2].(*tree.DBytes)
		if !ok {
			return nil, nil, nil, errors.AssertionFailedf("expected hint to be bytes, got %T", row[0])
		}
		// Deserialize from the protobuf.
		hint, err := hintpb.FromBytes([]byte(*hintBytes))
		if err != nil {
			return nil, nil, nil, errors.Wrap(err, "deserializing statement hint")
		}
		hints = append(hints, hint)
	}
	return rowIDs, fingerprints, hints, nil
}
