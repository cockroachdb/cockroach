...
// Cancel a transaction with a query running `show trace for ...` with a long-running statement, 2 seconds into execution.
func TestCancelSelectTransaction(t *testing.T) {
	defer leaktest.AfterTest(t)()

	const insertQuery = `INSERT INTO nums VALUES (1);`
	const selectQuery = `SHOW TRACE FOR SELECT * FROM generate_series(1,20000000);`
