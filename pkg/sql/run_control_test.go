...
func TestCancelDistSQLQuery(t *testing.T) {
	...
	if _, err := conn1.Exec(fmt.Sprintf("ALTER TABLE nums SPLIT AT VALUES (50)")); err != nil {
		t.Fatal(err)
	}
	...
	// Note the err != nil check. It exists because a successful
	// cancellation does not imply that the query was canceled.
