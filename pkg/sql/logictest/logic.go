...
func (t *logicTest) newCluster(
	...
	if cfg.UseTenant || testCluster.StartedDefaultTestTenant() {
		...
		var capabilityNames []tenantcapabilitiespb.FlagCapabilityName
		for capabilityName, _ := range capabilities {
			capabilityNames = append(capabilityNames, tenantcapabilitiespb.FlagCapabilityName(capabilityName))
		}
		testCluster.WaitForTenantCapabilities(tenantID, capabilityNames...)
	}
