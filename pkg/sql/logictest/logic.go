...
var (
	...
	// Output parameters
	showSQL = flag.Bool("show-sql", true,
		...
		if userSession, ok := userToHexSession[user]; ok {
			...
			for key, value := range vars {
				...
				if _, err := t.db.Exec(fmt.Sprintf("SET %s='%s'", key, value)); err != nil {
					...
					log.Infof(context.Background(), "this is the first error %# v", pretty.Formatter(err))
					if _, err := t.db.Exec(fmt.Sprintf("SET %s=%s", key, value)); err != nil {
						...
						log.Infof(context.Background(), "this is the second error %# v", pretty.Formatter(err))
						continue
					}
				}
			}
