...
// not initialize the cluster with a test tenant. This is becasue the caller is
...
func (t *logicTest) maybeBackupRestore(
	...
	// Check if the restore has created an active tenant record, if it has, we
	// need to create SQL pods to connect to it.
	cfg := t.cfg
	row := t.db.QueryRow(`SELECT count(*) FROM system.tenants WHERE active=true`)
	var count int
