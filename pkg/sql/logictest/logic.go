...
// server to force the usage of production values for some constants that might
// change via the metamorphic testing.
//
// There is a special directive '!metamorphic' which skips the test if
// metamorphic settings are enabled.
//
//
// ###########################################################
...
func RunLogicTest(
	...
	_, onlyNonMetamorphic, nonMetamorphicBatchSizes :=
		logictestbase.ReadTestFileConfigs(t, path, logictestbase.ConfigSet{configIdx})
	if onlyNonMetamorphic {
		skip.UnderMetamorphic(t, "This test doesn't work in metamorphic mode.")
	}
