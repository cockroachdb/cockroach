...
type logicTest struct {
	...
	knobOpts []knobOpt
	// tenantOverrideArgs are the options used by the host cluster
	// to configure tenant overrides during setup. They're persisted here
	// because a cluster can be recreated throughout the lifetime of a test, and
	// we should use tenant overrides each time this happens.
	tenantOverrideArgs tenantOverrideArgs
...
func (t *logicTest) resetCluster() {
	...
	t.newCluster(serverArgs, t.clusterOpts, t.knobOpts, t.tenantOverrideArgs)
}
...
func readTenantOverrideArgs(t *testing.T, path string) tenantOverrideArgs {
	...
	parseDirectiveOptions(t, path, "tenant-cluster-setting-override-opt", func(opt string) {
		...
		name := parts[0]
		value := parts[1]
		existingValue, ok := res.clusterSettings[name]
		if ok {
			t.Fatalf(
				"cannot set %q to value %q because setting has existing value %q",
				name, value, existingValue,
			)
		}
		res.clusterSettings[name] = value
	})
	return res
}
