...
func readTenantClusterSettingOverrideArgs(
	...
	for s.Scan() {
		...
		// # tenant-cluster-setting-override-opt: opt1 opt2 ...
		if len(fields) > 1 && cmd == "#" && fields[1] == "tenant-cluster-setting-override-opt:" {
			require.True(
				t,
				beginningOfFile,
				"tenant-cluster-setting-override-opt directive needs to be at the beginning of file",
			)
			require.False(
				t,
				directiveFound,
				"only one cluster-opt directive allowed per file; second one found: %s",
				s.Text(),
			)
			directiveFound = true
			if len(fields) == 2 {
