...
func (t *logicTest) processTestFile(path string, config testClusterConfig) error {
	subtests, err := fetchSubtests(path, config)
	if err != nil {
		return err
	}
	...
	for _, subtest := range subtests {
		...
		// If subtest has no name, than it is not a subtest, so just run the lines
...
func fetchSubtests(path string, config testClusterConfig) ([]subtestDetails, error) {
	file, err := os.Open(path)
	if err != nil {
		return nil, err
	}
	...
	for s.Scan() {
		...
		if len(fields) > 0 && fields[0] == "subtest" {
			...
			subtests = append(subtests, subtestDetails{
				name:          curName,
				buffer:        buffer,
				indexIntoFile: curIndexIntoFile,
			})
			buffer = bytes.Buffer{}
			curName = fields[1]
			curIndexIntoFile = s.line + 1
		} else {
...
func (t *logicTest) processSubtest(subtest subtestDetails, path string, config testClusterConfig) error {
	defer t.traceStop()
	...
	for s.Scan() {
		if *maxErrs > 0 && t.failures >= *maxErrs {
			return errors.Errorf("%s:%d: too many errors encountered, skipping the rest of the input",
				path, s.line+subtest.indexIntoFile,
			)
		}
		line := s.Text()
		t.emit(line)
		...
		case "repeat":
			...
			if err != nil {
				return errors.Errorf("%s:%d invalid repeat line: %s", path, s.line+subtest.indexIntoFile, err)
			}
			repeat = count
		...
		case "sleep":
			...
			if err != nil {
				return errors.Errorf("%s:%d invalid sleep line: %s",
					path, s.line+subtest.indexIntoFile, err,
				)
			}
			time.Sleep(duration)

		case "statement":
			stmt := logicStatement{pos: fmt.Sprintf("\n%s:%d", path, s.line+subtest.indexIntoFile)}
		...
		default:
			return errors.Errorf("%s:%d: unknown command: %s",
				path, s.line+subtest.indexIntoFile, cmd,
			)
		}
	}
	return s.Err()
