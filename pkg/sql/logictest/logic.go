...
func (t *logicTest) newCluster(
	...
	if cfg.useTenant {
		...
		if clusterSettingOverrideArgs.overrideMultiTenantMultiRegionAbstractionsAllowed {
			...
			if _, err := conn.Exec(
				"ALTER TENANT $1 SET CLUSTER SETTING sql.multi_region.abstractions.enabled = true",
				serverutils.TestTenantID().ToUint64(),
			); err != nil {
				t.Fatal(err)
			}
	...
	if clusterSettingOverrideArgs.overrideMultiTenantMultiRegionAbstractionsAllowed {
		t.waitForTenantReadOnlyClusterSettingToTakeEffectOrFatal(
			"sql.multi_region.abstractions.enabled", "true", params.ServerArgs.Insecure,
		)
	}
...
type tenantClusterSettingOverrideArgs struct {
	...
	// If set, the sql.multi_region.abstractions.enabled default is set to true
	// by the host. This allows logic tests that run on secondary tenants to make
	// use of multi-region abstractions.
