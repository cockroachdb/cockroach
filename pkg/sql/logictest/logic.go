...
type logicTest struct {
	...
	// randomizedVectorizeBatchSize stores the randomized batch size for vectorize
	// engine if it not turned off. The batch size will randomly be set to
	// 1 with 25% probability, {2, 3} with 25% probability or default batch size
	// with 50% probability.
	randomizeVectorizeBatchSize int
}
...
func RunLogicTest(t *testing.T, globs ...string) {
	...
	randVal := rng.Float64()
	randomizedVectorizeBatchSize := coldata.BatchSize()
	if randVal < 0.25 {
		randomizedVectorizeBatchSize = 1
	} else if randVal < 0.375 {
		randomizedVectorizeBatchSize = 2
	} else if randVal < 0.5 {
		randomizedVectorizeBatchSize = 3
	}
			...
			for _, path := range paths {
				...
				t.Run(filepath.Base(path), func(t *testing.T) {
					...
					lt := logicTest{
						...
						rng:                         rng,
						randomizeVectorizeBatchSize: randomizedVectorizeBatchSize,
					}
