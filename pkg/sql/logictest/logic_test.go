...
func (t *logicTest) processTestFile(path string, config testClusterConfig) error {
	...
	for s.Scan() {
		...
		if len(fields) > 0 && fields[0] == "subtest" {
			if len(fields) != 2 {
				return errors.Errorf("%s:%d expected on one field following the subtest command", path, s.line)
			}
			subtestName = fields[1]
			// Set first to false, so we skip the subtest line.
			first = false
		}
				...
				case "subtest":
					if len(fields) != 2 {
						subtestError = errors.Errorf(
							"%s:%d expected on one field following the subtest command",
							path, s.line,
						)
						subtestT.Error(subtestError)
						return
