...
func updateTableStatus(
	...
	for i, t := range progress.Tables {
		...
		if lifetime < 0 {
			log.Infof(ctx, "marking table %d for GC", t.ID)
			droppedTable.Status = jobspb.SchemaChangeGCProgress_DELETING
		} else {
			if log.V(2) {
				log.Infof(ctx, "table %d not ready for GC. Still needs to wait %+v", table.ID, lifetime)
			}
		}
		break
	}
...
func updateIndexesStatus(
	...
	for i := 0; i < len(progress.Indexes); i++ {
		...
		if lifetime > 0 && log.V(2) {
			log.Infof(ctx, "index %d of table %d not ready for GC. Still needs to wait %+v", idxProgress.IndexID, table.ID, lifetime)
		}
		if lifetime < 0 {
			expired = true
			log.Infof(ctx, "marking index %d of table %d for immanent GC", idxProgress.IndexID, table.ID)
