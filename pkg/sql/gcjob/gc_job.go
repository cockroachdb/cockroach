	...
	for _, tableID := range details.UnsplitTables {
		...
		if err := execCfg.DB.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
			...
			if errors.Is(err, catalog.ErrDescriptorNotFound) {
				log.Warningf(ctx, "table descriptor %d not found while attempting to GC, skipping", tableID)
				continue
			}
			return errors.Wrapf(err, "failed to fetch table %d", tableID)
		}
...
func (r schemaChangeGCResumer) Resume(ctx context.Context, execCtx interface{}) (err error) {
	...
	if len(details.UnsplitTables) > 0 && !progress.TablesUnsplitDone {
		// Unsplit all manually split ranges in the table, so they can be
		// automatically merged by the merge queue.
		if err := unsplitRangesForTables(ctx, execCfg, details); err != nil {
			return err
		}
