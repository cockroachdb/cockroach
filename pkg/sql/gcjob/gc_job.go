...
// unsplitRangesInSpanForSecondaryTenant unsplits any manually splits ranges
// within a span using an implementation that is appropriate for a secondary
// tenant.
//
// When operating in a secondary tenant, unsplitting is not guaranteed becuase:
//
//   - The tenant may no longer be allowed to unsplit.
//   - We use the range cache to look up range start keys and our range cache
//     may be out of date.
