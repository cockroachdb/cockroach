...
func gcTables(
	...
	if log.V(2) {
		log.Infof(ctx, "performing GC on tables: %+v", progress.Tables)
	}
	for _, droppedTable := range progress.Tables {
		...
		if err := markTableGCed(ctx, table.ID, progress); err != nil {
			return err
		}
	}
	return nil
...
// TODO(pbardea): When the GC job is being used and the old schema change code
// can be removed, move ClearTableDataInChunks to this file.
func clearTableData(
