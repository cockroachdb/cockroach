...
func gcIndexes(
	...
	if log.V(2) {
		log.Infof(ctx, "performing GC on indexes from table %d, indexes: %+v", parentID, droppedIndexes)
	}
...
func clearIndex(
	...
	if index.IsInterleaved() {
		log.Errorf(ctx, "trying to clear interleaved index %d from table %d", index.ID, tableDesc.ID)
