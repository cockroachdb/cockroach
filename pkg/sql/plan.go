...
func (p *planner) makeOptimizerPlan(ctx context.Context, stmt Statement) error {
	...
	// If a prepared memo doesn't yet exist, or if it's been invalidated by schema
	// or other changes, then entirely rebuild the memo from the AST.
	if prepMemo == nil || inPreparePhase || prepMemo.IsStale(ctx, p.EvalContext(), &catalog) {
