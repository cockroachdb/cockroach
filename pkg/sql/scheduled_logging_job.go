...
type loggingResumer struct {
	...
	// TODO(thomas): are the params for this callback limiting
...
func (r *loggingResumer) Resume(ctx context.Context, execCtx interface{}) error {
	...
	// TODO(thomas): implement rate-limiting logic here
...
func (e *loggingExecutor) createLoggingJob(
	...
	//_, err :=
	//	scheduledloggingjobs.CreateLoggingJob(ctx, &jobs.CreatedByInfo{
	//		ID:   sj.ScheduleID(),
	//		Name: jobs.CreatedByScheduledJobs,
	//	}, txn, cfg.InternalExecutor, p.(*planner).ExecCfg().JobRegistry, e.jobType)
