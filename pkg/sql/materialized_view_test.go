...
`); err != nil {
	...
	descBeforeRefresh := desctestutils.TestingGetPublicTableDescriptor(kvDB, keys.SystemSQLCodec, version, "t", "v")
...
CREATE MATERIALIZED VIEW t.v AS SELECT x FROM t.t;
`)
	desc := desctestutils.TestingGetPublicTableDescriptor(kvDB, keys.SystemSQLCodec, version, "t", "v")
	// Add a zone config to delete all table data.
