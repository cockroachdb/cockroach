...
func MakeBuilder(
	...
	*s = Builder{
	...
	s.indexColDirs = table.IndexFullColumnDirections(index)
	columns := table.IndexFullColumns(index)
...
func (s *Builder) SpanFromEncDatums(
	...
	return rowenc.MakeSpanFromEncDatums(
		values, s.indexColTypes[:len(values)], s.indexColDirs[:len(values)], s.index, &s.alloc, s.KeyPrefix)
}
