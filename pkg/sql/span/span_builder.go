...
// to be the end of the span and the start/end keys are generated by putting them
// in the values row at the prefixLen - 1 position.  Only one of start or end need
// be non-nil, omitted one causing an open ended range span to be generated.  Since
// the exec code knows nothing about index column sorting direction we assume
// ascending if they are descending we deal with that here.
func (s *Builder) SpanFromEncDatumsWithRange(
	...
	makeKeyFromRow := func(values rowenc.EncDatumRow, len int) (k roachpb.Key, cn bool, e error) {
		k, _, cn, e = rowenc.MakeKeyFromEncDatums(values[:len], s.indexColTypes[:len], s.indexColDirs[:len],
	...
	if startDatum != nil {
		...
		startKey, containsNull, err = makeKeyFromRow(values, prefixLen)
		if !startInclusive {
			startKey = startKey.Next()
		}
	} else {
		startKey, containsNull, err = makeKeyFromRow(values, prefixLen-1)
	}
	...
	if endDatum != nil {
		...
		endKey, containsNull, err = makeKeyFromRow(values, prefixLen)
		if endInclusive {
			endKey = endKey.PrefixEnd()
		}
	} else {
		endKey, containsNull, err = makeKeyFromRow(values, prefixLen-1)
		endKey = endKey.PrefixEnd()
	}
	if containsNull || err != nil {
		return roachpb.Span{}, containsNull, err
