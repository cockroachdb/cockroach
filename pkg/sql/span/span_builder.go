...
// MaybeAppendSpansAfterSpanSplitIntoSeparateFamilies uses the needed columns from
...
func (s *Builder) MaybeAppendSpansAfterSpanSplitIntoSeparateFamilies(
	...
	if s.neededFamilies != nil && s.CanSplitSpanIntoSeparateFamilies(len(s.neededFamilies), prefixLen, containsNull) {
		return sqlbase.AppendSpansAfterSpanSplitIntoSeparateFamilies(appendTo, span, s.neededFamilies)
	}
...
func (s *Builder) appendSpansFromConstraintSpan(
	...
	if !forDelete && needed.Len() > 0 && span.Key.Equal(span.EndKey) {
		...
		if s.CanSplitSpanIntoSeparateFamilies(len(neededFamilyIDs), cs.StartKey().Length(), containsNull) {
			return sqlbase.AppendSpansAfterSpanSplitIntoSeparateFamilies(appendTo, span, neededFamilyIDs), nil
		}
	}
