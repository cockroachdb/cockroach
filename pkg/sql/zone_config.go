...
// GetZoneConfigInTxn looks up the zone for the specified object ID,
// by fetching the zone config descriptor directly from the txn. Returns
// the ID of the matching entry, even if it's a placeholder, as well as
// the ID of the actual entry, which will be the parent database or root
// namespace ID if the matching entry was a placeholder.
func GetZoneConfigInTxn(
	ctx context.Context, txn *client.Txn, id uint32, getInheritedDefault bool,
) (uint32, *config.ZoneConfig, uint32, *config.ZoneConfig, error) {
