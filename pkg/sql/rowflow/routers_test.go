...
func TestRouterDiskSpill(t *testing.T) {
	...
	// will occur because routerOutput uses the same memory monitor as its
	// DiskBackedRowContainer does, and we're going to consume rows after the
	// spilling has occurred (meaning that monitor reached its limit). In order
	// for this to not happen we will create a separate memory account.
	for _, memErrorWhenConsumingRows := range []bool{false, true} {
		...
		if !memErrorWhenConsumingRows {
			...
			output.rowBufToPushFromMemAcc.Close(ctx)
			output.rowBufToPushFromMemAcc = &separateAcc
		}
