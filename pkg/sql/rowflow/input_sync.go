...
const (
	...
	// returningRows is the regular operation mode of operation.
	...
	// have been exhausted, and we might have some buffered metadata. Metadata records are
...
func (s *serialSynchronizerBase) ConsumerDone() {
	if s.state != draining {
...
func (s *serialSynchronizerBase) ConsumerClosed() {
	for i := range s.sources {
		s.sources[i].src.ConsumerClosed()
	}
}
...
func (u *serialUnorderedSynchronizer) Next() (rowenc.EncDatumRow, *execinfrapb.ProducerMetadata) {
	for u.srcIndex < len(u.sources) {
		...
		// RFC: do we need to "drainSources" when we go into draining mode?
