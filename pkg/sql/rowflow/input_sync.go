...
type serialUnorderedSynchronizer struct {
	...
	// the receiver wrapped in `ProducerMetadata` when srcIndex has incremented to
	// match serialSrcIndexExclusiveUpperBound.
	exceedsSrcIndexExclusiveUpperBoundErrorFunc func() *execinfrapb.ProducerMetadata
}
	...
	for u.srcIndex < len(u.sources) {
		...
		if row == nil && metadata == nil {
			...
			if u.serialSrcIndexExclusiveUpperBound > 0 && u.srcIndex >= int(u.serialSrcIndexExclusiveUpperBound) &&
				...
				return nil, u.exceedsSrcIndexExclusiveUpperBoundErrorFunc()
			}
			continue
		}
