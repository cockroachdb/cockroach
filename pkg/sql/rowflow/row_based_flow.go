...
// findProcByOutputStreamID looks in spec for a processor that has a pass-through output router
// connected to the specified stream.
...
func (f *rowBasedFlow) setupInputSyncs(
	...
	for pIdx, ps := range spec.Processors {
		for _, is := range ps.Input {
			...
			if is.Type == execinfrapb.InputSyncSpec_UNORDERED {
				if opt == flowinfra.Normal || len(is.Streams) == 1 {
			...
			if sync == nil {
				...
				for i, s := range is.Streams {
					if s.Type == execinfrapb.StreamEndpointSpec_LOCAL {
						numLocal++
					}
