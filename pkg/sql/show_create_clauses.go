...
func formatQuerySequencesForDisplay(
	ctx context.Context, semaCtx *tree.SemaContext, queries string, multiStmt bool, lang catpb.Function_Language,
) (string, error) {
...
func formatFunctionQueryTypesForDisplay(
	...
	case catpb.Function_PLPGSQL:
		...
		v := utils.SQLStmtVisitor{Fn: replaceFunc, TypFn: replaceTypeFunc}
		newStmt := plpgsqltree.Walk(&v, stmts)
		fmtCtx.FormatNode(newStmt)
		fmtCtx.WriteString(";")
	}
