...
func TestEncoderEqualityDatums(t *testing.T) {
	...
	testCases := []struct {
		...
		// TODO: array containing composite
		...
		// TODO(cucaroach):  Received unexpected error:
		...
		FAMILY f (orphan)`,
			...
			[]string{"CREATE INDEX ON t USING GIST(geom)"}},
		...
		{"i INT PRIMARY KEY, s STRING, k INT, INDEX(k,i)",
			...
			[]string{"CREATE INVERTED INDEX ON t (s gin_trgm_ops)"}},
	...
	for _, tc := range testCases {
		tableDef := "CREATE TABLE t (" + tc.cols + ")"
...
func TestEncoderEqualityRand(t *testing.T) {
	...
	sv := &s.ClusterSettings().SV
	for i := 0; i < 100; i++ {
		ct := randgen.RandCreateTableWithName(rng, "t", i, false /* isMultiRegion */)
...
func TestEncoderEqualityString(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			`CREATE TABLE t ("col1_%p0" UUID NOT NULL, col1_1 NAME NOT NULL, col1_2 OID, col1_3 REGNAMESPACE, col1_4 TIMETZ NOT NULL, " c%qol1_5" INET NULL, col1_6 TIMESTAMP NOT NULL, "co""l1_7" STRING NOT NULL, "col1_8" TIMESTAMP NOT NULL,
		...
		{`CREATE TABLE t ("coL1'0_0" FLOAT4, col10_1 TIMESTAMP NOT NULL, " col10_2" VARCHAR NOT NULL, col10_3 STRING NOT NULL AS (lower(" col10_2")) STORED, UNIQUE ("coL1'0_0" ASC) STORING (" col10_2", col10_3), UNIQUE ("coL1'0_0" ASC), FAMILY (col10_1), FAMILY (" col10_2"), FAMILY ("coL1'0_0"), FAMILY (col10_3))`,
			[][]string{
		...
		runComparison(t, desc, datums, tc.tableDef, sv)
		r.Exec(t, "DROP TABLE t")
	}
}
...
func TestMemoryQuota(t *testing.T) {
	...
	enc := colenc.MakeEncoder(keys.SystemSQLCodec, desc, &s.ClusterSettings().SV, cb, cols,
		nil /*metrics*/, nil /*partialIndexMap*/, func() bool {
			return kvb.ApproximateMutationBytes() > 50
		})
	...
	kvb = txn.NewBatch()
	err := enc.PrepareBatch(ctx, kvb, 0, cb.Length())
	require.Equal(t, err, colenc.ErrOverMemLimit)

	for i := 0; i < numRows; i++ {
		kvb = txn.NewBatch()
		err := enc.PrepareBatch(ctx, kvb, i, i+1)
		require.NoError(t, err)
	}
}
...
func buildVecKVs(
	...
	// Send in an empty map just to get better code coverage, partial indexes are
	// tested by copyfrom data driven tests.
	partialIndexMap := make(map[descpb.IndexID]coldata.Bools)
	be := colenc.MakeEncoder(keys.SystemSQLCodec, desc, sv, b, cols, nil /*metrics*/, partialIndexMap, func() bool { return false })
