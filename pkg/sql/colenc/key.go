// Copyright 2023 The Cockroach Authors.
//
// Use of this software is governed by the Business Source License
// included in the file licenses/BSL.txt.
//
// As of the Change Date specified in that file, in accordance with
// the Business Source License, use of this software will be governed
// by the Apache License, Version 2.0, included in the file
// licenses/APL.txt.

package colenc

import (
	"unsafe"

	"github.com/cockroachdb/cockroach/pkg/col/coldata"
	"github.com/cockroachdb/cockroach/pkg/roachpb"
	"github.com/cockroachdb/cockroach/pkg/sql/catalog/catalogkeys"
	"github.com/cockroachdb/cockroach/pkg/sql/catalog/fetchpb"
	"github.com/cockroachdb/cockroach/pkg/sql/rowenc/keyside"
	"github.com/cockroachdb/cockroach/pkg/sql/sem/tree"
	"github.com/cockroachdb/cockroach/pkg/sql/types"
	"github.com/cockroachdb/cockroach/pkg/util/encoding"
)

func partialIndexAndNullCheck[T []byte | roachpb.Key](
	kys []T, r int, nulls *coldata.Nulls, dir encoding.Direction,
) bool {
	if kys[r] == nil {
		return true
	}
	if nulls.NullAt(r) {
		if dir == encoding.Ascending {
			kys[r] = encoding.EncodeNullAscending(kys[r])
		} else {
			kys[r] = encoding.EncodeNullDescending(kys[r])
		}
		return true
	}
	return false
}

// encodeKeys is the columnar version of keyside.Encode.
// Cases taken from decodeTableKeyToCol.
func encodeKeys[T []byte | roachpb.Key](
	kys []T, typ *types.T, dir encoding.Direction, vec coldata.Vec, start, end int,
) error {
	if vec == nil {
		for r := start; r < end; r++ {
			b := kys[r]
			if b == nil {
				continue
			}
			if dir == encoding.Ascending {
				kys[r] = encoding.EncodeNullAscending(b)
			} else {
				kys[r] = encoding.EncodeNullDescending(b)
			}
		}
	}
	nulls := vec.Nulls()
	switch typ.Family() {
	case types.BoolFamily:
		bs := vec.Bool()
		for r := start; r < end; r++ {
			if partialIndexAndNullCheck(kys, r, nulls, dir) {
				continue
			}
			var x int64
			b := kys[r]
			if bs[r] {
				x = 1
			}
			if dir == encoding.Ascending {
				kys[r] = encoding.EncodeVarintAscending(b, x)
			} else {
				kys[r] = encoding.EncodeVarintDescending(b, x)
			}
		}
	case types.IntFamily, types.DateFamily:
		for r := start; r < end; r++ {
			b := kys[r]
			if partialIndexAndNullCheck(kys, r, nulls, dir) {
				continue
			}
			var i int64
			switch typ.Width() {
			case 16:
				is := vec.Int16()
				i = int64(is[r])
			case 32:
				is := vec.Int32()
				i = int64(is[r])
			default:
				is := vec.Int64()
				i = is[r]
			}
			if dir == encoding.Ascending {
				kys[r] = encoding.EncodeVarintAscending(b, i)
			} else {
				kys[r] = encoding.EncodeVarintDescending(b, i)
			}
		}
	case types.FloatFamily:
		fs := vec.Float64()
		for r := start; r < end; r++ {
			b := kys[r]
			if partialIndexAndNullCheck(kys, r, nulls, dir) {
				continue
			}
			f := fs[r]
			if dir == encoding.Ascending {
				kys[r] = encoding.EncodeFloatAscending(b, f)
			} else {
				kys[r] = encoding.EncodeFloatDescending(b, f)
			}
		}
	case types.DecimalFamily:
		ds := vec.Decimal()
		for r := start; r < end; r++ {
			b := kys[r]
			if partialIndexAndNullCheck(kys, r, nulls, dir) {
				continue
			}
			d := &ds[r]
			if dir == encoding.Ascending {
				kys[r] = encoding.EncodeDecimalAscending(b, d)
			} else {
				kys[r] = encoding.EncodeDecimalDescending(b, d)
			}
		}
	case types.BytesFamily, types.StringFamily, types.UuidFamily, types.EnumFamily:
		ss := vec.Bytes()
		for r := start; r < end; r++ {
			b := kys[r]
			if partialIndexAndNullCheck(kys, r, nulls, dir) {
				continue
			}
			s := ss.Get(r)
			if dir == encoding.Ascending {
				kys[r] = encoding.EncodeStringAscending(b, unsafeConvertBytesToString(s))
			} else {
				kys[r] = encoding.EncodeStringDescending(b, unsafeConvertBytesToString(s))
			}
		}
	case types.TimestampFamily, types.TimestampTZFamily:
		ts := vec.Timestamp()
		for r := start; r < end; r++ {
			b := kys[r]
			if partialIndexAndNullCheck(kys, r, nulls, dir) {
				continue
			}
			if dir == encoding.Ascending {
				kys[r] = encoding.EncodeTimeAscending(b, ts.Get(r))
			} else {
				kys[r] = encoding.EncodeTimeDescending(b, ts.Get(r))
			}
		}
	case types.IntervalFamily:
		ds := vec.Interval()
		for r := start; r < end; r++ {
			b := kys[r]
			if partialIndexAndNullCheck(kys, r, nulls, dir) {
				continue
			}
			var err error
			if dir == encoding.Ascending {
				b, err = encoding.EncodeDurationAscending(b, ds.Get(r))
			} else {
				b, err = encoding.EncodeDurationDescending(b, ds.Get(r))
			}
			if err != nil {
				return err
			}
			kys[r] = b
		}
	case types.EncodedKeyFamily:
		for r := start; r < end; r++ {
			b := kys[r]
			if partialIndexAndNullCheck(kys, r, nulls, dir) {
				continue
			}
			kys[r] = append(b, vec.Bytes().Get(r)...)
		}
	default:
		for r := start; r < end; r++ {
			b := kys[r]
			if partialIndexAndNullCheck(kys, r, nulls, dir) {
				continue
			}
			var err error
			b, err = keyside.Encode(b, vec.Datum().Get(r).(tree.Datum), dir)
			if err != nil {
				return err
			}
			kys[r] = b
		}
	}
	return nil
}

// encodeKeyCols is the vector version of rowenc.EncodeIndexKey
func (b *BatchEncoder) encodeIndexKey(
	keyCols []fetchpb.IndexFetchSpec_KeyColumn, nulls *coldata.Nulls,
) error {
	kys := b.keys
	for _, k := range keyCols {
		if k.IsComposite {
			b.compositeColumnIDs.Add(int(k.ColumnID))
		}
		dir, err := catalogkeys.IndexColumnEncodingDirection(k.Direction)
		if err != nil {
			return err
		}
		col, ok := b.colMap.Get(k.ColumnID)
		var vec coldata.Vec
		if ok {
			vec = b.b.ColVec(col)
		} else {
			nulls.SetNulls()
		}
		if err := encodeKeys(kys, k.Type, dir, vec, 0, b.rows); err != nil {
			return err
		}
		if vec.Nulls().MaybeHasNulls() {
			newnulls := nulls.Or(*vec.Nulls())
			*nulls = newnulls
		}
	}
	return nil
}

func unsafeConvertBytesToString(b []byte) string {
	return *(*string)(unsafe.Pointer(&b))
}
