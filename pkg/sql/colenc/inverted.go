...
func (b *BatchEncoder) encodeInvertedSecondaryIndex(
	...
	for row := 0; row < b.rows; row++ {
		...
		for _, key := range keys {
			...
			if err = b.encodeInvertedSecondaryIndexNoFamiliesOneRow(index, key, extraKeys[row], row); err != nil {
				return err
			}
		}
	}
...
func (b *BatchEncoder) encodeInvertedSecondaryIndexNoFamiliesOneRow(
	ind catalog.Index, key roachpb.Key, extraKeyCols []byte, row int,
) error {
	var value []byte
...
func encodeInvertedIndexPrefixKeys(
	kys []roachpb.Key, index catalog.Index, b coldata.Batch, colMap catalog.TableColMap,
) ([]roachpb.Key, error) {
	...
	if numColumns > 1 {
		...
		_, err = encodeColumns(colIDs, dirs, colMap, b.Length(), b.ColVecs(), kys)
		if err != nil {
			return nil, err
		}
	}
