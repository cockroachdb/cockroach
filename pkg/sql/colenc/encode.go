...
type BatchEncoder struct {
	...
	// Column ID that might contains composite encoded values.
...
func (b *BatchEncoder) PrepareBatch(ctx context.Context, p row.Putter, start, end int) error {
	...
	if b.count > math.MaxUint16 {
		return errors.AssertionFailedf(`PrepareBatch illegel arguments: num rows larger than %d; requested %d`, math.MaxUint16, b.count)
	}
	b.init()
...
func (b *BatchEncoder) init() {
	if b.count > cap(b.keys) {
		...
		// If we are doing less row shrink slices.
