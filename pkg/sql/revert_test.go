...
func TestDeleteTableWithPredicateNoHang(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		run func(t *testing.T, invoke func(context.Context) error)
	}{
		{
				...
				filterFn.Store(kvserverbase.ReplicaRequestFilter(
					func(_ context.Context, ba *kvpb.BatchRequest) *kvpb.Error {
						for _, ru := range ba.Requests {
							if dr, ok := ru.GetInner().(*kvpb.DeleteRangeRequest); ok && dr.Predicates.StartTime.IsSet() {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			tc.run(t, func(ctx context.Context) error {
