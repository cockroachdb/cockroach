	...
	if n.All {
		statement := fmt.Sprintf(`
			SELECT start_key
			 ...
			 WHERE table_name='%s'
			   AND index_name='%s'
				 AND manual_split_time IS NOT NULL
		`, n.TableOrIndex.Table.String(), n.TableOrIndex.Index)
		ranges, err := p.ExtendedEvalContext().InternalExecutor.Query(ctx, "split points query", p.txn, statement)
		if err != nil {
			return nil, err
		}
