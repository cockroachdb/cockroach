...
func (n *unsplitAllNode) startExec(params runParams) error {
	...
	rangesIter, err := params.p.ExtendedEvalContext().InternalExecutor.(*InternalExecutor).QueryIteratorEx(
	...
	var ok bool
	for ok, err = rangesIter.Next(params.ctx); ok; ok, err = rangesIter.Next(params.ctx) {
		n.run.keys = append(n.run.keys, []byte(*(rangesIter.Cur()[0].(*tree.DBytes))))
	}
	return err
}
