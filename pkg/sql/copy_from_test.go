...
func doCopyEx(ctx context.Context, t require.TestingT, s serverutils.TestServerInterface, txn *kv.Txn, rows []string, batchSizeOverride int) {
	numrows, err := sql.RunCopyFrom(ctx, s, "c", nil /*txn*/, "COPY lineitem FROM STDIN WITH CSV DELIMITER '|';", rows, batchSizeOverride)
	require.NoError(t, err)
	require.Equal(t, len(rows), numrows)
}

func doCopyImplicit(ctx context.Context, t require.TestingT, s serverutils.TestServerInterface, rows []string) {
	doCopyEx(ctx, t, s, nil, rows, 0)
}

func doCopyWithTxn(ctx context.Context, t require.TestingT, s serverutils.TestServerInterface, txn *kv.Txn, rows []string) {
	doCopyEx(ctx, t, s, txn, rows, 0)
}

func doCopyOneRowBatches(ctx context.Context, t require.TestingT, s serverutils.TestServerInterface, rows []string) {
	doCopyEx(ctx, t, s, nil, rows, 1)
}
...
func TestCopyFromImplicitNonAtomicTransaction(t *testing.T) {
	...
	r.Exec(t, lineitemSchema)
	rows := []string{fmt.Sprintf(csvData, 1), fmt.Sprintf(csvData, 2)}
	// Indicate we want each batch committed individually.
	sql.CopyFromAtomic.Override(ctx, &s.ClusterSettings().SV, false)
	doCopyOneRowBatches(ctx, t, s, rows)
