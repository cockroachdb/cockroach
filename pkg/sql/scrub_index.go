...
func (o *indexCheckOperation) Start(params runParams) error {
	...
	colTypes := func(cols []*sqlbase.ColumnDescriptor) []types.T {
		res := make([]types.T, len(cols))
		for i := range cols {
			res[i] = cols[i].Type
		}
		return res
	...
	// Because the row results include the values from both the primary index and
	// the secondary index, the row results will contain two copies of the column
	// data.
	columnTypes := append(colTypes(pkColumns), colTypes(otherColumns)...)
