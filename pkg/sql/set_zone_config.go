...
//
// Unlike the system tenant, we also catch validate prohibited constraints.
// This is because secondary tenant must operate in the narrow view exposed via
// the RegionServer and are not allowed to configure arbitrary constraints
	...
	for _, constraint := range toValidate {
		...
		case "":
			return errors.WithHint(pgerror.Newf(
				...
				`only "zone" and "region" are allowed in constraints`,
			)
		default:
			return errors.WithHint(pgerror.Newf(
				pgcode.CheckViolation,
