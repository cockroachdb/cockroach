...
func (n *setZoneConfigNode) startExec(params runParams) error {
	...
	applyZoneConfig := func(zs tree.ZoneSpecifier) error {
		...
		partialZoneWithRaw, err := params.p.Descriptors().GetZoneConfigWithRawByte(params.ctx, params.p.Txn(), targetID)
		if err != nil {
			return err
		}
		...
		if partialZoneWithRaw == nil {
			partialZoneWithRaw = catalog.NewZoneConfigWithRawBytes(zonepb.NewZoneConfig(), nil)
			if index != nil {
				subzonePlaceholder = true
			}
		}
		partialZone := partialZoneWithRaw.ZoneConfig()
		...
		// not completeZone.
		{
			...
			if index == nil {
				...
				if err := completeZoneConfig(
					&zoneInheritedFields, helper, targetID,
				); err != nil {
					return err
				}
		...
		if deleteZone {
			...
			if index == nil {
				...
				completeZoneWithRaw, err := params.p.Descriptors().GetZoneConfigWithRawByte(params.ctx, params.p.Txn(), targetID)
				if err != nil {
					return err
				}
				...
				if completeZoneWithRaw == nil {
					...
					completeZone = completeZoneWithRaw.ZoneConfig()
				}
...
type zoneConfigUpdate struct {
	id         descpb.ID
	zoneConfig *catalog.ZoneConfigWithRawBytes
}
...
func prepareZoneConfigWrites(
	...
	return &zoneConfigUpdate{id: targetID, zoneConfig: catalog.NewZoneConfigWithRawBytes(zone, expectedExistingRawBytes)}, nil
}
...
func writeZoneConfigUpdate(
	...
	if update.zoneConfig == nil {
		err = descriptors.DeleteZoneConfig(ctx, kvTrace, b, update.id)
	} else {
		numAffected = 1
...
func RemoveIndexZoneConfigs(
	...
	if zcRewriteNecessary {
		...
		_, err = writeZoneConfig(
			ctx, txn, tableDesc.GetID(), tableDesc, zone, zoneWithRaw.RawBytes(), execCfg, descriptors, false /* hasNewSubzones */, kvTrace,
		)
