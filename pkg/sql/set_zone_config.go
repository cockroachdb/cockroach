...
func (p *planner) SetZoneConfig(ctx context.Context, n *tree.SetZoneConfig) (planNode, error) {
	...
	if n.Options != nil {
		...
		for _, opt := range n.Options {
			...
			req := zoneconfig.GetOption(opt.Key)
			if req == nil {
				return nil, pgerror.Newf(pgcode.InvalidParameterValue,
			...
			valExpr, err := p.analyzeExpr(
				ctx, opt.Value, nil, tree.IndexedVarHelper{}, req.GetRequiredType(), true /*requireType*/, string(opt.Key))
			if err != nil {
				return nil, err
			}
...
func (n *setZoneConfigNode) startExec(params runParams) error {
	...
	if n.options != nil {
		...
		for _, optName := range zoneconfig.GetOptionNames() {
			name := (*tree.Name)(&optName)
