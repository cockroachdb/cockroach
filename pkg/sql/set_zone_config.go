...
func (p *planner) SetZoneConfig(ctx context.Context, n *tree.SetZoneConfig) (planNode, error) {
	...
	if !ZonesTableExists(ctx, p.ExecCfg().Codec, p.ExecCfg().Settings) {
...
func prepareZoneConfigWrites(
	...
	if !ZonesTableExists(ctx, execCfg.Codec, execCfg.Settings) {
...
func getZoneConfigRaw(
	...
	if !ZonesTableExists(ctx, codec, settings) {
