...
var secondaryTenantsAllZoneConfigsEnabled = settings.RegisterBoolSetting(
	...
	"sql.virtual_cluster.feature_access.zone_configs_all.enabled",
	"enable full usage of ALTER CONFIGURE ZONE in virtual clusters",
	false,
)
...
func validateZoneLocalitiesForSecondaryTenants(
	...
	// These should not be accessed directly - user getRegionsAndZones helper
	// instead.
	...
	for _, constraint := range toValidate {
		...
		default:
			if !secondaryTenantsAllZoneConfigsEnabled.Get(&settings.SV) {
				return requireSystemTenantOrClusterSetting(codec, settings, secondaryTenantsAllZoneConfigsEnabled)
			}
		}
	}
	return nil
