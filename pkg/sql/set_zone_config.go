	...
	applyZoneConfig := func(zs tree.ZoneSpecifier) error {
		...
		if deleteZone {
			...
			if index == nil {
				...
				// If we are writing to a zone that is not a subzone, we need to make sure that the zone
				// config is not considered a placeholder anymore. If the settings applied to this zone
				// don't touch the NumReplicas field, set it to nil so that the zone isn't considered a
				// placeholder anymore.
