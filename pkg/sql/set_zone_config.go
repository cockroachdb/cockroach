...
func (n *setZoneConfigNode) startExec(params runParams) error {
	...
	partialZone, err := getZoneConfigRawStrict(params.ctx, params.p.txn, targetID)
	if err != nil {
		return err
	}
...
// getZoneConfigRawStrict looks up the zone config with the given ID. Unlike
// getZoneConfigRaw, if no zone config exists for the given ID, it returns nil.
func getZoneConfigRawStrict(
	ctx context.Context, txn *client.Txn, id sqlbase.ID,
) (*config.ZoneConfig, error) {
	kv, err := txn.Get(ctx, config.MakeZoneKey(uint32(id)))
	if err != nil {
