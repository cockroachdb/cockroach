...
// constraints. This is because secondary tenant must operate in the narrow
// view exposed via the RegionServer and are not allowed to configure arbitrary
...
func validateZoneLocalitiesForSecondaryTenants(
	...
	zones := make(map[string]struct{}, 0)
		...
		case "zone":
			...
			if !found {
				return pgerror.Newf(
					...
					"zone %q matches no existing nodes within the cluster",
					constraint.Value,
				)
			}
		case "region":
