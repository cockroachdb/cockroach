		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for i := len(entries) - 1; i >= 0; i-- {
				...
				if strings.Contains(e.Message, tc.expectedLogStatement+"\"") {
					...
					require.Equal(t, uint32(1), sampledQueryFromLog.StmtCount)
...
// TestTelemetryLoggingStmtCount verifies that StmtCount is logged correctly to
// telemetry. The structure of the other test cases makes it difficult to verify
// this property, so we do it explicitly here.
func TestTelemetryLoggingStmtCount(t *testing.T) {
	defer leaktest.AfterTest(t)()
	sc := log.ScopeWithoutShowLogs(t)
	defer sc.Close(t)
	...
	for i, expected := range expectedQueries {
		...
		for _, e := range entries {
			if strings.Contains(e.Message, expected) {
				...
				require.Equal(t, uint32(i+1), sq.StmtCount, "%s", entries)
				found = true
				break
			}
		}
