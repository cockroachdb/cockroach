	...
	for _, tc := range testData {
		...
		for i := len(entries) - 1; i >= 0; i-- {
			...
			if strings.Contains(e.Message, tc.expectedLogStatement) {
				...
				for _, tag := range tc.expectedUnredactedTags {
					tagRe := regexp.MustCompile(fmt.Sprintf("%s=[^%s%s]+?,?.+$", tag, redact.StartMarker(), redact.EndMarker()))
					if !tagRe.MatchString(e.Tags) {
