...
func TestTelemetryLoggingTransactionMode(t *testing.T) {
	...
	logSpy := sampledQueryLogSpy{
		testState:          t,
		stripRedactMarkers: true,
	}
	logSpy.mu.filter = func(entry eventpb.SampledQuery) bool {
		return strings.Contains(entry.ApplicationName, appName)
	}
	...
	// sampled time. All subtests should enable query sampling to start logging.
	cleanupFn := func() {
	...
	t.Run("respects statements per transaction limit", func(t *testing.T) {
		...
		testutils.RunValues(t, "stmtsPerTxnLimit", []int{1, 2, 3, 4, 5}, func(t *testing.T, stmtsPerTxnLimit int) {
			...
			stmtLogs := logSpy.getStatementLogs()
		...
		},
		{
			...
			params: []fnParams{
				...
				},
				{
					...
					force:       true,
					isInternal:  true,
					expectedRet: true, // Tracing is on.
				},
			},
		},
		{
...
func TestTelemetrySamplesRetriedTxns(t *testing.T) {
	defer leaktest.AfterTest(t)()
	sc := log.ScopeWithoutShowLogs(t)
	defer sc.Close(t)

	appName := "telemetry-logging-transaction-mode-retries"
	ctx := context.Background()
	logSpy := sampledQueryLogSpy{
		testState:          t,
		stripRedactMarkers: true,
	...
	st.SetTime(timeutil.FromUnixMicros(1e6))
	conn.Exec(t, `SET application_name = $1`, appName)
	conn.Exec(t, `SET CLUSTER SETTING sql.telemetry.transaction_sampling.max_event_frequency = 1`)
	conn.Exec(t, `SET CLUSTER SETTING sql.telemetry.query_sampling.mode = "transaction"`)
	conn.Exec(t, `SET CLUSTER SETTING sql.telemetry.query_sampling.enabled = true`)
	conn.Exec(t, `CREATE SEQUENCE seq START WITH 1`)
	conn.Exec(t, `SELECT 1, 2`)

	// Since we haven't advanced the time, we should not have logged anything for the first
	// statement. The time will be advanced on the retry, so we should log the retried statements.
	conn.Exec(t, `BEGIN;
	SELECT 'hello';
	...
	// Verify we logged the retried statement.
	for _, stmt := range stmtLogs {
		if strings.Contains(string(stmt.Statement), "SELECT 'hello'") {
			require.NotZero(t, stmt.NumRetries, "First txn attempt should not have been logged.")
			return
		}
	}
	t.Fatalf("did not find expected query log in log entries")
}
