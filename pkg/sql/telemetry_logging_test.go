...
func TestTelemetryLoggingInternalEnabled(t *testing.T) {
	...
	// Meanwhile, the SELECT query itself should not be logged due to the above
	// query being logged first.
	stubTime := timeutil.FromUnixMicros(int64(1e6))
	st.SetTime(stubTime)
	_, err := db.DB.ExecContext(context.Background(), `SELECT crdb_internal.reset_sql_stats();`)
	if err != nil {
	...
	expectedQueries := []struct {
		...
		{
			logMsg:     `TRUNCATE TABLE system.public.statement_statistics`,
			isInternal: true,
		},
		{
			logMsg:     `TRUNCATE TABLE system.public.transaction_statistics`,
			isInternal: true,
		},
	}
	...
	for _, expected := range expectedQueries {
		found := false
		execMode := executorTypeExec
		if expected.isInternal {
			execMode = executorTypeInternal
		}

		for _, e := range entries {
			//t.Errorf("did not find expected query log in log entries: %s", entries)
			if strings.Contains(e.Message, `"ExecMode":"`+execMode.logLabel()) &&
				strings.Contains(e.Message, expected.logMsg) {
				found = true
				break
			}
		}
		...
		if !found {
			t.Errorf("did not find expected query log in log entries: %s", expected.logMsg)
		}
	}
