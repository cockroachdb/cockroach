...
func TestTelemetryLogging(t *testing.T) {
	...
	db.Exec(t, `SET CLUSTER SETTING sql.log.telemetry.enabled = true;`)
	...
	testData := []struct {
		...
		expectedEffectiveSampleRate string
	}{
		{
	...
	for _, tc := range testData {
		...
		effectiveSampleRateFound := false
		for _, e := range entries {
			if strings.Contains(e.Message, tc.expectedLogStatement) {
				logStatementFound = true
			}
			if strings.Contains(e.Message, tc.expectedEffectiveSampleRate) {
				effectiveSampleRateFound = true
			}
			if logStatementFound && effectiveSampleRateFound {
				break
			}
		}
		...
		if !effectiveSampleRateFound && tc.name != "select-*-limit-2-query" {
			t.Fatal(errors.Newf("matching log statement but no matching effective sample rate for test case %s", tc.name))
		}
		if logStatementFound && effectiveSampleRateFound && tc.name == "select-*-limit-2-query" {
