...
func TestDistSQLFlowsVirtualTables(t *testing.T) {
	...
	// Spin up a separate goroutine that will run the query which will
	...
	testutils.SucceedsSoon(t, func() error {
		for idx, s := range []*distsql.ServerImpl{
			...
			if numRunning != 1 {
				return errors.Errorf("%d flows are found to be running on node %d, %d expected", numRunning, idx+1, 1)
			}
		}
		return nil
	})
