...
CREATE TABLE nojob (k INT8);`); err != nil {
	...
	var databaseID, tableTID, tableFkTblID, tableTblID, tableNoJobID int

	databaseIDRow := sqlDB.QueryRow(`SELECT id FROM system.namespace WHERE name='t'`)
	tableTIDRow := sqlDB.QueryRow(`SELECT id FROM system.namespace WHERE name='test'`)
	tableFkTblIDRow := sqlDB.QueryRow(`SELECT id FROM system.namespace WHERE name='fktbl'`)
	tableTblRow := sqlDB.QueryRow(`SELECT id FROM system.namespace WHERE name='tbl'`)
	tableNoJobRow := sqlDB.QueryRow(`SELECT id FROM system.namespace WHERE name='nojob'`)

	if err := databaseIDRow.Scan(&databaseID); err != nil {
		t.Fatal(err)
	}
	if err := tableTIDRow.Scan(&tableTID); err != nil {
		t.Fatal(err)
	}
	if err := tableFkTblIDRow.Scan(&tableFkTblID); err != nil {
		t.Fatal(err)
	}
	if err := tableTblRow.Scan(&tableTblID); err != nil {
		t.Fatal(err)
	}
	if err := tableNoJobRow.Scan(&tableNoJobID); err != nil {
		t.Fatal(err)
	}
