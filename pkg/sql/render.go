...
func (s *renderNode) initTargets(
	...
	for i, target := range targets {
		...
		outputName, err := getRenderColName(target)
		if err != nil {
			return err
		}
		...
		cols, exprs, hasStar, err := s.planner.computeRender(ctx, newTarget, desiredType,
			s.source.info, s.ivarHelper, outputName, true)
		if err != nil {
			return err
		}
...
func getRenderColName(target parser.SelectExpr) (string, error) {
	if target.As != "" {
