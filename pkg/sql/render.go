...
func (v *srfExtractionVisitor) VisitPost(expr tree.Expr) tree.Expr {
	...
	case *tree.ColumnAccessExpr:
		...
		log.Infof(v.ctx, "replaced1 %q -> %q", t, newExpr)
		return newExpr
	case *tree.FuncExpr:
		if t.EscapeSRF {
			return expr
		...
		if foundSRF {
			...
			log.Infof(v.ctx, "replaced2 %q -> %q", t, newExpr)
			return newExpr
		}
	}
	return expr
...
func (p *planner) rewriteSRFs(
	...
	*v = srfExtractionVisitor{
	...
	for i, target := range targets {
		...
		log.Infof(v.ctx, "fini: replaced %q -> %q. New FROM clause:\n%s", target.Expr, newExpr, r.sourceInfo.String())
...
func (v *srfExtractionVisitor) transformSRF(
	...
	log.Infof(v.ctx, "replacing srf %q: new source info:\n%s", srfName, v.r.source.info.String())
	...
	for i, c := range src.info.SourceColumns {
