...
func (s *renderNode) initTargets(
	...
	for i, target := range targets {
		...
		var err error
		target, err = s.rewriteSRFs(ctx, target)
		if err != nil {
			return err
		}
...
// extended to extract all SRFs in the expression into a list instead of
// returning an error if more than one SRF is discovered.
