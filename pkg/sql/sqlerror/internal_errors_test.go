...
func TestInternalError(t *testing.T) {
	type pred struct {
		name string
		fn   func(*testing.T, *pgerror.Error, []SafeDetailPayload)
	}
	...
	testData := []struct {
		...
		// 0
		{
			...
			[]pred{
				...
				{"basefields", func(t *testing.T, e *pgerror.Error, safeDetails []SafeDetailPayload) {
				...
				{"safedetail", func(t *testing.T, e *pgerror.Error, safeDetails []SafeDetailPayload) {
				...
				{"format", func(t *testing.T, e *pgerror.Error, safeDetails []SafeDetailPayload) {
		...
		// 2
		{
			...
			NewError(pgerror.CodeSyntaxError, "syn %s"),
			[]pred{
				{"basefields", func(t *testing.T, e *pgerror.Error, safeDetails []SafeDetailPayload) {
				...
				{"no-details", func(t *testing.T, e *pgerror.Error, safeDetails []SafeDetailPayload) {
		...
		// 3
		{
			...
			Wrap(errors.Wrap(makeNormal(), "wrapA"), pgerror.CodeSyntaxError, "wrapB"),
			[]pred{
				{"basefields", func(t *testing.T, e *pgerror.Error, safeDetails []SafeDetailPayload) {
					eq(t, e.Code, pgerror.CodeSyntaxError)
		...
		// 4
		{
			...
			Wrap(errors.Wrap(makeBoo(), "wrapA"), pgerror.CodeSyntaxError, "wrapB"),
			[]pred{
				{"basefields", func(t *testing.T, e *pgerror.Error, safeDetails []SafeDetailPayload) {
					eq(t, e.Code, pgerror.CodeInternalError)
				...
				{"details", func(t *testing.T, e *pgerror.Error, safeDetails []SafeDetailPayload) {
					// Simple errors do not store stack trace details.
		...
		// 5
		{
			...
			Wrapf(NewError(pgerror.CodeSyntaxError, "syn foo"), pgerror.CodeAdminShutdownError, "wrap %s", "waa"),
			[]pred{
				{"basefields", func(t *testing.T, e *pgerror.Error, safeDetails []SafeDetailPayload) {
				...
				{"no-details", func(t *testing.T, e *pgerror.Error, safeDetails []SafeDetailPayload) {
					// Simple errors do not store stack trace details.
					eq(t, e.Detail, "")
					m(t, safeDetails[0].SafeMessage, `\*sqlerror.withMessage: caused by \*pgerror.Error`)
		...
		// 6
		{
			...
			Wrapf(fmt.Errorf("fmt err"), pgerror.CodeAdminShutdownError, "wrap %s", "waa"),
			[]pred{
				{"basefields", func(t *testing.T, e *pgerror.Error, safeDetails []SafeDetailPayload) {
					// Wrap adds a prefix to the message.
				...
				{"no-details", func(t *testing.T, e *pgerror.Error, safeDetails []SafeDetailPayload) {
					...
					m(t, safeDetails[1].SafeMessage, `\*sqlerror.withMessage: caused by \*errors.errorString`)
				}},
			},
		},
		// 7
		{
			...
			Wrap(fmt.Errorf("fmt err"), pgerror.CodeAdminShutdownError, "wrapx waa"),
			[]pred{
				{"basefields", func(t *testing.T, e *pgerror.Error, safeDetails []SafeDetailPayload) {
					// Wrap adds a prefix to the message.
		...
		// 8
		{
			...
			Wrapf(errors.Wrap(fmt.Errorf("fmt"), "wrap1"), pgerror.CodeAdminShutdownError, "wrap2 %s", "waa"),
			[]pred{
				{"basefields", func(t *testing.T, e *pgerror.Error, safeDetails []SafeDetailPayload) {
					// Wrap adds a prefix to the message.
		...
		// 9
		{
			...
			[]pred{
				...
				{"retained-details", func(t *testing.T, e *pgerror.Error, safeDetails []SafeDetailPayload) {
					ml(t, e.Detail,
						[]string{
					...
					m(t, safeDetails[1].SafeMessage, `\*sqlerror.withMessage`)
		...
		// 10
		{
			...
			NewAssertionErrorWithWrappedErrf(NewError(pgerror.CodeSyntaxError, "syn err"), "iewrap %s", "waa"),
			[]pred{
				{"basefields", func(t *testing.T, e *pgerror.Error, safeDetails []SafeDetailPayload) {
					// Wrap adds a prefix to the message.
		...
		// 12
		{
			...
			Wrapf(&roachpb.UnhandledRetryableError{}, "", "rwrap"),
			[]pred{
				{"basefields", func(t *testing.T, e *pgerror.Error, safeDetails []SafeDetailPayload) {
					// Ensure the restart prefix occurs in the message.
					m(t, e.Message, TxnRetryMsgPrefix+": rwrap: ")
		...
		{
			// Check that retry errors are wrapped properly.
			Wrapf(&roachpb.TransactionRetryWithProtoRefreshError{}, "", "rwrap"),
			[]pred{
				{"basefields", func(t *testing.T, e *pgerror.Error, safeDetails []SafeDetailPayload) {
					// Ensure the restart prefix occurs in the message.
					m(t, e.Message, TxnRetryMsgPrefix+": rwrap: ")
		...
		{
			// Check that retry errors are wrapped properly.
			Wrapf(&roachpb.AmbiguousResultError{}, "", "rwrap"),
			[]pred{
				{"basefields", func(t *testing.T, e *pgerror.Error, safeDetails []SafeDetailPayload) {
					// Ensure the restart prefix occurs in the message.
					m(t, e.Message, "rwrap: ")
		...
		t.Run(fmt.Sprintf("%d %s", i, test.err), func(t *testing.T) {
			for _, pred := range test.preds {
				t.Run(pred.name, func(t *testing.T) {
					// t.Logf("input error:\n%+v", test.err)
					pgErr, details, _ := Flatten(test.err)
...
func makeNormal() error {
	return NewError(pgerror.CodeSyntaxError, "syn")
}

func doWrap(err error) error {
	return Wrapf(err, pgerror.CodeAdminShutdownError, "wrap %s", "woo")
}
