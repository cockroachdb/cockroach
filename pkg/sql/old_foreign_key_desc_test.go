...
func TestOldForeignKeyRepresentationGetsUpgraded(t *testing.T) {
	...
	if _, err := sqlDB.Exec(`
CREATE DATABASE t;
CREATE TABLE t.fk1 (x INT);
CREATE TABLE t.fk2 (x INT, UNIQUE INDEX (x));
ALTER TABLE t.fk1 ADD CONSTRAINT fk1 FOREIGN KEY (x) REFERENCES t.fk2 (x);
CREATE INDEX ON t.fk1 (x);
`); err != nil {
		t.Fatal(err)
	}
	desc := sqlbase.GetTableDescriptor(kvDB, "t", "fk1")
	fmt.Println(desc.OutboundFKs)
	fmt.Println(desc.Mutations)
	desc = sqlbase.GetTableDescriptor(kvDB, "t", "fk2")
	fmt.Println(desc.InboundFKs)
	fmt.Println(desc.Mutations)
	...
	if _, err := sqlDB.Exec(`DROP INDEX t.fk1@fk1_auto_index_fk1`); err != nil {
		t.Fatal(err)
	}
	desc = sqlbase.GetTableDescriptor(kvDB, "t", "fk2")
