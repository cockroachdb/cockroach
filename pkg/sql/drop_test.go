...
func TestCreateTableFKRollbackCleansBackrefs(t *testing.T) {
	...
	// the RunBeforeResume knob can target the right job. We use a goroutine
	// because CREATE TABLE will block until the schema change job completes
	// (or fails).
	//
	...
	// The CREATE TABLE statement itself may or may not return an error
	// depending on timing. What matters is that the table ends up not
	// existing and the parent's InboundFKs are clean.
	_ = err
