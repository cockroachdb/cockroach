...
func TestDropTableDeleteData(t *testing.T) {
	...
	for i := 0; i < numTables; i++ {
		...
		if err := tests.CreateKVTable(sqlDB, name, numRows); err != nil {
			t.Fatal(err)
		}

		descs = append(descs, sqlbase.GetTableDescriptor(kvDB, "t", name))

		nameKey := sqlbase.MakeNameMetadataKey(keys.MaxReservedDescID+1, name)
		gr, err := kvDB.Get(ctx, nameKey)
		if err != nil {
			t.Fatal(err)
		}
		...
		if _, err := sqlDB.Exec(fmt.Sprintf(`DROP TABLE t.%s`, name)); err != nil {
			t.Fatal(err)
		}
	}
	...
	// Push a zone config reducing the TTL to 0 for descriptor i.
