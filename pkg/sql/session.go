...
type Session struct {
	...
	mu struct {
		...
		ActiveQueries map[parser.Statement]queryMeta
	}
...
func NewSession(
	...
	s.mu.ActiveQueries = make(map[parser.Statement]queryMeta)
...
func (s *Session) addActiveQuery(stmt parser.Statement) {
	s.mu.Lock()
	s.mu.ActiveQueries[stmt] = queryMeta{
		start: timeutil.Now(),
	}
	s.mu.Unlock()
}
...
func (s *Session) removeActiveQuery(stmt parser.Statement) {
	s.mu.Lock()
	delete(s.mu.ActiveQueries, stmt)
	s.mu.Unlock()
}
...
func (s *Session) Serialize() serverpb.Session {
	...
	for stmt, meta := range s.mu.ActiveQueries {
