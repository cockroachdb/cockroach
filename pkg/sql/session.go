...
type Session struct {
	...
	//
	// The reason why txnMon and mon are split is to enable separate
	// reporting of statistics per transaction and per session. This is
	// because the "interesting" behavior w.r.t memory is typically
	// caused by transactions, not sessions. The reason why sessionMon
	// and mon are split is to enable separate reporting of statistics
	// for result sets (which escape transactions).
	mon        mon.MemoryMonitor
	sessionMon mon.MemoryMonitor
	// txnMon tracks txn-bound objects like the running state of
...
func (ts *txnState) resetForNewSQLTxn(e *Executor, s *Session) {
	...
	*ts = txnState{}
	...
	s.txnMon.Start(ctx, &s.mon, mon.BoundAccount{})
