...
type Session struct {
	...
	// ClientAddr is the user's client IP address and port.
	ClientAddr string
...
func (s *Session) Finish(e *Executor) {
	...
	// from any SHOW SESSIONS queries.
...
// SerializeToResponse serializes a Session into a serverpb.ActiveSession
// that can be served over RPC.
func (s *Session) SerializeToResponse() serverpb.ActiveSession {
	s.mu.RLock()
	defer s.mu.RUnlock()
	s.TxnState.mu.RLock()
...
type txnState struct {
	...
	// Note that reads to mu.txn from the session's main goroutine
