...
type Session struct {
	...
	mu struct {
		...
		ActiveQueries map[*queryMeta]struct{}
	}
...
func NewSession(
	...
	s.mu.ActiveQueries = make(map[*queryMeta]struct{})
...
func (s *Session) addActiveQuery(stmt parser.Statement) *queryMeta {
	sql := stmt.String()
	if len(sql) > 1000 {
		sql = sql[:997] + "..."
	}
...
func (s *Session) removeActiveQuery(query *queryMeta) {
	s.mu.Lock()
