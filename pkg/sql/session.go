...
type queryMeta struct {
	...
	// States whether this query is using distSQL. Note that all queries,
	// including distSQL ones, have this field set to false until start of
	// execution; only at that point can we can actually determine whether
	// this query will use DistSQL.
	usingDistSQL bool
}
...
func (s *Session) addActiveQuery(stmt parser.Statement) queryHandle {
	sql := stmt.String()
	if len(sql) > 1000 {
		sql = sql[:997] + "..."
	}
...
// the query's metadata to indicate whether it's using distSQL or not
func (s *Session) setQueryExecutionMode(query queryHandle, usingDistSQL bool) {
	s.mu.Lock()
	query.usingDistSQL = usingDistSQL
	s.mu.Unlock()
}
...
func (s *Session) Serialize() serverpb.Session {
	...
	for query := range s.mu.ActiveQueries {
		activeQueries = append(activeQueries, serverpb.ActiveQuery{
			Start:        query.start,
			Sql:          query.sql,
			UsingDistSQL: query.usingDistSQL,
		})
	}
