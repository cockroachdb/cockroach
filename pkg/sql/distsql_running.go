...
func (dsp *DistSQLPlanner) PlanAndRunSubqueries(
	...
	for planIdx, subqueryPlan := range subqueryPlans {
		result, err := dsp.planAndRunSubquery(ctx, planner, evalCtxFactory, recv, maybeDistribute, subqueryPlan)
		subqueryPlans[planIdx].started = true
		if err != nil {
			recv.SetError(err)
			return false
		}
		subqueryPlan.result = result
	}
...
func (dsp *DistSQLPlanner) planAndRunSubquery(
	...
	if subqueryRecv.commErr != nil {
		return nil, subqueryRecv.commErr
	}
	...
	case distsqlrun.SubqueryExecModeExists:
		...
		hasRows := rows.Len() != 0
		return tree.MakeDBool(tree.DBool(hasRows)), nil
	...
	case distsqlrun.SubqueryExecModeOneRow:
		...
		case 1:
			...
			case 1:
				return row[0], nil
			default:
				return &tree.DTuple{D: rows.At(0)}, nil
			}
		default:
			return nil, fmt.Errorf("more than one row returned by a subquery used as an expression")
		}
	default:
		return nil, fmt.Errorf("unexpected subqueryExecMode: %d", subqueryPlan.execMode)
	}
}
