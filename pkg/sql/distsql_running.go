...
func (dsp *DistSQLPlanner) setupFlows(
	...
	// SetupLocalSyncFlow will overwrite ctx to the context of the local flow,
	// but we cannot use it in a couple of places around the remote flows
	// handling (namely, to issue the SetupFlow RPCs in parallel and to spin up
	// a new goroutine to wait for those RPCs to come back). This is the case
	// since the context of the local flow is always canceled in Flow.Cleanup()
	// which can happen before the "remote" things happen.
	origCtx := ctx
	...
	if len(flows) == 1 || err != nil {
		// If we there are no remote flows, or we fail to set up the local flow,
		// we can just short-circuit.
		//
