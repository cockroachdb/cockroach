...
func (r *DistSQLReceiver) makeScanEstimates(
	...
	for _, p := range physPlan.Processors {
		...
		// TODO(#168093): we might have multiple TableReaders for the same
		// stage in which we case we actually need to merge all spans together.
		// Probably we want to capture scanNode.spans before partitioning during
		// the physical planning and then propagate them here.
		...
		if r.fixMisestimates {
			spans = func() roachpb.Spans {
				...
				if typ := col.GetType(); typ.Family() == types.BoolFamily || typ.UserDefined() {
					// Don't try to fix up scan misestimates if we have a bool
					// or a UDT as the first index column since such a fixup
					// stats collection could result in a very large scan.
					return nil
				}
