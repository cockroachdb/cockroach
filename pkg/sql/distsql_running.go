...
func MakeDistSQLReceiver(
	...
	batchWriter, _ := resultWriter.(batchResultWriter)
	if _, isStreamingCommandResult := resultWriter.(*streamingCommandResult); isStreamingCommandResult {
		// streamingCommandResult currently doesn't support receiving batches.
		// TODO(yuzefovich): this is a bit hacky, but I'm not sure how to make
		// it cleaner.
		batchWriter = nil
	}
	if _, isLimitedCommandResult := resultWriter.(LimitedCommandResult); isLimitedCommandResult {
		// Currently the limiting logic isn't implemented in
		// pgwire.limitedCommandResult.AddBatch (the default non-limiting
