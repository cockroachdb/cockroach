...
type cancelFlowsCoordinator struct {
	mu struct {
		syncutil.Mutex
		// deadFlows is a ring of pointers to deadFlowsOnNode objects.
		deadFlows ring.Buffer
	}
...
func (c *cancelFlowsCoordinator) getFlowsToCancel() (
	*execinfrapb.CancelDeadFlowsRequest,
	...
	c.mu.Lock()
	defer c.mu.Unlock()
	if c.mu.deadFlows.Len() == 0 {
		return nil, roachpb.NodeID(0)
	}
	deadFlows := c.mu.deadFlows.GetFirst().(*deadFlowsOnNode)
	c.mu.deadFlows.RemoveFirst()
...
func (c *cancelFlowsCoordinator) addFlowsToCancel(flows map[roachpb.NodeID]*execinfrapb.FlowSpec) {
	...
	for nodeID, f := range flows {
		if nodeID != f.Gateway {
			// c.mu.deadFlows.Len() is at most the number of nodes in the
			// cluster, so a linear search for the node ID should be
			// sufficiently fast.
			found := false
			for j := 0; j < c.mu.deadFlows.Len(); j++ {
				deadFlows := c.mu.deadFlows.Get(j).(*deadFlowsOnNode)
			...
			if !found {
				c.mu.deadFlows.AddLast(&deadFlowsOnNode{
	...
	// Notify the canceling workers that there are some flows to cancel. Note
	// that we do it in a non-blocking fashion (because the workers might be
	// busy canceling other flows at the moment). Also because the channel is
	// buffered, they won't go to sleep once they are done.
	numWorkersToWakeUp := numCancelingWorkers
	if numWorkersToWakeUp < c.mu.deadFlows.Len() {
		numWorkersToWakeUp = c.mu.deadFlows.Len()
	}
