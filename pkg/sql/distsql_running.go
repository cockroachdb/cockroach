...
func (dsp *DistSQLPlanner) Run(
	...
	if txn == nil {
		...
		if noMutations {
			...
			safeForStreamer := planCtx.distSQLProhibitedCauses&distSQLProhibitedCauses(^safeForStreamerBits) == 0
...
func (dsp *DistSQLPlanner) planAndRunSubquery(
	...
	subqueryDistribution, noDistSQLCauses := planner.getPlanDistribution(ctx, subqueryPlan.plan, notPostquery)
	...
	subqueryPlanCtx.distSQLProhibitedCauses = noDistSQLCauses
...
func (dsp *DistSQLPlanner) planAndRunPostquery(
	...
	postqueryDistribution, noDistSQLCauses := planner.getPlanDistribution(ctx, postqueryPlan, postqueryInfo)
	distribute := DistributionType(LocalDistribution)
	if postqueryDistribution.WillDistribute() {
		distribute = FullDistribution
	}
	postqueryPlanCtx := dsp.NewPlanningCtx(ctx, evalCtx, planner, planner.txn, distribute)
	postqueryPlanCtx.distSQLProhibitedCauses = noDistSQLCauses
