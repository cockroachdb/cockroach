...
func (s *scanStageEstimate) logMisestimate(
	ctx context.Context, planner *planner, migrator log.StructuredEventMigrator,
) {
	...
	migrator.StructuredEvent(ctx, severity.WARNING, event)
}
...
func (r *DistSQLReceiver) maybeLogMisestimates(ctx context.Context, planner *planner) {
	...
	migrator := log.NewStructuredEventMigrator(func() bool {
		return log.ShouldMigrateEvent(planner.ExecCfg().SV())
	...
	for _, s := range r.scanStageEstimateMap {
		...
		s.logMisestimate(ctx, planner, migrator)
	}
}
