...
func (dsp *DistSQLPlanner) setupFlows(
	...
	if vectorizeMode := evalCtx.SessionData.VectorizeMode; vectorizeMode != sessiondatapb.VectorizeOff {
		if !vectorizeThresholdMet && (vectorizeMode == sessiondatapb.Vectorize201Auto || vectorizeMode == sessiondatapb.VectorizeOn) {
			...
			for _, spec := range flows {
				if setupReq.EvalContext.SessionData.VectorizeMode == sessiondatapb.VectorizeOff {
					// We have already determined that we don't support the
					// vectorization of at least one flow, so we can stop
					// further checks.
					break
				}
				for _, p := range spec.Processors {
					if err := colbuilder.IsSupported(vectorizeMode, &p); err != nil {
