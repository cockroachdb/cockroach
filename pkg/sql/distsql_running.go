...
func (r *DistSQLReceiver) Types() []types.ColumnType {
...
func (dsp *DistSQLPlanner) planAndRunSubquery(
	...
	if subqueryPlan.execMode == distsqlrun.SubqueryExecModeExists {
		...
		typ = sqlbase.ColTypeInfoFromColTypes([]types.ColumnType{})
	} else {
		...
		colTypes := make([]types.ColumnType, len(subqueryPhysPlan.PlanToStreamColMap))
