...
// In the future if we add TTL as a cluster setting and evict entries that reach a certain age,
// we would also store a timesource that indicates when the cache was created.
...
// In the future if we add TTL as a cluster setting and evict entries that reach
// a certain age, we would also store a timestamp in this node.
...
// Add adds a new statement to the RecentStatementsCache using the previously
// described recentStatementsCacheKey and recentStatementsNode.
