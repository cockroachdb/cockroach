...
func TestGCTenant(t *testing.T) {
	...
	t.Run("existent tenant deleted progress", func(t *testing.T) {
		...
		require.EqualError(
			...
			`GC state for tenant id:11 state:DROP name:"anonymous-tenant-11" dropped_name:"" tenant_replication_job_id:0 capabilities:<> is DELETED yet the tenant row still exists`,
		)
	})
