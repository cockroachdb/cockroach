...
func (sr *schemaResolver) ResolveFunctionByOID(
	...
	curDB, err := sr.MustGetCurrentSessionDatabase(ctx)
	if err != nil {
		return nil, nil, err
	}
	if curDB.GetID() != funcDesc.GetParentID() {
		return nil, nil, pgerror.Newf(
			pgcode.FeatureNotSupported, "cross database function references are not supported: %d",
			funcDesc.GetID(),
		)
	}
