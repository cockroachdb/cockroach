...
func (sr *schemaResolver) ResolveFunction(
	...
	if fn.ExplicitSchema && fn.Schema() != catconstants.CRDBInternalSchemaName {
		...
		sc := prefix.Schema
		udfDef, _ = sc.GetPrefixedFuncDefinition(fn.Object())
	} else {
		if err := path.IterateSearchPath(func(schema string) error {
			...
			curUdfDef, found := prefix.Schema.GetPrefixedFuncDefinition(fn.Object())
			if !found {
				return nil
			}
	...
	if builtinDef == nil && udfDef == nil {
		return nil, pgerror.Newf(pgcode.UndefinedFunction, "unknown function %s", fn.String())
	}
	if builtinDef == nil {
		return udfDef, nil
	}
	if udfDef == nil {
		...
		if props.UnsupportedWithIssue != 0 {
			...
			if props.UnsupportedWithIssue < 0 {
				return nil, unimplemented.New(builtinDef.Name+"()", msg)
			}
			return nil, unimplemented.NewWithIssueDetail(props.UnsupportedWithIssue, builtinDef.Name, msg)
		}
		return builtinDef, nil
	}

	return &tree.ResolvedFunctionDefinition{
		Name: fn.Object(),
		// ExplicitSchema is set to false since there are overloads from different
		// schemas.
		ExplicitSchema: false,
		Overloads:      append(builtinDef.Overloads, udfDef.Overloads...),
	}, nil
}
