...
func (p *planner) renameType(ctx context.Context, n *alterTypeNode, newName string) error {
	...
	arrayDesc, err := p.Descriptors().GetMutableTypeByID(ctx, p.txn, n.desc.ArrayTypeID)
	if err != nil {
		return err
	}
	if err := p.performRenameTypeDesc(
...
func (p *planner) alterTypeOwner(
	...
	arrayDesc, err := p.Descriptors().GetMutableTypeByID(ctx, p.txn, typeDesc.ArrayTypeID)
	if err != nil {
		return err
	}
