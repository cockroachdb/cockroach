...
func (p *planner) renameType(ctx context.Context, n *alterTypeNode, newName string) error {
	...
	arrayDesc, err := p.Descriptors().ByID(p.txn).Mutable().Type(ctx, n.desc.ArrayTypeID)
	if err != nil {
		return err
	}
	if err := p.performRenameTypeDesc(
...
func (p *planner) alterTypeOwner(
	...
	arrayDesc, err := p.Descriptors().ByID(p.txn).Mutable().Type(ctx, typeDesc.ArrayTypeID)
	if err != nil {
		return err
	}
