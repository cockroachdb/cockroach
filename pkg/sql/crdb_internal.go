...
CREATE TABLE crdb_internal.logical_replication_node_processors (
	...
	populate: func(ctx context.Context, p *planner, _ catalog.DatabaseDescriptor, addRow func(...tree.Datum) error) error {
		...
		for _, container := range sm.DebugGetLogicalConsumerStatuses(ctx) {
			...
			curOrLast := func(currentNanos int64, lastNanos int64, currentState string) tree.Datum {
			...
			if err := addRow(
				...
				tree.NewDString(status.CurrentState),                                                                                 // current_state
				dur(status.Recv.TotalWaitNanos+nanosSince(status.Recv.CurReceiveStart)),                                              // recv_time
				curOrLast(nanosSince(status.Recv.CurReceiveStart), status.Recv.LastWaitNanos, streampb.ReceiveState),                 // last_recv_time
				...
				curOrLast(nanosSince(status.Flushes.Last.CurFlushStart), status.Flushes.Last.LastFlushNanos, streampb.FlushingState), // last_flush_time
