  ...
  INDEX(status)
	...
	indexes: []virtualIndex{{
		populate: func(ctx context.Context, unwrappedConstraint tree.Datum, p *planner, _ catalog.DatabaseDescriptor, addRow func(...tree.Datum) error) (matched bool, err error) {
			...
			q := jobsQuery + qIndexFilter
			return generateJobRows(ctx, p, q, addRow)
		},
	}},
	comment: `decoded job metadata from system.jobs (KV scan)`,
	populate: func(ctx context.Context, p *planner, db catalog.DatabaseDescriptor, addRow func(...tree.Datum) error) error {
		_, err := generateJobRows(ctx, p, jobsQuery, addRow)
		return err
	},
}

// generateJobRows calls addRow for each job. It returns true if addRow was called
// successfully at least once.
func generateJobRows(
	ctx context.Context, p *planner, query string, addRow func(...tree.Datum) error,
