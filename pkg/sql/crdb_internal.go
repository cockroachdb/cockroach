...
func writeCreateTypeDescRow(
	db *dbdesc.Immutable, sc string, typeDesc *typedesc.Immutable, addRow func(...tree.Datum) error,
...
CREATE TABLE crdb_internal.lost_descriptors_with_data (
	...
	populate: func(ctx context.Context, p *planner, dbContext *dbdesc.Immutable, addRow func(...tree.Datum) error) error {
		...
		descriptors, err := catalogkv.GetAllDescriptorsAndNamespaceEntriesUnvalidated(ctx, p.txn, p.extendedEvalCtx.Codec)
		if err != nil {
			return err
		}
		descriptorMap := descriptors.Descriptors
