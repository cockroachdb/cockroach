...
func populateExecutionInsights(
	...
	for _, insight := range response.Insights {
		for _, s := range insight.Statements {
			...
			startTimestamp, err = tree.MakeDTimestamp(s.Statement.StartTime, time.Nanosecond)
			if err != nil {
				return err
			}

			var endTimestamp *tree.DTimestamp
			endTimestamp, err = tree.MakeDTimestamp(s.Statement.EndTime, time.Nanosecond)
			if err != nil {
				return err
			}

			execNodeIDs := tree.NewDArray(types.Int)
			for _, nodeID := range s.Statement.Nodes {
				if err = execNodeIDs.Append(tree.NewDInt(tree.DInt(nodeID))); err != nil {
					return err
				}
			}
			...
			if s.Statement.AutoRetryReason != "" {
				autoRetryReason = tree.NewDString(s.Statement.AutoRetryReason)
			}

			contentionTime := tree.DNull
			if s.Statement.Contention != nil {
				contentionTime = tree.NewDInterval(
					duration.MakeDuration(s.Statement.Contention.Nanoseconds(), 0, 0),
					types.DefaultIntervalTypeMetadata,
				)
			}

			contentionEvents := tree.DNull
			if len(s.Statement.ContentionEvents) > 0 {
				var contentionEventsJSON json.JSON
				contentionEventsJSON, err = convertContentionEventsToJSON(ctx, p, s.Statement.ContentionEvents)
				if err != nil {
					return err
				}
			...
			indexRecommendations := tree.NewDArray(types.String)
			for _, recommendation := range s.Statement.IndexRecommendations {
				if err = indexRecommendations.Append(tree.NewDString(recommendation)); err != nil {
					return err
				}
			}
			...
			err = errors.CombineErrors(err, addRow(
				...
				tree.NewDString(hex.EncodeToString(s.Statement.ID.GetBytes())),
				tree.NewDBytes(tree.DBytes(sqlstatsutil.EncodeUint64ToBytes(uint64(s.Statement.FingerprintID)))),
				tree.NewDString(s.Problem.String()),
				causes,
				tree.NewDString(s.Statement.Query),
				tree.NewDString(s.Statement.Status.String()),
				startTimestamp,
				endTimestamp,
				tree.MakeDBool(tree.DBool(s.Statement.FullScan)),
				tree.NewDString(insight.Transaction.User),
				tree.NewDString(insight.Transaction.ApplicationName),
				tree.NewDString(s.Statement.Database),
				tree.NewDString(s.Statement.PlanGist),
				tree.NewDInt(tree.DInt(s.Statement.RowsRead)),
				tree.NewDInt(tree.DInt(s.Statement.RowsWritten)),
				tree.NewDString(insight.Transaction.UserPriority),
				tree.NewDInt(tree.DInt(s.Statement.Retries)),
				autoRetryReason,
				execNodeIDs,
				contentionTime,
				contentionEvents,
