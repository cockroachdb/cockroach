...
func makeJobsTableRows(
	...
	sessionJobs := make([]*jobs.Record, 0, p.extendedEvalCtx.JobsInfo.numJobsToCreate())
	uniqueJobs := make(map[*jobs.Record]struct{})
	if err := p.extendedEvalCtx.JobsInfo.forEachJobToCreate(func(job *jobs.Record) error {
...
func collectMarshaledJobMetadataMap(
	...
	if err := p.ExtendedEvalContext().JobsInfo.forEachJobToCreate(func(record *jobs.Record) error {
