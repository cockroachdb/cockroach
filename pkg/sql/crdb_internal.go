...
func (p *planner) getAllNames(ctx context.Context) (map[sqlbase.ID]NamespaceKey, error) {
	return GetAllNamesInternal(ctx, p.txn, p.ExtendedEvalContext().ExecCfg.InternalExecutor)
}

// GetAllNamesInternal is the testable implementation of getAllNames.
// It is public so that it can be tested outside the sql package.
// TODO(sqlexec): this can be moved back to internal after 20.2.
func GetAllNamesInternal(
