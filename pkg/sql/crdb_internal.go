...
`,
	populate: func(ctx context.Context, p *planner, _ *DatabaseDescriptor, addRow func(...tree.Datum) error) error {
		...
		rows, _ /* cols */, err := p.ExtendedEvalContext().ExecCfg.InternalExecutor.QueryWithSessionArgs(
			ctx, p.txn, SessionArgs{User: p.SessionData().User}, query)
		if err != nil {
			return err
		}
...
`,
	populate: func(ctx context.Context, p *planner, _ *DatabaseDescriptor, addRow func(...tree.Datum) error) error {
		...
		rows, _ /* cols */, err := p.ExtendedEvalContext().ExecCfg.InternalExecutor.Query(
			ctx, p.txn, `SELECT id, config FROM system.zones`)
		if err != nil {
			return err
		}
		for _, r := range rows {
