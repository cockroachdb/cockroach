	...
	indexes: []virtualIndex{
		{
			populate: func(ctx context.Context, unwrappedConstraint tree.Datum, p *planner, db catalog.DatabaseDescriptor, addRow func(...tree.Datum) error) (matched bool, err error) {
				...
				flags := p.ObjectLookupFlags(true, false)
				flags.IncludeDropped = true
				flags.IncludeOffline = true
				tab, err := p.Descriptors().GetImmutableTableByID(ctx, p.Txn(), id, flags)
				...
				if p.CheckAnyPrivilege(ctx, tab) != nil {
					return false, nil
				}

				// TODO(ajwerner): Look up the database and schema individually and
				// allow them to not exist.
				tn, err := p.GetQualifiedTableNameByID(ctx, int64(id), tree.ResolveAnyTableKind)
				if err != nil {
					return false, err
				}
				if err := makeCrdbInternalTablesAddRowFn(p, addRow)(
					tab, (*tree.DString)(&tn.CatalogName), string(tn.SchemaName),
				); err != nil {
					return false, err
				}
				return true, nil
...
func crdbInternalTablesDatabaseLookupFunc(
	...
	dbDescs, err := p.Descriptors().GetAllDescriptorsForDatabase(ctx, p.Txn(), db)
	if err != nil {
		return false, err
	}
	scNames := make(map[descpb.ID]string)
	...
	scNames[keys.PublicSchemaID] = catconstants.PublicSchemaName
	// Record database descriptors for name lookups.
	_ = dbDescs.ForEachDescriptorEntry(func(desc catalog.Descriptor) error {
		if scDesc, ok := desc.(catalog.SchemaDescriptor); ok {
			scNames[scDesc.GetID()] = scDesc.GetName()
		}
		return nil
	...
	var seenAny bool
	if err := dbDescs.ForEachDescriptorEntry(func(desc catalog.Descriptor) error {
		table, ok := desc.(catalog.TableDescriptor)
		if !ok || p.CheckAnyPrivilege(ctx, table) != nil {
			return nil
		}
