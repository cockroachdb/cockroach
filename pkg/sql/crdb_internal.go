...
`,
	generator: func(ctx context.Context, p *planner, _ *DatabaseDescriptor) (func() (tree.Datums, bool, error), error) {
		...
		return func() (tree.Datums, bool, error) {
			if i >= len(ranges) {
			...
			if err := p.txn.Run(ctx, b); err != nil {
				return nil, false, errors.Wrap(err, "error getting lease info")
			}
