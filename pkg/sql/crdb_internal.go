	...
	indexes: []virtualIndex{
		{
			populate: func(
				...
				default:
					return false, errors.AssertionFailedf(
						"unexpected type %T for type id column in virtual table",
						d,
					)
				}
				typeDesc, err := p.Descriptors().GetImmutableTypeByID(ctx, p.txn, id, tree.ObjectLookupFlags{})
				if err != nil {
					return false, err
				}
				if typeDesc == nil {
				...
				sc, err := p.Descriptors().GetImmutableSchemaByID(
					ctx,
					p.txn,
					typeDesc.GetParentSchemaID(),
					tree.SchemaLookupFlags{Required: true},
				)
				if err != nil {
					return false, err
				}
				return writeCreateTypeDescRow(db, sc.GetName(), typeDesc, addRow)
			},
		},
	},
}
