...
var crdbInternalTxInfoTable = virtualSchemaTable{
	comment: `transaction information (in-memory, not durable; local node only)`,
	schema: `
CREATE TABLE crdb_internal.node_txn_info (
  node_id               INT NOT NULL,
  application_name      STRING NOT NULL,
  txn_id                STRING NOT NULL,
  total_time_in_seconds FLOAT NOT NULL,
		...
		for _, appName := range appNames {
			...
			for _, txnID := range txnIDs {
				...
				err := addRow(
					nodeID,
					tree.NewDString(appName),
					tree.NewDString(txnID.String()),
					tree.NewDFloat(tree.DFloat(t.TotalTimeInSeconds)),
