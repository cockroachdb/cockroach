...
func addPartitioningRows(
	...
	for _, l := range partitioning.List {
		...
		for _, value := range l.Values {
			...
			if err := listValues.Append(tree.NewDString(string(tuple.String()))); err != nil {
				return err
			}
		}
		if err := addRow(
	...
	for _, r := range partitioning.Range {
		fromTuple, _, err := sqlbase.DecodePartitionTuple(
			a, table, index, partitioning, r.From, fakePrefixDatums)
		if err != nil {
			return err
		}
		toTuple, _, err := sqlbase.DecodePartitionTuple(
			a, table, index, partitioning, r.To, fakePrefixDatums)
		if err != nil {
			return err
		}
		if err := addRow(
			...
			tree.NewDString(string(fromTuple.String())),
			tree.NewDString(string(toTuple.String())),
		); err != nil {
			return err
		}
	}
