...
CREATE TABLE crdb_internal.node_statement_statistics (
  ...
  estimated_cost_avg  FLOAT NOT NULL,
  estimated_cost_var  FLOAT NOT NULL,
		...
		for _, appName := range appNames {
			...
			for _, stmtKey := range stmtKeys {
				...
				err := addRow(
					...
					tree.NewDFloat(tree.DFloat(s.mu.data.EstimatedCost.Mean)),
					tree.NewDFloat(tree.DFloat(s.mu.data.EstimatedCost.GetVariance(s.mu.data.Count))),
