	...
	populate: func(
		...
		return forEachTableDesc(ctx, p, dbContext, opts, func(
			...
			for _, fk := range table.OutboundForeignKeys() {
				...
				refConstraint, err := catalog.FindFKReferencedUniqueConstraint(refTbl, fk, true /* considerSubsets */)
				if err != nil {
					return err
				}
	...
	populate: func(ctx context.Context, p *planner, _ catalog.DatabaseDescriptor, addRow func(...tree.Datum) error) error {
		...
		for _, sample := range response.Samples {
			...
			if err := addRow(
				...
				tree.NewDString(sample.User),
				tree.NewDString(sample.Database),
				tree.NewDString(sample.SessionID),
				tree.NewDString(sample.TxnID),
				tree.NewDInt(tree.DInt(sample.PlanHash)),
