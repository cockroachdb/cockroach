...
CREATE TABLE crdb_internal.kv_catalog_descriptor (
  ...
  descriptor    JSON NOT NULL
	...
	populate: func(
		...
		all, err := p.Descriptors().GetAllCatalog(ctx, p.Txn())
		if err != nil {
			return err
		}
		// Delegate privilege check to system table.
		{
			sysTable := all.LookupDescriptorEntry(systemschema.DescriptorTable.GetID())
			if p.CheckAnyPrivilege(ctx, sysTable) != nil {
				return nil
			}
		}
		return all.ForEachDescriptorEntry(func(desc catalog.Descriptor) error {
...
CREATE TABLE crdb_internal.kv_catalog_zones (
  ...
  config    JSON NOT NULL
	...
	populate: func(
		...
		// Delegate privilege check to system table.
		{
			sysTable := all.LookupDescriptorEntry(systemschema.ZonesTable.GetID())
			if p.CheckAnyPrivilege(ctx, sysTable) != nil {
				return nil
			}
		}
		// Loop over all zone configs.
		return all.ForEachZoneConfigEntry(func(id descpb.ID, zc catalog.ZoneConfig) error {
...
CREATE TABLE crdb_internal.kv_catalog_namespace (
  ...
  id               INT NOT NULL
	...
	populate: func(
		...
		// Delegate privilege check to system table.
		{
			sysTable := all.LookupDescriptorEntry(systemschema.NamespaceTable.GetID())
			if p.CheckAnyPrivilege(ctx, sysTable) != nil {
				return nil
			}
		}
...
CREATE TABLE crdb_internal.kv_catalog_comments (
  ...
  comment     STRING NOT NULL
	...
	populate: func(
		...
		// Delegate privilege check to system table.
		{
			sysTable := all.LookupDescriptorEntry(systemschema.CommentsTable.GetID())
			if p.CheckAnyPrivilege(ctx, sysTable) != nil {
				return nil
			}
		}
		...
		for _, ct := range catalogkeys.AllCommentTypes {
			...
			if err := all.ForEachCommentEntry(func(key catalogkeys.CommentKey, cmt string) error {
