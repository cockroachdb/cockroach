...
CREATE TABLE crdb_internal.store_liveness_support_from (
  ...
  support_expiration    TIMESTAMP NOT NULL
	...
	populate: func(ctx context.Context, p *planner, _ catalog.DatabaseDescriptor, addRow func(...tree.Datum) error) error {
		...
		return populateStureLivenessSupportResponse(resp, addRow)
	},
}
...
func populateStureLivenessSupportResponse(
	...
	for _, ssps := range resp.SupportStatesPerStore {
		for _, ss := range ssps.SupportStates {
			if err := addRow(
				...
				&tree.DTimestamp{Time: timeutil.Unix(0, ss.Expiration.WallTime).Round(time.Microsecond)},
			); err != nil {
				return err
			}
		}
