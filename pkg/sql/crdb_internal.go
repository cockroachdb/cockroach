	...
	populate: func(
		...
		if err := all.ForEachComment(func(key catalogkeys.CommentKey, cmt string) error {
			...
			case catalogkeys.ConstraintCommentType:
				...
				schema, err := p.Descriptors().ByIDWithLeased(p.txn).Get().Schema(ctx, tableDesc.GetParentSchemaID())
				if err != nil {
					return err
				}
				db, err := p.Descriptors().ByIDWithLeased(p.txn).Get().Database(ctx, schema.GetParentID())
				if err != nil {
					return err
				}
