...
var useOldJobsVTable = settings.RegisterBoolSetting(
	...
	true, // TODO(dt): flip this once we add permissive auth checks.
)
...
func makeJobsTableRows(
	...
	if !v.AtLeast(clusterversion.V25_1_AddJobsTables.Version()) || useOldJobsVTable.Get(&p.EvalContext().Settings.SV) {
...
func makeJobBasedJobsTableRows(
	...
	query := `SELECT 
j.id, j.job_type, j.description, j.owner, j.status as state,
...
LEFT OUTER JOIN system.public.job_status AS s ON j.id = s.job_id  
	...
	it, err := p.InternalSQLTxn().QueryIteratorEx(
		ctx, "crdb-internal-jobs-tables", p.txn, sessiondata.InternalExecutorOverride{User: p.User()}, query, params...)
	if err != nil {
		return emitted, err
	}
	defer func() {
	...
	for {
		...
		if ok {
			...
			// TODO(dt): replace this with the more permissive jobsauth.Authorize that
			// that allows non-owner non-admins access to jobs from other users before
			// we can make this version on by default.
			if owner != p.User() && !isAdmin {
				continue
			}
		} else if !ok {
			if len(sessionJobs) == 0 {
			...
			id, typStr, desc, ownerStr, state, status, created, finished, modified, fraction, resolved, errorMsg, instanceID =
				...
				tree.NewDString(string(jobs.StatusRunning)),
				tree.DNull,
