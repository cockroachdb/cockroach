...
var crdbInternalJobsView = virtualSchemaView{
	...
	// as left joins for now.
	// NB: the `union`, needs to be `union all` to avoid materializing all of the
	// columns even when they're not used, since a non-`all` union has to prepared
	// to compare them all to eliminate duplicates, even when in the vast majority
	// of calls we expect the session jobs generator to be empty.
