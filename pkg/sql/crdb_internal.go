	...
	populate: func(
		...
		if err := forEachTableDescWithTableLookupInternalFromDescriptors(
			ctx, p, dbContext, hideVirtual, allowAdding, c, func(
				_ catalog.DatabaseDescriptor, sc catalog.SchemaDescriptor, descriptor catalog.TableDescriptor, lCtx tableLookupFn,
			) error {
				return doDescriptorValidationErrors(sc.GetName(), descriptor, lCtx)
			}); err != nil {
			return err
		}
