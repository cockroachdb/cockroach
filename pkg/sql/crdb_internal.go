...
func createRoutinePopulateByFnIndex(
	...
	if err != nil || len(fnDescs) == 0 {
		if errors.Is(err, catalog.ErrDescriptorNotFound) || len(fnDescs) == 0 {
			return false, nil
		}
		return false, err
	}
	...
	if err != nil || sc == nil {
		if errors.Is(err, catalog.ErrDescriptorNotFound) || sc == nil {
			return false, nil
		}
		return false, err
	}
	if scID != sc.GetID() {
		return false, errors.AssertionFailedf("schema ID %d does not match schema descriptor ID %d", scID, sc.GetID())
	}
	scName := sc.GetName()
	dbName := db.GetName()
	dbID := db.GetID()
