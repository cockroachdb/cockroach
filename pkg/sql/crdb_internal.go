  ...
  INDEX(status)
	...
	indexes: []virtualIndex{{
		populate: func(ctx context.Context, unwrappedConstraint tree.Datum, p *planner, _ catalog.DatabaseDescriptor, addRow func(...tree.Datum) error) (matched bool, err error) {
			...
			qIndexFilter := fmt.Sprintf(` WHERE status = %s`, unwrappedConstraint)
			q := qSelect + `, last_run, COALESCE(num_runs, 0), ` + jobs.NextRunClause + ` as next_run` + qFrom + ", " + backoffArgs + qIndexFilter

			gen, cleanup, err := makeVirtualTableGenerator(ctx, p, nil, nil, q)
			defer cleanup(ctx)
	...
	populate: func(ctx context.Context, p *planner, db catalog.DatabaseDescriptor, addRow func(...tree.Datum) error) error {
		...
		const (
			...
			qFrom       = ` FROM system.jobs`
			backoffArgs = `(SELECT $1::FLOAT AS initial_delay, $2::FLOAT AS max_delay) args`
			q           = qSelect + `, last_run, COALESCE(num_runs, 0), ` + jobs.NextRunClause + ` as next_run` + qFrom + ", " + backoffArgs
		)
	...
	//	)
	//
	//	return makeVirtualTableGenerator(ctx, p, nil, nil, q)
	//},
}

func makeVirtualTableGenerator(
	ctx context.Context, p *planner, _ catalog.DatabaseDescriptor, _ *stop.Stopper, query string,
	...
	it, err := p.ExtendedEvalContext().ExecCfg.InternalExecutor.QueryIteratorEx(
		...
		sessiondata.InternalExecutorOverride{User: username.RootUserName()},
		query, p.execCfg.JobRegistry.RetryInitialDelay(), p.execCfg.JobRegistry.RetryMaxDelay())
	if err != nil {
		return nil, nil, err
	}
