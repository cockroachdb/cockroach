		...
		worker := func(pusher rowPusher) error {
			...
			addDesc := func(table catalog.TableDescriptor, dbName tree.Datum, scName string) error {
				...
				row = append(row,
					...
					tree.TimestampToInexactDTimestamp(table.GetModificationTime().ToClockTimestampUnchecked()),
					tree.TimestampToDecimalDatum(table.GetModificationTime().ToClockTimestampUnchecked()),
