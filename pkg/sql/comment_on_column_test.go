...
func TestCommentOnColumn(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			`COMMENT ON COLUMN t.c IS 'foo'`,
			`SELECT col_description(attrelid, attnum) FROM pg_attribute WHERE attrelid = 't'::regclass AND attname = 'c'`,
		...
		},
		{
			...
			`SELECT col_description(attrelid, attnum) FROM pg_attribute WHERE attrelid = 't'::regclass AND attname = 'c2'`,
			gosql.NullString{String: `foo`, Valid: true},
		},
		{
			`COMMENT ON COLUMN t.c2 IS NULL`,
			`SELECT col_description(attrelid, attnum) FROM pg_attribute WHERE attrelid = 't'::regclass AND attname = 'c2'`,
