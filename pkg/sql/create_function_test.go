...
`,
	...
	err := sql.TestingDescsTxn(ctx, s, func(ctx context.Context, txn *kv.Txn, col *descs.Collection) error {
		funcDesc, err := col.MustGetImmutableFunctionByID(ctx, txn, 110)
...
`,
	...
	err := sql.TestingDescsTxn(ctx, s, func(ctx context.Context, txn *kv.Txn, col *descs.Collection) error {
		funcDesc, err := col.MustGetImmutableFunctionByID(ctx, txn, 112)
		require.NoError(t, err)
		require.Equal(t, funcDesc.GetName(), "f")
...
`)
	...
	err = sql.TestingDescsTxn(ctx, s, func(ctx context.Context, txn *kv.Txn, col *descs.Collection) error {
		funcDesc, err := col.MustGetImmutableFunctionByID(ctx, txn, 112, descs.WithoutLeased())
		require.NoError(t, err)
		require.Equal(t, funcDesc.GetName(), "f")
