...
$$`,
	...
	err := sql.TestingDescsTxn(ctx, s, func(ctx context.Context, txn *kv.Txn, col *descs.Collection) error {
		funcDesc, err := col.GetImmutableFunctionByID(ctx, txn, 108, tree.ObjectLookupFlagsWithRequired())
		...
		require.Equal(t,
			[]descpb.TableDescriptor_Reference{
				{ID: 108, ColumnIDs: []catid.ColumnID{1}},
				{ID: 108, IndexID: 2, ColumnIDs: []catid.ColumnID{2}},
				{ID: 108, IndexID: 3, ColumnIDs: []catid.ColumnID{3}},
			},
			tbl.GetDependedOnBy(),
		)
...
CREATE FUNCTION f() RETURNS t_implicit_type IMMUTABLE LANGUAGE SQL AS $$
	SELECT a, b FROM t_implicit_type
$$`,
	)
	...
	err := sql.TestingDescsTxn(ctx, s, func(ctx context.Context, txn *kv.Txn, col *descs.Collection) error {
		...
		require.Equal(t,
			[]descpb.TableDescriptor_Reference{{ID: 105, ColumnIDs: []catid.ColumnID{1, 2}}},
			tbl.GetDependedOnBy(),
		)
