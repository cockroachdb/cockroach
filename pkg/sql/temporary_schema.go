					...
					for _, col := range dTableDesc.Columns {
						if dependentColIDs.Contains(int(col.ID)) {
							_, err = execQuery(
								...
								txn,
								fmt.Sprintf(
									"ALTER TABLE %s.%s.%s ALTER COLUMN %s DROP DEFAULT",
									tree.NameString(db.Name),
									tree.NameString(schema),
									tree.NameString(dTableDesc.Name),
		...
		if len(toDelete.ids) > 0 {
			...
			for i, id := range toDelete.ids {
				...
				query.WriteString(
					fmt.Sprintf(
						" %s.%s.%s",
						tbName.CatalogName.String(),
						tbName.SchemaName.String(),
						tree.NameString(tbName.Table()),
					),
				)
			}
