...
func createTempSchema(params runParams, sKey sqlbase.DescriptorKey) (sqlbase.ID, error) {
	...
	params.p.sessionDataMutator.SetHasCreatedTemporarySchema(true)
	return id, nil
}
		...
		for _, id := range dbIDs {
			...
			if len(tbNames) > 0 {
				sd := &sessiondata.SessionData{
					...
					HasCreatedTemporarySchema: true,
			...
			// exist (eg. a temporary table was created and then dropped). So we
			// remove the namespace table entry of the temporary schema.
