...
func zoneSpecifierNotFoundError(zs parser.ZoneSpecifier) error {
	if zs.NamedZone != "" {
		return pgerror.NewErrorf(
			pgerror.CodeInvalidCatalogNameError, "range %q does not exist", zs.NamedZone)
	} else if zs.Database != "" {
...
func (n *setZoneConfigNode) Start(params runParams) error {
	...
	if err == errNoZoneConfigApplies {
		// TODO(benesch): Handling this case shouldn't be the caller's
		// responsibility.
...
func (n *showZoneConfigNode) Start(params runParams) error {
...

	{
		...
		//
		// TODO(benesch): this logic should live in the config package.
		zs := n.zoneSpecifier
		if zoneID == keys.RootNamespaceID {
			// We had to traverse to the top of the hierarchy, so we're showing the
			// default zone config.
			zs.NamedZone = config.DefaultZoneName
		} else if zoneID != uint32(targetID) {
			// We traversed one level up, so we're showing the database zone config.
