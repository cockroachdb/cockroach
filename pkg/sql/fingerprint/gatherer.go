...
func (g *gatherer) run(ctx context.Context) (uint64, error) {
	...
	g.mu.prog = make([]struct{ total, done int }, len(partitions))
...
func (g *gatherer) record(result spanResult) error {
	...
	g.mu.prog[result.idx].total, g.mu.prog[result.idx].done = result.total, result.done
...
func (g *gatherer) checkpoint(ctx context.Context) error {
	...
	return g.persister.store(ctx, g.mu.checkpointState, float64(overallDone)/float64(overallTotal))
}
...
func worker(
	...
	for _, sp := range todo {
		...
		default:
			...
			i++
			res <- spanResult{r, idx, len(todo), i}
		}
	}
	return nil
}
