...
func showCreateInterleave(
	...
	intl := idx.Interleave
	parentName, err := GetParentAsTableName(lCtx, intl, dbPrefix)
	if err != nil {
		return err
	}
...
// GetParentAsTableName a TreeTable object of the parent table for a given
// InterleaveDescriptor. Used to get the parent table of a table with
// interleaved indexes.
func GetParentAsTableName(
	l *internalLookupCtx, intl sqlbase.InterleaveDescriptor, dbPrefix string,
...
func GetTableAsTableName(
	l *internalLookupCtx, table *sqlbase.TableDescriptor, dbPrefix string,
) (tree.TableName, error) {
