...
func (p *planner) dependentRelationError(
	...
	case desc.IsView():
		dependentType = "view"
	default:
		return errors.AssertionFailedf("%q is an unexpected relation type: %s", desc.GetName(), desc.DescriptorType())
	}
	...
	if desc.GetParentID() != parentID {
		...
		if err != nil {
			...
			return sqlerrors.NewDependentObjectErrorf(
				...
				op, typeName, objName, dependentType)
		}
		viewName = viewFQName.FQString()
	}
	return sqlerrors.NewDependentBlocksOpError(op, typeName, objName, dependentType, viewName)
}
