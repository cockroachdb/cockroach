...
func (n *renameTableNode) startExec(params runParams) error {
	...
	if oldTn.Catalog() != newTn.Catalog() {
		// After 22.1 all public schemas will be backed by a descriptor and will be a regular UDS.
		// We do not support moving tables from a UDS to another database even if an UDS with the
		// same name in the new database exists.
