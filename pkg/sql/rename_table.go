...
func (n *renameTableNode) startExec(params runParams) error {
	...
	if !newTn.ExplicitSchema && !newTn.ExplicitCatalog {
		...
		notice := pgnotice.Newf("database qualified RENAME targets are deprecated")
		params.p.noticeSender.AppendNotice(notice)
	...
	if newTn.Catalog() == oldTn.Catalog() && newTn.Schema() != oldTn.Schema() {
		return pgerror.Newf(pgcode.InvalidName, "cannot change schema of table with RENAME")
	}
