...
func (s *schemaChangePlanNode) startExec(params runParams) error {
	...
	// Our current state does not match what we planned, for this means that
	// potentially we are running CTE's with ALTER statements. So, we are going
	// to replan the state to include the current statement, since the statement
