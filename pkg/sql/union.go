...
func (n *unionNode) readRight(params runParams) (bool, error) {
	...
	for ; next; next, err = n.right.Next(params) {
		...
		if n.run.scratch, err = sqlbase.EncodeDatumsKeyAscending(n.run.scratch, n.right.Values()); err != nil {
			return false, err
		}
...
func (n *unionNode) readLeft(params runParams) (bool, error) {
	...
	for ; next; next, err = n.left.Next(params) {
		...
		n.run.scratch = n.run.scratch[:0]
		if n.run.scratch, err = sqlbase.EncodeDatumsKeyAscending(n.run.scratch, n.left.Values()); err != nil {
			return false, err
		}
