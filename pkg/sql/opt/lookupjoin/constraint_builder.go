...
func (b *ConstraintBuilder) Build(
	...
	var filterOrdsToRemove util.FastIntSet
	...
	for j := 0; j < numIndexKeyCols; j++ {
		...
		if eqIdx, ok := rightEq.Find(idxCol); ok {
			...
			filterOrdsToRemove.Add(eqFilterOrds[eqIdx])
			continue
		}
	...
	if shouldBuildMultiSpanLookupJoin {
		...
		var multiSpanFilterOrdsToRemove util.FastIntSet
		eqFilters, constFilters, rightSideCols, multiSpanFilterOrdsToRemove =
		...
		filterOrdsToRemove.UnionWith(multiSpanFilterOrdsToRemove)
	...
	for i := range onFilters {
		if !filterOrdsToRemove.Contains(i) {
...
func (b *ConstraintBuilder) findFiltersForIndexLookup(
	...
	rightSideCols opt.ColList,
	filterOrdsToRemove util.FastIntSet,
) {
	numIndexKeyCols := index.LaxKeyColumnCount()
	...
	for j := 0; j < numIndexKeyCols; j++ {
		...
		if eqIdx, ok := rightEq.Find(idxCol); ok {
			...
			rightSideCols = append(rightSideCols, idxCol)
			filterOrdsToRemove.Add(eqFiltersOrds[eqIdx])
			continue
		}
	...
	return eqFilters, constFilters, rightSideCols, filterOrdsToRemove
}
