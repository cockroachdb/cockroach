...
func (b *ConstraintBuilder) Build(
...
) (cons Constraint, foundEqualityCols bool) {
	...
	_, rightCmp, inEqFilterOrds := memo.ExtractJoinConditionColumns(
	...
	for j := 0; j < numIndexKeyCols; j++ {
		...
		startIdx, endIdx, foundStart, foundEnd := b.findJoinVariableRangeFilters(
			rightCmp, inEqFilterOrds, allFilters, idxCol, idxColIsDesc,
		)
		if foundStart {
			convertToLookupExpr()
...
func (b *ConstraintBuilder) findJoinVariableRangeFilters(
	...
	for i := range rightCmp {
		...
		if idxColIsDesc && op == opt.LtOp {
			...
			// only possible for a subset of types.
			typ := cond.Child(0).(*memo.VariableExpr).Typ
			switch typ.Family() {
			case types.IntFamily, types.OidFamily, types.UuidFamily, types.BoolFamily:
			default:
				continue
			}
		}
		...
		if !foundStart && isStartBound {
			foundStart = true
			startIdx = inEqFilterOrds[i]
		} else if !foundEnd && !isStartBound {
			foundEnd = true
			endIdx = inEqFilterOrds[i]
		}
	}
	return
}
...
func (b *ConstraintBuilder) findJoinConstantRangeFilter(
	...
	for i := range filters {
		...
		if props.TightConstraints && props.Constraints.Length() > 0 {
			...
			// should be normalized so that the column is ascending, so we can ignore
			// the descending case.
			...
			if needEnd && !span.EndKey().IsEmpty() && !span.EndKey().IsNull() {
				...
				if span.EndBoundary() == constraint.ExcludeBoundary && idxColIsDesc {
					...
					// "advance" the start boundary. In the case when the index column is
					// DESC, this is not trivial, so the Datum.Prev method is necessary.
			...
			if rangeExpr, ok := filters[i].Condition.(*memo.RangeExpr); ok && (!needStart || !needEnd) {
				// We need to split the range filter because only half of it is needed.
				and := rangeExpr.And.(*memo.AndExpr)
				leftItem := b.f.ConstructFiltersItem(and.Left)
				rightItem := b.f.ConstructFiltersItem(and.Right)
				if !needStart {
					return &rightItem, &leftItem, i
				}
				if !needEnd {
					return &leftItem, &rightItem, i
				}
			}
			return &filters[i], nil, i
		}
	}
	return nil, nil, -1
}
