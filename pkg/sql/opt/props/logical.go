...
type Relational struct {
	...
	Rule struct {
		...
		// an unconstrained, unlimited Scan operator can add its first column to
		// this property, but a Select operator cannot pass through any columns, as
		// it may have filtered rows. A project operator can pass through
		// UnfilteredColumns because while it may remove columns from its output, it
