...
func (d *Distribution) FromIndexScan(
	...
	if len(regions) == 0 {
		...
		if !regionsPopulated {
			if homeRegion, ok := tab.HomeRegion(); ok {
				regions = make(map[string]struct{})
				regions[homeRegion] = struct{}{}
			} else {
				// Use the leaseholder region(s).
...
// GetRegionOfDistribution returns the single region name of the distribution,
// if there is exactly one.
func (d *Distribution) GetRegionOfDistribution() string {
	if d == nil {
		return ""
	}
	if len(d.Regions) == 1 {
