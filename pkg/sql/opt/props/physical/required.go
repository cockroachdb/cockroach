...
type Required struct {
	...
	// remote branch is not executed. Note that, unlike LimitHint, RemoteBranch
	// does not propagate down the entire subtree, only to the topmost remote
	// expression. This is because it is used to scale down the cost of the
	// remote branch, which should happen once, not at every level of the tree.
	RemoteBranch bool
}
