...
func (oc *OrderingChoice) CanSimplify(fdset *FuncDepSet) bool {
	...
	if !fdset.IsClosureEqual(oc.Optional) {
		return true
	}

	closure := fdset.ComputeClosure(oc.Optional)
	for i := range oc.Columns {
		group := &oc.Columns[i]

		// If group contains an optional column, then group can be simplified
		// or removed entirely.
		if group.Group.Intersects(closure) {
			return true
		}
		...
		if !fdset.IsEquivClosureEqual(group.Group) {
			return true
		}
...
func (oc *OrderingChoice) Simplify(fdset *FuncDepSet) {
	...
	for i := range oc.Columns {
		...
		// ComputeClosureNoCopy is safe here because of the implicit copy in
		// the Union.
		closure = fdset.ComputeClosureNoCopy(closure)
