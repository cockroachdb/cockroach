...
func (oc *OrderingChoice) Implies(other *OrderingChoice) bool {
	...
	for left, right := 0, 0; right < len(other.Columns); {
		...
		case leftCol.Descending == rightCol.Descending && leftCol.Group.SubsetOf(rightCol.Group):
			// The columns match.
			optional = optional.unionWith(rightCol.Group)
...
func (oc *OrderingChoice) Intersects(other *OrderingChoice) bool {
	...
	for left, right := 0, 0; left < len(oc.Columns) && right < len(other.Columns); {
		...
		case leftCol.Descending == rightCol.Descending && leftCol.Group.Intersects(rightCol.Group):
			// The columns match.
			leftOptional = leftOptional.unionWith(leftCol.Group)
			rightOptional = rightOptional.unionWith(rightCol.Group)
			left, right = left+1, right+1
...
func (oc OrderingChoice) PrefixIntersection(
	...
	for {
		...
		case len(oc.Columns) > 0 && prefixHelper.intersects(oc.Columns[0].Group):
			...
			prefixHelper = prefixHelper.differenceWith(oc.Columns[0].Group)
			oc.Columns = oc.Columns[1:]
		default:
...
// LongestCommonPrefix returns the index of the OrderingChoice within the
// receiver that has the longest common prefix with the given OrderingChoice, as
// well as the length of the prefix. It also returns a boolean indicating
// whether the common prefix derived from the returned index will imply the
// 'other' OrderingChoice. LongestCommonPrefix returns early when an
// OrderingChoice is found which forms a common prefix that implies the given
// OrderingChoice.
func (os OrderingSet) LongestCommonPrefix(other *OrderingChoice) (idx, length int, implies bool) {
	...
	return bestPrefixIdx, bestPrefixLength, implies
}
...
// copies it now.
func (h colSetHelper) unionWith(other opt.ColSet) colSetHelper {
	if !h.copied {
...
// unionWith returns a colSetHelper reflecting a DifferenceWith operation
// performed on the receiver ColSet. If the original ColSet has not been copied,
// unionWith copies it now.
func (h colSetHelper) differenceWith(other opt.ColSet) colSetHelper {
	if !h.copied {
		h.colSet = h.colSet.Copy()
	}
