...
func TestEquivSet_Rand(t *testing.T) {
	...
	areColsEquivOracle := func(left, right opt.ColumnID) bool {
		leftGroup := testOracle[left]
		rightGroup := testOracle[right]
		rightContains := rightGroup.Contains(left)
		_ = rightContains
		return leftGroup.Contains(right)
	}
	...
	makeError := func(expected bool) string {
		if expected {
			return fmt.Sprintf("expected columns to be equivalent, but weren't")
		}
		return fmt.Sprintf("expected columns not to be equivalent, but were")
	}
