...
func getRangesBeforeAndAfter(
	...
	switch bucketLowerBound.ResolvedType().Family() {
	case types.IntFamily:
		rangeBefore = float64(*bucketUpperBound.(*tree.DInt)) - float64(*bucketLowerBound.(*tree.DInt))
	...
	case types.DateFamily:
		...
		upperBefore := bucketUpperBound.(*tree.DDate)
	...
	case types.DecimalFamily:
		...
		upperBefore, err := bucketUpperBound.(*tree.DDecimal).Float64()
		if err != nil {
			return 0, 0, false
		}
	...
	case types.FloatFamily:
		rangeBefore = float64(*bucketUpperBound.(*tree.DFloat)) - float64(*bucketLowerBound.(*tree.DFloat))
	...
	case types.TimestampFamily:
		...
		upperBefore := bucketUpperBound.(*tree.DTimestamp).Time
	...
	case types.TimestampTZFamily:
		...
		upperBefore := bucketUpperBound.(*tree.DTimestampTZ).Time
	...
	case types.TimeFamily:
		lowerBefore := bucketLowerBound.(*tree.DTime)
		upperBefore := bucketUpperBound.(*tree.DTime)
		lowerAfter := spanLowerBound.(*tree.DTime)
		upperAfter := spanUpperBound.(*tree.DTime)
		rangeBefore = float64(*upperBefore) - float64(*lowerBefore)
		rangeAfter = float64(*upperAfter) - float64(*lowerAfter)
		return rangeBefore, rangeAfter, true

	case types.TimeTZFamily:
		lowerBefore := bucketLowerBound.(*tree.DTimeTZ).TimeOfDay
		upperBefore := bucketUpperBound.(*tree.DTimeTZ).TimeOfDay
		lowerAfter := spanLowerBound.(*tree.DTimeTZ).TimeOfDay
		upperAfter := spanUpperBound.(*tree.DTimeTZ).TimeOfDay
		rangeBefore = float64(upperBefore) - float64(lowerBefore)
		rangeAfter = float64(upperAfter) - float64(lowerAfter)
		return rangeBefore, rangeAfter, true
