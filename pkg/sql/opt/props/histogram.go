...
// Returns the index of the matching column in the constraint if found.
func (h *Histogram) CanFilter(c *constraint.Constraint) (colIdx int, ok bool) {
...
func (h *Histogram) Filter(c *constraint.Constraint) *Histogram {
	...
	prefix := make([]tree.Datum, colIdx)
	for i := range prefix {
		prefix[i] = c.Spans.Get(0).StartKey().Value(i)
	}
	desc := c.Columns.Get(colIdx).Descending()
	...
	for spanIndex < spanCount {
		...
		if filteredSpan.Compare(&keyCtx, &left) != 0 {
			...
			// bucket.
			filteredBucket = getFilteredBucket(&iter, &keyCtx, &filteredSpan, colIdx)
			if !desc && filteredSpan.CompareStarts(&keyCtx, &left) != 0 {
				// We need to add an empty bucket before the new bucket.
				ub := h.getPrevUpperBound(filteredSpan.StartKey(), filteredSpan.StartBoundary(), colIdx)
		...
		if desc && filteredSpan.CompareEnds(&keyCtx, &left) != 0 {
			...
			ub := h.getPrevUpperBound(filteredSpan.EndKey(), filteredSpan.EndBoundary(), colIdx)
			filtered.addEmptyBucket(ub, desc)
		}
...
func (h *Histogram) getPrevUpperBound(
	currentLowerBound constraint.Key, boundary constraint.SpanBoundary, colIdx int,
) tree.Datum {
	prevUpperBound := currentLowerBound.Value(colIdx)
...
func getFilteredBucket(
	iter *histogramIter, keyCtx *constraint.KeyContext, filteredSpan *constraint.Span, colIdx int,
) *cat.HistogramBucket {
	spanLowerBound := filteredSpan.StartKey().Value(colIdx)
	spanUpperBound := filteredSpan.EndKey().Value(colIdx)
