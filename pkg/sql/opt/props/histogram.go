...
func (h *Histogram) join(
	...
	leftBucket := sb.makeSpanFromBucket(ctx, &leftIter)
	rightBucket := sb.makeSpanFromBucket(ctx, &rightIter)
	...
	bucIndex := sort.Search(leftCount, func(i int) bool {
		leftIter.setIdx(i)
		bucket := sb.makeSpanFromBucket(ctx, &leftIter)
	...
	for ok := true; ok; ok = leftIter.next() {
		...
		// constraint library.
		left := sb.makeSpanFromBucket(ctx, &leftIter)
		right := sb.makeSpanFromBucket(ctx, &rightIter)
