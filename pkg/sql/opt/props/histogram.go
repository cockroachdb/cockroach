...
func (h *Histogram) numEq(i int) float64 {
	return h.buckets[i].NumEq * h.selectivity.AsFloat()
}
...
func (h *Histogram) numRange(i int) float64 {
	return h.buckets[i].NumRange * h.selectivity.AsFloat()
}
...
func (h *Histogram) distinctRange(i int) float64 {
	...
	// d << n.
	return d - d*math.Pow(1-h.selectivity.AsFloat(), n/d)
}
...
func (h *Histogram) ApplySelectivity(selectivity Selectivity) *Histogram {
	...
	h2 := *h
	h2.selectivity.Multiply(selectivity)
	return &h2
}
...
func (w *histogramWriter) init(selectivity Selectivity, buckets []cat.HistogramBucket) {
	// This initialization pattern ensures that fields are not unwittingly
	// reused. Field reuse must be explicit.
	*w = histogramWriter{
		cells: [][]string{
	...
	for i, b := range buckets {
		w.cells[counts][i*2] = fmt.Sprintf(" %.5g ", b.NumRange*selectivity.AsFloat())
		w.cells[counts][i*2+1] = fmt.Sprintf("%.5g", b.NumEq*selectivity.AsFloat())
