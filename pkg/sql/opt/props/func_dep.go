...
func (f *FuncDepSet) Verify() {
	for i := range f.deps {
		...
		if fd.from.Intersects(fd.to) {
			panic(pgerror.NewAssertionErrorf("expected FD determinant and dependants to be disjoint: %s (%d)", f, i))
		}

		if fd.strict && fd.from.Empty() {
			if i != 0 {
				panic(pgerror.NewAssertionErrorf("expected strict constant FD to be first FD in set: %s (%d)", f, i))
			}
		}
		...
		if fd.equiv {
			...
			if fd.from.Len() != 1 {
				panic(pgerror.NewAssertionErrorf("expected equivalence determinant to be single col: %s (%d)", f, i))
			}

			if !f.ComputeEquivClosure(fd.from).Equals(fd.from.Union(fd.to)) {
				panic(pgerror.NewAssertionErrorf("expected equivalence dependants to be its closure: %s (%d)", f, i))
			}
		}
	}
	...
	if f.hasKey != noKey {
		...
		if f.hasKey == strictKey {
			...
			if !f.ComputeClosure(f.key).Equals(allCols) {
				panic(pgerror.NewAssertionErrorf("expected closure of FD key to include all known cols: %s", f))
			}
		}
	} else {
		if !f.key.Empty() {
