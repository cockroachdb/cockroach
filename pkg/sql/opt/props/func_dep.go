...
func (f *FuncDepSet) AddLaxKey(keyCols, allCols opt.ColSet) {
	...
	if keyCols.Empty() {
		panic(pgerror.NewAssertionErrorf("lax key cannot be empty"))
	}
...
func (f *FuncDepSet) Verify() {
	for i := range f.deps {
		...
		if fd.isConstant() {
			if i != 0 {
				panic(pgerror.NewAssertionErrorf("expected constant FD to be first FD in set: %s (%d)", log.Safe(f), log.Safe(i)))
			}
			if !fd.strict {
				panic(pgerror.NewAssertionErrorf("expected constant FD to be strict: %s", log.Safe(f)))
			}
		}

		if fd.equiv {
			if !fd.strict {
				panic(pgerror.NewAssertionErrorf("expected equivalency to be strict: %s (%d)", f, i))
			}
	...
	if f.hasKey != noKey {
		if f.hasKey == strictKey {
			if reduced := f.ReduceCols(f.key); !reduced.Equals(f.key) {
				panic(pgerror.NewAssertionErrorf("expected FD to have candidate key %s: %s", reduced, f))
			}
