...
func (f *FuncDepSet) MakeOuter(nullExtendedCols, notNullCols opt.ColSet) {
	var newFDs []funcDep
	var laxConstCols, allCols opt.ColSet
	...
	for i := range f.deps {
		...
		if fd.from.Empty() {
			...
			//    "d" value now maps to both 2 and NULL. So it must be weakened to
			...
			// Add any constant columns becoming lax to non-equivalence dependants.
			// Otherwise, once they become lax, they will no longer be part of these
			// other FD closures.
			if !fd.equiv && !laxConstCols.Empty() && !laxConstCols.SubsetOf(fd.to) {
				fd.to = fd.to.Union(laxConstCols)
				fd.to.DifferenceWith(fd.from)
			}
		}
