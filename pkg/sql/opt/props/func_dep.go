...
func (f *FuncDepSet) RemapFrom(fdset *FuncDepSet, fromCols, toCols opt.ColList) {
	...
	f.equiv.translateCols(fromCols, toCols)
...
func (f *FuncDepSet) AddFrom(fdset *FuncDepSet) {
	for i := 0; i < fdset.equiv.count(); i++ {
		// No copy necessary, since the ColSet of an equiv group is immutable.
		f.addEquivalency(fdset.equiv.get(i))
	}
...
func (f *FuncDepSet) MakeProduct(inner *FuncDepSet) {
	f.equiv.appendFrom(&inner.equiv)
...
func (f *FuncDepSet) MakeApply(inner *FuncDepSet) {
	for i := 0; i < inner.equiv.count(); i++ {
		// No copy necessary, since the ColSet of an equiv group is immutable.
		f.addEquivalency(inner.equiv.get(i))
	}
	for i := range inner.deps {
		fd := &inner.deps[i]
...
func (f *FuncDepSet) nullExtendRightRows(rightCols, notNullInputCols opt.ColSet) {
	...
	// Partition the groups so that they only reference one side of the join.
	f.equiv.makePartition(rightCols)
...
func (f *FuncDepSet) addEquivalency(equiv opt.ColSet) {
	f.equiv.addNoCopy(equiv)
	for i := 0; i < f.equiv.count(); i++ {
		if f.equiv.get(i).Intersects(equiv) {
			// This is the group that contains the equivalence closure.
			equiv = f.equiv.get(i)
			break
		}
	}
