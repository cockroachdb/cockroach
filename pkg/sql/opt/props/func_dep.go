...
func (f *FuncDepSet) CopyAndRemap(colMap opt.ColMap) FuncDepSet {
	...
	newFuncDepSet.deps = make([]funcDep, 0, len(f.deps))
	for i := range f.deps {
		newFuncDepSet.deps[i].from = f.deps[i].from.Remap(colMap)
		newFuncDepSet.deps[i].to = f.deps[i].to.Remap(colMap)
		newFuncDepSet.deps[i].strict = f.deps[i].strict
		newFuncDepSet.deps[i].equiv = f.deps[i].equiv
	}
	newFuncDepSet.key = f.key.Remap(colMap)
