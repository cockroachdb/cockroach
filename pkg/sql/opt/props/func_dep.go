...
// key, it becomes a lax key (unless it was the empty key).
...
func (f *FuncDepSet) AddLaxKey(keyCols, allCols opt.ColSet) {
	...
	// Ensure we have candidate key (i.e. has no columns that are functionally
	// determined by other columns).
	f.addDependency(keyCols, allCols, false, false /* equiv */)
...
func (f *FuncDepSet) MakeNotNull(notNullCols opt.ColSet) {
	...
	// Try to reduce the key based on notNullCols and any new strict FDs.
	if f.hasKey != noKey {
		...
		if f.hasKey == strictKey {
			...
			// not null; add back other columns.
			...
			if nullableKeyCols.Empty() {
				...
				// Collect the list of all columns.
				allCols := f.key.Copy()
				for i := range f.deps {
					allCols.UnionWith(f.deps[i].from)
					allCols.UnionWith(f.deps[i].to)
				}
				f.AddStrictKey(reduced, allCols)
			} else {
...
func (f *FuncDepSet) colsAreKey(cols opt.ColSet, typ keyType) bool {
	...
	case laxKey:
		...
		//   - (a,b) is the known weak key.
		// The strict closure of (b) is the lax key (a,b), but (b) is not a weak
		// key.
		//
...
func (f *FuncDepSet) tryToReduceKey() {
	// Without nullable column information, we can only reduce strict keys.
	if f.hasKey == strictKey {
