...
func (s *Statistics) Init(relProps *Relational) (zeroCardinality bool) {
	...
	*s = Statistics{}
	if relProps.Cardinality.IsZero() {
		s.RowCount = 0
		s.Selectivity = MakeSelectivity(0)
		s.Available = true
		return true
	}
	s.Selectivity = MakeSelectivity(1)
	return false
}
...
func (s *Statistics) ApplySelectivity(selectivity Selectivity) {
	s.RowCount *= selectivity.selectivity
	s.Selectivity.Multiply(selectivity)
}
// ApplyInverseSelectivity unapplies a given selectivity to the statistics.
// RowCount and Selectivity are updated. Note that DistinctCounts, NullCounts,
// and Histograms are not updated.
func (s *Statistics) ApplyInverseSelectivity(selectivity Selectivity) {
	inverseSelectivity := 1.0 / selectivity.selectivity
	s.RowCount *= inverseSelectivity
	s.Selectivity.selectivity *= inverseSelectivity
}
...
func (c *ColumnStatistic) ApplySelectivity(selectivity Selectivity, inputRows float64) {
	...
	c.NullCount *= selectivity.selectivity

	if c.Histogram != nil {
		c.Histogram = c.Histogram.ApplySelectivity(selectivity)
	}

	if selectivity.selectivity == 1 || c.DistinctCount == 0 {
		return
	}
	if selectivity.selectivity == 0 {
		c.DistinctCount = 0
		return
	}
	...
	// when d << n.
	c.DistinctCount = d - d*math.Pow(1-selectivity.selectivity, n/d)
	const epsilon = 1e-10
