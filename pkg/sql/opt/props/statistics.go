...
func (s *Statistics) ApplySelectivity(selectivity float64) {
	...
	// and that NullCount doesn't overlap with DistinctCount and violate
	...
	for i, n := 0, s.ColStats.Count(); i < n; i++ {
		...
		if (colStat.DistinctCount + colStat.NullCount - 1) > s.RowCount {
			colStat.NullCount = s.RowCount - colStat.DistinctCount + 1
		}
	}
}
...
type ColumnStatistic struct {
	...
	// columns for this expression. Note that this count tracks the "all
	// columns are null" case, not the "one of the columns has a null value"
	// case.
	NullCount float64
}
