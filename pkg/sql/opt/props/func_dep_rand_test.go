...
// notNullCols returns the set columns that have at least one null.
...
// specifically:
//  - an inner join if both leftOuter and rightOuter are empty;
//  - a left/right outer join if one of them is empty;
//  - a full outer join if neither is empty.
