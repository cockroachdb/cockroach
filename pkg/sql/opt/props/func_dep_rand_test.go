...
func joinTestRelations(
	...
	for filters != nil && filters.parent != nil {
		res = filters.op.FilterRelation(res)
	...
	remainingLeftRows := map[rowKey]struct{}{}
	remainingRightRows := map[rowKey]struct{}{}
	for _, row := range res {
		leftKey, _ := row.key(leftCols)
		rightKey, _ := row.key(shiftSet(rightCols, numLeftCols))
		if _, ok := remainingLeftRows[leftKey]; !ok {
			remainingLeftRows[leftKey] = struct{}{}
		}
		if _, ok := remainingRightRows[rightKey]; !ok {
			remainingRightRows[rightKey] = struct{}{}
		}
	}
	...
	if leftOuter {
		for _, row := range left {
			...
			if _, ok := remainingLeftRows[key]; !ok {
				add(row, nil)
			}
		}
	}
	...
	if rightOuter {
		for _, row := range right {
			...
			if _, ok := remainingRightRows[key]; !ok {
				add(nil, row)
			}
		}
	}
...
func newTestState(cfg *testConfig, makeRelation bool) *testState {
...
func TestFuncDepOpsRandom(t *testing.T) {
	...
	filterConfigs := []testParams{
...

		{
			...
			ops: []testOpGenerator{
				...
				genAddConst(3 /* minCols */, 6 /* maxCols */),
				genAddEquiv(),
			},
		},

		{
			...
			ops: []testOpGenerator{
				...
				genAddConst(2 /* minCols */, 7 /* maxCols */),
				genAddEquiv(),
			},
		},
	}
	...
	setupConfigs := func(configs []testParams, makeRelation bool) {
		for _, cfg := range configs {
			for r := 0; r < repeats; r++ {
				...
				run(newTestState(&cfg.testConfig, makeRelation), 1 /* depth */)
			}
		}
	}

	setupConfigs(testConfigs, false /* makeRelation */)
	setupConfigs(filterConfigs, false /* makeRelation */)
	...
	for r := 0; r < repeats; r++ {
		...
		genRandChain := func() *testState {
			...
			state := newTestState(&cfg.testConfig, true /* makeRelation */)
		...
		// Generate filter op chain.
		cfg := filterConfigs[filterIndex[nLeft+nRight]]
		state := newTestState(&cfg.testConfig, false /* makeRelation */)
		steps := 1 + rand.Intn(cfg.maxDepth)
		for i := 0; i < steps; i++ {
			opGenIdx := rand.Intn(len(cfg.ops))
			op := cfg.ops[opGenIdx](&cfg.testConfig)
		...
		if err := join.checkFDs(&fd); err != nil {
			t.Fatalf(
				"MakeProduct and AddFrom returned incorrect FDs\n"+
					...
					"filter steps: %s",
				&left.fds, &rightFDs, &filters.fds, &fd, err, left, nLeft, right, filters,
			)
		}
		...
		fd.CopyFrom(&left.fds)
		fd.MakeProduct(&rightFDs)
		fd.MakeLeftOuter(&left.fds, &filters.fds, leftCols, rightCols, notNullInputCols)
		if err := join.checkFDs(&fd); err != nil {
			t.Fatalf(
