...
func (tr testRow) key(cols opt.ColSet) (_ rowKey, hasNulls bool) {
	if cols.Len() > 16 {
		panic(pgerror.NewAssertionErrorf("max 16 columns supported"))
	}
	var key rowKey
	cols.ForEach(func(c int) {
		val := tr.value(opt.ColumnID(c))
		if val == null {
			hasNulls = true
		}
		if val > 15 {
			panic(pgerror.NewAssertionErrorf("testVal must be <= 15"))
		}
...
func (tr testRow) hasNulls(cols opt.ColSet) bool {
	var res bool
	cols.ForEach(func(c int) {
		res = res || tr.value(opt.ColumnID(c)) == null
	})
	return res
}
...
func (tr testRow) equalOn(other testRow, cols opt.ColSet) bool {
	eq := true
	cols.ForEach(func(c int) {
		if tr.value(opt.ColumnID(c)) != other.value(opt.ColumnID(c)) {
			eq = false
		}
	})
	return eq
...
func (tr testRelation) checkFD(dep funcDep) error {
	if dep.equiv {
		...
		for _, r := range tr {
			c, _ := dep.from.Next(0)
			val := r.value(opt.ColumnID(c))
			fail := false
			dep.to.ForEach(func(col int) {
				if r.value(opt.ColumnID(col)) != val {
					fail = true
				}
			})
			if fail {
...
func (tc *testConfig) randColSet(minLen, maxLen int) opt.ColSet {
	if maxLen > tc.numCols {
		panic(pgerror.NewAssertionErrorf("maxLen > numCols"))
	}
	...
	for j := tc.numCols - length; j < tc.numCols; j++ {
		if t := rand.Intn(j + 1); !res.Contains(t + 1) {
			res.Add(t + 1)
		} else {
			res.Add(j + 1)
		}
...
func (ts *testState) child(t *testing.T, op testOp) *testState {
	...
	err := func() (err error) {
		defer func() {
			if r := recover(); r != nil {
				err = pgerror.NewAssertionErrorf("%v", r)
			}
		}()
