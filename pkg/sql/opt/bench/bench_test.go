...
func (bm *benchmark) runUsingAPI(b *testing.B, bmType benchmarkType, query string) {
	...
	cat := testcat.New()
	cat.ExecuteDDL(`CREATE TABLE kv (k BIGINT NOT NULL PRIMARY KEY, v BYTES NOT NULL)`)
	cat.ExecuteDDL(`CREATE TABLE abc (a INT PRIMARY KEY, b INT, c INT, INDEX(b), UNIQUE INDEX(c))`)
