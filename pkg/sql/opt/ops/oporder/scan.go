...
func scanCanProvideOrdering(expr memo.RelExpr, required *props.OrderingChoice) bool {
	ok, _ := ScanPrivateCanProvideOrdering(
		expr.Memo().Metadata(),
...
func ScanIsReverse(scan *memo.ScanExpr, required *props.OrderingChoice) bool {
	ok, reverse := ScanPrivateCanProvideOrdering(
		scan.Memo().Metadata(),
		&scan.ScanPrivate,
		required,
	)
...
// ScanPrivateCanProvideOrdering returns true if the scan operator returns rows
...
func ScanPrivateCanProvideOrdering(
...
func init() {
	memo.ScanIsReverseFn = func(
		...
		ok, reverse := ScanPrivateCanProvideOrdering(md, s, required)
		if !ok {
			panic("scan can't provide required ordering")
		}
		return reverse
