...
func joinGeoIndexFromExpr(
	...
	return makeExprFromRelationshipAndParams(factory, expr, args, commuteArgs, rel, false)
}
...
func constrainGeoIndexFromExpr(
	...
	// forceConstruct is set to true since the pre-filterer only works with
	// functions in the RelationshipReverseMap.
	preFilterExpr :=
		makeExprFromRelationshipAndParams(factory, expr, args, commuteArgs, relationship, true)
...
// the desired function expression, unless forceContruct is true.
func makeExprFromRelationshipAndParams(
	...
	_, ok := expr.(*memo.FunctionExpr)
	if !ok || forceConstruct {
		// This expression was one of the bounding box comparison operators, or
		// forceConstruct was true. Construct a function that represents the same
