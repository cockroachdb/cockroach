...
func GetGeoIndexRelationship(expr opt.ScalarExpr) (_ geoindex.RelationshipType, ok bool) {
	...
	if _, ok := expr.(*memo.RegMatchExpr); ok {
...
func joinGeoIndex(
	...
	case *memo.OverlapsExpr, *memo.RegMatchExpr:
		...
		// If they are another type besides Geometry or Box2d, this operator
		// cannot be used to create a geospatial join condition.
		for i := 0; i < len(args); i++ {
			if args[i].DataType().Family() == types.Box2DFamily {
				args[i] = factory.ConstructCast(args[i], types.Geometry)
			} else if args[i].DataType().Family() != types.GeometryFamily {
				return nil
			}
		}
...
func constrainGeoIndex(
	...
	case *memo.OverlapsExpr, *memo.RegMatchExpr:
		...
		// Cast the arguments to type Geometry if they are type Box2d.
		// If they are another type besides Geometry or Box2d, this operator
		// cannot be used to constrain a geospatial index.
		for i := 0; i < len(args); i++ {
			if args[i].DataType().Family() == types.Box2DFamily {
				args[i] = factory.ConstructCast(args[i], types.Geometry)
			} else if args[i].DataType().Family() != types.GeometryFamily {
