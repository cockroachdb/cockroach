...
func TryGetInvertedJoinCondFromGeoFunc(
	...
	if commuteArgs {
		...
		commutedRel := commuteGeoRelationshipMap[rel]
		...
		if rel != commutedRel {
			...
			props, overload := findGeoFunction(name, args)
		...
		fn = factory.ConstructFunction(args, private)
	}

	return fn, inputGeoCol, indexGeoCol, true
}

// findGeoFunction returns the function properties and overload of the
// geospatial function with the given name and argument types matching the
// given inputs. If no overload is found, findGeoFunction panics.
func findGeoFunction(
	name string, args memo.ScalarListExpr,
	...
	panic(errors.AssertionFailedf("could not find geospatial overload for %s", name))
}
...
func constrainGeoIndexFromFunction(
	...
	if commuteArgs {
		relationship = commuteGeoRelationshipMap[relationship]
	}
