...
// otherwise false.
func (g *geoFilterPlanner) MakeSTDFullyWithin(
	op opt.Operator, args memo.ScalarListExpr, bound opt.ScalarExpr, fnIsLeftArg bool,
) opt.ScalarExpr {
...
func (g *geoFilterPlanner) makeSTDWithin(
	...
	if not {
		...
		within = g.factory.ConstructNot(within)
	}
	return within
}
...
func (g *geoFilterPlanner) maybeDeriveUsefulInvertedFilterCondition(
	...
	if private.Name == "st_distance" {
		makeSTDWithinFunc = g.MakeSTDWithin
	} else {
		makeSTDWithinFunc = g.MakeSTDFullyWithin
	}
