...
func (p *PreFilterer) Bind(d tree.Datum) interface{} {
	...
	case *tree.DGeography:
		...
		srid = g.SRID()
	default:
		panic(errors.Errorf("datum of unhandled type: ", d))
	}
...
// PreFilter pre-filters a retrieved inverted values against a set of
// pre-filter states. The function signature matches the PreFilter function of
// the DatumsToInvertedExpr interface: the result slice indicates which
// pre-filters matched and the single bool in the return value is true iff
// there is at least one result index with a true value.
