...
func TestTryJoinGeoIndex(t *testing.T) {
	...
	if _, err := tc.ExecuteDDL(
		"CREATE TABLE t2 (geom2 GEOMETRY, geog2 GEOGRAPHY, INVERTED INDEX (geom2), INVERTED INDEX (geog2))",
	); err != nil {
		t.Fatal(err)
	}
	...
	testCases := []struct {
		...
		},
		{
			...
			filters:      "st_covers(geom1, geom2) AND st_covers(geom2, 'LINESTRING ( 0 0, 0 2 )'::geometry)",
			indexOrd:     geomOrd,
			invertedExpr: "st_covers(geom1, geom2) AND st_coveredby('LINESTRING ( 0 0, 0 2 )'::geometry, geom2)",
		},
		{
