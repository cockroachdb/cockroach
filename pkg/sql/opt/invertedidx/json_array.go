...
func (j *jsonOrArrayJoinPlanner) canExtractJsonOrArrayJoinCondition(
	...
	if variable.Col != j.tabID.ColumnID(j.index.Column(0).InvertedSourceColumnOrdinal()) {
...
// getSpanExprForJsonOrArrayIndex gets a SpanExpression that constrains the
// given json or array index according to the given constant.
func getSpanExprForJsonOrArrayIndex(d tree.Datum) *invertedexpr.SpanExpression {
	spanExpr, err := invertedexpr.JsonOrArrayToSpanExpr(d)
	if err != nil {
		panic(err)
	}
	return spanExpr
	...
	getInvertedExprLeaf := func(expr tree.TypedExpr) (tree.TypedExpr, error) {
		...
		case *tree.ComparisonExpr:
			...
			if d, ok := nonIndexParam.(tree.Datum); ok {
				spanExpr = getSpanExprForJsonOrArrayIndex(d)
			}
...
func (g *jsonOrArrayDatumsToInvertedExpr) Convert(
	...
	evalInvertedExprLeaf := func(expr tree.TypedExpr) (invertedexpr.InvertedExpression, error) {
		...
		case *jsonOrArrayInvertedExpr:
			...
			return getSpanExprForJsonOrArrayIndex(d), nil
