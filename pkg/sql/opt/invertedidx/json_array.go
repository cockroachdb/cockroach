...
func (j *jsonOrArrayFilterPlanner) extractJSONFetchValEqCondition(
	...
	// scan resulting in the inverted expression to not be tight.
	for i := range keys {
...
// is built due to the filter being of the form j->0 = val. An array
// is built since the index position of the JSON array, denoted by 0 here,
// is not going to be present in an inverted expression. This removes the
// need for the actual integer value of the position in the JSON array
// being built as the original filter will be applied after the initial
// scan.
