...
func (j *jsonOrArrayFilterPlanner) extractJSONFetchValEqCondition(
	...
	var keys []string
	keys, foundKeys := j.collectKeys(keys, left)
	if !foundKeys {
		return inverted.NonInvertedColExpression{}
	}
...
// val expressions, collectKeys returns nil for keys and foundKeys is false.
// If successful, foundKeys is set to true and JSON fetch value indexes are
// collected in keys. The keys are ordered by the outer-most fetch val index
// first. The outer-most fetch val index is the right-most in the -> chain,
// for example (j->'a'->'b') is equivalent to ((j->'a')->'b') and 'b' is the
