...
func (j *jsonOrArrayFilterPlanner) extractJSONFetchValEqCondition(
	...
	// ordered by the outer-most fetch val index first. Later on, we iterate
	// forward through these keys to build a JSON object from the inside-out
	// with the inner-most value being the right scalar expression. In the
	// resulting JSON object, the outer-most JSON fetch value indexes are the
	// inner most JSON object keys.
	//
	// For example, when left is (j->'a'->'b') and right is ('1'), the keys
