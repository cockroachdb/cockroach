...
type Locking struct {
	...
	// persists until commit time, while a non-durable lock may sometimes be lost
	// before commit. We currently only require durable locks for SELECT FOR
	// UPDATE statements. Other locking statements, such as UPDATE, rely on the
	// durability of intents for correctness, rather than the durability of locks.
	Durability tree.LockingDurability
}

// Max returns the maximum of two locking properties.
