...
tested against every rule, and so on, until no further rules match. Rules are
applied depends on the order of the rules in each file, the lexicographical
ordering of files, and whether or not a rule is marked as high or low priority
as it is depicted below:

[FoldContainsArrayWithNulls, Normalize, HighPriority]
