...
type Scanner struct {
	...
	// lineInfo tracks the current file being scanned, as well as the current
...
// LineInfo returns the position within the input of the last token that was
// scanned.
func (s *Scanner) LineInfo() (line, pos int) {
	// Subtract length of current token so that position points to beginning
	// of token rather than end.
	return s.lineInfo.line + 1, s.lineInfo.pos - len(s.lit) + 1
}

// Scan reads the next token from the input and returns it. The Token, Literal,
// and LineInfo methods are also initialized with information about the token
// that was read.
