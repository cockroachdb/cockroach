...
// UnknownDataType is a data type about which nothing is known, and is assigned
// to custom functions:
//
//   (Scan $def:*) => (ConstrainScan $def)
//
// The ConstrainScan custom function has the UnknownDataType assigned to it.
var UnknownDataType = &ExternalDataType{Name: "<unknown>"}
...
// DoTypesContradict if two types cannot both be assigned to same expression.
func DoTypesContradict(dt1, dt2 DataType) bool {
	if dt1 == dt2 || dt1 == UnknownDataType || dt2 == UnknownDataType {
		return false
	}
	...
	case *ExternalDataType:
		...
		case *ExternalDataType:
			...
			// Otherwise, assume types are compatible.
			return false
		}
	}
...
func IsTypeMoreRestrictive(dt1, dt2 DataType) bool {
	...
	case *ExternalDataType:
		if dt1 == UnknownDataType {
			return false
		}
		if dt1 == ListDataType {
			return dt2 == UnknownDataType
		}
		return dt2 == ListDataType || dt2 == UnknownDataType
	}
	panic("unhandled data type")
}
