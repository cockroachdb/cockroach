...
func (c *Compiler) addErr(src *Source, err error) {
	if src != nil {
		err = fmt.Errorf("%s:%d:%d: %s", src.File, src.Line, src.Pos, err.Error())
	}
...
func (c *ruleCompiler) expandRule(opName OpNameExpr) {
	...
	match := &MatchExpr{Names: OpNamesExpr{opName}}
	match.Fields = append(match.Fields, c.rule.Match.Fields...)

	match = match.Visit(c.acceptRuleMatchExpr).(*MatchExpr)
	replace := c.rule.Replace.Visit(c.acceptRuleReplaceExpr)

	newRule := &RuleExpr{Name: c.rule.Name, Tags: c.rule.Tags, Match: match, Replace: replace}
	c.compiled.Rules = append(c.compiled.Rules, newRule)
}

// acceptRuleMatchExpr performs rewrites on expressions within the rule's match
// expression.
...
// acceptRuleReplaceExpr performs rewrites on expressions within the rule's
// replace expression.
