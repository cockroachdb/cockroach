...
func (c *ruleCompiler) expandRule(opName NameExpr) {
	...
	if errCnt == len(c.compiler.errors) {
		c.inferTypes(newRule.Match, UnknownDataType)
		c.inferTypes(newRule.Replace, UnknownDataType)
	}
...
func (c *ruleCompiler) inferTypes(e Expr, suggested DataType) {
	...
	case *FuncExpr:
		...
		if t.HasDynamicName() {
			...
			t.Typ = c.bindings[nameFunc.Args[0].(*RefExpr).Label].Typ
	...
	case *RefExpr:
		...
		t.Typ = mostRestrictiveDataType(c.bindings[t.Label].Typ, suggested)
	...
	case *ListExpr:
		...
		t.Typ = mostRestrictiveDataType(suggested, ListDataType)
		for _, item := range t.Items {
			c.inferTypes(item, UnknownDataType)
		}
...
func (c *ruleContentCompiler) compileBind(bind *BindExpr) Expr {
	...
	// Store pre-compiled bind expression in the bindings map. This is necessary
	// to handle cases like:
	//
	//   $input:* & (Func $input)
	//
	c.compiler.bindings[bind.Label] = bind

	newBind := bind.Visit(c.compile).(*BindExpr)

	// Replace pre-compiled bind expression with post-compiled bind. Otherwise,
	// bindings will sometimes point to expressions that have been superseded.
	c.compiler.bindings[bind.Label] = newBind
...
// names, and that they are legal in the current context. expandNames returns
