...
func (g *factoryGen) genMatchNameAndChildren(
	...
	// needs to be inverted. The code generator does this by setting a
	// "_match = false" flag, and then testing that flag after breaking out of
	...
	// children (e.g. just matching (Eq) with no child matching), then that can
	// easily be done by flipping == to/from != in one if statement.
	if noMatch && len(match.Args) != 0 {
