...
func (g *factoryGen) genCopyAndReplaceDefault() {
	...
	g.w.writeIndent("// function. Replacement callback methods can delegate to this method as a\n")
	g.w.writeIndent("// convenience, so that they don't need to copy every kind of operator\n")
	g.w.writeIndent("// themselves.\n")

	g.w.writeIndent("// CopyAndReplaceDefault constructs a copy of the given source operator using\n")
	g.w.writeIndent("// children copied (and potentially remapped) by the given replace function. This\n")
	g.w.writeIndent("// \n")
	g.w.writeIndent("//\n")
	g.w.writeIndent("// NOTE: CopyAndReplaceDefault should only be used in concert with the\n")
	g.w.writeIndent("// CopyAndReplace method, which first copies memo metadata that might be used\n")
	g.w.writeIndent("// by copied expressions. See its comments for more details on usage.\n")
	...
	for _, define := range g.compiled.Defines.WithTag("List") {
		...
		if itemType.isGenerated {
			...
			g.w.writeIndent("dst[i].%s = replace(src[i].%s).(opt.ScalarExpr)\n",
			...
			g.w.writeIndent("dst[i] = replace(src[i]).(opt.ScalarExpr)\n")
		}
		g.w.unnest("}\n")
