...
func (g *exprsGen) genExprType(define *lang.DefineExpr) {
	...
	if isValueType(define) {
		...
		if len(define.Fields) == 0 {
			fmt.Fprintf(g.w, "var %sSingleton = &%s{}\n\n", define.Name, exprType)
		}
	}
}
...
// func (e *SomeExpr) Source() *Source {
//   return e.Src
// }
func (g *exprsGen) genSourceFunc(define *lang.DefineExpr) {
	exprType := fmt.Sprintf("%sExpr", define.Name)

	fmt.Fprintf(g.w, "func (e *%s) Source() *Source {\n", exprType)
	if !isValueType(define) && !isSliceType(define) && len(define.Fields) > 0 {
