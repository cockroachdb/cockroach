...
// index, etc.) If a new catalog object is created, it will always be assigned a
// new StableID that has never, and will never, be reused by a different object
// in the same scope. This is true even if the new object has the same name and
// schema. The StableID will never change as long as that object exists.
//
