...
type Table interface {
	...
	// collection includes mutation columns, if present.
	//
	// Use the MutationColumn method to discover if a column is a mutation column
	// and to get more information about it.
	Column(i int) Column

	// MutationColumn returns information about the column at the ith ordinal
	// position within the table, if it is a mutation column. Mutation columns are
	// in the process of being added or dropped from the table, and may need to
	// have default or computed values set when inserting or updating rows. See
	// this RFC for more details:
	//
...
// the given ordinal position is a mutation column. See Table.MutationColumn for
// more details.
func IsMutationColumn(table Table, i int) bool {
	_, ok := table.MutationColumn(i)
	return ok
}
