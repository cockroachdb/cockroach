...
func formatColPrefix(idx Index, prefixLen int) string {
	...
	for i := 0; i < prefixLen; i++ {
		...
		colName := idx.Column(int(i)).Column.ColName()
...
// formatCatalogFKRef nicely formats a catalog foreign key reference using a treeprinter for
// debugging and testing.
func formatCatalogFKRef(cat Catalog, tab Table, idx Index, fkRef ForeignKeyReference, tp treeprinter.Node) {
