...
func (t *indexTrie) insert(ci tree.CreateIndex) {
	node := t.root
	for _, column := range ci.Columns {
		indexCol := indexedColumn{
			column:     column.Column,
			direction:  column.Direction,
			nullsOrder: column.NullsOrder,
		}
	...
	for _, column := range ci.Storing {
		node.storing[string(column)] = struct{}{}
	}
}
...
func assignStoringToShallowestLeaf(node *indexTrieNode, curDep int16) (*indexTrieNode, int16) {
	...
	// shallowest leaf node will be the left "c". Then we would like to assign the
	// storing part of node "a" to that shallowest leaf node "c". Before assign
	// all of them, we need to remove all the columns that covered by the path
	// from "c" to "a", which is "b" and "c" in this example. The following code
	// traverses from the shallowest leaf node to the current node and remove all
	// the columns covered by the path. Finally, the left leaf node "c" will
	// represent the index (a, b, c) storing (d).
	if len(node.storing) > 0 {
...
func collectAllLeaves4Tables(trie *indexTrie) ([][]indexedColumn, [][]tree.Name) {
