		...
		for i, indexedCols := range indexedColsArray {
			...
			for j, col := range indexedCols {
				...
				if col.direction == tree.Ascending {
					...
					if col.nullsOrder == tree.NullsFirst {
						cisIndexedCols[j].NullsOrder = tree.DefaultNullsOrder
					}
				} else if col.direction == tree.Descending {
					if col.nullsOrder == tree.NullsLast {
						cisIndexedCols[j].NullsOrder = tree.DefaultNullsOrder
					}
				}
			}
