...
// extractIndexCovering pushes down the storing part of the internal nodes: find whether it
...
func extractIndexCovering(tm map[tree.TableName]*indexTrie) ([]tree.CreateIndex, error) {
	...
	for table, trie := range tm {
		indexedColsArray, storingColsArray := collectAllLeaves4Tables(trie)
		// The length of indexedCols and storingCols must be equal
		if len(indexedColsArray) != len(storingColsArray) {
			return nil, errors.Newf("The leaves of the trie for table %s is wrong!", table)
		}
		var idx = 0
		for i, indexedCols := range indexedColsArray {
			...
			for _, col := range indexedCols {
				cisIndexedCols[idx] = tree.IndexElem{
					Column:     col.column,
