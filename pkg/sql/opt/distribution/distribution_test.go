...
func TestGetDistributions(t *testing.T) {
	...
	for tcIdx, tc := range testCases {
		t.Run(fmt.Sprintf("case%d", tcIdx+1), func(t *testing.T) {
			...
			f.Memo().SetBestProps(distributeRel, &physical.Required{}, parentProvided, memo.Cost{Cost: 0})
			f.Memo().SetBestProps(inputRel, &physical.Required{}, childProvided, memo.Cost{Cost: 0})
