...
func FormatTable(cat Catalog, tab Table, tp treeprinter.Node) {
	...
	for i := 0; i < tab.OutboundForeignKeyCount(); i++ {
		formatCatalogFKRef(cat, "CONSTRAINT", tab.OutboundForeignKey(i), child)
	}

	for i := 0; i < tab.InboundForeignKeyCount(); i++ {
		formatCatalogFKRef(cat, "REFERENCED BY CONSTRAINT", tab.InboundForeignKey(i), child)
	}
...
func formatCatalogFKRef(
	cat Catalog, title string, fkRef ForeignKeyConstraint, tp treeprinter.Node,
) {
