...
// to be used with tab.Statistic(). The returned stat should be older than
// the given timestamp parameter. Specifically, if inclusive==false, the
// returned stat should be strictly older than the timestamp, otherwise it
// can be equal to it.  If such doesn't exist (meaning that
// // either there are no full stats altogether or that the present ones
// // cannot be used based on the session variables), then
...
// IsEligibleFullStats returns true if the given statistic can be used as a full stats
// that matches the requirements from the session data, and also has an
// eligible creation timestamp.
func IsEligibleFullStats(
	...
	return !stat.IsPartial() &&
		...
		((inclusive && createdAtTs.LessEq(olderThan)) || (!inclusive && createdAtTs.Less(olderThan)))
}
