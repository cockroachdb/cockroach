...
func FormatTable(cat Catalog, tab Table, tp treeprinter.Node, redactableValues bool) {
	...
	for i := 0; i < tab.CheckCount(); i++ {
		child.Childf("CHECK (%s)", MaybeRedact(tab.Check(i).Constraint, redactableValues))
	}
	...
	for i := 0; i < tab.UniqueCount(); i++ {
		...
		if pred, isPartial := uniq.Predicate(); isPartial {
			c.Childf("WHERE %s", MaybeRedact(pred, redactableValues))
		}
	}
	...
	if n := idx.PartitionCount(); n > 0 {
		...
		for i := 0; i < n; i++ {
			...
			for _, datums := range p.PartitionByListPrefixes() {
				prefixes.Child(MaybeRedact(datums.String(), redactableValues))
			}
			FormatZone(p.Zone(), part)
		}
	}
	if pred, isPartial := idx.Predicate(); isPartial {
		child.Childf("WHERE %s", MaybeRedact(pred, redactableValues))
	}
}
...
func formatColumn(col *Column, buf *bytes.Buffer, redactableValues bool) {
	...
	if col.IsComputed() {
		exprStr := MaybeRedact(col.ComputedExprStr(), redactableValues)
	...
	if col.HasDefault() {
		...
		if generatedAsIdentityType == NotGeneratedAsIdentity {
			fmt.Fprintf(buf, " default (%s)", MaybeRedact(col.DefaultExprStr(), redactableValues))
		} else {
	...
	if col.HasOnUpdate() {
		fmt.Fprintf(buf, " on update (%s)", MaybeRedact(col.OnUpdateExprStr(), redactableValues))
	}
...
func MaybeRedact(expr string, redactExpr bool) string {
	if redactExpr {
		return string(redact.Sprintf("%s", expr))
	}
	return expr
}
