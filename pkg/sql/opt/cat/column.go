...
const (
	...
	// Invisible columns are not visible to queries in any way.
	Invisible
)
...
func (c *Column) InitNonVirtual(
	...
	if (kind == WriteOnly || kind == DeleteOnly) && visibility != Invisible {
		panic("foo")
		//panic(errors.AssertionFailedf("mutation columns should always be invisible"))
	}
