...
const (
	// NotIdentity column is created without
	// the GENERATED {ALWAYS | BY DEFAULT} AS IDENTITY token
	NotIdentity GeneratedAsIdentityType = iota

	// GeneratedAlways column is created with
	// the GENERATED ALWAYS AS IDENTITY token
	GeneratedAlways

	// GeneratedByDefault column is created with
	// the GENERATED BY DEFAULT AS IDENTITY token
	GeneratedByDefault
)
...
func (c *Column) Init(
	...
	*c = Column{
		...
		generatedAsIdentityType:     GeneratedAsIdentityType(generatedAsIDType),
	}
	if defaultExpr != nil {
...
func (c *Column) IsGeneratedAlways() bool {
	return c.generatedAsIdentityType == GeneratedAlways
}
