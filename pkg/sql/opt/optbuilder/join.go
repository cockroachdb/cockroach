...
func (b *Builder) buildJoin(
	join *tree.JoinTableExpr, inScope *scope, ctx buildCtx,
) (outScope *scope) {
	leftScope := b.buildDataSource(join.Left, nil /* indexFlags */, inScope, ctx)
	...
	rightScope := b.buildDataSource(join.Right, nil /* indexFlags */, inScopeRight, ctx)
