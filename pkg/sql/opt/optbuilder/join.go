...
func (b *Builder) buildJoin(join *tree.JoinTableExpr, inScope *scope) (outScope *scope) {
	...
	case *tree.OnJoinCond, nil:
		...
		left := leftScope.node.(memo.RelExpr)
		right := rightScope.node.(memo.RelExpr)
		outScope.node = b.constructJoin(joinType, left, right, filters)
		return outScope
...
func (jb *usingJoinBuilder) finishBuild() {
	...
	jb.outScope.node = jb.b.constructJoin(
		jb.joinType,
		jb.leftScope.node.(memo.RelExpr),
		jb.rightScope.node.(memo.RelExpr),
		jb.filters,
	)
	...
	if !jb.ifNullCols.Empty() {
		...
		jb.outScope.node = jb.b.constructProject(jb.outScope.node.(memo.RelExpr), jb.outScope.cols)
	}
}
