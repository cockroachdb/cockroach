...
type mutationBuilder struct {
	...
	// withID is set if we need to buffer the input for FK checks.
	withID opt.WithID
}
...
func (mb *mutationBuilder) buildFKChecks() {
	...
	for i, n := 0, mb.tab.OutboundForeignKeyCount(); i < n; i++ {
		...
		// Set up a LetRef; for this we have to synthesize new columns.
		letRefCols := make(opt.ColList, numCols)
		for i := 0; i < numCols; i++ {
			c := mb.b.factory.Metadata().ColumnMeta(inputCols[i])
			letRefCols[i] = mb.md.AddColumn(c.Alias, c.Type)
		}
					...
					if !notNullInputCols.Contains(inputCols[i]) {
						filters = append(filters, memo.FiltersItem{
							Condition: mb.b.factory.ConstructIsNot(
								mb.b.factory.ConstructVariable(letRefCols[i]),
								memo.NullSingleton,
							),
						})
					}
		...
		for j := 0; j < numCols; j++ {
			antiJoinFilters[j].Condition = mb.b.factory.ConstructEq(
				mb.b.factory.ConstructVariable(letRefCols[j]),
