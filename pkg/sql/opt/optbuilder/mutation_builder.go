...
func (mb *mutationBuilder) buildFKChecksForInsert() {
	...
	newRowsExpr, cols := mb.projectOrdinals(mb.insertOrds)
	for i, n := 0, mb.tab.OutboundForeignKeyCount(); i < n; i++ {
		mb.addInsertionCheck(i, newRowsExpr, cols)
	}
}
...
func (mb *mutationBuilder) buildFKChecksForUpdate() {
	...
	for i, n := 0, mb.tab.OutboundForeignKeyCount(); i < n; i++ {
		...
		newRowsExpr, cols := mb.projectOrdinals(newRowCols)
		mb.addInsertionCheck(i, newRowsExpr, cols)
	}
...
func (mb *mutationBuilder) buildFKChecksForUpsert() {
	...
	for i, n := 0, mb.tab.OutboundForeignKeyCount(); i < n; i++ {
		...
		newRowsExpr := mb.b.factory.ConstructWithScan(&memo.WithScanPrivate{
			With:         mb.withID,
			OutCols:      outColIDs,
			InCols:       inColIDs,
		...
		newRowsExpr = mb.b.factory.ConstructSelect(newRowsExpr, wasInsertionFilter)

		mb.addInsertionCheck(i, newRowsExpr, outColIDs)
	}
