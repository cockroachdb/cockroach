...
func (mb *mutationBuilder) buildForeignKeys() {
	...
	for i, n := 0, mb.tab.OutboundForeignKeyCount(); i < n; i++ {
		...
		checks = append(checks, memo.FKChecksItem{Check: expr})
	}

	if len(checks) > 0 {
		mb.outScope.expr = mb.b.factory.ConstructForeignKeys(mb.outScope.expr, checks)
	}
}
