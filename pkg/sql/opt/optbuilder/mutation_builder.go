...
// function will error.
//
// Assignment casts are always wrap mutation columns even if the type of a
// mutation column is identical to its target type. This is required in order to
// correctly cast placeholder values. When a memo is built at PREPARE-time, the
// type of a placeholder value provided at EXECUTE-time cannot be known. If a
// value provided does not have a type identical to its target column's type, an
// assignment cast must be performed. If the value's type is the correct type,
// the assignment cast is a no-op.
