...
// the mutating table, and each column in srcCols represents the new value of
// the column with the same ordinal in the target table. The columns in srcCols
// are set to the column IDs of the projected new assignment casts.
//
// If there is no valid assignment cast from a column type in srcCols to its
// corresponding target column type, then this function will panic. If all the
// columns in srcCols have types identical to their target column types, then no
// projection is built.
