...
func (mb *mutationBuilder) completeBuildReturning(returningInScope, returningOutScope *scope) {
	...
	// Update the RETURNING scopes with the current expression, in case the expression
	// changed since the scopes were first created.
	returningInScope.expr = mb.outScope.expr
	returningOutScope.expr = mb.outScope.expr
