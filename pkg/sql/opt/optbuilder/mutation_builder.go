...
func (mb *mutationBuilder) addCheckConstraintCols(
	isUpdate bool, policyCmdScope cat.PolicyCommandScope,
) {
	if mb.tab.CheckCount() != 0 {
		...
		for i, n := 0, mb.tab.CheckCount(); i < n; i++ {
			...
			if check.IsRLSConstraint() {
				...
				rlsScalar, check = mb.buildRLSCheckScalar(policyCmdScope, mb.outScope, referencedCols)
...
// buildRLSCheckScalar returns a scalar to use to enforce the policies on write.
func (mb *mutationBuilder) buildRLSCheckScalar(
	cmdScope cat.PolicyCommandScope, exprScope *scope, referencedCols *opt.ColSet,
) (opt.ScalarExpr, *rlsCheckConstraint) {
	// UPSERT is complex because the applied policies depend on whether a conflict
	// occurred. This function ends up being called recursively to build different
	// parts of the scalar expression.
	if cmdScope == cat.PolicyScopeUpsert {
		conflictScanScalar, check := mb.buildRLSCheckScalar(
			cat.PolicyScopeUpsertConflictScan, mb.fetchScope, referencedCols)
		conflictUpdateScalar, updateCheck := mb.buildRLSCheckScalar(
			cat.PolicyScopeUpdate, mb.outScope, referencedCols)
		noConflictScalar, insertCheck := mb.buildRLSCheckScalar(
			cat.PolicyScopeInsertWithSelect, mb.outScope, referencedCols)

		// Combine all three constraints into a single expression. This allows us to
		// correctly expose the involved columns to the caller.
		check.mergeConstraint(updateCheck)
		check.mergeConstraint(insertCheck)
		...
		return mb.b.factory.ConstructAnd(
	...
	texpr := exprScope.resolveAndRequireType(expr, types.Bool)
	return mb.b.buildScalar(texpr, mb.outScope, nil, nil, referencedCols), check
}
