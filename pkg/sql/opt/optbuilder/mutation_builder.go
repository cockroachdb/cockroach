...
// is created as an IDENTITY column with the `GENERATED ALWAYS AS IDENTITY` syntax.
...
//
// This function is used in codes for UPDATE, INSERT and UPSERT statement.
...
func checkUpdateExpression(col *cat.Column, updateExpr *tree.UpdateExpr) {
	...
	// if current column was created with the `GENERATED ALWAYS` token,
