...
type mutationBuilder struct {
	...
	// vectorIndexPutEncVectorColIDs lists the input column IDs storing the
	// encoded vectors that should be inserted into the index. Note that the
	// encoded vectors are not needed for deletions. The length is always equal
	// to the number of vector indexes on the table.
	vectorIndexPutEncVectorColIDs opt.OptionalColList
...
func (mb *mutationBuilder) init(b *Builder, opName string, tab cat.Table, alias tree.TableName) {
	...
	*mb = mutationBuilder{
	...
	mb.vectorIndexPutEncVectorColIDs = getSlice(numVectorIndexes)
...
func (mb *mutationBuilder) projectVectorIndexColsForInsert() {
	...
	mb.replaceUnsetColsWithNulls(mb.vectorIndexPutEncVectorColIDs)
}
...
// belongs to.
//
// See the vectorIndexPutPartitionColIDs, vectorIndexPutEncVectorColIDs, and
	...
	if vectorIndexCount(mb.tab) > 0 {
		...
		for i, n := 0, mb.tab.DeletableIndexCount(); i < n; i++ {
			...
			if putCol := getPutCol(vectorColOrd); putCol != 0 {
				...
				encVectorCol := addCol(fmt.Sprintf("vector_index_put_enc_vector%d", idxOrd+1), types.Bytes)
				outCols := mb.outScope.colSet()
				outCols.Add(partitionCol)
				outCols.Add(encVectorCol)
				mb.outScope.expr = mb.buildVectorMutationSearch(
					mb.outScope.expr, index, putCol, partitionCol, encVectorCol, opt.ColSet{}, /* pkCols */
				)
				mb.vectorIndexPutPartitionColIDs[idxOrd] = partitionCol
				mb.vectorIndexPutEncVectorColIDs[idxOrd] = encVectorCol
			}
			idxOrd++
		}
	}
...
// find the partition (and encoded vector, if requested) for vectors in the
...
func (mb *mutationBuilder) buildVectorMutationSearch(
	input memo.RelExpr,
	index cat.Index,
	queryVectorCol, partitionCol, encVectorCol opt.ColumnID,
	pkCols opt.ColSet,
) memo.RelExpr {
...
func (mb *mutationBuilder) makeMutationPrivate(needResults bool) *memo.MutationPrivate {
	...
	private := &memo.MutationPrivate{
		...
		VectorIndexPutEncVectorCols: checkEmptyList(mb.vectorIndexPutEncVectorColIDs),
