...
func (b *Builder) buildRoutine(
	...
	defer func(
		...
		b.checkExecutePrivilegeUser = checkExecutePrivilegeUser
	}(b.trackSchemaDeps, b.insideUDF, b.insideDataSource, b.insideSQLRoutine, b.checkPrivilegeUser, b.checkExecutePrivilegeUser)
	...
	if o.Type != tree.BuiltinRoutine {
		if o.SecurityMode == tree.RoutineDefiner {
			...
			b.checkExecutePrivilegeUser = checkPrivUser
		} else {
			// SECURITY INVOKER (default): reset checkPrivilegeUser to the
			// invoker. This is necessary because a view may have overridden
			// checkPrivilegeUser to the view owner for direct data source
			// access, but a SECURITY INVOKER function called by the view
			// should run with the invoker's privileges, matching PostgreSQL.
			b.checkPrivilegeUser = b.checkExecutePrivilegeUser
		}
	}
