...
func (b *Builder) buildRoutine(
	...
	if originalRTyp.UserDefined() {
		funcReturnType, err := tree.ResolveType(b.ctx,
			&tree.OIDTypeReference{OID: originalRTyp.Oid()}, b.semaCtx.TypeResolver)
		if err != nil {
			panic(err)
		}
		if !funcReturnType.Identical(originalRTyp) {
			panic(pgerror.Newf(
				pgcode.InvalidFunctionDefinition,
				"return type mismatch in function declared to return %s", originalRTyp.Name(),
			))
		}
	}
	...
	case tree.RoutineLangSQL:
		...
		if originalRTyp.Family() == types.VoidFamily {
	...
	case tree.RoutineLangPLpgSQL:
		...
		plBuilder := newPLpgSQLBuilder(
			b, def.Name, stmt.AST.Label, colRefs, routineParams, originalRTyp, isProc, outScope,
		)
	...
	// NOTE: originalRTyp may not be up-to-date after the last statement is
...
func (b *Builder) maybeResolvePolymorphicReturnType(f *tree.FuncExpr, polyArgTyp *types.T) {
	...
	} else if b.routineHasPolymorphicOutParam(f.ResolvedOverload().RoutineParams) {
...
// routineHasPolymorphicOutParam returns true if the routine with the given
// parameters has a polymorphic-typed OUT parameter.
func (b *Builder) routineHasPolymorphicOutParam(params tree.RoutineParams) bool {
	for i := range params {
		if params[i].IsOutParam() {
			typ, err := tree.ResolveType(b.ctx, params[i].Type, b.semaCtx.TypeResolver)
			if err != nil {
				panic(err)
			}
	...
	case types.ArrayFamily:
		if polyArgTyp.Family() == types.ArrayFamily {
			panic(pgerror.Newf(pgcode.UndefinedObject,
				"could not find array type for data type %s", polyArgTyp.SQLStringForError(),
			))
		}
