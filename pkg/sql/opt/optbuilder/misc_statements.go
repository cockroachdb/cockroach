...
func (b *Builder) buildWhereForStatistics(
	...
	// Find a non-partial forward visible index with the filter column as the
	// first key column.
	foundIndex := false
	...
	for i := 0; i < tab.IndexCount(); i++ {
		...
		if _, isPartial := idx.Predicate(); isPartial || idx.Type() != idxtype.FORWARD ||
			tabMeta.IsIndexNotVisible(i, b.evalCtx.GetRNG()) {
			continue
		}
	...
	if !foundIndex {
		panic(
			pgerror.Newf(pgcode.InvalidColumnReference,
				"table %s does not contain a non-partial forward visible index with %s as a prefix column",
