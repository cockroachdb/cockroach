...
func (b *Builder) buildAggregation(
	...
	aggOutScope.node = b.constructGroupBy(
		aggInScope.node.(memo.RelExpr),
		groupingColSet,
		aggCols,
		aggInScope.ordering,
	)
	...
	if having != nil {
		input := aggOutScope.node.(memo.RelExpr)
		filters := memo.FiltersExpr{{Condition: having}}
		aggOutScope.node = b.factory.ConstructSelect(input, filters)
	}
	return aggOutScope
}
