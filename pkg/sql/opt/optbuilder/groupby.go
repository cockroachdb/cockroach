...
func (b *Builder) needsAggregation(
	sel *tree.SelectClause, orderBy tree.OrderBy, scope *scope,
) bool {
...
func (b *Builder) buildAggregation(
	...
	labels []string,
	selExprs tree.TypedExprs,
	havingExpr tree.TypedExpr,
	orderBy tree.OrderBy,
	distinctOn tree.TypedExprs,
	fromScope *scope,
	...
	aggOutScope.appendColumns(aggInScope.getGroupingCols(groupingsLen))
	...
	for _, col := range aggInScope.getGroupingCols(groupingsLen) {
		groupingColSet.Add(int(col.id))
	}
...
func (b *Builder) analyzeHaving(having *tree.Where, inScope *scope) tree.TypedExpr {
	...
	return inScope.resolveAndRequireType(having.Expr, types.Bool, "HAVING")
}
...
func (b *Builder) buildHaving(having tree.TypedExpr, inScope *scope) memo.GroupID {
	return b.buildScalar(having, inScope)
}
...
func (b *Builder) buildGrouping(
	...
	col := colIndex(len(selects), groupBy, ctx)
	label := ""
	if col != -1 {
	...
	b.semaCtx.Properties.Require(ctx, tree.RejectSpecial)

	// Resolve types, expand stars, and flatten tuples.
	exprs := b.expandStarAndResolveType(groupBy, inScope, ctx)
...
func (b *Builder) buildAggregateFunction(
	...
	for i, pexpr := range f.Exprs {
		...
		col := b.buildScalarProjection(
			pexpr.(tree.TypedExpr), "" /* label */, inScope, tempScope,
		)
	...
	if info.col == nil {
		...
		info.col = b.synthesizeColumn(aggOutScope, label, f.ResolvedType(), f, 0 /* group */)
