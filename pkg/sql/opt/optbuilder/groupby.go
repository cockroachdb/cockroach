...
func (b *Builder) buildAggregation(
	...
	// aggregate arguments.
	aggInScope = b.buildPreProjection(fromScope, aggInScope)
...
func (b *Builder) buildPreProjection(fromScope *scope, aggInScope *scope) *scope {
	// Don't add an unnecessary "pass-through" projection.
	if fromScope.hasSameColumns(aggInScope) {
		return fromScope
	}

	preProjList := b.constructList(opt.ProjectionsOp, aggInScope.cols)
	aggInScope.group = b.factory.ConstructProject(fromScope.group, preProjList)
	return aggInScope
}
