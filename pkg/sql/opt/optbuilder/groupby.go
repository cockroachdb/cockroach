...
func (b *Builder) analyzeHaving(having *tree.Where, fromScope *scope) tree.TypedExpr {
	...
	if b.insideFuncDef {
		having.Expr = b.maybeRewriteColumnPrefix(typedHavingExpr)
	}
...
func (b *Builder) buildGroupingList(
	oriGroupBy *tree.GroupBy, selects tree.SelectExprs, projectionsScope *scope, fromScope *scope,
) {
	groupBy := *oriGroupBy
	g := fromScope.groupby
	...
	var expandedGroupBy tree.GroupBy
	for i := range groupBy {
		expanded := b.buildGrouping(groupBy[i], selects, projectionsScope, fromScope, g.aggInScope)
		if b.insideFuncDef {
			expandedGroupBy = append(expandedGroupBy, expanded...)
		}
	}
	if b.insideFuncDef {
		*oriGroupBy = expandedGroupBy
	}
...
func (b *Builder) buildGrouping(
	...
	if b.insideFuncDef {
		for _, e := range exprs {
			expandedGroupBy = append(expandedGroupBy, b.maybeRewriteColumnPrefix(e))
		}
	}
	return expandedGroupBy
}
