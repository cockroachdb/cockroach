...
func (b *Builder) buildAggregation(
	...
	aggOutScope.cols = append(aggOutScope.cols, aggInScope.cols...)
	...
	if len(preProjGroups) != len(aggInScope.cols) {
		panic(fmt.Sprintf("%d groups, cols: %v", len(preProjGroups), aggInScope.cols))
	}
	...
	if !fromScope.hasSameColumns(aggInScope) {
		...
		outGroup = b.factory.ConstructProject(outGroup, preProjList)
	}
