...
func (s *scope) VisitPre(expr tree.Expr) (recurse bool, newExpr tree.Expr) {
	...
	case *tree.ArrayFlatten:
		...
		if sub, ok := t.Subquery.(*tree.Subquery); ok {
			...
			copy := *t
			copy.Subquery = s.replaceSubquery(sub, false /* tupleType */, 1 /* desiredColumns */)
			expr = &copy
		}
	...
	case *tree.ComparisonExpr:
		...
		case tree.In, tree.NotIn, tree.Any, tree.Some, tree.All:
			if sub, ok := t.Right.(*tree.Subquery); ok {
				...
				copy := *t
				copy.Right = s.replaceSubquery(sub, true /* tupleType */, -1 /* desiredColumns */)
				expr = &copy
			}
		}
	...
	case *tree.Subquery:
		...
		if t.Exists {
			expr = s.replaceSubquery(t, true /* tupleType */, -1 /* desiredColumns */)
		} else {
			expr = s.replaceSubquery(t, false /* tupleType */, s.columns /* desiredColumns */)
		}
	}
...
// Replace a raw subquery node with a typed subquery. tupleType specifies
// whether the return type of the subquery should be wrapped in a tuple.
// tupleType is true for subqueries that may return multiple rows in comparison
// expressions (e.g., IN, ANY, ALL) and EXISTS expressions.
...
func (s *scope) replaceSubquery(sub *tree.Subquery, tupleType bool, desiredColumns int) *subquery {
	if s.replaceSRFs {
