...
func isOrderedSetAggregate(
	...
	case "percentile_disc":
		builtinDef := tree.PrefixBuiltinFunctionDefinition(tree.FunDefs["percentile_disc_impl"], catconstants.PgCatalogName)
		newDef := *builtinDef
		unsetPrivate(&newDef)
		return &newDef, true
	case "percentile_cont":
		builtinDef := tree.PrefixBuiltinFunctionDefinition(tree.FunDefs["percentile_cont_impl"], catconstants.PgCatalogName)
		newDef := *builtinDef
		unsetPrivate(&newDef)
		return &newDef, true
	}
	...
	if strings.EqualFold(def.Name, "count") && f.Type == 0 {
		if _, ok := vn.(tree.UnqualifiedStar); ok {
			if f.Filter != nil {
				...
				// TODO(Chengxiong) Fix this with proper resolver
