...
func (s *scope) VisitPre(expr tree.Expr) (recurse bool, newExpr tree.Expr) {
	...
	case *tree.ColumnItem:
		...
		if resolveErr != nil {
			if sqlerrors.IsUndefinedColumnError(resolveErr) {
				// Attempt to resolve as a routine argument's value.
	...
	case *tree.FuncExpr:
		def, err := t.Func.Resolve(s.builder.semaCtx, s.builder.semaCtx.SearchPath, nil)
		if err != nil {
			panic(err)
		}
	...
	if strings.EqualFold(def.Name, "count") && f.Type == 0 {
		if _, ok := vn.(tree.UnqualifiedStar); ok {
			if f.Filter != nil {
				...
				newDef, err := e.Func.Resolve(s.builder.semaCtx, s.builder.semaCtx.SearchPath, nil)
				if err != nil {
					panic(err)
				}
