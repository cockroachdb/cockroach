...
// replaceSRF replaces a raw SRF with an srf struct. When this struct is later
// encountered during the build process, it is replaced with a reference to the
// column returned by the SRF (if the SRF returns a single column) or a tuple
// of column references (if the SRF returns multiple columns).
//
...
func (s *scope) replaceSubquery(sub *tree.Subquery, multiRow bool, desiredColumns int) *subquery {
	...
	defer func(replaceSRFs bool) { s.replaceSRFs = replaceSRFs }(s.replaceSRFs)
	s.replaceSRFs = false
