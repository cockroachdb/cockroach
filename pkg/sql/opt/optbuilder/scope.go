...
func (s *scope) replaceCount(
	...
	if strings.EqualFold(def.Name, "count") && f.Type == 0 {
		if _, ok := vn.(tree.UnqualifiedStar); ok {
			if f.Filter != nil {
				// If we have a COUNT(*) with a FILTER, we need to synthesize an input for the aggregation
				// to be over, because otherwise we have no input to hang the AggFilter off of.
				// Thus, we convert
