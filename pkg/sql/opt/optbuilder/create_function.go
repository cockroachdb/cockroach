...
func (b *Builder) buildCreateFunction(cf *tree.CreateRoutine, inScope *scope) (outScope *scope) {
	...
	case tree.RoutineLangPLpgSQL:
		...
		if funcReturnType.Identical(types.Trigger) {
			...
			//
			// Stage 1: Reuse the DDL walk from above. If DDL requiring late
			// binding was found, it can't be optbuilt and is inherently
			// mutating — set CAN_MUTATE and skipSQL immediately.
			//
			// Stage 2: If no DDL, trial-optbuild the body (skipSQL=false) to
			// derive CanMutate from the RelExprs. If the trial fails (e.g.,
			// body references a table that doesn't exist yet, or contains
			// DCL like GRANT), fall back to skipSQL=true with conservative
			// CAN_MUTATE.
