...
func (b *Builder) buildLimit(limit *tree.Limit, parentScope, inScope *scope) {
	...
	if limit.Offset != nil {
		...
		inScope.node = b.factory.ConstructOffset(input, offset, inScope.makeOrderingChoice())
	}
	if limit.Count != nil {
		op := "LIMIT"
		...
		inScope.node = b.factory.ConstructLimit(input, limit, inScope.makeOrderingChoice())
	}
}
