...
func (r *optRLSConstraintBuilder) Build(ctx context.Context) *rlsCheckConstraint {
	expr, colIDs := r.genExpression(ctx)
	if expr == "" {
		panic(errors.AssertionFailedf("must return some expression but empty string returned for user: %v", r.user))
	}
...
func (r *rlsCheckConstraint) mergeConstraint(other *rlsCheckConstraint) {
	var colIDs intsets.Fast
	for _, colID := range r.colIDs {
		colIDs.Add(colID)
	}
	for _, colID := range other.colIDs {
		colIDs.Add(colID)
	}
	r.colIDs = colIDs.Ordered()
	// We don't merge the expressions together because we don't make assumptions
	// on how they are combined. Set a value to indicate the merged constraint
	// does not have a valid expression.
	r.constraint = "n/a (rls)"
}
