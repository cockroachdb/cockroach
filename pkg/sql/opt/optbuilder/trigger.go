...
func (tb *rowLevelAfterTriggerBuilder) Build(
	...
	return buildTriggerCascadeHelper(ctx, semaCtx, evalCtx, catalog, factoryI, func(b *Builder) memo.RelExpr {
		...
		inFetchCols := remapColumns(tb.fetchCols, colMap)
		inUpdateCols := remapColumns(tb.updateCols, colMap)
		inInsertCols := remapColumns(tb.insertCols, colMap)
		...
		if tb.canaryCol != 0 {
			inCanaryCol = remapColumn(tb.canaryCol, colMap)
