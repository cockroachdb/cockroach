...
//    check to see if the scalar expression is nullable. It outputs this result
//    as a boolean
//
//    The supported args (in addition to the ones supported by OptTester:
//
//      - vars=(type1(!NULL),type2(!NULL),...)
//
//      the '!NULL' suffix on a var type is used to identify the variable as
//      nullable.
		...
		datadriven.RunTest(t, path, func(d *datadriven.TestData) string {
			...
			case "build-scalar", "scalar-is-nullable":
				...
				for _, arg := range d.CmdArgs {
					...
					case "vars":
						for i := 0; i < len(vals); i++ {
							if strings.HasSuffix(vals[i], "!NULL") {
								vals[i] = strings.TrimSuffix(vals[i], "!NULL")
								notNullCols.Add(i + 1)
							}
						}
				...
				if d.Cmd == "build-scalar" {
					...
					return fmt.Sprintf("%v\n", memo.ExprIsNullable(o.Memo().RootExpr().(opt.ScalarExpr), &notNullCols))
				}
