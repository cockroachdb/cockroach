...
func (mb *mutationBuilder) buildFKChecksAndCascadesForDelete() {
	...
	for i, n := 0, mb.tab.InboundForeignKeyCount(); i < n; i++ {
		...
		withScanScope, _ := mb.buildCheckInputScan(checkInputScanFetchedVals, h.tabOrdinals, true /* fk */)
...
func (mb *mutationBuilder) buildFKChecksForUpdate() {
	...
	for i, n := 0, mb.tab.InboundForeignKeyCount(); i < n; i++ {
		...
		oldRowsScope, _ := mb.buildCheckInputScan(checkInputScanFetchedVals, h.tabOrdinals, true /* fk */)
		newRowsScope, _ := mb.buildCheckInputScan(checkInputScanNewVals, h.tabOrdinals, true /* fk */)
...
func (h *fkCheckHelper) buildInsertionCheck() memo.FKChecksItem {
	withScanScope, notNullWithScanCols := h.mb.buildCheckInputScan(
		checkInputScanNewVals, h.tabOrdinals, true, /* fk */
	)
