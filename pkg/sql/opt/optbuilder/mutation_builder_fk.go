...
//
// When enable_implicit_fk_locking is true, or when using a weaker isolation
// level than Serializable, the insertion FK check will lock the parent row(s)
...
// locks on the child row(s) regardless of enable_implicit_fk_locking or
// isolation level. Instead we rely on the intents created by the delete to
// conflict with the locks of the parent from the insertion FK check of any
// concurrent inserts or updates to the child.
//
...
//
//     As in insertion checks, if enable_implicit_fk_locking is true, or we're
//     using a weaker isolation level, the insertion-side check will lock the
//     parent row(s).
//
...
//     rows, regardless of enable_implicit_fk_locking or isolation level.
//
...
func (h *fkCheckHelper) buildOtherTableScan(parent bool) (outScope *scope, tabMeta *opt.TableMeta) {
	...
	// For insertion-side checks, if enable_implicit_fk_locking is true or we're
	// using a weaker isolation level, we lock the parent row(s) to prevent
	// concurrent mutations of the parent from violating the FK
