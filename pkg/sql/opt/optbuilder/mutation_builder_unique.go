...
func (h *uniqueCheckHelper) init(mb *mutationBuilder, uniqueOrdinal int) bool {
	if !mb.tab.Unique(uniqueOrdinal).WithoutIndex() {
		// This constraint is already enforced by an index. No need to plan a
		// check.
		return false
	}
	...
	*h = uniqueCheckHelper{
	...
	if numNullCols > 0 {
		// At least one unique column is getting a NULL value; unique check not needed.
		return false
	}
...
func (h *uniqueCheckHelper) buildInsertionCheck() memo.UniqueChecksItem {
	checkInput, withScanCols, notNullWithScanCols := h.mb.makeCheckInputScan(
	...
	if notNullWithScanCols.Len() < numCols {
		...
		// handling, since the row doesn't need to be unique if any columns in
		// the unique constraint are NULL.
		//
		...
		checkInput = f.ConstructSelect(checkInput, filters)
	}
	...
	semiJoinFilters := make(memo.FiltersExpr, len(h.uniqueOrdinals)+1)
	for i := 0; i < len(h.uniqueOrdinals); i++ {
		semiJoinFilters[i] = f.ConstructFiltersItem(
			f.ConstructEq(
	...
	semiJoinFilters[len(semiJoinFilters)-1] = f.ConstructFiltersItem(pkFilter)
