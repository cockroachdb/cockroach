...
func (h *uniqueCheckHelper) init(mb *mutationBuilder, uniqueOrdinal int) bool {
	...
	*h = uniqueCheckHelper{
	...
	for tabOrd, ok := h.uniqueOrdinals.Next(0); ok; tabOrd, ok = h.uniqueOrdinals.Next(tabOrd + 1) {
		...
		// If we don't require uniqueness checks for gen_random_uuid() and one of
		// the columns has UUID type and returns gen_random_uuid(), unique check not
		// needed.
		requireChecksForGenRandUUID := UniquenessChecksForGenRandomUUIDClusterMode.Get(&mb.b.evalCtx.Settings.SV)
		if !requireChecksForGenRandUUID {
