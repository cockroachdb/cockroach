...
func (b *Builder) buildScalarHelper(
	...
	case *tree.ComparisonExpr:
		if sub, ok := t.Right.(*subquery); ok && sub.multiRow {
			...
			// array is legal) were handled above.
			out = b.buildAnyArray(t, inScope)
		} else {
			left := b.buildScalarHelper(t.TypedLeft(), "", inScope, nil)
			right := b.buildScalarHelper(t.TypedRight(), "", inScope, nil)
...
func (b *Builder) buildAnyArray(t *tree.ComparisonExpr, inScope *scope) memo.GroupID {
	left := b.buildScalarHelper(t.TypedLeft(), "", inScope, nil)
	right := b.buildScalarHelper(t.TypedRight(), "", inScope, nil)
	...
	out := b.factory.ConstructAnyArray(
		left,
		right,
