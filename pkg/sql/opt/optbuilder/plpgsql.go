...
func (b *plpgsqlBuilder) getRaiseArgs(
	...
	switch logLevel {
	case "EXCEPTION", "":
		// ERROR is the equivalent severity to log-level EXCEPTION, and also the
		// default if no log-level is specified.
	...
	buildOptionExpr := func(name string, expr plpgsqltree.PLpgSQLExpr, isDup bool) opt.ScalarExpr {
		if isDup {
			panic(pgerror.Newf(pgcode.PLpgSQL, "RAISE option already specified: %s", name))
		}
