...
func (b *Builder) buildSubqueryProjection(
	...
	default:
		...
		for i := range s.cols {
			cols[i] = &s.cols[i]
			colGroups[i] = b.factory.ConstructVariable(b.factory.InternPrivate(s.cols[i].index))
		}
...
// buildSubquery builds a set of memo groups that represent the given subquery.
//
// See Builder.buildStmt for a description of the remaining input and
// return values.
func (b *Builder) buildSubquery(s *subquery, inScope *scope) (out memo.GroupID, outScope *scope) {
	if s.exists {
		return b.factory.ConstructExists(s.out), inScope
	}

	out, outScope = b.buildSubqueryProjection(s, inScope)
	v := b.factory.ConstructVariable(b.factory.InternPrivate(outScope.cols[0].index))
	out = b.factory.ConstructSubquery(out, v)
	return out, outScope
}

// transformAndBuildSubquery transforms a multi-row subquery into a single-row
...
//
func (b *Builder) transformAndBuildSubquery(
