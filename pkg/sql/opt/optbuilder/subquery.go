...
func (b *Builder) buildMultiRowSubquery(
	...
	if b.insideUDF {
		...
		// build them as regular subqueries instead.
...
func (b *Builder) buildAnyAsSubquery(
	...
	boolOrNullCol := md.AddColumn("bool_or_null", types.Int)
	aggs := memo.AggregationsExpr{
		...
		f.ConstructAggregationsItem(f.ConstructBoolOr(f.ConstructVariable(isNullCol)), boolOrNullCol),
	}
	out = f.ConstructScalarGroupBy(out, aggs, &memo.GroupingPrivate{})

	// Create a projection of:
	//   countCol > 0 AND (boolOrCol OR (boolOrNullCol AND NULL))
		...
		f.ConstructProjectionsItem(
			f.ConstructAnd(
				...
				f.ConstructOr(
					...
					f.ConstructAnd(
						f.ConstructVariable(boolOrNullCol),
						memo.NullSingleton,
					),
				),
			),
