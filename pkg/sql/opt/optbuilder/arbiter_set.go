...
// non-partial, or their partial predicates are identical.
//
...
type minArbiterSet struct {
	...
	// singleIndexConflictOrdsCache and indexConflictOrdsCache are used to cache
	// the conflict column sets of arbiter indexes in the set.
	// singleIndexConflictOrdsCache is used to avoid allocations in the common
	// case where there is only a single arbiter index in the set.
	singleIndexConflictOrdsCache util.FastIntSet
...
func (m *minArbiterSet) findRedundantIndex(
	...
	if !m.singleIndexConflictOrdsCache.Empty() {
		i, _ := m.as.indexes.Next(0)
		indexPred, _ := m.as.mb.tab.Index(i).Predicate()
		if pred == indexPred && conflictOrds.SubsetOf(m.singleIndexConflictOrdsCache) {
			return i, true
		}
		return -1, false
	}
	// Otherwise, find the first arbiter index that is made redundant by
	// the unique constraint.
...
// initCache initializes the conflict column caches. It initializes
// singleIndexConflictOrdsCache if the set contains only one arbiter index. If
// there are multiple arbiters indexes in the set, it initializes
// indexConflictOrdsCache. This avoids allocating a map in the common case where
// there is only a single arbiter index.
func (m *minArbiterSet) initCache() {
	...
	// If there is only a single arbiter index in the set, cache its conflict
	// columns and predicate without allocating maps.
	if m.as.indexes.Len() == 1 {
		i, _ := m.as.indexes.Next(0)
		index := m.as.mb.tab.Index(i)
		m.singleIndexConflictOrdsCache = getIndexLaxKeyOrdinals(index)
		return
	}
