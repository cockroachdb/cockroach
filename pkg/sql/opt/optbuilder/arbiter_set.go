...
func (a *arbiterSet) ForEach(
	f func(name string, conflictOrds intsets.FastIntSet, pred tree.Expr, canaryOrd int),
) {
...
type minArbiterSet struct {
	...
	// in the set.
	indexConflictOrdsCache map[cat.IndexOrdinal]intsets.FastIntSet
}
...
func (m *minArbiterSet) initCache() {
	...
	m.indexConflictOrdsCache = make(map[cat.IndexOrdinal]intsets.FastIntSet, m.as.indexes.Len())
