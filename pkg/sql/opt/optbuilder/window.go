...
func (b *Builder) buildWindow(outScope *scope, inScope *scope) {
	...
	for i := range inScope.windows {
		...
		argLists[i] = b.constructWindowArgs(argExprs, i, w.def.Name, inScope, argScope)

		// Build appropriate partitions.
		windowPartition := b.constructWindowPartition(def.Partitions, i, w.def.Name, inScope, argScope)
		partitions[i].UnionWith(windowPartition)

		// Build appropriate orderings.
		ord := b.constructWindowOrdering(def.OrderBy, i, w.def.Name, inScope, argScope)
		...
		if w.Filter != nil {
			...
			col := b.constructFilterCol(w.Filter, i, w.def.Name, inScope, argScope)
			filterCols[i] = col.id
		}
...
func (b *Builder) buildAggregationAsWindow(
	...
	for i, agg := range aggInfos {
		...
		// Build the appropriate arguments.
		argLists[i] = b.constructWindowArgs(argExprs, i, agg.def.Name, fromScope, aggInScope)
		...
		if !agg.IsCommutative() {
			ord := b.constructWindowOrdering(agg.OrderBy, i, agg.def.Name, fromScope, aggInScope)
			orderings[i].FromOrdering(ord)
		}
		...
		if agg.Filter != nil {
			...
			col := b.constructFilterCol(agg.Filter, i, agg.def.Name, fromScope, aggInScope)
			filterCols[i] = col.id
		}
	...
	aggOutScope.expr = b.constructWindowGroup(aggregateExpr, groupingColSet, aggInfos, aggInScope)
...
// constructWindowArgs constructs the argExprs into a memo.ScalarListExpr.
func (b *Builder) constructWindowArgs(
...
// constructWindowPartition constructs the appropriate partitions for window functions.
func (b *Builder) constructWindowPartition(
...
// constructWindowOrdering constructs the appropriate orderings for window functions.
func (b *Builder) constructWindowOrdering(
...
// constructFilterCol constructs the filter column from the filter Expr.
func (b *Builder) constructFilterCol(
