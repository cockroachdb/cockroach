...
func (mb *mutationBuilder) buildInputForDoNothing(
	...
	buildInputForArbiter := func(
		colCount int, colOrdinal func(int) int, isPartial bool, predExpr tree.Expr,
	) {
	...
	buildDistinctOnForArbiter := func(
		colCount int, colOrdinal func(int) int, isPartial bool, partialIndexDistinctCol *scopeColumn,
	) {
	...
	for idx, idxCount := 0, mb.tab.IndexCount(); idx < idxCount; idx++ {
		...
		buildDistinctOnForArbiter(index.LaxKeyColumnCount(), func(i int) int {
			return index.Column(i).Ordinal()
		}, isPartial, partialIndexDistinctCol)
	}

	// Loop over each arbiter constraint, creating an upsert-distinct-on for each
	// one.
	for uc, ucCount := 0, mb.tab.UniqueCount(); uc < ucCount; uc++ {
		// Skip non-arbiter constraints.
		if !arbiterConstraints.Contains(uc) {
