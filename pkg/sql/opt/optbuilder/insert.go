...
func (b *Builder) buildInsert(ins *tree.Insert, inScope *scope, ctx buildCtx) (outScope *scope) {
	var ctes []cteSource
	if ins.With != nil {
		inScope, ctes = b.buildCTE(ins.With.CTEList, inScope, ctx)
	}
	...
	if ins.OnConflict != nil && ins.OnConflict.IsUpsertAlias() {
		mb.init(b, "upsert", tab, *alias, ctx)
	} else {
		mb.init(b, "insert", tab, *alias, ctx)
	}
...
func (mb *mutationBuilder) buildInputForInsert(inScope *scope, inputRows *tree.Select) {
	...
	mb.outScope = mb.b.buildSelect(inputRows, desiredTypes, inScope, mb.ctx)
