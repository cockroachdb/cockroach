...
// inColList returns an ordered list of the input column IDs that are currently
// in scope.
func (ib *insertBuilder) inColList() opt.ColList {
	inCols := make(opt.ColList, len(ib.outScope.cols))
	for i := range ib.outScope.cols {
		inCols[i] = ib.outScope.cols[i].id
	}
	return inCols
}
...
func (ib *insertBuilder) checkPrimaryKey() {
	...
	for i, n := 0, primary.KeyColumnCount(); i < n; i++ {
		...
		if col.Column.IsDefault() || col.Column.IsComputed() {
...
// Alternatively, all columns can be unspecified and therefore have NULL values.
// If neither condition is true, checkForeignKeys raises an error. Here is an
// example:
//
//   CREATE TABLE orders (
//     id INT,
...
// specified in the INSERT statement. Either state must be specified as well, or
// else neither can be specified, so that both values are NULL.
...
func (ib *insertBuilder) addSynthesizedCols(addCol func(tabCol opt.Column) bool) {
	...
	for i, n := 0, ib.tab.ColumnCount()+ib.tab.MutationColumnCount(); i < n; i++ {
		...
		if i < ib.tab.ColumnCount() {
			tabCol = ib.tab.Column(i)
		} else {
			tabCol = ib.tab.MutationColumn(i - ib.tab.ColumnCount())
		}
...
func (ib *insertBuilder) buildInsert(returning tree.ReturningExprs) {
	...
	private := memo.InsertPrivate{
		Table:       ib.tabID,
		TableCols:   ib.targetColList,
		InputCols:   ib.inColList(),
