...
func (b *Builder) buildInsert(ins *tree.Insert, inScope *scope) (outScope *scope) {
	...
	mb.insertOverriding = ins.Overriding
...
func (mb *mutationBuilder) buildInputForInsert(inScope *scope, inputRows *tree.Select) {
	...
	for i := range mb.outScope.cols {
		...
		// written to, unless OVERRIDING SYSTEM VALUE is in effect (which COPY
		// FROM uses, matching PostgreSQL). When overridden, the supplied value
		// is used, exactly as for a GENERATED BY DEFAULT AS IDENTITY column.
		//
		// TODO(#68201): expose the OVERRIDING SYSTEM VALUE syntax for INSERT so
		// users can override a GENERATED ALWAYS AS IDENTITY column directly.
		if col := mb.tab.Column(ord); col.IsGeneratedAlwaysAsIdentity() &&
			mb.insertOverriding != tree.OverridingModeSystemValue {
