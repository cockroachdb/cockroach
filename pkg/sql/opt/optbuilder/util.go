...
func tableOrdinals(tab cat.Table, k columnKinds) (tableOrdinals, computedColOrdinals []int) {
	...
	for i := 0; i < n; i++ {
		...
		// We do this check for all columns in metadata instead of just ordinals
		// qualified by `shouldInclude`, to match the old looping logic in
		// addComputedColsForTable. We may want to move it into the `shouldInclude`
		// conditional, but more research and testing would be needed to determine
		// if it is safe to do so.
		if col.IsComputed() && (!col.IsMutation() || (k.includeMutations && col.IsVirtualComputed())) {
