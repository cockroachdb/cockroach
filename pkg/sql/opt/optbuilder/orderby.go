...
// already rendered in projectionsScope then that render is re-used.
func (b *Builder) addOrderingColumn(expr tree.TypedExpr, inScope, projectionsScope, orderByScope *scope) {
...
func (b *Builder) buildOrderByIndex(order *tree.Order, inScope, projectionsScope, orderByScope *scope) {
	...
	for i, n := 0, index.KeyColumnCount(); i < n; i++ {
		...
		exprs := b.expandStarAndResolveType(tree.NewColumnItem(tn, tree.Name(col.Column.ColName())), inScope)
		if len(exprs) != 1 {
			panic("single column should never have a star and should always expand to one column")
		}
		b.addOrderingColumn(exprs[0], inScope, projectionsScope, orderByScope)
	}
