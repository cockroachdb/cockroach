	...
	if nullsOrderingCount > 0 {
		...
		for i := range orderByScope.cols {
			...
			if orderByCol.nonDefaultNullsOrder {
				...
				// Use the ORDER BY column's metadata name if available,
				// otherwise fall back to the column's metadata alias.
				colName := orderByCol.name.MetadataName()
				if colName == "" {
					colName = b.factory.Metadata().ColumnMeta(orderByCol.id).Alias
				}
				metadataName := fmt.Sprintf("nulls_ordering_%s", colName)
