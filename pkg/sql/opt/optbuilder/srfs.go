...
func (b *Builder) buildZip(exprs tree.Exprs, inScope *scope) (outScope *scope) {
	...
	for i, expr := range exprs {
		...
		if def == nil || def.Class != tree.GeneratorClass || (len(texpr.ResolvedType().TupleLabels()) == 0 && texpr.ResolvedType() != types.EmptyTuple) {
...
func (b *Builder) finishBuildGeneratorFunction(
	...
	if f.ResolvedType() == types.EmptyTuple {
		// An EmptyTuple has neither labels nor columns,
