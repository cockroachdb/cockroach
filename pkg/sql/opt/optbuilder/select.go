...
func (b *Builder) buildScan(
	...
	if tab.IsVirtualTable() {
		if indexFlags != nil {
			panic(builderError{pgerror.NewErrorf(
				pgerror.CodeInvalidTableDefinitionError, "index flags not allowed with virtual tables",
			)})
		}
		...
		if indexFlags != nil {
			...
			if indexFlags.Index != "" {
				...
				if idx == -1 {
					panic(builderError{pgerror.NewErrorf(
						pgerror.CodeInvalidTableDefinitionError, "index %q not found",
						tree.ErrString(&indexFlags.Index),
					)})
				}
