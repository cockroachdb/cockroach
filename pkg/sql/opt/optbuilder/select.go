...
// return values.
func (b *Builder) buildDataSource(texpr tree.TableExpr, inScope *scope) (outScope *scope) {
	...
	case *tree.AliasedTableExpr:
		...
		outScope = b.buildDataSource(source.Expr, inScope)
	...
	case *tree.NormalizableTableName:
		...
		switch t := ds.(type) {
		case opt.Table:
			return b.buildScan(t, tn, nil /* ordinals */, inScope)
		case opt.View:
	...
	case *tree.ParenTableExpr:
		return b.buildDataSource(source.Expr, inScope)
	...
	case *tree.TableRef:
		...
		switch t := ds.(type) {
		case opt.Table:
			outScope = b.buildScanFromTableRef(t, source, inScope)
		default:
...
func (b *Builder) buildView(view opt.View, inScope *scope) (outScope *scope) {
	// Cache parsed text so that multiple references won't need to reparse.
	if b.views == nil {
...
func (b *Builder) buildScanFromTableRef(
	...
	return b.buildScan(tab, tab.Name(), ordinals, inScope)
}
...
func (b *Builder) buildFrom(from *tree.From, where *tree.Where, inScope *scope) (outScope *scope) {
	...
	for _, table := range from.Tables {
		tableScope := b.buildDataSource(table, inScope)
