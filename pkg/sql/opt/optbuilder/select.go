...
func (b *Builder) buildView(
	...
	skipAllPrivilegeChecks := sqlclustersettings.SkipAllViewPrivilegeChecks.Get(&b.evalCtx.Settings.SV)
	...
	// entirely since they are system-defined and always valid. We also
	// DisableUnsafeInternalCheck to allow reading system tables through views.
	//
	// The SkipAllViewPrivilegeChecks cluster setting can be used to revert to
	// the pre-v25.2 behavior where no privilege checks were performed on the
	// underlying tables.
	if skipAllPrivilegeChecks || view.IsSystemView() {
