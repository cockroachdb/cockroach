...
func (b *Builder) buildDataSource(
	...
	case *tree.TableName:
		...
		if cte := inScope.resolveCTE(tn); cte != nil {
			...
			outScope.expr = b.factory.ConstructLetRef(&memo.LetRefPrivate{
				ID:      cte.id,
				InCols:  inCols,
				OutCols: outCols,
			})
...
func (b *Builder) buildCTE(
	...
	for i := range ctes {
		...
		for i := range cols {
			passthrough.Add(cols[i].id)
		}
		cteScope.expr = b.factory.ConstructProject(cteScope.expr, memo.ProjectionsExpr{}, passthrough)

		id := b.factory.Memo().AddLetExpr(cteScope.expr)
...
// wrapWithCTEs adds Let expressions on top of an expression.
func (b *Builder) wrapWithCTEs(expr memo.RelExpr, ctes []cteSource) memo.RelExpr {
	for i := len(ctes) - 1; i >= 0; i-- {
		expr = b.factory.ConstructLet(
			ctes[i].expr,
			expr,
			&memo.LetPrivate{
				ID:           ctes[i].id,
