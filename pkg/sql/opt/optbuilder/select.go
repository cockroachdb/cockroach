...
func (b *Builder) buildSelect(stmt *tree.Select, inScope *scope) (outScope *scope) {
	...
	if outScope.physicalProps.Ordering == nil && orderBy != nil {
		...
		outScope = b.constructProject(projectionsScope, outScope)
	}

	if limit != nil {
		outScope = b.buildLimit(limit, inScope, outScope)
	}
...
func (b *Builder) buildFrom(from *tree.From, where *tree.Where, inScope *scope) (outScope *scope) {
	var left memo.GroupID

	for _, table := range from.Tables {
		var rightScope *scope
		rightScope = b.buildTable(table, inScope)

		if left == 0 {
			left = rightScope.group
			outScope = rightScope
			continue
		}
		...
		left = b.factory.ConstructInnerJoin(left, rightScope.group, b.factory.ConstructTrue())
	}
