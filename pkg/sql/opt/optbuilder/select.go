	...
	case *tree.TableName:
		...
		case cat.Table:
			...
			return b.buildScan(
				...
				// Enable not visible index feature because this scan is for buildJoin,
				// buildFromWithLateral, and buildFromTablesRightDeep (called by
				// buildInputForUpdate, buildSelectClause).
				false,
			)
...
func (b *Builder) buildScanFromTableRef(
	...
	// Enable not visible index feature because this scan is for a normal SELECT
	// query.
