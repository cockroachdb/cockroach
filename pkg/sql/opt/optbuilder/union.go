...
func (b *Builder) buildUnion(clause *tree.UnionClause, inScope *scope) (outScope *scope) {
	...
	if clause.All {
		switch clause.Type {
		case tree.UnionOp:
			outScope.node = b.factory.ConstructUnionAll(left, right, &private)
		case tree.IntersectOp:
			outScope.node = b.factory.ConstructIntersectAll(left, right, &private)
		case tree.ExceptOp:
			outScope.node = b.factory.ConstructExceptAll(left, right, &private)
		}
	} else {
		switch clause.Type {
		case tree.UnionOp:
			outScope.node = b.factory.ConstructUnion(left, right, &private)
		case tree.IntersectOp:
			outScope.node = b.factory.ConstructIntersect(left, right, &private)
		case tree.ExceptOp:
			outScope.node = b.factory.ConstructExcept(left, right, &private)
		}
	}
