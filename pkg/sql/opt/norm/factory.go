...
func (f *Factory) DisableOptimizations() {
	f.OnRuleMatch(func(opt.RuleName) bool { return false })
}

// OnRuleMatch sets a callback function which is invoked each time a normalize
// rule has been matched by the factory. If the function returns false, then the
// rule is not applied. By default, all rules are applied, but callers can set
// the callback function to override the default behavior. In addition, callers
// can invoke the DisableOptimizations convenience method to disable all rules.
func (f *Factory) OnRuleMatch(onRuleMatch func(ruleName opt.RuleName) bool) {
