...
// replacement node, or else delegating to the CopyAndReplaceDefault method,
// which simply constructs a copy of the source operator using children returned
// by recursive calls to the replace callback. Here is example usage:
//
...
//   }
//   f.CopyAndReplace(from, fromProps, replace)
//
...
func (f *Factory) CopyAndReplace(
	...
	// factory's memo.
	f.Memo().SetRoot(replace(from).(memo.RelExpr), fromProps)
}
...
func (f *Factory) AssignPlaceholders(from *memo.Memo) (err error) {
	...
	f.CopyAndReplace(from.RootExpr().(memo.RelExpr), from.RootProps(), replace)
