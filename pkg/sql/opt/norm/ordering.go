...
// CanSimplifyLimitOrdering returns true if the ordering required by the Limit
// operator can be made less restrictive, so that the input operator has more
// ordering choices.
func (c *CustomFuncs) CanSimplifyLimitOrdering(input memo.GroupID, def memo.PrivateID) bool {
	ordering := c.f.mem.LookupPrivate(def).(*props.OrderingChoice)
	return c.canSimplifyOrdering(input, ordering)
}

// SimplifyLimitOrdering makes the ordering required by the Limit operator less
// restrictive by removing optional columns, adding equivalent columns, and
// removing redundant columns.
func (c *CustomFuncs) SimplifyLimitOrdering(input memo.GroupID, def memo.PrivateID) memo.PrivateID {
...
func (c *CustomFuncs) simplifyOrdering(
	...
	if len(simplified.Columns) == 0 {
		// Discard any optional columns as well if there are no required columns.
		simplified = props.OrderingChoice{}
	}
	return simplified
}
