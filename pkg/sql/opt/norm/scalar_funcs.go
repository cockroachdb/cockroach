...
func (c *CustomFuncs) SplitTupleEq(tuple, rhs *memo.TupleExpr) memo.FiltersExpr {
	if len(tuple.Elems) != len(rhs.Elems) {
		panic(errors.AssertionFailedf("unequal tuple lengths"))
	}
	res := make(memo.FiltersExpr, len(tuple.Elems))
	for i := range res {
		res[i] = c.f.ConstructFiltersItem(c.f.ConstructEq(tuple.Elems[i], rhs.Elems[i]))
	}
	return res
}
