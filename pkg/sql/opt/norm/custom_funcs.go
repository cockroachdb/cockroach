...
// IsContradiction returns true if any of the given filters is false or contains
// a contradiction constraint. This returns true in a superset of the cases that
// IsFilterFalse returns true.
func (c *CustomFuncs) IsContradiction(filters memo.FiltersExpr) bool {
	for i := range filters {
		item := &filters[i]
		if item.Condition.Op() == opt.FalseOp {
			return true
		}
		if item.ScalarProps(c.mem).Constraints == constraint.Contradiction {
			return true
		}
	}
	return false
...
func (c *CustomFuncs) ExtractBoundConditions(
	...
	newFilters := make(memo.FiltersExpr, 0, len(filters)-1)
	for i := range filters {
...
func (c *CustomFuncs) SimplifyFilters(filters memo.FiltersExpr) memo.FiltersExpr {
	// This code assumes that the NormalizeNestedAnds rule has already run and
	// ensured a left deep And tree. If not (maybe because it's a testing
	// scenario), then this rule may rematch, but it should still make forward
	// progress).
	cnt := 0
...
// it returns propagates a false return value all the up the call stack, and
