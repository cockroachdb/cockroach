...
func (c *CustomFuncs) ExtractRedundantSubclause(
	...
	for _, item := range c.f.mem.LookupList(conditions) {
		...
		default:
			// The default case should be fully handled by the ExtractRedundantClause
			// rule, but include it here just in case. If we reach this point in the
			// code, we have a boolean expression such as `(A AND B) OR A`, which is
			// logically equivalent to `A`. (`A` is the redundant subclause).
			if item == subclause {
				return item
			}
