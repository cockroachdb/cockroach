...
// MapFilterOnLeft maps the filter onto the left expression by replacing the
// out columns of the filter with the appropriate corresponding columns in the
// left side of the operator.
// Useful for pushing filters to relations the set operation is composed of.
func (c *CustomFuncs) MapFilterOnLeft(
	filter *memo.FiltersItem, set *memo.SetPrivate,
) opt.ScalarExpr {
	return c.setMap(filter, set.OutCols, set.LeftCols)
}

// MapFilterOnRight maps the filter onto the right expression by replacing the
// out columns of the filter with the appropriate corresponding columns in the
// right side of the operator.
// Useful for pushing filters to relations the set operation is composed of.
func (c *CustomFuncs) MapFilterOnRight(
	filter *memo.FiltersItem, set *memo.SetPrivate,
) opt.ScalarExpr {
	return c.setMap(filter, set.OutCols, set.RightCols)
}
