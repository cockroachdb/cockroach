...
// statement. If only the ELSE remains, its expression is returned. condition
// must be a ConstValue.
	...
	for _, item := range whenList {
		...
		case opt.WhenOp:
			...
			if nwc.IsConstValue() {
				...
				// the ELSE (or just return it if there are no other values).
				if lb.Empty() {
					return when.Value()
				}
		...
		default:
			if lb.Empty() {
				// No ELSE is the only clause (there are no WHENs), remove the CASE.
				return item
			}
		}
		lb.AddItem(item)
