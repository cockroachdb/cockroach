...
func (c *CustomFuncs) FoldBinary(op opt.Operator, left, right memo.GroupID) memo.GroupID {
	...
	lType, rType := lDatum.ResolvedType(), rDatum.ResolvedType()

	if lType == types.Unknown {
		lType = lEv.Private().(types.T)
	}
	if rType == types.Unknown {
		rType = rEv.Private().(types.T)
	}
