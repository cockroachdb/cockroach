...
// CanInlineConstVal returns true if there is an opportunity in the filters to
...
func (c *CustomFuncs) CanInlineConstVar(f memo.FiltersExpr) bool {
	...
	for i := range f {
		if ok, l, _ := c.extractEq(f[i].Condition); ok {
...
func (c *CustomFuncs) InlineConstVar(f memo.FiltersExpr) memo.FiltersExpr {
	...
	for i := range f {
		if ok, v, e := c.extractEq(f[i].Condition); ok {
	...
	for i := range f {
		...
		result[i] = memo.FiltersItem{Condition: replace(f[i].Condition).(opt.ScalarExpr)}
	}
	return result
}
