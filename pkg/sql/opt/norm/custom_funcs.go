...
func (c *CustomFuncs) CanMap(filters, src, dst memo.GroupID) bool {
	...
	for i, ok := c.OuterCols(src).Next(0); ok; i, ok = c.OuterCols(src).Next(i + 1) {
		// We do not map columns that have a composite key encoding. This is a
		// conservative measure to ensure that we don't infer filters incorrectly.
		// For example, consider this query:
		//
		//   SELECT * FROM
		//     (VALUES (1.0)) AS t1(x),
		...
		srcColType := c.f.Metadata().ColumnType(opt.ColumnID(i))
		if sqlbase.DatumTypeHasCompositeKeyEncoding(srcColType) {
			return false
		}

		eqCols := fd.ComputeEquivClosure(util.MakeFastIntSet(i))
		...
		found := false
		for j, ok := eqCols.Next(0); ok; j, ok = eqCols.Next(j + 1) {
			dstColType := c.f.Metadata().ColumnType(opt.ColumnID(j))
			// The column types must be equivalent so we do not infer invalid
			// filters. Operations that are valid with one type may be invalid
			// with a different type.
			if srcColType.Equivalent(dstColType) {
				found = true
				break
			}
		}
...
func (c *CustomFuncs) Map(filters, src, dst memo.GroupID) memo.GroupID {
	...
	// (the one with the smallest ColumnID that has a matching type) if there are
	// multiple choices.
	...
	c.OuterCols(src).ForEach(func(srcCol int) {
		...
		if eqCols.Contains(srcCol) {
			...
			for dstCol, ok := eqCols.Next(0); ok; dstCol, ok = eqCols.Next(dstCol + 1) {
				srcColType := c.f.Metadata().ColumnType(opt.ColumnID(srcCol))
				dstColType := c.f.Metadata().ColumnType(opt.ColumnID(dstCol))
				// The column types must be equivalent so we do not infer invalid
				// filters. Operations that are valid with one type may be invalid
				// with a different type. We know that at least one such equivalent
				// column exists since CanMap checks this.
				if srcColType.Equivalent(dstColType) {
