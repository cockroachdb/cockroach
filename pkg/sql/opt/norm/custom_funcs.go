...
// CanConvertToValues takes in an input ZipExpr and returns true if the
// ProjectSet to which the zip belongs can be converted to an InnerJoinApply
// with a Values operator on the right input.
func (c *CustomFuncs) CanConvertToValues(zip memo.ZipExpr) bool {
	for _, zipItem := range zip {
		fn, ok := zipItem.Fn.(*memo.FunctionExpr)
		if !ok || fn.Name != "unnest" {
			return false // Not an unnest function.
		}
		if len(fn.Args) != 1 {
			return false // Unnest has more than one argument.
		}
...
// ConstructValuesFromArrays constructs a Values operator with the elements from
...
func (c *CustomFuncs) ConstructValuesFromArrays(zip memo.ZipExpr) memo.RelExpr {
	numCols := len(zip)
