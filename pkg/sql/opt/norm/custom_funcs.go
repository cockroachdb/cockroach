...
func (c *CustomFuncs) IsConstValueEqual(const1, const2 memo.GroupID) bool {
	...
	if op1 != op2 || op1 == opt.NullOp || op2 == opt.NullOp {
		return false
	}
	switch op1 {
...
// SimplifyWhens removes known unreachable WHEN cases and constructs a new
// CASE statement. If only the ELSE remains, its expression is returned.
func (c *CustomFuncs) SimplifyWhens(condition memo.GroupID, whens memo.ListID) memo.GroupID {
	...
	for _, item := range whenList {
		...
		default:
			if lb.Empty() {
				// No WHEN clauses in the list; remove the CASE.
				return item
			}
		}
		lb.AddItem(item)
