...
// operator that are passed through by the outer.
...
// EqualsOpName returns true if the two operators are the same.
func (c *CustomFuncs) EqualsOpName(left, right opt.Operator) bool {
	return left == right
}
...
// variables expression, if that conversion can round-trip and is monotonic.
