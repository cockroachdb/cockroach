...
func (c *CustomFuncs) CommuteInequality(
	...
	panic(pgerror.NewAssertionErrorf("called commuteInequality with operator %s", op))
}
...
func (c *CustomFuncs) IsConstValueEqual(const1, const2 opt.ScalarExpr) bool {
	...
	default:
		panic(pgerror.NewAssertionErrorf("unexpected Op type: %v", op1))
	}
}
...
func (c *CustomFuncs) CastToCollatedString(str opt.ScalarExpr, locale string) opt.ScalarExpr {
	...
	case *tree.DCollatedString:
		value = t.Contents
	default:
		panic(pgerror.NewAssertionErrorf("unexpected type for COLLATE: %T", str.(*memo.ConstExpr).Value))
	}
