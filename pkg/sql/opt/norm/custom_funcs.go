...
// For a FilterItem to be pushed down, it must satisfy the following two
// conditions:
// 1) None of the columns referenced can have a composite key encoding.
// 2) None of the filter items can have a side effect. See the comment above
//    CanHaveSideEffect in props/logical.go.
func (c *CustomFuncs) separatePushDownFilters(
	filters memo.FiltersExpr) (memo.FiltersExpr, memo.FiltersExpr) {
	...
	for _, filter := range filters {
		...
		if filterProps.CanHaveSideEffects || hasCompositeKeyEncoding {
...
func (c *CustomFuncs) UnmappableFilters(filters memo.FiltersExpr) memo.FiltersExpr {
	_, sideEffectFilters := c.separatePushDownFilters(filters)
	return sideEffectFilters
}
...
func (c *CustomFuncs) HasMappableFilters(filters memo.FiltersExpr) bool {
	return len(filters) > len(c.UnmappableFilters(filters))
}
...
// propogate to that side after mapping the column IDs appropriately. WLOG,
