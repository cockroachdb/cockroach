...
func (c *CustomFuncs) ConsolidateFilters(filters memo.FiltersExpr) memo.FiltersExpr {
	...
	newFilters := make(memo.FiltersExpr, 0, len(filters)-seenTwice.Len())

	// Add an empty item for each of the new Range expressions that will be
	// created below, and keep track of which column corresponds to each
	// item.
	...
	for i := range filters {
		if col, ok := c.canConsolidateFilter(&filters[i]); ok && seenTwice.Contains(col) {
			...
			if rangeItem.Condition == nil {
				...
				rangeItem.Condition = filters[i].Condition
			} else {
				// Build a left-deep tree of ANDs.
				rangeItem.Condition = c.f.ConstructAnd(rangeItem.Condition, filters[i].Condition)
			}
		} else {
			newFilters = append(newFilters, filters[i])
		}
