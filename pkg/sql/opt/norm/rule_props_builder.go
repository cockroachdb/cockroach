...
// in props.Relational. See the comment for the props.Relational.Rules field for
// more details.
type rulePropsBuilder struct {
	mem *memo.Memo
}

// buildProps fills the props.Relational.Rules field in the given expression's
...
//              the comment header for props.Relational.Rules.PruneCols.
//
...
func (b *rulePropsBuilder) buildScanProps(ev memo.ExprView) {
	...
	relational.Rules.PruneCols = ev.Logical().Relational.OutputCols
}
...
func (b *rulePropsBuilder) buildSelectProps(ev memo.ExprView) {
	...
	relational.Rules.PruneCols = inputProps.Rules.PruneCols.Difference(filterProps.OuterCols)
}
...
func (b *rulePropsBuilder) buildJoinProps(ev memo.ExprView) {
	...
	relational.Rules.PruneCols = leftProps.Rules.PruneCols.Union(rightProps.Rules.PruneCols)
	relational.Rules.PruneCols.DifferenceWith(rightProps.OuterCols)
	relational.Rules.PruneCols.DifferenceWith(onProps.OuterCols)
}
...
func (b *rulePropsBuilder) buildGroupByProps(ev memo.ExprView) {
	...
	if groupingColSet.Empty() {
		relational.Rules.PruneCols = relational.OutputCols
	} else {
		relational.Rules.PruneCols = relational.OutputCols.Difference(groupingColSet)
	}
}

func (b *rulePropsBuilder) buildSetProps(ev memo.ExprView) {
	// Don't allow any columns to be pruned, since that would trigger the
	// creation of a wrapper Project around the Max1Row, since there's not yet
	// a PruneCols rule for set operators.
}
...
func (b *rulePropsBuilder) buildLimitProps(ev memo.ExprView) {
	...
	relational.Rules.PruneCols = inputProps.Rules.PruneCols.Difference(ordering)
}
