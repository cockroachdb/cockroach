...
func (c *CustomFuncs) CanExtractJoinEquality(
	scalar opt.ScalarExpr, leftCols, rightCols opt.ColSet,
) bool {
	// Disallow non-equality.
	eq, ok := scalar.(*memo.EqExpr)
	if !ok {
		return false
	}
	...
	memo.BuildSharedProps(c.mem, eq.Left, &leftProps)
	memo.BuildSharedProps(c.mem, eq.Right, &rightProps)
