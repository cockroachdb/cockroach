...
// JoinPreservesLeftRows returns true when the given join is guaranteed to
// return all rows from its left input. This is true when the following
// conditions are satisfied:
//
//   1. Each conjunct in the join condition is an equality between not-null
//      columns from the left input and a column from the right input.
//   2. All left input equality columns come from a single table, as do all
//      right input equality columns (can be different table).
...
//
func (c *CustomFuncs) JoinPreservesLeftRows(left, right, filters memo.GroupID) bool {
	...
	for _, condition := range c.f.mem.LookupList(filtersExpr.Conditions()) {
		...
		if !leftCols.Contains(int(leftCol)) || !rightCols.Contains(int(rightCol)) {
			// Condition #1: columns don't come from both sides of join.
			return false
		}
...
func (c *CustomFuncs) deriveUnfilteredCols(group memo.GroupID) opt.ColSet {
	...
	case opt.InnerJoinOp, opt.InnerJoinApplyOp:
		...
		// JoinPreservesLeftRows conditions.
		if isCrossJoin || c.JoinPreservesLeftRows(left, right, on) {
			relational.Rule.UnfilteredCols.UnionWith(c.deriveUnfilteredCols(left))
		}
		if isCrossJoin || c.JoinPreservesLeftRows(right, left, on) {
