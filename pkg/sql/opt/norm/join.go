...
// CanExtractTupleEquality checks that
...
func isValidTupleEqualityExpression(
	f *memo.FiltersItem, projectedCols map[opt.ColumnID]memo.ProjectionsItem,
) bool {
	...
	for _, f := range filters {
		if !f.ScalarProps().OuterCols.Intersection(projectedColsSet).Empty() {
			...
			if ok {
				...
				panic(errors.AssertionFailedf("cannot extract tuple equality from non-equality comparison"))
			}
		} else {
