...
//
// The (v = 5) and (w = 10) filters are returned in separate filters because the
// (v = 5) filter can be pushed below the Project (because it is indexed), but
// the (w = 10) filter cannot be pushed below the Project (because it is not
// indexed).
func (c *CustomFuncs) TryInlineSelectVirtualColumns(
	...
	// so, inline the virtual column expressions in the filter. Collect the
	// inlined and non-inlined filters separately.
	var inlined memo.FiltersExpr
	var notInlined memo.FiltersExpr
	for i := range filters {
		item := &filters[i]
		...
		if item.ScalarProps().HasCorrelatedSubquery || !item.ScalarProps().OuterCols.Intersects(inlinableCols) {
			// Initialize notInlined lazily.
			if notInlined == nil {
				notInlined = make(memo.FiltersExpr, 0, len(filters)-i)
			}
