...
func (c *CustomFuncs) ConvertUDFToSubquery(
	...
	// with a Project expression. The ordering is preserved in the subquery
	// expression.
	//
	// Note that a Max1Row expression is not required because optbuilder always
	// wraps the last statement of a UDF with LIMIT 1.
	stmt := udfp.Body[0]
	returnColID := stmt.PhysProps.Presentation[0].ID
	ordering := stmt.PhysProps.Ordering.ToOrdering()
