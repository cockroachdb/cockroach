...
// CollapsedRepeatedLikePatternWildcards returns a new pattern string datum with
// repeated "%" wildcards collapsed. pattern must be a Const expression. It
// returns ok=false if the pattern is not a string or there are no repeated
...
func (c *CustomFuncs) CollapseRepeatedLikePatternWildcards(
	...
	patternStr, ok := pattern.(*memo.ConstExpr).Value.(*tree.DString)
	if !ok {
		return nil, false
	}
	orig := string(*patternStr)
	collapsed := eval.CollapseWildcards(orig)
	if orig == collapsed {
		return nil, false
	}
