...
func (f *Factory) hoistSelectSubquery(input, filter memo.GroupID) memo.GroupID {
	...
	return f.ConstructProject(sel, f.projectColsFrom(input))
}
...
func (f *Factory) hoistJoinSubquery(op opt.Operator, left, right, on memo.GroupID) memo.GroupID {
	...
	projections := f.projectCols(f.outputCols(left).Union(f.outputCols(right)))
	return f.ConstructProject(join, projections)
}
...
func (f *Factory) hoistValuesSubquery(rows memo.ListID, cols memo.PrivateID) memo.GroupID {
	...
	return f.ConstructProject(join, f.projectColsFrom(values))
}
...
func (f *Factory) canAggsIgnoreNulls(aggs memo.GroupID) bool {
	...
	for _, elem := range f.mem.LookupList(aggsExpr.Aggs()) {
		...
		case opt.AvgOp, opt.BoolAndOp, opt.BoolOrOp, opt.CountOp, opt.CountRowsOp,
			...
			opt.VarianceOp, opt.StdDevOp, opt.XorAggOp, opt.ExistsAggOp, opt.FirstAggOp:
...
// constructScalarGroupByLeftJoin maps an (InnerJoinApply (GroupBy)) expression
...
//   ON left.y = 10
//   =>
//   SELECT FIRST_AGG(left.x), FIRST_AGG(left.y), COUNT(input.t), SUM(input.c)
...
//   HAVING left.y = 10
//
// FIRST_AGG is an internal aggregation function that returns the first non-null
// row in the grouping set. It actually doesn't matter which value is selected,
// as all will be the same, since the grouping column(s) are a key of "left".
func (f *Factory) hoistCorrelatedScalarGroupBy(left, input, aggs memo.GroupID) memo.GroupID {
	...
	// Add each non-key column from the left input as a FIRST_AGG
	extraColSet.ForEach(func(i int) {
		outAgg := f.ConstructFirstAgg(
			f.ConstructVariable(
...
func (r *subqueryHoister) constructGroupByExists(subquery memo.GroupID) memo.GroupID {
	...
	trueCols := r.f.InternColList(opt.ColList{trueColID})
