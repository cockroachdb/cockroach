...
//
// EXISTS_AGG (and COUNT) takes a column argument so that it will ignore null
// values. This doesn't matter immediately, as True is never null. But later on,
// the TryDecorrelateScalarGroupBy rule will push a left join into the GroupBy,
// and null values produced by that will need to be ignored by EXISTS_AGG.
...
//
//   ANY(SELECT x = y+1 FROM xy AS q(comp))
//
// into a scalar GroupBy expression that returns a one row, one column relation:
//
//   SELECT
...
//   FROM
//   (
//     SELECT comp IS NOT NULL
//     (SELECT x = y+1 FROM xy) AS q(comp)
