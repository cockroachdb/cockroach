...
type FoldingControl struct {
	...
	// attemptedStableFold is true if canFoldOperator was called with
	// VolatilityStable.
...
// AttemptedStableFold returns true if we disallowed a stable fold; can only be
// called if AllowStable was not called (since Init).
func (fc *FoldingControl) AttemptedStableFold() bool {
	if fc.allowStable {
		panic(errors.AssertionFailedf("AttemptedStableFold called in allow-stable state"))
	}
	return fc.encounteredStableFold
}

// EncounteredStableFold returns true if we allowed a stable fold; can only be
...
func (fc *FoldingControl) EncounteredStableFold() bool {
	if !fc.allowStable {
		panic(errors.AssertionFailedf("EncounteredStableFold called in disallow-stable state"))
	}
	return fc.encounteredStableFold
}
