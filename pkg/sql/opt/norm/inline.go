...
// any outer column more than one time, or if it has a subquery. For example:
//
...
func (c *CustomFuncs) HasDuplicateRefs(
	...
	for i := range projections {
		...
		var findDupRefs func(nd opt.Expr) bool
		findDupRefs = func(nd opt.Expr) bool {
			switch t := nd.(type) {
...
// operator) for any variable references to columns from the given projections
// expression. Each variable is replaced by the corresponding inlined projection
// expression.
...
func (c *CustomFuncs) inlineProjections(nd opt.Expr, projections memo.ProjectionsExpr) opt.Expr {
