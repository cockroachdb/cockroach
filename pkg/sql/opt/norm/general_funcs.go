...
func copyAndMapFiltersItem(filter *memo.FiltersItem, colMap opt.ColMap) *memo.FiltersItem {
	...
	newFilters.ScalarProps().CopyScalarProps(filter.ScalarProps(), colMap)
	return newFilters
}
...
// fromFilter are shared and reused in the new FiltersItem. If fromFilter's
// Spans are mutable, the normal ConstructFiltersItem function is used to build
// the new FiltersItem.
