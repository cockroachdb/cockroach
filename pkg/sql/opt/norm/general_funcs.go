...
// remapColWithIdenticalType returns a column in to that is equivalent to fromCol
// and has the identical type. It returns ok=false if no such column exists.
func (c *CustomFuncs) remapColWithIdenticalType(
	...
	for toCol, ok := to.Next(0); ok; toCol, ok = to.Next(toCol + 1) {
		toColType := md.ColumnMeta(toCol).Type
		if fromColType.Identical(toColType) && fds.AreColsEquiv(fromCol, toCol) {
			return toCol, true
		}
	}
	return 0, false
