...
// crdb_internal.get_index_tuples and converts it into an
...
// IsIndexScanBuiltin determines if crdb_internal.get_index_tuples
...
func (c *CustomFuncs) IsIndexScanBuiltin(zip memo.ZipExpr) bool {
	...
	if functionExpr.Name == "crdb_internal.get_index_tuples" {
		...
		if _, ok := functionExpr.Args.Child(0).(*memo.ConstExpr); !ok {
			return false
		}
		if _, ok := functionExpr.Args.Child(1).(*memo.ConstExpr); !ok {
			return false
		}
		return true
	}
