...
// Executor is a helper for testing the various optimizer components. It
...
//
// The Executor is used by tests in various sub-packages of the opt package.
...
// NewExecutor constructs a new instance of the Executor for the given SQL
// statement. Metadata used by the SQL query is accessed via the catalog.
func NewExecutor(catalog opt.Catalog, sql string) *Executor {
	return &Executor{
		catalog: catalog,
		sql:     sql,
...
// transformations applied to it. This is a straightforward translation of the
// SQL text into an expression tree.
func (e *Executor) OptBuild() (xform.ExprView, error) {
...
func (e *Executor) OptSteps() (string, error) {
...
// by the exec engine.
func (e *Executor) ExecBuild(eng exec.TestEngine) (exec.Node, error) {
	ev, err := e.Optimize()
	if err != nil {
		return nil, err
	}
...
func (e *Executor) Explain(eng exec.TestEngine) ([]tree.Datums, error) {
...
func (e *Executor) Exec(eng exec.TestEngine) (sqlbase.ResultColumns, []tree.Datums, error) {
	node, err := e.ExecBuild(eng)
	if err != nil {
		return nil, nil, err
	}
...
func (e *Executor) optimizeExpr(maxSteps xform.OptimizeSteps) (xform.ExprView, error) {
	o := xform.NewOptimizer(&e.evalCtx)
	o.MaxSteps = maxSteps
	root, required, err := e.buildExpr(o.Factory())
	if err != nil {
