...
func (tc *Catalog) resolveFK(tab *Table, d *tree.ForeignKeyConstraintTableDef) {
	...
	// Foreign key require indexes in both tables:
	//
	...
	//     left in the source table. This index needs not be unique; in fact, if an
	//     existing index as the relevant columns as a prefix, that is good enough.
	...
	// 1. Search for an existing index in the target table.
	found := false
	...
	if !found {
		...
		idx := tree.IndexTableDef{
			Name:    tree.Name(fmt.Sprintf("%s_auto_index_%s", tab.Name(), d.Name)),
