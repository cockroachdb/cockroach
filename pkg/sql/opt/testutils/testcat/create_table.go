...
OuterLoop:
	...
	// Allow specifying the name of a FK constraint to use for lookup up region
	// column values for a REGIONAL BY ROW table.
	var rbrUsingConstraintName string
	if param := stmt.StorageParams.GetVal(catpb.RBRUsingConstraintSettingName); param != nil {
