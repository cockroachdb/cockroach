...
func (tc *Catalog) ResolveFunction(
	...
	if def, err := tree.GetBuiltinFuncDefinition(&fn, path); err == nil {
		return def, nil
	}
...
func (tc *Catalog) CreateFunction(c *tree.CreateFunction) {
	...
	prefixedOverload := tree.MakePrefixedOverload("public", overload)
	def := &tree.ResolvedFunctionDefinition{
		Name:           name,
		ExplicitSchema: true,
		// TODO(mgartner): Consider setting Class and CompositeInsensitive fo
		// overloads.
		Overloads: []*tree.PrefixedOverload{prefixedOverload},
	}
	tc.udfs[name] = def
}
