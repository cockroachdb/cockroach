...
func (tc *Catalog) ResolveSchema(_ context.Context, name *cat.SchemaName) (cat.Schema, error) {
	...
	if name.ExplicitSchema {
		...
		// Two parts: D.T.
		// Try to use the current database, and be satisfied if it's sufficient
		// to find the object.
		...
		// No luck so far. Compatibility with CockroachDB v1.1: use D.public.T
		// instead.
	...
	// Neither schema or catalog was specified, so use testdb.public.
