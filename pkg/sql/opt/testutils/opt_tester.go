...
func (ot *OptTester) RunCommand(tb testing.TB, d *datadriven.TestData) string {
	...
	case "build":
		...
		if err := ot.postProcess(nd); err != nil {
			tb.Fatal(err)
		}
		return memo.FormatExpr(nd, ot.Flags.ExprFormat)
...
func (ot *OptTester) postProcess(nd opt.Expr) error {
	fillInLazyProps(nd)

	if rel, ok := nd.(memo.RelExpr); ok {
...
func fillInLazyProps(nd opt.Expr) {
	if rel, ok := nd.(memo.RelExpr); ok {
...
func (ot *OptTester) optStepsDisplay(before string, after string, os *optSteps) {
	...
	// expression.
	bestHeader := func(nd opt.Expr, format string, args ...interface{}) {
		ot.separator("=")
