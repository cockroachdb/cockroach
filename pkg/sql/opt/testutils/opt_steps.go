...
func (os *optSteps) done() bool {
	...
	return os.co != nil && os.co.remaining != 0
}
...
func (os *optSteps) next() error {
	...
	co, err := newCustomOpt(os.tester, os.steps)
	if err != nil {
		return err
	}
	...
	if os.better {
		...
		co2, err := newCustomOpt(os.tester, os.steps)
		if err != nil {
			return err
		}

		var exprs util.FastIntSet
		exprs.AddRange(int(co.lastApplied.Expr), co.o.Memo().ExprCount(co.lastApplied.Group)-1)
		co2.restrictToExprs(co.o.Memo(), co.lastApplied.Group, exprs)
		os.ev = co2.optimize()
	}
