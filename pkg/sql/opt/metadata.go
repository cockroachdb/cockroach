...
type Metadata struct {
	...
	userDefinedTypes map[uint32]*types.T
...
func (md *Metadata) CopyFrom(from *Metadata) {
	...
	if (md.userDefinedTypes) == nil {
		md.userDefinedTypes = make(map[uint32]*types.T)
	}
	for id, typ := range from.userDefinedTypes {
		md.userDefinedTypes[id] = typ
	}
...
func (md *Metadata) CheckDependencies(
	...
	for _, typ := range md.userDefinedTypes {
...
func (md *Metadata) AddUserDefinedType(typ *types.T) {
	...
	md.userDefinedTypes[typ.StableTypeID()] = typ
}

// AllUserDefinedTypes returns all user defined types contained in this query.
func (md *Metadata) AllUserDefinedTypes() map[uint32]*types.T {
	return md.userDefinedTypes
}
