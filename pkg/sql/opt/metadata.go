...
type mdTable struct {
	...
	// anns allows the table metadata to be annotated with arbitrary interfaces.
	// Each interface is keyed by its reflect.Type.
	anns map[reflect.Type]interface{}
}
...
// TableAnnotation returns the annotation with the given type that is associated
// with the given table. If the table has no such annotation, TableAnnotation
// returns nil.
func (md *Metadata) TableAnnotation(tabID TableID, annType reflect.Type) interface{} {
	tab := md.tables[tabID]
	if tab.anns == nil {
		return nil
	}
	return tab.anns[annType]
}

// SetTableAnnotation associates the given annotation with the given table. The
// annotation is associated by its reflect.Type and overwrites any existing
// annotation with the same type. It is up to the caller to ensure that such
// clashes do not occur.
//
//   type MyAnnotation string
//   md.SetTableAnnotation(TableID(1), MyAnnotation("foo"))
//   ann := md.TableAnnotation(TableID(1), reflect.TypeOf(MyAnnotation(""))
//
func (md *Metadata) SetTableAnnotation(tabID TableID, ann interface{}) {
	tab := md.tables[tabID]
	if tab.anns == nil {
		tab.anns = make(map[reflect.Type]interface{})
	}
	tab.anns[reflect.TypeOf(ann)] = ann
}
