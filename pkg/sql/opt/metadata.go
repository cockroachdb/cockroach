...
//   -- [1] -> y
//
// An operator is allowed to reuse the column ids of its input if for every
// output row there exists an input row having identical values for those
// columns. For example, is it safe for a Select to use its input's column ids
// because it only filters rows. Likewise, pass-through column ids of a Project
// can be reused.
//
...
// PartialIndexPredicates.
//
// The ExplicitCatalog/ExplicitSchema fields of the table's alias are honored so
// that its original formatting is preserved for error messages,
// pretty-printing, etc.
func (md *Metadata) DuplicateTable(
	tabID TableID, remapColumnIDs func(e ScalarExpr, colMap util.FastIntMap) ScalarExpr,
) TableID {
