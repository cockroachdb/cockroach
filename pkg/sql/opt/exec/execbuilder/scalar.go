...
func (b *Builder) buildUDF(ctx *buildScalarCtx, scalar opt.ScalarExpr) (tree.TypedExpr, error) {
	...
	// A nonzero ResultBufferID indicates that the UDF is a set-returning
...
func (b *Builder) buildRoutinePlanGenerator(
	...
	planGen := func(
		...
		for i := range stmts {
			...
			//
			// Tail-call optimization is not allowed when resultBufferID is nonzero,
			// because nonzero resultBufferID means that expressions in the body will
