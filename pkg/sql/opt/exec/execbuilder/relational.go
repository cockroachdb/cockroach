...
func (b *Builder) scanParams(
	...
	if hardLimit == 0 && softLimit == 0 {
		...
		if ok && maxResults < getParallelScanResultThreshold(b.evalCtx.TestingKnobs.ForceProductionValues) {
			...
			// If 'parallelize_scans' is true, even though we cannot guarantee
			// a sufficiently small upper bound on the number of rows scanned,
			// we still will parallelize this scan.
			parallelize = b.evalCtx.SessionData().ParallelizeScans
		}
	}
