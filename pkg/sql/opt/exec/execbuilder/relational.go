...
func (b *Builder) buildScan(scan *memo.ScanExpr) (execPlan, error) {
	...
	if scan.InvertedConstraint != nil {
		...
		// Fix the outputCols to represent what the InvertedFilterer
		// will return. We could have constructed this output earlier
		// in this function instead of here.
		_, output = b.getColumns(scan.Cols, scan.Table)
		res.outputCols = output
	}
	return res, nil
}
