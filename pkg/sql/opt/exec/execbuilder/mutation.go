...
func (b *Builder) findKeyColOrd(
	keyCol opt.ColumnID, lookupJoin *memo.LookupJoinExpr, indexOrd cat.IndexOrdinal,
) (inputColOrd int, indexColOrd int, ok bool) {
	md := b.mem.Metadata()
	table := md.Table(lookupJoin.Table)
	tableMeta := md.TableMeta(lookupJoin.Table)
...
func (b *Builder) handleSingleRowInsert(
	...
	if localityOptimizedSearch, isLOS := rightExpr.(*memo.LocalityOptimizedSearchExpr); isLOS {
		...
		out.MkErr = func(values tree.Datums) error {
			// The first key columns may be hash shard or czrdb_region columns.
			// Pass the appropriate slice of key columns for the uniqueness check.
			return mkUniqueCheckErr(md, c, values[len(values)-len(c.KeyCols):])
		}
	} else {
		...
		out.MkErr = func(values tree.Datums) error {
			return mkUniqueCheckErr(md, c, values)
		}
	}
...
func (b *Builder) tryBuildFastPathInsert(ins *memo.InsertExpr) (_ execPlan, ok bool, _ error) {
	...
	if projectExpr, ok = insInput.(*memo.ProjectExpr); ok {
		...
		if !projectExpr.Passthrough.Equals(projectExpr.Input.Relational().OutputCols) {
			return execPlan{}, false, nil
		}
	...
	for i := range ins.UniqueChecks {
		...
		if len(lookupJoin.LookupExpr) == 0 {
			...
			for _, joinFilter := range lookupJoin.LookupExpr {
				...
				if InExpr, ok = joinFilter.Condition.(*memo.InExpr); ok {
					...
					inputColOrd, indexColOrd, foundKeyColOrd := b.findKeyColOrd(keyCol, lookupJoin, lookupJoin.Index)
					if !foundKeyColOrd {
						return execPlan{}, false, nil
					}
			...
			if InExprSeen {
				// If there's an IN expression the first INSERT value is a crdb_region
				// column value, or some other value which is not part of the UNIQUE
				// WITHOUT INDEX constraint, so don't include it in the error message.
				out.MkErr = func(values tree.Datums) error {
					return mkUniqueCheckErr(md, c, values[1:])
				}
			} else {
				out.MkErr = func(values tree.Datums) error {
					return mkUniqueCheckErr(md, c, values)
		...
		var ignorableNECheck func(expr opt.ScalarExpr) bool
		// ignorableNECheck checks that any ON clause predicates are only of the
		...
		// processing.
		ignorableNECheck = func(expr opt.ScalarExpr) bool {
			switch t := expr.(type) {
			case *memo.OrExpr:
				return ignorableNECheck(t.Left) && ignorableNECheck(t.Right)
			case *memo.NeExpr:
				...
				_, _, ok = b.findKeyColOrd(rightVariable.Col, lookupJoin, 0)
				if !ok {
					return false
				}
				return true
		...
		for _, onFilter := range lookupJoin.On {
			if !ignorableNECheck(onFilter.Condition) {
				return execPlan{}, false, nil
			}
		}
