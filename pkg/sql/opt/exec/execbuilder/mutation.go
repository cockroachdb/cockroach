...
func (b *Builder) ignorablePKNEConstCheck(expr opt.ScalarExpr, tableID opt.TableID) (ok bool) {
	switch t := expr.(type) {
	case *memo.OrExpr:
		return b.ignorablePKNEConstCheck(t.Left, tableID) && b.ignorablePKNEConstCheck(t.Right, tableID)
	case *memo.NeExpr:
		...
		leftVariable := t.Left.(*memo.VariableExpr)
		_, _, ok = b.findKeyColOrd(leftVariable.Col, tableID, 0 /* primary index */)
		if !ok {
			return false
		}
		return true
...
func (b *Builder) handleSingleRowInsert(
	...
	if localityOptimizedSearch, isLOS := rightExpr.(*memo.LocalityOptimizedSearchExpr); isLOS {
		...
		if selectExpr != nil {
			...
			if !b.ignorablePKNEConstCheck(selectExpr.Filters[0].Condition, scanExpr.Table) {
				return false
			}
		}
