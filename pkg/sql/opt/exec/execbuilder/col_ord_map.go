...
func (m *colOrdMap) Set(col opt.ColumnID, ord int) {
	...
	case m.ords[col] > 0 && m.ords[col] == int32(m.maxOrd) && ord < m.maxOrd:
		...
		m.setUnknownMax()
	default:
		// Otherwise, so the new known maximum ordinal.
...
//
// In most cases, MaxOrd has a constant time complexity. If the map had a
// previous maximum ordinal that was overwritten by a smaller ordinal, then
// MaxOrd has a linear time complexity with respect to the size of the map. This
// case should be rare in practice. The benefit of is approach is that it allows
// for constant time complexity in most cases with only constant additional
// space. Making MaxOrd constant-time in all cases would require non-constant
// additional space to keep a history of all previous maximum ordinals.
