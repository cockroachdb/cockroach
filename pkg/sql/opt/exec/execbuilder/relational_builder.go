...
func (b *Builder) indexConstraintAllPointSpans(scan *memo.ScanExpr) bool {
	...
	// There are two ways for indexCols to form a strict key:
	//
	// the cols form a strict key in the FuncDeps; or
	// the cols form a weak key and they are all non-null.
	//
	// IF neither of these conditions hold, we don't have all point spans.

	funcDeps := scan.Relational().FuncDeps

	if !funcDeps.ColsAreStrictKey(indexCols) &&
		!(funcDeps.ColsAreLaxKey(indexCols) && indexCols.SubsetOf(scan.Relational().NotNullCols)) {
		return false
	}
