...
func (b *Builder) extractFromOffset(e opt.ScalarExpr) (opt.ScalarExpr, bool) {
...
func (b *Builder) extractToOffset(e opt.ScalarExpr) (opt.ScalarExpr, bool) {
	if opt.IsWindowOp(e) || opt.IsAggregateOp(e) {
		return nil, false
	}
...
func (b *Builder) buildFrame(input execPlan, w *memo.WindowsItem) (*tree.WindowFrame, error) {
	...
	if boundExpr, ok := b.extractFromOffset(w.Function); ok {
		if !b.isOffsetMode(newDef.Bounds.StartBound.BoundType) {
			panic(pgerror.AssertionFailedf("expected offset to only be present in offset mode"))
		}
		// Copy StartBound so we're not mutating the existing frame.
		bound := *newDef.Bounds.StartBound
		newDef.Bounds.StartBound = &bound
	...
	if boundExpr, ok := b.extractToOffset(w.Function); ok {
		...
		// Copy EndBound so we're not mutating the existing frame.
		bound := *newDef.Bounds.EndBound
		newDef.Bounds.EndBound = &bound
