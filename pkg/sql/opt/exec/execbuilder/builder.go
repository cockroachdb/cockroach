...
func (b *Builder) wrapFunction(fnName string) tree.ResolvableFunctionReference {
	if b.semaCtx != nil && b.semaCtx.FunctionResolver != nil {
		name := tree.MakeUnresolvedName(fnName)
		fnDef, err := b.semaCtx.FunctionResolver.ResolveFunction(context.Background(), &name, b.semaCtx.SearchPath)
		if err != nil {
			panic(err)
		}
