...
func NewPlanGistFactory(wrappedFactory exec.Factory) *PlanGistFactory {
	...
	// anywhere we might want to bump this when adding/removing operators.
	f.encodeInt(version)
	return f
}
...
func (f *PlanGistFactory) encodeScanParams(params exec.ScanParams) {
	// TODO: I think most of this stuff isn't needed, presence of
	// constraints seems important, maybe just a span count?
	s := params.NeededCols
