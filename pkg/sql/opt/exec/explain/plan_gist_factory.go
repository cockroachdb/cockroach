...
// The notion is that the gist is the rough shape of the plan that
...
// dropping on the exec builder phase of compilation. It
...
func NewPlanGistFactory(wrappedFactory exec.Factory) *PlanGistFactory {
	...
	// TBD: not sure exactly how this will be used but if gists are stored durably
	// anywhere we might want to bump this when adding/removing operators.  That
	// would needlessly reject perfectly fine gists that didn't use those
	// operators.
	f.encodeInt(version)
	return f
}
...
func (f *PlanGistFactory) ConstructPlan(
	...
	// stamp the end, not sure we need this?
	f.encodeByte(byte(unknownOp))
	return plan, err
}
