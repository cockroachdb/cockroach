...
func init() {
	if exportOp != 56 {
		...
		// invalidate existing plan hashes.  If we are just adding an operator at
		// the end there's no need to update version below.
...
// use.
//
// A PlanGist starts out with just the []bytes and the string rep and hash are
...
type PlanGistFactory struct {
	...
	// buffer is used for reading and writing (ie decoding and encoding) but on
	...
	// hash.  This allows the hash to be id agnostic (ie hash's will be stable across
...
// encodeDataSource encodes Table's and Index's and handles doing an id
// based encoding to the gist and a symbolic encoding to the hash.
