...
func (e *emitter) emitNodeAttributes(n *Node) error {
	if stats, ok := n.annotations[exec.ExecutionStatsID]; ok {
		...
		e.ob.AddField("actual row count", humanize.Comma(int64(s.RowCount)))
	}
	...
	if stats, ok := n.annotations[exec.EstimatedStatsID]; ok {
		...
		if n.op != valuesOp && (e.ob.flags.Verbose || n.op == scanOp) {
			count := int64(math.Round(s.RowCount))
			if s.TableStatsAvailable {
				e.ob.AddField("estimated row count", humanize.Comma(count))
			} else {
				// No stats available.
				if e.ob.flags.Verbose {
					e.ob.Attrf("estimated row count", "%s (missing stats)", humanize.Comma(count))
				} else {
