...
func (e *emitter) emitNodeAttributes(n *Node) error {
	if stats, ok := n.annotations[exec.EstimatedStatsID]; ok {
		...
		// and accurate.
		if n.op != valuesOp && (e.ob.flags.Verbose || (s.TableStatsAvailable && n.op == scanOp)) {
			count := int(math.Round(s.RowCount))
			extra := ""
			if !s.TableStatsAvailable {
				extra = " (missing stats)"
			}
			e.ob.Attrf("estimated row count", "%d%s", count, extra)
		}
		e.ob.VAttrf("estimated cost", "%.9g", s.Cost)
	}
