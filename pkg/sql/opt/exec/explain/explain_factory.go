...
func (f *Factory) ConstructPlan(
	...
	for i := range wrappedCascades {
		...
		wrappedCascades[i].PlanFn = func(
			...
			var err error
			cascadePlan, err = origPlanFn(ctx, semaCtx, evalCtx, f, buffer, numBufferedRows, allowAutoCommit)
			if err != nil {
				return nil, err
			}
			return cascadePlan.(*Plan).WrappedPlan, nil
		}
		cascades[i].GetExplainPlan = func(ctx context.Context) (exec.Plan, error) {
			if cascadePlan != nil {
				// If we already created the plan, then use the cached one.
