...
// Options denotes a single input value to a plan, along with the set of legal
...
func (it *configIterator) Next() (Configuration, bool) {
	...
	for j := range it.choices {
		config[it.choices[j].Field] = it.choices[j].Values[it.state[j]]
	}
	it.increment()
...
func (it *configIterator) increment() {
	...
	for i < len(it.choices) {
		it.state[i]++
		if it.state[i] < len(it.choices[i].Values) {
			break
		}
		it.state[i] = 0
		i++
