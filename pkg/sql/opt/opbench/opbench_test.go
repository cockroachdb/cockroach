...
func TestOpBench(t *testing.T) {
	...
	for _, spec := range opbench.Benches {
...
// measureQuery runs a query against a running Cockroach and records how long
// it takes to run.
func measureQuery(planText string) (int64, error) {
	db, err := sql.Open("postgres", *pgurl)
	if err != nil {
		return 0, errors.Wrap(err, "can only recompute actual results when pointed at a running Cockroach")
	}
...
// of possible inputs for the Spec so that the framework can fill in the
// blanks.
func getBlankCSV(spec *opbench.Spec) io.Reader {
	...
	for ok {
		...
		for _, t := range inputs {
			rec = append(rec, fmt.Sprintf("%d", int(c.Get(t))))
		}
