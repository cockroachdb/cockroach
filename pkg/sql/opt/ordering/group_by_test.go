...
func TestDistinctOnProvided(t *testing.T) {
	...
	for tcIdx, tc := range testCases {
		t.Run(fmt.Sprintf("case%d", tcIdx+1), func(t *testing.T) {
			...
			tc.outCols.Difference(tc.groupingCols).ForEach(func(col int) {
				aggs = append(aggs, memo.AggregationsItem{
					Agg:        f.ConstructFirstAgg(f.ConstructVariable(opt.ColumnID(col))),
					ColPrivate: memo.ColPrivate{Col: opt.ColumnID(col)},
				})
			})
