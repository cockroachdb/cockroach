...
func lookupJoinCanProvideOrdering(expr memo.RelExpr, required *props.OrderingChoice) bool {
	...
	canProjectOnlyInputCols := required.CanProjectCols(inputCols)

	if canProjectOnlyInputCols && lookupJoin.IsSecondJoinInPairedJoiner {
	...
	if !canProjectOnlyInputCols {
		...
		// ordering columns from the index must be ASC. The following is a case
		// where a lookup join could maintain an ordering over both input and index
		// columns:
		...
		requiredCols := remainingRequired.ColSet()
		for i := 0; i < idx.KeyColumnCount(); i++ {
			...
			if idx.Column(i).Descending {
				// Index columns that take part in the ordering cannot be descending.
				return false
			}
	...
	return canProjectOnlyInputCols
}
