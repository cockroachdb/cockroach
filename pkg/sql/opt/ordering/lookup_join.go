...
func lookupJoinBuildProvided(expr memo.RelExpr, required *props.OrderingChoice) opt.Ordering {
	...
	if provided, toExtend = trySatisfyRequired(required, provided); toExtend != nil {
		...
		if lookupProvided, ok := getLookupOrdCols(lookupJoin, toExtend, provided.ColSet()); ok {
...
// is the set of columns referenced by the input ordering.
func getLookupOrdCols(
	...
	optionalCols := required.Optional.Union(inputOrderingCols)
