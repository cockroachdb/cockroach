...
func lookupOrIndexJoinCanProvideOrdering(expr memo.RelExpr, required *props.OrderingChoice) bool {
	...
	if !canProjectCols {
		...
		for i := 0; i < idx.KeyColumnCount(); i++ {
			...
			idxColDir := idx.Column(i).Descending
			indexOrder[i] = opt.MakeOrderingColumn(idxColID, idxColDir)
		}
...
func trySatisfyRequired(
	...
	for choiceIdx < len(required.Columns) {
		if providedIdx >= len(provided) {
			// There are still required columns remaining, but we have reached the end
			// of the provided columns list.
			break
		}
		...
		if choice.Descending != providedCol.Descending() {
			// The provided ordering must have the same column directions as the
			// OrderingChoice.
			break
		}
