...
respect at an ordering in the OrderingChoice set, perhaps by requiring specific
orderings of its inputs and/or configuring its execution in a specific way. This
package implements the logic that decides whether each operator can provide a
Required ordering, as well as what Required orderings on its input(s) are
necessary.
...
operator logically has a natural ordering, if multiple instances of that
...
²For example, imagine that node A produces rows (1,3), (1,4) and node B produces
rows (2,1), (2,2). If we configure execution to "maintain" an ordering of +b, it
will cause incorrect results or a runtime error.
