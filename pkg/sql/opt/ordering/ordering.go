...
// FDs) so that it only refers to columns in the given set. It also removes any
// columns that are redundant according to the FDs.
//
...
func checkProvided(expr memo.RelExpr, required *physical.OrderingChoice, provided opt.Ordering) {
	...
	// The provided ordering must intersect with the required ordering, after FDs
	// are applied.
