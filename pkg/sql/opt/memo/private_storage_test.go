...
func TestInternFuncOpDef(t *testing.T) {
	...
	nowDef := builtins.Builtins["now"]
	funcDef1 := &FuncOpDef{Name: "foo", Type: ttuple1, Properties: &nowDef.FunctionProperties, Overload: &nowDef.Overloads[0]}
	funcDef2 := &FuncOpDef{Name: "bar", Type: ttuple2, Properties: &nowDef.FunctionProperties, Overload: &nowDef.Overloads[0]}
	test(funcDef1, funcDef2, true)
	funcDef3 := &FuncOpDef{Name: "bar", Type: ttuple2, Properties: &nowDef.FunctionProperties, Overload: &nowDef.Overloads[1]}
...
func TestPrivateStorageAllocations(t *testing.T) {
	...
	op := opt.PlusOp
	concatDef := builtins.Builtins["concat"]
	funcOpDef := &FuncOpDef{
		Name:       "concat",
		Type:       types.String,
		Properties: &concatDef.FunctionProperties,
		Overload:   &concatDef.Overloads[0],
	}
