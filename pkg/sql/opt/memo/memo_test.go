...
func TestMemoIsStale(t *testing.T) {
	...
	stale := func() {
		t.Helper()
		if isStale, err := o.Memo().IsStale(ctx, &evalCtx, catalog, false /* ignoreCanaryStats */); err != nil {
			t.Fatal(err)
		} else if !isStale {
		...
		if isStale, err := o2.Memo().IsStale(ctx, &evalCtx, catalog, false /* ignoreCanaryStats */); err != nil {
			t.Fatal(err)
		} else if isStale {
	...
	_, err = o.Memo().IsStale(ctx, &evalCtx, catalog, false /* ignoreCanaryStats */)
