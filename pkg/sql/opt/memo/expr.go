...
// initUnexportedFields is called when a scan expression is created.
func (s *ScanExpr) initUnexportedFields(mem *Memo) {
	md := mem.Metadata()
	hardLimit := s.HardLimit.RowCount()
	pred := s.PartialIndexPredicate(md)
	tabFD := MakeTableFuncDep(md, s.Table)
	cols := tabFD.ColSet()

	// Initialize not-NULL columns from the table schema.
	s.notNullCols = tableNotNullCols(md, s.Table)
	// Union not-NULL columns with not-NULL columns in the constraint.
	if s.Constraint != nil {
		s.notNullCols.UnionWith(s.Constraint.ExtractNotNullCols(mem.logPropsBuilder.evalCtx))
	}
	// Union not-NULL columns with not-NULL columns in the partial index
	// predicate.
	if pred != nil {
		s.notNullCols.UnionWith(mem.logPropsBuilder.rejectNullCols(pred))
	}

	// Determine the "internal" functional dependencies (for all columns in the scan,
	// including those that are not output).
	...
	if hardLimit == 1 {
		s.internalFuncDeps.MakeMax1Row(cols)
	} else {
		...
		s.internalFuncDeps.CopyFrom(tabFD)
		if s.Constraint != nil {
			s.internalFuncDeps.AddConstants(s.Constraint.ExtractConstCols(mem.logPropsBuilder.evalCtx))
		}
		if tabMeta := md.TableMeta(s.Table); tabMeta.Constraints != nil {
			mem.logPropsBuilder.addFiltersToFuncDep(
				*tabMeta.Constraints.(*FiltersExpr), &s.internalFuncDeps,
			)
		}
		if pred != nil {
			mem.logPropsBuilder.addFiltersToFuncDep(pred, &s.internalFuncDeps)
			...
			for col := 0; col < index.LaxKeyColumnCount(); col++ {
				...
				keyCols.Add(s.Table.ColumnID(ord))
			}
			allCols := keyCols.Union(cols)
			...
			if index.LaxKeyColumnCount() < index.KeyColumnCount() {
				...
				s.internalFuncDeps.AddLaxKey(keyCols, allCols)
			} else {
				s.internalFuncDeps.AddStrictKey(keyCols, allCols)
			}
		}
		s.internalFuncDeps.MakeNotNull(s.notNullCols)
	}
}

// InternalFDs returns the functional dependencies for all columns in the scan,
// including those that are not output.
func (s *ScanExpr) InternalFDs() *props.FuncDepSet {
	return &s.internalFuncDeps
}
