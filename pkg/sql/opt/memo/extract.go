...
// ExtractJoinConditionRightColumnsWithFilterOrds returns columns from rightCols
// which are constrained by an equality or an inequality in a join (and have
// equivalent types) with columns in leftCols. The returned filterOrds contains
// ordinals of the on filters where each join condition was found. The
// inequality argument indicates whether to look for inequality conditions
// rather than equalities.
func ExtractJoinConditionRightColumnsWithFilterOrds(
	leftCols, rightCols opt.ColSet, on FiltersExpr, inequality bool,
) (rightCmp opt.ColList, filterOrds []int) {
	_, rightCmp, filterOrds = extractJoinConditionImpl(
		leftCols, rightCols, on, inequality,
