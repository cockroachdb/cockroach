...
func filtersMatchAllLeftRows(left, right RelExpr, filters FiltersExpr) bool {
	...
	if filters.IsTrue() {
		...
		rightUnfilteredCols := getColsFromTableIDs(md, deriveUnfilteredCols(right))
...
// an output column.
//
// The limitOne parameter is used when only a single valid foreign key relation
// needs to be found (the cross join case).
...
func getColsFromTableIDs(md *opt.Metadata, firstCols opt.ColSet) opt.ColSet {
	cols := opt.ColSet{}
	for col, next := cols.Next(0); next; col, next = cols.Next(col + 1) {
