...
func filtersMatchAllLeftRows(left, right RelExpr, filters FiltersExpr) bool {
	...
	if !verifyFilters(left, right, filters) {
		return false
	}
...
// verifyFilters returns true when all of the following conditions are
// satisfied:
...
// referenced columns on the right. Panics if verifyFilters is not checked
// first.
