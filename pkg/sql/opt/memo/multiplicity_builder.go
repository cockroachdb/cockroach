...
func verifyFiltersAreValidEqualities(
	...
	for i := range filters {
		...
		if !rightUnfilteredCols.Contains(rightColID) &&
			...
			// Condition #3a and #3b: The right column is filtered, and the left
			// and right aren't Selects where the left filters imply the right
			// filters when replacing the left column with the right column.
			return opt.ColSet{}, false
		}
