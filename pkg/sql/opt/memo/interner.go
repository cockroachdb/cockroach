...
func (c *internCache) Next() bool {
	// If item is set, then the previous lookup must not have matched, so either
	// get the next value in the collision list, or else generate a new hash.
	if c.prev.item != nil {
		if c.prev.collision != 0 {
			// A collision link already exists, so follow that.
			c.hash = c.prev.collision
		} else {
			// There is no collision link, so this is the end of the collision
			// list. Generate a new random hash value. Using global rand is OK,
			// since collisions should be very rare at this level, occurring only
			// when two uint64 values are the same. Collisions at the Go map level
			// would be more frequent, since those uint64 values must be mapped to
			// the much smaller len(c.cache) range.
			...
			// Link the previous hash entry to the new hash value. Note that even
			// the new random value could collide, so treat it no differently
			// than the original hash.
