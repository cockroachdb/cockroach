...
func (m *Memo) checkExpr(e opt.Expr) {
	...
	case *ProjectExpr:
		for _, item := range t.Projections {
			...
			if t.Passthrough.Contains(int(item.Col)) {
				panic(pgerror.NewAssertionErrorf("both passthrough and synthesized have column %d", item.Col))
			}

			// Check that columns aren't passed through in projection expressions.
			if v, ok := item.Element.(*VariableExpr); ok {
				if v.Col == item.Col {
					panic(pgerror.NewAssertionErrorf("projection passes through column %d", item.Col))
				}
			}
		}
		...
		checkAggs = func(scalar opt.ScalarExpr) {
			...
			default:
				if !opt.IsAggregateOp(scalar) {
					panic(pgerror.NewAssertionErrorf("aggregate contains illegal op: %s", scalar.Op()))
				}
			}
		}
		for _, item := range *t {
	...
	case *DistinctOnExpr:
		...
		for _, item := range t.Aggregations {
			...
			default:
				panic(pgerror.NewAssertionErrorf("distinct-on contains %s", item.Agg.Op()))
			}
		}
	...
	case *GroupByExpr, *ScalarGroupByExpr:
		...
		for _, item := range *t.Child(1).(*AggregationsExpr) {
			switch item.Agg.Op() {
			case opt.FirstAggOp:
				panic(pgerror.NewAssertionErrorf("group-by contains %s", item.Agg.Op()))
			}
		}
		...
		if !opt.IsListOp(e) {
			for i := 0; i < e.ChildCount(); i++ {
				...
				if opt.IsListItemOp(child) {
					panic(pgerror.NewAssertionErrorf("non-list op contains item op: %s", child.Op()))
				}
			}
		}

		if e.Op() == opt.StringAggOp && !CanExtractConstDatum(e.Child(1)) {
			panic(pgerror.NewAssertionErrorf("second argument to StringAggOp must always be constant, but got %s", e.Child(1).Op()))
		}
...
func checkExprOrdering(e opt.Expr) {
	...
	if outCols := e.(RelExpr).Relational().OutputCols; !ordering.SubsetOfCols(outCols) {
		panic(pgerror.NewAssertionErrorf("invalid ordering %v (op: %s, outcols: %v)", ordering, e.Op(), outCols))
	}
}
