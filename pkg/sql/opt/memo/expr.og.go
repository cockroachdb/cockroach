...
var opLayoutTable = [...]opLayout{
	...
	opt.SingleRowSubqueryOp: makeOpLayout(1 /*base*/, 0 /*list*/, 2 /*priv*/),
...
// SingleRowSubqueryExpr is a subquery in a single-row context such as
// `SELECT 1 = (SELECT 1)` or `SELECT (1, 'a') = (SELECT 1, 'a')`.
// In a single-row context, the outer query is only valid if the subquery
// returns exactly one row.
type SingleRowSubqueryExpr Expr

func MakeSingleRowSubqueryExpr(input GroupID, cols PrivateID) SingleRowSubqueryExpr {
	return SingleRowSubqueryExpr{op: opt.SingleRowSubqueryOp, state: exprState{uint32(input), uint32(cols)}}
}

func (e *SingleRowSubqueryExpr) Input() GroupID {
	return GroupID(e.state[0])
}

func (e *SingleRowSubqueryExpr) Cols() PrivateID {
	return PrivateID(e.state[1])
}

func (e *SingleRowSubqueryExpr) Fingerprint() Fingerprint {
	return Fingerprint(*e)
}

func (e *Expr) AsSingleRowSubquery() *SingleRowSubqueryExpr {
	if e.op != opt.SingleRowSubqueryOp {
		return nil
	}
	return (*SingleRowSubqueryExpr)(e)
}

// SubqueryExpr is a subquery in a multi-row context such as
...
func MakeSubqueryExpr(input GroupID, cols PrivateID) SubqueryExpr {
	return SubqueryExpr{op: opt.SubqueryOp, state: exprState{uint32(input), uint32(cols)}}
}
...
func (e *SubqueryExpr) Cols() PrivateID {
	return PrivateID(e.state[1])
}
