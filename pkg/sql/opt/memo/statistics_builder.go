...
func (sb *statisticsBuilder) colStatJoin(colSet opt.ColSet, ev ExprView) *props.ColumnStatistic {
	...
	default:
		...
		if leftStats.RowCount != 0 {
			f1 = leftNullCount / leftStats.RowCount
		}
		if rightStats.RowCount != 0 {
			f2 = rightNullCount / rightStats.RowCount
		}
...
func (sb *statisticsBuilder) adjustNullCountsForOuterJoins(
	...
	case opt.LeftJoinOp, opt.LeftJoinApplyOp:
		...
		if !rightCols.Empty() {
			f1 := leftNullCount / rowCount
			f2 := (rowCount - innerJoinRowCount) / rowCount
			colStat.NullCount += rowCount * (f2 - f1*f2)
		}
	...
	case opt.FullJoinOp, opt.FullJoinApplyOp:
		...
		if !rightCols.Empty() {
			f1 := leftNullCount / leftJoinRowCount
			f2 := (leftJoinRowCount - innerJoinRowCount) / leftJoinRowCount
			colStat.NullCount += leftJoinRowCount * (f2 - f1*f2)
		}
	}
}
...
func (sb *statisticsBuilder) joinSelectivityFromNullCounts(
	...
	for col, ok := cols.Next(0); ok; col, ok = cols.Next(col + 1) {
		...
		if crossJoinNullCount > rowCount {
			panic("rowCount passed in was too small")
		}
		...
		if colStat.NullCount == 0 {
			selectivity *= (1 - crossJoinNullCount/rowCount)
		}
	}
