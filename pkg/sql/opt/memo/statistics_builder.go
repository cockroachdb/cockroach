...
func (sb *statisticsBuilder) selectivityFromDistinctCounts(
	...
	for col, ok := cols.Next(0); ok; col, ok = cols.Next(col + 1) {
		...
		nullFraction := inputColStat.NullCount / inputStats.RowCount
		...
		// Account for the difference in non-null values.
		if newDistinct < oldDistinct {
			colSelectivity = newDistinct / oldDistinct
		}

		// Account for the difference in null values.
		colSelectivity *= 1 - nullFraction
		if colStat.NullCount > 0 {
			colSelectivity += nullFraction
		}

		// Avoid setting selectivity to 0.
		colSelectivity = max(colSelectivity, epsilon)
...
func (sb *statisticsBuilder) selectivityFromHistograms(
	...
	for col, ok := cols.Next(0); ok; col, ok = cols.Next(col + 1) {
		...
		if newCount < oldCount {
			colSelectivity *= newCount / oldCount
		}
...
func (sb *statisticsBuilder) selectivityFromNullsRemoved(
	...
	relProps.NotNullCols.ForEach(func(col opt.ColumnID) {
		...
		if !ok {
			...
			nullFraction := inputColStat.NullCount / inputStats.RowCount
			selectivity *= 1 - nullFraction
		}
	})

	// Avoid setting selectivity to 0.
	selectivity = max(selectivity, epsilon)
