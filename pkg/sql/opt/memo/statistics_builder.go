...
// constraints and builds a stats entry on the first index column from them if
// stats don't already exist on that column. In addition, if the
// optimizer_use_histograms session setting is on, and the first index column is
// not invertable, and the number of rows in the table is greater than the
// number of values found in the constraint, then an EquiDepthHistogram is built
// with them using an even data distribution.
	...
	for i := 0; i < len(filters); i++ {
		...
		for j := 0; j < filter.ScalarProps().Constraints.Length(); j++ {
			...
			for k := 0; k < numValues; k++ {
				startKeyPrefix := filterConstraint.Spans.Get(k).StartKey().Value(0)
				endKeyPrefix := filterConstraint.Spans.Get(k).EndKey().Value(0)
			...
			// Also, histogram building errors out when the number of samples is greater
