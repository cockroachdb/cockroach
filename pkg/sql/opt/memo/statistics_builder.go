...
// constraints which constrain indexes and builds a stats entry on the first
// index column in each constrained index if stats don't already exist on that
// column. In addition, if the optimizer_use_histograms session setting is on,
// and the first index column is not invertable, and the number of rows in the
// table is not less than the number of values found in the constraint, then an
// EquiDepthHistogram is built with them using an even data distribution.
func (sb *statisticsBuilder) buildStatsFromCheckConstraints(
	...
	for i := 0; i < len(filters); i++ {
		...
		for j := 0; j < filter.ScalarProps().Constraints.Length(); j++ {
			...
			if filterConstraint.IsContradiction() || filterConstraint.IsUnconstrained() {
				continue
			}
			firstColID = filterConstraint.Columns.Get(0).ID()
			cols := opt.MakeColSet(firstColID)
			...
			numValues := filterConstraint.Spans.Count()
			values := make(tree.Datums, 0, numValues)

			var columns constraint.Columns
			columns.InitSingle(opt.MakeOrderingColumn(firstColID, false /* desc */))
			keyCtx := constraint.MakeKeyContext(&columns, sb.evalCtx)
			canHandle := true
			numDuplicates := 0
			var prevDatum tree.Datum
			// Traverse all the spans and collect values where the span start prefix
			// matches the span end prefix. If they don't, don't build stats for now.
			// Duplicate prefix values are skipped over.
			for k := 0; k < numValues; k++ {
				startKey := filterConstraint.Spans.Get(k).StartKey()
				endKey := filterConstraint.Spans.Get(k).EndKey()
				if startKey.IsEmpty() || endKey.IsEmpty() {
					canHandle = false
					break
				}
				startKeyPrefix := startKey.Value(0)
				endKeyPrefix := endKey.Value(0)
				// Only handle point values for now, not ranges.
				if keyCtx.Compare(0, startKeyPrefix, endKeyPrefix) != 0 {
					canHandle = false
					break
				}
				datum := startKeyPrefix
				if datum == tree.DNull {
					canHandle = false
					break
				}
				if k > 0 {
					if keyCtx.Compare(0, prevDatum, datum) == 0 {
						numDuplicates++
						continue
					}
				}
			...
			numValues = numValues - numDuplicates
			invertedColOrds := invertedIndexCols[tabID.ColumnOrdinal(firstColID)]
			...
			useHistogram := sb.evalCtx.SessionData().OptimizerUseHistograms &&
				len(invertedColOrds) == 0 && int64(numValues) <= numRows
			var avgSize float64
			...
			if useHistogram {
				...
				if err == nil {
					...
					useHistogram = false
					log.Infof(
						sb.evalCtx.Ctx(), "histogram could not be generated from check constraint",
					)
				}
			}
			...
			// Make the actual stats entry in statistics.ColStats that can be looked
			// up via a ColSet.
