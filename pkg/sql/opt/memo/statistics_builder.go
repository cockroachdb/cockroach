...
// outputRowCount calculates the number of output rows returned after applying
// a predicate, using the following formula:
//
// row count = (fraction of non-null values preserved) * (number of non-null input rows) +
//             (fraction of null values preserved) * (number of null input rows)
//
func (sb *statisticsBuilder) outputRowCount(
	nonNullSelectivity, nullSelectivity, inputNullCount, inputRowCount float64,
) float64 {
	rowCount := nonNullSelectivity*(inputRowCount-inputNullCount) + nullSelectivity*inputNullCount

	// Avoid setting row count to 0. The stats may be stale, and we
	// can end up with weird and inefficient plans if we estimate 0 rows.
	return max(rowCount, epsilon)
}
