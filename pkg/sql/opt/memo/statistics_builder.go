...
func (sb *statisticsBuilder) colStatLeaf(
	...
	if colSet.Len() == 1 {
		...
		distinctCount := 1.0
		nullCount := 0.0
		// The collision factor tracks expected cases of rows containing
		// multiple nulls across the reflected columns. If we do a simple
		// sum of all null counts, the result overestimates the count by
		// this factor multiplied with RowCount.
		nullCountCollisionFactor := 1.0
		colSet.ForEach(func(i int) {
			...
			// This collision factor works okay for 2-column
			// cases, but starts to really underestimate collisions for
			// larger sets of columns.
			//
			// TODO(itsbilal): Come up with a reasonable average-case
			// calculation of null collisions across
			nullCountCollisionFactor *= colStatLeaf.NullCount / s.RowCount
		})
		colStat.DistinctCount = min(distinctCount, s.RowCount)
		nullCount -= s.RowCount * nullCountCollisionFactor
