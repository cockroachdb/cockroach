...
func (sb *statisticsBuilder) constrainScan(
	...
	s.ApplyInverseSelectivity(sb.selectivityFromSingleColDistinctCounts(histCols, scan, s))
}
...
func (sb *statisticsBuilder) buildInvertedFilter(
	...
	// Adjust the selectivity so we don't double-count the histogram columns.
	s.ApplyInverseSelectivity(sb.selectivityFromSingleColDistinctCounts(histCols, invFilter, s))
...
func (sb *statisticsBuilder) buildJoin(
	...
	if h.filterIsTrue {
		...
		s.Selectivity = props.MakeSelectivity(1.0)
		switch h.joinType {
	...
	// Adjust the selectivity so we don't double-count the histogram columns.
	s.ApplyInverseSelectivity(sb.selectivityFromSingleColDistinctCounts(histCols, join, s))
	...
	case opt.AntiJoinOp, opt.AntiJoinApplyOp:
		...
		s.Selectivity = props.MakeSelectivity(1 - s.Selectivity.Get())
	...
	default:
		...
		case opt.LeftJoinOp, opt.LeftJoinApplyOp, opt.RightJoinOp, opt.FullJoinOp:
			sb.adjustNullCountsForOuterJoins(
				...
				rightProps.Stats.RowCount,
				s.RowCount,
				s.Selectivity.Get()*inputRowCount,
			)
...
func (sb *statisticsBuilder) filterRelExpr(
	...
	// Adjust the selectivity so we don't double-count the histogram columns.
	s.ApplyInverseSelectivity(sb.selectivityFromSingleColDistinctCounts(histCols, e, s))
...
func (sb *statisticsBuilder) selectivityFromMultiColDistinctCounts(
	...
	singleColSelectivity := props.MakeSelectivity(1.0)
	...
	for col, ok := cols.Next(0); ok; col, ok = cols.Next(col + 1) {
		...
		if localSel.Get() < minLocalSel {
			minLocalSel = localSel.Get()
		}
	...
	if maxNewDistinct > 1 && multiColSet.Len() > 2 {
		...
		if lowDistinctCountCols.Len() > 1 {
			...
			if selLowDistinctCountCols.Get() < minLocalSel {
				minLocalSel = selLowDistinctCountCols.Get()
			}
		}
	}
	multiColSelectivity = props.MinSelectivity(multiColSelectivity, props.MakeSelectivity(minLocalSel))
	...
	return props.MakeSelectivity((1-multiColWeight)*singleColSelectivity.Get() + multiColWeight*multiColSelectivity.Get())
}
...
func (sb *statisticsBuilder) selectivityFromSingleColDistinctCounts(
	...
	selectivity = props.MakeSelectivity(1.0)
	for col, ok := cols.Next(0); ok; col, ok = cols.Next(col + 1) {
		colStat, ok := s.ColStats.Lookup(opt.MakeColSet(col))
		if !ok {
			continue
...
func (sb *statisticsBuilder) selectivityFromNullsRemoved(
	...
	notNullCols.ForEach(func(col opt.ColumnID) {
		if !ignoreCols.Contains(col) {
			...
			selectivity.Multiply(sb.predicateSelectivity(
				props.MakeSelectivity(1.0), /* nonNullSelectivity */
				props.MakeSelectivity(0), /* nullSelectivity */
...
func (sb *statisticsBuilder) predicateSelectivity(
	...
	outRowCount := nonNullSelectivity.Get()*(inputRowCount-inputNullCount) + nullSelectivity.Get()*inputNullCount
