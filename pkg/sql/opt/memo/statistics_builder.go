...
func (sb *statisticsBuilder) buildZigzagJoin(
	...
	rightStats := zigzag.rightProps.Stats
	equivReps := h.filtersFD.EquivReps()
	...
	// row counts. An example of such a case would be a zigzag on secondary
	// indexes of the same table.
	s.RowCount = max(leftStats.RowCount, rightStats.RowCount)
