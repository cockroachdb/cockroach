...
//  1. lb (lower bound): the larger value of a or b:
//     a. the value returned by multiplying the individual conjunct selectivities
//     together, estimated from single-column distinct counts. This would be the
//     selectivity of the entire predicate if the columns were completely
//     independent.
//     b. the minimum possible multi-column selectivity, calculated based on the
//     minimum new multi-column distinct count and the old row count.
...
func (sb *statisticsBuilder) correlationFromMultiColDistinctCounts(
	...
	selectivity, upperBound, multiColLowerBound := sb.selectivityFromMultiColDistinctCounts(cols, e, s)
	if multiColLowerBound.AsFloat() > lowerBound.AsFloat() {
		lowerBound = multiColLowerBound
	}
