...
func (sb *statisticsBuilder) colStatLeaf(
	...
	if colSet.Len() == 1 {
		...
		if sb.md.ColumnNotNull(opt.ColumnID(col)) {
			colStat.NullCount = 0
		}
	} else {
		distinctCount := 1.0
		...
		colStat.NullCount = min(s.RowCount*nullFactor, s.RowCount)
	}
...
func (sb *statisticsBuilder) buildScan(ev ExprView, relProps *props.Relational) {
	...
	if def.Constraint != nil {
		...
		s.ApplySelectivity(sb.selectivityFromNullCounts(cols, ev, s, savedRowCount))
	}
...
func (sb *statisticsBuilder) buildSelect(ev ExprView, relProps *props.Relational) {
	...
	s.ApplySelectivity(sb.selectivityFromNullCounts(constrainedCols, ev, s, savedRowCount))
...
func (sb *statisticsBuilder) colStatJoin(colSet opt.ColSet, ev ExprView) *props.ColumnStatistic {
	...
	default:
		...
		colStat.NullCount = min(s.RowCount-colStat.DistinctCount+1, colStat.NullCount)
		return colStat
	}
}
...
func (sb *statisticsBuilder) colStatIndexJoin(
	...
	if !reqLookupCols.Empty() {
		...
		colStat.NullCount = tableStats.RowCount * (colStat.NullCount / inputStats.RowCount) * (lookupColStat.NullCount / tableStats.RowCount)
	}
...
func (sb *statisticsBuilder) colStatGroupBy(colSet opt.ColSet, ev ExprView) *props.ColumnStatistic {
	...
	if !colSet.SubsetOf(groupingColSet) {
		// Some of the requested columns are aggregates. Estimate the distinct and null
		// counts to be the same as the grouping columns.
		colStat, _ := s.ColStats.Add(colSet)
...
func (sb *statisticsBuilder) finalizeFromCardinality(relProps *props.Relational) {
	...
	// The distinct counts should be no larger than the row count. And by pigeonhole
	// principle, the null count must always be less than RowCount - DistinctCount + 1.
	for i, n := 0, s.ColStats.Count(); i < n; i++ {
		colStat := s.ColStats.Get(i)
		colStat.DistinctCount = min(colStat.DistinctCount, s.RowCount)
		colStat.NullCount = min(colStat.NullCount, s.RowCount-colStat.DistinctCount+1)
	}
}
...
func (sb *statisticsBuilder) applyConstraint(
	...
	sb.updateNullCountsFromConstraint(c, ev, relProps)
...
func (sb *statisticsBuilder) applyConstraintSet(
	...
	for i := 0; i < cs.Length(); i++ {
		sb.updateNullCountsFromConstraint(cs.Constraint(i), ev, relProps)
...
// updateNullCountsFromConstraint updates the null count for each column
// in a constraint. Based on whether the provided constraint set includes
// or excludes null, and whether the column is nullable or not, the
// null count is either propagated through (i.e. unchanged), or it is
// updated to be zero.
//
...
func (sb *statisticsBuilder) updateNullCountsFromConstraint(
	...
	if prefix == c.Columns.Count() {
		// Bias the prefix by 1; we will still loop through all columns.
		prefix -= 1
	}
	for col := 0; col <= prefix; col++ {
		// Short circuit case where column is not nullable.
		if sb.md.ColumnNotNull(c.Columns.Get(col).ID()) {
			continue
		}
		...
		for i := 0; i < c.Spans.Count(); i++ {
			...
			// Nulls are included for unconstrained columns in cases
			// like /1/2/3: [/1 - /2]. Here, /1/NULL/NULL and /2/NULL/NULL
			...
			if sp.StartKey().Length() > col {
				startVal := sp.StartKey().Value(col)

				// See if the start boundary is null.
				if startVal == tree.DNull && sp.StartBoundary() == constraint.IncludeBoundary {
					nullsIncluded = true
					break
				}
			} else if c.Columns.Get(col).Ascending() {
				// This corresponds to a case like column 2 in
				// /1/2/3: [/1 - /1/3/4].
				nullsIncluded = true
				break
			}

			if sp.EndKey().Length() > col {
				// See if the end boundary is null, and if it's included.
				endVal := sp.EndKey().Value(col)
				if endVal == tree.DNull && sp.EndBoundary() == constraint.IncludeBoundary {
					nullsIncluded = true
					break
				}
			} else if c.Columns.Get(col).Descending() {
				// This corresponds to a case like column 2 in
				// /1/-2/-3: [/1/3/4 - /1].
				nullsIncluded = true
				break
		...
		// If nulls are not included, set the NullCount to 0. If nulls are included,
		// we leave the colStat untouched; letting it propagate from the input, or
		// letting it get auto-generated from the unknownNullCountRatio if there are
		// no stats.
		if !nullsIncluded {
			colID := c.Columns.Get(col).ID()
			colStat := sb.ensureColStat(util.MakeFastIntSet(int(colID)), math.MaxFloat64, ev, relProps)
			colStat.NullCount = 0
		}
	}
}
...
func (sb *statisticsBuilder) selectivityFromDistinctCounts(
	...
	for col, ok := cols.Next(0); ok; col, ok = cols.Next(col + 1) {
		...
		if inputStat.DistinctCount != 0 && colStat.DistinctCount < inputStat.DistinctCount {
			...
			// If one of the distincts is NULL, don't account for that in selectivity yet.
			// selectivityFromNullCounts will account for it.
			if colStat.NullCount > 0 && newDistinct > 1 {
				newDistinct -= 1
			}
			if inputStat.NullCount > 0 && oldDistinct > 1 {
				oldDistinct -= 1
			}
