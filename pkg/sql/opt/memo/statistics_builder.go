...
func (sb *statisticsBuilder) applyFiltersItem(
	...
	if scalarProps.Constraints != nil {
		...
		sb.applyConstraintSetSelectivity(e, constraintUnion[0], relProps, &unionStats)
		for i := 1; i < len(constraintUnion); i++ {
			tmpStats.CopyFrom(s)
			sb.applyConstraintSetSelectivity(e, constraintUnion[i], relProps, &tmpStats)
...
func (sb *statisticsBuilder) buildDisjunctionConstraints(filter *FiltersItem) []*constraint.Set {
	...
	collectConstraints = func(e opt.ScalarExpr) {
		// If a constraint can be build from e, collect the constraint and its
		// tightness.
		...
		if !ok {
			...
			// not an Or, set foundUnconstrained so we can return nil.
			unconstrained = true
			return
		}
...
// applyConstraintSetSelectivity calculates the stats for a relational
// expression based on the constraint set. The constraint set must be tight.
func (sb *statisticsBuilder) applyConstraintSetSelectivity(
