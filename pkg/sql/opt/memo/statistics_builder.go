...
func (sb *statisticsBuilder) buildJoin(
	...
	// ----------------------------------------------------------------------
	numUnappliedConjuncts, constrainedCols, histCols := sb.applyFilters(h.filters, join, relProps, true)
...
func (sb *statisticsBuilder) buildZigzagJoin(
	...
	numUnappliedConjuncts, constrainedCols, _ := sb.applyFilters(zigzag.On, zigzag, relProps, false)
...
func (sb *statisticsBuilder) filterRelExpr(
	...
	// Calculate distinct counts and histograms for constrained columns
	// ----------------------------------------------------------------
	numUnappliedConjuncts, constrainedCols, histCols := sb.applyFilters(filters, e, relProps, false)
...
_FilterLoop:
	for f := 0; f < len(h.filters); f++ {
		...
		for i := 0; i < len(filters); i++ {
			...
			// Catch cases where the above FuncDeps.Empty() check didn't detect that
			// we can't do proper estimation.
			if equivReps.Empty() || singleSelectivity == props.OneSelectivity {
				numUnappliedConjuncts++
				continue _FilterLoop
			}
		...
		//
		conjunctSelectivity = selectivities[0]
		var combinedSel props.Selectivity
		for i := 1; i < len(selectivities); i++ {
			// combinedSel represents P(A and B)
			combinedSel = props.MakeSelectivity((&conjunctSelectivity).AsFloat())
			(&combinedSel).Multiply(selectivities[i])
			conjunctSelectivity.Add(selectivities[i])
			conjunctSelectivity.Subtract(combinedSel)
		}
