...
func (sb *statisticsBuilder) buildScan(scan *ScanExpr, relProps *props.Relational) {
	...
	if scan.InvertedConstraint != nil ||
		...
		// calculate selectivity = 1/3 + 1/3 + 1/3 = 1 in spanStatsUnion, which is
		// clearly inaccurate. Instead, we should use the value calculated from the
		// combined spans, which is simply 1/3.
