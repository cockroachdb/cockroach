...
// If no statistics exist on any columns in this table (as indicated via
// statistics.Available), buildStatsFromCheckConstraints does not modify
// colStat. True is returned if colStat was modified, otherwise false.
func (sb *statisticsBuilder) buildStatsFromCheckConstraints(
	...
	for i := 0; i < len(filters); i++ {
		...
		for j := 0; j < filter.ScalarProps().Constraints.Length(); j++ {
			...
			// types of such columns (JSON, ARRAY, TUPLE) aren't likely to occur in
			// CHECK constraints.  So, let's play it safe and don't create a histogram
			// directly on an inverted column as this may invoke some untested code
			// paths, and the possible user benefit of adding this support seems low.
