...
// See statisticsBuilder.selectivityFromDistinctReducedCols for a more detailed
// explanation.
//
func (sb *statisticsBuilder) selectivityFromDistinctCounts(
	...
	allConstant := true
	if selectivity, allConstant = sb.selectivityFromDistinctReducedCols(inputStatsBuilder); allConstant {
		return selectivity
	}
	selectivity = 1.0
	for _, colStat := range sb.s.ColStats {
...
// This is used for selectivity calculation.
...
// only need to consider the selectivity of id, not name, since id and name fully correlated.
// To determine if we have a case such as this one, we functionally reduce the set of
// columns which have column statistics, eliminating columns that can be functionally
// determined by other columns.
//
func (sb *statisticsBuilder) selectivityFromDistinctReducedCols(
	inputStatsBuilder *statisticsBuilder,
) (selectivity float64, allConstant bool) {
	var cols opt.ColSet

	// We consider only single column stats since our goal
	// is to discard selectivity of fully correlated columns.
	// Non-trivial (multi-col stats determined by FDs) Multi-col stats
	// would imply the columns are not fully dependent.
