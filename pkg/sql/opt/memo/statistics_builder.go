...
func (sb *statisticsBuilder) colStatProject(colSet opt.ColSet) *opt.ColumnStatistic {
	// Columns may be passed through from the input, or synthesized by the
	// projection operation.
	inputCols := sb.ev.Child(0).Logical().Relational.OutputCols.Copy()
	inputCols.IntersectionWith(colSet)
	synthesizedCols := colSet.Difference(inputCols)
	if synthesizedCols.Len() > 0 {
		// Some of the columns in colSet were synthesized. We assume that the
		// statistics of the synthesized columns are the same as the statistics of
		// their input columns. For example, the distinct count of (x + 2) is the
		// same as the distinct count of x.
		outputCols := sb.ev.Child(1).Private().(opt.ColList)
		for i, col := range outputCols {
			if synthesizedCols.Contains(int(col)) {
				inputCols.UnionWith(sb.ev.Child(1).Child(i).Logical().Scalar.OuterCols)
			}
		}
	}
	...
	if inputCols.Len() > 0 {
		// Inherit column statistics from input, using the inputCols identified
		// above.
		inputStats := &sb.ev.childGroup(0).logical.Relational.Stats
		inputStatsBuilder := sb.makeStatisticsBuilder(inputStats, sb.ev.Child(0))
		inputColStat := inputStatsBuilder.colStat(inputCols)
