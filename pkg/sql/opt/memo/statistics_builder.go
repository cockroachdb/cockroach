...
func (sb *statisticsBuilder) buildSetNode(setNode RelExpr, relProps *props.Relational) {
	...
	// Set operations act as barriers for VirtualCols. It would only be useful to
	// pass VirtualCols up through a set operation if we had a virtual column
	// synthesized above the set operation using columns from below the set
	// operation. We don't currently generate plans like this. If we did, we would
	// also have to rewrite the virtual column expression in terms of the set operation
	// column mapping.
