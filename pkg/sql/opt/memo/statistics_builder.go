...
func (sb *statisticsBuilder) colStatSetOpImpl(
	...
	case opt.UnionOp, opt.IntersectOp, opt.ExceptOp:
		if outputCols.Len() == 1 {
			...
			leftRowCount := sb.statFromChild(ev, 0 /* childIdx */).RowCount
			leftNullCount *= (leftColStat.DistinctCount + 1) / leftRowCount
			rightRowCount := sb.statFromChild(ev, 1 /* childIdx */).RowCount
...
const (
	...
	// columns, which is used in the absence of any real statistics for non-null
	// columns.
