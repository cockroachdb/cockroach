...
func (sb *statisticsBuilder) buildSelect(ev ExprView, relProps *props.Relational) {
	...
	// on y and modify the row count accordingly.
...
func (sb *statisticsBuilder) buildJoin(
	...
	// -----------------------------------
	s.RowCount = leftStats.RowCount * rightStats.RowCount
	// If one of the RowCounts is less than 1, the final row count could be lower
	// than either side - resulting in an incorrect selectivityFromNullCount. So,
	// max it out to the biggest of the three for this purpose.
	...
	case opt.LeftJoinOp, opt.LeftJoinApplyOp, opt.RightJoinOp, opt.RightJoinApplyOp,
		...
		for i := 0; i < s.ColStats.Count(); i++ {
			...
			sb.adjustNullCountsForOuterJoins(
				...
				ev,
				relProps,
				leftCols.Intersection(colStat.Cols),
				rightCols.Intersection(colStat.Cols),
			)
		}
	}
...
func (sb *statisticsBuilder) adjustNullCountsForOuterJoins(
	cols opt.ColSet, ev ExprView, relProps *props.Relational, leftCols opt.ColSet, rightCols opt.ColSet,
) {
	s := &relProps.Stats
	colStat, ok := s.ColStats.Lookup(cols)

	if !ok {
		panic("colStats not created in advance")
	}

	leftStats := sb.statsFromChild(ev, 0 /* childIdx */)
	var rightStats *props.Statistics
	joinType := ev.Operator()
	if joinType == opt.LookupJoinOp {
		lookupJoinDef := ev.Private().(*LookupJoinDef)
		...
		rightStats = sb.statsFromChild(ev, 1 /* childIdx */)
	}
	...
	innerJoinRowCount := s.Selectivity * leftStats.RowCount * rightStats.RowCount
	...
	case opt.LeftJoinOp, opt.LeftJoinApplyOp:
		...
		if !rightCols.Empty() {
			colStat.NullCount += s.RowCount - innerJoinRowCount
		}
