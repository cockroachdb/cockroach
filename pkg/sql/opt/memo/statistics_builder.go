...
func (sb *statisticsBuilder) BuildStatsFromCheckConstraints(
	...
	for i := 0; i < len(filters); i++ {
		...
		skipHistogram := !sb.evalCtx.SessionData().OptimizerUseHistograms ||
			!statistics.Available || len(invertedColOrds) != 0
		...
		if !skipHistogram {
			...
			h, err := stats.EquiDepthHistogram(sb.evalCtx,
				dataType,
				values, /* samples */
				int64(statistics.RowCount),
			...
			if err != nil {
				...
				for j := range h.Buckets {
					dataSize += len(h.Buckets[j].UpperBound)
				}
				...
				// No need to assign a StatisticID to our manufactured TableStatistic.
				// No need to set ColumnIDs since the optimizer doesn't use stableIDs.
				// Possibly tabStat doesn't need to be created, but it's good as a
				// sanity check that everything's built properly.
				tabStat := &stats.TableStatistic{
					TableStatisticProto: stats.TableStatisticProto{
						TableID:       catid.DescID(tab.ID()),
						Name:          "",
						CreatedAt:     timeutil.Now(),
						RowCount:      rowCount,
						DistinctCount: uint64(numValues),
						NullCount:     0, // This is a non-null check constraint
						HistogramData: &h,
						AvgSize:       (uint64(dataSize) / uint64(numValues)),
					},
				}
				if err := stats.DecodeHistogramBuckets(tabStat); err != nil {
					// So, we failed to decode the buckets. Just build stats without them.
					skipHistogram = true
				} else {
					histogram = tabStat.Histogram
					distinctCount = max(float64(tabStat.DistinctCount), 1)
					nullCount = float64(tabStat.NullCount)
					avgSize = float64(tabStat.AvgSize)
				}
			}
		}
		if skipHistogram {
