...
func (cb *constraintsBuilder) binaryBuildConstraintsForOr(
	disjunctions FiltersExpr,
) (_ *constraint.Set, tight bool) {
	size := len(disjunctions)
	mid := size / 2
	if size == 1 {
		return cb.buildConstraints(disjunctions[0].Condition)
	}
