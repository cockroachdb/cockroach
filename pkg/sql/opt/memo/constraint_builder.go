...
func (cb *constraintsBuilder) makeSingleColumnConstraint(
	col opt.ColumnID, op opt.Operator, val ExprView,
) (_ *constraint.Set, tight bool) {
	// TODO(radu): handle IN

	if val.IsConstValue() {
		return cb.makeSingleColumnConstraintConst(col, op, ExtractConstDatum(val))
	}
...
func (cb *constraintsBuilder) makeSingleColumnConstraintConst(
...
func (cb *constraintsBuilder) makeConstraints(ev ExprView) (_ *constraint.Set, tight bool) {
	// TODO(radu): handle const bool value.

	switch ev.Operator() {
	case opt.AndOp, opt.FiltersOp:
		c, tight := cb.makeConstraints(ev.Child(0))
		for i := 1; i < ev.ChildCount(); i++ {
			ci, tighti := cb.makeConstraints(ev.Child(i))
	...
	if child0.Operator() == opt.VariableOp {
		return cb.makeSingleColumnConstraint(child0.Private().(opt.ColumnID), ev.Operator(), child1)
	}
	return unconstrained, false
}
