...
func (f *ExprFmtCtx) formatRelational(e RelExpr, tp treeprinter.Node) {
	...
	default:
		...
		colList = toList(e.Relational().OutputCols)
	}
	...
	case *GroupByExpr, *ScalarGroupByExpr, *DistinctOnExpr:
		...
		if !private.GroupingCols.Empty() {
			f.formatColList(e, tp, "grouping columns:", toList(private.GroupingCols))
		}
...
// toList converts a ColSet to a ColList, in column ID order.
func toList(set opt.ColSet) opt.ColList {
	res := make(opt.ColList, 0, set.Len())
	set.ForEach(func(x int) {
		res = append(res, opt.ColumnID(x))
	})
	return res
}
