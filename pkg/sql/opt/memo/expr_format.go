...
//
// If the reverse is true, ",rev" is appended. If the index is a partial index,
// ",partial" is appended.
func (f *ExprFmtCtx) formatIndex(tabID opt.TableID, idxOrd cat.IndexOrdinal, reverse bool) {
	...
	tab := f.Memo.Metadata().Table(tabID)
	if idxOrd == cat.PrimaryIndex {
		fmt.Fprintf(f.Buffer, " %s", tableAlias(f, tabID))
	} else {
		fmt.Fprintf(f.Buffer, " %s@%s", tableAlias(f, tabID), tab.Index(idxOrd).Name())
	}
	if reverse {
		f.Buffer.WriteString(",rev")
	}
	if _, isPartial := tab.Index(idxOrd).Predicate(); isPartial {
