	...
	case *ScanExpr, *PlaceholderScanExpr:
		...
		if c := private.Constraint; c != nil {
			if c.IsContradiction() {
				...
				spanStr := c.Spans.Get(0).String()
				if f.RedactableValues {
					spanStr = string(redact.Sprintf("%s", spanStr))
				}
				tp.Childf("constraint: %s: %s", c.Columns.String(), spanStr)
			} else {
				n := tp.Childf("constraint: %s", c.Columns.String())
				for i := 0; i < c.Spans.Count(); i++ {
					spanStr := c.Spans.Get(i).String()
					if f.RedactableValues {
						spanStr = string(redact.Sprintf("%s", spanStr))
					}
					n.Child(spanStr)
	...
	if propsExpr, ok := scalar.(ScalarPropsExpr); ok {
		...
		if !f.HasFlags(ExprFmtHideConstraints) {
			if scalarProps.Constraints != nil && !scalarProps.Constraints.IsUnconstrained() {
				...
				constrStr := scalarProps.Constraints.String()
				if f.RedactableValues {
					constrStr = string(redact.Sprintf("%s", constrStr))
				}
				emitProp("constraints=(%s%s)", constrStr, tight)
			}
		}
