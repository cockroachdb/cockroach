...
func (s *Statistics) updateStatsFromConstraint(
...
func (s *Statistics) updateStatsFromConstraintSet(cs *constraint.Set, evalCtx *tree.EvalContext) {
	...
	if exactPrefix < c.Columns.Count() {
		...
		for i := 0; i < c.Spans.Count(); i++ {
			...
			if sp.StartKey().Length() <= col || sp.EndKey().Length() <= col {
				// Use distinctCount = 0 as an indication that the stats for this
				// column are not valid.
				distinctCount = 0
				break
			}
		...
		if distinctCount != 0 {
			c := int(c.Columns.Get(col).ID())
			if v, ok := distinctCounts[c]; ok {
				distinctCounts[c] = min(v, distinctCount)
			} else {
				distinctCounts[c] = distinctCount
			}
		}
	}
...
func (s *Statistics) updateScanStats(def *ScanOpDef, evalCtx *tree.EvalContext) {
	...
	s.ColStats = append(s.ColStats, ColumnStatistics{
		Cols:          def.Cols,
		DistinctCount: uint64(float64(s.RowCount) * 0.9),
	})

	if def.Constraint != nil {
		s.updateStatsFromConstraint(def.Constraint, evalCtx)
	}
}
...
func (s *Statistics) updateSelectStats(
	...
	if constraintSet != nil {
		s.updateStatsFromConstraintSet(constraintSet, evalCtx)
	}
}
