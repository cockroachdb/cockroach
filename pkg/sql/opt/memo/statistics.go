...
func (c ColumnStatistics) Less(i, j int) bool {
	...
	sumi, sumj := 0, 0
	c[i].Cols.ForEach(func(x int) { sumi += x })
	c[j].Cols.ForEach(func(x int) { sumj += x })
...
func (s *Statistics) applyConstraint(c *constraint.Constraint, evalCtx *tree.EvalContext) {
	...
	applied := s.updateDistinctCountsFromConstraint(c, evalCtx, distinctCounts)
...
func (s *Statistics) applyConstraintSet(cs *constraint.Set, tight bool, evalCtx *tree.EvalContext) {
	...
	for i := 0; i < cs.Length(); i++ {
		applied := s.updateDistinctCountsFromConstraint(cs.Constraint(i), evalCtx, distinctCounts)
...
func (s *Statistics) colSetFromTableStatistic(
	...
	for i := 0; i < stat.ColumnCount(); i++ {
		cols.Add(stat.ColumnOrdinal(i) + int(tableID))
	}
	return cols
}

func (s *Statistics) initScan(def *ScanOpDef, tab opt.Table, evalCtx *tree.EvalContext) {
	if tab.StatisticCount() == 0 {
		...
		for i := 0; i < tab.StatisticCount(); i++ {
			...
			cols := s.colSetFromTableStatistic(stat, def.Table)
...
func (s *Statistics) initSelect(
	...
	if constraintSet != nil {
		s.applyConstraintSet(constraintSet, filter.Logical().Scalar.TightConstraints, evalCtx)
	} else {
...
func (s *Statistics) initSetOp(
	...
	for i := range smStats {
		...
		key.ForEach(func(i int) {
			colStatsMap[opt.ColumnID(i)] = smColStats
		})
	}
	...
	for i := range lgStats {
		...
		if smColStats, ok := colStatsMap[opt.ColumnID(val)]; ok {
			...
			s.updateDistinctCountsForSetOp(op, leftStats, rightStats, colMap, distinctCounts)
		}
	}
...
func (s *Statistics) updateDistinctCountsForSetOp(
	op opt.Operator,
	leftStats, rightStats *ColumnStatistic,
	colMap *SetOpColMap,
	...
	cols.IntersectionWith(translateColSet(rightStats.Cols, colMap.Right, colMap.Out))
