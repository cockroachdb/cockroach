...
func (b *logicalPropsBuilder) ensureLookupJoinInputProps(join *LookupJoinExpr) *props.Relational {
	relational := &join.lookupProps
	if relational.OutputCols.Empty() {
		md := join.Memo().Metadata()
		relational.OutputCols = join.LookupCols
		relational.NotNullCols = tableNotNullCols(md, join.Table)
		relational.NotNullCols.IntersectionWith(join.LookupCols)
		...
		relational.Stats = *b.sb.makeTableStatistics(join.Table)
	}
	return relational
}
...
func (h *joinPropsHelper) init(b *logicalPropsBuilder, join RelExpr) {
	...
	b.ensureLookupJoinInputProps(lookupJoin)
...
func (h *joinPropsHelper) outputCols() opt.ColSet {
	...
	outputCols := h.leftProps.OutputCols.Copy()
	switch h.joinType {
	case opt.SemiJoinOp, opt.AntiJoinOp, opt.SemiJoinApplyOp, opt.AntiJoinApplyOp:

	default:
		outputCols.UnionWith(h.rightProps.OutputCols)
	}
	return outputCols
}
...
func (h *joinPropsHelper) setFuncDeps(rel *props.Relational) {
	...
	default:
		...
		case opt.FullJoinOp, opt.FullJoinApplyOp:
			...
			//   NULL  NULL
			//
			if !rel.OutputCols.Intersects(notNullInputCols) {
