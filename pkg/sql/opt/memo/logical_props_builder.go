...
func (b *logicalPropsBuilder) buildJoinProps(ev ExprView) props.Logical {
	...
	case opt.SemiJoinOp, opt.SemiJoinApplyOp:
		...
		// constant columns. Any FDs involving columns from the right side will
		// be removed below.
		...
		for i := 0; i < ev.ChildCount(); i++ {
			...
			if child.Operator() == opt.EqOp {
				...
				if left.Operator() == opt.VariableOp && right.Operator() == opt.VariableOp {
					...
					// We currently only add equivalences for columns that do not have a
					// composite key encoding. This is a conservative measure to ensure
					// that we don't infer filters incorrectly. For example, consider
					// this query:
					//
					//   SELECT * FROM
					//     (VALUES (1.0)) AS t1(x),
					...
					//    1.0 | 1.00
					//
					// But if we use the equality predicate x=y to infer an additional
					// filter y::text = '1.0', the query would return nothing.
					//
					// TODO(rytaft): In the future, we may want to add the equivalency
					// anyway, but only map a predicate if it involves a comparison
					// operator, such as x < 5.
					if !sqlbase.DatumTypeHasCompositeKeyEncoding(ev.Metadata().ColumnType(colLeft)) &&
						!sqlbase.DatumTypeHasCompositeKeyEncoding(ev.Metadata().ColumnType(colRight)) {
