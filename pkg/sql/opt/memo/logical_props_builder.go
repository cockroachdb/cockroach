...
func (b *logicalPropsBuilder) buildScanProps(scan *ScanExpr, rel *props.Relational) {
	...
	if hardLimit == 1 {
		...
		// Add FDs from the partial index predicate if the scan is over a
		// non-inverted index, or a single key in an inverted index.
		if pred != nil && (!scan.IsInvertedScan(md) || singleKeyInvertedScan) {
