...
func (b *logicalPropsBuilder) buildMutationProps(mutation RelExpr, rel *props.Relational) {
	...
	for i, n := 0, tab.ColumnCount(); i < n; i++ {
		...
		// If both the insert and fetch/update columns are not null, then mark the
		// column as not null. Both must be not null because either may provide the
		// value for the output column.
		if private.InsertCols != nil {
			colID := private.InsertCols[i]
			if !inputProps.NotNullCols.Contains(int(colID)) {
				continue
			}
		}
		if private.UpdateCols != nil {
			colID := private.UpdateCols[i]
			if colID == 0 {
				colID = private.FetchCols[i]
			}
			if !inputProps.NotNullCols.Contains(int(colID)) {
				continue
			}
