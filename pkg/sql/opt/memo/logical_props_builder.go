...
func (b *logicalPropsBuilder) buildInsertProps(ins *InsertExpr, rel *props.Relational) {
	...
	for i, inputColID := range ins.InputCols {
		// Column is mutation column if its ordinal is >= count of regular columns
		// in the table.
		tabColID := ins.TableCols[i]
		tabOrd := md.ColumnOrdinal(tabColID)
		if tabOrd < tab.ColumnCount() {
			rel.OutputCols.Add(int(inputColID))

			// Also add to NotNullCols here, in order to avoid another loop below.
			if !tab.Column(tabOrd).IsNullable() {
				rel.NotNullCols.Add(int(inputColID))
			}
		}
	}
