...
func (b *logicalPropsBuilder) buildGroupByProps(ev ExprView) props.Logical {
	...
	if ev.Operator() == opt.ScalarGroupByOp {
		...
		// GroupBy and DistinctOn act like a filter, never returning more rows than
		// the input has. However, if the input has at least one row, then at least
		// one row will also be returned by GroupBy and DistinctOn.
