	...
	*h = joinPropsHelper{join: joinExpr}
	...
	case *LookupJoinExpr:
		...
		if len(join.RemoteLookupExpr) == 0 {
			...
			h.filters = append(join.On, extractRedundantFilters(join.LookupExpr, join.RemoteLookupExpr)...)
		}
...
// extractRedundantFilters extracts filters that exist in both left and right.
func extractRedundantFilters(left, right FiltersExpr) FiltersExpr {
