...
// always be >= Min, or method results are undefined.
type Cardinality struct {
	Min uint32
	Max uint32
}

// Filter ratchets the min bound downwards in order to ensure that it's no
// larger than the given min value.
func (c Cardinality) Filter(min uint32) Cardinality {
	if min < c.Min {
