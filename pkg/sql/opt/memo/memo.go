...
func (m *Memo) Init(ctx context.Context, evalCtx *eval.Context) {
	...
	*m = Memo{
		...
		pushSelectIntoOrdinality:                   evalCtx.SessionData().OptimizerPushSelectIntoOrdinality,
	}
	m.metadata.Init()
...
func (m *Memo) IsStale(
	...
	if m.reorderJoinsLimit != int(evalCtx.SessionData().ReorderJoinsLimit) ||
		...
		m.pushSelectIntoOrdinality != evalCtx.SessionData().OptimizerPushSelectIntoOrdinality ||
