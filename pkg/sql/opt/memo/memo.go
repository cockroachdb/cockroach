...
// RootIsExplain returns true if the root of the AST is an EXPLAIN operation,
// or a DistributeOp whose child is an EXPLAIN operation.
func (m *Memo) RootIsExplain() bool {
	if m.rootExpr == nil {
		return false
	}
	rootExpr := m.rootExpr
	if rootExpr.Op() == opt.DistributeOp {
		rootExpr = rootExpr.Child(0)
	}
	return rootExpr.Op() == opt.ExplainOp
}
