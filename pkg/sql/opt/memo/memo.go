...
type Memo struct {
	...
	avoidSuboptimalUnorderedScans              bool
...
func (m *Memo) Init(ctx context.Context, evalCtx *eval.Context) {
	...
	*m = Memo{
		...
		avoidSuboptimalUnorderedScans:              evalCtx.SessionData().OptimizerAvoidSuboptimalUnorderedScans,
...
func (m *Memo) IsStale(
	...
	if m.reorderJoinsLimit != int(evalCtx.SessionData().ReorderJoinsLimit) ||
		...
		m.avoidSuboptimalUnorderedScans != evalCtx.SessionData().OptimizerAvoidSuboptimalUnorderedScans ||
