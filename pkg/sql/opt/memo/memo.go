...
func (m *Memo) Init(ctx context.Context, evalCtx *eval.Context) {
	...
	*m = Memo{
		...
		disableSplitLimitedScanCostCheck:       evalCtx.SessionData().DisableSplitLimitedScanCostCheck,
	}
	m.metadata.Init()
...
func (m *Memo) IsStale(
	...
	if m.reorderJoinsLimit != int(evalCtx.SessionData().ReorderJoinsLimit) ||
		...
		m.disableSplitLimitedScanCostCheck != evalCtx.SessionData().DisableSplitLimitedScanCostCheck {
		return true, nil
	}
