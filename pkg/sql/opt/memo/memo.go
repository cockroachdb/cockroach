...
type Memo struct {
	...
	testingOptimizerRandomCostSeed         int64
...
func (m *Memo) Init(evalCtx *eval.Context) {
	...
	*m = Memo{
		...
		testingOptimizerRandomCostSeed:         evalCtx.SessionData().TestingOptimizerRandomCostSeed,
...
func (m *Memo) IsStale(
	...
	if m.reorderJoinsLimit != int(evalCtx.SessionData().ReorderJoinsLimit) ||
		...
		m.testingOptimizerRandomCostSeed != evalCtx.SessionData().TestingOptimizerRandomCostSeed ||
