...
func (m *Memo) makeFormatterSortedFrom(root GroupID) memoFormat {
	f := memoFormat{
...
func (f memoFormat) Format() string {
	tp := treeprinter.New()
	root := tp.Child("memo")
...
func (f *memoFormat) formatExpr(e *Expr) {
...
func (f *memoFormat) formatBestExprSet(tp treeprinter.Node, mgrp *group) {
...
func (f *memoFormat) formatBestExpr(be *BestExpr) {
