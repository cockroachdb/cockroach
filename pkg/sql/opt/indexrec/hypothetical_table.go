...
func BuildOptAndHypTableMaps(
	...
	for t, indexes := range indexCandidates {
		...
		hypTable := hypotheticalTable{
			Table:                t,
			primaryKeyColsOrdSet: tabPrimaryKeyCols,
		...
		for i, index := range indexes {
			...
			indexOrd := t.IndexCount() + i
			idx := hypotheticalIndex{
				tab:          &hypTable,
				name:         tree.Name(fmt.Sprintf("_hyp_%d", indexOrd)),
				cols:         index,
				colsOrdSet:   indexColsOrdSet,
				indexOrdinal: indexOrd,
				zone:         t.Index(cat.PrimaryIndex).Zone().(*zonepb.ZoneConfig),
			}
