...
func (ht *HypotheticalTable) existingRedundantIndex(index *hypotheticalIndex) cat.Index {
	for i, n := 0, ht.Table.IndexCount(); i < n; i++ {
		...
		if indexExists && !isPartialIndex && existingIndex.GetInvisibility() == 0.0 {
			// For vector indexes, also compare the distance metric. An L2 index
			// is not redundant with a cosine index on the same column.
			if index.vectorConfig != nil {
				existingVecConfig := existingIndex.VecConfig()
				if existingVecConfig != nil &&
					existingVecConfig.DistanceMetric != index.vectorConfig.DistanceMetric {
					continue
				}
			}
