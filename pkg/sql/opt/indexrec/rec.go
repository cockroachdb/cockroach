...
func getStoredCols(existingIndex cat.Index) intsets.FastIntSet {
...
func getAllCols(existingIndex cat.Index) intsets.FastIntSet {
...
// index is necessary. It returns TypeCreateIndex, nil, and
// intsets.FastIntSet{}. If there is a candidate that stores every column from
// actuallyScannedCols, typeUseless, nil, {} is returned. Theoretically, this
// should never happen.
func findBestExistingIndexToReplace(
	table cat.Table, hypIndex *hypotheticalIndex, actuallyScannedCols intsets.FastIntSet,
	...
	var existingIndexCandidateStoredCol intsets.FastIntSet
	...
	for i, n := 0, table.IndexCount(); i < n; i++ {
		...
		if existingIndex.IsNotVisible() {
			...
			if actuallyScannedCols.Difference(existingIndexAllCols).Empty() {
				...
				return TypeAlterIndex, existingIndex, intsets.FastIntSet{}
			}
		...
		if hasSameExplicitCols {
			...
			if storedColsDiffSet.Empty() {
				...
				return typeUseless, nil, intsets.FastIntSet{}
	...
	if existingIndexCandidate == nil {
		...
		return TypeCreateIndex, nil, intsets.FastIntSet{}
	}
...
func getColOrdSet(md *opt.Metadata, cols opt.ColSet, tabID opt.TableID) intsets.FastIntSet {
...
type indexRecommendation struct {
	...
	newStoredColOrds intsets.FastIntSet
}
...
func (ir *indexRecommendation) init(
	indexOrd int, hypTable *HypotheticalTable, scannedColOrds intsets.FastIntSet,
) {
...
func (ir *indexRecommendation) addStoredColOrds(scannedColOrds intsets.FastIntSet) {
