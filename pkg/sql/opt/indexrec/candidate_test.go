...
func TestFindExistingPredicates(t *testing.T) {
	...
	expectedPredicates := map[cat.Table][]PredicateInfo{
		tables[0]: {
			{predicate: "k > 0", ordinal: 0},
		},
		tables[1]: {
			{predicate: "k > 0", ordinal: 0},
			{predicate: "i = true AND j = false", ordinal: 1},
		},
	...
	for expectedTable, expectedPreds := range expectedPredicates {
		...
		for i, expectedPred := range expectedPreds {
			...
			if actualPred.predicate != expectedPred.predicate {
				t.Errorf("expected predicate %q at index %d for table %v, got %q",
					expectedPred.predicate, i, expectedTable, actualPred.predicate)
			}
			if actualPred.ordinal != expectedPred.ordinal {
				t.Errorf("expected ordinal %d at index %d for table %v, got %d",
					expectedPred.ordinal, i, expectedTable, actualPred.ordinal)
			}
		}
	}
