...
func testIndexCandidates1(
	...
	outputMap := map[cat.Table][][]cat.IndexColumn{
		...
		tables[1]: {[]cat.IndexColumn{
			indexCols[1], indexCols[2]}, []cat.IndexColumn{indexCols[0]},
...
func candidatesAreEqual(leftCandidates, rightCandidates map[cat.Table][][]cat.IndexColumn) bool {
	return checkCandidateEquality(leftCandidates, rightCandidates) &&
		checkCandidateEquality(rightCandidates, leftCandidates)
}

func checkCandidateEquality(
	leftCandidates, rightCandidates map[cat.Table][][]cat.IndexColumn,
) bool {
