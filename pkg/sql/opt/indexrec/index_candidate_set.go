...
// addRangeVariableExpr recursively walks a *memo.RangeExpr to find a
// *memo.VariableExpr, and adds this to indexCandidates.
func (ics *indexCandidateSet) addRangeVariableExpr(expr opt.Expr) {
	switch expr := expr.(type) {
	case *memo.VariableExpr:
		addVariableExprIndex(expr, ics.md, ics.rangeCandidates)
	}
	for i, n := 0, expr.ChildCount(); i < n; i++ {
		ics.addRangeVariableExpr(expr.Child(i))
	}
}
