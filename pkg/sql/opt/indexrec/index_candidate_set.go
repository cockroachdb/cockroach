...
// AddGeoSpatialIndexes is used to add single-column indexes on spatial
// functions to inverted candidates for spatial functions that can be
// index-accelerated.
	...
	case *memo.FunctionExpr:
		...
		if ok {
			...
			for i, n := 0, expr.Args.ChildCount(); i < n; i++ {
				...
				// Spatial Indexes are added to inverted candidates group.
				ics.addVariableExprIndex(child, ics.invertedCandidates)
			}
		}
	}
}
