...
func (hi *hypotheticalIndex) Column(i int) cat.IndexColumn {
	// If the index is inverted and the column ordinal refers to the last explicit
	// key column, return the inverted column.
	if hi.IsInverted() && i == (len(hi.cols)-1) {
		return hi.InvertedColumn()
	}
	if i >= len(hi.cols) {
