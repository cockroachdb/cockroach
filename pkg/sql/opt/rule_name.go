...
// RuleNameMap maps the string representation of a RuleName back to the
// RuleName itself.
var RuleNameMap map[string]RuleName

func init() {
	RuleNameMap = make(map[string]RuleName)
	for i := 1; i < int(NumRuleNames); i++ {
		rn := RuleName(i)
		RuleNameMap[fmt.Sprintf("%v", rn)] = RuleName(i)
	}
}
