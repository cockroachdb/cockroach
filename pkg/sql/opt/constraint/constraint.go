...
// constraint, as then the constrained would not constrain anything, and should
...
func (c *Constraint) tryUnionWith(evalCtx *tree.EvalContext, other *Constraint) bool {
	...
	for leftIndex < left.SpanCount() || rightIndex < right.SpanCount() {
		...
		if otherSpans == nil {
			// If firstSpan is unconstrained, then it hasn't been set yet.
	...
	// If the result of the union is unconstrained, then return false.
	if firstSpan.IsUnconstrained() {
		return false
	}

	// Not the unconstrained span, so update this constraint and return true.
...
func (c *Constraint) tryIntersectWith(evalCtx *tree.EvalContext, other *Constraint) bool {
	...
	// If firstSpan is unconstrained, then it hasn't been set yet, so
