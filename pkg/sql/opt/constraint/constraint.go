...
// Init initializes a Constraint on the given columns with a single span.
func (c *Constraint) Init(cols []opt.OrderingColumn, sp *Span) {
...
// InitSingleColumn is a more efficient version of Init for the common
// case of a single column.
func (c *Constraint) InitSingleColumn(col opt.OrderingColumn, sp *Span) {
	if sp.IsUnconstrained() {
		// Don't allow unconstrained span in a constraint. Instead just discard
		// the constraint, as its absence already means it's unconstrained.
		panic("unconstrained span cannot be part of a constraint")
