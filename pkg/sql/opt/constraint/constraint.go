...
func (c *Constraint) CollectFirstColumnValues(
	...
	for k := 0; k < numSpans; k++ {
		...
		for l, ok := 0, true; l < int(keyCount); l++ {
			...
			values = append(values, currKey.Value(0))
...
// CollectFirstColumnBoundaryValues examines the Span first-column prefix ranges
// in c and returns the included boundaries of all spans. If boundaries are
// excluded, we tighten the boundaries plus or minus 1 to collect values which
// are included. The presence of a null value causes collection to fail.
func (c *Constraint) CollectFirstColumnBoundaryValues(
	evalCtx *tree.EvalContext,
) (_ tree.Datums, ok bool) {
	if c.IsContradiction() || c.IsUnconstrained() {
		return nil, false
	}
	numSpans := c.Spans.Count()
	...
	for k := 0; k < numSpans; k++ {
		...
		if startKeyPrefix == tree.DNull || endKeyPrefix == tree.DNull {
			canHandle = false
			break
		}
		startEqualsEnd := keyCtx.Compare(0, startKeyPrefix, endKeyPrefix) == 0

		// Collect both start and end values, or if boundaries are not included,
		// the next higher or lower values within the range.
		if startKey.Length() > 1 || span.startBoundary == IncludeBoundary {
			addValueIfNotDup(startKeyPrefix)
		} else if span.startBoundary == ExcludeBoundary && !startEqualsEnd {
			if nextVal, ok := startKeyPrefix.Next(evalCtx); ok {
				if keyCtx.Compare(0, nextVal, endKeyPrefix) < 0 {
					addValueIfNotDup(nextVal)
				}
			} else {
				canHandle = false
				break
		...
		if endKey.Length() > 1 || span.endBoundary == IncludeBoundary {
			addValueIfNotDup(endKeyPrefix)
		} else if span.endBoundary == ExcludeBoundary && !startEqualsEnd {
			if prevVal, ok := endKeyPrefix.Prev(evalCtx); ok {
				if keyCtx.Compare(0, prevVal, startKeyPrefix) > 0 {
					addValueIfNotDup(prevVal)
				}
			} else {
				canHandle = false
				break
