...
// Get returns the idx-th span.
func (s *Spans) Get(idx int) *Span {
	if idx == 0 && s.numSpans > 0 {
		return &s.firstSpan
	}
	return &s.otherSpans[idx-1]
}
...
// isSorted returns true if the collection of spans is strictly ordered (see
// Span.Compare).
func (s *Spans) isSorted(keyCtx KeyContext) bool {
	for i := 1; i < s.Count(); i++ {
