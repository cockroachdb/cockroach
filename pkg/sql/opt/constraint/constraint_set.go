...
func (s *Set) ExtractNotNullCols() opt.ColSet {
	...
	for i := 0; i < s.Length(); i++ {
		c := s.Constraint(i)
		firstCol := c.Columns.Get(0)
		if firstCol.Descending() {
			// TODO(radu): handle descending columns. Currently this is not useful
			// because this function operates on constraints generated by the memo
			// constraint builder which doesn't use descending columns.
			continue
		}

		// Deduce a constraint on the first column.
		firstSpan := c.Spans.Get(0)
		// Unless the first span is of the form [ - x], or perhaps [/NULL - x], the
		// first column is not nullable.
		if !firstSpan.StartKey().IsEmpty() &&
			!(firstSpan.StartKey().Value(0) == tree.DNull && firstSpan.StartBoundary() == IncludeBoundary) {
			res.Add(int(firstCol.ID()))
		}
		// TODO(radu): try to deduce more not-null columns from multi-column
		// constraints; for example, in
		//   [/1/1/1 - /1/1/2], [/3/3/3 - /3/3/4]
		// all three columns are not-nullable.
	}
	return res
}
