...
func TestConstraintSetIntersect(t *testing.T) {
	test := func(t *testing.T, evalCtx *tree.EvalContext, cs *ConstraintSet, expected string) {
		t.Helper()
...
func TestConstraintSetUnion(t *testing.T) {
	...
	// Combine constraint that results in full span and unconstrained result.
	// @1 > 20 OR @1 = 10 OR @1 <= 40
	le40 := NewForColumn(1, &data.spLe40)
	res := gt20eq10.Union(&evalCtx, le40)
	test(t, &evalCtx, res, "unconstrained\n")
	gt20eq10 = le40.Union(&evalCtx, gt20eq10)
	test(t, &evalCtx, res, "unconstrained\n")
...
func newSpanTestData(evalCtx *tree.EvalContext) *spanTestData {
	...
	// [/10 - /10]
	_ = data.spEq10.TrySet(evalCtx, key10, IncludeBoundary, key10, IncludeBoundary)

	// (/20 - ∞]
	_ = data.spGt20.TrySet(evalCtx, key20, ExcludeBoundary, EmptyKey, IncludeBoundary)

	// [Ø - /40]
	_ = data.spLe40.TrySet(evalCtx, EmptyKey, IncludeBoundary, key40, IncludeBoundary)

	// [Ø - /80)
	_ = data.spLt80.TrySet(evalCtx, EmptyKey, IncludeBoundary, key80, ExcludeBoundary)

	// [/0 - /80)
	_ = data.spEq80.TrySet(evalCtx, key80, IncludeBoundary, key80, IncludeBoundary)

	// [/10/15 - ∞]
	key1015 := key10.Concat(key15)
	_ = data.spGe1015.TrySet(evalCtx, key1015, IncludeBoundary, EmptyKey, IncludeBoundary)
