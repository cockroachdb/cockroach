...
func TestConstraintUnion(t *testing.T) {
	...
	// Multiple spans that yield the full set (should return ok=false and not
	// modify either span).
	left = data.cLt10
	right = data.c5to25
...
func newConstraintTestData(evalCtx *tree.EvalContext) *constraintTestData {
	...
	// [Ø - /10)
	_ = span.TrySet(evalCtx, EmptyKey, IncludeBoundary, key10, ExcludeBoundary)
	data.cLt10.init(1, &span)

	// (/20 - ∞]
	_ = span.TrySet(evalCtx, key20, ExcludeBoundary, EmptyKey, IncludeBoundary)
	data.cGt20.init(1, &span)

	// [/1 - /10]
	_ = span.TrySet(evalCtx, key1, IncludeBoundary, key10, IncludeBoundary)
	data.c1to10.init(1, &span)

	// (/5 - /25)
	_ = span.TrySet(evalCtx, key5, ExcludeBoundary, key25, ExcludeBoundary)
	data.c5to25.init(1, &span)

	// [/20 - /30)
	_ = span.TrySet(evalCtx, key20, IncludeBoundary, key30, ExcludeBoundary)
	data.c20to30.init(1, &span)

	// [/30 - /40]
	_ = span.TrySet(evalCtx, key30, IncludeBoundary, key40, IncludeBoundary)
	data.c30to40.init(1, &span)

	// [/40 - /50]
	_ = span.TrySet(evalCtx, key40, IncludeBoundary, key50, IncludeBoundary)
	data.c40to50.init(1, &span)

	// (/60 - /70)
	_ = span.TrySet(evalCtx, key60, ExcludeBoundary, key70, ExcludeBoundary)
	data.c60to70.init(1, &span)

	// [/'cherry'/true - /'raspberry'/false)
	_ = span.TrySet(evalCtx, cherry, IncludeBoundary, raspberry, ExcludeBoundary)
	data.cherryRaspberry.initComposite([]opt.ColumnIndex{1, 2}, &span)

	// [/'mango'/true - /'strawberry']
	_ = span.TrySet(evalCtx, mango, IncludeBoundary, strawberry, IncludeBoundary)
