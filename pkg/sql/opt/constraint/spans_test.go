...
func TestSpansSortAndMergeOverlapping(t *testing.T) {
	keyCtx := testKeyContext(1)
	evalCtx := keyCtx.EvalCtx

	// To test SortAndMergeOverlapping, we note that the result can also be
	// obtained by creating a constraint per span and calculating the union.
	// We generate random cases and cross-check.
	for testIdx := 0; testIdx < 100; testIdx++ {
		...
		spans.SortAndMergeOverlaps(keyCtx)
