...
func unionSpans(left []InvertedSpan, right []InvertedSpan) []InvertedSpan {
	...
	makeLastLeft()
	for j < len(right) {
		if (currSpan.end != nil && bytes.Compare(currSpan.end, right[j].start) <= 0) ||
			(currSpan.end == nil && bytes.Compare(currSpan.start, right[j].start) > 0) {
			spans = append(spans, currSpan)
			pickSide()
			makeLastLeft()
		} else {
			// Extend current span.
			if extendCurrSpan(&currSpan, right[j]) {
				lastLeft = false
				makeLastLeft()
			}
		}
...
// Modifies the spans in left.
func subtractSpans(left []InvertedSpan, right []InvertedSpan) []InvertedSpan {
	// TODO(sumeer): implement
	return nil
}

func extendCurrSpan(curr *InvertedSpan, s InvertedSpan) bool {
	// TODO(sumeer): implement
	return false
}
