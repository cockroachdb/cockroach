...
// MakeSingleInvertedValSpan constructs a span of the form [val, val].
...
// IsSingleInvertedValSpan returns true iff the span is of the form
// [val, val].
func IsSingleInvertedValSpan(span InvertedSpan) bool {
	if len(span.start) == 0 {
		return len(span.end) == 1 && span.end[0] == '\x00'
	}
	return len(span.start)+1 == len(span.end) && span.end[len(span.end)-1] == '\x00' &&
		bytes.Equal(span.start, span.end[:len(span.end)-1])
}
...
func formatSpan(b *strings.Builder, span InvertedSpan) {
	...
	spanEndOpenOrClosed := ')'
	if IsSingleInvertedValSpan(span) {
		end = span.start
		spanEndOpenOrClosed = ']'
	}
