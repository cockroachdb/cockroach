...
type DatumsToInvertedExpr interface {
	...
	// Convert calls above, which will return an interface{}. When an inverted
	// index row is looked up and is known to be relevant to left rows 0, 2, 4
	// (based on the spans in the SpanExpressionProtos), then PreFilter will be
	// called with len(preFilters) == 3, containing the three interface{}
	// objects returned previously for 0, 2, 4. The results will indicate which
	// span expressions should actually use this inverted index row. More
	// concretely, the span expressions for ST_Intersects work on cell
	// coverings which are different from the bounding box, and the
	// pre-filtering works on the bounding box. The bounding box pre-filtering
	// complements the span expressions, by eliminating some false positives
	// caused by cell coverings.
