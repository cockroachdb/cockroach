...
//   inclusive end.
// - It primarily handles spans only on the inverted column, with a way to
...
func subtractSpans(left []InvertedSpan, right []InvertedSpan) []InvertedSpan {
	...
	for j < len(right) {
		...
		if cmpEndStart > 0 {
			...
			cmpStart := bytes.Compare(currSpan.start, right[j].start)
			if cmpStart < 0 {
				out = append(out, InvertedSpan{start: currSpan.start, end: right[j].start})
				currSpan.start = right[j].start
			}
			// Else cmpStart == 0
