...
func (c *CustomFuncs) CanUseUniqueChecksForInsertFastPath(
	...
	for i := range ins.UniqueChecks {
		...
		for ; expr != nil && expr.Op() == opt.LookupJoinOp; expr = expr.NextExpr() {
			...
			for skipProjectExpr, ok = inputExpr.(*memo.ProjectExpr); ok; skipProjectExpr, ok = inputExpr.(*memo.ProjectExpr) {
				inputExpr = skipProjectExpr.Input
			}
		...
		if lookupJoin == nil || expr == nil || withScan == nil {
			return memo.UniqueChecksExpr{}, nil, false
		}
...
func (c *CustomFuncs) handleSingleRowInsert(
	...
	_, valueRowIsTuple := insertValues.Rows[0].(*memo.TupleExpr)
	if !valueRowIsTuple {
		return false
	}
	...
	for check = check.FirstExpr(); check != nil; check = check.NextExpr() {
		...
		selectFilterIsOK := func(selectExpr *memo.SelectExpr, scanExpr *memo.ScanExpr) bool {
			...
			if !c.ignorablePKNEConstCheck(selectExpr.Filters[0].Condition, scanExpr.Table) {
				return false
			}
			return true
		}
...
func (c *CustomFuncs) ignorablePKNEConstCheck(expr opt.ScalarExpr, tableID opt.TableID) (ok bool) {
	switch t := expr.(type) {
	case *memo.OrExpr:
		return c.ignorablePKNEConstCheck(t.Left, tableID) && c.ignorablePKNEConstCheck(t.Right, tableID)
	case *memo.NeExpr:
		...
		leftVariable := t.Left.(*memo.VariableExpr)
		_, _, ok := c.findKeyColOrd(leftVariable.Col, tableID, 0 /* primary index */)
		if !ok {
			return false
		}
		return true
