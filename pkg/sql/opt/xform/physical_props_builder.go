...
func (b physicalPropsBuilder) canProvideOrdering(eid memo.ExprID, required memo.Ordering) bool {
	...
	case opt.WindowByOp:
		def := b.mem.LookupPrivate(mexpr.AsWindowBy().Def()).(*memo.WindowByDef)
...
func (b physicalPropsBuilder) buildChildProps(
	...
	if required == memo.MinPhysPropsID {
		switch mexpr.Operator() {
		case opt.LimitOp, opt.OffsetOp, opt.WindowByOp:
		default:
	...
	case opt.LimitOp, opt.OffsetOp, opt.WindowByOp:
		// Limit/Offset/WindowBy require the ordering in their private.
		if nth == 0 {
