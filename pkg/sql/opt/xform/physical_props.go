...
func BuildChildPhysicalProps(
	...
	case opt.DistinctOnOp:
		colSet := parent.(*memo.DistinctOnExpr).Private().(*memo.GroupingPrivate).GroupingCols
		if input, ok := parent.Child(nth).(memo.RelExpr); ok {
			if colStat, ok := input.Relational().Stats.ColStats.Lookup(colSet); ok {
				childProps.LimitHint = distinctOnLimitHint(colStat.DistinctCount, parentProps.LimitHint)
			}
		}
	case opt.SelectOp:
