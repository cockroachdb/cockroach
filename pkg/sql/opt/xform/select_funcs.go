...
func (c *CustomFuncs) GenerateInvertedIndexZigzagJoins(
	...
	iter.ForEach(func(index cat.Index, filters memo.FiltersExpr, indexCols opt.ColSet, _ bool) {
		if index.NonInvertedPrefixColumnCount() > 0 {
			// We don't yet support using multi-column inverted indexes with zigzag
			// joins.
			return
		}
		...
		idx := index.NonInvertedPrefixColumnCount()
		leftVals[idx] = c.e.f.ConstructConstVal(&leftVal, leftVal.ResolvedType())
		leftTypes[idx] = leftVal.ResolvedType()
		rightVals[idx] = c.e.f.ConstructConstVal(&rightVal, rightVal.ResolvedType())
		rightTypes[idx] = rightVal.ResolvedType()
		invertedCol := scanPrivate.Table.ColumnID(index.VirtualInvertedColumn().Ordinal())
		zigzagJoin.LeftFixedCols[idx] = invertedCol
		zigzagJoin.RightFixedCols[idx] = invertedCol
