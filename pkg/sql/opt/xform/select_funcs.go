...
func (c *CustomFuncs) GenerateInvertedIndexScans(
	...
	iter.ForEach(func(index cat.Index, filters memo.FiltersExpr, indexCols opt.ColSet, _ bool, _ memo.ProjectionsExpr) {
		...
		// The reason we also check that spanExpr.Operator is not inverted.None
		// here is that sometimes we eliminate the UNION operator in the tree,
		// replacing it with a non-nil FactoredUnionSpans in the SpanExpression,
		// and that case needs to be noticed and filtered.
