...
// addExpr adds a new logically equivalent expression to the memo group.
func (g *memoGroup) addExpr(e memoExpr) {
	g.otherExprs = append(g.otherExprs, e)
}

// bestExprCount returns the number of bestExprs in the group.
func (g *memoGroup) bestExprCount() int {
	if g.firstBestExpr.required == 0 {
		return 0
	}
