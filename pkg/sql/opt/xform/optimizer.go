...
func (o *Optimizer) enforceProps(
	...
	if !required.Ordering.Any() {
		...
		// required ordering.
		var impliesRequired bool
		var bestPrefixLength, bestPrefixIdx int
		interestingOrderings := ordering.DeriveInterestingOrderings(member)
		for i, orderingChoice := range interestingOrderings {
			length, implies := orderingChoice.CommonPrefixLength(&required.Ordering)
			if implies {
			...
			if length > bestPrefixLength {
				impliesRequired = implies
				bestPrefixLength = length
				bestPrefixIdx = i
			}
		}
		// We only add the enforcer if the required ordering is not implied by the
		// input ordering.
		if !impliesRequired {
			var inputOrdering props.OrderingChoice
			if bestPrefixLength > 0 {
				// Retrieve the common prefix of the interesting ordering and the
				// required ordering and add an enforcer using it.
				inputOrdering = (&interestingOrderings[bestPrefixIdx]).CommonPrefix(&required.Ordering)
			}
			enforcer := &memo.SortExpr{Input: state.best}
			enforcer.InputOrdering = inputOrdering
			memberProps := BuildChildPhysicalProps(o.mem, enforcer, 0, required)
