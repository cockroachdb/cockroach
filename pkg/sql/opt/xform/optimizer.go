...
func (o *Optimizer) TryFastPath() (ok bool, err error) {
	...
	newRoot, ok := fastpath.TryOptimizerFastPath(o.ctx, o.evalCtx, &o.f)
	if !ok {
		return ok, nil
	}
	rootProps := o.mem.RootProps()
	if !o.setProps(newRoot, rootProps) {
		// The expression tree requires enforcers or contains invalid expressions.
		return false, nil
	}
	// Fast path success.
...
// setProps sets the required and provided properties for the given expression
// and its descendants. It is used when TryFastPath (see above) successfully
// matches a fast-path rule and builds a fully-optimized expression to replace
// the root.
//
// The given expression tree is not modified in any way. If any of the required
// properties is not supplied by the expression tree, setProps returns ok=false,
// which indicates that the tree is not fully optimized.
func (o *Optimizer) setProps(parent opt.Expr, parentProps *physical.Required) (ok bool) {
	opt.MaybeInjectOptimizerTestingPanic(o.ctx, o.evalCtx)
	var relParent memo.RelExpr
	switch t := parent.(type) {
	case memo.RelExpr:
	...
	case memo.ScalarPropsExpr:
		// Disallow UDFs because it is difficult to prove their correctness, and
		// they are unlikely to be matched by fast-path rules.
		// TODO(drewk): think about whether it is ever OK to allow UDFs.
		if t.ScalarProps().HasUDF {
			return false
		}
		// Short-circuit traversal of scalar expressions with no nested subquery,
		// since there's only one possible tree, and no properties have to be set.
		if !t.ScalarProps().HasSubquery {
			return true
		}
	}
