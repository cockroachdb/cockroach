...
func planIsDominant(left, right memo.RelExpr) bool {
	...
	//
	// Rule 1: A Select is dominated by a Scan.
	//
	// Justification: It is generally cheaper to filter rows via a Scan constraint
	// than by materializing them and passing them through a Select operator. The
	// rule may not hold if very few rows are scanned, since the lone Scan may
	// perform more lookups. However, this overhead is expected to be very small
	// in comparison to the worst case when the Select plan has to produce and
	// process many more rows.
	_, leftIsScan := left.(*memo.ScanExpr)
	_, rightIsSelect := right.(*memo.SelectExpr)
	return leftIsScan && rightIsSelect
}
