...
type scanIndexIter struct {
	...
	// originalFilters is an alternative set of filters used to determine if
...
// SetOriginalFilters specifies an non-reduced, original set of filters used to
// determine if a partial index can be enumerated. If the filters do not imply a
// partial index predicate, the scanIndexIter will check if the original filters
// imply the predicate and enumerate the partial index if successful.
//
// Specifying original filters is useful in nested iteration when two partial
...
// remaining filters passed to the Init function, will be unable to prove
// implication of the second partial index. By specifying the filters passed to
// the outer Init as original filters of the inner loop, implication can be
// proven for the second partial index.
...
// partial index predicate expression. If the filters imply the predicate, it
// returns the remaining filters that must be applied after a scan over the
// partial index and true. If the filters do not imply the predicate and
// originalFilters is not null, it tries to prove that the originalFilters
// imply the predicate. If the predicate is not implied by the filters, ok=false
// is returned.
func (it *scanIndexIter) filtersImplyPredicate(
	pred memo.FiltersExpr,
) (remainingFilters memo.FiltersExpr, ok bool) {
	// First check if the filters imply the predicate.
	if remainingFilters, ok = it.im.FiltersImplyPredicate(it.filters, pred); ok {
		return remainingFilters, true
	}

	// If the filters did not imply the predicate, then check if the
	// originalFilters imply the predicate, if they exist.
