...
// they would imply idx2's predicate, but the remaining filters would be
// (a = 1 AND b > 0 AND c = 2). Only (d > 0) can be removed during implication
// of idx2's predicate. The (b > 0) expression would be applied in an
// unnecessary Select that wraps the zigzag join.
