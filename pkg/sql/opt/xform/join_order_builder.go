...
// and the edges are built using the ON conditions of the joins. For each edge,
// restrictions are placed on whether the edge can be used to construct a join
// between any two given sets of vertexes to prevent invalid plans from being
// constructed.
//
...
// edge, if the aforementioned edge conditions are satisfied, a new join is
// created using the filters from that edge and the relation subsets as inputs.
//
...
// right-asscom properties are provided by lookup tables. Depending on these
// properties, the TES will be expanded to take into account whether certain
// transformations of the join tree are valid. During execution of the DPSube
// algorithm, the TES is used to decide whether a given edge can be used to
...
//    SELECT *
//    FROM uv
//    INNER JOIN (SELECT * FROM xy LEFT JOIN uv ON x = a)
//    ON x = a
//
// The left join's TES will include all three relations. Because all relations
// in the join's TES must be a part of its inputs, the uv and ab relations
// cannot be pulled out of the left join. This prevents the invalid plan from
// being considered.
//
...
// -------------------------------
//
// Edges from inner joins are handled separately from non-inner edges because
// inner join trees have a unique property: they can be modeled as a series of
// cross joins followed by a series of selects with the inner join conjuncts.
// This allows inner join conjuncts to be treated as 'detached' from their
// original operator, free to be combined with conjuncts from other inner joins.
...
func (jb *JoinOrderBuilder) makeInnerEdge(op *operator, filters memo.FiltersExpr) {
	if len(filters) == 0 {
		...
		e := &edge{op: op, filters: filters, ses: jb.getSES(filters)}
		e.calcTES(jb.edges)
		jb.edges = append(jb.edges, *e)
		jb.innerEdges.Add(len(jb.edges) - 1)
		return
	}
	for i := range filters {
		// Create an edge for each conjunct.
		edgeFilters := filters[i : i+1]
		e := &edge{op: op, filters: edgeFilters, ses: jb.getSES(edgeFilters)}
		e.calcTES(jb.edges)
		jb.edges = append(jb.edges, *e)
		jb.innerEdges.Add(len(jb.edges) - 1)
	}
...
func (jb *JoinOrderBuilder) makeTransitiveEdge(col1, col2 opt.ColumnID) {
	...
	// construct the TES for this edge.
	...
	e.filters = memo.FiltersExpr{jb.f.ConstructFiltersItem(condition)}
	e.ses = jb.getSES(e.filters)
	e.calcTES(jb.edges)
...
// getSES returns a vertexSet containing all relations referenced by the join
// predicate (syntactic eligibility set from paper). If a join uses a predicate
// in its ON condition, all relations in the SES must be part of the join's
// inputs. For example, in this query:
//
//    SELECT *
//    FROM xy
...
func (jb *JoinOrderBuilder) getSES(predicate memo.FiltersExpr) vertexSet {
	...
	return jb.getRelations(freeVars)
}
...
// assoc returns true if the given edges are associative with each other.
...
// leftAsscom returns true if the given edges have the left-asscom property.
...
// rightAsscom returns true if the given edges have the right-asscom property.
...
func (s bitSet) next(startVal vertexIndex) (elem uint64, ok bool) {
