...
//
// The vertexes of the graph would represent the base relations ab,
...
//		      a = u
//
// Finally, the DPSube algorithm is executed (see citations: [8]). DPSube
...
// For each pair of relation subsets, a join is added to the memo as long as
// there exists at least one edge that connects the two subsets (which ensures
// that we don't generate cross joins). All connecting edges are added to the
// new join's ON condition.
//
...
func (jb *JoinOrderBuilder) populateGraph(rel memo.RelExpr) {
	...
	case *memo.InnerJoinExpr:
		...
		startRelations := jb.allRelations()
		...
		relSet := endRelations.difference(startRelations)
		jb.plans[relSet] = t
...
func (jb *JoinOrderBuilder) ensureClosure(join memo.RelExpr) {
	...
	equivFDs.AddEquivFrom(&join.Relational().FuncDeps)

	// Remove outer column references.
	nonOuterCols := join.Relational().OutputCols.Difference(join.Relational().OuterCols)
	equivFDs.ProjectCols(nonOuterCols)
...
// allRelations returns a relationSet that represents all relations that are
// currently part of the join graph
func (jb *JoinOrderBuilder) allRelations() relationSet {
	...
	return relationSet((1 << len(jb.vertexes)) - 1)
}
