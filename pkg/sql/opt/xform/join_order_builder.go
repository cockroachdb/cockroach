...
// xy rowcount: 5
// ab rowcount: 10
// cd rowcount: 10,000,000
// uv rowcount: 10,000,000,000
//
// The vertexes of the graph would be initialized with base relations xy,
// (SELECT * FROM ab WHERE u = 5), uv, and cd. The edges correspond to
// relationships between those base relations. For example, an edge exists
// between the xy and cd vertexes, because of the x = c filter.
//
// Next, equivalence closure is computed for the graph edges. In this example,
// the filter x = a (among others) would be added to the graph, since a = u and
// u = c and x = c. At this point, if the graph were translated back into query
...
//			     └── x = a
//
// Note that the uv lookup join uses an x = u edge that wasn't present in the
