...
func (jb *JoinOrderBuilder) addJoins(s1, s2 vertexSet) {
	...
	for i, ok := jb.innerEdges.Next(0); ok; i, ok = jb.innerEdges.Next(i + 1) {
		...
		if e.checkInnerJoin(s1, s2) {
			...
			appliedEdges.Add(i)
			if areFiltersRedundant(&jb.equivs, e.filters, &notNullCols) {
...
// the logical properties of the join inputs, or from other filters. notNullCols
// may be mutated to include columns that are null-rejected by the filters.
func areFiltersRedundant(
	...
	if !ok1 || !ok2 {
		if ok1 {
			notNullCols.Add(var1.Col)
		}
		if ok2 {
			notNullCols.Add(var2.Col)
		}
		return false
	}
