...
) (newSelect memo.RelExpr, newCols opt.ColSet) {
...
func (c *CustomFuncs) SplitJoinWithEquijoinDisjuncts(
	...
	switch joinRel.Op() {
	case opt.InnerJoinOp:
		allPKColsSet := c.e.funcs.UnionCols(
			c.e.funcs.PrimaryKeyCols(origLeftScanPrivate.Table),
			c.e.funcs.PrimaryKeyCols(origRightScanPrivate.Table))
		allJoinColsSet := c.e.funcs.UnionCols(
			origLeftScanPrivate.Cols,
			origRightScanPrivate.Cols)
		aggCols = c.e.funcs.DifferenceCols(allJoinColsSet, allPKColsSet)
		groupingCols = allPKColsSet
	case opt.SemiJoinOp, opt.AntiJoinOp:
		projectedPKColsSet := c.e.funcs.PrimaryKeyCols(origLeftScanPrivate.Table)
		projectedJoinColsSet := origLeftScanPrivate.Cols
		aggCols = c.e.funcs.DifferenceCols(projectedJoinColsSet, projectedPKColsSet)
