...
func (c *CustomFuncs) findJoinFilterRange(
	...
	for filterIdx := range filters {
		...
		if props.TightConstraints && props.Constraints.Length() > 0 {
			...
			// Range spans aren't yet supported for non-canonical filters like
			// they are for const filters so filter those out here
			// (non-canonical const spans can be turned into a canonical filter,
			// see makeConstFilter).
