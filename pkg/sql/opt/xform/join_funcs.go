...
func (c *CustomFuncs) GenerateLookupJoins(
	...
	iter.ForEach(func(index cat.Index, onFilters memo.FiltersExpr, indexCols opt.ColSet, isCovering bool) {
		...
		if len(lookupJoin.KeyCols) == 0 && (joinType == opt.LeftJoinOp || joinType == opt.AntiJoinOp) {
			// If we didn't find any equality columns above, it might be because some
			// of the index columns were constrained to multiple constant values. As
			// described above, we cannot use the method constructJoinWithConstants to
			// create a cross join as the input for left or anti joins, since it would
			...
			eqFilters, constFilters = c.findFiltersForIndexLookup(
