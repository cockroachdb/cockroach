	...
	for i := range filters {
		if filters[i].Condition.Op() == opt.OrOp {
			if leftPreds, rightPreds, ok =
				...
				// target ColSet.
				// This is required because ColumnID mapping of the join filters is done
				// in a single pass from a source ColSet including both left and right
				// join relations to the combined target ColSets of the duplicated left
				// and right scans (see newJoinFilters in
				// SplitJoinWithEquijoinDisjuncts) So, getting the order wrong would
				// mean incorrect mapping of ColumnIDs in the left table to ColumnIDs in
				// the right table, or vice versa.
...
func (c *CustomFuncs) makeFilteredSelectForJoin(
	...
	newCols = c.OutputCols(newScan)
	newSelect =
		c.e.f.ConstructSelect(
			newScan,
			c.MapFilterCols(filters, origCols, newCols),
		)
...
func (c *CustomFuncs) SplitJoinWithEquijoinDisjuncts(
	...
	newLeftScanPrivate, newRightScanPrivate :=
		c.duplicateScanPrivatesForJoin(leftSP, rightSP, swapDuplicateScanPrivateOrder)
	...
	newJoinFilters :=
		c.MapFilterCols(
			...
			amendedOrigCols,
			c.UnionCols(newLeftCols, newRightCols),
		)
	...
	newJoinFilters2 :=
		c.MapFilterCols(
			...
			amendedOrigCols,
			c.UnionCols(newLeftCols2, newRightCols2),
		)
