...
func (c *CustomFuncs) makeRangeFilter(col opt.ColumnID, filter memo.FiltersItem) memo.FiltersItem {
	...
	if props.Constraints.Length() == 0 || props.Constraints.Constraint(0).Spans.Count() != 1 {
		panic(errors.AssertionFailedf("makeRangeFilter needs at least one constraint with one span"))
	}
