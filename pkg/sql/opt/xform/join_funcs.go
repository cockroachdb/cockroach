...
func (c *CustomFuncs) GetLocalityOptimizedLookupJoinExprs(
	...
	// anti joins if the ON condition is empty, but only true for inner and left
	// joins (and for semi / anti joins with an ON condition) if
	// private.LookupColsAreTableKey is true.
	if (private.JoinType != opt.SemiJoinOp && private.JoinType != opt.AntiJoinOp) || len(on) > 0 {
		if !private.LookupColsAreTableKey {
			return
		}
	}
