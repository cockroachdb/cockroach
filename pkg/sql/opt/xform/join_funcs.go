	...
	iter.ForEach(func(index cat.Index, onFilters memo.FiltersExpr, indexCols opt.ColSet, isCovering bool, constProj memo.ProjectionsExpr) {
		...
		for j := 0; j < numIndexKeyCols; j++ {
			...
			if len(foundVals) > 1 || foundRange {
				if joinType == opt.LeftJoinOp || joinType == opt.AntiJoinOp || foundRange {
...
func (c *CustomFuncs) findFiltersForIndexLookup(
	...
	for j := 0; j < numIndexKeyCols; j++ {
		...
		if !c.isCanonicalFilter(constFilter) {
			if len(values) == 0 {
				constFilter = c.makeConstFilter(idxCol, values)
			} else if foundRange {
				// TODO(treilly): this is largely untested, forward looking code,
				// findJoinFilterRange apparently never encounters any constraint spans
				// built from expressions other than what isCanonicalFilter supports.
				// Also if we invoked this code the filter difference code in
				// GenerateLookupJoins won't kick in properly and the filter would be
				// duplicated in a span and a filter.  For this reason I made this dead
				// code by checking isCanonicalFilter in findJoinFilterRange so we won't
				// have any surprises.  A good canary case to turn on and fix this code
				// would be turning like operations into span constraints.
...
func (c *CustomFuncs) isCanonicalFilter(filter memo.FiltersItem) bool {
	...
	checkExpr = func(expr opt.Expr) bool {
		...
		case *memo.InExpr:
			return checkExpr(t.Left) && checkExpr(t.Right)
		case *memo.VariableExpr, *memo.ConstExpr:
			return true
		}
		return false
	}
...
// column is constrained by at least 1 tight constraint.
func (c *CustomFuncs) makeRangeFilter(col opt.ColumnID, filter memo.FiltersItem) memo.FiltersItem {
	...
	return c.e.f.ConstructFiltersItem(c.e.f.ConstructAnd(left, right))
}
...
// See if there exists an inequality range for this column.  Currently we
// only support <,>,<=,>=.
func (c *CustomFuncs) findJoinFilterRange(
	...
	for filterIdx := range filters {
		...
		if props.TightConstraints && !props.Constraints.IsUnconstrained() {
			constraintCol := props.Constraints.Constraint(0).Columns.Get(0).ID()
			if constraintCol != col {
				continue
			}
			// See comment in findFiltersForIndexLookup for why we check filter here.
			return filterIdx, c.isCanonicalFilter(filters[filterIdx])
		}
	}
	return 0, false
}
