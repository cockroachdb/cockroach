...
// findFiltersForIndexLookup finds the equality and constrained filters in
// filters that can be used to constrain the given index.  Constrained filters
...
func (c *CustomFuncs) findFiltersForIndexLookup(
...
) (eqFilters, constrainedFilters memo.FiltersExpr, rightSideCols opt.ColList) {
	numIndexKeyCols := index.LaxKeyColumnCount()
	...
	for j := 0; j < numIndexKeyCols; j++ {
		...
		if constrainedFilters == nil {
			constrainedFilters = make(memo.FiltersExpr, 0, numIndexKeyCols-j)
		}
		...
		if foundConstFilter {
			...
			constrainedFilters = append(constrainedFilters, constFilter)
		}
		// Non-canonical range filters aren't supported and are already filtered
		// out by findJoinFilterRange above.
		if foundRange {
			constrainedFilters = append(constrainedFilters, filters[allIdx])
	...
	return eqFilters, constrainedFilters, rightSideCols
}
...
func (c *CustomFuncs) findJoinFilterRange(
	...
	for filterIdx := range filters {
		...
		if props.TightConstraints && props.Constraints.Length() > 0 {
			...
			// Range spans aren't supported for non-canonical filters like
			// they are for const filters so filter those out here, see
			// findFiltersForIndexLookup where we make range filters for more.
