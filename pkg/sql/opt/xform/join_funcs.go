...
func (c *CustomFuncs) generateLookupJoinsImpl(
	...
	iter.ForEach(func(index cat.Index, onFilters memo.FiltersExpr, indexCols opt.ColSet, _ bool, _ memo.ProjectionsExpr) {
		...
		lookupConstraint, numEqualityCols := c.cb.Build(index, onFilters, optionalFilters, derivedfkOnFilters)
		...
		if numEqualityCols == 0 && !inputProps.Cardinality.IsZeroOrOne() &&
		...
		if lookupJoin.LookupExpr != nil {
			lookupJoin.NumEqualityCols = numEqualityCols
		}
