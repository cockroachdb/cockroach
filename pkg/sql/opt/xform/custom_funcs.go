...
// checkConstraintFilters finds all the constraints that are validated on the
// table and converts them into filters that can potentially allow for more
// constrained index scans. For more detail, see comment above
// GenerateConstrainedScans.
//
// A constraint is only converted into a filter if it constrains a non nullable
// column, has been validated and can not evaluate to NULL. This is because
// check constraints behave differently from filters on NULL values. Check
// constraints are satisfied when their expression evaluates to NULL, while
// filters are not.
//
...
func (c *CustomFuncs) checkConstraintFilters(tabID opt.TableID) memo.FiltersExpr {
	...
	for i := 0; i < numCheckConstraints; i++ {
		...
		if memo.CheckMaybeEvaluatesToNull(checkConstraint) {
			continue
		}

		// The check constraint may still evaluate to NULL if it involves
		// nullable columns. Add only the check constraints that constrain
...
func (c *CustomFuncs) GenerateConstrainedScans(
	...
	// use the secondary or tertiary indexes. However, given that we can use
	// the constraints on a, b and c, we can actually use the secondary and
	// tertiary indexes. In fact, for the above query we can do the following:
	//
	//select
	...
	for iter.next() {
		...
		// use it to further constrain
		// an index scan. We should revisit this once we have index skip scans.
		// A constraint that may not constrain an index scan may still allow
		// the index to be used more effectively if an index skip scan is
		// possible.
		if len(checkFilters) != 0 {
			remainingFilters.IntersectionWith(explicitFilters)
		}
