...
func (c *CustomFuncs) SplitScanIntoUnionScans(
	...
	if newSpans.Count() <= 1 {
		// We need more than one new span in order to split the existing Scan into
		// multiple Scans.
		return nil
	}
	...
	if float64(newSpans.Count()*limitVal*threshold) >= scan.Relational().Stats.RowCount {
		// Splitting the scan may not be worth the overhead - creating a sequence
		// of scans unioned together is expensive, so we don't want to create the
		// plan only for the optimizer to use something else. We only want to create
		// the plan if it is likely to be used.
		return nil
	}
	...
	// We have already checked that there is more than one Span in newSpans, and
	// ExtractSingleKeySpans has already ensured that all key lengths are the
	// same. Therefore, we can retrieve the key length from the first Span.
	keyLength := newSpans.Get(0).StartKey().Length()
	...
	// Reuse the old TableID to make the first scan. All subsequent scans will
	// take new TableIDs.
