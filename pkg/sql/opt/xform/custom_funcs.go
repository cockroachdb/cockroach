...
func (c *CustomFuncs) ConstructMergeJoins(
	...
	for _, o := range leftOrders {
		...
		merge := memo.MergeJoinExpr{Left: left, Right: right, On: on}
...
func (c *CustomFuncs) GenerateLookupJoins(
	...
	for iter.next() {
		...
		if iter.isCovering() {
			...
			lookupJoin.LookupCols = scanPrivate.Cols
		...
		lookupJoin.LookupCols = scanPrivate.Cols.Intersection(indexCols)
		for i := range pkCols {
			lookupJoin.LookupCols.Add(int(pkCols[i]))
		}

		var indexJoinOn memo.FiltersExpr
		...
		if c.FiltersBoundBy(lookupJoinOn, onCols) {
			...
			conditions := lookupJoinOn
			lookupJoinOn = c.ExtractBoundConditions(conditions, onCols)
			indexJoinOn = c.ExtractUnboundConditions(conditions, onCols)
		}

		indexJoin := memo.LookupJoinExpr{
			Input: c.e.f.ConstructLookupJoin(
				input,
				lookupJoinOn,
		...
		indexJoin.LookupCols = scanPrivate.Cols.Difference(indexCols)
