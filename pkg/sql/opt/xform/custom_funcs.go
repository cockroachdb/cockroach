...
// CanDoLookupJoin returns true if a join with the given join type can be
...
func (c *CustomFuncs) CanDoLookupJoin(
...
func (c *CustomFuncs) ConstructLookupJoin(
	...
	for i := 0; i < numCols; i++ {
		...
		if lookupJoinDef.KeyCols == nil {
			lookupJoinDef.KeyCols = make(opt.ColList, 0, numCols)
		}
		lookupJoinDef.KeyCols = append(lookupJoinDef.KeyCols, inputCol)
	}
	if lookupJoinDef.KeyCols == nil {
		// CanDoLookupJoin ensures this is not possible.
