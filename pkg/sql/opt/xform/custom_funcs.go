...
// EnsureNotNullCol ensures that there is at least one not-null column in the
// given expression. If there is already at least one not-null column,
// EnsureNotNullCol returns the expression unchanged. Otherwise, it calls
// TryAddKeyToScan, which will try to augment the expression with the primary
// key of the underlying table scan (guaranteed to be not-null). In order for
// this call to succeed, the input expression must be either a Scan or a Scan
// wrapped in a Select. Otherwise, EnsureNotNullCol will panic.
func (c *CustomFuncs) EnsureNotNullCol(expr memo.RelExpr) memo.RelExpr {
	...
	panic(errors.AssertionFailedf("TryAddKeyToScan failed. Input type: %T", expr))
}

// NotNullCol returns the first not-null column from the input expression.
// EnsureNotNullCol must have been called previously to ensure that such a
// column exists.
