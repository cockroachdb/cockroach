...
// non-nullable.  This is because check constraints behave differently from
// filters on NULL.  Check constraints are satisfied when their expression
// evaluates to NULL, while filters are not.
//
...
func (c *CustomFuncs) checkConstraintFilters(tabID opt.TableID) memo.FiltersExpr {
	...
	for i := 0; i < numCheckConstraints; i++ {
		...
		if memo.ExprIsNullable(checkConstraint, &notNullCols) {
			continue
		}
...
// by trying to use the check constraints that apply to the table being scanned.
