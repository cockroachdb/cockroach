...
type memoFormatter struct {
	...
	// groupIdx remembers the group index (in the groups slice) for each
	// expression.
...
func (mf *memoFormatter) numberExpr(expr opt.Expr) {
	firstExpr := func(expr opt.Expr) opt.Expr {
	...
	grp := firstExpr(expr)
	groupIdx, ok := mf.groupIdx[grp]
	if !ok {
		groupIdx = len(mf.groups)
		mf.groups = append(mf.groups, group{first: grp})
		mf.groupIdx[grp] = groupIdx
	}
	mf.groupIdx[expr] = groupIdx
}
...
func (mf *memoFormatter) formatExpr(e opt.Expr) {
	...
	for i := 0; i < e.ChildCount(); i++ {
		...
		fmt.Fprintf(mf.buf, " G%d", mf.groupIdx[child]+1)
	}
...
func (mf *memoFormatter) formatBest(best memo.RelExpr, required *props.Physical) {
	...
	for i := 0; i < best.ChildCount(); i++ {
		...
		fmt.Fprintf(mf.buf, " G%d", mf.groupIdx[bestChild]+1)
