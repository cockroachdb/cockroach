...
func (c *coster) computeSortCost(candidate *memo.BestExpr, logical *props.Logical) memo.Cost {
	...
	// Add the cost
	if rowCount > 1.0 {
		// This is the cost of a local, in-memory sort.
		cost = memo.Cost(rowCount*math.Log2(rowCount)) * cpuCostFactor
	}
	// When a certain amount of memory is used, distsql switches to a disk-based
	// sort with a temp RocksDB store. Add an IO cost, proportional to the number
	// of columns we are sorting.
	cost += memo.Cost(rowCount * float64(logical.Relational.OutputCols.Len()) * seqIOCostFactor)
...
func (c *coster) computeScanCost(candidate *memo.BestExpr, logical *props.Logical) memo.Cost {
	...
	return rowCount * c.rowScanCost(def.Table, def.Index, def.Cols.Len())
}

// rowScanCost is the cost to scan one row, which depends on the number of
...
func (c *coster) rowScanCost(table opt.TableID, index int, numScannedCols int) memo.Cost {
	...
	return memo.Cost(numCols)*seqIOCostFactor + memo.Cost(numScannedCols)*cpuCostFactor
}
...
func (c *coster) computeLookupJoinCost(candidate *memo.BestExpr, logical *props.Logical) memo.Cost {
	...
	perRowCost := randIOCostFactor + c.rowScanCost(def.Table, def.Index, def.LookupCols.Len())
