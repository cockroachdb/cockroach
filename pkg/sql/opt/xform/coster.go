...
func (c *coster) computeHashJoinCost(candidate *memo.BestExpr, logical *props.Logical) memo.Cost {
	...
	//
	// The right side is stored in a hashtable, so we add a factor for the
	// overhead of creating the table. This ensures that a join with the smaller
	// right side is preferred to the symmetric join.
	//
	...
	cost := memo.Cost(leftRowCount+2*rightRowCount) * cpuCostFactor
