...
const (
	...
	// that may be remotely located in a different locality. If latencies are
	// higher, then overall cluster throughput will suffer somewhat, as there will
	// be more queries in memory blocking on I/O. The impact on throughput is
	// expected to be relatively low, so latencyCostFactor is set to a small
...
// it do not match either. For example:
//
...
func (c *coster) rowScanCost(tabID opt.TableID, idxOrd int, numScannedCols int) memo.Cost {
	...
	if len(c.locality.Tiers) != 0 {
		...
		// additional cost. Anything in between scales linearly with the number of
		// matches.
