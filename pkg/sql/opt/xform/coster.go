...
func lookupExprCost(join memo.RelExpr) memo.Cost {
	...
	if ok {
		// 1.1 is a fudge factor to that pushes some plans over the knifes edge when
		// choosing between a partial index vs full index plus lookup expr in the
		// regional_by_row.   TODO(treilly) do some empirical analysis and determine
		// a better way to model this cost.
