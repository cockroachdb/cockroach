...
func TestJoinOrderBuilder_CalcTES(t *testing.T) {
	...
	for i, tc := range testCases {
		t.Run(fmt.Sprintf("Test case %d", i), func(t *testing.T) {
			...
			if len(tc.leftChildEdges) > 0 {
				leftEdges.AddRange(0, len(tc.leftChildEdges)-1)
			}
			if len(tc.rightChildEdges) > 0 {
				rightEdges.AddRange(0, len(tc.leftChildEdges)+len(tc.rightChildEdges)-1)
			}
