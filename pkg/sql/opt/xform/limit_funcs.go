...
func (c *CustomFuncs) GenerateLimitedTopKScans(
	...
	iter.ForEach(func(index cat.Index, filters memo.FiltersExpr, indexCols opt.ColSet, isCovering bool, constProj memo.ProjectionsExpr) {
		...
		for i, cnt := 0, index.KeyColumnCount(); i < cnt; i++ {
			colID := sp.Table.ColumnID(index.Column(i).Ordinal())
			if !tp.Ordering.Group(i).Contains(colID) {
				break
			}
			newPrivate.PartialOrdering.AppendCol(colID, tp.Ordering.Columns[i].Descending)
		}
...
func getPrefixFromOrdering(
	o opt.Ordering, intraOrd props.OrderingChoice, input memo.RelExpr, f func(id opt.ColumnID) bool,
) (props.OrderingChoice, bool) {
	var newOrd props.OrderingChoice
	// We are looking for a prefix of o that satisfies part of the required ordering
	oIdx, intraIdx := 0, 0
	for ; oIdx < len(o); oIdx++ {
		oCol := o[oIdx].ID()
		if intraOrd.Optional.Contains(oCol) || f(oCol) {
			// Optional column.
			continue
		}
...
func (c *CustomFuncs) GeneratePartialOrderTopK(
	...
	for _, ord := range orders {
		newOrd, found := getPrefixFromOrdering(ord.ToOrdering(), intraOrd, input, func(id opt.ColumnID) bool {
			return false
		})
		if !found {
			continue
