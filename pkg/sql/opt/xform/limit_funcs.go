...
// MaybeUnwrapSelectCanonicalScan matches an optional Select operator wrapping
// a canonical, non-locking Scan operator. If the match is successful, it
// returns the scan private, filters (if there was a Select) and ok=true.
func (c *CustomFuncs) MaybeUnwrapSelectCanonicalScan(
	expr memo.RelExpr,
) (sp *memo.ScanPrivate, filters memo.FiltersExpr, ok bool) {
	if selectExpr, isSelect := expr.(*memo.SelectExpr); isSelect {
		filters = selectExpr.Filters
		expr = selectExpr.Input
	}
	scanExpr, isScan := expr.(*memo.ScanExpr)
	if !isScan || !scanExpr.IsCanonical() || scanExpr.IsLocking() {
...
//  2. An index-join to retrieve the vector column and any other needed columns.
//  3. A projection of the distance between the vector column and query vector.
//  3. A top-k operator to perform re-ranking.
//
// Note that TryGenerateVectorSearch does not handle filters.
