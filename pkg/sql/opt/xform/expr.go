...
func (ev ExprView) ChildCount() int {
	if ev.best == normBestOrdinal {
		return ev.mem.lookupExpr(MakeNormExprID(ev.group)).childCount()
	}
...
func (ev ExprView) ChildGroup(nth int) opt.GroupID {
	if ev.best == normBestOrdinal {
		return ev.mem.lookupExpr(MakeNormExprID(ev.group)).childGroup(ev.mem, nth)
	}
...
func (ev ExprView) Private() interface{} {
	var eid exprID
	if ev.best == normBestOrdinal {
		eid = MakeNormExprID(ev.group)
	} else {
		eid = ev.lookupBestExpr().eid
	}
	return ev.mem.lookupExpr(eid).private(ev.mem)
}
