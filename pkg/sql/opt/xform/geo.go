...
func getSpanExprForGeographyIndex(
	...
	case geoindex.DWithin:
		...
		if !ok {
			panic(errors.AssertionFailedf("parameter is not float"))
		}
		...
		if len(additionalParams) == 2 {
			...
			if !ok {
				panic(errors.AssertionFailedf("parameter is not bool"))
			}
			if !*b {
...
func NewGeoDatumToInvertedExpr(
	...
	// TODO: extract the additional constant parameters for DWithin and DFullyWithin.
	// Are they the FuncExpr.Exprs[2] and FuncExpr.Exprs[3]
...
func (g *geoDatumToInvertedExpr) Convert(
	...
	// TODO: pass those additional params
	var fakeParams []tree.Datum
	if g.relationship == geoindex.DWithin || g.relationship == geoindex.DFullyWithin {
		distance := tree.DFloat(0.1)
		fakeParams = []tree.Datum{&distance}
	}
	spanExpr := g.getSpanExpr(ctx, d.Datum, fakeParams, g.relationship, g.indexConfig)
