...
// combination with a new expression which wasn't present during the last pass.
// Only combinations which consist solely of old expressions can be skipped.
//
...
func (e *explorer) generateIndexScans(scan memo.ExprID) {
	...
	for i := 1; i < tab.IndexCount(); i++ {
		...
		// Ensure that the index includes the set of needed columns (def.Cols).
		found := true
		for colID, ok := def.Cols.Next(0); ok; colID, ok = def.Cols.Next(colID + 1) {
			ord := e.mem.Metadata().ColumnOrdinal(def.Table, opt.ColumnID(colID))
			if !indexHasColumn(index, ord) {
				found = false
				break
			}
		}
...
type exploreState struct {
	// start and end specify which expressions need to be explored in the
	// current pass. Expressions before start have been partly explored during
	// previous passes. Expressions after end are new expression added during
	// the current pass.
...
// indexHasColumn returns true if the given index contains a column with the
// given ordinal position in its table.
func indexHasColumn(index opt.Index, ord int) bool {
	for indexCol := 0; indexCol < index.ColumnCount(); indexCol++ {
		if index.Column(indexCol).Ordinal == ord {
			return true
		}
	}
	return false
