...
// aggregate expressions (aggs) from a ScanExpr (input) into a
...
func (c *CustomFuncs) MakeMinMaxScalarSubqueries(
	...
	newScanExpr, inputSuccessfullyCopied := c.TryConstructScan(input)
	if !inputSuccessfullyCopied {
		return
	}
	...
	tuples := make(memo.ScalarListExpr, numRows)

	// Build the tuple
	for colIdx := range aggs {
		dataTypes = append(dataTypes, aggs[colIdx].DataType())
		newCols = append(newCols, aggs[colIdx].Col)
	}
	tupleTyp := types.MakeTuple(dataTypes)
	tuples[0] = factory.ConstructTuple(valuesEntries, tupleTyp)

	var newPrivate memo.ValuesPrivate
	newPrivate = memo.ValuesPrivate{
		Cols: newCols,
...
// TryConstructScan constructs a duplicate of "in" if "in" is a ScanExpr,
// otherwise returns nil, false.
func (c *CustomFuncs) TryConstructScan(in memo.RelExpr) (_ memo.RelExpr, ok bool) {

	switch t := in.(type) {
	case *memo.ScanExpr:
		private := &t.ScanPrivate
		return c.e.f.ConstructScan(private), true
	}
	return nil, false
}
