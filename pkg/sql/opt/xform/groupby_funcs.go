...
// RemapColID uses the baseColID, found from a call to NextColIDInMeta
// prior to calling DuplicateScanPrivate, to remap origColID.
func (c *CustomFuncs) RemapColID(
	sp *memo.ScanPrivate, origColID opt.ColumnID, baseColID opt.ColumnID,
) (opt.ColumnID, bool) {
	table := sp.Table
	md := c.e.mem.Metadata()
	...
	for i, n := 0, tab.ColumnCount(); i < n; i++ {
		if table.ColumnID(i) == origColID {
			return baseColID + opt.ColumnID(i), true
		}
	}
	return -1, false
}

// NextColIDInMeta finds the next column ID to be allocated in Metadata
func (c *CustomFuncs) NextColIDInMeta() opt.ColumnID {
	md := c.e.mem.Metadata()
	return opt.ColumnID(md.NumColumns() + 1)
}

// MakeMinMaxScalarSubqueries transforms a list of MIN and MAX
// aggregate expressions (aggs) from a ScanExpr (input) into
// multiple scalar subqueries, with one MIN or MAX expression
// per subquery.
func (c *CustomFuncs) MakeMinMaxScalarSubqueries(
	grp memo.RelExpr, scanPrivate *memo.ScanPrivate, aggs memo.AggregationsExpr,
) {
	valuesEntries := make(memo.ScalarListExpr, len(aggs))
	numCols := len(aggs)
	dataTypes := make([]*types.T, 0, numCols)
	newCols := make(opt.ColList, 0, numCols)

	factory := c.e.f
	for i := range aggs {
		baseColID := c.NextColIDInMeta()

		// Pushing scalar MIN/MAX into joins is not currently done,
		// so only build this transformation for simple ScanExprs.
		newScanPrivate := c.DuplicateScanPrivate(scanPrivate)
		newScanExpr := c.e.f.ConstructScan(newScanPrivate)

		var newAggrItem = aggs[i]
		// Variable expressions must have their ColIDs remapped,
		// which requires building a new AggregationsItem.
		if variable, ok := aggs[i].Agg.Child(0).(*memo.VariableExpr); ok {
			if varColID, ok2 := c.RemapColID(scanPrivate, variable.Col, baseColID); ok2 {
				newVarExpr := factory.ConstructVariable(varColID)
				var newAggrFunc opt.ScalarExpr
				switch aggs[i].Agg.(type) {
				case *memo.MaxExpr:
					newAggrFunc = factory.ConstructMax(newVarExpr)
				case *memo.MinExpr:
					newAggrFunc = factory.ConstructMin(newVarExpr)
				default:
					return // Should never happen, but skip this rewrite if it does
				}
				columnMeta := c.e.mem.Metadata().ColumnMeta(aggs[i].Col)
				newAggrItem =
					factory.ConstructAggregationsItem(
						newAggrFunc,
						c.e.mem.Metadata().AddColumn(columnMeta.Alias, columnMeta.Type),
					)
			} else {
				// Could not map the column.  Bail out of this rewrite.
				return
			}
		}

		valuesEntries[i] =
			factory.ConstructSubquery(
				factory.ConstructScalarGroupBy(
					newScanExpr,
		...
		// The VALUES clause inherits the target column ID
		// and datatype from the scalar group by, so the
		// outputs are identical.
		dataTypes = append(dataTypes, aggs[i].DataType())
		newCols = append(newCols, aggs[i].Col)
	}

	tupleTyp := types.MakeTuple(dataTypes)
	tuples := memo.ScalarListExpr{factory.ConstructTuple(valuesEntries, tupleTyp)}

	var newPrivate = memo.ValuesPrivate{
		Cols: newCols,
		ID:   factory.Metadata().NextUniqueID(),
	}
...
// TwoOrMoreMinOrMax returns true if the aggregations (aggs)
// consists of two or more MIN or MAX expressions and
// no other type of aggregate function.
