	...
	for i := range filters {
		...
		for j, n := 0, props.Constraints.Length(); j < n; j++ {
			...
			for k := 0; k < cons.Spans.Count(); k++ {
				...
				initialConjunction, ok := c.buildConjunctionFromConstraint(cons, span, tab, scanPrivate, constFilterCols)
				if !ok {
					continue
				}
				var newOp opt.ScalarExpr
				newOp, filterAdded = c.buildCombinedConjunction(initialConjunction, tabMeta, constFilterCols, cons, scanPrivate)
...
func (c *CustomFuncs) buildConjunctionFromConstraint(
...
) (conjunction opt.ScalarExpr, ok bool) {
	for m := 0; m < cons.Columns.Count(); m++ {
		...
		if d, isDatum := originalConstVal.(tree.Datum); isDatum {
			if d == tree.DNull {
				return nil, false
			}
		}
	...
	return conjunction, conjunction != nil
}

func (c *CustomFuncs) buildCombinedConjunction(
