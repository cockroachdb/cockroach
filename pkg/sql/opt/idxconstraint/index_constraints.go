...
func (c *indexConstraintCtx) makeSpansForOr(
	...
	or := e.(*memo.OrExpr)
	var disjunctions = memo.CollectContiguousOrExprs(or)
...
//
// nestedUnderOrExpr indicates if we are processing an OrExpr which is the child
// of another OrExpr.
func (c *indexConstraintCtx) simplifyFilter(
	...
	case *memo.AndExpr:
		left := c.simplifyFilter(t.Left, final, maxSimplifyPrefix, false)
		right := c.simplifyFilter(t.Right, final, maxSimplifyPrefix, false)
		return c.factory.ConstructAnd(left, right)

	case *memo.RangeExpr:
		return c.factory.ConstructRange(c.simplifyFilter(t.And, final, maxSimplifyPrefix, false))
	}
	...
	case *memo.OrExpr:
		...
		left := c.simplifyFilter(t.Left, final, maxSimplifyPrefix, leftIsOrExpr)
		right := c.simplifyFilter(t.Right, final, maxSimplifyPrefix, rightIsOrExpr)
...
func (ic *Instance) RemainingFilters() memo.FiltersExpr {
	...
	for i := range ic.requiredFilters {
		...
		condition := ic.simplifyFilter(ic.requiredFilters[i].Condition, &ic.constraint, prefix, false)
