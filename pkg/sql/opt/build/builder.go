...
// to the AST of the original SQL statement with a few simple transformations.
// These transformations include heuristics such as predicate push-down that
// should always be applied.  They do not include more complex transformations
// whose benefit must be evaluated with the optimizer's cost model (e.g., join
// reordering).
//
...
// If any subroutines panic as part of the build process, the panic is caught
// here and returned as an error.
...
func (b *Builder) buildTable(
	...
	default:
		unimplemented("table expr: %T", texpr)
	}
...
func (b *Builder) buildScalar(scalar tree.TypedExpr, inScope *scope) xform.GroupID {
	...
	case *tree.ComparisonExpr:
		...
		if comparisonOpMap[t.Operator] == nil {
			unimplemented("operator %s", t.Operator.String())
		}
...
func (b *Builder) IndexedVarEval(idx int, ctx *tree.EvalContext) (tree.Datum, error) {
	unimplemented("Builder.IndexedVarEval")
	return nil, fmt.Errorf("unimplemented: Builder.IndexedVarEval")
}
...
func (b *Builder) IndexedVarNodeFormatter(idx int) tree.NodeFormatter {
	unimplemented("Builder.IndexedVarNodeFormatter")
	return nil
}
