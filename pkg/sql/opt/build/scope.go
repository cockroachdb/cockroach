...
type groupby struct {
	...
	// inGroupingContext is true when the groupingsScope is active. This is true
	// when the builder is building expressions in a SELECT list, and aggregates,
	// GROUP BY, or HAVING are present. This is also true when the builder is
	// building expressions inside the HAVING clause. When inGroupingContext is
	// true, varsUsed will be utilized to enforce scoping rules. See the comment
	// below for more details.
	inGroupingContext bool

	// varsUsed is only utilized when inGroupingContext is true.
