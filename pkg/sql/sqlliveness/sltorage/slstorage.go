...
SELECT expiration FROM system.sqlliveness WHERE session_id = $1`, sid,
	...
	exp, err := tree.DecimalToHLC(&row[0].(*tree.DDecimal).Decimal)
	if err != nil {
		log.ReportOrPanic(ctx, nil, "Cannot parse expiration for session id: %s", sid.String())
	...
	// TODO(spaskob): also delete row or have separate daemon of Storage that
	// deletes rows?
	return false, nil
}
