...
func TestSQLInstanceDeadlines(t *testing.T) {
	...
	clock := hlc.NewClock(hlc.NewManualClock(hlc.UnixNano()).UnixNano, time.Nanosecond /* maxOffset */)
	settings := cluster.MakeTestingClusterSettingsWithVersions(
		clusterversion.TestingBinaryVersion,
		clusterversion.TestingBinaryMinSupportedVersion,
		true /* initializeVersion */)
...
func TestSQLInstanceDeadlinesExtend(t *testing.T) {
	...
	mt := hlc.NewManualClock(hlc.UnixNano())
	clock := hlc.NewClock(mt.UnixNano, time.Nanosecond /* maxOffset */)
	settings := cluster.MakeTestingClusterSettingsWithVersions(
		clusterversion.TestingBinaryVersion,
		clusterversion.TestingBinaryMinSupportedVersion,
		true /* initializeVersion */)
	...
	mt.Increment(slinstance.DefaultTTL.Get(&settings.SV).Nanoseconds())
