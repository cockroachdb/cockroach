...
func BenchmarkHeartbeatThroughput(b *testing.B) {
	//b.Skip("not intended to run regularly")
	...
	for _, tc := range []struct {
		...
		b.Run(tc.name, func(b *testing.B) {
			...
			// Wait for all instances to have an active session.
			// Session() blocks until a session is created.
			var wg sync.WaitGroup
			for i := 0; i < numInstances; i++ {
				wg.Add(1)
				go func(inst *slinstance.Instance) {
					defer wg.Done()
					_, _ = inst.Session(ctx)
				}(instances[i])
			}
			wg.Wait()

			// Reset metrics and timer after initial session creation.
