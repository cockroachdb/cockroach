...
// maxFixups specifies the maximum number of unique index fixups that can be
...
type fixupProcessor struct {
	...
	mu struct {
		...
		// uniquePartitions de-duplicates fixups that operate on a partition.
		uniquePartitions map[partitionFixupKey]bool
...
func (fp *fixupProcessor) Init(index *VectorIndex, seed int64) {
	...
	fp.mu.uniquePartitions = make(map[partitionFixupKey]bool, maxFixups)
	fp.mu.fixups = make(chan fixup, maxFixups)
}
...
// run synchronously processes all pending fixups in the queue. In production,
// this should only be called on the background goroutine. However, tests can
// call it on a foreground goroutine, so long as the background goroutine is not
// running.
func (fp *fixupProcessor) run(ctx context.Context) error {
	// Get next fixup from the map, but don't delete it from the map until it has
	// been processed, so that we can detect any duplicates that come in during
	// processing.
	getNextFixup := func(processed fixup) fixup {
		fp.mu.Lock()
		defer fp.mu.Unlock()

		// Delete already-processed fixup from the map.
		switch processed.Type {
		case splitFixup:
			key := partitionFixupKey{Type: processed.Type, PartitionKey: processed.PartitionKey}
			delete(fp.mu.uniquePartitions, key)
		}

		// Get next fixup from the queue.
		select {
		case key := <-fp.mu.fixups:
			return key
		default:
			// No more available fixups.
			return fixup{Type: invalidFixup}
		}
	}

	// Accumulate any errors that occur during processing.
	var errs []error
	var next fixup
	for {
		next = getNextFixup(next)
		if next.Type == invalidFixup {
			// No more fixups to run. Wrap any errors and return them.
			return errors.Join(errs...)
		}

		// Invoke the fixup function.
		var err error
		switch next.Type {
		case splitFixup:
			err = fp.splitPartition(ctx, next.ParentPartitionKey, next.PartitionKey)
			if err != nil {
...
// addFixup enqueues the given fixup for later processing.
func (fp *fixupProcessor) addFixup(ctx context.Context, fixup fixup) {
	fp.mu.Lock()
	defer fp.mu.Unlock()

	// Check whether fixup limit has been reached.
	if len(fp.mu.fixups) >= maxFixups {
		// Don't enqueue the fixup.
		log.Warningf(ctx, "reached limit of unprocessed fixups")
		return
	}
	...
	case splitFixup:
		...
		if _, ok := fp.mu.uniquePartitions[key]; ok {
			return
		}
		fp.mu.uniquePartitions[key] = true
	}

	// Note that the channel send operation should never fail, since it has
	// maxFixups capacity.
	fp.mu.fixups <- fixup
}
...
func (fp *fixupProcessor) splitPartition(
	...
	defer func() {
		if err == nil {
			...
			err = fp.index.store.AbortTransaction(ctx, txn)
		}
	}()
	...
	if errors.Is(err, vecstore.ErrPartitionNotFound) {
		log.Infof(ctx, "partition %d no longer exists, do not split", partitionKey)
		return nil
	} else if err != nil {
	...
	if partitionKey != vecstore.RootKey {
		...
		if errors.Is(err, vecstore.ErrPartitionNotFound) {
			log.Infof(ctx, "parent partition %d no longer exists, do not split", parentPartitionKey)
			return nil
		} else if err != nil {
		...
		if parentPartition.Find(vecstore.ChildKey{PartitionKey: partitionKey}) == -1 {
			log.Infof(ctx, "partition %d is no longer child of partition %d, do not split",
	...
	// Determine how the partition should be split.
	...
	// Insert the new partitions into the index.
	...
	if partitionKey == vecstore.RootKey {
		// Replace existing root partition with a partition having the two new
		// partitions as its children.
		...
		// Replace partition with left and right splits.
