...
func (tx *memTxn) GetPartition(
	...
	if err != nil {
		if err == cspann.ErrPartitionNotFound && partitionKey == cspann.RootKey {
...
func (tx *memTxn) DeletePartition(
	...
	if err != nil {
		...
		if err == cspann.ErrPartitionNotFound || err == cspann.ErrRestartOperation {
			return nil
		}
		return err
	}
...
func (tx *memTxn) AddToPartition(
	...
	if err != nil {
		if err == cspann.ErrPartitionNotFound && partitionKey == cspann.RootKey {
			...
			memPart = tx.ensureRootPartition(treeKey)
...
// ensureRootPartition lazily creates a root partition for the specified K-means
// tree.
func (tx *memTxn) ensureRootPartition(treeKey cspann.TreeKey) *memPartition {
	...
	memPart, ok := tx.store.getPartitionLocked(treeKey, cspann.RootKey)
	if !ok {
