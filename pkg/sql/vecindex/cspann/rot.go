...
// vector elements. The rotation mixes the elements at leftOffset and
// rightOffset using the provided cosine and sine values, corresponding to
// a rotation by some angle θ in the (leftOffset, rightOffset) plane.
//
...
type givensRotation struct {
	// leftOffset is the index of the first element to rotate.
	leftOffset int
	// rightOffset is the index of the second element to rotate.
	rightOffset int
	...
	*t = randomOrthoTransformer{
	...
	case RotGivens:
		...
		for rot := range numRotations {
			leftOffset := rng.Intn(dims)
			rightOffset := rng.Intn(dims - 1)
			...
			t.rotations[rot] = givensRotation{
				leftOffset: leftOffset, rightOffset: rightOffset, cos: cos, sin: sin}
		}
	}
}
...
func (t *randomOrthoTransformer) RandomizeVector(original vector.T, randomized vector.T) vector.T {
	...
	case RotGivens:
		...
		for i := range t.rotations {
			rot := &t.rotations[i]
			leftVal := randomized[rot.leftOffset]
			rightVal := randomized[rot.rightOffset]
			randomized[rot.leftOffset] = rot.cos*leftVal + rot.sin*rightVal
			randomized[rot.rightOffset] = -rot.sin*leftVal + rot.cos*rightVal
		}
	}
...
func (t *randomOrthoTransformer) UnRandomizeVector(randomized vector.T, original vector.T) vector.T {
	switch t.algo {
	case RotNone:
	...
	case RotGivens:
		...
		for i := len(t.rotations) - 1; i >= 0; i-- {
			rot := &t.rotations[i]
			leftVal := original[rot.leftOffset]
			rightVal := original[rot.rightOffset]
			original[rot.leftOffset] = rot.cos*leftVal - rot.sin*rightVal
			original[rot.rightOffset] = rot.sin*leftVal + rot.cos*rightVal
		}
	}
