...
func (km *BalancedKmeans) AssignPartitions(
	...
	// Compute the mean centroid norms that are needed to calculate the spherical
	// centroids.
	...
	for i := range count {
		...
		if spherical {
			// Compute inner product distance to spherical centroids. This also
			// works for Cosine distance, since input vectors are expected to be
			// normalized.
