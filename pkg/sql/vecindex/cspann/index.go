...
// IndexContext contains per-thread state needed during index operations. Public
// fields in the context are set by the caller. Private fields are used by the
// index.
//
...
type IndexContext struct {
	...
	randomized vector.T
	// txCtx caches context that's passed to store Txn methods.
	txCtx TxnContext
...
func (vi *Index) Insert(
	ctx context.Context, idxCtx *IndexContext, vec vector.T, key KeyBytes,
) error {
...
func (vi *Index) Delete(
	...
	_, err = vi.removeFromPartition(
		ctx, idxCtx.Txn, &idxCtx.txCtx, result.ParentPartitionKey, result.ChildKey)
	return err
}
...
func (vi *Index) SearchForInsert(
	ctx context.Context, idxCtx *IndexContext, vec vector.T,
) (*SearchResult, error) {
	...
	metadata, err := idxCtx.Txn.GetPartitionMetadata(
		ctx, &idxCtx.txCtx, partitionKey, true /* forUpdate */)
	if err != nil {
		return nil, err
	}
...
func (vi *Index) setupInsertContext(idxCtx *IndexContext, vec vector.T) {
...
func (vi *Index) insertHelper(
	ctx context.Context, idxCtx *IndexContext, childKey ChildKey, valueBytes ValueBytes,
) error {
	result, err := vi.searchForInsertHelper(ctx, idxCtx)
	if err != nil {
		return err
	...
	partitionKey := result.ChildKey.PartitionKey
	err = vi.addToPartition(ctx, idxCtx.Txn, &idxCtx.txCtx, parentPartitionKey,
...
func (vi *Index) searchForInsertHelper(
	ctx context.Context, idxCtx *IndexContext,
) (*SearchResult, error) {
...
func (vi *Index) addToPartition(
	...
	metadata, err := txn.AddToPartition(ctx, txCtx, partitionKey, vec, childKey, valueBytes)
	if err != nil {
...
func (vi *Index) removeFromPartition(
	...
	metadata, err = txn.RemoveFromPartition(ctx, txCtx, partitionKey, childKey)
	if err != nil {
...
func (vi *Index) searchHelper(ctx context.Context, idxCtx *IndexContext, searchSet *SearchSet) error {
...
func (vi *Index) searchChildPartitions(
	ctx context.Context, idxCtx *IndexContext, searchSet *SearchSet, parentResults SearchResults,
	...
	level, err = idxCtx.Txn.SearchPartitions(
		ctx, &idxCtx.txCtx, idxCtx.tempKeys, idxCtx.randomized, searchSet, idxCtx.tempCounts)
	if err != nil {
		return 0, err
	}
...
func (vi *Index) rerankSearchResults(
	ctx context.Context, idxCtx *IndexContext, candidates []SearchResult,
) ([]SearchResult, error) {
	if len(candidates) == 0 {
...
func (vi *Index) getRerankVectors(
	...
	err := idxCtx.Txn.GetFullVectors(ctx, &idxCtx.txCtx, idxCtx.tempVectorsWithKeys)
	if err != nil {
		return nil, err
	}
...
func (vi *Index) Format(
	ctx context.Context, idxCtx *IndexContext, options FormatOptions,
	...
	helper = func(partitionKey PartitionKey, parentPrefix string, childPrefix string) error {
		partition, err := idxCtx.Txn.GetPartition(ctx, &idxCtx.txCtx, partitionKey)
		if err != nil {
			return err
		}
		...
		for i, childKey := range partition.ChildKeys() {
			...
			if partition.Level() == LeafLevel {
				...
				if err = idxCtx.Txn.GetFullVectors(ctx, &idxCtx.txCtx, refs); err != nil {
					return err
				}
				buf.WriteString(parentPrefix)
				buf.WriteString("• ")
