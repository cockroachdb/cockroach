...
type Context struct {
	...
	// that is the last in its partition. This is used to avoid moving the last
	// vector to another partition, thereby creating an empty non-leaf partition,
	// which is not allowed in a balanced K-means tree.
...
func (vi *Index) ForceSplitOrMerge(
	...
	vi.fixups.AddSplitOrMerge(ctx, treeKey, parentPartitionKey, partitionKey)
}
