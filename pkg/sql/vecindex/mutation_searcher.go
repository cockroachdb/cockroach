...
func (s *MutationSearcher) SearchForInsert(
	...
	s.partitionKey = res.ChildKey.PartitionKey
	centroid := res.Vector
	randomizedVec := s.idxCtx.RandomizedVector()

	// Quantize and encode the input vector with respect to the insert partition's
	// centroid.
	quantizedVec, err := s.txn.QuantizeAndEncode(s.partitionKey, centroid, randomizedVec)
	if err != nil {
		return err
	}
...
func (s *MutationSearcher) SearchForDelete(
	...
	if res != nil {
		s.partitionKey = res.ParentPartitionKey
	} else {
		s.partitionKey = cspann.InvalidKey
	}
...
// PartitionKey returns the partition where an insertion or deletion will occur.
func (s *MutationSearcher) PartitionKey() cspann.PartitionKey {
