...
func TestEncodeKeys(t *testing.T) {
	...
	// EncodeMetadataKey.
	encoded := vecencoding.EncodeMetadataKey(input, input, 10)
	require.Equal(t, roachpb.Key{1, 2, 3, 1, 2, 3, 146, 136}, encoded)

	// EncodeStartVectorKey.
	require.Equal(t, roachpb.Key{1, 2, 4}, vecencoding.EncodeStartVectorKey(input))

	// EncodeEndVectorKey.
	require.Equal(t, roachpb.Key{1, 3}, vecencoding.EncodeEndVectorKey(input))

	// EncodePrefixVectorKey and EncodedPrefixVectorKeyLen.
	require.Equal(t, roachpb.Key{1, 2, 138}, vecencoding.EncodePrefixVectorKey(input, cspann.SecondLevel))
