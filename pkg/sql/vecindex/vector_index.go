...
func (vi *VectorIndex) searchChildPartitions(
	searchCtx *searchContext, searchSet *vecstore.SearchSet, toSearch vecstore.SearchResults,
) (level vecstore.Level, err error) {
	searchCtx.tempKeys = ensureSliceLen(searchCtx.tempKeys, len(toSearch))
	for i := range toSearch {
		searchCtx.tempKeys[i] = toSearch[i].ChildKey.PartitionKey
	}

	searchCtx.tempCounts = ensureSliceLen(searchCtx.tempCounts, len(toSearch))
	...
	for i := range toSearch {
		...
		partitionKey := toSearch[i].ChildKey.PartitionKey
		if count < vi.options.MinPartitionSize && partitionKey != vecstore.RootKey {
			vi.fixups.AddMerge(
				searchCtx.Ctx, toSearch[i].ParentPartitionKey, partitionKey)
