...
// VectorIndexOptions specifies options for building and searching the vector
// index.
...
// SearchOptions specifies options for searching the vector index.
type SearchOptions struct {
	// BaseBeamSize is the default number of child partitions that will be
	// searched at each level. Adaptive search will automatically decrease or
	// increase this value as the search proceeds.
	BaseBeamSize int
...
func (vi *VectorIndex) CreateRoot(ctx context.Context, txn vecstore.Txn) error {
	...
	dims := vi.quantizer.GetRandomDims()
	rootQuantizer := quantize.NewUnQuantizer(dims)
	vectors := vector.MakeSet(dims)
	rootQuantizedSet := rootQuantizer.Quantize(ctx, &vectors)
	rootPartition := vecstore.NewPartition(
		rootQuantizer, rootQuantizedSet, []vecstore.ChildKey{}, vecstore.LeafLevel)
...
func (vi *VectorIndex) searchHelper(
	...
	for {
		...
		if searchLevel > vecstore.LeafLevel {
			// Compute the z-score of the candidate list.
...
func (vi *VectorIndex) searchChildPartitions(
	...
	for i := 0; i < len(searchCtx.tempCounts); i++ {
		...
		if count < vi.options.MinPartitionSize {
			// TODO(andyk): Enqueue a merge of the partition.
		} else if count > vi.options.MaxPartitionSize {
			// TODO(andyk): Enqueue a split of the partition.
		}
	}
...
func (vi *VectorIndex) rerankSearchResults(
	...
	if searchCtx.Level == vecstore.LeafLevel {
		...
		// vector if available, or un-randomize the randomized vector.
