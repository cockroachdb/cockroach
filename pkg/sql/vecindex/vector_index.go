...
func (vi *VectorIndex) searchChildPartitions(
	searchCtx *searchContext, searchSet *vecstore.SearchSet, results vecstore.SearchResults,
) (level vecstore.Level, err error) {
	searchCtx.tempKeys = ensureSliceLen(searchCtx.tempKeys, len(results))
	for i := range results {
		searchCtx.tempKeys[i] = results[i].ChildKey.PartitionKey
	}

	searchCtx.tempCounts = ensureSliceLen(searchCtx.tempCounts, len(results))
	...
	for i := range results {
		...
		partitionKey := results[i].ChildKey.PartitionKey
		if count < vi.options.MinPartitionSize && partitionKey != vecstore.RootKey {
			vi.fixups.AddMerge(
				searchCtx.Ctx, results[i].ParentPartitionKey, partitionKey)
