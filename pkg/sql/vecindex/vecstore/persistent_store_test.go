...
func TestPersistentStore(t *testing.T) {
	...
	t.Run("insert a root partition into the store and read it back", func(t *testing.T) {
		...
		require.Equal(t, Level(2), readRoot.Level())
		require.Equal(t, []ChildKey{childKey2}, readRoot.ChildKeys())
		...
		require.NoError(t, err)
		require.Equal(t, Level(2), readRoot.Level())
		require.Equal(t, []ChildKey{childKey10, childKey20}, readRoot.ChildKeys())
		...
		readRoot, err = txn.GetPartition(ctx, RootKey)
		require.NoError(t, err)
		require.Equal(t, LeafLevel, readRoot.Level())
		require.Equal(t, []ChildKey{primaryKey200, primaryKey300, primaryKey400}, readRoot.ChildKeys())
	})
	...
	t.Run("insert a partition and then delete it", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.Equal(t, Level(2), newPartition.Level())
		require.Equal(t, []ChildKey{childKey2}, newPartition.ChildKeys())
