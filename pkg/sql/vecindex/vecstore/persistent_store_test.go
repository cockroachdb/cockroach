...
func TestPersistentStore(t *testing.T) {
	...
	keycols := tableDesc.GetPrimaryIndex().CollectPrimaryStoredColumnIDs()
	var col catalog.Column
	var err error
	for colID, more := keycols.Next(0); more; colID, more = keycols.Next(colID) {
	...
	indexDesc := descpb.IndexDescriptor{
		...
		StoreColumnIDs:      []descpb.ColumnID{tableDesc.GetPrimaryIndex().GetKeyColumnID(0)},
		StoreColumnNames:    []string{tableDesc.GetPrimaryIndex().GetKeyColumnName(0)},
	...
	store := NewPersistentStore(kvDB, quantizer, codec, tableDesc, index)
	pk1 := encoding.EncodeVarintAscending(encoding.EncodeUvarintAscending([]byte{}, 11), 0)
	pk2 := encoding.EncodeVarintAscending(encoding.EncodeVarintAscending([]byte{}, 12), 0)
