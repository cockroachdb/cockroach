...
func (k ChildKey) Equal(other ChildKey) bool {
	return k.PartitionKey == other.PartitionKey && bytes.Equal(k.PrimaryBytes, other.PrimaryBytes)
}
...
func (k ChildKey) Compare(other ChildKey) int {
	if k.PrimaryBytes != nil {
		return bytes.Compare(k.PrimaryBytes, other.PrimaryBytes)
	}
