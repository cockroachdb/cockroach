...
type InMemoryStore struct {
	raBitQuantizer quantize.Quantizer
	unquantizer    quantize.Quantizer
	seed           int64
...
func NewInMemoryStore(dims int, seed int64) *InMemoryStore {
	st := &InMemoryStore{
		raBitQuantizer: quantize.NewRaBitQuantizer(dims, seed),
		unquantizer:    quantize.NewUnQuantizer(dims),
		seed:           seed,
	}
...
func (s *InMemoryStore) GetFullVectors(ctx context.Context, txn Txn, refs []VectorWithKey) error {
	...
	for i := 0; i < len(refs); i++ {
		...
		if ref.Key.PartitionKey != InvalidKey {
			...
			vector, ok := s.mu.vectors[string(refs[i].Key.ForeignKey)]
			if ok {
				ref.Vector = vector
			} else {
				ref.Vector = nil
...
// InsertVector inserts a new full-size vector into the store, associated with
// the given foreign key.
func (s *InMemoryStore) InsertVector(txn Txn, foreignKey ForeignKey, vector vector.T) {
	s.acquireTxnLock(txn, dataLock, true /* updating */)

	s.mu.Lock()
	defer s.mu.Unlock()

	s.mu.vectors[string(foreignKey)] = vector
}

// DeleteVector deletes the vector associated with the given foreign key from
// the in-memory store.
func (s *InMemoryStore) DeleteVector(txn Txn, foreignKey ForeignKey) {
	s.acquireTxnLock(txn, dataLock, true /* updating */)

	s.mu.Lock()
	defer s.mu.Unlock()

	delete(s.mu.vectors, string(foreignKey))
}
...
func (s *InMemoryStore) MarshalBinary() (data []byte, err error) {
	...
	storeProto := StoreProto{
		Dims:       s.unquantizer.GetOriginalDims(),
		Seed:       s.seed,
	...
	for foreignKey, vector := range s.mu.vectors {
		vectorWithKey := VectorProto{ForeignKey: []byte(foreignKey), Vector: vector}
		storeProto.Vectors = append(storeProto.Vectors, vectorWithKey)
	}

	// Serialize the protobuf as bytes.
	return proto.Marshal(&storeProto)
}
...
func (s *InMemoryStore) UnmarshalBinary(data []byte) error {
	...
	if err := proto.Unmarshal(data, &storeProto); err != nil {
		return err
	}

	// Construct the InMemoryStore object.
	s.raBitQuantizer = quantize.NewRaBitQuantizer(storeProto.Dims, storeProto.Seed)
	s.unquantizer = quantize.NewUnQuantizer(storeProto.Dims)
	...
	for i := range storeProto.Partitions {
		...
		if partitionProto.RaBitQ != nil {
			partition.quantizer = s.raBitQuantizer
			partition.quantizedSet = partitionProto.RaBitQ
		} else {
			partition.quantizer = s.unquantizer
	...
	for i := range storeProto.Vectors {
		...
		s.mu.vectors[string(vectorProto.ForeignKey)] = vectorProto.Vector
	}
