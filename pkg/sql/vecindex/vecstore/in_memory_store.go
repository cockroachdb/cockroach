...
func (s *InMemoryStore) InsertVector(key PrimaryBytes, vector vector.T) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *InMemoryStore) DeleteVector(key PrimaryBytes) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
// is used for testing.
func (s *InMemoryStore) GetVector(key PrimaryBytes) vector.T {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *InMemoryStore) GetAllVectors() []VectorWithKey {
	...
	for key, vec := range s.mu.vectors {
		refs = append(refs, VectorWithKey{Key: ChildKey{PrimaryBytes: PrimaryBytes(key)}, Vector: vec})
	}
	return refs
}
...
func (s *InMemoryStore) MarshalBinary() (data []byte, err error) {
	...
	for key, vector := range s.mu.vectors {
		vectorWithKey := VectorProto{PrimaryBytes: []byte(key), Vector: vector}
...
func LoadInMemoryStore(data []byte) (*InMemoryStore, error) {
	...
	for i := range storeProto.Vectors {
		...
		inMemStore.mu.vectors[string(vectorProto.PrimaryBytes)] = vectorProto.Vector
	}
