...
func TestInMemoryStore(t *testing.T) {
	...
	t.Run("insert and get all vectors", func(t *testing.T) {
		...
		require.Equal(t, []VectorWithKey{
			{Key: ChildKey{PrimaryBytes: testPKs[0]}, Vector: testVectors[0]},
			{Key: ChildKey{PrimaryBytes: testPKs[1]}, Vector: testVectors[1]},
		}, vectors)
	})
	...
	t.Run("delete full vector", func(t *testing.T) {
		...
		refs := []VectorWithKey{{Key: ChildKey{PrimaryBytes: PrimaryBytes{10}}}}
	...
	t.Run("abort transaction", func(t *testing.T) {
		...
		err = txn.GetFullVectors(ctx, []VectorWithKey{
			{Key: ChildKey{PrimaryBytes: testPKs[0]}},
		})
		require.NoError(t, err)
	})
}
