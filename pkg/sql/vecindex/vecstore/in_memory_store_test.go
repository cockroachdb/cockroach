...
func TestInMemoryStore(t *testing.T) {
	...
	t.Run("insert and get all vectors", func(t *testing.T) {
		...
		store.InsertVector([]byte{11}, testVectors[0])
		store.InsertVector([]byte{12}, testVectors[1])
		...
		require.Equal(t, []VectorWithKey{
			{Key: ChildKey{PrimaryKey: PrimaryKey{11}}, Vector: testVectors[0]},
			{Key: ChildKey{PrimaryKey: PrimaryKey{12}}, Vector: testVectors[1]},
		}, vectors)
	})
