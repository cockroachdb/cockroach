...
func (tx *Txn) decodePartition(
	...
	// case).
	metadata, err := tx.getMetadataFromKVResult(treeKey, partitionKey, metaResult)
	if err != nil {
		return nil, err
	}
...
func (tx *Txn) GetPartitionMetadata(
	...
	metadata, err := tx.getMetadataFromKVResult(treeKey, partitionKey, &b.Results[0])
	if err != nil {
...
//
// NOTE: As a side effect, getMetadataFromKVResult updates metaResult.Rows to
// skip past the metadata row, if it was present. This makes it easier for
// callers to determine where data rows start in the case where the same KV
// result can contain both metadata and data rows.
