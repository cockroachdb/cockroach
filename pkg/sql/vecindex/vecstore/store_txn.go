...
func (tx *storeTxn) GetPartition(
	...
	// TODO DURING REVIEW: this is grabbing an exclusive lock on all keys in the
	// partition, not just the metadata key. Is that intentional/desirable?
	// Mutations of the partition will all grab a lock (either shared or
	// exclusive) on the metadata key, so locking the child keys as well seems
	// unnecessary.
