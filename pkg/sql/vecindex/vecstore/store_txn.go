...
// vector set.
func (sc *storeCodec) encodeVectorFromSet(
...
// cspann.Store API.
func newTxn(
	w *veclib.Workspace, store *Store, kv *kv.Txn,
) *storeTxn {
	tx := storeTxn{
		workspace: w,
		kv:        kv,
...
// otherwise.
func (tx *storeTxn) getCodecForPartitionKey(
...
func (tx *storeTxn) decodePartition(codec *storeCodec, result *kv.Result) (*cspann.Partition, error) {
	if result.Err != nil {
