...
func InitGetFullVectorsFetchSpec(
	...
	if err := rowenc.InitIndexFetchSpec(&spec.FetchSpec, evalCtx.Codec, tableDesc, sourceIndex, []descpb.ColumnID{vectorColID}); err != nil {
		return err
	}
...
func (tx *Txn) getFullVectorsFromPK(
	...
	splitter := span.MakeSplitterWithFamilyIDs(len(tx.fullVecFetchSpec.FetchSpec.KeyAndSuffixColumns), tx.fullVecFetchSpec.FamilyIDs)
