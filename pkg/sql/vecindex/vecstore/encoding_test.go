				...
				t.Run(fmt.Sprintf("level=%d", level), func(t *testing.T) {
					...
					for i := range set.Count {
						...
						case *quantize.UnQuantizedVectorSet:
							appendTo, err = EncodeUnquantizedVector(
								appendTo, quantizedSet.GetCentroidDistances()[i], set.At(i),
							)
							require.NoError(t, err)
					...
					case *quantize.UnQuantizedVectorSet:
						...
						for i := range encVectors {
							...
							require.NoError(t, err)
							encChildKey := encVectors[i][GetUnquantizedVectorChildKeyOffset(set.Dims):]
