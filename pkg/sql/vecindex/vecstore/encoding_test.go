				...
				t.Run(fmt.Sprintf("level=%d", level), func(t *testing.T) {
					...
					for i := range set.Count {
						...
						case *quantize.UnQuantizedVectorSet:
							appendTo, err = EncodeFullVector(appendTo, quantizedSet.GetCentroidDistances()[i], set.At(i))
						case *quantize.RaBitQuantizedVectorSet:
							appendTo = EncodeRaBitQVector(appendTo,
								quantizedSet.CodeCounts[i], quantizedSet.CentroidDistances[i],
								quantizedSet.DotProducts[i], quantizedSet.Codes.At(i),
							)
						}
						encVectors[i] = EncodeChildKey(appendTo, childKeys[i])
					}

					// Decode the encoded partition.
					decodedLevel, decodedCentroid, err := DecodePartitionMetadata(encMetadata)
					require.NoError(t, err)
					decodedChildKeys := make([]ChildKey, 0, len(encVectors))
					decodeChildKey := func(encChildKey []byte) {
						childKey, err := DecodeChildKey(encChildKey, decodedLevel)
						require.NoError(t, err)
						decodedChildKeys = append(decodedChildKeys, childKey)
					}
					...
					case *quantize.UnQuantizedVectorSet:
						...
						for i := range encVectors {
							err = DecodeFullVectorToSet(encVectors[i], decodedSet.(*quantize.UnQuantizedVectorSet))
							require.NoError(t, err)
							encChildKey := encVectors[i][GetFullVectorChildKeyOffset(set.Dims):]
							decodeChildKey(encChildKey)
						}
					case *quantize.RaBitQuantizedVectorSet:
						...
						for i := range encVectors {
							err = DecodeRaBitQVectorToSet(encVectors[i], decodedSet.(*quantize.RaBitQuantizedVectorSet))
							require.NoError(t, err)
							encChildKey := encVectors[i][GetRaBitQChildKeyOffset(set.Dims):]
							decodeChildKey(encChildKey)
						}
					}
