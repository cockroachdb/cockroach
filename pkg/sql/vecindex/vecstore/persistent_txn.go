...
// the refs slice. If getFullVectorsFromPK discovers references to partition IDs
// in the refs list, it returns true, false otherwise.
func (psTxn *persistentStoreTxn) getFullVectorsFromPK(
	...
	if len(psTxn.tmpSpans) > 0 {
		...
		for {
			...
			refs[refIdx].Vector = data[0].(*tree.DPGVector).T
		}
	}
	return err
}
