...
func (psTxn *PersistentStoreTxn) GetPartition(
	...
	// GetPartition() is used by fixup to split and merge partitions, so we want to block concurrent writes.
	...
	vectorSet := quantizer.NewQuantizedVectorSet(len(vectorEntries))
