...
func TestSplitPartitionData(t *testing.T) {
	...
	splitPartition := vecstore.NewPartition(
		...
		quantizedSet,
		[]vecstore.ChildKey{
			{PrimaryBytes: vecstore.KeyBytes("vec1")},
			{PrimaryBytes: vecstore.KeyBytes("vec2")},
			{PrimaryBytes: vecstore.KeyBytes("vec3")},
			{PrimaryBytes: vecstore.KeyBytes("vec4")},
			{PrimaryBytes: vecstore.KeyBytes("vec5")},
			{PrimaryBytes: vecstore.KeyBytes("vec6")},
			{PrimaryBytes: vecstore.KeyBytes("vec7")},
		},
		[]vecstore.ValueBytes{
