...
func TestSplitPartitionData(t *testing.T) {
	...
	splitPartition := vecstore.NewPartition(
		...
		quantizedSet,
		[]vecstore.ChildKey{
			{PrimaryBytes: vecstore.PrimaryBytes("vec1")},
			{PrimaryBytes: vecstore.PrimaryBytes("vec2")},
			{PrimaryBytes: vecstore.PrimaryBytes("vec3")},
			{PrimaryBytes: vecstore.PrimaryBytes("vec4")},
			{PrimaryBytes: vecstore.PrimaryBytes("vec5")},
			{PrimaryBytes: vecstore.PrimaryBytes("vec6")},
			{PrimaryBytes: vecstore.PrimaryBytes("vec7")},
		},
		[]vecstore.ValueBytes{
