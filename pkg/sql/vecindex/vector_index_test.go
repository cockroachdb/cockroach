...
func (s *testState) Search(d *datadriven.TestData) string {
	...
	for i := range results {
		...
		fmt.Fprintf(&buf, "%s: %s %s(centroid=%s)\n",
			string(result.ChildKey.PrimaryBytes), formatFloat(result.QuerySquaredDistance, 4),
...
func (s *testState) Insert(d *datadriven.TestData) string {
	...
	if count != 0 {
		...
		for i := 0; i < count; i++ {
			key := vecstore.PrimaryBytes(fmt.Sprintf("vec%d", i))
			childKeys = append(childKeys, vecstore.ChildKey{PrimaryBytes: key})
		}
	} else {
		// Parse vectors.
		for _, line := range strings.Split(d.Input, "\n") {
			...
			key := vecstore.PrimaryBytes(parts[0])
			childKeys = append(childKeys, vecstore.ChildKey{PrimaryBytes: key})
		}
	}
	...
	for i := 0; i < vectors.Count; i++ {
		...
		txn := beginTransaction(s.Ctx, s.T, s.InMemStore)
		s.InMemStore.InsertVector(childKeys[i].PrimaryBytes, vectors.At(i))
		require.NoError(s.T, s.Index.Insert(s.Ctx, txn, vectors.At(i), childKeys[i].PrimaryBytes))
		commitTransaction(s.Ctx, s.T, s.InMemStore, txn)
...
func (s *testState) Recall(d *datadriven.TestData) string {
	...
	calcTruth := func(queryVector vector.T, data []vecstore.VectorWithKey) []vecstore.PrimaryBytes {
		...
		for i := 0; i < len(truth); i++ {
			truth[i] = data[offsets[i]].Key.PrimaryBytes
		}
		return truth
	}
...
func (s *testState) ValidateTree(d *datadriven.TestData) string {
	...
	for {
		...
		if childKeys[0].PrimaryBytes == nil {
...
func (s *testState) parseKeyAndVector(line string) (vecstore.PrimaryBytes, vector.T) {
	parts := strings.Split(line, ":")
	if len(parts) == 1 {
		// Get the value from the store.
		key := vecstore.PrimaryBytes(line)
...
func TestVectorIndexConcurrency(t *testing.T) {
	...
	primaryKeys := make([]vecstore.PrimaryBytes, vectors.Count)
	for i := 0; i < vectors.Count; i++ {
		primaryKeys[i] = vecstore.PrimaryBytes(fmt.Sprintf("vec%d", i))
	}
...
func buildIndex(
	...
	index *VectorIndex,
	vectors vector.Set,
	primaryKeys []vecstore.PrimaryBytes,
) {
