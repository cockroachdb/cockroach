...
func (s *accumulator) addRow(path, user tree.DString, privArr, grantOptionArr *tree.DArray) error {
	...
	privs, err := privilege.ListFromStrings(privilegeStrings, privilege.PurposeFromSystemTable)
	if err != nil {
		return err
	}
	grantOptions, err := privilege.ListFromStrings(grantOptionStrings, privilege.PurposeFromSystemTable)
	if err != nil {
		return err
	}
