...
func (p *planner) DropDatabase(ctx context.Context, n *tree.DropDatabase) (planNode, error) {
	...
	for _, schema := range schemas {
		found, res, err := p.LogicalSchemaAccessor().GetSchema(ctx, p.txn, p.ExecCfg().Codec, dbDesc.ID, schema)
		if err != nil {
			return nil, err
		}
		if !found {
...
func (n *dropDatabaseNode) startExec(params runParams) error {
	...
	for _, schemaToDelete := range n.d.schemasToDelete {
		// Drop only temporary schemas here. All user defined schemas weill be
		// dropped as part of the job.
		if schemaToDelete.Kind == sqlbase.SchemaTemporary {
