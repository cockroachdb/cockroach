...
func (o *sqlForeignKeyCheckOperation) Start(params runParams) error {
	...
	// columns.
	columnsByID := make(map[descpb.ColumnID]*descpb.ColumnDescriptor, len(o.tableDesc.TableDesc().Columns))
	for i := range o.tableDesc.TableDesc().Columns {
		columnsByID[o.tableDesc.TableDesc().Columns[i].ID] = &o.tableDesc.TableDesc().Columns[i]
	}
