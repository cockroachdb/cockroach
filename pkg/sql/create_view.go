...
func (n *createViewNode) startExec(params runParams) error {
	...
	typeBackRefMutables := make(map[descpb.ID]*typedesc.Mutable, len(n.typeDeps))
	for id := range n.typeDeps {
		backRefMutable, err := params.p.Descriptors().GetMutableTypeByID(
			params.ctx, params.p.txn, id, tree.ObjectLookupFlags{})
		if err != nil {
			return err
		}
		typeBackRefMutables[id] = backRefMutable
	}
...
func (p *planner) replaceViewDesc(
	...
	typeBackRefMutables map[descpb.ID]*typedesc.Mutable,
) (*tabledesc.Mutable, error) {
	...
	// we want to see if we still depend on it. If not, then remove the
	// forward and back references.
	var outdatedTypeRefs []descpb.ID
	for _, id := range toReplace.DependsOnTypes {
		// Get the type descriptor that we were previously (and possibly still) depend on.
		desc, ok := typeBackRefMutables[id]
		if !ok {
			var err error
			desc, err = p.Descriptors().GetMutableTypeByID(ctx, p.txn, id, tree.ObjectLookupFlags{
			...
			typeBackRefMutables[id] = desc
		}

		// Create a list of all types we no longer depend on.
		if _, ok := n.typeDeps[id]; !ok {
			outdatedTypeRefs = append(outdatedTypeRefs, id)
		}
	}

	// Remove back references for any types that we no longer depend on.
