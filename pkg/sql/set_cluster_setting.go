...
func (p *planner) SetClusterSetting(
	...
	var setting settings.WritableSetting
	if m, ok := v.(*settings.MaskedSetting); ok {
		// Lookup returns MaskedSetting for non-reportable settings. Since
		// this SET statement is not a report, it's safe to access the
		// underlying setting directly.
		setting = m.UnderlyingSetting()
	} else {
