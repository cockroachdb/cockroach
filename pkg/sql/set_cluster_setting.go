...
var fullModifyClusterSettingEnabled = settings.RegisterBoolSetting(
	settings.TenantWritable,
	"sql.full_modify_cluster_setting.enabled",
...
func checkPrivilegesForSetting(ctx context.Context, p *planner, name string, action string) error {
	...
	if action == "set" {
		...
		fullModifyEnabled := fullModifyClusterSettingEnabled.Get(&p.ExecCfg().Settings.SV)
		...
		} else if !isAdmin && !fullModifyEnabled && !isSqlSetting {
			return pgerror.Newf(pgcode.InsufficientPrivilege,
				"users with the %s privilege need the cluster setting %s to be set to true to %s cluster setting '%s'",
				privilege.MODIFYCLUSTERSETTING, fullModifyClusterSettingEnabled.Key(), action, name)
		}
	}
