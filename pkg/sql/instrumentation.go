...
type instrumentationHelper struct {
	...
	// Parameter used only on EXPLAIN ANALYZE to be displayed at as top-level stat.
	regions []string
}
...
func (m execNodeTraceMetadata) annotateExplain(
	...
	// List of all regions on which any of the statements where executed on.
	...
	walk = func(n *explain.Node) {
		...
		if components, ok := m[wrapped]; ok {
			...
			if !incomplete {
				...
				allRegions = util.CombinesUniqueString(allRegions, regions)
