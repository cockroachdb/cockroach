...
// during merge process and confirms those rows are dropped remove.
func TestIndexOverwritesChunksDuringMerge(t *testing.T) {
	...
	retryTracker := struct {
		oncePerKey map[string]struct{}
		syncutil.Mutex
		commitChannel chan struct{}
	}{
		oncePerKey:    make(map[string]struct{}),
		...
		DistSQL: &execinfra.TestingKnobs{
			...
			IndexBackfillMergerTestingKnobs: &backfill.IndexBackfillMergerTestingKnobs{
				...
				RunDuringMergeTxn: func(ctx context.Context, txn *kv.Txn, startKey, endKey roachpb.Key) error {
					...
					if _, ok := retryTracker.oncePerKey[string(startKey)]; ok {
						return nil
					}
					retryTracker.oncePerKey[string(startKey)] = struct{}{}
	...
	require.Greater(t,
		...
		int64(0),
		"no chunks have been retried with keys removed %d.",
		smallerChunkSizeObserved.Load())
}
