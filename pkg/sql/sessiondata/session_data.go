...
type SessionData struct {
	...
	// DbToTempSchemaMap stores the temp schema ID for every database that has
	// created a temporary schema. The mapping is from decspb.ID -> descpb.ID,
	// but cannot be stored as such due to package dependencies.
	DbToTempSchemaMap map[uint32]uint32
...
func (s *SessionData) IsTemporarySchemaID(ID uint32) bool {
	for _, tempSchemaID := range s.DbToTempSchemaMap {
		if tempSchemaID == ID {
			return true
		}
	}
