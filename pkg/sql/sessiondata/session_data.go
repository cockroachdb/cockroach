...
type LocalUnmigratableSessionData struct {
	...
	// privileges that the user does not actually hold; see
	// DescriptorOverride for the semantics of the individual fields. Set
	// by job-owner-style identity flips that need to scan or modify a
	// specific table on the user's behalf for system-internal purposes
	// (e.g. CREATE-only schema-change issuers running validation count
	// queries, LDR write sessions applying rows under the stream
	// creator's identity). Any access check against a descriptor not in
	// the map falls through to the user's actual grants. Local-only:
	// privilege checks happen on the gateway at planning time. uint32
	// avoids pulling descpb into this package.
