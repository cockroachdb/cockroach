...
func (is *indexUsageStats) Start(ctx context.Context, stopper *stop.Stopper) {
	...
	is.startStatsClearingLoop(ctx, stopper)
}
...
func (is *indexUsageStats) insertIndexUsage(payload *indexusagestats.MetaData) {
	...
	case indexusagestats.FullScan:
		indexStats.FullScanCount++
		indexStats.LastScan = timeutil.Now()
	case indexusagestats.NonFullScan:
		indexStats.NonFullScanCount++
		indexStats.LastScan = timeutil.Now()
	case indexusagestats.ZigzagJoin:
		indexStats.ZigzagJoinCount++
		indexStats.LastJoin = timeutil.Now()
	case indexusagestats.InvertedJoin:
		indexStats.InvertedJoinCount++
		indexStats.LastJoin = timeutil.Now()
	case indexusagestats.LookupJoin:
		indexStats.LookupJoinCount++
		indexStats.LastJoin = timeutil.Now()
	}
}
...
func (is *indexUsageStats) startStatsClearingLoop(ctx context.Context, stopper *stop.Stopper) {
	var timer timeutil.Timer
	var deadline time.Time

	resetIntervalChanged := make(chan struct{}, 1)

	indexusagestats.ClearInterval.SetOnChange(&is.st.SV, func(ctx context.Context) {
		select {
	...
	maybeResetTimer := func() {
		is.mu.RLock()
		last := is.mu.lastReset
		is.mu.RUnlock()

		var newDeadline time.Time
		resetInterval := indexusagestats.ClearInterval.Get(&is.st.SV)
		if last.IsZero() {
			newDeadline = timeutil.Now().Add(resetInterval)
		} else {
			newDeadline = last.Add(resetInterval)
		}
	...
	_ = stopper.RunAsyncTask(ctx, "index-usage-stats-clearer", func(ctx context.Context) {
		for {
			maybeResetTimer()
			select {
			...
			if is.testingKnobs != nil && is.testingKnobs.OnIndexUsageStatsResetCompleteCallback != nil {
				is.testingKnobs.OnIndexUsageStatsResetCompleteCallback()
			}
		}
	})
}
