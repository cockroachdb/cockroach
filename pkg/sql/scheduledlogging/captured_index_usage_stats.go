...
var telemetryCaptureIndexUsageStatsEnabledInterval = settings.RegisterDurationSetting(
	settings.SystemOnly,
	"sql.telemetry.capture_index_usage_stats.enabled_interval",
...
var telemetryCaptureIndexUsageStatsStatusDisabledInterval = settings.RegisterDurationSetting(
	settings.SystemOnly,
	"sql.telemetry.capture_index_usage_stats.disabled_interval",
	"the scheduled interval time between checks of to see if index usage statistics has been enabled",
	8*time.Second,
	settings.NonNegativeDuration,
)
...
func (s *CaptureIndexUsageStatsLoggingScheduler) durationUntilNextInterval() time.Duration {
	...
	if !telemetryCaptureIndexUsageStatsEnabled.Get(&s.cs.SV) {
		return telemetryCaptureIndexUsageStatsStatusDisabledInterval.Get(&s.cs.SV)
	}
	...
	if s.getLoggingDuration() >= telemetryCaptureIndexUsageStatsEnabledInterval.Get(&s.cs.SV) {
		return s.durationOnOverlap()
	}
	// Otherwise, schedule the next interval normally.
	return telemetryCaptureIndexUsageStatsEnabledInterval.Get(&s.cs.SV)
}
