...
ORDER BY total_reads ASC`
		...
		for ok, err = it.Next(ctx); ok; ok, err = it.Next(ctx) {
			...
			mvccStats, err := s.captureMVCCStats(ctx, uint32(roachpb.IndexID(indexID)))
			if err != nil {
...
func (s *CaptureIndexUsageStatsLoggingScheduler) captureMVCCStats(
	ctx context.Context, tableID uint32,
) (*enginepb.MVCCStats, error) {
	startKey := roachpb.RKey(s.codec.TablePrefix(tableID))
