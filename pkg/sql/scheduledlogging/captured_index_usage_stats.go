...
type CaptureIndexUsageStatsTestingKnobs struct {
	...
	// scheduleCompleteChan allows tests to hook into when the current schedule
	...
	_ = stopper.RunAsyncTask(ctx, "capture-index-usage-stats", func(ctx context.Context) {
		...
		for {
			...
			case <-timer.C:
				...
				timer.Reset(dur)
				if s.knobs != nil && s.knobs.scheduleCompleteChan != nil {
					s.knobs.scheduleCompleteChan <- struct{}{}
					<-s.knobs.scheduleCompleteChan
				}
			}
		}
	})
