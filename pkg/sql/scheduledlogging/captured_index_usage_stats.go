...
var telemetryCaptureIndexUsageStatsInterval = settings.RegisterDurationSetting(
	...
	"the time between capturing index usage statistics intervals",
	8*time.Second,
...
func (s *CaptureIndexUsageStatsEmitter) logIndexUsageStatsWithDelay(
	...
	_ = stopper.RunAsyncTask(ctx, "logging-index-usage-stats-with-delay", func(ctx context.Context) {
		...
		ticker := time.NewTicker(captureIndexUsageStatsLoggingDelay)
...
func (s *CaptureIndexUsageStatsEmitter) Interval(cs *cluster.Settings) time.Duration {
	return telemetryCaptureIndexUsageStatsInterval.Get(&cs.SV)
}
