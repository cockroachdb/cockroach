...
func TestCaptureIndexUsageStats(t *testing.T) {
	...
	stubScheduleEnabledInterval := 20 * time.Second
	stubScheduleDisabledInterval := 1 * time.Second
	stubLoggingDelay := 0 * time.Second

	// timeBuffer is a short time buffer to account for delays in the schedule
	// timings when running tests. The time buffer smaller than the difference
