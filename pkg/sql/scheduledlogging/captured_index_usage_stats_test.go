...
func TestCaptureIndexUsageStats(t *testing.T) {
	...
	sd.setOverlapDuration(13 * time.Second)
	stubScheduleEnabledInterval := 23 * time.Second
	stubScheduleDisabledInterval := 3 * time.Second
	stubLoggingDelay := 0 * time.Second

	// timeBuffer is a short time buffer to account for miscellaneous delays in
	// the schedule timings when running tests.
	...
	// statistics.
	telemetryCaptureIndexUsageStatsEnabledInterval.Override(context.Background(), &settings.SV, stubScheduleEnabledInterval)
	// Configure the schedule interval at which we check whether capture index
	// usage statistics has been enabled.
	telemetryCaptureIndexUsageStatsStatusDisabledInterval.Override(context.Background(), &settings.SV, stubScheduleDisabledInterval)
	...
	testutils.SucceedsWithin(t, func() error {
		return checkNumTotalEntriesAndIndexEntries(expectedTotalNumEntriesInSingleInterval, expectedNumberOfIndividualIndexEntriesInSingleInterval)
	}, stubScheduleDisabledInterval+timeBuffer)
	...
	// interval duration.
	stubLoggingDuration := stubScheduleEnabledInterval * 2
	...
	// of logs for each index.
	testutils.SucceedsWithin(t, func() error {
		return checkNumTotalEntriesAndIndexEntries(expectedTotalNumEntriesAfterTwoIntervals, expectedNumberOfIndividualIndexEntriesAfterTwoIntervals)
	}, stubScheduleEnabledInterval+timeBuffer)
	...
	// of logs for each index.
	testutils.SucceedsWithin(t, func() error {
		return checkNumTotalEntriesAndIndexEntries(expectedTotalNumEntriesAfterThreeIntervals, expectedNumberOfIndividualIndexEntriesAfterThreeIntervals)
	...
	testData := []time.Duration{
		0 * time.Second,
		stubScheduleEnabledInterval, /* second difference between first and second schedule */
...
// checkNumTotalEntriesAndIndexEntries is a helper function that verifies that
// we are getting the correct number of total log entries and correct number of
// log entries for each index.
func checkNumTotalEntriesAndIndexEntries(
	...
	if err != nil {
		return errors.Newf("expected no error while fetching entries from files, got %v", err)
	}
