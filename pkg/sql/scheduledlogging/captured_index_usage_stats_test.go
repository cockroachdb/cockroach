...
func TestCaptureIndexUsageStats(t *testing.T) {
	...
	tenantSettings := srv.TenantOrServer().ClusterSettings()
	...
	telemetryCaptureIndexUsageStatsEnabled.Override(context.Background(), &srv.ClusterSettings().SV, false)
	telemetryCaptureIndexUsageStatsEnabled.Override(context.Background(), &tenantSettings.SV, false)
	// Configure the schedule interval at which we capture index usage statistics.kkk
