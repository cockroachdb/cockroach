...
func (n *changePrivilegesNode) startExec(params runParams) error {
	...
	compatiblePrivileges, err := convertPGIncompatibleDatabasePrivilegesToDefaultPrivileges(ctx, p, n)
	if err != nil {
		return err
	}
...
func convertPGIncompatibleDatabasePrivilegesToDefaultPrivileges(
	ctx context.Context, p *planner, n *changePrivilegesNode,
) (compatiblePrivs privilege.List, err error) {
	// If the targets are not databases, there is no work to be done.
	if n.grantOn != privilege.Database {
		return nil, nil
	}
	databaseTargets := n.targets.Databases
	grantees := make(tree.NameList, len(n.grantees))

	for i, grantee := range n.grantees {
		grantees[i] = tree.Name(grantee.SQLIdentifier())
	}
	...
	for _, database := range databaseTargets {
		...
		if len(incompatiblePrivs) > 0 {
			if n.isGrant {
				...
				if err := alterDefaultPrivilegesNode.startExec(p.RunParams(ctx)); err != nil {
					return nil, err
				}
			} else if !n.isGrant {
