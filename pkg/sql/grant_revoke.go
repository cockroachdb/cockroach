...
func (n *changePrivilegesNode) startExec(params runParams) error {
	...
	if len(n.targets.Databases) != 0 {
		...
		for _, desc := range descriptors {
			...
			defaultPrivileges := dbDesc.GetDefaultPrivileges()
			...
			if n.isGrant {
				...
				translatedStatement = fmt.Sprintf("ALTER DEFAULT PRIVILEGES "+
					"FOR ALL ROLES GRANT %s ON TABLES TO %s", incompatiblePrivs, n.grantees.String())
			} else {
				...
				translatedStatement = fmt.Sprintf("ALTER DEFAULT PRIVILEGES "+
					"FOR ALL ROLES REVOKE %s ON TABLES FROM %s", incompatiblePrivs, n.grantees.String())
			}
			...
			// TODO(richardjcai): Log an ALTER DEFAULT PRIVILEGES event here.
			//    need to add alter default privileges event logging in general.
		}
