...
func (n *changePrivilegesNode) startExec(params runParams) error {
	...
	if len(n.targets.Databases) != 0 {
		...
		for _, desc := range descriptors {
			...
			granteesSQLIdentifier := make([]string, len(n.grantees))
			for _, grantee := range n.grantees {
				granteesSQLIdentifier = append(granteesSQLIdentifier, grantee.Normalized())
			}
			if n.isGrant && len(incompatiblePrivs) > 0 {
				...
				translatedStatement = fmt.Sprintf("ALTER DEFAULT PRIVILEGES "+
					...
					incompatiblePrivs, granteesSQLIdentifier, dbDesc.GetName(),
				)
			} else {
