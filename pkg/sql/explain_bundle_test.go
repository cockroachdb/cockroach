...
`)
	...
	t.Run("foreign keys, read-only", func(t *testing.T) {
		...
		r.Exec(t, "CREATE TABLE grandchild1 (pk INT PRIMARY KEY, fk INT REFERENCES child1(pk));")
		r.Exec(t, "CREATE TABLE grandchild2 (pk INT PRIMARY KEY, fk INT REFERENCES child2(pk));")
		var targetTableNames, targetFKs []string
		...
		for _, tableName := range tableNames {
			...
			targetTableNames = []string{targetTableName}
		...
		targetTableNames = []string{"parent", "child1"}
		targetFKs = []string{"ALTER TABLE defaultdb.public.child1 ADD CONSTRAINT"}
		...
		targetTableNames = []string{"parent", "grandchild1"}
		...
		targetTableNames = []string{"parent", "child2", "grandchild1"}
		// Note that we omit the FK from grandchild1 since the FK referenced
		// table isn't being read.
		targetFKs = []string{"ALTER TABLE defaultdb.public.child2 ADD CONSTRAINT"}
		...
		targetTableNames = []string{"parent", "child1", "grandchild1"}
		targetFKs = []string{
