...
`)
	...
	t.Run("under testuser", func(t *testing.T) {
		const fprint = `SELECT * FROM abc`
		url := getBundleThroughBuiltin(fprint, fprint, "" /* planGist */, false /* redacted */, true /* underTestUser */)
		checkBundleContents(
...
func TestExplainBundleEnv(t *testing.T) {
	...
	c := makeStmtEnvCollector(ctx, p, s.InternalExecutor().(*InternalExecutor), "" /* userName */)
