...
CREATE TABLE s.a (a INT PRIMARY KEY);`)
	...
	t.Run("failed sensitive setting as bundle statement", func(t *testing.T) {
		const secret = "hunter2-bundle-secret-token"
		// A failing cast is the only fixture for an error that quotes the
		// attempted value of a secret-carrying SET on this branch: no sensitive
		// setting has a value-validating hook here.
		_, err := r.DB.ExecContext(context.Background(),
			"EXPLAIN ANALYZE (DEBUG) SET CLUSTER SETTING "+
				"server.oidc_authentication.client_secret = ('"+secret+"'::INT)::STRING")
		require.Error(t, err)
