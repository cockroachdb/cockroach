...
`)
	...
	t.Run("statement hints", func(t *testing.T) {
		...
		r.Exec(t, `SELECT information_schema.crdb_set_session_variable_hint('SELECT * FROM table161829 WHERE y = 10', 'vectorize', 'off', 'defaultdb')`)
		rows = r.QueryStr(t, "EXPLAIN ANALYZE (DEBUG) SELECT * FROM table161829 WHERE y = 10")
		checkBundle(
			t, fmt.Sprint(rows), "table161829", func(name, contents string) error {
				if name == "schema.sql" {
					reg := regexp.MustCompile(`crdb_set_session_variable_hint\(.*'vectorize'.*'defaultdb'\)`)
					if reg.FindString(contents) == "" {
