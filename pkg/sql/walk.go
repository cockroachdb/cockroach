...
// node wraps observer.enterNode() and provides it with the current node's name.
func (v *planVisitor) node(plan planNode) bool {
	recurse := v.observer.enterNode(v.nodeName, plan)
	if plan == nil {
		recurse = false
	}
	return recurse
...
// prepare creates a copy of the visitor with the name of the current
// node, and queries the observer to determine whether the node should
// be visiter further.
func (v *planVisitor) prepare(plan planNode) (planVisitor, bool) {
	if plan == nil {
		return planVisitor{}, false
	}

	lv := *v
	lv.nodeName = nodeName(plan)
	return lv, v.observer.enterNode(lv.nodeName, plan)
}
