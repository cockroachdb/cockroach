...
func (p *planner) EvalSubquery(expr *tree.Subquery) (result tree.Datum, err error) {
	...
	if expr.Idx < 0 || expr.Idx-1 >= len(p.curPlan.sqPlans) {
		return nil, pgerror.NewErrorf(pgerror.CodeInternalError,
			"programming error: invalid index %d for %q", expr.Idx, expr)
	}

	s := &p.curPlan.sqPlans[expr.Idx-1]
	if !s.started {
		return nil, pgerror.NewErrorf(pgerror.CodeInternalError,
...
func collectSubquerySpans(params runParams, plan planNode) (roachpb.Spans, error) {
	...
	for i := range params.p.curPlan.sqPlans {
		reads, writes, err := collectSpans(params, params.p.curPlan.sqPlans[i].plan)
		if err != nil {
			return nil, err
		}
		if len(writes) > 0 {
...
// called replaceSubquery on the given subquery node.  The condition
...
func (v *subqueryVisitor) subqueryAlreadyAnalyzed(t *tree.Subquery) bool {
	return t.Idx > 0 && t.Idx-1 < len(v.curPlan.sqPlans) && v.curPlan.sqPlans[t.Idx-1].plan != nil
}
...
func (v *subqueryVisitor) VisitPre(expr tree.Expr) (recurse bool, newExpr tree.Expr) {
	...
	case *tree.ArrayFlatten:
		if sub, ok := t.Subquery.(*tree.Subquery); ok {
			...
			sub.Typ = sub.Typ.(types.TTuple)[0]
		}
...
func (v *subqueryVisitor) replaceSubquery(
	...
	v.curPlan.sqPlans = append(v.curPlan.sqPlans, subquery{subquery: sub, plan: plan})
	sub.Idx = len(v.curPlan.sqPlans) // Note: subquery node are 1-indexed, so that 0 remains invalid.
	result := &v.curPlan.sqPlans[sub.Idx-1]
