...
// MakeIndexDescriptor creates an index descriptor from a CreateIndex node and
...
func MakeIndexDescriptor(
	...
	if n.Sharded != nil &&
		...
		return nil, invalidClusterForShardedIndex
	}
...
func maybeCreateAndAddShardCol(
	...
	_, dropped, err := desc.FindColumnByName(tree.Name(shardCol.Name))
	// FindColumnByName returns an error only when it cannot find a column by the given
	// name.
	if err != nil || dropped {
		if isNewTable {
