...
type baggageEventTxnStart struct {
	tranCtx transitionCtx
}

// baggageEventRetriable represents the baggage for eventRetriableErr.
type baggageEventRetriableErr struct {
...
// baggageEventNonRetriableErr represents the baggage for eventNonRetriableErr.
type baggageEventNonRetriableErr struct {
	// err is the error that caused the event
	err error
}

// baggageWithError is a common interface for the baggages that wrap an error.
type baggageWithError interface {
	errorCause() error
}

// errorCause implements the baggageWithError interface.
func (b baggageEventRetriableErr) errorCause() error {
	return b.err
}

// baggageEventRetriableError implements baggageWithError.
var _ baggageWithError = baggageEventRetriableErr{}

// errorCause implements the baggageWithError interface.
func (b baggageEventNonRetriableErr) errorCause() error {
	return b.err
}

// baggageEventNonRetriableError implements baggageWithError.
var _ baggageWithError = baggageEventNonRetriableErr{}
...
var TxnStateTransitions = Compile(Pattern{
		...
		eventTxnStart{Var("implicitTxn")}: {
			...
			Action: func(args Args) error {
				return args.Extended.(*txnState2).noTxnToOpen(
					...
					args.Baggage.(baggageEventTxnStart).tranCtx)
			},
		},
	},
		...
		eventRetriableErr{CanAutoRetry: True, IsCommit: Any}: {
			...
			Action: func(args Args) error {
				...
				args.Extended.(*txnState2).setAdvanceInfo(
					advanceInfo{
						code:   rewind,
						rewCap: args.Baggage.(baggageEventRetriableErr).rewCap,
						flush:  false})
				return nil
			},
		},
	...
	stateOpen{ImplicitTxn: False, RetryIntent: Var("retryIntent")}: {
		eventNonRetriableErr{IsCommit: False}: {
			...
			Action: func(args Args) error {
				ts := args.Extended.(*txnState2)
				ts.mu.txn.CleanupOnError(ts.Ctx, args.Baggage.(baggageWithError).errorCause())
	...
	stateRestartWait{}: {
		...
		eventNonRetriableErr{IsCommit: False}: {
			...
			Action: func(args Args) error {
				ts := args.Extended.(*txnState2)
				ts.mu.txn.CleanupOnError(ts.Ctx, args.Baggage.(baggageEventNonRetriableErr).err)
				ts.mu.txn = nil
				ts.setAdvanceInfo(advanceInfo{code: skipQueryStr, flush: true})
				return nil
			},
