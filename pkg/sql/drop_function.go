...
func (p *planner) DropFunction(
	...
	for _, fn := range n.Functions {
		...
		mut, err := p.prepareMutateFunction(ctx, fnID)
		if err != nil {
			return nil, err
		}
...
// matchFunction tries to resolve a user-defined function with the given
// signature from current search path, only overload with exactly the same
// arguments types is considered a match. If required==true, error is returned
// if function not found. Error is also returned if a builtin function is
// matched.
...
func (p *planner) prepareMutateFunction(
