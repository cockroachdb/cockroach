...
func TestShowTraceReplica(t *testing.T) {
	...
	sqlDB.Exec(t, `ALTER RANGE "default" CONFIGURE ZONE 'constraints: [+n4]'`)
	...
	sqlDB.Exec(t, `ALTER TABLE d.t1 CONFIGURE ZONE 'constraints: [+n1]'`)
	sqlDB.Exec(t, `ALTER TABLE d.t2 CONFIGURE ZONE 'constraints: [+n2]'`)
	sqlDB.Exec(t, `ALTER TABLE d.t3 CONFIGURE ZONE 'constraints: [+n3]'`)
