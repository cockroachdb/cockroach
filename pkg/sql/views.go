...
// checkViewMaterialized ensures that if a view is required, then the view
// is materialized or not as desired.
func checkViewMaterialized(desc sqlbase.TableDescriptor, requireView, wantMaterialized bool) error {
	if !requireView {
		return nil
	}
	if !desc.IsView() {
