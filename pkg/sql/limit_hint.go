...
func setLimitHint(plan planNode, numRows int64, soft bool) {
	switch n := plan.(type) {
	case *scanNode:
	...
	case *limitNode:
		...
		setLimitHint(n.plan, getLimit(hintCount, n.offset), soft)
	...
	case *sortNode:
		...
		if n.needSort {
			...
			soft = true
			setLimitHint(n.plan, numRows, soft)
		}
	...
	case *groupNode:
		if len(n.desiredOrdering) > 0 {
			...
			if match == len(n.desiredOrdering) {
				...
				setLimitHint(n.plan, 1, false /* !soft */)
	...
	case *indexJoinNode:
		setLimitHint(n.index, numRows, soft)
		setUnlimited(n.table)

	case *unionNode:
		setLimitHint(n.right, numRows, true)
		setLimitHint(n.left, numRows, true)

	case *distinctNode:
		setLimitHint(n.plan, numRows, true)

	case *filterNode:
		setLimitHint(n.source.plan, numRows, soft || !isFilterTrue(n.filter))

	case *renderNode:
		setLimitHint(n.source.plan, numRows, soft)
	...
	case *ordinalityNode:
		setLimitHint(n.source, numRows, soft)
	...
	case *createTableNode:
		if n.sourcePlan != nil {
			setLimitHint(n.sourcePlan, numRows, soft)
		}
	case *createViewNode:
...
func setUnlimited(plan planNode) {
	setLimitHint(plan, math.MaxInt64, true)
}
