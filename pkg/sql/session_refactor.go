...
// executes statements / produces results for one statement at a time. The
// cursor points at all times to the statement that the Session is currently
// executing. Results for statements before the cursor have already been
// produced (but not necessarily delivered to the client). Statements above the
// cursor are queued for future execution. Keeping already executed statements
// in the buffer is useful in case of automatic retries (in which case
// statements from the retried transaction have to be executed again); the
// Session is in charge of trimming old statements that are no longer needed for
// retries.
//
...
// have not actually been delivered to the client; see below.
//
//     +--------------------+
//     |stmtBuf             |                       +-----------------+
//     |                    |                       |Session          |
...
// client). The Session does not control when results are delivered to the
// client, but still it does have some influence over that; this is because of
// the fact that the possibility of doing automatic retries is broken the moment
// results for the transaction in question are delivered to the client. The
// communication module has full freedom in sending results whenever it sees
// fit; however the Session influences communication in the following ways:
//
// a) The Session calls clientComm.flush(), informing the implementer that all
// the previous results can be delivered to the client at will.
//
// b) When deciding whether an automatic retry can be performed for a
// transaction, the Session needs to
//
