...
var defaultTenantTemplate = func() *settings.StringSetting {
	s := settings.RegisterStringSetting(
		settings.SystemOnly,
		"sql.create_tenant.default_template",
		"default tenant configuration template when LIKE is not specified in CREATE TENANT",
...
func GetTenantTemplate(
	...
	if info == nil {
		if likeTenantID == 0 && likeTenantName == "" {
			...
			tmplName := defaultTenantTemplate.Get(&settings.SV)
			if tmplName == "" {
	...
	// Erase the fields that don't make sense to keep for a fresh tenant.
