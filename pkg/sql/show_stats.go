	...
	return &delayedNode{
		...
		constructor: func(ctx context.Context, p *planner) (planNode, error) {
		 ...
		 WHERE "statisticID" = $1`,
			...
			if len(row) != 3 {
				return nil, errors.Errorf("expected three columns from internal query")
			}
			if row[histoColIdx] == tree.DNull {
				// We found a statistic, but it has no histogram.
				return nil, errors.Errorf("histogram %d not found", n.HistogramID)
			}
			if row[tableColIdx] == tree.DNull {
				return nil, errors.Errorf("statistic with no table ID")
			}
			if row[colIDsColIdx] == tree.DNull {
			...
			valType := colDesc.Type.ToDatumType()
			if valType == nil {
