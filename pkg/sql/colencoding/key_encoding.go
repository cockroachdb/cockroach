...
func DecodeIndexKeyToCols(
	...
	if len(index.Interleave.Ancestors) > 0 {
		for i, ancestor := range index.Interleave.Ancestors {
			...
			key, _, err = DecodeKeyValsToCols(vecs, idx, indexColIdx[:length], types[:length], colDirs[:length],
				nil /* unseen */, key)
			if err != nil {
				return nil, false, false, err
			}
	...
	key, foundNull, err = DecodeKeyValsToCols(vecs, idx, indexColIdx, types, colDirs, nil /* unseen */, key)
	if err != nil {
		return nil, false, false, err
	}
