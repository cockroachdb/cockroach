...
func TestNoOpGrant(t *testing.T) {
	...
	_, ok := dbDesc.GetPrivileges().AtomicFindUser(userRoach)
	require.False(t, ok)
	_, ok = scDesc.GetPrivileges().AtomicFindUser(userRoach)
	require.False(t, ok)
	_, ok = tblDesc.GetPrivileges().AtomicFindUser(userRoach)
	require.False(t, ok)
	_, ok = typDesc.GetPrivileges().AtomicFindUser(userRoach)
	require.False(t, ok)
	...
	for _, testCase := range testCases {
		...
		for _, priv := range testCase.allowedPrivs {
			...
			userPriv, ok := desc.GetPrivileges().AtomicFindUser(userRoach)
			require.True(t, ok)
