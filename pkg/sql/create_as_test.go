...
func TestFormat(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			sql:            "CREATE MATERIALIZED VIEW cmvas_implicit_columns_tbl AS SELECT id FROM cmvas_implicit_columns_source_tbl",
			setup:          "CREATE TABLE cmvas_implicit_columns_source_tbl (id int PRIMARY KEY)",
			expectedFormat: "CREATE MATERIALIZED VIEW defaultdb.public.cmvas_implicit_columns_tbl AS SELECT id FROM defaultdb.public.cmvas_implicit_columns_source_tbl WITH DATA",
		},
		{
			sql:            "CREATE MATERIALIZED VIEW cmvas_explicit_columns_tbl (id2) AS SELECT id FROM cmvas_explicit_columns_source_tbl",
			setup:          "CREATE TABLE cmvas_explicit_columns_source_tbl (id int PRIMARY KEY)",
			expectedFormat: "CREATE MATERIALIZED VIEW defaultdb.public.cmvas_explicit_columns_tbl (id2) AS SELECT id FROM defaultdb.public.cmvas_explicit_columns_source_tbl WITH DATA",
		},
	}
