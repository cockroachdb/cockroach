...
func (n *commentOnConstraintNode) startExec(params runParams) error {
	...
	if n.n.Comment == nil {
		err = params.p.deleteComment(
			params.ctx, n.tableDesc.GetID(), uint32(constraint.ConstraintID), catalogkeys.ConstraintCommentType,
		)
	} else {
		err = params.p.updateComment(
			params.ctx, n.tableDesc.GetID(), uint32(constraint.ConstraintID), catalogkeys.ConstraintCommentType, *n.n.Comment,
		)
	}
	if err != nil {
		return err
