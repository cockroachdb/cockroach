...
func (dsp *DistSQLPlanner) tryCreatePlanForInterleavedJoin(
	...
	leftScan := n.left.plan.(*scanNode)
	rightScan := n.right.plan.(*scanNode)
...
// maximalJoinPrefix takes a key and an interleaved join defined by a joinNode
// and returns the maximal prefix of the key which is also a prefix of all keys
...
// both tables.
func maximalJoinPrefix(n *joinNode, key roachpb.Key) (roachpb.Key, bool, error) {
	ancestor, descendant := n.interleavedNodes()
	if ancestor == nil || descendant == nil {
...
func joinSpans(n *joinNode, parentSpans []spanPartition) ([]spanPartition, error) {
	...
	for i, parentPart := range parentSpans {
		...
		for j, parentSpan := range parentPart.spans {
			// Step 1: start key.
			joinSpanStartKey, startTruncated, err := maximalJoinPrefix(n, parentSpan.Key)
			if err != nil {
				return nil, err
			}
			if startTruncated {
			...
			// Step 2: end key.
			joinSpanEndKey, endTruncated, err := maximalJoinPrefix(n, parentSpan.EndKey)
			if err != nil {
				return nil, err
			}
