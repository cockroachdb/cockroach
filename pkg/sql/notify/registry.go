...
// stopper quiesces.
func (r *ListenersRegistry) Start(ctx context.Context) {
	// Make span that contains just pg_notifications table.
	notificationsTableSpan := roachpb.Span{
		Key:    keys.TODOSQLCodec.TablePrefix(keys.PGNotificationsTableID),
		EndKey: keys.TODOSQLCodec.TablePrefix(keys.PGNotificationsTableID + 1),
	}
	ch := make(chan *roachpb.RangeFeedEvent, 1024)
	ctx, _ = r.stopper.WithCancelOnQuiesce(ctx)
	// Until VersionRangefeedLeases is active, we don't yet have rangefeeds
	// available for system ranges.
