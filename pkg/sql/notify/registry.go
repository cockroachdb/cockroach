...
var channelQueueSize = settings.RegisterPublicIntSetting(
...
func NewRegistry(settings *cluster.Settings) *ListenersRegistry {
...
func (r *ListenersRegistry) DrainNotifications(u uint128.Uint128) []Payload {
	r.mu.Lock()
	defer r.mu.Unlock()
	var ret []Payload
	for _, ch := range r.mu.sessionsToChannels[u] {
		for len(ch) > 0 {
			ret = append(ret, <-ch)
		}
	}
...
func (r *ListenersRegistry) Start(
	ctx context.Context, now hlc.Timestamp, stopper *stop.Stopper, distSender *kvcoord.DistSender,
) context.Context {
	// Make span that contains just pg_notifications table.
	notificationsTableSpan := roachpb.Span{
		Key:    keys.SystemSQLCodec.TablePrefix(keys.PGNotificationsTableID),
		EndKey: keys.SystemSQLCodec.TablePrefix(keys.PGNotificationsTableID + 1),
	}
	ch := make(chan *roachpb.RangeFeedEvent, 1024)
	ctx, _ = stopper.WithCancelOnQuiesce(ctx)
	stopper.RunAsyncTask(ctx, "pg_notifications rangefeed", func(ctx context.Context) {
		go func() {
			rangefeedEnabledChan := make(chan struct{})
			kvserver.RangefeedEnabled.SetOnChange(&r.settings.SV, func() {
				rangefeedEnabledChan <- struct{}{}
			})
			for {
				if !kvserver.RangefeedEnabled.Get(&r.settings.SV) {
					select {
					case <-ctx.Done():
						return
					case <-rangefeedEnabledChan:
						continue
					}
				}
				err := distSender.RangeFeed(ctx,
					notificationsTableSpan,
					now,
					false, /* withDiff */
					ch,
				)
				...
				if ctx.Err() != nil {
					log.VEventf(ctx, 1, "exiting pg_notifications rangefeed")
					return
				}
			}
		}()
	})
	stopper.RunAsyncTask(ctx, "pg_notifications bus", func(ctx context.Context) {
		for {
			select {
			case rfEvent := <-ch:
				log.VEventf(ctx, 2, "Got notification event", rfEvent)
				if rfEvent.Val != nil {
