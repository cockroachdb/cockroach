...
func (p *planner) AlterRoleSet(ctx context.Context, n *tree.AlterRoleSet) (planNode, error) {
	...
	if n.AllRoles {
		...
		if !hasAdmin && n.DatabaseName == "" {
			return nil, pgerror.Newf(pgcode.InsufficientPrivilege,
				"only users with the admin role are allowed to ALTER ROLE ALL ... SET")
		}

		// If the user is not an admin, they must be the owner of the database given that ALL IN DATABASE ... is used
		if !hasAdmin {
			isOwner, err := p.HasOwnership(ctx, dbDesc)
			if err != nil {
				return nil, err
			}
			if !isOwner {
