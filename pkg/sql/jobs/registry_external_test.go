...
func TestRegistryResumeExpiredLease(t *testing.T) {
	...
	getRegistry := func(id roachpb.NodeID) *jobs.Registry {
	...
	hookCallCount := 0
	// resomeCounts maps jobs IDs to number of start/resumes.
	...
	jobs.AddResumeHook(func(_ jobs.Type, _ *cluster.Settings) jobs.Resumer {
		...
		hookCallCount++
		lock.Unlock()
		return dummyResumer{resume: func(job *jobs.Job) error {
			lock.Lock()
			select {
	...
	for i := 0; i < jobCount; i++ {
		...
		job, _, err := getRegistry(nodeID.Get()).StartJob(ctx, nil, jobs.Record{Details: jobs.BackupDetails{}})
		if err != nil {
			t.Fatal(err)
		}
		...
		<-resumeCalled
		lock.Lock()
		jobMap[nodeID.Get()] = *job.ID()
		lock.Unlock()
	}

	nodeID.Reset(jobCount + 1)
	getRegistry(nodeID.Get())
