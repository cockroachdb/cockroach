...
func (j *Job) load(ctx context.Context) error {
	...
	if err := j.runInTxn(ctx, func(ctx context.Context, txn *client.Txn) error {
		...
		row, err := j.registry.ex.QueryRow(ctx, txn, stmt, *j.id)
		if err != nil {
			return err
		}
		if row == nil {
...
func (j *Job) insert(ctx context.Context, id int64, payload *Payload) error {
	...
	if err := j.runInTxn(ctx, func(ctx context.Context, txn *client.Txn) error {
		...
		_, err = j.registry.ex.Exec(ctx, txn, stmt, id, StatusPending, payloadBytes)
		return err
	}); err != nil {
		return err
	}
...
func (j *Job) update(
	...
	if err := j.runInTxn(ctx, func(ctx context.Context, txn *client.Txn) error {
		...
		row, err := j.registry.ex.QueryRow(ctx, txn, selectStmt, *j.id)
		if err != nil {
			return err
		}
		...
		n, err := j.registry.ex.Exec(ctx, txn, updateStmt, updateArgs...)
		if err != nil {
			return err
		}
		if n != 1 {
