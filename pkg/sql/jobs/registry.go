...
func (r *Registry) maybeCancelJobs(ctx context.Context, nl nodeLiveness) {
	...
	r.mu.Lock()
	defer r.mu.Unlock()
	// TODO(benesch): be smarter about canceling jobs. This logic is correct but
	// too aggressive. Jobs created immediately after a liveness failure but
	// before we've updated our cached epoch will be unnecessarily canceled, as
	// will jobs created at the end of a slow heartbeat that takes longer than
	// nodeLiveness.livenessThreshold seconds to return.
