...
func (r *Registry) StartJob(
	...
	errCh, err := r.resume(resumeCtx, resultsCh, j)
	// resume only returns an error if it couldn't find a hook for the job. In
	// that case, fail the job.
	if err != nil {
		r.unregister(id)
		// We can pass a nil fn to Failed because the job never started, so there's
		// nothing to run on fail. If failed returns an error here, ignore it. The
		// job will be restarted by another node, hit the same resume error, and retry
		// the call to Failed. We still want to return the original error to the user.
		_ = j.Failed(ctx, err, nil)
		return nil, nil, err
	}
	return j, errCh, nil
}
		...
		for {
			...
			case <-time.After(adoptInterval):
				if err := r.maybeAdoptJob(ctx, nl); err != nil {
					log.Errorf(ctx, "%d: error while adopting jobs: %+v", r.nodeID.Get(), err)
				}
			case <-stopper.ShouldStop():
				return
			}
...
func (r *Registry) resume(
	ctx context.Context, resultsCh chan<- tree.Datums, job *Job,
) (<-chan error, error) {
	payload := job.Payload()
	resumer, err := getResumeHook(payload.Type(), r.settings)
	if err != nil {
...
func (r *Registry) maybeAdoptJob(ctx context.Context, nl nodeLiveness) error {
	...
	for _, row := range rows {
		...
		errCh, err := r.resume(resumeCtx, resultsCh, &job)
		if err != nil {
			if log.V(2) {
				log.Infof(ctx, "skipping job %d: %s", *id, err)
			}
			r.unregister(*id)
			continue
		}
...
func (r *Registry) unregister(jobID int64) {
	...
	// It is possible for a job to be double unregistired. unregister is always
