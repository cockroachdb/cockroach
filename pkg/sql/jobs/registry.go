...
func MakeRegistry(
	...
	r := &Registry{clock: clock, db: db, ex: ex, nodeID: nodeID}
	r.mu.epoch = 1
...
// WatchLiveness polls the current node for liveness failures and cancels all
// registered jobs if it observes a failure.
