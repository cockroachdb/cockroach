...
type testingRelocateLeaseNode struct {
	optColumnsSlot
...
// TestingRelocateLease moves leases to specific stores.
// (`ALTER TABLE/INDEX ... TESTING_RELOCATE LEASE ...` statement)
// Privileges: INSERT on table.
func (p *planner) TestingRelocateLease(
	ctx context.Context, n *tree.TestingRelocateLease,
) (planNode, error) {
	tableDesc, index, err := p.getTableAndIndex(ctx, n.Table, n.Index, privilege.INSERT)
	if err != nil {
		return nil, err
	...
	cols := planColumns(rows)
	if len(cols) < 2 {
		return nil, errors.Errorf("less than two columns in TESTING_RELOCATE LEASE data")
	}
	if len(cols) > len(index.ColumnIDs)+1 {
		return nil, errors.Errorf("too many columns in TESTING_RELOCATE LEASE data")
	}
	for i := range cols {
		if !cols[i].Typ.Equivalent(desiredTypes[i]) {
	...
	return &testingRelocateLeaseNode{
		tableDesc: tableDesc,
		index:     index,
		rows:      rows,
		run: testingRelocateRun{
...
func (n *testingRelocateLeaseNode) Next(params runParams) (bool, error) {
	...
	if !data[0].ResolvedType().Equivalent(types.Int) {
		return false, errors.Errorf(
			"expected int in the first TESTING_RELOCATE LEASE data column; got %s",
			data[0].ResolvedType(),
		)
	}
	storeID := roachpb.StoreID(*data[0].(*tree.DInt))
	if storeID <= 0 {
		return false, errors.Errorf("invalid target leaseholder store ID %d for TESTING_RELOCATE", storeID)
	}
...
func (n *testingRelocateLeaseNode) Values() tree.Datums {
	return tree.Datums{
		tree.NewDBytes(tree.DBytes(n.run.lastRangeStartKey)),
		tree.NewDString(keys.PrettyPrint(sqlbase.IndexKeyValDirs(n.index), n.run.lastRangeStartKey)),
	}
...
func (n *testingRelocateLeaseNode) Close(ctx context.Context) {
	n.rows.Close(ctx)
}
