...
const distSQLWrapperResultChanSize = 1024
...
func (p *planner) newDistSQLWrapper(plan planNode, stmtType tree.StatementType) planNode {
	n := &distSQLWrapper{
		plan:       plan,
		stmtType:   stmtType,
		resultRows: make(chan tree.Datums, distSQLWrapperResultChanSize),
	}
	n.resultWriter = newCallbackResultWriter(func(ctx context.Context, row tree.Datums) error {
		select {
		case n.resultRows <- row:
			return nil
		case <-ctx.Done():
			return errors.Wrap(ctx.Err(), "context canceled while writing distSQLWrapper result")
		}
	})
	return n
}
