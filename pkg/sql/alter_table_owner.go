...
func (n *alterTableOwnerNode) startExec(params runParams) error {
	...
	if err := p.setNewTableOwner(ctx, tableDesc, &n.prefix, newOwner); err != nil {
		return err
	}
...
func (p *planner) setNewTableOwner(
	...
	if prefix != nil {
		...
		tbName = &tbNameWithSchema
	} else {
		tbName, err = p.getQualifiedTableName(ctx, desc)
		if err != nil {
			return err
		}
	}
