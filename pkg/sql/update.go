...
// namesForExprs both preperforms early subqyery analysis and extraction and
...
func (p *planner) namesForExprs(
...
) (tree.NameList, []*tree.UpdateExpr, error) {
	...
	// the reader.
	setExprs := make([]*tree.UpdateExpr, len(exprs))
	...
	for _, expr := range exprs {
		if expr.Tuple {
			...
			if n < 0 {
				return nil, nil, errors.Errorf("unsupported tuple assignment: %T", expr.Expr)
			}
			if len(expr.Names) != n {
				return nil, nil, fmt.Errorf("number of columns (%d) does not match number of values (%d)",
					len(expr.Names), n)
			}
		}
