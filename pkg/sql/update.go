...
func (u *updateNode) processSourceRow(params runParams, sourceVals tree.Datums) error {
	...
	// centroids for puts. This information is passed to tableInserter.row below.
	...
	if n := len(u.run.tu.tableDesc().VectorIndexes()); n > 0 {
		...
		centroidVals := vectorIndexVals[n : n*2]
		delPartitionVals := vectorIndexVals[n*2 : n*3]
		vh.InitForPut(putPartitionVals, centroidVals, u.run.tu.tableDesc())
