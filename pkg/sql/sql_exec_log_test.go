...
type sqlExecLogSpy struct {
	logsChan chan eventpb.QueryExecute
...
func TestSqlExecLog(t *testing.T) {
	...
	logsChan := make(chan eventpb.QueryExecute, 100)
	...
	entries := make([]eventpb.QueryExecute, 0, len(queries))
	timer := timeutil.NewTimer()
	timer.Reset(1 * time.Minute)
	defer timer.Stop()
	for len(entries) != len(queries) {
		select {
		case qe := <-logsChan:
			...
			t.Logf("received log: %v", qe)
			entries = append(entries, qe)
		case <-timer.C:
			timer.Read = true
			t.Fatalf("timed out waiting for all entries %v", entries)
		}
	}

	for i, entry := range entries {
		// Verify expected log.
		expectedStmt := queries[i]
		require.Containsf(t, entry.Statement.StripMarkers(), expectedStmt, "entry: %v", entries[i])

		// Query age should be less than 3 seconds.
		require.Lessf(t, entry.Age, float32(3000), "entry: %v", entries[i])
	}
