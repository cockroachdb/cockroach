...
func (p {{template "opName" .}}) Next(ctx context.Context) coldata.Batch {
	...
	if sel := batch.Selection(); sel != nil {
		for _, i := range sel {
			arg1 := {{.LTyp.Get false "col1" "int(i)"}}
			arg2 := {{.RTyp.Get false "col2" "int(i)"}}
		...
		for {{.LTyp.Range "i" "col1"}} {
			arg1 := {{.LTyp.Get false "col1" "i"}}
			arg2 := {{.LTyp.Get false "col2" "i"}}
			{{(.Assign "projCol[i]" "arg1" "arg2")}}
		}
	}
