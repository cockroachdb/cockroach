...
func genMergeJoinOps(wr io.Writer) error {
	...
	s = incrementLeftSwitch.ReplaceAllString(s, `{{template "incrementLeftSwitch" buildDict "Global" $ "LTyp" .LTyp "JoinType" $1 "Sel" $2 "MJOverload" $3 "lHasNulls" $4}}`)

	incrementRightSwitch := makeFunctionRegex("_INCREMENT_RIGHT_SWITCH", 4)
	s = incrementRightSwitch.ReplaceAllString(s, `{{template "incrementRightSwitch" buildDict "Global" $ "LTyp" .LTyp "JoinType" $1 "Sel" $2 "MJOverload" $3 "rHasNulls" $4}}`)
