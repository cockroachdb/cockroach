...
if sel := batch.Selection(); sel != nil {
	...
	for _, i := range sel {
		var cmp bool
		arg := {{.Global.LTyp.Get false "col" "int(i)"}}
	...
	for {{.Global.LTyp.Range "i" "col"}} {
		var cmp bool
		arg := {{.Global.LTyp.Get false "col" "i"}}
		{{(.Global.Assign "cmp" "arg" "p.constArg")}}
...
if sel := batch.Selection(); sel != nil {
	...
	for _, i := range sel {
		var cmp bool
		arg1 := {{.Global.LTyp.Get false "col1" "int(i)"}}
		arg2 := {{.Global.RTyp.Get false "col2" "int(i)"}}
	...
	for {{.Global.LTyp.Range "i" "col1"}} {
		var cmp bool
		arg1 := {{.Global.LTyp.Get false "col1" "i"}}
		arg2 := {{.Global.RTyp.Get false "col2" "i"}}
		{{(.Global.Assign "cmp" "arg1" "arg2")}}
		if cmp {{if .HasNulls}}&& !nulls.NullAt(uint16(i)) {{end}}{
			sel[idx] = uint16(i)
			idx++
