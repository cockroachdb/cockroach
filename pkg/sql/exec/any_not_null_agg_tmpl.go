...
func _FIND_ANY_NOT_NULL(a *anyNotNull_TYPEAgg, nulls *coldata.Nulls, i int, _HAS_NULLS bool) { // */}}
	...
	if !a.foundNonNullForCurrentGroup && !isNull {
		...
		// output.
		// TODO(asubiotto): I can't use execgen.{Get,Set} here because the receiver
		// names are different in _FIND_ANY_NO_NULL and the main loop. Any ideas?
		v := execgen.GET(col, int(i))
		execgen.SET(a.vec, a.curIdx, v)
