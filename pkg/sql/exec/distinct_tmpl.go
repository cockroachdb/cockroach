...
func orderedDistinctColsToOperators(
	...
	distinctCol := make([]bool, col.BatchSize)
	var err error
...
type partitioner interface {
	...
	partition(colVec col.Vec, outputCol []bool, n uint64)
}
...
func (p *sortedDistinct_TYPEOp) Next() col.Batch {
	batch := p.input.Next()
	if batch.Length() == 0 {
		return batch
	}
...
func (p partitioner_TYPE) partition(colVec col.Vec, outputCol []bool, n uint64) {
