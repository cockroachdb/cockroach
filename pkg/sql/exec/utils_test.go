...
// hasNulls is a helper function that returns whether any of the columns in b
// (maybe) have nulls.
func hasNulls(b coldata.Batch) bool {
	for i := 0; i < b.Width(); i++ {
	...
	t.Run("verifySelAndNullResets", func(t *testing.T) {
		...
		for i := 0; i < 2; i++ {
			...
			if i == 0 {
				...
				secondBatchHasNulls = hasNulls(b)
			}
			if i == 1 {
				if secondBatchHasSelection {
				...
				if secondBatchHasNulls {
					assert.True(t, hasNulls(b))
				} else {
					assert.False(t, hasNulls(b))
				}
			}
		}
	})
