	...
	for {
		...
		if vec.HasNulls() {
			...
			if sel := batch.Selection(); sel != nil {
				...
				if si.negate {
					for _, i := range sel {
						if !nulls.NullAt(uint16(i)) && si.cmpIn_TYPE(col[i]) == siFalse {
							sel[idx] = uint16(i)
							idx++
						}
					}
					...
					for _, i := range sel {
						if !nulls.NullAt(uint16(i)) && si.cmpIn_TYPE(col[i]) == siTrue {
							sel[idx] = uint16(i)
							idx++
						}
					}
			...
			if sel := batch.Selection(); sel != nil {
				...
				if si.negate {
					for _, i := range sel {
						if si.cmpIn_TYPE(col[i]) == siFalse {
							sel[idx] = uint16(i)
							idx++
						}
					}
