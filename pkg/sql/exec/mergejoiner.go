...
// row by row, the operator performs a first scan where it
...
// The outputBuffer is used as in the previous version.
...
func copyAtToColVec(
	...
	if sel != nil {
		dest.CopyAtWithSelInt16(src, sel, uint64(destStartIdx), uint64(srcStartIdx), uint64(srcEndIdx), colType)
	} else {
		dest.CopyAt(src, uint64(destStartIdx), uint64(srcStartIdx), uint64(srcEndIdx), colType)
	}
}
...
// dumpRepeatedRows takes a []repeatedChunk and expands each chunk in the output by repeating
...
// then dumpRepeatedRows expands this to
//  L1 |  L2
//  --------
//  1  |  a
//  1  |  a
...
//  1  |  b
//  1  |  b
func (c *mergeJoinOp) dumpRepeatedRows(
	...
	savedOutCount := 0
	outCount := uint16(0)
	for _, colIdx := range src.outCols { // for each column
		savedOutCount = 0
		outCount = 0
		outStartIdx := int(destStartIdx)
		outCol := c.out.ColVec(int(colIdx))
		savedOutCol := c.savedOutput.ColVec(int(colIdx))
		srcCol := bat.ColVec(int(colIdx))
		colType := src.sourceTypes[colIdx]
			...
			for curSrcStartIdx := repeatedRow.rowStartIdx; curSrcStartIdx < repeatedRow.rowEndIdx; curSrcStartIdx++ { // for each row in a run
				for k := 0; k < repeatedRow.numRepeats; k++ { // repeat each row numRepeats times
					if outStartIdx < c.outputBatchSize {
						copyAtToColVec(outCol, outStartIdx, srcCol, curSrcStartIdx, curSrcStartIdx+1, sel, colType)
						outStartIdx++
						outCount++
					} else {
						addSliceToColVec(savedOutCol, c.savedOutputEndIdx+savedOutCount, srcCol, curSrcStartIdx, curSrcStartIdx+1, sel, colType)
						savedOutCount++
					}
				}
			}
...
// dumpRepeatedGroups takes a []repeatedChunk and repeats each group numRepeats times.
...
// then dumpRepeatedGroups expands this to
//  R1 |  R2
//  --------
//  1  |  a
//  1  |  b
...
//  1  |  a
//  1  |  b
func (c *mergeJoinOp) dumpRepeatedGroups(
	...
	for _, colIdx := range src.outCols { // for each column
		savedOutputCount = 0
		outStartIdx := int(destStartIdx)
		outCol := c.out.ColVec(int(colIdx) + colOffset)
		savedOutCol := c.savedOutput.ColVec(int(colIdx) + colOffset)
		srcCol := bat.ColVec(int(colIdx))
		colType := src.sourceTypes[colIdx]
		for i := 0; i < groupsLen; i++ { // for each group
			...
			for k := 0; k < repeatedGroup.numRepeats; k++ { // repeat each group slice numRepeats times
				...
				copyAtToColVec(outCol, outStartIdx, srcCol, repeatedGroup.rowStartIdx, repeatedGroup.rowStartIdx+toAppend, sel, colType)

				if toAppend < repeatedGroup.rowEndIdx-repeatedGroup.rowStartIdx {
					addSliceToColVec(savedOutCol, c.savedOutputEndIdx+savedOutputCount, srcCol, (repeatedGroup.rowStartIdx)+toAppend, repeatedGroup.rowEndIdx, sel, colType)
				}
...
func (c *mergeJoinOp) continueSavedRuns(
	lIdx int,
	lLength int,
	lBat ColBatch,
	lSel []uint16,
...
func (c *mergeJoinOp) flushSavedRunsToOutput() uint16 {
	repeatedRows := []repeatedChunk{{0, c.lGroupEndIdx, c.rGroupEndIdx}}
	repeatedGroups := []repeatedChunk{{0, c.rGroupEndIdx, c.lGroupEndIdx}}

	outCount, savedOutCount := c.dumpRepeatedRows(repeatedRows, 1, 0, &c.left, c.lGroup, nil, 0) // dump left cols
	c.dumpRepeatedGroups(repeatedGroups, 1, len(c.left.sourceTypes), &c.right, c.rGroup, nil, 0)
...
func (c *mergeJoinOp) Next() ColBatch {
	...
	for {
		...
		if (lLength == 0 || rLength == 0) && (c.lGroupEndIdx == 0 && c.rGroupEndIdx == 0) {
			c.out.SetLength(0)
			return c.out
		}

		// Phase 0: finish previous run if it exists (mini scan+dump, to only continue the current run).
		if c.lGroupEndIdx > 0 || c.rGroupEndIdx > 0 {
			lIdx, rIdx = c.continueSavedRuns(lIdx, lLength, lBat, lSel, lKeys, rIdx, rLength, rBat, rSel, rKeys)
			if lIdx != lLength && rIdx != rLength || (lLength == 0 || rLength == 0) {
				outCount = c.flushSavedRunsToOutput()
			}
		}
		...
		// Phase 2: dump
		rowOutCount, savedOutCount := c.dumpRepeatedRows(repeatedRows, repeatedRowsIdx, 0, &c.left, lBat, lSel, outCount)
		c.dumpRepeatedGroups(repeatedGroups, repeatedGroupsIdx, len(c.left.sourceTypes), &c.right, rBat, rSel, outCount)
		c.savedOutputEndIdx += savedOutCount
		...
		if outCount == 0 {
			...
			if lIdx == lLength {
				lIdx, lLength, lBat, lSel = getValInNextBatch(&c.left)
			}
			if rIdx == rLength {
				rIdx, rLength, rBat, rSel = getValInNextBatch(&c.right)
			}
		}

		if c.isOutputBatchFullOrAnInputBatchFullyProcessed(outCount, lIdx, lLength, rIdx, rLength) {
			c.saveRelevantBatchesToState(lIdx, lLength, lBat, rIdx, rLength, rBat)
		}
