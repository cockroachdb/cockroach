...
func (c *mergeJoinOp) getExpectedOutCount(groups []group) uint16 {
	count := 0
	for _, group := range groups {
		count += (group.rowEndIdx - group.rowStartIdx) * group.numRepeats
	}

	if count > c.outputBatchSize {
		count = c.outputBatchSize - int(c.output.Length())
	}
