...
type AppendArgs struct {
	...
	Sel []uint16
	// DestIdx is the first index that Append will append to. If 0, the end is
	// used.
	DestIdx uint64
	// SrcStartIdx is the index of the first element in Src that Append will
	// append. Ignored if Sel is set.
	SrcStartIdx uint16
...
type Vec interface {
	...
	// Vec. Note that there are special cases if indices are 0 in some cases.
