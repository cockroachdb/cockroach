...
type Nulls interface {
	...
	// between start and end to null.
	SetNullRange(s uint64, e uint64)
...
func (m *memColumn) SetNullRange(s uint64, e uint64) {
	if s > e {
		return
	}
