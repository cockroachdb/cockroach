		...
		if m.hasNulls {
			...
			if mod != 0 {
				...
				for i, j := startIdx, 0; i < endIdx-1; i, j = i+1, j+1 {
					...
					// And now or the remaining bits with the bits we want to bring over
					// from the next index, note that we handle endIdx-1 separately.
					nulls[j] |= (m.nulls[i+1] << (64 - mod))
				}
				// Get the first bit to where we want it for endIdx-1.
