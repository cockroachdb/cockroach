...
func TestEncDatumRowsToColVecBool(t *testing.T) {
	...
	vec := col.newMemColumn(types.Bool, 2)
	...
	expected := col.newMemColumn(types.Bool, 2)
...
func TestEncDatumRowsToColVecInt16(t *testing.T) {
	...
	vec := col.newMemColumn(types.Int16, 2)
	ct := sqlbase.ColumnType{SemanticType: sqlbase.ColumnType_INT, Width: 16}
	if err := EncDatumRowsToColVec(rows, vec, 0 /* columnIdx */, &ct, &alloc); err != nil {
		t.Fatal(err)
	}
	expected := col.newMemColumn(types.Int16, 2)
	expected.Int16()[0] = 17
	expected.Int16()[1] = 42
	if !reflect.DeepEqual(vec, expected) {
		t.Errorf("expected vector %+v, got %+v", expected, vec)
...
func TestEncDatumRowsToColVecDecimal(t *testing.T) {
	...
	expected := col.newMemColumn(types.Decimal, 3)
	...
	vec := col.newMemColumn(types.Decimal, 3)
