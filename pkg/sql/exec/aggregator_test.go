...
func (tc *aggregatorTestCase) init() error {
	...
	if tc.batchSize == 0 {
		tc.batchSize = col2.BatchSize
	}
	if tc.outputBatchSize == 0 {
		tc.outputBatchSize = col2.BatchSize
	}
	return nil
}
...
func TestAggregatorRandomCountSum(t *testing.T) {
	...
	for _, groupSize := range []int{1, 2, col2.BatchSize / 4, col2.BatchSize / 2} {
		for _, numInputBatches := range []int{1, 2, 64} {
			t.Run(fmt.Sprintf("groupSize=%d/numInputBatches=%d", groupSize, numInputBatches),
				func(t *testing.T) {
					...
					totalInputRows := numInputBatches * col2.BatchSize
					nOutputRows := totalInputRows / groupSize
					expBatches := (nOutputRows / col2.BatchSize)
					if nOutputRows%col2.BatchSize != 0 {
						expBatches++
					}
					if i != expBatches {
...
func BenchmarkAggregator(b *testing.B) {
	...
	for _, aggFn := range []distsqlpb.AggregatorSpec_Func{
		...
		b.Run(fName, func(b *testing.B) {
			for _, groupSize := range []int{1, col2.BatchSize / 2, col2.BatchSize} {
				for _, numInputBatches := range []int{1, 2, 64} {
					batch := col2.NewMemBatch([]types.T{types.Int64, types.Decimal})
					...
					b.Run(
						...
						func(b *testing.B) {
							...
							b.SetBytes(int64(8 * col2.BatchSize * numInputBatches))
							for i := 0; i < b.N; i++ {
