...
// Insertion sort
func (p *sort_TYPE_DIR_NULLSOp) insertionSort(a, b int) {
	for i := a + 1; i < b; i++ {
...
func (p *sort_TYPE_DIR_NULLSOp) siftDown(lo, hi, first int) {
	root := lo
	for {
		child := 2*root + 1
		if child >= hi {
...
func (p *sort_TYPE_DIR_NULLSOp) heapSort(ctx context.Context, a, b int) {
	first := a
	lo := 0
	hi := b - a
...
func (p *sort_TYPE_DIR_NULLSOp) medianOfThree(m1, m0, m2 int) {
	// sort 3 elements
	if p.Less(m1, m0) {
		p.Swap(m1, m0)
	}
...
func (p *sort_TYPE_DIR_NULLSOp) swapRange(a, b, n int) {
	for i := 0; i < n; i++ {
		p.Swap(a+i, b+i)
	}
}

func (p *sort_TYPE_DIR_NULLSOp) doPivot(lo, hi int) (midlo, midhi int) {
...
func (p *sort_TYPE_DIR_NULLSOp) quickSort(ctx context.Context, a, b, maxDepth int) {
