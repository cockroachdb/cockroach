...
type vecComparator interface {
	...
	// and rowIdx is the index of the value in that vector to compare. Returns -1,
	// 0, or 1.
	compare(vecIdx1, vecIdx2 int, rowIdx1, rowIdx2 uint16) int
...
func (o *orderedSynchronizer) compareRow(batchIdx1 int, batchIdx2 int) int {
	...
	for i := range o.ordering {
		info := o.ordering[i]
		res := o.comparators[i].compare(batchIdx1, batchIdx2, rowIdx1, rowIdx2)
		if res != 0 {
