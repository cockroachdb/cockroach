...
func MakeArrowBatchConverter(typs []types.T) ArrowBatchConverter {
...
// ColVecToArrow converts the first n elements of the given vec of type typ into
// an arrow array.Data. It is assumed that vecs are not larger than
// coldata.BatchSize.
func (c ArrowBatchConverter) BatchToArrow(batch coldata.Batch) []*array.Data {
	n := int(batch.Length())
	for i, typ := range c.typs {
		...
		if vec.HasNulls() {
			// Cast null bitmap.
			vecBm := vec.NullBitmap()
			vecBmHeader := (*reflect.SliceHeader)(unsafe.Pointer(&vecBm))
			arrowBmHeader := (*reflect.SliceHeader)(unsafe.Pointer(&arrowBm))
			arrowBmHeader.Data = vecBmHeader.Data
			arrowBmHeader.Len = vecBmHeader.Len * sizeOfInt64
			arrowBmHeader.Cap = vecBmHeader.Cap * sizeOfInt64
		}
		...
		if typ == types.Bool || typ == types.Bytes {
			...
			if arrowBm != nil {
				...
				data.Buffers()[0] = memory.NewBufferBytes(arrowBm)
			}
		...
		c.scratch.buffers[i][0] = memory.NewBufferBytes(arrowBm)
		...
		// information is inferred from the ArrowBatchConverter schema and null
		// count is an optimization we can use when working with nulls.
		c.scratch.arrowData[i] = array.NewData(
			nil /* dtype */, n, c.scratch.buffers[i], nil /* childData */, 0 /* nulls */, 0, /* offset */
		)
	}
	return c.scratch.arrowData
}

func (c ArrowBatchConverter) ArrowToBatch(data []*array.Data) coldata.Batch {
	for i, typ := range c.typs {
		...
		arrowBm := arr.NullBitmapBytes()
		if arrowBm != nil {
			var vecBm []int64
			arrowBmHeader := (*reflect.SliceHeader)(unsafe.Pointer(&arrowBm))
			vecBmHeader := (*reflect.SliceHeader)(unsafe.Pointer(&vecBm))
			vecBmHeader.Data = arrowBmHeader.Data
			vecBmHeader.Len = arrowBmHeader.Len / sizeOfInt64
			vecBmHeader.Cap = arrowBmHeader.Cap / sizeOfInt64

			vec.SetNullBitmap(vecBm)
		}
	}
