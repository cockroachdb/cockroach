...
type VectorizedRuntimeErrorMarker interface {
	...
	// Mark is used as a mark on a struct that implements it. It will never be
	// called.
	Mark()
}
...
// Mark is part of VectorizedRuntimeErrorMarker interface.
func (v VectorizedRuntimeError) Mark() {
	panic("Mark should not be called directly")
}
...
func CatchVectorizedRuntimeError(operation func()) (retErr error) {
	defer func() {
		if err := recover(); err != nil {
			...
			// No panic happened, then operation must have been executed
			// successfully.
			retErr = nil
		}
	}()
