...
type colSorter interface {
	...
	sort(ctx context.Context, cancelChecker *CancelChecker)
	// sortPartitions sorts this sorter's column once for every partition in the
	// partition slice.
	sortPartitions(ctx context.Context, cancelChecker *CancelChecker, partitions []uint64)
...
func (p *sortOp) sort(ctx context.Context) {
	...
	if partitionsCol == nil {
		...
		p.sorters[0].sort(ctx, &cancelChecker)
		if len(p.sorters) == 1 {
	...
	for i, sorter := range sorters {
		...
		sorter.sortPartitions(ctx, &cancelChecker, partitions)
	}
}
