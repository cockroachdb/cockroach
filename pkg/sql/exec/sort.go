...
const (
	...
	// chunkerEmittingFromBuffer is the state of chunker operator in which it
	// emits tuples that have been buffered. All those tuples belong to the same
	// chunk. After all tuples are emitted, the buffer is cleared, a zero-length
	// batch is emitted to indicate the end of the chunk, and chunker is moved
	// to chunkerEmittingFromBatch state.
	chunkerEmittingFromBuffer
	// chunkerEmittingFromBatch is the state of chunker operator in which it
	// emits all chunks that are fully-contained with the last read batch (i.e.
	// all except the last chunk which might include tuples from the next batch).
	// The last chunk is buffered, and chunker is moved to chunkerReading state.
...
// chunk chunker emits a single zero-length batch. To indicate that the input
// has been fully consumed it emits the second zero-length batch (and will
// continue to do so when Next()'ed).
//
