...
func (p *allSpooler) init() {
	p.input.Init()
	p.values = make([]col.Vec, len(p.inputTypes))
	for i := 0; i < len(p.inputTypes); i++ {
		p.values[i] = col.NewMemColumn(p.inputTypes[i], 0)
	}
}
...
func (p *allSpooler) getValues(i int) col.Vec {
	if !p.spooled {
...
type colSorter interface {
	...
	init(col col.Vec, order []uint64, workingSpace []uint64)
...
func (p *sortOp) Next() col.Batch {
	...
	case sortSorting:
		...
		fallthrough
	case sortEmitting:
		newEmitted := p.emitted + col.BatchSize
