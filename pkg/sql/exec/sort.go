...
type colSorter interface {
	...
	sort(cancelChecker CancelChecker)
	// sortPartitions sorts this sorter's column once for every partition in the
	// partition slice.
	sortPartitions(partitions []uint64, cancelChecker CancelChecker)
...
func (p *sortOp) sort(ctx context.Context) {
	...
	if partitionsCol == nil {
		...
		p.sorters[0].sort(cancelChecker)
		if len(p.sorters) == 1 {
	...
	for i, sorter := range sorters {
		...
		sorter.sortPartitions(partitions, cancelChecker)
	}
}
