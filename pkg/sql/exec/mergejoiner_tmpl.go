...
// {{/*
func _PROBE_SWITCH(sel perm, asc bool, lNull bool, rNull bool) { // */}}
	// {{define "probeSwitch"}}
	// {{ $sel := $.Sel }}
	switch colType {
...
// {{/*
func _LEFT_SWITCH(hasNulls bool, isSel bool) { // */}}
	// {{define "leftSwitch"}}
			...
			for ; o.builderState.left.curSrcStartIdx < leftGroup.rowEndIdx; o.builderState.left.curSrcStartIdx++ {
				...
				for ; o.builderState.left.numRepeatsIdx < leftGroup.numRepeats; o.builderState.left.numRepeatsIdx++ {
					...
					if outStartIdx < o.outputBatchSize {
						...
						// {{ if $.HasNulls }}
						// TODO (georgeutsin): create a SetNullRange(start, end) function in coldata.Nulls, and place this outside the tight loop.
...
// {{/*
func _RIGHT_SWITCH(hasNulls bool, isSel bool) { // */}}
	// {{define "rightSwitch"}}
