...
LeftColLoop:
		...
		case _TYPES_T:
			...
			if sel != nil {
				...
				for ; o.builderState.left.groupsIdx < groupsLen; o.builderState.left.groupsIdx++ {
					...
					if o.builderState.left.curSrcStartIdx == zeroMJBuilderState.curSrcStartIdx {
					...
					for ; o.builderState.left.curSrcStartIdx < leftGroup.rowEndIdx; o.builderState.left.curSrcStartIdx++ {
						...
						for ; o.builderState.left.numRepeatsIdx < leftGroup.numRepeats; o.builderState.left.numRepeatsIdx++ {
							...
							if outStartIdx < o.outputBatchSize {
								...
								if o.builderState.left.colIdx == len(input.outCols)-1 {
									o.builderState.left.colIdx = zeroMJBuilderState.colIdx
									return outStartIdx
								}
						...
						o.builderState.left.numRepeatsIdx = zeroMJBuilderState.numRepeatsIdx
					}
					o.builderState.left.curSrcStartIdx = zeroMJBuilderState.curSrcStartIdx
				}
				o.builderState.left.groupsIdx = zeroMJBuilderState.groupsIdx
			} else {
				// Loop over every group.
				for ; o.builderState.left.groupsIdx < groupsLen; o.builderState.left.groupsIdx++ {
					leftGroup := leftGroups[o.builderState.left.groupsIdx]
	...
	o.resetBuilderCrossProductStateForSide(&o.builderState.left)
	return outStartIdx
}
...
RightColLoop:
			...
			if sel != nil {
				...
				for ; o.builderState.right.groupsIdx < groupsLen; o.builderState.right.groupsIdx++ {
					...
					for ; o.builderState.right.numRepeatsIdx < rightGroup.numRepeats; o.builderState.right.numRepeatsIdx++ {
						if o.builderState.right.curSrcStartIdx == zeroMJBuilderState.curSrcStartIdx {
						...
						if toAppend < rightGroup.rowEndIdx-o.builderState.right.curSrcStartIdx {
							...
							if o.builderState.right.colIdx == len(input.outCols)-1 {
								o.builderState.right.colIdx = zeroMJBuilderState.colIdx
								return
							}
							o.builderState.right = initialBuilderState
							continue RightColLoop
						}
						o.builderState.right.curSrcStartIdx = zeroMJBuilderState.curSrcStartIdx
					}
					o.builderState.right.numRepeatsIdx = zeroMJBuilderState.numRepeatsIdx
				}
				o.builderState.right.groupsIdx = zeroMJBuilderState.groupsIdx
			} else {
				// Loop over every group.
				for ; o.builderState.right.groupsIdx < groupsLen; o.builderState.right.groupsIdx++ {
					rightGroup := rightGroups[o.builderState.right.groupsIdx]
	...
	o.resetBuilderCrossProductStateForSide(&o.builderState.right)
}
