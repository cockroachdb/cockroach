...
func BenchmarkDeselector(b *testing.B) {
	...
	batch := col.NewMemBatch(inputTypes)
	...
	for _, probOfOmitting := range []float64{0.1, 0.9} {
		sel, batchLen := generateSelectionVector(col.BatchSize, probOfOmitting)

		for _, nBatches := range []int{1 << 1, 1 << 2, 1 << 4, 1 << 8} {
			b.Run(fmt.Sprintf("rows=%d/after selection=%d", nBatches*col.BatchSize, nBatches*int(batchLen)), func(b *testing.B) {
