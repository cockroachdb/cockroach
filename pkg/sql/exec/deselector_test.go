	...
	for _, nBatches := range []int{1 << 1, 1 << 2, 1 << 4, 1 << 8} {
		b.Run(fmt.Sprintf("rows=%d/after selection=%d", nBatches*ColBatchSize, nBatches*int(batchLen)), func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				...
				// We don't need to reset anything here because input is returning the
				// same batch forever and deselector doesn't keep any state. We do,
				// however, want to keep already allocated memory by deselector so that
				// this memory allocation doesn't impact the benchmark.
			}
			b.StopTimer()
		})
	}
