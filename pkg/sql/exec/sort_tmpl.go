...
type sort_TYPE_DIROp struct {
	sortCol []_GOTYPE
	nulls   *coldata.Nulls
	// nullOffset holds onto the physical offset the nulls vector
	// which is used to index into nulls correctly when partitions are sorted.
	nullOffset    uint64
	order         []uint64
	workingSpace  []uint64
...
// Nulls always sort first, so we encode that logic here.
