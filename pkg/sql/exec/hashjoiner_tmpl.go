...
func _COLLECT_RIGHT_OUTER(
	prober *hashJoinProber, batchSize uint16, nResults uint16, batch col.Batch, _SEL_STRING string,
) uint16 { // */}}
...
func (ht *hashTable) rehash(
	buckets []uint64, keyIdx int, t types.T, col col.Vec, nKeys uint64, sel []uint16,
) {
	switch t {
...
func (prober *hashJoinProber) collect(batch col.Batch, batchSize uint16, sel []uint16) uint16 {
	nResults := uint16(0)
...
func (prober *hashJoinProber) distinctCollect(
	batch col.Batch, batchSize uint16, sel []uint16,
) uint16 {
	nResults := uint16(0)
