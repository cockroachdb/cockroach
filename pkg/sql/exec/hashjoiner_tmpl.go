...
func _REHASH_BODY(
	ctx context.Context, buckets []uint64, keys []interface{}, nKeys uint64, _SEL_STRING string,
) { // */}}
	// {{define "rehashBody"}}
	for i := uint64(0); i < nKeys; i++ {
		if nKeys&(1<<6-1) == 1<<6-1 {
...
func (ht *hashTable) rehash(
	...
	case _TYPES_T:
		...
		if sel != nil {
			_REHASH_BODY(ctx, buckets, keys, nKeys, "sel[i]")
		} else {
			_REHASH_BODY(ctx, buckets, keys, nKeys, "i")
		}
