...
// buffer.
func FileDeserializerFromBytes(buf []byte) (*FileDeserializer, error) {
...
func FileDeserializerFromPath(path string) (*FileDeserializer, error) {
	...
	// TODO(dan): While converting arrow data into coldata.Batch, we modify the
	// underlying bytes for the nulls. This means we have to mmap in a way that
	// allows mutating memory. If we mmap'd as MAP_SHARED, then any writes that
	// make it to other processes or the disk would mess up the nulls, since
	// they'd be double-migrated. So instead we mmap as MAP_PRIVATE with
	// PROT_WRITE, which lets us continue to open the file as os.O_RDONLY. This
	// makes the OS keep track of our writes though, so it sure would be nice if
	// we could make this conversion entirely read-only. Uncomment the block below
	// this if we do that.
	buf, err := unix.Mmap(int(f.Fd()), 0, int(fi.Size()), unix.PROT_WRITE, unix.MAP_PRIVATE)
	if err != nil {
		return nil, pgerror.Wrapf(err, pgerror.CodeIoError, `mmaping %s`, path)
	}

	// Uncomment this block for read-only mmap if we are ever able to do that.
	//
	...
	return newFileDeserializer(buf, func() error { return unix.Munmap(buf) })
}
...
func (d *FileDeserializer) init() ([]types.T, error) {
	...
	if magic, err := d.read(8); err != nil {
		return nil, pgerror.Wrap(err, pgerror.CodeDataExceptionError, `verifying arrow file header magic`)
	...
	if magic, err := d.readBackward(len(fileMagic)); err != nil {
		return nil, pgerror.Wrap(err, pgerror.CodeDataExceptionError, `verifying arrow file footer magic`)
	...
	if err != nil {
		return nil, pgerror.Wrap(err, pgerror.CodeDataExceptionError, `reading arrow file footer`)
	}
