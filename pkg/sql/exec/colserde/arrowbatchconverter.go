...
func (c *ArrowBatchConverter) BatchToArrow(batch coldata.Batch) ([]*array.Data, error) {
	...
	for i, typ := range c.typs {
		...
		if vec.HasNulls() {
			...
			arrowBitmapHeader.Len = vecBitmapHeader.Len * sizeOfInt64
			arrowBitmapHeader.Cap = vecBitmapHeader.Cap * sizeOfInt64
		}
...
func (c *ArrowBatchConverter) ArrowToBatch(data []*array.Data) (coldata.Batch, error) {
	...
	for i, typ := range c.typs {
		...
		if typ == types.Bool || typ == types.Bytes {
			switch typ {
			case types.Bool:
				boolArr := array.NewBooleanData(data[i])
				vecArr := vec.Bool()
			...
			case types.Bytes:
				...
				if bytes == nil {
					...
					// corresponds.
					// TODO(asubiotto): Will this break if the original value *was* nil? I
					// think this is technically impossible, right?
					bytes = make([]byte, 0)
				}
		...
		if arrowBitmap != nil {
			...
			vecBitmapHeader.Len = arrowBitmapHeader.Len / sizeOfInt64
			vecBitmapHeader.Cap = arrowBitmapHeader.Cap / sizeOfInt64

			vec.SetNullBitmap(vecBitmap)
		}
	}
	// Assume > 0 length data.
	c.scratch.batch.SetLength(uint16(data[0].Len()))
