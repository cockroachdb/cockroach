...
func TestArrowBatchConverterRandom(t *testing.T) {
	...
	typs := exec.RandomTypes(rng, rng.Intn(maxTyps)+1)
	for {
		foundDecimal := false
...
func BenchmarkArrowBatchConverter(b *testing.B) {
	...
	for i, typ := range typs {
		...
		setNullFraction := func(batch coldata.Batch, nullFraction float64) {
			...
			for i, nullsSet := uint16(0), 0; i < batch.Length() && nullsSet < numNulls; i, nullsSet = i+1, nullsSet+1 {
				vec.SetNull(i)
			}
		}
		...
		for _, nullFraction := range nullFractions {
			...
			b.Run(testPrefix+"/ArrowToBatch", func(b *testing.B) {
				b.SetBytes(numBytes[i])
				for i := 0; i < b.N; i++ {
					result, err := c.ArrowToBatch(data)
					require.NoError(b, err)
					if result.Width() != 1 {
