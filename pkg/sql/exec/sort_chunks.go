...
func (c *sortChunksOp) Next() ColBatch {
	for {
		batch := c.sorter.Next()
		if batch.Length() == 0 {
			if c.input.readFrom == inputDone {
				// We're done, so return a zero-length batch.
				return batch
			}
			// We're not yet done - need to process another chunk. We don't need to
			// explicitly reset the chunker because it will be reset by the sorter
			// as the sorter's input.
			if r, ok := c.sorter.(resetter); ok {
				r.reset()
			} else {
...
// readFrom indicates where the spooler needs to read tuples from for emitting.
type readFrom int

const (
	// readFromBuffer indicates that the tuples need to be read from the buffer.
	readFromBuffer = iota
	// readFromBatch indicates that the tuples need to be read from the last read
	// batch directly. Only tuples that are fully contained within the last read
	// batch are "emitted".
	readFromBatch
...
// vector, so it always puts a deselector on top of its input. It does the
// coalescing itself, so it does not use an extra coalescer.
type chunker struct {
	input Operator
...
func (s *chunker) prepareNextChunks() readFrom {
	for {
		switch s.state {
		case chunkerReading:
...
func (s *chunker) getValues(i int) ColVec {
	...
	default:
		panic(fmt.Sprintf("unexpected readFrom in getValues: %v", s.state))
	}
}
...
func (s *chunker) getNumTuples() uint64 {
	...
	case inputDone:
		return 0
	default:
		panic(fmt.Sprintf("unexpected readFrom in getNumTuples: %v", s.state))
	}
}
...
func (s *chunker) getPartitionsCol() []bool {
	...
	default:
		panic(fmt.Sprintf("unexpected readFrom in getPartitionsCol: %v", s.state))
	}
}

func (s *chunker) reset() {
	// We only need to set s.buffered to 0 to empty the buffer. Note that we
	// intentionally do not reset the input to the chunker or the chunker state
	// as well since we're likely in the middle of processing the input.
	s.buffered = 0
}
