...
func (s *chunker) prepareNextChunks() chunkerReadingState {
	for {
		...
		case chunkerReading:
			...
			for partitionerIdx, orderedCol := range s.alreadySortedCols {
				s.partitioners[partitionerIdx].partition(s.batch.ColVec(int(orderedCol.ColIdx)), s.partitionCol, uint64(s.batch.Length()))
			}
