...
func (a *sumInt64Agg) next() (int, ColBatch) {
	if a.nextBatch != nil {
...
func (a *sumInt64Agg) Next() ColBatch {
	...
	for i := uint16(0); i < a.outputBatchSize; i++ {
		a.scratch.vec[i] = 0
	}
	for curIdx < int(a.outputBatchSize) {
		...
		if curIdx == -1 {
			...
			// TODO(asubiotto): Modifying the input batch is probably not cool.
			if groups[firstIdx] {
		...
		// TODO(asubiotto): Can this be improved? A fresh perspective might help.
		toSum := int64(0)
		if sel != nil {
			for ; resumeIdx < len(sel) && curIdx < int(a.outputBatchSize); resumeIdx++ {
				i := sel[resumeIdx]
		...
		if curIdx == int(a.outputBatchSize) {
			...
			// a.scratch.curIdx is valid, flush the sum and loop again.
			a.scratch.vec[curIdx] += toSum
		}
	}
