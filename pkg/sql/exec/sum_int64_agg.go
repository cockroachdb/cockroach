...
func (a *sumInt64Agg) SetOutputIndex(idx int) {
	if a.scratch.curIdx != -1 {
		...
		carry := a.scratch.vec[a.scratch.curIdx]
		copy(a.scratch.vec[idx:], zeroInt64Batch)
		a.scratch.vec[idx] = carry
	}
}
...
func (a *sumInt64Agg) Compute(b ColBatch, inputIdxs []uint32) {
	...
	if a.carry != 0 {
		a.scratch.vec[a.scratch.curIdx] = a.carry
		a.carry = 0
	}
	inputLen := b.Length()
	if inputLen == 0 {
	...
	a.carry = a.scratch.vec[a.scratch.curIdx]
}
