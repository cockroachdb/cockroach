...
// ownership is transferred to RunWithStream.
...
func (i *Inbox) RunWithStream(streamCtx context.Context, stream flowStreamServer) error {
	...
	case <-streamCtx.Done():
		return fmt.Errorf("%s: streamCtx while waiting for reader", streamCtx.Err())
	}
	...
	case <-readerCtx.Done():
		...
		return fmt.Errorf("%s: readerCtx in Inbox stream handler", readerCtx.Err())
	case <-streamCtx.Done():
		// The client canceled the stream.
		return fmt.Errorf("%s: streamCtx in Inbox stream handler", streamCtx.Err())
	}
}
