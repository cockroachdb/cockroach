...
// maybeInit call Inbox.init if the inbox is not initialized and returns whether
...
/*
Test io.EOF in DrainMeta send of drain signal.
*/
...
func (i *Inbox) DrainMeta(ctx context.Context) []distsqlpb.ProducerMetadata {
	...
	// TODO(asubiotto): Do we want to panic here? Is there a possibility that
	// graceful execution will happen even though we're canceled? Calling
	// DrainMeta first without calling Next will return no meta, but how is the
	// caller expected to check for a cancellation?
