...
// NOTE: i.streamMu *must* be held when calling this function.
...
func (i *Inbox) Next(ctx context.Context) coldata.Batch {
	...
	defer func() {
		...
		if err := recover(); err != nil {
			if !stateMuLocked {
				// The panic occurred while we were Recv'ing when we are holding
				// i.streamMu and are not holding i.stateMu.
				i.stateMu.Lock()
				i.streamMu.Unlock()
			}
			i.closeLocked()
...
func (i *Inbox) DrainMeta(ctx context.Context) []distsqlpb.ProducerMetadata {
	...
	if i.stateMu.initialized {
		...
		// It is safe to Send without holding the mutex.
