...
// TODO(yuzefovich): do we care if Next buffers more metadata after DrainMeta
// has been called? My understanding is that if DrainMeta has been called, it
// means that materializer is wrapping everything up and will be shutting down
// the flow shortly, so any metadata received after DrainMeta is dropped on the
// floor. I think it is an expected behavior because we don't want to hold the
// shut down until we're sure there is no more metadata coming if Consumer has
