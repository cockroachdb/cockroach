...
func NewOutbox(input exec.Operator, typs []types.T) (*Outbox, error) {
	...
	o := &Outbox{
		...
		// be). TODO(asubiotto): Do we want a coalescer too?
...
func (o *Outbox) Run(
	...
	// TODO(asubiotto): Hopefully the context will be annotated with the flowID
	// already, but the outbox should annotate this context with a streamID as
	...
	if err != nil {
		log.Errorf(ctx, "Outbox Dial error: %s", err)
		return
	}
	...
	if err != nil {
		log.Errorf(ctx, "Outbox FlowStream error: %s", err)
		return
	}
	...
	if err := stream.Send(
		...
		log.Errorf(ctx, "Outbox Send error: %s", err)
		return
	}
...
func (o *Outbox) runWithStream(
	...
	go func() {
		...
		for {
			...
			if err != nil {
				if err != io.EOF {
					log.Errorf(ctx, "Outbox Recv error: %s", err)
				}
				break
			}
		}
	...
	for {
		...
		if b.Length() == 0 {
			if err := stream.CloseSend(); err != nil {
				...
				log.Errorf(ctx, "Outbox CloseSend error: %s", err)
				return
			}
		...
		if err != nil {
			// TODO(asubiotto): Send this error as metadata.
			log.Errorf(ctx, "Outbox BatchToArrow error: %s", err)
			return
		}
		if err := o.serializer.Serialize(o.scratch.buf, d); err != nil {
			// TODO(asubiotto): Send this error as metadata.
			log.Errorf(ctx, "Outbox Serialize error: %s", err)
			return
		}
