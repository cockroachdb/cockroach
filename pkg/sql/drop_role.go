...
func (n *DropRoleNode) startExec(params runParams) error {
	...
	if err := forEachDatabaseDesc(params.ctx, params.p, nil /*nil prefix = all databases*/, true, /* requiresPrivileges */
		func(db catalog.DatabaseDescriptor) error {
			...
			accumulateDependentDefaultPrivileges(db.GetDefaultPrivilegeDescriptor(), userNames)
			return nil
		}); err != nil {
		return err
	}
	...
	for _, schemaDesc := range lCtx.schemaDescs {
		...
		accumulateDependentDefaultPrivileges(schemaDesc.GetDefaultPrivilegeDescriptor(), userNames)
	}
