...
func (n *DropRoleNode) startExec(params runParams) error {
	...
	if err := forEachDatabaseDesc(params.ctx, params.p, nil /*nil prefix = all databases*/, true, /* requiresPrivileges */
		func(db catalog.DatabaseDescriptor) error {
			...
			accumulateDependentDefaultPrivileges(db, userNames)
			return nil
		}); err != nil {
		return err
	}
...
func accumulateDependentDefaultPrivileges(
	...
	if db.GetDefaultPrivileges() != nil {
		addDependentPrivileges := func(object tree.AlterDefaultPrivilegesTargetObject, defaultPrivs descpb.PrivilegeDescriptor, role descpb.AlterDefaultPrivilegesRole) {
		...
		for object, defaultPrivs := range db.GetDefaultPrivileges().DefaultPrivilegesForAllRoles {
			addDependentPrivileges(object, defaultPrivs, descpb.AlterDefaultPrivilegesRole{
				ForAllRoles: true,
			})
		}
		for _, defaultPrivilegesPerRole := range db.GetDefaultPrivileges().DefaultPrivileges {
			for object, defaultPrivs := range defaultPrivilegesPerRole.DefaultPrivilegesPerObject {
				addDependentPrivileges(object, defaultPrivs, descpb.AlterDefaultPrivilegesRole{
					Role: defaultPrivilegesPerRole.UserProto.Decode(),
				})
			}
		}
	}
