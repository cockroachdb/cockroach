...
// addEqualNotNullConj loops over the left/rightEqualityIndices to construct
// not-null constraint expressions.
// For Inner Join, both left and right column should fetch not-null values.
// For Left Join, right column should fetch not-null values.
// For Right Join, left column should fetch not-null values.
...
func addEqualNotNullConj(n *joinNode) tree.TypedExpr {
	...
	if n.joinType == joinTypeInner || n.joinType == joinTypeRightOuter {
		for _, c := range n.pred.leftEqualityIndices {
			...
			leftNotNull := tree.NewTypedComparisonExpr(tree.IsDistinctFrom, leftIndexedVar, tree.DNull)
			result = mergeConj(result, leftNotNull)
		}
	}

	// right equality not null conj
	if n.joinType == joinTypeInner || n.joinType == joinTypeLeftOuter {
		for _, c := range n.pred.rightEqualityIndices {
			...
			rightNotNull := tree.NewTypedComparisonExpr(tree.IsDistinctFrom, rightIndexedVar, tree.DNull)
