...
func TestSensitiveSettingFailedSetErrorRecording(t *testing.T) {
	...
	// On this branch there is no way to make a secret-carrying statement fail
	// during execution: no setting that is Sensitive here has a validating
	// hook (server.host_based_authentication.configuration, which master uses,
	// is not Sensitive on this branch), and failures during planning are not
	// recorded in statement statistics or execution insights at all. The
	// recording sinks under test are shared with newer branches, where this
	// test runs; the log surface is covered on this branch by
	// TestSensitiveSettingFailedSetLogRedaction.
	skip.IgnoreLint(t, "no execution-phase failure fixture for a sensitive setting on this branch")
...
func TestSensitiveSettingFailedSetLogRedaction(t *testing.T) {
	...
	// Note: master exercises this with an HBA config validation error, but
	// server.host_based_authentication.configuration is not Sensitive on this
	// branch. A failed cast in the value expression of a SET targeting a
	// sensitive setting echoes the value the same way.
	...
	for _, qe := range spy.GetLogs(logpb.Channel_SQL_EXEC) {
		...
		switch {
		case strings.Contains(stmt, "oidc_authentication.client_secret"):
