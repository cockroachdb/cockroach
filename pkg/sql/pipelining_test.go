...
func assertRunningCount(t *testing.T, pq *PipelineQueue, exp int) {
	if running := len(pq.running); running != exp {
		t.Errorf("expected running plan count of %d, found %d", exp, running)
	}
}

func assertPendingCount(t *testing.T, pq *PipelineQueue, exp int) {
	if pending := pq.pending.Len(); pending != exp {
		t.Errorf("expected pending plan count of %d, found %d", exp, pending)
	}
}

func waitAssertEmptyWithErr(t *testing.T, pq *PipelineQueue) error {
	err := pq.Wait()
	if l := pq.Len(); l != 0 {
...
func waitAndAssertEmpty(t *testing.T, pq *PipelineQueue) {
	if err := waitAssertEmptyWithErr(t, pq); err != nil {
...
func TestPipelineQueueNoDependencies(t *testing.T) {
	...
	pq.Add(newPlanNode(), func() error {
		...
		assertRunningCount(t, &pq, 3)
		assertPendingCount(t, &pq, 0)
		close(run3)
		return nil
	})
	pq.Add(newPlanNode(), func() error {
...
func TestPipelineQueueAllDependent(t *testing.T) {
	defer leaktest.AfterTest(t)()
	var res []int
	analyzer := newDependencyAnalyzerFunc(func(p *planNode, set planNodeSet) bool {
		return len(set) == 0
	})
...
func TestPipelineQueueError(t *testing.T) {
	...
	resErr := waitAssertEmptyWithErr(t, &pq)
	if resErr != planErr {
