...
func (m *sessionDataMutator) SaveTemporarySchemaID(dbID uint32, tempSchemaID uint32) {
	if m.data.DbToTempSchemaMap == nil {
		m.data.DbToTempSchemaMap = make(map[uint32]uint32)
	}
	m.data.DbToTempSchemaMap[dbID] = tempSchemaID
}
