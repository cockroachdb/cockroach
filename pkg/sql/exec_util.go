...
func (m *sessionDataMutator) SetRole(ctx context.Context, pc permissionsChecker, s string) error {
	...
	if s == security.NoneRole && m.data.SessionUserProto == "" && pc == nil {
		return nil
	}
	...
	if s != security.NoneRole {
		var err error
		becomeUser, err = security.MakeSQLUsernameFromUserInput(s, security.UsernameValidation)
		if err != nil {
			return err
		}
