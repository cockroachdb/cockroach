...
func (p *planner) EvalAsOfTimestamp(
	ctx context.Context, asOfClause tree.AsOfClause, opts ...asof.EvalAsOfTimestampOption,
) (tree.AsOfSystemTime, error) {
	asOf, err := asof.EvalAsOfTimestamp(ctx, asOfClause, &p.semaCtx, p.EvalContext(), opts...)
	if err != nil {
...
func (p *planner) isAsOf(ctx context.Context, stmt tree.Statement) (*tree.AsOfSystemTime, error) {
	...
	asOfRet, err := p.EvalAsOfTimestamp(ctx, asOf, asof.EvalAsOfTimestampOptionAllowBoundedStaleness)
	if err != nil {
		return nil, err
	}
	return &asOfRet, err
