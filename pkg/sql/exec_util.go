...
type SessionRegistry struct {
	...
	sessionsByBackendKeyData map[pgwirecancel.BackendKeyData]registrySession
}
...
func NewSessionRegistry() *SessionRegistry {
	return &SessionRegistry{
		...
		sessionsByBackendKeyData: make(map[pgwirecancel.BackendKeyData]registrySession),
	}
}

func (r *SessionRegistry) register(
	id ClusterWideID, backendKeyData pgwirecancel.BackendKeyData, s registrySession,
) {
	r.Lock()
	r.sessions[id] = s
	r.sessionsByBackendKeyData[backendKeyData] = s
	r.Unlock()
}

func (r *SessionRegistry) deregister(id ClusterWideID, backendKeyData pgwirecancel.BackendKeyData) {
	r.Lock()
	delete(r.sessions, id)
	delete(r.sessionsByBackendKeyData, backendKeyData)
	r.Unlock()
}
...
func (r *SessionRegistry) CancelQueryByPGWire(
	...
	r.Lock()
	defer r.Unlock()
	if session, ok := r.sessionsByBackendKeyData[backendKeyData]; ok {
		if session.cancelCurrentQueries() {
			return true, nil
		}
		return false, nil
	}
	return false, fmt.Errorf("session for BackendKeyData %d not found", backendKeyData)
}
