...
type TTLTestingKnobs struct {
	...
	// BeforeProcessorStart is called before the TTL select-delete loop starts.
	// This can be used for test synchronization. The context is the job's
	// context, which will be canceled when the job is canceled. Return an
	// error to abort the job (e.g., ctx.Err() when context is canceled).
