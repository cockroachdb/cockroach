...
func expSmoothQPS(queryCounts []QueryCountAndTime, alpha float64, rollingInterval int) int64 {
	// If the number of query counts is less than the interval provided, default to 0.
	if len(queryCounts) < rollingInterval {
		return 0
	}
	...
	for i := 1; i < len(queryCounts); i++ {
		curr := queryCounts[i]
		prev := queryCounts[i-1]
	...
	for i := last; i >= 0; i-- {
		...
		if i == last {
			smoothQPS += alpha * qpsVal
		} else {
			exp := float64(last - i)
			smoothQPS += alpha * math.Pow(1-alpha, exp) * qpsVal
		}
	}
