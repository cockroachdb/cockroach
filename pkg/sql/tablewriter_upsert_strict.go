...
func (tu *strictTableUpserter) getConflictingRows(
	...
	for i := 0; i < tu.insertRows.Len(); i++ {
		...
		if _, ok := seenKeys[string(upsertRowPK)]; ok {
			...
			if traceKV {
				// Report the request to the trace if tracing is requested.
				// We log the request ID so that we can match the responses below.
				var buf bytes.Buffer
				buf.WriteString("Get (")
				comma := ""
				for _, colID := range tableDesc.PrimaryIndex.ColumnIDs {
					buf.WriteString(comma)
					buf.WriteString(row[tu.ri.InsertColIDtoRowIndex[colID]].String())
					comma = ", "
				}
				buf.WriteByte(')')
				log.VEventfDepth(ctx, 1, 2, "%s", buf.String())
			}
		...
		for resIdx, result := range b.Results {
			row := result.Rows[0]
			...
			if row.Value != nil {
				conflictingRows[i] = struct{}{}
				if traceKV {
					log.VEventfDepth(ctx, 1, 2, "Get response %d found row", resIdx)
				}
			}
		}
