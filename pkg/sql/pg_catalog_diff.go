...
// PGCatalogColumns maps column names with datatype description
...
func (p PGCatalogTables) AddColumnMetadata(
	...
	columns[columnName] = &PGCatalogColumn{dataTypeOid, dataType, nil, nil}
}

// addDiff is for the second use case for pgTables which objective is create a datatype diff
func (p PGCatalogTables) addDiff(
	tableName string, columnName string, expected *PGCatalogColumn, got *PGCatalogColumn,
) {
	columns, ok := p[tableName]
	...
	columns[columnName] = &PGCatalogColumn{
		Oid:         got.Oid,
