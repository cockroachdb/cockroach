...
func NewLogicalAccessor(
	physical catalog.SchemaAccessor, vt catalog.VirtualSchemas,
) *LogicalSchemaAccessor {
	return &LogicalSchemaAccessor{
		SchemaAccessor: physical,
		vt:             vt,
	}
}
...
type LogicalSchemaAccessor struct {
	...
	vt catalog.VirtualSchemas

	// Used to avoid allocations.
	tn tree.TableName
}

var _ catalog.SchemaAccessor = &LogicalSchemaAccessor{}
...
func (l *LogicalSchemaAccessor) IsValidSchema(
	...
	if _, ok := l.vt.GetVirtualSchema(scName); ok {
...
) (catalog.TableNames, error) {
	if entry, ok := l.vt.GetVirtualSchema(scName); ok {
		names := make(catalog.TableNames, 0, entry.NumTables())
...
func (l *LogicalSchemaAccessor) GetObjectDesc(
	...
	case tree.TableObject:
		...
		if scEntry, ok := l.vt.GetVirtualSchema(schema); ok {
