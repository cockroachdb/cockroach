...
func (m *Manager) WaitForInitialVersion(
	...
	for lastCount, r := 0, retry.Start(retryOpts); r.Next(); {
		descs, err := m.maybeGetDescriptorsWithoutValidation(ctx, ids.Ordered())
		if err != nil {
			if errors.Is(err, catalog.ErrDescriptorNotFound) {
				log.Infof(ctx, "descriptor %v was dropped before initial leases was acquired", ids)
				return nil
			}
			return err
		}
		...
		for schemaID, descIDsForSchema := range idsPerSchema {
			...
			if count != lastCount {
				log.Infof(ctx, "waiting for descriptors %v to appear on %d nodes. Last count was %d", descIDsForSchema, expectedSessions, lastCount)
...
func (m *Manager) WaitForOneVersion(
	...
	for lastCount, r := 0, retry.Start(retryOpts); r.Next(); {
		...
		if descArr, err = m.maybeGetDescriptorsWithoutValidation(ctx, descpb.IDs{id}); err != nil {
			return nil, err
		}
		desc = descArr[0]
		// Check to see if there are any leases that still exist on the previous
