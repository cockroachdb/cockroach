...
func (s storage) acquire(
	...
	err := s.db.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
		...
		desc, err := catalogkv.GetDescriptorByID(ctx, txn, s.codec, id, catalogkv.Immutable, catalog.Any, true /* required */)
		if err != nil {
			return err
		}
...
func (m *Manager) WaitForOneVersion(
	...
	for lastCount, r := 0, retry.Start(retryOpts); r.Next(); {
		...
		if err := m.DB().Txn(ctx, func(ctx context.Context, txn *kv.Txn) (err error) {
			desc, err = catalogkv.GetDescriptorByID(ctx, txn, m.Codec(), id, catalogkv.Immutable, catalog.Any, true /* required */)
			return err
		}); err != nil {
			return 0, err
		}
