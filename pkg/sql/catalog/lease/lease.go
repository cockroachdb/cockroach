...
func (m *Manager) WaitForInitialVersion(
	...
	for lastCount, r := 0, retry.Start(retryOpts); r.Next(); {
		...
		if err := db.Txn(ctx, func(ctx context.Context, txn isql.Txn) error {
			...
			// We are going to gather schema leases on a per-region basis, so that we can
			// support region liveness. This will allow us to detect down regions in
			// multi-region serverless clusters, and skip rows corresponding to them.
		...
		if err := db.Txn(ctx, func(ctx context.Context, txn isql.Txn) error {
			...
			if regions != nil && regions.IsMultiRegion() {
				return regionMap.ForEach(func(region string) error {
					...
					queryRegionRows := func(ctx context.Context) error {
...
SELECT count(*)
 ...
 WHERE desc_id = %d
       ...
       AND crdb_region = '%s';`,
				...
				// Otherwise, we can query the intitial versions directly.
				row, err := txn.QueryRow(ctx, "wait-for-new-descriptor", txn.KV(),
					fmt.Sprintf(
						`
SELECT count(*)
		...
		storage: storage{
			...
			leasingMetrics: leasingMetrics{
				...
				longWaitForInitialVersionActive: metric.NewGauge(metric.Metadata{
					...
					Help:        "The number of wait for initial version taking more than the lease duration.",
