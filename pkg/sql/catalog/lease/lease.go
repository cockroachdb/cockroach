...
func (m *Manager) getDescriptorsInTxnAtTimestamp(
	...
	// We are going to export the entire descriptor table at timestamp, which will
	// fetch only the changed descriptors in the system.descriptor table. This export
	// request should be fairly inexpensive even with a large number of descriptors,
	...
	for {
		...
		req := &kvpb.ExportRequest{
			RequestHeader: requestHeader,
			// Only fetch the latest value at timestamp.
