...
var LeaseMonitorRangeFeedResetTime = settings.RegisterDurationSetting(
	settings.ApplicationLevel,
	"sql.catalog.descriptor_lease_monitor_range_feed_reset_time",
...
func (m *Manager) checkRangeFeedStatus(ctx context.Context) {
	...
	if lastCheckpoints == 0 {
		...
		// Start the timer only on the first unavalability event.
...
func (m *Manager) handleRangeFeedAvailability(ctx context.Context) error {
	...
	if resetAvailableTime {
		...
		log.Warning(ctx, "attempting restart leasing range feed")
