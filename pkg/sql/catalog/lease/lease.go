...
type Manager struct {
	...
	assertAllLeasesReleased atomic.Bool
...
func NewLeaseManager(
	...
	lm.stopper.AddCloser(stop.CloserFn(lm.AssertAllLeasesAreReleased))
...
func (m *Manager) SetDraining(
	...
	m.assertAllLeasesReleased.Store(assertOnLeakedDescriptor)
	if !drain {
		return
	}
...
// AssertAllLeasesAreReleased asserts that all leases are released.
func (m *Manager) AssertAllLeasesAreReleased() {
	if !m.assertAllLeasesReleased.Load() {
		return
	}
	m.mu.Lock()
	defer m.mu.Unlock()
