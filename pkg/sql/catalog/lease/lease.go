...
func (m *Manager) WaitForOneVersion(
	...
	for lastCount, r := 0, retry.Start(retryOpts); r.Next(); {
		...
		if err := m.DB().Txn(ctx, func(ctx context.Context, txn *kv.Txn) (err error) {
			desc, err = catalogkv.GetDescriptorByID(ctx, txn, m.Codec(), id)
			return err
		}); err != nil {
			return 0, err
		}
		...
		err := m.storage.db.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
			...
			for _, id := range ids {
				...
				if expectedVersions[id] != descsToUpdate[id].GetVersion() {
					...
					//if log.V(3) {
					log.Infof(ctx, "publish %d (version changed): %d != %d", id, expectedVersions[id], descsToUpdate[id].GetVersion())
					//}
...
func (m *Manager) watchForRangefeedUpdates(
	...
	handleEvent := func(ev *roachpb.RangeFeedValue) {
		...
		id, name, version, _, _ := sqlbase.GetDescriptorMetadata(&descriptor)
		if log.V(2) {
