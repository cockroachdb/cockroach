...
// hasDescUpdateBeenApplied checks if a descriptor update has been applied.
...
func (m *Manager) processDescriptorUpdate(
	...
	noEntriesLeft := func() bool {
		...
		for idx, descID := range entry.DescriptorIDs {
			if descID != id {
				continue
			}
			// The target version has not arrived yet for this update.
			if entry.DescriptorVersions[idx] > version {
				// Note: This should never happen.
				return false
			}
			// Otherwise, let's remove this descriptor.
			entry.DescriptorIDs = append(entry.DescriptorIDs[:idx], entry.DescriptorIDs[idx+1:]...)
			entry.DescriptorVersions = append(entry.DescriptorVersions[:idx], entry.DescriptorVersions[idx+1:]...)
			break
		}
		return len(entry.DescriptorIDs) == 0
	}()
...
func (m *Manager) markDescriptorUpdatesAsComplete() hlc.Timestamp {
	...
	for {
		...
		removeEntry := func() bool {
			minUpdate.mu.Lock()
			defer minUpdate.mu.Unlock()
			return len(minUpdate.DescriptorIDs) == 0
		}()
		if !removeEntry {
			break
		}
	...
	_ = s.RunAsyncTask(ctx, "refresh-leases", func(ctx context.Context) {
		for {
			...
			case descUpdate := <-m.descMetaDataUpdateCh:
				...
				allApplied, remaining := m.hasDescUpdateBeenApplied(descUpdate.DescriptorUpdates)
				descUpdate.DescriptorUpdates = remaining
...
func (m *Manager) watchForUpdates(ctx context.Context) {
	...
	handleEvent := func(
		...
		if len(ev.Value.RawBytes) == 0 {
			...
			return
		} else if isUpdateKey, err := m.Codec().DecodeDescUpdateKey(ev.Key); err != nil || isUpdateKey {
			if err != nil {
				log.Dev.Infof(ctx, "unable to decode update metadata key %v", ev.Key)
				return
			}
			...
			if err != nil {
				log.Dev.Infof(ctx, "unable to decode descriptor update value %v", err)
				return
			}
			select {
			case <-ctx.Done():
			case m.descMetaDataUpdateCh <- &descriptorTxnUpdate{
				key:               ev.Key,
				DescriptorUpdates: descUpdates,
				timestamp:         ev.Timestamp(),
			}:
			}
			return
		}
		// Check if its the special descrip
...
func (m *Manager) cleanupUpdateKeys(ctx context.Context) {
	...
	if err != nil {
		log.Dev.Infof(ctx, "unable to delete update keys from storage %s", err)
	}
}
