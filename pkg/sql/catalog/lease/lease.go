...
func (m *Manager) deleteOrphanedLeasesFromStaleSession(
	...
	row, err := exec.QueryRow(ctx, "is-system-db-multi-region", nil,
		"SELECT EXISTS (SELECT * FROM [SHOW REGIONS FROM DATABASE system])")
	if err != nil {
		log.Warningf(ctx, "unable to query if system database is multi-region: %s", err)
		return
	}
	// For multi-region system databases, only focus on our own
	// region.
	...
	// table.
	const DistinctSessionQuery = `SELECT DISTINCT(session_id) FROM system.lease AS OF SYSTEM TIME %s WHERE crdb_region=$1`
	...
	err = exec.WithSyntheticDescriptors(syntheticDescriptors, func() error {
		var err error
		distinctSessions, err = exec.QueryBuffered(ctx,
			"query-lease-table-sessions",
			nil,
			fmt.Sprintf(DistinctSessionQuery,
				aostTime.AsOfSystemTime()),
			region,
		)
		return err
	})
	...
	ctxGrp.GoCtx(func(ctx context.Context) error {
		...
		const DeleteOrphanedSessions = `DELETE FROM system.lease WHERE session_id=$1 AND crdb_region=$2`
		for sessionID := range cleanupCh {
			for {
				var rowsModified int
				if err := exec.WithSyntheticDescriptors(syntheticDescriptors, func() error {
					var err error
					rowsModified, err = exec.Exec(ctx, "delete-orphaned-leases-by-session", nil,
				...
				// Once no more rows are modified break out of this lop.
				if rowsModified == 0 {
					break
				}
			}
