...
func (m *Manager) processDescriptorUpdate(
	...
	noEntriesLeft := func() bool {
		...
		for idx, descID := range entry.mu.DescriptorIDs {
			...
			if idx != len(entry.mu.DescriptorIDs)-1 {
				entry.mu.DescriptorIDs[idx], entry.mu.DescriptorIDs[len(entry.mu.DescriptorIDs)-1] =
					entry.mu.DescriptorIDs[len(entry.mu.DescriptorIDs)-1], entry.mu.DescriptorIDs[idx]
				entry.mu.DescriptorVersions[idx], entry.mu.DescriptorVersions[len(entry.mu.DescriptorIDs)-1] =
					entry.mu.DescriptorVersions[len(entry.mu.DescriptorIDs)-1], entry.mu.DescriptorVersions[idx]
			}
