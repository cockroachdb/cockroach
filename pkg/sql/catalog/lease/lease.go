...
func (m *Manager) WaitForNoVersion(
	...
	incLongWaitCleanup := m.storage.incGaugeAfterLeaseDuration(m.storage.longWaitForNoVersionsActive)
	defer incLongWaitCleanup()
...
func (m *Manager) WaitForOneVersion(
	...
	incLongWaitCleanup := m.storage.incGaugeAfterLeaseDuration(m.storage.longWaitForOneVersionsActive)
	defer incLongWaitCleanup()
	for lastCount, r := 0, retry.Start(retryOpts); r.Next(); {
...
// IncTwoVersionWaitGaugeAfterLeaseDuration increments if a two version invariant
// wait exceeds the lease duration.
func (m *Manager) IncTwoVersionWaitGaugeAfterLeaseDuration() func() {
	return m.storage.incGaugeAfterLeaseDuration(m.storage.longTwoVersionInvariantViolationWaitActive)
}
