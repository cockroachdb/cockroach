...
func (m *Manager) checkRangeFeedStatus(ctx context.Context) {
	...
	if lastCheckpoints == 0 {
		...
		if !m.mu.rangeFeedIsUnavailable {
			// Note: This will be cleared with a full refresh inside
			// handleRangeFeedAvailability.
	...
	} else if m.mu.rangeFeedIsUnavailable {
	...
	_ = m.stopper.RunAsyncTask(ctx, "lease-refresher", func(ctx context.Context) {
		...
		for {
			...
			case <-refreshTimer.C:
				...
				forceRefreshAll, err := m.handleRangeFeedAvailability(ctx)
				if err != nil {
					log.Infof(ctx, "error handling range feed avalibility issue: %s", err)
				}
				...
				if (!m.sessionBasedLeasingModeAtLeast(ctx, SessionBasedDrain) &&
					!renewalsDisabled) ||
					forceRefreshAll {
					m.refreshSomeLeases(ctx, forceRefreshAll /*refreshAll*/)
				}
			}
		}
	})
...
// with the range feed and attempts restarts. Once availability has returned,
// then this function will indicate if a full refresh is needed.
func (m *Manager) handleRangeFeedAvailability(
	ctx context.Context,
) (refreshAllDescriptors bool, err error) {
	...
	if !m.mu.rangeFeedIsUnavailable && m.mu.rangeFeedIsUnavailableResetTimer.IsZero() {
		return
	}
	// If the range feed has lost availability and recovered, then do
	// a refresh of all descriptors to be safe.
	if m.mu.rangeFeedIsUnavailableResetTimer.IsZero() {
		m.mu.rangeFeedIsUnavailableResetTimer = time.Time{}
		log.Warningf(ctx, "refreshing all descriptors after range feed outage.")
		return true, nil
	} else if timeutil.Since(m.mu.rangeFeedIsUnavailableResetTimer) >=
	...
	// Once the range feed recovers, then we will force a manual
	// refresh of all descriptors to catch up.
	return false, nil
}
