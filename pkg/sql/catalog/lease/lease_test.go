...
func TestHistoricalExportRequestForTimeRange(t *testing.T) {
	...
	ts1, err := sql.ParseHLC(ts1Str)
	require.NoError(t, err)

	tdb.Exec(t, "ALTER SCHEMA sc RENAME TO sc2")
	tdb.Exec(t, "ALTER TABLE sc2.foo ADD COLUMN id UUID NOT NULL DEFAULT gen_random_uuid()")
	tdb.Exec(t, "ALTER TABLE sc2.foo RENAME COLUMN i TO former_id")
	tdb.Exec(t, "ALTER TABLE sc2.foo RENAME COLUMN id TO current_id")
	tdb.Exec(t, "CREATE TYPE status AS ENUM ('open', 'closed', 'inactive')")
	tdb.Exec(t, "ALTER TYPE status DROP VALUE 'inactive'")
	tdb.Exec(t, "ALTER TYPE status DROP VALUE 'open'")

	var ts2Str string
	tdb.QueryRow(t, "SELECT cluster_logical_timestamp()").Scan(&ts2Str)
	...
	manager := tc.Server(0).LeaseManager().(*lease.Manager)
	descriptorID := tableID.Load().(descpb.ID)
	_, err = manager.WaitForOneVersion(ctx, descriptorID, base.DefaultRetryOptions())
	require.NoError(t, err)
	historicalDescs, err := manager.TestingDescriptorExportRequest(ctx, descriptorID, ts1, ts2)
	assert.NoError(t, err)

	// Assert returned descriptors modification times are between ts1 and ts2 and the IDs match our query desc id
	for _, desc := range historicalDescs {
		modificationTime := desc.GetModificationTime()
		assert.Truef(t, ts1.Less(modificationTime), "ts1: %s, modification: %s", ts1.String(), modificationTime.String())
		assert.Truef(t, modificationTime.Less(ts2), "modification: %s, ts2: %s", modificationTime.String(), ts2.String())
		assert.Equalf(t, descriptorID, desc.GetID(), "(ID) Expected: %d, Got: %d", descriptorID, desc.GetID())
	}
}
