...
// a limitation if the transaction took longer then this
...
CREATE TABLE t1(val int);
	...
	t.Run("validate-lease-txn-deadline-ext", func(t *testing.T) {
		...
		// and let the lease duration expire before attempting
		// to insert. We should no longer see deadline errors
		// in this case.
		_, err = conn.Exec(`
BEGIN;
SELECT * FROM t1;
...
SELECT * FROM t1;
	`)
			...
			txnIDResult, err := conn.QueryContext(ctx, `SELECT id FROM crdb_internal.node_transactions WHERE session_id IN (SELECT * FROM [SHOW session_id]);`)
			if err != nil {
				waitChan <- err
				return
			}
			...
			require.NoError(t, err)
			txnIDResult.Close()
			// Inform the maine routine that it can cause an operation
			// to block us
			waitChan <- nil
			<-resumeChan
