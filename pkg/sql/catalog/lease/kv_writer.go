...
type kvWriter struct {
	...
	// TODO(jeffswenson): split this into a new and old inserter
...
func newKVWriter(codec keys.SQLCodec, db *kv.DB, id descpb.ID, settingsWatcher *settingswatcher.SettingsWatcher) *kvWriter {
	return &kvWriter{
		db:              db,
...
func (w *kvWriter) versionGuard(ctx context.Context, txn *kv.Txn) (settingswatcher.VersionGuard, error) {
...
func (w *kvWriter) do(ctx context.Context, txn *kv.Txn, lease leaseFields, addToBatch addToBatchFunc) error {
	run := (*kv.Txn).Run
