...
type countDetail struct {
	// The number of unexpired leases
	count int
			...
			err := txn.WithSyntheticDescriptors(descsToInject,
				func() error {
					...
					if (cachedDatabaseRegions != nil && cachedDatabaseRegions.IsMultiRegion()) ||
						...
						detail, err = countLeasesByRegion(ctx, txn, prober, regionMap, cachedDatabaseRegions, len(descsToInject) > 0, at, whereClause)
					} else {
...
crdb_region=$2 AND %[3]s`,
		getCountLeaseColumns(whereClauses[0]),
		at.AsOfSystemTime(),
...
func getCountLeaseColumns(whereClause string) string {
	...
	// Pick the correct version based on the columns used in WHERE clause.
	if strings.Contains(whereClause, "desc_id") {
