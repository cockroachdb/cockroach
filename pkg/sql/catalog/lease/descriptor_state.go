...
func (t *descriptorState) upsertLeaseLocked(
	...
	// so ensure that the new expiration is later. For session based
	// leases we are going to ignore this rule, since the expiration
	// could be shorter when linked to a session.
	if !s.mu.expiration.Less(expiration) && session == nil {
...
func (t *descriptorState) release(ctx context.Context, s *descriptorVersionState) {
	...
	maybeRemoveLease := func() *storedLease {
		...
		if l := maybeMarkRemoveStoredLease(s); l != nil {
			...
			// Force the release to happen synchronously, since releases are
			// asynchronous. This didn't matter with expiration based leases
			// since each renewal would have a different expiry. In the session
			// based model renewals will come in with the same session and
			// potentially end up with writes before deletes.
			if t.m.storage.testingKnobs.RemoveOnceDereferenced {
				res := make(chan struct{})
				releaseLeaseWithNotify(ctx, l, t.m, res)
				l = nil
				// Wait for the release before continuing.
				<-res
			}
			return l
		}
