...
func (t *descriptorState) release(ctx context.Context, s *descriptorVersionState) {
	...
	maybeRemoveLease := func() *storedLease {
		...
		if l := maybeMarkRemoveStoredLease(s); l != nil {
			...
			// Force the release to happen synchronously, when we force removals
			// for unit tests. This didn't matter with expiration based leases
			// since each renewal would have a different expiry (but the same version
			// in synthetic scenarios). In the session based model renewals will come
			// in with the same session ID, and potentially we can end up racing
			// with inserts and deletes on the storage side. For real world scenario,
			// this never happens because we release only if a new version exists.
