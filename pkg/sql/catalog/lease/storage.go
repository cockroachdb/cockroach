...
func (s storage) acquire(
	...
	acquireInTxn := func(ctx context.Context, txn *kv.Txn) (err error) {
		...
		// When the sessionID is set insert lease will upsert a new lease replacing any
		// existing ones. For session based leases we only need to refresh them if the
		// version has changed or if the previous lease was expiry based (i.e. the
		// sessionID was not set).
		if lastLease != nil &&
			lastLease.sessionID != nil &&
			descpb.DescriptorVersion(lastLease.version) == desc.GetVersion() {
			lf.sessionID = nil
			// This will be used by the release above in case of txn retry
			// errors.
			sessionID = nil
		}
