...
// only applicable to tables), an inactiveTableError is returned. A nil desc is
// returned if no newer version can be acquired.
func (s storage) acquire(
	...
	for r := retry.StartWithCtx(ctx, retry.Options{}); r.Next(); {
		...
		// If desc is nil then no new descriptor was avaialble to be leased.
