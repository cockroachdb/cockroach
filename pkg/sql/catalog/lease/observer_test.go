...
func TestLeaseObserver(t *testing.T) {
	...
	maxVersion := descpb.DescriptorVersion(1)
	grp.GoCtx(func(ctx context.Context) error {
		...
		for {
			...
			case v := <-obs.notified:
				...
				ld.Release(ctx)
				ld = newLease
				if ld.Underlying().GetVersion() != maxVersion {
					return errors.AssertionFailedf("expected version %d, got %d", maxVersion, ld.Underlying().GetVersion())
				}
	...
	require.Greaterf(t, maxVersion, descpb.DescriptorVersion(1), "new versions were not detected")
	// Confirm the observer stops firing after unregistering.
	lm.UnregisterLeaseObserver(obs)
