	...
	for _, tc := range []struct {
		...
		t.Run(fmt.Sprintf("test: %s", tc.name), func(t *testing.T) {
			...
			for size, expr := range tc.sqlExprs {
				...
				require.NoError(t, err)
				log.Infof(ctx, "%s", expr)
				descs[i] = desc
				i++
			}

			// Logging
			log.Info(ctx, "All descriptors sizes before comparison")
			for _, desc := range descs {
				log.Infof(ctx, "Descriptor %s with size %d", desc.GetName(), desc.Underlying().ByteSize())
			}
			...
			log.Infof(ctx, "Sorted Descriptors: %v", descs)
