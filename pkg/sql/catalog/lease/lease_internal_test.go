...
func TestLeaseManagerSessionIDChanges(t *testing.T) {
	...
	// Disable the range feed for this test to prevent race conditions between
	// the session ID hook being enabled / disabled. Previously, the initial acquisition of
	// the descriptor could be faster than the range feed, causing the range feed to
	// insert a lease with the real session ID (when the hook was disabled).
