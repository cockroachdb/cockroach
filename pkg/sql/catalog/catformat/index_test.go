...
func TestIndexForDisplay(t *testing.T) {
	...
	testData := []struct {
		...
		},
		{
			...
			partition:   "",
			displayMode: IndexDisplayDefOnly,
			expected:    "INDEX baz (a DESC) USING HASH WITH (bucket_count=8, shard_columns=(a))",
			pgExpected:  "INDEX baz USING btree (a DESC) USING HASH WITH (bucket_count=8, shard_columns=(a))",
		},
		{
			index:       shardedIndex,
			tableName:   tableName,
			partition:   "",
			displayMode: IndexDisplayShowCreate,
			expected:    "CREATE INDEX baz ON foo.public.bar (a DESC) USING HASH WITH (bucket_count=8, shard_columns=(a))",
			pgExpected:  "CREATE INDEX baz ON foo.public.bar USING btree (a DESC) USING HASH WITH (bucket_count=8, shard_columns=(a))",
		},
		...
		// so the output is valid SQL.
		{
			...
			partition:   "",
			displayMode: IndexDisplayDefOnly,
			expected:    "INDEX baz (a DESC) USING HASH STORING (c) WITH (bucket_count=8, shard_columns=(a))",
			pgExpected:  "INDEX baz USING btree (a DESC) USING HASH STORING (c) WITH (bucket_count=8, shard_columns=(a))",
		},
		{
			index:       shardedStoringIndex,
			tableName:   tableName,
