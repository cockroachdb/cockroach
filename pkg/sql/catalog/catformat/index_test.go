...
func TestIndexForDisplay(t *testing.T) {
	...
	// GEO INDEX baz (a)
	geoInvertedIndex := baseIndex
	geoInvertedIndex.Type = descpb.IndexDescriptor_INVERTED
	geoInvertedIndex.KeyColumnNames = []string{"a"}
	geoInvertedIndex.KeyColumnIDs = descpb.ColumnIDs{1}
	geoInvertedIndex.GeoConfig = *geoindex.DefaultGeometryIndexConfig()
	...
	testData := []struct {
		...
		{
			index:       geoInvertedIndex,
			tableName:   descpb.AnonymousTable,
			partition:   "",
			displayMode: IndexDisplayDefOnly,
			expected:    "INVERTED INDEX baz (a)",
			pgExpected:  "INDEX baz USING gist (a)",
		},
		{
			index:       geoInvertedIndex,
			tableName:   tableName,
			...
			displayMode: IndexDisplayShowCreate,
			expected:    "CREATE INVERTED INDEX baz ON foo.public.bar (a)",
			pgExpected:  "CREATE INDEX baz ON foo.public.bar USING gist (a)",
		},
		{
			index:       storingIndex,
			tableName:   descpb.AnonymousTable,
