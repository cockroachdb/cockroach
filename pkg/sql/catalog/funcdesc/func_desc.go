...
func (desc *immutable) ToTreeNode() (ret *tree.CreateFunction, err error) {
	...
	// We only store 4 function attributes at the moment. We may extend the
	// pre-allocated capacity in the future.
	ret.Options = make(tree.FunctionOptions, 0, 4)
	ret.Options = append(ret.Options, desc.getTreeNodeVolatility())
	ret.Options = append(ret.Options, tree.FunctionLeakproof(desc.LeakProof))
	ret.Options = append(ret.Options, desc.getTreeNodeNullInputBehavior())
