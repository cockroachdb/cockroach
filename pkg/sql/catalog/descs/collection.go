...
func (tc *Collection) hydrateTypesInTableDesc(
	...
	case *sqlbase.MutableTableDescriptor:
		...
		getType := func(ctx context.Context, id descpb.ID) (tree.TypeName, sqlbase.TypeDescriptorInterface, error) {
...
func (tc *Collection) HasUncommittedTables() bool {
	for _, desc := range tc.uncommittedDescriptors {
		if _, isTable := desc.immutable.(sqlbase.TableDescriptorInterface); isTable {
			return true
		}
	}
	return false
...
func (tc *Collection) HasUncommittedTypes() bool {
	for _, desc := range tc.uncommittedDescriptors {
		if _, isType := desc.immutable.(sqlbase.TypeDescriptorInterface); isType {
			return true
		}
	}
	return false
...
func (tc *Collection) GetAllDescriptors(
	...
	if tc.allDescriptors == nil {
		...
		if len(typDescs) > 0 {
			...
			typeLookup := func(ctx context.Context, id descpb.ID) (tree.TypeName, sqlbase.TypeDescriptorInterface, error) {
				typDesc := typDescs[id]
