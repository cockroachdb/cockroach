...
type Collection struct {
	...
	// monitor is injected to track memory usage of this Collection.
	// It will be further passed down to each descriptor set where we create an
	// account to track memory allocation/releases.
	// If the maker of this Collection does not provide a monitor, the Collection
	// factory's default, unlimited monitor will be used, which essentially turns
	// any downstream memory allocation/release calls to no-ops.
	monitor *mon.BytesMonitor
}
