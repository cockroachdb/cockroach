...
type Collection struct {
	...
	// maxTimestampBound is the maximum timestamp to read schemas at.
	// This is only set during the retries of bounded_staleness when
	// nearest_only=True, in which we want a schema read that should be
	// no older than maxTimestampBound.
...
func (tc *Collection) SetMaxTimestampBound(maxTimestampBound *hlc.Timestamp) {
	tc.maxTimestampBound = maxTimestampBound
}
