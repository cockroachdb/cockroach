...
func (tc *Collection) GetDatabaseByName(
	...
	// TODO (lucy): How does this even work? We're just returning the same
	// system table descriptor every time.
...
func (tc *Collection) getObjectByName(
	...
	// Resolve the schema.
	foundSchema, resolvedSchema, err := tc.GetSchemaByName(ctx, txn, dbID, name.Schema(),
	...
	if avoidCache {
		return tc.getDescriptorFromStore(
			ctx, txn, tc.codec(), dbID, schemaID, name.Object(), flags.RequireMutable)
	}

	desc, shouldReadFromStore, err := tc.getLeasedDescriptorByName(
		ctx, txn, dbID, schemaID, name.Object())
	if err != nil {
		return nil, false, err
	}
	if shouldReadFromStore {
...
func (tc *Collection) GetTableByName(
	...
	desc, found, err := tc.getObjectByName(ctx, txn, name, flags)
	if err != nil {
		return nil, err
	} else if !found {
		if flags.Required {
