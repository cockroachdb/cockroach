...
func (kd *kvDescriptors) getByID(
	...
	return kd.getDescriptor(ctx, txn, id, "" /* maybeLookedUpName */)
}
...
func (kd *kvDescriptors) getByName(
	...
	return kd.getDescriptor(ctx, txn, descID, name)
}

func (kd *kvDescriptors) getDescriptor(
	ctx context.Context, txn *kv.Txn, id descpb.ID, maybeLookedUpName string,
	...
	mut, err := catalogkv.MustGetMutableDescriptorByID(ctx, txn, kd.codec, id)
	if err != nil {
	...
	if withNameLookup {
		...
		// draining.
		if mut.GetName() != maybeLookedUpName {
			return nil, nil
		}
	}
	return mut, nil
