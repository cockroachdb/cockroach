...
func TestKVDescriptorsProperlyUsesMemoryMonitoring(t *testing.T) {
	...
	monitor := mon.NewMonitor("test_monitor1", mon.MemoryResource,
	...
	// Read all the descriptors into `col`.
	_, err1 := col.GetAllDescriptors(ctx, txn)

	// Assert this read finished without error.
	require.NoError(t, err1)

	// Clean up
	allocatedMemoryInBytes := monitor.MaximumBytes()
	col.ReleaseAll(ctx)
	monitor.Stop(ctx)

	// Change the monitor to a smaller budget (in fact, let's be bold by setting it to be only one byte below
	// what will be needed).
