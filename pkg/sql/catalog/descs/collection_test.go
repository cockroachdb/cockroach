...
func TestTxnClearsCollectionOnRetry(t *testing.T) {
	...
	err := cfg.DescsFactory.Txn(ctx, s.DB(), cfg.InternalExecutor, func(
...
func TestAddUncommittedDescriptorAndMutableResolution(t *testing.T) {
	...
	cfg := s0.ExecutorConfig().(sql.ExecutorConfig)
	ie := s0.InternalExecutor().(sqlutil.InternalExecutor)
	var dbID descpb.ID
	t.Run("database descriptors", func(t *testing.T) {
		require.NoError(t, cfg.DescsFactory.Txn(ctx, s0.DB(), ie, func(
			ctx context.Context, txn *kv.Txn, descriptors *descs.Collection,
		) error {
			...
			b := &kv.Batch{}
			b.CPut(catalogkeys.NewDatabaseKey(mut.Name).Key(cfg.Codec), mut.GetID(), nil)
			err = txn.Run(ctx, b)
			require.NoError(t, err)
...
`)
	...
	require.NoError(
		t,
		cfg.DescsFactory.Txn(ctx, kvDB, cfg.InternalExecutor, func(
