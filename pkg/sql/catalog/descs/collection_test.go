...
func TestSyntheticDescriptorResolution(t *testing.T) {
	...
	require.NoError(t, descs.Txn(ctx, s0.ClusterSettings(), lm, ie, s0.DB(), func(
		...
		require.Error(t, err, "attempted mutable access of synthetic descriptor %d", tableID)

		// Resolution by ID.

		desc, err = descriptors.GetTableVersionByID(ctx, txn, tableID, tree.ObjectLookupFlags{})
		require.NoError(t, err)
		require.Equal(t, "bar", desc.Columns[0].Name)

		// Attempting to resolve the table mutably is not allowed.
		_, err = descriptors.GetMutableTableVersionByID(ctx, tableID, txn)
		require.Error(t, err, "attempted mutable access of synthetic descriptor %d", tableID)
