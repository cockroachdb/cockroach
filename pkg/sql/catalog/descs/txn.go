...
func CheckTwoVersionInvariant(
	...
	count, err := lease.CountLeases(
		ctx, db, regionProviderFactory, settings, withNewVersion, txn.ProvisionalCommitTimestamp(),
	)
	if err != nil {
		return err
	}
	...
	for r := retry.StartWithCtx(ctx, base.DefaultRetryOptions()); r.Next(); {
		...
		now := clock.Now()
		count, err := lease.CountLeases(ctx, db, regionProviderFactory, settings, withNewVersion, now)
		if err != nil {
			return err
		}
		if count == 0 {
