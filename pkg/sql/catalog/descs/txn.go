...
func Txn(
	...
	descriptors := NewCollection(ctx, settings, leaseMgr, nil /* hydratedTables */)
	// Add an empty database cache for mixed-version cases.
	//
	// TODO(ajwerner): Remove in 21.1.
	if descriptors.databaseLeasingUnsupported {
		descriptors.databaseCache = database.NewCache(leaseMgr.Codec(), &config.SystemConfig{})
	}
	...
	for {
		if err := db.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
			...
			if err := f(ctx, txn, descriptors); err != nil {
				descriptors.ReleaseAll(ctx)
				return err
			}
			retryErr, err := CheckTwoVersionInvariant(
				ctx, db.Clock(), ie, descriptors, txn, nil /* onRetryBackoff */)
			if retryErr {
...
func CheckTwoVersionInvariant(
	...
	descs := descriptors.GetDescriptorsWithNewVersion()
	if descs == nil {
		return false, nil
	}
	if txn.IsCommitted() {
	...
	descriptors.ReleaseSpecifiedLeases(ctx, descs)
