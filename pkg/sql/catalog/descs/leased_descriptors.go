...
func (ld *leasedDescriptors) getResult(
	...
	if expiration.LessEq(txn.ReadTimestamp()) {
		return nil, false, errors.AssertionFailedf("bad descriptor for id=%d readTimestamp=%s, expiration=%s (leaseTimestamp: %d)", ldesc.GetID(), readTimestamp, expiration, ld.leaseTimestamp.GetTimestamp())
	}
