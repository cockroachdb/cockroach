...
func (tc *Collection) GetComment(key catalogkeys.CommentKey) (string, bool) {
	...
	if buildutil.CrdbTestBuild &&
		...
		panic(errors.AssertionFailedf("a leased descriptor exists but metadata data was not cached" +
...
func getDescriptorsByID(
	...
	// If metadata needs to be cached, then execute a read for everything but the descriptor.
	if !metadataNeeded.Empty() {
		_, err := tc.cr.GetByIDs(ctx, txn, metadataNeeded.Ordered(), false, catalog.Any, catkv.WithoutDescriptor())
		if err != nil {
			return err
		}
	}
