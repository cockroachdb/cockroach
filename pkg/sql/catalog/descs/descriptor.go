...
func (tc *Collection) ensureNameResolutionMatchesWithLeased(
	...
	// If both the name and entry are missing,
	// we don't have a problem. If they both match
...
func (tc *Collection) getNonVirtualDescriptorID(
	...
	// If we never returned results from any leasing or storage, then
	// no further checks are needed.
	if !usedLeasing && !usedStorage {
		return id, nil
	}
	// We will validate any returned value to detect for contradictions between the
	// leased descriptors and KV descriptor names. The version logic will detec if
	// they diverge for any other reason.
