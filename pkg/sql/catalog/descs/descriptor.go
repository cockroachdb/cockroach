...
func (tc *Collection) GetMutableDescriptorsByID(
	...
	descs, err := tc.getDescriptorsByID(ctx, txn, ids, flags)
	if err != nil {
		return nil, err
	}
...
func (tc *Collection) GetMutableDescriptorByID(
	...
	descs, err := tc.GetMutableDescriptorsByID(ctx, txn, []descpb.ID{id})
	if err != nil {
		return nil, err
	}
	return descs[0], nil
...
func (tc *Collection) GetImmutableDescriptorsByID(
	ctx context.Context, txn *kv.Txn, ids []descpb.ID, flags tree.CommonLookupFlags,
) ([]catalog.Descriptor, error) {
	flags.RequireMutable = false
	return tc.getDescriptorsByID(ctx, txn, ids, flags)
}
...
func (tc *Collection) GetImmutableDescriptorByID(
	...
	descs, err := tc.GetImmutableDescriptorsByID(ctx, txn, []descpb.ID{id}, flags)
	if err != nil {
		return nil, err
	}
	return descs[0], nil
	...
	defer func() {
		...
		for _, desc := range descs {
			...
			if desc.Adding() && (desc.IsUncommittedVersion() || flags.AvoidLeased || flags.RequireMutable) {
				...
				// Zero the descriptors when there's an error.
				descs = nil
				return
			}
		}
