...
func (tc *Collection) hydrateDescriptors(
	...
	var hydratableMutableIndexes, hydratableImmutableIndexes []int
	for i, desc := range descs {
		if desc == nil || !hydrateddesc.IsHydratable(desc) {
			continue
		}
		if _, ok := desc.(catalog.MutableDescriptor); ok {
			hydratableMutableIndexes = append(hydratableMutableIndexes, i)
		} else {
			hydratableImmutableIndexes = append(hydratableImmutableIndexes, i)
		}
	}

	// Hydrate mutable hydratable descriptors of the slice in-place.
	if len(hydratableMutableIndexes) > 0 {
		typeFn := makeMutableTypeLookupFunc(tc, txn, descs)
		for _, i := range hydratableMutableIndexes {
	...
	if len(hydratableImmutableIndexes) > 0 {
		typeFn := makeImmutableTypeLookupFunc(tc, txn, flags, descs)
		for _, i := range hydratableImmutableIndexes {
			desc := descs[i]
