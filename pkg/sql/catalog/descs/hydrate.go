...
func makeMutableTypeLookupFunc(
	...
	mutableLookupFunc := func(ctx context.Context, id descpb.ID) (catalog.Descriptor, error) {
		...
		// resolve types mutably, we resolve the mutable type descriptors they
		// reference, which may have in-memory changes. The problem with this is
		// that one is not permitted to mutably resolve the public schema
