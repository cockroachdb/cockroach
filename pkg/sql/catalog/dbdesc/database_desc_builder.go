...
func NewBuilder(desc *descpb.DatabaseDescriptor) DatabaseDescriptorBuilder {
	return &databaseDescriptorBuilder{
		...
		desc:     *protoutil.Clone(desc).(*descpb.DatabaseDescriptor),
	}
}

// TypeName implements the catalog.DescriptorBuilder interface.
func (ddb *databaseDescriptorBuilder) TypeName() catalog.DescriptorType {
...
func (ddb *databaseDescriptorBuilder) RunPostDeserializationChanges(
	...
	descpb.MaybeFixPrivileges(ddb.desc.ID, &ddb.desc.Privileges)
	return nil
}
...
func (ddb *databaseDescriptorBuilder) BuildImmutableDatabase() *Immutable {
	return &Immutable{DatabaseDescriptor: ddb.desc}
}
...
func (ddb *databaseDescriptorBuilder) BuildExistingMutableDatabase() *Mutable {
	return &Mutable{
		Immutable:      Immutable{DatabaseDescriptor: ddb.desc},
...
func (ddb *databaseDescriptorBuilder) BuildCreatedMutableDatabase() *Mutable {
	return &Mutable{Immutable: Immutable{DatabaseDescriptor: ddb.desc}}
}
