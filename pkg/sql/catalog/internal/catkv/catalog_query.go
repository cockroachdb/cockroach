...
// idsToSkipZoneConfigCache a set of object ids which do not have a descriptor
// in system.descriptor. We'd need to skip these ids when processing zone config
// kv because it breaks the assumption that a descriptor must be seen before a
// zone config can be inserted to cache.
var idsToSkipZoneConfigCache = func() util.FastIntSet {
	var ret util.FastIntSet
	ret.Add(keys.RootNamespaceID)
...
func (cq catalogQuery) processZonesResultRow(row kv.KeyValue, cb *nstree.MutableCatalog) error {
	...
	if idsToSkipZoneConfigCache.Contains(int(id)) {
		return nil
	}
	_, famID, err := encoding.DecodeUvarintAscending(remaining)
	if err != nil {
	...
	cb.UpsertZoneConfig(descpb.ID(id), &zoneConfig)
	return nil
}
