...
func (sc *StoredCatalog) ensureAll(ctx context.Context, c nstree.Catalog) error {
	...
	if err := c.ForEachCommentUnordered(func(key catalogkeys.CommentKey, cmt string) error {
		return sc.ensureComment(ctx, key, cmt)
	}); err != nil {
		return err
	}

	if err := c.ForEachZoneConfigUnordered(func(id descpb.ID, zoneConfig catalog.ZoneConfig) error {
		return sc.ensureZoneConfig(ctx, id, zoneConfig)
	}); err != nil {
		return err
	}
...
func (sc *StoredCatalog) ensureComment(
...
// ensureZoneConfig see storedZoneConfigs.upsert
func (sc *StoredCatalog) ensureZoneConfig(
...
func (c storedCatalogBackedDereferencer) DereferenceDescriptors(
	...
	if len(fallbackReqs) > 0 {
		...
		for _, id := range fallbackReqs {
			c.sc.markZoneConfigInfoSeen(id)
		}
