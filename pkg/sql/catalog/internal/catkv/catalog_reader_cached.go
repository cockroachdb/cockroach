...
func (c *cachedCatalogReader) GetByIDs(
	...
	for i, id := range ids {
		if c.byIDState[id].HasRequiredDescriptorEntries(!options.withoutDescriptor,
			!options.withoutComments, !options.withoutZoneConfig) || c.hasScanAll {
			continue
		}
	...
	if numUncached > 0 && !(c.hasScanAll && !isDescriptorRequired) {
		...
		for _, id := range uncachedIDs {
			...
			s.hasDescriptor = s.hasDescriptor || !options.withoutDescriptor
			s.hasComment = s.hasComment || !options.withoutComments
			s.hasZoneConfig = s.hasZoneConfig || !options.withoutZoneConfig
			c.setByIDState(id, s)
		}
	}
