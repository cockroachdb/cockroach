...
schema has changed. If this change is expected and occurred while patching an
existing major release, then this change probably needs to be reflected in any
hardcoded literals in the subsequent major release branches as well as in the 
main development branch.`, actualHash, d.Cmd)
			}
			return result
		})
	})
