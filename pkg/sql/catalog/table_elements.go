...
// the returned isStrictSubset reports whether the chosen one is such a subset
// match. Otherwise, only exact matches are considered. An error is returned
// if no qualifying constraint is found.
//
...
func FindFKReferencedUniqueConstraint(
	...
	if best.constraint != nil {
		return best.constraint, best.subset, nil
	}
