...
func TestHydratedCache(t *testing.T) {
	...
	t.Run("basic caching", func(t *testing.T) {
		...
		hydrated, _, err := c.GetHydratedTableDescriptor(ctx, td, res)
		...
		res.calls = 0
		cached, _, err := c.GetHydratedTableDescriptor(ctx, td, res)
		require.NoError(t, err)
	...
	t.Run("no UDT, no metrics", func(t *testing.T) {
		...
		_, _, err := c.GetHydratedTableDescriptor(ctx, td, res)
		require.NoError(t, err)
	...
	t.Run("name change causes eviction", func(t *testing.T) {
		...
		// the name change.
		retrieved, _, err := c.GetHydratedTableDescriptor(ctx, td, res)
		require.NoError(t, err)
	...
	t.Run("errors do not propagate to concurrent calls", func(t *testing.T) {
		...
		go func() {
			_, _, err := c.GetHydratedTableDescriptor(ctx, td, call2Res)
			callTwoErrCh <- err
		}()
	...
	t.Run("modified table gets rejected", func(t *testing.T) {
		...
		hydrated, canUse, err := c.GetHydratedTableDescriptor(ctx, td, res)
		require.NoError(t, err)
