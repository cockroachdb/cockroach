...
func TestHydratedCache(t *testing.T) {
	...
	t.Run("basic caching", func(t *testing.T) {
		...
		td := tableDescUDF.ImmutableCopy().(*tabledesc.Immutable)
		...
		// Show that the cache returned a new pointer and hydrated the UDF.
		require.NotEqual(t, tableDescUDF, hydrated)
	...
	t.Run("no udf, no metrics", func(t *testing.T) {
	...
	t.Run("version change causes eviction", func(t *testing.T) {
		...
		typDesc.Version++
		dg[typ1ID] = typDesc.ImmutableCopy()
	...
	t.Run("errors do not propagate to concurrent calls", func(t *testing.T) {
		...
		require.NoError(t, context.Canceled, <-callTwoErrCh)
...
var (
	...
	typ1Desc = typedesc.NewCreatedMutable(descpb.TypeDescriptor{
		Name:                     "enum",
		ID:                       typ1ID,
		Version:                  1,
		ParentID:                 dbID,
		ParentSchemaID:           scID,
		State:                    descpb.TypeDescriptor_PUBLIC,
		Kind:                     descpb.TypeDescriptor_ENUM,
		ReferencingDescriptorIDs: []descpb.ID{tableUDFID},
	...
	typ2Desc = typedesc.NewCreatedMutable(descpb.TypeDescriptor{
		Name:                     "other_enum",
		ID:                       typ2ID,
		Version:                  1,
		ParentID:                 dbID,
	...
	tableDescUDF    = tabledesc.NewCreatedMutable(descpb.TableDescriptor{
		Name:                    "foo",
		ID:                      tableUDFID,
		Version:                 1,
		ParentID:                dbID,
	...
	tableDescNoUDF = tabledesc.NewCreatedMutable(descpb.TableDescriptor{
		Name:                    "bar",
		ID:                      tableNoUDFID,
		Version:                 1,
		ParentID:                dbID,
	...
	descs = []catalog.MutableDescriptor{
		dbDesc, schemaDesc, typ1Desc, typ2Desc, tableDescUDF, tableDescNoUDF,
	}
)
