...
func TestGrantDefaultPrivileges(t *testing.T) {
	...
	creatorUser := security.MakeSQLUserInfoFromPreNormalizedString("creator", 54)
	...
	testCases := []struct {
		...
		},
		{
			defaultPrivilegesRole: catpb.DefaultPrivilegesRole{Role: creatorUser},
			privileges:            privilege.List{privilege.ALL},
			grantees:              []security.SQLUserInfo{fooUser, barUser, bazUser},
			targetObject:          tree.Tables,
			objectCreator:         creatorUser,
		},
		{
...
func TestRevokeDefaultPrivilegesFromEmptyList(t *testing.T) {
	...
	defaultPrivileges.RevokeDefaultPrivileges(catpb.DefaultPrivilegesRole{
		Role: creatorUser,
	}, privilege.List{privilege.ALL}, []security.SQLUserInfo{fooUser}, tree.Tables, false /* grantOptionFor */, false /*deprecateGrant*/)
...
func TestCreatePrivilegesFromDefaultPrivilegesForSystemDatabase(t *testing.T) {
	...
	if !newPrivileges.Owner().Username.IsNodeUser() {
		t.Errorf("expected owner to be node, owner was %s", newPrivileges.Owner().Username)
	}
}
...
func TestPresetDefaultPrivileges(t *testing.T) {
	...
	for _, targetObject := range targetObjectTypes {
		...
		if targetObject == tree.Types {
			if !newPrivileges.CheckPrivilege(security.PublicRoleInfo(), privilege.USAGE) {
...
func TestDefaultPrivileges(t *testing.T) {
	...
	testCases := []struct {
		objectCreator          security.SQLUserInfo
		defaultPrivilegesRole  security.SQLUserInfo
		dbID                   descpb.ID
		targetObject           tree.AlterDefaultPrivilegesTargetObject
	...
	}{
		{
			...
			// admin.
			objectCreator:         security.MakeSQLUserInfoFromPreNormalizedString("creator", 54),
			defaultPrivilegesRole: security.MakeSQLUserInfoFromPreNormalizedString("creator", 54),
			targetObject:          tree.Tables,
			...
			expectedGrantsOnObject: []userAndGrants{
				{
					user:   security.RootUserInfo(),
					grants: privilege.List{privilege.ALL},
				},
				{
					user:   security.AdminRoleInfo(),
					grants: privilege.List{privilege.ALL},
				},
			},
		},
		...
		},
		{
			...
			objectCreator:         security.MakeSQLUserInfoFromPreNormalizedString("creator", 54),
			defaultPrivilegesRole: security.MakeSQLUserInfoFromPreNormalizedString("bar", 52),
			targetObject:          tree.Tables,
			dbID:                  defaultDatabaseID,
			userAndGrants: []userAndGrants{
				{
	...
	for _, tc := range testCases {
		...
		for _, userAndGrant := range tc.userAndGrants {
			defaultPrivileges.GrantDefaultPrivileges(
				...
				userAndGrant.grants,
				[]security.SQLUserInfo{userAndGrant.user},
...
func TestModifyDefaultDefaultPrivileges(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		defaultPrivileges.RevokeDefaultPrivileges(
			...
			[]security.SQLUserInfo{creatorUser},
...
func TestModifyDefaultDefaultPrivilegesForPublic(t *testing.T) {
	...
	defaultPrivileges.RevokeDefaultPrivileges(
		...
		[]security.SQLUserInfo{security.PublicRoleInfo()},
	...
	user, found := privDesc.FindUser(security.PublicRoleInfo())
	if !found {
	...
	privDesc = defaultPrivilegesForCreator.DefaultPrivilegesPerObject[tree.Types]
	_, found = privDesc.FindUser(security.PublicRoleInfo())
	if found {
...
func TestApplyDefaultPrivileges(t *testing.T) {
	...
	testCases := []struct {
		...
		{catpb.NewPrivilegeDescriptor(testUser, privilege.List{}, privilege.List{}, security.AdminRoleInfo()),
			...
			privilege.List{privilege.SELECT, privilege.INSERT},
			privilege.List{privilege.SELECT, privilege.INSERT}},
		{catpb.NewPrivilegeDescriptor(testUser, privilege.List{privilege.CREATE, privilege.INSERT}, privilege.List{privilege.INSERT}, security.AdminRoleInfo()),
			testUser, privilege.Table,
		...
		{catpb.NewPrivilegeDescriptor(testUser, privilege.List{privilege.CREATE, privilege.INSERT}, privilege.List{privilege.CREATE}, security.AdminRoleInfo()),
			testUser, privilege.Table,
			privilege.List{privilege.ALL},
			privilege.List{privilege.ALL},
			privilege.List{privilege.ALL},
