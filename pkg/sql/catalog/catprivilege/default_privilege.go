...
func expandPrivileges(
	...
	switch targetObject {
	case tree.Tables:
		if GetRoleHasAllPrivilegesOnTargetObject(defaultPrivilegesForRole, tree.Tables) {
	...
	case tree.Sequences:
		if GetRoleHasAllPrivilegesOnTargetObject(defaultPrivilegesForRole, tree.Sequences) {
			privileges.Grant(defaultPrivilegesForRole.GetExplicitRole().UserProto.Decode(), privilege.List{privilege.ALL})
			defaultPrivilegesForRole.GetExplicitRole().RoleHasAllPrivilegesOnSequences = false
		}
	case tree.Schemas:
		if GetRoleHasAllPrivilegesOnTargetObject(defaultPrivilegesForRole, tree.Schemas) {
			privileges.Grant(defaultPrivilegesForRole.GetExplicitRole().UserProto.Decode(), privilege.List{privilege.ALL})
			defaultPrivilegesForRole.GetExplicitRole().RoleHasAllPrivilegesOnSchemas = false
		}
	case tree.Types:
		if GetRoleHasAllPrivilegesOnTargetObject(defaultPrivilegesForRole, tree.Types) {
			privileges.Grant(defaultPrivilegesForRole.GetExplicitRole().UserProto.Decode(), privilege.List{privilege.ALL})
...
func GetUserPrivilegesForObject(
	...
	switch targetObject {
	case tree.Tables:
		if GetRoleHasAllPrivilegesOnTargetObject(&p, tree.Tables) {
	...
	case tree.Sequences:
		if GetRoleHasAllPrivilegesOnTargetObject(&p, tree.Sequences) {
			return append(userPrivileges, descpb.UserPrivileges{
				UserProto:  userProto,
				Privileges: privilege.ALL.Mask(),
			})
	...
	case tree.Schemas:
		if GetRoleHasAllPrivilegesOnTargetObject(&p, tree.Schemas) {
			return append(userPrivileges, descpb.UserPrivileges{
				UserProto:  userProto,
				Privileges: privilege.ALL.Mask(),
			})
	...
	case tree.Types:
		if GetRoleHasAllPrivilegesOnTargetObject(&p, tree.Types) {
			userPrivileges = append(userPrivileges, descpb.UserPrivileges{
				UserProto:  userProto,
				Privileges: privilege.ALL.Mask(),
			})
...
func setPublicHasUsageOnTypes(p *descpb.DefaultPrivilegesForRole, publicHasUsageOnTypes bool) {
	if p.IsExplicitRole() {
