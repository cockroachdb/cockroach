...
func TestSchemaTelemetryJob(t *testing.T) {
	...
	// Check that the eventlog table has no schema telemetry log entries.
	tdb.CheckQueryResults(t, qHasLogs, [][]string{{"0"}})
	// Run a schema telemetry job and wait for the logs to appear.
	tdb.Exec(t, qJob)
	tdb.CheckQueryResultsRetry(t, qHasLogs, [][]string{{"1"}})
}
