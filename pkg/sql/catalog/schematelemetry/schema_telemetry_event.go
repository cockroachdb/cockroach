...
func CollectClusterSchemaForTelemetry(
	...
	events := make([]eventpb.EventPayload, 1, 1+meta.NumRecords)
	events[0] = meta
	newEvent := func(id descpb.ID) *eventpb.SchemaSnapshotDescriptor {
		return &eventpb.SchemaSnapshotDescriptor{
	...
	redacted := make(map[descpb.ID]*eventpb.SchemaSnapshotDescriptor, descIDsInSnapshot.Len())
	_ = raw.ForEachDescriptorEntry(func(rd catalog.Descriptor) error {
		...
		// Marshal protobuf and add all errors to the snapshot metadata event.
		var anyErr error
		ev.Desc, anyErr = gogotypes.MarshalAny(pb)

		for _, err := range append(redactErrs, errors.Wrap(anyErr, "marshal protobuf")) {
			if err != nil {
