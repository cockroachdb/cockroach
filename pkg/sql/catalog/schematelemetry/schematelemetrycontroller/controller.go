...
func (sc *scheduleController) CreateSchemaTelemetrySchedule(ctx context.Context) error {
	return sc.db.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
		if !sc.st.Version.IsActive(ctx, clusterversion.SQLSchemaTelemetryScheduledJobs) {
			return ErrVersionGate
		}
		_, err := CreateSchemaTelemetrySchedule(ctx, sc.ie, txn, sc.st)
		return err
	})
}
