...
func EncodeNameKey(codec keys.SQLCodec, nameKey catalog.NameKeyComponents) roachpb.Key {
	r := codec.IndexPrefix(uint32(systemschema.NamespaceTable.GetID()), uint32(systemschema.NamespaceTable.GetPrimaryIndexID()))
	...
	if nameKey.GetName() != "" {
		...
		r = keys.MakeFamilyKey(r, uint32(systemschema.NamespaceTable.PublicColumns()[3].GetID()))
	}
	return r
}
...
func DecodeNameMetadataKey(
	...
	if buf != uint64(systemschema.NamespaceTable.GetPrimaryIndexID()) {
		return nameKey, errors.Newf("tried get table %d, but got %d", systemschema.NamespaceTable.GetPrimaryIndexID(), buf)
	}
