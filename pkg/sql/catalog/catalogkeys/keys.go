...
// MakeCommentsMetadataPkFamKey returns the key for a comment string column
// family.
func MakeCommentsMetadataPkFamKey(
	codec keys.SQLCodec, cmtType keys.CommentType, descID descpb.ID, subID uint32,
) roachpb.Key {
	k := MakeSubObjectCommentsMetadataPrefix(codec, cmtType, descID, subID)
	return keys.MakeFamilyKey(k, keys.CommentsTablePrimaryColFamID)
}

// MakeCommentsMetadataCmtFamKey returns the key for a comment string column
// family.
func MakeCommentsMetadataCmtFamKey(
	codec keys.SQLCodec, cmtType keys.CommentType, descID descpb.ID, subID uint32,
) roachpb.Key {
	k := MakeSubObjectCommentsMetadataPrefix(codec, cmtType, descID, subID)
	return keys.MakeFamilyKey(k, keys.CommentsTableCommentColFamID)
}
