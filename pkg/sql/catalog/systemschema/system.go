...
CREATE TABLE system.transaction_diagnostics_requests(
	id INT8 DEFAULT unique_rowid() NOT NULL,
	completed BOOL NOT NULL DEFAULT FALSE,
	transaction_fingerprint_id BYTE NOT NULL,
	statement_fingerprint_ids BYTE[] NOT NULL,
	requested_at TIMESTAMPTZ NOT NULL,
	min_execution_latency INTERVAL NULL,
	expires_at TIMESTAMPTZ NULL,
	sampling_probability FLOAT NULL,
	...
	CONSTRAINT "primary" PRIMARY KEY (id),
	CONSTRAINT check_sampling_probability CHECK (sampling_probability BETWEEN 0.0 AND 1.0),
	INDEX completed_idx (completed, id) STORING (transaction_fingerprint_id, statement_fingerprint_ids, min_execution_latency, expires_at, sampling_probability, redact),
	FAMILY "primary" (id, completed, transaction_fingerprint_id, statement_fingerprint_ids, requested_at, min_execution_latency, expires_at, sampling_probability, redact)
);`
...
CREATE TABLE system.transaction_diagnostics(
	...
	CONSTRAINT "primary" PRIMARY KEY (id),
	FAMILY "primary" (id, transaction_fingerprint_id, statement_fingerprint_ids, collected_at, trace_chunks, statement_diagnostics_ids)
);`
	...
	TransactionDiagnosticsRequestsTable = makeSystemTable(
		...
		systemTable(
			...
			[]descpb.ColumnFamilyDescriptor{
				{
					Name:        "primary",
					ColumnNames: []string{"id", "completed", "transaction_fingerprint_id", "statement_fingerprint_ids", "requested_at", "min_execution_latency", "expires_at", "sampling_probability", "redact"},
					ColumnIDs:   []descpb.ColumnID{1, 2, 3, 4, 5, 6, 7, 8, 9},
				},
			},
			pk("id"),
			...
			descpb.IndexDescriptor{
				...
				Unique:              false,
				KeyColumnNames:      []string{"completed", "id"},
				StoreColumnNames:    []string{"transaction_fingerprint_id", "statement_fingerprint_ids", "min_execution_latency", "expires_at", "sampling_probability", "redact"},
				KeyColumnIDs:        []descpb.ColumnID{2, 1},
				KeyColumnDirections: []catenumpb.IndexColumn_Direction{catenumpb.IndexColumn_ASC, catenumpb.IndexColumn_ASC},
	...
	TransactionDiagnosticsTable = makeSystemTable(
		...
		systemTable(
			...
			[]descpb.ColumnDescriptor{
				...
				{Name: "trace_chunks", ID: 6, Type: types.IntArray, Nullable: true},
				{Name: "statement_diagnostics_ids", ID: 7, Type: types.IntArray, Nullable: true},
			},
			[]descpb.ColumnFamilyDescriptor{
				{
					Name:        "primary",
					ColumnNames: []string{"id", "transaction_fingerprint_id", "statement_fingerprint_ids", "transaction_fingerprint", "collected_at", "trace_chunks", "statement_diagnostics_ids"},
					ColumnIDs:   []descpb.ColumnID{1, 2, 3, 4, 5, 6, 7},
				},
			},
			pk("id"),
