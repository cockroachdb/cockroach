...
CREATE TABLE system.statement_statistics (
    ...
    INDEX "stmt_fp_ts_cov_counts" (fingerprint_id, aggregated_ts DESC) STORING (execution_count, exec_sample_count, svc_lat_sum, cpu_sql_nanos_sum, contention_time_sum, kv_cpu_time_nanos_sum, svc_lat_sum_sq, cpu_sql_nanos_sum_sq, contention_time_sum_sq, kv_cpu_time_nanos_sum_sq, admission_wait_time_sum, admission_wait_time_sum_sq, rows_read_sum, rows_written_sum, bytes_read_sum, bytes_read_sum_sq) WHERE app_name NOT LIKE '$ internal%',
    FAMILY "primary" (crdb_internal_aggregated_ts_app_name_fingerprint_id_node_id_plan_hash_transaction_fingerprint_id_shard_8, aggregated_ts, fingerprint_id, transaction_fingerprint_id, plan_hash, app_name, node_id, agg_interval, metadata, statistics, plan, index_recommendations, execution_count, service_latency, cpu_sql_nanos, contention_time, total_estimated_execution_time, p99_latency, exec_sample_count, svc_lat_sum, cpu_sql_nanos_sum, contention_time_sum, svc_lat_sum_sq, cpu_sql_nanos_sum_sq, contention_time_sum_sq, kv_cpu_time_nanos, kv_cpu_time_nanos_sum, kv_cpu_time_nanos_sum_sq, admission_wait_time_sum, admission_wait_time_sum_sq, rows_read_sum, rows_written_sum, bytes_read_sum, bytes_read_sum_sq)
	...
	StatementStatisticsTable = makeSystemTable(
		...
		systemTable(
			...
			[]descpb.ColumnDescriptor{
				...
				{Name: "rows_read_sum", ID: 32, Type: types.Float, Nullable: true, ComputeExpr: &rowsReadSumComputeExprStr},
				{Name: "rows_written_sum", ID: 33, Type: types.Float, Nullable: true, ComputeExpr: &rowsWrittenSumComputeExprStr},
				{Name: "bytes_read_sum", ID: 34, Type: types.Float, Nullable: true, ComputeExpr: &bytesReadSumComputeExprStr},
				{Name: "bytes_read_sum_sq", ID: 35, Type: types.Float, Nullable: true, ComputeExpr: &bytesReadSumSqComputeExprStr},
			},
			[]descpb.ColumnFamilyDescriptor{
				{
					Name: "primary",
			...
			descpb.IndexDescriptor{
				...
				StoreColumnIDs:     []descpb.ColumnID{14, 20, 21, 22, 23, 28, 24, 25, 26, 29, 30, 31, 32, 33, 34, 35},
				StoreColumnNames:   []string{"execution_count", "exec_sample_count", "svc_lat_sum", "cpu_sql_nanos_sum", "contention_time_sum", "kv_cpu_time_nanos_sum", "svc_lat_sum_sq", "cpu_sql_nanos_sum_sq", "contention_time_sum_sq", "kv_cpu_time_nanos_sum_sq", "admission_wait_time_sum", "admission_wait_time_sum_sq", "rows_read_sum", "rows_written_sum", "bytes_read_sum", "bytes_read_sum_sq"},
				CompositeColumnIDs: []descpb.ColumnID{21, 22, 23, 24, 25, 26, 28, 29, 30, 31, 32, 33, 34, 35},
				Version:            descpb.StrictIndexColumnIDGuaranteesVersion,
