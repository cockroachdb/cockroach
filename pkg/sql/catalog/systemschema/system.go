...
CREATE TABLE public.inspect_errors (
  ...
  CREATE TABLE system.statement_hints (
	...
	//
	// * kind: should be thought of as ENUM ('refreshStats', 'fixMisestimate')
	// and identifies the reason for why the auto stats collection was kicked
	// off:
	//   - 'refreshStats' uses value '1' and means that the collection was
	...
	// table-independent) concurrency. Thus, the PK (1, 0) controls the total
	// number of auto stats collections of the 'refreshStats' kind, and the PK
	// (2, 0) - of the 'fixMisestimate' kind.
	//
	// * job_ids: is an array of JobIDs of the corresponding auto stats jobs.
	// For valid table IDs this should contain at most one JobID, for fake table
	// ID of 0 the number of array elements will be limited by the target
  ...
  CREATE TABLE system.table_statistics_locks (
    ...
    table_id  INT8 NOT NULL,
    job_ids   INT8[],
    CONSTRAINT "primary" PRIMARY KEY (kind ASC, table_id ASC),
    FAMILY "primary" (kind, table_id, job_ids)
  );`
)
...
var (
	...
	TableStatisticsLocksTable = makeSystemTable(
		...
		systemTable(
			...
			descpb.InvalidID, // dynamically assigned
			[]descpb.ColumnDescriptor{
				{Name: "kind", ID: 1, Type: types.Int},
				{Name: "table_id", ID: 2, Type: types.Int},
				{Name: "job_ids", ID: 3, Type: types.IntArray, Nullable: true},
			},
			[]descpb.ColumnFamilyDescriptor{
				{
					Name:            "primary",
					ID:              0,
					ColumnNames:     []string{"kind", "table_id", "job_ids"},
					ColumnIDs:       []descpb.ColumnID{1, 2, 3},
					DefaultColumnID: 3,
				},
			},
			descpb.IndexDescriptor{
				...
				ID:                  1,
				Unique:              true,
				KeyColumnNames:      []string{"kind", "table_id"},
				KeyColumnDirections: []catenumpb.IndexColumn_Direction{catenumpb.IndexColumn_ASC, catenumpb.IndexColumn_ASC},
				KeyColumnIDs:        []descpb.ColumnID{1, 2},
			},
		),
