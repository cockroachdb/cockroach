...
CREATE TABLE system.statement_diagnostics_requests(
	...
	min_execution_latency_ms INT8 NULL,
	expires_at TIMESTAMPTZ NOT NULL DEFAULT now() + '100 years',
	CONSTRAINT "primary" PRIMARY KEY (id),
	INDEX completed_idx_v2 (completed, id) STORING (statement_fingerprint, min_execution_latency_ms, expires_at),

	FAMILY "primary" (id, completed, statement_fingerprint, statement_diagnostics_id, requested_at, min_execution_latency_ms, expires_at)
);`
...
var (
	...
	hundredYearsFromNowTZString = "now():::TIMESTAMPTZ + '100 years':::INTERVAL"
	...
	StatementDiagnosticsRequestsTable = registerSystemTable(
		...
		systemTable(
			...
			[]descpb.ColumnDescriptor{
				...
				{Name: "min_execution_latency_ms", ID: 6, Type: types.Int, Nullable: true},
				{Name: "expires_at", ID: 7, Type: types.TimestampTZ, Nullable: false, DefaultExpr: &hundredYearsFromNowTZString},
			},
			[]descpb.ColumnFamilyDescriptor{
				{
					Name:        "primary",
					ColumnNames: []string{"id", "completed", "statement_fingerprint", "statement_diagnostics_id", "requested_at", "min_execution_latency_ms", "expires_at"},
					ColumnIDs:   []descpb.ColumnID{1, 2, 3, 4, 5, 6, 7},
				},
			},
			pk("id"),
			...
			descpb.IndexDescriptor{
				...
				StoreColumnNames:    []string{"statement_fingerprint", "min_execution_latency_ms", "expires_at"},
