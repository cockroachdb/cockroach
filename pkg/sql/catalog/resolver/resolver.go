...
// Package resolver performs name resolution using a schema.SchemaResolver.
package resolver
...
func ResolveExistingTableObject(
...
) (res *catalog.ImmutableTableDescriptor, err error) {
	...
	return desc.(*catalog.ImmutableTableDescriptor), nil
}
...
func ResolveMutableExistingTableObject(
	...
	required bool,
	requiredType ResolveRequiredType,
) (res *catalog.MutableTableDescriptor, err error) {
	lookupFlags := tree.ObjectLookupFlags{
	...
	tn.ObjectNamePrefix = prefix
	return desc.(*catalog.MutableTableDescriptor), nil
}
...
func ResolveExistingObject(
	...
	case tree.TableObject:
		...
		if lookupFlags.RequireMutable {
			log.Infof(ctx, "table name: %v", un)
			return descI.(*catalog.MutableTableDescriptor), prefix, nil
		}

		return descI.(*catalog.ImmutableTableDescriptor), prefix, nil
	default:
...
func ResolveTargetObject(
	ctx context.Context, sc catalog.SchemaResolver, un *tree.UnresolvedObjectName,
) (*catalog.DatabaseDescriptor, tree.ObjectNamePrefix, error) {
	...
	return descI.(*catalog.DatabaseDescriptor), prefix, nil
}
...
// TODO(ajwerner,lucyzhang): this should take a SchemaResolver and us it.
func ResolveSchemaNameByID(
	...
	schemas, err := catalogkv.GetSchemasForDatabase(ctx, txn, codec, dbID)
