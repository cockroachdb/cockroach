...
// (3) table name is present from input, but the resolve object is not a table
// ore materialized view.
...
func ResolveIndex(
	...
	if tableIndexName.Table.ExplicitSchema {
		...
		tblFound, tbl, idx, err := findTableContainsIndex(
	...
	for path, ok := iter.Next(); ok; path, ok = iter.Next() {
		...
		candidateFound, tbl, idx, curErr := findTableContainsIndex(
...
// findTableContainsIndex tries to find and index in a schema by iterating
// through all tables.
