...
func (c Catalog) ForEachZoneConfigUnordered(
	fn func(id descpb.ID, zoneConfig *zonepb.ZoneConfig) error,
) error {
	if !c.IsInitialized() {
		return nil
	}
...
func (mc *MutableCatalog) UpsertComment(key keys.CommentKey, cmt string) {
	if mc.comments == nil {
		mc.comments = make(map[descpb.ID]map[keys.SubCommentKey]string)
	}
...
// catalog.
func (mc *MutableCatalog) UpsertZoneConfig(id descpb.ID, zoneConfig *zonepb.ZoneConfig) {
	if mc.zoneConfigs == nil {
		mc.zoneConfigs = make(map[descpb.ID]*zonepb.ZoneConfig)
	}
	if existing, ok := mc.zoneConfigs[id]; ok {
		mc.byteSize -= int64(existing.Size())
	}
	mc.zoneConfigs[id] = zoneConfig
	mc.byteSize += int64(zoneConfig.Size())
}
