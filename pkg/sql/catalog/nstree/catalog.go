...
type Catalog struct {
	underlying  NameMap
	byteSize    int64
	comments    map[descpb.ID]map[keys.SubCommentKey]string
	zoneConfigs map[descpb.ID]*catalog.ZoneConfigWithRawBytes
}
...
func (c Catalog) ForEachCommentUnordered(fn func(key keys.CommentKey, cmt string) error) error {
	if !c.IsInitialized() {
		return nil
	}

	for objID, objComments := range c.comments {
		for subKey, cmt := range objComments {
			k := keys.CommentKey{
				ObjectID:    uint32(objID),
				SubID:       subKey.SubID,
				CommentType: subKey.CommentType,
			}
...
func (c Catalog) ForEachZoneConfigUnordered(
	fn func(id descpb.ID, zoneConfig *catalog.ZoneConfigWithRawBytes) error,
) error {
	if !c.IsInitialized() {
		return nil
	}
...
func (c *Catalog) maybeInitialize() {
	...
	c.comments = make(map[descpb.ID]map[keys.SubCommentKey]string)
	c.zoneConfigs = make(map[descpb.ID]*catalog.ZoneConfigWithRawBytes)
}
...
func (mc *MutableCatalog) UpsertComment(key keys.CommentKey, cmt string) {
	mc.maybeInitialize()
	//var objComments map[keys.SubCommentKey]string
	descID := descpb.ID(key.ObjectID)
	if _, ok := mc.comments[descID]; !ok {
		mc.comments[descID] = make(map[keys.SubCommentKey]string)
	}
	objComments := mc.comments[descID]
	subKey := keys.SubCommentKey{SubID: key.SubID, CommentType: key.CommentType}
	mc.byteSize -= int64(len(objComments[subKey]))
	objComments[subKey] = cmt
...
func (mc *MutableCatalog) UpsertZoneConfig(
	...
	if mc.zoneConfigs == nil {
		mc.zoneConfigs = make(map[descpb.ID]*catalog.ZoneConfigWithRawBytes)
	}
	if existing, ok := mc.zoneConfigs[id]; ok {
		mc.byteSize -= int64(existing.Size())
	}
	zc := catalog.NewZoneConfigWithRawBytes(zoneConfig, rawBytes)
