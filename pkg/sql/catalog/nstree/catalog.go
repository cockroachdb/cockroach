...
func (mc *MutableCatalog) UpsertDescriptorEntry(desc catalog.Descriptor) {
	...
	//fmt.Printf("Xiang: added descriptor %v and grows %v bytes of memory.\n", desc.GetName(), desc.ByteSize())
...
func (mc *MutableCatalog) DeleteDescriptorEntry(id descpb.ID) {
	...
	if removed := mc.underlying.byID.delete(id); removed != nil {
		//fmt.Printf("Xiang: deleted descriptor %v and shrinks %v bytes of memory.\n", id, removed.(catalog.Descriptor).ByteSize())
...
func (mc *MutableCatalog) UpsertNamespaceEntry(key catalog.NameKey, id descpb.ID) {
	...
	//fmt.Printf("Xiang: added namespace entry %v and grows %v bytes of memory.\n", key.GetName(), nsEntry.ByteSize())
...
func (mc *MutableCatalog) DeleteNamespaceEntry(key catalog.NameKey) {
	...
	if removed := mc.underlying.byName.delete(key); removed != nil {
		//fmt.Printf("Xiang: removed namespace entry %v and shrinks %v bytes of memory.\n", key.GetName(), removed.(*namespaceEntry).ByteSize())
