...
func (p *Preemptor) EnsureNoSingleVersionLeases(
	...
	var timestampOfRemovals hlc.Timestamp
	{
		...
		g.GoCtx(func(ctx context.Context) (err error) {
			err, timestampOfRemovals = p.waitForOutstandingLeases(
	...
	if log.ExpensiveLogEnabled(ctx, 1) {
		log.Infof(ctx, "leases %d removed as of %v", descriptors, timestampOfRemovals)
	}
...
func (p *Preemptor) writeLockingIntents(
	...
	return p.s.Delete(ctx, txn, svstorage.ActionIDsToRows(svstorage.Lock, descriptors)...)
}
...
func (p *Preemptor) waitForOutstandingLeases(
...
) (_ error, leasesRemovedTimestamp hlc.Timestamp) {
	...
	if err := removeExpired(); err != nil {
		return err, hlc.Timestamp{}
	}
	if len(leases) == 0 {
		return nil, leasesRemovedTimestamp
	}
	...
	for len(leases) > 0 {
		...
		case <-timer.C:
			...
			timer.Reset(pollLivenessInterval)
		case err := <-rfErrCh:
			return err, leasesRemovedTimestamp
		case ev := <-rfEventCh:
			...
			if !ev.IsDeletion {
				...
				// waiting here.
				return transactionMustBeAbortedError, hlc.Timestamp{}
			}
...
var transactionMustBeAbortedError = errors.New(
	"new runBatch to the leases span indicates that the transaction is aborted",
)
