...
func (a *Acquirer) WatchForNotifications(
	...
	callback func(singleversion.Notification),
) (err error) {
	if log.ExpensiveLogEnabled(ctx, 1) {
		defer func() {
			log.Infof(ctx, "exiting notifications rangefeed for %s starting from %v: %v", ids, startTS, err)
		}()
		log.Infof(ctx, "starting notifications rangefeed for %s starting from %v", ids, startTS)
	}
		...
		if ids.Contains(event.Descriptor) {
			if !event.IsDeletion {
				...
				if a.warnEvery.ShouldLog() {
					log.Warningf(ctx,
						"unexpected write to singleversion notification table for %d", event.Descriptor)
				}
			} else {
				callback(singleversion.Notification{
					ID:        event.Descriptor,
