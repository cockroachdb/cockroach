...
func (s DescriptorIDSet) ForEach(f func(id descpb.ID)) {
	s.set.ForEach(func(i int) { f(descpb.ID(i)) })
}

// Empty returns true if the set is empty.
func (s DescriptorIDSet) Empty() bool { return s.set.Empty() }

// Ordered returns a slice with all the descpb.IDs in the set, in
// increasing order.
func (s DescriptorIDSet) Ordered() []descpb.ID {
	if s.Empty() {
		return nil
	}
	result := make([]descpb.ID, 0, s.Len())
	s.ForEach(func(i descpb.ID) {
