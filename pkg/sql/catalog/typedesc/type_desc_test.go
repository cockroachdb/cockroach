...
func TestValidateTypeDesc(t *testing.T) {
	...
	testData := []struct {
		...
		{ // 23
			`multi-region enum: parent database "multi-region-db" (200): primary region "us-east-1" differs from enum "us-east-2"`,
			descpb.TypeDescriptor{
				Name:           "t",
				ID:             typeDescID,
				ParentID:       200,
		...
		{ // 24
			`multi-region enum: primary region "us-east-2" not found in enum members`,
			descpb.TypeDescriptor{
				Name:           "t",
				ID:             typeDescID,
				ParentID:       200,
