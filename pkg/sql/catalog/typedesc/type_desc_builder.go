...
func NewBuilder(desc *descpb.TypeDescriptor) TypeDescriptorBuilder {
	return &typeDescriptorBuilder{
		...
		desc:     *protoutil.Clone(desc).(*descpb.TypeDescriptor),
	}
}

// TypeName implements the catalog.DescriptorBuilder interface.
func (tdb *typeDescriptorBuilder) TypeName() catalog.DescriptorType {
	return catalog.Type
}
...
func (tdb *typeDescriptorBuilder) BuildExistingMutableType() *Mutable {
	cv := makeImmutable(tdb.original)
	return &Mutable{Immutable: makeImmutable(tdb.desc), ClusterVersion: &cv}
}
...
func (tdb *typeDescriptorBuilder) BuildCreatedMutableType() *Mutable {
	return &Mutable{Immutable: makeImmutable(tdb.desc)}
}
