...
func (desc *Immutable) ValidateSelf(vea catalog.ValidationErrorAccumulator) {
	...
	if desc.GetID() == descpb.InvalidID {
		vea.Report(errors.AssertionFailedf("invalid ID %d", errors.Safe(desc.GetID())))
	}
	if desc.GetParentID() == descpb.InvalidID {
		vea.Report(errors.AssertionFailedf("invalid parentID %d", errors.Safe(desc.GetParentID())))
	}
...
func (desc *Immutable) ValidateCrossReferences(
	...
	case descpb.TypeDescriptor_ENUM, descpb.TypeDescriptor_MULTIREGION_ENUM:
		...
		if _, err := vdg.GetTypeDescriptor(desc.GetArrayTypeID()); err != nil {
			vea.Report(errors.Wrapf(err, "arrayTypeID %d does not exist for %q", errors.Safe(desc.GetArrayTypeID()), desc.GetKind().String()))
		}
	case descpb.TypeDescriptor_ALIAS:
		if desc.GetAlias().UserDefined() {
			...
			if _, err := vdg.GetTypeDescriptor(aliasedID); err != nil {
				vea.Report(errors.Wrapf(err, "aliased type %d does not exist", errors.Safe(aliasedID)))
			}
		}
	}
	...
	for _, id := range desc.GetReferencingDescriptorIDs() {
		...
		if tableDesc.Dropped() {
			vea.Report(errors.AssertionFailedf(
				"referencing table %d was dropped without dependency unlinking", errors.Safe(id)))
		}
	}
}
