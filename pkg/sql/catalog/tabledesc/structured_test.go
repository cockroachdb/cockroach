...
func TestKeysPerRow(t *testing.T) {
	...
	for i, test := range tests {
		t.Run(fmt.Sprintf("%s - %d", test.createTable, test.indexID), func(t *testing.T) {
			...
			desc := desctestutils.TestingGetPublicTableDescriptor(db, keys.SystemSQLCodec, version, "d", tableName)
...
func TestRemoveDefaultExprFromComputedColumn(t *testing.T) {
	...
	tbl := desctestutils.TestingGetPublicTableDescriptor(kvDB, keys.SystemSQLCodec, latestClusterVersionForValidationForTest, "t", "tbl")
