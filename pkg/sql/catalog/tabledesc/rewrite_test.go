...
func randomTableDesc(rng *rand.Rand) *descpb.TableDescriptor {
	...
	randEnum := func() *types.T {
		return types.MakeEnum(randTypeOID(), randTypeOID())
	}
	randUDT := func() *types.T {
		switch rng.Intn(3) {
		case 0:
			return randEnum()
		case 1:
			return types.MakeArray(randEnum())
		default:
			n := 1 + rng.Intn(3)
			...
			for i := range contents {
				if rng.Intn(2) == 0 {
					...
					contents[i] = types.Int
				}
			}
			t := types.MakeLabeledTuple(contents, nil)
			t.InternalType.Oid = randTypeOID()
			t.InternalType.UDTMetadata = &types.PersistentUserDefinedTypeMetadata{
				ArrayTypeOID: randTypeOID(),
			}
			return t
		}
	}
...
func getAllIDs(t *testing.T, desc *descpb.TableDescriptor) catalog.DescriptorIDSet {
	...
	require.NoError(t, walkutil.Walk(desc, func(id *catid.DescID) error {
		ids.Add(*id)
		return nil
	}))

	var collectTypeIDs func(typ *types.T)
	collectTypeIDs = func(typ *types.T) {
		if typ.UserDefined() {
		...
		case types.ArrayFamily:
			collectTypeIDs(typ.ArrayContents())
	...
	require.NoError(t, walkutil.Walk(desc, func(typ *types.T) error {
		collectTypeIDs(typ)
		return nil
	}))

	visitor := &tree.TypeCollectorVisitor{OIDs: make(map[oid.Oid]struct{})}
	require.NoError(t, walkutil.Walk(desc, func(expr *catpb.Expression) error {
		if *expr == "" {
			return nil
		}
	...
	require.NoError(t, walkutil.Walk(desc, func(stmt *catpb.Statement) error {
		if *stmt == "" {
			return nil
		}
	...
	require.NoError(t, walkutil.Walk(desc, func(body *catpb.RoutineBody) error {
		if *body == "" {
			return nil
		}
