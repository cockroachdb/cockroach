...
func (w column) IsGeneratedAsIdentity() bool {
	return w.desc.GeneratedAsIdentityType != descpb.GeneratedAsIdentityType_NOT_IDENTITY_COLUMN
}
...
func (w column) IsGeneratedAlwaysAsIdentity() bool {
	return w.desc.GeneratedAsIdentityType == descpb.GeneratedAsIdentityType_GENERATED_ALWAYS
}
...
func (w column) IsGeneratedByDefaultAsIdentity() bool {
	return w.desc.GeneratedAsIdentityType == descpb.GeneratedAsIdentityType_GENERATED_BY_DEFAULT
}
...
func makeIndexColumnCache(idx *descpb.IndexDescriptor, all []catalog.Column) (ic indexColumnCache) {
	...
	ic.allDirs = append(make([]descpb.IndexDescriptor_Direction, 0, nAll), idx.KeyColumnDirections...)
	ic.all = make([]catalog.Column, 0, nAll)
	for _, ids := range [][]descpb.ColumnID{
		idx.KeyColumnIDs,
		idx.KeySuffixColumnIDs,
		idx.StoreColumnIDs,
	} {
		for _, id := range ids {
			for _, col := range all {
				if col.GetID() == id {
					ic.all = append(ic.all, col)
					break
				}
			}
		}
