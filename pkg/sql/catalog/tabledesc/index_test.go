...
func TestIndexInterface(t *testing.T) {
	...
	immutable := desctestutils.TestingGetPublicTableDescriptor(db, keys.SystemSQLCodec, latestClusterVersionForValidationForTest, "d", "t")
...
func TestLatestIndexDescriptorVersionValues(t *testing.T) {
	...
	for _, name := range []string{`t`, `s`, `v`} {
		desc := desctestutils.TestingGetPublicTableDescriptor(kvDB, keys.SystemSQLCodec, version, "defaultdb", name)
		test(desc)
	}
