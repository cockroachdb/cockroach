...
func determineIfIndexNeedsForcePuts(index catalog.Index, tableDesc *descpb.TableDescriptor) bool {
	...
	if index.GetEncodingType() != catenumpb.PrimaryIndexEncoding {
		return false
	}
	...
	// new columns.
	return catalog.MakeTableColSet(tableDesc.PrimaryIndex.KeyColumnIDs...).Difference(
		catalog.MakeTableColSet(index.IndexDesc().KeyColumnIDs...)).Len() == 0
}
