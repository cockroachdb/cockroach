...
// All fields with catalog.Index types should be accessed via the return value
// of the initialized method.
type indexCache struct {
	// initOnce ensures that the computation of the slices in this struct only
	// takes place once.
	initOnce sync.Once
	// initDone does the same thing as the `done` flag in sync.Once and therefore
	// isn't really required other than for performance reasons: the Do() method
	// for sync.Once must be called with a closure and this flag saves us from
	// having to build it every time we access the cache.
	initDone syncutil.AtomicBool

	// The remaining fields store the values of interest.
...
// initialized returns the receiver, after initializing it if necessary.
func (c *indexCache) initialized(desc *wrapper) *indexCache {
	if c.initDone.Get() {
		return c
	}
	c.initOnce.Do(func() {
		defer c.initDone.Set(true)
		initializeIndexCache(c, desc)
	})
	return c
}

func initializeIndexCache(c *indexCache, desc *wrapper) {
