	...
	for _, tc := range []struct {
		...
		},
		{
			...
			exp: `tabledesc.Immutable: {` +
				...
				`{ID: 5, TypeID: 20, Null: false, State: ADD, MutationID: 2}` +
				`], ` +
				`NextFamilyID: 1, ` +
				...
				`{ID: 3, Unique: false, Columns: [{ID: 3, Dir: ASC}, {ID: 2, Dir: DESC}], ExtraColumns: [1], StoreColumns: [5], State: ADD, MutationID: 3}` +
				`], ` +
				`Checks: [` +
				`{Columns: [2], Validity: Validated}, ` +
				`{Columns: [2], Validity: Unvalidated, State: ADD, MutationID: 3}` +
				`], ` +
				`InboundFKs: [` +
				...
				`OutboundFKs: [` +
				`{OriginTableID: 2, OriginColumns: [3], ReferencedTableID: 12, ReferencedColumnIDs: [2], Validity: Validated}, ` +
				`{OriginTableID: 12, OriginColumns: [2], ReferencedTableID: 2, ReferencedColumnIDs: [3], Validity: Unvalidated, State: ADD, MutationID: 1}` +
				`]}`,
			f: func(mutable *tabledesc.Mutable) catalog.TableDescriptor {
		...
		t.Run("", func(t *testing.T) {
			...
			require.NoError(t, yaml.Unmarshal([]byte(redacted), &m), redacted)
		})
	}
}
