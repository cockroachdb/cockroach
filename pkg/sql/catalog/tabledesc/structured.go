			...
			if col.Name == indexCol {
				...
				if i == lastCol && !colinfo.ColumnTypeIsInvertedIndexable(col.Type) {
					return errors.WithHint(
						pgerror.New(
							...
							fmt.Sprintf("column %s of type %s is not allowed as the last column in an inverted index", col.Name, col.Type.Name()),
						),
				...
				if i < lastCol && !colinfo.ColumnTypeIsIndexable(col.Type) {
					return errors.WithHint(
						pgerror.New(
							pgcode.FeatureNotSupported,
							fmt.Sprintf("column %s of type %s is only allowed as the last column in an inverted index", col.Name, col.Type.Name()),
						),
						"see the documentation for more information about inverted indexes",
					)
				}
