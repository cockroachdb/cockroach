...
// It returns the column names to use for sharding. If not specified, returns nil
// which means all index columns should be used for sharding.
	...
	case *tree.ParenExpr:
		...
		if !ok {
			return nil, pgerror.Newf(
				pgcode.InvalidParameterValue,
				"shard_columns must be a column identifier or tuple of column identifiers, got %T",
				expr.Expr,
			)
		}
