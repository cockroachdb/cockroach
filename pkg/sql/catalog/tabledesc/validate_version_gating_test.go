...
func TestIndexDoesNotStorePrimaryKeyColumnMixedVersion(t *testing.T) {
	...
	skip.UnderStress(t, "test takes >10 seconds to run so stressing would take too long to complete")
	...
	*/
	...
	// Upgrade precondition checks for corrupt descriptor AOST '-10s', so, we wait
	// for 10s here to ensure the corruption will be seen by the time the upgrade
