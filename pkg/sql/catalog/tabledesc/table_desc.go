...
var _ catalog.TableDescriptor = (*tableDescriptorWrapper)(nil)

// tableDescriptorWrapper is the base implementation of the catalog.Descriptor
// interface, which is overloaded by Immutable and Mutable
type tableDescriptorWrapper struct {
...
func (desc *tableDescriptorWrapper) mutationIndexes(
...
func (desc *tableDescriptorWrapper) DeleteOnlyIndexes() []descpb.IndexDescriptor {
...
func (desc *tableDescriptorWrapper) WritableIndexes() []descpb.IndexDescriptor {
	if len(desc.Mutations) == 0 {
		return desc.Indexes
	}
...
func (desc *tableDescriptorWrapper) DeletableIndexes() []descpb.IndexDescriptor {
	if len(desc.Mutations) == 0 {
		return desc.Indexes
	}
	indexes := make([]descpb.IndexDescriptor, 0, len(desc.Indexes)+len(desc.Mutations))
...
func (desc *tableDescriptorWrapper) mutationColumns(
	mutationState descpb.DescriptorMutation_State,
...
// schema change) which can be scanned. Columns in the process of a schema
// change are all set to nullable while column backfilling is still in
// progress, as mutation columns may have NULL values.
func (desc *tableDescriptorWrapper) ReadableColumns() []descpb.ColumnDescriptor {
	if len(desc.Mutations) == 0 {
		return desc.Columns
	}
	...
	// Add non-public columns, aka mutation columns.
	for _, m := range desc.Mutations {
		if col := m.GetColumn(); col != nil {
			readableCol := *col
			...
			readableCol.Nullable = true
			columns = append(columns, readableCol)
		}
	}
	return columns
}
...
func (desc *tableDescriptorWrapper) DeletableColumns() []descpb.ColumnDescriptor {
	if len(desc.Mutations) == 0 {
		return desc.Columns
	}
	columns := make([]descpb.ColumnDescriptor, 0, len(desc.Columns)+len(desc.Mutations))
...
func (desc *tableDescriptorWrapper) WritableColumns() []descpb.ColumnDescriptor {
	if len(desc.Mutations) == 0 {
		return desc.Columns
	}
	columns := make([]descpb.ColumnDescriptor, 0, len(desc.Columns)+len(desc.Mutations))
...
func (desc *tableDescriptorWrapper) MutationColumns() []descpb.ColumnDescriptor {
	if len(desc.Mutations) == 0 {
		return nil
	}
	columns := make([]descpb.ColumnDescriptor, 0, len(desc.Mutations))
...
func (desc *tableDescriptorWrapper) ActiveChecks() []descpb.TableDescriptor_CheckConstraint {
...
func (desc *tableDescriptorWrapper) GetColumnOrdinalsWithUserDefinedTypes() []int {
	var ords []int
...
func (desc *tableDescriptorWrapper) DescriptorProto() *descpb.Descriptor {
...
func (desc *tableDescriptorWrapper) GetPrimaryIndexID() descpb.IndexID {
	return desc.PrimaryIndex.ID
}

// GetPublicNonPrimaryIndexes returns the public non-primary indexes of the descriptor.
func (desc *tableDescriptorWrapper) GetPublicNonPrimaryIndexes() []descpb.IndexDescriptor {
	return desc.Indexes
}

// IsTemporary returns true if this is a temporary table.
func (desc *tableDescriptorWrapper) IsTemporary() bool {
	return desc.GetTemporary()
}

// GetPublicColumns return the public columns in the descriptor.
func (desc *tableDescriptorWrapper) GetPublicColumns() []descpb.ColumnDescriptor {
	return desc.Columns
}

// GetColumnAtIdx returns the column at the specified index.
func (desc *tableDescriptorWrapper) GetColumnAtIdx(idx int) *descpb.ColumnDescriptor {
