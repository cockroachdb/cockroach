...
type constraintCache struct {
	...
	allActiveAndInactiveUniqueWithoutIndexs []catalog.Constraint
	allActiveUniqueWithoutIndexs            []catalog.Constraint
}
...
func newConstraintCache(desc *descpb.TableDescriptor, mutations *mutationCache) *constraintCache {
	...
	for _, cst := range []descpb.IndexDescriptor{desc.PrimaryIndex} {
		backingStruct := constraint{
			desc: &descpb.ConstraintToUpdate{
				ConstraintType:                descpb.ConstraintToUpdate_PRIMARY_KEY,
				Name:                          cst.Name,
				Index:                         cst,
				IndexBackedConstraintValidity: descpb.ConstraintValidity_Validated,
			},
		}
	...
	for _, cst := range desc.Indexes {
		if cst.Unique {
			backingStruct := constraint{
				desc: &descpb.ConstraintToUpdate{
					ConstraintType:                descpb.ConstraintToUpdate_UNIQUE,
					Name:                          cst.Name,
					Index:                         cst,
					IndexBackedConstraintValidity: descpb.ConstraintValidity_Validated,
				},
			...
			case descpb.PrimaryIndexEncoding:
				backingStruct := constraint{
					desc: &descpb.ConstraintToUpdate{
						ConstraintType:                descpb.ConstraintToUpdate_PRIMARY_KEY,
						Name:                          cst.GetName(),
						Index:                         cst.IndexDescDeepCopy(),
						IndexBackedConstraintValidity: validity,
					},
				}
				allConstraints = addIfNotExists(backingStruct, allConstraints, constraintIDs)
	...
	for _, cst := range allConstraints {
		...
		c.allUniqueWithoutIndexs, c.allActiveAndInactiveUniqueWithoutIndexs, c.allActiveUniqueWithoutIndexs =
			addConstraintToSetsByValidity(cst, c.allUniqueWithoutIndexs, c.allActiveAndInactiveUniqueWithoutIndexs, c.allActiveUniqueWithoutIndexs)
	}
