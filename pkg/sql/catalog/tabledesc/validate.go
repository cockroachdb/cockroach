	...
	if desc.GetParentSchemaID() != keys.PublicSchemaID && !desc.IsTemporary() {
		...
		if schemaDesc != nil && dbDesc != nil && schemaDesc.GetParentID() != dbDesc.GetID() {
			vea.Report(errors.AssertionFailedf("parent schema %d is in different database %d",
				errors.Safe(desc.GetParentSchemaID()), errors.Safe(schemaDesc.GetParentID())))
		}
	}
	...
	if indexI.NumInterleaveAncestors() > 0 {
		...
		if err != nil {
			return errors.Wrapf(err,
				"invalid interleave: missing table=%d index=%d", errors.Safe(ancestor.TableID), errors.Safe(ancestor.IndexID))
		}
...
func (desc *wrapper) ValidateSelf(vea catalog.ValidationErrorAccumulator) {
	...
	if desc.GetID() == descpb.InvalidID {
		vea.Report(errors.AssertionFailedf("invalid table ID %d", errors.Safe(desc.GetID())))
	}
	if desc.GetParentSchemaID() == descpb.InvalidID {
		vea.Report(errors.AssertionFailedf("invalid parent schema ID %d", errors.Safe(desc.GetParentSchemaID())))
	}
	...
	if desc.GetParentID() == descpb.InvalidID && !desc.IsVirtualTable() {
		vea.Report(errors.AssertionFailedf("invalid parent ID %d", errors.Safe(desc.GetParentID())))
	}
