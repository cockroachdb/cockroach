...
func (desc *wrapper) validateTableIndexes(columnNames map[string]descpb.ColumnID) error {
	...
	for _, idx := range desc.NonDropIndexes() {
		...
		if idx.Primary() {
			...
			if idx.GetVersion() != descpb.StrictIndexColumnIDGuaranteesVersion {
				return errors.AssertionFailedf("secondary index %q has invalid version %d, expected %d",
					idx.GetName(), idx.GetVersion(), descpb.StrictIndexColumnIDGuaranteesVersion)
			}
		}
		slices := []struct {
			name  string
