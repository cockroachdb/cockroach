...
func (desc *wrapper) validateMinStaleRows(vea catalog.ValidationErrorAccumulator, value *int64) {
	if value != nil {
		...
		if *value < 0 {
			vea.Report(errors.Newf("invalid integer value for %s: cannot be negative", settingName))
		}
	}
}
...
func (desc *wrapper) validateFractionStaleRows(
	...
	if value != nil {
		...
		desc.verifyProperTableForStatsSetting(vea, settingName)
		if *value < 0 {
			vea.Report(errors.Newf("invalid float value for %s: cannot be negative", settingName))
		}
	}
}
