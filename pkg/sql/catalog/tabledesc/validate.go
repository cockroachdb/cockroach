...
const (
	...
	unexpectedLocality                                                     = `unexpected_locality`
	missingBackReference                                                   = `missing_back_reference`
	missingUnupgradedBackReference                                         = `missing_pre_19_2_back_reference`
	badUnupgradedBackReference                                             = `bad_pre_19_2_back_reference`
	missingForwardReference                                                = `missing_forward_reference`
	missingUnupgradedForwardReference                                      = `missing_pre_19_2_forward_reference`
	badUnupgradedForwardReference                                          = `bad_pre_19_2_forward_references`
	missingPartitionBy                                                     = `missing_partition_by`
	missingFamilies                                                        = `missing_families`
	notInFamily                                                            = `not_in_family`
	virtualColumn                                                          = `virtual_column`
	badExpr                                                                = `bad_expr`
	badColumnRefs                                                          = `bad_column_refs`
...
func (desc *wrapper) ValidateCrossReferences(
	...
	if dbDesc != nil {
		...
		for _, id := range referencedTypeIDs {
			...
			if err != nil {
				vea.Report(catalog.NotFound, err)
			} else if !typ.GetReferencedDescIDs().Contains(desc.GetID()) {
				vea.ReportInternalf(missingBackReference, "referenced type %q (%d) is missing back reference to table",
					typ.GetName(), typ.GetID())
			}
		}

		// Validate table locality config.
		if !desc.validateTableLocalityConfig(vea, dbDesc, vdg) {
			return
		}
	}
...
func (desc *wrapper) validatePartitioningDescriptor(
	...
	if idx.NumInterleaveAncestors() > 0 {
		return errors.AssertionFailedf("cannot set a zone config for interleaved index %s; "+
	...
	if len(partDesc.List) == 0 && len(partDesc.Range) == 0 {
		return errors.AssertionFailedf("at least one of LIST or RANGE partitioning must be used")
	}
	if len(partDesc.List) > 0 && len(partDesc.Range) > 0 {
		return errors.AssertionFailedf("only one LIST or RANGE partitioning may used")
	}
	...
	checkName := func(name string) error {
		if len(name) == 0 {
			return errors.AssertionFailedf("PARTITION name must be non-empty")
		}
		if indexName, exists := partitionNames[name]; exists {
			if indexName == idx.GetName() {
				return errors.AssertionFailedf("PARTITION %s: name must be unique (used twice in index %q)",
					name, indexName)
			}
		}
	...
	if len(partDesc.List) > 0 {
		...
		for _, p := range partDesc.List {
			...
			if len(p.Values) == 0 {
				return errors.AssertionFailedf("PARTITION %s: must contain values", p.Name)
			}
			...
			for _, valueEncBuf := range p.Values {
				...
				if err != nil {
					return errors.AssertionFailedf("PARTITION %s: %v", p.Name, err)
				}
				if _, exists := listValues[string(keyPrefix)]; exists {
					return errors.AssertionFailedf("%s cannot be present in more than one partition", tuple)
				}
	...
	if len(partDesc.Range) > 0 {
		...
		for _, p := range partDesc.Range {
			...
			if overlaps := tree.Get(pi.Range()); len(overlaps) > 0 {
				return errors.AssertionFailedf("partitions %s and %s overlap",
					overlaps[0].(partitionInterval).name, p.Name)
			}
			if err := tree.Insert(pi, false /* fast */); errors.Is(err, interval.ErrEmptyRange) {
				return errors.AssertionFailedf("PARTITION %s: empty range: lower bound %s is equal to upper bound %s",
					p.Name, fromDatums, toDatums)
			} else if errors.Is(err, interval.ErrInvertedRange) {
				return errors.AssertionFailedf("PARTITION %s: empty range: lower bound %s is greater than upper bound %s",
					p.Name, fromDatums, toDatums)
			} else if err != nil {
				return errors.NewAssertionErrorWithWrappedErrf(err, "PARTITION %s", p.Name)
			}
		}
	}
...
func (desc *wrapper) validateTableLocalityConfig(
	...
	// Perform some preliminary checks while getting the regions enum from the DB.
	regionsEnum := func() catalog.TypeDescriptor {
		vea.PushContextf(`parent_database`, "parent database %q (%d)", db.GetName(), db.GetID())
		defer vea.PopContext()
		if desc.LocalityConfig == nil {
			vea.ReportPG(catalog.NotSet, pgcode.InvalidTableDefinition,
				"is multi-region enabled, but table has no locality set")
			return nil
		}
		...
		if !db.IsMultiRegion() {
			...
			vea.ReportPGf(unexpectedLocality, pgcode.InvalidTableDefinition,
				"is not multi-region enabled, but table has locality %q set", locality)
			return nil
		}
		...
		desc, err := vdg.GetTypeDescriptor(id)
		if err == nil {
			_, err = desc.RegionNames()
		}
		if err != nil {
			func() {
				vea.PushContextf(`multi_region_enum`, "multi-region enum")
				defer vea.PopContext()
				vea.Report(catalog.NotFound, err)
			}()
		}
	...
	case *descpb.TableDescriptor_LocalityConfig_RegionalByRow_:
		return func() bool {
			vea.PushContextf(`locality_config_regional_by_row`, "locality config %q", locality)
			defer vea.PopContext()
			if !desc.IsPartitionAllBy() {
				return !vea.ReportInternalf(catalog.NotSet, "table is missing PARTITION ALL BY clause")
			}
			return true
		}()
	case *descpb.TableDescriptor_LocalityConfig_RegionalByTable_:
		...
		return func() bool {
			vea.PushContextf(`locality_config_regional_by_table`, "locality config %q", locality)
			defer vea.PopContext()
			if lc.RegionalByTable.Region == nil {
				return true
			}
			regions, _ := regionsEnum.RegionNames()
			for _, r := range regions {
				if *lc.RegionalByTable.Region == r {
					return true
				}
			}
			return !vea.Report(catalog.NotFound, errors.WithHintf(
				pgerror.Newf(
					pgcode.InvalidTableDefinition,
					"region not found in multi-region enum %q (%d) from parent database %q (%d)",
					regionsEnum.GetName(), regionsEnum.GetID(), db.GetName(), db.GetID()),
				"available regions: %s",
				strings.Join(regions.ToStrings(), ", ")))
		}()
	default:
		return !vea.ReportPGf(catalog.Unclassified, pgcode.InvalidTableDefinition,
			"unknown locality level: %T", lc)
	}
}
