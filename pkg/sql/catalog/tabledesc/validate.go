...
func (desc *wrapper) validateTableLocalityConfig(
	...
	case *descpb.TableDescriptor_LocalityConfig_RegionalByRow_:
		...
		// For REGIONAL BY ROW tables, ensure partitions in the PRIMARY KEY match the database descriptor.
		// (We do validation that ensures all index partitions are the same on PARTITION ALL BY).
		...
		for _, partitioning := range desc.GetPrimaryIndex().GetPartitioning().List {
			if _, ok := regionNames[descpb.RegionName(partitioning.Name)]; !ok {
				return errors.AssertionFailedf(
			...
			delete(regionNames, descpb.RegionName(partitioning.Name))
		}
		// Any regions that are not deleted from above is extraneous.
