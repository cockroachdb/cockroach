...
func TestReaderCatalogAutoStatsDisabled(t *testing.T) {
	...
	// This test verifies that manual CREATE STATISTICS fails in read-only tenants.
	// The tenant-level read-only detection in create_stats.go prevents stats collection
	// for all tables in PCR reader tenants, including both replicated and non-replicated
	// system tables, since the system.table_statistics table itself is read-only.
}
