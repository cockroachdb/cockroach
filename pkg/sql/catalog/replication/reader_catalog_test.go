...
func TestReaderCatalogTSAdvance(t *testing.T) {
	...
	for _, useAOST := range []bool{false, true} {
		...
		checkAOSTError := func(err error) {
			...
			require.ErrorContains(t, err, "PCR reader timestamp has moved forward, existing descriptor")
			errorDetected = true
		}
		...
		require.Equalf(t, errorDetected, !useAOST, "error was detected unexpectedly")
	}
	now = newTS
