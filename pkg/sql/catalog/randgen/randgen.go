...
func NewTestSchemaGenerator(
	...
	txn *kv.Txn,
	hasAdminRole func(ctx context.Context) (bool, error),
	canCreateDatabase func(context.Context) error,
	checkPriv CheckPrivFn,
	...
	expandTableGlob ExpandTableGlobFn,
	rand *rand.Rand,
) TestSchemaGenerator {
	...
	return &testSchemaGenerator{
		...
		idGen:             idGen,
		expandTableGlob:   expandTableGlob,
		rand:              rand,
...
// ExpandTableGlobFn is the type of a wrapper around sql.expandTableGlob().
type ExpandTableGlobFn = func(context.Context, tree.TablePattern) ([]descpb.ID, error)

// CheckPrivFn is the type of (*planner).CheckPrivilegeForUser().
type CheckPrivFn = func(context.Context, privilege.Object, privilege.Kind, username.SQLUsername) error
...
type testSchemaGenerator struct {
	...
	// Privilege checkers, provided by the client.
	hasAdminRole      func(context.Context) (bool, error)
	canCreateDatabase func(context.Context) error
	checkPriv         CheckPrivFn
...
func (g *testSchemaGenerator) prepareCounts(
	...
	if len(counts) == 2 {
		...
		if counts[0] > 0 || !g.useGeneratedPublicSchema {
			...
			g.numSchemasPerDatabase = int(counts[0])
	...
	g.numTablesPerSchema = int(counts[0])
