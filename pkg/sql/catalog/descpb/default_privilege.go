...
func (p *DefaultPrivilegeDescriptor) GrantDefaultPrivileges(
	...
	for _, grantee := range grantees {
		...
		defaultPrivileges.DefaultPrivilegesPerObject[uint32(targetObject)] = defaultPrivilegesPerObject
	}
}
...
func CreatePrivilegesFromDefaultPrivileges(
	...
	if !found {
		...
		defaultPrivs := defaultPrivilegesForRole.DefaultPrivilegesPerObject[uint32(targetObject)]
...
func (p DefaultPrivilegeDescriptor) Validate() error {
	for _, defaultPrivilegesForRole := range p.DefaultPrivileges {
		for objectType, defaultPrivileges := range defaultPrivilegesForRole.DefaultPrivilegesPerObject {
			privilegeObjectType := targetObjectToPrivilegeObject[tree.AlterDefaultPrivilegesTargetObject(objectType)]
