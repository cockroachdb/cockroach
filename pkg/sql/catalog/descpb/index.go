...
//
// When allowSubset is false, the index must cover referencedColIDs exactly
// (a permutation match). When true, the index's key columns may be a non-empty
// subset of referencedColIDs; extra referenced columns then become implicit
// equality conditions enforced via a residual filter on the parent lookup.
func (desc *IndexDescriptor) IsValidReferencedUniqueConstraint(
	referencedColIDs ColumnIDs, allowSubset bool,
) bool {
