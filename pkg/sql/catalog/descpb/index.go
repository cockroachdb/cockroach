...
func (desc *IndexDescriptor) FillColumns(elems tree.IndexElemList) error {
	...
	desc.KeyColumnDirections = make([]fetchpb.IndexColumn_Direction, 0, len(elems))
	for _, c := range elems {
		if c.Expr != nil {
		...
		case tree.Ascending, tree.DefaultDirection:
			desc.KeyColumnDirections = append(desc.KeyColumnDirections, fetchpb.IndexColumn_ASC)
		case tree.Descending:
			desc.KeyColumnDirections = append(desc.KeyColumnDirections, fetchpb.IndexColumn_DESC)
		default:
