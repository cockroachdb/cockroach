...
// InvertableColumnName returns the name of the invertable column of the
// inverted index. This is always the last column in ColumnNames. Panics if the
// index is not inverted.
func (desc *IndexDescriptor) InvertableColumnName() string {
	if desc.Type != IndexDescriptor_INVERTED {
		panic(errors.AssertionFailedf("index in not inverted"))
	}
