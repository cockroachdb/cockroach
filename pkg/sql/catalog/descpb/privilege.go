...
func (p PrivilegeDescriptor) ValidateSuperuserPrivileges(
	...
	for _, user := range []security.SQLUsername{
		...
		if !ok {
			return privilegeErrorf("user %s does not have privileges over",
				parentID, objectType, objectName, user)
		}
		...
		if superPriv.Privileges != allowedSuperuserPrivileges.ToBitField() {
			return privilegeErrorf("user %s must have exactly %s privileges on",
				parentID, objectType, objectName, user, allowedSuperuserPrivileges)
		}
	}
	return nil
}
...
func (p PrivilegeDescriptor) Validate(
	...
	if p.Version >= OwnerVersion {
		if p.Owner().Undefined() {
			return errors.HandleAsAssertionFailure(
				privilegeErrorf("found no owner for", parentID, objectType, objectName))
		}
	}

	valid, u, remaining := p.IsValidPrivilegesForObjectType(objectType)
	if !valid {
		return errors.HandleAsAssertionFailure(privilegeErrorf(
			"user %s must not have %s privileges on",
			parentID,
			objectType,
			objectName,
			u.User(),
...
// privilegeError is a helper function for privilege errors.
func privilegeErrorf(
	fmtPrefix string,
	parentID ID,
	objectType privilege.ObjectType,
	objectName string,
	...
	return fmt.Errorf(fmtPrefix+maybeSystem+string(objectType)+" %q", append(args, objectName)...)
}
