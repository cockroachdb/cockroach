...
const (
	...
	// left side that don't match any rows on the right side (as per equality
	...
	// one row from the left side (as per equality columns and ON condition).
	JoinType_RIGHT_SEMI JoinType = 8
	// A right anti join is an "inverted" semi join: it returns the rows from the
	// right side that don't match any rows on the left side (as per equality
	// columns and ON condition).
...
func (JoinType) EnumDescriptor() ([]byte, []int) {
	return fileDescriptor_join_type_bc258021d6bdabac, []int{0}
}
...
func init() {
	proto.RegisterFile("sql/catalog/descpb/join_type.proto", fileDescriptor_join_type_bc258021d6bdabac)
}

var fileDescriptor_join_type_bc258021d6bdabac = []byte{
