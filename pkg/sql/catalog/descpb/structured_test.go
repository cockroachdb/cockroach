...
func TestColumnIDs_ElementWiseEquals(t *testing.T) {
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			result := tt.columnIDsOne.ElementWiseEquals(tt.columnIDsTwo)
			if result != tt.expectedResult {
				t.Errorf("DeepEquals() %s: got %v, want %v", tt.name, result, tt.expectedResult)
			}
		})
	}
}
