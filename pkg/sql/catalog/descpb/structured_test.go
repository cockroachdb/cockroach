...
func TestColumnIDsEquivalent(t *testing.T) {
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			result := tt.columnIDsOne.Equivalent(tt.columnIDsTwo)
			if result != tt.expectedResult {
				t.Errorf("Equivalent() %s: got %v, want %v", tt.name, result, tt.expectedResult)
			}
		})
	}
}
