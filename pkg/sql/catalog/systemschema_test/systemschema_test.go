	...
	datadriven.Walk(t, testutils.TestDataPath(t, "bootstrap"), func(t *testing.T, path string) {
		...
		datadriven.RunTest(t, path, func(t *testing.T, d *datadriven.TestData) string {
			...
			case "schema_telemetry":
				...
				require.EqualValues(t, len(events), 1+meta.NumPages, "unexpected page count")
				for i, event := range events[1:] {
					ev, ok := event.(*eventpb.SchemaSnapshotPage)
					require.Truef(t, ok, "expected a SchemaSnapshotPage event, instead got %T", event)
					require.EqualValues(t, meta.SnapshotID, ev.SnapshotID, "unexpected snapshot ID")
					require.EqualValues(t, i+1, ev.CurrentPage, "unexpected page number")
					...
					require.NoError(t, gogotypes.UnmarshalAny(ev.Desc, desc), "unexpected descriptor unmarshal error")
