...
// DecodedKeyInfo contains decoded key information.
...
func DecodeKey(
	...
	if tableDesc == nil {
		return nil, errors.Newf("table %d not found", tableID)
	}
	...
	if index == nil {
		return nil, errors.Newf("index %d not found on table %s", indexID, tableDesc.GetName())
	}
