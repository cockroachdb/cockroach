...
func builderFromKeyValue(
	...
	if expectedType != catalog.Any && b.TypeName() != expectedType {
		...
		return nil, pgerror.Newf(pgcode.WrongObjectType,
			"descriptor with ID %d is not a %s, instead is a %s", id, expectedType, b.TypeName())
	}
	return b, nil
}
...
func CheckObjectCollision(
	...
	desc, err := GetDescriptorCollidingWithObject(ctx, txn, codec, parentID, parentSchemaID, name)
	if err != nil {
		return err
	}
	if desc != nil {
