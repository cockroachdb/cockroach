...
func GetDescriptorByID(
	...
	dg := txnDescGetter{codec: codec, txn: txn}
	if mutable {
...
// NewDescGetter returns a new DescGetter backed by the passed Txn. It will
// use the transaction to resolve mutable descriptors using GetDescriptorByID
// but will pass a nil DescGetter into those lookup calls to ensure that the
// entire graph of dependencies is not traversed.
func NewDescGetter(txn *kv.Txn, codec keys.SQLCodec) catalog.DescGetter {
	return &txnDescGetter{
		txn:   txn,
		codec: codec,
	}
}

type txnDescGetter struct {
	codec keys.SQLCodec
	txn   *kv.Txn
}

func (t txnDescGetter) GetDesc(ctx context.Context, id descpb.ID) (catalog.Descriptor, error) {
...
var _ catalog.DescGetter = (*txnDescGetter)(nil)
