...
func desiredKindError(desc catalog.Descriptor, kind DescriptorKind, id descpb.ID) error {
	...
	case sqlbase.SchemaDescriptorInterface:
...
func unwrapDescriptor(
	...
	switch {
	case table != nil:
		if err := sqlbase.MaybeFillInDescriptor(ctx, txn, codec, table); err != nil {
			return nil, err
		}
		immTable := sqlbase.NewImmutableTableDescriptor(*table)
...
// unwraps it into a mutable implementation of ExtendedDescriptorInterface. It ensures
...
func unwrapDescriptorMutable(
	...
	case table != nil:
		...
		mutTable := sqlbase.NewMutableExistingTableDescriptor(*table)
...
func GetDatabaseDescByID(
	...
	desc, err := GetDescriptorByID(ctx, txn, codec, id, Immutable, DatabaseDescriptorKind, false)
	if err != nil || desc == nil {
		return nil, err
	}
...
// GetTableDescByID looks up the table descriptor given its ID,
// returning nil if the descriptor is not found. If you want the "not
// found" condition to return an error, use MustGetTableDescByID() instead.
func GetTableDescByID(
	ctx context.Context, txn *kv.Txn, codec keys.SQLCodec, id descpb.ID,
) (*sqlbase.ImmutableTableDescriptor, error) {
	desc, err := GetDescriptorByID(ctx, txn, codec, id, Immutable, TableDescriptorKind, false /* required */)
	if err != nil || desc == nil {
		return nil, err
	}
...
func MustGetTableDescByID(
	...
	desc, err := GetDescriptorByID(ctx, txn, codec, id, Immutable, TableDescriptorKind, true /* required */)
	if err != nil || desc == nil {
		return nil, err
	}
	return desc.(*sqlbase.ImmutableTableDescriptor), nil
...
func MustGetDatabaseDescByID(
	...
	desc, err := GetDescriptorByID(ctx, txn, codec, id, Immutable, DatabaseDescriptorKind, true /* required */)
	if err != nil || desc == nil {
		return nil, err
	}
	return desc.(*sqlbase.ImmutableDatabaseDescriptor), nil
