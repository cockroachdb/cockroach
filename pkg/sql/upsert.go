...
func (p *planner) newUpsertHelper(
	...
	computeExprs []tree.TypedExpr,
	upsertConflictIndex *catpb.IndexDescriptor,
...
func upsertExprsAndIndex(
	...
	updateExprs tree.UpdateExprs,
	conflictIdx *catpb.IndexDescriptor,
	err error,
) {
	if onConflict.IsUpsertAlias() {
		...
		indexColSet := make(map[catpb.ColumnID]struct{}, len(conflictIndex.ColumnIDs))
	...
	indexMatch := func(index catpb.IndexDescriptor) bool {
		if !index.Unique {
			return false
		}
