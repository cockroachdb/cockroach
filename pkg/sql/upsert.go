...
func (p *planner) makeUpsertHelper(
	...
	for i, expr := range evalExprs {
		...
		if len(c.Selector) > 0 ||
			!parser.EqualName(c.TableName.TableName, upsertExcludedTable.TableName) ||
			parser.NormalizeForCompare(c.ColumnName) != parser.ReNormalizeName(updateCols[i].Name) {
			helper.allExprsIdentity = false
			break
		}
	}
...
func upsertExprsAndIndex(
	...
	indexMatch := func(index sqlbase.IndexDescriptor) bool {
		...
		for i, colName := range index.ColumnNames {
			if parser.ReNormalizeName(colName) != parser.NormalizeForCompare(onConflict.Columns[i]) {
				return false
			}
		}
		return true
