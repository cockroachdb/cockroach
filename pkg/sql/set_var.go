...
func datumAsInt(evalCtx *tree.EvalContext, name string, value tree.TypedExpr) (int, error) {
	val, err := value.Eval(evalCtx)
	if err != nil {
		return -1, err
	}
	...
	if !ok {
		return -1, fmt.Errorf("set %s: requires an int value: %s is a %s",
			name, value, val.ResolvedType())
	}
	return int(s), nil
}

func getIntVal(evalCtx *tree.EvalContext, name string, values []tree.TypedExpr) (int, error) {
	if len(values) != 1 {
		return -1, fmt.Errorf("set %s: requires a single int value", name)
	}
