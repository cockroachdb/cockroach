...
func (p *planner) SetVar(ctx context.Context, n *tree.SetVar) (planNode, error) {
	...
	if typedValues == nil {
		...
		if _, ok := p.TopMutator().defaults[name]; !ok && v.GlobalDefault == nil {
			return nil, newCannotChangeParameterError(name)
		}
	}
...
func (n *setVarNode) startExec(params runParams) error {
	...
	if n.typedValues != nil {
		...
		_, strVal = getSessionVarDefaultString(n.name, n.v, params.p.TopMutator())
	}
	...
	if n.local {
		...
		return applyFunc(params.p.TopMutator())
	}
