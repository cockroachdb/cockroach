...
func (ex *connExecutor) txnStateTransitionsApplyWrapper(
	...
	case txnCommit:
		...
		// If we updated any databases, release the old leases on this node.
		if err := ex.extraTxnState.descCollection.ReleaseOldDatabaseLeases(
			ex.ctxHolder.connCtx,
		); err != nil {
			// TODO (lucy): Should we swallow this error?
			return advanceInfo{}, err
		}
