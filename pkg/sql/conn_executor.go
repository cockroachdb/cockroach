...
func (ex *connExecutor) execStmt(
	...
	if _, ok := stmt.AST.(tree.ObserverStatement); ok {
		err := ex.runObserverStatement(ex.Ctx(), stmt, res)
		return nil, nil, err
	}

	if log.V(2) || logStatementsExecuteEnabled.Get(&ex.server.cfg.Settings.SV) ||
		log.HasSpanOrEvent(ex.Ctx()) {
		log.VEventf(ex.Ctx(), 2, "executing: %s in state: %s", stmt, ex.machine.CurState())
	}
	...
	case stateOpen:
		...
		if filter := ex.server.cfg.TestingKnobs.StatementFilter; err == nil && filter != nil {
			...
			filter(ex.Ctx(), stmt.String(), execErr)
		}
	...
	case stateAborted, stateRestartWait:
		ev, payload = ex.execStmtInAbortedState(ex.Ctx(), stmt, res)
	case stateCommitWait:
...
func (ex *connExecutor) handleAutoCommit(stmt tree.Statement) error {
	...
	log.VEventf(ex.Ctx(), 2, "AutoCommit. err: %v", err)
	if err != nil {
		return err
	}
	return nil
...
func (ex *connExecutor) txnStateTransitionsApplyWrapper(
	...
	case txnAborted:
		...
		// Close all the portals. The pgwire protocol says they live until the end
		// of the transaction.
		// !!! if I do this a test fails. If I don't, it's bad.
		// ex.closeAllPortals(ex.Ctx())
	default:
