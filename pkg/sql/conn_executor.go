...
func (ex *connExecutor) txnStateTransitionsApplyWrapper(
	...
	case txnStart:
		// Execute the transaction BeforeStart testhook, if one exists.
		if beforeStart := ex.server.cfg.TestingKnobs.BeforeStart; beforeStart != nil {
			beforeStart(ex.server.cfg)
		}
