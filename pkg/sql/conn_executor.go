...
type connExecutor struct {
	...
	// is used to record statistics about transactions and is stored here
	// separately from the state so that txn in state could be safely reset and
	// the statistics could be recorded afterwards.
	txnInfo struct {
...
func (ex *connExecutor) txnStateTransitionsApplyWrapper(
	...
	// After txn is finished, we record the statistics about it.
	switch advInfo.txnEvent {
	case txnCommit, txnAborted:
		ex.recordTransactionStatistics(advInfo.txnEvent)
	}
