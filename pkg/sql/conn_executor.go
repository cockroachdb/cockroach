...
// just like more successful queries do and they produce inputEvents for the
// state machine just like the successful queries (the events in question
// generally type retriableErr/nonRetriableErr and they generally cause the
...
// of that txn. When there's no SQL transaction (i.e. txnState.sts.topLevelState
// == NoTxn), everything happens in the context of the connection. TODO(andrei):
// mention sessionEventf().
...
// pattern of taking an explicit context is used.
...
type connExecutor struct {
	...
	clientComm clientComm
	// state is the connection's state machine.
	state txnState2
	...
	// This field is only maintained while the transaction is in the Open state.
	//
...
// connExecutor implements rewindInterface; that's how txnState calls back into
// the connExecutor for its limited needs.
var _ rewindInterface = &connExecutor{}

func (ex *connExecutor) Ctx() context.Context {
	if ex.state.sts.topLevelState != StateNoTxn {
		return ex.state.Ctx
	}
	return ex.connCtx
}
...
func (ex *connExecutor) run(ctx context.Context) error {
	...
	for {
		...
		inputEvent, err := ex.execStmt(stmt, res, pinfo, pos)
		if err != nil {
			return err
		}
		// Feed the event to the state machine.
		advInfo, err := ex.state.performStateTransition(
			ctx, inputEvent, ex, pos, ex.transitionCtx)
		if err != nil {
			return err
		}
		...
		// We flush the results if either we're in anything but the Open state (as
		// there's no question of automatic retries) or if flush is set in
		// which case the statement execution told us to advance the point that we
		// consider the "start of the transaction".
		if ex.state.sts.topLevelState != StateOpen || advInfo.flush {
			...
			if advInfo.flush {
				...
				// rewound to.
				npos, err := ex.stmtBuf.nextPos(pos)
				if err != nil {
					return err
				}
				ex.setTxnStartPos(ex.Ctx(), npos)
			}
		}
	}
}
...
// state. Returns an inputEvent for the state machine.
//
// If an error is returned, the session is supposed to be consider done. Query
// execution errors are not returned explicitly; they're incorporated in the
// state transition and they have already been communicated to the client
// through res. res is closed.
//
// Args:
...
func (ex *connExecutor) execStmt(
	...
	if _, ok := stmt.AST.(*tree.ShowTransactionStatus); ok {
		if err := ex.runShowTransactionState(res); err != nil {
	...
	var ev inputEvent
	var err error
	switch ex.state.sts.topLevelState {
	case StateNoTxn:
		ev = ex.execStmtInNoTxnState(ex.Ctx(), stmt, pos)
		if ev.errDetails.err != nil {
			err = res.CloseResultWithError(ev.errDetails.err)
		}
	case StateOpen:
		ev, err = ex.execStmtInOpenState(ex.Ctx(), stmt, pinfo, pos, res)
		// Nothing further to do unless the query execution was uneventful. Note
		// that if the execution resulted in an error, res has been already closed.
		if err != nil || ev.typ != noTopLevelTransition {
			break
		}
		if ex.state.txnType == implicitTxn {
			ev = ex.handleAutoCommit(stmt.AST)
		}
		if ev.typ == noTopLevelTransition {
			ev2, restart := ex.handleSerializablePushMaybe(ex.Ctx(), stmt.AST)
			if restart {
				ev = ev2
			}
		}
	...
	case StateAborted, StateRestartWait:
		...
		ev = ex.execStmtInAbortedState(stmt, res)
	case StateCommitWait:
		// !!! take care of closing res
		ev = ex.execStmtInCommitWaitState(stmt, res)
	default:
		panic(fmt.Sprintf("unexpected txn state: %s", ex.state.sts))
	}
...
func (ex *connExecutor) runShowTransactionState(res StatementResult) error {
	...
	state := ex.state.sts.topLevelState.String()
...
// It advances txnStartPos to pos. It also removes all the previous statements
// from the statementBuffer.
...
// getRewindTxnCapability is part of rewindInterface.
...
// already. It also returns an inputEvent that finishes the SQL txn.
// This is to be called after an "implicit txn" has run.
//
...
// statements could report this back through the inputEvent.
//
// Args:
// stmt: The statement that we just ran.
func (ex *connExecutor) handleAutoCommit(stmt tree.Statement) inputEvent {
	if state := ex.state.sts.topLevelState; state != StateOpen {
		log.Fatalf(ex.Ctx(), "handleAutoCommit called in state: %s", state)
	}
	txn := ex.state.mu.txn
	if txn.IsCommitted() {
		return makeTxnFinishedEvent()
	}
	var skipCommit bool
	var err error
	...
	if err != nil {
		return makeErrEvent(err, txn, stmt)
	}
	return makeTxnFinishedEvent()
}

// handleSerializablePushMaybe takes action in some cases when a serializable
// transaction is detected to have been pushed - it generates an input event for
// a retriable error.
// It returns said event and a bool: true if the inputEvent has been generated,
// false otherwise.
//
...
func (ex *connExecutor) handleSerializablePushMaybe(
	...
	if state := ex.state.sts.topLevelState; state != StateOpen {
		log.Fatalf(ex.Ctx(), "handleSerializablePushMaybe called in state: %s", state)
	}
	...
	if !(canAutoRetry || !ex.state.sts.retryIntent) {
		return inputEvent{}, false
	}
	rewCap = &rc
	defer func() {
		if rewCap != nil {
			rewCap.rewindAndUnlock(ctx)
		}
	}()
	...
	if parErr := ex.synchronizeParallelStmts(ex.Ctx()); parErr != nil {
		...
		if _, ok := parErr.(*roachpb.HandledRetryableTxnError); !ok {
			return makeErrEvent(parErr, ex.state.mu.txn, stmt), true
		}
	...
	ev := makeErrEvent(injectedErr, ex.state.mu.txn, stmt)
	ev.errDetails.rewCap = rewCap
	rewCap = nil
	return ev, true
}
...
// This method doesn't actually execute the statement; it just generates an
// inputEvent that will create a transaction. The statement will then be
// executed again, but this time in the Open state. This is true for both
// explicit and implicit transactions: in the case of an explicit transaction,
// there's still a plan to run for the BEGIN statement and it will run when the
...
func (ex *connExecutor) execStmtInNoTxnState(
	...
	case *tree.BeginTransaction:
		// The first stmt in the txn is considered the next statement. The BEGIN is
		// not to be executed again in case of an auto-retry.
		nextPos, err := ex.stmtBuf.nextPos(stmtPos)
		if err != nil {
			log.Fatal(ctx, err)
		}
		ex.expBeginPos = stmtPos
		return makeBeginTxnEvent(explicitTxn, nextPos)
	...
	case *tree.Savepoint:
		goto statementNeedsExplicitTxn
	default:
		return makeBeginTxnEvent(implicitTxn, stmtPos)
	}
statementNeedsExplicitTxn:
	return makeErrEvent(errNoTransactionInProgress, ex.state.mu.txn, stmt.AST)
}
...
// the returned inputEvent. The result is Close()d if the returned event is
// anything but a noTopLevelTransition event; in such cases, the caller doesn't
// need to do anything further with the result. However, in case the returned
// event is of type noTopLevelTransition, then it is the caller's responsibility
// to Close() it. This is because in cases where the query execution is
// uneventful, the caller may want to contribute to the result by performing an
// auto-commit or such.
//
// Args:
// stmt: The statement to execute.
...
func (ex *connExecutor) execStmtInOpenState(
	...
	makeErrEventAndCloseRes := func(err error) (inputEvent, error) {
		if err := res.CloseResultWithError(err); err != nil {
			return inputEvent{}, err
		}
		return makeErrEvent(err, ex.state.mu.txn, stmt.AST), nil
	}
	...
	case *tree.Savepoint:
		...
		return makeNoTopLevelTransitionEvent(retryIntentSet, flushSpecialTxnPrefix), nil

	case *tree.RollbackToSavepoint:
		if err := tree.ValidateRestartCheckpoint(s.Savepoint); err != nil {
			return makeErrEventAndCloseRes(err)
		}
		if !ex.state.sts.retryIntent {
	...
	runInParallel := parallelize && ex.state.txnType == explicitTxn
	if runInParallel {
	...
	if runInParallel {
		...
		p.autoCommit = ex.state.txnType == implicitTxn && !ex.server.cfg.TestingKnobs.DisableAutoCommit
		...
		if resOrErr.err != nil {
			...
			return makeErrEvent(resOrErr.err, ex.state.mu.txn, stmt.AST), nil
		}
	}
...
func (ex *connExecutor) commitSQLTransaction(
	...
	if state := ex.state.sts.topLevelState; state != StateOpen {
		log.Fatalf(ctx, "commitSQLTransaction called in state: %s", state)
	}
	...
	if err := ex.state.mu.txn.Commit(ex.Ctx()); err != nil {
		...
		return makeErrEvent(err, ex.state.mu.txn, stmt), nil
	}
	...
	case commit:
		...
		return makeTxnFinishedEvent(), nil
	case release:
		if err := res.CloseResultWithError(nil); err != nil {
			return inputEvent{}, err
		}
		return makeTxnReleasedEvent(), nil
	}
	panic("unreached")
}
...
func (ex *connExecutor) rollbackSQLTransaction(
	...
	if state := ex.state.sts.topLevelState; state != StateOpen && state != StateRestartWait {
		log.Fatalf(ctx, "rollbackSQLTransaction called on txn in wrong state: %s", ex.state.sts)
	}
...
func (ex *connExecutor) execStmtInAbortedState(stmt Statement, res StatementResult) inputEvent {
	panic("!!! unimplemented")
}

func (ex *connExecutor) execStmtInCommitWaitState(stmt Statement, res StatementResult) inputEvent {
	panic("!!! unimplemented")
}
