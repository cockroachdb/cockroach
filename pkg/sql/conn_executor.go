...
func (s *Server) IncrementConnectionCount(
...
) (decrementConnectionCount func(), err error) {
	...
	maxNumNonRootConnectionsReason := maxNumNonRootConnectionsReason.Get(sv)
	...
	if maxNumNonRootConnectionsExceeded {
		err = errors.WithHintf(
			pgerror.Newf(pgcode.TooManyConnections, "%s", maxNumNonRootConnectionsReason),
			"the maximum number of allowed connections is %d",
			maxNumNonRootConnectionsValue,
		)
	} else if maxNumConnectionsExceeded {
		err = errors.WithHintf(
