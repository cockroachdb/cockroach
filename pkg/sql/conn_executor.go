...
// TODO(azhng): now that since ResetSQLStats can potentially return an error, we need to update the RPC endpoint
//  to handle this.
...
func (s *Server) ResetSQLStats(ctx context.Context) {
	...
	if err != nil {
		log.Errorf(ctx, "an error has occurred when SQL stats is being dumped into reported stats: %s", err)
	}
}

// ResetReportedStats resets the executor's collected reported stats.
func (s *Server) ResetReportedStats(ctx context.Context) {
	// Since we are not dumping to target, there would be no memory allocation. Hence we cannot run into errors.
	_ = s.reportedStats.resetAndMaybeDumpStats(ctx, nil /* target */)
}
