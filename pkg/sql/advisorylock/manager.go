...
// LockIsNotAvailable is the error returned when a lock is not available,
// and we are not allowed to wait for it.
var LockIsNotAvailable = errors.New("lock is not available")
...
const (
	...
	lockKeyTypeSingle lockKeyType = iota
...
// Manager is responsible for accquiring and releasing advisory locks, depending
...
func (m *Manager) getBaseKey(ctx context.Context, txn *kv.Txn) (roachpb.Key, error) {
	...
	baseKey := m.codec.IndexPrefix(uint32(advisoryLockTable.GetID()), uint32(advisoryLockTable.GetPrimaryIndexID()))
	baseKey = keys.MakeFamilyKey(baseKey, 0)
	return baseKey, nil
}
