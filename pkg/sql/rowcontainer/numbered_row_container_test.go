...
func generateLookupJoinAccessPattern(rng *rand.Rand, numRightRows int, repeatCount int) [][]int {
	...
	for i := 0; i < len(out); i++ {
		...
		// TODO(sumeer): confirm that the right rows will usually be ordered by
		// the lookup columns.
...
func BenchmarkNumberedContainerIteratorCaching(b *testing.B) {
	...
	// accesses per left row, there are a total of 10000 accesses, so each of the
	// 2500 right rows that are accessed is on average accessed 4 times.
