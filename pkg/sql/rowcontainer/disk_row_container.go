...
func MakeDiskRowContainer(
	...
	for i := range d.types {
		...
		if _, ok := orderingIdxs[i]; !ok || sqlbase.HasCompositeKeyEncoding(d.types[i].SemanticType) || sqlbase.MustBeValueEncoded(d.types[i].SemanticType) {
...
// ChangeOrdering implements SortableRowContainer. It creates a new
...
func (d *DiskRowContainer) ChangeOrdering(
	ctx context.Context, ordering sqlbase.ColumnOrdering,
) (SortableRowContainer, error) {
	// We need to create a new DiskRowContainer since it's ordering can only be
	...
	// TODO(yuzefovich): should it be called here?
	d.Close(ctx)
...
func (d *DiskRowContainer) keyValToRow(k []byte, v []byte) (sqlbase.EncDatumRow, error) {
	for i, orderInfo := range d.ordering {
		// Types with composite key encodings and types that must be value encoded
		// are decoded from the value.
		if sqlbase.HasCompositeKeyEncoding(d.types[orderInfo.ColIdx].SemanticType) || sqlbase.MustBeValueEncoded(d.types[orderInfo.ColIdx].SemanticType) {
