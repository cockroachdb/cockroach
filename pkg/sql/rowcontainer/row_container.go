...
type DeDupingRowContainer interface {
	...
	// added.
	AddRowWithDeDup(context.Context, sqlbase.EncDatumRow) (uint64, error)
	// UnsafeReset resets the container, allowing for reuse. It renders all
	// previously allocated rows unsafe.
	UnsafeReset(context.Context) error
...
// De-duplication is a special feature of the DiskBackedRowContainer that
// makes it behave as an implementation of DeDupingRowContainer. It should not
// be mixed with calls to AddRow(). It de-duplicates the keys such that only
// the first row with the given key will be stored. The index returned in
...
// duplication in constructing another row container.
