...
func (f *DiskBackedIndexedRowContainer) GetRow(pos int) (tree.IndexedRow, error) {
		...
		for ; ; f.diskRowIter.Next() {
			...
			if f.idxRowIter == f.nextPosToCache {
				...
				if rowIdx, ok := rowWithIdx[len(rowWithIdx)-1].Datum.(*tree.DInt); ok {
					...
					return nil, errors.Errorf("unexpected last column: should be integer pos")
				}
				if f.idxRowIter == pos {
	...
	if rowIdx, ok := rowWithIdx[len(rowWithIdx)-1].Datum.(*tree.DInt); ok {
		return IndexedRow{int(*rowIdx), rowWithIdx[:len(rowWithIdx)-1]}, nil
	}
	return nil, errors.Errorf("unexpected last column: should be integer pos")
}
...
func (f *DiskBackedIndexedRowContainer) getRowWithoutCache(pos int) tree.IndexedRow {
	if !f.UsingDisk() {
	...
	for ; ; f.diskRowIter.Next() {
		...
		if f.idxRowIter == pos {
			...
			row := rowWithIdx[:len(rowWithIdx)-1]
			if rowIdx, ok := rowWithIdx[len(rowWithIdx)-1].Datum.(*tree.DInt); ok {
				return IndexedRow{int(*rowIdx), f.rowAlloc.CopyRow(row)}
			}
