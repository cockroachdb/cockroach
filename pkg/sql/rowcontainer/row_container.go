			...
			if f.idxRowIter == f.nextPosToCache {
				...
				if idx, ok := rowIdx.(*tree.DInt); ok {
					if f.indexedRowsCache.Len() == f.maxCacheSize {
						...
						// f.firstCachedRowPos.
						if err := f.reuseFirstRowInCache(ctx, int(*idx), row, nil /* err */); err != nil {
							return nil, err
						}
					} else {
						...
						if err := f.cacheMemAcc.Grow(ctx, usage); err != nil {
							if sqlbase.IsOutOfMemoryError(err) {
								...
								if err := f.reuseFirstRowInCache(ctx, int(*idx), row, err); err != nil {
									return nil, err
								}
							} else {
								return nil, err
...
//
// The non-nil argument err will be returned if the cache contains zero rows.
// This situation is possible when we're trying to cache the first row of
// improbable big size that exceeds current memory budget.
func (f *DiskBackedIndexedRowContainer) reuseFirstRowInCache(
	ctx context.Context, idx int, row sqlbase.EncDatumRow, err error,
) error {
	newRowSize := row.Size()
	for {
