...
func TestDiskRowContainer(t *testing.T) {
	...
	t.Run("EncodeDecode", func(t *testing.T) {
		for i := 0; i < 100; i++ {
			...
			for _, ordering := range orderings {
				...
				func() {
					d := MakeDiskRowContainer(ctx, diskMonitor, typs, ordering, tempEngine)
					defer d.Close(ctx)
	...
	t.Run("NumberedRowIterator", func(t *testing.T) {
		...
		d := MakeDiskRowContainer(ctx, diskMonitor, types, nil /* ordering */, tempEngine)
		defer d.Close(ctx)
		for i := 0; i < numRows; i++ {
			require.NoError(t, d.AddRow(ctx, rows[i]))
		}
...
func TestDiskRowContainerFinalIterator(t *testing.T) {
	...
	d := MakeDiskRowContainer(ctx, diskMonitor, types.OneIntCol, nil /* ordering */, tempEngine)
	defer d.Close(ctx)
...
func TestDiskRowContainerUnsafeReset(t *testing.T) {
	...
	d := MakeDiskRowContainer(ctx, monitor, types.OneIntCol, nil /* ordering */, tempEngine)
	defer d.Close(ctx)
