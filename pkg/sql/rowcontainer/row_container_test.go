		...
		for i := 0; i < numTestRuns; i++ {
			...
			func() {
				...
				for i := 0; i < numRows; i++ {
					...
					for col, datum := range expectedRow.Row {
						if cmp := readRow.GetDatum(col).Compare(&evalCtx, datum.Datum); cmp != 0 {
...
func (n *sorter) Less(i, j int) bool { return n.Compare(i, j) < 0 }

func (n *sorter) Compare(i, j int) int {
	ra, rb := n.rows.rows[i], n.rows.rows[j]
	for _, o := range n.ordering {
		da := ra.GetDatum(o.ColIdx)
		db := rb.GetDatum(o.ColIdx)
