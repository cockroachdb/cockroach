...
// 0-based dense numbering. Optionally, if deDup is true, it ccan de-duplicate the
...
func NewDiskBackedNumberedRowContainer(
	...
	d.rc.Init(nil, types, evalCtx, engine, memoryMonitor, diskMonitor, rowCapacity)
	if deDup {
		...
		d.deduper = &DiskBackedRowContainer{}
		d.deduper.Init(ordering, types, evalCtx, engine, memoryMonitor, diskMonitor, rowCapacity)
	}
	return d
}
...
// Cache:
//
// The callers of GetRow() know the full pattern of future accesses, represented
// as a [][]int where the int is the row number. Within a slice they access in
// increasing order of index (since forward iteration is cheaper), but across
// different slices there is no such constraint. Knowing the future, one can
// construct an optimal caching policy for a given cache size by keeping track
// of the current reuse distance for each element of the cache and evicting the
// one with the highest reuse distance (and of course, not adding a missed entry
// to the cache if its reuse distance is greater than the highest reuse distance
// currently in the cache). This optimality requires some book-keeping overhead:
//
...
//   iterator such that sequential access was no less costly than random
...
// that the row container contains far more rows than R (the number of unique
...
type numberedDiskRowIterator struct {
	...
	// The current index the rowIter is positioned at.
	idxRowIter uint64
...
func newNumberedDiskRowIterator(
	...
	for _, accSlice := range accesses {
		for _, rowIdx := range accSlice {
			...
			if elem == nil {
				elem = &cacheElement{accesses: []int{accessIdx}}
...
func (n *numberedDiskRowIterator) GetRow(
	...
	var i int
	for i < len(elem.accesses) && elem.accesses[i] <= thisAccessIdx {
		i++
	}
	elem.accesses = elem.accesses[i:]
	var nextAccess int
	...
	// case.
	if idx >= n.idxRowIter && (int(idx-n.idxRowIter) <= n.meanRowsPerSSBlock) {
		...
		for i := int(idx - n.idxRowIter); i > 0; i-- {
			...
			if !ok {
				// This is an row that is never accessed.
				continue
			}
			if preElem.row != nil {
				// Already in cache.
			...
			if err := n.tryAddCache(ctx, n.idxRowIter, preElem, row, false); err != nil {
				return nil, err
			}
		}
		// Try adding to cache
...
func (n *numberedDiskRowIterator) hasNextAccess(idx uint64) *cacheElement {
	elem, ok := n.cache[idx]
	if !ok {
		return nil
	}
...
func (n *numberedDiskRowIterator) tryAddCacheAndReturnRow(
	...
	return row, n.tryAddCache(ctx, idx, elem, row, true)
}
...
func (n *numberedDiskRowIterator) tryAddCache(
	...
	elem *cacheElement,
	row sqlbase.EncDatumRow,
	alreadyDecodedAndCopied bool,
) error {
	if len(elem.accesses) == 0 {
		return nil
	}
	nextAccess := elem.accesses[0]
	usage := int64(row.Size())
	shouldAdd := false
	evict := func() error {
		...
		n.memAcc.Shrink(ctx, int64(evictElem.row.Size()))
		evictElem.row = nil
		return nil
	}
	for {
		...
		if err := n.memAcc.Grow(ctx, usage); err != nil {
