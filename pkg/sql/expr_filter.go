...
// If we have a tuple comparison, we want to try to see if we can use a
// prefix. For example:
//   (a, b, x) > (1, 2, 3)
// only a, b are convertible. We want to split off
//   (a, b) >= (1, 2)
//
// If weaker is false, we want a stronger condition; in this case
//   (a, b, x) >= (1, 2, 3)
// becomes
//   (a, b) > (1, 2, 3)
//
	...
	*newOp = *t
	...
	case 2:
		...
		newOp.Right = &tree.DTuple{
			D:      append(tree.Datums(nil), right.D[:prefix]...),
			Sorted: right.Sorted,
		}
	}
