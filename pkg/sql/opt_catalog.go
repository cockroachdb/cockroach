	...
	for _, col := range cols {
		...
		if !col.IsVirtual() {
			ot.columns[col.Ordinal()].Init(
				...
				mapGeneratedAsIdentityType(col.ColumnDesc().GeneratedAsIdentityType),
			)
		} else {
	...
	for _, sysCol := range ot.desc.SystemColumns() {
		...
		if found == nil || found.IsSystemColumn() {
			...
			col.Init(
				...
				mapGeneratedAsIdentityType(sysCol.ColumnDesc().GeneratedAsIdentityType),
			)
		}
	}
...
func newOptVirtualTable(
	...
	ot.columns[0].Init(
		...
		nil,                        /* defaultExpr */
		nil,                        /* computedExpr */
		cat.NotGeneratedAsIdentity, /* generatedAsIdentityType */
	)
	for i, d := range desc.PublicColumns() {
		ot.columns[i+1].Init(
			...
			mapGeneratedAsIdentityType(d.ColumnDesc().GeneratedAsIdentityType),
		)
	}
