...
type optTable struct {
	...
	outboundFKs []optForeignKeyRef
	inboundFKs  []optForeignKeyRef
	...
	if !ot.desc.IsVirtualTable() {
		...
		for i := range ot.indexes {
			...
			if fk := &idxDesc.ForeignKey; fk.IsSet() {
				ot.outboundFKs = append(ot.outboundFKs, optForeignKeyRef{
			...
			for j := range idxDesc.ReferencedBy {
				...
				ot.inboundFKs = append(ot.inboundFKs, optForeignKeyRef{
					name:            idxDesc.ForeignKey.Name,
...
// OutboundForeignKeyCount is part of the cat.Table interface.
func (ot *optTable) OutboundForeignKey(i int) cat.ForeignKeyReference {
...
func (ot *optTable) InboundForeignKey(i int) cat.ForeignKeyReference {
...
// optForeignKeyRef implements cat.ForeignKeyReference and represents a foreign
// key relationship. Both the origin and the referenced table store the same
// optForeignKeyRef (as an outbound and inbound reference, respectively).
...
var _ cat.ForeignKeyReference = &optForeignKeyRef{}

// Name is part of the cat.ForeignKeyReference interface.
func (fk *optForeignKeyRef) Name() string {
	return fk.name
}

// OriginTableID is part of the cat.ForeignKeyReference interface.
func (fk *optForeignKeyRef) OriginTableID() cat.StableID {
	return fk.originTable
}

// ReferencedTableID is part of the cat.ForeignKeyReference interface.
func (fk *optForeignKeyRef) ReferencedTableID() cat.StableID {
	return fk.referencedTable
}

// ColumnCount is part of the cat.ForeignKeyReference interface.
func (fk *optForeignKeyRef) ColumnCount() int {
	return fk.numCols
}

// OriginColumnOrdinal is part of the cat.ForeignKeyReference interface.
func (fk *optForeignKeyRef) OriginColumnOrdinal(originTable cat.Table, i int) int {
...
// ReferencedColumnOrdinal is part of the cat.ForeignKeyReference interface.
func (fk *optForeignKeyRef) ReferencedColumnOrdinal(referencedTable cat.Table, i int) int {
...
// Validated is part of the cat.ForeignKeyReference interface.
func (fk *optForeignKeyRef) Validated() bool {
	return fk.validity == sqlbase.ConstraintValidity_Validated
}

// MatchMethod is part of the cat.ForeignKeyReference interface.
func (fk *optForeignKeyRef) MatchMethod() tree.CompositeKeyMatchMethod {
