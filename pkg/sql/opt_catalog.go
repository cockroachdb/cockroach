...
func newOptTable(
	...
	for i := range ot.indexes {
		...
		if idxDesc.Type == descpb.IndexDescriptor_INVERTED {
			...
			invertedColumnOrdinal, _ := ot.lookupColumnOrdinal(idxDesc.ColumnIDs[0])

			// ADD VIRTUAL COLUMN
			invertedVirtualColOrd = ot.ColumnCount()
			ot.virtualColumns = append(ot.virtualColumns, optVirtualColumn{
				ordinal: invertedVirtualColOrd,
				name:    string(ot.Column(invertedColumnOrdinal).ColName()) + "_inverted_key",
				...
				typ:                   ot.Column(invertedColumnOrdinal).DatumType(),
				nullable:              false,
				invertedColumnOrdinal: invertedColumnOrdinal,
			})
		}
		ot.indexes[i].init(ot, i, idxDesc, idxZone, invertedVirtualColOrd)
	}
...
func (vc *optVirtualColumn) InvertedColumnOrdinal() int {
	return vc.invertedColumnOrdinal
}
...
// InvertedColumnOrdinal is part of the cat.Column interface.
func (optDummyVirtualPKColumn) InvertedColumnOrdinal() int {
