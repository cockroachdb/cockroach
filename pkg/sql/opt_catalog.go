...
type optTable struct {
	...
	// table. It can be different from desc's constraints because of synthesized
...
func newOptTable(
	...
	for i := range desc.Checks {
		ot.checkConstraints = append(ot.checkConstraints, cat.CheckConstraint{
			Constraint: desc.Checks[i].Expr,
			Validated:  desc.Checks[i].Validity == sqlbase.ConstraintValidity_Validated,
		})
	}
