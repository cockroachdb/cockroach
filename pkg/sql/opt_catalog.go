...
type optTable struct {
	...
	// mutations is a list of mutation column descriptors associated with this
	// table. These are present when the table is undergoing an online schema
	// change where one or more columns are being added or dropped.
...
func (ot *optTable) Column(i int) opt.Column {
	...
	return ot.mutations[i-len(ot.desc.Columns)].Column
}

// MutationColumn is part of the opt.Table interface.
func (ot *optTable) MutationColumn(i int) (col opt.MutationColumn, ok bool) {
	if i < len(ot.desc.Columns) {
		return opt.MutationColumn{}, false
	}
	return ot.mutations[i-len(ot.desc.Columns)], true
}
