...
// CollectTypes is part of the cat.DataSource interface.
func (os *optSequence) CollectTypes(ord int) (map[descpb.ID]struct{}, error) {
...
// CollectTypes is part of the cat.DataSource interface.
func (ot *optTable) CollectTypes(ord int) (map[descpb.ID]struct{}, error) {
...
// CollectTypes is part of the cat.DataSource interface.
func (ot *optVirtualTable) CollectTypes(ord int) (map[descpb.ID]struct{}, error) {
	col := ot.desc.AllColumns()[ord]
	return collectTypes(col)
}
...
func collectTypes(col catalog.Column) (map[descpb.ID]struct{}, error) {
	...
	if col.GetType() != nil && col.GetType().UserDefined() {
		visitor.OIDs[col.GetType().Oid()] = struct{}{}
	}

	ids := make(map[descpb.ID]struct{})
	for collectedOid := range visitor.OIDs {
		ids[typedesc.UserDefinedTypeOIDToID(collectedOid)] = struct{}{}
	}
	return ids, nil
}
