...
var (
	sessionID = uint128.FromInts(42, 0)
	txnID     = uuid.FastMakeV4()
	stmtID    = uint128.FromInts(0, 42)
)
...
func TestOutliersConcurrentAccess(t *testing.T) {
	...
	otherSessionID := sessionID.Add(1)
	otherTxnID := uuid.FastMakeV4()
	otherStmtID := stmtID.Add(1)
	...
	sort.Slice(actual, func(i, j int) bool {
		return uint128.FromBytes(actual[i].Session.ID).Compare(uint128.FromBytes(actual[j].Session.ID)) < 0
	})
