...
type Registry struct {
	...
	mu struct {
		syncutil.RWMutex
		statements map[uint128.Uint128][]*Outlier_Statement
...
func New(st *cluster.Settings) *Registry {
	...
	r := &Registry{st: st}
	r.mu.statements = make(map[uint128.Uint128][]*Outlier_Statement)
...
func (r *Registry) ObserveStatement(
	sessionID uint128.Uint128, statementID uint128.Uint128, latencyInSeconds float64,
) {
	if !r.enabled() {
		return
	}
...
func (r *Registry) ObserveTransaction(sessionID uint128.Uint128, id uuid.UUID) {
	if !r.enabled() {
		return
	}
	r.mu.Lock()
	...
	if hasOutlier {
		for _, s := range statements {
			r.mu.outliers.Add(uint128.FromBytes(s.ID), &Outlier{
				...
				Transaction: &Outlier_Transaction{ID: &id},
				Statement:   s,
			})
		}
	}
