...
func (s *StmtStatProcessor) Process(
	_ context.Context, meta eventagg.FlushMeta, key appstatspb.StmtFingerprintID, val *StmtStatistics,
) error {
	fmt.Printf("FingerprintID: '%s'\tExec Count: %d\tP99 Latency (ms): %f\tStart: %s End %s\n",
		...
		time.Unix(0, meta.StartTime).UTC().String(),
		time.Unix(0, meta.EndTime).UTC().String())
	return nil
}
