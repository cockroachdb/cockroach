...
func TestInsightsIntegrationForContention(t *testing.T) {
	...
	// Note that we shouldn't set this to a low value too early in the test (e.g. before the above queries execute)
	// as this can cause the contention event to be never resolved due to it being assigned the
	// 'retryBudgetForMissingResult' retry budget by the event resolver. This retry budget is assigned when the
	// txn id is still in the txn id cache's write buffer. Due to the contention simulation above we could hit
	// the retry budget before the txn id is flushed and never
	// resolve the event.
	...
	testutils.SucceedsSoon(t, func() error {
		...
		for rows.Next() {
			...
			if math.Abs(diff) > .1 {
				lastErr = fmt.Errorf("contention time from column should be the same as event value\n%s", prettyPrintRow)
				continue
			}
