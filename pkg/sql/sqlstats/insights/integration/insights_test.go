...
func TestInsightsIntegrationForContention(t *testing.T) {
	...
	testutils.SucceedsWithin(t, func() error {
		...
		t.database_name,
		t.name,
		(contention_events->0->>'indexId')::int,
		ind.column_name,
		...
		for rows.Next() {
			...
			err = rows.Scan(&queryText, &totalContentionFromQuerySeconds, &contentionFromEventMs, &schemaName, &dbName, &tableName, &indexID, &indexColumnName, &blockingTxnFingerprintID)
			if err != nil {
				return err
			}
			...
			if indexID != 1 {
				return fmt.Errorf("index names do not match 'tableName', %d", indexID)
			}
