...
func TestInsightsIntegration(t *testing.T) {
	...
	_, err = conn.ExecContext(ctx, "SELECT pg_sleep($1)", (latencyThreshold / 2).Seconds())
	require.NoError(t, err)
	...
	testutils.SucceedsWithin(t, func() error {
		...
		for rows.Next() {
			var decodedPlanGist string
			err = rows.Scan(&decodedPlanGist)
			require.NoError(t, err)
			require.NotNil(t, decodedPlanGist)
		}
