...
func TestInsightsIntegration(t *testing.T) {
	...
	testutils.SucceedsWithin(t, func() error {
		...
		var cpuNanos int64
		err = row.Scan(&query, &status, &startInsights, &endInsights, &fullScan, &implicitTxn, &cpuNanos)
	...
	testutils.SucceedsWithin(t, func() error {
		...
		var implicitTxn bool
		var cpuNanos int64
		err = row.Scan(&query, &startInsights, &endInsights, &implicitTxn, &cpuNanos)
		...
		if cpuNanos < 0 || (cpuNanos > (int64(maxCPUMs) * 1e6)) {
			return fmt.Errorf("expected cpuNanos to be between zero and %f ms, but was %d", maxCPUMs, cpuNanos)
		}
