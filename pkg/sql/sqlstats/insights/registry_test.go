...
func TestRegistry(t *testing.T) {
	...
	t.Run("retention", func(t *testing.T) {
		...
		// With the MaxPerNode set to 5, we retain the 5 most recently-seen insights.
		MaxPerNode.Override(ctx, &st.SV, 5)
		for id := 0; id < 10; id++ {
			observeStatementExecution(r, uint64(id), slow)
		}
		assertInsightStatementIDs(t, r, []uint64{9, 8, 7, 6, 5})

		// Lowering the MaxPerNode requires having a new insight to evict the others.
		MaxPerNode.Override(ctx, &st.SV, 2)
		assertInsightStatementIDs(t, r, []uint64{9, 8, 7, 6, 5})
