...
func TestRegistry(t *testing.T) {
	...
	t.Run("slow detection", func(t *testing.T) {
		...
		expected := []*Insight{{
			Session:     session,
			Transaction: makeTxnInsight(txns[0].txn),
	...
	t.Run("failure detection", func(t *testing.T) {
		...
		// when the expectedTxnInsight does not have this information.
		txn := &sqlstats.RecordedTxnStats{TransactionID: uuid.MakeV4()}
		...
		// if the expectedTxnInsight committed. We'll inject the failure here to align
	...
	t.Run("disabled", func(t *testing.T) {
		transaction := &sqlstats.RecordedTxnStats{TransactionID: uuid.MakeV4()}
	...
	t.Run("buffering statements per session", func(t *testing.T) {
		...
		expected := []*Insight{{
			...
			Transaction: makeTxnInsight(txns[1].txn),
			Statements: []*Statement{
				{
...
func assertInsightsEqual(t *testing.T, actual []*Insight, expected []*Insight) {
	...
	for i, insight := range actual {
		...
		for j, statement := range insight.Statements {
			require.Equal(t, expected[i].Statements[j].ID, statement.ID)
