...
func (s *Container) observeInsightStatement(
	...
	s.insights.ObserveStatement(value.SessionID, &insights.Statement{
		...
		ErrorCode:            errorCode,
		IdleLatSeconds:       value.IdleLatencySec,
		ParseLatSeconds:      value.ParseLatencySec,
		PlanLatSeconds:       value.PlanLatencySec,
		RunLatSeconds:        value.RunLatencySec,
		ServiceLatSeconds:    value.ServiceLatencySec,
...
func (s *Container) observerInsightTransaction(
	...
	s.insights.ObserveTransaction(value.SessionID, &insights.Transaction{
		...
		CPUSQLNanos:       cpuSQLNanos,
		ServiceLatSeconds: &value.ServiceLatency,
		RetryLatSeconds:   &value.RetryLatency,
		CommitLatSeconds:  &value.CommitLatency,
		IdleLatSeconds:    &value.IdleLatency,
		BytesRead:         value.BytesRead,
		Stats: &insights.ExecStats{
			NetworkBytesSent:    value.ExecStats.NetworkBytesSent,
			MaxMemUsage:         value.ExecStats.MaxMemUsage,
