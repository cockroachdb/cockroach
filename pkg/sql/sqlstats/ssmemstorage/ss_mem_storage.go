...
type StatsContainer struct {
	...
	mu struct {
		...
		// and txns within this StatsContainer struct.
		// Since currently we do not destroy the StatsContainer struct when we perform
...
var _ sqlstats.Writer = &StatsContainer{}
...
// stored in this StatsContainer.
func (s *StatsContainer) IterateStatementStats(
	...
	for _, stmtKey := range stmtKeys {
		...
		statementStats, _, _ :=
			s.getStatsForStmtWithKey(stmtKey, invalidStmtID, false /* createIfNonexistent */)
...
// stored in this StatsContainer.
func (s *StatsContainer) IterateTransactionStats(
...
func (s *StatsContainer) IterateAggregatedTransactionStats(
...
func (s *StatsContainer) GetStatementStats(
	key *roachpb.StatementStatisticsKey,
) (*roachpb.CollectedStatementStatistics, error) {
	statementStats, _, stmtID, _, _ := s.getStatsForStmt(
...
func (s *StatsContainer) GetTransactionStats(
	appName string, key sqlstats.TransactionFingerprintID,
) (*roachpb.CollectedTransactionStatistics, error) {
	txnStats, _, _ :=
		s.getStatsForTxnWithKey(key, nil /* stmtIDs */, false /* createIfNonexistent */)
...
// for the given stmt.
func (s *StatsContainer) getStatsForStmt(
	...
	stats, _, _ = s.getStatsForStmtWithKey(key, invalidStmtID, false /* createIfNonexistent */)
	if stats == nil {
		stmtID = constructStatementIDFromStmtKey(key)
		stats, created, throttled = s.getStatsForStmtWithKey(key, stmtID, createIfNonexistent)
		return stats, key, stmtID, created, throttled
	}
	return stats, key, stats.ID, false /* created */, false /* throttled */
}

func (s *StatsContainer) getStatsForStmtWithKey(
...
func (s *StatsContainer) getStatsForTxnWithKey(
...
func (s *StatsContainer) SaveToLog(ctx context.Context, appName string) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if len(s.mu.stmts) == 0 {
		return
...
// Clear clears the data stored in this StatsContainer.
func (s *StatsContainer) Clear(ctx context.Context) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
// ShallowClone returns a shallow copy of the current stats container. It does
// not perform deep copy of the underlying map data.
func (s *StatsContainer) ShallowClone() *StatsContainer {
	s.mu.Lock()
	defer s.mu.Unlock()
	cloned := &StatsContainer{
		st:                         s.st,
		uniqueStmtFingerprintLimit: s.uniqueStmtFingerprintLimit,
		uniqueTxnFingerprintLimit:  s.uniqueTxnFingerprintLimit,
	}

	cloned.atomic.uniqueStmtFingerprintCount = s.atomic.uniqueStmtFingerprintCount
	cloned.atomic.uniqueTxnFingerprintCount = s.atomic.uniqueTxnFingerprintCount

	cloned.mu.acc = s.mu.acc
	cloned.mu.stmts = s.mu.stmts
	cloned.mu.txns = s.mu.txns

	copyTxnCounts(&s.txnCounts, &cloned.txnCounts)
	return cloned
}

// Add combines one StatsContainer into another. Add manages locks on a, so taking
// a lock on a will cause a deadlock.
func (s *StatsContainer) Add(ctx context.Context, other *StatsContainer) (err error) {
	other.mu.Lock()
	...
	for k, v := range txnMap {
		...
		// function.
		t, created, throttled := s.getStatsForTxnWithKey(k, v.statementIDs, true /* createIfNonExistent */)
...
func (s *StatsContainer) recordTransactionCounts(txnTimeSec float64, commit bool, implicit bool) {
	if !sqlstats.TxnStatsEnable.Get(&s.st.SV) {
		return
	}
	s.txnCounts.recordTransactionCounts(txnTimeSec, commit, implicit)
}
...
func (s *StatsContainer) shouldSaveLogicalPlanDescription(stats *stmtStats) bool {
...
func copyTxnCounts(from, to *transactionCounts) {
	from.mu.Lock()
	to.mu.Lock()
	defer from.mu.Unlock()
	defer to.mu.Unlock()

	to.mu.TxnStats = from.mu.TxnStats
	to.mu.ImplicitCount = from.mu.ImplicitCount
	to.mu.TxnCount = from.mu.TxnCount
	to.mu.TxnTimeSec = from.mu.TxnTimeSec
}
