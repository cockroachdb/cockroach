...
func (s *Container) Add(ctx context.Context, other *Container) (err error) {
	...
	for k, v := range statMap {
		statCopy := func() *stmtStats {
			...
			statCopy.mu.fullScan = v.mu.fullScan
			statCopy.mu.distSQLUsed = v.mu.distSQLUsed
			statCopy.mu.vectorized = v.mu.vectorized
			statCopy.mu.querySummary = v.mu.querySummary
			return statCopy
		}()
