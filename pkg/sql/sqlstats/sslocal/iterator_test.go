...
func TestSQLStatsIteratorWithTelemetryFlush(t *testing.T) {
	...
	// First we collect all the statement fingerprint ID so that we can test the
		...
		require.NoError(t,
			sqlStats.IterateStatementStats(ctx, &sqlstats.IteratorOptions{},
				func(_ context.Context, statistics *roachpb.CollectedStatementStatistics) error {
					...
					// If we are running out test case, we reset the SQL Stats. The iterator
	...
	t.Run("transaction_iterator", func(t *testing.T) {
		...
		require.NoError(t, sqlStats.IterateTransactionStats(ctx, &sqlstats.IteratorOptions{}, func(ctx context.Context, id roachpb.TransactionFingerprintID, statistics *roachpb.CollectedTransactionStatistics) error {
			require.NotNil(t, statistics)
