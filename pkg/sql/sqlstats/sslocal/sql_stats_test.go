...
func TestTransactionLatencies(t *testing.T) {
	...
	verifyTxnStats := func(t *testing.T, lastTxnStats *sqlstats.RecordedTxnStats, expectedIdleLatency time.Duration, totalTxnTime time.Duration) {
		...
		// Note(alyshan): Most test runs the difference is < 1%, but under stress it can be slightly > 1%.
		require.InDelta(t, totalTxnTime.Seconds(), lastTxnStats.TransactionTimeSec, totalTxnTime.Seconds()*0.02)
	}
