...
func TestNodeLocalInMemoryViewDoesNotReturnPersistedStats(t *testing.T) {
	...
	sqlstatstestutil.WaitForStatementStatsCountAtLeast(t, obsConn, 1, withApp1)
...
func TestFingerprintCreation(t *testing.T) {
	...
	t.Run("test hide constants", func(t *testing.T) {
		...
		for _, tc := range testCases {
			...
			sqlstatstestutil.WaitForStatementStatsCountAtLeast(
...
func TestSQLStatsIdleLatencies(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			sqlstatstestutil.WaitForStatementStatsCountAtLeast(t, obsConn, len(tc.stmtLats), appFilter)
...
func TestSQLStatsIndexesUsed(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			sqlstatstestutil.WaitForStatementStatsCountEqual(t, testConn, 1, filter)
...
func TestSQLStatsLatencyInfo(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			sqlstatstestutil.WaitForStatementStatsCountAtLeast(t, testConn, 1, filter)
...
func TestSQLStatsRegions(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			sqlstatstestutil.WaitForStatementStatsCountEqual(t, obsConn, 1, appFilter)
...
func TestSQLStatsInternalStatements(t *testing.T) {
	...
	getStmtRow := func(appName string, attributedToUser bool) (sampledCnt int) {
		...
		sqlstatstestutil.WaitForStatementStatsCountEqual(t, conn, 1, filter)
		row := conn.QueryRow(t, `
SELECT
...
func TestSQLStatsDiscardStatsOnFingerprintLimit(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.stmtLimit == 0 {
				sqlstatstestutil.WaitForStatementStatsCountAtLeast(t, utilConn, tc.minStmts)
			} else {
				sqlstatstestutil.WaitForStatementStatsCountEqual(t, utilConn, tc.stmtLimit-1,
					sqlstatstestutil.StatementFilter{AllowInternal: true})
			}
			if tc.txnLimit == 0 {
				sqlstatstestutil.WaitForTransactionStatsCountAtLeast(t, utilConn, tc.minTxns,
					sqlstatstestutil.TransactionFilter{AllowInternal: true})
			} else {
				sqlstatstestutil.WaitForTransactionStatsCountEqual(t, utilConn, tc.txnLimit-1)
			}
			testutils.SucceedsSoon(t, func() error {
