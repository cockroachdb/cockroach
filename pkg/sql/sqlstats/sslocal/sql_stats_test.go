...
func makeRandomStatementStat() sqlstats.RecordedStmtStats {
	...
	return sqlstats.RecordedStmtStats{
		...
		AutoRetryReason:      fmt.Errorf("%s", randutil.RandString(rng, rand.Intn(50), randutil.PrintableKeyAlphabet)),
		...
		StatementError:       fmt.Errorf("%s", randutil.RandString(rng, rand.Intn(50), randutil.PrintableKeyAlphabet)),
		...
		StartTime:            timeutil.Now().Add(-2 * time.Second),
