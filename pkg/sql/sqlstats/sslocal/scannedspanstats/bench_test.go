				...
				for _, m := range []int{100, 500} {
					b.Run(fmt.Sprintf("scanning %d tables with %d rows", n, m), func(b *testing.B) {
						...
						for _, test := range []testSpec{
							{collectSpanStats: false},
							{collectSpanStats: true},
							{collectSpanStats: true,
								maxScannedSpanStats: 500},
						} {
							var name strings.Builder
							name.WriteString("CollectScannedSpanStats=")
							if test.collectSpanStats {
								name.WriteString("on")
								if test.maxScannedSpanStats != 0 {
							...
							b.Run(name.String(), func(b *testing.B) {
								...
								sqlRunner.Exec(b, `SET CLUSTER SETTING sql.metrics.statement_details.scanned_span_stats_collection.enabled = $1`, test.collectSpanStats)
								if test.maxScannedSpanStats != 0 {
