...
func TestSQLStatsScannedSpanStats(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:      "select two columns from table 2",
			statement: "SELECT j, n FROM t2",
		},
		{
		...
		{
			name:      "left join from table 1, table 2",
			statement: "SELECT * FROM t1 JOIN t2 ON t1.k = t2.j",
		},
	}
