...
type SpanStatsCache struct {
	...
	mu struct {
		...
		statsMap map[int64]spanStatsInfo
...
func NewSpanStatsCache(setting *cluster.Settings) *SpanStatsCache {
	...
	spanStatsCache.mu.statsMap = make(map[int64]spanStatsInfo)
...
// and if no span stats have been collected for the fingerprint in the past an hour.
...
func (spanStats *SpanStatsCache) ShouldCollectSpanStatsForFingerprint(
	fingerprint string, tableIDs []int64,
) bool {
...
func (spanStats *SpanStatsCache) ShouldCollectSpanStatsForTable(tableID int64) bool {
...
func (spanStats *SpanStatsCache) UpdateSpanStatistics(
	tableID int64, stats enginepb.MVCCStats, reset bool,
) enginepb.MVCCStats {
...
func (spanStats *SpanStatsCache) UpdateFingerprintInfo(
	fingerprint string, tableIDs []int64, statsArray []enginepb.MVCCStats, reset bool,
) []enginepb.MVCCStats {
...
func (spanStats *SpanStatsCache) getSpanStatsInfoFromTableID(key int64) (spanStatsInfo, bool) {
	spanStats.mu.RLock()
	defer spanStats.mu.RUnlock()

	statsInfo, found := spanStats.mu.statsMap[key]
...
func (spanStats *SpanStatsCache) getOrCreateSpanStatistics(key int64) (spanStatsInfo, bool) {
	statsInfo, found := spanStats.getSpanStatsInfoFromTableID(key)
	if found {
	...
	spanStats.mu.statsMap[key] = statsInfo

	return statsInfo, true
}

func (spanStats *SpanStatsCache) setSpanStatistics(
	key int64, time time.Time, stats enginepb.MVCCStats,
) {
	_, found := spanStats.getOrCreateSpanStatistics(key)

	if found {
		spanStats.mu.Lock()
		defer spanStats.mu.Unlock()

		spanStats.mu.statsMap[key] = spanStatsInfo{
...
func (spanStats *SpanStatsCache) setFingerprintInfo(
	fingerprint string, tableIDs []int64, time time.Time, executionCount int64,
) {
