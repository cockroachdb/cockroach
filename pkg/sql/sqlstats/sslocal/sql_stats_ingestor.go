...
func (i *SQLConcurrentBufferIngester) ingest(ctx context.Context, events *eventBuffer) {
	for idx, e := range events {
		...
		if e.statement != nil {
			...
			i.processTransaction(ctx, e.transaction)
		} else {
...
func NewSQLConcurrentBufferIngester(sinks ...SQLStatsSink) *SQLConcurrentBufferIngester {
...
// processTransaction processes a transaction event and its associated statements.
// The statements are retrieved by consuming the statements buffered in the
// statementsBySessionID map. This function will release the statement buffer
// for the session after processing.
