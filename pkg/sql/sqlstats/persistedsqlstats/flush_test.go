...
func TestSQLStatsFlush(t *testing.T) {
	...
	// Regular inserts.
	{
		...
		for _, tc := range testQueries {
			verifyNumOfInsertedEntries(t, secondSQLConn, tc.fingerprint, sqlInstanceId, 1 /* expectedStmtEntryCnt */, 1 /* expectedTxnEntryCtn */)
			verifyInsertedFingerprintExecCount(t, secondSQLConn, tc.fingerprint, fakeTime.getAggTimeTs(), sqlInstanceId, tc.count)
		}
	}
	...
	// no new entries will be created but the statistics is updated.
	{
		...
		for _, tc := range testQueries {
			...
			verifyInsertedFingerprintExecCount(t, secondSQLConn, tc.fingerprint, fakeTime.getAggTimeTs(), sqlInstanceId, tc.count+tc.count-1 /* expectedCount */)
		}
	}
	...
	// We change the time to be in a different aggregation window.
	{
		...
		for _, tc := range testQueries {
			...
			verifyNumOfInsertedEntries(t, secondSQLConn, tc.fingerprint, sqlInstanceId, 2 /* expectedStmtEntryCnt */, 2 /* expectedTxnEntryCtn */)
			verifyInsertedFingerprintExecCount(t, secondSQLConn, tc.fingerprint, fakeTime.getAggTimeTs(), sqlInstanceId, tc.count)
		}
	}
	...
	// We run queries in a different server and trigger the flush.
	{
		...
		if sqlstats.GatewayNodeEnabled.Get(&testCluster.Server(0).ClusterSettings().SV) {
			...
			// accidentally tamper the entries written by another server.
			for _, tc := range testQueries {
				verifyNumOfInsertedEntries(t, firstSQLConn, tc.fingerprint, sqlInstanceId, 2 /* expectedStmtEntryCnt */, 2 /* expectedTxnEntryCtn */)
				verifyInsertedFingerprintExecCount(t, firstSQLConn, tc.fingerprint, fakeTime.getAggTimeTs(), sqlInstanceId, tc.count*2 /*num of servers*/)
			}
		}
	}
}
