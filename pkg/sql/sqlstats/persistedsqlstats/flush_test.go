...
func TestSQLStatsMinimumFlushInterval(t *testing.T) {
	...
	s.SQLServer().(*sql.Server).
		GetSQLStatsProvider().(*persistedsqlstats.PersistedSQLStats).Flush(ctx, s.AppStopper())
...
func TestPersistedSQLStats_Flush(t *testing.T) {
	...
	t.Run("add stats before reset", func(t *testing.T) {
		...
		sqlStats.Flush(ctx, srv.AppStopper())
...
func TestSQLStatsFlushWorkerDoesntSignalJobOnAbort(t *testing.T) {
	...
	persistedsqlstats.SQLStatsFlushEnabled.Override(ctx, &tc.Server(0).ClusterSettings().SV, false)
	persistedsqlstats.SQLStatsFlushInterval.Override(ctx, &tc.Server(0).ClusterSettings().SV, 50*time.Millisecond)
