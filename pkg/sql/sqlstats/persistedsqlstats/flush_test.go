...
func TestSQLStatsFlush(t *testing.T) {
	...
	// Regular inserts.
	{
		...
		sqlstatstestutil.WaitForStatementStatsCountEqual(t, observerConn, len(testQueries), sqlstatstestutil.StatementFilter{
			App:       "flush_unit_test",
...
func TestSQLStatsMinimumFlushInterval(t *testing.T) {
	...
	sqlConn.Exec(t, "SELECT 1")
	sqlstatstestutil.WaitForStatementStatsCountEqual(t, obsConn, 1, sqlstatstestutil.StatementFilter{
		App: "min_flush_test",
	})
...
func TestInMemoryStatsDiscard(t *testing.T) {
	...
	t.Run("flush_disabled", func(t *testing.T) {
		...
		sqlstatstestutil.WaitForStatementStatsCountEqual(t, observerConn, 1, sqlstatstestutil.StatementFilter{
	...
	t.Run("flush_enabled", func(t *testing.T) {
		...
		sqlstatstestutil.WaitForTransactionStatsCountEqual(t, observerConn, 1, sqlstatstestutil.TransactionFilter{
			App: "flush_enabled_test",
		})
...
func TestSQLStatsPersistedLimitReached(t *testing.T) {
	...
	for smallestStatsCountAcrossAllShards(t, sqlConn) <= minCountByShard {
		...
		sqlstatstestutil.WaitForStatementStatsCountAtLeast(t, sqlConn, 1, sqlstatstestutil.StatementFilter{
			App: appName,
		})
		pss.MaybeFlush(ctx, s.AppStopper())
	}
	...
	sqlstatstestutil.WaitForStatementStatsCountAtLeast(t, sqlConn, appCount)
...
func TestSQLStatsReadLimitSizeOnLockedTable(t *testing.T) {
	...
	sqlstatstestutil.WaitForStatementStatsCountAtLeast(t, sqlConn, int(minNumExpectedStmts/2)+1)
