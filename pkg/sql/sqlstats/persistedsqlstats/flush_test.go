...
func TestSQLStatsFlush(t *testing.T) {
	...
	// Regular inserts.
	{
		...
		expectedFlushEvent += firstServerSQLStats.StartFlush(ctx)
		expectedFlushEvent += secondServerSQLStats.StartFlush(ctx)
	...
	// no new entries will be created but the statistics is updated.
	{
		...
		for _, tc := range testCases {
			...
			verifyInsertedFingerprintExecCount(t, secondSQLConn, tc.fingerprint, fakeTime.getAggTimeTs(), firstServer.NodeID(), tc.count*2)
		}
	}
...
`, fingerprint, nodeID)
	...
	row.Scan(&stmtFingerprintID, &numOfInsertedStmtEntry)
...
`, stmtFingerprintID), nodeID)
	...
	row.Scan(&txnFingerprintID, &numOfInsertedTxnEntry)
