...
type StatsCompactor struct {
	...
	TestingKnobs *StatsCompactorKnobs
}

// StatsCompactorKnobs can be used to tune the behavior of
// StatsCompactor.
type StatsCompactorKnobs struct {
...
func (c *StatsCompactor) getExistingStmtAndTxnStatsEntries(
	...
	row, err := c.ie.QueryRowEx(ctx,
		...
		stmt)
	if err != nil {
		return 0 /* stmtStatsEntryCnt */, 0 /* txnStatsEntryCnt */, err
	}

	if row.Len() != 1 {
		return 0 /* stmtStatsEntryCnt */, 0 /* txnStatsEntryCnt */, errors.AssertionFailedf("unexpected number of column returned")
	}
	stmtStatsEntryCnt = int64(tree.MustBeDInt(row[0]))
	...
	txnStatsEntryCnt = int64(tree.MustBeDInt(row[0]))
