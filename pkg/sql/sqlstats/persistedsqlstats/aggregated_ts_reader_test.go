...
func TestScanEarliestAggregatedTs(t *testing.T) {
	...
	for _, tc := range earliestAggTsTestCases {
		...
		t.Run(fmt.Sprintf("%s populated table", tc.tableName), func(t *testing.T) {
			...
			verifyFewerEntriesAtEarliestTimeThanShardBucketCount(t, sqlConn, tc.tableName, truncatedBeforeBaseTime)
...
func verifyFewerEntriesAtEarliestTimeThanShardBucketCount(
