...
func fillObjectHelper(t *testing.T, val reflect.Value, data *randomData, inExecStats bool) {
	...
	case reflect.Struct:
		...
		for i := 0; i < numFields; i++ {
			...
			if _, ok := fieldBlacklist[fieldName]; ok {
				// This is unfortunate because roachpb.ExecStats also contains Count
				// field.
				if fieldName == "Count" && inExecStats {
					fillObjectHelper(t, fieldAddr, data, inExecStats)
				}
				continue
			}

			inExecStats := inExecStats || fieldName == "ExecStats"
			fillObjectHelper(t, fieldAddr, data, inExecStats)
		}
	default:
...
func fillObject(t *testing.T, val reflect.Value, data *randomData) {
	fillObjectHelper(t, val, data, false /* inExecStats */)
}
