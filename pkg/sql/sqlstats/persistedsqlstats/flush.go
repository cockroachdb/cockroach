...
func (s *PersistedSQLStats) Flush(ctx context.Context) {
	...
	enabled := SQLStatsFlushEnabled.Get(&s.cfg.Settings.SV) && s.isClusterVersionHighEnough(ctx)
...
// isClusterVersionHighEnough returns true the cluster version is high enough to
// support persisted SQL Stats.
func (s *PersistedSQLStats) isClusterVersionHighEnough(ctx context.Context) bool {
