...
func (s *PersistedSQLStats) startSQLStatsFlushWorker(ctx context.Context, stopper *stop.Stopper) {
	_ = stopper.RunAsyncTask(ctx, "sql-stats-flush-worker", func(ctx context.Context) {
		for {
			select {
			case stmtStats := <-s.stmtStatsCh:
				s.doFlush(ctx, func() error {
					return s.doFlushSingleStmtStats(ctx, &stmtStats)
				}, "failed to flush statement statistics")
			case txnStats := <-s.txnStatsCh:
				s.doFlush(ctx, func() error {
					return s.doFlushSingleTxnStats(ctx, txnStats.fingerprintID, &txnStats.stats)
...
`
	...
	statistics := tree.MustBeDJSON(row[1])
...
`
	...
	if row == nil {
		return errors.Newf(
			"statement statistics not found fingerprint_id: %s, app: %s, aggregated_ts: %s, node_id: %d",
