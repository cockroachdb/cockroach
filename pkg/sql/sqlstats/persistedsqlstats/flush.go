...
func (s *PersistedSQLStats) doFlushSingleTxnStats(
	...
	return s.cfg.DB.Txn(ctx, func(ctx context.Context, txn isql.Txn) error {
		...
		err := s.upsertTransactionStats(ctx, txn, aggregatedTs, serializedFingerprintID, &scopedStats)
		if err != nil {
...
SET statistics = crdb_internal.merge_transaction_stats(ARRAY(statistics, $1))
...
SET statistics = crdb_internal.merge_statement_stats(ARRAY(statistics, $1)),
