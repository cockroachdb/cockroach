...
func TestSQLStatsCompactor(t *testing.T) {
	...
	firstSQLConn.Exec(t, fmt.Sprintf("SET CLUSTER SETTING sql.stats.persisted_rows.max = %d", maxPersistedRowLimit))
	...
	compactionExecutor := &persistedsqlstats.StatsCompactor{
		...
		TestingKnobs: &persistedsqlstats.StatsCompactorTestingKnobs{
			DisableFollowerRead: true,
		},
	}
	compactionExecutor.Init(firstServer.InternalExecutor().(sqlutil.InternalExecutor), firstServer.DB(), true /* disallowFollowerRead */)

	// Initial compaction should remove the all the oldest entries.
	expectedDeletedStmtFingerprints, expectedDeletedTxnFingerprints := getTopSortedFingerprints(t, firstSQLConn, maxPersistedRowLimit)

	err = compactionExecutor.DeleteOldestEntries(ctx)
	require.NoError(t, err)

	actualStmtFingerprints, actualTxnFingerprints := getTopSortedFingerprints(t, firstSQLConn, 0 /* limit */)
...
func TestAtMostOneSQLStatsCompactionJob(t *testing.T) {
	...
	testutils.SucceedsSoon(t, func() error {
		var cnt uint64
		row := conn.QueryRow(`SELECT id FROM system.jobs where id = $1 AND status = 'succeeded'`, jobID)
		return row.Scan(&cnt)
	})
...
func launchSQLStatsCompactionJob(server serverutils.TestServerInterface) (jobspb.JobID, error) {
	return persistedsqlstats.CreateCompactionJob(
		...
		server.JobRegistry().(*jobs.Registry), &persistedsqlstats.StatsCompactorTestingKnobs{
			DisableFollowerRead: true,
		})
}
...
func getStatsTablesIDs(
	...
	stmt := "select 'system.statement_statistics'::regclass::oid, 'system.transaction_statistics'::regclass::oid"
