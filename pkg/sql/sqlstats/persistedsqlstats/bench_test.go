	...
	for _, cluster := range []clusterSpec{
		...
		b.Run(cluster.name, func(b *testing.B) {
			type testSpec struct {
				...
				forceComputedIndex bool
			}
			for _, test := range []testSpec{
				{computedColumns: []string{"execution_count", "service_latency", "cpu_sql_nanos", "contention_time", "total_estimated_execution_time"}, forceComputedIndex: true},
				{computedColumns: []string{"execution_count", "service_latency", "cpu_sql_nanos", "contention_time", "total_estimated_execution_time"}, forceComputedIndex: false},
				{computedColumns: []string{"execution_count", "service_latency", "cpu_sql_nanos", "contention_time"}, forceComputedIndex: false},
				{computedColumns: []string{"execution_count", "service_latency", "cpu_sql_nanos", "contention_time"}, forceComputedIndex: false},
				{computedColumns: []string{"execution_count", "service_latency", "cpu_sql_nanos"}, forceComputedIndex: false},
				{computedColumns: []string{"execution_count", "service_latency"}, forceComputedIndex: false},
				{computedColumns: []string{"execution_count"}, forceComputedIndex: false},
				{computedColumns: []string{}, forceComputedIndex: false},
			} {
				...
				totalEstimatedExecutionTime := "(statistics->'execution_statistics'->>'cnt')::float * (statistics->'statistics'->'svcLat'->>'mean')::FLOAT"
				...
				computedIndexes := []string{executionCountIdx, serviceLatencyIdx, sqlCpuNanosIdx, contentionTimeIdx, totalEstimatedExecutionTimeIdx}
				...
				if test.forceComputedIndex {
					name.WriteString("forceComputedIndex=true")
				} else {
					name.WriteString("forceComputedIndex=false")
				}
				...
				for i := range computeExpressions {
					...
					baseQuery := fmt.Sprintf(`SELECT
										...
										metadata,
										statistics
									FROM system.transaction_statistics%s
				...
				for i := range computeExpressions {
					...
					baseQuery := fmt.Sprintf(`SELECT
										...
										metadata,
										statistics
									FROM system.statement_statistics%s
									WHERE app_name NOT LIKE '$ internal%%' AND aggregated_ts > (now() - INTERVAL '1 hour') %s`,
						computeExpressions[0],
						computeExpressions[1],
						computeExpressions[2],
