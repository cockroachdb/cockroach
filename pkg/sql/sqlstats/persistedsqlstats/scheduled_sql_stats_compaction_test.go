...
func newTestHelper(t *testing.T) (helper *testHelper, cleanup func()) {
	...
	server, db, _ := serverutils.StartServer(t, args)
...
func getSQLStatsCompactionSchedule(t *testing.T, helper *testHelper) *jobs.ScheduledJob {
	var jobID int64
	helper.sqlDB.QueryRow(t, `SELECT schedule_id FROM system.scheduled_jobs`).Scan(&jobID)
	sj, err := jobs.LoadScheduledJob(context.Background(), helper.env, jobID, helper.server.InternalExecutor().(sqlutil.InternalExecutor), nil /* txn */)
...
func TestScheduledSQLStatsCompaction(t *testing.T) {
	...
	require.Equal(t, persistedsqlstats.ScheduledStatusWaiting, schedule.ScheduleStatus())
	...
	schedule = getSQLStatsCompactionSchedule(t, helper)
	require.Equal(t, persistedsqlstats.ScheduledStatusCompleted, schedule.ScheduleStatus())
