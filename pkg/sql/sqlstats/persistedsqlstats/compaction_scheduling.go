...
import (
	...
	"github.com/cockroachdb/cockroach/pkg/util/jobutil"
)

// CreateCompactionJob creates a system.jobs record if there is no other
// SQL Stats compaction job running. This is invoked either through the
// crdb_internal.sql_stats_compact() builtin or the scheduled job Executor.
...
// that are either PAUCED, CANCELED, or RUNNING. If so, it returns a
...
func CheckExistingCompactionJob(
	...
	exists, err := jobutil.CheckRunningJobsHelper(ctx, job, ie, txn, func(payload *jobspb.Payload) bool {
	...
	if exists {
		return ErrConcurrentSQLStatsCompaction
	}
