...
func CreateCompactionJob(
	...
	knobs *StatsCompactorTestingKnobs,
) (jobspb.JobID, error) {
	if err := CheckExistingCompactionJob(ctx, nil /* job */, ie, txn); err != nil {
		return jobspb.InvalidJobID, err
	}

	var disallowFollowerRead bool
	if knobs != nil && knobs.DisableFollowerRead {
		disallowFollowerRead = true
	}
	...
	record := jobs.Record{
		...
		Details: jobspb.SQLStatsCompactionDetails{
			DisableFollowerRead: disallowFollowerRead,
		},
...
func CheckExistingCompactionJob(
	...
	exists, err := jobutil.RunningJobExists(ctx, job, ie, txn, func(payload *jobspb.Payload) bool {
