...
func (c *CombinedStmtStatsIterator) Next(ctx context.Context) (bool, error) {
	...
	if c.mem.canBeAdvaned && !c.mem.paused {
		c.mem.canBeAdvaned = c.mem.it.Next()
	}
	...
	if !c.mem.canBeAdvaned && !c.disk.canBeAdvanced {
		// Sanity check.
...
func compareStmtStats(lhs, rhs *roachpb.CollectedStatementStatistics) int {
	...
	if lhs.AggregatedTs.Equal(rhs.AggregatedTs) &&
		lhs.Key.App == rhs.Key.App &&
		lhs.ID == rhs.ID &&
		lhs.Key.PlanHash == rhs.Key.PlanHash {
		return 0
	}
	...
	if lhs.AggregatedTs.Before(rhs.AggregatedTs) {
		return -1
	} else if lhs.AggregatedTs.After(rhs.AggregatedTs) {
		return 1
	}
	...
	if lhs.Key.PlanHash < rhs.Key.PlanHash {
		return -1
	} else if lhs.Key.PlanHash > rhs.Key.PlanHash {
		return 1
	} else {
		return 0
	}
