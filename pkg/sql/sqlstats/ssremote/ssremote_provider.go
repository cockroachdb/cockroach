...
func (ss *SQLStats) DrainStats(
	...
	resp, err := ss.execConfig.SQLStatusServer.DrainSqlStats(ctx, &serverpb.DrainSqlStatsRequest{})
	if err != nil {
	...
	for _, stmt := range resp.Statements {
		stmtStats = append(stmtStats, &stmt)
	}
	for _, txn := range resp.Transactions {
		txnStats = append(txnStats, &txn)
	}

	log.Infof(ctx, "DrainSqlStats done stmts=%d txns=%d", len(stmtStats), len(txnStats))
	return stmtStats, txnStats, resp.FingerprintCount
}

// Reset Resets in-memory stats for cluster.
func (ss *SQLStats) Reset(ctx context.Context) error {
	_, err := ss.execConfig.SQLStatusServer.ResetSQLStats(ctx, &serverpb.ResetSQLStatsRequest{ResetPersistedStats: false})
	return err
}
