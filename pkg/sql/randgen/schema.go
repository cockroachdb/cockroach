...
func RandCreateTable(
	...
	return RandCreateTableWithColumnIndexNumberGenerator(rng, prefix, tableIdx, isMultiRegion, nil)
}
...
func RandCreateTableWithColumnIndexNumberGenerator(
	...
	if rng.Intn(8) != 0 {
		indexDef, ok := randIndexTableDefFromCols(rng, columnDefs, tableName, true, isMultiRegion)
	...
	nIdxs := rng.Intn(10)
	for i := 0; i < nIdxs; i++ {
		indexDef, ok := randIndexTableDefFromCols(rng, columnDefs, tableName, false, isMultiRegion)
		if !ok {
			continue
		}
