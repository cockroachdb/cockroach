	...
	if !isPrimaryIndex && len(prefix) > 0 && rng.Intn(10) == 0 {
		...
		for i := 0; i < rng.Intn(10)+1; i++ {
			...
			for j := 0; j < rng.Intn(10)+1; j++ {
				...
				for k := 0; k < prefixLen; k++ {
					t.Exprs = append(t.Exprs, RandDatum(rng, tree.MustBeStaticallyKnownType(cols[k].Type), cols[k].Nullable.Nullability != tree.NotNull))
				}
		...
		if rng.Intn(2) == 0 {
			...
			for i := 0; i < prefixLen; i++ {
				d, err := tree.NewDefaultDatum(&tree.EvalContext{}, tree.MustBeStaticallyKnownType(cols[i].Type))
				if err != nil {
					return def, false
				}
				t.Exprs = append(t.Exprs, d)
			}
			isDup, err := isDuplicateExpr(t.Exprs, def.PartitionByIndex.List)
			if err != nil {
				return def, false
			}
			if !isDup {
