...
func columnFamilyMutator(rng *rand.Rand, stmt tree.Statement) (changed bool) {
	...
	if len(columns) == 1 && !foundExplicitPK {
		...
		// set of columns. (Note that the name might differ if there is already
		// user-defined column with 'rowid' name, but it should be uncommon.)
		//
