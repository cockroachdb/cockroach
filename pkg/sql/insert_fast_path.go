...
func (c *insertFastPathCheck) init(params runParams) error {
	...
	if (c.Locking.IsLocking() && c.Locking.Durability == tree.LockDurabilityGuaranteed) ||
		c.Locking.WaitPolicy != tree.LockWaitBlock {
		c.spanSplitter = span.MakeSplitterForSideEffect(c.tabDesc, c.idx, intsets.Fast{} /* neededColOrdinals */)
	} else {
