...
func (p *planner) maybeLogStatementInternal(
	...
	rollingQueryCounts := telemetryMetrics.RollingQueryCounts
	smoothingAlpha := telemetryMetrics.smoothingAlpha
	rollingInterval := telemetryMetrics.rollingInterval
	shouldSampleEventToTelemetry := p.stmt.AST.StatementType() == tree.TypeDML &&
		expSmoothQPS(rollingQueryCounts, smoothingAlpha, rollingInterval) > qpsThreshold
