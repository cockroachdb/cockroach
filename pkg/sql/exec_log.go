...
// I180211 07:30:48.832004 317 sql/exec_log.go:90  [client=127.0.0.1:62503,user=root,n1] 13 exec "cockroach" {"ab"[53]:READ} "SELECT * FROM ab" {} 123.45 12 OK 0 123
// I180211 07:30:48.832004 317 sql/exec_log.go:90  [client=127.0.0.1:62503,user=root,n1] 13 exec "cockroach" {"ab"[53]:READ} "SELECT nonexistent FROM ab" {} 0.123 12 ERROR 0 123
// Example execution log:
// I180211 07:30:48.832004 317 sql/exec_log.go:90  [client=127.0.0.1:62503,user=root,n1] 13 exec "cockroach" {} "SELECT * FROM ab" {} 123.45 12 OK 0 123
// I180211 07:30:48.832004 317 sql/exec_log.go:90  [client=127.0.0.1:62503,user=root,n1] 13 exec "cockroach" {} "SELECT nonexistent FROM ab" {} 0.123 0 "column \"nonexistent\" not found" 0 123
//
...
//  - the transaction counter since the beginning of the SQL session.
...
func (p *planner) maybeLogStatement(
	...
	p.maybeLogStatementInternal(ctx, execType, numRetries, txnCounter, rows, err, queryReceived)
}
...
func (p *planner) maybeLogStatementInternal(
	...
	if auditEventsDetected {
		...
		logger.Logf(ctx, "%s %q %s %q %s %.3f %d %s %d %d",
			lbl, appName, logTrigger, stmtStr, plStr, age, rows, auditErrStr, numRetries, txnCounter)
	}
	if logExecuteEnabled {
		logger := p.execCfg.ExecLogger
		logger.Logf(ctx, "%s %q %s %q %s %.3f %d %q %d %d",
			lbl, appName, logTrigger, stmtStr, plStr, age, rows, execErrStr, numRetries, txnCounter)
	}
	if logV {
		// Copy to the main log.
		log.VEventf(ctx, execType.vLevel(), "%s %q %s %q %s %.3f %d %q %d %d",
			lbl, appName, logTrigger, stmtStr, plStr, age, rows, execErrStr, numRetries, txnCounter)
	}
}
