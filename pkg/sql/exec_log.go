...
// not BEGIN or COMMIT.
func shouldForceLogStatementType(ast tree.Statement) bool {
...
func (p *planner) maybeLogStatementInternal(
	...
	if telemetryLoggingEnabled && !p.SessionData().TroubleshootingMode {
		...
		forceLog := shouldForceLogStatementType(p.stmt.AST) || tracingEnabled || logConsoleQuery

		if telemetryMetrics.shouldEmitStatementLog(trackedForTelemetry, stmtCount, forceLog) {
...
func (p *planner) maybeLogTransaction(
	...
	if !trackedForTelemetry && !forceLog {
		telemetryLoggingMetrics.incSkippedTransactionCount()
		return
	}

	skippedTxns := telemetryLoggingMetrics.resetSkippedTransactionCount()
	var sampledExecStats *eventpb.SampledExecStats
	...
	sampledTxn := eventpb.SampledTransaction{
		SkippedTransactions:      int64(skippedTxns),
		...
		SampledExecStats:         sampledExecStats,
	}
