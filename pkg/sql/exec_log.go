...
func (p *planner) maybeLogStatementInternal(
	...
	if !shouldSampleEventToTelemetry {
		...
		p.logEventsOnlyExternally(ctx, eventLogEntry{event: &eventpb.TelemetryEvent{CommonSQLExecDetails: execDetails, EffectiveSampleRate: float64(qpsThreshold) / float64(smoothQPS)}})
	}
}
