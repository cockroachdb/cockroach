...
var telemetryLogEnabled = settings.RegisterBoolSetting(
	"sql.log.telemetry.enabled",
	"when set to true, executed queries will emit a telemetry log",
	false,
).WithPublic()
...
func (p *planner) maybeLogStatementInternal(
	...
	telemetryLogEnabled := telemetryLogEnabled.Get(&p.execCfg.Settings.SV)
	...
	if !logV && !logExecuteEnabled && !auditEventsDetected && !slowQueryLogEnabled &&
		!shouldLogToAdminAuditLog && !telemetryLogEnabled {
	...
	if telemetryLogEnabled {
		...
		} else if shouldSampleEventToTelemetry && sampleRatePass(p.rng, sampleRate) {
