...
func TestDDLAnalysis(t *testing.T) {
	...
	datadriven.Walk(t, "testdata/ddl_analysis", func(t *testing.T, path string) {
		...
		defer diagSrv.Close()
		stmtToKvBatchRequests := make(map[string]int)
		beforePlan := func(span opentracing.Span, stmt string) {
			if _, ok := stmtToKvBatchRequests[stmt]; ok {
				span.Finish()
				trace := tracing.GetRecording(span)
				stmtToKvBatchRequests[stmt] = countKvBatchRequestsInRecording(trace)
			}
		}
		...
		datadriven.RunTest(t, path, func(t *testing.T, td *datadriven.TestData) string {
			...
			case "count":
				stmtToKvBatchRequests[td.Input] = -1
				_, err := sqlConn.Exec(td.Input)
				if err != nil {
					if errors.HasAssertionFailure(err) {
						td.Fatalf(t, "%+v", err)
				...
				count := stmtToKvBatchRequests[td.Input]
				if count == -1 {
					t.Fatalf(
