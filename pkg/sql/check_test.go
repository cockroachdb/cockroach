...
func TestValidateTTLScheduledJobs(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			expectedErrRe: func(tableID descpb.ID, scheduleID int64) string {
				return fmt.Sprintf(`table id %d maps to a non-existent scheduled job id 0`, tableID)
			},
		},
		{
			...
			expectedErrRe: func(tableID descpb.ID, scheduleID int64) string {
				return fmt.Sprintf(`scheduled job id %d points to table id 0 instead of table id %d`, scheduleID, tableID)
			},
		},
	}
