...
type Config struct {
	...
	// AllowUnsafe, if true, attempts to pack float values inside
	// numeric slots using the unsafe package.
	AllowUnsafe bool
}
...
var Pervasives = []parser.Def{
	...
	{Kind: parser.PrimDef, Name: parser.TypeNameOccur{Name: "byte"}},
	{Kind: parser.PrimDef, Name: parser.TypeNameOccur{Name: "rune"}},
...
func (cfg *Config) Generate(
	w io.Writer, tmpl *template.Template, languages ...LanguageDef,
) error {
	root := template.NewRoot()

	// Common configuration parameters.
	root.AddReplacementf("'r'", "%d", cfg.NumRefSlots)
	root.AddReplacementf("'e'", "%d", cfg.NumNumericSlots)
	root.AddReplacementf("'s'", "%d", cfg.NumStrSlots)
...
var tSizes = map[parser.TypeName]int{
	"bool": 1,
	"byte": 8, "int8": 8, "uint8": 8,
	"rune": 16, "int16": 16, "uint16": 16,
	"int32": 32, "uint32": 32, "float32": 32,
	"int64": 64, "uint64": 64, "float64": 64,
}

// needUnsafe must contain every numeric primitive type that cannot be
// converted losslessly between its native Go type and the numeric
// slot type (an integer type).
var needUnsafe = map[parser.TypeName]struct{}{
	"float32": struct{}{},
	"float64": struct{}{},
}
...
func (a *slotAllocator) allocateSlots(sName parser.TypeName, items []ir.StructItem) error {
	...
	for _, item := range items {
		_, unsafeCastNeeded := needUnsafe[item.Type.Name]
		if unsafeCastNeeded && !a.AllowUnsafe {
			// We'd need an unsafe cast to access the item if it was stored
			// in the numeric slots, and unsafe casts are disabled. Simply
			// do not allocate in a numeric slot (it will be moved to an
			// extra slot below).
			continue
		}
...
func (a *slotAllocator) allocateExtraSlot(name parser.ItemName, typ parser.TypeName) slot {
	...
	return slot{
		getName:  fmt.Sprintf("$1.extra.(*extraStruct).%s", name),
...
func (a *slotAllocator) allocateStrSlot(name parser.ItemName, typ parser.TypeName) slot {
	...
	a.curStrs++
	return slot{
		getName:  fmt.Sprintf("%s($1.strs[%d])", typ, j),
		setName:  fmt.Sprintf("$1.strs[%d] = string($3)", j),
...
func (a *slotAllocator) allocateRefSlot() slot {
	if a.curRefs >= a.NumRefSlots {
		...
		a.numExtraRefs++
		return slot{
			getName:  fmt.Sprintf("$1.extra.(*extraStruct).refs[%d]", j),
...
func (a *slotAllocator) makeNumSlot(
	...
	if typ == "bool" {
		...
		if sz < a.NumericSlotSize {
			// Packed mode.
			if unsafeCast {
				// Packed mode, unsafe enabled.
				s.getName = fmt.Sprintf("*(*%s)(unsafe.Pointer(uintptr(unsafe.Pointer(&$1.nums[%d])) + %s_Slot_%s_ByteOffset))", typ, slotNum, sName, name)
				s.setName = fmt.Sprintf("*(*%s)(unsafe.Pointer(uintptr(unsafe.Pointer(&$1.nums[%d])) + %s_Slot_%s_ByteOffset)) = $3", typ, slotNum, sName, name)
			} else {
				// Packed mode, unsafe disabled.
			...
			// Unpacked mode.
			if unsafeCast {
				// Unpacked mode, unsafe enabled.
				s.getName = fmt.Sprintf("*(*%s)(unsafe.Pointer(&$1.nums[%d]))", typ, slotNum)
				s.setName = fmt.Sprintf("*(*%s)(unsafe.Pointer(&$1.nums[%d])) = $3", typ, slotNum)
			} else {
				// Unpacked mode, unsafe disabled.
