...
func makeType(symTab symbolTable, d parser.Def) (ir.Type, error) {
	...
	case parser.EnumDef:
		...
		for _, i := range d.Items {
			if !i.IsReserved() {
				typ.Items = append(typ.Items, ir.EnumItem{i.Name.Name, i.Tag.Tag})
			}
		}
		return typ, nil
	case parser.PrimDef:
	...
	case parser.StructDef:
		...
		for _, i := range d.Items {
			...
			typ.Items = append(typ.Items, ir.StructItem{i.Name.Name, i.IsSlice, sym.typ, i.Tag.Tag})
		}
		return typ, nil
	case parser.SumDef:
		...
		for _, i := range d.Items {
			...
			typ.Items = append(typ.Items, ir.SumItem{sym.typ, i.Tag.Tag})
		}
		return typ, nil
	default:
