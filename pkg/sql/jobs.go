...
// Succeeded marks the tracked job as having succeeded. Any errors encountered
// while updating the jobs table are logged but not returned, under the
// assumption that it's better to have an incorrect record in the jobs table
// than to fail an otherwise successful job.
func (jl *JobLogger) Succeeded(ctx context.Context) {
	err := jl.updateJobRecord(ctx, JobStatusSucceeded, func(payload *JobPayload) error {
		if payload.FinishedMicros != 0 {
			return errors.Errorf("JobLogger: job %d already finished", jl.jobID)
		}
		payload.FinishedMicros = jobTimestamp(timeutil.Now())
	...
	if err != nil {
		log.Errorf(ctx, "JobLogger: ignoring error while logging success for job %d: %+v",
			jl.jobID, err)
	}
}
...
func unmarshalJobPayload(datum parser.Datum) (*JobPayload, error) {
	...
	if !ok {
		return nil, errors.New("JobLogger: failed to unmarshal job payload as bytes")
	}
...
func jobTimestamp(ts time.Time) int64 {
	return ts.Round(time.Microsecond).UnixNano() / 1e3
}
