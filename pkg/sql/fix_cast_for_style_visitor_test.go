...
`); err != nil {
	...
	tests := []struct {
		...
		},
		{
			...
			typ:    types.String,
			expect: "extract('epoch', to_char(t)::STRING)",
		},
		{
			expr:   "extract(epoch from d::CHAR)",
			typ:    types.QChar,
			expect: "extract('epoch', to_char(d)::CHAR)",
		},
		//Expected failures
		{
			expr:   "('foo' + 1)::STRING",
	...
	for _, test := range tests {
		t.Run(test.expr, func(t *testing.T) {
			...
			newExpr, _, err := sql.ResolveCastForStyleUsingVisitor(&visitor, expr)
			if err != nil {
				require.Equal(t, err.Error(), test.expect)
			} else {
