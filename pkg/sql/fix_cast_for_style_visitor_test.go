...
func TestFixCastForStyleVisitor(t *testing.T) {
	...
	if _, err := sqlDB.Exec(`
CREATE DATABASE t;
CREATE TABLE t.ds (it INTERVAL, s STRING, vc VARCHAR, c CHAR, t TIMESTAMP, n NAME, ttz TIMESTAMPTZ, d DATE);
`); err != nil {
		t.Fatal(err)
	}
	...
	tests := []struct {
		...
		{
			expr:   "extract(epoch from t::STRING)",
			typ:    types.String,
			expect: "extract(epoch from to_char(t)::STRING)",
		},
		{
			expr:   "extract(epoch from t::CHAR)",
			typ:    types.QChar,
			expect: "extract(epoch from to_char(t)::CHAR)",
		},
	}
	...
	for _, test := range tests {
		t.Run(test.expr, func(t *testing.T) {
			...
			visitor = sql.MakeFixCastForStyleVisitor(ctx, &semaCtx, tDesc, test.typ)
