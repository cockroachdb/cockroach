...
func (b *stmtBundleBuilder) addEnv(ctx context.Context) {
	...
	triggerRoutineIsProcedure := make(map[oid.Oid]bool)
		...
		for i := 0; i < len(refTables); i++ {
			...
			for j := range triggers {
				...
				for _, depID := range trig.DependsOnRoutines {
					...
					triggerRoutineIsProcedure[funcOid] = ol.Type == tree.ProcedureRoutine
				}
	...
	})
	{
		...
		// triggerRoutineIsProcedure which was populated during collection.
		triggerFuncIDs.ForEach(func(descID int) {
			funcOid := catid.FuncIDToOID(descpb.ID(descID))
			if !ids.Contains(int(funcOid)) {
				ids.Add(int(funcOid))
				isProcedure[funcOid] = triggerRoutineIsProcedure[funcOid]
			}
		})
...
func (c *stmtEnvCollector) PrintCreateTrigger(
	...
	tnName := tree.Name(triggerName)
	query := fmt.Sprintf(
		"SELECT create_statement FROM [SHOW CREATE TRIGGER %s ON %s]",
		tnName.String(), tn.FQString(),
	)
	if redactValues {
		query = fmt.Sprintf(
	...
	createStatement, err := c.query(query)
	if err != nil {
		return err
	}
	printCreateStatement(w, tn.CatalogName, createStatement)
