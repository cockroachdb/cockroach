		...
		opt.VisitFKReferenceTables(
			...
			func(table cat.Table, fk cat.ForeignKeyConstraint) (recurse bool) {
				...
				if referencedByMetadata.Contains(int(table.ID())) {
					// For read-only queries we don't want to recurse into the
					// referenced by metadata tables.
					return hasMutation
				}
				if fk == nil {
				...
				if table.ID() == fk.ReferencedTableID() {
					// The table we're considering for recursing into is a
					...
					// considering recursing into 'parent', but we never want
					// to recurse into it.
					return false
				}
				// The table we're considering for recursing into is an origin
				...
				if hasDelete && fk.DeleteReferenceAction() == tree.Cascade {
					...
					// in order to additionally visit 'grandchild'.
					return true
				}
			...
			func(table cat.Table, fk cat.ForeignKeyConstraint) {
				...
				// We always want to include directly referenced tables.
