...
func (c *stmtEnvCollector) PrintCreateUDT(w io.Writer, id oid.Oid, redactValues bool) error {
	...
	if res[0] != "" {
		if res[1] != "" {
			printCreateStatement(w, tree.Name(res[0]) /* dbName */, fmt.Sprintf("CREATE SCHEMA IF NOT EXISTS %s", res[1]))
		}
...
func (c *stmtEnvCollector) PrintCreateRoutine(
	...
	if res[1] != "" {
		printCreateStatement(w, tree.Name(res[0]), fmt.Sprintf("CREATE SCHEMA IF NOT EXISTS %s", res[1]))
	}
	printCreateStatement(w, tree.Name(res[0]) /* dbName */, res[2] /* createStatement */)
	return nil
}
