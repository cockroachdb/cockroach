...
func makeStmtBundleBuilder(
	...
	b := stmtBundleBuilder{
		flags: flags, db: db, p: p, ie: ie, username: requesterUsername,
...
func (b *stmtBundleBuilder) addEnv(ctx context.Context) {
	c := makeStmtEnvCollector(ctx, b.p, b.ie, b.username)
...
func makeStmtEnvCollector(
	ctx context.Context, p *planner, ie *InternalExecutor, userName string,
) stmtEnvCollector {
	ieo := sessiondata.NoSessionDataOverride
	if userName != "" {
		ieo = sessiondata.InternalExecutorOverride{
			User: username.MakeSQLUsernameFromPreNormalizedString(userName),
		}
	}
