...
func (c *stmtEnvCollector) PrintClusterSettings(w io.Writer, all bool) error {
	...
	rows, err := c.ie.QueryBufferedEx(
		...
		nil, /* txn */
		c.ieo,
		fmt.Sprintf(`SELECT variable, value, default_value, sensitive, origin FROM crdb_internal.cluster_settings%s`, suffix),
	)
	if err != nil {
		return err
	}
	for _, r := range rows {
		...
		sensitive, ok4 := r[3].(*tree.DBool)
		origin, ok5 := r[4].(*tree.DString)
