...
func (b *stmtBundleBuilder) addEnv(ctx context.Context) {
	...
	err := b.db.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
		...
		tables, sequences, views, addFKs, err = mem.Metadata().AllDataSourceNames(
			ctx,
			b.plan.catalog,
			func(table cat.Table, directlyReferenced intsets.Fast, fk cat.ForeignKeyConstraint) (include, recurse bool) {
				...
				// directly referenced by the metadata, so we will end up
				// including an extra "layer" of FK references. Avoiding that
				// seems quite tricky.
				if directlyReferenced.Contains(int(table.ID())) {
					// We always want to include directly referenced tables. For
					// read-only queries we only want to include those and
					// nothing else, so we don't recurse.
					recurse = hasMutation
					return true, recurse
				}
				if fk == nil {
