...
// immutablePreparedPortal is the immutable "core" of PreparedPortal. It is a
// separate struct so that multiple references to PreparedPortal could share
// the same "core" yet we're able to track the mutable state separately so that
// we could rewind the portal to the current mutable state when necessary.
