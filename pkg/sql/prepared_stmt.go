...
type PreparedStatement struct {
	...
	//
	// It may be a fully-optimized memo if it contains a "perfect generic plan"
	// that is guaranteed to be optimal across all executions of the prepared
	// statement. Perfect generic plans are generated when the statement has no
	// placeholders nor fold-able stable expressions, or when the placeholder
	// fast-path is utilized.
	//
	// If it is not a perfect generic plan, it is an unoptimized, normalized
...
// planCostStats tracks statistics about a fixed numbed of plan costs.
