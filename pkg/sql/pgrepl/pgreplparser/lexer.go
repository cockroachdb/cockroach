	...
	scanNumLoop:
		for curPos < len(l.in) {
			...
			case isHexDigit(nextCh) || nextCh == '/':
				// Any presence of a hex digit may make it an LSN.
...
func isID(ch rune, firstCh bool) bool {
	if (ch >= 'A' && ch <= 'Z') || (ch >= 'a' && ch <= 'z') || (ch >= 200 && ch <= 377) || ch == '_' {
		return true
	}
...
func (l *lexer) checkLSN(lval *pgreplSymType, startPos int) bool {
	...
	lval.SetUnionVal(pgrepltree.LSN{Hi: hi, Lo: lo})
	return true
}
