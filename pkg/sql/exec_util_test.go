...
func TestSessionDefaultsSafeFormat(t *testing.T) {
	...
	require.EqualValues(t, `[‹database›:‹test›;‹statement_timeout›:‹250ms›;‹disallow_full_table_scans›:‹true›;]`, redact.Sprint(session))
	require.EqualValues(t, `[‹×›:‹×›;‹×›:‹×›;‹×›:‹×›;]`, redact.Sprint(session).Redact())
}
