...
func TestCalcAvgQPS(t *testing.T) {
	...
	tenSeconds := 10 * oneSecondInNanoSeconds

	startTime := timeutil.Now()
	startTimePlusOneSecond := startTime.Add(time.Duration(oneSecondInNanoSeconds))
	startTimePlusTenSeconds := startTime.Add(time.Duration(tenSeconds))
