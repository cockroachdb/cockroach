...
func (d *delegator) delegateControlSchedules(ctl *tree.ControlSchedules) error {
	return nil
	// var sqlStmt strings.Builder
	//
	// switch ctl.Command {
	// case tree.PauseSchedule:
	// 	fmt.Fprintf(&sqlStmt, "SELECT * FROM system.scheduled_jobs WHERE schedule_id IN (%s)", ctl.Schedules.String())
	// case tree.ResumeSchedule:
	// case tree.DropSchedule:
	// 	fmt.Fprintf(&sqlStmt, "DELETE FROM system.scheduled_jobs WHERE schedule_id IN (%s)", ctl.Schedules.String())
	// }
	// const (
