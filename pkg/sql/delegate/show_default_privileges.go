...
func (d *delegator) delegateShowDefaultPrivileges(
	...
	query := fmt.Sprintf(
		"SELECT schema_name, role, object_type, grantee, privilege_type FROM crdb_internal.default_privileges WHERE database_name = '%s'",
		currentDatabase.Normalize(),
	)

	targetRoles, err := tree.NameListToUsernames(n.Roles)
	if err != nil {
		return nil, err
	}
