...
AND s.end_key > r.start_key`)
	...
	case tree.ExpandTables:
		...
		expr := fmt.Sprintf("SELECT r.* %s, t.schema_name, t.name AS table_name ", dbNameCol)
		if n.Options.Details {
			expr += ", approximate_disk_bytes AS approx_disk_bytes "
		}
		expr += fmt.Sprintf("FROM ranges r\n LEFT OUTER JOIN %s.crdb_internal.tables t ON r.table_id = t.table_id\n", dbName.String())
		if n.Options.Details {
			expr += fmt.Sprintf("LEFT OUTER JOIN %s AS s ON r.table_id = s.table_id", tenantSpanStats)
		}
		buf.WriteString(expr)
	case tree.ExpandIndexes:
		// Add table/index name details and approximate_disk_bytes if WITH DETAILS is specified.
		expr := fmt.Sprintf("SELECT r.* %s, t.schema_name, t.name AS table_name, ti.index_name ", dbNameCol)
		if n.Options.Details {
			expr += ", approximate_disk_bytes AS approx_disk_bytes "
		}
		expr += fmt.Sprintf(`FROM ranges r
