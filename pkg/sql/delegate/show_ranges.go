...
//
// The overall semantics are best described using a diagram:
//
//                       SHOW RANGES
//                             |
//                       query from
...
//                             |
//        (next stage is to extend the rows
//         with table/index information if requested)
//                             |
//             .---------------^-----------------.
//             |                                  |
//          syntax:                               |
...
//             `---------------v-----------------'
//                             |
//           (next stage is to filter out the set to include just
//            the ranges for the target selection)
//                             |
//            .----------------^------------.---------------------------------.
//            |                             |                                 |
...
//            `----------------v------------^---------------------------------'
//                             |
//                  (next stage is to gather additional details
//                   for the tables/indexes if requested)
//                             |
//            .----------------^------------.---------------------------------.
//            |                             |                                 |
...
//            `----------------v------------^---------------------------------'
//                             |
//              (next stage is assembling the final projection)
//                             |
//            .----------------^------------.---------------------------------.
//            |                             |                                 |
//         syntax:                      syntax:                            syntax:
...
AND s.end_key > r.start_key`)
 ...
 LEFT OUTER JOIN %[1]s.crdb_internal.tables t
	...
	case tree.ShowRangesDatabase, tree.ShowRangesCurrentDatabase, tree.ShowRangesCluster:
		...
		buf.WriteString(`
  crdb_internal.pretty_key(start_key, -1) AS start_key,
  crdb_internal.pretty_key(end_key, -1) AS end_key,
`)
	...
	case tree.ShowRangesTable:
		...
		case tree.UniqueRanges, tree.ExpandTables:
			...
			fmt.Fprintf(&buf, `
  CASE
    WHEN start_key = crdb_internal.table_span(%[1]d)[1] THEN '…/<TableMin>'
    WHEN start_key < crdb_internal.table_span(%[1]d)[1] THEN '<before:'||crdb_internal.pretty_key(start_key,-1)||'>'
    ELSE '…'||crdb_internal.pretty_key(start_key, 1)
  END AS start_key,
  CASE
    WHEN end_key = crdb_internal.table_span(%[1]d)[2] THEN '…/<TableMax>'
    WHEN end_key > crdb_internal.table_span(%[1]d)[2] THEN '<after:'||crdb_internal.pretty_key(end_key,-1)||'>'
    ELSE '…'||crdb_internal.pretty_key(end_key, 1)
  END AS end_key,
`, idx.Table().ID())
