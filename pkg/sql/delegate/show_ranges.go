...
func checkPrivilegesForShowRanges(d *delegator, table cat.Table) error {
	...
	hasView := d.catalog.CheckPrivilege(d.ctx, table, privilege.VIEWACTIVITY)
	hasViewRedact := d.catalog.CheckPrivilege(d.ctx, table, privilege.VIEWACTIVITYREDACTED)
	if hasView == nil || hasViewRedact == nil {
		return nil
	}
	hasViewAct, err := d.catalog.HasRoleOption(d.ctx, roleoption.VIEWACTIVITY)
	if err != nil {
		return err
	}
