...
func checkPrivilegesForShowRanges(d *delegator, table cat.Table) error {
	...
	if err := d.catalog.CheckPrivilege(d.ctx, table, privilege.ZONECONFIG); err != nil {
		return pgerror.Wrapf(err, pgcode.InsufficientPrivilege, "only users with the VIEWACTIVITY or ZONECONFIG privilege or the admin role can use SHOW RANGES on %s", table.Name())
	}
	return nil
}
