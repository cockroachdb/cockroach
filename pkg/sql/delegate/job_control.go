...
AND jobs.status IN (%s) AND jobs.created_by_id IN (%s)`,
	...
	if stmt.Type != nil {
		return parse(fmt.Sprintf(`%s JOBS (with decoded as (select id, status, crdb_internal.pb_to_json('cockroach.sql.jobs.jobspb.Payload', payload)->'%s' as cf from system.jobs) select id from decoded where cf is not null and status IN (%s))`, tree.JobCommandToStatement[stmt.Command], protoNameForType[*stmt.Type], filterClause))
	}

	return parse(fmt.Sprintf(`%s JOBS SELECT id FROM system.jobs`, tree.JobCommandToStatement[stmt.Command]))
}
