...
SELECT tp.table_catalog AS database_name,
  ...
  LEFT JOIN
       "".information_schema.sequences s ON tp.table_name = s.sequence_name`
	const typePrivQuery = `
...
`, oidext.CockroachPredefinedOIDMax)
	...
	if n.Targets != nil && len(n.Targets.Databases) > 0 {
		...
		nameCols = "database_name, schema_name, object_type, object_name,"
		// No target: only look at types, tables and schemas in the current database.
		source.WriteString(
			`SELECT database_name, schema_name, object_type, table_name AS object_name, grantee, privilege_type, is_grantable FROM (`,
		)
		source.WriteString(tablePrivQuery)
		source.WriteByte(')')
		source.WriteString(` UNION ALL ` +
			`SELECT database_name, schema_name, object_type, NULL::STRING AS object_name, grantee, privilege_type, is_grantable FROM (`)
		source.WriteString(schemaPrivQuery)
		source.WriteByte(')')
		source.WriteString(` UNION ALL ` +
			`SELECT database_name, NULL::STRING AS schema_name, object_type, NULL::STRING AS object_name, grantee, privilege_type, is_grantable FROM (`)
		source.WriteString(dbPrivQuery)
		source.WriteByte(')')
		source.WriteString(` UNION ALL ` +
			`SELECT database_name, schema_name, object_type, type_name AS object_name, grantee, privilege_type, is_grantable FROM (`)
		source.WriteString(typePrivQuery)
		source.WriteByte(')')
		source.WriteString(` UNION ALL ` +
			`SELECT database_name, schema_name, object_type, routine_signature AS object_name, grantee, privilege_type, is_grantable FROM (`)
