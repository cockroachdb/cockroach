...
WITH fn_grants AS (
  ...
  WHERE reverse(split_part(reverse(specific_name), '_', 1))::INT > CRDB_MAX_PREDEFINED_OID
)
SELECT database_name,
       schema_name,
       function_id,
  ...
  FROM fn_grants
`
	getUDFQuery := func() string {
		return strings.Replace(udfQuery, "CRDB_MAX_PREDEFINED_OID", strconv.Itoa(oidext.CockroachPredefinedOIDMax), -1)
	}
	...
	if n.Targets != nil && len(n.Targets.Databases) > 0 {
		...
		if n.Targets != nil {
			...
			source.WriteString(getUDFQuery())
			source.WriteByte(')')
			// If the current database is set, restrict the command to it.
			if currDB := d.evalCtx.SessionData().Database; currDB != "" {
