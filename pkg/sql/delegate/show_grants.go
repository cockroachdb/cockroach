...
func (d *delegator) delegateShowGrants(n *tree.ShowGrants) (tree.Statement, error) {
	specifics := querySpecifics{}
	var err error

	if n.Targets == nil {
		specifics = d.delegateShowGrantsAll(specifics)
	} else if len(n.Targets.Databases) > 0 {
		specifics, err = d.delegateShowGrantsDatabase(n, specifics)
	} else if len(n.Targets.Schemas) > 0 {
		specifics, err = d.delegateShowGrantsSchema(n, specifics)
	} else if len(n.Targets.Types) > 0 {
		specifics, err = d.delegateShowGrantsType(n, specifics)
	} else if len(n.Targets.Functions) > 0 || len(n.Targets.Procedures) > 0 {
		specifics, err = d.delegateShowGrantsRoutine(n, specifics)
	} else if n.Targets.System {
		specifics = d.delegateShowGrantsSystem(specifics)
	} else if len(n.Targets.ExternalConnections) > 0 {
		specifics = d.delegateShowGrantsExternalConnections(specifics)
	} else {
		// This includes sequences also
		specifics, err = d.delegateShowGrantsTable(n, specifics)
	}
...
func (d *delegator) delegateShowGrantsDatabase(
	n *tree.ShowGrants, specifics querySpecifics,
) (querySpecifics, error) {
	...
	specifics.nameCols = "database_name,"
	if len(params) == 0 {
	...
	specifics.source = source
	specifics.params = params
	specifics.cond = cond

	return specifics, nil
}

func (d *delegator) delegateShowGrantsSchema(
	n *tree.ShowGrants, specifics querySpecifics,
) (querySpecifics, error) {
	source := specifics.source
	params := specifics.params
	...
	specifics.nameCols = "database_name, schema_name,"
...
func (d *delegator) delegateShowGrantsType(
	n *tree.ShowGrants, specifics querySpecifics,
) (querySpecifics, error) {
	source := specifics.source
	params := specifics.params
	...
	specifics.nameCols = "database_name, schema_name, type_name,"
	if len(params) == 0 {
		dbNameClause := "true"
...
func (d *delegator) delegateShowGrantsRoutine(
	n *tree.ShowGrants, specifics querySpecifics,
) (querySpecifics, error) {
	source := specifics.source
	cond := specifics.cond
	...
	specifics.nameCols = "database_name, schema_name, routine_id, routine_signature,"
...
func (d *delegator) delegateShowGrantsSystem(specifics querySpecifics) querySpecifics {
	fmt.Fprint(&specifics.source, systemPrivilegeQuery)
	specifics.cond.WriteString(`WHERE true`)

	return specifics
}

func (d *delegator) delegateShowGrantsTable(
	n *tree.ShowGrants, specifics querySpecifics,
) (querySpecifics, error) {
	source := specifics.source
	params := specifics.params
	...
	specifics.nameCols = "database_name, schema_name, table_name,"
...
func (d *delegator) delegateShowGrantsExternalConnections(specifics querySpecifics) querySpecifics {
	specifics.nameCols = "connection_name,"

	fmt.Fprint(&specifics.source, externalConnectionPrivilegeQuery)
	specifics.cond.WriteString(`WHERE true`)

	return specifics
}

func (d *delegator) delegateShowGrantsAll(specifics querySpecifics) querySpecifics {
	source := specifics.source

	specifics.nameCols = "database_name, schema_name, object_name, object_type,"
	...
	// If the current database is set, restrict the command to it.
	if currDB := d.evalCtx.SessionData().Database; currDB != "" {
		fmt.Fprintf(&specifics.cond, ` WHERE database_name = %s OR object_type = 'external_connection'`, lexbase.EscapeSQLString(currDB))
	}
