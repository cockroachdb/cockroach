...
SELECT
  ...
  -- NB: careful not to coalesce with allsettings.value! This is the value fo the system tenant and is not relevant to other tenants.
  COALESCE(tenantspecific.value, overrideall.value, crdb_internal.cluster_setting_default(allsettings.variable)) AS value,
  allsettings.type,
  ` + publicCol + `
