...
SELECT
  ...
  crdb_internal.decode_cluster_setting(allsettings.variable,
     ...
     COALESCE(tenantspecific.value,
              ...
              -- crdb_internal.cluster_setting_encoded_default(allsettings.variable).
              NULL)
  ) AS value,
  allsettings.type,
  ` + publicCol + `
  IF(tenantspecific.value IS NOT NULL, 'per-tenant-override',
    IF(overrideall.value IS NOT NULL, 'all-tenants-override',
     -- here we should determine either 'tenant-self-defined' or 'default'.
     'unsupported - see issue #77935'
    )) AS origin,
