...
func (d *delegator) delegateShowTypes() (tree.Statement, error) {
	...
	// The SHOW TYPES SQL doesn't accept a schema like other SHOW statements. We
	// call ResolveSchema with an empty schema to specifically retrieve current
	// database/schema name.
	_, name, err := d.catalog.ResolveSchema(d.ctx, flags, &cat.SchemaName{})
	if err != nil {
		return nil, err
	}
