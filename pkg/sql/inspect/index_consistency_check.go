...
func (c *indexConsistencyCheck) createIndexCheckQuery(
	...
	for i, col := range otherColumns {
		nullFilters[i] = fmt.Sprintf("%s IS NULL", col)
		nonNullFilters[i] = fmt.Sprintf("%s IS NOT NULL", col)
	}

	// Build WHERE clauses using helper function
	nullFilter := buildWhereClause(predicate, nullFilters, true)
	nonNullFilter := buildWhereClause(predicate, nonNullFilters, true)
...
// buildWhereClause constructs WHERE clauses for the query based on predicate and null filters.
func buildWhereClause(predicate string, nullFilters []string, includeNullFilter bool) string {
	...
	if includeNullFilter && len(nullFilters) > 0 {
		conditions = append(conditions, strings.Join(nullFilters, " AND "))
	}
