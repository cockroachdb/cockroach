...
func extractRegionFromSpan(
	...
	if !tableDesc.IsLocalityRegionalByRow() {
		if !buildutil.CrdbTestBuild { // For ease of testing, allow faked multi-region databases.
			return "", errors.Newf("table is not REGIONAL BY ROW, cannot extract region from span")
		}
	}
...
func spanForRegion(
	...
	// Skip past the region column in the original span to get the suffix
	// (e.g., the ID range). The original span has: indexPrefix + oldRegion + suffix
	// We want to create: indexPrefix + newRegion + suffix
	...
	return roachpb.Span{
		Key:    append(regionPrefix, startSuffix...),
		EndKey: append(regionPrefix, endSuffix...),
	}, nil
}
...
func getPredicateAndQueryArgs(
	...
	if buildutil.CrdbTestBuild {
		fmt.Printf("DEBUG: getPredicateAndQueryArgs: span=%s, hasRows=%v, bounds.Start=%v, bounds.End=%v\n",
			span, hasRows, bounds.Start, bounds.End)
	}
