...
func loadDatabaseDesc(
	...
	if err := execCfg.DistSQLSrv.DB.DescsTxn(ctx, func(ctx context.Context, txn descs.Txn) error {
		...
		var err error
		dbDesc, err = byIDGetter.WithoutNonPublic().Get().Database(ctx, databaseID)
		if err != nil {
			return err
		}
		if databaseVersion != 0 && dbDesc.GetVersion() != databaseVersion {
			return errors.WithHintf(
				errors.Newf(
					"database %s [%d] has had a schema change since the job has started at %s",
					dbDesc.GetName(),
					dbDesc.GetID(),
					dbDesc.GetModificationTime().GoTime().Format(time.RFC3339),
				),
				"use AS OF SYSTEM TIME to avoid schema changes during inspection",
			)
		}
