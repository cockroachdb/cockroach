...
func (c *inspectResumer) planInspectProcessors(
	...
	sqlInstanceIDToSpec := make(map[base.SQLInstanceID]*execinfrapb.InspectSpec, len(spanPartitions))
	for _, spanPartition := range spanPartitions {
		sqlInstanceIDToSpec[spanPartition.SQLInstanceID] = newProcessorSpec(spanPartition.Spans)
	}

	// Set up a one-stage plan with one proc per input spec.
	processorCorePlacements := make([]physicalplan.ProcessorCorePlacement, len(sqlInstanceIDToSpec))
	i := 0
	for sqlInstanceID, spec := range sqlInstanceIDToSpec {
		processorCorePlacements[i].SQLInstanceID = sqlInstanceID
		processorCorePlacements[i].Core.Inspect = spec
		i++
	}
