...
func newInspectRun(
	...
	if len(stmt.Options) == 0 || stmt.Options.HasIndexAll() {
		...
		checks, err := sql.InspectChecksByIndexName(ctx, p, stmt.Options.NamedIndexes())
		if err != nil {
			return inspectRun{}, err
		}
		run.checks = checks
...
func inspectPlanHook(
	...
	if len(run.checks) == 0 {
		p.BufferClientNotice(ctx, pgnotice.Newf("No checks to run for inspect. Noop."))
		fn := func(ctx context.Context, resultsCh chan<- tree.Datums) error {
			resultsCh <- tree.Datums{
				tree.NewDInt(tree.DInt(jobspb.InvalidJobID)),
