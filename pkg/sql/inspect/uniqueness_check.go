...
func (c *uniquenessCheck) buildUniquenessCheckQueries(
	...
	for i, predicate := range remotePredicates {
		rPredicate := renumberPlaceholders(predicate, currentPlaceholderOffset)
...
func (c *uniquenessCheck) loadCatalogInfo(ctx context.Context) error {
	...
	if !isMultiRegion {
		if !buildutil.CrdbTestBuild && !testing.Testing() { // For ease of testing, allow faked multi-region databases.
			return errors.AssertionFailedf("uniqueness check is only supported for multi-region databases")
		}
	}
