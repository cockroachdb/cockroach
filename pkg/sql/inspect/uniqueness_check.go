...
type uniquenessCheck struct {
	...
	// tableVersion and dbVersion are the descriptor versions recorded when the check was planned.
	// They are used to detect concurrent schema changes for non-AS OF inspections.
	tableVersion, dbVersion descpb.DescriptorVersion
	asOf                    hlc.Timestamp
...
func (c *uniquenessCheck) loadCatalogInfo(ctx context.Context) error {
	...
	if dbDesc, err := loadDatabaseDesc(ctx, c.execCfg, c.tableDesc.GetParentID(), c.dbVersion, c.asOf); err != nil {
		return err
	} else if dbDesc.IsMultiRegion() {
		if !buildutil.CrdbTestBuild && !testing.Testing() { // For ease of testing, allow faked multi-region databases.
