...
func (c *uniquenessCheck) Start(
	...
	remotePredicates, remoteArgs, err := c.getRemotePredicatesAndQueryArgs(ctx, remoteRegions, cfg, span, keyColNames)
	if err != nil {
	...
	var query string
	if len(queries) == 0 {
		// All the remote regions were empty.
		c.state = checkDone
		return nil
	...
	// TODO(bghal): dedupe logic for duplicates found in >2 regions
...
func (c *uniquenessCheck) getRemotePredicatesAndQueryArgs(
	...
	for _, region := range remoteRegions {
		...
		rPredicate, rQueryArgs, err := getPredicateAndQueryArgs(
			ctx, cfg, remoteSpan, c.tableDesc, c.tableDesc.GetPrimaryIndex(), c.asOf, keyColNames,
		)
		if err != nil {
...
func (c *uniquenessCheck) buildUniquenessCheckQueries(
	...
	queryArgs = append(queryArgs, lQueryArgs...)
	currentPlaceholderOffset := len(lQueryArgs)

	for i, predicate := range remotePredicates {
		rPredicate := c.renumberPlaceholders(predicate, currentPlaceholderOffset)

		// TODO(bghal): address this uniqueColIdx assumption
		query := c.buildUniquenessCheckQuery(keyColNames, 1, lPredicate, rPredicate)
		queries = append(queries, query)

		queryArgs = append(queryArgs, remoteQueryArgs[i]...)
		currentPlaceholderOffset += len(remoteQueryArgs[i])
	}
...
func (c *uniquenessCheck) buildUniquenessCheckQuery(
	keyColNames []string, uniqueColIdx int, lPredicate, rPredicate string,
) string {
	...
	joinOn := fmt.Sprintf("l.%s = r.%s", encodedColNames[uniqueColIdx], encodedColNames[uniqueColIdx])

	var rColList []string
	for _, col := range encodedColNames {
		rColList = append(rColList, "r."+col)
	}
	rColListStr := strings.Join(rColList, ", ")
...
func (c *uniquenessCheck) loadCatalogInfo(ctx context.Context) error {
	...
	if !isMultiRegion {
		if !buildutil.CrdbTestBuild { // For ease of testing, allow faked multi-region databases.
			return errors.Newf("uniqueness check is only supported for multi-region databases")
		}
	}
...
// adding an offset.
func (_ *uniquenessCheck) renumberPlaceholders(predicate string, offset int) string {
	placeholderRe := regexp.MustCompile(`\$(\d+)`)
	return placeholderRe.ReplaceAllStringFunc(predicate, func(match string) string {
		numStr := match[1:]
