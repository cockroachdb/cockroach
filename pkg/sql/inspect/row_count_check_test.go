...
func TestRowCountCheck(t *testing.T) {
	...
	// Create database once for all test cases
	...
	testCases := []struct {
	...
	}{
		{
			...
			expectedNumChecks: 2, // a consistency check should be inserted to support the row count check
		},
		{
			name:              "with_secondary_index",
			createTableStmt:   `CREATE TABLE test.t (id INT PRIMARY KEY, INDEX idx (id))`,
			expectedNumChecks: 2,
		},
		{
			name:              "with_unsupported_index_type",
			createTableStmt:   `CREATE TABLE test.t (id INT PRIMARY KEY, data JSONB DEFAULT '{}'::JSONB, INVERTED INDEX idx (data))`,
			expectedNumChecks: 2, // inverted index not supported, so consistency check inserted for primary
		},
		{
			// TODO(154762): Update the test when consistency checks on hash sharded indexes are supported or the
			// row count check can otherwise get counts
			name:              "hash_sharded_primary_index_only",
			createTableStmt:   `CREATE TABLE test.t (id INT PRIMARY KEY USING HASH)`,
			expectedNumChecks: 0,    // hash-sharded primary index can't use the consistency check for row counting
			expectedSuccess:   true, // expect the job to succeed since no checks will be run
		},
		{
			name:              "hash_sharded_primary_index_with_secondary_index",
			createTableStmt:   `CREATE TABLE test.t (id INT PRIMARY KEY USING HASH, INDEX idx (id))`,
			expectedNumChecks: 0,    // hash-sharded primary index can't use the consistency check for row counting
			expectedSuccess:   true, // expect the job to succeed since no checks will be run
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			require.Len(t, checks, tc.expectedNumChecks)
