...
func ChecksForTable(
	...
	for _, index := range table.PublicNonPrimaryIndexes() {
		if ok, _ := isSupportedIndexForIndexConsistencyCheck(index, table); !ok {
			if p != nil {
...
// isSupportedIndexForIndexConsistencyCheck returns whether the given index is
// supported for index consistency checking along with a reason if it is not.
func isSupportedIndexForIndexConsistencyCheck(
	index catalog.Index, table catalog.TableDescriptor,
) (ok bool, reason string) {
	if !index.Public() {
		return false, "index is not public"
	}

	if index.Primary() {
		return false, "cannot check primary index consistency against itself"
	}
	...
	for i := 0; i < index.NumKeyColumns(); i++ {
		...
		if col != nil && col.IsVirtual() {
			return false, "index on virtual column"
		}
	}
