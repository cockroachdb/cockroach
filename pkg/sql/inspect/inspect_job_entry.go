...
func ChecksForTable(
	...
	if uniquenessCheckEnabled.Get(execCfg.SV()) && execCfg.Settings.Version.IsActive(ctx, clusterversion.V26_2) {
		isMockedRegionalByRow := isTesting() && priIndex.IndexDesc().KeyColumnNames[0] == "crdb_region"
		if uniqueColIdxs, err := findUniqueColIdxs(table, priIndex); err != nil {
			return nil, err
		} else if (table.IsLocalityRegionalByRow() || isMockedRegionalByRow) && len(uniqueColIdxs) == 1 {
...
func checksByIndexNames(
	...
	for _, idx := range indexes {
		...
		if uniquenessCheckEnabled.Get(execCfg.SV()) && execCfg.Settings.Version.IsActive(ctx, clusterversion.V26_2) {
			if !index.Primary() {
				isMockedRegionalByRow := isTesting() && index.IndexDesc().KeyColumnNames[0] == "crdb_region"
