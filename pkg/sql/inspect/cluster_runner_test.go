...
func (m *mockInspectClusterCheck) Started() bool {
	return m.started
}

func (m *mockInspectClusterCheck) StartCluster(context.Context, *jobspb.InspectProgress) error {
	if m.started {
...
func TestClusterRunnerClose(t *testing.T) {
	...
	t.Run("close with errors", func(t *testing.T) {
		...
		err := runner.Close(ctx)
		require.Error(t, err)
		// errors.CombineErrors creates a combined error. The exact format depends on
		// the implementation, so we just verify we got a non-nil error.
...
func (e *errorInspectClusterCheck) Started() bool {
	return e.started
}

func (e *errorInspectClusterCheck) StartCluster(context.Context, *jobspb.InspectProgress) error {
	if e.startErr != nil {
		return e.startErr
	}
	e.started = true
