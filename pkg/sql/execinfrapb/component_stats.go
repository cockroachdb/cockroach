...
func (s *ComponentStats) formatStats(fn func(suffix string, value interface{})) {
	...
	if s.NetTx.TuplesSent.HasValue() {
		fn("network tuples sent", humanize.Comma(int64(s.NetTx.TuplesSent.Value())))
	}
	...
	case 1:
		if s.Inputs[0].NumTuples.HasValue() {
			fn("input tuples", humanize.Comma(int64(s.Inputs[0].NumTuples.Value())))
		}
	...
	case 2:
		if s.Inputs[0].NumTuples.HasValue() {
			fn("left tuples", humanize.Comma(int64(s.Inputs[0].NumTuples.Value())))
		}
		if s.Inputs[0].WaitTime.HasValue() {
			fn("left stall time", humanizeutil.Duration(s.Inputs[0].WaitTime.Value()))
		}
		if s.Inputs[1].NumTuples.HasValue() {
			fn("right tuples", humanize.Comma(int64(s.Inputs[1].NumTuples.Value())))
		}
	...
	if s.KV.TuplesRead.HasValue() {
		fn("KV tuples read", humanize.Comma(int64(s.KV.TuplesRead.Value())))
	}
	...
	if s.Output.NumBatches.HasValue() {
		fn("batches output", humanize.Comma(int64(s.Output.NumBatches.Value())))
	}
	if s.Output.NumTuples.HasValue() {
		fn("tuples output", humanize.Comma(int64(s.Output.NumTuples.Value())))
	}
}
