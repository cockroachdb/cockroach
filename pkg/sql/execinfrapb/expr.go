...
// AddExpression sets the given expression as the i-th expression in the helper.
func (eh *MultiExprHelper) AddExpr(ctx context.Context, expr Expression, i int) (err error) {
	if eh.exprs[i], err = eh.h.prepareExpr(ctx, expr); err != nil {
		return err
	}
	return nil
}
...
// prepareExpr returns converts the given Expression into a tree.TypedExpr.
