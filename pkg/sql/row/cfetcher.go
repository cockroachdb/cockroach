...
state: noRow
  ...
  3. parse key (past common prefix) into row buffer, setting last row prefix buffer
  4. interleave detected?
     - set skip prefix
     -> seekPrefix(noRow)
  5. parse value into row buffer.
  6. 1-cf or secondary index?
    -> doneRow(initFetch)
  else:
    -> hasRow

state: seekPrefix(nextState)
  1. fetch next kv into nextKV buffer
...
const (
	...
	// look at. Fetch the next key.
	stateInitFetch
	// stateNoRow is the state of looking at a key that is part of a row that
	// the fetcher hasn't processed before. s.machine.nextKV must be set.
	stateNoRow
	...
	stateSeekPrefix
	// stateHasRow is the state of getting a new key for the current row.
	stateHasRow
	// stateDoneRow is the state of finalizing a row. It assumes that no more keys
	// for the current row are present.
	// state[1] must be set, and doneRow will transition to that state once it
	// finishes finalizing the row.
	stateDoneRow
...
STATEMACHINE:
	for {
		...
		case stateNoRow:
			...
			rf.machine.state[0] = stateHasRow
