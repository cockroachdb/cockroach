...
type cTableInfo struct {
	...
	// fullExtraValColOrdinals is the same as extraValColOrdinals but
	// does not contain any -1's. It is meant to be used only in logging.
	fullExtraValColOrdinals []int
...
func (rf *CFetcher) Init(
	...
	table := cTableInfo{
		...
		fullExtraValColOrdinals: oldTable.fullExtraValColOrdinals[:0],
	}
	...
	for i, id := range indexColumnIDs {
		...
		if cHasExtraCols(&table) {
			...
			if cap(table.fullExtraValColOrdinals) >= nExtraColumns {
				table.fullExtraValColOrdinals = table.fullExtraValColOrdinals[:nExtraColumns]
			} else {
				table.fullExtraValColOrdinals = make([]int, nExtraColumns)
			}

			for i, id := range table.index.ExtraColumnIDs {
				table.fullExtraValColOrdinals[i] = tableArgs.ColIdxMap[id]
				if neededCols.Contains(int(id)) {
		...
		if cHasExtraCols(table) {
			...
			if rf.traceKV {
				...
				for j := range table.extraTypes {
					idx := table.fullExtraValColOrdinals[j]
					buf.WriteByte('/')
					buf.WriteString(rf.getDatumAt(idx, rf.machine.rowIdx, rf.table.cols[idx].Type).String())
				}
