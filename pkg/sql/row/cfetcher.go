...
func (rf *CFetcher) Init(
	...
	for tableIdx, tableArgs := range tables {
		...
		if cHasExtraCols(&table) {
			...
			if cap(table.extraValColIdx) >= nExtraColumns {
				table.extraValColIdx = table.extraValColIdx[:nExtraColumns]
			} else {
				table.extraValColIdx = make([]int, nExtraColumns)
			}
			for i, id := range table.index.ExtraColumnIDs {
				if table.neededCols.Contains(int(id)) {
					table.extraValColIdx[i] = table.colIdxMap[id]
				} else {
					table.extraValColIdx[i] = -1
				}
			}
			if err != nil {
				return err
...
func (rf *CFetcher) NextKey(ctx context.Context) (rowDone bool, err error) {
	...
	for {
		...
		if unchangedPrefix {
			...
			if _, ok := encoding.DecodeIfInterleavedSentinel(keySuffix); ok {
...
func (rf *CFetcher) ReadIndexKey(key roachpb.Key) (remaining []byte, ok bool, err error) {
...
func (rf *CFetcher) processValueSingle(
	...
	if rf.traceKV || table.neededCols.Contains(int(colID)) {
		if idx, ok := table.colIdxMap[colID]; ok {
			...
			// single values.
			value, err := sqlbase.UnmarshalColumnValue(nil, typ, kv.Value)
			if err != nil {
				return "", "", err
			}
			if rf.traceKV {
