...
func makeFkExistenceCheckBaseHelper(
	...
	// Since we are just checking for existence, we only need to check that family 0 is present.
	builder := MakeSpanBuilder(searchTable.TableDesc(), searchIdx)
	builder.SetNeededFamilies([]sqlbase.FamilyID{0})
