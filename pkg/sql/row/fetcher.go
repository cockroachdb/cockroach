...
type KVBatchFetcher interface {
	...
	// GetKVCpuTime returns the cumulative CPU time (in nanoseconds) spent
	// processing BatchResponses at the KV layer. It is safe for concurrent use
...
// GetKVCpuTime returns total number of CPU time spent by the underlying KVFetcher.
