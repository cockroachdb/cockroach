...
func prepareInsertOrUpdateBatch(
	...
	pkCPutFn func(ctx context.Context, b Putter, key *roachpb.Key, value *roachpb.Value, expVal []byte, traceKV bool),
	...
	for i := range families {
		...
		*kvKey = keys.MakeFamilyKey(primaryIndexKey, uint32(family.ID))
		...
		if len(family.ColumnIDs) == 1 && family.ColumnIDs[0] == family.DefaultColumnID && family.ID != 0 {
			...
			var oldVal []byte
			if pkCPutFn != nil && len(oldValues) > 0 {
			...
			if marshaled.RawBytes == nil {
				if overwrite {
					...
					if pkCPutFn != nil {
						pkCPutFn(ctx, batch, kvKey, &marshaled, oldVal, traceKV)
					} else {
		...
		var expBytes []byte
		if pkCPutFn != nil && len(oldBytes) > 0 {
			old := &roachpb.Value{}
		...
		if family.ID != 0 && len(rawValueBuf) == 0 {
			if overwrite {
				...
				if pkCPutFn != nil {
					pkCPutFn(ctx, batch, kvKey, kvValue, expBytes, traceKV)
				} else {
					insertDelFn(ctx, batch, kvKey, traceKV)
				}
			}
