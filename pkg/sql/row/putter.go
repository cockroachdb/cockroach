...
func (t *TracePutter) CPutValues(kys []roachpb.Key, values []roachpb.Value) {
	...
	t.Putter.CPutValues(kys, values)
...
func (t *TracePutter) CPutTuples(kys []roachpb.Key, values [][]byte) {
	...
	t.Putter.CPutTuples(kys, values)
}
...
func (s *SortingPutter) CPutValues(kys []roachpb.Key, values []roachpb.Value) {
	kvs := KVVals{Keys: kys, Values: values}
	sort.Sort(&kvs)
	s.Putter.CPutValues(kvs.Keys, kvs.Values)
}

func (s *SortingPutter) CPutTuples(kys []roachpb.Key, values [][]byte) {
	kvs := KVBytes{Keys: kys, Values: values}
	sort.Sort(&kvs)
	s.Putter.CPutTuples(kvs.Keys, kvs.Values)
}
...
func (k *KVBatchAdapter) CPutValues(kys []roachpb.Key, values []roachpb.Value) {
	k.Batch.CPutValues(&kvSparseSliceBulkSource[roachpb.Value]{kys, values})
}

func (k *KVBatchAdapter) CPutTuples(kys []roachpb.Key, values [][]byte) {
	k.Batch.CPutTuples(&kvSparseSliceBulkSource[[]byte]{kys, values})
}
