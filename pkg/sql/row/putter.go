...
type Putter interface {
	...
	PutLockExclusive(key, value interface{})
	Del(key ...interface{})
...
func (t *TracePutter) PutLockExclusive(key, value interface{}) {
	log.VEventfDepth(t.Ctx, 1, 2, "Put (locking) %v -> %v", key, value)
	t.Putter.PutLockExclusive(key, value)
}
...
func (s *SortingPutter) PutLockExclusive(key, value interface{}) {
	s.Putter.PutLockExclusive(key, value)
}
...
func (k *KVBatchAdapter) PutLockExclusive(key, value interface{}) {
	k.Batch.PutLockExclusive(key, value)
}
