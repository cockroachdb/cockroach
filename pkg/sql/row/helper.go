...
func newRowHelper(
	...
	rh.primIndexValDirs = catalogkeys.IndexKeyValDirs(rh.TableDesc.PrimaryIndexInterface().IndexDesc())
...
func (rh *rowHelper) encodePrimaryIndex(
	...
	primaryIndexKey, _, err = rowenc.EncodeIndexKey(
		rh.TableDesc, rh.TableDesc.PrimaryIndexInterface().IndexDesc(), colIDtoRowIndex, values, rh.primaryIndexKeyPrefix)
...
func (rh *rowHelper) skipColumnInPK(colID descpb.ColumnID, value tree.Datum) (bool, error) {
	if rh.primaryIndexCols.Empty() {
		for i := 0; i < rh.TableDesc.PrimaryIndexInterface().NumColumns(); i++ {
			pkColID := rh.TableDesc.PrimaryIndexInterface().GetColumnID(i)
