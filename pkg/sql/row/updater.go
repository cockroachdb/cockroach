...
func (ru *Updater) UpdateRow(
	...
	if rowPrimaryKeyChanged {
		...
		// index doesn't change, so rather than performing a Del followed by a
		// CPut we could skip the Del altogether. Furthermore, if we acquired
		// the lock on this index during the initial scan we could replace a
		// CPut with a Put.
