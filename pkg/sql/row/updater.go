			...
			for oldIdx < len(oldEntries) && newIdx < len(newEntries) {
				...
				if oldEntry.Family == newEntry.Family {
					...
					if !bytes.Equal(oldEntry.Key, newEntry.Key) {
						...
						//
						// For merging indexes we will compare timestamps during the merge
						// process so all updates should always be captured into the the
						// final secondary index that is being merged.
