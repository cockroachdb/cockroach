...
func (rd *Deleter) DeleteRow(
	...
	pkDeleter func(ctx context.Context, b *kv.Batch, key roachpb.Key, expVal []byte, traceKV bool),
	traceKV bool,
) error {
	...
	return rd.Helper.TableDesc.ForeachFamily(func(family *descpb.ColumnFamilyDescriptor) error {
		...
		if pkDeleter != nil {
			prevValue, familyRequiresEncoding, err := rd.encodeValueForPrimaryIndexFamily(family, values)
			if err != nil {
				return err
			}
			if !familyRequiresEncoding {
				return nil
			}
			var expValue []byte
			...
			pkDeleter(ctx, b, rd.key, expValue, traceKV)
		} else {
			if traceKV {
...
func (rd *Deleter) encodeValueForPrimaryIndexFamily(
	...
	if len(family.ColumnIDs) == 1 && family.ColumnIDs[0] == family.DefaultColumnID && family.ID != 0 {
		...
		if !ok {
			return roachpb.Value{}, false, nil
		}
		...
		if err != nil {
			return roachpb.Value{}, false, err
		}
	...
	if !ok {
		return roachpb.Value{}, false, errors.AssertionFailedf("invalid family sorted column id map")
	}
	for _, colID := range familySortedColumnIDs {
		...
		if lastColID > col.GetID() {
			return roachpb.Value{}, false, errors.AssertionFailedf("cannot write column id %d after %d", col.GetID(), lastColID)
		}
