...
func TestExternalRowData(t *testing.T) {
	...
	r.Exec(t, `CREATE TABLE t (k INT PRIMARY KEY)`)
	r.Exec(t, `CREATE TABLE t_copy (k INT PRIMARY KEY)`)
	...
	r.Exec(t, `INSERT INTO t SELECT generate_series(1, 3)`)
	asOf := hlc.Timestamp{WallTime: timeutil.Now().UnixNano()}
	r.Exec(t, `INSERT INTO t SELECT generate_series(4, 6)`)
