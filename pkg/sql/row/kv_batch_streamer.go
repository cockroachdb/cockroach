...
func (f *TxnKVStreamer) proceedWithLastResult(
	...
	if get := result.GetResp; get != nil {
		...
		pos := result.EnqueueKeys[f.lastResultState.numEmitted]
...
func (f *TxnKVStreamer) nextBatch(
	...
	if f.lastResultState.numEmitted < len(f.lastResultState.EnqueueKeys) {
	...
	if f.lastResultState.numEmitted == len(f.lastResultState.EnqueueKeys) && f.lastResultState.numEmitted > 0 {
		f.releaseLastResult(ctx)
	}
