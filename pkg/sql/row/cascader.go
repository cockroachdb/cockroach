...
type cascader struct {
	...
	indexPKRowFetchers map[sqlbase.ID]map[sqlbase.IndexID]Fetcher // PK RowFetchers by Table ID and Index ID

	// Row Deleters
	rowDeleters        map[sqlbase.ID]Deleter               // RowDeleters by Table ID
	deleterRowFetchers map[sqlbase.ID]Fetcher               // RowFetchers for rowDeleters by Table ID
	deletedRows        map[sqlbase.ID]*sqlbase.RowContainer // Rows that have been deleted by Table ID

	// Row Updaters
	rowUpdaters        map[sqlbase.ID]Updater               // RowUpdaters by Table ID
	updaterRowFetchers map[sqlbase.ID]Fetcher               // RowFetchers for rowUpdaters by Table ID
	originalRows       map[sqlbase.ID]*sqlbase.RowContainer // Original values for rows that have been updated by Table ID
	updatedRows        map[sqlbase.ID]*sqlbase.RowContainer // New values for rows that have been updated by Table ID
}
...
Outer:
	...
	return &cascader{
		...
		indexPKRowFetchers: make(map[sqlbase.ID]map[sqlbase.IndexID]Fetcher),
		rowDeleters:        make(map[sqlbase.ID]Deleter),
		deleterRowFetchers: make(map[sqlbase.ID]Fetcher),
		deletedRows:        make(map[sqlbase.ID]*sqlbase.RowContainer),
		rowUpdaters:        make(map[sqlbase.ID]Updater),
		updaterRowFetchers: make(map[sqlbase.ID]Fetcher),
		originalRows:       make(map[sqlbase.ID]*sqlbase.RowContainer),
		updatedRows:        make(map[sqlbase.ID]*sqlbase.RowContainer),
		evalCtx:            evalCtx,
		alloc:              alloc,
	}, nil
}
