...
func createMockImportJob(
	...
	seqIDToAllocatedChunks map[int32]*jobspb.SequenceChunks,
	resumePos int64,
) *jobs.Job {
...
func TestJobBackedSeqChunkProvider(t *testing.T) {
	...
	testCases := []struct {
		...
		allocatedChunks map[int32]*jobspb.SequenceChunks
		...
		},
		{
			...
			allocatedChunks: map[int32]*jobspb.SequenceChunks{56: {Chunks: []*jobspb.SequenceValChunk{{
				ChunkStartVal:     1,
				ChunkStartRow:     0,
				ChunkSize:         10,
		...
		},
		{
			...
			name: "need-new-chunk",
			allocatedChunks: map[int32]*jobspb.SequenceChunks{57: {Chunks: []*jobspb.SequenceValChunk{{
				ChunkStartVal:     1,
				ChunkStartRow:     0,
				ChunkSize:         10,
				NextChunkStartRow: 10,
		...
		},
		{
			...
			allocatedChunks: map[int32]*jobspb.SequenceChunks{58: {Chunks: []*jobspb.SequenceValChunk{{
				ChunkStartVal:     1,
				ChunkStartRow:     0,
				ChunkSize:         10,
				NextChunkStartRow: 10,
	...
	for _, test := range testCases {
		t.Run(test.name, func(t *testing.T) {
			...
			j := &JobBackedSeqChunkProvider{JobID: *job.ID()}
			...
			for id, val := range test.seqIDToExpectedVal {
				...
				seqMetadata := &SequenceMetadata{
					...
					curChunk:        nil,
					curVal:          0,
					increment:       test.seqIDToOpts[id].Increment,
				}
				require.NoError(t, j.RequestChunk(evalCtx, annot, nil, seqMetadata))
