...
// TODO(yuzefovich): since we nil out the 0th element, the caller of nextBatch()
// will have the only reference to it. As a result, the next time nextBatch() is
// called previously-returned element should become garbage, and we could shrink
// the memory usage accordingly. In other words, we're including garbage into
// the memory account. However, given our history of under-accounting in most
// places, this seems acceptable.
func popBatch(
