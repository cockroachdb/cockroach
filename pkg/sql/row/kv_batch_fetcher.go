...
// batchBytesLimit is set, the number of result keys per batch is limited; the
// limit grows between subsequent batches, starting at firstBatchKeyLimit (if
// not 0) to productionKVBatchSize.
//
...
func makeKVBatchFetcher(
	...
	return makeKVBatchFetcherWithSendFunc(
		sendFn, spans, reverse, batchBytesLimit, firstBatchKeyLimit, lockStrength,
