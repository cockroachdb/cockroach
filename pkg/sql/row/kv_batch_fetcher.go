...
type txnKVFetcher struct {
	...
	// spanIDs, if non-nil, is a slice of integers that are associated with the
	// corresponding spans in the spans slice. In other words, it contains
	// 1-to-1 mapping with the spans slice allowing the caller to easily
	// identify the input span that produced a particular kv.
	spanIDs []int
