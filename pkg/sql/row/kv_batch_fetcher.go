...
func makeKVBatchFetcherWithSendFunc(
	...
	if requestAdmissionHeader.CreateTime == 0 {
		log.Warningf(context.TODO(), "txnKVFetcher has uninitialized AdmissionHeader")
	}
	return txnKVFetcher{
		sendFn:                     sendFn,
		spans:                      copySpans,
...
func (f *txnKVFetcher) fetch(ctx context.Context) error {
	...
	if f.responseAdmissionQ != nil {
		...
		log.Warningf(ctx, "txnKVFetcher does not have a admission.WorkQueue for the response")
	}
	if br != nil {
