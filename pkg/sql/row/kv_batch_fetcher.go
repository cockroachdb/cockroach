...
// DefaultKVBatchSize is the default number of keys we request at a time.
...
// SetDefaultKVBatchSize changes the DefaultKVBatchSize, and returns a function
// that restores it.
func SetDefaultKVBatchSize(val int64) func() {
...
func (f *txnKVFetcher) getBatchSizeForIdx(batchIdx int) int64 {
	...
	if f.firstBatchLimit == 0 || f.firstBatchLimit >= f.batchSize {
		return f.batchSize
	}
	...
	case 1:
		...
		switch {
		case secondBatch < f.batchSize/10:
			return f.batchSize / 10
		case secondBatch > f.batchSize:
			return f.batchSize
		default:
			return secondBatch
		}
