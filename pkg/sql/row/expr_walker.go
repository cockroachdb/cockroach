...
func (j *SeqChunkProvider) RequestChunk(
	...
	return evalCtx.DB.Txn(evalCtx.Context, func(ctx context.Context, txn *kv.Txn) error {
		job, err := j.Registry.LoadJob(evalCtx.Context, j.JobID)
		if err != nil {
			return err
		}
		err = job.WithTxn(txn).Update(evalCtx.Context, func(txn *kv.Txn, md jobs.JobMetadata,
			ju *jobs.JobUpdater) error {
