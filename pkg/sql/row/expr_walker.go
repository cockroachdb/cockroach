...
// SeqChunkProvider is an interface for implementations which find/create a
// chunk of sequence values corresponding to the row being imported.
type SeqChunkProvider interface {
	RequestChunk(
		evalCtx *tree.EvalContext,
		c *CellInfoAnnotation,
		curSeqChunk *jobspb.SequenceValChunk,
		seqMetadata *SequenceMetadata,
	) error
}

// JobBackedSeqChunkProvider implements the SeqChunkProvider interface. It uses
// the import job progress to read and write its sequence value chunks.
type JobBackedSeqChunkProvider struct {
	JobID int64
}

var _ SeqChunkProvider = &JobBackedSeqChunkProvider{}
...
func (j *JobBackedSeqChunkProvider) RequestChunk(
	evalCtx *tree.EvalContext,
	c *CellInfoAnnotation,
	curSeqChunk *jobspb.SequenceValChunk,
	seqMetadata *SequenceMetadata,
	...
	// nextval().
	if found, err := j.checkForPreviouslyAllocatedChunks(evalCtx, seqMetadata, c); err != nil {
		return err
	} else if found {
		return nil
	}
	...
	if err := reserveChunkOfSeqVals(evalCtx, c, curSeqChunk, seqMetadata); err != nil {
		return err
	}
	...
	return evalCtx.DB.Txn(evalCtx.Context, func(ctx context.Context, txn *kv.Txn) error {
		getJobProgressQuery := `SELECT progress FROM system.jobs J WHERE J.id = $1 FOR UPDATE`
		ex := evalCtx.InternalExecutor.(sqlutil.InternalExecutor)
		row, err := ex.QueryRow(ctx, "nextval-import-job-progress", txn,
			getJobProgressQuery, j.JobID)
		if err != nil {
			return err
		}
		...
		if fileProgress.SeqIdToChunks == nil {
			fileProgress.SeqIdToChunks = make(map[int32]*jobspb.SequenceChunks)
		}
		seqID := seqMetadata.id
		if _, ok := fileProgress.SeqIdToChunks[int32(seqID)]; !ok {
			fileProgress.SeqIdToChunks[int32(seqID)] = &jobspb.SequenceChunks{
			...
			chunksCopy := fileProgress.SeqIdToChunks[int32(seqID)].Chunks[:0]
			for _, chunk := range fileProgress.SeqIdToChunks[int32(seqID)].Chunks {
				// If the resumePos is below the max bound of the current chunk we
				// need to keep this chunk in case the job is re-resumed.
				if chunk.NextChunkStartRow > resumePos {
					chunksCopy = append(chunksCopy, chunk)
				}
			}
			fileProgress.SeqIdToChunks[int32(seqID)].Chunks = chunksCopy
		}
		...
		updateJobProgressQuery := `UPDATE system.jobs SET progress = $1 WHERE id = $2`
		_, err = ex.Exec(ctx, "nextval-import-job-progress-update", txn, updateJobProgressQuery,
			progressBytes, j.JobID)
		if err != nil {
			return err
		}
...
func (j *JobBackedSeqChunkProvider) checkForPreviouslyAllocatedChunks(
	evalCtx *tree.EvalContext, seqMetadata *SequenceMetadata, c *CellInfoAnnotation,
) (bool, error) {
	var found bool
	getJobProgressQuery := `SELECT progress FROM system.jobs J WHERE J.id = $1`
	ex := evalCtx.InternalExecutor.(sqlutil.InternalExecutor)
	row, err := ex.QueryRow(evalCtx.Context, "nextval-import-job-progress", nil, /* txn */
		getJobProgressQuery, j.JobID)
	if err != nil {
		return found, err
	}
	...
	var allocatedSeqChunks *jobspb.SequenceChunks
	var ok bool
	...
	for _, chunk := range allocatedSeqChunks.Chunks {
		...
		if chunk.ChunkStartRow <= c.rowID && chunk.NextChunkStartRow > c.rowID {
			...
			seqMetadata.curVal = chunk.ChunkStartVal +
				seqMetadata.increment*(seqMetadata.instancesPerRow*relativeRowIndex)
			found = true
			return found, nil
		}
	}
...
func reserveChunkOfSeqVals(
	...
	// to job progress.
	if curSeqChunk != nil {
		if curSeqChunk.ChunkSize >= maxChunkSize {
			newChunkSize = curSeqChunk.ChunkSize
		} else {
	...
	incrementValBy := newChunkSize * seqMetadata.increment
	...
	seqMetadata.curChunk = &jobspb.SequenceValChunk{
		ChunkStartVal:     seqVal - incrementValBy + seqMetadata.increment,
...
func importNextVal(evalCtx *tree.EvalContext, args tree.Datums) (tree.Datum, error) {
	...
	curSeqChunk := seqMetadata.curChunk
	...
	if curSeqChunk == nil || c.rowID == curSeqChunk.NextChunkStartRow {
		if err := c.seqChunkProvider.RequestChunk(evalCtx, c, curSeqChunk, seqMetadata); err != nil {
			return nil, err
		}
	} else {
		// The current chunk of sequence values can be used for the row being
		// processed.
		seqMetadata.curVal += seqMetadata.increment
	}
