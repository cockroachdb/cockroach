...
func (c *DatumRowConverter) Row(ctx context.Context, sourceID int32, rowIndex int64) error {
	...
	annot := c.EvalCtx.Annotations.Get(cellInfoAddr).(cellInfoAnnot)
	annot.uniqueRowIDInst = 0
	annot.sourceID = sourceID
	annot.rowID = rowIndex
	c.EvalCtx.Annotations.Set(cellInfoAddr, annot)
	for i := range c.cols {
		col := &c.cols[i]
