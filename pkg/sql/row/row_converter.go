...
// our own function definition, which is why we have the overloads.
...
// For the set of non-immutable functions we support, we wrap them
// in a wrapper considering funcDef: the overriden definition (if any)
// we are using, and a "volatile" flag to indicate whether we should
// wait until the Row() stage to evaluate instead of the NewDatumRowConverter().
type overrideWrapper struct {
	funcDef  *tree.FunctionDefinition
	volatile bool
}

const (
	SourceAddr tree.AnnotationIdx = iota + 1
	RowAddr
	ColAddr
	uniqueRowIDAddr
)

func getAddressAnnotations(ann tree.Annotations) (int32, int64, int) {
	return ann.Get(SourceAddr).(int32), ann.Get(RowAddr).(int64), ann.Get(ColAddr).(int)
}
...
// all listed here.
var supportedImportFuncOverrides = map[string]overrideWrapper{
	...
	"current_date":          overrideWrapper{nil, false},
	"current_timestamp":     overrideWrapper{nil, false},
	"localtimestamp":        overrideWrapper{nil, false},
	"now":                   overrideWrapper{nil, false},
	"statement_timestamp":   overrideWrapper{nil, false},
	"timeofday":             overrideWrapper{nil, false},
	"transaction_timestamp": overrideWrapper{nil, false},
	...
	"unique_rowid": overrideWrapper{
		funcDef: makeBuiltinOverride(
			tree.FunDefs["unique_rowid"],
			tree.Overload{
				Types:      tree.ArgTypes{},
				ReturnType: tree.FixedReturnType(types.Int),
				Fn: func(evalCtx *tree.EvalContext, args tree.Datums) (tree.Datum, error) {
					sourceID, rowIndex, colID := getAddressAnnotations(*evalCtx.Annotations)
					uniqueRowIDMap := (*evalCtx.Annotations).Get(uniqueRowIDAddr).(map[int]int)
					avoidCollisionsWithSQLsIDs := uint64(1 << 63)
					shiftedIndex := int64(len(uniqueRowIDMap))*int64(rowIndex) + int64(uniqueRowIDMap[colID])
...
func (v *importDefaultExprVisitor) VisitPost(expr tree.Expr) (newExpr tree.Expr) {
	...
	case *tree.FuncExpr:
		if fn.ResolvedOverload().Volatility > tree.VolatilityImmutable {
			...
			if override, isSafe := supportedImportFuncOverrides[resolvedFnName]; !isSafe {
				v.err = errors.Newf(`function %s unsupported by IMPORT INTO`, resolvedFnName)
			} else {
				if override.volatile {
					v.volatile = true
				}
				// No override exists, means it's okay to use the definitions given in builtin.go.
				if override.funcDef == nil {
					return expr
				}
				funcExpr := &tree.FuncExpr{
					Func:  tree.ResolvableFunctionReference{FunctionReference: override.funcDef},
					Type:  fn.Type,
					Exprs: fn.Exprs,
				}
...
type DatumRowConverter struct {
	...
	defaultUniqueIDMap    map[int]int
	ri                    Inserter
	EvalCtx               *tree.EvalContext
...
func NewDatumRowConverter(
	...
	hidden := -1
	c.defaultUniqueIDMap = make(map[int]int, 0)
	for i := range cols {
		col := &cols[i]
		if col.HasDefault() && col.DefaultExprStr() == "unique_rowid()" {
			val := len(c.defaultUniqueIDMap)
			c.defaultUniqueIDMap[i] = val
		}
		if col.Hidden {
...
func (c *DatumRowConverter) Row(ctx context.Context, sourceID int32, rowIndex int64) error {
	...
	newAnnot := tree.MakeAnnotations(4)
	c.EvalCtx.Annotations = &newAnnot
	c.EvalCtx.Annotations.Set(SourceAddr, sourceID)
	c.EvalCtx.Annotations.Set(RowAddr, rowIndex)
	c.EvalCtx.Annotations.Set(uniqueRowIDAddr, c.defaultUniqueIDMap)
	for i := range c.cols {
		col := &c.cols[i]
		if !isTargetCol(i) && col.DefaultExpr != nil {
			c.EvalCtx.Annotations.Set(ColAddr, i)
