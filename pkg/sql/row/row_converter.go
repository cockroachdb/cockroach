...
func (c *DatumRowConverter) Row(ctx context.Context, sourceID int32, rowIndex int64) error {
	if c.hidden >= 0 {
		...
		// puts the uniqueness-ensuring per-generator part (nodeID) in the low-bits.
		// Instead, make our own IDs that attempt to keep each generator (sourceID)
		// writing to its own keu-space with sequential rowIndexes mapping to also
		// sequential unique IDs, by putting the rowID in the lower bits. To avoid
		// collisions with the SQL-genenerated IDs (at least for a very long time)
		// we also flip the top bit to 1.
		//
