...
// KVBatch represnets a batch of KVs generated from converted rows.
type KVBatch struct {
	// Source is where the row data in the batch came from.
	Source int32
	// LastRow is the index last converted row in source in this batch.
	LastRow uint64
	// Progress is represents the fraction of the input that generated this row.
	Progress float32
