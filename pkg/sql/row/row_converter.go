...
// if necessary.
func (c *DatumRowConverter) Row(ctx context.Context, sourceIndex int32, rowIndex int64) error {
	if c.hidden >= 0 {
		// We don't want to call unique_rowid() for the hidden PK column because it
		// is not idempotent and has unfortunate overlapping of output spans with
		// the "node" part used to avoid collisions being in the low-bits. Instead,
		// a) flip the top bit to minimize collisions with unique_rowid(), then
		// b) use 14 bits to indicate the i'th source (e.g. i'th file in IMPORT) and
		// c) use the remaining 49 low bits for time + i'th row of source.
		//
		// Flipping the order of the per-source (node of file ID) vs per-row
		// (incremented time/row index) bits compared to unique_rowid yields
		// contiguous spans of KVs for contiguous rows of a given source/file and
		// minimizes overlapping between different sources, yielding observed ~2-3x
		// improvements in performance and even more significant reductions in
		// required compactions during IMPORT.
		i := -1*(int64(sourceIndex)<<rowIDBits) ^ rowIndex
