...
// in a wrapper considering funcDef: the overridden definition (if any)
...
// These constants are addresses used to store relevant information
...
var supportedImportFuncOverrides = map[string]overrideWrapper{
	...
	"current_date":          {nil, false},
	"current_timestamp":     {nil, false},
	"localtimestamp":        {nil, false},
	"now":                   {nil, false},
	"statement_timestamp":   {nil, false},
	"timeofday":             {nil, false},
	"transaction_timestamp": {nil, false},
		...
		funcDef: makeBuiltinOverride(
			...
			tree.Overload{
				...
				Fn: func(evalCtx *tree.EvalContext, args tree.Datums) (tree.Datum, error) {
					...
					shiftedIndex := int64(len(uniqueRowIDMap))*rowIndex + int64(uniqueRowIDMap[colID])
...
func NewDatumRowConverter(
	...
	hidden := -1
	c.defaultUniqueIDMap = make(map[int]int)
	for i := range cols {
		col := &cols[i]
