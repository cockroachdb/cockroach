...
// TestAdminAuditLogBasicOldLogFormat verifies that after enabling the admin
// audit log, a SELECT statement executed by the admin user is logged to the
// audit log using the old log format.
func TestAdminAuditLogBasicOldLogFormat(t *testing.T) {
	defer leaktest.AfterTest(t)()
	sc := log.ScopeWithoutShowLogs(t)
	defer sc.Close(t)
	...
	db.Exec(t, `SET CLUSTER SETTING sql.log.admin_audit.enabled = true;`)
	db.Exec(t, `SET CLUSTER SETTING sql.log.unstructured_entries.enabled = true;`)
	db.Exec(t, `SELECT 1;`)

	var selectAdminRe = regexp.MustCompile(`‹exec› ‹""› ‹{admin}› ‹"SELECT 1"›`)
	...
	if len(entries) == 0 {
		t.Fatal(errors.New("entry not found"))
	}
}
