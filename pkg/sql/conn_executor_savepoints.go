...
func (ex *connExecutor) execSavepointInOpenState(
	...
	// Remember how many DDL statements were created so far. We'll need
	// this information if the savepoint is ever rolled back.
	entry.numDDL = ex.extraTxnState.numDDL
...
func (ex *connExecutor) execReleaseSavepointInOpenState(
	...
	if entry.isRestartSavepoint {
		...
		var ok bool
		retEv, retPayload, ok = ex.finalizeCommitTransactionInternal(ctx, s)
		if ok {
			retEv = eventTxnReleased{}
			retPayload = nil
		} else {
			// The release failed. All the savepoints are trashed, so we're
			// going to unwind the savepoint stack below in any case.
			// However, because the release failed it's likely the client
			// will want to rewind to the restart savepoint instead.  So we
			// don't finalize the release for that one and instead keep it,
			// operating like a rollback.
			idx++
		}
	} else {
		/* regular savepoint */
...
func (ex *connExecutor) execRollbackToSavepointInOpenState(
	...
	if entry.isRestartSavepoint {
		...
		// In the initial implementation, we don't yet support rolling
		// back over DDL. Instead of creating an inconsistent txn or
		// schema state, prefer to tell the users we don't know how to
		// proceed yet.
...
func (ex *connExecutor) execRollbackToSavepointInAbortedState(
	...
	if !entry.isRestartSavepoint {
		/* regular savepoint */
		return makeErr(unimplemented.NewWithIssueDetail(10735, "rollback-after-error", "ROLLBACK TO SAVEPOINT in aborted state"))
	}
