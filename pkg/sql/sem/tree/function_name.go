...
// NewRoutineUndefinedError returns an error that is marked with the
// ErrRoutineUndefined marker so that errors.Is will return true given the
// returned error and ErrRoutineUndefined.
func NewRoutineUndefinedError(format string, args ...interface{}) error {
	return errors.Mark(
		pgerror.Newf(pgcode.UndefinedFunction, format, args...),
		ErrRoutineUndefined,
	)
}
