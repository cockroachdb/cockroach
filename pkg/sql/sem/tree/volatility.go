...
// ToPostgres() returns the postgres "provolatile" string ("i" or "s" or "v")
// and the "proleakproof" flag.
...
func VolatilityFromPostgres(provolatile string, proleakproof bool) Volatility {
	switch provolatile {
	case "i":
		if proleakproof {
			return VolatilityLeakProof
		}
	...
	default:
		panic(errors.AssertionFailedf("invalid provolatile %s", provolatile))
	}
}
