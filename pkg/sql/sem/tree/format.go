...
const (
	...
	// FmtRedactNode instructs the pretty printer to redact datums, constants, and
	// simples names (i.e. Name, UnrestrictedName) from statements.
	FmtRedactNode
)
...
// formatNodeMaybeRedact redacts sensitive information from statements
// by inserting redaction markers around datums, constants, and simple names
// (i.e. Name, UnrestrictedName).
func (ctx *FmtCtx) formatNodeMaybeRedact(n NodeFormatter) {
	if ctx.flags.HasFlags(FmtRedactNode) {
		...
		case *DBool, *DInt, *DFloat:
			// DBool, DInt, DFloat types do not hold PII and should be distinguished
