...
const (
	...
	// - Show columns up to 15 characters.
	// INSERT SELECT: INSERT INTO {table} SELECT {columns} FROM {select-table}
	// - Show columns up to 15 characters.
	// UPDATE: UPDATE {table} SET {columns} WHERE {where-condition}
	// - Show columns up to 15 characters.
	// - Show where-condition up to 15 characters,
	FmtSummary
)
...
// FormatLimitLength pretty-prints a summarized select statement.
...
func (ctx *FmtCtx) FormatSummarySelect(node *Select) {
	if node.With == nil {
		...
		if s, ok := s.(*SelectClause); ok {
			...
			ctx.FormatLimitLength(&s.Exprs, 15)
			if len(s.From.Tables) > 0 {
				ctx.WriteByte(' ')
				ctx.FormatLimitLength(&s.From, 35)
			}
		}
	}
}
...
func (ctx *FmtCtx) FormatSummaryInsert(node *Insert) {
	...
	ctx.FormatNodeSummary(node.Table)
	rows := node.Rows
	expr := rows.Select
	if _, ok := expr.(*SelectClause); ok {
		...
		ctx.WriteString(" (")
		ctx.FormatLimitLength(&node.Columns, 15)
		ctx.WriteByte(')')
	}
}
...
func (ctx *FmtCtx) FormatSummaryUpdate(node *Update) {
	if node.With == nil {
		...
		ctx.FormatNodeSummary(node.Table)
		ctx.WriteString(" SET ")
		ctx.FormatLimitLength(&node.Exprs, 15)
		if node.Where != nil {
			ctx.WriteByte(' ')
			ctx.FormatLimitLength(node.Where, 21)
		}
	}
}
