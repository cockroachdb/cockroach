...
//
// If the target statement does not exactly match the donor (except for hints),
// an error is returned and no rewrite takes place.
//
// No semantic checks of the donor hints are performed. It is assumed that
// semantic checks happen elsewhere.
func (hd *HintInjectionDonor) InjectHints(
	stmt Statement, sv *settings.Values,
) (Statement, bool, error) {
	// Validate that the given statement exactly matches the hints donor (except
	// for constants and hints).
	sql := AsStringWithFlags(stmt, validationFmtFlags(sv))
	if sql != hd.validationSQL {
		return stmt, false, errors.Newf(
