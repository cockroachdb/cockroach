...
func typeCheckOverloadedExprs(
	...
	if inBinOp && len(s.overloadIdxs) == 1 {
		...
		// If we all AnyEnum, and we know at least one of the enums, try type cast
		// the rest.
		if attemptAnyEnumCast {
			// Copy exprs to prevent any overwrites of expr later.
			sCopy := s
