	...
	if len(s.constIdxs) > 0 {
		...
		if len(s.overloadIdxs) == 1 && allConstantsAreHomogenous {
			...
			if overloadParamsAreHomogenous {
				...
				return checkReturnPlaceholdersAtIdx(ctx, &s, int(s.overloadIdxs[0]))
			}
		}
		for _, i := range s.constIdxs {
			constExpr := exprs[i].(Constant)
...
// checkReturnPlaceholdersAtIdx is like checkReturn, but to be used in
// the special case where the constants in the input expressions have
// already been typed.
