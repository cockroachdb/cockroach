...
func typeCheckOverloadedExprs(
	...
	// In a binary expression, in the case of one of the arguments being untyped
	// NULL, we prefer overloads where we infer the type of the NULL to be an
	// array whose contents are the same type as the other argument. This is used
	// to choose INT || NULL::INT[] over INT || NULL::STRING.
	if inBinOp && len(s.exprs) == 2 {
		if ok, typedExprs, fns, err := filterAttempt(ctx, semaCtx, &s, func() {
			var err error
			left := s.typedExprs[0]
			...
			if oneIsNull {
				if leftIsNull {
					leftType = types.MakeArray(rightType)
				}
				if rightIsNull {
					rightType = types.MakeArray(leftType)
				}
				s.overloadIdxs = filterOverloads(s.overloads, s.overloadIdxs,
					func(o overloadImpl) bool {
						return o.params().GetAt(0).Equivalent(leftType) &&
