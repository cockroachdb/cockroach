...
type AsOfSystemTime struct {
	...
	NearestOnly bool
	// If this is a bounded staleness read with nearest_only=True, this is set
	// when we failed to satisfy a bounded staleness with a nearby replica as
	// we have no followers with an up-to-date schema.
	// In this case, we want a read that is >= Timestamp and <= MaxTimestampBound.
