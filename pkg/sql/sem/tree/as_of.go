...
func EvalAsOfTimestamp(
	...
	case *DInt:
		if *d >= 0 {
			...
			// Treat negative integer values as ns offsets from the stmtTimestamp.
			ts.WallTime = stmtTimestamp.Add(time.Duration(*d)).UnixNano()
		}
	case *DDecimal:
		ts, convErr = DecimalToHLC(&d.Decimal)
		// Treat negative decimal values as offsets from the stmtTimestamp
