...
func TestFormatNodeSummary(t *testing.T) {
	...
	testData := []struct {
	...
	}{
		{
			...
			expected: `SELECT (SELECT FROM sy... FROM system.schedule...`,
		},
		{
			...
			expected: `SELECT status AS s, ap... FROM system.apps, sy...`,
		},
		{
			...
			expected: `SELECT app_name, aggre... FROM system.app_stat...`,
		},
		{
		...
		},
		{
			...
			expected: `INSERT INTO system.settings SELECT '_', value, "la... FROM system.settings...`,
		},
		{
		...
		},
		{
			...
			expected: `UPDATE system.extra_ex... SET (schedule_state... WHERE schedule_id = '...`,
		},
	}
