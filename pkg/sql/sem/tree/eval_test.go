...
func TestEvalError(t *testing.T) {
	testData := []struct {
		...
		{`like_escape('__', '___', '_')`, `LIKE regexp compilation failed: LIKE pattern must not end with escape character`},
		{`like_escape('%%', '%%%', '%')`, `LIKE regexp compilation failed: LIKE pattern must not end with escape character`},
		{`like_escape('abc', 'ab%', '%')`, `LIKE regexp compilation failed: pattern ends with escape character`},
		{`like_escape('abc', '%b%', '%')`, `LIKE regexp compilation failed: pattern ends with escape character`},
		{`like_escape('abc', 'ab_', '_')`, `LIKE regexp compilation failed: pattern ends with escape character`},
		{`like_escape('abc', '%b_', '_')`, `LIKE regexp compilation failed: pattern ends with escape character`},
	}
	for _, d := range testData {
		expr, err := parser.ParseExpr(d.expr)
		if err != nil {
