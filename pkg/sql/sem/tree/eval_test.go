	...
	t.Run("sql", func(t *testing.T) {
		...
		walk(t, func(t *testing.T, d *datadriven.TestData) string {
			...
			case types.TupleFamily:
				// ParseStringAs doesn't handle tuples, so we have to convert them ourselves.
				...
				for i, s := range strings.Split(res.String[1:len(res.String)-1], ",") {
					...
					datum, err := tree.ParseStringMustBe(expr.ResolvedType(), s, evalCtx)
					if err != nil {
						t.Errorf("%s: %s", err, s)
						return err.Error()
					}
			...
			datum, err := tree.ParseStringMustBe(typedExpr.ResolvedType(), res.String, evalCtx)
			if err != nil {
