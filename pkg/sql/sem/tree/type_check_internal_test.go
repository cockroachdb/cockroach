...
func castFunc(p *tree.Placeholder, typ *types.T) tree.Expr {
...
func TestProcessPlaceholderAnnotations(t *testing.T) {
	...
	testData := []struct {
		...
		{ // 4
			tree.PlaceholderTypes{nil, nil},
			[]tree.Expr{
				castFunc(newPlaceholder(0), intType),
				castFunc(newPlaceholder(0), boolType),
			},
			tree.PlaceholderTypes{nil, nil},
		},
		{ // 5
		...
		{ // 6
			...
			[]tree.Expr{
				castFunc(newPlaceholder(0), intType),
				castFunc(newPlaceholder(1), boolType),
			},
		...
		{ // 9
			...
			[]tree.Expr{
				...
				castFunc(newPlaceholder(1), boolType),
				castFunc(newPlaceholder(0), intType),
				castFunc(newPlaceholder(1), intType),
			},
...
func TestProcessPlaceholderAnnotationsError(t *testing.T) {
	...
	testData := []struct {
		...
		},
		{
			...
			[]tree.Expr{
				annot(newPlaceholder(0), floatType),
				castFunc(newPlaceholder(0), floatType),
				castFunc(newPlaceholder(1), floatType),
				annot(newPlaceholder(0), intType),
			},
			"multiple conflicting type annotations around \\$1",
		},
