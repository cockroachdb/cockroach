...
// 'escape' must be either empty (which disables the escape mechanism) or a single character.
...
func optimizedLikeFunc(
	...
	default:
		if !strings.ContainsAny(pattern[1:len(pattern)-1], "_%") {
			...
			// comparison.
			var err error
			if pattern, err = unescapePattern(pattern, escape, escape == `\`); err != nil {
				return nil, err
			}
			switch {
...
// 'escape' which must be either empty (which disables the escape mechanism) or a single character.
