...
func replaceUnescaped(s, oldStr, newStr string, escapeToken string) string {
	...
	// be escaped (or an underestimate if len(newStr) < len(oldStr).
	// This is fine since we keep track of the running number of bytes
	// actually copied (and this will naturally extend if we
	// underestimate).
	...
	ret := make([]byte, len(s)+nOld*(len(newStr)-len(oldStr)))
	retWidth := 0
	start := 0
OldLoop:
	for i := 0; i < nOld; i++ {
