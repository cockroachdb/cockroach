...
// JSONExistAny return true if any value in dArray is exist in the json
func JSONExistAny(_ *EvalContext, json DJSON, dArray *DArray) (*DBool, error) {
...
var CmpOps = cmpOpFixups(map[ComparisonOperator]cmpOpOverload{
	...
	JSONSomeExists: {
		&CmpOp{
			...
			Fn: func(ctx *EvalContext, left Datum, right Datum) (Datum, error) {
				return JSONExistAny(ctx, MustBeDJSON(left), MustBeDArray(right))
			},
			Volatility: VolatilityImmutable,
		},
	},
