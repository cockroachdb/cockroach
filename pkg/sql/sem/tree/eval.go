...
type EvalPlanner interface {
	...
	// ResolveOIDFromStringOrOID looks up the name or OID of an input OID
	// or string in the pg_catalog table that the input oid.Oid belongs to.
	// If the input Datum is a DOid, the relevant table will be queried by
	// OID; if the input is a DString, the table will be queried by its name
	// column.
	//
	...
	ResolveOIDFromStringOrOID(
		ctx context.Context, oidTyp *types.T, stringOrOIDToResolve Datum,
	) (*DOid, error)
