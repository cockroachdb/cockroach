...
type EvalPlanner interface {
	...
	// CompactEngineSpan is used to compact the span for the given rangeID, at
	// the given (nodeID, storeID). The parameter list here will be replaced by
	// a struct union when we add more overloads to the compact_span builtin.
	CompactEngineSpan(ctx context.Context, nodeID int32, storeID int32, rangeID int64) error
}
