...
func matchLike(ctx *EvalContext, left, right Datum, caseInsensitive bool) (Datum, error) {
	...
	like, err := optimizedLikeFunc(pattern, caseInsensitive, rune('\\'))
	if err != nil {
		return DBoolFalse, pgerror.NewErrorf(
			pgerror.CodeInvalidRegularExpressionError, "LIKE regexp compilation failed: %v", err)
	}
...
func (k similarToKey) Pattern() (string, error) {
	var pattern string
	if k.escape == 0 {
		// k.escape is empty which disables the escape mechanism.
		pattern = similarEscapeCustomChar(k.s, k.escape, false)
	} else {
		pattern = similarEscapeCustomChar(k.s, k.escape, true)
	}
