...
func unescapePattern(pattern, escapeToken string) (string, error) {
	if pattern == escapeToken {
		return "", errors.Errorf(`pattern ends with escape character`)
	}

	// There are no escape tokens to remove.
	if len(pattern) <= len(escapeToken) {
		return pattern, nil
	}
	...
	for i := 0; i < nEscapes; i++ {
		nextIdx := start + strings.Index(pattern[start:], escapeToken)
		if nextIdx == len(pattern)-len(escapeToken) {
			return "", errors.Errorf(`pattern ends with escape character`)
		}

		retWidth += copy(ret[retWidth:], pattern[start:nextIdx])
		...
		if nextIdx < len(pattern)-len(escapedEscapeToken) && pattern[nextIdx:nextIdx+len(escapedEscapeToken)] == escapedEscapeToken {
			...
			start = nextIdx + len(escapedEscapeToken)
			continue
		}

		// Skip over the escape character we removed.
		start = nextIdx + len(escapeToken)
	}

	retWidth += copy(ret[retWidth:], pattern[start:])
	return string(ret[0:retWidth]), nil
}

// replaceUnescaped replaces all instances of old that are not escaped (read:
// preceded) with the specified unescape token with new.
// For example, with an escape token of `\\`
//    replaceUnescaped("TE\\__ST", "_", ".", `\\`) --> "TE\\_.ST"
//    replaceUnescaped("TE\\%%ST", "%", ".*", `\\`) --> "TE\\%.*ST"
// If the preceding escape token is escaped, then old will be replaced.
// For example
//    replaceUnescaped("TE\\\\_ST", "_", ".", `\\`) --> "TE\\\\.ST"
func replaceUnescaped(s, old, new string, escapeToken string) string {
	if len(s) == 0 {
		return s
	}

	// We count the number of occurrences of 'old'.
	// This however can be an overestimate since the old token could be
	// escaped.  e.g. `\\_`.
	nOld := strings.Count(s, old)
	if nOld == 0 {
		return s
	}

	// Allocate buffer for final string.
	// This can be an overestimate since some of the old tokens may
	// be escaped.
	...
	ret := make([]byte, len(s)+nOld*(len(new)-len(old)))
	retWidth := 0
	start := 0
OldLoop:
	for i := 0; i < nOld; i++ {
		nextIdx := start + strings.Index(s[start:], old)

		escaped := false
		for {
			// We need to look behind to check if the escape token
			// is really an escape token.
			// E.g. if our specified escape token is `\\` and old
			// is `_`, then
			//    `\\_` --> escaped
			//    `\\\\_` --> not escaped
			//    `\\\\\\_` --> escaped
			...
			retWidth += copy(ret[retWidth:], s[start:nextIdx+len(old)])
			// TODO(richardwu): if we require 'old' to be
			// under 1 byte (ASCII), then we can simply do + 1
			// instead of a runtime computation of len(old).
			start = nextIdx + len(old)

			// Continue with next old token.
			continue OldLoop
		}

		// Token was not escaped so we replace it with new.
		// Two copies is more efficient than concatenating the slices.
		retWidth += copy(ret[retWidth:], s[start:nextIdx])
		retWidth += copy(ret[retWidth:], new)
		// TODO(richardwu): if we require 'old' to be under 1 byte
		// (ASCII), then we can simply do + 1 instead of a runtime
		// computation of len(old).
		start = nextIdx + len(old)
	}
