...
// something that affects DistSQL execution is added, it needs to be marshalled
...
func (ctx *EvalContext) GetLocation() *time.Location {
	if ctx.SessData.Location == nil {
		return time.UTC
	}
	return ctx.SessData.Location
}
	...
	case *coltypes.TOid:
		...
		case *DString:
			...
			case coltypes.RegProc, coltypes.RegProcedure:
				...
				funcDef, err := name.ResolveFunction(ctx.SessData.SearchPath)
				if err != nil {
					return nil, err
				}
