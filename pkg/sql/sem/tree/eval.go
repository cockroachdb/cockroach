...
func (expr *FuncExpr) Eval(ctx *EvalContext) (Datum, error) {
	...
	if err != nil {
		...
		newErr := sqlerror.Wrapf(err, pgerror.CodeDataExceptionError, "%s()", log.Safe(fName))
		// Add some built-in function telemetry.
		newErr = sqlerror.WithTelemetryKey(fName+"()", newErr)
		return nil, newErr
	}
...
func (expr *IfErrExpr) Eval(ctx *EvalContext) (Datum, error) {
	...
	if expr.ErrCode != nil {
		...
		if code := sqlerror.GetCode(evalErr, pgerror.CodeUncategorizedError); code != errpatStr {
			return nil, evalErr
		}
	}
	if expr.Else == nil {
