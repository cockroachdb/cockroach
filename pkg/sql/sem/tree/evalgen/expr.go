...
// and the ResolvedType method as we known TypedExprs to both have. It returns
// a sorted list.
...
func isNormalizeMethod(fd *ast.FuncDecl) bool {
	return fd.Name.Name == "normalize" &&
		fd.Recv != nil &&
		len(fd.Type.Params.List) == 1 &&
		isStarIdentWithName(fd.Type.Params.List[0].Type, "NormalizeVisitor") &&
		len(fd.Type.Results.List) == 1 &&
		isIdentWithName(fd.Type.Results.List[0].Type, "TypedExpr")
...
func isStarIdentWithName(expr ast.Expr, name string) bool {
	st, ok := expr.(*ast.StarExpr)
	if !ok {
		return false
	}
	return isIdentWithName(st.X, name)
}
