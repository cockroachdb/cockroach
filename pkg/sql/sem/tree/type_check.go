...
func (expr *Placeholder) TypeCheck(ctx *SemaContext, desired types.T) (TypedExpr, error) {
	...
	// checking.
	if typ, ok := ctx.Placeholders.Type(expr.ID, true); ok {
		...
		// be overridden again.
		if err := ctx.Placeholders.SetType(expr.ID, typ); err != nil {
			return nil, err
		}
		expr.typ = typ
		return expr, nil
	...
	if err := ctx.Placeholders.SetType(expr.ID, desired); err != nil {
		return nil, err
	}
	expr.typ = desired
	return expr, nil
...
type placeholderAnnotationVisitor struct {
	placeholders map[types.PlaceholderID]annotationState
}
...
func (v *placeholderAnnotationVisitor) VisitPre(expr Expr) (recurse bool, newExpr Expr) {
	...
	case *CastExpr:
		if arg, ok := t.Expr.(*Placeholder); ok {
			castType := t.castType()
			if state, ok := v.placeholders[arg.ID]; ok {
	...
	case *Placeholder:
		if state, ok := v.placeholders[t.ID]; !(ok && state.sawAssertion) {
			// Ignore non-annotated placeholders once an assertion has been seen.
			state.shouldAnnotate = false
			v.placeholders[t.ID] = state
		}
		return false, expr
	}
	return true, expr
...
func (p PlaceholderTypes) ProcessPlaceholderAnnotations(stmt Statement) error {
	v := placeholderAnnotationVisitor{make(map[types.PlaceholderID]annotationState)}
