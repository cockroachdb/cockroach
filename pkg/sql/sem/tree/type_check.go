...
func (expr *FuncExpr) TypeCheck(
	...
	typeCheckedOverloads := make([]*PrefixedOverload, len(fns))
	for i := range fns {
		typeCheckedOverloads[i] = fns[i].(*PrefixedOverload)
	}
...
// getMostSignificantOverloads returns overloads from the most significant
...
func getMostSignificantOverloads(
	overloads []*PrefixedOverload, searchPath SearchPath,
) ([]*PrefixedOverload, error) {
	...
	ret := make([]*PrefixedOverload, 0, len(overloads))
