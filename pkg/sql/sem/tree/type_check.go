...
func typeCheckSplitExprs(
	...
	for i, expr := range exprs {
		strippedExpr := StripParens(expr)
		if isConstant(strippedExpr) {
			constIdxs.Add(i)
		} else {
			if _, isPlaceholder := strippedExpr.(*Placeholder); isPlaceholder {
				placeholderIdxs.Add(i)
			}
			if !semaCtx.isUnresolvedPlaceholder(strippedExpr) {
