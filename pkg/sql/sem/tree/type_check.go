...
func typeCheckTupleComparison(
	...
	for elemIdx := range left.Exprs {
		...
		if err != nil {
			...
			return nil, nil, pgerror.Wrapf(err, pgcode.DatatypeMismatch, "tuples %s are not comparable at index %d: %s",
				&exps, elemIdx+1, err)
		}
...
func typeCheckSameTypedTupleExprs(
	...
	for elemIdx := range first.Exprs {
		...
		if err != nil {
			return nil, nil, pgerror.Wrapf(err, pgcode.DatatypeMismatch, "tuples %s are not the same type: %v", Exprs(exprs))
		}
