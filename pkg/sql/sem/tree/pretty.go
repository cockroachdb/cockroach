...
func (node *ColumnTableDef) docRow(p *PrettyCfg) pretty.TableRow {
	...
	if node.GeneratedIdentity.IsGenerated {
		generatedConstraint := pretty.Nil
		switch node.GeneratedIdentity.GeneratedType {
		...
		clauses = append(clauses, p.maybePrependConstraintName(&node.GeneratedIdentity.ConstraintName, generatedConstraint))
	}
