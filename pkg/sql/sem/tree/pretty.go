...
func Pretty(stmt NodeFormatter) string {
	return PrettyWithOpts(stmt, DefaultPrettyWidth, true, 4, FmtShowPasswords)
}

// PrettyWithOpts pretty prints stmt with specified options.
func PrettyWithOpts(
	stmt NodeFormatter, lineWidth int, useTabs bool, tabWidth int, f FmtFlags,
) string {
	var tab string
	if useTabs {
		tab = "\t"
...
func (p prettyCfg) docAsString(f NodeFormatter) pretty.Doc {
	return pretty.Text(AsStringWithFlags(f, p.flags))
}
...
In general, a node implements doc by copying its Format method and
returning a Doc structure instead of writing to a buffer.
*/
