...
//
// Nodes should not preceed themselves with a space. Instead, the parent
// structure should correctly add spaces when needed.
//
// nestName should be used for most `KEYWORD <expr>` constructs.
//
// Nodes that are already correctly formatted by their .Format method should
// not implement doc but instead rely on the default fallback.
//
// AS should not use nestName. Instead it is always followed by a space and
// concatenated directly with the alias.
...
func PrettyWithOpts(
	stmt NodeFormatter, lineWidth int, useTabs bool, tabWidth int,
) string {
	var tab string
	if useTabs {
		tab = "\t"
