...
// OverridingMode represents the OVERRIDING clause of an INSERT, which controls
// whether explicitly-supplied values are permitted for identity columns. It
// mirrors PostgreSQL's OverridingKind.
type OverridingMode uint8

const (
	// OverridingModeNone means no OVERRIDING clause is in effect. Explicit
	// values for GENERATED ALWAYS AS IDENTITY columns are rejected. This is the
	// zero value, which is the meaningful default.
	OverridingModeNone OverridingMode = iota
	// OverridingModeSystemValue corresponds to OVERRIDING SYSTEM VALUE: explicit
	// values for GENERATED ALWAYS AS IDENTITY columns are accepted and used.
	// COPY FROM uses this mode unconditionally, matching PostgreSQL.
	OverridingModeSystemValue
	// OverridingModeUserValue corresponds to OVERRIDING USER VALUE: explicit
	// values for GENERATED BY DEFAULT AS IDENTITY columns are ignored in favor
	// of the sequence default. Reserved for the OVERRIDING syntax tracked in
	// https://github.com/cockroachdb/cockroach/issues/68201; not yet honored.
	OverridingModeUserValue
)
...
type Insert struct {
	...
	// Overriding is the OVERRIDING mode that governs writes to identity columns.
	// There is currently no parser syntax that sets it; it is only set
	// programmatically (COPY FROM sets OverridingModeSystemValue).
...
func (node *Insert) Format(ctx *FmtCtx) {
	...
	switch node.Overriding {
	case OverridingModeSystemValue:
		ctx.WriteString(" OVERRIDING SYSTEM VALUE")
	case OverridingModeUserValue:
