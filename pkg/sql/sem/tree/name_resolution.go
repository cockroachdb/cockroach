...
func (c *ColumnItem) Resolve(ctx context.Context, r ColumnItemResolver) (interface{}, error) {
	...
	prefix := TableName{tblName{
		TableName: Name(c.TableName.Parts[0]),
		TableNamePrefix: TableNamePrefix{
			SchemaName:      Name(c.TableName.Parts[1]),
			CatalogName:     Name(c.TableName.Parts[2]),
			ExplicitSchema:  c.TableName.NumParts >= 2,
			ExplicitCatalog: c.TableName.NumParts >= 3,
		},
	}}
...
// Resolve is used for table prefixes when the search path is not to be used. This
// is adequate for table patterns with stars, e.g. AllTablesSelector.
