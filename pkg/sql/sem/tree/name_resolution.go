...
type ColumnItemResolver interface {
	...
	// modified.
	FindSourceMatchingName(ctx context.Context, tn TableName) (res NumResolutionResults, prefix *TableName, srcMeta interface{}, err error)
	...
	// modified.
	FindSourceProvidingColumn(ctx context.Context, col Name) (prefix *TableName, srcMeta interface{}, colHint int, err error)

	// Resolve() is called if resolution succeeds.
	Resolve(ctx context.Context, prefix *TableName, srcMeta interface{}, colHint int, col Name) (interface{}, error)
}

// Resolve performs name resolution for a column item using a resolver.
func (c *ColumnItem) Resolve(ctx context.Context, r ColumnItemResolver) (interface{}, error) {
...
type TableNameTargetResolver interface {
	LookupSchema(ctx context.Context, dbName, scName string) (found bool, scMeta interface{}, err error)
}
...
type TableNameExistingResolver interface {
	LookupObject(ctx context.Context, dbName, scName, obName string) (found bool, objMeta interface{}, err error)
}
...
func (t *TableName) ResolveTarget(
	...
	iter := searchPath.Iter()
	for next, ok := iter(); ok; next, ok = iter() {
		if found, scMeta, err := r.LookupSchema(ctx, curDb, next); found || err != nil {
			if err == nil {
				t.CatalogName = Name(curDb)
				t.SchemaName = Name(next)
			}
...
// ResolveTargetWhenDBMayNotExist performs name resolution for a table name when
// the target object and neither the catalog prefix is not expected to exist already.
//
// This is an exotic variant of ResolveTarget() use specifically by
// the IMPORT/RESTORE statements (which are able to both create the
// target DB and the target table).
func (t *TableName) ResolveTargetWhenDBMayNotExist(
	ctx context.Context, r TableNameTargetResolver, curDb string, searchPath sessiondata.SearchPath,
) (bool, interface{}, error) {
	if t.ExplicitSchema {
		if t.ExplicitCatalog {
		...
		// No luck so far. Use D.public.T instead in all cases.
		found, scMeta, err := r.LookupSchema(ctx, t.Schema(), PublicSchema)
		if err == nil {
			t.CatalogName = t.SchemaName
			t.SchemaName = PublicSchemaName
			t.ExplicitCatalog = true
	...
	// No luck so far. Assume curDB.public.T.
	found, scMeta, err := r.LookupSchema(ctx, curDb, PublicSchema)
	if err == nil {
		t.CatalogName = Name(curDb)
		t.SchemaName = PublicSchemaName
	}
...
func (tp *TableNamePrefix) Resolve(
	...
	// No schema name, find a valid schema in searchPath.
	found, dbName, scName, scMeta, err := FindFirstValidSchema(ctx, r, curDb, searchPath)
	if found {
		tp.CatalogName = Name(dbName)
		tp.SchemaName = Name(scName)
	}
	return found, scMeta, err
}

// ResolveWhenDBMayNotExist is used for table prefixes when the database prefix may not exist.
//
// This is an exotic variant of Resolve() use specifically by
// the IMPORT/RESTORE statements (which are able to both create the
// target DB and the target table).
func (tp *TableNamePrefix) ResolveWhenDBMayNotExist(
	ctx context.Context, r TableNameTargetResolver, curDb string, searchPath sessiondata.SearchPath,
) (found bool, scMeta interface{}, err error) {
	if tp.ExplicitSchema {
		if tp.ExplicitCatalog {
		...
		// No luck so far. Use D.public.T instead in all cases.
		found, scMeta, err := r.LookupSchema(ctx, tp.Schema(), PublicSchema)
		if err != nil {
			tp.CatalogName = tp.SchemaName
			tp.SchemaName = PublicSchemaName
			tp.ExplicitCatalog = true
	...
	// No schema name, find a valid schema in searchPath.
	found, _, _, scMeta, err = FindFirstValidSchema(ctx, r, curDb, searchPath)
	if !found && err == nil {
		tp.CatalogName = Name(curDb)
		tp.SchemaName = PublicSchemaName
	}
...
// FindFirstValidSchema returns the first schema listed in searchPath that
// exists according to the resolver.
func FindFirstValidSchema(
	ctx context.Context, r TableNameTargetResolver, curDb string, searchPath sessiondata.SearchPath,
) (found bool, dbName, scName string, scMeta interface{}, err error) {
	iter := searchPath.Iter()
	for next, ok := iter(); ok; next, ok = iter() {
		if found, scMeta, err := r.LookupSchema(ctx, curDb, next); found || err != nil {
			return found, curDb, next, scMeta, err
		}
	}
	return false, "", "", nil, nil
}
...
func NewInvalidNameErrorf(fmt string, n *UnresolvedName) error {
	return pgerror.NewErrorWithDepthf(1, pgerror.CodeInvalidNameError, fmt, n)
}
