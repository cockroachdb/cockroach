...
func (wfr *WindowFrameRun) FrameEndIdx(evalCtx *EvalContext) int {
	...
	case GROUPS:
		...
		case OffsetPreceding:
			...
			return wfr.PeerHelper.GetFirstPeerIdx(peerGroupNum) + wfr.PeerHelper.GetPeerRowCount(peerGroupNum)
		case CurrentRow:
			return wfr.DefaultFrameSize()
		case OffsetFollowing:
			offset := MustBeDInt(wfr.EndBoundOffset)
...
func (wfr *WindowFrameRun) DefaultFrameSize() int {
	return wfr.PeerHelper.GetFirstPeerIdx(wfr.CurRowPeerGroupNum) + wfr.PeerHelper.GetPeerRowCount(wfr.CurRowPeerGroupNum)
}
