...
// WindowFrameBoundary specifies the offset and the type of boundary.
...
// WindowFrameBoundaries specifies boundaries of the window frame.
...
func (boundary *WindowFrameBound) write(ctx *FmtCtx) {
	...
	case ValueFollowing:
		ctx.WriteString(fmt.Sprintf("%v FOLLOWING", boundary.Offset))
...
func (node *WindowFrame) Format(ctx *FmtCtx) {
	...
	if node.Bounds.EndBound != nil {
		...
		node.Bounds.StartBound.write(ctx)
		ctx.WriteString(" AND ")
		node.Bounds.EndBound.write(ctx)
	} else {
		node.Bounds.StartBound.write(ctx)
	}
}
