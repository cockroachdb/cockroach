...
// doe snot contain any interesting information.
...
func (v *exprContainsOnlyConstantsAndPlaceholders) VisitPre(
	...
	case Datum, Constant, *Placeholder:
		return false, expr
	case *Tuple, *Array, *CastExpr, *BinaryExpr, *UnaryExpr:
		return true, expr
	default:
...
// Examples:
// VALUES (a,b,c), (d,e,f) -> VALUES (_, _, _), (__more__)
// VALUES ((a, b), (c, d)), ((e, f), (g, h)) -> VALUES ((_, _), (__more__)), (__more__)
...
// canCollapseExprs returns true if the expression contains only literals and/or
// placeholders and has more than one element. This is used to determine if the
// expression can be collapsed into a special shortened representation when
// generating a representative query for statement fingerprints.
...
// literals, placeholders and/or simple expressions containing only literals
// and/or placeholders as a tuple of its first element, scrubbed.
// Examples:
//
//	 (1)               -> (_)
//		(1, 2)            -> (_, __more__)
...
// expression of its first element, scrubbed.
// e.g. array[1]             -> array[_]
//
//	array[1, 2]          -> array[_, __more__]
//	array[1, 2, 3]       -> array[_, _, __more__]
//	array[1+2, 2+3, 3+4] -> array[_ + _, __more__]
