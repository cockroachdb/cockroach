...
var castMap = map[oid.Oid]map[oid.Oid]cast{
	...
	oid.T_bpchar: {
		...
		oid.T_unknown:      {maxContext: CastContextExplicit, origin: contextOriginAutomaticIOConversion},
	...
	oid.T_unknown: {
		...
		oid.T_unknown:      {maxContext: CastContextImplicit, origin: contextOriginSameType},
		...
		for tgt := range tgts {
			...
			if src == oid.T_unknown {
				...
				if tgt != oid.T_unknown && ent.origin != contextOriginNullConversion {
					panic(fmt.Sprintf(
						"cast from T_unknown to T_unknown must have an origin of contextOriginNullConversion",
					))
				}
			}
