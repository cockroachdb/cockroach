...
// Those types were manually deleted. Entries with an contextOriginAutomaticIOConversion
// origin were manually added.
var castMap = map[oid.Oid]map[oid.Oid]cast{
	...
	oid.T_unknown: {
		// Unknown is the type of an expression that statically evalutates to
...
func init() {
	isStringType := func(o oid.Oid) bool {
		switch o {
		case oid.T_bpchar, oid.T_char, oid.T_name, oid.T_varchar, oid.T_text:
			return true
		}
		return false
	}
	...
	for src, tgts := range castMap {
		for tgt := range tgts {
			...
			if src == oid.T_unknown {
				...
				if ent.origin != contextOriginNullConversion {
					panic(fmt.Sprintf(
						"cast from %s to %s must have an origin of contextOriginNullConversion",
						srcStr, tgtStr,
					))
				}
			}
