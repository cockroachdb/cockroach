...
func LookupCastVolatility(from, to *types.T, evalCtx *EvalContext) (_ Volatility, ok bool) {
	fromFamily := from.Family()
	toFamily := to.Family()
	// Special case for casting between arrays.
	if fromFamily == types.ArrayFamily && toFamily == types.ArrayFamily {
		return LookupCastVolatility(from.ArrayContents(), to.ArrayContents(), evalCtx)
	}
	...
	if fromFamily == types.TupleFamily && toFamily == types.TupleFamily {
		...
		for i := range fromTypes {
			v, ok := LookupCastVolatility(fromTypes[i], toTypes[i], evalCtx)
			if !ok {
				return 0, false
			}
			if v > maxVolatility {
	...
	case types.StringFamily, types.CollatedStringFamily:
		...
		case types.IntervalFamily:
			if evalCtx.SessionData.IntervalStyleEnabled {
				return VolatilityStable, true
			}
		}
	case types.IntervalFamily:
