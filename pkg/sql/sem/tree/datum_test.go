...
func TestDTimeTZ(t *testing.T) {
	...
	maxTime, err := tree.ParseDTimeTZ(ctx, "24:00:00-1559")
	require.NoError(t, err)
	minTime, err := tree.ParseDTimeTZ(ctx, "00:00:00+1559")
	require.NoError(t, err)

	// These are all the same UTC time equivalents.
	utcTime, err := tree.ParseDTimeTZ(ctx, "11:14:15+0")
	require.NoError(t, err)
	sydneyTime, err := tree.ParseDTimeTZ(ctx, "21:14:15+10")
	require.NoError(t, err)
