...
type Datum interface {
	...
	// (TotalOrderCompare/encoding order, where NULL <= x is true for all x) is true. This
	// is okay for now: the returned datum is used only to construct a span, which
	// uses TotalOrderCompare/encoding order and is guaranteed to be large enough by this
	...
	// (TotalOrderCompare/encoding order, where x >= NULL is true for all x) is true. This
	// is okay for now: the returned datum is used only to construct a span, which
	// uses TotalOrderCompare/encoding order and is guaranteed to be large enough by this
	// weaker contract. The original filter expression is left in place to catch
	// false positives.
...
func (d *DTuple) sort(ctx *EvalContext) {
	if !d.sorted {
		sort.Slice(d.D, func(i, j int) bool {
			return TotalOrderLess(ctx, d.D[i], d.D[j])
		})
		d.SetSorted()
	}
}
...
func (d *DTuple) makeUnique(ctx *EvalContext) {
	n := 0
	for i := 0; i < len(d.D); i++ {
		if n == 0 || TotalOrderLess(ctx, d.D[n-1], d.D[i]) {
			d.D[n] = d.D[i]
			n++
		}
	}
