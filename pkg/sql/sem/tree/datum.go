...
func (d *DOidWrapper) IsComposite() bool {
	if cdatum, ok := d.Wrapped.(CompositeDatum); ok && cdatum.IsComposite() {
		return true
	}
	return false
}
...
// NewDCitextFromDCollatedString is a helper routine to create a *DCitext (implemented as
// a *DOidWrapper) initialized from an existing *DCollatedString.
func NewDCitextFromDCollatedString(d *DCollatedString) (Datum, error) {
	if d.Locale != collatedstring.CaseInsensitiveLocale {
		return nil, pgerror.Newf(
			pgcode.InvalidParameterValue,
			"cannot create citext from collated string with locale %s; expected %s",
			d.Locale, collatedstring.CaseInsensitiveLocale,
		)
	}
...
func NewDCitext(contents string, env *CollationEnvironment) (Datum, error) {
	...
	return NewDCitextFromDCollatedString(d)
}
