...
// CompareError implements the Datum interface.
func (d *DBitArray) CompareError(du CompareContext, other Datum) (int, error) {
	if other == DNull {
		// NULL is less than any non-NULL value.
		return 1, nil
	}
	v, ok := du.UnwrapDatum(other).(*DBitArray)
	if !ok {
		return 0, makeUnsupportedComparisonMessage(d, other)
	}
...
// CompareError implements the Datum interface.
func (d *DInt) CompareError(du CompareContext, other Datum) (int, error) {
	if other == DNull {
		// NULL is less than any non-NULL value.
		return 1, nil
	}
	thisInt := *d
	var v DInt
	switch t := du.UnwrapDatum(other).(type) {
	case *DInt:
		v = *t
	case *DFloat, *DDecimal:
		res, err := t.CompareError(du, d)
		if err != nil {
			return 0, err
		}
		return -res, nil
...
// CompareError implements the Datum interface.
func (d *DFloat) CompareError(du CompareContext, other Datum) (int, error) {
	if other == DNull {
		// NULL is less than any non-NULL value.
		return 1, nil
	}
...
// Compare implements the Datum interface.
func (d *DDecimal) Compare(du CompareContext, other Datum) int {
	res, err := d.CompareError(du, other)
	if err != nil {
		panic(err)
	}
	return res
...
// CompareError implements the Datum interface.
func (d *DDecimal) CompareError(du CompareContext, other Datum) (int, error) {
	if other == DNull {
		// NULL is less than any non-NULL value.
		return 1, nil
	}
...
// CompareError implements the Datum interface.
func (d *DString) CompareError(du CompareContext, other Datum) (int, error) {
	if other == DNull {
		// NULL is less than any non-NULL value.
		return 1, nil
	}
	v, ok := du.UnwrapDatum(other).(*DString)
	if !ok {
		return 0, makeUnsupportedComparisonMessage(d, other)
	}
	if *d < *v {
...
// CompareError implements the Datum interface.
func (d *DCollatedString) CompareError(du CompareContext, other Datum) (int, error) {
	if other == DNull {
		// NULL is less than any non-NULL value.
		return 1, nil
	}
	v, ok := du.UnwrapDatum(other).(*DCollatedString)
...
// CompareError implements the Datum interface.
func (d *DBytes) CompareError(du CompareContext, other Datum) (int, error) {
	if other == DNull {
		// NULL is less than any non-NULL value.
		return 1, nil
	}
	v, ok := du.UnwrapDatum(other).(*DBytes)
	if !ok {
		return 0, makeUnsupportedComparisonMessage(d, other)
	}
	if *d < *v {
...
// Compare implements the Datum interface.
func (d *DUuid) Compare(du CompareContext, other Datum) int {
	res, err := d.CompareError(du, other)
	if err != nil {
		panic(err)
	}
...
// CompareError implements the Datum interface.
func (d *DUuid) CompareError(du CompareContext, other Datum) (int, error) {
	if other == DNull {
		// NULL is less than any non-NULL value.
		return 1, nil
	}
	v, ok := du.UnwrapDatum(other).(*DUuid)
	if !ok {
		return 0, makeUnsupportedComparisonMessage(d, other)
	}
...
// CompareError implements the Datum interface.
func (d *DIPAddr) CompareError(du CompareContext, other Datum) (int, error) {
	if other == DNull {
		// NULL is less than any non-NULL value.
		return 1, nil
	}
	v, ok := du.UnwrapDatum(other).(*DIPAddr)
	if !ok {
		return 0, makeUnsupportedComparisonMessage(d, other)
	}
