...
// d. Must be kept in sync with DArray.ResolvedType. Panics if t is not an array
// type.
func (d *DArray) MaybeSetCustomOid(t *types.T) {
	if t.Family() != types.ArrayFamily {
		panic(errors.AssertionFailedf("expected array type, got %s", t.SQLString()))
	}
	switch t.Oid() {
