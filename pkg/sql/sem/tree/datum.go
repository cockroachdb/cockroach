...
func (d *DUuid) Format(ctx *FmtCtx) {
	...
	d.UUID.StringBytes(buf[:uuid.RFC4122StrSize])
	ctx.Write(buf[:uuid.RFC4122StrSize])
...
func formatTime(t time.Time, layout string) string {
	...
	ctx.Grow(64)
	t.AppendFormat(ctx.Bytes(), layout)
...
func (d *DOid) Format(ctx *FmtCtx) {
	appendOID := func() {
		ctx.Grow(8)
		ctx.Write(strconv.AppendUint(ctx.Bytes()[ctx.Len():], uint64(d.Oid), 10))
	}
