...
func TestNormalizeExpr(t *testing.T) {
	...
	testData := []struct {
		...
		{`1 IS NULL`, `false`},
		{`1 IS NOT NULL`, `true`},
		{`d IS NULL`, `d IS NOT DISTINCT FROM NULL`},
		{`d IS NOT NULL`, `d IS DISTINCT FROM NULL`},
		{`NULL IS TRUE`, `false`},
		{`NULL IS NOT TRUE`, `true`},
		{`false IS TRUE`, `false`},
		{`false IS NOT TRUE`, `true`},
		{`d IS TRUE`, `(d = true) AND (d IS DISTINCT FROM NULL)`},
		{`d IS NOT TRUE`, `(d != true) OR (d IS NOT DISTINCT FROM NULL)`},
		{`NULL IS FALSE`, `false`},
		{`NULL IS NOT FALSE`, `true`},
		{`false IS FALSE`, `true`},
		{`false IS NOT FALSE`, `false`},
		{`d IS FALSE`, `(d = false) AND (d IS DISTINCT FROM NULL)`},
		{`d IS NOT FALSE`, `(d != false) OR (d IS NOT DISTINCT FROM NULL)`},
		...
		{`d IS NOT DISTINCT FROM TRUE`, `(d = true) AND (d IS DISTINCT FROM NULL)`},
		{`d IS DISTINCT FROM TRUE`, `(d != true) OR (d IS NOT DISTINCT FROM NULL)`},
		...
		{`d IS NOT DISTINCT FROM FALSE`, `(d = false) AND (d IS DISTINCT FROM NULL)`},
		{`d IS DISTINCT FROM FALSE`, `(d != false) OR (d IS NOT DISTINCT FROM NULL)`},
