...
// virtual table descriptor ID when remap is true and schemaName is
// "pg_catalog". Otherwise it returns the descriptor ID as an OID unchanged.
func RemapPgCatalogOid(schemaName string, descID uint32, remap bool) oid.Oid {
	if remap && schemaName == PgCatalogName {
