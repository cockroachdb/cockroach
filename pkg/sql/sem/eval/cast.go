...
// ValidAssignmentCast returns true if a valid cast exists from src to tgt as
// an assignment cast.
func ValidAssignmentCast(src, tgt *types.T) bool {
	return cast.ValidCast(src, tgt, cast.ContextAssignment)
}

// ValidExplicitCast returns true if a valid cast exists from src to tgt as
// an explicit cast.
func ValidExplicitCast(src, tgt *types.T) bool {
	return cast.ValidCast(src, tgt, cast.ContextExplicit)
}
