...
func performGeographyOperationUsingBestGeomProjection(
	...
	geom, err := g.AsGeometry()
	if err != nil {
		return nil, err
	}

	transformedGeom, err := geotransform.Transform(
		geom,
	...
	appliedGeom, err := f(transformedGeom)
	if err != nil {
		return nil, err
	}

	retransformedGeom, err := geotransform.Transform(
		appliedGeom,
		proj,
	...
	"st_intersection": makeBuiltin(
		...
		geographyOverload2(
			func(ctx *tree.EvalContext, a *tree.DGeography, b *tree.DGeography) (tree.Datum, error) {
				...
				aGeom, err := a.Geography.AsGeometry()
				if err != nil {
					return nil, err
				}
				bGeom, err := b.Geography.AsGeometry()
				if err != nil {
					return nil, err
				}

				aTransformed, err := geotransform.Transform(
					aGeom,
				...
				bTransformed, err := geotransform.Transform(
					bGeom,
				...
				intersection, err := geomfn.Intersection(aTransformed, bTransformed)
				if err != nil {
					return nil, err
				}
				...
				ret, err := retransformedGeom.AsGeography()
				if err != nil {
					return nil, err
				}
				return tree.NewDGeography(ret), nil
