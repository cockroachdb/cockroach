	...
	"st_bdpolyfromtext": makeBuiltin(
		...
		tree.Overload{
			...
			Fn: func(_ context.Context, _ *eval.Context, args tree.Datums) (tree.Datum, error) {
				...
				polygon, err := geomfn.MakePolygonFromMLString(g, srid)
				if err != nil {
					return nil, err
				}
			...
			Info: infoBuilder{
				info: "Returns a Polygon from multilinestring WKT with a SRID. If the input is not a multistring an error will be thrown.",
			}.String(),
			Volatility: volatility.Immutable,
		},
	),
