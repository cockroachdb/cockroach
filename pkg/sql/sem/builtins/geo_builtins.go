...
Note ST_Perimeter is only valid for Polygon - use ST_Length for LineString.`,
	...
	"st_transform": makeBuiltin(
		...
		tree.Overload{
			...
			Fn: func(_ *tree.EvalContext, args tree.Datums) (tree.Datum, error) {
				g := args[0].(*tree.DGeometry)
				toProj := *args[1].(*tree.DString)

				fromProj, exists := geoprojbase.Projection(g.SRID())
				if !exists {
					return nil, errors.Newf("projection for srid %d does not exist", g.SRID())
				}
				ret, err := geotransform.Transform(g.Geometry, fromProj.Proj4Text, geoprojbase.Proj4Text(toProj), geopb.SRID(0))
				if err != nil {
					return nil, err
				}
				return tree.NewDGeometry(ret), nil
		...
		tree.Overload{
			...
			Fn: func(_ *tree.EvalContext, args tree.Datums) (tree.Datum, error) {
				g := args[0].(*tree.DGeometry)
				fromProj := *args[1].(*tree.DString)
				toProj := *args[2].(*tree.DString)

				ret, err := geotransform.Transform(g.Geometry, geoprojbase.Proj4Text(fromProj), geoprojbase.Proj4Text(toProj), geopb.SRID(0))
				if err != nil {
					return nil, err
				}
				return tree.NewDGeometry(ret), nil
		...
		tree.Overload{
			...
			Fn: func(_ *tree.EvalContext, args tree.Datums) (tree.Datum, error) {
				...
				ret, err := geotransform.Transform(g.Geometry, geoprojbase.Proj4Text(fromProj), toProj.Proj4Text, srid)
				if err != nil {
					return nil, err
				}
				return tree.NewDGeometry(ret), nil
