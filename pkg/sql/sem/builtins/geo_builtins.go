...
var geoBuiltins = map[string]builtinDefinition{
			...
			func(ctx *tree.EvalContext, g *tree.DGeometry) (tree.Datum, error) {
				...
				case *geom.LineString:
					...
					retG, err := geo.NewGeometryFromGeom(
						geom.NewLineStringFlat(geom.XY, []float64{coord.X(), coord.Y()}).SetSRID(t.SRID()),
					)
					if err != nil {
						return nil, err
					}
	...
	"st_interiorringn": makeBuiltin(
		...
		tree.Overload{
			...
			Info: infoBuilder{
				info: `Returns the n-th interior ring of a Polygon as a LineString. Returns NULL if the shape is not a Polygon, or the ring does not exist.`,
			}.String(),
			Volatility: tree.VolatilityImmutable,
		},
	),
		...
		tree.Overload{
			...
			Fn: func(_ *tree.EvalContext, args tree.Datums) (tree.Datum, error) {
				...
				case *geom.MultiPoint:
					...
					g, err := geo.NewGeometryFromGeom(t.Point(n - 1).SetSRID(t.SRID()))
					if err != nil {
						return nil, err
					}
					return tree.NewDGeometry(g), nil
				case *geom.MultiLineString:
					...
					g, err := geo.NewGeometryFromGeom(t.LineString(n - 1).SetSRID(t.SRID()))
					if err != nil {
						return nil, err
					}
					return tree.NewDGeometry(g), nil
				case *geom.MultiPolygon:
					...
					g, err := geo.NewGeometryFromGeom(t.Polygon(n - 1).SetSRID(t.SRID()))
					if err != nil {
						return nil, err
					}
					return tree.NewDGeometry(g), nil
				case *geom.GeometryCollection:
					...
					g, err := geo.NewGeometryFromGeom(t.Geom(n - 1))
					if err != nil {
						return nil, err
					}
			...
			Info: infoBuilder{
				info: `Returns the n-th Geometry. Returns NULL if out of bounds.`,
			}.String(),
			Volatility: tree.VolatilityImmutable,
		},
	),
