	...
	return makeBuiltin(
		...
		tree.Overload{
			...
			Fn: func(_ *tree.EvalContext, args tree.Datums) (tree.Datum, error) {
				...
				srid := geopb.SRID(tree.MustBeDInt(args[1]))
				g, err := geo.ParseGeometryFromEWKBAndSRID(geopb.WKB(s), srid)
				if err != nil {
					return nil, err
				}
				if g.Shape() != shape {
	...
	"st_geomfromwkb": makeBuiltin(
		...
		tree.Overload{
			...
			Fn: func(_ *tree.EvalContext, args tree.Datums) (tree.Datum, error) {
				...
				srid := geopb.SRID(tree.MustBeDInt(args[1]))
				g, err := geo.ParseGeometryFromEWKBAndSRID(geopb.WKB(b), srid)
				if err != nil {
					return nil, err
				}
				return tree.NewDGeometry(g), nil
	...
	"st_geogfromwkb": makeBuiltin(
		...
		tree.Overload{
			...
			Fn: func(_ *tree.EvalContext, args tree.Datums) (tree.Datum, error) {
				b := string(tree.MustBeDBytes(args[0]))
				srid := geopb.SRID(tree.MustBeDInt(args[1]))
				g, err := geo.ParseGeographyFromEWKBAndSRID(geopb.WKB(b), srid)
				if err != nil {
					return nil, err
				}
				return tree.NewDGeography(g), nil
