	...
	"crdb_internal.encode_key": makeBuiltin(
		...
		tree.Overload{
			...
			Fn: func(ctx *tree.EvalContext, args tree.Datums) (tree.Datum, error) {
				...
				if !ok {
					return nil, errors.Newf("expected DTuple, found %s", args[2])
				}
				...
				if len(rowDatums.D) != len(tableDesc.Columns) {
					return nil, errors.New("number of values provided must equal number of columns in table")
				}
				...
				for i, d := range rowDatums.D {
					...
					if d.ResolvedType() == types.Unknown {
						if !tableDesc.Columns[i].Nullable {
							return nil, errors.New("NULL provided as a value for a non-nullable column")
						}
						newDatum = tree.DNull
					} else {
						...
						if err != nil {
							return nil, errors.Wrap(err, "try to explicitly cast each value to the corresponding column type")
						}
					}
				...
				if len(res) > 1 {
					return nil, errors.New("inverted indexes not supported right now")
				}
