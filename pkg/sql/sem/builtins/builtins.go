...
func extractStringFromTime(fromTime *tree.DTime, timeSpan string) (tree.Datum, error) {
	...
	case "second", "seconds":
		return tree.NewDFloat(tree.DFloat(float64(t.Second()) + float64(t.Microsecond())/(1000*1000))), nil
	case "millisecond", "milliseconds":
		return tree.NewDFloat(tree.DFloat(float64(t.Second()*1000) + float64(t.Microsecond())/microsPerMilli)), nil
	case "microsecond", "microseconds":
		return tree.NewDFloat(tree.DFloat((t.Second() * 1000 * 1000) + t.Microsecond())), nil
	case "epoch":
...
func extractStringFromTimestamp(
	...
	case "julian":
		julianDay := float64(dateToJulianDay(fromTime.Year(), int(fromTime.Month()), fromTime.Day())) +
			(float64(fromTime.Hour()*3600+fromTime.Minute()*60+fromTime.Second())+
				float64(fromTime.Nanosecond())/float64(time.Second))/86400
	...
	case "millisecond", "milliseconds":
		// This a PG extension not supported in MySQL.
		return tree.NewDFloat(tree.DFloat(float64(fromTime.Second()*1000) + float64(fromTime.Nanosecond())/float64(time.Millisecond))), nil

	case "microsecond", "microseconds":
		return tree.NewDFloat(tree.DFloat(float64(fromTime.Second()*1000*1000) + float64(fromTime.Nanosecond())/float64(time.Microsecond))), nil
