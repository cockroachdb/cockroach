...
| m/n  | no                               | yes                                  |`,
	...
	"current_date": {
		tree.Builtin{
			...
			Info: "Returns an approximation of the date of the current transaction." + txnTSEstimateDoc,
		},
	},
	...
	"current_time": {
		tree.Builtin{
			...
			Info: "Returns an approximate time of the current transaction, with time zone." + txnTSEstimateDoc,
		},
	},
...
// Note: the text below says "greated than txntime - 1µs" because the
// value is really = round(kvtxntime, 1µs). So if the kvtxntime is
// already exactly at a microsecond, the result value will be exactly
// the txntime. However, if the kvtxntime has some nanoseconds beyond
// that the result value will be lower than the kvtxntime. In the
// worst case (+999ns) the result will be slightly greater than
// kvtxntime - 1µs, hence the explanation.
var txnTSEstimateDoc = `

The value is an estimate: it is greater than the logical time the
...
var txnTSDoc = `Returns an approximate time of the current transaction.` + txnTSEstimateDoc
