...
func floatToTimestampTZ(ts float64) (tree.Datum, error) {
	...
	sec, err := strconv.Atoi(strts[0])
	if err != nil {
		return nil, err
	}
	nsec, err := strconv.Atoi(strts[1])
	if err != nil {
		return nil, err
	}
	nsec = int(float64(nsec) / math.Pow10(len(strts[1])) * float64(time.Second))
	return tree.MakeDTimestampTZ(timeutil.Unix(int64(sec), int64(nsec)), time.Microsecond)
}
