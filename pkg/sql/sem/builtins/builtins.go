	...
	"crdb_internal.set_compaction_concurrency": makeBuiltin(
		...
		tree.Overload{
			...
			Fn: func(ctx *eval.Context, args tree.Datums) (tree.Datum, error) {
				...
				if prevConcurrency, err = ctx.SetCompactionConcurrency(
					ctx.Context, nodeID, storeID, uint64(compactionConcurrency)); err != nil {
					return nil, err
				}

				// Block until the operation is canceled.
				<-ctx.Context.Done()

				// Restore the previous compaction concurrency upon cancellation.
				if _, err = ctx.SetCompactionConcurrency(
					ctx.Context, nodeID, storeID, prevConcurrency,
				); err != nil {
					return nil, err
				}
