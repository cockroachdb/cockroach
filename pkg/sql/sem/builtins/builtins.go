...
`,
	...
	"lca": makeBuiltin(
		...
		tree.Overload{
			...
			Volatility: volatility.Immutable,
			Fn: func(_ context.Context, _ *eval.Context, args tree.Datums) (tree.Datum, error) {
				ltrees := make([]ltree.LTree, len(args))
				for i, arg := range args {
					if arg == tree.DNull {
						return arg, nil
					}
				...
				return tree.NewDLTree(lca(ltrees)), nil
			},
		},
		tree.Overload{
			Types: tree.ParamTypes{
			...
			Fn: func(_ context.Context, _ *eval.Context, args tree.Datums) (tree.Datum, error) {
				...
				ltrees := make([]ltree.LTree, array.Len())
...
func lca(ltrees []ltree.LTree) ltree.LTree {
	if len(ltrees) == 0 {
		// This should not happen in practice, but we handle it gracefully.
		return ltree.LTree{}
	}
	var minLength = ltree.MaxNumOfLabels
	...
	for i = 0; i < minLength; i++ {
		...
		for _, t := range ltrees[1:] {
			if t.Path[i] != label {
				return ltree.LTree{Path: append([]ltree.Label(nil), ltrees[0].Path[:i]...)}
			}
		}
	}
	return ltree.LTree{Path: append([]ltree.Label(nil), ltrees[0].Path[:i]...)}
