	...
	"crdb_internal.get_namespace_id": makeBuiltin(
		...
		tree.Overload{
			...
			Fn: func(ctx *tree.EvalContext, args tree.Datums) (tree.Datum, error) {
				...
				id, found, err := ctx.PrivilegedAccessor.LookupNamespaceID(ctx.Context, int64(parentID), string(name))
				if err != nil {
					return nil, err
				}
				if !found {
	...
	"crdb_internal.get_zone_config": makeBuiltin(
		...
		tree.Overload{
			...
			Fn: func(ctx *tree.EvalContext, args tree.Datums) (tree.Datum, error) {
				...
				bytes, found, err := ctx.PrivilegedAccessor.LookupZoneConfigByNamespaceID(ctx.Context, int64(id))
				if err != nil {
					return nil, err
				}
				if !found {
