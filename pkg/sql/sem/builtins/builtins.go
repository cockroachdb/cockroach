...
may increase either contention or retry errors, or both.`,
	...
	"crdb_internal.set_verbosity_of_spans_in_trace": makeBuiltin(
		tree.FunctionProperties{Category: categorySystemInfo},
		tree.Overload{
			Types: tree.ArgTypes{
				{"trace_id", types.Int},
			...
			Fn: func(ctx *tree.EvalContext, args tree.Datums) (tree.Datum, error) {
				...
				row, err := ie.QueryRowEx(
					ctx.Ctx(),
					"crdb_internal.set_verbosity_of_spans_in_trace",
					ctx.Txn,
				...
				success := ctx.Settings.Tracer.SetAllDescendantsSpanVerbosity(rootSpan, verbosity)
				if !success {
					return tree.DBoolFalse, nil
				}
