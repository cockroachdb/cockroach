...
func hashData(h hash.Hash64, data []byte) error {
...
func fingerprintRangekeys(ctx context.Context, ssts [][]byte) (uint64, error) {
	...
	fingerprintRangeKey := func(bounds roachpb.Span, versions storage.MVCCRangeKeyVersions) (uint64, error) {
		defer h.Reset()
		if err := hashData(h, bounds.Key); err != nil {
			return 0, err
		}
		if err := hashData(h, bounds.EndKey); err != nil {
			return 0, err
		}
		for _, v := range versions {
			timestampBuf = storage.EncodeMVCCTimestampToBuf(timestampBuf, v.Timestamp)
			if err := hashData(h, timestampBuf); err != nil {
				return 0, err
			}
			if err := hashData(h, v.Value); err != nil {
				return 0, err
			}
		}
		return h.Sum64(), nil
