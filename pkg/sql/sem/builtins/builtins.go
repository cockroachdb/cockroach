...
| m/n  | no                               | yes                                  |`,
	...
	"current_database": {
		tree.Builtin{
			...
			Category:   categorySystemInfo,
			Fn: func(ctx *tree.EvalContext, args tree.Datums) (tree.Datum, error) {
				if len(ctx.SessData.Database) == 0 {
					return tree.DNull, nil
				}
				return tree.NewDString(ctx.SessData.Database), nil
			},
	...
	"current_schemas": {
		tree.Builtin{
			...
			Fn: func(ctx *tree.EvalContext, args tree.Datums) (tree.Datum, error) {
				...
				if len(ctx.SessData.Database) != 0 {
					if err := schemas.Append(tree.NewDString(ctx.SessData.Database)); err != nil {
						return nil, err
					}
				}
				var iter func() (string, bool)
				if includePgCatalog {
					iter = ctx.SessData.SearchPath.Iter()
				} else {
					iter = ctx.SessData.SearchPath.IterWithoutImplicitPGCatalog()
				}
				for p, ok := iter(); ok; p, ok = iter() {
					if p == ctx.SessData.Database {
						continue
					}
	...
	"current_user": {
		tree.Builtin{
			...
			Category:   categorySystemInfo,
			Fn: func(ctx *tree.EvalContext, args tree.Datums) (tree.Datum, error) {
				if len(ctx.SessData.User) == 0 {
					return tree.DNull, nil
				}
				return tree.NewDString(ctx.SessData.User), nil
			},
