...
func mergeStatsMetadataHelper(metadata *appstatspb.AggregatedStatementMetadata, metadataDatum tree.Datum) error {
	if metadataDatum == tree.DNull {
		return nil
	}
	...
	metadataJSON := tree.MustBeDJSON(metadataDatum).JSON
	// Only decode and set the query info it was not previously set. Avoid the
...
func mergeStatementStatsHelper(aggregatedStats *appstatspb.StatementStatistics, statsDatum tree.Datum) error {
	if statsDatum == tree.DNull {
		return nil
	}
...
func mergeTransactionStatsHelper(aggregatedStats *appstatspb.TransactionStatistics, statsDatum tree.Datum) error {
	if statsDatum == tree.DNull {
		return nil
	}
