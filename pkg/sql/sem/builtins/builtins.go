	...
	"crdb_internal.update_tenant_resource_limits": makeBuiltin(
		...
		tree.Overload{
			...
			Fn: func(ctx *tree.EvalContext, args tree.Datums) (tree.Datum, error) {
				...
				operationUUID := args[1].(*tree.DUuid).UUID
				availableRU := float64(tree.MustBeDFloat(args[2]))
				refillRate := float64(tree.MustBeDFloat(args[3]))
				maxBurstRU := float64(tree.MustBeDFloat(args[4]))
				asOf := tree.MustBeDTimestamp(args[5]).Time
				asOfConsumed := float64(tree.MustBeDFloat(args[6]))
