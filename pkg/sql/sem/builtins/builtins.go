...
var (
	...
	errChrValueTooLarge = pgerror.NewError(pgerror.CodeInvalidParameterValueError, "input value must be < 1,114,112 (maximum Unicode code point)")
)
...
var builtins = map[string]builtinDefinition{
	...
	"chr": makeBuiltin(tree.FunctionProperties{Category: categoryString},
		tree.Overload{
			...
			Info: `Returns the ASCII character value of a given integer.
Fails if integer falls outside of ASCII character set range.
Inverse function of 'ascii()'.`,
		},
	),
