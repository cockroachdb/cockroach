...
may increase either contention or retry errors, or both.`,
	...
	// parse_timestamp is converts strings to timestamps. It is useful in
	// expressions where casts (which are not immutable) cannot be used, like
	// computed column expressions or partial index predicates. Only absolute
	// timestamps that do not depend on the current context are supported
	// (relative timestamps like 'now' are not supported).
	"parse_timestamp": makeBuiltin(defProps(),
		tree.Overload{
			...
			Fn: func(ctx *tree.EvalContext, args tree.Datums) (tree.Datum, error) {
				...
				if dependsOnContext {
					return nil, errors.New("relative timestamps are not supported")
				}
				return ts, nil
			},
