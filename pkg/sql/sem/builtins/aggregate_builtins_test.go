...
func TestBitAndIntResultDeepCopy(t *testing.T) {
	...
	t.Run("with null", func(t *testing.T) {
		testAggregateResultDeepCopy(t, newBitAndIntAggregate, makeTestWillNullDatum(10, makeIntTestDatum))
	})
...
func TestBitOrIntResultDeepCopy(t *testing.T) {
	...
	t.Run("with null", func(t *testing.T) {
		testAggregateResultDeepCopy(t, newBitOrIntAggregate, makeTestWillNullDatum(10, makeIntTestDatum))
	})
	t.Run("without null", func(t *testing.T) {
...
// makeTestWillNullDatum will call the maker function
...
func makeTestWillNullDatum(count int, maker func(count int) []tree.Datum) []tree.Datum {
	rng, _ := randutil.NewPseudoRand()

	values := append(maker(count-1), nil)
	i := rng.Int() % count
