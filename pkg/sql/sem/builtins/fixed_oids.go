...
var builtinOidsBySignature = map[string]oid.Oid{
	...
	//`crdb_internal.system_jobs -> tuple{int AS id, string AS status, timestamp AS created, bytes AS payload, bytes AS progress, string AS created_by_type, int AS created_by_id, bytes AS claim_session_id, int AS claim_instance_id, int AS num_runs, timestamp AS last_run}`: 2059,
	...
	`crdb_internal.job_payload_user(data: bytes) -> string`:                                                                             2062,
	...
	`crdb_internal.hide_sql_constants(val: string) -> string`:                                 2055,
	`crdb_internal.hide_sql_constants(val: string[]) -> string[]`:                             2056,
	...
	`crdb_internal.start_replication_stream(tenant_name: string) -> bytes`:                                                                2044,
	...
	`tsquerysend(tsquery: tsquery) -> bytes`:                                                                                                                                        2047,
	`tsvectorsend(tsvector: tsvector) -> bytes`:                                                                                                                                     2048,
	`tsqueryout(tsquery: tsquery) -> bytes`:                                                                                                                                         2049,
	`tsvectorout(tsvector: tsvector) -> bytes`:                                                                                                                                      2050,
	`tsqueryrecv(input: anyelement) -> tsquery`:                                                                                                                                     2051,
	`tsvectorrecv(input: anyelement) -> tsvector`:                                                                                                                                   2052,
	`tsqueryin(input: anyelement) -> tsquery`:                                                                                                                                       2053,
	`tsvectorin(input: anyelement) -> tsvector`:                                                                                                                                     2054,
