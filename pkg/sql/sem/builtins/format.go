...
// sqlFormat formats according to a format specifier in the style of postgres format()
// and returns the resulting string.
func sqlFormat(ctx *eval.Context, format string, a ...tree.Datum) (string, error) {
	p := pp{
		ctx: ctx,
...
func (p *pp) printArg(arg tree.Datum, verb rune) error {
	var writeFunc func(*tree.FmtCtx) int
	if arg == tree.DNull {
		switch verb {
		case 's':
		...
		case 'L':
			writeFunc = func(buf *tree.FmtCtx) int { buf.WriteString(nullLiteral); return 4 }
		}
	} else {
		switch verb {
		case 's':
