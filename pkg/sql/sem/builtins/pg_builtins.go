	...
	"shobj_description": makeBuiltin(defProps(),
		tree.Overload{
			...
			Fn: func(ctx *tree.EvalContext, args tree.Datums) (tree.Datum, error) {
				...
				classOid, err := getCatalogOidForComments(catalogName)
				if err != nil {
...
func getCatalogOidForComments(catalogName string) (int, error) {
	switch catalogName {
	case "pg_class":
		return sqlbase.PgCatalogClassTableID, nil
	case "pg_database":
		return sqlbase.PgCatalogDatabaseTableID, nil
	default:
		// We currently only support comments on pg_class objects
		// (columns, tables) in this context.
		// see a different name, matching pg.
		return 0, errors.New("unsupported catalog name for comments " + catalogName)
	}
}
