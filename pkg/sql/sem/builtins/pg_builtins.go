			...
			Fn: func(ctx context.Context, evalCtx *eval.Context, args tree.Datums) (tree.Datum, error) {
				if evalCtx.Planner == nil {
					return nil, pgerror.New(pgcode.FeatureNotSupported,
						"pg_advisory_xact_lock requires a SQL session context")
				}
				key := int64(*args[0].(*tree.DInt))
				if err := evalCtx.Planner.AdvisoryXactLock(ctx, key, false /* shared */); err != nil {
					return nil, err
				}
	...
	"pg_advisory_xact_lock_shared": makeBuiltin(defProps(),
		tree.Overload{
			...
			Fn: func(ctx context.Context, evalCtx *eval.Context, args tree.Datums) (tree.Datum, error) {
				if evalCtx.Planner == nil {
					return nil, pgerror.New(pgcode.FeatureNotSupported,
						"pg_advisory_xact_lock_shared requires a SQL session context")
				}
				key := int64(*args[0].(*tree.DInt))
	...
	"pg_try_advisory_xact_lock": makeBuiltin(defProps(),
		tree.Overload{
			...
			Fn: func(ctx context.Context, evalCtx *eval.Context, args tree.Datums) (tree.Datum, error) {
				if evalCtx.Planner == nil {
					return nil, pgerror.New(pgcode.FeatureNotSupported,
						"pg_try_advisory_xact_lock requires a SQL session context")
				}
				key := int64(*args[0].(*tree.DInt))
	...
	"pg_try_advisory_xact_lock_shared": makeBuiltin(defProps(),
		tree.Overload{
			...
			Fn: func(ctx context.Context, evalCtx *eval.Context, args tree.Datums) (tree.Datum, error) {
				if evalCtx.Planner == nil {
					return nil, pgerror.New(pgcode.FeatureNotSupported,
						"pg_try_advisory_xact_lock_shared requires a SQL session context")
				}
				key := int64(*args[0].(*tree.DInt))
