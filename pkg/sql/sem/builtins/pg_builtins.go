...
func formatFunctionArguments(
	...
	// on overload.Types. Fall back to types-only formatting (just the type
	// names, no parameter names) — matches what the prior SQL implementation
	// emitted from pg_proc.proargtypes.
...
// formatBuiltinArguments returns the comma-separated input-argument types of a
// built-in overload. It exists for parity with the prior SQL implementation,
// which only ever read pg_proc.proargtypes.
func formatBuiltinArguments(overload *tree.Overload) tree.Datum {
	...
	for i := range paramTypes {
		...
		sb.WriteString(paramTypes[i].Typ.PGName())
	}
...
// PG happen to share most non-string OIDs). When the type is a *types.T, this
// is exactly t.PGName(); for unresolved or OID-based references we fall back
// to the standard SQLString form.
func typeReferencePGName(ref tree.ResolvableTypeReference) string {
	if t, ok := ref.(*types.T); ok {
		return t.PGName()
	}
