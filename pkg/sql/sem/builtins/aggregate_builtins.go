...
var aggregates = map[string]builtinDefinition{
	...
	"percentile_cont": makeBuiltin(aggProps(),
		makeAggOverload(
			...
			types.Float,
			newPercentileContFloatAggregate,
		...
		makeAggOverload(
			...
			types.Interval,
			newPercentileContIntervalAggregate,
			"Continuous percentile: returns a interval corresponding to the specified fraction in the ordering, "+
				"interpolating between adjacent input intervals if needed."),
	),
	"percentile_cont_impl": makeBuiltin(aggProps(),
		makeAggOverload(
			[]*types.T{types.Float, types.Float},
			types.Float,
			newPercentileContFloatAggregate,
...
var _ tree.AggregateFunc = &percentileContFloatAggregate{}
var _ tree.AggregateFunc = &percentileContIntervalAggregate{}
...
const sizeOfPercentileDiscreteAggregate = int64(unsafe.Sizeof(percentileDiscAggregate{}))
const sizeOfPercentileContinuousFloatAggregate = int64(unsafe.Sizeof(percentileContFloatAggregate{}))
const sizeOfPercentileContinuousIntervalAggregate = int64(unsafe.Sizeof(percentileContIntervalAggregate{}))
...
func (a *percentileDiscAggregate) Add(
	...
	if datum != tree.DNull {
		percent := float64(tree.MustBeDFloat(datum))
...
func (a *percentileDiscAggregate) Result() (tree.Datum, error) {
	...
	// exceeds the specified fraction.
	rowNumber := a.fraction * float64(a.arr.Len())
...
type percentileContFloatAggregate struct {
	arr *tree.DArray
	// Note that we do not embed singleDatumAggregateBase struct to help with
...
func newPercentileContFloatAggregate(
	params []*types.T, evalCtx *tree.EvalContext, _ tree.Datums,
) tree.AggregateFunc {
	return &percentileContFloatAggregate{
		arr: tree.NewDArray(params[1]),
		acc: evalCtx.Mon.MakeBoundAccount(),
	}
}
...
func (a *percentileContFloatAggregate) Add(
	ctx context.Context, datum tree.Datum, others ...tree.Datum,
) error {
	if datum != tree.DNull {
		percent := float64(tree.MustBeDFloat(datum))
...
func (a *percentileContFloatAggregate) Result() (tree.Datum, error) {
	// Return null if there are no values.
	if a.arr.Len() == 0 {
		return tree.DNull, nil
	}
	...
	var target float64
	rowNumber := 1.0 + (a.fraction * (float64(a.arr.Len()) - 1.0))
...
// Reset implements tree.AggregateFunc interface.
func (a *percentileContFloatAggregate) Reset(ctx context.Context) {
	a.arr = tree.NewDArray(a.arr.ParamTyp)
	a.acc.Empty(ctx)
}
...
// Close allows the aggregate to release the memory it requested during
// operation.
func (a *percentileContFloatAggregate) Close(ctx context.Context) {
	a.acc.Close(ctx)
}

// Size is part of the tree.AggregateFunc interface.
func (a *percentileContFloatAggregate) Size() int64 {
	return sizeOfPercentileContinuousFloatAggregate
}

type percentileContIntervalAggregate struct {
	arr *tree.DArray
	// Note that we do not embed singleDatumAggregateBase struct to help with
	// memory accounting because percentileContAggregate stores multiple datums
	// inside of arr.
...
func newPercentileContIntervalAggregate(
	params []*types.T, evalCtx *tree.EvalContext, _ tree.Datums,
) tree.AggregateFunc {
	return &percentileContIntervalAggregate{
		arr: tree.NewDArray(params[1]),
		acc: evalCtx.Mon.MakeBoundAccount(),
	}
}
...
// Add stores the input percentile and all the values to calculate the continuous percentile.
func (a *percentileContIntervalAggregate) Add(
	ctx context.Context, datum tree.Datum, others ...tree.Datum,
) error {
	if datum != tree.DNull {
		percent := float64(tree.MustBeDFloat(datum))
...
// Result finds the continuous percentile.
func (a *percentileContIntervalAggregate) Result() (tree.Datum, error) {
	// Return null if there are no values.
	if a.arr.Len() == 0 {
		return tree.DNull, nil
	}
...
// Reset implements tree.AggregateFunc interface.
func (a *percentileContIntervalAggregate) Reset(ctx context.Context) {
	a.arr = tree.NewDArray(a.arr.ParamTyp)
	a.acc.Empty(ctx)
}
...
// Close allows the aggregate to release the memory it requested during
// operation.
func (a *percentileContIntervalAggregate) Close(ctx context.Context) {
	a.acc.Close(ctx)
}

// Size is part of the tree.AggregateFunc interface.
func (a *percentileContIntervalAggregate) Size() int64 {
	return sizeOfPercentileContinuousIntervalAggregate
}
