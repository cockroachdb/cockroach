...
var aggregates = map[string]builtinDefinition{
	...
	"percentile_cont": makeBuiltin(aggProps(),
		makeAggOverload(
			...
			"Continuous percentile: returns a float corresponding to the specified fractions in the ordering, "+
				"interpolating between adjacent input floats if needed."),
		makeAggOverload(
			[]*types.T{types.Float},
			types.Interval,
			newPercentileContAggregate,
			"Continuous percentile: returns an interval corresponding to the specified fractions in the ordering, "+
...
func (a *percentileDiscAggregate) Add(
	...
	if len(a.fractions) == 0 && datum != tree.DNull {
		if datum.ResolvedType().Identical(types.Float) {
			...
			fractions := tree.MustBeDArray(datum)
			for _, f := range fractions.Array {
