...
var aggregates = map[string]builtinDefinition{
	...
	"avg": makeBuiltin(aggProps(),
		makeDefaultAggOverload([]*types.T{types.Int}, types.Decimal, newIntAvgAggregate,
			"Calculates the average of the selected values."),
		makeDefaultAggOverload([]*types.T{types.Float}, types.Float, newFloatAvgAggregate,
			"Calculates the average of the selected values."),
		makeDefaultAggOverload([]*types.T{types.Decimal}, types.Decimal, newDecimalAvgAggregate,
			"Calculates the average of the selected values."),
		makeDefaultAggOverload([]*types.T{types.Interval}, types.Interval, newIntervalAvgAggregate,
			"Calculates the average of the selected values."),
	),

	"bit_and": makeBuiltin(aggProps(),
		makeDefaultAggOverload([]*types.T{types.Int}, types.Int, newIntBitAndAggregate,
			"Calculates the bitwise AND of all non-null input values, or null if none."),
		makeDefaultAggOverload([]*types.T{types.VarBit}, types.VarBit, newBitBitAndAggregate,
			"Calculates the bitwise AND of all non-null input values, or null if none."),
	),

	"bit_or": makeBuiltin(aggProps(),
		makeDefaultAggOverload([]*types.T{types.Int}, types.Int, newIntBitOrAggregate,
			"Calculates the bitwise OR of all non-null input values, or null if none."),
		makeDefaultAggOverload([]*types.T{types.VarBit}, types.VarBit, newBitBitOrAggregate,
			"Calculates the bitwise OR of all non-null input values, or null if none."),
	),

	"bool_and": makeBuiltin(aggProps(),
		makeDefaultAggOverload([]*types.T{types.Bool}, types.Bool, newBoolAndAggregate,
			"Calculates the boolean value of `AND`ing all selected values."),
	),

	"bool_or": makeBuiltin(aggProps(),
		makeDefaultAggOverload([]*types.T{types.Bool}, types.Bool, newBoolOrAggregate,
			"Calculates the boolean value of `OR`ing all selected values."),
	),

	"concat_agg": makeBuiltin(aggProps(),
		makeDefaultAggOverload([]*types.T{types.String}, types.String, newStringConcatAggregate,
			"Concatenates all selected values."),
		makeDefaultAggOverload([]*types.T{types.Bytes}, types.Bytes, newBytesConcatAggregate,
			"Concatenates all selected values."),
	...
	"final_covar_pop": makePrivate(makeBuiltin(aggProps(),
		makeDefaultAggOverload([]*types.T{types.DecimalArray}, types.Float, newFinalCovarPopAggregate,
	...
	"final_regr_sxx": makePrivate(makeBuiltin(aggProps(),
		makeDefaultAggOverload([]*types.T{types.DecimalArray}, types.Float, newFinalRegrSXXAggregate,
			"Calculates sum of squares of the independent variable in final stage."),
	)),

	"final_regr_sxy": makePrivate(makeBuiltin(aggProps(),
		makeDefaultAggOverload([]*types.T{types.DecimalArray}, types.Float, newFinalRegrSXYAggregate,
	...
	"final_regr_syy": makePrivate(makeBuiltin(aggProps(),
		makeDefaultAggOverload([]*types.T{types.DecimalArray}, types.Float, newFinalRegrSYYAggregate,
			"Calculates sum of squares of the dependent variable in final stage."),
	)),

	"final_regr_avgx": makePrivate(makeBuiltin(aggProps(),
		makeDefaultAggOverload([]*types.T{types.DecimalArray}, types.Float, newFinalRegressionAvgXAggregate,
	...
	"final_regr_avgy": makePrivate(makeBuiltin(aggProps(),
		makeDefaultAggOverload([]*types.T{types.DecimalArray}, types.Float, newFinalRegressionAvgYAggregate,
	...
	"final_regr_intercept": makePrivate(makeBuiltin(aggProps(),
		makeDefaultAggOverload([]*types.T{types.DecimalArray}, types.Float, newFinalRegressionInterceptAggregate,
	...
	"final_regr_r2": makePrivate(makeBuiltin(aggProps(),
		makeDefaultAggOverload([]*types.T{types.DecimalArray}, types.Float, newFinalRegressionR2Aggregate,
			"Calculates square of the correlation coefficient in final stage."),
	)),

	"final_regr_slope": makePrivate(makeBuiltin(aggProps(),
		makeDefaultAggOverload([]*types.T{types.DecimalArray}, types.Float, newFinalRegressionSlopeAggregate,
	...
	"final_corr": makePrivate(makeBuiltin(aggProps(),
		makeDefaultAggOverload([]*types.T{types.DecimalArray}, types.Float, newFinalCorrAggregate,
	...
	"final_covar_samp": makePrivate(makeBuiltin(aggProps(),
		makeDefaultAggOverload([]*types.T{types.DecimalArray}, types.Float, newFinalCovarSampAggregate,
	...
	"regr_count": makeBuiltin(aggProps(),
		makeDefaultAggOverload([]*types.T{types.Float, types.Float}, types.Int, newRegressionCountAggregate,
			"Calculates number of input rows in which both expressions are nonnull."),
		makeDefaultAggOverload([]*types.T{types.Int, types.Int}, types.Int, newRegressionCountAggregate,
			"Calculates number of input rows in which both expressions are nonnull."),
		makeDefaultAggOverload([]*types.T{types.Decimal, types.Decimal}, types.Int, newRegressionCountAggregate,
			"Calculates number of input rows in which both expressions are nonnull."),
		makeDefaultAggOverload([]*types.T{types.Float, types.Int}, types.Int, newRegressionCountAggregate,
			"Calculates number of input rows in which both expressions are nonnull."),
		makeDefaultAggOverload([]*types.T{types.Float, types.Decimal}, types.Int, newRegressionCountAggregate,
			"Calculates number of input rows in which both expressions are nonnull."),
		makeDefaultAggOverload([]*types.T{types.Int, types.Float}, types.Int, newRegressionCountAggregate,
			"Calculates number of input rows in which both expressions are nonnull."),
		makeDefaultAggOverload([]*types.T{types.Int, types.Decimal}, types.Int, newRegressionCountAggregate,
			"Calculates number of input rows in which both expressions are nonnull."),
		makeDefaultAggOverload([]*types.T{types.Decimal, types.Float}, types.Int, newRegressionCountAggregate,
			"Calculates number of input rows in which both expressions are nonnull."),
		makeDefaultAggOverload([]*types.T{types.Decimal, types.Int}, types.Int, newRegressionCountAggregate,
			"Calculates number of input rows in which both expressions are nonnull."),
	),
	...
	"max": collectOverloads(aggProps(), allMaxMinAggregateTypes,
		func(t *types.T) tree.Overload {
			...
			return makeDefaultAggOverloadWithReturnType(
	...
	"sum_int": makeBuiltin(aggProps(),
		makeDefaultAggOverload([]*types.T{types.Int}, types.Int, newSmallIntSumAggregate,
			"Calculates the sum of the selected values."),
	),

	"sum": makeBuiltin(aggProps(),
		makeDefaultAggOverload([]*types.T{types.Int}, types.Decimal, newIntSumAggregate,
			"Calculates the sum of the selected values."),
		makeDefaultAggOverload([]*types.T{types.Float}, types.Float, newFloatSumAggregate,
			"Calculates the sum of the selected values."),
		makeDefaultAggOverload([]*types.T{types.Decimal}, types.Decimal, newDecimalSumAggregate,
			"Calculates the sum of the selected values."),
		makeDefaultAggOverload([]*types.T{types.Interval}, types.Interval, newIntervalSumAggregate,
			"Calculates the sum of the selected values."),
	),

	"sqrdiff": makeBuiltin(aggProps(),
		makeDefaultAggOverload([]*types.T{types.Int}, types.Decimal, newIntSqrDiffAggregate,
			"Calculates the sum of squared differences from the mean of the selected values."),
		makeDefaultAggOverload([]*types.T{types.Decimal}, types.Decimal, newDecimalSqrDiffAggregate,
			"Calculates the sum of squared differences from the mean of the selected values."),
		makeDefaultAggOverload([]*types.T{types.Float}, types.Float, newFloatSqrDiffAggregate,
			"Calculates the sum of squared differences from the mean of the selected values."),
	),
	...
	"var_pop": makeBuiltin(aggProps(),
		makeDefaultAggOverload([]*types.T{types.Int}, types.Decimal, newIntVarPopAggregate,
			"Calculates the population variance of the selected values."),
		makeDefaultAggOverload([]*types.T{types.Decimal}, types.Decimal, newDecimalVarPopAggregate,
			"Calculates the population variance of the selected values."),
		makeDefaultAggOverload([]*types.T{types.Float}, types.Float, newFloatVarPopAggregate,
			"Calculates the population variance of the selected values."),
	),
	...
	"stddev_pop": makeBuiltin(aggProps(),
		makeDefaultAggOverload([]*types.T{types.Int}, types.Decimal, newIntStdDevPopAggregate,
			"Calculates the population standard deviation of the selected values."),
		makeDefaultAggOverload([]*types.T{types.Decimal}, types.Decimal, newDecimalStdDevPopAggregate,
			"Calculates the population standard deviation of the selected values."),
		makeDefaultAggOverload([]*types.T{types.Float}, types.Float, newFloatStdDevPopAggregate,
			"Calculates the population standard deviation of the selected values."),
	),

	"xor_agg": makeBuiltin(aggProps(),
		makeDefaultAggOverload([]*types.T{types.Bytes}, types.Bytes, newBytesXorAggregate,
			"Calculates the bitwise XOR of the selected values."),
		makeDefaultAggOverload([]*types.T{types.Int}, types.Int, newIntXorAggregate,
			"Calculates the bitwise XOR of the selected values."),
	),
	...
	AnyNotNull: makePrivate(makeBuiltin(aggProps(),
		makeDefaultAggOverloadWithReturnType(
			[]*types.T{types.Any},
	...
	"percentile_disc_impl": makePrivate(collectOverloads(aggProps(), types.Scalar,
		func(t *types.T) tree.Overload {
			return makeDefaultAggOverload([]*types.T{types.Float, t}, t, newPercentileDiscAggregate,
				"Implementation of percentile_disc.",
			)
		},
		func(t *types.T) tree.Overload {
			return makeDefaultAggOverload([]*types.T{types.FloatArray, t}, types.MakeArray(t), newPercentileDiscAggregate,
				"Implementation of percentile_disc.",
			)
		},
	)),
...
func makeStdDevBuiltin() builtinDefinition {
	return makeBuiltin(aggProps(),
		makeDefaultAggOverload([]*types.T{types.Int}, types.Decimal, newIntStdDevAggregate,
			"Calculates the standard deviation of the selected values."),
		makeDefaultAggOverload([]*types.T{types.Decimal}, types.Decimal, newDecimalStdDevAggregate,
			"Calculates the standard deviation of the selected values."),
		makeDefaultAggOverload([]*types.T{types.Float}, types.Float, newFloatStdDevAggregate,
			"Calculates the standard deviation of the selected values."),
	)
}
...
func makeRegressionAggregate(
	...
	return makeBuiltin(aggProps(),
		makeDefaultAggOverload([]*types.T{types.Float, types.Float}, ret, aggregateFunc, info),
		makeDefaultAggOverload([]*types.T{types.Int, types.Int}, ret, aggregateFunc, info),
		makeDefaultAggOverload([]*types.T{types.Decimal, types.Decimal}, ret, aggregateFunc, info),
		makeDefaultAggOverload([]*types.T{types.Float, types.Int}, ret, aggregateFunc, info),
		makeDefaultAggOverload([]*types.T{types.Float, types.Decimal}, ret, aggregateFunc, info),
		makeDefaultAggOverload([]*types.T{types.Int, types.Float}, ret, aggregateFunc, info),
		makeDefaultAggOverload([]*types.T{types.Int, types.Decimal}, ret, aggregateFunc, info),
		makeDefaultAggOverload([]*types.T{types.Decimal, types.Float}, ret, aggregateFunc, info),
		makeDefaultAggOverload([]*types.T{types.Decimal, types.Int}, ret, aggregateFunc, info),
	)
}
...
func makeVarianceBuiltin() builtinDefinition {
	return makeBuiltin(aggProps(),
		makeDefaultAggOverload([]*types.T{types.Int}, types.Decimal, newIntVarianceAggregate,
			"Calculates the variance of the selected values."),
		makeDefaultAggOverload([]*types.T{types.Decimal}, types.Decimal, newDecimalVarianceAggregate,
			"Calculates the variance of the selected values."),
		makeDefaultAggOverload([]*types.T{types.Float}, types.Float, newFloatVarianceAggregate,
			"Calculates the variance of the selected values."),
	)
}
