...
var invalidCompressionCodecError = pgerror.New(
	...
	"only 'gzip', 'lz4', 'snappy', or 'zstd' codecs are supported for decompress()")
...
func (c zstdCodec) decompress(compressedData []byte) ([]byte, error) {
	return decompressUsing(
		...
		func(buf io.Reader) (io.ReadCloser, error) {
			...
			return noErrorCloser{readCloserNoError: r}, err
		})
}
...
// decompressUsing decompresses input data using compressor returned by
