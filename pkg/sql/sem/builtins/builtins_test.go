...
SELECT array_agg(c)
  FROM counts;`, prefixBits).Scan(&keyCounts)
	...
	// This test can flake when the random data is not distributed evenly so only
	// require it pass once.
	const maxAttempts = 2
	for i := 1; i <= maxAttempts; i++ {
		...
		if i < maxAttempts {
			t.Logf("runTest failure attempt %d/%d - error %v", i, maxAttempts, err)
		} else {
			require.NoErrorf(t, err, "runTest failure attempt %d/%d", i, maxAttempts)
		}
	}
}
