...
func (tssi *tableSpanStatsIterator) Start(ctx context.Context, _ *kv.Txn) error {
	var err error = nil
	// Fetch details for multiple table span stats.
	if !tssi.singleTableReq {
		tssi.it, err = tssi.p.GetDetailsForSpanStats(ctx, tssi.currDbId)
	}
	return err
}
...
func (tssi *tableSpanStatsIterator) Next(ctx context.Context) (bool, error) {
	var next bool
	var err error
	// If we have only a single table span's stats to get,
	// there are no more iterations once we have a response.
	if tssi.singleTableReq {
		next = tssi.currStatsResponse == nil
	} else if tssi.it == nil {
		// If we have multiple table spans' stats to get, but nil rows.
		return false, errors.AssertionFailedf("Start must be called before Next")
	} else {
		// If we have multiple table spans' stats to get.
