...
func makeTableSpanStatsGenerator(
	...
	// The user must have VIEWACTIVITY permission to use this builtin.
	hasViewActivity, err := evalCtx.SessionAccessor.HasViewActivity(ctx)
	if err != nil {
		return nil, err
	}
	if !hasViewActivity {
		return nil, pgerror.Newf(pgcode.InsufficientPrivilege, "user needs the VIEWACTIVITY permission to view span statistics")
	}
	dbId := 0
	tableId := 0
	if len(args) > 0 {
