...
var generators = map[string]builtinDefinition{
	...
	"crdb_internal.check_consistency": makeBuiltin(genProps(
		[]string{"range_id", "start_key", "start_key_pretty", "status", "detail"} /* labels */),
		makeGeneratorOverload(
			tree.ArgTypes{
			...
			"Runs a consistency check on ranges touching the specified key range. "+
				...
				"value is an array of tuples, "+"with each tuple consisting of the range "+
				"ID, the status (a "+"roachpb.CheckConsistencyResponse_Status), and "+
				"verbose detail.\n\n"+
				"Example usage:\n\n"+
