...
// spanRequestLimit is an arbitrary limit for the number of
// table span statistics we fetch in a single SpanStats request
const spanRequestLimit = 1000
...
func newTableSpanStatsIterator(eval *eval.Context, dbId int, tableId int) *tableSpanStatsIterator {
	return &tableSpanStatsIterator{codec: eval.Codec, p: eval.Planner, argDbId: dbId, argTableId: tableId}
}
...
func (tssi *tableSpanStatsIterator) fetchSpanStats(ctx context.Context) (bool, error) {
	defer timeutil.TimeTrack(time.Now(), "tssi - fetchSpanStats")
	// Reset span details.
	...
	for ok, err = tssi.it.Next(ctx); ok; ok, err = tssi.it.Next(ctx) {
		...
		if len(tssi.spanStatsDetails) >= spanRequestLimit {
			break
		}
	}
	...
	for k, v := range tssi.currStatsResponse.SpanToStats {
		fmt.Printf("final stats response table span: %s, stats: %s\n", k, v)
	}
	return ok, err
}
