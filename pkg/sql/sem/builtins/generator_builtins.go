...
var payloadsForSpanGeneratorLabels = []string{"payload_type", "payload_pretty", "payload_jsonb"}

var payloadsForSpanGeneratorType = types.MakeLabeledTuple(
	[]*types.T{types.String, types.String, types.Jsonb},
...
func makePayloadsForSpanGenerator(
	...
	if !isAdmin {
		return nil, pgerror.Newf(
			...
			"only users with the admin role are allowed to read crdb_internal.payloads_for_span",
		)
	}
...
func (p *payloadsForSpanGenerator) Values() (tree.Datums, error) {
	...
	pretty, err := json.Pretty(payload)
	if err != nil {
		return nil, err
	}
