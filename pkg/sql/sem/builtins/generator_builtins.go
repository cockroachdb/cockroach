...
var generators = map[string]builtinDefinition{
	...
	"crdb_internal.payloads_for_span_as_table": makeBuiltin(
		tree.FunctionProperties{
			Class:    tree.GeneratorClass,
			Category: categorySystemInfo,
		},
...
var payloadsForSpanGeneratorLabels = []string{"payload_type", "full_payload"}
...
type payloadsForSpanGenerator struct {
	...
	// recordingIndex maintains the current position of the index of the list of
	// recordings surfaced by a given span.
	recordingIndex int
	...
	// payloadIndex maintains the current position of the index of the list of
	// payloads associated with a given recording.
	payloadIndex int
}
...
// Start implements the tree.ValueGenerator interface.
func (p *payloadsForSpanGenerator) Start(ctx context.Context, txn *kv.Txn) error {
	// The user of the generator first calls Next(), then Values(), so the index
	// managing the iterator's position needs to start at -1 instead of 0.
	p.recordingIndex = -1
	p.payloadIndex = -1
...
func (p *payloadsForSpanGenerator) Values() (tree.Datums, error) {
	...
	payloadType, err := payload.FetchValKey("@type")
	if err != nil {
		return nil, err
	}
	...
	return tree.Datums{
		...
		tree.NewDJSON(p.payloads[p.payloadIndex]),
	}, nil
}
