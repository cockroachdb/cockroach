...
SELECT (SELECT count(DISTINCT (cols)) FROM t) -
	...
	t.Run("testTableWithColumnTypes values and nulls all work", func(t *testing.T) {
		...
		for i := range types {
			...
			go func() {
				defer wg.Done()
				t.Run(typ, func(t *testing.T) { testTableWithColumnTypes(t, typ) })
			}()
		}
	})
	t.Run("testTableWithColumnTypes various combinations work and seem unique", func(t *testing.T) {
