...
func PKCSUnpad(data []byte, blockSize int) ([]byte, error) {
	if blockSize <= 0 || blockSize > math.MaxUint8 {
		return nil, errors.Newf("invalid block size for PKCS unpadding: %d", blockSize)
	}
	if len(data) == 0 {
		return nil, errors.New("PKCS-padded data is empty")
	}
	if len(data)%blockSize != 0 {
		return nil, errors.Newf("PKCS-padded data length (%d) is not a multiple of the block size (%d)", len(data), blockSize)
	}

	paddingLen := data[len(data)-1]
	if paddingLen == 0 || int(paddingLen) > blockSize {
		return nil, errors.Newf("invalid final byte found in PKCS padding: %d", paddingLen)
	}
	for i := 1; i < int(paddingLen); i++ {
		if b := data[len(data)-i-1]; b != paddingLen {
			return nil, errors.Newf("invalid byte found in PKCS padding: expected %d, but found %d", paddingLen, b)
		}
	}
