...
// SlidingWindowRingBufferInitialSize defines the initial size of the ring buffer.
const SlidingWindowRingBufferInitialSize = 8
...
func makeSlidingWindow(
	...
	return &slidingWindow{
		values: ringBuffer{
			values: make([]*indexedValue, SlidingWindowRingBufferInitialSize),
			empty:  true,
		},
		evalCtx: evalCtx,
		cmp:     cmp,
...
func (sw *slidingWindow) add(iv *indexedValue) {
	...
	for newEndIdx = sw.values.len() - 1; newEndIdx >= 0; newEndIdx-- {
		if sw.cmp(sw.evalCtx, sw.values.get(newEndIdx).value, iv.value) > 0 {
			break
		}
...
func (sw *slidingWindow) removeAllBefore(idx int) {
	...
	for newStartIdx = 0; newStartIdx < sw.values.len() && newStartIdx < idx; newStartIdx++ {
		if sw.values.get(newStartIdx).idx >= idx {
			break
		}
