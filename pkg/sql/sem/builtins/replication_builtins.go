...
var replicationBuiltins = map[string]builtinDefinition{
	...
	"crdb_internal.setup_span_configs_stream": makeBuiltin(
		...
		tree.Overload{
			...
			Info: "This function can be used on the producer side to run a replication stream for " +
				"the span configs of the tenant. The client can run the returned subscriptions",
			Volatility: volatility.Volatile,
		},
	),
}
