...
type Server struct {
	...
	mu struct {
		...
		// draining is set true when server starts draining any connections that are
		// without query in flight. Remaining connections will be closed as soon as
		// their queries finish. After the timeout set by server.shutdown.query_wait,
...
func (s *Server) drainImpl(
	...
	queryWaitLogMessage := fmt.Sprintf(
		"draining remaining SQL connections that are without queries in "+
			"flight. The server will proceed to the next draining phase once all SQL "+
			"queries finish or after after timeout for %s, whichever earlier. "+
