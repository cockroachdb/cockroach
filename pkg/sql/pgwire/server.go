...
type Server struct {
	...
	mu struct {
		...
		// queries in flight.
		draining bool
		// rejectConn is set true when the server does not accept new
...
// DisallowNewSQLConn disallows new SQL connections to the server.
func (s *Server) DisallowNewSQLConn() {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.mu.rejectConn = true
}
...
// WaitSQLConnsToClose waits for the client to close all SQL connections for the
...
// draining phase.
func (s *Server) WaitSQLConnsToClose(ctx context.Context, connectionWait time.Duration) {
	...
	_, allConnDone := s.waitConnsDone(quitWaitingForConns)
	...
	for {
		...
		case <-timer.C:
			log.Ops.Infof(ctx,
				"server.shutdown.connection_wait times out, "+
					"there are still %d SQL connections not closed by the client",
...
func (s *Server) waitConnsDone(quitWaitingForConns chan struct{}) (cancelChanMap, chan struct{}) {
	...
	return connCancelMap, allConnsDone
}
...
func (s *Server) drainImpl(
	...
	connCancelMap, allConnsDone := s.waitConnsDone(quitWaitingForConns)
	...
	log.Ops.Info(ctx, "starting draining the SQL layer...")
...
func (s *Server) registerConn(
	ctx context.Context,
) (newCtx context.Context, rejectConn bool, onCloseFn func()) {
	onCloseFn = func() {}
	newCtx = ctx
	s.mu.Lock()
	rejectConn = s.mu.rejectConn
	if !rejectConn {
