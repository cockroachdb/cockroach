...
func (s *Server) handleCancel(ctx context.Context, conn net.Conn, buf *pgwirebase.ReadBuffer) {
	...
	req := &serverpb.CancelQueryByBackendKeyDataRequest{
		BackendKeyData: backendKeyData,
	}
	var resp *serverpb.CancelQueryByBackendKeyDataResponse
	resp, err = s.execCfg.SQLStatusServer.CancelQueryByBackendKeyData(ctx, req)
	if err == nil && len(resp.Error) > 0 {
		err = fmt.Errorf("error from CancelQueryByBackendKeyDataResponse: %s", resp.Error)
	}
}
