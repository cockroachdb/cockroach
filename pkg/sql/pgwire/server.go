...
func MakeServer(
	...
	server.tenantConnMonitor = mon.NewMonitor("conn",
		mon.MemoryResource,
	...
	server.tenantConnMonitor.StartNoReserved(ctx, server.sqlMemoryPool)
...
func (s *Server) ServeConn(ctx context.Context, conn net.Conn, socketType SocketType) (err error) {
	...
	case versionCancel:
		...
		s.handleCancel(ctx, readCancelKeyAndCloseConn(ctx, conn, &buf))
		return nil
	...
	tenantReserved, err := s.tenantConnMonitor.TransferAccount(ctx, &reserved)
	if err != nil {
		reserved.Close(ctx)
		return s.sendErr(ctx, conn, err)
	}
