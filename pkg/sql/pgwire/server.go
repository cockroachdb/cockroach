...
// IsQueryDraining returns true if the server is draining SQL queries.
...
// waitSqlQueryDraining waits for queryWait for
// open SQL queries to terminate before canceling them.
...
func (s *Server) waitSqlQueryDraining(ctx context.Context, queryWait time.Duration) error {
	return s.drainImpl(ctx, queryWait, cancelMaxWait)
}
...
func (s *Server) Undrain() {
	...
	s.setRejectingSqlConnLocked(false)
...
// returns whether the state changed (i.e. drain != s.mu.rejectingNewSqlConn).
// s.mu must be locked.
...
// s.mu must be locked.
// This is a helper function to the connectionWait process, which listens to the
...
// WaitSqlDraining waits for SQL connections and queries to close.
// It consecutively triggers the waiting periods with lengths determined by
// connectionWait and queryWait.
func (s *Server) WaitSqlDraining(
	ctx context.Context, connectionWait time.Duration, queryWait time.Duration,
) error {
	...
	if err := s.waitSqlQueryDraining(ctx, queryWait); err != nil {
		return err
	}
	return nil
}
...
func (s *Server) drainImpl(
	...
	queryWaitLogMessage := fmt.Sprintf(
		"draining remaining SQL queries."+
			"The server will proceed to the next draining phase once all SQL queries "+
			"finish or after after timeout for %s, whichever earlier. "+
			"(This timeout duration is set by cluster setting \"server.shutdown.query_wait\")",
		queryWait,
	)
	...
	// Wait for all connections to finish up to queryWait.
	select {
