...
// using a semaphore in PGWireCancelQuery. The most bullet-proof rate limit
// would be cluster-wide, but in practice a per-node rate limit is fine. See
// the comments in PGWireCancelQuery for details about the rate limit.
//
...
func (s *Server) handleCancel(ctx context.Context, conn net.Conn, buf *pgwirebase.ReadBuffer) {
	...
	defer func() {
		if err != nil {
			log.Warningf(ctx, "unexpected while handling pgwire cancellation request: %v", err)
		}
	...
	backendKeyData := pgwirecancel.BackendKeyData(backendKeyDataBits)
	...
	if err == nil && len(resp.Error) > 0 {
		err = fmt.Errorf("error from PGWireCancelQueryResponse: %s", resp.Error)
	}
}
