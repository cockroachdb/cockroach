...
func TestPGWireRejectsNewConnIfTooManyConns(t *testing.T) {
	...
	setMaxNonRootConnections := func(maxConnections int) {
		conn, cleanup := openConnWithUserSuccess(rootUser)
		defer cleanup()
		_, err := conn.Exec(ctx, "SET CLUSTER SETTING server.max_non_root_connections_per_gateway = $1", maxConnections)
		require.NoError(t, err)
	}
	...
	setMaxNonRootConnectionsReason := func(reason string) {
		conn, cleanup := openConnWithUserSuccess(rootUser)
		defer cleanup()
		_, err := conn.Exec(ctx, "SET CLUSTER SETTING server.max_non_root_connections_per_gateway_reason = $1", reason)
		require.NoError(t, err)
	}
