...
func (b *writeBuffer) writeBinaryDatum(
	...
	case *parser.DIPNet:
		// We calculate the Postgres binary format for an IPNet. For the spec see,
		...
		//  The IP bytes
		const pgIPNetBinaryHeaderSize = 4
		if v.IPNet.Family == ipnet.IPv4Family {
			b.putInt32(net.IPv4len + pgIPNetBinaryHeaderSize)
			b.writeByte(pgBinaryIPv4Family)
			b.writeByte(byte(ones))
			b.writeByte(0)
			b.writeByte(byte(net.IPv4len))
			// Ensure IPv4 byte slice is only len(4).
			b.write(v.IPNet.IP.To4())
		} else if v.IPNet.Family == ipnet.IPv6Family {
			b.putInt32(net.IPv6len + pgIPNetBinaryHeaderSize)
