...
// that have various width encodings and therefore need padding (chars).
// It is ignored (and can be nil) for types which do not need padding.
func (b *writeBuffer) writeTextPhysicalElement(
	ctx context.Context, vec coldata.Vec, idx int, conv sessiondata.DataConversionConfig, t *types.T,
) {
	if log.V(2) {
		log.Infof(ctx, "pgwire writing TEXT physical element of type: %s", t)
	}
	...
	case types.BitFamily:
		b.textFormatter.FormatNode(vec.Datum().Get(idx).(*coldataext.Datum).Datum)
		b.writeFromFmtCtx(b.textFormatter)

	case types.BoolFamily:
		b.putInt32(1)
		b.writeByte(tree.PgwireFormatBool(vec.Bool().Get(idx)))

	case types.IntFamily:
		var v int64
		switch t.Width() {
		case 16:
			v = int64(vec.Int16().Get(idx))
		case 32:
			v = int64(vec.Int32().Get(idx))
		default:
			v = vec.Int64().Get(idx)
		}
		// Start at offset 4 because `putInt32` clobbers the first 4 bytes.
	...
	case types.FloatFamily:
		// Start at offset 4 because `putInt32` clobbers the first 4 bytes.
		s := strconv.AppendFloat(b.putbuf[4:4], vec.Float64().Get(idx), 'g', conv.GetFloatPrec(), 64)
		b.putInt32(int32(len(s)))
		b.write(s)
	...
	case types.BytesFamily:
		result := lex.EncodeByteArrayToRawBytes(
			string(vec.Bytes().Get(idx)), conv.BytesEncodeFormat, false /* skipHexPrefix */)
		b.putInt32(int32(len(result)))
		b.write([]byte(result))
	...
	case types.INetFamily:
		b.writeLengthPrefixedString(vec.Datum().Get(idx).(*coldataext.Datum).Datum.(*tree.DIPAddr).IPAddr.String())

	case types.StringFamily:
		b.writeLengthPrefixedString(resolveBlankPaddedChar(string(vec.Bytes().Get(idx)), t))

	case types.CollatedStringFamily:
		b.writeLengthPrefixedString(vec.Datum().Get(idx).(*coldataext.Datum).Datum.(*tree.DCollatedString).Contents)
	...
	case types.TimeFamily:
		// Start at offset 4 because `putInt32` clobbers the first 4 bytes.
		s := formatTime(timeofday.TimeOfDay(*vec.Datum().Get(idx).(*coldataext.Datum).Datum.(*tree.DTime)), b.putbuf[4:4])
		b.putInt32(int32(len(s)))
		b.write(s)

	case types.TimeTZFamily:
		// Start at offset 4 because `putInt32` clobbers the first 4 bytes.
		s := formatTimeTZ(vec.Datum().Get(idx).(*coldataext.Datum).Datum.(*tree.DTimeTZ).TimeTZ, b.putbuf[4:4])
		b.putInt32(int32(len(s)))
		b.write(s)

	case types.GeographyFamily:
		s := vec.Datum().Get(idx).(*coldataext.Datum).Datum.(*tree.DGeography).Geography.EWKBHex()
		b.putInt32(int32(len(s)))
		b.write([]byte(s))

	case types.GeometryFamily:
		s := vec.Datum().Get(idx).(*coldataext.Datum).Datum.(*tree.DGeometry).Geometry.EWKBHex()
		b.putInt32(int32(len(s)))
		b.write([]byte(s))

	case types.TimestampFamily:
		// Start at offset 4 because `putInt32` clobbers the first 4 bytes.
		s := formatTs(vec.Timestamp().Get(idx), nil, b.putbuf[4:4])
		b.putInt32(int32(len(s)))
		b.write(s)

	case types.TimestampTZFamily:
		// Start at offset 4 because `putInt32` clobbers the first 4 bytes.
		s := formatTs(vec.Timestamp().Get(idx), conv.Location, b.putbuf[4:4])
		b.putInt32(int32(len(s)))
		b.write(s)
	...
	case types.JsonFamily:
		b.writeLengthPrefixedString(vec.Datum().Get(idx).(*coldataext.Datum).Datum.(*tree.DJSON).JSON.String())
	...
	case types.EnumFamily:
		// Enums are serialized with their logical representation.
		b.writeLengthPrefixedString(vec.Datum().Get(idx).(*coldataext.Datum).Datum.(*tree.DEnum).LogicalRep)

	default:
		b.setError(errors.Errorf("unsupported type %s", t))
	}
}
...
// that have various width encodings (floats, ints, chars). It is ignored
// (and can be nil) for types with a 1:1 datum:type mapping.
func (b *writeBuffer) writeBinaryPhysicalElement(
	ctx context.Context, vec coldata.Vec, idx int, sessionLoc *time.Location, t *types.T,
) {
	if log.V(2) {
		log.Infof(ctx, "pgwire writing BINARY physical element of type: %s", t)
	}
	if vec.MaybeHasNulls() && vec.Nulls().NullAt(idx) {
		// NULL is encoded as -1; all other values have a length prefix.
		b.putInt32(-1)
	...
	switch t.Family() {
	case types.BitFamily:
		v := vec.Datum().Get(idx).(*coldataext.Datum).Datum.(*tree.DBitArray)
		words, lastBitsUsed := v.EncodingParts()
		if len(words) == 0 {
			b.putInt32(4)
		} else {
	...
	case types.IntFamily:
		...
		case oid.T_int2:
			b.putInt32(2)
			b.putInt16(vec.Int16().Get(idx))
		case oid.T_int4:
			b.putInt32(4)
			b.putInt32(vec.Int32().Get(idx))
		case oid.T_int8:
			b.putInt32(8)
			b.putInt64(vec.Int64().Get(idx))
		default:
			b.setError(errors.Errorf("unsupported int oid: %v", t.Oid()))
		}

	case types.FloatFamily:
		f := vec.Float64().Get(idx)
		switch t.Oid() {
		case oid.T_float4:
			b.putInt32(4)
			b.putInt32(int32(math.Float32bits(float32(f))))
		case oid.T_float8:
			b.putInt32(8)
			b.putInt64(int64(math.Float64bits(f)))
		default:
			b.setError(errors.Errorf("unsupported float oid: %v", t.Oid()))
		}
	...
	case types.INetFamily:
		...
		//  The IP bytes.
		const pgIPAddrBinaryHeaderSize = 4
		v := vec.Datum().Get(idx).(*coldataext.Datum).Datum.(*tree.DIPAddr)
		if v.Family == ipaddr.IPv4family {
			b.putInt32(net.IPv4len + pgIPAddrBinaryHeaderSize)
			b.writeByte(pgwirebase.PGBinaryIPv4family)
			b.writeByte(v.Mask)
	...
	case types.DateFamily:
		b.putInt32(4)
		b.putInt32(pgdate.MakeCompatibleDateFromDisk(vec.Int64().Get(idx)).PGEpochDays())

	case types.TimeFamily:
		v := vec.Datum().Get(idx).(*coldataext.Datum).Datum.(*tree.DTime)
		b.putInt32(8)
		b.putInt64(int64(*v))

	case types.TimeTZFamily:
		v := vec.Datum().Get(idx).(*coldataext.Datum).Datum.(*tree.DTimeTZ)
		b.putInt32(12)
		b.putInt64(int64(v.TimeOfDay))
		b.putInt32(v.OffsetSecs)

	case types.IntervalFamily:
		v := vec.Interval().Get(idx)
		b.putInt32(16)
		b.putInt64(v.Nanos() / int64(time.Microsecond/time.Nanosecond))
		b.putInt32(int32(v.Days))
		b.putInt32(int32(v.Months))
	...
	case types.TupleFamily:
		v := vec.Datum().Get(idx).(*coldataext.Datum).Datum.(*tree.DTuple)
		// TODO(andrei): We shouldn't be allocating a new buffer for every array.
		subWriter := newWriteBuffer(nil /* bytecount */)
		// Put the number of datums.
		subWriter.putInt32(int32(len(v.D)))
	...
	case types.GeographyFamily:
		v := vec.Datum().Get(idx).(*coldataext.Datum).Datum.(*tree.DGeography)
		b.putInt32(int32(len(v.EWKB())))
		b.write(v.EWKB())

	case types.GeometryFamily:
		v := vec.Datum().Get(idx).(*coldataext.Datum).Datum.(*tree.DGeometry)
		b.putInt32(int32(len(v.EWKB())))
		b.write(v.EWKB())

	case types.ArrayFamily:
		v := vec.Datum().Get(idx).(*coldataext.Datum).Datum.(*tree.DArray)
		if v.ParamTyp.Family() == types.ArrayFamily {
			b.setError(unimplemented.NewWithIssueDetail(32552,
				"binenc", "unsupported binary serialization of multidimensional arrays"))
			return
		...
		b.writeLengthPrefixedBuffer(&subWriter.wrapped)
	case types.JsonFamily:
		s := vec.Datum().Get(idx).(*coldataext.Datum).Datum.(*tree.DJSON).JSON.String()
		b.putInt32(int32(len(s) + 1))
		// Postgres version number, as of writing, `1` is the only valid value.
		b.writeByte(1)
		b.writeString(s)
	case types.OidFamily:
		b.putInt32(4)
		b.putInt32(int32(vec.Int64().Get(idx)))
	default:
		b.setError(errors.AssertionFailedf("unsupported type %s", t))
	}
}
