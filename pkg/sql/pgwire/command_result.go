...
// RowsAffected is part of the sql.RestrictedCommandResult interface.
// Unlike commandResult.RowsAffected(), we don't assert the result is not
// released here. It's because we can reach here when executing the cleanup
// steps for a pausable portal after Sync is reached in an implicit transaction.
// In this case, we would have called res.Close because the execution of the
// previous pgwire command was done and we haven't created a new result.
func (r *limitedCommandResult) RowsAffected() int {
	return r.rowsAffected
}
