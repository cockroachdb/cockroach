...
func TestHbaEnhancedMapper(t *testing.T) {
	...
	t.Run("NoMapOption_UsesProvidedIdentities", func(t *testing.T) {
		...
		assert.Equal(t, "user1", mappings[0].SystemIdentity)
		assert.Equal(t, "user1", mappings[0].MappedUser.Normalized())
		assert.Equal(t, "user2", mappings[1].SystemIdentity)
		assert.Equal(t, "user2", mappings[1].MappedUser.Normalized())
	})
...
`
		...
		assert.NoError(t, err)
		assert.Len(t, mappings, 2)
		assert.Equal(t, "alice@example.com", mappings[0].SystemIdentity)
		assert.Equal(t, "alice", mappings[0].MappedUser.Normalized())
		assert.Equal(t, "bob@example.com", mappings[1].SystemIdentity)
		assert.Equal(t, "bob", mappings[1].MappedUser.Normalized())
	})
...
`
		...
		assert.Error(t, err)
		assert.Contains(t, err.Error(), "system identity \"admin@example.com\" mapped to reserved database role \"root\"")
...
`
		...
		assert.Error(t, err)
		assert.Contains(t, err.Error(), "system identity \"system@example.com\" mapped to reserved database role \"node\"")
		assert.Nil(t, mappings)
	})
