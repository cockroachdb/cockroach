...
func (c *conn) handleAuthentication(
	...
	if tlsConn, ok := c.conn.(*tls.Conn); ok {
		...
		if auth == nil {
			...
			if methodFn == nil {
				return sendError(errors.Errorf("no hba.conf entry for host %q, user %q", addr, c.sessionArgs.User))
			}
		}
...
var connAuthConf = settings.RegisterValidatedStringSetting(
	...
	"host-based authentication configuration file to use during connection authentication",
	"",
