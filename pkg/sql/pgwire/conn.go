...
func makeConn(netConn net.Conn, metrics *ServerMetrics) conn {
	wb := newWriteBuffer()
...
func (c *conn) setupSession(ctx context.Context, reserved mon.BoundAccount) {
	// WIP(andrei)
...
func (c *conn) serve(
	ctx context.Context, draining func() bool, reserved mon.BoundAccount,
) error {
...
func (c *conn) handleSimpleQuery(ctx context.Context, buf *pgwirebase.ReadBuffer) error {
	...
	for _, stmt := range sl {
		...
		if cp, ok := stmt.(*tree.CopyFrom); ok {
			...
			copyDone.Add(1)
			c.stmtBuf.Push(ctx, sql.CopyIn{Stmt: cp, CopyDone: &copyDone})
			copyDone.Wait()
...
func (c *conn) handleParse(ctx context.Context, buf *pgwirebase.ReadBuffer) error {
	...
	if cp, ok := stmt.(*tree.CopyFrom); ok {
		copyDone := sync.WaitGroup{}
		copyDone.Add(1)
		c.stmtBuf.Push(ctx, sql.CopyIn{Stmt: cp, Conn: c, CopyDone: &copyDone})
		copyDone.Wait()
		// The copyMachine always exits the extended protocol mode, if we were in
		// it.
		c.doingExtendedQueryMessage = false
...
// func (c *v3Conn2) flush(forceSend bool) error {
//   state := &c.streamingState
//   if state.buf.Len() == 0 {
//     return nil
//   }
