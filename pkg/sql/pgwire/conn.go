...
// connExecutor. A network writer goroutine is created to write the results
// produced by the connExecutor.
//
// There's three goroutines involved in servicing this network connection:
// - The network reader goroutine (this one on which serverConn is called).
// - The network writer goroutine.
// - The command processing goroutine (on which the connExecutor runs).
//
// The lifetimes of these goroutines are stacked: the reader outlives the writer
// which outlives the command processing.
// Each of the three actors can dismantle the whole troika. Here's how the
...
// actions will prompt the command processor to finish, which in turn will
// prompt the writer to finish.
// 2) The reader gets a read error from the network connection: like above, the
// reader closes the command processor which closes the writer.
...
// context and, like above, closes the processor which closes the writer.
// 4) The writer encounters a network error: the writer does two things:
//    a) It cancels the reader's context, which, as descibed above, closes the
//    processor and the writer.
//    b) It stores the error and opportunistically returns it to the processor
//    through the clientComm interface - some methods on the interface can
//    return errors. This is not strictly necessary, but the alternative seems
//    iffy - swallowing future calls on clientComm. The processor will handle
//    this error as descibed below; the effect will be that the processor will
//    terminate and notify the writer.
// 5) The processor encouters an error. This error can come from various fatal
// conditions encoutered internally by the processor, or from an error
// originating in the writer (the case above) and returned to the processor
// through clientComm. The processor will terminate and its goroutine will
// signal the writer on the terminateConnC channel. Like above, the writer will
// cancel the reader's context and terminate.
// Note that query processing errors are different; they don't cause the
// termination of the connection.
//
// Draining notes:
//
// The reader notices that the server is draining by polling the
// draining() closure passed to serveConn. At that point, the reader delegates
// the responsibility of closing the connection to the statement processor: it
// will push a DrainRequest to the stmtBuf which signal the processor to quit
// ASAP. The processor will quit immediately upon seeing that command if it's
// not currently in a transaction. If it is in a transaction, it will wait
// until the first time a Sync command is processed outside of a transaction -
// the logic being that we want to stop when we're both outside transactions and
// outside batches.
func serveConn(
	ctx context.Context,
	netConn net.Conn,
...
func (c *conn) serveImpl(
	...
	c.conn = newReadTimeoutConn(c.conn, func() error {
		...
		// time.
		if draining() {
			c.stmtBuf.Push(ctx, sql.DrainRequest{})
		}
		return nil
	})
...
Loop:
	for {
		...
		case pgwirebase.ClientMsgCopyData, pgwirebase.ClientMsgCopyDone, pgwirebase.ClientMsgCopyFail:
			...
			//
			// TODO(andrei): implement this draining in copyMachine.
...
func (c *conn) handleParse(ctx context.Context, buf *pgwirebase.ReadBuffer) error {
	...
	// WIP(andrei): This is broken - when the client is using the extended
	// protocol for copy, we'll also get bind, execute and maybe describe
	// messages. The copy machine should learn to ignore these. This means that
	// we'll only support preapring a COPY FROM statement if it is immediately
	// followed by binding and execution. That seems fine. Also note that COPY
	// FROM in extended mode seems to be quite broken in Postgres too:
...
type resultRowMsg struct {
	...
	// pos is the position of the statement to which the current message belongs.
	// Used by the writer to keep track of the highest command for which results
	// were flushed to the network.
	pos sql.CmdPos
}

// rowDescriptionMsg represents a ServerMsgRowDescription or ServerMsgNoData
// message.
type rowDescriptionMsg struct {
	// cols can be nil, in which case this will translate into a ServerMsgNoData.
...
type paramDescriptionMsg struct {
	types []oid.Oid
	pos   sql.CmdPos
}

type syncMsg struct {
	// If typ == readyForQuery, this is the txn status to be reported.
	txnStatus sql.TransactionStatusIndicator
	// pos is the position of the statement to which the current message belongs.
	// Used by the writer to keep track of the highest command for which results
	// were flushed to the network.
	pos sql.CmdPos
...
type cmdCompleteMsg struct {
	// tag is the tag to be written in the CommandComplete message.
	tag string
...
func (c *conn) bufferRow(
	...
	if err := c.msgBuilder.finishMsg(&c.writerState.buf); err != nil {
		panic(fmt.Sprint("unexpected err from buffer: %s", err))
	}
}
...
// CreateParseResult is part of the sql.ClientComm interface.
func (c *conn) CreateParseResult(pos sql.CmdPos) sql.ParseResult {
...
func (c *conn) handleAuthentication(ctx context.Context, insecure bool) error {
	...
	sendError := func(err error) error {
		writeErr(err, c.msgBuilder, c.conn)
		return err
	}
