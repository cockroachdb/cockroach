...
func (c *conn) bufferBatch(
	...
	n := batch.Length()
	if n > 0 {
		v := coldata.NewTypedVecs()
		defer v.Release()
		v.SetBatch(batch)
		width := int16(len(v.Vecs))
		for i := 0; i < n; i++ {
			rowIdx := i
			if sel != nil {
				rowIdx = sel[rowIdx]
			...
			for vecIdx := 0; vecIdx < len(v.Vecs); vecIdx++ {
				fmtCode := pgwirebase.FormatText
				if formatCodes != nil {
				...
				switch fmtCode {
				case pgwirebase.FormatText:
					c.msgBuilder.writeTextColumnarElement(ctx, v, vecIdx, rowIdx, conv, sessionLoc)
				case pgwirebase.FormatBinary:
					c.msgBuilder.writeBinaryColumnarElement(ctx, v, vecIdx, rowIdx, sessionLoc)
				default:
					c.msgBuilder.setError(errors.Errorf("unsupported format code %s", fmtCode))
				}
			}
