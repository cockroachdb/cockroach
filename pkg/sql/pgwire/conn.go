...
func serveConn(
	...
	readingErr := c.serveImpl(ctx, draining, sqlServer, reserved, authOpt, stopper)
	return readingErr
}
...
func (c *conn) serveImpl(
	...
	// We'll tie the writer-side (the provider of the password) to our context's
	// cancelation.
	authPipe := newAuthPipe(c, ctx.Done() /* writerDone */)
	...
	// connExecutor). Until then, we unfortunately still need some intSizer
	...
	var intSizer nakedIntSizer = fixedIntSizer{size: coltypes.Int8}
	var intSizerSet bool
	var refusePwdData bool
Loop:
	for {
		...
		// If we haven't yet gotten a nakedIntSizer from the authenticator
		// goroutine, see if one is available now.
		if !intSizerSet {
			if is := authenticator.authComplete(); is != nil {
				intSizer = is
				intSizerSet = true
			}
		}
		if typ != pgwirebase.ClientMsgPassword {
			// From now on we're not going to accept more password data, since we're
			// about to queue up other commands on the stmtBuf.
			refusePwdData = true
		}
		...
		case pgwirebase.ClientMsgSimpleQuery:
			...
			if err = c.handleSimpleQuery(
				ctx, &c.readBuf, timeReceived, intSizer.GetNakedIntSize(),
			); err != nil {
				break
			}
		...
		case pgwirebase.ClientMsgParse:
			doingExtendedQueryMessage = true
			err = c.handleParse(ctx, &c.readBuf, intSizer.GetNakedIntSize())
	...
	procErr := <-procCh
	// If we got an error, we'll return it. Except if the error we got seems to
	// be caused by us canceling the processor's ctx, in which case we swallow
	// the error.
	if errors.Cause(procErr) != ctx.Err() {
		err = procErr
	}
	...
	if draining() {
		...
		// Swallow whatever error we might have gotten. If we're draining, it's
		// probably a canceled context error.
		return nil
	}
	return err
}

// nakedIntSizer is used by a conn to get the SQL session's current int size
// setting.
//
// It's a restriction on the ConnectionHandler type.
type nakedIntSizer interface {
	// GetNakedIntSize returns the size that the parser should consider for an
	// unqualified INT.
	GetNakedIntSize() *coltypes.TInt
}
...
func (f fixedIntSizer) GetNakedIntSize() *coltypes.TInt {
	return f.size
}
		...
		defer func() {
			...
			if pgwireKnobs != nil && pgwireKnobs.CatchPanics {
				if r := recover(); r != nil {
					...
					retErr = pge
					_ = writeErr(retErr, &c.msgBuilder, &c.writerState.buf)
					_ /* n */, _ /* err */ = c.writerState.buf.WriteTo(c.conn)
					c.stmtBuf.Close()
			...
			// Inform the connection goroutine of either success or failure.
			if retErr != nil {
				retCh <- retErr
			} else {
				close(retCh)
...
type authenticator interface {
	// sendPwdData is used to push password data into the authenticator. This call is
	// blocking; authenticators are supposed to consume data hastily once they've
	// requested it.
	sendPwdData(data []byte) error
	// authComplete is used to query whether authentication succeeded. If it did,
	// a non-nil nakedIntSizer is returned. Otherwise (if authentication failed or
	// is still in progress) nil is returned. If authentication fails, this
	// interface will not carry that information. Instead, there's a separate
	// channel by which the goroutine performing authentication will return the
	// error to the goroutine processing the network connection.
	authComplete() nakedIntSizer
}
...
type AuthConn interface {
	...
	// nakedIntSizer, to be returned by authenticator.authComplete().
	AuthOK(nakedIntSizer)
	// AuthFail declares that authentication has failed. Future
...
type authPipe struct {
	...
	mu struct {
		...
		// intSizer is set once authentication is completed successfully.
		intSizer nakedIntSizer
	}
}

func newAuthPipe(c *conn, writerDone <-chan struct{}) *authPipe {
	ap := &authPipe{
		c:          c,
		ch:         make(chan []byte),
...
func (p *authPipe) AuthOK(intSizer nakedIntSizer) {
	p.mu.Lock()
	p.mu.intSizer = intSizer
	p.mu.Unlock()
	close(p.readerDone)
}

func (p *authPipe) AuthFail() {
	close(p.readerDone)
}

// authComplete is part of the authenticator interface.
func (p *authPipe) authComplete() nakedIntSizer {
	p.mu.Lock()
	defer p.mu.Unlock()
	return p.mu.intSizer
}
