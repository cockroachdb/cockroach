...
func (c *conn) serveImpl(
	...
	// by a session variable, and this layer doesn't have access to a session
	// variable. The callback below is called whenever default_int_size changes.
	// It happens in a different goroutine, so it has to be changed atomically.
	var unqualifiedIntSize = new(int32)
	onDefaultIntSizeChange := func(newSize int32) {
		atomic.StoreInt32(unqualifiedIntSize, newSize)
	}
		...
		breakLoop, err := func() (bool, error) {
			...
			case pgwirebase.ClientMsgSimpleQuery:
				if err = c.handleSimpleQuery(
					ctx, &c.readBuf, timeReceived, parser.NakedIntTypeFromDefaultIntSize(atomic.LoadInt32(unqualifiedIntSize)),
				); err != nil {
					return false, err
				}
			...
			case pgwirebase.ClientMsgParse:
				return false, c.handleParse(ctx, &c.readBuf, parser.NakedIntTypeFromDefaultIntSize(atomic.LoadInt32(unqualifiedIntSize)))
