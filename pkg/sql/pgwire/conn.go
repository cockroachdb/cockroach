...
Loop:
	for {
		...
		// If we haven't yet gotten an unqualified IntSizer from the authenticator
		...
		case pgwirebase.ClientMsgPassword:
			if refusePwdData {
				...
				_ /* err */ = writeErr(
					ctx, &sqlServer.GetExecutorConfig().Settings.SV, err, &c.msgBuilder, &c.writerState.buf)
				break Loop
			}
			var pwd []byte
...
type authenticator interface {
	...
	authComplete() unqualifiedIntSizer
}
...
func (p *authPipe) authComplete() unqualifiedIntSizer {
	p.mu.Lock()
	defer p.mu.Unlock()
