...
func (c *conn) handleSimpleQuery(
	...
	stmts, err := c.parser.ParseWithOptions(query, parser.DefaultParseOptions.WithIntType(unqualifiedIntSize))
	if err != nil {
...
func (c *conn) handleParse(ctx context.Context, nakedIntSize *types.T) error {
	...
	startParse := crtime.NowMono()
	stmts, err := c.parser.ParseWithOptions(query, parser.DefaultParseOptions.WithIntType(nakedIntSize))
	if err != nil {
