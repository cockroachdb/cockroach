...
// implemented by Postgres v7.4 and later). conn.serve() reads protocol messages,
// transforms them into commands that it pushes onto a StmtBuf (whence they'll
// be picked up and executed by the connExecutor. Results are then produced and
// sent through the conn back to the client.
...
// The method returns when the pgwire termination message is received or when
...
// it is consumed.
//
// WIP(andrei): rename when pgwireReader goes away.
