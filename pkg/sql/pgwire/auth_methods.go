...
func authJwtToken(
	...
	b.SetAuthenticator(func(
		...
		// If groups is an empty slice, revoke any existing role grants and fail the login
		// (behaviour matches ldapsearchfilter).
		if len(groups) == 0 {
			// groups list is explicitly empty -> drop all memberships
			if err := sql.EnsureUserOnlyBelongsToRoles(ctx, execCfg, user, nil); err != nil {
				c.LogAuthFailed(ctx, eventpb.AuthFailReason_AUTHORIZATION_ERROR, err)
				return err
			}
			// fail the login
