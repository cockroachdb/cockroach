...
type AuthBehaviors struct {
	...
	// ConnClose is an optional callback that should be invoked when the
	// incoming connection is shut down in order to allow any cleanup of
	// "ambient" authentication services, e.g.: GSSAPI.
	ConnClose func()

	// ReplaceSystemIdentity is an optional field that allows an
	// AuthMethod to replace the client-provided identity, based on
	// "ambient" authentication mechanisms, e.g.: GSSAPI.
	ReplaceSystemIdentity security.SQLUsername
}
...
func authPassword(
	...
	return AuthBehaviors{
		RoleMapper: UseProvidedIdentity,
		Authorizer: func(
			ctx context.Context,
			systemIdentity security.SQLUsername,
			clientConnection bool,
...
func authCert(
	...
	return AuthBehaviors{
		RoleMapper: HbaMapper(hbaEntry, identMap),
		Authorizer: func(
			ctx context.Context,
			systemIdentity security.SQLUsername,
			clientConnection bool,
...
func authTrust(
	...
	return AuthBehaviors{
		RoleMapper: UseProvidedIdentity,
		Authorizer: func(_ context.Context, _ security.SQLUsername, _ bool, _ PasswordRetrievalFn) error {
			return nil
		},
	}, nil
}
