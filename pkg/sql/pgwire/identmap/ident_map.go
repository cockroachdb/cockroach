...
// between system identities (e.g.: GSSAPI principals, X.509 Common
// Names) and database usernames.
package identmap
...
// A Conf provides a multi-level, user-configurable mapping
// between an external system identity (e.g.: GSSAPI principal or X.509
// Common Name) and zero or more database usernames which the external
// principal may act as.
//
...
func From(r io.Reader) (*Conf, error) {
	...
	for s.Scan() {
		...
		ret.data[mapName] = append(ret.data[mapName], elt)
	}
	return ret, nil
}
...
func (m *Conf) Map(mapName, systemIdentity string) ([]security.SQLUsername, error) {
	if m.data == nil {
		return nil, nil
	}
	x := m.data[mapName]
	...
	var ret []security.SQLUsername
	seen := make(map[string]bool)
	for i := range x {
		if n := x[i].substitute(systemIdentity); n != "" && !seen[n] {
