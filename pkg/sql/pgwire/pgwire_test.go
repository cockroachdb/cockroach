...
func TestPGWireDrainClient(t *testing.T) {
...

	{
		...
		t.Run("TestConnCancellationResponseFailure", func(t *testing.T) {
			txn, err := db.Begin()
			if err != nil {
				t.Fatal(err)
			}
			...
			// Make sure that the connection was disrupted. A retry loop is
			// needed because we must wait (since we told the pgServer not to)
			// until the connection registers the cancellation and closes
			// itself.
			testutils.SucceedsSoon(t, func() error {
		...
		t.Run("TestConnCancellationResponseSuccess", func(t *testing.T) {
			txn, err := db.Begin()
			if err != nil {
				t.Fatal(err)
			}
