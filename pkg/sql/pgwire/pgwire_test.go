...
func TestListenNotify(t *testing.T) {
	...
	testutils.RunTrueAndFalse(t, "sameconn", func(t *testing.T, sameconn bool) {
		...
		notif := <-ch
		assert.Equal(t, "foo", notif.Channel)
		assert.Equal(t, "", notif.Extra)
		...
		notif = <-ch
		assert.Equal(t, "foo", notif.Channel)
		assert.Equal(t, "bar", notif.Extra)
	})
	...
	t.Run("don't notify until commit", func(t *testing.T) {
		...
		mustSucceed(listenDb.Exec("SELECT 1"))
		assert.Equal(t, 0, len(listenerNotifChan),
	...
	t.Run("overflowing max pending on a channel drops messages", func(t *testing.T) {
		...
		testutils.SucceedsSoon(t,
			func() error {
				...
				if notifs["foo"] >= 10 {
					assert.Fail(t, "Expected notifications to be limited by max pending setting")
				}
				assert.Equal(t, 1, notifs["bar"])
				return nil
			})
	})
}
