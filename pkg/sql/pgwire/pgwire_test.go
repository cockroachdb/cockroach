...
// TestPGWireDrainCancelOngoingTxns tests that connections with open
// transactions are cancelled when they go on for too long.
func TestPGWireDrainCancelOngoingTxns(t *testing.T) {
	defer leaktest.AfterTest(t)()
	params := base.TestServerArgs{Insecure: true}
	s, _, _ := serverutils.StartServer(t, params)
	defer s.Stopper().Stop()
