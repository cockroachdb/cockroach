...
func TestPGWireOverUnixSocket(t *testing.T) {
	...
	//
	// See https://github.com/golang/go/blob/go1.8/src/os/file_unix.go#L266.
	const unixEnvKey = "TMPDIR"
	if tmpdir, ok := os.LookupEnv(unixEnvKey); ok {
		defer func() {
			if err := os.Setenv(unixEnvKey, tmpdir); err != nil {
				t.Fatal(err)
			}
		}()
		if err := os.Unsetenv(unixEnvKey); err != nil {
			t.Fatal(err)
		}
	}

	tempDir, err := ioutil.TempDir("", "cockroach-unix")
	if err != nil {
		t.Fatal(err)
	}
