...
func (c *conn) handleAuthentication(
	...
	// SAN identities are only populated in the authCert method.
	sanIdentities := behaviors.GetSANIdentities()
	// Delegate to the AuthMethod's MapRole to verify that the
	// client-provided username matches one of the mappings.
	matchedIdentity, err := c.checkClientUsernameMatchesMapping(ctx, ac, behaviors, systemIdentity, sanIdentities)
	if err != nil {
		log.Dev.Warningf(ctx, "unable to map incoming identity %q, or san identities %q to any database user: %+v", systemIdentity, sanIdentities, err)
...
//
// When no SAN identities are provided, the function falls back to MapRole with the
// single systemIdentity (e.g., certificate CN or GSSAPI principal).
//
...
func (c *conn) checkClientUsernameMatchesMapping(
	ctx context.Context, ac AuthConn, behaviors *AuthBehaviors, systemIdentity string, sanIdentities []string,
) (string, error) {
	// If we have multiple SAN identities, use the enhanced mapper
	if len(sanIdentities) > 0 {
		mappings, err := behaviors.EnhancedMapRole(ctx, sanIdentities)
		if err != nil {
			return "", err
		}
		if len(mappings) == 0 {
		...
		return "", errors.Newf(
			...
			c.sessionArgs.User, sanIdentities)
	}
