...
func (c *conn) handleAuthentication(
	...
	if behaviors.ReplaceSystemIdentity.Undefined() {
		systemIdentity = c.sessionArgs.User
	} else {
		ac.SetSystemIdentity(systemIdentity)
		systemIdentity = behaviors.ReplaceSystemIdentity
	}
	...
	if err := c.chooseDbRole(ctx, ac, behaviors.RoleMapper, systemIdentity); err != nil {
	...
	if err := behaviors.Authorizer(ctx, systemIdentity, true /* public */, pwDataFn); err != nil {
