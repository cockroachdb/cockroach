...
func (c *conn) handleAuthentication(
	...
	if err != nil {
		...
		ac.LogAuthFailed(ctx, "user retrieval error", err)
		return nil, sendError(err)
	}

	if !exists {
		ac.LogAuthFailed(ctx, "user does not exist", nil)
		return nil, sendError(errors.Errorf(security.ErrPasswordUserAuthFailed, c.sessionArgs.User))
	}

	if !canLogin {
		ac.LogAuthFailed(ctx, "user does not have privilege to log in", nil)
	...
	if connClose, err = authenticationHook(c.sessionArgs.User, true /* public */); err != nil {
		ac.LogAuthFailed(ctx, "authentication failed", err)
...
type AuthConn interface {
	...
	LogAuthFailed(ctx context.Context, reason string, err error)
...
func (p *authPipe) LogAuthFailed(ctx context.Context, reason string, detailedErr error) {
	if p.log {
		var errStr string
		if detailedErr != nil {
