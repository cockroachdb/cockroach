...
func (c *conn) populateLastLoginTime(
	...
	// We additionally validate if this update is being performed in a PCR origin
	// cluster before sending the estimated_last_login_time updates as currently
	// we treat system.users as a materialized view in readonly tenants for PCR
	// standbys and hence they are non-writable, issue:#150560
		...
		isPCRStandbyCluster := func(execCfg *sql.ExecutorConfig) bool {
			...
			err := execCfg.InternalDB.Txn(ctx, func(ctx context.Context, txn isql.Txn) error {
				...
				row, err := txn.QueryRow(
					...
					"SELECT status from [SHOW VIRTUAL CLUSTERS WITH REPLICATION STATUS] WHERE source_cluster_uri != NULL LIMIT 1",
				)
				if err != nil {
					return err
				}
			...
			if err != nil {
				log.Warningf(ctx, "failed to get PCR replication status for the cluster, error: %v", err)
			}
			return isPCRStandby
		}
