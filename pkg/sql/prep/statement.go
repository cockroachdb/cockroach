...
// Summary returns a string summarizing the custom and generic plan costs and
// full scan counts. The format for custom costs is:
//
//	average_custom_cost [num_custom_costs]{custom_cost_0:full_scan_count ...}
//
// The format for generic costs is: generic_cost:generic_full_scan_count.
//
// A full example:
//
...
func (p *planCosts) Summary() string {
	...
	if p.custom.length > 0 {
		sb.WriteString(fmt.Sprintf("%f [%d]{", p.avgCustom().C, p.custom.length))
		for i := 0; i < p.custom.length; i++ {
			if i == 0 {
			} else {
				sb.WriteByte(' ')
			}
			sb.WriteString(fmt.Sprintf("%f:%d", p.custom.costs[i].C, p.custom.costs[i].FullScanCount()))
		}
		sb.WriteByte('}')
	} else {
		sb.WriteString("none")
	...
	if p.HasGeneric() {
		sb.WriteString(fmt.Sprintf("%f:%d", p.generic.C, p.generic.FullScanCount()))
	} else {
		sb.WriteString("none")
	}
	return sb.String()
