...
func DestroyTenant(
	...
	return errors.Wrap(updateTenantRecord(ctx, execCfg, txn, info), "destroying tenant")
}
...
func (p *planner) DestroyTenant(ctx context.Context, tenID uint64) error {
	...
	return errors.Wrap(DestroyTenant(ctx, p.execCfg, p.txn, info), "destroying tenant")
}
