...
func (p *planner) DestroyTenant(
	ctx context.Context, tenantName roachpb.TenantName, syncronousImmediateDrop bool,
) error {
	...
	return destroyTenantInternal(ctx, p.txn, p.execCfg, &p.extendedEvalCtx, p.User(), info, syncronousImmediateDrop)
}

// DestroyTenantByID implements the tree.TenantOperator interface.
func (p *planner) DestroyTenantByID(
	ctx context.Context, tenID uint64, syncronousImmediateDrop bool,
) error {
	if err := p.validateDestroyTenant(ctx); err != nil {
		return err
	}
