...
func (p *planner) CreateTenant(ctx context.Context, tenID uint64) error {
...

	{
		...
		// to know what migrations need to be run in the future.
		v := clusterversion.ClusterVersion{
			Version: p.EvalContext().Settings.Version.BinaryVersion(),
		}
...
func generateTenantClusterSettingKV(
	...
	if err != nil {
		return roachpb.KeyValue{}, errors.Wrapf(err,
	...
	if err != nil {
		return roachpb.KeyValue{}, errors.Wrap(err,
			"failed to encode cluster setting")
	}
	if len(kvs) != 1 {
		return roachpb.KeyValue{}, errors.Errorf(
