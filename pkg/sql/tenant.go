...
func CreateTenantRecord(
	...
	// keyspace. Whatever's inserted will get cleared out by the tenant's
	...
	return execCfg.SpanConfigKVAccessor.UpdateSpanConfigEntriesWithTxn(
...
func GCTenantSync(ctx context.Context, execCfg *ExecutorConfig, info *descpb.TenantInfo) error {
	...
	err := execCfg.DB.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
		...
		scKVAccessor := execCfg.SpanConfigKVAccessor
		entries, err := scKVAccessor.GetSpanConfigEntriesForWithTxn(ctx, []roachpb.Span{tenantSpan}, txn)
		if err != nil {
			return err
		}
		...
		return scKVAccessor.UpdateSpanConfigEntriesWithTxn(ctx, toDelete, nil /* toUpsert */, txn)
	})
