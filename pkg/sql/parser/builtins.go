...
var Builtins = map[string][]Builtin{
	...
	"abbrev": {
		Builtin{
			...
			Info: "Converts the IP address to an abbreviated display format as text." +
				"For INET types, this will omit the mask length if it's not the default (32 or IPv4, 128 for IPv6)" +
	...
	"broadcast": {
		Builtin{
			...
			Info: "Gets the broadcast address for network represented by the value." +
	...
	"host": {
		Builtin{
			...
			Info: "Extracts the address part of value as text, omitting the mask length." +
	...
	"hostmask": {
		Builtin{
			...
			Info: "Creates an IP address retaining only the host mask part of the value." +
	...
	"masklen": {
		Builtin{
			...
			Info: "Gets the mask length of the value." +
				"\n\nFor example, `netmask('192.168.1.2/16')` returns `16`",
		},
	},
	...
	"netmask": {
		Builtin{
			...
			Info: "Creates an IP address retaining only the network mask part of the value." +
	...
	"set_masklen": {
		Builtin{
			...
			Info: "Sets the mask length of `val` to `mask`.\n\n" +
	...
	"text": {
		Builtin{
			...
			Info: "Converts the IP address and mask length to text.",
		},
	},
