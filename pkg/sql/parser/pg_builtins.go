...
var typeBuiltinsHaveUnderscore = map[oid.Oid]bool{
	TypeAny.Oid():         true,
	TypeDate.Oid():        true,
	TypeDecimal.Oid():     true,
	TypeInterval.Oid():    true,
	TypeTimestamp.Oid():   true,
	TypeTimestampTZ.Oid(): true,
	TypeTuple.Oid():       true,
}
...
func PGIOBuiltinPrefix(typ Type) string {
	...
	if typeBuiltinsHaveUnderscore[typ.Oid()] {
	...
	return map[string][]Builtin{
		...
		builtinPrefix + "recv": {
			Builtin{
				...
				ReturnType: fixedReturnType(typ),
				fn:         func(_ *EvalContext, _ Datums) (Datum, error) { return nil, errors.New("unimplemented") },
				category:   categoryCompatibility,
				Info:       notUsableInfo,
			},
