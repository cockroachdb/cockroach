...
func ParseOne(sql string) (Statement, error) {
	...
	if len(stmts) != 1 {
		return nil, pgerror.NewErrorf(
			pgerror.CodeSyntaxError, "expected 1 statement, but found %d", len(stmts))
	}
	return stmts[0], nil
}
...
func ParseTableName(sql string) (*TableName, error) {
	...
	if !ok {
		return nil, pgerror.NewErrorf(pgerror.CodeSyntaxError, "expected an ALTER TABLE statement, but found %T", stmt)
	}
...
func parseExprs(exprs []string) (Exprs, error) {
	...
	set, ok := stmt.(*Set)
	if !ok {
		return nil, pgerror.NewErrorf(pgerror.CodeSyntaxError, "expected a SET statement, but found %T", stmt)
	}
...
func ParseExpr(sql string) (Expr, error) {
	...
	if len(exprs) != 1 {
		return nil, pgerror.NewErrorf(pgerror.CodeSyntaxError, "expected 1 expression, found %d", len(exprs))
	}
	return exprs[0], nil
}
...
func ParseType(sql string) (CastTargetType, error) {
	...
	if !ok {
		return nil, pgerror.NewErrorf(pgerror.CodeSyntaxError, "expected a CastExpr, but found %T", expr)
	}
