...
var errBitLengthNotPositive = errpgcode.WithCandidateCode(
...
var errFloatPrecAtLeast1 = errpgcode.WithCandidateCode(
	errors.New("precision for type float must be at least 1 bit"), pgcode.InvalidParameterValue)
var errFloatPrecMax54 = errpgcode.WithCandidateCode(
...
func newDecimal(prec, scale int32) (*types.T, error) {
	if scale > prec {
		err := errpgcode.WithCandidateCode(
