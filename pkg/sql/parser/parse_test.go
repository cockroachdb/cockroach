...
func TestParse(t *testing.T) {
	testData := []struct {
		...
		{`EXPORT INTO CSV 's3://my/path/%part%.csv' WITH comma = '|' FROM TABLE a`},
		{`EXPORT INTO CSV 's3://my/path/%part%.csv' WITH comma = '|' FROM SELECT a, sum(b) FROM c WHERE d = 1 ORDER BY sum(b) DESC LIMIT 10`},
		{`SET ROW (1, true, NULL)`},
