...
func TestParse(t *testing.T) {
	testData := []struct {
		...
		{`COMPLETE JOBS SELECT a`},
		{`EXPLAIN COMPLETE JOBS SELECT a`},
		...
		{`PREPARE a AS COMPLETE JOBS SELECT 1`},
		{`PREPARE a (INT8) AS COMPLETE JOBS SELECT $1`},
...
$function$`,
		...
		{`COMPLETE JOB a`, `COMPLETE JOBS VALUES (a)`},
		{`EXPLAIN COMPLETE JOB a`, `EXPLAIN COMPLETE JOBS VALUES (a)`},
		...
		{
			`ALTER TABLE a SET REGIONAL AFFINITY AT ROW LEVEL AS bobby`,
		...
		{
			`ALTER TABLE a SET REGIONAL AFFINITY ROW LEVEL AS bobby`,
			`ALTER TABLE a SET LOCALITY REGIONAL BY ROW AS bobby`,
		},
		{
		...
		{
			`ALTER TABLE a SET ROW LEVEL REGIONAL AFFINITY AS bobby`,
			`ALTER TABLE a SET LOCALITY REGIONAL BY ROW AS bobby`,
		},
...
SKIP_MISSING_FOREIGN_KEYS, SKIP_MISSING_SEQUENCES, SKIP_MISSING_SEQUENCE_OWNERS, SKIP_MISSING_VIEWS`,
		...
		{`CREATE TABLE a (b INT) PARTITION ALL BY RANGE (b) (PARTITION p1 VALUES FROM (MINVALUE) TO (1), PARTITION p2 VALUES FROM (2, MAXVALUE) TO (4, 4), PARTITION p3 VALUES FROM (4, 4) TO (MAXVALUE))`,
			`CREATE TABLE a (b INT8) PARTITION ALL BY RANGE (b) (PARTITION p1 VALUES FROM (minvalue) TO (1), PARTITION p2 VALUES FROM (2, maxvalue) TO (4, 4), PARTITION p3 VALUES FROM (4, 4) TO (maxvalue))`},
