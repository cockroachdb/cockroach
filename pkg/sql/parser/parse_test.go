...
$function$`,
		...
		{`RESTORE foo, baz FROM 'bar' AS OF SYSTEM TIME '1'`,
			`RESTORE TABLE foo, baz FROM 'bar' AS OF SYSTEM TIME '1'`},
		{`BACKUP foo TO 'bar' WITH ENCRYPTION_PASSPHRASE = 'secret', revision_history, include_deprecated_interleaves`,
			`BACKUP TABLE foo TO 'bar' WITH revision_history, encryption_passphrase='secret', include_deprecated_interleaves`},
		{`BACKUP foo TO 'bar' WITH KMS = 'foo', revision_history, include_deprecated_interleaves`,
			`BACKUP TABLE foo TO 'bar' WITH revision_history, kms='foo', include_deprecated_interleaves`},
		{`BACKUP foo TO 'bar' WITH KMS = ('foo', 'bar'), revision_history, include_deprecated_interleaves`,
			`BACKUP TABLE foo TO 'bar' WITH revision_history, kms=('foo', 'bar'), include_deprecated_interleaves`},
		{`BACKUP foo TO 'bar' WITH OPTIONS (detached, ENCRYPTION_PASSPHRASE = 'secret', revision_history, include_deprecated_interleaves)`,
			`BACKUP TABLE foo TO 'bar' WITH revision_history, encryption_passphrase='secret', detached, include_deprecated_interleaves`},
		{`BACKUP foo TO 'bar' WITH OPTIONS (detached, KMS = ('foo', 'bar'), revision_history, include_deprecated_interleaves)`,
			`BACKUP TABLE foo TO 'bar' WITH revision_history, detached, kms=('foo', 'bar'), include_deprecated_interleaves`},
