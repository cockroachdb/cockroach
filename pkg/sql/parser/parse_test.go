...
func TestParse(t *testing.T) {
	testData := []struct {
		...
		{`CREATE TABLE a (a INT4) LOCALITY REGIONAL BY ROW ON bobby`},
...
$function$`,
		...
		{
			`ALTER TABLE a SET REGIONAL AFFINITY AT ROW LEVEL ON bobby`,
			`ALTER TABLE a SET LOCALITY REGIONAL BY ROW ON bobby`,
		},
		{
		...
		{
			`ALTER TABLE a SET REGIONAL AFFINITY ROW LEVEL ON bobby`,
			`ALTER TABLE a SET LOCALITY REGIONAL BY ROW ON bobby`,
		},
		{
		...
		{
			`ALTER TABLE a SET ROW LEVEL REGIONAL AFFINITY ON bobby`,
			`ALTER TABLE a SET LOCALITY REGIONAL BY ROW ON bobby`,
		},
