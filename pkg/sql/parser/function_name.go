...
func WrapFunction(n string) ResolvableFunctionReference {
	fd := Builtins[n]
	return ResolvableFunctionReference{&FunctionDefinition{Name: n, Definition: fd}}
}
...
func (n UnresolvedName) ResolveFunction(searchPath []string) (*FunctionDefinition, error) {
	...
	if d, ok := Builtins[fn.Function()]; ok && fn.Prefix() == "" {
		// Fast path: return early.
		return &FunctionDefinition{Name: fn.Function(), Definition: d}, nil
	}
	...
	if !ok {
		...
		if prefix == "" {
			...
			for _, alt := range searchPath {
				...
				if def, ok = Builtins[fullName]; ok {
					found = true
					break
				}
			}
	...
	return &FunctionDefinition{Name: fullName, Definition: def}, nil
}
