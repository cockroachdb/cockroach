...
type WindowFunc interface {
	...
	Compute(context.Context, *mon.MemoryAccount, *EvalContext, WindowFrame) (Datum, error)
}
...
func (w *aggregateWindowFunc) Compute(
	ctx context.Context, acc *mon.MemoryAccount, evalCtx *EvalContext, wf WindowFrame,
) (Datum, error) {
	...
	for i := 0; i < wf.PeerRowCount; i++ {
		if err := w.agg.Add(ctx, acc, evalCtx, wf.argsWithRowOffset(i)[0]); err != nil {
			return nil, err
		}
	}
...
func (rowNumberWindow) Compute(
	_ context.Context, _ *mon.MemoryAccount, _ *EvalContext, wf WindowFrame,
) (Datum, error) {
