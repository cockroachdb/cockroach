...
func TestJsonbOrdering(t *testing.T) {
...
func TestJsonbRoundTrip(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc, func(t *testing.T) {
			j, err := ParseDJsonb(tc)
			if err != nil {
				t.Fatal(err)
			}
			s := j.String()

			j2, err := ParseDJsonb(s)
			if err != nil {
...
func TestJsonbErrors(t *testing.T) {
	testCases := []struct {
		input string
		msg   string
	}{
...
func TestJsonbSize(t *testing.T) {
	testCases := []struct {
		input string
		size  uintptr
	}{
