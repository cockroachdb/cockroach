...
func LimitDecimalWidth(d *apd.Decimal, precision, scale int) error {
	...
	if scale > precision {
		return errors.Errorf("scale (%d) must be between 0 and precision (%d)", scale, precision)
	}
	...
	if _, err := c.Quantize(d, d, -int32(scale)); err != nil {
		...
		return errors.Errorf("value with precision %d, scale %d must round to an absolute value less than %s", precision, scale, lt)
	}
	return nil
}
