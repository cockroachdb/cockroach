...
func TestTypeCheckError(t *testing.T) {
	testData := []struct {
		...
		{`1 IN ('a', 'b')`, `unsupported comparison operator: 1 IN ('a', 'b'): unsupported comparison operator: <int> = <string>`},
		{`1 IN (1, 'a')`, `unsupported comparison operator: 1 IN (1, 'a'): unsupported comparison operator: <int> = <string>`},
