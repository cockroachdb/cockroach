...
func TestEvalError(t *testing.T) {
	testData := []struct {
		...
		{`'3 4:'::interval`,
			`could not parse '3 4:' as type interval: could not parse '4:' as type interval: time: unknown unit hm in duration 4hm`},
		{`'3 4'::interval`,
			`could not parse '3 4' as type interval: interval: unknown unit  in postgres duration 3 4`},
		{`ANNOTATE_TYPE('a', int)`,
