...
const (
	...
	timestampRFC3339NanoWithoutZoneFormat = dateFormat + "T15:04:05"
...
var loneZeroRMatch = regexp.MustCompile(`:0([^\d]|$)`)
var loneZeroLMatch = regexp.MustCompile(`([^\d])0:`)
...
func parseTimestampInLocation(s string, loc *time.Location, typ Type) (time.Time, error) {
	...
	// Thus, if we see `2015-10-05 3:0:50 +0:0:0` we need to change it to
	// `... 3:00:50 +00:00:00`.
	s = loneZeroRMatch.ReplaceAllString(s, ":00${1}")
	s = loneZeroLMatch.ReplaceAllString(s, "${1}00:")
