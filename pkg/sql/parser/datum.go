...
func (d *DArray) ResolvedType() Type {
	...
	default:
		panic(fmt.Sprintf("invalid type for DArray: %s", d.ParamTyp))
	}
}
...
// AppendString appends a string to the array, whose parameterized type must be
// TypeString.
func (d *DArray) AppendString(s string) error {
	if d.ParamTyp != TypeString {
		return errors.Errorf("cannot append string to a DArray containing %+v",
			d.ParamTyp)
	}
	d.Array = append(d.Array, NewDString(s))
	return nil
}
...
func (d *DArray) Append(v Datum) error {
	switch v := v.(type) {
	case *DString:
		return d.AppendString(string(*v))
	case *DInt:
		return d.AppendInt64(int64(*v))
	default:
		return errors.Errorf("cannot append %v (%T) to array", v, v)
	}
}
