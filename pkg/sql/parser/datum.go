...
func (d *DIPAddr) Prev() (Datum, bool) {
	// We will do one of the following to get the Prev IP address:
	...
	// Note that this function is undefined for ipAddr.IsMax() as per the
	// interface spec.
	if d.Family == ipaddr.IPv6Family && bytes.Equal(d.IP.GetBytes(), dIPv6min.GetBytes()) {
		if d.Mask == 0 {
			// Jump down IP family.
			return dMaxIPv4Addr, true
		}
		// Decrease mask size, wrap IPv6 IP value.
		return NewDIPAddr(DIPAddr{ipaddr.IPAddr{Family: ipaddr.IPv6Family, IP: dIPv6max, Mask: d.Mask - 1}}), true
	} else if d.Family == ipaddr.IPv4Family && bytes.Equal(d.IP.GetBytes(), dIPv4min.GetBytes()) {
		// Decrease mask size, wrap IPv4 IP value.
		return NewDIPAddr(DIPAddr{ipaddr.IPAddr{Family: ipaddr.IPv4Family, IP: dIPv4max, Mask: d.Mask - 1}}), true
	}
	// Decrement IP.
	return NewDIPAddr(DIPAddr{ipaddr.IPAddr{Family: d.Family, IP: d.IP.Sub(1), Mask: d.Mask}}), true
}

// Next implements the Datum interface.
func (d *DIPAddr) Next() (Datum, bool) {
	// We will do one of a few things to get the Next IPAddress:
	...
	// Note that this function is undefined for ipAddr.IsMax() as per the
	// interface spec.
	if d.Family == ipaddr.IPv4Family && bytes.Equal(d.IP.GetBytes(), dIPv4max.GetBytes()) {
		if d.Mask == 32 {
			// Jump up IP family.
			return dMinIPv6Addr, true
		}
		// Increase mask size, wrap IPv4 IP value.
		return NewDIPAddr(DIPAddr{ipaddr.IPAddr{Family: ipaddr.IPv4Family, IP: dIPv4min, Mask: d.Mask + 1}}), true
	} else if d.Family == ipaddr.IPv6Family && bytes.Equal(d.IP.GetBytes(), dIPv6max.GetBytes()) {
		// Increase mask size, wrap IPv6 IP value.
		return NewDIPAddr(DIPAddr{ipaddr.IPAddr{Family: ipaddr.IPv6Family, IP: dIPv6min, Mask: d.Mask + 1}}), true
	}
	// Increment IP.
	return NewDIPAddr(DIPAddr{ipaddr.IPAddr{Family: d.Family, IP: d.IP.Add(1), Mask: d.Mask}}), true
}
...
var dIPv4min = uint128.FromBytes([]byte(net.ParseIP("0.0.0.0")))
var dIPv4max = uint128.FromBytes([]byte(net.ParseIP("255.255.255.255")))
var dIPv6min = uint128.FromBytes([]byte(net.ParseIP("::")))
var dIPv6max = uint128.FromBytes([]byte(net.ParseIP("ffff:ffff:ffff:ffff:ffff:ffff:ffff:ffff")))

// dMaxIPv4Addr and dMinIPv6Addr are used as global constants to prevent extra
// heap extra allocation
var dMaxIPv4Addr = NewDIPAddr(DIPAddr{ipaddr.IPAddr{Family: ipaddr.IPv4Family, IP: dIPv4max, Mask: 32}})
var dMinIPv6Addr = NewDIPAddr(DIPAddr{ipaddr.IPAddr{Family: ipaddr.IPv6Family, IP: dIPv6min, Mask: 0}})

// dMinIPAddr and dMaxIPAddr are used as the DIPAddr global min and max.
var dMinIPAddr = NewDIPAddr(DIPAddr{ipaddr.IPAddr{Family: ipaddr.IPv4Family, IP: dIPv4min, Mask: 0}})
var dMaxIPAddr = NewDIPAddr(DIPAddr{ipaddr.IPAddr{Family: ipaddr.IPv6Family, IP: dIPv6max, Mask: 128}})
