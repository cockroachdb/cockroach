...
func ParseDIPNetFromINetString(s string) (*DIPNet, error) {
	ipNet, err := ipnet.ParseINet(s)
	if err != nil {
		return nil, err
	}
	return NewDIPNet(DIPNet{*ipNet}), nil
}

// TODO(joey): CIDR col
// func ParseDIPNetFromCIDRString(s string) (*DIPNet, error) {
// }
...
func (*DIPNet) ResolvedType() Type {
	// TODO(joey): CIDR col type. Do we add that here, do we have multiple types?
	return TypeINet
}
...
func (d *DIPNet) Prev() (Datum, bool) {
	return nil, false
	// Check for full mask, otherwise we won't define a Prev/Next
	// ones, size := d.IPNet.IPNet.Mask.Size()
	// if ones != size {
	// 	return nil, false
	// } else if d.IPNet.Family == ipnet.IPv4Family && d.IPNet.IPNet.IP.Equal(minIPv4) {
	// 	return nil, false
	// } else if d.IPNet.Family == ipnet.IPv6Family && d.IPNet.IPNet.IP.Equal(minIPv6) {
	// 	return nil, false
	// }
	//
	// // Decrement IP
	// var newIP []byte
	// copy(newIP, d.IPNet.IP)
	...
	// 	}
	// }
	// return NewDIPNet(DIPNet{ipnet.IPNet{Family: d.IPNet.Family, IPNet: net.IPNet{IP: newIP, Mask: d.IPNet.IPNet.Mask}}}), true
}
...
func (d *DIPNet) Next() (Datum, bool) {
	...
	// if ones != size {
	// 	return nil, false
	// } else if d.IPNet.Family == ipnet.IPv4Family && d.IPNet.IPNet.IP.Equal(maxIPv4) {
	// 	return nil, false
	// } else if d.IPNet.Family == ipnet.IPv6Family && d.IPNet.IPNet.IP.Equal(maxIPv6) {
	// 	return nil, false
	// }
	//
	// // Increment IP
...
var dMinIPNet = NewDIPNet(DIPNet{ipnet.IPNet{Family: ipnet.IPv4Family, IPNet: net.IPNet{IP: minIPv4, Mask: net.CIDRMask(1, 32)}}})
