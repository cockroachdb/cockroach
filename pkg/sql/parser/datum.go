...
func (d *DIPNet) Prev() (Datum, bool) {
	...
	if d.Family == ipnet.IPv6Family && d.IP.Equal(net.IPv6zero) {
		if ones == 0 {
			// Jump down IP family.
			return NewDIPNet(DIPNet{ipnet.IPNet{Family: ipnet.IPv4Family, IPNet: net.IPNet{IP: net.IPv4bcast, Mask: net.CIDRMask(32, 32)}}}), true
		}
	...
	} else if d.Family == ipnet.IPv4Family && d.IP.Equal(net.IPv4zero) {
		// Decrease mask size, wrap IPv4 IP value.
		return NewDIPNet(DIPNet{ipnet.IPNet{Family: ipnet.IPv4Family, IPNet: net.IPNet{IP: net.IPv4bcast, Mask: net.CIDRMask(ones-1, 32)}}}), true
	}
	// Decrement IP.
...
func (d *DIPNet) Next() (Datum, bool) {
	...
	ones, _ := d.IPNet.Mask.Size()
	if d.Family == ipnet.IPv4Family && d.IP.Equal(net.IPv4bcast) {
		if ones == 32 {
			// Jump up IP family.
			return NewDIPNet(DIPNet{ipnet.IPNet{Family: ipnet.IPv6Family, IPNet: net.IPNet{IP: net.IPv6zero, Mask: net.CIDRMask(0, 128)}}}), true
		}
		// Increase mask size, wrap IPv4 IP value.
		return NewDIPNet(DIPNet{ipnet.IPNet{Family: ipnet.IPv4Family, IPNet: net.IPNet{IP: net.IPv4zero, Mask: net.CIDRMask(ones+1, 32)}}}), true
	} else if d.Family == ipnet.IPv6Family && d.IP.Equal(maxIPv6) {
		// Increase mask size, wrap IPv6 IP value.
		return NewDIPNet(DIPNet{ipnet.IPNet{Family: ipnet.IPv6Family, IPNet: net.IPNet{IP: net.IPv6zero, Mask: net.CIDRMask(ones+1, 128)}}}), true
	}
	// Increment IP.
	newIP := append([]byte(nil), d.IP...)
	for i := len(newIP) - 1; i >= 0; i-- {
...
var dMinIPNet = NewDIPNet(DIPNet{ipnet.IPNet{Family: ipnet.IPv4Family, IPNet: net.IPNet{IP: net.IPv4zero, Mask: net.CIDRMask(0, 32)}}})
