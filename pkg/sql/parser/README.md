...
- do not continue parsing (the generated parser executes all actions
  in a big loop; a `return` will interrupt this loop);
- stop parsing with an indicator an error has occurred (the `helpWith`
  function returns non-zero);
- what `helpWith` does is extend the parsing error message with a help
