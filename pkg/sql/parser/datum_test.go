...
func TestParseDTimestampPQRegression(t *testing.T) {
	testData := []struct {
		str      string
		expected time.Time
	}{
	...
	for _, td := range testData {
		actual, err := ParseDTimestampTZ(td.str, time.UTC, time.Nanosecond)
		if err != nil {
			t.Errorf("unexpected error while parsing TIMESTAMPTZ %s: %s", td.str, err)
			continue
		}
