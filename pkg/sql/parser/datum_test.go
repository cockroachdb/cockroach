...
func TestDatumOrdering(t *testing.T) {
	testData := []struct {
		...
		{`current_schemas(false)`, `min`, `max`, `{}`, `{}`},

		{`array[NULL]`, `min`, `max`, `{NULL}`, `{NULL}`},

		{`array[true]`, `{false}`, `max`, `{false}`, `{true}`},
		{`array[false]`, `min`, `{true}`, `{false}`, `{true}`},
		...
		{`array[true, NULL, false]`, `{false,NULL,true}`, `{true,NULL,true}`,
			`{false,NULL,false}`, `{true,NULL,true}`},
		{`array[false, NULL, true]`, `{false,NULL,false}`, `{true,NULL,false}`,
			`{false,NULL,false}`, `{true,NULL,true}`},
		...
		// Multi-dim arrays.
		{`ARRAY[ARRAY[true], ARRAY[false]]`, `{{false},{true}}`, `{{true},{true}}`,
			`{{false},{false}}`, `{{true},{true}}`},
		{`ARRAY[ARRAY[false], ARRAY[true]]`, `{{false},{false}}`, `{{true},{false}}`,
			`{{false},{false}}`, `{{true},{true}}`},

		// Mixed tuple/array datums.
		{`row(ARRAY[true], row(false))`, `({false}, (true))`, `({true}, (true))`,
			`({false}, (false))`, `({true}, (true))`},
		{`row(row(false), ARRAY[true])`, `((false), {false})`, `((true), {false})`,
			`((false), {false})`, `((true), {true})`},
	}
