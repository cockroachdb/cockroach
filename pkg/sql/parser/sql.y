...
| REFERENCES table_name opt_name_parens key_match reference_actions
 {
    ...
    $$.val = &tree.ColumnFKConstraint{
      ...
      // A single column constraint cannot have a composite key match method
      // since both MATCH SIMPLE and MATCH FULL act the exact same manner on a
      // single column key. But postgres does allow you to specify it. So we
      // won't error here, but the value is just tossed out.
    }
 }
| AS '(' a_expr ')' STORED
 {
