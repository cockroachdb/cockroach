...
| with_clause PAUSE SCHEDULES FROM table_ref
  {
     $$.val = &tree.ControlAllSchedules{
       With: $1.with(),
       From: $5.tblExpr(),
       Command: tree.PauseSchedule,
     }
...
// %Category: Misc
// %Text:
// RESUME JOBS <selectclause> [WITH action JOBS]
// RESUME JOB <jobid> [WITH action JOBS]
//
// WITH action JOBS:
//   action: PAUSE|CANCEL
//   Apply specified action to any jobs that were created by the
//   selected schedule(s) and that are currently running.
