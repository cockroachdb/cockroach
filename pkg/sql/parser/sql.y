...
func (u *sqlSymUnion) nullOrder() tree.NullsOrder {
...
sortby:
  a_expr opt_asc_desc opt_nulls_order
  {
    /* FORCE DOC */
    dir := $2.dir()
    nullOrder := $3.nullOrder()
    // We currently only support the opposite of Postgres defaults.
    if nullOrder != tree.DefaultNullsOrder {
      if dir == tree.Descending && nullOrder == tree.NullsFirst {
        return unimplementedWithIssue(sqllex, 6224)
      }
      if dir != tree.Descending && nullOrder == tree.NullsLast {
        return unimplementedWithIssue(sqllex, 6224)
      }
    }
    $$.val = &tree.Order{
