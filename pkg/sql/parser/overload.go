...
func typeCheckOverloadedExprs(
	...
	s.constExprs, s.placeholderExprs, s.resolvableExprs = typeCheckSplitExprs(ctx, exprs)

	// If no overloads are provided, just type check parameters and return.
	if len(overloads) == 0 {
		for _, i := range s.resolvableExprs {
	...
	if len(s.resolvableExprs) > 0 {
		homogeneousTyp = s.typedExprs[s.resolvableExprs[0]].ResolvedType()
		for _, i := range s.resolvableExprs[1:] {
	...
	if len(s.constExprs) > 0 {
		...
		if bestConstType, ok = commonConstantType(s.exprs, s.constExprs); ok {
			for _, i := range s.constExprs {
				s.overloadIdxs = filterOverloads(s.overloads, s.overloadIdxs,
					func(o overloadImpl) bool {
	...
	if homogeneousTyp != nil && len(s.placeholderExprs) > 0 {
		for _, i := range s.placeholderExprs {
			s.overloadIdxs = filterOverloads(s.overloads, s.overloadIdxs,
				func(o overloadImpl) bool {
					return o.params().getAt(i).Equivalent(homogeneousTyp)
				})
