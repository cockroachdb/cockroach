...
func typeCheckOverloadedExprs(
	...
	filterAttempt := func(attempt func()) (bool, []overloadImpl, error) {
		before := overloads
		attempt()
		if len(overloads) == 1 {
			_, fns, err := checkReturn()
			return true, fns, err
		}
		overloads = before
		return false, nil, nil
	...
	if len(constExprs) > 0 {
		if ok, fns, err := filterAttempt(func() {
	...
	return typedExprs, overloads, nil
}
