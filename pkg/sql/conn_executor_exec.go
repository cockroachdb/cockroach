...
func (ex *connExecutor) dispatchToExecutionEngine(
	...
	if limitsErr := ex.handleTxnRowsWrittenReadLimits(ctx); limitsErr != nil && err == nil {
		err = limitsErr
	}
...
func (ex *connExecutor) handleTxnRowsWrittenReadLimits(ctx context.Context) error {
	...
	if shouldLogRowsWritten || shouldErrRowsWritten {
		...
		if shouldLogRowsWritten {
			...
			if ex.executorType == executorTypeInternal {
				event = &eventpb.TxnRowsWrittenLimitReachedInternal{CommonTxnRowsLimitReachedDetails: info}
			} else {
				event = &eventpb.TxnRowsWrittenLimitReached{CommonTxnRowsLimitReachedDetails: info}
			}
	...
	if shouldLogRowsRead || shouldErrRowsRead {
		...
		if shouldLogRowsRead {
			var event eventpb.EventPayload
			if ex.executorType == executorTypeInternal {
				event = &eventpb.TxnRowsReadLimitReachedInternal{CommonTxnRowsLimitReachedDetails: info}
			} else {
				event = &eventpb.TxnRowsReadLimitReached{CommonTxnRowsLimitReachedDetails: info}
			}
			log.StructuredEvent(ctx, event)
		}
