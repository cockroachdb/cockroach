...
func (ex *connExecutor) dispatchToExecutionEngine(
	...
	defer func() {
		planner.maybeLogStatement(
			ctx,
			ex.executorType,
			int(atomic.LoadInt32(&ex.extraTxnState.autoRetryCounter)),
	...
	ex.sessionTracing.TraceRetryInformation(ctx, int(atomic.LoadInt32(&ex.extraTxnState.autoRetryCounter)), ex.extraTxnState.autoRetryReason)
	...
	ex.recordStatementSummary(
		ctx, planner,
		int(atomic.LoadInt32(&ex.extraTxnState.autoRetryCounter)), res.RowsAffected(), res.Err(), stats,
	)
...
func (ex *connExecutor) recordTransaction(
	...
	recordedTxnStats := sqlstats.RecordedTxnStats{
		...
		RetryCount:              int64(atomic.LoadInt32(&ex.extraTxnState.autoRetryCounter)),
