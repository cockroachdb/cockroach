...
func (ex *connExecutor) execStmtInOpenState(
	...
	var sp *tracing.Span
	if !isPausablePortal(portal) || !portal.pauseInfo.execStmtInOpenStateCleanup.isComplete {
	...
	defer func(ctx context.Context, res RestrictedCommandResult) {
		...
		processCleanupFunc(portal, "cancel query", func() {
			...
			if resToPushErr != nil && cancelQueryCtx.Err() != nil && !resToPushErr.IsReleased() && resToPushErr.Err() != nil {
				...
				resToPushErr.SetError(cancelchecker.QueryCanceledError)
	...
	if isPausablePortal(portal) {
		if portal.pauseInfo.ihWrapper == nil {
			...
			ih.CopyTo(&portal.pauseInfo.ihWrapper.ih)
		} else {
