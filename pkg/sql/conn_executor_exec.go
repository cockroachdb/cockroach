...
func (ex *connExecutor) execStmtInOpenState(
	...
	defer func(ctx context.Context, res RestrictedCommandResult) {
		...
		processCleanupFunc("cancel query", func() {
			...
			if resToPushErr != nil && cancelQueryCtx.Err() != nil && !resToPushErr.IsReleased() && resToPushErr.Err() != nil {
				...
				// error.
				if isPausablePortal && (errors.Is(resToPushErr.Err(), sqlerrors.QueryTimeoutError) || errors.Is(resToPushErr.Err(), sqlerrors.TxnTimeoutError)) {
	...
	if isPausablePortal {
		...
		p.cancelChecker.Reset(portal.pauseInfo.cancelQueryCtx)
	} else {
...
func (ex *connExecutor) dispatchToExecutionEngine(
	...
	defer func() {
		...
		if ppInfo := getPausablePortalInfo(); ppInfo != nil && !ppInfo.dispatchToExecEngCleanup.isComplete {
			ppInfo.dispatchToExecEngCleanup.appendFunc(namedFunc{
				...
				f: func() {
					var resErr error
					if !ppInfo.curRes.IsReleased() {
						resErr = ppInfo.curRes.Err()
					}
	...
	if ppInfo := getPausablePortalInfo(); ppInfo != nil && !ppInfo.dispatchToExecEngCleanup.isComplete {
		ppInfo.dispatchToExecEngCleanup.appendFunc(namedFunc{
			...
			f: func() {
				// TODO(janexing): properly set the instrumentation helper.
				planner.curPlan = ppInfo.planTop
				populateQueryLevelStatsAndRegions(ctx, planner, &ppInfo.ihWrapper.ih, ex.server.cfg, ppInfo.queryStats, &ex.cpuStatsCollector)
			},
		})
	} else {
		populateQueryLevelStatsAndRegions(ctx, planner, &planner.instrumentation, ex.server.cfg, &stats, &ex.cpuStatsCollector)
	}
