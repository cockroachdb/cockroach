...
func (ex *connExecutor) dispatchToExecutionEngine(
	...
	defer func() {
		planner.maybeLogStatement(
			ctx,
			ex.executorType,
			int(ex.extraTxnState.mu.atomicAutoRetryCounter),
	...
	ex.sessionTracing.TraceRetryInformation(ctx, int(ex.extraTxnState.mu.atomicAutoRetryCounter), ex.extraTxnState.mu.autoRetryReason)
	if ex.server.cfg.TestingKnobs.OnTxnRetry != nil && ex.extraTxnState.mu.autoRetryReason != nil {
		ex.server.cfg.TestingKnobs.OnTxnRetry(ex.extraTxnState.mu.autoRetryReason, planner.EvalContext())
	}
	...
	ex.recordStatementSummary(
		ctx, planner,
		int(ex.extraTxnState.mu.atomicAutoRetryCounter), res.RowsAffected(), res.Err(), stats,
	)
...
func (ex *connExecutor) onTxnRestart(ctx context.Context) {
	if ex.extraTxnState.shouldExecuteOnTxnRestart {
		...
		if ex.server.cfg.TestingKnobs.BeforeRestart != nil {
			ex.server.cfg.TestingKnobs.BeforeRestart(ctx, ex.extraTxnState.mu.autoRetryReason)
		}
	}
}
...
func (ex *connExecutor) recordTransactionFinish(
	...
	recordedTxnStats := sqlstats.RecordedTxnStats{
		...
		RetryCount:              int64(ex.extraTxnState.mu.atomicAutoRetryCounter),
