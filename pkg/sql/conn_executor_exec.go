...
func (ex *connExecutor) execStmtInOpenState(
	...
	defer func() {
		// Log the statement. We need to ensure the error state is the latest one.

		// Wait for the timer callbacks to complete to avoid a data race on
		// queryTimedOut and txnTimedOut.
		if queryTimeoutTicker != nil && !queryTimeoutTicker.Stop() {
			<-queryDoneAfterFunc
		}
		if txnTimeoutTicker != nil && !txnTimeoutTicker.Stop() {
			<-txnDoneAfterFunc
		}

		processCleanupFunc("set error and log statement", func() {
			cancelQueryCtx := ctx
			...
			// the plan here.
			resErr = resToPushErr.Err()
			if resErr == nil && !dispatchToExecEngine {
			...
			rowsAffected := nonBulkJobNumRows
			if isPausablePortal() {
				...
				resErr = ppInfo.curRes.ErrAllowReleased()
			}
