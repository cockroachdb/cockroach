...
func (ex *connExecutor) createJobs(ctx context.Context) error {
	...
	for _, record := range ex.extraTxnState.schemaChangeJobsCache {
		records = append(records, record)
	}
	jobsList, err := ex.server.cfg.JobRegistry.CreateJobsWithTxn(ctx, ex.planner.extendedEvalCtx.Txn, records)
	if err != nil {
		return err
	}
