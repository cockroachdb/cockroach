	...
	if shouldLog || shouldErr {
		...
		if shouldErr {
			if e, ok := event.(error); ok {
				...
				// This case should never be reached since all non-internal
				// events implement error interface and internal queries cannot
				// error out. Still, we choose to be safe, just in case.
				err = &commonTxnRowsLimitDetails
			}
