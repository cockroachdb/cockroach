...
// error is returned.
func (ex *connExecutor) maybeUpgradeToSerializable(ctx context.Context) error {
	p := &ex.planner
	...
	if tree.CanModifySchema(p.stmt.AST) {
...
func (ex *connExecutor) makeExecPlan(ctx context.Context, planner *planner) error {
	if err := ex.maybeUpgradeToSerializable(ctx); err != nil {
		return err
	}
