...
// be modified at type checking, with tree.StrVal component replaced
...
func (ex *connExecutor) maybeReparsePrepStmt(
	...
	log.Eventf(ctx, "reparsing prepared statement %q because user-defined types are stale: %s", name, staleEnumNames)
...
func (ex *connExecutor) execStmtInOpenStateWithPausablePortal(
	...
	if !portal.isPausable() || !portal.pauseInfo.execStmtInOpenState.cleanup.isComplete {
		...
		ex.state.mu.stmtCount++
		ex.state.mu.Unlock()
		ex.addActiveQuery(parserStmt, vars.stmt.NoSecret, pinfo, queryID, vars.cancelQuery)
	...
	if e, ok := vars.ast.(*tree.Execute); ok {
		// Replace the `EXECUTE foo` statement with the prepared statement, and
		// continue execution. The raw client text is captured first: the stmt
		// is about to be overwritten with the prepared statement, and the text
		// is needed to un-redact the active query below.
		origSQL := vars.stmt.SQL
		name := e.Name.String()
		ps, ok := ex.extraTxnState.prepStmtsNamespace.prepStmts.Get(name)
		if !ok {
			return makeErrEvent(newPreparedStmtDNEError(ex.sessionData(), name))
		...
		vars.stmt.NoSecret = ps.NoSecret
...
func (ex *connExecutor) onTxnRestart(ctx context.Context) {
	if ex.extraTxnState.shouldExecuteOnTxnRestart {
		...
		// (accumulatedStats, idleLatency) measure time and work a retry does
		// not re-produce — most notably the contention that caused the retry
		// — so they intentionally survive the restart and accumulate across
		// attempts, consistent with ServiceLatency and RetryCount. See the
		// field comments on extraTxnState.
