...
func (ex *connExecutor) execStmtInParallel(
	...
	if err := ex.parallelizeQueue.Add(params, func() error {
		...
		if shouldUseDistSQL {
			if distributePlan {
				planFlags.Set(planFlagDistributed)
			} else {
				planFlags.Set(planFlagDistSQLLocal)
			}
		...
		ex.recordStatementSummary(
			planner, stmt, planFlags, ex.extraTxnState.autoRetryCounter,
...
func (ex *connExecutor) dispatchToExecutionEngine(
	...
	planFlags, err := planner.optionallyUseOptimizer(ctx, ex.sessionData, stmt)
	if err == nil && !planFlags.IsSet(planFlagOptUsed) {
