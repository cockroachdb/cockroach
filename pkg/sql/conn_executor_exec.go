...
func (ex *connExecutor) execStmtInOpenState(
	...
	queryDone := func(ctx context.Context, res RestrictedCommandResult) {
		if timeoutTicker != nil {
			// We don't need cancelQuery to run before unregisterFn, but we do want
			// queryTimedOut to be set by the callback before reading it below, in
			// order to write the most appropriate error. For example, we don't want
			// to report a cancellation error if the query timed out and there was no
			// issued CANCEL QUERY.
	...
	if ex.sessionData.StmtTimeout > 0 {
		if _, ok := stmt.AST.(tree.HiddenFromShowQueries); !ok {
			timeoutTicker = time.AfterFunc(
				ex.sessionData.StmtTimeout-time.Since(ex.phaseTimes[sessionQueryReceived]),
				func() {
