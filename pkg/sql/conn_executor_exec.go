...
func (ex *connExecutor) makeExecPlan(ctx context.Context, planner *planner) error {
	...
	if optMode := ex.sessionData.OptimizerMode; optMode != sessiondata.OptimizerOff {
		...
		if isCorrelated {
			...
			// makeOptimizerPlan() can determine isCorrelated
			// but fail with a non-nil error and a nil result
			// -- for example, when it runs into an unsupported SQL feature
			// that the HP supports, after having processed
			// a correlated subquery (which the HP won't support).
