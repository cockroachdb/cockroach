...
func matchFullUnacceptableKeyQuery(
	...
	srcAllNullClause := make([]string, nCols)
	srcAllNotNullClause := make([]string, nCols)
	...
	for i := 0; i < nCols; i++ {
		...
		srcAllNullClause[i] = fmt.Sprintf("%s IS NULL", srcCols[i])
		srcAllNotNullClause[i] = fmt.Sprintf("%s IS NOT NULL", srcCols[i])
	}
	...
	return fmt.Sprintf(
		`SELECT %[1]s FROM [%[2]d AS tbl] WHERE NOT ((%[3]s) OR (%[4]s)) %[5]s`,
		strings.Join(returnedCols, ","),            // 1
		srcTbl.ID,                                  // 2
		strings.Join(srcAllNullClause, " AND "),    // 3
		strings.Join(srcAllNotNullClause, " AND "), // 4
		limit, // 5
	), returnedCols, nil
}
