...
func matchFullUnacceptableKeyQuery(
	srcTbl sqlbase.TableDescriptorInterface, fk *descpb.ForeignKeyConstraint, limitResults bool,
...
func nonMatchingRowQuery(
	srcTbl sqlbase.TableDescriptorInterface,
	fk *descpb.ForeignKeyConstraint,
	targetTbl sqlbase.TableDescriptorInterface,
	limitResults bool,
...
func validateForeignKey(
	...
	// TODO(ajwerner): Fix this up to use some other mechanism to access this
	// descriptor which deals with the right type and error or make catalogkv
	// specifically do what is desired.
	desc, err := catalogkv.GetDescriptorByID(ctx, txn, codec, fk.ReferencedTableID)
	if err != nil {
		return err
	}
	targetTable, ok := desc.(sqlbase.TableDescriptorInterface)
	if !ok {
