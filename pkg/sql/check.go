...
func matchFullUnacceptableKeyQuery(
	...
	if !asOf.Equal(hlc.MaxTimestamp) {
		asOfStr = fmt.Sprintf("AS OF SYSTEM TIME %s", asOf.AsOfSystemTime())
	}
	return fmt.Sprintf(
		`SELECT %[1]s FROM [%[2]d AS tbl]@[%[3]d] %[4]s WHERE NOT ((COALESCE(%[5]s) IS NULL) OR (%[6]s)) %[7]s`,
		strings.Join(returnedCols, ","),         // 1
		srcTbl.ID,                               // 2
		srcIdx.ID,                               // 3
		asOfStr,                                 // 4
		strings.Join(srcCols, ", "),             // 5
		strings.Join(srcNotNullClause, " AND "), // 6
		limit,                                   // 7
	), returnedCols, nil
}
...
func nonMatchingRowQuery(
	...
	for i, n := range colNames {
		srcCols[i] = fmt.Sprintf("%s", tree.NameString(n))
	...
	return fmt.Sprintf(
		`SELECT %[1]s FROM 
		  (SELECT %[2]s FROM [%[3]d AS src]@{FORCE_INDEX=[%[4]d],IGNORE_FOREIGN_KEYS} %[11]s WHERE %[5]s) AS s
			LEFT OUTER JOIN
			(SELECT * FROM [%[6]d AS target]@[%[7]d] %[11]s) AS t
			ON %[8]s
...
func validateForeignKey(
	...
	if prefix > 1 && srcIdx.ForeignKey.Match == sqlbase.ForeignKeyReference_FULL {
		query, colNames, err := matchFullUnacceptableKeyQuery(
			...
			true /* limitResults */, hlc.MaxTimestamp,
		)
		if err != nil {
			return err
		}
