...
// `indexIDToValidate`, if non-zero, is used to explicit hint the
...
func validateCheckExpr(
	...
	if indexIDToValidate != 0 {
		queryStr = fmt.Sprintf(`SELECT %s FROM [%d AS t]@[%d] WHERE NOT (%s) LIMIT 1`, columns, tableDesc.GetID(), indexIDToValidate, exprStr)
	}
