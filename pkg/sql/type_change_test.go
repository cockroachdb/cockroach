...
// - We drop 'a' in job 1, which is slow, and finishes after job 2.
// - We drop 'b' in job 2, which is fast, and finishes before job 1.
...
func TestEnumMemberTransitionIsolation(t *testing.T) {
	...
	go func() {
		_, err := sqlDB.Exec(`ALTER TYPE ab DROP VALUE 'a'`)
		if err != nil {
			t.Fatal(err)
		}
	...
	go func() {
		...
		_, err := sqlDB.Exec(`ALTER TYPE ab DROP VALUE 'b'`)
		if err == nil {
			t.Fatal("expected error, found nil")
		}
		if !testutils.IsError(err, "boom") {
