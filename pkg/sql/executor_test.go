...
func TestPrepareCanAcquireLeases(t *testing.T) {
	...
	_, err := db.Exec("CREATE DATABASE d;" +
	...
	// To setup the conditions for #14473, we need the prepare to not do any KV
	// ops before acquiring the lease. For that, we need the table name cache to
	// be populated. We'll do that by acquiring a lease on child.
