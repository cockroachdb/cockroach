...
func CreateUserDefinedArrayTypeDesc(
	...
	case descpb.TypeDescriptor_COMPOSITE:
		elemTyp = types.MakeLabeledTuple(typDesc.Composite.TupleContents, typDesc.Composite.TupleLabels)
	default:
...
func (p *planner) createUserDefinedType(params runParams, n *createTypeNode) error {
	...
	case tree.Composite:
		if !p.execCfg.Settings.Version.IsActive(params.ctx, clusterversion.UserDefinedCompositeTypes) {
			return pgerror.Newf(pgcode.FeatureNotSupported,
				"version %v must be finalized to create composite types",
				clusterversion.ByKey(clusterversion.UserDefinedCompositeTypes))
		}
...
func CreateCompositeTypeDesc(
	...
	labels := make([]string, len(compositeTypeList))
	types := make([]*types.T, len(compositeTypeList))
