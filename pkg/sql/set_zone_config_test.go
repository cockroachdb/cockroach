...
func TestValidateZoneAttrsAndLocalitiesForSecondaryTenants(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			errRe: `zone "us-east1" matches no existing nodes within the cluster`,
		},
		{
			cfg:   `voter_constraints: ["+region=us-east1-a"]`,
			errRe: `region "us-east1-a" matches no existing nodes within the cluster`,
		},
		{
			cfg:   `constraints: ["-zone=us-east2"]`,
			errRe: `zone "us-east2" matches no existing nodes within the cluster`,
		},
		{
			cfg:   `voter_constraints: ["-region=us-east2-b"]`,
			errRe: `region "us-east2-b" matches no existing nodes within the cluster`,
		},
		{
			cfg:   `constraints: ["+rack=us-east1"]`,
			errRe: `invalid constraint attribute: "rack" for tenants`,
		},
		{
			cfg:   `constraints: ["-rack=us-east1"]`,
			errRe: `invalid constraint attribute: "rack" for tenants`,
		...
		{
			cfg:   `constraints: ["+ssd"]`,
			errRe: "non-locality constraints are not allowed for tenants",
		},
		{
			cfg:   `constraints: ["-ssd"]`,
			errRe: "non-locality constraints are not allowed for tenants",
	...
	for _, tc := range testCases {
		...
		err = validateZoneLocalitiesForSecondaryTenants(context.TODO(), getRegions, &zone)
		if tc.errRe == "" {
			require.NoError(t, err)
		} else {
			require.Error(t, err)
