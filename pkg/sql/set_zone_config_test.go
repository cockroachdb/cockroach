...
func TestValidateVoterConstraints(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{`["-foo=bar"]`, `{"+foo=bar": 1}`, true, "conflicts with voter_constraint"},
		{`["-foo=bar", "+duck=bar"]`, `["-foo=duck", "+duck=bar"]`, true, "voter_constraints cannot contain prohibitive"},
		{`["-foo=bar"]`, `["-foo=bar"]`, true, "voter_constraints cannot contain prohibitive"},
		{`["-foo=bar"]`, `["-duck=bar"]`, true, "voter_constraints cannot contain prohibitive"},
		{`["+foo=bar"]`, `["+foo=bar"]`, false, ""},
		{`{"+foo=bar": 1, "+foo=duck": 2}`, `{"+foo=bar": 3}`, false, ""},
		// TODO DURING REVIEW: Allowing for configurations like this one seems very
		// desirable, but it also allows the user to more easily walk themselves
		// into unsatisfiable zone configurations. Do we want to allow the
		// flexibility permitted by allowing `constraints` and `voter_constraints`
		// to be compatible but disjoint? Or do we want to force `voter_constraints`
		// to be a strictly narrower subset of `constraints`?
		//
		// I cant foresee any real complications with this flexibility at the
		// allocator level but its likely there will be some.
		{`{"+region=A": 1, "+region=B": 1, "+region=C": 1}`, `["+ssd"]`, false, ""},
	} {
		zone := zonepb.NewZoneConfig()
		zone.NumVotersConfiguredSeparately = true
		...
		if err := yaml.UnmarshalStrict([]byte(`voter_constraints: `+tc.voterConstraints), &zone); err != nil {
			t.Fatal(err)
		}
		err := zone.Validate()
