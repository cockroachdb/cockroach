...
func (p *planner) showClusterSetting(ctx context.Context, name string) (planNode, error) {
	if name == "all" {
		return p.delegateQuery(ctx, "TABLE crdb_internal.cluster_settings", nil)
	}
...
`
	...
	plan, err := p.delegateQuery(ctx, fmt.Sprintf(traceClause, whereClause), nil)
	if err != nil {
		return nil, err
	}
...
func (p *planner) ShowGrants(ctx context.Context, n *parser.ShowGrants) (planNode, error) {
		...
		for _, tableTarget := range n.Targets.Tables {
			...
			for i := range tables {
				...
				params = append(params, fmt.Sprintf("(%s,%s)",
					parser.EscapeSQLString(tables[i].Database()),
					parser.EscapeSQLString(tables[i].Table())))
			}
		}
	...
	return p.delegateQuery(ctx, grantQuery.String(), nil)
}
...
func (p *planner) ShowQueries(ctx context.Context, n *parser.ShowQueries) (planNode, error) {
	...
	return p.delegateQuery(ctx, query, nil)
}
