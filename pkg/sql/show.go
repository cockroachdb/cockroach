...
func (p *planner) ShowColumns(n *parser.ShowColumns) (planNode, error) {
	...
	return &delayedNode{
		...
		constructor: func(p *planner) (planNode, error) {
			...
			{
				// Check if the database is exists by using the security.RootUser.
			...
			{
				// Check if the table is exists by using the security.RootUser.
...
func (p *planner) ShowGrants(n *parser.ShowGrants) (planNode, error) {
	...
	return &delayedNode{
		...
		constructor: func(p *planner) (planNode, error) {
			...
			// Queries the grants of target and filters the grants by grantees if grantees are specified.
			...
			if n.Targets.Databases != nil {
				var dbNames []interface{}
				paramHolders := []string{}
				for i, db := range n.Targets.Databases.ToStrings() {
					isExist, err := checkFn(checkSchema, db)
					if err != nil {
						v.rows.Close()
						return nil, err
					}
					...
					paramHolders = append(paramHolders, fmt.Sprintf("$%d", i+1))
				...
				if err := appendGrants(schemaGrants, dbNames...); err != nil {
					v.rows.Close()
					return nil, err
				}
			}
			...
			if n.Targets.Tables != nil {
				...
				for _, tableTarget := range n.Targets.Tables {
					...
					for i := range tables {
						isExist, err := checkFn(checkTable, tables[i].Database(), tables[i].Table())
						if err != nil {
							v.rows.Close()
							return nil, err
						}
				...
				if err := appendGrants(tableGrants, params...); err != nil {
					v.rows.Close()
					return nil, err
				}
			}
...
func (p *planner) ShowTables(n *parser.ShowTables) (planNode, error) {
	...
	return &delayedNode{
		...
		constructor: func(p *planner) (planNode, error) {
			...
			queryFn := func(sql string, args ...interface{}) (planNode, error) {
