...
func makeAny(t *testing.T, msg proto.Message) *pbtypes.Any {
...
func TestMessageToJSONBRoundTrip(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.pbname, func(t *testing.T) {
			...
			require.NoError(t, err)
			jsonb, err := MessageToJSON(tc.pbname, protoData)
			require.NoError(t, err)

			decoded := reflect.New(reflect.TypeOf(tc.message).Elem()).Interface().(protoutil.Message)
			unmarshaler := &jsonpb.Unmarshaler{}
			require.NoError(t, unmarshaler.Unmarshal(strings.NewReader(jsonb.String()), decoded))
...
func TestInvalidConversions(t *testing.T) {
	...
	t.Run("no such messagge", func(t *testing.T) {
		_, err := MessageToJSON("no.such.message", nil)
		require.Error(t, err)
	})

	t.Run("must be message type", func(t *testing.T) {
		// Valid proto enum, but we require types.
		_, err := MessageToJSON("cockroach.sql.sqlbase.SystemColumnKind", nil)
		require.Error(t, err)
	})
}
