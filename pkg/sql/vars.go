...
var varGen = map[string]sessionVar{
	...
	`database`: {
		...
		Get: func(evalCtx *extendedEvalContext) string { return evalCtx.SessData.Database },
		Reset: func(m sessionDataMutator) error {
	...
	`default_transaction_isolation`: {
		...
		Get: func(evalCtx *extendedEvalContext) string {
			return evalCtx.SessData.DefaultIsolationLevel.ToLowerCaseString()
		},
		Reset: func(m sessionDataMutator) error {
	...
	`default_transaction_read_only`: {
		...
		Get: func(evalCtx *extendedEvalContext) string {
			return formatBoolAsPostgresSetting(evalCtx.SessData.DefaultReadOnly)
		},
		Reset: func(m sessionDataMutator) error {
	...
	`distsql`: {
		Set: func(
			...
			case "off":
				m.SetDistSQLMode(tree.DistSQLOff)
			case "on":
				m.SetDistSQLMode(tree.DistSQLOn)
			case "auto":
				m.SetDistSQLMode(tree.DistSQLAuto)
			case "always":
				m.SetDistSQLMode(tree.DistSQLAlways)
			default:
		...
		Get: func(evalCtx *extendedEvalContext) string {
			return evalCtx.SessData.DistSQLMode.String()
		},
		Reset: func(m sessionDataMutator) error {
			m.SetDistSQLMode(tree.DistSQLExecMode(
	...
	`sql_safe_updates`: {
		Get: func(evalCtx *extendedEvalContext) string {
			return strconv.FormatBool(evalCtx.SessData.SafeUpdates)
		},
		Set: func(
			_ context.Context, m sessionDataMutator,
			evalCtx *extendedEvalContext, values []tree.TypedExpr,
	...
	`search_path`: {
		Set: func(
			...
			m.SetSearchPath(tree.MakeSearchPath(paths))
			return nil
		},
		Get: func(evalCtx *extendedEvalContext) string {
			return evalCtx.SessData.SearchPath.String()
		},
		Reset: func(m sessionDataMutator) error {
	...
	`session_user`: {
		Get: func(evalCtx *extendedEvalContext) string { return evalCtx.SessData.User },
	},
	...
	`timezone`: {
		Get: func(evalCtx *extendedEvalContext) string {
			...
			_, origRepr, parsed := timeutil.ParseFixedOffsetTimeZone(evalCtx.SessData.Location.String())
			if parsed {
				return origRepr
			}
			return evalCtx.SessData.Location.String()
		},
		Set: setTimeZone,
		Reset: func(m sessionDataMutator) error {
