...
var varGen = map[string]sessionVar{
	...
	`syntax`: {
		...
		Reset: func(p *planner) error {
			var zeroVal parser.Syntax
			p.session.Syntax = zeroVal
			return nil
		},
	},
	`default_transaction_isolation`: {
		...
		Reset: func(p *planner) error {
			var zeroVal enginepb.IsolationType
			p.session.DefaultIsolationLevel = zeroVal
			return nil
		},
	},
	`distsql`: {
		...
		Reset: func(p *planner) error {
			var zeroVal distSQLExecMode
			p.session.DistSQLMode = zeroVal
			return nil
		},
	},
	`search_path`: {
