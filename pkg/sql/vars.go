...
var varGen = map[string]sessionVar{
	...
	// CockroachDB extension.
	`stub_catalog_tables_enabled`: {
		GetStringVal: makePostgresBoolGetStringValFn(`stub_catalog_tables_enabled`),
		Set: func(_ context.Context, m *sessionDataMutator, s string) error {
			b, err := paramparse.ParseBoolVar("stub_catalog_tables_enabled", s)
			if err != nil {
				return err
			}
