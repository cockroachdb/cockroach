...
func TestAddUniqChecks(t *testing.T) {
	...
	out := exec.InsertFastPathFKUniqCheck{ReferencedTable: tab, ReferencedIndex: referencedIndex}

	// Test that builds the DatumsFromConstraint element of
	// `exec.InsertFastPathFKUniqCheck` and checks the output of calling
	// `addUniqChecks`.
	buildDatumsFromConstraintAndVerifyAddedUniqueChecks := func(c constraint.Constraint, expectedError string) {
		...
		uniqChecks := make([]insertFastPathFKUniqCheck, 1)
		uniqChecks[0] = insertFastPathFKUniqCheck{InsertFastPathFKUniqCheck: out}
