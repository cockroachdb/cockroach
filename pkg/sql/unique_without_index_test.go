...
func TestUWIConstraintReferencingTypes(t *testing.T) {
	...
	testutils.RunTrueAndFalse(t, "test-in-both-legacy-and-declarative-schema-changer", func(
		...
		tableDesc := desctestutils.TestingGetPublicTableDescriptor(kvDB, keys.SystemSQLCodec, "defaultdb", "t")
		typDesc := desctestutils.TestingGetPublicTypeDescriptor(kvDB, keys.SystemSQLCodec, "defaultdb", "typ")
