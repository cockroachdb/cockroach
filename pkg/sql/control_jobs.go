...
func (n *controlJobsNode) startExec(params runParams) error {
	...
	if !userIsAdmin {
		...
		if !hasControlJob {
			return pgerror.Newf(pgcode.InsufficientPrivilege,
				...
				params.p.User(), roleoption.CONTROLJOB.String())
		}
	}
	...
	for {
		...
		job, err := reg.LoadJob(params.ctx, int64(jobID))
		owner := job.Payload().Username

		if !userIsAdmin {
			ok, err := params.p.IsUserAdminRole(params.ctx, owner)
			if err != nil {
				return err
			}
			...
			if ok {
				return errors.New("only admins can control jobs owned by other admins")
			}
		}
