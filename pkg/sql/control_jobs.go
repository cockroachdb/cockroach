...
func (n *controlJobsNode) startExec(params runParams) error {
	...
	userHasControlJob, err := params.p.HasRoleOption(params.ctx, roleoption.CONTROLJOB)
	if err != nil {
		return err
	}
	...
	for {
		...
		payload := job.Payload()
		canAccess, userErr, err := JobTypeSpecificPrivilegeCheck(params.ctx, params.p,
			job.ID(), &payload, userIsAdmin, userHasControlJob, false)
		if err != nil {
			return err
		}
		if !canAccess {
