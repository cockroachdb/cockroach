	...
	for i, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expectedError == "" {
				...
				testutils.SucceedsSoon(t, func() error {
					registry1.mu.Lock()
					defer registry1.mu.Unlock()
					registry2.mu.Lock()
					defer registry2.mu.Unlock()
					registry3.mu.Lock()
					defer registry3.mu.Unlock()
					shouldStart1 := registry1.findTxnRequestLocked(RequestID(requestID))
					shouldStart2 := registry2.findTxnRequestLocked(RequestID(requestID))
					shouldStart3 := registry3.findTxnRequestLocked(RequestID(requestID))
