...
var pollingInterval = settings.RegisterDurationSetting(
	...
	"rate at which the stmtdiagnostics.Registry polls for requests",
	10*time.Second,
	func(val time.Duration) error {
		if val <= 0 {
			return errors.Errorf(
				"disabling the polling mechanism of the stmt diagnostics " +
					"is disallowed because it breaks the feature",
			)
		}
		return nil
	},
...
func (r *Registry) poll(ctx context.Context) {
	var (
		...
		maybeResetTimer     = func() {
			newDeadline := lastPoll.Add(pollingInterval.Get(&r.st.SV))
