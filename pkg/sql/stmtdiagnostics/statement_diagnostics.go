...
func (r *Registry) insertRequestInternal(
	...
	err = r.db.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
		...
		if count != 0 {
			...
			// new requests have different min_execution_latency and expires_at
			// fields.
		...
		row, err = r.ie.QueryRowEx(ctx, "stmt-diag-insert-request", txn,
			...
			"INSERT INTO system.statement_diagnostics_requests (statement_fingerprint, requested_at, min_execution_latency, expires_at) "+
				"VALUES ($1, $2, $3, $4) RETURNING id",
			fprint, now, minExecutionLatency, expiresAt)
		if err != nil {
			return err
		}
		if row == nil {
...
// added back to the registry if requestID is non-zero.
...
func (r *Registry) pollRequests(ctx context.Context) error {
	...
	// completed? It is possible that there is an ongoing execution right now
	// for an already expired request, so we'll need to be careful.
	...
	for _, row := range rows {
		...
		minExecutionLatency := time.Duration((*row[2].(*tree.DInterval)).Nanos())
