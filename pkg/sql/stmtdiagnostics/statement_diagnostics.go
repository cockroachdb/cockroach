...
func (r *Registry) findRequestLocked(requestID RequestID) bool {
	...
	if ok {
		if f.expiresAt.Before(timeutil.Now()) {
...
func (r *Registry) InsertRequest(
	...
	_, err := r.insertRequestInternal(ctx, fprint, minExecutionLatencyMilliseconds, expiresAfter)
	return err
}
...
func (r *Registry) insertRequestInternal(
	...
	err = r.db.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
		...
		row, err := r.ie.QueryRowEx(ctx, "stmt-diag-check-pending", txn,
			...
			"SELECT count(1) FROM system.statement_diagnostics_requests "+
				"WHERE completed = false AND statement_fingerprint = $1 AND expires_at > now()",
			fprint)
		if err != nil {
			return err
		}
		...
		if count != 0 {
			...
			// new requests have different min_execution_latency_ms and
			// expires_at fields.
			return errors.New("a pending request for the requested fingerprint already exists")
		}

		if expiresAfter == 0 {
			// If there is no expiration interval, then set it to 100 years.
			expiresAfter = 36525 * 24 * time.Hour
		}
		now := timeutil.Now()
		...
		row, err = r.ie.QueryRowEx(ctx, "stmt-diag-insert-request", txn,
			...
			"INSERT INTO system.statement_diagnostics_requests (statement_fingerprint, requested_at, min_execution_latency_ms, expires_at) "+
				"VALUES ($1, $2, $3, $4) RETURNING id",
			fprint, now, minExecutionLatencyMilliseconds, expiresAt)
		if err != nil {
			return err
		}
		if row == nil {
	...
	defer r.mu.Unlock()
	r.mu.epoch++
	r.addRequestInternalLocked(ctx, reqID, fprint, minExecutionLatencyMilliseconds, expiresAt)
...
func (r *Registry) CheckExecLatency(
	...
	if req.minExecutionLatencyMilliseconds <= execLatency.Milliseconds() {
		return true
	}
	if requestID != 0 {
		r.mu.Lock()
...
func (r *Registry) ShouldCollectDiagnostics(
	...
	for id, f := range r.mu.requestFingerprints {
		if f.fingerprint == fingerprint {
			reqID = id
			req = f
			expired = f.expiresAt.Before(timeutil.Now())
			break
		}
	}
	if reqID == 0 {
...
func (r *Registry) pollRequests(ctx context.Context) error {
	...
	for _, row := range rows {
		...
		if row[2] != tree.DNull {
			minExecutionLatencyMilliseconds = int64(*row[2].(*tree.DInt))
		}
		expiresAt := (*row[3].(*tree.DTimestampTZ)).Time
		ids.Add(int(id))
		r.addRequestInternalLocked(ctx, id, fprint, minExecutionLatencyMilliseconds, expiresAt)
	}

	// Remove all other requests.
	for id, req := range r.mu.requestFingerprints {
		if !ids.Contains(int(id)) || req.expiresAt.Before(now) {
