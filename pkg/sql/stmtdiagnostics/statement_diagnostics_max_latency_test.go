...
func TestIsConditionSatisfied(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:        "tight range - within",
			minLatency:  100 * time.Millisecond,
			maxLatency:  110 * time.Millisecond,
			execLatency: 105 * time.Millisecond,
			satisfied:   true,
			description: "narrow 10ms window, latency fits",
		},
		{
			name:        "tight range - exceeds",
			minLatency:  100 * time.Millisecond,
			maxLatency:  110 * time.Millisecond,
			execLatency: 115 * time.Millisecond,
			satisfied:   false,
			description: "narrow 10ms window, latency exceeds",
		},
		{
		...
		},
		{
			...
			execLatency: 150 * time.Microsecond,
			satisfied:   true,
			description: "verify precision at microsecond level",
		},
	}
...
func TestMaxExecutionLatencyBasic(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			query:         "SELECT pg_sleep(0.35), 1, 2",
			minLatency:    100 * time.Millisecond,
			maxLatency:    300 * time.Millisecond,
			shouldCapture: false,
		...
		{
			name:          "latency equals min (inclusive)",
			fingerprint:   "SELECT _, pg_sleep(_)",
			query:         "SELECT 1, pg_sleep(0.1)",
			minLatency:    100 * time.Millisecond,
			maxLatency:    300 * time.Millisecond,
			shouldCapture: true,
			description:   "lower bound is inclusive: min <= latency",
		...
		{
			name:          "latency equals max (exclusive)",
			fingerprint:   "SELECT _, pg_sleep(_), _",
			query:         "SELECT 1, pg_sleep(0.3), 2",
			minLatency:    100 * time.Millisecond,
			maxLatency:    300 * time.Millisecond,
			shouldCapture: false,
			description:   "upper bound is exclusive: latency < max",
...
func TestMaxExecutionLatencyBackwardCompatibility(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:          "min only - very high latency",
			fingerprint:   "SELECT pg_sleep(_), _, _",
			query:         "SELECT pg_sleep(10), 1, 2",
			minLatency:    100 * time.Millisecond,
			maxLatency:    0,
			shouldCapture: true,
			description:   "no upper limit when max=0 (existing behavior)",
		},
	}
...
// TestMaxExecutionLatencyWithSampling tests that max latency works correctly
...
func TestMaxExecutionLatencyWithSampling(t *testing.T) {
	...
	const (
		anyPlan      = ""
		noAntiMatch  = false
		noSampling   = 0.0 // No sampling - single capture
		noExpiration = 0 * time.Second
	)

	// Test that max latency works with conditional capture (min+max thresholds)
	minLatency := 100 * time.Millisecond
	maxLatency := 300 * time.Millisecond

	reqID, err := registry.InsertRequestInternal(
		ctx, "SELECT pg_sleep(_)", anyPlan, noAntiMatch,
		noSampling, minLatency, maxLatency, noExpiration,
	)
	require.NoError(t, err)

	// Execute query within bounds - should capture
	runner.Exec(t, "SELECT pg_sleep(0.15)")
	...
	`, reqID).Scan(&completed, &diagID)
	require.NoError(t, err)
	require.True(t, completed, "should capture when latency in range")
	require.NotNil(t, diagID, "bundle should exist")
}
...
func TestMaxExecutionLatencyMultipleRequests(t *testing.T) {
	...
	// Create three requests for different fingerprints with different latency ranges
	// Request 1: capture 50-100ms queries for "SELECT pg_sleep(_)"
	reqID1, err := registry.InsertRequestInternal(
		ctx, "SELECT pg_sleep(_)", anyPlan, noAntiMatch,
		sampleAll, 50*time.Millisecond, 100*time.Millisecond, noExpiration,
	)
	require.NoError(t, err)

	// Request 2: capture 100-200ms queries for "SELECT pg_sleep(_), _"
	reqID2, err := registry.InsertRequestInternal(
		ctx, "SELECT pg_sleep(_), _", anyPlan, noAntiMatch,
		sampleAll, 100*time.Millisecond, 200*time.Millisecond, noExpiration,
	)
	require.NoError(t, err)

	// Request 3: capture 200ms+ queries (no upper bound) for "SELECT pg_sleep(_), _, _"
	reqID3, err := registry.InsertRequestInternal(
		ctx, "SELECT pg_sleep(_), _, _", anyPlan, noAntiMatch,
		sampleAll, 200*time.Millisecond, 0, noExpiration,
	)
	require.NoError(t, err)

	// Execute queries at different latencies
	runner.Exec(t, "SELECT pg_sleep(0.075)")       // 75ms - should match req1 [50, 100)
	runner.Exec(t, "SELECT pg_sleep(0.150), 1")    // 150ms - should match req2 [100, 200)
	runner.Exec(t, "SELECT pg_sleep(0.300), 1, 2") // 300ms - should match req3 [200, ∞)
	...
	checkCompleted(reqID1, true, "request 1 should have captured 75ms query [50, 100)")
	checkCompleted(reqID2, true, "request 2 should have captured 150ms query [100, 200)")
	checkCompleted(reqID3, true, "request 3 should have captured 300ms query [200, ∞)")
}
...
func TestMaxExecutionLatencyValidation(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectError: false, // Empty range [100, 100) captures nothing but is technically valid
			description: "max == min creates empty range (valid but useless)",
		},
		{
