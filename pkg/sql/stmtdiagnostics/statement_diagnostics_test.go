...
func TestDiagnosticsRequest(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		reqID, err := registry.InsertRequestInternal(ctx, "INSERT INTO test VALUES (_)", minExecutionLatencyMilliseconds, expiresAfter)
		require.NoError(t, err)
	...
	t.Run("multiple", func(t *testing.T) {
		id1, err := registry.InsertRequestInternal(ctx, "INSERT INTO test VALUES (_)", minExecutionLatencyMilliseconds, expiresAfter)
		require.NoError(t, err)
		id2, err := registry.InsertRequestInternal(ctx, "SELECT x FROM test", minExecutionLatencyMilliseconds, expiresAfter)
		require.NoError(t, err)
		id3, err := registry.InsertRequestInternal(ctx, "SELECT x FROM test WHERE x > _", minExecutionLatencyMilliseconds, expiresAfter)
		require.NoError(t, err)
	...
	t.Run("execute", func(t *testing.T) {
		id, err := registry.InsertRequestInternal(ctx, "SELECT x + $1 FROM test", minExecutionLatencyMilliseconds, expiresAfter)
		require.NoError(t, err)
	...
	t.Run("conditional", func(t *testing.T) {
		minExecutionLatencyMilliseconds := int64(100)
		reqID, err := registry.InsertRequestInternal(ctx, "SELECT pg_sleep(_)", minExecutionLatencyMilliseconds, expiresAfter)
		require.NoError(t, err)
		checkNotCompleted(reqID)
