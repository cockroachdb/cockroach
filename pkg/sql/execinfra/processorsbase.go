...
// OutputStatsToTrace associates the given execution statistics with this
// processor's tracing span.
//
// Note: MakeDeterministic may be called on stats.
func (pb *ProcessorBase) OutputStatsToTrace(stats *execstatspb.ComponentStats) {
	if sp := tracing.SpanFromContext(pb.Ctx); sp != nil {
		if pb.FlowCtx.Cfg.TestingKnobs.DeterministicStats {
			stats.MakeDeterministic()
		}
		sp.SetSpanStats(stats)
	}
}
