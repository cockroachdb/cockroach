...
// TestPlanningDuringSplits verifies that table reader planning works while
// splits are happening. The purpose is to test that the spans are being split
// correctly.
func TestPlanningDuringSplits(t *testing.T) {
	...
	for i := 0; i < numQueriers; i++ {
		go func(idx int) {
			...
			// Limit to 1 connection because we set a connection variable.
