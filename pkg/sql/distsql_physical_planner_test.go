...
func TestPartitionSpans(t *testing.T) {
	...
	localityTag := func(nodeID int) string {
		// If you don't have cpu implant: 1: "x=1,y=1", 2: "x=1,y=0", 3: "x=2,y=1"
	...
	testCases := []struct {
		...
		// expected.
		{
			...
			spans:     [][2]string{{"A1", "C1"}, {"D1", "X"}},
			locFilter: []string{"x=1"}, // nodes 1, 2, and 3 should match this.
			partitions: map[int][][2]string{
				1: {{"A1", "B"}, {"C", "C1"}},
				2: {{"B", "C"}, {"D1", "X"}},
			},
		...
		// 13: filter with multiple localities. esentially the same as test case 9,
		...
		t.Run(strconv.Itoa(testIdx), func(t *testing.T) {
			...
			if len(locFilters) > 0 {
				...
				planCtxStrict := dsp.NewPlanningCtxWithOracle(
					ctx, &extendedEvalContext{Context: *evalCtx}, nil, /* planner */
					nil /* txn */, FullDistribution, physicalplan.DefaultReplicaChooser, locFilters, true,
				)
				planCtxStrict.spanPartitionState.testingOverrideRandomSelection = tc.partitionState.testingOverrideRandomSelection

				partitionsStrict, strictErr := dsp.PartitionSpans(ctx, planCtxStrict, spans, PartitionSpansBoundDefault)
				if !tc.strictCompatible {
					require.Error(t, strictErr, testIdx)
				} else {
					resMapStrict := stringifyPartition(partitionsStrict)
					reflect.DeepEqual(resMapStrict, tc.partitions)
				}
			}
		})
	}
