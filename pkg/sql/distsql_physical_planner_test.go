...
func TestDistSQLUnavailableNodes(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)
	...
	startAndSetupCluster := func(t *testing.T, d base.DefaultTestTenantOptions) (
		serverutils.TestClusterInterface, *sqlutils.SQLRunner) {
		tc := serverutils.StartCluster(t, numNodes, base.TestClusterArgs{
			ReplicationMode: base.ReplicationManual,
			ServerArgs:      base.TestServerArgs{UseDatabase: "test", DefaultTestTenant: d},
		})

		if d == base.ExternalTestTenantAlwaysEnabled {
	...
	t.Run("unhealthy-nodes-in-single-tenant-mode", func(t *testing.T) {
		...
		tc, r := startAndSetupCluster(t, base.DefaultTestTenantOptions{})
		defer tc.Stopper().Stop(context.Background())
	...
	t.Run("unhealthy-nodes-in-external-mode", func(t *testing.T) {
		...
		tc, r := startAndSetupCluster(t, base.ExternalTestTenantAlwaysEnabled)
		defer tc.Stopper().Stop(context.Background())
