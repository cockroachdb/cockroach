...
func (m *MembershipCache) RunAtCacheReadTS(
	...
	if options.checkDatabaseRoleSettingsVersion {
		dbRoleSettingsDesc, err = txn.Descriptors().ByIDWithLeased(txn.KV()).WithoutLockedTimestamp().Get().Table(ctx, keys.RoleOptionsTableID)
		if err != nil {
			return err
		}
	}
		...
		defer func() {
			if roleOptionsDesc != nil {
				if roleOptionsDesc.IsUncommittedVersion() {
					...
					// Update versions and trigger a cache refresh.
					readTS = hlc.Timestamp{}
