...
// PipelineQueue maintains a set of plans running with pipelined execution.
...
// details.
func (pq *PipelineQueue) Add(ctx context.Context, p plan, exec func(plan) error) {
...
func (pq *PipelineQueue) insertInQueue(ctx context.Context, p plan) ([]doneChan, func()) {
	pq.mu.Lock()
	defer pq.mu.Unlock()
...
// immediately.
func (pq *PipelineQueue) prereqsForPlanLocked(ctx context.Context, newPlan plan) []doneChan {
	...
	pq.analyzer.Analyze(ctx, newPlan)
...
func (a *spanBasedDependencyAnalyzer) Analyze(ctx context.Context, p plan) {
	...
	readSpans, writeSpans := p.queryPlan.Spans(ctx)
