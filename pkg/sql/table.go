...
func (p *planner) createSchemaChangeJob(
	...
	for i := len(tableDesc.ClusterVersion.Mutations); i < len(tableDesc.Mutations); i++ {
		if tableDesc.Mutations[i].JobID == 0 {
			spanList = append(spanList,
				jobspb.ResumeSpanList{
					ResumeSpans: []roachpb.Span{span},
				},
			)
			mutations = append(mutations, &tableDesc.Mutations[i])
		}
	}
	...
	return tableDesc.ClusterVersion.NextMutationID, nil
}
