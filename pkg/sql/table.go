...
func (p *planner) createOrUpdateSchemaChangeJob(
	...
	if tableDesc.MutationJobID != 0 {
		var err error
		job, err = p.ExecCfg().JobRegistry.LoadJobWithTxn(ctx, tableDesc.MutationJobID, p.txn)
		if err != nil {
	...
	if tableDesc.MutationJobID == 0 {
		...
		tableDesc.MutationJobID = *job.ID()
	} else {
		...
		if err := job.WithTxn(p.txn).SetDescription(
			...
			func(ctx context.Context, description string) (string, error) {
				return strings.Join([]string{description, stmt}, ";\n"), nil
			},
		); err != nil {
			return sqlbase.InvalidMutationID, err
		}
