...
func newInvalidSchemaError(tn *tree.TableName) error {
	log.VEventf(context.TODO(), 2, "XXX invalid schema: %s", tn)
	debug.PrintStack()
	return pgerror.NewErrorf(pgerror.CodeInvalidSchemaNameError,
		"unsupported schema specification: %s", tree.ErrString(tn))
}
...
func (tc *TableCollection) getTableVersion(
	...
	if flags.allowAdding || !flags.required {
	...
	if tn.SchemaName != tree.PublicSchemaName {
		return nil, nil, newInvalidSchemaError(tn)
	}
	...
	if isSystemDB || testDisableTableLeases {
		...
		phyAccessor := PhysicalObjectAccessor{PhysicalDBAccessor{}}
		tbl, dbDesc, err := phyAccessor.GetObjectDesc(tn, flags)
		if err != nil {
			return nil, nil, err
		}
		return tbl, dbDesc, nil
	}
	...
	if dbID == 0 {
		...
		dbID, err = tc.databaseCache.getDatabaseID(ctx, tc.leaseMgr.execCfg.DB.Txn, tn.Catalog())
		if err != nil {
			return nil, nil, err
		}
	}
