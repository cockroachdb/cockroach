...
func (tc *TableCollection) getTableVersion(
	...
	dbID, err := tc.getUncommittedDatabaseID(tn, flags.required)
	if err != nil {
		return nil, nil, err
	}
	...
	if table, err := tc.getUncommittedTable(dbID, tn, flags.required); err != nil {
		return nil, nil, err
	} else if table != nil {
...
func (tc *TableCollection) getUncommittedDatabaseID(
	tn *tree.TableName, required bool,
) (sqlbase.ID, error) {
	...
	for _, table := range tc.uncommittedTables {
		...
		for _, drain := range table.DrainingNames {
			if drain.Name == string(tn.TableName) &&
				...
				var err error
				if required {
					// table name has gone away. If it's required here,
