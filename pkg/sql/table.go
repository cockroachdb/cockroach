...
// schemaChangeDisallowedInMixedStateError signifies that an attempted schema
// change was disallowed from running in a mixed-version
var schemaChangeDisallowedInMixedStateError = errors.New("schema change cannot be initiated in this version until the version upgrade is finalized")
...
func (p *planner) createDropDatabaseJob(
	...
	if !p.ExecCfg().Settings.Version.IsActive(ctx, clusterversion.VersionSchemaChangeJob) {
		if ctx.Value(migrationSchemaChangeRequiredHint{}) == nil {
			return schemaChangeDisallowedInMixedStateError
		}
	}
