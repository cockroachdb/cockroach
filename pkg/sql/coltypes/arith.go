...
func (node *TBool) PGTypeName() string { return "boolean" }
...
type TInt struct {
	...
	// IsBit is only used with BIT.
	// TODO(knz): remove this with the fix to #20991.
	IsBit bool

	// ExplicitWidth is only used with BIT.
	// TODO(knz): remove this with the fix to #20991.
	ExplicitWidth bool
}

var intNames = map[int]string{
	0:  "INT",
	16: "INT2",
	32: "INT4",
	64: "INT8",
...
// TypeName implements the ColTypeFormatter interface.
func (node *TInt) TypeName() string {
	if node.IsBit {
		return "BIT"
	}
	return intNames[node.Width]
}

var pgIntNames = map[int]string{
	0:  "integer", // TODO(knz): This is a lie. Change this.
	16: "smallint",
	32: "integer",
	64: "bigint",
}
...
// PGTypeName implements the ColTypeFormatter interface.
func (node *TInt) PGTypeName() string {
	if node.IsBit {
		return "bit"
	}
	return pgIntNames[node.Width]
}

// Format implements the ColTypeFormatter interface.
func (node *TInt) Format(buf *bytes.Buffer, f lex.EncodeFlags) {
	buf.WriteString(node.TypeName())
	if node.ExplicitWidth {
...
// TypeName implements the ColTypeFormatter interface.
func (node *TSerial) TypeName() string { return serialNames[node.IntType.Width] }

// PGTypeName implements the ColTypeFormatter interface.
func (node *TSerial) PGTypeName() string { panic("SERIAL cannot show up in introspection") }
...
// PGTypeName implements the ColTypeFormatter interface.
func (node *TFloat) PGTypeName() string {
	if node.Width == 32 {
		return "real"
	}
...
// PGTypeName implements the ColTypeFormatter interface.
func (node *TDecimal) PGTypeName() string { return "numeric" }
