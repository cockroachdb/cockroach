...
func (t *tableState) acquire(
	...
	if s := t.mu.active.findNewest(); s != nil && !s.hasExpired(timestamp) && !t.mu.asyncRenewalInProgress && !s.leaseHasEnoughLifeLeft() {
		// Set a flag denoting async acquisition has begun to prevent this from
...
// asynchronous renewal. This function is started as a new routine if a lease is
// about to expire, to renew it in the background.
func (t *tableState) acquireIfRenewalNotInProgress(m *LeaseManager) {
	...
	// If another routine is already acquiring a new lease, short circuit and
	// avoid this routines acquisition. This happens when racing another routine
	// if the lease has just expired or an acquisition has been started by
	// ensureVersion.
	...
	if err != nil {
		log.Error(ctx, errors.Wrapf(err, "Asynchronous acquireIfRenewalNotInProgress failed"))
	}
