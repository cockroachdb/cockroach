...
type tableState struct {
	...
	mu struct {
		...
		// one call to happen concurrently.
...
func (t *tableState) acquire(
	...
	for s := t.mu.active.findNewest(); s == nil || s.hasExpired(timestamp); s = t.mu.active.findNewest() {
		...
		t.mu.Unlock()
		result := <-resultChan
		if m.testingKnobs.LeaseStoreTestingKnobs.LeaseAcquirePrelockEvent != nil {
			m.testingKnobs.LeaseStoreTestingKnobs.LeaseAcquirePrelockEvent(result.Err)
		}
		t.mu.Lock()
		if result.Err != nil {
...
func (t *tableState) acquireFreshestFromStoreLocked(ctx context.Context, m *LeaseManager) error {
	...
	for {
		...
		t.mu.Unlock()
		result := <-resultChan
		if m.testingKnobs.LeaseStoreTestingKnobs.LeaseAcquireFreshestPrelockEvent != nil {
			m.testingKnobs.LeaseStoreTestingKnobs.LeaseAcquireFreshestPrelockEvent(result.Err)
		}
		t.mu.Lock()
		if result.Err != nil {
			return result.Err
...
type LeaseStoreTestingKnobs struct {
	...
	// Called after a lease is acquired in acquire() but before the tableState
	// lock is re-acquired, with any operation error.
	LeaseAcquirePrelockEvent func(err error)
	// Called after a lease is acquired in acquireFreshestLeaseFromStore() but
	// before the tableState lock is re-acquired, with any operation error.
	LeaseAcquireFreshestPrelockEvent func(err error)
