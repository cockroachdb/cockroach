...
func (s LeaseStore) acquire(
	...
	if s.testingKnobs.LeaseAcquiredEvent != nil {
		if table != nil {
			s.testingKnobs.LeaseAcquiredEvent(table.TableDescriptor, err)
		} else {
			s.testingKnobs.LeaseAcquiredEvent(sqlbase.TableDescriptor{}, err)
		}
	}
	return table, err
}
...
func (t *tableState) acquire(
	...
	if s := t.active.findNewest(); s == nil || s.hasExpired(timestamp) {
		if err := t.acquireFromStoreLocked(ctx, 0, m); err != nil {
			return nil, err
		}
	}
...
func (t *tableState) ensureVersion(
	...
	return t.acquireFromStoreLocked(ctx, minVersion, m)
}
...
func (t *tableState) acquireFromStoreLocked(
	...
	s, err := t.acquireNodeLease(ctx, minVersion, m, hlc.Timestamp{})
	if err != nil {
		return err
	}
...
func (t *tableState) acquireFreshestFromStoreLocked(ctx context.Context, m *LeaseManager) error {
	...
	s, err := t.acquireNodeLease(ctx, 0, m, minExpirationTime)
	if err != nil {
		return err
	}
	t.upsertLocked(ctx, s, m)
...
func (t *tableState) acquireNodeLease(
	...
	t.mu.Unlock()
	defer t.mu.Lock()
	table, err := m.LeaseStore.acquire(ctx, t.id, minVersion, minExpirationTime)
	if err != nil {
		return nil, err
	}
