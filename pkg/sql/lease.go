...
func (m *LeaseManager) RefreshLeases(
	ctx context.Context, s *stop.Stopper, db *kv.DB, g *gossip.Gossip,
) {
...
func (m *LeaseManager) refreshLeases(
	ctx context.Context, g *gossip.Gossip, db *kv.DB, s *stop.Stopper,
) {
...
func (m *LeaseManager) watchForGossipUpdates(
	...
	if g == nil {
		panic(errors.Errorf("must have non-nil gossip until %v is active", clusterversion.VersionRangefeedLeases))
	}
	...
	s.RunWorker(ctx, func(ctx context.Context) {
		...
		ctx, cancel := s.WithCancelOnStop(ctx)
		defer cancel()
		for {
			select {
			case <-gossipUpdateC:
...
func (m *LeaseManager) watchForRangefeedUpdates(
	...
	if err := s.RunAsyncTask(ctx, "lease rangefeed", func(ctx context.Context) {
		for {
			...
			if log.V(1) {
				log.Infof(ctx, "starting rangefeed from %v on %v", ts, span)
			}
