...
func ensureIncrementVersion(
	...
	desc.Version = desc.ClusterVersion.Version + 1
...
func (s LeaseStore) Publish(
	...
	for r := retry.Start(base.DefaultRetryOptions()); r.Next(); {
		...
		err = s.execCfg.DB.Txn(ctx, func(ctx context.Context, txn *client.Txn) error {
			...
			// transactionally.
			desc, err := sqlbase.GetMutableTableDescFromID(ctx, txn, tableID)
			if err != nil {
				return err
			}
			tableDesc = desc
			...
			if err := ensureIncrementVersion(ctx, tableDesc, txn); err != nil {
				return err
			}
