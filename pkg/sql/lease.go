...
// lifetime left until expiration.
func (s *tableVersionState) hasSomeLifeLeft(timestamp hlc.Timestamp) bool {
	if !s.leased {
		panic(fmt.Sprintf("trying to check life of expiration without lease: %+v", s))
	}
...
func (s LeaseStore) getForExpiration(
	...
	err := s.db.Txn(ctx, func(ctx context.Context, txn *client.Txn) error {
		...
		if !tableDesc.ModificationTime.Less(prevTimestamp) {
			return errors.Errorf("unable to read table = %d with expiration = %s", id, expiration)
		}
...
// of the transaction. The table will have its refcount incremented,
// so the caller is responsible for calling release() on it.
...
func (t *tableState) findForTimestamp(
	...
	for i := len(t.active.data) - 1; i >= 0; i-- {
		...
		if table := t.active.data[i]; !timestamp.Less(table.ModificationTime) {
			...
			// table has a valid ModificationTime but an invalid
			// expiration time. We need to find a table version between
			// this and the next one that is a match for the timestamp.
