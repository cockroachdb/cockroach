...
func (t *tableState) acquireFreshestFromStoreLocked(ctx context.Context, m *LeaseManager) error {
	...
	for {
		...
		t.mu.Unlock()
		l, wasCalled, err := acquireNodeLease(ctx, m, t.id, minExpirationTime)
		t.mu.Lock()
		if err != nil {
			return err
		}
...
func acquireNodeLease(
	...
	resultChan, wasCalled := m.group.DoChan(fmt.Sprintf("acquire%d", id), func() (interface{}, error) {
		if m.isDraining() {
	...
	return result.Val.(leaseToken), wasCalled, nil
}
