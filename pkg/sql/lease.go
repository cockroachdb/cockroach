...
// Does this table have a larger expiration than the existing
// one it is replacing. This can be used to check the monotonicity
// of the expiration times on a table at a particular version.
...
func (t *tableState) upsertLocked(
	...
	if !table.hasValidExpiration(s) {
		...
		return nil, errors.Errorf("lease expiration monotonicity violation, (%+v) vs (%+v)", s, table)
	}
