...
func (s LeaseStore) Acquire(
	...
	p := makeInternalPlanner("lease-acquire", txn, security.RootUser.Username(), s.memMetrics)
	...
	err = s.db.Txn(txn.Context, func(txn *client.Txn) error {
		...
		p := makeInternalPlanner("lease-insert", txn, security.RootUser.Username(), s.memMetrics)
		defer finishInternalPlanner(p)
...
func (s LeaseStore) Release(lease *LeaseState) error {
	err := s.db.Txn(context.TODO(), func(txn *client.Txn) error {
		...
		p := makeInternalPlanner("lease-release", txn, security.RootUser.Username(), s.memMetrics)
		defer finishInternalPlanner(p)
...
func (s LeaseStore) countLeases(
	...
	var count int
	err := s.db.Txn(context.TODO(), func(txn *client.Txn) error {
		p := makeInternalPlanner("leases-count", txn, security.RootUser.Username(), s.memMetrics)
		defer finishInternalPlanner(p)
