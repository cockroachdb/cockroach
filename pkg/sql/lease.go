...
func (tn *tableNotifier) Register(tableID sqlbase.ID) (<-chan struct{}, func()) {
	...
	tn.Lock()
	if tn.chans[tableID] == nil {
		tn.chans[tableID] = make(map[chan struct{}]struct{})
	}
	tn.chans[tableID][waiterC] = struct{}{}
	tn.Unlock()

	return waiterC, func() {
		tn.Lock()
		delete(tn.chans[tableID], waiterC)
		if len(tn.chans[tableID]) == 0 {
