...
// information related to running that plan under the vectorized engine.
...
func (n *explainVecNode) startExec(params runParams) error {
	...
	for _, flow := range sortedFlows {
		...
		for i, op := range opChains {
			formatOpChain(op, node.Childf("Flow %d/%d", i+1, len(opChains)))
		}
	}
