...
func (o *indexCheckOperation) Start(ctx context.Context, p *planner) error {
	...
	// Set the MergeJoiner NullEquality to true. This changes the
	// behaviour of the query's equality semantics in the ON predicate.
	// The equalities will now evaluate NULL = NULL to true, which is what
	// we desire when testing the equivilance of two index entries. We do
	// not know which processor is the merge joiner, but we do know that
	// there will only be one.
	var mergeJoinerSpec *distsqlrun.MergeJoinerSpec
	for i := range physPlan.Processors {
		if physPlan.Processors[i].Spec.Core.MergeJoiner != nil {
			mergeJoinerSpec = physPlan.Processors[i].Spec.Core.MergeJoiner
			break
		}
	}
	if mergeJoinerSpec == nil {
		return errors.Errorf("could not find MergeJoinerSpec in plan")
	}
	mergeJoinerSpec.NullEquality = true
...
func createIndexCheckQuery(
	...
	return fmt.Sprintf(checkIndexQuery,
		...
		tableColumnsIsNullPredicate("leftside", tableDesc.PrimaryIndex.ColumnNames, true /* isNull */),
		tableColumnsIsNullPredicate("rightside", tableDesc.PrimaryIndex.ColumnNames, true /* isNull */),
		strings.Join(columnNames, ","),
	)
}
