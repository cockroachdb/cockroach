...
func (n *scrubNode) startIndexCheck(
	...
	// FIXME(joey): Usage of rows.Close() may be incorrect. I was not
	// completely when to use this. Currently it is:
	// - Closed after reading the rows
	// - Closed if an error is received, and we don't expect rows, or use
	//   the container.
...
func tableColumnsIsNullPredicate(tableName string, columns []string) string {
	...
	for i, col := range columns {
		...
		buffer.WriteString(fmt.Sprintf("%[1]s.%[2]s IS NULL", tableName, col))
	}
...
func tableColumnsProjection(tableName string, columns []string) string {
	...
	for i, col := range columns {
		...
		buffer.WriteString(fmt.Sprintf("%[1]s.%[2]s", tableName, col))
	}
	return buffer.String()
}
