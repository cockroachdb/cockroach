...
func (n *scrubNode) Start(params runParams) error {
	...
	// No options were provided. By default all checks are run.
	...
	if indexes != nil {
		...
		for _, index := range indexes {
			if err := n.startIndexCheck(params.ctx, params.p, tableDesc, index); err != nil {
				return err
			}
		}
	}
...
func createIndexCheckQuery(
	columns []*sqlbase.ColumnDescriptor, tableName string, indexID sqlbase.IndexID,
) string {
...
func (n *scrubNode) startIndexCheck(
	...
	checkQuery := createIndexCheckQuery(columns, tableDesc.Name, indexDesc.ID)
	log.Infof(ctx, "Check query is:\n%s", checkQuery)
	...
	if rows.Len() > 0 {
		n.checkedIndexes = append(n.checkedIndexes, &checkedIndex{
			...
			// FIXME(joey): Allow cross-database checks.
			databaseName:              p.session.Database,
...
func (n *scrubNode) Next(params runParams) (bool, error) {
	if n.checkedIndexes != nil && len(n.checkedIndexes) > 0 {
		...
		if isMissingIndexReferenceError {
			errorType = parser.NewDString("missing_index_reference")
		} else {
			errorType = parser.NewDString("dangling_index_reference")
		}
		...
		if isMissingIndexReferenceError {
			...
			// data, because no primary index was found.
			for rowIdx, col := range checkedIndex.columns {
				// FIXME(joey): Try to the underlying type of the datum.
