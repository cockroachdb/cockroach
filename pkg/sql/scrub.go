...
func createConstraintCheckOperations(
	ctx context.Context,
	p *planner,
	constraintNames parser.NameList,
	tableDesc *sqlbase.TableDescriptor,
	tableName *parser.TableName,
) (results []checkOperation, err error) {
...
func (o *sqlCheckConstraintCheckOperation) Start(ctx context.Context, p *planner) error {
	...
	normalizableTableName := &parser.NormalizableTableName{TableNameReference: o.tableName}
	sel := &parser.SelectClause{
		Exprs: sqlbase.ColumnsSelectors(o.tableDesc.Columns),
		From:  &parser.From{Tables: parser.TableExprs{normalizableTableName}},
	...
	rows, err := p.SelectClause(ctx, sel, nil /* orderBy */, nil, /* limit */
		false /* lockForUpdate */, nil /* desiredTypes */, publicColumns)
	if err != nil {
		return err
	}
...
func (o *sqlCheckConstraintCheckOperation) Next(
	...
	var err error
	o.hasRowsLeft, err = o.rows.Next(runParams{
		ctx: ctx,
		p:   p,
	})
	if err != nil {
	...
	return parser.Datums{
		// TODO(joey): Add the job UUID once the SCRUB command uses jobs.
		parser.DNull, /* job_uuid */
		parser.NewDString(ScrubErrorCheckConstraintViolation),
		parser.NewDString(o.tableName.Database()),
		parser.NewDString(o.tableName.Table()),
		parser.NewDString(primaryKeyDatums.String()),
		timestamp,
		parser.DBoolFalse,
		detailsJSON,
	}, nil
...
// Next implements the checkOperation interface.
func (o *sqlForeignKeyCheckOperation) Next(ctx context.Context, p *planner) (parser.Datums, error) {
	row := o.rows.At(o.rowIndex)
	o.rowIndex++
	...
	return parser.Datums{
		// TODO(joey): Add the job UUID once the SCRUB command uses jobs.
		parser.DNull, /* job_uuid */
		parser.NewDString(ScrubErrorForeignKeyConstraintViolation),
		parser.NewDString(o.tableName.Database()),
		parser.NewDString(o.tableName.Table()),
		primaryKey,
		timestamp,
