...
// different types of checks implement the interface, and the check will
// be executed with all other types.
//
...
type checkOperation interface {
	// Started indicates when a checkOperation has already been
	// initialized by calling Start during the lifetime of the operation.
	Started() bool
	...
	// Next will return the next check result. Done must be
	// tested before iterating.
	//
	// Datums will be returned that conform to the types specified by
	// scrubTypes. These are the column values that are returned to the
	// user.
...
func (n *scrubNode) Next(params runParams) (bool, error) {
	for len(n.checkQueue) > 0 {
		...
		// Begin the next check operation if it has not begun.
		...
		// Check if the iterator is finished. This happens if once the
		// iterator is out of errors, or if there were no errors found at
