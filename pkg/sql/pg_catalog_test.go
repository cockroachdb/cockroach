...
const (
	...
	virtualTableTempl        = `
var %s = virtualSchemaTable{
	comment: "%s is not yet implemented",
	schema: vtable.%s,
...
// fixConstants helps to update catconstants that are needed for pgCatalog
func fixConstants(t *testing.T, notImplemented PGCatalogTables) {
...
func fixVtable(t *testing.T, notImplemented PGCatalogTables) {
	...
	rewriteFile(fileName, func(input *os.File, output outputFile) {
		tableFinder := regexp.MustCompile(`(?i)CREATE TABLE pg_catalog\.([^\s]+)\s`)
		...
		for reader.Scan() {
			...
			output.appendString(text)
			output.appendString("\n")
			createTable := tableFinder.FindStringSubmatch(text)
			if createTable != nil {
...
// fixPgCatalogGo will update pgCatalog.allTableNames, pgCatalog.tableDefs and will add needed virtualSchemas
func fixPgCatalogGo(notImplemented PGCatalogTables) {
	...
	rewriteFile(pgCatalogGo, func(input *os.File, output outputFile) {
		...
		for reader.Scan() {
			...
			case tableDefsDeclaration:
				printBeforeClosure(reader, output, tableDefsClosure, tableDefinitionText)
			case allTableNamesDeclaration:
				printBeforeClosure(reader, output, allTableNamesClose, allTableNamesText)
			}
		}
	})
}

// printBeforeClosure will skip all the lines and print `s` text when finds the closure text
func printBeforeClosure(reader *bufio.Scanner, output outputFile, closure string, s string) {
	for reader.Scan() {
		text := reader.Text()
		trimText := strings.TrimSpace(text)
...
func getPgCatalogConstants(
	t *testing.T, inputFileName string, notImplemented PGCatalogTables,
) []string {
...
func (rw outputFile) appendString(s string) {
	if _, err := rw.f.WriteString(s); err != nil {
		panic(fmt.Errorf("error while writing string: %s: %v", s, err))
	}
}

// rewriteFile recreate a file by using the f func, this creates a temporary file to place all the output first
...
// dClose is a helper that eliminates the need of error checking and defer the io.Closer Close() and pass lint checks
...
// constantName create constant names for pg_catalog tables following constant names standards
...
func createTableConstant(tableName string, columns PGCatalogColumns) (string, error) {
	var sb strings.Builder
	constName := constantName(tableName, "")
	if notImplementedTypes := columns.getNotImplementedTypes(); len(notImplementedTypes) > 0 {
...
// printVirtualSchemas formats the golang code to create the virtualSchema structure
func printVirtualSchemas(newTableNameList PGCatalogTables) string {
	var sb strings.Builder
	for tableName := range newTableNameList {
		variableName := "p" + constantName(tableName, "Table")[1:]
		vTableName := constantName(tableName, "")
		sb.WriteString(fmt.Sprintf(virtualTableTempl, variableName, tableName, vTableName))
	}
	return sb.String()
}

// getAllTableNamesText retrieves pgCatalog.allTableNames, then it merges the new table names and formats the
// replacement text
func getAllTableNamesText(notImplemented PGCatalogTables) string {
...
// getTableDefinitionsText retrieves pgCatalog.tableDefs, then it merges the missing table in that object and formats
// the replacement text
func getTableDefinitionsText(fileName string, notImplemented PGCatalogTables) string {
...
func TestPGCatalog(t *testing.T) {
	...
	if *addMissingTables {
		notImplemented := diffs.getNotImplementedTables(pgTables)
