...
func RevertSpansFanout(
	...
	// We cap the overall number of workers based on the cluster
	// setting.
	...
	progressWorker := func(ctx context.Context) error {
		for sp := range completedSpanCh {
			if onCompletedCallback != nil {
				if err := onCompletedCallback(ctx, sp); err != nil {
					return err
				}
			}
		}
	...
	g.GoCtx(func(ctx context.Context) error {
		...
		for i := range workerPartitions {
			...
			errGroup.Go(func() error {
				...
				return RevertSpans(workerCtx, db, spans, targetTime, ignoreGCThreshold, batchSize,
...
// RevertSpans reverts the passed span to the target time, which much be above
// the GC threshold for every range (unless the flag ignoreGCThreshold is passed
// which should be done with care -- see RevertRangeRequest.IgnoreGCThreshold).
//
