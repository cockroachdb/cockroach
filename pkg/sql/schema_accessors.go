...
//
// See phy_schema_accessors.go and logical_schema_accessors.go for
// reference implementations of these interfaces.
//
// A set of common services is provided "on top" of any implementation
// of ObjectAccessor: getTableDesc(), getViewDesc(),
// getSequenceDesc(). These call the underlying GetObjectDesc() and
// simply validate its type.
...
type (
	...
	// ObjectDescriptor is provided for convenience and to make the interface definitions below more intuitive.
...
// The following functions are common services defined on top of
// any valid ObjectAccessor.

// getViewDesc is a convenience function that interfaces any
// ObjectAccessor's GetObjectDesc method and asserts it has type view.
func getViewDesc(
	a ObjectAccessor, name *ObjectName, flags ObjectLookupFlags,
) (*ViewDescriptor, *DatabaseDescriptor, error) {
	desc, dbDesc, err := a.GetObjectDesc(name, flags)
	if err != nil {
		return desc, dbDesc, err
	}
	if desc != nil && !desc.IsView() {
		return nil, nil, sqlbase.NewWrongObjectTypeError(name, "view")
	}
	return desc, dbDesc, nil
}

// getSequenceDesc is a convenience function that interfaces any
// ObjectAccessor's GetObjectDesc method and asserts it has type
// sequence.
func getSequenceDesc(
	a ObjectAccessor, name *ObjectName, flags ObjectLookupFlags,
) (*SequenceDescriptor, *DatabaseDescriptor, error) {
	desc, dbDesc, err := a.GetObjectDesc(name, flags)
	if err != nil {
		return desc, dbDesc, err
	}
	if desc != nil && !desc.IsSequence() {
		return nil, nil, sqlbase.NewWrongObjectTypeError(name, "sequence")
	}
	return desc, dbDesc, nil
}
