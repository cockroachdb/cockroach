...
// batches are in separate when non atomic mode is enabled.
func TestCopyFromTransaction(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			tc.testf(func() {
				numrows, err := conn.GetDriverConn().CopyFrom(ctx, strings.NewReader(strings.Join(tc.data, "\n")), tc.query)
				require.NoError(t, err)
				require.Equal(t, len(tc.data), int(numrows))

				result, err := conn.QueryRow(ctx, "SELECT l_partkey FROM lineitem WHERE l_orderkey = 1")
				require.NoError(t, err)
				partKey, ok := result[0].(int64)
				require.True(t, ok)
				require.Equal(t, int64(155190), partKey)

				results, err := conn.Query(ctx, "SELECT crdb_internal_mvcc_timestamp FROM lineitem")
				require.NoError(t, err)
				var lastts driver.Value
				firstTime := true
				vals := make([]driver.Value, 1)
				for {
					if err := results.Next(vals); err == io.EOF {
						break
					}
					require.NoError(t, err)
					if !firstTime {
