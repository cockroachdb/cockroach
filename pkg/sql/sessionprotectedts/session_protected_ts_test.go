...
func createSessionAndProtect(
	...
	getUserConn := func(t *testing.T, server serverutils.ApplicationLayerInterface) *gosql.DB {
		pgURL := url.URL{
			Scheme: "postgres",
			User:   url.UserPassword("foo", "hunter2"),
	...
	targetDB := getUserConn(t, s0)
	sqlutils.MakeSQLRunner(targetDB).Exec(t, `SELECT 1`)

	// Fetch the IDLE session's ID.
	var idleSessionID string
	sqlDB.QueryRow(t, `SELECT session_id FROM crdb_internal.cluster_sessions WHERE last_active_query LIKE 'SELECT 1%' ORDER BY session_start DESC LIMIT 1`).Scan(&idleSessionID)
...
func TestSessionProtectedTimestampReconciler(t *testing.T) {
	...
	_ = sqlDB.Exec(t, `
CREATE USER foo with password 'hunter2';
`)
	...
	// Create two sessions, these will be IDLE as they are running SELECT queries.
	...
	t.Run("reconcile", func(t *testing.T) {
		...
		go func() {
			defer wg.Done()
			aboutToSleep <- struct{}{}
			_, err := s3.Exec(`SELECT pg_sleep(3)`)
			require.NoError(t, err)
		}()
		<-aboutToSleep
