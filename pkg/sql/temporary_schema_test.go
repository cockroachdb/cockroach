...
func TestBatchedTempCleanupWithMixedObjectTypes(t *testing.T) {
	...
	_, err = conn.ExecContext(ctx, `SET experimental_enable_temp_tables=true`)
	require.NoError(t, err)
	// create_table_with_schema_locked=false is required because this test
	// creates a permanent table (perm_table) with a DEFAULT expression
	// referencing a temp sequence. Cleanup must modify the permanent table's
	// schema to remove the temp reference, which fails on schema-locked tables.
	_, err = conn.ExecContext(ctx, `SET create_table_with_schema_locked=false`)
	require.NoError(t, err)
	...
	// sequence. This exercises the cleanupTempSequenceDeps preHook path.
	_, err = conn.ExecContext(ctx, `
