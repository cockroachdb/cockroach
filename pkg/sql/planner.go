...
func (p *planner) initInternalExecutor(ctx context.Context) sqlutil.InternalExecutor {
	ie := p.ExecCfg().InternalExecutorProto.IeFactory(ctx, p.SessionData())
	ie.SetExtraTxnState(extraTxnStateUnderPlanner{
		...
		txnExplicit:            !p.EvalContext().TxnImplicit,
	})
	return ie
}
...
func (p *planner) QueryIteratorEx(
	...
	override sessiondata.InternalExecutorOverride,
	stmt string,
	qargs ...interface{}) (eval.InternalRows, error) {
	ie := p.initInternalExecutor(ctx)
...
// WithInternalExecutor let user run sql statements with an internal executor
// initialized under a planner context.
func (p *planner) WithInternalExecutor(
	...
	return ie.WithSyntheticDescriptors(
		p.execCfg.InternalExecutorProto.SyntheticDescs,
