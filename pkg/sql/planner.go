...
func makeInternalPlanner(ctxName string, txn *client.Txn, user string) *planner {
	p := makePlanner(ctxName)
	p.setTxn(txn)
	p.resetContexts()
	p.session.User = user
	return p
