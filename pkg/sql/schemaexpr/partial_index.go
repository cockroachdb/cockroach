...
func MakePartialIndexExprs(
	...
	semaCtx *tree.SemaContext,
) (_ map[sqlbase.IndexID]tree.TypedExpr, refColIDs sqlbase.TableColSet, _ error) {
	...
	exprs := make(map[sqlbase.IndexID]tree.TypedExpr, partialIndexCount)

	tn := tree.NewUnqualifiedTableName(tree.Name(tableDesc.Name))
	nr := newNameResolver(evalCtx, tableDesc.ID, tn, sqlbase.ColumnDescriptorsToReferences(cols))
