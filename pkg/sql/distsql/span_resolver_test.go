...
func TestSpanResolver(t *testing.T) {
	...
	replicas, err := resolveSpans(
		ctx, it, makeSpan(tableDesc, 0, 10000, kv.Ascending))
	if err != nil {
		t.Fatal(err)
	}
	...
	replicas, err = resolveSpans(
		ctx, it,
		makeSpan(tableDesc, 0, 9, kv.Ascending),
		makeSpan(tableDesc, 11, 19, kv.Ascending),
		makeSpan(tableDesc, 21, 29, kv.Ascending))
	if err != nil {
		t.Fatal(err)
	}
	if err := expectResolved(replicas,
...
func makeSpan(tableDesc *sqlbase.TableDescriptor, i, j int, dir kv.ScanDirection) spanWithDir {
