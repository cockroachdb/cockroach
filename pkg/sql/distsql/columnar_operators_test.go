...
func TestSorterAgainstProcessor(t *testing.T) {
	...
	//   spill to disk.
	// - 30720 - the in-memory sorter will hit the memory limit after spooling
	//   several batches, so the external sort will take over.
	for _, memoryLimit := range []int64{0, 3 * mon.DefaultPoolAllocationSize} {
		for run := 0; run < nRuns; run++ {
