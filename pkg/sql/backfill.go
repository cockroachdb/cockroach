	...
	for _, m := range tableDesc.Mutations {
		...
		case sqlbase.DescriptorMutation_ADD:
			...
			case *sqlbase.DescriptorMutation_Column:
				...
				if err := columnBackfillInTxn(ctx, txn, tc, evalCtx, tableDesc.TableDescriptor, traceKV); err != nil {
					return err
				}
				doneColumnBackfill = true

			case *sqlbase.DescriptorMutation_Index:
				if err := indexBackfillInTxn(ctx, txn, tableDesc.TableDescriptor, traceKV); err != nil {
					return err
				}
		...
		case sqlbase.DescriptorMutation_DROP:
			...
			case *sqlbase.DescriptorMutation_Index:
				if err := indexTruncateInTxn(ctx, txn, execCfg, tableDesc.TableDescriptor, traceKV); err != nil {
					return err
				}
