	...
	for _, m := range tableDesc.AllMutations() {
		...
		if m.Adding() {
			if col := m.AsColumn(); col != nil {
				...
				isValidating := false
				if c.IsCheck() {
					isValidating, err = shouldValidateCheckConstraint(tableDesc, c.Check())
					if err != nil {
						return err
					}
...
func shouldValidateCheckConstraint(
	...
	// And shard column constraints should have `Hidden==true`.
	if len(checkConstraint.ColumnIDs) != 1 || !checkConstraint.Hidden {
	...
	for _, index := range tableDesc.NonDropIndexes() {
		if index.IsSharded() {
			shard := index.GetSharded()
			if checkCol.GetName() == shard.Name {
				return false, nil
			}
		}
	}
