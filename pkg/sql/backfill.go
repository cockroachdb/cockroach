...
func (sc *SchemaChanger) distColumnBackfill(
	...
	backfillUpdateRunThresholdBytes uint64,
	filter backfill.MutationFilter,
) error {
	...
	for len(todoSpans) > 0 {
		...
		if err := sc.txn(ctx, func(
			...
			spec, err := initColumnBackfillerSpec(tableDesc, duration, chunkSize, backfillUpdateRunThresholdBytes, readAsOf)
			if err != nil {
				return err
			}
...
func (sc *SchemaChanger) truncateAndBackfillColumns(
	...
	if err := sc.distColumnBackfill(
		...
		uint64(columnBackfillUpdateRunThresholdBytes.Get(&sc.settings.SV)),
...
func columnBackfillInTxn(
	...
	for sp.Key != nil {
		...
		updateRunThresholdBytes := rowinfra.BytesLimit(columnBackfillUpdateRunThresholdBytes.Get(&evalCtx.Settings.SV))
		const alsoCommit = false
		sp.Key, err = backfiller.RunColumnBackfillChunk(
			ctx, txn, tableDesc, sp, scanBatchSize, updateRunThresholdBytes, alsoCommit, traceKV,
		)
		if err != nil {
			return err
		}
