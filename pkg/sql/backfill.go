		...
		grp.GoCtx(func(ctx context.Context) error {
			...
			return runHistoricalTxn(ctx, func(ctx context.Context, txn *kv.Txn, evalCtx *extendedEvalContext, ie sqlutil.InternalExecutor) error {
				...
				if ck := c.AsCheck(); ck != nil {
					...
					return errors.Errorf("unsupported constraint type: %s", c.String())
				}
				return nil
			})
		})
...
func runSchemaChangesInTxn(
		...
		if m.Adding() {
			...
			if col := m.AsColumn(); col != nil {
				...
				if c.AsCheck() != nil {
					...
					return errors.AssertionFailedf("unsupported constraint type: %s", c.String())
				}
			}
		}
	...
	for i, c := range constraintAdditionMutations {
		...
		if ck := c.AsCheck(); ck != nil {
			...
			return errors.AssertionFailedf("unknown constraint type: %s", c.String())
		}
