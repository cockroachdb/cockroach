...
func (sc *SchemaChanger) validateForwardIndexes(
	...
	for _, idx := range indexes {
		...
		grp.GoCtx(func(ctx context.Context) error {
			...
			if err := runHistoricalTxn(ctx, func(ctx context.Context, txn *kv.Txn, evalCtx *extendedEvalContext) error {
				...
				if idx.IsPartial() {
					filter = fmt.Sprintf(`WHERE %s`, idx.Predicate)
				}

				row, err := ie.QueryRowEx(ctx, "verify-idx-count", txn,
					sqlbase.InternalExecutorSessionDataOverride{},
					fmt.Sprintf(`SELECT count(1) FROM [%d AS t]@[%d]%s`, tableDesc.ID, idx.ID, filter))
				if err != nil {
					return err
				}
			...
			case <-tableCountsReady:
				...
				if idxLen != expectedCount {
					...
					// TODO(mgartner): This should be an assertion error,
					// similar to the error in validateInvertedIndexes. It is
					// not possible to reach this code in the case of duplicates
					// keys in a unique index. In that case, backfilling errors
					// before this function is called when trying to add the
					// error via kvserverbase.BulkAdder.
					return pgerror.Newf(
	...
	grp.GoCtx(func(ctx context.Context) error {
		...
		if err := runHistoricalTxn(ctx, func(ctx context.Context, txn *kv.Txn, evalCtx *extendedEvalContext) error {
			...
			query := fmt.Sprintf(`SELECT count(1)%s FROM [%d AS t]`, partialIndexCounts, tableDesc.ID)
