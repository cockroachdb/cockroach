...
func (sc *SchemaChanger) makeFixedTimestampRunner(readAsOf hlc.Timestamp) historicalTxnRunner {
	runner := func(ctx context.Context, retryable scTxnFn) error {
		return sc.fixedTimestampTxnWithExecutor(ctx, readAsOf, func(
			...
			ie.SetSessionData(evalCtx.SessionData())
			// TODO(janexing): Do we want the following so that the internal executor
			// and the eval ctx share the schema change job records and the job
			// collection?
			ie.SetExtraTxnState(&sqlutil.ExtraTxnStateArgs{
				Txn:                    txn,
				DescCollection:         evalCtx.Descs,
...
func (sc *SchemaChanger) fixedTimestampTxnWithExecutor(
	ctx context.Context,
	readAsOf hlc.Timestamp,
	retryable func(ctx context.Context, txn *kv.Txn, descriptors *descs.Collection, ie sqlutil.InternalExecutor) error,
) error {
	return sc.txnWithExecutor(ctx, func(ctx context.Context, txn *kv.Txn, descriptors *descs.Collection, ie sqlutil.InternalExecutor) error {
		...
		return retryable(ctx, txn, descriptors, ie)
	})
}
