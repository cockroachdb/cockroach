...
func (sc *SchemaChanger) dropConstraints(
	...
	if err := sc.txn(ctx, func(ctx context.Context, txn *kv.Txn, descriptors *descs.Collection) error {
		scTable, err := descriptors.GetMutableTableVersionByID(ctx, sc.descID, txn)
		if err != nil {
			return err
		}
		...
		if err := descriptors.AddUncommittedDescriptor(scTable); err != nil {
			return err
		}
		b := txn.NewBatch()
		for i := range constraints {
			...
			case descpb.ConstraintToUpdate_FOREIGN_KEY:
				...
				for j := range scTable.OutboundFKs {
					...
					backrefTable, err := descriptors.GetMutableTableVersionByID(ctx,
	...
	if err := sc.txn(ctx, func(
		...
		if tableDescs[sc.descID], err = descriptors.GetTableVersionByID(
			ctx, txn, sc.descID, tree.ObjectLookupFlagsWithRequired(),
		); err != nil {
			return err
		}
		for id := range fksByBackrefTable {
			if tableDescs[id], err = descriptors.GetTableVersionByID(
		...
		for i := range constraints {
			...
			case descpb.ConstraintToUpdate_FOREIGN_KEY:
				...
				if !foundExisting {
					...
					backrefTable, err := descriptors.GetMutableTableVersionByID(ctx, constraint.ForeignKey.ReferencedTableID, txn)
					if err != nil {
						return err
					}
