...
func numRangesInSpans(
	ctx context.Context, distSQLPlanner *DistSQLPlanner, spans []roachpb.Span,
) (int, error) {

	spanResolver := distSQLPlanner.spanResolver.NewSpanResolverIterator(nil /* txn */)
...
func (sc *SchemaChanger) distIndexBackfill(
	...
	updateJobProgress = func() error {
		...
		nRanges, err := numRangesInSpans(ctx, sc.distSQLPlanner, mu.updatedTodoSpans)
		if err != nil {
			return err
		}
		if origNRanges == -1 {
...
func (sc *SchemaChanger) distColumnBackfill(
	...
	for len(todoSpans) > 0 {
		...
		if err := sc.txn(ctx, func(
			...
			nRanges, err := numRangesInSpans(ctx, sc.distSQLPlanner, todoSpans)
			if err != nil {
				return err
			}
			if origNRanges == -1 {
