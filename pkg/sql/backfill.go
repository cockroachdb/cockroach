...
func ValidateInvertedIndexes(
	...
	for i, idx := range indexes {
		...
		grp.GoCtx(func(ctx context.Context) error {
			...
			// index. It is used in the query below that verify that the number
			// of entries in the inverted index is correct. If the index is an
			// expression index, the column name cannot be used because it is
			// inaccessible; the query would result in a "column does not exist"
			// error.
