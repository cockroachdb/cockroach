...
// - noBatchReuse, if true, indicates that a new coldata.Batch must be allocated
// on each Deserialize call.
func (d *Deserializer) Init(allocator *colmem.Allocator, typs []*types.T, noBatchReuse bool) error {
	d.allocator = allocator
	d.typs = typs
	d.noBatchReuse = noBatchReuse
...
// provided in Init. If noBatchReuse=true was passed in Init, then this method
// does **not** invalidate previously returned batches, and all the batches are
// still tracked by the allocator.
