...
// AppendTuples. If nil, columns at indices 0...len(typs)-1 will be used.
//
...
func (b *SpillingBuffer) AppendTuples(
	...
	if !memLimitReached && !b.alreadySpilled && !maxInMemTuplesLimitReached {
		...
		b.unlimitedAllocator.PerformOperation(b.bufferedTuples.ColVecs(), func() {
			for i, colIdx := range b.colIdxs {
				b.bufferedTuples.ColVec(i).Append(
					coldata.SliceArgs{
						Src:         batch.ColVec(colIdx),
						DestIdx:     originalLength,
						SrcStartIdx: startIdx,
						SrcEndIdx:   endIdx,
					},
				)
			}
			b.bufferedTuples.SetLength(originalLength+endIdx-startIdx)
		})
		return
	}
	...
	if b.diskQueue == nil {
		if b.fdSemaphore != nil {
			if err = b.fdSemaphore.Acquire(ctx, numFDsOpenAtAnyGivenTime); err != nil {
	...
	// Create a window into the correct columns and rows of the input batch before
	// enqueueing.
...
func (b *SpillingBuffer) Close(ctx context.Context) error {
	...
	if b.diskQueue != nil {
		...
		if b.fdSemaphore != nil {
			b.fdSemaphore.Release(numFDsOpenAtAnyGivenTime)
		}
		b.closed = true
	}
	return nil
...
func (b *SpillingBuffer) Reset(ctx context.Context) {
	...
	b.closed = false
	b.unlimitedAllocator.PerformOperation(b.bufferedTuples.ColVecs(), func() {
		b.bufferedTuples.DeepInternalReset(b.unlimitedAllocator)
	})
	...
	b.length = 0
	if b.dequeueScratch != nil {
		// Since the allocator will be tracking the size of the dequeue scratch
		// batch, we don't want to double-count its memory usage in the memory
		// reserved for disk operations, or we may spill to disk earlier than
		// necessary.
		b.diskReservedMem = int64(colmem.EstimateBatchSizeBytes(b.typs, coldata.BatchSize()))
		if b.lastDequeuedBatchMemUsage > b.diskReservedMem {
			// Reserve at least as much as is estimated by EstimateBatchSizeBytes.
			b.diskReservedMem = b.lastDequeuedBatchMemUsage
		}
		b.diskReservedMem += int64(b.diskQueueCfg.BufferSizeBytes)

		// Set the length to zero to ensure that the now-invalid data stored in this
		// batch is not accidentally used in subsequent calls to GetBatchWithTuple.
		b.dequeueScratch.SetLength(0)
	}
}
