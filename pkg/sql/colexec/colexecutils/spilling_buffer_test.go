...
func TestSpillingBuffer(t *testing.T) {
	...
	for _, memoryLimit := range []int64{
		...
		log.Infof(context.Background(), "MemoryLimit=%s/DiskQueueCacheMode=%d/AlwaysCompress=%t/NumBatches=%d/InMemTuplesLimited=%t",
			humanizeutil.IBytes(memoryLimit), diskQueueCacheMode, alwaysCompress, numBatches, setInMemTuplesLimit)
