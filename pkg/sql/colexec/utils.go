...
// overloadScratch is a utility struct that helps us avoid allocations of
// temporary decimals on every overloaded operation with them. In order for the
// templates to see it correctly, a local variable named `scratch` of this type
// must be declared before the inlined overloaded code.
type overloadScratch struct {
