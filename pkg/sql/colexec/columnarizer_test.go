...
func TestColumnarizerResetsInternalBatch(t *testing.T) {
	...
	c, err := NewColumnarizer(ctx, testAllocator, flowCtx, 0, input, ColumnarizerDefaultMode)
	if err != nil {
		t.Fatal(err)
	}
	c.Init()
...
func TestColumnarizerDrainsAndClosesInput(t *testing.T) {
	...
	c, err := NewColumnarizer(ctx, testAllocator, flowCtx, 0 /* processorID */, rb, ColumnarizerDefaultMode)
