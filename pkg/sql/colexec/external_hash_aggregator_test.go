...
func TestExternalHashAggregator(t *testing.T) {
	...
	for _, diskSpillingDisabled := range []bool{false, true} {
		HashAggregationDiskSpillingDisabled.Override(&flowCtx.Cfg.Settings.SV, diskSpillingDisabled)
		// Test the case in which the default memory is used as well as the case
		// in which the hash aggregator spills to disk.
		for _, spillForced := range []bool{false, true} {
			if diskSpillingDisabled && spillForced {
				continue
			}
