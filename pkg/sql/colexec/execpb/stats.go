...
const (
	...
	maxMemoryBytesTagSuffix = "mem.max"
	maxDiskBytesTagSuffix   = "disk.max"
)
...
func (vs *VectorizedStats) Stats() map[string]string {
	...
	return map[string]string{
		...
		maxMemoryBytesTagSuffix: fmt.Sprintf("%d", vs.MaxAllocatedMem),
		maxDiskBytesTagSuffix:   fmt.Sprintf("%d", vs.MaxAllocatedDisk),
	}
}
...
const (
	...
	maxMemoryBytesQueryPlanSuffix = "max memory allocated"
	maxDiskBytesQueryPlanSuffix   = "max disk allocated"
)
...
func (vs *VectorizedStats) StatsForQueryPlan() []string {
	...
	if vs.MaxAllocatedMem != 0 {
		stats = append(stats,
			fmt.Sprintf("%s: %s", maxMemoryBytesQueryPlanSuffix, humanizeutil.IBytes(vs.MaxAllocatedMem)))
	}

	if vs.MaxAllocatedDisk != 0 {
		stats = append(stats,
			fmt.Sprintf("%s: %s", maxDiskBytesQueryPlanSuffix, humanizeutil.IBytes(vs.MaxAllocatedDisk)))
	}
	return stats
}
