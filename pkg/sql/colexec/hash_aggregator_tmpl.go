		...
		for keyIdx, colIdx := range keyCols {
			...
			case _TYPES_T:
				...
				if lhsHasNull {
					...
					if rhsHasNull {
						_MATCH_LOOP(sel, lhs, rhs, aggKeyIdx, lhsNull, diff, firstDefiniteMatch, true, true)
					} else {
						_MATCH_LOOP(sel, lhs, rhs, aggKeyIdx, lhsNull, diff, firstDefiniteMatch, true, false)
					}
				} else {
					if rhsHasNull {
						_MATCH_LOOP(sel, lhs, rhs, aggKeyIdx, lhsNull, diff, firstDefiniteMatch, false, true)
					} else {
						_MATCH_LOOP(sel, lhs, rhs, aggKeyIdx, lhsNull, diff, firstDefiniteMatch, false, false)
					}
				}
			// {{end}}
			default:
