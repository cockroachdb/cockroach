...
func _MATCH_LOOP(
	...
	for selIdx, rowIdx := range sel {
		...
		if diffNull {
			diff[selIdx] = diff[selIdx] || diffNull
			break
		}
		// {{end}}
		// {{end}}
...
// unmatched tuples and a boolean to indicate whether or not if there is any
// matching tuples. It directly write the result of matched tuples into the
// selection vector of 'b' and set the length of the batch to the number of
