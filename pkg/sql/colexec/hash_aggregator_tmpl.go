...
// well as the index of the first distinct group found.
//
...
//
// If partialOrder is false, there are no guarantees on ordering and all input
// tuples are processed before emitting any data.
	...
	for {
		...
		case hashAggregatorBuffering:
					...
					if op.bufferingState.splitGroup {
						...
						if sel != nil {
							hasDistinct, idx = findSplit(op, op.bufferingState.unprocessedIdx,
								op.bufferingState.pendingBatch.Length(), sel,
			...
			if partialOrder {
				...
				// batches in order.
				if op.bufferingState.splitGroup || op.bufferingState.tuples.Length()+toBuffer >= hashAggregatorMaxBuffered {
			...
			if op.curOutputBucketIdx >= len(op.buckets) {
				if partialOrder {
					if op.bufferingState.pendingBatch.Length() > 0 {
						...
						// Clear the emitted tuples from trackingState.tuples. Add back
						// unprocessed tuples from the pending batch that were not emitted.
						if op.inputTrackingState.tuples != nil {
							op.inputTrackingState.tuples.Reset(op.Ctx)
							op.inputTrackingState.zeroBatchEnqueued = false
							if op.bufferingState.unprocessedIdx < op.bufferingState.pendingBatch.Length() {
								b := colexecutils.NewAppendOnlyBufferedBatch(op.accountingHelper.Allocator, op.inputTypes, nil /* colsToStore */)
								b.AppendTuples(op.bufferingState.pendingBatch, op.bufferingState.unprocessedIdx, op.bufferingState.pendingBatch.Length())
								op.inputTrackingState.tuples.Enqueue(op.Ctx, b)
							}
						}
					} else {
						op.state = hashAggregatorDone
