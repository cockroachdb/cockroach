	...
	for {
		...
		case stateDecodeFirstKVOfRow:
			...
			if rf.table.isSecondaryIndex && rf.table.index.Unique {
				// TODO (rohany): I'm not sure if there is a way to get around this now that
				//  secondary indexes can have multiple keys for a single row. Ask around to
				//  see if there is a better way of doing this.
				foundNull := false
				for _, ord := range rf.table.indexColOrdinals {
					if rf.machine.colvecs[ord].Nulls().NullAt(rf.machine.rowIdx) {
						foundNull = true
						break
					}
				}
