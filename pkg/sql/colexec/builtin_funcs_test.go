		...
		t.Run(tc.desc, func(t *testing.T) {
			runTests(t, []tuples{tc.inputTuples}, tc.outputTuples, orderedVerifier,
				func(input []Operator) (Operator, error) {
					return createTestProjectingOperator(ctx, flowCtx, input[0], tc.inputTypes, tc.expr)
				})
		})
	}
}
...
func benchmarkBuiltinFunctions(b *testing.B, useSelectionVector bool, hasNulls bool) {
	...
	op, err := createTestProjectingOperator(ctx, flowCtx, source, []types.T{*types.Int}, "abs(@1)")
