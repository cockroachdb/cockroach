		...
		for _, memoryLimit := range []int64{10 << 10 /* 10 KiB */, 1<<20 + int64(rng.Intn(64<<20)) /* 1 MiB up to 64 MiB */} {
			...
			if testReuseCache {
				...
				if diskQueueReadMode == colcontainer.DiskQueueReadModeRewindable {
					// Clear-and-reuse cache mode is incompatible with rewindable read
					// mode.
			...
			t.Run(fmt.Sprintf("%sMemoryLimit=%s/DiskQueueCacheMode=%d/AlwaysCompress=%t",
				prefix, humanizeutil.IBytes(memoryLimit), diskQueueCacheMode, alwaysCompress), func(t *testing.T) {
				// Create random input.
