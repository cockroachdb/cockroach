...
func TestSortedDistinct(t *testing.T) {
	...
	tcs := []struct {
	...
	}{
		{
			...
			colTypes:     []colphystypes.T{colphystypes.Float64, colphystypes.Int64, colphystypes.Bytes},
			numCols:      4,
			tuples: tuples{
				{nil, nil, nil, nil},
				{nil, nil, nil, nil},
...
func BenchmarkSortedDistinct(b *testing.B) {
	...
	distinct, err := NewOrderedDistinct(source, []uint32{1, 2}, []colphystypes.T{colphystypes.Int64, colphystypes.Int64, colphystypes.Int64})
	if err != nil {
		b.Fatal(err)
	}
