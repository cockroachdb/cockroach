...
func (s *JoinProbeState) PrepareForNewBatch(ht *HashTable, batch coldata.Batch) {
	...
	if cap(s.Buckets) < batchSize {
		s.Buckets = make([]uint64, batchSize)
	} else {
	...
	ht.ProbeScratch.SetupLimitedSlices(batchSize, ht.BuildMode)
}
...
func (s *JoinProbeState) PrepareForCollecting(batchSize int) {
	...
	if s.Type.IsLeftAntiOrExceptAll() || s.Type == descpb.IntersectAllJoin {
		...
		// hashJoiner.collectSingleMatch method that only uses the ProbeIdx
		// slice.
		return
	}
	if s.Type.IsLeftOuterOrFullOuter() {
		if cap(s.ProbeRowUnmatched) < batchSize {
			s.ProbeRowUnmatched = make([]bool, batchSize)
		} else {
			s.ProbeRowUnmatched = s.ProbeRowUnmatched[:batchSize]
		}
	}
