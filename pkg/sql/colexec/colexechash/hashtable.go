...
type hashChains struct {
	...
	// hash table at the moment plus one.
	Next []uint64
}
...
type hashTableProbeBuffer struct {
	...
	// collisions, then ToCheck will be updated to {1, 2} and GroupID will be
	// advanced to the next elements in the corresponding hash chains.
	ToCheck []uint64

	// GroupID stores keyID of the "candidate" tuple against which the equality
	// check is performed on the current iteration for tuples from the probing
	// batch included in ToCheck. The result of the comparison is stored in
	// differs and/or distinct slices.
	GroupID []uint64
	...
	// HeadID stores keyID of the first tuple (i.e. the head of the
	// corresponding hash chain) that matched with the probing batch at any
	// given index.
	//
	// Where the hash chains come from differs depending on the context:
	// - if the hash table is used by the hash joiner, then the hash chains
	// describe the partitioning of the build (right) side table, and the
	// probing batch comes from the left side;
	// - if the hash table is used by the hash aggregator, then these hash
	// chains describe the partitioning of the probing batch or of already found
	// grouping buckets, depending on the phase of the algorithm. See an
	// extensive comment on hashAggregator.onlineAgg for more details.
	//
	// The unordered distinct uses HeadID slightly differently:
	// - when duplicates are being removed from the probing batch, HeadID[i] set
	// to the first tuple that matches i'th tuple. Since i'th tuple always
	// matches itself (and possibly others), HeadID values will never be zero;
	// - once the batch only contains unique tuples, non-zero HeadID value is
	// set for a particular tuple once the tuple is determined to not have
	// duplicates with any tuples already in the hash table.
	HeadID []uint64
...
type HashTable struct {
	...
	// moment. This number will increase as more tuples are added into the hash
	// table.
	numBuckets uint64
...
// then probing de-duplicated batch against the hash table (which contains
// already emitted distinct tuples). Duplicates of the emitted tuples are also
// removed the batch, and if there are any tuples left, all of them are distinct
// and, thus, are appended into the hash table and outputted.
//
...
//         whereas tuple 0 (-8) is distinct (because it's GroupID is 0), no more
//         tuples to check:
//              ToCheck = []
//              HeadID  = [1, 0]
