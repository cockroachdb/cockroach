...
var HashTableInitialToCheck []uint32

func init() {
	HashTableInitialToCheck = make([]uint32, coldata.MaxBatchSize)
	for i := range HashTableInitialToCheck {
		HashTableInitialToCheck[i] = uint32(i)
	}
}
...
func probeBufferInternalMaxMemUsed() int64 {
	...
	return memsize.Uint32*int64(5*coldata.BatchSize()) + memsize.Bool*int64(2*coldata.BatchSize())
}
