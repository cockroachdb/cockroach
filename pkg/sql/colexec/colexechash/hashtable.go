...
func probeBufferInternalMaxMemUsed() int64 {
	...
	return memsize.Int64*int64(5*coldata.BatchSize()) + memsize.Bool*int64(2*coldata.BatchSize())
}
...
// corresponding memory reservations. Notably, this doesn't release the Vals
// append-only batch.
// TODO(yuzefovich): lose the reference to the Vals as well, when possible.
func (ht *HashTable) Release() {
	// Note that we don't release references to Same and Visited slices since
	// the user of the HashTable accounts for it.
	ht.BuildScratch = hashChains{}
	if ht.unlimitedSlicesNumUint64AccountedFor != 0 {
		ht.allocator.ReleaseMemory(memsize.Uint64 * ht.unlimitedSlicesNumUint64AccountedFor)
		ht.unlimitedSlicesNumUint64AccountedFor = 0
	}
