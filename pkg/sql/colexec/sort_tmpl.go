...
func isSorterSupported(t phystypes.T, dir execinfrapb.Ordering_Column_Direction) bool {
	switch t {
...
func newSingleSorter(
	t phystypes.T, dir execinfrapb.Ordering_Column_Direction, hasNulls bool,
) colSorter {
	switch t {
	// {{range $typ, $ := . }} {{/* for each type */}}
	case _TYPES_T:
