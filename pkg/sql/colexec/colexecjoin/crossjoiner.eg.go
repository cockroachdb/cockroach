			...
			if sel := currentBatch.Selection(); sel != nil {
				{
					...
					for colIdx := range b.left.types {
						// Get a fresh copy of the builder state so that each column has a fresh
						// start.
						bs := b.builderState.left
						if colIdx == len(b.left.types)-1 {
							// This is the last column. We need to update the builder state so
							// that we continue where we left off on the next call.
							defer func() {
								b.builderState.left = bs
							}()
						}
