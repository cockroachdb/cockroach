...
// batchStatsCollector is a helper used by collector implementations.
//
...
type vectorizedStatsCollectorImpl struct {
	...
	// idTagKey is the span tag key that will be set to OperatorID.
	idTagKey string
...
func (vsc *vectorizedStatsCollectorImpl) finish() *execstatspb.OperatorStats {
	numBatches, numTuples, time := vsc.batchInfoCollector.finish()

	s := &execstatspb.OperatorStats{OperatorID: vsc.operatorID}
	...
	if ioTime {
		...
		// themselves).
		s.KV.RowsRead.Set(uint64(vsc.ioReader.GetRowsRead()))
...
// finish returns the collected stats.
func (nvsc *networkVectorizedStatsCollectorImpl) finish() *execstatspb.OperatorStats {
	numBatches, numTuples, time := nvsc.batchInfoCollector.finish()

	s := &execstatspb.OperatorStats{OperatorID: nvsc.operatorID}

	s.NetRx.Latency = nvsc.latency
	s.NetRx.WaitTime = time
	s.NetRx.DeserializationTime = nvsc.networkReader.GetDeserializationTime()
	s.NetRx.RowsReceived.Set(uint64(nvsc.networkReader.GetRowsRead()))
...
func createStatsSpan(
	...
	span.SetTag(idTagKey, stats.OperatorID)
