...
func TestIsNullProjOperator(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			negate:       false,
			// The input contains only NULLs already.
	...
	for _, c := range testCases {
		t.Run(c.desc, func(t *testing.T) {
			...
			runner(t, []tuples{c.inputTuples}, nil /* typs */, c.outputTuples, orderedVerifier, opConstructor)
		})
	}
}

func TestIsNullSelOperator(t *testing.T) {
	defer leaktest.AfterTest(t)()
	ctx := context.Background()
	st := cluster.MakeTestingClusterSettings()
	evalCtx := tree.MakeTestingEvalContext(st)
