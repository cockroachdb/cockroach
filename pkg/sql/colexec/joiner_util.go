...
func newFilterFeedOperator(allocator *Allocator, inputTypes []colphystypes.T) *filterFeedOperator {
...
func newJoinerFilter(
	allocator *Allocator,
	leftSourceTypes []colphystypes.T,
	rightSourceTypes []colphystypes.T,
...
func (f *joinerFilter) setInputBatch(lBatch, rBatch coldata.Batch, lIdx, rIdx int) {
	...
	setOneSide := func(colOffset int, batch coldata.Batch, sourceTypes []colphystypes.T, idx int) {
		sel := batch.Selection()
		if sel != nil {
			idx = int(sel[idx])
		}
