...
func OrderedDistinctColsToOperators(
	input Operator, distinctCols []uint32, typs []colphystypes.T,
) (Operator, []bool, error) {
...
// input columns with the given colphystypes.
...
func newSingleOrderedDistinct(
	input Operator, distinctColIdx int, outputCol []bool, t colphystypes.T,
) (Operator, error) {
	switch t {
	// {{range .}}
	case _TYPES_T:
...
func newPartitioner(t colphystypes.T) (partitioner, error) {
	switch t {
	// {{range .}}
	case _TYPES_T:
