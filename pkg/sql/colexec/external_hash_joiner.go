...
func newExternalHashJoiner(
	...
	// partitions (8 for the left and 8 for the right). This is a hard maximum
	// of partitions that will be used by the external hash joiner. Below we
	// check whether we have enough RAM to support the caches of this number of
	// partitions.
	numBucketsPerSide := fdSemaphore.GetLimit() / 32
...
StateChanged:
	for {
		...
		case externalHJRecursivePartitioning:
			...
			// TODO(yuzefovich): we calculated hj.numBuckets based on the available
			// RAM for the caches of disk queues assuming that we will be actively
			// using partitions (i.e. file descriptors) on both sides, but here we're
			// repartitioning one side at a time, so we actually could increase the
			// number of buckets.
			for parentPartitionIdx, parentPartitionInfo := range hj.partitionsToJoin {
				for _, side := range []joinSide{leftSide, rightSide} {
					...
					// We're done reading from this partition.
					if err := partitioner.CloseAllOpenReadFileDescriptors(); err != nil {
						execerror.VectorizedInternalPanic(err)
					}
				}
		...
		case externalHJJoinNewPartition:
			...
			// repartition. Close any open read file descriptors from previous joins.
