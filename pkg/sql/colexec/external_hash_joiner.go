...
type externalHashJoiner struct {
	...
	// This chain is lazily created using diskBackedSortMergeCreator right
	// before transitioning to externalHJSortMergeNewPartition state for the
	// first time.
	diskBackedSortMerge        resettableOperator
	diskBackedSortMergeCreator func() resettableOperator
...
func newExternalHashJoiner(
	...
	createReusableDiskBackedSorter func(input Operator, inputTypes []coltypes.T, orderingCols []execinfrapb.Ordering_Column) (Operator, error),
	numForcedRepartitions int,
) Operator {
	...
	diskBackedSortMergeCreator := func() resettableOperator {
		...
		leftPartitionSorter, err := createReusableDiskBackedSorter(
			leftJoinerInput, spec.left.sourceTypes, leftOrdering,
		)
		if err != nil {
		...
		rightPartitionSorter, err := createReusableDiskBackedSorter(
			rightJoinerInput, spec.right.sourceTypes, rightOrdering,
		)
		if err != nil {
			execerror.VectorizedInternalPanic(err)
		}
		diskBackedSortMerge, err := newMergeJoinOp(
			// TODO(yuzefovich): maybe different cache mode for disk queues.
			unlimitedAllocator, externalOperatorUsageModeReusable, memoryLimit, diskQueueCfg,
	...
	ehj := &externalHashJoiner{
		...
		diskBackedSortMergeCreator: diskBackedSortMergeCreator,
	}
		...
		case externalHJJoinNewPartition:
			...
			if len(hj.partitionsToJoinUsingInMemHash) == 0 {
				...
				if len(hj.partitionsToJoinUsingSortMerge) > 0 {
					...
					// join strategy.
					hj.diskBackedSortMerge = hj.diskBackedSortMergeCreator()
