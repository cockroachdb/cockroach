...
func newExternalHashJoiner(
	...
	if cfg.BufferSizeBytes > 0 {
		numDiskQueuesThatFit := diskQueuesTotalMemLimit / cfg.BufferSizeBytes
	...
	for {
		...
		case externalHJRecursivePartitioning:
			...
			if hj.cfg.BufferSizeBytes > 0 {
				...
				diskQueuesUsedMem := 2 * len(hj.partitionsToJoin) * hj.cfg.BufferSizeBytes
				diskQueuesAvailableMem := hj.memState.diskQueuesTotalMemLimit - diskQueuesUsedMem
				numAvailableDiskQueues := diskQueuesAvailableMem / hj.cfg.BufferSizeBytes
			...
			for parentPartitionIdx, parentPartitionInfo := range hj.partitionsToJoin {
				for _, side := range []joinSide{leftSide, rightSide} {
					...
					for {
						if err := partitioner.Dequeue(parentPartitionIdx, batch); err != nil {
							execerror.VectorizedInternalPanic(err)
						}
						if batch.Length() == 0 {
							break
