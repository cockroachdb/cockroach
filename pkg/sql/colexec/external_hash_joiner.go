...
const (
	...
	// After repartitioning, the operator transitions to
...
func newExternalHashJoiner(
	...
	ehj.recursiveScratch.leftBatch = allocator.NewMemBatch(spec.left.sourceTypes)
	...
	if sameSourcesSchema {
		...
		ehj.recursiveScratch.rightBatch = allocator.NewMemBatch(spec.right.sourceTypes)
	}
	return ehj
}
...
func (hj *externalHashJoiner) partitionBatch(
	...
	for idx, sel := range selections {
		...
		if len(sel) > 0 {
			...
			hj.allocator.PerformOperation(colVecs, func() {
...
func (hj *externalHashJoiner) Next(ctx context.Context) coldata.Batch {
	for {
		...
		case externalHJRecursivePartitioning:
			...
			// We also need to update how many buckets we're using when
			// repartitioning.
			hj.numBuckets = externalHJMaxNumBuckets
			if hj.memState.memUsagePerDiskQueue > 0 {
				...
				if int(numAvailableDiskQueues)/2 < hj.numBuckets {
					hj.numBuckets = int(numAvailableDiskQueues) / 2
				}
				if hj.numBuckets < 2 {
					...
					if partitionInfo, ok := hj.partitionsToJoin[hj.partitionIdxOffset+idx]; ok {
						...
						if before > 0 {
							...
							if sizeDecrease < externalHJRecursivePartitioningSizeDecreaseThreshold {
								...
								// sort + merge join.
								execerror.VectorizedInternalPanic(errors.Errorf("recursive partitioning didn't sufficiently decrease "+
									"the size of the partition: before %s, after %s", humanize.Bytes(uint64(before)), humanize.Bytes(uint64(after))))
							}
						}
					}
				}
