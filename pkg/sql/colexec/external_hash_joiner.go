...
func NewExternalHashJoiner(
	...
	inMemMainOpConstructor := func(partitionedInputs []*partitionerToOperator) ResettableOperator {
		// Note that the external operator will make sure that partitions to
		// join using in-memory hash joiner fit under the limit, so we use the
		// same unlimited allocator for both buildSideAllocator and
