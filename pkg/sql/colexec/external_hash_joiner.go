...
const externalHJNumPartitions = 1 << 12
...
func (hj *externalHashJoiner) partitionBatch(
	...
	for partitionIdx, sel := range selections {
		if len(sel) > 0 {
			...
			if err := partitioner.Enqueue(partitionIdx, batch); err != nil {
...
func (hj *externalHashJoiner) Next(ctx context.Context) coldata.Batch {
	for {
		...
		case externalHJPartitioning:
			...
			hj.partitionBatch(ctx, leftBatch, hj.spec.left, hj.leftPartitioner)
			hj.partitionBatch(ctx, rightBatch, hj.spec.right, hj.rightPartitioner)
