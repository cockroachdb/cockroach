...
func TestEncDatumRowsToColVecBool(t *testing.T) {
	...
	vec := testAllocator.NewMemColumn(phystypes.Bool, 2)
	ct := types.Bool
	...
	expected := testAllocator.NewMemColumn(phystypes.Bool, 2)
...
func TestEncDatumRowsToColVecInt16(t *testing.T) {
	...
	vec := testAllocator.NewMemColumn(phystypes.Int16, 2)
	if err := EncDatumRowsToColVec(testAllocator, rows, vec, 0 /* columnIdx */, types.Int2, &alloc); err != nil {
		t.Fatal(err)
	}
	expected := testAllocator.NewMemColumn(phystypes.Int16, 2)
	expected.Int16()[0] = 17
	expected.Int16()[1] = 42
	if !reflect.DeepEqual(vec, expected) {
		t.Errorf("expected vector %+v, got %+v", expected, vec)
...
func TestEncDatumRowsToColVecString(t *testing.T) {
	...
	vec := testAllocator.NewMemColumn(phystypes.Bytes, 2)
	for _, width := range []int32{0, 25} {
		...
		expected := testAllocator.NewMemColumn(phystypes.Bytes, 2)
...
func TestEncDatumRowsToColVecDecimal(t *testing.T) {
	...
	expected := testAllocator.NewMemColumn(phystypes.Decimal, 3)
	...
	vec := testAllocator.NewMemColumn(phystypes.Decimal, 3)
	ct := types.Decimal
	if err := EncDatumRowsToColVec(testAllocator, rows, vec, 0 /* columnIdx */, ct, &alloc); err != nil {
		t.Fatal(err)
	}
