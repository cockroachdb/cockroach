...
type orderedAggregator struct {
	...
	aggCols  [][]uint32
	aggTypes [][]colphystypes.T

	outputTypes []colphystypes.T
...
func makeAggregateFuncs(
	allocator *Allocator, aggTyps [][]colphystypes.T, aggFns []execinfrapb.AggregatorSpec_Func,
) ([]aggregateFunc, []colphystypes.T, error) {
	funcs := make([]aggregateFunc, len(aggFns))
	outTyps := make([]colphystypes.T, len(aggFns))
...
func extractAggTypes(aggCols [][]uint32, colTypes []colphystypes.T) [][]colphystypes.T {
	aggTyps := make([][]colphystypes.T, len(aggCols))

	for aggIdx := range aggCols {
		aggTyps[aggIdx] = make([]colphystypes.T, len(aggCols[aggIdx]))
