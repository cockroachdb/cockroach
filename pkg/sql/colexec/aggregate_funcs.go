	...
	for _, aggFn := range spec.Aggregations {
		if !isAggOptimized(aggFn.Func) {
			for _, vecIdx := range aggFn.ColIdx {
				idxsToConvert.Add(int(vecIdx))
			}
		}
	}
	vecIdxsToConvert := make([]int, 0, idxsToConvert.Len())
	for idx, ok := idxsToConvert.Next(0); ok; idx, ok = idxsToConvert.Next(idx + 1) {
		vecIdxsToConvert = append(vecIdxsToConvert, idx)
	}
...
func ProcessAggregations(
	...
	for i, aggFn := range aggregations {
		constructors[i], constArguments[i], outputTypes[i], err = execinfrapb.ProcessAggFnSpec(
