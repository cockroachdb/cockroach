...
type ordinalityOp struct {
	...
	// the ordinal number.
	ordinalityCol       int
	ordinalityColStatus colAddedStatus
...
func (c *ordinalityOp) Next(ctx context.Context) coldata.Batch {
	...
	if c.ordinalityColStatus == colNotAdded {
		c.ordinalityCol = bat.Width()
		c.allocator.AddColumn(bat, coltypes.Int64, c.ordinalityCol)
		c.ordinalityColStatus = colAdded
	}
