...
func (s *externalSorter) Next() (coldata.Batch, *execinfrapb.ProducerMetadata) {
	for {
		...
		case externalSorterRepeatedMerging:
			...
			s.merger.Init(s.Ctx)
			s.numPartitions -= n
			for b, meta := s.merger.Next(); ; b, meta = s.merger.Next() {
				if buildutil.CrdbTestBuild && meta != nil {
					colexecerror.InternalError(errors.AssertionFailedf("non-nil meta from merger: %v", meta))
				}
				partitionDone := s.enqueue(b)
				if b.Length() == 0 || partitionDone {
		...
		case externalSorterEmitting:
			// TODO: can meta be non-nil here?
			b, meta := s.emitter.Next()
			if meta != nil {
				return nil, meta
			}
			if b.Length() == 0 {
