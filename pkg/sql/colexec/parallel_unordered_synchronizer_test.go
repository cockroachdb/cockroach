...
func TestParallelUnorderedSynchronizer(t *testing.T) {
	...
	type parallelUnorderedSynchronizerTerminationScenario int
	const (
		// parallelUnorderedSynchronizerGracefulTermination is a termination
		// scenario where the synchronizer terminates gracefully.
		parallelUnorderedSynchronizerGracefulTermination parallelUnorderedSynchronizerTerminationScenario = iota
		// parallelUnorderedSynchronizerContextCanceled is a termination scenario
		// where a cancellation requests that a synchronizer terminates.
		parallelUnorderedSynchronizerContextCanceled
		// parallelUnorderedSynchronizerPrematureDrainMeta is a termination scenario
		// where DrainMeta is called prematurely on the synchronizer.
		parallelUnorderedSynchronizerPrematureDrainMeta
		// parallelUnorderedSynchronizerMaxTerminationScenario should be at the end
		// of the termination scenario list so that it can be used as an upper bound
		// to generate any other termination scenario.
		parallelUnorderedSynchronizerMaxTerminationScenario
	)
	terminationScenario := parallelUnorderedSynchronizerTerminationScenario(rng.Intn(int(parallelUnorderedSynchronizerMaxTerminationScenario)))
	...
	t.Run(fmt.Sprintf("numInputs=%d/numBatches=%d/terminationScenario=%d", numInputs, numBatches, terminationScenario), func(t *testing.T) {
		if terminationScenario == parallelUnorderedSynchronizerContextCanceled {
			wg.Add(1)
		...
		for {
			...
			if terminationScenario == parallelUnorderedSynchronizerPrematureDrainMeta && batchesReturned < expectedBatchesReturned {
				// Call DrainMeta before the input is finished. Intentionally call Next
				// even though it's not technically supported to ensure that a
			...
			if b.Length() == 0 {
				...
				if terminationScenario == parallelUnorderedSynchronizerGracefulTermination {
					meta := s.DrainMeta(ctx)
					require.Equal(t, len(inputs), len(meta), "metadata length mismatch, returned metadata is: %v", meta)
				}
				break
		...
		if terminationScenario != parallelUnorderedSynchronizerContextCanceled && terminationScenario != parallelUnorderedSynchronizerPrematureDrainMeta {
