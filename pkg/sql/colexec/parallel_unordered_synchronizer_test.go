...
func TestParallelUnorderedSynchronizer(t *testing.T) {
	...
	for i := range inputs {
		...
		if i < numInputs-1 {
			s := colexecop.NewRepeatableBatchSource(
				testAllocator,
	...
	t.Run(fmt.Sprintf("numInputs=%d/numBatches=%d/terminationScenario=%d", numInputs, numBatches, terminationScenario), func(t *testing.T) {
		...
		for {
			...
			if terminationScenario == synchronizerPrematureDrainMeta && batchesReturned < expectedBatchesReturned {
				// Call DrainMeta before the input is finished. Make sure that
				// the last input is pre-emptied and isn't blocked for the whole
				// blockDuration period.
				start := timeutil.Now()
				meta := s.DrainMeta()
				actualDuration := timeutil.Since(start)
				require.True(t, actualDuration < blockDuration/2)
