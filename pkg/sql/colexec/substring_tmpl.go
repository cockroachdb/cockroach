...
func newSubstringOperator(
	...
	if startType.Family() != types.IntFamily {
		colexecerror.InternalError(fmt.Sprintf("non-int start argument type %s", startType.Name()))
	}
	if lengthType.Family() != types.IntFamily {
		colexecerror.InternalError(fmt.Sprintf("non-int length argument type %s", lengthType.Name()))
	}
