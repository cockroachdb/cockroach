...
func wrapRowSources(
	...
	for i := range inputs {
		...
		if c, ok := inputs[i].Root.(*colexec.Columnarizer); ok {
			...
			toWrapInput := colexec.NewMaterializer(
				getStreamingAllocator(ctx, args, flowCtx),
				flowCtx,
				processorID,
				inputs[i],
				inputTypes[i],
	...
	streamingAllocator := getStreamingAllocator(ctx, args, flowCtx)
	var c *colexec.Columnarizer
	if proc.MustBeStreaming() {
		c = colexec.NewStreamingColumnarizer(batchAllocator, streamingAllocator, flowCtx, processorID, toWrap)
	} else {
		c = colexec.NewBufferingColumnarizer(batchAllocator, streamingAllocator, flowCtx, processorID, toWrap)
	}
	return c, nil
}
