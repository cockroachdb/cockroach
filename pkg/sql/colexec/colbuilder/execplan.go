...
func NewColOperator(
	...
	if err = supportedNatively(spec); err != nil {
		...
		case core.Aggregator != nil:
			...
			if len(aggSpec.Aggregations) == 0 {
				...
				result.Root, err = colexecutils.NewFixedNumTuplesNoInputOp(streamingAllocator, numTuples, inputs[0].Root), nil
	...
	err = ppr.planPostProcessSpec(ctx, flowCtx, evalCtx, args, post, factory, &r.Releasables, args.Spec.EstimatedRowCount)
	if err != nil {
...
func (r *postProcessResult) planPostProcessSpec(
	...
	if post.Projection {
		...
		var err error
		for i := range exprs {
			exprs[i], err = args.ExprHelper.ProcessExpr(post.RenderExprs[i], semaCtx, flowCtx.EvalCtx, r.ColumnTypes)
			if err != nil {
				return err
			}
		}
