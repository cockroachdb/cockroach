...
func wrapRowSources(
	...
	columnarizerMode := colexec.ColumnarizerDefaultMode
	if _, mustBeStreaming := toWrap.(execinfra.StreamingProcessor); mustBeStreaming {
		columnarizerMode = colexec.ColumnarizerStreamingMode
	}
	op, err := colexec.NewColumnarizer(
		ctx, colmem.NewAllocator(ctx, acc, factory), flowCtx, processorID, toWrap, columnarizerMode,
	)
