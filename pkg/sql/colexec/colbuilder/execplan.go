...
func supportedNatively(spec *execinfrapb.ProcessorSpec) error {
	...
	default:
		return errUnsupportedNativelyCore
	}
}

var (
	errUnsupportedNativelyCore        = errors.New("unsupported processor core")
...
func canWrap(mode sessiondatapb.VectorizeExecMode, spec *execinfrapb.ProcessorSpec) error {
	...
	// We do not wrap MetadataTest{Sender,Receiver} because of the way metadata
	// is propagated through the vectorized flow - it is drained at the flow
	...
	// rowexec.tableReader which the ColBatchScan will attempt to do and will
	// fail. The inverted column is not of the same type as the original column
	// that was indexed (e.g. for geometry, the inverted column contains an
	// int, and for arrays the inverted column contains the array element
	// type). See #50695.
	case spec.Core.InvertedFilterer != nil:
		return errInvertedFiltererWrap
	// All processor cores below don't implement execinfra.RowSource interface,
	// so we currently can't wrap them into the vectorized flow.
...
func NewColOperator(
	...
	if log.V(2) {
		log.Eventf(ctx, "planning col operator for spec %q", spec)
	}
	...
	if err = supportedNatively(spec); err != nil {
		...
		if log.V(1) {
			log.Eventf(ctx, "planning a wrapped processor because %s", err.Error())
		}
...
func (r *postProcessResult) planPostProcessSpec(
	...
	if post.Projection {
		...
		if log.V(2) {
			log.Eventf(ctx, "planning render expressions %+v", post.RenderExprs)
		}
		semaCtx := flowCtx.TypeResolverFactory.NewSemaContext(flowCtx.EvalCtx.Txn)
