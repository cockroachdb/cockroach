...
func TestHashAggregator(t *testing.T) {
	...
	for _, numOfHashBucket := range []uint64{0 /* no limit */, 1, uint64(coldata.BatchSize())} {
		for _, tc := range tcs {
			if err := tc.init(); err != nil {
				t.Fatal(err)
			}
			t.Run(fmt.Sprintf("numOfHashBuckets=%d", numOfHashBucket), func(t *testing.T) {
				runTests(t, []tuples{tc.input}, tc.expected, unorderedVerifier, func(sources []Operator) (Operator, error) {
					...
					a.(*hashAggregator).testingKnobs.numOfHashBuckets = numOfHashBucket
					return a, err
				})
			})
		}
