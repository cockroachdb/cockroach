...
func TestAggregatorRandom(t *testing.T) {
	...
	for _, groupSize := range []int{1, 2, int(coldata.BatchSize()) / 4, int(coldata.BatchSize()) / 2} {
		...
		// TODO(@Azhng): modify the unorderedVerifier to sort before compare actual
		//               and expected output, since it reduce the complexity from
		//               O(n^2) to O(nlogn)
		for _, numInputBatches := range []int{1, 2, 10} {
