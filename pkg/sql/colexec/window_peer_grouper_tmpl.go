...
// partition and are equal on the ordering columns. If ordCols is empty, than
// all tuples within the partition are peers.
...
func NewWindowPeerGrouper(
	...
	allPeers := len(ordCols) == 0
	var distinctCol []bool
	if !allPeers {
		orderIdxs := make([]uint32, len(ordCols))
		for i, ordCol := range ordCols {
