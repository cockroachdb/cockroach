...
const (
	...
	// only populate first and next array for each hash bucket.
	hashTableFullMode hashTableMode = iota

	// hashTableDistinctMode is the mode where hashTable only buffer distinct
	// tuples. In this mode,  user of the hashTable should directly copy tuples
	// from the buffering batch ht.vals.
...
func newHashTable(
	...
	if mode == hashTableDistinctMode {
		...
		ht.probeScratch.hashBuffer = make([]uint64, coldata.BatchSize()+1)
		ht.probeScratch.distinct = make([]bool, coldata.BatchSize()+1)
...
func (ht *hashTable) build(ctx context.Context, input Operator) {
	...
	case hashTableDistinctMode:
		for {
			...
			ht.removeDuplicatesInBatch(batch)
			ht.removeDuplicatesInBufferedBatch(batch)
...
// removeDuplicatesInBufferedBatch removes tuples in the given batch that are already
// buffered in the hash table by updating the input batch's selection vector
// and length.
// NOTE: the hashTable *must* have been already built.
func (ht *hashTable) removeDuplicatesInBufferedBatch(batch coldata.Batch) {
	// Short circuit the logic when there is no tuple buffered.
	if ht.vals.Length() == 0 {
		return
	}
	...
	for i := uint64(0); i < nToCheck; i++ {
		ht.probeScratch.groupID[i] = ht.buildScratch.first[ht.probeScratch.hashBuffer[i]]
	...
	for nToCheck > 0 {
		...
		// array is non-empty.
		nToCheck =
			ht.findDistinctTuples(ht.probeScratch.keys, nToCheck, sel)
		ht.findNext(ht.buildScratch.next, nToCheck)
	}

	ht.updateSel(batch)
}

// removeDuplicatesInBatch removes the duplicated tuples in the given
// batch by updating the input batch's selection vector and length.
// NOTE: the hashTable *must* have been already built.
func (ht *hashTable) removeDuplicatesInBatch(batch coldata.Batch) {
	sel := batch.Selection()
	nToCheck := uint64(batch.Length())

	for i := uint64(0); i < nToCheck; i++ {
		ht.probeScratch.groupID[i] = ht.probeScratch.first[ht.probeScratch.hashBuffer[i]]
		ht.probeScratch.toCheck[i] = i
	}

	for nToCheck > 0 {
		nToCheck = ht.checkProbeForDistinct(ht.probeScratch.keys, nToCheck, sel)
		ht.findNext(ht.probeScratch.next, nToCheck)
	}
	ht.updateSel(batch)
}
