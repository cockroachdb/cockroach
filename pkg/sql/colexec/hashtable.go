...
// so it doesn't appear in this file. Look at hashtable_tmpl.go for the source
// code.
//
// initHash initializes the hash value of each key to its initial state for
// rehashing purposes.
func initHash(buckets []uint64, nKeys uint64) {
...
func (ht *hashTable) computeBuckets(
	...
	for i, k := range ht.keyCols {
		rehash(ctx, buckets, i, ht.valTypes[k], keys[i], nKeys, sel, ht.cancelChecker)
	}
