...
func (ht *hashTable) lookupInitial(
	ctx context.Context, keyTypes []coltypes.T, batchSize uint64, sel []uint64,
) {
...
func (ht *hashTable) checkCols(probeKeyTypes []coltypes.T, nToCheck uint64, sel []uint64) {
	for i := range ht.keyCols {
...
func (ht *hashTable) distinctCheck(
	probeKeyTypes []coltypes.T, nToCheck uint64, sel []uint64,
) uint64 {
