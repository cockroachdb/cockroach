...
// - loadFactor determines the average number of tuples per bucket exceeding of
// which will trigger resizing the hash table. This number can have noticeable
// effect on the performance, so every user of the hash table should choose the
// number that works well for the corresponding use case. 1.0 could be used as
// the initial default value, and most likely the best value will be in
// [0.1, 10.0] range.
func newHashTable(
	allocator *colmem.Allocator,
	loadFactor float64,
