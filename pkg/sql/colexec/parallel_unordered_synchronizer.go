...
type ParallelUnorderedSynchronizer struct {
	...
	// they should actually start consuming batches from their inputs. 'true'
	// will be sent when the worker should short-circuit.
	blockWorkersCh chan bool
...
func (s *ParallelUnorderedSynchronizer) Init(ctx context.Context) {
	...
	for i := range s.inputs {
		...
		go func(ctx context.Context, inputIdx int) {
			...
			s.nextBatch[inputIdx] = func(inputOp colexecop.Operator, inputIdx int) func() {
				return func() {
					s.batches[inputIdx], s.metas[inputIdx] = inputOp.Next()
				}
			}(input.Root, inputIdx)
			if cpuHandle := admission.SQLCPUHandleFromContext(inputCtx); cpuHandle != nil {
				gh := cpuHandle.RegisterGoroutine()
				defer gh.Close(inputCtx)
			}
			s.runWorker(input, inputIdx)
		}(inputCtx, i)
	}
}
...
// runWorker is the body of the worker goroutine that reads batches from the
...
func (s *ParallelUnorderedSynchronizer) runWorker(input colexecargs.OpWithMetaInfo, inputIdx int) {
	if shouldExit := <-s.blockWorkersCh; shouldExit {
		return
	}
...
func (s *ParallelUnorderedSynchronizer) Close(ctx context.Context) error {
	...
	case parallelUnorderedSynchronizerStateUninitialized:
		...
		return nil
	case parallelUnorderedSynchronizerStateWorkersBlocked:
		// The workers are in the blocked state, we'll short-circuit them and
		// the synchronizer becomes responsible for closing the tracing spans as
		// well as decrementing the wait groups.
		s.blockWorkersCh <- true
		...
		return nil
	default:
		// Input goroutines have been unblocked and will take care of finishing
		// the tracing spans.
		return nil
	}
}
