			...
			for {
				...
				case parallelUnorderedSynchronizerStateDraining:
					if input.MetadataSources != nil {
						msg.meta = input.MetadataSources[i].DrainMeta(ctx)
						sentMeta = true
					}
				default:
...
func (s *ParallelUnorderedSynchronizer) Next(ctx context.Context) coldata.Batch {
	for {
		...
		case msg := <-s.batchCh:
			...
			if msg.meta != nil {
				fmt.Printf("appending meta from %d and continuing\n", msg.inputIdx)
...
func (s *ParallelUnorderedSynchronizer) DrainMeta(
	...
	if prevState == parallelUnorderedSynchronizerStateUninitialized {
		...
		// Some data must have been read in Next since the inputs were initialized.
		// This means that the last input to return data is waiting to be told to
		// fetch more data. Preempt it here. It will read the new state and proceed
		// to drain.
		s.notifyInputToReadNextBatch(s.lastReadInputIdx)
	}
