...
func TestOrderedSync(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		columnTypes := make([]phystypes.T, numCols)
		for i := range columnTypes {
			columnTypes[i] = phystypes.Int64
		}
...
func TestOrderedSyncRandomInput(t *testing.T) {
	...
	op := OrderedSynchronizer{
		...
		columnTypes: []phystypes.T{phystypes.Int64},
	}
	op.Init()
...
func BenchmarkOrderedSynchronizer(b *testing.B) {
	...
	for i := range batches {
		batches[i] = testAllocator.NewMemBatch([]phystypes.T{phystypes.Int64})
