...
func NewColOperator(
	...
	if !supported {
		...
		case core.TableReader != nil:
			...
			scanOp, err = newColBatchScan(colmem.NewAllocator(ctx, streamingMemAccount), flowCtx, core.TableReader, post)
			if err != nil {
				return result, err
			}
		...
		case core.Aggregator != nil:
			...
			if err = typeconv.AreSupported(typs); err != nil {
				return result, err
			}
			if needHash {
		...
		case core.Distinct != nil:
			...
			result.ColumnTypes = make([]types.T, len(spec.Input[0].ColumnTypes))
			copy(result.ColumnTypes, spec.Input[0].ColumnTypes)
			if err = typeconv.AreSupported(result.ColumnTypes); err != nil {
				return result, err
			}
			if len(core.Distinct.OrderedColumns) == len(core.Distinct.DistinctColumns) {
		...
		case core.HashJoiner != nil:
			...
			if err := typeconv.AreSupported(leftTypes); err != nil {
				return result, err
			}
			rightTypes := make([]types.T, len(spec.Input[1].ColumnTypes))
			copy(rightTypes, spec.Input[1].ColumnTypes)
			if err := typeconv.AreSupported(rightTypes); err != nil {
				return result, err
			}
...
func planProjectionOperators(
	...
	case *tree.CaseExpr:
		...
		if !typeconv.IsSupported(caseOutputType) {
			return nil, resultIdx, typs, internalMemUsed, errors.Newf(
		...
		for i, when := range t.Whens {
			...
			if !typeconv.IsSupported(whenResolvedType) {
				return nil, resultIdx, typs, internalMemUsed, errors.Newf(
