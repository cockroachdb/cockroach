...
func NewColOperator(
	...
	if !supported {
		...
		case core.TableReader != nil:
			...
			scanOp, err = newColBatchScan(colmem.NewAllocator(ctx, streamingMemAccount, factory),
		...
		case core.Aggregator != nil:
			...
			if len(aggSpec.Aggregations) == 0 {
				...
				result.Op, result.IsStreaming, err =
					NewSingleTupleNoInputOp(colmem.NewAllocator(ctx, streamingMemAccount, factory)), true, nil
				// We make ColumnTypes non-nil so that sanity check doesn't panic.
				result.ColumnTypes = []*types.T{}
				break
			}
			if aggSpec.IsRowCount() {
				result.Op, result.IsStreaming, err = NewCountOp(
					colmem.NewAllocator(ctx, streamingMemAccount, factory), inputs[0],
				), true, nil
			...
			if needHash {
				...
				result.Op, err = NewOrderedAggregator(
					colmem.NewAllocator(ctx, streamingMemAccount, factory), inputs[0], typs, aggFns,
		...
		case core.Ordinality != nil:
			...
			result.Op = NewOrdinalityOp(
				colmem.NewAllocator(ctx, streamingMemAccount, factory), inputs[0], outputIdx,
			)
			result.IsStreaming = true
			...
			for _, wf := range core.Windower.WindowFns {
				...
				if len(core.Windower.PartitionBy) > 0 {
					...
					input, err = NewWindowSortingPartitioner(
						colmem.NewAllocator(ctx, streamingMemAccount, factory), input, typs,
				...
				if windowFnNeedsPeersInfo(*wf.Func.WindowFunc) {
					...
					input, err = NewWindowPeerGrouper(
						colmem.NewAllocator(ctx, streamingMemAccount, factory),
				...
				case execinfrapb.WindowerSpec_ROW_NUMBER:
					result.Op = NewRowNumberOperator(
						colmem.NewAllocator(ctx, streamingMemAccount, factory), input, outputIdx, partitionColIdx,
					)
				case execinfrapb.WindowerSpec_RANK, execinfrapb.WindowerSpec_DENSE_RANK:
					result.Op, err = NewRankOperator(
						colmem.NewAllocator(ctx, streamingMemAccount, factory), input, windowFn,
