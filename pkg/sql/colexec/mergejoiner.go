...
func (o *mergeJoinBase) initWithOutputBatchSize(outBatchSize int) {
	...
	o.proberState.lBufferedGroup.spillingQueue = newSpillingQueue(
		...
		o.diskQueueCfg, o.fdSemaphore, coldata.BatchSize(),
	)
...
func (o *mergeJoinBase) appendToBufferedGroup(
	...
	if input == &o.left {
		...
		scratchBatch = o.unlimitedAllocator.NewMemBatchWithSize(o.left.sourceTypes, groupLength)
	} else {
		...
		scratchBatch = o.unlimitedAllocator.NewMemBatchWithSize(o.right.sourceTypes, groupLength)
	}
...
func (o *mergeJoinBase) completeBufferedGroup(
	...
	for !isBufferedGroupComplete {
		...
		// TODO(yuzefovich): I think we can copy only up to batchLength.
