...
func newMinHashAggAlloc(
	...
	case types.EnumFamily:
		...
		case -1:
		default:
			return &minEnumHashAggAlloc{aggAllocBase: allocBase}
		}
...
var _ AggregateFunc = &minEnumHashAgg{}

func (a *minEnumHashAgg) Compute(
	vecs []coldata.Vec, inputIdxs []uint32, startIdx, endIdx int, sel []int,
) {
	oldCurAggSize := len(a.curAgg)
	vec := vecs[inputIdxs[0]]
...
func (a *minEnumHashAgg) Flush(outputIdx int) {
	// The aggregation is finished. Flush the last value. If we haven't found
	// any non-nulls for this group so far, the output for this group should
	// be null.
	col := a.vec.Enum()
...
type minEnumHashAggAlloc struct {
	aggAllocBase
	aggFuncs []minEnumHashAgg
}

var _ aggregateFuncAlloc = &minEnumHashAggAlloc{}

const sizeOfminEnumHashAgg = int64(unsafe.Sizeof(minEnumHashAgg{}))
const minEnumHashAggSliceOverhead = int64(unsafe.Sizeof([]minEnumHashAgg{}))

func (a *minEnumHashAggAlloc) newAggFunc() AggregateFunc {
	if len(a.aggFuncs) == 0 {
		a.allocator.AdjustMemoryUsage(minEnumHashAggSliceOverhead + sizeOfminEnumHashAgg*a.allocSize)
		a.aggFuncs = make([]minEnumHashAgg, a.allocSize)
	}
	f := &a.aggFuncs[0]
	f.allocator = a.allocator
	a.aggFuncs = a.aggFuncs[1:]
...
func newMaxHashAggAlloc(
	...
	case types.EnumFamily:
		...
		case -1:
		default:
			return &maxEnumHashAggAlloc{aggAllocBase: allocBase}
		}
	case typeconv.DatumVecCanonicalTypeFamily:
		switch t.Width() {
		case -1:
...
var _ AggregateFunc = &maxEnumHashAgg{}

func (a *maxEnumHashAgg) Compute(
	vecs []coldata.Vec, inputIdxs []uint32, startIdx, endIdx int, sel []int,
) {
	oldCurAggSize := len(a.curAgg)
	vec := vecs[inputIdxs[0]]
...
func (a *maxEnumHashAgg) Flush(outputIdx int) {
	// The aggregation is finished. Flush the last value. If we haven't found
	// any non-nulls for this group so far, the output for this group should
	// be null.
	col := a.vec.Enum()
...
type maxEnumHashAggAlloc struct {
	aggAllocBase
	aggFuncs []maxEnumHashAgg
}

var _ aggregateFuncAlloc = &maxEnumHashAggAlloc{}

const sizeOfmaxEnumHashAgg = int64(unsafe.Sizeof(maxEnumHashAgg{}))
const maxEnumHashAggSliceOverhead = int64(unsafe.Sizeof([]maxEnumHashAgg{}))

func (a *maxEnumHashAggAlloc) newAggFunc() AggregateFunc {
	if len(a.aggFuncs) == 0 {
		a.allocator.AdjustMemoryUsage(maxEnumHashAggSliceOverhead + sizeOfmaxEnumHashAgg*a.allocSize)
		a.aggFuncs = make([]maxEnumHashAgg, a.allocSize)
	}
	f := &a.aggFuncs[0]
	f.allocator = a.allocator
	a.aggFuncs = a.aggFuncs[1:]
