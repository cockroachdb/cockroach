...
func newMinWindowAggAlloc(
	...
	case types.EnumFamily:
		...
		case -1:
		default:
			return &minEnumWindowAggAlloc{aggAllocBase: allocBase}
		}
...
var _ AggregateFunc = &minEnumWindowAgg{}

func (a *minEnumWindowAgg) Compute(
	vecs []coldata.Vec, inputIdxs []uint32, startIdx, endIdx int, sel []int,
) {
	oldCurAggSize := len(a.curAgg)
	vec := vecs[inputIdxs[0]]
...
func (a *minEnumWindowAgg) Flush(outputIdx int) {
	// The aggregation is finished. Flush the last value. If we haven't found
	// any non-nulls for this group so far, the output for this group should
	// be null.
	col := a.vec.Enum()
...
type minEnumWindowAggAlloc struct {
	aggAllocBase
	aggFuncs []minEnumWindowAgg
}

var _ aggregateFuncAlloc = &minEnumWindowAggAlloc{}

const sizeOfminEnumWindowAgg = int64(unsafe.Sizeof(minEnumWindowAgg{}))
const minEnumWindowAggSliceOverhead = int64(unsafe.Sizeof([]minEnumWindowAgg{}))

func (a *minEnumWindowAggAlloc) newAggFunc() AggregateFunc {
	if len(a.aggFuncs) == 0 {
		a.allocator.AdjustMemoryUsage(minEnumWindowAggSliceOverhead + sizeOfminEnumWindowAgg*a.allocSize)
		a.aggFuncs = make([]minEnumWindowAgg, a.allocSize)
	}
	f := &a.aggFuncs[0]
	f.allocator = a.allocator
	a.aggFuncs = a.aggFuncs[1:]
...
// the window frame only grows. For the case when the window frame can shrink,
// a specialized implementation is needed (see min_max_removable_agg_tmpl.go).
func (*minEnumWindowAgg) Remove(vecs []coldata.Vec, inputIdxs []uint32, startIdx, endIdx int) {
	colexecerror.InternalError(errors.AssertionFailedf("Remove called on minEnumWindowAgg"))
}
...
func newMaxWindowAggAlloc(
	...
	case types.EnumFamily:
		...
		case -1:
		default:
			return &maxEnumWindowAggAlloc{aggAllocBase: allocBase}
		}
	case typeconv.DatumVecCanonicalTypeFamily:
		switch t.Width() {
		case -1:
...
var _ AggregateFunc = &maxEnumWindowAgg{}

func (a *maxEnumWindowAgg) Compute(
	vecs []coldata.Vec, inputIdxs []uint32, startIdx, endIdx int, sel []int,
) {
	oldCurAggSize := len(a.curAgg)
	vec := vecs[inputIdxs[0]]
...
func (a *maxEnumWindowAgg) Flush(outputIdx int) {
	// The aggregation is finished. Flush the last value. If we haven't found
	// any non-nulls for this group so far, the output for this group should
	// be null.
	col := a.vec.Enum()
...
type maxEnumWindowAggAlloc struct {
	aggAllocBase
	aggFuncs []maxEnumWindowAgg
}

var _ aggregateFuncAlloc = &maxEnumWindowAggAlloc{}

const sizeOfmaxEnumWindowAgg = int64(unsafe.Sizeof(maxEnumWindowAgg{}))
const maxEnumWindowAggSliceOverhead = int64(unsafe.Sizeof([]maxEnumWindowAgg{}))

func (a *maxEnumWindowAggAlloc) newAggFunc() AggregateFunc {
	if len(a.aggFuncs) == 0 {
		a.allocator.AdjustMemoryUsage(maxEnumWindowAggSliceOverhead + sizeOfmaxEnumWindowAgg*a.allocSize)
		a.aggFuncs = make([]maxEnumWindowAgg, a.allocSize)
	}
	f := &a.aggFuncs[0]
	f.allocator = a.allocator
	a.aggFuncs = a.aggFuncs[1:]
...
// the window frame only grows. For the case when the window frame can shrink,
// a specialized implementation is needed (see min_max_removable_agg_tmpl.go).
func (*maxEnumWindowAgg) Remove(vecs []coldata.Vec, inputIdxs []uint32, startIdx, endIdx int) {
	colexecerror.InternalError(errors.AssertionFailedf("Remove called on maxEnumWindowAgg"))
}
