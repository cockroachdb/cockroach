...
func newMinOrderedAggAlloc(
	...
	case types.EnumFamily:
		...
		case -1:
		default:
			return &minEnumOrderedAggAlloc{aggAllocBase: allocBase}
		}
...
var _ AggregateFunc = &minEnumOrderedAgg{}

func (a *minEnumOrderedAgg) SetOutput(vec coldata.Vec) {
	a.orderedAggregateFuncBase.SetOutput(vec)
	a.col = vec.Enum()
}

func (a *minEnumOrderedAgg) Compute(
	vecs []coldata.Vec, inputIdxs []uint32, startIdx, endIdx int, sel []int,
) {
	oldCurAggSize := len(a.curAgg)
	vec := vecs[inputIdxs[0]]
...
func (a *minEnumOrderedAgg) Flush(outputIdx int) {
	// The aggregation is finished. Flush the last value. If we haven't found
	// any non-nulls for this group so far, the output for this group should
	// be null.
	// Go around "argument overwritten before first use" linter error.
...
func (a *minEnumOrderedAgg) Reset() {
	a.orderedAggregateFuncBase.Reset()
	a.numNonNull = 0
	oldCurAggSize := len(a.curAgg)
	// Release the reference to curAgg.
...
type minEnumOrderedAggAlloc struct {
	aggAllocBase
	aggFuncs []minEnumOrderedAgg
}

var _ aggregateFuncAlloc = &minEnumOrderedAggAlloc{}

const sizeOfminEnumOrderedAgg = int64(unsafe.Sizeof(minEnumOrderedAgg{}))
const minEnumOrderedAggSliceOverhead = int64(unsafe.Sizeof([]minEnumOrderedAgg{}))

func (a *minEnumOrderedAggAlloc) newAggFunc() AggregateFunc {
	if len(a.aggFuncs) == 0 {
		a.allocator.AdjustMemoryUsage(minEnumOrderedAggSliceOverhead + sizeOfminEnumOrderedAgg*a.allocSize)
		a.aggFuncs = make([]minEnumOrderedAgg, a.allocSize)
	}
	f := &a.aggFuncs[0]
	f.allocator = a.allocator
	a.aggFuncs = a.aggFuncs[1:]
...
func newMaxOrderedAggAlloc(
	...
	case types.EnumFamily:
		...
		case -1:
		default:
			return &maxEnumOrderedAggAlloc{aggAllocBase: allocBase}
		}
	case typeconv.DatumVecCanonicalTypeFamily:
		switch t.Width() {
		case -1:
...
var _ AggregateFunc = &maxEnumOrderedAgg{}

func (a *maxEnumOrderedAgg) SetOutput(vec coldata.Vec) {
	a.orderedAggregateFuncBase.SetOutput(vec)
	a.col = vec.Enum()
}

func (a *maxEnumOrderedAgg) Compute(
	vecs []coldata.Vec, inputIdxs []uint32, startIdx, endIdx int, sel []int,
) {
	oldCurAggSize := len(a.curAgg)
	vec := vecs[inputIdxs[0]]
...
func (a *maxEnumOrderedAgg) Flush(outputIdx int) {
	// The aggregation is finished. Flush the last value. If we haven't found
	// any non-nulls for this group so far, the output for this group should
	// be null.
	// Go around "argument overwritten before first use" linter error.
...
func (a *maxEnumOrderedAgg) Reset() {
	a.orderedAggregateFuncBase.Reset()
	a.numNonNull = 0
	oldCurAggSize := len(a.curAgg)
	// Release the reference to curAgg.
...
type maxEnumOrderedAggAlloc struct {
	aggAllocBase
	aggFuncs []maxEnumOrderedAgg
}

var _ aggregateFuncAlloc = &maxEnumOrderedAggAlloc{}

const sizeOfmaxEnumOrderedAgg = int64(unsafe.Sizeof(maxEnumOrderedAgg{}))
const maxEnumOrderedAggSliceOverhead = int64(unsafe.Sizeof([]maxEnumOrderedAgg{}))

func (a *maxEnumOrderedAggAlloc) newAggFunc() AggregateFunc {
	if len(a.aggFuncs) == 0 {
		a.allocator.AdjustMemoryUsage(maxEnumOrderedAggSliceOverhead + sizeOfmaxEnumOrderedAgg*a.allocSize)
		a.aggFuncs = make([]maxEnumOrderedAgg, a.allocSize)
	}
	f := &a.aggFuncs[0]
	f.allocator = a.allocator
	a.aggFuncs = a.aggFuncs[1:]
