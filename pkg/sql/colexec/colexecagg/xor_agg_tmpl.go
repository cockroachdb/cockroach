...
//
// This file is the execgen template for xor_agg.eg.go. It's formatted in a
...
// _ASSIGN_XOR is the template XOR function for assigning the first input
// to the result of the second input XOR the third input.
func _ASSIGN_XOR(_, _, _, _, _, _ string) {
...
func newXor_AGGKINDAggAlloc(
	...
	case _CANONICAL_TYPE_FAMILY:
		...
		case _TYPE_WIDTH:
			// {{with .Overload}}
			return &xor_TYPE_AGGKINDAggAlloc{aggAllocBase: allocBase}, nil
			// {{end}}
			// {{end}}
		}
		// {{end}}
...
type xor_TYPE_AGGKINDAgg struct {
	// {{if eq "_AGGKIND" "Ordered"}}
...
var _ AggregateFunc = &xor_TYPE_AGGKINDAgg{}

// {{if eq "_AGGKIND" "Ordered"}}
func (a *xor_TYPE_AGGKINDAgg) SetOutput(vec *coldata.Vec) {
...
func (a *xor_TYPE_AGGKINDAgg) Compute(
	...
	a.allocator.PerformOperation([]*coldata.Vec{a.vec}, func() {
		...
		if sel == nil {
			...
			if nulls.MaybeHasNulls() {
				for i := startIdx; i < endIdx; i++ {
					_ACCUMULATE_XOR(a, nulls, i, true, false)
				}
			} else {
				for i := startIdx; i < endIdx; i++ {
					_ACCUMULATE_XOR(a, nulls, i, false, false)
				}
			}
		} else
		// {{end}}
		{
			...
			if nulls.MaybeHasNulls() {
				for _, i := range sel {
					_ACCUMULATE_XOR(a, nulls, i, true, true)
				}
			} else {
				for _, i := range sel {
					_ACCUMULATE_XOR(a, nulls, i, false, true)
				}
			}
		}
	},
...
func (a *xor_TYPE_AGGKINDAgg) Flush(outputIdx int) {
...
func (a *xor_TYPE_AGGKINDAgg) Reset() {
	// {{if eq "_AGGKIND" "Ordered"}}
...
type xor_TYPE_AGGKINDAggAlloc struct {
	aggAllocBase
	aggFuncs []xor_TYPE_AGGKINDAgg
}

var _ aggregateFuncAlloc = &xor_TYPE_AGGKINDAggAlloc{}

const sizeOfXor_TYPE_AGGKINDAgg = int64(unsafe.Sizeof(xor_TYPE_AGGKINDAgg{}))
const xor_TYPE_AGGKINDAggSliceOverhead = int64(unsafe.Sizeof([]xor_TYPE_AGGKINDAgg{}))

func (a *xor_TYPE_AGGKINDAggAlloc) newAggFunc() AggregateFunc {
	if len(a.aggFuncs) == 0 {
		a.allocator.AdjustMemoryUsage(xor_TYPE_AGGKINDAggSliceOverhead + sizeOfXor_TYPE_AGGKINDAgg*a.allocSize)
		a.aggFuncs = make([]xor_TYPE_AGGKINDAgg, a.allocSize)
	}
	f := &a.aggFuncs[0]
...
func (a *xor_TYPE_AGGKINDAgg) Remove(
...
// {{/*
// _ACCUMULATE_XOR XORs the value of the ith row to the output for the current
...
// null.
func _ACCUMULATE_XOR(
	a *xor_TYPE_AGGKINDAgg, nulls *coldata.Nulls, i int, _HAS_NULLS bool, _HAS_SEL bool,
) { // */}}
	// {{define "accumulateXor"}}
	...
	if !isNull {
		...
		// {{end}}
		v := col.Get(i)
		_ASSIGN_XOR(a.curAgg, a.curAgg, v, _, _, col)
		a.numNonNull++
	}
	// {{end}}
...
// current aggregation. For XOR, this is the same as adding since XOR is
// self-inverse.
func _REMOVE_ROW(a *xor_TYPE_AGGKINDAgg, nulls *coldata.Nulls, i int, _HAS_NULLS bool) { // */}}
	// {{define "removeRow"}}
	var isNull bool
	// {{if .HasNulls}}
	isNull = nulls.NullAt(i)
