	...
	dstutil.Apply(f, func(cursor *dstutil.Cursor) bool {
		...
		case *dst.FuncDecl:
			...
			for _, args := range info.instantiateArgs {
				for j := range args {
					if letInfo := ret.letInfos[args[j]]; letInfo != nil {
						// We're going to expand this instantiate into a list of
						// instantiates, since we know we have a "list" instantiation
						// that uses a let decl.
					}
				}
