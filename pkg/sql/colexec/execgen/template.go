...
type funcInfo struct {
	...
	// foundParams is a list of lists of the template arguments that were actually
	// used for this template function.
	foundParams [][]dst.Expr
}
	...
	return dstutil.Apply(n, func(cursor *dstutil.Cursor) bool {
		...
		case *dst.SwitchStmt:
			...
			for _, item := range n.Body.List {
				...
				if len(c.List) != 1 {
					panic("invalid execgen:switch with more than 1 case")
				}
				if prettyPrintExprs(c.List[0]) == t {
