...
func genAvgAgg(wr io.Writer) error {
	...
	for i, typ := range supportedTypes {
		...
		if overload == nil {
			colexecerror.InternalError(fmt.Sprintf("unexpectedly didn't find plus binary overload for %s", typ.Name()))
		}
		if len(overload.WidthOverloads) != 1 {
			// TODO: remove this restriction.
			colexecerror.InternalError(fmt.Sprintf("unexpectedly plus binary overload for %s doesn't contain a single overload", typ.Name()))
		}
