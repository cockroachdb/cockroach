...
func rangeOffsetHandlerGenerator(inputFileContents string, wr io.Writer) error {
	...
	assignAddRe := makeFunctionRegex("_VALUE_BY_OFFSET", 6)
	s = assignAddRe.ReplaceAllString(s, makeTemplateFunctionCall("ValueByOffset", 6))

	assignCmpRe := makeFunctionRegex("_ASSIGN_CMP", 5)
	s = assignCmpRe.ReplaceAllString(s, makeTemplateFunctionCall("AssignCmp", 5))
				...
				for _, typeFamily := range rangeOrderColTypeFamilies {
					...
					for _, width := range getTypeWidths(typeFamily) {
						...
						widthOverload := windowFrameOrderWidthOverload{
							...
							OpString:        opString,
							NeedsHelper:     toVecMethod(canonicalTypeFamily, width) == "Datum",
							IsStart:         isStart,
...
func (overload windowFrameOrderWidthOverload) ValueByOffset(
	...
	return overload.assignFunc(lawo, targetElem, leftElem, rightElem, targetCol, leftCol, rightCol)
}
...
func (overload windowFrameOrderWidthOverload) AssignCmp(
	...
	return overload.cmpFunc(targetElem, leftElem, rightElem, leftCol, rightCol)
}
