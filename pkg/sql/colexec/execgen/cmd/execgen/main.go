...
type generator func(inputFile string, outputFile io.Writer) error
...
func (g *execgenTool) run(args ...string) bool {
	...
	for _, out := range args {
		...
		if printDeps {
			...
			if err := g.generate(out, entry); err != nil {
				g.reportError(err)
				return false
			}
		}
...
func (g *execgenTool) generate(path string, entry entry) error {
	...
	var err error
	if entry.inputFile != "" {
		inputFile, err = ioutil.ReadFile(entry.inputFile)
		if err != nil {
			return err
		}
	}

	err = entry.fn(string(inputFile), &buf)
	if err != nil {
		return err
	}
