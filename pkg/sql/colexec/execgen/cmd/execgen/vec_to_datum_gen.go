...
func genVecToDatum(inputFileContents string, wr io.Writer) error {
	...
	setDestIdx := makeFunctionRegex("_SET_DEST_IDX", 4)
	s = setDestIdx.ReplaceAllString(s, `{{template "setDestIdx" buildDict "Sparsely" $4}}`)
	...
	s = vecToDatum.ReplaceAllString(s, `{{template "vecToDatum" buildDict "Global" . "HasNulls" $6 "HasSel" $7 "Sparsely" $8}}`)
