...
func intToDecimal(to, from, _, _ string) string {
	convStr := `
...
func floatToDecimal(to, from, _, _ string) string {
	convStr := `
		{
...
func datumToBool(to, from, _, fromCol string) string {
	convStr := `
		{
			castedDatum, err := %[2]s.(*coldataext.Datum).Cast(%[3]s, types.Bool)
			if err != nil {
				colexecerror.ExpectedError(err)
			}
			%[1]s = castedDatum == tree.DBoolTrue
		}
	`
...
// datumCastCustomizer specifies casts from types that are backed by datumVec
// to other types.
