...
var sameTypeBinaryOpToOverloads = make(map[tree.BinaryOperator][]*oneArgOverload, len(execgenbase.BinaryOpName))

var binOpOutputTypes = make(map[tree.BinaryOperator]map[typePair]*types.T)

func registerBinOpOutputTypes() {
	for binOp := range execgenbase.BinaryOpName {
...
func populateBinOpOverloads() {
	...
	for _, op := range []tree.BinaryOperator{tree.Plus, tree.Minus, tree.Mult, tree.Div} {
		ob := &overloadBase{
			kind:  binaryOverload,
			Name:  execgenbase.BinaryOpName[op],
			BinOp: op,
