...
// the constant is on the left side.
...
func init() {
	projConstOpsGenerator := func(isConstLeft bool) generator {
		return func(wr io.Writer) error {
			...
			// Now we get the template for actually generating the projection
			// operators as well as the operator-getter function.
			genTmpl, err := ioutil.ReadFile("pkg/sql/colexec/proj_const_ops_gen_tmpl.go")
			if err != nil {
				return err
			}
			tmpl, err = tmpl.Parse(replaceProjConstTmplVariables(string(genTmpl), isConstLeft))
			if err != nil {
				return err
			}
