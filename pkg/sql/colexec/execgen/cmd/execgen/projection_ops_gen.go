...
func genProjNonConstOps(inputFile string, wr io.Writer) error {
	s := replaceProjTmplVariables(inputFile)
...
func init() {
	projConstOpsGenerator := func(isConstLeft bool) generator {
		return func(inputFile string, wr io.Writer) error {
			tmplString := replaceProjConstTmplVariables(inputFile, isConstLeft)
