			...
			for _, tc := range sortTestCases {
				runTests(
					...
					func(input []Operator) (Operator, error) {
						sorter, err, accounts, monitors := createDiskBackedSorter(
							ctx, flowCtx, input, tc.logTypes, tc.ordCols,
						)
...
func BenchmarkExternalSort(b *testing.B) {
	...
	for _, nBatches := range []int{1 << 1, 1 << 4, 1 << 8} {
		for _, nCols := range []int{1, 2, 4} {
			for _, memoryLimit := range []int64{0, 1} {
				flowCtx.Cfg.TestingKnobs.MemoryLimitBytes = memoryLimit
				spilled := memoryLimit > 0
				name := fmt.Sprintf("rows=%d/cols=%d/spilled=%t", nBatches*int(coldata.BatchSize()), nCols, spilled)
				b.Run(name, func(b *testing.B) {
					...
					for n := 0; n < b.N; n++ {
						...
						sorter, err, accounts, monitors := createDiskBackedSorter(ctx, flowCtx, []Operator{source}, logTypes, ordCols)
						memAccounts = append(memAccounts, accounts...)
						memMonitors = append(memMonitors, monitors...)
						if err != nil {
							b.Fatal(err)
...
func createDiskBackedSorter(
...
) (Operator, error, []*mon.BoundAccount, []*mon.BytesMonitor) {
	...
	args := NewColOperatorArgs{
		...
		// the streaming memory account and will be accumulating newly
		// created accounts and monitors below.
		UseStreamingMemAccountForBuffering: false,
	}
	result, err := NewColOperator(ctx, flowCtx, args)
	return result.Op, err, result.BufferingOpMemAccounts, result.BufferingOpMemMonitors
}
