...
// externalOperatorState indicates the current state of the external operator.
type externalOperatorState int

const (
	// externalOpInitialPartitioning indicates that the operator is currently
	// reading batches from its inputs and distributing tuples to different
	// partitions based on the hash values. Once all inputs are exhausted, the
	// external operator transitions to externalOpProcessNewPartitionUsingMain
	// state.
	externalOpInitialPartitioning externalOperatorState = iota
	// externalOpRecursivePartitioning indicates that the operator is
	// recursively partitioning one of the existing partitions (that is too big
	// to process using the "main" strategy at once). It will do so using a
	// different hash function and will spill newly created partitions to disk.
	// We also keep track whether repartitioning reduces the size of the
	// partitions in question - if we see that the newly created largest
	// partition is about the same in size as the "parent" partition (the
	// percentage difference is less than
	// externalOpRecursivePartitioningSizeDecreaseThreshold), it is likely that
	// the partition consists of the tuples not distinct on the equality
	...
	// externalOpProcessNewPartitionUsingMain state.
	externalOpRecursivePartitioning
	// externalOpProcessNewPartitionUsingMain indicates that the operator should
	// choose a partition index and process the corresponding partitions from
	// all inputs using the "main" operator. We will only process the partitions
	// if the partition fits into memory. If there are no partition indices that
	// the operator can process, it transitions into
	// externalOpRecursivePartitioning state. If there are no partition indices
	// to process using the main operator, but there are indices to process
	// using the "fallback" strategy, the operator transitions to
	// externalOpProcessNewPartitionUsingFallback state. If there are no
	// partition indices left at all to process, the operator transitions to
	// externalOpFinished state.
	externalOpProcessNewPartitionUsingMain
	// externalOpProcessingUsingMain indicates that the operator is currently
	...
	// emitted), the external operator transitions to
	// externalOpProcessNewPartitionUsingMain state.
	externalOpProcessingUsingMain
	// externalOpProcessNewPartitionUsingFallback indicates that the operator
	// should choose a partition index to process using the "fallback" strategy.
	// If there are no partition indices for this strategy left, the operator
	// transitions to externalOpFinished state.
	externalOpProcessNewPartitionUsingFallback
	// externalOpProcessingUsingFallback indicates that the operator is
	// currently processing tuples from the corresponding partitions from all
	// inputs using (disk-backed) "fallback" strategy. Once the "fallback"
	// returns a zero-length batch (indicating that full output for the current
	// partitions has been emitted), the external operator transitions to
	// externalOpProcessNewPartitionUsingFallback state.
	externalOpProcessingUsingFallback
	// externalOpFinished indicates that the external operator has emitted all
	// tuples already and only zero-length batch will be emitted from now on.
	externalOpFinished
)

const (
	// externalOpRecursivePartitioningSizeDecreaseThreshold determines by how
	...
	// operator.
	externalOpRecursivePartitioningSizeDecreaseThreshold = 0.05
	// externalOpDiskQueuesMemFraction determines the fraction of the available
	// RAM that is allocated for the in-memory cache of disk queues.
	externalOpDiskQueuesMemFraction = 0.5
	// externalOpMinimalMaxPartitionSizeForMain determines the minimum value for
	// the partition size that can be processed using the "main" in-memory
	// strategy.
	externalOpMinimalMaxPartitionSizeForMain = 64 << 10 /* 64 KiB */
)

// externalOperator is an operator that extracts the logic of Grace hash join
// (see the comment in external_hash_joiner.go for more details) to be reused by
// other operations.
//
...
type externalOperator struct {
	...
	partitionsToProcessUsingMain map[int]*externalOpPartitionInfo
	...
	// numRepartitions tracks the number of times the external operator had to
	// recursively repartition another partition because the latter was too big
	// to process using the "main" operator.
	numRepartitions int
	...
	testingKnobs struct {
		// numForcedRepartitions is a number of times that the external operator
		// is forced to recursively repartition (even if it is otherwise not
		// needed) before it proceeds to actually processing the partitions.
		...
		// external operator will open/close. This disables the default behavior
		// of acquiring all file descriptors up front in Next.
		delegateFDAcquisitions bool
	}
}

var _ closableOperator = &externalOperator{}

// externalOpPartitionInfo is a helper struct that tracks the memory usage of a
// partition. Note that if the external operator has two inputs, we take the
// right partition size for this since that is the input we're buffering from.
type externalOpPartitionInfo struct {
	memSize       int64
	parentMemSize int64
}
...
// newExternalOperator returns a disk-backed operator that utilizes partitioning
// by hash approach to divide up the input set into separate partitions which
// are then processed using the "main" in-memory operator if they fit under the
// memory limit. If a partition is too big, it is attempted to be recursively
// repartitioned; if that is not successful, the partition in question is
// handled by the "fallback" disk-backed operator.
func newExternalOperator(
	...
	if maxPartitionSizeToProcessUsingMain < externalOpMinimalMaxPartitionSizeForMain {
		maxPartitionSizeToProcessUsingMain = externalOpMinimalMaxPartitionSizeForMain
	}
	op := &externalOperator{
...
func calculateMaxNumberActivePartitions(
	...
	// external operator. Below we check whether we have enough RAM to support
	...
	if args.DiskQueueCfg.BufferSizeBytes > 0 {
		diskQueuesTotalMemLimit := int(float64(memoryLimit) * externalOpDiskQueuesMemFraction)
...
func (op *externalOperator) Init() {
	for i := range op.inputs {
		op.inputs[i].Init()
	}
	op.partitionsToProcessUsingMain = make(map[int]*externalOpPartitionInfo)
	// If we are initializing the external operator, it means that we had to
	// fallback from the in-memory one since the inputs had more tuples that
	...
	op.state = externalOpInitialPartitioning
}

func (op *externalOperator) partitionBatch(
	...
	for idx, sel := range selections {
		...
		if len(sel) > 0 {
			...
			if !ok {
				partitionInfo = &externalOpPartitionInfo{}
...
func (op *externalOperator) Next(ctx context.Context) coldata.Batch {
	var batches [2]coldata.Batch
StateChanged:
	for {
		switch op.state {
		case externalOpInitialPartitioning:
			allZero := true
			for i := range op.inputs {
			...
			if allZero {
				...
				op.state = externalOpProcessNewPartitionUsingMain
				continue
			}
		...
		case externalOpRecursivePartitioning:
			op.numRepartitions++
			...
			for parentPartitionIdx, parentPartitionInfo := range op.partitionsToProcessUsingMain {
				for i := range op.inputs {
					...
					for {
						...
						op.partitionBatch(ctx, batch, i, parentPartitionInfo.parentMemSize)
					}
					...
					// externalOpRecursivePartitioning, at that stage we have at
				...
				for idx := 0; idx < op.numBuckets; idx++ {
					...
					if partitionInfo, ok := op.partitionsToProcessUsingMain[newPartitionIdx]; ok {
						...
						if before > 0 {
							...
							if sizeDecrease < externalOpRecursivePartitioningSizeDecreaseThreshold {
								// We will need to process this partition using the
								// "fallback" strategy.
		...
		case externalOpProcessNewPartitionUsingMain:
			...
			for partitionIdx, partitionInfo := range op.partitionsToProcessUsingMain {
				if partitionInfo.memSize <= op.maxPartitionSizeToProcessUsingMain {
					...
					op.state = externalOpProcessingUsingMain
					continue StateChanged
				}
			}
			if len(op.partitionsToProcessUsingMain) == 0 {
				...
				if len(op.partitionsToProcessUsingFallback) > 0 {
					...
					op.state = externalOpProcessNewPartitionUsingFallback
					continue
				}
				// All partitions have been processed, so we transition to
				// finished state.
				op.state = externalOpFinished
				continue
			}
		...
		case externalOpProcessingUsingMain:
		...
		case externalOpProcessNewPartitionUsingFallback:
			if len(op.partitionsToProcessUsingFallback) == 0 {
				// All partitions have been processed, so we transition to
				// finished state.
				op.state = externalOpFinished
			...
			op.state = externalOpProcessingUsingFallback
			continue

		case externalOpProcessingUsingFallback:
		...
		default:
			colexecerror.InternalError(errors.AssertionFailedf("unexpected externalOperatorState %d", op.state))
		}
	}
}

func (op *externalOperator) Close(ctx context.Context) error {
	if !op.close() {
		return nil
	}
	var retErr error
...
func (op *externalOperator) ChildCount(_ bool) int {
	return len(op.inputs)
}

func (op *externalOperator) Child(nth int, _ bool) execinfra.OpNode {
