...
func newMJBufferedGroup(allocator *Allocator, types []phystypes.T) *mjBufferedGroup {
	bg := &mjBufferedGroup{
...
func (bg *mjBufferedGroup) Reset(types []phystypes.T, length int) {
	execerror.VectorizedInternalPanic("Reset([]phystypes.T, int) should not be called on mjBufferedGroup")
}
...
func (bg *mjBufferedGroup) reset() {
	...
	for _, colVec := range bg.colVecs {
		...
		if colVec.Type() == phystypes.Bytes {
