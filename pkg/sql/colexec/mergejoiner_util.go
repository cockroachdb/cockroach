...
type circularGroupsBuffer struct {
	...
	// cap is the limit on the number of groups that current {left,right}Groups
	// slices can support. numGroupsInBuffer will never reach this number,
	// instead, we'll allocate bigger slices and increase cap accordingly.
	cap int
	// size is the number used to calculate the current values of cap and the
	// groups slices' lengths. It will be doubling when reallocating.
	size int
...
func makeGroupsBuffer(batchSize int) circularGroupsBuffer {
	return circularGroupsBuffer{
		cap:         getGroupsBufferCap(groupsBufferInitialSize),
...
// getGroupsBufferCap returns cap value for circularGroupsBuffer for the given
// size.
//
...
// buffers.
func getGroupsBufferCap(size int) int {
	return 2*size + 1
}
...
//
// Since we have a circular buffer, it is possible for groups to wrap when cap
// is reached. Consider an example when size = 3 and startIdx = 6 when maximum
// number of groups is present:
...
func (b *circularGroupsBuffer) ensureCapacityForNewGroup() {
	if b.size == b.maxSize || b.numGroupsInBuffer+1 < b.cap {
	...
	if b.startIdx <= b.endIdx {
		...
		// Current groups are wrapped at position b.cap. Namely, if we have
		// size = 3, cap = 7, we might have the following:
		//   buffer = [1, 2, 0', 1', 2', x, 0]
		// where startIdx = 6, endIdx = 4, nextColStartIdx = 2, so we need to
		// copy over with the adjustments.
		copy(newLeftGroups, b.leftGroups[b.startIdx:b.cap])
		copy(newRightGroups, b.rightGroups[b.startIdx:b.cap])
		if b.endIdx > 0 {
			offset := b.cap - b.startIdx
			...
			//
			// Consider the following, size = 3, cap = 7
	...
	b.cap = getGroupsBufferCap(newSize)
	b.size = newSize
