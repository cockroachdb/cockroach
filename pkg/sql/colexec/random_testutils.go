...
func randomType(rng *rand.Rand) phystypes.T {
	return phystypes.AllTypes[rng.Intn(len(phystypes.AllTypes))]
}

// randomTypes returns an n-length slice of random phystypes.T.
func randomTypes(rng *rand.Rand, n int) []phystypes.T {
	typs := make([]phystypes.T, n)
	for i := range typs {
...
// bytes slice to be generated. It is used only if typ == phystypes.Bytes.
func RandomVec(
	rng *rand.Rand,
	typ phystypes.T,
...
func NewRandomDataOp(allocator *Allocator, rng *rand.Rand, args RandomDataOpArgs) *RandomDataOp {
	var (
		availableTyps   = phystypes.AllTypes
	...
	if typs == nil {
		...
		typs = make([]phystypes.T, 1+rng.Intn(maxSchemaLength))
		for i := range typs {
