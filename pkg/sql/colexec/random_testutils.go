...
// less than batchSize.
func randomSel(rng *rand.Rand, batchSize uint64, probOfOmitting float64) []uint64 {
...
func randomBatchWithSel(
	...
	if selProbability != 0 {
		sel := randomSel(rng, uint64(n), 1-selProbability)
		batch.SetSelection(true)
		copy(batch.Selection(), sel)
		batch.SetLength(uint64(len(sel)))
	}
	return batch
}
