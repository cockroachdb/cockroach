...
func TestOrderedSync(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		colexectestutils.RunTests(t, testAllocator, tc.sources, tc.expected, colexectestutils.OrderedVerifier, func(inputs []colexecop.Operator) (colexecop.Operator, error) {
			return NewOrderedSynchronizer(context.Background(), testAllocator, colexecop.DefaultMemoryLimit, colexectestutils.MakeInputs(inputs), typs, tc.ordering), nil
		})
	}
}
...
func TestOrderedSyncRandomInput(t *testing.T) {
	...
	op := NewOrderedSynchronizer(context.Background(), testAllocator, colexecop.DefaultMemoryLimit, inputs, typs, ordering)
	op.Init()
...
func BenchmarkOrderedSynchronizer(b *testing.B) {
	...
	ordering := colinfo.ColumnOrdering{{ColIdx: 0, Direction: encoding.Ascending}}
	op := NewOrderedSynchronizer(ctx, testAllocator, colexecop.DefaultMemoryLimit, inputs, typs, ordering)
	op.Init()
