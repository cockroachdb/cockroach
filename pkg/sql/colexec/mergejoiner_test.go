...
var mjTestCases = []*joinTestCase{
	...
	},
	{
		...
		leftPhysTypes:  []coltypes.T{coltypes.Bool, coltypes.Int16, coltypes.Float64},
		rightPhysTypes: []coltypes.T{coltypes.Bool, coltypes.Int16, coltypes.Float64},
	...
	},
	{
		...
		leftPhysTypes:  []coltypes.T{coltypes.Bool, coltypes.Int16, coltypes.Float64},
		rightPhysTypes: []coltypes.T{coltypes.Int16, coltypes.Float64, coltypes.Bool},
		leftTuples:     tuples{{false, int16(10), 1.2}, {true, int16(20), 2.2}, {true, int16(30), 3.2}},
	...
	},
	{
		...
		joinType:       sqlbase.JoinType_FULL_OUTER,
		leftPhysTypes:  []coltypes.T{coltypes.Int64, coltypes.Bytes},
		rightPhysTypes: []coltypes.T{coltypes.Int64, coltypes.Bytes},
					...
					runner(t, []tuples{tc.leftTuples, tc.rightTuples},
						...
						func(input []colbase.Operator) (colbase.Operator, error) {
							...
							args := NewColOperatorArgs{
								...
								FDSemaphore:         colbase.NewTestingSemaphore(mjFDLimit),
							}
		...
		t.Run(fmt.Sprintf("outBatchSize=%d", outBatchSize),
			func(t *testing.T) {
				...
				a, err := newMergeJoinOp(
					...
					colbase.NewTestingSemaphore(mjFDLimit), sqlbase.FullOuterJoin,
		...
		t.Run(fmt.Sprintf("outBatchSize=%d", outBatchSize),
			func(t *testing.T) {
				...
				mj, err := newMergeJoinOp(
					testAllocator, defaultMemoryLimit, queueCfg,
					colbase.NewTestingSemaphore(mjFDLimit), sqlbase.InnerJoin,
			...
			t.Run(fmt.Sprintf("numInputBatches=%d", numInputBatches),
				func(t *testing.T) {
					...
					a, err := newMergeJoinOp(
						...
						queueCfg, colbase.NewTestingSemaphore(mjFDLimit), sqlbase.InnerJoin,
						leftSource, rightSource, typs, typs,
						[]execinfrapb.Ordering_Column{{ColIdx: 0, Direction: execinfrapb.Ordering_Column_ASC}},
						[]execinfrapb.Ordering_Column{{ColIdx: 0, Direction: execinfrapb.Ordering_Column_ASC}},
						testDiskAcc,
		...
		b.Run(fmt.Sprintf("rows=%d", nBatches*coldata.BatchSize()), func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				...
				base, err := newMergeJoinBase(
					colbase.NewAllocator(ctx, &benchMemAccount), defaultMemoryLimit, queueCfg, colbase.NewTestingSemaphore(mjFDLimit),
