	...
	for _, tc := range tcs {
		...
		runner(t, []tuples{tc.leftTuples, tc.rightTuples}, nil /* typs */, tc.expected, mergeJoinVerifier,
			func(input []Operator) (Operator, error) {
				lOrderingCols := make([]execinfrapb.Ordering_Column, len(tc.leftEqCols))
				for i, colIdx := range tc.leftEqCols {
					lOrderingCols[i].ColIdx = colIdx
					lOrderingCols[i].Direction = tc.leftDirections[i]
				}
				rOrderingCols := make([]execinfrapb.Ordering_Column, len(tc.rightEqCols))
				for i, colIdx := range tc.rightEqCols {
					rOrderingCols[i].ColIdx = colIdx
					rOrderingCols[i].Direction = tc.leftDirections[i]
				}
