...
var mjTestCases = []joinTestCase{
	...
	},
	{
		...
		leftLogTypes:  []types.T{*types.Bool, *types.Int2, *types.Float},
		rightLogTypes: []types.T{*types.Bool, *types.Int2, *types.Float},
	...
	},
	{
		...
		leftLogTypes:  []types.T{*types.Bool, *types.Int2, *types.Float},
		rightLogTypes: []types.T{*types.Int2, *types.Float, *types.Bool},
		leftTuples:    tuples{{false, int16(10), 1.2}, {true, int16(20), 2.2}, {true, int16(30), 3.2}},
	...
	},
	{
		...
		joinType:      sqlbase.JoinType_FULL_OUTER,
		leftLogTypes:  []types.T{*types.Int, *types.Bytes},
		rightLogTypes: []types.T{*types.Int, *types.Bytes},
	...
	for _, outBatchSize := range []uint16{1, 16, coldata.BatchSize() - 1, coldata.BatchSize(), coldata.BatchSize() + 1} {
		t.Run(fmt.Sprintf("outBatchSize=%d", outBatchSize),
			func(t *testing.T) {
				...
				colsLeft := []coldata.Vec{testAllocator.NewMemColumn(physTypes[0], nTuples)}
				colsRight := []coldata.Vec{testAllocator.NewMemColumn(physTypes[0], nTuples)}
				...
				leftSource := newChunkingBatchSource(physTypes, colsLeft, uint64(nTuples))
				rightSource := newChunkingBatchSource(physTypes, colsRight, uint64(nTuples))
		...
		for _, outBatchSize := range []uint16{1, 16, coldata.BatchSize()} {
			t.Run(fmt.Sprintf("numInputBatches=%d", numInputBatches),
				func(t *testing.T) {
					...
					logTypes := []types.T{*types.Int}
					physTypes := []coltypes.T{coltypes.Int64}
					cols := []coldata.Vec{testAllocator.NewMemColumn(physTypes[0], nTuples)}
					groups := cols[0].Int64()
					for i := range groups {
						groups[i] = int64(i)
					}

					leftSource := newChunkingBatchSource(physTypes, cols, uint64(nTuples))
					rightSource := newChunkingBatchSource(physTypes, cols, uint64(nTuples))
		...
		for _, numInputBatches := range []int{1, 2, 16} {
			t.Run(fmt.Sprintf("groupSize=%d/numInputBatches=%d", groupSize, numInputBatches),
				func(t *testing.T) {
					...
					physTypes := []coltypes.T{coltypes.Int64, coltypes.Int64}
					cols := []coldata.Vec{
						testAllocator.NewMemColumn(physTypes[0], nTuples),
						testAllocator.NewMemColumn(physTypes[1], nTuples),
					}
				...
				for _, randomIncrement := range []int64{0, 1} {
					t.Run(fmt.Sprintf("numInputBatches=%dmaxRunLength=%dskipValues=%trandomIncrement=%d", numInputBatches, maxRunLength, skipValues, randomIncrement),
						func(t *testing.T) {
							...
							logTypes := []types.T{*types.Int}
							physTypes := []coltypes.T{coltypes.Int64}
							lCols, rCols, exp := newBatchesOfRandIntRows(nTuples, physTypes, maxRunLength, skipValues, randomIncrement)
							leftSource := newChunkingBatchSource(physTypes, lCols, uint64(nTuples))
							rightSource := newChunkingBatchSource(physTypes, rCols, uint64(nTuples))
