		...
		for _, outBatchSize := range []int{1, 16, coldata.BatchSize()} {
			t.Run(fmt.Sprintf("numInputBatches=%d", numInputBatches),
				func(t *testing.T) {
					...
					a, err := newMergeJoinOp(
						testAllocator, externalOperatorUsageModeDefault, defaultMemoryLimit,
		...
		b.Run(fmt.Sprintf("rows=%d", nBatches*coldata.BatchSize()), func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				...
				base, err := newMergeJoinBase(
					NewAllocator(ctx, &benchMemAccount), externalOperatorUsageModeDefault,
					defaultMemoryLimit, queueCfg, NewTestingSemaphore(mjFDLimit),
					leftSource, rightSource, []uint32{0, 1}, []uint32{2, 3},
