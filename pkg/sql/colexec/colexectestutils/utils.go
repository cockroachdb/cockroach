...
func RunTestsWithOrderedCols(
	...
	OUTER:
		...
		for {
			originalBatch := getBatch(originalOp)
			batchWithNulls := getBatch(opWithNulls)
...
func RunTestsWithoutAllNullsInjectionWithErrorHandler(
	...
	if !skipRandomNullsInjection {
		...
		if err = colexecerror.CatchVectorizedRuntimeError(func() {
			op.Init(ctx)
			for b := getBatch(op); b.Length() > 0; b = getBatch(op) {
			}
		}); err != nil {
			errorHandler(err)
		}
...
func getBatch(op colexecop.Operator) coldata.Batch {
	batch, meta := op.Next()
	if meta != nil {
		colexecerror.InternalError(errors.AssertionFailedf("non-nil meta: %v", meta))
	}
	return batch
}
...
func (f *finiteChunksSource) Next() (coldata.Batch, *execinfrapb.ProducerMetadata) {
	if f.usableCount > 0 {
		f.usableCount--
		batch, _ := f.repeatableBatch.Next()
