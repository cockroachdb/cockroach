	...
	for {
		...
		case externalSorterSpillPartition:
			...
			if b.Length() == 0 {
				...
				s.inMemSorterInput.notPropagateReset = true
				s.inMemSorter.reset()
				s.numPartitions++
...
type inputPartitioningOperator struct {
	...
	standaloneMemAccount *mon.BoundAccount
	memoryLimit          int64
	// notPropagateReset determines whether the reset method will be called on
...
func (o *inputPartitioningOperator) reset() {
	if !o.notPropagateReset {
		if r, ok := o.input.(resetter); ok {
			r.reset()
		}
	}
	o.notPropagateReset = false
