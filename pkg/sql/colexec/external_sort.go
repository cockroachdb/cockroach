...
func (s *externalSorter) reset() {
	// We need the "deep" resetting behavior of inputPartitioningOperator, so
	// we have to set the corresponding knob.
	s.inMemSorterInput.propagateReset = true
...
type inputPartitioningOperator struct {
	...
	standaloneMemAccount *mon.BoundAccount
	memoryLimit          int64
	// propagateReset determines whether the reset method will be called on the
	// input to this operator when the latter is being reset. This field is
