...
type externalSorter struct {
	...
	// (those in [currentPartitionIdx, currentPartitionIdx+numPartitions) range).
	partitionsInfo struct {
		...
		//
		// This struct may no longer be in need after the most recent
...
func (s *externalSorter) createMergerForPartitions(
	...
	if log.V(2) {
		...
		log.Infof(ctx,
			"external sorter is merging partitions with partition indices %d with sizes [%s]",
