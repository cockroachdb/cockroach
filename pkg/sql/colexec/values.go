...
func (v *valuesOp) Init(ctx context.Context) {
	...
	if len(v.data) < coldata.BatchSize() {
		v.batch = v.allocator.NewMemBatchWithFixedCapacity(v.typs, len(v.data))
	} else {
		v.batch = v.allocator.NewMemBatchWithMaxCapacity(v.typs)
	}
