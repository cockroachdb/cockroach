...
// a one-column colphystypes.Int64 batch where each element is its ordinal and an
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			o := newRouterOutputOpWithBlockedThresholdAndBatchSize(
				testAllocator, []colphystypes.T{colphystypes.Int64}, unblockEventsChan, tc.blockedThreshold, tc.outputBatchSize,
			)
...
func TestRouterOutputNext(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			o := newRouterOutputOp(testAllocator, []colphystypes.T{colphystypes.Int64}, unblockedEventsChan)
	...
	t.Run("AddBatchDoesntBlockWhenOutputIsBlocked", func(t *testing.T) {
		...
		o := newRouterOutputOpWithBlockedThresholdAndBatchSize(
			testAllocator, []colphystypes.T{colphystypes.Int64}, ch, blockThreshold, int(coldata.BatchSize()),
		)
...
func TestHashRouterComputesDestination(t *testing.T) {
	...
	r := newHashRouterWithOutputs(in, []colphystypes.T{colphystypes.Int64}, []int{0}, nil /* ch */, outputs)
...
func TestHashRouterCancellation(t *testing.T) {
	...
	r := newHashRouterWithOutputs(in, []colphystypes.T{colphystypes.Int64}, []int{0}, unbufferedCh, outputs)
...
func BenchmarkHashRouter(b *testing.B) {
	...
	types := []colphystypes.T{colphystypes.Int64}
