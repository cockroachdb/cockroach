...
func BenchmarkValues(b *testing.B) {
	...
	for _, numRows := range []int{1 << 4, 1 << 8, 1 << 12, 1 << 16} {
		for _, numCols := range []int{1, 2, 4} {
			...
			b.Run(fmt.Sprintf("rows=%d,cols=%d,valuesOp", numRows, numCols), func(b *testing.B) {
				...
				b.ResetTimer()
				for i := 0; i < b.N; i++ {
					op := NewValuesOp(testAllocator, &spec)
					op.Init(ctx)
			...
			b.Run(fmt.Sprintf("rows=%d,cols=%d,valuesProcessor", numRows, numCols), func(b *testing.B) {
				typs := types.MakeIntCols(numCols)
				rows := randgen.MakeIntRows(numRows, numCols)
				spec, err := execinfra.GenerateValuesSpec(typs, rows)
				if err != nil {
				...
				for i := 0; i < b.N; i++ {
					...
					op, err := NewBufferingColumnarizer(
