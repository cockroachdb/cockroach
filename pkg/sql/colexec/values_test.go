...
func typeConvFn(t *types.T) func(tree.Datum) interface{} {
	// GetDatumToPhysicalFn panics if it cannot find a conversion function, but we
	// simply don't use the type in that case, so discard the panic and return
	// nil.
	defer func() {
		recover()
	}()
...
func BenchmarkValues(b *testing.B) {
	...
	for _, numRows := range []int{1 << 4, 1 << 8, 1 << 12, 1 << 16} {
		for _, numCols := range []int{1, 2, 4} {
			...
			subBenchmarkValues(b, ctx, numRows, numCols, "valuesOpNative",
			...
			subBenchmarkValues(b, ctx, numRows, numCols, "valuesProcWrap",
				func(spec *execinfrapb.ValuesCoreSpec) (colexecop.Operator, error) {
