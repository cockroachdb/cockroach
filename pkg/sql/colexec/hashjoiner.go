...
func (hj *hashJoiner) emitUnmatched() {
	...
	for outColIdx, inColIdx := range hj.ht.outCols {
		...
		// TODO(yuzefovich): I think this behavior is acceptable - we allocated
		// hj.output batch already, so the concern here is only for the
		// variable-sized types that exceed our estimations.
		outCol.Copy(
...
func (hj *hashJoiner) reset() {
	...
	// TODO(yuzefovich): hash table does *not* release the memory under
	// hj.ht.vals, so fully clearing the allocator is incorrect. Not sure what to
	// do here, but we wouldn't want to release that memory, yet we need to reset
	// the allocator partially.
	hj.allocator.Clear()
	hj.state = hjBuilding
	hj.ht.reset()
