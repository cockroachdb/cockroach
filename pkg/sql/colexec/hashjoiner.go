...
func (hj *hashJoiner) congregate(nResults uint64, batch coldata.Batch, batchSize uint64) {
...
func (hj *hashJoiner) reset() {
	...
	copy(hj.probeState.probeIdx[:coldata.BatchSize()], zeroUint64Column)
	if hj.spec.left.outer {
...
func newHashJoiner(
	...
	hj.probeState.probeIdx = make([]uint64, coldata.BatchSize())
	if spec.left.outer {
