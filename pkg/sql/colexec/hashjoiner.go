...
type hashJoiner struct {
	...
	// allocator should be used when building the hash table from the right
	// input.
	allocator *colmem.Allocator
	// unlimitedAllocator should *only* be used when populating the output when
	// we have already fully built the hash table from the right input and are
	// only populating output one batch at a time. If we were to use a limited
	// allocator, we could hit the memory limit during output population, and
	// it would have been very hard to fall back to disk backed hash joiner
	// because we might have already emitted partial output.
...
func (hj *hashJoiner) emitUnmatched() {
	...
	hj.unlimitedAllocator.PerformOperation(hj.output.ColVecs(), func() {
...
func (hj *hashJoiner) resetOutput(nResults int) {
	...
	hj.output, _ = hj.unlimitedAllocator.ResetMaybeReallocate(hj.outputTypes, hj.output, minCapacity)
}
...
// right input tables.
// allocator should use a limited memory account and will be used for the build
// side whereas unlimitedAllocator should use an unlimited memory account and
// will only be used when populating the output.
func NewHashJoiner(
	allocator, unlimitedAllocator *colmem.Allocator,
	spec HashJoinerSpec,
