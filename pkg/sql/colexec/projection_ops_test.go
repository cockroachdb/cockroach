...
func TestRandomComparisons(t *testing.T) {
	...
	for _, ct := range types.Scalar {
		...
		if typ == colphystypes.Unhandled {
			continue
		}
		typs := []colphystypes.T{typ, typ, colphystypes.Bool}
		bytesFixedLength := 0
...
func benchmarkProjOp(
	b *testing.B,
	makeProjOp func(source *RepeatableBatchSource, intType colphystypes.T) Operator,
	useSelectionVector bool,
	hasNulls bool,
	intType colphystypes.T,
	outputType colphystypes.T,
) {
	ctx := context.Background()

	batch := testAllocator.NewMemBatch([]colphystypes.T{intType, intType, outputType})
	switch intType {
...
func BenchmarkProjOp(b *testing.B) {
	projOpMap := map[string]func(*RepeatableBatchSource, colphystypes.T) Operator{
		"projPlusIntIntOp": func(source *RepeatableBatchSource, intType colphystypes.T) Operator {
			base := projOpBase{
				OneInputNode: NewOneInputNode(source),
				allocator:    testAllocator,
				col1Idx:      0,
		...
		"projMinusIntIntOp": func(source *RepeatableBatchSource, intType colphystypes.T) Operator {
			base := projOpBase{
				OneInputNode: NewOneInputNode(source),
				allocator:    testAllocator,
				col1Idx:      0,
		...
		"projMultIntIntOp": func(source *RepeatableBatchSource, intType colphystypes.T) Operator {
			base := projOpBase{
				OneInputNode: NewOneInputNode(source),
				allocator:    testAllocator,
				col1Idx:      0,
		...
		"projDivIntIntOp": func(source *RepeatableBatchSource, intType colphystypes.T) Operator {
			base := projOpBase{
				OneInputNode: NewOneInputNode(source),
				allocator:    testAllocator,
				col1Idx:      0,
	...
	for projOp, makeProjOp := range projOpMap {
		for _, intType := range []colphystypes.T{colphystypes.Int64, colphystypes.Int32} {
			for _, useSel := range []bool{true, false} {
				for _, hasNulls := range []bool{true, false} {
					b.Run(fmt.Sprintf("op=%s/type=%s/useSel=%t/hasNulls=%t",
						...
						if projOp == "projDivIntIntOp" {
							outputType = colphystypes.Decimal
						}
