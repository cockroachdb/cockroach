						...
						b.Run(fmt.Sprintf("proj%s%s%s/useSel=%t/hasNulls=%t",
							...
							func(b *testing.B) {
								benchmarkProjOp(b, func(source *colexecbase.RepeatableBatchSource, left, right *types.T) (colexecbase.Operator, error) {
									var (
										projOp          colexecbase.Operator
										typeCheckingErr error
									)
									// Two kinds of errors could occur during the creation of a
									// projecting operator:
									// 1. a projecting operator with the given left and right types
									//    doesn't exist, and an error will occur during type
									//    checking. It is assigned inside of the panic-catcher.
									// 2. there is a valid projecting operator with the given left
									//    and right types, but the vectorized engine doesn't
									//    support it. In such case in the production code we fall
									//    back to row-by-row engine, but this benchmark doesn't
									//    concern itself with such scenario, so we pass in 'false'
									//    to prohibit the fallback. As a result, a nil-pointer
									//    exception will occur, but we will catch it and return as
									//    an error for such configuration to be skipped.
									if vecPlanningErr := colexecerror.CatchVectorizedRuntimeError(func() {
										projOp, typeCheckingErr = createTestProjectingOperator(
											ctx, flowCtx, source, []*types.T{left, right},
											fmt.Sprintf("@1 %s @2", opInfixForm), false, /* canFallbackToRowexec */
										)
									}); vecPlanningErr != nil {
										return projOp, vecPlanningErr
									}
									return projOp, typeCheckingErr
