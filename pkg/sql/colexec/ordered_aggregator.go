...
type orderedAggregator struct {
	...
	state struct {
		current, next orderedAggregatorState
	}
	...
	for {
		...
		case orderedAggregatorAggregating:
			...
			if a.scratch.resumeIdx >= coldata.BatchSize() {
				a.state.current = orderedAggregatorOutputting
				continue
			}
			...
			if batchLength == 0 {
				a.state.current = orderedAggregatorOutputting
				a.state.next = orderedAggregatorDone
				continue
			}
		...
		case orderedAggregatorReallocating:
			...
			a.scratch.shouldResetInternalBatch = false
			a.state.current = orderedAggregatorAggregating
			continue
		...
		default:
			colexecerror.InternalError(errors.AssertionFailedf("unexpected orderedAggregatorState %d", a.state.current))
		}
	}
}
