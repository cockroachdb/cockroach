				...
				t.Run(fmt.Sprintf("sizeLimit=%d", sizeLimit), func(t *testing.T) {
					for _, useSel := range []bool{true, false} {
						t.Run(fmt.Sprintf("sel=%v", useSel), func(t *testing.T) {
							...
							colBuilder := NewColSpanAssembler(keys.TODOSQLCodec, testAllocator, testTable,
								testTable.GetPrimaryIndex(), typs, neededColumns, sizeLimit)
							defer func() {
								colBuilder.Close()
								colBuilder.Release()
							}()
							...
							for batch := source.Next(); ; batch = source.Next() {
								...
								if colBuilder.ConsumeBatch(batch) {
