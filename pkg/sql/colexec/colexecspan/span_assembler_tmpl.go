...
type ColSpanAssembler interface {
	...
	// no work. The newly accumulated spans are accounted for.
	...
	//
	// The returned object is no longer accounted for by the ColSpanAssembler,
	// so it is the caller's responsibility to do so.
