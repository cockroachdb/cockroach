...
func (op *_OP_STRING) ConsumeBatch(batch coldata.Batch) (reachedLimit bool) {
	...
	// Check whether the number of key bytes allocated so far for this batch has
	// exceeded the limit.
	return op.keyBytes >= op.sizeLimit
}
...
// operator, but release it once the SpanAssembler loses its references though
// it can still be referenced elsewhere. The two cases are the spans slice (it
// is put into a pool) and the underlying bytes for the keys (they are
