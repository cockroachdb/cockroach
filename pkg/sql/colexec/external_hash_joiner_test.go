...
// that can be the maximum limit on the number of partitions for the external
// hash joiner. External hash joiner needs at least two partitions per side
// (i.e. we need at least 4), but during the fallback to sort + merge join we
// will be using 6 (external sorter on each side will use 2 to read its own
// partitions and 1 more to write out).
