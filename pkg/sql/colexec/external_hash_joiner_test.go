	...
	for _, hasNulls := range []bool{false, true} {
		...
		for _, fullOuter := range []bool{false, true} {
			for _, rightDistinct := range []bool{true, false} {
				for _, nBatches := range []int{1 << 2, 1 << 10} {
					for _, memoryLimit := range []int64{0, 1} {
						flowCtx.Cfg.TestingKnobs.MemoryLimitBytes = memoryLimit
						shouldSpill := memoryLimit > 0
						name := fmt.Sprintf(
							"nulls=%t/fullOuter=%t/distinct=%t/batches=%d/spilled=%t",
							hasNulls, fullOuter, rightDistinct, nBatches, shouldSpill)
						...
						spec := createSpecForHashJoiner(joinTestCase{
							...
							rightEqColsAreKey: rightDistinct,
						})
...
func createDiskBackedHashJoiner(
	...
	err := NewColOperator(ctx, flowCtx, args, &result)
