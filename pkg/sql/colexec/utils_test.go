...
type testRunner func(*testing.T, []tuples, [][]phystypes.T, tuples, verifier, func([]Operator) (Operator, error))
...
//   testing with t.
func runTestsWithFn(
	t *testing.T, tups []tuples, typs [][]phystypes.T, test func(t *testing.T, inputs []Operator),
) {
...
// of arbitrary Go phystypes. It's meant to be used in Operator unit tests in
...
//   {5,6,7.0,false},
// }
// tupleSource := newOpTestInput(inputTuples, phystypes.Bool)
...
func newOpTestInput(batchSize uint16, tuples tuples, typs []phystypes.T) *opTestInput {
	ret := &opTestInput{
		batchSize: batchSize,
		tuples:    tuples,
		typs:      typs,
...
func newOpTestSelInput(
	rng *rand.Rand, batchSize uint16, tuples tuples, typs []phystypes.T,
) *opTestInput {
	ret := &opTestInput{
		useSel:    true,
		rng:       rng,
...
func (s *opFixedSelTestInput) Init() {
	...
	typs := make([]phystypes.T, len(s.tuples[0]))
	for i := range typs {
		// Default type for test cases is Int64 in case the entire column is null
		// and the type is indeterminate.
		...
		for _, tup := range s.tuples {
			if tup[i] != nil {
				typs[i] = phystypes.FromGoType(tup[i])
				break
			}
		}
	}
...
func getTupleFromBatch(batch coldata.Batch, tupleIdx uint16) tuple {
	...
	for colIdx := range ret {
		...
		if vec.Nulls().NullAt(tupleIdx) {
			...
			} else if vec.Type() == phystypes.Decimal {
				colDec := vec.Decimal()
...
func newChunkingBatchSource(
	typs []phystypes.T, cols []coldata.Vec, len uint64,
) *chunkingBatchSource {
