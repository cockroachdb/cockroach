...
func (w *lagBoolWindow) processBatch(batch coldata.Batch, startIdx, endIdx int) {
	...
	lagVec := batch.ColVec(w.outputColIdx)
	lagCol := lagVec.Bool()
	lagNulls := lagVec.Nulls()
	_ = lagCol.Get(startIdx)
	_ = lagCol.Get(endIdx - 1)
		...
		if defaultNulls.MaybeHasNulls() {
			for i := startIdx; i < endIdx; i++ {
				...
				if requestedIdx < 0 || requestedIdx >= w.partitionSize {
					...
					lagCol.CopySlice(defaultCol, i, i, i+1)
					continue
				}
				vec := w.buffer.GetWindowedBatch(w.Ctx, requestedIdx).ColVec(0)
				if vec.Nulls().NullAt(0) {
