...
func _VALUE_BY_OFFSET(_, _, _, _, _, _ interface{}) {
...
func _ASSIGN_CMP(_, _, _, _, _ interface{}) {
	colexecerror.InternalError(errors.AssertionFailedf(""))
}
...
type rangeOffsetHandler interface {
	...
	// location of the last bound index.
...
type _OP_STRING struct {
	...
	// {{if .NeedsHelper}}
	// {{/*
	// overloadHelper is used only when we work with Datum columns which is the
	// case when {{if .NeedsHelper}} evaluates to true. In all other cases we
	// don't want to wastefully allocate the helper.
	// */}}
...
func (h *_OP_STRING) getIdx(ctx context.Context, currRow, lastIdx int) (idx int) {
	...
	batch, batchIdx := h.storedCols.GetBatchWithTuple(ctx, currRow)
	vec := batch.ColVec(h.ordColIdx)
	...
	if vec.Nulls().MaybeHasNulls() && vec.Nulls().NullAt(batchIdx) {
		...
		for {
			...
			batch, batchIdx = h.storedCols.GetBatchWithTuple(ctx, idx)
			col := batch.ColVec(h.peersColIdx).Bool()
	...
	currRowVal := col.Get(batchIdx)
	_VALUE_BY_OFFSET(seekVal, currRowVal, h.offset, _, _, _)
	...
	if vec.Nulls().MaybeHasNulls() && vec.Nulls().NullAt(batchIdx) {
		...
		for {
			...
			batch, batchIdx = h.storedCols.GetBatchWithTuple(ctx, idx)
			peersCol := batch.ColVec(h.peersColIdx).Bool()
			for ; batchIdx < batch.Length(); batchIdx++ {
				if peersCol[batchIdx] {
	...
	for {
		...
		for ; batchIdx < batch.Length(); batchIdx++ {
			if peersCol[batchIdx] {
				...
				// {{end}}
				cmpVal := col.Get(batchIdx)
				_ASSIGN_CMP(cmpResult, cmpVal, seekVal, col, _)
				// {{if .IsOrdColAsc}}
				// {{if .IsStart}}
				if cmpResult >= 0 {
					return idx
...
func getOffsetType(orderColType *types.T) *types.T {
	switch orderColType {
	case types.Timestamp, types.TimestampTZ, types.Date, types.Time, types.TimeTZ:
