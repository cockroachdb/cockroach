...
const (
	...
	// been reached, in which the next state is nTileProcessing.
	...
	// disk resources and emit the zero-length batch. nTileFinished is the
	// terminal state.
	nTileFinished
)
...
type nTileBase struct {
	...
	// processingIdx is an index into the head of the queue (or currentBatch when
	// the queue is empty). It is used to preserve state when the nTileProcessing
	// state spans more than one batch.
	processingIdx int
...
// {{/*
// _GET_NUM_BUCKETS is a code snippet that scans past rows until either the
// start of the next partition is found, or the first non-null value for
// num_buckets is found.
func _GET_NUM_BUCKETS(_HAS_SEL bool) { // */}}
	...
	if partitionCol[i] {
		...
		// {{if $.HasSel}}
		if selIdx != r.seekIdx || !r.bufferQueue.Empty() {
			break
		}
		// {{else}}
...
// {{/*
// _COMPUTE_PARTITION_SIZE is a code snippet that scans past rows until the
// start of the next partition is reached.
...
func (r *_NTILE_STRINGOp) Init(ctx context.Context) {
	...
	r.bufferQueue = colexecutils.NewSpillingQueue(
		&colexecutils.NewSpillingQueueArgs{
			...
			Types:              append(outputTypes, types.Int),
...
// that the loading and seeking states have run to completion for the portion of
// the current partition represented by the interval [r.processingIdx, endIdx).
