...
const (
	// nTileLoading is the state in which case ntile operators load an additional
	...
	// been reached, in which the next state is nTileProcessing or nTileFinished
	// depending on whether all rows have already been emitted.
	nTileLoading nTileState = iota
	// nTileSeeking is the state in which case ntile operators seek to the index
	// of the next partition and retrieve the number of ntile buckets for the
	// current partition, as well as the number of tuples in the partition.
	//
	...
	nTileSeeking
	// nTileProcessing is the state in which case ntile operators fill in the
	...
	// partition has been fully processed. It can transition to nTileLoading when
	// a batch is already filled with nulls (the argument is null on every row).
	// Finally, nTileProcessing will transition to nTileFinished when all batches
	// have been fully processed and emitted.
	nTileProcessing
	// nTileFinished is the state in which case ntile operators close any
...
type nTileBase struct {
	...
	// processingIdx is an index into the tuples buffered in the queue, or into
	// currentBatch if the queue is empty). It is used to preserve state when the
	// nTileProcessing state spans more than one batch.
	processingIdx int
...
func (r *_NTILE_STRINGOp) Init(ctx context.Context) {
	...
	outputTypes := make([]*types.T, len(r.inputTypes), len(r.inputTypes)+1)
	copy(outputTypes, r.inputTypes)
	...
	for {
		...
		case nTileLoading:
			...
			if batch.Length() == 0 {
				...
				if !r.bufferQueue.Empty() || r.currentBatch.Length() > 0 {
			...
			if r.currentBatch.Selection() != nil {
				// The copying that is performed above should leave the current batch
				// without a selection vector since selected tuples are densely packed
				// into the new batch.
				colexecerror.InternalError(
					errors.AssertionFailedf("expected copy to densely pack batch"))
			}
			r.seekIdx = 0
		...
		case nTileProcessing:
			if !r.bufferQueue.Empty() {
				...
				// batch. Note that if a batch is in r.bufferQueue, the end of the
				// current partition must be located beyond it.
				r.setNTile(output, output.Length())
				r.processingIdx -= output.Length()
				if r.processingIdx < 0 {
					// The first of the tuples to be processed is located in this batch.
					r.processingIdx = 0
				}
				return output
			}
