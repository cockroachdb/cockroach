...
func NewLastValueOperator(
	...
	case types.EnumFamily:
		...
		case -1:
		default:
			windower := &lastValueEnumWindow{lastValueBase: base}
			return newBufferedWindowOperator(args, windower, argType, mainMemLimit), nil
		}
...
type lastValueEnumWindow struct {
	lastValueBase
}

var _ bufferedWindower = &lastValueEnumWindow{}

// processBatch implements the bufferedWindower interface.
func (w *lastValueEnumWindow) processBatch(batch coldata.Batch, startIdx, endIdx int) {
	if startIdx >= endIdx {
		// No processing needs to be done for this portion of the current partition.
		return
	}
