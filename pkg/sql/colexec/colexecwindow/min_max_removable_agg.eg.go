...
func newMinRemovableAggregator(
	...
	case types.EnumFamily:
		...
		case -1:
		default:
			return &minEnumAggregator{minMaxRemovableAggBase: base}
		}
...
// processBatch implements the bufferedWindower interface.
func (a *minEnumAggregator) processBatch(batch coldata.Batch, startIdx, endIdx int) {
	if endIdx <= startIdx {
		// There is no processing to be done.
		return
	}
...
// aggregateOverIntervals accumulates all rows represented by the given
// intervals into the current aggregate.
func (a *minEnumAggregator) aggregateOverIntervals(intervals []windowInterval) {
	for _, interval := range intervals {
		var cmp bool
		for j := interval.start; j < interval.end; j++ {
			idxToAdd := uint32(j)
...
func (a *minEnumAggregator) Close(ctx context.Context) {
	a.queue.close()
	a.framer.close()
	a.buffer.Close(ctx)
	*a = minEnumAggregator{}
...
func newMaxRemovableAggregator(
	...
	case types.EnumFamily:
		...
		case -1:
		default:
			return &maxEnumAggregator{minMaxRemovableAggBase: base}
		}
	case typeconv.DatumVecCanonicalTypeFamily:
		switch argTyp.Width() {
		case -1:
...
// processBatch implements the bufferedWindower interface.
func (a *maxEnumAggregator) processBatch(batch coldata.Batch, startIdx, endIdx int) {
	if endIdx <= startIdx {
		// There is no processing to be done.
		return
	}
...
// aggregateOverIntervals accumulates all rows represented by the given
// intervals into the current aggregate.
func (a *maxEnumAggregator) aggregateOverIntervals(intervals []windowInterval) {
	for _, interval := range intervals {
		var cmp bool
		for j := interval.start; j < interval.end; j++ {
			idxToAdd := uint32(j)
...
func (a *maxEnumAggregator) Close(ctx context.Context) {
	a.queue.close()
	a.framer.close()
	a.buffer.Close(ctx)
	*a = maxEnumAggregator{}
