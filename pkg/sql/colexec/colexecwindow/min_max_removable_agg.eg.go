...
const (
	...
	// The slice of uint32s in the dequeue can have up to 10,000 values (40KB).
...
type minMaxRemovableAggBase struct {
	...
	// A partial dequeue of indices into the current partition ordered by the
	// value of the input column at each index. It contains only indices that are
	// part of the current window frame. The first value in the queue is the index
	// of the current value for the aggregation (NULL if empty). Under the
