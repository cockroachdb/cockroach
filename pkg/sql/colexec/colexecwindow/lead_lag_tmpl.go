...
func (w *_OP_NAME_TYPEWindow) processBatch(batch coldata.Batch, startIdx, endIdx int) {
	...
	_OP_NAMECol := _OP_NAMEVec.TemplateType()
	_OP_NAMENulls := _OP_NAMEVec.Nulls()
	// {{if .Sliceable}}
	_ = _OP_NAMECol.Get(startIdx)
	_ = _OP_NAMECol.Get(endIdx-1)
	// {{end}}
...
func _PROCESS_BATCH(_OFFSET_HAS_NULLS bool, _DEFAULT_HAS_NULLS bool) { // */}}
	...
	for i := startIdx; i < endIdx; i++ {
		...
		if requestedIdx < 0 || requestedIdx >= w.partitionSize {
			...
			// {{end}}
			// {{if .IsBytesLike}}
			_OP_NAMECol.CopySlice(defaultCol, i, i, i+1)
			// {{else}}
			// {{with .Global}}
			val := defaultCol.Get(i)
			execgen.SET(_OP_NAMECol, i, val)
			// {{end}}
			// {{end}}
			continue
		}
		...
		_OP_NAMECol.CopySlice(col, i, 0, 1)
		// {{else}}
		val := col.Get(0)
		// {{if .Sliceable}}
		//gcassert:bce
