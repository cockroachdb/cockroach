...
func NewLeadOperator(
	...
	bufferMemLimit := int64(float64(memoryLimit) * 0.25)
	buffer := colexecutils.NewSpillingBuffer(
		unlimitedAllocator, bufferMemLimit, diskQueueCfg, fdSemaphore, []*types.T{argType}, diskAcc)
	base := leadBase{
		buffer:          buffer,
...
func (w *leadBoolWindow) seekNextPartition(
	...
	w.buffer.AppendTuples(w.Ctx, batch, startIdx, nextPartitionIdx, w.argIdx)
...
func (w *leadBoolWindow) processBatch(batch coldata.Batch, startIdx, endIdx int) {
	...
	leadVec := batch.ColVec(w.outputColIdx)
	leadCol := leadVec.Bool()
	leadNulls := leadVec.Nulls()
	_ = leadCol.Get(startIdx)
	_ = leadCol.Get(endIdx - 1)
	...
	if offsetNulls.MaybeHasNulls() {
		if defaultNulls.MaybeHasNulls() {
			for i := startIdx; i < endIdx; i++ {
				...
				vec := w.buffer.GetWindowedBatch(w.Ctx, requestedIdx).ColVec(0)
				if vec.Nulls().NullAt(0) {
					leadNulls.SetNull(i)
					continue
				}
				col := vec.Bool()
		...
		if defaultNulls.MaybeHasNulls() {
			for i := startIdx; i < endIdx; i++ {
				...
				if requestedIdx < 0 || requestedIdx >= w.partitionSize {
					...
					leadCol.CopySlice(defaultCol, i, i, i+1)
					continue
				}
				vec := w.buffer.GetWindowedBatch(w.Ctx, requestedIdx).ColVec(0)
				if vec.Nulls().NullAt(0) {
