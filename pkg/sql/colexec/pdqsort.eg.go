...
// insertionSort sorts data[a:b] using insertion sort.
func (p *sortINetAscWithNullsOp) insertionSort(a, b int) {
	for i := a + 1; i < b; i++ {
		for j := i; j > a && p.Less(j, j-1); j-- {
			p.Swap(j, j-1)
		}
...
// siftDown implements the heap property on data[lo:hi].
// first is an offset into the array where the root of the heap lies.
func (p *sortINetAscWithNullsOp) siftDown(lo, hi, first int) {
	root := lo
	for {
		child := 2*root + 1
		if child >= hi {
...
func (p *sortINetAscWithNullsOp) heapSort(a, b int) {
	first := a
	lo := 0
	hi := b - a
...
// Rust implementation: https://docs.rs/pdqsort/latest/pdqsort/
// limit is the number of allowed bad (very unbalanced) pivots before falling back to heapsort.
func (p *sortINetAscWithNullsOp) pdqsort(a, b, limit int) {
	const maxInsertion = 12
...
// Moves elements in data[a:b] around, so that data[i]<p and data[j]>=p for i<newpivot and j>newpivot.
// On return, data[newpivot] = p
func (p *sortINetAscWithNullsOp) partition(
	a, b, pivot int,
) (newpivot int, alreadyPartitioned bool) {
	p.Swap(a, pivot)
	i, j := a+1, b-1 // i and j are inclusive of the elements remaining to be partitioned
...
// partitionEqual partitions data[a:b] into elements equal to data[pivot] followed by elements greater than data[pivot].
// It assumed that data[a:b] does not contain elements smaller than the data[pivot].
func (p *sortINetAscWithNullsOp) partitionEqual(a, b, pivot int) (newpivot int) {
	p.Swap(a, pivot)
	i, j := a+1, b-1 // i and j are inclusive of the elements remaining to be partitioned
...
// partialInsertionSort partially sorts a slice, returns true if the slice is sorted at the end.
func (p *sortINetAscWithNullsOp) partialInsertionSort(a, b int) bool {
	const (
		maxSteps         = 5  // maximum number of adjacent out-of-order pairs that will get shifted
		shortestShifting = 50 // don't shift any elements on short arrays
	)
...
// breakPatterns scatters some elements around in an attempt to break some patterns
// that might cause imbalanced partitions in quicksort.
func (p *sortINetAscWithNullsOp) breakPatterns(a, b int) {
	length := b - a
	if length >= 8 {
		random := xorshift(length)
		modulus := nextPowerOfTwo(length)
...
// [8,shortestNinther): uses the simple median-of-three method.
// [shortestNinther,∞): uses the Tukey ninther method.
func (p *sortINetAscWithNullsOp) choosePivot(a, b int) (pivot int, hint sortedHint) {
	const (
		shortestNinther = 50
		maxSwaps        = 4 * 3
	)
...
// order2 returns x,y where data[x] <= data[y], where x,y=a,b or x,y=b,a.
func (p *sortINetAscWithNullsOp) order2(a, b int, swaps *int) (int, int) {
	if p.Less(b, a) {
		*swaps++
		return b, a
	}
...
// median returns x where data[x] is the median of data[a],data[b],data[c], where x is a, b, or c.
func (p *sortINetAscWithNullsOp) median(a, b, c int, swaps *int) int {
	a, b = p.order2(a, b, swaps)
	b, _ = p.order2(b, c, swaps)
	_, b = p.order2(a, b, swaps)
	return b
...
// medianAdjacent finds the median of data[a - 1], data[a], data[a + 1] and stores the index into a.
func (p *sortINetAscWithNullsOp) medianAdjacent(a int, swaps *int) int {
	return p.median(a-1, a, a+1, swaps)
}

func (p *sortINetAscWithNullsOp) reverseRange(a, b int) {
	i := a
	j := b - 1
	for i < j {
		p.Swap(i, j)
...
// insertionSort sorts data[a:b] using insertion sort.
func (p *sortINetDescWithNullsOp) insertionSort(a, b int) {
	for i := a + 1; i < b; i++ {
		for j := i; j > a && p.Less(j, j-1); j-- {
			p.Swap(j, j-1)
		}
...
// siftDown implements the heap property on data[lo:hi].
// first is an offset into the array where the root of the heap lies.
func (p *sortINetDescWithNullsOp) siftDown(lo, hi, first int) {
	root := lo
	for {
		child := 2*root + 1
		if child >= hi {
...
func (p *sortINetDescWithNullsOp) heapSort(a, b int) {
	first := a
	lo := 0
	hi := b - a
...
// Rust implementation: https://docs.rs/pdqsort/latest/pdqsort/
// limit is the number of allowed bad (very unbalanced) pivots before falling back to heapsort.
func (p *sortINetDescWithNullsOp) pdqsort(a, b, limit int) {
	const maxInsertion = 12
...
// Moves elements in data[a:b] around, so that data[i]<p and data[j]>=p for i<newpivot and j>newpivot.
// On return, data[newpivot] = p
func (p *sortINetDescWithNullsOp) partition(
	a, b, pivot int,
) (newpivot int, alreadyPartitioned bool) {
	p.Swap(a, pivot)
	i, j := a+1, b-1 // i and j are inclusive of the elements remaining to be partitioned
...
// partitionEqual partitions data[a:b] into elements equal to data[pivot] followed by elements greater than data[pivot].
// It assumed that data[a:b] does not contain elements smaller than the data[pivot].
func (p *sortINetDescWithNullsOp) partitionEqual(a, b, pivot int) (newpivot int) {
	p.Swap(a, pivot)
	i, j := a+1, b-1 // i and j are inclusive of the elements remaining to be partitioned
...
// partialInsertionSort partially sorts a slice, returns true if the slice is sorted at the end.
func (p *sortINetDescWithNullsOp) partialInsertionSort(a, b int) bool {
	const (
		maxSteps         = 5  // maximum number of adjacent out-of-order pairs that will get shifted
		shortestShifting = 50 // don't shift any elements on short arrays
	)
...
// breakPatterns scatters some elements around in an attempt to break some patterns
// that might cause imbalanced partitions in quicksort.
func (p *sortINetDescWithNullsOp) breakPatterns(a, b int) {
	length := b - a
	if length >= 8 {
		random := xorshift(length)
		modulus := nextPowerOfTwo(length)
...
// [8,shortestNinther): uses the simple median-of-three method.
// [shortestNinther,∞): uses the Tukey ninther method.
func (p *sortINetDescWithNullsOp) choosePivot(a, b int) (pivot int, hint sortedHint) {
	const (
		shortestNinther = 50
		maxSwaps        = 4 * 3
	)
...
// order2 returns x,y where data[x] <= data[y], where x,y=a,b or x,y=b,a.
func (p *sortINetDescWithNullsOp) order2(a, b int, swaps *int) (int, int) {
	if p.Less(b, a) {
		*swaps++
		return b, a
	}
...
// median returns x where data[x] is the median of data[a],data[b],data[c], where x is a, b, or c.
func (p *sortINetDescWithNullsOp) median(a, b, c int, swaps *int) int {
	a, b = p.order2(a, b, swaps)
	b, _ = p.order2(b, c, swaps)
	_, b = p.order2(a, b, swaps)
	return b
...
// medianAdjacent finds the median of data[a - 1], data[a], data[a + 1] and stores the index into a.
func (p *sortINetDescWithNullsOp) medianAdjacent(a int, swaps *int) int {
	return p.median(a-1, a, a+1, swaps)
}

func (p *sortINetDescWithNullsOp) reverseRange(a, b int) {
	i := a
	j := b - 1
	for i < j {
		p.Swap(i, j)
...
// insertionSort sorts data[a:b] using insertion sort.
func (p *sortINetAscOp) insertionSort(a, b int) {
	for i := a + 1; i < b; i++ {
		for j := i; j > a && p.Less(j, j-1); j-- {
			p.Swap(j, j-1)
		}
...
// siftDown implements the heap property on data[lo:hi].
// first is an offset into the array where the root of the heap lies.
func (p *sortINetAscOp) siftDown(lo, hi, first int) {
	root := lo
	for {
		child := 2*root + 1
		if child >= hi {
...
func (p *sortINetAscOp) heapSort(a, b int) {
	first := a
	lo := 0
	hi := b - a
...
// Rust implementation: https://docs.rs/pdqsort/latest/pdqsort/
// limit is the number of allowed bad (very unbalanced) pivots before falling back to heapsort.
func (p *sortINetAscOp) pdqsort(a, b, limit int) {
	const maxInsertion = 12
...
// Moves elements in data[a:b] around, so that data[i]<p and data[j]>=p for i<newpivot and j>newpivot.
// On return, data[newpivot] = p
func (p *sortINetAscOp) partition(
	a, b, pivot int,
) (newpivot int, alreadyPartitioned bool) {
	p.Swap(a, pivot)
	i, j := a+1, b-1 // i and j are inclusive of the elements remaining to be partitioned
...
// partitionEqual partitions data[a:b] into elements equal to data[pivot] followed by elements greater than data[pivot].
// It assumed that data[a:b] does not contain elements smaller than the data[pivot].
func (p *sortINetAscOp) partitionEqual(a, b, pivot int) (newpivot int) {
	p.Swap(a, pivot)
	i, j := a+1, b-1 // i and j are inclusive of the elements remaining to be partitioned
...
// partialInsertionSort partially sorts a slice, returns true if the slice is sorted at the end.
func (p *sortINetAscOp) partialInsertionSort(a, b int) bool {
	const (
		maxSteps         = 5  // maximum number of adjacent out-of-order pairs that will get shifted
		shortestShifting = 50 // don't shift any elements on short arrays
	)
...
// breakPatterns scatters some elements around in an attempt to break some patterns
// that might cause imbalanced partitions in quicksort.
func (p *sortINetAscOp) breakPatterns(a, b int) {
	length := b - a
	if length >= 8 {
		random := xorshift(length)
		modulus := nextPowerOfTwo(length)
...
// [8,shortestNinther): uses the simple median-of-three method.
// [shortestNinther,∞): uses the Tukey ninther method.
func (p *sortINetAscOp) choosePivot(a, b int) (pivot int, hint sortedHint) {
	const (
		shortestNinther = 50
		maxSwaps        = 4 * 3
	)
...
// order2 returns x,y where data[x] <= data[y], where x,y=a,b or x,y=b,a.
func (p *sortINetAscOp) order2(a, b int, swaps *int) (int, int) {
	if p.Less(b, a) {
		*swaps++
		return b, a
	}
...
// median returns x where data[x] is the median of data[a],data[b],data[c], where x is a, b, or c.
func (p *sortINetAscOp) median(a, b, c int, swaps *int) int {
	a, b = p.order2(a, b, swaps)
	b, _ = p.order2(b, c, swaps)
	_, b = p.order2(a, b, swaps)
	return b
...
// medianAdjacent finds the median of data[a - 1], data[a], data[a + 1] and stores the index into a.
func (p *sortINetAscOp) medianAdjacent(a int, swaps *int) int {
	return p.median(a-1, a, a+1, swaps)
}

func (p *sortINetAscOp) reverseRange(a, b int) {
	i := a
	j := b - 1
	for i < j {
		p.Swap(i, j)
...
// insertionSort sorts data[a:b] using insertion sort.
func (p *sortINetDescOp) insertionSort(a, b int) {
	for i := a + 1; i < b; i++ {
		for j := i; j > a && p.Less(j, j-1); j-- {
			p.Swap(j, j-1)
		}
...
// siftDown implements the heap property on data[lo:hi].
// first is an offset into the array where the root of the heap lies.
func (p *sortINetDescOp) siftDown(lo, hi, first int) {
	root := lo
	for {
		child := 2*root + 1
		if child >= hi {
...
func (p *sortINetDescOp) heapSort(a, b int) {
	first := a
	lo := 0
	hi := b - a
...
// Rust implementation: https://docs.rs/pdqsort/latest/pdqsort/
// limit is the number of allowed bad (very unbalanced) pivots before falling back to heapsort.
func (p *sortINetDescOp) pdqsort(a, b, limit int) {
	const maxInsertion = 12
...
// Moves elements in data[a:b] around, so that data[i]<p and data[j]>=p for i<newpivot and j>newpivot.
// On return, data[newpivot] = p
func (p *sortINetDescOp) partition(
	a, b, pivot int,
) (newpivot int, alreadyPartitioned bool) {
	p.Swap(a, pivot)
	i, j := a+1, b-1 // i and j are inclusive of the elements remaining to be partitioned
...
// partitionEqual partitions data[a:b] into elements equal to data[pivot] followed by elements greater than data[pivot].
// It assumed that data[a:b] does not contain elements smaller than the data[pivot].
func (p *sortINetDescOp) partitionEqual(a, b, pivot int) (newpivot int) {
	p.Swap(a, pivot)
	i, j := a+1, b-1 // i and j are inclusive of the elements remaining to be partitioned
...
// partialInsertionSort partially sorts a slice, returns true if the slice is sorted at the end.
func (p *sortINetDescOp) partialInsertionSort(a, b int) bool {
	const (
		maxSteps         = 5  // maximum number of adjacent out-of-order pairs that will get shifted
		shortestShifting = 50 // don't shift any elements on short arrays
	)
...
// breakPatterns scatters some elements around in an attempt to break some patterns
// that might cause imbalanced partitions in quicksort.
func (p *sortINetDescOp) breakPatterns(a, b int) {
	length := b - a
	if length >= 8 {
		random := xorshift(length)
		modulus := nextPowerOfTwo(length)
...
// [8,shortestNinther): uses the simple median-of-three method.
// [shortestNinther,∞): uses the Tukey ninther method.
func (p *sortINetDescOp) choosePivot(a, b int) (pivot int, hint sortedHint) {
	const (
		shortestNinther = 50
		maxSwaps        = 4 * 3
	)
...
// order2 returns x,y where data[x] <= data[y], where x,y=a,b or x,y=b,a.
func (p *sortINetDescOp) order2(a, b int, swaps *int) (int, int) {
	if p.Less(b, a) {
		*swaps++
		return b, a
	}
...
// median returns x where data[x] is the median of data[a],data[b],data[c], where x is a, b, or c.
func (p *sortINetDescOp) median(a, b, c int, swaps *int) int {
	a, b = p.order2(a, b, swaps)
	b, _ = p.order2(b, c, swaps)
	_, b = p.order2(a, b, swaps)
	return b
...
// medianAdjacent finds the median of data[a - 1], data[a], data[a + 1] and stores the index into a.
func (p *sortINetDescOp) medianAdjacent(a int, swaps *int) int {
	return p.median(a-1, a, a+1, swaps)
}

func (p *sortINetDescOp) reverseRange(a, b int) {
	i := a
	j := b - 1
	for i < j {
		p.Swap(i, j)
