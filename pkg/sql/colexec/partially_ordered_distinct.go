...
func (c *chunkerOperator) Next(ctx context.Context) coldata.Batch {
	...
	if c.newChunksCol == nil {
		...
		if outputTupleEndIdx-c.outputTupleStartIdx > uint64(coldata.BatchSize()) {
			outputTupleEndIdx = c.outputTupleStartIdx + uint64(coldata.BatchSize())
		}
	} else {
	...
	c.windowedBatch.SetLength(uint16(outputTupleEndIdx - c.outputTupleStartIdx))
