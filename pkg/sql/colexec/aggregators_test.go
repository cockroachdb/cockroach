...
func (tc *aggregatorTestCase) init(agg aggType) error {
	if tc.convToDecimal {
	...
	if agg.name == "ordered" {
		tc.spec.OrderedGroupCols = tc.groupCols
	}
	return nil
}
...
func TestHashAggregator(t *testing.T) {
	...
	for i := range aggTypes {
		if aggTypes[i].name == "hash" {
			hashAgg = aggTypes[i]
			break
		}
	}
	for _, tc := range tcs {
		if err := tc.init(hashAgg); err != nil {
			t.Fatal(err)
		}
		constructors, constArguments, outputTypes, err := ProcessAggregations(
			&evalCtx, nil /* semaCtx */, tc.spec.Aggregations, tc.typs,
		...
		runTests(t, []tuples{tc.input}, tc.expected, unorderedVerifier, func(sources []colexecbase.Operator) (colexecbase.Operator, error) {
			return hashAgg.new(
				testAllocator, testMemAcc, sources[0], tc.typs, tc.spec, &evalCtx,
				constructors, constArguments, outputTypes, false, /* isScalar*/
			)
		})
	}
