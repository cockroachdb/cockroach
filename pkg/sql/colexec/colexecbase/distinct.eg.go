...
// distinctEnumOp runs a distinct on the column in distinctColIdx, writing
// true to the resultant bool column for every value that differs from the
// previous one.
type distinctEnumOp struct {
	colexecop.OneInputHelper
...
var _ colexecop.ResettableOperator = &distinctEnumOp{}

func (p *distinctEnumOp) Reset(ctx context.Context) {
	p.foundFirstRow = false
	p.lastValNull = false
	if resetter, ok := p.Input.(colexecop.Resetter); ok {
		resetter.Reset(ctx)
...
func (p *distinctEnumOp) Next() coldata.Batch {
	batch := p.Input.Next()
	if batch.Length() == 0 {
		return batch
	}
