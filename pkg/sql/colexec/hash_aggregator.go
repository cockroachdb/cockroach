...
func (op *hashAggregator) Next(ctx context.Context) coldata.Batch {
	for {
		...
		case hashAggregatorBuffering:
			...
			op.buildSelectionForHashCode(ctx)
...
func (op *hashAggregator) bufferBatch(ctx context.Context) (done bool) {
...
func (op *hashAggregator) buildSelectionForHashCode(ctx context.Context) {
	...
	hashBuffer := op.hashBuffer[:op.scratch.Length()]
	...
	for hashCode, sel := range op.scratch.sels {
		...
		if aggFuncs, ok := op.aggFuncMap[hashCode]; ok {
			for _, aggFunc := range aggFuncs {
				...
				remaining = aggFunc.match(
					remaining, op.scratch, op.groupCols, op.groupTypes, op.keyMappig, &matched,
				)
				if len(matched) > 0 {
					op.scratch.SetLength(uint16(len(matched)))
		...
		// This is nil only if the hashCode does not exist in the aggFuncMap.
		if remaining == nil {
			remaining = sel
		}
		...
		for len(remaining) > 0 {
			...
			aggFunc := &hashAggFunc{keyIdx: op.keyMappig.nextAvailableIdx}
			op.keyMappig.nextAvailableIdx++

			// Store the key of the current aggregating group into keyMapping.
			for keyIdx, colIdx := range op.groupCols {
				op.keyMappig.ColVec(keyIdx).Append(coldata.SliceArgs{
					Src:         op.scratch.ColVec(int(colIdx)),
					ColType:     op.valTypes[colIdx],
					DestIdx:     uint64(aggFunc.keyIdx),
					SrcStartIdx: uint64(groupSel[0]),
					SrcEndIdx:   uint64(groupSel[0] + 1),
				})
			}
			...
			// Select rest of the tuples that matches the current key. Stores the
			// selection vector of matched tuples into groupSel.
			remaining = aggFunc.match(
				remaining, op.scratch, op.groupCols, op.groupTypes, op.keyMappig,
				&groupSel,
			)

			op.scratch.SetLength(uint16(len(groupSel)))

			// Hack required to get aggregation function working. See '.scratch.group'
			// field comment in hashAggregator for more details.
			op.scratch.group[groupSel[0]] = true
			aggFunc.init(op.scratch.group, op.output)
			aggFunc.compute(op.scratch, op.aggCols)
			op.scratch.group[groupSel[0]] = false
		}
	}
}
...
func (v *hashAggFunc) compute(b coldata.Batch, aggCols [][]uint32) {
	for fnIdx, fn := range v.fns {
