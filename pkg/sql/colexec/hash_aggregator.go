...
func (op *hashAggregator) Close(ctx context.Context) error {
	// toClose contains defaultHashAggAllocs which are responsible for closing
	// all tree.AggregateFuncs they have created. tree.AggregateFunc.Close
	// doesn't return an error, and that's why it's ok to "swallow" errors here
	// because they won't actually occur.
