...
// buffered up for aggregation at once. We want this number to be
// coldata.MaxBatchSize, but then we would lose some test coverage due to
// disabling of the randomization of the batch size, so we, instead, use
// 4 x coldata.BatchSize() (which ends up being coldata.MaxBatchSize in
// non-test environment).
var hashAggregatorMaxBuffered = 4 * coldata.BatchSize()
...
func (op *hashAggregator) Init() {
	...
	op.scratch.eqChains = make([][]int, hashAggregatorMaxBuffered)
	op.scratch.intSlice = make([]int, hashAggregatorMaxBuffered)
	op.scratch.anotherIntSlice = make([]int, hashAggregatorMaxBuffered)
