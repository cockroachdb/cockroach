...
type hashAggregator struct {
	...
	testingKnobs struct {
		// numOfHashBuckets is the maximum hash buckets each tuple will be hash to.
		// When it is 0 hash aggregator will no enforce maximum number of hash
		// buckets. It is used to test hash collision.
...
// onlineAgg probes aggFuncMap using the built sels map and lazily create
// aggFunctions for each group if it doesn not exist. Then it calls Compute() on
// each aggregation function to perform aggregation.
