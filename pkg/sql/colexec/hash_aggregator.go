...
func (op *hashAggregator) onlineAgg(b coldata.Batch) {
	...
	op.ht.FindBuckets(
		b, op.ht.Keys, op.ht.ProbeScratch.First, op.ht.ProbeScratch.Next,
		op.ht.CheckProbeForDistinct, false, /* zeroHeadIDForDistinctTuple */
	)
	...
	if len(op.buckets) > 0 {
		...
		op.ht.FindBuckets(
			b, op.ht.Keys, op.ht.BuildScratch.First, op.ht.BuildScratch.Next,
			op.ht.CheckBuildForAggregation, zeroHeadIDForDistinctTuple,
		)
