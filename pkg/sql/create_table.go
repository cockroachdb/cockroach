...
func newTableDesc(
	...
	params.p.runWithOptions(resolveFlags{skipCache: true, contextDatabaseID: db.GetID()}, func() {
		...
		// table descriptor. Self-referencing FKs are skipped because the table
		// does not exist yet and the creator implicitly has all privileges on it.
		for _, def := range n.Defs {
			if d, ok := def.(*tree.ForeignKeyConstraintTableDef); ok {
				if d.Table.ObjectName == n.Table.ObjectName {
					continue
				}
