...
// invalidClusterForShardedIndex defines the error to be returned when the user tries
// to create a hash sharded index on a cluster that hasn't migrated to 20.1.
		...
		if d, ok := def.(*tree.ColumnTableDef); ok {
			...
			if d.PrimaryKey.Sharded {
				if !cluster.Version.IsActive(ctx, st, cluster.VersionHashShardedIndexes) {
					return desc, invalidClusterForShardedIndex
				}
