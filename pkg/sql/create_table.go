...
// ResolveUniqueConstraint looks up the columns mentioned in a UNIQUE WITHOUT INDEX
	...
	for _, def := range n.Defs {
		...
		case *tree.UniqueConstraintTableDef:
			if d.WithoutIndex {
				...
				if len(d.Storing) > 0 {
					return nil, pgerror.New(pgcode.FeatureNotSupported,
						"stored columns for unique constraints without an index are not supported",
					)
				}
				if d.Interleave != nil {
					return nil, pgerror.New(pgcode.FeatureNotSupported,
...
func replaceLikeTableOpts(n *tree.CreateTable, params runParams) (tree.TableDefs, error) {
	...
	for i, def := range n.Defs {
		...
		if opts.Has(tree.LikeTableOptConstraints) {
			...
			for _, c := range td.UniqueConstraints {
