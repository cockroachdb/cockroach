...
// this new expression tree alongsize a set containing the ColumnID of each
...
func makeCheckConstraint(
	...
	sortedColIDsUsed := make([]sqlbase.ColumnID, 0, len(colIDsUsed))
	for colID := range colIDsUsed {
		sortedColIDsUsed = append(sortedColIDsUsed, colID)
	}
	sort.Slice(sortedColIDsUsed, func(i, j int) bool {
		return sortedColIDsUsed[i] < sortedColIDsUsed[j]
	})

	return &sqlbase.TableDescriptor_CheckConstraint{
		Expr:      tree.Serialize(d.Expr),
		Name:      name,
		ColumnIDs: sortedColIDsUsed,
	}, nil
}
