...
func MakeTableDesc(
	...
	for i := range desc.Columns {
		...
		if col.IsComputed() {
			...
			expr, err = schemaexpr.DequalifyColumnRefs(sourceInfo, expr)
			if err != nil {
				return desc, err
			}
	...
	idxValidator := schemaexpr.NewIndexPredicateValidator(n.Table, &desc, semaCtx)
	...
	ckBuilder := schemaexpr.NewCheckConstraintBuilder(n.Table, &desc, semaCtx)
	for _, def := range n.Defs {
		switch d := def.(type) {
	...
	computedColValidator := schemaexpr.NewComputedColumnValidator(&desc, semaCtx)
	for _, def := range n.Defs {
		switch d := def.(type) {
		case *tree.ColumnTableDef:
			if d.IsComputed() {
