...
func (p *planner) CreateTable(ctx context.Context, n *tree.CreateTable) (planNode, error) {
	...
	for _, def := range n.Defs {
		...
		case *tree.ForeignKeyConstraintTableDef:
			if _, err := t.Table.NormalizeWithDatabaseName(p.evalCtx.SessData.Database); err != nil {
				return nil, err
			}
		}
	}
...
func (n *createTableNode) startExec(params runParams) error {
	...
	if n.n.As() {
		desc, err = makeTableDescIfAs(
			...
			privs, &params.p.semaCtx, &params.evalCtx.EvalContext)
	} else {
	...
	if err := MakeEventLogger(params.p.LeaseMgr()).InsertEventRecord(
		...
		struct {
			...
			Statement string
			User      string
		}{n.n.Table.String(), n.n.String(), params.p.evalCtx.SessData.User},
	); err != nil {
		return err
	}
...
func (p *planner) addInterleave(
	...
	return addInterleave(
		...
		interleave, p.evalCtx.SessData.Database)
}
...
func (p *planner) makeTableDesc(
	...
	return MakeTableDesc(
		...
		privileges,
		affected,
		p.evalCtx.SessData.Database,
		&p.semaCtx,
		&p.evalCtx.EvalContext,
	)
}
