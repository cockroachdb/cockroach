...
func (n *createTableNode) startExec(params runParams) error {
	...
	if n.n.As() {
		desc, err = makeTableDescIfAs(
			...
			privs, &params.p.semaCtx, &params.extendedEvalCtx.EvalContext)
	} else {
	...
	if err := MakeEventLogger(params.p.LeaseMgr()).InsertEventRecord(
		...
		struct {
			...
			Statement string
			User      string
		}{n.n.Table.String(), n.n.String(), params.extendedEvalCtx.SessionData.User},
	); err != nil {
		return err
	}
