...
func NewTableDesc(
	...
	if isRegionalByRow {
		...
		for _, def := range n.Defs {
			...
			case *tree.ColumnTableDef:
				if d.Name == regionalByRowCol {
					...
					if t.Oid() != typedesc.TypeIDToOID(regionEnumID) {
						err = pgerror.Newf(
							pgcode.InvalidTableDefinition,
						...
						if t, terr := vt.ResolveTypeByOID(
							ctx,
							typedesc.TypeIDToOID(regionEnumID),
						); terr == nil {
		...
		if !regionalByRowColExists {
			...
			oid := typedesc.TypeIDToOID(regionEnumID)
			n.Defs = append(
				n.Defs,
...
func newTableDesc(
	...
	multiRegionEnumID := descpb.InvalidID
	if dbDesc.IsMultiRegion() {
		regionConfig, err = SynthesizeRegionConfig(params.ctx, params.p.txn, dbDesc.ID, params.p.Descriptors())
		if err != nil {
			return nil, err
		}
		multiRegionEnumID, err = dbDesc.MultiRegionEnumID()
	}
