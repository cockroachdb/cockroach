...
func (v *FixCastForStyleVisitor) VisitPost(expr tree.Expr) tree.Expr {
	...
	if expr, ok := expr.(*tree.CastExpr); ok {
		...
		if err != nil {
			// We run DequalifyAndValidateExpr twice because the first time we need
			// the error to verify we are targeting on a stable cast. The second run
			// is to get expr's type, since there is a possibility in nested casts
			// that v.typ will not match expr.typ.
			...
			case types.StringFamily:
				if outerTyp.Family() == types.IntervalFamily {
