...
func TestPortalsDestroyedOnTxnFinish(t *testing.T) {
	...
	// Execute).
	cmdPos := 0
	stmt, sql := mustParseOne("SELECT 1")
	if err != nil {
		t.Fatal(err)
	}
	if err = buf.Push(ctx, PrepareStmt{Name: "ps_nontxn", Stmt: stmt, SQL: sql}); err != nil {
		t.Fatal(err)
	}
	...
	cmdPos++
	stmt, sql = mustParseOne("BEGIN")
	if err != nil {
		t.Fatal(err)
	}
	if err := buf.Push(ctx, ExecStmt{Stmt: stmt, SQL: sql}); err != nil {
		t.Fatal(err)
	}
	...
	if err = buf.Push(ctx, PrepareStmt{Name: "ps1", Stmt: stmt, SQL: sql}); err != nil {
		t.Fatal(err)
	}
	...
	cmdPos++
	stmt, sql = mustParseOne("SELECT 2")
	if err != nil {
		t.Fatal(err)
	}
	if err := buf.Push(ctx, ExecStmt{Stmt: stmt, SQL: sql}); err != nil {
...
func mustParseOne(s string) (tree.Statement, string) {
	stmts, sqls, err := parser.Parse(s)
	if err != nil {
		log.Fatal(context.TODO(), err)
	}
	return stmts[0], sqls[0]
}
