...
func releaseAutoStatsConcurrencyLocks(
	...
	// Simply delete the table-specific row since we allow at most one auto
	// stats job on any table.
	_, err := txn.ExecEx(
		ctx, "create-stats-release-table-lock", txn.KV(), sessiondata.InternalExecutorOverride{},
		"DELETE FROM system.table_statistics_locks WHERE table_id = $1 AND kind = $2",
		tableID, kind,
	)
	if err != nil {
		return err
