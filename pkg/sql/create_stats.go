...
func getAutoStatsKind(details jobspb.CreateStatsDetails) int {
	...
	case jobspb.AutoPartialStatsName:
		...
		if details.UsingSpans {
			...
			// collection based on 'partialPredicate' column.
			return 3
		}
		fallthrough
	default:
...
func (n *createStatsNode) makeJobRecord(ctx context.Context) (*jobs.Record, error) {
	...
	if n.Options.Where != nil {
		...
		skip := 2 // omit '/Table/<TableID>' prefix
		if !n.p.ExecCfg().Codec.ForSystemTenant() {
			skip = 4 // for tenants also omit '/Tenant/<TenantID>' prefix
		}
		// TODO(discuss during review): should we have an upper bound on the
		// size of the prettySpans string?
