...
func createStatsDefaultColumns(
	...
	for i := range desc.PrimaryIndex.ColumnIDs {
		...
		addSingleColumnStatIfNotExists(desc.PrimaryIndex.ColumnIDs[i], false /* isInverted */)
	...
	for i := range desc.Indexes {
		...
		for j := range desc.Indexes[i].ColumnIDs {
			...
			addSingleColumnStatIfNotExists(desc.Indexes[i].ColumnIDs[j], isInverted)
		...
		if desc.Indexes[i].IsPartial() {
			...
			for _, colID := range colIDs.Ordered() {
				addSingleColumnStatIfNotExists(colID, isInverted)
			}
		}
	}
