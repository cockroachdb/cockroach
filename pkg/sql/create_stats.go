...
func createStatsDefaultColumns(
	...
	// returns true.
	trackStatsIfNotExists := func(colIDs []sqlbase.ColumnID) bool {
	...
	addSingleColumnHistogramIfNotExists := func(colID sqlbase.ColumnID) {
		colList := []sqlbase.ColumnID{colID}
	...
	for i := range desc.Indexes {
		for j := range desc.Indexes[i].ColumnIDs {
			...
			if desc.Indexes[i].Type == sqlbase.IndexDescriptor_FORWARD {
