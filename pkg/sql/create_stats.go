...
func (n *createStatsNode) runJob(ctx context.Context) error {
	...
	if err := n.p.ExecCfg().InternalDB.Txn(ctx, func(ctx context.Context, txn isql.Txn) (err error) {
		...
		} else if n.isAuto() && !jobCheckBefore {
			// Don't start the job if there is already a CREATE STATISTICS job running.
...
// checkStatsJobsStillActive takes a list of job IDs and checks whether each job
// is still active, returning the list of still active jobs.
func checkStatsJobsStillActive(
	ctx context.Context, txn isql.Txn, jobIDs tree.Datums,
) (tree.Datums, error) {
	var stillActive tree.Datums
	for _, jobID := range jobIDs {
		id, ok := jobID.(*tree.DInt)
		if !ok {
			return nil, errors.AssertionFailedf("unexpectedly jobID is not *tree.DInt, found %T", jobID)
		}
		r, err := txn.QueryRowEx(
			ctx, "check-stats-job-still-active", txn.KV(), sessiondata.InternalExecutorOverride{},
			"SELECT status IN "+jobs.NonTerminalStateTupleString+" FROM system.jobs WHERE id = $1", int64(*id),
		)
		if err != nil {
			return nil, err
		}
		if r != nil && r[0] == tree.DBoolTrue {
			stillActive = append(stillActive, jobID)
		}
	}
...
// job ID should be used for the new stats job.
func (n *createStatsNode) checkConcurrencyLimit(
	...
	var kind, globalConcurrencyLimit int
	switch n.Name {
	case jobspb.AutoStatsName:
		kind = fullKind
		globalConcurrencyLimit = int(automaticFullStatsConcurrencyLimit.Get(n.p.ExecCfg().SV()))
	case jobspb.AutoPartialStatsName:
		kind = extremesKind
		globalConcurrencyLimit = int(automaticExtremesStatsConcurrencyLimit.Get(n.p.ExecCfg().SV()))
	}

	// First, limit the concurrency on the target table to at most one.
	row, err := txn.QueryRowEx(
		ctx, "create-stats-check-table-limit", txn.KV(), sessiondata.InternalExecutorOverride{},
		"SELECT job_ids FROM system.table_statistics_locks WHERE kind = $1 AND table_id = $2 FOR UPDATE",
		kind, tableID,
	)
	if err != nil {
		return err
	...
	if row != nil && row[0] != tree.DNull {
		// There already exists a lock on this table. We need to check whether
		// the jobs are still active (we expect at most one job ID).
		jobIDs := tree.MustBeDArray(row[0])
		stillActive, err := checkStatsJobsStillActive(ctx, txn, jobIDs.Array)
		if err != nil {
			return err
		}
	...
	// Now limit the global concurrency of the stats kind.
	row, err = txn.QueryRowEx(
	...
	if row != nil && row[0] != tree.DNull {
		// Check how many stats jobs of the target kind are still active
		// across all tables.
		jobIDs := tree.MustBeDArray(row[0])
		globalJobIDs, err = checkStatsJobsStillActive(ctx, txn, jobIDs.Array)
		if err != nil {
			return err
		}
		if len(globalJobIDs) >= globalConcurrencyLimit {
	...
	// We can start this stats job, so claim the locks accordingly.
	jobIDAsDatum := tree.DInt(jobID)
	_, err = txn.ExecEx(
		ctx, "create-stats-update-table-limit", txn.KV(), sessiondata.InternalExecutorOverride{},
		"UPSERT INTO system.table_statistics_locks VALUES ($1, $2, $3)",
		kind, tableID, tree.NewDArrayFromDatums(types.Int, tree.Datums{&jobIDAsDatum}),
	)
	if err != nil {
		return err
	}
	...
	_, err = txn.ExecEx(
		...
		"UPSERT INTO system.table_statistics_locks VALUES ($1, $2, $3)",
		kind, globalTableID, tree.NewDArrayFromDatums(types.Int, globalJobIDs),
	)
	return err
}
