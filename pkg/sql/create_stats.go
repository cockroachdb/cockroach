...
func checkRunningStatsJobs(ctx context.Context, job *jobs.Job, p *planner) error {
	err := checkRunningJobs(ctx, job, p, []jobspb.Type{jobspb.TypeCreateStats, jobspb.TypeAutoCreateStats})
...
// DuplicateJobError is reported when two redundant jobs are issued
// concurrently. This is a sentinel error.
var DuplicateJobError error = errors.New("another job is already running")
