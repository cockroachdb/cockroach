...
type copyMachine struct {
	...
	buf []byte
	// lastBufIdx is the index of the first byte in buf that has not been seen.
	lastBufIdx int
...
func (c *copyMachine) readTextData(ctx context.Context, final bool) (brk bool, err error) {
	idx := bytes.IndexByte(c.buf[c.lastBufIdx:], lineDelim)
	var line []byte
	if idx == -1 {
		if !final {
			// Leave the incomplete row in the buffer, to be processed next time.
			c.lastBufIdx = len(c.buf)
			return true, nil
		}
	...
	for {
		...
		if idx == -1 {
			if final {
				...
				// try to process it.
				fullLine = append(fullLine, c.buf...)
				c.buf = c.buf[len(c.buf):]
				break
			} else {
