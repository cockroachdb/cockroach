...
// copyRetryBufferSize can be used to enable the transparent retry of
...
func (c *copyMachine) readBinaryTuple(ctx context.Context) (err error) {
	var fieldCount int16
	var fieldCountBytes [2]byte
	_, err = io.ReadFull(c.reader, fieldCountBytes[:])
	if err != nil {
		return err
	}
	...
	for i := range datums {
		_, err = io.ReadFull(c.reader, byteCountBytes[:])
		if err != nil {
			return err
		}
