...
// ExtractTags Parses the comments the provided comment to extract sqlcommenter
// key value tags. If the comment is in the expected format, it returns a list
// of tags. Otherwise, it returns an empty list.
//
// The algorithm followed for parsing: https://google.github.io/sqlcommenter/spec/#parsing.
func ExtractTags(comment string) []Tags {
	...
	tags := make([]Tags, 0, len(pairs))
	sqlCommenterTags := make(map[string]struct{})
	for _, pair := range pairs {
		...
		tags = append(tags, Tags{key, redact.SafeString(value)})
	}

	return tags
}

func MaybeWithComments(sv *settings.Values) parser.ParseOption {
	if sqlCommenterEnabled.Get(sv) {
		return parser.WithComments(true)
	}
	return parser.WithComments(false)
}
