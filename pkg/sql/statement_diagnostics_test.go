...
func TestStmtDiagnosticsRequestRegistry_GetAllRequests_and_GetCompletedRequestWithDiagnostics_completed(t *testing.T) {
	defer leaktest.AfterTest(t)()
	s, db, _ := serverutils.StartServer(t, base.TestServerArgs{})
	ctx := context.Background()
	defer s.Stopper().Stop(ctx)
	...
	request, err := s.ExecutorConfig().(ExecutorConfig).stmtInfoRequestRegistry.GetCompletedRequestWithDiagnostics(ctx, requestId)
	require.NoError(t, err)
	require.True(t, request.Completed)
	require.NotNil(t, request.CompletedAt)

	json := request.TraceJSON
	require.Contains(t, json, "traced statement")
	require.Contains(t, json, "statement execution committed the txn")
}
