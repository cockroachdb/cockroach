...
type deleteRun struct {
	...
	// rows contains the accumulated result rows if rowsNeeded is needsValues.
...
// maxDeleteBatchRowCount is the max number of rows to process in a
...
// This combines with maxDeleteBatchRowCount above to limit
// the amount of work done in each DELETE batch.
...
func (d *deleteNode) startExec(params runParams) error {
	...
	if d.run.rowsNeeded == needValues {
		d.run.rows = sqlbase.NewRowContainer(
			params.EvalContext().Mon.MakeBoundAccount(),
			sqlbase.ColTypeInfoFromResCols(planColumns(d.source)),
			maxDeleteBatchRowCount)
	}
	return d.run.td.init(params.p.txn, params.EvalContext())
}

// Next implements the batchedPlanNode interface.
func (d *deleteNode) Next(params runParams) (bool, error) {
	...
	for {
		...
		if d.run.rows != nil {
			...
			if _, err := d.run.rows.AddRow(params.ctx, d.source.Values()); err != nil {
				return false, err
			}
		}
		d.run.rowCount++

		// Have we done yet with the current batch?
		if d.run.rowCount >= maxDeleteBatchRowCount ||
			d.run.td.batchSize >= maxDeleteBatchSize {
			break
		}
	}
	...
	if d.run.rowCount > 0 {
		...
		if lastBatch {
			...
			// Flush the current KV batch and start a new one.
...
// Values implement the batchedPlanNode interface.
