...
func (p *planner) AlterTenantSetClusterSetting(
	...
	// Disallow setting the 'version' setting for all tenants.
	if name == clusterversion.KeyVersionSetting && n.TenantSpec.All {
		return nil, errors.Errorf("cannot set '%s' for all tenants", name)
	}
