...
func (n *createTableNode) Start() error {
	...
	if n.n.As() {
		// TODO(knz): Ideally we would want to plug this sourcePlan as a
		// data source into the insertNode. However this is complicated
		// because at the point we reach this expandPlan() has already
		// been called on the sourcePlan (needed for EXPLAIN), and
		// insertPlan.expandPlan() would cause a 2nd call, which is
		// invalid. So we let the Insert constructor make a new one.
