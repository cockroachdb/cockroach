...
func resolveFK(
	...
	if d.Actions.Delete != parser.NoAction &&
		...
		feature := fmt.Sprintf("ON DELETE %s", d.Actions.Delete)
		return pgerror.Unimplemented(feature, "unsupported "+feature)
	}
	if d.Actions.Update != parser.NoAction &&
		d.Actions.Update != parser.Restrict {
		feature := fmt.Sprintf("ON UPDATE %s", d.Actions.Update)
		return pgerror.Unimplemented(feature, "unsupported "+feature)
	}
