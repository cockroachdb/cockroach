...
func (p *planner) DropType(ctx context.Context, n *tree.DropType) (planNode, error) {
	...
	for _, name := range n.Names {
		...
		mutArrayDesc, err := p.Descriptors().GetMutableTypeByID(ctx, p.txn, typeDesc.ArrayTypeID)
		if err != nil {
			return nil, err
		}
...
func (p *planner) addTypeBackReference(
	...
	mutDesc, err := p.Descriptors().GetMutableTypeByID(ctx, p.txn, typeID)
	if err != nil {
		return err
	}
...
func (p *planner) addBackRefsFromAllTypesInTable(
	...
	typeIDs, err := desc.GetAllReferencedTypeIDs(func(id descpb.ID) (catalog.TypeDescriptor, error) {
		mutDesc, err := p.Descriptors().GetMutableTypeByID(ctx, p.txn, id)
		if err != nil {
			return nil, err
		}
		return mutDesc, nil
