...
func (p *planner) DropType(ctx context.Context, n *tree.DropType) (planNode, error) {
	...
	for _, name := range n.Names {
		...
		arrayDesc, err := sqlbase.GetTypeDescFromID(ctx, p.txn, p.ExecCfg().Codec, typeDesc.ArrayTypeID)
		if err != nil {
			return nil, err
		}
		mutArrayDesc := sqlbase.NewMutableExistingTypeDescriptor(*arrayDesc.TypeDesc())
...
func (p *planner) addBackRefsFromAllTypesInTable(
	...
	typeIDs, err := desc.GetAllReferencedTypeIDs(func(id sqlbase.ID) (*sqlbase.TypeDescriptor, error) {
		// TODO (rohany): This should use the desc.Collection.
		desc, err := sqlbase.GetTypeDescFromID(ctx, p.txn, p.ExecCfg().Codec, id)
		if err != nil {
			return nil, err
		}
