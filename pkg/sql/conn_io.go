...
// StmtBuf maintains a list of gs that a SQL client has sent for execution
// over a network connection. The gs are SQL queries to be execututed,
// statements to be prepared, etc. At any point in time the buffer contains
// outstanding gs that have yet to be executed, and it can also contain
// some history of gs that we might want to retry - in the case of a
// retriable error, we'd like to retry all the gs pertaining to the
// current SQL transaction.
//
// The buffer is supposed to be used by one reader and one writer.  The writer
// adds gs to the buffer using Push(). The reader reads one command at a
...
// advanceOne(), seekToNextBatch() and rewind().
...
//
// The StmtBuf supports grouping gs into "batches". The writer specifies
// when a batch starts and ends via StartBatch. A reader can then at any time
// chose to skip over gs from the current batch. This is used to implement
...
// protocol), or to different gs pipelined by the cliend, separated from
// "sync" messages.
//
...
// executing gs again in case of an automatic retry. The ConnExecutor is
// in charge of trimming completed gs from the buffer when it's done with
// them.
type StmtBuf struct {
	mu struct {
		syncutil.Mutex
		...
		// cond is signaled when new gs are pushed.
		cond *sync.Cond
		...
		startPos cmdPos
		// cmdPos is the current position of the cursor going through the gs.
		// At any time, curPos indicates the position of the command to be returned
		// by curCmd().
		curPos cmdPos

		// curWriteBatch maintains the batch number that pushed gs belong to.
...
// gs were already available in the buffer before the Close()).
...
// StartBatch makes next gs to be pushed be part of a new batch.
...
// which we want to rewind within the stream of gs with:
