...
func RecordError(ctx context.Context, err error, sv *settings.Values) {
	...
	if pgErr, ok := pgerror.GetPGCause(err); ok && pgErr.Code == pgerror.CodeInternalError {
		...
		if len(pgErr.SafeDetail) == 0 {
			log.SendReport(ctx, "<redacted>", log.CrashReportOnly, nil)
			return
		}
		...
		if exc != nil {
			log.SendReport(ctx, headMsg, log.CrashReportOnly, extras, details, exc)
		} else {
			log.SendReport(ctx, headMsg, log.CrashReportOnly, extras, details)
		}
	}
}
