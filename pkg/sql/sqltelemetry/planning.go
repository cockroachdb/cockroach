...
func init() {
	...
	for i := 0; i < reorderJoinsCounters; i++ {
		reorderJoinLimitUseCounters[i] = telemetry.GetCounterOnce(fmt.Sprintf("sql.plan.reorder-joins.set-limit-%d", i))
	}
}
...
// IncJoinReorder is to be called whenever the reorder joins session variable
// is set.
func IncJoinReorder(value int) {
