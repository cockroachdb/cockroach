...
func (h *hashJoiner) Run(ctx context.Context, wg *sync.WaitGroup) {
	...
	useTempStorage := (distSQLUseTempStorage.Get() && distSQLUseTempStorageJoins.Get()) ||
	...
	if useTempStorage {
		...
		// limit. Since we fully consume the chosen stream in the buffer phase
		// and consume an equal amount of space from the opposite stream, we
		// only need to worry about falling back to disk in the buffer phase,
		// since the build and probe phases will use a fraction of memory used
		// to store rows.
	...
	if earlyExit, err := func() (bool, error) {
		...
		storedDiskRows := makeHashDiskRowContainer(h.flowCtx.tempStorage)
