...
func (h *hashJoiner) Run(ctx context.Context, wg *sync.WaitGroup) {
	...
	useTempStorage := distSQLUseTempStorage.Get() || h.testingKnobMemLimit > 0
	...
	fallBackToDisk := func(row sqlbase.EncDatumRow) error {
		storedDiskRows, err := makeHashDiskRowContainer(
			ctx,
			h.eqCols[h.storedSide],
			h.eqCols[otherSide(h.storedSide)],
			h.rows[h.storedSide],
			nil, /* marks */
	...
	if storedRows == nil {
		storedMemRows, err := makeHashRowContainer(
			ctx, h.eqCols[h.storedSide], h.eqCols[otherSide(h.storedSide)], &h.rows[h.storedSide],
		)
		if err != nil {
			...
			log.Infof(ctx, "error making hash row container %s", err)
			DrainAndClose(ctx, h.out.output, err /* cause */, h.leftSource, h.rightSource)
			return
		}
...
func (h *hashJoiner) probeRow(
	...
	// one match.
	probeMatched := false
	// storedMatched specifies, for each row matched on the equality columns,
	// whether this row also passes the ON condition.
	// TODO(asubiotto): A sparse representation might be better. storedMatched
	// is still a memory issue. Maybe rethink marking API.
	var storedMatched []bool
	...
	for i.Rewind(); ; i.Next() {
		...
		storedMatched = append(storedMatched, false)
		// If the ON condition failed, renderedRow is nil.
		if renderedRow != nil {
			probeMatched = true
			storedMatched[len(storedMatched)-1] = true
	...
	// Mark the rows on the stored side that matched the probe row. The unmarked
	// rows can then be iterated over for {right, left} outer joins (depending
	// on storedSide) and full outer joins.
	if probeMatched {
		if err := storedRows.SetMarks(ctx, row, storedMatched); err != nil {
			return false, err
		}
	}
