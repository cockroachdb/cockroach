...
func (h *hashJoiner) buildPhase(ctx context.Context) (bool, error) {
	...
	for {
		...
		encoded, hasNull, err := encodeColumnsOfRow(&h.datumAlloc, scratch, rrow, h.rightEqCols, false)
		if err != nil {
			return false, err
		}
...
func (h *hashJoiner) probePhase(ctx context.Context) (bool, error) {
	...
	for {
		...
		encoded, hasNull, err := encodeColumnsOfRow(&h.datumAlloc, scratch, lrow, h.leftEqCols, false)
		if err != nil {
			return true, err
		}
		scratch = encoded[:0]
