...
type hashJoiner struct {
	...
	// case we store that one in the buckets. So one of the containers will
	// contain the entire "stored" stream, the other just the start of the other
	// stream.
	rows [2]rowContainer
...
func (h *hashJoiner) probePhase(ctx context.Context) (earlyExit bool, _ error) {
	...
	for i := 0; i < bufferedRows.Len(); i++ {
		...
		earlyExit, err := h.probeRow(ctx, row, side)
	...
	for {
		...
		if earlyExit, err := h.probeRow(ctx, row, side); earlyExit || err != nil {
			return earlyExit, err
		}
	}
