...
const (
	...
	// read in hjReadingProbeSide to readProbeSide the stored hash map with.
	hjProbingRow
...
// hashJoiner performas a hash join. There is no guarantee on the output
// ordering.
...
func (h *hashJoiner) build() (hashJoinerState, sqlbase.EncDatumRow, *ProducerMetadata) {
	...
	// hashJoiner and performs initialization before a transition to nextState.
	setStoredSideTransition := func(
		side joinSide, nextState hashJoinerState,
	...
	if h.forcedStoredSide != nil {
		return setStoredSideTransition(*h.forcedStoredSide, hjConsumingStoredSide)
	}
	...
	for {
		...
		if leftUsage >= h.initialBufferSize && rightUsage >= h.initialBufferSize {
			...
			return setStoredSideTransition(rightSide, hjConsumingStoredSide)
		}
		...
		row, meta, emitDirectly, err := h.receiveNext(srcs[side], side)
		if err != nil {
			h.moveToDraining(err)
			return hjStateUnknown, nil, h.drainHelper()
		} else if meta != nil {
		...
		if row == nil {
			...
			return setStoredSideTransition(side, hjConsumingStoredSide)
		}
...
func (h *hashJoiner) consumeStoredSide() (hashJoinerState, sqlbase.EncDatumRow, *ProducerMetadata) {
	...
	for {
		row, meta, emitDirectly, err := h.receiveNext(source, side)
		if err != nil {
			h.moveToDraining(err)
			return hjStateUnknown, nil, h.drainHelper()
		} else if meta != nil {
		...
		if row == nil {
			...
			if rc, ok := h.storedRows.(*hashMemRowContainer); ok {
				err = h.maybeMakeMemErr(err, "reserving mark memory")
				if err == nil {
		...
		err = h.maybeMakeMemErr(err, "consuming stored side")
		if err == nil {
...
func (h *hashJoiner) readProbeSide() (hashJoinerState, sqlbase.EncDatumRow, *ProducerMetadata) {
	...
	if h.rows[side].Len() > 0 {
		...
		// The otherSide(h.storedSide) has been fully consumed. Move on to
		// hjEmittingUnmatched if unmatched rows on the stored side need to be
		// emitted, otherwise finish.
		if row == nil {
...
// receiveNext reads from the given source and returns the next row or metadata
// to be processed by the hashJoiner. rows with NULLs in their equality columns
// are only returned if the joinType specifies that unmatched rows should be
// returned for the given side. In this case, a rendered row and true is
// returned, notifying the caller that the returned row may be emitted directly.
func (h *hashJoiner) receiveNext(
	source RowSource, side joinSide,
...
func (h *hashJoiner) initStoredRows() error {
	...
	if err == nil {
		err = h.maybeMakeMemErr(err, "initializing mem rows")
...
// probability of h.testingKnobMemFailProbability if the given err is nil and
// h.runningState matches the h.testingKnobMemFailPoint.
func (h *hashJoiner) maybeMakeMemErr(err error, action string) error {
	if err == nil &&
		h.testingKnobMemFailPoint == h.runningState &&
