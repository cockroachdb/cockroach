...
type bucket struct {
	// rows hold indices of rows into the hashJoiner's rows container.
	rows []int
	seen []bool
}
...
func (h *hashJoiner) Run(ctx context.Context, wg *sync.WaitGroup) {
	...
	if err != nil {
		...
		// the consumer.
		log.Infof(ctx, "MEH build phase error %s", err)
	...
	if moreRows || err != nil {
		...
		// point.
		log.Infof(ctx, "MEH probe phase error %s", err)
...
func (h *hashJoiner) buildPhase(ctx context.Context) (bool, error) {
	...
	for {
		...
		if err := h.bucketsAcc.Grow(ctx, sizeOfRowIdx); err != nil {
			return false, err
		}
