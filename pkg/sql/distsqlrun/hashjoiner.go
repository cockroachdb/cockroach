...
func (h *hashJoiner) Run(ctx context.Context, wg *sync.WaitGroup) {
	...
	// error.
	if pgErr, ok := pgerror.GetPGCause(err); earlyExit || (err != nil && !(ok && pgErr.Code == pgerror.CodeOutOfMemoryError)) {
		if err != nil {
	...
	// Build hashRowContainer. If we didn't get an error, the build phase will
