...
func (h *hashJoiner) Run(ctx context.Context, wg *sync.WaitGroup) {
	...
	if err != nil {
		...
		// the consumer.
		DrainAndForwardMetadataFinal(ctx, h.out.output, err, h.leftSource, h.rightSource)
		return
	}
	if !moreRows {
		return
	...
	if moreRows || err != nil {
		...
		// point.
		DrainAndForwardMetadataFinal(ctx, h.out.output, err, h.leftSource)
	}
}
...
func (h *hashJoiner) buildPhase(ctx context.Context) (bool, error) {
	var scratch []byte
	for {
		rrow, meta := h.rightSource.NextRow()
		if !meta.Empty() {
			if meta.Err != nil {
...
func (h *hashJoiner) probePhase(ctx context.Context) (bool, error) {
	...
	for {
		lrow, meta := h.leftSource.NextRow()
		if !meta.Empty() {
			if meta.Err != nil {
				return true, meta.Err
			}
