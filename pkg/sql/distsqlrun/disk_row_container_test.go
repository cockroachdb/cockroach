...
func distinctRows(
	...
	for _, orderInfo := range ordering {
		...
		isDistinct, err := l[col].Distinct(&lTypes[col], d, e, &r[orderInfo.ColIdx])
		if err != nil {
			return false, err
		}
		if isDistinct {
		...
		for i := 0; i < 100; i++ {
			...
			for _, ordering := range orderings {
				...
				func() {
					...
					for i := range row {
						if isDistinct, err := readRow[i].Distinct(
							&types[i], &d.datumAlloc, &evalCtx, &row[i]); err != nil {
							t.Fatal(err)
						} else if isDistinct {
		...
		for _, ordering := range orderings {
			...
			func() {
				...
				for i.Rewind(); ; i.Next() {
					...
					// Check sorted order.
					if isDistinct, err := distinctRows(
...
func TestDiskRowContainerFinalIterator(t *testing.T) {
	...
	checkEqual := func(row sqlbase.EncDatumRow, otherRow sqlbase.EncDatumRow) error {
		for j, c := range row {
			if isDistinct, err := c.Distinct(&intType, alloc, &evalCtx, &otherRow[j]); err != nil {
				return err
			} else if isDistinct {
				return fmt.Errorf(
