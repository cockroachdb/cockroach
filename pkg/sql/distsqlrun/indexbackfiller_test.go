...
`); err != nil {
	...
	tableDesc.Mutations[0].ResumeSpan = []roachpb.Span{
	...
	expected := []roachpb.Span{
		...
		{Key: roachpb.Key("a2"), EndKey: roachpb.Key("a3")},
		{Key: roachpb.Key("a3"), EndKey: roachpb.Key("b")},
		// Work completed in the middle of a span.
		...
		// Work performed in the left of a span.
		{Key: roachpb.Key("i1"), EndKey: roachpb.Key("i2")},
		{Key: roachpb.Key("i2"), EndKey: roachpb.Key("j")},
		// Work completed in the left of a span.
	...
	got := tableDesc.Mutations[0].ResumeSpan
