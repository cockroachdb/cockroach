...
func makeReaderSpans(
	t *testing.T, desc *sqlbase.TableDescriptor, constraint string,
) ([]TableReaderSpan, error) {
	spans, err := MakeTestSpans(desc, &desc.PrimaryIndex, constraint)
	if err != nil {
		return nil, err
	}
	log.Warningf(context.TODO(), "%#v\n", spans)
	readerSpans := make([]TableReaderSpan, len(spans))
	for i, span := range spans {
		readerSpans[i].Span = span
	}
	return readerSpans, nil
}
...
func TestInterleaveReaderJoiner(t *testing.T) {
	...
	pdSpans, err := makeReaderSpans(t, pd, "id <= 18")
	if err != nil {
		t.Fatal(err)
	}
	cd2Spans, err := makeReaderSpans(t, cd2, "pid >= 12")
	if err != nil {
		t.Fatal(err)
	}
	...
	testCases := []struct {
		...
		// Check that even though InterleaveReaderJoiner scans all
