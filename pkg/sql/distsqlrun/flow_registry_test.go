...
func TestFlowCancelPartiallyBlocked(t *testing.T) {
	...
	// occurred.
	left := &RowChannel{}
	left.initWithBufSizeAndNumSenders(nil, 1, 1)
	right := &RowChannel{}
	right.initWithBufSizeAndNumSenders(nil, 1, 1)
	...
	// RegisterFlow with an immediate timeout.
	flow := &Flow{}
	flow.inboundStreams = inboundStreams
	flow.id = distsqlpb.FlowID{UUID: uuid.FastMakeV4()}
	flow.flowRegistry = fr
	if err := fr.RegisterFlow(
