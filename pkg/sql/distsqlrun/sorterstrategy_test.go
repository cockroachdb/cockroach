...
func sortRocksDB(b *testing.B, input RowSource, out procOutputHelper) error {
	return sortRocksDBHelper(b, input, out, false)
}

func sortRocksDBWithBatching(b *testing.B, input RowSource, out procOutputHelper) error {
	return sortRocksDBHelper(b, input, out, true)
}

func sortRocksDBHelper(b *testing.B, input RowSource, out procOutputHelper, batching bool) error {
	path := "./rocksBench"
...
func external(b *testing.B, input RowSource, out procOutputHelper, sort bool) error {
	...
	for {
		...
		keyToWrite := []byte(hex.EncodeToString(key) + "\n")
		b.StartTimer()
		if _, err := f.Write(keyToWrite); err != nil {
			return err
		}
		b.StopTimer()
	}
...
func sortBadger(b *testing.B, input RowSource, out procOutputHelper) error {
	path := "badger_bench"
	opts := badger.DefaultOptions
	opts.Dir = path
	bstore, err := badger.NewKV(&badger.DefaultOptions)
	if err != nil {
		return err
	}
	defer func() {
	...
	// on-disk map.
	i := bstore.NewIterator(badger.IteratorOptions{
		PrefetchSize: 0,
		FetchValues:  true,
		Reverse:      false,
	})
