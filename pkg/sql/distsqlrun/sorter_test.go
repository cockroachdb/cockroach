...
func BenchmarkSortAll(b *testing.B) {
	...
	for _, inputSize := range []int{0, 1, 2, 4, 8, 16, 32, 64, 128} {
		...
		repeatableSink := NewRepeatableSink(types, input)
		s, err := newSorter(&flowCtx, &spec, repeatableSink, &post, repeatableSink)
		if err != nil {
			b.Fatal(err)
		}
...
// BenchmarkSortLimit times how long it takes to sort a 128 row input with
// varying limits.
func BenchmarkSortLimit(b *testing.B) {
	...
	input := make(sqlbase.EncDatumRows, 128)
	for i := range input {
		input[i] = sqlbase.EncDatumRow{
			sqlbase.DatumToEncDatum(columnTypeInt, parser.NewDInt(parser.DInt(rng.Int()))),
		}
	...
	for _, limit := range []uint64{0, 1, 2, 4, 8, 16, 32, 64, 128} {
		s, err := newSorter(
			&flowCtx, &spec, repeatableSink, &PostProcessSpec{Limit: limit}, repeatableSink,
		)
		if err != nil {
			b.Fatal(err)
		}
