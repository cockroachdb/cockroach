...
type JoinReaderSpec struct {
	...
	IndexIdx uint32 `protobuf:"varint,2,opt,name=index_idx,json=indexIdx" json:"index_idx"`
	// If `lookup_columns` is empty, joinReader will lookup values from the
	// `index_idx` provided.
	// If populated, will perform a `lookup_join` with these columns as the
	// equality columns. These columns must match those in the `index_idx`.
