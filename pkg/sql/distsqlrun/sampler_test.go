...
func runSampler(t *testing.T, numRows, numSamples int, isAutoStats bool, idleTime float64) []int {
	...
	stats.AutomaticStatisticsIdleTime.Override(&st.SV, idleTime)
	...
	for r := 0; r < maxRuns; r += minRuns {
		for i := 0; i < minRuns; i++ {
			for _, v := range runSampler(
				t, numRows, numSamples, false /* isAutoStats */, 0, /* idleTime */
			) {
				freq[v]++
			}
		}
...
func TestSamplerThrottling(t *testing.T) {
	...
	for r := 0; r < maxRuns; r += minRuns {
		for i := 0; i < minRuns; i++ {
			...
			runSampler(t, numRows, numSamples, true /* isAutoStats */, idleTime)
			durationThrottled += timeutil.Now().Sub(startTime)

			// Run the sampler without throttling.
			startTime = timeutil.Now()
			runSampler(t, numRows, numSamples, true /* isAutoStats */, 0 /* idleTime */)
