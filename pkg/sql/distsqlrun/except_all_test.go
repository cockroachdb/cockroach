...
type testInputs struct {
	...
	inputEvensOrdered sqlbase.EncDatumRows
}
...
func initTestData() testInputs {
	...
	return testInputs{
		...
		inputEvensOrdered: inputEvensOrdered,
	}
}
...
func runProcessors(tc testCase) (sqlbase.EncDatumRows, error) {
	...
	s, err := newExceptAll(&flowCtx, &tc.spec, inL, inR, &PostProcessSpec{}, out)
	if err != nil {
		return nil, err
	}
...
func TestExceptAll(t *testing.T) {
	...
	td := initTestData()
	v := td.v
	setSpecFirstColumnOrderedAscending := ExceptAllSpec{
		Ordering: Ordering{
