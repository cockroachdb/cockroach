...
type distinct struct {
	...
	orderedColsScratch map[int]bool
}
...
func newDistinct(
	...
	d := &distinct{
		...
		orderedColsScratch: make(map[int]bool),
	}
...
func (d *distinct) encode(appendTo []byte, row sqlbase.EncDatumRow) ([]byte, error) {
	...
	for k := range d.orderedColsScratch {
		d.orderedColsScratch[k] = false
	}
	for _, ordered := range d.ordering {
		d.orderedColsScratch[ordered.ColIdx] = true
	}
