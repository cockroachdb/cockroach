...
// StreamDecoder converts a sequence of ProducerMessage to RowOrMetadata's.
//
// Sample usage:
//   sd := StreamDecoder{}
...
type StreamDecoder struct {
	...
	// streamClosed is set by StreamClosed() to mean that no more records are
	// coming.
	streamClosed bool
...
// IsDone returns true if all the rows were returned and StreamClosed() has been
// called.
func (sd *StreamDecoder) IsDone() bool {
	if len(sd.data) > 0 || len(sd.metadata) > 0 || !sd.streamClosed {
		return false
	}
	return true
}
