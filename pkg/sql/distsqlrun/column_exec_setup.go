...
func planExpressionOperators(
	...
	case *tree.ComparisonExpr:
		...
		cmpOp := t.Operator
		leftOp, leftIdx, columnTypes, err := planExpressionOperators(t.TypedLeft(), columnTypes, input)
		if err != nil {
			return nil, resultIdx, columnTypes, err
		}
		ct := columnTypes[leftIdx]
		if constArg, ok := t.Right.(tree.Datum); ok {
			op, err := exec.GetSelectionConstOperator(ct, cmpOp, leftOp, leftIdx, constArg)
	...
	case *tree.BinaryExpr:
		...
		if constArg, ok := t.Right.(tree.Datum); ok {
			...
			op, err := exec.GetProjectionConstOperator(ct, binOp, leftOp, leftIdx, constArg, resultIdx)
