...
// vectorizedFlowRunner contains all the logic needed to run the vectorized
// infrastructure asynchronously as well as to perform some sanity checks.
type vectorizedFlowRunner interface {
	addStreamEndpoint(distsqlpb.StreamID, *colrpc.Inbox, *sync.WaitGroup)
	checkInboundStreamID(distsqlpb.StreamID) error
	runRouter(*exec.HashRouter)
	runOutbox(*colrpc.Outbox, *distsqlpb.StreamEndpointSpec)
	addMaterializer(*materializer)
}

// vectorizedFlowSetuper performs all the setup of vectorized flows. Depending
// on embedded vectorizedFlowRunner, it can either do the actual setup in order
// to run the flow or do the setup needed to check that the flow is supported
// through the vectorized engine.
type vectorizedFlowSetuper struct {
	vectorizedFlowRunner
...
func newVectorizedFlowSetuper(
	runner vectorizedFlowRunner,
	recordingStats bool,
	waitGroup *sync.WaitGroup,
	syncFlowConsumer RowReceiver,
) *vectorizedFlowSetuper {
	setuper := &vectorizedFlowSetuper{
		vectorizedFlowRunner:           runner,
...
func (s *vectorizedFlowSetuper) setupRemoteOutputStream(
	op exec.Operator,
	outputTyps []types.T,
...
func (s *vectorizedFlowSetuper) setupRouter(
	input exec.Operator,
	outputTyps []types.T,
...
// calling DrainMeta.
func (s *vectorizedFlowSetuper) setupInput(
...
func (s *vectorizedFlowSetuper) setupOutput(
	ctx context.Context,
	flowCtx *FlowCtx,
...
func (s *vectorizedFlowSetuper) setupFlow(
	ctx context.Context,
	flowCtx *FlowCtx,
...
// actualVectorizedFlowRunner is a vectorizedFlowRunner that sets up all the
// vectorized infrastructure to be actually run.
type actualVectorizedFlowRunner struct {
	f *Flow
}

var _ vectorizedFlowRunner = &actualVectorizedFlowRunner{}

func (r *actualVectorizedFlowRunner) addStreamEndpoint(
...
func (r *actualVectorizedFlowRunner) checkInboundStreamID(sid distsqlpb.StreamID) error {
	return r.f.checkInboundStreamID(sid)
}

func (r *actualVectorizedFlowRunner) runRouter(router *exec.HashRouter) {
...
func (r *actualVectorizedFlowRunner) runOutbox(
	outbox *colrpc.Outbox, stream *distsqlpb.StreamEndpointSpec,
) {
	r.f.startables = append(
		r.f.startables,
		startableFn(func(ctx context.Context, wg *sync.WaitGroup, cancelFn context.CancelFunc) {
			...
			go func() {
				outbox.Run(ctx, r.f.nodeDialer, stream.TargetNodeID, r.f.id, stream.StreamID, cancelFn)
				if wg != nil {
					wg.Done()
				}
			}()
...
func (r *actualVectorizedFlowRunner) addMaterializer(m *materializer) {
...
func (f *Flow) setupVectorizedFlow(ctx context.Context, acc *mon.BoundAccount) error {
	...
	runner := &actualVectorizedFlowRunner{f: f}
	setuper := newVectorizedFlowSetuper(runner, recordingStats, &f.waitGroup, f.syncFlowConsumer)
	return setuper.setupFlow(ctx, &f.FlowCtx, f.spec.Processors, acc)
}

// noopVectorizedFlowRunner is a vectorizedFlowRunner that only performs
// sanity checks.
type noopVectorizedFlowRunner struct {
	inboundStreams map[distsqlpb.StreamID]struct{}
}

var _ vectorizedFlowRunner = &noopVectorizedFlowRunner{}

func newNoopVectorizedFlowRunner() *noopVectorizedFlowRunner {
	return &noopVectorizedFlowRunner{
		inboundStreams: make(map[distsqlpb.StreamID]struct{}),
	}
}

func (r *noopVectorizedFlowRunner) addStreamEndpoint(
	streamID distsqlpb.StreamID, _ *colrpc.Inbox, _ *sync.WaitGroup,
) {
	r.inboundStreams[streamID] = struct{}{}
}

func (r *noopVectorizedFlowRunner) checkInboundStreamID(sid distsqlpb.StreamID) error {
...
func (r *noopVectorizedFlowRunner) runRouter(_ *exec.HashRouter) {
}

func (r *noopVectorizedFlowRunner) runOutbox(_ *colrpc.Outbox, _ *distsqlpb.StreamEndpointSpec) {
}

func (r *noopVectorizedFlowRunner) addMaterializer(m *materializer) {
}
...
func SupportsVectorized(
	...
	setuper := newVectorizedFlowSetuper(
		newNoopVectorizedFlowRunner(),
		false,        /* recordingStats */
		nil,          /* waitGroup */
		&RowBuffer{}, /* syncFlowConsumer */
	)

	return setuper.setupFlow(ctx, flowCtx, processorSpecs, acc)
}
