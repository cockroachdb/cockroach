...
// inputDAG is a helper struct that stores an operator DAG as well as the
// metadataSources that need to be drained.
type inputDAG struct {
...
type vectorizedRemoteComponentsCreator struct{}

func (vectorizedRemoteComponentsCreator) newOutbox(
...
func (vectorizedRemoteComponentsCreator) newInbox(typs []types.T) (*colrpc.Inbox, error) {
...
type vectorizedFlowCreator struct {
	...
	streamIDToInputOp              map[distsqlpb.StreamID]inputDAG
	recordingStats                 bool
...
func newVectorizedFlowCreator(
	...
	return &vectorizedFlowCreator{
		...
		streamIDToInputOp:              make(map[distsqlpb.StreamID]inputDAG),
...
// local outputs to pass that responsibility along. In any case, it will always
// be fully consumed.
func (s *vectorizedFlowCreator) setupRouter(
	...
	// This check includes the PASS_THROUGH type. Callers should handle that.
	...
	for i, op := range outputs {
		...
		case distsqlpb.StreamEndpointSpec_LOCAL:
			...
			s.streamIDToInputOp[stream.StreamID] = inputDAG{rootOperator: op, metadataSources: metadataSourcesQueue}
		}
		// Either the metadataSourcesQueue will be drained by an outbox or we
		// created an inputDAG to pass along these metadataSources. We don't need to
...
func (s *vectorizedFlowCreator) setupOutput(
	...
	case distsqlpb.StreamEndpointSpec_LOCAL:
		s.streamIDToInputOp[outputStream.StreamID] = inputDAG{rootOperator: op, metadataSources: metadataSourcesQueue}
	case distsqlpb.StreamEndpointSpec_REMOTE:
...
func (f *Flow) setupVectorizedFlow(ctx context.Context, acc *mon.BoundAccount) error {
	...
	creator := newVectorizedFlowCreator(
		helper, vectorizedRemoteComponentsCreator{}, recordingStats, &f.waitGroup, f.syncFlowConsumer, f.nodeDialer, f.id,
	)
...
func SupportsVectorized(
	...
	creator := newVectorizedFlowCreator(
		...
		vectorizedRemoteComponentsCreator{},
