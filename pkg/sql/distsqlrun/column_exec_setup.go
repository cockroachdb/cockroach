...
func planProjectionOperators(
	...
	case *tree.CaseExpr:
		...
		for i, when := range t.Whens {
			...
			caseOps[i], resultIdx, ct, thenUsed, err = planProjectionOperators(ctx, when.Val.(tree.TypedExpr), ct, caseOps[i])
		...
		elseOp, resultIdx, ct, elseUsed, err = planProjectionOperators(ctx, elseExpr.(tree.TypedExpr), ct[:len(ct)-1],
			buffer)
		memUsed += elseUsed
