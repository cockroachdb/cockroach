...
func newColOperator(
	...
	case core.Aggregator != nil:
		...
		if !orderedCols.SubsetOf(groupCols) {
			return nil, nil, pgerror.AssertionFailedf("ordered cols must be a subset of grouping cols")
		}
	...
	case core.Distinct != nil:
		...
		if !orderedCols.SubsetOf(distinctCols) {
			return nil, nil, pgerror.AssertionFailedf("ordered cols must be a subset of distinct cols")
		}
	...
	if columnTypes == nil {
		return nil, nil, pgerror.AssertionFailedf("output columnTypes unset after planning %T", op)
	}
	...
	if !post.Filter.Empty() {
		...
		if err != nil {
			return nil, nil, pgerror.Wrapf(err, pgerror.CodeDataExceptionError,
	...
	if post.Projection {
		...
		for i, j := range post.OutputColumns {
			columnTypes[i] = columnTypes[j]
		}
		columnTypes = columnTypes[:len(post.OutputColumns)]
		...
		for _, expr := range post.RenderExprs {
			...
			if outputIdx < 0 {
				return nil, nil, pgerror.AssertionFailedf("missing outputIdx")
			}
...
// outputs their stats to the trace contained in the ctx's span, returning these
// spans keyed by processor ID.
func finishVectorizedStatsCollectors(
	ctx context.Context,
	deterministicStats bool,
	vectorizedStatsCollectors []*exec.VectorizedStatsCollector,
	procIDs []int32,
) map[int32]opentracing.Span {
...
func (f *Flow) setupVectorizedRouter(
	...
	for i, op := range outputs {
		...
		case distsqlpb.StreamEndpointSpec_SYNC_RESPONSE:
			return pgerror.AssertionFailedf("unexpected sync response output when setting up router")
	...
	if !consumedMetadataSources {
		return pgerror.AssertionFailedf("router had no remote outputs so metadata could not be consumed")
	}
	return nil
}
	...
	for len(queue) > 0 {
		...
		if output.Type != distsqlpb.OutputRouterSpec_PASS_THROUGH {
			...
			case distsqlpb.StreamEndpointSpec_REMOTE:
				...
				// TODO(asubiotto): Maybe we need to do a similar thing with the
				//  materializer below?
				if recordingStats {
					...
					vscs := append([]*exec.VectorizedStatsCollector{}, vectorizedStatsCollectorsQueue...)
					...
					metadataSourcesQueue = append(
						...
						distsqlpb.CallbackMetadataSource{
							DrainMetaCb: func(ctx context.Context) []distsqlpb.ProducerMetadata {
								spans := finishVectorizedStatsCollectors(ctx, f.FlowCtx.testingKnobs.DeterministicStats, vscs, procIDs)
								meta := make([]distsqlpb.ProducerMetadata, 0, len(spans))
								for _, span := range spans {
									meta = append(meta, distsqlpb.ProducerMetadata{TraceData: tracing.GetRecording(span)})
								}
								return meta
							},
						},
