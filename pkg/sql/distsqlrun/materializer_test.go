...
func TestColumnarizeMaterialize(t *testing.T) {
	...
	m, err := newMaterializer(flowCtx, 1, c, types, []int{0, 1}, &distsqlpb.PostProcessSpec{}, nil, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestMaterializeTypes(t *testing.T) {
	...
	m, err := newMaterializer(flowCtx, 1, c, types, outputToInputColIdx, &distsqlpb.PostProcessSpec{}, nil, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
