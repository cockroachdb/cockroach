...
func (rb *routerBase) updateStreamState(streamIdx int, newState ConsumerStatus) {
	if newState != rb.streamStatus[streamIdx] {
...
func (rb *routerBase) fwdMetadata(meta ProducerMetadata) {
	...
	for i := range rb.streams {
		if rb.streamStatus[i] != ConsumerClosed {
			newStatus := rb.streams[i].PushRow(nil /*row*/, meta)
...
func (mr *mirrorRouter) PushRow(row sqlbase.EncDatumRow, meta ProducerMetadata) ConsumerStatus {
	if !meta.Empty() {
		mr.fwdMetadata(meta)
	...
	for i := range mr.streams {
		if mr.streamStatus[i] == NeedMoreRows {
			newStatus := mr.streams[i].PushRow(row, ProducerMetadata{})
...
// PushRow is part of the RowReceiver interface.
func (hr *hashRouter) PushRow(row sqlbase.EncDatumRow, meta ProducerMetadata) ConsumerStatus {
	if !meta.Empty() {
		hr.fwdMetadata(meta)
	...
	// accelerated).
	streamIdx := int(crc32.Update(0, crc32Table, hr.buffer) % uint32(len(hr.streams)))

	if hr.streamStatus[streamIdx] == NeedMoreRows {
		newStatus := hr.streams[streamIdx].PushRow(row, ProducerMetadata{})
