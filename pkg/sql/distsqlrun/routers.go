...
// makeRouter creates a router and returns it as a RowReceiver. If the router
// needs to be started, also returns a startable. The router's init must be
// called before the router can be started.
//
// Passthrough routers are not supported; they should be handled separately.
...
type routerOutput struct {
	...
	mu     struct {
		...
		// rowBuf allows. The rowContainer always contains rows "older" than those
		// in rowBuf.
...
func (rb *routerBase) start(ctx context.Context, wg *sync.WaitGroup) {
	...
	for i := range rb.outputs {
		go func(rb *routerBase, ro *routerOutput, wg *sync.WaitGroup, x int) {
