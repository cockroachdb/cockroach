...
// errMeta informs the consumer that we're going to drain it, and transitions
// the processor to the draining state. If given an error, it will produce error
// metadata as well.
func (ag *aggregator) errMeta(err error) (aggregatorState, sqlbase.EncDatumRow, *ProducerMetadata) {
	var meta *ProducerMetadata
