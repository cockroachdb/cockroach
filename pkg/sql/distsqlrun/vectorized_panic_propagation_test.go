...
func TestVectorizedInternalPanic(t *testing.T) {
	...
	vee := newTestVectorizedErrorEmitter(col, execerror.VectorizedInternalPanic)
...
func TestNonVectorizedPanicPropagation(t *testing.T) {
	...
	nvee := newTestVectorizedErrorEmitter(col, nil /* panicFn */)
	mat, err := newMaterializer(
		&flowCtx,
		1, /* processorID */
		nvee,
...
func newTestVectorizedErrorEmitter(input exec.Operator, panicFn func(interface{})) exec.Operator {
