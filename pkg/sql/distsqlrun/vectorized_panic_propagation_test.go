...
func TestNonVectorizedPanicDoesntHangServer(t *testing.T) {
	...
	quitMockStartable := make(chan struct{})
	flow := &Flow{
		processors: []Processor{mat},
		// This test specifically verifies that a flow doesn't get stuck in Wait for
		// asynchronous components that haven't been signalled to exit. To simulate
		// this we just create a mock startable.
		startables: []startable{
			startableFn(func(_ context.Context, wg *sync.WaitGroup, _ context.CancelFunc) {
				wg.Add(1)
				go func() {
					<-quitMockStartable
					wg.Done()
	...
	// Make sure we don't leak the asynchronous goroutine in our test.
	close(quitMockStartable)
	flow.waitGroup.Wait()
}
