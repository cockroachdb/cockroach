...
// Additionally, we all Outboxes have a single metadata source. In ConsumerDone
	...
	for run := 0; run < 10; run++ {
		for _, shutdownOperation := range shutdownScenarios {
			t.Run(fmt.Sprintf("shutdownScenario=%s", shutdownOperation.string), func(t *testing.T) {
				...
				// It is important for this test to work that we start all Outboxes and
				// Inboxes after materializer has been started so that ctx is the
				// updated context.
				...
				for _, inbox := range inboxes {
					...
					runOutboxInbox(ctx, exec.NewRepeatableBatchSource(batch), inbox, streamID, func() {})
					streamID++
				}
