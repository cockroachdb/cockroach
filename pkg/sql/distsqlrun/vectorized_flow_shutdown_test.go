				...
				for i, inbox := range inboxes {
					if i < len(inboxes)-1 {
						if i < numHashRouterOutputs {
							runOutboxInbox(ctxRemote, cancelRemote, hashRouterOutputs[i], inbox, streamID)
						} else {
							batch := coldata.NewMemBatch(typs)
							batch.SetLength(coldata.BatchSize)
							runOutboxInbox(ctxRemote, cancelRemote, exec.NewRepeatableBatchSource(batch), inbox, streamID)
						}
					} else {
						ctxAnotherRemote, cancelAnotherRemote := context.WithCancel(context.Background())
						runOutboxInbox(ctxAnotherRemote, cancelAnotherRemote, synchronizer, inbox, streamID)
					}
					streamID++
				}
				wg.Add(1)
