		...
		for _, shutdownOperation := range shutdownScenarios {
			t.Run(fmt.Sprintf("shutdownScenario=%s", shutdownOperation.string), func(t *testing.T) {
				...
				runOutboxInbox := func(
					...
					outboxRegistry *colrpc.OutboxRegistry,
				) {
				...
				remoteOutboxRegistry := &colrpc.OutboxRegistry{}
				for i := 0; i < numInboxes; i++ {
					...
					if i < numHashRouterOutputs {
						...
						runOutboxInbox(ctxRemote, cancelRemote, hashRouterOutputs[i], inboxes[i], streamID, outboxMetadataSources, remoteOutboxRegistry)
					} else {
						batch := coldata.NewMemBatch(typs)
						batch.SetLength(coldata.BatchSize)
						runOutboxInbox(ctxRemote, cancelRemote, exec.NewRepeatableBatchSource(batch), inboxes[i], streamID, outboxMetadataSources, remoteOutboxRegistry)
					}
					streamID++
				}
				...
				if addAnotherRemote {
					...
					anotherRemoteOutboxRegistry := &colrpc.OutboxRegistry{}
					runOutboxInbox(ctxAnotherRemote, cancelAnotherRemote, synchronizer, inbox, streamID, materializerMetadataSources, anotherRemoteOutboxRegistry)
					streamID++
