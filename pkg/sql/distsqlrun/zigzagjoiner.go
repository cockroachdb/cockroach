...
func (z *zigzagJoiner) Run(wg *sync.WaitGroup) {
	if z.out.output == nil {
		panic("zigzagJoiner output not initialized for emitting rows")
	}
	Run(z.flowCtx.Ctx, z, z.out.output)
	if wg != nil {
		wg.Done()
	}
}
...
func (z *zigzagJoiner) produceKeyFromBaseRow() (roachpb.Key, error) {
	...
	// Construct correct row by concatenating right locked datums with
...
func (z *zigzagJoiner) Next() (sqlbase.EncDatumRow, *ProducerMetadata) {
	...
	if z.maybeStart("zigzag joiner", "ZigzagJoiner") {
		z.evalCtx = z.flowCtx.NewEvalCtx()
		z.cancelChecker = sqlbase.NewCancelChecker(z.ctx)
		log.VEventf(z.ctx, 2, "starting zigzag joiner run")
		...
		fetchedRow, _, _, err := z.infos[0].fetcher.NextRow(ctx)
		z.baseRow = make(sqlbase.EncDatumRow, len(fetchedRow))
		copy(z.baseRow, fetchedRow)
		if err != nil {
