...
func newZigzagJoiner(
	...
	for i := 0; i < z.numTables; i++ {
		...
		z.side = i
		if err := z.setupInfo(spec, colOffset); err != nil {
			return nil, err
		}
		colOffset += len(z.curInfo().table.Columns)
	}
	z.side = 0
	return z, nil
}
...
// of the join.
func (z *zigzagJoiner) setupInfo(spec *ZigzagJoinerSpec, colOffset int) error {
	...
	maxCol := colOffset + len(z.curInfo().table.Columns)
	for i, ok := outCols.Next(colOffset); ok && i < maxCol; i, ok = outCols.Next(i + 1) {
		neededCols.Add(i - colOffset)
	}

	// Add the fixed columns.
	indexCols := append(z.curInfo().index.ColumnIDs, z.curInfo().index.ExtraColumnIDs...)
	for i := 0; i < len(z.curInfo().fixedValues); i++ {
		neededCols.Add(int(indexCols[i]) - 1)
	}

	// Add the equality columns.
	for _, col := range z.curInfo().eqColumnIDs {
...
func (z *zigzagJoiner) fetchRowFromSide(
	...
	// Assume fetched row has null in its equality columns until proved otherwise.
	hasNull := true
	var fetchedRow sqlbase.EncDatumRow
	var err error
...
func (z *zigzagJoiner) nextRow(
	...
	for {
		...
		// we break and return nil to indicate that to the caller.
		if fetchedRow == nil {
			break
		}
...
func (z *zigzagJoiner) Next() (sqlbase.EncDatumRow, *ProducerMetadata) {
	...
	for {
		...
		outRow, ok, err := z.out.ProcessRow(z.ctx, row)
		if err != nil {
			return nil, z.producerMeta(err)
		}
		...
		if !ok {
			// If not ok, then a drain was requested.
			continue
		}
		return outRow, nil
	}
