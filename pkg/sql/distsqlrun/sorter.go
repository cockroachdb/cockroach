...
func (s *sortAllProcessor) fill() (bool, error) {
	ctx := s.evalCtx.Ctx()
	...
	if err != nil {
		...
		valid, _, err = s.fillWithContainer(ctx, s.diskContainer)
		if !valid {
			return false, err
		}
	}
...
//
// Metadata is buffered in s.meta.
//
...
//
// The bool retval is true if the container is valid, false otherwise (if an
// error occurred or if the input returned an error metadata record). The caller
// is expected to inspect the error (if any) and if it's not recoverable, drain.
func (s *sortAllProcessor) fillWithContainer(
	ctx context.Context, r sortableRowContainer,
) (bool, sqlbase.EncDatumRow, error) {
	for {
...
//
// Metadata is buffered in s.meta. Returns true if a valid chunk of rows has
// been read and sorted, false otherwise (if the input had no more rows or if a
// metadata record was encountered). The caller is expected to drain when this
// returns false.
