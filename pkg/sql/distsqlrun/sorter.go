...
// silently if stats are not being collected. Note that disk stats are not
// output. The sortAll processor therefore overrides finishTrace with its own
// outputStatsToTrace implementation. When all the sorters fall back to disk,
// this difference will go away.
