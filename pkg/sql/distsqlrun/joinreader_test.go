...
CREATE TABLE test.t (a INT, s STRING, INDEX (a, s))`); err != nil {
	...
	if count != numRows {
		t.Fatalf("expected %d rows in result", numRows)
	}
	if !jr.lookedUpRows.(*rowcontainer.DiskBackedIndexedRowContainer).Spilled() {
		t.Fatal("expected rowcontainer to have spilled to disk")
	}
}
