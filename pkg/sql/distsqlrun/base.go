...
// ConsumerStatus is the type returned by RowReceiver.PushRow(), informing a
...
type RowReceiver interface {
	// PushRow sends a record to the consumer of this RowReceiver. Exactly one of the
	// arguments must be non-nil. May block.
	//
	...
	// implementations have to ignore further calls to PushRow() (such calls are
	...
	//
	// Implementations of PushRow() must be thread-safe.
	PushRow(row sqlbase.EncDatumRow, meta ProducerMetadata) ConsumerStatus
	...
	//
	// ProducerDone() cannot be called concurrently with PushRow(), and after it
...
type RowSource interface {
	...
	// NextRow returns the next record that a producer has pushed into this
	...
	// producers). Therefor, consumers need to be aware that some rows might have
	...
	NextRow() (sqlbase.EncDatumRow, ProducerMetadata)
	...
	// NextRow indicates that there are no more rows, ConsumerDone() and/or
	// ConsumerClosed() must be called; it is a no-op to call these methods after
	// all the rows were consumed (i.e. after NextRow() returned an empty row).
	ConsumerDone()
	...
	// before NextRow indicates that there are no more rows, ConsumerDone() and/or
	// ConsumerClosed() must be called; it is a no-op to call these methods after
	// all the rows were consumed (i.e. after NextRow() returned an empty row).
	ConsumerClosed()
}
...
//
// It is OK to call DrainAndForwardMetadata() multiple times concurrently,
// passing the same RowReceiver (as RowReceiver.PushRow() is guaranteed to be
// thread safe).
//
...
func DrainAndForwardMetadata(ctx context.Context, src RowSource, dst RowReceiver) {
	...
	for {
		...
		case NeedMoreRows:
			log.Fatal(ctx, "NeedMoreRows while draining")
		case DrainRequested:
		case ConsumerClosed:
			src.ConsumerClosed()
			return
...
// DrainAndForwardMetadataFinal is a version of DrainAndForwardMetadata that
// drains multiple sources. These sources are assumed to be the only producers
// left for dst, so dst is closed once they're all exhauster (this is different
// from DrainAndForwardMetadata).
//
...
func DrainAndForwardMetadataFinal(
	ctx context.Context, dst RowReceiver, cause error, srcs ...RowSource,
) {
	if cause == nil {
		consumerStatus := dst.PushRow(nil /* row */, ProducerMetadata{Err: cause})
...
// the data is unwrapped and returned as EncDatumRow.
//
...
// Types is analogous to RowSource.Types.
func (rs *NoMetadataRowSource) Types() []sqlbase.ColumnType {
	return rs.src.Types()
}

// NextRow is analogous to RowSource.NextRow. If the producer sends an error, we
...
func (rs *NoMetadataRowSource) NextRow() (sqlbase.EncDatumRow, error) {
	for {
		...
		_ = rs.metadataSink.PushRow(nil /* row */, meta)
	}
}

// ConsumerDone is analogous to RowSource.ConsumerDone().
func (rs *NoMetadataRowSource) ConsumerDone() {
	rs.src.ConsumerDone()
}

// ConsumerClosed is analogous to RowSource.ConsumerClosed().
func (rs *NoMetadataRowSource) ConsumerClosed() {
	rs.src.ConsumerClosed()
}
...
type RowChannel struct {
	...
	// accepting draining metadata or is no longer accepting any rows via PushRow.
...
func (rc *RowChannel) PushRow(row sqlbase.EncDatumRow, meta ProducerMetadata) ConsumerStatus {
	consumerStatus := ConsumerStatus(
...
func (rc *RowChannel) NextRow() (sqlbase.EncDatumRow, ProducerMetadata) {
	d, ok := <-rc.C
	if !ok {
		// No more rows.
...
func (rc *RowChannel) ConsumerClosed() {
	...
	// row. Note that, if the producer is done, then it has also closed the channel.
	<-rc.dataChan
}
...
// PushRow is part of the RowReceiver interface.
func (mrc *MultiplexedRowChannel) PushRow(row sqlbase.EncDatumRow, meta ProducerMetadata) ConsumerStatus {
	return mrc.rowChan.PushRow(row, meta)
}
...
// NextRow is part of the RowSource interface.
func (mrc *MultiplexedRowChannel) NextRow() (row sqlbase.EncDatumRow, meta ProducerMetadata) {
...
type RowBuffer struct {
	mu struct {
		...
		// records represent the data that has been buffered. PushRow appends a row
		// to the back, NextRow removes a row from the front.
...
// PushRow is part of the RowReceiver interface.
func (rb *RowBuffer) PushRow(row sqlbase.EncDatumRow, meta ProducerMetadata) ConsumerStatus {
	if rb.ProducerClosed {
		panic("PushRow called after ProducerDone")
	}
	// We mimic the behavior of RowChannel.
	storeRow := func() {
		row := append(sqlbase.EncDatumRow(nil), row...)
		rb.mu.Lock()
		rb.mu.records = append(rb.mu.records, BufferedRecord{Row: row, Meta: meta})
		rb.mu.Unlock()
	}
...
// NextRow is part of the RowSource interface.
//
// There's no synchronization here with PushRow(). The assumption is that these
// two methods are not called concurrently.
func (rb *RowBuffer) NextRow() (sqlbase.EncDatumRow, ProducerMetadata) {
