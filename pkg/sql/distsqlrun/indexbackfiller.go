...
func WriteResumeSpan(
	...
	ctx, traceSpan := tracing.ChildSpan(ctx, "index backfiller write checkpoint")
		...
		for i, sp := range mutation.ResumeSpans {
			if sp.Key.Compare(origSpan.Key) <= 0 &&
				...
				maybeRemoveCheckpoint := func(idx int) {
					if resume.Key == nil {
						mutation.ResumeSpans =
							append(mutation.ResumeSpans[:idx], mutation.ResumeSpans[idx+1:]...)
					}
				}
				addCheckpoint := func(spans []roachpb.Span) {
					suffix := append(spans, mutation.ResumeSpans[i+1:]...)
					mutation.ResumeSpans = append(mutation.ResumeSpans[:i], suffix...)

				}
				if sp.Key.Equal(origSpan.Key) {
					if sp.EndKey.Equal(origSpan.EndKey) {
						// Update the checkpoint; common case.
						mutation.ResumeSpans[i] = resume
					} else {
						right := sp
