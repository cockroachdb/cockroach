...
func (ib *indexBackfiller) mainLoop() error {
	...
	for row := int64(0); sp.Key != nil; row += chunkSize {
		...
		var err error
		sp.Key, err = ib.runChunk(desc, addedIndexDescs, sp, chunkSize)
		if err != nil {
			return err
		}
	...
	return WriteResumeSpan(ib.flowCtx.Context,
		ib.flowCtx.clientDB,
		ib.spec.Table.ID,
		work,
		resume,
...
func WriteResumeSpan(
	...
	return db.Txn(ctx, func(txn *client.Txn) error {
		...
		// resume. If resume is empty, origSpan is removed from the
		// checkpoint.
		mutation := &tableDesc.Mutations[mutationIdx]
		for i, sp := range mutation.ResumeSpan {
			if sp.Key.Compare(origSpan.Key) <= 0 &&
				...
				maybeRemoveCheckpoint := func(idx int) {
					if resume.Key == nil {
						mutation.ResumeSpan =
							append(mutation.ResumeSpan[:idx], mutation.ResumeSpan[idx+1:]...)
					}
				}
				addCheckpoint := func(spans []roachpb.Span) {
					suffix := append(spans, mutation.ResumeSpan[i+1:]...)
					mutation.ResumeSpan = append(mutation.ResumeSpan[:i], suffix...)

				}
				if sp.Key.Equal(origSpan.Key) {
					if sp.EndKey.Equal(origSpan.EndKey) {
						// Update the checkpoint; common case.
						mutation.ResumeSpan[i] = resume
					} else {
						right := sp
				...
				if log.V(2) {
					log.Infof(txn.Context, "ckpt %+v", mutation.ResumeSpan)
				}
		...
		return errors.Errorf(
			"span %+v not found among %+v", origSpan, mutation.ResumeSpan,
		)
	})
}
