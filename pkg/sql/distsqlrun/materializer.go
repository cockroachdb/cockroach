...
type materializer struct {
	...
	// ctxCancel will cancel the context that is passed to input (which will pass
	// it down further). This allows for the cancellation of tree rooted at this
	// materializer when it is closed.
...
func (m *materializer) InternalClose() bool {
	if m.ProcessorBase.InternalClose() {
		fmt.Printf("materializer is canceling the flow\n")
		m.ctxCancel()
		return true
	}
	return false
