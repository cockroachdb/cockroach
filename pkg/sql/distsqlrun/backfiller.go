...
type backfiller interface {
	...
	// backfiller.
	mutationFilter(sqlbase.DescriptorMutation) bool

	// name returns the name of the kind of entity this backfiller processes.
	name() string
}

// abstractBackfiller is a processor that implements a distributed backfill of
// an entity, like indexes or columns, during a schema change.
type abstractBackfiller struct {
	backfiller
...
func (b *abstractBackfiller) nextRow() (sqlbase.EncDatumRow, error) {
	fetcherRow, err := b.fetcher.NextRow()
...
func (b *abstractBackfiller) Run(ctx context.Context, wg *sync.WaitGroup) {
	if wg != nil {
		defer wg.Done()
	}
...
func (b *abstractBackfiller) mainLoop(ctx context.Context) error {
