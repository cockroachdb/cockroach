	...
	for {
		...
		case NeedMoreRows:
			if outRow == nil && err == nil {
				log.Infof(e.evalCtx.Ctx(), "outrow is nil, but need more rows. skipping")
				continue
			}
		case DrainRequested:
			e.leftSource.ConsumerDone()
			e.rightSource.ConsumerDone()
			log.Infof(e.evalCtx.Ctx(), "drain requested. skipping")
			continue
		}
		return outRow, nil
	}
...
// nextExceptAll pushes all rows in the left stream that are not present in the
		...
		if e.cmp > 0 {
			...
			for e.leftIdx < len(e.leftGroup) {
				...
				encoded, _, err := encodeColumnsOfRow(
					&e.datumAlloc, nil, encDatumRow, e.allCols, e.types, true, /* encodeNull */
				)
				if err != nil {
					return nil, e.producerMeta(err)
				}
...
func (e *algebraicSetOp) maybeRefreshRightMap() {
	if e.rightMap == nil {
		...
		for _, encDatumRow := range e.rightGroup {
			...
			if _, ok := e.rightMap[string(encoded)]; ok {
				e.rightMap[string(encoded)]++
			} else {
				e.rightMap[string(encoded)] = 1
			}
		}
	}
}
