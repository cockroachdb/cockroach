...
type flowRegistry struct {
	...
	// runBeforeDrainSleep is a testing knob executed when a draining flowRegistry
	// has no registered flows but must still wait for a minimum time for any
	// incoming flows to register.
	runBeforeDrainSleep func()
}
...
func (fr *flowRegistry) Drain(flowDrainWait time.Duration, minFlowDrainWait time.Duration) {
	...
	sleep := func(t time.Duration) {
		if fr.runBeforeDrainSleep != nil {
			fr.runBeforeDrainSleep()
		}
		time.Sleep(t)
	}
