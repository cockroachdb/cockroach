...
func TestOutboxInboundStreamIntegration(t *testing.T) {
	...
	RegisterDistSQLServer(rpcSrv, srv)
	...
	connectionInfo := map[StreamID]*inboundStreamInfo{
		streamID: {
			receiver:  consumer,
	...
	require.NoError(
		t,
		srv.flowRegistry.RegisterFlow(ctx, FlowID{}, f, connectionInfo, time.Hour /* timeout */),
	)
