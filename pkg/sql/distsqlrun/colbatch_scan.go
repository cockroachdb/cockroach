...
func initCRowFetcher(
	...
	if scanVisibility == distsqlpb.ScanVisibility_PUBLIC_AND_NOT_PUBLIC {
		if len(immutDesc.ReadableNonPublicColumns) > 0 {
			cols = make([]sqlbase.ColumnDescriptor, 0, len(immutDesc.Columns)+len(immutDesc.ReadableNonPublicColumns))
			cols = append(cols, immutDesc.Columns...)
			cols = append(cols, immutDesc.ReadableNonPublicColumns...)
		}
	}
