...
func initCRowFetcher(
	...
	if scanVisibility == ScanVisibility_PUBLIC_AND_NOT_PUBLIC {
		if len(immutDesc.MutationColumns) > 0 {
			cols = make([]sqlbase.ColumnDescriptor, 0, len(immutDesc.Columns)+len(immutDesc.MutationColumns))
			cols = append(cols, immutDesc.Columns...)
			for _, col := range immutDesc.MutationColumns {
