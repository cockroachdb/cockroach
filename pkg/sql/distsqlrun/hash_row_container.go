...
// Spilled returns whether or not the hashDiskBackedRowContainer spilled to
// disk in its lifetime.
func (h *hashDiskBackedRowContainer) Spilled() bool {
	return h.spilled
}
...
func (h *hashDiskBackedRowContainer) spillToDisk(ctx context.Context) error {
	...
	if h.shouldMark && h.hmrc.marked != nil {
		// We need to preserve marks.
		for i := 0; i < h.hmrc.Len(); i++ {
			copy(h.scratchEncRow, h.hmrc.EncRow(i))
			h.scratchEncRow[len(h.types)] = sqlbase.EncDatum{Datum: tree.MakeDBool(tree.DBool(h.hmrc.marked[i]))}
			if err := hdrc.AddRow(ctx, h.scratchEncRow); err != nil {
				return err
			}
		}
	} else {
		...
		for i.Rewind(); ; i.Next() {
			...
			if err := hdrc.AddRow(ctx, memRow); err != nil {
				return err
			}
		}
	}
