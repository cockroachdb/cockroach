...
func (d *diskRowContainer) NewIterator(ctx context.Context) rowIterator {
	...
	i := diskRowIterator{rowContainer: d, SortedDiskMapIterator: d.diskMap.NewIterator()}
	// Check if the caller is only interested in the top k rows.
	if d.topK > 0 {
...
func (d *diskRowContainer) NewFinalIterator(ctx context.Context) rowIterator {
	i := diskRowFinalIterator{diskRowIterator: d.NewIterator(ctx).(diskRowIterator)}
	if d.topK > 0 {
		return &diskRowTopKIterator{rowIterator: i, k: d.topK}
	}
	return i
