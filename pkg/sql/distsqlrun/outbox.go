	...
	for {
		...
		case drainSignal := <-drainCh:
			if drainSignal.err != nil {
				// Stop work from proceeding in this flow.
				// This also causes FlowStream RPCs that have this node as consumer to return
				// errors. On sync flow outboxes, flowCtxCancel is set to nil in RunSyncFlow,
				// so only run this for non-sync flow outboxes.
				if m.syncFlowStream == nil {
					m.flowCtxCancel()
				}
