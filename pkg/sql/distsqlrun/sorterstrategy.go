...
func newSortAllStrategy(
	s *sorterBase, rows *memRowContainer, useTempStorage bool,
) sortImplementation {
	ss := sortAllStrategy{
		sorterBase:     s,
		rows:           rows,
...
func (s *sortAllStrategy) Next() (sqlbase.EncDatumRow, *ProducerMetadata) {
	...
	if s.maybeStart("sortAllStrategy", "sortAllStrategy") {
		...
		row, err, meta := s.fill(ctx, s.rows)
		if meta != nil {
			s.meta = meta
		} else if err != nil {
			...
			if _, err, meta := s.fill(ctx, s.diskContainer); meta != nil {
...
func (s *sortChunksStrategy) fill() (error, *ProducerMetadata) {
	ctx := s.evalCtx.Ctx()
