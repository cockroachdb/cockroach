...
type sortAllStrategy struct {
	*sorterBase
	...
	// cache it and output the rows its received so far, before emitting the metadata.
	meta *ProducerMetadata
	// doneEmittingRows is set, indicating that we return the meta until we're done.
	doneEmittingRows bool
}
...
func (ss *sortAllStrategy) Next() (sqlbase.EncDatumRow, *ProducerMetadata) {
	log.Errorf(ss.evalCtx.Ctx(), "Next() called: i:%+v, closed:%+v\n", ss.i, ss.closed)
	...
	if ss.maybeStart("sortAllStrategy", "sortAllStrategy") {
		...
		if meta != nil {
			...
			ss.diskContainer = &diskContainer

			// Transfer the rows from memory to disk. This frees up the memory taken up by ss.rows.
			...
			i.Close()
			log.Errorf(ctx, "setting ss.i to nil\n")
			ss.i = nil
			...
			if _, err, meta := ss.fill(ctx, ss.diskContainer); meta != nil {
				log.Errorf(ctx, "setting metadata for use after sending all rows: %+v", meta)
				ss.meta = meta
			} else if err != nil {
				log.Errorf(ctx, "non nil error on second fill: %+v", err)
				return nil, ss.producerMeta(err)
			}
			ss.rows.Close(ctx)
		}
	...
	outRow, status, err := ss.out.ProcessRow(ss.evalCtx.Ctx(), row)
	if err != nil {
		return nil, ss.producerMeta(err)
	}
	...
	switch status {
	case DrainRequested:
		ss.input.ConsumerDone()
		return ss.nextMetadata()
	}
...
func (ss *sortAllStrategy) nextMetadata() (sqlbase.EncDatumRow, *ProducerMetadata) {
	log.Errorf(ss.evalCtx.Ctx(), "nextMetadata called")
	if ss.meta != nil {
		meta := ss.meta
		ss.meta = nil
		return nil, meta
...
) (sqlbase.EncDatumRow, error, *ProducerMetadata) {
	log.Errorf(ss.evalCtx.Ctx(), "fill called\n")
	var meta *ProducerMetadata
	for {
		row, inputMeta := ss.input.Next()
		if inputMeta != nil {
			meta = inputMeta
			break
		}
...
func (s *sortAllStrategy) producerMeta(err error) *ProducerMetadata {
	log.Errorf(s.evalCtx.Ctx(), "producerMeta called with err: %+v", err)
	var meta *ProducerMetadata
	if !s.closed {
		if err != nil {
