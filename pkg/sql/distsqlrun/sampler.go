	...
	for {
		...
		if rowCount%SamplerProgressInterval == 0 {
			...
			if s.isAutoStats {
				...
				// Throttle the processor according to the value of the cluster setting
				// AutomaticStatisticsIdleTime.
				// Wait time is calculated as follows:
				//
				//       idle_time = t_wait / (t_run + t_wait)
				//  ==>  t_wait = t_run * idle_time / (1 - idle_time)
				//
				idleTime := stats.AutomaticStatisticsIdleTime.Get(&s.flowCtx.Settings.SV)
				timer := time.NewTimer(
					time.Duration(float64(elapsed) * idleTime / (1 - idleTime)),
				)
				defer timer.Stop()
				select {
				case <-timer.C:
