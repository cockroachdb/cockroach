...
type windower struct {
	...
	// Partition sizes are tracked so that we can presize the
	// diskBackedIndexedRowContainer to the size of the biggest partition.
	// TODO(yuzefovich): instead of keeping track of partition sizes in
	// accumulateRows, we could just iterate over the rows once after
	// accumulation is done. We wouldn't need this map then. Would this be a
	// better way?
	allRowsPartitionSizes      map[string]int
...
func newWindower(
	...
	w.evalCtx = flowCtx.NewEvalCtx()
	w.inputTypes = input.OutputTypes()
	// TODO(yuzefovich): should this be processorBase.Ctx?
	ctx := w.evalCtx.Ctx()
	memMonitor := NewMonitor(ctx, w.evalCtx.Mon, "windower-mem")
	...
	w.allRowsPartitionSizes = make(map[string]int)
...
func (w *windower) accumulateRows() (windowerState, sqlbase.EncDatumRow, *ProducerMetadata) {
	for {
		...
		// TODO(yuzefovich): we can avoid this rehashing if we get rid off
		// w.allRowsPartitionSizes map (see TODO above w.allRowsPartitionSizes).
		if len(w.partitionBy) > 0 {
			...
			hash := string(w.scratch)
			w.allRowsPartitionSizes[hash] = w.allRowsPartitionSizes[hash] + 1
			if w.allRowsPartitionSizes[hash] > w.maxPartitionSize {
				w.maxPartitionSize = w.allRowsPartitionSizes[hash]
			}
		}
	}
...
// spillAllRowsToDisk spills w.allRowsPartitioned to disk if it's using memory.
// We choose to not to force w.partition to spill as well since it might be
// resorted multiple times with different orderings, so it's better to keep it
// in memory (if it hasn't spilled on its own).
...
func (w *windower) processPartition(
	...
	usage := rowSliceOverhead + sizeOfRow*int64(len(w.windowFns))
	// TODO(yuzefovich): what happens to the iterator over w.allRowsPartitioned
	// in computeWindowFunctions() if the container is forced to spill to disk
	// here? We might need to track the number of rows already iterated over, and
	// if the spill occurs, we need to create a new iterator and advance it as
	// needed.
...
func (w *windower) computeWindowFunctions(ctx context.Context, evalCtx *tree.EvalContext) error {
	...
	usage := sliceOfIntsOverhead + sliceOfRowsSliceOverhead + (sizeOfInt+sizeOfSliceOfRows)*int64(len(w.allRowsPartitionSizes))
	if err := w.growMemAccount(&w.acc, usage); err != nil {
		return err
	}
	w.partitionSizes = make([]int, 0, len(w.allRowsPartitionSizes))
	w.windowValues = make([][][]tree.Datum, 0, len(w.allRowsPartitionSizes))

	bucket := ""
	// containerSize is the maximum size of any partition, and we use it to
	// preallocate the row container.
	var containerSize int
	if len(w.partitionBy) == 0 {
		containerSize = w.totalNumRows
	} else {
		containerSize = w.maxPartitionSize
	}
	...
	i := w.allRowsPartitioned.NewUnmarkedIterator(w.Ctx)
	defer i.Close()
