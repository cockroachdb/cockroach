...
type windower struct {
	...
	populated            bool
	bucketToPartitionIdx map[string]int
	rowsInBucketEmitted  map[string]int
...
func newWindower(
	...
	w.memAcc = memMonitor.MakeBoundAccount()
	...
	w.bucketToPartitionIdx = make(map[string]int)
	w.rowsInBucketEmitted = make(map[string]int)
...
func (w *windower) accumulateRows() (windowerState, sqlbase.EncDatumRow, *ProducerMetadata) {
	for {
		...
		if err := w.memAcc.Grow(w.ctx, int64(row.Size())); err != nil {
	...
	for _, encodedPartition := range w.encodedPartitions {
		for _, encRow := range encodedPartition {
			for i := range encRow {
				...
				// TODO: how can we learn the size of Datum before decoding it?
				if err := w.memAcc.Grow(w.ctx, int64(encRow[i].Datum.Size())); err != nil {
					return err
				}
			}
		}
...
func (w *windower) computeWindowFunctions(ctx context.Context, evalCtx *tree.EvalContext) error {
	...
	usage := int(unsafe.Sizeof([][]tree.Datum{}))*len(w.windowFns) + int(sizeOfIndexedRowsStruct)*len(w.encodedPartitions)
	if err := w.memAcc.Grow(w.ctx, int64(usage)); err != nil {
		return err
	}
	...
	for bucket, encodedPartition := range w.encodedPartitions {
		...
		// to be consistent.
		w.bucketToPartitionIdx[bucket] = partitionIdx
		usage = int(sizeOfIndexedRowStruct) * len(encodedPartition)
		if err := w.memAcc.Grow(w.ctx, int64(usage)); err != nil {
			return err
		}
	...
	usage = int(unsafe.Sizeof(int(0))+unsafe.Sizeof([]indexedRows{})+unsafe.Sizeof(false)) * len(w.windowFns)
	if err := w.memAcc.Grow(w.ctx, int64(usage)); err != nil {
		return err
	}
	...
	for windowFnIdx, windowFn := range w.windowFns {
		usage = int(unsafe.Sizeof([]tree.Datum{})) * len(w.encodedPartitions)
		if err := w.memAcc.Grow(w.ctx, int64(usage)); err != nil {
			return err
		}
		...
		for partitionIdx := 0; partitionIdx < len(partitions); partitionIdx++ {
			...
			usage = int(unsafe.Sizeof(tree.Datum(nil))) * partition.Len()
			if err := w.memAcc.Grow(w.ctx, int64(usage)); err != nil {
				return err
			}
				...
				if funcIdx := partitionPreviouslySortedFuncIdx[windowFnIdx]; funcIdx != -1 {
					...
					if shouldCacheSortedPartitions[windowFnIdx] {
						...
						if sortedPartitionsCache[windowFnIdx] == nil {
							usage = int(sizeOfIndexedRowsStruct) * len(partitions)
							if err := w.memAcc.Grow(w.ctx, int64(usage)); err != nil {
								return err
							}
						...
						// deep copy.
						usage = int(sizeOfIndexedRowStruct) * partition.Len()
						if err := w.memAcc.Grow(w.ctx, int64(usage)); err != nil {
							return err
						}
...
func (w *windower) populateNextOutputRow() bool {
	for bucket, encodedPartition := range w.encodedPartitions {
		if w.rowsInBucketEmitted[bucket] == len(encodedPartition) {
			// All output rows corresponding to partition 'bucket' have been fully
			// emitted already, so we skip it.
			continue
		}

		// We reuse the same EncDatumRow since caller of Next() should've copied it.
		w.outputRow = w.outputRow[:0]
		// rowIdx is the index of the next row to be emitted from partition 'bucket'.
		rowIdx := w.rowsInBucketEmitted[bucket]
		inputRow := encodedPartition[rowIdx]
		partitionIdx := w.bucketToPartitionIdx[bucket]
		inputColIdx := 0
		for windowFnIdx, windowFn := range w.windowFns {
		...
		w.rowsInBucketEmitted[bucket] = rowIdx + 1
		return true
	}
	return false
}
...
const sizeOfIndexedRowsStruct = unsafe.Sizeof(indexedRows{})
const sizeOfIndexedRowStruct = unsafe.Sizeof(indexedRow{})
