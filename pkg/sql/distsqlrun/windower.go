...
func (w *windower) computeWindowFunctions(ctx context.Context, evalCtx *tree.EvalContext) error {
	...
	for windowFnIdx, windowFn := range w.windowFns {
		...
		if windowFn.frame != nil {
			...
			if startBound.Offset != nil {
				encOffset := sqlbase.EncDatumFromEncoded(startBound.OffsetType.Encoding, startBound.Offset)
				if err := encOffset.EnsureDecoded(&startBound.OffsetType.Type, &w.datumAlloc); err != nil {
					return err
				}
				frameRun.StartBoundOffset = int(tree.MustBeDInt(encOffset.Datum))
			}
			if endBound != nil && endBound.Offset != nil {
				encOffset := sqlbase.EncDatumFromEncoded(endBound.OffsetType.Encoding, endBound.Offset)
				if err := encOffset.EnsureDecoded(&endBound.OffsetType.Type, &w.datumAlloc); err != nil {
					return err
				}
				frameRun.EndBoundOffset = int(tree.MustBeDInt(encOffset.Datum))
			}
		}
...
// to original - if offsetExprs were present in original WindowFrameBounds,
