...
func (w *windower) decodePartitions() {
	...
	for _, encodedPartition := range w.encodedPartitions {
		...
		for idx, encRow := range encodedPartition {
			partition = append(partition, tree.IndexedRow{Idx: idx, Row: w.decodeRow(encRow)})
		}
...
func (w *windower) computeWindowFunctions(ctx context.Context, evalCtx *tree.EvalContext) error {
	...
	for _, windowFn := range w.windowFns {
		// rowIdx is the index of the current row among all partitions.
		// It is needed to correctly populate outputRows.
		rowIdx := 0
		...
		for _, partition := range w.partitions {
			...
			if len(windowFn.ordering.Columns) > 0 {
				...
				// TODO(yuzefovich): we can cache all sorted partitions and not
				// resort them again if the same column ordering is encountered.
			...
			for frameRun.RowIdx < len(partition) {
				...
				for ; frameRun.RowIdx < frameRun.FirstPeerIdx+frameRun.PeerRowCount; frameRun.RowIdx++ {
					...
					// of the current row.
					w.outputRows[rowIdx] = append(w.outputRows[rowIdx], frameRun.Rows[frameRun.RowIdx].Row[inputColIdx:windowFn.argIdxStart]...)

					// We put the result of window function computation at windowFn.argIdxStart.
					w.outputRows[rowIdx] = append(w.outputRows[rowIdx], res)
					rowIdx++
				}
			}
		}
	...
	for _, partition := range w.partitions {
		for _, row := range partition {
			w.outputRows[rowIdx] = append(w.outputRows[rowIdx], row.Row[inputColIdx:]...)
			rowIdx++
		}
	}
