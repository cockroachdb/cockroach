...
func GetWindowFunctionInfo(
	funcSpec WindowerSpec_Func, inputTypes ...sqlbase.ColumnType,
) (
	windowConstructor func(*tree.EvalContext) tree.WindowFunc,
	returnType sqlbase.ColumnType,
	err error,
) {
	if !funcSpec.IsWindowFunc && funcSpec.AggregateFunc == AggregatorSpec_ANY_NOT_NULL {
	...
	funcStr := funcSpec.AggregateFunc.String()
	if funcSpec.IsWindowFunc {
		funcStr = funcSpec.WindowFunc.String()
	}
