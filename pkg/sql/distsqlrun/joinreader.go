...
func newJoinReader(
	...
	colIdxMap := jr.desc.ColumnIdxMap()
	if jr.index == &jr.desc.PrimaryIndex ||
		indexIsCovering(jr.index, colIdxMap, jr.out.neededColumns(), len(jr.inputTypes)) {
		...
		neededIndexColumns = getIndexColSet(&jr.desc.PrimaryIndex, colIdxMap)
		jr.primaryFetcher = &sqlbase.RowFetcher{}
		_, _, err = initRowFetcher(
			jr.primaryFetcher, &jr.desc, 0 /* indexIdx */, colIdxMap, false, /* reverse */
	...
	_, _, err = initRowFetcher(
		&jr.fetcher, &jr.desc, int(spec.IndexIdx), colIdxMap, false, /* reverse */
...
func (jr *joinReader) indexLookup(
	...
	for {
		...
		if !jr.isLookupJoin() {
			...
			for _, rowIdx := range rowIndices {
				...
				if jr.renderedRow, err = jr.render(rows[rowIdx], indexRow); err != nil {
					return false, err
				}
	...
	if emitted != nil {
		...
		for i := 0; i < len(rows); i++ {
			if !emitted[i] {
				jr.renderedRow = jr.renderUnmatchedRow(rows[i], leftSide)
				if !emitHelper(
					ctx, &jr.out, jr.renderedRow, nil /* meta */, jr.pushTrailingMeta, jr.input,
				) {
					return true, nil
