...
func newJoinReader(
	...
	if jr.index == &jr.desc.PrimaryIndex ||
		...
		// output columns.
		neededCols = getColumnIndices(jr.desc.PrimaryIndex, jr.desc.ColumnIdxMap())
...
func (jr *joinReader) indexLookup(
	...
	if jr.joinType == sqlbase.LeftOuterJoin {
		...
		for i := 0; i < len(rows); i++ {
			if !emittedRowIndices.Contains(i) {
