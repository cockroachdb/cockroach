...
func (jr *joinReader) Next() (sqlbase.EncDatumRow, *ProducerMetadata) {
	...
	for jr.state == stateRunning {
		if len(jr.toEmit) > 0 {
			// There is a row ready to emit.
			row := jr.toEmit[0]
			jr.toEmit = jr.toEmit[1:]
			return row, nil
		}

		if !jr.inputBatchReady {
