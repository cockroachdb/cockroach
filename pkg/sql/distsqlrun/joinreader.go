...
func newJoinReader(
	...
	_, _, err = initRowFetcher(
		...
		jr.rowFetcherColumns(spec.RightColumnMap), false /* isCheck */, &jr.alloc,
	)
	if err != nil {
		return nil, err
	}
...
func (jr *joinReader) rowFetcherColumns(rightColMap []int32) util.FastIntSet {
	neededCols := jr.out.neededColumns()
	if !jr.isLookupJoin() {
		return neededCols
	}
	neededCols = neededCols.Shift(-1 * len(jr.inputTypes))

	rightSide := util.MakeFastIntSet()
	rightSide.AddRange(0, len(jr.desc.Columns)-1)
	neededCols.IntersectionWith(rightSide)

	for i, val := range rightColMap {
		if val != -1 {
			neededCols.Add(i)
		}
	}
