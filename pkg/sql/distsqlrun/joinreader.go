...
// necessarily been drained; the caller should attempt to drain.
...
func (jr *joinReader) Run(ctx context.Context, wg *sync.WaitGroup) {
	...
	err := jr.mainLoop(ctx)
	if err != nil {
		DrainAndForwardMetadataFinal(ctx, jr.out.output, err, jr.input)
	} else {
		jr.out.close()
	}
}
