...
// sortedEdges implements the sort.Sort interface.
type sortedEdges []diagramEdge

func (s sortedEdges) Len() int { return len(s) }
func (s sortedEdges) Less(i, j int) bool {
	edge1, edge2 := s[i], s[j]

	if edge1.SourceProc != edge2.SourceProc {
		return edge1.SourceProc < edge2.SourceProc
	}

	if edge1.SourceOutput != edge2.SourceOutput {
		return edge1.SourceOutput < edge2.SourceOutput
	}
	if edge1.DestProc != edge2.DestProc {
		return edge1.DestProc < edge2.DestProc
	}

	return edge1.DestInput < edge2.DestInput
}
func (s sortedEdges) Swap(i, j int) { s[i], s[j] = s[j], s[i] }
