...
type procOutputHelper struct {
	...
	offset uint64
	// totalLimit is the number of rows after which we can stop (offset + limit),
...
func (h *procOutputHelper) init(
	...
	if post.Limit == 0 || post.Limit >= math.MaxUint64-h.offset {
		h.totalLimit = math.MaxUint64
	} else {
		h.totalLimit = h.offset + post.Limit
	}
