...
func (pb *ProcessorBase) Next() (sqlbase.EncDatumRow, *ProducerMetadata) {
	for pb.State == StateRunning {
		...
		if nextRow := pb.nextRow; nextRow != nil {
			pb.nextRow = nil
			return nextRow, nil
		}
		var row sqlbase.EncDatumRow
...
// rowSourceWrapper is a RowSourceInput that wraps a RowSource,
// presenting the RowSourceInput interface to a downstream ProcessorBase
...
var _ RowSourceInput = rowSourceWrapper{}

// MakeRowSourceInput creates a RowSourceInput that wraps an inputRowSource
...
func MakeRowSourceInput(source RowSource, consumer *ProcessorBase) RowSourceInput {
	return rowSourceWrapper{RowSource: source, consumer: consumer}
}

// NextFromInput implements the RowSourceInput interface.
