...
//
// The ok retval is true if more rows can be processed, false if the limit has
// been reached (if there's a limit). Upon seeing a false value, the caller is
// expected to start draining. Note that both a row and ok=false can be returned
// at the same time: the row that satisfies the limit is returned at the same
// time as a DrainRequested status. In that case, the caller is supposed to both
// deal with the row and start draining.
func (h *ProcOutputHelper) ProcessRow(
	ctx context.Context, row sqlbase.EncDatumRow,
) (_ sqlbase.EncDatumRow, ok bool, _ error) {
	if h.rowIdx >= h.maxRowIdx {
