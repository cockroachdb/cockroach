...
func convertFromAST(e tree.WindowFrameExclusion) WindowerSpec_Frame_Exclusion {
	switch e {
	case tree.ExcludeCurrentRow:
		return WindowerSpec_Frame_EXCLUDE_CURRENT_ROW
	case tree.ExcludeGroup:
		return WindowerSpec_Frame_EXCLUDE_GROUP
	case tree.ExcludeTies:
		return WindowerSpec_Frame_EXCLUDE_TIES
	case tree.ExcludeNoOthers:
		return WindowerSpec_Frame_EXCLUDE_NO_OTHERS
	default:
...
func (spec *WindowerSpec_Frame) InitFromAST(f *tree.WindowFrame, evalCtx *tree.EvalContext) error {
	...
	if f.Exclusion != nil {
		exclusion := convertFromAST(*f.Exclusion)
