...
func emitHelper(
	...
	if !meta.Empty() {
		...
		consumerStatus = output.output.PushRow(nil /* row */, meta)
	} else {
		var err error
		...
		if err != nil {
			...
			output.output.PushRow(nil /* row */, ProducerMetadata{Err: err})
			output.close()
			return false
		}
	}
	...
	case DrainRequested:
		...
		DrainAndForwardMetadataFinal(ctx, output.output, nil /* cause */, inputs...)
		return false
	case ConsumerClosed:
...
func (h *procOutputHelper) emitRow(
	...
	if r := h.output.PushRow(outRow, ProducerMetadata{}); r != NeedMoreRows {
