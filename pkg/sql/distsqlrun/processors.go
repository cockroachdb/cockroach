...
// should continue if vectorize=always and an error occurred when setting up
// the vectorized flow. Consider the case in which vectorize=always. The user
// must be able to unset this session variable without getting an error.
type VectorizeAlwaysException interface {
	...
	// on SET statements when running with vectorize = always. If there is a
	// better way, we should get rid of this interface.
	IsException() bool
}
