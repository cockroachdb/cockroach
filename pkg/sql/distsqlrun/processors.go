...
func (pb *processorBase) startInternal(ctx context.Context, name string) context.Context {
	...
	logTag += fmt.Sprintf("/%d", pb.processorID)
	pb.ctx = log.WithLogTag(ctx, logTag, nil)

	pb.origCtx = pb.ctx
	pb.ctx, pb.span = processorSpan(pb.ctx, name)
	if pb.span != nil {
		pb.span.SetTag("cockroach.processorid", pb.processorID)
	}
...
func newProcessor(
	...
	if core.ReadCSV != nil {
		...
		return NewReadCSVProcessor(flowCtx, processorID, *core.ReadCSV, outputs[0])
	}
	...
	if core.MetadataTestSender != nil {
		...
		// TODO(asubiotto): We're probably duplicating some functionality here with
		// the sender id.
...
// ccl/sqlccl/csv.go.
var NewReadCSVProcessor func(*FlowCtx, int32, ReadCSVSpec, RowReceiver) (Processor, error)
