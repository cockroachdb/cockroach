...
func newProjectSetProcessor(
	...
	outputTypes = append(outputTypes, input.OutputTypes()...)
	for _, t := range spec.GeneratedColumns {
		outputTypes = append(outputTypes, t)
	}
	n := &projectSetProcessor{
		input:       input,
		spec:        spec,
		exprHelpers: make([]*exprHelper, len(spec.Exprs)),
		funcs:       make([]*tree.FuncExpr, len(spec.Exprs)),
		rowBuffer:   make(sqlbase.EncDatumRow, len(input.OutputTypes())+len(spec.GeneratedColumns)),
...
func (ps *projectSetProcessor) Run(ctx context.Context, wg *sync.WaitGroup) {
	if ps.out.output == nil {
		panic("projectSetProcessor output not initialized for emitting rows")
	}
	ctx = ps.Start(ctx)
	Run(ctx, ps, ps.out.output)
	if wg != nil {
		wg.Done()
	}
}
	...
	for i := range ps.exprHelpers {
		...
		if gen := ps.gens[i]; gen != nil {
			...
			if !ps.done[i] {
				...
				value, err := ps.exprHelpers[i].expr.Eval(ps.evalCtx)
				if err != nil {
					return false, err
				}
				ps.rowBuffer[colIdx] = ps.toEncDatum(value, colIdx)
