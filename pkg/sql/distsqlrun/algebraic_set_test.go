...
func runProcessors(tc testCase) (sqlbase.EncDatumRows, error) {
	...
	s, err := newAlgebraicSet(&flowCtx, &tc.spec, inL, inR, &PostProcessSpec{}, out)
	if err != nil {
		return nil, err
	}
...
func TestUnionAll(t *testing.T) {
	...
	td := initTestData()
	v := td.v
	setSpecFirstColumnOrderedAscending := AlgebraicSetSpec{
		Ordering: Ordering{
		...
		Type: AlgebraicSetSpec_UnionAll,
	}
...
func TestExceptAll(t *testing.T) {
	...
	setSpecFirstColumnOrderedAscending := AlgebraicSetSpec{
		...
		Type: AlgebraicSetSpec_ExceptAll,
	}
	testCases := []testCase{
		{
			spec: AlgebraicSetSpec{
