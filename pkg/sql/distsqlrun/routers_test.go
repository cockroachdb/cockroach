						...
						for b2Idx, r2 := range rows {
							...
							for _, row2 := range r2 {
								...
								for _, c := range tc.spec.HashColumns {
									isDistinct, err := row[c].Distinct(&types[c], alloc, evalCtx, &row2[c])
									if err != nil {
										t.Fatal(err)
									}
									if isDistinct {
			...
			case OutputRouterSpec_MIRROR:
				...
				for bIdx, r := range rows {
					...
					for i, row := range r {
						...
						equal := true
						for j, c := range row {
							isDistinct, err := c.Distinct(&types[j], alloc, evalCtx, &row2[j])
							if err != nil {
								t.Fatal(err)
							}
							if isDistinct {
...
func TestRouterDiskSpill(t *testing.T) {
	...
	for i := 0; ; i++ {
		...
		for j, c := range row {
			if isDistinct, err := c.Distinct(&intType, alloc, &flowCtx.EvalCtx, &rows[i][j]); err != nil {
				t.Fatal(err)
			} else if isDistinct {
				t.Fatalf(
