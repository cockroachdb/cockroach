...
var (
	testRangeRouterSpans = []roachpb.Span{
		{
			Key:    keys.MinKey,
...
func TestConsumerStatus(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name: "RangeRouter",
			spec: OutputRouterSpec{Type: OutputRouterSpec_BY_RANGE, Spans: testRangeRouterSpans},
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			case *rangeRouter:
				d := parser.NewDBytes(parser.DBytes(testRangeRouterSpans[0].Key))
				row0 = sqlbase.EncDatumRow{sqlbase.DatumToEncDatum(colTypes[0], d)}
				d = parser.NewDBytes(parser.DBytes(testRangeRouterSpans[1].Key))
...
func TestRouterBlocks(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			spec: OutputRouterSpec{
				...
				Spans: testRangeRouterSpans,
				Other: &testRangeRouterOther,
			},
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			flowCtx := FlowCtx{Settings: cluster.MakeClusterSettings(), EvalCtx: parser.MakeTestingEvalContext()}
