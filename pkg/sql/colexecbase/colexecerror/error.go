...
// file name and the line number) should be caught by the vectorized engine.
//
...
func shouldCatchPanic(panicEmittedFrom string) bool {
	...
	// We have a few exceptions from the testing code that shouldn't be caught.
	for _, testExceptionPrefix := range []string{
		"github.com/cockroachdb/cockroach/pkg/sql/colexecbase/colexecerror_test.TestCatchVectorizedRuntimeError.func1.1.1()",
		"github.com/cockroachdb/cockroach/pkg/sql/colflow_test.(*testNonVectorizedPanicEmitter)",
	} {
