...
// HashIndexIncludeImplicitPartitionColFromRBR shows that hash sharded index cannot include
// implicit cols from an RBR table.
var HashIndexIncludeImplicitPartitionColFromRBR = pgerror.New(
	pgcode.FeatureNotSupported,
	`hash sharded indexes cannot include implicit partitioning columns from "LOCALITY REGIONAL BY ROW"`,
)

// HashIndexIncludeImplicitPartitionColFromPartitionAllBy shows that hash sharded index cannot include
// implicit cols from a PARTITION ALL BY table.
var HashIndexIncludeImplicitPartitionColFromPartitionAllBy = pgerror.New(
	pgcode.FeatureNotSupported,
	`hash sharded indexes cannot include implicit partitioning columns from "PARTITION ALL BY"`,
)

// NewIndexIncludeImplicitPartitionColFromRBR shows that the newly created index cannot include
// implicit cols from an RBR table.
var NewIndexIncludeImplicitPartitionColFromRBR = pgerror.New(
	pgcode.FeatureNotSupported,
