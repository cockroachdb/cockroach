...
// NewGeneratedAlwaysAsIdentityColumnOverrideError creates an error for explicitly writing a column
// created with `GENERATED ALWAYS AS IDENTITY` token.
// TODO(janexing): Should add a HINT with "Use OVERRIDING SYSTEM VALUE to override." once issue #68201 is resolved.
...
// NewGeneratedAlwaysColumnUpdateError creates an error for updating a column
// created with `GENERATED ALWAYS` token to an expression other than "DEFAULT".
func NewGeneratedAlwaysColumnUpdateError(columnName string) error {
	return errors.WithDetailf(
