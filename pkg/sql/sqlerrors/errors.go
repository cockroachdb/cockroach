...
// NewSchemaChangeToTableWithLockedSchema creates an error signaling schema
// change statement is attempted on a table with locked schema.
func NewSchemaChangeToTableWithLockedSchema(tableName string) error {
	return errors.WithHintf(pgerror.Newf(pgcode.SchemaChangeOnSchemaLockedTable,
		`schema changes are disallowed on table %q because its schema is locked.`, tableName),
		"To unlock its schema, try ALTER TABLE %v SET (schema_locked = false);", tableName)
}
