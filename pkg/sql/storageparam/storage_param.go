...
func Set(
	...
	if err := storageParamPreChecks(evalCtx, params, nil /* unsetParam */); err != nil {
		return err
	}
...
func Reset(
	...
	if err := storageParamPreChecks(evalCtx, nil /* setParams */, params); err != nil {
		return err
	}
...
func storageParamPreChecks(
	evalCtx *eval.Context, setParams tree.StorageParams, resetParams tree.NameList,
) error {
	...
	for _, key := range keys {
		if key == `schema_locked` {
			...
			// single-statement implicit transaction and no other storage params.
