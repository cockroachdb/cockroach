...
func Set(
	...
	if err := storageParamPreChecks(ctx, evalCtx, params, nil /* resetParams */, setter.IsNewObject()); err != nil {
		return err
	}
...
func Reset(
	...
	if err := storageParamPreChecks(ctx, evalCtx, nil /* setParam */, params, false /* isNewObject */); err != nil {
		return err
	}
...
func storageParamPreChecks(
	...
	for _, key := range keys {
		if key == `schema_locked` {
			...
			// For newly created tables we will allow schema_locked to be set upon creation, later operations
			// cannot unset this.
