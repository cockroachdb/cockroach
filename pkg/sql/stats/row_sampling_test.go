...
func runSampleTest(
	t *testing.T, evalCtx *tree.EvalContext, numSamples int, ranks []int, memAcc *mon.BoundAccount,
) {
	ctx := context.Background()
	var sr SampleReservoir
	if err := sr.Init(
		numSamples, 1, []*types.T{types.Int}, memAcc, util.MakeFastIntSet(0),
	); err != nil {
		t.Fatal(err)
	}
	for _, r := range ranks {
	...
	// If we decreased capacity to stay within memory limits, adjust expectations.
	if memAcc != nil && numSamples > sr.Cap() {
		numSamples = sr.Cap()
	}
	...
	for _, n := range []int{10, 100, 1000, 10000} {
		...
		for _, k := range []int{1, 5, 10, 100} {
			t.Run(fmt.Sprintf("n=%d/k=%d/mem=nolimit", n, k), func(t *testing.T) {
				runSampleTest(t, &evalCtx, k, ranks, nil)
			})
			for _, mem := range []int64{1 << 8, 1 << 10, 1 << 12} {
				t.Run(fmt.Sprintf("n=%d/k=%d/mem=%d", n, k, mem), func(t *testing.T) {
					...
					runSampleTest(t, &evalCtx, k, ranks, &memAcc)
				})
			}
		}
	}
