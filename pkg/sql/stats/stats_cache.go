...
func NewTableStatisticProto(
	...
	if partialStatisticsColumnsVerActive {
		expectedTypes = append(expectedTypes,
			[]struct {
				...
				{
					"fullStatisticsID", fullStatisticsIdIndex, types.Int, true,
				},
			}...,
		)
	}
	...
	if partialStatisticsColumnsVerActive && datums[partialPredicateIndex] != tree.DNull {
		res.PartialPredicate = string(*datums[partialPredicateIndex].(*tree.DString))
		res.FullStatisticsID = uint64(*datums[fullStatisticsIdIndex].(*tree.DInt))
	}
...
func (sc *TableStatisticsCache) getTableStatsFromDB(
	...
	var partialStatisticsColumns string
	if partialStatisticsColumnsVerActive {
		partialStatisticsColumns = `
"partialPredicate",
"fullStatisticsID",`
	}
