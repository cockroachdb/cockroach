...
// based on the cluster version. The delayDelete column was added in V26_1_AddTableStatisticsDelayDeleteColumn.
func GetTableStatisticsStmt(
	...
	if st.Version.IsActive(ctx, clusterversion.V26_1_AddTableStatisticsDelayDeleteColumn) {
	...
	WHERE "tableID" = $1
	ORDER BY "createdAt" %s, "columnIDs" %s, "statisticID" %s
`, delayDeleteColumn, ordering, ordering, ordering)
}
