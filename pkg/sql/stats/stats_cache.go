...
func (sc *TableStatisticsCache) parseStats(
...
) (*TableStatistic, descpb.ColumnID, *types.T, error) {
	...
	if len(res.ColumnIDs) == 1 {
		colID = res.ColumnIDs[0]
	}
	return res, colID, udt, nil
}
...
`
	...
	for ok, err = it.Next(ctx); ok; ok, err = it.Next(ctx) {
		stats, colID, udt, err := sc.parseStats(ctx, it.Cur())
		if err != nil {
