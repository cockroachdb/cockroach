...
func statsUsageAllowed(table catalog.TableDescriptor, clusterSettings *cluster.Settings) bool {
	...
	return statsCollectionAllowed(table)
}
...
// statsCollectionAllowed returns true if manual collection of statistics on the
// table type via CREATE STATISTICS or ANALYZE is allowed. Note that specific
// system tables may have stats collection disabled in the parser. This function
// just indicates if the type of table may have stats manually collected.
func statsCollectionAllowed(table catalog.TableDescriptor) bool {
