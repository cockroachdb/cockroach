...
type TableStatisticsCache struct {
	...
	tableStatisticsLocksTableID descpb.ID
...
func (sc *TableStatisticsCache) Start(
	...
	var err error
	sc.tableStatisticsLocksTableID, err = systemTableIDResolver.LookupSystemTableID(ctx, systemschema.TableStatisticsLocksTable.GetName())
	if err != nil {
		return err
	}
...
func (sc *TableStatisticsCache) DisallowedOnSystemTable(tableID descpb.ID) bool {
	...
	case keys.TableStatisticsTableID, keys.LeaseTableID, keys.ScheduledJobsTableID,
		keys.SpanConfigurationsTableID, sc.tableStatisticsLocksTableID:
		return true
	}
	return false
}
