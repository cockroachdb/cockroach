...
//
// The logic is:
//   - If we're within the canary window (latest stats creation time < asOf <
//     creation time + window), return the stable stats (older stats from before
//     the latest full stats).
...
func (e *cacheEntry) getStableStatsLocked(
	...
	if canaryWindowSize != 0 &&
		...
		if e.latestStableFullStatsTimestamp.After(asOf) {
			log.VEventf(ctx, 1, "time-travel stats limitation: asOf %s predates available stable stats from %s", asOf, e.latestStableFullStatsTimestamp)
		}
		log.VEventf(ctx, 1, "stable stats is chosen for table %d", e.tableID)
...
func (sc *TableStatisticsCache) refreshCacheEntry(
	...
	if len(stableStats) > 0 {
		e.latestStableFullStatsTimestamp = hlc.Timestamp{WallTime: stableStats[0].CreatedAt.UnixNano()}
	}
	e.refreshing = false
