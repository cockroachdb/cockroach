...
func (sc *TableStatisticsCache) getTableStatsFromDB(
	...
	// latestFullStatsCreatedAt to track the timestamp of the latest stats.
	...
	// It could happen that there are only one version of stats. In this
	// case, the stable stats is the same as the canary stats cache.
	if len(stableStatsList) == 0 {
		stableStatsList = statsList
	}
