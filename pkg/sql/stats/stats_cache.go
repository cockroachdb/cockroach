...
//
// Requires: caller must hold sc.mu.
func (sc *TableStatisticsCache) adjustEntryMemLocked(
	ctx context.Context, tableID descpb.ID, e *cacheEntry,
) {
	if sc.mon == nil {
		return
	}
	...
	// tracked.
	if v, ok := sc.mu.cache.StealthyGet(tableID); !ok || v.(*cacheEntry) != e {
		return
	}
