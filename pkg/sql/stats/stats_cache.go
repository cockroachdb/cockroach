...
func parseStats(datums tree.Datums, includeHistogram bool) (*Stats, error) {
	...
	tableStatistic := &Stats{
		...
		RowCount:      (uint32)(*datums[rowCountIndex].(*tree.DInt)),
		DistinctCount: (uint32)(*datums[distinctCountIndex].(*tree.DInt)),
		NullCount:     (uint32)(*datums[nullCountIndex].(*tree.DInt)),
	}
