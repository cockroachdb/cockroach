...
func (tabStat *TableStatistic) columnsString() string {
	cols := strconv.FormatUint(uint64(tabStat.ColumnIDs[0]), 10)
	for i := 1; i < len(tabStat.ColumnIDs); i++ {
		cols += ", " + strconv.FormatUint(uint64(tabStat.ColumnIDs[i]), 10)
	}
	return cols
}
