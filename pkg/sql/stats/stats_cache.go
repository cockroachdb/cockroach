...
func NewTableStatisticsCache(
	size func() int64, sqlExecutor sqlutil.InternalExecutor,
) *TableStatisticsCache {
	tableStatsCache := &TableStatisticsCache{}
	tableStatsCache.cache = cache.NewUnorderedCache(cache.Config{
		Policy:      cache.CacheLRU,
		ShouldEvict: func(s int, key, value interface{}) bool { return int64(s) > size() },
	})
	tableStatsCache.SQLExecutor = sqlExecutor
...
func (sc *TableStatisticsCache) Lookup(ctx context.Context, tableID sqlbase.ID) ([]*Stats, bool) {
	sc.mu.Lock()
	defer sc.mu.Unlock()
	if v, ok := sc.cache.Get(tableID); ok {
		if log.V(2) {
...
func (sc *TableStatisticsCache) Refresh(
	...
	if tableStatistics == nil {
		if log.V(2) {
			log.Infof(ctx, "r%d: updating statistics for table: not found", tableID)
		}
		return nil, nil
	}
	...
	sc.cache.Add(tableID, tableStatistics)
	return tableStatistics, nil
}

// LookupAndMaybeRefresh is a convenience function to look
// up statistics for the requested table ID in the cache using Lookup,
// and if the stats are not present in the cache, it looks them up in
// system.table_statistics using Refresh.
func (sc *TableStatisticsCache) LookupAndMaybeRefresh(
	...
	return sc.Refresh(ctx, txn, tableID)
}
...
func parseStats(datums tree.Datums, includeHistogram bool) (*Stats, error) {
	...
	tableStatistic.TableID = sqlbase.ID((int32)(*datums[tableIDIndex].(*tree.DInt)))
	tableStatistic.StatisticID = (uint32)(*datums[statisticsIDIndex].(*tree.DInt))
	...
	tableStatistic.CreatedAt = datums[createdAtIndex].(*tree.DTimestamp).Time
	tableStatistic.RowCount = (uint32)(*datums[rowCountIndex].(*tree.DInt))
	tableStatistic.DistinctCount = (uint32)(*datums[distinctCountIndex].(*tree.DInt))
	tableStatistic.NullCount = (uint32)(*datums[nullCountIndex].(*tree.DInt))
