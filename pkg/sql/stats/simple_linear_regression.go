...
// The R² (goodness of fit) measurement of the model is also returned.
func quantileSimpleLinearRegression(
	...
	// is ok. We'll leave them the way they are, and after using them will fix all
	// the outputs ŷᵢ(t) to be well-formed.

	// Now use the model to calculate R² and predict ŷₙ(t) given xₙ.
	var Σεε float64
	for i := 0; i < n; i++ {
		yHat := α.add(β.mult(x[i])).fixMalformed()
		ε := y[i].sub(yHat)
		Σεε += ε.integrateSquared()
	}
	yₙ = α.add(β.mult(xₙ)).fixMalformed()
