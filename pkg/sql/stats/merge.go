	...
	for _, partialStat := range partialStatsList {
		...
		if !seenCols.Contains(int(col)) {
			...
			if fullStat, ok := fullStatsMap[col]; ok && partialStat.CreatedAt.After(fullStat.CreatedAt) {
				merged, err = mergeExtremesStatistic(fullStat, partialStat, col)
				if err != nil {
...
//
// Merged Statistic: {row: 11, dist: 7, null: 4, size: 1}
// CreatedAt: 2022-01-03
...
func mergeExtremesStatistic(
	fullStat *TableStatistic, partialStat *TableStatistic, colID descpb.ColumnID,
	...
	//fmt.Printf("This is the column ID %d \n", colID)
	...
	if ConstructUsingExtremesPredicate(fullHistogram[0].UpperBound, fullHistogram[len(fullHistogram)-1].UpperBound, colID) != partialStat.PartialPredicate {
		fmt.Printf("This is predicate string column ID %v %v \n", ConstructUsingExtremesPredicate(fullHistogram[0].UpperBound, fullHistogram[len(fullHistogram)-1].UpperBound, colID), partialStat.PartialPredicate)
		return nil, errors.New("attempting to merge a partial histogram that was derived from a different full histogram")
	}
	...
	for i < len(partialHistogram) && j < len(fullHistogram) {
		if val, err := partialHistogram[i].UpperBound.CompareError(cmpCtx, fullHistogram[j].UpperBound); err == nil {
			if val == -1 {
				...
				return nil, errors.New("the full statistic histogram overlaps with the partial statistic histogram")
			} else {
