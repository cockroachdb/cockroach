...
func mergePartialStatistic(
	...
	// spans or that histogram collections are disabled. We can do better here by
	// subtracting from the full histogram based on the stat predicate, but return
	// the full statistic for simplicity since overestimates are generally safe.
...
func mergeHistograms(
	...
	for ; j < len(partial); j++ {
		...
		if cmp <= 0 {
			...
			if cmp < 0 {
				// The partial bucket overlaps with the first full bucket, so we only
