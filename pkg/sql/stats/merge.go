...
// the partial histogram buckets with UpperBound less than the first histogram
// bucket to start of the histogram and appending the partial histogram buckets
// that with UpperBounds that are greater than the full histogram bucket to the
// end of the histogram, and recalculating the counts for the new merged
...
// assigned to jobspb.MergedStatsName (check pkg/jobs/jobspb/wrap.go).
//
// For example, consider this case:
// Full Statistic: {row: 14, dist: 9, null: 4, size: 1}
// CreatedAt: 2022-01-02
// Histogram (format is: {NumEq, NumRange, DistinctRange, UpperBound}):
// [{1, 0, 0, 2}, {1, 1, 0, 4}, {1, 1, 0, 6}]
//
// Partial Statistic: {row, 19, dist: 3, null: 0, size: 1}
// CreatedAt: 2022-01-03
// Histogram: [{2, 0, 0, 0}, {2, 0, 0, 1}, {2, 0, 0, 7}, {2, 0, 0, 8}]
//
// Merged Statistic: {row: 29, dist: 18, null: 0, size: 1}
// CreatedAt: 2022-01-03
// Histogram: [{2, 0, 0, 0}, {2, 0, 0, 1}, {1, 0, 0, 2}, {1, 1, 0, 4},
// {1, 1, 0, 6}, {2, 0, 0, 7}, {2, 0, 0, 8}]
//
...
func mergeExtremesStatistic(
	...
	if fullStatColKey != partialStatColKey {
		return fullStat, errors.New("column sets for full table statistics and partial table statistics column sets do not match")
	}
	...
	if len(partialHistogram) == 0 {
		fullStat.Name = jobspb.MergedStatsName
		fullStat.CreatedAt = partialStat.CreatedAt
		return fullStat, nil
	}
