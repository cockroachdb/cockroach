...
func mergePartialStatistic(
	...
	if atExtremes {
		...
		if len(lowerExtremeBuckets) > 0 {
			mergedHistogram, err = MergeHistograms(ctx, cmpCtx, fullHistogram, lowerExtremeBuckets, fullStat.ColumnIDs[0])
			if err != nil {
				return nil, err
			}
		}
		...
		var err error
		mergedHistogram, err = MergeHistograms(ctx, cmpCtx, fullHistogram, partialHistogram, fullStat.ColumnIDs[0])
		if err != nil {
			return nil, err
		}
	}
