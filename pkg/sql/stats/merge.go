...
func MergedStatistics(
	...
	for i := len(stats) - 1; i >= 0; i-- {
		...
		if fullStat, ok := fullStatsMap[col]; ok && partialStat.CreatedAt.After(fullStat.CreatedAt) {
			...
			merged, err := mergePartialStatistic(ctx, fullStat, partialStat, st, atExtremes)
			if err != nil {
...
func mergePartialStatistic(
	...
	if atExtremes {
		...
		if len(lowerExtremeBuckets) > 0 {
			mergedHistogram, err = mergeHistograms(ctx, cmpCtx, mergedHistogram, lowerExtremeBuckets)
			if err != nil {
				return nil, err
			}
		}
		if len(upperExtremeBuckets) > 0 {
			mergedHistogram, err = mergeHistograms(ctx, cmpCtx, mergedHistogram, upperExtremeBuckets)
			if err != nil {
				return nil, err
			}
		}
		...
		var err error
		mergedHistogram, err = mergeHistograms(ctx, cmpCtx, mergedHistogram, partialHistogram)
		if err != nil {
			return nil, err
		}
	}
...
func mergeHistograms(
	...
	for ; j < len(partial); j++ {
		...
		if cmp, err := pBucket.UpperBound.Compare(ctx, evalCtx, fLowerBound); err == nil {
			if cmp <= 0 {
				...
				if cmp, err := pBucket.lowerBound.Compare(ctx, evalCtx, fLowerBound); err == nil {
					if cmp < 0 {
						// The partial bucket overlaps with the first full bucket, so we
						// only emit the non-overlapping part.
	...
	for ; i < len(full); i++ {
		...
		b := cat.HistogramBucket{
			NumEq:         full[i].NumEq,
			NumRange:      full[i].NumRange,
			DistinctRange: full[i].DistinctRange,
			UpperBound:    full[i].UpperBound,
		}
