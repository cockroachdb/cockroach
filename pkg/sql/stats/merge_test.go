...
func TestMergePartialStatistics(t *testing.T) {
	...
	for i, tc := range testCases {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			...
			merged, err := mergePartialStatistic(ctx, initial, partial, st, tc.partial.atExtremes)
			if err != nil {
				if !tc.err {
...
func TestMergedStatistics(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			partial: []*testStat{
				{
					at: 6, row: 17, dist: 10, null: 6, size: 1, colID: 1, atExtremes: true,
					hist: testHistogram{
						{1, 0, 0, 20},
						{1, 9, 7, 60},
					},
				...
				{
					at: 8, row: 12, dist: 8, null: 0, size: 1, colID: 1,
					hist: testHistogram{
						{2, 0, 0, 35},
						{2, 8, 3, 40},
						{1, 4, 2, 45},
			...
			expected: []*testStat{
				{
					at: 8, row: 41, dist: 23, null: 6, size: 1, colID: 1,
					hist: testHistogram{
						{1, 0, 0, 20},
						{1, 0, 0, 30},
						{2, 13, 6, 40},
