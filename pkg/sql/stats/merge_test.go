...
func TestMergeStatistics(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			initial: &testStat{
				at: 1, row: 11, dist: 9, null: 4, size: 1,
		...
		},
		{
			...
			partial: &testStat{
				at: 3, row: 3, dist: 3, null: 3, size: 5,
				hist: testHistogram{
					{1, 0, 0, 2},
					{1, 0, 0, 3},
					{1, 0, 0, 4},
			...
			expected: &testStat{
				at: 3, row: 29, dist: 15, null: 3, size: 3,
				hist: testHistogram{
					{1, 0, 0, 2},
					{1, 0, 0, 3},
					{1, 0, 0, 4},
		...
		},
		{
			...
			expected: &testStat{
				at: 2, row: 37, dist: 21, null: 0, size: 6,
				hist: testHistogram{
					{1, 0, 0, 0},
					{3, 3, 2, 5},
					{3, 3, 2, 8},
	...
	for i, tc := range testCases {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			...
			merged, err := mergeExtremesStatistic(initial, partial, descpb.ColumnID(1))
			if err != nil {
				if !tc.err {
...
func TestMergedStatistics(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			full: []*testStat{
				{
					at: 5, row: 14, dist: 12, null: 4, size: 1, colID: 1,
			...
			partial: []*testStat{
				...
				{
					at: 6, row: 5, dist: 5, null: 0, size: 1, colID: 4,
			...
			expected: []*testStat{
				...
				{
					at: 6, row: 12, dist: 8, size: 1, colID: 4,
		...
		},
		{
			...
			full: []*testStat{
				{
					at: 5, row: 11, dist: 9, null: 4, size: 1, colID: 1,
					hist: testHistogram{{1, 0, 0, 30}, {1, 9, 7, 40}},
				},
				{
					at: 5, row: 11, dist: 9, null: 4, size: 1, colID: 2,
					hist: testHistogram{{1, 0, 0, 20}, {1, 9, 7, 30}},
				},
			},
			partial: []*testStat{
				{
					at: 8, row: 22, dist: 18, null: 4, size: 1, colID: 1,
					hist: testHistogram{{2, 0, 0, 50}, {1, 9, 8, 60}, {1, 9, 7, 70}},
				},
				{
					at: 7, row: 2, dist: 2, null: 4, size: 1, colID: 2,
					hist: testHistogram{{1, 0, 0, 40}, {1, 0, 0, 50}},
				},
				{
					at: 7, row: 11, dist: 9, null: 4, size: 1, colID: 1,
					hist: testHistogram{{1, 0, 0, 50}, {1, 9, 7, 60}},
				},
				{
					at: 6, row: 1, dist: 1, null: 4, size: 1, colID: 2,
					hist: testHistogram{{1, 0, 0, 50}},
				},
			},
			expected: []*testStat{
				{
					at: 8, row: 37, dist: 27, null: 4, size: 1, colID: 1,
					hist: testHistogram{{1, 0, 0, 30.0}, {1, 9, 7, 40.0}, {2, 0, 0, 50.0}, {1, 9, 8, 60.0}, {1, 9, 7, 70.0}},
				},
				{
					at: 7, row: 17, dist: 11, null: 4, size: 1, colID: 2,
		...
		},
		{
			...
			partial: []*testStat{
				{
					at: 4, row: 11, dist: 9, null: 4, size: 1, colID: 1,
					hist: testHistogram{{1, 0, 0, 50}, {1, 9, 7, 60}},
				},
			},
			expected: []*testStat{},
