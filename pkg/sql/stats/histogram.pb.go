...
type HistogramData_Bucket struct {
	...
	EqRows int64 `protobuf:"varint,1,opt,name=eq_rows,json=eqRows,proto3" json:"eq_rows,omitempty"`
	...
	RangeRows int64 `protobuf:"varint,2,opt,name=range_rows,json=rangeRows,proto3" json:"range_rows,omitempty"`
...
func (m *HistogramData_Bucket) MarshalTo(dAtA []byte) (int, error) {
	...
	if m.EqRows != 0 {
		dAtA[i] = 0x8
		i++
		i = encodeVarintHistogram(dAtA, i, uint64(m.EqRows))
	}
	if m.RangeRows != 0 {
		dAtA[i] = 0x10
		i++
		i = encodeVarintHistogram(dAtA, i, uint64(m.RangeRows))
	}
...
func (m *HistogramData_Bucket) Size() (n int) {
	...
	_ = l
	if m.EqRows != 0 {
		n += 1 + sovHistogram(uint64(m.EqRows))
	}
	if m.RangeRows != 0 {
		n += 1 + sovHistogram(uint64(m.RangeRows))
	}
	l = len(m.UpperBound)
	if l > 0 {
		n += 1 + l + sovHistogram(uint64(l))
...
func (m *HistogramData_Bucket) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 1:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field EqRows", wireType)
			}
			m.EqRows = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
		...
		case 2:
			...
			for shift := uint(0); ; shift += 7 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.RangeRows |= (int64(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
...
var fileDescriptorHistogram = []byte{
	...
	0x83, 0x58, 0x10, 0xa5, 0x4a, 0x7b, 0x19, 0xb9, 0x78, 0x3d, 0x60, 0xda, 0x5d, 0x12, 0x4b, 0x12,
	...
	,
}
