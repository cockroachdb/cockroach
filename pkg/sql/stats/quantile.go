...
// makeQuantile and quantile.toHistogram might need to change if we introduce a
// new histogram version.
...
func CanMakeQuantile(colType *types.T) bool {
	...
	case types.IntFamily,
		...
		// TODO(michae2): Check that there are no constraints making this a de facto
		// ENUM. (Could also check histogram for sum(NumRange) > 0.)
		return true
	default:
		return false
	}
...
func (qf quantile) toHistogram(colType *types.T, rowCount float64) (histogram, error) {
	...
	// None of our supported types need information from eval.Context, but we do
	// need UnwrapDatum for CompareError to work correctly, so use a typed nil.
	var nilEvalContext *eval.Context
	...
	closeCurrentBucket := func() error {
		...
		// Calculate DistinctRange for this bucket now that NumRange is final.
		lowerBound := getNextLowerBound(nilEvalContext, currentUpperBound)
		distinctRange := estimatedDistinctValuesInRange(
			nilEvalContext, currentBucket.NumRange, lowerBound, currentUpperBound,
		)
		if !isPositive(distinctRange) {
	...
	for ; i < len(qf); i++ {
		...
		cmp, err := upperBound.CompareError(nilEvalContext, currentUpperBound)
		if err != nil {
			return histogram{}, err
		}
		if cmp < 0 {
