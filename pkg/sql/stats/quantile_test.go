	...
	for _, colType := range colTypes {
		for i := 0; i < 5; i++ {
			t.Run(fmt.Sprintf("%v/%v", colType.Name(), i), func(t *testing.T) {
				...
				if err != nil {
					t.Errorf("unexpected makeQuantile error: %v", err)
					return
				}
				hist2, err := qfun.toHistogram(evalCtx, colType, rowCount)
				if err != nil {
					t.Errorf("unexpected quantile.toHistogram error: %v", err)
					return
				}
				if !reflect.DeepEqual(hist, hist2) {
					t.Errorf("incorrect histogram:\n%v\nexpected:\n%v\nseed: %v", hist2, hist, seed)
				}
			})
		}
	}
...
func randBounds(evalCtx *eval.Context, colType *types.T, rng *rand.Rand, num int) tree.Datums {
	...
	randFloat64s := func(num int, lo, hi float64) []float64 {
...
func randomQuantile(rng *rand.Rand) quantile {
	// We'll generate [2, 32] total points with [1, numPoints] unique v values and
	// [0, numPolints-2] unique p values (not counting p=0 and p=1 which we add
	// manually).
	numPoints := rng.Intn(31) + 2
	numUniqueVs := rng.Intn(numPoints) + 1
	numUniquePs := rng.Intn(numPoints - 1)
	qf := make(quantile, numPoints)
	vs := make([]float64, numUniqueVs, numPoints)
	ps := make([]float64, numUniquePs, numPoints)
	for i := range vs {
		vs[i] = rng.NormFloat64() * 1000
	}
	for len(vs) < numPoints {
		vs = append(vs, vs[rng.Intn(numUniqueVs)])
	}
	sort.Float64s(vs)
	for i := range ps {
		ps[i] = rng.Float64()
	...
	ps = append(ps, 0)
	ps = append(ps, 1)
	for len(ps) < numPoints {
		ps = append(ps, ps[rng.Intn(numUniquePs+2)])
	}
	sort.Float64s(ps)
	for i := range qf {
		qf[i].v, qf[i].p = vs[i], ps[i]
	}
	return qf
}
