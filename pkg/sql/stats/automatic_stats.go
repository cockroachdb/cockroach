...
func (r *Refresher) automaticStatisticsEnabled(desc catalog.TableDescriptor) bool {
...
func (r *Refresher) autoStatsEnabled(ctx context.Context, tableID descpb.ID) bool {
	var desc catalog.TableDescriptor
	var err error
	if desc, err = r.getTableDescriptor(ctx, tableID); err != nil {
		return AutomaticStatisticsClusterMode.Get(&r.st.SV)
	}
	return r.automaticStatisticsEnabled(desc)
}

func (r *Refresher) autoStatsMinStaleRows(ctx context.Context, tableID descpb.ID) int64 {
	var desc catalog.TableDescriptor
	var err error
	if desc, err = r.getTableDescriptor(ctx, tableID); err != nil {
...
func (r *Refresher) autoStatsFractionStaleRows(ctx context.Context, tableID descpb.ID) float64 {
	var desc catalog.TableDescriptor
	var err error
	if desc, err = r.getTableDescriptor(ctx, tableID); err != nil {
				...
				if err := stopper.RunAsyncTask(
					ctx, "stats.Refresher: maybeRefreshStats", func(ctx context.Context) {
						...
						for tableID, rowsAffected := range mutationCounts {
							...
							if !r.autoStatsEnabled(ctx, tableID) {
...
WHERE
	...
	autoStatsEnableOrNotSpecifiedPredicate = `AND 
	(crdb_internal.pb_to_json('cockroach.sql.sqlbase.Descriptor',
...
func (r *Refresher) NotifyMutation(table catalog.TableDescriptor, rowsAffected int) {
	if !r.automaticStatisticsEnabled(table) {
		return
	}
...
func (r *Refresher) maybeRefreshStats(
	...
	statsFractionStaleRows := r.autoStatsFractionStaleRows(ctx, tableID)
	statsMinStaleRows := r.autoStatsMinStaleRows(ctx, tableID)
