...
// misestimates. It de-duplicates and merges spans while keeping track of the
// estimated row count.
func (r *Refresher) addMisestimate(mis misestimate) {
	id := mis.tableIDIndexID
	oldSpans := r.misestimateSpans[id].Spans
	if len(oldSpans) == 0 {
		r.misestimateSpans[id] = mis.spansWithRowCount
		return
	}
	...
	r.misestimateSpans[id] = spansWithRowCount{
		Spans: updatedSpans,
		// Since we included at least partial new span, we'll have a crude
		// estimate of simply adding the precise row count in the new spans.
