...
func (r *Refresher) ensureAllTables(ctx context.Context, settings *settings.Values) {
	...
	// We wait for the asOfTime delay and some more before initialization,
	// to ensure that the historical read will pick up all system tables.
	time.Sleep(r.asOfTime + time.Second)

	rows, err := r.ex.Query(
		ctx,
		"get-tables",
		nil, /* txn */
		fmt.Sprintf(`SELECT table_id FROM crdb_internal.tables AS OF SYSTEM TIME '-%s'`, r.asOfTime),
	)
	if err != nil {
