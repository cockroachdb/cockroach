...
// to the Refresher thread by calling MaybeRefreshStats, which sends mutation
...
func (r *Refresher) Start(
	...
	stopper.RunWorker(context.Background(), func(ctx context.Context) {
		...
		for {
			select {
			case <-timer.C:
				go func(mutationCounts map[sqlbase.ID]int) {
...
// MaybeRefreshStats is called by SQL mutation operations to signal to the
...
// alternate AS OF time for testing purposes. It is called by the background
// Refresher thread.
func (r *Refresher) maybeRefreshStats(
