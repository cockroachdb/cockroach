...
func (r *Refresher) NotifyMutation(table catalog.TableDescriptor, rowsAffected int) {
	...
	if table.GetState() != descpb.DescriptorState_PUBLIC {
		// Offline tables or tables being dropped or added shouldn't have stats
		// collected.
		return
	}
	if table.GetID() == catid.InvalidDescID {
		log.Warningf(context.TODO(),
			"auto stats mutations notified for invalid descriptor. Table name: %q with %d rows affected",
			table.GetName(), rowsAffected)
		return
	}
