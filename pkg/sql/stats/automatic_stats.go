...
type Refresher struct {
	...
	// TODO(yuzefovich): we should add memory accounting for spans stored in
	// this map.
...
func (r *Refresher) FixMisestimate(
	...
	case r.misestimates <- misestimate{
		indexInfo: indexInfo{
			...
			indexID:       indexID,
			keyColumnName: idx.GetKeyColumnName(0 /* columnOrdinal */),
			keyColumnDir:  idx.GetKeyColumnDirection(0 /* columnOrdinal */),
			keyColumnType: col.GetType(),
		},
		// TODO(yuzefovich): we could consider stripping TableID/IndexID prefix
...
func (r *Refresher) refreshAutoPartialWhere(
	...
	for _, span := range spans {
		filter, err := spanToBounds(r.codec, span, target, &da)
		if err != nil {
			return err
		}
		if filter == "false" {
...
// will remain unset. The remainder of the undecoded key is returned.
func decodeIndexKey(
	codec keys.SQLCodec,
	...
	remainingKey, _, err := rowenc.DecodeKeyVals(vals, colDirs, key)
	return remainingKey, err
}
...
// TODO(yuzefovich): consider column names with special characters that might
// need quoting.
func spanToBounds(
	codec keys.SQLCodec, span roachpb.Span, info indexInfo, da *tree.DatumAlloc,
) (string, error) {
	var startArr, endArr [1]rowenc.EncDatum
	colDirs := []catenumpb.IndexColumn_Direction{info.keyColumnDir}
	remainingStart, err := decodeIndexKey(codec, startArr[:], colDirs, span.Key)
	if err != nil {
		return "", err
	}
	...
	remainingEnd, err := decodeIndexKey(codec, endArr[:], colDirs, span.EndKey)
	if err != nil {
		return "", err
	}
	// The datum might be left unset, which indicates an open interval (i.e.
	...
	case start != nil && end != nil:
		// Closed interval on both ends: [start, end].
		decodedStart := span.Key[:len(span.Key)-len(remainingStart)]
		decodedEnd := span.EndKey[:len(span.EndKey)-len(remainingEnd)]
		if bytes.Compare(decodedStart, decodedEnd) == 0 {
