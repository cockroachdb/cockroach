...
// WriteStatsWithOldDeleted implements the "DELETE staled stats -> INSERT
// new stats" retention rule. It deletes old statistics for the given
// columns (if not partial) and then inserts the new statistic if the
// new statistics is not partial. For createdAt and statisticID, if the
// provided value are empty, we use the default value defined in the
