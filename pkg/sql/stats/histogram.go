...
func EquiDepthHistogram(
	...
	sort.Slice(samples, func(i, j int) bool {
		return tree.TotalOrderLess(evalCtx, samples[i], samples[j])
	})
	...
	for i, b := 0, 0; b < numBuckets && i < numSamples; b++ {
		...
		for ; numLess < num-1; numLess++ {
			if c := tree.TotalOrderCompare(evalCtx, samples[i+numLess], upper); c == 0 {
				break
			} else if c > 0 {
		...
		for ; i+num < numSamples; num++ {
			if tree.Distinct(evalCtx, samples[i+num], upper) {
				break
			}
		}
