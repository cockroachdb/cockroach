...
func EquiDepthHistogram(
	...
	i := 0
	for b := 0; b < numBuckets && i < numSamples; b++ {
		...
		h.Buckets = append(h.Buckets, HistogramData_Bucket{
			EqRows:     int64(num-numLess) * numRows / int64(numSamples),
			RangeRows:  int64(numLess) * numRows / int64(numSamples),
			UpperBound: encoded,
		})
	}
	return h, nil
