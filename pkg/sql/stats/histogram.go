...
func (h *histogram) adjustCounts(
	...
	if remDistinctCount > distinctCountRange {
		...
		if maxDistinct, ok := tree.MaxDistinctCount(evalCtx, lowerBound, upperBound); ok {
			// Remove len(buckets) to account for the upper bounds of the buckets, along with
			// the current range distinct count which has already been accounted for.
	...
	if rowCountRange == 0 || distinctCountRange == 0 {
		// This could happen for some types like enum. Return here so we avoid a
	...
	for i := range h.buckets {
		...
		h.buckets[i].NumRange *= adjustmentFactorNumRange
	}
}
...
func (h histogram) toHistogramData(colType *types.T) (HistogramData, error) {
	...
	for i := range h.buckets {
		...
		histogramData.Buckets[i] = HistogramData_Bucket{
			NumEq:         int64(h.buckets[i].NumEq),
			NumRange:      int64(h.buckets[i].NumRange),
