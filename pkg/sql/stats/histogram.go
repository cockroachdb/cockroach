...
// splitPartial partitions a partial statistic histogram into a slice of
// histograms, where each histogram represents a single span over which the
// partial statistic was collected.
func (h *histogram) splitPartial() [][]cat.HistogramBucket {
	var res [][]cat.HistogramBucket
	start := 0
	for i, b := range h.buckets {
		if b.UpperBound == tree.DNull {
			if i > start {
				res = append(res, h.buckets[start:i])
			}
			start = i + 1
		}
	}
	if start < len(h.buckets) {
		res = append(res, h.buckets[start:])
	}
	return res
}

// ConstructPartialHistogram that are equi-depth histograms stitched together
// for each of the provided partitionedValues, representing spans over which
// the partial statistic was collected. partitionedValues should be a slice of
// non-empty tree.Datums, where each element is the set of sampled values for a
// single span, where the spans are in ascending order.
func ConstructPartialHistogram(
	ctx context.Context,
	compareCtx tree.CompareContext,
	colType *types.T,
	partitionedValues []tree.Datums,
	numRows, distinctCount int64,
	maxBuckets int,
	st *cluster.Settings,
) (HistogramData, []cat.HistogramBucket, error) {
	// If there are no new values, we just return an empty partial histogram.
	var numTotalSamples int64
	for _, values := range partitionedValues {
	...
	numSpans := len(partitionedValues)
	estimatedNumRowsPerSpan := make([]int64, numSpans)

	var buckets []cat.HistogramBucket
	bucketsPerSpan := maxBuckets / numSpans
	for i, spanValues := range partitionedValues {
		estimatedNumRowsPerSpan[i] = (int64(spanValues.Len()) * numRows) / numTotalSamples
		hist, err := equiDepthHistogramWithoutAdjustment(ctx, compareCtx, spanValues, estimatedNumRowsPerSpan[i], bucketsPerSpan, st)
		if err != nil {
			return HistogramData{}, nil, err
		}

		// Add a delimiter bucket between spans.
		if i < numSpans-1 {
		...
		buckets = append(buckets, hist.buckets...)
	}
...
func (h *histogram) adjustCounts(
	...
	if remDistinctCount > distinctCountRange {
		...
		if maxDistinct, ok := tree.MaxDistinctCount(ctx, compareCtx, lowerBound, upperBound); ok {
			...
			// been accounted for.
			maxDistinctCountRange = float64(maxDistinct) - float64(numBuckets) - distinctCountRange
		}
		...
		if maxDistinctCountRange > 0 {
			...
			avgRemPerBucket := remDistinctCount / float64(numBuckets-1)
			for i := 1; i < len(h.buckets); i++ {
				if h.buckets[i].IsDelimiter() {
					continue
				}
				lowerBound := h.buckets[i-1].UpperBound
				if h.buckets[i-1].IsDelimiter() && i-2 >= 0 {
					// maybe we should continue instead?
					lowerBound = h.buckets[i-2].UpperBound
				}
