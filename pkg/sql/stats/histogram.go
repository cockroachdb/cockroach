...
func EquiDepthHistogram(
	...
	_, h, err := equiDepthHistogramWithoutAdjustment(compareCtx, colType, samples, numRows, distinctCount, maxBuckets)
	if err != nil {
		return HistogramData{}, []cat.HistogramBucket{}, err
	}
...
func equiDepthHistogramWithoutAdjustment(
	...
	if maxBuckets < 2 {
		return HistogramData{}, histogram{}, errors.Errorf("histogram requires at least two buckets")
	}
	if numRows < int64(numSamples) {
		return HistogramData{}, histogram{}, errors.Errorf("more samples than rows")
	}
	if distinctCount == 0 {
		return HistogramData{}, histogram{}, errors.Errorf("histogram requires distinctCount > 0")
	}
	for _, d := range samples {
		if d == tree.DNull {
			return HistogramData{}, histogram{}, errors.Errorf("NULL values not allowed in histogram")
		}
	}
	...
	for i, b := 0, 0; b < numBuckets && i < numSamples; b++ {
		...
		for ; numLess < numSamplesInBucket-1; numLess++ {
			if c := samples[i+numLess].Compare(compareCtx, upper); c == 0 {
				...
				return HistogramData{}, histogram{}, errors.AssertionFailedf("%+v", "samples not sorted")
			}
		}
	...
	histogramData, err := h.toHistogramData(colType)
	return histogramData, h, err
}

// ConstructExtremesHistogram returns a histogram that are is equi-depth
...
// samples up to the value specified by lowerBound and the second histogram
// covers the values up from lowerBound to the maximum. This function assumes
// that the sample only includes values from the extremes of the column.
func ConstructExtremesHistogram(
	...
	if maxBuckets < 2 {
		return HistogramData{}, []cat.HistogramBucket{}, errors.Errorf("partial histogram requires at least four buckets")
	}

	var lowerSamples tree.Datums
	var upperSamples tree.Datums
	for _, val := range values {
		if c := val.Compare(compareCtx, lowerBound); c == -1 {
	...
	_, lowerHist, err := equiDepthHistogramWithoutAdjustment(compareCtx, colType, lowerSamples, estNumRowsLower, distinctCount, maxBuckets/2)
	if err != nil {
		return HistogramData{}, nil, err
	}
	_, upperHist, err := equiDepthHistogramWithoutAdjustment(compareCtx, colType, upperSamples, estNumRowsUpper, distinctCount, maxBuckets/2)
	if err != nil {
		return HistogramData{}, nil, err
	}
