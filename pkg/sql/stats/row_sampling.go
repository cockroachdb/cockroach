...
// MaybeResize safely decreases the capacity of the reservoir (K) without
// introducing bias. (Note that the capacity can only decrease without
...
func (sr *SampleReservoir) retryMaybeResize(ctx context.Context, op func() error) error {
	for {
		if err := op(); err == nil || !sqlerrors.IsOutOfMemoryError(err) ||
			len(sr.samples)/2 < sr.minNumSamples {
			return err
		}
