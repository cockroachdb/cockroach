...
func DeleteOldStatsForOtherColumns(
	...
	const columnIDsPlaceholdersStart = 3
	columnIDsVals := make(tree.Datums, len(columnIDs))
	for i := range columnIDs {
		...
		columnIDsVals[i] = columnIDsVal
	}

	args := make([]interface{}, 0, len(columnIDsVals)+2)
	args = append(args, tableID, keepTime)
	for i := range columnIDsVals {
		args = append(args, columnIDsVals[i])
	}
