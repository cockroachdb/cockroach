...
// for statistics.
//
// The keepTimeIdx in the returned parameter is to keep track of the index of the
// keepTime entry in the returned placeholderVals. We will need to set this entry
// to zero in DeleteExpiredStatsForOtherColumns.
func GetPlaceholderValsFromColumnIDs(
	...
	keepTimeIdx = len(placeholderVals) - 1
	columnIDsPlaceholdersStart := len(placeholderVals) + 1
	for i := range columnIDs {
		columnIDsVal := tree.NewDArray(types.Int)
		for _, c := range columnIDs[i] {
			if err := columnIDsVal.Append(tree.NewDInt(tree.DInt(int(c)))); err != nil {
				return columnIDsPlaceholders, placeholderVals, keepTimeIdx, err
			}
		}
		if i > 0 {
...
func DeleteExpiredStatsForOtherColumns(
	...
	// We don't need the keepTime constraint, but we have to keep its
	// placeholder ($2) because columnIDsPlaceholders is fixed.
	withZeroTime := append([]interface{}(nil), placeholderVals...)
	withZeroTime[keepTimeIdx] = time.Duration(0)
	_, err := txn.Exec(
