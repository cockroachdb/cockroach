...
func TestMaybeRefreshStats(t *testing.T) {
	...
	explicitSettings := catpb.ClusterSettingsForTable{SqlStatsAutomaticCollectionMinStaleRows: &minStaleRows}
	refresher.maybeRefreshStats(
	...
	explicitSettings = catpb.ClusterSettingsForTable{SqlStatsAutomaticCollectionFractionStaleRows: &fractionStaleRows}
	refresher.maybeRefreshStats(
		ctx, descA.GetID(), &explicitSettings, 10 /* rowsAffected */, time.Microsecond, /* asOf */
	)
	if err := checkStatsCount(ctx, cache, descA, 1 /* expected */); err != nil {
		...
		for i := 0; i < b.N; i++ {
			for j := 0; j < numOps; j++ {
				for _, desc := range tabDescrs {
					refresher.maybeRefreshStats(
						ctx, desc.GetID(), nil /* explicitSettings */, 0 /* rowsAffected */, time.Microsecond, /* asOf */
					)
				}
			}
		}
...
func TestMutationsAndSettingOverrideChannels(t *testing.T) {
	...
	// items than can fit in the buffer.
	clusterSettings := &catpb.ClusterSettingsForTable{}
	tableDesc.TableDesc().ClusterSettingsForTable = clusterSettings
	minStaleRows := int64(1)
	clusterSettings.SqlStatsAutomaticCollectionMinStaleRows = &minStaleRows
	for i := 0; i < refreshChanBufferLen+10; i++ {
		int64CurrIteration := int64(i)
		clusterSettings.SqlStatsAutomaticCollectionMinStaleRows = &int64CurrIteration
		r.NotifyMutation(tableDesc, 5 /* rowsAffected */)
	}
