...
func TestMaybeRefreshStats(t *testing.T) {
	...
	if err := checkStatsCount(ctx, cache, descB.ID, 0 /* expected */); err != nil {
		t.Fatal(err)
	}

	// Disable automatic stats. Stats must not be generated.
	evalCtx.SessionData.AutomaticStatistics = false
	maybeRefreshStats(evalCtx, cache, executor, descA.ID, 10 /* rowsAffected */, 0 /* asOf */)
	if err := checkStatsCount(ctx, cache, descA.ID, 0 /* expected */); err != nil {
		t.Fatal(err)
	}

	// Enable automatic stats. There are no stats yet, so this must refresh the
	// statistics on table t even though rowsAffected=0.
	evalCtx.SessionData.AutomaticStatistics = true
	maybeRefreshStats(evalCtx, cache, executor, descA.ID, 0 /* rowsAffected */, 0 /* asOf */)
	if err := checkStatsCount(ctx, cache, descA.ID, 1 /* expected */); err != nil {
		t.Fatal(err)
	}

	// Try to refresh again. It won't work since the most recent stats are less
	// than 1 hour old.
	maybeRefreshStats(evalCtx, cache, executor, descA.ID, 10 /* rowsAffected */, 0 /* asOf */)
	if err := checkStatsCount(ctx, cache, descA.ID, 1 /* expected */); err != nil {
		t.Fatal(err)
	...
	// But it will work to refresh a different table.
	if err := checkStatsCount(ctx, cache, descB.ID, 0 /* expected */); err != nil {
		t.Fatal(err)
	}
	maybeRefreshStats(evalCtx, cache, executor, descB.ID, 10 /* rowsAffected */, 0 /* asOf */)
	if err := checkStatsCount(ctx, cache, descB.ID, 1 /* expected */); err != nil {
		t.Fatal(err)
	}

	// Now update the stats to look like they were created two hours ago.
	sqlRun.Exec(t,
		`UPDATE system.table_statistics
		SET "createdAt" = now()::TIMESTAMP - '2h'::INTERVAL
		WHERE "tableID" = $1;`, descA.ID)
	if err := checkStatsCount(ctx, cache, descA.ID, 1 /* expected */); err != nil {
		t.Fatal(err)
	}

	// Try to refresh. With rowsAffected=0, the probability of a refresh is 0, so
	// refreshing will not succeed.
	maybeRefreshStats(evalCtx, cache, executor, descA.ID, 0 /* rowsAffected */, 0 /* asOf */)
	if err := checkStatsCount(ctx, cache, descA.ID, 1 /* expected */); err != nil {
		t.Fatal(err)
	}
	...
	maybeRefreshStats(
		evalCtx, cache, executor, descA.ID, 10 /* rowsAffected */, time.Second, /* asOf */
	)
...
func TestAverageRefreshTime(t *testing.T) {
	...
	checkAverageRefreshTime := func(expected time.Duration) error {
		...
		if actual := averageRefreshTime(stats); actual != expected {
			return fmt.Errorf("expected averageRefreshTime %s but found %s",
	...
	checkMostRecentStat := func(expectedAge time.Duration, lessThan bool) error {
		...
		if stat == nil {
			fmt.Errorf("no recent automatic statistic found")
		}
		if !lessThan && stat.CreatedAt.After(time.Now().Add(-1*expectedAge)) {
			fmt.Errorf("most recent stat is less than %s old. Created at: %s Current time: %s",
				expectedAge, stat.CreatedAt, time.Now(),
			)
		}
		if lessThan && stat.CreatedAt.Before(time.Now().Add(-1*expectedAge)) {
			fmt.Errorf("most recent stat is more than %s old. Created at: %s Current time: %s",
				expectedAge, stat.CreatedAt, time.Now(),
			)
		}
		return nil
	...
	// Since there are no stats yet, averageRefreshTime should return the default
	// value.
	...
	if err := s.DB().Txn(ctx, func(ctx context.Context, txn *client.Txn) error {
		for i := 0; i < 10; i++ {
			...
			columnIDsVal.Append(tree.NewDInt(tree.DInt(1)))
			createdAt := tree.MakeDTimestamp(time.Now().Add(time.Duration(-1*(i*3+7))*time.Hour), time.Hour)
	...
	// None of the stats have the name autoStatsName, so averageRefreshTime
	...
	if err := s.DB().Txn(ctx, func(ctx context.Context, txn *client.Txn) error {
		for i := 0; i < 10; i++ {
			columnIDsVal := tree.NewDArray(types.Int)
			columnIDsVal.Append(tree.NewDInt(tree.DInt(2)))
			createdAt := tree.MakeDTimestamp(time.Now().Add(time.Duration(-1*(i*4+6))*time.Hour), time.Hour)
	...
	// is 0.
	maybeRefreshStats(evalCtx, cache, executor, tableID, 0 /* rowsAffected */, 0 /* asOf */)
	if err := checkStatsCount(ctx, cache, tableID, 20 /* expected */); err != nil {
		t.Fatal(err)
	}

	// Add some stats on column k in table a with name autoStatsName, separated
	// by two hours each, starting 5 hours ago.
	if err := s.DB().Txn(ctx, func(ctx context.Context, txn *client.Txn) error {
		for i := 0; i < 10; i++ {
			columnIDsVal := tree.NewDArray(types.Int)
			columnIDsVal.Append(tree.NewDInt(tree.DInt(1)))
			createdAt := tree.MakeDTimestamp(time.Now().Add(time.Duration(-1*(i*2+5))*time.Hour), time.Hour)
			if err := insertStat(txn, autoStatsName, columnIDsVal, createdAt); err != nil {
				return err
			}
		}
	...
	// Check that the calculated average refresh time is two hours, based on the
	// automatic column statistic that was most recently updated (in this case,
	// column k, 5 hours ago).
	if err := checkAverageRefreshTime(2 * time.Hour); err != nil {
		t.Fatal(err)
	}
