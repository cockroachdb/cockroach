...
func TestMaybeRefreshStats(t *testing.T) {
	...
	refresher := MakeRefresher(executor, cache, time.Nanosecond /* asOfTime */)
	...
	refresher.maybeRefreshStats(ctx, s.Stopper(), descA.ID, 0 /* rowsAffected */, time.Nanosecond /* asOf */)
...
func TestAverageRefreshTime(t *testing.T) {
	...
	// is 0.
	refresher.maybeRefreshStats(ctx, s.Stopper(), tableID, 0 /* rowsAffected */, time.Nanosecond /* asOf */)
	if err := checkStatsCount(ctx, cache, tableID, 20 /* expected */); err != nil {
		t.Fatal(err)
	}
