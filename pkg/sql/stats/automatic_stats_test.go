...
func TestMaybeRefreshStats(t *testing.T) {
	...
	maybeRefreshStats(
		evalCtx, &asCtx, cache, executor, descA.ID, 0 /* rowsAffected */, 0, /* asOf */
	)
	...
	maybeRefreshStats(
		evalCtx, &asCtx, cache, executor, descA.ID, 10 /* rowsAffected */, time.Second, /* asOf */
	)
...
func TestAverageRefreshTime(t *testing.T) {
	...
	// is 0.
	maybeRefreshStats(
		evalCtx, &asCtx, cache, executor, tableID, 0 /* rowsAffected */, 0, /* asOf */
	)
	if err := checkStatsCount(ctx, cache, tableID, 20 /* expected */); err != nil {
		t.Fatal(err)
	}
	...
	if err := s.DB().Txn(ctx, func(ctx context.Context, txn *client.Txn) error {
		for i := 0; i < 10; i++ {
			...
			createdAt := tree.MakeDTimestamp(
				timeutil.Now().Add(time.Duration(-1*(i*2+5))*time.Hour), time.Hour,
			)
			if err := insertStat(txn, autoStatsName, columnIDsVal, createdAt); err != nil {
				return err
			}
