...
func checkStatsForTable(
	...
	statsList, err := sc.getTableStatsFromCache(ctx, tableID, nil, nil, nil, false, 0)
	if err != nil {
...
func TestCacheUserDefinedTypes(t *testing.T) {
	...
	stats, err := sc.GetTableStats(ctx, tbl, nil, false, 0)
	if err != nil {
		t.Fatal(err)
	}
	if len(stats) != 2 {
...
func TestCacheWait(t *testing.T) {
	...
	for run := 0; run < 10; run++ {
		...
		for n := 0; n < 10; n++ {
			wg.Add(1)
			go func() {
				stats, err := sc.getTableStatsFromCache(ctx, id, nil, nil, nil, false, 0)
				if err != nil {
					t.Error(err)
...
func TestCacheAutoRefresh(t *testing.T) {
	...
	expectNStats := func(n int) error {
		stats, err := sc.GetTableStats(ctx, tableDesc, nil, false, 0)
		if err != nil {
			t.Fatal(err)
		}
		if len(stats) != n {
