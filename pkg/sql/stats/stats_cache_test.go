...
func checkHistForTable(
	...
	if histogram, ok := sc.lookupHistogram(ctx, tableID, statisticID); ok {
		return errors.Errorf("lookup of missing key {table %d, statistic %d} returned: %s", tableID, statisticID, histogram)
	}
	...
	if !reflect.DeepEqual(histogram, expected) {
		return errors.Errorf("for lookup of table %d and statistic %d, expected stat %s, got %s", tableID, statisticID, expected, histogram)
	}
...
func TestTableStatisticsCache(t *testing.T) {
	...
	if _, ok := sc.lookupHistogram(ctx, key.TableID, key.StatisticID); !ok {
		t.Fatalf("for lookup of table %d and statistic %d, expected histogram %s", key.TableID, key.StatisticID, expectedHist[key])
	}
	...
	if histogram, ok := sc.lookupHistogram(ctx, key.TableID, key.StatisticID); ok {
		t.Fatalf("lookup of invalidated key {table %d, statistic %d} returned: %s", key.TableID, key.StatisticID, histogram)
	}
}
