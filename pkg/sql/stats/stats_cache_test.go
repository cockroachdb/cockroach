...
func checkStatsForTable(
	...
	statsList, err := sc.getTableStatsFromCache(ctx, tableID, false /* forecast */, nil /* udtCols */, nil /* typeResolver */)
	if err != nil {
...
func TestCacheWait(t *testing.T) {
	...
	for run := 0; run < 10; run++ {
		...
		for n := 0; n < 10; n++ {
			wg.Add(1)
			go func() {
				stats, err := sc.getTableStatsFromCache(ctx, id, false /* forecast */, nil /* udtCols */, nil /* typeResolver */)
				if err != nil {
					t.Error(err)
