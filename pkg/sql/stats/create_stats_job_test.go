...
func TestCreateStatsControlJob(t *testing.T) {
	...
	sqlDB.Exec(t, `INSERT INTO d.t SELECT generate_series(1,20000)`)
...
func TestCreateStatsLivenessWithRestart(t *testing.T) {
	...
	sqlDB.Exec(t, `INSERT INTO liveness.t SELECT generate_series(1,20000)`)
	...
	if partialProgress.Details.(*jobspb.Progress_CreateStats).CreateStats.Completed() == 1 {
	...
	if rescheduledProgress.Details.(*jobspb.Progress_CreateStats).CreateStats.Completed() != 1 {
