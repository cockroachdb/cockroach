...
// forecastColumnStatistics, makeQuantile, and quantile.toHistogram might need to
// be changed if we introduce a new histogram version.
const _ uint = 1 - uint(histVersion)

// A quantile function is one way of describing a probability distribution. It
// is defined as a function from p to v, over (p=0, p=1] where p is the
// probability that the value will be <= v. It is the inverse of the cumulative
// distribution function for the same probability distribution. See
// https://en.wikipedia.org/wiki/Quantile_function for additional information.
//
...
//   distribution functions or probability density functions. For example, we
//   can always take the definite integral of a quantile function from p=0 to
//   p=1.
//
...
func ForecastTableStatistics(
	ctx context.Context, observed []*TableStatistic,
...
// the given time, based on the observed statistics. Observed statistics must be
// sorted by CreatedAt, descending (latest observation first). Parts of the
// forecast with R² (goodness of fit) < minRequiredFit copy the equivalent part
// from the latest observed statistics instead of the prediction. A score is
// also returned, indicating how valuable we think the forecast is. If every
// part of the forecast is copied from the latest observed statistics instead of
// predicted, the score will be 0.
func forecastColumnStatistics(
	ctx context.Context, observed []*TableStatistic, at time.Time, n int, minRequiredFit float64,
	...
	if histData != nil {
		...
		if canMakeQuantile(colType) {
			...
			if len(quantiles) == n {
				q, r2 := quantileLinearRegression(createdAts, quantiles, forecastAt)
				log.VEventf(
					ctx, 2, "forecast for table %v columns %v quantile %v r2 %v",
					tableID, columnIDs, q, r2,
				)
				if r2 >= minRequiredFit {
					h, err := q.toHistogram(colType, rowCount-nullCount)
					if err != nil {
						log.VErrEventf(
		...
		// If we did not successfully predict a histogram, then we need to copy the
		// latest histogram so we can adjust it below.
		if !predictedHist {
			hist.buckets = append(hist.buckets, observed[0].nonNullHistogram().buckets...)
		}
	}

	// Enforce consistency between measures.
	if nullCount > rowCount {
		nullCount = rowCount
	}
	if nullCount > 0 && distinctCount < 1 {
		distinctCount = 1
	}
	...
	if histData != nil {
		...
		hist.adjustCounts(nil, nnRowCount, nnDistinctCount)
		...
		if err != nil {
			...
			// Revert back to the latest observed histogram.
...
// error if it cannot. The histogram should not contain a null bucket.
func makeQuantile(hist histogram, rowCount float64) (quantile, error) {
	...
	for i := range hist.buckets {
		...
		val, err := quantileValue(hist.buckets[i].UpperBound)
		if err != nil {
			return nil, err
		}
		if val < prevVal {
		...
		p += hist.buckets[i].NumRange / rowCount
		...
		p += hist.buckets[i].NumEq / rowCount
		if p > 1 {
			p = 1
		}
		q = append(q, quantilePoint{p: p, v: val})
...
func (q quantile) toHistogram(colType *types.T, rowCount float64) (histogram, error) {
	...
	hist.buckets = append(hist.buckets, cat.HistogramBucket{
		NumEq:         0,
		NumRange:      0,
		DistinctRange: 0,
	...
	for i := 1; i < len(q); i++ {
		...
		if cmp == 0 {
			...
			if math.IsNaN(numEq) || math.IsInf(numEq, 0) {
				return histogram{}, errors.AssertionFailedf("invalid histogram NumEq: %v", numEq)
			}
			if numEq < 1 && hist.buckets[len(hist.buckets)-1].NumRange+numEq >= 1 {
				...
				hist.buckets[len(hist.buckets)-1].NumRange -= 1 - numEq
				numEq = 1
			}
			hist.buckets[len(hist.buckets)-1].NumEq = numEq
			eqP = 0
			...
			if math.IsNaN(distinctRange) || math.IsInf(distinctRange, 0) {
				return histogram{}, errors.AssertionFailedf(
...
// We can only create quantiles for types that make sense converted to float64.
...
// quantileValue converts from a datum to a float suitable for use in a quantile
// function. This is a little different from tree.PerformCast. We allow some
// conversions that are not legal casts (e.g. TIME to FLOAT) and we error on NaN
// or infinite values because they will break our model.
func quantileValue(d tree.Datum) (float64, error) {
	switch v := d.(type) {
	case *tree.DInt:
...
func histogramValue(colType *types.T, val float64) (tree.Datum, error) {
	...
	case types.TimeFamily:
		...
		// here. If it happens here it will simply fail in quantile.toHistogram.
		// (Clamping doesn't have this problem.)
...
// points, which can be represented with the equation: v = mp + b. This function
// must not be called on a discontinuity (vertical line where p₁ = p₂).
...
func (q quantile) fix() quantile {
	...
	//
	// when calculating p for v = 100, we are adding together the length of these
	// intervals (the first interval has a length of 0).
	//
	//       +       +-------+
	//
	//   200 |   *
	...
	//       0  .25 .5  .75  1
	//
	// After recalculating p for each distinct v, the fixed quantile is:
	//
	//   {{0, 0}, {0.5, 100}, {1, 200}}
	//
	// Which notably has the same (distinct) v's and the same definite integral as
	// the original malformed quantile.
