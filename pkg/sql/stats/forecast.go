...
// forecastColumnStatistics, makeQuantile, and quantile.toHistogramData will
// need to be changed if we introduce a new histogram version.
...
func forecastColumnStatistics(
	...
	if histogram != nil {
		...
		if len(quantiles) == n {
			...
			if r2 >= minRequiredFit {
				hist, err := q.toHistogramData(colType, rowCount, distinctCount)
				if err != nil {
					log.VErrEventf(
