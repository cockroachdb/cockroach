...
func forecastColumnStatistics(
	...
	if observed[0].HistogramData != nil && observed[0].HistogramData.ColumnType != nil {
		...
		// Verify that the initial NULL bucket and the first non-NULL bucket both
		// have NumRange=0 and DistinctRange=0. (We must check this after calling
		// setHistogramBuckets to account for rounding.) See #93892.
		for _, bucket := range forecast.Histogram {
			if bucket.NumRange != 0 || bucket.DistinctRange != 0 {
				// Build a JSON representation of the first few buckets in each observed
				// histogram so that we can figure out what happened.
				...
				addStat := func(stat *TableStatistic) {
					...
					if err := jsonStat.SetHistogram(stat.HistogramData); err == nil &&
						...
						// Limit the histogram to the first few buckets.
					...
					for i := range jsonStat.HistogramBuckets {
						hash := util.CRC32([]byte(jsonStat.HistogramBuckets[i].UpperBound))
						jsonStat.HistogramBuckets[i].UpperBound = "_" + strconv.FormatInt(int64(hash), 10)
					}
