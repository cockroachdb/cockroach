...
// nondecreasing in both p and v. The first point must have p=0, and the last
...
var zeroQuantile = quantile{
	quantilePoint{p: 0, v: 0},
	quantilePoint{p: 1, v: 0},
}
...
func ForecastTableStatistics(
	...
	// Find the time of the latest statistics collection. If we produce a
	// forecast, it will be for each columnset in the latest collection.
	...
	// the most recent N statistics collections, where N >=
...
NLoop:
	...
	// passing the same minRequiredFit each time.
	at := latest.Add(avgStatTime(observedCol0[:n]))
	var totalScore float64
	forecasts := make([]*TableStatisticProto, len(forecastCols))
	for i := range forecastCols {
		observedCol := observedByCols[forecastCols[i]]
		forecast, score := forecastTableStatisticsForColumns(ctx, observedCol, at, n, minGoodnessOfFit)
...
// forecastTableStatisticsForColumns produces a statistics forecast for a
// columnset at the given time, based on the observed statistics. Parts of the
// forecast with R² (goodness of fit) < minRequiredFit use the equivalent part
...
func forecastTableStatisticsForColumns(
...
func scalarLinearRegression(x, y []float64, xₙ float64) (yₙ, r2 float64) {
	...
	// Assuming we have multiple x's and y's, n will never be zero.
	...
	// Assuming we have multiple distinct x's, Σxx will never be zero.
	β := Σxy / Σxx
	α := yM - β*xM
...
func quantileLinearRegression(x []float64, y []quantile, xₙ float64) (yₙ quantile, r2 float64) {
	...
	// different model than Verde and Irpino to accomodate scalar x's, but the
	...
	// Assuming we have multiple x's and y(t)'s, n will never be zero.
	...
	// Assuming we have multiple distinct x's, Σxx will never be zero. Note that
	// α̂(t) and β̂(t) could be malformed quantiles, which is ok. We'll fix all the
	// outputs ŷᵢ(t) to not be malformed.
	β := Σxy.scale(1 / Σxx)
	α := yM.sub(β.scale(xM))
...
func makeQuantile(histogram *HistogramData, rowCount float64) (quantile, error) {
	...
	case types.IntFamily, types.FloatFamily, types.DecimalFamily, types.DateFamily,
		types.TimeFamily, types.TimeTZFamily, types.TimestampFamily, types.TimestampTZFamily:
		break
	default:
...
func (q quantile) toHistogramData(
	...
	// The first bucket must always have NumRange = 0.
...
// This is a little different from tree.PerformCast. We allow some conversions
// that are not legal casts (e.g. TIME to FLOAT) and we error on NaN or
// infinite values because they will break our model.
func quantileValue(a *tree.DatumAlloc, colType *types.T, encoded []byte) (float64, error) {
	...
	case *tree.DDate:
		if !v.IsFinite() {
			...
			// We use PG epoch instead of Unix epoch so that we can easily truncate
			// when converting back.
			return float64(v.PGEpochDays()), nil
		}
	case *tree.DTime:
		return float64((*timeofday.TimeOfDay)(v).Microsecond()), nil
	case *tree.DTimeTZ:
...
// In general this is more permissive than tree.PerformCast. We allow some
// conversions that are not legal casts (e.g. FLOAT to TIME) and we prefer to
// truncate or cap values rather than error out the conversion (and thus the
// entire histogram).
func histogramValue(colType *types.T, val float64) (tree.Datum, error) {
	...
	case types.IntFamily:
		i := int64(math.Round(val))
		if colType.Width() == 16 {
			return tree.NewDInt(tree.DInt(int16(i))), nil
		} else if colType.Width() == 32 {
			return tree.NewDInt(tree.DInt(int32(i))), nil
		} else {
	...
	case types.DateFamily:
		days := int32(math.Round(val))
		return tree.NewDDate(pgdate.MakeDateFromPGEpochWithTruncation(days)), nil
	case types.TimeFamily:
		micros := int64(math.Round(val))
		t := timeofday.FromInt(micros)
	...
	case types.TimestampFamily:
		...
		roundTo := tree.TimeFamilyPrecisionToRoundDuration(colType.Precision())
		return tree.MakeDTimestampWithTruncation(t, roundTo), nil
	case types.TimestampTZFamily:
		micros := int64(math.Round(val))
		t := time.UnixMicro(micros).UTC()
		roundTo := tree.TimeFamilyPrecisionToRoundDuration(colType.Precision())
		return tree.MakeDTimestampTZWithTruncation(t, roundTo), nil
	default:
