...
func forecastColumnStatistics(
	...
	log.VEventf(ctx, 3, "forecasting statistics for table %v columns %v", tableID, columnIDs)
	...
	predict := func(name string, y, createdAts []float64) (float64, error) {
		yₙ, r2 := float64SimpleLinearRegression(createdAts, y, forecastAt)
		log.VEventf(
			ctx, 3, "forecast for table %v columns %v predicted %v %v R² %v",
	...
	if observed[0].HistogramData != nil {
		...
		if err != nil {
			...
			log.VEventf(ctx, 3, "could not predict histogram due to: %v", err)
