...
func TestForecastColumnStatistics(t *testing.T) {
	...
	testCases := []struct {
		...
		// Test consistency adjustments to predicted values.
		{
			observed: []*TableStatistic{
				{
					TableStatisticProto: TableStatisticProto{
	...
	for i, tc := range testCases {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			forecast, score := forecastColumnStatistics(
				ctx, tc.observed, tc.at, len(tc.observed), tc.minRequiredFit,
			)
			if !reflect.DeepEqual(forecast, tc.forecast) {
				t.Errorf("test case %d incorrect forecast %#v expected %#v", i, forecast, tc.forecast)
			}
			if score != tc.score {
...
func TestQuantileToHistogram(t *testing.T) {
	...
	for i, tc := range testCases {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			h, err := tc.q.toHistogram(types.Float, tc.rowCount)
			if err != nil {
				if !tc.err {
