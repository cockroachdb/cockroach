	...
	for i, tc := range testCases {
		t.Run(fmt.Sprintf("%d", i), func(t *testing.T) {
			...
			for i, b := range h.Buckets {
				...
				if b.EqRows != exp.numEq {
					t.Errorf("bucket %d: incorrect EqRows %d, expected %d", i, b.EqRows, exp.numEq)
				}
				if b.RangeRows != exp.numLess {
					t.Errorf("bucket %d: incorrect RangeRows %d, expected %d", i, b.RangeRows, exp.numLess)
				}
			}
		})
	}
