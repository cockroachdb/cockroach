...
func TestEquiDepthHistogram(t *testing.T) {
	testCases := []struct {
		...
		{
			// Same test as the previous one, but using 2 MCVs. As a result, the
			// bucket boundaries are shifted and we have an additional bucket
			// output.
			samples: []int64{
				1, 2, 2, 3, 4, 5, 6, 7, 8, 8, 9, 10,
			},
		...
		{
			// When we add 2 MCVs, the output doesn't change since the MCVs already
	...
	for i, tc := range testCases {
		t.Run(fmt.Sprintf("%d", i), func(t *testing.T) {
			...
			MaxHistogramMCVs.Override(ctx, &st.SV, int64(tc.maxMCVs))
