...
var defaultCleanupInterval = 2 * time.Hour
	...
	for _, dbID := range dbIDs {
		...
		for _, scName := range schemaNames {
			...
			if isTempSchema && !activeSessions[sessionID.Uint128] {
				sd := sessionDataToAccessTempSchemaObjects(sessionID)
				ie := r.job.MakeSessionBoundInternalExecutor(ctx, &sd)
				if err := cleanupSessionTempObjects(ctx, r.job.DB(), ie, sessionID); err != nil {
					return err
				}
			}
		}
...
func checkRunningTempTablesJob(ctx context.Context, job *jobs.Job, p *planner) error {
	if err := checkRunningJobsImpl(ctx, job, p, jobspb.TypeDeleteTempTables); err != nil {
