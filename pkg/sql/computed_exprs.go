...
func ProcessComputedColumns(
...
) ([]sqlbase.ColumnDescriptor, []tree.TypedExpr, error) {
	...
	for _, col := range cols {
		...
		expr, _, _, err := resolveNames(exprs[comExprIdx], multiSourceInfo{sourceInfo}, ivarHelper, evalCtx.SessionData.SearchPath)
		if err != nil {
			return nil, nil, err
		}
