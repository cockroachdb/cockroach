	...
	for idx, tc := range testCases {
		...
		for i := 0; rows.Next(); i++ {
			...
			if id != tc.expectedOutput[0] || k != tc.expectedOutput[1] || r != tc.expectedOutput[2] || a != tc.expectedOutput[3] {
				t.Fatalf("%d: expected %q, %q, %q, %q, got %q, %q, %q, %q", idx, tc.expectedOutput[0], tc.expectedOutput[1],
					tc.expectedOutput[2], tc.expectedOutput[3], id, k, r, a)
			}
		}
		rows.Close()
