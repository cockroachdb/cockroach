...
func (ef *execFactory) ConstructInsert(
	...
	tabDesc := table.(*optTable).desc
	colDescs := make([]sqlbase.ColumnDescriptor, len(tabCols))
	for i, c := range tabCols {
		// Ordinals refer to positions of regular columns, plus any mutation
		// columns (as if they were appended).
		if int(c) < table.ColumnCount() {
			colDescs[i] = *table.Column(int(c)).(*sqlbase.ColumnDescriptor)
		} else {
			colDescs[i] = *table.MutationColumn(int(c) - i).(*sqlbase.ColumnDescriptor)
		}
	}
