...
func (ef *execFactory) ConstructInvertedFilterer(
	...
	// Get descriptors for the primary key columns.
	pkColIDs := tableDesc.Indexes[0].ColumnIDs
	tableCols := tableDesc.ColumnsWithMutations(false /* mutations */)
	var colDescs []sqlbase.ColumnDescriptor
	for _, pkColID := range pkColIDs {
		for _, tableCol := range tableCols {
			if pkColID == tableCol.ID {
				colDescs = append(colDescs, tableCol)
				break
			}
		}
	}
	f := &invertedFiltererNode{
		...
		resultColumns: sqlbase.ResultColumnsFromColDescs(tableDesc.GetID(), colDescs),
	}
	return f, nil
}
