...
func (ef *execFactory) constructVirtualScan(
	...
	indexName := ""
	if indexDesc != nil {
		indexName = fmt.Sprintf("@%s", indexDesc.Name)
	}
	var n exec.Node
	n = &delayedNode{
		name:            fmt.Sprintf("%s%s", table.Name(), indexName),
		columns:         columns,
	...
	if needed.Contains(0) || (indexConstraint != nil && indexConstraint.Columns.ColSet().Contains(0)) {
