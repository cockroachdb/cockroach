...
func (ef *execFactory) constructScanForZigzag(
	...
	// Convert the equality column table ordinals to scan column ordinals.
	// makeScanColumnsConfig sets up the scan columns in increasing order.
	eqColOrdinals = make([]int, len(eqCols))
	for i, eqOrd := range eqCols {
		j := 0
		for ord, ok := cols.Next(0); ; ord, ok = cols.Next(ord + 1) {
			if !ok {
				return nil, nil, errors.AssertionFailedf("equality column not among scanned columns")
			}
			if ord == int(eqOrd) {
				eqColOrdinals[i] = j
				break
