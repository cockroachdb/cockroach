...
func TestLeaseAcquireAndReleaseConcurrenctly(t *testing.T) {
	...
	// expiration. This prevents a flake if two leases (randomly) happen to have
	// the same expiration.
	savedLeaseJitterFraction := LeaseJitterFraction
	defer func() {
		LeaseJitterFraction = savedLeaseJitterFraction
	}()

	LeaseJitterFraction = 0

	// acquireBlockAndRelease calls Acquire and then releases the lease.
	acquireBlockAndRelease := func(ctx context.Context, m *LeaseManager, acquireChan chan Result, releaseChan chan Result) {
	...
	// lease.
	acquireFreshestAndRelease := func(ctx context.Context, m *LeaseManager, acquireChan chan Result, releaseChan chan Result) {
	...
	testCases := []struct {
		...
		acquireCount int32
		// The routines being called during the test.
		routineFunc1 func(ctx context.Context, m *LeaseManager, acquireChan chan Result, _ chan Result)
		routineFunc2 func(ctx context.Context, m *LeaseManager, acquireChan chan Result, _ chan Result)
		...
		// This test case checks if the race condition occurs where thread 2 calls
		// tableState.acquire().
		{
			name:                        "CallAcquireConcurrently",
			acquireCount:                2,
			routineFunc1:                acquireBlockAndRelease,
			routineFunc2:                acquireBlockAndRelease,
		...
		// tableState.acquireFreshestFromStore().
		{
			...
			acquireCount:                1,
			routineFunc1:                acquireBlockAndRelease,
			routineFunc2:                acquireFreshestAndRelease,
	...
	for _, test := range testCases {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			preblock.Add(2)
			go test.routineFunc1(ctx, leaseManager, acquireResultChan, releaseResultChan)
			go test.routineFunc2(ctx, leaseManager, acquireResultChan, releaseResultChan)
