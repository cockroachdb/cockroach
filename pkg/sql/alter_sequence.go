...
func (n *alterSequenceNode) startExec(params runParams) error {
	...
	if hasRestart {
		...
		if sequenceVal == oldStart-oldIncrement {
			...
			if currentVal, err := params.extendedEvalCtx.EvalContext.Sequence.GetLatestValueInSessionForSequenceByID(
				params.extendedEvalCtx.EvalContext.Ctx(), int64(desc.GetID())); err != nil {
				return err
			} else {
				sequenceVal = currentVal
			}
			if sequenceVal > opts.MaxValue {
				return pgerror.Newf(
					pgcode.InvalidParameterValue,
					"MAXVALUE (%d) cannot be made to be less than the current value (%d) ", opts.MaxValue, sequenceVal)
			}
			if sequenceVal < opts.MinValue {
				return pgerror.Newf(
					pgcode.InvalidParameterValue,
					"MINVALUE (%d) cannot be made to be exceed than the current value (%d) ", opts.MinValue, sequenceVal)
			}
		}
	}
	...
	// It is necessary to clear out the cache and update the last value.
