...
func (n *alterSequenceNode) startExec(params runParams) error {
	...
	if hasRestart {
		...
		if sequenceVal == oldStart-oldIncrement {
			...
			currentVal, err := params.extendedEvalCtx.EvalContext.Sequence.GetLatestValueInSessionForSequenceByID(
				params.extendedEvalCtx.EvalContext.Ctx(), int64(desc.GetID()))
			if err != nil {
				return err
			}
