...
func expandConstraint(
	...
	if vars, ok := c.Left.(*parser.Tuple); ok {
		...
		case parser.LT:
			// (a,b,c) < (x,y,z)  -> [a<=x]
			a = append(a,
				&parser.ComparisonExpr{Operator: parser.LE, Left: vars.Exprs[0], Right: vals[0]})
		case parser.GT:
			// (a,b,c) > (x,y,z)  -> [a>=x]
			a = append(a,
				&parser.ComparisonExpr{Operator: parser.GE, Left: vars.Exprs[0], Right: vals[0]})
		case parser.LE, parser.GE:
			// (a,b,c) <= (x,y,z)  -> [a<=x]
			a = append(a,
				&parser.ComparisonExpr{Operator: c.Operator, Left: vars.Exprs[0], Right: vals[0]})
...
// applyConstraint tries to simplify the expression t on the right
...
func applyConstraintFlat(
	...
	// +infinity. 0 represents the smaller of cdatum and datum (both if
	// they are equal), 10 represents the larger (unused if they are
	// equal).
	// First set tx = 0 if datum <= cdatum, or 10 otherwise.
	// Then e.g. if t.Operator=GE, t corresponds to [tx, +100]. If it is
	// LE, [-100, tx]. If it is GT, [tx+1, +100].
	// The other is similar, we just set cx = 0 if datum >= cdatum or 10
	// otherwise.
	// Then it's simply a matter of detecting if the two intervals are
	// disjoint (return False) or if c is inside t (return True), which
	// are very simple conditions.
	// The actual datums are not important, just the relationship
	// between them (cmp); so we represent them by two fixed values (0
	// and 10). Exclusivity is implemented by just adding or subtracting
	// 1 from the integer (which is why I chose 0 and 10 and not 0 and
	// 1). Possible values for integers are from the set: -100, -1, 0,
	// +1, 9, 10, 11, 100.
