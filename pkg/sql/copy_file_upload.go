...
func newFileUploadMachine(
	...
	f.writeError = make(chan copyError)
	go func() {
		f.writeError <- copyError{err: localStorage.WriteFile(opts[copyOptionDest], pr)}
		close(f.writeError)
	}()
	f.writeToFile = pw
...
func CopyInFileStmt(destination, schema, table string, columns ...string) string {
	return fmt.Sprintf(
...
func (f *fileUploadMachine) run(ctx context.Context) error {
	...
	if err = f.writeToFile.Close(); err != nil {
		return err
	}
	return (<-f.writeError).err
}
