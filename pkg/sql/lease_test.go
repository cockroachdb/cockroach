...
func TestTxnObeysLeaseExpiration(t *testing.T) {
	...
	csql.MinLeaseDuration = 50 * time.Millisecond
	csql.LeaseDuration = 2 * csql.MinLeaseDuration
	...
	defer s.Stopper().Stop()
	// TODO(vivek): remove this sleep in this PR. Sleep because the server
	// clock is set forward when we configure the MaxOffset.
	time.Sleep(params.MaxOffset)
	clock := s.Clock()
	t.Logf("time: server %s vs actual %s, offset %d",
		clock.Now(), hlc.Timestamp{WallTime: timeutil.Now().UnixNano()}, clock.MaxOffset())
