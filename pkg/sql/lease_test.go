...
`); err != nil {
	...
	// Write transaction uses old table descriptor and inserts into the index.
	tests.CheckKeyCount(t, kvDB, tableDesc.TableSpan(), 4)
	...
	gcThresholdErr := `batch timestamp \d+.\d+,\d+ must be after GC threshold \d+.\d+,\d+`
	if _, err := txUpdate.Exec(`UPDATE t.kv SET v = 'c' WHERE k = 'a';`); !testutils.IsError(err, gcThresholdErr) {
		t.Fatalf("err = %v", err)
	}

	if err := txUpdate.Rollback(); err != nil {
		t.Fatal(err)
	}
}
