...
CREATE DATABASE d;
CREATE TABLE d.kv (k CHAR PRIMARY KEY, v CHAR);
CREATE TABLE d.foo (k CHAR PRIMARY KEY, v CHAR);
`); err != nil {
		t.Fatal(err)
	}
	...
	for i := 0; i < 2; i++ {
		...
		// acquire a timestamp for the txn which is greater than
		// the creation timestamp for table d.kv.
		if _, err := tx.Exec(`INSERT INTO d.foo (k,v) VALUES ('a', 'b');`); err != nil {
			t.Fatal(err)
		}

		if i == 0 {
			// upgrade table kv the first time around.
...
`); !testutils.IsError(err, "encountered table with future timestamp") {
				t.Fatalf("err = %v", err)
			}
			...
			if _, err := tx.Exec(`INSERT INTO d.kv (k,v) VALUES ('c', 'd');`); err != nil {
				t.Fatal(err)
			}
	...
	// Ensure that a transaction started before a table was created and
	// which touches the table fails.
	for i := 0; i < 2; i++ {
		tx, err := sqlDB.Begin()
		if err != nil {
			t.Fatal(err)
		...
		// acquire a timestamp for the tx.
		if _, err := tx.Exec(`INSERT INTO d.kv (k,v) VALUES ('e', 'f');`); err != nil {
			t.Fatal(err)
		}

		if i == 0 {
			// create table bar at a future timestamp.
			if _, err := sqlDB.Exec(`CREATE TABLE d.bar (k CHAR PRIMARY KEY, v CHAR);`); err != nil {
				t.Fatal(err)
			}

			if _, err := tx.Exec(`
INSERT INTO d.bar (k,v) VALUES ('a', 'b');
`); !testutils.IsError(err, "encountered table with future timestamp") {
				t.Fatalf("err = %v", err)
			}
			...
			if _, err := tx.Exec(`INSERT INTO d.bar (k,v) VALUES ('a', 'b');`); err != nil {
				t.Fatal(err)
			}
