...
func (n *alterTableSetSchemaNode) startExec(params runParams) error {
	...
	// We do not check for privileges on the public schema.
	if desiredSchemaID != keys.PublicSchemaID {
	...
	// Update the descriptor to the schema. We leave the mapping of the old key
	// so that the name is not used in the old schema until the schema changer
	// has made sure it's not in use anymore.
	b := &kv.Batch{}
	...
	err = catalogkv.WriteDescToBatch(ctx, p.extendedEvalCtx.Tracing.KVTracingEnabled(),
		params.p.EvalContext().Settings, b, p.ExecCfg().Codec, tableDesc.ID, tableDesc)
	if err != nil {
		return err
	}
