...
// SetupAllNodesPlanningWithOracle but with a txn param that if non-nil allows
// the flow to plan on followers.
func (dsp *DistSQLPlanner) SetupAllNodesPlanningWithOracleAndFollowers(
	...
	if dsp.codec.ForSystemTenant() {
		return dsp.setupAllNodesPlanningSystem(ctx, evalCtx, execCfg, oracle, localityFilter, txn)
	}
	return dsp.setupAllNodesPlanningTenant(ctx, evalCtx, execCfg, oracle, localityFilter, txn)
}
