...
func (a *TraceAnalyzer) Reset(
	...
	// Initialize/clear maps.
	if a.processorStats == nil {
		a.processorStats = make(map[execinfrapb.ProcessorID]*processorStats)
	}
	if a.streamStats == nil {
		a.streamStats = make(map[execinfrapb.StreamID]*streamStats)
	}
	for k := range a.processorStats {
		delete(a.processorStats, k)
	}
	for k := range a.streamStats {
		delete(a.streamStats, k)
	}
