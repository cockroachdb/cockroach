...
func zoneConfigForMultiRegionDatabase(
	...
	voterConstraints, err := regionutils.SynthesizeVoterConstraints(regionConfig.PrimaryRegion(), regionConfig)
	if err != nil {
		return zonepb.ZoneConfig{}, err
	}

	leasePreferences := regionutils.SynthesizeLeasePreferences(regionConfig.PrimaryRegion())
...
func zoneConfigForMultiRegionTable(
	...
	case *catpb.LocalityConfig_RegionalByTable_:
		...
		if regionConfig.IsMemberOfExplicitSuperRegion(affinityRegion) {
			err := regionutils.AddConstraintsForSuperRegion(&zc, regionConfig, affinityRegion)
			if err != nil {
				return zonepb.ZoneConfig{}, err
			}
		...
		voterConstraints, err := regionutils.SynthesizeVoterConstraints(affinityRegion, regionConfig)
		if err != nil {
			return zonepb.ZoneConfig{}, err
		}
		zc.VoterConstraints = voterConstraints
		...
		leasePreferences := regionutils.SynthesizeLeasePreferences(affinityRegion)
		zc.LeasePreferences = leasePreferences
		zc.InheritedLeasePreferences = false
