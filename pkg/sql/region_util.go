...
func (p *planner) updateZoneConfigsForLocalityRegionalByTable(
	...
	for _, schema := range schemas {
		...
		for i := range tbNames {
			...
			switch l := tbDesc.LocalityConfig.Locality.(type) {
			case *descpb.TableDescriptor_LocalityConfig_RegionalByTable_:
				if l.RegionalByTable.Region == nil {
					// REGIONAL BY TABLE tables with no region set will leverage the database level
					// zone configuration. Nothing to update here.
					continue
				}
