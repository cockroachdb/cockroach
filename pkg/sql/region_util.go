...
func (p *planner) updateZoneConfigsForAllTables(ctx context.Context, desc *dbdesc.Mutable) error {
	return p.forEachTableWithLocalityConfigInDatabase(
		...
		func(ctx context.Context, schema string, tbName tree.TableName, tbDesc *tabledesc.Mutable) error {
			regionConfig, err := SynthesizeRegionConfig(ctx, p.txn, &desc.Immutable, p.Descriptors())
			if err != nil {
				return err
			}
...
func (p *planner) maybeInitializeMultiRegionDatabase(
	ctx context.Context,
	desc *dbdesc.Mutable,
	initializationConfig *multiregion.InitializationRegionConfig,
) error {
	...
	regionLabels := make(tree.EnumValueList, 0, len(initializationConfig.Regions()))
	for _, regionName := range initializationConfig.Regions() {
	...
	if err := ApplyZoneConfigFromDatabaseRegionConfig(
		ctx,
		desc.ID,
		&initializationConfig.RegionConfig,
		p.txn,
		p.execCfg); err != nil {
		return err
	}
...
func (p *planner) CurrentDatabaseRegionConfig(
	...
	return SynthesizeRegionConfig(ctx, p.txn, dbDesc, p.Descriptors())
}
...
func (p *planner) validateZoneConfigForMultiRegionDatabaseWasNotModifiedByUser(
	ctx context.Context, dbDesc *dbdesc.Immutable, force bool,
) error {
