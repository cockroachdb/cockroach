...
// super region.
// Requires: regionConfig.IsMemberOfExplicitSuperRegion(affinityRegion).
func addConstraintsForSuperRegion(
	...
	regions := regionConfig.GetSuperRegionRegionsForRegion(affinityRegion)
...
func zoneConfigForMultiRegionPartition(
	...
	if regionConfig.IsMemberOfExplicitSuperRegion(partitionRegion) {
		addConstraintsForSuperRegion(&zc, regionConfig, partitionRegion)
	} else if !regionConfig.RegionalInTablesInheritConstraints(partitionRegion) {
		zc.NumReplicas = &numReplicas
...
func zoneConfigForMultiRegionTable(
	...
	case *catpb.LocalityConfig_Global_:
		...
		if !regionConfig.GlobalTablesInheritConstraints() {
	...
	case *catpb.LocalityConfig_RegionalByTable_:
		...
		if regionConfig.IsMemberOfExplicitSuperRegion(affinityRegion) {
			addConstraintsForSuperRegion(&zc, regionConfig, affinityRegion)
		} else if !regionConfig.RegionalInTablesInheritConstraints(affinityRegion) {
			zc.NumReplicas = &numReplicas
