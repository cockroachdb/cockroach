...
func makeRequireZoneConstraintForRegion(r descpb.Region) zonepb.Constraint {
...
func genZoneConfigFromRegionConfigForDatabase(
	...
	for _, region := range regionConfig.Regions {
		conjunctions = append(
			...
			zonepb.ConstraintsConjunction{
				NumReplicas: 1,
				Constraints: []zonepb.Constraint{makeRequireZoneConstraintForRegion(region)},
			},
		)
	}
	return &zonepb.ZoneConfig{
		...
		LeasePreferences: []zonepb.LeasePreference{
			{Constraints: []zonepb.Constraint{makeRequireZoneConstraintForRegion(regionConfig.PrimaryRegion)}},
		},
