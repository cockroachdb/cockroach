...
func (p *planner) CheckZoneConfigChangePermittedForMultiRegion(
	...
	// The request is to discard the zone configuartion. Error in all discard
	// cases.
	if options == nil {
		// User is trying to update a zone config value that's protected for
		// multi-region databases. Return the constructed error.
		err := errors.WithDetail(errors.Newf("attempting to discard the zone configuration of a multi-region entity"),
