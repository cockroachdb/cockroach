...
// maxToleratedFailures returns the maximum number of individual failures that
// can be tolerated, among `numVoters` voting replicas, before a given range is
// unavailable.
func maxToleratedFailures(numVoters int32) int32 {
...
func getNumVotersAndNumReplicas(
	...
	case descpb.SurvivalGoal_REGION_FAILURE:
		...
		numReplicas = maxToleratedFailures(numVotersForRegionSurvival) + (numRegions - 1)
		...
		return []zonepb.ConstraintsConjunction{
			{
				...
				// failure tolerance due to it's diversity heuristic (see
		...
		return []zonepb.ConstraintsConjunction{
			{
				...
				// "region" failure tolerance), each with 3 AZs, we'd expect to see a
				...
				// +--------------------+   +-------------------+    +--------------------+
				//
				NumReplicas: maxToleratedFailures(numVoters),
				Constraints: []zonepb.Constraint{makeRequiredConstraintForRegion(region)},
			},
		}, nil
	default:
