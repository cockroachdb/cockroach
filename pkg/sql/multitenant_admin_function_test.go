...
func verifyResults(t *testing.T, rows *gosql.Rows, expectedResults [][]string) {
	i := 0
	columns, err := rows.Columns()
	require.NoError(t, err)
	actualNumColumns := len(columns)
	for ; rows.Next(); i++ {
		require.Greaterf(t, len(expectedResults), i, "row=%d", i)
		expectedResult := expectedResults[i]
		require.Equal(t, len(expectedResult), actualNumColumns, "row=%d", i)
		actualResult := make([]interface{}, actualNumColumns)
		for i := range actualResult {
			actualResult[i] = new(gosql.NullString)
		}
		err := rows.Scan(actualResult...)
		require.NoErrorf(t, err, "row=%d", i)
		for j := 0; j < actualNumColumns; j++ {
			actualResult := actualResult[j].(*gosql.NullString)
			require.Containsf(t, actualResult.String, expectedResult[j], "row=%d col=%d", i, j)
		}
	}
	require.NoError(t, rows.Err())
	require.Equal(t, len(expectedResults), i)
}
...
func TestMultiTenantAdminFunction(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			execQueries := func(db *gosql.DB, expectedResults [][]string, expectedErrorMessage string) {
				setups := tc.setups
				setup := tc.setup
				if setup != "" {
					setups = []string{setup}
				...
				if expectedErrorMessage == "" {
					...
					if len(expectedResults) > 0 {
						verifyResults(t, rows, expectedResults)
					}
				} else {
					require.Error(t, err)
			...
			func() {
				...
				execQueries(db, tc.expectedSystemResult, tc.expectedSystemErrorMessage)
			}()
			...
			func() {
				...
				execQueries(db, tc.expectedSecondaryResult, tc.expectedSecondaryErrorMessage)
			}()
		})
	}
}
...
func TestTruncateTable(t *testing.T) {
	...
	ctx := context.Background()
	execQueries := func(db *gosql.DB, expectedResults [][]string) {
