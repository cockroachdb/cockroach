...
type testCase struct {
	...
	queryClusterSetting *settings.BoolSetting
	// Used for tests that have a capability prereq (eq SPLIT AT is required for UNSPLIT AT).
	setupCapability string
...
func (tc testCase) runTest(
	...
	waitForTenantCapabilities := func() {}
	setCapabilities := func(
		...
		if len(capabilities) > 0 {
			...
			waitForTenantCapabilities = func() {
				...
				testCluster.WaitForTenantCapabilities(tenantID, capabilities...)
			}
		}
	}
