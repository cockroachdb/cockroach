...
var errInvalidVirtualDefEntry = pgerror.NewError(pgerror.CodeInternalError, "programming error: virtualDefEntry.virtualDef must be either a virtualSchemaTable or a virtualSchemaView")
...
func (e virtualDefEntry) getPlanInfo() (sqlbase.ResultColumns, virtualTableConstructor) {
	...
	constructor := func(ctx context.Context, p *planner, dbName string) (planNode, error) {
		...
		case virtualSchemaView:
			source, err := p.getViewPlan(ctx, &p.tableName, e.desc)
			if err != nil {
				return nil, err
			}
