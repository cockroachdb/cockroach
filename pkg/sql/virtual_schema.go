...
func (e virtualDefEntry) makeConstrainedRowsGenerator(
	...
	return func(pusher rowPusher) error {
		...
		for currentConstraint = 0; currentConstraint < idxConstraint.Spans.Count(); currentConstraint++ {
		...
		if currentConstraint == idxConstraint.Spans.Count() {
		...
		nSpans := idxConstraint.Spans.Count() - currentConstraint
		newConstraint.Spans.Alloc(nSpans)
		for ; currentConstraint < idxConstraint.Spans.Count(); currentConstraint++ {
			newConstraint.Spans.Append(idxConstraint.Spans.Get(currentConstraint))
		}
