...
func (vs *VirtualSchemaHolder) GetVirtualObjectByID(id descpb.ID) (catalog.VirtualObject, bool) {
	...
	case id == catconstants.CrdbInternalRangesNoLeasesTableID && deprecatedshowranges.EnableDeprecatedBehavior(context.TODO(), vs.st, nil):
		entry = vs.schemasByID[catconstants.CrdbInternalID].extraRangesNoLeasesDef
	}
...
type virtualSchemaEntry struct {
	...
	// Remove in v23.2.
	st                     *cluster.Settings
	extraRangesDef         *virtualDefEntry
	extraRangesNoLeasesDef *virtualDefEntry
}
