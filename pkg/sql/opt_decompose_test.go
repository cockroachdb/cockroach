...
func TestSplitOrExpr(t *testing.T) {
	...
	for _, d := range testData {
		t.Run(d.expr+"~"+d.expected, func(t *testing.T) {
			...
			exprs := splitOrExpr(&p.extendedEvalCtx.EvalContext, expr, nil /* exprs */)
...
func TestSplitAndExpr(t *testing.T) {
	...
	for _, d := range testData {
		t.Run(d.expr+"~"+d.expected, func(t *testing.T) {
			...
			sel := makeSelectNode(t, p)
			expr := parseAndNormalizeExpr(t, p, d.expr, sel)
			exprs := splitAndExpr(&p.extendedEvalCtx.EvalContext, expr, nil /* exprs */)
			if s := exprs.String(); d.expected != s {
				t.Errorf("%s: expected %s, but found %s", d.expr, d.expected, s)
			}
		})
...
func TestSimplifyExpr(t *testing.T) {
	...
	for _, d := range testData {
		t.Run(d.expr+"~"+d.expected, func(t *testing.T) {
			...
			p.extendedEvalCtx.IVarHelper = &sel.ivarHelper
			expr := parseAndNormalizeExpr(t, p, d.expr, sel)
			expr, equiv := SimplifyExpr(&p.extendedEvalCtx.EvalContext, expr)
...
func TestSimplifyNotExpr(t *testing.T) {
	...
	for _, d := range testData {
		t.Run(d.expr+"~"+d.expected, func(t *testing.T) {
			...
			expr2, equiv := SimplifyExpr(&p.extendedEvalCtx.EvalContext, expr1)
			...
			if d.checkEquiv {
				if err := checkEquivExpr(&p.extendedEvalCtx.EvalContext, expr1, expr2, sel); err != nil {
					t.Error(err)
				}
			}
		})
...
func TestSimplifyAndExprCheck(t *testing.T) {
	...
	for _, d := range testData {
		t.Run(d.expr+"~"+d.expected, func(t *testing.T) {
			...
			err := checkEquivExpr(&p.extendedEvalCtx.EvalContext, expr1, expr2, sel)
			...
			if _, ok := expr2.(*tree.AndExpr); !ok {
				...
				expr3, equiv := SimplifyExpr(&p.extendedEvalCtx.EvalContext, andExpr)
...
func TestSimplifyOrExpr(t *testing.T) {
	...
	for _, d := range testData {
		t.Run(d.expr+"~"+d.expected, func(t *testing.T) {
			...
			sel := makeSelectNode(t, p)
			expr1 := parseAndNormalizeExpr(t, p, d.expr, sel)
			expr2, _ := SimplifyExpr(&p.extendedEvalCtx.EvalContext, expr1)
			if s := expr2.String(); d.expected != s {
				t.Errorf("%s: expected %s, but found %s", d.expr, d.expected, s)
			}
		})
	...
	for _, d := range testData {
		t.Run(d.expr+"~"+d.expected, func(t *testing.T) {
			...
			if _, ok := expr2.(*tree.OrExpr); !ok {
				...
				expr3, equiv := SimplifyExpr(&p.extendedEvalCtx.EvalContext, orExpr)
				if s := expr3.String(); d.expected != s {
					t.Errorf("%s: expected %s, but found %s", expr1, d.expected, s)
				}
				if !equiv {
