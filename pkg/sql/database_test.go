...
// TODO(ajwerner): Rework the layering so that UncachedPhysicalAccessor lives
// below the database and table collection at the level of schemakv and then
...
func TestDatabaseAccessors(t *testing.T) {
	...
	if err := kvDB.Txn(context.TODO(), func(ctx context.Context, txn *kv.Txn) error {
		...
		databaseCache := database.NewDatabaseCache(keys.SystemSQLCodec, config.NewSystemConfig(zonepb.DefaultZoneConfigRef()), catalogkv.UncachedPhysicalAccessor{})
