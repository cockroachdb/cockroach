...
func CreateRoleForProvisioning(
	...
	return execCfg.InternalDB.DescsTxn(ctx, func(ctx context.Context, txn descs.Txn) error {
		if len(provisioningSource) > 0 {
			userProvisioningStmt := fmt.Sprintf("CREATE USER IF NOT EXISTS %s WITH PROVISIONING_SOURCE %q", user.SQLIdentifier(), provisioningSource)
			if _, err := txn.Exec(
