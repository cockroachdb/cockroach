...
func (p *planner) checkPasswordAndGetHash(
	...
	if security.AutoDetectPasswordHashes.Get(&st.SV) {
		...
		var issueLink int
		isPreHashed, schemeSupported, issueLink, schemeName, hashedPassword, err = security.CheckPasswordHashValidity(ctx, []byte(password))
		if err != nil {
			return hashedPassword, pgerror.WithCandidateCode(err, pgcode.Syntax)
		}
		if isPreHashed {
			if !schemeSupported {
				return hashedPassword, unimplemented.NewWithIssueDetailf(issueLink, schemeName, "the password hash scheme %q is not supported", schemeName)
			}
	...
	method := security.HashMethod(security.PasswordHashMethod.Get(&st.SV))
	hashedPassword, err = security.HashPassword(ctx, password, method)
	if err != nil {
