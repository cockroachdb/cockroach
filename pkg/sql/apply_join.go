...
func (a *applyJoinNode) Next(params runParams) (bool, error) {
	...
	for {
		for a.run.curRightRow < a.run.rightRows.Len() {
			...
			if predMatched {
				...
				if a.joinType == sqlbase.JoinType_LEFT_ANTI {
					...
					// with this left row.
					break
				} else if a.joinType == sqlbase.JoinType_LEFT_SEMI {
		...
		// We're out of right side rows. Clear them.
		a.run.rightRows.Clear(params.ctx)

		// If we have a row at all, make sure to do any anti or outer join emitting
		// if we need to.
		if a.run.leftRow != nil && !a.run.leftRowFoundAMatch {
