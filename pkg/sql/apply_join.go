...
func runPlanInsidePlan(
	...
	if sqlStatsBuilder != nil {
		planCtx.collectExecStats = true
		planCtx.saveFlows = getDefaultSaveFlowsFunc(ctx, &plannerCopy, planComponentTypeSubquery)
		defer func() {
			...
			queryLevelStats, err := execstats.GetQueryLevelStats(recording,
				false, flowsMetadata,
			)
			if err == nil {
