...
func (p *planner) AlterPrimaryKey(
	...
	shouldRewriteIndex := func(idx *descpb.IndexDescriptor) bool {
		...
		return !idx.Unique || idx.Type == descpb.IndexDescriptor_INVERTED
	}
	var indexesToRewrite []*descpb.IndexDescriptor
	for _, idx := range tableDesc.PublicNonPrimaryIndexes() {
		if idx.GetID() != newPrimaryIndexDesc.ID && shouldRewriteIndex(idx.IndexDesc()) {
	...
	for _, idx := range indexesToRewrite {
		...
		// TODO(#multiregion): decide what happens if any REGIONAL BY ROW/GLOBAL/
		//                     normal tables have PARTITION BY statements.
		// Detect partitioning if PARTITION BY ALL.
		if partitionAllBy != nil && isNewPartitionAllBy {
