...
func (p *planner) shouldCreateIndexes(
	...
	if alterPKNode.Interleave != nil {
		...
		if oldPK.IndexDesc().Interleave.Ancestors[len(oldPK.IndexDesc().Interleave.Ancestors)-1].TableID !=
			parentTable.GetID() {
			return true, nil
		}
		if oldPK.IndexDesc().Interleave.Ancestors[len(oldPK.IndexDesc().Interleave.Ancestors)-1].IndexID !=
	...
	if alterPrimaryKeyLocalitySwap != nil {
		...
		if alterPrimaryKeyLocalitySwap.localityConfigSwap.NewRegionalByRowColumnID != nil &&
			*alterPrimaryKeyLocalitySwap.localityConfigSwap.NewRegionalByRowColumnID != oldPK.IndexDesc().ColumnIDs[0] {
			return true, nil
		}
	}
	return false, nil
