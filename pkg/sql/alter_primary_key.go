...
type alterPrimaryKeyLocalitySwap struct {
	...
	// AllowMutationOnSameTxnIdx is the index of the mutation to allow exist on
	// the same transaction as an ALTER PRIMARY KEY. It is set when an additional
	// column must be added for the ALTER TABLE LOCALITY REGIONAL BY ROW case,
	// where we allow a column add an a primary key change in the same
	// transaction.
	// It is nilable to prevent the 0 index being allowed in case the struct is
	// initialized with values improperly set.
	allowMutationOnSameTxnIdx *int
	// newColumnName is set if we are creating a new column before doing the
	// ALTER PRIMARY KEY. It is set when we use REGIONAL BY ROW without AS,
	// and a new implicit crdb_internal_region type column is created.
	...
	for i := range tableDesc.Mutations {
		...
		if mut.MutationID == currentMutationID {
			...
			if alterPrimaryKeyLocalitySwap != nil {
				allowIdx := alterPrimaryKeyLocalitySwap.allowMutationOnSameTxnIdx
