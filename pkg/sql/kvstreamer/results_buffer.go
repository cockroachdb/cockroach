...
type resultsBuffer interface {
	...
	add(_ []Result, numCompleteResponses int)
...
type resultsBufferBase struct {
	budget *budget
	syncutil.Mutex
	// numExpectedResponses tracks the number of responses that the results
	// buffer expects to process until the next call to init().
...
func (b *outOfOrderResultsBuffer) add(results []Result, numCompleteResponses int) {
	b.Lock()
	defer b.Unlock()
