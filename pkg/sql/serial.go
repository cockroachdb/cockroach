...
// FIXME(knz): make this a cluster setting / session var combo.
var serialAutoSeqOpt = true
...
func (p *planner) processSerialInColumnDef(
...
) (*tree.ColumnTableDef, *DatabaseDescriptor, *ObjectName, error) {
	...
	if d.HasDefaultExpr() {
		...
		return nil, nil, nil, pgerror.NewErrorf(pgerror.CodeSyntaxError,
	...
	if d.Nullable.Nullability == tree.Null {
		...
		return nil, nil, nil, pgerror.NewErrorf(pgerror.CodeSyntaxError,
			"conflicting NULL//NO NULL declarations for column %q of table %q",
			tree.ErrString(&d.Name), tree.ErrString(tableName))
	}

	newSpec := *d

	// Make the column non-nullable in all cases.
	...
	// Now attempt to find an object with that name.
	var res *TableDescriptor
	p.runWithOptions(resolveFlags{skipCache: true}, func() {
		res, err = ResolveExistingObject(ctx, p, seqName, false, anyDescType)
	})
	if err != nil {
		return nil, nil, nil, err
	}
	if res != nil {
		// The name already existed. Try harder.
	...
	log.VEventf(ctx, 2, "new column %q of %q will have sequence name %q and default %q",
		d, tableName, seqName, defaultExpr)

	newSpec.DefaultExpr.Expr = defaultExpr

	return &newSpec, dbDesc, seqName, nil
}
