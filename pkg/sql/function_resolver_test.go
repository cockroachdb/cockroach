...
`)
	...
	err := sql.TestingDescsTxn(ctx, s, func(ctx context.Context, txn *kv.Txn, col *descs.Collection) error {
		...
		sort.Slice(funcDef.Overloads, func(i, j int) bool {
			return funcDef.Overloads[i].(*tree.QualifiedOverload).Overload.Oid < funcDef.Overloads[j].(*tree.QualifiedOverload).Overload.Oid
		})
		require.Equal(t, 100110, int(funcDef.Overloads[0].(*tree.QualifiedOverload).Oid))
		require.True(t, funcDef.Overloads[0].(*tree.QualifiedOverload).UDFContainsOnlySignature)
		require.True(t, funcDef.Overloads[0].(*tree.QualifiedOverload).IsUDF)
		require.Equal(t, 1, len(funcDef.Overloads[0].(*tree.QualifiedOverload).Types.Types()))
		require.NotZero(t, funcDef.Overloads[0].(*tree.QualifiedOverload).Types.Types()[0].TypeMeta)
		require.Equal(t, types.EnumFamily, funcDef.Overloads[0].(*tree.QualifiedOverload).Types.Types()[0].Family())
		require.Equal(t, types.Int, funcDef.Overloads[0].(*tree.QualifiedOverload).ReturnType([]tree.TypedExpr{}))

		require.Equal(t, 100111, int(funcDef.Overloads[1].(*tree.QualifiedOverload).Oid))
		require.True(t, funcDef.Overloads[1].(*tree.QualifiedOverload).UDFContainsOnlySignature)
		require.True(t, funcDef.Overloads[1].(*tree.QualifiedOverload).IsUDF)
		require.Equal(t, 0, len(funcDef.Overloads[1].(*tree.QualifiedOverload).Types.Types()))
		require.Equal(t, types.Void, funcDef.Overloads[1].(*tree.QualifiedOverload).ReturnType([]tree.TypedExpr{}))

		require.Equal(t, 100112, int(funcDef.Overloads[2].(*tree.QualifiedOverload).Oid))
		require.True(t, funcDef.Overloads[2].(*tree.QualifiedOverload).UDFContainsOnlySignature)
		require.True(t, funcDef.Overloads[2].(*tree.QualifiedOverload).IsUDF)
		require.Equal(t, 1, len(funcDef.Overloads[2].(*tree.QualifiedOverload).Types.Types()))
		require.Equal(t, types.Int, funcDef.Overloads[2].(*tree.QualifiedOverload).Types.Types()[0])
		require.Equal(t, types.Int, funcDef.Overloads[2].(*tree.QualifiedOverload).ReturnType([]tree.TypedExpr{}))

		_, overload, err := funcResolver.ResolveFunctionByOID(ctx, funcDef.Overloads[0].(*tree.QualifiedOverload).Oid)
		require.NoError(t, err)
...
SELECT nextval(105:::REGCLASS);`, overload.Body)
		...
		_, overload, err = funcResolver.ResolveFunctionByOID(ctx, funcDef.Overloads[1].(*tree.QualifiedOverload).Oid)
		require.NoError(t, err)
		...
		_, overload, err = funcResolver.ResolveFunctionByOID(ctx, funcDef.Overloads[2].(*tree.QualifiedOverload).Oid)
		require.NoError(t, err)
	...
	err := sql.TestingDescsTxn(ctx, s, func(ctx context.Context, txn *kv.Txn, col *descs.Collection) error {
		...
		for _, tc := range testCases {
			t.Run(tc.testName, func(t *testing.T) {
				...
				for i, ol := range funcDef.Overloads {
					o := ol.(*tree.QualifiedOverload)
