	...
	for _, insecure := range [...]bool{true, false} {
		...
		if err := trivialQuery(pgBaseURL); err != nil {
			if insecure {
				...
				if !testutils.IsError(err, fmt.Sprintf("pq: user %s must authenticate using a client certificate", security.RootUser.Username())) {
		...
		{
			for _, optUser := range []*url.Userinfo{server.TestUser, security.RootUser} {
				pgWithCertRequireURL := pgBaseURL
				pgWithCertRequireURL.User = optUser
				...
				if insecure {
					...
					if optUser.Username() == server.TestUser.Username() {
						// The user TestUser has not been created so authentication
						// will fail with a valid certificate.
						if !testutils.IsError(err, fmt.Sprintf("pq: user %s does not exist", server.TestUser.Username())) {
							t.Errorf("unexpected error: %v", err)
						}
					} else {
...
func TestPGWireDBName(t *testing.T) {
	...
	pgURL, cleanupFn := sqlutils.PGUrl(t, s.ServingAddr(), "TestPGWireDBName", security.RootUser)
	pgURL.Path = "foo"
	defer cleanupFn()
	{
		db, err := gosql.Open("postgres", pgURL.String())
...
func TestPGPrepareFail(t *testing.T) {
	...
	pgURL, cleanupFn := sqlutils.PGUrl(t, s.ServingAddr(), "TestPGPrepareFail", security.RootUser)
	defer cleanupFn()
...
func TestPGPreparedQuery(t *testing.T) {
	...
	queryTests := map[string][]preparedQueryTest{
		...
		"SHOW GRANTS ON system.users": {
			baseTest.Results("users", security.RootUser.Username(), "DELETE,GRANT,INSERT,SELECT,UPDATE"),
		},
	...
	pgURL, cleanupFn := sqlutils.PGUrl(t, s.ServingAddr(), "TestPGPreparedQuery", security.RootUser)
	defer cleanupFn()
...
func TestPGPreparedExec(t *testing.T) {
	...
	pgURL, cleanupFn := sqlutils.PGUrl(t, s.ServingAddr(), "TestPGPreparedExec", security.RootUser)
	defer cleanupFn()
...
func TestPGPrepareNameQual(t *testing.T) {
	...
	pgURL, cleanupFn := sqlutils.PGUrl(t, s.ServingAddr(), "TestPGPrepareNameQual", security.RootUser)
	defer cleanupFn()
...
func TestCmdCompleteVsEmptyStatements(t *testing.T) {
	...
	pgURL, cleanupFn := sqlutils.PGUrl(
		t, s.ServingAddr(), "TestCmdCompleteVsEmptyStatements", security.RootUser)
	defer cleanupFn()
...
func TestPGCommandTags(t *testing.T) {
	...
	pgURL, cleanupFn := sqlutils.PGUrl(t, s.ServingAddr(), "TestPGCommandTags", security.RootUser)
	defer cleanupFn()
...
func TestSQLNetworkMetrics(t *testing.T) {
	...
	// Setup pgwire client.
	pgURL, cleanupFn := sqlutils.PGUrl(
		t, s.ServingAddr(), "TestSQLNetworkMetrics", security.RootUser)
	defer cleanupFn()
...
func TestPrepareSyntax(t *testing.T) {
	...
	pgURL, cleanupFn := sqlutils.PGUrl(t, s.ServingAddr(), "TestPrepareSyntax", security.RootUser)
	defer cleanupFn()
...
func TestPGWireAuth(t *testing.T) {
	...
	defer s.Stopper().Stop()
	{
		...
		t.Run("RootUserAuth", func(t *testing.T) {
			...
			rootPgUrl, cleanupFn := sqlutils.PGUrl(
				t, s.ServingAddr(), "TestPGWireAuth", security.RootUser)
			defer cleanupFn()
			...
			if _, err := db.Exec(fmt.Sprintf("CREATE USER %s;", server.TestUser.Username())); err != nil {
				t.Fatal(err)
			}
	...
	t.Run("TestUserAuth", func(t *testing.T) {
		testUserPgUrl, cleanupFn := sqlutils.PGUrl(
			t, s.ServingAddr(), "TestPGWireAuth", server.TestUser)
		defer cleanupFn()
