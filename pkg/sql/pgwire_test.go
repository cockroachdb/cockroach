			...
			for _, optUser := range []string{server.TestUser, security.RootUser} {
				...
				if insecure {
					...
					if optUser == server.TestUser {
						// The user TestUser has not been created so
						// authentication will fail with a valid certificate.
...
func TestPGWireDrainClient(t *testing.T) {
	...
	pgBaseURL := sqlutils.PGSSLModeURL(t, s.ServingAddr(), "disable", url.User(security.RootUser))
...
func TestPGWireAuth(t *testing.T) {
	...
	defer s.Stopper().Stop()
	{
...

		{
			...
			if _, err := db.Exec(fmt.Sprintf("CREATE USER %s WITH PASSWORD '蟑螂';", server.TestUser)); err != nil {
				t.Fatal(err)
			}
		...
		}
		{
			...
			unicodeUserPgUrl := sqlutils.PGSSLModeURL(
				t, s.ServingAddr(), "require", url.User(unicodeUser))
