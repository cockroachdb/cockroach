...
func getRowKey(
	...
	if len(values) < len(index.ColumnIDs) {
		...
		values = append([]parser.Datum(nil), values...)
		for len(values) < len(index.ColumnIDs) {
	...
	key, _, err := sqlbase.EncodeIndexKey(tableDesc, index, colMap, values, prefix)
	if err != nil {
		return nil, err
	}
...
func (p *planner) Relocate(ctx context.Context, n *parser.Relocate) (planNode, error) {
	...
	for i := range cols {
		if !cols[i].Typ.Equivalent(desiredTypes[i]) {
			return nil, errors.Errorf(
				"TESTING_RELOCATE data column %d must be of type %s, not type %s",
				i+1, desiredTypes[i], cols[i].Typ,
			)
		}
	}
...
func (n *relocateNode) Next(ctx context.Context) (bool, error) {
	...
	// Step 1: Add any targets that are not already replicas.
	//
