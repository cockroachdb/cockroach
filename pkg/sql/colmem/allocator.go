...
func (a *Allocator) PerformOperation(destVecs []coldata.Vec, operation func()) {
	...
	a.UpdateMemoryUsage(after - before)
}
...
// UpdateMemoryUsage updates the number of bytes currently allocated through
// this allocator by delta bytes (which can be both positive or negative).
func (a *Allocator) UpdateMemoryUsage(delta int64) {
	if delta > 0 {
