...
type SetAccountingHelper struct {
	...
	// varLengthVecIdxs stores the indices of all vectors with variable length
	// values except for the bytes-like ones.
	varLengthVecIdxs util.FastIntSet
	// varLengthDatumSizes stores the amount of space we have accounted for for
	...
	// accounted for.
	varLengthDatumSizes []int64
	// varLengthEstimatePerRow is the total estimated size of single values from
	// varLengthVecIdxs vectors which is accounted for by
	// EstimateBatchSizeBytes. It serves as the initial value for
	// varLengthDatumSizes values.
	varLengthEstimatePerRow int64
}
...
func (h *SetAccountingHelper) Init(allocator *Allocator, typs []*types.T, notNeededVecIdxs []int) {
	...
	for vecIdx, typ := range typs {
		...
		case types.DecimalFamily:
			h.varLengthVecIdxs.Add(vecIdx)
			h.varLengthEstimatePerRow += decimalEstimate
		case typeconv.DatumVecCanonicalTypeFamily:
			estimate, isVarlen := tree.DatumTypeSize(typ)
			if isVarlen {
				h.varLengthVecIdxs.Add(vecIdx)
				h.varLengthEstimatePerRow += int64(estimate) + memsize.DatumOverhead
			}
		}
	}
}
...
func (h *SetAccountingHelper) ResetMaybeReallocate(
	...
	if reallocated {
		...
		if !h.varLengthVecIdxs.Empty() {
			if cap(h.varLengthDatumSizes) < newBatch.Capacity() {
				h.varLengthDatumSizes = make([]int64, newBatch.Capacity())
			} else {
				h.varLengthDatumSizes = h.varLengthDatumSizes[:newBatch.Capacity()]
			}
			for i := range h.varLengthDatumSizes {
				h.varLengthDatumSizes[i] = h.varLengthEstimatePerRow
			}
		}
	}
	return newBatch, reallocated
...
func (h *SetAccountingHelper) AccountForSet(batch coldata.Batch, rowIdx int) {
	if h.bytesLikeVecIdxs.Empty() && h.varLengthVecIdxs.Empty() {
	...
	if !h.varLengthVecIdxs.Empty() {
		...
		for vecIdx, ok := h.varLengthVecIdxs.Next(0); ok; vecIdx, ok = h.varLengthVecIdxs.Next(vecIdx + 1) {
			vec := batch.ColVec(vecIdx)
			switch vec.CanonicalTypeFamily() {
			case types.DecimalFamily:
		...
		h.Allocator.AdjustMemoryUsage(newVarLengthDatumSize - h.varLengthDatumSizes[rowIdx])
		h.varLengthDatumSizes[rowIdx] = newVarLengthDatumSize
	}
}
