...
//
// If noBatchReuse=true is used, then the old batch is never reused and a new
...
func (a *Allocator) ResetMaybeReallocateNoMemLimit(
	...
	newBatch, reallocated, _ = a.resetMaybeReallocate(
		...
		true /* desiredCapacitySufficient */, false, /* noBatchReuse */
	)
...
type AccountingHelper struct {
	...
	maxCapacity int
	// noBatchReuse, if set, indicates that a new batch must be returned on each
...
func (h *AccountingHelper) ResetMaybeReallocate(
	...
	newBatch, reallocated, oldBatchReachedMemSize = h.allocator.resetMaybeReallocate(
		typs, oldBatch, minDesiredCapacity, h.memoryLimit, desiredCapacitySufficient, h.noBatchReuse,
	)
...
// other component.
// - noBatchReuse indicates whether a fresh batch must be returned on each
...
func (h *SetAccountingHelper) Init(
	allocator *Allocator, memoryLimit int64, typs []*types.T, noBatchReuse bool,
) {
	h.helper.Init(allocator, memoryLimit)
	h.helper.noBatchReuse = noBatchReuse
