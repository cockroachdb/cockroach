...
func TestResetMaybeReallocate(t *testing.T) {
	...
	t.Run("ResettingBehavior", func(t *testing.T) {
		...
		b, _, _ = testAllocator.resetMaybeReallocate(typs, b, coldata.BatchSize(), math.MaxInt64, false /* desiredCapacitySufficient */, false /* noBatchReuse */)
		b.SetSelection(true)
		b.Selection()[0] = 1
	...
	t.Run("LimitingByMemSize", func(t *testing.T) {
		...
		b, _, _ = testAllocator.resetMaybeReallocate(typs, smallBatch, minDesiredCapacity, smallMemSize, false /* desiredCapacitySufficient */, false /* noBatchReuse */)
		...
		b, _, _ = testAllocator.resetMaybeReallocate(typs, b, minDesiredCapacity/2, smallMemSize, true /* desiredCapacitySufficient */, false /* noBatchReuse */)
		require.Equal(t, smallBatch, b)
		require.Equal(t, minDesiredCapacity/2, b.Capacity())

		// Reset the batch and confirm that a new batch is allocated because we
		// have given larger memory limit.
		b, _, _ = testAllocator.resetMaybeReallocate(typs, b, minDesiredCapacity, largeMemSize, false /* desiredCapacitySufficient */, false /* noBatchReuse */)
