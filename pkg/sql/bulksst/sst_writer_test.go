...
// createVFSBatcher creates a batcher with VFS file allocator for testing.
func createVFSBatcher(
	ctx context.Context, s serverutils.TestServerInterface, path string, batchSize int64,
) (*bulksst.Writer, bulksst.FileAllocator) {
	fs := s.StorageLayer().Engines()[0].Env()
	fileAllocator := bulksst.NewVFSFileAllocator(path, fs)
	if batchSize > 0 {
	...
	return batcher, fileAllocator
}

// setupExternalStorage creates an external storage instance for testing.
func setupExternalStorage(
	...
	nodelocalURI := "nodelocal://1/bulksst-test"
...
func createExternalBatcher(
	...
	allocator := bulksst.NewExternalFileAllocator(storage, baseURI)
	if batchSize > 0 {
		bulksst.BatchSize.Override(ctx, &s.ClusterSettings().SV, batchSize)
	}
	batcher := bulksst.NewUnsortedSSTBatcher(s.ClusterSettings(), allocator)
	return batcher, allocator
}
...
func TestBulkSSTWriter(t *testing.T) {
	...
	batcher, fileAllocator := createVFSBatcher(ctx, s, "test_gsst", 1024)
	fs := s.StorageLayer().Engines()[0].Env()
...
func TestBulkAdderInterface(t *testing.T) {
	...
	t.Run("Add with timestamp", func(t *testing.T) {
		batcher, fileAllocator := createVFSBatcher(ctx, s, "test_bulk_adder_add", 1024)
		writeTS := s.Clock().Now()
		...
		values := readKeyValuesFromSST(t, fs, sstFiles.SST[0].URI)
	...
	t.Run("IsEmpty", func(t *testing.T) {
		batcher, _ := createVFSBatcher(ctx, s, "test_bulk_adder_empty", 0)
	...
	t.Run("CurrentBufferFill", func(t *testing.T) {
		batcher, _ := createVFSBatcher(ctx, s, "test_bulk_adder_fill", 1024)
		batcher.SetWriteTS(s.Clock().Now())
	...
	t.Run("GetSummary and SetOnFlush", func(t *testing.T) {
		batcher, _ := createVFSBatcher(ctx, s, "test_bulk_adder_summary", 1024)
		batcher.SetWriteTS(s.Clock().Now())
...
func TestExternalFileAllocator(t *testing.T) {
	...
	externalStorage := setupExternalStorage(t, ctx, s)

	t.Run("Basic file allocation", func(t *testing.T) {
		baseURI := "nodelocal://1/bulksst-test/"
		batcher, allocator := createExternalBatcher(ctx, s, externalStorage, baseURI, 1024)
		batcher.SetWriteTS(s.Clock().Now())
		...
		require.Equal(t, baseURI+"0.sst", fileList.SST[0].URI)
	...
	t.Run("Multiple files", func(t *testing.T) {
		baseURI := "nodelocal://1/bulksst-test-multi/"
		batcher, allocator := createExternalBatcher(ctx, s, externalStorage, baseURI, 512) // Small buffer to force multiple files.
		batcher.SetWriteTS(s.Clock().Now())
		...
		for i, sstFile := range fileList.SST {
			expectedURI := fmt.Sprintf("%s%d.sst", baseURI, i)
			require.Equal(t, expectedURI, sstFile.URI)
	...
	t.Run("Metadata accumulation", func(t *testing.T) {
		batcher, allocator := createExternalBatcher(ctx, s, externalStorage, "nodelocal://1/bulksst-test-accum/", 256) // Very small buffer.
		batcher.SetWriteTS(s.Clock().Now())
