...
func (c *Compiler) Compile(evalCtx *eval.Context, mem *memo.Memo) (_ vm.Program, ok bool) {
	*c = Compiler{
		md:      mem.Metadata(),
		evalCtx: evalCtx,
	}
	...
	pres := mem.RootExpr().(memo.RelExpr).RequiredPhysical().Presentation
	c.p.Push(vm.OpSetResultCols(c.md, pres))

	// Compile the expression.
	nextRowIP, ok := c.compileExpr(mem.RootExpr())
	if !ok {
