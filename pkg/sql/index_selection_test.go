...
func TestMakeConstraints(t *testing.T) {
	...
	testData := []struct {
		...
		{`c = true`, `c`, `[c = true]`},
		{`c = false`, `c`, `[c = false]`},
		{`c != true`, `c`, `[c IS NOT NULL]`},
		{`c != false`, `c`, `[c IS NOT NULL]`},
		{`NOT c`, `c`, `[c IS NOT NULL]`},
		{`c IS TRUE`, `c`, `[c = true]`},
		{`c IS NOT TRUE`, `c`, `[c IS NOT NULL] OR [c IS NULL]`},
		{`c IS FALSE`, `c`, `[c = false]`},
		{`c IS NOT FALSE`, `c`, `[c IS NOT NULL] OR [c IS NULL]`},
...
func TestMakeSpans(t *testing.T) {
	...
	testData := []struct {
		...
		{`NOT c`, `c`, `/#-`, `-/#`},
		{`c IS TRUE`, `c`, `/1-/2`, `/1-/0`},
		{`c IS NOT TRUE`, `c`, `-`, `-/# /NULL-`},
		{`c IS FALSE`, `c`, `/0-/1`, `/0-/-1/NULL`},
		{`c IS NOT FALSE`, `c`, `-`, `-/# /NULL-`},
