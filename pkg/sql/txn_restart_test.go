...
`); err != nil {
	...
	for i := 0; i < 2; i++ {
		...
		if i == 0 {
			// TODO(vivek): use isRetryableErr once we assign a Txn ID
			// at the start of a transaction instead of in the TxnCoordinator.
			if !testutils.IsError(err, "retryable error from another txn") {
				t.Fatalf("err = %v", err)
			}
