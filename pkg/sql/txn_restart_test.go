...
// Test that a retryable error intended for a different transaction doesn't
// cause another transaction to be retried.
...
`); err != nil {
	...
	if !testutils.IsError(err, "pq: terminated retryable error: forced by crdb_internal.force_retry()") {
