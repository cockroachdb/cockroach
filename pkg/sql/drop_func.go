...
func (p *planner) DropFunction(ctx context.Context, n *tree.DropFunction) (planNode, error) {
	...
	for _, name := range n.Names {
		...
		// If we've already seen this function, then skip it.
		if _, ok := node.fd[funcDesc.ID]; ok {
			continue
		}

		// Check if we can drop the function.
...
func (p *planner) canModifyFunc(ctx context.Context, desc *funcdesc.Mutable) error {
	...
	if !hasOwnership {
		return pgerror.Newf(pgcode.InsufficientPrivilege,
			"must be owner of function %s", tree.Name(desc.GetName()))
	}
	return nil
}
...
func (p *planner) canDropFuncDesc(
	...
	if behavior != tree.DropCascade {
		// TODO(jordan): when it's possible to reference UDFs from other places,
		// add this back in.
		// referencingIds := desc.ReferenceingDescriptorIDs
		var referencingIds []descpb.ID
		if err := p.checkReferencingObjects(ctx, "function", desc.Name(), referencingIds); err != nil {
			return err
		}
	}
	return nil
...
func (p *planner) dropFuncImpl(
	...
	if funcDesc.Dropped() {
		return errors.Errorf("function %q is already being dropped", funcDesc.Name())
	}
