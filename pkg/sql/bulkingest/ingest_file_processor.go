...
func (p *ingestFileProcessor) Close(ctx context.Context) {
	// Each worker manages its own cloudMux and closes it on completion.
}
		...
		group.GoCtx(func(ctx context.Context) error {
			...
			for {
				...
				case sst, ok := <-sstCh:
					...
					var err error
					buffer, err = p.processSST(ctx, sst, workerIndex, cloudMux, buffer)
					if err != nil {
...
func newIngestFileProcessor(
	...
	if post != nil && len(post.RenderExprs) > 0 {
		return nil, errors.New("IngestFile processor does not support rendering")
	}
