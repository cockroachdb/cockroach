...
func TestIngestFileProcessor(t *testing.T) {
	...
	require.Contains(t, sst.URI, "nodelocal://1/bulk_ingest_test/")
	require.Contains(t, sst.URI, ".sst")
...
func writeSST(
	...
	nodeStorage, err := exec.DistSQLSrv.ExternalStorageFromURI(
		context.Background(), nodeURI, username.RootUserName(),
	)
	...
	allocator := bulksst.NewExternalFileAllocator(nodeStorage, nodeURI, exec.Clock)
	...
	for _, r := range values {
		key := storage.MVCCKey{Key: tableEncoder.encodePrimaryKey(r.key)}
		value := tableEncoder.encodeValue(r.value)
	...
	files := allocator.GetFileList()
	require.Len(t, files.SST, 1)
	file := files.SST[0]
...
func getEncoder(exec sql.ExecutorConfig, tableName string) encoder {
	return encoder{
		tableDesc: desctestutils.TestingGetTableDescriptor(
			exec.DB, exec.Codec, "defaultdb", "public", tableName,
		),
		codec: exec.Codec,
	}
}
...
func (e *encoder) encodePrimaryKey(keyValue string) roachpb.Key {
	key := rowenc.MakeIndexKeyPrefix(
		e.codec, e.tableDesc.GetID(), e.tableDesc.GetPrimaryIndex().GetID(),
	)
