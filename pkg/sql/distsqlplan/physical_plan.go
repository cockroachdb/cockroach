...
type PhysicalPlan struct {
	...
	// streams eventually merge. The column indexes refer to columns for the rows
	// produced by ResultRouters.
	// Ignored when there is a single result router.
...
// SetOrdering sets p.Ordering.
func (p *PhysicalPlan) SetOrdering(o distsqlrun.Ordering) {
...
func (p *PhysicalPlan) AddProjection(columns []uint32) {
	...
	if len(p.Ordering.Columns) > 0 {
		newOrdering := make([]distsqlrun.Ordering_Column, len(p.Ordering.Columns))
		for i, c := range p.Ordering.Columns {
		...
		p.Ordering.Columns = newOrdering
	}
