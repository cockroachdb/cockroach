...
func (p *PhysicalPlan) AddDistinctSetOpStage(
	...
	// Create a distinct stage, where left and right sources are sent by hash to
	// the node which will contain the join processor. The distinct stage must be
	// before the join stage for EXCEPT queries to produce correct results (e.g.,
	// (VALUES (1),(1),(2)) EXCEPT (VALUES (1)) would return (1),(2) instead of
	// (2) if there was no distinct processor before the EXCEPT ALL join).
	...
	for bucket := 0; bucket < len(nodes); bucket++ {
		// Get the processor index offset for the current hash bucket.
		bucketOffset := bucket * numSides
		for side, routers := range inputResultRouters {
			pIdx := ProcessorIdx(distinctIdxStart + bucketOffset + side)
