...
// aggregation. If we have multiple sources of data, we first perform
// computation on each group ("local stage") and then we aggregate those results
// (final "stage").
//
// A simple example is COUNT: the local stage counts (COUNT), the final stage
// adds the counts (SUM_INT). Other simple examples are SUM, MAX, MIN, boolean
// operations. A more complex example is AVG.
type DistAggregationInfo struct {
	...
	// if there is more than one aggregation in the stages.
	//
	...
	distsqlrun.AggregatorSpec_AVG: {
		...
		FinalRendering: func(h *parser.IndexedVarHelper, varIdxOffset int) (parser.TypedExpr, error) {
			...
			if sum.ResolvedType().Equivalent(parser.TypeFloat) {
				// There is no "FLOAT / INT" operator; cast the denominator to float.
