...
var _ catalog.SchemaResolver = &planner{}
...
func getDescriptorsFromTargetList(
	...
	for _, tableTarget := range targets.Tables {
		...
		for i := range tableNames {
			descriptor, err := resolver.ResolveMutableExistingTableObject(ctx, p, &tableNames[i], true, resolver.ResolveAnyDescType)
			if err != nil {
				return nil, err
			}
			descs = append(descs, descriptor)
...
type fkSelfResolver struct {
	catalog.SchemaResolver
	newTableName *tree.TableName
	newTableDesc *sqlbase.TableDescriptor
}

var _ catalog.SchemaResolver = &fkSelfResolver{}
