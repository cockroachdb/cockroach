...
func (p *planner) getQualifiedTableName(
	ctx context.Context, desc sqlbase.TableDescriptorInterface,
) (*tree.TableName, error) {
	dbDesc, err := sqlbase.GetDatabaseDescFromID(ctx, p.txn, p.ExecCfg().Codec, desc.GetParentID())
	if err != nil {
		return nil, err
	}
...
// appropriate implementation of ExtendedDescriptorInterface before constructing a
...
func (l *internalLookupCtx) getTableByID(id descpb.ID) (sqlbase.TableDescriptorInterface, error) {
	tb, ok := l.tbDescs[id]
	if !ok {
...
func (l *internalLookupCtx) getParentName(table sqlbase.TableDescriptorInterface) string {
...
func getTableNameFromTableDescriptor(
	l simpleSchemaResolver, table sqlbase.TableDescriptorInterface, dbPrefix string,
) (tree.TableName, error) {
...
type simpleSchemaResolver interface {
	...
	getTableByID(id descpb.ID) (sqlbase.TableDescriptorInterface, error)
}
