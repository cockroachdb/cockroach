...
// TestVisibleTypeChangeOnly looks at the case where we convert a
// column's visible type e.g. INT -> INTEGER, which is otherwise a
// no-op behavior.
func TestVisibleTypeChangeOnly(t *testing.T) {
	...
	sqlDB.Exec(t, "CREATE TABLE d.t (a INT)")

	var a, expr string
	sqlDB.QueryRow(t, "SHOW CREATE TABLE d.t").Scan(&a, &expr)
	if idx := strings.Index(expr, "a INT NULL"); idx == -1 {
		t.Fatal("did not find 'a INT NULL'")
	}

	sqlDB.Exec(t, "ALTER TABLE d.t ALTER COLUMN a SET DATA TYPE INTEGER")

	sqlDB.QueryRow(t, "SHOW CREATE TABLE d.t").Scan(&a, &expr)
	if idx := strings.Index(expr, "a INTEGER NULL"); idx == -1 {
		t.Fatal("did not find 'a INTEGER NULL'")
	}
}
...
func TestColumnConversions(t *testing.T) {
	...
	columnConversionInfo := testMap{
		...
		{SemanticType: sqlbase.ColumnType_BYTES, Width: 20}: {
			...
			{SemanticType: sqlbase.ColumnType_BYTES, Width: 20}: columnConversionNoOp,
