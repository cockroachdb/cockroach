...
func (sc *SchemaChanger) truncateTable(
	...
	// requests to each range in the table sequentially, rather than letting
	// DistSender send them all in parallel, to hopefully give the compaction
	// queue time to compact the range tombstones away in between requests.
	//
	...
	for ri.Seek(ctx, tableSpan.Key, kv.Ascending); ; ri.Next(ctx) {
		...
		if startKey.Less(tableSpan.Key) {
			startKey = tableSpan.Key
		}
		...
		log.VEventf(ctx, 2, "ClearRange %s - %s", startKey, endKey)
