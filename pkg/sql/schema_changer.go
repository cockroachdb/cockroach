...
func (sc *SchemaChanger) done(ctx context.Context) error {
	...
	err := sc.db.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
		...
		desc, err := catalogkv.MustGetTableDescByID(ctx, txn, sc.execCfg.Codec, sc.tableID)
		if err != nil {
			return err
		}

		referencedTypeIDs, err = desc.GetAllReferencedTypeIDs(func(id descpb.ID) (sqlbase.TypeDescriptorInterface, error) {
	...
	update := func(txn *kv.Txn, descs map[descpb.ID]catalog.MutableDescriptor) error {
		...
		if !scTable.Dropped() {
			newReferencedTypeIDs, err := scTable.GetAllReferencedTypeIDs(func(id descpb.ID) (sqlbase.TypeDescriptorInterface, error) {
...
func (r schemaChangeResumer) Resume(
	...
	// nothing left to do.
	if details.DroppedDatabaseID != descpb.InvalidID && len(details.DroppedTables) == 0 {
		return nil
	}
