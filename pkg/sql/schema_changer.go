...
func (sc *SchemaChanger) validateBackfillQueryIntoTable(
	...
	if entryCountWrittenToPrimaryIdx > 0 {
		if entryCount != entryCountWrittenToPrimaryIdx {
			return errors.AssertionFailedf(
				"backfill query did not populate index %q with expected number of rows (expected: %d, got: %d)",
				index.GetName(), entryCount, entryCountWrittenToPrimaryIdx,
			)
		}
	}
	if count != entryCount {
...
func (sc *SchemaChanger) backfillQueryIntoTable(
	...
	if sc.execCfg.Settings.Version.IsActive(ctx, clusterversion.V25_3 /* replace with v25_4 when available */) {
	...
	if !skipValidation {
		if err := sc.validateBackfillQueryIntoTable(ctx, table, entriesWrittenToPrimaryIdx, validationTime, query); err != nil {
			return err
		}
	}
	return nil
