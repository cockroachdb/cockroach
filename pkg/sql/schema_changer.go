...
func maybeUpdateZoneConfigsForPKChange(
	...
	zoneWithRaw, err := descriptors.GetZoneConfigWithRawByte(ctx, txn, table.GetID())
	if err != nil {
		return err
	}
	...
	for oldIdx, newIdx := range oldIdxToNewIdx {
		for i := range zoneWithRaw.ZoneConfig().Subzones {
			subzone := &zoneWithRaw.ZoneConfig().Subzones[i]
			if subzone.IndexID == uint32(oldIdx) {
				...
				zoneWithRaw.ZoneConfig().SetSubzone(subzoneCopy)
			}
		}
	}
	...
	_, err = writeZoneConfig(
		ctx, txn, table.ID, table, zoneWithRaw.ZoneConfig(), zoneWithRaw.RawBytes(), execCfg, descriptors, false, kvTrace,
	)
