...
func (sc *SchemaChanger) exec(ctx context.Context) error {
	...
	// If this descriptor is a table, then check that we are the first in line
	// to execute the schema change.
			...
			if mutation.MutationID == sc.mutationID {
				if i != 0 {
					log.Infof(ctx,
						...
						tableDesc.GetName(), tableDesc.GetVersion(),
					)
	...
	// If the target descriptor is a table, then handle table specific schema
	// change steps before the backfill.
	if tableDesc, ok := desc.(*sqlbase.ImmutableTableDescriptor); ok {
		...
		// Otherwise, just wait for leases to drain and return.
...
func (r schemaChangeResumer) Resume(
	...
	return execSchemaChange(details.DescID, details.MutationID, details.DroppedDatabaseID)
}
		...
		case errors.Is(rollbackErr, sqlbase.ErrDescriptorNotFound):
			...
			log.Infof(
				...
				"descriptor %d not found for rollback of schema change processing mutation %d;"+
					"assuming it was dropped, and exiting",
				details.DescID, details.MutationID,
			)
		case ctx.Err() != nil:
