...
func (sc *SchemaChanger) DropTableDesc(
	...
	return sc.db.Txn(ctx, func(ctx context.Context, txn *client.Txn) error {
		...
		if tableDesc.GetDropJobID() != 0 {
			if err := sc.updateDropTableJob(ctx, txn, tableDesc.GetDropJobID(), tableDesc.ID, jobspb.DroppedTableDetails_DONE); err != nil {
...
func (sc *SchemaChanger) updateDropTableJob(
	...
	case jobspb.DroppedTableDetails_DONE:
...
func (sc *SchemaChanger) drainNames(
	...
	_, err := sc.leaseMgr.Publish(
		...
		func(txn *client.Txn) error {
			...
			if dropJobID != 0 {
				if err := sc.updateDropTableJob(ctx, txn, dropJobID, sc.tableID, jobspb.DroppedTableDetails_WAIT_FOR_GC_INTERVAL); err != nil {
					return err
				}
			}
