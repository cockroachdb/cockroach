...
func (sc *SchemaChanger) validateBackfillQueryIntoTable(
	...
	if err := sc.execCfg.InternalDB.Txn(ctx, func(ctx context.Context, txn isql.Txn) error {
		...
		// Copy over the session data from the schema changer into our count query,
		// which will pick up any session variables / temporary schemas for it to
		// function.

		txn.SessionData().SessionData = *sc.sessionData
