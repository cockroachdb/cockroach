...
// validateTablePrimaryKeys ensures that if this schema changer points to a DROP PRIMARY KEY
// mutation then the DROP PRIMARY KEY mutation is enclosed by an ADD PRIMARY KEY mutation.
// This is validated before txn commit to ensure that there are no tables in a state
