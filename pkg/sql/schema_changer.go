	...
	err := sc.txn(ctx, func(
		...
		for _, m := range scTable.AllMutations() {
			...
			if pkSwap := m.AsPrimaryKeySwap(); pkSwap != nil {
				oldIndexIDs := append([]descpb.IndexID{pkSwap.PrimaryKeySwapDesc().OldPrimaryIndexId}, pkSwap.PrimaryKeySwapDesc().OldIndexes...)
				newIndexIDs := append([]descpb.IndexID{pkSwap.PrimaryKeySwapDesc().NewPrimaryIndexId}, pkSwap.PrimaryKeySwapDesc().NewIndexes...)
