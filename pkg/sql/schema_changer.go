...
func (sc *SchemaChanger) exec(
	...
	log.Warningf(ctx, "schema change tableID=%d", sc.tableID)
	...
	// Set progress to check if the schema change has been canceled.
	if err := sc.job.Progressed(ctx, 0.0, jobs.Noop); err != nil {
		if err, ok := errors.Cause(err).(*jobs.InvalidStatusError); ok && err.Status == jobs.StatusPaused {
			log.Warningf(ctx, "schema change job %d has been paused", *sc.job.ID())
			return pgerror.NewErrorf(pgerror.CodeSchemaChangeJobPaused, "schema change job %d has been paused", *sc.job.ID())
		} else if ok && err.Status == jobs.StatusCanceled {
			log.Warningf(ctx, "schema change job %d has been canceled", *sc.job.ID())
			if err := sc.rollbackSchemaChange(ctx, err, evalCtx); err != nil {
				return err
			}
			return pgerror.NewErrorf(pgerror.CodeSchemaChangeJobCanceled, "schema change job %d has been paused", *sc.job.ID())
		}
		log.Warningf(ctx, "schema change job %d had unexpected error during progressing: %s", *sc.job.ID(), err)
		return err
	}
	...
	err = sc.runStateMachineAndBackfill(ctx, evalCtx, false /* isRollback */)
...
func (sc *SchemaChanger) rollbackSchemaChange(
	...
	if errPurge := sc.runStateMachineAndBackfill(ctx, evalCtx, true /* isRollback */); errPurge != nil {
...
func (sc *SchemaChanger) runStateMachineAndBackfill(
	ctx context.Context, evalCtx parser.EvalContext, isRollback bool,
) error {
	...
	// Run backfill(s).
	if err := sc.runBackfill(ctx, evalCtx); err != nil {
		return err
	}
...
func SetupSchemaChangeJobResume(
	st *cluster.Settings,
	testingKnobs *SchemaChangerTestingKnobs,
	db client.DB,
	...
	// FIXME(joey): SchemaChangeManager is used purely retain all of the
	// parameters passed in. Should we: keep it to visibly scope the
	// variables introduced into the closure, or just pass them in.
	// Secondly, will this cause anything to now be heap allocated that we
	// don't want? Since all of these are server-scoped, I assume it's not
	// a problem, if they aren't already on the heap.

	s := &SchemaChangeManager{
		db:           db,
		leaseMgr:     leaseMgr,
	...
	schemaChangeResume := func(
		typ jobs.Type, settings *cluster.Settings,
	) func(ctx context.Context, job *jobs.Job) error {
		if typ != jobs.TypeSchemaChange {
			return nil
		}

		return func(ctx context.Context, job *jobs.Job) error {
			details := job.Record.Details.(jobs.SchemaChangeDetails)
			log.Warningf(ctx, "Attempting to kick start a schema change job! job.id=%d", job.ID) // FIXME(joey): WIP logging
			...
			if err := sc.exec(ctx, false /* inSession */, evalCtx); err != nil {
				if shouldLogSchemaChangeError(err) {
					log.Warningf(ctx, "Error executing schema change: %s", err) // FIXME(joey): WIP logging
				}
				if err == sqlbase.ErrDescriptorNotFound {
					// FIXME(joey): job.Failed. Someone deleted this table, so
					// don't run the schema changer again.
					log.Warningf(ctx, "Error executing schema change (DescNotFound): %s", err) // FIXME(joey): WIP logging
				} else if sqlbase.IsPermanentSchemaChangeError(err) {
					log.Warningf(ctx, "Beginning rollback of schema change due to err: %s", err) // FIXME(joey): WIP logging
					if err := sc.rollbackSchemaChange(ctx, err, evalCtx); err != nil {
						log.Warningf(ctx, "Error rolling back schema change: %s", err) // FIXME(joey): WIP logging
						return err
					}
				} else {
					log.Warningf(ctx, "Schema change encountered non-fatal error: %s", err) // FIXME(joey): WIP logging
					// FIXME(joey): Sleep for some delay then re-run. This may
					// best be captured by just iterating through a for-loop.
				}

			}
			// FIXME(joey): job.Succeeded. Schema change was successful.
			return nil
		}
	}

	jobs.AddResumeHook(schemaChangeResume)
}
