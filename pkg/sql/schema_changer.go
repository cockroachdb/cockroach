...
func (sc *SchemaChanger) reverseMutations(ctx context.Context, causingError error) error {
	...
	update := func(descs map[sqlbase.ID]*sqlbase.MutableTableDescriptor) error {
		...
		// If there is a PrimaryKeySwap mutation with mutationID equal to sc.mutationID,
		// remove it from the mutations queue. There is only at most one PrimaryKeySwap
		// mutation per mutation ID.
		...
		if pkSwapMutationIdx != -1 {
			scDesc.Mutations = append(scDesc.Mutations[:pkSwapMutationIdx], scDesc.Mutations[pkSwapMutationIdx+1:]...)
		}
