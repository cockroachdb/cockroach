...
func (sc *SchemaChanger) DropTableDesc(
	...
	return sc.db.Txn(ctx, func(ctx context.Context, txn *client.Txn) error {
		...
		if tableDesc.GetDropJobID() != 0 {
			...
			if ok {
				jobDone := true
				for i := 0; i < len(details.DroppedTables); i++ {
					droppedTable := &details.DroppedTables[i]
					if droppedTable.ID == tableDesc.ID {
						droppedTable.Status = jobspb.DroppedTableDetails_StatusDone
					} else if droppedTable.Status != jobspb.DroppedTableDetails_StatusDone {
						jobDone = false
					}
				}
	...
	_, err := sc.leaseMgr.Publish(
		...
		func(txn *client.Txn) error {
			...
			if dropJobID != 0 {
				...
				details, ok := job.Details().(jobspb.SchemaChangeDetails)
				if ok {
					for i, _ := range details.DroppedTables {
						details.DroppedTables[i].Status = jobspb.DroppedTableDetails_StatusWaitForGCInterval
						break
					}

					if err := job.SetDetails(ctx, details); err != nil {
						return errors.Wrapf(err, "failed to update job %d", *job.ID())
					}
				}
			}
