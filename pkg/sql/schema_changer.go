...
func (sc *SchemaChanger) maybeMakeAddTablePublic(
	...
	return sc.txn(ctx, func(ctx context.Context, txn *kv.Txn, descriptors *descs.Collection) error {
		mut, err := descriptors.GetMutableTableVersionByID(ctx, table.ID, txn)
		if err != nil {
			return err
		}
		if !mut.Adding() {
		...
		return descriptors.WriteDesc(ctx, true /* kvTrace */, mut, txn)
	})
}
...
func drainNamesForDescriptor(
	...
	run := func(ctx context.Context, txn *kv.Txn, descriptors *descs.Collection) error {
		...
		mutDesc, err := descriptors.GetMutableDescriptorByID(ctx, descID, txn)
		if err != nil {
			return err
		}
		...
		if _, isSchema := mutDesc.(catalog.SchemaDescriptor); isSchema {
			mutDB, err := descriptors.GetMutableDescriptorByID(ctx, mutDesc.GetParentID(), txn)
			if err != nil {
				return err
			}
...
func (sc *SchemaChanger) RunStateMachineBeforeBackfill(ctx context.Context) error {
	...
	if err := sc.txn(ctx, func(
		...
		tbl, err := descriptors.GetMutableTableVersionByID(ctx, sc.descID, txn)
		if err != nil {
			return err
		}
		runStatus = ""
...
func (sc *SchemaChanger) done(ctx context.Context) error {
	...
	var didUpdate bool
	err := sc.txn(ctx, func(ctx context.Context, txn *kv.Txn, descriptors *descs.Collection) error {
		childJobs = nil
		...
		referencedTypeIDs, err = scTable.GetAllReferencedTypeIDs(func(id descpb.ID) (catalog.TypeDescriptor, error) {
			desc, err := descriptors.GetTypeVersionByID(ctx, txn, id, tree.ObjectLookupFlagsWithRequired())
			if err != nil {
				return nil, err
			}
			return desc, nil
		...
		for _, mutation := range scTable.Mutations {
			...
			if constraint := mutation.GetConstraint(); constraint != nil &&
				...
				if err := descriptors.WriteDescToBatch(ctx, kvTrace, backrefTable, b); err != nil {
					return err
				}
			}
			...
			if pkSwap := mutation.GetPrimaryKeySwap(); pkSwap != nil {
				...
				for _, idxID := range append(
					...
					if len(oldIndex.Interleave.Ancestors) != 0 {
						...
						ancestor, err := descriptors.GetMutableTableVersionByID(ctx, ancestorInfo.TableID, txn)
						if err != nil {
							return err
						}
						...
						for k, ref := range ancestorIdx.InterleavedBy {
							if ref.Table == scTable.ID && ref.Index == oldIndex.ID {
								...
								foundAncestor = true
								if err := descriptors.WriteDescToBatch(ctx, kvTrace, ancestor, b); err != nil {
									return err
								}
							}
						}
		...
		if !scTable.Dropped() {
			...
			for _, id := range referencedTypeIDs {
				...
				if err := descriptors.WriteDescToBatch(ctx, kvTrace, typ, b); err != nil {
					return err
				}
			}
		...
		if err := descriptors.WriteDescToBatch(ctx, kvTrace, scTable, b); err != nil {
			return err
		}
		...
		for i, mutation := range scTable.Mutations {
			...
			if constraint := mutation.GetConstraint(); constraint != nil &&
				...
				if constraint.ConstraintType == descpb.ConstraintToUpdate_FOREIGN_KEY {
					fk := &constraint.ForeignKey
					backrefTable, err := descriptors.GetMutableTableVersionByID(ctx, fk.ReferencedTableID, txn)
					if err != nil {
						return err
					}
