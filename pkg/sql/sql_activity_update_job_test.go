...
func TestSqlActivityUpdateJob(t *testing.T) {
	...
	ts.SQLServer().(*Server).GetSQLStatsProvider().(*persistedsqlstats.PersistedSQLStats).MaybeFlush(ctx)
...
func TestMergeFunctionLogic(t *testing.T) {
	...
	srv.SQLServer().(*Server).GetSQLStatsProvider().(*persistedsqlstats.PersistedSQLStats).MaybeFlush(ctx)
