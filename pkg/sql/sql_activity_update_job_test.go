...
func verifyTopActivityTableContentHelper(t *testing.T, db *sqlutils.SQLRunner, limitCnt int) {
	...
	for _, table := range stmtTables {
		...
		//  TODO remove: added only to show more logs during team city for debugging,
		// since it passes locally (including with stress)
		// -----
		r := db.QueryStr(t, `select * from (select 
		                        aggregated_ts,
		                        fingerprint_id,
		                        app_name,
					...
					from system.public.statement_statistics 
						where app_name not like '$ internal%%' 
						group by aggregated_ts, fingerprint_id, app_name)
				order by (statistics->'statistics'->>'cnt')::int limit 500`)
		t.Logf(">>> Values on first query part: %v", r)

		r = db.QueryStr(t, fmt.Sprintf(`SELECT * FROM %s`, table))
		t.Logf(">>>> Values from %s: %v", table, r)

		r = db.QueryStr(t, fmt.Sprintf(`SELECT *
		FROM 
		    (select * from (select 
		                        aggregated_ts,
		                        fingerprint_id,
					...
					from system.public.statement_statistics 
						where app_name not like '$ internal%%' 
						group by aggregated_ts, fingerprint_id, app_name)
				order by (statistics->'statistics'->>'cnt')::int limit 500) ts
		LEFT JOIN	(SELECT * FROM %s ) ta using (aggregated_ts, fingerprint_id, app_name)
		WHERE ta.fingerprint_id != ts.fingerprint_id OR
			ta.statistics != ts.statistics OR
		  ta.metadata != ts.merged_metadata `, table))
		t.Logf(">>>> Values of final query with %s: %v", table, r)
		// ----
