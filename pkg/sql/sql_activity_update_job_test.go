...
func TestSqlActivityUpdateJob(t *testing.T) {
	...
	err = row.Scan(&count)
	require.NoError(t, err)
	require.Equal(t, 1, count, "jobs: expect:1, actual:%d", count)
	...
	testutils.SucceedsWithin(t, func() error {
		...
		row = db.QueryRowContext(ctx, "SELECT count_rows(), aggregated_ts "+
			"FROM system.public.transaction_statistics AS OF SYSTEM TIME follower_read_timestamp() where app_name = $1 group by aggregated_ts", appName)
		...
		row = db.QueryRowContext(ctx, "SELECT count_rows(), aggregated_ts "+
			"FROM system.public.statement_statistics AS OF SYSTEM TIME follower_read_timestamp() where app_name = $1 group by aggregated_ts", appName)
	...
	for i := 0; i < 100; i++ {
		tempAppName := fmt.Sprintf("TestSqlActivityUpdateJobLoop%d", i)
	...
	_, err = db.ExecContext(ctx, "SET CLUSTER SETTING sql.stats.activity.top.limit = 5")
	require.NoError(t, err)
}
...
func TestSqlActivityUpdateTopLimitJob(t *testing.T) {
	...
	su := st.MakeUpdater()
	topLimit := 5
	su.Set(ctx, "sql.stats.activity.top.limit", settings.EncodedValue{
	...
	testutils.SucceedsWithin(t, func() error {
		...
		var stmtAggTs time.Time
		row = db.QueryRowContext(ctx, "SELECT count_rows(), aggregated_ts "+
			"FROM system.public.statement_statistics AS OF SYSTEM TIME follower_read_timestamp() where app_name like 'TestSqlActivityUpdateJobLoop%' group by aggregated_ts")
		err = row.Scan(&count, &stmtAggTs)
		if err != nil {
			return err
		}
	...
	row = db.QueryRowContext(ctx, "SELECT count_rows() "+
		"FROM system.public.transaction_activity where app_name like 'TestSqlActivityUpdateJobLoop%'")
	err = row.Scan(&count)
	require.NoError(t, err)
	require.LessOrEqual(t, count, maxRows, "transaction_activity after transfer: actual:%d, max:%d", count, maxRows)

	row = db.QueryRowContext(ctx, "SELECT count_rows() "+
		"FROM system.public.statement_activity where app_name like 'TestSqlActivityUpdateJobLoop%'")
	err = row.Scan(&count)
	require.NoError(t, err)
...
func TestScheduledSQLStatsCompaction(t *testing.T) {
	...
	_, err := db.ExecContext(ctx, "SET CLUSTER SETTING sql.stats.flush.interval = '5s'")
	require.NoError(t, err)
	_, err = db.ExecContext(ctx, "SET CLUSTER SETTING sql.stats.activity.flush.interval = '6s';")
	require.NoError(t, err)
