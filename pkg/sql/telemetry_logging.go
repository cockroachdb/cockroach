...
func (t *TelemetryLoggingMetrics) shouldEmitTransactionLog(force, isInternal bool) bool {
	...
	if !telemetryLoggingEnabled.Get(&t.st.SV) || !isTxnMode || !allowExecType || maxEventFrequency == 0 {
		return false
	}
...
func (t *TelemetryLoggingMetrics) shouldEmitStatementLog(
	...
	if !telemetryLoggingEnabled.Get(&t.st.SV) ||
		(isTxnMode && telemetryTransactionSamplingFrequency.Get(&t.st.SV) == 0) ||
		(!isTxnMode && maxEventFrequency == 0) {
		return false
	}
