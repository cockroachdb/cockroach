...
var telemetryTrackedTxnsLimit = settings.RegisterIntSetting(
	settings.TenantWritable,
	"sql.telemetry.tracked_txns.limit",
...
type TelemetryLoggingMetrics struct {
	...
	// skippedTxnCount is used to produce the count of non-sampled txns, when txn
	// sampling mode is on.
	skippedTxnCount uint64
}
...
// Setting force to true ensures the lastLog time gets updated to the time provided.
func (t *TelemetryLoggingMetrics) maybeUpdateLastLogTime(
	newTime time.Time, txnExecutionID string, force bool,
) (logTimeUpdated, txnTrackingStarted bool) {
	...
	shouldTryUpdate := func() bool {
		...
		return newTime.Sub(t.mu.lastEmittedTime) >= requiredTimeElapsed
	}
	...
	return true, txnTrackingStarted
}
