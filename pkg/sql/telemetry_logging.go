...
var telemetryTransactionSamplingFrequency = settings.RegisterIntSetting(
	...
	"the max event frequency (events per second) at which we sample transactions for "+
		"telemetry, If sampling mode is set to 'statement', this setting is ignored.",
	defaultMaxEventFrequency,
	settings.NonNegativeInt,
	settings.WithPublic,
)
...
// shouldTrackTransaction returns true if the transaction should be tracked for telemetry.
...
//   - the isTracingOn parameter is true and the transaction is not internal or sampling for internal statements
//     is enabled
//
// If the conditions are met, the last sampled time is updated.
func (t *TelemetryLoggingMetrics) shouldTrackTransaction(isTracingOn, isInternal bool) bool {
	isTxnMode := telemetrySamplingMode.Get(&t.st.SV) == telemetryModeTransaction
	allowExecType := !isInternal || telemetryInternalQueriesEnabled.Get(&t.st.SV)
	maxEventFrequency := telemetryTransactionSamplingFrequency.Get(&t.st.SV)
	if !telemetryLoggingEnabled.Get(&t.st.SV) || !isTxnMode || !allowExecType {
		return false
	}
	...
	if txnStart.Sub(t.mu.lastEmittedTime) < requiredTimeElapsed {
		return false
	}
...
func (t *TelemetryLoggingMetrics) shouldEmitStatementLog(
	...
	if isTxnMode {
		...
		if txnIsTracked {
			...
			// We don't need to update hte last emitted time in this case.
	...
	// We are in statement mode. Check if enough time has elapsed since the last emitted time.
