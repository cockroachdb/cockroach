...
func (n *NodeIDContainer) Set(ctx context.Context, val roachpb.NodeID) {
	...
	if atomic.CompareAndSwapInt32(&n.nodeID, 0, int32(val)) {
		log.Infof(ctx, "NodeID set to %d", val)
	} else if currVal := atomic.LoadInt32(&n.nodeID); currVal != int32(val) {
		log.Fatalf(ctx, "different NodeIDs set: %d, then %d", currVal, val)
	}
}
