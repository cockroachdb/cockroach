...
// mainly use a NodeIDContainer but can be instantiated for a SQL instance server
// are able to use the SQL instance ID as node ID.
...
func NewSQLIDContainerForNode(nodeID *NodeIDContainer) *SQLIDContainer {
	if nodeID.sqlInstance {
		log.Fatalf(context.Background(), "container is already for a SQL instance")
	}
