...
var (
	...
	// TestExternalTenantAlwaysEnabled will always redirect the test workload to
	// an external process virtual cluster. This is useful for quickly verifying that a
	//
	// Note: this value should not be used for checked in test code
	// unless there is a good reason to do so. We want the common case
	TestExternalTenantAlwaysEnabled = DefaultTestTenantOptions{testBehavior: ttEnabled | ttExternalProcess, allowAdditionalTenants: true}
