...
const (
	...
	// are assumed to range from 10ms to 400ms (maximum GCP inter-region latency).
	// Heartbeat offsets refer to the duration from the last heartbeat to the node
	// crash -- for example, with a heartbeat interval of 1s and a timeout of 3s,
	// if the node crashes 1s after the previous heartbeat (just before it's about
	// to heartbeat again), then the timeout will fire after 2s of unavailability
	// rather than 3s, so the heartbeat offset is -1s.
	//
	// Raft election:
