...
const (
	...
	// (-2.5s / 2 + 5.0s) and 2.5s for Raft elections ((-1.0s + 2.0s + 4.0s) / 2).
	// However, the worst-case Raft election latency (4.0s) being greater than the
	// best-case lease acquisition latency (2.5s) for a given RTT will skew the
	// average upwards, so we can approximate the typical unavailability to be
...
var (
	...
	// 1 second is sufficient for a network roundtrip and retransmit even in
	// multi-region clusters (see NetworkTimeout), so 2 seconds should be enough.
	// Furthermore, the actual election timeout per replica is multiplied by a
	// random factor of 1-2.
