...
func (cfg *Config) ValidateAddrs(ctx context.Context) error {
	...
	advHost, advPort, err := validateAdvertiseAddr(ctx, cfg.AdvertiseAddr, cfg.Addr, "")
	if err != nil {
	...
	advHTTPHost, advHTTPPort, err := validateAdvertiseAddr(ctx,
		cfg.HTTPAdvertiseAddr, cfg.HTTPAddr, advHost)
	if err != nil {
...
func UpdateAddrs(ctx context.Context, addr, advAddr *string, ln net.Addr) error {
	...
	*addr = net.JoinHostPort(lnHost, lnPort)
	...
	// Update the advertised port number. We don't touch the advertised
	// host, as this may have nothing to do with the listen address.
...
func validateAdvertiseAddr(
	ctx context.Context, advAddr, listenAddr, defaultHost string,
) (string, string, error) {
	if !strings.Contains(listenAddr, ":") {
		// We want SplitHostPort to complete successfully, so the port
		// number must be filled in already.
	...
	if !strings.Contains(advAddr, ":") {
		// We want SplitHostPort to complete successfully, so the port
		// number must be filled in already.
		advAddr = advAddr + ":" + listenPort
	}
	advHost, advPort, err := net.SplitHostPort(advAddr)
	if err != nil {
		return "", "", err
	...
	// address. Otherwise, resolve non-numeric to numeric.
	if advPort == "" {
		advPort = listenPort
	} else {
...
func resolveAddr(ctx context.Context, host, port string) (string, string, error) {
	...
	// Resolves the port number. This may translate service names
...
func (cfg *Config) CheckCertificateAddrs(ctx context.Context) {
	...
	// been called already.
	checkCertAddr(ctx, cfg.Addr, "listen", addrInfo, cert)
	checkCertAddr(ctx, cfg.AdvertiseAddr, "advertise", addrInfo, cert)
	...
	// Verify the compatibility. This requires that ValidateAddr() has
	// been called already.
	checkCertAddr(ctx, cfg.HTTPAddr, "HTTP listen", addrInfo, cert)
	checkCertAddr(ctx, cfg.HTTPAdvertiseAddr, "HTTP advertise", addrInfo, cert)
}

// checkCertAddr verifies that the given address in addr is compatible
// with the provided certificate.
func checkCertAddr(ctx context.Context, addr, msg, addrInfo string, cert *x509.Certificate) {
	...
	if err := cert.VerifyHostname(host); err != nil {
		log.Shout(ctx, log.Severity_WARNING,
			fmt.Sprintf("%s address %q not in node certificate (%s)", msg, host, addrInfo))
	}
}
...
func certAddrs(cert *x509.Certificate) string {
	...
	// hostname validation will only use the IPAddresses field. So
	// this needs to be printed in all cases.
	...
	// extensions here so we print both if available.
	if len(cert.DNSNames) > 0 {
		if buf.Len() > 0 {
			buf.WriteString("; ")
		}
		buf.WriteString("dnsNames: ")
		comma := ""
	...
	if cert.Subject.CommonName != "" {
		...
		buf.WriteString(cert.Subject.CommonName)
	}
	return buf.String()
}
