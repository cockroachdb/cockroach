...
// StoreSpecString returns a fully parsable version of the store spec.
func StoreSpecString(ss storageconfig.Store) string {
...
func (ssl StoreSpecList) String() string {
	...
	for _, ss := range ssl.Specs {
		fmt.Fprintf(&buffer, "--%s=%s ", cliflags.Store.Name, StoreSpecString(ss))
	}
