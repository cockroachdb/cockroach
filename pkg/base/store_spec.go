...
func NewSizeSpec(
	...
	if fractionRegex.MatchString(value) {
		...
		if percentRange != nil {
			if (percentRange.min != nil && size.Percent < *percentRange.min) || (percentRange.max != nil && size.Percent > *percentRange.max) {
				return SizeSpec{}, fmt.Errorf("store size (%s) must be between %d%% and %d%%", value, int(*percentRange.min), int(*percentRange.max))
			}
		}
	} else {
		var err error
