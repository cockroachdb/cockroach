...
func (c *constraintsConjunctionBounds) clamp(t *roachpb.SpanConfig, f Field) (changed bool) {
	...
	case voterConstraints:
		...
		// exactly one region, either for all replicas or for quorum less 1.
		// We'll treat this situation as something special, and we'll use the
		// first constraint in fallback to mirror this policy.
