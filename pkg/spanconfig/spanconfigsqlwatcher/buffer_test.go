...
func TestBufferCombinesDescriptorTypes(t *testing.T) {
	...
	makeEvent := func(descID int, descType spanconfig.DescriptorType, timestamp hlc.Timestamp) event {
		return event{
			update: spanconfig.DescriptorUpdate{
				ID:             descpb.ID(descID),
	...
	makeUpdates := func(descID int, descType spanconfig.DescriptorType) []spanconfig.DescriptorUpdate {
	...
	concreteDescriptorTypes := []spanconfig.DescriptorType{
		spanconfig.Table,
		spanconfig.Database,
		spanconfig.Schema,
		spanconfig.Type,
	...
	// (eg. spanconfig.Table) flanked by spanconfig.Any on either side. This
	// should all combine to the concrete descriptor type.
	{
		for _, descType := range concreteDescriptorTypes {
			err := buffer.add(makeEvent(1, spanconfig.Any, ts(nanos)))
			require.NoError(t, err)
			nanos++
	...
	// other.
	{
		for _, descType := range append(concreteDescriptorTypes, spanconfig.Any) {
			err := buffer.add(makeEvent(1, descType, ts(nanos)))
			require.NoError(t, err)
			nanos++
			err = buffer.add(makeEvent(1, descType, ts(nanos)))
