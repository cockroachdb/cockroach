...
// TestEventBuffer sends forth descriptor/zones rangefeed events to the buffer
// and ensures the checkpointing and ID flushing semantics work correctly.
func TestEventBuffer(t *testing.T) {
	...
	makeUpdates := func(descIDs []int) []spanconfig.SQLWatcherUpdate {
		updates := make([]spanconfig.SQLWatcherUpdate, 0, len(descIDs))
		for _, descID := range descIDs {
			updates = append(updates, spanconfig.SQLWatcherUpdate{
				ID:             descpb.ID(descID),
				DescriptorType: catalog.Any,
			})
		}
	...
	updates, combinedFrontierTS := buffer.flush(ctx)
	...
	require.Equal(t, makeUpdates([]int{1}), updates)
	...
	require.Equal(t, makeUpdates([]int{2}), updates)
	...
	require.Equal(t, makeUpdates([]int{1, 2}), updates)
}
