...
func TestBufferedEventsSort(t *testing.T) {
	...
	events := make(bufferedEvents, 0)

	// Insert a few descriptor updates.
	events = append(events, makeDescriptorEvent(1, ts(10)))
	events = append(events, makeDescriptorEvent(2, ts(5)))
	events = append(events, makeDescriptorEvent(2, ts(6)))

	// Insert a few cluster updates.
	events = append(events, makePTSEvent(true, roachpb.TenantID{}, ts(3)))
	events = append(events, makePTSEvent(true, roachpb.TenantID{}, ts(2)))

	// Insert a few tenant updates that should sort before the cluster updates.
	events = append(events, makePTSEvent(false, roachpb.MakeTenantID(2), ts(3)))
	events = append(events, makePTSEvent(false, roachpb.MakeTenantID(1), ts(7)))
	events = append(events, makePTSEvent(false, roachpb.MakeTenantID(2), ts(2)))

	// Insert a few more descriptor updates to interleave the updates.
	events = append(events, makeDescriptorEvent(4, ts(12)))
	events = append(events, makeDescriptorEvent(5, ts(11)))

	sort.Sort(events)

	require.Equal(t, bufferedEvents{
