...
type bufferedEvents []event

// Len implements the sort.Interface methods.
func (s bufferedEvents) Len() int {
	return len(s)
}
...
func (s bufferedEvents) Swap(i, j int) {
	s[i], s[j] = s[j], s[i]
}

var _ sort.Interface = &bufferedEvents{}
...
func (b *buffer) flush(
	...
	bufferedEvents := make(bufferedEvents, 0, len(events))
	for _, ev := range events {
		bufferedEvents = append(bufferedEvents, ev.(event))
	}
	...
	for i, ev := range bufferedEvents[:descriptorUpdatesIdx] {
		...
		prevUpdate := bufferedEvents[i-1].update
	...
	bufferedEvents = bufferedEvents[descriptorUpdatesIdx:]
	...
	for i, ev := range bufferedEvents {
		...
		prevUpdate := bufferedEvents[i-1].update.GetProtectedTimestampUpdate()

		// If the previous buffered event, and the current event are different
