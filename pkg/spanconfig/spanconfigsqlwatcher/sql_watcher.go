...
// state to the implied span configuration state.
func (s *SQLWatcher) FullReconcile(ctx context.Context) ([]spanconfig.Update, error) {
	...
	if err := sql.DescsTxn(
		...
		func(ctx context.Context, txn *kv.Txn, descsCol *descs.Collection) error {
			...
			// span configuration state by starting from RANGE DEFAULT. Thus, by
			// pretending as if RANGE DEFAULT has changed, we'll get the entire
			// implied span configuration state for the tenant.
			updates, err = s.handleIDUpdate(ctx, keys.RootNamespaceID, txn, descsCol)
			return err
		}); err != nil {
		return nil, err
	}
	...
	if err := s.stopper.RunAsyncTask(ctx, "span-config-reconciliation", func(ctx context.Context) {
		for {
			...
			case <-s.stopper.ShouldQuiesce():
				return
			case id = <-descUpdatesCh:
			case id = <-zonesUpdateCh:
			}
			err := s.onIDUpdate(ctx, id, updatesCh)
			// TODO(zcfgs-pod): Is swallowing errors here the right thing to do?
			if err != nil {
				log.Errorf(ctx, "could not react to desc id %d update err: %v", id, err)
			}
		}
	}); err != nil {
		return nil, err
...
func (s *SQLWatcher) watchForDescriptorUpdates(ctx context.Context) (<-chan descpb.ID, error) {
	...
	// TODO(arul): We need this initial scan because there could be a descriptor
	// update between the time we performed the full reconciliation and the
	// SQLWatcher started watching for updates. Instead of doing a full scan here,
	// we probably want to record the time at which we did a full reconciliation
	// and start the rangefeed from that time.
	// More generally, we probably want to save the last timestamp at which we
	// sent an update to KV in the job. Then, in the event that the job moves nodes,
	// we can forgo the full reconciliation and instead just establish rangefeeds
	// over system.zones/system.descriptors starting from such a timestamp.
...
func (s *SQLWatcher) watchForZoneConfigUpdates(ctx context.Context) (<-chan descpb.ID, error) {
	...
	// TODO(arul): See comment above in watchForDescriptorUpdates. We need this
	// initial scan for similar reasons but we should be able to get rid of it
	// soon once we can begin establishing the rangefeed from a given timestamp.
	var opts []rangefeed.Option
	if !s.knobs.SQLWatcherDisableInitialScan {
		opts = append(opts, rangefeed.WithInitialScan(nil))
		opts = append(opts, rangefeed.WithOnInitialScanError(func(ctx context.Context, err error) bool {
			log.Errorf(ctx, "error performing full reconciliation: %v", err)
...
// onIDUpdate generates the span configurations implied by the given
// {descriptor, pseudo} ID and sends them to the provided updatesCh.
func (s *SQLWatcher) onIDUpdate(
	ctx context.Context, id descpb.ID, updatesCh chan spanconfig.Update,
) error {
	var updates []spanconfig.Update
	if err := sql.DescsTxn(
		ctx,
	...
	// Now that we've constructed all the implied updates by the given ID we send
	// them on the updatesCh.
...
// handleIDUpdate generates all span configurations implied by the given
// {descriptor, pseudo} ID. These are configurations for all objects/ranges
// below the given ID in the zone config hierarchy.
func (s *SQLWatcher) handleIDUpdate(
	...
	if zonepb.IsNamedZoneID(id) {
		updates, affectedIDs, err = s.handleNamedZoneUpdate(ctx, id, txn, descsCol)
		if err != nil {
			return nil, err
		}
	} else {
		// We're dealing with a SQL Object.
		updates, affectedIDs, err = s.handleDescriptorIDUpdate(ctx, id, txn, descsCol)
		if err != nil {
			return nil, err
		}
	}
	...
	for _, affectedID := range affectedIDs {
		affectedIDUpates, err := s.handleIDUpdate(ctx, affectedID, txn, descsCol)
		if err != nil {
			return nil, err
		}
...
func (s *SQLWatcher) handleDescriptorIDUpdate(
	ctx context.Context, id descpb.ID, txn *kv.Txn, descsCol *descs.Collection,
...
func (s *SQLWatcher) generateSpanConfigurationsForNamedZone(
