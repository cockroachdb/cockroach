...
func (s *SQLWatcher) watchForProtectedTimestampUpdates(
	...
	handleEvent := func(ctx context.Context, ev *roachpb.RangeFeedValue) {
		...
		case *ptpb.Target_Cluster:
			e := event{timestamp: ts, update: spanconfig.MakeClusterProtectedTimestampSQLUpdate()}
			onEvent(ctx, e)
		case *ptpb.Target_Tenants:
			...
			for _, tenID := range t.Tenants.IDs {
				e := event{timestamp: ts, update: spanconfig.MakeTenantProtectedTimestampSQLUpdate(tenID)}
				onEvent(ctx, e)
			}
		case *ptpb.Target_SchemaObjects:
			...
			for _, id := range t.SchemaObjects.IDs {
				e := event{timestamp: ts, update: spanconfig.MakeDescriptorSQLUpdate(id, catalog.Any)}
				onEvent(ctx, e)
			}
		}
	}
