...
//
// When the callback is first installed, it's invoked with the [min,max) span --
// a shorthand to indicate that callers should consult the StoreReader for all
...
// StoreReader for the given span would retrieve the last config observed for
// the span[3].
//
...
type KVSubscriber interface {
	StoreReader
	OnSpanConfigUpdate(func(updated roachpb.Span))
}
