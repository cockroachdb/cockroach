...
// configs infrastructure in KV. It switches every each store in the cluster
// from using the gossiped back system config span to instead using the span
// configs infrastructure. It has no effect unless
// COCKROACH_EXPERIMENTAL_SPAN_CONFIGS is set.
...
func (s *Store) Apply(
	...
	for _, entry := range entriesToDelete {
		...
		deleted = append(deleted, entry.Span)
	}

	added = make([]roachpb.SpanConfigEntry, 0, len(entriesToAdd))
	for _, entry := range entriesToAdd {
		...
		added = append(added, entry.SpanConfigEntry)
	}
