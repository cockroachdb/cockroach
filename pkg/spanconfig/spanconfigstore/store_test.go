	...
	datadriven.Walk(t, testutils.TestDataPath(t), func(t *testing.T, path string) {
		...
		datadriven.RunTest(t, path, func(t *testing.T, d *datadriven.TestData) string {
			...
			case "overlapping":
				...
				var results []string
				require.NoError(t, store.ForEachOverlapping(ctx, span,
...
func TestRandomized(t *testing.T) {
	...
	if !expFound {
		require.NoError(t, store.ForEachOverlapping(ctx, testSpan,
			func(entry roachpb.SpanConfigEntry) error {
				return errors.Newf("found unexpected entry: %s",
		...
		require.NoError(t, store.ForEachOverlapping(ctx, testSpan,
			func(entry roachpb.SpanConfigEntry) error {
				if found {
					return errors.Newf("expected single overlapping entry, found second: %s",
						spanconfigtestutils.PrintSpanConfigEntry(entry))
				}
				found = true
	...
	require.NoError(t, store.ForEachOverlapping(ctx, everythingSpan,
...
func TestStoreClone(t *testing.T) {
	...
	require.NoError(t, original.ForEachOverlapping(ctx, everything,
		func(entry roachpb.SpanConfigEntry) error {
			originalEntries = append(originalEntries)
			return nil
		},
	))

	require.NoError(t, clone.ForEachOverlapping(ctx, everything,
		func(entry roachpb.SpanConfigEntry) error {
			clonedEntries = append(clonedEntries)
			return nil
		},
	))
