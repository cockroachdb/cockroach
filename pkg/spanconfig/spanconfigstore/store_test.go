...
// configRe matches a single word.
...
// XXX: Test using the StoreReader interface. All the get split stuff.
...
func TestDatadriven(t *testing.T) {
	...
	datadriven.Walk(t, "testdata", func(t *testing.T, path string) {
		storage := New(roachpb.TestingDefaultSpanConfig())
		datadriven.RunTest(t, path, func(t *testing.T, d *datadriven.TestData) string {
			...
			case "get":
				...
				entries := storage.TestingGetAllOverlapping(ctx, span)
				var results []string
			...
			case "set":
				...
				storage.Apply(ctx, spanconfig.Update{Span: span, Config: config}, false)
				return ""

			case "del":
				d.ScanArgs(t, "span", &spanStr)
				span := parseSpan(t, spanStr)
				storage.Apply(ctx, spanconfig.Update{Span: span}, false)
				return ""
...
func TestRandomized(t *testing.T) {
	...
	gotConfigs := store.TestingGetAllOverlapping(ctx, testSpan)
	if !expFound {
		...
		require.Truef(t, gotSpan.Contains(testSpan), "improper result: expected retrieved span (%s) to contain test span (%s)",
	...
	for i, cur := range store.TestingGetAllOverlapping(ctx, parseSpan(t, "[a,z)")) {
		if i == 0 {
			last = cur
			continue
		}
