...
func TestDatadriven(t *testing.T) {
	...
	datadriven.Walk(t, "testdata", func(t *testing.T, path string) {
		store := New(testutils.ParseConfig(t, "FALLBACK"))
		datadriven.RunTest(t, path, func(t *testing.T, d *datadriven.TestData) string {
			...
			case "set":
				...
				span, config := testutils.ParseSpan(t, spanStr), testutils.ParseConfig(t, confStr)
				...
				for _, sp := range deleted {
					b.WriteString(fmt.Sprintf("deleted %s\n", testutils.PrintSpan(sp)))
				}
				for _, ent := range added {
					b.WriteString(fmt.Sprintf("added %s\n", testutils.PrintSpanConfigEntry(ent)))
				}
				return b.String()

			case "delete":
				d.ScanArgs(t, "span", &spanStr)
				span := testutils.ParseSpan(t, spanStr)
			...
			case "get":
				...
				return fmt.Sprintf("conf=%s", testutils.PrintSpanConfig(config))
			...
			case "overlapping":
				...
				for _, entry := range entries {
					results = append(results, testutils.PrintSpanConfigEntry(entry))
				}
...
func TestRandomized(t *testing.T) {
	...
	genRandomSpan := func() roachpb.Span {
		...
		sp := testutils.ParseSpan(t, spanStr)
	...
	getRandomConf := func() roachpb.SpanConfig {
		...
		return testutils.ParseConfig(t, confStr)
	}
	...
	testSpan := testutils.ParseSpan(t, "[f,g)") // pin a single character span to test with
	...
	if !expFound {
		...
		require.Truef(t, gotSpan.Contains(testSpan),
			...
			testutils.PrintSpan(gotSpan), testutils.PrintSpan(testSpan))

		require.Truef(t, expConfig.Equal(gotConfig),
			"mismatched configs: expected %s, got %s",
			testutils.PrintSpanConfig(expConfig), testutils.PrintSpanConfig(gotConfig))
	...
	everythingSpan := testutils.ParseSpan(t, fmt.Sprintf("[%s,%s)",
	...
	for i, cur := range store.TestingGetAllOverlapping(ctx, everythingSpan) {
		...
		require.True(t, last.Span.Key.Compare(cur.Span.Key) < 0,
			...
			testutils.PrintSpan(last.Span), testutils.PrintSpan(cur.Span))
