...
// - kvsubscriber-subscribe starts the subscription process. It can also be used
//   to verify behavior when re-estabslishing subscriptions after hard errors.
...
// - kvsubscriber-inject-buffer-overflow can be used to inject a single
//   rangefeed buffer overflow errors within the kvsubscriber. It pokes into the
//   internals of the kvsubscriber and is useful to test teardown behavior.
//
...
func TestDataDriven(t *testing.T) {
	...
	datadriven.Walk(t, testutils.TestDataPath(t), func(t *testing.T, path string) {
		...
		mu := struct {
			syncutil.Mutex
			lastFrontierTS    hlc.Timestamp // serializes kvsubscriber-updates and kvaccessor-update
			subscriberRunning bool          // serializes kvsubscriber-{updates,subscribe,inject-buffer-overflow}
		...
		kvSubscriber.OnSpanConfigUpdate(ctx, func(span roachpb.Span) {
			mu.Lock()
			defer mu.Unlock()
		...
		datadriven.RunTest(t, path, func(t *testing.T, d *datadriven.TestData) string {
			...
			case "kvsubscriber-inject-buffer-overflow":
