		...
		case <-frontierBumpedCh:
			...
			if handler != nil {
				if !handlerInitialized {
					// NB: It's possible to hit this code path when
					// re-establishing a rangefeed, with a previously
					// initialized handler. In such cased we still want to
					// re-"initialize" the handler -- it's possible now have a
