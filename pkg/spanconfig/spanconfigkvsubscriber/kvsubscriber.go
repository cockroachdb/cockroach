...
// Subscribe establishes a rangefeed over the global store of span configs. This
// is a blocking operation, returning (read: unsubscribing) only when the
// surrounding stopper is stopped or when an error occurs. The installed handler
// is invoked in this single main thread. When unsubscribed, the exposed
// spanconfig.StoreReader continues to be readable (though it's no longer
// incrementally maintained -- the view gets progressively staler overtime).
//
// It's expected that callers will re-resubscribe in the face of retryable
// errors. Any existing handler is kept intact and will be notified when the
// rangefeed is re-established. After a successful re-subscription, the exposed
// StoreReader will be up-to-date and continue to be incrementally maintained.
func (s *KVSubscriber) Subscribe(ctx context.Context) error {
	...
	if initialTS.Less(s.lastFrontierTS) {
		log.Fatalf(ctx, "initial scan timestamp (%s) regressed from last recorded frontier (%s)", initialTS, s.lastFrontierTS)
	}

	rangefeed := s.rangefeedFactory.New("spanconfig-rangefeed", s.spanConfigTableSpan, initialTS,
		onValue,
	...
	for {
		...
		case <-initialScanDoneCh:
			...
			if handler != nil {
				...
				// The handler was installed before our initial scan was done,
				// giving we're notifying it using the everything span, we might
				// as well mark it as initialized.
				handlerInitialized = true
			}
