...
func (s *KVSubscriber) updateMetrics(ctx context.Context) {
	...
	s.metrics.UpdateBehindNanos.Update(timeutil.Since(lastUpdated.GoTime()).Nanoseconds())
	if earliestTS.IsEmpty() {
		s.metrics.OldestProtectedRecordNanos.Update(0)
	} else {
		s.metrics.UpdateBehindNanos.Update(timeutil.Since(earliestTS.GoTime()).Nanoseconds())
	}
}
