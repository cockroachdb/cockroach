...
func (r *Reconciler) Reconcile(
	...
	{ // Translate the entire SQL state and ensure KVAccessor reflects the latest.
		entries, timestamp, err := spanconfigsqltranslator.FullTranslate(ctx, r.sqlTranslator)
		if err != nil {
			return err
		}
	...
	return r.sqlWatcher.WatchForSQLUpdates(ctx, fullTranslationTS,
		func(ctx context.Context, updates []spanconfig.DescriptorUpdate, checkpoint hlc.Timestamp) error {
					...
					for _, update := range updates {
						...
						if update.DescriptorType == catalog.Table {
							...
							if errors.Is(err, catalog.ErrDescriptorNotFound) ||
								r.knobs.ExcludeDroppedDescriptorsFromLookup && desc.Dropped() {
								// Accumulate the set of missing table IDs.
								missingIDs = append(missingIDs, update.ID)
							} else if err != nil {
								return err
							}
						}
			...
			if len(toDelete) == 0 && len(toUpsert) == 0 {
				return callback(checkpoint) // nothing to do
			}
