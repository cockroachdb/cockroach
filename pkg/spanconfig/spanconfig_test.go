...
func TestScalability(t *testing.T) {
	...
	for epochIdx := 0; epochIdx < numEpochs; epochIdx++ {
		...
		require.NoError(t, ctxgroup.GroupWorkers(ctx, numBatchesPerEpoch, func(ctx context.Context, batchIdx int) error {
			return crdb.ExecuteTx(ctx, tdb.DB.(*gosql.DB), nil /* txopts */, func(tx *gosql.Tx) error {
				...
				log.Infof(ctx, "epoch=%d batch=%d created %d tables",
					epochIdx, batchIdx, numTablesPerBatch)
				return nil
			})
		}))

		log.Infof(ctx, "created %d tables during epoch=%d, in %s (total: %d tables in %s)",
			numTablesPerBatch*numBatchesPerEpoch,
			epochIdx,
			time.Since(epochStart),
			numTablesPerBatch*numBatchesPerEpoch*(epochIdx+1),
			time.Since(tableCreationStart),
		)
	}
	...
	log.Infof(ctx, "end-to-end span config reconciliation for %d tables took %s",
		expectedNumRecords, time.Since(reconciliationStart))
