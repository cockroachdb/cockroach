...
func PrintSpanConfigDiffedAgainstDefaults(conf roachpb.SpanConfig) string {
	...
	if !reflect.DeepEqual(conf.ProtectedTimestamps, defaultConf.ProtectedTimestamps) {
		...
		timestamps := make([]string, 0, len(conf.ProtectedTimestamps))
		for _, pts := range conf.ProtectedTimestamps {
			timestamps = append(timestamps, strconv.Itoa(int(pts.WallTime)))
		}
