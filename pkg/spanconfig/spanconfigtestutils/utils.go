...
var systemTargetRe = regexp.MustCompile(`^{source=(system|\d*),\s??target=(system|\d*)}$`)
...
func parseSystemTarget(t *testing.T, systemTarget string) spanconfig.SystemTarget {
	...
	if matches[2] == "system" {
		...
		targetID, err = strconv.Atoi(matches[2])
		require.NoError(t, err)
	}
	target, err := spanconfig.MakeSystemTarget(
		roachpb.MakeTenantID(uint64(sourceID)),
		roachpb.MakeTenantID(uint64(targetID)),
	)
	require.NoError(t, err)
	return target
}
...
func printSystemTarget(target spanconfig.SystemTarget) string {
	return fmt.Sprintf("{source=%s,target=%s}", target.SourceTenantID, target.TargetTenantID)
}
