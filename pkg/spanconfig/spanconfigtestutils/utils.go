...
// PrintSystemTargetSpanConfigDiffedAgainstDefault is a helper function that
// diffs the given config against the default config that applies to
// spanconfig.SystemTargets, and returns a string for the mismatched fields.
func PrintSystemTargetSpanConfigDiffedAgainstDefault(conf roachpb.SpanConfig) string {
	if conf.Equal(roachpb.TestingSystemTargetDefaultSpanConfig()) {
		return "system target default"
	}

	var diffs []string
	defaultSystemTargetConf := roachpb.TestingSystemTargetDefaultSpanConfig()
