...
func TestRandomized(t *testing.T) {
	...
	gotConfigs := storage.Get(testSpan)
	if !expFound {
		require.Len(t, gotConfigs, 0)
	} else {
		require.Len(t, gotConfigs, 1)
		gotSpan, gotConfig := gotConfigs[0].Span, gotConfigs[0].Config
		require.Truef(t, testSpan.Equal(gotSpan), "mismatched spans: expected=%s got=%s",
			printSpan(testSpan), printSpan(gotSpan))
	...
	for i, cur := range storage.Get(parseSpan(t, "[a,z)")) {
		if i == 0 {
			last = cur
			continue
		}
