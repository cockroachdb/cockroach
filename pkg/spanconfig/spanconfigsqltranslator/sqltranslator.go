...
var _ spanconfig.SQLTranslatorFactory = &SQLTranslatorFactory{}
...
// SQLTranslatorFactory is used to construct single-use, transaction-scoped
// spanconfig.SQLTranslators.
type SQLTranslatorFactory struct {
	ptsProvider protectedts.Provider
	codec       keys.SQLCodec
	knobs       *spanconfig.TestingKnobs
}

// NewFactory constructs and returns a SQLTranslatorFactory.
func NewFactory(
...
// SQLTranslator is part of the spanconfig.SQLTranslatorFactory interface.
func (f *SQLTranslatorFactory) SQLTranslator(
	txn *kv.Txn, descsCol *descs.Collection,
...
func (s *SQLTranslator) Translate(
	...
	if s.used {
		return nil, hlc.Timestamp{}, errors.AssertionFailedf("SQLTranslator already used")
	}

	// Mark the Translator as used to disallow subsequent calls to Translate.
	s.used = true
