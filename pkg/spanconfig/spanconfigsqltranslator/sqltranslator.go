...
func (s *SQLTranslator) generateSpanConfigurationsForNamedZone(
	...
	zoneConfig, err := sql.GetHydratedZoneConfigForNamedZone(ctx, txn, s.codec, s.txnBundle.descsCol, name)
	if err != nil {
		return nil, err
	}
...
func (s *SQLTranslator) generateSpanConfigurationsForTable(
	...
	zone, err := sql.GetHydratedZoneConfigForTable(ctx, txn, s.codec, s.txnBundle.descsCol, table.GetID())
	if err != nil {
		return nil, err
	}
...
func (s *SQLTranslator) maybeGeneratePseudoTableRecords(
	...
	for _, id := range ids {
		...
		//      database.
		zone, err := sql.GetHydratedZoneConfigForDatabase(ctx, txn, s.codec, s.txnBundle.descsCol, keys.SystemDatabaseID)
		if err != nil {
			return nil, err
		}
		tableSpanConfig := zone.AsSpanConfig()
...
func (s *SQLTranslator) maybeGenerateScratchRangeRecord(
	...
	for _, id := range ids {
		...
		zone, err := sql.GetHydratedZoneConfigForDatabase(ctx, txn, s.codec, s.txnBundle.descsCol, keys.RootNamespaceID)
		if err != nil {
