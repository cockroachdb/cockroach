...
// TestProtectedTimestampTableReader is a data driven test for the
// spanconfigsqltranslator.ProtectedTimestampTableReader. It allows users to
// protect and release protected timestamp records on the cluster, tenants, or
// schema objects and validate that the ProtectedTimestampTableReader maintains
...
//
// - "tablereader" [cluster] [tenants] [schemaObject=<int>]
...
func TestProtectedTimestampTableReader(t *testing.T) {
	...
	datadriven.Walk(t, testutils.TestDataPath(t), func(t *testing.T, path string) {
		...
		// table reader works as expected.
		...
		datadriven.RunTest(t, path, func(t *testing.T, d *datadriven.TestData) string {
			...
			case "tablereader":
				var output string
				require.NoError(t, execCfg.DB.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
					tableReader, err := spanconfigsqltranslator.TestingNewProtectedTimestampTableReader(ctx, txn, ptp)
					if err != nil {
						return err
					}

					if d.HasArg("cluster") {
						output = outputTimestamps(tableReader.GetProtectedTimestampsForCluster())
					} else if d.HasArg("tenants") {
						output = outputTenants(tableReader.GetProtectedTimestampsForTenants())
					} else if d.HasArg("schemaObject") {
						var schemaObjectID int
						d.ScanArgs(t, "schemaObject", &schemaObjectID)
						output = outputTimestamps(tableReader.GetProtectedTimestampsForSchemaObject(
...
func outputTenants(tenantProtections []spanconfigsqltranslator.TenantProtectedTimestamps) string {
	...
	for i, p := range tenantProtections {
		output.WriteString(fmt.Sprintf("id=%d %s", p.TenantID.ToUint64(),
