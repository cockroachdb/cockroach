...
func (s *SQLTranslator) translateID(
	...
	if zonepb.IsNamedZoneID(id) {
		entries, affectedIDs, err = s.translateNamedZoneConfig(ctx, id, txn, descsCol)
		if err != nil {
			return nil, err
		}
	} else {
		// We're dealing with a SQL Object.
		entries, affectedIDs, err = s.translateDescriptorIDZoneConfig(ctx, id, txn, descsCol)
		if err != nil {
			return nil, err
		}
	}
	...
	for _, affectedID := range affectedIDs {
		affectedIDEntries, err := s.translateID(ctx, affectedID, txn, descsCol)
		if err != nil {
			return nil, err
		}
		entries = append(entries, affectedIDEntries...)
	}
	return entries, nil
}

// translateDescriptorIDZoneConfig generates the implied span configuration
// state for an ID that belongs to a descriptor. It also returns a list of
// affected IDs that need to be further translated to their implied span
// configuration.
func (s *SQLTranslator) translateDescriptorIDZoneConfig(
	ctx context.Context, id descpb.ID, txn *kv.Txn, descsCol *descs.Collection,
) (entries []roachpb.SpanConfigEntry, affectedIDs descpb.IDs, err error) {
	var desc catalog.Descriptor
	affectedIDs, desc, err = getAllAffectedTableIDs(ctx, id, txn, descsCol)
	if err != nil {
		if errors.Is(err, catalog.ErrDescriptorNotFound) {
			// If the descriptor has been deleted then there is no implied span
			// configuration for that descriptor.
			return nil, nil, nil
		}
		return nil, nil, err
	}
	...
	// database itself, but we need to pass the list of affected IDs to the caller
	// to translate.
	return nil, affectedIDs, nil
}

// translateNamedZoneConfig generates the implied span configuration state for
// an ID that corresponds to a named zone. It also returns a list of affected
// IDs that need to be further translated to their implied span configurations.
// This is only ever the case for RANGE DEFAULT.
func (s *SQLTranslator) translateNamedZoneConfig(
	...
	default:
		// No more IDs are affected if the named zone isn't RANGE DEFAULT.
		return entries, nil, nil
	}
	...
	return entries, affectedIDs, nil
}
...
// getAllAffectedTableIDs returns a list of table IDs that lie below the given
// id in the zone configuration hierarchy. The descriptor corresponding to the
// given ID is also returned.
func getAllAffectedTableIDs(
	...
	// There's nothing to do here if the database has been dropped.
	if desc.Dropped() {
		return nil, desc, nil
	}
