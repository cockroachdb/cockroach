...
var _ spanconfig.Manager = &Manager{}

// Manager implements the spanconfig.Manager interface.
type Manager struct {
	db       *kv.DB
	jr       *jobs.Registry
...
func (m *Manager) Start(ctx context.Context) error {
	return m.stopper.RunAsyncTask(ctx, "start-span-config-reconciliation-job",
		func(ctx context.Context) {
			...
			// Periodically check the span config reconciliation job exists and start
			// it if for some reason it does not.
			for {
