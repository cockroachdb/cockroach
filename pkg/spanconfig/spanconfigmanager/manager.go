...
func (m *Manager) createAndStartJobIfNoneExists(ctx context.Context) (bool, error) {
	...
	err := m.stopper.RunAsyncTask(ctx, "notify-job-adoption", func(ctx context.Context) {
		if err := m.jr.NotifyToAdoptJobs(ctx); err != nil {
			log.Errorf(ctx, "error notifying registry of the auto span configuration job %v", err)
		}
	})
	return true, err
}
...
`, jobspb.TypeAutoSpanConfigReconciliation.String())
