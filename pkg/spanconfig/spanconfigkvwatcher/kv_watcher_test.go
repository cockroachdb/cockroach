...
func TestKVWatcher(t *testing.T) {
	...
	initialWatcher := spanconfigkvwatcher.New(tc.Stopper(), ts.DB(), ts.Clock(), ts.RangeFeedFactory().(*rangefeed.Factory), dummyTableID)
	...
	for _, op := range ops {
		...
		case update := <-initialUpdateCh:
			require.True(t, update.Span.Equal(op.span))
			require.Equal(t, update.Deletion, op.delete)
			if op.delete {
				require.True(t, update.Config.Equal(op.prev))
			} else {
	...
	finalWatcher := spanconfigkvwatcher.New(tc.Stopper(), ts.DB(), ts.Clock(), ts.RangeFeedFactory().(*rangefeed.Factory), dummyTableID)
