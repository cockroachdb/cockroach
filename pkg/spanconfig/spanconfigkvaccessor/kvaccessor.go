...
func (k *KVAccessor) constructGetStmtAndArgs(targets []spanconfig.Target) (string, []interface{}) {
	...
	for i, target := range targets {
		...
		queryArgs[startKeyIdx] = target.Encode().Key
		queryArgs[endKeyIdx] = target.Encode().EndKey
...
func (k *KVAccessor) constructDeleteStmtAndArgs(
	...
	for i, toDel := range toDelete {
		startKeyIdx, endKeyIdx := i*2, (i*2)+1
		deleteQueryArgs[startKeyIdx] = toDel.Encode().Key
		deleteQueryArgs[endKeyIdx] = toDel.Encode().EndKey
...
func validateUpdateArgs(toDelete []spanconfig.Target, toUpsert []spanconfig.Record) error {
	spansToUpdate := func(recs []spanconfig.Record) []spanconfig.Target {
	...
	for _, list := range [][]spanconfig.Target{toDelete, spansToUpdate} {
