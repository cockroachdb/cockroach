...
func (k *KVAccessor) GetSpanConfigEntriesFor(
	...
	// query start if it also overlaps with the query span.
	//
	// To batch multiple query spans into the same statement, we make use of
	// UNION ALL and a virtualized index.
	//
	//   SELECT 0 as idx, * FROM ( ... statement above for 1st query span ...)
	//   UNION ALL
	//   SELECT 1 as idx, * FROM ( ... statement above for 2nd query span ...)
	//   UNION ALL
	//   ...
	//
	var getStmtBuffer bytes.Buffer
	queryArgs := make([]interface{}, len(spans)*2)
	for i, sp := range spans {
		if i > 0 {
			getStmtBuffer.WriteString(`UNION ALL`)
		}
		...
		getStmtBuffer.WriteString(fmt.Sprintf(`
SELECT %[1]d AS idx, * FROM (
  SELECT start_key, end_key, config FROM %[2]s
   WHERE start_key >= $%[3]d AND start_key < $%[4]d
  UNION ALL
  SELECT start_key, end_key, config FROM (
    SELECT start_key, end_key, config FROM %[2]s
    WHERE start_key < $%[3]d ORDER BY start_key DESC LIMIT 1
  ) WHERE end_key > $%[3]d
)
`,
			i,             // [1]
			k.tableName,   // [2]
			startKeyIdx+1, // [3] -- prepared statement placeholder (1-indexed)
			endKeyIdx+1,   // [4] -- prepared statement placeholder (1-indexed)
		))
	}

	if err := k.db.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
		resp = make([][]roachpb.SpanConfigEntry, len(spans))
		it, err := k.ie.QueryIteratorEx(ctx, "get-span-cfgs", txn,
			sessiondata.InternalExecutorOverride{User: security.RootUserName()},
			getStmtBuffer.String(), queryArgs...,
		)
		if err != nil {
			return err
		}
		...
		for ok, err = it.Next(ctx); ok; ok, err = it.Next(ctx) {
			...
			span := roachpb.Span{
				Key:    []byte(*row[1].(*tree.DBytes)),
				EndKey: []byte(*row[2].(*tree.DBytes)),
			}
			var scfg roachpb.SpanConfig
			if err := protoutil.Unmarshal(([]byte)(*row[3].(*tree.DBytes)), &scfg); err != nil {
				return err
			}

			resp[idx] = append(resp[idx], roachpb.SpanConfigEntry{
				Span:   span,
				Config: scfg,
			})
		}
...
func (k *KVAccessor) UpdateSpanConfigEntries(
	...
	// TODO(irfansharif): Once we have the reconciliation job populating this
	// table, we could write a datadriven test asserting on its contents.
	...
	if len(toUpsert) > 0 {
		...
		for i, entry := range toUpsert {
			marshalled, err := protoutil.Marshal(&entry.Config)
			if err != nil {
				return err
			}
			...
			upsertQueryArgs[configIdx] = marshalled
		...
		//   )
		//
		var validationInnerStmtBuffer bytes.Buffer
		validationQueryArgs = make([]interface{}, len(toUpsert)*2)
		for i, entry := range toUpsert {
			if i > 0 {
				validationInnerStmtBuffer.WriteString(`UNION ALL`)
			}
			...
			validationInnerStmtBuffer.WriteString(fmt.Sprintf(`
SELECT COUNT(*) = 1 FROM (
...
`,
		...
		validationStmt = fmt.Sprintf("SELECT true = ALL(%s)", validationInnerStmtBuffer.String())
	}
	...
	if err := k.db.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
		...
		if n, err := k.ie.ExecEx(ctx, "upsert-span-cfgs", txn,
			...
			return errors.AssertionFailedf("expected to delete %d row(s), deleted %d", len(toUpsert), n)
		}
