...
import { INodeStatus, MetricConstants } from '../util/proto'
...
function makeNodesState(...addresses: { id: number, address: string, status?: NodeLivenessStatus }[]) {
  ...
  const livenessData: {statuses: {[key: string]: NodeLivenessStatus}} = {
    statuses: {},
  };
...
describe("selectCommissionedNodeStatuses", function() {
  ...
  function makeStateForLiveness(livenessStatuses: { [id: string]: NodeLivenessStatus }) {
    return {
      cachedData: {
        nodes: {
          data: nodeStatuses,
  ...
  const testCases: [string, NodeLivenessStatus, INodeStatus[]][] = [
...
describe("sumNodeStats", function() {
  it("sums stats from an array of nodes", function() {
    ...
    const livenessStatusByNodeID: { [key: string]: NodeLivenessStatus } = {
