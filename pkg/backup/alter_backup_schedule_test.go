...
func TestAlterBackupScheduleSpecifiedAndUnspecifiedIncLocation(t *testing.T) {
	...
	var numBackups int
	th.sqlDB.QueryRow(t,
		`SELECT count(DISTINCT start_time) FROM [SHOW BACKUP FROM LATEST IN 'nodelocal://1/backup']`).
		Scan(&numBackups)
	require.Equal(t, 3, numBackups)

	// Verify RESTORE works without specifying incremental_location.
	th.sqlDB.Exec(t, `DROP TABLE foo`)
	th.sqlDB.Exec(t, `RESTORE TABLE foo FROM LATEST IN 'nodelocal://1/backup'`)
	th.sqlDB.CheckQueryResults(t, `SELECT count(*) FROM foo`, [][]string{{"9"}})
	...
	th.sqlDB.Exec(t, `DROP TABLE foo`)
	th.sqlDB.Exec(t, `RESTORE TABLE foo FROM LATEST IN 'nodelocal://1/backup'`)
	th.sqlDB.CheckQueryResults(t, `SELECT count(*) FROM foo`, [][]string{{"12"}})
}
