...
func TestRestoreWithGrants(t *testing.T) {
	...
	tests := []struct {
		...
		// Table-level operations
		{
			...
			restoreStmt:     "RESTORE TABLE tablebackupdb.t1 FROM LATEST IN 'nodelocal://1/test-table-backup' WITH GRANTS, into_db = testdb",
			privilegeChecks: hasPrivs("user8", "TABLE", "testdb.t1", "SELECT", "INSERT"),
		},
...
func hasPrivs(grantee, objectType, objectName string, priviledges ...string) []privilegeCheck {
	checks := make([]privilegeCheck, len(priviledges))
	for i, priv := range priviledges {
