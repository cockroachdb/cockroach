...
func maybeStartCompactionJob(
	...
	// newly completed compaction job. As a result, it is possible that the chosen
	// times for this compaction job actually no longer exist in the chain because
	// it was compacted away. To avoid this, we need to check for the lock before
	// fetching the backup chain.
	//
