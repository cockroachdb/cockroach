...
// the statement. It can be either of type tree.Expr or
// tree.StringOrPlaceholderOptList.
func ResolveEncryptionOptionsFromExpr(
	...
	case jobspb.EncryptionMode_KMS:
		arrExpr, ok := encryptionExpr.(tree.StringOrPlaceholderOptList)
		if !ok {
			return nil, errors.New("unexpected type for KMS encryption expression")
		}
		kms, err := exprEval.StringArray(ctx, tree.Exprs(arrExpr))
		if err != nil {
			return nil, err
		}
