...
func (r *restoreResumer) cleanupTempSystemTables(ctx context.Context) error {
	...
	// Clean up the temp system database. The database name is determined by the
	// job's creation cluster version.
	...
	log.Dev.Infof(ctx, "cleaned up temporary system database %q", dbName)
