...
func maybeSetJobRetryingStatus(
	...
	return execCtx.ExecCfg().InternalDB.Txn(ctx, func(ctx context.Context, txn isql.Txn) error {
		status, _, err := resumer.job.StatusStorage().Get(ctx, txn)
		if err != nil {
			return errors.Wrap(err, "failed to get current job status")
		}
		// If status has already been set, no point in setting it again.
		if status != "" {
			return nil
		}
