...
func (r *restoreRetrier) setRetryPolicies() {
	...
	// If we've reached the secondary retry policy, it implies that the job has
	// been making progress. In this case, we have a longer backoff because we
	// will be retrying for a much longer duration. If the job has been running
	// and failing for a day, there is not much benefit in putting the retry loop
	// in a hot loop.
	secondaryRetryOpts := retry.Options{
