...
func TestRevlogSiblingJobE2E(t *testing.T) {
	...
	// Run on the system tenant: kv.rangefeed.enabled (which the
	// producer's rangefeed requires) is only operator-settable, and
	// the metamorphically-injected secondary tenant has hit
	// span-config reconciliation issues unrelated to this code path.
	...
	// The revlog producer's rangefeed requires kv.rangefeed.enabled.
