...
func resolveRestoreSubdirAndEndTime(
	...
	// NB: This is only able to detect if the backup token is a subdir because
	// subdirs contain a hyphen, which is not valid base64.
	_, _, err := backupinfo.DecodeBackupID(backupToken)
	if !useIDs || (err != nil && !strings.EqualFold(backupToken, backupbase.LatestFileName)) {
		// The user either passed a subdir or is not using backup IDs, so revert to
		// the legacy pre-ID behavior.
		subdir := backupToken
