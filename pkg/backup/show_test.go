...
func TestShowBackupWithIDs(t *testing.T) {
	...
	t.Run("show backups only shows one backup per id", func(t *testing.T) {
	...
	t.Run("fallback to subdir", func(t *testing.T) {
		// Ensure that specifying a subdirectory even when use_backups_with_ids is
		// set still works.
		var count int
		sqlDB.QueryRow(
			t,
		...
		// If we supply a subdirectory, it should revert to the old behavior of
		// displaying the whole chain, giving us multiple end times.
		require.Greater(t, count, 1, "expected multiple end times when querying by subdir")
	})
}
