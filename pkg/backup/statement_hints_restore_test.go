...
// backup with the old schema (pre-26.1) to a cluster with the new schema.
func TestStatementHintsRestoreFromOldBackup(t *testing.T) {
	...
	// enabled, and where_expr columns.
	...
	sqlDBRestoreRunner.Exec(t, `
		INSERT INTO system.statement_hints (fingerprint, hint, hint_type, hint_name, enabled, where_expr)
		VALUES ('UPDATE t SET x = _', '\xBEEF'::BYTES, 'optimization', 'update_hint', false, 'current_database() = ''mydb''')
	`)
