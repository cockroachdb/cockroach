...
// collectionURIs and backupToken. The token could either be the legacy
// subdirectory, a backup ID, or the string "LATEST".
//
// If the session setting use_backups_with_ids is enabled and the token is a
// parseable backup ID or "LATEST", the new backup info collection logic is used.
// Otherwise, the legacy logic is used.
//
// The returned memory reservation must be released by the caller.
func collectBackupInfo(
	...
	if !useIDs || (err != nil && !strings.EqualFold(backupToken, backupbase.LatestFileName)) {
		return legacyCollectBackupInfo(ctx, p, mem, kmsEnv, stmt, collectionURIs, backupToken)
	}
	return backupInfo{}, 0, errors.New("SHOW BACKUP with backup IDs is not yet supported")
}
...
func legacyCollectBackupInfo(
	...
	var encryptionExpr any
	encryptionMode := jobspb.EncryptionMode_None
	if stmt.Options.EncryptionPassphrase != nil {
		encryptionMode = jobspb.EncryptionMode_Passphrase
		encryptionExpr = stmt.Options.EncryptionPassphrase
	} else if stmt.Options.DecryptionKMSURI != nil {
		encryptionMode = jobspb.EncryptionMode_KMS
		encryptionExpr = stmt.Options.DecryptionKMSURI
	}
	encryption, err = backupencryption.ResolveEncryptionOptionsFromExpr(
		ctx, p, exprEval, encStore, encryptionMode, encryptionExpr,
	)
	if err != nil {
		return backupInfo{}, 0, err
	}
