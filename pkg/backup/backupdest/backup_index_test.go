...
func TestListIndexes(t *testing.T) {
	...
	expectedSort := make([]string, len(indexes))
	copy(expectedSort, indexes)
...
func TestGetBackupTreeIndexMetadata(t *testing.T) {
	...
	indexes := []struct {
		...
		// Contains compacted backup covering t=10-16.
		{
			subdirEnd: 10,
			times:     [][2]int{{0, 10}, {10, 12}, {12, 14}, {14, 16}, {10, 16}, {16, 18}, {18, 20}},
		},
		{
			subdirEnd: 22,
			times:     [][2]int{{0, 22}},
	...
	testcases := []struct {
		...
		},
		{
			...
			subdirEnd: 10,
			expectedIndexTimes: [][2]int{
				{0, 10}, {10, 12}, {12, 14}, {14, 16}, {10, 16}, {16, 18}, {18, 20},
			},
		},
		{
		...
		{
			name:               "end time between incrementals with compacted backup",
			subdirEnd:          10,
			endTime:            17,
