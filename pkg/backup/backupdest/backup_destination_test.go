...
func TestResolveBackupManifestsFallbackLogic(t *testing.T) {
	...
	args.Knobs.Server = &server.TestingKnobs{
		ClusterVersionOverride: clusterversion.V25_3.Version(),
	}
	tc, db, _, cleanup := backuptestutils.StartBackupRestoreTestCluster(
		t, backuptestutils.SingleNode, backuptestutils.WithParams(
	...
	t.Run("indexed full backup with custom incremental location", func(t *testing.T) {
		db.Exec(t, "SET CLUSTER SETTING version = $1", clusterversion.V25_4.Version().String())
