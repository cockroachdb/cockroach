...
// It expects that subdir has been resolved and is not LATEST. Backups are
// returned in ascending end time order.
//
...
func FindAllIncrementalPaths(
	...
	// caller), we can check do an existence check for the backup metadata. This
	// also avoids any shenanigans of dealing with custom incremental locations
	// that actually point to the default location.
