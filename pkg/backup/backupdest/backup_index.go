...
func GetBackupTreeIndexMetadata(
	...
	// Find the first index who encapsulates the end time. Remember that indexes
	// are sorted in reverse chronological order of the time they were created.
	for startIdx = 0; startIdx < len(indexes); startIdx++ {
		index := indexes[startIdx]
		if index.StartTime.Less(endTime) && !index.EndTime.Less(endTime) {
			break
		}
	}

	return indexes[startIdx:], nil
}
