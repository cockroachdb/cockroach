...
// maybeFlushOOOSpans checks if the new span's startKey precedes the last spans' endKey. If it does, the files are
// flushed as the underlying SST writer expects keys in order. If `skipSameSpan` is true, the files are not flushed
...
// this is a no-op.
func (s *fileSSTSink) setMidRowForPrevKey(endKey roachpb.Key) {
	if s.prevKey != nil {
		endRowKey, err := keys.EnsureSafeSplitKey(endKey)
		if err != nil {
			// If the key does not parse a family key, it must be from reaching the end
