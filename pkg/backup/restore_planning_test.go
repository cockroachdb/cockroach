...
func TestRestoreWithBackupIDs(t *testing.T) {
	...
	testcases := []struct {
		...
		{
			name:         "fallback/non-RH restore from latest",
			collection:   classicColl,
			token:        "LATEST",
			expectedRows: 2,
			disableIDs:   true,
		...
		{
			name:         "fallback/non-RH restore from non-latest subdir",
			collection:   classicColl,
			token:        backupSubdirsByColl[classicColl][0],
			expectedRows: 3,
			// Do not disable IDs here since we want to test that subdirs still work
			// even when IDs are enabled.
		},
		{
		...
		{
			name:         "fallback/RH restore before incremental backup",
			collection:   rhColl,
		...
		{
			name:         "fallback/RH restore before LATEST",
			collection:   rhColl,
			token:        "LATEST",
			aost:         rhTimes[3],
			expectedRows: 3,
		...
		{
			name:         "fallback/AOST restore works on subdir",
			collection:   classicColl,
			token:        backupSubdirsByColl[classicColl][0],
			aost:         classicTimes[2],
			expectedRows: 2,
		...
		{
			name:         "fallback/AOST restore works on LATEST",
			collection:   classicColl,
			token:        "LATEST",
			aost:         classicTimes[2],
			expectedRows: 2,
