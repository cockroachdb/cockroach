...
func TestBackupCompactionExecLocality(t *testing.T) {
	...
	// bring all node dirs up to an identical starting point
	// with one full backup and 3 uncompacted incrementals.
	// returns start and end timestamps
	bringUp := func(subCollection string) (hlc.Timestamp, hlc.Timestamp) {
		start := getTime()
	...
	t.Run("pin-tier", func(t *testing.T) {
		...
		start, end := bringUp("pin-tier")
		processorCollection := []string{"nodelocal://0/pin-tier"}

		// run and wait for locality filtered compaction job
		jobutils.WaitForJobToSucceed(
			t, db,
			triggerCompaction(
				t, east0Node,
				fullBackupQuery(targets, processorCollection, start, "WITH EXECUTION LOCALITY = 'tier=0'"),
		...
		// check that at least one node matching locality filter has all the backups, including compacted
		require.True(t, numEast0 == numInitialBackups+1 || numWest0 == numInitialBackups+1)
		// check that the rest of the nodes only have the initial backups
		require.True(t, numWest1 == numInitialBackups && numEast1 == numInitialBackups)
	})

	t.Run("pin-region", func(t *testing.T) {
		ensureLeaseholder(t, db)
		start, end := bringUp("pin-region")
		...
		jobutils.WaitForJobToSucceed(
			...
			triggerCompaction(
				t, east0Node,
				fullBackupQuery(targets, processorCollection, start, "WITH EXECUTION LOCALITY = 'region=east'"),
				getLatestFullDir(t, db, processorCollection[0]),
				start, end,
			),
		)
		...
		// check that the rest of the nodes only have the initial backups
		require.True(t, numWest0 == numInitialBackups && numWest1 == numInitialBackups)
	})

	t.Run("pin-single", func(t *testing.T) {
		ensureLeaseholder(t, db)
		start, end := bringUp("pin-single")
		...
		jobutils.WaitForJobToSucceed(
			...
			triggerCompaction(
				t, west1Node,
				fullBackupQuery(targets, processorCollection, start, "WITH EXECUTION LOCALITY = 'tier=1,region=west'"),
				getLatestFullDir(t, db, processorCollection[0]),
				start, end,
			),
		)
		...
		//validate that expected node got the compaction, and the rest didn't
		require.Equal(t, numWest1, numInitialBackups+1)
		require.True(t, numWest0 == numInitialBackups && numEast0 == numInitialBackups && numEast1 == numInitialBackups)

		// validate that node dir with compaction restores properly using the correct number of backups
