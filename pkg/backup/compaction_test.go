...
func TestBackupCompactionLocAware(t *testing.T) {
	...
	const numAccounts = 1000
	_, db, tempDir, cleanupFn := backupRestoreTestSetupWithParams(t, 3, numAccounts, InitManualReplication, args)
	defer cleanupFn()
	...
	t.Run("partition-by-unique-key", func(t *testing.T) {
		...
		// Query the backup to discover the actual directory structure of the compacted layer.
		...
		for i := 1; i <= 3; i++ {
			requireHasSSTs(t, filepath.Join(
				tempDir,
	...
	t.Run("partition-by-different-tiers", func(t *testing.T) {
		...
		const testPath = "foo/TestBackupCompactionLocAware/partition-by-different-tiers"
		requireHasNoSSTs(t,
			filepath.Join(tempDir, testPath, "1", compactedPath),
			filepath.Join(tempDir, testPath, "2", compactedPath),
		)
		requireHasSSTs(t,
			filepath.Join(tempDir, testPath, "3", compactedPath),
			filepath.Join(tempDir, testPath, "4", compactedPath),
		)
