...
func TestBackupCompaction(t *testing.T) {
	...
	startCompaction := func(
		...
		db.QueryRow(t, compactionQuery(t, backupStmt, fullPath, start, end)).Scan(&jobID)
		return jobID
	}
	...
	t.Run("basic operations insert, update, and delete", func(t *testing.T) {
		...
		var numBackups int
		db.QueryRow(
			t, fmt.Sprintf("SELECT count(DISTINCT (start_time, end_time)) FROM [%s]", showBackupQuery(collectionURI, nil)),
		).Scan(&numBackups)
	...
	t.Run("create and drop tables", func(t *testing.T) {
		...
		jobutils.WaitForJobToSucceed(t, db, startCompaction(db, backupStmt, getLatestFullDir(collectionURI), start, end))
		validateCompactedBackupForTables(t, db, collectionURI, []string{"foo", "bar", "baz"}, start, end, nil, 2)
		...
		db.Exec(t, restoreQuery(t, fullCluster, collectionURI, noAOST, nil))
	...
	t.Run("create indexes", func(t *testing.T) {
		...
		db.Exec(t, "DROP TABLE foo")
		db.Exec(t, restoreQuery(t, "TABLE foo", collectionURI, noAOST, nil))
	...
	t.Run("compact middle of backup chain", func(t *testing.T) {
		...
		db.Exec(t, "INSERT INTO foo VALUES (1, 1)")
		backupStmt := fullBackupQuery(t, fullCluster, collectionURI, noAOST, nil)
		db.Exec(t, backupStmt)

		db.Exec(t, "INSERT INTO foo VALUES (2, 2)")
		start := getTime()
		db.Exec(t, incBackupQuery(t, fullCluster, collectionURI, start, nil))
		...
		jobutils.WaitForJobToSucceed(t, db, startCompaction(db, backupStmt, getLatestFullDir(collectionURI), start, end))
		validateCompactedBackupForTables(t, db, collectionURI, []string{"foo"}, start, end, nil, 4)
	})
	...
	t.Run("table-level backups", func(t *testing.T) {
		...
		jobutils.WaitForJobToSucceed(t, db, startCompaction(db, backupStmt, getLatestFullDir(collectionURI), start, end))
		validateCompactedBackupForTables(t, db, collectionURI, []string{"foo"}, start, end, nil, 2)
	})

	t.Run("encrypted backups", func(t *testing.T) {
		bucketNum++
		collectionURI := []string{fmt.Sprintf("nodelocal://1/backup/%d", bucketNum)}
		encryptOpts := &tree.BackupOptions{
		...
		db.Exec(t, "INSERT INTO foo VALUES (1, 1)")
		start := getTime()
		backupStmt := fullBackupQuery(t, fullCluster, collectionURI, start, encryptOpts)
		db.Exec(t, backupStmt)

		db.Exec(t, "INSERT INTO foo VALUES (2, 2)")
		db.Exec(t, incBackupQuery(t, fullCluster, collectionURI, noAOST, encryptOpts))

		db.Exec(t, "UPDATE foo SET b = b + 1 WHERE a = 2")
		end := getTime()
		db.Exec(t, incBackupQuery(t, fullCluster, collectionURI, end, encryptOpts))
		...
		validateCompactedBackupForTables(t, db, collectionURI, []string{"foo"}, start, end, &tree.RestoreOptions{
			EncryptionPassphrase: encryptOpts.EncryptionPassphrase,
		}, 2)
	})
	...
	t.Run("compaction of chain ending in a compacted backup", func(t *testing.T) {
		...
		db.Exec(t, "INSERT INTO foo VALUES (1, 1)")
		mid := getTime()
		db.Exec(t, incBackupQuery(t, fullCluster, collectionURI, mid, nil))
		...
		ensureBackupExists(t, db, collectionURI, mid, end, nil)
		ensureBackupExists(t, db, collectionURI, start, end, nil)
	})
...
func TestCompactionCheckpointing(t *testing.T) {
	...
	var jobID jobspb.JobID
	db.QueryRow(t, compactionQuery(t, backupStmt, backupPath, start, end)).Scan(&jobID)
	...
	jobutils.WaitForJobToSucceed(t, db, jobID)
	validateCompactedBackupForTables(t, db, collectionURI, []string{"bank"}, start, end, nil, 2)
}
...
func TestToggleCompactionForRestore(t *testing.T) {
	...
	db.QueryRow(t, compactionQuery(t, backupStmt, backupPath, start, end)).Scan(&compactionID)
	waitForSuccessfulJob(t, tc, compactionID)

	var compRestoreID, classicRestoreID jobspb.JobID
	var unused any
	db.QueryRow(
		t, restoreQuery(t, "DATABASE data", collectionURI, noAOST, &tree.RestoreOptions{
			NewDBName: tree.NewDString("data1"),
		}),
	).Scan(&compRestoreID, &unused, &unused, &unused)
	db.Exec(t, "SET CLUSTER SETTING restore.compacted_backups.enabled = false")
	db.QueryRow(
		t, restoreQuery(t, "DATABASE data", collectionURI, noAOST, &tree.RestoreOptions{
			NewDBName: tree.NewDString("data2"),
		}),
...
func validateCompactedBackupForTables(
	...
	row := db.QueryRow(t, restoreQuery(t, "TABLE "+tablesList, collectionURI, noAOST, opts))
...
func ensureBackupExists(
	...
	times := db.Query(t,
		...
		FROM [%s]`, showBackupQuery(collectionURI, opts),
		))
	defer times.Close()
	found := false
	for times.Next() {
...
func fullBackupQuery(
	...
	return fmt.Sprintf(
		"BACKUP %s INTO (%s) %s %s",
		targets, uri, aostExpr(t, aost), optsExpr(opts),
	)
}
...
func compactionQuery(
	t *testing.T, backupStmt string, fullPath string, start, end hlc.Timestamp,
) string {
	t.Helper()
	startTime, err := strconv.ParseFloat(start.AsOfSystemTime(), 64)
	require.NoError(t, err)
...
// the collection.
func showBackupQuery(collectionURI []string, opts *tree.ShowBackupOptions) string {
	uri := stringifyCollectionURI(collectionURI)
	return fmt.Sprintf(
		`SHOW BACKUP FROM LATEST IN (%s) %s`, uri, optsExpr(opts),
	)
}
...
func optsExpr(opts tree.NodeFormatter) string {
...
func aostExpr(t *testing.T, aost hlc.Timestamp) string {
	if aost.IsEmpty() {
		return ""
	}
