...
// the requested time. It returns the manifests in a chain sorted by end time
// and then start time in ascending order.
//
...
func ValidateEndTimeAndTruncate[E ManifestLike](
	...
	slices.SortFunc(manifests, func(a, b E) int {
		if a.End().Less(b.End()) {
			return -1
		} else if b.End().Less(a.End()) {
			return 1
		} else if a.Start().Less(b.Start()) {
			return -1
		} else {
			return 1
		}
