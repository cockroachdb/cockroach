...
func maybeCompactIncrementals(
	...
	if int64(len(manifests)) < threshold {
		return nil
	}
...
func compactIncrementals(
	...
	backupManifest, newDetails, dest, err := prepareCompactedBackupMeta(
		ctx, execCtx, jobID, oldDetails, backupChain,
	)
	if err != nil {
		return err
	}
	if err := backupinfo.WriteBackupManifestCheckpoint(
		ctx, dest.DefaultURI, encryption, kmsEnv,
	...
	layerToIterFactory, err := backupinfo.GetBackupManifestIterFactories(
		ctx, execCtx.ExecCfg().DistSQLSrv.ExternalStorage, chainToCompact, encryption, kmsEnv,
	)
	if err != nil {
		return err
	}
	...
	genSpan := func(ctx context.Context, spanCh chan execinfrapb.RestoreSpanEntry) error {
		...
		for _, desc := range backupManifest.Descriptors {
			...
			catalogDescs = append(catalogDescs, catDesc)
	...
	store, err := execCtx.ExecCfg().DistSQLSrv.ExternalStorageFromURI(ctx, dest.DefaultURI, execCtx.User())
	if err != nil {
		return err
	}
	defer store.Close()
...
func processSpanEntry(
	...
	entry := sstIter.entry
	startKey, endKey := storage.MVCCKey{Key: entry.Span.Key}, storage.MVCCKey{Key: entry.Span.EndKey}
	prefix, err := backupsink.ElidedPrefix(entry.Span.Key, entry.ElidedPrefix)
	if err != nil {
		return err
	}
	if prefix != nil {
		startKey.Key = bytes.TrimPrefix(startKey.Key, prefix)
		endKey.Key = bytes.TrimPrefix(endKey.Key, prefix)
	}
	iter := sstIter.iter
	if err := sink.Reset(ctx, sstIter.entry.Span); err != nil {
		return err
	}
	scratch := make([]byte, 0, len(prefix))
	scratch = append(scratch, prefix...)
	for iter.SeekGE(startKey); ; iter.NextKey() {
		var key storage.MVCCKey
		if ok, err := iter.Valid(); err != nil {
			return err
		} else if key = iter.UnsafeKey(); !ok || !key.Less(endKey) {
			break
		}
		...
		fullKey.Key = scratch
		if err := sink.WriteKey(ctx, fullKey, value); err != nil {
			return err
		}
	}
	return nil
...
func prepareCompactedBackupMeta(
	...
	newDetails, err := makeCompactionBackupDetails(
		oldDetails, backupChain[1:],
	)
	if err != nil {
		return
	}
	dest, err = backupdest.ResolveDest(
		...
		newDetails.EndTime.AddDuration(10*time.Millisecond),
		execCtx.ExecCfg(),
	)
	if err != nil {
		return
	...
	if err = insqlDB.Txn(ctx, func(ctx context.Context, txn isql.Txn) error {
		tenantSpans, tenantInfos, err = getTenantInfo(ctx, execCtx.ExecCfg().Codec, txn, newDetails)
		return err
	}); err != nil {
		return
	}
	_, manifest, err = getBackupDetailAndManifest(
		ctx, execCtx.ExecCfg(), tenantSpans, tenantInfos, newDetails, execCtx.User(), dest,
	)
	if err != nil {
		return
	}
	// The manifest returned by getBackupDetailAndManifest have its
	// start and end times set based on the job details. We need to update them to
	// reflect the compacted backup's start and end times.
	manifest.StartTime, manifest.EndTime = newDetails.StartTime, newDetails.EndTime
...
func concludeBackupCompaction(
	...
	if backupinfo.WriteMetadataSST.Get(&execCtx.ExecCfg().Settings.SV) {
