...
func processSpanEntry(
	...
	entry := sstIter.entry
	trimmedStart, trimmedEnd := storage.MVCCKey{Key: entry.Span.Key}, storage.MVCCKey{Key: entry.Span.EndKey}
	...
	trimmedStart.Key = bytes.TrimPrefix(trimmedStart.Key, prefix)
	trimmedEnd.Key = bytes.TrimPrefix(trimmedEnd.Key, prefix)
	iter := sstIter.iter
...
func getIterFactoriesForCompaction(
	...
	allIters map[int]*backupinfo.IterFactory,
	compactedIters map[int]*backupinfo.IterFactory,
	err error,
) {
	allIters, err = backupinfo.GetBackupManifestIterFactories(
		ctx, storeFactory, backupChain, encryption, kmsEnv,
	)
	if err != nil {
		return
	}
