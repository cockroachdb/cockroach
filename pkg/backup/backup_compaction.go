...
func compactIntroducedSpans(
	ctx context.Context, manifest backuppb.BackupManifest, compactionChain compactionChain,
) (roachpb.Spans, error) {
...
type compactionChain struct {
	// allManifests is the entire chain of backups starting from the root full
	// backup.
...
func (c *compactionChain) baseBackup() backuppb.BackupManifest {
	return c.allManifests[c.startIdx-1]
}

// fullBackup returns the root full backup of the chain.
func (c *compactionChain) fullBackup() backuppb.BackupManifest {
...
func (c *compactionChain) lastBackup() backuppb.BackupManifest {
	return c.allManifests[c.endIdx-1]
}
