...
// scheduled incremental job if the backup chain length exceeds the threshold.
...
func maybeStartCompactionJob(
	...
	if triggerJob.RevisionHistory ||
		triggerJob.StartTime.IsEmpty() {
		return 0, nil
	}
	...
	if int64(len(chain)) < backupCompactionThreshold.Get(&execCfg.Settings.SV) {
		return 0, nil
	}
...
func (b *backupResumer) ResumeCompaction(
	...
	return b.maybeNotifyScheduledJobCompletion(ctx, jobs.StateSucceeded, execCtx, updatedDetails)
}
