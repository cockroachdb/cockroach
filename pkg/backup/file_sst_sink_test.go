...
func TestFileSSTSinkWriteKey(t *testing.T) {
	...
	type testCase struct {
		name      string
		exportKVs []*mvccKVSet
		// spans of files that should have been flushed during writeKey and before the final manual flush.
		flushedSpans []roachpb.Spans
	...
	for _, tt := range []testCase{
		...
		},
		{
			...
			exportKVs: []*mvccKVSet{
				newMVCCKeySet("a", "e").withKVs(
					[]kvAndTS{
						{key: "a", timestamp: 10}, {key: "b", timestamp: 10, value: gtFileSizeVal},
						{key: "c", timestamp: 10, value: gtFileSizeVal}, {key: "d", timestamp: 10},
					},
				),
			},
			flushedSpans: []roachpb.Spans{
		...
		},
		{
			...
			exportKVs: []*mvccKVSet{
				newMVCCKeySet("a", "d").withKVs([]kvAndTS{ // size
					{key: "a", timestamp: 10, value: gtFileSizeVal},
					{key: "c", timestamp: 10},
				}).withStartEndTS(10, 15),
				newMVCCKeySet("a", "d"). // ooo-key
																	withKVs([]kvAndTS{{key: "b", timestamp: 10}}).
																	withStartEndTS(10, 15),
				newMVCCKeySet("c", "f"). // ooo-span
		...
		// Ensure that when keys are written out of order, flushes occur to avoid writing keys to SST out of order.
		{
		...
		// Two spans that are contiguous with each other and have the same timestamp should be merged into one span.
		{
		...
		// Two spans that are contiguous with each other but don't have the same timestamp should not be merged.
		{
		...
		// Two spans that have the same timestamp but are not contiguous should not be merged.
		{
		...
		// If a span precedes the previous span but does not overlap, a flush must occur first.
		{
		...
		// If fileSpanByteLimit is reached, extending the file should be prevented and a new file created.
		{
		...
		// If fileSpanByteLimit is reached but the last key written was mid-row, the file must be extended.
		{
