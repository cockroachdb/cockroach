...
func TestBackupCompaction(t *testing.T) {
	...
	t.Run("create tables", func(t *testing.T) {
		...
		defer func() {
			db.Exec(t, fmt.Sprintf("DROP TABLE %s", strings.Join(tables, ", ")))
		}()
		db.Exec(t, fullBackupStmt)
		for i, tbl := range tables {
			db.Exec(t, fmt.Sprintf("CREATE TABLE %s (a INT, b INT)", tbl))
			db.Exec(t, fmt.Sprintf("INSERT INTO %s VALUES (1, 1)", tbl))
			doIncrementalBackup(t, db, "'nodelocal://1/backup'", false)
			db.Exec(t, fmt.Sprintf("INSERT INTO %s VALUES (2, 2)", tbl))
			doIncrementalBackup(t, db, "'nodelocal://1/backup'", true)
			validateCompactedBackupForTables(t, db, tables[:i+1], "'nodelocal://1/backup'")
		}
	})
