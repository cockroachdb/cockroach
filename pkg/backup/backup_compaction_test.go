...
func TestBackupCompaction(t *testing.T) {
	...
	startCompaction := func(bucket int, start, end string) jobspb.JobID {
		compactionBuiltin := `SELECT crdb_internal.backup_compaction(
ARRAY['nodelocal://1/backup/%d'], 'LATEST', ''::BYTES, '%s'::DECIMAL, '%s'::DECIMAL
)`
	...
	fullBackupAostCmd := "BACKUP INTO 'nodelocal://1/backup/%d' AS OF SYSTEM TIME '%s'"
	incBackupCmd := "BACKUP INTO LATEST IN 'nodelocal://1/backup/%d'"
	incBackupAostCmd := "BACKUP INTO LATEST IN 'nodelocal://1/backup/%d' AS OF SYSTEM TIME '%s'"
...
func validateCompactedBackupForTables(
	t *testing.T, db *sqlutils.SQLRunner, tables []string, collectionURIs string, start, end string,
) {
	t.Helper()
...
func validateCompactedBackupForTablesWithOpts(
	...
	db.QueryRow(t, fmt.Sprintf(`SELECT count(*) FROM [%s] 
	WHERE start_time::DECIMAL = ('%s'::DECIMAL / 1e9) AND
	end_time::DECIMAL = ('%s'::DECIMAL / 1e9)
	`, showBackupQ, start, end)).Scan(&count)
	require.NotZero(t, count, "no backup found that spans the start and end times")
