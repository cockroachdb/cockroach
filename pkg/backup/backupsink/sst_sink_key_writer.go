...
func (s *SSTSinkKeyWriter) maybeFlushOOOKey(ctx context.Context, newKey roachpb.Key) error {
	...
	if len(s.flushedFiles) == 0 {
		return errors.AssertionFailedf("unexpected empty flushedFiles despite prevKey being set")
	}
	...
	if newKey.Compare(s.prevKey) < 0 || !bytes.Equal(keyPrefix, s.elidedPrefix) {
		log.VEventf(
			ctx, 1, "flushing backup file %s of size %d because key %s is out of order with prevKey %s",
