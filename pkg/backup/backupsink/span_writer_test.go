...
func TestFileSSTSinkExtendOneFile(t *testing.T) {
	...
	sink, _ := fileSSTSinkSpanWriterSetup(t, st)
	...
	for _, tt := range []testCase{{name: "out-of-order-key-boundary",
		...
		for _, elide := range []execinfrapb.ElidePrefix{execinfrapb.ElidePrefix_None, execinfrapb.ElidePrefix_TenantAndTable} {
			t.Run(fmt.Sprintf("%s/elide=%s", tt.name, elide), func(t *testing.T) {
				...
				sink, store := fileSSTSinkSpanWriterSetup(t, st)
				defer func() {
...
func fileSSTSinkSpanWriterSetup(
	t *testing.T, settings *cluster.Settings,
) (*FileSSTSinkExportedSpanWriter, cloud.ExternalStorage) {
	...
	sink := NewFileSSTSinkExportedSpanWriter(sinkConf, store, nil /* pacer */)
	return sink, store
}
