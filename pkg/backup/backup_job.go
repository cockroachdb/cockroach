...
func (b *backupResumer) processScheduledBackupCompletion(
	...
	if scheduleID != 0 && jobState == jobs.StateSucceeded {
		if err := b.maybeTriggerBackupCompaction(ctx, execCtx, env, scheduleID, details); err != nil {
...
// maybeTriggerBackupCompaction checks if the scheduled backup job should trigger
// a backup compaction job and if so, triggers it.
func (b *backupResumer) maybeTriggerBackupCompaction(
	ctx context.Context,
	execCtx sql.JobExecContext,
	env scheduledjobs.JobSchedulerEnv,
	scheduleID jobspb.ScheduleID,
	...
	if err := execCtx.ExecCfg().InternalDB.Txn(ctx, func(ctx context.Context, txn isql.Txn) error {
		...
		if err != nil {
			return errors.Wrapf(err, "failed to get scheduled backup execution args for schedule %d", scheduleID)
		}
	...
	_, err := maybeStartCompactionJob(
		ctx, execCtx.ExecCfg(), execCtx.User(), details, backupStmt,
	)
	return err
}
