...
func findTimeSeries(
	snapshot engine.Reader, startKey, endKey roachpb.RKey, timestamp hlc.Timestamp,
	...
	for iter.Seek(next); iter.Valid() && iter.Less(end); iter.Seek(next) {
		...
		// pruning threshold.
		if pruneThreshold, ok := pruneThresholdByResolution[res]; !ok ||
			(timestamp.WallTime-pruneThreshold) > tsNanos {
