...
func TestServerDumpChildMetrics(t *testing.T) {
	...
	parentMetric := "cr.node.changefeed.emitted_bytes"
	...
	childMetric1 := `cr.node.changefeed.emitted_bytes{feed_id="123",scope="default"}`
	childMetric2 := `cr.node.changefeed.emitted_bytes{feed_id="456",scope="system"}`
	...
	t.Run("includeChildMetrics=false excludes child metrics", func(t *testing.T) {
		dumpClient, err := client.Dump(ctx, &tspb.DumpRequest{
		...
		for {
			...
			require.NoError(t, err)
			resultMap[msg.Name] = true
		}

		// Should only have parent metric
		require.True(t, resultMap[parentMetric], "parent metric should be included")
		require.False(t, resultMap[childMetric1], "child metric 1 should be excluded")
		require.False(t, resultMap[childMetric2], "child metric 2 should be excluded")
		require.Equal(t, 1, len(resultMap), "should only have parent metric")
	})

	t.Run("includeChildMetrics=true includes child metrics", func(t *testing.T) {
		dumpClient, err := client.Dump(ctx, &tspb.DumpRequest{
			Names:               []string{parentMetric},
			StartNanos:          100 * 1e9,
			EndNanos:            300 * 1e9,
	...
	t.Run("DumpRaw with includeChildMetrics=true", func(t *testing.T) {
		dumpRawClient, err := client.DumpRaw(ctx, &tspb.DumpRequest{
			Names:               []string{parentMetric},
			StartNanos:          100 * 1e9,
			EndNanos:            300 * 1e9,
