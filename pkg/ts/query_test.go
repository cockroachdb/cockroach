...
func TestQueryRollup(t *testing.T) {
	...
	tm.storeTimeSeriesData(resolution50nsRUP, []tspb.TimeSeriesData{
		{
			Name:   "metric.test",
			Source: "source1",
			Datapoints: []tspb.TimeSeriesDatapoint{
	...
	{
		query := tm.makeQuery("metric.test", resolution50nsRUP, 100, 1500)
		query.assertSuccess(13, 2)
	}

	{
		query := tm.makeQuery("metric.test", resolution50nsRUP, 450, 850)
		query.setDownsampler(tspb.TimeSeriesQueryAggregator_MAX)
