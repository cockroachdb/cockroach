...
// Dump returns a stream of raw internal timeseries data that has been stored in
// the KV layer. Data will be returned by series (alphabetically) and within a
// series, usually in ascending timestamp order, though this should not be
// relied upon.
//
// TODO(lunevalex): needs testing that restricting to individual timeseries works
...
func dumpImpl(db *kv.DB, req *tspb.DumpRequest, stream tspb.TimeSeries_DumpServer) error {
	for _, seriesName := range req.Names {
		// NB: we know from rollups that 30m-data is older than 10s-data. There are
		// ways in which that may not be true but it's probably fine.
		for _, res := range []Resolution{Resolution30m, Resolution10s} {
