...
func (tsi *timeSeriesSpanIterator) isColumnar() bool {
	return len(tsi.span[tsi.outer].Offset) > 0
}

func (tsi *timeSeriesSpanIterator) isRollup() bool {
	return len(tsi.span[tsi.outer].Count) > 0
}
...
// truncateSpan truncates the span underlying this iterator to the current iterator,
// *not including* the current position.  That is, the logical underlying span
// is truncated to [0, current).
...
func (db *DB) queryChunk(
	...
	if timespan.SampleDurationNanos != diskResolution.SampleDuration() {
		...
		// downsampleSpans always produces single-valued spans, so convert the downsampler to sum.
