...
func GenerateCatalog(metadata map[string]metric.Metadata) ([]ChartSection, error) {
	...
	// Range over all sectionDescriptions.
...
func checkOrginazationValues(sd sectionDescription) error {

	return nil
}

// createIndividualChart creates a slice of valid IndividualChart, which will
// later be added to the approrpiate ChartSection.
func createIndividualCharts(
	...
	for _, cd := range sd.Charts {
		...
		err = ic.addDisplayProperties(cd)
	...
	for _, org := range sd.Organization {
		...
		level0CatalogIndex, ok := catalogKey[org[0]]

		if !ok {
			return errors.Errorf(`Undefined Level 0 organization; you must 
			use a const defined in catalog_generator.go for %v`, sd)
		}

		chartCatalog[level0CatalogIndex].addChartAndSubsections(org, ics)
	}
