...
// TODO(otan): document mitigation for import weirdness.
//
...
// unless it is a brand new node (in which case, the upgrade needs external information
// from the cluster and is handled by Connect below).
//
// To make sure engines are not behind, when a new node joins a cluster, the Connect
...
// It will only be complete when all available nodes that are not decommissioned
// or dead have the desired TargetVersion, meaning any subsequent attempts to connect
// must also have TargetVersion set on the node's local engines.
...
// TODO(otan): consider introducing a progress function.
...
// TODO(otan): use cockroach_versions instead.
...
// TODO(otan): remove when we use this.
var _ = Register
...
func UpgradeCluster(ctx context.Context) error {
	// TODO(otan): implement this.

	// First, we RPC to every node or upgrade, and block until this is complete.

	// Next, we fetch every node again, and double check each node has the desired
	// target version.
	...
	// Once we return here, every node that is defined to be alive is upgraded.
	// New nodes that join must have this version.
	return nil
}
...
// It will be up to gossip.Connect to inform the engine to what to upgrade to later.
...
func upgradeEngineToTarget(
	...
	// This should have been handled by `Connect`, so error here.
	if state == nil {
		return errors.AssertionFailedf("state is nil for this engine")
	}
	...
	for current := state.CurrentVersion + 1; current <= state.TargetVersion; current++ {
		...
		registry.mu.RUnlock()
		// TODO(otan): if this is not required at a later stage
...
// UpgradeEngines is part of the NodeUpgraderServer interface.
...
// Status is part of the the NodeUpgraderServer interface.
...
// TODO(otan): when this is used, remove this.
