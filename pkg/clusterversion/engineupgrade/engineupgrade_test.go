...
// putEngineVersionKey forces an engine to be a certain version for a given key.
func putEngineVersionKey(t *testing.T, eng engine.Engine, key roachpb.Key, v Version) {
	val := roachpb.Value{}
	val.SetInt(int64(v))
	err := engine.MVCCPut(
		context.Background(),
...
func TestRunUpgrades(t *testing.T) {
	...
	t.Run("test RunUpgrades moves forward if target version is higher", func(t *testing.T) {
		// Pretend that engine 1 has lagged behind, and has an older current version
		// and previous version. Things should still progress.
		putEngineVersionKey(t, serv.engs[1], keys.StorePendingVersionKey(), Version(1))
		putEngineVersionKey(t, serv.engs[1], keys.StoreCurrentVersionKey(), Version(0))
	...
	t.Run("test RunUpgrades defensive assert", func(t *testing.T) {
		putEngineVersionKey(t, serv.engs[0], keys.StoreCurrentVersionKey(), Version(5))
