...
type Handle interface {
	...
	// currently active one. This has a very specific intended usage pattern,
	// and is probably only appropriate for usage in the EveryNode RPC. When
	// a new active version is set, all subsequent calls to `ActiveVersion`,
	// `IsActive`, etc. will reflect as much. The ClusterVersion supplied here
	// is one retrieved from other node.
	//
	// NB: It's important to note that this method is tightly coupled to cluster
	// version initialization (through `Initialize` above). Specifically the
	// following invariant must hold true:
	//
