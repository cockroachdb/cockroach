...
#### 7. Update pkg/storage/pebble.go

Add a version mapping for the new release version to pebbleFormatVersionMap.

**Find the latest Pebble format version:**

The value should be the latest format version constant from the Pebble package. You can find this by looking at existing entries in pebbleFormatVersionMap or checking Pebble's format version constants.

**Common latest versions:**
- `pebble.FormatMarkForCompactionInVersionEdit` (as of v25.4)
- `pebble.FormatV2BlobFiles` (earlier)
- `pebble.FormatValueSeparation` (earlier)

**Add the mapping** (around line 2508):
```go
// Before
...
var pebbleFormatVersionMap = map[clusterversion.Key]pebble.FormatMajorVersion{
	clusterversion.V25_4_PebbleFormatV2BlobFiles: pebble.FormatV2BlobFiles,
	clusterversion.V25_4:                         pebble.FormatMarkForCompactionInVersionEdit,  // Add this
	clusterversion.V25_3:                         pebble.FormatValueSeparation,
}
```

**Why this is important:**
- This mapping will be used later in M.4 when bumping MinSupported
- It ensures the version has a defined Pebble format version
- The format version should be the latest available at the time of the release

**Note:** Do NOT update `MinimumSupportedFormatVersion` - that only happens in M.4.

#### 8. Update Schema Changer Rules
...
A typical M.1 bump should modify approximately 16-21 files:
...
7. `pkg/storage/pebble.go`

**Generated/updated files:**
8. `pkg/BUILD.bazel`
9. `pkg/testutils/release/cockroach_releases.yaml`
10. `pkg/sql/logictest/REPOSITORIES.bzl`
11. `pkg/cli/testdata/declarative-rules/invalid_version`
12. `docs/generated/settings/settings-for-tenants.txt`
13. `docs/generated/settings/settings.html`

**Schema changer files:**
14. `pkg/sql/schemachanger/scplan/plan.go`
15. `pkg/sql/schemachanger/scplan/BUILD.bazel`
16. `pkg/sql/schemachanger/scplan/internal/rules/current/helpers.go`
17. `pkg/sql/schemachanger/scplan/internal/rules/current/testdata/deprules`
18. `pkg/sql/schemachanger/scplan/internal/rules/release_25_4/` (entire new directory)
...
- [ ] pebbleFormatVersionMap includes new version with latest Pebble format
...
- [ ] Git status shows expected number of modified files (~16-21)
