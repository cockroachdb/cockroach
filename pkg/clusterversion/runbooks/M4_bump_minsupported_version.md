...
After removing bootstrap data, descriptor error messages change format. Update test expectations in `pkg/upgrade/upgrades/first_upgrade_test.go` (around lines 287-290):
...
**Why this change is needed here:** Removing bootstrap data affects how descriptor errors are formatted during validation. The simpler error message format now surfaces earlier in the validation pipeline.
...
#### Error 1: TestFirstUpgradeRepair test failure
...
**Cause:** After removing bootstrap data (Commit 3), the error message format for descriptor validation changed. The descriptor lookup error now occurs earlier in the validation pipeline, before the detailed FK-specific validation runs.

**Why this happens:**
- Validation now happens earlier during lookup (`pkg/sql/catalog/errors.go`)
- The simpler "descriptor not found" error surfaces before the detailed FK validation runs
- This matches the pattern in `pkg/sql/tests/repair_test.go` which already expects the simpler format

**Fix:**

Update test expectations in `pkg/upgrade/upgrades/first_upgrade_test.go` (around lines 287-290):
...
**When to apply:** This fix should be included in **Commit 3 (Remove Bootstrap Data)** as it's directly related to the bootstrap data removal. See the detailed instructions in the Commit 3 section above.

**Verification:**
```bash
./dev test pkg/upgrade/upgrades -f TestFirstUpgradeRepair -v
```

Expected: Test passes with the simpler error format.

#### Error 2: TestMinimumSupportedFormatVersion test failure
