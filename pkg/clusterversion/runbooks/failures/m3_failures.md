...
**Verify the test function list:** Each `TestLogic_*` function in `generated_test.go`
corresponds to a logic test file whose `# LogicTest:` header includes
`cockroach-go-testserver-configs` (the named config set). Files that explicitly
list only specific versions (e.g. `cockroach-go-testserver-25.4 cockroach-go-testserver-26.1`)
must NOT be included — check each file's header and omit if it doesn't cover the new version.

```bash
# Find logic test files that include the new version via cockroach-go-testserver-configs:
grep -rl "cockroach-go-testserver-configs" pkg/sql/logictest/testdata/logic_test/

# Find files that explicitly exclude the new version (check their headers manually):
grep -rl "cockroach-go-testserver-" pkg/sql/logictest/testdata/logic_test/ \
  | xargs grep "^# LogicTest:" | grep -v "cockroach-go-testserver-configs"
```
...
Find all `generated_test.go` files whose current `configIdx` is >= your new
config's index, then increment them:

```bash
NEW_IDX=<your computed configIdx>

# Preview what will change (exclude the new dir you just created)
grep -r "const configIdx" pkg/sql/logictest/tests/ \
  | grep -v "cockroach-go-testserver-X.Y"

# Apply the increment (macOS sed syntax)
...
| `TestDeclarativeRules` fails with version mismatch | Forgot to run `./dev test pkg/cli -f=TestDeclarativeRules --rewrite` |
| `check_generated_code` fails | See M3 QUICK runbook Step 6 for the gitignored-file fix |
| `TestLogic_*` for a version-specific file appears in new generated_test.go | The file's `# LogicTest:` header explicitly lists versions and excludes the new one. Remove the function from `generated_test.go`. |
