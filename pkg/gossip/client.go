...
func (c *client) sendGossip(g *Gossip, stream Gossip_GossipClient, firstReq bool) error {
	...
	if firstReq {
		g.mu.is.populateMostDistantSentinels(delta)
	}
	if firstReq || len(delta) > 0 {
...
func (c *client) gossip(
	...
	updateCallback := func(key string, _ roachpb.Value) {
		select {
	...
	// stamps. Unfortunately, old versions of cockroach were not required to send
	// a response when receiving an incoming connection, so we also start a timer
	// and perform initialization after 1s if we haven't heard from the remote.
	var unregister func()
	defer func() {
		if unregister != nil {
			unregister()
