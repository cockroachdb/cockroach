...
func New(
	...
	g.mu.is.registerCallback(MakePrefixPattern(KeyStoreDescPrefix), g.updateStoreAddress)
...
func (g *Gossip) GetStoreDescriptor(
...
func (g *Gossip) updateStoreAddress(key string, content roachpb.Value) {
	...
	if log.V(1) {
		log.Infof(ctx, "updateStoreAddress called on %q with desc %+v", key, desc)
	}
