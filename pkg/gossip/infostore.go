...
+------------------+     +------------------+     +------------------+
        ...
        v                        v                        v
+------------------+     +------------------+     +------------------+
| callbackWork 1   |     | callbackWork 2   | ... | callbackWork N   |
...
// Each worker goroutine independently processes its own workQueue in the FIFO
// order.
		...
		for {
			for {
				...
				for _, w := range work {
					...
					queueDur := afterQueue.Sub(w.schedulingTime)
			...
			select {
			case <-cw.callbackCh: // new work just arrived
