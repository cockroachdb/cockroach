...
func startGossipAtAddr(
	...
	// newInsecureRPCContext generates a fresh UUID for every instance.
	// However, callers of startGossip and startGossipAtAddr want
	// multiple gossipers that can talk to each other, and thus share a
	// cluster ID. So we ignore the ID auto-generated by
	// newInsecureRPCContext and force the common ID here.
	rpcContext.ClusterID.Reset(clusterID)

	server := rpc.NewServer(rpcContext)
	g := NewTest(nodeID, rpcContext, server, stopper, registry, config.DefaultZoneConfig())
...
func startFakeServerGossips(
	...
	// Force use the common cluster ID.
	lRPCContext.ClusterID.Reset(clusterID)

	lserver := rpc.NewServer(lRPCContext)
	local := NewTest(localNodeID, lRPCContext, lserver, stopper, metric.NewRegistry(), config.DefaultZoneConfig())
	...
	// Force use the common cluster ID.
	rRPCContext.ClusterID.Reset(clusterID)
...
func gossipSucceedsSoon(
	...
	rpcContext := newInsecureRPCContext(stopper)
	// Ensure we're using the cluster ID for the entire test.
	// Since the shared code in newInsecureRPCContext generates
	// a random cluster ID, we need to reset that.
	rpcContext.ClusterID.Reset(clusterID)
...
func TestClientRegisterWithInitNodeID(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		gnode := NewTest(
			nodeID, RPCContext, server, stopper, metric.NewRegistry(), config.DefaultZoneConfig(),
		)
		g = append(g, gnode)
