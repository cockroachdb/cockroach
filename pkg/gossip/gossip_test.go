...
func TestGossipInfoStore(t *testing.T) {
	...
	g := NewTest(1, rpcContext, rpc.NewServer(rpcContext), stopper, metric.NewRegistry(), config.DefaultZoneConfig())
	slice := []byte("b")
...
func TestGossipGetNextBootstrapAddress(t *testing.T) {
	...
	g := NewTest(0, nil, server, stopper, metric.NewRegistry(), config.DefaultZoneConfig())
...
func TestGossipLocalityResolver(t *testing.T) {
	...
	g := NewTestWithLocality(1, rpcContext, rpc.NewServer(rpcContext), stopper, metric.NewRegistry(), gossipLocalityAdvertiseList, config.DefaultZoneConfig())
...
func TestGossipJoinTwoClusters(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		gnode := NewTest(
			roachpb.NodeID(i+1), rpcCtx, server, stopper, metric.NewRegistry(), config.DefaultZoneConfig())
		g = append(g, gnode)
