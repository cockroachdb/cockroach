...
func TestGossipJoinTwoClusters(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		rpcCtx := newInsecureRPCContext(stoppers[i])
		server := rpc.NewServer(rpcCtx)
		ln, err := netutil.ListenAndServeGRPC(stoppers[i], server, util.IsolatedTestAddr)
		if err != nil {
			t.Fatal(err)
		}
		...
		gnode := NewTest(
			roachpb.NodeID(i+1), rpcCtx, server, resolvers, stoppers[i], metric.NewRegistry(),
		)
		g = append(g, gnode)
