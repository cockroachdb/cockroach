...
func TestS2GGIndexBasic(t *testing.T) {
	...
	datadriven.RunTest(t, "testdata/s2_geography", func(t *testing.T, d *datadriven.TestData) string {
		...
		case "geometry":
			...
			return ""
		case "index-keys":
			keys, err := index.InvertedIndexKeys(context.TODO(), g)
			if err != nil {
				return err.Error()
			}
			var cells []string
			for _, k := range keys {
				cells = append(cells, s2.CellID(k).ToToken())
			}
			return strings.Join(cells, ",")
		case "covers":
			spans, err := index.Covers(context.TODO(), g)
			if err != nil {
				return err.Error()
			}
			return spans.String()
		case "covered-by":
			expr, err := index.CoveredBy(context.TODO(), g)
			if err != nil {
				return err.Error()
			}
			return expr.String()
