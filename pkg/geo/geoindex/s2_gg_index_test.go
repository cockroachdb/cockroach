...
// TODO(sumeer): for this and the geometry test. The current test is verbose in
// printing out the actual expressions, which are useful as a sanity check, but
// hard to validate.
...
func TestS2GGIndexBasic(t *testing.T) {
	...
	datadriven.RunTest(t, "testdata/s2_geography", func(t *testing.T, d *datadriven.TestData) string {
		...
		case "init":
			cfg := s2Config(t, d)
			index = NewS2GGIndex(S2GeographyConfig{S2Config: &cfg})
			return ""
		case "geometry":
