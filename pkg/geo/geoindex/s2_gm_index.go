...
// s2GMIndex is an implementation of GeometryIndex that uses the S2 geometry
// library.
type s2GMIndex struct {
	rc                     *s2.RegionCoverer
	minX, maxX, minY, maxY float64
}

var _ GeometryIndex = (*s2GMIndex)(nil)

// NewS2GMIndex returns an index with the given configuration. All reads and
...
func NewS2GMIndex(cfg S2GeometryConfig) *s2GMIndex {
...
func (s *s2GMIndex) InvertedIndexKeys(c context.Context, g *geo.Geometry) ([]Key, error) {
...
func (s *s2GMIndex) Covers(c context.Context, g *geo.Geometry) (UnionKeySpans, error) {
	return s.Intersects(c, g)
}

// CoveredBy implements the GeometryIndex interface.
func (s *s2GMIndex) CoveredBy(c context.Context, g *geo.Geometry) (RPKeyExpr, error) {
	gt, err := g.AsGeomT()
	if err != nil {
		return nil, err
	}
	// If the geometry exceeds the bounds, the only geometries that will cover it
	// are those indexed under the special cell.
	if s.geomExceedsBounds(gt) {
		return RPKeyExpr{Key(exceedsBoundsCellID)}, nil
	}
	r := s.s2RegionsFromPlanarGeom(gt)
	return coveredBy(c, s.rc, r), nil
}

// Intersects implements the GeometryIndex interface.
func (s *s2GMIndex) Intersects(c context.Context, g *geo.Geometry) (UnionKeySpans, error) {
...
func (s *s2GMIndex) convertToGeomTAndTryClip(g *geo.Geometry) (geom.T, bool, error) {
	gt, err := g.AsGeomT()
	if err != nil {
	...
	if s.geomExceedsBounds(gt) {
		clipped = true
		// TODO(sumeer): can't just cast from EWKB to WKB. Should keep WKB in Geometry?
		clippedWKB, err :=
			geos.ClipWKBByRect(geopb.WKB(g.EWKB()), s.minX, s.minY, s.maxX, s.maxY)
		if err != nil {
			return nil, false, err
		}
		gt = nil
		if clippedWKB != nil {
			g = geo.NewGeometry(geopb.EWKB(clippedWKB))
			if g == nil {
...
// bounds of the index.
func (s *s2GMIndex) xyExceedsBounds(x float64, y float64) bool {
...
func (s *s2GMIndex) geomExceedsBounds(g geom.T) bool {
	switch repr := g.(type) {
	case *geom.Point:
...
func (s *s2GMIndex) planarPointToS2Point(x float64, y float64) s2.Point {
	ss := (x - float64(s.minX)) / float64(s.maxX-s.minX)
	tt := (y - float64(s.minY)) / float64(s.maxY-s.minY)
	u := stToUV(ss)
	v := stToUV(tt)
	return face0UVToXYZPoint(u, v)
}

func (s *s2GMIndex) s2RegionsFromPlanarGeom(geomRepr geom.T) []s2.Region {
