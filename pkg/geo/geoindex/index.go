...
type GeographyIndex interface {
	...
	// For testing
	innerCovering(g *geo.Geography) s2.CellUnion
}
...
func (s UnionKeySpans) String() string {
	...
	for _, span := range s {
		if span.Start == span.End {
			spans = append(spans, fmt.Sprintf("%s", span.Start))
		} else {
...
// TODO: break into multiple lines based on parameter.
...
// used to find shapes that contain shape s. A covering that includes a cell c
// not completely covered by shape s could result in false negatives, since
// shape x that covers shape s could use a finer cell covering (using cells
// below c). The interior covering of shape s includes only cells covered by
// s. This is computed by RegionCoverer.InteriorCovering() and is intuitively
// what we need. But the interior covering is naturally empty for points and
// lines (and can be empty for polygons too), and an empty covering is not
// useful for constraining index lookups. We observe that leaf cells that
// intersect shape s can be used in the covering, since the covering of shape
// x must also cover these cells. This allows us to compute non-empty
// coverings for all shapes. Since this is not technically the interior
// covering, we use the term "inner covering".
...
func coveredBy(_ context.Context, rc *s2.RegionCoverer, r []s2.Region) RPKeyExpr {
	...
	// of another. Arrange these cells and their ancestors in a quadt-tree. Any cell
...
// Given a quad-tree represented by presentCells, this generates the factored
// expression represented by that quad-tree.
// Reusing an example from earlier in this file, say the final factored
// expression is:
//   I(c0) \union I(c3) \union
//    ((I(c13) \union I(c53)) \intersection
//     (I(c15) \union (I(c61) \intersection I(c64)))
// (note that RPExprElement will be c0 for I(c0) and so on).
// The quad-tree input for this expression is:
//                      c0
//                      |
//                      c3
//                      |
...
//                      c61    c64
//
// We can generate the factored expression in reverse polish notation using
