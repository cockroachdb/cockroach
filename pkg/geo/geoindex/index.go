...
func (s UnionKeySpans) toString(wrap int) string {
	b := newBuilderWithWrap(&strings.Builder{}, wrap)
	for i, span := range s {
...
// builderWithWrap is a strings.Builder that approximately wraps at a
// certain number of characters. Newline characters should only be
// written using tryWrap and doWrap.
type builderWithWrap struct {
	*strings.Builder
	wrap     int
	lastWrap int
}

func newBuilderWithWrap(b *strings.Builder, wrap int) *builderWithWrap {
	return &builderWithWrap{
		Builder:  b,
		wrap:     wrap,
		lastWrap: b.Len(),
	}
...
func (b *builderWithWrap) tryWrap() {
	if b.Len()-b.lastWrap > b.wrap {
		b.doWrap()
	}
}

func (b *builderWithWrap) doWrap() {
	fmt.Fprintln(b)
	b.lastWrap = b.Len()
}
