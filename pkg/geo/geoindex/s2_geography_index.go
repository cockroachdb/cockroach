...
func (i *s2GeographyIndex) DWithin(
	...
	// on the sphere by the angle. The spheroid can have a flattening that can create a
	// maximum error wrt the sphere calculation of (2 * spheroid.Flattening)%. For
	// now we just expand distanceMeters by 5% which is a very safe margin.
	angle := s1.Angle(1.05 * distanceMeters / projInfo.Spheroid.SphereRadius)
