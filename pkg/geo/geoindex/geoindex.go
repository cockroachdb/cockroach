...
//
// TODO(dan): This currently refers to Geography. When we add Geometry indexing,
// should that be Geometry vs Geography variants of each of these functions or a
// GeometryIndex and GeographyIndex?
type Index interface {
	// InvertedIndexKeys returns the keys to store this object under when adding
	// it to the index.
	InvertedIndexKeys(context.Context, *geo.Geography) ([]Key, error)

	// Contains is an index-accelerated execution of ST_Contains.
	Contains(context.Context, *geo.Geography) (KeySpanQuery, error)

	// TildeOperator is an index-accelerated execution of the ~ operator.
	TildeOperator(context.Context, *geo.Geography) (KeySpanQuery, error)
}
...
// Key is one entry under which a geospatial figure is stored on behalf of an
// Index.
type Key uint64
...
// KeySpanQuery is an execution plan for an index-accelerated spatial query. See
// Index for details.
type KeySpanQuery struct {
	// Union is the set of indexed keys to retrieve and combine via set union.
	// Duplicate primary keys will not be retrieved by any individual span, but
	// they may be present if more than one span is retrieved. Additionally, false
	// positives may be retrieved. These are handled in a post-retrieval step by
	// Filter.
	Union []KeySpan
	...
	Filter func(*geo.Geography, *geo.Geography) (bool, error)
