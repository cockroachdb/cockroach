...
func (it *GeomTIterator) next() (geom.T, bool, error) {
	...
	case *geom.GeometryCollection:
		for {
			...
			// Use the geomCollectionIdx instead when iterating over a sub-shape of
			// GeometryCollections.
			subIt := GeomTIterator{g: t.Geom(it.idx), emptyBehavior: it.emptyBehavior, idx: it.geomCollectionIdx}
			ret, next, err := subIt.next()
			if err != nil {
				return nil, false, err
			}
			if next {
				it.geomCollectionIdx = subIt.idx
				return ret, next, nil
			}
			// Reset and move to the next item in the collection.
			it.idx++
			it.geomCollectionIdx = 0
		}
	default:
