...
func (s *Spheroid) Project(point s2.LatLng, distance float64, azimuth float64) s2.LatLng {
	var lat, lng C.double

	C.geod_direct(
		&s.cRepr,
		C.double(math.Pi*point.Lat/180.0),
		C.double(math.Pi*point.Lng/180.0),
	...
	return s2.LatLng{
		...
		Lng: s1.Angle(float64(lng) * math.Pi / 180.0)}
}
