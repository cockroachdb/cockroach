...
// MarshalOptionPrecision sets the XY precision for the TWKB.
...
// MarshalOptionPrecision sets the Z precision for the TWKB.
...
// MarshalOptionPrecision sets the M precision for the TWKB.
...
func typeAndPrecisionHeaderByte(t geom.T, o marshalOptions) (byte, error) {
	...
	return byte(typ&0x0F) | (byte(precisionZigZagged&0x0F) << 4), nil
}
...
func zigzagInt8(x int8) byte {
	if x < 0 {
		return byte((uint8(-1-x) << 1) | 0x01)
	}
	return byte(uint8(x) << 1)
}
