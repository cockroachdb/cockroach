...
func TestShiftLongitude(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			input:    geom.NewPointFlat(geom.XY, []float64{180, -90}),
			expected: geom.NewPointFlat(geom.XY, []float64{180, -90}),
		},
		{
			desc:     "shift longitude for a 2D point that has longitude less than 0",
			input:    geom.NewPointFlat(geom.XY, []float64{-1, 60}),
			expected: geom.NewPointFlat(geom.XY, []float64{359, 60}),
		},
		{
			desc:     "shift longitude for a 2D point that has longitude greater than 180",
			input:    geom.NewPointFlat(geom.XY, []float64{181, 60}),
			expected: geom.NewPointFlat(geom.XY, []float64{-179, 60}),
		},
		{
		...
		},
		{
			...
			input: geom.NewGeometryCollection().MustPush(
				...
				geom.NewPolygonFlat(geom.XY, []float64{5, 5, 270, 90, -118, -85, 5, 5}, []int{8}),
			),
			expected: geom.NewGeometryCollection().MustPush(
				...
				geom.NewPolygonFlat(geom.XY, []float64{5, 5, -90, 90, 242, -85, 5, 5}, []int{8}),
			),
		},
		{
