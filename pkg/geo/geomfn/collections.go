...
//
// This is the binary version of `ST_Collect()`, but since it's not possible to have an aggregate
// and non-aggregate function with the same name (different args), this is not used. Code is left
// behind for when we add support for this. Be sure to handle NULL args when adding the builtin for
// this, where it should return the non-NULL arg unused like PostGIS.
