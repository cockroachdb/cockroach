...
func TestVoronoiPolygons(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			actual, err := VoronoiDiagram(tc.args.a, tc.args.tol, tc.args.onlyEdges)
...
func TestVoronoiPolygonsWithEnvelope(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			actual, err := VoronoiDiagramWithEnvelope(tc.args.a, tc.args.b, tc.args.tol, tc.args.onlyEdges)
			if tc.expectedErr != nil && tc.expectedErr.Error() != err.Error() {
				t.Errorf("VoronoiPolygons() error = %v, wantErr %v", err, tc.expectedErr)
				return
			}
