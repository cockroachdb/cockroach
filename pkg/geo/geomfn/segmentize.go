...
// return resultant points as flat []float64. Points are inserted in such a
...
func segmentizeCoords(a geom.Coord, b geom.Coord, maxSegmentLength float64) []float64 {
	distanceBetweenPoint := math.Sqrt(math.Pow(a.X()-b.X(), 2) + math.Pow(b.Y()-a.Y(), 2))
	allSegmentizedCoordinates := a.Clone()
	if maxSegmentLength < distanceBetweenPoint {
		// numberOfSegmentToCreate represent the total number of segments
		// in which given two coordinates will be divided.
		numberOfSegmentToCreate := int(math.Ceil(distanceBetweenPoint / maxSegmentLength))
