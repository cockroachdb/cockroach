	...
	case *geom.Polygon:
		...
		for i := 0; i < t.NumLinearRings(); i++ {
			...
			if lr.NumCoords() < 4 {
				...
				// return nil (we should have 4 points from simplifySimplifiableShape otherwise).
				if i == 0 {
					return nil, nil
				}
...
func simplifySimplifiableShape(t simplifiableShape, tolerance float64, minPoints int) []float64 {
	...
	if tolerance == 0 {
		...
		for i := 1; i < t.NumCoords()-1; i++ {
			...
			// Let AB be the line from the prev coord to the coord one ahead.
			// Let AC by the line from the prev coord to the current coord.
			ac := coordSub(currCoord, prevCoord)
			ab := coordSub(nextCoord, prevCoord)
			...
			if coordCross(ab, ac) == 0 {
				// Project B onto line AC. If the projection lies on the edge bounded by
		...
		recurse = func(startIdx, endIdx int) {
			...
			// If there is no point to split at, keep the start and end point.
