...
func addMeasureToMultiLineString(
	...
	newMultiLineString := geom.NewMultiLineString(augmentLayoutWithM(multiLineString.Layout()))
	...
	return newMultiLineString.SetSRID(multiLineString.SRID()), nil
}
...
func addMeasureToLineString(
	...
	if lineString.Empty() {
		return geom.NewLineString(augmentLayoutWithM(lineString.Layout())), nil
	}
	...
	newLineString, err := geom.NewLineString(augmentLayoutWithM(lineString.Layout())).SetCoords(lineCoords)
	if err != nil {
		return lineString, err
	}

	return newLineString.SetSRID(lineString.SRID()), nil
}
