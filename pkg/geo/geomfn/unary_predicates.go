...
func IsClosed(g *geo.Geometry) (bool, error) {
	...
	return isClosedFromGeomT(t), nil
}

// isClosedFromGeomT returns whether the given geom.T is closed, recursing
// into collections.
func isClosedFromGeomT(t geom.T) bool {
	if t.Empty() {
		return false
	}
	switch t := t.(type) {
	case *geom.LinearRing:
		return t.Coord(0).Equal(t.Layout(), t.Coord(t.NumCoords()-1))
	case *geom.LineString:
		return t.Coord(0).Equal(t.Layout(), t.Coord(t.NumCoords()-1))
	case *geom.MultiLineString:
		for i := 0; i < t.NumLineStrings(); i++ {
			if !isClosedFromGeomT(t.LineString(i)) {
				return false
			}
		}
	case *geom.Polygon:
		for i := 0; i < t.NumLinearRings(); i++ {
			if !isClosedFromGeomT(t.LinearRing(i)) {
				return false
			}
		}
	case *geom.MultiPolygon:
		for i := 0; i < t.NumPolygons(); i++ {
			if !isClosedFromGeomT(t.Polygon(i)) {
				return false
			}
		}
	case *geom.GeometryCollection:
		for _, g := range t.Geoms() {
			if !isClosedFromGeomT(g) {
				return false
			}
		}
	}
	// Any non-linear, non-collection type is considered closed.
	return true
}
