...
func (c *geomDistanceCalculator) PointIntersectsLinearRing(
	...
	return findPointSideOfLinearRing(point, linearRing) == insideLinearRing ||
		findPointSideOfLinearRing(point, linearRing) == onLinearRing
}
...
type PointPolygonRelationType bool

const (
	// PointPolygonIntersects is the relationship where a (multi)point
	// intersects a (multi)polygon.
	PointPolygonIntersects PointPolygonRelationType = true
	// PointPolygonWithin is the relationship where a (multi)point
	// is within a (multi)polygon.
	PointPolygonWithin PointPolygonRelationType = false
)
...
pointOuterLoop:
	for {
		...
		pointGeometry, err := geo.MakeGeometryFromGeomT(point)
		if err != nil {
			return false, err
		}
		...
		for {
			...
			polygonGeometry, err := geo.MakeGeometryFromGeomT(polygon)
			if err != nil {
				return false, err
			}
			intersects, err := PointIntersectsPolygon(pointGeometry, polygonGeometry)
			if err != nil {
				return false, err
			}
			if intersects {
		...
		// Case where a point does not intersect with any polygon.
		if relationType == PointPolygonWithin {
			return false, nil
		}
	}
	return ret, nil
...
func PointIntersectsPolygon(point geo.Geometry, polygon geo.Geometry) (bool, error) {
	// Convert point from a geo.Geometry to a *geodist.Point.
	pointT, err := point.AsGeomT()
	if err != nil {
		return false, err
	}
	_, ok := pointT.(*geom.Point)
	if !ok {
		return false, errors.Newf("first geometry passed to PointIntersectsPolygon must be a point")
	}
	pointGeodistShape, err := geomToGeodist(pointT)
	if err != nil {
		return false, err
	}
