...
// forceLayout forces a geom.T into the given layout and uses the specified default Z and M values as needed.
func forceLayout(t geom.T, layout geom.Layout, defaultZ float64, defaultM float64) (geom.T, error) {
	...
	case *geom.GeometryCollection:
		...
		return ret, nil
	case *geom.Point:
		return geom.NewPointFlat(layout, forceFlatCoordsLayout(t, layout, defaultZ, defaultM)).SetSRID(t.SRID()), nil
	case *geom.LineString:
		return geom.NewLineStringFlat(layout, forceFlatCoordsLayout(t, layout, defaultZ, defaultM)).SetSRID(t.SRID()), nil
	case *geom.Polygon:
