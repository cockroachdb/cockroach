...
func LineSubString(g geo.Geometry, start, end float64) (geo.Geometry, error) {
	...
	for i := range lineString.Coords() {
		currentDistance, err := geom.NewLineString(geom.XY).SetCoords(lineString.Coords()[0 : i+1])
		if err != nil {
			return geo.Geometry{}, err
		}
		...
		if currentDistance.Length() >= endDistance {

			if len(newFlatCoords) == 0 {
				coords, err := getInterpolateLine(g, startDistance)
				if err != nil {
					return geo.Geometry{}, err
				}
			...
			coords, err := getInterpolateLine(g, endDistance)
			if err != nil {
				return geo.Geometry{}, err
			}
			newFlatCoords = append(newFlatCoords, coords...)
		...
		if currentDistance.Length() >= startDistance {
			if len(newFlatCoords) == 0 {
				coords, err := getInterpolateLine(g, startDistance)
				if err != nil {
					return geo.Geometry{}, err
				...
				if startDistance != 0 {
					newFlatCoords = append(newFlatCoords, currentDistance.Coord(i)...)
				}
			} else {
...
func getInterpolateLine(g geo.Geometry, distance float64) ([]float64, error) {
