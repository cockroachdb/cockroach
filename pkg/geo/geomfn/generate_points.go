...
func polygonGeometryToPoints(g geo.Geometry, npoints int, seed int64) (*geom.MultiPoint, error) {
	...
	for i := 0; i < sampleWidth; i++ {
		for j := 0; j < sampleHeight; j++ {
			cells[i*int(sampleHeight)+j] = geom.Coord{float64(i), float64(j)}
		}
	}
...
func multipolygonGeometryToPoints(g geo.Geometry, npoints int, seed int64) (*geom.MultiPoint, error) {
	results := geom.NewMultiPoint(geom.XY)
