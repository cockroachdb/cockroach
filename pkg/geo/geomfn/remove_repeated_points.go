...
func removeRepeatedPointsFromGeomT(t geom.T, tolerance2 float64) (geom.T, error) {
	...
	case *geom.LineString:
		if coords, modified := removeRepeatedCoords(t.Layout(), t.Coords(), tolerance2); modified {
			return t.SetCoords(coords)
		}
	case *geom.Polygon:
		if coords, modified := removeRepeatedCoords2(t.Layout(), t.Coords(), tolerance2); modified {
			return t.SetCoords(coords)
		}
	...
	case *geom.MultiPolygon:
		if coords, modified := removeRepeatedCoords3(t.Layout(), t.Coords(), tolerance2); modified {
			return t.SetCoords(coords)
		}
...
func removeRepeatedCoords(
	layout geom.Layout, coords []geom.Coord, tolerance2 float64,
) ([]geom.Coord, bool) {
	modified := false
	switch tolerance2 {
	case 0:
...
func removeRepeatedCoords2(
	layout geom.Layout, coords2 [][]geom.Coord, tolerance2 float64,
) ([][]geom.Coord, bool) {
	modified := false
	for i, coords := range coords2 {
		if c, m := removeRepeatedCoords(layout, coords, tolerance2); m {
			coords2[i] = c
			modified = true
		}
	}
...
func removeRepeatedCoords3(
	layout geom.Layout, coords3 [][][]geom.Coord, tolerance2 float64,
) ([][][]geom.Coord, bool) {
	modified := false
