...
// forceClockwiseOrientation reverses the Polygon's rings that are CCW.
// Works for MultiPolygons and GeometryCollections too.
func forceClockwiseOrientation(gt geom.T) {
	switch gt := gt.(type) {
	case *geom.Polygon:
		if gt.Empty() {
			return
		}
		externalRing := gt.LinearRing(0)
		if isCCW(externalRing) {
			externalRing.Reverse()
		}
		for i := 1; i < gt.NumLinearRings(); i++ {
			internalRing := gt.LinearRing(i)
			if !isCCW(internalRing) {
				internalRing.Reverse()
			}
		}
	case *geom.MultiPolygon:
		for i := 0; i < gt.NumPolygons(); i++ {
			forceClockwiseOrientation(gt.Polygon(i))
		}
	case *geom.GeometryCollection:
		for i := 0; i < gt.NumGeoms(); i++ {
			forceClockwiseOrientation(gt.Geom(i))
		}
	}
}

type areaCalculatable interface {
	Area() float64
}

func isCCW(geometry areaCalculatable) bool {
