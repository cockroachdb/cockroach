...
// VoronoiDiagramWithEnvelope Computes the Voronoi Diagram from the vertices of the supplied EWKBs with envelope EWKBs.
func VoronoiDiagramWithEnvelope(a geopb.EWKB, env geopb.EWKB, tolerance float64, onlyEdges bool) (geopb.EWKB, error) {
	g, err := ensureInitInternal()
	if err != nil {
		return nil, err
	}
...
// VoronoiDiagramWithEnvelope Computes the Voronoi Diagram from the vertices of the supplied EWKBs.
func VoronoiDiagram(a geopb.EWKB, tolerance float64, onlyEdges bool) (geopb.EWKB, error) {
	g, err := ensureInitInternal()
	if err != nil {
		return nil, err
	}
	...
	if err := statusToError(
		C.CR_GEOS_VoronoiDiagram(g, goToCSlice(a), C.CR_GEOS_Slice{nil, 0}, C.double(tolerance), C.int(flag), &cEWKB),
	); err != nil {
		return nil, err
	}
	return cStringToSafeGoBytes(cEWKB), nil
