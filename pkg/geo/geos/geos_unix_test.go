...
func TestBootstrap(t *testing.T) {
	t.Run("test invalid bootstrap paths", func(t *testing.T) {
		holder := bootstrap([]string{"/invalid/path"})
		assert.False(t, holder.valid)
	})

	t.Run("test valid bootstrap paths", func(t *testing.T) {
		holder := bootstrap(defaultGEOSLocations)
		assert.True(t, holder.valid)
		fmt.Printf("%#v\n", defaultGEOSLocations)
	})
}
