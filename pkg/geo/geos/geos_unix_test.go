...
func TestInitCockroachGEOSLib(t *testing.T) {
	t.Run("test invalid initCockroachGEOSLib paths", func(t *testing.T) {
		ret := initCockroachGEOSLib([]string{"/invalid/path"})
		assert.Error(t, validOrError(ret))
	})

	t.Run("test valid initCockroachGEOSLib paths", func(t *testing.T) {
		ret := initCockroachGEOSLib(defaultGEOSLocations)
