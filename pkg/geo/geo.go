...
// SpatialObjectFromEWKBRaw creates a geopb.SpatialObject from an EWKB
// without doing any SRID based checks.
// You most likely want SpatialObjectFromEWKB instead.
func SpatialObjectFromEWKBRaw(in geopb.EWKB) (geopb.SpatialObject, error) {
...
// GeometryFromEWKBRaw returns a new Geometry from an EWKB, without any SRID checks.
// You should only do this if you trust the EWKB is setup correctly.
// You must likely want geo.NewGeometry(SpatialObjectFromEWKB(ewkb, ...)) instead.
func GeometryFromEWKBRaw(ewkb geopb.EWKB) (*Geometry, error) {
	base, err := SpatialObjectFromEWKBRaw(ewkb)
	if err != nil {
		return nil, err
	}
...
// MustGeometryFromEWKBRaw behaves as GeometryFromEWKBRaw, but panics if an error occurs.
func MustGeometryFromEWKBRaw(ewkb geopb.EWKB) *Geometry {
	ret, err := GeometryFromEWKBRaw(ewkb)
	if err != nil {
		panic(err)
	}
	return ret
...
// GeographyFromEWKBRaw returns a new Geography from an EWKB, without any SRID checks.
// You should only do this if you trust the EWKB is setup correctly.
// You must likely want geo.NewGeometry(SpatialObjectFromEWKB(ewkb, ....)) instead.
func GeographyFromEWKBRaw(ewkb geopb.EWKB) (*Geography, error) {
	base, err := SpatialObjectFromEWKBRaw(ewkb)
	if err != nil {
		return nil, err
	}
...
// MustGeographyFromEWKBRaw behaves as GeographyFromEWKBRaw, but panics if an error occurs.
func MustGeographyFromEWKBRaw(ewkb geopb.EWKB) *Geography {
	ret, err := GeographyFromEWKBRaw(ewkb)
	if err != nil {
		panic(err)
	}
	return ret
