...
func TestSpatialObjectFromWKB(t *testing.T) {
	testCases := []struct {
		desc          string
		b             []byte
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			ret, err := SpatialObjectFromWKB(tc.b, tc.defaultSRID)
			if tc.expectedError != "" {
				require.Error(t, err)
...
func TestSpatialObjectFromEWKB(t *testing.T) {
	testCases := []struct {
		desc        string
		b           []byte
		defaultSRID geopb.SRID
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			ret, err := SpatialObjectFromEWKB(tc.b, tc.defaultSRID, tc.overwrite)
			require.NoError(t, err)
			require.Equal(t, tc.expected, ret)
		})
	}
...
func TestSpatialObjectFromEWKT(t *testing.T) {
	testCases := []struct {
		desc        string
		wkt         geopb.EWKT
		defaultSRID geopb.SRID
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			ret, err := SpatialObjectFromEWKT(tc.wkt, tc.defaultSRID, tc.overwrite)
			require.NoError(t, err)
			require.Equal(t, tc.expected, ret)
		})
	}
