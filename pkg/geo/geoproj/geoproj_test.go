...
func TestProject(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			from:            "+proj=merc +a=6378137 +b=6378137 +lat_ts=0.0 +lon_0=0.0 +x_0=0.0 +y_0=0 +k=1.0 +units=m +nadgrids=@null +wktext +no_defs",
	...
	t.Run("test error handling", func(t *testing.T) {
		err := Project("+bad", "+bad", []float64{1}, []float64{2}, []float64{3})
		require.Error(t, err)
	})
}
