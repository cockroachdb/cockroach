...
func distanceSpheroidRegions(
	...
	for _, aRegion := range aRegions {
		for _, bRegion := range bRegions {
			// The way PostGIS evaluated the distance between spheroid regions is to
			// find the two closest points using the shortest spherical distance,
			// then finding the spheroid distance between them.
			//
			// If there are multiple collections of shapes to compare against (e.g. POINT a against MULTIPOINT b, c)
			// then it will find the minimum spheroid distance between all these shapes
			// and use the minimum spheroid distance between them.
			// e.g. for POLYGON a against MULTIPOLYGON (b, c), it will calculate the minimum
			// spherical distance between a<->b and a<->c, and use the closest points between
			// a<->b and a<->c to find the spheroid distance and taking the minimum of that.
			//
			// This is technically incorrect, as we are judging the minimum distance as
...
func (u *spheroidMinDistanceUpdater) onPolylineToPolygon(a *s2.Polyline, b *s2.Polygon) bool {
	...
	// Now we are guaranteed that there is at least one point is inside the exterior ring.
	// Keep track if any lines are inside a hole.
	insideHole := false
	for _, hole := range b.Loops()[1:] {
		...
		// We should only ever be inside one hole. We can terminate here.
		if insideHole {
			break
		}
	}
	...
	// has already been calculated, so if we're not early exiting from the above checks
	// then we have not found the condition where we should early exit.
	if insideHole {
		return false
	}
...
func (u *spheroidMinDistanceUpdater) onPolygonToPolygon(a *s2.Polygon, b *s2.Polygon) bool {
	...
	// If there is at least one point on the the exterior ring of B that is also the exterior ring
	...
	// This applies if one point on the exterior ring of A is on the exterior ring of B.
	// As such, if we detect this, only compare exterior rings.
	...
	// If we find that any point of the exterior ring of polygon a is inside one of the
	// holes of polygon b, then with the same reasoning as above we can compare the hole
	// and the exterior ring only.
