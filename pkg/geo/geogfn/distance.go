...
func distanceSpheroidRegions(
	...
	for _, aRegion := range aRegions {
		for _, bRegion := range bRegions {
			...
			found, err := minDistanceUpdater.onRegionToRegion(aRegion, bRegion)
			if err != nil {
				return 0, err
			}
...
func (u *spheroidMinDistanceUpdater) minDistance() float64 {
	return spheroidInverse(u.spheroid, u.minEdge.V0, u.minEdge.V1)
}
...
// the closest two edges. This is in line with PostGIS, which are are aiming
...
func (u *spheroidMinDistanceUpdater) updateMinDistance(a s2.Point, b s2.Point) bool {
	...
	if sphereDistance < u.minD {
		...
		if u.stopAfterLE > 0 && sphereDistance <= u.stopAfterLE {
			return true
		}
	}
	return false
	...
	for aEdgeIdx := 0; aEdgeIdx < a.NumEdges(); aEdgeIdx++ {
		...
		for bEdgeIdx := 0; bEdgeIdx < b.NumEdges(); bEdgeIdx++ {
			...
			for _, toCheck := range []struct {
				...
				if u.updateMinDistance(toCheck.vertex, toCheck.edge.V0) || u.updateMinDistance(toCheck.vertex, toCheck.edge.V1) {
					return true
				}
...
func (u *spheroidMinDistanceUpdater) onPolylineToPolygon(a *s2.Polyline, b *s2.Polygon) bool {
	// If any of the vertexes of a is contains by polygon b, return.
	for _, p := range *a {
	...
	// Now all points are outside the polygon, but they may intersect.
	// Fall back to the shape edge detection.
	for _, loop := range b.Loops() {
		if u.onShapeEdgesToShapeEdges(a, loop) {
			return true
		}
	}
	return false
...
func (u *spheroidMinDistanceUpdater) onPolygonToPolygon(a *s2.Polygon, b *s2.Polygon) bool {
	// If two polygons intersect, then we can say the distance is 0.
	if a.Intersects(b) {
		return u.onIntersects()
	}
	// Fallback to shape edge detection.
	// Edge is really inefficient for polygons, so do the iterations.
	for _, aLoop := range a.Loops() {
		for _, bLoop := range b.Loops() {
			if u.onShapeEdgesToShapeEdges(aLoop, bLoop) {
				return true
			}
		}
	}
