...
func polygonIntersectsPolyline(a *s2.Polygon, b *s2.Polyline) bool {
	// Check if the polygon contains any point of the line b.
	for _, point := range *b {
		if a.IntersectsCell(s2.CellFromPoint(point)) {
			return true
		}
	}
	// Avoid using NumEdges / Edge of the Polygon type as it is not O(1).
	// Now we have to check whether anything intersects but points are outside
	// the polygon. This is possible if an outside points connects to an
	// outside point through the polygon.
