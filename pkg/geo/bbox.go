...
func boundingBoxFromGeomTGeographyType(g geom.T) (s2.Rect, error) {
	...
	case *geom.LineString:
		return geogLineBBox(g, false /* isRing */), nil
	case *geom.MultiLineString:
		for i := 0; i < g.NumLineStrings(); i++ {
			rect = rect.Union(geogLineBBox(g.LineString(i), false /* isRing */))
		}
	case *geom.Polygon:
		for i := 0; i < g.NumLinearRings(); i++ {
			rect = rect.Union(geogLineBBox(g.LinearRing(i), true /* isRing */))
		}
...
func geogLineBBox(g flatGeom, isRing bool) s2.Rect {
