...
// ReplicaTypeEphemeralLearner returns a LEARNER_EPHEMERAL pointer suitable for use in
// a nullable proto field.
func ReplicaTypeEphemeralLearner() *ReplicaType {
	t := LEARNER_EPHEMERAL
	return &t
}
...
func predEphemeralLearner(rDesc ReplicaDescriptor) bool {
...
//
// TODO(tbg,aayush): go through the callers and figure out the few which want a
...
// For this reason, the replicate queue always first removes any ephemeral
// learners it sees before doing anything else. We could instead try to finish
// off the learner snapshot and promotion, but this is more complicated and it's
// not yet clear the efficiency win is worth it.
//
...
// are handled by retrying if an ephemeral learner disappears during the
// snapshot/promotion.
//
...
// by rolling back the addition of the ephemeral learner.
//
...
// try to send snapshots to ephemeral learners if there is already a snapshot to
...
//
// Merges are blocked if either side has an ephemeral learner (to avoid working
// out the edge cases) but it's historically turned out to be a bad idea to get
// in the way of splits, so we allow them even when some of the replicas are
// ephemeral learners. This orphans an ephemeral learner on each side of the
