...
func (d ReplicaSet) Contains(rDesc ReplicaDescriptor) bool {
	descs := d.Descriptors()
	for i := range descs {
		if descs[i].StoreID == rDesc.StoreID && descs[i].NodeID == rDesc.NodeID {
			return true
		}
	}
	return false
...
func (d ReplicaSet) ReplicationTargets() (out []ReplicationTarget) {
	descs := d.Descriptors()
	out = make([]ReplicationTarget, 0, len(descs))
	for i := range descs {
		out = append(out, ReplicationTarget{NodeID: descs[i].NodeID, StoreID: descs[i].StoreID})
	}
	return out
}
