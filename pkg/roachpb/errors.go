...
func (e *LeaseRejectedError) SafeFormatError(p errors.Printer) (next error) {
	p.Printf("cannot replace lease %s with %s: %s", e.Existing, e.Requested.String(), e.Message)
	return nil
}
...
func (e *TransactionStatusError) SafeFormatError(p errors.Printer) (next error) {
	if e.MsgRedactable != "" {
		p.Printf("TransactionStatusError: %s (%s)", e.MsgRedactable, e.Reason)
	} else {
		p.Printf("TransactionStatusError: %s (%s)", e.Msg, e.Reason)
	}
	return nil
}
...
func (e *RangeFeedRetryError) SafeFormatError(p errors.Printer) (next error) {
	p.Printf("retry rangefeed (%s)", e.Reason)
	return nil
}
