...
func (ba *BatchRequest) CreateReply() *BatchResponse {
	...
	var buf33 []AdminScatterResponse
	var buf34 []AddSSTableResponse
	var buf35 []RecomputeStatsResponse
	var buf36 []RefreshResponse
	var buf37 []RefreshRangeResponse
	...
	for i, r := range ba.Requests {
		...
		case r.AdminScatter != nil:
			if buf33 == nil {
				buf33 = make([]AdminScatterResponse, counts[33])
			}
			br.Responses[i].AdminScatter = &buf33[0]
			buf33 = buf33[1:]
		case r.AddSstable != nil:
			if buf34 == nil {
				buf34 = make([]AddSSTableResponse, counts[34])
			}
			br.Responses[i].AddSstable = &buf34[0]
			buf34 = buf34[1:]
		case r.RecomputeStats != nil:
			if buf35 == nil {
				buf35 = make([]RecomputeStatsResponse, counts[35])
			}
			br.Responses[i].RecomputeStats = &buf35[0]
			buf35 = buf35[1:]
		case r.Refresh != nil:
			if buf36 == nil {
				buf36 = make([]RefreshResponse, counts[36])
			}
			br.Responses[i].Refresh = &buf36[0]
			buf36 = buf36[1:]
		case r.RefreshRange != nil:
			if buf37 == nil {
				buf37 = make([]RefreshRangeResponse, counts[37])
			}
			br.Responses[i].RefreshRange = &buf37[0]
			buf37 = buf37[1:]
		default:
			panic(fmt.Sprintf("unsupported request: %+v", r))
		}
