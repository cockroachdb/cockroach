...
func (ba *BatchRequest) CreateReply() *BatchResponse {
	...
	var buf38 []UpdateResponse
	var buf39 []UpdateRangeResponse
	...
	for i, r := range ba.Requests {
		...
		case r.Update != nil:
			if buf38 == nil {
				buf38 = make([]UpdateResponse, counts[38])
			}
			br.Responses[i].Update = &buf38[0]
			buf38 = buf38[1:]
		case r.UpdateRange != nil:
			if buf39 == nil {
				buf39 = make([]UpdateRangeResponse, counts[39])
			}
			br.Responses[i].UpdateRange = &buf39[0]
			buf39 = buf39[1:]
		default:
			panic(fmt.Sprintf("unsupported request: %+v", r))
		}
