...
func (v Version) String() string {
	if v.Unstable == 0 {
		...
		// We expect patch always to be zero, but handle here to avoid
		// confusion if that assumption is violated.
		return fmt.Sprintf("%d.%d.%d", v.Major, v.Minor, v.Patch)
	}
	return fmt.Sprintf("%d.%d.%d.%d", v.Major, v.Minor, v.Patch, v.Unstable)
}
