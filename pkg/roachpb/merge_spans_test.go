		...
		for i := 0; i < iterations; i++ {
			...
			for i := 0; i < max; i++ {
				...
				if before.ContainsKey(k) {
					if covered.ContainsKey(k) {
						if after.ContainsKey(k) {
							t.Fatalf("before and covering contain %s after should not", k)
						}
					} else if !after.ContainsKey(k) {
						t.Fatalf("before %v contains %s, covering %v does not, after %v should still contain it", before, k, covered, after)
					}
				} else if after.ContainsKey(k) {
					t.Fatalf("before %v does not contain %s, after %v does", before, k, after)
				}
			}
		}
	})
