...
func (s *StatementStatistics) Add(other *StatementStatistics) {
	...
	statCollectionCount := s.StatCollectionCount
	if statCollectionCount == 0 && other.StatCollectionCount == 0 {
		// If both are zero, artificially set the receiver's count to one to avoid
		// division by zero in Add.
		statCollectionCount = 1
	}
	s.BytesSentOverNetwork.Add(other.BytesSentOverNetwork, s.StatCollectionCount, statCollectionCount)
	s.MaxMemUsage.Add(other.MaxMemUsage, s.StatCollectionCount, statCollectionCount)
