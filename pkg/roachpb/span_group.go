...
// Add will attempt to add the provided Span to the SpanGroup,
// returning whether the addition increased the span of the group
// or not.
func (g *SpanGroup) Add(s Span) bool {
	g.checkInit()
	ret := g.rg.Add(s2r(s))
	return ret
}

// AddAll will attempt to add the provided Spans to the SpanGroup,
// returning whether the addition(s) increased the span of the group
// or not.
func (g *SpanGroup) AddAll(spans []Span) bool {
	if len(spans) == 0 {
		return false
	}
	ret := false
...
func s2r(s Span) interval.Range {
	return interval.Range{
		...
		End:   interval.Comparable(s.EndKey),
	}
}
