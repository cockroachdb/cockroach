...
// Batch request that caused the retry. And it's not used if a new Transaction
// needs to be created; in this case the priority will be initialized later,
// before running the first batch in the new retry. Feels like something more
// consistent should be done about this.
func PrepareTransactionForRetry(
	retryErr *InternalRetryableTxnError, pri UserPriority, isolation enginepb.IsolationType, name string,
) Transaction {
