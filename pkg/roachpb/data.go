...
func (v *Value) allocRawBytes(size int) {
...
func (v *Value) SetBytes(b []byte) {
	v.allocRawBytes(headerSize + len(b))
	copy(v.dataBytes(), b)
...
func (v *Value) SetString(s string) {
	v.allocRawBytes(headerSize + len(s))
	copy(v.dataBytes(), s)
	v.setTag(ValueType_BYTES)
}
...
func (v *Value) SetInt(i int64) {
	v.allocRawBytes(headerSize + binary.MaxVarintLen64)
...
func (v *Value) SetProto(msg protoutil.Message) error {
	...
	// []byte and copying.
	v.allocRawBytes(headerSize + msg.Size())
...
func (v *Value) SetTime(t time.Time) {
	...
	v.allocRawBytes(headerSize + encodingSizeOverestimate)
...
func (v *Value) SetDuration(t duration.Duration) error {
	var err error
	v.allocRawBytes(headerSize + encoding.EncodedDurationMaxLen)
...
func (v *Value) SetBitArray(t bitarray.BitArray) {
	...
	v.allocRawBytes(headerSize + encoding.NonsortingUvarintMaxLen + 8*len(words))
...
func (v *Value) SetDecimal(dec *apd.Decimal) error {
	...
	v.allocRawBytes(headerSize + decSize)
...
func (v *Value) SetTuple(data []byte) {
	v.allocRawBytes(headerSize + len(data))
