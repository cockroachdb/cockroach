...
func PrepareTransactionForRetry(
	...
	case *TransactionRetryError:
		...
		if tErr.Reason == RETRY_SERIALIZABLE {
			...
			// as well as transactions that hit read-write contention and can't bump
			// read timestamp.
			...
			// from succeeding if read timestamp can't be bumped again.
			// The tradeoff here is that transactions that failed because they were
			// waiting on locksor were slowed down in their first epoch for any other
