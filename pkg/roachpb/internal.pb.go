...
type InternalTimeSeriesData struct {
	...
	// data set. This column is always populated.
	...
	// period. This column is always populated.
	...
	// during this sample period. This column is optional, and is only populated
	// for samples which are "rollups" of multiple measurements.
	...
	// this sample period. This column is optional, and is only populated for
	// samples which are "rollups" of multiple measurements; if missing, its value
	...
	// during this sample period. This column is optional, and is only populated
	// for samples which are "rollups" of multiple measurements; ; if missing, its
	// value for all samples is equal to "last".
	...
	// taken during this sample period. This column is optional, and is only
	// populated for samples which are "rollups" of multiple measurements; if
	// missing, its value for all samples is equal to "last".
	...
	// period. This column is optional, and is only populated for samples which
	// are "rollups" of multiple measurements; if missing, its value for all
