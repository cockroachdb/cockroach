...
func TestErrorRedaction(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			expect: "ReadWithinUncertaintyIntervalError: read at time 0,0 encountered previous write with future timestamp 0,0 (local=0,0) within uncertainty interval `t <= (local=0,0, global=0,0)`; observed timestamps: []",
		},
		{
