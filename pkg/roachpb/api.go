...
// IsBlindWrite returns true iff the request is a blind-write. A request
// is a blind-write if it is a write that does not observe any key-value
// state when modifying that state (such as puts and deletes). This is
// in contrast with read-write requests, which do observe key-value
// state when modifying the state and may base their modifications off
// of this existing state (such as conditional puts and increments). As
// a result of being "blind", blind-writes are allowed to be more freely
// re-ordered with other writes.
