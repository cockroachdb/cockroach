...
const gcpBucket = "test-bazel-artifacts"
...
func uploadFile(client *storage.Client, localPath, remotePath string, result chan error) {
	...
	defer in.Close()
	out := client.Bucket(gcpBucket).Object(remotePath).If(storage.Conditions{DoesNotExist: true}).NewWriter(context.Background())
