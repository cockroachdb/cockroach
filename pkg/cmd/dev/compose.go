...
func (d *dev) compose(cmd *cobra.Command, _ []string) error {
	...
	if err := filepath.WalkDir(artifactsDir, func(path string, entry os.DirEntry, err error) error {
		if err != nil {
			return err
		}
		if entry.IsDir() {
		...
		if !strings.HasPrefix(relPath, "libgeos") {
			return nil
		}
		...
		if err := d.os.CopyFile(path, dst); err != nil {
			return err
		}
		return nil
	}); err != nil {
