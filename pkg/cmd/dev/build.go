...
const (
	crossFlag                = "cross"
	noHoistGeneratedCodeFlag = "no-hoist-generated-code"
)
...
func makeBuildCmd(runE func(cmd *cobra.Command, args []string) error) *cobra.Command {
	...
	buildCmd.Flags().Bool(noHoistGeneratedCodeFlag, false, "skip hoisting generated code out of the Bazel sandbox into the workspace")
...
func (d *dev) build(cmd *cobra.Command, commandLine []string) error {
	...
	hoistGeneratedCode := !mustGetFlagBool(cmd, noHoistGeneratedCodeFlag)

	args, buildTargets, err := d.getBasicBuildArgs(ctx, targets, hoistGeneratedCode)
	if err != nil {
		return err
	}
...
func (d *dev) getBasicBuildArgs(
	ctx context.Context, targets []string, hoistGeneratedCode bool,
