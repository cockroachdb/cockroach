...
func ensureBinaryInPath(dev *dev, bin string) error {
	if !isTesting {
		if _, err := dev.exec.LookPath(bin); err != nil {
...
func setupPathReal(dev *dev) error {
	...
	for _, compiler := range knownCompilers {
		...
		log.Printf("INFO: removed ccache directory from PATH. New PATH: %s", newPath)
