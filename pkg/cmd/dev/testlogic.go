...
func (d *dev) testlogic(cmd *cobra.Command, commandLine []string) error {
	...
	var (
		...
		// when rewriting logic test files, so we force the default workmem
		// setting it if the --rewrite flag is present.
		defaultWorkmem = rewrite || mustGetFlagBool(cmd, workmemFlag)
	)
	if rewrite {
		ignoreCache = true
	}
