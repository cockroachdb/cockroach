...
func main() {
	...
	for name, db := range opts.Databases {
		...
		if opts.Databases[name].AllowMutations {
			conns[name], err = NewConn(db.Addr, rng, sqlMutators, db.InitSQL, opts.InitSQL)
		} else {
			conns[name], err = NewConn(db.Addr, rng, []mutations.Mutator{}, db.InitSQL, opts.InitSQL)
		}
		if err != nil {
	...
	for i := 0; true; i++ {
		...
		for name, conn := range conns {
			...
			if err := conn.Ping(); err != nil {
				...
				var err error
				var newConn *Conn
				if db.AllowMutations {
					newConn, err = NewConn(db.Addr, rng, sqlMutators, db.InitSQL, opts.InitSQL)
				} else {
					newConn, err = NewConn(db.Addr, rng, []mutations.Mutator{}, db.InitSQL, opts.InitSQL)
				}
				if err != nil {
