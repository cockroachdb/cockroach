...
func FilterAndWrite(in io.Reader, out io.Writer, modesSequence []string) (int64, error) {
	type pipe struct {
		reader *os.File
		writer *os.File
	}
	...
	for idx, mode := range modesSequence {
		var pipeErr error
		pipesArray[idx].reader, pipesArray[idx].writer, pipeErr = os.Pipe()
		if written, err := func() (int64, error) {
			defer func() {
				_ = pipesArray[idx].writer.Close()
			}()
			if pipeErr != nil {
				return 0, pipeErr
			}
			if idx == 0 {
				if err := filter(in, pipesArray[idx].writer, mode); err != nil {
					return 0, err
				}
			} else {
				if err := filter(pipesArray[idx-1].reader, pipesArray[idx].writer, mode); err != nil {
					return 0, err
				}
			}
			return 0, nil
	...
	return io.Copy(out, pipesArray[len(pipesArray)-1].reader)
}
