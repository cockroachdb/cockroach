...
func postOptions(sha string, branch string) *issues.Options {
...
func filer(res *testResultWithXml, opts *issues.Options) githubpost.Filer {
...
func process() error {
	...
	postOpts := postOptions(sha, branch)
	...
	for _, results := range fullTestResults {
		for _, res := range results {
			...
			if err := githubpost.PostFromTestXMLWithFiler(ctx, filer(res, postOpts), testXml); err != nil {
