...
func (w *watcher) Watch() error {
	for {
		...
		case buildErr := <-w.completion:
			// Do this one last time irrespective of the completion
			// status.
...
func (w *watcher) stageTestArtifacts() error {
	for _, test := range w.info.tests {
		...
		err = w.maybeStageArtifact(testlogsSourceDir, relXMLFilePath, 0666,
			func(srcContent []byte, outFile *os.File) error {
...
func copyContentTo(srcContent []byte, outFile *os.File) error {
	_, err := outFile.Write(srcContent)
	return err
}
...
func (w *watcher) maybeStageArtifact(
	...
	relPath string,
	perm os.FileMode,
	stagefn func(srcContent []byte, outFile *os.File) error,
) error {
	var rootPath string
