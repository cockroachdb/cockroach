...
func getBuildInfo(args parsedArgs) (*buildInfo, error) {
	...
	for _, target := range args.targets {
		...
		if len(outputSplit) != 3 {
			return nil, errors.Newf("Could not parse bazel query output: %v", output)
		}
		...
		case "test_suite":
			...
			ret.tests = append(ret.tests, strings.Split(allTests, "\n")...)
		default:
			return nil, errors.Newf("Got unexpected target kind %v", targetKind)
		}
	}
