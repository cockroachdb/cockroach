...
type Filer func(context.Context, Failure) error
...
func DefaultIssueFilerFromFormatter(formatter Formatter) func(ctx context.Context, f Failure) error {
...
func getIssueFilerFromFormatterName(formatterName string) Filer {
...
func PostFromJSON(formatterName string, in io.Reader) {
	...
	fileIssue := getIssueFilerFromFormatterName(formatterName)
...
// and posts issues for any failed tests to GitHub. If there are no failed
// tests, it does nothing. Unlike PostFromTestXMLWithFiler, it takes a formatter
// name.
func PostFromTestXMLWithFormatterName(formatterName string, in io.Reader) error {
	ctx := context.Background()
	fileIssue := getIssueFilerFromFormatterName(formatterName)
	return PostFromTestXMLWithFiler(ctx, fileIssue, in)
}

// PostFromTestXMLWithFiler consumes a Bazel-style `test.xml` stream and posts
// issues for any failed tests to GitHub. If there are no failed tests, it does
// nothing.
func PostFromTestXMLWithFiler(ctx context.Context, fileIssue Filer, in io.Reader) error {
