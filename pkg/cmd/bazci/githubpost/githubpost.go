...
// Post parses the JSON-formatted output from a Go test session,
// as generated by either 'go test -json' or './pkg.test | go tool test2json -t',
// and posts issues for any failed tests to GitHub. If there are no failed
// tests, it assumes that there was a build error and posts the entire log to
// GitHub.
func Post(formatterName string, in io.Reader) {
	...
	if err := listFailures(ctx, in, fileIssue); err != nil {
...
func writeSlowTestsReport(report string) error {
	if err := os.MkdirAll("artifacts", 0777); err != nil {
		return err
	}
...
func getFileLine(
	...
	for {
		...
		cmd := exec.Command(`/bin/bash`, `-c`,
			fmt.Sprintf(`git grep -n 'func %s(' '%s/*_test.go'`,
				testName, packageName))
		gopath := os.Getenv("GOPATH")
		if gopath == "" {
			gopath = build.Default.GOPATH
		}
		cmd.Dir = filepath.Join(gopath, "src/github.com/cockroachdb/cockroach")
