...
// httpHandlerFunc is the function called to register http handler.
// keeping it as a variable enables us to override the value for unit testing
var httpHandlerFunc = http.HandleFunc
...
type Registry interface {
	...
	// To register, us "flag" library. e.g flag.StringVar(&strVar, "strVar", "a string var")
	RegisterFlags()
...
func RegisterHandlers(ctx context.Context, r Registry) error {
	...
	for _, registry := range r.GetRegistrations(ctx) {
		...
		if ph, ok = handlerMap[registry.Url]; !ok {
			...
			// register the url
			httpHandlerFunc(registry.Url, ph.handleProxy)
		}
