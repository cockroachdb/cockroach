...
type executor struct {
	executorConfig
	excludeBenchmarks [][]*regexp.Regexp
	packages          []string
	log               *logger.Logger
}
...
func makeExecutor(config executorConfig) (*executor, error) {
	// Gather package info from the first binary.
	packages, err := readArchivePackages(config.binaries[0])
	if err != nil {
		return nil, err
	}
	...
	for _, binariesName := range config.binaries {
		...
		if !strings.HasSuffix(binariesName, ".tar") && !strings.HasSuffix(binariesName, ".tar.gz") {
			return nil, fmt.Errorf("the binaries archive %s must have the extention .tar or .tar.gz", binariesName)
		}
	}
	...
	return &executor{executorConfig: config, excludeBenchmarks: excludeBenchmarks, packages: packages, log: l}, nil
}
...
func (e *executor) prepareCluster() (int, error) {
	...
	if e.copyBinaries {
		...
		for binIndex, bin := range e.binaries {
	...
	cluster.ExecuteRemoteCommands(roachprodLog, e.cluster, commands, numNodes, true, func(response cluster.RemoteResponse) {
		...
		outer:
			for _, benchmarkName := range strings.Split(response.Stdout, "\n") {
				...
				for _, exclusionPair := range e.excludeBenchmarks {
