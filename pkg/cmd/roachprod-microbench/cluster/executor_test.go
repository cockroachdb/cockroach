...
func TestExecutionScheduling(t *testing.T) {
	...
	require.NoError(t, quick.Check(makeVerify(false), &quick.Config{
		MaxCount: 100,
		Rand:     rng,
		Values:   makeGen(false),
	}))

	require.NoError(t, quick.Check(makeVerify(true), &quick.Config{
		MaxCount: 100,
		Rand:     rng,
		Values:   makeGen(true),
	}))
