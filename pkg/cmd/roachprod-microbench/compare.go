...
func makeCompare(config compareConfig) (*compare, error) {
	...
	return &compare{compareConfig: config, service: service, packages: packages}, nil
}
...
func (c *compare) publishToGoogleSheets(tableResults map[string][]*benchstat.Table) error {
	for pkgGroup, tables := range tableResults {
		...
		url, err := c.service.CreateSheet(context.Background(), sheetName, tables)
		if err != nil {
			return err
		}
