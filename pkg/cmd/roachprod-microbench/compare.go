...
/*
Google Sheet for combined packages of {{ .PackageGroup }}

Microbenchmark comparison summary: {{ .Description }} {{ range .Packages }}


Top {{ .NumChanges }} significant changes for metric {{ .Metric }}:
{{ .Package }} {{ .BenchmarkName }} changed by {{ .PercentChange }}
In total {{ .NumChanges }} had a significant change exceeding {{ .Threshold }}.
