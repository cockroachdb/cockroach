...
func newSlackClient(slackToken string) *slackClient {
	return &slackClient{slack.New(slackToken)}
}
...
func (c *slackClient) Post(options ...slack.MsgOption) error {
	options = append(options, slack.MsgOptionUsername(slackUser))
	channelID, err := c.findChannel(slackChannel, "")
	if err != nil {
		return err
	}
