...
func main() {
	...
	f := func(ctx context.Context, title, packageName, testName, testMessage, authorEmail string) error {
		...
		return issues.Post(ctx, title, packageName, testName, testMessage, authorEmail)
	}
...
func listFailures(
	...
	outstendingOutput := make(map[string][]testEvent)
	failures := make(map[string][]testEvent)
	slowPassingTests := make([]testEvent, 0, 1000)
	slowFailingTests := make([]testEvent, 0, 1000)
	...
	for {
		...
		if len(te.Test) > 0 {
			...
			case "output":
				outstendingOutput[te.Test] = append(outstendingOutput[te.Test], te)
			...
			case "pass", "skip":
				...
				delete(outstendingOutput, te.Test)
			...
			case "fail":
				...
				if timedOutTestName != te.Test {
					failures[te.Test] = outstendingOutput[te.Test]
				}
				delete(outstendingOutput, te.Test)
			}
	...
	// outstending and the test is not registered in the slowFailingTests
	...
	if timedOutTestName != "" {
		if _, ok := outstendingOutput[timedOutTestName]; ok {
			slowFailingTests = append(slowFailingTests, timedOutEvent)
			delete(outstendingOutput, timedOutTestName)
		}
	} else {
		...
		// might be other outstending tests; we ignore those.
		if _, ok := outstendingOutput[lastTestName]; ok {
			log.Printf("found outstending output. Considering last test failed: %s", lastTestName)
			failures[lastTestName] = outstendingOutput[lastTestName]
		}
	}
...
func writeSlowTestsReport(report string) error {
	f, err := os.Create("artifacts/slow-tests-report.txt")
	if err != nil {
		return err
	}
	defer f.Close()
