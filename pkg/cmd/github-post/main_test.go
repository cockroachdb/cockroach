...
func TestRunGH(t *testing.T) {
	...
	for fileName, expectations := range map[string]struct {
		mode  string // "stress" or "jepsen"
		title string
		body  string
	}{
		"stress-failure": {
			mode:  "stress",
			title: "foo/bar/baz: TestReplicateQueueRebalance failed under stress",
		...
		"stress-fatal": {
			mode:  "stress",
			title: "foo/bar/baz: TestGossipHandlesReplacedNode failed under stress",
			body:  "F170517 07:33:43.763059 69575 storage/replica.go:1360  [n3,s3,r1/3:/M{in-ax}] on-disk and in-memory state diverged:",
		},
		"jepsen-failure": {
			mode:  "jepsen",
			title: "jepsen: Bank failed under majority-ring",
			body:  "See artifacts tab for logs",
		},
	} {
...
`, regexp.QuoteMeta(expectations.body)),
				...
				postIssue := func(_ context.Context, owner string, repo string, issue *github.IssueRequest) (*github.Issue, *github.Response, error) {
					...
					if *issue.Title != expectations.title {
						t.Fatalf("got %s, expected %s", *issue.Title, expectations.title)
					}
				...
				if err := runGH(context.Background(), args, file, postIssue, searchIssues, postComment); err != nil {
					t.Fatal(err)
				}
				expectedIssues := 1
				expectedComments := 0
