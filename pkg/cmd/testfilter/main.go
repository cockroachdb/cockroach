				...
				for key := range m {
					if key.pkg == ev.Package && key.test != ev.Test {
						...
						if !strings.Contains(key.test, "/") {
							buf := m[key]
							// Synthetize a "fail" message.
							synthEv := testEvent{Time: ev.Time, Action: "fail", Package: ev.Package, Test: key.test, Elapsed: 0, Output: "unfinished due to package-level failure" + helpMessage}
