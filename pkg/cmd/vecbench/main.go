...
func searchIndex(ctx context.Context, stopper *stop.Stopper, datasetName string) {
	...
	var idxCtx cspann.IndexContext
	doSearch := func(beamSize int) {
		start := timeutil.Now()

		idxCtx.Txn = beginTransaction(ctx, index.Store())
		defer commitTransaction(ctx, index.Store(), idxCtx.Txn)
...
func buildIndex(
	...
	insertBlock := func(idxCtx *cspann.IndexContext, start, end int) {
		idxCtx.Txn = beginTransaction(ctx, store)
		defer commitTransaction(ctx, store, idxCtx.Txn)
	...
	for i := 0; i < data.Train.Count; i += countPerProc {
		...
		go func(start, end int) {
			var indexCtx cspann.IndexContext
