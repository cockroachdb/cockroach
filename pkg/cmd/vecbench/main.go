...
var flagBatchSize = flag.Int("batch-size", 16, "Batch size for inserting vectors.")
	...
	for i := 0; i < vb.buildData.Train.Count; i += countPerProc {
		...
		go func(start, end int) {
			...
			for j := start; j < end; j += blockSize {
				...
				vectors := vb.buildData.Train.Slice(j, min(j+blockSize, end)-j)
