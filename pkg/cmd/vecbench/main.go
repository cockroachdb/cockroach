...
func searchIndex(ctx context.Context, stopper *stop.Stopper, datasetName string) {
	...
	doSearch := func(beamSize int) {
		...
		for i := 0; i < count; i++ {
			...
			prediction := make([]vecstore.PrimaryBytes, searchSet.MaxResults)
			for res := 0; res < len(results); res++ {
				prediction[res] = results[res].ChildKey.PrimaryBytes
			}

			primaryKeys := make([]byte, searchSet.MaxResults*4)
			truth := make([]vecstore.PrimaryBytes, searchSet.MaxResults)
...
// set.
func findMAP(prediction, truth []vecstore.PrimaryBytes) float64 {
