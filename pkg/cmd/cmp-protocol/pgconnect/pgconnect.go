...
func Connect(
	...
	g.GoCtx("send messages", func(ctx context.Context) error {
		defer close(send)
		for {
			select {
			case <-ctx.Done():
	...
	g.GoCtx("receive messages", func(ctx context.Context) error {
		defer close(recv)
		for {
			msg, err := fe.Receive()
			if err != nil {
	...
	g.GoCtx("main connect loop", func(ctx context.Context) error {
