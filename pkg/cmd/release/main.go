...
func pickSHA(_ *cobra.Command, _ []string) error {
	...
	trackingIssue, err := createTrackingIssue(jiraUsername, jiraToken, nextRelease)
	if err != nil {
		return fmt.Errorf("cannot create tracking issue: %w", err)
	}
	// TODO: add another Jira post for SRE and add it to the template
	// TODO: submit the SRE ticket before the tracking ticket and mention it in the tracking ticket (blocked-by)
