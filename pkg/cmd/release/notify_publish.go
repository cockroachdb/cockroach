...
// buildBlessedSlackMessage renders the announcement as Slack mrkdwn: a
// Jira-comment permalink on the first line (rendered with the ticket key
// as the link text), then the body and bullets. The message is
// self-contained — PostMessage disables Slack's link unfurling, so the
// body must not depend on the Jira link expanding.
...
// Jira link — the IBM-OEM audience doesn't track our Jira tickets.
