...
// GitHub usually uses "Merge pull request #NNN" and Bors uses "Merge #NNN" in the generated commit messages.
...
func getMergeCommits(fromRef, toRef string) ([]string, error) {
	...
	out, err := cmd.Output()
	if err != nil {
		return []string{}, fmt.Errorf("cannot read git log output: %s", err)
	}
...
func findCandidateCommits(prevRelease string, releaseSeries string) ([]string, error) {
	refs, err := getMergeCommits(prevRelease, fmt.Sprintf("origin/release-%s", releaseSeries))
	if err != nil {
		return []string{}, fmt.Errorf("cannot get merge commits: %s", err)
	}
	return refs, nil
}
