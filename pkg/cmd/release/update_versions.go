...
func generateRepoList(
	...
	if dryRun {
		...
		owner = "crltest"
		// prefix = "crltest-"
		// need to clone crltest/cockroach using gh and chdir to it
		ghCmd := exec.Command("gh", "repo", "clone", "crltest/cockroach")
		ghCmd.Dir = "."
		if err := ghCmd.Run(); err != nil {
			return []prRepo{}, fmt.Errorf("cloning crltest/cockroach: %w", err)
		}
		if err := os.Chdir("cockroach"); err != nil {
			return []prRepo{}, fmt.Errorf("chdir to cockroach: %w", err)
		}
	}
