...
func productionRepos(version *semver.Version) ([]prRepo, error) {
	...
	defaultPrBranch := fmt.Sprintf("update-versions-%s", version)
	latest, err := isLatestBranch(version)
	if err != nil {
	...
	if latest {
		...
		repos = append(repos, prRepo{
			...
			commitMessage:  defaultCommitMessage,
			commands: []*exec.Cmd{
				exec.Command("go", "run", "build/bump.go", version.Original()),
			},
		})
	}
	return repos, nil
...
func isLatestBranch(version *semver.Version) (bool, error) {
	latestRelease, err := findPreviousRelease("")
	if err != nil {
