...
func generateRepoList(
	...
	for _, mergeBranch := range bakingBranches {
		...
		// merge is not needed.
		alreadyOnBaseBranch, err := isAncestor(baseBranch, mergeBranch)
		if err != nil {
			return []prRepo{}, fmt.Errorf("checking if %s is ancestor of %s: %w", baseBranch, mergeBranch, err)
		}
		if alreadyOnBaseBranch {
			log.Printf("skipping merge of %s to %s, because %s is already an ancestor of %s", mergeBranch, baseBranch, baseBranch, mergeBranch)
			continue
		}
