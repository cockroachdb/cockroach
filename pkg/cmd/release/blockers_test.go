...
func newTestClient(t *testing.T, issues []githubIssue) *githubTestClient {
...
func (c *githubTestClient) addEvents(events ...githubEvent) error {
	length := len(c.events)
	for i, e := range events {
		// first event will be 2006-01-01, second event 2006-01-02, etc
		e.CreatedAt = testCreatedAtDay(c.t, length+i+1)
...
func TestFetchOpenBlockers(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.descr, func(t *testing.T) {
			testClient := newTestClient(t, test.openIssues)
...
func TestMostRecentProjectName(t *testing.T) {
	testIssueNum := 12345
	testClient := newTestClient(t, nil /* issues */)
	...
	for _, step := range steps {
		testClient.addEvents(step.eventsToAdd...)
