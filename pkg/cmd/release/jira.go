...
func createReleaseTrackingIssue(release releaseInfo, setCustomFields bool) *jira.Issue {
	summary := fmt.Sprintf("Release: %s", release.nextReleaseVersion)
	shaUrl := fmt.Sprintf("https://github.com/cockroachlabs/release-staging/commit/%s", release.buildInfo.SHA)
...
`,
		...
		release.buildInfo.Tag, tagUrl,
		release.releaseSeries, branchUrl,
		release.buildInfo.SHA, shaUrl,
	)
	...
	if setCustomFields {
		...
		customFields[customFieldBuildId] = release.buildInfo.Tag // We probably don't need to set "Build ID",
...
// TODO: use template
func createDeployToClusterIssue(release releaseInfo) *jira.Issue {
...
func createTrackingIssue(username string, token string, release releaseInfo) (IssueDetails, error) {
