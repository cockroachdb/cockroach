...
//const testTemplatesDir = "pkg/cmd/release/templates"
//const testTemplatesDir = "./"
...
func TestPickSHA(t *testing.T) {
	...
	//require.Equal(t, "", currentWorkingDirectory)
	//const testTemplatesDir = "pkg/cmd/release/templates"
	//const testTemplatesDir = "./"
	dirs := []string{
		"",
	...
	for _, td := range dirs {
		t.Run(fmt.Sprintf("Trying: %s", td), func(t *testing.T) {
			require.NoError(t, sendMailPickSHA(
				args, sendOpts{templatesDir: td},
			))
...
func SKIP_TestPostBlockers(t *testing.T) {
	tests := []struct {
		testCase            string
		version             string
...
const (
	...
	htmlBodyGoldenFileSuffix = ".gohtml.golden"
