...
func handleStart() bool {
	...
	// flags package used by cli.Start to manage the rest of the flags as usual.
...
func configureArtificialLatencies(latenciesConfig string) error {
	if latenciesConfig == "" {
		return errors.New("--latencies flag was specified without any accompanying config")
	}
...
func main() {
	...
	if *numLocalities > 0 {
		...
		// TODO: Enable actual configuration of this and make it work for different
		// numbers of nodes before merging.
		if len(c.Nodes) != 4 {
