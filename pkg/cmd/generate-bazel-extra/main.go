...
func parseQueryXML(data []byte) (map[string][]string, error) {
	targetNameRegex, err := regexp.Compile(`<rule class="go_test" location=".*" name="(.*)">`)
	if err != nil {
		return nil, err
	}
	...
	sizeToTargets := map[string][]string{}
	var currentTargetName string
	for _, line := range strings.Split(string(data), "\n") {
		// Check if the line contains a target name.
		line = strings.TrimSpace(line)
		submatch := targetNameRegex.FindStringSubmatch(line)
		if submatch != nil {
			if currentTargetName != "" {
				// Default size is medium so if not found then it will be medium.
				sizeToTargets["medium"] = append(sizeToTargets["medium"], currentTargetName)
			}
			currentTargetName = submatch[1]
			continue
		}
		// Check if the line contains a target size.
		submatch = targetSizeRegex.FindStringSubmatch(line)
		if submatch != nil {
			sizeToTargets[submatch[1]] = append(sizeToTargets[submatch[1]], currentTargetName)
			currentTargetName = ""
		}
	}
	if currentTargetName != "" {
		// If the last target uses the default size we need to set it here.
		sizeToTargets["medium"] = append(sizeToTargets["medium"], currentTargetName)
	}
