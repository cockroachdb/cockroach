...
func (a *Api) Init(l *logger.Logger) {
	...
	// In metrics-only mode (workers command), we only need a minimal gin engine
	// with the metrics endpoint. Skip API-specific middleware like authentication,
	// security headers, and request size limits.
	if a.metricsOnly {
		if !a.metrics {
			l.Warn("Metrics-only mode enabled but metrics are disabled. No HTTP server will be started.")
			return
		}

		l.Info("Starting in metrics-only mode (workers)", slog.Int("port", a.metricsPort))
