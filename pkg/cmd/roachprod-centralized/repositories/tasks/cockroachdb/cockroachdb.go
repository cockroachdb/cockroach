...
func (r *CRDBTasksRepo) cleanupStaleTasks(ctx context.Context, l *logger.Logger, limit int) {
	// Use the same health timeout that the health service uses (taskTimeout as approximation)
	// In production, this should ideally use the actual health service timeout constant
	healthTimeout := r.taskTimeout
