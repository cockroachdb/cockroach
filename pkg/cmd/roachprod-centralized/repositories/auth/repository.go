...
type IAuthRepository interface {
	...
	ListAllTokens(context.Context, *logger.Logger, filtertypes.FilterSet) ([]*auth.ApiToken, int, error) // Admin view
	...
	// Groups (SCIM-managed)
	CreateGroup(context.Context, *logger.Logger, *auth.Group) error
	...
	GetUserGroups(context.Context, *logger.Logger, uuid.UUID) ([]*auth.Group, error)
	// PatchGroupMembers atomically adds/removes members from a group.
	// Each operation specifies "add" or "remove" with the user ID.
	PatchGroupMembers(context.Context, *logger.Logger, uuid.UUID, []GroupMemberOperation) error
	...
	// GetPermissionsForGroups returns all permission mappings for the given group names.
	// Used to dynamically resolve user permissions based on their group memberships.
	...
	// ReplaceGroupPermissions atomically replaces all group permissions.
	// Uses transaction: BEGIN → DELETE all → INSERT all → COMMIT
