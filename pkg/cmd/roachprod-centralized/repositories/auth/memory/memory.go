...
func (r *MemAuthRepo) CreateGroup(ctx context.Context, l *logger.Logger, group *auth.Group) error {
	r.lock.Lock()
	defer r.lock.Unlock()

	// Check for duplicate external ID
	for _, g := range r.groups {
		if group.ExternalID != "" && g.ExternalID == group.ExternalID {
			return errors.New("group with this external ID already exists")
		}
	}
...
func (r *MemAuthRepo) GetUserGroups(
	ctx context.Context, l *logger.Logger, userID uuid.UUID,
) ([]*auth.Group, error) {
	r.lock.Lock()
	defer r.lock.Unlock()

	var result []*auth.Group
	for groupID, members := range r.groupMembers {
		for _, m := range members {
			if m.UserID == userID {
	...
	slices.SortFunc(result, func(a, b *auth.Group) int {
...
func (r *MemAuthRepo) PatchGroupMembers(
	...
	for _, op := range operations {
		...
		case "add":
			...
			alreadyMember := false
			for _, m := range r.groupMembers[groupID] {
			...
			if !alreadyMember {
				...
				r.groupMembers[groupID] = append(r.groupMembers[groupID], member)
			}
		case "remove":
			for i, m := range r.groupMembers[groupID] {
				if m.UserID == op.UserID {
					r.groupMembers[groupID] = append(r.groupMembers[groupID][:i], r.groupMembers[groupID][i+1:]...)
					break
				}
			}
		default:
