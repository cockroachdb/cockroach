...
func (r *CRDBAuthRepo) CreateGroup(ctx context.Context, l *logger.Logger, group *auth.Group) error {
	query := `
...
func (r *CRDBAuthRepo) GetUserGroups(
	ctx context.Context, l *logger.Logger, userID uuid.UUID,
) ([]*auth.Group, error) {
	query := `
		SELECT g.id, g.external_id, g.display_name, g.created_at, g.updated_at
		FROM groups g
...
func (r *CRDBAuthRepo) PatchGroupMembers(
	ctx context.Context, l *logger.Logger, groupID uuid.UUID, operations []rauth.GroupMemberOperation,
) (retErr error) {
	tx, err := r.db.BeginTx(ctx, nil)
	if err != nil {
		return errors.Wrap(err, "failed to begin transaction")
