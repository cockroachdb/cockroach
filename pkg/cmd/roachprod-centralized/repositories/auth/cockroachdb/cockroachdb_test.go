...
func TestCRDBAuthRepo_Groups(t *testing.T) {
	...
	t.Run("CRUD", func(t *testing.T) {
		...
		err := repo.CreateGroup(ctx, logger.DefaultLogger, group)
		require.NoError(t, err)
	...
	t.Run("Members", func(t *testing.T) {
		...
		require.NoError(t, repo.CreateGroup(ctx, logger.DefaultLogger, group))

		// Add members using PatchGroupMembers
		operations := []rauth.GroupMemberOperation{
			{Op: "add", UserID: user1.ID},
			{Op: "add", UserID: user2.ID},
		}
		err := repo.PatchGroupMembers(ctx, logger.DefaultLogger, group.ID, operations)
		require.NoError(t, err)
		...
		// Get user groups
		userGroups, err := repo.GetUserGroups(ctx, logger.DefaultLogger, user1.ID)
		require.NoError(t, err)
		require.Len(t, userGroups, 1)
		require.Equal(t, group.ID, userGroups[0].ID)

		// Remove a member
		removeOps := []rauth.GroupMemberOperation{
			{Op: "remove", UserID: user1.ID},
		}
		err = repo.PatchGroupMembers(ctx, logger.DefaultLogger, group.ID, removeOps)
		require.NoError(t, err)
	...
	t.Run("UserPermissionsFromGroups", func(t *testing.T) {
		...
		require.NoError(t, repo.CreateGroup(ctx, logger.DefaultLogger, group1))
		require.NoError(t, repo.CreateGroup(ctx, logger.DefaultLogger, group2))

		// Add user to groups
		err := repo.PatchGroupMembers(ctx, logger.DefaultLogger, group1.ID, []rauth.GroupMemberOperation{
			{Op: "add", UserID: user.ID},
		})
		require.NoError(t, err)
		err = repo.PatchGroupMembers(ctx, logger.DefaultLogger, group2.ID, []rauth.GroupMemberOperation{
			{Op: "add", UserID: user.ID},
		})
		require.NoError(t, err)
