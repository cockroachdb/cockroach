...
// GetTempDirsDiagnostics gathers disk usage and file listings for temporary directories.
func (s *Service) GetTempDirsDiagnostics(
	ctx context.Context, l *logger.Logger,
) *types.TempDirsDiagnostics {
	dirs := []string{
		os.TempDir(), // /tmp or OS temp dir
		filepath.Join(os.Getenv("HOME"), ".roachprod"), // roachprod cache
	}

	dto := &types.TempDirsDiagnostics{
		Directories: make([]*types.DiskUsageDiagnostics, 0, len(dirs)),
	}
	...
	for _, dir := range dirs {
		...
		dto.Directories = append(dto.Directories, usage)
	}

	return dto
}

func getDiskUsage(path string) *types.DiskUsageDiagnostics {
	usage := &types.DiskUsageDiagnostics{
		Path:  path,
	...
	if err := syscall.Statfs(path, &stat); err != nil {
		...
		usage.TotalBytes = stat.Blocks * uint64(stat.Bsize)
		usage.AvailBytes = stat.Bavail * uint64(stat.Bsize)
		usage.UsedBytes = usage.TotalBytes - usage.AvailBytes
	...
	err := filepath.Walk(path, func(filePath string, info os.FileInfo, err error) error {
		...
		usage.FileCount++
		usage.Files = append(usage.Files, &types.FileInfo{
			Path:         filePath,
			SizeBytes:    info.Size(),
