...
type IService interface {
	// Token Management
	ValidateToken(context.Context, *logger.Logger, string, string) (*auth.ApiToken, error)
	...
	GetUserByOktaID(context.Context, *logger.Logger, *pkgauth.Principal, string) (*auth.User, error)
	GetUserPermissions(context.Context, *logger.Logger, *pkgauth.Principal, uuid.UUID) ([]*auth.UserPermission, error)
	...
	RevokeUserToken(context.Context, *logger.Logger, *pkgauth.Principal, uuid.UUID, uuid.UUID) error           // Admin revocation of user token (userID, tokenID)
	RevokeServiceAccountToken(context.Context, *logger.Logger, *pkgauth.Principal, uuid.UUID, uuid.UUID) error // Revoke token for a specific SA (saID, tokenID)
