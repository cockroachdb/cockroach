...
```go
func (t *MyTask) Process(ctx context.Context, l *logger.Logger, errChan chan<- error) {
    l.Info("Starting my task", "task_id", t.ID)

    // Do the work
    err := t.myService.DoSomething(ctx, l)
    if err != nil {
...
func (t *MyTask) Process(ctx context.Context, l *logger.Logger, errChan chan<- error) {
    ...
    err := t.Service.DoSomethingWith(ctx, l, opts.Param1, opts.Param2)
    errChan <- err
}
```
