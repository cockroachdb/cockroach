...
func (w *workloadRunner) runWorkloadStep(ctx context.Context, workload workloadConfig, workloadIdx int, step workloadStep) {
	...
	if err != nil {
		...
		w.errChan <- fmt.Errorf("error when getting PgURL for workload %d: %s\n", workloadIdx, err.Error())
		return
	}
