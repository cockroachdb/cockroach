...
func (s *Suite) hasPerformanceChange(status Status) bool {
	for _, status := range []Status{status} {
...
func makeCompareCommand() *cobra.Command {
	...
	cmdFunc := func(cmd *cobra.Command, commandLine []string) error {
		...
		// label to the PR, if it is not already present. If a performance gain
		// was detected, we do not post but only add a performance gain label.
		if post {
