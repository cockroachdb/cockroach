...
func NewGithubConfig() (*GitHubConfig, error) {
	...
	if len(repoInfo) != 2 {
		return nil, errors.New("invalid GitHub repository flag, %s")
	}
	owner := repoInfo[0]
	repo := repoInfo[1]
...
func (c *GitHubConfig) postComment(summaryText string) error {
	...
	if err != nil {
		...
		if strings.Contains(err.Error(), "403") {
			log.Printf("WARNING: Skipping post, because GitHub token does not have write access to the repository")
			return nil
		}
		return err
	}
