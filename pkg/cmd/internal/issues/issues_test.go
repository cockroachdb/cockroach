...
`, regexp.QuoteMeta(c.message)),
				...
				p.createIssue = func(_ context.Context, owner string, repo string,
					...
					if expected := fmt.Sprintf("%s: %s failed under stress", envPkg, c.testName); *issue.Title != expected {
				...
				if err := p.post(ctx, " under stress", c.packageName, c.testName, c.message); err != nil {
					t.Fatal(err)
				}
