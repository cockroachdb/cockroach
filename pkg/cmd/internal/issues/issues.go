...
func buildIssueQueries(
	...
	var mustHave, mustNotHave []string
	...
	if len(mustHave) == 0 {
		// Sometimes users open issues about flakes and don't assign all the labels.
		// We want to at least require the one label to avoid pathological
		// situations in which a test name is so generic that it matches lots of
		// random issues.
		if labels[TestFailureLabel] {
			mustHave = append(mustHave, TestFailureLabel)
		} else {
			mustHave = append(mustHave, RobotLabel)
		}
	}
