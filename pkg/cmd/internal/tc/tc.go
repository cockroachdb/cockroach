...
func NewController(ifces []string) *Controller {
	return &Controller{
		ifces:     ifces,
...
func (c *Controller) CleanUp() error {
	for _, ifce := range c.ifces {
		...
		if err != nil {
			return errors.Wrapf(err, "failed to remove tc rules for %q -- you may have to remove them manually: %s", ifce, out)
		}
	}
	return nil
}
