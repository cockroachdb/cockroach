...
`, costfuzzSep, control, setting1, setting2, perturb)
	...
	if unoptimizedOracle {
		...
		settings2, lineIdx := findPreviousSetStatements(lines, lineIdx, logger)
		unoptimized, lineIdx := findPreviousQuery(lines, lineIdx)
		settings1, lineIdx := findPreviousSetStatements(lines, lineIdx, logger)
		inputString = strings.Join(lines[:lineIdx], "\n")
...
func findPreviousSetStatements(lines []string, lineIdx int, logger *log.Logger) (string, int) {
	// Skip empty lines.
	for lines[lineIdx] == "" {
		lineIdx--
	}
