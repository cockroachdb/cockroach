...
{{range $testSuffix, $testFilePath := .TestCases}}
func TestDockerCLI_{{$testSuffix}}(t *testing.T) {
	s := log.Scope(t)
	defer s.Close(t)
	...
	if err := testDockerOneShot(ctx, t, "cli_test_{{$testSuffix}}", containerConfig); err != nil {
