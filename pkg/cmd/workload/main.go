...
var batch = rootFlags.Int("batch", 1, "Number of blocks to insert in a single SQL statement")
...
var maxOps = rootFlags.Uint64("max-ops", 0, "Maximum number of blocks to read/write")
...
func init() {
	for _, genFn := range workload.Registered() {
		...
		name, description := gen.Meta()
		genCmd := &cobra.Command{Use: name, Short: description}
...
func runRoot(gen workload.Generator, args []string) error {
	...
	defer func() {
		...
		benchmarkName := strings.Join([]string{
			"BenchmarkWorkload",
			fmt.Sprintf("generator=%s", genName),
