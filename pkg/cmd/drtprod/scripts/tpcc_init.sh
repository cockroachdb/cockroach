...
${pwd}/cockroach workload init tpcc $@ --secure --families \$PGURLS |tee -a ${pwd}/tpcc_init_1.log
EOF"
roachprod ssh "${WORKLOAD_CLUSTER}":1 -- "chmod +x tpcc_init.sh"
roachprod run "${WORKLOAD_CLUSTER}":1 -- "sudo systemd-run --unit tpccinit ${pwd}/tpcc_init.sh"
