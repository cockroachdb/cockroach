...
if [ "$execute_script" = "true" ]; then
  drtprod run "${WORKLOAD_CLUSTER}":1 -- "sudo systemd-run --unit bank_run_${suffix} --same-dir --uid \$(id -u) --gid \$(id -g) bash ${pwd}/bank_run_${suffix}.sh"
else
  echo "Run --> drtprod run "${WORKLOAD_CLUSTER}":1 -- \"sudo systemd-run --unit bank_run_${suffix} --same-dir --uid \\\$(id -u) --gid \\\$(id -g) bash ${pwd}/bank_run_${suffix}.sh\""
fi
