...
`, existingFile.Name()), getTestYaml()))
	...
	t.Run("run command remotely with no failure and env set", func(t *testing.T) {
		...
		require.Equal(t, 1, len(runCmds["cp"]))
		require.Equal(t, 1, len(runCmds["systemd"]))
		require.Equal(t, "sudo systemd-run --unit test-monitor --same-dir --uid $(id -u) --gid $(id -g) --setenv=DD_API_KEY='the\"test'\"'\"'secret' --setenv=SLACK_TOKEN=the_demo_token --setenv=SLACK_CHANNEL=the_demo_channel drtprod execute ./location/to/test.yaml",
			runCmds["systemd"][0])
	})
