...
func setEnv(ctx context.Context, environment map[string]string, displayOnly bool) error {
	for key, value := range environment {
		if displayOnly {
			fmt.Printf("export %s=%s", key, value)
		} else {
			fmt.Printf("Setting env %s to %s", key, value)
		}
...
func waitForDependentTargets(
	...
	for _, dt := range t.DependentTargets {
		if twg, ok := waitGroupTracker[dt]; ok {
			fmt.Printf("[%s] waiting on <%s>", t.TargetName, dt)
...
func buildTargetCmdsAndRegisterWaitGroups(
	...
	for i := 0; i < len(targets); i++ {
		...
		if shouldSkipTarget(targetNameMap, t, userProvidedTargetNames) {
			fmt.Printf("Ignoring execution for target %s", t.TargetName)
			continue
		}
...
func displayCommands(ctx context.Context, t target) {
	if len(t.DependentTargets) > 0 {
		fmt.Printf("For target <%s> after [%s]:", t.TargetName, strings.Join(t.DependentTargets, ", "))
	} else {
		fmt.Printf("For target <%s>:", t.TargetName)
	}
	for _, cmd := range t.commands {
		fmt.Printf("|-> %s", cmd)
		for _, rCmd := range cmd.rollbackCmds {
			fmt.Printf("    |-> (Rollback) %s", rCmd)
		}
	}
}
...
func executeCommands(
	...
	for _, cmd := range cmds {
		if cmd.waitBefore > 0 {
			fmt.Printf("[%s] Waiting for %d seconds", logPrefix, cmd.waitBefore)
		...
		fmt.Printf("[%s] Starting <%v>", logPrefix, cmd)
		...
		if err != nil {
			...
			fmt.Printf("[%s] Completed <%v>", logPrefix, cmd)
			if cmd.waitAfter > 0 {
				fmt.Printf("[%s] Waiting for %d seconds", logPrefix, cmd.waitAfter)
