...
func uploadAllDependentFiles(
	...
	log.Dev.Infof(ctx, "All the dependencies are uploaded")
	return nil
}
...
func setEnv(ctx context.Context, environment map[string]string, displayOnly bool) error {
	for key, value := range environment {
		if displayOnly {
			log.Dev.Infof(ctx, "export %s=%s", key, value)
		} else {
			log.Dev.Infof(ctx, "Setting env %s to %s", key, value)
		}
...
func processTargets(
	...
	for _, theTarget := range targets {
		...
		g.Go(func() error {
			...
			if err != nil {
				log.Dev.Errorf(ctx, "[%s] Error executing commands: %v", t.TargetName, err)
				if t.NotifyProgress {
...
func waitForDependentTargets(
	...
	for _, dt := range t.DependentTargets {
		if twg, ok := waitGroupTracker[dt]; ok {
			log.Dev.Infof(ctx, "[%s] waiting on <%s>", t.TargetName, dt)
			...
			if !t.IgnoreDependentFailure && sr.getTargetStatus(dt) == targetResultFailure {
				log.Dev.Errorf(ctx, "[%s] Not proceeding as the dependent target %s was not successful.", t.TargetName, dt)
...
func buildTargetCmdsAndRegisterWaitGroups(
	...
	for i := 0; i < len(targets); i++ {
		...
		if shouldSkipTarget(targetNameMap, t, userProvidedTargetNames) {
			log.Dev.Infof(ctx, "Ignoring execution for target %s", t.TargetName)
			continue
		}
...
func displayCommands(t target) {
	if len(t.DependentTargets) > 0 {
		log.Dev.Infof(context.Background(), "For target <%s> after [%s]:", t.TargetName, strings.Join(t.DependentTargets, ", "))
	} else {
		log.Dev.Infof(context.Background(), "For target <%s>:", t.TargetName)
	}
	for _, cmd := range t.commands {
		log.Dev.Infof(context.Background(), "|-> %s", cmd)
		for _, rCmd := range cmd.rollbackCmds {
			log.Dev.Infof(context.Background(), "    |-> (Rollback) %s", rCmd)
		}
	}
}
...
func executeCommands(
	...
	for _, cmd := range cmds {
		if cmd.waitBefore > 0 {
			log.Dev.Infof(ctx, "[%s] Waiting for %d seconds", logPrefix, cmd.waitBefore)
		...
		log.Dev.Infof(ctx, "[%s] Starting <%v>", logPrefix, cmd)
		...
		if err != nil {
			...
			log.Dev.Errorf(ctx, "[%s] Failed <%v>, Error Ignored: %v", logPrefix, cmd, err)
		} else {
			if cmd.notifyProgress {
				_ = notifier.SendStepNotification(logPrefix, cmd.String(), StatusCompleted)
			}
			log.Dev.Infof(ctx, "[%s] Completed <%v>", logPrefix, cmd)
			if cmd.waitAfter > 0 {
				log.Dev.Infof(ctx, "[%s] Waiting for %d seconds", logPrefix, cmd.waitAfter)
