...
var (
	...
	// printf is used for printing output. This helps in unit tests
	printf = fmt.Printf
...
func uploadAllDependentFiles(
	...
	_, _ = printf("All the dependencies are uploaded\n")
	return nil
}
...
func setEnv(_ context.Context, environment map[string]string, displayOnly bool) error {
	for key, value := range environment {
		if displayOnly {
			_, _ = printf("export %s=%s\n", key, value)
		} else {
			_, _ = printf("Setting env %s to %s\n", key, value)
		}
...
func processTargets(
	...
	for _, theTarget := range targets {
		...
		g.Go(func() error {
			...
			if err != nil {
				_, _ = printf("[%s] Error executing commands: %v\n", t.TargetName, err)
				if t.NotifyProgress {
	...
	for _, dt := range t.DependentTargets {
		if twg, ok := waitGroupTracker[dt]; ok {
			...
			if !t.IgnoreDependentFailure && sr.getTargetStatus(dt) == targetResultFailure {
				_, _ = printf("[%s] Not proceeding as the dependent target %s was not successful.\n", t.TargetName, dt)
...
func buildTargetCmdsAndRegisterWaitGroups(
	...
	for i := 0; i < len(targets); i++ {
		...
		if shouldSkipTarget(targetNameMap, t, userProvidedTargetNames) {
			_, _ = printf("Ignoring execution for target %s\n", t.TargetName)
			continue
		}
...
func executeCommands(
	...
	for _, cmd := range cmds {
		if cmd.waitBefore > 0 {
			_, _ = printf("[%s] Waiting for %d seconds\n", logPrefix, cmd.waitBefore)
