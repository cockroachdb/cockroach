...
- ForkTask: Must have Tasks (>=1), Next; all branches converge to same EndTask
- ConditionTask: Must have ExecutorFn returning (bool, error), Then, Else; branches converge to same EndTask
- AsyncTask: Must have ExecutionFn and ResultProcessorFn with correct signatures, Next
...
- Function signature matches task type requirements:
  ...
  - AsyncTask ExecutionFn: Returns (string, error)
  - AsyncTask ResultProcessorFn: Takes (context.Context, interface{}, string, string) and returns (interface{}, error)
