...
// This file defines all task types (ExecutionTask, ForkTask, ConditionTask, AsyncTask,
...
const (
	...
	TaskTypeConditionTask TaskType = "if task"
...
// Type returns TaskTypeConditionTask, identifying this as an if task.
...
func (i *ConditionTask) validate() error {
	if i.ExecutorFn == nil {
...
func (c *ChildPlanTask) validate() error {
	if c.taskName == "" {
		return errors.Newf("task name is missing")
	}
	if c.PlanName == "" {
...
// GetExecutorName returns the name of the child task info function for this child plan task.
