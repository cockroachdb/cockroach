...
// NewConditionTask creates and registers a new if task with the given name.
// An if task provides conditional branching based on executor results.
func (sr *BasePlanner) NewConditionTask(ctx context.Context, name string) *ConditionTask {
	task := &ConditionTask{}
	task.taskName = name
	sr.registerTask(ctx, task)
	return task
...
func (sr *BasePlanner) NewChildPlanTask(ctx context.Context, name string) *ChildPlanTask {
	task := &ChildPlanTask{}
	task.taskName = name
	sr.registerTask(ctx, task)
	return task
...
func (sr *BasePlanner) validateTaskChain(
	...
	case TaskTypeConditionTask:
		// An ConditionTask requires both Then and Else branches to be defined.
		conditionTask := task.(*ConditionTask)
		// The Then branch is validated first.
		thenEnd, err := sr.validateTaskChain(ctx, conditionTask.Then, visited, currentPath, failedPath)
		if err != nil {
			return nil, err
		}

		// The Else branch is validated next.
		elseEnd, err := sr.validateTaskChain(ctx, conditionTask.Else, visited, currentPath, failedPath)
		if err != nil {
			return nil, err
		}
