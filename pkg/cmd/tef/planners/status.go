...
func SerializeTask(task Task, executorRegistry map[string]*Executor) TaskInfo {
	...
	// Common fields for step tasks (ExecutionTask, ForkTask, ChildPlanTask, SleepTask)
	if task.isStepTask() {
	...
	case TaskTypeIfTask:
		ifTask := task.(*IfTask)
		taskInfo.Executor = ifTask.GetExecutorName(executorRegistry)
		taskInfo.Params = getTaskNames(ifTask.Params)
		if ifTask.Then != nil {
			taskInfo.Then = ifTask.Then.Name()
		}
		if ifTask.Else != nil {
			taskInfo.Else = ifTask.Else.Name()
		}

	case TaskTypeSleepTask:
		sleepTask := task.(*SleepTask)
		taskInfo.Executor = sleepTask.GetExecutorName(executorRegistry)
		taskInfo.Params = getTaskNames(sleepTask.Params)
