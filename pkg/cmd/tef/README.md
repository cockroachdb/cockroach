...
* **Task**: An individual step in a workflow. TEF provides six task types (ExecutionTask, ForkTask, ConditionTask, AsyncTask, ChildWorkflowTask, EndTask).
...
ForkTask executes multiple branches concurrently. All branches must converge to the same EndTask.
...
All execution paths must end with an EndTask. The framework validates that all branches converge.

## Additional Task Types

The example above shows the most common task types. TEF also supports:

* **AsyncTask**: For operations that start work and wait for external completion signals
...
| ForkTask | Parallel execution | Multiple branches, all converge to same EndTask |
...
| EndTask | Path termination | All paths must converge here |
...
- ForkTask branches must converge to the same EndTask
