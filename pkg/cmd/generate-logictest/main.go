...
func (t *testdir) addExecBuildLogicTests(testPrefix string, calc logictestbase.ConfigCalculator) error {
...
func (t *testdir) addSqliteLogicTests(testPrefix string, calc logictestbase.ConfigCalculator) error {
...
func (t *testdir) dump() error {
	for configIdx := range logictestbase.LogicTestConfigs {
		...
		tplCfg.Package = cfgNameMunged
		tplCfg.RelDir = t.relPathToParent
		tplCfg.TestCount = testCount
		subdir := filepath.Join(t.dir, cfgNameMunged)
		...
		for _, configPaths := range t.logicTestsConfigPaths {
			...
			for _, file := range paths {
				dumpTestForFile(f, configPaths.testPrefix, cfgNameMunged, filepath.Base(file), "runLogicTest")
			}
		}
		for _, configPaths := range t.cclLogicTestsConfigPaths {
			paths := configPaths.configPaths[configIdx]
			for _, file := range paths {
				dumpTestForFile(f, configPaths.testPrefix, cfgNameMunged, filepath.Base(file), "runCCLLogicTest")
			}
		}
		for _, configPaths := range t.execBuildLogicTestsConfigPaths {
			paths := configPaths.configPaths[configIdx]
			for _, file := range paths {
				dumpTestForFile(f, configPaths.testPrefix, cfgNameMunged, filepath.Base(file), "runExecBuildLogicTest")
			}
		}
		for _, configPaths := range t.sqliteLogicTestsConfigPaths {
			paths := configPaths.configPaths[configIdx]
			for _, file := range paths {
				dumpTestForFile(f, configPaths.testPrefix, cfgNameMunged, strings.TrimPrefix(file, t.g.sqliteLogicTestsPath), "runSqliteLogicTest")
			}
		}
