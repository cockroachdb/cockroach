...
`RandomGCECreateOptions` generates a valid, internally consistent configuration using the test's seeded RNG, along with matching assertions for each randomized setting.
```go
func TestExample(t *testing.T) {
    ...
    // Verify all randomized settings match the live cluster.
    // Assertions are co-generated with the config — each setting
    // registers a closure that checks the expected value.
    opts.AssertAll(rpt)
}
```
