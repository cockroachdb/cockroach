...
func TestRandomGCECreateOptions(t *testing.T) {
	...
	for i := 0; i < numIterations; i++ {
		...
		// and should never be pd-extreme.
		if !cfg.CreateOpts.SSDOpts.UseLocalSSD && gceOpts.PDVolumeType != "" {
			require.NotEqual(t, "pd-extreme", gceOpts.PDVolumeType,
				"%s: pd-extreme should be excluded", desc)
			require.Contains(t, info.StorageTypes, gceOpts.PDVolumeType,
				"%s: PD volume type should be in machine's storage types", desc)
		}

		// Boot disk type (when set) should follow same rules.
		if gceOpts.BootDiskOnly && gceOpts.BootDiskType != "" {
			require.NotEqual(t, "pd-extreme", gceOpts.BootDiskType,
				"%s: boot disk pd-extreme should be excluded", desc)
...
func TestFilterPDTypes(t *testing.T) {
	tests := []struct {
		...
		name:  "excludes pd-extreme",
		input: []string{"pd-ssd", "pd-extreme", "pd-balanced"},
		want:  []string{"pd-ssd", "pd-balanced"},
	}, {
		name:  "empty input",
		input: []string{},
		want:  nil,
