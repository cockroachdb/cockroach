...
//
// Assertions are co-generated alongside the config: each randomized setting
// appends a corresponding assertion closure that captures the expected value.
// Call AssertAll to run them all against a live cluster. See assertions.go
// for the assertion types and methods.
...
//
// Assertions are co-generated alongside the config. Each randomized setting
// registers a closure that captures the expected value and calls the
// appropriate Assert* method when executed. For example:
//
...
func RandomGCECreateOptions(rng *rand.Rand) *RandomizedClusterConfig {
	...
	mt := providerOpts.MachineType
	cfg.expect(fmt.Sprintf("machine type = %s", mt), func(rpt *RoachprodTest) {
		rpt.AssertClusterMachineType(mt)
	})

	// 2. Architecture: derived from machine type. FIPS only on amd64.
	cfg.CreateOpts.Arch = info.Architecture
	if info.Architecture == gcedb.ArchAMD64 && rng.Float32() < probFIPSOnAMD64 {
		cfg.CreateOpts.Arch = "fips"
	}
	// FIPS is a software mode on amd64 hardware; the VM reports amd64.
	expectedArch := cfg.CreateOpts.Arch
	if expectedArch == "fips" {
		expectedArch = "amd64"
	}
	cfg.expect(fmt.Sprintf("architecture = %s", expectedArch), func(rpt *RoachprodTest) {
		rpt.AssertClusterArchitecture(expectedArch)
	})

	// 3. Local SSD: only enable when the machine type supports it.
	if len(info.AllowedLocalSSDCount) > 0 {
		cfg.CreateOpts.SSDOpts.UseLocalSSD = rng.Float32() < probLocalSSD
	} else {
		cfg.CreateOpts.SSDOpts.UseLocalSSD = false
	}

	// 4. Persistent disk config (when not using local SSD).
	pdTypes := filterPDTypes(info.StorageTypes)
	if !cfg.CreateOpts.SSDOpts.UseLocalSSD && len(pdTypes) > 0 {
	...
	cfg.CreateOpts.Lifetime = time.Duration(lifetimeHours) * time.Hour
	lifetime := cfg.CreateOpts.Lifetime
	cfg.expect(fmt.Sprintf("lifetime = %s", lifetime), func(rpt *RoachprodTest) {
		rpt.AssertClusterLifetime(lifetime)
	})

	// 6. Random filesystem.
	cfg.CreateOpts.SSDOpts.FileSystem = SupportedFilesystems[rng.Intn(len(SupportedFilesystems))]
	fs := string(cfg.CreateOpts.SSDOpts.FileSystem)
	cfg.expect(fmt.Sprintf("filesystem = %s", fs), func(rpt *RoachprodTest) {
		rpt.AssertClusterFilesystem(fs)
	})

	// 7. Spot instance usage.
	providerOpts.UseSpot = rng.Float32() < probSpotInstance
	spot := providerOpts.UseSpot
	cfg.expect(fmt.Sprintf("preemptible = %v", spot), func(rpt *RoachprodTest) {
		rpt.AssertClusterPreemptible(spot)
	})
	...
	// Storage assertions depend on the combination of local SSD and boot-disk-only.
	if providerOpts.BootDiskOnly {
		cfg.expect("boot disk only: no non-boot volumes", func(rpt *RoachprodTest) {
			rpt.AssertClusterNonBootVolumeCount(0)
		})
	} else if !cfg.CreateOpts.SSDOpts.UseLocalSSD {
		pdType := providerOpts.PDVolumeType
		pdSize := providerOpts.PDVolumeSize
		cfg.expect(fmt.Sprintf("PD volume type = %s", pdType), func(rpt *RoachprodTest) {
			rpt.AssertClusterNonBootVolumeType(pdType)
		})
		cfg.expect(fmt.Sprintf("PD volume size = %dGB", pdSize), func(rpt *RoachprodTest) {
			rpt.AssertClusterNonBootVolumeSize(pdSize)
		})
	}

	// 9. Cron service.
	// Not asserted — cron status is not exposed by roachprod list --json.
	...
	if len(providerOpts.Zones) == 1 {
		zone := providerOpts.Zones[0]
		cfg.expect(fmt.Sprintf("zone = %s", zone), func(rpt *RoachprodTest) {
			rpt.AssertClusterZone(zone)
		})
	} else {
		zones := providerOpts.Zones
		cfg.expect(fmt.Sprintf("zones ⊇ VMs, from %v", zones), func(rpt *RoachprodTest) {
			...
			for _, v := range info.VMs {
				...
				require.True(rpt.t, found,
					"VM %s zone %s not in requested zones %v", v.Name, v.Zone, zones)
			}
		})
	}

	// Node count — set last since it doesn't affect other decisions.
	cfg.NumNodes = minNodes + rng.Intn(maxNodes-minNodes+1)
	numNodes := cfg.NumNodes
	cfg.expect(fmt.Sprintf("node count = %d", numNodes), func(rpt *RoachprodTest) {
		rpt.AssertClusterNodeCount(numNodes)
	})

	// Always assert cluster exists and cloud provider.
	cfg.expect("cluster exists", func(rpt *RoachprodTest) {
		rpt.AssertClusterExists()
	})
	cfg.expect("cloud = gce", func(rpt *RoachprodTest) {
