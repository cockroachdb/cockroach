...
func checkLatest(
	...
	errMsg := func(err error) error {
		return fmt.Errorf("Unable to read blob: %q, err: %v", shaObj.ObjectName(), err)
	}
...
func DownloadLatestRoachprod(toFile string, branch string, osName string, arch string) error {
	...
	if err != nil {
		return fmt.Errorf("unable to determine 'mtime' of the roachprod binary: %v", err)
	}
	if age.Minutes() <= 0 {
		return fmt.Errorf("Refusing to update; current binary is newer than the latest: %s", age)
	}
	binObj := getBlob(client, branch, osName, arch, "roachprod."+sha)
	// Get the blob and write it to a file.
	r, err := binObj.NewReader(ctx)
	if err != nil {
		return fmt.Errorf("Unable to read blob: %q, err: %v", binObj.ObjectName(), err)
	}
	defer func() { _ = r.Close() }()

	f, err := os.Create(toFile)
	if err != nil {
		return fmt.Errorf("Unable to create destination file: %q, error: %v", toFile, err)
	}
	if err := f.Chmod(0700); err != nil {
		return fmt.Errorf("Unable to chmod destination file: %q, error: %v", toFile, err)
	}
	defer func() { _ = f.Close() }()
	if _, err := io.Copy(f, r); err != nil {
		return fmt.Errorf("Unable to copy blob: %q to the destination file: %q, error: %v", binObj, toFile, err)
	}
	return nil
}
