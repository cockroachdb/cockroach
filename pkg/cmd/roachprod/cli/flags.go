...
func (cr *commandRegistry) initRootCmdFlags() {
	cr.rootCmd.PersistentFlags().BoolVarP(&config.Quiet, "quiet", "q",
		!term.IsTerminal(int(os.Stdout.Fd())), "disable fancy progress output")
	cr.rootCmd.PersistentFlags().IntVarP(&config.MaxConcurrency, "max-concurrency", "", 32,
		"maximum number of operations to execute on nodes concurrently, set to zero for infinite",
	)
	cr.rootCmd.PersistentFlags().BoolVarP(&config.FastDNS, "fast-dns", "",
		term.IsTerminal(int(os.Stdout.Fd())), "enable fast DNS resolution via the standard Go net package")
	cr.rootCmd.PersistentFlags().StringVarP(&config.EmailDomain, "email-domain", "",
		config.DefaultEmailDomain, "email domain for users")
	cr.rootCmd.PersistentFlags().BoolVar(&config.UseSharedUser,
...
func (cr *commandRegistry) initCreateCmdFlags(createCmd *cobra.Command) {
...
func (cr *commandRegistry) initClusterFlagsForMultiProjects() {
	for _, providerName := range vm.AllProviderNames() {
		if vm.Providers[providerName].Active() {
			for _, cmd := range cr.rootCmd.Commands() {
				excludeCmd := false
				for _, c := range cr.excludeFromClusterFlagsMulti {
					if cmd == c {
						excludeCmd = true
						break
					}
...
func (cr *commandRegistry) initDestroyCmdFlags(destroyCmd *cobra.Command) {
...
func (cr *commandRegistry) initListCmdFlags(listCmd *cobra.Command) {
...
func (cr *commandRegistry) initAdminurlCmdFlags(adminurlCmd *cobra.Command) {
...
func (cr *commandRegistry) initGCCmdFlags(gcCmd *cobra.Command) {
...
func (cr *commandRegistry) initPprofCmdFlags(pprofCmd *cobra.Command) {
...
func (cr *commandRegistry) initStartCmdFlags(startCmd *cobra.Command) {
...
func (cr *commandRegistry) initStartInstanceCmdFlags(startInstanceCmd *cobra.Command) {
...
func (cr *commandRegistry) initSyncCmdFlags(syncCmd *cobra.Command) {
...
func (cr *commandRegistry) initStageCmdFlags(stageCmd *cobra.Command) {
...
func (cr *commandRegistry) initLogsCmdFlags(logsCmd *cobra.Command) {
...
func (cr *commandRegistry) initStageURLCmdFlags(stageURLCmd *cobra.Command) {
...
func (cr *commandRegistry) initMonitorCmdFlags(monitorCmd *cobra.Command) {
...
func (cr *commandRegistry) initUpdateCmdFlags(updateCmd *cobra.Command) {
...
func (cr *commandRegistry) initGrafanaAnnotationCmdFlags(grafanaAnnotationCmd *cobra.Command) {
...
func (cr *commandRegistry) initGrafanaStartCmdFlags(grafanaStartCmd *cobra.Command) {
...
func (cr *commandRegistry) initFluentBitStartCmdFlags(fluentBitStartCmd *cobra.Command) {
...
func (cr *commandRegistry) initOpentelemetryStartCmdFlags(opentelemetryStartCmd *cobra.Command) {
