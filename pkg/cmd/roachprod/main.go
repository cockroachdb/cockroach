...
var deployCmd = &cobra.Command{
	Use:   "deploy <cluster> <application> <version>/<binary-path>",
...
Currently available application options are:
  %s`, strings.TrimSpace(cockroachApp+releaseApp+customizedApp+localApp)),
	...
	Run: wrap(func(cmd *cobra.Command, args []string) error {
		...
		if args[1] == "local" {
			if len(args) < 3 {
				return errors.New("local application requires a binary path: deploy <cluster> local <binary-path>")
			}
			binaryPath = args[2]
		} else if len(args) == 3 {
			versionArg = args[2]
		...
		return roachprod.Deploy(context.Background(), config.Logger, args[0], args[1],
			versionArg, binaryPath, pause, deploySig, deployWaitFlag, deployGracePeriod, secure)
	}),
}
