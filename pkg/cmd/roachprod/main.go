...
// Return codes come from rperrors.Error.ReturnCode().
func wrap(f func(cmd *cobra.Command, args []string) error) func(cmd *cobra.Command, args []string) {
	return func(cmd *cobra.Command, args []string) {
		err := f(cmd, args)
		if err != nil {
			roachprodError, ok := err.(rperrors.Error)
			if !ok {
				roachprodError = rperrors.UnclassifiedError{err}
			}

			cmd.Println("Error: ", roachprodError.Error())

			os.Exit(roachprodError.ReturnCode())
		}
	}
}
