...
pids=$(ps axeww -o pid -o command | \
  ...
  awk '/ROACHPROD=(%[2]s\/%[3]d%[4]s)[ \/]/ { print $1 }')
if [ -n "${pids}" ]; then
  kill -%[5]d ${pids}
%[6]s
fi`,
...
func (c *SyncedCluster) Status() {
  ...
  sed 's/export ROACHPROD=//g' | \
  awk '/ROACHPROD=(%s\/%d%s)[ \/]/ {print $2, $1}'`,
			c.Name, c.Nodes[i], c.escapedTag())
		cmd += ` | sort | uniq);
...
func (c *SyncedCluster) Run(stdout, stderr io.Writer, nodes []int, title, cmd string) error {
	...
	c.Parallel(display, len(nodes), 0, func(i int) ([]byte, error) {
		...
		nodeCmd := fmt.Sprintf(`export ROACHPROD=%s/%d%s GOTRACEBACK=crash && bash -c %s`,
			c.Name, nodes[i], c.Tag, ssh.Escape1(expandedCmd))
		if c.IsLocal() {
...
func (c *SyncedCluster) DistributeCerts() {
	...
	c.Parallel(display, 1, 0, func(i int) ([]byte, error) {
		...
		if c.IsLocal() {
			...
			nodeNames = append(nodeNames, "$(hostname)", c.VMs[0].Name)
		} else {
			...
			for i := range c.VMs {
				nodeNames = append(nodeNames, c.VMs[i].Name)
			}
			for i := range c.VMs {
...
func (c *SyncedCluster) SSH(sshArgs, args []string) error {
	...
	if c.IsLocal() {
		...
		if len(args) > 0 {
			cmd += fmt.Sprintf("export ROACHPROD=%s/%d%s ; ", c.Name, c.Nodes[0], c.Tag)
		...
		if len(args) > 0 {
			allArgs = append(allArgs, fmt.Sprintf("export ROACHPROD=%s/%d%s ;", c.Name, c.Nodes[0], c.Tag))
		}
