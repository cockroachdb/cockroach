...
// set distribute certs, set cluster settings, and initialize the cluster. Also,
// if we're only starting a single node in the cluster and it happens to be the
// first node, we use `start-single-node` (this was written to provide a short
// hand to start a single node cluster with a replication factor of 1)
func (r Cockroach) Start(c *SyncedCluster, extraArgs []string) {
	...
	c.Parallel("", len(c.ServerNodes()), parallelism, func(nodeIdx int) ([]byte, error) {
		vers, err := getCockroachVersion(c, c.ServerNodes()[nodeIdx])
		if err != nil {
			return nil, err
		}
		...
		if h.skipInitialization(nodeIdx, vers) {
			return nil, nil
		}

		if node := c.ServerNodes()[nodeIdx]; node != 1 {
			panic(fmt.Sprintf(
				"programming error: expecting to run initialization through node 1, found node %d", node))
		}
...
func (h *crdbStartHelper) setClusterSettings(nodeIdx int) (string, error) {
	...
	sess, err := h.c.newSession(nodeIdx)
	if err != nil {
		return "", err
	}
	defer sess.Close()
...
func (h *crdbStartHelper) skipInitialization(nodeIdx int, vers *version.Version) bool {
	nodes := h.c.ServerNodes()

	// We don't init when invoking with `start-single-node`, for nodes other
	// than n1, and for nodes running <20.1 (where the --join flags are
	// constructed in a manner such that n1 doesn't have any, and is left to
	// auto-init).
