...
func New() *Provider {
	...
	p.mu.vnets = make(map[string]network.VirtualNetwork)
	return p
}
...
// Clean SSH implements vm.Provider, is a no-op, and returns nil.
...
func (p *Provider) Create(names []string, opts vm.CreateOpts) error {
	...
	vnets := make([]network.VirtualNetwork, len(p.opts.locations))
	for locIdx, nodes := range nodesByLocIdx {
		...
		errs.Go(func() error {
			...
			// Create the VNet and the subset
			vnet, subnet, err := p.createVNet(ctx, group, locIdx+1)
			vnets[locIdx] = vnet
	...
	return p.createVNetPeerings(ctx, vnets)
}
...
func (p *Provider) FindActiveAccount() (string, error) {
	// It's a JSON Web Token, so we'll just dissect it enough to
	// get the data that we want. There are three base64-encoded segments
	// separated by periods.  The second segment is the "claims" JSON object.
...
func (p *Provider) List() (vm.List, error) {
	...
	for it.NotDone() {
		...
		m := vm.VM{
			...
			// code assumes particular formats.
...
func (p *Provider) createVM(
	...
	// locally-attached SSD is available at /mnt, so we just need
	// to create the necessary directory and preflight.
	//
...
func (p *Provider) createVNet(
	ctx context.Context, group resources.Group, offset int,
) (vnet network.VirtualNetwork, subnet network.Subnet, err error) {
	vnetName := fmt.Sprintf("%s-%d", *group.Name, offset)

	p.mu.Lock()
	vnet, ok := p.mu.vnets[vnetName]
	p.mu.Unlock()
	if ok {
	...
	vnet = network.VirtualNetwork{
		...
		VirtualNetworkPropertiesFormat: &network.VirtualNetworkPropertiesFormat{
			AddressSpace: &network.AddressSpace{
				AddressPrefixes: &[]string{fmt.Sprintf("10.%d.0.0/16", offset)},
			},
			Subnets: &[]network.Subnet{
				{
					Name: group.Name,
					SubnetPropertiesFormat: &network.SubnetPropertiesFormat{
						AddressPrefix: to.StringPtr(fmt.Sprintf("10.%d.0.0/18", offset)),
					},
				},
			},
		},
	...
	if err != nil {
		err = errors.Wrapf(err, "creating Azure VNet %s", vnetName)
		return
	}
	if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
		err = errors.Wrapf(err, "creating Azure VNet %s", vnetName)
	...
	if err == nil {
		p.mu.Lock()
		p.mu.vnets[vnetName] = vnet
		p.mu.Unlock()
		subnet = (*vnet.Subnets)[0]
		log.Printf("created Azure VNet %s", vnetName)
	}
	return
}

// createVNetPeerings creates a full compliment of
func (p *Provider) createVNetPeerings(
	ctx context.Context, vnets []network.VirtualNetwork,
) error {
	sub, err := p.getSubscription(ctx)
	if err != nil {
		return err
...
func (p *Provider) getSubscription(ctx context.Context) (sub subscriptions.Subscription, err error) {
	p.mu.Lock()
