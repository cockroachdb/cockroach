...
// Custom marshaller augments ebsDisk configurations with additional attributes.
func (d *ebsDisk) MarshalJSON() ([]byte, error) {
	type disk ebsDisk
...
func (d *ebsDisk) Set(s string) error {
	...
	if d.VolumeSize == 0 {
		return errors.AssertionFailedf("VolumeSize required for any disk")
	}
	...
	case "gp3":
		...
		if d.Throughput == 0 {
			return errors.AssertionFailedf("Throughput required for gp3 disk: 125+ Mib/s")
		}
	case "io1", "io2":
		if d.IOPs == 0 {
