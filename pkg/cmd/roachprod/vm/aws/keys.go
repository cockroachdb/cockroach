...
func (p *Provider) sshKeyImport(keyName string, region string) error {
	...
	timeString := fmt.Sprintf("%d-%02d-%02dT%02d:%02d:%02dZ", timestamp.Year(), timestamp.Month(), timestamp.Day(), timestamp.Hour(), timestamp.Minute(), timestamp.Second())
	...
	args := []string{
		...
		"--tag-specifications", fmt.Sprintf("ResourceType=key-pair,Tags=[{Key=IAM-UserName,Value=%s}, {Key=CreatedAt,Value=%s}]", user, timeString),
	}
	err = p.runJSONCommand(args, &data)
