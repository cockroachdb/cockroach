...
func TestPkgsFromDiff(t *testing.T) {
	for filename, expPkgs := range map[string]map[string]pkg{
		"testdata/10305.diff": {
			filepath.Join("pkg", "roachpb"): {tests: []string{"TestLeaseEquivalence"}},
			filepath.Join("pkg", "storage"): {tests: []string{"TestStoreRangeLease", "TestStoreRangeLeaseSwitcheroo"}},
		},
		"testdata/skip.diff": {
		...
		"testdata/27595.diff": {
			...
			filepath.Join("pkg", "storage", "closedts", "container"): {tests: []string{"TestTwoNodes"}},
