...
// clusterOptWithPreAllocateClusterFn returns a clusterOpt with
// preAllocateClusterFn set to a function that always returns nil to mock a
// cluster provisioning error.
func clusterOptWithPreAllocateClusterFn() clustersOpt {
	return clustersOpt{
		typ:       roachprodCluster,
		user:      "test_user",
		cpuQuota:  1000,
...
//  2. After provisioning error, the worker will get a GitHub POST error (mocked)
//  3. Test should still run, and after the runner should have a count of the
//     provisioning errors and GitHub errors.
//  4. Runner should return a joined error for failing to provision and a GitHub error
func TestRunnerProvisionErrorGithubError(t *testing.T) {
	...
	var buf syncedBuffer
	copt := clusterOptWithPreAllocateClusterFn()
	lopt := defaultLoggingOpt(&buf)
	test := registry.TestSpec{
		Name:             `TestPass`,
		Owner:            OwnerUnitTest,
	...
	if !(errors.Is(err, errGithubPostFailed) &&
		errors.Is(err, errSomeClusterProvisioningFailed)) {
		t.Fatalf("expected joined errors: %s and %s", errGithubPostFailed,
			errSomeClusterProvisioningFailed)
	}

	// Assert test runner workers' error counts are as expected
	if !(runner.numGithubPostErrs == 1 && runner.numClusterErrs == 1) {
		t.Fatalf("expected 1 error each for numGithubPostErrs and numClusterErrs but got "+
			"numGithubPostErrs:%d and numClusterErrsL%d", runner.numGithubPostErrs,
			runner.numClusterErrs)
	}
}
