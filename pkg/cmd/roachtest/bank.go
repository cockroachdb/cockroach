...
func (s *bankState) splitMonkey(
	...
	for curRound := uint64(1); !s.done(ctx); curRound++ {
		...
		nodes := make([]string, c.nodes, c.nodes)
		for i := 0; i < c.nodes; i++ {
		...
		case 1:
			...
			const relocateQuery = `ALTER TABLE bank.accounts EXPERIMENTAL_RELOCATE VALUES (ARRAY[%s], %d);`
			relocateQueryFormat := fmt.Sprintf(relocateQuery, strings.Join(nodes[1:], ", "), key)
			c.l.Printf("round %d: relocating key %d to nodes %s\n",
				curRound, key, nodes[1:])

			_, err := client.db.Exec(relocateQueryFormat)
			if err != nil && !(testutils.IsSQLRetryableError(err) || isExpectedRelocateError(err)) {
				s.errChan <- err
			}
		}
...
func isExpectedRelocateError(err error) bool {
	return testutils.IsError(err, "(descriptor changed)")
}
