...
func registerSQLAlchemy(r *registry) {
	runSQLAlchemy := func(
		...
		// Set the default ttl seconds to 5 sec since we're droping and createing so many tables.
		if _, err := db.Exec(`ALTER RANGE default CONFIGURE ZONE USING gc.ttlseconds = 5`); err != nil {
			t.Fatal(err)
		}
