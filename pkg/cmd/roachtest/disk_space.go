...
func runDiskUsage(t *test, c *cluster, duration time.Duration, tc diskUsageTestCase) {
	...
	loadDuration := " --duration=" + (time.Second).String()
	...
	const stmtZone = "ALTER RANGE default EXPERIMENTAL CONFIGURE ZONE 'gc: {ttlseconds: 10000000}'"
	run(stmtZone)
		...
		for {
			...
			case <-ticker.C:
				...
				if i == 0 {
					...
					// less than 150MB increase for ttl of 10 secs.
