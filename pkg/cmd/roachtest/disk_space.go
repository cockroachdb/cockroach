...
type diskUsageTestCase struct {
	name             string
	ratioDiskFilled  []float64
	diskEmptyInBytes []uint64
...
func runDiskUsage(t *test, c *cluster, duration time.Duration, tc diskUsageTestCase) {
	...
	for i := 1; i <= numNodes; i++ {
		...
		if tc.ratioDiskFilled !=nil {
			fillBallastCommand =append(fillBallastCommand, "--fill_ratio", fmt.Sprint(tc.ratioDiskFilled[i-1]))
		}
	...
	diskUsages := func() []int {
		...
		for i := 1;i<=numNodes;i++ {
			var err error
			diskUsages[i-1], err = c.getDiskUsageInByte(ctx, c.Node(i))
			if err!=nil {
			...
			func() error {
				...
				for time.Since(start) < (duration / 2) {
					...
					if randomSeed < tc.singleKeyOpProb {
						// to simulate hot row update loop
						k = 0
					} else {
	...
	testCases := []diskUsageTestCase{
		{
			...
			diskEmptyInBytes: []uint64{uint64(70) * mbInBytes, uint64(70) * mbInBytes, uint64(70) * mbInBytes, uint64(70) * mbInBytes},
			singleKeyOpProb:   0.5,
		},
		{
			name:              "update_hot_key",
		...
		},
		{
			...
			ratioDiskFilled: []float64{0.9, 0.9, 0.9, 0.9},
			singleKeyOpProb: 0.5,
		},
		{
