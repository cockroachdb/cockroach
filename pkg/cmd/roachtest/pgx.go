...
func registerPgx(r *testRegistry) {
	runPgx := func(
		...
		// Using apantel's fork of pgx as a temporary solution to flakiness. Revert to jackc's when fixed.
		t.Status("installing pgx")
		if err := repeatRunE(
			ctx, c, node, "install pgx", "go get -u github.com/apantel/pgx",
		); err != nil {
			t.Fatal(err)
		}
		...
		// Change the path from apantel to jackc when the flakiness is fixed.
		t.Status("running pgx test suite")
		xmlResults, err := repeatRunWithBuffer(
			ctx, c, t.l, node,
			"run pgx test suite",
			"cd `go env GOPATH`/src/github.com/apantel/pgx && "+
				"PGX_TEST_DATABASE='postgresql://root:@localhost:26257/pgx_test' go test -v 2>&1 | "+
				"`go env GOPATH`/bin/go-junit-report",
		)
		if err != nil {
			t.Fatal("Running pgx tests failed: %v", err)
		}
