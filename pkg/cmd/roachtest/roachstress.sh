...
branches.
If roachstress detects that there's an uncomitted change, it would always
try to rebuild into artifacts/<commit-hash>-dirty to allow temporary test
or cockroach changes for reproductions.
Local change rebuilding behaviour could be disabled by -b flag.
EOF
}
...
# to distinguish since cases OPTIND points to test name or OPTIND points to the first arg
# to roachtest look identical.

# Check if we parsed beyond last arg already, then its an error.
[ $# -lt "${OPTIND}" ] && echo -e "No test name provided\n" && help && exit 1

# We are at arg, but it may be test name or first roachtest arg.
if [ $OPTIND -gt 1 ] ; then
  # Check if arg to the left is '--' then we don't have test name.
  shift $((OPTIND - 2))
  [ "$1" = "--" ] && echo -e "No test name provided\n" && help && exit 1
  shift 1
fi
...
shift 1
if [ $# -gt 0 ] ; then
  [ "$1" != "--" ] && echo -e "Can only provide single test name, did you forget -- before roachtest arguments\n" && help && exit 1
  shift 1
fi
