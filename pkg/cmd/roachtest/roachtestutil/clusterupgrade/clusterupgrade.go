...
const (
	// MainVersion is the default used to represent that the binary
	// passed to roachtest should be used.
	MainVersion = ""
)
...
func UploadVersion(
	...
	if newVersion == "" {
		if err := c.PutE(ctx, l, currentCockroachPath, binaryName, nodes); err != nil {
			return "", err
		}
	} else if binary, ok := versionsBinaryOverride[newVersion]; ok {
...
func StartWithBinary(
	...
	nodes option.NodeListOption,
	binaryPath string,
	roachprodSequential bool,
) {
	settings := install.MakeClusterSettings(install.BinaryOption(binaryPath))
	startOpts := option.DefaultStartOpts()
	startOpts.RoachprodOpts.Sequential = roachprodSequential
...
func RestartNodesWithNewBinary(
	...
	for _, node := range nodes {
		...
		binary, err := UploadVersion(ctx, l, c, c.Node(node), newVersion, currentCockroachPath, versionsBinaryOverride)
		if err != nil {
			return err
		}
		StartWithBinary(ctx, l, c, nodes, binary, true /* sequential */)
...
// first node's binary version (which is assumed to be every node's
// binary version). We rely on the cluster's internal self-upgrading
// mechanism.
func WaitForClusterUpgrade(
	ctx context.Context, l *logger.Logger, nodes option.NodeListOption, dbFunc func(int) *gosql.DB,
) error {
	newVersion, err := BinaryVersion(ctx, dbFunc(nodes[0]))
	if err != nil {
		return err
	}
