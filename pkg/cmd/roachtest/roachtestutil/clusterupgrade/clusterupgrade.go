...
func UploadBinaryVersion(
	...
	overrideBinary, isOverriden := t.VersionsBinaryOverride()[v.String()]
	isOverriden = isOverriden && binary == "cockroach"
	if isOverriden {
		l.Printf("using cockroach binary override for version %s: %s", v, overrideBinary)
		defaultBinary = overrideBinary
	}

	if v.IsCurrent() || isOverriden {
