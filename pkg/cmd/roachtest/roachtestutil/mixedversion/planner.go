...
//     - InitialUpgradeStage: upgrade all nodes to the next cockroach version
//     (running mixed-version hooks at times determined by the planner).
//     - RollbackUpgradeStage: maybe downgrade all nodes back to the previous
//     version (running mixed-version hooks again).
//     - FinalUpgradeStage: if a rollback was performed, upgrade all nodes back
//     to the next version one more time (running mixed-version hooks).
//     - RunningUpgradeMigrationsStage: reset `preserve_downgrade_option`,
//     allowing the cluster to upgrade. Mixed-version hooks may be executed while
//     this is happening.
