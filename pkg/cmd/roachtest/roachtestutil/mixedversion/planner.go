...
func (p *testPlanner) startupSteps() []testStep {
	...
	return p.hooks.StartupSteps(p.longRunningContext(), p.newRNG(), p.isLocal)
}
...
func (p *testPlanner) testStartSteps() []testStep {
	return append(
		p.startupSteps(),
		p.hooks.BackgroundSteps(p.longRunningContext(), p.bgChans, p.newRNG(), p.isLocal)...,
	)
}
...
func (p *testPlanner) afterUpgradeSteps(
	...
	if scheduleHooks {
		return p.hooks.AfterUpgradeFinalizedSteps(p.currentContext, p.newRNG(), p.isLocal)
	}
...
func (p *testPlanner) changeVersionSteps(
	...
	for j, node := range previousVersionNodes {
		...
		if scheduleHooks {
			steps = append(steps, p.hooks.MixedVersionSteps(p.currentContext, p.newRNG(), p.isLocal)...)
		} else if j == waitIndex {
			...
			steps = append(steps, p.newSingleStep(waitStep{
				dur: pickRandomDelay(p.newRNG(), p.isLocal, possibleWaitDurations),
			}))
		}
	}
...
func (p *testPlanner) finalizeUpgradeSteps(
	...
	if scheduleHooks {
		mixedVersionStepsDuringMigrations = p.hooks.MixedVersionSteps(p.currentContext, p.newRNG(), p.isLocal)
	}
