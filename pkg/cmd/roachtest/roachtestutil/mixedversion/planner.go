...
// mapSingleStep iterates over every step in the test plan and calls
// the given function `f` for every `singleStep` (i.e., every step
// that actually performs an action). The function should return a
// list of testSteps that are be used in the plan instead of the given
// step.
func (plan *TestPlan) mapSingleStep(f func(*singleStep, bool) []testStep) {
...
func newStepIndex(plan *TestPlan) *stepIndex {
	var data []singleStepInfo

	plan.mapSingleStep(func(ss *singleStep, isConcurrent bool) []testStep {
		data = append(data, singleStepInfo{
			step:         ss,
...
func (si *stepIndex) ContextForInsertion(step *singleStep, op mutationOp) Context {
	for j, info := range si.data {
		if info.step == step {
			...
			if j == 0 || j == len(si.data)-1 || op == mutationInsertConcurrent {
				return info.step.context.clone()
			}

			idx := j // inserting before `step`
			if op == mutationInsertAfter {
				idx = j + 1 // inserting after `step`
			}

			return si.data[idx].step.context.clone()
		}
	}
...
func (plan *TestPlan) singleSteps() []*singleStep {
	...
	plan.mapSingleStep(func(ss *singleStep, _ bool) []testStep {
...
func (plan *TestPlan) newStepSelector() *stepSelector {
	return &stepSelector{
		selectedSteps: plan.singleSteps(),
	}
}
