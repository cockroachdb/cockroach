...
type (
	...
	TestPlan struct {
		...
		initSteps []testStep
		// startClusterID is step ID after which the cluster should be
...
func (p *testPlanner) Plan() *TestPlan {
	...
	for prevVersionIdx := 0; prevVersionIdx+1 < len(p.versions); prevVersionIdx++ {
		...
		performingRollback := p.shouldRollback(toVersion)
		firstUpgradeStage := FinalUpgradeStage
		if performingRollback {
			firstUpgradeStage = InitialUpgradeStage
		}

		// previous -> next
		plan.Add(p.upgradeSteps(firstUpgradeStage, fromVersion, toVersion))
		if performingRollback {
