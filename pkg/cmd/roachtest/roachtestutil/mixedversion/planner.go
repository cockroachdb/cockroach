...
const (
	...
	// runs, so that we are able to implementat mutators that select,
	// for example, "stages after rollback" by doing `stage >
	// RollbackUpgrade`.  Note that `BackgroundStage` is special in the
	// sense that it may continue to run across stage changes, so doing
	// direct stage comparisons as mentioned above doesn't make sense
