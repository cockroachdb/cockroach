...
const (
	// MultiRegionSimulation is a mutator that simulates a multi-region
	// environment by introducing an artificial network latency between nodes
	// in different regions.
	MultiRegionSimulation = "multi_region_simulation"
)

type multiRegionSimulationMutator struct{}

func (m multiRegionSimulationMutator) Name() string {
	return MultiRegionSimulation
}

func (m multiRegionSimulationMutator) Probability() float64 {
	return 0.3
}

func (m multiRegionSimulationMutator) Generate(
	rng *rand.Rand, plan *TestPlan, planner *testPlanner,
) ([]mutation, error) {
	var mutations []mutation
	steps := plan.newStepSelector()
	nodeList := planner.currentContext.System.Descriptor.Nodes
	...
	if err != nil {
		return nil, fmt.Errorf("failed to create network latency args: %w", err)
	}

	firstStep := stepSelector{steps[0]}
	lastStep := stepSelector{steps[len(steps)-1]}

	addLatencyStep := firstStep.InsertBefore(simulateMultiRegionStep{f, args, regionToNodes})
	addRecoveryStep := lastStep.InsertAfter(endMultiRegionSimulationStep{f, args})
