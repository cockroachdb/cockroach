...
func (m panicNodeMutator) Generate(
	...
	for _, upgrade := range upgrades {
		...
		concurrentStepMarked := false
		var concurrentStep *singleStep
		...
		isIncompatibleStep := func(s *singleStep) bool {
			...
			// cause the cluster to lose quorum, and we cannot run restartWithNewBinaryStep on the
			// panicked node as it is already dead, so we avoid any system restarts.
			...
			if idx.IsConcurrent(s) {
				if !concurrentStepMarked {
					concurrentStepMarked = true
					concurrentStep = s
				}
		...
		// If validEndStep is nil, it means that there are no steps after the panic step (not including the step we insert
		// before the panic) that are compatible with a dead node, so we immediately restart the node after the panic.
		if validEndStep == nil {
