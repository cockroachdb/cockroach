...
func (m panicNodeMutator) Generate(
	...
	for _, upgrade := range upgrades {
		...
		panicIdx, err := upgrade.FindStep(stepToPanic)
		...
		isValidStartStep := func(s *singleStep) bool {
			implStart, validStart := s.impl.(restartWithNewBinaryStep)
			...
			return !((validStart && implStart.node == targetNode[0]) || (validStartVirtual && implStartVirtual.node == targetNode[0]) || waitForStable || runHook)
		}

		validStartInserts := upgrade.FindValidSteps(panicIdx, isValidStartStep)
