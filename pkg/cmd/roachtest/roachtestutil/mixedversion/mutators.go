...
func (m panicNodeMutator) Generate(
	...
	possiblePointsInTime := plan.
		newStepSelector().
		// We don't want to panic nodes concurrently with other steps, and due to the chance of InsertBefore inserting
		// concurrently, we filter out any steps that are running concurrently.
		Filter(func(s *singleStep) bool {
