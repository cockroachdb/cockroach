...
var (
	...
	// minBootstrapARM64Version is the minimum version for which there
	...
	minBootstrapARM64Version = clusterupgrade.MustParseVersion("v22.2.0")
...
func (t *Test) RunE() (*TestPlan, error) {
	...
	t.logger.Printf("mbs=%s,msv=%s,minUpgrades=%d,maxUpgrades=%d", t.options.minimumBootstrapVersion, t.options.minimumSupportedVersion, t.options.minUpgrades, t.options.maxUpgrades)
...
// -- minimum supported version <= "from" version of a skip upgrade.
// -- if skip upgrades are present, then the skip to final version is present.
func (t *Test) assertSkipUpgrade(plan *TestPlan, skipVersions bool) error {
	...
	hasEligibleSkips := false
	{
		if n, err := release.MajorReleasesBetween(&t.options.minimumSupportedVersion.Version, &lastUpgrade.to.Version); err != nil {
			...
			hasEligibleSkips = supportsSkipUpgradeTo(lastUpgrade.to) && n >= 2
		}
	}
	...
	for i := 0; i < len(upgrades); i++ {
		...
		if n, err := release.MajorReleasesBetween(&upgrades[i].from.Version, &upgrades[i].to.Version); err != nil {
			...
			if upgrades[i].from.LessThan(t.options.minimumSupportedVersion) && t.options.minimumSupportedVersion.LessThan(upgrades[i].to) {
	...
	if numSkips > 0 && supportsSkipUpgradeTo(upgrades[len(upgrades)-1].to) && numSkipsToFinal == 0 {
...
func (t *Test) chooseUpgradePath(
	...
	//  - it is not older than the minimum boostrap ARM64 version
	//	- it is not older than the minimum bootstrap version
	isAvailable := func(v *clusterupgrade.Version) bool {
		if t.clusterArch() == vm.ArchARM64 && !v.AtLeast(minBootstrapARM64Version) {
			return false
		}
	...
	// predecessor along with the predecessor's predecessor.
	// This is the function to change in case the rules around what upgrades are
	// possible in CRDB change.
	possiblePredecessorsFor := func(v *clusterupgrade.Version) ([]*clusterupgrade.Version, error) {
		if v.Series() == t.options.minimumSupportedVersion.Series() && t.options.minimumSupportedVersion.LessThan(v) {
			return []*clusterupgrade.Version{t.options.minimumSupportedVersion}, nil
		}
		...
		// N.B. assertValidTest ensures that minimumBootstrapVersion <= minimumSupportedVersion.
		// Thus, after clamping wrt same release series: minimumBootstrapVersion <= minimumSupportedVersion <= pred.
		...
		// If the immediate predecessor is of the same release series as msv, then skip upgrade isn't feasible since
		// it would end up bypassing user-specified hooks.
		if pred.Series() == t.options.minimumSupportedVersion.Series() {
			// Owing to maybeClampMsbMsv above, we have msv <= pred. However, predPred is necessarily from the _previous_
			// release series, thus predPred < msv, and hence any user-specified hooks would not execute.
			return []*clusterupgrade.Version{pred}, nil
		}

		// Otherwise, we look for feasible skip upgrades by examining the next available predecessor.
		...
		// When skip-version upgrades are enabled, we prioritize the skip to the _current_ release,
		// which is the most important upgrade to be tested on any release branch.
		if v.IsCurrent() {
			return []*clusterupgrade.Version{predPred}, nil
		}
		// If we got this far, then both skip and non-skip upgrades are feasible.
	...
	if len(upgradePath) < numUpgrades+1 {
		...
		if skipVersions {
			...
			t.logger.Printf("%s: ARM64 is only supported on %s+", warnMsg, minBootstrapARM64Version)
		} else {
