...
const (
	...
	clusterInitLabel           = "run cluster init hooks"
...
// InClusterInit registers a callback that is run during cluster
...
// stage.
func (t *Test) InClusterInit(desc string, fn stepFunc) {
	...
	// gating them.
	t.hooks.AddClusterInit(versionUpgradeHook{name: desc, fn: fn})
}
...
func (t *Test) choosePreviousReleases() ([]*clusterupgrade.Version, error) {
	...
	findPossibleUpgradePaths = func(v *clusterupgrade.Version, numUpgrades int) ([][]*clusterupgrade.Version, error) {
		...
		for _, pred := range predecessors {
			...
			if t.options.minimumBootstrapVersion != nil && !pred.AtLeast(t.options.minimumBootstrapVersion) {
				continue
			}
...
func (th *testHooks) AddClusterInit(hook versionUpgradeHook) {
	th.clusterInit = append(th.clusterInit, hook)
}
...
func (th *testHooks) ClusterInitSteps(testContext *Context, rng *rand.Rand) []testStep {
	return th.clusterInit.AsSteps(rng, testContext, nil)
}
...
func assertValidTest(test *Test, fatalFunc func(...interface{})) {
	...
	if minBootstrapVersion := test.options.minimumBootstrapVersion; minBootstrapVersion != nil {
		...
		maxUpgradesFromBootstrapVersion, err := release.MajorReleasesBetween(&minBootstrapVersion.Version, &clusterupgrade.CurrentVersion().Version)
		if err != nil {
			fail(err)
		}
		...
		// many upgrades.
		test.options.maxUpgrades = min(maxUpgrades, maxUpgradesFromBootstrapVersion)
	}
}
