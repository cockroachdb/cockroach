...
func (t *Test) assertPlanValid(plan *TestPlan, numUpgrades int, skipVersions bool) error {
	...
	if err := t.assertMsbAndMsv(plan); err != nil {
		return err
	}
...
func (t *Test) assertMsbAndMsv(plan *TestPlan) error {
	...
	for _, v := range allVersions {
		...
		if v.Series() == mbvSeries {
			if mbv != nil && v.LessThan(mbv) {
				return errors.Newf("minimum bootstrap version is not <= %s", mbv, v)
			}
		}
	}
	return nil
...
func (t *Test) assertSkipUpgrade(plan *TestPlan, skipVersions bool) error {
	...
	for i := 0; i < len(upgrades); i++ {
		...
		if n, err := release.MajorReleasesBetween(&upgrades[i].from.Version, &upgrades[i].to.Version); err != nil {
			...
			if upgrades[i].from.LessThan(t.options.minimumSupportedVersion) {
				return errors.Newf("minimum supported version %s is not <= upgrade from %s to %s", t.options.minimumSupportedVersion, upgrades[i].from, upgrades[i].to)
			}
		}
		prev = upgrades[i]
	}
