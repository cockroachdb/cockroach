...
func (t *Test) plan() (plan *TestPlan, retErr error) {
	...
	if len(plan.upgrades) == 0 {
		return nil, errors.Newf("internal error: plan has no upgrades\n%s", plan.prettyPrintInternal(true))
	}
	// Finally, check that user-specified hooks occur in the plan.
	err := t.assertExpectedUserHooks(plan)
	if err != nil {
		return nil, err
	}
...
func (t *Test) assertExpectedUserHooks(plan *TestPlan) error {
	...
	var startHooks, firstVersionHooks, otherVersionHooks hooks
	startHooks = t.hooks.beforeClusterStart
	firstVersionHooks = append(firstVersionHooks, t.hooks.startup...)
	firstVersionHooks = append(firstVersionHooks, t.hooks.background...)
	...
	startHooksFound := 0
	firstVersionHooksFound := 0
	otherVersionHooksFound := 0
	assertStartHooks := len(startHooks) > 0
	assertFirstVersionHooks := len(firstVersionHooks) > 0
	assertOtherVersionHooks := len(otherVersionHooks) > 0
	...
	for i := 0; i < len(allVersions)-1; i++ {
		...
		// firstVersionHooks are only run once we reach an upgrade which satisfies minimumSupportedVersion.
		if firstVersionHooksFound == 0 {
			for _, h := range firstVersionHooks {
				if userHooks[from] == nil {
					userHooks[from] = make(map[string]bool)
				}
				userHooks[from][h.name+"_"+h.id] = true
				firstVersionHooksFound++
			}
		}
		for _, h := range otherVersionHooks {
			...
			userHooks[from][h.name+"_"+h.id] = true
			otherVersionHooksFound++
		}
	}

	if assertStartHooks && startHooksFound == 0 {
		return errors.Newf("expected at least one start hook, but none found\n%s", plan.prettyPrintInternal(true))
	}
	if assertFirstVersionHooks && firstVersionHooksFound == 0 {
		return errors.Newf("expected at least one hook in the first upgrade, but none found\n%s", plan.prettyPrintInternal(true))
	}
	if assertOtherVersionHooks && otherVersionHooksFound == 0 {
		return errors.Newf("expected at least one hook in other upgrades, but none found\n%s", plan.prettyPrintInternal(true))
	}
	...
	// Check that no unused hooks remain.
	for from := range userHooks {
		for hookName := range userHooks[from] {
			return errors.Newf("unused user hook: %q in version: %q\n%s", hookName, from, plan.prettyPrintInternal(true))
		}
	}
	return nil
}
...
func (t *Test) chooseUpgradePath() ([]*clusterupgrade.Version, error) {
	...
	// Checks whether the predecessor is feasible subject to these constraints:
	//  - it is not older than the minimum supported ARM64 version
	//	- it is not older than the minimum bootstrap version
	isAvailable := func(v *clusterupgrade.Version) bool {
		if t.clusterArch() == vm.ArchARM64 && !v.AtLeast(minSupportedARM64Version) {
			return false
		}
		return t.options.minimumBootstrapVersion == nil || v.AtLeast(t.options.minimumBootstrapVersion)
	}
	// Set to true after the first skip-version upgrade is chosen.
	skipToCurrentChosen := false
	...
	possiblePredecessorsFor := func(v *clusterupgrade.Version) ([]*clusterupgrade.Version, error) {
		...
		pred, err = maybeClampMsbMsv(t.prng, pred, t.options.minimumBootstrapVersion, t.options.minimumSupportedVersion)
		if err != nil {
			return nil, err
		}
		// N.B. clamping must be down before `isAvailable`, otherwise we could miss a feasible predecessor.
		if !isAvailable(pred) {
			// We found an infeasible upgrade path.
			return nil, nil
		}
		...
		predPred, err := t.options.predecessorFunc(t.prng, pred)
		if err != nil {
			return nil, err
		}
		predPred, err = maybeClampMsbMsv(t.prng, predPred, t.options.minimumBootstrapVersion, t.options.minimumSupportedVersion)
		if err != nil {
			return nil, err
		}
		if !isAvailable(predPred) {
			// We can't proceed further than `pred`.
			return []*clusterupgrade.Version{pred}, nil
		}
		// We need to check that msv <= predPred, otherwise user-specified hooks may not be invoked.
		if predPred.LessThan(t.options.minimumSupportedVersion) {
			// Skip upgrade isn't possible due to msv.
			return []*clusterupgrade.Version{pred}, nil
		}

		if pred.Series() == t.options.minimumSupportedVersion.Series() {
			maxTestUpgrades, err := release.MajorReleasesBetween(&t.options.minimumSupportedVersion.Version, &v.Version)
			if err != nil {
				return nil, err
			}
		...
		// This logic makes sure that, when skip-version upgrades are enabled, it happens
		// when upgrading to the _current_ release, which is the most important
		// upgrade to be tested on any release branch.
		if !skipToCurrentChosen {
			skipToCurrentChosen = true
	...
	curVersion := clusterupgrade.CurrentVersion()

	for j := 0; j < numUpgrades; j++ {
		predecessors, err := possiblePredecessorsFor(curVersion)
		if err != nil {
			return nil, err
		}
	...
	if skipVersions && !skipToCurrentChosen {
...
func maybeClampMsbMsv(
	...
	// If the patch version of `minVersion` is 0, then it follows that minVersion <= v, so we're done.
	if minVersion.Patch() == 0 {
		return v, nil
	}
	// Otherwise, pick a random patch between minVersion.Patch and minVersion.LatestPatch.
