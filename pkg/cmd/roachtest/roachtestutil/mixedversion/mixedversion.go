...
func latestPredecessor(_ *rand.Rand, v, minSupported *clusterupgrade.Version) (*clusterupgrade.Version, error) {
...
func randomPredecessor(rng *rand.Rand, v, minSupported *clusterupgrade.Version) (*clusterupgrade.Version, error) {
	...
	if predV.PreRelease() != "" && !predV.AtLeast(minSupported) {
		return nil, fmt.Errorf(
			...
			predV.Series(), predV, minSupported,
		)
	}

	// At this point, we know that the latest predecessor is not a
	// pre-release. If the minimum supported version's patch version
	// is 0, we can pick any predecessor, so return early.
