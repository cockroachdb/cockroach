...
func TestMapSingleStepsGeneratesDelays(t *testing.T) {
	...
	// Capture plan representation before mutation.
	planBefore := plan.PrettyPrint()
	...
	// Capture plan representation after mutation.
	planAfter := plan.PrettyPrint()

	// Verify we actually inserted some steps.
	require.Greater(t, insertCount, 0, "should have inserted at least one step")

	// The plan SHOULD have changed because we inserted new steps.
	require.NotEqual(t, planBefore, planAfter,
		"plan should change after mapSingleSteps inserts new steps")
}
