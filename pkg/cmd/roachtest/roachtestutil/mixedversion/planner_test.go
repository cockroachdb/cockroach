...
func TestDeterministicHookSeeds(t *testing.T) {
	...
	generateData := func(generateMoreRandomNumbers bool) [][]int {
		...
		mvt := newTest(ctrl, NumUpgrades(1), EnabledDeploymentModes(SystemOnlyDeployment))
...
func Test_startClusterID(t *testing.T) {
	...
	mvt := newTest(ctrl, NeverUseFixtures, EnabledDeploymentModes(SystemOnlyDeployment))
	plan, err := mvt.plan()
	require.NoError(t, err)
	...
	mvt = newTest(ctrl, AlwaysUseFixtures, EnabledDeploymentModes(SystemOnlyDeployment))
	plan, err = mvt.plan()
	require.NoError(t, err)
...
func Test_maxNumPlanSteps(t *testing.T) {
	...
	mvt = newBasicUpgradeTest(ctrl, MaxNumPlanSteps(15))
	...
	mvt = newBasicUpgradeTest(ctrl, MaxNumPlanSteps(5))
	plan, err = mvt.plan()
...
func newTest(ctrl *gomock.Controller, options ...CustomOption) *Test {
...
func Test_stepSelectorFilter(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			mvt := newBasicUpgradeTest(ctrl, NumUpgrades(3), DisableSkipVersionUpgrades, EnabledDeploymentModes(SystemOnlyDeployment))
			plan, err := mvt.plan()
			require.NoError(t, err)
...
func Test_stepSelectorMutations(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			mvt := newBasicUpgradeTest(ctrl, NumUpgrades(tc.numUpgrades), DisableSkipVersionUpgrades, EnabledDeploymentModes(SystemOnlyDeployment))
			plan, err := mvt.plan()
			require.NoError(t, err)
...
func Test_DisableAllMutators(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	mvt := newTest(ctrl, DisableAllMutators())
...
func Test_DisableAllClusterSettingMutators(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	mvt := newTest(ctrl, DisableAllClusterSettingMutators())
