...
func Test_stepSelectorFilter(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			assertStepsFunc: func(t *testing.T, sel *stepSelector) {
				require.Empty(t, sel.selectedSteps)
			},
		...
		},
		{
			...
			assertStepsFunc: func(t *testing.T, sel *stepSelector) {
				require.Len(t, sel.selectedSteps, 3) // number of upgrades we are performing
				for _, s := range sel.selectedSteps {
		...
		},
		{
			...
			assertStepsFunc: func(t *testing.T, sel *stepSelector) {
				require.Len(t, sel.selectedSteps, 1)
				require.IsType(t, runHookStep{}, sel.selectedSteps[0].impl)
				rhs := sel.selectedSteps[0].impl.(runHookStep)
				require.Equal(t, "after finalization", rhs.hook.name)
			},
			expectedRandomStepType: runHookStep{},
		},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			allSteps := sel.selectedSteps

			newSelector := sel.Filter(tc.predicate)
			if tc.expectedAllSteps {
				require.Equal(t, allSteps, newSelector.selectedSteps)
			} else {
			...
			if tc.expectedRandomStepType == nil {
				require.Empty(t, randomSelector.selectedSteps)
			} else {
				require.Len(t, randomSelector.selectedSteps, 1)
				require.IsType(t, tc.expectedRandomStepType, randomSelector.selectedSteps[0].impl)
			}
		})
	}
}
