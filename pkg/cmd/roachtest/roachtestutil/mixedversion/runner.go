...
func (tr *testRunner) run() error {
	...
	for {
		...
		case err := <-stepsErr:
			return err
		case event := <-tr.background.Events():
			if event.Err == nil {
...
func (tr *testRunner) runStep(step testStep) error {
	...
	default:
		...
		return tr.runSingleStep(tr.ctx, ss, stepLogger)
	}
}
...
func (tr *testRunner) runSingleStep(ctx context.Context, ss singleStep, l *logger.Logger) error {
	...
	if err := ss.Run(ctx, l, tr.cluster, tr.newHelper(l)); err != nil {
...
// the error passed with extra information. Specifically, the error
// message will include the ID of the step that failed, the random
// seed used, the binary version on each node when the error occurred,
// and the cluster version before and after the step (in case the
// failure happened *while* the cluster version was updating).
...
func (h *Helper) loggerFor(name string) (*logger.Logger, error) {
	atomic.AddInt64(&h.backgroundCount, 1)

	fileName := invalidChars.ReplaceAllString(strings.ToLower(name), "")
	fileName = fmt.Sprintf("background_%s_%d", fileName, h.backgroundCount)
