...
//     mixed-version hooks at arbitrary points).
//   - downgrade all nodes back to the predecessor version (running
//     mixed-version hooks at arbitrary points again).
...
// back multiple releases instad of just one; picking a patch release
...
func (p *testPlanner) initSteps() []testStep {
	preserveDowngradeNode := randomNode(p.prng, p.opts.crdbNodes)
	return append([]testStep{
		startFromCheckpointStep{id: p.nextID(), version: p.initialVersion, opts: p.opts},
		waitForStableClusterVersionStep{id: p.nextID(), nodes: p.opts.crdbNodes},
...
// nodes to be the same and then run any after-test hooks the user may
// have provided.
func (p *testPlanner) finalSteps() []testStep {
	return append([]testStep{
		waitForStableClusterVersionStep{id: p.nextID(), nodes: p.opts.crdbNodes},
	}, p.hooks.AfterTestSteps(p.nextID)...)
}

func (p *testPlanner) upgradeSteps(from, to string) []testStep {
	msg := fmt.Sprintf("upgrade nodes %v from %q to %q", p.opts.crdbNodes, versionMsg(from), versionMsg(to))
	return p.changeVersionSteps(from, to, msg)
}

func (p *testPlanner) downgradeSteps(from, to string) []testStep {
	msg := fmt.Sprintf("downgrade nodes %v from %q to %q", p.opts.crdbNodes, versionMsg(from), versionMsg(to))
	return p.changeVersionSteps(from, to, msg)
}
...
func (p *testPlanner) changeVersionSteps(from, to, label string) []testStep {
	...
	oldVersionNodes := append(option.NodeListOption{}, p.opts.crdbNodes...)
	...
	for _, node := range nodeOrder {
		steps = append(steps, restartWithNewBinaryStep{id: p.nextID(), version: to, node: node, opts: p.opts})
...
func (p *testPlanner) finalizeUpgradeSteps() []testStep {
	...
	finalizeNode := randomNode(p.prng, p.opts.crdbNodes)
	return append([]testStep{
...
// is assigned an ID, making it easy to correspond the step that
...
func (plan *TestPlan) prettyPrintStep(out *strings.Builder, step testStep, prefix string) {
	...
	// a runnableStep. It can include extra information, such as whether
	// there's a delay associated with the step (in the case of
	// concurrent execution), and what database node the step is
	// connecting to.
	writeRunnable := func(rs runnableStep, extraContext ...string) {
	...
	case delayedStep:
		...
		writeRunnable(s.step.(runnableStep), delayStr)
	default:
		writeRunnable(s.(runnableStep))
	}
}
