...
Same-series upgrades test that patch-level upgrades work correctly, which is important since most customer upgrades are patch upgrades within the same release series. The probability value (0.0 to 1.0) determines how likely it is that a same-series upgrade will be inserted before each eligible version in the upgrade path. A value of 0 disables same-series upgrades entirely.
