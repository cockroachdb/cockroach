...
* eventually, every node in the cluster is upgraded all the way to most recent binary (i.e., the one passed to roachtest with the `--cockroach` flag). That upgrade is then finalized.
...
The code above is safe as it will connect to the system tenant in all deployment modes.
