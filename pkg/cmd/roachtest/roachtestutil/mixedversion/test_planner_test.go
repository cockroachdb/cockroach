...
│   │   ├── run "mixed-version 1", with connection to node 1, after 579ms delay (12)
│   │   └── run "mixed-version 2", with connection to node 1, after 794ms delay (13)
...
`, previousVersion,
	...
	// Assert that AfterTest hooks are scheduled to run in the last step
	// of the test.
	mvt = newTest(t)
	mvt.AfterTest("finalizer 1", dummyHook)
	mvt.AfterTest("finalizer 2", dummyHook)
	mvt.AfterTest("finalizer 3", dummyHook)
	plan, err = mvt.plan()
	require.NoError(t, err)
	require.Len(t, plan.steps, 9)
...
func newTest(t *testing.T) *Test {
	...
	return &Test{
		ctx:          ctx,
		logger:       nilLogger,
		opts:         &testOptions{crdbNodes: nodes},
