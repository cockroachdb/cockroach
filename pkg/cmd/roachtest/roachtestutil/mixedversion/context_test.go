...
func requireContextEqual(t *testing.T, expected, actual *Context) {
	require.Equal(t, expected.CockroachNodes, actual.CockroachNodes)
	require.Equal(t, expected.FromVersion, actual.FromVersion)
	require.Equal(t, expected.ToVersion, actual.ToVersion)
	require.Len(t, actual.nodesByVersion, len(expected.nodesByVersion))

	for v, expectedNodes := range expected.nodesByVersion {
		actualNodes, exists := actual.nodesByVersion[v]
		require.True(t, exists, "version = %v", v)

		require.True(t, actualNodes.Equals(*expectedNodes), "version = %v", v)
	}
}
