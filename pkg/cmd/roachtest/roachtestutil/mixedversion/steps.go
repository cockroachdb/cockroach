...
func (s pinVoterReplicasStep) Run(
	...
	// Disable load and range based rebalancing to prevent the allocator from fighting against
	// the zone config constraints we're about to set. It should still work with this
	// on, but it takes significantly longer for the ranges to stabilize.
	// TODO(darryl): the range based balancing appears to be especially problematic, investigate why.
	if err := h.System.Exec(rng, "SET CLUSTER SETTING kv.allocator.load_based_rebalancing = 'off'"); err != nil {
		return errors.Wrap(err, "failed to disable load-based rebalancing")
	}
	if err := h.System.Exec(rng, "SET CLUSTER SETTING kv.allocator.range_rebalance_threshold = 1.0"); err != nil {
		return errors.Wrap(err, "failed to set range rebalance threshold")
	}
