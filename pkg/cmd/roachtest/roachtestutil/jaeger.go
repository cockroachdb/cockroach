...
func InstallLaunchAndConfigureJaegerAllInOne(
	...
	if err := c.RunE(ctx, option.OnNodes(c.Node(1)), `[ -f jaeger-all-in-one ] ||
...
tar --strip-components=1 -xvzf -`); err != nil {
	...
	m.Go(func(ctx context.Context) error {
		err := c.RunE(ctx, option.OnNodes(c.Node(1)), `SPAN_STORAGE_TYPE=badger BADGER_EPHEMERAL=false \
...
./jaeger-all-in-one --collector.zipkin.host-port=:9411`)
	...
	c.Run(ctx, option.OnNodes(c.Node(1)), "ln -s /mnt/data1/jaeger.badger logs/jaeger.badger")
