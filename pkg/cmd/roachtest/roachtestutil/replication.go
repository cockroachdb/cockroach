...
func WaitForReplication(
	...
	return roachprod.WaitForReplication(ctx, db, replicationFactor, waitForReplicationType, roachprod.RetryEveryDuration(time.Second), roachprod.WithVerboseLogger(l))
}
...
func WaitForUpdatedReplicationReport(ctx context.Context, t test.Test, db *gosql.DB) {
	if err := roachprod.WaitForUpdatedReplicationReport(ctx, db, roachprod.WithVerboseLogger(t.L()), roachprod.WithMaxRetries(0)); err != nil {
		t.Fatal(err)
	}
}
