...
// GetDiskUsageInBytes executes the command `du {store-dir}` on node `nodeIdx`.
// Optional args are appended, so it can parse the result to bytes. nodeIdx starts at one.
