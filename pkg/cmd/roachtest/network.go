...
func runNetworkSanity(ctx context.Context, t *test, origC *cluster, nodes int) {
	...
	m.Go(func(ctx context.Context) error {
		...
		for i := 0; i < 10; i++ {
			duration := c.Measure(ctx, 1, fmt.Sprintf("BEGIN; INSERT INTO test.commit VALUES (2, %[1]d), (1, %[1]d), (3, %[1]d); COMMIT", i))
			c.l.Printf("%s\n", duration)
		}

		c.Measure(ctx, 1, "set tracing=on; insert into test.commit values(3,1000), (1,1000), (2,1000); select age, message from [ show trace for session ]")
