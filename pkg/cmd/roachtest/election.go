	...
	r.Add(testSpec{
		...
		Run: func(ctx context.Context, t *test, c *cluster) {
			...
			c.Run(ctx, c.Node(1), `./cockroach sql --insecure -e "
				CREATE TABLE timing(ts TIMESTAMP);
				INSERT INTO timing VALUES(now());
...
SET TRACING = on;
UPDATE timing SET ts = now() WHERE true;
UPDATE timing SET ts = now() WHERE true;
SELECT count(1) FROM test.kv;
SET TRACING = off;
...
"`)
				...
				require.NoError(t, c.Conn(ctx, 1).QueryRow(
					`SELECT 1000*1000*extract(milliseconds FROM (now()-ts))::int FROM timing`,
				).Scan(&duration))
				t.l.Printf("post-restart, query took %s\n", duration)
				if expected := 10 * time.Second; duration > expected {
