...
func createIndexAddJob(ctx context.Context, c *cluster, prefix string) (int64, chan error, error) {
	conn := c.Conn(ctx, 1)
	defer conn.Close()
	...
	jobID, err := jobutils.QueryJobID(conn, 0)
	if err != nil {
		return 0, nil, err
	}
		...
		Run: func(ctx context.Context, t *test, c *cluster) {
			runTPCC(ctx, t, c, tpccOptions{
				...
				During: func(ctx context.Context) error {
					...
					rollbackID, err := jobutils.QueryJobID(conn, 0)
					if err != nil {
						return err
