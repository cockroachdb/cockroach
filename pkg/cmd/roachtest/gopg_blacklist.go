...
var gopgIgnoreList19_2 = blacklist{
	// TODO(yuzefovich): remove the error matching and an ORDER BY ignores once
	// the new version of gopg is released.
	"pg | DB Prepare | returns an error when query can't be prepared": "error matching is too specific",
	"pg | ORM slice model | fetches Book relations":                   "expects ordering without ORDER BY",
	"pg | ORM slice model | fetches Genre relations":                  "expects ordering without ORDER BY",
	"pg | ORM slice model | fetches Translation relation":             "expects ordering without ORDER BY",
	"pg | ORM struct model | fetches Author relations":                "expects ordering without ORDER BY",
	"pg | ORM struct model | fetches Book relations":                  "expects ordering without ORDER BY",
	"pg | ORM struct model | fetches Genre relations":                 "expects ordering without ORDER BY",
	"pg | Tx | supports CopyFrom and CopyIn":                          "different Tx isolation level",
}
