...
func runOperations(register func(registry.Registry), filter, clusterName string) error {
	...
	cluster, ok := roachprod.CachedCluster(clusterName)
	if !ok {
		return errors.New("roachtest: run-operation: cluster does not exist")
	}
	...
	parallelism := int(math.Min(float64(roachtestflags.OperationParallelism), 10))
...
func (r *opsRunner) runWorker(ctx context.Context, workerIdx int, runForever bool) {
	...
	for {
		...
		if opSpec == nil {
			sleepDuration := time.Duration(15+rng.Intn(15)) * time.Second
