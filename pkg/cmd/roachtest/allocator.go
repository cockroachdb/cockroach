...
func init() {
	tests.Add(`upreplicate/1to3small`, func(t *test) {
		const start, end = 1, 3
		const fixture = `1node-10gb-262ranges`
		...
		for node := 1; node <= start; node++ {
			...
			go func() {
				...
				path := fmt.Sprintf(`gs://cockroach-fixtures/store-dumps/`+
					fixture+`/crdb=1.1,stores=%d/%d.tgz`, start, node-1)
