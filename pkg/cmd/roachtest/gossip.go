...
func registerGossip(r *registry) {
	runGossipChaos := func(ctx context.Context, t *test, c *cluster) {
		...
		c.StartOrFatal(ctx, t, c.All(), startArgs("--sequential"))
...
`
		...
		for j := 0; j < 100; j++ {
			...
			waitForGossip()
			c.StartOrFatal(ctx, t, c.Node(deadNode))
		}
	}
...
func runGossipRestartNodeOne(ctx context.Context, t *test, c *cluster) {
	...
	c.StartOrFatal(ctx, t, racks(c.nodes), startArgs("--env=COCKROACH_SCAN_MAX_IDLE_TIME=5ms"))
...
`
	...
	c.StartOrFatal(ctx, t, c.Range(2, c.nodes))
...
func runCheckLocalityIPAddress(ctx context.Context, t *test, c *cluster) {
	...
	for i := 1; i <= c.nodes; i++ {
		...
		c.StartOrFatal(ctx, t, c.Node(i), startArgs("--racks=1",
