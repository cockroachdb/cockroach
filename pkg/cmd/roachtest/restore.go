...
func (hc *HealthChecker) Runner(ctx context.Context) (err error) {
	...
	for {
		...
		logger.printf("running health check\n")
		for _, nodeIdx := range hc.nodes {
			...
			rows, err := db.Query(`SELECT * FROM crdb_internal.gossip_alerts WHERE description != 'queue.replicate.process.failure' AND description != 'ranges.underreplicated' AND description != 'queue.raftsnapshot.process.failure' ORDER BY node_id ASC, store_id ASC`)
			_ = db.Close()
			if err != nil {
				return err
			}
...
func (dul *DiskUsageLogger) Runner(ctx context.Context) error {
	...
	quietLogger, err := newLogger("diskusage-exec", "diskusage-exec", "diskusage", ioutil.Discard, ioutil.Discard)
	if err != nil {
		return err
	}
