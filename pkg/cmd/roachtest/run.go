...
func maybeEmitDatadogEvent(
	client *statsd.Client, opSpec *registry.OperationSpec, eventType ddEventType, operationID uint64,
) {
	if client == nil {
		return
	}
	...
	ev := statsd.NewEvent(fmt.Sprintf("op %s %s", opSpec.Name, status), "")
...
func runOperation(register func(registry.Registry), filter string, clusterName string) error {
	...
	maybeEmitDatadogEvent(statsdClient, opSpec, eventOpStarted, operationRunID)
	...
	if op.Failed() {
		...
		maybeEmitDatadogEvent(statsdClient, opSpec, eventOpError, operationRunID)
		return op.mu.failures[0]
	}

	maybeEmitDatadogEvent(statsdClient, opSpec, eventOpRan, operationRunID)
	if cleanup == nil {
	...
	maybeEmitDatadogEvent(statsdClient, opSpec, eventOpFinishedCleanup, operationRunID)
