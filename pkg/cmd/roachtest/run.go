...
func runTests(register func(registry.Registry), filter *registry.TestFilter) error {
	...
	fallbackLog := fallbackLogger(context.Background(), filepath.Join(artifactsDir, "roachtest.crdb.fallback.log"))
	logger.InitCRDBLogConfig(fallbackLog)
...
func fallbackLogger(ctx context.Context, path string) *logger.Logger {
	verboseCfg := logger.Config{}
	var err error
