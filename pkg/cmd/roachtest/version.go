...
func registerVersion(r *registry) {
	runVersion := func(ctx context.Context, t *test, c *cluster, version string) {
		...
		c.StartOrFatal(ctx, t, c.Range(1, nodes), startArgsDontEncrypt)
		...
		m.Go(func(ctx context.Context) error {
			...
			for i := 1; i < nodes; i++ {
				...
				c.StartOrFatal(ctx, t, c.Node(i), startArgsDontEncrypt)
				if err := sleepAndCheck(); err != nil {
					return err
				}
			}
			...
			c.StartOrFatal(ctx, t, c.Node(nodes), startArgsDontEncrypt)
			if err := sleepAndCheck(); err != nil {
				return err
			}
