...
func init() {
	runVersion := func(t *test, version string, nodes int) {
		...
		m.Go(func() error {
			...
			stop := func(node int) {
				port := 26257 + portMul*(node-1)
				if err := c.RunE(ctx, node, "./cockroach quit --insecure --port "+strconv.Itoa(port)); err != nil {
					panic(err)
				}
				// NB: we still call Stop to make sure the process is dead when we try to restart it (or we'll catch
				// an error from the RocksDB dir being locked). This won't happen unless run with --local due to
				// timing. However, it serves as a reminder that `./cockroach quit` doesn't yet work well enough --
				// ideally all listeners and engines are closed by the time it returns to the client.
	...
	// TODO: write TODO about gossip deadlock
	const nodes = 3
