...
func (hc *HistogramConverter) Convert(labels []model.Label, src model.FileInfo) error {
	...
	if postProcessFn == nil {
		return errors.Newf("post-process function not found for test: %s", testName)
	}

	// Initialize buffers and exporters
...
func (hc *HistogramConverter) writeAggregatedMetrics(
	...
	aggregatedMetrics, finalLabels, err := roachtestutil.PostProcessMetrics(testName, rawBuffer, postProcessFn)
	if err != nil {
