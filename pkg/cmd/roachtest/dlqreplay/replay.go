...
// Package dlqreplay implements a Cloud Function that replays failed GitHub
// issue posts from a GCS dead letter queue. It reads DLQ entries written by
// roachtest, reconstructs the issues.PostRequest and issues.Options, and
// calls issues.Post to create the GitHub issue.
//
// Deployment: deploy as a Gen2 Cloud Function with HTTP trigger.
// Environment variables:
//   - DLQ_BUCKET: GCS bucket name containing the DLQ entries
//   - GITHUB_API_TOKEN: GitHub API token for posting issues
package dlqreplay
...
func ReplayDLQ(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	bucketName := os.Getenv("DLQ_BUCKET")
	if bucketName == "" {
		http.Error(w, "DLQ_BUCKET env var not set", http.StatusInternalServerError)
		return
	}
	...
	for {
		...
		result := replayEntry(ctx, bucket, attrs.Name, token)
		switch result {
		case replaySuccess:
			processed++
		case replayFailed:
...
func replayEntry(
	ctx context.Context, bucket *storage.BucketHandle, objectName string, token string,
) replayResult {
...
func reconstructHelpCommand(entry *dlq.DLQEntry) func(*issues.Renderer) {
