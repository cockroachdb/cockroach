...
func TestFailoverOversizedArtifactsZip(t *testing.T) {
	...
	pointer, err := os.ReadFile(filepath.Join(artifactsDir, "artifacts-failover.txt"))
	require.NoError(t, err)
	require.Contains(t, string(pointer), result.gcsURI)
	require.Contains(t, string(pointer), "Size: 10 bytes")
	require.NotContains(t, string(pointer), "TeamCity limit:")
}
