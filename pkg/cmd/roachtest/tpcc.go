...
func loadTPCCBench(
	ctx context.Context, c *cluster, b tpccBenchSpec, roachNodes, loadNode nodeListOption, zones []string,
) error {
	c.l.printf("here")
	db := c.Conn(ctx, 1)
	defer db.Close()
	...
	cmd = fmt.Sprintf(
		"./workload run tpcc --warehouses=%d --split --scatter "+
			"--duration=1ms %s {pgurl:1}", b.LoadWarehouses, zonesArg)
...
func runTPCCBench(ctx context.Context, t *test, c *cluster, b tpccBenchSpec) {
	...
	loadNodes := c.Range(c.nodes, c.nodes)
	zoneCount := 1
	nodesPerRegion := roachNodeCount
	...
	if useHAProxy {
		...
		c.Install(ctx, c.Node(loadNodes[0]), "haproxy")
		c.Run(ctx, c.Node(loadNodes[0]), fmt.Sprintf("./cockroach gen haproxy --insecure --host %s",
			c.InternalIP(ctx, c.Node(1))[0]))
		c.Run(ctx, c.Node(loadNodes[0]), "haproxy -f haproxy.cfg -D")
	}
				...
				eg.Go(func() error {
					...
					if b.Geo {
						partitionFlag = fmt.Sprintf("--split --partitions=%d --partition-affinity=%d --active-warehouses=%d",
							zoneCount, count, warehouses/zoneCount)
					...
					cmd := fmt.Sprintf("ulimit -n 32768; "+
						"./workload run tpcc --warehouses=%d --ramp=30s --duration=%s%s %s {pgurl:%d-%d}",
						totalWarehouses, loadDur, extraFlags, partitionFlag, (count*nodesPerRegion)+1, (count*nodesPerRegion+1)+2)
					out, err := c.RunWithBuffer(ctx, c.l, c.Node(loadNodes[count]), cmd)
					...
					tpmCs[count], err = strconv.ParseFloat(fields[1], 64)
					if err != nil {
						return err
					}
					return nil
