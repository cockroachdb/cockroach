...
func runTPCC(ctx context.Context, t *test, c *cluster, warehouses int) {
	...
	m.Go(func(ctx context.Context) error {
		duration := "--duration=" + ifLocal("10s", "120m")
		cmd := fmt.Sprintf(
			"./workload run tpcc --warehouses=%d --histograms=logs/stats.json %s {pgurl%s}",
