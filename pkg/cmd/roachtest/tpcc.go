...
type tpccBenchSpec struct {
	...
	// If set, the spec will be run as a roachtest instead of a benchmark. This
	// allows the tool to be run in CI without running all configurations.
	RegisterAsTest bool
}
	...
	m.Go(func(ctx context.Context) error {
		...
		res, err := s.Search(func(warehouses int) (bool, error) {
			...
			if b.Chaos {
				...
				ch := Chaos{
					Timer:   Periodic{Down: 15 * time.Second, Up: 45 * time.Second},
