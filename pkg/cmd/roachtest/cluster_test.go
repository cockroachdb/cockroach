...
func TestRandomArchProbabilities(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:             "FIPS requested but zero FIPS probability",
		...
		{
			name:             "AMD64 and FIPS only",
			validArchs:       spec.Archs(vm.ArchAMD64, vm.ArchFIPS),
			arm64Probability: 0.3,
			fipsProbability:  0.4,
			expectedDistribution: map[vm.CPUArch]float64{
				vm.ArchAMD64: 0.6,
		...
		{
			name:             "ARM64 and FIPS only",
			validArchs:       spec.Archs(vm.ArchARM64, vm.ArchFIPS),
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			for expectedArch, expectedProb := range test.expectedDistribution {
				...
				// Calculate a 95% confidence interval and assert our actual probability falls within it.
				z := 1.96 // 95% confidence level
				margin := z * math.Sqrt(expectedProb*(1-expectedProb)/float64(numSamples))
				lowerBound := expectedProb - margin
				upperBound := expectedProb + margin

				require.GreaterOrEqual(t, actualProb, lowerBound)
				require.LessOrEqual(t, actualProb, upperBound)
			}
		})
	}
}
