...
func TestLoadGroups(t *testing.T) {
	...
	t.Run("panics with too many load nodes", func(t *testing.T) {
		defer func() {
			if r := recover(); r == nil {
				t.Fatalf("Failed to panic when number of load nodes exceeded number of zones")
			}
		}()
	...
	t.Run("panics with unequal zones per load node", func(t *testing.T) {
		defer func() {
			if r := recover(); r == nil {
				t.Fatalf("Failed to panic when number of zones is not divisible by number of load nodes")
			}
		}()
