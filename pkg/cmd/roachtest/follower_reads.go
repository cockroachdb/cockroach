...
func registerFollowerReads(r *testRegistry) {
	register := func(survival survivalGoal, locality localitySetting) {
		r.Add(testSpec{
			...
			Run: func(ctx context.Context, t *test, c *cluster) {
				...
				c.Wipe(ctx)
				c.Start(ctx, t)
				topology := topologySpec{locality: locality, survival: survival}
	...
	r.Add(testSpec{
		Name:  fmt.Sprintf("follower-reads/mixed-version/single-region"),
		Owner: OwnerKV,
		Cluster: makeClusterSpec(
			3, /* nodeCount */
			cpu(2),
...
const (
	...
	singleRegion localitySetting = "single-region"
)
...
func runFollowerReadsTest(
	...
	if topology.locality == singleRegion {
		// We expect one node - the leaseholder - to have a low ratio because it
		// doesn't perform follower reads.
		expectedLowRatioNodes = 1
	} else {
		// The leaseholder and one of the nodes in the non-primary region that has
		// two replicas.
...
func verifyHighFollowerReadRatios(
	...
	// with low follower read ratios. The first 20 seconds are ignored, as a
	// "ramp-up" period.
...
const selectsMetric = "sql_select_count"
...
func runFollowerReadsMixedVersionSingleRegionTest(
	...
	topology := topologySpec{locality: singleRegion}
	data := initFollowerReadsDB(ctx, t, c, topology)
	...
	runFollowerReadsTest(ctx, t, c, topologySpec{locality: singleRegion}, data)
