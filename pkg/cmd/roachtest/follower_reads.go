...
func runFollowerReadsTest(
	...
	// Wait until all regions have either a voting or non-voting replica for the
	// table.
	// TODO(nvanbenschoten): once we split out voting and non-voting replicas in
	// crdb_internal.ranges_no_leases, we can assert more here.
	t.l.Printf("waiting for up-replication...")
	tStart := timeutil.Now()
	for ok := false; !ok; time.Sleep(time.Second) {
		const q = `
			SELECT
				count(DISTINCT substring(unnest(replica_localities), 'region=([^,]*)')) = 3
			FROM
				crdb_internal.ranges_no_leases
			WHERE
				table_name = 'test'`
		require.NoError(t, db.QueryRowContext(ctx, q).Scan(&ok))
	...
	verifySelect := func(ctx context.Context, node, k int, expectedVal int64) func() error {
		return func() error {
			nodeDB := conns[node-1]
			aost := map[localitySetting]string{
				// For REGIONAL tables, only stale reads can be served off
				// followers.
				regional: "AS OF SYSTEM TIME follower_read_timestamp()",
