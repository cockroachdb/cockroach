...
exported by a cockroach cluster. This is instrumented via connecting a
prometheus process on a node in a roachtest, which scrapes the _status/vars
...
manages the prometheus process lifecycle during a run and provides a simple
interface to collect stats and return them in a parseable format, without pkg
dependencies.
...
that are contained in the interval given (map[string]map[string][]StatPoint).
...
iteration, the currently registered queries are made against the StatCollector.
The results are combined and given to ProcessStatEventFn. The loop continues
indefinitely until either the context is cancelled or ProcessStatEventFn
returns true, indicating that it is done processing this stream.
...
Alternatively, a user defined function may be used, which takes in the
resulting per-label time series and returns a single aggregated timeseries
along with a label to identify the time series.

e.g. For the same above:

AggQuery.AggFn = func(query string, vals map[string][]float64) (string, []float64) {
...
export is written to the perfartifacts directory.
*/
