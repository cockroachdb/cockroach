...
func (k kafkaManager) configureAuth(ctx context.Context) *testCerts {
	...
	err = k.c.PutString(ctx, testCerts.KafkaKey, kafkaKeyPath, 0600)
	if err != nil {
		k.c.t.Fatal(err)
	}

	err = k.c.PutString(ctx, testCerts.KafkaCert, kafkaCertPath, 0600)
	if err != nil {
		k.c.t.Fatal(err)
	}
	...
	err = k.c.PutString(ctx, testCerts.CAKey, caKeyPath, 0600)
	if err != nil {
		k.c.t.Fatal(err)
	}

	err = k.c.PutString(ctx, testCerts.CACert, caCertPath, 0600)
	if err != nil {
		k.c.t.Fatal(err)
	}
	...
	err = k.c.PutString(ctx, kafkaConfig, filepath.Join(configDir, "server.properties"), 0600)
	if err != nil {
		k.c.t.Fatal(err)
	}

	err = k.c.PutString(ctx, kafkaJAASConfig, filepath.Join(configDir, "server_jaas.conf"), 0600)
	if err != nil {
		k.c.t.Fatal(err)
	}
