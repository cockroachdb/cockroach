...
const (
	tpccWorkloadType workloadType = iota
	ledgerWorkloadType
)
...
func cdcBasicTest(ctx context.Context, t *test, c *cluster, args cdcTestArgs) {
	...
	// Apportion nodes, distribute executables, and start cockroach.
	...
	// Additional arcane test setup, just to make things run more smoothly.
	db := c.Conn(ctx, 1)
	...
	// Install and run the selected workload.
	...
	// Run changefeed and verifier workload.
...
func registerCDC(r *registry) {
	...
	r.Add(testSpec{
		Name:       "cdc/ledger/nodes=3/init=false",
		MinVersion: "2.1.0",
		Nodes:      nodes(4, cpu(16)),
		Stable:     false,
		Run: func(ctx context.Context, t *test, c *cluster) {
...
func (tw *tpccWorkload) run(ctx context.Context, c *cluster, workloadDuration string) {
	c.Run(ctx, tw.workloadNodes, fmt.Sprintf(
		`./workload run tpcc --warehouses=%d {pgurl%s} --workloadDuration=%s`,
...
func (lw *ledgerWorkload) run(ctx context.Context, c *cluster, workloadDuration string) {
	c.Run(ctx, lw.workloadNodes, fmt.Sprintf(
		`./workload run ledger --mix=balance=50,withdrawal=25,deposit=12,reversal=0 {pgurl%s} --workloadDuration=%s`,
		lw.sqlNodes,
		workloadDuration,
	))
}
...
func getChangefeedInfo(db *gosql.DB, jobID int) (changefeedInfo, error) {
	...
	return changefeedInfo{
		...
		highwaterTime: progress.GetHighWater().GoTime(),
	}, nil
}
