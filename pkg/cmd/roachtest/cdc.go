...
func runCDCErrors(ctx context.Context, t *test, c *cluster) {
	crdbNode, kafkaNode := c.Node(1), c.Node(c.nodes)
	c.Put(ctx, cockroach, "./cockroach", crdbNode)
	c.Start(ctx, t, crdbNode)
	kafka := kafkaManager{
		c:     c,
		nodes: kafkaNode,
	}
	...
	if _, err := db.Exec(`CREATE TABLE foo (a INT PRIMARY KEY, b STRING)`); err != nil {
		t.Fatal(err)
	}
	...
	into := kafka.sinkURL(ctx) + `?kafka_topic_prefix=foo`
	if _, err := db.Exec(
		`CREATE CHANGEFEED FOR foo INTO $1`, into,
	); !testutils.IsError(err, `unknown sink query parameter: kafka_topic_prefix`) {
		t.Fatalf(`expected "unknown sink query parameter: kafka_topic_prefix" error got: %v`, err)
	}
	into = kafka.sinkURL(ctx) + `?schema_topic=foo`
	if _, err := db.Exec(
		`CREATE CHANGEFEED FOR foo INTO $1`, into,
	); !testutils.IsError(err, `schema_topic is not yet supported`) {
		t.Fatalf(`expected "schema_topic is not yet supported" error got: %v`, err)
	}
}
