...
func registerCDC(r *registry) {
	runCDC := func(ctx context.Context, t *test, c *cluster, warehouses int, initialScan bool) {
		...
		// Clean up kafka if it was running to make development iteration
		// easier.
		c.Run(ctx, kafkaNode, `sudo killall -9 java | true`)
		c.Run(ctx, kafkaNode, `rm -rf /mnt/data1/confluent`)
