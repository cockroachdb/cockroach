...
func (g *githubIssues) createPostRequest(
	...
	} else if !spec.NonReleaseBlocker && !metamorphicBuild && !coverageBuild {
	...
	if coverageBuild {
		topLevelNotes = append(topLevelNotes,
			"This is a special code-coverage build. If the same failure was hit in a non-coverage run, "+
				"there should be a similar issue without the B-coverage label. If not, it is possible that "+
	...
	if metamorphicBuild {
		topLevelNotes = append(topLevelNotes,
			"This build has metamorphic test constants enabled. If the same failure was hit in a "+
				"non-metamorphic run, there should be a similar issue without the B-metamorphic label.If not, it is possible "+
				"that this failure is caused by a metamorphic constant")
	}
