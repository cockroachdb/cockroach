...
func registerEncryption(r *registry) {
	...
	runEncryption := func(ctx context.Context, t *test, c *cluster) {
		...
		for _, addr := range c.InternalAdminUIAddr(ctx, c.Range(1, nodes)) {
			if err := c.RunE(ctx, c.Node(nodes), fmt.Sprintf("curl http://%s/_status/stores/local | grep encryptionStatus", addr)); err != nil {
				t.Fatalf("failed to retrieve encryption status from /_status/stores/local endpoint")
			}
		}
