...
func (r *testRunner) runWorker(
	...
	defer func() {
		//TODO: Consider removing the test_run_id label here.
		// Currently it is only removed when a cluster is unregistered, via c.Destroy()
...
func (r *testRunner) runTest(
	...
	if grafanaAvailable {
		// This links to an auto-refreshing dashboard for this test. Since it uses a `now-15m` time to `now` range
		// it will only show data while the test is running.
		shout(ctx, l, stdout, "grafana metrics can be followed at: https://go.crdb.dev/roachtest-grafana-live/%s/%s note: this will only show data while the test is running",
			vm.SanitizeLabel(runID), vm.SanitizeLabel(testRunID))
	}
	select {
	case <-testReturnedCh:
		s := "successfully"
