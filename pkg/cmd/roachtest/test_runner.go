...
func (r *testRunner) allocateCluster(
	...
	if existingClusterName != "" {
		...
		if t.Operation {
			// Operations don't require the cluster to be registered in the registry,
			// so errors here are acceptable. Also allows for better cluster reuse
			// as operations don't destroy() clusters.
			opt.tolerateRegistrationErrors = true
		}
		...
		if err == nil {
			if t.Operation {
				c.localCertsDir = roachtestflags.CertsDir
			}
