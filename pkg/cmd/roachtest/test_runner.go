...
func (r *testRunner) runWorker(
	...
	// cluster destroy can be done concurrently.
	// The WaitGroup just ensures that all pending Destroy calls have completed.
	...
	for {
		...
		if testToRun.noWork {
			if c != nil {
				...
				r.destroyCluster(clusterDestroyWg, c, l) // do not wait as we can free the worker
				wStatus.SetCluster(nil)
				c = nil
			}
