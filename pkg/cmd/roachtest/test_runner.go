...
// inspectBlacklistRegex is a compiled regex of test name patterns that should
// skip INSPECT validation. Tests matching any of these patterns will not run
// INSPECT.
var inspectBlacklistRegex = regexp.MustCompile(
	`^acceptance|` +
		`^activerecord|` +
		`^admission|` +
		`^allocbench|` +
...
// isInspectBlacklisted returns true if the test name matches the blacklist
// and should skip INSPECT validation. Tests NOT matching any pattern will
// run INSPECT (currently: alterpk*, schemachange*, inspect*).
func isInspectBlacklisted(testName string) bool {
	return inspectBlacklistRegex.MatchString(testName)
}
...
func (r *testRunner) postTestAssertions(
	...
	_ = r.stopper.RunAsyncTask(ctx, "test-post-assertions", func(ctx context.Context) {
		...
		if t.spec.SkipPostValidations&registry.PostValidationInspect == 0 {
			func() {
				// Check if test is blacklisted for INSPECT
				if isInspectBlacklisted(t.Name()) {
					t.L().Printf("Skipping INSPECT validation (test is blacklisted)")
					return
				}
