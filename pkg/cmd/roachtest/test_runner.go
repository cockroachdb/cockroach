...
func zipArtifacts(t *testImpl) error {
	...
	list, err := filterDirEntries(t.ArtifactsDir(), func(entry os.DirEntry) bool {
		fmt.Printf("filterDirEntries: entry.Name = %s\n", entry.Name())
		...
		// a `.perf` directory. Since this function is invoked _before_ 'getPerfArtifacts', scp'd 'stats.json' is never moved to the zip archive.
