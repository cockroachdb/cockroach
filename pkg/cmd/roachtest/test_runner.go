...
func (r *testRunner) allocateOrAttachToCluster(
	...
	if existingClusterName != "" {
		...
		shout(ctx, lopt.runnerL, lopt.stdout, "Attaching to existing cluster %s for test %s (logs: %s)",
			...
			filepath.Join(runnerLogsDir, "cluster-create", existingClusterName+".log"))
	...
	for {
		...
		if c == nil {
			...
			if clusterCreateErr != nil {
				...
				if c != nil {
					shout(ctx, workerL, stdout, "Cluster logs: %s", filepath.Join(runnerLogsDir, "cluster-create", c.Name()+".log"))
				}
			} else {
				if c.arch != arch {
				...
				shout(ctx, workerL, stdout, "Cluster %s created for test %s (arch=%q, logs: %s)",
					...
					filepath.Join(runnerLogsDir, "cluster-create", c.Name()+".log"))
			}
		}
