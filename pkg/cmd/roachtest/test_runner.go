	...
	for {
		...
		if clusterCreateErr != nil {
			...
			if setupErr != nil {
				...
				useufferedSender := roachtestutil.UsingRuntimeAssertions(t)
				if useufferedSender {
					c.clusterSettings["kv.rangefeed.buffered_sender.enabled"] = "true"
				}
				c.status(fmt.Sprintf("metamorphically using buffered sender: %t", useufferedSender))
				t.AddParam("metamorphicBufferedSender", fmt.Sprint(useufferedSender))
