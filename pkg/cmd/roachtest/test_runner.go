	...
	for {
		...
		if c != nil && testToRun.canReuseCluster {
			...
			if c.spec.Cloud != spec.Local && c.spec.Arch != "" && c.arch != c.spec.Arch {
				panic(fmt.Sprintf("cluster arch %q does not match specified arch %q on cloud: %q", c.arch, c.spec.Arch, c.spec.Cloud))
			}
		}
		...
		if c != nil && c.spec.Cloud != spec.Local && testToRun.canReuseCluster {
		...
		if c != nil && c.arch != arch {
			if c.spec.Cloud != spec.Local {
				panic("infeasible path: non-local cluster arch mismatch")
			}
