...
const (
	...
	// Zfs file system.
	Ext4  fileSystemType = 0
	Zfs   fileSystemType = 1
	Xfs   fileSystemType = 2
	F2fs  fileSystemType = 3
	Btrfs fileSystemType = 4
...
func (s *ClusterSpec) RoachprodOpts(
	...
	case Ext4:
		...
		if s.RandomlyUseZfs {
			...
			if rng.Float64() <= 0.2 {
				createVMOpts.SSDOpts.FileSystem = string(vm.Zfs)
			}
		}
	case Zfs:
		createVMOpts.SSDOpts.FileSystem = string(vm.Zfs)
	case Xfs:
		createVMOpts.SSDOpts.FileSystem = string(vm.Xfs)
	case F2fs:
		createVMOpts.SSDOpts.FileSystem = string(vm.F2fs)
	case Btrfs:
		createVMOpts.SSDOpts.FileSystem = string(vm.Btrfs)
	default:
