...
func registerPsycopg(r *registry) {
	runPsycopg := func(
		...
		blacklistName, blacklist, ignoredlistName, ignoredlist := getPsycopgBlacklistForVersion(version)
		if blacklist == nil {
		...
		for scanner.Scan() {
			...
			if match != nil {
				...
				issue, expectedFailure := blacklist[test]
				switch {
		...
		for test, issue := range blacklist {
		...
		fmt.Fprintf(&bResults, "%d %s ignored\n",
			ignoredCount, plural("test", "tests", ignoredCount),
		)
