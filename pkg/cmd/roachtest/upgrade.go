...
func runVersionUpgrade(ctx context.Context, t *test, c *cluster) {
	...
	uploadVersion := func(newVersion string) option {
		...
		return startArgs("--binary="+target, "--skip-settings")
	}
	...
	// the cluster. If the wipe option is specified the nodes are wiped before
	// upgrading.
	binaryVersionUpgrade := func(newVersion string, nodes nodeListOption, wipe bool) versionStep {
		return versionStep{
			run: func() {
				c.l.Printf("%s: binary\n", newVersion)
				...
				for i := 1; i < c.nodes; i++ {
					err := retry.ForDuration(30*time.Second, func() error {
						...
						if version != newVersion {
							return fmt.Errorf("%d: expected version %s, got %s", i, newVersion, version)
						}
						return nil
					})
					if err != nil {
	...
	steps := []versionStep{
		binaryVersionUpgrade(baseVersion, nodes, false /* wipe */),
		// v1.1.0 is the first binary version that knows about cluster versions,
		// but thinks it can only support up to 1.0-3.
		binaryVersionUpgrade("v1.1.0", nodes, false /* wipe */),
		clusterVersionUpgrade("1.0", true /* manual */),
		clusterVersionUpgrade("1.0-3", true /* manual */),

		binaryVersionUpgrade("v1.1.1", nodes, false /* wipe */),
		clusterVersionUpgrade("1.1", true /* manual */),

		// Wipe the last node to start it fresh.
		binaryVersionUpgrade("v2.0.0", c.Node(3), true /* wipe */),

		binaryVersionUpgrade("v2.0.0", nodes, false /* wipe */),
