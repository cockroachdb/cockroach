...
func runDecommission(ctx context.Context, t *test, c *cluster, nodes int, duration time.Duration) {
	...
	m.Go(func() error {
		...
		stop := func(node int) error {
			...
			return c.RunE(ctx, c.Node(node), fmt.Sprintf("./cockroach quit --host=:%s %s", port, cockroachSecureFlag()))
		}
...
func execCLI(
	...
	args := []string{"./cockroach", cockroachSecureFlag(), fmt.Sprintf("--port={pgport:%d}", runNode)}
