...
func registerKVBench(r *testRegistry) {
	specs := []kvBenchSpec{
		{
			// Sharded primary key without a secondary index.
			Nodes:                  5,
			CPUs:                   8,
			...
			RampDurationSecs:       60,
			LoadDurationSecs:       120,
			NumShards:              10,
			ShouldPreSplitTable:    true,
		},
		{
			// Unsharded primary key without a secondary index.
			Nodes:                  5,
			CPUs:                   8,
			KeyDistribution:        sequential,
			EstimatedMaxThroughput: 30000,
...
func runKVBench(ctx context.Context, t *test, c *cluster, b kvBenchSpec) {
	...
	searchPredicate := func(maxrate int) (bool, error) {
		...
		m.Go(func(ctx context.Context) error {
			...
			db := c.Conn(ctx, 1)
			cleanupCmd := strings.Builder{}
			cleanupCmd.WriteString(`CREATE DATABASE IF NOT EXISTS kv;` +
				`USE kv; DROP TABLE IF EXISTS kv;`)

			if _, err := db.Exec(cleanupCmd.String()); err != nil {
				return err
			}
			...
			if b.NumShards > 0 {
				fmt.Fprintf(&initCmd,
					`, shard INT4 as (k %% %d) STORED CHECK (shard in (`, b.NumShards)
				for i := 0; i < b.NumShards; i++ {
					if i != 0 {
						initCmd.WriteString(",")
					}
					fmt.Fprintf(&initCmd, "%d", i)
				}
				initCmd.WriteString("))")
				initCmd.WriteString(", PRIMARY KEY (shard, k)")
			} else {
				initCmd.WriteString(", PRIMARY KEY (k)")
			}
			if b.SecondaryIndex {
				initCmd.WriteString(", INDEX (v)")
			}
			initCmd.WriteString(");")

			if _, err := db.Exec(initCmd.String()); err != nil {
				t.l.Printf(initCmd.String())
				return err
			}
		...
		if failErr == nil {
			...
			t.l.Printf(`--- PASS: kv workload maintained a p50 latency of %0.1fms`+
				`with avg throughput of %d\n\n`, res.latency(), res.throughput())
		} else {
			ttycolor.Stdout(ttycolor.Red)
			t.l.Printf(`--- FAIL: kv workload maintained a latency of %0.1fms (threshold: %0.1fms)`+
