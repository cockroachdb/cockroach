...
func (cl *cleanupDiskFillBulkIngest) Cleanup(
	...
	if cl.bucket != "" {
		...
		if rmCmd != "" {
			...
			if err := c.RunE(ctx, option.WithNodes(c.Node(1)), rmCmd); err != nil {
...
func runDiskFillBulkIngest(
	...
	defer func() {
		if r := recover(); r != nil {
			o.Errorf("panic during disk-fill/bulk-ingest-recovery: %v", r)
		}
	}()
	...
	jobID := startDetachedRestore(ctx, o, c, dbName, bucket, restoreDBName)
	cl.jobID = jobID
	...
	if jobStatus == "succeeded" {
		o.Status(fmt.Sprintf("RESTORE job %d already succeeded under disk pressure", jobID))
	} else {
