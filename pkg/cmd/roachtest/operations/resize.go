...
func resizeCluster(
	...
	o.Status(fmt.Sprintf("waiting for %s before draining and decommissioning nodes", pauseDuration))
	select {
	case <-time.After(pauseDuration):
	case <-ctx.Done():
		o.Fatalf("context canceled while pausing for: %s", pauseDuration)
	}

	for i := 0; i < growCount; i++ {
		drainNode(ctx, o, c, c.Node(origClusterSize+i+1))
		decommissionNode(ctx, o, c, c.Node(origClusterSize+i+1))
	}
