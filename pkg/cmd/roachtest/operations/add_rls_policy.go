...
func (cl *cleanupRLSPolicy) Cleanup(ctx context.Context, o operation.Operation, c cluster.Cluster) {
	...
	// Create a new context that is not tied to the parent context's cancellation
	// This ensures the cleanup will run even if the parent operation is canceled
	...
	go func() {
		...
		select {
		case <-time.After(cl.waitDuration):
			o.Status(fmt.Sprintf("Wait time of %s elapsed, performing cleanup", cl.waitDuration))
		}

		conn := c.Conn(newCtx, o.L(), 1, option.VirtualClusterName(roachtestflags.VirtualCluster))
		defer conn.Close()

		// Switch to the database where the table is located
		o.Status(fmt.Sprintf("switching to database %s for cleanup", cl.db))
		if _, err := conn.ExecContext(newCtx, fmt.Sprintf("USE %s", cl.db)); err != nil {
			o.Fatal(err)
		}

		if cl.locked {
			helpers.SetSchemaLocked(newCtx, o, conn, cl.db, cl.table, false /* lock */)
			defer helpers.SetSchemaLocked(newCtx, o, conn, cl.db, cl.table, true /* lock */)
		}
		...
		for _, policy := range cl.policies {
			...
			_, err := conn.ExecContext(newCtx, fmt.Sprintf("DROP POLICY %s ON %s.%s", policy, cl.db, cl.table))
			if err != nil {
				o.Fatal(err)
			}
		}
		...
		if cl.originalRLSStmt != "" {
			...
			if _, err := conn.ExecContext(newCtx, cl.originalRLSStmt); err != nil {
				o.Fatal(err)
			}
		} else {
			// If the table didn't have RLS before, disable it
			o.Status(fmt.Sprintf("disabling row level security for %s.%s", cl.db, cl.table))
			_, err := conn.ExecContext(newCtx, fmt.Sprintf("ALTER TABLE %s.%s DISABLE ROW LEVEL SECURITY, NO FORCE ROW LEVEL SECURITY", cl.db, cl.table))
			if err != nil {
				o.Fatal(err)
			}
		}
