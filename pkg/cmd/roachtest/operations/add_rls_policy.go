...
func (cl *cleanupRLSPolicy) Cleanup(ctx context.Context, o operation.Operation, c cluster.Cluster) {
	...
	if cl.originalRLSStmt != "" {
		...
		_, err := conn.ExecContext(ctx, cl.originalRLSStmt)
		if err != nil {
			o.Fatal(err)
		}
	} else {
		// If table didn't have RLS before, disable it
...
func runAddRLSPolicy(
	...
	if strings.Contains(createStmt, "ROW LEVEL SECURITY") {
		...
		for _, line := range lines {
			if strings.Contains(line, "ROW LEVEL SECURITY") {
				// Format the original statement for reuse during cleanup
				originalRLSStmt = fmt.Sprintf("ALTER TABLE %s.%s %s", dbName, tableName,
					strings.TrimPrefix(strings.TrimSpace(line), "ALTER TABLE public."+tableName+" "))
				break
			}
		}
	}
	...
	waitTime := time.Hour
	o.Status(fmt.Sprintf("waiting for %s", waitTime))

	// Create the cleanup object before the wait, so we can return it even if context is canceled
	cleanup := &cleanupRLSPolicy{
		db:              dbName,
		table:           tableName,
		policies:        policies,
	...
	case <-ctx.Done():
		o.Status("context canceled, skipping wait but returning cleanup object")
		return cleanup
	}
