...
func (cl *cleanupRLSPolicy) Cleanup(ctx context.Context, o operation.Operation, c cluster.Cluster) {
	...
	// Get the current database name so we can reset it
	var currentDB string
	if err := conn.QueryRowContext(ctx, "SHOW DATABASE").Scan(&currentDB); err != nil {
		o.Fatal(err)
	}

	useThisDB := fmt.Sprintf("use %s;", cl.db)
	resetUseDB := fmt.Sprintf("use %s;", currentDB)

	// Restore the original RLS state
	o.Status(fmt.Sprintf("restoring original row level security state for %s.%s", cl.db, cl.table))
	if _, err := conn.ExecContext(ctx, useThisDB+cl.originalRLSStmt+resetUseDB); err != nil {
		o.Fatal(err)
	}
