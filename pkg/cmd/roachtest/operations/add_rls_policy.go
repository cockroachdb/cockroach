...
func (cl *cleanupRLSPolicy) Cleanup(ctx context.Context, o operation.Operation, c cluster.Cluster) {
	...
	go func() {
		...
		case <-ctx.Done():
			// The parent context was canceled before our wait completed.
			// Exit without cleanup since the operation has been abruptly terminated.
			o.Status("Parent context canceled before wait completed, skipping cleanup")
			return
		}
