...
// and populates the same in the jobdetails and returns
func updatePayloadForJobs(
...
func checkAndCreateChangefeed(
	...
	if len(allCFJobs) >= mcf {
		...
		return waitForJobToComplete(ctx, conn, "CREATE", createdJob,
...
func createChangefeed(
	...
	allCFJobs, err := updatePayloadForJobs(ctx, conn, allCFJobs)
	if err != nil {
		return err
	}
