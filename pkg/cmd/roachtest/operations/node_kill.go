...
func runNodeKill(
	...
	o.Status(fmt.Sprintf("sent signal %d to cockroach on node %s, now waiting for it to exit", signal, node.NodeIDsString()))
	for {
		...
		if err != nil {
			if strings.Contains(err.Error(), "status 1") {
				// pgrep exiting with exit status 1 means no process was matched.
				break
			}
			o.Fatal(err)
		}
