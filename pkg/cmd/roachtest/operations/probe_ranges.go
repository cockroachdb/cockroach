...
func runProbeRanges(
	...
	probeReadRangeStmt := `select range_id, error from crdb_internal.probe_ranges(INTERVAL '1s', 'read')`

	probeWriteRangeStmt := `select range_id, error from crdb_internal.probe_ranges(INTERVAL '1s', 'write')`
