...
func runProbeRanges(
	...
	if !range_errors.Next() {
		o.Fatalf(fmt.Sprintf("Failed to scan value from crdb_internal.probe-ranges %s statement against node %d", probeType, nid))
	}
	if scanErr := range_errors.Scan(&errors); scanErr != nil {
		...
		if errors > 0 {
			...
			o.Fatal(fmt.Sprintf("Found range errors when probing via crdb_internal.probe-ranges %s statement against node %d", probeType, nid))
		} else {
