...
func captureHeapProfile(
	...
	// The GET request triggers server-side profile generation; we
	// only need to drain the body so the connection can be reused.
	...
	o.Status(fmt.Sprintf(
		"heap profile %d: triggered (%d bytes)", profileNum, n,
	))
}
