...
func newFailure(errs ...error) failure {
...
func (t *testImpl) FailNow() {
	t.addFailure(newFailure(errors.NewWithDepthf(1, "FailNow called")))
	panic(errTestFatal)
}
...
func formatFailure(b *strings.Builder, reportFailures ...failure) {
	for i, failure := range reportFailures {
		...
		squashedErr := failure.errors[0]
		file, line, fn, ok := errors.GetOneLineSource(squashedErr)
		if !ok {
			file, line, fn = "<unknown>", 0, "unknown"
		}
		fmt.Fprintf(b, "(%s:%d).%s: %v", file, line, fn, squashedErr)
	}
}
...
func (t *testImpl) addFailure(reportFailure failure) {
	...
	// main log as they are highly verbose.
	{
		...
		if err == nil {
			...
			_ = os.WriteFile(path, []byte(fmt.Sprintf("%+v", reportFailure.errors[0])), 0644)
		}
	}
