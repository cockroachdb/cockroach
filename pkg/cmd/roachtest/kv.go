...
func registerKVGracefulDraining(r *testRegistry) {
	r.Add(testSpec{
		...
		Run: func(ctx context.Context, t *test, c *cluster) {
			...
			// samples to succeeds. This would be too close to call, so
			...
			m.Go(func(ctx context.Context) error {
				t.Status("starting to gracefully drain and restart nodes")
			...
			t.Status("checking for perf throughout the test")
