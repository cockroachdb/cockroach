...
func installLaunchAndConfigureJaegerAllInOne(ctx context.Context, c *cluster, m *monitor) error {
	if local {
...
./jaeger-all-in-one --collector.zipkin.host-port=:9411`)
	...
	hps, err := c.InternalIP(ctx, c.Node(1))
	if err != nil {
		return err
	}
	if _, err := c.Conn(ctx, 1).Exec(`SET CLUSTER SETTING trace.zipkin.collector = $1`, hps[0]+":9411"); err != nil {
		return err
	}
	time.Sleep(time.Second) // give jaeger a moment to start
	hps, err = c.ExternalIP(ctx, c.Node(1))
	if err != nil {
		return err
	}
	c.Run(ctx, c.Node(1), "ln -s /mnt/data1/jaeger.badger logs/jaeger.badger")
	c.l.Printf("jaeger UI should now be running at http://%s:16686", hps[0])
	return nil
}
...
func registerKVContention(r *testRegistry) {
	...
	r.Add(testSpec{
		...
		Run: func(ctx context.Context, t *test, c *cluster) {
			...
			t.Status("running workload")
			m := newMonitor(ctx, c, c.Range(1, nodes))
			if err := installLaunchAndConfigureJaegerAllInOne(ctx, c, m); err != nil {
				t.Fatal(err)
			}
			m.Go(func(ctx context.Context) error {
