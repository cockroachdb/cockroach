...
var (
	...
	admissionAvgWaitSecs = clusterstats.ClusterStat{
		LabelName: "node",
		Query:     divQuery("rate(admission_wait_sum_kv[1m])", int(time.Second.Microseconds())),
	}
	// TODO(kvoli): Swap the query with the io overload metric, once #87424 is
	// complete.
	admissionControlIOOverload = clusterstats.ClusterStat{
		LabelName: "node",
		// NV: We take the union of the two queries and return whichever is
		// greater, or both if equal (true).
		Query: "((storage_l0_sublevels / 20 > storage_l0_num_files / 1000) or (storage_l0_sublevels / 20 <= storage_l0_num_files / 1000)) * 100",
	}
...
func divQuery(query string, max int) string {
	return fmt.Sprintf("%s / %d", query, max)
}
