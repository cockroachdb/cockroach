...
func (tn *tenantNode) start(ctx context.Context, t test.Test, c cluster.Cluster, binary string) {
	...
	tn.errCh = startTenantServer(
		...
		tn.httpPort, tn.sqlPort, randomSeed, extraArgs...,
	)
...
func startTenantServer(
	...
	go func() {
		// Set the same random seed for every tenant; this picked up by
		// runs that use a build with runtime assertions enabled, and
		// ignored otherwise.
		env := append(
			[]string{fmt.Sprintf("COCKROACH_RANDOM_SEED=%d", randomSeed)},
			config.DefaultEnvVars()...,
		)
		errCh <- c.RunE(tenantCtx, node,
			append(append(env, binary, "mt", "start-sql"), args...)...,
		)
		close(errCh)
	}()
	return errCh
