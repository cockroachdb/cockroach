...
func (tn *tenantNode) start(ctx context.Context, t test.Test, c cluster.Cluster, binary string) {
	...
	cmd := fmt.Sprintf("%s cert create-tenant-client --certs-dir=certs --ca-key=certs/ca.key %d %s", binary, tn.tenantID, strings.Join(names, " "))
	c.Run(ctx, c.Node(tn.node), cmd)

	tn.binary = binary
	extraArgs := []string{"--log=\"file-defaults: {dir: '" + tn.logDir() + "', exit-on-error: false}\"", "--store=" + tn.storeDir()}
