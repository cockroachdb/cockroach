...
func runConnectionLatencyTest(
	...
	urlTemplate := func(user string) string {
		if password {
			return fmt.Sprintf("postgres://testuser:123@%s:{pgport:1}?sslmode=require&sslrootcert=%s/ca.crt", user, install.CockroachNodeCertsDir)
		}

		return fmt.Sprintf("postgres://testuser@%s:{pgport:1}?sslcert=%[2]s/client.testuser.crt&sslkey=%[2]s/client.testuser.key&sslrootcert=%[2]s/ca.crt&sslmode=require", user, install.CockroachNodeCertsDir)
	}
	...
	if password {
		err = c.RunE(ctx, option.WithNodes(c.Node(1)), fmt.Sprintf(`./cockroach sql --certs-dir %s --port={pgport:1} -e "CREATE USER testuser WITH PASSWORD '123' CREATEDB"`, install.CockroachNodeCertsDir))
		require.NoError(t, err)
		...
		err = c.RunE(ctx, option.WithNodes(c.Node(1)), fmt.Sprintf(`./cockroach sql --certs-dir %s --port={pgport:1} -e "CREATE USER testuser CREATEDB"`, install.CockroachNodeCertsDir))
		require.NoError(t, err)
		require.NoError(t, err)
