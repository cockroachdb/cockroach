...
func validateLastLoginTime(
	...
	for {
		...
		if lastLogin != nil {
			propagationTime := timeutil.Since(authTime)
			t.L().Printf(
				"estimated_last_login_time validated for user %s: %v (propagation: %v)",
				username, *lastLogin, propagationTime)
			return
		}

		if timeutil.Now().After(deadline) {
			t.Fatal("timed out waiting for estimated_last_login_time to be populated " +
				"for user " + username)
		}
