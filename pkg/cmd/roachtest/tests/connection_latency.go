...
func setupCRDBForLDAPAuth(ctx context.Context, t test.Test, c cluster.Cluster, hostName string) {
	...
	row1, err := conn.Query(ctx,
		fmt.Sprintf("SET cluster setting server.host_based_authentication.configuration='%s'", hba))
	require.NoError(t, err)
	...
	row2, err := conn.Query(ctx,
		fmt.Sprintf("SET cluster setting server.ldap_authentication.domain.custom_ca='%s'", trimmed))
	require.NoError(t, err)
	// The row must be closed for the connection to be used again
	row2.Close()

	// This is required for better debugging in-case of failures in authentication.
	row3, err := conn.Query(ctx, "SET CLUSTER SETTING server.auth_log.sql_sessions.enabled = true;")
	require.NoError(t, err)
	// The row must be closed for the connection to be used again
	row3.Close()
}
