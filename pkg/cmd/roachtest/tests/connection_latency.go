...
func runConnectionLatencyTest(
	...
	if password {
		err = c.RunE(ctx, option.OnNodes(c.Node(1)), `./cockroach sql --certs-dir certs --port={pgport:1} -e "CREATE USER testuser WITH PASSWORD '123' CREATEDB"`)
		require.NoError(t, err)
		err = c.RunE(ctx, option.OnNodes(c.Node(1)), fmt.Sprintf("./workload init connectionlatency --user testuser --password '123' --secure '%s'", urlTemplate("localhost")))
		require.NoError(t, err)
		passwordFlag = "--password 123 "
	} else {
		// NB: certs for `testuser` are created by `roachprod start --secure`.
		err = c.RunE(ctx, option.OnNodes(c.Node(1)), `./cockroach sql --certs-dir certs --port={pgport:1} -e "CREATE USER testuser CREATEDB"`)
		require.NoError(t, err)
		require.NoError(t, err)
		err = c.RunE(ctx, option.OnNodes(c.Node(1)), fmt.Sprintf("./workload init connectionlatency --user testuser --secure '%s'", urlTemplate("localhost")))
		require.NoError(t, err)
	}
	...
	runWorkload := func(roachNodes, loadNode option.NodeListOption, locality string) {
		...
		err = c.RunE(ctx, option.OnNodes(loadNode), workloadCmd)
		require.NoError(t, err)
	}
