...
func runClearRange(ctx context.Context, t test.Test, c cluster.Cluster, aggressiveChecks bool) {
	...
	m.Go(func(ctx context.Context) error {
		...
		postDropBankStats := getBigBankStats()
		...
		const (
			targetRangeCount = 10
			targetDiskBytes  = 100 << 20 // 100 MiB
		)
		curBankStats := getBigBankStats()
		for curBankStats.rangeCount > targetRangeCount || curBankStats.approximateDiskBytes > targetDiskBytes {
			after := time.After(5 * time.Minute)
			estimatedProgress := 1 - (float64(curBankStats.rangeCount) / float64(postDropBankStats.rangeCount))
			estimatedProgress = min(estimatedProgress,
				1-(float64(curBankStats.approximateDiskBytes)/float64(postDropBankStats.approximateDiskBytes)))
			t.WorkerProgress(estimatedProgress)
			...
			t.WorkerStatus("waiting for ~", max(0, curBankStats.rangeCount-targetRangeCount-1),
				" merges to complete and ",
				humanizeutil.IBytes(max(0, curBankStats.approximateDiskBytes-targetDiskBytes)),
				" disk space to be reclaimed")
			select {
			case <-after:
			case <-ctx.Done():
				return ctx.Err()
