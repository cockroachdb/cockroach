...
func runClearRange(ctx context.Context, t test.Test, c cluster.Cluster, aggressiveChecks bool) {
	...
	m.Go(func(ctx context.Context) error {
		...
		tBegin := timeutil.Now()
		c.Run(ctx, option.OnNodes(c.Node(1)), "./cockroach", "workload", "fixtures", "import", "bank",
	...
	c.Run(ctx, option.OnNodes(c.Node(1)), "./cockroach", "workload", "fixtures", "import", "bank", "--db=tinybank",
...
ORDER BY raw_start_key ASC LIMIT 1`,
	...
	m.Go(func(ctx context.Context) error {
		c.Run(ctx, option.OnNodes(c.Node(1)), `./cockroach workload init kv`, pgurl)
		c.Run(ctx, option.OnNodes(c.All()), fmt.Sprintf(`./cockroach workload run kv --concurrency=32 --duration=1h --tolerate-errors {pgurl%s}`, c.All()))
		return nil
	})
	m.Go(func(ctx context.Context) error {
		conn := c.Conn(ctx, t.L(), 1)
