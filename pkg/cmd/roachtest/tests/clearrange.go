...
func runClearRange(ctx context.Context, t test.Test, c cluster.Cluster, aggressiveChecks bool) {
	...
	m.Go(func(ctx context.Context) error {
		...
		progressFn := func() float64 {
			...
			// immediately after dropping the table. That is 1 - (current /
			// initial).
			//
			// We compute the progress as the minimum of the two metrics'
			// progress.
			mergeProgress := 1 - (float64(curBankStats.rangeCount) /
				float64(postDropBankStats.rangeCount))
			diskProgress := 1 - (float64(curBankStats.approximateDiskBytes) /
				float64(postDropBankStats.approximateDiskBytes))
			return max(0, min(mergeProgress, diskProgress))
		}
		// Terminate when progress is 0.99 or greater. That is, we've reclaimed
		// 99% of the disk space and merged 99% of the ranges.
		for progress := progressFn(); progress < 0.99; progress = progressFn() {
