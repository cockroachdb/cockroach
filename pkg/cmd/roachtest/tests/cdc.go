...
func (ct *cdcTester) setupSink(args feedArgs) string {
	...
	case webhookSink:
		...
		m.Go(func(ctx context.Context) error {
			return ct.cluster.RunE(ct.ctx, option.OnNodes(webhookNode), serverExecCmd, rootFolder)
		})
...
func runCDCBank(ctx context.Context, t test.Test, c cluster.Cluster) {
	...
	// spam.
	c.Run(ctx, option.OnNodes(c.All()), `mkdir -p logs`)
	...
	c.Run(ctx, option.OnNodes(workloadNode), `./workload init bank {pgurl:1}`)
	db := c.Conn(ctx, t.L(), 1)
	defer stopFeeds(db)
	...
	m.Go(func(ctx context.Context) error {
		err := c.RunE(ctx, option.OnNodes(workloadNode), `./workload run bank {pgurl:1} --max-rate=10`)
...
func (k kafkaManager) install(ctx context.Context) {
	...
	k.c.Run(ctx, option.OnNodes(k.nodes), `mkdir -p `+folder)
	...
	k.c.Run(ctx, option.OnNodes(k.nodes), downloadScriptPath, folder)
	if !k.c.IsLocal() {
		k.c.Run(ctx, option.OnNodes(k.nodes), `mkdir -p logs`)
...
func (k kafkaManager) installJRE(ctx context.Context) error {
	...
	return retry.WithMaxAttempts(ctx, retryOpts, 3, func() error {
		err := k.c.RunE(ctx, option.OnNodes(k.nodes), `sudo apt-get -q update 2>&1 > logs/apt-get-update.log`)
		if err != nil {
			return err
		}
		return k.c.RunE(ctx, option.OnNodes(k.nodes), `sudo DEBIAN_FRONTEND=noninteractive apt-get -yq --no-install-recommends install openssl default-jre maven 2>&1 > logs/apt-get-install.log`)
	})
}
...
func (k kafkaManager) runWithRetry(ctx context.Context, cmd string) {
	...
	err := retry.WithMaxAttempts(ctx, retryOpts, 3, func() error {
		return k.c.RunE(ctx, option.OnNodes(k.nodes), cmd)
	})
	if err != nil {
		k.t.Fatal(err)
	}
...
func (k kafkaManager) configureHydraOauth(ctx context.Context) (string, string) {
	k.c.Run(ctx, option.OnNodes(k.nodes), `rm -rf /home/ubuntu/hydra`)
	...
	mon.Go(func(ctx context.Context) error {
		err := k.c.RunE(ctx, option.OnNodes(k.nodes), `/home/ubuntu/hydra-serve.sh`)
...
func (k kafkaManager) configureOauth(ctx context.Context) (clientcredentials.Config, string) {
	...
	k.c.Run(ctx, option.OnNodes(k.nodes), `rm -rf /home/ubuntu/kafka-oauth`)
	k.runWithRetry(ctx, `git clone https://github.com/jairsjunior/kafka-oauth.git /home/ubuntu/kafka-oauth`)
	k.c.Run(ctx, option.OnNodes(k.nodes), `(cd /home/ubuntu/kafka-oauth; git checkout c2b307548ef944d3fbe899b453d24e1fc8380add; mvn package)`)
...
func (k kafkaManager) configureAuth(ctx context.Context) *testCerts {
	...
	k.c.Run(ctx, option.OnNodes(k.nodes),
	...
	k.c.Run(ctx, option.OnNodes(k.nodes), fmt.Sprintf("rm -f %s", keystorePath))
	k.c.Run(ctx, option.OnNodes(k.nodes), fmt.Sprintf("rm -f %s", truststorePath))
...
func (k kafkaManager) addSCRAMUsers(ctx context.Context) {
	...
	k.c.Run(ctx, option.OnNodes(k.nodes), filepath.Join(k.binDir(), "kafka-configs"),
...
func (k kafkaManager) start(ctx context.Context, service string, envVars ...string) {
	...
	k.c.Run(ctx, option.OnNodes(k.nodes), k.makeCommand("confluent", "local destroy || true"))
...
func (k kafkaManager) restart(ctx context.Context, targetService string, envVars ...string) {
	...
	k.c.Run(ctx, option.OnNodes(k.nodes), "touch", k.serverJAASConfig())
	for _, svcName := range services {
		...
		k.c.Run(ctx, option.OnNodes(k.nodes), startCmd)
	}
}
...
func (k kafkaManager) stop(ctx context.Context) {
	k.c.Run(ctx, option.OnNodes(k.nodes), fmt.Sprintf("rm -f %s", k.serverJAASConfig()))
	k.c.Run(ctx, option.OnNodes(k.nodes), k.makeCommand("confluent", "local services stop"))
}
...
func (tw *tpccWorkload) install(ctx context.Context, c cluster.Cluster) {
	...
	c.Run(ctx, option.OnNodes(tw.workloadNodes), fmt.Sprintf(
...
func (tw *tpccWorkload) run(ctx context.Context, c cluster.Cluster, workloadDuration string) {
	...
	c.Run(ctx, option.OnNodes(tw.workloadNodes), cmd.String())
}
...
func (lw *ledgerWorkload) install(ctx context.Context, c cluster.Cluster) {
	...
	c.Run(ctx, option.OnNodes(lw.workloadNodes.RandNode()), fmt.Sprintf(
...
func (lw *ledgerWorkload) run(ctx context.Context, c cluster.Cluster, workloadDuration string) {
	// TODO(#94136): remove --data-loader=INSERT
	c.Run(ctx, option.OnNodes(lw.workloadNodes), fmt.Sprintf(
...
func setupKafka(
	...
	if !c.IsLocal() {
		...
		c.Run(ctx, option.OnNodes(kafka.nodes), `echo "advertised.listeners=PLAINTEXT://`+kafka.consumerURL(ctx)+`" >> `+
