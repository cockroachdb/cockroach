...
func verifyLeaseHolderLocality(db *gosql.DB, t test.Test, primaryRegion string) {
	...
	ok := false
	for {
		if timeutil.Since(tStart) > 5*time.Minute {
			t.L().Printf("Timeout waiting for lease holders to be in region %s", primaryRegion)
			t.Fatalf("Timeout waiting for lease holders to be in region %s; waited for %s", primaryRegion, timeutil.Since(tStart))
		}
