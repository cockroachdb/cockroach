...
func (tester *cdcTester) setupSink(t test.Test, args feedArgs) string {
	var sinkURI string
	...
	if whichSink == nullSink {
		...
		kafkaNode := tester.cluster.Node(tester.cluster.Spec().NodeCount)
		kafka := kafkaManager{
			t:     t,
			c:     tester.cluster,
			nodes: kafkaNode,
		}
		kafka.install(tester.ctx)
		kafka.start(tester.ctx)

		if args.kafkaChaos {
			tester.m.Go(func(ctx context.Context) error {
				period, downTime := 2*time.Minute, 20*time.Second
				return kafka.chaosLoop(ctx, period, downTime, tester.doneCh)
			})
		}

		sinkURI = kafka.sinkURL(tester.ctx)
	}
...
func (cj *changefeedJob) runFeedPoller(
	ctx context.Context, pollInterval time.Duration, stopper chan struct{}, onInfo func(info changefeedInfo),
) error {
	for {
		select {
		case <-ctx.Done():
