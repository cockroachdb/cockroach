...
func configureHydraOauth(
	ctx context.Context, t test.Test, c cluster.Cluster, node option.NodeListOption,
) (string, string) {
	c.Run(ctx, node, `rm -rf /home/ubuntu/hydra`)
	c.Run(ctx, node, `bash <(curl https://raw.githubusercontent.com/ory/meta/master/install.sh) -d -b . hydra v2.0.3`)

	err := c.PutString(ctx, hydraServerStartScript, "/home/ubuntu/hydra-serve.sh", 0700, node)
	if err != nil {
		t.Fatal(err)
	}
	mon := c.NewMonitor(ctx, node)
	mon.Go(func(ctx context.Context) error {
		err := c.RunE(ctx, node, `/home/ubuntu/hydra-serve.sh`)
		return errors.Wrap(err, "hydra failed")
	})
	result, err := c.RunWithDetailsSingleNode(ctx, t.L(), node, "/home/ubuntu/hydra create oauth2-client",
...
func (k kafkaManager) configureOauth(ctx context.Context) (clientcredentials.Config, string) {
	...
	k.c.Run(ctx, k.nodes, `git clone https://github.com/jairsjunior/kafka-oauth.git /home/ubuntu/kafka-oauth`)
	...
	clientID, clientSecret := configureHydraOauth(ctx, k.t, k.c, k.nodes)
