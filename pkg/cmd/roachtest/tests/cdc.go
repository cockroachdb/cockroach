...
func (ct *cdcTester) runTPCCWorkload(args tpccArgs) {
	...
	if !ct.t.SkipInit() {
		...
		if args.SchemaLockTables.enabled(globalEnthropy) {
...
func (f *featureFlag) enabled(r enthropy) bool {
	if f.v != nil {
		return *f.v
	}
...
func (cfc *changefeedCreator) applySettings() error {
	...
	cfc.logger.Printf("Setting changefeed.mux_rangefeed.enabled to %t", cfc.flags.MuxRangefeed.enabled(cfc.rng))
	if _, err := cfc.db.Exec(
		"SET CLUSTER SETTING changefeed.mux_rangefeed.enabled = $1", cfc.flags.MuxRangefeed.enabled(cfc.rng),
	); err != nil {
		return err
	}
	cfc.logger.Printf("Setting kv.rangefeed.scheduler.enabled to %t", cfc.flags.RangeFeedScheduler.enabled(cfc.rng))
	if _, err := cfc.db.Exec(
		"SET CLUSTER SETTING kv.rangefeed.scheduler.enabled = $1", cfc.flags.RangeFeedScheduler.enabled(cfc.rng),
	); err != nil {
		return err
	}
	return nil
