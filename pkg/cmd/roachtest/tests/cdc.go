...
func (t *cdcTester) startCRDBChaos() {
	...
	t.mon.Go(ch.Runner(t.cluster, t.t, t.mon))
}
...
func (ct *cdcTester) runFeedLatencyVerifier(cj changefeedJob, targets latencyTargets) {
	...
	verifier := makeLatencyVerifier(cj.Label(), info.statementTime, targets, ct.logger, ct.t.Status)

	pollDuration := time.Second
	ct.mon.Go(func(ctx context.Context) error {
		err := cj.runFeedPoller(ctx, pollDuration, ct.doneCh, func(info changefeedInfo) {
...
func (lv *latencyVerifier) noteHighwater(highwaterTime time.Time) {
	...
	if lv.initialScanLatency == 0 {
		...
		lv.logger.Printf("[%s] initial scan completed: latency %s\n", lv.label, lv.initialScanLatency)
		return
	}
	...
	if !lv.latencyBecameSteady {
		...
		if lv.maxSeenSteadyEveryN.ShouldLog() {
			lv.setTestStatus(fmt.Sprintf(
				"[%s] watching changefeed: end-to-end latency %s not yet below target steady latency %s",
				lv.label, latency.Truncate(time.Millisecond), lv.latencyTargets.steadyLatency.Truncate(time.Millisecond)))
		}
		return
	}
	...
	if lv.maxSeenSteadyEveryN.ShouldLog() {
		lv.setTestStatus(fmt.Sprintf(
			"[%s] watching changefeed: end-to-end steady latency %s; max steady latency so far %s",
			lv.label, latency.Truncate(time.Millisecond), lv.maxSeenSteadyLatency.Truncate(time.Millisecond)))
	}
}
