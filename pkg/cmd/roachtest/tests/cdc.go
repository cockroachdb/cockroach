...
func runCDCMultiTablePTSBenchmark(
	...
	initCmd := fmt.Sprintf("./cockroach workload init bank --rows=%d --ranges=%d --tables=%d --import-concurrency-limit=8 {pgurl%s}",
		...
		for _, cfg := range []struct {
			...
			r.Add(registry.TestSpec{
				...
				Run: func(ctx context.Context, t test.Test, c cluster.Cluster) {
					...
					initCmd := fmt.Sprintf(
						"./cockroach workload init bank --tables=%d --ranges=%d --rows=%d --import-concurrency-limit=8 {pgurl%s}",
