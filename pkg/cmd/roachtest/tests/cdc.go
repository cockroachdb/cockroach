...
func registerCDC(r registry.Registry) {
	...
	r.Add(registry.TestSpec{
		...
		Run: func(ctx context.Context, t test.Test, c cluster.Cluster) {
			...
			if brokers.connectURI != "" {
				t.L().Printf("creating changefeed with iam: %s", brokers.connectURI)
				_, err := newChangefeedCreator(db, t.L(), globalRand, "auth_test_table", brokers.connectURI, makeDefaultFeatureFlags()).Create()
				if err != nil {
...
func (m *mskManager) WaitForClusterActiveAndDNSUpdated(
	...
	const iamRoleArn = "arn:aws:iam::541263489771:role/roachprod-msk-full-access" // See pkg/roachprod/vm/aws/terraform/iam.tf
	broker := *resp.BootstrapBrokerStringSaslIam
	connectInfo := mskIAMConnectInfo{
		broker:     broker,
		connectURI: fmt.Sprintf("kafka://%s?tls_enabled=true&sasl_enabled=true&sasl_mechanism=AWS_MSK_IAM&sasl_aws_region=%s&sasl_aws_iam_role_arn=%s&sasl_aws_iam_session_name=cdc-msk", broker, awsRegion, iamRoleArn),
	}
...
func (m *mskManager) CreateTopic(ctx context.Context, topic string, c cluster.Cluster) {
	...
	require.NoError(m.t, c.RunE(ctx, withCTN, path.Join(createMSKTopicBinPath, "run.sh"), m.connectInfo.broker, topic))
}
