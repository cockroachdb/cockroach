...
func veryifyLeaseHolderDistribution(db *gosql.DB, t test.Test, nodeToZone map[int]string) {
	...
	rows, err := db.Query("select r.start_pretty, r.replicas, r.replica_localities, r.lease_holder from crdb_internal.ranges r join crdb_internal.tables t on r.start_pretty like concat('/Table/', t.table_id::STRING,'%') where t.database_name = 'tpcc'")
...
func registerCDC(r registry.Registry) {
	r.Add(registry.TestSpec{
		...
		Run: func(ctx context.Context, t test.Test, c cluster.Cluster) {
			...
			feed := cdcTester.newChangefeed(feedArgs{
