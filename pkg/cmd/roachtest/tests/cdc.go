...
func registerCDC(r registry.Registry) {
	...
	r.Add(registry.TestSpec{
		...
		Run: func(ctx context.Context, t test.Test, c cluster.Cluster) {
			...
			// cdc/kafka-topics tests the work done for
			// https://github.com/cockroachdb/cockroach/commit/4820ffdd0ae2d39ac54583bb6d2f4bd3b3c109d4.
			// The patch changes the configuration for sarama clients so that
			// changefeed only fetches metadata for a minimal set of topics that have
			// been necessary. This test verifies the work by 1. creating lots of
			// random kafka topics on the kafka cluster 2. running some tpcc workload
			// and changefeed 3. configure changefeed target to be all tpcc tables
			// (note that we create kafka topics for every target tables) 3. assert
			// that changefeed only fetches metadata for tpcc tables and not for other
			...
			// Create random topics and make sure that sarama client is not fetching
			// metadata for these random topics.
			...
			// Run changefeed and configure target table to be all tpcc tables.
			feed := ct.newChangefeed(feedArgs{
				sinkType: kafkaSink,
				targets:  allTpccTargets,
				opts:     map[string]string{"initial_scan": "'only'"},
			...
			testutils.SucceedsWithin(t, func() error {
				...
				pass := func(str string) error {
					// We do not expect to see the log line which indicates fetching
					// metadata for any topics with ignoreTopicPrefix.
					...
					for _, target := range allTpccTargets {
						...
						// We expect to see the log line which indicates fetching metadata
						// for tpcc topics.
