...
type feedArgs struct {
	...
	// sinkChaos enables chaos on the sink. Only implemented for kafka and webhook sinks.
	sinkChaos bool
	// validateOrder enables order validation. Only implemented for kafka and webhook sinks.
	validateOrder bool
...
func (m *webhookManager) tail() io.Reader {
	opts := install.DefaultRunOptions().WithRetryDisabled()
	opts.Nodes = m.webhookSinkNode.InstallNodes()
	stdouts, errCh, err := m.cluster.SpawnE(m.ctx, opts, m.t.L(), "tail", "-F", fmt.Sprintf("/mnt/data2/webhook-output-%d.jsonl", m.chosenPort))
	stdout := stdouts[0] // Only one sink node curently.
	if err != nil {
