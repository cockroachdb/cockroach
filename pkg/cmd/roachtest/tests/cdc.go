...
func (m *mskManager) WaitForClusterActiveAndDNSUpdated(
	...
	connectInfo := mskIAMConnectInfo{
		broker:     broker,
		connectURI: fmt.Sprintf("kafka://%s?tls_enabled=true&sasl_enabled=true&sasl_mechanism=AWS_MSK_IAM&sasl_aws_region=%s&sasl_aws_iam_role_arn=%s&sasl_aws_iam_session_name=cdc-msk", broker, awsRegion, mskRoleArn),
	}
