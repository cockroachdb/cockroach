...
func (m *mskManager) WaitForClusterActiveAndDNSUpdated(
	...
	if iamBroker == nil {
		...
		iamConnectURI = fmt.Sprintf("kafka://%s?tls_enabled=true&sasl_enabled=true&sasl_mechanism=AWS_MSK_IAM&sasl_aws_region=%s&sasl_aws_iam_role_arn=%s&sasl_aws_iam_session_name=cdc-msk", awsRegion, iamRoleArn, *iamBroker)
	}
	connectInfo := mskConnectInfo{
