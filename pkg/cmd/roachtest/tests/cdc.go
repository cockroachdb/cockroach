...
// N.B. this allocates a workload + sink only node, without this workload and sink will share a node.
func withNumSinkNodes(num int) opt {
	return func(ct *cdcTester) {
		// We can't use WorkloadNodes here because the test differentiates between
		// workload nodes and sink nodes, but the framework just cares they aren't CRDB nodes.
...
func registerCDC(r registry.Registry) {
	...
	r.Add(registry.TestSpec{
		...
		Owner:            `cdc`,
		Benchmark:        true,
		Cluster:          r.MakeClusterSpec(6, spec.CPU(16), spec.WorkloadNodes(3)),
		Leases:           registry.MetamorphicLeases,
		CompatibleClouds: registry.AllExceptAWS,
		Suites:           registry.Suites(registry.Nightly),
		RequiresLicense:  true,
