...
// The nodes on which the sink will be installed and run.
func (ct *cdcTester) sinkNodes() option.NodeListOption {
	// by convention the nodes are split up like [crdb..., workload, sink...]. the sink nodes are always the last numSinkNodes ones.
	return ct.cluster.Range(ct.cluster.Spec().NodeCount-ct.numSinkNodes+1, ct.cluster.Spec().NodeCount)
}
...
func withNumSinkNodes(num int) opt {
	return func(ct *cdcTester) {
		ct.numSinkNodes = num
		ct.crdbNodes = ct.cluster.Range(1, ct.cluster.Spec().NodeCount-ct.numSinkNodes-1)
		ct.workloadNode = ct.cluster.Node(ct.cluster.Spec().NodeCount - ct.numSinkNodes)
	}
}
...
func registerCDC(r registry.Registry) {
	...
	r.Add(registry.TestSpec{
		...
		Run: func(ctx context.Context, t test.Test, c cluster.Cluster) {
			...
			err := c.Install(ctx, t.L(), ct.sinkNodes(), "go")
			if err != nil {
				t.Fatalf("failed to install on webhook sink node: %v", err)
			}
...
type webhookManager struct {
	t               test.Test
	cluster         cluster.Cluster
	webhookSinkNode option.NodeListOption
	workloadNode    option.NodeListOption
	mon             cluster.Monitor
	ctx             context.Context
	logger          *logger.Logger
...
func (m *webhookManager) Start() (dest *url.URL) {
	...
	nodeIPs, err := m.cluster.ExternalIP(m.ctx, m.logger, m.webhookSinkNode)
	...
	err = m.cluster.PutString(m.ctx, webhookServerScript(m.chosenPort), serverSrcPath, 0700, m.webhookSinkNode)
	if err != nil {
		m.t.Fatal(err)
	}
	...
	err = m.cluster.PutString(m.ctx, certs.SinkKey, filepath.Join(rootFolder, "key.pem"), 0700, m.webhookSinkNode)
	if err != nil {
		m.t.Fatal(err)
	}
	err = m.cluster.PutString(m.ctx, certs.SinkCert, filepath.Join(rootFolder, "cert.pem"), 0700, m.webhookSinkNode)
	if err != nil {
		m.t.Fatal(err)
	}
	...
	m.mon.Go(func(ctx context.Context) error {
		...
		for ctx.Err() == nil {
			...
			opts := option.WithNodes(m.webhookSinkNode)
...
func (m *webhookManager) tail() io.Reader {
	stdout, errCh, err := m.cluster.Tail(m.ctx, m.t.L(), fmt.Sprintf("/mnt/data2/webhook-output-%d.jsonl", m.chosenPort), m.webhookSinkNode)
	if err != nil {
...
func (m *webhookManager) runValidateOrder(stdout io.Reader) {
	...
	scanner.Buffer(buf, 10*1024*1024) // Need to make the buffers big enough for our largest rows. 10MiB is good enough empirically.
	var i int
	for scanner.Scan() {
		...
		if payload.Resolved != "" {
			for _, v := range validators {
				...
				require.NoError(m.t, err)
				require.NoError(m.t, v.NoteResolved(partition, resolved))
			}
			continue
		}
		for _, p := range payload.Payload {
			...
			err = validators[p.Topic].NoteRow(partition, string(p.Key), "value doesnt matter", updated)
			require.NoError(m.t, err)
...
func (m *webhookManager) runChaosLoop() {
	...
	stop := func(ctx context.Context) {
		_, err := m.cluster.RunWithDetails(ctx, m.logger, option.WithNodes(m.webhookSinkNode), `bash`, `-c`, `set -eo pipefail; ps aux | grep 'webhook-server' | grep -v grep | awk '{print $2}' | xargs kill`)
		require.NoError(m.t, err)
		m.t.L().Printf("webhooks chaos loop: killed server")
	}
	restart := func(ctx context.Context) {
		select {
		case m.restartSink <- struct{}{}:
		case <-ctx.Done():
			return
		}
