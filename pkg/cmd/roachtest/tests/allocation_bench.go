...
func (r kvAllocBenchEventRunner) run(ctx context.Context, c cluster.Cluster, t test.Test) error {
	...
	setupCmd += " {pgurl:1}"
	err := c.RunE(ctx, option.WithNodes(c.WorkloadNodes()), setupCmd)
	if err != nil {
		return err
	}
...
func registerAllocationBenchSpec(r registry.Registry, allocSpec allocationBenchSpec) {
	specOptions := []spec.Option{spec.CPU(allocSpec.cpus), spec.WorkloadNodes(1), spec.WorkloadNodeCPU(allocSpec.cpus)}
