...
const (
	...
	defaultAllocBenchDuration = 40 * time.Minute
	...
	defaultStartRecord = 20 * time.Minute
...
// kvAllocBenchEventRunner implements the clampResourceEventRunner interface.
...
func (kvr *kvAllocBenchEventRunner) run(ctx context.Context, c cluster.Cluster, t test.Test) error {
	workloadNode := c.Spec().NodeCount
	name := fmt.Sprintf("kv%d%db", kvr.readPercent, kvr.blockSize)
	...
	// If the replication factor has been altered (!=0) or pinned attrbutes
	// declared, alter the database config.
	stmt := fmt.Sprintf("alter database %s configure zone using num_replicas=%d",
		name, kvr.replFactor)
	if kvr.pinnedAttribute != "" {
		stmt = fmt.Sprintf("%s, constraints='[+%s]'", stmt, kvr.pinnedAttribute)
	}
	...
	runCmd := fmt.Sprintf(
		...
		name, kvr.readPercent, kvr.blockSize, kvr.blockSize, kvr.rate)
	if kvr.skew {
		runCmd = fmt.Sprintf("%s --zipfian", runCmd)
	}

	// When span is specifed, add in 10% spans to the workload with 10k
	// spanning limit batched in sizes of 10. This puts an read (cpu) intensive
	// load on the cluster.
	if kvr.span {
		runCmd = fmt.Sprintf("%s --span-percent=10 --span-limit=10000 --batch=10", runCmd)
	}
