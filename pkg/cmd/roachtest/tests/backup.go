...
func registerBackupMixedVersion(r registry.Registry) {
	r.Add(registry.TestSpec{
		...
		Run: func(ctx context.Context, t test.Test, c cluster.Cluster) {
			...
			backupFailsWithExpectedErrorStep := func(nodeID int) versionStep {
				return func(ctx context.Context, t test.Test, u *versionUpgradeTest) {
			...
			u := newVersionUpgradeTest(c,
				...
				// Upgrade from new node should succeed
				successfulBackupStep(1),
				// Upgrade from new node with revision history should succeed
				successfulBackupStep(1, "revision_history"),
				// Upgrade from old node fails
				backupFailsWithExpectedErrorStep(3),
			)
			u.run(ctx, t)
		},
	})
