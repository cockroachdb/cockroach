...
func verifyBackupStep(
	node option.NodeListOption, backupLoc string, dbName, tableName, intoDB string,
) versionStep {
	return func(ctx context.Context, t test.Test, u *versionUpgradeTest) {
		db := u.conn(ctx, t, node[0])

		// First fingerprint the original table.
		origFingerprint, err := fingerprint(ctx, db, dbName, tableName)
		require.NoError(t, err)
	...
	r.Add(registry.TestSpec{
		...
		Run: func(ctx context.Context, t test.Test, c cluster.Cluster) {
			...
			u := newVersionUpgradeTest(c,
				...
				// Run an incremental backup from an old node so that it is planned on the old node
				// but the job is adopted on a new node.
				planAndRunBackup(t, c, oldNodes.RandNode(), oldNodes,
