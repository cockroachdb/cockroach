...
func verifyBackupStep(node option.NodeListOption, backupLoc string, intoDB string) versionStep {
	return func(ctx context.Context, t test.Test, u *versionUpgradeTest) {
		db := u.conn(ctx, t, node[0])

		// First fingerprint the original table.
		origFingerprint, err := fingerprint(ctx, db, "bank", "bank")
		require.NoError(t, err)

		// Now, restore the backup.
		_, err = db.ExecContext(ctx, fmt.Sprintf(`CREATE DATABASE %s`, intoDB))
		require.NoError(t, err)
		_, err = db.ExecContext(ctx, fmt.Sprintf(`RESTORE TABLE bank.bank FROM LATEST IN '%s' WITH into_db = '%s'`,
			backupLoc, intoDB))
		require.NoError(t, err)

		restoredFingerPrint, err := fingerprint(ctx, db, intoDB, "bank")
		require.NoError(t, err)
...
func registerBackupMixedVersion(r registry.Registry) {
	...
	// haltJobAdoptionOnNodesStep writes the sentinel file to prevent a node's
