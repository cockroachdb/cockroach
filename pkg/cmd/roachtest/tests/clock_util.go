...
func (oi *offsetInjector) deploy(ctx context.Context) error {
	if err := oi.c.RunE(ctx, option.OnNodes(oi.c.All()), "test -x ./bumptime"); err == nil {
		oi.deployed = true
		return nil
	}
	...
	if err := oi.c.RunE(ctx, option.OnNodes(oi.c.All()), "sudo", "service", "ntp", "stop"); err != nil {
		return err
	}
	if err := oi.c.RunE(ctx, option.OnNodes(oi.c.All()),
...
func (oi *offsetInjector) offset(ctx context.Context, nodeID int, s time.Duration) {
	...
	oi.c.Run(
		ctx,
		option.OnNodes(oi.c.Node(nodeID)),
