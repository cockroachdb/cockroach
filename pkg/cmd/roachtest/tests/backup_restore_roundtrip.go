...
func startBackgroundWorkloads(
	...
	err := c.RunE(ctx, option.OnNodes(workloadNode), bankInit.String())
	if err != nil {
		return nil, err
	}

	err = c.RunE(ctx, option.OnNodes(workloadNode), tpccInit.String())
	if err != nil {
		return nil, err
	}
	err = c.RunE(ctx, option.OnNodes(workloadNode), scInit.String())
	if err != nil {
		return nil, err
	}
	...
	run := func() (func(), error) {
		...
		stopBank := workloadWithCancel(m, func(ctx context.Context) error {
			return c.RunE(ctx, option.OnNodes(workloadNode), bankRun.String())
		})

		stopTPCC := workloadWithCancel(m, func(ctx context.Context) error {
			return c.RunE(ctx, option.OnNodes(workloadNode), tpccRun.String())
		})
		stopSC := workloadWithCancel(m, func(ctx context.Context) error {
			return c.RunE(ctx, option.OnNodes(workloadNode), scRun.String())
		})
