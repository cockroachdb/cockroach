...
func registerSingleNodeIndexBackfill(r registry.Registry) {
	for _, withProvisionedBandwidth := range []bool{false, true} {
		...
		if withProvisionedBandwidth {
			nameSuffix = "/with-provisioned-bandwidth"
			snapshotPrefix += "-bw"
		}
...
func runSingleNodeIndexBackfill(
	...
	const (
		customers            = singleNodeIndexBackfillCustomers
	...
	workloadDuration := 240 * time.Minute
	...
	// metrics collection. Without the provisioned bandwidth set, the backfill
	// took ~38min.
	//
