	...
	if !metricsStart.IsZero() && !metricsEnd.IsZero() {
		exportingStats, err := statCollector.Exporter().Export(
			...
			func(stats map[string]clusterstats.StatSummary) (string, float64) {
				return "index_backfill_duration", backfillDuration.Seconds()
			},
			func(stats map[string]clusterstats.StatSummary) (string, float64) {
				return "mean_total_bandwidth", avgTotalBW
			},
			func(stats map[string]clusterstats.StatSummary) (string, float64) {
				return "mean_p99_latency", meanNonNaN(stats, sqlServiceLatency.Query)
			},
			func(stats map[string]clusterstats.StatSummary) (string, float64) {
				return "mean_qps", meanNonNaN(stats, indexBackfillQPS.Query)
			},
		)
		if err != nil {
			t.Fatal(err)
