...
func registerKVBenchSpec(r registry.Registry, b kvBenchSpec) {
	...
	opts := []spec.Option{spec.CPU(b.CPUs), spec.WorkloadNodes(1), spec.WorkloadNodeCPU(b.CPUs)}
...
func runKVBench(ctx context.Context, t test.Test, c cluster.Cluster, b kvBenchSpec) {
	...
	searchPredicate := func(maxrate int) (bool, error) {
		...
		m.Go(func(ctx context.Context) error {
			...
			if err := c.RunE(ctx, option.WithNodes(c.WorkloadNodes()), initCmd.String()); err != nil {
				return err
			}
			...
			err := c.RunE(ctx, option.WithNodes(c.WorkloadNodes()), workloadCmd.String())
			if err != nil {
			...
			if err := c.Get(ctx, t.L(), clusterHistPath, localHistPath, c.WorkloadNodes()); err != nil {
				t.Fatal(err)
			}
