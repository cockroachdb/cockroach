...
func (d *dbConsoleCypressTest) installDeps(ctx context.Context) {
	...
	// The embedded package.json has cypress ^10.3.0 which doesn't support
	// Node 22. Patch it to ^13.13.2 (matching the old Docker image) on the
	// workload node so we don't have to touch the shared lockfile.
	require.NoError(t, rtCluster.RunE(ctx, option.WithNodes(workloadNode),
		fmt.Sprintf(`sed -i 's/"cypress": "[^"]*"/"cypress": "^13.13.2"/' %s/package.json`, d.cypressWorkingDir)))
