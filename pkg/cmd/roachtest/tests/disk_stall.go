...
func runDiskStalledDetection(ctx context.Context, t test.Test, c cluster.Cluster, s diskStaller) {
	...
	c.Run(ctx, c.All(), "ln -s /home/ubuntu/logs-external/cockroach.exit.log logs/cockroach.exit.log || true")

	t.Status("starting cluster")
	c.Put(ctx, t.Cockroach(), "./cockroach")
	c.Start(ctx, t.L(), startOpts, startSettings, c.Range(1, 3))
	defer c.Stop(ctx, t.L(), option.DefaultStopOpts(), c.Range(1, 3))
	...
	defer n2conn.Close()
	if err := n1Conn.PingContext(ctx); err != nil {
		t.Fatal(err)
	}
	if _, err := n2conn.ExecContext(ctx, `USE kv;`); err != nil {
		t.Fatal(err)
	}
...

	{
		...
		preStallTPS := totalTxnsPreStall / float64(stallAt.Sub(workloadStartAt).Seconds())
		postStallTPS := totalTxnsPostStall / float64(now.Sub(stallAt).Seconds())
		...
		if postStallTPS < preStallTPS*0.60 {
			t.Fatalf("post-stall TPS %.2f is less than 60%% of pre-stall TPS %.2f", postStallTPS, preStallTPS)
		}
	}

	{
		...
		var rowCount int
		if err := n2conn.QueryRowContext(ctx, `SELECT COUNT(v) FROM kv`).Scan(&rowCount); err != nil {
			t.Fatal(err)
		}
