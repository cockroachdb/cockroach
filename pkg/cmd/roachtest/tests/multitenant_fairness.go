	...
	for _, acEnabled := range []bool{true, false} {
		...
		for i := range kvSpecs {
			...
			r.Add(registry.TestSpec{
				Name:              fmt.Sprintf("multitenant/admission-control/kv-fairness/%s/%s", s.name, acStr[s.acEnabled]),
		...
		for i := range storeSpecs {
			...
			r.Add(registry.TestSpec{
				Name:              fmt.Sprintf("multitenant/admission-control/store-fairness/%s/%s", s.name, acStr[s.acEnabled]),
				Cluster:           r.MakeClusterSpec(5),
				Owner:             registry.OwnerSQLQueries,
				NonReleaseBlocker: false,
				Run: func(ctx context.Context, t test.Test, c cluster.Cluster) {
