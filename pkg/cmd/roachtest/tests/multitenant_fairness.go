...
func registerMultiTenantFairness(r registry.Registry) {
	...
	for _, acEnabled := range []bool{true, false} {
		kvSpecs := []mtFairnessSpec{
			{
				name:        "same",
				concurrency: func(int) int { return 150 },
			},
			{
	...
	for i := 0; i < numTenants; i++ {
		...
		m.Go(func(ctx context.Context) error {
			cmd := fmt.Sprintf(
				...
				pgurl, s.blockSize, s.blockSize, s.loadBatchSize, s.maxLoadOps)
			err := c.RunE(ctx, c.Node(node), cmd)
		...
		m.Go(func(ctx context.Context) error {
			cmd := fmt.Sprintf(
				"./cockroach workload run kv '%s' --write-seq=%s --secure --min-block-bytes %d "+
					"--max-block-bytes %d --batch 20 --duration=%s --read-percent=%d --concurrency=%d",
				pgurl, fmt.Sprintf("R%d", s.maxLoadOps*s.loadBatchSize), s.blockSize, s.blockSize, duration,
				s.readPercent, s.concurrency(node-1))
			err := c.RunE(ctx, c.Node(node), cmd)
