...
func runMultiTenantFairness(
	...
	cmd := fmt.Sprintf(
		`echo '{ "max_tput_delta": %f, "max_latency_delta": %f, "max_tput": %f, "min_tput": %f, "max_latency": %f, "min_latency": %f}' > %s/stats.json`,
		maxThroughputDelta, maxLatencyDelta, maxFloat(throughput), minFloat(throughput), maxFloat(meanLatencies), minFloat(meanLatencies), t.PerfArtifactsDir(),
	)
