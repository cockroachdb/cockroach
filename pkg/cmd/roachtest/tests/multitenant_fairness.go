...
func registerMultiTenantFairness(r registry.Registry) {
	...
	for _, acEnabled := range []bool{true, false} {
		kvSpecs := []mtFairnessSpec{
			{
				name:        "same",
				concurrency: func(int) int { return 48 },
			...
			{
				name:        "size-skew",
				concurrency: func(_ int) int { return 48 },
				blockSizes:  func(int) (int, int) { return 4, 8 },
			},
		}
		...
		storeSpecs := []mtFairnessSpec{
			{
				...
				blockSizes:  func(int) (int, int) { return 100, 1000 },
			},
			{
				name:        "concurrency-skew",
				concurrency: func(i int) int { return i * 10 },
			...
			},
			{
				name:        "size-skew",
				concurrency: func(i int) int { return 32 },
				blockSizes:  func(i int) (int, int) { return 50 * i, 500 * i },
			},
		}
...
func runMultiTenantFairness(
	...
	// For now don't actually fail, we'll just watch the numbers over time.
	failThreshold := .3
	...
	if s.acEnabled && !ok {
		...
		t.L().Printf("Throughput not within expectations: %f > %f %v", maxDeltaPercentage, failThreshold, throughput)
	}
	...
	if s.acEnabled && !ok {
		t.L().Printf("Latency not within expectations: %f > %f %v", maxDeltaPercentage, failThreshold, meanLatencies)
	}
