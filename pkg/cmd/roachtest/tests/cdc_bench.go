		...
		for _, ranges := range []int64{100, 100000} {
			for _, protocol := range cdcBenchProtocols {
				...
				r.Add(registry.TestSpec{
					Name: fmt.Sprintf(
						"cdc/scan/%s/nodes=%d/cpu=%d/rows=%s/ranges=%d/protocol=%s/format=%s/sink=null",
						scanType, nodes, cpus, formatSI(rows), ranges, protocol, format),
					Owner:           registry.OwnerCDC,
					Benchmark:       true,
					Cluster:         r.MakeClusterSpec(nodes+1, spec.CPU(cpus)),
					RequiresLicense: true,
					Timeout:         90 * time.Minute, // catchup scan benchmarks can take >1 hour
...
func runCDCBenchWorkload(
	...
	if cdcEnabled {
		...
		require.NoError(t, conn.QueryRowContext(ctx, fmt.Sprintf(
			`CREATE CHANGEFEED FOR kv.kv INTO '%s' WITH `+
				`format = '%s', initial_scan = 'no', min_checkpoint_frequency = '10s'`,
			sink, format)).
			Scan(&jobID))
