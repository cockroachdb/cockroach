...
func registerPsycopg(r registry.Registry) {
	runPsycopg := func(
		...
		//	t.Fatal(err)
		// }
		if err = c.RunE(ctx, option.OnNodes(node), "git clone https://github.com/psycopg/psycopg2.git /mnt/data1/psycopg"); err != nil {
			t.Fatal(err)
		}
		if err = c.RunE(ctx, option.OnNodes(node), fmt.Sprintf("cd /mnt/data1/psycopg/ && git checkout %s", supportedPsycopgTag)); err != nil {
			t.Fatal(err)
		}
