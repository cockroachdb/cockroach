...
func smithcmpTestRun(
	...
	result, err := c.RunWithDetailsSingleNode(ctx, t.L(), firstNode, "echo -n {pgport:1}")
	if err != nil {
		t.Fatal(err)
	}
	if err := c.RunE(ctx, firstNode, fmt.Sprintf(`sed -i "s|26257|%s|g" %s`, result.Stdout, configFile)); err != nil {
		t.Fatal(err)
	}
