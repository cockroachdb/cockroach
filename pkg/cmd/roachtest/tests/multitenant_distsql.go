...
func runMultiTenantDistSQL(
	...
	// Test takes too long and too much resources with default range sizes.
	...
	for {
		...
		if err != nil {
			t.L().Errorf("WARNING: error during explain: %v", err)
			continue
		}
	...
	for rowCount == 0 {
		...
		if err := inst1Conn.QueryRow("SELECT row_count FROM [SHOW STATISTICS FOR TABLE t]").Scan(&rowCount); err != nil {
			t.L().Errorf("WARNING: error during statistics query: ", err)
		}
	}
	...
	if timeoutMillis != 0 {
		...
		// TODO(cucaroach): should we try to open bundle and verify its contents?
	}
}
