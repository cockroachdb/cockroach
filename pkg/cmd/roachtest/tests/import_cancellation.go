...
func runImportCancellation(ctx context.Context, t test.Test, c cluster.Cluster) {
	...
	for tbl := range tablesToNumFiles {
		...
		ttl := randutil.RandIntInRange(rng, 60 /* 1m */, 60*10 /* 10m */)
	...
	// Query the number of RangeKeySet keys from the timeseries metrics.
	rangeKeySets := getRangeKeySetsTotal(ctx, c, t)
	t.L().PrintfCtx(ctx, "Cluster contains %d RangeKeySet keys.", rangeKeySets)
	if rangeKeySets == 0 {
		t.Errorf("expected at least 1 RangeKeySet; none found")
	}
	...
	afterRangeKeySets := getRangeKeySetsTotal(ctx, c, t)
	t.L().PrintfCtx(ctx, "Cluster contains %d RangeKeySet keys.", afterRangeKeySets)
}

func getRangeKeySetsTotal(ctx context.Context, c cluster.Cluster, t test.Test) int {
	...
	response := mustGetMetrics(t, adminURLs[0], start, now, []tsQuery{
		{
			name:      "cr.node.storage.keys.range-key-set.count",
			queryType: total,
		},
	})
	...
	return int(datapoints[len(datapoints)-1].Value)
}
