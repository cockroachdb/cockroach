...
func (cmvt *cdcMixedVersionTester) waitForResolvedTimestamps(
...
func (cmvt *cdcMixedVersionTester) validate(
	...
	// Assert that no restarts takes place during validation because the fingerprint validator relies on a db
	// connection. We expect no restarts because  `InMixedVersion` hooks run synchronously between node restarts.
...
func (cmvt *cdcMixedVersionTester) runWorkload(
	ctx context.Context, l *logger.Logger, r *rand.Rand, h *mixedversion.Helper,
) error {
...
func (cmvt *cdcMixedVersionTester) initWorkload(
	...
	if err := cmvt.c.RunE(ctx, option.NodeListOption{h.RandomNode(r, cmvt.crdbNodes)}, bankInit.String()); err != nil {
		return err
	}
