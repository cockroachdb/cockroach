...
func runLeasePreferences(
	...
	// quicker. Insert 2x the number of ranges, this isn't realistic but keeps
	// the test quick.
	require.NoError(t, WaitForReplication(ctx, t, conn, spec.replFactor))
	c.Run(ctx, c.Node(numNodes), fmt.Sprintf(
		`./cockroach workload init kv --scatter --splits %d --insert-count %d {pgurl:%d}`,
		spec.ranges, 2*spec.ranges, numNodes))
