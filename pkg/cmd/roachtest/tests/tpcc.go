...
func registerTPCC(r registry.Registry) {
	...
	r.Add(registry.TestSpec{
		...
		Run: func(ctx context.Context, t test.Test, c cluster.Cluster) {
			...
			time.Sleep(30 * time.Second)
			t.L().Printf("stopping node 3 to trigger node liveness error")
			require.NoError(t, c.StopE(ctx, t.L(), option.DefaultStopOpts(), c.Node(3)))

			importErr := <-errCh
			if importErr != nil {
				t.L().Printf("import command failed: %s", importErr)
				watcherDB := c.Conn(ctx, t.L(), 1)
			...
			require.NoError(t, c.StartE(ctx, t.L(),
				option.NewStartOpts(option.NoBackupSchedule), settings, c.Node(3)))
		},
	})
