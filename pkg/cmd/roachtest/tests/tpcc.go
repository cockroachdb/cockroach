...
func registerTPCC(r registry.Registry) {
	...
	r.Add(registry.TestSpec{
		Name:      "tpcc/published/multiple-workloadnodes-medium-vanilla",
		Owner:     registry.OwnerKV,
		Benchmark: true,
		Cluster: spec.MakeClusterSpec(
			18,
...
func runTPCCPublished(
	...
	crdbNodeCount := c.Spec().NodeCount - workloadCount
	...
	if !t.SkipInit() {
		...
		t.L().Printf("Step 6 - This step intentionally left blank to match public docs")
		{
			...
			cmd = fmt.Sprintf(
				"./cockroach1 workload run tpcc %s --warehouses=%d  --duration=1m --workers=1000 --wait=0.0 {pgurl%s}",
				extraArgs,
				opts.warehouses,
				crdbNodes,
			)
