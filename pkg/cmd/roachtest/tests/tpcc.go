		...
		m.Go(func(ctx context.Context) error {
			...
			if t.ExportOpenmetrics() {
				...
				labelsMap = map[string]string{
					"warehouses": fmt.Sprintf("%d", opts.Warehouses),
					"ramp":       fmt.Sprintf("%d", rampDur),
					"database":   opts.DB,
			...
			cmd := roachtestutil.NewCommand("%s workload run %s", test.DefaultCockroachPath, opts.getWorkloadCmd()).
				...
				MaybeFlag(t.ExportOpenmetrics(), "openmetrics-labels", clusterstats.GetDefaultOpenmetricsLabelString(t, c, labelsMap)).
				Flag("ramp", rampDur).
...
func runTPCCMixedHeadroom(ctx context.Context, t test.Test, c cluster.Cluster) {
	...
	runTPCCWorkload := func(ctx context.Context, l *logger.Logger, rng *rand.Rand, h *mixedversion.Helper) error {
		...
		histogramsPath := fmt.Sprintf("%s/stats.json", t.PerfArtifactsDir())
		var labelsMap map[string]string
		if t.ExportOpenmetrics() {
			histogramsPath = fmt.Sprintf("%s/stats.om", t.PerfArtifactsDir())
			labelsMap = map[string]string{
				"duration":   fmt.Sprintf("%d", workloadDur/time.Millisecond),
				"ramp":       rampDur.String(),
				"warehouses": fmt.Sprintf("%d", headroomWarehouses),
			}
		}
	...
	if res, err := s.Search(func(warehouses int) (bool, error) {
		...
		for groupIdx, group := range loadGroups {
			...
			m.Go(func(ctx context.Context) error {
				...
				labels := roachtestutil.GetLabelMapFromStruct(b)
				labelsCustom := map[string]string{
					"warehouses": fmt.Sprintf("%d", warehouses),
					"ramp":       fmt.Sprintf("%d", rampDur),
					"duration":   fmt.Sprintf("%d", loadDur),
				}
				maps.Copy(labels, labelsCustom)
				cmd := fmt.Sprintf("./cockroach workload run tpcc --warehouses=%d --active-warehouses=%d "+
					"--tolerate-errors --ramp=%s --duration=%s%s"+roachtestutil.GetWorkloadHistogramArgsString(t, c, labels)+"{pgurl%s%s}",
