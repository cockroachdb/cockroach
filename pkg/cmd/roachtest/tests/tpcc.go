...
func runTPCC(ctx context.Context, t test.Test, c cluster.Cluster, opts tpccOptions) {
	...
	promCfg, cleanupFunc := setupPrometheus(ctx, t, c, opts)
	defer cleanupFunc()
	if promCfg != nil && opts.ChaosEventsProcessor != nil {
	...
	if cfg == nil {
		...
		cfg = &prometheus.Config{
			...
			ScrapeConfigs: []prometheus.ScrapeConfig{
				...
				prometheus.MakeWorkloadScrapeConfig("workload", makeWorkloadScrapeNodes(workloadNode, opts.WorkloadInstances)),
			},
		}
	}
	if c.IsLocal() {
