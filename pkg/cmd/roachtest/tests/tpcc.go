...
func runTPCC(
	...
	if matches, _ := filepath.Glob(filepath.Join(t.ArtifactsDir(),
		"run_*_n*_cockroach-workload-*.log")); matches != nil {
	...
	if err := os.MkdirAll(profilesDir, 0755); err != nil {
		...
		if matches, _ := filepath.Glob(filepath.Join(t.ArtifactsDir(), "run_*_n*_cockroach-workload-*.log")); matches != nil {
			for _, logPath := range matches {
...
func mergeAndExportTPCCProfiles(
	...
	for typ := range mergedProfiles {
		...
		t.L().Printf("successfully exported merged %s profile", typ)
	}
	...
	for i := range numNodes {
		for typ := range profiles {
			...
			t.L().Printf("successfully exported individual %s profile for node %d", typ, i+1)
		}
	}
