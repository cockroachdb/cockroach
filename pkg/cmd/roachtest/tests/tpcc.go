...
func setupTPCC(
	...
	func() {
		...
		case usingImport:
			...
			c.Run(ctx, option.OnNodes(crdbNodes[:1]), tpccImportCmd(opts.Warehouses, opts.ExtraSetupArgs, pgurl))
		case usingInit:
...
func runTPCC(ctx context.Context, t test.Test, c cluster.Cluster, opts tpccOptions) {
	...
	for i := range workloadInstances {
		...
		m.Go(func(ctx context.Context) error {
			...
			return c.RunE(ctx, option.OnNodes(workloadNode), cmd)
		})
	}
	if opts.Chaos != nil {
		chaos := opts.Chaos()
	...
	if !opts.SkipPostRunCheck {
		c.Run(ctx, option.OnNodes(workloadNode), fmt.Sprintf(
...
func runTPCCMixedHeadroom(ctx context.Context, t test.Test, c cluster.Cluster) {
	...
	importTPCC := func(ctx context.Context, l *logger.Logger, rng *rand.Rand, h *mixedversion.Helper) error {
		...
		return c.RunE(ctx, option.OnNodes(randomNode), cmd)
	}
...
func loadTPCCBench(
	...
	if err = c.RunE(ctx, option.OnNodes(roachNodes[:1]), cmd); err != nil {
		return err
	}
...
func runTPCCBench(ctx context.Context, t test.Test, c cluster.Cluster, b tpccBenchSpec) {
	...
	const restartWait = 15 * time.Second
	{
		...
		if useHAProxy {
			...
			c.Run(ctx, option.OnNodes(loadNodes), "./cockroach gen haproxy --insecure --url {pgurl:1}")
			...
			c.Run(ctx, option.OnNodes(loadNodes), "sed -i 's/maxconn [0-9]\\+/maxconn 21000/' haproxy.cfg")
			if b.LoadWarehouses(c.Cloud()) > 1e4 {
				t.Fatal("HAProxy config supports up to 10k warehouses")
			}
			c.Run(ctx, option.OnNodes(loadNodes), "haproxy -f haproxy.cfg -D")
		}
	...
	if res, err := s.Search(func(warehouses int) (bool, error) {
		...
		for groupIdx, group := range loadGroups {
			...
			m.Go(func(ctx context.Context) error {
				...
				err := c.RunE(ctx, option.OnNodes(group.loadNodes), cmd)
