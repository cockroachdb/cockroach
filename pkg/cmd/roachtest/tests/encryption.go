...
func registerEncryption(r registry.Registry) {
	...
	runEncryption := func(ctx context.Context, t test.Test, c cluster.Cluster) {
		...
		for _, addr := range adminAddrs {
			if err := c.RunE(ctx, option.OnNodes(c.Node(nodes)), fmt.Sprintf(`curl http://%s/_status/stores/local | (! grep '"encryptionStatus": null')`, addr)); err != nil {
		...
		testCLIGenKey := func(size int) error {
			...
			if err := c.RunE(ctx, option.OnNodes(c.Node(nodes)), fmt.Sprintf("./cockroach gen encryption-key -s=%[1]d aes-%[1]d.key", size)); err != nil {
			...
			if err := c.RunE(ctx, option.OnNodes(c.Node(nodes)), fmt.Sprintf(`size=$(wc -c <"aes-%d.key"); test $size -eq %d && exit 0 || exit 1`, size, 32+size/8)); err != nil {
