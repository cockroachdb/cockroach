	...
	runEncryptionRotation := func(ctx context.Context, t test.Test, c cluster.Cluster) {
		...
		for _, size := range []int{128, 192, 256} {
			for _, version := range []int{1, 2} {
				...
				if err := c.RunE(ctx, option.WithNodes(c.Node(nodes)), fmt.Sprintf("./cockroach gen encryption-key -s %d --version=%d %s",
		...
		for keyIdx := 1; keyIdx < len(keys); keyIdx++ {
			...
			opts.RoachprodOpts.ExtraArgs = []string{fmt.Sprintf("--enterprise-encryption=path=data,key=%s,old-key=%s", keys[keyIdx], keys[keyIdx-1])}
