...
func runCDCSessionFiltering(ctx context.Context, t test.Test, c cluster.Cluster) {
	...
	require.NoError(t, conn.QueryRowContext(ctx, fmt.Sprintf(
		`CREATE CHANGEFEED FOR TABLE events INTO 'nodelocal://1/events' WITH updated, resolved, min_checkpoint_frequency='1s'`)).
		Scan(&jobID))
...
INSERT INTO events VALUES ('C', 1);`)
	...
	execWithChangefeedReplication := func(conn *sql.DB, r replicate, stmts []string) {
	...
	for i, e := range events {
		...
		sb.WriteString(fmt.Sprintf("%s@%d", e.After.ID, e.After.Revision))
	}

	// We expect to see the following sequence:
	want := "A@1, B@1, C@1, B@2, C@2, C@3"
