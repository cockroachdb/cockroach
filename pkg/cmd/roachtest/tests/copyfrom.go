...
func initTest(ctx context.Context, t test.Test, c cluster.Cluster, sf int) {
	...
	c.Run(ctx, option.OnNodes(c.Node(1)), "rm -f /tmp/lineitem-table.csv")
	c.Run(ctx, option.OnNodes(c.Node(1)), fmt.Sprintf("curl '%s' -o /tmp/lineitem-table.csv", csv))
}
...
func runTest(ctx context.Context, t test.Test, c cluster.Cluster, pg string) {
	...
	c.Run(ctx, option.OnNodes(c.Node(1)), "mkdir", t.PerfArtifactsDir())
	cmd := fmt.Sprintf(
...
func runCopyFromPG(ctx context.Context, t test.Test, c cluster.Cluster, sf int) {
	initTest(ctx, t, c, sf)
	c.Run(ctx, option.OnNodes(c.Node(1)), "sudo -i -u postgres psql -c 'DROP TABLE IF EXISTS lineitem'")
	c.Run(ctx, option.OnNodes(c.Node(1)), fmt.Sprintf("sudo -i -u postgres psql -c '%s'", lineitemSchema))
...
func runCopyFromCRDB(ctx context.Context, t test.Test, c cluster.Cluster, sf int, atomic bool) {
	...
	m.Go(func(ctx context.Context) error {
		...
		urlstr = u.String()
		c.Run(ctx, option.OnNodes(c.Node(1)), fmt.Sprintf("psql %s -c 'SELECT 1'", urlstr))
		c.Run(ctx, option.OnNodes(c.Node(1)), fmt.Sprintf("psql %s -c '%s'", urlstr, lineitemSchema))
