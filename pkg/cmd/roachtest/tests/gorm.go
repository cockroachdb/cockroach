...
func registerGORM(r registry.Registry) {
	runGORM := func(ctx context.Context, t test.Test, c cluster.Cluster) {
		...
		if err := c.RunE(ctx, option.OnNodes(node), fmt.Sprintf("mkdir -p %s", resultsDir)); err != nil {
			t.Fatal(err)
		}
		...
		err = c.RunE(ctx, option.OnNodes(node), fmt.Sprintf(`./cockroach sql -e "CREATE DATABASE gorm" --insecure --url=%s`, pgurl))
