...
// variations specifies the different variations to use.
...
// T5: End the test.
// The latency from T2-T3 is the baseline.
// The latency is measured from T3-T4 and T4-T5 and compared to the baseline.
...
var maxBlockBytes = []int{1, 1000, 5000}
...
func (v variations) String() string {
	return fmt.Sprintf("seed: %d, fill: %s, validation: %s, perturbation: %s, splits: %d, stable: %d, target: %d, workload: %d, crdb: %d,  lease: %s, udp: %s:%d, nodes: %d, vcpu: %d, disks: %d",
		v.seed,
		v.fillDuration,
...
func setupMetamorphic(p perturbation) variations {
	// TODO: Allow specifying the seed to reproduce a metamorphic run.
...
func setupDev(p perturbation) variations {
	...
	v.perturbationDuration = 20 * time.Second
	v.ratioOfMax = 0.5
	v.perturbation = p
	return v
}
...
func finishSetup(c cluster.Cluster, v *variations, port int) {
	...
	// TODO: Specify how many nodes we are partitioned from. We need to set up the localities correctly for this to work.
	if c.IsLocal() {
...
func registerLatencyTests(r registry.Registry) {
	addTest(r, "hitless/metamorphic/restart", setupMetamorphic(restart{}))
	addTest(r, "hitless/metamorphic/partition", setupMetamorphic(partition{}))
	addTest(r, "hitless/metamorphic/add-node", setupMetamorphic(addNode{}))
	addTest(r, "hitless/metamorphic/decommission", setupMetamorphic(decommission{}))

	addTest(r, "hitless/full/restart", setupFull(restart{}))
	addTest(r, "hitless/full/partition", setupFull(partition{}))
	addTest(r, "hitless/full/add-node", setupFull(addNode{}))
	addTest(r, "hitless/full/decommission", setupFull(decommission{}))

	addTest(r, "hitless/dev/restart", setupDev(restart{}))
	addTest(r, "hitless/dev/partition", setupDev(partition{}))
	addTest(r, "hitless/dev/add-node", setupDev(addNode{}))
	addTest(r, "hitless/dev/decommission", setupDev(decommission{}))
}

func addTest(r registry.Registry, testName string, v variations) {
	// TODO: Make num worker nodes be a factor of numNodes.
	numWorkers := 1
	r.Add(registry.TestSpec{
		...
		Cluster:          r.MakeClusterSpec(v.numNodes+numWorkers, spec.CPU(v.vcpu), spec.SSD(v.disks)),
		Leases:           v.leaseType,
		Run:              v.runTest,
	})
}
...
func (a addNode) startPerturbation(
	...
	startTime := timeutil.Now()
	v.startNoBackup(ctx, l, v.cluster.Node(v.targetNode))
	// Wait out the time until the store is no longer suspect.
...
func (d decommission) startPerturbation(
	...
	v.cluster.Run(ctx, option.WithNodes(v.cluster.Node(v.targetNode)), fmt.Sprintf("./cockroach node drain --self --certs-dir=%s --port={pgport:%d}", install.CockroachNodeCertsDir, v.targetNode))
	// Wait for all the other nodes to see the drain.
	time.Sleep(10 * time.Second)
	l.Printf("decommissioning node")
	v.cluster.Run(ctx, option.WithNodes(v.cluster.Node(v.targetNode)), fmt.Sprintf("./cockroach node decommission --self --certs-dir=%s --port={pgport:%d}", install.CockroachNodeCertsDir, v.targetNode))
	return timeutil.Since(startTime)
}
...
// TODO: Consider better ways to do this comparison.
...
// TODO: parameterize the workload to allow running TPCC.
