	...
	return registry.TestSpec{
		...
		Run: func(ctx context.Context, t test.Test, c cluster.Cluster) {
			...
			m.Go(func(ctx context.Context) error {
				...
				// Disable elastic control if requested. Default is on.
				if !enableElasticCPUControl {
					if _, err := db.Exec("SET CLUSTER SETTING kvadmission.elastic_control_bulk_low_priority.enabled = false"); err != nil {
						t.Fatal(err)
					}
				}
