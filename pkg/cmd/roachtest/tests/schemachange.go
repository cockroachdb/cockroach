...
func registerSchemaChangeDuringKV(r registry.Registry) {
	r.Add(registry.TestSpec{
		...
		Run: func(ctx context.Context, t test.Test, c cluster.Cluster) {
			...
			c.Run(ctx, option.OnNodes(c.Node(1)), `./workload init kv --drop --db=test`, pgurl)
			for node := 1; node <= c.Spec().NodeCount; node++ {
				...
				go func() {
					...
					defer l.Close()
					_ = c.RunE(ctx, option.OnNodes(c.Node(node)), fmt.Sprintf(cmd, c.Nodes(node)))
				}()
			}
...
func makeSchemaChangeBulkIngestTest(
	...
	return registry.TestSpec{
		...
		Run: func(ctx context.Context, t test.Test, c cluster.Cluster) {
			...
			c.Run(ctx, option.OnNodes(workloadNode), cmdWrite)
			...
			m.Go(func(ctx context.Context) error {
				c.Run(ctx, option.OnNodes(workloadNode), cmdWriteAndRead)
				return nil
			})
