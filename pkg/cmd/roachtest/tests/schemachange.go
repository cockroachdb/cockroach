...
const (
	...
	// pdStandardDiskType uses pd-standard persistent disks (slowest, HDD).
...
func makeSchemaChangeBulkIngestScaleTest(
	...
	// Configure disk type
	switch disk {
	case pdBalancedDiskType:
		clusterOpts = append(clusterOpts, spec.VolumeType("pd-balanced"))
	case pdStandardDiskType:
		clusterOpts = append(clusterOpts, spec.VolumeType("pd-standard"))
	case pdSsdDiskType:
		fallthrough
	default:
		clusterOpts = append(clusterOpts, spec.VolumeType("pd-ssd"))
	}
