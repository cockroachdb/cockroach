...
func registerAsim(r registry.Registry) {
	for _, asimSpec := range []asimSpec{
		...
		},
		{
			...
			nodes:    7,
			duration: 60 * time.Minute,
			load:     asimWorkloadSpec{readRatio: 95, rate: 30000, minBlockSize: 128, maxBlockSize: 128},
			settings: *config.DefaultSimulationSettings(),
		},
		{
			desc:     "kv0",
			nodes:    7,
			duration: 60 * time.Minute,
			load:     asimWorkloadSpec{readRatio: 0, rate: 30000, minBlockSize: 128, maxBlockSize: 128},
			settings: *config.DefaultSimulationSettings(),
		},
		{
			desc:     "kv95/zipf",
	...
	for tick := 0; tick < ticks; tick++ {
		...
		for store := 0; store < stores; store++ {
			sm := metrics.StoreMetrics{
				Tick:               time.Unix(0, tickArray[tick]),
