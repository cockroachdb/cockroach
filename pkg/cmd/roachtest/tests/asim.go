	...
	for _, asimSpec := range []asimSpec{
		{
			...
			load:     asimWorkloadSpec{readRatio: 95, rate: 6000, minBlockSize: 128, maxBlockSize: 128},
		...
		},
		{
			...
			nodes:    3,
			duration: 60 * time.Minute,
			load:     asimWorkloadSpec{readRatio: 0, rate: 6000, minBlockSize: 128, maxBlockSize: 128},
			settings: *config.DefaultSimulationSettings(),
		},
		{
			desc:     "kv95/zipf",
		...
		},
		{
			...
			nodes:    7,
			duration: 60 * time.Minute,
			load:     asimWorkloadSpec{readRatio: 95, rate: 13500, minBlockSize: 128, maxBlockSize: 128},
			settings: *config.DefaultSimulationSettings(),
		},
		{
			desc:     "kv0",
			nodes:    7,
			duration: 60 * time.Minute,
			load:     asimWorkloadSpec{readRatio: 0, rate: 13500, minBlockSize: 128, maxBlockSize: 128},
			settings: *config.DefaultSimulationSettings(),
		},
		{
			desc:     "kv95/zipf",
...
func runSimCluster(ctx context.Context, t test.Test, c cluster.Cluster, spec asimSpec) error {
	...
	// NB: We always add 30 ranges to represent the system ranges.
