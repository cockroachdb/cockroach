...
func registerDeclarativeSchemaChangerJobCompatibilityInMixedVersion(r registry.Registry) {
	...
	r.Add(registry.TestSpec{
		...
		Run: func(ctx context.Context, t test.Test, c cluster.Cluster) {
			...
			currentVersion := "22.2.6"
			currentV, err := version.Parse("v" + currentVersion)
			require.NoError(t, err)
			predV, err := version.PredecessorVersion(*currentV)
			require.NoError(t, err)
			...
			u := newVersionUpgradeTest(c,
				...
				// Upgrade some nodes.
				binaryUpgradeStep(upgradedNodes, currentVersion),
