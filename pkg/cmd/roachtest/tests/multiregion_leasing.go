...
func runSchemaChangeMultiRegionBenchmarkLeasing(
	...
	sort.SliceStable(durations[0], func(i, j int) bool {
		return durations[0][i] < durations[0][j]
	})
	sort.SliceStable(durations[1], func(i, j int) bool {
		return durations[1][i] < durations[1][j]
	})
	// Confirm that session based leasing took less time for the selects.
	mid := len(durations[0]) / 2
	if durations[0][mid] > durations[1][mid] {
	...
	improvementPct := ((durations[1][mid] * 100) / durations[0][mid]) - 100
