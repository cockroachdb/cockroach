...
// queue. It does so in several scenarios:
//
...
func runChangeReplicasMixedVersion(ctx context.Context, t test.Test, c cluster.Cluster) {
	...
	scanTableStep := func(table string, expectRows int) versionStep {
		return func(ctx context.Context, t test.Test, u *versionUpgradeTest) {
			...
			t.Status(fmt.Sprintf("scanning table %s via gateway n%d", table, gateway))
	...
	scatterTableStep := func(table string) versionStep {
		return func(ctx context.Context, t test.Test, u *versionUpgradeTest) {
			gateway := randomNodeID()
			t.Status(fmt.Sprintf("scattering table %s via n%d", table, gateway))
			conn := u.c.Conn(ctx, t.L(), gateway)
			defer conn.Close()
			...
			for _, target := range targets {
				...
				for attempt := 1; attempt <= 5; attempt++ {
					t.Status(fmt.Sprintf(
						"moving replicas from n%d to n%d via gateway n%d using ALTER TABLE RELOCATE (attempt %d)",
						nodeID, target, gateway, attempt))
					...
					t.Status(fmt.Sprintf("%d ranges failed", len(rangeErrors)))
				}

				if len(rangeErrors) > 0 {
					for rangeID, result := range rangeErrors {
						t.Status(fmt.Sprintf("failed to move r%d from n%d to n%d via n%d: %s",
	...
	changeReplicasZoneConfigFromNodeStep := func(table string, nodeID int) versionStep {
		return func(ctx context.Context, t test.Test, u *versionUpgradeTest) {
			...
			t.Status(fmt.Sprintf("moving replicas off of n%d using a zone config via gateway n%d",
				nodeID, gateway))
			...
			require.Eventually(t, func() bool {
				...
				for n, c := range conns {
					...
					if err != nil {
						t.Status(fmt.Sprintf("kv_enqueue_replica failed: %s", err))
					}
				}
				...
				t.Status(fmt.Sprintf("table %s has %d replicas on n%d", table, rangeCount, nodeID))
				return rangeCount == 0
	...
	u := newVersionUpgradeTest(c,
		// Start the cluster with preVersion and wait for it to bootstrap.
		// Then disable auto-upgrades to mainVersion.
		uploadAndStartSequential(c.All(), preVersion),
		...
		func(ctx context.Context, t test.Test, u *versionUpgradeTest) {
			const ranges = 100
			gateway := randomNodeID()
			t.Status(fmt.Sprintf("splitting table test into %d ranges via gateway n%d", ranges, gateway))
