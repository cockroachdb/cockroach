...
func runChangeReplicasMixedVersion(ctx context.Context, t test.Test, c cluster.Cluster) {
	...
	createTable := func(
		...
		if _, err := db.ExecContext(ctx, `CREATE TABLE test (id INT PRIMARY KEY)`); err != nil {
			return err
		}
		...
		if _, err := db.ExecContext(ctx, `ALTER TABLE test SCATTER`); err != nil {
			return err
		}
		return nil
	}
	...
	evacuateNodeUsingZoneConfig := func(
		...
		_, err = db.ExecContext(ctx, `ALTER TABLE test CONFIGURE ZONE USING constraints = '[]'`)
		if err != nil {
			return err
		}
	...
	evacuateNodeUsingRelocate := func(
		...
		for _, target := range c.All() {
			...
			for i := 0; i < 30; i++ {
				...
				// Relocate ranges.
				rows, err := db.QueryContext(ctx, `ALTER RANGE RELOCATE FROM $1::int TO $2::int FOR `+
	...
	moveReplicas := func(
		...
		gwNode, db := h.RandomDB(r, c.All())
		l.Printf("using gateway n%d", gwNode)
		...
		for _, node := range nodes {
			if r.Float64() < 0.5 {
				evacuateNodeUsingZoneConfig(ctx, l, r, h, db, node)
			} else {
				evacuateNodeUsingRelocate(ctx, l, r, h, db, node)
			}
		}
