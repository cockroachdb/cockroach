...
func registerDraining(r registry.Registry) {
	{
		...
		r.Add(registry.TestSpec{
			Name:    fmt.Sprintf("draining/early-exit-query-wait"),
			Owner:   registry.OwnerSQLExperience,
			Cluster: r.MakeClusterSpec(numNodes),
			Run: func(ctx context.Context, t test.Test, c cluster.Cluster) {
...
func testEarlyExitInConnectionWait(
	...
	t.Status(
		fmt.Sprintf("readiness endpoint's stdout after draining starts: %s",
			errorMessage,
		),
	)
	...
	if err != nil {
		t.Fatal(errors.Wrapf(err, "%ds after drain starts (at drain_wait), "+
			"cannot get a new SQL connection from the connection pool", pokeHealthBeforeConnDrainTimestamp))
	}
	...
	if err != nil {
		t.Status(fmt.Sprintf("%ds after draining starts, server is rejecting "+
			"new SQL connections: %v", pokeHealthAfterConnDrainTimestamp, err))
	} else {
...
// getNodeHealth is to probe the health\?ready\=1 endpoint of the node,
// parse the json output to a map variable, and return the map or possible
// errors.When the node is in "ready" status, the returned map should be empty.
func getNodeHealth(
	...
	if err != nil {
		return nil, errors.New("cannot prob the health\\?ready\\=1 endpoint")
	}
