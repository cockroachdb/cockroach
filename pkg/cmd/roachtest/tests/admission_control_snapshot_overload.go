		...
		Run: func(ctx context.Context, t test.Test, c cluster.Cluster) {
			...
			t.Status(fmt.Sprintf("setting up prometheus/grafana (<%s)", 2*time.Minute))
			{
				...
				promCfg.ScrapeConfigs = append(promCfg.ScrapeConfigs, prometheus.MakeWorkloadScrapeConfig("workload",
					"/", makeWorkloadScrapeNodes(c.WorkloadNodes().InstallNodes()[0], []workloadInstance{
						{nodes: c.WorkloadNodes()},
					})))
			...
			if !t.SkipInit() {
				...
				c.Run(ctx, option.WithNodes(c.WorkloadNodes()), "./cockroach workload init kv "+splits+" {pgurl:1}")
			...
			if !t.SkipInit() {
				...
				c.Run(ctx, option.WithNodes(c.WorkloadNodes()), "./cockroach workload fixtures import tpcc --checks=false"+warehouses+" {pgurl:1}")
			}
