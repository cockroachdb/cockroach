	...
	r.Add(registry.TestSpec{
		...
		Run: func(ctx context.Context, t test.Test, c cluster.Cluster) {
			...
			if !t.SkipInit() {
				...
				c.Run(ctx, option.OnNodes(c.Node(workloadNode)), "./cockroach workload init kv "+splits+" {pgurl:1}")
			...
			if !t.SkipInit() {
				...
				c.Run(ctx, option.OnNodes(c.Node(workloadNode)), "./cockroach workload fixtures import tpcc --checks=false"+warehouses+" {pgurl:1}")
			}
			...
			m.Go(func(ctx context.Context) error {
				...
				splits := ifLocal(c, "  --splits=10", " --splits=100")
				c.Run(ctx, option.OnNodes(c.Node(crdbNodes+1)),
