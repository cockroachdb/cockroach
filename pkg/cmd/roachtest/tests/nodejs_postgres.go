...
func registerNodeJSPostgres(r registry.Registry) {
	runNodeJSPostgres := func(
		...
		err = c.RepeatRunE(ctx, t, node, "create sql user",
			fmt.Sprintf(
				`./cockroach sql --certs-dir certs -e "CREATE USER %s CREATEDB"`, user,
			))
		require.NoError(t, err)

		err = c.RepeatRunE(ctx, t, node, "create test database",
		...
		err = c.RepeatRunE(
			ctx, t, node, "install yarn", `sudo npm i -g yarn`,
		)
		require.NoError(t, err)

		err = c.RepeatRunE(
			ctx, t, node, "install lerna", `sudo npm i --g lerna`,
		)
		require.NoError(t, err)

		err = c.RepeatRunE(
			ctx, t, node, "remove old node-postgres", `sudo rm -rf /mnt/data1/node-postgres`,
		)
		require.NoError(t, err)
		...
		err = c.RepeatRunE(
			ctx, t, node, "configure git to avoid unauthenticated protocol",
