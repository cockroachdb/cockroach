...
func runSchemaChangeMixedVersions(
	...
	numFeatureRuns := 0
	// We set AlwaysUseLatestPredecessors and NumUpgrades(1) because the workload does not handle working more than 1 back from
	// the current version very well.
	// TODO(annie): In order to imitate the workflow of our users performing upgrades better, we should stage different workload
	// binaries based on the upgrade being performed (this will also help us catch bugs that might not manifest in the [1 step before current
	// -> upgrade to current] workflow).
	mvt := mixedversion.NewTest(ctx, t, t.L(), c, c.All(), mixedversion.AlwaysUseLatestPredecessors, mixedversion.NumUpgrades(1))
