...
func registerLiquibase(r registry.Registry) {
	runLiquibase := func(
		...
		if err = c.RunE(ctx, option.OnNodes(node), "cd /mnt/data1/ && git clone https://github.com/liquibase/liquibase-test-harness.git"); err != nil {
			t.Fatal(err)
		}
		if err = c.RunE(ctx, option.OnNodes(node), fmt.Sprintf("cd /mnt/data1/liquibase-test-harness/ && git checkout %s",
		...
		if err = c.RunE(ctx, option.OnNodes(node), `sudo mkdir /cockroach && sudo ln -sf /home/ubuntu/cockroach /cockroach/cockroach.sh`); err != nil {
			t.Fatal(err)
		}
		if err = c.RunE(ctx, option.OnNodes(node), `/mnt/data1/liquibase-test-harness/src/test/resources/docker/setup_db.sh localhost`); err != nil {
			t.Fatal(err)
		}
		...
		err = c.RunE(ctx, option.OnNodes(node), cmd)
		if err != nil {
