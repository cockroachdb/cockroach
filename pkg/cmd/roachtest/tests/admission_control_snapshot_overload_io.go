	...
	r.Add(registry.TestSpec{
		...
		Run: func(ctx context.Context, t test.Test, c cluster.Cluster) {
			...
			m.Go(func(ctx context.Context) error {
				c.Run(ctx, option.WithNodes(c.Node(workloadNode)),
					fmt.Sprintf("./cockroach workload run kv --tolerate-errors --splits=1000 --histograms=%s/stats.json --read-percent=50 --max-block-bytes=4096 --min-block-bytes=4096 --concurrency=1024 {pgurl:1}",
						t.PerfArtifactsDir()))
				return nil
			})
