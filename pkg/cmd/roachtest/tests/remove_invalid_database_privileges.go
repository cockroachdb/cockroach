...
func showDefaultPrivileges(dbName string, expectedDefaultPrivileges [][]string) versionStep {
	return func(ctx context.Context, t test.Test, u *versionUpgradeTest) {
		...
		require.NoError(t, err)
		rows, err := conn.Query(fmt.Sprintf("SELECT object_type, grantee, privilege_type FROM [SHOW DEFAULT PRIVILEGES FOR ALL ROLES]"))
		require.NoError(t, err)
		...
		for rows.Next() {
			...
			rows.Scan(&objectType, &grantee, &privilegeType)
