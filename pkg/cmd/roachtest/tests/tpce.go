...
func runTPCE(ctx context.Context, t test.Test, c cluster.Cluster, opts tpceOptions) {
	...
	m.Go(func(ctx context.Context) error {
		...
		if opts.exportMetrics {
			return exportTPCEResults(t, c, opts, result.Stdout)
		}
		return nil
	})
	if opts.during != nil {
...
func exportTPCEResults(t test.Test, c cluster.Cluster, opts tpceOptions, result string) error {
	...
	for s.Scan() {
		...
		if t.ExportOpenmetrics() {
			labels := roachtestutil.GetLabelMapFromStruct(opts)
			labels["workload"] = "tpce"
			labelString := clusterstats.GetDefaultOpenmetricsLabelString(t, c, labels)
