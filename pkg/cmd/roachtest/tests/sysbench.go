...
func exportSysbenchResults(t test.Test, result string, start time.Time) error {
	...
	for s.Scan() {
		...
		if len(fields) != 15 {
			return errors.New("metrics output in unexpected format")
		}
		...
		if err != nil {
			return errors.New("error marshaling metrics")
		}
	...
	// Just write for one node, since the metrics we get are aggregated amongst the cluster.
