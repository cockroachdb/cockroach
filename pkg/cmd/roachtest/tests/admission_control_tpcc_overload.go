...
func (s tpccOLAPSpec) run(ctx context.Context, t test.Test, c cluster.Cluster) {
	...
	c.Run(ctx, option.OnNodes(workloadNode), "echo", queryLine, "> "+queryFileName)
	t.Status("waiting")
	...
	m.Go(func(ctx context.Context) error {
		...
		c.Run(ctx, option.OnNodes(workloadNode), cmd)
		return nil
	})
	m.Wait()
...
func registerTPCCSevereOverload(r registry.Registry) {
	r.Add(registry.TestSpec{
		...
		Run: func(ctx context.Context, t test.Test, c cluster.Cluster) {
			...
			c.Run(ctx, option.OnNodes(c.Node(workloadNode)), "./cockroach workload fixtures import tpcc --checks=false --warehouses=10000 {pgurl:1}")
			...
			c.Run(ctx, option.OnNodes(c.Node(workloadNode)), "./cockroach workload run tpcc --ramp=6h --tolerate-errors --warehouses=10000 '{pgurl:1-6}'")
		},
	})
}
