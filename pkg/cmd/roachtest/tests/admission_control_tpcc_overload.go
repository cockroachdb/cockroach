...
func (s tpccOLAPSpec) run(ctx context.Context, t test.Test, c cluster.Cluster) {
	...
	c.Run(ctx, option.WithNodes(c.WorkloadNodes()), "echo", queryLine, "> "+queryFileName)
	t.Status("waiting")
...
func registerTPCCSevereOverload(r registry.Registry) {
	r.Add(registry.TestSpec{
		...
		Owner:            registry.OwnerAdmissionControl,
		Benchmark:        true,
		Cluster:          r.MakeClusterSpec(7, spec.CPU(8), spec.WorkloadNodes(1), spec.WorkloadNodeCPU(8)),
