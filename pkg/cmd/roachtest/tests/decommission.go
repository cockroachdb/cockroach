...
func runDrainAndDecommission(
	...
	c.Run(ctx, option.OnNodes(c.Node(pinnedNode)),
	...
	m.Go(
		func() error {
			return c.RunE(ctx, option.OnNodes(c.Node(pinnedNode)),
	...
	for nodeID := nodes - 2; nodeID <= nodes; nodeID++ {
		...
		m.Go(func() error {
			drain := func(id int) error {
				...
				return c.RunE(ctx, option.OnNodes(c.Node(id)), fmt.Sprintf("./cockroach node drain --insecure --url=%s", pgurl))
			}
			return drain(id)
		})
	}
	...
	m.Go(func() error {
		...
		decom := func(id int) error {
			...
			return c.RunE(ctx, option.OnNodes(c.Node(id)), fmt.Sprintf("./cockroach node decommission --self --insecure --url=%s", pgurl))
		}
		return decom(id)
	})
...
func runDecommission(
	...
	for _, cmd := range workloads {
		...
		m.Go(func() error {
			return c.RunE(ctx, option.OnNodes(c.Node(pinnedNode)), cmd)
		})
	}
	...
	m.Go(func() error {
		...
		for timeutil.Since(tBegin) <= duration {
			...
			if err := c.RunE(ctx, option.OnNodes(c.Node(node)), "rm -rf {store-dir}"); err != nil {
				return err
			}
