...
func runDrainAndDecommission(
	...
	run(fmt.Sprintf(`SET CLUSTER SETTING kv.snapshot_rebalance.max_rate='2GiB'`))
	run(fmt.Sprintf(`SET CLUSTER SETTING kv.snapshot_recovery.max_rate='2GiB'`))

	t.Status(fmt.Sprintf("waiting for initial up-replication"))
	db := c.Conn(ctx, pinnedNode)
	defer func() {
		_ = db.Close()
	}()
	...
	for nodeID := nodes - 2; nodeID <= nodes; nodeID++ {
		...
		m.Go(func() error {
			drain := func(id int) error {
				...
				return c.RunL(ctx, t.L(), c.Node(id), fmt.Sprintf("./cockroach node drain --insecure"))
			}
			return drain(id)
		})
	}
	...
	m.Go(func() error {
		decom := func(id int) error {
			...
			return c.RunL(ctx, t.L(), c.Node(id), fmt.Sprintf("./cockroach node decommission --self --insecure"))
		}
		return decom(id)
	})
