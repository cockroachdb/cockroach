...
func runDecommission(
	...
	m.Go(func() error {
		...
		for timeutil.Since(tBegin) <= duration {
			...
			if _, err := h.decommission(ctx, c.Node(nodeID), pinnedNode, "--wait=all"); err != nil {
				return err
			}
...
// unPinRangesOnNode removes the constraint forcing all default ranges to be
// replicated to the target node (allowing the node to be decommissioned).
func (h *decommTestHelper) unpinRangesOnNode(ctx context.Context, target, runNode int) error {
	return h.setPinnedRangesOnNode(ctx, target, runNode, true /* unpin */)
}

// setPinnedRangesOnNode applies (or removes) constraints on the default zone
// config to ensure that all default ranges are replicated to the target node.
func (h *decommTestHelper) setPinnedRangesOnNode(
	...
	stmt := fmt.Sprintf(`ALTER RANGE default CONFIGURE ZONE = 'constraints: {"%snode%d"}'`, pin, target)
	h.t.Status(stmt)
	_, err := db.ExecContext(ctx, stmt)
	return err
}
