...
func runDBConsoleMixedVersion(ctx context.Context, t test.Test, c cluster.Cluster) {
	...
	mvt.InMixedVersion(
		"test db console endpoints", func(ctx context.Context, l *logger.Logger, rng *rand.Rand,
			...
			ids, err := initializeSchemaAndIDs(ctx, c, l, h.VersionedCockroachPath(t))
			if err != nil {
				t.Fatal(err)
			}
	...
	for _, ep := range endpoints {
		...
		if ep.hasNodeID() {
			for nodeID := 1; nodeID <= len(idMap); nodeID++ {
				...
				// Test with "local".
				if err := testEndpoint(ctx, client, baseURL, ep, "local", ids, ep.getVerifyResponse(), l, multiVersionTest); err != nil {
					return errors.Wrapf(err, "failed testing endpoint %s with local", ep.URL)
				}
				// Test with own node ID.
				if err := testEndpoint(ctx, client, baseURL, ep, idMap[nodeID].String(), ids, ep.getVerifyResponse(), l, multiVersionTest); err != nil {
				...
				if err := testEndpoint(ctx, client, baseURL, ep, idMap[otherNodeID].String(), ids, ep.getVerifyResponse(), l, multiVersionTest); err != nil {
			...
			for nodeID := 1; nodeID <= len(idMap); nodeID++ {
				if err := testEndpoint(ctx, client, urlMap[nodeID], ep, "", ids, ep.getVerifyResponse(), l, multiVersionTest); err != nil {
