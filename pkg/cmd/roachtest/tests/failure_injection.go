...
func runNetworkPartitionExample(ctx context.Context, t test.Test, c cluster.Cluster) {
	...
	// Wait for the failure to propagate (cluster to stabilize).
...
func runDiskStallExample(ctx context.Context, t test.Test, c cluster.Cluster) {
	...
	failer, args, err := roachtestutil.MakeDmsetupDiskStallFailer(t.L(), c, stalledNode)
	if err != nil {
		t.Fatal(err)
	}
	...
	// For disk stalls with StallWrites=true, this waits for the node to become
	// unavailable (disk stall detection kills the node).
	if err := failer.WaitForFailureToPropagate(ctx, failerLogger); err != nil {
		t.Fatal(err)
	}
	...
	// Wait for recovery (node restarts, replicas rebalance).
	if err := failer.WaitForFailureToRecover(ctx, failerLogger); err != nil {
		t.Fatal(err)
	}
