...
var bidirectionalNetworkPartitionTest = failureSmokeTest{
	...
	validateFailure: func(ctx context.Context, l *logger.Logger, c cluster.Cluster) error {
		blocked, err := roachtestutil.CheckPortBlocked(ctx, l, c, c.Nodes(1), c.Nodes(3), "{pgport:3}")
		if err != nil {
			return err
		}
		if !blocked {
		...
		blocked, err = roachtestutil.CheckPortBlocked(ctx, l, c, c.Nodes(1), c.Nodes(2), "{pgport:2}")
		if err != nil {
			return err
		}
		if blocked {
...
var asymmetricNetworkPartitionTest = failureSmokeTest{
	...
	validateFailure: func(ctx context.Context, l *logger.Logger, c cluster.Cluster) error {
		...
		blocked, err = roachtestutil.CheckPortBlocked(ctx, l, c, c.Nodes(3), c.Nodes(1), "{pgport:1}")
		if err != nil {
			return err
		}
		if !blocked {
