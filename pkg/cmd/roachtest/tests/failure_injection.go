...
var cgroupsDiskStallTests = func(c cluster.Cluster) []failureSmokeTest {
	...
	assertRWBytes := func(ctx context.Context, l *logger.Logger, bytes rwBytes, readsStalled, writesStalled bool) error {
		...
		// sometimes see some throughput. The threshold of one page size is picked as cgroups
		// will very slowly ramp up the limit up to one page size, so we may see one page size
		// worth of reads/writes, but more would be unexpected for the given period of 30 seconds.
		threshold := 1 << 12
		...
		// Same assertions as above but for reads instead. Note that if
		// writes are stalled, the node should be dead which means no
		// reads as well.
		if readsStalled || writesStalled {
		...
		if bytes.unaffectedRead < threshold {
