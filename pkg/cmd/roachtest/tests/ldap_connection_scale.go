...
func runLDAPConnectionScaleTest(ctx context.Context, t test.Test, c cluster.Cluster, numNodes int) {
	...
	// Do NOT pre-create SQL roles — auto-provisioning will create them on
	// first LDAP login when security.provisioning.ldap.enabled is true.
...
func validateAutoProvisioningScale(
	...
	var count int
	err := conn.QueryRow(ctx,
		"SELECT count(*) FROM system.role_options WHERE option = 'PROVISIONSRC'",
	).Scan(&count)
	require.NoError(t, err)
