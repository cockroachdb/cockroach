...
func runIndexBackfill(
	...
	if !metricsStart.IsZero() && !metricsEnd.IsZero() {
		exportingStats, err := statCollector.Exporter().Export(
			...
			func(stats map[string]clusterstats.StatSummary) (string, float64) {
				return "primary_key_change_duration", pkChangeDuration.Seconds()
			},
			func(stats map[string]clusterstats.StatSummary) (string, float64) {
...
// Value timeseries. Returns 0 if no valid data points exist.
func meanNonNaN(stats map[string]clusterstats.StatSummary, key string) float64 {
