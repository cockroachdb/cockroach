	...
	spec := func(subtest string, cfg admissionControlFollowerOverloadOpts) registry.TestSpec {
		return registry.TestSpec{
			...
			Cluster: func() spec.ClusterSpec {
				c := r.MakeClusterSpec(4, spec.CPU(4), spec.WorkloadNodes(1), spec.ReuseNone(), spec.DisableLocalSSD())
...
func runAdmissionControlFollowerOverload(
	...
	// Set up prometheus.
	{
		cfg := (&prometheus.Config{}).
			...
			WithWorkload("kv-n12", c.WorkloadNodes().InstallNodes()[0], 2112). // kv-n12
			WithWorkload("kv-n3", c.WorkloadNodes().InstallNodes()[0], 2113)   // kv-n3 (if present)
	...
	if cfg.kv0N12 {
		...
		c.Run(ctx, option.WithNodes(c.WorkloadNodes()), args...)
	}
	if cfg.kv50N3 {
	...
	if cfg.kv0N12 {
		...
		c.Run(ctx, option.WithNodes(c.WorkloadNodes()), deployWorkload)
	}
	if cfg.kv50N3 {
