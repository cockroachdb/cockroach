...
func registerJasyncSQL(r registry.Registry) {
	runJasyncSQL := func(ctx context.Context, t test.Test, c cluster.Cluster) {
		...
		if err := c.RunE(
			ctx,
			node,
			"cd /mnt/data1/jasyncsql && git clone https://github.com/jasync-sql/jasync-sql.git",
		); err != nil {
			t.Fatal(err)
		}

		if err := c.RunE(ctx, node, fmt.Sprintf("cd /mnt/data1/jasyncsql && git checkout %s",
		...
		_ = c.RunE(
			ctx,
			node,
			`cd /mnt/data1/jasyncsql && PGUSER=root PGHOST=localhost PGPORT=26257 PGDATABASE=defaultdb ./gradlew :postgresql-async:test`,
		)
		...
		if err := repeatRunE(
			...
			`cp /mnt/data1/jasyncsql/postgresql-async/build/test-results/test/*.xml ~/logs/report/jasyncsql-results -a`,
		); err != nil {
			t.Fatal(err)
		}
		...
		output, err := repeatRunWithBuffer(
			...
			`ls /mnt/data1/jasyncsql/postgresql-async/build/test-results/test/*xml`,
		)
		if err != nil {
			t.Fatal(err)
		}
