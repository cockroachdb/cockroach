...
`); err != nil {
	...
	mvt.InMixedVersion(
		...
		func(ctx context.Context, l *logger.Logger, r *rand.Rand, h *mixedversion.Helper) error {
			if err := h.Exec(r, `DELETE FROM t WHERE x = 13`); err != nil {
				return err
			}
			if err := h.Exec(r, `INSERT INTO t VALUES (13, 14, 15, 16)`); err != nil {
				return err
			}
			if err := h.Exec(r, `UPDATE t SET w = 17 WHERE y = 14`); err != nil {
				return err
			}
