	...
	for _, peerIP := range peerIPs {
		...
		if f.input && f.output {
			f.c.Run(ctx, f.c.Node(nodeID), fmt.Sprintf(
				`sudo iptables -A INPUT -m multiport -p tcp -s %s --ports 26257 -j DROP`, peerIP))
			f.c.Run(ctx, f.c.Node(nodeID), fmt.Sprintf(
				`sudo iptables -A OUTPUT -m multiport -p tcp -d %s --ports 26257 -j DROP`, peerIP))
		} else if f.input {
			f.c.Run(ctx, f.c.Node(nodeID), fmt.Sprintf(
...
func configureZone(
	...
	excludeNodes := func(nodes []int) []int {
	...
	if len(cfg.onlyNodes) > 0 {
		for _, nodeID := range excludeNodes(cfg.onlyNodes) {
