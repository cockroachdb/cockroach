...
func runFailoverChaos(ctx context.Context, t test.Test, c cluster.Cluster, readOnly bool) {
	...
	c.Run(ctx, option.OnNodes(c.Node(10)), fmt.Sprintf(
	...
	cancelWorkload := m.GoWithCancel(func(ctx context.Context) error {
		...
		err := c.RunE(ctx, option.OnNodes(c.Node(10)), fmt.Sprintf(
...
func runFailoverPartialLeaseGateway(ctx context.Context, t test.Test, c cluster.Cluster) {
	...
	c.Run(ctx, option.OnNodes(c.Node(6)), `./cockroach workload init kv --splits 1000 {pgurl:1}`)
	...
	t.L().Printf("running workload")
	cancelWorkload := m.GoWithCancel(func(ctx context.Context) error {
		err := c.RunE(ctx, option.OnNodes(c.Node(8)), `./cockroach workload run kv --read-percent 50 `+
			`--concurrency 256 --max-rate 2048 --timeout 1m --tolerate-errors `+
...
func runFailoverPartialLeaseLeader(ctx context.Context, t test.Test, c cluster.Cluster) {
	...
	t.L().Printf("running workload")
	cancelWorkload := m.GoWithCancel(func(ctx context.Context) error {
		err := c.RunE(ctx, option.OnNodes(c.Node(7)), `./cockroach workload run kv --read-percent 50 `+
			`--concurrency 256 --max-rate 2048 --timeout 1m --tolerate-errors `+
...
func runFailoverNonSystem(
	...
	require.NoError(t, err)
	configureZone(t, ctx, conn, `DATABASE kv`, zoneConfig{replicas: 3, onlyNodes: []int{4, 5, 6}})
	c.Run(ctx, option.OnNodes(c.Node(7)), `./cockroach workload init kv --splits 1000 {pgurl:1}`)
...
func runFailoverLiveness(
	...
	c.Run(ctx, option.OnNodes(c.Node(5)), `./cockroach workload init kv --splits 1000 {pgurl:1}`)
	...
	t.L().Printf("running workload")
	cancelWorkload := m.GoWithCancel(func(ctx context.Context) error {
		err := c.RunE(ctx, option.OnNodes(c.Node(5)), `./cockroach workload run kv --read-percent 50 `+
			`--concurrency 256 --max-rate 2048 --timeout 1m --tolerate-errors `+
			`--histograms=`+t.PerfArtifactsDir()+`/stats.json {pgurl:1-3}`)
		if ctx.Err() != nil {
			return nil // test requested workload shutdown
...
func (f *blackholeFailer) Cleanup(ctx context.Context) {
	f.c.Run(ctx, option.OnNodes(f.c.All()), `sudo iptables -F`)
}
...
func (f *blackholeFailer) Fail(ctx context.Context, nodeID int) {
	...
	if f.input && f.output {
		...
		f.c.Run(ctx, option.OnNodes(f.c.Node(nodeID)), fmt.Sprintf(`sudo iptables -A INPUT -p tcp --dport %s -j DROP`, pgport))
		f.c.Run(ctx, option.OnNodes(f.c.Node(nodeID)), fmt.Sprintf(`sudo iptables -A OUTPUT -p tcp --sport %s -j DROP`, pgport))
		// Outbound TCP connections, both sent and received packets.
		f.c.Run(ctx, option.OnNodes(f.c.Node(nodeID)), fmt.Sprintf(`sudo iptables -A OUTPUT -p tcp --dport %s -j DROP`, pgport))
		f.c.Run(ctx, option.OnNodes(f.c.Node(nodeID)), fmt.Sprintf(`sudo iptables -A INPUT -p tcp --sport %s -j DROP`, pgport))
	} else if f.input {
		f.c.Run(ctx, option.OnNodes(f.c.Node(nodeID)), fmt.Sprintf(`sudo iptables -A INPUT -p tcp --dport %s -j DROP`, pgport))
	} else if f.output {
		f.c.Run(ctx, option.OnNodes(f.c.Node(nodeID)), fmt.Sprintf(`sudo iptables -A OUTPUT -p tcp --dport %s -j DROP`, pgport))
...
func (f *blackholeFailer) FailPartial(ctx context.Context, nodeID int, peerIDs []int) {
	...
	for _, peerIP := range peerIPs {
		...
		if f.input && f.output {
			// Inbound TCP connections, both received and sent packets.
			f.c.Run(ctx, option.OnNodes(f.c.Node(nodeID)), fmt.Sprintf(
...
func (f *blackholeFailer) Recover(ctx context.Context, nodeID int) {
	f.c.Run(ctx, option.OnNodes(f.c.Node(nodeID)), `sudo iptables -F`)
}
