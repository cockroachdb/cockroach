...
func registerKVRestartImpact(r registry.Registry) {
	r.Add(registry.TestSpec{
		...
		Run: func(ctx context.Context, t test.Test, c cluster.Cluster) {
			...
			t.Status("waiting ", duration, " to establish a base QPS")
			time.Sleep(duration)
			qpsInitial := measureQPS(ctx, t, db, 5*time.Second)
			t.Status("initial (single node) qps: ", qpsInitial)
			...
			gracefulOpts.RoachprodOpts.Wait = true
			c.Stop(ctx, t.L(), gracefulOpts, c.Node(nodes))
			t.Status("waiting ", duration, " after stopping node to allow the node to fall behind")
			time.Sleep(duration)
			...
			t.Status("post outage qps: ", qpsFinal)

			// TODO(baptist): Once this is fixed, add this assertion. Today this will
			// always fail.
