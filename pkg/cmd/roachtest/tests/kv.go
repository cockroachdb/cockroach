	...
	r.Add(registry.TestSpec{
		...
		Run: func(ctx context.Context, t test.Test, c cluster.Cluster) {
			...
			for i := 0; i < 3; i++ {
				...
				c.Run(ctx, option.WithNodes(c.Node(nodes)), "./cockroach node drain --self --insecure")
				...
				// utility function.
				if !c.IsLocal() {
					c.Run(ctx, option.WithNodes(c.Node(nodes)), `sudo iptables -A INPUT -p tcp --dport 26257 -j DROP`)
					c.Run(ctx, option.WithNodes(c.Node(nodes)), `sudo iptables -A OUTPUT -p tcp --dport 26257 -j DROP`)
				}
				c.Stop(ctx, t.L(), option.DefaultStopOpts(), c.Node(nodes))
				...
				if !c.IsLocal() {
					c.Run(ctx, option.WithNodes(c.Node(nodes)), `sudo iptables -F`)
				}
