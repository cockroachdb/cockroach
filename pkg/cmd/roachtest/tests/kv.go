...
func registerKVLongRunningTxn(r registry.Registry) {
	...
	r.Add(registry.TestSpec{
		...
		Run: func(ctx context.Context, t test.Test, c cluster.Cluster) {
			...
			// and resolve (rewrite) the intent.
			m.Go(func(ctx context.Context) error {
				cmd := fmt.Sprintf("./cockroach workload run kv --init --duration=%s --read-percent=0 "+
					...
					"--cycle-length=100 --concurrency=1 --batch=1 --zipfian=true {pgurl%s}", duration, c.CRDBNodes())
				c.Run(ctx, option.WithNodes(c.WorkloadNode()), cmd)
				return nil
			})
			m.Go(func(ctx context.Context) error {
				histograms := " " + roachtestutil.GetWorkloadHistogramArgs(t, c, nil)
				cmd := fmt.Sprintf("./cockroach workload run kv --init %s --duration=%s --read-percent=100 "+
					"--cycle-length=100 --batch=1 --zipfian=true {pgurl%s}", histograms, duration, c.CRDBNodes())
				c.Run(ctx, option.WithNodes(c.WorkloadNode()), cmd)
				return nil
			})
			m.Wait()
