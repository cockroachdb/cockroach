...
func registerKVContention(r registry.Registry) {
	...
	r.Add(registry.TestSpec{
		...
		Benchmark:        true,
		Cluster:          r.MakeClusterSpec(nodes+1, spec.WorkloadNodes(1)),
...
func registerKVQuiescenceDead(r registry.Registry) {
	r.Add(registry.TestSpec{
		...
		Run: func(ctx context.Context, t test.Test, c cluster.Cluster) {
			...
			c.Run(ctx, option.WithNodes(c.WorkloadNodes()), "./cockroach workload run kv --init --max-ops=1 --splits 10000 --concurrency 100 {pgurl:1}")
			c.Run(ctx, option.WithNodes(c.WorkloadNodes()), kv+" --seed 0 {pgurl:1}")
			// Measure qps with all nodes up (i.e. with quiescence).
			qpsAllUp := qps(func() {
				c.Run(ctx, option.WithNodes(c.WorkloadNodes()), kv+" --seed 1 {pgurl:1}")
			})
			...
			qpsOneDown := qps(func() {
				...
				c.Run(ctx, option.WithNodes(c.WorkloadNodes()), kv+" --seed 2 {pgurl:1}")
			})
...
func registerKVGracefulDraining(r registry.Registry) {
	r.Add(registry.TestSpec{
		...
		Owner:            registry.OwnerKV,
		Cluster:          r.MakeClusterSpec(7, spec.WorkloadNodes(1)),
...
func registerKVRangeLookups(r registry.Registry) {
	...
	for _, item := range []struct {
		...
		r.Add(registry.TestSpec{
			...
			Owner:            registry.OwnerKV,
			Cluster:          r.MakeClusterSpec(nodes+1, spec.CPU(cpus), spec.WorkloadNodes(1), spec.WorkloadNodeCPU(cpus)),
			CompatibleClouds: registry.AllExceptAWS,
			Suites:           registry.Suites(registry.Nightly),
			Leases:           registry.MetamorphicLeases,
			Run: func(ctx context.Context, t test.Test, c cluster.Cluster) {
...
func registerKVRestartImpact(r registry.Registry) {
	r.Add(registry.TestSpec{
		...
		Timeout:          4 * time.Hour,
		Cluster:          r.MakeClusterSpec(13, spec.CPU(8), spec.WorkloadNodes(1), spec.WorkloadNodeCPU(8), spec.DisableLocalSSD()),
		Leases:           registry.MetamorphicLeases,
		Run: func(ctx context.Context, t test.Test, c cluster.Cluster) {
			nodes := len(c.CRDBNodes())
			startOpts := option.NewStartOpts(option.NoBackupSchedule)
			...
			c.Run(ctx, option.WithNodes(c.WorkloadNodes()), fmt.Sprintf("./cockroach workload init kv --splits=%d {pgurl:1}", splits))
