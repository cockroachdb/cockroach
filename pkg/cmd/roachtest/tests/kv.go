...
func registerKVQuiescenceDead(r registry.Registry) {
	r.Add(registry.TestSpec{
		...
		Run: func(ctx context.Context, t test.Test, c cluster.Cluster) {
			...
			c.Run(ctx, option.OnNodes(c.Node(nodes+1)), "./workload run kv --init --max-ops=1 --splits 10000 --concurrency 100 {pgurl:1}")
			c.Run(ctx, option.OnNodes(c.Node(nodes+1)), kv+" --seed 0 {pgurl:1}")
			// Measure qps with all nodes up (i.e. with quiescence).
			qpsAllUp := qps(func() {
				c.Run(ctx, option.OnNodes(c.Node(nodes+1)), kv+" --seed 1 {pgurl:1}")
			})
			...
			qpsOneDown := qps(func() {
				...
				c.Run(ctx, option.OnNodes(c.Node(nodes+1)), kv+" --seed 2 {pgurl:1}")
			})
	...
	r.Add(registry.TestSpec{
		...
		Run: func(ctx context.Context, t test.Test, c cluster.Cluster) {
			...
			m.Go(func(ctx context.Context) error {
				...
				return c.RunE(ctx, option.OnNodes(c.Node(nodes+1)), cmd)
			})
...
func registerKVRangeLookups(r registry.Registry) {
	...
	runRangeLookups := func(ctx context.Context, t test.Test, c cluster.Cluster, workers int, workloadType rangeLookupWorkloadType, maximumRangeLookupsPerSec float64) {
		...
		m.Go(func(ctx context.Context) error {
			...
			if err = c.RunE(ctx, option.OnNodes(c.Node(nodes+1)), cmd); err != nil {
				return err
			}
			concurrency := ifLocal(c, "", " --concurrency="+fmt.Sprint(nodes*64))
	...
	r.Add(registry.TestSpec{
		...
		Run: func(ctx context.Context, t test.Test, c cluster.Cluster) {
			...
			m.Go(func(ctx context.Context) error {
				...
				// large raft backlog.
				c.Run(ctx, option.OnNodes(c.Node(workloadNode)),
