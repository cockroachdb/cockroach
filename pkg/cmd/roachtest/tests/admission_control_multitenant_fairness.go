...
func runMultiTenantFairness(
	...
	for i := 0; i < numTenants; i++ {
		...
		require.NoError(t, c.RunE(ctx, option.OnNodes(tenantNode), cmd))
	...
	for i := 0; i < numTenants; i++ {
		...
		m1.Go(func(ctx context.Context) error {
			...
			err := c.RunE(ctx, option.OnNodes(c.Node(tenantNodeID(i))), cmd)
	...
	c.Run(ctx, option.OnNodes(crdbNode), "mkdir", "-p", t.PerfArtifactsDir())
	...
	c.Run(ctx, option.OnNodes(crdbNode), fmt.Sprintf(`echo '%s' > %s/stats.json`, results, t.PerfArtifactsDir()))
}
