	...
	for _, sp := range []ldrTestSpec{
		{
			...
			run: func(ctx context.Context, t test.Test, c cluster.Cluster, setup multiClusterSetup) {
				...
				startLDR := func(db *sqlutils.SQLRunner) int {
					// TODO(ssd): Name resolution is pretty broken.
					db.Exec(t, "USE kv")
					r := db.QueryRow(t,
				...
				leftJobID := startLDR(setup.left.sysSQL)
				rightJobID := startLDR(setup.right.sysSQL)
