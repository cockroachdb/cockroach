...
func TestLDRSchemaChange(
	...
	// and right sides.
	setup.left.sysSQL.Exec(t, fmt.Sprintf("ALTER TABLE %s.%s ADD COLUMN new_col INT NULL", ldrWorkload.dbName, ldrWorkload.tableName))
	setup.right.sysSQL.Exec(t, fmt.Sprintf("CREATE INDEX idx ON %s.%s(v, k)", ldrWorkload.dbName, ldrWorkload.tableName))
