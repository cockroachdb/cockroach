...
func runTPCHBench(ctx context.Context, t test.Test, c cluster.Cluster, b tpchBenchSpec) {
	...
	m.Go(func(ctx context.Context) error {
		...
		labels["max_ops"] = fmt.Sprintf("%d", maxOps)
		labels["num_queries"] = fmt.Sprintf("%d", numQueries)
		...
		cmd := fmt.Sprintf(
			"./workload run querybench --db=tpch --concurrency=1 --query-file=%s "+
				"--num-runs=%d --max-ops=%d {pgurl%s} "+roachtestutil.GetWorkloadHistogramArgsString(t, c, labels)+"--histograms-max-latency=%s",
			filename,
			b.numRunsPerQuery,
			maxOps,
			c.CRDBNodes(),
