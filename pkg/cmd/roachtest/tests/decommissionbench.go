...
func setupDecommissionBench(
	...
	SetAdmissionControl(ctx, t, c, benchSpec.admissionControl)
	{
		...
		if benchSpec.slowWrites {
			for _, stmt := range []string{
				fmt.Sprintf(`SET CLUSTER SETTING kv.range_split.by_load_enabled=false`),
				fmt.Sprintf(`SET CLUSTER SETTING kv.allocator.load_based_rebalancing=0`),
			} {
				t.Status(stmt)
				_, err := db.ExecContext(ctx, stmt)
				if err != nil {
...
func runSingleDecommission(
	...
	if stopFirst {
		...
		h.c.StopCockroachGracefullyOnNode(ctx, h.t.L(), target)
