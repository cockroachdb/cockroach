...
func setupDecommissionBench(
	...
	}
	{
		...
		c.Run(ctx, option.OnNodes(c.Node(pinnedNode)), importCmd)
...
func uploadPerfArtifacts(
	...
	if err := c.RunE(ctx, option.OnNodes(c.Node(pinnedNode)), "mkdir -p "+filepath.Dir(dest)); err != nil {
	...
	if !benchSpec.noLoad {
		...
		if err := c.RunE(ctx, option.OnNodes(c.Node(pinnedNode)),
		...
		if err := c.RunE(
			ctx, option.OnNodes(c.Node(pinnedNode)), fmt.Sprintf("rm %s", workloadStatsDest),
		); err != nil {
	...
	if !benchSpec.noLoad {
		m.Go(
			func(ctx context.Context) error {
				...
				err := c.RunE(ctx, option.OnNodes(c.Node(workloadNode)), workloadCmd)
...
func runSingleDecommission(
	...
	if drainFirst {
		...
		if err := h.c.RunE(ctx, option.OnNodes(h.c.Node(target)), cmd); err != nil {
			return err
		}
	}
	...
	if slowWrites {
		...
		if err := h.c.RunE(
			ctx, option.OnNodes(h.c.Node(target)),
	...
	if reuse {
		...
		if err := h.c.RunE(ctx, option.OnNodes(h.c.Node(target)), "rm -rf {store-dir}"); err != nil {
			return err
		}
