...
func initJepsen(ctx context.Context, t test.Test, c cluster.Cluster, j jepsenConfig) {
	...
	if err := c.RunE(ctx, option.OnNodes(c.Node(1)), "test -e jepsen_initialized"); err == nil {
	...
	c.Run(ctx, option.OnNodes(c.All()), "sh", "-c", `"sudo apt-get -y update > logs/apt-upgrade.log 2>&1"`)
	c.Run(ctx, option.OnNodes(c.All()), "sh", "-c", `"sudo DEBIAN_FRONTEND=noninteractive apt-get -y upgrade -o Dpkg::Options::='--force-confold' -o DPkg::options::='--force-confdef' > logs/apt-upgrade.log 2>&1"`)
	...
	c.Run(ctx, option.OnNodes(c.All()), "tar --transform s,^,cockroach/, -c -z -f cockroach.tgz cockroach")
	...
	c.Run(ctx, option.OnNodes(controller), "sh", "-c",
	...
	c.Run(ctx, option.OnNodes(controller), "sh", "-c", `"ssh-keygen -p -f .ssh/id_rsa -m pem -P '' -N ''"`)
	...
	c.Run(ctx, option.OnNodes(workers), "sh", "-c", `"cat controller_id_rsa.pub >> .ssh/authorized_keys"`)
	...
	for _, ip := range ips {
		c.Run(ctx, option.OnNodes(controller), "sh", "-c", fmt.Sprintf(`"ssh-keyscan -t rsa %s >> .ssh/known_hosts"`, ip))
	}

	t.L().Printf("cluster initialization complete\n")
	c.Run(ctx, option.OnNodes(c.Node(1)), "touch jepsen_initialized")
}
...
func (j jepsenConfig) prepareBinary(
	...
	if j.buildFromSource {
		...
		for r := retry.StartWithCtx(ctx, retry.Options{MaxRetries: 3, InitialBackoff: 5 * time.Second}); r.Next(); {
			...
			err = c.RunE(ctx, option.OnNodes(ctr), "bash", "-e", "-c",
...
func runJepsen(ctx context.Context, t test.Test, c cluster.Cluster, testName, nemesis string) {
	...
	runE := func(c cluster.Cluster, ctx context.Context, node option.NodeListOption, args ...string) error {
		...
		return c.RunE(ctx, option.OnNodes(node), args...)
	}
