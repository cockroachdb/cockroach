...
Then upload newly built version

gcloud storage cp cockroachdb-x.x.x-standalone.jar gs://cockroach-jepsen
...
const binaryVersion = "0.1.0-SNAPSHOT-standalone"
...
func (j jepsenConfig) prepareBinary(ctx context.Context, t test.Test, c cluster.Cluster,
	ctr option.NodeListOption,
) {
	// Install jepsen. This part is fast if the repo is already there,
	// so do it before the initialization check for ease of iteration.
...
func (j jepsenConfig) startTest(ctx context.Context, t test.Test, run func(args ...string) error,
	testArgs string,
) <-chan error {
