...
//
// The default targetSplits is sized so that, on the standard perturbation
// cluster (12 nodes × 16 vcpu, RF=3), we land at roughly 10k replicas per
// vcpu per node — the highest density that still leaves headroom for the
// concurrent KV workload to make progress. See issue #170041 for the
// motivation behind reframing this as a perturbation test.
//
// The test runs in two flavors selected by the asleep field. asleep=true
// (the default density target above) measures total replica capacity:
// followers fall asleep via store liveness quiescence, so the bulk of the
// replica population costs no per-tick CPU. asleep=false disables follower
// sleep so every replica costs its full per-tick CPU; this measures the
// cluster's capacity for active replicas, which is the relevant ceiling
// during outages where store liveness can't withdraw and followers can't
// fall asleep. Because awake replicas are far more expensive, the
// asleep=false variant scales the target population down by 5× across
// every cluster size.
//
...
func (s splits) setup() variations {
	...
	//
	// The asleep=true target lands at ~10k replicas/vcpu/node; asleep=false
	// scales down 5× because each replica costs its full per-tick CPU. The
	// batch size scales by the same factor to keep the number of batches
	// (and therefore the density-reporting cadence) constant across flavors.
	if s.asleep {
		s = splits{targetSplits: 600_000, batchSize: 10_000, asleep: true}
	} else {
		s = splits{targetSplits: 120_000, batchSize: 2_000, asleep: false}
	}
	...
	v.splits = 500
	v.scatter = true
	// The 600k-split storm plus end-of-perturbation lease rebalance over
	// 75k ranges × 8 groups can run for over an hour on its own; combined
	// with the standard fill / validation windows the default 3h timeout
	// is too tight.
	...
	// jobs) at 1.8M replicas, occasionally observing transient non-zero
	// diffs that never represented real outstanding state. A longer
	// window gives SucceedsWithin's retry loop a better chance to land on
	// a clean snapshot.
	v.tokenReturnTime = 1 * time.Hour
	// The split storm itself is expected to crush foreground throughput
	// (every node is admitting and replicating split commands across 1.8M
	// replicas while also serving the workload). The interesting question
	// is whether the cluster recovers afterwards, so let the perturbation
	// interval slide and gate on the recovery interval instead.
...
// an hour on a developer's machine. asleep=false scales down 5× for the
// same reason as in setup().
...
// asleep=false scales down 5× for the same reason as in setup().
func (s splits) sizeForMetamorphic() perturbation {
	if s.asleep {
		s.targetSplits = 60_000
		s.batchSize = 5_000
	} else {
		s.targetSplits = 12_000
		s.batchSize = 1_000
	}
	return s
}
...
func rebalanceLeases(
	...
	for _, m := range moves {
		// Acquire concurrency token, honoring ctx so a parent cancel
		// (timeout, sibling failure) doesn't wedge the dispatch loop.
		select {
		case sem <- struct{}{}:
		case <-ctx.Done():
			return ctx.Err()
