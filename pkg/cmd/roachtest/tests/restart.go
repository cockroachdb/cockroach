...
func runRestart(ctx context.Context, t test.Test, c cluster.Cluster, downDuration time.Duration) {
	...
	c.Run(ctx, option.OnNodes(workloadNode),
	...
	// came back up.
	c.Run(ctx, option.OnNodes(workloadNode), "./cockroach workload run tpcc --warehouses=100 "+
	...
	c.Run(ctx, option.OnNodes(restartNode), fmt.Sprintf(`./cockroach sql --insecure --url=%s -e "%s"`, pgurl, tracedQ))
