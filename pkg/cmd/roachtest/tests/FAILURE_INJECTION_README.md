...
3. **WaitForFailureToPropagate()**: Block until the failure has taken full effect
   (e.g., node is marked as dead, replicas have moved).

4. **Recover()**: Reverse the effects of Inject() (e.g., remove iptables rules,
   restart nodes).

5. **WaitForFailureToRecover()**: Block until the cluster has fully recovered
   (e.g., replicas are balanced, nodes are healthy).
...
failer, args, err := roachtestutil.MakeDmsetupDiskStallFailer(t.L(), c, c.Node(1))
```
...
// 7. Wait for failure to take effect
...
// 10. Wait for cluster to stabilize
