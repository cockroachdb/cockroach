...
type stepFunc func(context.Context, *logger.Logger, *rand.Rand, *mixedversion.Helper) error
...
func runGetRequests(
	...
	inMemTable := server.CrdbInternalStmtStatsCombined
	if statsType == serverpb.CombinedStatementsStatsRequest_TxnStatsOnly {
		inMemTable = server.CrdbInternalTxnStatsCombined
	}
	...
	if !foundFlushedStats {
		return errors.Newf("failed to find flushed stats, found: %v, %v", maps.Keys(foundTableSources))
	}
