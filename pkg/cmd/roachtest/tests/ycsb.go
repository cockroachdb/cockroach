	...
	runYCSB := func(
		...
		m.Go(func(ctx context.Context) error {
			...
			if readCommitted {
				...
				// Under Read Committed isolation, omit the transactional
				// read-modify-write and the SELECT FOR UPDATE in Workload F.
				// Instead, run the workload as its official version is written.
				args += " --read-modify-write-in-txn=false"
			}
