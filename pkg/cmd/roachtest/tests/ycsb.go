	...
	for _, wl := range workloads {
		for _, cpus := range cpusConfigs {
			...
			r.Add(registry.TestSpec{
				...
				Benchmark: true,
				Cluster:   r.MakeClusterSpec(4, spec.CPU(cpus), spec.WorkloadNodes(1)),
			...
			if wl == "A" {
				r.Add(registry.TestSpec{
					...
					Benchmark: true,
					Cluster:   r.MakeClusterSpec(4, spec.CPU(cpus), spec.WorkloadNodes(1), spec.SetFileSystem(spec.Zfs)),
					Run: func(ctx context.Context, t test.Test, c cluster.Cluster) {
						runYCSB(ctx, t, c, wl, cpus, ycsbOptions{})
					},
