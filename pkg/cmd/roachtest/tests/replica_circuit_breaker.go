...
func runReplicaCircuitBreaker(ctx context.Context, t test.Test, c cluster.Cluster) {
	...
	opts := tpccOptions{
		...
		Start: func(ctx context.Context, t test.Test, c cluster.Cluster) {
			...
			// Manually set the cluster setting to a reasonable but low enough value
			// (it is set more conservatively by default, to something like 60s).
			// Since the replicas need to circuit-break one by one, a very large
			// number here will fail the AOST validations in the test because the
			// DistSQL streams will time out before the errors are returned.
			_, err := c.Conn(ctx, t.L(), 1).Exec(
				`SET CLUSTER SETTING kv.replica_circuit_breaker.slow_replication_threshold = '4s'`,
			)
