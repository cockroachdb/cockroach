...
func registerIndexOverload(r registry.Registry) {
	r.Add(registry.TestSpec{
		...
		Run: func(ctx context.Context, t test.Test, c cluster.Cluster) {
			hl := hardwareLimiter{context: ctx, test: t, cluster: c}
			...
			medianWriteIO := hl.getWriteIO(nodeList)
			// Set the max IO allowed to be 1.5x the max usage.
			maxIO := uint64(float64(medianWriteIO) * 1.5)
			hl.throttleWriteIO(nodeList, maxIO)
			t.Status(fmt.Sprintf("throttle write IO baseline (%s)", duration))
			time.Sleep(duration)

			t.Status("starting index creation (%s)")
