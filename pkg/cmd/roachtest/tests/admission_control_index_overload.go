...
// large TPC-C dataset, and sets up a foreground load of kv95/1b. It then
...
func registerIndexOverload(r registry.Registry) {
	r.Add(registry.TestSpec{
		...
		Run: func(ctx context.Context, t test.Test, c cluster.Cluster) {
			...
			duration, err := time.ParseDuration(ifLocal(c, "20s", "5m"))
			assert.NoError(t, err)

			t.Status(fmt.Sprintf("starting kv workload thread to run for %s (<%s)", duration, time.Minute))
			m := c.NewMonitor(ctx, c.Range(1, crdbNodes))
			m.Go(func(ctx context.Context) error {
				duration := " --duration=" + (duration * 4).String()
				concurrency := ifLocal(c, "  --concurrency=8", " --concurrency=2048")
				c.Run(ctx, c.Node(crdbNodes+1),
					"./cockroach workload run kv --read-percent=50 --max-rate=4000"+
						duration+concurrency+fmt.Sprintf(" {pgurl:1-%d}", crdbNodes),
				)
				return nil
			})

			t.Status(fmt.Sprintf("measuring performance IO baseline (<%s)", duration))
			time.Sleep(duration)

			nodeList := c.Range(1, crdbNodes)
			medianWriteIO := getWriteIO(ctx, t, c, nodeList)
			// Set the max IO allowed to be 1.5x the average usage.
			maxIO := int(float64(medianWriteIO) * 1.5)
			throttleWriteIO(ctx, t, c, nodeList, maxIO)
			time.Sleep(duration)

			// TODO: Iterate through different index types (small, medium, large)
			t.Status("starting index creation ")
			db := c.Conn(ctx, t.L(), crdbNodes)
			defer db.Close()

			if _, err := db.ExecContext(ctx,
				"CREATE INDEX test_index ON tpcc.history(h_date);",
			); err != nil {
			...
			t.Status("drop complete - waiting for workload to finish")
			m.Wait()
		},
	})
}
...
/*
	...
	`CREATE UNIQUE INDEX test_index ON tpcc.order (o_entry_d, o_w_id, o_d_id, o_carrier_id, o_id)`,
*/
