...
func runMVCCGC(ctx context.Context, t test.Test, c cluster.Cluster) {
	...
	m.Go(func(ctx context.Context) error {
		...
		t.L().Printf("finished prepping initial GC data")
		...
		for i := 0; i < cleanupRuns; i++ {
			...
			if err := retry.WithMaxAttempts(ctx, retry.Options{}, 3, func() error {
				// This is not atomic, but should be ok if we lay more range tombstones if
				// they still cover full range.
			...
			if err := checkTableHasNoLiveData(t, conn, "kv", "kv"); err != nil {
				t.Fatal(err)
			}

			execSQLOrFail("alter database kv configure zone using gc.ttlseconds = $1", 120)

			c.Run(ctx, c.Node(1), "./cockroach", "workload", "run", "kv", "--duration", "2m",
				"--read-percent", "10")

			t.L().Printf("run batch of workload on top of deleted data")
			...
			for {
				...
				err := assertTableHasNoGarbage(t, conn, "kv", "kv")
				if err == nil {
					break
				}
				if timeutil.Now().After(deadline) {
					t.Fatalf("tombstones were not GCd: %s", err)
				}
				select {
...
func assertTableHasNoGarbage(t test.Test, conn *gosql.DB, tableName, databaseName string) error {
	totals, rangeCount, sts := collectTableMVCCStatsOrFatal(t, conn, tableName, databaseName)
	t.L().Printf("assert range tombstones and garbage in ranges: %d stats: %s", rangeCount, totals.String())
	if rangeCount == 0 {
	...
	if c := sts[roachpb.CheckConsistencyResponse_RANGE_INCONSISTENT.String()]; c > 0 {
		return errors.Errorf("consistency check reported %d inconsistent ranges", c)
	}
	return nil
}

func checkTableHasNoLiveData(t test.Test, conn *gosql.DB, tableName, databaseName string) error {
	totals, rangeCount, _ := collectTableMVCCStatsOrFatal(t, conn, tableName, databaseName)
	t.L().Printf("assert live data in ranges: %d stats: %s", rangeCount, totals.String())
	if rangeCount == 0 {
		return errors.New("failed to find any ranges for table")
	}
...
// rangeConsistencies maps consistency responses to how many times they were
// found across table ranges.
type rangeConsistencies map[string]int

func collectTableMVCCStatsOrFatal(
	t test.Test, conn *gosql.DB, tableName, databaseName string,
) (enginepb.MVCCStats, int, rangeConsistencies) {
	...
	queryRowOrFatal(t, conn,
		...
		[]interface{}{tableName, databaseName}, []interface{}{&startKey, &endKey})

	rows, err := conn.Query(`select range_id, start_key, status, crdb_internal.range_stats(start_key) from crdb_internal.check_consistency(false, $1, $2) order by start_key`,
		startKey, endKey)
	if err != nil {
	...
	var statuses = make(rangeConsistencies)
	...
	for rows.Next() {
		...
		if err := rows.Scan(&rangeID, &rangeStartKey, &status, &jsonb); err != nil {
			t.Fatalf("failed to run consistency check query on table: %s", err)
		}
		statuses[status]++
	...
	return tableStats, rangeCount, statuses
}
...
func (m tableMetadata) String() string {
	return fmt.Sprintf("TableMetadata{tableID=%d, tableName='%s', databaseName='%s', startKey='%s', endKey='%s'}",
		m.tableID, m.tableName, m.databaseName, m.startKey.String(), m.endKey.String())
}
...
func queryTableMetaOrFatal(
	...
	queryRowOrFatal(t, conn,
		`select min(start_key) from crdb_internal.ranges_no_leases where database_name = $1 and table_name = $2`,
		[]interface{}{databaseName, tableName}, []interface{}{&startKey})
