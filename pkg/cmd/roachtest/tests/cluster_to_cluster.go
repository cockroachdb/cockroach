...
func c2cRegisterWrapper(
	...
	// Save some money and CPU quota by using a smaller workload node,
	// but only for clusters of size 5 or smaller to avoid regressions.
	clusterOps = append(clusterOps, spec.WorkloadNode())
	if sp.srcNodes+sp.dstNodes > 5 && sp.cpus != 0 {
		clusterOps = append(clusterOps, spec.WorkloadNodeCPU(sp.cpus))
	}
