			...
			Run: func(ctx context.Context, t test.Test, c cluster.Cluster) {
				...
				go func() {
					...
					if workloadCtx.Err() == nil {
						...
						// its own.
						t.Fatalf(`Workload context was not cancelled. Error returned by workload cmd: %s`, err)
					}
				...
				t.Status(fmt.Sprintf(`initial scan complete. run workload and repl. stream for another %d minutes`,
					int(sp.additionalDuration.Minutes())))
				...
				case <-time.After(sp.additionalDuration):
					workloadCancel()
					t.L().Printf("workload has finished after %d", int(sp.additionalDuration.Minutes()))
				case <-ctx.Done():
