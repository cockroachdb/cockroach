...
func registerLease(r registry.Registry) {
	r.Add(registry.TestSpec{
		Name:    "acquire-lease/crash/non-system",
		Owner:   registry.OwnerKV,
		Timeout: time.Hour,
		Cluster: r.MakeClusterSpec(7, spec.CPU(4)),
		Run:     runAcquireLeaseCrashNonSystem,
	})
}

// runAcquireLeaseCrashNonSystem benchmarks the maximum duration of range
// unavailability following a leaseholder crash. It tests the simplest possible
// failure:
//
...
//
// We expect the worst-case (pMax) latency to be slightly larger than the lease
// interval (9s), to account for lease acquisition and retry latencies. We do
// not assert this, but instead export latency histograms for graphing.
//
...
// workers directed at n1-n3. n4-n6 are killed and restarted in order, with 30
// seconds between each operation, for 3 cycles totaling 9 crashes.
func runAcquireLeaseCrashNonSystem(ctx context.Context, t test.Test, c cluster.Cluster) {
	c.Put(ctx, t.Cockroach(), "./cockroach")
	c.Put(ctx, t.DeprecatedWorkload(), "./workload")

	// Create cluster with one rack locality per node and configure it.
	...
	for rows.Next() {
		...
		_, err = conn2.ExecContext(ctx, fmt.Sprintf(
			`ALTER %s CONFIGURE ZONE USING num_replicas = 3, constraints = '[-rack=3, -rack=4, -rack=5]'`,
			target))
		require.NoError(t, err)
	}
	...
	_, err = conn.ExecContext(ctx, `ALTER DATABASE kv CONFIGURE ZONE USING `+
		`num_replicas = 3, constraints = '[-rack=0, -rack=1, -rack=2]'`)
	require.NoError(t, err)
	c.Run(ctx, c.Node(7), `./workload init kv --splits 1000 {pgurl:1}`)

	// The replicate queue takes forever to move the kv ranges from n1-n3 to
	// n4-n6, so we do it ourselves.
	var count int
	for _, source := range []int{1, 2, 3} {
		for {
			require.NoError(t, conn.QueryRowContext(ctx, `SELECT count(*) FROM crdb_internal.ranges `+
				`WHERE database_name = 'kv' AND $1::int = ANY(replicas)`, source).Scan(&count))
			if count == 0 {
				break
			}
			t.Status(fmt.Sprintf("moving %d kv ranges off of n%d", count, source))
			for _, target := range []int{4, 5, 6} {
				_, err = conn.ExecContext(ctx, `ALTER RANGE RELOCATE FROM $1::int TO $2::int FOR `+
					`SELECT range_id FROM crdb_internal.ranges WHERE database_name = 'kv' AND $1::int = ANY(replicas)`,
					source, target)
				require.NoError(t, err)
			}
	...
	require.NoError(t, conn.QueryRowContext(ctx, `SELECT count(*) FROM crdb_internal.ranges `+
		`WHERE database_name = 'kv' AND replicas @> ARRAY[1, 2, 3]`).Scan(&count))
	require.Zero(t, count, "found kv ranges on n1-n3")
	...
	m.Go(func(ctx context.Context) error {
		c.Run(ctx, c.Node(7), `./workload run kv --read-percent 50 `+
			`--duration 600s --concurrency 256 --max-rate 2048 --tolerate-errors `+
	...
	m.Go(func(ctx context.Context) error {
		...
		for i := 0; i < 3; i++ {
			for _, node := range []int{4, 5, 6} {
				...
				// Verify that there are no system ranges on the node.
				var count int
				err := conn.QueryRowContext(ctx, `SELECT count(*) FROM crdb_internal.ranges `+
					`WHERE database_name != 'kv' AND $1::int = ANY(replicas)`, node).Scan(&count)
				if err != nil {
					return err
				}
				if count > 0 {
					return errors.Errorf("found %d unexpected system ranges on n%d", count, node)
				}
