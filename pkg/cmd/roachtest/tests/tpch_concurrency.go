...
func registerTPCHConcurrency(r registry.Registry) {
	...
	checkConcurrency := func(ctx context.Context, t test.Test, c cluster.Cluster, concurrency int) error {
		...
		m.Go(func(ctx context.Context) error {
			...
			// The reasoning behind choosing a pretty long duration is so that
			// all queries get a chance to run at least once. Each connection is
			// executing queries in order (i.e. q1, q2, etc), and because the
			// cluster is totally overloaded, it might take really long time
			// (like minutes or tens of minutes) for a single query to finish.
			// There are 22 queries in the benchmark, so we chose 45 minutes as
			// a reasonable guess for the duration that should often be
			// sufficient for q22 to run on many connections at least once.
			cmd := fmt.Sprintf(
				"./workload run tpch {pgurl:1-%d} --tolerate-errors "+
					"--duration=45m --concurrency=%d", numNodes-1, concurrency,
			)
