...
func registerTPCHConcurrency(r registry.Registry) {
	...
	setupCluster := func(ctx context.Context, t test.Test, c cluster.Cluster) {
	...
	runTPCHConcurrency := func(ctx context.Context, t test.Test, c cluster.Cluster) {
	...
	r.Add(registry.TestSpec{
		...
		// order of one hour, so in order to let each the test run to completion
