...
func runInconsistency(ctx context.Context, t test.Test, c cluster.Cluster) {
	...
	//   }
	// }
	c.Run(ctx, option.OnNodes(c.Node(1)), "./cockroach debug pebble db set {store-dir} "+
	...
	c.Run(ctx, option.OnNodes(c.Node(1)), "grep "+expr+" {log-dir}/cockroach.log")

	// Make sure that every node creates a checkpoint.
	for n := 1; n <= 3; n++ {
		// Notes it in the log.
		const expr = "creating.checkpoint.*with.spans"
		c.Run(ctx, option.OnNodes(c.Node(n)), "grep "+expr+" {log-dir}/cockroach.log")
		...
		c.Run(ctx, option.OnNodes(c.Node(n)), "find {store-dir}/auxiliary/checkpoints -name checkpoint.txt")
		// The checkpoint can be inspected by the tooling.
		c.Run(ctx, option.OnNodes(c.Node(n)), "./cockroach debug range-descriptors "+
			"$(find {store-dir}/auxiliary/checkpoints/* -maxdepth 0 -type d | head -n1)")
		c.Run(ctx, option.OnNodes(c.Node(n)), "./cockroach debug range-data --limit 10 "+
