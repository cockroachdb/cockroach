...
func registerMVCCRangeTombstones(r registry.Registry) {
	r.Add(registry.TestSpec{
		...
		Timeout: 2 * time.Hour,
		Cluster: r.MakeClusterSpec(5, spec.CPU(32)),
...
func runMVCCRangeTombstones(ctx context.Context, t test.Test, c cluster.Cluster) {
	...
	for tbl := range tablesToNumFiles {
		...
		ttl := randutil.RandIntInRange(rng, 60 /* 1m */, 60*15 /* 15m */)
...
func (t *mvccRangeTombstoneTest) makeFilename(tableName string, number int, numFiles int) string {
...
func (t *mvccRangeTombstoneTest) runImportSequence(
	...
	for i := 0; i < attempts && len(filesToImport) > 0; i++ {
		...
		// Cancel the job after a random duration in [10s,3m).
		...
		if status == "succeeded" {
			// Remove the files that succeeded so we don't try to import
			// them again.
