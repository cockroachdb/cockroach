					...
					case <-ticker.C:
						...
						ops := []func(){
							func() {
								...
								err := c.RunE(ctx, option.OnNodes(c.Node(n)), startCommands[n-1])
							...
							func() {
								...
								err := c.RunE(ctx, option.OnNodes(c.Node(n)), fmt.Sprintf("./cockroach debug compact /mnt/data1/cockroach %s", enterpriseEncryption[n-1]))
