...
func registerRestore(r registry.Registry) {
	...
	for _, sp := range []restoreSpecs{
		...
		// Full-cluster restores of the large empty-schema fixtures, generated by
		// the large-schema introspection benchmark. These exercise restore on
		// clusters with many descriptors but little row data.
		{
			hardware:        makeHardwareSpecs(hardwareSpecs{}),
			backup:          backupSpecs{cloud: spec.GCE, fixture: LargeEmptySchemaFixture{NumTables: 10_000}},
			timeout:         2 * time.Hour,
			suites:          registry.Suites(registry.Weekly),
			skipFingerprint: true,
		},
		{
			hardware:        makeHardwareSpecs(hardwareSpecs{}),
			backup:          backupSpecs{cloud: spec.GCE, fixture: LargeEmptySchemaFixture{NumTables: 100_000}},
			timeout:         4 * time.Hour,
			suites:          registry.Suites(registry.Weekly),
			skipFingerprint: true,
			skip:            "restore of 100k empty tables is known to fail; registered to track future work",
		},
		{
			hardware:        makeHardwareSpecs(hardwareSpecs{}),
			backup:          backupSpecs{cloud: spec.GCE, fixture: LargeEmptySchemaFixture{NumTables: 1_000_000}},
			timeout:         8 * time.Hour,
			suites:          registry.Suites(registry.Weekly),
			skipFingerprint: true,
			skip:            "restore of 1M empty tables is known to fail; registered to track future work",
		},
		...
		r.Add(registry.TestSpec{
			...
			Run: func(ctx context.Context, t test.Test, c cluster.Cluster) {
				...
				m.Go(func(ctx context.Context) error {
					...
					t.Status(`running restore`)
					metricCollector := rd.initRestorePerfMetrics(ctx, durationGauge)
					if err := rd.run(ctx, rd.sp.backup.fixture.RestoreTarget()); err != nil {
						return err
					}
					metricCollector()
					rd.maybeValidateFingerprint(ctx)
