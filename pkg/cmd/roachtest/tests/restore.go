...
func (hw hardwareSpecs) makeClusterSpecs(r registry.Registry, backupCloud string) spec.ClusterSpec {
	...
	if len(hw.zones) > 0 {
		// Tests are set up to run on one specific cloud, so it's ok that the zones
		// will only make sense for one of them.
	...
	if backupCloud == spec.AWS && s.VolumeSize != 0 {
		...
		s.AWS.MachineType, _ = spec.SelectAWSMachineType(s.CPUs, s.Mem, vm.ArchAMD64)
