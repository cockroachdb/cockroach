...
// sortRows is similar to joinAndSortRows, but it uses float comparison for
// float columns (and decimal columns when on s390x).
func sortRows(rowMatrix1, rowMatrix2 [][]string, colTypes []string) {
	floatsLess := func(i, j int, rowMatrix [][]string) bool {
		for idx := range colTypes {
			if (runtime.GOARCH == "s390x" && colTypes[idx] == "DECIMAL") ||
				...
				if err != nil {
					panic(errors.AssertionFailedf("error comparing floats %s and %s: %s",
						rowMatrix[i][idx], rowMatrix[j][idx], err))
				}
				if res != 0 {
					return res == -1
				}
...
func unsortedMatricesDiffWithFloatComp(
	...
	sortRows(rowMatrix1, rowMatrix2, colTypes)
