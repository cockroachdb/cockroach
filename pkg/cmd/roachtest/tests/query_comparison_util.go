...
func runOneRoundQueryComparison(
	...
	if qct.setupName == "workload-replay" {
		...
		if qct.isMultiRegion {
			...
			node2 := rnd.Intn(qct.nodeCount) + 1
		...
		for i := 1; ; i++ {
			...
			if i < numInitialMutations || i%25 == 0 {
				// Flip a coin to determine which connection is used.
				if rnd.Intn(2) == 0 {
					logStmt("-- executing with conn1")
					runMutationStatement(conn, mutatingSmither, logStmt)
				} else {
					logStmt("-- executing with conn2")
					runMutationStatement(conn2, mutatingSmither, logStmt)
				}
				continue
			}
...
func (h *queryComparisonHelper) runQuery(stmt string) ([][]string, error) {
	...
	if h.rnd.Intn(2) == 0 {
		conn = h.conn1
		connInfo = "-- querying with conn1\n"
	} else {
		conn = h.conn2
		connInfo = "-- querying with conn2\n"
	}
	...
	h.logStmt(fmt.Sprintf(connInfo+"-- %s: %s", timeutil.Now(),
