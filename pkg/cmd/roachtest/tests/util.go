...
func profileTopStatements(
	...
	// The minimum number of occurances of the statement for it to be included in profiling.
	minFrequency := .001

	// The minimum number of times the statement must be executed prior to
	// calling this to be included. By using a count here it removes the need to
...
FROM (
	...
	WHERE cnt > %d
)`, minFrequency, minDuration, minNumExpectedStmts)
	if _, err := db.Exec(sql); err != nil {
		return err
	}
	return nil
