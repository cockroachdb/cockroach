...
func registerLOQRecovery(r registry.Registry) {
	spec := r.MakeClusterSpec(6, spec2.WorkloadNodes(1))
...
func runRecoverLossOfQuorum(ctx context.Context, t test.Test, c cluster.Cluster, s loqTestSpec) {
	...
	m.Go(func(ctx context.Context) error {
		...
		c.Run(ctx, option.WithNodes(c.WorkloadNodes()), s.wl.initCmd(pgURL, dbName))
		...
		c.Run(ctx, option.WithNodes(c.WorkloadNodes()), s.wl.runCmd(pgURL, dbName, ifLocal(c, "10s", "30s"), ""))
		...
		for _, node := range remaining {
			...
			c.Put(ctx, path.Join(t.ArtifactsDir(), name), name, c.WorkloadNodes())
			planArguments += " " + name
		}
		t.L().Printf("running plan creation")
		planCmd := "./cockroach debug recover make-plan --confirm y -o " + planName + planArguments
		if err = c.RunE(ctx, option.WithNodes(c.WorkloadNodes()), planCmd); err != nil {
		...
		if err := c.Get(ctx, t.L(), planName, path.Join(t.ArtifactsDir(), planName),
			c.WorkloadNodes()); err != nil {
			t.Fatalf("failed to collect plan %s from controller node %s: %s", planName, c.WorkloadNodes(), err)
		}
		...
		if err := timeutil.RunWithTimeout(ctx, "mark-nodes-decommissioned", 5*time.Minute,
			func(ctx context.Context) error {
				...
				return c.RunE(ctx, option.WithNodes(c.WorkloadNodes()), decommissionCmd)
			}); err != nil {
		...
		if err = c.RunE(ctx, option.WithNodes(c.WorkloadNodes()),
			s.wl.runCmd(
...
func runHalfOnlineRecoverLossOfQuorum(
	...
	m.Go(func(ctx context.Context) error {
		...
		c.Run(ctx, option.WithNodes(c.WorkloadNodes()), applyCommand)
		...
		if err = timeutil.RunWithTimeout(ctx, "wait-for-restart", 2*time.Minute,
			func(ctx context.Context) error {
				for {
					res, err := c.RunWithDetailsSingleNode(ctx, t.L(), option.WithNodes(c.WorkloadNodes()), verifyCommand)
