...
func (s loqTestSpec) String() string {
	return fmt.Sprintf("loqrecovery/workload=%s/rangeSize=%d", s.wl, s.rangeSize)
}
...
func registerLOQRecovery(r registry.Registry) {
	...
	for _, s := range []loqTestSpec{
		...
		{wl: tpccLoqWorkload{warehouses: 100, concurrency: 32}, rangeSize: 1024 * 1024 * 10},
...
const (
	...
	workloadFailed testOutcomeMetric = 30
...
func runRecoverLossOfQuorum(ctx context.Context, t test.Test, c cluster.Cluster, s loqTestSpec) {
	...
	m.Go(func(ctx context.Context) error {
		...
		if s.rangeSize > 0 {
			t.L().Printf("setting range size limit small to force more splits in workload")
			_, err = db.Exec(
	...
	if err = c.RunE(ctx, c.Node(controller), planCmd); err != nil {
		...
		m.Go(func(ctx context.Context) error {
			...
			if err = c.RunE(ctx, c.Node(controller),
				s.wl.runCmd(
					fmt.Sprintf("{pgurl:1,4-%d}", maxNode), dbName, ifLocal(c, "10s", "30s"),
					workloadHistogramFile)); err != nil {
				t.L().Printf("workload failed")
				// This may not to obvious, but workload process will fail with error
				// if nodes are unavailable and not when it can't perform a write.
				// In all observed cases it was caused by node panicing some time after
				// restart (due to conflicting range descriptors etc).
				return &RecoveryImpossibleError{testOutcome: restartFailed}
			}
			t.L().Printf("workload finished")
	...
	if testOutcome == success {
		...
		t.L().Printf("recovery failed with error code %d", testOutcome)
		if debugFailures && testOutcome > 0 {
			t.Fatalf("Test failed with code %d", testOutcome)
		}
	}
}
