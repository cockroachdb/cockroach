		...
		Run: func(ctx context.Context, t test.Test, c cluster.Cluster) {
			...
			m.Go(func(ctx context.Context) error {
				...
				cmd := fmt.Sprintf("./cockroach workload run kv %s--concurrency=2 "+
					...
					roachtestutil.GetWorkloadHistogramArgsString(t, c, labels), foregroundDB, dur, url)
			...
			m.Go(func(ctx context.Context) error {
				...
				cmd := fmt.Sprintf("./cockroach workload run kv %s --concurrency=1024 "+
					...
					"--txn-qos='background' --tolerate-errors %s %s %s", roachtestutil.GetWorkloadHistogramArgsString(t, c, labels), backgroundDB, dur, url)
				c.Run(ctx, option.WithNodes(c.WorkloadNode()), cmd)
				return nil
			})
