...
func runAWSDMS(ctx context.Context, t test.Test, c cluster.Cluster) {
	...
	if err := func() error {
		...
		return errors.Newf("target is not in sync")
	}(); err != nil {
		t.Fatal(err)
	}
...
func setupAWSDMS(
	...
	if err := func() error {
		...
		if err := setupDMSEndpointsAndTasks(ctx, t, c, dmsCli, rdsCluster, awsdmsPassword, replicationARN); err != nil {
			return err
		}
		return nil
	}(); err != nil {
				...
				Parameters: []rdstypes.Parameter{
					{
						...
						// Using ApplyMethodImmediate will error.
						// We haven't started the cluster yet, so we can rely on this being setup on first
						// instantiation of the cluster.
...
func setupDMSEndpointsAndTasks(
	ctx context.Context,
	t test.Test,
	c cluster.Cluster,
	dmsCli *dms.Client,
