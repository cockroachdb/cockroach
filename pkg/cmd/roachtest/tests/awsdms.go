...
const (
	...
	awsdmsPassword       = "securepassword123"
	awsdmsDatabase       = "testdb"
...
func runAWSDMS(ctx context.Context, t test.Test, c cluster.Cluster) {
	...
	// Attempt a clean-up of new instances on startup.
	t.L().Printf("attempting to delete old instances")
	if err := tearDownDMSReplication(ctx, t.L(), rdsCli, dmsCli); err != nil {
		t.Fatal(err)
	}
	...
	defer func() {
		...
		if err := tearDownDMSReplication(context.Background(), t.L(), rdsCli, dmsCli); err != nil {
			t.L().Printf("failed to delete old instances on cleanup: %+v", err)
		}
	}()

	sourcePGConn, err := setupDMSReplication(ctx, t, c, rdsCli, dmsCli)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := func() error {
		...
		return errors.Newf("failed to wait for target to be in sync")
	}(); err != nil {
		t.Fatal(err)
	}
	...
	if err := func() error {
		for r := retry.StartWithCtx(ctx, waitForReplicationRetryOpts); r.Next(); {
			err := func() error {
				...
				expectedRows := awsdmsNumInitialRows + numExtraRows - 1
...
func setupDMSReplication(
	...
	g.Go(func() error {
		...
		t.L().Printf("pgurl: %s\n", pgURL)
		sourcePGConn, err = pgx.Connect(ctx, pgURL)
		if err != nil {
			return err
		}
		for _, stmt := range []string{
	...
	g.Go(func() error {
		...
		t.L().Printf("waiting for all replication instances to be available")
...
func tearDownDMSReplication(
	ctx context.Context, l *logger.Logger, rdsCli *rds.Client, dmsCli *dms.Client,
) error {
	// Delete DMS tasks, endpoints and replication instances that may be created.
	...
	if err != nil {
		...
		if wasRunning {
			l.Printf("waiting for tasks to be stopped")
		...
		l.Printf("waiting for tasks to be deleted")
	...
	g.Go(func() error {
		...
		if err != nil {
			...
			for _, rdsGroup := range rdsParamGroups.DBClusterParameterGroups {
				...
				// group and that may not be deleted yet- ideally we wait for the
				// cluster to get deleted. However, there is no provided
