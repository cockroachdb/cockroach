...
func (d *BackupRestoreTestDriver) verifyBackupCollection(
	...
	restoredTables, _, err := d.runRestore(ctx, l, rng, bc, checkFiles)
	if err != nil {
		return fmt.Errorf("error restoring backup: %w", err)
	}
	restoredContents, err := d.getRestoredContents(
		ctx, l, rng, restoredTables, bc, bc.restoreAOST, internalSystemJobs,
	)
	if err != nil {
		return err
	}
...
// a job success.
// It returns the tables that were restored and the database that was
// restored into.
func (d *BackupRestoreTestDriver) runRestore(
	ctx context.Context, l *logger.Logger, rng *rand.Rand, bc *backupCollection, checkFiles bool,
) ([]string, string, error) {
	...
	if err := d.testUtils.waitForJobSuccess(ctx, l, rng, jobID, false /* internalSystemJobs */); err != nil {
		return nil, "", err
	}
	return restoredTables, restoreDB, nil
}

// getRestoredContents loads the contents of the tables that were restored from
// the backup collection. This should be called after runRestore has been
// called.
...
// buildRestoreCmd builds the restore statement for the backup collection and
// returns the command, the tables being restored and the database being
// restored.
