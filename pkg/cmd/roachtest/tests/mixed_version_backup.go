...
func (mvb *mixedVersionBackup) waitForJobSuccess(
	...
	for r := retry.StartWithCtx(mvb.ctx, backupCompletionRetryOptions); r.Next(); {
		...
		err := h.QueryRow(rng, `SELECT status, payload FROM system.jobs WHERE id = $1`, jobID).Scan(&status, &payloadBytes)
		if err != nil {
		...
		if jobs.Status(status) == jobs.StatusFailed {
			...
			if err := protoutil.Unmarshal(payloadBytes, payload); err == nil {
				lastErr = fmt.Errorf("job %d failed with error: %w", jobID, err)
			} else {
...
func (mvb *mixedVersionBackup) runBackup(
	...
	case fullBackup:
		...
		collection = backupCollection{uri: fmt.Sprintf("nodelocal://%d/%s", destNode, name)}
	...
	stmt := fmt.Sprintf(
		"BACKUP TABLE %s INTO%s '%s' AS OF SYSTEM TIME '%s' WITH detached",
		mvb.table, latest, collection.uri, backupTime,
	)
...
func (mvb *mixedVersionBackup) verifyBackups(
	...
	l.Printf("verifying %d fingerprints created during this test", len(mvb.collections))
	for _, collection := range mvb.collections {
		...
		restoreDB := fmt.Sprintf(
			"RESTORE TABLE %s FROM LATEST IN '%s' WITH into_db = '%s'",
			mvb.table, collection.uri, intoDB,
		)
