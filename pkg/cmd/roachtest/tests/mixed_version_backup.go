...
func (bc *backupCollection) verifyOnlineRestore(
	...
	randomNode := d.testUtils.RandomNode(rng, d.testUtils.roachNodes)
	conn := d.testUtils.cluster.Conn(ctx, l, randomNode)
	defer conn.Close()
	parentDatabase := strings.Split(restoredTables[0], ".")[0]

	// Even during table restore, just check the whole database to avoid extra
	// casing.
	externalBytes, err := jobutils.CheckDownloadCompletenessOnDatabase(ctx, conn, parentDatabase)
	if err != nil {
		return nil, err
	}
	if externalBytes != 0 {
