...
func (u *CommonTestUtils) disableJobAdoption(
	...
	for _, node := range nodes {
		...
		eg.Go(func() error {
			...
			if err := u.cluster.RunE(ctx, option.OnNodes(u.cluster.Node(node)), "touch", sentinelFilePath); err != nil {
...
func (u *CommonTestUtils) enableJobAdoption(
	...
	for _, node := range nodes {
		...
		eg.Go(func() error {
			...
			if err := u.cluster.RunE(ctx, option.OnNodes(u.cluster.Node(node)), "rm -f", sentinelFilePath); err != nil {
