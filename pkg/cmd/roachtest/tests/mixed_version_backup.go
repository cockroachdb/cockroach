...
func (bc *backupCollection) verifyOnlineRestore(
	...
	for _, node := range d.testUtils.roachNodes {
		conn := d.testUtils.cluster.Conn(ctx, l, node)
		defer conn.Close()
		externalBytes, err := jobutils.GetExternalBytesOnNode(ctx, conn)
		if err != nil {
			return nil, err
		}
		if externalBytes != 0 {
			return nil, fmt.Errorf("download job %d did not download all data on node %d", downloadJobID, node)
		}
	}
