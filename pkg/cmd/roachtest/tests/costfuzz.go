...
func runOneRoundCostFuzz(
	...
	for i := 1; ; i++ {
		...
		if err := runCostFuzzQuery(conn, smither, logStmt); err != nil {
			t.Fatal(err)
		}
	}
}
...
func runCostFuzzQuery(conn *gosql.DB, smither *sqlsmith.Smither, logStmt func(string)) error {
	...
	seedStmt := fmt.Sprintf("SET testing_optimizer_random_cost_seed = %d", rng.Int63())
	_, err = conn.Exec(seedStmt)
	if err != nil {
		logStmt(seedStmt)
	...
	if err != nil {
		if pErr := (*pq.Error)(nil); errors.As(err, &pErr) && !strings.HasPrefix(string(pErr.Code), "XX") {
			// Ignore non-internal errors.
			return nil
		}
		logStmt(seedStmt)
		logStmt(stmt)
		return errors.Wrap(err, "internal error while running perturbed query")
	}
	defer rows2.Close()
