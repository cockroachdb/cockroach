...
func buildTopology(zones string) superRegionTopology {
	regionName := map[string]string{
...
func runSuperRegionFailover(ctx context.Context, t test.Test, c cluster.Cluster) {
	topo := buildTopology(c.Spec().GCE.Zones)
...
func runSurviveZoneFailure(
	...
	targetNode := topo.regionNodes["us-east1"][0] // n3
	...
	// USE test_sr so that REGIONAL BY ROW writes can resolve crdb_region.
	survivingUsEast := topo.regionNodes["us-east1"][1] // n4
	...
	verifyLeasesInRegion(ctx, t, sConn, topo, "test_sr.rbt_americas", "us-east1")
...
// verifyLeasesInRegion checks that all leases for a table are held by nodes
// in the expected region.
func verifyLeasesInRegion(
	ctx context.Context,
	t test.Test,
	conn *gosql.DB,
	...
	require.NoError(t, err, "failed to query lease holders for %s", table)
	require.NotEmpty(t, leaseHolders, "no ranges found for table %s", table)
	for _, lh := range leaseHolders {
		region := topo.nodeRegion[lh]
		require.Equal(t, expectedRegion, region,
			"lease holder n%d is in region %s, expected %s (table %s)",
			lh, region, expectedRegion, table)
	}
	t.L().Printf("verified all leases for %s are in %s", table, expectedRegion)
}
