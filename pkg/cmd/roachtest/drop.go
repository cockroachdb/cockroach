...
'`
			...
			for i := 0; i < 10; i++ {
				...
				for j := 1; j <= nodes; j++ {
					...
					// Return if the size of the directory is less than 100mb
					if size > 1.048E8 {
						c.l.printf(fmt.Sprintf("size 1 %s size actual %s\n", humanizeutil.IBytes(int64(size)), humanizeutil.IBytes(int64(1.024E8))))
			...
			if !allNodesSpaceCleared {
				sizeReport += "Disk space has not been freed within 10 minutes of deletion."
				t.Fatalf(sizeReport)
			}
