...
var urlMap = map[tpchBench]string{
	...
	tpch_vec: `https://raw.githubusercontent.com/cockroachdb/cockroach/master/pkg/workload/querybench/tpch-queries-vec`,
}
	...
	m.Go(func(ctx context.Context) error {
		...
		cmd := fmt.Sprintf(
			"./workload run querybench --db=tpch --concurrency=1 --query-file=%s "+
				"--num-runs=%d --max-ops=%d --use-vec=%t {pgurl%s} --histograms=logs/stats.json",
			filename,
			b.numRunsPerQuery,
			maxOps,
			b.benchType == tpch_vec,
			roachNodes,
		)
