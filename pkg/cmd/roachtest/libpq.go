...
func registerLibPQ(r *testRegistry) {
	runLibPQ := func(ctx context.Context, t *test, c *cluster) {
		...
		const (
			...
			resultsPath = "~/logs/report/libpq-results"
		)
		...
		_ = c.RunE(ctx, node, fmt.Sprintf("mkdir -p %s", resultsPath))
		...
		_ = c.RunE(
			ctx,
			node,
			fmt.Sprintf("cd %s && PGPORT=26257 PGUSER=root PGSSLMODE=disable go test -v 2>&1 | %s/bin/go-junit-report > %s/report.xml", libPQPath, goPath, resultsPath),
		)

		// TODO(asubuiotto): Do something with the results.
	}
