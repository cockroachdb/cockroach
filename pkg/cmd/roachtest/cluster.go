...
func (c *clusterImpl) FetchPebbleCheckpoints(ctx context.Context, l *logger.Logger) error {
	// Don't grab checkpoints on empty clusters or local runs.
	if c.spec.NodeCount == 0 || c.IsLocal() {
		return nil
	}
	...
	return timeutil.RunWithTimeout(ctx, "checkpoints", 3*time.Minute, func(ctx context.Context) error {
		...
		for storeIdx := 1; storeIdx <= numStores; storeIdx++ {
			...
			//
			// TODO(erikgrinaker): expose and use SyncedCluster.NodeDir(). We can't
			// use the {store-dir} expansion which only refers to the first store.
			storePath := fmt.Sprintf("/mnt/data%d/cockroach", storeIdx)
			var checkpointNodes option.NodeListOption
			results, err := c.RunWithDetails(ctx, l, c.All(), "sudo", "/bin/bash", "-c",
				fmt.Sprintf("'find %s/auxiliary -maxdepth 1 -type d -name checkpoints'", storePath))
			if err != nil {
				return err
			}
			for _, result := range results {
				...
				if output := strings.TrimSpace(result.Output(false /* decorate */)); output != "" {
					l.Printf("found Pebble checkpoints in n%d:%s", result.Node, storePath)
			...
			if len(checkpointNodes) > 0 {
				src := filepath.Join(storePath, "auxiliary", "checkpoints")
				dest := filepath.Join(c.t.ArtifactsDir(), "checkpoints", fmt.Sprintf("data%d", storeIdx))
				if err := os.MkdirAll(filepath.Dir(dest), 0755); err != nil {
					return err
				}
				if err := c.Get(ctx, c.l, src, dest, checkpointNodes); err != nil {
					return err
				}
			}
		}
