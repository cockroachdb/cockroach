...
func cockroachSqlSecureFlagsSlice() []string {
	if insecure {
		return []string{"--insecure"}
	}
	return []string{"--certs-dir", "certs"}
}
...
func (c *cluster) StopCockroachGracefullyOnNode(ctx context.Context, node int) error {
	...
	if err := c.RunE(ctx, c.Node(node), fmt.Sprintf("./cockroach quit --port=%s %s", port, cockroachSqlSecureFlags())); err != nil {
		return err
	}
