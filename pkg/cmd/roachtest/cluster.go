...
func (c *clusterImpl) FetchTimeseriesData(ctx context.Context, t test.Test) error {
	return contextutil.RunWithTimeout(ctx, "fetch tsdata", 5*time.Minute, func(ctx context.Context) error {
		i := 1
		for ; i <= c.spec.NodeCount; i++ {
			db, err := c.ConnE(ctx, i)
			if err == nil {
				err = db.Ping()
				db.Close()
			}
			if err != nil {
				t.L().Printf("node %d not responding to SQL, trying next one", i)
				continue
			}
			break
		}
		...
		if err := c.RunE(
			ctx, c.Node(i), "./cockroach debug tsdump --insecure --format=raw > tsdump.gob",
		); err != nil {
			return err
		}
		...
		return ioutil.WriteFile(tsDumpGob+"-run.sh", []byte(`#/usr/local/bin bash
