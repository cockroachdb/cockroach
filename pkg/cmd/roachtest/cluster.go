...
// StartOrFatal is like Start() except it takes a test and, on error, calls
// t.Fatal().
func (c *cluster) StartOrFatal(ctx context.Context, t *test, opts ...option) {
	if t.Failed() {
		// If the test has failed, don't try to limp along.
		return
	}
	FatalIfErr(t, c.Start(ctx, opts...))
}
