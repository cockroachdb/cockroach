...
// RunCommand runs a command on the specified node, returning install.RunResultDetails and an error.
// RunCommand should be the only Run command but multiple ones exist now because we are still migrating.
func (c *clusterImpl) RunCommand(
	ctx context.Context, l *logger.Logger, node option.NodeListOption, args ...string,
	...
	result := results[0] // only one result because only one node was passed
	if result.Err != nil {
