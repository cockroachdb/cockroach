...
func (f *clusterFactory) newCluster(
	...
	for i := 1; ; i++ {
		...
		if cfg.spec.Cloud != spec.Local {
			providersOptsContainer.SetProviderOpts(cfg.spec.Cloud, providerOpts)
		}
		...
		err = roachprod.Create(cfg.username, cfg.spec.NodeCount, createVMOpts, providersOptsContainer)
		if err == nil {
			if err := f.r.registerCluster(c); err != nil {
				return nil, err
			}
