...
func updateSpecForSelectiveTests(ctx context.Context, specs []registry.TestSpec) {
	...
	// run and select 30% of successful tests which gives a window of 6 days for all tests to run
	selectedTests, err := testselector.CategoriseTests(ctx,
		testselector.NewDefaultSelectTestsReq(30, roachtestflags.Cloud, roachtestflags.Suite))
	if err != nil {
