...
func init() {
	...
	for app, flags := range apps {
		...
		for _, n := range []int{3, 6} {
			tests.Add(testSpec{
				Name:  fmt.Sprintf("sqlapp/%s/nodes=%d", app, n),
				Nodes: nodes(n + 1),
...
func runSqlapp(ctx context.Context, t *test, c *cluster, app, flags string, dur time.Duration) {
	...
	b, err := binfetcher.Download(ctx, binfetcher.Options{
		Component: "rubrik",
		Binary:    app,
		// TODO DURING REVIEW: We could use LATEST, but I suspect reproducibility
		// is more important than automatically picking up changes.
		Version: "e83e992a9f4bb03fd2c9c6ed5d7313d868fbf269",
		GOOS:    "linux",
		GOARCH:  "amd64",
	})
	if err != nil {
	...
	c.Put(ctx, b, app, c.Node(appNode))
