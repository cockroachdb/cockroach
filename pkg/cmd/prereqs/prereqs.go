...
func runW(w io.Writer, path, target string, includeTest bool) error {
...
func run(path, target string, includeTest bool) error {
	f, err := ioutil.TempFile("", "prereqs")
	if err != nil {
		return err
	}
	defer f.Close()

	if err := runW(f, path, target, includeTest); err != nil {
		_ = os.Remove(f.Name())
		return err
	}
	return os.Rename(f.Name(), target+".d")
}

func main() {
	includeTest := flag.Bool("test", false, "include test dependencies")
	flag.Usage = func() { fmt.Fprintf(os.Stderr, "usage: %s [-test] <package> <target>\n", os.Args[0]) }
	flag.Parse()
	if flag.NArg() != 2 {
		flag.Usage()
		os.Exit(1)
	...
	if err := run(flag.Arg(0), flag.Arg(1), *includeTest); err != nil {
