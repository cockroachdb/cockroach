...
// when raw descriptors from a backup that could predate that migraiton, and
// thus not have that field populated, are injected into the catalog, they are
// migrated accordingly to uphold the invarient that all descriptors in the
// catalog are migrated. Such a migration would use `RestoreActionImplmented`
// and then a sentence specifying what is done during restore, for example:
// RestoreActionImplmented("new field is populated during allocateDescriptorRequests").
//
...
// RestoreActionNotRequired is used to signal an upgrade migraion only affects
...
// RestoreActionImplmented is used to signal an upgrade migration affects state
// that is written by restore and has implmented the requisit logic in restore
// to perform the equlivilant upgrade on restored data as it is restored. The
// string should mention what is done during restore and where it is done.
type RestoreActionImplmented = RestoreBehavior
