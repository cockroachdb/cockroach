...
func runTestRoleOptionsMigration(t *testing.T, numUsers int) {
	...
	if numUsers > 100 {
		...
		tdb.QueryRow(t, `SELECT last_value FROM system.role_id_seq`).Scan(&id)
		id-- // last_value has not yet been assigned to a user.
	}
	...
	tdb.CheckQueryResults(t, `SELECT * FROM system.users WHERE username = 'testuser'`, [][]string{
	...
	tdb.CheckQueryResults(t, `SELECT * FROM system.role_options WHERE username = 'testuser' or username = 'testuser_last'`, [][]string{
		{"testuser", "CONTROLJOB", "NULL", "100"},
		{"testuser", "CREATEROLE", "NULL", "100"},
		{"testuser", "NOLOGIN", "NULL", "100"},
		{"testuser", "VALID UNTIL", "2021-01-01 00:00:00+00:00", "100"},
		{"testuser_last", "CREATEROLE", "NULL", fmt.Sprintf(`%d`, id+1)},
	})
	...
	row := tc.Conns[0].QueryRow(`SELECT create_statement FROM [SHOW CREATE TABLE system.role_options]`)
