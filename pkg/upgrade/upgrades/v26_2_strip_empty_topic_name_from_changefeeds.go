...
// param. Stripping the param does not change changefeed behavior because,
// prior to V26_2_ChangefeedsRejectEmptyTopicName, an empty topic_name
// param is treated the same as not having one at all.
//
...
func stripEmptyTopicNameFromChangefeeds(
	...
	// Collect non-terminal changefeed job IDs.
	// It's okay if we miss any new ones since the previous version gate
	// prevents an empty topic_name from being added.
	var jobIDs []jobspb.JobID
	if err := d.DB.Txn(ctx, func(ctx context.Context, txn isql.Txn) error {
		var err error
		jobIDs, err = jobs.RunningJobs(ctx, d.Settings, jobspb.InvalidJobID, txn, jobspb.TypeChangefeed)
		return err
	}); err != nil {
		return err
	}

	// Strip from each changefeed job payload in its own transaction to avoid
	// a single large transaction if there are many changefeed jobs.
	for _, jobID := range jobIDs {
		if err := d.DB.Txn(ctx, func(ctx context.Context, txn isql.Txn) error {
			return maybeStripEmptyTopicName(ctx, txn, jobID)
		}); err != nil {
			return err
		}
	}

	// Collect external connection names.
	// It's okay if we miss any new ones since the previous version gate
	// prevents an empty topic_name from being added.
	var ecNames []string
	if err := d.DB.Txn(ctx, func(ctx context.Context, txn isql.Txn) error {
		...
		for _, row := range rows {
			ecNames = append(ecNames, string(tree.MustBeDString(row[0])))
		}
		return nil
	}); err != nil {
		return err
	...
	// Strip from each external connection in its own transaction to avoid
	// a single large transaction if there are many external connections.
	for _, name := range ecNames {
		if err := d.DB.Txn(ctx, func(ctx context.Context, txn isql.Txn) error {
			return maybeStripEmptyTopicNameFromExternalConnection(ctx, txn, name)
		}); err != nil {
			return err
		}
	}
...
func maybeStripEmptyTopicNameFromExternalConnection(
	...
	if err != nil {
		// The external connection may have been dropped between when we
		// collected the names and now.
