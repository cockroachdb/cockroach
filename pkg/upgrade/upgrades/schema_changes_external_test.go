	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			migrationFunc, expectedDescriptor := upgrades.
				MakeFakeTestingMigrationForSchemaChangesTest()
			...
			tdb.Exec(t, upgrades.TestingCreateTableAfter)
			var desc catalog.TableDescriptor
			s.CollectionFactory().(*descs.CollectionFactory).Txn(ctx, s.DB(), func(
			...
			tdb.Exec(t, upgrades.TestingCreateTableQueryBefore)
