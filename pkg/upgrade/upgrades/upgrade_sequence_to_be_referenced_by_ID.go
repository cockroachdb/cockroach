...
func upgradeSetUpForTableOrView(
	...
	tableDesc, err := descriptors.MustGetMutableTableByID(ctx, txn, idToUpgrade)
	if err != nil {
		return nil, nil, nil, err
	}

	// Get the database of the table to pass to the planner constructor.
	dbDesc, err := descriptors.MustGetImmutableDatabaseByID(ctx, txn, tableDesc.GetParentID())
	if err != nil {
		return nil, nil, nil, err
	}
