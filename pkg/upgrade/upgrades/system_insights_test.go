...
func TestSystemInsights(t *testing.T) {
	...
	upgrades.Upgrade(
		t,
		sqlDB,
		clusterversion.V23_2_AddSystemInsightsTable-1,
		nil,
		false,
	)
	...
	var seqVal int
	err := sqlDB.QueryRow(`SELECT data FROM system.public.insights`).Scan(&seqVal)
	require.Error(t, err)
	...
	upgrades.Upgrade(
		t,
		sqlDB,
		clusterversion.V23_2_AddSystemInsightsTable,
		nil,
		false,
	)
