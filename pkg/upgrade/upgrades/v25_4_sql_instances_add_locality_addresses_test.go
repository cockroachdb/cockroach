...
func TestSqlInstancesAddlocalityAddresses(t *testing.T) {
	...
	_, err = db.Exec("SELECT locality_addresses FROM system.sql_instances WHERE id = 1001")
	require.Error(t, err, "locality_addresses column should not exist before upgrade")

	// Perform the upgrade
	upgrades.Upgrade(t, db, clusterversion.V25_4_SQLInstancesAddLocalityAddresses, nil, false)

	// Verify the column exists now and the row is still there
	var binVer string
	err = db.QueryRow(
		"SELECT binary_version FROM system.sql_instances WHERE id = 1001 AND locality_addresses IS NULL",
	).Scan(&binVer)
	...
	var localityAddresses []byte
	err = db.QueryRow(
		"SELECT locality_addresses FROM system.sql_instances WHERE id = 1001",
	).Scan(&localityAddresses)
	require.NoError(t, err)

	// Parse and verify the JSON content
	var addresses []map[string]string
	err = json.Unmarshal(localityAddresses, &addresses)
	require.NoError(t, err)
