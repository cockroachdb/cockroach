...
func createRegionLivenessTables(
	...
	if err := d.DB.DescsTxn(ctx, func(ctx context.Context, txn descs.Txn) error {
		...
		tablePrefix := keys.SystemSQLCodec.TablePrefix(keys.RegionLivenessTableID)
	...
	if err := createSystemTable(ctx, d.DB.KV(), d.Settings, keys.SystemSQLCodec,
		systemschema.RegionLivenessTable); err != nil {
		return err
	}
	...
	if setDBLocality {
		if err := d.DB.Txn(ctx, func(ctx context.Context, txn isql.Txn) error {
			_, err := txn.Exec(ctx, "set-regional-liveness-global", txn.KV(), "ALTER TABLE system."+string(catconstants.RegionalLiveness)+" SET LOCALITY GLOBAL;")
			return err
		}); err != nil {
			return err
		}
