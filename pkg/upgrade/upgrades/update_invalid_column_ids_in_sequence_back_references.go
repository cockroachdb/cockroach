...
func updateInvalidColumnIDsInSequenceBackReferences(
	...
	// Repeatedly start a new transaction that scans all sequences and update invalid
	// column IDs, if any, in their back references, until there are no repairs needed.
	for {
		hasUpgrade := false
		...
		if err := d.CollectionFactory.TxnWithExecutor(ctx, d.DB, d.SessionData, func(
			...
			for {
				...
				hasUpgrade, err = maybeUpdateInvalidColumnIdsInSequenceBackReferences(ctx, txn, idToUpgrade, descriptors)
				if err != nil {
					return err
				}
				if hasUpgrade {
		...
		// Break out of the loop if we have reached the fixed point.
		if !hasUpgrade {
			break
		}
	}
