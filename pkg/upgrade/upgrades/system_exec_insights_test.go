...
func TestExecSystemInsights(t *testing.T) {
	...
	_, err := sqlDB.Exec("SELECT * FROM system.public.stmt_exec_insights")
	require.NoError(t, err, "system.public.stmt_exec_insights exists")

	_, err = sqlDB.Exec("SELECT * FROM system.public.txn_exec_insights")
	require.NoError(t, err, "system.public.txn_exec_insights exists")
