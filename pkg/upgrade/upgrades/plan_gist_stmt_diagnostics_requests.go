...
ALTER TABLE system.statement_diagnostics_requests
  ADD COLUMN IF NOT EXISTS plan_gist STRING NULL FAMILY "primary",
  ADD COLUMN IF NOT EXISTS anti_plan_gist BOOL NULL FAMILY "primary"`

	createCompletedIdxV2 = `
CREATE INDEX IF NOT EXISTS completed_idx_v2 ON system.statement_diagnostics_requests (completed, ID)
