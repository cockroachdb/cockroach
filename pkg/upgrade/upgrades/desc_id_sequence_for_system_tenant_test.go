...
WHERE id = %d`,
	...
	preMigrationCreateOld := getOld()
	require.Less(t, initOld, preMigrationCreateOld, "legacy counter should be bumped")
	require.Zero(t, getNew(), "sequence should be zero before upgrade")
	...
	// Descriptor ID creation should fail.
	const errRE = `descriptor ID generator unavailable, migration in progress, retry later`
	tdb.ExpectErr(t, errRE, `CREATE TABLE t2 (x INT PRIMARY KEY)`)

	// Check that the counters remain unchanged.
	require.Equal(t, preMigrationCreateOld, getOld(), "legacy counter should remain unchanged")
	require.Zero(t, getNew(), "sequence should be zero before upgrade")
	...
	require.Equal(t, preMigrationCreateOld, getOld(), "legacy counter should remain unchanged")
	require.Equal(t, preMigrationCreateOld, getNew(), "sequence should have legacy counter value")

	// Descriptor ID creation should succeed.
	tdb.Exec(t, `CREATE TABLE t2 (x INT PRIMARY KEY)`)

	// Check that the sequence is now in use.
	require.Equal(t, preMigrationCreateOld, getOld(), "legacy counter should remain unchanged")
	require.Less(t, preMigrationCreateOld, getNew(), "sequence counter should be bumped")
}
