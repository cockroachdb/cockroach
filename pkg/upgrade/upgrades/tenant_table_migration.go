...
GENERATE ALWAYS AS (crdb_internal.pb_to_json('cockroach.sql.sqlbase.TenantInfo', info)->>'name') VIRTUAL
`

const addTenantNameIndex = `
CREATE UNIQUE INDEX tenant_name_idx ON system.public.tenants (name ASC)
`

const addSystemTenantEntry = `
UPSERT INTO system.public.tenants (id, active, info)
VALUES (1, true, crdb_internal.json_to_pb('cockroach.sql.sqlbase.TenantInfo', '{"id":1,"state":0,"name":"system"}'))
`
   ...
   AND n2.name = 'tenants'`)
	...
	for _, op := range []operation{
		...
		},
		{
			...
			schemaList:     []string{"tenant_name_idx"},
