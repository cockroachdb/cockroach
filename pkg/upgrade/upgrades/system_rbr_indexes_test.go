...
func TestBackfillRegionalByRow(t *testing.T) {
	...
	// Delete old index is expected to remove all kvs from the previous index
	// and leave the new index untouched.
	// everything in the old index.
...
func patchDescriptor(desc catalog.TableDescriptor) catalog.TableDescriptor {
	...
	table.SetPrimaryIndex(descpb.IndexDescriptor{
		...
		Unique: true,
		// TODO: get key from existing descriptor
		KeyColumnNames:      []string{"crdb_region", "key"},
		KeyColumnIDs:        []catid.ColumnID{crdbRegionColumn.ID, 1},
