...
func TestRaftAppliedIndexTermMigration(t *testing.T) {
	...
	makeArgs := func() (args base.TestServerArgs) {
		...
		args.Knobs.Server = &server.TestingKnobs{
			// Start at the version immediately preceding the upgrade.
	...
	testutils.SucceedsSoon(t, func() error {
		return forAllReplicas(tc, func(repl *kvserver.Replica) error {
			...
			// the highest Upgrade() command that has been executed on the range. So
			// the upgrade will set it to AddRaftAppliedIndexTermMigration, since
			// there is no Range upgrade associated with
