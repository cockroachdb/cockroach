...
func SQLInstancesAddLocalityAddressesColumn(
	...
	// This upgrade adds a new column to the sql_instances table to store the locality aware addresseses.
	...
	op := operation{
		...
		query: `ALTER TABLE system.sql_instances 
  		ADD COLUMN IF NOT EXISTS locality_addresses JSONB DEFAULT NULL FAMILY "primary"`,
		// The locality_addresses column won't need to be backfilled and is nullable.
	}
