...
func backfillSystemUsersIDColumnAndAddIndex(
	...
	// Query all the usernames upfront to do only one full table scan.
	// Update 10000 usernames at a time so we don't have unbound
	// username slices.
	const batchSize = 10000
	// NB: We only have to run the query below once since by the time
	// we perform this migration, all NEWLY created users will have IDs.
	...
	for {
		// Avoid making an unbound length array.
		...
		// Preallocate IDs in one call to kv.IncrementValRetryable (one
		// round trip).
...
UPDATE system.users
   SET user_id = id
  FROM (
        SELECT unnest($1:::STRING[]) AS name,
               unnest($2:::INT4[]) AS id
       )
 WHERE username = name;`
		_, err = d.InternalExecutor.ExecEx(ctx, "update-role-ids", nil,
			sessiondata.NodeUserSessionDataOverride, updateUserIDs, usernames, ids)
		if err != nil {
			return err
		}
	}
