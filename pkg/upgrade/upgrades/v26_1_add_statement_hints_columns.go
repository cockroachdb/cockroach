...
ALTER TABLE system.statement_hints
  ...
  FAMILY "primary"
`
	addHintWhereExprColumn = `
ALTER TABLE system.statement_hints
  ADD COLUMN IF NOT EXISTS "where_expr" STRING
  FAMILY "primary"
`
)
...
func statementHintsAddColumnsTableMigration(
	...
	for _, op := range []operation{
		...
		{
			name:           "add-where-expr-column-to-statement-hints",
			schemaList:     []string{"where_expr"},
			query:          addHintWhereExprColumn,
			schemaExistsFn: hasColumn,
		},
	} {
...
var backfillHintTypeColumn = fmt.Sprintf(`
UPDATE system.statement_hints
  SET "hint_type" = '%s'
  WHERE "hint_type" IS NULL
  LIMIT 1000
`, hintpb.HintTypeRewriteInlineHints,
)

func statementHintsBackfillTypeColumnTableMigration(
	ctx context.Context, version clusterversion.ClusterVersion, deps upgrade.TenantDeps,
) error {
	for {
		rowsAffected, err := deps.InternalExecutor.ExecEx(
			ctx, "backfill-hint-type-column-in-statement-hints", nil, /* txn */
			sessiondata.NodeUserSessionDataOverride, backfillHintTypeColumn,
		)
		if err != nil {
			return err
		}
