...
func TestStatsTablesAutostatsFunctionMigration(t *testing.T) {
	...
	if err := sqlDB.QueryRow("SELECT create_statement FROM [SHOW CREATE TABLE system.statement_statistics]").Scan(&createStatmentStats); err != nil {
		t.Fatal(err)
	}
	require.NotContains(t, createStatmentStats,
	...
	if err := sqlDB.QueryRow("SELECT create_statement FROM [SHOW CREATE TABLE system.transaction_statistics]").Scan(&createTransactionStats); err != nil {
		t.Fatal(err)
	}
	require.NotContains(t, createTransactionStats,
		`WITH (sql_stats_automatic_collection_fraction_stale_rows = 4, sql_stats_automatic_partial_collection_fraction_stale_rows = 1);`)
	...
	var createStatementStatsAfter string
	if err := sqlDB.QueryRow("SELECT create_statement FROM [SHOW CREATE TABLE system.statement_statistics]").Scan(&createStatementStatsAfter); err != nil {
		t.Fatal(err)
	}
	require.Contains(t, createStatementStatsAfter,
		`WITH (sql_stats_automatic_collection_fraction_stale_rows = 4, sql_stats_automatic_partial_collection_fraction_stale_rows = 1);`)

	var createTransactionStatsAfter string
	if err := sqlDB.QueryRow("SELECT create_statement FROM [SHOW CREATE TABLE system.transaction_statistics]").Scan(&createTransactionStatsAfter); err != nil {
		t.Fatal(err)
	}
	require.Contains(t, createTransactionStatsAfter,
		`WITH (sql_stats_automatic_collection_fraction_stale_rows = 4, sql_stats_automatic_partial_collection_fraction_stale_rows = 1);`)
}
