...
func TestFirstUpgradeRepairBatchSize(t *testing.T) {
	...
	const totalDescriptorsToTest = 5000
	sqlRunner := sqlutils.MakeSQLRunner(sqlDB)
	idb := testServer.InternalDB().(*sql.InternalDB)
	for i := 0; i < totalDescriptorsToTest; i++ {
		sqlRunner.Exec(t, fmt.Sprintf("CREATE TABLE t_%d(n int)", i))
		sqlRunner.Exec(t, fmt.Sprintf("SELECT 't_%d'::REGCLASS::OID", i))
