...
func TestRoleMembersIDMigration11000Users(t *testing.T) {
	skip.UnderRace(t)
	skip.UnderStress(t)
	runTestRoleMembersIDMigration(t, 11000)
}
...
func runTestRoleMembersIDMigration(t *testing.T, numUsers int) {
	...
	_, err = tc.Conns[0].ExecContext(ctx, `SET CLUSTER SETTING version = $1`,
		clusterversion.ByKey(clusterversion.V23_1RoleMembersTableHasIndexesForIDColumns).String())
	require.NoError(t, err)
	_, err = tc.Conns[0].ExecContext(ctx, `SET CLUSTER SETTING version = $1`,
	...
	// TODO check some rows from table

	// TODO compare expected schema
