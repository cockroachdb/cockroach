...
func TestAlterSystemJobsTableAddJobTypeColumn(t *testing.T) {
	...
	var (
		validationSchemas = []upgrades.Schema{
			...
			{Name: "jobs_type_idx", ValidationFn: upgrades.HasIndex},
		}
	)
	...
	// Run the first upgrade and ensure the type column exists.
	upgrades.Upgrade(
		t,
		sqlDB,
	...
	// Verify old rows have not been backfilled.
	var count int
	...
	// Run the next upgrade and ensure the type column is backfilled.
	upgrades.Upgrade(
		t,
		sqlDB,
	...
	var typStr string
	rows, err := sqlDB.Query("SELECT distinct(type) FROM system.jobs")
	require.NoError(t, err)
