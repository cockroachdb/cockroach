	...
	for name, descriptorStringsToInject := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			for _, s := range strings.Split(strings.TrimSpace(descriptorStringsToInject), "\n") {
				...
				if databaseDescriptor := desc.GetDatabase(); databaseDescriptor != nil {
					privileges = databaseDescriptor.Privileges
				} else if schemaDescriptor := desc.GetSchema(); schemaDescriptor != nil {
					privileges = schemaDescriptor.Privileges
				} else if tableDesciptor := desc.GetTable(); tableDesciptor != nil {
					privileges = tableDesciptor.Privileges
				} else if typeDescriptor := desc.GetType(); typeDescriptor != nil {
					privileges = typeDescriptor.Privileges
				} else {
					t.FailNow()
				}
				for _, user := range privileges.Users {
					if user.User().Normalized() == "test_user" {
						require.True(t, privilege.DEPRECATED_GRANT.IsSetIn(user.Privileges))
					}
				}
				...
				ServerArgs: base.TestServerArgs{
					Knobs: base.TestingKnobs{
						Server: &server.TestingKnobs{
							...
							BinaryVersionOverride:          clusterversion.ByKey(clusterversion.Start22_2 - 1), // changed cluster version
						},
					},
				},
			})
			...
			_, err := tc.Conns[0].ExecContext(ctx, `SET CLUSTER SETTING version = $1`,
				clusterversion.ByKey(clusterversion.Start22_2).String())
			require.NoError(t, err)
			...
			err = sql.TestingDescsTxn(ctx, tc.Server(0), func(ctx context.Context, txn *kv.Txn, col *descs.Collection) error {
				...
				for _, desc := range cat.OrderedDescriptors() {
					...
					for _, u := range privilegeDesc.Users {
						if privilege.DEPRECATED_GRANT.IsSetIn(u.Privileges) {
							return errors.Newf(
