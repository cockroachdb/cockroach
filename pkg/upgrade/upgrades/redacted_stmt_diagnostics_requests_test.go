...
func TestStmtDiagRedactedMigration(t *testing.T) {
		...
		ServerArgs: base.TestServerArgs{
			Knobs: base.TestingKnobs{
				Server: &server.TestingKnobs{
					...
					OverrideClusterVersion:         (clusterversion.V24_2_StmtDiagRedacted - 1).Version(),
				},
			},
		},
	}
	...
	var (
		validationStmts = []string{
			`SELECT redacted FROM system.statement_diagnostics_requests LIMIT 0`,
			`SELECT redacted FROM system.statement_diagnostics_requests@completed_idx LIMIT 0`,
		}
		validationSchemas = []upgrades.Schema{
			{Name: "redacted", ValidationFn: upgrades.HasColumn},
			{Name: "primary", ValidationFn: upgrades.HasColumnFamily},
			{Name: "completed_idx", ValidationFn: upgrades.HasIndex},
	...
	upgrades.Upgrade(
		t,
		sqlDB,
		clusterversion.V24_2_StmtDiagRedacted,
		nil,   /* done */
		false, /* expectError */
	)
...
// before adding the redacted column to the current version.
