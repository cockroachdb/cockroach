...
// GetMigration returns the upgrade associated with this key.
func (m *Manager) GetMigration(key clusterversion.ClusterVersion) (upgrade.Migration, bool) {
	if m.knobs.RegistryOverride != nil {
		if m, ok := m.knobs.RegistryOverride(key); ok {
			return m, ok
		}
	}
	return upgrades.GetMigration(key)
}
...
func (m *Manager) runMigration(
	...
	_, isSystemMigration := mig.(*upgrade.SystemMigration)
...
func (m *Manager) checkPreconditions(
	...
	for _, v := range versions {
		...
		tm, ok := mig.(*upgrade.TenantMigration)
		if !ok {
			continue
		}
