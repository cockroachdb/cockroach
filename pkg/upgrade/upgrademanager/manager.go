...
func (m *Manager) Migrate(
	...
	// version is executed before any node (or pod) in the cluster has the
...
func validateTargetClusterVersion(
	...
	return forEveryNodeUntilClusterStable(ctx, op, c, func(
		...
		if err != nil && strings.Contains(err.Error(), "unknown service cockroach.server.serverpb.Migration") {
			err = errors.Handled(errors.Wrap(err, "validate cluster version failed: some tenant pods running on binary less than 23.1"))
		}
