...
func safeToUpgradeIfPhysicalReplicationJobsExist(
	...
	for _, c := range targetClusters {
		...
		if c.version.LessEq(from.Version) {
			continue
		}
	...
	if len(clustersBlockingUpgrades) > 0 {
		...
		return errors.Newf("cannot upgrade to %s because of physical replication source clusters: ", to, badClusterMsg.String())
	}
...
func getPhysicalReplicationTargetClusters(
	...
	if err := db.Txn(ctx, func(ctx context.Context, txn isql.Txn) (txErr error) {
		query := `SELECT name, job_id FROM (SELECT name, crdb_internal.pb_to_json('cockroach.multitenant.ProtoInfo', info)->>'physicalReplicationConsumerJobId' AS job_id FROM system.tenants) WHERE job_id IS NOT NULL;`
...
func (m *Manager) Migrate(
	...
	if err := safeToUpgradeIfPhysicalReplicationJobsExist(ctx, m.settings, m.clusterID, m.deps.DB, from, to); err != nil {
		return err
	}
