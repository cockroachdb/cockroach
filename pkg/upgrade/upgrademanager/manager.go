...
func (m *Manager) RunPermanentUpgrades(ctx context.Context, upToVersion roachpb.Version) error {
	...
	if len(permanentUpgrades) == 0 {
		...
		return m.runPermanentMigrationsWithoutJobsForTests(startup.RetryContext(ctx), user)
	}
	...
	lastUpgradeCompleted, err := startup.RunWithStartupRetryEx(ctx, "check if migration completed",
	...
	for _, u := range permanentUpgrades {
		...
		if startupMigrationAlreadyRan {
			...
			if err := startup.RunWithStartupRetry(ctx, "mark upgrade complete", func(ctx context.Context) (err error) {
...
func checkOldStartupMigrationRan(
	...
	kv, err := startup.RunWithStartupRetryEx(ctx, "check old startup migration",
...
func (m *Manager) runMigration(
	...
	if !useJob {
		...
		return startup.RunWithoutRetry(ctx, func(ctx context.Context) error {
		...
		if err := startup.RunWithStartupRetry(ctx, "upgrade create job", func(ctx context.Context) (err error) {
		...
		if alreadyExisting {
			...
			return startup.RunWithStartupRetry(ctx, "upgrade wait jobs", func(ctx context.Context) error {
				return m.jr.WaitForJobs(ctx, []jobspb.JobID{id})
			})
		} else {
			log.Infof(ctx, "running %s", mig.Name())
			return startup.RunWithStartupRetry(ctx, "upgrade run jobs", func(ctx context.Context) error {
