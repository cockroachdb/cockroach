...
func (m *Manager) RunPermanentUpgrades(ctx context.Context, upToVersion roachpb.Version) error {
	...
	lastUpgradeCompleted, err := startup.RunWithRetryEx(ctx, "check if migration completed",
	...
	for _, u := range permanentUpgrades {
		...
		if startupMigrationAlreadyRan {
			...
			if err := startup.RunWithRetry(ctx, "mark upgrade complete", func(ctx context.Context) (err error) {
...
func checkOldStartupMigrationRan(
	...
	kv, err := startup.RunWithRetryEx(ctx, "check old startup migration",
...
func (m *Manager) runMigration(
	...
	if !useJob {
		...
		if err := startup.RunWithRetry(ctx, "upgrade create job", func(ctx context.Context) (err error) {
		...
		if alreadyExisting {
			...
			return startup.RunWithRetry(ctx, "upgrade wait jobs", func(ctx context.Context) error {
				return m.jr.WaitForJobs(ctx, []jobspb.JobID{id})
			})
		} else {
			log.Infof(ctx, "running %s", mig.Name())
			return startup.RunWithRetry(ctx, "upgrade run jobs", func(ctx context.Context) error {
