...
func (m *Manager) RunPermanentUpgrades(ctx context.Context, upToVersion roachpb.Version) error {
	...
	for _, u := range permanentUpgrades {
		...
		// was introduced. If it has, we don't want to run this upgrade. Allowing
		// the upgrade to run could be destructive. Specifically, the permanent
		// upgrade that adds the root user will unconditionally remove any password
		// that has been set.
		//
