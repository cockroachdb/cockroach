...
func computeTruncateDecision(input truncateDecisionInput) truncateDecision {
	...
	// XXX: RecentlyActive is used as "RecentlyActive", confirm via commit
	// message this is the behavior we want/expect. We basically are using that
	// for getBehindIndex for the nodes we're interested in.
	// XXX: Why can't we use input.RaftStatus.Commit instead? CommitIndex and
	// RaftStatus.Commit are far too similar for comfort. Why was it renamed
	// from getBehindIndex? We're already using raftStatus.Progress below.
	...
	// XXX: Ask about this during review, if still applicable.
	...
	//             NewFirstIndex <= CommitIndex (if != 0)
	//
	...
	valid := (decision.NewFirstIndex >= input.FirstIndex) &&
		...
		(decision.NewFirstIndex <= decision.CommitIndex || decision.CommitIndex == 0)
	if !valid {
