...
// a quorum has acked the truncation index. Exceptions are made for replicas
// for which information is missing ("probing state") as long as they are known
// to have been online recently, and for in-flight snapshots (in particular
// preemptive snapshots) which are not adequately reflected in the Raft status
// and would otherwise be cut off with regularity.
// A challenge for the log truncation is to deal with sideloaded log entries,
// that is, entries which contain SSTables for direct ingestion into the storage
...
//
// The raft log size used in the decision making progress is unfortunately not
// very robust as it suffers from two limitations at the time of writing:
