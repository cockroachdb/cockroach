...
type PebbleFileRegistry struct {
	...
	// Version is the cluster version handle for the cluster.
	// TODO(ayang): remove this field when we deprecate the old registry
...
func (r *PebbleFileRegistry) loadRegistryFromFile() error {
	...
	err = r.maybeLoadNewRecordsRegistry()
	if err != nil {
		return err
	}
	return nil
...
func (r *PebbleFileRegistry) maybeLoadNewRecordsRegistry() error {
	...
	rdr, err := rr.Next()
	if err == io.EOF {
		return errors.New("pebble new file registry exists but is missing a header")
	}
	if err != nil {
		return err
	}
	...
	if registryHeader.Version == enginepb.RegistryVersion_Base {
		return errors.New("new encryption registry with version Base should not exist")
	}
...
func (r *PebbleFileRegistry) maybeElideEntries() error {
	...
	if filesChanged {
		return r.rewriteOldRegistry(newProto)
	}
	return nil
}
...
func (r *PebbleFileRegistry) SetFileEntry(filename string, entry *enginepb.FileEntry) error {
	...
	r.mu.Lock()
	defer r.mu.Unlock()
	if err := r.maybeUpgradeRegistryVersion(); err != nil {
		return err
	}
...
func (r *PebbleFileRegistry) maybeUpgradeRegistryVersion() error {
	if r.Version.IsActive(context.Background(), clusterversion.RecordsBasedRegistry) {
		return r.upgradeToRecordsVersion()
	}
	return nil
}
