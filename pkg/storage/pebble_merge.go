...
func (t *MVCCValueMerger) returnToPool() {
	...
	t.metaSubset.RawBytes = t.metaSubset.RawBytes[:0]
	t.metaSubset.MergeTimestamp = nil
	// TODO(radu): we should also reuse slices inside meta.
	t.meta = enginepb.MVCCMetadata{}
	t.inPool = true
...
func (t *MVCCValueMerger) deserializeMVCCValueAndAppend(value []byte) error {
	...
	if t.meta.RawBytes[mvccTagPos] == byte(roachpb.ValueType_TIMESERIES) {
		...
		if err := ts.UnmarshalReusingSlices(t.meta.RawBytes[mvccHeaderSize:]); err != nil {
...
func (t *MVCCValueMerger) Finish(includesBase bool) ([]byte, io.Closer, error) {
	...
	// We use our custom marshaller which avoids heap allocations when encoding
	// arrays of varints.
	marshaller := roachpb.MakeInternalTimeSeriesDataMarshaller(merged)

	meta := &t.metaSubset // avoid allocation
	rawBytesSize := mvccHeaderSize + marshaller.Size()
	...
	marshaller.MarshalToSizedBuffer(meta.RawBytes[mvccHeaderSize:])
	res, err := protoutil.Marshal(meta)
	if err != nil {
		return nil, nil, err
	}
