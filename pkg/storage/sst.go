...
func CheckSSTConflicts(
	...
	var writeIntentErr = new(roachpb.WriteIntentError)
	...
	for extErr == nil && sstErr == nil && extOK && sstOK && ctx.Err() == nil {
		...
		if !extKey.IsValue() {
			...
			if len(mvccMeta.RawBytes) > 0 {
				...
				// in order to return a large batch for intent resolution.
				writeIntentErr.Intents = append(writeIntentErr.Intents,
					roachpb.MakeIntent(mvccMeta.Txn, extIter.Key().Key))
				if int64(len(writeIntentErr.Intents)) >= maxIntents {
	...
	case len(writeIntentErr.Intents) > 0:
		return enginepb.MVCCStats{}, writeIntentErr
	case ctx.Err() != nil:
