...
func CheckSSTConflicts(
	...
	for extErr == nil && sstErr == nil && extOK && sstOK {
		...
		// If disallowShadowing is true and the ingested key/value pair is identical
		// to an existing one then we do not consider it to be a collision.
		//
		// TODO(erikgrinaker): We retain this behavior mostly for backwards
		// compatibility. It can result in e.g. emitting rangefeed events below the
		// closed timestamp, so it should be removed in 22.2 if possible.
