...
func errorIfReaderContainsRangeKeys(
	ctx context.Context, reader Reader, UpperBound roachpb.Key,
) error {
	...
	if ok, err := engRKIter.Valid(); err != nil {
		...
		return errors.New("engine contains range keys")
	}
	return nil
}
...
func ComputeSSTStatsDiff(
	...
	if err := errorIfReaderContainsRangeKeys(ctx, reader, end.Key); err != nil {
		return ms, err
	}
	...
	for {
		...
		if sstKeyIsLive {
			...
			// If the sst key is not live, it must contribute to GCBytesAge. If the key is a tombstone it
			// accrues GCBytesAge at its own timestamp, else at the timestamp which it is shadowed.
			nonLiveTime := prevSSTKey.Timestamp.WallTime
			if sstValueIsTombstone {
				nonLiveTime = sstIterKey.Timestamp.WallTime
			}
			ms.GCBytesAge += (metaKeySize + MVCCVersionTimestampSize + valSize) * (nowNanos/1e9 - nonLiveTime/1e9)
		}
		if isMetaKey {
		...
		if sstKeyShadowsEng {
			...
			engTotal := engMetaKeySize + MVCCVersionTimestampSize + engValSize
			...
			if isMetaKey {
				...
				if !engValueIsTombstone {
					ms.LiveCount--
					ms.LiveBytes -= engTotal
					ms.GCBytesAge += engTotal * (nowNanos/1e9 - sstIterKey.Timestamp.WallTime/1e9)
				}
				ms.KeyBytes -= engMetaKeySize
				ms.KeyCount--
			} else if !engValueIsTombstone {
				ms.GCBytesAge -= engTotal * (prevSSTKey.Timestamp.WallTime/1e9 - sstIterKey.Timestamp.WallTime/1e9)
			}
		}
