...
func CheckSSTConflicts(
	...
	for !usePrefixSeek && sstErr == nil && sstOK && extOK && extErr == nil {
		...
		extValueDeleted := extHasRange && extHasPoint && extIter.RangeKeys()[0].RangeKey.Deletes(extKey)
		...
		if sstHasPoint && extHasPoint {
			...
			oldKey := sstIter.Key()
			if extHasPoint { // !sstHasPoint
				// Similar logic as above, but with the iterators swapped.
				sstIter.Next()
				sstOK, sstErr = sstIter.Valid()
				if sstErr != nil {
			...
			} else if sstHasPoint { // !extHasPoint
