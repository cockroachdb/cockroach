...
func ComputeSSTStatsDiff(
	...
	for ; ; sstIter.Next() {
		...
		// At this point, engIterKey >= sstIterKey.
		//
		...
		// if prevKey.Key == sstIterKey.Key, that implies sstIterKey.Timestamp.Less(prevSSTKey.Timestamp).
		prevKeyShadowsSSTKey := prevSSTKey.Key.Compare(sstIterKey.Key) == 0
