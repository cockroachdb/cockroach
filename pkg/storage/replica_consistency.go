...
// stats mismatch is the event in which the consistency checker finds that all
// replicas are consistent (i.e. byte-by-byte identical) but that the
// (identical) stats tracked in them does not correspond to what the stats
// should be given the data in the range, but the stats claim that they are
// accurate (ContainsEstimates==false).
