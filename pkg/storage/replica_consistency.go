...
func (r *Replica) sha512(
	...
	visitor := func(unsafeKey engine.MVCCKey, unsafeValue []byte) error {
		...
		if snapshot != nil {
			...
			alloc, kv.Key = alloc.Copy(unsafeKey.Key, len(unsafeValue))
	...
	if !ok {
		return replicaHash{}, errors.New("no MVCCStats persisted")
	}
