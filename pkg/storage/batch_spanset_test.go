...
func TestSpanSetBatchTimestamps(t *testing.T) {
	...
	ss.AddMVCC(spanset.SpanReadWrite, roachpb.Span{
	...
	for _, batch := range []engine.Batch{batchAfter, batchBefore} {
	...
	for _, batch := range []engine.Batch{batchAfter} {
		//lint:ignore SA1019 historical usage of deprecated batch.Get is OK
...
func TestSpanSetIteratorTimestamps(t *testing.T) {
...

	{
		...
		iter := batchAt3.NewIterator(engine.IterOptions{UpperBound: roachpb.KeyMax})
		defer iter.Close()
