...
func TestSpanSetBatchTimestamps(t *testing.T) {
	...
	var ss spanlatch.SpanSet
	ss.AddAt(spanlatch.SpanReadOnly, roachpb.Span{
		Key: roachpb.Key("a"), EndKey: roachpb.Key("c")}, hlc.Timestamp{WallTime: 2})
	ss.AddAt(spanlatch.SpanReadWrite, roachpb.Span{
	...
	batchNonMVCC := spanlatch.NewBatchAt(eng.NewBatch(), &ss, hlc.Timestamp{WallTime: 0})
	defer batchNonMVCC.Close()

	batchBefore := spanlatch.NewBatchAt(eng.NewBatch(), &ss, hlc.Timestamp{WallTime: 1})
	defer batchBefore.Close()

	batchDuring := spanlatch.NewBatchAt(eng.NewBatch(), &ss, hlc.Timestamp{WallTime: 2})
	defer batchDuring.Close()

	batchAfter := spanlatch.NewBatchAt(eng.NewBatch(), &ss, hlc.Timestamp{WallTime: 3})
...
func TestSpanSetIteratorTimestamps(t *testing.T) {
	...
	batchAt1 := spanlatch.NewBatchAt(eng.NewBatch(), &ss, hlc.Timestamp{WallTime: 1})
	defer batchAt1.Close()

	batchAt2 := spanlatch.NewBatchAt(eng.NewBatch(), &ss, hlc.Timestamp{WallTime: 2})
	defer batchAt2.Close()

	batchAt3 := spanlatch.NewBatchAt(eng.NewBatch(), &ss, hlc.Timestamp{WallTime: 3})
...
func TestSpanSetNonMVCCBatch(t *testing.T) {
	...
	var ss spanlatch.SpanSet
	ss.Add(spanlatch.SpanReadOnly, roachpb.Span{Key: roachpb.Key("a"), EndKey: roachpb.Key("c")})
	ss.Add(spanlatch.SpanReadWrite, roachpb.Span{Key: roachpb.Key("d"), EndKey: roachpb.Key("f")})
	...
	batchNonMVCC := spanlatch.NewBatch(eng.NewBatch(), &ss)
	defer batchNonMVCC.Close()

	batchMVCC := spanlatch.NewBatchAt(eng.NewBatch(), &ss, hlc.Timestamp{WallTime: 1})
