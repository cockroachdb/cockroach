...
func (r *Replica) evalAndPropose(
	...
	g *concurrency.Guard,
) (_ chan proposalResult, _ func(), _ int64, async bool, pErr *roachpb.Error) {
	idKey := makeIDKey()
	...
	if proposal.command == nil {
		...
		return proposalCh, func() {}, 0, async, nil
	}
	...
	if ba.AsyncConsensus {
		if ets := proposal.Local.DetachEndTxns(false /* alwaysOnly */); len(ets) != 0 {
			...
			// command succeeds.
			return nil, nil, 0, false, roachpb.NewErrorf("cannot perform consensus asynchronously for "+
	...
	if maxSize := uint64(MaxCommandSize.Get(&r.store.cfg.Settings.SV)); quotaSize > maxSize {
		return nil, nil, 0, false, roachpb.NewError(errors.Errorf(
	...
	if err != nil {
		return nil, nil, 0, false, roachpb.NewError(err)
	}
	...
	return proposalCh, abandon, maxLeaseIndex, async, nil
}
