...
var pointScannerPool = sync.Pool{
	New: func() interface{} {
		mvccScanner := &pointScanner{
...
// pointScanner handles MVCCScan / MVCCGet using a Pebble iterator.
...
func (p *pointScanner) get(ctx context.Context) {
...
func (p *pointScanner) seekToStartOfScan() (ok bool) {
	if p.reverse {
...
// machine.
func (p *pointScanner) advance() bool {
	switch p.machine.fn {
...
// next result.
func (p *pointScanner) afterScan() (*roachpb.Span, kvpb.ResumeReason, int64, error) {
	p.maybeFailOnMoreRecent()
...
func (p *pointScanner) incrementItersBeforeSeek() {
	p.itersBeforeSeek++
...
func (p *pointScanner) decrementItersBeforeSeek() {
	p.itersBeforeSeek--
...
func (p *pointScanner) getFromIntentHistory() (value []byte, found bool) {
...
func (p *pointScanner) maybeFailOnMoreRecent() {
...
func (p *pointScanner) uncertaintyError(
...
func (p *pointScanner) getOne(ctx context.Context) (ok, added bool) {
...
func (p *pointScanner) nextKey() bool {
...
func (p *pointScanner) backwardLatestVersion(key []byte, i int) bool {
...
func (p *pointScanner) prevKey(key []byte) bool {
...
func (p *pointScanner) advanceKeyForward() bool {
	return p.nextKey()
}

// advanceKeyReverse advances to the next key in the reverse direction.
func (p *pointScanner) advanceKeyReverse() bool {
...
// end" state.
func (p *pointScanner) setAdvanceKeyAtEnd() {
	if p.reverse {
...
func (p *pointScanner) advanceKeyAtEndReverse() bool {
...
// at new key" state.
func (p *pointScanner) setAdvanceKeyAtNewKey(origKey []byte) {
...
func (p *pointScanner) advanceKeyAtNewKeyReverse() bool {
...
func (p *pointScanner) addSynthetic(
...
//   - added indicates whether the key and value were included into the result
//     set.
func (p *pointScanner) seekVersion(
...
//
// gcassert:inline
func (p *pointScanner) coveredByRangeKey(ts hlc.Timestamp) (rkv MVCCRangeKeyVersion, ok bool) {
...
func (p *pointScanner) doCoveredByRangeKey(ts hlc.Timestamp) (MVCCRangeKeyVersion, bool) {
...
// land on them last.
func (p *pointScanner) processRangeKeys(seeked bool, reverse bool) bool {
...
func (p *pointScanner) updateCurrent() bool {
...
func (p *pointScanner) getFromLazyValue() (v []byte, valid bool) {
...
func (p *pointScanner) decodeCurrentMetadata() bool {
...
//
//gcassert:inline
func (p *pointScanner) mvccHeaderRequired(uncertaintyCheckRequired bool) bool {
	return uncertaintyCheckRequired || p.decodeMVCCHeaders
}

//gcassert:inline
func (p *pointScanner) decodeCurrentValueIgnoringHeader(v []byte) bool {
	p.curUnsafeValue, p.err = decodeMVCCValueIgnoringHeader(v)
	return p.err == nil
}

//gcassert:inline
func (p *pointScanner) tryDecodeCurrentValueSimple(v []byte) (extended bool, valid bool) {
	var simple bool
...
//gcassert:inline
func (p *pointScanner) decodeCurrentValueExtended(v []byte) bool {
	p.curUnsafeValue, p.err = decodeExtendedMVCCValue(v, true)
	return p.err == nil
}

func (p *pointScanner) iterValid() bool {
...
func (p *pointScanner) iterSeek(key MVCCKey) bool {
	p.clearPeeked()
	p.parent.SeekGE(key)
	if !p.iterValid() {
		return false
...
func (p *pointScanner) iterSeekReverse(key MVCCKey) bool {
	p.clearPeeked()
	p.parent.SeekLT(key)
	if !p.iterValid() {
		return false
...
func (p *pointScanner) iterNext() bool {
	if p.reverse && p.peeked {
...
func (p *pointScanner) iterPrev() bool {
	if p.peeked {
		p.peeked = false
	} else {
		p.parent.Prev()
...
// range key or not.
func (p *pointScanner) iterPeekPrev() ([]byte, bool, bool) {
	if !p.peeked {
		p.peeked = true
...
func (p *pointScanner) clearPeeked() {
	if p.reverse {
		p.peeked = false
	}
}
...
func (p *pointScanner) isKeyLockedByConflictingTxn(
...
func (p *pointScanner) addCurIntent(ctx context.Context) bool {
	v, valid := p.getFromLazyValue()
	if !valid {
		return false
	}
...
// the intents set.
func (p *pointScanner) addKeyAndMetaAsIntent(
...
func (p *pointScanner) addRawIntent(ctx context.Context, key, value []byte) bool {
	// p.intents is a pebble.Batch which grows its byte slice capacity in
	// chunks to amortize allocations. The memMonitor is under-counting here
	// by only accounting for the key and value bytes.
...
func (p *pointScanner) intentsRepr() []byte {
...
func (p *pointScanner) assertOwnedRangeKeys() {
