...
type dropFuncs struct {
	...
	dropSnap func(*storage.SnapshotRequest_Header) error
}
...
func (h *unreliableRaftHandler) HandleSnapshot(
	...
	if header.RaftMessageRequest.RangeID == h.rangeID && h.dropSnap != nil {
		if err := h.dropSnap(header); err != nil {
			return err
		}
	}
...
func setupPartitionedRangeWithHandlers(
	...
	for i := range mtc.stores {
		...
		if h.dropSnap == nil {
			h.dropSnap = func(header *storage.SnapshotRequest_Header) error {
				pr.mu.RLock()
				defer pr.mu.RUnlock()
				if !pr.mu.partitioned {
					return nil
