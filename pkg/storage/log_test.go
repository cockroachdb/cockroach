...
func TestLogSplits(t *testing.T) {
	...
	// Verify that every the count has increased by at least one. Realistically
	// it's almost always one, but if there are any other splits they might race
	// in after the previous call to countSplits().
