...
type MVCCIncrementalIterOptions struct {
	...
	// between eligible keys because of resource limits.
	Limiter ResourceLimiter
}
...
func (i *MVCCIncrementalIterator) advance(skipFirstLimitCheck bool) {
	for {
		i.maybeSkipKeys()
		if !i.valid {
			return
		...
		if i.limiter != nil && i.limiter.IsExhausted() && !skipFirstLimitCheck {
			i.err = &ResourceLimitError{Message: i.limiter.Report(), ResumeKey: i.Key()}
			i.valid = false
			return
		}
		skipFirstLimitCheck = false
