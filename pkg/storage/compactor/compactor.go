...
func (c *Compactor) fetchSuggestions(ctx context.Context) (
	...
	err = c.eng.Iterate(
		...
		func(kv engine.MVCCKeyValue) (bool, error) {
			...
			if ok, err := dataIter.Valid(); err != nil {
				...
				delBatch.Clear(kv.Key)
				log.VEventf(ctx, 2, "purging suggested compaction for range %s - %s that contains data",
