...
func TestReplicaRankings(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		for i := range want {
			repl := rr.topQPS()
			if repl.repl == nil {
				t.Errorf("not enough replicas in output (i=%d); input: %v", i, tc.replicasByQPS)
			}
			if repl.qps != want[i] {
				t.Errorf("got %f for %d'th element; want %f (input: %v)", repl.qps, i, want, tc.replicasByQPS)
				break
			}
		}
		repl := rr.topQPS()
		if repl.repl != nil {
			t.Errorf("too many replicas in output (%d); input: %v", len(want)+1, tc.replicasByQPS)
		}
	}
}
