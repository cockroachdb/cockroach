...
// SetSeparatedIntents sets the config option(s) for separated intents. In
// order for separated intents to be written, both the bool arguments must
// be false.
//
// NOTE: Having oldClusterVersion = true rewrites the Settings object in
// engineConfig. If this ConfigOption is used alongside a Settings option and
// oldClusterVersion = true, one of the two Settings will be overwritten.
func SetSeparatedIntents(oldClusterVersion, disableSeparatedIntents bool) ConfigOption {
	return func(cfg *engineConfig) error {
		cfg.DisableSeparatedIntents = disableSeparatedIntents
		if !oldClusterVersion && cfg.Settings != nil {
			return nil
		}
		return Settings(makeSettingsForSeparatedIntents(oldClusterVersion))(cfg)
	}
}
