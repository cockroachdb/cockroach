...
// HasAllGaugesZero is used when testing to assert that all gauges are zero.
func (metrics Metrics) HasAllGaugesZero() bool {
	return metrics.PusheeWaiting.Value() == 0 &&
	...
	return &Metrics{
		...
		QueryWaiting: metric.NewGauge(
			metric.Metadata{
				...
				Help: "Number of queries (waiting for status update on Txns that are pushees on " +
					"another queue) on the txn wait queue",
				Measurement: "QueryWaiting",
				Unit:        metric.Unit_COUNT,
			},
		),
