...
// for the committed intent is published. This creates a scenerio where the
...
// An "unresolved intent" in the context of the rangefeed primitive is an an
// intent that may at some point in the future result in a RangeFeedValue
// publication. Based on this definition, there are two possible states that an
// extent intent can be in while fitting the requirement to be an "unresolved
// intent":
...
// unresolvedIntentQueue implements heap.Interface and holds unresolvedTxns.
...
// unresolvedIntentQueue, or nil if the queue is empty.
...
// returns whether the update had an effect on the oldest transaction in
// the queue.
...
// returns whether the update had an effect on the oldest transaction in the
// queue.
...
// its intent reference count. It returns whether the update had an effect on
// the oldest transaction in the queue.
...
func (uiq *unresolvedIntentQueue) updateTxn(
	...
	// Will changes to the txn change the queue's earliest timestamp?
	wasMin := txn.index == 0
...
func (uiq *unresolvedIntentQueue) Del(txnID uuid.UUID) bool {
	...
	// Will deleting the txn change the queue's earliest timestamp?
	wasMin := txn.index == 0
