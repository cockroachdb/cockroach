...
func TestProcessorSlowConsumer(t *testing.T) {
	...
	// Consume one more event without blocking. Should tear down processor.
	sent := p.ConsumeLogicalOpsWithoutBlocking(
		writeValueOpWithKV(roachpb.Key("k"), hlc.Timestamp{WallTime: 15}, []byte("val")),
	)
	...
	unblock()
	require.Equal(t, errBufferCapacityExceeded, <-r1ErrC)
}
