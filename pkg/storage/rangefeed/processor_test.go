...
func commitIntentOpWithKey(
	txnID uuid.UUID, key roachpb.Key, ts hlc.Timestamp,
...
func commitIntentOp(txnID uuid.UUID, ts hlc.Timestamp) enginepb.MVCCLogicalOp {
	return commitIntentOpWithKey(txnID, nil /* key */, ts)
}
...
func TestProcessor(t *testing.T) {
	...
	// Commit intent, but first write the key to engine so that the lookup
	// succeeds. Should forward resolved timestamp to closed timestamp.
	require.Nil(t, p.Eng.Put(
		engine.MVCCKey{Key: roachpb.Key("e"), Timestamp: hlc.Timestamp{WallTime: 13}},
		[]byte("intent value"),
	))
	p.ConsumeLogicalOp(commitIntentOpWithKey(txn2, roachpb.Key("e"), hlc.Timestamp{WallTime: 13}))
	p.Synchronize()
	require.Equal(t,
