...
func TestRegistrationBasic(t *testing.T) {
	...
	go noCatchupReg.runOutputLoop(context.Background(), nil)
	...
	go catchupReg.runOutputLoop(context.Background(), nil)
	...
	// Completed Callback.
	callbackReg := newTestRegistration(spAB, hlc.Timestamp{WallTime: 1}, nil)
	callbackReg.publish(ev1)
	callbackReg.publish(ev2)
	completed := false
	completedCb := func(r *registration) {
		completed = true
	}
	go callbackReg.runOutputLoop(context.Background(), completedCb)
	callbackReg.waitForCaughtUp()
	callbackReg.disconnect(nil)
	<-callbackReg.errC
	testutils.SucceedsSoon(t, func() error {
		if !completed {
			return fmt.Errorf("completed callback was not yet called")
		}
		return nil
	})
	...
	go disconnectReg.runOutputLoop(context.Background(), nil)
	disconnectReg.waitForCaughtUp()
	...
	go overflowReg.runOutputLoop(context.Background(), nil)
	...
	go streamErrReg.runOutputLoop(context.Background(), nil)
	...
	go streamCancelReg.runOutputLoop(context.Background(), nil)
	streamCancelReg.waitForCaughtUp()
...
func TestRegistryBasic(t *testing.T) {
	...
	go rAB.runOutputLoop(context.Background(), nil)
	go rBC.runOutputLoop(context.Background(), nil)
	go rCD.runOutputLoop(context.Background(), nil)
	go rAC.runOutputLoop(context.Background(), nil)
...
func TestRegistryPublishBeneathStartTimestamp(t *testing.T) {
	...
	go r.runOutputLoop(context.Background(), nil)
	...
	require.NoError(t, reg.waitForCaughtUp())
