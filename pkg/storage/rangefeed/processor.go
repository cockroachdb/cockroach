...
func (p *Processor) StopWithErr(pErr *roachpb.Error) {
	...
	// that the Processor goroutine has canceled all registrations and shut
	// down.
	p.stopC <- pErr
}
...
func (p *Processor) consumeLogicalOps(ctx context.Context, ops []enginepb.MVCCLogicalOp) {
	for _, op := range ops {
		...
		case *enginepb.MVCCCommitIntentOp:
			...
			if err != nil {
				// If we see an error then we disconnect all registrations
				// that overlap the key. However, we still need to let the
				// resolvedTimestamp see the operation or we risk leaking
				// a transaction reference.
				pErr := roachpb.NewErrorf("error consuming commit intent op for key %v @ ts %v: %v",
					key, t.Timestamp, err)
				p.reg.DisconnectWithErr(roachpb.Span{Key: key}, pErr)
			} else {
				p.publishValue(ctx, key, *val)
			}
