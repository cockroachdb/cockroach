...
type metamorphicIterator struct {
	...
	isForward bool
	// TODO(tbg): all of the cases that set this can be a bit more stringent. They
	// could assert more about the override they put in place. But it's basically
	// a re-implementation of RangeKeyChanged in the end so is it worth it?
	// This is already really hiding the metamorphic movement from checks sitting
	// at the caller.
	coerceRangeKeyChanged int // 0 -> leave alone, -1 -> false, 1 -> true
	actions               int // counter
}

func newMetamorphicIterator(t *testing.T, seed int64, it SimpleMVCCIterator) *metamorphicIterator {
	return &metamorphicIterator{t: t, seed: seed, r: rand.New(rand.NewSource(seed)), it: it}
}
...
func (m *metamorphicIterator) moveAround() {
	...
	// TODO(tbg): remove iit==nil once it supports RangeKeyChanged.
	rangeKeyChanged := (iit == nil) && m.it.RangeKeyChanged()
	rangeKeys := m.it.RangeKeys().Clone()
	printfln("original position: %s [hasPoint=%t]; rangeKeysChanged=%t rangeKeys=%s", cur, hasPoint, rangeKeyChanged, rangeKeys)
	...
	resetActions = append(resetActions, action{
		...
		func() {
			m.it.SeekGE(cur)
			// Preserve the changed range key result. If the action
			// moved us to a different range key, then the seek here
			// may generate an event that wasn't originally there.
			// Conversely, if there was an event and we never left
			// that range key, re-seeking to within it won't generate
			// the event, so we need to force it.
			if rangeKeyChanged {
				m.coerceRangeKeyChanged = 1
			} else {
				m.coerceRangeKeyChanged = -1
			}
		},
	...
	if hasPoint && (mvccIt == nil || !mvccIt.IsPrefix()) {
		...
		resetActions = append(resetActions, action{
			...
			func() {
				...
				// By stepping forward it's possible that a rangekey that seemed
				// new before is no longer new (since we enter it on the non-ultimate
				// step). The reverse is also possible, we might've been on this
				// rangedel already (coming from the right via Prev) and are now
				// stepping into it on the last step.
				if rangeKeyChanged {
					m.coerceRangeKeyChanged = 1
				} else {
					m.coerceRangeKeyChanged = -1
				...
				for {
					...
					// TODO remove nil check once iit learns RangeKeyChanged
					printfln("step: %s %s [changed=%t]", m.it.UnsafeKey(), m.it.RangeKeys(), iit == nil && m.it.RangeKeyChanged())
	...
	if !m.isForward && hasPoint && mvccIt.SupportsPrev() && !mvccIt.IsPrefix() {
		resetActions = append(resetActions, action{
			...
			func() {
				...
				// Here too we need to coerce RangeKeyChanged, since we may be
				// triggering an event that wasn't originally there.
				if rangeKeyChanged {
					m.coerceRangeKeyChanged = 1
				} else {
					m.coerceRangeKeyChanged = -1
	...
	resetAction.do()
	{
		hasPoint2, _ := m.it.HasPointAndRange() // hate this lint
		// TODO(tbg): remove nil checks once MVCCIncrementalIterator learns RangeKeyChanged
		printfln("recovered position: %s [hasPoint=%t]; rangeKeysChanged=%t->%t rangeKeys=%s",
			cur, hasPoint2, iit == nil && m.it.RangeKeyChanged(), iit == nil && m.RangeKeyChanged(), m.it.RangeKeys(),
		)
	}
	...
	// NB: we intentionally call our RangeKeyChanged which sometimes
	// coerces to false.
	coercedRangeKeyChanged := iit == nil && m.RangeKeyChanged()     // ditto about nil
	internalRangeKeyChanged := iit == nil && m.it.RangeKeyChanged() // ditto
	require.Equal(m.t, rangeKeyChanged, coercedRangeKeyChanged, "internalRangeKeyChanged=%t", internalRangeKeyChanged)
}
...
func (m *metamorphicIterator) RangeKeyChanged() bool {
	underlying := m.it.RangeKeyChanged()
	switch m.coerceRangeKeyChanged {
	case -1:
		return false
	case 0:
		return underlying
	...
	m.t.Fatalf("invalid value: %d", m.coerceRangeKeyChanged)
	return underlying
}
