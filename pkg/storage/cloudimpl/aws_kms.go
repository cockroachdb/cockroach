...
func init() {
	cloud.RegisterKMSFromURIFactory(MakeKMSFromURI, awsScheme)
}
...
func resolveKMSURIParams(kmsURI url.URL) kmsURIParams {
	kmsURIParams := kmsURIParams{
		kmsURI.Query().Get(AWSAccessKeyParam),
		kmsURI.Query().Get(AWSSecretParam),
		kmsURI.Query().Get(AWSTempTokenParam),
		kmsURI.Query().Get(AWSEndpointParam),
		kmsURI.Query().Get(KMSRegionParam),
		kmsURI.Query().Get(AuthParam),
	}
	...
	kmsURIParams.secret = strings.Replace(kmsURIParams.secret, " ", "+", -1)
	return kmsURIParams
}

// MakeKMSFromURI is the factory method which returns a configured, ready-to-use
// AWS KMS object.
func MakeKMSFromURI(uri string, env cloud.KMSEnv) (cloud.KMS, error) {
