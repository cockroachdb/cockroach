...
func newChunkWriter(
	...
	// On creation bytesBuffer.Len() returns chunkSize. We reset it to 0 to allow
	// for accurate accounting when buffering/writing in Write().
	bytesBuffer.Reset()
	return &chunkWriter{
		bytesBuffer, pw, execSessionDataOverride, chunkSize,
	}
}
...
func (w *chunkWriter) Write(buf []byte) (int, error) {
	...
	// Total number of written bytes are those which have been sent to the
	// underlying SQL table plus those still in the bytes buffer. The latter will
	// be flushed on Close().
	return w.pw.byteOffset + w.buf.Len(), nil
}
