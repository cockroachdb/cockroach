...
func makeS3Storage(ctx context.Context, uri, user string) (cloud.ExternalStorage, error) {
...
func TestPutS3(t *testing.T) {
	...
	t.Run("server-side-encryption-invalid-params", func(t *testing.T) {
		...
		s, err := makeS3Storage(ctx, invalidSSEModeURI, user)
		defer s.Close()
		require.NoError(t, err)
		err = s.WriteFile(ctx, "test", bytes.NewReader([]byte("1")))
		...
		invalidKMSURI := fmt.Sprintf(
			...
			cloudimpl.AuthParam, cloudimpl.AuthParamImplicit, cloudimpl.AWSServerSideEncryptionMode,
			"aws:kms")
		s2, err := makeS3Storage(ctx, invalidKMSURI, user)
		defer s2.Close()
		require.NoError(t, err)
		err = s2.WriteFile(ctx, "test", bytes.NewReader([]byte("1")))
