...
func TestPutS3(t *testing.T) {
	...
	if err != nil {
		testutils.SkipIgnoreLint(t, "No AWS credentials")
	}
...
func TestPutS3Endpoint(t *testing.T) {
	...
	for env, param := range expect {
		v := os.Getenv(env)
		if v == "" {
			testutils.SkipIgnoreLintf(t, "%s env var must be set", env)
		}
		q.Add(param, v)
	}
	...
	if bucket == "" {
		testutils.SkipIgnoreLint(t, "AWS_S3_ENDPOINT_BUCKET env var must be set")
	}
	user := security.RootUser
