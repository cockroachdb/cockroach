...
const (
	...
	mergeQueueTimerDuration = 0 // zero duration to process merges greedily.
...
func (mq *mergeQueue) shouldQueue(
	...
	if desc.EndKey.Equal(roachpb.RKeyMax) {
		// The last range cannot be merged away.
		return false, 0
	}
...
func (mq *mergeQueue) process(
	...
	if minBytes := lhsRepl.GetMinBytes(); lhsStats.Total() >= minBytes {
		log.VEventf(ctx, 2, "skipping merge: range meets minimum size threshold %d with %d bytes",
	...
	if maxBytes := MergeMaxRHSSize.Get(&mq.store.ClusterSettings().SV); rhsStats.Total() > maxBytes {
		log.VEventf(ctx, 2, "skipping merge: right-hand neighbor exceeds size threshold %d with %d bytes",
