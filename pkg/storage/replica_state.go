...
func setTruncatedState(
	...
	if (truncState == roachpb.RaftTruncatedState{}) {
		return errors.New("cannot persist trivial RaftTruncatedState")
	}
...
func writeInitialState(
	...
	if lease, err := loadLease(ctx, eng, desc.RangeID); err != nil {
		return enginepb.MVCCStats{}, errors.Wrap(err, "error reading lease")
	} else if (lease != nil && *lease != roachpb.Lease{}) {
		log.Fatalf(ctx, "expected trivial lease, but found %+v", lease)
	}
