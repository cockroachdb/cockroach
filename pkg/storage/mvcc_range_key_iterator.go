...
type MVCCRangeKeyIterOptions struct {
	...
	// Fragmented disables defragmentation, emitting fragments like
	// SimpleMVCCIterator would. When enabled, this results in an iteration order
	// of StartKey,Timestamp rather than EndKey,Timestamp.
	Fragmented bool
}

// MVCCRangeKeyIterator is an iterator over range keys in an engine. Unlike
// SimpleMVCCIterator, range keys are defragmented into contiguous logical range
// keys. It does not support seeking or backtracking, see
...
// be buffered in memory. But see the Fragmented option to emit fragments in
// StartKey,Timestamp order.
...
func (p *MVCCRangeKeyIterator) updateRangeKeys() {
	...
	p.complete = p.defragmenter.Append(rangeKeys)
	...
	if p.opts.Fragmented {
		p.complete = p.defragmenter.Append(nil)
	}
}
...
// MVCCRangeKeyDefragmenter defragments range keys into contiguous logical range
// keys. Abutting range keys at the same timestamp with the same value are
// considered the same logical range key and will be merged.
//
// Range keys are emitted as soon as they are completed. This results in an
// EndKey,Timestamp,StartKey ordering of the defragmented range keys.
...
// Append appends a new stack of range key fragments to the tracked range keys,
// extending or creating logical range keys as appropriate. Any tracked range
// keys that are not present in the given stack will be considered fully
// defragmented and returned.
//
// The given fragments must be the next fragment stack when iterating across
// range keys in sequential order, ordered in descending timestamp order.
func (d *MVCCRangeKeyDefragmenter) Append(fragments []MVCCRangeKeyValue) []MVCCRangeKeyValue {
	...
	for fragmentsIdx < len(fragments) {
		...
		// This fragment is a different range key at the same timestamp: complete
		// the existing one and start a new one.
		case cmp == 0:
		...
		// This incomplete range key is not present in the fragment stack: complete
		// it and remove it from the list, then try the current fragment again.
		case cmp == 1:
			complete = append(complete, *incomplete)
