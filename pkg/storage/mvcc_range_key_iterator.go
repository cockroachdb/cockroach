...
type MVCCRangeKeyIterOptions struct {
	...
	// this would be pointless: giving e.g. a@4 would skip a tombstone starting at
...
// SimpleMVCCIterator, range key are defragmented into contiguous deterministic
...
func NewMVCCRangeKeyIterator(
	r Reader, opts MVCCRangeKeyIterOptions,
...
// discarded.
func (p *MVCCRangeKeyIterator) findCompleteTombstones() {
...
func (p *MVCCRangeKeyIterator) updateTombstones() {
...
// Key returns the value of the current range key. It will not be invalidated by
// the iterator, but will be shared by all callers.
