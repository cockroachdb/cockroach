...
func TestConstrainToKeysNonEmptyRange(t *testing.T) {
	...
	insertedKey := roachpb.Key("c")
	err := engine.MVCCPut(context.Background(), eng, nil, insertedKey, hlc.Timestamp{}, roachpb.MakeValueFromString("value"), nil)
	require.NoError(t, err)

	span, empty, err := ConstrainToKeys(eng, span)
	require.NoError(t, err)
