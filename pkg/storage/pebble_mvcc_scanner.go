...
type results interface {
	...
	clear()
	// numKeys returns the number of KVs currently in the results.
	numKeys() int64
	// numBytes returns the current memory footprint of the results in bytes as
	// well as by how much the memory footprint of the results will increase (in
	// bytes) if a KV (with the corresponding lengths of the key and the value
	// parts) is put into it.
	//
	// Note that we choose to include the second return value into signature of
	// this method rather than defining a separate method for it out of
	// performance considerations.
	numBytes(lenKey, lenValue int) (int64, int64)
...
func (s *singleResults) numBytes(lenKey, lenValue int) (int64, int64) {
	// TODO: come up with a better formulate for sizeOf.
	return s.bytes, int64(lenKey + lenValue)
}
...
// numKeys implements the results interface.
func (p *pebbleResults) numKeys() int64 {
	return p.count
}
...
// numBytes implements the results interface.
func (p *pebbleResults) numBytes(lenKey, lenValue int) (int64, int64) {
	return p.bytes, int64(p.sizeOf(lenKey, lenValue))
}
...
func (p *pebbleMVCCScanner) add(
	...
	numKeys := p.results.numKeys()
	numBytes, numBytesInc := p.results.numBytes(len(rawKey), len(rawValue))
