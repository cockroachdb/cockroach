...
func (p *pebbleResults) put(
	ctx context.Context, key []byte, value []byte, monitor ScannerMemoryMonitor,
) error {
...
func (p *pebbleMVCCScanner) getAndAdvance(ctx context.Context) bool {
	...
	if p.inconsistent {
		...
		if p.err = p.memMonitor.Grow(ctx, int64(len(p.curRawKey)+len(p.curValue))); p.err != nil {
...
func (p *pebbleMVCCScanner) addAndAdvance(ctx context.Context, rawKey []byte, val []byte) bool {
	...
	if len(val) > 0 || p.tombstones {
		if err := p.results.put(ctx, rawKey, val, p.memMonitor); err != nil {
