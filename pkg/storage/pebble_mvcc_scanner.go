...
func (p *pebbleMVCCScanner) getAndAdvance() bool {
	if p.curKey.Timestamp != (hlc.Timestamp{}) {
		if p.curKey.Timestamp.LessEq(p.ts) {
			...
			return p.addAndAdvance(p.curValue, p.curRawKey)
		}
	...
	if len(p.meta.RawBytes) != 0 {
		...
		return p.addAndAdvance(p.meta.RawBytes, p.curRawKey)
	}
	...
	if p.txnEpoch == p.meta.Txn.Epoch {
		...
		if value, found := p.getFromIntentHistory(); found {
			...
			return p.addAndAdvance(value, p.keyBuf)
		}
...
// results limit.
func (p *pebbleMVCCScanner) addAndAdvance(val []byte, rawKey []byte) bool {
...
func (p *pebbleMVCCScanner) iterPeekPrev() ([]byte, bool) {
	if !p.peeked {
		...
		var ok bool
		p.curKey.Key, _, ok = enginepb.SplitMVCCKey(p.curRawKey)
		if !ok {
