...
func (p *pebbleMVCCScanner) processRangeKeys(seeked bool, reverse bool) bool {
	...
	// Keep looking for new range keys to process and skip until we find a point
	// key. We check iterValid last, to avoid the call in the common Next case.
	// Calling RangeKeyChanged() before Valid() is legal.
	for (seeked || reverse || p.parent.RangeKeyChanged()) && p.iterValid() {
...
func (p *pebbleMVCCScanner) iterNext() bool {
	if p.reverse && p.peeked {
		...
		if !p.iterValid() {
			...
			// We must process range keys because the seek can land on a bare range
			// key that must be skipped.
			p.parent.SeekGE(MVCCKey{Key: p.start})
			if !p.processRangeKeys(true /* seeked */, false /* reverse */) {
				return false
			}
