...
// pebbleMVCCScanner handles MVCCScan / MVCCGet for a Pebble iterator. If any
...
type pebbleMVCCScanner struct {
	...
	// when an MVCC range tombstone is encountered. We keep a separate reference
	// to it in order to release it back to its pool when the scanner is done.
	//
...
func (p *pebbleMVCCScanner) maybeEnablePointSynthesis() bool {
	if _, hasRange := p.parent.HasPointAndRange(); hasRange {
		...
		p.pointIter.SeekGE(p.parent.UnsafeKey())
