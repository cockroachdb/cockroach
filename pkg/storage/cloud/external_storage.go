...
func ExternalStorageFromURI(
	ctx context.Context, uri string, settings *cluster.Settings, blobClient blobs.BlobClient,
) (ExternalStorage, error) {
	conf, err := ExternalStorageConfFromURI(uri)
	if err != nil {
		return nil, err
	}
	return MakeExternalStorage(ctx, conf, settings, blobClient)
}
...
func MakeExternalStorage(
	...
	case roachpb.ExternalStorageProvider_LocalFile:
		...
		return makeLocalStorage(dest.LocalFile, settings, blobClient)
...
func makeLocalStorage(
	...
	if cfg.NodeID == 0 {
		cfg.NodeID = blobClient.GetNodeID()
	}
	...
	return &localFileStorage{base: cfg.Path, cfg: cfg, blobClient: blobClient}, nil
}
...
func (l *localFileStorage) WriteFile(
	...
	return l.blobClient.WriteFile(ctx, l.cfg.NodeID, filepath.Join(l.base, basename), content)
}

func (l *localFileStorage) ReadFile(ctx context.Context, basename string) (io.ReadCloser, error) {
	return l.blobClient.ReadFile(ctx, l.cfg.NodeID, filepath.Join(l.base, basename))
}

func (l *localFileStorage) ListFiles(ctx context.Context) ([]string, error) {
	var fileList []string
	matches, err := l.blobClient.List(ctx, l.cfg.NodeID, l.base)
	if err != nil {
...
func (l *localFileStorage) Delete(ctx context.Context, basename string) error {
	return l.blobClient.Delete(ctx, l.cfg.NodeID, filepath.Join(l.base, basename))
}

func (l *localFileStorage) Size(ctx context.Context, basename string) (int64, error) {
	stat, err := l.blobClient.Stat(ctx, l.cfg.NodeID, filepath.Join(l.base, basename))
	if err != nil {
		return 0, nil
	}
