...
func MakeExternalStorage(
	...
	case roachpb.ExternalStorageProvider_LocalFile:
		...
		return makeLocalStorage(dest.LocalFile, settings, blobClientFactory)
...
func makeNodeLocalURIWithNodeID(nodeID roachpb.NodeID, path string) string {
	return fmt.Sprintf("nodelocal://%d/%s", nodeID, strings.TrimPrefix(path, "/"))
}
...
func makeLocalStorage(
	...
	client, err := blobClientFactory(cfg.NodeID)
	if err != nil {
...
func (l *localFileStorage) WriteFile(
	...
	return l.blobClient.WriteFile(ctx, filepath.Join(l.base, basename), content)
}

func (l *localFileStorage) ReadFile(ctx context.Context, basename string) (io.ReadCloser, error) {
	return l.blobClient.ReadFile(ctx, filepath.Join(l.base, basename))
}
...
func (l *localFileStorage) Delete(ctx context.Context, basename string) error {
	return l.blobClient.Delete(ctx, filepath.Join(l.base, basename))
}

func (l *localFileStorage) Size(ctx context.Context, basename string) (int64, error) {
	stat, err := l.blobClient.Stat(ctx, filepath.Join(l.base, basename))
	if err != nil {
		return 0, nil
	}
