...
type ExternalStorage interface {
	...
	// destination's configured prefix If the passed function returns a
	// non-nil error the iteration is stopped and unless it is the sentinel
	// `ErrListingDone`, it is returned. If delimiter is non-empty names which
	// have the same prefix, prior to the delimiter, are grouped into a single
	// result which is that prefix.
