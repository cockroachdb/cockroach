...
type ExternalStorage interface {
	...
	//
	// The delimiter string, if non-empty, is used to group results that have a a
	...
	// then lists "b*" with delimiter "/", the constant prefix listed is "/a/b"
	// so the remaining suffix of each result -- 1/x, 1/y, and 2/z -- is then
	// grouped until the delimiter into results "/a/b1" and "/a/b2", which are as
	// usual returned without the configured prefix, resulting in "b1" and "b2".
