...
func (r *resumingHTTPReader) sendRequest(
	...
	prev := timeutil.Now()
	attempt := 0
	for retries := retry.StartWithCtx(r.ctx, httpRetryOptions); retries.Next(); {
		attempt++
		...
		log.Errorf(r.ctx, "HTTP:Req error: err=%s (delay %s, attempt %d)", err, delay, attempt)
...
func (r *resumingHTTPReader) requestNextRange() (err error) {
	...
	return checkHTTPContentRangeHeader(resp.Header.Get("Content-Range"), r.pos)
}
...
func (r *resumingHTTPReader) Read(p []byte) (n int, err error) {
	...
	for n == 0 && err == nil {
		...
		if r.canResume &&
			(errors.Is(err, io.ErrUnexpectedEOF) || sysutil.IsErrConnectionReset(err)) {
