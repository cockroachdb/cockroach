...
func TestListAndDeleteFiles(t *testing.T) {
	...
	fileTableReadWriter := NewFileTableReadWriter(database,
...
func TestWriteSingleFile(t *testing.T) {
	...
	const size = 1024 * 1024 * 10 // 10 MiB
	...
	// Check that the size returned by the FileTableReadWriter matches that of the
	// test file.
...
// the FileTableReadWriter methods after the first write, which is responsible
...
func TestDifferentUserDisallowed(t *testing.T) {
	...
	// payload tables created by john above. FileTableReadWriter should have
...
// TestDatabaseScope tests that the FileTableReadWriter executes all of its
...
func TestDatabaseScope(t *testing.T) {
	...
	require.NoError(t, err)
	newFileTableReadWriter := NewFileTableReadWriter("newdb",
