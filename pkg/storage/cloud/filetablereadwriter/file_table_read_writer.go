...
// FileTableReadWriter can be used to store, retrieve and delete the blobs and
...
type FileTableReadWriter struct {
	databaseName string
...
// NewFileTableReadWriter returns a FileTableReadWriter object.
func NewFileTableReadWriter(
	databaseName string, ie *sql.InternalExecutor, db *kv.DB, username string,
) *FileTableReadWriter {
	return &FileTableReadWriter{
		databaseName: databaseName, ie: ie, db: db, username: username,
	}
}

// FileSize returns the size of the filename blob in bytes.
func (f *FileTableReadWriter) FileSize(ctx context.Context, filename string) (int64, error) {
...
// user scoped tables.
func (f *FileTableReadWriter) ListFiles(ctx context.Context) ([]string, error) {
	var files []string
...
func (f *FileTableReadWriter) DeleteAllFiles(ctx context.Context) error {
...
// tables.
func (f *FileTableReadWriter) DeleteFile(ctx context.Context, filename string) error {
...
// ReadFile returns the blob for filename by aggregating all of its chunks.
func (f *FileTableReadWriter) ReadFile(ctx context.Context, filename string) ([]byte, error) {
	payloadTableName := payloadTableNamePrefix + f.username
...
func (f *FileTableReadWriter) WriteFile(
	...
	rows, err := f.ie.QueryEx(ctx, "tables-exist", txn,
		sqlbase.InternalExecutorSessionDataOverride{User: f.username, Database: f.databaseName},
		tableExistenceQuery)
	if err != nil {
		return err
	...
	if len(rows) == 1 {
		return errors.New("expected both File and Payload tables to exist, " +
			"but one of them has been dropped")
	}
	tablesExist = len(rows) == 2
...
username STRING, upload_time TIMESTAMP)`,
		...
		payloadTableCreateQuery := fmt.Sprintf(`CREATE TABLE %s (filename STRING, 
byte_offset INT, payload BYTES, PRIMARY KEY(filename, byte_offset)) INTERLEAVE IN PARENT %s(
filename)`, payloadTableName, fileTableName)
		...
		// Revoke all privileges of every other user and role on the file and
		// payload tables.
...
users WHERE NOT "username" = 'root' AND NOT "username" = 'admin' AND NOT "username" = '%s'`,
	...
	for {
		...
		n, err := content.Read(send)
		if n > 0 {
			insertChunkQuery := fmt.Sprintf(`INSERT INTO %s VALUES ($1, $2, $3)`, payloadTableName)
			_, err = f.ie.QueryEx(ctx, "insert-file-chunk", txn,
				sqlbase.InternalExecutorSessionDataOverride{User: f.username, Database: f.databaseName},
				insertChunkQuery, filename, byteOffset, send[:n])
			if err != nil {
				return err
			}
			byteOffset += n
			...
			// payload.
			if byteOffset == 0 {
				insertEmptyChunkQuery := fmt.Sprintf(`INSERT INTO %s VALUES ($1, $2, $3)`, payloadTableName)
				_, err = f.ie.QueryEx(ctx, "insert-empty-file-chunk", txn,
					sqlbase.InternalExecutorSessionDataOverride{User: f.username, Database: f.databaseName},
					insertEmptyChunkQuery, filename, byteOffset, send[:n])
				if err != nil {
					return err
				}
			}
