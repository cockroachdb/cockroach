...
file_size INT, username STRING, upload_time TIMESTAMP DEFAULT NOW())`
...
const payloadTableSchema = `CREATE TABLE %s (filename STRING, byte_offset INT, 
...
// The FileToTableSystem is unusable after this method returns.
...
func newChunkWriter(
	...
	pw := &payloadWriter{
		username, database, filename, ie, ctx, txn, 0,
	}
	return &chunkWriter{
		bufio.NewWriterSize(pw, chunkSize), pw,
	}
}
...
func (w *chunkWriter) Close() error {
	...
	fileTableName := fileTableNamePrefix + w.pw.username
	...
	_, err := w.pw.ie.QueryEx(w.pw.ctx, "insert-file-name", w.pw.txn,
		sqlbase.InternalExecutorSessionDataOverride{User: w.pw.username, Database: w.pw.database},
		fileNameQuery, w.pw.filename, w.pw.byteOffset, w.pw.username)
	if err != nil {
		w.pw.txn.CleanupOnError(w.pw.ctx, err)
		return err
	}
...
func newFileTableReader(
	...
	log.Info(ctx, "DATABASE "+database)
	rows, err := ie.QueryEx(
...
func (f *FileToTableSystem) ReadFile(ctx context.Context, filename string) (io.ReadCloser, error) {
	...
	var err error
	if reader, err = newFileReader(ctx, filename, f.username, f.databaseName, f.ie); err != nil {
		return nil, err
	}
	return reader, nil
}
...
func (f *FileToTableSystem) checkIfFileExists(
	ctx context.Context, fileName, fileTableName string,
) (bool, error) {
	fileExistenceQuery := fmt.Sprintf(`SELECT filename FROM %s WHERE filename='%s'`, fileTableName,
		fileName)
	rows, err := f.ie.QueryEx(ctx, "file-exists", nil,
		sqlbase.InternalExecutorSessionDataOverride{User: f.username, Database: f.databaseName},
		fileExistenceQuery)
	if err != nil {
		return false, err
...
func (f *FileToTableSystem) NewFileWriter(
	...
	if !tablesExist {
		return nil, errors.Newf("File and/or Payload tables do not exist, " +
			"create a new FileToTableSystem")
	}

	if exists, err := f.checkIfFileExists(ctx, filename, fileTableName); exists || err != nil {
		return nil, errors.Wrap(os.ErrExist, filename)
	}

	writer := newChunkWriter(ctx, chunkSize, filename, f.username, f.databaseName, f.ie,
		f.db.NewTxn(ctx, f.databaseName))
