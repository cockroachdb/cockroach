...
func TestCheckSSTConflictsMaxLockConflicts(t *testing.T) {
	...
	testcases := []struct {
		...
		{maxLockConflicts: 6, targetBytesPerLockConflict: 195, expectLockConflicts: []string{"a", "b", "c", "d"}},
		{maxLockConflicts: 6, targetBytesPerLockConflict: 215, expectLockConflicts: []string{"a", "b", "c", "d", "e"}},
	}
			...
			for _, usePrefixSeek := range []bool{false, true} {
				t.Run(fmt.Sprintf("usePrefixSeek=%v", usePrefixSeek), func(t *testing.T) {
					...
					_, err := CheckSSTConflicts(ctx, sstFile.Bytes(), engine, startKey, endKey, startKey.Key, endKey.Key.Next(),
						false /*disallowShadowing*/, hlc.Timestamp{} /*disallowShadowingBelow*/, hlc.Timestamp{} /* sstReqTS */, tc.maxLockConflicts, tc.targetBytesPerLockConflict, usePrefixSeek)
					require.Error(t, err)
