...
// WriteBatch is the serialized representation of two RocksDB write batches.
// This is used in the context of storage.EvalResult where we propose through
// raft two write batches corresponding to the two RocksDB instances, the
// dedicated raft engine and the original.
// A wrapper message is used so that the absence of fields it can be
// distinguished from a zero-length batch, and so structs containing pointers
// to it can be compared with the == operator (we rely on this in
// storage.EvalResult)
type WriteBatch struct {
	Data     []byte `protobuf:"bytes,1,opt,name=data" json:"data,omitempty"`
	RaftData []byte `protobuf:"bytes,2,opt,name=raft_data,json=raftData" json:"raft_data,omitempty"`
}
...
func (m *WriteBatch) MarshalTo(dAtA []byte) (int, error) {
	...
	if m.RaftData != nil {
		dAtA[i] = 0x12
		i++
		i = encodeVarintProposerKv(dAtA, i, uint64(len(m.RaftData)))
		i += copy(dAtA[i:], m.RaftData)
	}
	return i, nil
}
...
func (m *WriteBatch) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 2:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field RaftData", wireType)
			}
			var byteLen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.RaftData = append(m.RaftData[:0], dAtA[iNdEx:postIndex]...)
			if m.RaftData == nil {
				m.RaftData = []byte{}
			}
			iNdEx = postIndex
...
var fileDescriptorProposerKv = []byte{
	...
	,
}
