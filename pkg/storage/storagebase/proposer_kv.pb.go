...
type ReplicatedEvalResult struct {
	...
	// This is the expiration of lease which is current while creating a LeaseRequest. We need
	// this to check if a lease request is being applied under the same lease with which it was
	// proposed. In a simple scenario when there are no lease extensions, this can be verified
	// by looking at sequence numbers of the leases. However a lease extension doesn't bump
	// the lease index so there are scenarios where a lease extension would be immediately
	// overwritten because it would check to the sequence number which would have been the same
	// following an extension, but the lease would have a difference expiration.
