...
func (e *engineState) Unregister() {
	...
	//
	// NB: we get the index after we are done with the loop that waits for
	// isGranting to become false, since that loop releases the mutex, which
	// can change the index (due to concurrent Unregister calls).
...
func (s *MultiEngineCompactionScheduler) tryGrant(scratch []candidate) (scratch2 []candidate) {
	...
	for s.mu.totalRunning < s.opts.GetMaxConcurrency() {
		...
		// Unregister not setting slice entries to nil since it waits for
		// isGranting to become false. The only change to the slice  that can
		// happen during this loop is new entries being added to the slice, which
		// will be ignored.
