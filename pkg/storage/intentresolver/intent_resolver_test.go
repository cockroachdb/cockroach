	...
	for i, tc := range testCases {
		...
		t.Run(tc.pusher.ID.String(), func(t *testing.T) {
			wiErr := &roachpb.WriteIntentError{Intents: []roachpb.Intent{
				roachpb.MakeErrorIntent(origTxn.TxnMeta, roachpb.Span{Key: keyA})}}
	...
	for resp := range resps {
		...
		case 4:
			...
			f(&roachpb.WriteIntentError{Intents: []roachpb.Intent{
				roachpb.MakeErrorIntent(unrelatedRWTxn.TxnMeta, roachpb.Span{Key: keyA})}}, nil)
		...
		case 5:
			...
			f(&roachpb.WriteIntentError{Intents: []roachpb.Intent{
				roachpb.MakeErrorIntent(rwTxn1.TxnMeta, roachpb.Span{Key: keyB})}}, nil)
		case 6:
