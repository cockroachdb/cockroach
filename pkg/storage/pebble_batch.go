...
func (p *pebbleBatch) ScanInternal(ctx context.Context, lower, upper roachpb.Key, visitPointKey func(key *pebble.InternalKey, value pebble.LazyValue) error, visitRangeDel func(start []byte, end []byte, seqNum uint64) error, visitRangeKey func(start []byte, end []byte, keys []rangekey.Key) error, visitSharedFile func(sst *pebble.SharedSSTMeta) error) error {
	panic("ScanInternal only supported on Engine and Snapshot.")
}

// ClearRawEncodedRange implements the Reader interface.
