...
func (p *pebbleBatch) NewMVCCIterator(ctx context.Context, iterKind MVCCIterKind, opts IterOptions) (MVCCIterator, error) {
	if p.writeOnly {
...
func (p *pebbleBatch) NewEngineIterator(ctx context.Context, opts IterOptions) (EngineIterator, error) {
	if p.writeOnly {
		panic("write-only batch")
	}
