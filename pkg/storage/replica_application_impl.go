...
// replica_application_impl.go provides concrete implementations of
// apply.Decoder (replicaDecoder) and apply.StateMachine (replicaApplier),
...
// once it has been reviewed. The diff would have been too annoying.
...
func (d *replicaDecoder) DecodeAndBind(ctx context.Context, ents []raftpb.Entry) (bool, error) {
	...
	return d.retrieveLocalProposals(ctx)
}
...
func (d *replicaDecoder) decode(ctx context.Context, ents []raftpb.Entry) error {
	for i := range ents {
		ent := &ents[i]
		if _, err := d.cmdBuf.allocate().decode(ctx, ent); err != nil {
			return err
		}
	}
	return nil
...
// on all commands.
func (d *replicaDecoder) retrieveLocalProposals(ctx context.Context) (anyLocal bool, _ error) {
	d.r.mu.Lock()
	defer d.r.mu.Unlock()
...
// getApplier returns the Replica's apply.StateMachine.
...
// to the replicated state machine after it has been committed to the Raft log.
// This decision is deterministic on all replicas, such that a command that is
// rejected "beneath raft" on one replica will be rejected "beneath raft" on all
// replicas.
//
...
//    Timestamp.
//
// The method sets the command's leaseIndex, proposalRetry, and forcedErr
// fields. It returns whether command should be applied or rejected.
func (r *Replica) checkShouldApplyCommand(
...
func (b *replicaAppBatch) Stage(cmdI apply.Command) (apply.CheckedCommand, error) {
	...
	ctx := cmd.ctx
	if cmd.ent.Index == 0 {
		return nil, errors.Errorf("processRaftCommand requires a non-zero index")
	}
	if idx, applied := cmd.ent.Index, b.state.RaftAppliedIndex; idx != applied+1 {
		// If we have an out of order index, there's corruption. No sense in
		// trying to update anything or running the command. Simply return.
		return nil, errors.Errorf("applied index jumped from %d to %d", applied, idx)
	}
	if log.V(4) {
	...
	if !b.r.checkShouldApplyCommand(ctx, cmd, &b.state) {
	...
	// The command was checked by checkShouldApplyCommand, so it can be returned
	// as a apply.CheckedCommand.
	return cmd, nil
}
