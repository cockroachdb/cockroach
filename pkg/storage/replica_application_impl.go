...
func wrapWithNonDetermFailure(err error, msg string) error {
	return &nonDeterministicFailure{
...
func (b *replicaAppBatch) Stage(cmdI apply.Command) (apply.CheckedCommand, error) {
	...
	ctx := cmd.ctx
	if cmd.ent.Index == 0 {
		return nil, makeNonDetermFailure("processRaftCommand requires a non-zero index")
	}
	if idx, applied := cmd.ent.Index, b.state.RaftAppliedIndex; idx != applied+1 {
		// If we have an out of order index, there's corruption. No sense in
		// trying to update anything or running the command. Simply return.
		return nil, makeNonDetermFailure("applied index jumped from %d to %d", applied, idx)
	}
	if log.V(4) {
	...
	if splitMergeUnlock, err := b.r.maybeAcquireSplitMergeLock(ctx, cmd.raftCmd); err != nil {
		return nil, wrapWithNonDetermFailure(err, "unable to acquire split lock")
...
func (b *replicaAppBatch) stageWriteBatch(ctx context.Context, cmd *cmdAppCtx) error {
	...
	if err := b.batch.ApplyBatchRepr(wb.Data, false); err != nil {
		return wrapWithNonDetermFailure(err, "unable to apply WriteBatch")
	}
	return nil
}
...
func (b *replicaAppBatch) runPreApplyTriggers(ctx context.Context, cmd *cmdAppCtx) error {
	...
	if merge := res.Merge; merge != nil {
		...
		if err != nil {
			return wrapWithNonDetermFailure(err, "unable to get replica for merge")
		}
		...
		if err := rhsRepl.preDestroyRaftMuLocked(
			...
			return wrapWithNonDetermFailure(err, "unable to destroy range before merge")
		}
	}
	...
	if res.State != nil && res.State.TruncatedState != nil {
		if apply, err := handleTruncatedStateBelowRaft(
			...
			return wrapWithNonDetermFailure(err, "unable to handle truncated state")
		} else if !apply {
...
func (b *replicaAppBatch) Commit(ctx context.Context) error {
	...
	if err := b.batch.Commit(sync); err != nil {
		return wrapWithNonDetermFailure(err, "unable to commit Raft entry batch")
	}
	b.batch.Close()
	b.batch = nil
...
func (b *replicaAppBatch) addAppliedStateKeyToBatch(ctx context.Context) error {
	...
	if b.migrateToAppliedStateKey {
		...
		if err != nil {
			return wrapWithNonDetermFailure(err, "unable to migrate to range applied state")
		}
	...
	if b.state.UsingAppliedStateKey {
		...
		if err := loader.SetRangeAppliedState(
			...
			return wrapWithNonDetermFailure(err, "unable to set range applied state")
		}
	} else {
		...
		if err := loader.SetLegacyAppliedIndexBlind(
			...
			return wrapWithNonDetermFailure(err, "unable to set applied index")
		}
		...
		if err := loader.SetMVCCStats(ctx, b.batch, b.state.Stats); err != nil {
			return wrapWithNonDetermFailure(err, "unable to update MVCCStats")
		}
	}
	return nil
}
...
func (a *replicaApplier) ApplySideEffects(cmdI apply.CheckedCommand) (apply.AppliedCommand, error) {
	...
	if err := a.maybeApplyConfChange(ctx, cmd); err != nil {
		return nil, wrapWithNonDetermFailure(err, "unable to apply conf change")
	}
