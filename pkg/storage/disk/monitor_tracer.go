...
type monitorTracer struct {
	// The tracing service is best-effort and may contain events outside the
	// bounds defined by the period.
...
func (m *monitorTracer) RecordEvent(event traceEvent) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.removeExpired(event.time.Add(-m.period))
...
// Latest retrieves the last traceEvent that was queued. If the trace
...
// floorSearch retrieves the offset from the trace's start for the traceEvent
...
// mutex is released.
func (m *monitorTracer) floorSearch(t time.Time) (int, error) {
	if m.mu.size == 0 {
...
// ceilSearch retrieves the offset from trace's start for the traceEvent
// that occurred at or after a specified time, t. If all events occurred before
// t, an error is thrown. Note that it is the responsibility of the caller to
// acquire the tracer mutex and the returned offset may become invalid after the
// mutex is released.
func (m *monitorTracer) ceilSearch(t time.Time) (int, error) {
	if m.mu.size == 0 {
		return -1, errors.Errorf("trace is empty")
	}
	// Apply binary search to find the offset of the traceEvent that occurred at
...
// removeExpired removes events from the trace that occurred before a specified
// time, t. This should only be invoked when recording new events. Note that
// it is the responsibility of the caller to acquire the tracer mutex.
