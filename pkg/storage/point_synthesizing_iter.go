...
// emitOnSeekGE is set, it will also synthesize point tombstones around the
// SeekGE key if it does not have an existing point key.
//
...
type pointSynthesizingIter struct {
	...
	// rangeKeys contains the timestamps of MVCC range tombstones at the current
	// key position, for which point tombstones will be synthesized.
	rangeKeys []hlc.Timestamp
	...
	// differ from the underlying iterator position. See atPoint for details.
	rangeKeysPos roachpb.Key

	// rangeKeysIdx is the rangeKeys index of the current/pending range tombstone
	// to synthesize a point for. See atPoint for details.
	rangeKeysIdx int
	...
	//
	// When false, the synthesizing iterator is positioned on a synthetic point
	// tombstone given by rangeKeysIdx. In that case, the underlying iterator
	// points to a following key (either a different version of the current key or
	// a different point/range key) unless it is exhausted.
	//
	...
	// direction. Similarly, the underlying iterator is always >= rangeKeysPos in
	// the forward direction and <= in reverse.
	//
	...
	if _, hasRange := i.iter.HasPointAndRange(); hasRange {
		...
		if rangeStart, _ := i.iter.RangeBounds(); !rangeStart.Equal(i.rangeKeysStart) {
			...
			for _, rk := range i.iter.RangeKeys() {
				i.rangeKeys = append(i.rangeKeys, rk.Timestamp)
			}
...
func (i *pointSynthesizingIter) updateAtPoint() {
	if hasPoint, _ := i.iter.HasPointAndRange(); !hasPoint {
		...
		i.atPoint = i.rangeKeysIdx >= len(i.rangeKeys) ||
			...
			i.rangeKeys[i.rangeKeysIdx].LessEq(point.Timestamp)
	} else {
		i.atPoint = i.rangeKeysIdx < 0 || (point.Timestamp.IsSet() &&
			point.Timestamp.LessEq(i.rangeKeys[i.rangeKeysIdx]))
	}
}
...
func (i *pointSynthesizingIter) SeekGE(key MVCCKey) {
	...
	if len(i.rangeKeys) > 0 && key.Timestamp.IsSet() && key.Key.Equal(i.rangeKeysPos) {
		i.rangeKeysIdx = sort.Search(len(i.rangeKeys), func(idx int) bool {
			return i.rangeKeys[idx].LessEq(key.Timestamp)
		})
	}
	...
	if key.Timestamp.IsSet() {
		if hasPoint, hasRange := i.iter.HasPointAndRange(); hasRange {
			if !hasPoint || !i.iter.UnsafeKey().Key.Equal(key.Key) {
				if _, rangeEnd := i.iter.RangeBounds(); key.Key.Compare(rangeEnd) < 0 {
					i.iter.Next()
...
func (i *pointSynthesizingIter) assertInvariants() error {
	...
	if minIdx >= 0 && minIdx < len(i.rangeKeys) {
		minKey = MVCCKey{Key: i.rangeKeysPos, Timestamp: i.rangeKeys[minIdx]}
	}
	if maxIdx >= 0 && maxIdx < len(i.rangeKeys) {
		maxKey = MVCCKey{Key: i.rangeKeysPos, Timestamp: i.rangeKeys[maxIdx]}
	}
