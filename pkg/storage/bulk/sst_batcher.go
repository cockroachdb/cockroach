...
// than ideally -- if a flush fails to to spanning a range boundary, it will have
// to regenerate SSTs for each side of the reported split and retry. This could
// in worst-case result in many expensive regeneration/retry loops. Ideally the
// batcher should be aware of split points and try to flush before crossing a
// split boundary (though it will still need to handle a mismatched error on new
// splits).
...
func MakeSSTBatcher(
	ctx context.Context, db *client.DB, flushBytes int64, time hlc.Timestamp,
) (*SSTBatcher, error) {
	b := &SSTBatcher{
		db: db, maxSize: flushBytes, timestamp: time,
	}
	err := b.reset()
	return b, err
}

// Add a key/value pair with the batch's timestamp, flushing if needed.
func (b *SSTBatcher) Add(ctx context.Context, key roachpb.Key, value []byte) error {
