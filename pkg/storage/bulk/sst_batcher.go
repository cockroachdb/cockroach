...
func (b *SSTBatcher) doFlush(ctx context.Context, reason int) error {
	...
	if reason == sizeFlush {
		...
		if b.flushCounts.total == 1 {
			if err := b.db.SplitAndScatter(ctx, start, hour); err != nil {
				log.Warning(ctx, err)
			}
		}
	...
	if b.flushKey != nil {
		...
		if bytes.Equal(b.flushKey, b.lastFlushKey) && files == 1 {
