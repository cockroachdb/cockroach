	...
	for len(work) > 0 {
		...
		if err := func() error {
			...
			for i := 0; i < maxAddSSTableRetries; i++ {
				...
				before := timeutil.Now()
				// If this SST is "too small", the fixed costs associated with adding an SST –
				// in terms of triggering flushes, extra compactions, etc – would exceed the
				// savings we get from skipping regular, key-by-key writes, and we're better
				// off just putting its contents in a regular batch. *Ideally* this doesn't
				// happen as we're still incurring extra overhead constructing SSTables just
				// as the wire-format, but in otherwise bulky jobs, handling it this case here
				// if it does arise, rather than asking the caller to call a differnet RPC
				// with differnet semantics, avoids pushing a lot of complexity into those
				// callers, while still getting most of the benefit of keeping tiny SSTs out
				// of our storage engine.
				ingestAsWriteBatch := false
				if int64(len(item.sstBytes)) < tooSmallSSTSize.Get(&settings.SV) {
