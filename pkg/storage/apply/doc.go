...
to a raft group's "raft log" and using them to drive the state machines of each
member of the raft group (i.e. each replica). Committed entries are decoded into
commands in the same order that they are arranged in the raft log (i.e. in order
of increasing log index). This ordering of decoded commands is then treated as
the input to state transitions on each replica.
...
performance. A first challenge falls out from the requirement that all
replicated commands be sequentially applied on each replica to enforce
determinism. This requirement must hold even as the concurrency of the systems
processing requests and driving replication grows. This problem, colloquially
known as the "parallelism gap", is fundementally due to the loss of context on
the interaction between replicated commands and an inability to determine
whether concurrent application of commands would be possible without
