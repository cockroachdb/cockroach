...
func TestMVCCStatsRandomized(t *testing.T) {
	...
	actions["DelRange"] = func(s *state) (bool, string) {
		...
		if !mvccRangeDel {
			...
			const targetBytesPerLockConflict = 0 // unlimited
			...
			err = MVCCDeleteRangeUsingTombstone(
				...
				nil /* rightPeekBound */, idempotent, maxLockConflicts, targetBytesPerLockConflict, msCovered,
			)
		} else {
			...
			_, err = MVCCPredicateDeleteRange(ctx, s.batch, s.MSDelta, mvccRangeDelKey, mvccRangeDelEndKey,
				...
				predicates, 0, 0, rangeTombstoneThreshold, maxLockConflicts, targetBytesPerLockConflict)
		}
		if err != nil {
			return false, desc + ": " + err.Error()
		}
