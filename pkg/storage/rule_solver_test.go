...
func TestRuleSolver(t *testing.T) {
	...
	storePool.mu.storeDetails[1].desc.Node.Locality.Tiers = []roachpb.Tier{
		{Key: "datacenter", Value: "us"},
		{Key: "rack", Value: "1"},
		{Key: "slot", Value: "5"},
	}
	storePool.mu.storeDetails[2].desc.Node.Locality.Tiers = []roachpb.Tier{
		{Key: "datacenter", Value: "us"},
		{Key: "rack", Value: "1"},
	}
	storePool.mu.storeDetails[3].desc.Node.Locality.Tiers = []roachpb.Tier{
		{Key: "datacenter", Value: "us"},
		{Key: "floor", Value: "1"},
		{Key: "rack", Value: "2"},
	}
	storePool.mu.storeDetails[5].desc.Node.Locality.Tiers = []roachpb.Tier{
		{Key: "datacenter", Value: "eur"},
		{Key: "rack", Value: "1"},
	}

	storePool.mu.storeDetails[1].desc.Capacity = roachpb.StoreCapacity{
		Capacity:   100,
		Available:  1,
		RangeCount: 99,
	}
	storePool.mu.storeDetails[2].desc.Capacity = roachpb.StoreCapacity{
		Capacity:   100,
		Available:  100,
		RangeCount: 0,
	}
	storePool.mu.storeDetails[3].desc.Capacity = roachpb.StoreCapacity{
		Capacity:   100,
		Available:  50,
		RangeCount: 50,
	}
	storePool.mu.storeDetails[5].desc.Capacity = roachpb.StoreCapacity{
		Capacity:   100,
		Available:  60,
		RangeCount: 40,
	}
	...
	for i, tc := range testCases {
		...
		// TODO(bram): add a corrupt replica test and remove the 0 range ID.
		candidates, err := solver.Solve(tc.c, tc.existing, roachpb.RangeID(0))
		if err != nil {
			t.Fatal(err)
		}
