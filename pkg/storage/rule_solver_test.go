...
func TestCandidateSelection(t *testing.T) {
	...
	formatter := func(cl candidateList) string {
		...
		for i, c := range cl {
			...
			buffer.WriteString(fmt.Sprintf("%d:%.2f", int(c.constraintScore), c.rangesPerGB))
		}
	...
	for _, tc := range testCases {
		...
		t.Run(fmt.Sprintf("good-%s", formatter(cl)), func(t *testing.T) {
			...
			actual := scoreTuple{int(good.constraintScore), good.rangesPerGB}
			if actual != tc.good {
		...
		t.Run(fmt.Sprintf("bad-%s", formatter(cl)), func(t *testing.T) {
			...
			actual := scoreTuple{int(bad.constraintScore), bad.rangesPerGB}
			if actual != tc.bad {
