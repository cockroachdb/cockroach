...
// sample duration.
func (f *Finder) Ready(nowNanos time.Time) bool {
	return nowNanos.Sub(f.startNanos) > durationThreshold
}
...
func (f *Finder) Record(span roachpb.Span, intNFn func(int) int) {
	...
	if count < splitKeySampleSize {
		...
		for i := range f.samples {
			if span.ProperContainsKey(f.samples[i].key) {
...
func (f *Finder) Key() (bool, roachpb.Key) {
	...
	for i, s := range f.samples {
		...
		containedScore := (float64(s.contained) / float64(mostContainedKeyCount))
