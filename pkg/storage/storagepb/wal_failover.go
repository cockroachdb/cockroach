...
func (c *WALFailover) SafeFormat(p redact.SafePrinter, _ rune) {
	switch c.Mode {
	case WALFailoverMode_WALFailoverDefault:
		// Empty
	case WALFailoverMode_WALFailoverDisabled:
		p.SafeString("disabled")
		if c.PrevPath.IsSet() {
			p.SafeString(",prev_path=")
			p.SafeString(redact.SafeString(c.PrevPath.Path))
		}
	case WALFailoverMode_WALFailoverAmongStores:
		p.SafeString("among-stores")
	case WALFailoverMode_WALFailoverExplicitPath:
		p.SafeString("path=")
...
func (c *WALFailover) Set(s string) error {
	...
	case strings.HasPrefix(s, "disabled"):
		c.Mode = WALFailoverMode_WALFailoverDisabled
		var ok bool
	...
	case s == "among-stores":
		c.Mode = WALFailoverMode_WALFailoverAmongStores
	case strings.HasPrefix(s, "path="):
		c.Mode = WALFailoverMode_WALFailoverExplicitPath
		var ok bool
...
func (e EncryptedPath) IsSet() bool { return e.Path != "" }
