...
//     write timestamp cache when aborting a transaction, which has the same
//     effect as writing an ABORTED transaction record. The second reason still
//     holds but is fairly weak. A transaction coordinator can kick off intent
//     resolution for an aborted transaction without needing to write these
//     intents into the record itself. In the worst case, this intent resolution
//     fails and each intent is cleaned up individually as it is discovered. All
//     in all, neither justification for this state holds much weight anymore.
//
