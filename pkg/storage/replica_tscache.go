...
func (r *Replica) CanCreateTxnRecord(
	...
	case uuid.UUID{} /* noTxnID */ :
		// Compare against the original timestamp because the transaction's
		// provisional commit timestamp may be moved forward over the course
		// of a single epoch.
		if !wTS.Less(txn.OrigTimestamp) {
	...
	// Compare against the original timestamp because the transaction's provisional
	// commit timestamp may be moved forward over the course of a single epoch.
	//
	// See #9265.
