...
// CollapseAccess collapses all spans into the specified access.
func (ss *SpanSet) CollapseAccess(access SpanAccess) {
	for i := SpanAccess(0); i < NumSpanAccess; i++ {
		if i == access {
			continue
		}
		for j := SpanScope(0); j < NumSpanScope; j++ {
			ss.spans[access][j] = append(ss.spans[access][j], ss.spans[i][j]...)
			ss.spans[i][j] = nil
		}
	}
}
