...
//
// TODO(irfansharif): This does not currently work for spans that thread across
// multiple added spans. Specifically a spanset with spans [a-c) and [b-d) added
// under read only and read write access modes respectively would fail at
// checking if read only access over the span [a-d) was requested. This is also
// a problem if the added spans were read only and the spanset wasn't already
// SortAndDedup-ed.
