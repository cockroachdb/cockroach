...
type Iterator struct {
	...
	// Timestamp the access is taking place. If timestamp is non-zero, the
	// timestamps associated with the spans are considered when ensuring access.
	// Else we only consider span boundaries.
	ts hlc.Timestamp
...
func NewIteratorAt(iter engine.Iterator, spans *SpanSet, ts hlc.Timestamp) *Iterator {
	if ts.IsEmpty() {
		// Empty timestamps is how we distinguish between batches ensuring span
		// access at a certain timestamp vs. ensuring access based on span
		// boundaries.
		panic("empty timestamp, invalid usage")
	}
...
func (i *Iterator) ComputeStats(
	...
	if i.ts.IsEmpty() {
		if err := i.spans.CheckAllowed(SpanReadOnly, roachpb.Span{Key: start.Key, EndKey: end.Key}); err != nil {
