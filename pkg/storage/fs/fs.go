...
// OnDiskSlow configures the Env to call the provided function when a disk
// operation is slow. There must be no concurrent disk operations when
// onDiskSlowFunc is invoked.
func (e *Env) OnDiskSlow(fn func(vfs.DiskSlowInfo)) {
	e.onDiskSlowFunc = fn
}
...
func (e *Env) onDiskSlow(info vfs.DiskSlowInfo) {
	if e.onDiskSlowFunc != nil {
		e.onDiskSlowFunc(info)
	}
}
