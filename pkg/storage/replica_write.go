...
func (r *Replica) evaluateWriteBatch(
	...
	if isOnePhaseCommit(ba) {
		...
		synthesizeEndTxnResponse := func() (_res onePCResult) {
			...
			defer func() {
				if etArg.Require1PC {
					...
					if !_res.success && _res.pErr == nil {
						if pErr != nil {
							_res.pErr = pErr
							return
						} else if ba.Timestamp != br.Timestamp {
							_res.pErr = roachpb.NewError(
		...
		batch.Close()
		log.VEventf(ctx, 2,
			"1PC execution failed, reverting to regular execution for batch. pErr: %v", pErr)
	}
...
func maybeBumpReadTimestampToWriteTimestamp(ctx context.Context, ba *roachpb.BatchRequest) {
	if arg, ok := ba.GetArg(roachpb.EndTxn); ok {
		etArg := arg.(*roachpb.EndTxnRequest)
...
func bumpBatchTimestamp(ctx context.Context, ba *roachpb.BatchRequest, ts hlc.Timestamp) {
	...
	if txn := ba.Txn; txn != nil {
		if ts.Less(txn.ReadTimestamp) || ts.Less(txn.WriteTimestamp) {
		...
		txn.WriteTimestamp = ba.Timestamp
