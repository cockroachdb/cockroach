...
func (r *Replica) executeWriteBatch(
	...
	ch, abandon, maxLeaseIndex, async, pErr := r.evalAndPropose(ctx, &lease, ba, spans, g)
	if pErr != nil {
		if maxLeaseIndex != 0 {
			log.Fatalf(
	...
	for {
		...
		case propResult := <-ch:
			...
			return propResult.Reply, async, propResult.Err
		case <-slowTimer.C:
			slowTimer.Read = true
...
and the following Raft status: %+v`,
		...
		case <-ctxDone:
			...
			return nil, async, roachpb.NewError(roachpb.NewAmbiguousResultError(ctx.Err().Error()))
		case <-shouldQuiesce:
			...
			log.VEventf(ctx, 2, "shutdown cancellation after %0.1fs of attempting command %s",
				timeutil.Since(startTime).Seconds(), ba)
			return nil, async, roachpb.NewError(roachpb.NewAmbiguousResultError("server shutdown"))
		}
	}
}
...
func (r *Replica) evaluateWriteBatch(
	...
	if isOnePhaseCommit(ba) {
		...
		if pErr == nil && (ba.Timestamp == br.Timestamp ||
			...
			// Even though the transaction is 1PC and hasn't written any intents, it
