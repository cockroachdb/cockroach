...
func maybeStripInFlightWrites(ba roachpb.BatchRequest) (roachpb.BatchRequest, error) {
	...
	// TODO(nvanbenschoten): clean this up.
	origET := et
	copiedTo := 0
	for _, ru := range ba.Requests[:len(ba.Requests)-1] {
		...
		if roachpb.IsTransactionWrite(req) {
			...
			// Remove the write from the in-flight writes set.
			seq := req.Header().Sequence
			i := roachpb.SequencedWriteBySeq(origET.InFlightWrites).Find(seq)
			if i < 0 {
	...
	if et != origET {
		...
		// batch overlap with each other.
		// TODO DURING THIS PR: fix this based on the outcome of
		//   https://github.com/cockroachdb/cockroach/pull/6857#discussion_r281913566.
		allInFlight := len(origET.IntentSpans) == 0
		var inFlightDistinct bool
		et.IntentSpans, ba.Header.DistinctSpans = roachpb.MergeSpans(et.IntentSpans)
		ba.Header.DistinctSpans = ba.Header.DistinctSpans || (allInFlight && inFlightDistinct)
	}
	return ba, nil
}
