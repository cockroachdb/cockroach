...
var backpressurableReqMethods = map[roachpb.Method]struct{}{
	roachpb.BeginTransaction: {},
	roachpb.EndTransaction:   {},
	roachpb.Put:              {},
	roachpb.InitPut:          {},
	roachpb.ConditionalPut:   {},
	roachpb.Merge:            {},
	roachpb.Increment:        {},
	roachpb.Delete:           {},
	roachpb.DeleteRange:      {},
...
func canBackpressureBatch(ba roachpb.BatchRequest) bool {
	...
	// try to detect the keys that a split writes to?
	...
	for _, union := range ba.Requests {
		...
		if _, ok := backpressurableReqMethods[method]; !ok {
			return false
		}
	}
	return true
...
func (r *Replica) maybeBackpressureWriteBatch(ctx context.Context, ba roachpb.BatchRequest) error {
	...
	for first := true; r.shouldBackpressureWrites(); first = false {
		if first {
			r.store.metrics.BackpressuredRequests.Inc(1)
			defer r.store.metrics.BackpressuredRequests.Dec(1)
