...
func TestMultiSSTWriterInitSST(t *testing.T) {
	...
	ts := hlc.Timestamp{}
	rangeIDLocalKey := keys.RangeAppliedStateKey(desc.RangeID)
	rangeLocalKey := keys.TransactionKey(roachpb.Key(desc.StartKey.Next()), uuid.MakeV4())
	userKey := roachpb.Key("e")
	...
	for numberOfPuts := 0; numberOfPuts < 4; numberOfPuts++ {
		if numberOfPuts != 0 {
			err := engine.MVCCPut(ctx, eng, nil, insertedKeys[numberOfPuts-1], ts, roachpb.MakeValueFromString("value"), nil)
			require.NoError(t, err)
		}
		scratch = sstSnapshotStorage.NewScratchSpace(testRangeID, testSnapUUID)
	...
	for i := range keyRanges {
		func() {
			...
			defer sst.Close()
			err := sst.ClearRange(engine.MakeMVCCMetadataKey(insertedKeys[i]), engine.MakeMVCCMetadataKey(insertedKeys[i].Next()))
			require.NoError(t, err)
			err = sst.Finish()
			require.NoError(t, err)
