...
type livenessUpdate struct {
	Liveness
	// when noCache is set, we won't assume that our in-memory cached version
	// of the liveness record is accurate and will use a CPut on the liveness
	// table with whatever the client supplied. This is used for operations
	// that don't want to deal with the inconsistencies of using the cache.
	noCache bool
}
...
func (nl *NodeLiveness) updateLivenessAttempt(
	...
	// put failures.
	if l, err := nl.GetLiveness(update.NodeID); !update.noCache && err == nil &&
