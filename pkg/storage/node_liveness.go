...
func (nl *NodeLiveness) updateLiveness(
	...
	for {
		for _, eng := range nl.engines {
			...
			/*
				key := keys.StoreWriteDiskKey()
				now := nl.clock.Now()
				mvccKey := engine.MVCCKey{Key: key, Timestamp: now}

				if err := eng.Put(mvccKey, []byte("")); err != nil {
					log.Warningf(ctx, "updateLiveness failed because write to RocksDB failed, err: %s", err)
				}
			*/
		}
