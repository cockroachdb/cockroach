		...
		for {
			if !nl.pauseHeartbeat.Load().(bool) {
				...
				for r := retry.StartWithCtx(ctx, retryOpts); r.Next(); {
					liveness, err := nl.Self()
					// Check whether heartbeats are paused after getting
					// nl.Self() to avoid a race condition in which draining
					// is enabled and heartbeats are paused but there is a
					// heartbeat that would disable draining by getting the
					// new version of nl.Self() set by the draining call and
					// updating that successfully.
					if nl.pauseHeartbeat.Load().(bool) {
						break
					}
					if err != nil && err != ErrNoLivenessRecord {
