...
type multiTestContext struct {
	...
	// advanceClockActive restricts advacing the clock so only one is active at a
	// time. This int32 value is set to 1 atomically to indicate that an advance
	// is active.
	advanceClockActive int32
}
...
func (m *multiTestContext) transferLease(
	...
	// lease we just transferred can be used. Otherwise, the liveness record might
	// be expired and the node is considered down, making this transfer
	// irrelevant. In particular, this can happen if the clock was advanced
	// recently, so all the liveness records (including the destination) are
	// expired. In that case, the simple fact that the transfer succeeded doesn't
	// mean that the destination now has a usable lease.
	m.mu.RLock()
...
// epoch-based leases to be considered expired until the liveness record are
// heartbeated).
//
// This method should only be used when all the stores share the manual clock.
// Otherwise, the desired effect would be ambiguous.
func (m *multiTestContext) advanceClock(_ context.Context) {
	// Allow only one expiration in progress at a time.
	if !atomic.CompareAndSwapInt32(&m.advanceClockActive, 0, 1) {
		return
	}

	for _, clock := range m.clocks {
		if clock != m.clock {
			panic("advanceClock() can't be used when some of the stores are sharing " +
				"the mtc's clock")
		}
	}
	...
	m.mu.RUnlock()
	atomic.StoreInt32(&m.advanceClockActive, 0)
}
