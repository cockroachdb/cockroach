...
func createTestStoreWithOpts(
	...
	if !opts.dontBootstrap {
		...
		kvs, tableSplits := sqlbase.MakeMetadataSchema(storeCfg.DefaultZoneConfig).GetInitialValues()
...
func (m *multiTestContext) addStore(idx int) {
	...
	if needBootstrap && idx == 0 {
		...
		var splits []roachpb.RKey
		kvs, tableSplits := sqlbase.MakeMetadataSchema(cfg.DefaultZoneConfig).GetInitialValues()
