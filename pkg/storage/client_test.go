...
func (m *multiTestContext) addStore(idx int) {
	...
	// Send an initial heartbeat. Without this, some nodes may
	// not have a heartbeat presence when they're needed.
	m.nodeLivenesses[idx].Heartbeat(context.Background(), nil)
...
func (m *multiTestContext) restartStore(i int) {
	...
	m.nodeLivenesses[i].StartHeartbeat(context.Background(), m.stoppers[i])
}
