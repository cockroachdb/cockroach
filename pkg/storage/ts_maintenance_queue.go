...
const (
	...
	timeSeriesMaintenanceQueueMaxSize       = 200
)
...
// * Local replica is a convenient way to discover the names of time series
// stored on the cluster; the names are required in order to effectively prune
// time series without expensive scans.
...
func (tsmq *timeSeriesMaintenanceQueue) process(
	...
	desc := repl.Desc()
	eng := repl.store.Engine()
	return tsmq.tsData.PruneTimeSeries(ctx, eng, desc.StartKey, desc.EndKey, tsmq.db, now)
}
