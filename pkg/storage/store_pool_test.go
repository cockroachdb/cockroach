...
func TestStorePoolFindDeadReplicas(t *testing.T) {
	...
	// Mark nodes 4 as merely unavailable.
	mnl.setNodeStatus(4, nodeStatusUnavailable)
...
func TestStorePoolDefaultState(t *testing.T) {
	...
	if len(liveReplicas) != 0 || len(deadReplicas) != 0 {
		t.Errorf("expected 1 live and 0 dead replicas; got %v and %v", liveReplicas, deadReplicas)
	}
