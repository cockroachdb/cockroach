...
func TestStorePoolUpdateLocalStore(t *testing.T) {
	...
	if expectedBytes := int64(36); desc.Capacity.LogicalBytes != expectedBytes {
		t.Fatalf("expected logical bytes %d, but got %d", expectedBytes, desc.Capacity.LogicalBytes)
	}
	if expectedQPS := 100 + QPS; desc.Capacity.QueriesPerSecond != expectedQPS {
		t.Fatalf("expected QueriesPerSecond %f, but got %f", expectedQPS, desc.Capacity.QueriesPerSecond)
	}
	if expectedWPS := 30 + WPS; desc.Capacity.WritesPerSecond != expectedWPS {
		t.Fatalf("expected WritesPerSecond %f, but got %f", expectedWPS, desc.Capacity.WritesPerSecond)
	}
