...
func TestNodeLivenessStatusMap(t *testing.T) {
	...
	// Also check that the dead node is not listed if the liveness map
	// is configured not to include removed nodes.
	t.Run("liveness map excludes removed node", func(t *testing.T) {
		t.Parallel()

		testutils.SucceedsSoon(t, func() error {
			log.Infof(ctx, "checking liveness map for removed node")
			nodeStatuses := callerNodeLiveness.GetLivenessStatusMap(false /*includeRemovedNodes*/)
			for _, expectedNodeID := range []roachpb.NodeID{liveNodeID, deadNodeID} {
				if _, ok := nodeStatuses[expectedNodeID]; !ok {
					return fmt.Errorf("expected node %d in map, was not present", expectedNodeID)
				}
			}
			if _, ok := nodeStatuses[removedNodeID]; ok {
				return fmt.Errorf("expected node %d not in map, was present", removedNodeID)
			}
			return nil
		})
	})
