...
// TestNodeIsAliveCallback verifies that the liveness callback for a
// node is invoked when it changes from state false to true.
func TestNodeIsAliveCallback(t *testing.T) {
	defer leaktest.AfterTest(t)()
	mtc := &multiTestContext{}
	defer mtc.Stop()
	mtc.Start(t, 3)
...
// TestNodeAliveCallback verifies that AliveCallback is invoked whenever this
// node updates its own liveness status.
func TestNodeAliveCallback(t *testing.T) {
	defer leaktest.AfterTest(t)()
	mtc := &multiTestContext{}
	defer mtc.Stop()
	mtc.Start(t, 3)
