...
func TestUpdateLivenessBlocks(t *testing.T) {
	...
	for idx, nl := range mtc.nodeLivenesses {
		...
		for {
			...
			select {
			case err = <-errors:
				t.Errorf("expected node %d to be considered not live within 5 milliseconds", nodeID)
			...
			if err == nil {
				...
				if err != nil {
					...
					t.Errorf("expected node %d to be considered live after waiting 5 milliseconds", nodeID)
				}
				break
			}
