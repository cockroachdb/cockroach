...
func TestMVCCOpLogWriter(t *testing.T) {
	...
	if err := MVCCPut(ctx, ol, nil, testKey1, hlc.Timestamp{Logical: 1}, hlc.ClockTimestamp{}, value1, NoLogicalReplication, nil); err != nil {
		t.Fatal(err)
	}
	txn1ts := makeTxn(*txn1, hlc.Timestamp{Logical: 2})
	if err := MVCCPut(ctx, ol, nil, testKey1, txn1ts.ReadTimestamp, hlc.ClockTimestamp{}, value2, NoLogicalReplication, txn1ts); err != nil {
		t.Fatal(err)
	}

	// Write a value and an intent on local keys.
	localKey := keys.MakeRangeIDPrefix(1)
	if err := MVCCPut(ctx, ol, nil, localKey, hlc.Timestamp{Logical: 1}, hlc.ClockTimestamp{}, value1, NoLogicalReplication, nil); err != nil {
		t.Fatal(err)
	}
	if err := MVCCPut(ctx, ol, nil, localKey, txn1ts.ReadTimestamp, hlc.ClockTimestamp{}, value2, NoLogicalReplication, txn1ts); err != nil {
		t.Fatal(err)
	}
	...
	if err := MVCCPut(ctx, ol, nil, testKey2, txn1LargerTS.ReadTimestamp, hlc.ClockTimestamp{}, value3, NoLogicalReplication, txn1LargerTS); err != nil {
		t.Fatal(err)
	}
	...
	if err := MVCCPut(ctx, ol, nil, testKey3, txn2.ReadTimestamp, hlc.ClockTimestamp{}, value4, NoLogicalReplication, txn2); err != nil {
		t.Fatal(err)
	}
	txn2Pushed := *txn2
	...
	if err := MVCCPut(ctx, ol, nil, testKey6, hlc.Timestamp{}, hlc.ClockTimestamp{}, value6, NoLogicalReplication, nil); err != nil {
		t.Fatal(err)
	}
