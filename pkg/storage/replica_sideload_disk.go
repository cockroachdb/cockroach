...
func newDiskSideloadStorage(
	...
	if st.Version.IsActive(cluster.VersionSideloadedStorageNoReplicaID) {
		...
		// MacBook Pro, this case averages ~3245ns. At 50k replicas, that adds
		// up to 0.16s of added startup time -- not enough to matter. On the
		// other hand, successful (i.e. directory found) calls are a lot slower,
		// 23k ns in my setup, so at 50k replicas we'll expect an additional
		// delay of 1.2s. Still not bad.
