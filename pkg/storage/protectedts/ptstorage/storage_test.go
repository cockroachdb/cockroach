...
func TestCorruptData(t *testing.T) {
	...
	t.Run("corrupt spans", func(t *testing.T) {
		...
		res, err := tc.ServerConn(0).Exec(
			"UPDATE system.protected_ts_records SET spans = $1 WHERE id = $2:::UUID",
		...
		require.Equal(t, []roachpb.Span(nil), got.Spans)
		require.Regexp(t, "invalid empty set of spans", got.Validate().Error())
		require.NoError(t, s.DB().Txn(ctx, func(ctx context.Context, txn *client.Txn) (err error) {
			_, err = pts.GetState(ctx, txn)
			return err
		}))
		require.Equal(t, got.ID, rec.ID)
		log.Flush()
		re := regexp.MustCompile("failed to unmarshal spans for " + rec.ID.String() + ": ")
		entries, err := log.FetchEntriesFromFiles(0, math.MaxInt64, 100, re)
		require.NoError(t, err)
	...
	t.Run("corrupt hlc timestamp", func(t *testing.T) {
		...
		res, err := tc.ServerConn(0).Exec(
			"UPDATE system.protected_ts_records SET ts = $1 WHERE id = $2:::UUID",
		...
		require.Equal(t, hlc.Timestamp{}, got.Timestamp)
		require.Equal(t, rec.Spans, got.Spans)
		require.Regexp(t, "invalid zero value timestamp", got.Validate().Error())
		require.NoError(t, s.DB().Txn(ctx, func(ctx context.Context, txn *client.Txn) (err error) {
			_, err = pts.GetState(ctx, txn)
			return err
		}))
		require.Equal(t, got.ID, rec.ID)
		log.Flush()
		re := regexp.MustCompile("failed to parse decimal as HLC for " + rec.ID.String() +
...
func TestErrorsFromSQL(t *testing.T) {
	...
	wrappedIE := &wrappedInternalExecutor{InternalExecutor: ie}
...
func (ie *wrappedInternalExecutor) Exec(
	...
	return ie.InternalExecutor.Exec(ctx, opName, txn, statement, params...)
}
...
func (ie *wrappedInternalExecutor) Query(
	...
	return ie.InternalExecutor.Query(ctx, opName, txn, statement, params...)
}
...
func (ie *wrappedInternalExecutor) QueryRow(
	...
	return ie.InternalExecutor.QueryRow(ctx, opName, txn, statement, params...)
}
