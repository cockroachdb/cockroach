...
func (p *storage) MarkVerified(ctx context.Context, txn *client.Txn, id uuid.UUID) error {
	...
	row, err := p.ex.QueryRow(ctx, "protectedts-MarkVerified", txn, markVerifiedQuery, id.GetBytesMut())
	if err != nil {
...
func (p *storage) Release(ctx context.Context, txn *client.Txn, id uuid.UUID) error {
	...
	row, err := p.ex.QueryRow(ctx, "protectedts-Release", txn, releaseQuery, id.GetBytesMut())
	if err != nil {
...
const (
	...
	currentMetaCTE = `SELECT version, num_records, num_spans, total_bytes FROM system.protected_ts_meta ` +
		`UNION ALL ` +
...
SELECT
    ...
    (
       ...
       OR EXISTS(SELECT * FROM system.protected_ts_records WHERE id = $4::UUID)
    ) AS failed
FROM (
    SELECT
