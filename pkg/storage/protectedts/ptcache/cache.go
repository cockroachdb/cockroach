...
func (c *Cache) periodicallyRefreshProtectedtsCache(ctx context.Context) {
	...
	for {
		...
		case <-settingChanged:
			...
			timer.Reset(protectedts.PollInterval.Get(&c.settings.SV) - timeutil.Since(lastReset))
...
func (c *Cache) doSingleFlightUpdate() (interface{}, error) {
	...
	defer cancel()
	return nil, c.stopper.RunTaskWithErr(ctx, "refresh-protectedts-cache", c.doUpdate)
}
...
func (c *Cache) upToDate(asOf hlc.Timestamp) bool {
	c.mu.RLock()
	defer c.mu.RUnlock()
	return c.mu.lastUpdate.LessEq(asOf)
}
