...
func TestStoreResolveWriteIntentPushOnRead(t *testing.T) {
	...
	testCases := []struct {
		pusherMaxPriority   bool
		pusheeAlreadyPushed bool
		pusheeStagingRecord bool
	...
	for _, tc := range testCases {
		name := fmt.Sprintf("pusherMaxPriority=%t,pusheePushed=%t,pusheeStaging=%t",
			tc.pusherMaxPriority, tc.pusheeAlreadyPushed, tc.pusheeStagingRecord)
