...
func TestMaybeDropMsgApp(t *testing.T) {
	...
	msgHeartbeat := &RaftMessageRequest{Message: raftpb.Message{
		Type: raftpb.MsgHeartbeat,
	}}
	msgAppNoContext := &RaftMessageRequest{Message: raftpb.Message{
		Type:    raftpb.MsgApp,
		Context: nil,
	}}
	msgAppFoo := &RaftMessageRequest{Message: raftpb.Message{
		Type:    raftpb.MsgApp,
		Context: []byte("foo"),
	}}
	...
	for dropper, exp := range testCases {
		t.Run(fmt.Sprintf("%v", dropper), func(t *testing.T) {
			...
			if exp {
				assert.Equal(t, string(msgAppFoo.Message.Context), dropper.startKey)
			}
		})
	}
}
