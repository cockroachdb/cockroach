...
func (r gcQueueScore) ValuesScore() float64 {
	return r.DeadFraction * r.ValuesScalableScore
}
...
// makeGCQueueScoreImpl us used to compute when to trigger the GC Queue. It's
...
func makeGCQueueScoreImpl(
	...
	abs := func(n int64) float64 { return math.Abs(float64(n)) }
	r.DeadFraction = 1 - abs(ms.LiveBytes)/(1+abs(ms.ValBytes)+abs(ms.KeyBytes))
	r.DeadFraction = math.Max(lo, r.DeadFraction)
	r.DeadFraction = math.Min(hi, r.DeadFraction)
