...
func makeGCQueueScoreImpl(
	...
	// Random factor in [0.75, 1.00] to cause decoherence of replicas with
	// similar load. This isn't 100% symmetric due to rounding issues near zero,
	// but not an issue in practice. Also note that we'll only ever reduce the
	// score, never increase it (for increasing it could lead to a fruitless
	// run).
	r.FuzzFactor = 0.75 + rand.New(rand.NewSource(fuzzSeed)).Float64()/4.0
