...
type cleanupTxnIntentsFunc func(context.Context, *roachpb.Transaction, []roachpb.Intent) error
...
func processLocalKeyRange(
	...
	handleTxnIntents := func(key roachpb.Key, txn *roachpb.Transaction) error {
		...
		if txn.Status == roachpb.PENDING || len(txn.Intents) > 0 {
			...
			return cleanupTxnIntentsFn(ctx, txn, roachpb.AsIntents(txn.Intents, txn))
		}
