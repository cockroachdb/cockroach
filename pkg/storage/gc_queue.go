...
const (
	...
	// gcKeyVersionChunkSize is the threshold size for splitting
	// GCRequests into multiple batches.
	gcKeyVersionChunkSize = 1024 * 1000
)
...
func RunGC(
	...
	var batchGCKeysByteCount int64
		...
		if len(keys) > 1 {
			...
			if err := protoutil.Unmarshal(vals[0], meta); err != nil {
				...
				if idx, gcTS := gc.Filter(keys[startIdx:], vals[startIdx:]); gcTS != (hlc.Timestamp{}) {
					...
					var byteCount int64
					for _, key := range keys[startIdx+idx:] {
						byteCount += int64(len(key.Key))
					}
					if len(batchGCKeys) > 0 && (batchGCKeysByteCount+byteCount) > gcKeyVersionChunkSize {
