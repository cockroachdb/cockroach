...
func RunGC(
	...
	var gcKeys [][]roachpb.GCRequest_GCKey
	var localGCKeys []roachpb.GCRequest_GCKey
	var localGCKeysVersionCount int64
		...
		if len(keys) > 1 {
			...
			if err := protoutil.Unmarshal(vals[0], meta); err != nil {
				...
				if idx, gcTS := gc.Filter(keys[startIdx:], vals[startIdx:]); gcTS != (hlc.Timestamp{}) {
					...
					// successfully set. This avoids sending potentially large
					...
					if len(localGCKeys) > 0 && (localGCKeysVersionCount+numVersions) > gcKeyVersionChunkSize {
						...
						localGCKeysVersionCount = 0
					}
					localGCKeys = append(localGCKeys, roachpb.GCRequest_GCKey{Key: expBaseKey, Timestamp: gcTS})
					localGCKeysVersionCount += numVersions
