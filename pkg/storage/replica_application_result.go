...
func isTrivial(r *storagepb.ReplicatedEvalResult, usingStateAppliedKey bool) (ret bool) {
...
func (r *Replica) finishRaftCommand(ctx context.Context, cmd *cmdAppCtx) {
	...
	if cmd.proposedLocally() {
		...
		if cmd.proposalRetry == proposalIllegalLeaseIndex && r.tryReproposeWithNewLeaseIndex(cmd.proposal) {
			return
		}
