...
func TestMVCCIterationLimit(t *testing.T) {
	...
	for _, s := range []struct {
		...
		// Non tbi iteration verify that we always progress even if limit is always firing
		{"full_time_range", 0,
			hlc.Timestamp{}, hlc.Timestamp{WallTime: math.MaxInt64},
			1, numKeys + 1, false},
		// Verify that in TBI mode we always progress even if limit is always firing
		{"partial_time_range", 0,
			hlc.Timestamp{WallTime: startBatch * batchTimeSpan}, hlc.Timestamp{WallTime: stopBatch * batchTimeSpan},
			1, numKeys + 1, true},
	} {
...
func assertIntentCount(
	...
	iter := NewMVCCIncrementalIterator(eng, MVCCIncrementalIterOptions{
		...
		EndTime:                             end,
		IntentPolicy:                        MVCCIncrementalIterIntentPolicyAggregate,
		Limiter:                             &CountingResourceLimiter{count: totalKeys / 2},
	})
	defer iter.Close()
...
func (l *CountingResourceLimiter) IsExhausted() bool {
	if l.count == 0 {
		return true
	}
	l.count--
...
func (l *CountingResourceLimiter) Report() string {
...
iterationLoop:
	for {
		iter := NewMVCCIncrementalIterator(eng, MVCCIncrementalIterOptions{
			...
			EndTime:                             end,
			IntentPolicy:                        MVCCIncrementalIterIntentPolicyError,
			Limiter:                             &CountingResourceLimiter{count: iteratorLimit},
			StartKey:                            resume,
		})
...
func TestResourceLimiter(t *testing.T) {
	...
	assertLimiterState := func(expected bool, message string) {
		for i := 0; i < ClockCheckEveryNIterations; i++ {
			if l.IsExhausted() == expected {
				return
			}
		}
		require.Equal(t, expected, l.IsExhausted(), message)
	}

	assertLimiterState(false, "iteration exhausted unexpectedly")
	clock.Advance(time.Minute * 2)
	require.False(t, l.IsExhausted(), "iteration exhausted instead of time check throttle")
	assertLimiterState(false, "iteration not exhausted even after throttling")
}
