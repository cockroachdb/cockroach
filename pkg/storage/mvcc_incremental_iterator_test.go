...
func assertExpectErr(
	...
	iter := NewMVCCIncrementalIterator(e.(ReaderWithMustIterators), MVCCIncrementalIterOptions{
		EndKey:    endKey,
		StartTime: startTime,
		EndTime:   endTime,
	})
...
func BenchmarkMVCCIncrementalIteratorForOldData(b *testing.B) {
	...
	for _, valueSize := range []int{100, 500, 1000, 2000} {
		...
		b.Run(fmt.Sprintf("valueSize=%d", valueSize), func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				func() {
					it := NewMVCCIncrementalIterator(eng.(ReaderWithMustIterators), MVCCIncrementalIterOptions{
						EndKey:    endKey,
