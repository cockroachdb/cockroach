...
func TestMVCCIncrementalIteratorNextIgnoringTime(t *testing.T) {
	...
	for _, kv := range kvs(kv1_1_1, kv1_2_2, kv2_2_2) {
		...
		if err := MVCCPut(ctx, e, nil, kv.Key.Key, kv.Key.Timestamp, hlc.ClockTimestamp{}, v, NoLogicalReplication, nil); err != nil {
			t.Fatal(err)
		}
	}
	...
	// Exercise deletion.
	if _, err := MVCCDelete(ctx, e, nil, testKey1, ts3, hlc.ClockTimestamp{}, NoLogicalReplication, nil); err != nil {
		t.Fatal(err)
	}
	// Returns the kv_1_1_1 even though it is outside (startTime, endTime].
	...
	if err := MVCCPut(ctx, e, nil, txn1.TxnMeta.Key, txn1.ReadTimestamp, hlc.ClockTimestamp{}, testValue4, NoLogicalReplication, &txn1); err != nil {
		t.Fatal(err)
	}
...
func TestMVCCIncrementalIteratorIntentPolicy(t *testing.T) {
	...
	if err := MVCCPut(ctx, e, nil, txn.TxnMeta.Key, txn.ReadTimestamp, hlc.ClockTimestamp{}, testValue2, NoLogicalReplication, &txn); err != nil {
		t.Fatal(err)
	}
...
func TestMVCCIncrementalIterator(t *testing.T) {
	...
	t.Run("latest", func(t *testing.T) {
		...
		if err := MVCCPut(ctx, e, nil, txn2.TxnMeta.Key, txn2.ReadTimestamp, hlc.ClockTimestamp{}, testValue4, NoLogicalReplication, &txn2); err != nil {
			t.Fatal(err)
		}
		t.Run("intents-1",
...
func TestMVCCIncrementalIteratorIntentRewrittenConcurrently(t *testing.T) {
	...
	if err := MVCCPut(ctx, e, nil, kA, ts1, hlc.ClockTimestamp{}, vA1, NoLogicalReplication, txn); err != nil {
		t.Fatal(err)
	}
	...
	g.Go(func() error {
		...
		b := e.NewBatch()
		defer b.Close()
		if err := MVCCPut(ctx, b, nil, kA, ts1, hlc.ClockTimestamp{}, vA2, NoLogicalReplication, txn); err != nil {
			return err
		}
...
func TestMVCCIncrementalIteratorIntentDeletion(t *testing.T) {
	...
	// kA:2 -> vA2
	// kB -> (intent deletion)
	require.NoError(t, MVCCPut(ctx, db, nil, kA, txnA1.ReadTimestamp, hlc.ClockTimestamp{}, vA1, NoLogicalReplication, txnA1))
	require.NoError(t, MVCCPut(ctx, db, nil, kB, txnB1.ReadTimestamp, hlc.ClockTimestamp{}, vB1, NoLogicalReplication, txnB1))
	require.NoError(t, MVCCPut(ctx, db, nil, kC, txnC1.ReadTimestamp, hlc.ClockTimestamp{}, vC1, NoLogicalReplication, txnC1))
	...
	require.NoError(t, err)
	require.NoError(t, MVCCPut(ctx, db, nil, kA, ts2, hlc.ClockTimestamp{}, vA2, NoLogicalReplication, nil))
	require.NoError(t, MVCCPut(ctx, db, nil, kA, txnA3.WriteTimestamp, hlc.ClockTimestamp{}, vA3, NoLogicalReplication, txnA3))
	require.NoError(t, db.Flush())
...
func TestMVCCIncrementalIteratorIntentStraddlesSStables(t *testing.T) {
	...
	put := func(key, value string, ts int64, txn *roachpb.Transaction) {
		...
		if err := MVCCPut(ctx, db1, nil, roachpb.Key(key), hlc.Timestamp{WallTime: ts}, hlc.ClockTimestamp{}, v, NoLogicalReplication, txn); err != nil {
			t.Fatal(err)
		}
	}
...
func BenchmarkMVCCIncrementalIteratorForOldData(b *testing.B) {
	...
	setupData := func(b *testing.B, eng Engine, valueSize int) {
		...
		for i := 0; i < numKeys; i++ {
			...
			if err := MVCCPut(context.Background(), batch, nil, key, ts, hlc.ClockTimestamp{}, value, NoLogicalReplication, nil); err != nil {
				b.Fatal(err)
			}
		}
		batch.Close()
