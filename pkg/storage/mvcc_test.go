		...
		t.Run(engineImpl.name, func(t *testing.T) {
			...
			for _, scan := range scanCases {
				t.Run(scan.name, func(t *testing.T) {
					res, err := MVCCScan(ctx, engine, testKey1, testKey5.Next(),
					...
					if len(intents) > 0 != !scan.consistent {
						t.Errorf("expected different intents slice; got %+v", intents)
						return
					}
					...
					if !reflect.DeepEqual(kvs, scan.expValues) {
						t.Errorf("expected values %+v; got %+v", scan.expValues, kvs)
					}
				})
			}
		})
