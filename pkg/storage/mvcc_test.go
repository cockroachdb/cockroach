...
func TestValidSplitKeys(t *testing.T) {
	...
	testCases := []struct {
		...
		{keys.SystemTenantKeyGen.TablePrefix(keys.MaxSystemConfigDescID), false},
		{keys.SystemTenantKeyGen.TablePrefix(keys.MaxSystemConfigDescID + 1), true},
	}
	for i, test := range testCases {
...
func TestFindValidSplitKeys(t *testing.T) {
	...
	addTablePrefix := func(prefix roachpb.Key, id uint32, rowVals ...string) roachpb.Key {
		tableKey := append(prefix, keys.SystemTenantKeyGen.TablePrefix(id)...)
	...
	testCases := []struct {
		...
		// All system span cannot be split.
		{
			...
			rangeStart: keys.SystemTenantKeyGen.TablePrefix(1),
			expSplit:   nil,
			expError:   false,
		},
		...
		// break for such ranges.
		{
			...
			rangeStart: keys.SystemTenantKeyGen.TablePrefix(userID),
		...
		// be split properly even in the presence of a large first row.
		{
			...
			rangeStart: keys.SystemTenantKeyGen.TablePrefix(keys.MinUserDescID),
			expSplit:   tablePrefix(userID, "b"),
			expError:   false,
		},
