...
func exportAllData(t *testing.T, engine Engine, limits queryLimits) []MVCCKey {
	...
	sstFile := &MemFile{}
	_, _, err := MVCCExportToSST(context.Background(), st, engine, ExportOptions{
...
func assertDataEqual(
	...
	for {
		...
		sstFile := &MemFile{}
		st := cluster.MakeTestingClusterSettings()
		_, key, err = MVCCExportToSST(context.Background(), st, engine, ExportOptions{
			StartKey:           key,
