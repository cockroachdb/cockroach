...
func TestMVCCGarbageCollectRanges(t *testing.T) {
	...
	testData := []struct {
		...
		{
			name: "range above del under multiple versions",
			before: rangeTestData{
				del(keyB, ts1),
				rng(keyA, keyD, ts2),
				pt(keyB, ts3),
		...
		{
			name: "range above multiple dels under many versions",
			before: rangeTestData{
				del(keyB, ts1),
				del(keyB, ts2),
				rng(keyA, keyD, ts3),
		...
		t.Run(engineImpl.name, func(t *testing.T) {
			for _, d := range testData {
				t.Run(d.name, func(t *testing.T) {
					...
					rangeKeys := MakeCollectableGCRangesFromGCRequests(rangeStart, rangeEnd, d.request)
					require.NoError(t, MVCCGarbageCollectRanges(ctx, engine, &ms, rangeKeys, ts),
		...
		t.Run(engineImpl.name, func(t *testing.T) {
			for _, d := range testData {
				t.Run(d.name, func(t *testing.T) {
					...
					rangeKeys := MakeCollectableGCRangesFromGCRequests(rangeStart, rangeEnd, d.request)
					err := MVCCGarbageCollectRanges(ctx, engine, nil, rangeKeys, ts)
