...
func newGuard(spans *spanset.SpanSet, ts hlc.Timestamp) *Guard {
	...
	if nLatches <= 1 {
		...
		guard = &alloc.g
		latches = alloc.latches[:nLatches]
	} else if nLatches <= 2 {
		alloc := new(struct {
			g       Guard
			latches [2]latch
...
// snapshot captures an immutable snapshot of the latch manager. It takes a
// spanset to limit the amount of state captured. Must be called with mu held.
func (m *Manager) snapshot(spans *spanset.SpanSet) snapshot {
	var snap snapshot
	for s := spanset.SpanScope(0); s < spanset.NumSpanScope; s++ {
		sm := &m.scopes[s]
...
func (sm *scopedManager) flushReadSet() {
...
// insert inserts the latches owned by the provided Guard into the Manager.
// Must be called with mu held.
func (m *Manager) insert(lg *Guard) {
	for s := spanset.SpanScope(0); s < spanset.NumSpanScope; s++ {
		sm := &m.scopes[s]
		for a := spanset.SpanAccess(0); a < spanset.NumSpanAccess; a++ {
