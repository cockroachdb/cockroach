...
type SpanSet struct {
	spanLatches [NumSpanAccess][NumSpanScope][]spanLatch
}
...
func (s *SpanSet) String() string {
	...
	for sa := SpanAccess(0); sa < NumSpanAccess; sa++ {
		for ss := SpanScope(0); ss < NumSpanScope; ss++ {
			for _, sl := range s.getSpanLatches(sa, ss) {
...
func (s *SpanSet) Len() int {
	...
	for sa := SpanAccess(0); sa < NumSpanAccess; sa++ {
		for ss := SpanScope(0); ss < NumSpanScope; ss++ {
			count += len(s.getSpanLatches(sa, ss))
		}
	}
	return count
}
...
func (s *SpanSet) Reserve(access SpanAccess, scope SpanScope, n int) {
	existing := s.spanLatches[access][scope]
	s.spanLatches[access][scope] = make([]spanLatch, len(existing), n+cap(existing))
	copy(s.spanLatches[access][scope], existing)
}
...
func (s *SpanSet) AddAt(access SpanAccess, span roachpb.Span, timestamp hlc.Timestamp) {
	...
	s.spanLatches[access][scope] = append(s.spanLatches[access][scope], spanLatch{span: span, ts: timestamp})
}
...
func (s *SpanSet) SortAndDedup() {
	for sa := SpanAccess(0); sa < NumSpanAccess; sa++ {
		for ss := SpanScope(0); ss < NumSpanScope; ss++ {
			s.spanLatches[sa][ss], _ /* distinct */ = mergeSpanLatches(s.spanLatches[sa][ss])
		}
	}
}

// getSpanLatches returns a slice of span latches with the given parameters.
func (s *SpanSet) getSpanLatches(access SpanAccess, scope SpanScope) []spanLatch {
	return s.spanLatches[access][scope]
}
...
func (s *SpanSet) BoundarySpan(scope SpanScope) roachpb.Span {
	...
	for sa := SpanAccess(0); sa < NumSpanAccess; sa++ {
		for _, sl := range s.spanLatches[sa][scope] {
			if !boundary.Valid() {
				boundary = sl.span
				continue
			}
...
func (s *SpanSet) CheckAllowedAt(access SpanAccess, span roachpb.Span, timestamp hlc.Timestamp) error {
	...
	for ac := access; ac < NumSpanAccess; ac++ {
		for _, sl := range s.spanLatches[ac][scope] {
