...
type sortedLatches []latch

func (s sortedLatches) Less(i, j int) bool {
...
func (s sortedLatches) Swap(i, j int) {
	s[i], s[j] = s[j], s[i]
}

func (s sortedLatches) Len() int {
	return len(s)
}

// mergeSpanLatches sorts the given span latches and merges ones with overlapping
// spans. The implementation is a graft of roachpb.MergeSpans.
//
// Returns true iff all of the spans are distinct.
// The input spans are not safe for re-use.
func mergeSpanLatches(latches []latch) ([]latch, bool) {
	if len(latches) == 0 {
		return latches, true
	}

	sort.Sort(sortedLatches(latches))
