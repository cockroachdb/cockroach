...
const (
	...
	// a single batch. Batches that span many ranges (which is possible for the
	// commit of a transaction that spans many ranges) will be split into many
	// batches by the DistSender, leading to high CPU overhead and quadratic
	// memory usage.
...
// writes to determining whether any of them failed. While doing so, it also
// monitors the state of the transaction and returns early if it ever changes.
// Once the result of all in-flight writes is determined, the method issues a
// RecoverTxn request with a summary of their outcome.
func (m *manager) resolveIndeterminateCommitForTxn(
	...
	return resTxn, m.stopper.RunTaskWithErr(ctx,
...
// transaction's in-flight writes to determining whether any of them failed.
// While doing so, it also monitors the state of the transaction and returns
// early if it ever changes.
func (m *manager) resolveIndeterminateCommitForTxnProbe(
	...
	for seq, idx := range txn.InFlightWrites {
		if len(txn.Intents) <= int(idx) {
			return false, nil, errors.Errorf("malformed in-flight write ref %d->%d: %v", seq, idx, txn)
		}
		span := txn.Intents[idx]
		if len(span.EndKey) != 0 {
			return false, nil, errors.Errorf("in-flight write references ranged intent span %s: %v", span, txn)
		}
...
func (m *manager) resolveIndeterminateCommitForTxnRecover(
	...
	b.AddRawRequest(&roachpb.RecoverTxnRequest{
		...
		Txn:            txn.TxnMeta,
		AllWritesFound: !preventedIntent,
	})
