...
func addInFlightWrite(txn *roachpb.Transaction, key roachpb.Key, seq int32) {
	txn.Intents = append(txn.Intents, roachpb.Span{Key: key})
	if txn.InFlightWrites == nil {
		txn.InFlightWrites = make(map[int32]int32)
	}
	...
	testutils.RunTrueAndFalse(t, "prevent", func(t *testing.T, prevent bool) {
		mockSender = client.SenderFunc(func(
			...
			mockSender = client.SenderFunc(func(
				...
				assert.Equal(t, !prevent, recTxnReq.AllWritesFound)
		...
		t.Run(c.name, func(t *testing.T) {
			mockSender = client.SenderFunc(func(
				...
				mockSender = client.SenderFunc(func(
					...
					assert.Equal(t, roachpb.Key(txn.Key), recTxnReq.Key)
					assert.Equal(t, txn.TxnMeta, recTxnReq.Txn)
					assert.Equal(t, false, recTxnReq.AllWritesFound)

					br = ba.CreateReply()
					br.Responses[0].GetInner().(*roachpb.RecoverTxnResponse).RecoveredTxn = c.changedTxn
					return br, nil
				})
				return br, nil
			})
...
func TestResolveIndeterminateCommitTxnWithoutInFlightWrites(t *testing.T) {
	...
	mockSender = client.SenderFunc(func(
		...
		assert.Equal(t, roachpb.Key(txn.Key), recTxnReq.Key)
		assert.Equal(t, txn.TxnMeta, recTxnReq.Txn)
		assert.Equal(t, true, recTxnReq.AllWritesFound)
