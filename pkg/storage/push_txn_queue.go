		...
		case <-queryCh:
			...
			if req.PusherTxn.ID != nil {
				...
				if pErr != nil {
					...
					if haveDependency || log.V(2) {
						var dependents []string
						for id := range push.mu.dependents {
							dependents = append(dependents, id.Short())
						}
						dependencies = strings.Join(dependents, ", ")
						if log.V(2) {
							log.Infof(ctx, "%s has dependencies=%s", req.PusherTxn.ID.Short(), dependencies)
						}
					}
					push.mu.Unlock()
