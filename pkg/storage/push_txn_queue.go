...
// proceed without queueing. This is true for TOUCH pushes, but also
// for ABORT and TIMESTAMP pushes where the pushee has min priority or
...
func isExpired(now hlc.Timestamp, txn *roachpb.Transaction) bool {
	expires := txn.LastActive().Add(2*base.DefaultHeartbeatInterval.Nanoseconds(), 0)
	return expires.Less(now)
}
...
func (ptq *pushTxnQueue) MaybeWait(
	...
	for {
		var pusheeTxnExpiresCh <-chan time.Time
