...
func (ptq *pushTxnQueue) MaybeWaitForQuery(
	...
	// If the transaction we're waiting to query has a queue of pushers
	...
	if ok && ptq.isTxnUpdated(pending, req) {
		...
		// If the transaction we're waiting to query has no queue of
		...
		// updated (i.e. already been aborted).
