...
func TestGCQueueScoreString(t *testing.T) {
	...
	for i, c := range []struct {
		...
		{gcQueueScore{
			...
			`queue=true with 4.31/fuzz(1.25)=3.45=valScalScore(4.00)*deadFrac(0.25)+intentScore(0.45)
...
func TestGCQueueShouldQueueInvariantQuick(t *testing.T) {
	defer leaktest.AfterTest(t)()
	...
	if err := quick.Check(func(
		...
		r := shouldQueueImpl(ctx, int64(seed), now, ms, ttlSec)
		wouldHaveToDeleteSomething := gcBytes*int64(ttlSec) < ms.GCByteAge(now.WallTime)
		result := r.FinalScore == 0 || wouldHaveToDeleteSomething
		if !result {
...
func (cws *cachedWriteSimulator) singleKeySteady(
	...
	cached, ok := cws.cache[cachee{*ms, cacheKey}]
	if !ok && duration > 0 {
...
func (cws *cachedWriteSimulator) shouldQueue(
	...
	r := shouldQueueImpl(context.Background(), 0 /* seed */, ts, ms, int32(ttl.Seconds()))
...
// TestGCQueueShouldQueue verifies conditions which inform priority and whether
// or not the range should be queued into the GC queue. Ranges are queued for GC
// based on two conditions. The age of bytes available to be GC'd, and the age
// of unresolved intents.
func TestGCQueueShouldQueue(t *testing.T) {
	defer leaktest.AfterTest(t)()

	// Now on to "real-life" situations.
	...
	{
		// Hammer a key with 1MB blobs for a minute. Logically, at the end of that
		// you have around 60MB that are non-live, but cannot immediately be
		// deleted. Deletion is only possible for keys that are (more or less) past
		// TTL, and shouldQueue should respect that.
		ms := initialMS()
