...
func TestGCQueueMakeGCScoreRealistic(t *testing.T) {
...

	{
		...
		cws.shouldQueue(true, 29.59, time.Duration(0), 0, ms)
		cws.shouldQueue(true, 29.59, time.Duration(0), 0, ms)

		// Right after we finished writing, we don't want to GC yet with a one-minute TTL.
		cws.shouldQueue(false, 0.49, time.Duration(0), minuteTTL, ms)
		...
		cws.shouldQueue(true, 2.19, 7*time.Minute/4, minuteTTL, ms)
		// After an hour, that's (of course) still true with a very high priority.
		cws.shouldQueue(true, 58.70, time.Hour, minuteTTL, ms)
		...
		cws.shouldQueue(false, 0.98, time.Hour, hourTTL, ms)
		...
		cws.shouldQueue(false, 1.46, 90*time.Minute, hourTTL, ms)
		cws.shouldQueue(true, 2.03, 125*time.Minute, hourTTL, ms)
	}

	{
		...
		cws.shouldQueue(false, 0.70, 12*time.Hour, minuteTTL, ms)

		// Two days in we're more than ready to go, would queue for GC, and
		// delete.
		cws.shouldQueue(true, 2.82, 48*time.Hour, minuteTTL, ms)
	}

	{
		...
		cws.shouldQueue(false, 0.99, 24*time.Hour, irrelevantTTL, ms)
		cws.shouldQueue(false, 1.97, 2*24*time.Hour, irrelevantTTL, ms)
		cws.shouldQueue(false, 3.95, 4*24*time.Hour, irrelevantTTL, ms)
		cws.shouldQueue(false, 6.90, 7*24*time.Hour, irrelevantTTL, ms)
		cws.shouldQueue(true, 11.84, 12*24*time.Hour, irrelevantTTL, ms)
	}
}
...
func TestGCQueueChunkRequests(t *testing.T) {
	...
	const keySize = gcKeyVersionChunkBytes / keyCount
