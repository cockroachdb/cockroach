...
func evalEndTransaction(
	...
	if args.Commit {
		...
		// itself contains intents, in the RightData snapshot, that will be
		// owned and thus resolved by the new range.
...
func (r *Replica) AdminMerge(
	...
	if err := r.store.DB().Txn(ctx, func(ctx context.Context, txn *client.Txn) error {
		...
		// trigger.
		br, pErr := client.SendWrapped(ctx, r.store.DB().GetSender(), &roachpb.SubsumeRequest{
...
func mergeTrigger(
	...
	//
	// TODO(benesch): this may need to be tuned.
	...
	// Rewrite the abort span from the RHS with the LHS's range ID.
	var ignoredMS enginepb.MVCCStats
	_, err := rec.AbortSpan().CopyFrom(ctx, eng, &ignoredMS, merge.RightDesc.RangeID)
	if err != nil {
		return result.Result{}, err
	}
