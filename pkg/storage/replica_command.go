...
func (r *Replica) adminSplitWithDescriptor(
	...
	rightDesc, err := r.store.NewRangeDescriptor(ctx, splitKey, desc.EndKey, desc.Replicas().Wrapped())
	if err != nil {
...
func (r *Replica) AdminMerge(
	...
	runMergeTxn := func(txn *client.Txn) error {
		...
		if l, r := origLeftDesc.Replicas(), rightDesc.Replicas(); !replicaSetsEqual(l.Wrapped(), r.Wrapped()) {
...
func waitForApplication(
	...
	return contextutil.RunWithTimeout(ctx, "wait for application", 5*time.Second, func(ctx context.Context) error {
		...
		for _, repl := range desc.Replicas().Wrapped() {
...
func (r *Replica) changeReplicas(
	...
	for i, existingRep := range desc.Replicas().Wrapped() {
	...
	case roachpb.ADD_REPLICA:
		...
		newReplicas := updatedDesc.Replicas()
		newReplicas.AddReplica(repDesc)
		updatedDesc.SetReplicas(newReplicas)
	...
	case roachpb.REMOVE_REPLICA:
		...
		// WIP replica removal
		updatedDesc.InternalReplicas[repDescIdx] = updatedDesc.InternalReplicas[len(updatedDesc.InternalReplicas)-1]
		updatedDesc.InternalReplicas = updatedDesc.InternalReplicas[:len(updatedDesc.InternalReplicas)-1]
	}
		...
		b.AddRawRequest(&roachpb.EndTransactionRequest{
			...
			InternalCommitTrigger: &roachpb.InternalCommitTrigger{
				...
				ChangeReplicasTrigger: &roachpb.ChangeReplicasTrigger{
					...
					Replica:         repDesc,
					UpdatedReplicas: updatedDesc.Replicas().Wrapped(),
...
func (s *Store) AdminRelocateRange(
	...
	replicas := rangeDesc.Replicas().DeepCopy()
	...
	for _, t := range targets {
		found := false
		for _, replicaDesc := range replicas.Wrapped() {
	...
	updateRangeDesc := func(
		...
		case roachpb.REMOVE_REPLICA:
			// WIP replica removal
			desc.InternalReplicas = removeTargetFromSlice(desc.InternalReplicas, target)
		default:
	...
	for len(addTargets) > 0 || len(removeTargets) > 0 {
		...
		if len(addTargets) > 0 && len(addTargets) >= len(removeTargets) {
			...
			targetStore, _ := s.allocator.allocateTargetFromList(
				...
				storeList,
				zone,
				rangeInfo.Desc.Replicas().Wrapped(),
				rangeInfo,
...
func (r *Replica) adminScatter(
	...
	if args.RandomizeLeases && r.OwnsValidLease(r.store.Clock().Now()) {
		desc := r.Desc()
		newLeaseholderIdx := rand.Intn(len(desc.Replicas().Wrapped()))
		targetStoreID := desc.Replicas().Wrapped()[newLeaseholderIdx].StoreID
