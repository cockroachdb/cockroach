...
func (r *Replica) adminSplitWithDescriptor(
	...
	// Note that the client API for the sticky bit has it as non-nullable, but
	// the internal representation has it for nullable for backwards
	// compatibility. We still need to check if stickyBitEnabled because the byte
	// representation of setting the stickyBit to nil is different than setting
	// it to hlc.Timestamp{}. This check ensures that CPuts would not fail on
	// older versions.
	stickyBitEnabled := r.store.ClusterSettings().Version.IsActive(cluster.VersionStickyBit)
	...
	if desc.StartKey.Equal(splitKey) {
		...
		if stickyBitEnabled && (desc.StickyBit == nil || desc.StickyBit.Less(args.ExpirationTime)) {
			newDesc := *desc
