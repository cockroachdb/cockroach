...
func (r *Replica) addReplica(
	...
	if fn := r.store.cfg.TestingKnobs.ReplicaAddStopAfterLearner; fn != nil {
		if fn() {
			return learnerDesc, nil
		}
	}
	...
	if err != nil {
		...
		rollbackLearnerReplica(ctx, r.store, learnerDesc, target, reason, details)
		return nil, err
	}
...
func rollbackLearnerReplica(
	...
	// If (for example) the promotion failed because of
	// a context deadline exceeded, we do still want to clean up after
	// ourselves, so always use a new context (but with the old tags and with
	...
	defer cancel()
	ctx = logtags.WithTags(ctx, tags)
	if err := execChangeReplicasTxn(
		ctx, store, roachpb.REMOVE_REPLICA, desc, replDesc, &newDesc, reason, details,
	); err != nil {
		log.Infof(ctx,
