...
func (r *Replica) adminScatter(
	...
	refreshDescAndZone := func() error {
		if r.IsDestroyed() == nil {
			// If our replica still exists, we can just ask it for the updated
			// descriptor.
			desc = r.Desc()
		} else {
			// Otherwise, we need to issue a RangeLookupRequest.
			b := &client.Batch{}
			b.AddRawRequest(&roachpb.RangeLookupRequest{
				Span: roachpb.Span{
					Key: keys.RangeMetaKey(r.Desc().StartKey),
				},
				MaxRanges: 1,
			})
	...
	// Limit concurrent scatter requests to avoid dumping too many leases or
	// replicas on one underfull store when many ranges are scattered
	select {
	case r.store.scatterSem <- struct{}{}:
	case <-ctx.Done():
	...
	for i := int32(0); i < zone.NumReplicas; i++ {
		...
		if err != nil {
			...
			default:
				<-r.store.scatterSem
				return roachpb.AdminScatterResponse{}, errors.Wrap(err, fmt.Sprintf("%s", r))
			}
		}
		if ctx.Err() != nil {
			<-r.store.scatterSem
	...
	for re := retry.StartWithCtx(ctx, retryOpts); re.Next(); {
		...
		if !r.IsLeaseValid(lease, r.store.Clock().Now()) {
			...
			// already transferred it away to balance the cluster. Not an error; move
			// on to the next step.
			break
		}
