...
// scatter moves replicas and leaseholders for a selection of ranges. Scatter is
// best-effort; ranges that cannot be moved will include an error detail in the
// response and won't fail the request.
func (r *Replica) scatter(
	ctx context.Context, args roachpb.ScatterRequest,
) (roachpb.ScatterResponse, error) {
	db := r.store.DB()
	rangeDesc := *r.Desc()
