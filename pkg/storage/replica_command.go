...
type Command struct {
	...
	// *CommandArgs.Stats.
	Eval func(context.Context, engine.ReadWriter, batcheval.CommandArgs, roachpb.Response) (batcheval.Result, error)
}
...
func evaluateCommand(
	...
	if h.ReturnRangeInfo {
		...
		if err != nil {
			return batcheval.Result{}, roachpb.NewError(err)
		}
		desc, err := rec.Desc()
		if err != nil {
			return batcheval.Result{}, roachpb.NewError(err)
...
func intentsToEvalResult(
	...
	if alwaysReturn {
		pd.Local.IntentsAlways = &[]batcheval.IntentsWithArg{{Args: args, Intents: intents}}
	} else {
		pd.Local.Intents = &[]batcheval.IntentsWithArg{{Args: args, Intents: intents}}
	}
	return pd
}
...
func evalPut(
	...
	if args.Blind {
		return batcheval.Result{}, engine.MVCCBlindPut(ctx, batch, ms, args.Key, ts, args.Value, h.Txn)
	}
	return batcheval.Result{}, engine.MVCCPut(ctx, batch, ms, args.Key, ts, args.Value, h.Txn)
}
*/
...
func evalConditionalPut(
	...
	if args.Blind {
		return batcheval.Result{}, engine.MVCCBlindConditionalPut(ctx, batch, cArgs.Stats, args.Key, h.Timestamp, args.Value, args.ExpValue, h.Txn)
	}
	return batcheval.Result{}, engine.MVCCConditionalPut(ctx, batch, cArgs.Stats, args.Key, h.Timestamp, args.Value, args.ExpValue, h.Txn)
}
...
func evalInitPut(
	...
	if args.Blind {
		return batcheval.Result{}, engine.MVCCBlindInitPut(ctx, batch, cArgs.Stats, args.Key, h.Timestamp, args.Value, args.FailOnTombstones, h.Txn)
	}
	return batcheval.Result{}, engine.MVCCInitPut(ctx, batch, cArgs.Stats, args.Key, h.Timestamp, args.Value, args.FailOnTombstones, h.Txn)
}
...
func evalDelete(
	...
	return batcheval.Result{}, engine.MVCCDelete(ctx, batch, cArgs.Stats, args.Key, h.Timestamp, h.Txn)
}
...
func evalBeginTransaction(
	...
	if ok {
		...
		case roachpb.ABORTED:
			...
			// txn.
			return batcheval.Result{}, roachpb.NewTransactionAbortedError()
		...
		case roachpb.PENDING:
			if h.Txn.Epoch > tmpTxn.Epoch {
				...
				return batcheval.Result{}, roachpb.NewTransactionRetryError(roachpb.RETRY_POSSIBLE_REPLAY)
			}

		case roachpb.COMMITTED:
			return batcheval.Result{}, roachpb.NewTransactionStatusError(
	...
	return batcheval.Result{}, engine.MVCCPutProto(ctx, batch, cArgs.Stats, key, hlc.Timestamp{}, nil, reply.Txn)
}
...
func evalEndTransaction(
	...
	if args.Require1PC {
		return batcheval.Result{}, roachpb.NewTransactionStatusError("could not commit in one phase as requested")
	}
	...
	if ok, err := engine.MVCCGetProto(
		...
		return batcheval.Result{}, roachpb.NewTransactionStatusError("does not exist")
	}
	...
	case roachpb.COMMITTED:
		return batcheval.Result{}, roachpb.NewTransactionStatusError("already committed")
	...
	if args.Commit {
		if retry, reason := isEndTransactionTriggeringRetryError(h.Txn, reply.Txn); retry {
			return batcheval.Result{}, roachpb.NewTransactionRetryError(reason)
		}
	...
	if reply.Txn.Status == roachpb.COMMITTED {
		...
		if pd, err = runCommitTrigger(ctx, cArgs.EvalCtx, batch.(engine.Batch), ms, *args, reply.Txn); err != nil {
			return batcheval.Result{}, NewReplicaCorruptionError(err)
		}
	}
...
func runCommitTrigger(
	ctx context.Context,
	rec batcheval.ReplicaEvalContextI,
	batch engine.Batch,
	ms *enginepb.MVCCStats,
	args roachpb.EndTransactionRequest,
	txn *roachpb.Transaction,
...
func evalRangeLookup(
	...
	if !key.Equal(args.Key) {
		return batcheval.Result{}, errors.Errorf("illegal lookup of range-local key %q", args.Key)
	}
	ts, txn, consistent, rangeCount := h.Timestamp, h.Txn, h.ReadConsistency != roachpb.INCONSISTENT, int64(args.MaxRanges)
	if rangeCount < 1 {
		return batcheval.Result{}, errors.Errorf("range lookup specified invalid maximum range count %d: must be > 0", rangeCount)
	}
...
func evalHeartbeatTxn(
	...
	if ok, err := engine.MVCCGetProto(ctx, batch, key, hlc.Timestamp{}, true, nil, &txn); err != nil {
		...
		return batcheval.Result{}, errors.Errorf("heartbeat for transaction %s failed; record not present", h.Txn)
	}
...
func evalPushTxn(
	...
	if cArgs.Header.Txn != nil {
		return batcheval.Result{}, errTransactionUnsupported
	}
	if args.Now == (hlc.Timestamp{}) {
		return batcheval.Result{}, errors.Errorf("the field Now must be provided")
	}
	if args.PushType == roachpb.PUSH_QUERY {
		return batcheval.Result{}, errors.Errorf("PUSH_QUERY no longer supported")
	}

	if !bytes.Equal(args.Key, args.PusheeTxn.Key) {
		return batcheval.Result{}, errors.Errorf("request key %s should match pushee's txn key %s", args.Key, args.PusheeTxn.Key)
	}
...
func evalQueryTxn(
	...
	if !bytes.Equal(args.Key, args.Txn.Key) {
		return batcheval.Result{}, errors.Errorf("request key %s does not match txn key %s", args.Key, args.Txn.Key)
	}
	...
	// HACK reply.WaitingTxns = cArgs.EvalCtx.pushTxnQueue().GetDependents(args.Txn.ID)
	return batcheval.Result{}, nil
}
...
func evalResolveIntent(
	...
	if intent.Status == roachpb.ABORTED {
		return batcheval.Result{}, setAbortCache(ctx, cArgs.EvalCtx, batch, ms, args.IntentTxn, args.Poison)
	}
	return batcheval.Result{}, nil
}
...
func evalMerge(
	...
	return batcheval.Result{}, engine.MVCCMerge(ctx, batch, cArgs.Stats, args.Key, h.Timestamp, args.Value)
}
...
func evalTruncateLog(
	...
	if cArgs.EvalCtx.ClusterSettings().Version.IsActive(cluster.VersionRaftLogTruncationBelowRaft) {
		...
		if err != nil {
			return batcheval.Result{}, errors.Wrap(err, "while computing stats of Raft log freed by truncation")
		}
...
func newFailedLeaseTrigger(isTransfer bool) batcheval.Result {
	var trigger batcheval.Result
	trigger.Local.LeaseMetricsResult = new(storagebase.LeaseMetricsType)
	if isTransfer {
		*trigger.Local.LeaseMetricsResult = storagebase.LeaseTransferError
	} else {
		*trigger.Local.LeaseMetricsResult = storagebase.LeaseRequestError
	}
	return trigger
}
...
func evalNewLease(
	...
	pd.Local.LeaseMetricsResult = new(storagebase.LeaseMetricsType)
	if isTransfer {
		*pd.Local.LeaseMetricsResult = storagebase.LeaseTransferSuccess
	} else {
		*pd.Local.LeaseMetricsResult = storagebase.LeaseRequestSuccess
	}
	return pd, nil
}
...
func splitTrigger(
	...
	ts hlc.Timestamp,
) (enginepb.MVCCStats, batcheval.Result, error) {
	...
	desc, err := rec.Desc()
	if err != nil {
		return enginepb.MVCCStats{}, batcheval.Result{}, err
	}
	if !bytes.Equal(desc.StartKey, split.LeftDesc.StartKey) ||
		!bytes.Equal(desc.EndKey, split.RightDesc.EndKey) {
		return enginepb.MVCCStats{}, batcheval.Result{}, errors.Errorf("range does not match splits: (%s-%s) + (%s-%s) != %s",
	...
	if err != nil {
		return enginepb.MVCCStats{}, batcheval.Result{}, errors.Wrap(err, "unable to compute stats for LHS range after split")
	}
	...
	if err != nil {
		return enginepb.MVCCStats{}, batcheval.Result{}, errors.Wrap(err, "unable to fetch last replica GC timestamp")
	}
	if err := engine.MVCCPutProto(ctx, batch, nil, keys.RangeLastReplicaGCTimestampKey(split.RightDesc.RangeID), hlc.Timestamp{}, nil, &replicaGCTS); err != nil {
		return enginepb.MVCCStats{}, batcheval.Result{}, errors.Wrap(err, "unable to copy last replica GC timestamp")
	}
	...
	if err != nil {
		...
		return enginepb.MVCCStats{}, batcheval.Result{}, errors.Wrap(err, "unable to copy abort cache to RHS split range")
	}
	...
	if origBothMS.ContainsEstimates || bothDeltaMS.ContainsEstimates {
		...
		if err != nil {
			return enginepb.MVCCStats{}, batcheval.Result{}, errors.Wrap(err, "unable to compute stats for RHS range after split")
		}
	} else {
	...
	// both sides, we need to update it as well.
	{
		...
		if err := engine.AccountForSelf(&rightMS, split.RightDesc.RangeID); err != nil {
			return enginepb.MVCCStats{}, batcheval.Result{}, errors.Wrap(err, "unable to account for enginepb.MVCCStats's own stats impact")
		}
		...
		if err != nil {
			return enginepb.MVCCStats{}, batcheval.Result{}, errors.Wrap(err, "unable to load lease")
		}
		...
		if !found {
			return enginepb.MVCCStats{}, batcheval.Result{}, errors.Errorf(
		...
		if err != nil {
			return enginepb.MVCCStats{}, batcheval.Result{}, errors.Wrap(err, "unable to load GCThreshold")
		}
		...
		if err != nil {
			return enginepb.MVCCStats{}, batcheval.Result{}, errors.Wrap(err, "unable to load TxnSpanGCThreshold")
		}
		...
		if err != nil {
			return enginepb.MVCCStats{}, batcheval.Result{}, errors.Wrap(err, "unable to write initial Replica state")
		}
		...
		if !st.Version.IsActive(cluster.VersionSplitHardStateBelowRaft) {
			...
			if err := rsl.SynthesizeRaftState(ctx, batch); err != nil {
				return enginepb.MVCCStats{}, batcheval.Result{}, errors.Wrap(err, "unable to synthesize initial Raft state")
			}
		}
...
func mergeTrigger(
	...
	if !bytes.Equal(desc.StartKey, merge.LeftDesc.StartKey) {
		return batcheval.Result{}, errors.Errorf("LHS range start keys do not match: %s != %s",
			desc.StartKey, merge.LeftDesc.StartKey)
	}

	if !desc.EndKey.Less(merge.LeftDesc.EndKey) {
		return batcheval.Result{}, errors.Errorf("original LHS end key is not less than the post merge end key: %s >= %s",
			desc.EndKey, merge.LeftDesc.EndKey)
	}

	rightRangeID := merge.RightDesc.RangeID
	if rightRangeID <= 0 {
		return batcheval.Result{}, errors.Errorf("RHS range ID must be provided: %d", rightRangeID)
	}
	...
	*ms = enginepb.MVCCStats{}
	...
	if _, err := rec.AbortCache().CopyFrom(ctx, batch, &mergedMS, rightRangeID); err != nil {
		return batcheval.Result{}, errors.Errorf("unable to copy abort cache to new split range: %s", err)
	}
	...
	if _, _, _, err := engine.MVCCDeleteRange(ctx, batch, nil, localRangeIDKeyPrefix, localRangeIDKeyPrefix.PrefixEnd(), math.MaxInt64, hlc.Timestamp{}, nil, false); err != nil {
		return batcheval.Result{}, errors.Errorf("cannot remove range metadata %s", err)
	}
	...
	if err != nil {
		return batcheval.Result{}, errors.Errorf("unable to compute RHS range's local stats: %s", err)
	}
	mergedMS.Add(msRange)

	// Set stats for updated range.
	if err := rec.MakeReplicaStateLoader().SetMVCCStats(ctx, batch, &mergedMS); err != nil {
		return batcheval.Result{}, errors.Errorf("unable to write MVCC stats: %s", err)
	}
