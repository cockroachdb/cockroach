...
// memtable and existing SSTs in RocksDB. These SST files are created lazily, so
// in the case where the keyspace is empty, no file will be created. Each of the
// SSTs has a range deletion tombstone written to it to delete the existing data
// in the range, however if the keyspace is empty, then the tombstone will be
// omited.
//
