...
func TestTimestampCacheLargeKeys(t *testing.T) {
	...
	forEachCacheImpl(t, func(t *testing.T, tc Cache, clock *hlc.Clock, manual *hlc.ManualClock) {
		...
		tc.Add(keyStart, keyEnd, ts, txn, true)
		if ts, txn := tc.GetMaxRead(keyStart, keyEnd); ts != ts {
			t.Errorf("expected key range to have timestamp %s, but found %s", ts, ts)
		} else if txn != txn {
			t.Errorf("expected key range to have txn id %s, but found %s", txn, txn)
		}
	})
}
