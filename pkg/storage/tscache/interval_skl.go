...
const (
	...
	// This is set on nodes who were unable to ratchet their values at some
...
func (p *sklPage) addNode(
	...
	return p.ratchetValueSet(it, keyVal, gapVal, false /* setInit */)
}
...
//    because an arena is full, the nodes must never be initialized.
//
...
func (p *sklPage) ensureInitialized(it *arenaskl.Iterator, key []byte) error {
	...
	// If the node isn't initialized, do so.
	if (it.Meta() & initialized) == 0 {
		return p.ratchetValueSet(it, prevGapVal, prevGapVal, true /* setInit */)
	}
	return nil
}
...
func (p *sklPage) ensureFloorValue(it *arenaskl.Iterator, to []byte, val cacheValue) bool {
	for it.Valid() {
		...
		err := p.ratchetValueSet(it, val, val, false /* setInit */)
		switch err {
		case nil:
			// Continue scanning.
		case arenaskl.ErrArenaFull:
...
func (p *sklPage) ratchetValueSet(
	...
	for {
		...
		newMeta := meta
		updateInit := setInit && (meta&initialized) == 0
		if updateInit {
			// If the meta has the cantInit flag set to true, we fail with an
			// ErrArenaFull error to force whoever is trying to initialize this
			// node to try on a new page.
			if (meta & cantInit) != 0 {
		...
		if updateVals {
			...
			case arenaskl.ErrArenaFull:
				...
				if (meta&initialized) == 0 && (meta&cantInit) == 0 {
...
// maximum value found. When finished, the iterator will be positioned the same
// as if it.Seek(to) had been called.
func (p *sklPage) maxInRange(it *arenaskl.Iterator, from, to []byte, opt rangeOptions) cacheValue {
	...
	// Scan the rest of the way.
	_, maxVal := p.scanTo(it, to, opt)
	maxVal, _ = ratchetValue(maxVal, prevGapVal)
	return maxVal
}
...
func (p *sklPage) incomingGapVal(it *arenaskl.Iterator, key []byte) cacheValue {
	...
	prevGapVal, _ := p.scanTo(it, key, 0)
	return prevGapVal
}

// scanTo scans from the current iterator position until the key "to". The
// function then returns the maximum value seen along with the gap value at the
// end of the scan. If the iterator is positioned at a key > "to", the function
// will return empty values. When finished, the iterator will be positioned the
// same as if it.Seek(to) had been called.
func (p *sklPage) scanTo(
	it *arenaskl.Iterator, to []byte, opt rangeOptions,
	...
	for {
		...
		// Ratchet uninitialized nodes.
		ratchetByPrev := false
		if (it.Meta() & initialized) == 0 {
			// Since the node is not initialized, its values are not final.
			// Ratchet them by the previous gap value.
			err := p.ratchetValueSet(it, prevGapVal, prevGapVal, false /* setInit */)
			if err == arenaskl.ErrArenaFull {
				// If we fail to ratchet the node, the ratcheting won't be
				// reflected in the decoded values below. Perform the ratcheting
				// manually.
				ratchetByPrev = true
			}
		}
