...
//
// Each key in the intervalSkl is associated with the latest read timestamp for
// that key. In addition, the data structure maintains the latest read timestamp
// for the range of keys between itself and the next key that is present in the
// skiplist. This space between keys is called the "gap", and the timestamp for
// that range is called the "gap timestamp". Here is a simplified representation
// that would result after these ranges were added to an empty intervalSkl:
//   ["apple", "orange") = 200
//   ["kiwi", "raspberry"] = 100
//
...
func (s *intervalSkl) rotatePages(filledPage *sklPage) {
	...
	if s.earlier != nil {
		...
		if s.floorTs.Less(newFloorTs) {
			s.floorTs = newFloorTs
		}
	}
...
// has filled up, it returns an error. At that point, a new fixed page must
// be allocated and used instead.
type sklPage struct {
...
func (p *sklPage) addNode(
	...
	// TODO(nvanbenschoten): This will be forced onto the heap when we slice it
	// and pass the slice to other functions. We should look into making a byte
	// slice reservation in the Skiplist's arena and writing directly to that.
...
func (p *sklPage) ratchetMaxTimestamp(ts hlc.Timestamp) {
	...
	// changes may force a transaction to lose "ownership" over a range, but
	// they'll never allow a transaction to gain "ownership" over a range that
	// it wouldn't otherwise have. In other words, it's ok for the intervalSkl
	// to produce false negatives but never ok for it to produce false
	// positives.
	//
