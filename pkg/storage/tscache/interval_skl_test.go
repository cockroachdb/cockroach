...
func TestIntervalSklFill(t *testing.T) {
	...
	s := newIntervalSkl(nil /* clock */, 0 /* minRet */, 1500)
...
func TestIntervalSklFill2(t *testing.T) {
	...
	s := newIntervalSkl(nil /* clock */, 0 /* minRet */, 500)
	key := []byte("some key")
...
func TestIntervalSklMinRetentionWindow(t *testing.T) {
	...
	const minRet = 500
	s := newIntervalSkl(clock, minRet, 1500)
	s.floorTS = floorTS
...
func BenchmarkIntervalSklAdd(b *testing.B) {
	...
	s := newIntervalSkl(clock, MinRetentionWindow, pageSize)
	rng := rand.New(rand.NewSource(timeutil.Now().UnixNano()))
