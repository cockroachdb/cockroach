...
func ExampleTracker_Close_epochChange() {
	...
	fmt.Println("The next Close() returns the previous closed timestamp with an empty mlai map:", closed, mlai, ok, ".")
	...
	// Meanwhile a proposal arrives on range 2 and quickly finishes with LAI 2 but is still in epoch 2.
	//
	//   closed=0.000000000,1
	//       |            next=1.000000000,0
	//       |          left | right
	...
	// The next Close() returns the previous closed timestamp with an empty mlai map: 1.000000000,0 map[] true .
