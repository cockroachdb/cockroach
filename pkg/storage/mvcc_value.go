...
// equates to a zero-length encoded value. For the extended encoding, this
// equates to an encoded MVCC value header with no additional value. To identify
// a deletion tombstone from an encoded MVCCValue, code should decode the value
// using DecodeMVCCValue and then use the IsTombstone method. For example:
//
