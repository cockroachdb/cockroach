...
// TODO(sumeer): randomized test with random intents that are interleaved and
...
func checkAndOutputIter(cmd string, iter MVCCIterator, b *strings.Builder) {
	valid, err := iter.Valid()
	if err != nil {
		fmt.Fprintf(b, "cmd: %s err: %s\n", cmd, err)
		return
	}
	if !valid {
		fmt.Fprintf(b, "cmd: %s .\n", cmd)
	...
	k2 := iter.Key()
	if !k1.Equal(k2) {
		fmt.Fprintf(b, "cmd: %s key: %s != %s\n", cmd, k1, k2)
		return
	}
	v1 := iter.UnsafeValue()
	v2 := iter.Value()
	if !bytes.Equal(v1, v2) {
		fmt.Fprintf(b, "cmd: %s value: %x != %x\n", cmd, v1, v2)
		return
	}
	if k1.Timestamp.IsEmpty() {
		var meta enginepb.MVCCMetadata
		if err := protoutil.Unmarshal(v1, &meta); err != nil {
			fmt.Fprintf(b, "cmd: %s meta parsing: %s\n", cmd, err)
			return
		}
		if hlc.Timestamp(meta.Timestamp).IsEmpty() {
			fmt.Fprintf(b, "cmd: %s meta k=%s\n", cmd, string(k1.Key))
		} else {
			...
			fmt.Fprintf(b, "cmd: %s meta k=%s ts=%d txn=%s%d\n",
				cmd, string(k1.Key), meta.Timestamp.WallTime, hiStr, uuid.Lo)
		}
		return
	}
	fmt.Fprintf(b, "cmd: %s value k=%s ts=%d v=%s\n",
		cmd, string(k1.Key), k1.Timestamp.WallTime, string(v1))
}
...
func TestIntentInterleavingIter(t *testing.T) {
	...
	datadriven.RunTest(t, "testdata/ii_iter", func(t *testing.T, d *datadriven.TestData) string {
		switch d.Cmd {
		case "define":
			if eng != nil {
				eng.Close()
			...
			for i, line := range strings.Split(d.Input, "\n") {
				...
				case "meta":
					...
					if mvccSection {
						...
						if err := batch.PutUnversioned(roachpb.Key(key), val); err != nil {
							return err.Error()
						}
					} else {
						ltKey := LockTableKey{
							Key: roachpb.Key(key), Strength: lock.Exclusive, TxnUUID: txnUUID[:]}
				...
				case "value":
					...
					mvccKey := MVCCKey{
						Key: roachpb.Key(key), Timestamp: hlc.Timestamp{WallTime: int64(ts)}}
					batch.PutMVCC(mvccKey, []byte(value))
				}
			}
			d.Pos = pos
		...
		case "iter":
			...
			if d.HasArg("lower") {
				var l string
				d.ScanArgs(t, "lower", &l)
				opts.LowerBound = []byte(l)
			}
			if d.HasArg("upper") {
				var u string
				d.ScanArgs(t, "upper", &u)
				opts.UpperBound = []byte(u)
			}
			if d.HasArg("prefix") {
			...
			for i, line := range strings.Split(d.Input, "\n") {
				...
				switch d.Cmd {
				case "seek-ge":
					iter.SeekGE(scanSeekKey(t, d))
					checkAndOutputIter("seek-ge", iter, &b)
				case "next":
					iter.Next()
					checkAndOutputIter("next", iter, &b)
				case "seek-lt":
					iter.SeekLT(scanSeekKey(t, d))
					checkAndOutputIter("seek-lt", iter, &b)
				case "prev":
					iter.Prev()
					checkAndOutputIter("prev", iter, &b)
				case "next-key":
					iter.NextKey()
					checkAndOutputIter("next-key", iter, &b)
				case "set-upper":
					var k string
					d.ScanArgs(t, "k", &k)
					iter.SetUpperBound(roachpb.Key(k))
					fmt.Fprintf(&b, "cmd: set-upper %s\n", k)
				default:
