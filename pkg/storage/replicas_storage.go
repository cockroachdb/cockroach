...
//   TODO: add discussion link from tbg's state machine comment PR.
//
...
//   being applied.
// The correctness problem with doing C3 before C2 is that the node violates
// raft promises it has made earlier. For example, if the state machine had
// applied index 20 and the raft log contained [15, 25), then this node is
// part of the quorum that causes [21, 25) to commit. If after the crash this
// node has applied index 20 and has no raft state, it is in effect an
...
// that we could later execute C1 when adding the range back to this node, and
...
type ReplicasStorage interface {
	...
	// TODO: also contains intent resolution of RHS RangeDescriptor.
	//
	...
	// TODO(sumeer): move D2 up to the first step. And make sure we are deleting
	// the RangeDescriptor. Synchronous resolution of the RHS RangeDescriptor is
	// already part of the smBatch. And the RHS is already durable due to WaitForApplication.
	// Don't need to sync I think after applying smBatch. Do the sync.
	//
	//
	// TODO: mention  mergeTombstoneReplicaID is also a thing.

	// REQUIRES: LHS and RHS are in state InitializedStateMachine, and RHS has
	// applied all commands up to the merge.
	// - Code above this layer ensures that the right-hand side of a merge is
	...
	//   descriptor. If a merge transaction commits, regardless of which
	//   replicas know about this yet, the LHS and RHS will be fully colocated.
	//
	//
	...
	//   See all callers of preDestroyRaftMyLocked.
	//   - There is also destruction as part of rebalancing. This happens as part of applying
	//     a raft entry with a ReplicatedEvalResult.ChangeReplicas. It also sets changeRemoveReplicas=true
	//     Delete the state machine and sync it. Commit is not synced necessarily. Even if synced,
	//     don't want to replay and remove since state machine already removed.****
	//     May also happen out of band. Don't want to know what this nextReplicaID logic is. It
	//     is too hard and complicated!!
