...
func (tc *timestampCache) add(
	...
	if tc.lowWater.Less(timestamp) {
		...
		for _, entry := range tcache.GetOverlaps(r.Start, r.End) {
			...
			// due change the size of the entry. To capture all of these
			// modifications we compute the current size of the entry and then use
			// the new size at the end of this iteration to update
			...
			if cv.timestamp.Less(timestamp) {
				...
				case sCmp <= 0 && eCmp >= 0:
					...
					continue // deleted the current entry, don't update tc.bytes
				case sCmp > 0 && eCmp < 0:
