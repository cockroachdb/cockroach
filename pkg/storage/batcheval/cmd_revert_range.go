...
func isEmptyKeyTimeRange(
	batch engine.ReadWriter, from, to roachpb.Key, since hlc.Timestamp,
) (bool, error) {
	...
	// that there is some key in the SST that is in the time range, so we might as
	// well proceed with our iteration that actually checks the timestamps on each
...
func RevertRange(
	...
	if empty, err := isEmptyKeyTimeRange(batch, args.Key, args.EndKey, args.TargetTime); err != nil || empty {
