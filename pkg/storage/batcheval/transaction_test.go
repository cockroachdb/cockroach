...
func TestSetAbortSpan(t *testing.T) {
	...
	// Used to detect updates to the AbortSpan entry. Timestamp and Priority
	// don't matter other than that they allow us to detect changes in the
	// AbortSpanEntry.
	prevTxn := txn.Clone()
	prevTxn.Timestamp.Add(-1, 0)
	prevTxn.Priority--
	prevTxnAbortSpanEntry := roachpb.AbortSpanEntry{
		Key:       prevTxn.Key,
		Timestamp: prevTxn.Timestamp,
	...
	addIntent := func(b engine.ReadWriter, _ EvalContext) error {
		...
		return engine.MVCCPut(ctx, b, nil /* ms */, intentKey, txn.OrigTimestamp, val, &txn)
	}
