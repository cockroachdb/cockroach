	...
	if args.AllWritesFound {
		...
		case roachpb.ABORTED:
			return result.Result{}, roachpb.NewTransactionStatusError(
				fmt.Sprintf("found ABORTED record for implicitly committed transaction: %v", reply.RecoveredTxn),
			)
		case roachpb.PENDING:
			...
			return result.Result{}, roachpb.NewTransactionStatusError(
				fmt.Sprintf("found PENDING record for implicitly committed transaction: %v", reply.RecoveredTxn),
			)
		case roachpb.STAGING, roachpb.COMMITTED:
			if was, is := args.Txn.Epoch, reply.RecoveredTxn.Epoch; was != is {
				return result.Result{}, roachpb.NewTransactionStatusError(
					fmt.Sprintf("epoch change by implicitly committed transaction: %v->%v", was, is),
				)
			}
			if was, is := args.Txn.Timestamp, reply.RecoveredTxn.Timestamp; was != is {
				return result.Result{}, roachpb.NewTransactionStatusError(
					fmt.Sprintf("timestamp change by implicitly committed transaction: %v->%v", was, is),
				)
			}
		...
		case roachpb.PENDING:
			...
			// a transaction to move back to the PENDING status in the same epoch.
			return result.Result{}, roachpb.NewTransactionStatusError(
				fmt.Sprintf("cannot recover PENDING transaction in same epoch: %s", reply.RecoveredTxn),
			)
		case roachpb.COMMITTED:
			...
			if !legalChange {
				return result.Result{}, roachpb.NewTransactionStatusError(
					fmt.Sprintf("found COMMITTED record for prevented implicit commit: %v", reply.RecoveredTxn),
				)
			}
