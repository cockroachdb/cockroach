...
func evalEndTransaction(
	...
	if reply.Txn.Status == roachpb.COMMITTED {
		...
		if pd, err = RunCommitTrigger(ctx, cArgs.EvalCtx, batch.(engine.Batch), ms, *args, reply.Txn); err != nil {
			return result.Result{}, storagebase.NewReplicaCorruptionError(err)
		}
	}
	...
	for _, span := range args.IntentSpans {
		if err := func() error {
			...
			if len(span.EndKey) == 0 {
				...
				resolveMS := ms
				if preMergeDesc != nil && !storagebase.ContainsKey(*preMergeDesc, span.Key) {
...
func mergeTrigger(
	...
	*ms = enginepb.MVCCStats{}
	...
	if err := MakeStateLoader(rec).SetMVCCStats(ctx, batch, &mergedMS); err != nil {
