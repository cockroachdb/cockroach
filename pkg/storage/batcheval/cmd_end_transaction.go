...
func evalEndTransaction(
	...
	if ok, err := engine.MVCCGetProto(
		...
		case roachpb.STAGING:
			...
			// An EndTransaction request that is moving a STAGING transaction
			// to a COMMITTED transaction doesn't need to repeat its intent
			// spans. Use the spans from the already-staged record.
			if h.Txn.Epoch == reply.Txn.Epoch {
				args.IntentSpans = reply.Txn.Intents
			}
