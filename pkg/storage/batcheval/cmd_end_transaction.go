	...
	for _, span := range args.LockSpans {
		if err := func() error {
			...
			if len(span.EndKey) == 0 {
				...
				resolveMS := ms
				ok, err := engine.MVCCResolveWriteIntentUsingIter(ctx, readWriter, iterAndBuf, resolveMS, update)
				if err != nil {
					return err
				}
				if ok {
			...
			if inSpan != nil {
				update.Span = *inSpan
				num, resumeSpan, err := engine.MVCCResolveWriteIntentRangeUsingIter(ctx, readWriter, iterAndBuf, ms, update, resolveAllowance)
				if err != nil {
					return err
				}
