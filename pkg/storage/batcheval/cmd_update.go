...
func init() {
	RegisterCommand(roachpb.Update, DefaultDeclareKeys, Update)
}

// Update checks the key for more recently written values than the
// txn's original timestamp and less recently than the txn's current
// timestamp.
func Update(
	ctx context.Context, batch engine.ReadWriter, cArgs CommandArgs, resp roachpb.Response,
) (result.Result, error) {
	args := cArgs.Args.(*roachpb.UpdateRequest)
	h := cArgs.Header
	...
	log.VEventf(ctx, 2, "update %s @[%s-%s]", args.Header(), h.Txn.OrigTimestamp, h.Txn.Timestamp)
