...
// - DeltaBatchEstimated: the writes in the batch. Since we have no reason to
//   introduce ContainsEstimates in a split trigger, this typically has
//   ContainsEstimates unset, but this code does not rely on that.
...
//   i.e. the data taken over from the left hand side plus whatever was written
//   to the right hand side in the process (metadata etc). We can recompute this,
//   but try to avoid it unless necessary (when CombinedError below is nonzero).
...
//   left).
// - CombinedError: the difference between (AbsPreSplitBoth+DeltaBatch) and the
//   recomputation of the pre-split range including the batch. This is zero if
//   neither of the inputs contains estimates. If it's not zero, we need to
//   recompute from scratch to obtain AbsPostSplitRight. What's interesting
//   about this quantity is that we never care what exactly it is, but we do
//   care whether it's zero or not because if it's zero we get to do less work.
//
...
// because
//
// (1) AbsPreSplitBoth + DeltaBatch - CombinedError = AbsPostSplitLeft + AbsPostSplitRight
//
...
//
//     CombinedError =   AbsPreSplitBothEstimated + DeltaBatchEstimated
//                     -(AbsPostSplitLeft + AbsPostSplitRight)
//
// and the fact that the second line coontains no estimates, we know that
// CombinedError is zero if the first line contains no estimates. Using this,
// we can rearrange as
//
//     AbsPostSplitRight() = AbsPreSplitBoth + DeltaBatch - AbsPostSplitLeft.
//
// where all quantities on the right are known. If CombinedError is nonzero,
// we effectively have one more unknown in our linear system and we need to
// recompute AbsPostSplitRight from scratch. (As fallout, we can in principle
// compute CombinedError, but we don't care).
...
// SplitStatHelperInput is passed to MakeSplitStatsHelper.
...
func MakeSplitStatsHelper(input SplitStatsHelperInput) (SplitStatsHelper, error) {
	...
	if !h.in.AbsPreSplitBothEstimated.ContainsEstimates &&
		...
		// We have CombinedError zero, so use arithmetic to compute
		// AbsPostSplitRight().
