...
func TestAddSSTableDisallowShadowing(t *testing.T) {
	...
	// SST. The colliding key is not equal to header start key.
	{
		sstKVs := mvccKVsFromStrs([]strKv{
			...
			{"h", 4, "hh"},
			{"s", 1, "ss"},
			{"z", 3, "zz"}, // colliding key has a higher timestamp than existing version.
		})
	...
	// Test that a key collision is not reported when ingesting a key for which we
	// find a tombstone from an MVCC delete.
	{
		sstKVs := mvccKVsFromStrs([]strKv{
