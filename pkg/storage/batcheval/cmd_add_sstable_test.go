...
func createTestRocksDBEngine() engine.Engine {
	return engine.NewInMem(roachpb.Attributes{}, 1<<20)
}
...
func createTestPebbleEngine() engine.Engine {
	...
	opts.FS = vfs.NewMem()
	peb, err := engine.NewPebble(engine.PebbleConfig{
		Opts: opts,
	})
	if err != nil {
		return nil
...
func TestAddSSTableMVCCStats(t *testing.T) {
	...
	for _, engineImpl := range engineImpls {
		t.Run(engineImpl.name, func(t *testing.T) {
			...
			dbFile, err := e.OpenFile("sst")
			if err != nil {
				t.Fatalf("%+v", err)
			}
			if _, err := dbFile.Write(sstBytes); err != nil {
				t.Fatalf("%+v", err)
			}
