...
func init() {
	RegisterCommand(roachpb.UpdateSpan, DefaultDeclareKeys, UpdateSpan)
}

// UpdateSpan scans the key range specified by start key through end
...
func UpdateSpan(
	...
	args := cArgs.Args.(*roachpb.UpdateSpanRequest)
	h := cArgs.Header
	...
	for _, i := range intents {
		...
		log.Errorf(ctx, "when updating span %s @%s, encountered a too-old write intent: %s @%s",
			args.Header(), h.Txn.OrigTimestamp, i.Span.Key, ts)
		return result.Result{}, errors.Errorf("intent written more recently than original span; intent %s @%s",
			i.Span.Key, ts)
	}
