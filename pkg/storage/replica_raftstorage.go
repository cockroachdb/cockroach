...
func (r *Replica) applySnapshot(
	...
	var stats struct {
		// Time to clear unreplicated range-ID local keys and update unreplicated
		// state.
	...
	defer func(start time.Time) {
		...
		log.Infof(ctx, "applied %s snapshot [%s%s%s%sid=%s index=%d]",
			snapType, totalLog, unreplicatedStateLog, subsumedReplicasLog,
			ingestionLog, inSnap.SnapUUID.Short(), snap.Metadata.Index)
	}(timeutil.Now())
...
func (r *Replica) clearSubsumedReplicaDiskData(
	...
	for _, sr := range subsumedRepls {
		...
		if err := ssss.WriteAll(ctx, &subsumedReplSST); err != nil {
			return err
		}
