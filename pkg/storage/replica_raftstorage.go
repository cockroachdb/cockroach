...
func newReplicaRaftStorage(r *Replica) *replicaRaftStorage {
	return &replicaRaftStorage{r}
}
...
func (r *replicaRaftStorage) InitialState() (raftpb.HardState, raftpb.ConfState, error) {
	ctx := r.replica.AnnotateCtx(context.TODO())
	hs, err := r.replica.mu.stateLoader.loadHardState(ctx, r.replica.store.Engine())
	// For uninitialized ranges, membership is unknown at this point.
	if raft.IsEmptyHardState(hs) || err != nil {
		return raftpb.HardState{}, raftpb.ConfState{}, err
	}
	var cs raftpb.ConfState
	for _, rep := range r.replica.mu.state.Desc.Replicas {
...
func (r *replicaRaftStorage) Entries(lo, hi, maxBytes uint64) ([]raftpb.Entry, error) {
	snap := r.replica.store.NewSnapshot()
	defer snap.Close()
	ctx := r.replica.AnnotateCtx(context.TODO())
	return entries(ctx, snap, r.replica.RangeID, r.replica.store.raftEntryCache, lo, hi, maxBytes)
}

// raftEntriesLocked requires that r.mu is held.
func (r *Replica) raftEntriesLocked(lo, hi, maxBytes uint64) ([]raftpb.Entry, error) {
	return r.mu.raftStorage.Entries(lo, hi, maxBytes)
}
...
func (r *replicaRaftStorage) Term(i uint64) (uint64, error) {
	...
	defer snap.Close()
	ctx := r.replica.AnnotateCtx(context.TODO())
	return term(ctx, snap, r.replica.RangeID, r.replica.store.raftEntryCache, i)
}

// raftTermLocked requires that r.mu is held.
func (r *Replica) raftTermLocked(i uint64) (uint64, error) {
	return r.mu.raftStorage.Term(i)
}
...
func (r *replicaRaftStorage) FirstIndex() (uint64, error) {
	ctx := r.replica.AnnotateCtx(context.TODO())
	ts, err := r.replica.raftTruncatedStateLocked(ctx)
	if err != nil {
		return 0, err
	}
...
func (r *Replica) raftFirstIndexLocked() (uint64, error) {
	return r.mu.raftStorage.FirstIndex()
}
...
func (r *replicaRaftStorage) Snapshot() (raftpb.Snapshot, error) {
	r.replica.mu.AssertHeld()
	appliedIndex := r.replica.mu.state.RaftAppliedIndex
...
func (r *Replica) raftSnapshotLocked() (raftpb.Snapshot, error) {
	return r.mu.raftStorage.Snapshot()
}
