...
func (r *Replica) append(
	...
	for i := range entries {
		...
		if ent.Index > prevLastIndex {
			...
			// We type assert batch to also be an engine.Reader only in the case
			// where we're replacing existing entries.
			eng, ok := eng.(engine.ReadWriter)
			if !ok {
				return 0, 0, 0, errors.Errorf("expected eng to be a engine.ReadWriter when overridding log entries")
			}
	...
	if prevLastIndex > 0 {
		// We type assert batch to also be an engine.Reader only in the case where
...
func clearRangeData(
	...
	if destroyData {
		...
		keyRanges = []rditer.KeyRange{rditer.MakeRangeIDLocalKeyRange(desc, true)}
	}
	for _, keyRange := range keyRanges {
		if _, ok := writer.(*engine.RocksDBSstFileWriter); ok {
			// Use ClearRange if the writer is a RocksDBSstFileWriter since it does not
			// support ClearRangeIter.
			return writer.ClearRange(keyRange.Start, keyRange.End)
		}
