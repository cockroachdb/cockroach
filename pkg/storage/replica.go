	...
	if ba.ReadConsistency != roachpb.INCONSISTENT {
		...
		for i := SpanAccess(0); i < numSpanAccess; i++ {
			for j := spanScope(0); j < numSpanScope; j++ {
				...
				// prerequisites.
				for k := 0; k < len(newCmd.prereqs); k++ {
					prereq := newCmd.prereqs[k]
					select {
					case <-prereq.pending:
					...
					case <-ctxDone:
						...
						// prerequisites that have not yet been waited on. Truncate capacity to
						// allow GC of all prerequisite commands that have already completed.
						prs := len(newCmd.prereqs)
						newCmd.prereqs = newCmd.prereqs[k:prs:prs]
