...
func (r *Replica) beginCmds(
	...
	if r.SplitByLoadEnabled() {
		...
		if record {
			if boundaryReadOnlySpan := spans.BoundarySpan(spanset.SpanReadOnly,
				spanset.SpanGlobal); boundaryReadOnlySpan != nil {
				r.splitMu.splitFinder.Record(*boundaryReadOnlySpan, rand.Intn)
			}

			if boundaryReadWriteSpan := spans.BoundarySpan(spanset.SpanReadWrite,
				spanset.SpanGlobal); boundaryReadWriteSpan != nil {
				r.splitMu.splitFinder.Record(*boundaryReadWriteSpan, rand.Intn)
			}
		}
		r.splitMu.Unlock()
...
// SplitByLoadRateThreshold returns the QPS request rate for a given replica.
...
func (r *Replica) needsSplitByLoadLocked() (bool, bool) {
	...
	nowNanos := r.store.Clock().PhysicalTime()
	duration := nowNanos.Sub(r.splitMu.nowNanos)
	...
	if r.splitMu.qps >= r.SplitByLoadQPSThreshold() {
		if r.splitMu.splitFinder != nil {
			if r.splitMu.splitFinder.Ready(nowNanos) {
				// We're ready to add this range to the split queue.
				return true, true
			}
		} else {
			r.splitMu.splitFinder = split.New(nowNanos)
		}
		return true, false
	}
