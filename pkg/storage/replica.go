			...
			case leaseValid, leaseStasis:
				if !status.lease.OwnedBy(r.store.StoreID()) {
					...
					if !stillMember {
						...
						// everything. But lease commands get created on followers based on their
						...
						// makes sure that leases for a replica not in the descriptor are bounced.
...
func (r *Replica) processRaftCommand(
	...
	{
		if raftCmd.ReplicatedEvalResult.Equal(storagebase.ReplicatedEvalResult{}) && forcedErr == nil {
