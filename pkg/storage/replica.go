...
func (r *Replica) insertProposalLocked(pd *ProposalData, originReplica roachpb.ReplicaDescriptor) {
	...
	if pd.ReplicatedProposalData != (storagebase.ReplicatedProposalData{}) {
		isLeaseRequest = pd.ReplicatedProposalData.IsLeaseRequest
	} else {
		isLeaseRequest = pd.Cmd.IsLeaseRequest()
	}
	if !isLeaseRequest {
...
func (r *Replica) propose(
	...
	idKey := makeIDKey()
	var pCmd *ProposalData
	if propEvalKV {
		var pErr *roachpb.Error
		pCmd, pErr = r.evaluateProposal(ctx, idKey, repDesc, ba)
...
func (r *Replica) processRaftCommand(
	...
	if forcedErr != nil {
		...
		if splitMergeUnlock := r.maybeAcquireSplitMergeLock(split, merge); splitMergeUnlock != nil {
	...
	var writeBatch *storagebase.WriteBatch
	{
		...
		if forcedErr != nil {
			...
			raftCmd.ReplicatedProposalData = &storagebase.ReplicatedProposalData{} //Strip()
...
func (r *Replica) maybeAcquireSplitMergeLock(
	split *storagebase.Split, merge *storagebase.Merge,
