...
func (r *Replica) beginCmds(
	...
	if ba.ReadConsistency != roachpb.INCONSISTENT {
		...
		for i := SpanAccess(0); i < numSpanAccess; i++ {
			...
			chans = append(chans, r.cmdQMu.global.getWait(readOnly, reqTS, spans.getSpans(i, spanGlobal))...)
			chans = append(chans, r.cmdQMu.local.getWait(readOnly, hlc.Timestamp{}, spans.getSpans(i, spanLocal))...)
		}
		for i := SpanAccess(0); i < numSpanAccess; i++ {
			readOnly := i == SpanReadOnly
			cmds[i].global = r.cmdQMu.global.add(readOnly, reqTS, spans.getSpans(i, spanGlobal))
			cmds[i].local = r.cmdQMu.local.add(readOnly, hlc.Timestamp{}, spans.getSpans(i, spanLocal))
		}
		r.cmdQMu.Unlock()
