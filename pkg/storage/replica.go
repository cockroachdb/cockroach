...
//
// TODO(ajwerner): Ideally if this store had ever learned that the split
// replica were removed then it would always remember that. In the case
// that the first raft message this store ever receives for the this
// replica is at a higher replica ID than the replica ID for this store in the
// split that creates the range and then crashes. We will not create a tombstone
// when creating an uninitialized replica and so will have no durable record
// of being that replica.
