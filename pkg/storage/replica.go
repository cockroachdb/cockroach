...
func (r *Replica) enforceMinProposal(ctx context.Context, ba *roachpb.BatchRequest) (bool, func()) {
	...
	if ba.Txn != nil {
		if ba.Txn.Timestamp.Less(minProp) {
			...
			case enginepb.SNAPSHOT:
				r.store.metrics.FollowerReadEnforceMaxTxnAgeSnapshot.Inc(1)
			case enginepb.SERIALIZABLE:
				r.store.metrics.FollowerReadEnforceMaxTxnAgeSerializable.Inc(1)
			}
		}
	} else {
