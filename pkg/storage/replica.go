...
func (r *Replica) collectSpans(ba *roachpb.BatchRequest) (*spanset.SpanSet, error) {
	...
	// With clockless reads, everything is treated as a write.
	clockless := r.store.Clock().MaxOffset() == timeutil.ClocklessMaxOffset
	if clockless {
		spans.CollapseAccess(spanset.SpanReadWrite)
	}
