...
func (r *Replica) insertProposalLocked(
	...
	// If the version is and always will be VersionLeaseSequence or greater,
	// we can use the Lease.Sequence field instead of sending the entire Lease
	// through Raft. This decision is based on the minimum supported version and
	// not the active version because Raft entries need to be usable even across
	// allowable version downgrades.
...
func (r *Replica) checkForcedErrLocked(
	...
	if leaseMismatch {
		log.VEventf(
			ctx, 1,
			"command proposed from replica %+v with %d incompatible to %v",
