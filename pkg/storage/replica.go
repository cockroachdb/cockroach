...
type Replica struct {
	...
	mu struct {
		...
		// includes the minimum committed log index at which the replica
		...
		closed struct {
			...
			minIdx        uint64        // min committed log index for closed
		}
	}
...
func (r *Replica) canServiceFollowerRead(
	...
	r.mu.RLock()
	if !r.mu.closed.confTS.Less(readTS) {
		r.mu.RUnlock()
		return true
	}
	closed := r.mu.closed
	...
	if status := r.RaftStatus(); status.Commit >= closed.minIdx {
	...
	if crt := p.command.ReplicatedEvalResult.ChangeReplicas; crt != nil {
		...
		return r.withRaftGroupLocked(true, func(raftGroup *raft.RawNode) (bool, error) {
			if r.unquiesceLocked() {
				r.markRangeUnquiescedLocked(p.ctx, raftGroup.Status())
			}
...
func (r *Replica) markRangeUnquiescedLocked(ctx context.Context, status *raft.Status) {
	...
	for id := range status.Progress {
		if roachpb.ReplicaID(id) != r.mu.replicaID {
			...
			if err != nil {
				...
				r.store.coalescedMu.unquiescedRanges[toStore] = append(r.store.coalescedMu.unquiescedRanges[toStore], r.RangeID)
			}
		}
	}
}
...
func (r *Replica) sendRaftMessage(ctx context.Context, msg raftpb.Message) {
	...
	case raftpb.MsgHeartbeat, raftpb.MsgHeartbeatResp:
		...
		// index as the closed index. Leader-not-leaseholder replicas do
		// not send a closed index, preventing follower reads.
