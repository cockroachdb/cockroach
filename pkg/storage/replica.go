...
// minReplicaID is the minimum ID that this Replica could ever have. If the
// Replica knows its ID then it returns that value, otherwise it returns
// r.mu.minReplicaID which was bootstrapped from the tombstone.
func (r *Replica) minReplicaIDRLocked() roachpb.ReplicaID {
	if r.mu.replicaID != 0 {
		return r.mu.replicaID
	}
