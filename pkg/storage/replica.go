...
func (r *Replica) propose(
	...
	// An error here corresponds to a failfast-proposal: It resulted in an
	// error and did not need to commit a batch (the common case).
	if pErr != nil {
		pCmd.ReplicatedProposalData.Strip()
...
func defaultSubmitProposalLocked(r *Replica, p *ProposalData) error {
	if !propEvalKV && p.Cmd.Timestamp == hlc.ZeroTimestamp {
...
func (r *Replica) processRaftCommand(
	...
	var response proposalResult
	{
		if !propEvalKV && forcedErr == nil {
			...
			// pErr is a global and needs to be passsed up!
			forcedErr = pErr
		}
	...
	if sendToClient {
		// TODO(tschottdorf)
		// fmt.Println("applied ", index, leaseIndex, raftCmd.Cmd, cmd.Err, cmd.intents)
...
//
// TODO(tschottdorf): the setting of WriteTooOld doesn' work. With
...
func (r *Replica) applyRaftCommandInBatch(
	...
	// applies, saving work on the proposer.
	pd.Batch.Close()
	return pd
}
