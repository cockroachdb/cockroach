...
func (r *Replica) addWriteCmd(
	...
	for count := 0; ; count++ {
		...
		case proposalIllegalLeaseIndex:
			continue // retry
		case proposalShouldBeReevaluated:
...
func (r *Replica) refreshProposalsLocked(refreshAtDelta int, reason refreshRaftReason) {
	...
	for idKey, p := range r.mu.proposals {
		if p.command.MaxLeaseIndex <= r.mu.state.LeaseAppliedIndex {
			// The command's designated lease index slot was filled up and we don't
			// know if it was filled by this p.command or some other command.
			// Therefore, it's ambiguous if p.command applied or not. We'll send it
			// back to the proposer for a retry - the request needs to be re-evaluated
			// and the command re-proposed with a new MaxLeaseIndex. The proposer
			// needs to be aware that, if the retry fails, an AmbiguousResultError
			// needs to be returned to the higher layers.
			...
			// the 2nd application will somehow fail (e.g. a command resulting from a
			// RequestLease is not idempotent, but the 2nd application will be
			// rejected by the OriginLease verification).
			//
			...
			p.finish(proposalResult{ProposalRetry: proposalShouldBeReevaluated})
			numShouldRetry++
