...
func (r *Replica) refreshProposalsLocked(refreshAtDelta int, reason refreshRaftReason) {
	if refreshAtDelta != 0 && reason != reasonTicks {
		log.Fatal(context.TODO(), fmt.Sprintf(
			"refreshAtDelta specified for a reason %s != reasonTicks", reason))
	}
	...
	for idKey, p := range r.mu.proposals {
		if p.command.MaxLeaseIndex <= r.mu.state.LeaseAppliedIndex {
			...
			// re-evaluate and re-propose. This situation happens when we've proposed
			// commands that never made it to the leader - usually because we didn't
			// know who the leader is. When we do discover the leader we need to
			// repropose the command. In local testing, by far the most common reason
			// for these reproposals is the initial leader election after a split.
			// Specifically, when a range is split when the next command is proposed
			// to the RHS the leader is elected. After the election completes the
			// command is reproposed for both reasonNewLeader and
			// reasonNewLeaderOrConfigChange.
