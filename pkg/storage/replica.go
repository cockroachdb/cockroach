...
type Replica struct {
	...
	mu struct {
		...
		// mst holds the max safe timestamp information for the range, which
		// includes the minimum committed log index at which the replica
		// contains all data at or before the max safe timestamp.
		mst struct {
			hlc.Timestamp
			safeIdx uint64
		}
...
func (r *Replica) canServiceFollowerRead(
	...
	// First check the read timestamp against the most recent MST that
	// was received for the range.
	if r.mu.mst.Less(readTS) {
		// If we're trying to read at an earlier timestamp, and the range
		// is quiesced, we can use the leader / leaseholder's most recent
		// MST if the lease is a valid epoch lease (status.Liveness != nil
		// means an epoch lease).
		if !r.mu.quiescent || status.State != LeaseState_VALID || status.Liveness == nil || leaseholder == nil {
			return false
		}
		maxSafe, ok := r.store.getMaxSafe(ctx, leaseholder, status.Liveness.Epoch, r.RangeID)
		if !ok || !maxSafe.Less(readTS) {
			return false
		}
	}

	if status := r.RaftStatus(); status.Commit >= r.mu.mst.safeIdx {
		return true
	}
	return false
}
