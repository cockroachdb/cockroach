...
func (r *Replica) updateProposalQuotaRaftMuLocked(
	...
	if r.mu.leaderID != lastLeaderID {
		if r.mu.replicaID == r.mu.leaderID {
			...
			if len(r.mu.quotaReleaseQueue) != 0 {
				log.Fatal(ctx, "quotaReleaseQueue was non-zero length before becoming the leader")
			}
			if len(r.mu.commandSizes) != 0 {
				log.Fatal(ctx, "commandSizes was non-zero length before becoming the leader")
			}
	...
	if r.mu.proposalQuotaBaseIndex < minIndex {
		...
		// therefore may not have all 'delta' command sizes in our quotaReleaseQueue.
