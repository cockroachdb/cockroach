			...
			for _, newCmd := range accessCmds {
				...
				for i := 0; i < len(newCmd.prereqs); i++ {
					...
					case <-prereq.pending:
						...
						// so the append below will be a no-op.
						//
						// If the command was cancelled, we have to handle the cancellation here.
						// We do this by migrating transitive dependencies from cancelled
						// prerequisite to the current command. All prerequisites of the
						...
						// be mutated on the other side of the prereq.pending closing (see below),
						// which the Go Memory Model promises is safe.
						//
