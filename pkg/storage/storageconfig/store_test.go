...
l0_stop_writes_threshold=10`,
	...
	datadriven.RunTest(t, datapathutils.TestDataPath(t, "store_yaml"), func(t *testing.T, td *datadriven.TestData) string {
		...
		case "from-yaml":
			var s Store
			if err := yaml.Unmarshal([]byte(td.Input), &s); err != nil {
				return err.Error()
			}
		...
		case "validate":
			var s Store
			require.NoError(t, yaml.Unmarshal([]byte(td.Input), &s))
...
func testRoundtrip(t *testing.T, input Store) {
	...
	var parsed Store
	require.NoError(t, yaml.Unmarshal(marshaled, &parsed), "marshaled:\n%s", marshaled)
