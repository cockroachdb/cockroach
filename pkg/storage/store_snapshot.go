...
func (s *Store) canApplySnapshotLocked(
	...
	if v, ok := s.mu.replicas.Load(int64(desc.RangeID)); ok && (*Replica)(v).IsInitialized() {
		// We have an initialized replica. Raft snapshots can always be applied with
		// no further checks. Preemptive snapshots can be applied with no further
		// checks if they do not widen the existing replica.
