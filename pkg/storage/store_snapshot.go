...
func assertStrategy(header SnapshotRequest_Header, expect SnapshotRequest_Strategy) {
	if header.Strategy != expect {
		panic(fmt.Sprintf("expected strategy %s, found strategy %s", expect, header.Strategy))
	}
}
...
func (kvSS *kvBatchSnapshotStrategy) Receive(
	...
	assertStrategy(header, SnapshotRequest_KV_BATCH)
