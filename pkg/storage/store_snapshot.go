...
func (kvSS *kvBatchSnapshotStrategy) Receive(
	...
	for {
		...
		if req.Final {
			// We finished receiving all batches. It's possible that
			...
			if inSnap.State.RaftAppliedIndex != inSnap.State.TruncatedState.Index {
				log.Fatalf(ctx, "snapshot raft applied index %d doesn't match its truncated state index %d",
					inSnap.State.RaftAppliedIndex, inSnap.State.TruncatedState.Index)
			}
