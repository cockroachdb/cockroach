...
type kvBatchSnapshotStrategy struct {
	...
	// snapshot.
	batchSize int64
	// The approximate size of the SST chunk to buffer in-memory before flushing
	// to disk.
	sstChunkSize int64
...
// Send implements the snapshotStrategy interface. This function assumes that
// the key-value pairs are sent in sorted order. The key-value pairs are sent
// from the replicated range-id local key range, the range local key range,
// then the user key range.
