...
func (p *Pebble) spanPolicyFunc(bounds pebble.UserKeyBounds) (pebble.SpanPolicy, error) {
	if cockroachkvs.Compare(bounds.Start, spanPolicySystemEndKey) < 0 {
		...
		// For a key >= spanPolicyLockTableEndKey, which is the last configured
		// policy, spanPolicyLocalKeys() returns a policy with key range
		// [spanPolicyLockTableEndKey, ""), so we need to fix this to not extend
		// until infinity.
