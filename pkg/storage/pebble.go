...
func (MVCCBlockIntervalSuffixReplacer) ApplySuffixReplacement(
	...
	if interval.Upper >= synthDecodedWalltime {
		return sstable.BlockInterval{}, errors.AssertionFailedf(
			"the synthetic suffix %d is less than or equal to the original upper bound %d", synthDecoded, interval.Upper)
	}
