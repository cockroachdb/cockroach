...
func NewPebble(ctx context.Context, cfg PebbleConfig) (p *Pebble, err error) {
	...
	if minVerFileExists {
		...
		// TODO(radu): if a previous attempt to create the store failed right before
		// writing out the min version file, we would require manual cleanup.
		cfg.Opts.ErrorIfExists = true
	}
