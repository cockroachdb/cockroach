...
// values. A true setting is also gated on clusterversion.TODO. After all nodes
// in a cluster are at or beyond clusterversion.TODO, different nodes will see
// the version state transition at different times. Nodes that have not yet seen
// the transition may remove the local timestamp from an intent that has one
// during intent resolution. This will not cause problems.
//
