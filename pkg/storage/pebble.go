...
var compressionAlgorithm = settings.RegisterEnumSetting(
	...
	util.ConstantWithMetamorphicTestChoice(
		"storage.sstable.compression_algorithm", "snappy", "zstd").(string),
	map[int64]string{
...
func getCompressionAlgorithm(ctx context.Context, settings *cluster.Settings) pebble.Compression {
	...
	case compressionAlgorithmZstd:
		...
		// in-memory corruption. We require that the cluster version is 24.1 or an
		// upgrade version towards 24.1 which implies that all nodes are running
		// 24.1 code and will never run code < 24.1 again.
		if settings.Version.ActiveVersionOrEmpty(ctx).IsActive(clusterversion.V24_1Start) {
