...
// TODO: consider renaming to storage.Compare, storage.Merger.
//
...
type pebbleReadOnly struct {
	...
	// requests in the batch. These requests will need an
	// intentInterleavingIter, which need both a iterator over the MVCC key
	// space and an iterator over the lock table key space. The latter will
	// use the *StorageIter below.
	prefixIter        pebbleIterator
	normalIter        pebbleIterator
	prefixStorageIter pebbleIterator
	normalStorageIter pebbleIterator
	closed            bool
}
...
func (p *pebbleReadOnly) newIterator(opts IterOptions, iterKind IterKind) Iterator {
	...
	if iterKind == StorageKeyIterKind {
		iter = &p.normalStorageIter
		if opts.Prefix {
			iter = &p.prefixStorageIter
		}
	} else {
		iter = &p.normalIter
		if opts.Prefix {
