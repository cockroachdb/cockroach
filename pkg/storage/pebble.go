...
var (
	...
	valueSeparationMVCCGarbageMinimumSize = settings.RegisterIntSetting(
		settings.SystemVisible,
		"storage.value_separation.mvcc_garbage_minimum_size",
		"the minimum size of a value that will be separated into a blob file given the value is "+
			"likely MVCC garbage",
		int64(metamorphic.ConstantWithTestRange("storage.value_separation.mvcc_garbage_minimum_size",
			32 /* 32 bytes (default) */, 25 /* 25 bytes (minimum) */, 512 /* 512 bytes (maximum) */)),
		settings.IntWithMinimum(1),
	)
)
...
func defaultPebbleOptions(vsp pebble.ValueStoragePolicyAdjustment) *pebble.Options {
	opts := &pebble.Options{
	...
	opts.Experimental.SpanPolicyFunc = spanPolicyFuncFactory(vsp)
...
func DefaultPebbleOptions() *pebble.Options {
	return defaultPebbleOptions(pebble.ValueStorageLatencyTolerant)
}
...
func DefaultPebbleOptionsForOpen(sv *settings.Values) *pebble.Options {
	return defaultPebbleOptions(pebble.ValueStoragePolicyAdjustment{
...
func spanPolicyFuncFactory(
	vsp pebble.ValueStoragePolicyAdjustment,
) func([]byte) (pebble.SpanPolicy, []byte, error) {
