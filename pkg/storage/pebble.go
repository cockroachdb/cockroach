...
func (p *Pebble) ScanInternal(
	ctx context.Context,
	lower, upper roachpb.Key, visitPointKey func(key *pebble.InternalKey, value pebble.LazyValue) error,
...
func (p *Pebble) IngestAndExciseExternalFiles(ctx context.Context, paths []string, shared []pebble.SharedSSTMeta, exciseSpan roachpb.Span) (pebble.IngestOperationStats, error) {
...
func (p *pebbleReadOnly) ScanInternal(ctx context.Context, lower, upper roachpb.Key, visitPointKey func(key *pebble.InternalKey, value pebble.LazyValue) error, visitRangeDel func(start []byte, end []byte, seqNum uint64) error, visitRangeKey func(start []byte, end []byte, keys []rangekey.Key) error, visitSharedFile func(sst *pebble.SharedSSTMeta) error) error {
...
// ScanInternal implements the Reader interface.
func (p *pebbleSnapshot) ScanInternal(ctx context.Context, lower, upper roachpb.Key, visitPointKey func(key *pebble.InternalKey, value pebble.LazyValue) error, visitRangeDel func(start []byte, end []byte, seqNum uint64) error, visitRangeKey func(start []byte, end []byte, keys []rangekey.Key) error, visitSharedFile func(sst *pebble.SharedSSTMeta) error) error {
	rawLower := EngineKey{Key: lower}.Encode()
	rawUpper := EngineKey{Key: upper}.Encode()
