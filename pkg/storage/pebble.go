...
func (p *Pebble) GetTableMetrics(start, end roachpb.Key) ([]enginepb.SSTableMetricsInfo, error) {
	...
	metricsInfo := make([]enginepb.SSTableMetricsInfo, totalTables)
	i := 0
	...
	for level, sstableInfos := range tableInfo {
		for _, sstableInfo := range sstableInfos {
			...
			metricsInfo[i] = enginepb.SSTableMetricsInfo{TableID: uint64(tableID), Level: int32(level), TableInfo: marshalTableInfo}
			i++
		}
	}
