...
// PebbleComparer is a pebble.Comparer object that implements MVCC-specific
...
var PebbleComparer = &pebble.Comparer{
	...
	Split: func(k []byte) int {
		...
		// Pebble and RocksDB require that keys generated via a split be
		// comparable with normal encoded engine keys. Encoded engine keys have a
		// suffix indicating the number of bytes of version data. Engine keys
		// without a version have a suffix of 0. We're careful in EncodeKey to
		// make sure that the user-key always has a trailing 0. If there is no
		// version this falls out naturally. If there is a version we prepend a 0
		// to the encoded version data.
		return len(key) + 1
	},
