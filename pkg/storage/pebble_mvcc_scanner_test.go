...
func TestMVCCScanWithManyVersionsAndSeparatedIntents(t *testing.T) {
	...
	for i := 1; i < 10; i++ {
		for _, k := range keys {
			require.NoError(t, eng.PutMVCC(mvcc.Key{Key: k, Timestamp: hlc.Timestamp{WallTime: int64(i)}},
	...
	kvs := make([]kv, numKeys)
	var i int
	require.NoError(t, MVCCScanDecodeKeyValues(kvData, func(k mvcc.Key, v []byte) error {
		kvs[i].k = k
		kvs[i].v = v
		i++
		return nil
	...
	for i := range expectedKVs {
		expectedKVs[i].k = mvcc.Key{Key: keys[i], Timestamp: hlc.Timestamp{WallTime: 2}}
...
func TestMVCCScanWithLargeKeyValue(t *testing.T) {
	...
	require.NoError(t, eng.PutMVCC(mvcc.Key{Key: keys[0], Timestamp: hlc.Timestamp{WallTime: 1}},
		[]byte("a")))
	require.NoError(t, eng.PutMVCC(mvcc.Key{Key: keys[1], Timestamp: hlc.Timestamp{WallTime: 1}},
		largeValue))
	require.NoError(t, eng.PutMVCC(mvcc.Key{Key: keys[2], Timestamp: hlc.Timestamp{WallTime: 1}},
		[]byte("c")))
	require.NoError(t, eng.PutMVCC(mvcc.Key{Key: keys[3], Timestamp: hlc.Timestamp{WallTime: 1}},
		largeValue))
