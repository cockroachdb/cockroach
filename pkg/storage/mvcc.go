...
// the LockUpdate), and the current value of the intent (meta).
...
// MVCCCheckAcquireLock scans the replicated lock table to determine whether a
// lock acquisition at the specified key and strength by the specified
...
func MVCCAcquireLock(
	...
	for _, iterStr := range equalOrStrongerStrengths(str) {
		...
		if foundLock != nil && !rolledBack {
			...
			// number and clobber the existing locks with its older sequence
			...
			log.VInfof(ctx, 2, "skipping lock acquisition for txn %s on key %s "+
