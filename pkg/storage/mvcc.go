...
// (including one written previously by the same transaction). NB: This is
// evaluated at the transaction's write timestamp rather than it's read
// timestamp, and requires coordination with KV command flags to remain correct.
// It is primarily for use with MVCCDelete, see DeleteRequest.flags() comment
// for details.
...
// rather than the returned boolean where needed.
//
