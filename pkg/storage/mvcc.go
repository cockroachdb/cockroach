...
var TargetBytesPerLockConflictError = settings.RegisterIntSetting(
	settings.ApplicationLevel,
	"storage.mvcc.target_bytes_per_error",
	...
	if !blind {
		...
		if !inlinePut {
			ltScanner, err = newLockTableKeyScanner(
				ctx, rw, opts.Txn, lock.Intent, opts.MaxLockConflicts, opts.TargetBytesPerLockConflict, opts.Category)
			if err != nil {
				return roachpb.LockAcquisition{}, err
			}
...
func MVCCDeleteRange(
	...
	res, err := MVCCScan(ctx, rw, key, endKey, scanTs, MVCCScanOptions{
		...
		TargetBytesPerLockConflict: opts.TargetBytesPerLockConflict,
		ReadCategory:               opts.Category,
	})
	if err != nil {
...
func MVCCPredicateDeleteRange(
	...
	targetBytesPerLockConflict int64,
) (*roachpb.Span, error) {
	...
	if locks, err := ScanLocks(
		ctx, rw, startKey, endKey, maxLockConflicts, targetBytesPerLockConflict, BatchEvalReadCategory); err != nil {
		return nil, err
	} else if len(locks) > 0 {
	...
	if !inlineDelete {
		ltScanner, err = newLockTableKeyScanner(
			ctx, rw, nil /* txn */, lock.Intent, maxLockConflicts, targetBytesPerLockConflict, BatchEvalReadCategory)
		if err != nil {
			return nil, err
		}
		defer ltScanner.close()
	...
	flushDeleteKeys := func() error {
		...
		if runSize >= rangeTombstoneThreshold ||
			...
			if err := MVCCDeleteRangeUsingTombstone(ctx, rw, ms,
				...
				false /* idempotent */, maxLockConflicts, targetBytesPerLockConflict, nil); err != nil {
				return err
			}
...
type MVCCWriteOptions struct {
	...
	MaxLockConflicts int64
	// TargetBytesPerLockConflict is the number of bytes returned in LockConflictError.
...
func MVCCCheckForAcquireLock(
	...
	ltScanner, err := newLockTableKeyScanner(
		ctx, reader, txn, str, maxLockConflicts, targetBytesPerLockConflict, BatchEvalReadCategory)
	if err != nil {
		return err
	}
	defer ltScanner.close()
...
func MVCCAcquireLock(
	...
	ltScanner, err := newLockTableKeyScanner(
		ctx, rw, txn, str, maxLockConflicts, targetBytesPerLockConflict, BatchEvalReadCategory)
	if err != nil {
		return err
	}
	defer ltScanner.close()
...
func mvccExportToWriter(
	...
	if iter.NumCollectedIntents() > 0 {
		...
		err := iter.TryGetIntentError(opts.TargetBytesPerLockConflict)
		return kvpb.BulkOpSummary{}, ExportRequestResumeInfo{}, err
	}
...
type MVCCExportOptions struct {
	...
	// TargetBytesPerLockConflict specifies the size target of locks collected in
	...
	TargetBytesPerLockConflict uint64
