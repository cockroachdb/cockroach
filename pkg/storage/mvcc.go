...
func MVCCResolveWriteIntentRange(
	...
	for {
		...
		if intent.Txn.ID == meta.Txn.ID {
			...
			unsafeKey := sepIter.UnsafeKey()
			keyBuf = append(keyBuf[:0], unsafeKey.Key...)
			key = keyBuf
		} else {
		...
		if err != nil {
			log.Warningf(ctx, "failed to resolve intent for key %q: %+v", key, err)
		} else if ok {
			num++
		}
		if max > 0 && num == max {
			// We could also compute a tighter nextKey here if we wanted to.
			return num, &roachpb.Span{Key: key.Next(), EndKey: intentEndKey}, nil
		}
		sepIter.nextEngineKey()
	}
	return num, nil, nil
