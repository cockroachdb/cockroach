...
// lock.Shared or lock.Exclusive are resolved, a boolean indicating as such is
// also returned.
//
...
func MVCCResolveWriteIntent(
	...
	numBytes int64,
	resumeSpan *roachpb.Span,
	replicatedSharedOrExclusiveLocksResolved bool,
	err error,
) {
	if len(intent.Key) == 0 {
	...
	for valid, err := ltIter.SeekEngineKeyGE(ltSeekKey); ; valid, err = ltIter.NextEngineKey() {
		...
		if str == lock.Intent {
			...
			replicatedSharedOrExclusiveLocksResolved = true
		}
		if err != nil {
			return false, 0, nil, false, err
		}
	...
	return ok, numBytes, nil, replicatedSharedOrExclusiveLocksResolved, nil
}
...
// with strength lock.Shared or lock.Exclusive are resolved, a boolean
...
func MVCCResolveWriteIntentRange(
	...
	for valid, err := ltIter.SeekEngineKeyGE(EngineKey{Key: ltStart}); ; valid, err = ltIter.NextEngineKey() {
		...
		if !sameLockedKey {
			...
			if keysExceeded || bytesExceeded {
				...
				return numKeys, numBytes, resumeSpan, resumeReason, replicatedSharedOrExclusiveLocksResolved, nil
			}
	...
	return numKeys, numBytes, nil, 0, replicatedSharedOrExclusiveLocksResolved, nil
}
