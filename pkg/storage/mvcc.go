...
func computeStatsForIterWithVisitors(
	...
	isConsistentItersForRaceEnabled bool,
	...
	for ; ; iter.Next() {
		...
		// TODO: remove true from predicate before merging.
		if (util.RaceEnabled || true) && isConsistentItersForRaceEnabled && !resumeSoon {
			resumeSoon = true
		}
		...
		if !isValue {
			...
			first = true
			// TODO: remove true from predicate before merging.
			if (util.RaceEnabled || true) && first != firstPointAtKey {
				return enginepb.MVCCStats{}, nil, errors.AssertionFailedf(
...
func computeLockTableStatsWithVisitors(
	...
	iterFunc := func() error {
		...
		for ok, err = iter.SeekEngineKeyGE(resumeKey); ok; ok, err = iter.NextEngineKey() {
			...
			// TODO: remove true from predicate before merging.
			if (util.RaceEnabled || true) && consistentIters && !resumeSoon {
				resumeSoon = true
			}
