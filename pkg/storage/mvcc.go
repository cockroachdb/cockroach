...
// response from the MVCC layer. There is only a Grow method since the
// provider of this monitor knows when the memory is no longer needed, as
// multiple responses are typically coalesced into a batch response.
//
// Currently only used for pebbleMVCCScanner. It is possible that the
...
// ResponseMemoryAccount is safe to use.
