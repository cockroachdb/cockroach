...
type MVCCGetOptions struct {
	...
	// returned.
	MaxKeys int64
	// MaxBytes is a byte threshold to limit the amount of data pulled into
...
func MVCCGetWithValueHeader(
	...
	if opts.MaxKeys < 0 || opts.MaxBytes < 0 {
		// Receipt of a GetRequest with negative MaxKeys or MaxBytes indicates that
		// the request was part of a batch that has already exhausted its limit,
		// which means that we should *not* serve the request and return a
	...
	if err == nil && val != nil {
		...
		if opts.MaxBytes > 0 && opts.AllowEmpty && numBytes > opts.MaxBytes {
			if opts.Reply != nil {
				opts.Reply.ResumeSpan = &roachpb.Span{Key: key}
				opts.Reply.ResumeReason = roachpb.RESUME_BYTE_LIMIT
...
// maxKeys limit or the number of bytes hits the maxBytes limit.
func MVCCPagination(
	ctx context.Context,
	maxKeys, maxBytes int64,
	done func() bool,
	f func(maxKeys, maxBytes int64) (numKeys, numBytes int64, resumeSpan *roachpb.Span, err error),
	...
	for {
		...
		addedKeys, addedBytes, resumeSpan, errF := f(maxKeys, maxBytes)
		if errF != nil {
			err = errF
			return
		}
		...
		if resumeSpan != nil {
			if maxKeys >= 0 && maxBytes >= 0 {
				log.Fatalf(ctx, "non-nil resume span returned, but both key and byte limit have not been hit")
			}
		}
	}
}
...
//
// An opts.maxBytes of -1 means resolve nothing and returns the intent as the
// resume span. If opts.MaxBytes >= 0, then resolve intent and resume span is
// nil.
//
...
//
// An opts.MaxKeys of zero means unbounded. An opts.MaxKeys of -1 means resolve
// nothing and returns the entire intent span as the resume span. An
// opts.maxBytes of 0 means no byte limit. An opts.MaxBytes of -1 means resolve
// nothing and returns the entire intent span as the resume span. If
// opts.MaxBytes > 0, then resolve intents in the range until the number of
// bytes added to the write batch by intent resolution exceeds opts.MaxBytes.
//
...
func MVCCResolveWriteIntentRange(
	...
	for {
		...
		bytesExceeded = opts.MaxBytes > 0 && numBytes >= opts.MaxBytes
		if keysExceeded || bytesExceeded {
			if keysExceeded {
				resumeReason = roachpb.RESUME_KEY_LIMIT
			} else if bytesExceeded {
