...
// newMVCCIterator sets up a suitable iterator for internal MVCC operations:
//
// * Empty timestamp: inline reads, so disables separated intents.
//
// * IterKeyTypePointsOnly: enables MVCC point tombstone synthesis at MVCC
//   range tombstone start keys and around point keys that they overlap.
//
// * Prefix: enables MVCC point tombstone synthesis at the SeekGE key too,
//   even if no existing point key exists there. Typically used for e.g. point
//   gets, to handle conflict/uncertainty checks of MVCC range tombstones at a
//   key that does not have an existing point key.
//
// * rangeKeyMasking: enables range key masking, i.e. hides point keys below an
//   MVCC range tombstone below the given timestamp.
func newMVCCIterator(
	...
	// Synthesize MVCC point tombstones when range keys are disabled.
	pointSynthesis := opts.KeyTypes == IterKeyTypePointsOnly
	if pointSynthesis {
		opts.KeyTypes = IterKeyTypePointsAndRanges
	}
	// Enable range key masking if requested.
	if rangeKeyMasking && opts.RangeKeyMaskingBelow.IsEmpty() {
		opts.RangeKeyMaskingBelow = timestamp
	}
	// Construct basic MVCC iterator.
	iter := reader.NewMVCCIterator(iterKind, opts)
	// Enable MVCC point tombstone synthesis. If Prefix is enabled, also
	// synthesize points around a SeekGE key overlapping a range tombstone.
	if pointSynthesis {
		iter = newPointSynthesizingIter(iter, opts.Prefix)
	}
	return iter
}
...
// real point key was found. This may differ from the metadata timestamp when
// the key is covered by multiple MVCC range tombstones, or when a point
// tombstone is covered by a range tombstone. It is needed to correctly account
// for the GCBytesAge contribution of the key prefix.
//
...
func mvccGetMetadata(
	...
	// Synthesize metadata for a point key.
	meta.Reset()
	...
	if hasRange && !hasPoint {
		rkTimestamp := iter.RangeKeys()[0].Timestamp
	...
	// and synthesize point tombstone metadata for it in that case. realKeyChanged
	// is set to the timestamp where the point key ceased to exist -- either the
	...
	if hasRange {
		...
		if rk, ok := firstRangeKeyAbove(rangeKeys, unsafeKey.Timestamp); ok {
			meta.Deleted = true
			meta.Timestamp = rangeKeys[0].Timestamp.ToLegacyTimestamp()
			keyLastSeen := rk.Timestamp
...
// MVCC range tombstones in MVCC stats. It is not sufficient to pass a
// pointSynthesizingIter, as constructed by newMVCCIterator by default.
//
// Note that, when writing transactionally, the txn's timestamps
...
func mvccPutInternal(
	...
	// Fetch or construct synthetic MVCC metadata. This must use an iterator which
	// surfaces range keys, to correctly account for MVCC range tombstones in stats.
	...
	//
	// MVCC range tombstones cannot exist across inline values, so we don't need
	// any special handling for them here.
	if putIsInline {
		if txn != nil {
...
func ExperimentalMVCCDeleteRangeUsingTombstone(
	...
	for {
		...
		if hasPoint {
			...
			if ms != nil && len(value) > 0 {
				ms.LiveCount--
				ms.LiveBytes -= int64(key.EncodedSize()) + int64(len(value))
			}
		}

		if hasRange {
			// Check if we've encountered a new range key stack.
			if rangeStart, rangeEnd := iter.RangeBounds(); !rangeEnd.Equal(prevRangeEnd) {
				latest := iter.RangeKeys()[0]

				// Check for conflict with newer range key.
				if timestamp.LessEq(latest.Timestamp) {
					return roachpb.NewWriteTooOldError(
						timestamp, latest.Timestamp.Next(), latest.StartKey.Clone())
				}
				...
				if ms != nil {
					...
					if !rangeStart.Equal(prevRangeEnd) {
						ms.RangeKeyCount++
						ms.RangeKeyBytes += int64(EncodedMVCCTimestampSuffixLength(timestamp) +
							EncodedMVCCKeyPrefixLength(prevRangeEnd) + EncodedMVCCKeyPrefixLength(rangeStart))
					}
					...
					// bounds up from the latest existing range key to this one.
					ms.RangeKeyBytes += int64(EncodedMVCCTimestampSuffixLength(timestamp))
					ms.GCBytesAge -= (timestamp.WallTime/1e9 - latest.Timestamp.WallTime/1e9) *
						int64(EncodedMVCCKeyPrefixLength(rangeStart)+EncodedMVCCKeyPrefixLength(rangeEnd))
				}

				prevRangeEnd = append(prevRangeEnd[:0], rangeEnd...)
			}
		}
	...
	if ms != nil {
		...
		fragmentRangeKeys := func(rangeKeys []MVCCRangeKey, splitKey roachpb.Key) {
			for i, rk := range rangeKeys {
				keyBytes := int64(EncodedMVCCTimestampSuffixLength(rk.Timestamp))
				if i == 0 {
					ms.RangeKeyCount++
					keyBytes += 2 * int64(EncodedMVCCKeyPrefixLength(splitKey))
				}
				ms.RangeKeyBytes += keyBytes
				ms.GCBytesAge += keyBytes * (timestamp.WallTime/1e9 - rk.Timestamp.WallTime/1e9)
			}
		}

		// maybeMergeRangeKeys adjusts ms to merge two abutting range key
		// stacks if they have the same timestamps. It assumes the lhs end key
		// equals the rhs start key, and that they are in descending order.
		maybeMergeRangeKeys := func(lhs, rhs []MVCCRangeKey) {
			if len(lhs) != len(rhs) || len(lhs) == 0 {
				return
			}
			for i, l := range lhs {
				if !l.Timestamp.Equal(rhs[i].Timestamp) {
					return
				}
			}
			mergeKey := rhs[0].StartKey
			for i, rk := range lhs {
				keyBytes := int64(EncodedMVCCTimestampSuffixLength(rk.Timestamp))
				if i == 0 {
					ms.RangeKeyCount--
		...
		if !leftPeekBound.Equal(startKey) {
			...
			if ok, err := iter.Valid(); err != nil {
				...
				switch rangeEnd.Compare(startKey) {
				case 1: // fragment
					fragmentRangeKeys(iter.RangeKeys(), startKey)
				case 0: // merge
					lhs := iter.RangeKeys()
					rhs := []MVCCRangeKey{rangeKey}
					iter.SeekGE(MVCCKey{Key: startKey})
					if ok, err := iter.Valid(); err != nil {
						return err
					} else if ok {
		...
		if !rightPeekBound.Equal(endKey) {
			...
			if ok, err := iter.Valid(); err != nil {
				...
				switch rangeStart.Compare(endKey) {
				case -1: // fragment
					fragmentRangeKeys(iter.RangeKeys(), endKey)
				case 0: // merge
					lhs := []MVCCRangeKey{rangeKey}
					rhs := iter.RangeKeys()
...
func (s *separatedIntentAndVersionIter) RangeKeys() []MVCCRangeKey {
			...
			if valid, err := iter.Valid(); err != nil {
				...
				if hasPoint, hasRange := iter.HasPointAndRange(); hasPoint {
					if unsafeKey := iter.UnsafeKey(); unsafeKey.Key.Equal(oldKey.Key) {
						if !hasRange || iter.RangeKeys()[0].Timestamp.Less(unsafeKey.Timestamp) {
...
func MVCCResolveWriteIntentRange(
	...
	if rw.ConsistentIterators() {
		...
		iterAndBuf := GetBufUsingIter(rw.NewMVCCIterator(MVCCKeyIterKind, IterOptions{
			KeyTypes:   IterKeyTypePointsAndRanges,
		...
		// TODO(erikgrinaker): We don't handle MVCC range keys here, because this
		// code path will shortly be removed. See:
...
func ComputeStatsForRangeWithVisitors(
	...
	rangeKeyVisitor func(MVCCRangeKey) error,
) (enginepb.MVCCStats, error) {
	var ms enginepb.MVCCStats
	...
	for iter.SeekGE(MakeMVCCMetadataKey(start)); ; iter.Next() {
		...
		if hasRange {
			if rangeStart, _ := iter.RangeBounds(); !bytes.Equal(rangeStart, prevRangeStart) {
				...
				for i, rk := range rangeKeys {
					...
					if i == 0 {
						ms.RangeKeyCount++
						keyBytes += int64(EncodedMVCCKeyPrefixLength(rk.StartKey) +
							EncodedMVCCKeyPrefixLength(rk.EndKey))
					}
					keyBytes += int64(EncodedMVCCTimestampSuffixLength(rk.Timestamp))
					ms.RangeKeyBytes += keyBytes
					ms.GCBytesAge += keyBytes * (nowNanos/1e9 - rk.Timestamp.WallTime/1e9)

					if rangeKeyVisitor != nil {
						if err := rangeKeyVisitor(rk); err != nil {
		...
		if isValue {
			if rk, ok := firstRangeKeyAbove(rangeKeys, unsafeKey.Timestamp); ok {
				nextRangeTombstone = rk.Timestamp
			}
		}
