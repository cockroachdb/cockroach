...
func mvccPutInternal(
	...
	if !putIsBlind {
		...
		meta = &buf.meta
		if ok && !meta.IsInline() {
			// If at least one version is found, scan the lock table for
			// conflicting locks and/or an intent on the key from different
			// transactions. If any such conflicts are found, the lock table
