...
func MVCCExportFingerprint(
...
) (roachpb.BulkOpSummary, ExportRequestResumeInfo, uint64, bool, error) {
	...
	if exportErr != nil {
		return roachpb.BulkOpSummary{}, ExportRequestResumeInfo{}, 0, false, exportErr
	}

	fingerprint, err := fingerprintWriter.Finish()
	if err != nil {
		return roachpb.BulkOpSummary{}, ExportRequestResumeInfo{}, 0, false, err
	}
...
func MVCCExportToSST(
	ctx context.Context, cs *cluster.Settings, reader Reader, opts MVCCExportOptions, dest io.Writer,
) (roachpb.BulkOpSummary, ExportRequestResumeInfo, error) {
	...
	if exportErr != nil {
		return roachpb.BulkOpSummary{}, ExportRequestResumeInfo{}, exportErr
	}
	...
	if err := sstWriter.Finish(); err != nil {
		return roachpb.BulkOpSummary{}, ExportRequestResumeInfo{}, err
	}
	...
	for {
		...
		if iter.RangeKeyChanged() {
			...
			for _, v := range rangeKeys.Versions {
				...
				if err != nil {
					return roachpb.BulkOpSummary{}, ExportRequestResumeInfo{}, errors.Wrapf(err,
			...
			if hasRange && !skipTombstones {
				...
				if reachedMaxSize {
					return roachpb.BulkOpSummary{}, ExportRequestResumeInfo{}, &ExceedMaxSizeError{
		...
		if unsafeKey.IsValue() {
			...
			if err != nil {
				return roachpb.BulkOpSummary{}, ExportRequestResumeInfo{}, errors.Wrapf(err, "decoding mvcc value %s", unsafeKey)
			}
		...
		if !skip {
			if err := rows.Count(unsafeKey.Key); err != nil {
				return roachpb.BulkOpSummary{}, ExportRequestResumeInfo{}, errors.Wrapf(err, "decoding %s", unsafeKey)
			}
			curSize := rows.BulkOpSummary.DataSize
			...
			if curSize == 0 && opts.MaxSize > 0 && kvSize > int64(opts.MaxSize) {
				...
				return roachpb.BulkOpSummary{}, ExportRequestResumeInfo{}, &ExceedMaxSizeError{reached: kvSize, maxSize: opts.MaxSize}
			}
			...
			if unsafeKey.Timestamp.IsEmpty() {
				...
				if err := writer.PutUnversioned(unsafeKey.Key, unsafeValue); err != nil {
					return roachpb.BulkOpSummary{}, ExportRequestResumeInfo{}, errors.Wrapf(err, "adding key %s", unsafeKey)
				}
			} else {
