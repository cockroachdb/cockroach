...
func MVCCAcquireLock(
	...
	maxLockConflicts int64,
	targetLockConflictBytes int64,
	expectPossibleSequenceNumberRegression bool,
) error {
	if txn == nil {
	...
	for _, iterStr := range strongerOrEqualStrengths(str) {
		...
		if foundLock.Txn.Epoch > txn.Epoch {
			...
			//    a response.
			//
			// IFF the caller sets expectPossibleSequenceNumberRegression, we don't
			// consider this an error. The only callers who set this are callers who
			// are flushing locks that were originally taken as unreplicated locks as
			...
			if !expectPossibleSequenceNumberRegression {
				return errors.Errorf(
