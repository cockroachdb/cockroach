...
// ComputeStatsForIter is like ComputeStats, but instead seeks to the lower
// bound of the given iterator and scans across it until exhausted. The iterator
// must have appropriate bounds, key types, and intent options set. We don't
// take start/end here, because that would require expensive key comparisons.
//
...
func ComputeStatsForIter(iter SimpleMVCCIterator, nowNanos int64) (enginepb.MVCCStats, error) {
	// NB: Seeking to MinKey in effect seeks to the iterator's lower bound, but
	// spanset assertions will trigger on this. This function is therefore only
	// suitable for SST iterators, not engine iterators.
	iter.SeekGE(MVCCKey{Key: keys.MinKey})
