...
// updateStatsOnRangeKeyCoverStats updates MVCCStats for when an MVCC range key
// covers existing data whose stats are already known.
...
func MVCCDeleteRangeUsingTombstone(
	...
	// If we have MVCC stats for the covered data, we need to do a time-bound scan
	...
	// it to find conflicts and update MVCC stats within it. If the caller has
	// provided MVCC stats for the covered data, we only scan for range keys.
