...
// bandwidth is itself a function of the tokens and upper-bound-cc(l) and
// MaxConcurrentCompactions. To tease apart this interaction, we note that
...
// upper-bound-cc(l) is always infinity since Pebble can increase up to
...
// simplification), which we can write as ac-tokens(l,
// permitted-concurrent-compactions). Furthermore, we consider a value
// upper-bound-ac-tokens, which is a function of only l and upper-bound-cc,
// and pretends that MaxConcurrentCompactions is infinity. We translate our
// original objective into the following simplified objective:
//
// upper-bound-ac-tokens should be greater than upper-bound-cc as l increases,
// and it should be equal to or greater than upper-bound-cc when
// upper-bound-cc becomes equal to MaxConcurrentCompactions. The following
// diagram shows an example that achieves this objective:
//
//    ^
//    |-------
//    |       --      +              - upper-bound-ac-tokens
//    |         --  +                + upper-bound-cc
// cc |*********+*--***********      * MaxConcurrentCompactions
//    |      +
//    |   +
...
//	         l
//
// Note that the objective does not say anything about upper-bound-ac-tokens
// after upper-bound-cc reaches MaxConcurrentCompactions for 2 reasons (a)
// what happens at higher l values did not prevent us from achieving the
// maximum compaction concurrency, (b) the definition of upper-bound-ac-tokens
// does not take into account MaxConcurrentCompactions, so its value is
// meaningless once upper-bound-cc increases beyond MaxConcurrentCompactions.
//
// upper-bound-ac-tokens for regular traffic with lslcc=2:
//
...
// stable point of 10 sub-levels under regular traffic overload. At sub-level
// count 5, the upper-bound-cc is floor(1+5/2)=3, so upper-bound-ac-tokens are
// representative of a concurrency of 3*2=6. At sub-level count of 10, the
// upper-bound-cc is floor(1+10/2)=6, so tokens are also representative of a
// compaction concurrency of 6*1=6. This regular traffic token shaping
// behavior is hard-wired in ioLoadListener (with code constants), and we
// don't currently have a reason to change it. If MaxConcurrentCompactions is
// <= 6, the objective stated earlier is achieved. But if
// MaxConcurrentCompactions > 6, AC will throttle to compaction concurrency of
// 6, which fails the objective since we have ignored the ability to increase
// compaction concurrency. Note that this analysis is somewhat pessimistic
// since if we are consistently operating at 5 or more sub-levels, other
// levels in the LSM are also building up compaction debt, and there is
// another mechanism in Pebble that increases compaction concurrency in
// response to compaction debt. Nevertheless, this pessimistic analysis shows
// that we are ok with MaxConcurrentCompactions <= 6. We will consider the
// possibility of reducing lslcc below.
//
// upper-bound-ac-tokens for elastic traffic with lslcc=2:
//
...
// upper-bound-ac-tokens is 1.25*floor(1+2/2)=2.5 compaction concurrency. And
// at sub-level count of 4, the tokens are equal to 1x the compaction
// bandwidth, so upper-bound-ac-tokens is 1*floor(1+4/2)=3 compaction
// concurrency). AC wants to operate at a stable point of 4 sub-levels under
// elastic traffic overload. Even though 2.5 < 3, we deem the value of
// upper-bound-ac-tokens as acceptable with the default value of
...
// MaxConcurrentCompactions. In those cases elastic traffic will be throttled
...
// upper-bound-ac-tokens will be:
//
//   - Elastic: sub-level=2, 1.25*3=3.75; sub-level=4, 1*5=5; sub-level=6,
//     0.75*7=5.25. So elastic traffic will be able to utilize a compaction
//     concurrency of approximately 4 to begin with. Also, compaction debt will
//     increase and allow for utilizing even higher concurrency, eventually, and
//     since this is elastic traffic that eventual behavior is deemed
//     acceptable.
//
//   - Regular: sub-level=5, 2*6=12; sub-level=10, 1*11=11. Regular traffic will
//     be able to utilize a compaction concurrency of approximately 11.
	...
	if ok {
		...
		if meta.Txn != nil {
			...
			if opts.Txn == nil || meta.Txn.ID != opts.Txn.ID {
				...
				// lockTableKeyScanner.
				return false, errors.AssertionFailedf("conflicting intent %s,"+
