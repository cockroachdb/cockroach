...
func mvccGetMetadata(
	...
	meta.SetLocalTimestamp(unsafeVal.LocalTimestamp, localTs)
...
func mvccPutInternal(
	...
	newMeta.SetLocalTimestamp(versionValue.LocalTimestamp, &buf.newLocalTs)
	...
	for {
		...
		if len(clearedMetaKey.Key) > 0 {
			...
			if bytes.Equal(clearedMetaKey.Key, k.Key) {
				...
				restoredMeta.SetLocalTimestamp(v.LocalTimestamp, &restoredLocalTs)
		...
		if startTime.Less(k.Timestamp) && k.Timestamp.LessEq(endTime) {
			...
			clearedMeta.SetLocalTimestamp(v.LocalTimestamp, &clearedLocalTs)
...
func mvccResolveWriteIntent(
	...
	if commit || pushed || rolledBackVal != nil {
		...
		if timestampChanged {
			...
			newValue, err := DecodeMVCCValue(iter.UnsafeValue())
			if err != nil {
				return false, err
			}
			...
			newMeta.SetLocalTimestamp(newValue.LocalTimestamp, &buf.newLocalTs)
			newMeta.KeyBytes = MVCCVersionTimestampSize
			...
			if valid, err := iter.Valid(); err != nil {
				...
				prevVal, err := DecodeMVCCValue(iter.UnsafeValue())
				if err != nil {
					return false, err
				}
				prevIsValue = prevVal.Value.IsPresent()
...
func ComputeStatsForRange(
	...
	for ; ; iter.Next() {
		...
		if implicitMeta {
			...
			meta.Deleted = mvccValue.IsTombstone()
