...
// (including one written previously by the same transaction).
// TODO(erikgrinaker): This return value exists solely because valueFn incurs an
// additional seek via maybeGetValue(). However, in most cases we have already
// read the value via other means, e.g. mvccGetMetadata(). We should restructure
// the code such that valueFn omits unnecessary reads in the common case and
// then use it rather than the returned boolean where needed.
//
