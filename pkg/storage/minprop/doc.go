...
//
// A Tracker consists of two timestamps last and cur with associated ref
// counts, as well as locking which we'll ignore in the description. The goal
// of the Tracker is to regularly move cur and last forward in time, and to
// close out the last timestamp (with some optimizations to close out cur when
// it isn't referenced).
//
// last is the safe value below which no new proposals can exist, and cur is
