...
type Tracker struct {
	mu struct {
		...
		// the process of evaluating. `next` can only be closed out once `leftRef`
		// has dropped to zero; new proposals are always forced above `next` and
		// consequently count towards `rightRef`.
