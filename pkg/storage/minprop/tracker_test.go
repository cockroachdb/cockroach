...
func TestTrackerDoubleRelease(t *testing.T) {
	var exited bool
	log.SetExitFunc(func(int) { exited = true })
...
func TestTrackerConcurrentUse(t *testing.T) {
	...
	close := func(newNext hlc.Timestamp) error {
		...
		if closed != prevClosed {
			...
			// Rotate released commands bucket, so that any command in this
			// bucket is known to have finished before the following call to
			// Close. This means
