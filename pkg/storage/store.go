...
type Store struct {
	...
	raftRepairQueue    *raftRepairQueue            // Raft repair queue
...
func NewStore(cfg StoreConfig, eng engine.Engine, nodeDesc *roachpb.NodeDescriptor) *Store {
	...
	if s.cfg.Gossip != nil {
		...
		s.raftRepairQueue = newRaftRepairQueue(s, s.cfg.Gossip, s.cfg.Clock)
		s.consistencyQueue = newConsistencyQueue(s, s.cfg.Gossip)
		s.scanner.AddQueues(
			s.gcQueue, s.splitQueue, s.replicateQueue, s.replicaGCQueue,
			s.raftLogQueue, s.raftRepairQueue, s.consistencyQueue)
