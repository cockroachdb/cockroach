...
// carrying out any changes, collecting all trace messages along the way.
// Intended to help power a debug endpoint.
func (s *Store) AllocatorDryRun(repl *Replica) (string, error) {
	sysCfg, ok := s.cfg.Gossip.GetSystemConfig()
	if !ok {
		return "", errors.New("allocator dry runs require a valid system config")
	}
	ctx, collect := tracing.MakeRecordCtx()
	canTransferLease := func() bool { return true }
	dryRun := true
	_, err := s.replicateQueue.processOneChange(ctx, repl, sysCfg, canTransferLease, dryRun)
	if err != nil {
