...
func (s *Store) tryGetOrCreateReplica(
	...
	if value, ok := s.mu.replicas.Load(int64(rangeID)); ok {
		...
		removePreemetiveSnapshot := isPreemptiveSnapshot(repl) && isLearner
		// The current replica needs to be removed, remove it and go back around.
		if toTooOld || removePreemetiveSnapshot {
...
// does not know its replicaID.
func isPreemptiveSnapshot(repl *Replica) bool {
	repl.mu.AssertHeld()
	return repl.mu.replicaID == 0 && repl.isInitializedRLocked()
}
