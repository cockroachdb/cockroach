...
func (s *Store) recordNewPerSecondStats(newQPS, newWPS float64) {
	...
	// TODO(a-robinson): Use these values to avoid having to recalculate them in
	// GossipStore.
	if newQPS < oldQPS*.5 || newQPS > oldQPS*1.5 {
		s.asyncGossipStore(context.TODO(), "queries-per-second change", false /* useCached */)
	} else if newWPS < oldWPS*.5 || newWPS > oldWPS*1.5 {
