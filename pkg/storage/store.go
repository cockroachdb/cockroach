...
func splitPostApply(
	...
	}
	{
		...
		if rightRng.minReplicaIDRLocked() > rightDesc.ReplicaID {
			rightRng.mu.Unlock()
...
func (s *Store) SplitRange(
	...
	if oldRightDesc == nil {
		...
		// uninitialized RHS.
		hs, err := rightRepl.raftMu.stateLoader.LoadHardState(ctx, rightRepl.Engine())
		if err != nil {
		...
		defer batch.Close()
		if err := clearRangeData(oldRightDesc, rightRepl.Engine(), batch, clearAll, false); err != nil {
			return errors.Wrap(err, "failed to clear range data for removed rhs")
		}
		if err := rightRepl.raftMu.stateLoader.SetHardState(ctx, batch, hs); err != nil {
