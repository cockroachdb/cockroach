...
func splitPreApply(
	...
	// If we're not in the range then the below logic is fine.
	...
	// lest it be leaked potentially forever. Furthermore, the split trigger
	// writes out raft state to the RHS which does not apply to the
	// uninitialized RHS.
...
func splitPostApply(
	...
	}
	{
		...
		if rightRng.isNewerThanSplitRLocked(split) {
			rightRng.mu.Unlock()
			r.setDescAfterSplit(ctx, &split.LeftDesc)
			r.writeStats.halve()
			return
		}
...
func (s *Store) SplitRange(
	...
	leftRepl.setDescAfterSplit(ctx, &newLeftDesc)
