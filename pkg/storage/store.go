...
func splitPostApply(
	...
	}
	{
		...
		if !ok {
			...
			if r.mu.replicaID != 0 {
				log.Fatalf(ctx, "we can't have had the local store change replica ID")
			}
		}
...
func (s *Store) removeUninitializedReplicaRaftMuLocked(
	...
	// TODO(ajwerner): consider some more sanity checks.
	...
	rep.destroyUninitializedReplicaRaftMuLocked(ctx, nextReplicaID)
	...
	rep.mu.Lock()
	defer rep.mu.Unlock()
	if !rep.mu.destroyStatus.RemovalPending() {
		log.Fatalf(ctx, "cannot remove uninitialized replica which is not removal pending: %v", rep.mu.destroyStatus)
	}
...
func (s *Store) processRaftSnapshotRequest(
	...
	return s.withReplicaForRequest(ctx, &snapHeader.RaftMessageRequest, func(
		...
		if ds, err := r.IsDestroyed(); ds == destroyReasonRemoved || ds == destroyReasonRemovalPending {
			return roachpb.NewError(err)
		}
	...
	return s.stopper.RunTaskWithErr(ctx, name, func(ctx context.Context) error {
		...
		case *roachpb.Error:
			...
			case *roachpb.ReplicaTooOldError:
				...
				return s.removeReplicaImpl(ctx, repl, tErr.ReplicaID+1, RemoveOptions{
					DestroyData: true,
				})
