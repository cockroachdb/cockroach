...
func (s *Store) tryGetOrCreateReplica(
	...
	if value, ok := s.mu.replicas.Load(int64(rangeID)); ok {
		...
		if err := s.tryGetOrCreateHandleFromReplicaTooOld(ctx, repl, creatingReplica); err != nil {
			repl.raftMu.Unlock()
			return nil, false, err
		}
		...
		if err := s.tryGetOrCreateHandleToReplicaTooOld(ctx, repl, replicaID); err != nil {
			repl.raftMu.Unlock()
			return nil, false, err
		}
		if err := tryGetOrCreateHandleNonVoterWithPreemptiveSnapshot(ctx, s, repl, replicaID, isLearner); err != nil {
			repl.raftMu.Unlock()
			return nil, false, err
		}
...
func tryGetOrCreateHandleNonVoterWithPreemptiveSnapshot(
	ctx context.Context, s *Store, repl *Replica, replicaID roachpb.ReplicaID, isLearner bool,
) error {
	repl.raftMu.AssertHeld()
	repl.mu.AssertHeld()
