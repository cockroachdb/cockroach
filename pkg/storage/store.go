...
func (s *Store) withReplicaForRequest(
	...
	if req.ToReplica.ReplicaID == 0 {
		log.Fatalf(ctx, "unexpected request address to replicaID zero: %+v", req)
	}
...
func (s *Store) processRaftSnapshotRequestWithReplicaRaftMuLocked(
	...
	if snapHeader.IsPreemptive() {
		log.Fatal(ctx, "this code should be dead")
...
func (s *Store) getOrCreateReplica(
	...
	if false && replicaID == 0 {
		...
		// cancelled), or even just return an error, whichever one works better.
