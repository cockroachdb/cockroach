...
type Store struct {
	...
	coalescedMu struct {
		syncutil.Mutex
		// Heartbeat requests generated by Raft groups since the last coalesced heartbeat
		// was sent. To be sent with the next coalesced heartbeat.
		heartbeats map[roachpb.StoreIdent][]RaftHeartbeat
		// Heartbeat responses generated by Raft groups since the last coalesced heartbeat
		// was sent. To be sent with the next coalesced heartbeat.
...
type StoreConfig struct {
	...
	// ClosedTimestampInterval is the target duration for setting the
	// min proposal timestamp for a store. Setting this higher or lower
	// either loosens or tightens bounds for the closed timestamps
	// propagated to followers for follower reads.
...
func (sc *StoreConfig) SetDefaults() {
	...
	if sc.ClosedTimestampInterval == 0 {
		sc.ClosedTimestampInterval = defaultClosedTimestampInterval
	}
}
...
func (sc *StoreConfig) FollowerReadInterval() time.Duration {
	...
	return time.Duration(sc.ClosedTimestampInterval.Nanoseconds() +
...
func NewStore(cfg StoreConfig, eng engine.Engine, nodeDesc *roachpb.NodeDescriptor) *Store {
	...
	s.coalescedMu.infosByRange = map[roachpb.RangeID]*ClosedTSInfo{}
	s.coalescedMu.closedTSInfos = map[roachpb.StoreIdent]map[roachpb.RangeID]*ClosedTSInfo{}
	s.coalescedMu.closedSequences = map[roachpb.StoreIdent]int64{}
	s.coalescedMu.Unlock()

	s.closedMu.Lock()
	s.closedMu.leaseholders = map[roachpb.StoreIdent]*storeClosedTS{}
	s.closedMu.Unlock()
...
func (s *Store) Start(ctx context.Context, stopper *stop.Stopper) error {
	...
	// Initialize the minimum proposal timestamp.
	s.minPropMu.Lock()
	s.minPropMu.cur = now.Add(-s.cfg.ClosedTimestampInterval.Nanoseconds(), 0)
	s.minPropMu.Unlock()
...
func splitPostApply(
	...
	// Copy the closed timestamp info from the LHS. Since the Raft log
	// is guaranteed to be empty here, set minLeaseAppliedIndex to 0.
	rightRng.mu.closed.ts = r.mu.closed.ts
	rightRng.mu.closed.confTS = r.mu.closed.confTS
	// TODO(spencer): need to check this.
	rightRng.mu.closed.minLeaseAppliedIndex = 0
...
func (s *Store) updateClosedTS(
	...
	if log.V(4) {
		log.Infof(ctx, "%s: updating closed timestamp %s, epoch %d from %s for ranges (%+v)",
			s, closed, epoch, fromReplica, ranges)
	}
	...
	storeCTS, ok := s.closedMu.leaseholders[storeKey]
	if ok {
		// If the sequence has not advanced by one, or the epoch has
		// changed, invalidate our quiesced map.
		if closedSequence != storeCTS.sequence+1 || epoch != storeCTS.epoch || epoch == 0 {
			storeCTS.quiesced = map[roachpb.RangeID]struct{}{}
		}
		storeCTS.sequence = closedSequence
		storeCTS.epoch = epoch
	} else {
		storeCTS = &storeClosedTS{
			sequence: closedSequence,
			epoch:    epoch,
			quiesced: map[roachpb.RangeID]struct{}{},
		}
		s.closedMu.leaseholders[storeKey] = storeCTS
	}
	storeCTS.Timestamp = closed
	// Process ranges to set minimum lease applied indexes and
	// add/remove quiesced ranges to the quiesced set as appropriate.
	for _, info := range ranges {
		if r, err := s.GetReplica(info.RangeID); err == nil {
			r.mu.Lock()
			canSet := true
			if lai := r.mu.state.LeaseAppliedIndex; lai >= info.MinLeaseAppliedIndex {
				r.mu.closed.confTS.Forward(closed)
			} else if lai >= r.mu.closed.minLeaseAppliedIndex {
				r.mu.closed.confTS.Forward(r.mu.closed.ts)
			} else {
				// If the replica has not caught up to the lease applied index
				// necessary to use the last closed timestamp, don't set a new one.
				canSet = false
			}
			if canSet {
				r.mu.closed.ts = closed
				r.mu.closed.minLeaseAppliedIndex = info.MinLeaseAppliedIndex
			}
			r.mu.Unlock()
		}
		if info.Quiescing {
			...
			storeCTS.quiesced[info.RangeID] = struct{}{}
		} else {
			delete(storeCTS.quiesced, info.RangeID)
		}
	}
}
...
func (s *Store) HandleRaftRequest(
	...
	// TODO(tschottdorf): I think the two branches can be merged.
	if len(req.Heartbeats)+len(req.HeartbeatResps)+len(req.Ranges) > 0 {
		if req.RangeID != 0 {
		...
		s.updateClosedTS(ctx, req.ClosedSequence, req.Closed, req.Epoch, req.Ranges, req.FromReplica)
		return nil
	} else if req.Closed != (hlc.Timestamp{}) {
		s.updateClosedTS(ctx, req.ClosedSequence, req.Closed, req.Epoch, nil, req.FromReplica)
		return nil
	}
...
// Since coalesced heartbeats add latency to heartbeat messages, it is
...
func (s *Store) sendQueuedHeartbeatsToNode(
	...
	if s.cfg.ClosedTimestampInterval == 0 {
		closed = hlc.Timestamp{}
		ranges = nil // empty anyway
	}
...
func (s *Store) sendQueuedHeartbeats(ctx context.Context) {
	...
	closed, epoch := s.getClosedTimestampAndEpoch(ctx)

	s.coalescedMu.Lock()
	heartbeats := s.coalescedMu.heartbeats
	heartbeatResponses := s.coalescedMu.heartbeatResponses
	closedTSInfos := s.coalescedMu.closedTSInfos
	closedSequences := s.coalescedMu.closedSequences
	s.coalescedMu.heartbeats = map[roachpb.StoreIdent][]RaftHeartbeat{}
	s.coalescedMu.heartbeatResponses = map[roachpb.StoreIdent][]RaftHeartbeat{}
	s.coalescedMu.infosByRange = map[roachpb.RangeID]*ClosedTSInfo{}
	s.coalescedMu.closedTSInfos = map[roachpb.StoreIdent]map[roachpb.RangeID]*ClosedTSInfo{}
	...
	// closedTSInfos.
	keys := make(map[roachpb.StoreIdent]struct{}, len(heartbeats)+len(closedTSInfos))
	for k, v := range heartbeats {
		if len(v) > 0 {
			keys[k] = struct{}{}
		}
	}
	for k, v := range closedTSInfos {
		if len(v) > 0 {
			keys[k] = struct{}{}
		}
	}
	...
	var beatsSent int
	var ranges []ClosedTSInfo
	// Send a heartbeat to every store which has either heartbeats or
	// closed timestamp infos pending delivery.
	for to := range keys {
		if _, ok := closedSequences[to]; !ok {
			closedSequences[to] = 0
		}
		for _, info := range closedTSInfos[to] {
			ranges = append(ranges, *info)
		}
		beatsSent += s.sendQueuedHeartbeatsToNode(
			ctx, raftpb.MsgHeartbeat, heartbeats[to], nil, to, closedSequences[to], closed, epoch, ranges,
		)
		// Reset slice, but maintain high water size. We can't reuse the same
		// slice since we passed it to an async RPC goroutine.
		ranges = make([]ClosedTSInfo, len(ranges))
	}
	// Also send an empty heartbeat which includes the closed timestamp to any
	// node which didn't have any heartbeats or closed timestamp infos.
	for to, v := range closedSequences {
		if _, ok := keys[to]; !ok && v > 0 && closed != (hlc.Timestamp{}) {
			beatsSent += s.sendQueuedHeartbeatsToNode(
				ctx, raftpb.MsgHeartbeat, nil, nil, to, closedSequences[to], closed, epoch, nil,
			)
		}
	}
	// Finally, send pending heartbeat responses.
	for to := range heartbeatResponses {
		beatsSent += s.sendQueuedHeartbeatsToNode(
			ctx, raftpb.MsgHeartbeatResp, nil, heartbeatResponses[to], to, 0, hlc.Timestamp{}, 0, nil,
		)
	}
	s.metrics.RaftCoalescedHeartbeatsPending.Update(int64(beatsSent))

	// Increment the values of all closed timestamp sequences. The
	// sequences allow recipients to realize they've missed a heartbeat.
	s.coalescedMu.Lock()
	// Nobody else will have touched coalescedMu.closedSequences since
	// we reset it above, because it's only written to in this method
	// and this method is called from a single ticker goroutine.
	//
	//  TODO(tschottdorf): refactor this away. In particular, it seems
	// that closedSequences can be state local to the ticker loop and
	// does not need to sit on the replica.
	if len(s.coalescedMu.closedSequences) != 0 {
		log.Fatal(ctx, "closedSequences updated out-of-band")
	}
	for k, v := range closedSequences {
		s.coalescedMu.closedSequences[k] = v + 1
	}
	s.coalescedMu.Unlock()
}
...
// proposed to Raft. This allows the closed timestamp sent to
...
func (s *Store) getMinProposal(ctx context.Context) (hlc.Timestamp, func()) {
	if s.cfg.ClosedTimestampInterval == 0 {
		return hlc.Timestamp{}, func() {}
	}

	s.minPropMu.RLock()
	defer s.minPropMu.RUnlock()
	minProp := s.minPropMu.cur
	atomic.AddInt32(&s.minPropMu.curRef, 1)
	cleanup := func() {
		s.minPropMu.RLock()
		defer s.minPropMu.RUnlock()
		if minProp == s.minPropMu.cur {
			val := atomic.AddInt32(&s.minPropMu.curRef, -1)
			if val < 0 {
				log.Fatalf(ctx, "min proposal current ref count < 0")
			}
		} else if minProp == s.minPropMu.last {
			val := atomic.AddInt32(&s.minPropMu.lastRef, -1)
			if val < 0 {
				log.Fatalf(ctx, "min proposal last ref count < 0")
			}
		} else {
			log.Fatalf(ctx, "min proposal %s doesn't match current (%s) or last (%s)", minProp, s.minPropMu.cur, s.minPropMu.last)
		}
	}
	return minProp, cleanup
}

// getClosedTimestamp returns the closed timestamp for serving a follower read
// for the specified range, along with a bool indicating whether the range
// allows a follower read under the conditions discussed below (these conditions
// imply that the range is quiescent at the returned timestamp). When the conditions
// don't hold (or no closed timestamp is found), returns zero values.
//
// To use the returned timestamp, the caller needs to prove that no other store could
// have proposed conflicting writes under a new lease. To this end, it checks
// whether the leaseholder is live at the returned closed timestamp with an epoch matching the
// replica's known lease.
//
// The leaseholder only closes out timestamps it has promised won't appear after the
// replica's min lease applied index (MLAI). If the leaseholder transfers its
// lease away, the new lease will only begin at a timestamp higher than the
// largest closed timestamp sent out.
//
// TODO(tschottdorf): double check where we make sure that the transfer and the
// closed TS can't invert.
//
// TODO(tschottdorf): rename method to reflect the use case specific to quiescent replicas.
//
...
func (s *Store) getClosedTimestamp(
	ctx context.Context, lh *roachpb.ReplicaDescriptor, epoch int64, id roachpb.RangeID,
) (hlc.Timestamp, bool) {
	s.closedMu.RLock()
	defer s.closedMu.RUnlock()
	storeKey := roachpb.StoreIdent{StoreID: lh.StoreID, NodeID: lh.NodeID}
	if closed, ok := s.closedMu.leaseholders[storeKey]; ok {
		if _, ok := closed.quiesced[id]; ok && closed.epoch == epoch {
			return closed.Timestamp, true
		}
	}
	return hlc.Timestamp{}, false
}

// getClosedTimestampAndEpoch returns the current value of the min
// proposal timestamp as closed and this node's liveness epoch. The
// min proposal timestamp is forwarded and persisted. On error,
// closed is returned as the empty timestamp and epoch as 0, which
...
func (s *Store) getClosedTimestampAndEpoch(
	...
	if s.cfg.NodeLiveness == nil || s.cfg.ClosedTimestampInterval == 0 {
		return
	}
	s.minPropMu.Lock()
	if s.minPropMu.lastRef > 0 {
		closed = s.minPropMu.last
	} else {
		// TODO(tschottdorf): if s.minPropMu.curRef == 0, we can return newMinProposal as closed
		closed = s.minPropMu.cur
		// We can advance the min proposal if the last min proposal is no longer in use.
		s.minPropMu.last = s.minPropMu.cur
		atomic.StoreInt32(&s.minPropMu.lastRef, atomic.SwapInt32(&s.minPropMu.curRef, 0))
		newMinProposal := s.Clock().Now().Add(-s.cfg.ClosedTimestampInterval.Nanoseconds(), 0)
	...
	if err != nil || !nl.IsLive(s.Clock().Now(), s.Clock().MaxOffset()) {
		closed = hlc.Timestamp{}
	} else {
		epoch = nl.Epoch
	}
