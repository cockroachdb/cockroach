...
func (s *Store) systemGossipUpdate(sysCfg *config.SystemConfig) {
	...
	s.mu.replicaShims.Range(func(_ int64, v unsafe.Pointer) bool {
		...
		if !zone.Equal(shim.GetZoneConfig()) {
...
func (s *Store) addReplicaShimToRangeMapLocked(shim *ReplicaShim) error {
	...
	if existing, loaded := s.mu.replicaShims.LoadOrStore(int64(rangeID), unsafe.Pointer(shim)); loaded {
		...
		// is added to the replicas map before it is initialized. Note that we never
		// allow a replica to be made non-resident if it's uninitialized, so it will
		// not be nil in this case.
		exShim := (*ReplicaShim)(existing)
		if repl, exRepl := shim.replica, exShim.replica; repl == nil || exRepl == nil || repl != exRepl {
