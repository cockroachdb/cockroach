...
type StoreConfig struct {
	...
	// EnableCoalescedHeartbeats sets whether heartbeats are coalesced. If it
	// has not been proactively set to true when calling NewStore() (i.e.
	// through using a config from TestStoreConfig()) it will be set off of the
	// COCKROACH_ENABLE_COALESCED_HEARBEATS environment variable. By default it
	// is set to false.
...
func NewStore(cfg StoreConfig, eng engine.Engine, nodeDesc *roachpb.NodeDescriptor) *Store {
	...
	if !cfg.EnableCoalescedHeartbeats {
		s.cfg.EnableCoalescedHeartbeats = envutil.EnvOrDefaultBool(
			"COCKROACH_ENABLE_COALESCED_HEARBEATS", false)
	}
