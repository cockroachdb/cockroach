...
// ranges (filling in meta1 and meta2).
//
// Default configurations for zones are created. All configs are specified for
// the empty key prefix, meaning they apply to the entire database. The zone
// requires three replicas with no other specifications.  It also adds the range
// tree and the root node, the first range, to it.
//
// Args:
...
func (s *Store) WriteInitialData(
	...
	if bootstrapVersion != cluster.VersionByKey(cluster.VersionBase) {
		// We'll add the "bootstrap version" to the list of initialValues, so that
		// we don't have to handle it specially (particularly since we don't want to
		// manually figure out which range it falls into).
	...
	// firstaRangeMS is going to accumulate the stats for the first range, as we
	...
	filterInitialValues := func(desc *roachpb.RangeDescriptor) []roachpb.KeyValue {
		...
		for _, kv := range initialValues {
			if kv.Key.Compare(desc.StartKey.AsRawKey()) >= 0 &&
				kv.Key.Compare(desc.EndKey.AsRawKey()) < 0 {
				r = append(r, kv)
			}
		}
		return r
	...
	// to the stats of the first range (e.g., because they all write meta2 records
	// in the first range), and so we want to create the first range last.
	...
	for i := len(splits) - 1; i >= -1; i-- {
		...
		log.VEventf(ctx, 2, "creating range %d [%s, %s). Initial values: %d", desc.RangeID, desc.StartKey, desc.EndKey, len(rangeInitialValues))
		batch := s.engine.NewBatch()
		defer batch.Close()

		// The first range gets some special treatment.
		firstRange := startKey.Equal(roachpb.RKeyMin)
		...
		// Range descriptor.
		if err := engine.MVCCPutProto(
			ctx, batch, ms, keys.RangeDescriptorKey(desc.StartKey),
			now, nil /* txn */, desc,
		); err != nil {
			return err
		}
		...
		if firstRange {
			...
			if err := engine.MVCCPutProto(
				ctx, batch, ms, meta1Key.AsRawKey(), now, nil /* txn */, desc,
			); err != nil {
				return err
			}
		}
		...
		for _, kv := range rangeInitialValues {
			...
			if err := engine.MVCCPut(ctx, batch, ms, kv.Key, now, kv.Value, nil /* txn */); err != nil {
				return err
			}
		}
