...
// replica, identified by rightDesc. The right-hand replica, or a placeholder
// for it, must exist on this store. The raftMus for both the left-hand and
// right-hand replicas must be held.
func (s *Store) MergeRange(
	...
	const placeholdersOK = true
	if err := s.removeReplicaImpl(ctx, rightRepl, rightDesc, destroyData, placeholdersOK); err != nil {
...
func (s *Store) RemoveReplica(
	...
	return s.removeReplicaImpl(ctx, rep, consistentDesc, destroyData, false)
}
...
//
// PREMERGE(benesch): Avoid the double bool signature.
