...
// into the merge's intent and attempts to push the merge. Since the split has
// the same priority as the merge, this amounts to waiting for the merge to
// complete. The merge then tries to update Q's local range descriptor but runs
// into the split's intent. While pushing the split, the merge realizes that
// waiting for the split to complete would cause deadlock, so it aborts the
// split instead.
//
