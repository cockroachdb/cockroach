...
func TestInvalidSubsumeRequest(t *testing.T) {
	...
	// A subsume request that succeeds when it shouldn't will wedge a store
	// because it waits for a merge that is not actually in progress. Set a
	...
	subsumeArgs := roachpb.SubsumeRequest{
	...
	// Subsume from a non-neighboring LHS should fail.
	{
		...
		if !testutils.IsError(pErr.GoError(), "ranges are not adjacent") {
			t.Fatalf("expected 'ranges are not adjacent' error, but got %v", pErr)
		}
	}
	...
	if !testutils.IsError(pErr.GoError(), "range missing intent on its local descriptor") {
		t.Fatalf("expected 'range missing intent' error, but got %v", pErr)
	}

	// Subsume when a non-deletion intent is present on the local range
	...
	err := store.DB().Txn(ctx, func(ctx context.Context, txn *client.Txn) error {
		...
		// NB: The subsume request intentionally takes place outside of txn so
		// that it sees an intent rather than the value the txn just wrote.
		_, pErr := client.SendWrappedWith(ctx, store.TestSender(), roachpb.Header{
			RangeID: bDesc.RangeID,
		}, &subsumeArgs)
		if !testutils.IsError(pErr.GoError(), "non-deletion intent on local range descriptor") {
			return fmt.Errorf("expected 'non-deletion intent' error, but got %v", pErr)
		}
		return nil
	})
	if err != nil {
