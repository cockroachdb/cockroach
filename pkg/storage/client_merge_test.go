	...
	storeCfg.TestingKnobs.BeforeSnapshotSSTIngestion = func(
		...
		for i := range sstNamesSubset {
			...
			if !bytes.Equal(actualSST, expectedSSTs[i]) {
				...
				actualSSTIter, err := engine.NewMemSSTIterator(actualSST, false)
				if err != nil {
					return err
				}
				expectedSSTIter, err := engine.NewMemSSTIterator(expectedSSTs[i], false)
				if err != nil {
					return err
				}
				actualSSTIter.SeekGE(engine.MakeMVCCMetadataKey(keys.MinKey))
				expectedSSTIter.SeekGE(engine.MakeMVCCMetadataKey(keys.MinKey))
				defer actualSSTIter.Close()
				defer expectedSSTIter.Close()

				for {
					ok, err := actualSSTIter.Valid()
					if err != nil {
						return err
					}
					ok2, err := expectedSSTIter.Valid()
					if err != nil {
						return err
					}
					if !ok || !ok2 {
						fmt.Println("Iterators not ok")
						break
					}

					actualKey, expectedKey := actualSSTIter.UnsafeKey(), expectedSSTIter.UnsafeKey()
					if actualKey.String() != expectedKey.String() {
						return errors.Errorf("Actual and expected keys don't match. Actual: %v Expected: %v\n", actualKey, expectedKey)
					}
					if !bytes.Equal(actualSSTIter.UnsafeValue(), expectedSSTIter.UnsafeValue()) {
						return errors.Errorf("Values associated with Key %v are mismatched\n", actualKey)
					}
					actualSSTIter.Next()
					expectedSSTIter.Next()
				}
			}
		}
