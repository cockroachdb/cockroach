...
func TestQuotaPool(t *testing.T) {
	...
	if leaderRepl.QuotaAvailable() > quota/4 {
		t.Fatalf("didn't observe the expected quota acquisition, available: %d", leaderRepl.QuotaAvailable())
	}
	...
	testutils.SucceedsSoonDepth(1, t, func() error {
		if curQuota := leaderRepl.QuotaAvailable(); curQuota != quota {
			log.Infof(context.Background(), "r: cur quota: %d", curQuota)
