...
func TestStoreRangeWaitForApplication(t *testing.T) {
	...
	// Waiting for a command that is already applied should return immediately.
	...
	for _, target := range targets {
		...
		go func() {
			...
			log.Errorf(ctx, "%d has returned", target.header.StoreID-1)
			errCh <- err
		}()
	}

	// WaitForApplication should not return when less than `count` commands have
	// been issued.
	...
	// Once the `count`th command has been issued, WaitForApplication should
	// return.
	if _, pErr := client.SendWrapped(ctx, distSender, putArgs); pErr != nil {
		t.Fatal(pErr)
	}
	...
	// A WaitForApplication request should get aborted if the replica gets GC'd
	...
	// Canceling the client's context should abort a WaitForApplication request,
	// even when there is no traffic on the range.
	//
	// NOTE: be very careful not to inadvertently cause traffic on the range after
	// this WaitForApplication request is sent, e.g. by moving this stanza earlier
	// in the test. We want to test that canceling the context broadcasts on the
	// stateChanged condition variable; if we accidentally send a request to the
	// range, we'll just test the normal code path in which applying a Raft
	// command broadcasts on stateChanged. We're relying on the goroutine leak
	// detector here to notice whether the server's WaitForApplication handler is
	// cleaned up or not.
	{
