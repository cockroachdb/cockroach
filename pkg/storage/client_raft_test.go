...
func TestReportUnreachableHeartbeats(t *testing.T) {
	...
	testStoreConfig := mtc.makeStoreConfig(0)
	ticksToWait := 2 * testStoreConfig.RaftElectionTimeoutTicks
	cb := mtc.transports[0].GetCircuitBreaker(roachpb.NodeID(3))
	cb.Break()
	time.Sleep(time.Duration(ticksToWait) * (testStoreConfig.RaftTickInterval))
