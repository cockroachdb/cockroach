...
func TestReplicateRogueRemovedNode(t *testing.T) {
	...
	partRange, err := setupPartitionedRange(mtc, 1, 0, 0, false /* activated */, dropFuncs{})
...
// In the third case:
//
//   *  s1 never recieves a message for r21/3.
//
//   *  Before the r20 processes the split r21 is removed and re-added to
...
func TestProcessSplitAfterRightHandSideHasBeenRemoved(t *testing.T) {
		...
		f := storagebase.ReplicaProposalFilter(func(args storagebase.ProposalFilterArgs) *roachpb.Error {
			...
			setupOnce.Do(func() {
				...
				*partRange, err = basePartition.extend(mtc, split.RightDesc.RangeID, replDesc.ReplicaID,
					0 /* partitionedNode */, true /* activated */, dropFuncs{})
				require.NoError(t, err)
				proposalFilter.Store(noopProposalFilter)
			})
			return nil
	...
	setup := func(t *testing.T) (
		...
		lhsPartition, err = setupPartitionedRange(mtc, lhsID,
			0 /* replicaID */, 0 /* partitionedNode */, false /* activated */, dropFuncs{})
		require.NoError(t, err)
		// Wait for all nodes to catch up.
	...
	// state.
	t.Run("no RHS partition", func(t *testing.T) {
	...
	t.Run("no RHS partition, with restart", func(t *testing.T) {
		mtc, db, keyA, keyB, _, lhsPartition := setup(t)
		defer mtc.Stop()
	...
	t.Run("initial replica RHS partition, no restart", func(t *testing.T) {
		mtc, db, keyA, keyB, _, lhsPartition := setup(t)
		defer mtc.Stop()
	...
	t.Run("initial replica RHS partition, with restart", func(t *testing.T) {
		mtc, db, keyA, keyB, _, lhsPartition := setup(t)
		defer mtc.Stop()
		var rhsPartition *mtcPartitionedRange
