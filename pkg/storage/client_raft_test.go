...
func TestDefaultConnectionDisruptionDoesNotInterfereWithSystemTraffic(t *testing.T) {
	...
	if _, err := client.SendWrapped(context.Background(), mtc.stores[0].TestSender(), putReq); err != nil {
		t.Fatal(err)
	}
	// Wait for all nodes to catch up.
	...
	if _, err := client.SendWrapped(withTimeout, mtc.stores[0].TestSender(), putReq); err == nil {
		t.Fatal("expected an error when sending to a disconnected store")
	}
	...
	if _, err := client.SendWrapped(ctx, mtc.stores[0].TestSender(), putReq); err != nil {
		t.Fatalf("expected to succeed after healing the partition: %v", err)
	}
