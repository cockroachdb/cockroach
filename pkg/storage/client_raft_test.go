...
func TestLogGrowthWhenRefreshingPendingCommands(t *testing.T) {
	...
	incRes := make(chan *roachpb.Error)
	go func() {
		_, err := client.SendWrapped(context.Background(), mtc.stores[0].TestSender(), incArgs)
		incRes <- err
	}()
...
Loop:
	for {
		...
		case <-wait:
			break Loop
		case <-ticker:
			const logSizeLimit = 16 << 10 // 16 KB
