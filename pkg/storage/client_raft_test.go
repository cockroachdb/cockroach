...
func TestSnapshotAfterTruncation(t *testing.T) {
	...
	for _, changeTerm := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			// Verify that the cached term (r.mu.lastTerm) on the stopped store has
			// been updated correctly. #18327 fixed an issue where the cached term
			// was left unchanged after applying a snapshot leading to a persistently
			// unavailable range.
			repl0, err = mtc.stores[0].GetReplica(1)
			if err != nil {
				t.Fatal(err)
			...
			repl1, err := mtc.stores[stoppedStore].GetReplica(1)
			if err != nil {
				t.Fatal(err)
			}
			if lastTerm := repl1.GetCachedLastTerm(); expectedLastTerm != lastTerm {
				t.Fatalf("expected last term %d, but found %d", expectedLastTerm, lastTerm)
			}
		})
	}
}
