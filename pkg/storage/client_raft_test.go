...
func TestTransferRaftLeadership(t *testing.T) {
	...
	testutils.SucceedsSoon(t, func() error {
		err := repl1.RaftTransferLeader(context.Background(), rd0.ReplicaID)
		if !testutils.IsError(err, "is not Raft leader") {
			return err
		}
	...
	testutils.SucceedsSoon(t, func() error {
		if err := store1.ComputeMetrics(context.Background(), 0); err != nil {
			return err
		}
		status := repl0.RaftStatus()
		if status.Lead != 2 {
