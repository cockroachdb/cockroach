...
type initialState interface {
	// Base may return an initialState to extend. If Base is non-nil, Build we
	// supplied an engine with the initial state Base. This allows initial
...
func getInitialStateEngine(
	...
	const initialStatesDir = `initial_states`
	...
	if _, err := os.Stat(dir); oserror.IsNotExist(err) {
		// There's no existing engine directory for these initial condtions.
		// Produce it.
		b.Logf("dir %q does not exist; building initial state first", dir)
	...
	if !inMemory {
		...
		ok, err := vfs.Clone(vfs.Default, vfs.Default, dir, testRunDir, vfs.CloneSync)
	...
	if buildFS != nil {
		...
		fs = vfs.NewMem()
		ok, err := vfs.Clone(vfs.Default, fs, dir, "")
		require.NoError(b, err)
		require.True(b, ok)
	}
...
func buildInitialState(
	...
	ok, err := vfs.Clone(buildFS, vfs.Default, "", dir, vfs.CloneSync)
	require.NoError(b, err)
	require.True(b, ok)
	return buildFS
}
