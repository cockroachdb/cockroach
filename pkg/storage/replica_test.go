...
func TestAmbiguousResultErrorOnReproposal(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			checkFn: func(pErr *roachpb.Error) error {
				...
				if err, ok := pErr.GetDetail().(*roachpb.AmbiguousResultError); !ok {
					return errors.Wrapf(err, "unexpected error %T", err)
				}
				return nil
			},
		},
		{
			name: "endtxn/commit=true",
			ba:   baCommitTxn,
			checkFn: func(pErr *roachpb.Error) error {
				if pErr == nil {
					return errors.Errorf("received no error")
				}
				if err, ok := pErr.GetDetail().(*roachpb.AmbiguousResultError); !ok {
					return errors.Wrapf(err, "unexpected error %T", err)
				}
		...
		{
			name: "endtxn/commit=false",
			ba:   baAbortTxn,
			checkFn: func(pErr *roachpb.Error) error {
				if pErr == nil {
					return errors.Errorf("received no error")
				}
				if err, ok := pErr.GetDetail().(*roachpb.TransactionStatusError); !ok {
					return errors.Wrapf(err, "unexpected error %T", err)
				}
				return nil
			},
