...
func testRangeDanglingMetaIntent(t *testing.T, legacy, isReverse bool) {
	...
	rs, _, err := rangeLookup(ctx, tc.Sender(), key, roachpb.READ_UNCOMMITTED, 0, isReverse)
	if err != nil {
		t.Fatal(err)
	}
	origDesc := rs[0]
	...
	rs, _, err = rangeLookup(ctx, tc.Sender(), newKey, roachpb.READ_UNCOMMITTED, 0, isReverse)
	if err != nil {
		t.Fatal(err)
	}
	if len(rs) != 2 {
