...
func TestMaybeStripInFlightWrites(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			reqs:           []roachpb.Request{put1, et},
			expIFW:         []roachpb.SequencedWrite{{Key: keyB, Sequence: 2}},
			expIntentSpans: []roachpb.Span{{Key: keyA}, {Key: keyC}},
		},
		{
...
func TestTxnRecordLifecycleTransitions(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			// Should not be possible, but also not prevented. The second stage
			// will always either bump the commit timestamp or bump the epoch.
