...
func TestReplicaDestroy(t *testing.T) {
	...
	if err := tc.store.removeReplicaImpl(tc.repl, *origDesc, true); !testutils.IsError(err, "replica ID has changed") {
		t.Fatalf("expected error 'replica ID has changed' but got %v", err)
	}

	// Now try a fresh descriptor and succeed.
	if err := tc.store.removeReplicaImpl(tc.repl, *repl.Desc(), true); err != nil {
		t.Fatal(err)
	}
}
...
func TestAmbiguousResultErrorOnReproposal(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			checkFn: func(pErr *roachpb.Error) error {
				if pErr != nil {
					return errors.Errorf("expected success on re-proposal of puts-only txn")
				}
				return nil
			},
		},
		{
			...
			checkFn: func(pErr *roachpb.Error) error {
				...
				// unittest.
				if _, ok := pErr.GetDetail().(*roachpb.AmbiguousResultError); !ok && pErr != nil {
					return errors.Errorf("expected either no error or an AmbiguousResultError but got %T: %s", pErr, pErr)
				}
				return nil
			},
		},
		{
			...
			checkFn: func(pErr *roachpb.Error) error {
				if _, ok := pErr.GetDetail().(*roachpb.AmbiguousResultError); !ok {
					return errors.Errorf("expected AmbiguousResultError but got %T: %s", pErr, pErr)
				}
				return nil
			},
		},
	...
	setCustomProposalFn := func(expBatch roachpb.BatchRequest) {
	...
	for i, c := range testCases {
		...
		// Install the custom proposal function (it is unset after the first .
		setCustomProposalFn(c.ba)
