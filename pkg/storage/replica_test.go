...
// removed a txn record created through a successful push (by a concurrent
// actor), the original transaction's subsequent attempt to create its initial
// record fails.
//
...
func TestTxnRecordUnderTxnSpanGCThreshold(t *testing.T) {
	...
	// would not allow transactions to be revived.
	{
...
func TestPushTxnAlreadyCommittedOrAborted(t *testing.T) {
	...
	// record but verifies that the transaction has been finalized using the
...
// TestResolveIntentPushTxnInTxn makes sure that PushTxn and
// ResolveIntent{,Range} can not be carried out in a transaction.
func TestResolveIntentPushTxnInTxn(t *testing.T) {
	defer leaktest.AfterTest(t)()
	tc := testContext{}
	stopper := stop.NewStopper()
	defer stopper.Stop(context.TODO())
