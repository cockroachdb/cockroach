...
func (q *testQuiescer) Clock() *hlc.Clock {
	// The clock isn't important, we just need one to respond to Now(). The
	// timestamp will be swallowed by ownsValidLeaseRLocked() and
	// maybeTransferRaftLeader.
	return hlc.NewClock(hlc.UnixNano, 0)
}
...
func TestShouldReplicaQuiesce(t *testing.T) {
	...
	test := func(expected bool, transform func(q *testQuiescer) *testQuiescer) {
		t.Run("", func(t *testing.T) {
			...
			q = transform(q)
			_, ok := shouldReplicaQuiesce(context.Background(), q, q.numProposals)
			if expected != ok {
