...
func TestContainsEstimatesClampProposal(t *testing.T) {
	...
	t.Run("clamp to 1 when version < VersionContainsEstimatesCounter", func(t *testing.T) {
		ctx := context.Background()
		stopper := stop.NewStopper()
		defer stopper.Stop(ctx)
		cfg := TestStoreConfig(nil)
		version := cluster.VersionByKey(cluster.VersionContainsEstimatesCounter - 1)
		cfg.Settings = cluster.MakeTestingClusterSettingsWithVersion(version, version)
		var tc testContext
		tc.StartWithStoreConfig(t, stopper, cfg)
	...
	t.Run("double when version == VersionContainsEstimatesCounter", func(t *testing.T) {
		ctx := context.Background()
		stopper := stop.NewStopper()
		defer stopper.Stop(ctx)
		cfg := TestStoreConfig(nil)
		version := cluster.VersionByKey(cluster.VersionContainsEstimatesCounter)
		cfg.Settings = cluster.MakeTestingClusterSettingsWithVersion(version, version)
		var tc testContext
		tc.StartWithStoreConfig(t, stopper, cfg)
