...
func TestReplicaTxnIdempotency(t *testing.T) {
	...
	keyAtSeqHasVal := func(txn *roachpb.Transaction, key []byte, seq int32, val *roachpb.Value) error {
		args := getArgs(key)
		args.Sequence = seq
	...
	testCases := []struct {
		...
		},
		{
			...
			// then the previously successful portion will be evaluated as a no-op
			// while the rest will evaluate as normal.
