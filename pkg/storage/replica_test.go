...
func TestReplicaDestroy(t *testing.T) {
	...
	if err := tc.store.removeReplicaImpl(context.Background(), tc.repl, *origDesc, true, false); !testutils.IsError(err, expectedErr) {
		t.Fatalf("expected error %q but got %v", expectedErr, err)
	}

	// Now try a fresh descriptor and succeed.
	if err := tc.store.removeReplicaImpl(context.Background(), tc.repl, *repl.Desc(), true, false); err != nil {
		t.Fatal(err)
	}
...
func TestQuotaPoolAccessOnDestroyedReplica(t *testing.T) {
	...
	if err := tc.store.removeReplicaImpl(context.TODO(), repl, *repl.Desc(), true, false); err != nil {
		t.Fatal(err)
	}
