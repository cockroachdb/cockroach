...
func newTransaction(
	...
	txn := roachpb.MakeTransaction(name, baseKey, userPriority, enginepb.SERIALIZABLE, now, offset)
	return &txn
}
...
func TestReplicaPushed1PC(t *testing.T) {
	...
	ts1 := tc.Clock().Now()
	txn := roachpb.MakeTransaction("test", k, roachpb.NormalUserPriority, enginepb.SERIALIZABLE, ts1, 0)
