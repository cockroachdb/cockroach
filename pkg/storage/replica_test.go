...
func recoverTxnArgs(txn *roachpb.Transaction, allWritesFound bool) roachpb.RecoverTxnRequest {
	return roachpb.RecoverTxnRequest{
		RequestHeader: roachpb.RequestHeader{
			Key: txn.Key,
		},
		Txn:            txn.TxnMeta,
		AllWritesFound: allWritesFound,
	}
}
...
func TestPushTxnHeartbeatTimeout(t *testing.T) {
	...
	testCases := []struct {
		...
		// returns a TransactionPushError will continue to return that error,
...
// even when the push occurred before the trasaction record was created.
func TestTxnRecordLifecycleTransitions(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			// sent after discovering a transaction record.
			name: "recover transaction (all writes found)",
			run: func(txn *roachpb.Transaction, _ hlc.Timestamp) error {
				rt := recoverTxnArgs(txn, true /* allWritesFound */)
		...
		},
		{
			// Should not happen because RecoverTxn requests are only
			// sent after discovering a transaction record.
			name: "recover transaction (writes prevented)",
			run: func(txn *roachpb.Transaction, _ hlc.Timestamp) error {
				rt := recoverTxnArgs(txn, false /* allWritesFound */)
				return sendWrappedWithErr(roachpb.Header{}, &rt)
			},
			expError: "txn record synthesized with non-ABORTED status",
			expTxn:   noTxnRecord,
		...
		{
			name: "recover transaction (all writes found) after begin transaction",
			setup: func(txn *roachpb.Transaction, _ hlc.Timestamp) error {
				bt, btH := beginTxnArgs(txn.Key, txn)
				return sendWrappedWithErr(btH, &bt)
			},
		...
		{
			name: "recover transaction (all writes found) after end transaction (abort)",
			setup: func(txn *roachpb.Transaction, _ hlc.Timestamp) error {
				et, etH := endTxnArgs(txn, false /* commit */)
				return sendWrappedWithErr(etH, &et)
			},
			...
			// the same assertion that it does in the can without eager gc.
			expTxn: noTxnRecord,
		},
		{
			name: "recover transaction (all writes found) after end transaction (commit)",
			setup: func(txn *roachpb.Transaction, _ hlc.Timestamp) error {
				et, etH := endTxnArgs(txn, true /* commit */)
				return sendWrappedWithErr(etH, &et)
			},
		...
		{
			name: "recover transaction (all writes found) after end transaction (abort) without eager gc",
			setup: func(txn *roachpb.Transaction, _ hlc.Timestamp) error {
				et, etH := endTxnArgs(txn, false /* commit */)
				return sendWrappedWithErr(etH, &et)
			},
		...
		{
			name: "recover transaction (all writes found) after end transaction (commit) without eager gc",
			setup: func(txn *roachpb.Transaction, _ hlc.Timestamp) error {
				et, etH := endTxnArgs(txn, true /* commit */)
				return sendWrappedWithErr(etH, &et)
			},
		...
		{
			name: "recover transaction (writes prevented) after begin transaction",
			setup: func(txn *roachpb.Transaction, _ hlc.Timestamp) error {
				bt, btH := beginTxnArgs(txn.Key, txn)
				return sendWrappedWithErr(btH, &bt)
			},
		...
		{
			name: "recover transaction (writes prevented) after begin transaction with epoch bump",
			setup: func(txn *roachpb.Transaction, now hlc.Timestamp) error {
				clone := txn.Clone()
				clone.Restart(-1, 0, now)
				bt, btH := beginTxnArgs(clone.Key, &clone)
		...
		{
			name: "recover transaction (writes prevented) after end transaction (abort)",
			setup: func(txn *roachpb.Transaction, _ hlc.Timestamp) error {
				et, etH := endTxnArgs(txn, false /* commit */)
				return sendWrappedWithErr(etH, &et)
			},
		...
		{
			name: "recover transaction (writes prevented) after end transaction (commit)",
			setup: func(txn *roachpb.Transaction, _ hlc.Timestamp) error {
				et, etH := endTxnArgs(txn, true /* commit */)
				return sendWrappedWithErr(etH, &et)
			},
		...
		{
			name: "recover transaction (writes prevented) after end transaction (abort) without eager gc",
			setup: func(txn *roachpb.Transaction, _ hlc.Timestamp) error {
				et, etH := endTxnArgs(txn, false /* commit */)
				return sendWrappedWithErr(etH, &et)
			},
		...
		{
			name: "recover transaction (writes prevented) after end transaction (commit) without eager gc",
			setup: func(txn *roachpb.Transaction, _ hlc.Timestamp) error {
				et, etH := endTxnArgs(txn, true /* commit */)
				return sendWrappedWithErr(etH, &et)
			},
