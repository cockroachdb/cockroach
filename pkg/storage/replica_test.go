...
func TestReplicaDanglingMetaIntent(t *testing.T) {
	...
	for _, compat := range []bool{false, true} {
		t.Run(fmt.Sprintf("compat=%t", compat), func(t *testing.T) {
			for _, reverse := range []bool{false, true} {
				t.Run(fmt.Sprintf("reverse=%t", reverse), func(t *testing.T) {
					testRangeDanglingMetaIntent(t, compat, reverse)
				})
			}
		})
	}
...
func testRangeDanglingMetaIntent(t *testing.T, compat, isReverse bool) {
	rangeLookup := client.LookupRange
	if compat {
		rangeLookup = client.LookupRangeCompat
	}
