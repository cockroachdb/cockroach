...
func TestPushTxnHeartbeatTimeout(t *testing.T) {
	...
	for i, test := range testCases {
		key := roachpb.Key(fmt.Sprintf("key-%d", i))
		pushee := newTransaction(fmt.Sprintf("test-%d", i), key, roachpb.UnspecifiedUserPriority, enginepb.SERIALIZABLE, tc.Clock())
		pusher := newTransaction("pusher", key, roachpb.UnspecifiedUserPriority, enginepb.SERIALIZABLE, tc.Clock())
