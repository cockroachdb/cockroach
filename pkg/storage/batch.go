...
//
//   <key>[<wall_time>[<logical>[<notFromClock>]]]<#timestamp-bytes>
//
// The <wall_time>, <logical>, and <notFromClock> portions of the key are
// encoded as 64-bit, 32-bit, and 8-bit big-endian integers, respectively. A
// custom RocksDB comparator is used to maintain the desired ordering as these
...
func encodeKeyToBuf(buf []byte, key MVCCKey, keyLen int) {
	...
	if timestampLength > 0 {
		...
		if key.Timestamp.Logical != 0 || !key.Timestamp.FromClock {
			binary.BigEndian.PutUint32(buf[pos:], uint32(key.Timestamp.Logical))
			pos += logicalEncodedLen
		}
		if !key.Timestamp.FromClock {
			buf[pos] = 1
			pos += notFromClockEncodedLen
		}
	}
