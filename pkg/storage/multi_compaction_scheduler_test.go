...
type testDBForCompaction struct {
	...
	// onScheduleAccepted is called when Schedule accepts a handle.
	onScheduleAccepted func(pebble.CompactionGrantHandle)
	// rejectNext causes the next Schedule call to return false.
	rejectNext bool
	// log records calls for debugging.
	log []string
}
...
func (d *testDBForCompaction) GetWaitingCompaction() (bool, pebble.WaitingCompaction) {
	d.log = append(d.log, fmt.Sprintf("%s: GetWaitingCompaction", d.name))
	if d.waiting == nil {
...
func (d *testDBForCompaction) Schedule(h pebble.CompactionGrantHandle) bool {
	if d.rejectNext {
		d.rejectNext = false
		d.log = append(d.log, fmt.Sprintf("%s: Schedule => rejected", d.name))
		return false
	}
	if d.onScheduleAccepted != nil {
		d.onScheduleAccepted(h)
	}
	...
	d.log = append(d.log, fmt.Sprintf("%s: Schedule => accepted", d.name))
	return true
}

func (d *testDBForCompaction) reset() {
	d.log = nil
}
...
func TestMultiCompactionScheduler(t *testing.T) {
	...
	datadriven.RunTest(t, datapathutils.TestDataPath(t, "multi_compaction_scheduler"),
		func(t *testing.T, d *datadriven.TestData) string {
			...
			case "init":
				...
				opts.testingDisablePeriodicGranter = true
				if stopper != nil {
					stopper.Stop(context.Background())
				}
				stopper = stop.NewStopper()
				scheduler = NewMultiEngineCompactionScheduler(opts, stopper)
				engines = map[string]*engineState{}
				dbs = map[string]*testDBForCompaction{}
				handles = map[string]*handleList{}
				...
				for _, spec := range []struct {
					...
					{"log", LogEngine, logAllowed},
					{"state", StateEngine, stateAllowed},
				} {
					e := scheduler.OpeningEngine(spec.et).(*engineState)
					engines[spec.name] = e
					hl := &handleList{}
					handles[spec.name] = hl
					db := &testDBForCompaction{name: spec.name, allowed: spec.allowed}
					db.onScheduleAccepted = func(h pebble.CompactionGrantHandle) {
						hl.h = append(hl.h, h)
					}
					dbs[spec.name] = db
					e.Register(1, db)
			...
			case "set-allowed":
				...
				if d.HasArg("inform") {
					...
					// Periodic granter is disabled; directly trigger granting.
					scheduler.mu.Lock()
					scheduler.tryGrantLockedAndUnlock()
				}
				logs := collectLogs()
				if logs != "" {
					return logs + "\n"
			...
			case "compaction-done":
				...
				hl := handles[name]
				if idx >= len(hl.h) {
					t.Fatalf("no handle at index %d for %s", idx, name)
				}
				hl.h[idx].Done()
				logs := collectLogs()
				if logs != "" {
			...
			case "tick":
				// Directly trigger granting (periodic granter is disabled).
				scheduler.mu.Lock()
				scheduler.tryGrantLockedAndUnlock()
				logs := collectLogs()
				if logs != "" {
			...
			case "print-state":
				...
				for _, e := range scheduler.mu.engines {
					...
					fmt.Fprintf(&buf, "%s: running=%d waiting=%v registered=%v\n",
						typ, e.running, e.waiting.value, e.registered)
				}
				scheduler.mu.Unlock()
				return buf.String()
...
func TestMultiCompactionSchedulerConcurrency(t *testing.T) {
	...
	opts := makeSchedulerOpts(maxConcurrency, 0.5)
	stopper := stop.NewStopper()
	...
	// remainingScheduled limits Schedule-initiated compactions per engine.
	// This is less than iterations (100) so that the Schedule path is
	// exercised concurrently with the TrySchedule goroutines, but
	// eventually exhausted — preventing unbounded Schedule → Done →
	// tryGrant → Schedule feedback. Once a TrySchedule goroutine finishes
	// its iterations and the Schedule budget is exhausted for that engine,
	// the goroutine exits. It's fine if remainingScheduled is not fully
	// consumed — that just means the granter didn't find enough
	// opportunities.
	const scheduleBudget = 50
	...
	logEngine := scheduler.OpeningEngine(LogEngine).(*engineState)
	stateEngine := scheduler.OpeningEngine(StateEngine).(*engineState)
	...
	for i := 0; i < numGoroutines; i++ {
		...
		go func(e *engineState, rc *atomic.Int32, allowed int32) {
			defer wg.Done()
...
func (d *concurrencyTestDB) Schedule(h pebble.CompactionGrantHandle) bool {
	if d.remainingScheduled.Add(-1) < 0 {
		d.remainingScheduled.Add(1) // undo
		return false
	}
	d.wg.Add(1)
	go func() {
...
func TestMultiCompactionSchedulerPebble(t *testing.T) {
	...
	db1 := openDB("/db1", LogEngine)
	db2 := openDB("/db2", StateEngine)
