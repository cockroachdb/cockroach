...
CockroachDB uses cluster versions to accomodate backwards-incompatible
...
Pebble has its own concept of backwards-incompatiblity gating, using
...
atomic with the persistence of the `STORAGE_MIN_VERSION` file, Pebble
re-sets the min-version on store open, ensuring that the storage engine
is using the same format dictated by the `STORAGE_MIN_VERSION` file.
