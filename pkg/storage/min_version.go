...
func WriteMinVersionFile(unencryptedFS vfs.FS, dir string, version *roachpb.Version) error {
	// TODO(jackson): Assert that unencryptedFS supports atomic renames
	// once Pebble is bumped to the appropriate SHA.
	if version == nil {
		return errors.New("min version should not be nil")
	}
	ok, err := MinVersionIsAtLeastTargetVersion(unencryptedFS, dir, version)
	if err != nil {
		return err
	}
	if ok {
	...
	filename := unencryptedFS.PathJoin(dir, MinVersionFilename)
	if err := SafeWriteToFile(unencryptedFS, dir, filename, b); err != nil {
		return err
	}
	return nil
}
...
func MinVersionIsAtLeastTargetVersion(
	unencryptedFS vfs.FS, dir string, target *roachpb.Version,
) (bool, error) {
	// TODO(jackson): Assert that unencryptedFS supports atomic renames
	// once Pebble is bumped to the appropriate SHA.
	if target == nil {
	...
	minVersion, err := getMinVersion(unencryptedFS, dir)
	if err != nil {
		return false, err
	}
	if minVersion == nil {
...
func getMinVersion(unencryptedFS vfs.FS, dir string) (*roachpb.Version, error) {
	// TODO(jackson): Assert that unencryptedFS supports atomic renames
	// once Pebble is bumped to the appropriate SHA.

	filename := unencryptedFS.PathJoin(dir, MinVersionFilename)
	f, err := unencryptedFS.Open(filename)
