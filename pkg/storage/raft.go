...
const is14231HelpMessage = "Server crashing due to missing data. Was the server restarted with a " +
	"different store directory than before? A --join parameter must be specified in order to make " +
	"restarts with a new store directory safe. Please try again with a new directory and a valid " +
	"--join flag. If this server was started with its old data or with a valid --join parameter " +
	"and you are still seeing this message, please report an issue at " +
	"https://github.com/cockroachdb/cockroach/issues/new"
