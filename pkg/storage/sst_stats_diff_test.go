...
func TestMVCCComputeSSTStatsDiff(t *testing.T) {
	...
	// pfixed is like p, but simply sets the value to the key.
	...
	testCases := []struct {
		...
		},
		{
			...
			local: storageutils.KVs{storageutils.PointKV("a", 1, "a1")},
		},
		{
			name:  "exhaustLocal",
			sst:   p("b1"),
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			testutils.RunTrueAndFalse(t, "randomDeletes", func(t *testing.T, randomDeletes bool) {
				...
				if tc.skipRandomDeletes && randomDeletes {
