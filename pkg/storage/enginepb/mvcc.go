...
// SetLocalTimestamp sets the local timestamp field to the provided value, if
// necessary. See MVCCKey's LocalTimestampNeeded and Normalize methods for
// details on when the local timestamp can be omitted.
//
...
func (meta *MVCCMetadata) SetLocalTimestamp(localTs hlc.ClockTimestamp, alloc *hlc.ClockTimestamp) {
	if localTs.IsEmpty() || meta.Timestamp.ToTimestamp().LessEq(localTs.ToTimestamp()) {
