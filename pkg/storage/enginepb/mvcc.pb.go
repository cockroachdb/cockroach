...
type MVCCMetadata struct {
	...
	//
	// Empty for MVCCMetadata structs that do not represent intents.
	...
	// The timestamp that the intent was last written at by its own transaction,
	// in cases where the intent has been pushed to a higher timestamp than this
	// by a conflicting transaction. The timestamp is originally empty before the
	// intent has been pushed and is reset to empty if the intent is rewritten by
	// its own transaction with a newer epoch or sequence number.
	//
	// If set, the timestamp represents the highest time that the intent held in
	// response to an action taken by its own transaction before committing. As a
	// result, if the transaction does commit, this timestamp also serves as a
	// lower bound on the clock of the leaseholder that holds this intent at the
	// time that the transaction committed, because leaseholders forward their
	// clock to the timestamp of incoming writes. By extension, it also serves as
	// a lower bound on the clock of any leaseholder that holds the intent at any
	// time after the transaction committed, because the clock across subsequent
	// leaseholders for a range increases monotonically.
	//
	// Is set, written_timestamp < timestamp.
	//
	// If unset, the intent has not been pushed by a conflicting transaction since
	// it was last written by its own transaction, so this is already represented
	// by the timestamp field. Morally, written_timestamp == timestamp.
	//
	// Empty for MVCCMetadata structs that do not represent intents.
	WrittenTimestamp *hlc.Timestamp `protobuf:"bytes,10,opt,name=written_timestamp,json=writtenTimestamp" json:"written_timestamp,omitempty"`
}
...
var fileDescriptor_324ef2186f146e22 = []byte{
	...
	,
}
...
func (m *MVCCMetadata) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	var l int
	_ = l
	if m.WrittenTimestamp != nil {
		{
			size, err := m.WrittenTimestamp.MarshalToSizedBuffer(dAtA[:i])
			if err != nil {
				return 0, err
			}
			i -= size
...
func NewPopulatedMVCCMetadata(r randyMvcc, easy bool) *MVCCMetadata {
	...
	if r.Intn(5) != 0 {
		this.WrittenTimestamp = hlc.NewPopulatedTimestamp(r, easy)
	}
	if !easy && r.Intn(10) != 0 {
	}
	return this
...
func (m *MVCCMetadata) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 10:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field WrittenTimestamp", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if m.WrittenTimestamp == nil {
				m.WrittenTimestamp = &hlc.Timestamp{}
			}
			if err := m.WrittenTimestamp.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		default:
