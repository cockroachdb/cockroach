...
type SimpleMVCCIterator interface {
	...
	// point, if will be surfaced before any point keys except when seeking
	// directly to a specific version that exists.
	SeekGE(key MVCCKey)
	...
	//
	// If range keys are enabled, range and point keys are treated separately. For
	// example, with point keys a@1,b@1,c@1 and range key [b-d)@2, calling
	// NextKey() repeatedly will yield a@1,[b-d)@2,b@1,c@1.
	NextKey()
	...
	// RangeBounds returns the range bounds for the current range key fragment, or
	// (nil, nil) if there is none. See RangeKeys() for more info on range keys.
	RangeBounds() (roachpb.Key, roachpb.Key)
	// RangeKeys returns all range key fragments (for different timestamps) at the
	// current key position, or an empty list if there are none. When at a point
	// key, it will return all range key fragments (at any timestamp) overlapping
	// that point key.
	//
	// For defragmented iteration, use MVCCRangeKeyIterator instead. Fragmented
	// iteration is primarily useful in two cases: point key iteration while
	// accessing overlapping range keys (e.g. to see if it is covered by a range
	// tombstone), and partial iteration with later resumption (e.g. export
	// requests with byte limits that must return point+range key data for a
	// completed key span and then resume from there in the next request).
	//
	...
	//
	// Fragmentation can be non-deterministic, since Pebble may merge fragments
	// during compaction. The fragment structure also depends on the write
	// history, since a past range key that caused fragmentation and was since
	// cleared would result in a different fragment structure than if it had
	// never existed in the first place.
	//
	// Range keys will also split and merge along with CRDB ranges, can be
	// partially removed by GC, and may be truncated by iterator bounds.
	//
...
const (
	...
	// IterKeyTypePointsWithRanges iterates over point keys only, but surfaces
...
type Writer interface {
	...
	// ExperimentalPutMVCCRangeKey writes a value to an MVCC range key. It will
	// replace any existing keys, or any segments that it overlaps. This is
	...
	// via special iterator options and methods such as IterOptions.KeyTypes,
	...
	// key continuum. They can be fragmented by Pebble, split/merged along with
	// CRDB ranges, partially removed or replaced by other mutations, and
	// truncated during bounded iteration.
	//
	// TODO(erikgrinaker): Write a tech note on range keys and link it here.
	//
	// This method is EXPERIMENTAL: range keys are under active development, and
