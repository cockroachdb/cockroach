...
type Engine interface {
	...
	// database across multiple iterators. If a caller only needs consistent
	...
	// space.
	//
	// A snapshot will not prohibt reading data outside of the snapshotted key
	// ranges, but the data visible outside the snapshotted key ranges is not
	// guaranteed to be consistent between iterators created from the same
	// snapshot. Callers should take care to constrain their reads to the
	// snapshotted key ranges, setting appropriate iterator bounds and seek
	// keys.
	//
