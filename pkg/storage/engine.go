...
type SimpleMVCCIterator interface {
	...
	// point, it will be surfaced before any point keys except when seeking
...
type MVCCIterator interface {
	...
	// deleted intents. If range keys are enabled and a range key exists across
	// the intent, the iterator will skips ahead to the point key.
