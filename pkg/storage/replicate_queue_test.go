...
func TestReplicateQueueDownReplicate(t *testing.T) {
	...
	for _, server := range tc.Servers {
		...
		testutils.SucceedsSoon(t, func() error {
			...
			// This first error occurs when the node already has a replica for
			// this range.
			if testutils.IsError(err, "unable to add replica .* which is already present") {
				return nil
			}
			// This second error occurs when a replica for this range was
			// previously present on this store, it has already been removed
			// but was not yet GCed.
			if testutils.IsError(err, storage.IntersectingSnapshotMsg) {
				return nil
			}
			return err
		})
