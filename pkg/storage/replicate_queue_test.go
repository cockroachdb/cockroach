...
// using the replicate queue, and to ensure that's the case, the test cluster
// needs to be kept in auto replication mode. This complicates everything. Every
// time a new replica is added, there's a very good chance that another one is
// removed. So all the replicas can't be added at once and instead need to be
// added one at a time.
func TestReplicateQueueDownReplicate(t *testing.T) {
	...
	// This process must be done one at a time, ensuring that the replica did
	// indeed make it to the desired target, which testCluster.AddReplica()
	// ensures.
	for _, server := range tc.Servers {
		...
		// retry is in order.
		testutils.SucceedsSoon(t, func() error {
			if _, err := tc.AddReplicas(testKey, roachpb.ReplicationTarget{
				NodeID:  nodeID,
				StoreID: server.GetFirstStoreID(),
			}); err != nil &&
				// This error occurs when the node already has a replica for
				// this range.
				!testutils.IsError(err, "which is already present") &&
				// There's a chance that the replica has already been removed
				// but not GCed yet. This is the expected error for that case.
				!testutils.IsError(err, "snapshot intersects with existing range") {
				return err
			}
			return nil
		})
