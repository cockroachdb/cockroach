...
func (c candidate) compare(o candidate) float64 {
	...
	if math.Abs(c.balanceScore.totalScore()-o.balanceScore.totalScore()) > epsilon {
...
func (c byScoreAndID) Less(i, j int) bool {
	if math.Abs(c[i].diversityScore-c[j].diversityScore) < epsilon &&
		c[i].convergesScore == c[j].convergesScore &&
		math.Abs(c[i].balanceScore.totalScore()-c[j].balanceScore.totalScore()) < epsilon &&
...
func (cl candidateList) best() candidateList {
	...
	for i := 1; i < len(cl); i++ {
		if cl[i].necessary == cl[0].necessary &&
			math.Abs(cl[i].diversityScore-cl[0].diversityScore) < epsilon &&
...
func (cl candidateList) worst() candidateList {
	...
	for i := len(cl) - 2; i >= 0; i-- {
		if cl[i].necessary == cl[len(cl)-1].necessary &&
			math.Abs(cl[i].diversityScore-cl[len(cl)-1].diversityScore) < epsilon &&
...
func betterRebalanceTarget(target1, existing1, target2, existing2 *candidate) *candidate {
	...
	if math.Abs(comp1-comp2) > epsilon {
		if comp1 > comp2 {
			return target1
		}
		if comp1 < comp2 {
