...
// dropped.
func (bq *baseQueue) MaybeAdd(repl *Replica, now hlc.Timestamp, qs storagebase.QueueState) {
	// Skip this queue if the replica doesn't need to be processed yet.
	if bq.needsLease && !shouldQueueAgain(now, qs.GetLastProcessed(bq.name), bq.minInterval) {
		return
	}
...
func (bq *baseQueue) processReplica(
	...
	if bq.needsLease {
		log.Infof(ctx, "setting last processed time %s for queue %s", now, bq.name)
