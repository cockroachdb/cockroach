...
//
// Replicas are typically added asynchronously through `MaybeAddAsync` or
// `AddAsync`, though this can also be done synchronously though `maybeAdd` or
// `addInternal`. The "maybe" variants check the various requirements selected
// by the queue config (needsSystemConfig, needsLease, acceptsUnsplitRanges) as
// well as the queueImpl's `shouldQueue`. The non-"maybe" variants do not check
// any of this and accept a priority directly instead of getting it from
// `shouldQueue`. The "Async" variants run with shared a maximum concurrency of
// `addOrMaybeAddSemSize`. If the maximum concurrency is reached, MaybeAddAsync
// will silently drop the replica but AddAsync will block.
//
