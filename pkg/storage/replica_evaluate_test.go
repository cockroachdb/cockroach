...
func TestEvaluateBatch(t *testing.T) {
	...
	tcs = append(tcs, testCase{
		...
		check: func(t *testing.T, r resp) {
			verifyScanResult(t, r, []string{"a"}, nil /* get */, nil)
		...
		check: func(t *testing.T, r resp) {
			verifyScanResult(t, r, []string{"e"}, nil /* get */, nil)
		...
		check: func(t *testing.T, r resp) {
			verifyScanResult(t, r, []string{"a", "b"}, nil /* get */, []string{"c"})
		...
		check: func(t *testing.T, r resp) {
			verifyScanResult(t, r, []string{"d", "c"}, nil /* get */, []string{"b"})
...
func runEvalBatchTest(t *testing.T, tc testCase) {
...
func verifyScanResult(t *testing.T, r resp, keysPerResp ...[]string) {
	...
	for i, keys := range keysPerResp {
		...
		if rows == nil {
			// Current response is not a scan. Make sure we're asked to make
			// sure there are no results (indicating that the caller intends
			// for us to skip this request).
			require.Nil(t, keys)
			continue
		}
