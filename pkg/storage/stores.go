...
func SynthesizeClusterVersionFromEngines(
	...
	for _, eng := range engines {
		...
		if cv.Version.Less(minUseVersion.Version) {
			minUseVersion.Version = cv.Version
	...
	// to use.
	if minUseVersion.Version.Less(minSupportedVersion) {
		return cluster.ClusterVersion{}, errors.Errorf("store %s, last used with cockroach version v%s, "+
			...
			minUseVersion.origin, minUseVersion.Version, serverVersion, minSupportedVersion)
	}
