...
func (ls *Stores) ReadBootstrapInfo(bi *gossip.BootstrapInfo) error {
	...
	log.StartupInfof(ctx, "read %d node addresses from persistent storage", len(bi.Addresses))
...
func (ls *Stores) WriteBootstrapInfo(bi *gossip.BootstrapInfo) error {
	...
	ctx := ls.AnnotateCtx(context.TODO())
	log.StartupInfof(ctx, "wrote %d node addresses to persistent storage", len(bi.Addresses))
	return nil
}
