...
// TestTxnRecoveryFromStaging tests the recover process for a transaction that
...
func TestTxnRecoveryFromStaging(t *testing.T) {
	...
	testutils.RunTrueAndFalse(t, "commit", func(t *testing.T, commit bool) {
		...
		txnRecord.InFlightWrites = map[int32]int32{1: 0, 2: 1}
		...
		gArgs := getArgs(keyA)
		reply, pErr := client.SendWrapped(ctx, store.TestSender(), &gArgs)
		if pErr != nil {
			t.Fatal(pErr)
		}
		if commit {
