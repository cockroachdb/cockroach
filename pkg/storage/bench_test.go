...
func runCheckSSTConflicts(
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, err := CheckSSTConflicts(context.Background(), sstFile.Data(), eng, sstStart, sstEnd, sstStart.Key, sstEnd.Key.Next(), 0, false, hlc.Timestamp{}, math.MaxInt64, usePrefixSeek)
		require.NoError(b, err)
	}
}
