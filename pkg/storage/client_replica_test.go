...
//
//   1) In reponse to a ChangeReplicasTrigger which removes it.
//   2) In response to a ReplicaTooOldError from a sent raft message
//   3) Due to the replicaGCQueue
//      * sub case is when this detects a merge.
//   4) Due to a raft message addressed to a newer replica ID.
//      * sub case when the older replica is not initialized.
//   5) Due to a merge
//   6) Due to subsumption
//
