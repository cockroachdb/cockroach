...
func TestRangeTransferLease(t *testing.T) {
	...
	sendRead := func(i int) *roachpb.Error {
		_, pErr := client.SendWrappedWith(
			context.Background(),
			mtc.senders[i],
			roachpb.Header{RangeID: rangeID, Replica: replicaDescs[i]},
			getArgs(leftKey),
		)
		return pErr
	}
	...
	checkHasLease := func(t *testing.T, i int) {
		t.Helper()
		testutils.SucceedsSoon(t, func() error {
			return sendRead(i).GoError()
		})
	}
