...
func TestReplicaTombstone(t *testing.T) {
	...
	t.Run("ReplicaTooOldError", func(t *testing.T) {
		...
		part, err := setupPartitionedRange(mtc, rangeID, 0, 2, true, dropFuncs{
			dropResp: func(resp *storage.RaftMessageResponse) bool {
				if resp.ToReplica.StoreID == 3 {
		...
		// Write a value to ensure tha the remaining replicas have processed the
		// removal.
		args := incrementArgs(keyA, 1)
		_, pErr := client.SendWrappedWith(context.Background(), mtc.Store(0).TestSender(),
			roachpb.Header{RangeID: rangeID}, args)
		require.Nil(t, pErr)
		mtc.waitForValues(keyA, []int64{1, 1, 0})

		// Advance the clock to expire all leases.
		mtc.advanceClock(context.TODO())

		// Try to send a request to the partitioned range so that it sends raft
		// messages when it attempts to campaign.
		_, pErr = client.SendWrappedWith(context.Background(), mtc.Store(2).TestSender(),
			roachpb.Header{RangeID: rangeID}, args)
		// The replica may return NotLeaseHolderError or may have already discovered
		// that it's been removed.
		require.True(t, testutils.IsPError(pErr, "NotLeaseHolderError|was not found on s3"), pErr)
		// Wait until we're sure that the replica has seen ReplicaTooOld.
		<-sawTooOld
		// We want to make sure that we actually see
