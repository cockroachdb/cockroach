...
func setupLeaseTransferTest(t *testing.T) *leaseTransferTest {
	...
	if pErr := l.sendRead(l.mtc.senders[0]); pErr != nil {
		t.Fatal(pErr)
	}
	return l
}

func (l *leaseTransferTest) sendRead(sender *storage.Stores) *roachpb.Error {
	_, pErr := client.SendWrappedWith(
		context.Background(),
		sender,
		roachpb.Header{Replica: l.replica0Desc},
		getArgs(l.leftKey),
	)
	return pErr
}
...
func (l *leaseTransferTest) checkHasLease(t *testing.T, sender *storage.Stores) {
	t.Helper()
	testutils.SucceedsSoon(t, func() error {
		return l.sendRead(sender).GoError()
	})
}
...
func (l *leaseTransferTest) forceLeaseExtension(sender *storage.Stores, lease roachpb.Lease) error {
	shouldRenewTS := lease.Expiration.Add(-1, 0)
	l.mtc.manualClock.Set(shouldRenewTS.WallTime + 1)
	err := l.sendRead(sender).GoError()
	if err != nil {
...
func (l *leaseTransferTest) ensureLeaderAndRaftState(t *testing.T, leader *storage.Replica, follower roachpb.ReplicaDescriptor) {
	t.Helper()
...
func TestRangeTransferLeaseExpirationBased(t *testing.T) {
	...
	t.Run("Transfer", func(t *testing.T) {
		...
		pErr := l.sendRead(l.mtc.senders[0])
		...
		l.checkHasLease(t, l.mtc.senders[1])
	...
	t.Run("TransferWithExtension", func(t *testing.T) {
		...
		go func() {
			...
			renewalErrCh <- l.forceLeaseExtension(l.mtc.senders[1], lease)
		}()
		...
		extensionSem <- struct{}{}
		l.checkHasLease(t, l.mtc.senders[0])
