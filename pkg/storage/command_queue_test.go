...
func BenchmarkCommandQueueReadWriteMix(b *testing.B) {
	...
	// of reads per write.
	// See #15544
	for _, reads_per_write := range []int{1, 4, 16, 64, 128, 256} {
		b.Run(fmt.Sprintf("%d", reads_per_write), func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				total_cmds := 1 << 10
				live_cmd_queue := make(chan *cmd, 16)
				cq := NewCommandQueue( /*coveringOptimization=*/ true)
				for j := 0; j < total_cmds; j++ {
					...
					var cmd *cmd
					if j%(reads_per_write+1) == 0 {
						_ = cq.getWait( /*readOnly=*/ false, zeroTS, spans)
						cmd = cq.add( /*readOnly=*/ false, zeroTS, spans)
					} else {
						_ = cq.getWait( /*readOnly=*/ true, zeroTS, spans)
						cmd = cq.add( /*readOnly=*/ true, zeroTS, spans)
					}
					if len(live_cmd_queue) == cap(live_cmd_queue) {
						cq.remove(<-live_cmd_queue)
					}
					live_cmd_queue <- cmd
				}
			}
