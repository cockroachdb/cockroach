...
//
// txn_begin      t=<name> [ts=<int>[,<int>]] [maxTs=<int>[,<int>]] [obsMaxTs=<int>[,<int>]]
// txn_remove     t=<name>
// txn_restart    t=<name>
...
func cmdTxnBegin(e *evalCtx) error {
	...
	obsMaxTs := e.getTsWithName(nil, "obsMaxTs")
	key := roachpb.KeyMin
	if e.hasArg("k") {
		key = e.getKey()
	}
	txn, err := e.newTxn(txnName, ts, maxTs, obsMaxTs, key)
	e.results.txn = txn
	return err
}
...
func (e *evalCtx) getTsWithName(txn *roachpb.Transaction, name string) hlc.Timestamp {
	...
	var tsS string
	e.scanArg(name, &tsS)
	if !strings.Contains(tsS, ",") {
		// Append logical, if not specified.
		tsS = tsS + ",0"
	}
...
func (e *evalCtx) newTxn(
	txnName string, ts, maxTs, obsMaxTs hlc.Timestamp, key roachpb.Key,
