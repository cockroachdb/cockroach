...
// identified by newRangeID. It takes care to only copying records that are
// required: certain workloads create sizable abort spans, and repeated
// splitting can blow them up further. Once it reaches approximately the Raft
// MaxCommandSize, splits become impossible, which is pretty bad (see #25233).
func (sc *AbortSpan) CopyTo(
	...
	var scratch [64]byte
	if err := sc.Iterate(ctx, e, func(k roachpb.Key, entry roachpb.AbortSpanEntry) error {
		...
		return engine.MVCCPutProto(ctx, e, ms,
