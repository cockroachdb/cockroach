	...
	datadriven.RunTest(t, "testdata/lock_table", func(t *testing.T, d *datadriven.TestData) string {
		...
		case "acquire":
		  ...
		  if s[0] == 'r' {
			...
			return lt.acquireLock(roachpb.Key(key), Exclusive, durability, g).Error()
		...
		case "release":
			...
			return lt.releaseLocks(&txnMeta.ID, span, ss).Error()
		...
		case "update":
			...
			txnMeta.WriteTimestamp = ts
			span, ss := getSpanAndScope(t, d)
			return lt.updateLocks(txnMeta, ts, span, ss).Error()
		...
		case "add-discovered":
			...
			return lt.addDiscoveredLock(roachpb.Key(key), txnMeta, txnMeta.WriteTimestamp, g, sa).Error()
