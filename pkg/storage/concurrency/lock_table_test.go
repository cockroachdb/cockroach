...
func doWork(item *workItem, e *workloadExecutor) {
	defer func() {
		e.doneWork <- item
	}()
	if item.request != nil {
			...
			for !aborted {
				...
				if state.stateKind == doneWaiting {
					if !lastID.Equal(uuid.UUID{}) && item.request.tM != nil {
						e.waitingFor(item.request.tM.ID, lastID, uuid.UUID{})
					}
					break
				}
				if state.stateKind == waitSelf {
					if item.request.tM == nil {
						panic("non-transactional request cannot waitSelf")
					}
				} else if item.request.tM != nil {
					aborted = e.waitingFor(item.request.tM.ID, lastID, state.txn.ID)
					if !aborted {
						lastID = state.txn.ID
					}
				}
		...
		for _, k := range item.locksToAcquire {
			e.acquireLock(k, g, item.request.tM.ID)
		}
		e.lt.done(g)
		e.lm.Release(lg)
		return
...
func (e *workloadExecutor) acquireLock(k roachpb.Key, g requestGuard, txnID uuid.UUID) {
	...
	if !ok {
		panic(fmt.Errorf("testbug: lock acquiring request with txnID %v has no transaction", txnID))
	}
...
func (e *workloadExecutor) findCycle(node *uuid.UUID, cycleNode *uuid.UUID) bool {
	if node.Equal(*cycleNode) {
		return true
	}
	tstate, ok := e.transactions[*node]
	if !ok {
		panic("edge to txn that is not in map")
	}
	for k, _ := range tstate.dependsOn {
		if e.findCycle(&k, cycleNode) {
			return true
		}
	}
	return false
...
func (e *workloadExecutor) waitingFor(waiter uuid.UUID, lastID uuid.UUID, currID uuid.UUID) bool {
	...
	if !ok {
		panic(fmt.Errorf("testbug: request calling waitingFor with txnID %v has no transaction", waiter))
	}
	if !lastID.Equal(uuid.UUID{}) {
		...
		if refcount > 0 {
			...
			panic(fmt.Errorf("testbug: txn %v has a negative refcount %d for dependency on %v", waiter, refcount, lastID))
		}
	}
	if !currID.Equal(uuid.UUID{}) {
		...
		if refcount == 0 && e.findCycle(&currID, &waiter) {
			return true
		}
		refcount++
...
func (e *workloadExecutor) execute(strict bool) {
	numOutstanding := 0
	i := 0
	...
	for i < len(e.items) || numOutstanding > 0 {
		if numOutstanding >= e.concurrency || (i == len(e.items) && numOutstanding > 0) {
			...
			if w != nil {
				...
				if !txnID.Equal(uuid.UUID{}) {
					tstate, ok := e.transactions[txnID]
					if !ok {
						panic(fmt.Errorf("testbug: finished request with txnID %v has no transaction", txnID))
					}
					delete(tstate.ongoingRequests, w)
					if e.tryFinishTxn(txnID, tstate) {
						numOutstanding++
						continue
					}
				}
		...
		if !ok {
			panic(fmt.Errorf("testbug: txn to finish not found: %v", wi.finish))
		}
		tstate.finish = true
		if e.tryFinishTxn(wi.finish, tstate) {
			numOutstanding++
		}
	}
	fmt.Printf("items: %d, aborted: %d, concurrency violations: %d, lock table: %v\n",
		len(e.items), e.numAborted, e.numConccViolations, e.lt)
}
...
func TestLockTableConcurrentSingleRequests(t *testing.T) {
	...
	for i := 0; i < numRequests; i++ {
		...
		for i := 0; i < numKeys; i++ {
			...
			acc := accesses[rng.Intn(len(accesses))]
