...
func (w *lockTableWaiterImpl) pushRequestTxn(
	...
	// Regardless of whether we're reading or writing, we always perform a
	// PUSH_ABORT when pushing a conflicting request because we want to
	// block until either a) either the pushee or the pusher is aborted due
	// to a deadlock or b) the request exits the lock wait-queue and the
	// caller of this function cancels the push.
	pushType := roachpb.PUSH_ABORT
