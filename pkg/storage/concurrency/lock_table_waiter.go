...
type intentResolver interface {
	// PushTransaction pushes the provided transaction.
	PushTransaction(
		context.Context, *enginepb.TxnMeta, roachpb.Header, roachpb.PushTxnType,
	) (roachpb.Transaction, *Error)

	// ResolveIntents resolves the provided intents according to the options.
	ResolveIntents(context.Context, []roachpb.Intent, intentresolver.ResolveOptions) *Error
}
	...
	for {
		...
		case <-newStateC:
			...
			case waitElsewhere:
				...
				// immediately to wait in that transaction's txnWaitQueue.
				if err := w.pushTxn(ctx, req, state); err != nil {
					return err
				}

			case waitSelf:
				// The request's own transaction acquired the lock it was waiting
		...
		case <-timerC:
			...
			timerC = nil
			if err := w.pushTxn(ctx, req, timerState); err != nil {
				return err
			}
...
func (w *lockTableWaiterImpl) pushTxn(ctx context.Context, req Request, ws waitingState) *Error {
	...
	resolveIntent[0].SetTxn(&pusheeTxn)
	opts := intentresolver.ResolveOptions{Wait: false, Poison: true}
	return w.ir.ResolveIntents(ctx, resolveIntent, opts)
}

func hasExtremePriority(txn *roachpb.Transaction) bool {
	p := txn.Priority
