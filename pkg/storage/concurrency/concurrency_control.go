...
// head of and releases its latches. However, as long as the request was
// successful, any locks that it inserted into the lockTable remain.
...
type lockTable interface {
	...
	// Dequeue removes the guard from its lock wait-queue. The guard is not
	// guaranteed to be at the front of hte wait-queue.
