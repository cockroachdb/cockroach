...
type contentionHandler interface {
	...
	// pushed. After doing so, it releases the guard's latches. It returns an
...
// record stored on the manager's range.
...
type lockTable interface {
	...
	//
	// It is only permitted for requests that have a non-nil TxnMeta. It must
	// only be called in the evaluation phase before calling Dequeue, which
	// means all the latches needed by the request are held. The key must be in
	// the request's SpanSet with the appropriate SpanAccess: currently the
	// strength is always Exclusive, so the span containing this key must be
	// SpanReadWrite. This contract ensures that the lock is not held in a
	// conflicting manner by a different transaction. Acquiring a lock that is
	// already held by this transaction upgrades the lock's timestamp and
...
// for the pushee transaction to commit or abort.
//
...
//
// The following diagream demonstrates how the txnWaitQueue interacts with
