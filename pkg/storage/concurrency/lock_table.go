...
func (g *lockTableGuardImpl) findNextLockAfter(notify bool) {
	...
	for span != nil {
		...
		for iter.FirstOverlap(&lockState{key: startKey, endKey: span.EndKey}); iter.Valid(); iter.NextOverlap() {
			l := iter.Cur()
			if resumingInSameSpan {
				resumingInSameSpan = false
...
func (t *lockTableImpl) UpdateLocks(intent *roachpb.Intent) error {
	...
	tree.mu.RLock()
	iter := tree.MakeIter()
	for iter.FirstOverlap(&lockState{key: span.Key, endKey: span.EndKey}); iter.Valid(); iter.NextOverlap() {
		changeFunc(iter.Cur())
