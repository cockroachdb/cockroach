...
type sortLocks []*lockState

func (sl sortLocks) Len() int           { return len(sl) }
func (sl sortLocks) Swap(i, j int)      { sl[i], sl[j] = sl[j], sl[i] }
func (sl sortLocks) Less(i, j int) bool { return sl[i].key.Compare(sl[j].key) < 0 }
...
func (t *lockTableImpl) Dequeue(guard lockTableGuard) {
	...
	for i := 0; i < len(locksToGC); i++ {
		if len(locksToGC[i]) > 0 {
			sort.Sort(sortLocks(locksToGC[i]))
...
func (t *lockTableImpl) tryGCLocks(tree *treeMu, locks []*lockState) {
	...
	for _, l := range locks {
		...
		if !iter.Valid() || iter.Cur().id != l.id {
			continue
		}
		l = iter.Cur()
		l.mu.Lock()
...
func (t *lockTableImpl) UpdateLocks(intent *roachpb.Intent) error {
	...
	tree.mu.RLock()
	iter := tree.MakeIter()
	iter.SeekGE(&lockState{key: span.Key})
	for ; iter.Valid() && ((len(span.EndKey) == 0 && iter.Cur().key.Equal(span.Key)) ||
		(len(span.EndKey) > 0 && iter.Cur().key.Compare(span.EndKey) < 0)); iter.Next() {
		changeFunc(iter.Cur())
