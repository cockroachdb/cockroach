...
// that a read and a conflicting lock do not happen concurrently. The lock table
// provides both locking and sequencing of requests (in concert with the use of
// latches, as specified below).
//
...
type lockState struct {
	...
	// the review for #43740. Additionally, for reasons discussed there, reads
	// do not queue for their turn at all -- they are held in the waitingReaders
	// list while the lock is held and removed when the lock is not released, so
	// they race with reservations.
	//
...
// In increasing order of Start, and for equal Start in decreasing order of
// End.
