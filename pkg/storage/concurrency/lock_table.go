...
// guardState implements the lockTableGuardState interface.
func (*waitingState) guardState() {}
...
func (g *lockTableGuardImpl) CurState() lockTableGuardState {
	g.mu.Lock()
	defer g.mu.Unlock()
...
func (l *lockState) acquireLock(
	txn *enginepb.TxnMeta, _ lock.Strength, durability lock.Durability,
) (doneWaiting []*lockTableGuardImpl, err error) {
	l.mu.Lock()
	defer l.mu.Unlock()
	if l.holder.locked {
		// Already held.
		updateTxn, updateTs := txn, txn.WriteTimestamp
	...
	l.holder.holder[durability].ts = txn.WriteTimestamp
...
func (t *lockTableImpl) AcquireLock(
	...
	doneWaiting, err := l.acquireLock(txn, strength, durability)
	tree.mu.Unlock()
	if err != nil {
		return err
	}
