...
// of one or more requests. Isolation id needed across requests. Additionally,
// since transactions represent a group of requests, isolation is needed across
// such groups. Part of this isolation is accomplished by maintaining multiple versions
// and part by allowing requests to acquire locks. Even the isolation
// based on multiple versions requires some form of mutual exclusion to ensure
// that a read and a conflicting lock do not happen concurrently. The
// lock table provides both locking and sequencing of requests (in concert with
// the use of latches, as specified below).
//
// Locks outlive the requests themselves and thereby extend the duration of
...
// in the MVCC keyspace, so they are often not detectable until request evaluation
// time. To accommodate this form of lock storage, the lock table exposes an
// addDiscoveredLock method. In the future, we intend to pull all locks, including
// those associated with write intents, into the lock table directly.
//
// The lock table also provides fairness between requests. If two requests conflict
// then the request that arrived first will typically be sequenced first. There are
// some exceptions:
// - a request that is part of a transaction which has already acquired a
//   lock does not need to wait on that lock during sequencing, and can therefore
//   ignore any queue that has formed on the lock.
// - contending requests that encounter different levels of contention may
//   be sequenced in non-FIFO order. This is to allow for more concurrency.
//   e.g. if request R1 and R2 contend on key K2, but R1 is also waiting at key K1,
...
type Request interface {
	...
	// A key must not appear in more than one span, i.e., it must be either SpanReadOnly or
	// SpanReadWrite but not both.
...
//   waitForDistinguished state is a sub-case -- a distinguished waiter is responsible for taking
//   extra actions e.g. immediately pushing the transaction it is waiting for. The implementation
//   ensures that if there are multiple requests in waitFor state waiting on the same transaction
//   at least one will be a distinguished waiter.
...
//   considered ok: the one exception we will need to make is if T has the min priority or the
//   waiting transaction has max priority -- in both cases it will push immediately. The bad case is
//   if T is ABORTED: the push will succeed after, and if T left N intents, each push would wait for
//   50ms, incurring a latency of 50*N ms. A cache of recently encountered ABORTED transactions on
//   each Store should mitigate this latency increase. Whenever a transaction sees a waitFor state,
//   it will consult this cache and if T is found, push immediately (if there isn't already a
//   push in-flight) -- even if T is not initially in the cache, the first push will place it in
//   the cache, so the maximum latency increase is 50ms.
//
...
//
// - The waitSelf state is a rare state when a different request from the same transaction has a
//   reservation. See the comment about "Reservations" in lockState.
type requestGuard interface {
	// startWaiting must be called after each scanAndEnqueue(). The request should proceed to evaluation
...
// Concurrency: in addition to holding latches, we require for a particular request
// scanAndEnqueue() and currentState() must be called by the same thread.
//
// Recommended usage:
//
// Request evaluation:
...
type lockTable interface {
	...
	// which means all the latches needed by the request are held. The key must be in the request's
	...
	// occur on transaction.
	//
	// For spans containing Replicated locks, this must be called after intent resolution has been
	...
	//
	// Note that spans can be wider than the actual keys on which locks were acquired,
	// and it is ok if no locks are found or locks held by other transactions are found (for those
	// lock this call is a noop).
	//
	...
	//   acquire latches using the oldest timestamp at which the intent could have been written: if the
	//   intent was at ts=5 and the intent resolution is using ts=10 (since the transaction has been pushed),
	...
	// acquiring its own locks, since the request needs to repeat scanAndEnqueue().
	//
	// A latch consistent with the access desired by guard and guard.ts() must be held on the span
	// containing key.
	addDiscoveredLock(
...
type lockTableImpl struct {
	...
	// while they wait for on contended locks after releasing their latches.
	...
	// - req1 (from txn1) arrives at lockTable and wants to write to A and B. It queues at B.
	// - req2 (from txn2) arrives at lockTable and only wants to write A. It proceeds to evaluation
	//   and acquires the lock at A for txn2 and then the request is done. The lock is still held.
	// - req3 (from txn3) wants to write to A and B. It queues at A.
	// - txn2 releases A. req3 is in the front of the queue at A and gets the reservation and starts
	//   waiting at B behind req1.
	// - txn0 releases B. req1 gets the reservation at B and does another scan and adds itself to the
	//   queue at A, behind req3 which holds the reservation at A.
	// Now in the queues for A and B req1 is behind req3 and vice versa and this deadlock has been
	// created entirely due to the lock table's behavior.
...
type requestGuardImpl struct {
	...
	// A request whose startWait is set to true in scanAndEnqueue is actively waiting at a particular
	// key. This is the first key encountered when iterating through spans that it needs to wait at.
	// A future event (lock release etc.) may cause the request to no longer need to wait at this
	// key. It then needs to continue iterating through spans to find the next key to wait at (we
	// don't want to wastefully start at the beginning since this request probably has a reservation
	// at the contended keys there): sa, ss, index, key collectively track the current position
	// to allow it to continue iterating.

	// The key for the lockState.
	key roachpb.Key
	// The key for the lockState is contained in the Span specified by spans[sa][ss][index].
...
// Waiting writers in a lockState are wrapped in a queuedGuard. A waiting writer is typically
// waiting in an active state, i.e., the requestGuardImpl.key refers to this lockState. However,
// breaking of reservations (see the comment on reservations below, in lockState) can cause a
// writer to be an inactive waiter.
...
type lockHolderInfo struct {
	// nil if there is no holder. Else this is the TxnMeta of the latest call to acquire/update the
	// lock by this transaction. For a given transaction if the lock is continuously held by a
	// succession of different TxnMetas, the epoch must be monotonic and the ts (derived from
	// txn.WriteTimestamp for some calls, and request.ts for other calls) must be monotonic.
	txn *enginepb.TxnMeta
	// All the TxnSeqs in the current epoch at which this lock has been acquired. In increasing
	// order.
	// We track these so that if a lock is acquired at both seq 5 and seq 7, rollback of 7 does not
	// cause the lock to be released. This is also consistent with PostgreSQL semantics
	// https://www.postgresql.org/docs/12/sql-select.html#SQL-FOR-UPDATE-SHARE
	seqs []enginepb.TxnSeq
	// The timestamp at which the lock is held. If the transaction
	ts hlc.Timestamp
}

// Per lock state in lockTableImpl.
type lockState struct {
	// The key being locked and the scope of that key. These state is never mutated.
	key roachpb.Key
	ss  spanset.SpanScope
	...
	// Information about whether the lock is held and the holder. We track information for each
	// durability level separately since a transaction can go through multiple epochs and TxnSeq
	// and may acquire the same lock in replicated and unreplicated mode at different stages.
	holder struct {
		locked bool
	...
	// jointly hold the reservation introduces code complexity since joint reservations can be
	// partially broken (see deadlock example below), and is not necessarily fair to other requests.
	// Additionally, if req1 from txn1 is holding a a reservation and req2 from txn1 is waiting,
	// they must conflict wrt latches and cannot evaluate concurrently so there isn't a benefit to
	// joint reservations. However, if one of the requests acquires the lock the other request no
	// longer needs to wait on this lock. This situation motivates the waitSelf state.
	//
	// Deadlock example if joint reservations were supported and we did not allow partial breaking
	// of such reservations:
	// - Keys are A, B, C, D.
	// - Key D is locked by some random txn.
	// - req1 from txn1 writes A, B, D. It waits at D.
	// - Some other request from some random txn that writes C arrives, evaluates, and locks C.
	// - req2 from txn2 that writes A, C. It waits at C.
	// - Some other request from some random txn that writes A arrives, evaluates, and locks A.
	// - req3 from txn1 that writes A, C. It waits at A. Note that req1 and req3 are from the same txn.
	// - A is unlocked. req3 reserves A and waits at C behind req2.
	// - B is locked by some random txn.
	// - D is unlocked. req1 reserves D and proceeds to scan again and finds A is reserved by req3
	//   which is the same txn so becomes a joint reservation holder at A.
	// - Since B is locked, req1 waits at B.
	// - C is unlocked. req2 reserves C. It scans and finds req1+req3 holding the joint reservation
	//   at A. If it queues behind this joint reservation we have the following situation:
	//        reservation   waiter
	//   A     req1+req3     req2
	//   C       req2        req3
	//   This is a deadlock caused by the lock table unless req2 partially breaks the reservation at A.
	reservation *requestGuardImpl

	// TODO(sbhola): There are a number of places where we iterate over these lists looking for
	// something, as described below. If some of these turn out to be inefficient, consider better
	// data-structures. One idea is that for cases that find a particular guard the
	// requestGuardImpl.locks can be a map instead of a set to point directly to the *list.Element.
	//
	// queuedWriters:
	...
	// An active waiter needs to be notified about changes in who it is waiting for.
	...
	// held. By definition these cannot be in waitSelf state since that state is only used when there
	// is a reservation.
	waitingReaders list.List

	// If there is a non-empty set of active waiters that are not waitSelf, then at least one must
	// be distinguished.
...
// Informs active waiters about reservation or lock holder. The reservation may have changed
// so this needs to fix any inconsistencies wrt waitSelf and waitForDistinguished states.
// REQUIRES: l.mu is locked.
func (l *lockState) informActiveWaiters() {
	...
	for e := l.waitingReaders.Front(); e != nil; e = e.Next() {
		// Since there are waiting readers we could not have transitioned out of or into a state
		// with a reservation, since readers do not wait for reservations.
...
// Returns true iff the lock is currently held by the transaction with the given id.
// REQUIRES: l.mu is locked.
...
// Returns information about the current lock holder if the lock is held, else returns nil.
// REQUIRES: l.mu is locked.
func (l *lockState) getLockerInfo() (*enginepb.TxnMeta, hlc.Timestamp) {
	if !l.holder.locked {
		return nil, hlc.Timestamp{}
	}
	// If the lock is held as both replicated and unreplicated we want to provide the lower of the
	// two timestamps, since the lower timestamp contends with more transactions. Else we provide
	// whichever one it is held at.
	...
	if l.holder.holder[index].txn == nil || (l.holder.holder[lock.Unreplicated].txn != nil &&
		// If we are evaluating the following clause we are sure that it is held as both replicated
		// and unreplicated.
...
// Decides whether the request g with access sa should actively wait at this lock and if yes,
// adjusts the data-structures appropriately. The notify parameter is true iff the request's
// new state channel should be notified -- it is set to false when the call to tryActiveWait is
// happening due to an event for a different request or transaction (like a lock release) since
// in that case the channel is notified first and the call to tryActiveWait() happens later in
// requestGuard.currentState().
...
func (l *lockState) tryActiveWait(g *requestGuardImpl, sa spanset.SpanAccess, notify bool) (bool, error) {
	...
	if sa == spanset.SpanReadWrite {
		if _, inQueue := g.mu.locks[l]; inQueue {
			// Already in queue and must be in the right position, so mark as active waiter there. We
...
// TODO(sbhola): the deferred call to tryActiveWait() removes the need to return these doneWaiting
// slices -- the callee can notify the channel itself. Simplify this when switching to the
// copy-on-write btree since that will cause some related code restructuring.

// Acquires this lock. Returns the list of guards that are done actively waiting at this key -- these
// will be requests from the same transaction that is acquiring the lock.
// Acquires l.mu.
func (l *lockState) acquireLock(_ lock.Strength, durability lock.Durability, g *requestGuardImpl) (doneWaiting []*requestGuardImpl, err error) {
	l.mu.Lock()
	defer l.mu.Unlock()
	if l.holder.locked {
		// Already held.
...
// A replicated lock held by txn with timestamp ts was discovered by guard g where g is trying to
// access this key with access sa.
// Acquires l.mu.
func (l *lockState) discoveredLock(
	...
	if l.reservation != nil {
		...
		// No reservation, so either the lock was already known to be held, in which case the active
		// waiters know about the holder, or it was not held and so there are no active waiters.
		informWaiters = false
	}
	...
	for e := l.queuedWriters.Front(); e != nil; {
		...
		if qg.active {
			if g.mu.state.stateKind == waitSelf {
				// We can't tell it to waitElsewhere since we haven't given it a txn to push, so it should
				// just stop waiting.
...
// Returns true iff the lockState is empty, i.e., there is no lock holder or reservation.
// REQUIRES: l.mu is locked.
...
type sortIgnored []enginepb.IgnoredSeqNumRange

func (s sortIgnored) Len() int { return len(s) }
func (s sortIgnored) Less(i, j int) bool {
...
func (s sortIgnored) Swap(i, j int) {
	s[i], s[j] = s[j], s[i]
}

func removeIgnored(heldSeqNums []enginepb.TxnSeq, ignoredSeqNums []enginepb.IgnoredSeqNumRange) []enginepb.TxnSeq {
	if len(ignoredSeqNums) == 0 {
		return heldSeqNums
	}
	sorted := sortIgnored(append([]enginepb.IgnoredSeqNumRange(nil), ignoredSeqNums...))
	sort.Sort(sorted)
	ignored := sorted[:1]
	for _, r := range sorted {
		last := ignored[len(ignored)-1]
		if r.Start <= last.End {
			if r.End > last.End {
				last.End = r.End
			}
	...
	for _, n := range heldSeqNums {
		i := sort.Search(len(ignored), func(i int) bool { return ignored[i].End >= n })
		if ignored[i].Start > n {
			held = append(held, n)
		}
	}
	return held
...
// Tries to update the lock: noop if this lock is held by a different transaction, else the lock
// is updated. Returns the list of guards that are done actively waiting at this key and whether
// the lockState can be garbage collected.
// Acquires l.mu.
func (l *lockState) tryUpdateLock(
	intent *roachpb.Intent) (doneWaiting []*requestGuardImpl, gc bool, err error) {
	l.mu.Lock()
	defer l.mu.Unlock()
	if !l.isLockedBy(intent.Txn.ID) {
		return nil, false, nil
	}
	if intent.Status == roachpb.COMMITTED || intent.Status == roachpb.ABORTED {
...
// The lock holder timestamp has increased. Returns the list of guards that are done actively
// waiting at this key.
// REQUIRES: l.mu is locked.
...
// Returns the list of guards that are done actively waiting at this key and whether the lockState
...
// The lock has transitioned from locked/reserved to unlocked. There could be waiters, but there
// cannot be a reservation.
// REQUIRES: l.mu is locked.
func (l *lockState) lockIsFree() (doneWaiting []*requestGuardImpl, gc bool, err error) {
	if l.reservation != nil {
		return nil, false, errors.Errorf("lockTable bug")
	}
	// There may not be a distinguished waiter currently because of who had the previous reservation
	// but we may be able to find one.
	findDistinguished := l.distinguishedWaiter == nil
...
func (t *lockTableImpl) acquireLock(
	key roachpb.Key, strength lock.Strength, durability lock.Durability, guard requestGuard) error {
...
// Removes all locks that have active waiters and tells those waiters to wait elsewhere.
// A replicated lock which has been discovered by a request but the request is not yet actively
// waiting on it will be preserved since we need to tell that request who it is waiting for when
// it next calls scanAndEnqueue(). If we aggressively removed even those requests, the next
// scanAndEnqueue() would not find that lock, the request would evaluate again, again discover
// that lock and if clearMostLocks() keeps getting called would be stuck in this loop without
// pushing.
...
func findAccessInSpans(key roachpb.Key, ss spanset.SpanScope, spans *spanset.SpanSet) (spanset.SpanAccess, error) {
...
func (t *lockTableImpl) tryGCLocks(tree *treeMu, locks []*lockState) error {
	...
	for _, l := range locks {
		...
		// lockState for the same key we do not want to delete it accidentally if it is non-empty.
		if i == nil {
			continue
		}
		l = i.(*lockState)
...
// Iteration helper for findNextLockAfter. Returns the next span to search over, or nil if the
// iteration is done.
...
// Finds the next lock, after the current one, to actively wait at. If it finds the next lock the
// request starts actively waiting there, else it is told that it is done waiting.
...
func (t *lockTableImpl) findNextLockAfter(g *requestGuardImpl, notify bool) error {
	...
	for span != nil {
		...
		if len(span.EndKey) == 0 {
			...
			var err error
			tree.mu.RLock()
			// From here on, the use of resumingInSameSpan is just a performance optimization to deal
			// with the interface limitation of AscendRange() that prevents us from specifying an
			// exclusive start key. We need to check that the lock is not the same as our exclusive
			// start key and only need to do that check once -- for the first lock.
			//
			// We will hold tree.mu in read mode while calling tryActiveWait() -- this is harmless.
