...
func IsDiskFull(fs vfs.FS, spec base.StoreSpec) (bool, error) {
	...
	// the disk space freed by truncation will allow us to start. If we need
	// to create or grow the ballast but are unable because there's
	// insufficient disk space, it'll be resized by the periodic capacity
	// calculations when the conditions are met.
