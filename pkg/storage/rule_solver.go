...
func rebalanceFromConvergesOnMean(sl StoreList, candidate roachpb.StoreDescriptor) bool {
	return rangesPerGB(candidate.Capacity) > sl.candidateRangesPerGB.mean+0.5
}
...
func (c candidate) String() string {
	return fmt.Sprintf("s%d, valid:%t, con:%.2f, rangesPerGB:%.2f, details:(%s)",
		c.store.StoreID, c.valid, c.constraintScore, c.rangesPerGB, c.details)
}
...
func (c candidate) less(o candidate) bool {
	...
	return c.rangesPerGB > o.rangesPerGB
}
...
func (c byScoreAndID) Less(i, j int) bool {
	if c[i].constraintScore == c[j].constraintScore &&
		c[i].rangesPerGB == c[j].rangesPerGB &&
		c[i].valid == c[j].valid {
...
func rangesPerGB(capacity roachpb.StoreCapacity) float64 {
	// Don't consider differences in store size for stores of less than 1GB -
	// this is primarily useful to avoid huge rangesPerGB numbers for unit tests
	// that use 1MB store capacities.
	capacityGB := math.Max(float64(capacity.Capacity/bytesPerGB), 1.0)
...
func allocateCandidates(
	...
	for _, s := range sl.stores {
		...
		candidates = append(candidates, candidate{
			...
			rangesPerGB:     rangesPerGB(s.Capacity),
...
func shouldRebalance(ctx context.Context, store roachpb.StoreDescriptor, sl StoreList) bool {
	...
	overfullThreshold := math.Ceil(sl.candidateRangesPerGB.mean * (1 + baseRebalanceThreshold))
	rangeCountAboveOverfullThreshold := rangesPerGB(store.Capacity) > overfullThreshold
	...
	if rangesPerGB(store.Capacity) > sl.candidateRangesPerGB.mean {
		underfullThreshold := math.Floor(sl.candidateRangesPerGB.mean * (1 - baseRebalanceThreshold))
		for _, desc := range sl.stores {
			if rangesPerGB(desc.Capacity) < underfullThreshold {
	...
	if log.V(2) && shouldRebalance {
		log.Infof(ctx,
			...
			store.StoreID, store.Capacity.FractionUsed(), store.Capacity.RangeCount, rangesPerGB(store.Capacity),
			sl.candidateRangesPerGB.mean, overfullThreshold, maxCapacityUsed,
