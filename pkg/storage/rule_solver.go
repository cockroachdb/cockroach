...
type rule struct {
	...
	// stores. The score will be weighted and then summed together with the
	// other rule scores to create a store ranking (higher is better).
...
func makeDefaultRuleSolver() ruleSolver {
	return makeRuleSolver(defaultSolverRules)
}
...
// Solve calls computeCandidate on all stores and returns a any of those stores
// that meet the constraints order by best to worst score.
...
// computeCandidate runs all the rules for the state and returns each
// candidate's score. Returns false if it is not a valid candidate.
...
// in common. It always returns true.
...
// byScore implements sort.Interface to sort by scores in a descending way.
