...
func rebalanceCandidates(
	...
	preferredByStoreID := make(map[roachpb.StoreID]int)
	preferredExistingWorst := math.MaxInt32
	preferredCandidateBest := math.MinInt32
	var rebalanceConstraintsCheck bool
	for _, s := range sl.stores {
		constraintsOk, preferredMatched := constraintCheck(s, constraints)
		constraintsOkByStoreID[s.StoreID] = constraintsOk
		preferredByStoreID[s.StoreID] = preferredMatched
		...
		if constraintsOk {
			...
			if exists && preferredMatched < preferredExistingWorst {
				...
				preferredCandidateBest = preferredMatched
			}
		} else if exists {
	...
	if preferredCandidateBest > preferredExistingWorst {
		rebalanceConstraintsCheck = true
		if log.V(2) {
			log.Infof(ctx, "attempting a rebalance to improve preferred score from %d to %d",
				preferredExistingWorst, preferredCandidateBest)
		}
	}
	...
	// Only rebalance away if the constraints don't match, the max
	// capacity check fails, there's a possible candidate with a more preferred
	// constraint or if should rebalance is true.
	...
	var candidates candidateList
	for _, s := range sl.stores {
		constraintsOk := constraintsOkByStoreID[s.StoreID]
		preferredMatched := preferredByStoreID[s.StoreID]
...
func shouldRebalance(ctx context.Context, store roachpb.StoreDescriptor, sl StoreList) bool {
	...
	shouldRebalance := maxCapacityUsed || rangeCountAboveTarget || rebalanceToUnderfullStore
	if log.V(2) && shouldRebalance {
		log.Infof(ctx,
			"s%d: should-rebalance: fraction-used=%.2f range-count=%d "+
				"(mean=%.1f, target=%d, fraction-used=%t, above-target=%t, underfull=%t)",
