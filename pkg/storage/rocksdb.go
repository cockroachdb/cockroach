...
func (r *RocksDB) Stat(name string) (os.FileInfo, error) {
	...
	// names a directory or a file, so try both.
	if _, listErr := r.List(name); listErr == nil {
		return rocksDBFileInfo{name: name, isDir: true}, nil
	}
	f, err := r.Open(name)
	if err != nil {
		return nil, err
	}
	return rocksDBFileInfo{name: name, isDir: false}, f.Close()
}

type rocksDBFileInfo struct {
	name  string
	isDir bool
}

func (fi rocksDBFileInfo) Name() string       { return filepath.Base(fi.name) }
func (fi rocksDBFileInfo) Size() int64        { return 64 }
func (fi rocksDBFileInfo) Mode() os.FileMode  { return os.ModePerm }
func (fi rocksDBFileInfo) ModTime() time.Time { return time.Time{} }
func (fi rocksDBFileInfo) IsDir() bool        { return fi.isDir }
func (fi rocksDBFileInfo) Sys() interface{}   { return nil }
