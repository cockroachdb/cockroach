...
func CalculateThreshold(now hlc.Timestamp, policy config.GCPolicy) (threshold hlc.Timestamp) {
...
func isGarbage(threshold hlc.Timestamp, cur, next *engine.MVCCKeyValue, isNewest bool) bool {
	...
	if belowThreshold := !threshold.Less(cur.Key.Timestamp); !belowThreshold {
		return false
	}
	...
	return isDelete || !threshold.Less(next.Key.Timestamp)
}
