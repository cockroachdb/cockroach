...
type cmdAppBatch struct {
	...
	// haveDecodedEntry indicates that decodeBuf was used to decode a non-trivial
	// entry that could not be processed immediately.
	haveDecodedEntry bool
	// decodeBuf is a scratch object used to decode entries before adding the
	// decoded entry to the batch in case the entry cannot be applied immediately.
...
func (b *cmdAppBatch) decode(
...
) (numEmptyEntries int, remaining []raftpb.Entry, errExpl string, err error) {
	for len(toProcess) > 0 {
		e := &toProcess[0]
		// If we haven't already decoded this entry then we decode it into the
		// batch's scratch space.
		if b.haveDecodedEntry {
			b.haveDecodedEntry = false
		} else if len(e.Data) == 0 {
			numEmptyEntries++
		} else if errExpl, err = b.decodeBuf.decode(ctx, e); err != nil {
			return numEmptyEntries, nil, errExpl, err
		}
		// Check if this entry is trivial.
		decodedIsTrivial := isTrivial(&b.decodeBuf.raftCmd.ReplicatedEvalResult,
		...
		s := b.cmdBuf.allocate()
		s.decodedRaftEntry = b.decodeBuf
		b.decodeBuf = decodedRaftEntry{}
		s.e = e
	...
	return numEmptyEntries, toProcess, "", nil
}

func (b *cmdAppBatch) reset() {
	b.cmdBuf.clear()
	*b = cmdAppBatch{
		haveDecodedEntry: b.haveDecodedEntry,
		decodeBuf:        b.decodeBuf,
	}
}
