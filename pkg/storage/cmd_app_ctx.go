...
type cmdAppCtx struct {
	...
	// that a command applies given the current lease and GC threshold. They are
	// set when transforming an apply.Command into an apply.CheckedCommand.
	leaseIndex    uint64
	...
	// storage engine in prepareLocalResult. They are set when transforming
	// an apply.CheckedCommand into an apply.AppliedCommand.
