...
func (b *ringBuf) clear(hi uint64) (bytesRemoved, entriesRemoved int32) {
...
func reallocSize(length int) int {
	if size := 2 * length; size > minBufSize {
		return size
	}
	return minBufSize
}
...
func extend(b *ringBuf, before, after int) {
	...
	if size > len(b.buf) {
		realloc(b, before, reallocSize(size))
	} else {
...
// here are inclusive. Note that before and after are counts, not indices.
// If [lo, hi] is not overlapping or directly adjacent to the current cache
// bounds, ok will be false.
