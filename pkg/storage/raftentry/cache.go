...
// the cache (Add, Clear, Get, or Scan) in order to locate the partition for the
// operation and update the LRU state. In the case of Add operations, partitions
// are lazily constructed under the lock. In addition to partition location, Add
// operations record the maximal amount of space that the write may add to the
// cache, accepting that in certain cases, less space may actually be consumed
// leading to unnecessary evictions. Once a partition has been located (or not
// found) and LRU state has been appropriately modified, operations release
// Cache.mu and proceed by operating on the partition under its RWMutex.
//
...
// and by taking care to not mutate the partition's cached date upon eviction.
...
// reality. This update (Cache.recordUpdate) is mediated the use of an atomic
// compare and swap operation on partition.size. If the operation succeeds, then
// we know that future evictions of this partition will see the new updated
// partition.size and so any delta from what was optimistically recorded in
// the Cache stats should be updated (using atomics, see add(Bytes|Entries)).
// If the operation fails, then we know that any change just made to the
// partition are no longer stored in the cache and thus the Cache stats shall
// not change.
//
