...
func (c *Cache) recordUpdate(p *partition, bytesAdded, bytesGuessed, entriesAdded int32) {
	...
	// occur when either:
	//
	//   1) a new write adds it guessed write size to p
	//   2) when p is evicted to make room for a write
	//
	...
	// The fact that while p is not evicted, p.size can only increase, adding
	// delta to p.size cannot make it negative.
