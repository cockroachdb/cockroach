...
func TestLearnerFollowerRead(t *testing.T) {
	...
	// Perform a lease transfer, which sets a closed timestamp floor which is
	// greater than the request timestamp generated above.
	//
	// TODO(dan): This shouldn't be necessary (nor should the third node and
	// replica), but for some reason I'm not seeing closed timestamp updates. It'd
	// be interesting to dig into the differences between this test and
	// TestClosedTimestampCanServe.
	scratchDesc = tc.AddReplicasOrFatal(t, scratchStartKey, tc.Target(2))
	require.NoError(t, tc.TransferRangeLease(scratchDesc, tc.Target(2)))
