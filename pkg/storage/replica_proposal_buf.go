...
// whichever happens earlier. This introduces no additional latency into the
// pipeline compared to moving them into Raft directly because Raft would not
// begin replicating the proposals until the next handleRaftReady iteration
// anyway.
//
...
func (b *propBuf) ReinsertLocked(p *ProposalData) error {
	...
	// This prevents us from waiting on the full condition variable.
