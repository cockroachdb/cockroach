...
func (b *propBuf) FlushLockedWithRaftGroup(raftGroup *raft.RawNode) error {
	...
	for i, p := range buf {
		...
		// of the proposal, we don't try to propose any more proposals. The rest
		// of the proposals will still be registered with the proposer, so they
		// will eventually be reproposed.
