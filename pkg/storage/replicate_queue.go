	...
	if lease, _ := repl.GetLease(); repl.IsLeaseValid(lease, now) {
		if rq.canTransferLease() &&
			rq.allocator.ShouldTransferLease(
				ctx, zone, desc.Replicas().Wrapped(), lease.Replica.StoreID, desc.RangeID, repl.leaseholderStats) {
...
func (rq *replicateQueue) processOneChange(
	...
	liveReplicas, deadReplicas := rq.allocator.storePool.liveAndDeadReplicas(
		desc.RangeID, desc.Replicas().Wrapped())
	{
	...
	case AllocatorAdd:
		...
		willHave := len(desc.Replicas().Wrapped()) + 1
		...
		log.VEventf(ctx, 1, "adding replica %+v due to under-replication: %s",
			newReplica, rangeRaftProgress(repl.RaftStatus(), desc.Replicas().Wrapped()))
	...
	case AllocatorRemove:
		...
		for r := retry.StartWithCtx(ctx, retryOpts); r.Next() && timeutil.Now().Before(deadline); {
			...
			candidates = filterUnremovableReplicas(raftStatus, desc.Replicas().Wrapped(), lastReplAdded)
			log.VEventf(ctx, 3, "filtered unremovable replicas from %v to get %v as candidates for removal: %s",
				desc.Replicas(), candidates, rangeRaftProgress(raftStatus, desc.Replicas().Wrapped()))
			if len(candidates) > 0 {
				break
			}
		...
		if len(candidates) == 0 {
			...
			return false, errors.Errorf("no removable replicas from range that needs a removal: %s",
				rangeRaftProgress(repl.RaftStatus(), desc.Replicas().Wrapped()))
		}
		...
		if removeReplica.StoreID == repl.store.StoreID() {
			...
			log.VEventf(ctx, 1, "removing replica %+v due to over-replication: %s",
				removeReplica, rangeRaftProgress(repl.RaftStatus(), desc.Replicas().Wrapped()))
		...
		if !rq.store.TestingKnobs().DisableReplicaRebalancing {
			...
			if rebalanceStore == nil {
				...
				log.VEventf(ctx, 1, "rebalancing to %+v: %s",
					rebalanceReplica, rangeRaftProgress(repl.RaftStatus(), desc.Replicas().Wrapped()))
				if err := rq.addReplica(
					ctx,
					repl,
					rebalanceReplica,
...
func (rq *replicateQueue) findTargetAndTransferLease(
	...
	candidates := filterBehindReplicas(repl.RaftStatus(), desc.Replicas().Wrapped())
