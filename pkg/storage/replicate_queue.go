...
func (rq *replicateQueue) shouldQueue(
	...
	target, _ := rq.allocator.RebalanceTarget(ctx, zone.Constraints, rangeInfo, storeFilterThrottled, false)
	if target != nil {
	...
	case AllocatorConsiderRebalance:
		...
		if !rq.store.TestingKnobs().DisableReplicaRebalancing {
			rebalanceStore, details := rq.allocator.RebalanceTarget(
				ctx, zone.Constraints, rangeInfo, storeFilterThrottled, disableStatsBasedRebalance)
