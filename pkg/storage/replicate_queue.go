...
func (rq *replicateQueue) shouldQueue(
	...
	// consider the current store to be the lease holder if no current lease is
	// present which allows us to consider the current store as a source for
	// rebalancing. Note that the current store will acquire the lease (if
	// necessary) in order to process the replica and the lease acquisition will
	// make sure that we have sufficient leases in the cluster to perform
	// lease-based rebalancing.
