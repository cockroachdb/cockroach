...
func (rq *replicateQueue) shouldQueue(
	...
	if !rq.store.TestingKnobs().DisableReplicaRebalancing {
		target, _ := rq.allocator.RebalanceTarget(ctx, zone.Constraints, repl, repl.RaftStatus(), rangeInfo, storeFilterThrottled)
		if target != nil {
	...
	case AllocatorConsiderRebalance:
		...
		if !rq.store.TestingKnobs().DisableReplicaRebalancing {
			rebalanceStore, details := rq.allocator.RebalanceTarget(
				ctx, zone.Constraints, repl, repl.RaftStatus(), rangeInfo, storeFilterThrottled)
