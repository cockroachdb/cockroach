...
func (r *pebbleMap) makeKeyWithTimestamp(k []byte) []byte {
	byteKey := r.makeKey(k)
	if r.allowDuplicates {
		r.keyID++
...
func (r *pebbleMap) Put(k []byte, v []byte) error {
	return r.store.Set(r.makeKeyWithTimestamp(k), v, pebble.NoSync)
}
...
func (i *pebbleMapIterator) UnsafeKey() []byte {
	...
	if i.allowDuplicates {
		...
		unsafeKey = unsafeKey[:len(unsafeKey)-8]
	}
	return unsafeKey[len(i.prefix):]
}
