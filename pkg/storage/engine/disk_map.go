...
func (i *pebbleMapIterator) Value() []byte {
	...
	safeValue := make([]byte, len(unsafeValue), len(unsafeValue)+1)
...
func (b *pebbleMapBatchWriter) Put(k []byte, v []byte) error {
	if err := b.batch.Set(EncodeKey(b.makeKey(k)), v, nil); err != nil {
		return err
	}
...
func (b *pebbleMapBatchWriter) Flush() error {
	if len(b.batch.Repr()) == 0 {
		return nil
	}
	if err := b.batch.Commit(&pebble.WriteOptions{Sync: false}); err != nil {
		return err
	}
