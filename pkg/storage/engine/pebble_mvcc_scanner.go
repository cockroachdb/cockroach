...
func (p *pebbleMVCCScanner) get() {
	p.parent.Seek(MVCCKey{Key: p.start})
	if !p.updateCurrent() {
		return
	}
	p.getAndAdvance()
...
func (p *pebbleMVCCScanner) advanceKeyAtEnd() bool {
	if p.reverse {
		...
		p.peeked = false
		p.parent.SeekReverse(MVCCKey{Key: keys.MaxKey})
		if !p.updateCurrent() {
			return false
		}
		return p.advanceKey()
...
func (p *pebbleMVCCScanner) iterSeekReverse(key MVCCKey) bool {
	...
	if bytes.Equal(key.Key, p.curKey) && key.Timestamp == p.curTS {
		p.parent.Prev()
	}
	if !p.updateCurrent() {
		// We have seeked to before the start key. Return.
