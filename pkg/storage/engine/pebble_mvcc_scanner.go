...
func (p *pebbleMVCCScanner) iterSeek(key roachpb.Key) bool {
	p.clearPeeked()
	p.parent.SeekGE(key)
...
func (p *pebbleMVCCScanner) iterSeekReverse(key roachpb.Key) bool {
	p.clearPeeked()

	if !p.parent.SeekLT(key) {
