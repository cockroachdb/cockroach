...
type metaTestRunner struct {
	...
	pastTSManager   *pastTSManager
	nextTSManager   *nextTSManager
...
func (m *metaTestRunner) init() {
	...
	m.pastTSManager = &pastTSManager{
		rng:         m.rng,
		tsGenerator: &m.tsGenerator,
	}
	m.nextTSManager = &nextTSManager{
		pastTSManager: pastTSManager{
			rng:         m.rng,
			tsGenerator: &m.tsGenerator,
		},
	...
	m.managers = map[operandType]operandManager{
		...
		operandPastTS:      m.pastTSManager,
		operandNextTS:      m.nextTSManager,
...
func (m *metaTestRunner) closeManagers() {
	closingOrder := []operandManager{
		m.iterManager,
		m.rwManager,
		m.txnManager,
	}
	for _, manager := range closingOrder {
		manager.closeAll()
	}
}
