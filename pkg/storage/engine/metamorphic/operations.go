...
var operations = []mvccOp{
	...
	},
	{
		...
		dependentOps: func(m *metaTestRunner, args ...operand) (result []opRun) {
			txnManager := m.managers[OPERAND_TRANSACTION].(*txnManager)
			rwManager := m.managers[OPERAND_READWRITER].(*readWriterManager)
			txn := args[0].(*roachpb.Transaction)
			...
			for _, batch := range txnManager.inFlightBatches[txn] {
				...
				// Check if this batch is live. There's a chance it could have been
				// closed by a previous batch_commit.
				found := false
				for _, liveBatch := range rwManager.liveBatches {
					if batch == liveBatch {
						found = true
						break
					}
