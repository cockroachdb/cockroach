...
type opGenerator struct {
	...
	// call to the matching operand manager, instead of the usual get().
	isOpener bool
}
...
func generateMVCCScan(
	...
	var txn txnID
	if inconsistent {
		ts = m.pastTSManager.parse(args[2])
	} else {
		txn = txnID(args[2])
	}
	return &mvccScanOp{
...
var opGenerators = []opGenerator{
	...
	},
	{
		...
		generate: func(ctx context.Context, m *metaTestRunner, args ...string) mvccOp {
			...
			key := m.keyManager.parse(args[1]).Key
			endKey := m.keyManager.parse(args[2]).Key
			startTime := m.pastTSManager.parse(args[3])
			endTime := m.pastTSManager.parse(args[3])
	...
	},
	{
		...
		generate: func(ctx context.Context, m *metaTestRunner, args ...string) mvccOp {
			return &txnOpenOp{
				m:  m,
				id: txnID(args[1]),
				ts: m.nextTSManager.parse(args[0]),
			}
		},
		operands: []operandType{
			operandNextTS,
	...
	},
	{
		...
		generate: func(ctx context.Context, m *metaTestRunner, args ...string) mvccOp {
			...
			m.iterManager.generateOpen(rw, iterID)
			return &iterOpenOp{
				m:      m,
				rw:     rw,
				key:    key.Key,
