...
func TestStickyEngines(t *testing.T) {
	...
	require.Error(t, errors.Errorf("sticky engine engine1 has not been closed"))
	...
	require.Error(t, errors.Errorf("sticky in-mem engine engine3 does not exist"))
	...
	require.Len(t, stickyInMemEnginesCache, 0)
	for _, engine := range []Engine{engine1, newEngine1, engine2} {
