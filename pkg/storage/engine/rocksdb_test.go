...
func TestRocksDBSstFileWriterTruncate(t *testing.T) {
	...
	// Truncate wil not be used on this writer.
	...
	for i := 0; i < entries; i++ {
		...
		if i > 0 && i%truncateChunk == 0 {
			...
			// because the keys may be in un-flushed blocks. This test had been
			// tuned such that every other batch chunk is always large enough to
			// require at least one block to be flushed.
...
func TestSstFileWriterTimeBound(t *testing.T) {
	...
	for walltime := int64(1); walltime < 5; walltime++ {
		...
		defer sst.Close()
		if err := sst.Put(MVCCKey{Key: []byte("key"), Timestamp: hlc.Timestamp{WallTime: walltime}}, []byte("value")); err != nil {
			t.Fatal(err)
		}
		sstContents, err := sst.Finish()
		if err != nil {
