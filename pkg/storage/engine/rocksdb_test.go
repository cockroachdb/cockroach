...
`)
	...
	if err := batch.ClearRange(MakeMVCCMetadataKey(makeKey("d", 0)),
		MakeMVCCMetadataKey(makeKey("e", numEntries))); err != nil {
		t.Fatal(err)
	}
...
`)
	...
	_ = db.CompactRange(makeKey("d", 0), makeKey("e", numEntries), false)
	verifySSTables(`
