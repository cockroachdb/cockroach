...
// pebbleCache is a struct that manages a shared Pebble cache instance as well
// as does ref counts.
...
var cacheSingleton pebbleCache

// get returns a handle to a shared pebble cache.
func (p *pebbleCache) get() *pebble.Cache {
	p.Lock()
	defer p.Unlock()

	if p.handle == nil {
		// Size copied over from server.DefaultCacheSize.
		//
		// TODO(itsbilal): Investigate whether DefaultCacheSize works well in this
		// case.
		p.handle = pebble.NewCache(128 << 20)
	}
...
// release is called to denote a pebble cache object is no longer in use.
func (p *pebbleCache) release() {
	p.Lock()
	defer p.Unlock()
...
// and pebbleBatch.
func iterateOnReader(reader Reader, start, end MVCCKey, f func(MVCCKeyValue) (stop bool, err error)) error {
	if reader.Closed() {
	...
	// Since the bounds could have timestamps as well, and pebbleIterator assumes
	// bounds do not contain timestamps, add a byte to the end key to make it
	// "inclusive", then manually do bound checks in this function.
	endBuf := make([]byte, 0, len(end.Key) + int(MVCCVersionTimestampSize) + 1)
	endBuf = append(endBuf, end.Key...)
	endBuf = append(endBuf, 0x00)
	it := reader.NewIterator(IterOptions{LowerBound: start.Key, UpperBound: endBuf})
	defer it.Close()
