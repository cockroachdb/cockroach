...
var MVCCComparer = &pebble.Comparer{
	...
	Separator: func(dst, a, b []byte) []byte {
		fmt.Printf("Separator: a: %s, b: %s\n", strings.ToUpper(hex.EncodeToString(a)), strings.ToUpper(hex.EncodeToString(b)))
		...
		fmt.Printf("Separator: %s\n", strings.ToUpper(hex.EncodeToString(dst)))
		// The separator is > aKey, so we only need to add the timestamp sentinel.
		return append(dst, 0)
	},

	Successor: func(dst, a []byte) []byte {
		fmt.Printf("Successor: a: %s\n", strings.ToUpper(hex.EncodeToString(a)))
		aKey, _, ok := enginepb.SplitMVCCKey(a)
		if !ok {
			return append(dst, a...)
		}
