...
func (t *TeePebbleRocksDB) Closed() bool {
	...
	if pebbleClosed && rocksDBClosed {
		...
		panic(fmt.Sprintf("only one of pebble or rocksdb closed: %v != %v", pebbleClosed, rocksDBClosed))
	}
	return false
}
...
func (t *TeePebbleRocksDB) ExportToSst(
	...
	if !bytes.Equal(pebbleSst, rocksSst) {
		panic(fmt.Sprintf("mismatching SSTs returned by pebble and rocksdb: %v != %v", pebbleSst, rocksSst))
	}
