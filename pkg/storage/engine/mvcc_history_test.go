	...
	for _, engineImpl := range mvccEngineImpls {
		t.Run(engineImpl.name, func(t *testing.T) {
			...
			datadriven.Walk(t, "testdata/mvcc_histories", func(t *testing.T, path string) {
				...
				if strings.HasSuffix(path, "~") || strings.HasPrefix(path, "#") || strings.HasPrefix(path, ".") {
					return
				}
...
func cmdResolve(e *evalCtx) error {
	...
	key := e.getKey()
	return e.resolveIntent(e.engine, key, txn)
}

func (e *evalCtx) resolveIntent(
	engine ReadWriter, key roachpb.Key, txn *roachpb.Transaction,
) error {
...
func cmdPut(e *evalCtx) error {
	...
	return e.withWriter("put", func(engine ReadWriter) error {
		...
		if resolve {
			return e.resolveIntent(engine, key, txn)
		}
		return nil
	})
}
...
func cmdAdvanceTxn(e *evalCtx) error {
	txn := e.getTxn(mandatory)
	ts := e.getTs(txn)
	if ts.Less(txn.OrigTimestamp) {
		e.Fatalf("cannot advance txn to earlier (%s) than its OrigTimestamp (%s)",
...
// in a k=v pair with more characters (includes + = / ,) and
// allows empty values.
