...
func (t *TeeEngine) remapPath(path1 string) (ok bool, path2 string) {
...
func (t *TeeEngine) CreateFile(filename string) (fs.File, error) {
	...
	ok, filename2 := t.remapPath(filename)
	if !ok {
		return file1, err
	}
	os.MkdirAll(filepath.Dir(filename2), 0755)
...
func (t *TeeEngine) LinkFile(oldname, newname string) error {
	...
	ok, oldname2 := t.remapPath(oldname)
	if !ok {
		return err
	}
	_, newname2 := t.remapPath(newname)
