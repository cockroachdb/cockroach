...
func TestRocksDBMap(t *testing.T) {
	...
	tempEngine, err := NewTempEngine(ctx, base.DefaultTestStoreSpec, base.DefaultTestStoreSpec, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func BenchmarkRocksDBMapWrite(b *testing.B) {
	...
	ctx := context.Background()
	tempEngine, err := NewTempEngine(ctx, base.DefaultTestStoreSpec, base.StoreSpec{Path: dir}, nil)
	if err != nil {
		b.Fatal(err)
	}
	defer tempEngine.Close()
