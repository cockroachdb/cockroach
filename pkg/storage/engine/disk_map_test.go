...
func TestRocksDBMap(t *testing.T) {
	...
	tempEngine, err := NewTempEngine(ctx, base.DefaultTestStoreSpec, base.DefaultTestStoreSpec)
	if err != nil {
		t.Fatal(err)
	}
...
func BenchmarkRocksDBMapWrite(b *testing.B) {
	...
	ctx := context.Background()
	tempEngine, err := NewTempEngine(ctx, base.DefaultTempStoreSpec, base.StoreSpec{Path: dir})
	if err != nil {
		b.Fatal(err)
	}
	defer tempEngine.Close()
