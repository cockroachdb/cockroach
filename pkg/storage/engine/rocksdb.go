...
func MakeRocksDBSstFileReader() RocksDBSstFileReader {
	return RocksDBSstFileReader{NewInMem(roachpb.Attributes{}, 1<<20)}
}
...
func (fr *RocksDBSstFileReader) IngestExternalFile(data []byte) error {
	...
	return statusToError(C.DBEngineIngestExternalFile(fr.rocksDB.rdb, goToCSlice([]byte(filename))))
}
...
// WriteFile writes data to a file in this RocksDB's in-memory env.
func (r InMem) WriteFile(filename string, data []byte) error {
	return statusToError(C.DBMemEnvWriteFile(
		r.RocksDB.rdb, goToCSlice([]byte(filename)), goToCSlice(data)))
}
