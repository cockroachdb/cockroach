...
func (r *RocksDB) syncLoop() {
	...
	for {
		...
		for _, b := range pending {
			// TODO(peter): blindly assigning to b.commitErr is considered a data
			// race. Need to figure out why.
			if err != nil {
				b.commitErr = err
			}
			b.commitCond.Signal()
...
func (r *rocksDBBatch) Commit(syncCommit bool) error {
	...
	if r.writeOnly {
		...
		r.commitCond.L = &c.Mutex

		// The leader for the commit is the first batch to be added to the pending
		// slice. Every batch has an associated condition variable which is
		// signalled when the commit is complete.
		c.Lock()
		...
		if leader {
			...
			if len(syncing) > 0 {
				// The commit was successful and one of the batches requires syncing:
				// notify the sync goroutine.
				s := &r.parent.syncer
				if r.syncCommit {
					// Special handling for the leader if it requires syncing. We need to
					// atomically add it to the sync pending list and wait for the commit
					// condvar to be notified.
					r.commitCond.L = &s.Mutex
				}
				s.Lock()
			...
			// We're a follower. Wait for the commit/sync to finish.
			r.commitCond.Wait()
			c.Unlock()
		}
		return r.commitErr
