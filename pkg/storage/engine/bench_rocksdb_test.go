...
func BenchmarkIterOnReadOnly_RocksDB(b *testing.B) {
	for _, writes := range []int{10, 100, 1000, 10000} {
		b.Run(fmt.Sprintf("%d", writes), func(b *testing.B) {
			benchmarkIterOnReadWriter(b, writes, func (engine Engine) ReadWriter { return engine.NewReadOnly() }, true)
		})
	}
}
...
func BenchmarkIterOnEngine_RocksDB(b *testing.B) {
	for _, writes := range []int{10, 100, 1000, 10000} {
		b.Run(fmt.Sprintf("%d", writes), func(b *testing.B) {
			benchmarkIterOnReadWriter(b, writes, func (engine Engine) ReadWriter { return engine }, false)
		})
	}
}
