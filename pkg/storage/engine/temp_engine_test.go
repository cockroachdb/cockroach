...
// TestCleanupTempStorageDirs checks that cleanupTempStorageDirs
// takes a temp directory and cleans up all other temporary
// directories with the DefaultTempStorePrefix.
func TestCleanupTempStorageDirs(t *testing.T) {
	...
	tempDir, tempDirCleanup := tempStoreDir(t, dir, DefaultTempStorePrefix)
	defer tempDirCleanup()

	// These will be the abandoned temporary directories left over
	// from a previous node startup.
	abandonedDir1, abandonedDir1Cleanup := tempStoreDir(t, dir, DefaultTempStorePrefix)
	defer abandonedDir1Cleanup()
	abandonedDir2, abandonedDir2Cleanup := tempStoreDir(t, dir, DefaultTempStorePrefix)
	...
	wg := sync.WaitGroup{}
	if err := cleanupTempStorageDirs(context.TODO(), tempDir, &wg); err != nil {
