...
func TestNewTempEngine(t *testing.T) {
	...
	engine, err := NewTempEngine(base.TempStorageConfig{ParentDir: tempDir, RecordDir: dir})
	if err != nil {
...
func TestTempEngineClose(t *testing.T) {
	...
	engine, err := NewTempEngine(base.TempStorageConfig{ParentDir: dir, RecordDir: dir})
	if err != nil {
		t.Fatalf("error encountered when invoking NewTempEngine: %v", err)
	}
...
func TestAppendTempStorageDir(t *testing.T) {
	...
	if err := appendTempStorageDir(dir, tempPathRecord); err != nil {
		t.Fatalf("appendTempStorageDir could not create new record file to append: %v", err)
	}
...
func TestCleanupTempStorageDirsNoRecord(t *testing.T) {
	...
	if err := cleanupTempStorageDirs(dir); err != nil {
