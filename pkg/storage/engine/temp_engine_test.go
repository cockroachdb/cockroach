...
func TestNewTempEngine(t *testing.T) {
	...
	// This will be cleaned up with abandonedDirCleanup.
...
// TestCleanupTempStoreDirsNoop verifies that cleanupTempStoreDirs does not
// error if there is no record file available.
func TestCleanupTempStoreDirsNoop(t *testing.T) {
	defer leaktest.AfterTest(t)()
	dir, dirCleanup := testutils.TempDir(t)
	defer dirCleanup()

	if err := cleanupTempStoreDirs(context.TODO(), dir, nil); err != nil {
		t.Fatalf("error encountered when cleanupTempStoreDirs encountered a non-existing record file %v", err)
	}
}
...
func TestCleanupTempStoreDirs(t *testing.T) {
	...
	abandonedDir1, abandonedDir1Cleanup := tempStoreDir(t, dir)
	defer abandonedDir1Cleanup()
	abandonedDir2, abandonedDir2Cleanup := tempStoreDir(t, dir)
	...
	// cleanup.
	if err := appendTempStoreDir(dir, abandonedDir1); err != nil {
		t.Fatal(err)
	}
	if err := appendTempStoreDir(dir, abandonedDir2); err != nil {
		t.Fatal(err)
	}

	// This is our "current" temporary directory that will not be in the
	// record file and thus should  not be removed.
	curDir, curDirCleanup := tempStoreDir(t, dir)
	defer curDirCleanup()
	...
	for _, file := range files {
		if file.Name() != filepath.Base(curDir) && file.Name() != TempStoreDirsRecordFilename {
			t.Fatalf("expected either %s or %s, actual: %s", curDir, TempStoreDirsRecordFilename, file.Name())
		}
	}
