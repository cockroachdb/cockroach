...
func cleanupTempStoreDirs(ctx context.Context, storePath string, wg *sync.WaitGroup) error {
	...
	go func() {
		...
		if remainingDirs.Len() != 0 {
			if err := logTempStoreDirs(storePath, remainingDirs.Bytes()); err != nil {
				log.Warningf(ctx, "could not record non-removable temporary store directories to log file: %v", err.Error())
			}
		}
	}()
