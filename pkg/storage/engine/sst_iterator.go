...
// TODO(ajkr): once Pebble supports disabling block cache we can remove this file
// number forgery for preventing cache key conflicts.
var fileNumAlloc = struct {
	sync.Mutex
	seq uint64
}{seq: 1}

func allocFileNum() uint64 {
	fileNumAlloc.Lock()
	num := fileNumAlloc.seq
	fileNumAlloc.seq++
	fileNumAlloc.Unlock()
	return num
}
...
func NewSSTIterator(path string) (SimpleIterator, error) {
	...
	fileNum := allocFileNum()
	var sst *sstable.Reader
	sst, err = sstable.NewReader(file, 0 /* dbNum */, fileNum, readerOpts)
	if err != nil {
		// sstable.NewReader does not clean itself up on error. That is different
		// from NewSSTIterator which historically has cleaned up before returning
		// an error.
		if sst != nil {
			// Ignore potential error result as we should return the original error.
			sst.Close()
		}
		return nil, err
	}
...
func NewMemSSTIterator(data []byte, verify bool) (SimpleIterator, error) {
	readerOpts := &sstable.Options{
		Comparer: MVCCComparer,
	}
	fileNum := allocFileNum()
	sst, err := sstable.NewReader(newMemFile(data), 0 /* dbNum */, fileNum, readerOpts)
	if err != nil {
		// sstable.NewReader does not clean itself up on error. That is different
		// from NewMemSSTIterator which historically has cleaned up before returning
		// an error.
		if sst != nil {
			// Ignore potential error as we need to return the original error.
			sst.Close()
		}
		return nil, err
	}
