...
func TestMVCCOpLogWriter(t *testing.T) {
	...
	for _, engineImpl := range mvccEngineImpls {
		t.Run(engineImpl.name, func(t *testing.T) {
			...
			if err := MVCCPut(ctx, ol, nil, testKey1, txn1ts.DeprecatedOrigTimestamp, value2, txn1ts); err != nil {
				t.Fatal(err)
			}
			...
			if err := MVCCPut(ctx, ol, nil, localKey, txn1ts.DeprecatedOrigTimestamp, value2, txn1ts); err != nil {
				t.Fatal(err)
			}
			...
			if err := MVCCPut(ctx, olDist, nil, testKey1, txn1ts.DeprecatedOrigTimestamp, value2, txn1ts); err != nil {
				t.Fatal(err)
			}
			if err := MVCCPut(ctx, olDist, nil, localKey, txn1ts.DeprecatedOrigTimestamp, value2, txn1ts); err != nil {
				t.Fatal(err)
			}
			...
			if err := MVCCPut(ctx, olDist, nil, testKey2, txn1LargerTS.DeprecatedOrigTimestamp, value3, txn1LargerTS); err != nil {
				t.Fatal(err)
			}
			olDist.Close()
			...
			if err := MVCCPut(ctx, ol, nil, testKey3, txn2ts.DeprecatedOrigTimestamp, value4, txn2ts); err != nil {
				t.Fatal(err)
			}
			txn2Pushed := *txn2
