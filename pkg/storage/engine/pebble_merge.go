...
func sortAndDeduplicateRows(ts *roachpb.InternalTimeSeriesData) {
	...
	sort.SliceStable(rank, func(i, j int) bool {
		return ts.Samples[rank[i]].Offset < ts.Samples[rank[j]].Offset
	})
	...
	for destIdx := range rank {
		if destIdx == len(rank)-1 || ts.Samples[rank[destIdx]].Offset != ts.Samples[rank[destIdx+1]].Offset {
			uniqRank = append(uniqRank, rank[destIdx])
		}
	}
...
func sortAndDeduplicateColumns(ts *roachpb.InternalTimeSeriesData) {
	...
	sort.SliceStable(rank, func(i, j int) bool {
		return ts.Offset[rank[i]] < ts.Offset[rank[j]]
	})
	...
	for destIdx := range rank {
		if destIdx == len(rank)-1 || ts.Offset[rank[destIdx]] != ts.Offset[rank[destIdx+1]] {
			uniqRank = append(uniqRank, rank[destIdx])
		}
	}
