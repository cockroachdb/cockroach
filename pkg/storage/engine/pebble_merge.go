...
func merge(key, oldValue, newValue, buf []byte) []byte {
	...
	if err := protoutil.Unmarshal(oldValue, &oldMeta); err != nil {
		log.Fatalf(context.Background(), "corrupted old mvcc: %v", err)
	}
	if len(oldMeta.RawBytes) < headerSize {
		log.Fatalf(context.Background(), "old mvcc value too short")
	}
	if err := protoutil.Unmarshal(newValue, &newMeta); err != nil {
		log.Fatalf(context.Background(), "corrupted new mvcc: %v", err)
	}
	if len(newMeta.RawBytes) < headerSize {
		log.Fatalf(context.Background(), "new mvcc value too short")
	}

	tsTag := byte(roachpb.ValueType_TIMESERIES)
	if oldMeta.RawBytes[tagPos] == tsTag || newMeta.RawBytes[tagPos] == tsTag {
		if oldMeta.RawBytes[tagPos] != tsTag || newMeta.RawBytes[tagPos] != tsTag {
			log.Fatalf(context.Background(), "inconsistent value types for timeseries merge")
		}
		if tsBytes, err := mergeTimeSeriesValues(
			oldMeta.RawBytes[headerSize:], newMeta.RawBytes[headerSize:]); err != nil {
			log.Fatalf(context.Background(), "mergeTimeSeriesValues: %v", err)
		} else {
	...
	if err != nil {
		log.Fatalf(context.Background(), "corrupted merged mvcc: %v", err)
	}
	return res
}
