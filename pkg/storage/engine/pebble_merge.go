...
func merge(key, oldValue, newValue, buf []byte) ([]byte, error) {
	...
	if err := protoutil.Unmarshal(oldValue, &oldMeta); err != nil {
		return nil, errors.Errorf("corrupted old mvcc: %v", err)
	}
	if len(oldMeta.RawBytes) < headerSize {
		return nil, errors.Errorf("old mvcc value too short")
	}
	if err := protoutil.Unmarshal(newValue, &newMeta); err != nil {
		return nil, errors.Errorf("corrupted new mvcc: %v", err)
	}
	if len(newMeta.RawBytes) < headerSize {
		return nil, errors.Errorf("new mvcc value too short")
	}
	...
	if err != nil {
		return nil, errors.Errorf("corrupted merged mvcc: %v", err)
	}
	return res, nil
}
