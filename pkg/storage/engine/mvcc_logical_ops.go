...
type MVCCLogicalOpDetails struct {
	...
	// Unsafe indicates that the values in this struct may be invalidated at a
	// later point. If the details object cannot promise that its values will
	// never be invalidated, an OpLoggerBatch will make a copy of all references
	// before adding it to the log. TestMVCCOpLogWriter fails without this.
	Unsafe bool
}
