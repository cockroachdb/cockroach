...
type MVCCOpLogWriter struct {
	Batch
	distinct     distinctLogWriter
	distinctOpen bool
	ops          []enginepb.MVCCLogicalOp
}

// NewOpLoggerBatch creates a new batch that logs mvcc logical operations and
// wraps the provided batch.
func NewOpLoggerBatch(b Batch) *MVCCOpLogWriter {
	w := &MVCCOpLogWriter{Batch: b}
	w.distinct.parent = w
	return w
}
...
type distinctLogWriter struct {
	ReadWriter
	parent *MVCCOpLogWriter
}

// LogLogicalOp implements the Writer interface.
func (dlw *distinctLogWriter) LogLogicalOp(op MVCCLogicalOpType, details MVCCLogicalOpDetails) {
	dlw.parent.logLogicalOp(op, details)
	dlw.ReadWriter.LogLogicalOp(op, details)
}

// Close implements the Reader interface.
func (dlw *distinctLogWriter) Close() {
