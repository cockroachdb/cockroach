...
func MVCCClearTimeRange(
	...
	for it.Seek(MVCCKey{Key: key}); ; it.Next() {
		...
		if ms != nil && len(clearedKey.Key) > 0 {
			metaKeySize := int64(clearedKey.EncodedSize())
			if bytes.Equal(clearedKey.Key, k.Key) {
				...
				ms.Add(updateStatsOnClear(
					clearedKey.Key, metaKeySize, 0, metaKeySize, 0, &clearedMeta, &restoredMeta, k.Timestamp.WallTime,
				))
			} else {
				// We cleared a revision of a different key, so nothing was "restored".
				ms.Add(updateStatsOnClear(clearedKey.Key, metaKeySize, 0, 0, 0, &clearedMeta, nil, 0))
			}
			clearedKey.Key = clearedKey.Key[:0]
		}
	...
	if ms != nil && len(clearedKey.Key) > 0 {
		...
		origMetaKeySize := int64(clearedKey.EncodedSize())
		ms.Add(updateStatsOnClear(clearedKey.Key, origMetaKeySize, 0, 0, 0, &clearedMeta, nil, 0))
	}
