...
func MVCCClearTimeRange(
	...
	for it.Seek(MVCCKey{Key: key}); ; it.Next() {
		...
		if ms != nil && len(clearedKey) > 0 {
			...
			if bytes.Equal(clearedKey, k.Key) {
				...
				ms.Add(updateStatsOnClear(
					clearedKey, metaKeySize, 0, metaKeySize, 0, &clearedMeta, &restoredMeta, k.Timestamp.WallTime,
				))
			} else {
				// We cleared a revision of a different key, so nothing was "restored".
				ms.Add(updateStatsOnClear(clearedKey, metaKeySize, 0, 0, 0, &clearedMeta, nil, 0))
			}
	...
	if ms != nil && len(clearedKey) > 0 {
		...
		origMetaKeySize := int64(len(clearedKey) + 1) // EncodedSize().
		ms.Add(updateStatsOnClear(clearedKey, origMetaKeySize, 0, 0, 0, &clearedMeta, nil, 0))
	}
