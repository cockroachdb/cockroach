...
// apparent effect of "reverting" the range to just before startTime if all of
// the older revisions of cleared keys are still available (i.e. have not been
// GC'ed).
//
...
func MVCCClearTimeRange(
	ctx context.Context, batch ReadWriter, key, endKey MVCCKey, startTime, endTime hlc.Timestamp,
) error {
	var cleared int64
	...
	if err := batch.Iterate(
		...
		func(kv MVCCKeyValue) (bool, error) {
			...
			// workloads, it is likely that large blocks of keys are written at
