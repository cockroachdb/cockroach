...
func mvccGetInternal(
	...
	if !consistent && meta.Txn != nil && !timestamp.Less(metaTimestamp) {
		...
		intent := roachpb.MakeErrorIntent(*meta.Txn, roachpb.Span{Key: metaKey.Key})
		...
		if !maxVisibleTimestamp.Less(metaTimestamp) {
			return nil, nil, safeValue, &roachpb.WriteIntentError{
				Intents: []roachpb.Intent{
					roachpb.MakeErrorIntent(*meta.Txn, roachpb.Span{Key: metaKey.Key}),
				},
			}
		}
	}
		...
		if meta.Txn != nil {
			...
			if txn == nil || meta.Txn.ID != txn.ID {
				...
				return &roachpb.WriteIntentError{Intents: []roachpb.Intent{
					roachpb.MakeErrorIntent(*meta.Txn, roachpb.Span{Key: key}),
				}}
		...
		if !k.IsValue() {
			...
			if meta.Txn != nil && startTime.Less(ts) && !endTime.Less(ts) {
				err := &roachpb.WriteIntentError{
					Intents: []roachpb.Intent{
						roachpb.MakeErrorIntent(*meta.Txn, roachpb.Span{Key: append([]byte{}, k.Key...)}),
					}}
				return nil, err
			}
		}
...
func buildScanIntents(data []byte) ([]roachpb.Intent, error) {
	...
	for reader.Next() {
		...
		intents = append(intents, roachpb.MakeErrorIntent(*meta.Txn, roachpb.Span{Key: key.Key}))
	}
		...
		if !existingKey.IsValue() {
			...
			if len(mvccMeta.RawBytes) > 0 {
				...
				writeIntentErr := roachpb.WriteIntentError{
					Intents: []roachpb.Intent{
						roachpb.MakeErrorIntent(*mvccMeta.Txn, roachpb.Span{Key: existingIter.Key().Key}),
					},
				}
