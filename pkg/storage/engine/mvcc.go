...
func MVCCPutProto(
	...
	return MVCCPut(ctx, readWriter, ms, key, timestamp, value, txn)
}
...
func MVCCPut(
	...
	return mvccPutUsingIter(ctx, readWriter, iter, ms, key, timestamp, value, txn, nil /* valueFn */)
}
...
func MVCCDelete(
	...
	return mvccPutUsingIter(ctx, readWriter, iter, ms, key, timestamp, noValue, txn, nil /* valueFn */)
}
...
func MVCCIncrement(
	...
	err := mvccPutUsingIter(ctx, readWriter, iter, ms, key, timestamp, noValue, txn, func(value *roachpb.Value) ([]byte, error) {
		if value.IsPresent() {
			var err error
...
func MVCCConditionalPut(
	...
	return mvccConditionalPutUsingIter(ctx, readWriter, iter, ms, key, timestamp, value, expVal, allowIfDoesNotExist, txn)
}
...
func MVCCInitPut(
	...
	iter := readWriter.NewIterator(IterOptions{Prefix: true})
	defer iter.Close()
	return mvccInitPutUsingIter(ctx, readWriter, iter, ms, key, timestamp, value, failOnTombstones, txn)
}
...
func MVCCBlindInitPut(
	...
	return mvccInitPutUsingIter(ctx, readWriter, nil, ms, key, timestamp, value, failOnTombstones, txn)
}
...
func mvccInitPutUsingIter(
	...
	return mvccPutUsingIter(
		ctx, readWriter, iter, ms, key, timestamp, noValue, txn,
		func(existVal *roachpb.Value) ([]byte, error) {
...
func MVCCMerge(
	...
	*meta = enginepb.MVCCMetadata{RawBytes: rawBytes}
	...
	if err == nil {
		if err = readWriter.Merge(metaKey, data); err == nil && ms != nil {
...
func MVCCClearTimeRange(
	...
	flushClearedKeys := func(nonMatch MVCCKey) error {
		if len(clearRangeStart.Key) != 0 {
			if err := readWriter.ClearRange(clearRangeStart, nonMatch); err != nil {
				return err
			}
			batchSize++
			...
			for i := 0; i < bufSize; i++ {
				if err := readWriter.Clear(buf[i]); err != nil {
					return err
				}
			}
	...
	it := readWriter.NewIterator(IterOptions{LowerBound: key, UpperBound: endKey})
	defer it.Close()
...
func MVCCDeleteRange(
	...
	kvs, resumeSpan, _, err := MVCCScan(
		ctx, readWriter, key, endKey, max, scanTs, MVCCScanOptions{Txn: scanTxn})
	if err != nil {
	...
	for i := range kvs {
		err = mvccPutInternal(
			ctx, readWriter, iter, ms, kvs[i].Key, timestamp, nil, txn, buf, nil)
		if err != nil {
			break
		}
	}
...
func MVCCResolveWriteIntent(
	ctx context.Context, readWriter ReadWriter, ms *enginepb.MVCCStats, intent roachpb.Intent,
) error {
	iterAndBuf := GetBufUsingIter(readWriter.NewIterator(IterOptions{Prefix: true}))
	err := MVCCResolveWriteIntentUsingIter(ctx, readWriter, iterAndBuf, ms, intent)
	// Using defer would be more convenient, but it is measurably slower.
	iterAndBuf.Cleanup()
	return err
}
...
func MVCCResolveWriteIntentUsingIter(
	...
	_, err := mvccResolveWriteIntent(
		ctx, readWriter, iterAndBuf.iter, ms, intent, iterAndBuf.buf, false, /* forRange */
	)
	return err
}
...
func mvccResolveWriteIntent(
	...
	if commit || pushed {
		...
		if pushed {
			...
			metaKeySize, metaValSize, err = buf.putMeta(readWriter, metaKey, &buf.newMeta)
		} else {
			metaKeySize = int64(metaKey.EncodedSize())
			err = readWriter.Clear(metaKey)
		}
		if err != nil {
			return false, err
		}
		...
		if buf.newMeta.Timestamp != meta.Timestamp {
			...
			if err = readWriter.Put(newKey, iter.UnsafeValue()); err != nil {
				return false, err
			}
			if err = readWriter.Clear(oldKey); err != nil {
				return false, err
			}
		...
		readWriter.LogLogicalOp(logicalOp, MVCCLogicalOpDetails{
			Txn:       intent.Txn,
			Key:       intent.Key,
	...
	if err := readWriter.Clear(latestKey); err != nil {
		return false, err
	}

	// Log the logical MVCC operation.
	readWriter.LogLogicalOp(MVCCAbortIntentOpType, MVCCLogicalOpDetails{
		Txn: intent.Txn,
		Key: intent.Key,
	})
	...
	if !ok {
		...
		if err = readWriter.Clear(metaKey); err != nil {
			return false, err
		}
...
func MVCCResolveWriteIntentRange(
	ctx context.Context, readWriter ReadWriter, ms *enginepb.MVCCStats, intent roachpb.Intent, max int64,
) (int64, *roachpb.Span, error) {
	iterAndBuf := GetIterAndBuf(readWriter, IterOptions{UpperBound: intent.Span.EndKey})
	defer iterAndBuf.Cleanup()
	return MVCCResolveWriteIntentRangeUsingIter(ctx, readWriter, iterAndBuf, ms, intent, max)
}
	...
	for {
		...
		if !key.IsValue() {
			...
			ok, err = mvccResolveWriteIntent(
				ctx, readWriter, iterAndBuf.iter, ms, intent, iterAndBuf.buf, true, /* forRange */
			)
		}
		if err != nil {
...
func MVCCGarbageCollect(
	...
	for _, gcKey := range keys {
		...
		if !gcKey.Timestamp.Less(hlc.Timestamp(meta.Timestamp)) {
			...
			if !implicitMeta {
				if err := readWriter.Clear(iter.UnsafeKey()); err != nil {
					return err
				}
				count++
			}
		...
		for ; ; iter.Next() {
			...
			if !gcKey.Timestamp.Less(unsafeIterKey.Timestamp) {
				...
				count++
				if err := readWriter.Clear(unsafeIterKey); err != nil {
					return err
				}
			}
