...
func mvccPutInternal(
	...
	if ok {
		...
		if meta.Txn != nil {
			...
			// to write, but then return a write-too-old error indicating what
			// the timestamp ended up being. This timestamp can then be used to
			// increment the txn timestamp and be returned with the response.
			//
			// NB: even if metaTimestamp is less than writeTimestamp, we can't
			// avoid the WriteTooOld error if metaTimestamp is equal to or
			// greater than readTimestamp.
			// TODO DURING REVIEW: I'm actually not 100% sure why this is. It
			// seems kind of legit to me, but I see consistency violations if I
			// do this in tests like TestSplitTriggerRaftSnapshotRace.
			// Presumably it has something to do with CPuts observing the wrong
			// existing value because we still read at the orig timestamp. But
