...
func (p *pebbleIterator) NextKey() {
	...
	if p.UnsafeKey().Key.Equal(p.keyBuf) {
		...
		// to b), it will be surfaced first as a bare range key. However, it has
		// already been emitted, so we step past it to the next key, which may be
		// either a point key or range key starting past the seek key.
