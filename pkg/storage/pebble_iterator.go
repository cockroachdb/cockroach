...
func (p *pebbleIterator) RangeBounds() (roachpb.Key, roachpb.Key) {
	...
	// overlaps.
	//
	// TODO(erikgrinaker): The above will be resolved by Pebble defragmenting
	// range keys across SST boundaries. Consider removing these checks then.
	// However, we may still see this when setting iterator bounds to a
	// versioned key, which needs to be handled somehow.
