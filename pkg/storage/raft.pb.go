...
type RaftHeartbeat struct {
	...
	// safe_index is the maximum log index of proposals made for this
	// range at the time the heartbeat was generated. Reads may be
	// served by any replica at or before the max_safe timestamp so long
	// as the replica has committed log entries >= safe_index.
	SafeIndex uint64 `protobuf:"varint,7,opt,name=safe_index,json=safeIndex" json:"safe_index"`
}
...
type RaftMessageRequest struct {
	...
	// max_safe is the timestamp at or before which follower replicas
	// being heartbeat can serve reads as long as they've committed up
	// to the safe_index specified with the heartbeat. Heartbeats only.
	MaxSafe cockroach_util_hlc.Timestamp `protobuf:"bytes,8,opt,name=max_safe,json=maxSafe" json:"max_safe"`
	// epoch is the node liveness epoch of the sender. This is supplied
	// only on heartbeat requests. If this value changes between
	// heartbeats, all prior knowledge of safe indexes learned from the
	// sender must be discarded. Heartbeats only.
	Epoch int64 `protobuf:"varint,9,opt,name=epoch" json:"epoch"`
	// unsafe_ranges is a set of ranges for which previously learned
	// safe indexes can no longer be relied on and should be discarded.
	// The set includes leader-not-leaseholder and leaseholder-not-leader
	// ranges. Leader-not-leaseholder ranges are reported when the lease
	// is transferred, and leaseholder-not-leader ranges when the
	// leaseholder has forwarded a proposal since the last heartbeat.
	// Heartbeats only.
	UnsafeRanges []github_com_cockroachdb_cockroach_pkg_roachpb.RangeID `protobuf:"varint,10,rep,name=unsafe_ranges,json=unsafeRanges,casttype=github.com/cockroachdb/cockroach/pkg/roachpb.RangeID" json:"unsafe_ranges,omitempty"`
	// unsafe_ranges_ackd is a set of ranges acknowledged as marked
	// unsafe. This prevents lost heartbeats from failing to notify a
	// follower replica that the max safe timestamp does not apply to a
	// range anymore. Heartbeat responses only.
	UnsafeRangesAckd []github_com_cockroachdb_cockroach_pkg_roachpb.RangeID `protobuf:"varint,11,rep,name=unsafe_ranges_ackd,json=unsafeRangesAckd,casttype=github.com/cockroachdb/cockroach/pkg/roachpb.RangeID" json:"unsafe_ranges_ackd,omitempty"`
}
...
func (m *RaftHeartbeat) MarshalTo(dAtA []byte) (int, error) {
	...
	dAtA[i] = 0x38
	i++
	i = encodeVarintRaft(dAtA, i, uint64(m.SafeIndex))
	return i, nil
}
...
func (m *RaftMessageRequest) MarshalTo(dAtA []byte) (int, error) {
	...
	dAtA[i] = 0x42
	i++
	i = encodeVarintRaft(dAtA, i, uint64(m.MaxSafe.Size()))
	n4, err := m.MaxSafe.MarshalTo(dAtA[i:])
	if err != nil {
		return 0, err
	}
	i += n4
	...
	if len(m.UnsafeRanges) > 0 {
		for _, num := range m.UnsafeRanges {
			dAtA[i] = 0x50
			i++
			i = encodeVarintRaft(dAtA, i, uint64(num))
		}
	}
	if len(m.UnsafeRangesAckd) > 0 {
		for _, num := range m.UnsafeRangesAckd {
			dAtA[i] = 0x58
			i++
			i = encodeVarintRaft(dAtA, i, uint64(num))
		}
...
func (m *RaftHeartbeat) Size() (n int) {
	...
	n += 2
	n += 1 + sovRaft(uint64(m.SafeIndex))
	return n
}
...
func (m *RaftMessageRequest) Size() (n int) {
	...
	if len(m.UnsafeRanges) > 0 {
		for _, e := range m.UnsafeRanges {
			n += 1 + sovRaft(uint64(e))
		}
	}
	if len(m.UnsafeRangesAckd) > 0 {
		for _, e := range m.UnsafeRangesAckd {
			n += 1 + sovRaft(uint64(e))
		}
	}
	return n
...
func (m *RaftHeartbeat) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 7:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field SafeIndex", wireType)
			}
			m.SafeIndex = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.SafeIndex |= (uint64(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
...
func (m *RaftMessageRequest) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 8:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field MaxSafe", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if err := m.MaxSafe.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 9:
		...
		case 10:
			if wireType == 0 {
				...
				m.UnsafeRanges = append(m.UnsafeRanges, v)
			} else if wireType == 2 {
				var packedLen int
				for shift := uint(0); ; shift += 7 {
					if shift >= 64 {
				...
				return fmt.Errorf("proto: wrong wireType = %d for field UnsafeRanges", wireType)
			}
		case 11:
			if wireType == 0 {
				var v github_com_cockroachdb_cockroach_pkg_roachpb.RangeID
				...
				m.UnsafeRangesAckd = append(m.UnsafeRangesAckd, v)
			} else if wireType == 2 {
				var packedLen int
				for shift := uint(0); ; shift += 7 {
					if shift >= 64 {
				...
				return fmt.Errorf("proto: wrong wireType = %d for field UnsafeRangesAckd", wireType)
			}
		default:
			iNdEx = preIndex
			skippy, err := skipRaft(dAtA[iNdEx:])
...
var fileDescriptorRaft = []byte{
	...
	,
}
