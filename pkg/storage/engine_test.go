		...
		t.Run(engineImpl.name, func(t *testing.T) {
			...
			for _, tc := range testcases {
				t.Run(fmt.Sprintf("maxIntents=%d", tc.maxIntents), func(t *testing.T) {
					...
					iter := engine.NewMVCCIterator(MVCCKeyAndIntentsIterKind, IterOptions{UpperBound: roachpb.Key("z")})
					defer iter.Close()
					iter.SeekGE(MVCCKey{Key: roachpb.Key("a")})
					_, err := iter.CheckForKeyCollisions(sstFile.Bytes(), roachpb.Key("a"), roachpb.Key("z"), tc.maxIntents)
					require.Error(t, err)
