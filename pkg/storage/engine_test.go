	...
	for name, enableSeparatedIntents := range map[string]bool{"interleaved": false, "separated": true} {
		t.Run(name, func(t *testing.T) {
			...
			for name, tc := range testcases {
				tc := tc
				t.Run(name, func(t *testing.T) {
					intents, err := ScanIntents(eng, tc.from, tc.to, tc.max)
					require.NoError(t, err)
					require.Len(t, intents, len(tc.expectIntents), "unexpected number of intents")
					for i, intent := range intents {
						require.Equal(t, roachpb.Key(keys[i]), intent.Key)
					}
