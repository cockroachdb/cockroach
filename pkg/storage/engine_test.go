...
func TestEngineConsistentIterators(t *testing.T) {
	...
	for name, tc := range testcases {
		t.Run(name, func(t *testing.T) {
			...
			iterOld := r.NewMVCCIterator(MVCCKeyAndIntentsIterKind, opts)
			defer iterOld.Close()
			...
			iterPinned := rPinned.NewMVCCIterator(MVCCKeyAndIntentsIterKind, opts)
			...
			iterNew := r.NewMVCCIterator(MVCCKeyAndIntentsIterKind, opts)
			defer iterNew.Close()
			...
			if tc.canWrite {
				...
				// were created.
				//
				// TODO(erikgrinaker): existing batch iterator sees new writes -- it shouldn't.
				if name != "Batch" {
					require.Equal(t, expectOld, scanIter(t, iterOld))
				}
				if r.ConsistentIterators() {
				...
				// engine either way.
				iterOwn := r.NewMVCCIterator(MVCCKeyAndIntentsIterKind, opts)
				defer iterOwn.Close()
				if tc.readOwnWrites {
					if r.ConsistentIterators() {
						// TODO(erikgrinaker): new batch iterators don't see new range keys.
						if name != "Batch" {
...
func TestEngineRangeKeyMutations(t *testing.T) {
	...
	testutils.RunTrueAndFalse(t, "batch", func(t *testing.T, useBatch bool) {
		...
		require.NoError(t, rw.ExperimentalClearMVCCRangeKey(rangeKey("c", "f", 1)))
		require.NoError(t, rw.ExperimentalClearMVCCRangeKey(rangeKey("c", "f", 2)))
		require.Equal(t, []MVCCRangeKey{
			rangeKey("a", "c", 1),
			rangeKey("f", "g", 2),
			rangeKey("g", "h", 1),
