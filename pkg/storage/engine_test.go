...
func TestScanIntents(t *testing.T) {
	...
	testcases := map[string]struct {
		...
		"no keys":     {keys[0], keys[0], 0, keys[:0]},
		"one key":     {keys[0], keys[1], 0, keys[0:1]},
		"two keys":    {keys[0], keys[2], 0, keys[0:2]},
		"all keys":    {keys[0], maxKey, 0, keys},
		"offset mid":  {keys[1], keys[2], 0, keys[1:2]},
		"offset last": {keys[2], maxKey, 0, keys[2:]},
		"offset post": {roachpb.Key("x"), maxKey, 0, []roachpb.Key{}},
	...
	for name, enableSeparatedIntents := range map[string]bool{"interleaved": false, "separated": true} {
		t.Run(name, func(t *testing.T) {
			...
			for name, tc := range testcases {
				tc := tc
				t.Run(name, func(t *testing.T) {
					intents, err := ScanIntents(ctx, eng, tc.from, tc.to, tc.max)
					require.NoError(t, err)
