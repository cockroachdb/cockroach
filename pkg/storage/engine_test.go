...
func TestEngineFSFileNotFoundError(t *testing.T) {
	...
	data := "random data"
	if f, err := env.Create(fname, UnspecifiedWriteCategory); err != nil {
...
func TestFS(t *testing.T) {
	...
	for name, loc := range engineDest {
		t.Run(name, func(t *testing.T) {
			...
			f, err := e.Create(path("a/b/c/foo"), UnspecifiedWriteCategory)
			require.NoError(t, err)
			require.NoError(t, f.Close())
			expectLS(path("a/b/c"), []string{"foo"})

			// Create a file at a/b/c/bar.
			f, err = e.Create(path("a/b/c/bar"), UnspecifiedWriteCategory)
			require.NoError(t, err)
			require.NoError(t, f.Close())
...
func TestGetIntent(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(string(test.key), func(t *testing.T) {
			intent, err := GetIntent(ctx, eng, test.key, fs.UnknownReadCategory)
			if test.expErr {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
