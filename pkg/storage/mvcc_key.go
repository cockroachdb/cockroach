...
// IsEmpty returns true iff there is a non-zero key or timestamp.
func (k MVCCKey) IsEmpty() bool {
	return k.Key == nil && k.Timestamp.IsEmpty()
}
...
func (k MVCCRangeKey) Compare(o MVCCRangeKey) int {
	...
	if k.Timestamp.IsEmpty() && !o.Timestamp.IsEmpty() {
		...
		return -c // MVCC sorts timestamps in reverse
	}
