...
func TestCockroachComparer(t *testing.T) {
	...
	if x := PebbleComparer.Compare(EncodeKey(keyAMetadata), EncodeKey(keyA1)); x != -1 {
		t.Errorf("expected key metadata to sort first got: %d", x)
	}
	if x := PebbleComparer.Compare(EncodeKey(keyA2), EncodeKey(keyA1)); x != -1 {
		t.Errorf("expected higher timestamp to sort first got: %d", x)
	}
	if x := PebbleComparer.Compare(EncodeKey(keyA2), EncodeKey(keyB2)); x != -1 {
