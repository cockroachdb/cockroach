...
// and its command sizes map. Only safe to call on the leader replica.
...
// safe to call on the replica that is both leaseholder and range leader.
func (r *Replica) QuotaAvailable() int64 {
	r.mu.Lock()
	defer r.mu.Unlock()
	return r.mu.proposalQuota.quota()
}
