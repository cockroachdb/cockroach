...
// AssertionFailedf on any failures. It is intended to be called after complete
// state transitions, e.g. during Valid().
func (i *intentInterleavingIter) assertInvariants() error {
	// If the underlying iterator has errored, that error should be propagated.
	iterValid, err := i.iter.Valid()
	if err != nil {
		if i.valid || i.err == nil {
			return errors.NewAssertionErrorWithWrappedErrf(err, "erroring iter with i.valid=%t i.err=%t",
				i.valid, i.err != nil)
		}
		return nil
	}
	...
	// Don't assert anything for an invalid iterator.
	if !i.valid {
		return nil
	}
	...
	if iterValid {
		if key := i.iter.Key(); !i.iterKey.Equal(key) {
			return errors.AssertionFailedf("i.iterKey %q != iter.UnsafeKey %q", i.iterKey, key)
		}
	}
	if intentValid {
		...
		// TODO(erikgrinaker): should it be?
		if i.intentIterState != pebble.IterValid {
			return errors.AssertionFailedf("i.intentKey=%q, but i.intentIterState=%v",
		...
		if i.intentKeyAsNoTimestampMVCCKey != nil &&
			...
			return errors.AssertionFailedf("i.intentKeyAsNoTimestampMVCCKey=%q but i.intentKey=%q",
	...
	if iterValid && intentValid {
		if i.intentCmp != i.intentKey.Compare(i.iterKey.Key) {
			return errors.AssertionFailedf("i.intentCmp=%v does not match intentKey=%q iterKey=%q",
				i.intentCmp, i.intentKey, i.iterKey)
		}
	} else if iterValid {
	...
	if i.dir > 0 && i.isCurAtIntentIterForward() {
		if hasPoint, hasRange := i.iter.HasPointAndRange(); !hasPoint {
			return errors.AssertionFailedf(
				"i.intentKey=%q with i.iterKey=%q on bare range key in forward direction",
				i.intentKey, i.iterKey)
		} else if hasRange {
			if bounds := i.iter.RangeBounds(); !bounds.ContainsKey(i.intentKey) {
				return errors.AssertionFailedf("i.iter.RangeBounds=%q does not cover i.intentKey=%q",
					bounds, i.intentKey)
			}
		}
	}
