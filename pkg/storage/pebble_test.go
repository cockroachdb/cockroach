...
func makeRandEncodedKeys() [][]byte {
	...
	for i := range keys {
		...
		if rng.Int31n(100) < 3 {
			// 3% of keys have a local timestamp.
			k.LocalTimestamp.WallTime = rng.Int63n(5)
			if rng.Int31n(5) == 0 {
				// Of which, 20% have a logical component to the local timestamp.
				k.LocalTimestamp.Logical = rng.Int31n(4) + 1
			}
		}
