...
func TestPebbleMVCCTimeIntervalCollectorAndFilter(t *testing.T) {
	...
	// Write point keys at 3-6 and range keys at 5-8.
	...
	require.NoError(t, eng.PutMVCC(pointKey("a", 6), stringValue("a6")))
	...
	require.NoError(t, eng.ExperimentalPutMVCCRangeKey(rangeKey("b", "c", 7), MVCCValue{}))
	require.NoError(t, eng.ExperimentalPutMVCCRangeKey(rangeKey("b", "c", 8), MVCCValue{}))
	...
	testcases := map[string]struct {
		...
		"all": {wallTS(1), wallTS(10), IterKeyTypePointsAndRanges, []interface{}{
			...
			pointKV("a", 4, "a4"),
			pointKV("a", 3, "a3"),
			rangeKV("b", "c", 8, MVCCValue{}),
			rangeKV("b", "c", 7, MVCCValue{}),
