...
func TestPebbleMVCCTimeIntervalCollectorAndFilter(t *testing.T) {
	...
	// Range keys [b-c)@5, [c-d)@7, [d-e)@9 in separate blocks in a single SST.
	//
	// TODO(erikgrinaker): Seems like these all end up in the same block anyway?
	...
	// Range keys [x-z)@5, [x-z)@7 in the same block in a single SST.
...
// TestPebbleMVCCTimeIntervalWithRangeClears tests that point and range key
// time interval collection and filtering works in the presence of
...
func TestPebbleMVCCTimeIntervalWithRangeClears(t *testing.T) {
	...
	testcases := map[string]struct {
		...
		// TODO(erikgrinaker): We sometimes see range keys reappear despire the
		// clear, possibly a race condition with compations or flushes.
