...
func TestIntentFailureHandling(t *testing.T) {
	...
	t.Run("Filter all failing intents by key range", checkReportedErrors([]TestValue{
		...
		intent(key(10001), "after end", ts(1002)),
	}, []int{}))

	testDataCount := maxIntentsPerWriteIntentError+1
	...
	t.Run("Receive no more than limit intents", checkReportedErrors(testData, expectedErrors[:maxIntentsPerWriteIntentError]))
}
