...
func TestEagerReplication(t *testing.T) {
	...
	if err := server.WaitForInitialSplits(
		store.DB(), server.ExpectedInitialRangeCountWithoutMigrations(),
	); err != nil {
		t.Fatal(err)
	}
	...
	testutils.SucceedsSoon(t, func() error {
		...
		expected := server.ExpectedInitialRangeCountWithoutMigrations()
