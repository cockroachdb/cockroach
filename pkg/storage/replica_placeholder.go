...
//
// Placeholders are inserted in `(*Store).processRaftSnapshotRequest)` when a
// Raft Ready cycle plans to apply a snapshot and the snapshot overlaps keyspace
// that is not currently associated to a Replica on the Store, and contain a
// descriptor that refers to the RangeID of the currently active Range (i.e.
// that addressed by the snapshot) and contains as a key range only the "empty"
// (i.e. not associated to a Replica prior to the snapshot) keyspace (i.e. it
// isn't necessarily the key range of the snapshot, but could just be a part of
// it).
//
