...
func (m mvccCheckForAcquireLockOp) run(ctx context.Context) string {
	...
	err := storage.MVCCCheckForAcquireLock(ctx, writer, txn, m.strength, m.key, int64(m.maxLockConflicts), m.targetBytesPerLockConflict)
	if err != nil {
		return fmt.Sprintf("error: %s", err)
	}
...
func (m mvccAcquireLockOp) run(ctx context.Context) string {
	...
	err := storage.MVCCAcquireLock(ctx, writer, txn, m.strength, m.key, nil, int64(m.maxLockConflicts), m.targetBytesPerLockConflict)
	if err != nil {
		return fmt.Sprintf("error: %s", err)
	}
...
func (m mvccDeleteRangeUsingRangeTombstoneOp) run(ctx context.Context) string {
	...
	err := storage.MVCCDeleteRangeUsingTombstone(ctx, writer, nil, m.key, m.endKey, m.ts,
		hlc.ClockTimestamp{}, m.key, m.endKey, false /* idempotent */, math.MaxInt64, 0, /* maxLockConflicts */
		nil /* msCovered */)
	if err != nil {
		return fmt.Sprintf("error: %s", err)
	}
