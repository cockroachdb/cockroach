...
func createTestPebbleVarOpts(path string, seed int64) (storage.Engine, error) {
	...
	opts.BytesPerSync = int(rngIntRange(rng, 1 << 8, 1 << 20))
	opts.LBaseMaxBytes = rngIntRange(rng, 1 << 8, 128 << 20)
	opts.L0CompactionThreshold = int(rngIntRange(rng, 1, 8))
	opts.L0StopWritesThreshold = int(rngIntRange(rng, 1, 32))
	for i := range opts.Levels {
		if i == 0 {
			opts.Levels[i].BlockRestartInterval = int(rngIntRange(rng, 1, 32))
			opts.Levels[i].BlockSize = int(rngIntRange(rng, 128, 8192))
			opts.Levels[i].BlockSizeThreshold = int(rngIntRange(rng, 40, 180))
			opts.Levels[i].IndexBlockSize = opts.Levels[i].BlockSize
			opts.Levels[i].TargetFileSize = rngIntRange(rng, 1 << 8, 4 << 20)
		} else {
			opts.Levels[i] = opts.Levels[i-1]
			opts.Levels[i].TargetFileSize = opts.Levels[i-1].TargetFileSize * 2
		}
	}
	opts.MaxManifestFileSize = rngIntRange(rng, 1, 256 << 20)
	opts.MaxOpenFiles = int(rngIntRange(rng, 10, 2000))
	opts.MemTableSize = int(rngIntRange(rng, 1 << 8, 8 << 20))
	opts.MemTableStopWritesThreshold = int(rngIntRange(rng, 1, 4))
	...
	opts.Cache = pebble.NewCache(rngIntRange(rng, 1 << 8, 2 << 20))
	defer opts.Cache.Unref()

	fmt.Printf("pebble options:\n%s\n", opts.String())
