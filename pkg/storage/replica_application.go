...
func (r *Replica) handleCommittedEntriesRaftMuLocked(
	...
	for len(toProcess) > 0 {
		if haveDecodedEntry {
			haveDecodedEntry = false
			b.add(&toProcess[0], decodeBuf)
			toProcess = toProcess[1:]
		} else {
			var numEmptyEntries int
			haveDecodedEntry, numEmptyEntries, toProcess, errExpl, err = b.decode(
				ctx, toProcess, &decodeBuf)
			if err != nil {
...
// retreiveLocalProposals populates the proposal and ctx fields for each of the
// commands in b.
...
// and violations of the LeaseAppliedIndex (in which case the proposal is
// retried if it was proposed locally).
//
...
func (r *Replica) applyRaftCommandToBatch(
	...
	if haveTruncatedState {
		apply, err := handleTruncatedStateBelowRaft(ctx, replicaState.TruncatedState,
			cmd.replicatedResult().State.TruncatedState, r.raftMu.stateLoader, writer)
		if err != nil {
			return err
		}
		if !apply {
