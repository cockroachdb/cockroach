...
func (r *Replica) handleCommittedEntriesRaftMuLocked(
	...
	prevReason refreshRaftReason,
) (refreshReason refreshRaftReason, expl string, err error) {
	...
	for len(committedEntries) > 0 {
		...
		sawEmptyEntry, committedEntries, expl, err = b.decode(ctx, committedEntries)
		...
		if err != nil {
			return refreshReason, expl, err
		}
		r.retreiveLocalProposals(ctx, b)
		...
		for ok := it.init(&b.appStates); ok; ok = it.next() {
			...
			updatedTruncatedState := applyTrivialReplicatedEvalResult(s.ctx,
				s.rResult(), s.e.Index, s.leaseIndex, &b.replicaState)
	...
	return refreshReason, "", nil
}

func (r *Replica) retreiveLocalProposals(ctx context.Context, b *entryApplicationBatch) {
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	var writeBatch *storagepb.WriteBatch
	{
		if filter := r.store.cfg.TestingKnobs.TestingApplyFilter; s.forcedErr == nil && filter != nil {
			...
			newPropRetry, s.forcedErr = filter(storagebase.ApplyFilterArgs{
				CmdID:                s.idKey,
				ReplicatedEvalResult: *s.rResult(),
		...
		if s.forcedErr != nil {
			...
			*s.rResult() = storagepb.ReplicatedEvalResult{}
		...
		if deprecatedDelta := s.rResult().DeprecatedDelta; deprecatedDelta != nil {
			s.rResult().Delta = deprecatedDelta.ToStatsDelta()
			s.rResult().DeprecatedDelta = nil
		}
		...
		if s.rResult().AddSSTable != nil {
			...
			s.rResult().AddSSTable = nil
		}

		if s.rResult().Split != nil {
			...
			splitPreApply(ctx, tmpBatch, s.rResult().Split.SplitTrigger)
			writeBatch.Data = tmpBatch.Repr()
			tmpBatch.Close()
		}

		if merge := s.rResult().Merge; merge != nil {
		...
		{
			var err error
			*s.rResult(), err = r.applyRaftCommandToBatch(
				s.ctx, s.idKey, *s.rResult(), s.e.Index, s.leaseIndex, writeBatch, replicaState, batch, writeAppliedState)
...
func (r *Replica) applyBatch(
	...
	if s := b.appStates.last(); !isTrivial(s.rResult(), b.replicaState.UsingAppliedStateKey) {
		...
		if unlock := s.splitMergeUnlock; unlock != nil {
			defer unlock(s.rResult())
			s.splitMergeUnlock = nil
		}
		if s.rResult().BlockReads {
			r.readOnlyCmdMu.Lock()
			defer r.readOnlyCmdMu.Unlock()
			s.rResult().BlockReads = false
		}
	}
	...
	*r.mu.state.Stats = *b.replicaState.Stats
	...
	for ok := it.init(&b.appStates); ok; ok = it.next() {
		s := it.state()
		applyRaftLogDelta(s.rResult().RaftLogDelta)
	...
	for ok := it.init(&b.appStates); ok; ok = it.next() {
		...
		clearTrivialReplicatedEvalResultFields(s.rResult(), b.replicaState.UsingAppliedStateKey)
		isNonTrivialEntry := batchIsNonTrivial && it.isLast()
		if isNonTrivialEntry && !r.handleComplexReplicatedEvalResult(s.ctx, *s.rResult()) {
			panic("non-trivial batch did not require state assertion")
		}
		if !isNonTrivialEntry && !s.rResult().Equal(storagepb.ReplicatedEvalResult{}) {
			panic(fmt.Errorf("failed to handle all side-effects of ReplicatedEvalResult: %v", s.rResult()))
		}
		...
		case raftpb.EntryNormal:
			if s.rResult().ChangeReplicas != nil {
...
func clearTrivialReplicatedEvalResultFields(
	rResult *storagepb.ReplicatedEvalResult, usingAppliedStateKey bool,
) {
...
// prepareLocalResultis performed after the command has been committed to the
...
func (r *Replica) finishRaftCommand(
	...
	if expensiveSplitAssertion && s.rResult().Split != nil {
		split := s.rResult().Split
	...
	return s.rResult().ChangeReplicas != nil
}
...
func (b *entryApplicationBatch) decode(
	...
	for i := range toProcess {
		...
		if expl, err := s.decode(ctx, e); err != nil {
			return sawEmptyEntry, nil, expl, err
		}
		// This is a non-trivial entry which needs to be processed last in this
		// batch.
		if !isTrivial(s.rResult(), b.replicaState.UsingAppliedStateKey) {
			return sawEmptyEntry, toProcess[i+1:], "", nil
		}
	}
	return sawEmptyEntry, nil, "", nil
}
