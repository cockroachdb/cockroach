...
func (a *Allocator) ComputeAction(
	...
	// We're adjusting the replication factor for system ranges so that if there are less nodes than
	// replicas specified in the zone config, the cluster can still function.
	if bytes.HasPrefix(rangeInfo.Desc.EndKey, keys.SystemPrefix) ||
		bytes.HasPrefix(rangeInfo.Desc.EndKey, keys.NodeLivenessPrefix) {
