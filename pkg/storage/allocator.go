...
func (a *Allocator) ComputeAction(
	...
	if have < need {
		...
		if log.V(3) {
			log.Infof(ctx, "add - need=%d, have=%d, priority=%.2f", need, have, priority)
		}
	...
	if len(deadReplicas) > 0 {
		...
		if log.V(3) {
			log.Infof(ctx, "remove dead - dead=%d, live=%d, quorum=%d, priority=%.2f",
	...
	if have > need {
		...
		if log.V(3) {
			log.Infof(ctx, "remove - need=%d, have=%d, priority=%.2f", need, have, priority)
		}
