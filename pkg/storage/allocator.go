...
func (a Allocator) shouldTransferLeaseUsingStats(
	...
	for _, repl := range existing {
		...
		remoteLatency, ok := a.rpcContext.RemoteClocks.Latency(addr.String())
		if !ok {
			continue
		}
		...
		rebalanceThreshold :=
			baseRebalanceThreshold - 0.1*math.Log10(remoteWeight/localWeight)*math.Log1p(remoteLatencyMillis)
