...
func (m *modelTimeSeriesDataStore) PruneTimeSeries(
	ctx context.Context, e engine.Engine, start, end roachpb.RKey, db *client.DB, now hlc.Timestamp,
) error {
	if e == nil {
		m.t.Fatal("PruneTimeSeries was passed a nil engine")
	}
	if db == nil {
...
func TestTimeSeriesMaintenanceQueueServer(t *testing.T) {
	...
	// datapoints at the current time and one datapoint older than the pruning
