...
func (qp *quotaPool) acquire(ctx context.Context, v int64) error {
	...
	for {
		select {
		case <-slowTimer.C:
			log.Warningf(ctx, "have been waiting %s attempting to acquire quota, quota available: %d",
				base.SlowRequestThreshold, qp.quota())
		case <-ctx.Done():
...
// quota returns the currently available quota in the pool. Safe for concurrent
// use.
