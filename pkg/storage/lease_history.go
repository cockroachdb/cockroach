...
// how much memory they take up. Turning them on will encour some memory usage.
var leaseHistoryMaxEntries = envutil.EnvOrDefaultInt("COCKROACH_LEASE_HISTORY", 0)
...
func (lh *leaseHistory) get() []roachpb.Lease {
	...
	firstResult := make([]roachpb.Lease, len(first))
	secondResult := make([]roachpb.Lease, len(second))
	copy(firstResult, first)
	copy(secondResult, second)
	return append(firstResult, secondResult...)
}
