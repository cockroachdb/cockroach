...
const defaultLeaseHistoryMaxEntries = 0

// leaseHistoryMaxEntries controls if replica lease histories are enabled. Turning
// them on will encour some memory usage.
var leaseHistoryMaxEntries = envutil.EnvOrDefaultInt("COCKROACH_LEASE_HISTORY", defaultLeaseHistoryMaxEntries)
...
func (lh *leaseHistory) get() []roachpb.Lease {
	...
	if len(lh.history) < leaseHistoryMaxEntries {
		result := make([]roachpb.Lease, len(lh.history))
		copy(result, lh.history)
		return lh.history
	}
	return append(lh.history[lh.index:], lh.history[:lh.index]...)
}
