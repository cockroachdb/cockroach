...
func (buf *cmdAppCtxBuf) allocate() *cmdAppCtx {
	if buf.tail == nil { // lazy initialization
		n := entryApplicationStateBufNodeSyncPool.Get().(*cmdAppCtxBufNode)
		buf.head, buf.tail = n, n
	}
	if buf.tail.len == cmdAppCtxBufNodeSize {
		newTail := entryApplicationStateBufNodeSyncPool.Get().(*cmdAppCtxBufNode)
...
// cur returns the entryApplicationState currently pointed to by it.
