...
func StripTablePrefixE(key roachpb.Key) ([]byte, error) {
	var err error
	key, err = StripTenantPrefixE(key)
	if err != nil {
		return nil, err
	}
...
func StripIndexPrefixE(key roachpb.Key) ([]byte, error) {
	var err error
	key, err = StripTablePrefixE(key)
	if err != nil {
		return nil, err
	}
	if encoding.PeekType(key) != encoding.Int {
