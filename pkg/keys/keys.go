...
func LockTableSingleKey(key roachpb.Key, buf []byte) (roachpb.Key, []byte) {
	// The +3 accounts for the bytesMarker and terminator. Note that this is an
	// estimate, since the escaping done by EncodeBytesAscending depends on what
	// bytes are in the key.
