...
// RangeProbeKey returns a range-local key for blackbox probing. The
// purpose of the key is test CRDB in production; if any data is present at
// the key, it has no purpose except in allowing testing CRDB in production.
func RangeProbeKey(key roachpb.RKey) roachpb.Key {
	return MakeRangeKey(key, RangeProbeSuffix, nil)
}
