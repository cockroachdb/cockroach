...
func RaftLogKey(rangeID roachpb.RangeID, logIndex roachpb.RaftIndex) roachpb.Key {
...
func RaftLogKeyFromPrefix(raftLogPrefix []byte, logIndex roachpb.RaftIndex) roachpb.Key {
...
func DecodeRaftLogKeyFromSuffix(raftLogSuffix []byte) (roachpb.RaftIndex, error) {
	_, logIndex, err := encoding.DecodeUint64Ascending(raftLogSuffix)
	return roachpb.RaftIndex(logIndex), err
}
...
// RaftLogKey returns a system-local key for a Raft log entry.
func (b RangeIDPrefixBuf) RaftLogKey(logIndex roachpb.RaftIndex) roachpb.Key {
