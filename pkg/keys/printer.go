...
var (
	...
	keyDict = []struct {
		...
		{name: "/Meta1", start: Meta1Prefix, end: Meta1KeyMax, entries: []dictEntry{
			{name: "", prefix: Meta1Prefix, ppFunc: printMetaKeyValue,
				psFunc: func(input string) (string, roachpb.Key) {
		...
		{name: "/Meta2", start: Meta2Prefix, end: Meta2KeyMax, entries: []dictEntry{
			{name: "", prefix: Meta2Prefix, ppFunc: printMetaKeyValue,
				psFunc: func(input string) (string, roachpb.Key) {
					input = mustShiftSlash(input)
					unq, err := strconv.Unquote(input)
					if err != nil {
...
func printMetaKeyValue(key roachpb.Key, valueBytes []byte) (string, string) {
	return print(key), metaPrint(valueBytes)
}
...
// PrettyPrintKeyOnly just pretty print key only.
func PrettyPrintKeyOnly(key roachpb.Key) string {
	str, _ := PrettyPrint(key, []byte{})
	return str
}
...
outer:
	...
	if out := PrettyPrintKeyOnly(output); out != origInput {
...
func init() {
	...
	roachpb.PrettyPrintKey = PrettyPrintKeyOnly
}
...
func PrettyPrintRange(b *bytes.Buffer, start, end roachpb.Key, maxChars int) {
	...
	prettyStart := PrettyPrintKeyOnly(start)
	prettyEnd := PrettyPrintKeyOnly(end)
	i := 0
