...
// keyOverride defines an override for how to translate some pretty-printed keys.
...
var (
	// ConstKeyOverrides defines the default override configuration for how to
	// translate some pretty-printed keys.
	ConstKeyOverrides = []keyOverride{
		{"/Max", MaxKey},
		{"/Min", MinKey},
...
var lockTablePrintLockedKey func(
	valDirs []encoding.Direction, key roachpb.Key, quoteRawKeys QuoteOpt, overrides []keyOverride,
) string
...
func localRangeLockTablePrint(valDirs []encoding.Direction, key roachpb.Key) string {
	...
	buf.WriteString(lockTablePrintLockedKey(valDirs, lockedKey, QuoteRaw, ConstKeyOverrides))
	return buf.String()
}
...
//
// We provide a list of KeyOverrides to handle scenarios where we want to quickly resolve
// a key name and exit. By default, this is ConstKeyOverrides. This is configurable to
// provide sibling functions like SafeFormat, which may recursively call into
// prettyPrintInternal to format the remainder of a key, to avoid the remainder being
// treated as an override from ConstKeyOverrides. For example: The key `/Meta1/""` would
// have `/Meta1` trimmed from the key, and prettyPrintInternal would be called for a
// remainder of `""`. This would incorrectly be interpreted as `/Min` if we had
// hardcoded usage of ConstKeyOverrides.
...
func PrettyPrint(valDirs []encoding.Direction, key roachpb.Key) string {
	return prettyPrintInternal(valDirs, key, QuoteRaw, ConstKeyOverrides)
}
...
func safeFormatInternal(valDirs []encoding.Direction, key roachpb.Key) redact.RedactableString {
	...
	helper := func(key roachpb.Key, isRemainder bool) redact.RedactableString {
		...
		keyOverrides := ConstKeyOverrides
		if isRemainder {
			// Avoid treating remainder keys as relevant to `ConstKeyOverrides`
			// when falling back on prettyPrintInternal.
		...
		//		Redacted:   `‹x›`
		return redact.Sprint(prettyPrintInternal(valDirs, key, QuoteRaw, keyOverrides))
	}
...
func PrettyPrintRange(start, end roachpb.Key, maxChars int) string {
	...
	prettyStart := prettyPrintInternal(nil /* valDirs */, start, DontQuoteRaw, ConstKeyOverrides)
	if len(end) == 0 {
	...
	prettyEnd := prettyPrintInternal(nil /* valDirs */, end, DontQuoteRaw, ConstKeyOverrides)
	i := 0
