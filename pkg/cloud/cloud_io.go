...
func ResumingReaderRetryOnErrFnForSettings(
	...
	return func(err error) bool {
		...
		// We could test this error specifically for ETIMEOUT
		// by grabbing the Errno:
		//
		//    err.(*net.OpError).Err.(*os.SyscallError).Err.(syscall.Errno)
		//
		// But that assumes that those error types surive all
		// the way here along all call paths.
		//
		// For now, we test for the string instead.
...
func (r *ResumingReader) Open(ctx context.Context) error {
	return DelayedRetry(ctx, "Open", r.ErrFn, func() error {
		...
		if readErr != nil {
			errors.Wrapf(readErr, "open %s", r.Filename)
		}
		return nil
	})
}
