...
func makeS3Storage(
	...
	s, err := cloud.MakeExternalStorage(ctx, conf, base.ExternalIODirConfig{}, testSettings, clientFactory, nil, nil, nil, nil)
	if err != nil {
		return nil, err
	}
...
func TestPutS3(t *testing.T) {
	...
	t.Run("auth-empty-no-cred", func(t *testing.T) {
		_, err := cloud.ExternalStorageFromURI(ctx, fmt.Sprintf("s3://%s/%s", bucket,
			"backup-test-default"), base.ExternalIODirConfig{}, testSettings, blobs.TestEmptyBlobClientFactory, user, nil, nil, nil, nil)
...
func TestPutS3Endpoint(t *testing.T) {
	...
	cloudtestutils.CheckExportStore(t, u.String(), false, user, nil, nil, nil, testSettings)
}
