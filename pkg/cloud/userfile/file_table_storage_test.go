...
func TestPutUserFileTable(t *testing.T) {
	...
	t.Run("reject-normalized-basename", func(t *testing.T) {
		...
		store, err := cloud.ExternalStorageFromURI(ctx, userfileURL.String()+"/", base.ExternalIODirConfig{}, cluster.NoSettings, blobs.TestEmptyBlobClientFactory, username.RootUserName(), ie, cf, kvDB, nil)
...
func TestUserScoping(t *testing.T) {
	...
	// Write file as user1.
	fileTableSystem1, err := cloud.ExternalStorageFromURI(ctx, dest, base.ExternalIODirConfig{}, cluster.NoSettings, blobs.TestEmptyBlobClientFactory, user1, ie, cf, kvDB, nil)
	require.NoError(t, err)
	...
	fileTableSystem2, err := cloud.ExternalStorageFromURI(ctx, dest, base.ExternalIODirConfig{}, cluster.NoSettings, blobs.TestEmptyBlobClientFactory, user2, ie, cf, kvDB, nil)
	require.NoError(t, err)
	...
	fileTableSystem3, err := cloud.ExternalStorageFromURI(ctx, dest, base.ExternalIODirConfig{}, cluster.NoSettings, blobs.TestEmptyBlobClientFactory, username.RootUserName(), ie, cf, kvDB, nil)
	require.NoError(t, err)
