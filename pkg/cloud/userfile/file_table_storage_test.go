...
func TestPutUserFileTable(t *testing.T) {
	...
	cloudtestutils.CheckListFiles(t, "userfile://defaultdb.public.file_list_table/listing-test/basepath", username.RootUserName(), ie, cf, kvDB, testSettings)
	...
	t.Run("empty-qualified-table-name", func(t *testing.T) {
		...
		cloudtestutils.CheckListFilesCanonical(t, "userfile:///listing-test/basepath", "userfile://defaultdb.public.userfiles_root/listing-test/basepath", username.RootUserName(), ie, kvDB, testSettings, cf)
	})
	...
	t.Run("reject-normalized-basename", func(t *testing.T) {
		...
		store, err := cloud.ExternalStorageFromURI(ctx, userfileURL.String()+"/", base.ExternalIODirConfig{}, cluster.NoSettings, blobs.TestEmptyBlobClientFactory, username.RootUserName(), ie, nil, kvDB, nil)
...
func TestUserScoping(t *testing.T) {
	...
	// Write file as user1.
	fileTableSystem1, err := cloud.ExternalStorageFromURI(ctx, dest, base.ExternalIODirConfig{}, cluster.NoSettings, blobs.TestEmptyBlobClientFactory, user1, ie, nil, kvDB, nil)
	require.NoError(t, err)
	...
	fileTableSystem2, err := cloud.ExternalStorageFromURI(ctx, dest, base.ExternalIODirConfig{}, cluster.NoSettings, blobs.TestEmptyBlobClientFactory, user2, ie, nil, kvDB, nil)
	require.NoError(t, err)
	...
	fileTableSystem3, err := cloud.ExternalStorageFromURI(ctx, dest, base.ExternalIODirConfig{}, cluster.NoSettings, blobs.TestEmptyBlobClientFactory, username.RootUserName(), ie, nil, kvDB, nil)
	require.NoError(t, err)
