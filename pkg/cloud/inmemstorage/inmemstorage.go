...
// New returns a new, empty in-memory cloud.ExternalStorage. See the package
// doc for the intended use cases and limitations.
func New() cloud.ExternalStorage {
	return &Storage{files: make(map[string]*bytes.Buffer)}
}
...
type Storage struct {
	...
	// Writer.Close call. The buffer pointer for a given key is replaced
	// atomically on overwrite so that an outstanding reader continues to see
	// the bytes it observed at ReadFile time.
...
// Settings returns a freshly minted testing settings instance. This
// implementation does not consult any cluster setting; the method exists only
// to satisfy the cloud.ExternalStorage interface.
func (s *Storage) Settings() *cluster.Settings { return cluster.MakeTestingClusterSettings() }
