...
func TestWithInitialContents(t *testing.T) {
	defer leaktest.AfterTest(t)()
	ctx := context.Background()
	original := []byte("seed")
	s := inmemstorage.New(inmemstorage.WithInitialContents(map[string][]byte{
		"seeded": original,
	}))
	defer func() { require.NoError(t, s.Close()) }()

	data, _ := readAll(t, ctx, s, "seeded", cloud.ReadOptions{})
	require.Equal(t, []byte("seed"), data)

	// Mutating the caller's slice does not leak into the storage.
	original[0] = 'X'
	data, _ = readAll(t, ctx, s, "seeded", cloud.ReadOptions{})
	require.Equal(t, []byte("seed"), data)
}
