...
const (
	Implicit AzureAuth = iota
	RoleBased
	// Legacy auth not supported on Azure KeyVault.
)
...
func resolveKMSURIParams(kmsURI *url.URL) (kmsURIParams, error) {
	...
	var kmsAuth AzureAuth
	if auth == cloudpb.ExternalStorage_Azure_AUTH_EXPLICIT {
		kmsAuth = RoleBased
	} else if auth == cloudpb.ExternalStorage_Azure_AUTH_IMPLICIT {
		kmsAuth = Implicit
	} else {
...
func MakeAzureKMS(ctx context.Context, uri string, env cloud.KMSEnv) (cloud.KMS, error) {
	...
	if kmsURIParams.auth == RoleBased {
	...
	if kmsURIParams.auth == Implicit {
		if kmsURIParams.clientID != "" {
			missingParams = append(extraParams, AzureClientIDParam)
		}
		if kmsURIParams.clientSecret != "" {
			missingParams = append(extraParams, AzureClientSecretParam)
		}
		if kmsURIParams.tenantID != "" {
			missingParams = append(extraParams, AzureTenantIDParam)
		}
	}
