...
func MakeGCSKMS(uri string, env cloud.KMSEnv) (cloud.KMS, error) {
	...
	// Check the given cmk to see if key version is given.
	// If it is, strip it from the cmk.
	cmkID := strings.TrimPrefix(kmsURI.Path, "/")
	versionIndex := strings.Index(cmkID, "/cryptoKeyVersions/")
	if versionIndex != -1 {
		cmkID = strings.Split(cmkID, "/cryptoKeyVersions/")[0]
	}
