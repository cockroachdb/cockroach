...
func CheckExportStore(
	...
	t.Run("read-single-file-by-uri", func(t *testing.T) {
		...
		singleFile := storeFromURI(ctx, t, appendPath(t, storeURI, testingFilename), clientFactory, user, ie, cf, kvDB, testSettings)
...
func uploadData(
	...
	s, err := cloud.MakeExternalStorage(ctx, dest, base.ExternalIODirConfig{}, testSettings, nil, nil, nil, nil, nil)
	require.NoError(t, err)
...
func CheckAntagonisticRead(
	...
	ctx := context.Background()
	s, err := cloud.MakeExternalStorage(ctx, conf, base.ExternalIODirConfig{}, testSettings, nil, nil, nil, nil, nil)
	require.NoError(t, err)
	defer s.Close()
