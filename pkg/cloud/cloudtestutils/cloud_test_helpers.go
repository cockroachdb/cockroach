...
func checkExportStore(t *testing.T, info StoreInfo, skipSingleFile bool) {
	...
	t.Run("simple round trip", func(t *testing.T) {
		...
		for i := 0; i < 10; i++ {
			...
			defer r.Close(ctx) //nolint:deferloop
	...
	t.Run("List", func(t *testing.T) {
		...
		for _, tc := range []struct {
			...
			},
			{
				...
				info.URI,
				"",
				cloud.ListOptions{AfterKey: "file/abc/B"},
				[]string{"/file/abc/B.csv", "/file/abc/C.csv", "/file/letters/dataA.csv", "/file/letters/dataB.csv", "/file/letters/dataC.csv", "/file/numbers/data1.csv", "/file/numbers/data2.csv", "/file/numbers/data3.csv"},
			},
			{
