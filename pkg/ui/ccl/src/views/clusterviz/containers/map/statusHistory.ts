...
export default class NodeStatusHistory {
    ...
    private computeClientActivityRate() {
        ...
        if (this.statusHistory.length > 1) {
            ...
            const totalQPS = (latest.metrics["sql.select.count"] - prev.metrics["sql.select.count"]) +
                (latest.metrics["sql.distsql.select.count"] - prev.metrics["sql.distsql.select.count"]) +
            ...
            this.clientActivityRate = totalQPS / seconds;
        }
    }
}
