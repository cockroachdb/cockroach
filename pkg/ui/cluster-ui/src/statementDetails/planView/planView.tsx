...
import { Fraction } from "../../../dist/types/statementDetails/statementDetails.d";
...
function distributionToString(dist: Fraction): string {
  if (Number.isNaN(dist.numerator)) {
    return "unknown";
  }
  if (dist.numerator === 0) return "local";

  return dist.numerator === dist.denominator ? "full" : "partial";
}

function vecToString(vec: Fraction): string {
  if (Number.isNaN(vec.numerator)) {
    return "unknown";
  }
...
function NodeAttribute({ attribute }: { attribute: FlatPlanNodeAttribute }) {
...
function PlanNodes({ node }: { node: FlatPlanNode }): React.ReactElement<{}> {
  return (
    <ul>
      <PlanNode node={node} />
    </ul>
...
function PlanNode({ node }: PlanNodeProps) {
  ...
  return (
    <li>
      ...
      {node.children &&
        node.children.map((child, idx) => <PlanNodes node={child} key={idx} />)}
    </li>
  );
}

type GlobalPropertiesType = {
...
export class PlanView extends React.Component<PlanViewProps, PlanViewState> {
  ...
  render() {
    ...
    const globalAttrs: FlatPlanNodeAttribute[] = [
      {
        key: "distribution",
        values: [distributionToString(globalProperties.distribution)],
        warn: false,
      },
      {
        key: "vectorized",
        values: [vecToString(globalProperties.vectorized)],
        warn: false,
      },
    ];
            ...
            >
              <div className={cx("plan-view-container")}>
                <div id="plan-view-inner-container" ref={this.innerContainer}>
                  ...
                  <PlanNodes node={flattenedPlanNodeRoot} />
                </div>
              </div>
            </td>
          </tr>
