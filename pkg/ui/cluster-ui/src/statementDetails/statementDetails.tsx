...
> {
  ...
  renderContent = () => {
    ...
    const nodes: string[] = stats.nodes
      ? unique(stats.nodes.map(node => node.toString())).sort()
      : [];
    const regions = stats.nodes
      ? unique(stats.nodes.map(node => nodeRegions[node.toString()])).sort()
      : [];
    const explainPlan =
