...
function sparklineChart(config: SparklineConfig) {
  ...
  return function renderSparkline(sel: Selection<SVGElement, SparklineChartProps, null, undefined>) {
    // TODO(couchand): unsingletonize this
    const { results } = sel.datum();

    xScale.domain(extent(results, (d: Datapoint) => d.timestamp) as [number, number]);
    yScale.domain(extent(results, (d: Datapoint) => d.value) as [number, number]);
