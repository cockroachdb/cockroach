...
function InstructionsBox(): React.ReactElement {
  ...
  const collapsed = useSelector(instructionsBoxCollapsedSelector);
  const { nodeStatuses } = useNodes();
  const hasLocality = allNodesHaveLocality(nodeStatuses ?? []);

  const expand = () =>
    setInstructionsBoxCollapsed(store.dispatch, store.getState, false);
  const collapse = () =>
    setInstructionsBoxCollapsed(store.dispatch, store.getState, true);
        ...
        <ol>
          <li
            className={classNames("instructions-box-content__todo-item", {
              "instructions-box-content__todo-item--done": hasLocality,
            })}
          >
            {hasLocality ? (
