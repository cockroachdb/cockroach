...
function InstructionsBox(): React.ReactElement {
  const dispatch: AppDispatch = useDispatch();
  const collapsed = useSelector((state: AdminUIState) =>
    instructionsBoxCollapsedSelector(state),
  );
  const { nodeStatuses } = useNodes();
  const allHaveLocality = allNodesHaveLocality(nodeStatuses);

  const expand = () => dispatch(setInstructionsBoxCollapsed(false));
  const collapse = () => dispatch(setInstructionsBoxCollapsed(true));
  ...
  return (
    <div className="instructions-box instructions-box--expanded">
      <div className="instructions-box-top-bar">
        ...
        <span className="instructions-box-top-bar__x-out" onClick={collapse}>
          ✕
        </span>
      </div>
        ...
        <ol>
          <li
            className={classNames("instructions-box-content__todo-item", {
              "instructions-box-content__todo-item--done": allHaveLocality,
            })}
          >
            {allHaveLocality ? (
