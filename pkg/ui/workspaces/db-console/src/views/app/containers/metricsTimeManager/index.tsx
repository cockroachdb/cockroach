...
function MetricsTimeManager({
  ...
  const prevTimeScaleRef = useRef<timewindow.TimeScaleState | null>(null);
  ...
  // Run on mount and whenever timeScale changes (deep comparison).
  useEffect(() => {
    if (
      prevTimeScaleRef.current === null ||
      !isEqual(prevTimeScaleRef.current, timeScale)
    ) {
