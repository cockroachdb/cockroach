...
const interpolateSpectral = (t: number): string => {
  ...
  // Simple linear interpolation between colors (simplified)
  return colors[index];
};
...
export const HexagonHeatmap: React.FC<NodeHeatmapProps> = ({
  ...
  useEffect(() => {
    ...
    const legendNode = Legend(colorScale, {
      ...
      marginLeft: 0,
      marginBottom: 30,
      tickValues: metricType === "cpu" ? [0, 75] : undefined,
      tickFormat:
        metricType === "cpu"
          ? (d: any) => (d >= 75 ? "75+" : d.toString())
          : undefined,
      ticks: metricType === "cpu" ? 2 : 5,
    });
