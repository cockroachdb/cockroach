...
import { commonStyles, Visualization } from "@cockroachlabs/cluster-ui";
...
const ClusterExplorerPage = (_props: RouteComponentProps) => {
  ...
  // Sample data sets
  const emptyRangeData: RangeData[] = [
  ...
  // State for current range chart data
  const [currentRangeData, setCurrentRangeData] =
    useState<RangeData[]>(emptyRangeData);
      ...
      try {
        ...
        await Promise.all(
          nodeIds.map(async (nodeId: number) => {
            try {
              // Fetch both CPU and write bytes data
  ...
  const generateNodeData = (): NodeData[] => {
    // Create a roughly square cluster
    // Hexagons are wider than tall, so we need fewer columns and more rows
  ...
  // Function to fetch hot ranges for a specific node
  const fetchHotRangesForNode = async (
    nodeId: number,
  ): Promise<RangeData[]> => {
    try {
      const request = new cockroach.server.serverpb.HotRangesRequest({
        nodes: [String(nodeId)],
        per_node_limit: 20, // Fetch more ranges initially, we'll filter to top N locally
      ...
      // and limit to top N ranges to keep the bar chart readable
      return deduplicatedRangeData
        .sort((a: RangeData, b: RangeData) => b.value - a.value)
        .slice(0, rangesLimit);
    } catch (error) {
  ...
  const handleNodeClick = async (nodeId: number) => {
    // Reset views when selecting new node
    setSelectedNodeId(nodeId);
    setSelectedRangeId(null);

    // Fetch hot ranges data for the clicked node
    try {
      const hotRangesData = await fetchHotRangesForNode(nodeId);

      // Update range chart via state (prop-based update)
      setCurrentRangeData(hotRangesData);
    } catch (error) {
      setCurrentRangeData(emptyRangeData);
    }
  };

  // Store the handlers in refs to avoid creating new functions on every render
  const handleNodeClickRef = useRef(handleNodeClick);
  handleNodeClickRef.current = handleNodeClick;

  // Create stable function references that won't change on re-renders
  const stableHandleNodeClick = useRef((nodeId: number) => {
      ...
      <div style={{ marginTop: "10px", marginBottom: "20px" }}>
        ...
        <select
          ...
          onChange={async e => {
            ...
            setRangesLimit(newLimit);
            // If a node is selected, refresh the range data with the new limit
            if (selectedNodeId) {
              try {
                const hotRangesData =
                  await fetchHotRangesForNode(selectedNodeId);
                setCurrentRangeData(hotRangesData);
              } catch (error) {
                setCurrentRangeData(emptyRangeData);
              }
        ...
        >
          ...
          <option value={30}>30 Ranges</option>
        </select>
      </div>

      <div style={{ display: "flex", gap: "20px", marginTop: "20px" }}>
        {/* Hexagon Heatmap for all nodes */}
        <div>
          <Visualization
          ...
          >
            <HexagonHeatmap
              ...
              onHexagonClick={stableHandleNodeClick}
            />
          </Visualization>
        </div>

        {/* Bar Chart for node ranges */}
        <div>
          <Visualization
            title={
              selectedNodeId
                ? `Range ${getMetricDisplayName()} - Node ${selectedNodeId}`
                : `Range ${getMetricDisplayName()} Metrics`
            }
            subtitle={
              selectedNodeId
                ? `Showing top ${rangesLimit} ranges for node ${selectedNodeId} (${getMetricUnit()})`
                : "Select a node to see range metrics"
            }
          >
            <BarChart
              ref={rangeChartRef}
