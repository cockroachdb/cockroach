...
export const HotRangesFilter = (props: HotRangesFilterProps) => {
  ...
  const nodeIdsOptions = Array.from(new Set(hotRanges.map(r => `${r.node_id}`)))
    .sort()
  ...
  const databaseOptions = Array.from(
    new Set(hotRanges.map(r => r.database_name)),
  )
    .filter(i => !isEmpty(i))
    .sort()
  ...
  // provided list of hot ranges.
  const localitiesOptions = Array.from(nodeIdToLocalityMap.values())
    .sort()
