...
export const HotRangesFilter = (props: HotRangesFilterProps) => {
  ...
  const filtersDisabled = node_ids.length === 0;
  ...
  // to pass in the node_ids.
  const onNodeIdApply = (node_ids: string[]) => {
    applyFilters({ ...localFilters, node_ids });
  };
  ...
  return (
    <Row className={cx("hotranges-dropdown-filter")}>
      <Col className={cx("hotranges-node-selector")}>
        <NodeRegionsSelector
          includeAllOption
          startingNodes={node_ids}
          onApply={onNodeIdApply}
        />
      </Col>
