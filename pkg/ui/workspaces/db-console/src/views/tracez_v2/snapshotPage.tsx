...
const selectSnapshotsState = createSelector(
  [
    (state: AdminUIState) => state.cachedData.snapshots,
    (_state: AdminUIState, key: string) => key,
  ],
  (
    snapshots,
    key,
  ): CachedDataReducerState<api.ListTracingSnapshotsResponseMessage> => {
    if (!snapshots) {
      return null;
    }
...
const mapStateToProps = (
  ...
  const snapshotsState = selectSnapshotsState(state, snapshotsKey());
