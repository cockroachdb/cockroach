...
// Creates a mapping of regions to nodes, based on
// the nodes list provided and nodeRegions, which is
// a full list of node id to the region it resides.
...
// Converts a map of regions to node ids, ordered by region name, e.g.
// Converts:
...
export function nodesByRegionMapToString(
  ...
  const nodesByRegionString = regions
    .map((region: string) => {
