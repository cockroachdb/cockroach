...
function makeTimestamp(date: string): Timestamp {
  return new protos.google.protobuf.Timestamp({
    seconds: new Long(Date.parse(date)),
  });
}
...
class TestDriver {
  ...
  async resetIndexUsageStats() {
    return this.actions.resetIndexUsageStats(this.database, this.table);
  }
}
      ...
      stats: [
        {
          ...
          lastUsedString: "Last read: Nov 19, 2021 at 6:01 PM",
          indexName: "jobs_status_created_idx",
        },
      ],
      lastReset: "Nov 11, 2021 at 3:18 PM (UTC)"
    });
  });
});
