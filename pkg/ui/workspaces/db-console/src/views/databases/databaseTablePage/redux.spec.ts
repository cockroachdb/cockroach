...
class TestDriver {
  ...
  assertIndexStats(expected: DatabaseTablePageIndexStats) {
    // Compare moments first and then remove from object.
    this.properties().indexStats.stats[0].lastUsedTime.isSame(
      expected.stats[0].lastUsedTime,
    );
    delete this.properties().indexStats.stats[0].lastUsedTime;
    delete expected.stats[0].lastUsedTime;
  ...
  it("starts in a pre-loading state", function() {
    driver.assertProperties({
      ...
      indexStats: {
        ...
        loaded: false,
        stats: [],
        lastReset: formatDate(moment(), "MMM DD, YYYY [at] h:mm A [(UTC)]"),
      },
    });
  });
      ...
      stats: [
        {
          ...
          totalReads: 2,
          lastUsedTime: moment("2021-11-19T23:01:05.167627Z"),
          lastUsedString: "Last read: Nov 19, 2021 at 11:01 PM",
        },
      ],
      lastReset: "Nov 12, 2021 at 8:18 PM (UTC)",
    });
  });
});
