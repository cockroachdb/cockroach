...
function drawBucket(pixels: Uint8ClampedArray, x: number, y: number, width: number, height: number, color: number[], showSpanBoundaries: boolean) {
...
export default class KeyVisualizer extends React.PureComponent<KeyVisualizerProps, KeyVisualizerState> {
  ...
  computeBucketDimensions(sampleIndex: number, bucket: KeyVisSamplesResponse.IBucket) {
    const startKey = this.props.keys[bucket.start_key_id];
    const endKey = this.props.keys[bucket.end_key_id];
    ...
    const y =
      this.props.yOffsetsForKey[startKey] * this.yZoomFactor +
      this.yPanOffset;

    const width = ((CanvasWidth - YAxisLabelPadding) * this.xZoomFactor) / nSamples;
    const height = this.props.yOffsetsForKey[endKey] * this.yZoomFactor - y + this.yPanOffset;
  ...
  renderXAxis() {
    const xOffsetForSampleTime = this.props.samples.reduce(
      (acc, sample, index) => {
        const wallTimeMs = sample.timestamp.wall_time.toNumber() / 1e6;
    ...
    requestAnimationFrame(() => {
      ...
      for (let i = 0; i < nSamples; i++) {
        ...
        for (let j = 0; j < sample.buckets.length; j++) {
          ...
          const dim = this.computeBucketDimensions(
            i,
            bucket
          );
          ...
          const color = [
            ...
            Math.log(this.props.highestBucketInCurrentWindow),
            0,
            0,
          ];
    ...
    if (!this.throttledHandler) {
      this.throttledHandler = throttle(e => {
        ...
        iterate_samples: for (let i = 0; i < nSamples; i++) {
          let sample = this.props.samples[i];
            ...
            if (
              ...
              this.setState({
                tooltipState: {
                  x: mouseX,
                  y: mouseY,
                  startKey: this.props.keys[bucket.start_key_id],
                  endKey: this.props.keys[bucket.end_key_id],
                  requests: bucket.requests.toNumber(),
                  epochTime: sample.timestamp.wall_time.toNumber() / 1e9
                }
              });
  ...
  toggleShowSpanBoundaries = () => {
    this.setState({showSpanBoundaries: !this.state.showSpanBoundaries}, () => {
      this.renderKeyVisualizer()
    })
  }
