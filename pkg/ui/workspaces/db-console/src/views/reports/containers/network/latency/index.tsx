...
const getLatencyCell = (
  ...
  const isPartitioned =
    isHealthyTargetNode && connectivity === ConnectionStatus.PARTITIONED;
  const isNotConnected = connectivity === ConnectionStatus.NOT_CONNECTED;
  const isClosedConnection =
    isHealthyLivenessStatus(identityA.livenessStatus) &&
    isHealthyLivenessStatus(identityB.livenessStatus) &&
    connectivity === ConnectionStatus.CLOSED;
  const isUnknown =
    isHealthyLivenessStatus(identityA.livenessStatus) &&
    isHealthyLivenessStatus(identityB.livenessStatus) &&
    connectivity === ConnectionStatus.UNKNOWN;
  ...
  const type: any = classNames({
    ...
    red: isPartitioned,
    white: isNotConnected,
    yellow: isClosedConnection || isUnknown,
