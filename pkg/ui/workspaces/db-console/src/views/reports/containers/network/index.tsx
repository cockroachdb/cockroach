...
export class Network extends React.Component<NetworkProps, INetworkState> {
  ...
  onShowDeadNodesChange = (showDeadNodes: boolean) => {
    ...
    search.set("showDeadNodes", `${showDeadNodes}`);
    this.props.location.search = search.toString();
  ...
  filteredDisplayIdentities = (displayIdentities: Identity[]) => {
    const { filter } = this.state;
    const { showDeadNodes } = this.props;
    let data: Identity[] = [];
    let selectedIndex = 0;
    if (
      !filter ||
  ...
  renderLatencyTable(latencies: number[], displayIdentities: Identity[]) {
    const { match, showDeadNodes } = this.props;
  ...
  renderContent(
    ...
    // 3. node is removed/decommissioned and
    //    a) it still has connection to another nodes;
    //    b) it doesn't have connections
...
const selectShowDeadNodes = createSelector(
  (state: AdminUIState) => state.router.location,
  location => {
    return location.query["showDeadNodes"] === "true";
  },
);
...
const mapStateToProps = (state: AdminUIState) => ({
  ...
  showDeadNodes: selectShowDeadNodes(state),
});
