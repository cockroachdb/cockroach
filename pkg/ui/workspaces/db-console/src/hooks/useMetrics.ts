...
/**
 * serializeRequest produces a stable string key for a TSRequest so
 ...
 */
function serializeRequest(req: TSRequest): string {
...
export function useMetrics(request: TSRequest | undefined): UseMetricsResult {
  // SWR key: null means "don't fetch". Otherwise a stable string
  ...
  const { data, error, isLoading, isValidating } = useSWR<TSResponse, Error>(
    key,
    ...
    {
      // Keep stale data visible while revalidating (default SWR
      // behavior). This prevents graph flickering on time window
      // changes.
