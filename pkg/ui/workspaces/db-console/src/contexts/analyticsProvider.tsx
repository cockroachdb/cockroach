...
import { getValidatedNodes } from "src/redux/nodes";
...
export const AnalyticsProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  // Derive and feed version tags from node statuses. Filter out
  // decommissioned/non-active nodes so their build tags don't skew the
  // versions reported to analytics.
  useEffect(() => {
    if (analyticsRef.current && nodeStatuses) {
      const validNodes = getValidatedNodes(nodeStatuses, livenessByNodeID);
      const versions = uniq(
        validNodes.map(ns => ns.build_info?.tag).filter(Boolean),
      );
