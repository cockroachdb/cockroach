...
import {
  ...
  resetIndexUsageStatsFailedAction, resetIndexUsageStatsPayload,
} from "./indexUsageStatsActions";

import ResetIndexUsageStatsRequest = cockroach.server.serverpb.ResetIndexUsageStatsRequest;
import {invalidateIndexStats, KeyedCachedDataReducerState, refreshIndexStats} from "oss/src/redux/apiReducers";
import {IndexStatsResponseMessage, resetIndexUsageStats} from "oss/src/util/api";
...
export function* resetIndexUsageStatsSaga(
  ...
  try {
    ...
    let keys: string[];
    keys = yield select(selectIndexStatsKeys);
    yield keys.forEach((key) => put(invalidateIndexStats(KeyToTableRequest(key))));
