...
export const filterStatementInsights = (
  statements: StatementInsightEvent[] | null,
  filters: WorkloadInsightEventFilters,
  internalAppNamePrefix: string,
  search?: string,
): StatementInsightEvent[] => {
...
export function mergeTxnInsightDetails(
  ...
  stmtInsights: StatementInsightEvent[],
  txnInsightDetails: TxnInsightDetails | null,
): TxnInsightDetails {
  let statements: StatementInsightEvent[] = null;
...
export function getStmtInsightRecommendations(
  insightDetails: Partial<StatementInsightEvent> | null,
): InsightRecommendation[] {
...
export function timeScaleRangeToObject(ts: TimeScale): {
  start?: moment.Moment;
  end?: moment.Moment;
} {
  if (ts == null) return {};
