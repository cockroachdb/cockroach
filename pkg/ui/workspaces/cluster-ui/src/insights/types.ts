...
const highContentionInsight = (
  ...
  if (latencyThreshold && contentionDuration && contentionDuration < latencyThreshold) {
    waitDuration = `${contentionDuration}ms`;
  } else if (!latencyThreshold) {
    waitDuration = "longer than the value of the 'sql.insights.latency_threshold' cluster setting";
  } else {
