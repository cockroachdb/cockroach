...
export const DEFAULT_JOBS_REQUEST = new cockroach.server.serverpb.JobsRequest({
  limit: 50 * typeOptions.length * statusOptions.length,
  status: "",
  type: 0,
});
...
export class JobsPage extends React.Component<JobsPageProps, PageState> {
  ...
  refresh(): void {
    const jobsRequest = new cockroach.server.serverpb.JobsRequest({
      limit:
        parseInt(this.props.show, 10) *
        typeOptions.length *
        statusOptions.length,
      status: DEFAULT_JOBS_REQUEST.status,
      type: DEFAULT_JOBS_REQUEST.type,
    });
  ...
  onShowSelected = async (item = "50") => {
    await this.props.setShow(item);
    this.resetPagination();
    syncHistory(
      {
        show: item,
          ...
          <PageConfig>
            ...
            <PageConfigItem>
              <Dropdown
                ...
                onChange={this.onShowSelected.bind(this)}
              >
                Show:{" "}
