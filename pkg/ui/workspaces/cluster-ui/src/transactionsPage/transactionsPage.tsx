...
import { ExecutionInsightCount } from "src/insights/types";
...
export interface TransactionsPageDispatchProps {
  ...
  refreshInsights: (req: TxnInsightsRequest) => void;
...
> {
  ...
  refreshData = (ts?: TimeScale): void => {
    ...
    if (!this.props.insightCounts) {
      const insightsReq = timeScaleRangeToObj(time);
      this.props.refreshInsights(insightsReq);
    }
