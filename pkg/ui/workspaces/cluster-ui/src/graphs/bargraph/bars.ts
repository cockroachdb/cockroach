...
export const getGroupedStackedBarOpts = (
  ...
  // Restack both groups based on the current group 1 visibility state.
  // Group 2 show flags are synced to match group 1 before restacking.
  function restackGroups(u: uPlot): void {
    // Sync group 2 show flags directly (no setSeries to avoid hooks).
    for (let i = 0; i < n; i++) {
    ...
    // Restack group 2, using group 1's visibility.
