...
const generateSeriesLegend = (uPlot: uPlot, seriesLegend: HTMLDivElement) => {
  ...
  uPlot.series.forEach((series: uPlot.Series, index: number) => {
    if (index === 0) {
      // skip the series for x axis
      return;
    }

    if (series.show === false) {
      // if series is hidden, then skip
      return;
    }

    // series.stroke can be either a function that returns a valid canvas stroke
    // value, or a valid canvas stroke value.
    const strokeColor =
