...
import { getStmtSummary } from "src/util/sql/summarize";
...
export const textCell = ({
  ...
  const summary = getStmtSummary(transactionText, transactionSummary);
  return (
    <div>
      <Tooltip
        placement="bottom"
      ...
      >
        <div className={textWrapper}>
          <div onClick={onClick} className={hoverAreaClassName}>
            {getHighlightedText(limitText(summary, 200), search, false, true)}
          </div>
        </div>
      </Tooltip>
    </div>
