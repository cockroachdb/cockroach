...
import { SpanMetadataRow, SpanMetadataTable } from "./spanMetadataTable";
...
export const SpanComponent: React.FC<{
  ...
  const childrenMetadataAsJson = JSON.stringify(childrenMetadata);
  const childMetadataRows = useMemo(
    () =>
      childrenMetadata
        ? Object.keys(childrenMetadata).map((name): SpanMetadataRow => {
            const v = span.children_metadata[name];
            return {
              name: name,
              count: v.count.toNumber(),
              duration: v.duration.toNumber(),
              containsUnfinished: v.contains_unfinished,
            };
          })
        : [],
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [childrenMetadataAsJson],
  );
      ...
      {childMetadataRows.length > 0 && (
        <section className={cx("span-section")}>
          ...
          <Loading
            ...
            error={snapshotError}
            render={() => (
              <SpanMetadataTable spanMetadataRows={childMetadataRows} />
            )}
          />
        </section>
      )}
