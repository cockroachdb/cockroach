...
export function* receivedInsightDetailsSaga(delayMs: number) {
  yield delay(delayMs);
...
export function* insightDetailsSaga(
  ...
  yield all([
    throttleWithReset(
      ...
      refreshInsightDetailsSaga,
    ),
    takeLatest(actions.request, requestInsightDetailsSaga),
    takeLatest(
      actions.received,
      receivedInsightDetailsSaga,
      cacheInvalidationPeriod,
    ),
  ]);
}
