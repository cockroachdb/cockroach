...
describe("SQLDetailsStats sagas", () => {
  ...
  // TODO marylia fix tests
  describe("requestSQLDetailsStatsSaga", () => {
    it("successfully requests statement details", () => {
      return expectSaga(requestSQLDetailsStatsSaga)
        .provide(stmtDetailsStatsAPIProvider)
        .put(actions.received(SQLDetailsStatsResponse))
        .withReducer(reducer)
        .hasFinalState<SQLDetailsStatsState>({
  ...
  // TODO marylia fix tests
  describe("receivedSQLDetailsStatsSaga", () => {
    it("sets valid status to false after specified period of time", () => {
      const timeout = 500;
      return expectSaga(receivedSQLDetailsStatsSaga, timeout)
        .delay(timeout)
