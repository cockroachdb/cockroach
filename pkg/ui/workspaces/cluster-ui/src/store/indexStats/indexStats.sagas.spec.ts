...
import { actions, IndexStatsReducerState, reducer } from "./indexStats.reducer";

describe("IndexStats sagas", () => {
  const database = "test_db";
  const table = "test_table";
  const action: PayloadAction<TableIndexStatsRequest> = {
  ...
  const tableIndexStatsResponse = new cockroach.server.serverpb.TableIndexStatsResponse(
    {
      statistics: [
        {
          statistics: {
  ...
  const resetIndexStatsResponse = new cockroach.server.serverpb.ResetIndexUsageStatsResponse();
  ...
  describe("refreshIndexStatsSaga", () => {
    it("dispatches request IndexStats action", () => {
      return expectSaga(refreshIndexStatsSaga)
        .put(actions.request())
        .run();
    });
  });
  ...
  describe("requestIndexStatsSaga", () => {
    it("successfully requests index stats", () => {
      return expectSaga(requestIndexStatsSaga, action)
  ...
  describe("resetIndexStatsSaga", () => {
    it("successfully resets index stats", () => {
      return expectSaga(resetIndexStatsSaga, action)
        .provide(indexStatsAPIProvider)
        .put(actions.invalidateAll())
        .put(actions.refresh(action.payload))
        .withReducer(reducer)
        .hasFinalState<IndexStatsReducerState>({
          cachedData: {
            "test_db/test_table": {
