...
function isSQLStringer(s: any): s is SQLStringer {
  return (s as SQLStringer).SQLString !== undefined;
}
...
// TODO(thomas): check what this actually does, do we need?
...
export function Join<Type>(args: Type[], sep: SQL): SQL {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    b = writeFormattedArg(args[i], b);
  }
  return new SQL(b);
}

function writeFormattedArg(arg: any, b: string): string {
