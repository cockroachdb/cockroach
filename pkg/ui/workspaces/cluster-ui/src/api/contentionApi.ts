...
// Note: this API is no longer consumed aside from a .spec file, but is left for future
...
export async function getContentionDetailsApi(
  ...
  result.execution?.txn_results.forEach(x => {
    x.rows.forEach(row => {
      contentionDetails.push(formatContentionResponseRow(row));
    });
  });
  ...
  return formatApiResult<ContentionDetails[]>(
    contentionDetails,
    result.error,
    "retrieving contention details information",
  );
}

export function formatContentionResponseRow(
  row: ContentionResponseColumns,
): ContentionDetails {
  return {
    ...
    contentionTimeMs: moment.duration(row.contention_duration).asMilliseconds(),
