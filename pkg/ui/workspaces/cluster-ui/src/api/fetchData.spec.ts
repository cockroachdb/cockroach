  ...
  it.each([
    {
      ...
      path: "get/json/error-in-status-text",
    ...
    async ({ path, request, response, body, expected }) => {
      // fetchMock.mockResponseOnce(body, { ...response });
      // fetchMock.mockResponseOnce(() => Promise.resolve([body, response]));
      ...
      await expect(fetchDataJSON(path, request)).rejects.toThrow(expected);
    },
  );
});
