...
export const withBasePath = (p: string): string => joinPathsEnsureRelative(getBasePath(), p)
...
//
const joinPathsEnsureRelative = (left: string, right: string): string => {
  if (right.startsWith('/')) {
    throw new Error("Application paths must remain compatible with relative base. Remove prefix `/` character.")
  }
  if (left === "") {
    return right
  }
