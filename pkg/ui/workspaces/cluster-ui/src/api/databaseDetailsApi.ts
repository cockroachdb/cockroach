...
const DATABASE_DETAILS_PATH = "_admin/v1/databases";

export async function getDatabaseDetails(
  req: cockroach.server.serverpb.DatabaseDetailsRequest,
): Promise<cockroach.server.serverpb.DatabaseDetailsResponse> {
  const params = new URLSearchParams([["include_stats", "true"]]);
  return fetchData(
    cockroach.server.serverpb.DatabaseDetailsResponse,
    `${DATABASE_DETAILS_PATH}/${req.database}?${params.toString()}`,
...
// }
//
// export async function getDatabaseDetails111(
//   params: DatabaseDetailsReqParams,
