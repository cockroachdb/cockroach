...
function transactionContentionResultsToEventState(
  ...
  return response.execution.txn_results[0].rows.map(row => ({
    ...
    insightName: InsightNameEnum.highContentionTime,
...
WHERE 
  app_name != '${apiAppName}' AND encode(fingerprint_id, 'hex') = ANY string_to_array('${txn_fingerprint_ids}', ',')`;

function txnStmtFingerprintsResultsToEventState(
  response: SqlExecutionResponse<TxnStmtFingerprintsResponseColumns>,
): TxnStmtFingerprintEventState[] {
  if (!response.execution.txn_results[0].rows) {
...
const fingerprintStmtsQuery = (stmt_fingerprint_ids: string[]) => `SELECT 
...
FROM 
  crdb_internal.statement_statistics WHERE encode(fingerprint_id, 'hex') = ANY string_to_array('${stmt_fingerprint_ids}', ',')`;

function fingerprintStmtsResultsToEventState(
  response: SqlExecutionResponse<FingerprintStmtsResponseColumns>,
): FingerprintStmtsEventState[] {
  if (!response.execution.txn_results[0].rows) {
...
export function getTransactionInsightEventState(): Promise<TransactionInsightEventsResponse> {
  ...
  return executeSql<TransactionContentionResponseColumns>(
    ...
    const txnFingerprintIDs =
      contentionResults.execution.txn_results[0].rows.map(
        row => row.blocking_txn_fingerprint_id,
      );
...
export function combineTransactionInsightEventState(
  ...
  txnFingerprintState: TxnStmtFingerprintEventState[],
  fingerprintStmtState: FingerprintStmtsEventState[],
): TransactionInsightEventState[] {
...
const txnContentionDetailsQuery = (id: string) => `SELECT 
  collection_ts, 
  blocking_txn_id, 
  encode(
    blocking_txn_fingerprint_id, 'hex'
...
export function combineTransactionInsightEventDetailsState(
  txnContentionDetailsState: TransactionContentionEventDetailsState,
  blockingTxnFingerprintState: TxnStmtFingerprintEventState[],
  waitingTxnFingerprintState: TxnStmtFingerprintEventState[],
  blockingFingerprintStmtState: FingerprintStmtsEventState[],
  waitingFingerprintStmtState: FingerprintStmtsEventState[],
): TransactionInsightEventDetailsState {
  ...
  if (
    ...
    res = {
      ...
      waitingQueries: blockingTxnFingerprintState[0].queryIDs.map(
        id =>
