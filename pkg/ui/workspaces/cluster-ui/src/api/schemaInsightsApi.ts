...
type SchemaInsightResponse = ClusterIndexUsageStatistic | CreateIndexRecommendationsResponse;
...
function clusterIndexUsageStatsToEventState(
  ...
  txn_result.rows.forEach(row => {
    let result = recommendDropUnusedIndex(row)
    console.log("recommend drop?", result.recommend);
...
function createIndexRecommendationsToState(
  ...
  txn_result.rows.forEach(row => {
    results.push({
      ...
      query: row.index_recommendations?.length > 0 ? row.index_recommendations[0].split(" : ")[1] : "",
    });
  });
  return results;
}
...
export function getSchemaInsightEventState(): Promise<InsightRecommendation[]> {
  const request: SqlExecutionRequest = {
    statements: schemaInsightQueries.map(insightQuery => ({ sql: insightQuery.query})),
    execute: true,
  };
  return executeSql<SchemaInsightResponse>(request).then(
    result => {
      ...
      result.execution.txn_results.map(txn_result => {
        ...
        let insightQuery: SchemaInsightQuery<SchemaInsightResponse> = schemaInsightQueries[txn_result.statement - 1];
        if (txn_result.rows) {
          results.push(...insightQuery.toState(txn_result));
        }
      });
      // TODO(thomas): remove
      console.log("final toState results from getSchemaInsightEventState", results);
      return results;
    },
  );
}
