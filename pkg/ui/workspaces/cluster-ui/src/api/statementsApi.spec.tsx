...
import { useStatements, SqlStatsSortOptions } from "./statementsApi";
...
describe("useStatements", () => {
  ...
  it("fetches statements and returns data", async () => {
    ...
    const { result, waitForNextUpdate } = renderHook(
      () => useStatements(timeScale, 100, SqlStatsSortOptions.PCT_RUNTIME),
      { wrapper },
    );
  ...
  it("skips fetch when timeScale is null", async () => {
    const { result } = renderHook(
      () => useStatements(null, 100, SqlStatsSortOptions.PCT_RUNTIME),
      { wrapper },
    );
