...
export const selectDisplayRefreshAlert = (state: AppState): boolean => {
  return localStorageSelector(state)[
...
export const mapStateToActiveStatementsPageProps = (
  ...
  displayRefreshAlert: selectDisplayRefreshAlert(state),
});
...
export const mapDispatchToActiveStatementsPageProps = (
  ...
  onSetDisplayRefreshAlert: (displayRefreshAlert: boolean) => {
    dispatch(
      localStorageActions.update({
        key: LocalStorageKeys.ACTIVE_EXECUTIONS_DISPLAY_REFRESH_ALERT,
        value: displayRefreshAlert,
      }),
    );
  },
});
