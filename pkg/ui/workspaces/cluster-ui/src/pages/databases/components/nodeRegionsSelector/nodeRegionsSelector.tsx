...
export const NodeRegionsSelector: React.FC<NodeRegionsSelectorProps> = ({
  ...
  const nodesByRegion: Record<string, NodeID[]> = useMemo(() => {
    ...
    for (const [id, node] of Object.entries(nodeStatusByID)) {
      // if there is a search applied and the node in question
      // does not match it, skip this node.
      if (search && !`n${id}`.includes(search)) {
        continue;
      }
    ...
    return optionsMap;
  }, [nodeStatusByID, search]);

  // Separate regionless nodes into their own list.
  const regionlessNodes = nodesByRegion[""] || [];
  ...
  const onSearch = (search: string) => {
    // If we search for a node, selected nodes may not be visible,
    // making it confusing if the user applies the selection and
    // sees a node in the tags which wasn't visibly selected.
    // To avoid this, we remove selected nodes which no longer match
    // the filter.
    setSelectedNodes(selectedNodes.filter(n => matchesFilter(n, search)));
    setSearch(search);
  };
...
function matchesFilter(id: NodeID, search: string): boolean {
  return search && `n${id}`.includes(search);
}
