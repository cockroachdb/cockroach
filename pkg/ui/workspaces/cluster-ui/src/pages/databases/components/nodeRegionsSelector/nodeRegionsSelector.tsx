...
export const NodeRegionsSelector: React.FC<NodeRegionsSelectorProps> = ({
  ...
  const onSelect = (...values: NodeID[]) => {
    setSelectedNodes([...selectedNodes, ...values]);
  };
  ...
  const onApply = () => {
    ...
    onSubmit(selectedNodes.map(n => n.toString()));
  };
  ...
  const selection =
    selectedNodes.length === nodeIds.length && includeAllOption
      ? [ALL_LABEL]
      : selectedNodes.map(n => ({ label: `n${n}`, value: n }));
