...
export function makeSessionsColumns(
  ...
  const columns: ColumnDescriptor<SessionInfo>[] = [
    ...
    },
    {
      ...
      className: cx("cl-table__col-session"),
      cell: session =>
        BytesWithPrecision(FixLong(session.session.alloc_bytes ?? 0).toNumber(), 0) +
        "/" +
        BytesWithPrecision(FixLong(session.session.max_alloc_bytes ?? 0).toNumber(), 0),
      sort: session => FixLong(session.session.max_alloc_bytes ?? 0).toNumber(),
    },
    {
      name: "clientAddress",
