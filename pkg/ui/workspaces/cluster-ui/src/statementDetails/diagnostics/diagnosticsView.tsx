...
const StmtDiagnosticLabel = ({
  currentScale,
  requestTime,
  numReportsOutsideTimeWindow,
  undisplayedActiveReportExists,
}: {
  currentScale: TimeScale;
  requestTime: moment.Moment;
  numReportsOutsideTimeWindow: number;
  ...
  return (
    <>
      <p className={timeScaleStylesCx("time-label", "label-margin")}>
        ...
        {numReportsOutsideTimeWindow > 0 && (
          <p>
            There are {numReportsOutsideTimeWindow} statement diagnostic reports
            outside the current selected time window. Expand the time window to
            view more statement diagnostic reports.
          </p>
        )}
      </p>
      {undisplayedActiveReportExists && (
        <div>
          There is currently an <em>active</em> statement diagnostic request
          that is not displayed in the current time window.
          <div>
            Further attempts to active a diagnostic for this statement will
            fail.
          </div>
        </div>
      )}
...
> {
  ...
  render(): React.ReactElement {
    ...
    // Number of reports outside the time window.
    const numReportsOutsideTimeWindow =
      diagnosticsReports.length - dataSource.length;
      ...
      return (
        <>
          ...
          <StmtDiagnosticLabel
            ...
            numReportsOutsideTimeWindow={numReportsOutsideTimeWindow}
