...
//
// When latencyByGist is provided and contains at least one of the
// chart's plan gists, plan gists are sorted by latency ascending
// (lowest latency at the bottom of the stack) and each gist is
// assigned a heat-map color (light purple = low latency, dark purple
// = high latency). The latency range is returned so the caller can
// render a legend. Otherwise the chart falls back to evenly-spaced
// purple shades and no latency range is returned.
export function generateCanaryVsStablePlanDistribution(
  ...
  const hasLatencyData =
    !!latencyByGist && planGistArray.some(g => latencyByGist.has(g));
