...
export function StatementDetails(
  ...
  const hasDiagnosticReports = useCallback(
    (): boolean => diagnosticsReports.length > 0,
    [diagnosticsReports],
  );
  ...
  }, [timeScale, statementFingerprintID, refreshStatementFingerprintInsights]);
  ...
  const onTabChange = useCallback(
    ...
    [history, onTabChanged],
  );

  const backToStatementsClick = useCallback((): void => {
    history.push("/sql-activity?tab=Statements&view=fingerprints");
    if (onBackToStatementsClick) {
      onBackToStatementsClick();
    }
  }, [history, onBackToStatementsClick]);
  ...
  useEffect(() => {
    ...
    // updated.
    if (timeScale.key !== "Custom" || !lastUpdated) {
      const now = moment();
      const nextRefresh = lastUpdated?.clone().add(5, "minutes") || now;
      setTimeout(
        refreshStatementDetailsInternal,
  ...
  const renderNoDataTabContent = useCallback(
    (): React.ReactElement => (
      <>
        <PageConfig>
          <PageConfigItem>
  ...
  const renderNoDataWithTimeScaleAndSqlBoxTabContent = useCallback(
    ...
    [timeScale, changeTimeScale, formattedQuery, statementsError],
  );

  const renderOverviewTabContent = useCallback(
    (hasTimeout: boolean, hasData: boolean): React.ReactElement => {
      if (!hasData) {
      ...
      const unavailableTooltip = !statementSampled && (
        <div>
          This metric is part of the statement execution and therefore will not
          be available until the statement is sampled.
        </div>
      );
  ...
  const renderExplainPlanTabContent = useCallback(
    (hasTimeout: boolean, hasData: boolean): React.ReactElement => {
      if (!hasData) {
        return renderNoDataWithTimeScaleAndSqlBoxTabContent(hasTimeout);
      }
                    ...
                    <BarGraphTimeSeries
                      ...
                      tooltip={
                        <>
                          ...
                          Stacked bars show the total execution count broken
                          down by plan.
                        </>
                      }
  ...
  const renderDiagnosticsTabContent = useCallback(
    (hasData: boolean): React.ReactElement => {
      if (!hasData && !query) {
    ...
    [
      ...
      onDiagnosticBundleDownload,
      onDiagnosticCancelRequest,
      uiConfig?.showStatementDiagnosticsLink,
      onSortingChange,
      timeScale,
      changeTimeScale,
    ],
  ...
  const renderTabs = useCallback((): React.ReactElement => {
