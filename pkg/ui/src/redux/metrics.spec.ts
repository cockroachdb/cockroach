...
describe("metrics reducer", function() {
  ...
  describe("saga functions", function() {
    ...
    function createResponse(
      queries: protos.cockroach.ts.tspb.Query$Properties[],
    ...
    describe("integration test", function() {
      ...
      const createMetricsState = (
        id: string, timespan: [Long, Long], metricNames: string[], datapointCount: number,
      ): metrics.MetricsQuery => {
        const request = createRequest(timespan, ...metricNames);
      ...
      const createMetricsErrorState = (
        id: string, timespan: [Long, Long], metricNames: string[], err: Error,
      ): metrics.MetricsQuery => {
        const request = createRequest(timespan, ...metricNames);
        const state = new metrics.MetricsQuery(id);
        state.nextRequest = request;
      ...
      const createMetricsInFlightState = (
        id: string, timespan: [Long, Long], metricNames: string[],
      ): metrics.MetricsQuery => {
        const request = createRequest(timespan, ...metricNames);
        const state = new metrics.MetricsQuery(id);
        state.nextRequest = request;
