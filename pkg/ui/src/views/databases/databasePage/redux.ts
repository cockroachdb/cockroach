...
import { DatabasePageData } from "@cockroachlabs/cluster-ui";
...
function normalizeRoles(raw: string[]): string[] {
  ...
  return _.sortBy(_.uniq(_.filter(raw)), (role) => rolePrecedence[role] || 100);
}
...
export const mapStateToProps = createSelector(
  ...
  (database, databaseDetails, tableDetails, tableStats): DatabasePageData => {
    return {
