...
import {
  DatabasePageActions,
  DatabasePageData,
  DatabasePageDataTableDetails,
  DatabasePageDataTableStats,
...
class TestDriver {
  private readonly actions: DatabasePageActions;
  private readonly properties: () => DatabasePageData;
  ...
  assertProperties(expected: DatabasePageData) {
    assert.deepEqual(this.properties(), expected);
  }

  assertTableDetails(name: string, expected: DatabasePageDataTableDetails) {
  ...
  assertTableStats(name: string, expected: DatabasePageDataTableStats) {
...
describe("Database Page", function () {
  ...
  it("sorts roles meaningfully", async function () {
    ...
    driver.assertTableRoles("foo", ["root", "admin", "public", "bloblaw"]);
  });
