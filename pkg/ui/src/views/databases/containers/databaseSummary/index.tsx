    ...
    if (props.tableInfos && props.tableInfos.length > 0) {
      for (const tblInfo of props.tableInfos) {
        ...
        if (_.isUndefined(tblInfo.numColumns)) {
          await props.refreshTableDetails(new protos.cockroach.server.serverpb.TableDetailsRequest({
            database: props.name,
            table: tblInfo.name,
          }));
        }
        if (_.isUndefined(tblInfo.physicalSize)) {
          await props.refreshTableStats(new protos.cockroach.server.serverpb.TableStatsRequest({
            database: props.name,
            table: tblInfo.name,
          }));
        }
