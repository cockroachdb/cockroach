...
class CustomModal extends React.Component<ICustomModalProps, ICustomModalState> {
  state = { visible: false };
  ...
  render() {
    const { trigger, visible, children, triggerStyle, triggerTitle } = this.props;
    return (
      <Fragment>
        {trigger ? trigger : <a onClick={this.showModal} className={`${triggerStyle}`}>{triggerTitle}</a>}
        <Modal
