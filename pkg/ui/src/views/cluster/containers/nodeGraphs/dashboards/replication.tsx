...
export default function (props: GraphDashboardProps) {
  ...
  return [
    ...
    <LineGraph title="Keys Written per Second per Store" tooltip={`The average number of KV keys written per second on each store.`}>
      <Axis>
        {
          _.map(nodeIDs, (nid) => (
            <Metric
              key={nid}
              name="cr.store.writes_per_second"
              title={nodeAddress(nodesSummary, nid)}
              sources={storeIDsForNode(nodesSummary, nid)}
            />
          ))
