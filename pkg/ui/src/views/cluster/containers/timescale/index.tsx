...
class TimeScaleDropdown extends React.Component<TimeScaleDropdownProps, {}> {
  ...
  generateDisabledArrows = () => {
    const { currentWindow } = this.props;
    const differenceEndToNow = currentWindow && moment.duration(moment().diff(currentWindow.end)).asMinutes();
    const differenceEndToStart = currentWindow && moment.duration(moment(currentWindow.end).diff(currentWindow.start)).asMinutes();
