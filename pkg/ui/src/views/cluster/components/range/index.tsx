...
class RangeSelect extends React.Component<RangeSelectProps, RangeSelectState> {
  ...
  renderTimePickerAddon = (direction: DateTypes) => () => (
    <Button
      type="default"
      onClick={() => this.onChangeDate(direction)(moment())}
      size="small"
    >
      Now
    </Button>
  ...
  renderDatePickerAddon = (direction: DateTypes) => (
  ...
  clearPanelValues = () =>
    this.setState({ selectMonthEnd: null, selectMonthStart: null });
  ...
  toggleCustomPicker = (custom: boolean) => () =>
    this.setState({ custom }, this.clearPanelValues);
  ...
  optionButton = (option: RangeOption) => (
    <Button
      ...
      ghost
    >
      <span className="dropdown__range-title">
  ...
  getDisabledSeconds = (isStart?: boolean) => () => {
    ...
    if (startHour === endHour && startMinutes === endMinutes) {
      for (
        let i = 0;
        i < (isStart ? moment().second() : startSeconds + 1);
        i++
      ) {
        if (isStart) {
  ...
  headerRender = (item: any) => (
    <div className="calendar-month-picker">
      <Button
        type="default"
        onClick={() => item.onChange(moment(item.value).subtract(1, "months"))}
      >
        <Icon type="left" />
      </Button>
      ...
      <Button
        type="default"
        onClick={() => item.onChange(moment(item.value).add(1, "months"))}
      >
        <Icon type="right" />
      </Button>
    </div>
  ...
  onPanelChange = (direction: DateTypes) => (date: Moment) => {
    ...
    this.setState({
      selectMonthStart:
        direction === DateTypes.DATE_FROM ? date : selectMonthStart,
  ...
  renderContent = () => {
    ...
    const calendarStartValue = selectMonthStart
      ? selectMonthStart
      : start
      ? start
      : moment();
    const calendarEndValue = selectMonthEnd
      ? selectMonthEnd
      : end
      ? end
      : moment();
        ...
        <div className="_start">
          ...
          <TimePicker
            ...
            format={`${timePickerFormat} ${
              (moment(start).isSame(moment.utc(), "minute") && "[- Now]") || ""
            }`}
            use12Hours
            addon={this.renderTimePickerAddon(DateTypes.DATE_FROM)}
            onChange={this.onChangeDate(DateTypes.DATE_FROM)}
            disabledHours={
              (isSameDate && this.getDisabledHours(true)) || undefined
            }
            disabledMinutes={
              (isSameDate && this.getDisabledMinutes(true)) || undefined
            }
            disabledSeconds={
              (isSameDate && this.getDisabledSeconds(true)) || undefined
            }
          />
        </div>
        <div className="_end">
          ...
          <div className="range-calendar">
            <Calendar
              ...
              disabledDate={(currentDate) =>
                currentDate > moment() || currentDate < (start || moment())
              }
              headerRender={this.headerRender}
          ...
          <TimePicker
            ...
            allowClear={false}
            format={`${timePickerFormat} ${
              (moment(end).isSame(moment.utc(), "minute") && "[- Now]") || ""
            }`}
            use12Hours
            ...
            disabledMinutes={
              (isSameDate && this.getDisabledMinutes()) || undefined
            }
            disabledSeconds={
              (isSameDate && this.getDisabledSeconds()) || undefined
            }
          />
        </div>
      </React.Fragment>
