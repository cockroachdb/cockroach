...
const rangeTableDisplayList: RangeTableRow[] = [
  ...
  {
    variable: "maxLockWaitQueueWaitersForLock",
    display: "Max Lock Wait-Queue Waiters",
    compareToLeader: false,
  },
  {
  ...
  render() {
    ...
    _.forEach(infos, (info) => {
      ...
      detailsByStoreID.set(info.source_store_id, {
        ...
        locksWithWaitQueues: this.createContent(FixLong(info.locks_with_wait_queues)),
        lockWaitQueueWaiters: this.createContent(FixLong(info.lock_wait_queue_waiters)),
        maxLockWaitQueueWaitersForLock: this.createContent(FixLong(info.max_lock_wait_queue_waiters_for_lock)),
