...
export default class RangeTable extends React.Component<RangeTableProps, {}> {
  ...
  // contentNoValue returns an empty value if the condition is false, and if
  // true, executes and returns the content function.
  contentNoValue(
    showContent: boolean,
  ...
  render() {
    ...
    _.forEach(infos, info => {
      ...
      detailsByStoreID.set(info.source_store_id, {
        ...
        raftLeader: this.contentNoValue(!dormant, () => this.createContent(
          FixLong(info.raft_state.lead),
          raftLeader ? "range-table__cell--raftstate-leader" : "range-table__cell--raftstate-follower",
        )),
        vote: this.contentNoValue(!dormant, () => this.createContent(vote.greaterThan(0) ? vote : "-")),
        term: this.contentNoValue(!dormant, () => this.createContent(FixLong(info.raft_state.hard_state.term))),
        applied: this.contentNoValue(!dormant, () => this.createContent(FixLong(info.raft_state.applied))),
        commit: this.contentNoValue(!dormant, () => this.createContent(FixLong(info.raft_state.hard_state.commit))),
