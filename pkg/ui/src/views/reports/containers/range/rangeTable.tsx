...
function convertLeaseState(leaseState: protos.cockroach.kv.storage.LeaseState) {
  return protos.cockroach.kv.storage.LeaseState[leaseState].toLowerCase();
}
...
export default class RangeTable extends React.Component<RangeTableProps, {}> {
  ...
  contentGCAvgAge(mvcc: cockroach.kv.storage.engine.enginepb.IMVCCStats): RangeTableCellContent {
    if (mvcc === null) {
  ...
  createContentIntentAvgAge(mvcc: cockroach.kv.storage.engine.enginepb.IMVCCStats): RangeTableCellContent {
    if (mvcc === null) {
      return this.contentDuration(Long.fromNumber(0));
    }
    ...
    _.forEach(infos, info => {
      ...
      if (_.isNil(info.lease_status)) {
        ...
        leaseState = this.createContent(
          ...
          info.lease_status.state === protos.cockroach.kv.storage.LeaseState.VALID ? "" :
            "range-table__cell--warning",
        );
      }
