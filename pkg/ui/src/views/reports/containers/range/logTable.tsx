...
export default class LogTable extends React.Component<LogTableProps, {}> {
  ...
  renderRangeID(currentRangeID: Long | number, otherRangeID: Long | number) {
  ...
  printLogEventType(eventType: protos.cockroach.storage.RangeLogEventType) {
    switch (eventType) {
            ...
            </tr>
            {
              _.map(log.events, (event, key) => (
                <tr key={key} className="log-table__row">
                  ...
                  <td className="log-table__cell">{this.printLogEventType(event.event_type)}</td>
