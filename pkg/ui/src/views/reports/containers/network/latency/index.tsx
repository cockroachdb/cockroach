...
const getLatencyCell = (
  ...
  if (latency === -2) {
    return (
      <td
        className={generateClassName([
          "latency-table__cell",
          "latency-table__cell--no-connection",
        ])}
      >
  ...
  const type: any = classNames({
    green: std.stddev > 0 && latency < std.stddevMinus2,
    lightgreen:
      std.stddev > 0 &&
      latency < std.stddevMinus1 &&
      latency >= std.stddevMinus2,
    ...
    lightblue:
      std.stddev > 0 && latency > std.stddevPlus1 && latency <= std.stddevPlus2,
    blue: std.stddev > 0 && latency > std.stddevPlus2,
  });
  return (
            ...
            <div>
              <div className="Chip--tooltip__nodes">
                <div className="Chip--tooltip__nodes--item">
                  ...
                  <p className="Chip--tooltip__nodes--item-description">
                    {identityB.locality}
                  </p>
                </div>
              ...
              <p className={`color--${type}`}>{`${latency.toFixed(
                2,
              )}ms roundtrip`}</p>
            </div>
          }
