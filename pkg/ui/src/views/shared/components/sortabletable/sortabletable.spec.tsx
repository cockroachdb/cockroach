...
describe("<SortableTable>", () => {
  describe("renders correctly.", () => {
    ...
    it("renders rows and columns correctly.", () => {
      ...
      const headers = wrapper.find(`tr.${styles["sort-table__row--header"]}`);
      _.each(columns, (c, index) => {
        const header = headers.childAt(index);
        assert.isTrue(header.is(`.${styles["sort-table__cell"]}`), "header is correct class.");
      ...
      // Nothing is sorted.
      assert.lengthOf(wrapper.find(`th.${styles["sort-table__cell--ascending"]}`), 0, "expected zero sorted columns.");
      assert.lengthOf(wrapper.find(`th.${styles["sort-table__cell--descending"]}`), 0, "expected zero sorted columns.");
    });
    ...
    it("renders sorted column correctly.", () => {
      ...
      let sortHeader = wrapper.find(`th.${styles["sort-table__cell--descending"]}`);
      ...
      sortHeader = wrapper.find(`th.${styles["sort-table__cell--ascending"]}`);
      ...
      sortHeader = wrapper.find(`th.${styles["sort-table__cell--descending"]}`);
  ...
  describe("changes sort setting on clicks.", () => {
    ...
    it("does nothing if unsortable column is clicked.", () => {
      ...
      wrapper.find(`thead th.${styles["sort-table__cell"]}`).last().simulate("click");
