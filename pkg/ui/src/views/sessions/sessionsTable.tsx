...
export function makeSessionsColumns(
  ...
  return [
    ...
    },
    {
      ...
      cell: session => Bytes(session.session.alloc_bytes?.toNumber()) + "/" + Bytes(session.session.max_alloc_bytes?.toNumber()),
    ...
    },
    {
      ...
      cell: (session) => {
        ...
        let stmt = session.session.active_queries[0].sql
        let anon_stmt = session.session.active_queries[0].sql_anon
        return <StatementLink
          statement={ stmt }
          anonStatement={ anon_stmt }
...
const SessionLink = (props: { session: ISession }) => {
  ...
  return (
    ...
    >
      <Link to={ `${base}/${encodeURIComponent(props.session.id.join(""))}` }>
        <div>
          { start.fromNow(true) }
        </div>
      </Link>
