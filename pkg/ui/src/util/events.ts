...
export function getEventDescription(e: Event$Properties): string {
  ...
  switch (e.event_type) {
    ...
    case eventTypes.GRANT_PRIVILEGE:
      return `Privileges granted: ${info.Privileges} added to User(s) ${info.User} on ${info.Target}`;
    case eventTypes.REVOKE_PRIVILEGE:
      return `Privileges revoked: ${info.Privileges} removed from User(s) ${info.User} on ${info.Target}`;
    default:
