...
export function getSaveError(state: AdminUIState, key: string): Error {
  return (state.uiData[key] && (state.uiData[key].state === UIDataState.SAVE_ERROR)) ? state.uiData[key].error : null;
}
...
export function getLoadError(state: AdminUIState, key: string): Error {
  return (state.uiData[key] && (state.uiData[key].state === UIDataState.LOAD_ERROR)) ? state.uiData[key].error : null;
}
