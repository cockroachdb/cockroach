...
describe("UIData reducer", function() {
  describe("actions", function() {
    ...
    it("errorUIData() creates the correct action type.", function() {
      assert.equal(uidata.errorUIData(null, null).type, uidata.ERROR);
    });
  ...
  describe("reducer", function() {
    ...
    it("should correctly dispatch errorUIData.", function () {
      ...
      dispatch(uidata.errorUIData(key1, err));
      assert.equal(state[key1].lastError, err);

      dispatch(uidata.setUIDataKey(key2, 4));
      assert.isNull(state[key2].lastError);
    });
  ...
  describe("asynchronous actions", function() {
    ...
    it("correctly saves UIData", function() {
      ...
      return Promise.all([p, p2]).then(() => {
        ...
        assert.isNull(state[uiKey1].lastError);
        assert.isNull(state[uiKey2].lastError);
    ...
    it("correctly reacts to error during save", function() {
      ...
      return p.then(() => {
        ...
        assert.instanceOf(state[uiKey1].lastError, Error);
        assert.instanceOf(state[uiKey2].lastError, Error);
      });
    });
    ...
    it("handles missing keys", function () {
      ...
      return p.then(() => {
        ...
        assert.isNull(state[missingKey].lastError);
