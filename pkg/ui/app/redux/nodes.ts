...
const nodeStatuses = (state: AdminUIState) => state.cachedData.nodes.data;
const livenesses = (state: AdminUIState) => state.cachedData.liveness.data;
const livenessCheckedAt = (state: AdminUIState) => state.cachedData.liveness.setAt;

/**
 * livenessByNodeID returns a map from NodeID to the Liveness record for that
 * node.
 */
const livenessByNodeID = createSelector(
  livenesses,
  (livenessRecord) => {
    if (livenessRecord) {
      return _.keyBy(livenessRecord.getLivenesses(), (l) => l.getNodeId());
    }
    return {};
  },
);
 ...
 * that node.
 */
const livenessStatusByNodeID = createSelector(
  livenessByNodeID,
  livenessCheckedAt,
  (liveByNodeID, liveCheckedAt) => {
    if (liveCheckedAt) {
      const deadCutoff = liveCheckedAt.clone().subtract(deadTimeout);
      const suspectCutoff = liveCheckedAt.clone().subtract(suspectTimeout);
 ...
 */
const nodeStatusByID = createSelector(
  nodeStatuses,
  (nss) => {
  ...
  (ns, liveMap) => {
    ...
    if (_.isArray(ns) && _.isObject(liveMap)) {
      ns.forEach((n) => {
        ...
        const status = liveMap[n.getDesc().getNodeId()];
        switch (status) {
 ...
 */
export const nodesSummary = createSelector(
  nodeStatuses,
  nodeIDs,
  nodeStatusByID,
  nodeSums,
