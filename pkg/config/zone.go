...
// ClearSelf removes any configuration that applies to the specific zone
// targeted by this ZoneConfig, leaving only the subzone configs, if any. After
// calling ClearSelf, IsZubzonePlaceholder will return true.
func (z *ZoneConfig) ClearSelf() {
	*z = ZoneConfig{
		Subzones:     z.Subzones,
...
// GetSubzone returns the Subzone for the specified index ID and partition, if
// it exists.
