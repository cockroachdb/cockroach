...
func TestConstraintsListYAML(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			input: "['a:b']",
			err:   "constraints before separator ':' must be wrapped in parentheses",
		},
		{
			input: "['a:b:2']",
			err:   "no more than one separator ':' is allowed",
		},
		{
			input: "['(a):b']",
	...
	for _, tc := range testCases {
		t.Run(tc.input, func(t *testing.T) {
			...
			err := yaml.Unmarshal([]byte(tc.input), &constraints)
			if !testutils.IsError(err, tc.err) {
				t.Fatalf("expected error matching %q, but got %v", tc.err, err)
			}
		})
	}
}
...
func TestMarshalableZoneConfigRoundTrip(t *testing.T) {
	...
	original := NewPopulatedZoneConfig(rand.New(rand.NewSource(time.Now().UnixNano())), false /* easy */)
