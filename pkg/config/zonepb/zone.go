...
func (z *ZoneConfig) CopyFromZone(other ZoneConfig, fieldList []tree.Name) {
	for _, fieldName := range fieldList {
		...
		if fieldName == "num_voters" {
			if other.NumReplicas != nil {
				// We copy the "effective" num_voters value from `other`. This means
				// that if `num_voters` is not explicitly defined in `other`, we copy
				// the `num_replicas` value.
				z.NumVoters = proto.Int32(other.GetNumVoters())
			}
		}
