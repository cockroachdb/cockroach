...
func (Constraint_Type) EnumDescriptor() ([]byte, []int) {
	return fileDescriptor_zone_89fdc7d03f235849, []int{1, 0}
}
...
func (*GCPolicy) Descriptor() ([]byte, []int) {
	return fileDescriptor_zone_89fdc7d03f235849, []int{0}
}
...
func (*Constraint) Descriptor() ([]byte, []int) {
	return fileDescriptor_zone_89fdc7d03f235849, []int{1}
}
...
func (*ConstraintsConjunction) Descriptor() ([]byte, []int) {
	return fileDescriptor_zone_89fdc7d03f235849, []int{2}
}
...
func (*LeasePreference) Descriptor() ([]byte, []int) {
	return fileDescriptor_zone_89fdc7d03f235849, []int{3}
}
...
type ZoneConfig struct {
	...
	// TODO DURING REVIEW: We probably want to rename this field from
	// `num_replicas` to `num_quorum_replicas` (or `num_voter_replicas`), since we
	// now have multiple types of replicas (especially with consistent read
	// replicas coming in the next couple releases). However, `num_replicas` is a
	// part of the user visible zone config syntax. What can we do here?
	// NumReplicas specifies the desired number of (voter) replicas
	NumReplicas *int32 `protobuf:"varint,5,opt,name=num_replicas,json=numReplicas" json:"num_replicas,omitempty" yaml:"num_replicas"`
	// NumLearnerReplicas specifies the desired number of persistent learner replicas
	NumLearnerReplicas *int32 `protobuf:"varint,12,opt,name=num_learner_replicas,json=numLearnerReplicas" json:"num_learner_replicas,omitempty" yaml:"num_learner_replicas"`
	...
	// TODO DURING REVIEW: Likewise, this field should probably be something like
	// `voter_constraints`.
	Constraints []ConstraintsConjunction `protobuf:"bytes,6,rep,name=constraints" json:"constraints" yaml:"constraints,flow"`
	// LearnerConstraints constrain which stores the learners can be stored on.
	// The sum of the num_replicas fields of the LearnerConstraints must add up to
	// ZoneConfig.num_learner_replicas, or there must be no more than a single
	// LearnerConstraints field
	LearnerConstraints []ConstraintsConjunction `protobuf:"bytes,13,rep,name=learner_constraints,json=learnerConstraints" json:"learner_constraints" yaml:"learner_constraints,flow"`
	...
	// InheritedLearnerContraints specifies if the value in the Constraints field was
	// inherited from the zone's parent or specified explicitly by the user.
	InheritedLearnerConstraints bool `protobuf:"varint,14,opt,name=inherited_learner_constraints,json=inheritedLearnerConstraints" json:"inherited_learner_constraints"`
...
func (*ZoneConfig) Descriptor() ([]byte, []int) {
	return fileDescriptor_zone_89fdc7d03f235849, []int{4}
}
...
func (*Subzone) Descriptor() ([]byte, []int) {
	return fileDescriptor_zone_89fdc7d03f235849, []int{5}
}
...
func (*SubzoneSpan) Descriptor() ([]byte, []int) {
	return fileDescriptor_zone_89fdc7d03f235849, []int{6}
}
...
func (this *ZoneConfig) Equal(that interface{}) bool {
	...
	if this.NumLearnerReplicas != nil && that1.NumLearnerReplicas != nil {
		if *this.NumLearnerReplicas != *that1.NumLearnerReplicas {
			return false
		}
	} else if this.NumLearnerReplicas != nil {
		return false
	} else if that1.NumLearnerReplicas != nil {
		return false
	}
	if len(this.Constraints) != len(that1.Constraints) {
		return false
	...
	if len(this.LearnerConstraints) != len(that1.LearnerConstraints) {
		return false
	}
	for i := range this.LearnerConstraints {
		if !this.LearnerConstraints[i].Equal(&that1.LearnerConstraints[i]) {
			return false
		}
	}
	...
	if this.InheritedLearnerConstraints != that1.InheritedLearnerConstraints {
		return false
	}
...
func (m *ZoneConfig) MarshalTo(dAtA []byte) (int, error) {
	...
	i++
	if m.NumLearnerReplicas != nil {
		dAtA[i] = 0x60
		i++
		i = encodeVarintZone(dAtA, i, uint64(*m.NumLearnerReplicas))
	}
	if len(m.LearnerConstraints) > 0 {
		for _, msg := range m.LearnerConstraints {
			dAtA[i] = 0x6a
			i++
			i = encodeVarintZone(dAtA, i, uint64(msg.Size()))
			n, err := msg.MarshalTo(dAtA[i:])
	...
	dAtA[i] = 0x70
	i++
	if m.InheritedLearnerConstraints {
		dAtA[i] = 1
	} else {
		dAtA[i] = 0
	}
...
func NewPopulatedZoneConfig(r randyZone, easy bool) *ZoneConfig {
	...
	if r.Intn(10) != 0 {
		...
		this.NumLearnerReplicas = &v16
	}
	if r.Intn(10) != 0 {
		v17 := r.Intn(5)
		this.LearnerConstraints = make([]ConstraintsConjunction, v17)
		for i := 0; i < v17; i++ {
			v18 := NewPopulatedConstraintsConjunction(r, easy)
			this.LearnerConstraints[i] = *v18
		}
	}
	this.InheritedLearnerConstraints = bool(bool(r.Intn(2) == 0))
	if !easy && r.Intn(10) != 0 {
	}
	return this
}
...
func NewPopulatedSubzone(r randyZone, easy bool) *Subzone {
	...
	v19 := NewPopulatedZoneConfig(r, easy)
	this.Config = *v19
	if !easy && r.Intn(10) != 0 {
	}
	return this
...
func NewPopulatedSubzoneSpan(r randyZone, easy bool) *SubzoneSpan {
	...
	if r.Intn(10) != 0 {
		v20 := r.Intn(100)
		this.Key = make(github_com_cockroachdb_cockroach_pkg_roachpb.Key, v20)
		for i := 0; i < v20; i++ {
	...
	if r.Intn(10) != 0 {
		v21 := r.Intn(100)
		this.EndKey = make(github_com_cockroachdb_cockroach_pkg_roachpb.Key, v21)
		for i := 0; i < v21; i++ {
...
func randFieldZone(dAtA []byte, r randyZone, fieldNumber int, wire int) []byte {
	...
	case 0:
		...
		dAtA = encodeVarintPopulateZone(dAtA, uint64(v23))
	case 1:
		dAtA = encodeVarintPopulateZone(dAtA, uint64(key))
...
func (m *ZoneConfig) Size() (n int) {
	...
	n += 2
	if m.NumLearnerReplicas != nil {
		n += 1 + sovZone(uint64(*m.NumLearnerReplicas))
	}
	if len(m.LearnerConstraints) > 0 {
		for _, e := range m.LearnerConstraints {
			l = e.Size()
			n += 1 + l + sovZone(uint64(l))
		}
	}
...
func (m *ZoneConfig) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 12:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field NumLearnerReplicas", wireType)
			}
			var v int32
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.NumLearnerReplicas = &v
		case 13:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field LearnerConstraints", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.LearnerConstraints = append(m.LearnerConstraints, ConstraintsConjunction{})
			if err := m.LearnerConstraints[len(m.LearnerConstraints)-1].Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 14:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field InheritedLearnerConstraints", wireType)
			}
			var v int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.InheritedLearnerConstraints = bool(v != 0)
		default:
			iNdEx = preIndex
			skippy, err := skipZone(dAtA[iNdEx:])
			if err != nil {
...
func init() { proto.RegisterFile("config/zonepb/zone.proto", fileDescriptor_zone_89fdc7d03f235849) }

var fileDescriptor_zone_89fdc7d03f235849 = []byte{
	...
	,
}
