...
func (s *SystemConfig) GetDesc(key roachpb.Key) *roachpb.Value {
	...
	id, err := keys.TODOTenantKeyGen.DecodeDescMetadataID(key)
	if err != nil {
...
func (s *SystemConfig) GetLargestObjectID(maxID uint32) (uint32, error) {
	...
	highBound := keys.TODOTenantKeyGen.TablePrefix(keys.DescriptorTableID + 1)
	highIndex := sort.Search(len(s.Values), func(i int) bool {
		return bytes.Compare(s.Values[i].Key, highBound) >= 0
	})
	lowBound := keys.TODOTenantKeyGen.TablePrefix(keys.DescriptorTableID)
	...
	// table.
	if maxID == 0 {
		id, err := keys.TODOTenantKeyGen.DecodeDescMetadataID(s.Values[highIndex-1].Key)
		if err != nil {
			return 0, err
		}
	...
	maxIdx := sort.Search(len(searchSlice), func(i int) bool {
		var id uint64
		id, err = keys.TODOTenantKeyGen.DecodeDescMetadataID(searchSlice[i].Key)
		if err != nil {
			return false
		}
	...
	if maxIdx < len(searchSlice) {
		id, err := keys.TODOTenantKeyGen.DecodeDescMetadataID(searchSlice[maxIdx].Key)
		if err != nil {
			return 0, err
		}
		if uint32(id) == maxID {
	...
	id, err := keys.TODOTenantKeyGen.DecodeDescMetadataID(searchSlice[maxIdx-1].Key)
	if err != nil {
		return 0, err
	}
	return uint32(id), nil
...
func (s *SystemConfig) ComputeSplitKey(startKey, endKey roachpb.RKey) (rr roachpb.RKey) {
	...
	findSplitKey := func(startID, endID uint32) roachpb.RKey {
		...
		for id := startID; id <= endID; id++ {
			tableKey := roachpb.RKey(keys.TODOTenantKeyGen.TablePrefix(id))
...
func (s *SystemConfig) shouldSplit(ID uint32) bool {
	...
	if ID < keys.MinUserDescID {
		...
		desc := s.GetDesc(keys.TODOTenantKeyGen.DescMetadataKey(ID))
