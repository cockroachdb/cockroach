...
// implies that the lookup is actually the system tenant's view of the TTL
...
func (s *SystemConfig) GetZoneConfigForObject(
	...
	// secondary tenant object we're trying to look up, we're in a bit of a
	// pickle. This assumes that if we're in the appropriate version, then so
	// too is the system tenant and things are reconciled. Is it possible
