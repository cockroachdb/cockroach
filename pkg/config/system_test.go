...
func TestGetLargestID(t *testing.T) {
	...
	testCases := []testCase{
		...
		// Real SQL layout.
		func() testCase {
			ms := sqlbase.MakeMetadataSchema(config.DefaultZoneConfigRef())
...
func TestComputeSplitKeySystemRanges(t *testing.T) {
	...
	cfg := config.NewSystemConfig(config.DefaultZoneConfigRef())
	kvs, _ /* splits */ := sqlbase.MakeMetadataSchema(cfg.DefaultZoneConfig).GetInitialValues()
...
func TestComputeSplitKeyTableIDs(t *testing.T) {
	...
	schema := sqlbase.MakeMetadataSchema(config.DefaultZoneConfigRef())
