...
func TestGetLargestID(t *testing.T) {
	...
	testCases := []testCase{
		...
		// Real SQL layout.
		func() testCase {
			ms := bootstrap.MakeMetadataSchema(keys.SystemSQLCodec, zonepb.DefaultZoneConfigRef(), zonepb.DefaultSystemZoneConfigRef(), nil)
...
func TestComputeSplitKeySystemRanges(t *testing.T) {
	...
	kvs, _ /* splits */ := bootstrap.MakeMetadataSchema(
		keys.SystemSQLCodec, cfg.DefaultZoneConfig, zonepb.DefaultSystemZoneConfigRef(), nil,
	).GetInitialValues()
