...
func checkNode(
	...
	for _, urlID := range urlIDs {
		...
		get(ctx, t, c.URL(i), fmt.Sprintf("/_status/gossip/%s", urlID))
		get(ctx, t, c.URL(i), fmt.Sprintf("/_status/nodes/%s", urlID))
		get(ctx, t, c.URL(i), fmt.Sprintf("/_status/logfiles/%s", urlID))
		get(ctx, t, c.URL(i), fmt.Sprintf("/_status/logs/%s", urlID))
		get(ctx, t, c.URL(i), fmt.Sprintf("/_status/stacks/%s", urlID))
	}
}
...
func testStatusServerInner(
	...
	for i := 0; i < c.NumNodes(); i++ {
		...
		get(ctx, t, c.URL(i), "/_status/nodes")
	}
