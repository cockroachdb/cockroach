...
int main(int argc, char const *argv[]) {
	...
	for (int resultFormat = 0; resultFormat <= 1; ++resultFormat) {
			...
			PGdate recvdate;
			if (!PQgetf(result, 0, "%date", 2, &recvdate)) {
			...
			if (!PQgetf(result, 0, "%numeric", 3, &recvnumeric1)) {
			...
			if (!PQgetf(result, 0, "%numeric", 4, &recvnumeric2)) {
				fprintf(stderr, "ERROR resultFormat=%d PQgetf(numeric): %s\n", resultFormat, PQgeterror());
				return 1;
			}
			...
			PGfloat8 recvf8;
			if (!PQgetf(result, 0, "%float8", 5, &recvf8)) {
			...
			PGint8 recvi8;
			if (!PQgetf(result, 0, "%int8", 6, &recvi8)) {
			...
			if (0) {
				...
				if (!PQgetf(result, 0, "%interval", 7, &recvinterval)) {
			...
			PGtext recvtext;
			if (!PQgetf(result, 0, "%text", 7, &recvtext)) {
			...
			PGtimestamp recvts;
			if (!PQgetf(result, 0, "%timestamp", 8, &recvts)) {
			...
			// TODO(dt): ERROR resultFormat={0,1} PQgetf(timestamp): Trying to get type 1114 'pg_catalog.timestamp' but server returned 1184 (position 1)
			if (0) {
				PGtimestamp recvtstz;
				if (!PQgetf(result, 0, "%timestamp", 9, &recvtstz)) {
					fprintf(stderr, "ERROR resultFormat=%d PQgetf(timestamp): %s\n", resultFormat, PQgeterror());
					return 1;
				}
