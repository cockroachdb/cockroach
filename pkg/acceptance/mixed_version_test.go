...
func testMixedVersionHarness(ctx context.Context, t *testing.T, cfg cluster.TestConfig) {
	...
	var cnt int
	if err := db.QueryRow("SELECT COUNT(*) FROM system.settings WHERE name = 'version';").Scan(&cnt); err != nil {
		t.Fatal(err)
	}
	...
	// won't declare startup complete until migrations have run.
	if cnt < 1 {
