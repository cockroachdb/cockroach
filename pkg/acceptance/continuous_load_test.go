...
func (cl continuousLoadTest) Run(ctx context.Context, t *testing.T) {
	...
	if fl := flag.Lookup("test.timeout"); fl != nil {
		...
		if setupTeardownDuration := 10 * time.Minute; timeout <= setupTeardownDuration {
			...
			timeoutCtx, cancel = context.WithTimeout(timeoutCtx, timeout-setupTeardownDuration)
			defer cancel()
		}
	}
	if deadline, ok := timeoutCtx.Deadline(); ok {
		log.Infof(timeoutCtx, "load test will end at %s", deadline)
	} else {
		log.Infof(timeoutCtx, "load test will run indefinitely")
	}
	...
	checkGossip(timeoutCtx, t, f, longWaitTime, hasPeers(cl.NumNodes))
	start := timeutil.Now()
	if err := cl.startLoad(timeoutCtx, f); err != nil {
		t.Fatal(err)
	}
	cl.assert(timeoutCtx, t, f)
	...
	for {
		...
		case <-healthCheckTimer.C:
			...
			lastHealthCheck = timeutil.Now()
			healthCheckTimer.Reset(healthCheckInterval)
			log.Infof(timeoutCtx, "health check ok %s (%.1f qps)", timeutil.Now().Sub(start), qps)
		case <-timeoutCtx.Done():
			log.Infof(timeoutCtx, "load test finished")
			cl.assert(timeoutCtx, t, f)
			if err := f.Stop(timeoutCtx, 0, cl.Process); err != nil {
				t.Error(err)
			}
			return
