...
func fetchAndCacheBinary(ctx context.Context, destDir, version string) (string, error) {
	...
	binary := filepath.Join(destDir, base)

	if _, err := os.Stat(binary); err == nil {
		return binary, nil // cache hit
	} else if !os.IsNotExist(err) {
		return "", err
	}

	log.Infof(ctx, "downloading %s to %s", u.String(), binary)
	destFile, err := os.Create(binary)
	if err != nil {
		return "", err
	}
	...
	for {
		...
		case tar.TypeReg:
			...
			if _, err := io.Copy(destFile, tarReader); err != nil {
				return "", errors.Wrapf(err, "while downloading %s to %s", u, binary)
			}
		default:
