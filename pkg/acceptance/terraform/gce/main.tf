...
FILE
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "[ $(stat --format=%s cockroach) -ne 0 ] || curl -sfSL https://edge-binaries.cockroachdb.com/cockroach/cockroach.linux-gnu-amd64.LATEST -o cockroach && chmod +x cockroach",
      ...
      "curl -sfSL https://edge-binaries.cockroachdb.com/examples-go/block_writer.LATEST -o block_writer && chmod +x block_writer",
      "curl -sfSL https://edge-binaries.cockroachdb.com/examples-go/photos.LATEST -o photos && chmod +x photos",
    ]
  }
}
