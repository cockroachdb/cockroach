...
func TestDockerJava(t *testing.T) {
	...
	classPath := ".:/junit.jar:/hamcrest.jar:/postgres.jar:src/main/java"
	testDockerSuccess(ctx, t, "java", []string{"/bin/sh", "-c", strings.Replace(java, "%v", classPath, 2)})
	testDockerFail(ctx, t, "java", []string{"/bin/sh", "-c", strings.Replace(java, "%v", ".", 2)})
}
