...
func TestJSONBUpgrade(t *testing.T) {
	...
	RunLocal(t, func(t *testing.T) {
		...
		if err := exec(`ALTER TABLE test.t ADD COLUMN j JSONB`); !testutils.IsError(err, "cluster version does not support JSONB") {
			t.Fatalf("unexpected: %v", err)
		}
		clusterVersionUpgrade(clusterversion.BinaryServerVersion.String()).run(ctx, t, c)
		if err := exec(`ALTER TABLE test.t ADD COLUMN j JSONB`); err != nil {
			t.Fatal(err)
		}
	})
}
