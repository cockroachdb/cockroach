...
// be use in, and only in, the RefreshSettings loop.
//
...
// updaterMu ensures that at-most-one Updater exists at any given time.
// This usually is true anyway as there is only one RefreshSettings loop
// started by Server, so it is only interesting in testcluster where multiple
// Servers are running in a single process.
var updaterMu syncutil.Mutex

// NewUpdater returns a new Updater, pre-alloced based on the current settings.
// The returned Updater must be either Apply()'ed or Close()'ed.
...
func (u Updater) Add(key, rawValue, vt string) error {
	...
	// here to the current value, to preserve it in case we can't read new one.
	if ok {
		u[key] = current
	}
...
// Close releases the updater lock.
