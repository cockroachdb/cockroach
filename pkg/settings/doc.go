...
can be specified using `SetName()`, for example:

		var mySetting = settings.RegisterBoolSetting(
			"mykey", ...,
	  )
	  mySetting.SetName("descriptive.name.enabled")
...
There are two situations possible:
  ...
  - The setting already has another name than its key. In this case,
    modify the name in the `SetName()` method and also call the
    `SetRetiredName()` method with the previous name. A SQL notice
