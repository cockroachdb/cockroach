...
accidentally reused, and surpasses log spam about the existing value.

That list of retired settings can periodically (i.e. in major versions) be
"flushed" by adding a migration that deletes any stored values for those keys
...
older nodes are still using the values for those old settings though.
