...
// MakeTestingClusterSettings is the same as MakeClusterSettings except the
// cluster version setting is initialized to this binary's version.
//
...
func MakeTestingClusterSettings() *Settings {
	s := MakeClusterSettings()

	// Initialize cluster version to specified binaryVersion.
	if err := clusterversion.Initialize(context.TODO(), s.Version.BinaryVersion(), &s.SV); err != nil {
