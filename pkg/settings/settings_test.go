...
var eA = RegisterEnumSetting("e", "", "foo", map[string]int64{"foo": 1, "bar": 2, "baz": 3})
...
func TestCache(t *testing.T) {
	...
	t.Run("read and write each type", func(t *testing.T) {
		...
		if err := u.Set("e", "bar", "e"); err != nil {
			t.Fatal(err)
		}
		if expected, err := "cannot set cluster setting to value 'notAValidValue', acceptable values are *",
