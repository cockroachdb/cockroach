...
func TestAliasedSettings(t *testing.T) {
	...
	s2 := settings.RegisterBoolSetting(settings.SystemOnly, "s2key", "desc", false)
	s2.SetName("s2name")
	s3 := settings.RegisterBoolSetting(settings.SystemOnly, "s3key", "desc", false)
	s3.SetName("s3name-new")
	s3.SetRetiredName("s3name-old")
