...
// locked becomes true once the registry is "live".
var locked bool

// Lock ensures that no new settings can be defined after the gossip worker
...
func RegisterBoolSetting(key, desc string, defVal bool) func() bool {
	if locked {
		panic(fmt.Sprintf("registration is too late: %s", key))
	}
	_, ok := registry[key]
	if ok {
