...
// NewRecoveryServer creates recovery server and underlying plan storage.
// Plan storage is created inside first store directory or inside a vfs for
// the first store if in memory configuration is used.
func NewRecoveryServer(cfg Config, stores *kvserver.Stores) (*loqrecovery.Server, error) {
	...
	if spec.InMemory {
		...
		if spec.StickyInMemoryEngineID != "" {
			if cfg.TestingKnobs.Server == nil {
				return nil, errors.AssertionFailedf("Could not create a sticky " +
			...
			if knobs.StickyEngineRegistry == nil {
				return nil, errors.Errorf("Could not create a sticky " +
	...
	return loqrecovery.NewRecoveryServer(stores, loqrecovery.NewPlanStore(path, fs),
		cfg.TestingKnobs.LOQRecovery), nil
}
