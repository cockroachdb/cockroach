...
func (cfg *Config) CreateEngines(ctx context.Context) (Engines, error) {
	...
	for i, spec := range cfg.Stores.Specs {
		...
		if spec.InMemory {
			...
			bothOpt(storage.MaxSize(sizeInBytes))
			stateOpt(storage.BallastSize(sizeInBytes)) // log engine doesn't need ballast
		...
		if spec.ExperimentalSeparateRaftLogPath != "" {
			// TODO(sep-raft-log): we'll need the options for the raft log engine to be determined separately.
			// This will do for basic experimentation though.
