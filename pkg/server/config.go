...
func MakeConfig(st *cluster.Settings) Config {
	...
	var tsMaxSizeBytes int64
	if storeSpec.InMemory {
		tsMaxSizeBytes = defaultTempStorageMaxSizeBytes
	} else {
		tsMaxSizeBytes = DefaultTempStorageMaxSizeBytesInMemStore
	}
	...
	cfg := Config{
		...
		TempStorageConfig: base.MakeTempStorageConfig(
			storeSpec.InMemory,
			tsMaxSizeBytes,
			storeSpec.Path,
		),
