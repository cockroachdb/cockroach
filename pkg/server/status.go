...
func userFromContext(ctx context.Context) (string, error) {
	...
	if !ok {
		log.Warningf(ctx, "context does not contain the expected incoming metadata: %+v", ctx)
		return "", errors.New("context does not contain the expected incoming metadata")
	}
	usernames, ok := md[webSessionUserKeyStr]
	if !ok {
		// If the incoming context has metadata but no attached web session user,
		// it's a gRPC connection which has root on the cluster (#32253).
