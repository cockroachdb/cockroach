...
func (s *statusServer) Profile(
	...
	if req.SenderServerVersion != nil {
		...
		if !resp.Desc.ServerVersion.Equal(*req.SenderServerVersion) {
			return nil, errors.Newf("node %d version %s != sender version %s", resp.Desc.NodeID,
				resp.Desc.ServerVersion.String(), req.SenderServerVersion.String())
		}
	}
