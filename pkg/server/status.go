...
func (si *systemInfoOnce) systemInfo(ctx context.Context) serverpb.SystemInfo {
	si.once.Do(func() { si.info = getSystemInfo(ctx) })
	return si.info
}

// getSystemInfo returns a populated SystemInfo. If an error occurs it is logged
// but the corresponding field in the returned struct is just left empty as
// there isn't anything a client ought to do with an error from this function.
func getSystemInfo(ctx context.Context) (si serverpb.SystemInfo) {
	...
	si.SystemInfo = string(bytes.TrimSpace(output))
	...
	si.KernelInfo = string(bytes.TrimSpace(output))
	return si
}
