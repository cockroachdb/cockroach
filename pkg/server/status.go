...
func (s *statusServer) GetFiles(
	...
	case serverpb.FileType_HEAP: // Requesting for saved Heap Profiles.
		...
		for _, pattern := range req.Patterns {
			if !validFilePattern(pattern) {
				return nil, grpcstatus.Errorf(codes.InvalidArgument, "pattern not allowed: %s", pattern)
			}
			...
			for _, path := range filepaths {
				...
				if os.IsNotExist(err) {
					return nil, grpcstatus.Errorf(codes.Internal, "invalid path: %s", path)
				}
				...
				resp.Files = append(resp.Files,
					&serverpb.File{FileName: fileinfo.Name(), FileSize: fileinfo.Size(), FileContents: contents})
			}
		}
		return &resp, err
	default:
...
// validFilePattern returns whether a pattern represents a possible
// file in current directory.
func validFilePattern(pattern string) bool {
	if strings.Contains(pattern, string(os.PathSeparator)) ||
		strings.Contains(pattern, string(os.PathListSeparator)) {
		return false
	}
	return true
}
