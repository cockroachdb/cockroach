...
func (s *adminServer) getDatabaseTableSpans(
	...
	for _, tableName := range tableNames {
		escQualTable, err := getFullyQualifiedTableName(dbName, tableName)
		if err != nil {
			return nil, err
		}
		tableID, err := s.queryTableID(ctx, userName, dbName, escQualTable)
		if err != nil {
			return nil, s.serverError(err)
		}
...
func (s *adminServer) getDatabaseStats(
	...
	for tableName, tableSpan := range tableSpans {
		err := s.server.stopper.RunAsyncTask(
