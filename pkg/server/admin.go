...
func (s *adminServer) SetUIData(
	...
	for key, val := range req.KeyValues {
		...
		ie := s.sqlServer.internalExecutorFactory.MakeInternalExecutorWithoutTxn()
...
func (s *systemAdminServer) VisitAvailableNodesWithRetry(
	...
	// We can't dial nodes directly from iterate because it holds the lock on
	// gossip and replica collection operation is slow.
	err := s.server.gossip.IterateInfos(gossip.KeyNodeDescPrefix, func(key string, i gossip.Info) error {
		b, err := i.Value.GetBytes()
		if err != nil {
			return errors.Wrapf(err, "failed to get node gossip info")
		}

		var d roachpb.NodeDescriptor
		if err := protoutil.Unmarshal(b, &d); err != nil {
			return errors.Wrapf(err, "failed to unmarshal node gossip info")
		}
	...
	collectNodeWithRetry := func(nodeID roachpb.NodeID) error {
		...
		for r := retry.StartWithCtx(ctx, retryOpts); r.Next(); {
			...
			if iterutil.Map(err) == nil {
				// Pass through stop iterator error to caller, so that it can stop
				// iteration gracefully.
				return err
			}
			var r bool
			if r, err = loqrecovery.IsRetryError(err); !r {
	...
	for _, nodeID := range nodeIDs {
		err := collectNodeWithRetry(nodeID)
		if err != nil {
