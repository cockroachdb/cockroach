...
func (s *adminServer) Databases(
	...
	r, err := s.server.sqlExecutor.ExecuteStatements(session, "SHOW DATABASES;", nil)
	if err != nil {
...
func (s *adminServer) DatabaseDetails(
	...
	r, err := s.server.sqlExecutor.ExecuteStatements(session, query, nil)
	if err != nil {
		return nil, s.serverError(err)
	}
	defer r.Close(ctx)
...
func (s *adminServer) Events(
	...
	r, err := s.server.sqlExecutor.ExecuteStatements(session, q.String(), q.QueryArguments())
	if err != nil {
		return nil, s.serverError(err)
	}
	defer r.Close(ctx)
...
func (s *adminServer) getUIData(
	...
	r, err := s.server.sqlExecutor.ExecuteStatements(session, query.String(), query.QueryArguments())
	if err != nil {
		return nil, s.serverError(err)
	}
	defer r.Close(ctx)
...
func (s *adminServer) SetUIData(
	...
	for key, val := range req.KeyValues {
		...
		r, err := s.server.sqlExecutor.ExecuteStatements(session, query, &qargs)
		if err != nil {
			return nil, s.serverError(err)
		}
		defer r.Close(ctx)
...
func (s *adminServer) queryZone(
	...
	r, err := s.server.sqlExecutor.ExecuteStatements(session, query, &params)
	if err != nil {
	...
	if err := scanner.ScanIndex(result.Rows.At(0), 0, &zoneBytes); err != nil {
		return config.ZoneConfig{}, false, err
	}
...
func (s *adminServer) queryNamespaceID(
	...
	var id int64
	scanner := resultScanner{}
	if err := scanner.ScanIndex(result.Rows.At(0), 0, &id); err != nil {
		return 0, err
	}
