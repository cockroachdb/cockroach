...
func (s *Server) doDrain(
	...
	for _, mode := range modes {
		...
		case serverpb.DrainMode_CLIENT:
			...
			if err := func() error {
				...
				// work.
				defer s.sqlServer.leaseMgr.SetDraining(setTo)

				if !setTo {
					s.sqlServer.distSQLServer.Undrain(ctx)
					s.sqlServer.pgServer.Undrain()
					return nil
				}

				drainMaxWait := queryWait.Get(&s.st.SV)
				if err := s.sqlServer.pgServer.Drain(drainMaxWait); err != nil {
					return err
				}
				s.sqlServer.distSQLServer.Drain(ctx, drainMaxWait)
				return nil
			}(); err != nil {
				return nil, err
			}
	...
	if s.sqlServer.pgServer.IsDraining() {
