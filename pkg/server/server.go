...
func NewServer(cfg Config, stopper *stop.Stopper) (*Server, error) {
	...
	// Preferably we'd record the temporary path before creating it, but
	// ioutil.TempDir automatically creates the directory when generating
	// the arbitrary temporary directory path.
	// In case where the node crashes before writing to the record file,
	// This is a non-issue since the empty directory is insignificantly
	// small.
	var err error
	...
	if !firstStore.InMemory {
		...
		if err = recordTempDir(firstStore.Path, s.cfg.TempStorageConfig.Path); err != nil {
			return nil, errors.Wrap(err, "could not record temporary directory path to record file")
		}
	}
	...
	// Remove temporary directory linked to tempEngine on shutdown.
