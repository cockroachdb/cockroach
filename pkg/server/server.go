...
func maybeImportTS(ctx context.Context, s *Server) error {
	...
	if knobs.ImportTimeseriesMapping == "" {
		return errors.Errorf("need to specify COCKROACH_DEBUG_TS_IMPORT_MAPPING; it should point at " +
			"a YAML file that maps StoreID to NodeID. For example, if s1 is on n1 and s2 is on n5:\n\n1: 1\n2:5")
	}
	mapBytes, err := ioutil.ReadFile(knobs.ImportTimeseriesMapping)
	if err != nil {
		return err
	}
	...
	if len(storeIDs) > 0 {
		return errors.Errorf(
			...
			storeIDs, nodeIDs, knobs.ImportTimeseriesMapping)
	}
