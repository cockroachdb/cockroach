...
func (s *Server) Start(ctx context.Context) error {
...

	{
		...
		cb := func(ctx context.Context, newCV clusterversion.ClusterVersion) {
			...
			prevCV, err := kvserver.SynthesizeClusterVersionFromEngines(ctx, s.engines, v.BinaryVersion(), v.BinaryMinSupportedVersion())
			if err != nil {
				log.Fatal(ctx, err)
			}
		...
		// The version setting loaded from disk (in setupInitServer above) is
		// the maximum cluster version seen on any engine. If new stores are
		// being added to the server right now, or if the process crashed
		// earlier half-way through the callback, that version won't be on
		// all engines. For that reason, we backfill once.
