...
func (s *Server) startSampleEnvironment(frequency time.Duration) {
	...
	ctx := s.AnnotateCtx(context.Background())
	rssChan := make(chan heapprofiler.RssVal)
	s.stopper.RunWorker(ctx, func(ctx context.Context) {
		...
		for {
			...
			case <-ticker.C:
				...
				rssChan <- heapprofiler.RssVal{Timestamp: timeutil.Now(), Rss: s.runtime.Rss.Value()}
	...
	heapProfilerWorker, err := heapprofiler.NewWorker(
		rssChan, s.cfg.OOMHeuristicProfileDirName, s.stopper, systemMemory,
		frequency, s.st,
	)
	if err != nil {
