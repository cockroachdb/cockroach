...
func publishPendingLossOfQuorumRecoveryEvents(
	...
	if err := stopper.RunAsyncTask(ctx, "publish-loss-of-quorum-events", func(ctx context.Context) {
		if err := stores.VisitStores(func(s *kvserver.Store) error {
		...
		// Should only happen if server is being stopped before fully started.
		log.Errorf(ctx, "failed to schedule publishing loss of quorum events to RangeLog")
	}
}
