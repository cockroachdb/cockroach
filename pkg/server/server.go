...
func (s *Server) PreStart(ctx context.Context) error {
	...
	if s.cfg.SpanConfigsEnabled && s.spanConfigSubscriber != nil {
		err := s.stopper.RunAsyncTask(ctx, "spanconfig-subscriber", func(ctx context.Context) {
			ctx, cancel := s.stopper.WithCancelOnQuiesce(ctx)
			defer cancel()

			for r := retry.Start(base.DefaultRetryOptions()); r.Next(); {
				if ctx.Err() != nil {
					log.Errorf(ctx, "spanconfig-subscriber stopped: %v", ctx.Err())
					return
				}
				if err := s.spanConfigSubscriber.Subscribe(ctx); err != nil {
					log.Errorf(ctx, "spanconfig-subscriber failed with %v, retrying...", err)
				}
			}
		})
		if err != nil {
