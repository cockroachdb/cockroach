...
func (s *Server) PreStart(ctx context.Context) error {
	...
	if err := s.node.stores.VisitStores(func(s *kvserver.Store) error {
		...
		eventCount, err := loqrecovery.RegisterOfflineRecoveryEvents(
			ctx,
			s.Engine(),
			func(ctx context.Context, record loqrecoverypb.ReplicaRecoveryRecord) error {
	...
	if err := s.node.stores.VisitStores(func(s *kvserver.Store) error {
		useOldRangeLogEvents := !s.ClusterSettings().Version.IsActive(ctx,
		...
		_, err := loqrecovery.RegisterOfflineRecoveryEvents(
			...
			func(ctx context.Context, record loqrecoverypb.ReplicaRecoveryRecord) error {
				...
				if err := loqrecovery.UpdateRangeLogWithRecovery(ctx, sqlExec, record,
					useOldRangeLogEvents); err != nil {
					return errors.Wrap(err, "loss of quorum recovery failed to write RangeLog entry")
				}
