...
func TestJoinVersionGate(t *testing.T) {
	...
	if err := serv.Start(); !errors.Is(errors.Cause(err), server.ErrImproperBinaryVersion) {
		t.Fatalf("expected error %s, got %v", server.ErrImproperBinaryVersion.Error(), err.Error())
	}
}
