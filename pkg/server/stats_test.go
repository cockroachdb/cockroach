...
func TestClusterResetSQLStats(t *testing.T) {
	...
	resp, err := status.ResetSQLStats(ctx, &serverpb.ResetSQLStatsRequest{})
	require.NoError(t, err)

	if resp.NumOfNodesReset != 3 {
		t.Fatal("expected all 3 nodes to reset stats, but found:", resp.NumOfNodesReset)
	}
