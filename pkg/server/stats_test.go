...
`); err != nil {
	...
	go func() {
		for i := 0; i < 20; i++ {
			if _, err := sqlDB.Exec(`INSERT INTO t.test VALUES ($1)`, i); err != nil {
				t.Error(err)
			}
	...
	go func() {
		before := timeutil.Now()
		for i := 0; i < 10; i++ {
			// Run the telemetry report function in a loop.
	...
	for _, stat := range stats {
		if strings.HasPrefix(stat.Key.Query, "INSERT INTO t.test VALUES") {
			foundStat = true
			if stat.Stats.Count != 20 {
				t.Fatal("expected to find 20 invocations, found", stat.Stats.Count)
			}
		}
	}
	if !foundStat {
