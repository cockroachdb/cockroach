...
var roSetting = settings.RegisterStringSetting(settings.TenantReadOnly, "str.baz", "desc", "initial")
var sysSetting = settings.RegisterStringSetting(settings.SystemOnly, "str.yay", "desc", "")
...
func TestNotifyCalledUponReadOnlySettingChanges(t *testing.T) {
	...
	notify := func(updated kvpb.TenantSetting) {
		mu.Lock()
		defer mu.Unlock()
