...
func (s *SettingsWatcher) Start(ctx context.Context) error {
	...
	rf, err := s.f.RangeFeed(ctx, "settings", settingsTableSpan, now, func(
		...
		// The below logic is specialized to allow cluster version propagation
		// via the cluster setting. For non-system tenants, there is a different
		// protocol to control this propagation.
