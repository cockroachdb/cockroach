...
func MakeRowDecoder(codec keys.SQLCodec) RowDecoder {
	return RowDecoder{
		codec:     codec,
		colIdxMap: row.ColIDtoRowIndexFromCols(systemschema.SettingsTable.GetPublicColumns()),
	}
}
	...
	// column IDs followed by their values.
	{
		...
		for len(bytes) > 0 {
			...
			if idx, ok := d.colIdxMap.Get(colID); ok {
				res, bytes, err = rowenc.DecodeTableValue(&d.alloc, tbl.Columns[idx].Type, bytes)
				if err != nil {
					return "", "", "", false, err
				}
				switch colID {
