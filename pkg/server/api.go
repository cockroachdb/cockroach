...
func (a *apiV2Server) registerRoutes(innerMux *mux.Router, authMux http.Handler) {
	...
	// - `name` is a string that must uniquely identify this endpoint. Used for
	//   telemetry counters.
	...
	routeDefinitions := []struct{
		...
		{"login/", "login",a.authServer.ServeHTTP, false /* requiresAuth */, regularRole, noOption},
		{"logout/", "logout",a.authServer.ServeHTTP, false /* requiresAuth */, regularRole, noOption},

		// Directly register other endpoints in the api server.
		{"sessions/", "list-sessions", a.listSessions, true /* requiresAuth */ , adminRole, noOption},
	}
