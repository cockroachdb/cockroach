...
func TestMarkerOrdering(t *testing.T) {
	...
	sort.Slice(startMarkers, func(i, j int) bool {
		return bytes.Compare(startMarkers[i], startMarkers[j]) < 0
	})
	...
	sort.Slice(completionMarkers, func(i, j int) bool {
		return bytes.Compare(completionMarkers[i], completionMarkers[j]) < 0
	})
