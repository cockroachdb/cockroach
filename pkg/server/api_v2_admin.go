...
//
// TODO: This API endpoint needs to be split into separate endpoints for grants
// and tables, individually paginated and limited. After those endpoints are
// added, this endpoint might get deprecated.
//
// ---
// parameters:
// - name: database
...
func (a *apiV2Server) databaseDetails(w http.ResponseWriter, r *http.Request) {
	...
	// databaseDetailsHelper limits the list of tables, but not the list of grants
	// which can be an arbitrarily large payload with no limit.
	//
	// TODO(bilal): Split this endpoint into separate grants and tables ones,
	// individually paginated and limited.
	...
	if limit > 0 && len(resp.TableNames) >= limit {
		resp.Next = offset + len(resp.TableNames)
	}
	writeJSONResponse(ctx, w, 200, resp)
}
