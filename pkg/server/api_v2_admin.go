...
type databaseDetailsResponse struct {
	...
	ZoneConfig zonepb.ZoneConfig `json:"zone_config"`
	// The level at which this object's zone configuration is set.
	ZoneConfigLevel serverpb.ZoneConfigurationLevel `json:"zone_config_level,omitempty"`
}
...
//
// Returns details about a database's descriptor name and zone config.
//
// ---
// parameters:
// - name: database
...
func (a *apiV2Server) databaseDetails(w http.ResponseWriter, r *http.Request) {
	...
	resp := databaseDetailsResponse{
		...
		ZoneConfig:      dbDetailsResp.ZoneConfig,
		ZoneConfigLevel: dbDetailsResp.ZoneConfigLevel,
	}
	writeJSONResponse(ctx, w, 200, resp)
}
