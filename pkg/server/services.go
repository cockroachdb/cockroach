...
FROM defaultdb.services AS t`)
	...
	for ; ok; ok, err = it.Next(ctx) {
		...
		j, err := tree.AsJSON(row[0], sessiondatapb.DataConversionConfig{}, time.UTC)
		if err != nil {
...
func (s *serviceMgr) runExec(
	ctx context.Context, username security.SQLUsername, sql string, args ...interface{},
...
func (s *serviceRoute) serveHTTP(w http.ResponseWriter, r *http.Request) {
	...
	username := security.MakeSQLUsernameFromPreNormalizedString("anonymous")
	if s.authnMethod != "none" {
	...
	funcs := map[string]interface{}{
		...
		"query": func(sql string, args ...interface{}) ([][]json.JSON, error) {
			return s.s.runQuery(ctx, username, sql, args...)
		},
		"exec": func(sql string, args ...interface{}) (int, error) {
			return s.s.runExec(ctx, username, sql, args...)
		},
	}
