...
// executeBatchRPCInternal is a helper function which executes a KV
...
func executeBatchRPCInternal(
	...
	sender func(context.Context, roachpb.BatchRequest) (*roachpb.BatchResponse, *roachpb.Error),
	...
	if err := stopper.RunTaskWithErr(ctx, taskName, func(ctx context.Context) error {
		...
		br, pErr = sender(ctx, *args)
		if pErr != nil {
			br = &roachpb.BatchResponse{}
			log.VErrEventf(ctx, 3, "error from %s.Send: %s", senderName, pErr)
		}
		if br.Error != nil {
			panic(roachpb.ErrorUnexpectedlySet(senderName, br))
		}
		if finishCall != nil {
