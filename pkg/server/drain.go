...
func (s *Server) drainClients(ctx context.Context, reporter func(int, redact.SafeString)) error {
	...
	if shouldDelayDraining {
		...
		connectionWaitLogMessage := fmt.Sprintf(
			"draining SQL connections, no new SQL connections allowed now. "+
				"The server will proceed to the next draining phase once all SQL connections "+
				"are closed or after timeout for %s, whichever earlier. "+
		...
		s.sqlServer.pgServer.WaitForDrainConnections(ctx, connectionMaxWait)
	}
