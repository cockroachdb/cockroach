...
func (s *drainServer) drainClients(
	...
	// or the connectionMaxWait timeout elapses, whichever earlier.
	if err := s.sqlServer.pgServer.WaitForSQLConnsToClose(ctx, connectionWait.Get(&s.sqlServer.execCfg.Settings.SV)); err != nil {
		return err
	}
