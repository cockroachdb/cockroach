...
func (s *adminServer) Drain(req *serverpb.DrainRequest, stream serverpb.Admin_DrainServer) error {
	...
	if doDrain {
		progress, info, err := s.server.Drain(ctx)
		if err != nil {
			log.Errorf(ctx, "drain failed: %v", err)
			return err
		}
		res.DrainProgressIndicator = progress
		res.DrainProgressDescription = info
	}
...
func (s *Server) Drain(ctx context.Context) (progress uint64, info string, err error) {
	...
	defer func() {
		...
		for what, howMany := range reports {
			progress += uint64(howMany)
			fmt.Fprintf(&descBuf, "%s%s: %d", comma, what, howMany)
			comma = ", "
		}
		info = descBuf.String()
		log.Infof(ctx, "drain progress: %d", progress)
		if info != "" {
...
func (s *Server) doDrain(ctx context.Context, reporter func(int, string)) error {
	...
	// below would deadlock (trying to set the liveness to draining
