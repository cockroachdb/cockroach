...
func (s *statusServer) StatementDiagnostics(
	ctx context.Context,
	req *serverpb.StatementDiagnosticsPayloadRequest,
) (*serverpb.StatementDiagnosticsPayloadResponse, error) {
	ctx = propagateGatewayMetadata(ctx)
	ctx = s.AnnotateCtx(ctx)

	diagnostics, err := s.stmtDiagnosticsRequester.GetCompletedRequestWithDiagnostics(ctx, int(req.RequestId))
	if err != nil {
		return nil, err
	}

	diagnosticsProto := diagnostics.ToProto()
	response := &serverpb.StatementDiagnosticsPayloadResponse{
