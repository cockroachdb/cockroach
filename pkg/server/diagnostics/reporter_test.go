...
func TestTenantReport(t *testing.T) {
	...
	// Verify Tenant.
	require.Equal(t, tenantArgs.TenantID, roachpb.MakeTenantID(last.Tenant.TenantID))
	require.NotEmpty(t, last.Tenant.Build.Tag)
	require.NotEmpty(t, last.Tenant.Build.Distribution)
	require.NotEmpty(t, last.Tenant.LicenseType)

	// Verify FeatureUsage.
	require.NotZero(t, len(last.FeatureUsage))
}

// TestServer checks nodes, stores, localities, and zone configs. Telemetry
// metrics are checked in datadriven tests (see sql.TestTelemetry).
...
func (t *reporterTest) Close() {
	...
	defer t.diagServer.Close()
	defer t.server.Stopper().Stop(context.Background())
}
...
func setupCluster(t *testing.T, db *gosql.DB) {
	...
	_, err = db.Exec(`SET CLUSTER SETTING diagnostics.reporting.enabled = false`)
	require.NoError(t, err)
