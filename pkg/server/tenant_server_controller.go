...
func (s *Server) StartInMemoryAppTenant(ctx context.Context) error {
	...
	datums, err := ie.QueryRowEx(ctx, "get-app-tenant-id", nil, /* txn */
		sessiondata.InternalExecutorOverride{User: username.RootUserName()},
		// TODO(knz): This might benefit from an index?
		`
SELECT id
  FROM system.tenants
 WHERE COALESCE(crdb_internal.pb_to_json('cockroach.sql.sqlbase.TenantInfo', info)->'autostart', 'false'::JSONB)::bool
 LIMIT 1
`)
	if err != nil {
		return err
	...
	if datums == nil {
		log.Ops.Infof(ctx, "no app tenant found or auto_start_network not set")
		// Nothing else to do.
		return nil
	}
...
func checkTenantExists(
	...
	if rowCount == 0 {
		return errors.Newf("tenant %v not found", tenantID)
	}
	return nil
}
