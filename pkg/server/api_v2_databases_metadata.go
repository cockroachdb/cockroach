...
// GetTableMetadataForId fetches table metadata for a specific table id.
//
...
//
//	"200":
//	  description: A tableMetadataForIdResponse containing the table metadata and table create statement.
//	"404":
//		description: If the table for the provided id doesn't exist or the user doesn't have necessary permissions
//								 to access the table
func (a *apiV2Server) GetTableMetadataForId(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodGet {
		http.Error(w, http.StatusText(http.StatusMethodNotAllowed), http.StatusMethodNotAllowed)
		return
	}
	...
	resp := tableMetadataForIdResponse{
		Metadata:        tmd,
...
type tableMetadataForIdResponse struct {
