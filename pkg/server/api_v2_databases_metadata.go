...
// all the tokens are matched against both schema and table name. For a table to match this parameter, each token must
// match either the schema name or the table name.
//
// name: sortBy
// type: string
// description: Which column to sort by. This currently supports: "replicationSize", "ranges", "liveData", "columns",
// "indexes", "lastUpdated". If a non supported value is provided, it will be ignored.
//
// name: sortOrder
// type: string
...
// name: storeId
// type: integer
// description: The id of the store to filter tables by. If the table contains data the store, it will be included in
// the result set. Multiple storeId query parameters are support. If multiple are provided, a table will be included in
// the result set if it contains data in at least one of the stores.
func (a *apiV2Server) GetTableMetadata(w http.ResponseWriter, r *http.Request) {
	...
	qs := struct {
		dbId      apiutil.QueryStringVal[int]
		...
		dbId:      apiutil.GetIntQueryStringVal(queryValues, dbIdKey),
		tableName: apiutil.GetQueryStringVal(queryValues, nameKey),
	...
	if qs.tableName.Exists {
		tokenized := strings.Split(qs.tableName.Value, ".")
		for _, token := range tokenized {
			nameFilters = append(nameFilters, fmt.Sprintf("%%%s%%", token))
		}
	}

	tmd, totalRowCount, err := a.getTableMetadata(ctx, sqlUser, dbId, nameFilters, qs.storeIds.Value, sortBy, sortOrder,
		pageSize, offset)
