...
func TestInitHandshake(t *testing.T) {
	...
	go func() {
		errReturned <- InitHandshake(ctx, nil, cfg1, "foobar", 3, []string{addr2, addr3}, cfg1.SSLCertsDir, listener1)
	}()
	go func() {
		errReturned <- InitHandshake(ctx, nil, cfg2, "foobar", 3, []string{addr1, addr3}, cfg1.SSLCertsDir, listener2)
	}()
	go func() {
		errReturned <- InitHandshake(ctx, nil, cfg3, "foobar", 3, []string{addr1, addr2}, cfg1.SSLCertsDir, listener3)
	}()
...
func TestInitHandshakeWrongToken(t *testing.T) {
	...
	go func() {
		errReturned <- InitHandshake(ctx, nil, cfg2, "foobarbaz", 3, []string{addr1, addr3}, cfg1.SSLCertsDir, listener2)
	}()
	go func() {
		errReturned <- InitHandshake(ctx, nil, cfg3, "foobar", 3, []string{addr1, addr2}, cfg1.SSLCertsDir, listener3)
	}()
