...
func TestInitHandshakeWrongToken(t *testing.T) {
	...
	go func() {
		errReturned <- InitHandshake(ctx, cfg2, "foobarbaz", []string{"localhost:8080", "localhost:8082"}, cfg2.SSLCertsDir, "localhost", "8081")
	}()
	go func() {
		errReturned <- InitHandshake(ctx, cfg3, "foobar", []string{"localhost:8080", "localhost:8081"}, cfg3.SSLCertsDir, "localhost", "8082")
	}()
	...
	for count < 3 {
		...
		case err := <-errReturned:
			if err == nil {
				t.Fatal("expected non-nil error from node with wrong token")
			}
