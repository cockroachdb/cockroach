...
func TestHotRangeLoggerMultitenant(t *testing.T) {
	...
	fmt.Printf("registry %T\n", tt.JobRegistry())
	// check the registrys that for the main server there is none, for the
	// tenant there is one.

	// check the log duration behavior for each.
	defer teardown()
}
...
func setupTestServer(
	...
	for range 1000 {
		ts.SQLConn(t).Exec("SELECT * FROM system.namespace")
	}
...
func setupTest(
	...
	structlogging.LogCPUThreshold = logCPUThreshold
