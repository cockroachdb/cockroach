...
// LogCPUThreshold defines the cpu duration per second
// which needs to be exceeded for the system to automatically
// log the hot ranges.
var LogCPUThreshold = time.Second / 4
...
type HotRangesServer interface {
	HotRangesV2(serverpb.HotRangesRequest) (*serverpb.HotRangesResponseV2, error)
}
...
// shouldLog is a passthrough to the below should log implementation,
// where the requisite information is pulled out so that the
// rules can be applied.
//
// The enabled check is pulled upwards so that we don't have to
// make the rpc call if it's disabled.
...
// should emit logs.
//   - Have we either:
//     -- Gone past the server.telemetry.hot_ranges_stats.interval
//     setting since th last time we logged or...
//     -- Have we seen a hot range with a CPU time per second
...
func shouldLog(
	...
	if maxCPU(ranges) > LogCPUThreshold {
		return true
	}

	if timeSrc.Since(lastLogged) > logInterval {
		return true
	}
