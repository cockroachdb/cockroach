...
func (s *systemStatusServer) spanStatsFanOut(
	...
	nodeFn := func(ctx context.Context, client interface{}, nodeID roachpb.NodeID) (interface{}, error) {
		...
		if req.SkipApproxTotalStats {
			skipMvccStats = true
			for _, span := range spansPerNode[nodeID] {
...
func (s *systemStatusServer) getSpansPerNode(
	ctx context.Context, req *roachpb.SpanStatsRequest,
) (spansPerNode map[roachpb.NodeID][]roachpb.Span, mvccNodeForSpan map[string]roachpb.NodeID, err error) {
...
func nodeIDsForSpan(
	...
	// Track all voters for each range to determine the best MVCC node.
	rangeToVoters := make(map[roachpb.RangeID][]roachpb.NodeID)
	...
	err = scanner.Scan(ctx, pageSize, func() {
		...
		for _, desc := range scanned {
			...
			// Collect all voter node IDs for this range.
			voters := desc.Replicas().VoterDescriptors()
			voterNodeIDs := make([]roachpb.NodeID, len(voters))
			for i, v := range voters {
				voterNodeIDs[i] = v.NodeID
			}
			rangeToVoters[desc.RangeID] = voterNodeIDs
		}
		return nil
	})
	if err != nil {
	...
	// Calculate which node is a voter for the most ranges - this node
	// is most likely to be leaseholder for many ranges and is a good
	// candidate for fetching MVCC stats.
	nodeCoverage := make(map[roachpb.NodeID]int)
	for _, voters := range rangeToVoters {
