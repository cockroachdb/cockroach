...
func (s *SpanStatsServer) getSamplesFromFanOut(
	...
	responseFn := func(nodeID roachpb.NodeID, resp interface{}) {
		...
		for _, sampleFragment := range nodeResponse.Samples {
			t := sampleFragment.SampleTime.GoTime().Truncate(samplePeriod).UnixNano()
			if _, ok := globalSamples[t]; ok {
				globalSamples[t] = append(globalSamples[t], sampleFragment)
			} else {
				globalSamples[t] = []*keyvispb.Sample{sampleFragment}
			}
		}
	}
	...
	for timestampNanos, sampleFragments := range globalSamples {
		...
		samples = append(samples, &keyvispb.Sample{
			SampleTime: &hlc.Timestamp{WallTime: timestampNanos},
