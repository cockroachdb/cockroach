...
func (s *systemStatusServer) spanStatsFanOut(
	...
	// Cannot have a request with both the legacy and current format.
	if isLegacyRequest(req) && len(req.Spans) != 0 {
		return nil, errors.New(InvalidRequestPayloadMsg)
	}
...
func (s *systemStatusServer) getLocalStats(
	...
	for _, span := range req.Spans {
		...
		for {
			...
			if rSpan.ContainsKeyRange(descSpan.Key, desc.EndKey) {
				...
				if len(fullyContainedKeysBatch) > int(roachpb.RangeStatsBatchLimit.Get(&s.st.SV)) {
					...
					fullyContainedKeysBatch, err = flushBatchedContainedKeys(ctx, s.rangeStatsFetcher, fullyContainedKeysBatch, spanStats, rSpan)
					if err != nil {
						return nil, err
					}
				}
		...
		if len(fullyContainedKeysBatch) > 0 {
			// Obtain stats for fully contained ranges via RangeStats.
			_, err = flushBatchedContainedKeys(ctx, s.rangeStatsFetcher, fullyContainedKeysBatch, spanStats, rSpan)
			if err != nil {
				return nil, err
			}
			// Nil the batch.
...
func (s *systemStatusServer) buildNodeSpanAndServerMapping(
	...
	//fmt.Println("buildNodeSpanAndServerMapping - req spans", req.Spans)
