...
// GetTenantRanges implements the keyvispb.KeyVisualizerServer interface.
func (s *SpanStatsServer) GetTenantRanges(
	...
	tenantPrefix := keys.MakeTenantPrefix(*req.Tenant)
	tenantKeySpan := roachpb.Span{
		Key:    tenantPrefix,
		EndKey: tenantPrefix.PrefixEnd(),
	}

	ranges := make([]*roachpb.Span, 0)
	...
	return &keyvispb.GetTenantRangesResponse{Ranges: ranges}, nil
}
