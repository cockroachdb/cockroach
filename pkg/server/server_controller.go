...
type newServerFn func(ctx context.Context, parentStopper *stop.Stopper, tenantName string, index int) (onDemandServer, error)
...
func (c *serverController) get(ctx context.Context, tenantName string) (onDemandServer, error) {
	...
	s, err := c.newServerFn(ctx, c.stopper, tenantName, idx)
	if err != nil {
		return nil, err
	}
...
func (c *serverController) Close() {
	...
	// Stop all stoppable servers concurrently.
	for _, e := range c.mu.servers {
		if e.shouldStop {
			e.server.stop(context.Background())
		}
	}
}
...
func (s *Server) newServerForTenant(
	ctx context.Context, parentStopper *stop.Stopper, tenantName string, index int,
...
func (s *Server) startInMemoryTenantServerInternal(
	...
	// TODO(knz): Don't capture 'stopper' in the closure, instead
	// use a method on the server collection.
