...
func (c *serverController) sqlMux(
	...
	case pgwire.PreServeCancel:
		...
		// Wait for at least one of the servers to report completion.
		//
		// We don't want to log a warning if cancellation
		// has succeeded.
		// This is achieved here because when the cancellation succeeds
		// in at least one server, that server's task above will
		// complete quickly and report nil quickly on the error channel.
		if firstErr := <-errCh; firstErr != nil {
			log.Sessions.Warningf(ctx, "unexpected while handling pgwire cancellation request: %v", firstErr)
		}
		return nil
	...
	case pgwire.PreServeReady:
		...
		if tenantName == "" {
			// TODO(knz): Make the default tenant route for SQL configurable.
			// See: https://github.com/cockroachdb/cockroach/issues/91741
