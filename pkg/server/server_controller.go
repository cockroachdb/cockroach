...
func (c *serverController) sqlMux(
	...
	case pgwire.PreServeCancel:
		...
		// pick it up.
		var err error
		atLeastOneCancellationSucceeded := false
		for _, s := range c.getServers() {
			thisErr := s.server.handleCancel(ctx, status.CancelKey)
			atLeastOneCancellationSucceeded = atLeastOneCancellationSucceeded || thisErr == nil
			err = errors.CombineErrors(err, thisErr)
		}
		if !atLeastOneCancellationSucceeded && err != nil {
			// We only log a warning if none of the servers was able to
