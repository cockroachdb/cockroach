...
func TestApplyMetricsFilter(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			result, matchCounts, err := applyMetricsFilter(metadata, tc.filter)
			...
			for pattern, expectedCount := range tc.expectedMatchCounts {
				require.Equal(t, expectedCount, matchCounts[pattern], "match count for pattern %s", pattern)
			}
		})
	}
}
...
func TestGetInternalTimeseriesNamesFromServer(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			names, matchCounts, err := GetInternalTimeseriesNamesFromServer(ctx, mockClient, tc.filter)
			require.NoError(t, err)
			...
			if tc.expectedMatchCounts != nil {
				...
				// No regex patterns means no match counts (nil or empty)
				require.Empty(t, matchCounts)
			}
		})
	}
}
