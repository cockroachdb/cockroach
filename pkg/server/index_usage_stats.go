...
func getTableIndexUsageStats(
	...
	q := makeSQLQuery()
	// TODO: Implement virtual indexes on index_usages_statistics and table_indexes (Issue #72930)
	q.Append(`
		SELECT
			ti.index_id,
			ti.index_name,
...
func getTableIDFromDatabaseAndTableName(
	...
	datums, err := ie.QueryIteratorEx(ctx, "get-table-id", nil,
		sessiondata.InternalExecutorOverride{
			User:     security.NodeUserName(),
			Database: database,
		}, q.String(), q.QueryArguments()...)
	...
	row := datums.Cur()
	tableID := int(tree.MustBeDInt(row[0]))
	return tableID, nil
}
