...
func computeFaultToleranceImpl[RD replicaDescriber](
	...
	failureDomainMap, err := makeFailureDomains(nodeLocality)
	if err != nil {
		return nil, err
	}
	...
	for replica := range replicas {
		for domainKey, fd := range failureDomainMap {
			...
			// Compute margin by simulating complete failure of this domain (fd).
			// Replicas in fd.nodes are treated as dead; others use actual liveness.
			margin := replica.Desc().Replicas().ReplicationMargin(func(rd roachpb.ReplicaDescriptor) bool {
				if _, ok := fd.nodes[rd.NodeID]; ok {
					return false // Simulate domain failure
				}
...
func makeFailureDomains(
	nodeLocality map[roachpb.NodeID]roachpb.Locality,
) (map[string]*failureDomain, error) {
	leafDomains := make(map[string]*failureDomain)
	for nodeID, locality := range nodeLocality {
		k := locality.String()
		if fd, ok := leafDomains[k]; ok {
			fd.nodes[nodeID] = struct{}{}
		} else {
			leafDomains[k] = &failureDomain{
				domain: locality,
				nodes:  map[roachpb.NodeID]struct{}{nodeID: {}},
			}
		}
	}

	domainMap := maps.Clone(leafDomains)
	unresolved := make([]string, 0, len(leafDomains))
	for k := range leafDomains {
	...
	for len(unresolved) > 0 {
		...
		if parentFailureDomain, ok := domainMap[pdKey]; !ok {
			...
			if err := parentFailureDomain.merge(fd); err != nil {
				return nil, err
			}
		}
	}
...
func (fd *failureDomain) merge(rhs *failureDomain) error {
	if match, _ := rhs.domain.Matches(fd.domain); !match {
		return errors.AssertionFailedf("cannot merge failure domain %s", fd.domain)
	}
