...
func processSystemConfigKVs(
	...
	colIdxMap := row.ColIDtoRowIndexFromCols(tbl.TableDesc().Columns)
	...
	processKV := func(ctx context.Context, kv roachpb.KeyValue, u settings.Updater) error {
		...
		{
			types := []*types.T{tbl.TableDesc().Columns[0].Type}
		...
		// column IDs followed by their values.
		{
			...
			for len(bytes) > 0 {
				...
				if idx, ok := colIdxMap.Get(colID); ok {
					res, bytes, err = rowenc.DecodeTableValue(a, tbl.TableDesc().Columns[idx].Type, bytes)
					if err != nil {
						return err
					}
					switch colID {
					case tbl.TableDesc().Columns[1].ID: // value
						v = string(tree.MustBeDString(res))
					case tbl.TableDesc().Columns[3].ID: // valueType
						t = string(tree.MustBeDString(res))
					case tbl.TableDesc().Columns[2].ID: // lastUpdated
