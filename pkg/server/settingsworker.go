...
func (s *Server) refreshSettings() {
	...
	s.stopper.RunWorker(func() {
		...
		for {
			...
			case <-gossipUpdateC:
				if broken {
					log.Warningf(ctx, "skipping settings update due to previous fatal error")
				} else {
					...
					for _, kv := range cfg.Values {
						if err := processKV(ctx, kv, u); err != nil {
							log.Warningf(ctx, `error decoding settings data: %+v
								This likely indicates table structure or encoding altered, node upgrade required.
								Node will no longer receive settings updates.`, err)
							broken = true
							break
						}
					}
