...
const QueryFileNamePrefix = "activequeryprof"
...
// OOM is probable.
func (o *ActiveQueryProfiler) MaybeDumpQueries(ctx context.Context, registry *sql.SessionRegistry) {
	cgMemUsage, _, err := cgroups.GetMemoryUsage()
	if err != nil {
		log.Errorf(ctx, "failed to fetch cgroup memory usage: %v", err)
		return
	}
	cgInactiveFileUsage, _, err := cgroups.GetMemoryInactiveFileUsage()
	if err != nil {
	...
	if prevMemUsage == 0 || curMemUsage <= prevMemUsage || o.knobs.dontWriteProfiles {
		return
	}
	now := o.now()
	diff := curMemUsage - prevMemUsage
	if curMemUsage+diff >= (o.cgroupMemLimit-varianceBytes) &&
		o.takeQueryProfile(ctx, registry, now, curMemUsage) {
