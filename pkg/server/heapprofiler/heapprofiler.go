...
var (
	percentMinSystemMemory = settings.RegisterFloatSetting(
		"server.heap_profiler.percent_min_system_memory",
		"minimum system memory beyond which profile is triggered if double "+
			" since last profile heuristic matches",
		20,
	)
	...
	minProfileInterval = settings.RegisterDurationSetting(
		...
		"minimum time difference between two heap profiles taken due to rss"+
			" crossing server.heap_profiler.percent_system_memory_threshold",
		time.Second*100,
	)
...
// doubleSinceLastProfileHeuristic is true if latest Rss is more than double of
// the rss at which last profile was taken and latest Rss is more than
// percentMinSystemMemory of system memory. score is the latest Rss.
var doubleSinceLastProfileHeuristic = heuristic{
	name: "double_since_last_profile",
	isTrue: func(s *stats, st *cluster.Settings) (score int64, isTrue bool) {
		currentValue := s.rss
		if currentValue > 2*s.lastProfileRss &&
			(float64(currentValue)*100/float64(s.systemMemory) > percentMinSystemMemory.Get(&st.SV)) {
...
// percentSystemMemoryThreshold percent of system memory. No new profile is
...
var percentSystemMemoryHeuristic = heuristic{
	...
	isTrue: func(s *stats, st *cluster.Settings) (score int64, isTrue bool) {
		...
		if float64(currentValue)*100/float64(s.systemMemory) > percentSystemMemoryThreshold.Get(&st.SV) {
			if s.currentTime().Sub(s.lastProfileTime) < minProfileInterval.Get(&st.SV) ||
...
func NewWorker(
	rssChan <-chan int64, dir string, stopper *stop.Stopper, systemMemory int64, st *cluster.Settings,
