...
// This regexp is constructed from client-side queries written in
...
var localityMetadataQueryRegexp = regexp.MustCompile(`SELECT.+AS store_ids\s+FROM \[SHOW RANGES FROM (DATABASE|TABLE) .+\], unnest\(replicas\) AS unnested`)
