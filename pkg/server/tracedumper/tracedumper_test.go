...
func TestTraceDumperZipCreation(t *testing.T) {
	...
	filename := "foo"
	td.MaybeDump(ctx, filename, 123, s.InternalExecutor().(sqlutil.InternalExecutor))
	expectedFilename := fmt.Sprintf("%s.%s.%s", jobTraceDumpPrefix, baseTime.Format(timeFormat), filename)
	fullpath := td.store.GetFullPath(expectedFilename) + ".zip"
