...
// MaybeDump attempts to dump a trace zip of cluster wide inflight trace spans
...
func (t *TraceDumper) MaybeDump(
	...
	if err != nil {
		log.Errorf(ctx, "failed to collect inflight trace zip: %v", err)
		return
	}
	path := t.store.GetFullPath(traceZipFile) + ".zip"
	f, err := os.Create(path)
	if err != nil {
		log.Errorf(ctx, "error creating file %s for trace dump: %v", path, err)
	}
	defer f.Close()
	_, err = f.Write(zipBytes)
	if err != nil {
		log.Errorf(ctx, "error writing zip file %s for trace dump", path)
	}
}
