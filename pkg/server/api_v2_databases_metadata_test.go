...
func TestTableMetadata(t *testing.T) {
	...
	t.Run("unknown db id", func(t *testing.T) {
		mdResp := makeApiRequest[tableMetadataResponse](t, client, ts.AdminURL().WithPath("/api/v2/table_metadata/?dbId=1000").String())
	...
	t.Run("authorization", func(t *testing.T) {
		...
		mdResp := makeApiRequest[tableMetadataResponse](t, userClient, ts.AdminURL().WithPath(uri1).String())
		...
		mdResp = makeApiRequest[tableMetadataResponse](t, userClient, ts.AdminURL().WithPath(uri2).String())
	...
	t.Run("sorting", func(t *testing.T) {
		...
		var sortTests = []struct {
			...
			{"not supoprt arg", db1Id, "?sortBy=asdfas", defaultTMComparator},
		...
		for _, tt := range sortTests {
			t.Run(tt.name, func(t *testing.T) {
				...
				mdResp := makeApiRequest[tableMetadataResponse](t, client, ts.AdminURL().WithPath(uri).String())
				require.NotEmpty(t, mdResp.Results)
				require.True(t, slices.IsSortedFunc(mdResp.Results, tt.comparator))
				for _, tbmd := range mdResp.Results {
					require.EqualValues(t, tbmd.DbId, tt.dbId)
				}
			})
		}
	})
	...
	t.Run("filter store id", func(t *testing.T) {
		storeIds := []int64{1, 2}
		uri := fmt.Sprintf("/api/v2/table_metadata/?dbId=1&storeId=%d&storeId=%d", storeIds[0], storeIds[1])
		mdResp := makeApiRequest[tableMetadataResponse](t, client, ts.AdminURL().WithPath(uri).String())
