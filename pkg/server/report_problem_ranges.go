...
func (s *reportServer) ProblemRanges(
	ctx context.Context, req *serverpb.ReportProblemRangesRequest,
) (*serverpb.ReportProblemRangesResponse, error) {
	ctx = s.AnnotateCtx(ctx)

	response := &serverpb.ReportProblemRangesResponse{}
	if len(req.NodeID) > 0 {
		var err error
		response.NodeID, _, err = s.status.parseNodeID(req.NodeID)
		if err != nil {
	...
	for nodeID, alive := range isLiveMap {
		if !alive {
			response.Failures = append(response.Failures, serverpb.ReportProblemRangesResponse_Failures{
				NodeID:       nodeID,
		...
		if err := s.stopper.RunAsyncTask(
			...
			func(ctx context.Context) {
				status, err := s.status.dialNode(nodeID)
