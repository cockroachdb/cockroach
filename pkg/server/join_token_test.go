...
func TestJoinToken(t *testing.T) {
	...
	marshalled, err := j.MarshalText()
	require.NoError(t, err)
	j2 := &joinToken{}
	require.NoError(t, j2.UnmarshalText(marshalled))
...
func TestGenerateJoinToken(t *testing.T) {
	...
	var j joinToken
	require.NoError(t, j.UnmarshalText([]byte(token)))
	require.True(t, j.VerifySignature(caCert))
}
