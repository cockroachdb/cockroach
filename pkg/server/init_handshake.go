...
// Create a nodeHostnameAndCA message for sending to a peer node.
...
// Struct to deliver the initialization bundle.
	...
	for {
		...
		if err != nil {
			...
			if err == errInvalidHMAC {
				// TODO(bilal): This node, or the other node, has the wrong signature.
				// This error should be returned to the caller of InitHandshake, instead
				// of just killing the client.
				return
			}
			continue
		}
