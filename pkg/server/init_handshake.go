...
func createNodeInitTempCertificates(
	ctx context.Context, hostname string, lifespan time.Duration,
) (certs ServiceCertificateBundle, err error) {
	log.Ops.Infof(ctx, "creating temporary initial certificates for host %q, duration %s", hostname, lifespan)
	...
	serviceCert, serviceKey, err := security.CreateServiceCertAndKey(serviceCtx, log.Ops.Infof, lifespan, security.NodeUser, initServiceName, hostname, caCert, caKey)
	if err != nil {
		return certs, err
	}
...
func enhanceHandlerContextWithHTTPClient(
	...
	return func(w http.ResponseWriter, req *http.Request) {
		ctx := logtags.AddTag(baseCtx, "client", req.RemoteAddr)
		fn(ctx, w, req)
	}
}
...
func (t *tlsInitHandshaker) runClient(
	...
	for {
		...
		if err != nil {
			log.Ops.Errorf(ctx, "peer CA retrieval error: %v", err)
			// Non-blocking channel send.
			select {
			case t.errors <- err:
			default:
...
func initHandshakeHelper(
	...
	tempCerts, err := createNodeInitTempCertificates(ctx, listenHost, defaultInitLifespan)
	if err != nil {
	...
	if numExpectedPeers > 0 {
		...
		go func() {
			...
			if err := handshaker.startServer(listener); err != nil {
				log.Ops.Errorf(ctx, "handshake server failed to start: %v", err)
			}
		}()
		...
		for len(peerCACerts) < numExpectedPeers {
			...
			case err := <-handshaker.errors:
				...
				log.Ops.Errorf(ctx, "error from client when connecting to peers (retrying): %s", err)
	...
	if trustLeader {
		...
		log.Ops.Infof(ctx, "we not trust leader; now waiting for bundle from trust leader")
	}
