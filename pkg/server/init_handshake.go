...
func (t *tlsInitHandshaker) sendBundle(address string, peerCACert []byte, caBundle nodeTrustBundle) (err error) {
	...
	client := &http.Client{Transport: clientTransport}
	clientTransport.TLSHandshakeTimeout = 3 * time.Second
	clientTransport.ResponseHeaderTimeout = 3 * time.Second
