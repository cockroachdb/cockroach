...
func TestGetDatabaseGrants(t *testing.T) {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, client := range clients {
				...
				if tc.expectedStatus == http.StatusOK {
					...
					require.Equal(t, tc.pageSize, int(apiResp.PaginationInfo.PageSize))
					require.Equal(t, tc.pageNum, int(apiResp.PaginationInfo.PageNum))
	...
	t.Run("Non-existent database", func(t *testing.T) {
		url := fmt.Sprintf("%s/api/v2/grants/databases/99999/?limit=10&offset=0", s.AdminURL())
		req, err := http.NewRequest("GET", url, nil)
		require.NoError(t, err)
...
func TestGetTableGrants(t *testing.T) {
	...
	t.Run("Non-existent table", func(t *testing.T) {
		url := fmt.Sprintf("%s/api/v2/grants/tables/99999/?pageSize=10&pageNum=0", s.AdminURL())
		req, err := http.NewRequest("GET", url, nil)
		require.NoError(t, err)
		resp, err := adminClient.Do(req)
		require.NoError(t, err)
	...
	t.Run("Retrieving grants for tables with special names", func(t *testing.T) {
		...
		conn.Exec(t, fmt.Sprintf(`CREATE database "my.db name"`))
		conn.Exec(t, fmt.Sprintf(`CREATE SCHEMA "my.db name"."my special. schema"`))
