...
// TestCheckRestartSafe_Criticality tests that we treat raft leader nodes and nodes that aren't draining as critical.
func TestCheckRestartSafe_Criticality(t *testing.T) {
	...
	require.Greater(t, res.RaftLeadershipCount, int32(0))
...
// TestCheckRestartSafe_RangeStatus verifies that checkRestartSafe is sensitive to other nodes being down.
func TestCheckRestartSafe_RangeStatus(t *testing.T) {
	...
	require.Equal(t, int32(0), res.RaftLeadershipCount)
...
// TestCheckRestartSafe_AllowMinimumQuorum_Pass sets up an rf=5 cluster and verifies that we treat a node as
// restart-safe when one node is down, and the minimum-quorum flag is set.
...
// TestCheckRestartSafe_AllowMinimumQuorum_Fail verifies that with 2 nodes down, an RF=5 cluster treats the remaining
// nodes as not safe to restart.
...
// TestCheckRestartSafe_Integration relies on actual changes to the cluster state rather than mocked node vitality to
// verify that checkRestartSafe treats remaining nodes as not restart safe when another node is already down.
