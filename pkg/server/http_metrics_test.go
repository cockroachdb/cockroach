...
func TestRegisterMetricsMiddleware(t *testing.T) {
	...
	t.Run("cluster settings", func(t *testing.T) {
		...
		serverMetrics := NewHttpServerMetrics(metric.NewRegistry(), clusterSettings)
...
func BenchmarkHTTPNoMiddleware(b *testing.B) {
	router := mux.NewRouter()
	router.Handle("/{param}", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		time.Sleep(time.Millisecond)
		w.WriteHeader(http.StatusOK)
	...
	for i := 0; i < b.N; i++ {
		http.NewRequest("GET", server.URL+"/1/", nil)
		http.NewRequest("GET", server.URL+"/2/", nil)
		http.NewRequest("POST", server.URL+"/2/", nil)
		http.NewRequest("PUT", server.URL+"/1/", nil)
	}
}
