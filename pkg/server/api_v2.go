...
func planRollingRestart(
	...
	if len(nodeVitality) == len(doneNodes) {
	...
	for nID := range candidateNodes {
		...
		// at this point, if nID is still a candidate, it has no ranges that are under-replicated, or about to be.
	...
	// Fetch metadata and sort by locality to improve grouping
	...
	moreBatches = len(toRestart)+len(doneNodes) < len(nodeVitality)
