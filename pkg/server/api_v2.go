...
func checkRestartSafe(
	...
	err := stores.VisitStores(func(store *kvserver.Store) error {
		...
		store.VisitReplicas(func(replica *kvserver.Replica) bool {
			...
			if neededVoters >= 5 && allowMinimumQuorum && statusString == Underreplicated {
				...
				if futureStatus.Available {
					// Caller has opted in to allowMinimumQuorum restarts, so we don't care if ranges are currently under-replicated,
