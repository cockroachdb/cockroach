...
func (a *apiV2Server) registerRoutes(innerMux *mux.Router, authMux http.Handler) {
	...
	for _, route := range routeDefinitions {
		...
		if !route.tenantEnabled && !a.tenantID.IsSystem() {
			a.mux.Handle(apiV2Path+route.url, http.HandlerFunc(UnimplementedHandlerFunc))
		}
...
func UnimplementedHandlerFunc(w http.ResponseWriter, _ *http.Request) {
