...
func TestLocalSpanStats(t *testing.T) {
	...
	for _, tcase := range testCases {
		...
		require.NoError(t, err)
		// Test a legacy request format from a 22.2 node
		legacyResult, err := s.status.getLocalStats(ctx,
			&roachpb.SpanStatsRequest{
				NodeID:   "0",
				StartKey: rSpan.Key,
				EndKey:   rSpan.EndKey,
		...
		// Assert expected values from legacy request.
		// Note we use the immediate 'RangeCount' and 'TotalStats' fields as the legacy request returns a legacy response format.
		require.Equal(t, legacyResult.RangeCount, tcase.expectedRanges, fmt.Sprintf(
			"Legacy span: expected %d ranges in span [%s - %s], found %d", tcase.expectedRanges, rSpan.Key.String(), rSpan.EndKey.String(), legacyResult.RangeCount))
		require.Equal(t, legacyResult.TotalStats.LiveCount, tcase.expectedKeys, fmt.Sprintf(
			"Legacy span: expected %d keys in span [%s - %s], found %d", tcase.expectedKeys, rSpan.Key.String(), rSpan.EndKey.String(), legacyResult.TotalStats.LiveCount))
