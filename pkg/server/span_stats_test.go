...
func TestSpanStatsFanOutFaultTolerance(t *testing.T) {
	...
	require.Contains(t, res.Errors, "error dialing node 1")
	require.Contains(t, res.Errors, "error dialing node 2")
	require.Contains(t, res.Errors, "kv error on node 3")
	require.Contains(t, res.Errors, "node 4 timed out")
	require.Contains(t, res.Errors, "node 5 timed out")
