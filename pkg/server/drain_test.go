...
`)
	...
	systemStatsQuery := `SELECT count(*) FROM system.statement_statistics WHERE node_id = 1`

	// Sanity check: verify that the statement statistics system table is empty.
	sqlDB.CheckQueryResults(t, systemStatsQuery, [][]string{{"0"}})
	...
	require.NotEqualf(t,
		func() int {
			q := sqlDB.Query(t, systemStatsQuery)
			q.Next()
			var c int
		...
		0,
		"expected to find system table stats")
}
