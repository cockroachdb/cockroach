...
func (s *Server) DecommissionPreCheck(
	...
	decommissionCheckNodeIDs := make(map[roachpb.NodeID]struct{})
	...
	for _, nodeID := range nodeIDs {
		decommissionCheckNodeIDs[nodeID] = struct{}{}
	}
	...
	// Define our node liveness override function to simulate that the nodeIDs
	// for which we are checking decommission readiness are in the DECOMMISSIONING
	// state. All other nodeIDs should use their actual liveness status.
	var livenessFn storepool.NodeLivenessFunc = func(nid roachpb.NodeID, now time.Time, timeUntilStoreDead time.Duration) livenesspb.NodeLivenessStatus {
		if _, ok := decommissionCheckNodeIDs[nid]; ok {
			return livenesspb.NodeLivenessStatus_DECOMMISSIONING
		}

		// By returning unknown, we signal to the store to look up the true liveness.
	...
	err = rangeDescScanner.Scan(ctx, pageSize, initCounters, keys.EverythingSpan, func(descriptors ...roachpb.RangeDescriptor) error {
		for _, desc := range descriptors {
			...
			action, _, recording, rErr := evalStore.AllocatorCheckRange(ctx, &desc, livenessFn)
			rangesChecked += 1
...
func evaluateRangeCheckResult(
	...
	if strictReadiness && !(action == allocatorimpl.AllocatorReplaceDecommissioningVoter ||
		action == allocatorimpl.AllocatorReplaceDecommissioningNonVoter ||
		action == allocatorimpl.AllocatorRemoveDecommissioningVoter ||
		action == allocatorimpl.AllocatorRemoveDecommissioningNonVoter) {
		checkResult.err = errors.Errorf(
			"range r%d needs repair beyond replacing/removing the decommissioning replica",
		)
		return false, checkResult
	}
