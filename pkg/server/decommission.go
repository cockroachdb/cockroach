...
func (ms *decommissionMonitor) onNodeDecommissioned(nodeID roachpb.NodeID) {
	...
	nudger.stop()
	delete(ms.mu.nudgersByNodeID, nodeID) // save some memory
}
...
func (n *nodeDecommissionNudger) start(ctx context.Context) {
	...
	n.AddLogTag(fmt.Sprintf("decommission-nudger,n%d", n.decommissioningNodeID), nil)
	ctx = n.AnnotateCtx(ctx)

	taskName := fmt.Sprintf("decommission-nudger-n%d", n.decommissioningNodeID)
	if err := n.stopper.RunAsyncTask(ctx, taskName, func(ctx context.Context) {
		log.Infof(ctx, "n%d is decommissioning; enqueuing its replicas for rebalancing", n.decommissioningNodeID)
		n.enqueueDecommissioningReplicas(ctx)
...
func (n *nodeDecommissionNudger) enqueueDecommissioningReplicas(ctx context.Context) {
