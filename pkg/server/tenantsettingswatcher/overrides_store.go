...
func spliceOverrideDefaults(
	...
	// 	pretty.Formatter(explicitKeys),
	// 	pretty.Formatter(overrides),
	// 	pretty.Formatter(alternateDefaults))
	aIter, bIter := 0, 0
	for aIter < len(overrides) && bIter < len(alternateDefaults) {
		...
		if overrides[aIter].InternalKey == alternateDefaults[bIter].InternalKey {
			...
			aIter++
			bIter++
			// log.VEventf(context.Background(), 1, "after update 1: overrides=\n%# v", pretty.Formatter(overrides))
			...
			// log.VEventf(context.Background(), 1, "after update 2: overrides=\n%# v", pretty.Formatter(overrides))
		} else {
			numOverrides := 0
			...
			aIter += numOverrides
			bIter += numOverrides
			// log.VEventf(context.Background(), 1, "after update 3 (%d): overrides=\n%# v",
			//				numOverrides, pretty.Formatter(overrides))
		}
	}
	// log.VEventf(context.Background(), 1, "after main iter: overrides=\n%# v", pretty.Formatter(overrides))
	...
	// log.VEventf(context.Background(), 1, "after next iter: overrides=\n%# v", pretty.Formatter(overrides))
	if bIter < len(alternateDefaults) {
		overrides = append(overrides, alternateDefaults[bIter:]...)
	}
	// log.VEventf(context.Background(), 1, "after splice: overrides=\n%# v", pretty.Formatter(overrides))
	return overrides
}
	...
	for aIter < len(dst) && bIter < len(src) {
		if dst[aIter].InternalKey == src[bIter].InternalKey {
			...
			for ; (aIter+numDst) < len(dst) &&
				(bIter+numDst) < len(src) &&
				dst[aIter+numDst].InternalKey > src[bIter+numDst].InternalKey; numDst++ {
			}
			tail := dst[aIter:]
