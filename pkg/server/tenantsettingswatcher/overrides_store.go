...
type tenantOverrides struct {
	...
	// override keys that got their value from the rangefeed.
	// This is used when an alternate default is changed asynchronously
	// after the overrides have been loaded from the rangefeed already.
...
func spliceOverrideDefaults(
	...
	for aIter < len(overrides) && bIter < len(alternateDefaults) {
		if overrides[aIter].InternalKey == alternateDefaults[bIter].InternalKey {
			...
			numOverrides := 0
			for ; (aIter+numOverrides) < len(overrides) &&
				(bIter+numOverrides) < len(alternateDefaults) &&
				overrides[aIter+numOverrides].InternalKey > alternateDefaults[bIter+numOverrides].InternalKey; numOverrides++ {
			}
