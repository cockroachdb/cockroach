...
func TestHeuristic(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(tt *testing.T) {
			...
			assert.Equal(tt, c.expectedDumps, dumps)
		})
	}
}

func TestNewGoroutineDumper(t *testing.T) {
	t.Run("fails because no directory is specified", func(tt *testing.T) {
		_, err := NewGoroutineDumper("")
		assert.EqualError(tt, err, "directory to store dumps could not be determined")
	})

	t.Run("fails because goroutine_dump already exists as a file", func(tt *testing.T) {
		tempDir, dirCleanupFn := testutils.TempDir(tt)
		defer dirCleanupFn()
		path := filepath.Join(tempDir, "goroutine_dump")
		emptyFile, err := os.Create(path)
		assert.NoError(tt, err, "failed to create goroutine_dump file")
		err = emptyFile.Close()
		assert.NoError(tt, err, "failed to close goroutine_dump file")

		_, err = NewGoroutineDumper(tempDir)
		assert.Error(tt, err)
		assert.Equal(tt, "mkdir", err.(*os.PathError).Op)
	})

	t.Run("succeeds", func(tt *testing.T) {
		tempDir, dirCleanupFn := testutils.TempDir(tt)
		defer dirCleanupFn()
		gd, err := NewGoroutineDumper(tempDir)
		assert.NoError(tt, err, "unexpected error in NewGoroutineDumper")
		assert.Equal(tt, int64(0), gd.goroutinesThreshold)
		assert.Equal(tt, int64(0), gd.maxGoroutinesDumped)
		assert.Equal(tt, filepath.Join(tempDir, "goroutine_dump"), gd.dir)
	})
}
	...
	for _, c := range cases {
		t.Run(c.name, func(tt *testing.T) {
			...
			for _, f := range c.files {
				...
				assert.NoError(tt, err, "unexpected error when creating file %s", path)
				err = fi.Close()
				assert.NoError(tt, err, "unexpected error when closing file %s", path)
				err = os.Truncate(path, f.size)
				assert.NoError(tt, err, "unexpected error when truncating file %s", path)
			}
			ctx := context.TODO()
			gc(ctx, tempDir, c.sizeLimit)
			files, err := ioutil.ReadDir(tempDir)
			assert.NoError(tt, err, "unexpected error when listing files in %s", tempDir)
			var actual []string
			for _, f := range files {
...
func TestTakeGoroutineDump(t *testing.T) {
	t.Run("fails because dump already exists as a directory", func(tt *testing.T) {
		tempDir, dirCleanupFn := testutils.TempDir(tt)
		defer dirCleanupFn()
		filename := "goroutine_dump"
		path := filepath.Join(tempDir, filename)
		err := os.Mkdir(path, 0755)
		assert.NoError(tt, err, "failed to make dump directory %s", path)
	...
	t.Run("succeeds", func(tt *testing.T) {
		...
		assert.NoError(tt, err, "unexpected error when dumping goroutines")
	})
}
