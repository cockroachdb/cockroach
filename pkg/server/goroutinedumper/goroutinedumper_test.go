...
func TestNewGoroutineDumper(t *testing.T) {
	...
	func() {
		...
		assert.Error(t, err)
		assert.Equal(t, syscall.ENOTDIR, err.(*os.PathError).Err)
...
func TestTakeGoroutineDump(t *testing.T) {
	...
	t.Run("take goroutine dump fails", func(tt *testing.T) {
	...
	t.Run("take goroutine dump succeeds", func(tt *testing.T) {
		tempDir, dirCleanupFn := testutils.TempDir(t)
		defer dirCleanupFn()
