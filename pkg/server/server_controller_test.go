...
func TestServerController(t *testing.T) {
	...
	d, err := ts.serverController.get(ctx, "system")
	require.NoError(t, err)
	if d.(*systemServerWrapper).s != ts.Server {
		t.Fatal("expected wrapped system server")
	}

	d, err = ts.serverController.get(ctx, "somename")
	require.Nil(t, d)
	...
	_, err = ts.serverController.get(ctx, "hello")
