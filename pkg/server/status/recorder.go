...
func (mr *MetricsRecorder) scrapePrometheusLocked() {
	mr.scrapeIntoPrometheusLocked(mr.promMu.prometheusExporter)
}

// scrapeIntoPrometheusLocked updates the passed-in prometheusExporter's
// metrics snapshot.
func (mr *MetricsRecorder) scrapeIntoPrometheusLocked(pm metric.PrometheusExporter) {
	mr.mu.RLock()
	defer mr.mu.RUnlock()
	if mr.mu.nodeRegistry == nil {
...
func (mr *MetricsRecorder) ExportToGraphite(ctx context.Context, endpoint string) error {
	pm := metric.MakePrometheusExporter()
	mr.scrapeIntoPrometheusLocked(pm)
	graphiteExporter := metric.MakeGraphiteExporter(&pm, mr.settings)
