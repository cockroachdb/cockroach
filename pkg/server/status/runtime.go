...
func (rsr *RuntimeStatSampler) SampleEnvironment(ctx context.Context) {
	...
	diskStats, err := getSummedDiskCounters(ctx)
	if err != nil {
		log.Warningf(ctx, "problem fetching disk stats: %s; disk stats will be empty.", err)
	} else {
		deltaDisk = diskStats
		subtractDiskCounters(&deltaDisk, rsr.last.disk)
		rsr.last.disk = diskStats
		subtractDiskCounters(&diskStats, rsr.initialDiskCounters)

		rsr.HostDiskReadBytes.Update(diskStats.readBytes)
		rsr.HostDiskReadTime.Update(int64(diskStats.readTime))
		rsr.HostDiskReadCount.Update(diskStats.readCount)
		rsr.HostDiskWriteBytes.Update(diskStats.writeBytes)
		rsr.HostDiskWriteTime.Update(int64(diskStats.writeTime))
		rsr.HostDiskWriteCount.Update(diskStats.writeCount)
		rsr.IopsInProgress.Update(diskStats.iopsInProgress)
	}

	netStats, err := getSummedNetStats(ctx)
	if err != nil {
		log.Warningf(ctx, "problem fetching net stats: %s; net stats will be empty.", err)
	} else {
		deltaNet = netStats
		subtractNetworkCounters(&deltaNet, rsr.last.net)
		rsr.last.net = netStats
		subtractNetworkCounters(&netStats, rsr.initialNetCounters)

		rsr.HostNetSendBytes.Update(int64(netStats.BytesSent))
		rsr.HostNetSendPackets.Update(int64(netStats.PacketsSent))
		rsr.HostNetRecvBytes.Update(int64(netStats.BytesRecv))
		rsr.HostNetRecvPackets.Update(int64(netStats.PacketsRecv))
	}
