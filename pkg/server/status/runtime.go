...
// separately, on another schedule.
func (rsr *RuntimeStatSampler) SampleEnvironment(ctx context.Context, ms base.GoMemStats) {
	...
	log.Infof(ctx, "runtime stats: %s RSS, %d goroutines, %s/%s/%s GO alloc/idle/total%s, "+
		...
		humanize.IBytes(ms.HeapAlloc), humanize.IBytes(ms.HeapIdle),
		humanize.IBytes(ms.Sys-ms.HeapReleased),
		staleMsg,
	...
	rsr.GoAllocBytes.Update(int64(ms.Alloc))
	rsr.GoTotalBytes.Update(int64(ms.GoInUse()))
