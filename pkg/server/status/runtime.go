...
func (rsr *RuntimeStatSampler) SampleEnvironment(
	...
	log.Infof(ctx, "%s", redact.Safe(fmt.Sprintf(
		"runtime stats: %s RSS, %d goroutines, "+
			"%s/%s/%s GO alloc/idle/total%s "+
			"(HeapInuse: %s, HeapReleased: %s StackSys: %s), "+
			"%s/%s CGO alloc/total, %.1f CGO/sec, "+
			"%.1f/%.1f %%(u/s)time, %.1f %%gc (%dx), "+
			"%s/%s (r/w)net",
		humanize.IBytes(mem.Resident), numGoroutine,
		humanize.IBytes(ms.HeapAlloc), humanize.IBytes(ms.HeapIdle), humanize.IBytes(goTotal), staleMsg,
		humanize.IBytes(ms.HeapInuse), humanize.IBytes(ms.HeapReleased), humanize.IBytes(ms.StackSys),
