...
network issues. Tail loss probes are agressive, so the base rate is often nonzero
...
func getSummedNetStats(ctx context.Context) (netCounters, error) {
	...
	m, err := mockableMaybeReadProcStatFile(ctx, "TcpExt", "/proc/net/netstat")
	if err == nil {
		if n, ok := m["TCPFastRetrans"]; ok {
			tcpFastRetrans = n
		}
		if n, ok := m["TCPTimeouts"]; ok {
			tcpTimeouts = n
		}
		if n, ok := m["TCPSlowStartRetrans"]; ok {
			tcpSlowStartRetrans = n
		}
		if n, ok := m["TCPLossProbes"]; ok {
			tcpLossProbes = n
		}
	}

	if log.V(3) {
		var tcp map[string]int64
		if len(pc) == 1 {
			tcp = pc[0].Stats
		}
		log.Infof(ctx, "tcp stats: Tcp: %+v TcpExt: %+v", tcp, m)
	}
...
var errProcNotSupported = errors.Errorf("/proc not supported on %s", runtime.GOOS)
...
func parseProcStatFile(
	...
	for i, h := range headers {
		...
		if err != nil {
			return nil, errors.Wrapf(err, "could not parse value for %s", headers[i])
		}
		stats[h] = v
	}
	return stats, nil
