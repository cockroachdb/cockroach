...
var trackedMetrics = map[string]threshold{
	...
	"sys.goroutines":              threshold{gauge: true, min: 5000},

	// Latencies (which are really histograms, but we get to see a fixed number
	// of percentiles as gauges)
	"raft.process.logcommit.latency-90": threshold{gauge: true, min: int64(500 * time.Millisecond)},
	"round-trip-latency-p90":            threshold{gauge: true, min: int64(time.Second)},
