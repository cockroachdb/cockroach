...
func getDiskCounters(_ context.Context) ([]diskStats, error) {
	...
	for i, counters := range driveStats {
		output[i] = diskStats{
			...
			readTime:       int64(counters.TotalReadTime),
			writeBytes:     counters.BytesWritten,
			writeCount:     counters.NumWrite,
			writeTime:      int64(counters.TotalWriteTime),
			iopsInProgress: 0, // Not reported by this library.
		}
	}
