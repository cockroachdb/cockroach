...
func TestGetVCPUs(t *testing.T) {
	...
	// GetVCPUs should never return more than runtime.NumCPU() in a non-cgroup
	...
	require.LessOrEqual(t, vcpus, numCPU*2, "GetVCPUs should not return more than 2x NumCPU (sanity check)")
	...
	if currentMaxProcs > 1 {
		...
		// (it should still return runtime.NumCPU())
