...
`, users[0], users[1], users[2]))
	...
	openSessions := func(user string) {
		// Open a session for the target user.
		targetDB := getUserConn(t, user, testCluster.Server(0))
		defer targetDB.Close()
		sqlutils.MakeSQLRunner(targetDB).Exec(t, `SELECT pg_sleep(5)`)
	}

	// Open 5 sessions for each user and leave them open by running pg_sleep(5).
	for _, user := range users {
		for i := 0; i < 5; i++ {
			go openSessions(user)
		}
	}
	...
	for _, user := range users {
		...
		open, idle, closed := countSessionStatus(allSessions)
	...
	// sleep for 6 seconds to give the pg_sleep() command enough time to finish
