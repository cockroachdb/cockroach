...
func TestStatusAPIListSessions(t *testing.T) {
	...
	firstServerProto := testCluster.Server(0)
	firstServerSQL := sqlutils.MakeSQLRunner(testCluster.ServerConn(0))

	appName := "test_sessions_api"
	firstServerSQL.Exec(t, fmt.Sprintf(`SET application_name = "%s"`, appName))
	...
	// Grant VIEWACTIVITYREDACTED.
	firstServerSQL.Exec(t, fmt.Sprintf("ALTER USER %s VIEWACTIVITYREDACTED", userNoAdmin.Normalized()))
	firstServerSQL.Exec(t, "SELECT 1")
	err = getStatusJSONProtoWithAdminOption(firstServerProto, "sessions", &resp, false)
	require.NoError(t, err)
	...
	firstServerSQL.Exec(t, fmt.Sprintf("ALTER USER %s VIEWACTIVITY", userNoAdmin.Normalized()))
	firstServerSQL.Exec(t, "SELECT 1, 1")
	err = getStatusJSONProtoWithAdminOption(firstServerProto, "sessions", &resp, false)
	require.NoError(t, err)
	session = getSessionWithTestAppName(&resp)
	...
	firstServerSQL.Exec(t, fmt.Sprintf("ALTER USER %s NOVIEWACTIVITYREDACTED", userNoAdmin.Normalized()))
	firstServerSQL.Exec(t, "SELECT 2")
	err = getStatusJSONProtoWithAdminOption(firstServerProto, "sessions", &resp, false)
	require.NoError(t, err)
	session = getSessionWithTestAppName(&resp)
	require.Equal(t, "SELECT _", session.LastActiveQueryNoConstants)
