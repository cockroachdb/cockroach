...
func TestLicenseExpiryMetricNoLicense(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"No License", "", nil},
		{"Valid 1 second License", "seconds_until_license_expiry 1\n", func(
			_ context.Context, _ *cluster.Settings, _ time.Time,
		) (time.Duration, error) {
			return time.Second, nil
		}},
		{"Valid Long License", "seconds_until_license_expiry 1603926294\n", func(
			_ context.Context, _ *cluster.Settings, _ time.Time,
		) (time.Duration, error) {
			return timeutil.Unix(1603926294, 0).Sub(timeutil.Unix(0, 0)), nil
		}},
		{"Valid Long Past License", "seconds_until_license_expiry -1603926294\n", func(
			_ context.Context, _ *cluster.Settings, _ time.Time,
		) (time.Duration, error) {
		...
		t.Run(tc.name, func(t *testing.T) {
			varsHandler := varsHandler{ts.status.metricSource, ts.status.st}
			if tc.expiryFunc != nil {
				base.TimeToLicenseExpiry = tc.expiryFunc
			}

			buf := new(bytes.Buffer)
			varsHandler.appendLicenseExpiryMetric(context.Background(), buf)
