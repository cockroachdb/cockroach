...
func TestListSessionsV2(t *testing.T) {
	...
	rootClient, err := ts1.GetRootAuthenticatedHTTPClient()
	require.NoError(t, err)
	sessionsResponse := doSessionsRequest(rootClient, 0, "")
	...
	for limit := 1; limit <= 15; limit++ {
		...
		for {
			sessionsResponse := doSessionsRequest(rootClient, limit, next)
	...
	// An non-superuser admin user cannot see sessions across all users.
	adminClient, err := ts1.GetAdminAuthenticatedHTTPClient()
	require.NoError(t, err)
	sessionsResponse2 := doSessionsRequest(adminClient, 0, "")
	require.Equal(t, 0, len(sessionsResponse2.Sessions))

	// An non-admin user cannot see sessions at all.
	...
	require.Contains(t, string(bytesResponse),"requires admin")
}
