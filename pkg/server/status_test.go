...
func TestMergeDistSQLFlows(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)
	...
	for _, tc := range []struct {
		a        []serverpb.DistSQLRemoteFlow
		b        []serverpb.DistSQLRemoteFlow
		expected []serverpb.DistSQLRemoteFlow
	}{
		// a is empty
		{
			a: []serverpb.DistSQLRemoteFlow{},
		...
		require.Equal(t, tc.expected, mergeDistSQLFlows(tc.a, tc.b))
	}
}
