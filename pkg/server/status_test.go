...
func TestStatusAPITransactions(t *testing.T) {
	...
	var uniqueTxns []string
	for _, tc := range testCases {
		txn := tc.query
		if tc.fingerprinted != "" {
			txn = tc.fingerprinted
		}
		if _, found := expectedTxnCounts[txn]; !found {
			uniqueTxns = append(uniqueTxns, txn)
		}
		expectedTxnCounts[txn] += 1
	}
	...
	for _, respTransaction := range resp.Transactions {
		if strings.HasPrefix(respTransaction.App, catconstants.InternalAppNamePrefix) {
			// Ignore internal queries, they aren't relevant to this test.
			continue
		}
		respCounts = append(respCounts, int(respTransaction.Count))
	}
