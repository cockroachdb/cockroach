...
func TestStatusServer_StatementExecutionInsights(t *testing.T) {
	...
	splitAt := 5
	_, err := sqlConn.Exec(fmt.Sprintf("CREATE TABLE t (a INT); INSERT INTO t VALUES(5); ALTER TABLE t SPLIT AT VALUES(%d)", splitAt))
	...
	require.NoError(t, err)
	row := sqlConn.QueryRow(fmt.Sprintf("SELECT raw_start_key FROM [SHOW CLUSTER RANGES WITH KEYS] WHERE start_key LIKE '%%/Table/%%/%d' LIMIT 1", splitAt))
	var keyHex []byte
	require.NoError(t, row.Scan(&keyHex))
	...
	origTxnInsight := &insights.Transaction{
		...
		Problems:      []insights.Problem{insights.Problem_SlowExecution},
	}
	startTime := time.Date(2000, 1, 1, 9, 30, 0, 0, time.UTC)
	endTime := time.Date(2000, 1, 1, 10, 0, 0, 0, time.UTC)
	stmtID, err := clusterunique.IDFromString("178da6adb3af6f480000000000000001")
	require.NoError(t, err)
		...
		for _, role := range roles {
			t.Run(role, func(t *testing.T) {
				...
				if role == "VIEWACTIVITYREDACTED" {
					...
					require.Equal(t, "", resp.Statements[0].ContentionEvents[0].PrettyKey)
				}
			})
		}
	})
