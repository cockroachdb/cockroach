...
func TestThrottlingMetadata(t *testing.T) {
	...
	gracePeriodEndTs, _ := enforcer.GetGracePeriodEndTS()
	gracePeriodEndSeconds := gracePeriodEndTs.Unix()
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			expected: serverpb.GetThrottlingMetadataResponse{
				HasGracePeriod:               true,
				GracePeriodEndSeconds:        gracePeriodEndSeconds,
		...
		},
		{
			...
			expected: serverpb.GetThrottlingMetadataResponse{
				Throttled:                    true,
				ThrottleExplanation:          "License expired. The maximum number of open transactions has been reached.",
				HasGracePeriod:               true,
		...
		},
		{
			...
			expected: serverpb.GetThrottlingMetadataResponse{
				Throttled:                    true,
				ThrottleExplanation:          "The maximum number of open transactions has been reached because the license requires diagnostic reporting, but none has been received by Cockroach Labs.",
				HasGracePeriod:               true,
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := srvtestutils.GetStatusJSONProto(app, "throttling", &resp); err != nil {
				t.Fatal(err)
			}
