...
func TestListSessionsPrivileges(t *testing.T) {
	...
	go func() {
		_, err := serverSQL.DB.ExecContext(context.Background(), sleepQuery)
	...
	// Selects the query ID of the sleep query and then cancelling it.
	cancelQuery := func() {
		rows, err := sqlDB.Query(fmt.Sprintf(`SELECT query_id FROM [SHOW QUERIES] WHERE query = '%s'`, sleepQuery))
		if err != nil {
			t.Errorf("unexpected failure selecting query_id: %s", err)
		}
		defer rows.Close()
		var queryID string
		for rows.Next() {
			if err := rows.Scan(&queryID); err != nil {
				t.Errorf("unexpected failure scanning rows: %s", err)
			}

			// Cancel the query using the retrieved query ID
			_, err := sqlDB.Exec(fmt.Sprintf(`CANCEL QUERY '%s'`, queryID))
			if err != nil {
				t.Errorf("unexpected failure cancelling query: %s", err)
			}
		}
		if err := rows.Err(); err != nil {
			t.Errorf("unexpected error iterating through rows: %s", err)
		}
	}
