...
// TestLisSessionsPrivileges tests that the VIEWACTIVITY and VIEWACTIVITYREDACTED privileges
// are respected when listing sessions, particularly for other users' sessions.
func TestLisSessionsPrivileges(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)
	...
	serverSQL.Exec(t, fmt.Sprintf(`CREATE USER %s`, apiconstants.TestingUserNameNoAdmin().Normalized()))

	// Run a sleep query as root in another goroutine to make sure that root's session has one
	// active query while we list sessions.
	var wg sync.WaitGroup
	wg.Add(1)
	go func() {
		serverSQL.Exec(t, sleepQuery)
		defer wg.Done()
	}()
