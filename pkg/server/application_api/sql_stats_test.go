...
func TestStatusAPICombinedStatementsWithFullScans(t *testing.T) {
	...
	checkCombinedStmtStats := func(statements []TestCases) {
		...
		// The additional timeout that will be added. Default is 0s. If the test is
		// running in stress mode, the additional timeout will be 30s.
		timeout := 0 * time.Second
		if skip.Stress() {
			timeout = 30 * time.Second
		}

		err := srvtestutils.GetStatusJSONProtoWithAdminOptionAndTimeout(firstServerProto, endpoint, &resp, false, timeout)
		if err != nil {
			t.Fatal(err)
		}
