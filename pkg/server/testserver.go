...
func (ts *TestServer) SplitRange(
	...
	if err := ts.DB().Txn(ctx, func(ctx context.Context, txn *client.Txn) error {
		...
		// Errors returned from scanMeta cannot be wrapped or retryable errors
		// won't be retried. Instead, the message to wrap is stored in case of
		// non-retryable failures and then wrapped when the full transaction
		// fails.
