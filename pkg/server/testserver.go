...
func (ts *TestServer) maybeStartTestSQLServer(ctx context.Context) error {
	...
	if err := base.CheckEnterpriseEnabled(ts.st, ts.ClusterID(), org, "SQL servers"); err != nil {
...
func (t *TestTenant) RPCAddr() string {
	// The RPC and SQL functionality for SQLServers is multiplexed
...
func (ts *TestServer) StartTenant(
	...
	if !params.DisableCreateTenant {
		rowCount, err := ts.InternalExecutor().(*sql.InternalExecutor).Exec(
			...
			"SELECT 1 FROM system.SQLServers WHERE id=$1 AND active=true",
