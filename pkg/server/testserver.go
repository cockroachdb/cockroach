...
func (ts *TestServer) StartTenant(
	...
	go func() {
		// If the sqlServer requests a shutdown, do that simply by stopping the
		// tenant's stopper.
		select {
		case <-sqlServer.ShutdownRequested():
			stopper.Stop(sqlServer.AnnotateCtx(context.Background()))
