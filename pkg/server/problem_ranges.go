	...
	for remainingResponses := len(isLiveMap); remainingResponses > 0; remainingResponses-- {
		...
		case resp := <-responses:
			...
			for _, info := range resp.resp.Ranges {
				...
				if info.Problems.LogTooLarge {
					problems.LogSizeTooLargeRangeIDs =
						append(problems.LogSizeTooLargeRangeIDs, info.State.Desc.RangeID)
				}
			}
			...
			sort.Sort(roachpb.RangeIDSlice(problems.LogSizeTooLargeRangeIDs))
