...
func getStatementDetails(
	...
	// This isn't a great way to propagate the
	...
	rb := statementDetailsRespBuilder{
		...
		qargs:            args,
		limit:            limit,
		tryActivityTable: activityHasData,
	}
...
LIMIT 1`
	...
	if rb.tryActivityTable {
		mergeAggMetadataCol := `merge_aggregated_stmt_metadata(metadata)`
		if !rb.settings.Version.IsActive(ctx, clusterversion.V24_1) {
			mergeAggMetadataCol = `crdb_internal.merge_aggregated_stmt_metadata(array_agg(metadata))`
		}
...
GROUP BY
    fingerprint_id
LIMIT 1`, mergeAggMetadataCol, rb.whereClause), rb.qargs...)
		if err != nil {
...
GROUP BY
    aggregated_ts
ORDER BY aggregated_ts ASC
LIMIT $%d`, mergeAggMetadataCol, rb.whereClause, len(args)),
			args...)
...
GROUP BY
    ...
    plan_gist,
    index_recommendations
LIMIT $%d`, mergeAggMetadataCol, rb.whereClause, len(args)), args...)
		if iterErr != nil {
			return nil, srverrors.ServerError(ctx, err)
		}
	}
