...
func simplePaginate(input interface{}, limit, offset int) (result interface{}, next int) {
	...
	if limit <= 0 || offset <= 0 || val.Kind() != reflect.Slice {
...
func (p *paginationState) mergeNodeIDs(sortedNodeIDs []roachpb.NodeID) {
	...
	for i := range sortedNodeIDs {
		j := sort.Search(len(allNodeIDs), func(k int) bool {
			return allNodeIDs[k] >= sortedNodeIDs[i]
		})
...
// start/end indices that the response should be sliced at (if it is a slice;
// otherwise inclusion/exclusion is denoted by end > start). Note that this
// method expects that it is called serially, with nodeIDs in the same order
// as p.nodesToQuery (nodes skipped in that slice are considered to have
// returned an error; out-of-order nodeIDs generate a panic).
func (p *paginationState) paginate(limit int, nodeID roachpb.NodeID, length int) (start, end, newLimit int) {
	...
	if p.inProgress != nodeID {
		...
		if p.inProgress == 0 {
			...
			panic(fmt.Sprintf("could not find node %d in pagination state %v", nodeID, p))
		}
	}
	doneWithNode := false
	if length > 0 {
	...
	for numNodes > 0 {
		...
		case res, ok := <-r.responseChan:
			if res.err != nil {
				...
				start, end, newLimit := next.paginate(limit, res.nodeID, res.len)
