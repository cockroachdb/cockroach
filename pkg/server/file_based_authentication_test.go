...
func TestMaxEmergencyTokenValidityDuration(t *testing.T) {
	defer leaktest.AfterTest(t)()

	_, _, _, err := GenerateEmergencyCredentials(security.RootUserName(),
		true /* isAdmin */, 365*24*time.Hour)

	if !testutils.IsError(err, "expiry too far in the future") {
		t.Fatalf("expected validity duration error, got %v", err)
	}
}
