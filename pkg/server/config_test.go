...
func TestParseInitNodeAttributes(t *testing.T) {
	...
	cfg.Stores = storagepb.StoreSpecList{Specs: []storagepb.StoreSpec{{InMemory: true, Properties: storagepb.SizeSpec{Capacity: storagepb.MinimumStoreSize * 100}}}}
...
func TestCreateEnginesWithMultipleStores(t *testing.T) {
	...
	defer cleanup2()
	cfg.Stores = storagepb.StoreSpecList{Specs: []storagepb.StoreSpec{
		{Properties: storagepb.SizeSpec{Capacity: storagepb.MinimumStoreSize}, Path: tmpDir1},
		{Properties: storagepb.SizeSpec{Capacity: storagepb.MinimumStoreSize}, Path: tmpDir2},
		{InMemory: true, Properties: storagepb.SizeSpec{Capacity: storagepb.MinimumStoreSize * 100}},
	}}
	engines, err := cfg.CreateEngines(context.Background())
	if err != nil {
		t.Fatalf("Failed to initialize stores: %s", err)
