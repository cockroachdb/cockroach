...
func TestTempStoreDerivation(t *testing.T) {
	...
	storeDir, err := ioutil.TempDir("", "store-dir-")
	if err != nil {
		t.Fatal(err)
	}
	...
	testCases := []struct {
		...
		},
		{
			...
			expectedTempSpec: base.StoreSpec{Path: fmt.Sprintf("%s", filepath.Join(storeDir, "temp-dir-*"))},
		},
		{
			name:             "StorePathWithTempPath",
			firstStoreArg:    fmt.Sprintf("path=%s", storeDir),
			tempStoreDir:     tempDir,
			expectedTempSpec: base.StoreSpec{Path: fmt.Sprintf("%s", filepath.Join(tempDir, "temp-dir-*"))},
		},
		{
