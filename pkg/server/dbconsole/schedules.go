...
FROM s`,
	...
	if statusFilter != "" {
		query.Append(` WHERE schedule_status = $`, strings.ToUpper(statusFilter))
	}
...
func scanScheduleRow(row tree.Datums) (ScheduleInfo, error) {
	...
	if row[3] != tree.DNull {
		info.NextRun = row[3].(*tree.DTimestampTZ).Time.Format(time.RFC3339)
	}
	...
	info.Created = row[8].(*tree.DTimestampTZ).Time.Format(time.RFC3339)

	// command: nullable JSONB field (the delegate strips @type from execution args).
	if row[9] != tree.DNull {
		info.Command = row[9].(*tree.DJSON).JSON.String()
	}
