...
func (n *Node) startWriteNodeStatus(ctx, workerCtx context.Context, frequency time.Duration) error {
	...
	if err := startup.RunWithStartupRetry(logtags.AddTag(ctx, "summaries", nil),
	...
	return n.stopper.RunAsyncTask(logtags.AddTag(workerCtx, "summaries", nil), "write-node-status",
