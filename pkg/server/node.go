...
func (n *Node) writeNodeStatus(ctx context.Context, alertTTL time.Duration) error {
	...
	if runErr := n.stopper.RunTask(ctx, "node.Node: writing summary", func(ctx context.Context) {
		...
		if result := n.recorder.CheckHealth(ctx, *nodeStatus); len(result.Alerts) != 0 {
			...
			// TODO(tschottdorf): decide if we want to report these to sentry. At the very least,
			// we need to ignore some alerts. For example, underreplicated ranges
			// are always present when a cluster is first bootstrapped. Besides,
			// these alerts won't be actionable for the most part.
			//
			// In principle we could reverse-engineer most cluster outages today
			// where reporting is enabled, since we get the metrics.
		}
