...
var (
	...
	perConsumerCatchupLimit = settings.RegisterBoolSetting(
		settings.SystemOnly,
...
func makePerConsumerScanLimiter(
	...
	getLimit := func() int {
		// Set the limit to 75% of the total limit.
		return min(int(float64(kvserver.ConcurrentRangefeedItersLimit.Get(sv))*0.75), 1)
	}
	l := limit.MakeConcurrentRequestLimiter(
		fmt.Sprintf("PerConsumerCatchupLimit-%d", consumerID),
		getLimit())
	kvserver.ConcurrentRangefeedItersLimit.SetOnChange(sv, func(ctx context.Context) {
		l.SetLimit(getLimit())
	})
	return l
}
...
func (n *Node) MuxRangeFeed(muxStream kvpb.Internal_MuxRangeFeedServer) error {
	...
	for {
		...
		default:
			...
			// same consumer.
			if perConsumerCatchupLimit.Get(n.execCfg.SV()) {
				if consumerID == 0 {
					if req.ConsumerID == 0 {
			...
			if err := n.stores.RangeFeed(streamCtx, req, streamSink, limiter); err != nil {
