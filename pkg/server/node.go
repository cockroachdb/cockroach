...
func (n *Node) computeMetricsPeriodically(
	...
	return n.updateFaultToleranceMetrics(ctx)
}
...
func (n *Node) updateFaultToleranceMetrics(ctx context.Context) error {
	...
	if nodesMissingLocality > 0 {
		log.Dev.Warningf(ctx, "skipping fault tolerance metric update: some nodes missing locality")
		return nil
	}

	domainMargins, err := faulttolerance.ComputeFaultTolerance(
		ctx,
	...
	n.metrics.FaultTolerance.Clear()
