...
func (n *Node) initNodeID(id roachpb.NodeID) {
	...
	var err error
	if id == 0 {
		allocCtx, span := n.AnnotateCtxWithSpan(context.TODO(), "alloc-node-id")
		defer span.Finish()
		id, err = allocateNodeID(allocCtx, n.storeCfg.DB)
		if err != nil {
			log.Fatal(allocCtx, err)
		}
		log.Infof(allocCtx, "new node allocated ID %d", id)
		if id == 0 {
			log.Fatal(allocCtx, "new node allocated illegal ID 0")
		}
