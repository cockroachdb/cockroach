...
func (n *Node) onStoreDiskSlow(ctx context.Context, storeID roachpb.StoreID, info pebble.DiskSlowInfo) {
	...
	// NB: A zero value for last means that we are not currently running the below
	// async task, and this is a new disk slowness event that needs a start/clear
	// pair emitted for it.
		...
		for {
			...
			case <-ticker.C:
				...
				if timeutil.Since(last) < diskSlowClearInterval {
					clearing = true
					n.diskSlowCoalescerMu.lastDiskSlow[storeID] = time.Time{}
				}
				n.diskSlowCoalescerMu.Unlock()
