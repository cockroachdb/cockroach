...
const (
	...
	FirstNodeID          = 1
	graphiteFrequencyKey = "external.graphite.frequency"
)
...
var (
	...
	graphiteFrequency = settings.RegisterValidatedDurationSetting(
		graphiteFrequencyKey,
		"the frequency with which metrics are pushed to Graphite (if enabled)",
		10*time.Second,
		func(v time.Duration) error {
			if v < 0 {
				return errors.Errorf("cannot set %s to a negative duration: %s", graphiteFrequencyKey, v)
			} else if v > 15*time.Minute {
				return errors.Errorf("cannot set %s greater than 15 minutes: %s", graphiteFrequencyKey, v)
			}
			return nil
		},
	)
...
func (n *Node) startGraphiteStatsExporter(st *cluster.Settings) {
	...
	n.stopper.RunWorker(ctx, func(ctx context.Context) {
		...
		defer timer.Stop()
		for {
			timer.Reset(graphiteFrequency.Get(&st.SV))
			select {
			case <-n.stopper.ShouldStop():
				return
			case <-timer.C:
