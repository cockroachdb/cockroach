...
func (n *Node) maybeProxyRequest(
	...
	// If the request didn't start, we know it didn't have any impact on the
	// remote server so ignore our proxy attempt. Otherwise, treat the error as
	// though it could be ambiguous and allow the client to process it. This is
	// the common case when we have a full partition from the target node.
	if grpcutil.RequestDidNotStart(pErr.GoError()) {
		log.VEvent(ctx, 2, "proxy failed before the request was sent")
		return nil
	}
	log.VEventf(ctx, 2, "proxy attempt resulted in error %v", pErr)
	br = &kvpb.BatchResponse{}
	br.Error = kvpb.NewError(kvpb.NewAmbiguousResultError(pErr.GoError()))
	return br
}
