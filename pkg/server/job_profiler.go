...
func (s *statusServer) RequestJobProfilerExecutionDetails(
	...
	if local {
		...
		if err != nil {
			// Request job execution details is best effort, so swallow the error
			// instead of returning it.
			log.Warningf(ctx, "failed to get resumer for job %d: %v", jobID, err)
			return nil, nil
		}
		...
		if err := r.CollectProfile(ctx, jobExecCtx); err != nil {
			// Request job execution details is best effort, so swallow the error
			// instead of returning it.
			log.Warningf(ctx, "failed to get collect profile for job %d: %v", jobID, err)
		}
