...
// ReuseEngines configures a sticky VFS registry to return exactly the same
// Engine without closing and re-opening from the underlying in-memory FS.
var ReuseEngines StickyVFSOption = func(cfg *stickyConfig) {
...
type StickyVFSRegistry interface {
	// Open returns an engine associated with the given id. It will create a new
	// in-memory filesystem if one does not already exist. Otherwise, the new
	// Engine will re-open the existing store held within the in-memory
	// filesystem.
	//
	// If the registry was created with the ReuseEngines option, Open will
	// return the previous Engine on a subsequent Open. With this option, at
	// most one engine with a given id can be active at any given time. Note
	// that if you re-create an existing sticky engine the new attributes and
	// cache size will be ignored. One must Close() on the sticky engine before
	// another can be fetched. When using the ReuseEngines option, the caller
...
// Open implements the StickyInMemEnginesRegistry interface.
...
// CloseAllEngines closes and removes all sticky in-memory engines.
