...
// getDatabaseGrants returns a paginated response of database metadata and statistics. This is not a live view of
// the database data but instead is cached data that had been precomputed at an earlier time.
//
// If the user does not have the required permissions to access the database we will return a 403 Forbidden error.
//
// ---
// parameters:
//
...
//
// produces:
// - application/json: databaseGrantsResponse
//
// responses:
//
//	"200":
...
func (a *apiV2Server) getDatabaseGrants(w http.ResponseWriter, r *http.Request) {
	...
	resp, err := getDatabaseGrantsResponseWithPagination(ctx, a.sqlServer.internalExecutor, dbId, sqlUser, pageSize, offset)
	if err != nil {
...
FROM [SHOW GRANTS ON DATABASE %s]`, escDbName))
...
// getTableGrants returns a paginated response of table grants metadata.
// If the user does not have the required permissions to access the table, we will return a 403 Forbidden error.
//
// ---
// parameters:
//   - name: tableId
...
//
// produces:
// - application/json: tableGrantsResponse
//
// responses:
//
//	"200":
...
func (a *apiV2Server) getTableGrants(w http.ResponseWriter, r *http.Request) {
	...
	resp, err := getTableGrantsResponseWithPagination(ctx, a.sqlServer.internalExecutor, tableId, sqlUser, pageSize, offset)
	if err != nil {
		srverrors.APIV2InternalError(ctx, err, w)
		return
	}
...
`, tableId)
	...
	// This is used to properly escape the table name.
	tableNameParts := tree.UnresolvedName{NumParts: 3, Parts: tree.NameParts{resp.TableName, resp.SchemaName, resp.DatabaseName}}

	query := safesql.NewQuery()
	query.Append(fmt.Sprintf(`
SELECT grantee, privilege_type, count(*) OVER() as total_row_count
FROM [SHOW GRANTS ON TABLE %s]`, tableNameParts.String()))
