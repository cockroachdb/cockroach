...
func (s *initServer) ServeAndWait(
	...
	case state := <-s.bootstrapReqCh:
		// Bootstrap() did its job. At this point, we know that the cluster version
		// will be bootstrapVersion (=state.clusterVersion.Version), but the version
		// setting does not know yet (it was initialized as the BinaryMinSupported
		// Version because the engines were all uninitialized). We *could* just
		// let the server start, and it would populate system.settings, which is
		// then gossiped, and then the callback would update the version, but we
		// take this shortcut to avoid having every freshly bootstrapped cluster
		// spend time at an old cluster version.
