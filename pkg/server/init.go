...
func newInitServer(
	...
	if inspectedDiskState.initialized() {
...
// constructing. It's encapsulates of all the details needed to fully start up a
// CockroachDB server, and reflects this node's view into a bootstrapped cluster
// (i.e. it has a cluster ID, and a node ID for itself). See `ServeAndWait` for
// more details.
...
func (i *initState) initialized() bool {
...
func (s *initServer) ServeAndWait(
	...
	if s.inspectedDiskState.initialized() {
