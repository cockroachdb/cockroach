...
func TestSettingsTraceSamplingRate(t *testing.T) {
	...
	execStatement(fmt.Sprintf(`SET CLUSTER SETTING trace.zipkin.collector = '%s'`, traceCollectorServerURL.Host))
	execStatement(`SET CLUSTER SETTING trace.zipkin.collector.batch.size = 200`)
	...
	testutils.SucceedsSoon(t, func() error {
		...
		if currentTraces < 450 && currentTraces > 550 {
			return fmt.Errorf(`could not trace half of SQL 'INSERT INTO' statements. traced %d out of 1000`, currentTraces)
		}
		return nil
	})
}
