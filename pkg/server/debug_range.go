...
func removeDuplicateLeases(entries leaseSlice) leaseSlice {
...
func (d *debugRangeData) postProcessing() {
	...
	for _, lease := range leaseHistory {
		...
		if lease.Epoch != nil {
			...
			if lease.Expiration.WallTime == 0 {
				...
				detail.ExpirationOrEpoch = debugRangeOutput{
					...
					Value: floorMilliseconds(lease.Expiration.GoTime().Sub(lease.Start.GoTime())).String(),
				}
			}
		}
		...
		if lease.ProposedTS != nil && lease.ProposedTS.WallTime != 0 {
			detail.ProposedTS = debugRangeOutput{
				Title: fmt.Sprintf("%s\n%s", convertTimestamp(*lease.ProposedTS), *lease.ProposedTS),
				Value: floorMilliseconds(lease.ProposedTS.GoTime().Sub(lease.Start.GoTime())).String(),
			}
		} else {
			detail.ProposedTS = debugRangeOutput{
				Title: debugRangeValueEmpty,
