...
type debugLeaseDetails struct {
	...
	ExpirationDelta debugRangeOutput
...
type debugRangeData struct {
	...
	LeaseEpoch        bool // true of epoch based, false of expiration based
}
...
func (d *debugRangeData) postProcessing() {
	...
	// leaderlessLatestTermInfo is used when there is no leader but we want to
	// display some details about one of the replicas.
	var leaderlessLatestTermInfo serverpb.RangeInfo
	...
	for _, info := range d.rangeInfos {
		...
		if info.RaftState.HardState.Term > leaderlessLatestTermInfo.RaftState.HardState.Term {
			leaderlessLatestTermInfo = info
		}
	...
	if leaderStoreInfo.SourceStoreID > 0 {
		leaderlessLatestTermInfo = leaderStoreInfo
	}
	...
	for i := len(leaderlessLatestTermInfo.LeaseHistory) - 1; i >= 0; i-- {
		lease := leaderlessLatestTermInfo.LeaseHistory[i]
		if lease.ProposedTS == nil {
			failedInfo := leaderlessLatestTermInfo
			failedInfo.ErrorMessage = fmt.Sprintf("Lease has a nil proposedTS: %+v", lease)
			d.Failures = append(d.Failures, failedInfo)
			continue
		}
		if i == len(leaderlessLatestTermInfo.LeaseHistory)-1 {
			if lease.Epoch != nil {
				d.LeaseEpoch = true
			}
		}
		...
		if lease.ProposedTS.WallTime != 0 {
			...
			detail.ProposedTS.Value = floorMilliseconds(lease.ProposedTS.GoTime().Sub(lease.Start.GoTime())).String()
		} else {
		...
		if i > 0 {
			prevLease := leaderlessLatestTermInfo.LeaseHistory[i-1]
			...
			if !d.LeaseEpoch {
				if prevLease.Expiration.WallTime != 0 {
					detail.ExpirationDelta.Title = floorMilliseconds(lease.Expiration.GoTime().Sub(prevLease.Expiration.GoTime())).String()
				} else {
					detail.ExpirationDelta.Title = debugRangeValueEmpty
				}
				detail.ExpirationDelta.Value = detail.ExpirationDelta.Title
			}
		}
        ...
        <DIV CLASS="lease-table">
          <DIV CLASS="lease-row">
            ...
            {{- else -}}
              ...
              <DIV CLASS="lease-cell">Expiration Delta</DIV>
            {{- end -}}
          ...
          {{- range $_, $lease := $.LeaseHistory}}
            <DIV CLASS="lease-row">
              ...
              {{- else -}}
                ...
                <DIV CLASS="lease-cell" TITLE="{{$lease.ExpirationDelta.Title}}">{{$lease.ExpirationDelta.Value}}</DIV>
              {{- end}}
