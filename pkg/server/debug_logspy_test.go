	...
	testCases := []struct {
	...
	}{
		{
			...
			expOpts: logSpyOptions{
				...
				Message:  regexpType{regexp.MustCompile(`^foo$`)},
				FileLine: regexpType{regexp.MustCompile(`raft\.go$`)},
			},
		},
		{
		...
		},
		{
			...
			expOpts: logSpyOptions{
				Count:    1,
				Duration: durationAsString(maxLogSpyDuration),
			},
		},
		{
			// Can't spy for too long.
			vals: map[string][]string{
				"Duration": {(2 * maxLogSpyDuration).String()},
			},
		...
		{
			vals: map[string][]string{
				"Message": {"(unresolved parentheses = tension"},
			},
