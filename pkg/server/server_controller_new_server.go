...
func (s *Server) startTenantServerInternal(
	...
	// If the stopper is being requested (by the surrounding server) to
	// shut down early, cancel the startCtx to stop the server
	// initialization early too.
	var cancel func()
	startCtx, cancel = stopper.WithCancelOnQuiesce(startCtx)
	defer cancel()
