...
func makeSharedProcessTenantServerConfig(
	...
	tempStorageCfg := base.TempStorageConfigFromKVConfig(ctx, st, kvServerCfg.SQLConfig.TempStorageConfig)
	if !tempStorageCfg.InMemory {
		// TODO(knz): Make tempDir configurable.
		storePath := tempStorageCfg.Spec.Path
		if tempStorageCfg.Path, err = fs.CreateTempDir(storePath, TempDirPrefix, stopper); err != nil {
			return BaseConfig{}, SQLConfig{}, errors.Wrap(err, "could not create temporary directory for temp storage")
		}

		recordPath := filepath.Join(storePath, TempDirsRecordFilename)
