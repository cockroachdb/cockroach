...
//
// The metadataFn callback is invoked with source="pprof_endpoint" to emit the
// metadata event into the trace buffer.
func ExecutionTraceHandler(
	metadataFn func(ctx context.Context, source string), w http.ResponseWriter, r *http.Request,
) {
	...
	if metadataFn != nil {
		metadataFn(r.Context(), "pprof_endpoint")
	}
