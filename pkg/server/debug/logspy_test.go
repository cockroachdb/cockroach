...
func TestDebugLogSpyBadJSON(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)
	send := make(chan log.Interceptor, 1)
	spy := logSpy{
	...
	go func() {
		...
		err := spy.run(ctx, &buf, logSpyOptions{
			vmoduleOptions: vmoduleOptions{Duration: durationAsString(5 * time.Second)},
			Count:          2,
		})
		require.ErrorContains(t, err, "could not unmarshal entry")
	}()

	t.Logf("waiting for interceptor")
	f := <-send
	t.Logf("got interceptor, sending malformed JSON...")

	f.Intercept([]byte(`{"bad":"json")`))
	if undoF := <-send; undoF != nil {
		t.Fatal("interceptor closed with non-nil function")
	}
