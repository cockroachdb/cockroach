...
func (spy *logSpy) run(ctx context.Context, w io.Writer, opts logSpyOptions) (err error) {
	...
	for {
		...
		case err := <-interceptor.errCh:
			return errors.Wrapf(err, "while intercepting entry")
		}
	}
}
...
type logSpyInterceptor struct {
	...
	// errCh is for passing along errors encountered by Intercept().
	// Errors are only expected if we're unable to unmarshall a log
	// entry, which we do not tolerate in the log spy system.
	errCh chan error
}
