...
func setupProcessWideRoutes(
	...
	mux.HandleFunc("/debug/pprof/trace", authzFunc(func(w http.ResponseWriter, r *http.Request) {
		ExecutionTraceHandler(execTraceMetadataFn, w, r)
	}))
...
// NewServer sets up a debug server. The execTraceMetadataFn callback, if
// non-nil, is called when serving /debug/pprof/trace to embed CRDB metadata
// (node ID, version, wallclock) into Go execution traces.
func NewServer(
	...
	vsrv := &vmoduleServer{}
	setupProcessWideRoutes(mux, st, tenantID, authorizer, vsrv, profiler, execTraceMetadataFn)
