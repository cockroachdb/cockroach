...
func analyzeLSM(dir string, writer io.Writer) error {
	...
	t := pebbletool.New()
	t.RegisterComparer(storage.MVCCComparer)
...
func (ds *Server) AddLSMVisualizationDebugForEngines(id int, spec base.StoreSpec) {
	ds.mux.HandleFunc(fmt.Sprintf("/debug/lsm/%d", id),
		func(w http.ResponseWriter, req *http.Request) {
			if err := analyzeLSM(spec.Path, w); err != nil {
				fmt.Fprintf(w, "error analyzing LSM at %s: %v", spec.Path, err)
			}
		})
}
