...
func NewServer(storage Storage) *Server {
	...
	// When `dot` (from `graphviz`) is not installed, the default ui page throws
	// an error without mentioning the other pages that work. Work around this by
	// making the flamegraph page the landing page instead.
	{
		_, err := exec.Command("dot", "-V").CombinedOutput()
		s.haveDot = err == nil
	}
...
func (s *Server) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if profileName == "" {
		...
		sort.Strings(names)
		msg := fmt.Sprintf("Try %s for either of %s", path.Join(r.RequestURI, "<profileName>"), strings.Join(names, ", "))
	...
	var maybeFlamegraph string
	if !s.haveDot {
		maybeFlamegraph = "flamegraph"
	}

	http.Redirect(w, r, r.RequestURI+"/"+id+"/"+maybeFlamegraph, http.StatusTemporaryRedirect)
}
