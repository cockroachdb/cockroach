...
// every 30 seconds and triggers an upgrade attempt if needed. Other than
// that, it also starts an upgrade attempt 30 seconds after a new sql server
	...
	return s.stopper.RunAsyncTask(ctx, "tenant-auto-upgrade-checker", func(ctx context.Context) {
		...
		for {
			...
			case <-time.After(time.Second * 30):
				...
				// the storage cluster version changed.
...
func (s *SQLServer) upgradeStatus(
	...
	// Upgrade Logic:
	// If tenant logical version == minimum instance binary version then upgrade already completed; exit.
	// Else if minimum instance binary version supports upgrading to storage logical version; upgrade to storage logical version.
	// Else upgrade to minimum instance binary version binary version.
	//
	// In any of the cases above, if storage logical version is less than the upgradeTo version,
	// return upgradeBlockedDueToLowStorageClusterVersion and do not upgrade.
