	...
	return s.stopper.RunAsyncTask(ctx, "tenant-auto-upgrade-checker", func(ctx context.Context) {
		...
		for {
			...
			case <-time.After(time.Second * 10):
				...
				if (storageClusterVersion != latestStorageClusterVersion && storageClusterVersion.Internal == 0) || firstAttempt {
					firstAttempt = false
...
func (s *SQLServer) startAttemptUpgrade(ctx context.Context) error {
	...
	if k := s.cfg.TestingKnobs.Server; k != nil {
		upgradeTestingKnobs := k.(*TestingKnobs)
		if tenantAutoUpgradeInfoCh := upgradeTestingKnobs.TenantAutoUpgradeInfo; tenantAutoUpgradeInfoCh != nil {
...
func (s *SQLServer) upgradeStatus(
	...
	if preserveDowngradeVersion := s.settingsWatcher.GetPreserveDowngradeVersionSettingValue(); preserveDowngradeVersion != "" {
		downgradeVersion, err := roachpb.ParseVersion(preserveDowngradeVersion)
		if err != nil {
