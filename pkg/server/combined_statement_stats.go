...
func getTotalRuntimeSecs(
	...
	getRuntime := func(table string) (float32, error) {
		it, err := ie.QueryIteratorEx(
			ctx,
			fmt.Sprintf(`console-%s-total-runtime`, table),
			nil,
			sessiondata.NodeUserSessionDataOverride,
			fmt.Sprintf(`
SELECT COALESCE(
...
func getIterator(
	...
	query := fmt.Sprintf(
		...
		aostClause,
		orderAndLimit)
	opName := fmt.Sprintf(`console-%s`, queryInfo)
...
`
	...
	if activityTableHasAllData {
		it, err = ie.QueryIteratorEx(ctx, "console-stmts-activity-for-txn", nil,
...
GROUP BY
	...
	if it == nil || !it.HasResults() {
		...
		query = fmt.Sprintf(
			...
			"crdb_internal.statement_statistics_persisted"+tableSuffix,
			whereClause)
		it, err = ie.QueryIteratorEx(ctx, "console-stmts-persisted-for-txn", nil,
			sessiondata.NodeUserSessionDataOverride, query, args...)
	...
	if !it.HasResults() {
		...
		it, err = ie.QueryIteratorEx(ctx, "console-stmts-with-memory-for-txn", nil,
			sessiondata.NodeUserSessionDataOverride, query, args...)
