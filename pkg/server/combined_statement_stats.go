...
func getCombinedStatementStats(
	...
	earliestStatementAggregatedTs, err := statsProvider.(*persistedsqlstats.PersistedSQLStats).ScanEarliestAggregatedTs(
		ctx, ie, "system.statement_statistics",
	)
	if err != nil {
		return nil, err
	}

	earliestTransactionAggregatedTs, err := statsProvider.(*persistedsqlstats.PersistedSQLStats).ScanEarliestAggregatedTs(
		ctx, ie, "system.transaction_statistics",
	)
	if err != nil {
		return nil, err
	}
