...
func getSourceStatsInfo(
	...
	getRuntime := func(table string, useActivityTable bool) (float32, error) {
...
FROM %s %s`, table, whereClauseOldestDate), args...)
	...
	if activityTableHasAllData {
		...
		stmtsRuntime, err = getRuntime(stmtSourceTable, true /*useActivityTable*/)
		if err != nil {
	...
	if stmtsRuntime == 0 {
		...
		stmtsRuntime, err = getRuntime(stmtSourceTable, false /*useActivityTable*/)
		if err != nil {
			return 0, 0, nil, stmtSourceTable, "", err
		}
		oldestDate, err = getOldestDate(stmtSourceTable)
	...
	if req.FetchMode == nil || req.FetchMode.StatsType != serverpb.CombinedStatementsStatsRequest_StmtStatsOnly {
		if activityTableHasAllData {
			...
			txnsRuntime, err = getRuntime(txnSourceTable, true /*useActivityTable*/)
			if err != nil {
		...
		if txnsRuntime == 0 {
			...
			txnsRuntime, err = getRuntime(txnSourceTable, false /*useActivityTable*/)
			if err != nil {
				return 0, 0, nil, stmtSourceTable, txnSourceTable, err
			}
		}
