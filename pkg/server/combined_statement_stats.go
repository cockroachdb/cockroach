...
func (s *statusServer) CombinedStatementStats(
	...
	if rb.req.FetchMode == nil || rb.req.FetchMode.StatsType == serverpb.CombinedStatementsStatsRequest_TxnStatsOnly {
...
func (rb *sqlStatsRespBuilder) activityTablesHaveFullData(
	...
	limit := rb.req.Limit
	order := serverpb.StatsSortOptions_SERVICE_LAT
	if rb.req.FetchMode != nil {
		order = rb.req.FetchMode.Sort
	}
...
func (rb *sqlStatsRespBuilder) initSetSourceTables(ctx context.Context) error {
	...
	activityTableValidation := func() bool {
	...
	stmtTablesInPrecedence := [3]tableSource{
		{table: CrdbInternalStmtStatsCached, validationFn: activityTableValidation},
		{table: CrdbInternalStmtStatsPersisted},
		{table: CrdbInternalStmtStatsCombined},
	}

	txnTablesInPrecedence := [3]tableSource{
		{table: CrdbInternalTxnStatsCached, validationFn: activityTableValidation},
		{table: CrdbInternalTxnStatsPersisted},
		{table: CrdbInternalTxnStatsCombined},
	}

	// Used for mixed cluster version, where we need to use the persisted view with _v22_2.
	if !rb.st.Version.IsActive(ctx, clusterversion.V23_1AddSQLStatsComputedIndexes) {
		stmtTablesInPrecedence[1].table = crdbInternalStmtStatsPersisted_V22_2
		txnTablesInPrecedence[1].table = CrdbInternalTxnStatsPersisted_V22_2
	}
	...
	isValidSourceTable := func(tableSource tableSource) bool {
		table := tableSource.table

		if tableSource.validationFn != nil && !tableSource.validationFn() {
			return false
		}
...
FROM %s %s`, table, whereClause), args...)
	...
	if rb.req.FetchMode == nil || rb.req.FetchMode.StatsType != serverpb.CombinedStatementsStatsRequest_StmtStatsOnly {
		for i := range txnTablesInPrecedence {
			rb.txnsTableSource = txnTablesInPrecedence[i].table
			if isValidSourceTable(txnTablesInPrecedence[i]) {
				break
			}
		}
	}

	// We always need to query the stmts tables, even for txns.
	for i := range stmtTablesInPrecedence {
		rb.stmtsTableSource = stmtTablesInPrecedence[i].table
		if isValidSourceTable(stmtTablesInPrecedence[i]) {
			break
		}
	}
...
func (rb *sqlStatsRespBuilder) getCombinedStatementsQueryClausesAndArgs() (
	...
	limit := rb.req.Limit
	if limit == 0 {
		limit = SQLStatsResponseMax.Get(&rb.st.SV)
	}
	...
	if rb.req.FetchMode == nil {
		col = "fingerprint_id"
	} else if rb.req.FetchMode.StatsType == serverpb.CombinedStatementsStatsRequest_StmtStatsOnly {
		col = getStmtColumnFromSortOption(rb.req.FetchMode.Sort)
	} else if rb.req.FetchMode.StatsType == serverpb.CombinedStatementsStatsRequest_TxnStatsOnly {
		col = getTxnColumnFromSortOption(rb.req.FetchMode.Sort)
	}
