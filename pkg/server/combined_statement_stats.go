...
func getCombinedStatementStats(
	...
	var fetchStatsType serverpb.CombinedStatementsStatsRequest_StatsType
	if req.FetchMode != nil {
		fetchStatsType = req.FetchMode.StatsType
	}

	if fetchStatsType != serverpb.CombinedStatementsStatsRequest_StmtStatsOnly {
	...
	if fetchStatsType == serverpb.CombinedStatementsStatsRequest_TxnStatsOnly {
...
`
	...
	if req.FetchMode.StatsType != serverpb.CombinedStatementsStatsRequest_TxnStatsOnly {
	...
	if req.FetchMode.StatsType != serverpb.CombinedStatementsStatsRequest_StmtStatsOnly {
...
const (
	...
	sortContentionTimeDesc = `(statistics -> 'statistics' -> 'execution_statistics' -> 
                         'contentionTime' ->> 'mean')::FLOAT DESC`
