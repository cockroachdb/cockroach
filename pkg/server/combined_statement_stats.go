...
`
	...
	if req.FetchMode == nil || req.FetchMode.StatsType != serverpb.CombinedStatementsStatsRequest_TxnStatsOnly {
		if activityTableHasAllData {
			stmtsRuntime, err = getRuntime("system.statement_activity")
			if err != nil {
				return 0, 0, err
			}
		}
	...
	if req.FetchMode == nil || req.FetchMode.StatsType != serverpb.CombinedStatementsStatsRequest_StmtStatsOnly {
		if activityTableHasAllData {
			txnsRuntime, err = getRuntime("system.transaction_activity")
			if err != nil {
				return 0, 0, err
			}
		}
...
`
	...
	if activityTableHasAllData {
		query = fmt.Sprintf(queryFormat, "system.statement_activity", whereClause)
