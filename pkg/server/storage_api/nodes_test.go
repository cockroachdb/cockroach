...
func TestNodeStatusResponse(t *testing.T) {
	...
	testCases := []testCase{
		...
		},
		{
			...
			expectErr:                "status: 500", // `can_view_node_info` capability is missing
		},
	}
	...
	for _, tc := range testCases {
		...
		srv.TenantController().StartedDefaultTestTenant()
		...
		if err := srvtestutils.GetStatusJSONProto(s, "nodes", &wrapper); !testutils.IsError(err, tc.expectErr) {
			t.Fatal(err)
		}
