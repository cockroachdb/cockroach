...
func (s *SQLServerWrapper) PreStart(ctx context.Context) error {
	...
	s.externalStorageBuilder.init(
		...
		s.
			sqlServer.execCfg.InternalExecutorFactory,
		s.db,
		s.costController,
	)
	...
	if err := schedulerlatency.StartSampler(
		ctx, s.sqlServer.cfg.Settings, s.stopper, s.registry, base.DefaultMetricsSampleInterval,
	); err != nil {
		return err
	}
	...
	if err := s.costController.Start(
		ctx, s.stopper, instanceID, s.sqlServer.sqlLivenessSessionID,
