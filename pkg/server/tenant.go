...
func makeTenantSQLServerArgs(
	...
	if p, ok := baseCfg.TestingKnobs.Server.(*TestingKnobs); ok && p.EnableNodelocalStorageForTenants {
		blobClientFactory = blobs.NewLocalOnlyBlobClientFactory(st.ExternalIODir)
	}
