...
having min(completed_at)-interval '1:00' <= '%[1]s'
   and min(completed_at)+interval '1:00' >= '%[1]s'`,
		e.getStartTime().UTC().Format(timeutil.FullTimeFormat),
	)

	data, err := txn.QueryRow(ctx, "check if enforcer start is near cluster init time", txn.KV(), query)
	if err != nil {
		return false, err
	}
	if len(data) != 0 {
	...
	// The cluster isn't considered new. Log an entry to show the min completed_at
	// value for debugging purposes.
	data, err = txn.QueryRow(ctx, "fetch min completed_at", txn.KV(),
		"select completed_at from system.migrations WHERE internal = 99")
	if err != nil {
		return false, err
	}
	if len(data) == 0 {
		log.Infof(ctx, "no migrations found")
		return false, nil
	}
	log.Infof(ctx, "cluster init is not within the bounds of the enforcer start time: %s", data[0].String())
	return false, nil
}
