...
func TestClusterInitGracePeriod_NoOverwrite(t *testing.T) {
	...
	err := enforcer.Start(ctx, srv.ClusterSettings(), srv.InternalDB().(descs.DB))
...
func TestThrottleErrorMsg(t *testing.T) {
	...
	licenseEnforcer := srv.ExecutorConfig().(sql.ExecutorConfig).LicenseEnforcer
	...
	for _, tc := range []struct {
		...
		{"at-threshold-expired-license-past-grace-valid-telemetry-SET_CLUSTER", true, t60d1ms, t55d,
			"SET CLUSTER SETTING trace.debug.enable = false", ""},
