		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			err := db.Txn(ctx, func(ctx context.Context, txn isql.Txn) error {
				_, err := txn.Exec(ctx, "add new min to system.migrations", txn.KV(),
					"INSERT INTO system.migrations VALUES (0, 0, 0, 99, $1)", tc.minSysMigrationTime)
				return err
			})
			require.NoError(t, err)
			// Delete the row we inserted to set up for the next test unit
			defer func() {
				_ = db.Txn(ctx, func(ctx context.Context, txn isql.Txn) error {
					_, err := txn.Exec(ctx, "add new min to system.migrations", txn.KV(),
						"DELETE FROM system.migrations where internal = 99")
					return err
				})
			}()
