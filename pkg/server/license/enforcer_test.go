...
func TestClusterInitTSCache(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)

	// This is the timestamp that we'll override the cluster init timestamp with.
	...
	// Create a new enforcer, to test that it won't overwrite the cluster init
	...
	// Ensure request for the cluster init ts1 before start just returns the start
	// time used when the enforcer was created.
	require.Equal(t, ts2, enforcer.GetClusterInitTS())
	// Start the enforcer to read the timestamp from the KV.
	err := enforcer.Start(ctx, srv.SystemLayer().InternalDB().(descs.DB))
	require.NoError(t, err)
	require.Equal(t, ts1, enforcer.GetClusterInitTS())
}
