...
func (sb *ServiceCertificateBundle) loadOrCreateServiceCertificates(
	...
	if err == nil {
		...
		if err != nil {
			return errors.Wrapf(
				err, "loaded service CA cert but failed to load service CA key file: %s", caKeyPath,
			)
		}
...
func writeCertificateFile(certPath string, certPEM []byte) error {
	if _, err := os.Stat(certPath); err == nil {
		return errors.Newf("found existing certfile at: %s", certPath)
	} else if !oserror.IsNotExist(err) {
		return errors.Wrapf(err,
			"problem writing keyfile at: %s", certPath)
	}
...
func writeKeyFile(keyPath string, keyPEM []byte) error {
	if _, err := os.Stat(keyPath); err == nil {
		return errors.Newf("found existing keyfile at: %s", keyPath)
	} else if !oserror.IsNotExist(err) {
		return errors.Wrapf(err,
			"problem writing keyfile at: %s", keyPath)
	}
	// TODO(aaron-crl): Add logging here.
