...
`
	...
	insertOldSessions := func(column string) {
		...
		for i := 0; i < 3; i++ {
			switch column {
			case "expiresAt":
				expiresAt = expiresAt.Add(purgeTTL * time.Duration(-1))
			case "revokedAt":
				revokedAt = revokedAt.Add(purgeTTL * time.Duration(-1))
			case "lastUsedAt":
				lastUsedAt = lastUsedAt.Add(autoLogoutTimeout * time.Duration(-1))
			}
