	...
	for _, tc := range []struct {
		...
		t.Run("", func(t *testing.T) {
			...
			if tc.shouldAuthenticate {
				...
				if err == nil {
					t.Errorf(
						"credentials %s/%s succeeded, wanted error containing '%s'",
						tc.username,
						tc.password,
						err,
					)
				}
				if !testutils.IsError(err, tc.expectedErrString) {
					t.Errorf(
						"credentials %s/%s got error %s, wanted error matching %s",
						tc.username,
						tc.password,
						err,
						tc.expectedErrString,
					)
				}
			}
		})
