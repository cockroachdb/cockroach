...
func TestPurgeSession(t *testing.T) {
	...
	if _, err := db.Exec(`SET CLUSTER SETTING server.web_session.purge.period = '1s'`); err != nil {
		t.Fatal(err)
	}
	...
	purgePeriod := webSessionPurgePeriod.Get(settingsValues)
...
`
	...
	// Each iteration of the loop inserts a session, rewinding the age of the given timestamp column with each iteration.
	// These inserted sessions are to be deleted periodically.
	insertOldSessions := func(column string) {
		...
		// This helps avoid having deletion checks at the boundary of valid deletion.
		margin := 5 * time.Second

		// Create three seemingly-old sessions, rewinding the age of the desired column with each iteration.
		for i := 0; i < 3; i++ {
			// Rewind the age of the timestamp column such that it's older than the configured cluster setting values.
			switch column {
			case "expiresAt":
			...
			if !ts.authentication.hasActivePurgeTask {
				if err = ts.authentication.purgeOldSessions(ctx); err != nil {
					t.Fatal(err)
				}
				ts.authentication.hasActivePurgeTask = true
			}
		}
	}
