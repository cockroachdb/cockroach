...
func TestAuthenticationMux(t *testing.T) {
	...
	runRequest := func(
		...
		if a, e := resp.StatusCode, expected; a != e {
			...
			return fmt.Errorf("got status code %d (msg %s), wanted %d", a, string(message), e)
		}
		return nil
	}

	checkIsBehindAuth := func(method string, path string, body []byte) {
		t.Run("", func(t *testing.T) {
			// Verify normal client returns 401 Unauthorized.
			if err := runRequest(normalClient, method, path, body, http.StatusUnauthorized); err != nil {
				t.Fatalf("request %s failed when not authorized: %s", path, err)
			}

			// Verify authenticated client returns 200 OK.
			if err := runRequest(authClient, method, path, body, http.StatusOK); err != nil {
				t.Fatalf("request %s failed when authorized: %s", path, err)
			}
		})
	}

	// Admin service.
	checkIsBehindAuth("GET", adminPrefix+"users", nil)

	// Status service.
	checkIsBehindAuth("GET", statusPrefix+"sessions", nil)
	...
	checkIsBehindAuth("POST", ts.URLPrefix+"query", tsReqBuffer.Bytes())
}
