...
func (c *Cache) Start(ctx context.Context, stopper *stop.Stopper) error {
	return rangefeedcache.Start(ctx, stopper, c.w)
}
...
func (c *Cache) handleUpdate(ctx context.Context, update rangefeedcache.Update) {
	...
	case rangefeedcache.PartialUpdate:
		...
		// missing anything.
		prev := c.GetSystemConfig().Values // okay if prev is nil
		if len(updateKVs) == 0 {
			updatedData = prev
		} else {
	...
	updated := config.NewSystemConfig(c.defaultZoneConfig)
	updated.Values = updatedData
	toNotify := c.setUpdatedConfig(updated, update.Timestamp)
