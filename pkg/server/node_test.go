...
func TestBootstrapCluster(t *testing.T) {
	...
	if _, err := bootstrapCluster(
		ctx, []engine.Engine{e}, cluster.TestingClusterVersion, config.DefaultZoneConfigRef(),
	); err != nil {
		t.Fatal(err)
	}
	...
	kvs, tableSplits := GetBootstrapSchema(config.DefaultZoneConfigRef()).GetInitialValues()
	for _, kv := range kvs {
...
func TestNodeStatusWritten(t *testing.T) {
	...
	initialRanges, err := ExpectedInitialRangeCount(kvDB, ts.cfg.DefaultZoneConfig)
	if err != nil {
		t.Fatal(err)
	}
	testutils.SucceedsSoon(t, func() error {
