...
// TestNodeCrossRangeBatchRequestsMetrics verifies that node.Batch() correctly
// updates the cross-region byte count metrics for sent batch requests and
// received batch responses.
func TestNodeCrossRangeBatchRequestsMetrics(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)
