...
func TestNodeStatusWritten(t *testing.T) {
	...
	// Split the range.
	if err := ts.db.AdminSplit(context.Background(), splitKey, hlc.MaxTimestamp, roachpb.AdminSplitRequest_Ingestion); err != nil {
		t.Fatal(err)
	}
...
func TestGetTenantWeights(t *testing.T) {
	...
	require.NoError(t, s.DB().AdminSplit(ctx, prefix, hlc.MaxTimestamp, roachpb.AdminSplitRequest_Ingestion))
