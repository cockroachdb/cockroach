...
func TestDiskStatsMap(t *testing.T) {
	...
	admissionStats, err := statsMap.tryPopulateAdmissionDiskStats(10)
	require.NoError(t, err)
	for id, stat := range admissionStats {
		switch id {
		case 1:
			expectedDS := admission.DiskStats{
				ProvisionedBandwidth: 10,
				BytesRead:            disk.SectorSizeBytes,
				BytesWritten:         2 * disk.SectorSizeBytes,
			}
			require.Equal(t, expectedDS, stat)
		case 2:
			expectedDS := admission.DiskStats{
				ProvisionedBandwidth: 5,
