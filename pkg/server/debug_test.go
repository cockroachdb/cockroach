...
func TestHandleVModule(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{true, "storage=2,server=3", http.StatusOK},
	} {
		t.Run(fmt.Sprintf("vmodule=%t/%s", tc.vmodule, tc.path), func(t *testing.T) {
			ClusterSettings.DebugVModule.Override(tc.vmodule)
			req := httptest.NewRequest("GET", vmodulePrefix+tc.path, nil)
