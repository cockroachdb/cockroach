...
func (s *SQLServer) preStart(
	...
	regionPhysicalRep, err := startup.RunWithRetryEx(ctx, "sql get locality",
	...
	instance, err := startup.RunWithRetryEx(ctx, "sql create node instance row",
	...
	if s.execCfg.Codec.ForSystemTenant() {
		if err := startup.RunWithRetry(ctx, "sql get cluster version", func(ctx context.Context) error {
	...
	if err := startup.RunWithRetry(ctx, "sql get tenant version", func(ctx context.Context) error {
