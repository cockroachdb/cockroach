...
func (s *SQLServer) preStart(
	...
	regionPhysicalRep, err := startup.RunWithStartupRetryEx(ctx, "sql get locality",
	...
	instance, err := startup.RunWithStartupRetryEx(ctx, "sql create node instance row",
	...
	if s.execCfg.Codec.ForSystemTenant() {
		if err := startup.RunWithStartupRetry(ctx, "sql get cluster version", func(ctx context.Context) error {
	...
	if err := startup.RunWithStartupRetry(ctx, "sql get tenant version", func(ctx context.Context) error {
