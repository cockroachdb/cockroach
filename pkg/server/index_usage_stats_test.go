...
func compareStatsHelper(
	...
	compareTimeHelper(t, expected.LastScan, actual.LastScan, delta)
	compareTimeHelper(t, expected.LastJoin, actual.LastJoin, delta)
	compareTimeHelper(t, expected.LastUpdate, actual.LastUpdate, delta)

	// We don't perform deep comparison of time.Time. So we set them to a dummy
	// value before deep equal.
	dummyTime := timeutil.Now()

	expected.LastScan = dummyTime
	expected.LastJoin = dummyTime
	expected.LastUpdate = dummyTime
	actual.LastScan = dummyTime
	actual.LastJoin = dummyTime
	actual.LastUpdate = dummyTime

	require.Equal(t, expected, actual)
}

func createIndexStatsIngestedCallback() (
	func(indexusagestats.MetaData),
	...
	cb := func(payload indexusagestats.MetaData) {
		notify <- payload.Key
	}
...
func createIndexStatsResetCallback() (func(), chan struct{}) {
	// Create a buffered channel so the callback is non-blocking.
...
func waitForStatsResetCompletion(
	t *testing.T, notify chan struct{}, nodeCount int, timeout time.Duration,
) {
	var timer timeutil.Timer
	timer.Reset(timeout)
...
func TestStatusAPIIndexUsage(t *testing.T) {
	...
	statsResetCb, statsResetNotifer := createIndexStatsResetCallback()

	testCluster := serverutils.StartNewTestCluster(t, 3, base.TestClusterArgs{
		ServerArgs: base.TestServerArgs{
			Knobs: base.TestingKnobs{
				IndexUsageStatsKnobs: &indexstatslocal.TestingKnobs{
					OnIndexUsageStatsProcessedCallback:     statsIngestionCb,
					OnIndexUsageStatsResetCompleteCallback: statsResetCb,
				},
			},
		},
	})
	...
	expectedStatsIndexA := roachpb.IndexUsageStatistics{
		ZigzagJoinCount:  1,
		NonFullScanCount: 1,
		LastScan:         timeutil.Now(),
		LastJoin:         timeutil.Now(),
	}
	...
	waitForStatsIngestion(t, statsIngestionNotifier, map[roachpb.IndexUsageKey]struct{}{
		indexKeyA: {},
		indexKeyB: {},
	}, /* expectedKeys */ 3 /* expectedEventCnt*/, 5*time.Second /* timeout */)

	// First node should have nothing.
	stats := firstLocalStatsReader.GetIndexUsageStats(indexKeyA)
	require.Equal(t, roachpb.IndexUsageStatistics{}, stats, "expecting empty stats on node 1, but found %v", stats)

	stats = firstLocalStatsReader.GetIndexUsageStats(indexKeyB)
	require.Equal(t, roachpb.IndexUsageStatistics{}, stats, "expecting empty stats on node 1, but found %v", stats)
	require.True(t, firstLocalStatsReader.GetLastReset().IsZero(), "expecting index usage stats not to be reset, but it's being reset at %v", firstLocalStatsReader.GetLastReset())

	// Third node should have nothing.
	stats = thirdLocalStatsReader.GetIndexUsageStats(indexKeyA)
	require.Equal(t, roachpb.IndexUsageStatistics{}, stats, "expecting empty stats on node 3, but found %v", stats)

	stats = thirdLocalStatsReader.GetIndexUsageStats(indexKeyB)
	require.Equal(t, roachpb.IndexUsageStatistics{}, stats, "expecting empty stats on node 1, but found %v", stats)
	require.True(t, thirdLocalStatsReader.GetLastReset().IsZero(), "expecting index usage stats not to be reset, but it's being reset at %v", secondLocalStatsReader.GetLastReset())

	// Second server should have nonempty local storage.
	stats = secondLocalStatsReader.GetIndexUsageStats(indexKeyA)
	compareStatsHelper(t, expectedStatsIndexA, stats, time.Minute)

	stats = secondLocalStatsReader.GetIndexUsageStats(indexKeyB)
	compareStatsHelper(t, expectedStatsIndexB, stats, time.Minute)
	require.True(t, secondLocalStatsReader.GetLastReset().IsZero(), "expecting index usage stats not to be reset, but it's being reset at %v", thirdLocalStatsReader.GetLastReset())
	...
	url := thirdServer.ServingRPCAddr()
	conn, err := rpcContext.GRPCDialNode(url, thirdServer.NodeID(), rpc.DefaultClass).Connect(ctx)
	require.NoError(t, err)
	...
	// Test reset loop.
	_, err = secondServerSQLConn.Exec("SET CLUSTER SETTING sql.metrics.index_usage_stats.reset_interval = '1s'")
	require.NoError(t, err)

	waitForStatsResetCompletion(t, statsResetNotifer, 3 /* nodeCount */, 10*time.Second /* timeout */)
	...
	for _, stats := range resp.Statistics {
		...
		switch stats.Key.IndexID {
		case 2: // t@t_a_idx
			require.Equal(t, roachpb.IndexUsageStatistics{}, stats.Stats)
		case 3: // t@t_b_idx
			require.Equal(t, roachpb.IndexUsageStatistics{}, stats.Stats)
		}
	}
	require.True(t, statsEntries == 0, "expect to find no entries in RPC response, but found %d", statsEntries)
}
