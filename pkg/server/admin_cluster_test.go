...
func TestAdminAPIDatabaseDetails(t *testing.T) {
	...
	if _, err := db.Exec("CREATE TABLE test.foo (id INT PRIMARY KEY, val STRING)"); err != nil {
		t.Fatal(err)
	}
	for i := 0; i < 10; i++ {
		if _, err := db.Exec("INSERT INTO test.foo VALUES($1, $2)", i, "test"); err != nil {
			t.Fatal(err)
		}
	}
	...
	url := server0.AdminURL() + "/_admin/v1/databases/test"
	var response serverpb.DatabaseDetailsResponse

	testutils.SucceedsSoon(t, func() error {
		if err := httputil.GetJSON(client, url, &response); err != nil {
			t.Fatal(err)
		}

		assert.Equal(t, int64(1), response.Stats.RangeCount, "RangeCount")
		...
		assert.Equal(t, uint64(0), response.Stats.ApproximateDiskBytes, "ApproximateDiskBytes")
