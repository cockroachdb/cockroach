...
func TestCollectSpanStatsResponses(t *testing.T) {
		...
		nodeResponses: []nodeResponse{
			{nodeID: 1, resp: &roachpb.SpanStatsResponse{
				SpanToStats: map[string]*roachpb.SpanStats{
					"span1": {RangeCount: 1, ApproximateDiskBytes: 1, RemoteFileBytes: 1, ExternalFileBytes: 1, StoreIDs: []roachpb.StoreID{1}},
				},
			}},
			{nodeID: 2, resp: &roachpb.SpanStatsResponse{
				SpanToStats: map[string]*roachpb.SpanStats{
					"span2": {RangeCount: 1, ApproximateDiskBytes: 1, RemoteFileBytes: 1, ExternalFileBytes: 1, StoreIDs: []roachpb.StoreID{2}},
				},
			}},
		},
		expectedRes: &roachpb.SpanStatsResponse{
	...
	tc2 := testCase{
		...
		nodeResponses: []nodeResponse{
			{nodeID: 1, resp: &roachpb.SpanStatsResponse{
				SpanToStats: map[string]*roachpb.SpanStats{
					"span1": {TotalStats: totalStats1, RangeCount: 1, ApproximateDiskBytes: 1, RemoteFileBytes: 1, ExternalFileBytes: 1, StoreIDs: []roachpb.StoreID{1, 2}},
				},
			}},
			{nodeID: 2, resp: &roachpb.SpanStatsResponse{
				SpanToStats: map[string]*roachpb.SpanStats{
					"span1": {TotalStats: totalStats2, RangeCount: 1, ApproximateDiskBytes: 1, RemoteFileBytes: 1, ExternalFileBytes: 1, StoreIDs: []roachpb.StoreID{2}},
				},
			}},
		},
		expectedRes: &roachpb.SpanStatsResponse{
			SpanToStats: map[string]*roachpb.SpanStats{
				"span1": {TotalStats: totalStats1, ApproximateTotalStats: expectedApproxTotalStats, RangeCount: 1, ApproximateDiskBytes: 2, RemoteFileBytes: 2, ExternalFileBytes: 2, StoreIDs: []roachpb.StoreID{1, 2}},
			},
		},
	}
	...
	// test case 4 - node response contains span not in original response object
	tc4 := testCase{
