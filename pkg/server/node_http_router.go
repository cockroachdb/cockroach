...
type nodeProxy struct {
	...
	parseNodeID          ParseNodeIDFunc
	getNodeIDHTTPAddress GetNodeIDHTTPAddressFunc
	proxyCache           reverseProxyCache
}

// ParseNodeIDFunc is a callback to parse strings into `roachpb.NodeID`
// instances. It exists to prevent a dependency on `gossip`.
type ParseNodeIDFunc func(string) (roachpb.NodeID, bool, error)

// GetNodeIDHTTPAddressFunc is a callback to look up HTTP addresses
// for a given NodeID. It exists to prevent a dependency on `gossip`.
type GetNodeIDHTTPAddressFunc func(roachpb.NodeID) (*util.UnresolvedAddr, error)
