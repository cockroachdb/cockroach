...
// header and query params. If the `NodeRoutingIDKey` key is found in
...
var ErrNoNodeID = errors.New("http: named cookie not present")
...
func (np *nodeProxy) parseNodeID(nodeIDParam string) (roachpb.NodeID, bool, error) {
	// No parameter provided or set to local.
	if len(nodeIDParam) == 0 || localRE.MatchString(nodeIDParam) {
		return np.gossip.NodeID.Get(), true, nil
	}
	...
	return nodeID, nodeID == np.gossip.NodeID.Get(), nil
}
