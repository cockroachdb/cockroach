...
func MapRangesToIndexes(
	...
	for i < len(ranges) && j < len(indexes) {
		if spanFromRange(ranges[i]).Overlaps(indexes[j].Span) {
...
func RangesToTableSpans(codec keys.SQLCodec, ranges []roachpb.RangeDescriptor) []roachpb.Span {
	...
	for _, rangeDesc := range ranges {
		span := spanFromRange(rangeDesc).Clamp(bounds)
		if !span.ZeroLength() {
...
func SpansToOrderedTableDescriptors(
	ctx context.Context, txn descs.Txn, codec keys.SQLCodec, spans []roachpb.Span,
...
func spanFromRange(rangeDesc roachpb.RangeDescriptor) roachpb.Span {
	return rangeDesc.KeySpan().AsRawSpanWithNoLocals()
}
