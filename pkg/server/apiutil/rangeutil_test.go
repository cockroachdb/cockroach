...
func TestMapRangesToIndexes(t *testing.T) {
	...
	for rangeID, expectedIndexes := range expected {
		...
		if !ok {
			t.Fatalf("expected rangeID %d not found", rangeID)
		}
		if len(actualIndexes) != len(expectedIndexes) {
			t.Fatalf("expected %d indexes for rangeID %d, got %d", len(expectedIndexes), rangeID, len(actualIndexes))
		}
		for i, expectedIndex := range expectedIndexes {
			if actualIndexes[i].Index != expectedIndex.Index {
				t.Fatalf("expected index %v, got %v", expectedIndex, actualIndexes[i])
			}
		}
	}
}
...
func TestRangesToTableSpans(t *testing.T) {
	...
	if len(result) != len(expectedSpans) {
		t.Fatalf("expected %d spans, got %d", len(expectedSpans), len(result))
	}

	for i, expectedSpan := range expectedSpans {
		if !result[i].Equal(expectedSpan) {
			t.Fatalf("expected span %v, got %v", expectedSpan, result[i])
		}
	}
}
...
func makeTableDesc(databaseID uint32, name string, indexes []string) catalog.TableDescriptor {
	...
	for i, name := range indexes {
		if i == 0 {
			table.AddPrimaryIndex(descpb.IndexDescriptor{Name: name})
		} else {

			table.AddSecondaryIndex(descpb.IndexDescriptor{Name: name})
		}
	}
