...
// ServerTestingKnobs groups testing knobs for the Server.
type ServerTestingKnobs struct {
...
func (*ServerTestingKnobs) ModuleTestingKnobs() {}
	...
	if err := s.stopper.RunAsyncTask(ctx, "auto-upgrade", func(ctx context.Context) {
		...
		for r := retry.StartWithCtx(ctx, retryOpts); r.Next(); {
			...
			if k := s.cfg.TestingKnobs.Server; k != nil {
				upgradeTestingKnobs := k.(*ServerTestingKnobs)
