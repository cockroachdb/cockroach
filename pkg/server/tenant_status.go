...
func (t *tenantStatusServer) CancelQuery(
	...
	if err := t.checkCancelPrivilege(ctx, request.Username); err != nil {
		return nil, err
	}
	var (
	...
	output.Canceled, err = t.cancelLocalQuery(ctx, request)
	if err != nil {
...
func (t *tenantStatusServer) CancelSession(
	...
	output.Canceled, err = t.cancelLocalSession(ctx, request)
	if err != nil {
		output.Error = err.Error()
	}
	return output, nil
