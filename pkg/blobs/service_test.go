...
func TestBlobServiceGet(t *testing.T) {
	...
	service := NewBlobService(tmpDir)
	ctx := context.TODO()

	t.Run("get-correct-file", func(t *testing.T) {
		resp, err := service.Get(ctx, &blobspb.GetBlobRequest{
			Filename: filename,
		})
		if err != nil {
			t.Fatal(err)
	...
	t.Run("file-not-exist", func(t *testing.T) {
		_, err := service.Get(ctx, &blobspb.GetBlobRequest{
	...
	t.Run("not-in-external-io-dir", func(t *testing.T) {
		_, err := service.Put(ctx, &blobspb.PutBlobRequest{
			Filename: "file/../../content.txt",
		})
		if err == nil {
			t.Fatal("expected error but was not caught")
		}
		if !strings.Contains(err.Error(), "outside of external-io-dir is not allowed") {
			t.Fatal("incorrect error message: " + err.Error())
		}
	})
}

func TestBlobServicePut(t *testing.T) {
	tmpDir, cleanupFn := testutils.TempDir(t)
	defer cleanupFn()
...
func TestBlobServiceList(t *testing.T) {
	...
	t.Run("list-correct-files", func(t *testing.T) {
		resp, err := service.List(ctx, &blobspb.ListBlobRequest{
			Dir: "file/dir/*.csv",
		})
		if err != nil {
			t.Fatal(err)
	...
	t.Run("not-in-external-io-dir", func(t *testing.T) {
		_, err := service.List(ctx, &blobspb.ListBlobRequest{
			Dir: "file/../../*.csv",
		})
		if err == nil {
			t.Fatal("expected error but was not caught")
...
func TestBlobServiceDelete(t *testing.T) {
	...
	t.Run("delete-correct-file", func(t *testing.T) {
		_, err := service.Delete(ctx, &blobspb.DeleteBlobRequest{
			Filename: filename,
		})
		if err != nil {
			t.Fatal(err)
...
func TestBlobServiceSize(t *testing.T) {
	tmpDir, cleanupFn := testutils.TempDir(t)
	defer cleanupFn()
	...
	t.Run("get-correct-file-size", func(t *testing.T) {
		resp, err := service.Size(ctx, &blobspb.GetBlobSizeRequest{
			Filename: filename,
		})
		if err != nil {
			t.Fatal(err)
	...
	t.Run("file-not-exist", func(t *testing.T) {
		_, err := service.Size(ctx, &blobspb.GetBlobSizeRequest{
			Filename: "file/does/not/exist",
		})
		if err == nil {
			t.Fatal("expected error but was not caught")
