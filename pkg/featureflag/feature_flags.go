...
func CheckEnabled(
	ctx context.Context, config FeatureFlagConfig, s *settings.BoolSetting, featureName string,
) error {
	sv := config.SV()
...
// FeatureFlagConfig is an interface used to pass required values from
// ExecutorConfig to check if a feature flag is enabled.
type FeatureFlagConfig interface {
	SV() *settings.Values
