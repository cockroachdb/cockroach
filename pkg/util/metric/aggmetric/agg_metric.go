...
func (cs *childSet) initWithCacheStorageType(metricName string, labels []string) {
	...
	// We track all metrics which uses cache storage type in the registry so that
	// we can reinitialise children when AppNameLabelEnabled or DBNameLabelEnabled
	// settings are changed.
	registerMetric(metricName, cs)
}
...
func (cs *childSet) add(metric ChildMetric) {
	...
	// Check if the childSet has labels. If not, then it is as good
	// as it's parent value. Hence, return to avoid redundant values
	// in metric scrape.
	if len(cs.mu.labels) == 0 {
		return
	}
