...
func (r *Registry) ReinitialiseChildMetrics(isDBNameEnabled, isAppNameEnabled bool) {
	...
	for _, metric := range r.tracked {
		// Check if the metric implements the metric.PrometheusConfigurable interface as we want to
		// reinitialise the child metrics.
		if m, ok := metric.(PrometheusConfigurable); ok {
			m.ReinitialiseChildMetrics(uint64(labelConfig))
		}
	}
