...
func newHistogram(meta Metadata, duration time.Duration, buckets []float64,
	bucketConfig staticBucketConfig,
) *Histogram {
	...
	// If not buckets are provided, generate buckets from status bucket
	// configuration
	if buckets == nil && bucketConfig.count != 0 {
		buckets = GetBucketsFromBucketConfig(bucketConfig)
	}
