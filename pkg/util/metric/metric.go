...
// value of opts.Buckets, this is suitable for recording any kind of
// quantity. The histogram is very similar to Histogram produced by NewHistogram
// with the main difference being that Histogram supports collecting values
// over time using the Histogram.RecordValue whereas this histogram does not
// support collecting values. Instead, this histogram supports replacing the cumulative and
// windowed histogram. This means that it is the responsibility of the creator
// of this histogram to replace the values by calling
...
// Internally there are two sets of histograms: one is the cumulative set (i.e.
// data is never evicted) which is a prometheus.Histogram and the other is the windowed set
