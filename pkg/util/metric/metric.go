...
func NewHistogramVecOpts(metadata Metadata, histOpts prometheus.HistogramOpts) HistogramVecOpts {
	// Align Name and Help fields
	histOpts.Name = metadata.Name
	histOpts.Help = metadata.Help
...
// persists the aggregation of n labeled child metrics).
func NewExportedHistogramVec(histVecOpts HistogramVecOpts, labelNames []string) *HistogramVec {
	vec := newVector(labelNames)
	promVec := prometheus.NewHistogramVec(histVecOpts.histOpts, vec.orderedLabelNames)

	return &HistogramVec{
		Metadata: histVecOpts.metadata,
		vector:   vec,
		promVec:  promVec,
	}
}
