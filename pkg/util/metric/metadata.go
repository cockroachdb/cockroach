...
// at the given depth (0 = callerSourceFile's caller). Go records
// module-relative paths in frame.File for code within a module, so
// the result is stable across local builds, Bazel sandboxes, and CI.
//
...
func callerSourceFile(skip int) string {
	...
	return strings.TrimPrefix(frame.File, modulePrefix)
}
