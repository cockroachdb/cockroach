...
format at /api/v2/rules/. These rules are not consumed programmatically and are
meant to be used as guidelines from engineers implementing metric(s) on how
metric(s) can be aggregated and used in alerts.
...
1. AlertingRule: This rule is used to provide guidelines on how
   one or metric can be used for alerts.
2. AggregationRule: This rule is used to provide guidelines on
   how one or more metric can be aggregated to provide indicators
...
the RuleRegistry to confirm the rules are initialized without any error.
