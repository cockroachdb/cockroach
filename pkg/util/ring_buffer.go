...
// RingBuffer is a deque maintained over a ring buffer.
...
func (r *RingBuffer) AddFirst(element interface{}) {
	if cap(r.buffer) == 0 {
		r.buffer = make([]interface{}, RingBufferInitialSize)
		r.buffer[0] = element
		r.tail = 1
	} else {
...
// and doubles it's underlying slice if necessary.
func (r *RingBuffer) AddLast(element interface{}) {
	if cap(r.buffer) == 0 {
		r.buffer = make([]interface{}, RingBufferInitialSize)
		r.buffer[0] = element
		r.tail = 1
...
func (r *RingBuffer) Get(pos int) interface{} {
...
func (r *RingBuffer) GetFirst() interface{} {
	if !r.nonEmpty {
...
func (r *RingBuffer) GetLast() interface{} {
	if !r.nonEmpty {
...
// Reset makes RingBuffer treat its underlying memory as if it were empty. This
