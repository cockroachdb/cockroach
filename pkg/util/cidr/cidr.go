...
func NewLookup(ctx context.Context, st *settings.Values, stopper *stop.Stopper) *Lookup {
	...
	cidrRefreshInterval.SetOnChange(st, func(ctx context.Context) {
		intervalChangedChan <- struct{}{}
	})

	getTickDuration := func() time.Duration {
		tickDuration := cidrRefreshInterval.Get(st)
		// If the tickDuration is 0, set to a year to avoid refreshing.
		if tickDuration == 0 {
