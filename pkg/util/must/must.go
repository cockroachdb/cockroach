...
// failures kill the process (with a stack trace) to ensure they are noticed. In
// Go tests, they instead panic to allow other tests to run.
//
...
// the failure and and return early, since it has already been stopped.
//
...
// Fail triggers an assertion failure. The behavior depends on the
// build type:
//
//   - Dev: fatals with a stack trace.
//   - Test: panics, to continue running other tests.
//   - Release: returns an assertion error, logs it with a stack
//     trace, and reports it to Sentry (if enabled).
...
// True requires v to be true. Otherwise, fatals/panics in dev/test builds, or
...
// False requires v to be false. Otherwise, fatals/panics in dev/test builds, or
// returns an assertion error that's also logged and reported in release builds.
//
// gcassert:inline
...
// Equal requires a and b to be equal. Otherwise, fatals/panics in dev/test
...
// NotEqual requires a and b not to be equal. Otherwise, fatals/panics in
...
// Greater requires a > b. Otherwise, fatals/panics in dev/test builds, or
// returns an assertion error that's also logged and reported in release builds.
//
// gcassert:inline
...
// GreaterOrEqual requires a >= b. Otherwise, fatals/panics in dev/test builds,
// or returns an assertion error that's also logged and reported in release
// builds.
//
// gcassert:inline
...
// Less requires a < b. Otherwise, fatals/panics in dev/test builds, or returns
...
// LessOrEqual requires a <= b. Otherwise, fatals/panics in dev/test builds, or
// returns an assertion error that's also logged and reported in release builds.
//
// gcassert:inline
...
// Error requires a non-nil error. Otherwise, fatals/panics in dev/test builds,
// or returns an assertion error that's also logged and reported in release
// builds.
//
// gcassert:inline
...
// NoError requires a nil error. Otherwise, fatals/panics in dev/test builds, or
// returns an assertion error that's also logged and reported in release builds.
//
// gcassert:inline
func NoError(ctx context.Context, err error, format string, args ...interface{}) error {
	...
	format += ": errored with %T %v"
	return failDepth(ctx, 1, format, append(args, err, err)...)
}

// Len requires a slice to have the specified length. Otherwise, fatals/panics in
// dev/test builds, or returns an assertion error that's also logged and
// reported in release builds.
//
...
// Empty requires an empty slice. Otherwise, fatals/panics in dev/test builds, or
// returns an assertion error that's also logged and reported in release builds.
//
// TODO(erikgrinaker): This should handle maps and strings too, if possible.
//
...
func Empty[V any, T ~[]V](ctx context.Context, c T, format string, args ...interface{}) error {
	...
	format += ": contains %d elements"
	return failDepth(ctx, 1, format, append(args, l)...)
}

// NotEmpty requires a non-empty slice. Otherwise, fatals/panics in dev/test
// builds, or returns an assertion error that's also logged and reported in
// release builds.
//
// TODO(erikgrinaker): This should handle maps and strings too, if possible.
...
// Nil requires v to be nil. Otherwise, fatals/panics in dev/test builds, or
// returns an assertion error that's also logged and reported in release builds.
//
...
// NotNil requires v not to be nil. Otherwise, fatals/panics in dev/test builds, or
// returns an assertion error that's also logged and reported in release builds.
//
// gcassert:inline
...
// Zero requires v to be zero-valued for its type. Otherwise, fatals/panics in dev/test
// builds, or returns an assertion error that's also logged and reported in
// release builds.
//
// gcassert:inline
...
// NotZero requires v not to be zero-valued for its type. Otherwise,
// fatals/panics in dev/test builds, or returns an assertion error that's also
// logged and reported in release builds.
//
// gcassert:inline
...
// Otherwise, fatals/panics in dev/test builds, or returns an assertion error
// that's also logged and reported in release builds.
//
// gcassert:inline
...
// location). Otherwise, fatals/panics in dev/test builds, or returns an
