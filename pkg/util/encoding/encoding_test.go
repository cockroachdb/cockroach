...
func TestPrettyPrintValueEncoded(t *testing.T) {
	...
	err = ipnet.ParseINet(ip, &ipNet)
	if err != nil {
		t.Fatalf("Bad test case. Attempted ipnet.ParseINet(%q) got err: %d", ip, err)
	}
	tests := []struct {
		buf      []byte
		expected string
...
func BenchmarkEncodeIPNetValue(b *testing.B) {
	rng, _ := randutil.NewPseudoRand()
	rd := randData{rng}

	vals := make([]ipnet.IPNet, 10000)
	for i := range vals {
		vals[i] = rd.ipNet()
	}
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = EncodeIPNetValue(buf, NoColumnID, vals[i%len(vals)])
	}
}

func BenchmarkDecodeIPNetValue(b *testing.B) {
	rng, _ := randutil.NewPseudoRand()
	rd := randData{rng}

	vals := make([][]byte, 10000)
	for i := range vals {
		vals[i] = EncodeIPNetValue(nil, uint32(rng.Intn(100)), rd.ipNet())
	}

	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		if _, _, err := DecodeIPNetValue(vals[i%len(vals)]); err != nil {
			b.Fatal(err)
		}
	}
}
