...
// convertStringToBytes converts a string to a byte array to be used with string encoding functions.
...
func encodeStringAscendingWithTerminatorAndPrefix(b []byte, s string, terminator byte, prefix byte) []byte {
	str := convertStringToBytes(s)
	return encodeBytesAscendingWithTerminatorAndPrefix(b, str, terminator, prefix)
}
