...
func DecodeUntaggedIPNetValue(b []byte) (remaining []byte, u ipnet.IPNet, err error) {
	...
	if u.Family == ipnet.IPv4Family {
		remaining = b[ipnet.IPv4Size:]
	} else if u.Family == ipnet.IPv6Family {
		remaining = b[ipnet.IPv6Size:]
	}
...
func PeekValueLength(b []byte) (typeOffset int, length int, err error) {
	...
	case IPNet:
		...
		if family == ipnet.IPv4Family {
			...
			panic("IPNet has wrong family size")
		}
