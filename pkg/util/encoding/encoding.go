...
const (
	...
	Box2D        Type = 24
	geoInverted  Type = 25 // Not exported since only used for pretty-printing
)
...
func prettyPrintFirstValue(dir Direction, b []byte) ([]byte, string, error) {
	...
	case geoInverted:
		// For conciseness, only print the cellid.
		var cellid uint64
		_, cellid, err := DecodeUvarintAscending(b[1:])
		if err != nil {
			return b, "", err
		}
		_, _, _, _, b, err := DecodeGeoInvertedKey(b)
		if err != nil {
			return b, "", err
		}
		return b, strconv.FormatUint(cellid, 10), nil
	default:
		if len(b) >= 1 {
			switch b[0] {
			case jsonInvertedIndex:
