...
func DecodeBitArrayAscending(b []byte) ([]byte, bitarray.BitArray, error) {
	...
	numWords, _, err := getBitArrayWordsLen(b)
	if err != nil {
...
func getBitArrayWordsLen(b []byte) (int, int, error) {
	bSearch := b
	numWords := 0
	sz := 0
	for {
		...
		if bSearch[0] == bitArrayDataTerminator {
			break
		}
...
func PeekLength(b []byte) (int, error) {
	...
	case bitArrayMarker, bitArrayDescMarker:
		_, n, err := getBitArrayWordsLen(b[1:])
		if err != nil {
			return 1 + n, err
		}
...
func UpperBoundValueEncodingSize(colID uint32, typ Type, size int) (int, bool) {
	...
	case BitArray:
		if size > 0 {
			return len(encodedTag) + maxVarintSize + (size+63)/8, true
		}
		return 0, false
	case Float:
