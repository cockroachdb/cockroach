...
// EncodeIPAddrValue encodes a net.IPNet value with its value tag, appends it to
// the supplied buffer, and returns the final buffer.
...
// EncodeUntaggedIPAddrValue encodes a net.IPNet value, appends it to the supplied buffer,
// and returns the final buffer.
...
func PeekValueLength(b []byte) (typeOffset int, length int, err error) {
	...
	case IPAddr:
		...
		if family == ipaddr.IPv4Family {
			size = ipaddr.IPv4Size
		} else if family == ipaddr.IPv6Family {
			size = ipaddr.IPv6Size
		} else {
			return 0, 0, errors.Errorf("IPAddr has wrong family size: got %d", family)
		}
