...
type AdditionMode int64

const (
	// AdditionModeAuto is a sentinel to detect sessions sent from
	// legacy clients.  It will use the cluster version to determine
	// which mode should be supported.
	AdditionModeAuto AdditionMode = iota
	...
	AdditionModeCompatible
)

// AdditionModeFromString parses s to produce an AdditionMode, or
// returns (AdditionModeAuto, false).
func AdditionModeFromString(s string) (AdditionMode, bool) {
	switch s {
	case "auto":
		return AdditionModeAuto, true
	case "legacy":
		return AdditionModeLegacy, true
	case "compatible":
		return AdditionModeCompatible, true
	default:
		return AdditionModeAuto, false
	}
}

func (m AdditionMode) String() string {
	switch m {
	case AdditionModeAuto:
		return "auto"
	case AdditionModeLegacy:
		return "legacy"
	case AdditionModeCompatible:
		return "compatible"
	default:
		return "unknown"
	}
...
func Add(ctx Context, t time.Time, d Duration) time.Time {
	...
	// We'll see this if we're being called from some place without
	// an EvalContext, so we can just choose our default.
	if mode == AdditionModeAuto {
