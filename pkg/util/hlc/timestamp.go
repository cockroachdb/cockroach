...
func (t Timestamp) AddDuration(duration time.Duration) Timestamp {
	return Timestamp{
		WallTime:  t.WallTime + duration.Nanoseconds(),
		Logical:   t.Logical,
		Synthetic: true,
	}
}

// Sub returns the delta of this time from a previous timestamp.
func (t Timestamp) Sub(previous Timestamp) time.Duration {
	return time.Duration(t.WallTime - previous.WallTime)
}
