...
type Clock struct {
	...
	// The maximal offset of the physical clock from a quorum of known nodes. If
	// higher offsets are detected to a quorum of known nodes, the node will
	// terminate.
	//
...
func (c *Clock) PhysicalNow() int64 {
	c.mu.Lock()
	wallTime := c.getPhysicalClockLocked()
	c.mu.Unlock()
	return wallTime
}
