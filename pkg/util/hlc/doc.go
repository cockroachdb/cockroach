...
correctness. It is intended to be exhaustive.
 ...
 - Observed timestamps (Raft + BatchRequest channels). During the lifetime of a
   ...
   the written_timestamp of the write. As a result, these "observed timestamps"
   captured during the lifetime of a transaction can be used to make a claim
   about values that could not have been written yet at the time that the
   transaction first visited the node, and by extension, at the time that the
   transaction began. This allows the transaction to avoid uncertainty restarts
   in some circumstances. For more, see pkg/kv/kvserver/observedts/doc.go.
 ...
 - Non-transactional requests (Raft + BatchRequest channels). Most KV operations
   ...
   requests do not carry uncertainty intervals, so to avoid stale reads, they
   receive their read timestamp on the server. Specifically, because these
   requests can not cross ranges, they receive their timestamp on the
   leaseholder of the range that they are querying. As mentioned before, when a
   leaseholder on a node serves a write, it ensures that the node's HLC clock is
   >= the timestamp of the write. This ensures that any later non-transactional
   read receives a timestamp >= all writes served previously.
