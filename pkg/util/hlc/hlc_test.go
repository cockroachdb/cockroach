...
func TestHLCLess(t *testing.T) {
	m := NewManualClock(1)
	c := NewClockWithTimeSource(m, time.Nanosecond)
	a := c.Now()
	b := a
...
func TestHLCPhysicalClockJump(t *testing.T) {
	...
	for _, test := range testCases {
		t.Run(test.name, func(t *testing.T) {
			...
			m := NewManualClock(1)
			c := NewClockWithTimeSource(m, test.maxOffset)
...
func TestHLCClock(t *testing.T) {
	m := NewManualClock(1)
	c := NewClockWithTimeSource(m, 1000*time.Nanosecond)
...
func TestHLCMonotonicityCheck(t *testing.T) {
	...
	c := NewClockWithTimeSource(m, 100*time.Nanosecond)
...
func TestClock_UpdateAndCheckMaxOffset_UntrustworthyValue(t *testing.T) {
	...
	c := NewClockWithTimeSource(m, 500*time.Millisecond)
...
func BenchmarkUpdate(b *testing.B) {
	...
	b.StartTimer()
	for n := 0; n < b.N; n++ {
		clock := NewClockWithTimeSource(NewManualClock(123), time.Second)
		wg := sync.WaitGroup{}
		for w := 0; w < concurrency; w++ {
