...
func getCgroupMemInactiveFileUsage(root string) (usage int64, warnings string, err error) {
	...
	mounts, versions, err := getCgroupDetails(filepath.Join(root, "/proc/self/mountinfo"), path, "memory")
	if err != nil {
		return 0, "", err
	}
	// assert len(mounts) == len(versions)
	if len(versions) == 2 {
		// Look up against V2 first. Fall back to V1.
		usage, warnings, err = detectMemInactiveFileUsageInV2(filepath.Join(root, mounts[1], path))
		if err != nil {
			usage, warnings, err = detectMemInactiveFileUsageInV1(filepath.Join(root, mounts[0]))
		}
	} else {
		...
		case 1:
			usage, warnings, err = detectMemInactiveFileUsageInV1(filepath.Join(root, mounts[0]))
		case 2:
			usage, warnings, err = detectMemInactiveFileUsageInV2(filepath.Join(root, mounts[0], path))
		}
	}
...
func getCgroupMemUsage(root string) (usage int64, warnings string, err error) {
	...
	if len(versions) == 2 {
		// Look up against V2 first. Fall back to V1.
		usage, warnings, err = detectMemUsageInV2(filepath.Join(root, mounts[1], path))
		if err != nil {
			usage, warnings, err = detectMemUsageInV1(filepath.Join(root, mounts[0]))
		}
	} else {
		// assert len(versions) == 1
		switch versions[0] {
		case 1:
			usage, warnings, err = detectMemUsageInV1(filepath.Join(root, mounts[0]))
		case 2:
			usage, warnings, err = detectMemUsageInV2(filepath.Join(root, mounts[0], path))
		}
	}
...
func getCgroupMemLimit(root string) (limit int64, warnings string, err error) {
	...
	if len(versions) == 2 {
		// Look up against V2 first. Fall back to V1.
		limit, warnings, err = detectMemLimitInV2(filepath.Join(root, mounts[1], path))
		if err != nil {
			limit, warnings, err = detectMemLimitInV1(filepath.Join(root, mounts[0]))
		}
	} else {
		// assert len(versions) == 1
		switch versions[0] {
		case 1:
			limit, warnings, err = detectMemLimitInV1(filepath.Join(root, mounts[0]))
		case 2:
			limit, warnings, err = detectMemLimitInV2(filepath.Join(root, mounts[0], path))
		}
	}
...
//	If only one version was found, first output param. has the mountpoint; second has the version.
//	Otherwise, first two output params are empty and an error is returned.
//
...
func getCgroupDetails(mountinfoPath string, cRoot string, controller string) ([]string, []int, error) {
	...
	if err != nil {
		return []string{}, []int{}, errors.Wrapf(err, "failed to read mounts info from file: %s", log.SafeManaged(mountinfoPath))
	}
	defer func() {
		_ = info.Close()
	}()
	...
	return []string{}, []int{}, fmt.Errorf("failed to detect cgroup root mount and version")
}
...
func getCgroupCPU(root string) (CPUUsage, error) {
	...
	mounts, versions, err := getCgroupDetails(filepath.Join(root, "/proc/self/mountinfo"), path, "cpu,cpuacct")
	if err != nil {
		return CPUUsage{}, err
	}
	...
	if len(versions) == 2 {
		// Look up against V2 first. Fall back to V1.
		res.Period, res.Quota, err = detectCPUQuotaInV2(filepath.Join(root, mounts[1], path))
		if err != nil {
			res.Period, res.Quota, err = detectCPUQuotaInV1(filepath.Join(root, mounts[0]))
		}
		if err != nil {
			return res, err
		}
		res.Stime, res.Utime, err = detectCPUUsageInV2(filepath.Join(root, mounts[1], path))
		if err != nil {
			res.Stime, res.Utime, err = detectCPUUsageInV1(filepath.Join(root, mounts[0]))
		}
		if err != nil {
			return res, err
		}
		...
		case 2:
			res.Period, res.Quota, err = detectCPUQuotaInV2(filepath.Join(root, mounts[0], path))
			if err != nil {
				return res, err
			}
			res.Stime, res.Utime, err = detectCPUUsageInV2(filepath.Join(root, mounts[0], path))
			if err != nil {
				return res, err
			}
		}
