...
func detectMemCntrlPath(cgroupFilePath string) (string, error) {
	...
	for scanner.Scan() {
		...
		// If the number of fields we got is different from what we expect
		// as per the current file format, then something fishy is happening
		if len(fields) == 3 {
			f1, f2 := string(fields[0]), string(fields[1])
			// First case if v1, second - v2. There is also a `hybrid` mode when both
			// versions are enabled, but no known container solutions support it afaik
