...
func walkAST(path jsonpath.Path) error {
	return walk(path, 0 /* depth */, false /* inArray */)
}

func walk(path jsonpath.Path, depth int, inArray bool) error {
	switch path := path.(type) {
	case jsonpath.Paths:
		for _, p := range path {
			if err := walk(p, depth, inArray); err != nil {
				return err
			}
		}
		return nil
	case jsonpath.ArrayList:
		for _, p := range path {
			if err := walk(p, depth, true /* inArray */); err != nil {
				return err
			}
		}
		return nil
	case jsonpath.ArrayIndexRange:
		if err := walk(path.Start, depth, inArray); err != nil {
			return err
		}
		if err := walk(path.End, depth, inArray); err != nil {
			return err
		}
		return nil
	case jsonpath.Operation:
		if err := walk(path.Left, depth, inArray); err != nil {
			return err
		}
		if path.Right != nil {
			if err := walk(path.Right, depth, inArray); err != nil {
				return err
			}
		}
		return nil
	case jsonpath.Filter:
		if err := walk(path.Condition, depth+1, inArray); err != nil {
			return err
		}
		return nil
