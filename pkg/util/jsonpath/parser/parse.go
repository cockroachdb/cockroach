...
func init() {
	tree.ValidateJSONPath = func(jsonpath string) (*jsonpath.Jsonpath, []string, error) {
		jp, vars, err := Parse(jsonpath)
		if err != nil {
			return nil, nil, err
		}
		return jp.AST, vars, nil
	}
}
...
func Parse(path string) (statements.JsonpathStatement, []string, error) {
	var p Parser
	stmt, err := p.Parse(path)
	if err != nil {
		return statements.JsonpathStatement{}, nil, err
	}
	// Similar to flattenJsonPathParseItem in postgres, we do a pass over the AST
	// to perform some semantic checks.
	vars, err := stmt.AST.ValidateAndCollectVariables()
	if err != nil {
		return statements.JsonpathStatement{}, nil, err
	}
	return stmt, vars, nil
}
