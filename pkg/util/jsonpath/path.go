...
var errHandling = func(err error) {
...
func buildInvertedIndexSpans(
	...
	default:
		...
		errHandling(errors.AssertionFailedf("unexpected path pattern in inverted index span generation: %s", pathComponents))
		return nil
	}
...
func EncodeJsonPathInvertedIndexSpans(b []byte, p Path) (invertedExpr inverted.Expression) {
	...
	case Paths:
		ps = t
	default:
		errHandling(errors.AssertionFailedf("unsupported json path type %T for inverted index", p))
		return nil
	}

	if !isSupportedPathPattern(ps, true /* atRoot */) {
		errHandling(errors.AssertionFailedf("unsupported json path pattern for inverted index: %T", p))
		return nil
	}

	if len(ps) == 0 {
		errHandling(errors.AssertionFailedf("empty json path for inverted index"))
		return nil
	}
	res := buildInvertedIndexSpans([][]byte{encoding.EncodeJSONAscending(b)}, ps, 0, lastKeyIndex(ps), nil /* filterValue */)
	return res
}
