	...
	if _, err := f.Forward(s, hlc.Timestamp{WallTime: wall}); err != nil {
		...
		case 1:
			if !errors.Is(err, interval.ErrInvertedRange) {
				return errors.Newf("expected inverted span error for span %s, found %+v instead", s, err)
			}
		case 0:
			if len(s.Key) == 0 && len(s.EndKey) == 0 {
				if !errors.Is(err, interval.ErrNilRange) {
					return errors.Newf("expected nil range error for span %s, got %+v instead", s, err)
				}
			} else if !errors.Is(err, interval.ErrEmptyRange) {
				return errors.Newf("expected empty span error for span %s found %+v instead", s, err)
			}
		default:
			return errors.Newf("err=%+v f=%s", err, f)
		}
	}
	return nil
}
