...
func (f *Frontier) entriesStr() string {
	...
	f.Entries(func(sp roachpb.Span, ts hlc.Timestamp) (done bool) {
		if buf.Len() != 0 {
			buf.WriteString(` `)
		}
...
func TestSpanEntries(t *testing.T) {
	...
	spanEntries := func(sp roachpb.Span) string {
		var buf strings.Builder
		f.SpanEntries(sp, func(s roachpb.Span, ts hlc.Timestamp) (done bool) {
			if buf.Len() != 0 {
				buf.WriteString(` `)
			}
...
func TestUpdatedEntries(t *testing.T) {
	...
	updatedEntries := func(cutoff int64) string {
		var buf strings.Builder
		f.UpdatedEntries(hlc.Timestamp{WallTime: cutoff}, func(s roachpb.Span, ts hlc.Timestamp) (done bool) {
			if buf.Len() != 0 {
				buf.WriteString(` `)
			}
			fmt.Fprintf(&buf, `%s@%d`, s, ts.WallTime)
