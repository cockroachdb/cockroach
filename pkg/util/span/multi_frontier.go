...
//
// An example function would be one that returns the table ID that
// a span belongs to and an error if the span overlaps multiple tables.
...
// multiple sub-frontiers using a given PartitionerFunc. It starts
// with no sub-frontiers and creates new ones lazily when spans that
// map to previously unseen partitions are added.
//
