...
func TestTimeTZ(t *testing.T) {
	maxTime, err := ParseTimeTZ(timeutil.Now(), "24:00:00-1459")
	require.NoError(t, err)
	minTime, err := ParseTimeTZ(timeutil.Now(), "00:00:00+1459")
	require.NoError(t, err)
	...
	testCases := []struct {
		...
		{
			t:           minTime,
			toTime:      time.Date(1970, 1, 1, 0, 0, 0, 0, timeutil.FixedOffsetTimeZoneToLocation(14*60*60+59*60, "TimeTZ")),
			largerThan:  []TimeTZ{},
		...
		{
			t:           maxTime,
			toTime:      time.Date(1970, 1, 2, 0, 0, 0, 0, timeutil.FixedOffsetTimeZoneToLocation(-(14*60*60+59*60), "TimeTZ")),
