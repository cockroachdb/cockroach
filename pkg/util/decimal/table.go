...
// powerTenTableSize is the magnitude of the maximum power of 10 exponent
// that is stored in the pow10LookupTable. The table will store both
// negative and positive powers 10 up to this magnitude. For instance, if
// the powerTenTableSize if 3, then the lookup table will store power of 10
// values from 10^-3 to 10^3.
const powerTenTableSize = 32

var pow10LookupTable [2*powerTenTableSize + 1]inf.Dec

func init() {
	tmpInt := new(big.Int)
	for i := -powerTenTableSize; i <= powerTenTableSize; i++ {
		setDecWithPow(&pow10LookupTable[i+powerTenTableSize], tmpInt, i)
	}
}
...
func PowerOfTenDec(pow int) *inf.Dec {
	if pow >= -powerTenTableSize && pow <= powerTenTableSize {
		return &pow10LookupTable[pow+powerTenTableSize]
	}
	dec := new(inf.Dec)
