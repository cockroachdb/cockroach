...
func TestIPNetParseInet(t *testing.T) {
	...
	for i, testCase := range testCases {
		if actual, err := ParseINet(testCase.s); err != nil {
		...
		} else if actual != nil && testCase.exp != nil && !actual.Equal(testCase.exp) {
...
func TestIPNetBinaryMarshalling(t *testing.T) {
	...
	for i, testCase := range testCases {
		var data []byte
		var err error
		if data, err = testCase.input.MarshalBinary(); err != nil {
			t.Errorf("%d: MarshalBinary(%s) caused an unexpected error:%s", i, testCase.input, err)
		}
...
func TestIPNetCompare(t *testing.T) {
	...
	for i, testCase := range testCases {
		ip1, _ := ParseINet(testCase.s1)
		ip2, _ := ParseINet(testCase.s2)
