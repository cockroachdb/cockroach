...
func (u *IPNet) MarshalBinary() ([]byte, error) {
	...
	if u.Family == IPv4Family {
		...
		_, err = buffer.Write(u.IP.To4())
	} else {
		_, err = buffer.Write(u.IP)
	}
...
func (u *IPNet) UnmarshalBinary(data []byte) error {
	u.Family = int(data[0])
	if u.Family == IPv4Family {
		u.Mask = net.CIDRMask(int(data[1]), 32)
		u.IP = make(net.IP, net.IPv4len)
		copy(u.IP, data[2:IPv4Size])
	} else if u.Family == IPv6Family {
		u.Mask = net.CIDRMask(int(data[1]), 128)
		u.IP = make(net.IP, net.IPv6len)
