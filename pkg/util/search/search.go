...
type Searcher interface {
	...
	Search(func(int) (bool, error)) (int, error)

	// The following two methods are un-exported and are by searchWithSearcher
	// to provide a default implementation of Search.
...
func searchWithSearcher(s Searcher, pred func(int) (bool, error)) (int, error) {
	for {
...
func (ss *searchSpace) bound(pass bool, cur, prec int) (bool, int) {
	...
	if pass {
		if cur >= ss.max {
			panic("passed at index above max")
		}
		ss.min = cur
	} else {
		if cur <= ss.min {
			panic("failed at index below min")
		}
		ss.max = cur
	}
...
func (bs *binarySearcher) Search(pred func(int) (bool, error)) (int, error) {
...
func (bs *binarySearcher) step(pass bool) (found bool) {
	...
	if pass {
		bs.cur = mid(bs.cur, bs.ss.max)
	} else {
		bs.cur = mid(bs.ss.min, bs.cur)
	}
	return false
}
...
func NewLineSearcher(min, max, start, stepSize, prec int) Searcher {
	...
	if start < min || start > max {
		panic("start must be between [min, max]")
	}
	if stepSize < 1 {
		panic("stepSize must be >= 1")
	}
	if prec < 1 {
		panic("precision must be >= 1")
	}
...
func (ls *lineSearcher) Search(pred func(int) (bool, error)) (int, error) {
