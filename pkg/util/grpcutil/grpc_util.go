...
// gRPC due to failed precondition. This error is returned when trying to connect
// to nodes that are decommissioned and thus would never be reachable again. The
// recipient of this error should either try to connect to a different node when
// possible, or pass the error up to the caller (similar to AuthError).
