...
func TestRandomDatums(t *testing.T) {
	...
	ReadFileAndVerifyDatums(t, f.Name(), numRows, numCols, writer.Config(), writer.SchemaDefinition(), datums)
}
...
func TestVersions(t *testing.T) {
	...
	for version := range allowedVersions {
		...
		require.Equal(t, reader.MetaData().Version(), writer.Config().version)
	...
	_, err = NewWriter(schemaDef, f, WithVersion("invalid"))
	require.Error(t, err)
}
