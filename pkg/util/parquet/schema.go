...
// makeColumn constructs a datumColumn.
func makeColumn(colName string, typ *types.T, repetitions parquet.Repetition) (datumColumn, error) {
	...
	case types.TupleFamily:
		...
		//   }
		// }
		// TODO: why differentiating NULL and (NULL) is not possible.
		// TODO: explain why we dont use separate nodes per datumColumn.
		...
		for i, innerTyp := range contents {
			...
			if !ok {
				return datumColumn{}, errors.AssertionFailedf("TODO")
			}
		...
		result.numPhysicalCols = uint32(len(colWriters))
		if err != nil {
			return datumColumn{}, err
		}
		return result, nil
