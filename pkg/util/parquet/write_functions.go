...
// A writeFn encodes a datum and writes using the provided column chunk writer.
type writeFn func(d tree.Datum, w file.ColumnChunkWriter) error

func writeInt32(d tree.Datum, w file.ColumnChunkWriter) error {
	if d == tree.DNull {
...
func writeInt64(d tree.Datum, w file.ColumnChunkWriter) error {
	if d == tree.DNull {
...
func writeBool(d tree.Datum, w file.ColumnChunkWriter) error {
	if d == tree.DNull {
...
func writeString(d tree.Datum, w file.ColumnChunkWriter) error {
	if d == tree.DNull {
...
func writeTimestamp(d tree.Datum, w file.ColumnChunkWriter) error {
	if d == tree.DNull {
		return writeNilBatch[parquet.ByteArray](w)
	}
...
func writeUUID(d tree.Datum, w file.ColumnChunkWriter) error {
	if d == tree.DNull {
...
func writeDecimal(d tree.Datum, w file.ColumnChunkWriter) error {
	if d == tree.DNull {
		return writeNilBatch[parquet.ByteArray](w)
	}
