...
//   work getting done, which is better modeled by allowing a to be less
...
//   [~0, 1]. For the same reason as the previous bullet, we use [~0, 1.5].
//
...
type tokensLinearModelFitter struct {
	...
	// Set to true for the ingested bytes model: if all bytes are ingested below
	// L0, the actual bytes will be zero and the accounted bytes non-zero. We
	// need to update the model in this case.
	updateWithZeroActualNonZeroAccounted bool
}

func makeTokensLinearModelFitter(
	multMin float64, multMax float64, updateWithZeroActualNonZeroAccounted bool,
	...
	return tokensLinearModelFitter{
		...
		updateWithZeroActualNonZeroAccounted: updateWithZeroActualNonZeroAccounted,
	}
}
...
func (f *tokensLinearModelFitter) updateModelUsingIntervalStats(
	...
	if workCount <= 1 || (actualBytes <= 0 &&
		(!f.updateWithZeroActualNonZeroAccounted || accountedBytes <= 0)) {
	...
	// Put a lower bound of 1 on constant. So workCount tokens go into that.
	constant := int64(1)
