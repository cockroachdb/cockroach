...
type perWorkTokensAux struct {
	...
	// intAdjustedWriteBytes represent the total write bytes for the interval,
	// excluding ignored bytes.
	intAdjustedLSMWrites  int64
	intAdjustedWriteBytes int64
}
...
func (e *storePerWorkTokenEstimator) updateEstimates(
	...
	adjustedIntWrites := intDiskWrite - intIgnoredIngestedBytes - intL0IgnoredWriteBytes
	if adjustedIntWrites < 0 {
		adjustedIntWrites = 0
	}
	e.atDoneWriteAmpLinearModel.updateModelUsingIntervalStats(adjustedIntLSMWrites, adjustedIntWrites, intWorkCount)
	...
	e.aux = perWorkTokensAux{
		...
		intAdjustedWriteBytes:     adjustedIntWrites,
