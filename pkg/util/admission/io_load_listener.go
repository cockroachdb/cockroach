...
func (res adjustTokensResult) SafeFormat(p redact.SafePrinter, _ rune) {
	...
	p.Printf("write-iops-model %.2fx+%s (smoothed %.2fx+%s) + ",
		res.aux.perWorkTokensAux.intWriteIOPSLinearModel.multiplier,
		ib(res.aux.perWorkTokensAux.intWriteIOPSLinearModel.constant),
		res.writeIOPSLM.multiplier, ib(res.writeIOPSLM.constant))
