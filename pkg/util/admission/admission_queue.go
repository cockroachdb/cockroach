...
// WorkPriority represents the priority of work. In an AdmissionQueue, it is
// only used for ordering within a tenant. High priority work can starve lower
// priority work.
...
type WorkInfo struct {
	...
	// Optional information specified only for AdmissionQueues where the work is
	// tied to a range. This allows queued work to return early as soon as the
	// range is no longer in a relevant state at this node. Currently only
	// KVWork is tied to a range.
...
// AdmissionQueue maintains a queue of work waiting to be admitted. Ordering
// of work is achieved via 2 heaps: a tenant heap orders the tenants with
// waiting work in increasing order of used slots or tokens. Within each
// tenant, the waiting work is ordered based on priority and create time.
// Tenants with non-zero values of used slots or tokens are tracked even if
// they have no more waiting work. Token usage is reset to zero every second.
// The choice of 1 second of memory for token distribution fairness is
// somewhat arbitrary. The same 1 second interval is also used to garbage
// collect tenants who have no waiting requests and no used slots or tokens.
//
...
func (q *AdmissionQueue) String() string {
	return redact.StringWithoutMarkers(q)
}

// SafeFormat implements the redact.SafeFormatter interface.
func (q *AdmissionQueue) SafeFormat(s redact.SafePrinter, verb rune) {
	q.mu.Lock()
	defer q.mu.Unlock()
...
// closeForTesting only exists to "cleanly" shutdown AdmissionQueue in a unit test setting.
func (q *AdmissionQueue) closeForTesting() {
	q.gcStopCh <- struct{}{}
