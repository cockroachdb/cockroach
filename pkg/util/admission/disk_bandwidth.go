...
// complicated (and incomplete), we adopt a goal of simplicity of design, and
// strong abstraction boundaries.
//
// - The disk load is abstracted using an enum. The diskLoadWatcher, that maps
//   load signals to this enum, can be evolved independently.
//
...
func (d *diskLoadWatcher) setIntervalInfo(
	ctx context.Context, load intervalDiskLoadInfo, storeID int32,
) {
	d.lastInterval = load
	util := float64(load.readBandwidth+load.writeBandwidth) / float64(load.provisionedBandwidth)
	log.Infof(logtags.AddTag(ctx, "s", storeID),
		"diskLoadWatcher bandwidth: read: %s, write: %s, provisioned: %s, util: %.2f",
		humanizeutil.IBytes(load.readBandwidth),
		humanizeutil.IBytes(load.writeBandwidth),
	...
	d.lastUtil = util
	// TODO(sumeer): Use history of fsync latency and the value in the current
...
func makeDiskBandwidthLimiter(storeID int32) diskBandwidthLimiter {
...
func (d *diskBandwidthLimiter) computeElasticTokens(
	...
	d.diskLoadWatcher.setIntervalInfo(ctx, id, d.storeID)
	const alpha = 0.5
	...
	case diskLoadLow:
		...
		if d.elasticTokens != lastElasticTokens {
			log.Infof(logtags.AddTag(ctx, "s", d.storeID), "diskBandwidthLimiter low load")
		}
	...
	case diskLoadModerate:
		...
		if tokensFullyUtilized() {
			...
			log.Infof(logtags.AddTag(ctx, "s", d.storeID),
				"diskBandwidthLimiter moderate: elastic-fr: %.2f, incoming-bytes: %s, "+
					"unused-bw: %s/s, increasing elastic-tokens: %s",
	...
	case diskLoadHigh:
		log.Infof(logtags.AddTag(ctx, "s", d.storeID),
			"diskBandwidthLimiter high: elastic-fr: %.2f, incoming-bytes: %s, "+
				"unused-bw: %s/s, elastic-tokens: limit %s, used %s",
			d.smoothedElasticFraction, humanizeutil.IBytes(int64(d.smoothedIncomingBytes)),
	...
	case diskLoadOverload:
		...
		log.Infof(logtags.AddTag(ctx, "s", d.storeID),
			"diskBandwidthLimiter overload: elastic-tokens: limit %s, used: %s",
