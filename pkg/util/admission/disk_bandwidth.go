...
//   goal to avoid exceeding by a huge amount. Historically, we've had
//   implementations that significantly undershoot the goal (see
...
//
// - Incoming writes that are deemed "elastic": In practice this is done by
//   implying a work-class from the priority (e.g. priorities < NormalPri are
//   deemed elastic). Additionally, incoming range snapshots writing to disk
//   can also be limited using this mechanism.
//
...
//
//   TODO(sumeer): this compaction control does not exist, though there is
//   some related prototype code in
//   https://github.com/cockroachdb/cockroach/pull/141501.
//
...
type diskBandwidthLimiterState struct {
	...
	// writeTokenUtil is the fraction of prevTokens.writeByteTokens used across
	// the sum of usedTokens[i].writeByteTokens (for all work types i).
	writeTokenUtil float64
	// diskLoad represents the disk load info for the previous
	// adjustmentInterval.
...
// for the next adjustmentInterval.
func (d *diskBandwidthLimiter) computeElasticTokens(
	...
	// the read (which could be late, if reads happen in say a 300ms burst near
	// the end of every second). Admittedly, this estimate can be poor, but we
	// account for errors in this estimate separately at a higher frequency. See
