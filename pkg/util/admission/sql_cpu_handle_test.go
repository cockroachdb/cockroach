...
// the original (Admit-time) container. The lastAdmitGroupKey field
// captures the key at Admit time precisely so this scenario stays
// correct: re-deriving from the WorkQueue's current useResourceGroup
// would route to a freshly-created container that never received the
// tokens.
func TestSQLCPUHandleCloseAfterModeFlip(t *testing.T) {
	...
	require.Equal(t, tenantKey, h.mu.lastAdmitGroupKey)
