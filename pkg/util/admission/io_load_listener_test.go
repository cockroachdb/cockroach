...
func TestIOLoadListener(t *testing.T) {
	...
	var cumFlushWorkSec, cumFlushIdleSec time.Duration
	...
	datadriven.RunTest(t, testutils.TestDataPath(t, "io_load_listener"),
		func(t *testing.T, d *datadriven.TestData) string {
			...
			case "init":
				...
				cumFlushBytes = 0
				cumFlushWorkSec = time.Duration(0)
				cumFlushIdleSec = time.Duration(0)
				return ""
			...
			case "set-state":
				...
				cumFlushIdleSec += time.Duration(flushIdleSec) * time.Second
				cumFlushWorkSec += time.Duration(flushWorkSec) * time.Second
				cumFlushBytes += int64(flushBytes)

				metrics.Flush.WriteThroughput.Bytes = cumFlushBytes
				metrics.Flush.WriteThroughput.IdleDuration = cumFlushIdleSec
				metrics.Flush.WriteThroughput.WorkDuration = cumFlushWorkSec
