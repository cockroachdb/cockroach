...
func (q *WorkQueueWithCPUTimeTokenEstimators) start() {
	ticker := q.timeSource.NewTicker(time.Minute)
	go func() {
		for {
			select {
			case _ = <-ticker.Ch():
...
// updateEstimatorsAndGC iterates through all estimators owned by
...
func (q *WorkQueueWithCPUTimeTokenEstimators) Admit(
	...
	func() {
		...
		q.initEstimatorIfNeeded(tenantID)
...
func (q *WorkQueueWithCPUTimeTokenEstimators) AdmittedWorkDone(
	...
	func() {
		...
		q.initEstimatorIfNeeded(tenantIDUint64)
...
func (q *WorkQueueWithCPUTimeTokenEstimators) initEstimatorIfNeeded(tenantID uint64) {
...
//
//   - update() should be called periodically (e.g., once per minute).
...
//
//   - estimateTokensUsed() returns the current smoothed estimate (with a
//     minimum of 1).
//
// Rationale:
//
