	...
	for i := range coord.granters {
		...
		case KVWork:
			...
			case *kvStoreTokenGranter:
				s.Printf(" io-avail: %d(%d), disk-write-tokens-avail: %d",
				...
				if g.coordMu.diskTokensError.diskReadTokensAlreadyDeducted < unlimitedTokens/loadedDuration.ticksInAdjustmentInterval() {
					s.Printf(", disk-read-tokens-deducted: %d", g.coordMu.diskTokensError.diskReadTokensAlreadyDeducted)
				}
			}
