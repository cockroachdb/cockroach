				...
				if ticks%ticksInAdjustmentInterval == 0 {
					...
					for idx, m := range metrics {
						if unsafeGc, ok := sgc.gcMap.Load(int64(m.StoreID)); ok {
							...
							storeMetricsWithDiff.StoreMetrics = &m
							storeMetricsWithDiff.InternalIntervalMetrics = computeIntervalMetrics(m.Metrics, lastMetrics[idx])
...
func computeIntervalMetrics(m1, m2 *pebble.Metrics) *pebble.InternalIntervalMetrics {
	...
	iim := &pebble.InternalIntervalMetrics{}
