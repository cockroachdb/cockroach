...
func (p *testCPUMetricsProvider) GetCPUUsage() (totalCPUTimeMillis int64, err error) {
	if p.retErr != nil {
		return 0, p.retErr
	}
	cycle := p.millis[p.i]
...
func (p *testCPUMetricsProvider) append(millis int64, count int) {
	for i := 0; i < count; i++ {
		p.millis = append(p.millis, millis)
	}
}
