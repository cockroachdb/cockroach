...
func TestCPUTimeTokenLinearModel(t *testing.T) {
	...
	// The first call to fit inits the model, by setting tokenToCPUTimeMultiplier to one,
	// since in prod on the first call to fit, there will be no CPU usage data to use
	// to determine tokenToCPUTimeMultiplier.
	refillRates := model.fit()
	require.Equal(t, float64(1), model.tokenToCPUTimeMultiplier)
	// Given that tokenToCPUTimeMultiplier equals one, refillRates is equal to target
	// utilization for the bucket * the vCPU count (10 vCPUs in this test). The unit of
	// refillRates is nanoseconds. The target utilizations are a function of the
	// admission.cpu_time_tokens.target_util cluster, plus a prioritization scheme. The
	// lowest priority bucket, [testTier1][noBurst] is controlled by
	// admission.cpu_time_tokens.target_util. The next highest priority bucket has a
	// target utilization that is 5% higher than the one below it. And so on. So if
	// admission.cpu_time_tokens.target_util is set to 80%, then 80%, 85%, 90%, 95%, as
	// per below.
	//
	...
	// described up above in the test case of the first call to fit. So here we focus on
	// tokenToCPUTimeMultiplier.
	//
	// 2x
	// Token time is half of actual time, so tokenToCPUTimeMultiplier is two. 100 data
	// points are appended, to give the filter time to converge on two.
	...
	// 4x
	// Token time is one fourth of actual time, so tokenToCPUTimeMultiplier is four.
	tokenCPUTime.append(dur.Nanoseconds()/2, 100)
	...
	// tokenToCPUTimeMultiplier is left alone. If tokenToCPUTimeMultiplier is greater
