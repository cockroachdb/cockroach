...
// Below two are the non-burstable utilization goals. See resetInterval for more.
var KVCPUTimeAppUtilGoal = settings.RegisterFloatSetting(
	settings.SystemOnly,
	"admission.cpu_time_tokens.target_util.app_tenant",
	"the target CPU utilization for app tenant work if using the KV CPU time token system",
	0.8,
...
var KVCPUTimeSystemUtilGoal = settings.RegisterFloatSetting(
	...
	"the target CPU utilization for system tenant work if using the KV CPU time token system",
	0.95,
	settings.FloatWithMinimum(0.2))

// Burstable work is given this much CPU headroom above non-burstable. See resetInterval for more.
var KVCPUTimeUtilBurstDelta = settings.RegisterFloatSetting(
	settings.SystemOnly,
	"admission.cpu_time_tokens.target_util.burst_delta",
	"the delta between non-burstable & burstable CPU utilization target if using the KV CPU time token system",
	0.05)
...
func (f *cpuTimeTokenFiller) start() {
	...
	go func() {
		// We start with the assumption that a full interval worth of ticks are remaining.
		// Thus, in the unlikely case where a full 1s passes before the first tick, the
		// below allocateTokens(1) invariant is still respected.
		...
		for {
			...
			case t := <-ticker.Ch():
				...
				if elapsedSinceIntervalStart >= time.Second {
					...
					// resetInterval() can be called. Without this invariant, cpuTimeTokenAllocator.refillRates
					// tokens would not be allocated every 1s.
					//
					// The below conditional ensures the rate provisioned since the last tick was
					// fully emitted, which may not be the case if ticks arrived late. Ideally, this
					// already happened in the else branch during the previous tick which typically
					// would have occurred at millisecond 999 and then would compute remainingTicks <=
					// 1 and would have called allocateTokens(1) (i.e. "emit everything that's left
					// for this second"). But if the previous tick was not the designated "last" tick
					// yet (say it occurred at 900ms), and a delay had occurred before our tick arrived,
					// we need to call allocateTokens(1) here to release the quota held back by the delay.
...
// every interval is left to cpuTimeModel.
...
func (a *cpuTimeTokenAllocator) allocateTokens(expectedRemainingTicksInInterval int64) {
	...
	// Each bucket has a max capacity. The max capacity for each bucket is one second
	// worth of tokens at the current refill rate. This is a fairly arbitrary decision.
...
func (a *cpuTimeTokenAllocator) resetInterval() {
	// Compute target utilizations from cluster settings. The noBurst targets are configurable
	// by cluster settings. A canBurst target adds a delta to the corresponding
	// noBurst target, and the delta is also configurable by a cluster setting. The code here
	// is not general with respect to numResourceTiers & numBurstQualifications. This isn't
	// necessary for the Serverless use case on which we will first introduce CPU time token AC.
	...
	targets[appTenant][noBurst] = KVCPUTimeAppUtilGoal.Get(&a.settings.SV)
	targets[appTenant][canBurst] = KVCPUTimeAppUtilGoal.Get(&a.settings.SV) + burstDelta
	targets[systemTenant][noBurst] = KVCPUTimeSystemUtilGoal.Get(&a.settings.SV)
	targets[systemTenant][canBurst] = KVCPUTimeSystemUtilGoal.Get(&a.settings.SV) + burstDelta
	...
	// refillRates. The idea here is this: The model itself handles smoothing;
	// once a decision has been made by the model, the allocator should immediately execute
	// on the decision.
	// TODO(josh): This is missing logic to prevent token counts from becoming negative.
	// Also, the above comment needs to be beefed up.
...
// computes refill rates from tokenToCPUTimeMultiplier and the targets parameter.
// targets tracks a target CPU utilization for all buckets in the multi-dimensional
// token buckets owned by cpuTimeTokenGranter. fit returns the refill rates.
func (m *cpuTimeTokenLinearModel) fit(targets targetUtilizations) rates {
	...
	if isLowCPUUtil {
		...
		// get 11, then 6.5, 4.25, 3.125, so we have multiple seconds of under-admission. With 0.8 alpha,
		// it's 5.6, 2.72, 2.144. So much faster. Note that the specific alphas we have here were chosen
