...
// kvCPUTimeSystemUtilGoalRetired keeps the pre-collapse
// admission.cpu_time_tokens.target_util.system_tenant setting registered
// as a no-op so external automation that still references it does not
// see a missing-setting error. Its value is ignored; KVCPUTimeUtilGoal
// is the sole target.
var kvCPUTimeSystemUtilGoalRetired = settings.RegisterFloatSetting(
	settings.SystemOnly,
