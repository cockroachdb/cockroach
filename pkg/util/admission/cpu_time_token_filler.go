...
// Below two are the non-burstable utilization goals. A burstable goal is a fixed 5% (0.05)
// above the corresponding non-burstable goal. See resetInterval for more.
var KVCPUTimeAppUtilGoal = settings.RegisterFloatSetting(
	settings.SystemOnly,
	"admission.cpu_time_tokens.target_util.app_tenant",
	"the target CPU utilization for app tenant work if using the KV CPU time token system", 0.8)

var KVCPUTimeSystemUtilGoal = settings.RegisterFloatSetting(
	settings.SystemOnly,
	"admission.cpu_time_tokens.target_util.system_tenant",
	"the target CPU utilization for system tenant work if using the KV CPU time token system", 0.95)
...
func (a *cpuTimeTokenAllocator) resetInterval() {
	...
	// by cluster settings. A canBurst target adds a fixed 5% to the corresponding noBurst target.
	// The code here is not general with respect to numResourceTiers & numBurstQualifications. This
	// isn't necessary for the Serverless use case on which we will first introduce CPU time token AC.
	...
	targets[appTenant][canBurst] = KVCPUTimeAppUtilGoal.Get(&a.settings.SV) + 0.05
	targets[systemTenant][noBurst] = KVCPUTimeSystemUtilGoal.Get(&a.settings.SV)
	targets[systemTenant][canBurst] = KVCPUTimeSystemUtilGoal.Get(&a.settings.SV) + 0.05
...
// can be fractional.
func (m *cpuTimeTokenLinearModel) computeRefillRates(
