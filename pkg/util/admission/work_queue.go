...
func (q *WorkQueue) SafeFormat(s redact.SafePrinter, _ rune) {
	...
	for _, id := range ids {
		...
		if len(tenant.waitingWorkHeap) > 0 {
			...
			sortedWaitingWorkHead := slices.Clone(tenant.waitingWorkHeap)
			sort.Sort(&sortedWaitingWorkHead)
			s.Printf(" waiting work heap:")
			for i := range sortedWaitingWorkHead {
				var workOrdering string
				if sortedWaitingWorkHead[i].arrivalTimeWorkOrdering == lifoWorkOrdering {
					workOrdering = ", lifo-ordering"
				}
				s.Printf(" [%d: pri: %d, ct: %d, epoch: %d, qt: %d%s]", i,
					sortedWaitingWorkHead[i].priority,
					sortedWaitingWorkHead[i].createTime/int64(time.Millisecond),
					sortedWaitingWorkHead[i].epoch,
					sortedWaitingWorkHead[i].enqueueingTime.UnixNano()/int64(time.Millisecond), workOrdering)
			}
		}
