...
func (q *WorkQueue) refillBurstBucketForGroup(groupID uint64, toAdd int64, capacity int64) {
	...
	// newly created groups and will be overwritten on the next refill.
	q.mu.burstBucketCapacity = capacity
	q.refillBurstBucketLocked(group, toAdd, capacity)
}
...
// (false, i.e. 90%-fullness burst qualification). Existing groups have
// their burst buckets updated; new groups will pick up their flag when
// created. The map is captured by reference; the caller must not modify
// it after calling.
