	...
	if (len(q.mu.groupHeap) == 0 ||
		...
		if q.granter.tryGet(burstQual, info.RequestedCount) {
			...
			if info.ReplicatedWorkInfo.Enabled {
				...
				// useResourceGroup). gKey is therefore tenant-keyed; if a
				// future change extends RM mode to replicated work, this
				// MustMakeTenantID call would mis-attribute an rg ID as a
				// tenant ID.
				if buildutil.CrdbTestBuild && !gKey.isTenant() {
					panic(errors.AssertionFailedf(
						"replicated work admitted with non-tenant gKey: %v", gKey))
				}
...
func (q *WorkQueue) granted(grantChainID grantChainID) int64 {
	...
	if !item.replicated.Enabled {
		...
		if buildutil.CrdbTestBuild && !gKey.isTenant() {
			panic(errors.AssertionFailedf(
				"replicated work granted with non-tenant gKey: %v", gKey))
		}
		q.onAdmittedReplicatedWork.admittedReplicatedWork(
			roachpb.MustMakeTenantID(gKey.id),
			item.priority,
...
func (q *WorkQueue) refillBurstBucketForGroup(gKey groupKey, toAdd int64, capacity int64) {
	if !gKey.isRg() {
		panic(errors.AssertionFailedf("refillBurstBucketForGroup called for non-resource group: %v", gKey))
	}
	q.mu.Lock()
	defer q.mu.Unlock()
	group, ok := q.mu.groups[gKey]
