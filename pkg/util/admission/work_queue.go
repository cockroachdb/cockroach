...
// priority.
func (ps *priorityStates) requestAtPriority(priority admissionpb.Priority) {
...
func (ps *priorityStates) updateDelayLocked(
	priority admissionpb.Priority, delay time.Duration, canceled bool,
) {
	i := 0
	n := len(ps.ps)
	for ; i < n; i++ {
...
func (ps *priorityStates) getFIFOPriorityThresholdAndReset(
	...
	if !foundLowestPriority && ps.lowestPriorityWithRequests != admissionpb.OneAboveHighPri &&
		...
		handlePriorityState(priorityState{
			priority:      admissionpb.Priority(ps.lowestPriorityWithRequests),
			maxQueueDelay: 0,
			admittedCount: 0,
		})
	}
...
type tenantInfo struct {
	...
	// than Priority since the threshold can be > MaxPri.
