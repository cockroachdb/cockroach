...
func (th *tenantHeap) Less(i, j int) bool {
	return cmp.Or(
		...
		// left on the table.
		cmp.Compare(
			(*th)[i].cpuTimeBurstBucket.burstQualification(),
			(*th)[j].cpuTimeBurstBucket.burstQualification(),
		),
		...
		cmp.Compare(
			(*th)[i].used*uint64((*th)[j].weight),
			(*th)[j].used*uint64((*th)[i].weight),
		),
		cmp.Compare((*th)[j].weight, (*th)[i].weight),
		cmp.Compare((*th)[i].id, (*th)[j].id),
	) < 0
}
