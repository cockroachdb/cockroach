...
func (q *WorkQueue) Admit(ctx context.Context, info WorkInfo) (enabled bool, err error) {
	...
	if len(q.mu.tenantHeap) == 0 && !q.knobs.DisableWorkQueueFastPath {
		...
		// concurrent requests.
		//
		// TODO(sumeer): set a proper getterQualification.
		if q.granter.tryGet(burstableQualification /*arbitrary*/, info.RequestedCount) {
			q.metrics.incAdmitted(info.Priority)
			if info.ReplicatedWorkInfo.Enabled {
...
func (q *WorkQueue) hasWaitingRequests() (bool, getterQualification) {
	q.mu.Lock()
	defer q.mu.Unlock()
	// TODO(sumeer): return a proper getterQualification.
	return len(q.mu.tenantHeap) > 0, burstableQualification /*arbitrary*/
}
