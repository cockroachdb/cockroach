...
type WorkQueue struct {
	...
	mu struct {
		...
		// buckets. Updated by refillBurstBuckets. Only used if mode ==
		// usesCPUTimeTokens.
...
func (q *WorkQueue) Admit(ctx context.Context, info WorkInfo) (AdmitResponse, error) {
	...
	if (len(q.mu.tenantHeap) == 0 ||
		...
		(tenant.heapIndex < 0 &&
			tenant.cpuTimeBurstBucket.burstQualification() == canBurst &&
		...
		// concurrent requests.
		if q.granter.tryGet(tenant.cpuTimeBurstBucket.burstQualification(), info.RequestedCount) {
			q.metrics.incAdmitted(info.Priority)
			if info.ReplicatedWorkInfo.Enabled {
...
func (q *WorkQueue) SafeFormat(s redact.SafePrinter, _ rune) {
	...
	if q.mode == usesCPUTimeTokens && len(ids) > 0 {
		...
		for i, id := range ids {
			...
			tenant := q.mu.tenants[id]
			s.Printf("t%d=%s", id, tenant.cpuTimeBurstBucket.String())
		}
	}
}
...
func (th *tenantHeap) Less(i, j int) bool {
	...
	// left on the table. Sorting on used as per below will approximately
	// have the same effect as sorting on burstQualification, but the
	// source of truth of whether a tenant can burst is cpuTimeBurstBucket's
