...
func (th *tenantHeap) Less(i, j int) bool {
	...
	// left on the table.
	//
	// A reader may wonder if sorting on used would have the same effect.
	// It is indeed similar, but it is not the same -- for example, used
	// is reset every 1s, thus right after a reset it can fall out of sync
	// with cpuTimeBurstBucket's burstQualification method. The source of
	// truth for whether a tenant can burst is cpuTimeBurstBucket's
