...
// WorkQueue is mostly agnostic of what kind of resource is being managed.
// With that said, there are a few different kind of resource types. If the
...
// AdmitResponse.Err!=nil, the request was not admitted, potentially
// due to the deadline being exceeded. The enabled return value is
// relevant when Err=nil, and represents whether admission control is
// enabled. AdmittedWorkDone must be called iff Enabled=true && Err!=nil,
// and the WorkKind for this queue is KVWork.
func (q *WorkQueue) Admit(ctx context.Context, info WorkInfo) AdmitResponse {
	if !info.ReplicatedWorkInfo.Enabled {
		...
		if enabledSetting != nil && !enabledSetting.Get(&q.settings.SV) {
			...
			return AdmitResponse{Enabled: false, Err: nil}
		}
	}
	...
	case <-ctx.Done():
		...
		if deadline, hasDeadline := ctx.Deadline(); hasDeadline {
			...
			admitResponse.Enabled = true
			admitResponse.Err = errors.Newf("deadline expired while waiting in queue: %s, pri: %s, deadline: %v, start: %v, dur: %v",
		...
		admitResponse.Enabled = true
		admitResponse.Err = errors.Newf("context canceled while waiting in queue: %s, pri: %s, start: %v, dur: %v",
...
func (q *WorkQueue) AdmittedWorkDone(resp AdmitResponse, cpuTime time.Duration) {
	...
	// adjustTenantUsed adjusts tenant.used. tenant.used tracks usage of resources
