...
func TestGranterBasic(t *testing.T) {
	...
	datadriven.RunTest(t, testutils.TestDataPath(t, "granter"), func(t *testing.T, d *datadriven.TestData) string {
		...
		case "init-store-grant-coordinator":
			...
			kvStoreGranter := coord.mu.granters[KVWork].(*kvStoreTokenGranter)
		...
		case "set-tokens":
			...
			coord.mu.granters[KVWork].(*kvStoreTokenGranter).setAvailableTokens(int64(ioTokens), int64(elasticTokens))
