	...
	tests := []struct {
		...
		},
		{
			...
			prev: ioLoadListenerState{
				...
				smoothedIntL0BytesIngestFraction:     0.3,
...
func TestBadIOLoadListenerStats(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		for j := 0; j < adjustmentInterval; j++ {
			...
			require.LessOrEqual(t, float64(0), ioll.smoothedIntL0BytesIngestFraction)
