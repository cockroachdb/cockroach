...
func (tr *testRequester) granted(grantChainNum uint64) bool {
	fmt.Fprintf(tr.buf, "%s: granted in chain %d, and returning %t\n", workKindString(tr.workKind),
		grantChainNum, !tr.returnFalseFromGranted)
	tr.grantChainNum = grantChainNum
...
func (tr *testRequester) continueGrantChain() {
	...
	tr.granter.continueGrantChain(tr.grantChainNum)
}
