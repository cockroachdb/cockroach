...
type kvStoreTokenGranter struct {
	...
	// - Most of the stores in a cluster are healthy, ignoring snapshots, and
	//   are not being subject to any store token limiting. Adding 1 sublevel
	//   and 5 files once will not immediately push them over the admission
	//   control thresholds.
	...
	//   user-facing traffic.
	// - If the token count is consumed by a 512MB snapshot, this could prevent
	//   user-facing traffic from making progress for some time. Even though we
	//   can partially address this giving lower priority to snapshot work, one
	//   can still get unlucky if there is an instant where there is no waiting
	...
	//  start: snapshot-tokens=25B, regular-tokens=375MB
	//  One 500MB snapshot ingestion into L0, and regular work 300MB
