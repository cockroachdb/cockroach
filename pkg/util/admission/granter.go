...
// Merge combines default options with override to update values that aren't
// the Go default values.
func Merge(defaults Options, override *Options) Options {
	o := defaults
	...
	if override.DisableSkipEnforcement {
		o.DisableSkipEnforcement = true
	}
	return o
}
...
func NewGrantCoordinators(
	...
	coord := &GrantCoordinator{
		...
		useGrantChains:         true,
		disableSkipEnforcement: opts.DisableSkipEnforcement,
		numProcs:               1,
		grantChainID:           1,
	}
...
func (coord *GrantCoordinator) CPULoad(runnable int, procs int, samplePeriod time.Duration) {
	...
	if coord.granters[KVWork] != nil {
		...
		if !coord.disableSkipEnforcement {
