...
func (coord *GrantCoordinator) returnGrant(workKind WorkKind) {
	...
	if coord.grantChainActive {
		if coord.grantChainIndex > workKind && coord.granters[workKind].hasWaitingRequests() {
...
func (coord *GrantCoordinator) tryGrant() {
	...
	for ; coord.grantChainIndex < numWorkKinds; coord.grantChainIndex++ {
		localDone := false
		for coord.granters[coord.grantChainIndex].hasWaitingRequests() && !localDone {
			res := coord.granters[coord.grantChainIndex].tryGetLocked()
			switch res {
			case grantSuccess:
				if !coord.granters[coord.grantChainIndex].granted() {
					coord.granters[coord.grantChainIndex].returnGrantLocked()
				} else {
...
// MetricsStruct implements the metric.Struct interface.
