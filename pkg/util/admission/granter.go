...
func (sg *slotGranter) tryGetLocked() grantResult {
	...
	if sg.usedSlots < sg.totalSlots {
		...
		if sg.usedSlots == sg.totalSlots {
			log.Infof(context.Background(), "granter: slots exhausted")
		}
		return grantSuccess
	}
...
func (tg *tokenGranter) tryGetLocked() grantResult {
	...
	if tg.availableBurstTokens > 0 {
		...
		if tg.availableBurstTokens == 0 {
			log.Infof(context.Background(), "granter: tokens exhausted")
		}
		return grantSuccess
	}
	return grantFailLocal
		...
		for req.hasWaitingRequests() && !localDone {
			...
			case grantSuccess:
				...
				if !req.granted(chainNum) {
					log.Infof(context.Background(), "granter: ??? granted returned false")
			...
			case grantFailDueToSharedResource:
				if !startingChain {
					log.Infof(context.Background(), "granter: grant chain died at index %d, burst: %d",
						coord.grantChainIndex, grantBurstCount)
				} else if grantBurstCount > 0 {
					log.Infof(context.Background(), "granter: index %d, burst %d", coord.grantChainIndex,
						grantBurstCount)
				}
				break OuterLoop
			case grantFailLocal:
