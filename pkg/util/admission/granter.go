...
// CockroachDB nodes, both for regular clusters and serverless. In the
// latter, both KV and SQL nodes are expected to use these abstractions.
//
...
//   latency of the former. Additionally, for serverless KV nodes, the
...
//   will differ for regular clusters and serverless-KV nodes and
//   serverless-SQL nodes.
//
// The interfaces involved:
// - requester: handles all requests for a particular WorkKind. Implemented
//   by AdmissionQueue. The requester implementation is responsible for
//   controlling the admission order within a WorkKind based on tenant
//   fairness, importance of work etc.
...
//   The kvSlotAdjuster is the concrete implementation, except for serverless-SQL
...
// WorkKind will interact with a granter.
...
// indirect backpressure. However, we do not have control over the CPU
// scheduler, so we cannot preempt work with widely different cpu consumption.
// Additionally, (non-preemptible) memory is also a shared resource, and we
// wouldn't want to have partially done KVWork not finish, due to preemption
// in the CPU scheduler, since it can be holding significant amounts of memory
// (e.g. in scans).
//
...
type GrantCoordinator struct {
	...
	granters [SQLStatementRootStartWork + 1]granterWithLockedCalls
	...
	queues               [SQLStatementRootStartWork + 1]*AdmissionQueue
...
func NewGrantCoordinator(opts Options) (*GrantCoordinator, []metric.Struct) {
	...
	coord.queues[KVWork] = MakeAdmissionQueue(KVWork, sg, false, true)
	...
	coord.queues[SQLKVResponseWork] = MakeAdmissionQueue(SQLKVResponseWork, tg, true, false)
	...
	coord.queues[SQLSQLResponseWork] = MakeAdmissionQueue(SQLSQLResponseWork, tg, true, false)
	...
	coord.queues[SQLStatementLeafStartWork] =
		MakeAdmissionQueue(SQLStatementLeafStartWork, sg, false, false)
	...
	coord.queues[SQLStatementRootStartWork] =
		MakeAdmissionQueue(SQLStatementRootStartWork, sg, false, false)
