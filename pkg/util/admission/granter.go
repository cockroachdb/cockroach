...
type GrantCoordinator struct {
	...
	oldestAttemptToTerminateGrantChain time.Time
}
...
func (coord *GrantCoordinator) tryTerminateGrantChain() bool {
	now := timeutil.Now()
	if coord.oldestAttemptToTerminateGrantChain.IsZero() {
		coord.oldestAttemptToTerminateGrantChain = now
	}
	if delayForGrantChainTermination > 0 &&
		now.Sub(coord.oldestAttemptToTerminateGrantChain) < delayForGrantChainTermination {
		return false
	}
	...
	coord.oldestAttemptToTerminateGrantChain = time.Time{}
	return true
}
