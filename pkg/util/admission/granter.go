...
// setModerateSlotsClamp is used in testing to force a value for moderateSlotsClamp.
func (sg *slotGranter) setModerateSlotsClamp(val int) {
	sg.moderateSlotsClamp = val
}
...
func NewGrantCoordinators(
	...
	kvg := &slotGranter{
		...
		runnableAlpha:          opts.RunnableAverageAlpha,
		...
		case KVWork:
			...
			case *slotGranter:
				s.Printf(
					...
					g.usedSlots, g.totalHighLoadSlots, g.totalModerateLoadSlots, g.moderateSlotsClamp)
...
func (kvsa *kvSlotAdjuster) CPULoad(runnable int, procs int, _ time.Duration) {
	...
	// moderate slots in an additive increase/additive decrease manner based on
	// the current CPU load, and we also use a historic average of the runnable
	// count to clamp down on the moderate slots. It becomes difficult to test
	// these mechanisms separately if they tune the same value. Instead, we
	// store two values, the granter.totalModerateSlots, granter.moderateSlotsClamp.
	// The granter.moderateSlotsClamp clamps down on the granter.totalModerateSlots
	...
	// which is fine.
	if kvsa.granter.totalModerateLoadSlots > kvsa.granter.moderateSlotsClamp {
		kvsa.granter.totalModerateLoadSlots = kvsa.granter.moderateSlotsClamp
	}
	if kvsa.granter.totalModerateLoadSlots < 0 {
		kvsa.granter.totalModerateLoadSlots = 0
	}

	kvsa.granter.runnableEWMA = kvsa.granter.runnableEWMA*kvsa.granter.runnableAlpha +
		float64(runnable)*(1-kvsa.granter.runnableAlpha)
	...
	// Take the runnable go routines into account to clamp down on the available
	// moderate load slots. There are cases where work not accounted for by
	// admission control will cause runnable goroutine spikes. In such situations,
	// we don't want to grant soft slots.
	//
	// Note that dividing (threshold * procs) by 2 is somewhat arbitrary. It resembles
	// the moderately underloaded case.
	kvsa.granter.moderateSlotsClamp = int(float64(threshold*procs)/2 - kvsa.granter.runnableEWMA)
