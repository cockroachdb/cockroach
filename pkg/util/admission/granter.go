...
//
// The kvStoreTokenChildGranter delegate the actual interaction to
...
// apply to all the elastic incoming bytes into th LSM.
...
func (sg *kvStoreTokenGranter) storeWriteDone(
	...
	actualTokens := sg.writeLM.applyLinearModel(doneInfo.WriteBytes) +
		sg.ingestedLM.applyLinearModel(doneInfo.IngestedBytes)
	additionalTokensNeeded := actualTokens - originalTokens
	sg.subtractTokens(additionalTokensNeeded, false)
	actualDiskBWTokens := sg.diskBWLM.applyLinearModel(doneInfo.WriteBytes + doneInfo.IngestedBytes)
