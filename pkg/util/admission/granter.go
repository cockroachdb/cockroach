...
func (sg *kvStoreTokenGranter) tryGetLocked(count int64, demuxHandle int8) grantResult {
	...
	case admissionpb.RegularStoreWorkType:
		if sg.coordMu.availableIOTokens[admissionpb.RegularWorkClass] > 0 {
			...
			sg.coordMu.diskTokensUsed[admissionpb.RegularWorkClass].writeByteTokens += adjustedDiskWriteTokens
			return grantSuccess
		}
	case admissionpb.ElasticStoreWorkType:
		if sg.coordMu.diskTokensAvailable.writeByteTokens > 0 &&
			...
			sg.coordMu.diskTokensAvailable.writeByteTokens -= adjustedDiskWriteTokens
			sg.coordMu.diskTokensUsed[admissionpb.ElasticWorkClass].writeByteTokens += adjustedDiskWriteTokens
			return grantSuccess
		}
	case admissionpb.SnapshotIngestStoreWorkType:
		// Snapshot ingests do not go into L0 so we only subject them to writeByteTokens.
		if sg.coordMu.diskTokensAvailable.writeByteTokens > 0 {
			sg.coordMu.diskTokensAvailable.writeByteTokens -= count
			// TODO(aaditya): maybe consider separating the metrics here by admissionpb.StoreWorkType
			sg.coordMu.diskTokensUsed[admissionpb.ElasticWorkClass].writeByteTokens += count
			return grantSuccess
		}
	}
	return grantFailLocal
...
func (sg *kvStoreTokenGranter) tryGrantLocked(grantChainID grantChainID) grantResult {
	...
	// TODO(aaditya): add some commentary about work types and priorities.
