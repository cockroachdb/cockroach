...
//gcassert:inline
func (sg *slotGranter) setTotalSlotsLocked(totalHighLoadSlots int) {
	// Mid-stack inlining.
	if totalHighLoadSlots == sg.totalSlots {
		return
	}
...
func (sg *slotGranter) setTotalHighLoadSlotsLockedInternal(totalHighLoadSlots int) {
	if sg.slotsExhaustedDurationMetric != nil {
		if totalHighLoadSlots > sg.totalSlots {
			if sg.totalSlots <= sg.usedSlots && totalHighLoadSlots > sg.usedSlots {
				now := timeutil.Now()
				exhaustedMicros := now.Sub(sg.exhaustedStart).Microseconds()
				sg.slotsExhaustedDurationMetric.Inc(exhaustedMicros)
			}
		} else if totalHighLoadSlots < sg.totalSlots {
			if sg.totalSlots > sg.usedSlots && totalHighLoadSlots <= sg.usedSlots {
				sg.exhaustedStart = timeutil.Now()
			}
		}
	}
	sg.totalSlots = totalHighLoadSlots
}
