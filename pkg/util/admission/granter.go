...
func (sg *kvGranter) setAvailableIOTokensLocked(tokens int64) {
	alreadyEnabled := sg.ioTokensEnabled
	sg.ioTokensEnabled = true
	if alreadyEnabled && sg.availableIOTokens <= 0 && tokens > 0 {
...
func (coord *GrantCoordinator) SetPebbleMetricsProvider(pmp PebbleMetricsProvider) {
	...
	coord.ioLoadListener.start(func(d time.Duration) ticker {
...
type newTickerFunc func(time.Duration) ticker
