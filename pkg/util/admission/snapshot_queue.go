...
func (s *SnapshotQueue) Admit(
	...
	//
	// Reminder: there is a race here where a call to hasWaitingRequests before
	// the mutex is acquired will not see the waiting request. We ignore this
	// case since hasWaitingRequests is called periodically at a high enough
	// frequency when tokens are limited.
	shouldRelease := true
