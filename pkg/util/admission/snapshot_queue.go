...
func (s *SnapshotQueue) hasWaitingRequests() (bool, getterQualification) {
	s.mu.Lock()
	defer s.mu.Unlock()
	return !s.mu.q.Empty(), burstableQualification /*arbitrary*/
}
...
func (s *SnapshotQueue) Admit(ctx context.Context, count int64) error {
	...
	if s.snapshotGranter.tryGet(burstableQualification /*arbitrary*/, count) {
		return nil
	}
