...
//
// The requester and granter call into each other. To prevent deadlock, any
// mutex in granter is ordered before any mutex in requester. Therefore, a
// requester must not hold its own mutex when calling into requester. Of
// course, a granter could choose to release its own mutex before calling into
// requester, but it is not necessary for deadlock prevention.
