...
type granterWithIOTokens interface {
	// setAvailableTokens bounds the available tokens that can be granted to the
	// value provided in the ioTokens parameter. This is not a tight bound when
	// the callee has negative available tokens, due to the use of
	// granter.tookWithoutPermission, since in that the case the callee increments
	// that negative value with the value provided by tokens. This method needs to
	// be called periodically. The return value is the number of used tokens in
	// the interval since the prior call to this method. Note that tokensUsed can
	// be negative, though that will be rare, since it is possible for tokens to
	// be returned. elasticDiskBandwidthTokens are based on disk bandwidth being a
	// bottleneck resource.
	...
	getDiskTokensUsedAndReset() [numWorkClasses]int64
