...
// getterQualification is an optional behavior of certain WorkQueues (which
...
type getterQualification uint8

const (
	burstableQualification getterQualification = iota
	notBurstableQualification
	numGetterQualifications
)
...
type requester interface {
	...
	// importance getter.
	hasWaitingRequests() (bool, getterQualification)
...
type granter interface {
	...
	// getterQualification identifies the qualification of the getter, which is
	// useful for certain granters.
	//
	// REQUIRES: count > 0. count == 1 for slots.
	tryGet(getterQual getterQualification, count int64) (granted bool)
