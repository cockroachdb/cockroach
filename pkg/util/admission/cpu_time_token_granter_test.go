...
func TestCPUTimeTokenGranter(t *testing.T) {
	...
	flushAndReset := func(init bool) string {
		...
		if init {
			fmt.Fprint(&buf, "requester: "+req.String()+"\n")
		}
		str := buf.String()
		buf.Reset()
		return str
	...
	req.returnValueFromHasWaitingRequests = noBurst
	...
	datadriven.RunTest(t, datapathutils.TestDataPath(t, "cpu_time_token_granter"), func(t *testing.T, d *datadriven.TestData) string {
		...
		case "init":
			...
			if d.HasArg("burst") {
				var n int64
				d.ScanArgs(t, "burst", &n)
				granter.mu.buckets[canBurst].tokens = n
			}
			if d.HasArg("noburst") {
				var n int64
				d.ScanArgs(t, "noburst", &n)
				granter.mu.buckets[noBurst].tokens = n
			}
			if d.HasArg("waiter") {
				var n int64
				d.ScanArgs(t, "waiter", &n)
				req.waitingRequests = true
				req.returnValueFromGranted = n
			} else {
				req.waitingRequests = false
				req.returnValueFromGranted = 0
			}
			if d.HasArg("burstwaiter") {
				if !d.HasArg("waiter") {
					panic("must set waiter")
				}
				req.returnValueFromHasWaitingRequests = canBurst
			} else {
				req.returnValueFromHasWaitingRequests = noBurst
			}
		...
		case "took-without-permission":
			...
			req.tookWithoutPermission(int64(v))
			return flushAndReset(false /* init */)
		...
		// continueGrantChain is a NOP. Still, it will be called in
		// production. So best to test it doesn't panic, or similar.
		case "continue-grant-chain":
			req.continueGrantChain()
			return flushAndReset(false /* init */)
