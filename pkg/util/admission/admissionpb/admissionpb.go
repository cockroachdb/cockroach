...
const (
	...
	// LockingHighPri is for user high priority transactions that are acquiring
	// locks.
	LockingHighPri WorkPriority = 100
...
var WorkPriorityDict = map[WorkPriority]string{
	...
	// This ought to be called "locking-high-pri", but we retain the old name
	// for continuity with metrics in older versions.
	LockingHighPri: "locking-pri",
	HighPri:        "high-pri",
}
...
// NormalPri maps to LockingNormalPri, and UserHighPri maps to LockingHighPri.
// Say users are running at these different priorities in different parts of
// the key space, say key-low, key-normal, key-high, then even after the
// mapping, a txn holding locks (or resolving intents) in key-low will have
// lower priority (BulkNormalPri) than the non-adjusted priority in key-normal
// (NormalPri). The same holds true for txn holding locks in key-normal, since
// LockingNormalPri is lower priority than UserHighPri.
//
