...
func (m *workMap) setAdmitted(id int, handle StoreWorkHandle, resp AdmitResponse) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func TestWorkQueueBasic(t *testing.T) {
	...
	datadriven.Walk(t, datapathutils.TestDataPath(t, "work_queue_basic"),
				...
				func(t *testing.T, d *datadriven.TestData) string {
					...
					case "init":
						...
						if d.HasArg("cpu-time-token") {
							q.isCPUTimeTokenQueue = true
							q.setTenantCPUTokensBurstLimit(0, true)
						}
						tg.r = q
						wrkMap.resetMap()
						return ""
					...
					case "admit":
						...
						go func(ctx context.Context, info WorkInfo, id int) {
							...
							if resp.Err != nil {
								...
								wrkMap.setAdmitted(id, StoreWorkHandle{}, resp)
							}
						}(ctx, workInfo, id)
					...
					case "set-tenant-cpu-burst-limit":
						var tokens int64
						d.ScanArgs(t, "tokens", &tokens)
						q.setTenantCPUTokensBurstLimit(tokens, true)
						return q.String()
			...
			case "admit":
				...
				go func(ctx context.Context, info StoreWriteWorkInfo, id int) {
					...
					if err != nil {
						...
						wrkMap.setAdmitted(id, handle, AdmitResponse{})
					}
				}(ctx, workInfo, id)
				// Need deterministic output, and this is racing with the goroutine
				// which is trying to get admitted. Retry to let it get scheduled.
