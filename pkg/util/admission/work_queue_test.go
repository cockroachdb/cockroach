...
func TestCPUTimeTokenWorkQueue(t *testing.T) {
	...
	datadriven.RunTest(t, datapathutils.TestDataPath(t, "cpu_time_token_work_queue"),
		func(t *testing.T, d *datadriven.TestData) string {
			...
			case "set-burst-qual":
				tenant := scanTenantID(t, d)
				var qual burstQualification
				if d.HasArg("no-burst") {
					qual = noBurst
				} else if d.HasArg("can-burst") {
					qual = canBurst
				} else {
					t.Error("must set no-burst or can-burst")
				}
				func() {
					q.mu.Lock()
					defer q.mu.Unlock()
					q.mu.tenants[tenant.ToUint64()].cpuTimeBurstBucket.hardcodedRetValue = &qual
				}()
				return ""
...
func TestCPUTimeTokenBurst(t *testing.T) {
	...
	// not have a capacity set yet, and so no tenants are allowed to burst.
	...
	// buckets start empty, so on the first call to admit, it also cannot
	// burst.
	info1 := WorkInfo{
		TenantID:       roachpb.MustMakeTenantID(1),
		RequestedCount: 1,
	...
	// Fill tenant 2's burst bucket to the top with 100 tokens. Since tenant 1's
	// bucket is mostly full, the cap on tokens is also tested here. At the end
	// of this call, both tenant's buckets should be full.
	q.refillBurstBuckets(100, 100)
