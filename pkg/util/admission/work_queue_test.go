	...
	datadriven.RunTest(t, testutils.TestDataPath(t, "work_queue"),
		func(t *testing.T, d *datadriven.TestData) string {
			...
			case "admit":
				...
				workInfo := WorkInfo{
					TenantID:        tenant,
					Priority:        admissionpb.Priority(priority),
	...
	datadriven.RunTest(t, "testdata/priority_states",
		func(t *testing.T, d *datadriven.TestData) string {
			...
			case "request-received":
				var priority int
				d.ScanArgs(t, "priority", &priority)
				ps.requestAtPriority(admissionpb.Priority(priority))
				return printFunc()
			...
			case "update":
				...
				ps.updateDelayLocked(admissionpb.Priority(priority), time.Duration(delayMillis)*time.Millisecond,
					canceled)
				return printFunc()
