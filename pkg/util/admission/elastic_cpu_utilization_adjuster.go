...
func (e *elasticCPUGranter) setTargetUtilization(targetUtilization float64) {
	...
	log.Infof(context.Background(), "elastic cpu granter refill rate = %0.4f cpu seconds per second (utilization across %d procs = %0.2f%%)",
...
func (e *elasticCPUGranter) getObservedUtilization() float64 {
	...
	totalElasticCPUTime := e.getTargetUtilization() * float64(int64(runtime.GOMAXPROCS(0))*time.Second.Nanoseconds())
	return e.getTargetUtilization() * float64(int64(totalElasticCPUTime)-availableElasticCPUTime.Nanoseconds()) / totalElasticCPUTime
}
