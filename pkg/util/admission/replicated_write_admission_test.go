...
//     per-tenant, physical admission statistics, etc).
//
// TODO(irfansharif): Add tests for epoch-LIFO and tenant weights.
			...
			case "init":
				...
				knobs := &TestingKnobs{
					AdmittedReplicatedWorkInterceptor: func(
						...
						buf.printf("admitted (tenant=t%d pri=%s create-time=%s size=%s range=r%s origin=n%s log-position=%s ingested=%t)",
							tenantID.ToUint64(), pri, timeutil.FromUnixNanos(createTime).Sub(tzero),
							printTrimmedBytes(requestedTokens), rwi.RangeID, rwi.Origin, rwi.LogPosition, rwi.Ingested)
					},
				}
...
func printWorkQueue(q *WorkQueue) string {
	...
	for _, id := range ids {
		...
		buf.WriteString(fmt.Sprintf("\n tenant=t%d used-tokens=%s",
			tenant.id, printTrimmedBytes(int64(tenant.used))))
		if len(tenant.waitingWorkHeap) > 0 {
			...
			for i := range tenant.waitingWorkHeap {
				...
				buf.WriteString(fmt.Sprintf(" [%d: pri=%s create-time=%s size=%s range=r%d origin=n%d log-position=%s ingested=%t]", i,
					w.priority,
...
func newTestReplicatedWriteGranter(t *testing.T, wc admissionpb.WorkClass, buf *builderWithMu) *testReplicatedWriteGranter {
