...
type cpuTimeBurstBucket struct {
	...
	disabled bool
	// Only used in tests.
	hardcodedRetValue *burstQualification
}

func (m *cpuTimeBurstBucket) init(capacity int64, disabled bool) {
	// The bucket is inited empty. This is a conservative choice. If the
	// tenant doesn't use many tokens, it will fill up quickly as calls
	// to refill are made.
	// TODO(josh): Need to think over this more, before merge. Sumeer, if
	// you have thoughts, lmk.
	*m = cpuTimeBurstBucket{tokens: 0, capacity: capacity, disabled: disabled}
}
...
func (m *cpuTimeBurstBucket) burstQualification() burstQualification {
	if m.hardcodedRetValue != nil {
		return *m.hardcodedRetValue
	}
	if m.disabled {
		return noBurst
	}
	// At CRDB startup time, the capacity that is passed into
	// cpuTimeBurstBucket.init will be zero, until 1ms passes, and the first
	// call to refillBurstBuckets is made by cpuTimeTokenAllocator. For this
	// 1ms period, we don't allow any tenants to burst.
	if m.capacity == 0 {
		return noBurst
	}
...
func (m *cpuTimeBurstBucket) String() string {
	...
	return fmt.Sprintf("fullness=%.1f%% tokens=%d capacity=%d qual=%s",
