...
// cpuTimeTokenGranter implements granter. It uses token buckets to limit
// CPU usage. There is one token bucket per burstQualification. Requests
// are only admitted (tryGet returns true) if the bucket for the request's
// burstQualification has positive tokens. Before a request is admitted,
// tokens are deducted from all buckets, not just the one checked. This
// enables a priority hierarchy where burstable work can use more CPU than
// non-burstable work.
//
// For example, on an 8 vCPU machine with an 80% target:
//
//   - Burstable work -> 6.8 seconds of CPU time per second
//   - Non-burstable work -> 6.4 seconds of CPU time per second
//
// A request for 5s of burstable work would be admitted immediately
// (burstable bucket is positive). It deducts from both buckets,
// resulting in a balance of (1.8, 1.4). Non-burstable work continues
// to be admissible. If the burstable bucket empties first, only
// non-burstable work stalls; burstable work continues until its own
// bucket is drained.
//
// Note that cpuTimeTokenGranter does not handle replenishing the
// buckets; see cpuTimeTokenFiller.
//
...
type cpuTimeTokenGranter struct {
	...
	mu         struct {
		...
		//
		// Since admission deducts from all buckets, this invariant holds
...
func newCPUTimeTokenGranter(
	...
	for qual := burstQualification(0); qual < numBurstQualifications; qual++ {
		...
		g.mu.buckets[qual].exhaustedDuration =
			metrics.ExhaustedDurationNanos[perBucketIdx(qual)]
	}
	return g
}
...
func (stg *cpuTimeTokenGranter) SafeFormat(s redact.SafePrinter, _ rune) {
	...
	row[0] = "tokens"
	for gk := canBurst; gk < numBurstQualifications; gk++ {
		row[gk+1] = fmt.Sprint(stg.mu.buckets[gk].tokens)
	}
	tw.Append(row[:])
	tw.Render()
