...
// the burst budgets stored in burstBudget. That is, if a bucket is already
// at its burst budget, no more tokens will be added. delta is always positive,
...
func (stg *cpuTimeTokenGranter) refill(
	...
	for wc := range stg.mu.buckets {
		for kind := range stg.mu.buckets[wc] {
			...
			if tokens > burstBudget[wc][kind] {
				tokens = burstBudget[wc][kind]
			}
