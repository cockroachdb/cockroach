...
func TestBatch(t *testing.T) {
	const totalBatchSize = int64(1000)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			retryCount := int64(0)
			batch := Batch{
				Do: func(context.Context, int64, int64) error {
				...
				OnRetry: func(error, int64) error {
					retryCount++
					return nil
				},
				IsRetriableError: isRetriableError,
			}
			err := batch.Start(ctx, totalBatchSize)
			if tc.expectedFinalErr == nil {
				require.NoError(t, err)
				require.Equal(t, batch.processed, totalBatchSize)
			} else {
				require.True(t, isRetriableError(tc.expectedFinalErr) == isRetriableError(err))
				require.Equal(t, batch.processed, int64(0))
			}
