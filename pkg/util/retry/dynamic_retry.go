	...
	for d.retry = StartWithCtx(ctx, initialPolicy); d.retry.Next(); {
		...
		case newState:
			...
			if d.hooks.OnNewState != nil {
				if hookErr := d.hooks.OnNewState(&d.DynamicRetryState, result, err); hookErr != nil {
					return result, hookErr
				}
			}
			continue
...
func (d *DynamicRetryState) State() string {
	return d.currState
}
...
// CurrentAttempt returns the number of the last attempt across all states
// (0-based index).
