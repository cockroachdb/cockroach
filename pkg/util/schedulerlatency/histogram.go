...
// scheme. It also trims the bucket range to the specified min and max values;
// everything outside the range is merged into (-Inf, ..] and [.., +Inf)
// buckets. The following example shows how it works, lifted from
...
func reBucketExpAndTrim(buckets []float64, base, min, max float64) []float64 {
	...
	for i := range b {
		...
		// We're looking at the boundary after the first one that's crossed the
		// max limit. Since we expect recordings at the max value, we don't want
		// that bucket to end at +Inf, so we merge the bucket after.
		b[i] = math.Inf(1)
		b = b[:i+1]
		break
	}
...
func reBucketExp(buckets []float64, base float64) []float64 {
	...
	// We may see -Inf here, in which case, add it and skip it since we risk
	// producing NaNs otherwise.
	//
	// We need to preserve -Inf values to maintain runtime/metrics conventions.
	// We'll strip it out later.
