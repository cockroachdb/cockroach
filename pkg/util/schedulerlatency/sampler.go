...
var samplePercentile = settings.RegisterFloatSetting(
	...
	0.99, // p99
	func(p float64) error {
		if p < 0 || p > 1 {
			return fmt.Errorf("p expected to be in range [0,1], got %s", p)
		}
		return nil
	},
)
...
func StartSampler(
	...
	return stopper.RunAsyncTask(ctx, "scheduler-latency-sampler", func(ctx context.Context) {
		...
		s := newSampler(settingsValuesMu.period, settingsValuesMu.duration, settingsValuesMu.percentile)
...
func newSampler(period, duration time.Duration, p float64) *sampler {
	s := &sampler{}
...
func (s *sampler) sampleOnTickAndInvokeCallbacks(period time.Duration) {
	...
	for i := range cbs {
		cbs[i].cb(schedulingLatency, period)
	}
}
