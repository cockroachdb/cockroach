...
func percentile(h *metrics.Float64Histogram, p float64) float64 {
	...
	//
	// Goal: we want the `p * total`-th value from the histogram.
	...
	//			subtract left-bound cumulative to get the subset rank.
	//
	// Step 3: Apply the approximation to the bucket's lower boundary.
	//    - Calculate the subset_percentile.
	//				Since,
	//								rank = percentile * count
	//				we get,
	//								percentile = rank / count
	//        scoping this to the subset dataset,
	//       					subset_percentile = subset_rank / subset_count
	//    - The value within the bucket will be:
	//   		          bucket_size * subset_percentile
	//        where,
	//       					bucket_size = bucket_start - bucket_end
	//        Thus, our target value will be given by,
	//                bucket_start + (bucket_start - bucket_end) * subset_percentile
	//
