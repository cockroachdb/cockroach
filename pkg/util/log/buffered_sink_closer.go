...
// RegisterBufferSink notifies the BufferedSinkCloser of the existence
...
func (l *BufferedSinkCloser) RegisterBufferSink(bs *bufferedSink) context.Context {
	l.mu.Lock()
	defer l.mu.Unlock()
	...
	l.wg.Add(1)
	// We will be calling cancel on the parent l.ctx in Close, so discard the cancellation function.
	ctx, _ := context.WithCancel(l.ctx)
	return ctx
}

// BufferSinkDone notifies the BufferedSinkCloser that one of the buffered
// log sinks registered via RegisterBufferSink has finished processing
// & has terminated.
func (l *BufferedSinkCloser) BufferSinkDone(bs *bufferedSink) {
	l.mu.Lock()
	defer l.mu.Unlock()
...
// failing to call BufferSinkDone causing the process to indefinitely hang.
func (l *BufferedSinkCloser) Close() {
	l.cancel()
