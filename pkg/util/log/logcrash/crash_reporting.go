...
func RecoverAndReportNonfatalPanic(ctx context.Context, sv *settings.Values) {
	if r := recover(); r != nil {
		reportNonfatalPanic(ctx, sv, r, depthForRecoverAndReportPanic)
	}
}

// reportNonfatalPanic is an alternative to ReportPanic that
// does not re-panic in release builds. The argument r is
