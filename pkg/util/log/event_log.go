...
func RegisterEventLogWriter(
	...
	if eventLogWriterRegistry == nil {
		...
		Error(ctx, "Structured event writer registry not set, cannot register writer")
		return
	}
...
func RemoveEventLogWriter(serverId serverident.ServerIdentifier) {
	if eventLogWriterRegistry == nil {
		...
		Errorf(context.Background(), "Structured event writer registry not set, cannot remove writer")
		return
	}
...
func WithFatal() StructuredEventSettingsFunc {
	return func(o EventLogSettings) EventLogSettings {
