...
func getTestConfig(fileDir *string) (testConfig logconfig.Config, err error) {
	...
	if fileDir == nil {
		...
		testConfig.Sinks.Stderr.Channels = logconfig.SelectChannels(logconfig.SelectAllChannels()...)
		...
		testConfig.Sinks.FileGroups = map[string]*logconfig.FileSinkConfig{
			"storage": {
				Channels: logconfig.FilterChannels(channel.STORAGE),
			},
			"health": {
				Channels: logconfig.FilterChannels(channel.HEALTH),
			},
...
func (l *TestLogScope) SetupSingleFileLogging(
	...
	cfg.Sinks.FileGroups = map[string]*logconfig.FileSinkConfig{
		"default": {
			Channels: logconfig.FilterChannels(logconfig.SelectAllChannels()...)},
	}
