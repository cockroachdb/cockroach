...
var _ log.SEventSinkRegistry = &SinkRegistry{}

func (e *SinkRegistry) RegisterSink(
	ctx context.Context, ambientContext log.AmbientContext, sink log.SEventSink,
) {
	e.mu.Lock()
	defer e.mu.Unlock()
	e.mu.serverSinks[ambientContext.ServerIDs] = sink
}

func (e *SinkRegistry) DeregisterSink(ambientContext log.AmbientContext) {
	e.mu.Lock()
	defer e.mu.Unlock()
	delete(e.mu.serverSinks, ambientContext.ServerIDs)
}

func (e *SinkRegistry) GetSink(ambientContext log.AmbientContext) (log.SEventSink, bool) {
	e.mu.Lock()
	defer e.mu.Unlock()
	sink, ok := e.mu.serverSinks[ambientContext.ServerIDs]
	return sink, ok
}
...
var _ log.SEventSink = &EventTableSink{}
...
func (e *EventTableSink) WriteEvent(ctx context.Context, ev logpb.EventPayload, writeAsync bool) {
...
func (e *EventTableSink) writeToSystemEventsTable(
...
func init() {
	registry := &SinkRegistry{
		mu: struct {
			...
			serverSinks: make(map[serverident.ServerIdentificationPayload]log.SEventSink),
		},
	}
