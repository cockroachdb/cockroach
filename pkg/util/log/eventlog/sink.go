...
type SinkRegistry struct {
	mu struct {
		syncutil.Mutex
		tenantSinks map[uint64]log.SEventSink
	}
}

var _ log.SEventSinkRegistry = &SinkRegistry{}

func (e *SinkRegistry) RegisterSink(ctx context.Context, tenant uint64, sink log.SEventSink) {
	e.mu.Lock()
	defer e.mu.Unlock()
	e.mu.tenantSinks[tenant] = sink
}

func (e *SinkRegistry) GetSink(tenantId uint64) (log.SEventSink, bool) {
	e.mu.Lock()
	defer e.mu.Unlock()
	sink, ok := e.mu.tenantSinks[tenantId]
	return sink, ok
}
...
func (e *EventTableSink) WriteEvent(ctx context.Context, ev logpb.EventPayload) {
	...
	if e.writeAsync {
		log.VInfof(ctx, 2, "ASYNC Writing event to eventlog table")
		e.asyncWrite(ctx, ev)
	} else {
		log.VInfof(ctx, 2, "SYNC Writing event to eventlog table")
		e.syncWrite(ctx, ev)
	}
}
...
func init() {
	registry := &SinkRegistry{
		mu: struct {
			...
			tenantSinks: make(map[uint64]log.SEventSink),
		},
	}
