...
func TestSink(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(fmt.Sprintf("sinkAsync=%t", tc.writeAsync), func(t *testing.T) {
			ambientCtx := log.MakeServerAmbientContext(nil, &ServerId{instanceId: tc.instanceId})
			sink := eventlog.NewEventTableSink(s.InternalDB().(isql.DB), tc.writeAsync, s.Stopper(), ambientCtx, s.ClusterSettings())
			// This can be any event type
			event := &eventpb.StatusChange{}
			eventName := fmt.Sprintf("my_event_%d", tc.instanceId)
			event.CommonDetails().EventType = eventName
			event.CommonDetails().Timestamp = timeutil.Now().UnixNano()

			sink.WriteEvent(ambientCtx.AnnotateCtx(ctx), event, false)
			runner := sqlutils.MakeSQLRunner(conn)
			var rows int
			testutils.SucceedsSoon(t, func() error {
				runner.QueryRow(t,
					fmt.Sprintf(`SELECT count(*) from system.eventlog where "eventType" = '%s' and "reportingID"=%d`,
						eventName, tc.instanceId),
				).Scan(&rows)
