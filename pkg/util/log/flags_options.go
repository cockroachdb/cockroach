...
type flagsOptions struct {
	// redactionPolicyManaged is set by WithRedactionPolicyManaged. If set,
	// indicates we are logging from an instance that's run as part of a managed
	// service. This impacts certain log redaction policies. See safeManaged for
	// additional details.
...
type redactionPolicyManagedFlagsOption struct{}

// WithRedactionPolicyManaged configures logging to be aware that it's being run as
...
func WithRedactionPolicyManaged(managed bool) FlagsOption {
	if !managed {
		return nil
	}
...
func (m redactionPolicyManagedFlagsOption) apply(opts flagsOptions) flagsOptions {
	opts.redactionPolicyManaged = true
	return opts
}

var managedFlagsSingleton = FlagsOption(redactionPolicyManagedFlagsOption{})
