...
func TestCrdbV1EncodeDecode(t *testing.T) {
	...
	datadriven.RunTest(t, "testdata/crdb_v1", func(t *testing.T, td *datadriven.TestData) string {
		...
		case "entries":
			...
			// Decode.
			entryStr := buf.String()
			decoder := NewEntryDecoder([]byte(entryStr), strings.NewReader(entryStr), WithMarkedSensitiveData, "crdb-v1")
...
func TestCrdbV1EntryDecoderForVeryLargeEntries(t *testing.T) {
	...
	readAllEntries := func(contents string) []logpb.Entry {
		decoder := NewEntryDecoder([]byte(contents), strings.NewReader(contents), WithFlattenedSensitiveData, "crdb-v1")
