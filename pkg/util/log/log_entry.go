...
func (p entryPayload) redactableMessage() redact.RedactableString {
	if !p.redactable {
		panic("API misuse")
	}
	return redact.RedactableString(p.message)
}
...
func makeUnstructuredEntry(
	ctx context.Context, s Severity, c Channel, depth int, format string, args ...interface{},
) (res logEntry) {
	res = makeEntry(ctx, s, c, depth+1)
...
func MakeLegacyEntry(
	...
	return makeUnstructuredEntry(ctx, s, c, depth+1, format, args...).convertToLegacy()
}
