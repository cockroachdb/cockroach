...
// ErrorOnPanic recovers from a panic, logging at Error severity.
// This makes it so that panics in HTTP handlers are not ignored.
func ErrorOnPanic() {
	if r := recover(); r != nil {
		Errorf(context.Background(), "unexpected panic: %s", r)
	}
}
