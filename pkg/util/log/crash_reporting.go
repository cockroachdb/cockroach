...
// GetContextString returns a string with the values of registered tags in the
// context. The format is "key1: val1; key2: val2".
func GetContextString(ctx context.Context) string {
	var buf bytes.Buffer
	for _, f := range tagFns {
		v := f.value(ctx)
		if v != "" {
