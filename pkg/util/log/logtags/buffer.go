...
func (b *Buffer) Merge(other *Buffer) *Buffer {
	...
	// Check for a common case where the "left" tags are a subsequence of the
	// "right" tags.
	if len(other.tags) >= len(b.tags) {
		i, j := 0, 0
		for i < len(b.tags) && j < len(other.tags) {
	...
	for ; i < len(other.tags); i++ {
		res.addInPlace(other.tags[i].key, other.tags[i].value)
	}
	return res
}
...
func (b *Buffer) init(length, capacity int) {
	if length < staticSlots {
		b.tags = b.prealloc[:length]
	} else {
		b.tags = make([]Tag, length, capacity)
	}
}

func (b *Buffer) addInPlace(key string, value interface{}) {
	for i := range b.tags {
		if b.tags[i].key == key {
