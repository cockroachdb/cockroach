...
func TestFormattableTags(t *testing.T) {
	...
	for i, tc := range testCases {
		...
		assert.Equal(t, tc.buf, string(buf.Bytes()), "bufprint %d", i)

		buf = buffer{}
		tags.formatJSONToBuffer(&buf)
		assert.Equal(t, tc.json, string(buf.Bytes()), "jsonprint %d", i)

		buf = buffer{}
		rtags := tags.redactTagValues(false /* preserve markers */)
		rtags.formatToBuffer(&buf)

		assert.Equal(t, tc.rvals, string(buf.Bytes()), "redactvals %d", i)
	}
}
