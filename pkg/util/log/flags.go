...
func init() {
	...
	// using TestLogScope.
	cfg, err := getTestConfig(nil /* output to files disabled */, true /* mostly inline */)
	if err != nil {
		panic(err)
	}
...
func ApplyConfig(config logconfig.Config) (cleanupFn func(), err error) {
	...
	if config.CaptureFd2.Enable {
		...
		fakeConfig := logconfig.FileSinkConfig{
			...
			Channels: logconfig.FilterChannels(channel.DEV),
		}
		if err := fakeConfig.Channels.Update(fakeConfig.CommonSinkConfig.Filter); err != nil {
