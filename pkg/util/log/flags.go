...
// ApplyConfig applies the given configruation.
//
...
func ApplyConfig(config logconfig.Config) (cleanupFn func(), err error) {
	...
	logging.stderrSinkInfo.applyConfig(config.Sinks.Stderr.CommonSinkConfig)
	...
	for _, ch := range config.Sinks.Stderr.Channels.Channels {
		...
		l := chans[ch]
		l.sinkInfos = append(l.sinkInfos, &logging.stderrSinkInfo)
	}
...
func DescribeAppliedConfig() string {
	...
	config.Sinks.Stderr.CommonSinkConfig = logging.stderrSinkInfo.describeAppliedConfig()
	// TODO(knz): extract the formatter into the configuration.
	...
	for ch, logger := range logging.channels {
		describeConnections(logger, ch,
			&logging.stderrSinkInfo, &config.Sinks.Stderr.Channels)
	}
