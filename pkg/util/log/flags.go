...
//
// See logconfig.LoggingCloser for a more graceful approach to shutting
...
func ApplyConfig(config logconfig.Config) (resFn func(), err error) {
	...
	config.Closer.SetShutdownFn(cleanupFn)
	...
	for fileGroupName, fc := range config.Sinks.FileGroups {
		...
		attachBufferWrapper(secLoggersCtx, fileSinkInfo, fc.CommonSinkConfig, config.Closer)
	...
	for _, fc := range config.Sinks.FluentServers {
		...
		attachBufferWrapper(secLoggersCtx, fluentSinkInfo, fc.CommonSinkConfig, config.Closer)
	...
	for _, fc := range config.Sinks.HTTPServers {
		...
		attachBufferWrapper(secLoggersCtx, httpSinkInfo, fc.CommonSinkConfig, config.Closer)
...
func attachBufferWrapper(
	ctx context.Context, s *sinkInfo, c logconfig.CommonSinkConfig, ls *logconfig.LoggingCloser,
) {
	b := c.Buffering
	if b.IsNone() {
		return
