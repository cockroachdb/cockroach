...
func (buf *buffer) maybeMultiLine(
	...
	for i := 0; i < len(msg); i++ {
		...
		if markersStartWithSingleByteRune && msg[i] < utf8.RuneSelf {
			lastLen++
			continue
		}
		if redactable {
...
var markersStartWithSingleByteRune = startRedactionMarker[0] < utf8.RuneSelf && endRedactionMarker[0] < utf8.RuneSelf
