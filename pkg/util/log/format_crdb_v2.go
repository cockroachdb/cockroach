...
func (f entryDecoderV2Fragment) getTags(editor redactEditor) string {
	...
	// Strip out the tenant ID tag. We handle it separately in getIDPayload().
...
func (f entryDecoderV2Fragment) getIDPayload() *logpb.IdentificationPayload {
	out := &logpb.IdentificationPayload{
	...
	default:
		...
		if strings.HasPrefix(tags, TenantIDLogTagKey) {
			out.TenantID = strings.Split(tags, ",")[0][1:]
		}
	}
	return out
}
