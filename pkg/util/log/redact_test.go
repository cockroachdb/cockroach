...
func TestRedactedLogOutput(t *testing.T) {
	...
	defer func(prev bool) { logging.redactableLogs = prev }(logging.redactableLogs)

	logging.redactableLogs = false
...
func TestRedactTags(t *testing.T) {
	...
	testData := []struct {
		...
		{logtags.AddTag(baseCtx, "k", 123), "[k" + quote("123") + "] "},
		{logtags.AddTag(baseCtx, "k", Safe(123)), "[k123] "},
		{logtags.AddTag(baseCtx, "k", startRedactable), "[k" + quote(escapeMark) + "] "},
		{logtags.AddTag(baseCtx, "kg", 123), "[kg=" + quote("123") + "] "},
		{logtags.AddTag(baseCtx, "kg", Safe(123)), "[kg=123] "},
		{logtags.AddTag(logtags.AddTag(baseCtx, "k", nil), "n", 55), "[k,n" + quote("55") + "] "},
		{logtags.AddTag(logtags.AddTag(baseCtx, "k", nil), "n", Safe(55)), "[k,n55] "},
	}
...
func TestMakeFormat(t *testing.T) {
	...
	for _, test := range testData {
		result := fmt.Sprintf(test, formatTester{})
...
func (formatTester) Format(s fmt.State, verb rune) {
	fmt := MakeFormat(s, verb)
	_, _ = s.Write([]byte(fmt))
}
...
func TestRedactStream(t *testing.T) {
	testData := []struct {
		...
		{"%v", "abc \n\n", "abc"},
		{"%v", " \n\nabc", " \n\nabc"},
		{"%v", startRedactable + "abc" + endRedactable, escapeMark + "abc" + escapeMark},
		{"%v", 123, "123"},
		{"%05d", 123, "00123"},
		{"%#x", 17, "0x11"},
		{"%+v", &complexObj{startRedactable + endRedactable}, "&{v:" + escapeMark + escapeMark + "}"},
	}

	for _, tc := range testData {
		var buf strings.Builder
		n, _ := fmt.Fprintf(&buf, tc.f, &redactable{tc.input})
		result := buf.String()
		expected := quote(tc.expectedInner)
