...
// The log file format is {process}.{host}.{username}.{timestamp}.{pid}.{counter}.{severity}.log
// cockroach.Brams-MacBook-Pro.bram.2015-06-09T16-10-48Z.30209.0001.WARNING.log
...
var logFileRE = regexp.MustCompile(`([^\.]+)\.([^\.]+)\.([^\.]+)\.([^\.]+)\.(\d+)\.(\d+)\.(ERROR|WARNING|INFO)\.log`)
...
func logName(severity Severity, t time.Time) (name, link string) {
	...
	fCnt := atomic.AddInt64(&fileNameCounter, 1)

	name = fmt.Sprintf("%s.%s.%s.%s.%06d.%04d.%s.log",
...
func parseLogFilename(filename string) (FileDetails, error) {
	...
	counter, err := strconv.ParseInt(matches[6], 10, 0)
	if err != nil {
		return FileDetails{}, err
	}

	sev, sevFound := SeverityByName(matches[7])
	if !sevFound {
