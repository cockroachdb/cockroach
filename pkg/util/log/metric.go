...
type LogMetrics interface {
	...
	// provided LogMetricName by the given amount, assuming the
	// metric has been registered.
	//
	// The LogMetrics implementation must have metadata defined
	// for the given LogMetricName within its own scope. See
	// pkg/util/metric/log_metrics.go for details.
	IncrementCounter(metric LogMetricName, amount int64)
	// Registry returns the *metric.Registry underlying the interface's
	// implementation. We return an interface here to avoid creating a
	// circular dependency with the metrics package. However, the type
	// returned by Registry should always be a *metric.Registry instance,
	// to allow us to register said Registry with the MetricsRecorder used
	// by servers.
	Registry() interface{}
}

// LogMetricName represents the name of a metric registered &
// used within the log package, available to use in the LogMetrics
// interface.
type LogMetricName string

// FluentSinkConnectionError is the LogMetricName for the metric
...
const FluentSinkConnectionError LogMetricName = "fluent.sink.conn.errors"
