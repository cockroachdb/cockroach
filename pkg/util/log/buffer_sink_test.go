...
func TestBufferErrCallback(t *testing.T) {
	defer leaktest.AfterTest(t)()
	var errCallbackCalled int32
	errCallback := func(error) {
		atomic.StoreInt32(&errCallbackCalled, 1)
	}
	...
	err := testError{}
	mock.EXPECT().
		output(gomock.Eq(message), gomock.Any()).Do(addArgs(wg.Done)).Return(err)
	require.NoError(t, sink.output(message, sinkOutputOptions{extraFlush: true}))

	wg.Wait()
	require.Equal(t, int32(1), atomic.LoadInt32(&errCallbackCalled))
}
