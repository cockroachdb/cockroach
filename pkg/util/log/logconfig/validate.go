...
func (c *Config) Validate(defaultLogDir *string) (resErr error) {
	...
	for serverName, fc := range c.Sinks.FluentServers {
		if fc == nil {
			fc = &FluentSinkConfig{Channels: FilterChannels()}
	...
	for sinkName, fc := range c.Sinks.HTTPServers {
		if fc == nil {
			fc = &HTTPSinkConfig{Channels: FilterChannels()}
	...
	if def := fileSinks[devch]; len(def) == 0 {
		...
		list := fc.Channels.Filters[logpb.Severity_UNKNOWN]
		list.Channels = append(list.Channels, devch)
		fc.Channels.Filters[logpb.Severity_UNKNOWN] = list
		if err := fc.Channels.Update(fc.Filter); err != nil {
			// Should never happen.
