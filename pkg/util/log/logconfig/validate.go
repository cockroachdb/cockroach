...
func (c *Config) Validate(defaultLogDir *string) (resErr error) {
	...
	propogateCommonDefaults(&baseFileDefaults.CommonSinkConfig, baseCommonSinkConfig)
	propogateCommonDefaults(&baseFluentDefaults.CommonSinkConfig, baseCommonSinkConfig)

	propogateFileDefaults(&c.FileDefaults, baseFileDefaults)
	propogateFluentDefaults(&c.FluentDefaults, baseFluentDefaults)
	...
	propogateCommonDefaults(&c.Sinks.Stderr.CommonSinkConfig, c.FileDefaults.CommonSinkConfig)
	...
	if def := fileSinks[devch]; def == nil {
		...
		fc.prefix = "default"
		propogateFileDefaults(&fc.FileDefaults, c.FileDefaults)
		if err := c.validateFileSinkConfig(fc, defaultLogDir); err != nil {
...
func (c *Config) validateFluentSinkConfig(fc *FluentSinkConfig) error {
	propogateFluentDefaults(&fc.FluentDefaults, c.FluentDefaults)
...
func propogateCommonDefaults(target *CommonSinkConfig, source CommonSinkConfig) {
	propogateDefaults(target, source)
}

func propogateFileDefaults(target *FileDefaults, source FileDefaults) {
	propogateDefaults(target, source)
}

func propogateFluentDefaults(target *FluentDefaults, source FluentDefaults) {
	propogateDefaults(target, source)
}

// propogateDefaults takes (target *T, source T) where T is a struct
...
func propogateDefaults(target, source interface{}) {
	...
	for i := 0; i < t.NumField(); i++ {
		...
		if tf.Kind() == reflect.Struct {
			propogateDefaults(tf.Addr().Interface(), sf.Interface())
