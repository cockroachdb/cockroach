...
func TestSignalAndWaitForShutdown(t *testing.T) {
	...
	t.Run("returns after all registered sinks exit", func(t *testing.T) {
		...
		fakeSinkRoutine := func(ctx context.Context, ls *LoggingShutdown) {
		...
		logShutdown := NewLoggingShutdown()
		...
		for {
			...
			case <-doneChan:
				return
			case <-timeout:
				t.Error("SignalAndWaitForShutdown hanging")
				return
			}
		}
	})
