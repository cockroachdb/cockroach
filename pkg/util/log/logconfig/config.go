...
// FilterChannels is a constructor for ChannelFilters that
// helps in tests.
func FilterChannels(chs ...logpb.Channel) ChannelFilters {
...
// Update changes the expressed filters to substitute the UNKNOWN
// severity by the proposed default; it also propagates the user
// configuration into the other accessor fields.
func (c *ChannelFilters) Update(defSev logpb.Severity) error {
	...
	// If anything was specified at severity NONE, remove it.
	// This means "no channel connected".
	delete(c.Filters, logpb.Severity_NONE)

	// Ensure the configurations are stable.
...
// AllFiltered returns true if there are no channels listed or
// all of them are filtered at level NONE.
func (c ChannelFilters) AllFiltered() bool {
	filtered := true
