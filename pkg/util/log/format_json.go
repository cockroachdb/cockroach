...
func getString(obj map[string]interface{}, f string) (string, bool) {
	if v, ok := obj[f]; ok {
		vString, ok := v.(string)
		if ok {
			return vString, true
...
func getInt64(obj map[string]interface{}, f string) (int64, bool) {
	if v, ok := obj[f]; ok {
		vFloat, ok := v.(float64)
		if ok {
...
func (e *JsonCompactEntry) Populate(entry *JsonEntry) {
