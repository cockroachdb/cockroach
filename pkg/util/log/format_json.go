...
type entryDecoderJSON struct {
	// scanner is used to read the log file line by line, so it must use a split
	// function that returns one line at a time. The default split function for
	// bufio.Scanner is ScanLines, which does exactly this.
