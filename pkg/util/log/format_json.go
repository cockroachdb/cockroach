...
func shouldSplitOnValue(totalLen int, isStringValue bool, isSplittable bool, isEvent bool) bool {
	...
	// If the payload is unstructured, we split if the
	// long line length has been exceeded.
	if isEvent && (!isStringValue || !isSplittable) {
		return false
	}
	if totalLen >= int(longLineLenJSON) {
		return true
	}
	return false
}
...
func (buf *buffer) maybeJSONMultiLine(prefixLen int, redactable bool, msg string, isEvent bool) {
	...
	for i, w := 0, 0; i < len(msg); i += w {
		...
		if !beforeColon {
			...
			if shouldSplitOnValue(lastLen+len(keyBuffer)+len(valueBuffer), betweenQuotesInValue, betweenSplittableMarkers, isEvent) {
				buf.Write(keyBuffer)
