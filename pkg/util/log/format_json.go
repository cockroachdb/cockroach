...
func (e *JSONEntry) populate(entry *logpb.Entry, d *entryDecoderJSON) (*redactablePackage, error) {
	...
	if e.Tags != nil {
		...
		for k, v := range e.Tags {
			// We reserve these tags for the IdentificationPayload in the
			// logpb.Entry.
			if k == TenantIDLogTagKey || k == TenantIDLogTagKeyJSON {
				entry.IDPayload.TenantID = fmt.Sprint(v)
				continue
			}
			t = t.Add(k, v)
		}
		// If the only tag was a tenant ID, the buffer may be nil.
