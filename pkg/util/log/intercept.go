...
func Intercept(ctx context.Context, f InterceptorFn) {
	mainLog.Intercept(ctx, f)
	// TODO(knz): This should probably intercept secondary log traffic too,
	// conditional on some flag (e.g. rocksdb traffic should be intercepted,
	// but SQL audit logging probably not).
}
