...
func Intercept(ctx context.Context, f InterceptorFn) {
	...
	// receiver.
	if f != nil {
		log(ctx, severity.WARNING, channel.DEV, "log traffic is now intercepted; log files will be incomplete")
	}
	logging.interceptor.Store(f) // intentionally also when f == nil
	if f == nil {
		log(ctx, severity.INFO, channel.DEV, "log interception is now stopped; normal logging resumes")
	}
}
