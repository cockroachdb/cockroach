...
func (d *devnull) Config() logconfig.CommonSinkConfig {
	return logconfig.DefaultConfig().FileDefaults.CommonSinkConfig
}
...
func addInterceptor(t *testing.T, ch logpb.Channel, interceptor Interceptor) func() {
	t.Helper()
	cleanup, err := AddInterceptor(ch, interceptor)
...
func TestManyInterceptors(t *testing.T) {
	...
	require.NoError(t,
		ctxgroup.GroupWorkers(context.Background(), 100, func(ctx context.Context, worker int) error {
			defer addInterceptor(t, channel.DEV, &devnull{})()
			for i := 0; i < 10; i++ {
...
func (c *captureInterceptor) Config() logconfig.CommonSinkConfig {
	return logconfig.DefaultConfig().FileDefaults.CommonSinkConfig
}
...
func TestIntercept(t *testing.T) {
	...
	defer addInterceptor(t, channel.DEV, first)()
	defer addInterceptor(t, channel.DEV, second)()
	defer addInterceptor(t, channel.DEV, empty)()
