...
func TestEncodeContainingJSONInvertedIndexSpans(t *testing.T) {
	...
	runTest := func(left, right JSON, expected, expectNoDuplicates bool) (tight bool) {
		keys, err := EncodeInvertedIndexKeys(nil, left)
		require.NoError(t, err)

		spansSlice, tight, noDuplicates, err := EncodeContainingInvertedIndexSpans(nil, right)
		require.NoError(t, err)
	...
	for _, c := range testCases {
		...
		tight := runTest(value, contains, c.expected, c.noDuplicates)
	...
	for i := 0; i < 100; i++ {
		...
		// path ending in a scalar (i.e., not an empty object or array).
		...
		expectNoDuplicates := len(paths) == 1 && !hasContainerLeaf

		// Now check that we get the same result with the inverted index spans.
		runTest(left, right, res, expectNoDuplicates)
	}
}
