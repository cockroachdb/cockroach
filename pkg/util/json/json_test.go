...
var parseJSONImpls = []struct {
	...
	{name: "lexer", typ: useJSONLexer, opts: []ParseOption{WithJSONLexer()}},
}
...
...|^.....|...: invalid string literal token`, useJSONLexer),
	...
	for _, tc := range testCases {
		t.Run(fmt.Sprintf("%s/%s", tc.implName, tc.input), func(t *testing.T) {
			...
			//if !strings.Contains(expectErr.Error(), tc.msg[impl]) {
			//	t.Fatalf("expected error message to be '%s', but was '%s'", tc.msg[impl], expectErr.Error())
			//}
