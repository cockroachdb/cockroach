...
func TestEncodeContainedJSONInvertedIndexSpans(t *testing.T) {
	testCases := []struct {
		...
		// include and exclude the first value, indicated by containsKeys.
		...
		{`{"baz": []}`, `{"HhX2z": {}, "baz": [{}, []], "foobar": {"8YX0K@Km-}": [[true], [null]]}, "oC-k+(:\\&Rl8": true, "z*R:A<M&:": "ofK['v@~X"}`, true, true, false},
		...
		{`{"foobar": []}`, `{"]|@=_O)R20]": {"3ax\"dy,[,CX<": {"y#~69G": null}, "bar": false, "foobar": 0.29775049433461365, "vRAacj},jz": []}, "foo": {"b": [{}, true]}, "foobar": [1.0934912722511079]}`, true, true, false},
	...
	runTest := func(indexedValue, value JSON, expectContainsKeys, expected, expectUnique bool) {
		...
		if containsKeys != expectContainsKeys {
			if expectContainsKeys {
				t.Errorf("expected spans of %s to include %s but it did not", value, indexedValue)
			} else {
				t.Errorf("expected spans of %s not to include %s but it did", value, indexedValue)
			}
		}
