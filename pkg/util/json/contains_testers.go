...
func (j jsonArray) slowContains(other JSON) bool {
	...
	if ary, ok := other.(jsonArray); ok {
		for i := 0; i < len(ary); i++ {
			found := false
			for k := 0; k < len(j); k++ {
				if j[k].jsonType() == ary[i].jsonType() && j[k].slowContains(ary[i]) {
					found = true
					break
				}
			}
...
func (j jsonObject) slowContains(other JSON) bool {
	if obj, ok := other.(jsonObject); ok {
		for i := 0; i < len(obj); i++ {
			...
			if leftVal == nil || !leftVal.slowContains(obj[i].v) {
				return false
			}
		}
		return true
...
func (j jsonArray) subdocument(isRoot bool, rng *rand.Rand) JSON {
	...
	for i < len(j) {
		if rng.Intn(2) == 0 {
			result = append(result, j[i].subdocument(false /* isRoot */, rng))
		}
		if rng.Intn(2) == 0 {
			i++
		}
	...
	for _, e := range j {
		if rng.Intn(2) == 0 {
			result = append(result, jsonKeyValuePair{
				k: e.k,
				v: e.v.subdocument(false /* isRoot */, rng),
			})
		}
	}
	return result
