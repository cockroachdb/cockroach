...
func (j jsonArray) encodeContainingInvertedIndexSpans(
	...
	for i := range j {
		isRoot := true
		if j[i].isScalar() {
			// Scalars should not be treated as a root element if they are contained
			// in an array, but nested arrays and objects should.
			isRoot = false
		}
		...
		// If the child is also an array with more than one element, we cannot
		// produce tight spans.
		switch t := j[i].(type) {
		case jsonArray:
			tight = tight && t.Len() <= 1
		case *jsonEncoded:
			if t.typ == ArrayJSONType {
				tight = tight && t.containerLen <= 1
			}
		}
	}
