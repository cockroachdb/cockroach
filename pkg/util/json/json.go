...
type JSON interface {
	...
	encodeContainingInvertedIndexSpans(b []byte, isRoot, isObjectValue bool) (
		_ []roachpb.Spans, tight bool, err error,
	)
...
func (j jsonArray) encodeContainingInvertedIndexSpans(
	...
	for i := range j {
		...
		if isContainerWithMoreThanOneElement(j[i]) {
			tight = false
		}
	}
...
func (j jsonObject) encodeContainingInvertedIndexSpans(
	...
	for i := range j {
		...
		children, childTight, err := j[i].v.encodeContainingInvertedIndexSpans(
		...
		if isContainerWithMoreThanOneElement(j[i].v) {
			tight = false
		}
	}
	return spans, tight, nil
...
// isContainerWithMoreThanOneElement returns true if the given JSON is a
// container (i.e., JSON object or array) with more than one element.
func isContainerWithMoreThanOneElement(json JSON) bool {
	switch t := json.(type) {
	case jsonArray, jsonObject:
		return t.Len() > 1
	case *jsonEncoded:
...
func encodeContainingInvertedIndexSpansFromLeaf(
	...
	if isObjectValue {
		...
		// objects and arrays before encoding the contents of the object/array. The
		// call to EncodeJSONKeyStringAscending(..., false) adds that suffix, and
		// since the key is already encoded, we pass the empty string as the key.
	...
	for _, key := range keys {
		...
		spans = append(spans, roachpb.Span{Key: key, EndKey: endKey})
	}
