	...
	case jsonObject:
		...
		if root {
			...
			spans = append(spans, roachpb.Span{
				...
				// empty arrays. Starting this span with the PrefixEnd() of that prefix
				// ensures that we exclude arrays from the result.
			...
			spans = append(spans, roachpb.Span{
				...
				// objects and arrays before encoding the contents of the object/array.
				// The call to EncodeJSONKeyStringAscending(..., false) adds that suffix,
				// and since the key is already encoded, we pass the empty string as the
				// key. The result is passed to EncodeArrayAscending, which returns the
				// prefix of all non-empty arrays. Starting this span with the PrefixEnd()
				// of that prefix ensures that we exclude arrays from the result.
