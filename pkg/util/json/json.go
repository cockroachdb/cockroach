...
func (j jsonArray) EncodeInvertedIndexKeys(b []byte) [][]byte {
	...
	for i := range j {
		for _, childBytes := range j[i].EncodeInvertedIndexKeys(b) {
			encodedKey := append(encoding.EncodeArrayAscending(b), childBytes...)
...
func (j jsonObject) EncodeInvertedIndexKeys(b []byte) [][]byte {
	var outKeys [][]byte
	for i := range j {
		for _, childBytes := range j[i].v.EncodeInvertedIndexKeys(b) {
