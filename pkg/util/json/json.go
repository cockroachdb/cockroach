...
type JSON interface {
	...
	encodeInvertedIndexSpans(b []byte) ([]roachpb.Spans, error)
...
func EncodeInvertedIndexSpans(b []byte, json JSON) ([]roachpb.Spans, error) {
	return json.encodeInvertedIndexSpans(encoding.EncodeJSONAscending(b))
}
...
func (j jsonNull) encodeInvertedIndexSpans(b []byte) ([]roachpb.Spans, error) {
	return encodeInvertedIndexSpansFromLeaf(j, b)
}
...
func (j jsonTrue) encodeInvertedIndexSpans(b []byte) ([]roachpb.Spans, error) {
	return encodeInvertedIndexSpansFromLeaf(j, b)
}
...
func (j jsonFalse) encodeInvertedIndexSpans(b []byte) ([]roachpb.Spans, error) {
	return encodeInvertedIndexSpansFromLeaf(j, b)
}
...
func (j jsonString) encodeInvertedIndexSpans(b []byte) ([]roachpb.Spans, error) {
	return encodeInvertedIndexSpansFromLeaf(j, b)
}
...
func (j jsonNumber) encodeInvertedIndexSpans(b []byte) ([]roachpb.Spans, error) {
	return encodeInvertedIndexSpansFromLeaf(j, b)
}
...
func (j jsonArray) encodeInvertedIndexSpans(b []byte) ([]roachpb.Spans, error) {
	// Checking for an empty array.
	if len(j) == 0 {
		return encodeInvertedIndexSpansFromLeaf(j, b)
	}
	...
	for i := range j {
		children, err := j[i].encodeInvertedIndexSpans(prefix[:len(prefix):len(prefix)])
		if err != nil {
			return nil, err
		}
...
func (j jsonObject) encodeInvertedIndexSpans(b []byte) ([]roachpb.Spans, error) {
	if len(j) == 0 {
		return encodeInvertedIndexSpansFromLeaf(j, b)
	}
	...
	for i := range j {
		...
		children, err := j[i].v.encodeInvertedIndexSpans(prefix)
		if err != nil {
			return nil, err
		}
...
func encodeInvertedIndexSpansFromLeaf(j JSON, b []byte) ([]roachpb.Spans, error) {
	...
	case jsonArray:
		...
		// This first key covers arrays that are the value of a JSON object key.
		// It is needed for JSON objects such as '{"a": []}' to match '{"a": [1]}'
		// (i.e., '{"a": [1]}' @> '{"a": []}').
		keys = append(keys, encoding.EncodeArrayAscending(
			encoding.EncodeJSONKeyStringAscending(prefix, "", false),
		))

		// This second key covers all stand-alone arrays. It is needed for JSON
		// arrays such as '[]' to match '[1]' (i.e., '[1]' @> '[]').
	...
	case jsonObject:
		...
		// This first span covers keys in-between the two keys used for jsonArray
		// above, and is used for objects that are the value of another JSON object
		// key. It is needed for JSON objects such as '{"a": {}}' to match
		// '{"a": {"b": "c"}}' (i.e., '{"a": {"b": "c"}}' @> '{"a": {}}').
		...
		// This second span covers keys after the second key used for jsonArray
		// above, and is used for stand-alone objects. It is needed for JSON
		// objects such as '{}' to match '{"a": "b"}' (i.e., '{"a": "b"}' @> '{}').
		spans = append(spans, roachpb.Span{
