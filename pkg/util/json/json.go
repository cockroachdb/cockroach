...
// Clear clears the state of this builder so that it can be reused.
func (b *ArrayBuilder) Clear() {
	b.jsons = b.jsons[:0]
}
...
func (b *FixedKeysObjectBuilder) Build() (JSON, error) {
	if b.updated.Len() != len(b.pairs) {
		return nil, errors.AssertionFailedf(
			"expected all %d keys to be updated, updated %d instead",
			len(b.pairs), b.updated.Len())
	}
	b.updated = util.FastIntSet{}
	return jsonObject(b.pairs), nil
}
