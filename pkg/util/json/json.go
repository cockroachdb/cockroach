...
func (j jsonArray) encodeContainedInvertedIndexSpans(
	...
	if !isObjectValue {
		...
		// by everything. Empty array values are already accounted for below when
		// getting the spans for the child, so they should be excluded.
...
func (j jsonObject) encodeContainedInvertedIndexSpans(
	...
	// by everything. Empty object values are already accounted for below when
	// getting the spans for the child, so they should be excluded.
	if !isObjectValue {
	...
	for i := range j {
		...
		child, err := j[i].v.encodeContainedInvertedIndexSpans(
			prefix, false /* isRoot */, !end, /* isObjectValue */
		)
		if err != nil {
			return nil, err
		}
		...
		if !end {
			...
			if v != nil {
				...
				childWithEnd, err := v.encodeContainedInvertedIndexSpans(
					prefixWithEnd, false /* isRoot */, false, /* isObjectValue */
				)
				if err != nil {
					return nil, err
				}
