...
type JSON interface {
	...
	// preprocessForContains sorts all arrays in the JSON object. We could
	// potentially use a different type to represent a JSON object that has been
	// preprocessed in this way, but that seems a bit heavyweight for one operator.
	preprocessForContains() JSON
	// contains implements the <@, @> operators. It should only be called on a
	// JSON document which has already been preprocessed by
