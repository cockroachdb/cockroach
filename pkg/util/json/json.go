...
type JSON interface {
	...
	// JSON hierarchy.
	encodeContainingInvertedIndexSpans(b []byte, root bool) ([]roachpb.Spans, error)
...
//   ["a", "b") ["c", "d")
//
func EncodeContainingInvertedIndexSpans(b []byte, json JSON) ([]roachpb.Spans, error) {
...
func (j jsonNull) encodeContainingInvertedIndexSpans(b []byte, root bool) ([]roachpb.Spans, error) {
...
func (j jsonTrue) encodeContainingInvertedIndexSpans(b []byte, root bool) ([]roachpb.Spans, error) {
	return encodeContainingInvertedIndexSpansFromLeaf(j, b, root)
}
...
func (j jsonArray) encodeContainingInvertedIndexSpans(
	...
	for i := range j {
		children, err := j[i].encodeContainingInvertedIndexSpans(
			prefix[:len(prefix):len(prefix)], false, /* root */
		)
		if err != nil {
			return nil, err
		}
...
func (j jsonObject) encodeContainingInvertedIndexSpans(
	...
	for i := range j {
		...
		children, err := j[i].v.encodeContainingInvertedIndexSpans(prefix, false /* root */)
		if err != nil {
			return nil, err
		}
...
func encodeContainingInvertedIndexSpansFromLeaf(
	...
	case jsonArray:
		...
		if root {
			...
			// (i.e., '{"a": [1]}' @> '{"a": []}' is true). prefix contains the
			// encoded key but not the escape + escapedJSONObjectKeyTerm suffix which
			// is used for non-empty arrays before encoding the contents of the array.
			// The call to EncodeJSONKeyStringAscending(..., false) adds that suffix,
			// and since the key is already encoded, we pass the empty string as the
			// key. The result is passed to EncodeArrayAscending, which returns the
			// prefix of all non-empty arrays.
	...
	case jsonObject:
		...
		if root {
			// Add a span to cover keys for stand-alone objects. It is needed for
			// JSON objects such as '{}' to match '{"a": "b"}', but not '[1]'.
			// (i.e., '{"a": "b"}' @> '{}' is true, but '[1]' @> '{}' is false).
			spans = append(spans, roachpb.Span{
				// EncodeObjectSpanStartAscending generates the first possible value
				// for JSON objects.
				Key: roachpb.Key(encoding.EncodeObjectSpanStartAscending(prefix)),
			...
			// JSON object key. The span is needed for JSON objects such as
			// '{"a": {}}' to match '{"a": {"b": "c"}}', but not '{"a": [1]}' or
			// ["a"]. (i.e., '{"a": {"b": "c"}}' @> '{"a": {}}' is true, but
			// '{"a": [1]}' @> '{"a": {}}' and '["a"]' @> '{"a": {}}' are false).
			spans = append(spans, roachpb.Span{
				// prefix contains the encoded key but not the
				...
				// the key is already encoded, we pass the empty string as the key. The
				// result is passed to EncodeObjectSpanStartAscending, which returns
				// the first possible value for JSON objects.
				Key: roachpb.Key(encoding.EncodeObjectSpanStartAscending(
					encoding.EncodeJSONKeyStringAscending(prefix, "", false /* end */),
				)),
				// EncodeArrayAscending generates the prefix that is used for all non-
				// empty arrays. Ending this span with that prefix ensures that we
