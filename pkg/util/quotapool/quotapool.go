...
// single-dimension integer quota. The abstraction is not unlike a semaphore
// but it additionally offers FIFO ordering for serving requests.
package quotapool
...
type Request interface {
	...
	// not intend to use. It is up to the implementer to decide if it makes
	// sense to return a zero-valued, non-nil unused or nil when using acquiring
	// all of the passed Resource. If nil is returned and there is a notion of
	// acquiring a zero-valued Resource unit from the pool then those acquisitions
	// may need to wait until the pool is non-empty before proceeding.
...
func (qp *QuotaPool) notifyNextLocked() {
	...
	// waiting to be notified, notify the goroutine and truncate our queue so to
	// ensure the said goroutine is at the head of the queue. Normally the next
	// lined up waiter is the one waiting for notification, but if others behind
	// us have also gotten their context canceled, they will leave behind waiters
	// that we would skip below.
	//
