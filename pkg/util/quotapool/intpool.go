...
// Releasing it later is an error.
...
func (p *IntPool) decCapacity(c uint64) {
	...
	// Wake up the request at the front of the queue. The decrement above races
	// may race with an ongoing request (which is why it's an atomic access), but
	// in any case that request is evaluated again.
