...
func BenchmarkPatternCompilation(b *testing.B) {
	for i := 0; i < b.N; i++ {
		_ = Compile(Pattern{
			state1{}: {
				event4{True, Wildcard}:  {state2{}, noAction},
				event4{False, Wildcard}: {state1{}, noAction},
			},
			state2{}: {
				event4{Wildcard, Wildcard}: {state2{}, noAction},
			},
			state3{True}: {
				event1{}: {state1{}, noAction},
			},
			...
			state4{Binding("x"), Binding("y")}: {
				event4{True, True}:   {state1{}, noAction},
				event4{True, False}:  {state2{}, noAction},
				event4{False, True}:  {state3{Binding("x")}, noAction},
				event4{False, False}: {state4{Binding("y"), Binding("x")}, noAction},
			},
		})
	}
}
