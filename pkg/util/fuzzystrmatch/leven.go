...
// LevenshteinLessThanDistance is the standard Levenshtein distance where the cost of
// insertion, deletion and substitution are all one and we care about distances less than
// or equal to maxDist.
func LevenshteinLessThanDistance(source, target string, maxD int) int {
	return LevenshteinLessThanDistanceWithCost(source, target, 1, 1, 1, maxD)
}

// LevenshteinLessThanDistanceWithCost is the Levenshtein distance between the source and
// the target with the given costs of insertion, deletion and substitution.
func LevenshteinLessThanDistanceWithCost(source, target string, insCost, delCost, subCost, maxDist int) int {
	// Converting to a slice of runes allows us to count multi-byte characters
	// properly.
	s, t := []rune(source), []rune(target)
	lenS, lenT := len(s), len(t)
	fmt.Println(s, len(s), lenS)
	fmt.Println(t, len(t), lenT)
	...
	fmt.Println(prev)
	fmt.Println(startColumn, stopColumn)
	return prev[lenS-1]
}
