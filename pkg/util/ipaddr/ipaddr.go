...
var errResultOutOfRange = errpgcode.WithCandidateCode(errors.New("result out of range"), pgcode.NumericValueOutOfRange)
...
func (ipAddr *IPAddr) And(other *IPAddr) (IPAddr, error) {
	var newIPAddr IPAddr
	if ipAddr.Family != other.Family {
		return newIPAddr, errpgcode.WithCandidateCode(
...
func (ipAddr *IPAddr) SubIPAddr(other *IPAddr) (int64, error) {
	var diff int64
	if ipAddr.Family != other.Family {
		return diff, errpgcode.WithCandidateCode(
