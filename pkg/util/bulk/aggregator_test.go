...
func TestAggregator(t *testing.T) {
	...
	ctx, agg := bulk.MakeAggregatorWithSpan(ctx, "mockAggregator")
	aggSp := tracing.SpanFromContext(ctx)
	defer agg.Close()

	child := tr.StartSpan("child", tracing.WithParent(root),
		tracing.WithEventListeners([]tracing.EventListener{agg}))
	defer child.Finish()
	...
	ctx, childChild := tracing.ChildSpan(ctx, "childChild")
	...
	// have not imported the remote childs' Recording.
