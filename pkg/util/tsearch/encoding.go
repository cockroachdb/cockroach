...
func EncodeTSVector(vector TSVector, appendTo []byte) []byte {
...
func EncodeTSVectorPGBinary(vector TSVector, appendTo []byte) ([]byte, error) {
	appendTo = encoding.EncodeUint32Ascending(appendTo, uint32(len(vector)))
	for _, term := range vector {
		l := term.lexeme
...
// EncodeTSQuery encodes a tsquery into a serialized representation.
...
func EncodeTSQuery(query TSQuery, appendTo []byte) []byte {
	...
	var encoder tsNodeEncoder
	appendTo = encoder.encodeTSNode(query.root, appendTo)
	return encoding.PutUint16Ascending(appendTo, uint16(encoder.nNodes), lengthIdx)
}

type tsNodeEncoder struct {
	nNodes int
}

func (e *tsNodeEncoder) encodeTSNode(node *tsNode, appendTo []byte) []byte {
	e.nNodes++
	if node.op == invalid {
		appendTo = append(appendTo, byte(1))
	...
	appendTo = e.encodeTSNode(node.l, appendTo)
	if node.r != nil {
		appendTo = e.encodeTSNode(node.r, appendTo)
	}
	return appendTo
}
