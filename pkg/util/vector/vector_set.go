...
// MakeVectorSet constructs a new empty vector set with the given number of
// dimensions. New vectors can be added via the Add or AddSet methods.
func MakeVectorSet(dims int) Set {
	return Set{Dims: dims}
}

// MakeVectorSetFromRawData constructs a new vector set from a raw slice of
// vectors. The vectors in the slice have the given number of dimensions and are
// laid out contiguously in row-wise order.
// NB: The data slice is directly used rather than copied; any outside changes
// to it will be reflected in the vector set.
func MakeVectorSetFromRawData(data []float32, dims int) Set {
	if len(data)%dims != 0 {
