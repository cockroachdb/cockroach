...
func TestParseDate(t *testing.T) {
	tests := []struct {
		...
		err bool
		// Override the expectetd value for a given ParseMode.
		...
		t.Run(mode.String(), func(t *testing.T) {
			for _, tc := range tests {
				t.Run(tc.s, func(t *testing.T) {
					ctx := pgdate.ISOContext()
					ctx = pgdate.WithParseMode(ctx, mode)
					ctx = pgdate.WithExposedErrors(ctx)
					res, err := pgdate.ParseDate(ctx, tc.s)
...
func TestOneOff(t *testing.T) {
	ctx := pgdate.WithParseMode(pgdate.ISOContext(), pgdate.ParseModeISO)
	res, err := pgdate.ParseDate(ctx, "2017-12-05 04:04:04.913231+00:00")
	t.Logf("%v %v", res, err)
}
