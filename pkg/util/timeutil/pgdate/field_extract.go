...
type fieldExtract struct {
	...
	wanted fieldSet
	// Tracks whether the current timestamp formes db2 format
	isDB2 bool
}
...
func (fe *fieldExtract) interpretNumber(numbers []numberChunk, idx int, textMonth bool) error {
	...
	case chunk.separator == '.':
		...
		//               ^^^
		// Example: 04.04.04.913231+00:00, odb timestamp
		switch {
...
func (fe *fieldExtract) validate() error {
	...
	if (fe.has.HasAny(timeRequiredFields) || fe.isDB2) && !fe.has.HasAll(timeRequiredFields) {
