...
func (fe *fieldExtract) Extract(s string) error {
	...
	if count < 0 {
		return errors.New("too many chunks in input")
	} else if count == 0 {
		return errors.New("empty or blank input")
	}
	...
	appendNumber := func(prefix, number string) error {
		...
		for _, r := range prefix {
			...
			default:
				return errors.Errorf(`detected multiple separators in "%s""`, prefix)
			}
		}
		...
		default:
			...
			if m, ok := keywordSetters[match]; ok {
				...
				if idx == 2 && fe.Wants(fieldYear) && !fe.Wants(fieldMonth) && fe.Wants(fieldDay) {
					return errors.New("no such thing as a YDM or DYM format")
				}
				continue
			}
...
func badFieldPrefixError(field field, prefix rune) error {
	return errors.Errorf("unexpected separator %v for field %s", prefix, field.String()[5:])
}
