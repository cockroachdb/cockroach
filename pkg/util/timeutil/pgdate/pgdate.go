...
var errDateOutOfRange = errpgcode.WithCandidateCode(errors.New("date is out of range"), pgcode.DatetimeFieldOverflow)
...
func (d Date) ToTime() (time.Time, error) {
	if d.days == math.MinInt32 || d.days == math.MaxInt32 {
		return time.Time{}, errpgcode.WithCandidateCode(
...
func (d Date) AddDays(days int64) (Date, error) {
	...
	if !ok {
		return Date{}, errpgcode.WithCandidateCode(
