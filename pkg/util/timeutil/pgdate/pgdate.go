...
// Date is a postgres-compatible date implementation. It stores the number
// of seconds from the postgres epoch (2000-01-01). Its properties are
// unexported so that it cannot be misused by external packages. This
...
type Date struct {
	...
	// orig is the original on-disk number of days. It is only set
	// when creating a Date from an on-disk encoding and will be used
	// instead of days when encoding for on-disk. This is required
	// so that we can roundtrip to the same on-disk value, which is
	// necessary deleting old index entries. This value should never
	// be copied to another Date, but always cleared and forgotten on
	// any mutation.
	hasOrig bool
	orig    int64
}
...
func MakeDateFromTime(t time.Time, loc *time.Location) (Date, error) {
	t = t.In(loc)
	sec := t.Unix()
	_, offset := t.Zone()
	sec += int64(offset)
	days := sec / secondsPerDay
	return MakeDateFromUnixEpoch(days)
}
...
func MakeDateFromUnixEpoch(days int64) (Date, error) {
	...
	if !ok || days <= math.MinInt32 || days >= math.MaxInt32 {
		return Date{}, pgerror.NewError(pgerror.CodeDatetimeFieldOverflowError,
...
func makeDateFromYMD(year, month, day int) Date {
	...
	d, err := MakeDateFromTime(t, time.UTC)
	if err != nil {
		panic(err)
	}
	return d
...
func (d Date) ToTime(loc *time.Location) (time.Time, error) {
	if d.days == math.MinInt32 || d.days == math.MaxInt32 {
		return time.Time{}, pgerror.NewErrorf(pgerror.CodeDatetimeFieldOverflowError,
			"%s out of range for timestamp", d)
	}
	return timeutil.Unix(d.UnixEpochDays()*secondsPerDay, 0).In(loc), nil
}
...
func (d Date) AddDays(days int64) (Date, error) {
	...
	if !ok {
		return Date{}, pgerror.NewErrorf(pgerror.CodeDatetimeFieldOverflowError,
