...
// FixedTimeZoneStringToLocation creates a time.Location with an offset and a
// time zone string.
func FixedTimeZoneStringToLocation(offset int, origRepr string) *time.Location {
...
// by FixedTimeZoneStringToLocation().
func TimeZoneStringToLocation(
	...
	if err == nil {
		...
		if intVal < -maxUTCHourOffset || intVal > maxUTCHourOffset {
			return nil, pgerror.New(pgcode.InvalidParameterValue, "UTC timezone offset is out of range.")
		}
	...
	return FixedTimeZoneStringToLocation(int(tzOffset), locStr), nil
}
