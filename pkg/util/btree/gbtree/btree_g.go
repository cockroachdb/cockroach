...
import "github.com/cockroachdb/cockroach/pkg/util/btree"
...
func New[T any](cmp func(T, T) int) *BTreeG[T] {
	return &BTreeG[T]{s: btree.NewSetConfig(cmp).MakeSet(), cmp: cmp}
}
...
func (a *BTreeG[T]) AscendLessThan(pivot T, iterator ItemIteratorG[T]) {
	...
	it.First()
	for ; it.Valid(); it.Next() {
		if a.cmp(it.Cur(), pivot) >= 0 || !iterator(it.Cur()) {
			return
		}
	}
}
...
func (a *BTreeG[T]) AscendRange(greaterOrEqual, lessThan T, iterator ItemIteratorG[T]) {
	it := a.s.Map.MakeIter()
	it.SeekGTE(greaterOrEqual)
	for ; it.Valid(); it.Next() {
		if a.cmp(it.Cur(), lessThan) >= 0 || !iterator(it.Cur()) {
			return
		}
	}
}
...
func (a *BTreeG[T]) Clone() (t2 *BTreeG[T]) {
	return &BTreeG[T]{s: a.s.Clone(), cmp: a.cmp}
}
...
func (a *BTreeG[T]) DescendRange(lessOrEqual, greaterThan T, iterator ItemIteratorG[T]) {
	it := a.s.Map.MakeIter()
	it.SeekLTE(lessOrEqual)
	for ; it.Valid(); it.Prev() {
		if a.cmp(it.Cur(), greaterThan) <= 0 || !iterator(it.Cur()) {
			return
		}
	}
}
