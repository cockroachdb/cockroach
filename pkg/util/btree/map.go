...
func (c *MapConfig[K, V]) MakeMap() Map[K, V] {
	return Map[K, V]{
		m: (*abstract.Config[K, V, struct{}, updater[K, V]])(c).MakeMap(),
	}
}

// Map is a ordered map from K to V.
type Map[K, V any] struct {
	m abstract.Map[K, V, struct{}, updater[K, V]]
}
...
func (m *Map[K, V]) MakeIter() Iterator[K, V] {
	var it Iterator[K, V]
