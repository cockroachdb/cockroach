...
func (it *Iterator[I, K, V]) NextOverlap(item I) {
	if it.it.Node == nil {
		return
	}
	it.it.Pos++
...
func (it *Iterator[I, K, V]) constrainMinSearchBounds(cmp Cmp[K], item I) {
	k := item.Key()
	n := it.it.Node
	j := sort.Search(int(n.Count), func(j int) bool {
		return cmp(k, n.Keys[j].Key()) <= 0
	})
	it.o.constrMinN = n
	it.o.constrMinPos = int16(j)
}

func (it *Iterator[I, K, V]) constrainMaxSearchBounds(cmp Cmp[K], item I) {
	up := upperBound[K](item)
	n := it.it.Node
	j := sort.Search(int(n.Count), func(j int) bool {
		return !up.contains(cmp, n.Keys[j].Key())
	})
	it.o.constrMaxN = n
	it.o.constrMaxPos = int16(j)
}

func (it *Iterator[I, K, V]) findNextOverlap(item I) {
	for {
		if it.it.Pos > it.it.Node.Count {
			...
			if it.o.constrMinReached ||
				it.it.Node.Children[it.it.Pos].Aug.
					contains(it.it.Updater.cmp, item.Key()) {
				...
				if par == it.o.constrMinN && pos == it.o.constrMinPos {
					it.constrainMinSearchBounds(it.it.Updater.cmp, item)
				}
				if par == it.o.constrMaxN && pos == it.o.constrMaxPos {
					it.constrainMaxSearchBounds(it.it.Updater.cmp, item)
				}
				continue
			}
		}
		...
		if pos < n.Count {
			...
			ub := upperBound[K](it.it.Cur())
			if ub.contains(it.it.Updater.cmp, item.Key()) {
				return
			}
		}
		it.it.Pos++
