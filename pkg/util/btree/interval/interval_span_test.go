...
func (l *latch) UpperBound() (Key, bool) {
	if l.span.endKey == nil {
		return l.span.key, true
	}
	return l.span.endKey, false
}

func (l *latch) EndKey() Key {
	if l.span.endKey == nil {
		return l.span.key
	}
...
type btree = Set[*latch, Key]

var latchSetConfig = NewSetConfig(Comparators[*latch, Key]{
	CmpK: Key.Compare,
	CmpI: compareLatches,
})

func makeBTree() btree {
	return latchSetConfig.MakeSet()
}
...
func TestBTree(t *testing.T) {
	...
	for i := 0; i < count; i++ {
		tr.Upsert(newLatch(span(count - i)))
		//tr.Verify(t)
		if e := i + 1; e != tr.Len() {
			t.Fatalf("expected length %d, but found %d", e, tr.Len())
		}
...
func TestBTreeSeek(t *testing.T) {
	...
	tr := makeBTree()
	for i := 0; i < count; i++ {
		tr.Upsert(newLatch(span(i * 2)))
	}

	it := tr.MakeIter()
	for i := 0; i < 2*count-1; i++ {
		it.SeekGTE(newLatch(span(i)))
		if !it.Valid() {
	...
	it.SeekGTE(newLatch(span(2*count - 1)))
	if it.Valid() {
...
func all(tr *Set[*latch, Key]) (out []*latch) {
	it := tr.MakeIter()
	it.First()
	for it.Valid() {
