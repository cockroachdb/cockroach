...
func (u *UUID) UnmarshalText(text []byte) error {
	...
	if l < 32 || l > 48 {
		return errors.Newf(
			"uuid: %s",
			redact.Safe(fmt.Sprintf("incorrect UUID length: %s", text)),
		)
...
func (u *UUID) decodeHyphenated(t []byte) error {
	...
	if l < 32 || l > 40 {
		return errors.Newf(
			"uuid: %s",
			redact.Safe(fmt.Sprintf("incorrect UUID format: %s", t)),
		)
	}
	...
	if i != len(hashLike) {
		return errors.Newf("uuid: %s", redact.Safe(fmt.Sprintf("incorrect UUID format: %s", t)))
	}
...
func (u *UUID) UnmarshalBinary(data []byte) error {
	if len(data) != Size {
		return errors.Newf(
			"uuid: %s",
			redact.Safe(fmt.Sprintf("UUID must be exactly 16 bytes long, got %d bytes", len(data))),
		)
	}
	copy(u[:], data)
