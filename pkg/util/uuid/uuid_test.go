...
func TestUUID(t *testing.T) {
	...
	t.Run("WriteShort", testUUIDShort)
	t.Run("WriteShort", testUUIDShortBytes)
}
...
func testUUIDShortBytes(t *testing.T) {
	got := NamespaceDNS.ShortBytes()
	want := [8]byte([]byte("6ba7b810"))
	if got != want {
		t.Errorf("%v.ShortBytes() = %q, want %q", NamespaceDNS, got, want)
	}
}
