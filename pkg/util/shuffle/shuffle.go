...
// collection be enumerated by an integer index. This interface is similar to
// sort.Interface.
...
// Between shuffles only the data between the start and end index.
func Between(data Interface, startIndex int, endIndex int) {
	length := endIndex - startIndex + 1
	for i := 1; i < length; i++ {
		data.Swap(startIndex+i, startIndex+rand.Intn(i+1))
	}
}
