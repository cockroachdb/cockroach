...
func TestToUInt64(t *testing.T) {
	testData := []struct {
		...
		{`0100111011001010111111101011101010111110110010101101110100011110`, 5677630358272269598},
		{"1010101010101" + "0100111011001010111111101011101010111110110010101101110100011110", 5677630358272269598},
		{"0100111011001010111111101011101010111110110010101101110100011110" +
			"0100111011001010111111101011101010111110110010101101110100011110" +
			"0100111011001010111111101011101010111110110010101101110100011110",
			5677630358272269598},
		{"0" + "0100111011001010111111101011101010111110110010101101110100011110" +
			"0100111011001010111111101011101010111110110010101101110100011110" +
			"0100111011001010111111101011101010111110110010101101110100011110",
			5677630358272269598},
	}
	...
	for _, test := range testData {
		t.Run(fmt.Sprintf("%s", test.str), func(t *testing.T) {
			ba, err := Parse(test.str)
			if err != nil {
				t.Fatal(err)
			}
			...
			res := ba.AsUInt64()
			if res != test.exp {
				t.Fatalf("expected %d (%b), got %d (%b)", test.exp, test.exp, res, res)
			}
		})
	}
}
