...
func Parse(s string) (res BitArray, err error) {
	...
	for _, c := range s {
		...
		if bitVal != val {
			...
			return res, errpgcode.WithCandidateCode(err, pgcode.InvalidTextRepresentation)
		}
...
func FromEncodingParts(words []uint64, lastBitsUsed uint64) (BitArray, error) {
	if lastBitsUsed > numBitsPerWord {
		...
		return BitArray{}, errpgcode.WithCandidateCode(err, pgcode.InvalidParameterValue)
	}
	return BitArray{
		words:        words,
