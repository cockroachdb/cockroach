...
func (d BitArray) GetBitAtIndex(index int) (int, error) {
	// Checking whether index asked is inside BitArray.
	if index < 0 || uint(index) >= d.BitLen() {
		err := fmt.Errorf("GetBitAtIndex: bit index %d out of valid range (0..%d)", index, d.BitLen()-1)
...
func (d BitArray) SetBitAtIndex(index, toSet int) (BitArray, error) {
	...
	if index < 0 || uint(index) >= res.BitLen() {
		err := fmt.Errorf("SetBitAtIndex: bit index %d out of valid range (0..%d)", index, res.BitLen()-1)
