...
//
// This uses a packed encoding (i.e. multiple bits per word) for
// memory efficiency and speed of bitwise operations, but there is
// something fancy needed to handle sorting values properly: the last
// group of bits must be right padded inside its word to compare
// properly according to pg semantics.
//
...
func (d BitArray) ToWidth(n uint) BitArray {
	bitlen := d.BitLen()
	if bitlen == n {
	...
	if n < bitlen {
		...
		words, lastBitsUsed := EncodingPartsForBitLen(n)
	...
	numWords, lastBitsUsed := SizesForBitLen(n)
	var words []word
...
// Size returns the size in bytes of the bit array and its components.
func (d BitArray) Size() uintptr {
...
func MakeBitArrayFromInt64(bitLen uint, val int64, valWidth uint) BitArray {
	...
	val = val >> (numBitsPerWord - valWidth)
	// Store the left part of the value in the last word.
	d.words[len(d.words)-1] = word(val << (numBitsPerWord - d.lastBitsUsed))
	// Store the right part in the next-to-last word, if any.
...
func Not(d BitArray) BitArray {
	...
	if res.lastBitsUsed > 0 {
		...
		res.words[lastWord] &= (^word(0) << (64 - res.lastBitsUsed))
	}
	return res
}
...
func (d BitArray) EncodingParts() ([]uint64, uint64) {
	return []uint64(d.words), uint64(d.lastBitsUsed)
}
...
// Range generates a random bit array of the specified length.
func Rand(rsg *rand.Rand, bitLen uint) BitArray {
	d := MakeZeroBitArray(bitLen)
	for i := range d.words {
		d.words[i] = rsg.Uint64()
	}
	if len(d.words) > 0 {
