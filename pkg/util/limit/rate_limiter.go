...
func NewLimiter(r rate.Limit) *LimiterBurstDisabled {
	// Unfortunately there is no way to disable the burst parameter on the
	// limiter, so we have to provide some value to it. To remove the cognitive
	// burden from the user, we set this value to be equal to the limit, which
	// is a sensible default given the situation.
