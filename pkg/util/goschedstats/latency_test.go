...
func TestSchedulerLatencyListener(t *testing.T) {
	...
	slcbID := RegisterSchedulerLatencyCallback(func(p99 time.Duration, period time.Duration) {
		require.Equal(t, schedulerLatencySamplePeriod.Default(), period)
		mu.Lock()
		defer mu.Unlock()
		mu.p99 = p99
	})
	defer UnregisterSchedulerLatencyCallback(slcbID)

	require.NoError(t, StartStatsTicker(ctx, st, stopper))
