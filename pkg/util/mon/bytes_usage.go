...
// ExportTree exports the current state of the tree of monitors rooted in the
// BytesMonitor.
//
...
// exportTree recursively traverses the tree of monitors rooted in the current
// monitor. If the monitor is stopped, then the tree is not traversed and nil
// slice is returned.
func (mm *BytesMonitor) exportTree(level int) []ExportEntry {
	...
	for _, c := range children {
		// Note that if this child has been stopped, this call will return nil
		// and nothing will be appended to entries.
