...
func (mm *BytesMonitor) doStop(ctx context.Context, check bool) {
	...
	if check && mm.mu.curAllocated != 0 {
		...
		mm.releaseBytesAlreadyLocked(ctx, mm.mu.curAllocated)
	}
...
func (mm *BytesMonitor) releaseBytes(ctx context.Context, sz int64) {
	mm.mu.Lock()
	defer mm.mu.Unlock()
	mm.releaseBytesAlreadyLocked(ctx, sz)
}

// releaseBytesAlreadyLocked is similar to releaseBytes but requires that mm.mu
// has already been locked.
func (mm *BytesMonitor) releaseBytesAlreadyLocked(ctx context.Context, sz int64) {
	mm.mu.AssertHeld()
