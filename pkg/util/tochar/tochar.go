...
func timeToChar(t time.Time, formatNodes []formatNode) (string, error) {
	...
	for _, fn := range formatNodes {
		...
		case DCH_HH, DCH_HH12:
			...
			sb.WriteString(fmt.Sprintf("%0*d", fn.suffix.zeroPadHMS(t.Hour()), hour))
			sb.WriteString(fn.suffix.thVal(hour))
		case DCH_HH24:
			sb.WriteString(fmt.Sprintf("%0*d", fn.suffix.zeroPadHMS(t.Hour()), t.Hour()))
			sb.WriteString(fn.suffix.thVal(t.Hour()))
		case DCH_MI:
			sb.WriteString(fmt.Sprintf("%0*d", fn.suffix.zeroPadHMS(t.Minute()), t.Minute()))
			sb.WriteString(fn.suffix.thVal(t.Minute()))
		case DCH_SS:
			sb.WriteString(fmt.Sprintf("%0*d", fn.suffix.zeroPadHMS(t.Second()), t.Second()))
		...
		case DCH_MM:
			val := int(t.Month())
			sb.WriteString(fmt.Sprintf("%0*d", fn.suffix.zeroPadHMS(val), val))
			sb.WriteString(fn.suffix.thVal(val))
		...
		case DCH_YYYY, DCH_IYYY, DCH_YYY, DCH_IYY, DCH_YY, DCH_IY, DCH_Y, DCH_I:
			...
			sb.WriteString(fn.suffix.thVal(val))
		case DCH_RM, DCH_rm:
			// Only useful for intervals, blocking for now.
			return "", pgerror.Newf(
...
func (d dchSuffix) zeroPadHMS(val int) int {
	var zeroPad int
	if !d.FM() {
		// I think this logic makes more sense for intervals, but keeping it
		// just in case for now.
		if val >= 0 {
			zeroPad = 2
		} else {
			// NOTE(otan): i think this only gets hit for intervals, which we don't support atm.
			zeroPad = 3
		}
	}
	return zeroPad
...
func indexSeqSearch(s string) *keyWord {
	...
	if idxOffset > -1 && idxOffset < len(dchIndex) {
		...
		for pos >= 0 && pos < len(dchKeywords) {
			...
			pos++
			// Since we have dchKeywords sorted in alphabetical order, if don't match
			// the first character, break.
