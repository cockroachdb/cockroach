...
func fromCharParseIntLen(
	s string, pos int, maxLen int, fm bool, nodeName string,
) (int, int, error) {
	if pos >= len(s) {
	...
	for _, fn := range formatNodes {
		...
		case formatNodeChar, formatNodeSeparator:
			...
			if pos < len(s) {
				if fx {
					...
					expectR, _ := utf8.DecodeRuneInString(fn.character)
					if r != expectR {
						return pgerror.Newf(
							pgcode.InvalidDatetimeFormat,
							"invalid value for \"%s\"",
		...
		case formatNodeAction:
			...
			case DCH_HH, DCH_HH12:
				val, n, err := fromCharParseIntLen(s, pos, 2, fm, fn.key.name)
				if err != nil {
					return err
				}
			...
			case DCH_MS:
				val, n, err := fromCharParseIntLen(s, pos, 3, fm, fn.key.name)
				if err != nil {
					return err
				}
			...
			case DCH_US:
				val, n, err := fromCharParseIntLen(s, pos, 6, fm, fn.key.name)
				if err != nil {
					return err
				}
			...
			case DCH_FF1, DCH_ff1:
				val, n, err := fromCharParseIntLen(s, pos, 1, fm, fn.key.name)
				if err != nil {
					return err
				}
			...
			case DCH_FF4, DCH_ff4:
				val, n, err := fromCharParseIntLen(s, pos, 4, fm, fn.key.name)
				if err != nil {
					return err
				}
				origN := n
			...
			case DCH_FF5, DCH_ff5:
				val, n, err := fromCharParseIntLen(s, pos, 5, fm, fn.key.name)
				if err != nil {
					return err
				}
				origN := n
			...
			case DCH_Y_YYY, DCH_y_yyy:
				...
				val2, n2, err := fromCharParseIntLen(s, pos, 3, fm, fn.key.name)
				if err != nil {
					return err
				}
				val = val*1000 + val2
			...
			case DCH_Q, DCH_q:
				...
				_, n, err := fromCharParseIntLen(s, pos, 1, fm, fn.key.name)
				if err != nil {
					return err
				}
				pos += n
			...
			case DCH_J, DCH_j:
				val, n, err := fromCharParseIntLen(s, pos, 7, fm, fn.key.name)
				if err != nil {
					return err
				}
...
func tmFromCharToTime(tmfc *tmFromChar, loc *time.Location) (time.Time, error) {
	...
	if tmfc.clock == 12 {
		...
		if tmfc.pm != fromCharFieldUnset && tmfc.pm == 1 && hour < 12 {
			hour += 12
		} else if (tmfc.pm == fromCharFieldUnset || tmfc.pm == 0) && hour == 12 {
			hour = 0
		}
	}
