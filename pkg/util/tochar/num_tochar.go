...
func numToCharEEEE(d *apd.Decimal, desc *numDesc) (string, error) {
	...
	// Format using Go's scientific notation, then adjust to PostgreSQL format.
	// PostgreSQL uses %+.*e format with desc.post decimal places.
	f, err := d.Float64()
	if err != nil {
		return "", err
	}
	orgnum := fmt.Sprintf("%+.*e", desc.post, f)
	// Swap leading '+' for ' '.
	if orgnum[0] == '+' {
		orgnum = " " + orgnum[1:]
	}
...
func numToCharNormal(d *apd.Decimal, nodes []numFormatNode, desc *numDesc) (string, error) {
	...
	if numStr == "NaN" {
		...
		return numProcessorToChar(nodes, desc, overflow, sign, desc.pre)
	}
