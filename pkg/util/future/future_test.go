...
func TestBrokenPromise(t *testing.T) {
	...
	// Sometimes promises are broken.  However, we still cleanup
	// the mess after.
	f := future.MakePromise[int]()
	...
	// Join future; this doesn't block.  It's the responsibility of the caller
	// ensure that future is waited on.  But, sometimes, early termination
	// needs to happen, and the resources acquired during execution must be released.
	// Join does that.
	f.Join()
	<-cleanedup
}
