...
type Future[T any] struct {
	// values contains the actual value, along with any optional callbacks to
	// execute when value becomes ready. futureValue[T].v  may be set only once,
	// but any number of ready callbacks can be associated with future.
...
// Wait waits until future becomes ready and returns its value.
