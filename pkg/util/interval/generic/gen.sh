...
#    file, and the stripped contract file and pass this all to go_generics.
# 3. Crlfmt the result because go_generics might re-order imports.
# 4. remove the temporary stripped contract file.
grep -vE '(//|package)' ${internal_pkg}/contract.go > ${internal_pkg}/contract_stripped.tmp
for template in "${templates[@]}" ; do
    dst=${dst_prefix}_${template}.go
    echo -e ${gen_header_comment} \
    | cat - ${internal_pkg}/${template}_tmpl.go ${internal_pkg}/contract_stripped.tmp \
