...
func (n *rangeListNode) findIdx(r Range, inclusive bool) (int, bool) {
	for i, nr := range n.slots[:n.len] {
		if inclusive {
			switch {
			case InclusiveOverlapper.Overlap(nr, r):
				return i, true
			case r.End.Compare(nr.Start) < 0:
				// Past where inclusive overlapping ranges would be.
				return i, false
			}
		} else {
			switch {
			case ExclusiveOverlapper.Overlap(nr, r):
				return i, true
			case r.End.Compare(nr.Start) <= 0:
...
func (rl *rangeList) findNode(r Range, inclusive bool) (prev, cur *list.Element, inCur bool) {
	...
	for e := rl.ll.Front(); e != nil; e = e.Next() {
		...
		if inclusive {
			// Check if the range starts at a value less than or equal to the
			// maximum value in this node. This is what we want.
			if n.max().Compare(r.Start) >= 0 {
				// Determine whether the range inclusively overlap the node's bounds.
				inCur := n.min().Compare(r.End) <= 0
				return prev, e, inCur
			}
		} else {
			// Check if the range starts at a value less than the maximum value
			// in this node. This is what we want.
			if n.max().Compare(r.Start) > 0 {
				// Determine whether the range exclusively overlap the node's bounds.
				inCur := n.min().Compare(r.End) < 0
				return prev, e, inCur
			}
		}
...
func (rl *rangeList) Add(r Range) bool {
	...
	if cur != nil {
		...
		for {
			...
			for ; j < origLen; j++ {
				...
				if InclusiveOverlapper.Overlap(newR, mergeR) {
			...
			if broke {
				// If we broke, that means index j and up are still needed in
				// the current node. Shift these over.
	...
	if prevN != nil {
		// There is a previous node and it is full. If it was not, we would have
		// fell in one of the two cases above.
...
func (rl *rangeList) Sub(r Range) bool {
	...
	for {
		nr := n.slots[i]
		if !ExclusiveOverlapper.Overlap(nr, r) {
