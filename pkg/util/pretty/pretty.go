...
// output. For unions, it takes the broken (y) branch, which has the
// shorter first line; under the union invariant (x's first line ≥
// y's first line), if the broken branch's first line doesn't fit,
// the flat branch's won't either.
func (b *beExec) fits(k docPos, xlist *iDoc) bool {
	for {
		...
		case line, softbreak, hardline:
			return true
		case *union:
			// Take the broken branch — it has the shorter first
			// line per the union invariant.
			xlist = b.newiDoc(d.i, t.y, z)
		case pad:
			k = docPos{k.tabs, k.spaces + t.n}
			xlist = z
