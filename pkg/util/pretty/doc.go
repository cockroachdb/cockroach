...
//
// In general, the paper describes a simple algorithm for printing a document
// tree with a single layout defined by text, newlines, and indentation. This
// concept is then expanded for documents that have more than one possible
// layout using a union type of two documents where both documents reduce
// to the same document, but one document has been flattened to a single
// line. A method ("best") is described that chooses the best of these
// two layouts (i.e., it removes all union types from a document tree). It
// works by tracking the desired and current line length and choosing either
// the flattened side of a union if it fits on the current line, or else
// the non-flattened side. The paper then describes various performance
// improvements that reduce the search space of the best function such that
// it can complete in O(n) instead of O(n^2) time.
package pretty
