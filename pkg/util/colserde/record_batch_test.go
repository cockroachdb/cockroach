...
import (
	...
	"github.com/cockroachdb/cockroach/pkg/util/colserde"
...
func randomDataFromType(
	rng *rand.Rand, dataType arrow.DataType, n int, nullProbability float64,
) *array.Data {
...
func TestRecordBatchSerializer(t *testing.T) {
	...
	typs := []arrow.DataType{arrow.PrimitiveTypes.Int64, arrow.PrimitiveTypes.Int64}

	t.Run("UnsupportedSchema", func(t *testing.T) {
		_, err := colserde.NewRecordBatchSerializer([]arrow.DataType{})
		require.True(t, testutils.IsError(err, "zero length"), err)
		_, err = colserde.NewRecordBatchSerializer([]arrow.DataType{arrow.FixedWidthTypes.Time32ms})
...
func TestRecordBatchSerializerSerializeDeserializeRandom(t *testing.T) {
	...
	var (
		supportedTypes  = colserde.SupportedTypes()
		typs            = make([]arrow.DataType, rng.Intn(maxTypes)+1)
...
func BenchmarkRecordBatchSerializerInt64(b *testing.B) {
	...
	var (
		typs             = []arrow.DataType{arrow.PrimitiveTypes.Int64}
		buf              = bytes.Buffer{}
