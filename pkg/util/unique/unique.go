...
// UniquifySpans takes as input a slice of Spans, and deduplicates them using a
// sort and unique. The output will not contain any duplicates but it will be
// sorted.
func UniquifySpans(slices []roachpb.Spans) []roachpb.Spans {
	if len(slices) == 0 {
		return slices
	}
	...
	sort.Slice(slices, func(i int, j int) bool {
		return slices[i].Compare(slices[j]) < 0
	})

	// Then distinct.
	lastUniqueIdx := 0
	for i := 1; i < len(slices); i++ {
		if slices[i].Compare(slices[lastUniqueIdx]) != 0 {
			// We found a unique entry, at index i. The last unique entry in the array
			// was at lastUniqueIdx, so set the entry after that one to our new unique
			// entry, and bump lastUniqueIdx for the next loop iteration.
			lastUniqueIdx++
