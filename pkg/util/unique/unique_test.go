...
func TestUniquifyAcrossSlices(t *testing.T) {
	...
	for i, tt := range tests {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			UniquifyAcrossSlices(&tt.left, &tt.right,
				func(l, r int) int {
			...
			if !reflect.DeepEqual(tt.left, tt.expectedLeft) {
				t.Errorf("expected %v, got %v", tt.expectedLeft, tt.left)
			}
			if !reflect.DeepEqual(tt.right, tt.expectedRight) {
				t.Errorf("expected %v, got %v", tt.expectedRight, tt.right)
			}
		})
	}
}
