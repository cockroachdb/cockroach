...
const (
	...
	// RootSpan makes the task's span be a root, with no reference to the caller's
	// span. Use this when the caller is a long-running background process.
	RootSpan

	// NoSpan makes the task not have a span. The task will not run in the
	// caller's span; it will simply run in a context with no span. Use this when
	// creating spans for the task is not useful (perhaps because the task is trivial).
	NoSpan
)
...
func (s *Stopper) RunAsyncTaskEx(ctx context.Context, opt TaskOpts, f func(context.Context)) error {
	...
	case RootSpan:
		ctx, sp = s.tracer.StartSpanCtx(ctx, opt.TaskName)
	case NoSpan:
		ctx = tracing.ContextWithoutSpan(ctx)
	default:
