...
const (
	...
	// Sterile makes the task run in a span that doesn't get any children.
	// Anybody trying to create a child of the task's span will get a root span.
	// This is suitable for long-running tasks. Connecting children to these tasks
	// would lead to infinitely-long traces.
	//
	// The task's span will also not have a relationship to the caller's span
	// (i.e. the task will be a root span).
	Sterile
)
