...
func WithParent(sp *Span) SpanOption {
	...
	// we acquired above. On the happy path, the spanRef will be moved into the
	// child-to-be, which will own it (it will release() it when either the child
	// or the parent are Finish()ed).
	ref := makeSpanRef(sp)
	defer ref.release()
	sp.decRef()
