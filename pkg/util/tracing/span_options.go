...
// must be passed to StartSpan(opt). The child span will be responsible for
// ultimately doing the decrement. The fact that WithParent takes a reference on
// sp is important to avoid the possibility of deadlocks when buggy code uses a
// Span after Finish() (which is illegal). See comments on Span.refCnt for
// details. By taking the reference, it becomes safe (from a deadlock
// perspective) to call WithParent(sp) concurrently with sp.Finish(). Note that
// calling WithParent(sp) after sp was re-allocated cannot result in deadlocks
