...
type spanOptions struct {
	...
	// parentDoesNotCollectRecording means that, although this span has a parent,
	// the parent should generally not include this child's recording when the
	// parent is asked for its own recording. Usually, a parent span includes all
	// its children in its recording. However, sometimes that's not desired;
	// sometimes the creator of a child span has a different plan for how the
	// recording of that child will end up being collected and reported to where
	// it ultimately needs to go. Still, even in these cases, a parent-child
