...
func TestSimpleFlightRecorder(t *testing.T) {
	...
	t.Run("metadata log event appears in trace snapshot", func(t *testing.T) {
		// The logMetadata callback should have been invoked at least once.
		require.Greater(t, metadataCallCount.Load(), int32(0),
			"logMetadata callback should have been invoked before WriteTo")
