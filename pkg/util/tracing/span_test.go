...
func TestSpanRecordLimit(t *testing.T) {
	...
	require.Equal(t, first.Fields[0].Value.StripMarkers(sp.Redactable()), msg(extra+1))
	require.Equal(t, last.Fields[0].Value.StripMarkers(sp.Redactable()), msg(numLogs+extra))
}
