...
func TestNonVerboseChildSpanRegisteredWithParent(t *testing.T) {
	...
	defer ch.Finish()
	require.Equal(t, 1, sp.i.crdb.mu.recording.childrenV2.len())
	require.Equal(t, ch.i.crdb, sp.i.crdb.mu.recording.childrenV2.get(0))
...
func TestSpanMaxChildren(t *testing.T) {
	...
	for i := 0; i < maxChildrenPerSpan+123; i++ {
		...
		require.Equal(t, exp, sp.i.crdb.mu.recording.childrenV2.len())
	}
}
