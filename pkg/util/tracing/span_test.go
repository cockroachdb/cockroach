...
func TestSpanRecordStructured(t *testing.T) {
	...
	sr := d1.Message.(*tracingpb.StructuredRecord)
	var d2 types.DynamicAny
	require.NoError(t, types.UnmarshalAny(sr.Payload, &d2))
...
func TestSpanRecordStructuredLimit(t *testing.T) {
	...
	srFirst := d1.Message.(*tracingpb.StructuredRecord)
	var d2 types.DynamicAny
	require.NoError(t, types.UnmarshalAny(srFirst.Payload, &d2))
	require.IsType(t, (*types.StringValue)(nil), d2.Message)

	var res string
	require.NoError(t, types.StdStringUnmarshal(&res, srFirst.Payload.Value))
	require.Equal(t, pad(extra+1), res)

	var d3 types.DynamicAny
	require.NoError(t, types.UnmarshalAny(last, &d3))
	srLast := d3.Message.(*tracingpb.StructuredRecord)
	require.NoError(t, types.StdStringUnmarshal(&res, srLast.Payload.Value))
