...
func TestStructuredRecordingFinishedChildrenDurations(t *testing.T) {
	checkChildrenMetadata := func(
		actualOperations map[string]tracingpb.RecordedSpan_OperationMetadata,
		expectedOperations map[string]tracingpb.RecordedSpan_OperationMetadata) {
		t.Helper()
	...
	require.Len(t, rec, 1)
	checkChildrenMetadata(rec[0].ChildrenMetadata, map[string]tracingpb.RecordedSpan_OperationMetadata{})
...
func TestFinishedChildrenMetadata(t *testing.T) {
	checkChildrenMetadata := func(
		sp *Span, expectedOperations map[string]tracingpb.RecordedSpan_OperationMetadata) {
		t.Helper()
		c := sp.i.crdb
		c.mu.Lock()
		defer c.mu.Unlock()
		require.Len(t, c.mu.childrenMetadata, len(expectedOperations))
		for opName, actualMetadata := range c.mu.childrenMetadata {
			expectedMetadata, ok := expectedOperations[opName]
			require.True(t, ok)
			require.Greater(t, actualMetadata.Duration, time.Duration(0))
			require.Equal(t, expectedMetadata.Count, actualMetadata.Count)
	...
	fn := func(recordingMode RecordingType) {
		tr := NewTracer()
		...
		ggc1.Finish()
		checkChildrenMetadata(gc1, map[string]tracingpb.RecordedSpan_OperationMetadata{
			"greatgrandchild1": {Count: 1},
		})

		ggc2.Finish()
		checkChildrenMetadata(gc2, map[string]tracingpb.RecordedSpan_OperationMetadata{
			"greatgrandchild2": {Count: 1},
		})

		// Finish() one of the grand children.
		gc1.Finish()
		checkChildrenMetadata(child, map[string]tracingpb.RecordedSpan_OperationMetadata{
			"grandchild1":      {Count: 1},
			"greatgrandchild1": {Count: 1},
		})
		...
		child.Finish()
		checkChildrenMetadata(root, map[string]tracingpb.RecordedSpan_OperationMetadata{
			"child":            {Count: 1},
			"grandchild1":      {Count: 1},
			"greatgrandchild1": {Count: 1},
