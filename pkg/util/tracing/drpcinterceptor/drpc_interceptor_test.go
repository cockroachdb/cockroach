...
import (
	...
	"github.com/cockroachdb/cockroach/pkg/util/tracing/drpcinterceptor/tracingdrpcinterceptorspb"
...
func (s *testServer) ClientStreamingCall(
	stream tracingdrpcinterceptorspb.DRPCTestService_ClientStreamingCallStream,
) error {
	//TODO check for span in the context when server interceptors are implemented
	...
	return stream.SendAndClose(&tracingdrpcinterceptorspb.ResponseMessage{
		Message: msg.Message,
	})
}

func checkForSpan(ctx context.Context) error {
	...
	if len(recs) != 1 {
		return errors.Newf("expected exactly one recorded span, not %+v", recs)
	}
	return nil
}
...
func TestDRPCInterceptors(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		do          func(
			context.Context,
			tracingdrpcinterceptorspb.DRPCTestServiceClient,
		) (*tracingdrpcinterceptorspb.ResponseMessage, error)
	}{
		{
			name: "UnaryCall",
			do: func(
				ctx context.Context,
				c tracingdrpcinterceptorspb.DRPCTestServiceClient,
			) (*tracingdrpcinterceptorspb.ResponseMessage, error) {
		...
		},
		{
			...
			do: func(
				ctx context.Context,
				client tracingdrpcinterceptorspb.DRPCTestServiceClient,
			) (*tracingdrpcinterceptorspb.ResponseMessage, error) {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// Create a dialer function that connects to the test server
			dialer := func(ctx context.Context) (drpc.Conn, error) {
				// Dial the server
				conn := drpcconn.New(c2)
				// Create a basic DRPC connection
				return conn, nil
			}
			...
			client := tracingdrpcinterceptorspb.NewDRPCTestServiceClient(clientConn)
			...
			log.Infof(bgCtx, "span: %+v", rootSpan)
			log.Infof(bgCtx, "root context: %+v", rootCtx)
