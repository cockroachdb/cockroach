...
func (t *Tracer) StartSpan(operationName string, os ...SpanOption) *Span {
	...
	// TODO(during review): `./scripts/goescape ./pkg/util/tracing/tracer.go` claims that
	// opts doesn't escape here (or anywhere else). How is this possible? I see that escape
	// analysis learns that for each `func WithX`, Apply(opts) does not let opts escape.
	// But other packages can implement the SpanOption interface just the same, so if they
	// did and called it, wouldn't it cause the var to escape? Unfortunately building `./pkg/cmd/cockroach`
	// with `-m` does not produce the escape analysis output for all children.
	for _, o := range os {
		o.Apply(&opts)
	}
