...
// RecordableRootOpt specifies whether a root span should be recordable.
type RecordableRootOpt bool

const (
	// RecordableRoot means that the root span will be recordable. This means that
	// a real span will be created (and so it carries a cost).
	RecordableRoot RecordableRootOpt = true
	// NonRecordableRoot means that the root span will be not recordable. This
	// means that the static noop span might be returned.
	NonRecordableRoot RecordableRootOpt = false
)
...
func (t *Tracer) StartRootSpan(
	opName string, logTags *logtags.Buffer, recordable RecordableRootOpt,
) opentracing.Span {
	shadowTracer := t.getShadowTracer()

	// In the usual case, we return noopSpan.
	if !t.useNetTrace() && shadowTracer == nil &&
		!t.forceRealSpans && recordable == NonRecordableRoot {
		return &t.noopSpan
	}
...
func EnsureContext(
	...
	if opentracing.SpanFromContext(ctx) == nil {
		sp := tracer.(*Tracer).StartRootSpan(opName, logtags.FromContext(ctx), NonRecordableRoot)
...
func EnsureChildSpan(
	...
	if opentracing.SpanFromContext(ctx) == nil {
		sp := tracer.(*Tracer).StartRootSpan(name, logtags.FromContext(ctx), NonRecordableRoot)
