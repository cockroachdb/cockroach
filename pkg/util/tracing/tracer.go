...
func (t *Tracer) getLightstepSpanIDs(
	...
	carrier := &lightstepExtractIDsCarrier{}
...
func (t *Tracer) StartSpan(
	...
	if t.isNoop() {
		// Fast paths to avoid the allocation of sso when tracing is disabled: if we
		// have no options or a single SpanReference (the common case) with a noop
		// context, return a noop span now.
		switch len(opts) {
		case 0:
			return &t.noopSpan
		case 1:
	...
	err := mapReader.ForeachKey(func(k, v string) error {
		...
		default:
			...
			if strings.HasPrefix(lowercaseK, prefixBaggage) {
				...
				sc.Baggage[strings.TrimPrefix(lowercaseK, prefixBaggage)] = v
			}
		}
		return nil
	})
