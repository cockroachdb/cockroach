...
func (t *Tracer) StartSpan(operationName string, os ...SpanOption) *Span {
	...
	// TODO(tbg): opts escapes to the heap, but one simple way to make that a non-issue
	// is to use a `sync.Pool` here. Alternatively, we can change the Apply API to take
	// and return a SpanOptions by value. Finally, we could also use the more efficient
	// though less composable pattern `SpanOptions{...}.StartSpan(t)`.
