...
// testingAll returns (pointers to) all the spans in the registry, in a random
// order. Since spans can generally finish at any point and use of a finished
// span is not permitted, this method is only suitable for tests.
...
func (t *Tracer) startSpanGeneric(
	...
	if traceID == 0 {
		traceID = tracingpb.TraceID(uint64(rand.Int63()))
	}
