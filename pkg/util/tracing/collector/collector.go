...
type Iterator struct {
	...
	// curNode maintains the nodeV from which the iterator has pulled inflight
	// span recordings and buffered them in `recordedSpans` for consumption via
	// the iterator.
	curNode roachpb.NodeID

	// recordingIndex maintains the current position of of the iterator in the
	// list of tracing.Recordings. The tracing.Recording that the iterator points
	// to is buffered in `recordings`.
	recordingIndex int

	// recordings represent the all the tracing.Recordings for a given node
