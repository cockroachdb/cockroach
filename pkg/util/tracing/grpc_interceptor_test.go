...
func TestGRPCInterceptors(t *testing.T) {
	...
	s.RunAsyncTask(ctx, "serve", func(ctx context.Context) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			ctx, sp := tr.StartSpanCtx(ctx, tc.name, tracing.WithForceRealSpan())
...
Span /cockroach.testutils.grpcutils.GRPCTest/%[1]s:
  tags: component=gRPC span.kind=server test-baggage-key=test-baggage-value`,
	...
	require.NoError(t, tr.VisitSpans(func(sp *tracing.Span) error {
		t.Errorf("leaked span: %s", sp.GetRecording()[0].Operation)
		return nil
	}))
}
