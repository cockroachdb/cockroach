...
func newTracingClientStream(
	...
	*isFinished = 0
	...
	if ctx.Done() != nil {
		...
		// Well, that's mighty surprising.  The context is never done, so, we'll never
		// invoke finishFunc due to context completion.  It may be strange, but probably
		// not broken -- context.Background(), theoretically can be used.
	}
