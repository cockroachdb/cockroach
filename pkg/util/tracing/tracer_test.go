...
// getSpanOpsInActiveTracesWithFinishedFlag is a helper method that returns a
// map of spans in in-flight traces, keyed on operation names and with values
// representing whether the span is finished.
func getSpanOpsInActiveTracesWithFinishedFlag(t *testing.T, tr *Tracer) map[string]bool {
	t.Helper()

	spanOpsInActiveTracesWithFinishedFlag := make(map[string]bool)

	require.NoError(t, tr.VisitSpans(func(sp *Span) error {
		for _, rec := range sp.GetRecording() {
			spanOpsInActiveTracesWithFinishedFlag[rec.Operation] = rec.Finished
		}
		return nil
	}))

	return spanOpsInActiveTracesWithFinishedFlag
}

// getSortedSpanOpsInActiveTraces is a helper method that returns a sorted list
// of span operation names from in-flight traces.
func getSortedSpanOpsInActiveTraces(t *testing.T, tr *Tracer) []string {
	t.Helper()

	var spanOpsInActiveTraces []string

	require.NoError(t, tr.VisitSpans(func(sp *Span) error {
		for _, rec := range sp.GetRecording() {
			spanOpsInActiveTraces = append(spanOpsInActiveTraces, rec.Operation)
		}
		return nil
	}))

	sort.Strings(spanOpsInActiveTraces)
	return spanOpsInActiveTraces
}
...
func TestTracer_VisitSpans(t *testing.T) {
	...
	require.Equal(t, []string{"root", "root.child", "root.child.remotechilddone"}, getSortedSpanOpsInActiveTraces(t, tr1))
	require.Len(t, getSortedSpanOpsInActiveTraces(t, tr1), 3)
	require.Equal(t, []string{"root.child.remotechild"}, getSortedSpanOpsInActiveTraces(t, tr2))
	...
	require.Len(t, getSortedSpanOpsInActiveTraces(t, tr1), 0)
	require.Len(t, getSortedSpanOpsInActiveTraces(t, tr2), 0)
...
func TestSpanRecordingFinished(t *testing.T) {
	...
	sortedSpanOps := getSortedSpanOpsInActiveTraces(t, tr1)
	require.Equal(t, []string{"root", "root.child", "root.child.child", "root.child.remotechild"}, sortedSpanOps)
	activeSpanMap := getSpanOpsInActiveTracesWithFinishedFlag(t, tr1)
	for _, finished := range activeSpanMap {
