...
func TestTracerRecording(t *testing.T) {
	...
	noop2 := tr.StartSpan("noop2", WithRemoteParent(noop1.Context()))
	if !noop2.isNoop() {
	...
	s1 := tr.StartSpan("a", WithRealSpan)
	if s1.isNoop() {
		t.Error("WithRealSpan (but not recording) Span should not be noop")
	}
	if !s1.IsBlackHole() {
		t.Error("WithRealSpan Span should be black hole")
	}

	// Unless recording is actually started, child spans are still noop.
	noop3 := tr.StartSpan("noop3", WithRemoteParent(s1.Context()))
	if !noop3.isNoop() {
		t.Error("expected noop child Span")
	}
	noop3.Finish()
...
func TestStartChildSpan(t *testing.T) {
	tr := NewTracer()
	sp1 := tr.StartSpan("parent", WithRealSpan)
	sp1.StartRecording(SingleNodeRecording)
	sp2 := tr.StartChildSpan("child", sp1, nil /* logTags */, false /* recordable */, false /*separateRecording*/)
	sp2.Finish()
	sp1.Finish()
	...
	sp1 = tr.StartSpan("parent", WithRealSpan)
	sp1.StartRecording(SingleNodeRecording)
	sp2 = tr.StartChildSpan("child", sp1, nil /* logTags */, false /* recordable */, true /*separateRecording*/)
	sp2.Finish()
	sp1.Finish()
	if err := TestingCheckRecordedSpans(sp1.GetRecording(), `
		Span parent:
	...
	sp2 = tr.StartChildSpan(
		"child", sp1, logtags.SingleTagBuffer("key", "val"), false /* recordable */, false, /*separateRecording*/
	)
	sp2.Finish()
	sp1.Finish()
	if err := TestingCheckRecordedSpans(sp1.GetRecording(), `
...
func TestLightstepContext(t *testing.T) {
	...
	require.Equal(t, exp, s2.Context().Baggage)
