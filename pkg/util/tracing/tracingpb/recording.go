...
// reMinimalStrip strips from this kind of message (as seen in LogRecord.Message):
//
// util/tracing/tracingpb_test/recording_test.go:63 the answer to the universe and everything is ‹43›
// |----------strip--------------------------------|
//
// to capture only the "actual message", minus the file:line prefix.
var reMinimalStrip = regexp.MustCompile(`^event:.*?:[0-9]+ `)
...
func writeEntry(w redact.SafeWriter, entry traceLogData, start time.Time, mode formatMode) {
	...
	if mode == formatMinimal {
		// Strip file:line information and event: prefix from event messages, preserving redaction markers.
		msgStr := string(msg)
		msgStr = reMinimalStrip.ReplaceAllString(msgStr, "")
		msg = redact.RedactableString(msgStr)
	}
	w.Printf("%s\n", msg)
}
...
func (r Recording) visitSpanWithWriter(
	...
	// In minimal mode we never emitted an operation line, so no need to trim.
	...
	for _, osp := range r {
		...
		slice := r.visitSpanWithWriter(osp, depth+1, w, mode, start)
		if len(slice) > 0 {
...
func (r Recording) visitSpan(sp RecordedSpan, depth int) []traceLogData {
	return r.visitSpanWithWriter(sp, depth, nil, formatFull, r[0].StartTime)
}
