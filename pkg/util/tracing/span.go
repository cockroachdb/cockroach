...
func (s *Span) GetRecording() Recording {
	return s.crdb.getRecording(s.tracer.backgroundTracingEnabled)
}

func (s *crdbSpan) getRecording(backgroundTracingEnabled bool) Recording {
	if s == nil {
		return nil
	}
	if !backgroundTracingEnabled && s.recordingType() == RecordingOff {
	...
	result = append(result, s.getRecordingLocked(backgroundTracingEnabled))
	result = append(result, s.mu.recording.remoteSpans...)
	s.mu.Unlock()

	for _, child := range children {
		result = append(result, child.getRecording(backgroundTracingEnabled)...)
	}
...
func (s *Span) ImportRemoteSpans(remoteSpans []tracingpb.RecordedSpan) error {
	if !s.tracer.backgroundTracingEnabled && s.crdb.recordingType() == RecordingOff {
		return nil
	}
...
// children.
func (s *crdbSpan) getRecordingLocked(backgroundTracingEnabled bool) tracingpb.RecordedSpan {
	...
	onlyBackgroundTracing := backgroundTracingEnabled && s.recordingType() == RecordingOff
