...
func BenchmarkLookupJoinEqColsAreKeyOrdering(b *testing.B) {
	...
	ForEachDB(b, func(b *testing.B, db *sqlutils.SQLRunner) {
		...
		for i := 0; i < b.N; i++ {
			db.Exec(b, `SELECT count(b) FROM t1 INNER LOOKUP JOIN t2 ON t1.a = t2.a GROUP BY t1.a ORDER BY t1.a`)
		}
	})
}
...
func BenchmarkLookupJoinNoOrdering(b *testing.B) {
	...
	ForEachDB(b, func(b *testing.B, db *sqlutils.SQLRunner) {
		...
		for i := 0; i < b.N; i++ {
			db.Exec(b, `SELECT t2.a FROM t1 INNER LOOKUP JOIN t2 ON t1.a = t2.a`)
		}
	})
}
...
func BenchmarkLookupJoinOrdering(b *testing.B) {
	...
	ForEachDB(b, func(b *testing.B, db *sqlutils.SQLRunner) {
		...
		for i := 0; i < b.N; i++ {
			db.Exec(b, `SELECT count(t2.a) FROM t1 INNER LOOKUP JOIN t2 ON t1.a = t2.a GROUP BY t1.a ORDER BY t1.a`)
		}
	})
}
