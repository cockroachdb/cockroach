...
`
	...
	// dbMux will be used to serialize access to the database. This is
	// because the DB handle provided to us by the caller is not safe
	// for concurrent use.
	var dbMux syncutil.Mutex
		...
		go func(i int) {
			...
			errChan <- func() error {
				...
				for {
					...
					err := func() error {
						// Serialize access to db.DB: it's not safe for concurrent
						// use by multiple goroutines.
						dbMux.Lock()
						defer dbMux.Unlock()
						_, err := db.DB.ExecContext(context.Background(), buf.String())
						return err
					}()
					if err != nil {
