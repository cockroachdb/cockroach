...
func BenchmarkTPCC(b *testing.B) {
	...
	defer cleanup()
	baseOptions := options{
		mkOption("inMem", func(config *benchmarkConfig) {
			config.argsGenerator = func(
				b testing.TB,
			) (_ base.TestServerArgs, cleanup func()) {
				return cloneInMemEngine(b, vfs.Default, engPath)
			}
		}),
	}
	...
	// One might think that the in-mem storage engine leads to excess
	// allocations. In practice, it doesn't seem to show up. This config
	// will swap it out for an on-disk engine and will disable fsyncs.
	_ = options{
		mkOption("onDisk", func(config *benchmarkConfig) {
			config.argsGenerator = func(
				b testing.TB,
			) (_ base.TestServerArgs, cleanup func()) {
				return cloneOnDiskEngine(b, vfs.Default, engPath)
			}
		}),
		mkOption("disableFsyncs", func(config *benchmarkConfig) {
			config.setupStmts = append(config.setupStmts, `
...
func (bm *benchmark) startClient(b *testing.B) (wait func() error) {
	cmd := makeClientCommand(b.N, bm.pgURL, bm.eventURL, bm.workloadFlags)
