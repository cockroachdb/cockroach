...
func runBenchmarkExpectationTests(t *testing.T, r *Registry) {
	...
	concurrency := system.NumCPU() * 4
	if stress := skip.NightlyStress(); stress && util.RaceEnabled {
		concurrency = 1
	} else if stress {
		concurrency = 2
	}
	...
	if isRewrite {
		...
		results.iterate(func(result benchmarkResult) {
			res = append(res, result)
		})
		writeExpectationsFile(t, mergeExpectations(
			readExpectationsFile(t),
			resultsToExpectations(t, res)))
	} else {
		expectations := readExpectationsFile(t)
		checkResults(t, results, expectations)
	}
}
...
func mergeExpectations(existing, new benchmarkExpectations) (merged benchmarkExpectations) {
	...
	for len(existing) > 0 && len(new) > 0 {
		...
		case existing[0].name < new[0].name:
			...
			existing = existing[1:]
		default: // discard the existing value if they are equal
			existing = existing[1:]
			fallthrough
		case existing[0].name > new[0].name:
			merged = append(merged, new[0])
			new = new[1:]
		}
	}
	// Only one of existing or new should be non-empty.
...
func resultsToExpectations(t *testing.T, results []benchmarkResult) benchmarkExpectations {
	...
	for i := 1; i < len(res); i++ {
		if res[i-1].name == res[i].name {
			t.Fatalf("duplicate expecatations for Name %s", res[i].name)
		}
	}
	return res
}
