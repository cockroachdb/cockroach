// Copyright 2021 The Cockroach Authors.
//
// Use of this software is governed by the Business Source License
// included in the file licenses/BSL.txt.
//
// As of the Change Date specified in that file, in accordance with
// the Business Source License, use of this software will be governed
// by the Apache License, Version 2.0, included in the file
// licenses/APL.txt.

package rttanalysis

import (
	"strings"
	"testing"

	"github.com/cockroachdb/cockroach/pkg/util/log"
)

type testingTB interface {
	testing.TB
	N() int
	ResetTimer()
	StopTimer()
	StartTimer()
	ReportMetric(float64, string)
	Run(string, func(testingTB))

	// logScope is used to wrap log.Scope and make it available only in
	// appropriate contexts.
	logScope() func()
}

type bShim struct {
	*testing.B
}

func (b bShim) logScope() func() {
	sc := log.Scope(b)
	return func() { sc.Close(b) }
}

func (b bShim) N() int { return b.B.N }
func (b bShim) Run(name string, f func(b testingTB)) {
	b.B.Run(name, func(b *testing.B) {
		f(bShim{b})
	})
}

// tShim is used by the expectation tests to appear as though they are a
// testing.B and capture the output. They also suppress creation of a new
// log.Scope in order to permit parallel execution.
type tShim struct {
	*testing.T
	results *resultSet
}

func (ts tShim) logScope() func() {
	return func() {}
}

func (ts tShim) N() int { return 1 }

func (ts tShim) ResetTimer() {}

func (ts tShim) StopTimer() {}

func (ts tShim) StartTimer() {}

func (ts tShim) ReportMetric(f float64, s string) {
	if s == roundTripsMetric {
		ts.results.add(benchmarkResult{
			name:   ts.Name(),
			result: int(f),
		})
	}
}

func (ts tShim) Name() string {
	tn := ts.T.Name()
	return tn[strings.Index(tn, "/")+1:]
}

func (ts tShim) Run(s string, f func(testingTB)) {
	ts.T.Run(s, func(t *testing.T) {
		f(tShim{results: ts.results, T: t})
	})
}
