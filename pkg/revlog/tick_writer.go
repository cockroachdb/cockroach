...
// checksum of manifest objects (format §4).
...
// (per-producer-per-tick monotonic counter; see format §7).
func NewTickWriter(
	ctx context.Context,
...
// marshaled body, followed by the marshaled Manifest proto (format
// §4). The leading-checksum layout lets readers verify with a single
// GET — no separate Size() call to locate a trailer.
//
