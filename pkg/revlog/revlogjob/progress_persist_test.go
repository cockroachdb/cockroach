...
func TestJobPersisterRoundTrip(t *testing.T) {
	...
	require.NoError(t, rig.persister.Store(ctx, want.HighWater, want.Frontier, want.OpenTicks))
...
func TestJobPersisterStoreOverwrites(t *testing.T) {
	...
	require.NoError(t, rig.persister.Store(
		ctx,
		hlc.Timestamp{WallTime: 90},
		frontier1,
		map[hlc.Timestamp][]revlogpb.File{
