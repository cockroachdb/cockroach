...
func (p *proc) run(ctx context.Context) error {
	...
	// runErr captures the first error to surface; we still wait for
	// the dispatcher goroutine to exit before returning so the
	// caller can safely close progCh (which the dispatcher writes
	// to via metaSink.Flush -> OnCheckpoint).
	var runErr error
	select {
	case runErr = <-dispErr:
		return runErr
	case err := <-errCh:
		runErr = errors.Wrap(err, "rangefeed internal error")
	case <-ctx.Done():
		runErr = ctx.Err()
	}
	cancel()
