...
func (s *descFeedState) handleValue(ctx context.Context, v *kvpb.RangeFeedValue) error {
	...
	if !s.lastSpansSet || !sameSpans(s.lastSpans, newSpans) {
		...
		if widening {
			select {
			case s.sigs.replan <- cloneSpans(newSpans):
			default:
				// A prior replan signal is still in the buffer;
				// the outer loop will pick it up. The next replan
				// will use whatever spans were posted last.
			}
		}
	}
