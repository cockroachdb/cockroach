...
// IterateRangeDescriptors provides a handle on every range in descriptor in the
...
func (h *Helper) IterateRangeDescriptors(
	ctx context.Context, blockSize int, fn func(...roachpb.RangeDescriptor) error,
) error {
...
func (h *Helper) EveryNode(
	...
	for {
		...
		log.Infof(ctx, "executing %s on nodes=%s", op, nodeIDs)
		for _, nodeID := range nodeIDs {
			conn, err := h.m.dialer.Dial(ctx, nodeID, rpc.DefaultClass)
			if err != nil {
				return err
			}
