...
func (m *Manager) Migrate(ctx context.Context, from, to clusterversion.ClusterVersion) error {
	...
	for _, cv := range clusterversion.GetVersionsBetween(from, to) {
		h := &Helper{Manager: m}
		clusterVersion := cv
...

		{
			...
			req := &serverpb.BumpClusterVersionRequest{&fenceVersion}
			err := h.EveryNode(ctx, "bump-fence-cv", func(ctx context.Context, client serverpb.MigrationClient) error {
		...
		}
		{
			...
			if err := h.EveryNode(ctx, "bump-cv-actual", func(ctx context.Context, client serverpb.MigrationClient) error {
				_, err := client.BumpClusterVersion(ctx, req)
				return err
			}); err != nil {
				return err
