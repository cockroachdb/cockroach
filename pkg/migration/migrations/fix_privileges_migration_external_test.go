...
`
	...
	for _, descriptorStringsToInject := range testCases {
		...
		db := tc.ServerConn(0)
		kvDB := tc.Server(0).DB()
		require.NoError(t, sqlutils.InjectDescriptors(ctx, db, descriptorsToInject, true))
		...
		err = kvDB.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
			...
			for _, desc := range descs {
				...
				if privilegeDesc.Owner().Undefined() {
					return errors.New("privilege descriptor cannot have an empty owner")
				}
				if desc.GetID() == keys.SystemDatabaseID || desc.GetParentID() == keys.SystemDatabaseID {
					if !privilegeDesc.Owner().IsNodeUser() {
						return errors.New("objects in the system database must be owned by node")
					}
				}
				...
				case catalog.TableDescriptor:
					if err != privilegeDesc.Validate(desc.GetID(), privilege.Table) {
						return err
					}
				case catalog.SchemaDescriptor:
					if err != privilegeDesc.Validate(desc.GetID(), privilege.Schema) {
						return err
					}
				case catalog.DatabaseDescriptor:
					if err != privilegeDesc.Validate(desc.GetID(), privilege.Database) {
						return err
					}
				case catalog.TypeDescriptor:
					if err != privilegeDesc.Validate(desc.GetID(), privilege.Type) {
						return err
					}
				default:
					return errors.New("unexpected descriptor type")
				}
			}
			return nil
		})
