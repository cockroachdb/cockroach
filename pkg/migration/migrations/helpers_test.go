...
func validateSchemaExists(
	...
	storedTable := getTable(t, ctx, s, storedTableID)
	str := "not have"
	if expectExists {
		str = "have"
	}
	for _, sch := range schemas {
		updated, err := sch.validationFn(storedTable, expectedTable, sch.name)
		require.NoError(t, err)
		require.Equal(t, expectExists, updated,
			"expected table to %s %s", str, sch)
	}
}

// getTable returns the system table descriptor, reading it from storage.
func getTable(
	t *testing.T, ctx context.Context, s serverutils.TestServerInterface, tableID descpb.ID,
