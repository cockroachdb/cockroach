				...
				case *roachpb.ScanInterleavedIntentsRequest:
					...
					for i := range ri.ranges {
						if bytes.HasPrefix(request.Key, keys.LocalRangePrefix) {
							rangeKeyRange := roachpb.RSpan{
								Key:    roachpb.RKey(keys.MakeRangeKeyPrefix(ri.ranges[i].desc.StartKey)),
								EndKey: roachpb.RKey(keys.MakeRangeKeyPrefix(ri.ranges[i].desc.EndKey)),
							}
							if rangeKeyRange.ContainsKeyRange(roachpb.RKey(request.Key), roachpb.RKey(request.EndKey)) {
								r = &ri.ranges[i]
		...
		func(t *testing.T, d *datadriven.TestData) string {
			...
			case "add-range":
				...
				for _, line := range strings.Split(d.Input, "\n") {
					...
					intentKey := []byte(key)
					if localKeys {
						intentKey = keys.MakeRangeKey(roachpb.RKey(startKey), keys.LocalQueueLastProcessedSuffix, roachpb.RKey(key))
					}
