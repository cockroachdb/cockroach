...
func TestEnsureNoDrainingNames(t *testing.T) {
	...
	// schema mapping.
	{
		...
		db := desctestutils.TestingGetDatabaseDescriptor(s.DB(), c, version, "t")
		...
		foo := desctestutils.TestingGetSchemaDescriptor(s.DB(), c, version, db.GetID(), "foo")
	...
	// is correct.
	{
		...
		bar := desctestutils.TestingGetSchemaDescriptor(s.DB(), c, version, db.GetID(), "bar")
