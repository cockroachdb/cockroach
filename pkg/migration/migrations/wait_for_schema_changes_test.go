	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			params.Knobs.Server = &server.TestingKnobs{
				...
				BinaryVersionOverride:          clusterversion.ByKey(clusterversion.NoNonMVCCIndexBackfills - 1),
			}

			var scStartedChan chan struct{}
			var scAllowResumeChan chan struct{}
			...
			if tc.run != nil {
				<-scStartedChan
				// TODO(ssd): Some tests may succeed just because the
				// schema change finishes first, without actually
				// testing our wait loop. The sleep is a poor way to help ensure
				// the migration starts before the schema change finishes.
