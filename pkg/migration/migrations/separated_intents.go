...
// being a range command takes care of that.
...
func (m *migrateLockTablePool) run(ctx context.Context, workerIdx int) {
	...
	for {
		...
		select {
		case <-ctx.Done():
			log.Warningf(ctx, "lock table migration canceled on range r%d", retryRequest.rangeID)
			return
		default:
		}

		atomic.StoreInt64(statusSlot, int64(retryRequest.rangeID))
