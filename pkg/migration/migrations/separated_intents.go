...
func runSeparatedIntentsMigration(
	...
	for ri.Seek(ctx, roachpb.RKeyMin, kvcoord.Ascending); ri.Valid(); ri.Next(ctx) {
		...
		request := migrateLockTableRequest{
			start:        start.AsRawKey(),
			end:          end.AsRawKey(),
			debugRangeID: desc.RangeID,
		}
		select {
		case workerPool.requests <- request:
		case <-ctx.Done():
			return errors.New("lock table migration canceled")
		}

		// Also enqueue a request for range local keys.
		rangeKeyStart := keys.MakeRangeKeyPrefix(start)
		rangeKeyEnd := keys.MakeRangeKeyPrefix(end)

		request2 := migrateLockTableRequest{
			start:        rangeKeyStart,
			end:          rangeKeyEnd,
			debugRangeID: desc.RangeID,
		}
		select {
		case workerPool.requests <- request2:
		case <-ctx.Done():
			return errors.New("lock table migration canceled")
		}
		numMigratedRanges++
