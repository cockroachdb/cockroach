...
func deleteDeprecatedNamespaceTableDescriptorMigration(
	...
	return d.DB.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
		results, err := txn.Scan(ctx, deprecatedTablePrefix, deprecatedTablePrefix.PrefixEnd(), 1 /* maxRows */)
		if err != nil {
			return err
		}
		if len(results) > 0 {
			log.Fatal(ctx, "Unexpected data in deprecated system.namespace table.")
		}
		b := txn.NewBatch()
