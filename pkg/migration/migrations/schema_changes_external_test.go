...
`
	...
	for _, test := range []struct {
		...
		{
			name:                    "adding columns",
			query:                   migrations.TestingAddColsQuery,
		...
		{
			name:                    "adding index",
			query:                   migrations.TestingAddIndexQuery,
			waitForMigrationRestart: false, // Does not matter.
			cancelSchemaJob:         false, // Does not matter.
			expectedSkipped:         0,     // Will be ignored.
		},
		...
		t.Run(test.name, func(t *testing.T) {
			...
			migrationFunc, expectedDescriptor := migrations.
					...
					Knobs: base.TestingKnobs{
						...
						SQLExecutor: &sql.ExecutorTestingKnobs{
							BeforeExecute: func(ctx context.Context, stmt string) {
								if stmt == migrations.WaitForJobStatement {
									select {
						...
						MigrationManager: &migration.TestingKnobs{
							...
							RegistryOverride: func(cv clusterversion.ClusterVersion) (migration.Migration, bool) {
								if cv.Equal(endCV) {
									return migration.NewTenantMigration("testing",
										endCV,
			...
			ie := s.ExecutorConfig().(sql.ExecutorConfig).InternalExecutor
			cf := s.CollectionFactory().(*descs.CollectionFactory)
			require.NoError(t, cf.Txn(ctx, ie, s.DB(), func(
			...
			go migrations.UpgradeToVersion(
			...
			go migrations.UpgradeToVersion(t, sqlDB, endCV.Version, finishChan, false /* expectError */)
