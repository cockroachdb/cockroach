...
func convertIncompatibleDatabasePrivilegesToDefaultPrivileges(
	...
	return d.CollectionFactory.Txn(ctx, d.InternalExecutor, d.DB,
		func(ctx context.Context, txn *kv.Txn, collections *descs.Collection) error {
			...
			const minBatchSizeInBytes = 1 << 19 /* 512 KiB batch size */
			currSize := 0
			...
			err = descriptors.ForEachDescriptorEntry(func(desc catalog.Descriptor) error {
				...
				modifiedDescs = append(modifiedDescs, database)
				currSize += database.Size()

				if currSize > minBatchSizeInBytes || minBatchSizeInBytes == 0 {
					err = writeModifiedDescriptors(ctx, d, modifiedDescs)
					if err != nil {
						return err
					}
			...
			// Write remaining descriptors.
			return writeModifiedDescriptors(ctx, d, modifiedDescs)
		},
	)
}
