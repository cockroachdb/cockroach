...
func retryJobsWithExponentialBackoff(
	...
	const numRunsQuery = `ALTER TABLE system.jobs ADD COLUMN num_runs INT8 FAMILY claim`
	const lastRunQuery = `ALTER TABLE system.jobs ADD COLUMN last_run TIMESTAMP FAMILY claim;`
	const addIndexQuery = `CREATE INDEX jobs_run_stats_idx
		ON system.jobs (claim_session_id, status, created)
		STORING (claim_instance_id, num_runs, last_run)
		WHERE
			status
			IN (
					'running',
	...
	ops := [...]struct {
		...
		// Schema change query.
		query string
		// Function to validate the existence of the schema.
		validateFn func(catalog.TableDescriptor, string) (bool, error)
	}{
	...
	for _, op := range ops {
		if err := runJobsTableMigration(ctx, cs, d, "jobs-migration-"+op.name, op.query,
			func(table catalog.TableDescriptor) (bool, error) {
				return op.validateFn(table, op.name)
			}); err != nil {
			return err
		}
	}
...
func runJobsTableMigration(
	...
	hasAlreadyChanged func(descriptor catalog.TableDescriptor) (bool, error),
) error {
	for {
		...
		if ok, err := hasAlreadyChanged(jobsTable); err != nil {
			return err
		} else if ok {
			log.Info(ctx, fmt.Sprintf("Ignoring %s operation as it has already done.", opTag))
			return nil
		}

		// Modify the jobs table.
		log.Info(ctx, fmt.Sprintf("[SR] Performing %s operation.", opTag))
...
func hasColumn(jobsTable catalog.TableDescriptor, colName string) (bool, error) {
	...
	//TODO(sajjad): To discuss: I am not sure which fields to ignore. May be I
	// should match only those fields that are explicitly declared in the schema
	// in system.go
	storedCopy.ID = expectedCopy.ID
	storedCopy.Nullable = expectedCopy.Nullable
	storedCopy.DefaultExpr = expectedCopy.DefaultExpr
	storedCopy.Hidden = expectedCopy.Hidden
	storedCopy.Inaccessible = expectedCopy.Inaccessible

	return equalMessages(&expectedCopy, &storedCopy)
}
...
func hasIndex(jobsTable catalog.TableDescriptor, indexName string) (bool, error) {
	...
	// TODO(sajjad): Why should we ignore ID?
	storedCopy.ID = expectedCopy.ID
	// TODO(sajjad): To discuss: Should we ignore CreatedExplicitly? Why?
	storedCopy.CreatedExplicitly = expectedCopy.CreatedExplicitly
	// Ignoring the version because it can be bumped in subsequent attempts.
	storedCopy.Version = expectedCopy.Version

	return equalMessages(&expectedCopy, &storedCopy)
}

func equalMessages(expected, found protoutil.Message) (bool, error) {
	...
	return false, errors.Errorf("expected descriptor doesn't match "+
		"with found descriptor (type %T): %s",
		expected, found, strings.Join(pretty.Diff(expected, found), "\n"))
}
