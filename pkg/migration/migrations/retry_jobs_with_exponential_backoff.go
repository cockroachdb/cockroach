...
func runJobsTableMigration(
	...
	for {
		...
		log.Infof(ctx, "Schema-change: %v", opTag)
		...
		if mutations := jt.GetMutationJobs(); len(mutations) > 0 {
			for _, mutation := range mutations {
				log.Infof(ctx, "[% ] waiting for a mutation job %v to complete", mutation.JobID)
...
func hasIndex(jobsTable catalog.TableDescriptor, indexName string) (bool, error) {
	...
	// TODO(sajjad): To discuss: Please check am I ignoring the correct fields.
	storedCopy.ID = 0
	expectedCopy.ID = 0
