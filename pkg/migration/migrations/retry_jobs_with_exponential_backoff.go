...
func runJobsTableMigration(
	...
	for {
		...
		if ok, err := schemaExists(jt); err != nil {
			...
			log.Info(ctx, fmt.Sprintf("skipping %s operation as the schema change already exists.", opTag))
			// TODO(sajjad): To discuss: This is not the best way to ensure that we
			// are ignoring a mutation. We should have some other direct invariant,
			// e.g., reading some counter from the table that counts the number of times
			// the table has been successfully updated.
			if d.TestingKnobs.SkippedMutation != nil {
		...
		log.Info(ctx, fmt.Sprintf("performing operation: %s", opTag))
...
func hasIndex(jobsTable catalog.TableDescriptor, indexName string) (bool, error) {
	...
	// TODO(sajjad): FIXME: Please check am I ignoring the correct fields or not.
	storedCopy.ID = 0
	expectedCopy.ID = 0
