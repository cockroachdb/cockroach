...
func runJobsTableMigration(
	...
	for {
		...
		if ok, err := schemaExists(jt); err != nil {
			...
			log.Info(ctx, fmt.Sprintf("[SR] Ignoring %s operation as it has already done.", opTag))
			return nil
		}
...
func hasIndex(jobsTable catalog.TableDescriptor, indexName string) (bool, error) {
	...
	// TODO(sajjad): FIXME: Ignoring predicate for debugging purposes
	storedCopy.Predicate = expectedCopy.Predicate
