...
// migrateTable changes the table schema based on the given schema-change query.
// The change is ignored if the table already has the required changes, which are
// explicitly limited to two new columns (last_run and num_runs) and an index
// (jobs_run_stats_idx) in this migration.
func migrateTable(
	ctx context.Context,
	...
	for {
		...
		// - Check if any mutation job exists for the table. These mutations can
		...
		//	   that started in while waiting.
		// - Check if the intended schema-changes already exist.
		//   - If already exists, skip the schema-change and return as it has already
		//     completed in a previous migration attempt.
...
// hasColumn return true if storedTable already has the given column. storedTable
// descriptor must be read from the system as compared to reading from the
// systemschema package.
// This function returns an error if the column exists but doesn't match with the
// table's descriptor defined in systemschema/system.go. The comparison is not strict
// as several descriptor fields are ignored.
func hasColumn(storedTable catalog.TableDescriptor, colName string) (bool, error) {
...
// as several descriptor fields are ignored.
func hasIndex(storedTable catalog.TableDescriptor, indexName string) (bool, error) {
