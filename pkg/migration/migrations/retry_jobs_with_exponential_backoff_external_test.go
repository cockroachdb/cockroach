...
// TestNumRunsColumn tests migration and failures while adding num_runs column.
func TestNumRunsColumn(t *testing.T) {
	testWithFailures(t, "num_runs", migrations.NumRunsQuery, 0)
}

// TestNumRunsColumn tests migration and failures while adding last_run column.
func TestLastRunColumn(t *testing.T) {
	testWithFailures(t, "last_run", migrations.LastRunQuery, 1)
}

// TestNumRunsColumn tests migration and failures while adding the new index.
func TestAddIndex(t *testing.T) {
	testWithFailures(t, "index", migrations.AddIndexQuery, 2)
}

// testWithFailures tests modification of system.jobs table during
...
// schema-change job moves to running state or succeeded state. Both scenarios
// are tested with and without the schema-change job to fail. The combination is tested
// with the scenarios that the restarted migration job observes the ongoing schema-change
// or not.
//
// expectSkipped is the number of schema-changes skipped, which depends on the
// number of already completed schema changes, which depends on the order of their
// execution.
func testWithFailures(t *testing.T, tag string, query string, expectSkipped int) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)
	...
	//
	// Wait for the canceled migration-job to finish, expecting its failure.
	//
	...
	// to complete without errors. If we canceled the schema-job, expect it to rerun
	...
	var tests []testConf
	for _, waitForMigration := range []bool{false, true} {
		for _, cancelSchemaJob := range []bool{false, true} {
			for _, interceptAtStatus := range []jobs.Status{jobs.StatusSucceeded, jobs.StatusRunning} {
				...
				skipped := expectSkipped
				if !cancelSchemaJob {
					skipped++
				}
				tests = append(tests, testConf{
					fmt.Sprintf(
						"%v-wait_%v-cancel_%v-status_%v-skip_%v",
						tag, waitForMigration, cancelSchemaJob, interceptAtStatus, skipped),
					interceptAtStatus,
					query,
					waitForMigration,
					cancelSchemaJob,
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			testutils.SucceedsSoon(t, func() error {
				...
				return errors.Errorf("[SR] waiting for the migration job to finish.")
			})
			// Make sure that we have observed the expected number of ignored schema change jobs.
...
func validateSchemaExists(
	...
	for _, schema := range [...]struct {
		...
		// TODO(sajjad): To discuss: Shouldn't we use an independent way to verify
