...
func TestRetriesWithExponentialBackoffMigration(t *testing.T) {
	...
	clusterArgs := func() base.TestClusterArgs {
	...
	setup := func(
		cArgs *base.TestClusterArgs,
	) (context.Context, *testcluster.TestCluster, serverutils.TestServerInterface, *gosql.DB, *sqlutils.SQLRunner) {
		ctx := context.Background()
		tc := testcluster.StartTestCluster(t, 1, *cArgs)
		s := tc.Server(0)
		sqlDB := tc.ServerConn(0)
		tdb := sqlutils.MakeSQLRunner(sqlDB)
		return ctx, tc, s, sqlDB, tdb
	}

	t.Run("no failures", func(t *testing.T) {
		cArgs := clusterArgs()
		cArgs.ServerArgs.Knobs.JobsTestingKnobs = jobs.NewTestingKnobsWithShortIntervals()
		ctx, tc, s, sqlDB, tdb := setup(&cArgs)
		defer tc.Stopper().Stop(ctx)

		// Inject the old copy of the descriptor.
		injectLegacyTable(t, ctx, s, sqlDB)
		// Run the migration.
		migrate(t, sqlDB, false)
		// Validate that the jobs table has new schema.
		validateNewSchema(t, ctx, s, tdb)
	})

	schemaQuery := migrations.NumRunsQuery
	t.Run("fail num_runs column", func(t *testing.T) {
		var s serverutils.TestServerInterface
		var ctx context.Context
		var injectedError atomic.Value
		injectedError.Store(false)
		beforeResume := func(jobID jobspb.JobID) error {
			if injectedError.Load().(bool) {
				return nil
			}
			...
			pl := scJob.Payload()
			stmt := strings.Replace(schemaQuery, "system.jobs", "system.public.jobs", -1)
			if stmt != pl.Description {
				return nil
			}
			injectedError.Store(true)
			fmt.Println("[SR] Injecting error...")
			return errors.Errorf("Injecting error for job: %s", pl.Description)
		}

		cArgs := clusterArgs()
		cArgs.ServerArgs.Knobs.JobsTestingKnobs = jobs.NewTestingKnobsWithShortIntervals()
		cArgs.ServerArgs.Knobs.SQLSchemaChanger = &sql.SchemaChangerTestingKnobs{
			RunBeforeResume: beforeResume,
		}
		ctx, tc, s, sqlDB, tdb := setup(&cArgs)
		defer tc.Stopper().Stop(ctx)
		...
		migrate(t, sqlDB, true)
		// Run the migration again, expecting success.
		fmt.Println("[SR] Expect success")
		migrate(t, sqlDB, false)
		// Validate that the jobs table has new schema.
		validateNewSchema(t, ctx, s, tdb)
	})
...
func validateNewSchema(
	t *testing.T, ctx context.Context, s serverutils.TestServerInterface, tdb *sqlutils.SQLRunner,
) {
...
func migrate(t *testing.T, sqlDB *gosql.DB, expectError bool) {
...
func injectLegacyTable(
	t *testing.T, ctx context.Context, s serverutils.TestServerInterface, sqlDB *gosql.DB,
) {
	err := descs.Txn(
		ctx,
		s.ClusterSettings(),
