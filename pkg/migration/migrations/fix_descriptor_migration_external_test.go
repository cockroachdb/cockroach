...
// TestCrossDBReferencesTelemetry tests that telemetry for cross-database views,
// sequences, and foreign-key constraints are recorded.
func TestCrossDBReferencesTelemetry(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)
...
`)
	...
	require.Equal(t, int32(1), counts[`sql.schema.cross_database_reference.foreign-key`])
	require.Equal(t, int32(1), counts[`sql.schema.cross_database_reference.sequence`])
	require.Equal(t, int32(1), counts[`sql.schema.cross_database_reference.view`])
}
