...
func TestSeparatedIntentsMigration(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = txn.Exec("INSERT INTO test.kv SELECT generate_series(1, 10000), 'test123';")
	require.NoError(t, err)

	interleavedIntentCount := 0
	for i := 0; i < numServers; i++ {
		tc.Server(i).GetStores().(*kvserver.Stores).VisitStores(func(s *kvserver.Store) error {
	...
	_, err = tdb.Exec(`SET CLUSTER SETTING version = $1`,
		clusterversion.ByKey(clusterversion.SeparatedIntentsMigration).String())
	require.NoError(t, err)
