...
func ensureNoDrainingNames(
	...
	for r := retry.StartWithCtx(ctx, retryOpts); r.Next(); {
		rows, err := d.InternalExecutor.QueryIterator(
			ctx, "ensure-no-draining-names", nil /* txn */, query,
		)
		if err != nil {
			return err
		}
		hasDrainingNames, err := handleQueryResults(ctx, rows)
		if err != nil || !hasDrainingNames {
			return err
		}
	}
	panic("unreachable")
...
func handleQueryResults(
	ctx context.Context, rows sqlutil.InternalRows,
) (hasDrainingNames bool, err error) {
	defer func() { _ = rows.Close() }()
	hasDrainingNames, err = rows.Next(ctx)
	if err != nil || !hasDrainingNames {
		return hasDrainingNames, err
	}
	datums := rows.Cur()
