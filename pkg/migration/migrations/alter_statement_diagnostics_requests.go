...
ALTER TABLE system.statement_diagnostics_requests
  ADD COLUMN min_execution_latency INTERVAL NOT NULL DEFAULT '0',
  ADD COLUMN expires_at TIMESTAMPTZ NOT NULL DEFAULT now() + '100 years'`

	createNewCompletedIdx = `
CREATE INDEX completed_idx_new ON system.statement_diagnostics_requests (completed, ID)
  STORING (statement_fingerprint, min_execution_latency, expires_at)`

	dropOldCompletedIdx = `DROP INDEX IF EXISTS completed_idx`
)
...
func alterSystemStmtDiagReqs(
	...
	for _, op := range []operation{
		...
		},
		{
			...
			schemaList:     []string{"completed_idx_new"},
			query:          createNewCompletedIdx,
			schemaExistsFn: hasIndex,
		},
		{
			name:       "drop-stmt-diag-reqs-old-index",
			schemaList: []string{"completed_idx"},
			query:      dropOldCompletedIdx,
