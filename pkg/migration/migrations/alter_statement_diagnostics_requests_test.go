...
func TestAlterSystemStmtDiagReqs(t *testing.T) {
	...
	var (
		validationStmts = []string{
			`SELECT min_execution_latency_ms, expires_at FROM system.statement_diagnostics_requests LIMIT 0`,
			`SELECT min_execution_latency_ms, expires_at FROM system.statement_diagnostics_requests@completed_idx_v2 LIMIT 0`,
		}
		validationSchemas = []migrations.Schema{
			{Name: "min_execution_latency_ms", ValidationFn: migrations.HasColumn},
