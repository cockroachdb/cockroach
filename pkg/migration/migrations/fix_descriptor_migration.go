...
SELECT id, descriptor, crdb_internal_mvcc_timestamp FROM system.descriptor ORDER BY ID ASC`)
	...
	for ; ok; ok, err = rows.Next(ctx) {
		...
		if err := fixDescriptor(ctx, desc, d); err != nil {
			return err
		}
	}
	return err
...
func fixDescriptor(
	...
	return descs.Txn(ctx, d.Settings, d.LeaseManager, d.InternalExecutor, d.DB, func(
		...
		datums, err := d.InternalExecutor.QueryRow(ctx, "fix-descriptor-migration", txn,
			`SELECT id, descriptor, crdb_internal_mvcc_timestamp FROM system.descriptor WHERE id = $1`, desc.GetID())
		if err != nil {
			return err
		}
		desc, err := getMutableDescFromDescriptorRow(datums)
		...
		descBuilder := catalogkv.NewBuilder(desc.DescriptorProto())
		dg := catalogkv.NewOneLevelUncachedDescGetter(txn, d.Codec)
		changed, err := descBuilder.RunPostDeserializationChanges(ctx, dg)
		if err != nil {
			return err
		}
		if changed {
			writeDesc := descBuilder.BuildExistingMutable()
