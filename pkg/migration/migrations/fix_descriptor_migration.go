...
func fixDescriptorMigration(
	...
	fixDescriptorFunc := func(ids []descpb.ID, desc descpb.Descriptor, ts hlc.Timestamp) error {
		for id := range ids {
...
type fixDescriptorFunction func(ids []descpb.ID, desc descpb.Descriptor, ts hlc.Timestamp) error
...
func descriptorUpgradeMigration(
	...
	for ; ok; ok, err = rows.Next(ctx) {
		...
		if currSize > minBatchSizeInBytes || minBatchSizeInBytes == 0 {
			// Reset size and id array.
			currSize = 0
			ids = nil
			err = fixDescFunc(ids, desc, ts)
			if err != nil {
				return err
			}
		}
