...
func crossDBReferenceDuringMigrationCounter(typ string) telemetry.Counter {
	return telemetry.GetCounter("sql.schema.cross_database_reference." + typ)
}
...
func maybeRecordTelemetryForCrossDBReferences(
	...
	if tbDesc.IsTable() {
		...
		// Check for columns that use sequences.
		for _, col := range tbDesc.PublicColumns() {
			for i := 0; i < col.NumUsesSequences(); i++ {
				sequenceID := col.GetUsesSequenceID(i)
				seqDesc, err := catalog.GetTableDescFromID(ctx, dg, sequenceID)
				if err != nil {
					return err
				}
				if seqDesc.GetParentID() != tbDesc.GetParentID() {
					telemetry.Inc(crossDBReferenceDuringMigrationCounter("sequence"))
				}
			}
		}
		return nil
