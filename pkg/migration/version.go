...
// clusterVersionVar returns the *StateMachineSetting which tracks the configured
// minimum cluster version. It panics when called more than once.
...
func NewDefaultExposedClusterVersion(initialCV base.ClusterVersion) *ExposedClusterVersion {
	...
	// This is a hack, but thanks to the singleton-ness of Settings we can't set this up at
	// init time.

	// We would like to uncomment the below, but in-memory test clusters
	// all share the singleton.
	//
	// if clusterVersion.Transformer != nil {
	// 	panic(
	// 		"cluster version setting is already initialized, " +
	// 			"must not call NewDefaultExposedClusterVersion more than once!")
	// }
	clusterVersion.Transformer = transformer
...
// Atomically runs the supplied closure atomically with respect to version
// changes (as in, while the closure uses the version, it won't change out from
// under it).
func (ecv *ExposedClusterVersion) Atomically(f func(base.ClusterVersion)) {
	ecv.mu.Lock()
	defer ecv.mu.Unlock()
	f(ecv.mu.ClusterVersion)
}
...
// SetTo changes the exposed cluster version. It is typically set from a gossip
// callback.
func (ecv *ExposedClusterVersion) SetTo(cv base.ClusterVersion) {
	...
	ecv.mu.RLock()
	defer ecv.mu.RUnlock()
	for _, cb := range ecv.mu.callbacks {
		cb(cv)
	}
}
