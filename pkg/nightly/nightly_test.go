...
func findLocalBinary(binary string) (string, error) {
...
func initLocalBinaries(t *testing.T) {
	var err error
	if *cockroach == "" {
		*cockroach = "cockroach"
	}
	*cockroach, err = findLocalBinary(*cockroach)
	if err != nil {
		t.Fatal(err)
	}
	if *workload == "" {
	...
	*workload, err = findLocalBinary(*workload)
	if err != nil {
		t.Fatal(err)
	}
}
