...
// StartedDefaultSQLServer returns whether this cluster started a default SQL
// server.
func (tc *TestCluster) StartedDefaultSQLServer() bool {
	return !tc.Servers[0].Cfg.DisableDefaultSQLServer
}
...
func (tc *TestCluster) Start(t testing.TB) {
	...
	// cluster.
	probabilityOfStartingTestTenant := serverutils.ProbabilityOfStartingTestTenant(t)
	...
	for i := 0; i < nodes; i++ {
		...
		if !probabilisticallyStartTestSQLServer {
			...
			// We're starting the default SQL server (i.e. we're running this
			// test in a tenant). Log this for easier debugging, but only on
			// the first node, to avoid spamming the logs.
			log.Infof(context.Background(),
				"NOTE NOTE NOTE: Running test with the default SQL Server (i.e. within a tenant). "+
					"If you are only seeing a test case failure when this message appears, there may be a problem with "+
					"your test case running within tenants.")
		}
		if tc.Servers[i].Cfg.DisableDefaultSQLServer {
