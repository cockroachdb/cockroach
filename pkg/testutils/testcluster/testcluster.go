...
func (tc *TestCluster) CrashNode(idx int) {
	...
	// Enable the partitioner so that the partitions take effect.
	// TODO(pav-kv): we don't disable it again below because it could have been
	// enabled. This stateful side effect isn't great, consider cleaning up.
	...
	// TODO(pav-kv): this cancels any pre-existing partitions. We could fix
	// that by remembering the previous partitions and restoring them.
	for peerIdx := range tc.Servers {
		if peerIdx == idx {
			continue
		}
		peerNodeID := tc.Servers[peerIdx].NodeID()
		_ = tc.partitioner.RemovePartition(crashingNodeID, peerNodeID)
		_ = tc.partitioner.RemovePartition(peerNodeID, crashingNodeID)
	}
}
