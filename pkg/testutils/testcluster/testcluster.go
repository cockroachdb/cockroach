...
func (tc *TestCluster) MoveRangeLeaseNonCooperatively(
	...
	err = retry.ForDuration(retryDur, func() error {
		...
		ls, err := r.AcquireLease(ctx)
		if err != nil {
			log.Infof(ctx, "AcquireLease failed: %s", err)
			if lErr, ok := err.(*roachpb.NotLeaseHolderError); ok && lErr.Lease != nil {
				newLease = lErr.Lease
			} else {
				return err
			}
	...
	log.Infof(context.TODO(), "acquired lease: %s. err: %v", newLease, err)
	return newLease, err
}
