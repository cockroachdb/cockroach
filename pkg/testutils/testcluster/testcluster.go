...
func (tc *TestCluster) WaitForTenantCapabilities(
	t *testing.T, tenID roachpb.TenantID, capIDs ...tenantcapabilitiesapi.CapabilityID,
) {
	for i, ts := range tc.Servers {
		testutils.SucceedsSoon(t, func() error {
			...
			if len(capIDs) > 0 {
				missingCapabilityError := func(capID tenantcapabilitiesapi.CapabilityID) error {
				...
				for _, capID := range capIDs {
					if tenantcapabilitiesapi.CapabilityType(capID) != tenantcapabilitiesapi.Bool {
