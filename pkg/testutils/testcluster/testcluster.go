...
func StartTestCluster(t testing.TB, nodes int, args base.TestClusterArgs) *TestCluster {
	...
	for i := 0; i < nodes; i++ {
		...
		if err := tc.doAddServer(serverArgs, true /* initialClusterStart */); err != nil {
			t.Fatal(err)
		}
	}
...
func (tc *TestCluster) AddServer(serverArgs base.TestServerArgs) error {
	return tc.doAddServer(serverArgs, false /* initialClusterStart */)
}

func (tc *TestCluster) doAddServer(
	serverArgs base.TestServerArgs, initialClusterStart bool,
) error {
