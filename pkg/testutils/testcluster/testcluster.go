...
func (tc *TestCluster) doAddServer(t testing.TB, serverArgs base.TestServerArgs) error {
	...
	// this methods might be called for servers being added after the cluster was
	...
	if err := checkServerArgsForCluster(
		...
		// started should have a JoinAddr filled it at this point.
		allowJoinAddr,
	); err != nil {
		return err
	}
	...
	if tc.replicationMode == base.ReplicationManual {
		...
		if serverArgs.Knobs.Store == nil {
			stkCopy = storage.StoreTestingKnobs{}
		} else {
			stkCopy = *serverArgs.Knobs.Store.(*storage.StoreTestingKnobs)
		}
