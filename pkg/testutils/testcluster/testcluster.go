	...
	testutils.SucceedsSoon(t, func() error {
		...
		for _, s := range tc.Servers {
			...
			if curR.RaftStatus().RaftState == raftpb.StateLeader {
				...
				// Make sure that the leader has a valid leader lease because the next step we will stop
				// store liveness messages to the leader and we want that to cause it to step down.
				// If the leader does not have a valid leader lease yet, stopping store liveness messages
				// to it will not cause it to step down.
