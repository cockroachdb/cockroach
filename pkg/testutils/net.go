...
// addData adds data to the buffer. If there's zero free capacity, it will
...
// called - the signal interrupted the read. The caller is expected to try the
// read again after the partition is gone.
...
func NewPartitionableConn(serverConn net.Conn) *PartitionableConn {
	...
	c.mu.c2sBuffer = newBuf("c2sBuf", bufferSize, &c.mu.Mutex)
	c.mu.s2cBuffer = newBuf("s2cBuf", bufferSize, &c.mu.Mutex)
