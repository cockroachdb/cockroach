...
type buf struct {
	...
	*syncutil.Mutex
	...
	// readerWait is signaled when a reader should wake up and check the buffer's
	// state: when new data is put in the buffer, when the buffer is
...
func (b *buf) Close(err error) {
	...
	b.closed = true
	b.closedErr = err
	b.readerWait.Broadcast()
	b.capacityWait.Broadcast()
	b.Unlock()
}

// wakeReadersLocked wakes all the readers that are currently blocked.
// See comments on readerWait.
//
// This needs to be called while holding the buffer's mutex.
func (b *buf) wakeReadersLocked() {
	b.readerWait.Broadcast()
}
...
func (c *PartitionableConn) PartitionC2S() {
	...
	c.mu.c2sBuffer.wakeReadersLocked()
	c.mu.Unlock()
}
...
func (c *PartitionableConn) PartitionS2C() {
	...
	c.mu.s2cBuffer.wakeReadersLocked()
	c.mu.Unlock()
}
