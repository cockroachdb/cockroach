...
var (
	...
	maxLineDistance = 5
	mutexFunctions  = []loopvarcapture.Function{
...
func (lt *LockTracker) traverseBody(n ast.Node) bool {
	...
	case *ast.CallExpr:
		if lt.isMutexCall(node) {
			return false
		} else if lt.shouldReportNonLinearControlFlow() {
	...
	// Unlock.
	case *ast.IfStmt:
		if lt.shouldReportNonLinearControlFlow() {
...
// distance between is greater than maxLineDistance and reports on that if true.
...
func (lt *LockTracker) shouldReportNonLinearControlFlow() bool {
	for i := len(lt.locks) - 1; i >= 0; i-- {
		lock := lt.locks[i]
		if !lock.foundUnlockMatch {
			return true
...
func (lt *LockTracker) addNonLinearControlFlowIssue(pos token.Pos, stmt string) {
	lt.issues = append(lt.issues, analysis.Diagnostic{
		Pos:     pos,
		Message: fmt.Sprintf("Non linear control flow: %s between {RW}Mutex {R}Lock and {R}Unlock", stmt),
	})
}
