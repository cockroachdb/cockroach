...
func run(pass *analysis.Pass) (interface{}, error) {
	...
	inspect.Preorder(nodeFilter, func(n ast.Node) {
		...
		if fd, ok := n.(*ast.FuncDecl); ok {
			...
			// parameter.
			fmtOrMsgStr = maybeGetCstStr(pass, fd)
			return
		}
		// At a call site.
		call := n.(*ast.CallExpr)
...
func maybeGetCstStr(pass *analysis.Pass, fd *ast.FuncDecl) (res *types.Var) {
	if fd.Body == nil {
...
func checkCallExpr(pass *analysis.Pass, call *ast.CallExpr, fv *types.Var) {
	...
	// pass.Reportf(call.Lparen, "in %s", fnName)
...
var Tip = `
Tip: use YourFuncf("%s", ...) or list new formatting wrappers in
