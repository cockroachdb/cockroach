...
func TestLint(t *testing.T) {
	...
	t.Run("TestMegacheck", func(t *testing.T) {
		...
		ignores := strings.Join([]string{
			...
			// 	}
			//
			// if the generated code does not make use of the matched expression, sqlDollar will be
			...
			"github.com/cockroachdb/cockroach/pkg/*/*/*.pb.gw.go:SA1019",
		}, "\n")

		opt := lintutil.Options{
			GoVersion: goVersion,
			Ignores:   ignores,
			// Linting the test code makes the code twice as slow and use even more
			// memory.
			LintTests: false,
			// Limiting the number of concurrent jobs reduces memory usage a little
			// bit.
			MaxConcurrentJobs: 2,
		}
		...
		for _, p := range problems {
			t.Errorf("%s: %s: %s", p.Checker, p.Position, &p)
		}
	})
