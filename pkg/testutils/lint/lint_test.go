...
`)
	...
	t.Run("TestAmbientContextConstruction", func(t *testing.T) {
		...
		if err := stream.ForEach(filter, func(s string) {
			t.Errorf("\n%s <- don't construct an AC directly; reuse an existing AC when possible, or use one of the constructor functions instead: log.MakeServerAC(), log.MakeClientAC() or testutils.MakeAC()\n"+
				"For more details and information, see the context tech note.", s)
		}); err != nil {
			t.Error(err)
		}
