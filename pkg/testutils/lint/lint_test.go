	...
	t.Run("TestStaticCheck", func(t *testing.T) {
		...
		if err := stream.ForEach(
			stream.Sequence(
				...
				stream.GrepNot(`pkg/sql/instrumentation.go:.* evalCtx.SetDeprecatedContext is deprecated: .*`),
				stream.GrepNot(`pkg/sql/planner.go:.* evalCtx.SetDeprecatedContext is deprecated: .*`),
				...
				stream.GrepNot(`pkg/sql/execinfra/processorsbase.go:.* evalCtx.SetDeprecatedContext is deprecated: .*`),
