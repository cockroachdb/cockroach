...
`)
	...
	t.Run("TestNoFileIOInCode", func(t *testing.T) {
		...
		cmd, stderr, filter, err := dirCmd(
			...
			":!cmd/docgen/",
			":!sql/opt/optgen/cmd/",
j			":!sql/schemachanger/scop/generate_visitor.go",
			...
			":!util/log/",
			// The fileutil package implements file copying and moving, and changing file permissions during those operations would result in unexpected behaviour.
			":!util/fileutil/",
