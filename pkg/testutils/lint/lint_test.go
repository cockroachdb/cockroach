...
`)
	...
	t.Run("TestForbiddenImports", func(t *testing.T) {
		...
		filter := stream.FilterFunc(func(arg stream.Arg) error {
			var pkgPaths []string
			if len(lintTargetPackages) > 0 {
				for _, pkg := range lintTargetPackages {
					pkgPaths = append(pkgPaths, filepath.Join(cockroachDB, pkg))
				}
			} else {
				pkgPaths = []string{filepath.Join(cockroachDB, pkgScope)}
			}
			...
			if err != nil {
				return errors.Wrapf(err, "error loading package %s", pkgPaths)
			}
