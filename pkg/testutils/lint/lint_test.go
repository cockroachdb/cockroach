...
func TestLint(t *testing.T) {
	...
	t.Run("TestNoEnumeratingAllTables", func(t *testing.T) {
		...
		const (
			...
			// virtual tables respectively, that have chosen to serve as indicators,
			// if they are detected in a test, that that test may be enumerating *all*
			// system or virtual tables which is generally undesirable outside of a
			// few specific allow-listed cases. There is nothing special about these
			// two tables other than that are not directly referenced in tests other
			// than those deliberately enumerating all tables so they're well-suited
			// for this purpose. We could add others here as well if needed, and add
		...
		cmd, stderr, filter, err := dirCmd(
			...
			":!sql/catalog/internal/catkv/testdata/",              // exempt: deliberate test of bootstrap catalog.
			":!cli/testdata/doctor/",                              // TODO(foundations): fix overly broad tests.
