...
func (dn *dockerNode) removeAllContainers(ctx context.Context) error {
	...
	if err != nil {
		return errors.Wrap(
			err,
			fmt.Sprintf("cannot list all containers on docker image %s", imageName),
		)
	}
	for _, cont := range conts {
		...
		if err != nil {
			return errors.Wrap(err, fmt.Sprintf("cannot remove container %s", cont.Names))
		}
	}
	return nil
}
...
func (dn *dockerNode) waitServerStarts(ctx context.Context) error {
	...
	if err != nil {
		return errors.Wrap(err, fmt.Sprintf("cannot run fsnotify to listen to %s:\nres:%#v", listenURLFile, res))
	}
...
func (dn *dockerNode) execSQLQuery(
	...
	if err != nil {
		return nil, errors.Wrap(err, fmt.Sprintf("error executing query \"%s\"", sqlQuery))
	}
...
func (dn *dockerNode) rmContainer(ctx context.Context) error {
	if err := dn.cl.ContainerRemove(ctx, dn.contID, types.ContainerRemoveOptions{
		...
		return errors.Wrap(err, fmt.Sprintf("cannot remove container %s", dn.contID))
	}
	dn.contID = ""
	return nil
}
