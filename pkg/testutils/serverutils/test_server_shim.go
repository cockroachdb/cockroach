...
// or just for test tenants via COCKROACH_DISABLE_TEST_TENANT.
func ShouldStartDefaultTestTenant(t testing.TB) bool {
	...
	enable := !util.ConstantWithMetamorphicTestBool("disable-test-tenant", false)
	forceDisable := envutil.EnvOrDefaultBool("COCKROACH_DISABLE_TEST_TENANT", false)
	if forceDisable || skip.UnderBench() {
