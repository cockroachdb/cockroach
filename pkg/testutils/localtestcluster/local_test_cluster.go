...
func (ltc *LocalTestCluster) Start(t testing.TB, baseCtx *base.Config, initFactory InitFactoryFn) {
	...
	cfg.NodeLiveness = nodeliveness.NewNodeLiveness(nodeliveness.NodeLivenessOptions{
	...
	if !ltc.DisableLivenessHeartbeat {
		cfg.NodeLiveness.Start(ctx,
			nodeliveness.NodeLivenessStartOptions{Stopper: ltc.stopper, Engines: []storage.Engine{ltc.Eng}})
	}
