...
// AllPatchReleases returns all non-withdrawn patch releases in the
// given series. This is useful for pre-release versions (e.g., on release
// branches) where we can't compare patch numbers but want to find
// available patches in the same series.
func AllPatchReleases(seriesStr string) ([]string, error) {
	series, ok := releaseData[seriesStr]
	if !ok {
		return nil, fmt.Errorf("no release data for series %s", seriesStr)
	}

	return activePatchReleases(series), nil
}
