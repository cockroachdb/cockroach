...
var backwardCompatibleMigrations = []migrationDescriptor{
	...
	{
		// Introduced in v20.1.
		// TODO(radu): Bake this migration into v20.2.
		name: "create statement_info_requests, statement_info and " +
			"system.statement_bundle_chunks tables",
		workFn:              createStatementInfoSystemTables,
		includedInBootstrap: cluster.VersionByKey(cluster.VersionStatementInfoSystemTables),
		newDescriptorIDs: staticIDs(keys.StatementBundleChunksTableID,
			keys.StatementInfoRequestsTableID, keys.StatementInfoTableID),
	},
}
...
func createStatementInfoSystemTables(ctx context.Context, r runner) error {
	...
	if err := createSystemTable(ctx, r, sqlbase.StatementInfoRequestsTable); err != nil {
		return errors.Wrap(err, "failed to create system.statement_info_requests")
	}
	if err := createSystemTable(ctx, r, sqlbase.StatementInfoTable); err != nil {
		return errors.Wrap(err, "failed to create system.statement_info")
	}
	return nil
}
