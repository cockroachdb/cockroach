...
func retireOldTsPurgeIntervalSettings(ctx context.Context, r runner) error {
	// We are going to deprecate `timeseries.resolution_10s.storage_duration` into
	// `timeseries.storage.10s_resolution_ttl` if the latter is not yet defined,
	// then `timeseries.storage.10s_resolution_ttl` into
	// `timeseries.storage.resolution_10s.ttl` if the latter is not defined.
	//
	// Ditto for the `30m` resolution.

	// Copy 'timeseries.resolution_10s.storage_duration' into
	// 'timeseries.storage.10s_resolution_ttl' if the former is defined
...
INSERT INTO system.settings (name, value, "lastUpdated", "valueType")
   SELECT 'timeseries.storage.10s_resolution_ttl', value, "lastUpdated", "valueType"
     FROM system.settings WHERE name = 'timeseries.resolution_10s.storage_duration'
...
ON CONFLICT (name) DO NOTHING`,
	...
	if _, err := r.sqlExecutor.Exec(ctx, "copy-setting", nil /* txn */, `
INSERT INTO system.settings (name, value, "lastUpdated", "valueType")
   SELECT 'timeseries.storage.30m_resolution_ttl', value, "lastUpdated", "valueType"
     FROM system.settings WHERE name = 'timeseries.resolution_30m.storage_duration'
ON CONFLICT (name) DO NOTHING`,
	); err != nil {
		return err
	}
