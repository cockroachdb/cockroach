...
func addDefaultMetaAndLivenessZoneConfigs(ctx context.Context, r runner) error {
	...
	metaZone, err := getZoneConfigByID(ctx, r, keys.MetaRangesID)
	if err != nil {
		return err
	}
	...
	livenessZone, err := getZoneConfigByID(ctx, r, keys.SystemRangesID)
	if err != nil {
		return err
	}
...
func getZoneConfigByID(ctx context.Context, r runner, id uint32) (config.ZoneConfig, error) {
	return getZoneConfigFull(ctx, r, fmt.Sprintf("RANGE %s", config.NamedZonesByID[id]))
}

func getZoneConfigByTable(ctx context.Context, r runner, table string) (config.ZoneConfig, error) {
	return getZoneConfigFull(ctx, r, fmt.Sprintf("TABLE %s", tree.Name(table)))
}

func getZoneConfigFull(ctx context.Context, r runner, stmtFor string) (config.ZoneConfig, error) {
	stmt := fmt.Sprintf(
...
func addDefaultSystemJobsZoneConfig(ctx context.Context, r runner) error {
	...
	jobsZone, err := getZoneConfigByTable(ctx, r, "system.jobs")
	if err != nil {
		return err
	}
