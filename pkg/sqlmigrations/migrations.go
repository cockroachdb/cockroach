...
var backwardCompatibleMigrations = []migrationDescriptor{
	...
	{
		name:                "create schema supporting system.namespace table",
		workFn:              createSchemaSupportingSystemNamespaceTable,
...
func createSchemaSupportingSystemNamespaceTable(ctx context.Context, r runner) error {
	err := r.db.Txn(ctx, func(ctx context.Context, txn *client.Txn) error {
		b := txn.NewBatch()
...
ON CONFLICT("parentID", "parentSchemaID", name) DO NOTHING`,
