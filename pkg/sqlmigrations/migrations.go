...
func createSystemTable(ctx context.Context, r runner, desc sqlbase.TableDescriptor) error {
	...
	err := r.db.Txn(ctx, func(ctx context.Context, txn *client.Txn) error {
		b := txn.NewBatch()
		if cluster.Version.IsActive(ctx, r.sqlExecutor.Settings(), cluster.VersionNamespaceTableWithSchemas) {
			b.CPut(sqlbase.NewPublicTableKey(desc.GetParentID(), desc.GetName()).Key(), desc.GetID(), nil)
		} else {
