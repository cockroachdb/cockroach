...
`
	...
	require.Equal(t, 7, len(newJobsTable.PublicColumnsNew()))
	require.Equal(t, "created_by_type", newJobsTable.PublicColumnsNew()[5].GetName())
	require.Equal(t, "created_by_id", newJobsTable.PublicColumnsNew()[6].GetName())
...
func TestVersionAlterSystemJobsAddSqllivenessColumnsAddNewSystemSqllivenessTable(t *testing.T) {
	...
	newJobsTable := catalogkv.TestingGetTableDescriptor(
		mt.kvDB, keys.SystemSQLCodec, "system", "jobs")
	require.Equal(t, 9, len(newJobsTable.PublicColumnsNew()))
	require.Equal(t, "claim_session_id", newJobsTable.PublicColumnsNew()[7].GetName())
	require.Equal(t, "claim_instance_id", newJobsTable.PublicColumnsNew()[8].GetName())
