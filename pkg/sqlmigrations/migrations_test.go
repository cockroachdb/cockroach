...
func TestAlterSystemJobsTableAddLeaseColumn(t *testing.T) {
	defer leaktest.AfterTest(t)()
	ctx := context.Background()
	...
	newJobsTable := sqlbase.GetTableDescriptor(
		mt.kvDB, keys.SystemSQLCodec, "system", "jobs")
	require.Equal(t, 8, len(newJobsTable.Columns))
	require.Equal(t, "lease", newJobsTable.Columns[7].Name)
	...
	require.Equal(t, "lease", newJobsTable.Families[2].Name)
