...
func TestAddDefaultMetaZoneConfigMigration(t *testing.T) {
	...
	newMigrations := make([]migrationDescriptor, 0, len(backwardCompatibleMigrations))
	for _, m := range backwardCompatibleMigrations {
		if m.name == "add default .meta and .liveness zone configs" {
			continue
		}
		newMigrations = append(newMigrations, m)
	}
	...
	s, sqlDB, kvDB := serverutils.StartServer(t, base.TestServerArgs{})
	defer s.Stopper().Stop(ctx)
	ids := []int{keys.MetaRangesID, keys.LivenessRangesID}
	...
	checkZoneConfig(keys.MetaRangesID, expected)
	expected.GC.TTLSeconds = 60
	checkZoneConfig(keys.LivenessRangesID, expected)

	for _, id := range ids {
		// Modify the .meta and .liveness zone configs.
	...
	checkZoneConfig(keys.MetaRangesID, config.DefaultZoneConfig())
	checkZoneConfig(keys.LivenessRangesID, config.DefaultZoneConfig())
}
