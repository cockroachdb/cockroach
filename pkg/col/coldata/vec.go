...
func NewMemColumn(t *types.T, n int, factory ColumnFactory) Vec {
	canonicalTypeFamily, ok := typeconv.TypeFamilyToCanonicalTypeFamily[t.Family()]
	if !ok {
		panic(fmt.Sprintf("unexpectedly didn't find canonical type family for %s", t.Family()))
	}
	return &memColumn{
		t:                   t,
