...
func (m *memColumn) CanonicalTypeFamily() types.Family {
	// TODO(yuzefovich): figure out whether we want to store the canonical type
	// family explicitly in memColumn.
	return typeconv.TypeFamilyToCanonicalTypeFamily[m.t.Family()]
}
...
func (m *memColumn) Length() int {
	switch typeconv.TypeFamilyToCanonicalTypeFamily[m.t.Family()] {
	case types.BoolFamily:
