...
type Vec interface {
	...
	// behavior).
	Window(colType phystypes.T, start uint64, end uint64) Vec
	...
	PrettyValueAt(idx uint16, colType phystypes.T) string
...
func NewMemColumn(t phystypes.T, n int) Vec {
	nulls := NewNulls(n)
...
func (m *memColumn) Type() phystypes.T {
	return m.t
}
