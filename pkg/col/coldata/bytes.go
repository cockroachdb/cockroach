...
type Bytes struct {
	...
	// Also, this helps us in maintains the assumption of non-decreasing offsets.
	maxSetIndex int
}
...
func (b *Bytes) AssertOffsetsAreNonDecreasing(n uint64) {
	for j := uint64(1); j <= n; j++ {
		if b.offsets[j] < b.offsets[j-1] {
			panic(errors.AssertionFailedf("unexpectedly found decreasing offsets: %s", b))
		}
	}
}
...
func (b *Bytes) UpdateOffsetsToBeNonDecreasing(n uint64) {
	...
	for j := uint64(1); j <= n; j++ {
		if b.offsets[j] == 0 {
			...
			// Having a zero offset is expected (when an actuall NULL value is there)
...
// maybeBackfillOffsets is an optimized version of UpdateOffsetsToBeNonDecreasing
// that assumes that all offsets up to b.maxSetIndex+1 are non-decreasing.
