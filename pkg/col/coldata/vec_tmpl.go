...
func (m *memColumn) Window(colType phystypes.T, start uint64, end uint64) Vec {
	switch colType {
	// {{range .}}
	case _TYPES_T:
		col := m._TemplateType()
...
func (m *memColumn) PrettyValueAt(colIdx uint16, colType phystypes.T) string {
...
func SetValueAt(v Vec, elem interface{}, rowIdx uint16, colType phystypes.T) {
	switch colType {
	// {{range .}}
	case _TYPES_T:
