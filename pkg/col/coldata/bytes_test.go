...
func TestBytes(t *testing.T) {
	...
	t.Run("Simple", func(t *testing.T) {
		...
		// Verify that we can overwrite a value.
		b1.Set(1, []byte("overwrite"))
		require.Equal(t, "overwrite", string(b1.Get(1)))
		// "hello" should still be there.
		require.Equal(t, "hello", string(b1.Get(0)))

		// Verify that if we zero the Bytes, we get back a zero bytes slice.
		b1.Zero()
		require.Equal(t, []byte{}, b1.Get(1))
		require.Panics(t, func() { b1.Get(2) }, "expected index out of bounds")
	})
