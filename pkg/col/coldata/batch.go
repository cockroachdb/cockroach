...
var unexpectedAllocIdxErr = errors.New("unexpected allocIdx")
...
func NewMemBatchWithCapacity(typs []*types.T, capacity int, factory ColumnFactory) Batch {
	...
	// family appears in typs. Datum-backed vectors are ignored.
	//
	//gcassert:noescape
	var numForCanonicalType [numCanonicalTypes]int
	for i, t := range typs {
		cf := typeconv.TypeFamilyToCanonicalTypeFamily(t.Family())
		cols[i] = memColumn{
			t:                   t,
			canonicalTypeFamily: cf,
		}
		b.b[i] = &cols[i]
		if cf != typeconv.DatumVecCanonicalTypeFamily {
			allocIdx := toAllocIdx(cf, cols[i].t.Width())
	...
	if maxSame > 1 {
		...
		for allocIdx, numColumns := range numForCanonicalType {
			if numColumns > 1 {
				...
				for i := range cols {
					if cf := cols[i].canonicalTypeFamily; cf != typeconv.DatumVecCanonicalTypeFamily {
						if toAllocIdx(cf, cols[i].t.Width()) == allocIdx {
