...
// tests.
func SetBatchSizeForTests(newBatchSize uint64) {
...
func (b *zeroBatch) Length() uint64 {
	return 0
}

func (b *zeroBatch) SetLength(uint64) {
...
func (m *MemBatch) Selection() []uint64 {
	if !m.useSel {
		return nil
	}
	return m.sel
...
func (m *MemBatch) SetLength(n uint64) {
	m.n = n
	for _, v := range m.b {
...
func (m *MemBatch) String() string {
	...
	for i := 0; i < int(m.Length()); i++ {
		...
		for colIdx, v := range m.ColVecs() {
			strs[colIdx] = fmt.Sprintf("%v", GetValueAt(v, uint64(i), v.Type()))
		}
