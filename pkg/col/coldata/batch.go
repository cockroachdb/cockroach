...
func (m *MemBatch) Reset(types []coltypes.T, length int) {
	m = ResetNoTruncation(m, types, length).(*MemBatch)
	m.b = m.b[:len(types)]
}
...
// type schema).
func ResetNoTruncation(b Batch, types []coltypes.T, length int) Batch {
	var m *MemBatch
	if b != nil {
		var ok bool
		m, ok = b.(*MemBatch)
		if !ok {
			panic("unexpectedly passed-in batch is not coldata.MemBatch")
		}
	}
	...
	if cannotReuse {
		res := NewMemBatchWithSize(types, length).(*MemBatch)
