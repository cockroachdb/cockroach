...
type Batch interface {
	...
	// with the given phystypes. If it's possible, Reset will reuse the existing
	...
	Reset(types []phystypes.T, length int)
...
// NewMemBatch allocates a new in-memory Batch. A phystypes.Unknown type
// will create a placeholder Vec that may not be accessed.
// TODO(jordan): pool these allocations.
func NewMemBatch(types []phystypes.T) Batch {
...
func NewMemBatchWithSize(types []phystypes.T, size int) Batch {
...
func (m *MemBatch) Reset(types []phystypes.T, length int) {
