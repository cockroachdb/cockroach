...
func newTestMemColumn(typ *types.T, n int) coldata.Vec {
	return coldata.NewMemColumn(typ, n, coldata.StandardVectorizedColumnFactory)
}
...
func TestMemColumnWindow(t *testing.T) {
	...
	c := newTestMemColumn(types.Int, coldata.BatchSize())
...
func TestAppend(t *testing.T) {
	...
	src := newTestMemColumn(typ, coldata.BatchSize())
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			dest := newTestMemColumn(typ, coldata.BatchSize())
			dest.Append(tc.args)
...
func TestCopyNulls(t *testing.T) {
	...
	dst := newTestMemColumn(typ, coldata.BatchSize())
	dstInts := dst.Int64()
	for i := range dstInts {
		dstInts[i] = int64(1)
	}
