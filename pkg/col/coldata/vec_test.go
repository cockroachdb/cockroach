...
func TestVecWindow(t *testing.T) {
	...
	c := coldata.NewVec(context.Background(), types.Int, coldata.BatchSize(), coldata.StandardColumnFactory)
...
func TestAppend(t *testing.T) {
	...
	src := coldata.NewVec(context.Background(), typ, coldata.BatchSize(), coldata.StandardColumnFactory)
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			dest := coldata.NewVec(context.Background(), typ, coldata.BatchSize(), coldata.StandardColumnFactory)
			dest.Append(tc.args)
...
func TestCopyNulls(t *testing.T) {
	...
	dst := coldata.NewVec(context.Background(), typ, coldata.BatchSize(), coldata.StandardColumnFactory)
	dstInts := dst.Int64()
	for i := range dstInts {
		dstInts[i] = int64(1)
	}
