...
type DatumVec interface {
	...
	// size of the individual datum objects starting from the given index. So,
	// Size is relatively cheap when startIdx = length, and expensive when
	// startIdx = 0. A nonzero startIdx should only be used when elements before
	// startIdx are guaranteed not to have been modified.
