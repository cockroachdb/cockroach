...
// flushed to a file. Files will be created in QueueCfg.Dir with increasing
// sequence numbers. When a file reaches QueueCfg.MaxFileSizeBytes, a new file
...
// diskQueue. QueueCfg.Dir will also be removed on Close, deleting all files.
...
func (cfg *QueueCfg) EnsureDefaults() error {
	if cfg.Typs == nil {
		return errors.New("typs unset on QueueCfg")
	}
...
func NewDiskQueue(cfg QueueCfg, fs vfs.FS) (Queue, error) {
...
func (d *diskQueue) rotateFile() error {
	fName := path.Join(d.cfg.Path, d.cfg.Dir, strconv.Itoa(d.seqNo))
