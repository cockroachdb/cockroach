...
func randomDataFromType(
	rng *rand.Rand, t colphystypes.T, n int, nullProbability float64,
) *array.Data {
...
func TestRecordBatchSerializer(t *testing.T) {
	...
	t.Run("UnsupportedSchema", func(t *testing.T) {
		_, err := colserde.NewRecordBatchSerializer([]colphystypes.T{})
	...
	t.Run("SerializeDifferentColumnLengths", func(t *testing.T) {
		s, err := colserde.NewRecordBatchSerializer([]colphystypes.T{colphystypes.Int64, colphystypes.Int64})
...
func TestRecordBatchSerializerSerializeDeserializeRandom(t *testing.T) {
	...
	var (
		supportedTypes  = make([]colphystypes.T, 0, len(colphystypes.AllTypes))
		typs            = make([]colphystypes.T, rng.Intn(maxTypes)+1)
	...
	for _, t := range colphystypes.AllTypes {
		if t == colphystypes.Decimal {
			continue
		}
