...
// NewFileSerializer creates a FileSerializer for the given phystypes. The caller is
// responsible for closing the given writer.
func NewFileSerializer(w io.Writer, typs []phystypes.T) (*FileSerializer, error) {
...
func (d *FileDeserializer) Typs() []phystypes.T {
	return d.typs
}
...
func (d *FileDeserializer) init() ([]phystypes.T, error) {
	...
	typs := make([]phystypes.T, schema.FieldsLength())
...
func schema(fb *flatbuffers.Builder, typs []phystypes.T) flatbuffers.UOffsetT {
...
func schemaMessage(fb *flatbuffers.Builder, typs []phystypes.T) flatbuffers.UOffsetT {
...
func fileFooter(
	fb *flatbuffers.Builder, typs []phystypes.T, recordBatches []fileBlock,
...
func typeFromField(field *arrowserde.Field) (phystypes.T, error) {
	var typeTab flatbuffers.Table
	field.Type(&typeTab)
	typeType := field.TypeType()
	switch typeType {
	case arrowserde.TypeBool:
		return phystypes.Bool, nil
	case arrowserde.TypeBinary:
		return phystypes.Bytes, nil
	case arrowserde.TypeInt:
		...
		if intType.IsSigned() > 0 {
			...
			case 16:
				return phystypes.Int16, nil
			case 32:
				return phystypes.Int32, nil
			case 64:
				return phystypes.Int64, nil
			default:
				return phystypes.Unhandled, errors.Errorf(`unhandled bit width %d`, intType.BitWidth())
			}
		}
	case arrowserde.TypeFloatingPoint:
		...
		case arrowserde.PrecisionDOUBLE:
			return phystypes.Float64, nil
		default:
			return phystypes.Unhandled, errors.Errorf(`unhandled float precision %d`, floatType.Precision())
		}
	case arrowserde.TypeTimestamp:
		return phystypes.Timestamp, nil
	}
	...
	if typeName, ok := arrowserde.EnumNamesType[typeType]; ok {
		return phystypes.Unhandled, errors.Errorf(`unknown type: %s`, typeName)
	}
	return phystypes.Unhandled, errors.Errorf(`unknown type: %d`, typeType)
}
