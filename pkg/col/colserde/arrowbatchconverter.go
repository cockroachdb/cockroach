...
func (c *ArrowBatchConverter) BatchToArrow(batch coldata.Batch) ([]*array.Data, error) {
	...
	for vecIdx, typ := range c.typs {
		...
		case types.BytesFamily:
			values = arrowSerializeBytes(n, &offsets, vec.Bytes())
		case types.JsonFamily:
			values = arrowSerializeBytes(n, &offsets, &vec.JSON().Bytes)
...
// arrowSerializeBytes returns the bytes of a serialized flat bytes array. It
// unsafely updates the passed-in offsets array.
func arrowSerializeBytes(batchLength int, offsets *[]byte, bytes *coldata.Bytes) []byte {
