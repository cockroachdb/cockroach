...
type DatumWithEvalCtx struct {
	tree.Datum
	// TODO(yuzefovich): refactor the template generators to pass in evalCtx
	// that is stored in datumVec and do not store it in this struct. It will
	// probably be useful to do it at the same time that we add support for
	// mixed-representation (i.e. datum-backed with non-datum backed)
	// overloads.
