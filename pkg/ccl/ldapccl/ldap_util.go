...
func (lu *ldapUtil) LDAPSConn(ctx context.Context, conf ldapAuthenticatorConf) (err error) {
	...
	// (Fix 2) Every authN attempt acquires a lock on ldapAuthenticatorConf, so
...
func (lu *ldapUtil) Bind(ctx context.Context, username string, password string) (err error) {
	if err = lu.conn.Bind(username, password); err != nil {
...
func (lu *ldapUtil) Search(
	...
	// provided ref:
	// https://github.com/mark-rushakoff/ldapserver/blob/master/search.go#L77-L79
...
// functionalities exposed by `go-ldap` library for crdb modules for authN and
// authZ. The interface has following methods:
// LDAPSConn() - creates a mTLS connection with the LDAP server taking arguments
// for domain CA, ldap client key and cert, ldap server & port
// Bind() - perform a bind given a valid DN and LDAP password
// Search() - performs search on LDAP server binding with bindDN and bindpwd
// expecting search arguments from HBA conf and crdb database connection string.
