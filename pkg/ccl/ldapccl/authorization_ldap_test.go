...
func TestLDAPAuthorization(t *testing.T) {
	...
	testCases := []struct {
		...
		{testName: "empty group list filter",
			...
			expectedErr:            "LDAP authorization: unable to validate authManager authZ options",
			expectedDetailedErrMsg: "error validating authZ hba conf options for LDAP: ldap authorization params in HBA conf missing group list attribute"},
