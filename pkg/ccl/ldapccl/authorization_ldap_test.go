...
func TestLDAPProvisioningDBConsole(t *testing.T) {
	...
	t.Run("provisioning enabled, new user succeeds login", func(t *testing.T) {
		...
		// Verify role membership was synced via LDAP authorization. Poll
		// to account for potential schema cache propagation delay.
		testutils.SucceedsSoon(t, func() error {
			rows = tdb.QueryStr(t,
				`SELECT role FROM system.role_members WHERE member = 'newuser2'`)
			if len(rows) != 1 {
				return errors.Newf("expected 1 role for newuser2, got %d", len(rows))
			}
			if rows[0][0] != "db_admins" {
				return errors.Newf("expected role db_admins, got %s", rows[0][0])
			}
			return nil
		})
	})
