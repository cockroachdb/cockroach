...
func (authManager *ldapAuthManager) ValidateLDAPLogin(
	...
	entry *hba.Entry,
	_ *identmap.Conf,
) (retrievedUserDN string, detailedErrorMsg redact.RedactableString, authError error) {
	...
	if err := authManager.validateLDAPAuthNOptions(); err != nil {
		return "", redact.Sprintf("error validating authN hba conf options for LDAP: %v", err),
			errors.Newf("LDAP authentication: unable to validate authManager authN options")
	}
	...
	if err != nil {
		return userDN, redact.Sprintf("error when binding as user %s with DN(%s) in LDAP server: %v",
