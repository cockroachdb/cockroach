...
func (authenticator *ldapAuthenticator) ValidateLDAPLogin(
	...
	if err := authenticator.setLDAPConfigOptions(entry); err != nil {
		return redact.Sprintf("error parsing hba conf options for LDAP: %s", redact.Sprint(err)),
			errors.Newf("LDAP authentication: unable to parse hba conf options")
	}

	if err := authenticator.validateLDAPOptions(); err != nil {
		return redact.Sprintf("error validating hba conf options for LDAP: %s", redact.Sprint(err)),
	...
	if err != nil {
		return redact.Sprintf("error when trying to create LDAP connection: %s", redact.Sprint(err)),
	...
	if err != nil {
		return redact.Sprintf("error when searching for user in LDAP server: %v", redact.Sprint(err)),
			errors.WithDetailf(
	...
	if err != nil {
		return redact.Sprintf("error when binding as user %s with DN(%s) in LDAP server: %v",
				user.Normalized(), userDN, redact.Sprint(err),
			),
			errors.WithDetailf(
