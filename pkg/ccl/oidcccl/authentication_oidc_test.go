...
// failingOidcManager embeds the happy‑path mock and overrides
// ExchangeVerifyGetTokenInfo so that verification always fails.
type failingOidcManager struct {
	mockOidcManager
}

func (f failingOidcManager) ExchangeVerifyGetTokenInfo(
	ctx context.Context, code, idTokenKey string, _ bool,
) (map[string]json.RawMessage, map[string]json.RawMessage, string, string, error) {
...
func TestOIDCExchangeVerifyFailure(t *testing.T) {
	...
	NewOIDCManager = func(
		...
		baseMock := mockOidcManager{oauth2Config: c, claimEmail: fmt.Sprintf("%s@example.com", usernameUnderTest)}
		return &failingOidcManager{mockOidcManager: baseMock}, nil
	}
...
// issuerMismatchOidcManager reuses mockOidcManager for happy-path helpers
// but forces issuer verification to fail.
type issuerMismatchOidcManager struct {
	mockOidcManager
}

// Verify returns the canonical issuer-mismatch error.
func (m issuerMismatchOidcManager) Verify(ctx context.Context, raw string) (*oidc.IDToken, error) {
	return nil, fmt.Errorf("oidc: token issuer mismatch")
}

// ExchangeVerifyGetTokenInfo also fails with the same error so the end-to-end
// flow reaches the callback handler with a verification failure.
func (m issuerMismatchOidcManager) ExchangeVerifyGetTokenInfo(
	ctx context.Context, code, idTokenKey string, _ bool,
) (map[string]json.RawMessage, map[string]json.RawMessage, string, string, error) {
	return nil, nil, "", "", fmt.Errorf("oidc: token issuer mismatch")
}

var _ IOIDCManager = &issuerMismatchOidcManager{}

// externalIssuerOidcManager simulates "provider != issuer" federation.
// Verify succeeds on a token whose Issuer does not match providerURL.
type externalIssuerOidcManager struct {
	oauth2Config *oauth2.Config
	username     string
}

func (m externalIssuerOidcManager) Verify(ctx context.Context, raw string) (*oidc.IDToken, error) {
	return &oidc.IDToken{
		Issuer:   "https://issuer.example.com", // issuer different from provider
		Subject:  "sub",
		Audience: []string{m.oauth2Config.ClientID},
		Expiry:   time.Now().Add(time.Hour),
		IssuedAt: time.Now(),
	}, nil
}

func (m externalIssuerOidcManager) Exchange(
	context.Context, string, ...oauth2.AuthCodeOption,
) (*oauth2.Token, error) {
	tok := &oauth2.Token{AccessToken: "opaque"}
	tok = tok.WithExtra(map[string]interface{}{idTokenKey: "dummy"})
	return tok, nil
}

func (m externalIssuerOidcManager) AuthCodeURL(state string, opt ...oauth2.AuthCodeOption) string {
	return m.oauth2Config.AuthCodeURL(state, opt...)
}

func (m externalIssuerOidcManager) ExchangeVerifyGetTokenInfo(
	ctx context.Context, code, idTokenKey string, _ bool,
) (map[string]json.RawMessage, map[string]json.RawMessage, string, string, error) {
	// go-oidc will throw an error unless SkipIssuerCheck is configured
	return nil, nil, "", "", fmt.Errorf("oidc: token issuer mismatch")
}

func (m externalIssuerOidcManager) UserInfo(
	context.Context, oauth2.TokenSource,
) (*oidc.UserInfo, error) {
	return nil, nil
}

var _ IOIDCManager = &externalIssuerOidcManager{}

func TestOIDCIssuerValidation(t *testing.T) {
	// Sub‑test 1: provider is issuer; mismatch must fail.
	t.Run("issuer mismatch fails when provider is issuer", func(t *testing.T) {
		defer leaktest.AfterTest(t)()
		defer log.Scope(t).Close(t)
		...
		restore := testutils.TestingHook(
			...
			func(ctx context.Context, c oidcAuthenticationConf, redirectURL string, scopes []string) (IOIDCManager, error) {
				...
				// The base mock needs a config to generate the auth URL.
				baseMock := mockOidcManager{oauth2Config: conf}
				return &issuerMismatchOidcManager{mockOidcManager: baseMock}, nil
			},
		)
		defer restore()
	...
	t.Run("external issuer rejected when provider not issuer", func(t *testing.T) {
		...
		restore := testutils.TestingHook(
			...
			func(ctx context.Context, c oidcAuthenticationConf, redirectURL string, scopes []string) (IOIDCManager, error) {
				...
				return &externalIssuerOidcManager{oauth2Config: conf, username: usernameUnderTest}, nil
			},
		)
		defer restore()
