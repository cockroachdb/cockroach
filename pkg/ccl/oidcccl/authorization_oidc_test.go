...
func (m mockManager) ExchangeVerifyGetTokenInfo(
	...
	// Attempt to parse claims from the access token if it's a JWT.
	var accessTokenClaims map[string]json.RawMessage
	if tok, err := jwt.ParseInsecure([]byte(m.accessToken)); err == nil {
		if claimsMap, err := tok.AsMap(context.Background()); err == nil {
...
func TestOIDCAuthorization_TokenPaths(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			idTokenClaims: map[string]any{
				"groups": []string{roleOwners, roleUsers},
			},
		...
		},
		{
			...
			accessTokenClaims: nil,
			expectedRoles:     []string{roleOwners}, // whatever role was there previously
			expectedStatus:    http.StatusForbidden,
			wantErrSubstring:  "unable to complete authentication",
		},
		{
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			NewOIDCManager = func(_ context.Context, conf oidcAuthenticationConf,
				...
				return &mockManager{
					cfg:         cfg,
					rawIDToken:  makeJWT(t, tc.idTokenClaims, jwkKey),
					accessToken: makeJWT(t, tc.accessTokenClaims, jwkKey),
...
func TestOIDCAuthorization_RoleGrantAndRevoke(t *testing.T) {
	...
	t.Run("grant non-existent role", func(t *testing.T) {
		...
		performOIDCLogin(t, &client, claims, http.StatusForbidden)
		// Since the login failed, the user's roles should remain unchanged from the previous step.
		checkHasRole(t, testUser, roleUsers, true)
		checkHasRole(t, testUser, roleOwners, false)
	})
}
