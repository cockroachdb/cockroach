...
func (oidcAuthentication *oidcAuthenticationServer) authorize(
	...
	tokenSources := func() []struct{ raw, name string } {
		if oidcAuthentication.conf.generateJWTAuthTokenUseToken == useAccessToken {
			return []struct{ raw, name string }{
	...
	for _, ts := range tokenSources {
		...
		if tok, parseErr := jwt.ParseInsecure([]byte(ts.raw)); parseErr == nil {
			...
			if err != nil {
				...
				// Successfully extracted a groups list, stop processing tokens.
				break
			}
		} else {
