...
func (o *oidcManager) ExchangeVerifyGetTokenInfo(
	...
	tokensToProcess := []struct {
		...
		isIDToken        bool                        // marks the token that must be present
	...
	for _, tokenInfo := range tokensToProcess {
		...
		*tokenInfo.rawToken, err = tokenInfo.getVerifiedToken()
		...
		if *tokenInfo.rawToken == "" {
			// Both ID- and Access-tokens are mandatory in the OIDC Authorization Code flow.
			log.Ops.Warningf(ctx, "OIDC: required %s missing from credentials", tokenInfo.name)
