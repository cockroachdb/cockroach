...
var ConfigureOIDC = func(
	...
	mux.HandleFunc(oidcCallbackPath, func(w http.ResponseWriter, r *http.Request) {
		...
		if oidcAuthentication.enabled && !oidcAuthentication.initialized {
			reloadConfig(ctx, oidcAuthentication, st)
		}
		...
		if stateCookie.Value != string(statePb.Secret) {
	...
	reloadConfigLocked(serverCtx, oidcAuthentication, st)
...
func (s *oidcAuthenticationServer) SetOIDCStateCookie(w http.ResponseWriter, r *http.Request) {
	size := 16
	...
	s.stateValidator.add(encodedState)
}
