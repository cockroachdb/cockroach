...
func (o *oidcManager) ExchangeVerifyGetTokenInfo(
	...
	// tokensToProcess defines the tokens to be extracted from the OIDC credentials.
	tokensToProcess := []struct {
		name      string                      // name for logging (e.g., "id_token").
		getValue  func() string               // function to extract the raw token string.
		isIDToken bool                        // marks the token that requires OIDC verification.
		claims    *map[string]json.RawMessage // destination for parsed claims.
		rawToken  *string                     // destination for the raw token string.
	}{
		{
			name:      idTokenKey,
			getValue:  func() string { val, _ := credentials.Extra(idTokenKey).(string); return val },
			isIDToken: true,
		...
		{
			name:      "access_token",
			getValue:  func() string { return credentials.AccessToken },
			isIDToken: false,
	...
	// This is the for loop your boss suggested. It processes each token in the list.
	for _, tokenInfo := range tokensToProcess {
		// Extract the raw token string.
		*tokenInfo.rawToken = tokenInfo.getValue()
		if *tokenInfo.rawToken == "" {
			if tokenInfo.isIDToken {
				// The ID token is required, so we must error out if it's missing.
				return nil, nil, "", "", errors.Newf("OIDC: required %s not found in credentials", tokenInfo.name)
			}
			// The access token might be missing or opaque, so we can safely continue.
			continue
		}

		// If it's the ID token, it MUST be verified against the OIDC provider.
		if tokenInfo.isIDToken {
			if _, err := o.Verify(ctx, *tokenInfo.rawToken); err != nil {
		...
		// This will succeed for standard JWTs and fail gracefully for opaque (non-JWT) tokens.
	...
	// Final sanity check.
	if rawIDToken == "" {
		return nil, nil, "", "", errors.New("OIDC: critical failure, ID token is empty after processing")
	}
...
type IOIDCManager interface {
	...
	ExchangeVerifyGetTokenInfo(context.Context, string, string) (idTokenClaims, accessTokenClaims map[string]json.RawMessage, rawIDToken, rawAccessToken string, err error)
...
var ConfigureOIDC = func(
	...
	handleHTTP(oidcCallbackPath, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		idTokenClaims, _, rawIDToken, rawAccessToken, err := oidcAuthentication.manager.
			ExchangeVerifyGetTokenInfo(ctx, r.URL.Query().Get(codeKey), idTokenKey)
