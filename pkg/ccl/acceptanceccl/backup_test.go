...
func BenchmarkRestoreBig(b *testing.B) {
	...
	container := envutil.EnvOrDefaultString("AZURE_CONTAINER", "")
	accountName := envutil.EnvOrDefaultString("AZURE_ACCOUNT_NAME", "")
	accountKey := envutil.EnvOrDefaultString("AZURE_ACCOUNT_KEY", "")
	...
	b.Run("", func(b *testing.B) {
		...
		r.Exec(fmt.Sprintf(`RESTORE TABLE bench.* FROM '%s' WITH OPTIONS ('into_db'='%s')`, restoreBaseURI, dbName))
