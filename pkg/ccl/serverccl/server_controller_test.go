...
ALTER TENANT application GRANT CAPABILITY can_use_blob_storage;
ALTER TENANT application START SERVICE SHARED`)

	var tenantID uint64
	db.QueryRow(t, "SELECT id FROM [SHOW TENANT application]").Scan(&tenantID)
	tc.WaitForTenantCapabilities(t, roachpb.MustMakeTenantID(tenantID), map[tenantcapabilities.CapabilityID]string{
		tenantcapabilities.CanUseBlobStorage: "true",
	})
