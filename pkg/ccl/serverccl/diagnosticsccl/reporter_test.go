...
func TestTenantReport(t *testing.T) {
	...
	require.NoError(t, rt.server.SQLServer().(*sql.Server).GetSQLStatsProvider().ResetLocalStats(ctx))
...
func TestUsageQuantization(t *testing.T) {
	...
	require.NoError(t, ts.SQLServer().(*sql.Server).GetSQLStatsProvider().ResetLocalStats(ctx))
