...
func testResetSQLStatsRPCForTenant(
	...
	for _, flushed := range []bool{false, true} {
		...
		t.Run(fmt.Sprintf("flushed=%t", flushed), func(t *testing.T) {
			...
			sqlstatstestutil.WaitForStatementStatsCountAtLeast(t,
				sqlutils.MakeSQLRunner(controlCluster.TenantDB(serverccl.RandomServer)),
				1, sqlstatstestutil.StatementFilter{Query: "SELECT _, _, _"})
