...
func TestResetIndexUsageStatsRPCForTenant(t *testing.T) {
	...
	for _, resetTest := range testCases {
...
WHERE
  table_id = ` + testTableIDStr
		...
		// Reset index usage stats.
		resetTest.indexStatsResetter(testHelper)
...
`);
	...
	actual := cluster.tenantConn(0).QueryStr(t,
		`SELECT * from crdb_internal.tables WHERE database_name='test_db1'`);

	//Get index usage stats given a table and database
	//status := cluster.tenantStatusSrv(0 /* idx */)
	//req := &serverpb.TableIndexStatsRequest{Table: "test", Database: "test_db1"}
	// See the session over HTTP on tenant SQL pod 1.
	httpPod1 := testHelper.testCluster().tenantHTTPClient(t, 1)
	defer httpPod1.Close()
	resp := &serverpb.TableIndexStatsResponse{}
	httpPod1.GetJSON("/_status/databases/test_db1/tables/test/indexstats", resp)
	require.NoError(t, err)

	// Assert expected results
	require.Equal(t, actual, &serverpb.TableIndexStatsResponse_ExtendedCollectedIndexUsageStatistics{})
