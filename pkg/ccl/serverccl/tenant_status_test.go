...
func TestTenantStatusCancelQuery(t *testing.T) {
	...
	sqlPod0 := helper.testCluster().tenantConn(0)
	expectedCancellationError := make(chan error)
	defer close(expectedCancellationError)
	go func() {
		if _, err := sqlPod0.DB.ExecContext(ctx, "SELECT pg_sleep(60)"); err != nil {
			expectedCancellationError <- err
		}
	}()
	...
	require.NotEmpty(t, query.ID, "query not found")
	...
	err = httpPod1.PostJSON("/_status/cancel_query/"+session.NodeID.String(), &cancelQueryReq, &cancelQueryResp)
	require.NoError(t, err)
	require.Equal(t, true, cancelQueryResp.Canceled, cancelQueryResp.Error)

	// No longer see the query over SQL on tenant SQL pod 0.
	require.NotContains(t, selectClusterQueryIDs(t, sqlPod0), query.ID)

	select {
	case err := <-expectedCancellationError:
