...
func TestResetSQLStatsTenant(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)
	...
	for _, flushed := range []bool{false, true} {
		t.Run(fmt.Sprintf("flushed=%t", flushed), func(t *testing.T) {
			...
			if !flushed {
				// Disable flush to ensure tests are deterministic.
			...
			ensureExpectedStmtFingerprintExistsInRPCResponse(t, stmts, statsPreReset)
			...
			for _, txn := range statsPostReset.Transactions {
				for _, previousTxn := range statsPreReset.Transactions {
					require.NotEqual(t, txn, previousTxn,
						"expected to have reset SQL stats, but still found transaction %+v", txn)
				}
			}

			for _, stmt := range statsPostReset.Statements {
				for _, previousStmt := range statsPreReset.Statements {
					require.NotEqual(t, stmt, previousStmt,
						"expected to have reset SQL stats, but still found statement %+v", stmt)
				}
			}
			...
			ensureExpectedStmtFingerprintExistsInRPCResponse(t, stmts, statsFromControlCluster)
		})
	}
}

func ensureExpectedStmtFingerprintExistsInRPCResponse(
	t *testing.T, expectedStmts []string, resp *serverpb.StatementsResponse,
) {
	t.Helper()
	...
	for _, stmt := range expectedStmts {
		...
		require.True(t, found, "expected %s to be found in "+
			"control tenant cluster, but it was not found", fingerprint)
	}
}
