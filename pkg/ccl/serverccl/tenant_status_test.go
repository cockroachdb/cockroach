...
func TestContentionEventsForTenant(t *testing.T) {
	...
	controllCluster := testHelper.controlCluster()
...
`)
	...
	fmt.Printf("DELETE ME: %+v\n", testingCluster.tenantConn(0).QueryStr(t, "SELECT * FROM crdb_internal.cluster_contention_events"))
	...
	resp, err = controllCluster.tenantStatusSrv(0).ListContentionEvents(ctx, &serverpb.ListContentionEventsRequest{})
	require.NoError(t, err)
	for _, event := range resp.Events.IndexContentionEvents {
		if event.TableID == descpb.ID(testTableID) && event.IndexID == descpb.IndexID(1) {
