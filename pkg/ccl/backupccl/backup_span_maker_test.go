...
func TestBackupSpanBoundOnRange(t *testing.T) {
	...
	testutils.RunTrueAndFalse(t, "split-on-range", func(t *testing.T, splitOnRange bool) {
		...
		requestSpans, err := makeRequestSpans(ctx, specs, tc.Servers[0].RangeDescIteratorFactory().(rangedesc.IteratorFactory), splitOnRange)
		require.NoError(t, err)
		expectedSpanCount := 1
		if splitOnRange {
			expectedSpanCount = expectedSpanCount * numRanges
		}
		require.Equal(t, expectedSpanCount, len(requestSpans))
		f, err := span.MakeFrontier()
		require.NoError(t, err)
		for _, sp := range requestSpans {
	...
	testutils.RunTrueAndFalse(t, "split-on-range", func(t *testing.T, splitOnRange bool) {
		for _, spanCount := range []int{1, 5, 20, 100, 1000} {
			...
			for i := range requestSpans {
				requestSpans[i].start = hlc.Timestamp{WallTime: int64(i)}
			}
			...
			for {
				select {
				case spanSet := <-makeWorkQueue(numSenders, requestSpans, splitOnRange):
					chunkCount++
					switch {
					case !splitOnRange:
