...
func selectTargets(
	...
	if tableCoverage == tree.Complete {
		return fullClusterTargets(allDescs)
	}
...
func allocateTableRewrites(
	...
	if tableCoverage == tree.Complete {
		var err error
		maxDefaultID := keys.MinUserDescID + len(defaultUserDBs)
		numberOfIncrements := maxDescIDInBackup - uint32(maxDefaultID)
	...
	if err := p.ExecCfg().DB.Txn(ctx, func(ctx context.Context, txn *client.Txn) error {
		...
		for name := range restoreDBNames {
			...
			if found && tableCoverage == tree.Complete {
				if foundID > 52 {
		...
		for _, table := range tablesByID {
			...
			if renaming {
				targetDB = overrideDB
			} else if tableCoverage == tree.Complete && table.ParentID < keys.MinUserDescID+sqlbase.ID(len(defaultUserDBs)) {
			...
			} else if tableCoverage == tree.Complete {
...
func doRestorePlan(
	...
	//if mainBackupDescs[0].TableCoverage != int32(restoreStmt.TableCoverage) {
	if restoreStmt.TableCoverage == tree.Complete && mainBackupDescs[0].TableCoverage == int32(tree.Partial) {
	...
	descCount, err := userDescriptorCount(ctx, p.ExecCfg().DB, p.ExecCfg().Clock.Now())
	if err != nil {
		return errors.Wrap(err, "looking up user descriptors during restore")
	}
	if descCount != 0 && restoreStmt.TableCoverage == tree.Complete {
		return errors.Errorf(
	...
	sqlDescs, restoreDBs, err := selectTargets(ctx, p, mainBackupDescs, restoreStmt.Targets, restoreStmt.TableCoverage, endTime)
	if err != nil {
		return err
	}
	...
	tableRewrites, err := allocateTableRewrites(ctx, p, databasesByID, filteredTablesByID, restoreDBs, restoreStmt.TableCoverage, opts)
	if err != nil {
		return err
	}
	...
	_, errCh, err := p.ExecCfg().JobRegistry.CreateAndStartJob(ctx, resultsCh, jobs.Record{
		...
		Details: jobspb.RestoreDetails{
			...
			TableCoverage:      int32(restoreStmt.TableCoverage),
		},
...
func userDescriptorCount(ctx context.Context, db *client.DB, asOf hlc.Timestamp) (int, error) {
	allDescs, err := loadAllDescs(ctx, db, asOf)
	if err != nil {
		return 0, err
	}
	...
	for _, desc := range allDescs {
		if t := desc.Table(hlc.Timestamp{}); t != nil {
			// Don't count the default tables or system tables.
			if t.ParentID != sqlbase.SystemDB.ID {
				count++
			}
		} else if db := desc.GetDatabase(); db != nil {
			if db.ID != sqlbase.SystemDB.ID && db.ID >= keys.MinUserDescID+sqlbase.ID(len(defaultUserDBs)) {
				count++
			}
		}
	}
...
func createImportingTables(
	...
	if tree.TableCoverage(details.TableCoverage) == tree.Complete {
		databases = append(databases, &sqlbase.DatabaseDescriptor{ID: tempSystemDBID, Name: restoreTempSystemDB})
	}
...
func (r *restoreResumer) Resume(
	...
	r.tables = tables
	r.tableCoverage = tree.TableCoverage(details.TableCoverage)
...
func (r *restoreResumer) OnSuccess(ctx context.Context, txn *client.Txn) error {
	...
	// accessed.
	descriptorTxn := r.execCfg.DB.NewTxn(ctx, "descriptor-txn")
	b := descriptorTxn.NewBatch()
	...
	if err := descriptorTxn.Run(ctx, b); err != nil {
		return errors.Wrap(err, "publishing tables")
	}
	descriptorTxn.Commit(ctx)

	if r.tableCoverage == tree.Complete {
		err := r.restoreSystemTables(ctx)
		if err != nil {
			return err
		}
	}
...
// restoreSystemTables atomically replaces the contents of the system tables with the data
// from the restored system tables.
func (r *restoreResumer) restoreSystemTables(ctx context.Context) error {
	tables := make([]string, 0, len(fullClusterSystemTables))
	for name := range fullClusterSystemTables {
		tables = append(tables, name)
	}
	executor := r.execCfg.InternalExecutor
	var err error
	for _, table := range tables {
		if table == sqlbase.JobsTable.Name {
			// FIXME(pbardea): Skip restoring jobs table for now.
			continue
		}

		systemTxn := r.execCfg.DB.NewTxn(ctx, "system-restore-txn")
		deleteQuery := fmt.Sprintf("DELETE FROM system.%s WHERE true;", table)
		restoreQuery := fmt.Sprintf("INSERT INTO system.%s (SELECT * FROM %s.%s);", table, restoreTempSystemDB, table)
		txnDebugName := fmt.Sprintf("restore-system-table-%s", table)
		_, err = executor.Exec(ctx, txnDebugName+"-data-deletion", systemTxn, deleteQuery)
		if err != nil {
			return errors.Wrapf(err, "restoring system.%s", table)
		}
		...
		if err != nil {
			return errors.Wrap(err, "committing system table restoration")
		}
	}

	// After restoring the system tables, the temporary database holding the
	// system tables.
