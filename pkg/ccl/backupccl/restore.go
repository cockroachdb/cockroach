...
func WriteTableDescs(
	...
	err := func() error {
		...
		for _, desc := range databases {
			...
			if err := sql.WriteNewDescToBatch(ctx, false /* kvTrace */, settings, txn, b, desc.ID, desc); err != nil {
				return err
			}
		...
		for i := range tables {
			...
			if err := sql.WriteNewDescToBatch(ctx, false /* kvTrace */, settings, txn, b, tables[i].ID, tables[i]); err != nil {
				return err
			}
...
func (r *restoreResumer) OnFailOrCancel(ctx context.Context, txn *client.Txn) error {
	...
	for _, tableDesc := range details.TableDescs {
		...
		if err := sql.WriteNewDescToBatch(ctx, false /* kvTrace */, r.settings, txn, b, tableDesc.ID, tableDesc); err != nil {
			return err
		}
	}
