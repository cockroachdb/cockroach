...
func isDatabaseEmpty(
	...
	ignoredTables []*sql.TableDescriptor,
) (bool, error) {
	...
	ignoredIDs := make(map[sqlbase.ID]struct{}, 0)
	for _, table := range ignoredTables {
		ignoredIDs[table.ID] = struct{}{}
	}

	for _, desc := range allDescs {
		if t := desc.Table(hlc.Timestamp{}); t != nil {
			if _, ok := ignoredIDs[t.GetID()]; ok {
				continue
			}
...
func (r *restoreResumer) OnFailOrCancel(ctx context.Context, txn *client.Txn) error {
	...
	for _, dbDesc := range r.databases {
		// We need to ignore details.TableDescs since we haven't commited the txn that deletes these.
		isDBEmpty, err = isDatabaseEmpty(ctx, r.execCfg.DB, dbDesc, details.TableDescs)
		if err != nil {
