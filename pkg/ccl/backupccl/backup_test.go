...
func TestBackupNoOverwriteCheckpoint(t *testing.T) {
	...
	// progress loop, we need to add 1 to account for the checkpoint written
	// at the start of Resume.
	require.Equal(t, numCheckpointsWritten+1, actualNumCheckpointsWritten)
}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, checkpoint := range checkpoints {
				var desc []byte
				writeNewCheckpointFile(ctx, execCfg.Settings, store, checkpoint, desc)
			}
			require.NoError(t, err)
			var actual string
			require.NoError(t, store.List(ctx, "/progress/", "", func(f string) error {
			...
			require.Equal(t, checkpoints[numCheckpoints-1], actual)
		})
	}
}
