...
// TestBackupRestoreInsideTenant verifies that backup and restore work inside
// tenants. It tests both single- and multi-pod tenants. Currently, verification
...
// "starting restore data" for nsql1, nsql2, and nsql3.
...
func TestBackupRestoreInsideTenant(t *testing.T) {
	...
	for _, npods := range []int{singleNode, multiNode} {
		...
		tenant11C2 := make([]*sqlutils.SQLRunner, npods)
		cleanupT11C2 := make([]func(), npods)
		for i := 0; i < npods; i++ {
			...
			tenant11C2[i], cleanupT11C2[i] = makeTenant(srv2, 11, i != 0)
			defer cleanupT11C2[i]()
		}
		...
		t.Run("tenant-backup", func(t *testing.T) {
			...
			t.Run("cluster-restore", func(t *testing.T) {
				...
				t.Run("into-different-tenant-id", func(t *testing.T) {
					tenant11C2[0].ExpectErr(t, `cannot cluster RESTORE backups taken from different tenant: 10`,
		...
		t.Run("system-backup", func(t *testing.T) {
			...
			tenant20C2 := make([]*sqlutils.SQLRunner, npods)
			cleanupT20C2 := make([]func(), npods)
			for i := 0; i < npods; i++ {
				tenant20C2[i], cleanupT20C2[i] = makeTenant(srv2, 20, i != 0)
				defer cleanupT20C2[i]()
			}
			...
			t.Run("cluster-restore", func(t *testing.T) {
				t.Run("with-tenant", func(t *testing.T) {
					...
					tenant20C2[0].ExpectErr(t, `only the system tenant can restore other tenants`,
						`RESTORE FROM $1`, httpAddr)
				})
				...
				t.Run("with-no-tenant", func(t *testing.T) {
					...
					tenant20C2[0].ExpectErr(t, `cannot cluster RESTORE backups taken from different tenant: system`,
