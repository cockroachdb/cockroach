...
func TestBackupJobFailsInRestoredTenant(t *testing.T) {
	...
	_, err = t10.SQLConn(t, "").Exec("BACKUP INTO 'nodelocal://1/example-schedule'")
	require.Contains(t, err.Error(), pausePoint)
	var backupJobID int
	...
	systemDB.Exec(t, `BACKUP TENANT 10 TO 'nodelocal://1/foo'`)

	systemDB.Exec(t, `RESTORE TENANT 10 FROM 'nodelocal://1/foo' WITH virtual_cluster = '20',virtual_cluster_name = 'cluster-20'`)
