...
func TestBackupCheckpointInBaseDirectory(t *testing.T) {
	...
	err = store.List(ctx, string(latest)+"/"+backupProgressDirectory, "", func(p string) error {
		latestFile = p
		return nil
	}, 1)
	require.NoError(t, err)

	// Delete the BACKUP-CHECKPOINT file in the progress directory so we can
	// test that if we try an incremental backup it correctly find the LATEST
	// file in the base directory.
	err = store.Delete(ctx, string(latest)+"/"+backupProgressDirectory+latestFile)
	require.NoError(t, err)
