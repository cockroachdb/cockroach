...
func TestBackupRestoreAppend(t *testing.T) {
	...
	for _, test := range []struct {
		...
		if test.name == "userfile" {
			...
			require.NoError(t, err)
			defer store.Close()
			files, err := store.ListFiles(ctx, "*/*/*/BACKUP_MANIFEST")
			require.NoError(t, err)
			full1 = files[0]
			full1 = strings.TrimSuffix(full1, "BACKUP_MANIFEST")
			full2 = files[1]
			full2 = strings.TrimSuffix(full2, "BACKUP_MANIFEST")
			...
			subdirFiles, err := store.ListFiles(ctx, path.Join("foo", fmt.Sprintf("%s*",
				specifiedSubdir), "BACKUP_MANIFEST"))
			require.NoError(t, err)
			subdirFull1 = strings.TrimSuffix(strings.TrimPrefix(subdirFiles[0], "foo"), "BACKUP_MANIFEST")
			subdirFull2 = strings.TrimSuffix(strings.TrimPrefix(subdirFiles[1], "foo"), "BACKUP_MANIFEST")
			runRestores(test.collectionsWithSubdir, subdirFull1, subdirFull2)
		} else {
			// Find the backup times in the collection and try RESTORE'ing to each, and
			// within each also check if we can restore to individual times captured with
			// incremental backups that were appended to that backup.
			full1, full2 = findFullBackupPaths(tmpDir, path.Join(tmpDir, "*/*/*/BACKUP_MANIFEST"))
			runRestores(test.collections, full1, full2)
			...
			subdirFull1, subdirFull2 = findFullBackupPaths(path.Join(tmpDir, "foo"),
				path.Join(tmpDir, "foo", fmt.Sprintf("%s*", specifiedSubdir), "BACKUP_MANIFEST"))
			runRestores(test.collectionsWithSubdir, subdirFull1, subdirFull2)
		}
