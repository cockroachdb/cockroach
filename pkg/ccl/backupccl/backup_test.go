...
CREATE TABLE d2.plain (x int);
INSERT INTO d2.plain VALUES (1), (2);
`)
...
`)
		...
		sqlDB.Exec(t, `
CREATE TYPE d2.farewell AS ENUM ('another');
CREATE TABLE d2.t1 (x d2.farewell);
`)
		sqlDB.QueryRow(t, `SELECT cluster_logical_timestamp()`).Scan(&ts3)
		// Make a full backup that includes ts1, ts2 and ts3.
		sqlDB.Exec(t, `BACKUP DATABASE d2 TO 'nodelocal://0/rev-history-backup' WITH revision_history`)
...
`)
		...
		sqlDB.Exec(t, `
CREATE TYPE d2.farewell AS ENUM ('third');
CREATE TABLE d2.t1 (x d2.farewell);
`)
		...
		t.Run("ts1", func(t *testing.T) {
			...
			sqlDB.Exec(t,
				fmt.Sprintf(`
RESTORE DATABASE d2 FROM 'nodelocal://0/rev-history-backup' 
  AS OF SYSTEM TIME %s
`, ts1))
			sqlDB.ExpectErr(t, `pq: type "d2.public.farewell" already exists`,
				`CREATE TYPE d2.farewell AS ENUM ('bye', 'cya')`)
			sqlDB.Exec(t, `INSERT INTO d2.t1 VALUES ('bye'), ('cya');`)
			sqlDB.ExpectErr(t, `pq: invalid input value for enum farewell`, `INSERT INTO d2.t1 VALUES ('another');`)
			sqlDB.ExpectErr(t, `pq: invalid input value for enum farewell`, `INSERT INTO d2.t1 VALUES ('third');`)
		})
		...
		RESTORE DATABASE d2 FROM 'nodelocal://0/rev-history-backup'
		 AS OF SYSTEM TIME %s
		`, ts2))
			sqlDB.Exec(t, `CREATE TYPE d2.farewell AS ENUM ('bye', 'cya')`)
		})
		...
		`, ts3))
			sqlDB.ExpectErr(t, `pq: type "d2.public.farewell" already exists`,
				`CREATE TYPE d2.farewell AS ENUM ('bye', 'cya')`)
			sqlDB.ExpectErr(t, `pq: invalid input value for enum farewell`, `INSERT INTO d2.t1 VALUES ('bye'), ('cya'), ('third');`)
			sqlDB.Exec(t, `INSERT INTO d2.t1 VALUES ('another');`)
		})
		...
		`, ts5))
			sqlDB.ExpectErr(t, `pq: type "d2.public.farewell" already exists`,
				`CREATE TYPE d2.farewell AS ENUM ('bye', 'cya')`)
			sqlDB.ExpectErr(t, `pq: invalid input value for enum farewell`, `INSERT INTO d2.t1 VALUES ('bye'), ('cya'), ('another');`)
			sqlDB.Exec(t, `INSERT INTO d2.t1 VALUES ('third');`)
		})
	})
