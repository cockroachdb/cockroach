...
func TestEncryptedBackup(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			sqlDB.Exec(t, fmt.Sprintf(`SHOW BACKUP $1 WITH %s,encryption_dir='%s'`, encryptionOption, backupLoc1),
				backupLoc1inc)
			...
			sqlDB.ExpectErr(t, `NOTE: If you are running SHOW BACKUP exclusively on an incremental backup`,
