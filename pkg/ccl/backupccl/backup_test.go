...
func TestBackupRestoreTenant(t *testing.T) {
	...
	t.Run("restore-tenant10-to-latest", func(t *testing.T) {
		...
		restoreDB.CheckQueryResults(t, `select id, active, name, data_state, service_mode, crdb_internal.pb_to_json('cockroach.multitenant.TenantInfo', info) from system.tenants`, [][]string{
			{
				`1`, `true`, `system`,
		...
		restoreDB.CheckQueryResults(t,
			`SELECT id, active, name, data_state, service_mode, crdb_internal.pb_to_json('cockroach.multitenant.TenantInfo', info) FROM system.tenants`,
			[][]string{
				{
					`1`, `true`, `system`,
					strconv.Itoa(int(mtinfopb.DataStateReady)),
