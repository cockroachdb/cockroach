...
func TestBackupRestoreAppend(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{
			"userfile",
			makeCollections(`userfile:///1`, `userfile:///2`, `userfile:///3`),
			makeCollectionsWithSubdir(`userfile:///1`, `userfile:///2`, `userfile:///3`),
		},
	} {
...
func TestBackupRestoreControlJob(t *testing.T) {
	...
	t.Run("foreign", func(t *testing.T) {
		foreignDir := "nodelocal://1/foreign"
	...
	t.Run("pause", func(t *testing.T) {
		pauseDir := "nodelocal://1/pause"
		noOfflineDir := "nodelocal://1/no-offline"
...
func TestManifestTooNew(t *testing.T) {
	...
	defer cleanupFn()
	sqlDB.Exec(t, `CREATE DATABASE r1`)
	sqlDB.Exec(t, `BACKUP DATABASE r1 TO 'nodelocal://1/too_new'`)
	sqlDB.Exec(t, `DROP DATABASE r1`)
	// Prove we can restore.
	sqlDB.Exec(t, `RESTORE DATABASE r1 FROM 'nodelocal://1/too_new'`)
	sqlDB.Exec(t, `DROP DATABASE r1`)
	...
	// Verify we reject it.
	sqlDB.ExpectErr(t, "backup from version 2147483647.1 is newer than current version", `RESTORE DATABASE r1 FROM 'nodelocal://1/too_new'`)
