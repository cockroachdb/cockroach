...
func TestBackupRestoreGeodistributed(t *testing.T) {
	defer leaktest.AfterTest(t)()
	...
	backupURIs := []string{
		fmt.Sprintf("%s?LOCALITY=%s", localFoo1, url.QueryEscape("default")),
		fmt.Sprintf("%s?LOCALITY=%s", localFoo2, url.QueryEscape("dc=dc1")),
		fmt.Sprintf("%s?LOCALITY=%s", localFoo3, url.QueryEscape("dc=dc2")),
	}
	restoreURIs := []string{
		localFoo1,
		localFoo2,
...
func TestBackupRestoreGeodistributedMergeDirectories(t *testing.T) {
	defer leaktest.AfterTest(t)()
	...
	// TODO (lucy): This test writes a geodistributed backup where all files are
	...
	backupURIs := []string{
		fmt.Sprintf("%s?LOCALITY=%s", localFoo1, url.QueryEscape("default")),
		fmt.Sprintf("%s?LOCALITY=%s", localFoo1, url.QueryEscape("dc=dc1")),
		fmt.Sprintf("%s?LOCALITY=%s", localFoo1, url.QueryEscape("dc=dc2")),
	}
	restoreURIs := []string{
		localFoo1,
	}
...
func TestBackupRestoreGeodistributedIncremental(t *testing.T) {
	defer leaktest.AfterTest(t)()
	...
	var checksums []uint32
	{
		for backupNum := 0; backupNum < numBackups; backupNum++ {
			...
			sqlDB.Exec(
				t,
				fmt.Sprintf(`BACKUP TABLE data.bank TO ('%s?LOCALITY=%s', '%s?LOCALITY=%s') %s`,
