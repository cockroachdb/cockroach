...
// TestOnFailOrCancelExecCfgNPE is a regression test that ensures the execCfg on
// the restore resumer is set correctly at the beginning of the OnFailOrCancel
// hook. Previously, this test would result in a nil pointer exception when
// accessing the `r.execCfg`.
//
// TODO(adityamaru): Migrate to datadriven test once 78430 merges.
func TestOnFailOrCancelExecCfgNPE(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)
