...
CREATE TABLE d2.t1 (x d.greeting);
INSERT INTO d2.t1 VALUES ('hello'), ('howdy');
CREATE TABLE d2.t2 (x d.greeting[]);
INSERT INTO d2.t2 VALUES (ARRAY['howdy']), (ARRAY['hi']);
`)
		// TODO (rohany): I can't quite get full cluster backup restore to work,
		//  because types aren't dropped yet when the parent database is dropped,
		//  leaving some orphaned descriptors that restore won't restore over.
		//  I think in the meantime I'll just manually delete the surviving descriptors.
	})
...
`)
		...
		// Create a new database to backup the table into.
	...
	// TODO (rohany): Can I not use the same data directory as the first backup for a different backup?
	//  I get an error like "x is not a backup of y" when trying.
}
