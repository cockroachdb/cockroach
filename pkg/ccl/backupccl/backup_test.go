...
func TestBackupRestoreCheckpointing(t *testing.T) {
	...
	checkRestore := func(ctx context.Context, ip inProgressState) error {
		...
		low := keys.SystemTenantKeyGen.TablePrefix(ip.backupTableID)
		high := keys.SystemTenantKeyGen.TablePrefix(ip.backupTableID + 1)
...
func TestRestoreAsOfSystemTimeGCBounds(t *testing.T) {
	...
	gcr := roachpb.GCRequest{
		...
		RequestHeader: roachpb.RequestHeader{
			Key:    keys.SystemTenantKeyGen.TablePrefix(keys.MinUserDescID),
			EndKey: keys.MaxKey,
		},
