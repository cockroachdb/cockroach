...
// disjoint span. This implies the restore will process x requiredSpans and that
// the persisted checkpoint frontier will have x entries at completion. Since
// each required span has two rows, each required span will result in a single
// restoreSpanEntry, and consequently, a single AddSSTable flush.
//
...
func TestRestoreCheckpointing(t *testing.T) {
	...
	checkPersistedSpanLength := func(expectedLength int) {
		testutils.SucceedsSoon(t, func() error {
			jobProgress := jobutils.GetJobProgress(t, sqlDB, jobID)
			if len(jobProgress.GetRestore().Checkpoint) != expectedLength {
				return errors.New("expected checkpoints not persisted yet")
			}
			return nil
		})
	}
	...
	checkPersistedSpanLength(entriesBeforePause)
	...
	checkPersistedSpanLength(totalEntries)
