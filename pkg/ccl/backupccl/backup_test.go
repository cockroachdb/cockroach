...
func TestBackupEncryptedWithPassphrase(t *testing.T) {
	...
	sqlDB.ExpectErr(t, `file appears encrypted -- try specifying one of "encryption_passphrase" or "kms"`, `SHOW BACKUP $1`, backupLoc1)
...
func TestBackupEncryptedWithKMS(t *testing.T) {
	...
	sqlDB.Exec(t, fmt.Sprintf(`BACKUP TO ($1, $2) WITH kms='%s'`, uri), backupLoc1,
		backupLoc2)
	// Add the actual content with our sentinel too.
	sqlDB.Exec(t, `UPDATE neverappears.neverappears SET other = 'neverappears'`)
	sqlDB.Exec(t, fmt.Sprintf(`BACKUP TO ($1, 
$2) INCREMENTAL FROM $3 WITH kms='%s'`, uri), backupLoc1inc, backupLoc2inc, backupLoc1)
	...
	sqlDB.Exec(t, fmt.Sprintf(`SHOW BACKUP $1 WITH kms='%s'`, uri), backupLoc1)
	...
	sqlDB.ExpectErr(t, `one of the provided URIs was not used when encrypting the base BACKUP`,
		fmt.Sprintf(`SHOW BACKUP $1 WITH kms='%s'`, wrongKeyURI), backupLoc1)
	sqlDB.ExpectErr(t,
	...
	sqlDB.ExpectErr(t, `could not find or read encryption information`,
		fmt.Sprintf(`SHOW BACKUP $1 WITH kms='%s'`, uri), plainBackupLoc1)
...
func TestValidateKMSURIsAgainstFullBackup(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		masterKeyIDToDataKey := make(map[string][]byte)
		...
		for _, uri := range tc.fullBackupURIs {
			...
			masterKeyIDToDataKey[hashedMasterKeyID] = []byte("efgh-" + tc.name)
		...
		kmsInfo, err := validateKMSURIsAgainstFullBackup(tc.incrementalBackupURIs, masterKeyIDToDataKey,
...
func TestGetEncryptedDataKeyByKMSMasterKeyID(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		for _, uri := range tc.fullBackupURIs {
			...
			expectedMap[string(hashedMasterKeyID)] = encryptedDataKey
		}
