...
func benchmarkRestoreProcessorSize(b *testing.B, numRows int, numTables int) {
	...
	ctx, span := tracing.EnsureChildSpan(context.Background(), tr, "top of benchmark", tracing.WithForceRealSpan())
	span.SetVerbose(true)
	defer func() {
		span.Finish()
		json, err := span.GetRecording().ToJaegerJSON("RESTORE BENCHMARK")
