...
func canChainProtectedTimestampRecords(
	ctx context.Context, p sql.PlanHookState, eval *scheduledBackupEval,
) bool {
	...
	for _, t := range eval.Targets.Tables {
		...
		if _, ok := p.(*tree.AllTablesSelector); ok {
			return false
		}
	}
...
func doCreateBackupSchedules(
	...
	if incRecurrence != nil {
		chainProtectedTimestampRecords = canChainProtectedTimestampRecords(ctx, p, eval)
