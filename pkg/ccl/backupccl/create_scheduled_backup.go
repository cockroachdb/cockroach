...
const (
	...
	optUpdatesLastBackupMetric = "updates_last_backup_metric"
)
...
func doCreateBackupSchedules(
	...
	_, updateMetricOnSucess := scheduleOptions[optUpdatesLastBackupMetric]
	...
	if incRecurrence != nil {
		...
		inc, err := makeBackupSchedule(
			...
			incRecurrence, details, unpauseOnSuccessID, updateMetricOnSucess, backupNode)
	...
	full, err := makeBackupSchedule(
		env, p.User(), scheduleLabel,
		fullRecurrence, details, unpauseOnSuccessID, updateMetricOnSucess, backupNode)
	if err != nil {
		return err
	}
