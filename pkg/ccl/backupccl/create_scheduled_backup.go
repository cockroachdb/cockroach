...
// It is a plan hook implementation responsible for for the creating of scheduled backup.
...
const scheduleBackupOp = "SCHEDULED_BACKUP"
...
func (se *scheduledBackupExecutor) ExecuteJob(
	...
	if !backupStmt.Options.Detached {
		...
		log.Errorf(ctx, "force setting detached option for backup schedule %d", sj.ScheduleID())
	}
	...
	if err != nil {
		return errors.Wrapf(err, "backup evaluation error")
	}
	if planBackup == nil {
		return errors.Newf("backup evaluation error")
	}
	...
	g.GoCtx(func(ctx context.Context) error {
		if err := planBackup(ctx, nil, resultCh); err != nil {
			return errors.Wrap(err, "backup execution error")
		}
		return nil
	})
...
func createBackupScheduleHook(
	...
	}, scheduledBackupHeader, nil, false, nil
}
