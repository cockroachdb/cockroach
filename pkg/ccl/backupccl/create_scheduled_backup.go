...
func doCreateBackupSchedules(
	...
	if incRecurrence != nil {
		...
		if err := inc.Create(ctx, ex, p.ExtendedEvalContext().TxnToDelete); err != nil {
			return err
		}
	...
	if err := full.Create(ctx, ex, p.ExtendedEvalContext().TxnToDelete); err != nil {
		return err
	}
	...
	if incRecurrence != nil {
		if err := setDependentSchedule(ctx, ex, fullScheduledBackupArgs, full, inc.ScheduleID(),
			p.ExtendedEvalContext().TxnToDelete); err != nil {
...
func checkScheduleAlreadyExists(
	...
	row, err := p.ExecCfg().InternalExecutor.QueryRowEx(ctx, "check-sched",
		p.ExtendedEvalContext().TxnToDelete, sessiondata.InternalExecutorOverride{User: security.RootUserName()},
...
func dryRunBackup(ctx context.Context, p sql.PlanHookState, backupNode *tree.Backup) error {
	sp, err := p.ExtendedEvalContext().TxnToDelete.CreateSavepoint(ctx)
	if err != nil {
		return err
	}
	err = dryRunInvokeBackup(ctx, p, backupNode)
	if rollbackErr := p.ExtendedEvalContext().TxnToDelete.RollbackToSavepoint(ctx, sp); rollbackErr != nil {
		return rollbackErr
	}
	return err
}
