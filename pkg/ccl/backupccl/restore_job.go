...
func restore(
	...
	runRestore := func(ctx context.Context) error {
		...
		if details.ExperimentalOnline {
			...
			//  During the link phase of online restore, we do not update stats progress as job occurs. We merely reuse the
			// `progressTracker.mu.res` var to reduce the number of local vars floating around in `restore`.
