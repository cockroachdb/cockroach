...
func (r *restoreResumer) restoreSystemTables(
	...
	for _, table := range tables {
		...
		if err := db.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
			...
			// the restore.
			if systemTableName != systemschema.JobsTable.Name {
			...
			restoreQuery := fmt.Sprintf("INSERT INTO system.%s (SELECT * FROM %s.%s);", systemTableName, restoreTempSystemDB, systemTableName)
