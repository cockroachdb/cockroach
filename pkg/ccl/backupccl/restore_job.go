...
func makeBackupLocalityMap(
	backupLocalityInfos []jobspb.RestoreDetails_BackupLocalityInfo,
...
func restore(
	...
	backupLocalityMap, err := makeBackupLocalityMap(backupLocalityInfo)
	if err != nil {
		return mu.res, errors.Wrap(err, "resolving locality locations")
	}
...
func loadBackupSQLDescs(
	...
	// an upgrade?).
	if err := maybeUpgradeTableDescsInSlice(ctx, allDescs, true /* skipFKsWithNoMatchingTable */); err != nil {
		return nil, BackupManifest{}, nil, err
	}
...
func (r *restoreResumer) restoreSystemTables(
	...
	for _, table := range tables {
		...
		if err := db.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
			details := r.job.WithTxn(txn).Details().(jobspb.RestoreDetails)
			...
			} else if systemTableName != sqlbase.JobsTable.Name {
