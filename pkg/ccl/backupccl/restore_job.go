	...
	if !details.PrepareCompleted {
		err := p.ExecCfg().DB.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
			...
			for _, tenant := range details.Tenants {
				...
				if err := sql.CreateTenantRecord(ctx, txn, p.ExecCfg(), tenant.ID, inactive, tenant.Info); err != nil {
					return err
				}
			}
...
func (r *restoreResumer) Resume(
	...
	for _, tenant := range details.Tenants {
		prefix := keys.MakeSQLCodec(roachpb.MakeTenantID(tenant.ID)).TenantPrefix()
