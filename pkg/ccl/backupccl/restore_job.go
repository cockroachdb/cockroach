...
func checkUsersInRestoringCluster(
	...
	it, err := p.ExecCfg().InternalExecutor.QueryIteratorEx(
		ctx, "query-system-users", txn,
		sessiondata.InternalExecutorOverride{User: p.User()},
		`SELECT username FROM system.users WHERE "isRole" = false`)
	if err != nil {
		return err
	}
	...
	for ok, err = it.Next(ctx); ok; ok, err = it.Next(ctx) {
		...
		row := it.Cur()
		sqlUser := security.MakeSQLUsernameFromPreNormalizedString(string(tree.MustBeDString(row[0])))
		if exists := users[sqlUser]; exists {
			delete(users, sqlUser)
		}
		if len(users) == 0 {
			break
		}
	...
	for u := range users {
		return errors.Errorf("Failed to keep privilege grants for non-existent user %s in the restoring cluster", u.Normalized())
	}
	return nil
}
...
func createImportingDescriptors(
	...
	preserveGrantsForUsersMutable := make(map[security.SQLUsername]bool)
	...
	if details.PreserveGrantsFor == "*" {
		...
		for _, user := range strings.Split(details.PreserveGrantsFor, ",") {
			if user != "" {
				...
				preserveGrantsForUsersMutable[sqlUser] = true
			}
		}
	}
	...
	if !details.PrepareCompleted {
		err := sql.DescsTxn(ctx, p.ExecCfg(), func(
			...
			err := checkUsersInRestoringCluster(ctx, txn, p, preserveGrantsForUsersMutable)
			if err != nil {
				return err
			}
...
func getRestoringPrivileges(
	...
	case catalog.TypeDescriptor, catalog.DatabaseDescriptor:
		if descCoverage == tree.RequestedDescriptors {
			// Beside keeping the default privileges, we preserve grants for specified users as
