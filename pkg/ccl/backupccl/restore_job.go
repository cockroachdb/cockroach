...
func validateRestoredDescriptors(ctx context.Context, execConfig *sql.ExecutorConfig) error {
	...
	for _, desc := range allDescs {
		...
		// database. Thus, it seems easiest to include all descriptors in the
		// cluster in the check.
	...
	if !ok {
		return errors.Newf("descriptor validation check failed: %s", invalidMsg)
	}
	return nil
}
