...
// TODO (lucy): We should refactor the usages of this and isDatabaseEmpty to
// avoid the repeated lookups of all descriptors.
func isSchemaEmpty(
	ctx context.Context,
	txn *kv.Txn,
	...
	for _, desc := range allDescs {
		...
		if desc.GetParentSchemaID() == schemaDesc.GetID() {
			return false, nil
		}
	}
	return true, nil
...
func (r *restoreResumer) dropDescriptors(
	...
	for _, schemaDesc := range details.SchemaDescs {
		...
		isSchemaEmpty, err := isSchemaEmpty(ctx, txn, sc, ignoredChildDescIDs)
		if err != nil {
