...
func (rd *restoreDataProcessor) openSSTs(
	...
	sendIter := func(iter storage.SimpleMVCCIterator, dirsToSend []cloud.ExternalStorage, mSSTCh chan mergedSST, iterMem int64, lastInSpan bool) error {
		...
		cleanup := func() {
			...
			rd.mu.Lock()
			if rd.mu.memWaitCh != nil {
				close(rd.mu.memWaitCh)
				rd.mu.memWaitCh = nil
			}
			rd.mu.Unlock()
	...
	for idx := 0; idx < len(entry.Files); {
		...
		if err := rd.mem.Grow(ctx, sstOverheadBytesPerFile); err != nil {
			...
			if rd.mu.memWaitCh != nil {
				rd.mu.Unlock()
				return errors.AssertionFailedf("there should only be 1 instance of openSSTs per restore data processor")
			}
			rd.mu.memWaitCh = waitCh
			rd.mu.Unlock()
...
func makeProgressUpdate(
	...
	progDetails.Incomplete = !lastInSpan
	return
}
