...
// sstReaderMemBytesPerFile and sstReaderEncryptedMemBytesPerFile were obtained
// benchmarking external SST iterators on GCP and AWS and selecting the highest
// observed memory per file.
const sstReaderMemBytesPerFile = 5 << 20
const sstReaderEncryptedMemBytesPerFile = 8 << 20
...
func (rd *restoreDataProcessor) openSSTs(
	...
	if rd.spec.Encryption != nil {
		bytesPerFile = sstReaderEncryptedMemBytesPerFile
	} else {
		bytesPerFile = sstReaderMemBytesPerFile
	}
	...
	for idx := 0; idx < len(entry.Files); {
		...
		if err := rd.mem.Grow(ctx, bytesPerFile); err != nil {
			if len(storeFiles) > 0 {
				...
				if err := sendIter(iter, dirs, workerSSTChannels[workerNum], int64(len(storeFiles))*sstReaderMemBytesPerFile, false); err != nil {
					return err
				}
				storeFiles = make([]storageccl.StoreFile, 0, len(entry.Files)-idx)
			}

			time.Sleep(sstReaderMemWaitSec * time.Second)
			continue
		}
	...
	return sendIter(iter, dirs, sstCh, int64(len(storeFiles))*sstReaderMemBytesPerFile, true)
}
