	...
	if err := rd.Init(ctx, rd, post, restoreDataOutputTypes, flowCtx, processorID, nil, /* memMonitor */
		execinfra.ProcStateOpts{
			...
			TrailingMetaCallback: func() []execinfrapb.ProducerMetadata {
				...
				if testingKnobs, ok := flowCtx.TestingKnobs().BackupRestoreTestingKnobs.(*sql.BackupRestoreTestingKnobs); ok {
					if testingKnobs != nil && testingKnobs.RunBeforeRestoreProcDrains != nil {
						testingKnobs.RunBeforeRestoreProcDrains()
					}
				}
