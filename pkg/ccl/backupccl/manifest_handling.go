...
func VerifyUsableExportTarget(
	...
	if r, err := exportStore.ReadFile(ctx, BackupManifestName); err == nil {
		...
		return errors.Wrap(err, fmt.Sprintf("%s returned an unexpected error when checking for the existence of %s file", readable, BackupManifestName))
	}
	...
	if r, err := exportStore.ReadFile(ctx, BackupManifestCheckpointName); err == nil {
		...
		return errors.Wrap(err, fmt.Sprintf("%s returned an unexpected error when checking for the existence of %s file", readable, BackupManifestCheckpointName))
	}
