...
// readEncryptionOptions takes in a backup location and tries to find and return all encryption option files in the
// backup. A backup normally only creates one encryption option file, but if the user uses ALTER BACKUP to add new
// keys, a new encryption option file will be placed side by side with the old one. Since the old file is still valid,
// as we never want to modify or delete an existing backup, we return both new and old files.
func readEncryptionOptions(
	...
	// The user is more likely to pass in a KMS URI that was used to encrypt the backup recently,
	// so we iterate the ENCRYPTION-INFO files from latest to oldest.
