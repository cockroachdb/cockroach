...
const (
	...
	latestHistoryDirectory = "latest-history/LATEST"
)
...
func readBackupManifest(
	...
	r, err := readLatestCheckpointFile(ctx, exportStore, filename)
	if err != nil {
		return BackupManifest{}, 0, err
	}
	defer r.Close(ctx)
...
func checkForLatestFileInCollection(
	...
	_, err := store.ReadFile(ctx, latestHistoryDirectory)
	if err != nil {
		if !errors.Is(err, cloud.ErrFileDoesNotExist) {
...
// readLatestCheckpointFile returns an io.reader of the latest checkpoint
// file.
