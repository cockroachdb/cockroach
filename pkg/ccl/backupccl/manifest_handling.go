...
func readLatestCheckpointFile(
	...
	if exportStore.Conf().Provider == roachpb.ExternalStorageProvider_http {
		...
		// If the read was successful, return the ioctx.ReaderCloserCtx, as
		// a BACKUP-CHECKPOINT was successfully found.
		if err == nil {
			return r, nil
		}
	} else {
		...
		// List doesn't return error if it didn't find anything, so we know
		// something else went wrong, we can just return the error.
		if err != nil {
			return nil, err
		}
	}
...
// writeNewCheckpointFile writes a new checkpoint (or checksum) file
// to the appropriate directory depending on the cluster version.
func writeNewCheckpointFile(
	...
	// HTTP storage does not support listing, and so we continue overwriting
	// a non-timestamp versioned file in the progress directory.
	if exportStore.Conf().Provider == roachpb.ExternalStorageProvider_http {
