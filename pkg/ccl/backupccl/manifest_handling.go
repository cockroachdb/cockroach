...
// it into an immutable implementation of ExtendedDescriptorInterface. It ensures that
// the ModificationTime is set properly.
func unwrapDescriptor(ctx context.Context, desc *descpb.Descriptor) catalog.Descriptor {
...
func loadSQLDescsFromBackupsAtTime(
...
) ([]sqlbase.DescriptorInterface, BackupManifest) {
	lastBackupManifest := backupManifests[len(backupManifests)-1]

	unwrapDescriptors := func(raw []descpb.Descriptor) []sqlbase.DescriptorInterface {
		ret := make([]sqlbase.DescriptorInterface, 0, len(raw))
		for i := range raw {
	...
	allDescs := make([]sqlbase.DescriptorInterface, 0, len(byID))
	for _, raw := range byID {
		// A revisions may have been captured before it was in a DB that is
		// backed up -- if the DB is missing, filter the table.
		desc := unwrapDescriptor(context.TODO(), raw)
		if byID[desc.GetParentID()] == nil {
			continue
		}
