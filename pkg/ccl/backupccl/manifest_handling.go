...
func containsManifest(ctx context.Context, exportStore cloud.ExternalStorage) (bool, error) {
	...
	if err != nil {
		return false, nil /* TOODO(dt): only silence non-exists errors */
	}
	r.Close()
	return true, nil
}
...
func resolveBackupManifests(
	...
	if len(from) > 1 {
		...
		for i, uris := range from {
			...
			if len(uris) > 1 {
				localityInfo[i], err = getLocalityInfo(ctx, stores, uris, mainBackupManifests[i], encryption, "")
				if err != nil {
					return nil, nil, nil, err
				}
			}
		...
		localityInfo[0], err = getLocalityInfo(ctx, baseStores, from[0], baseManifest, encryption, "")
		if err != nil {
			return nil, nil, nil, err
		}
