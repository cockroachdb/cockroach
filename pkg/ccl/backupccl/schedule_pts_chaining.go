...
func maybeUpdateSchedulePTSRecord(
	...
	return exec.DB.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
		...
		if args.BackupType == ScheduledBackupExecutionArgs_INCREMENTAL {
			err := manageIncrementalBackupPTSChaining(ctx, backupDetails.UpdateOnSuccessPTSRecord,
...
func manageFullBackupPTSChaining(
	...
	if err := releaseProtectedTimestamp(ctx, txn,
		exec.ProtectedTimestampProvider, backupDetails.ReleaseOnSuccessPTSRecord); err != nil {
	...
	incSj.SetExecutionDetails(incSj.ExecutorType(), *incSj.ExecutionArgs())
