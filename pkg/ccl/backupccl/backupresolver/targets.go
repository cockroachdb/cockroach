...
func GetQualifiedTablePatterns(
	...
	var currDBDesc catalog.Descriptor
	...
	for _, pattern := range targetTables {
		...
		case *tree.AllTablesSelector:
			if !tp.ExplicitCatalog && !tp.ExplicitSchema {
				tp.ExplicitSchema = true
				tp.SchemaName = tree.Name(p.CurrentDatabase())
			} else if !tp.ExplicitCatalog && tp.ExplicitSchema {
				// Continue to the next iteration if currentDB is not one of the
				// complete DB targets.
				if currDBDesc == nil {
				...
				resolvedSchema, _, err := catalogkv.ResolveSchemaID(ctx, txn, execCfg.Codec, currDBDesc.GetID(), tp.SchemaName.String())
				if err != nil {
					return nil, err
				}
