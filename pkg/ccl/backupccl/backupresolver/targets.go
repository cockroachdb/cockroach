...
func DescriptorsMatchingTargets(
	...
	for _, pattern := range targets.Tables {
		...
		case *tree.AllTablesSelector:
			...
			// If the table pattern does not have explicit db AND explicit schema,
			// try looking up the specified schema name in the current database.
			// All tables in the specified schema should be backed up if the schema
			// name can be resolved under the current DB, in particular, no tables
			// outside of the specified schema should be backed up.
			if !hasSchemaScope {
				hasSchemaScope, _, err = r.LookupSchema(ctx, currentDatabase, scName)
				if err != nil {
					return ret, err
				}
			}
			...
			// (db.schema.*) -> p.CatalogName would be non-empty before resolving the
			// prefix, OR
