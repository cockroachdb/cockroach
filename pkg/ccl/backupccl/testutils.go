...
// span.
func waitForTableSplit(t *testing.T, sqlRunner *sqlutils.SQLRunner, tableName, dbName string) {
	t.Helper()
	testutils.SucceedsSoon(t, func() error {
		count := 0
...
func getTableStartKey(
	t *testing.T, sqlRunner *sqlutils.SQLRunner, tableName, dbName string,
) roachpb.Key {
	t.Helper()
	...
	sqlRunner.QueryRow(t,
		"SELECT start_key "+
			...
			"LIMIT 1", tableName, dbName).Scan(&startKey)
	return startKey
}
...
func getStoreAndReplica(
	...
	t.Helper()
	startKey := getTableStartKey(t, sqlRunner, tableName, dbName)
...
func waitForReplicaFieldToBeSet(
	...
	t.Helper()
	testutils.SucceedsSoon(t, func() error {
		_, r := getStoreAndReplica(t, tc, sqlRunner, tableName, dbName)
