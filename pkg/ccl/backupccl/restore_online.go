		...
		for entry := range restoreSpanEntriesCh {
			...
			for _, file := range entry.Files {
				...
				if batchSize+file.BackupFileEntryCounts.DataSize > targetBatchSize {
					log.Infof(ctx, "flushing %s batch of %d SSTs due to size limit up to %s in in span (old keyspace) %s", sz(batchSize), len(toAdd), file.BackupFileEntrySpan.Key, entry.Span)
