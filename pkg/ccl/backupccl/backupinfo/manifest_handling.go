...
func WriteFilesListMetadataWithSSTs(
	...
	return errors.Wrap(writeFilesListMetadata(ctx, exportStore, backupbase.BackupMetadataName, encryption,
...
func LoadSQLDescsFromBackupsAtTime(
	...
	if asOf.IsEmpty() {
		if lastBackupManifest.DescriptorCoverage != tree.AllDescriptors {
			descs, err := BackupManifestDescriptors(ctx, &lastBackupManifest, lastIterFactory)
...
func GetBackupManifestIterFactories(
	...
	for layer := range backupManifests {
		...
		f, err := NewIterFactory(&backupManifests[layer], es, encryption, kmsEnv)
		if err != nil {
			return nil, err
		}
