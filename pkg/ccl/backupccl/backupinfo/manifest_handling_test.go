...
func TestManifestHandlingIteratorOperations(t *testing.T) {
	...
	require.NoError(t, WriteMetadataWithExternalSSTs(ctx, store, nil, nil, &m))

	iterFactory := NewIterFactory(&m, store, nil, nil)

	fileLess := func(left backuppb.BackupManifest_File, right backuppb.BackupManifest_File) bool {
		return FileCmp(left, right) < 0
	}
	...
	descRevsLess := func(
		...
		return descChangesLess(&left, &right)
	}
...
func mustCreateFileIterFactory(
	t *testing.T, iterFactory *IterFactory,
