...
func runBackupProcessor(
	...
	spanMaker := initRequestSpanMaker(rangeSizedSpans, spec, flowCtx.Cfg.ExecutorConfig.(*sql.ExecutorConfig).RangeDescIteratorFactory)

	if err := spanMaker.createRequestSpans(ctx); err != nil {
		return err
	}
	...
	if len(spec.URIsByLocalityKV) > 0 {
		...
		if localitySinkURI != "" {
			log.Infof(ctx, "backing up %d spans to destination specified by locality %s", spanMaker.spanCountFromSpecs(), destLocalityKV)
			...
			log.Infof(ctx, "backing up %d spans to default locality because backup localities %s have no match in node's localities %s", spanMaker.spanCountFromSpecs(), backupLocalities, nodeLocalities)
		}
	}
	...
	todo := spanMaker.queueWork(numSenders)
