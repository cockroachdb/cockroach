...
func runBackupProcessor(
	...
	grp.GoCtx(func(ctx context.Context) error {
		...
		csvDestURI := fmt.Sprintf("nodelocal://1/backuplog-%s-%s-%s.csv", time.Now().Format(time.RFC3339),
			flowCtx.NodeID.SQLInstanceID().String(), flowCtx.ID.String())
		csvConf, err := cloud.ExternalStorageConfFromURI(csvDestURI, spec.User())
		if err != nil {
			return err
		}
		csvDest, err := flowCtx.Cfg.ExternalStorage(ctx, csvConf)
		if err != nil {
			return err
		}
		csvWriter, err := csvDest.Writer(ctx, "")
		if err != nil {
			return err
		}

		sink := &sstSink{conf: sinkConf, dest: storage, csvWriter: csv.NewWriter(csvWriter)}
		...
		defer func() {
			...
			err = csvWriter.Close()
			if err != nil {
				log.Warningf(ctx, "failed to close csv write: %+v", err)
			}
		}()
