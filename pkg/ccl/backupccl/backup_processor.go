		...
		for {
			...
			case span := <-todo:
				...
				if priority {
					...
					// time, we set FailOnIntents so that the export will return a retry
					// error to us, instead of instead doing blocking wait on the intent.
					// This lets us move on to other ranges we have to export, provide an
					// indication of why we're blocked, etc.
