...
func (s *fileSSTSink) write(ctx context.Context, resp exportedSpan) (roachpb.Key, error) {
	...
	// must occur if the previous span ended mid row, or if the new span is a
	...
	if extend {
		last.Span.EndKey = span.EndKey
		last.EntryCounts.Add(resp.metadata.EntryCounts)
		last.ApproximatePhysicalSize += resp.metadata.ApproximatePhysicalSize
		s.stats.spanGrows++
	} else {
		f := resp.metadata
		f.Path = s.outName
