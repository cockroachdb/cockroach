...
func (s *fileSSTSink) push(ctx context.Context, resp returnedSpans) error {
	s.queue = append(s.queue, resp)
	s.queueSize += len(resp.spanDataAsSST)

	if s.queueSize >= int(s.queueCap) {
		sort.Slice(s.queue, func(i, j int) bool {
			return s.queue[i].partialFile.Span.Key.Compare(s.queue[j].partialFile.Span.Key) < 0
		})
		...
		for i := range s.queue[:drain] {
			...
			s.queueSize -= len(s.queue[i].spanDataAsSST)
		}
...
func (s *fileSSTSink) write(ctx context.Context, resp returnedSpans) error {
	s.stats.files++

	span := resp.partialFile.Span
	...
	// Copy SST content.
	sst, err := storage.NewMemSSTIterator(resp.spanDataAsSST, false)
	if err != nil {
		return err
	}
	defer sst.Close()
	...
	if l := len(s.flushedFiles) - 1; l > 0 && s.flushedFiles[l].Span.EndKey.Equal(span.Key) &&
		s.flushedFiles[l].EndTime.EqOrdering(resp.partialFile.EndTime) &&
		s.flushedFiles[l].StartTime.EqOrdering(resp.partialFile.StartTime) {
		s.flushedFiles[l].Span.EndKey = span.EndKey
		s.flushedFiles[l].EntryCounts.Add(resp.partialFile.EntryCounts)
		s.stats.spanGrows++
	} else {
		f := resp.partialFile
		f.Path = s.outName
	...
	s.flushedSize += int64(len(resp.spanDataAsSST))
