...
func TestOnlineRestoreBasic(t *testing.T) {
	...
	externalBytesBelow, err := jobutils.GetExternalBytesOnNode(ctx, rtc.Conns[0])
	require.NoError(t, err)
...
func TestOnlineRestoreWaitForDownload(t *testing.T) {
	...
	externalBytes, err := jobutils.GetExternalBytesOnNode(ctx, tc.Conns[0])
	require.NoError(t, err)
...
func TestOnlineRestoreTenant(t *testing.T) {
	...
	externalBytes, err := jobutils.GetExternalBytesOnNode(ctx, cBelow)
	require.NoError(t, err)
	require.Zero(t, externalBytes)

	externalBytes, err = jobutils.GetExternalBytesOnNode(ctx, cAbove)
	require.NoError(t, err)
	require.Zero(t, externalBytes)
}
