...
func TestAlterBackupStatement(t *testing.T) {
	...
	r, err := store.ReadFile(ctx, "ENCRYPTION-INFO-2")
	r.Close()
	require.NoError(t, err)
	...
	r, err = store.ReadFile(ctx, "ENCRYPTION-INFO-3")
	r.Close()
	require.NoError(t, err)
}
...
func TestAlterBackupRestore(t *testing.T) {
	...
	// Tests to see that show backup correctly recognizes the new encryption-info file
	// when SHOW BACKUP is called on an encrypted backup that was altered.
	t.Run(fmt.Sprintf("alter-backup-show-backup"), func(t *testing.T) {
	...
	t.Run(fmt.Sprintf("alter-backup-restore-multiple-new-kms"), func(t *testing.T) {
	...
	// Test to see that using an invalid OLD_KMS still works as long as one is valid.
	t.Run(fmt.Sprintf("alter-backup-restore-invalid-old-kms"), func(t *testing.T) {
		newURIA := getAWSEncryptionOption(t, "NEW_AWS_KMS_REGION_A", "NEW_AWS_KEY_ID_A")
	...
	t.Run(fmt.Sprintf("alter-backup-restore-old-kms-is-new-kms"), func(t *testing.T) {
