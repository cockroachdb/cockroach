...
func TestAlterBackupStatement(t *testing.T) {
	...
	numFiles, err := getNumEncryptionInfoFiles(ctx, store)
	require.NoError(t, err)
	require.True(t, numFiles == 2)

	query = fmt.Sprintf("ALTER BACKUP LATEST in %s ADD NEW_KMS=%s WITH OLD_KMS=%s", userfile, key1, key2)
	sqlDB.Exec(t, query)
	numFiles, err = getNumEncryptionInfoFiles(ctx, store)
	require.NoError(t, err)
	require.True(t, numFiles == 3)
}
...
func TestAlterBackupRestore(t *testing.T) {
	...
	t.Run(fmt.Sprintf("alter-backup-restore-with-%s", oldURI), func(t *testing.T) {
		...
		error := " one of the provided URIs was not used when encrypting the base BACKUP: " +
			"could not find an entry in the encryptedDataKeyMap"
		sqlDB.ExpectErr(t, error, query)
	})
}
