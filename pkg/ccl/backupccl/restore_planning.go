...
func moveConflictingSystemTables(
	...
	// backup.
	if err := bumpDescIDSeqKey(ctx, p, databasesByID, schemasByID, tablesByID, typesByID); err != nil {
		return err
	}
...
//
// We do the move by creating new temporary tables in the system database.
// Data from the conflicting tables is moved to the temporary tables. The
// namespace and descriptor entry for the conflicting table is removed, and
// then the temporary table is renamed.
//
// TODO(ssd): We currently do all of the descriptor modification "bare handed"
// by writing to KV directly. Should we be using WriteDescToBatch and friends
// instead? I avoided it for now because I was having some trouble with
// validation when using it.
func moveConflictingSystemTable(
	...
	return sql.DescsTxn(ctx, p.ExecCfg(), func(ctx context.Context, txn *kv.Txn, col *descs.Collection) error {
		b := txn.NewBatch()
		// Create a new table with the same schema as the table we want to
		// move.
		//
		// TODO(ssd): Think about whether we need to do cleanup on failure here. I
		// _think_ that since we do this all in a single transaction, we should be
		// OK.
		newTable := oldTable.NewBuilder().BuildCreatedMutable().(*tabledesc.Mutable)
		newTable.ID = descpb.InvalidID
		newTable.Name = fmt.Sprintf("crdb_internal_copy_%s", newTable.Name)
		id, created, err := upgrades.CreateSystemTableInTxn(ctx, p.ExecCfg().DB, txn, codec, newTable)
		if err != nil {
			return err
		}
		if !created {
		...
		// Copy data from the existing table over to the temporary table.
		insertStmt := fmt.Sprintf("INSERT INTO system.%s (SELECT * FROM system.%s)", newTable.Name, newTable.GetName())
		if _, err := p.ExecCfg().InternalExecutor.Exec(ctx, "copy-to-temp-table", txn, insertStmt); err != nil {
			return err
		}

		// Delete the old namespace entry for the new table.
		b.Del(catalogkeys.EncodeNameKey(codec, newTable))

		// Delete the descriptor entry for the old system table
		b.Del(catalogkeys.MakeDescMetadataKey(codec, oldTable.GetID()))
		col.AddDeletedDescriptor(oldTable.GetID())

		// Overwrite the namespace entry for the system table with our new id, and update the
		// descriptor of the new table with the old table's name.
		newTable.ID = id
		newTable.Name = oldTable.GetName()
		b.Put(catalogkeys.EncodeNameKey(codec, newTable), newTable.ID)
		b.Put(catalogkeys.MakeDescMetadataKey(codec, newTable.GetID()), newTable.DescriptorProto())

		if err := txn.Run(ctx, b); err != nil {
			return err
		}

		// ClearRange on the table span for the old table that we've just migrated.
		...
		if err := p.ExecCfg().DB.Run(ctx, &clearBatch); err != nil {
			return errors.Wrapf(err, "ClearRange %s", spanToClear)
		}
		return nil
	})
}
