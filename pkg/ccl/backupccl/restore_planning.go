	...
	if err := p.ExecCfg().DB.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
		...
		for _, table := range tablesByID {
			...
			if _, ok := restoreDBNames[targetDB]; ok {
				...
				err := catalogkv.CheckObjectCollision(ctx, txn, p.ExecCfg().Codec, parentID, table.GetParentSchemaID(), table.Name)
				if err != nil {
					return err
				}
		...
		for _, typ := range typesByID {
			...
			if _, ok := restoreDBNames[targetDB]; ok {
				...
				if desc == nil {
					...
					err := catalogkv.CheckObjectCollision(ctx, txn, p.ExecCfg().Codec, parentID, typ.GetParentSchemaID(), arrTyp.Name)
					if err != nil {
