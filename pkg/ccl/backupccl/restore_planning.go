...
func allocateDescriptorRewrites(
	...
	descriptorsToRemap := make([]sqlbase.DescriptorInterface, 0, len(tablesByID))
	for _, table := range tablesByID {
		if descriptorCoverage == tree.AllDescriptors {
			if table.ParentID == sqlbase.SystemDB.GetID() {
...
func maybeUpgradeTableDescsInBackupManifests(
	...
	for i := range backupManifests {
		...
		for j := range backupManifest.Descriptors {
			if table := sqlbase.TableFromDescriptor(&backupManifest.Descriptors[j], hlc.Timestamp{}); table != nil {
				_, err := sqlbase.MaybeUpgradeForeignKeyRepresentation(ctx, protoGetter, codec, skipFKsWithNoMatchingTable, table)
				if err != nil {
					return err
				}
				// TODO(lucy): Is this necessary?
				backupManifest.Descriptors[j] =
					*sqlbase.NewMutableExistingTableDescriptor(*table).DescriptorProto()
			}
		}
	}
	return nil
