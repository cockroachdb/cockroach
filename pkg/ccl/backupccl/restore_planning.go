...
func allocateDescriptorRewrites(
	...
	// At this point, descriptorRewrites contains an entry for every database,
	// schema, table and type that needs to be restored. As a final step we
	// populate the parent schema ID of all tables and types. This is easiest to
	// do once the descriptorRewrites map contains a rewrite for every schema that
	// is being restored.
	for _, table := range tablesByID {
		var newSchemaTableID descpb.ID
		curTableSchemaID := table.GetParentSchemaID()
		if curTableSchemaID == keys.PublicSchemaID && !table.IsTemporary() {
			newSchemaTableID = curTableSchemaID
		}
		if rw, ok := descriptorRewrites[curTableSchemaID]; ok {
			newSchemaTableID = rw.ID
		} else {
			newSchemaTableID = curTableSchemaID
		}
		descriptorRewrites[table.ID].ParentSchemaID = newSchemaTableID
	}
	for _, typ := range typesByID {
		var newSchemaTypeID descpb.ID
		curTypeSchemaID := typ.GetParentSchemaID()
		if curTypeSchemaID == keys.PublicSchemaID {
			newSchemaTypeID = curTypeSchemaID
		}
		if rw, ok := descriptorRewrites[curTypeSchemaID]; ok {
			newSchemaTypeID = rw.ID
		} else {
			newSchemaTypeID = curTypeSchemaID
		}
		descriptorRewrites[typ.ID].ParentSchemaID = newSchemaTypeID
	}
