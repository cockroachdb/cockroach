...
func moveConflictingSystemTable(
	...
	return sql.DescsTxn(ctx, p.ExecCfg(), func(ctx context.Context, txn *kv.Txn, descsCol *descs.Collection) error {
		b := txn.NewBatch()
		...
		if err := descsCol.WriteDescToBatch(ctx, false /* kvTrace */, newTable, b); err != nil {
			return err
		}
