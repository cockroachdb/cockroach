...
func maybeUpgradeDescriptorsInBackupManifests(
	...
	// because they would've been generated by at least version 22.2.
	if backupManifests[0].HasExternalFilesList {
		return nil
	}

	// TODO: figure out if current descriptors need to be upgraded?
	descriptors := make([]catalog.Descriptor, 0, len(backupManifests[0].Descriptors))
	for i := range backupManifests {
		descs, err := backupinfo.BackupManifestDescriptors(ctx, &backupManifests[i], layerToIterFactory[i])
		if err != nil {
			return err
		}
