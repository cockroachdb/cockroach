...
func allocateDescriptorRewrites(
	...
	if descriptorCoverage == tree.AllDescriptors {
		...
		// add it to the namespace table. We then remap the temporary object
		// descriptors to point to this schema.
		...
		for _, table := range tablesByID {
			if table.IsTemporary() {
				...
				//descriptorRewrites[table.GetID()] = &jobspb.RestoreDetails_DescriptorRewrite{ParentID: table.GetParentID()}
			}
		}
	}
...
// restore.
func dropDefaultUserDBs(
	ctx context.Context, descriptorCoverage tree.DescriptorCoverage, execCfg *sql.ExecutorConfig,
) error {
...
func doRestorePlan(
	...
	if err := dropDefaultUserDBs(ctx, restoreStmt.DescriptorCoverage, p.ExecCfg()); err != nil {
		return err
	}
