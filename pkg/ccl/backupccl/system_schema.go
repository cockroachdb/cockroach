...
func jobsRestoreFunc(
	...
	err := func() (err error) {
		...
		// given queryBuilder.
		makeJobsList := func(jobIDs []int64, queryBuilder *strings.Builder) {
		...
		// Note: This will keep the payload of all non-terminal jobs in memory.
		jobPayloads := make(map[int64]*jobspb.Payload)
		...
		for {
			...
			if payload.Type() == jobspb.TypeSchemaChange {
				...
				jobPayloads[jobID] = payload
			} else {
				jobsToRevert = append(jobsToRevert, jobID)
			}
		}

		for job, payload := range jobPayloads {
			payloadBytes, err := protoutil.Marshal(payload)
			if err != nil {
				return err
			}
			q := fmt.Sprintf("UPDATE %s SET payload = $1 WHERE id = $2", tempTableName)
			if _, err := executor.Exec(ctx, "updating-job-progress", txn, q, payloadBytes, job); err != nil {
				return errors.Wrapf(err, "inserting data to system.%s", systemTableName)
			}
		}
		...
		makeJobsList(jobsToRevert, &updateStatusQuery)
