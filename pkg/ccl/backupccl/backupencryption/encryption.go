...
// GetEncryptedDataKey returns the encrypted data key corresponding to the
// passed in masterKeyID from the EncryptedDataKeyMap.
...
func WriteEncryptionInfoIfNotExists(
	...
	if !errors.Is(err, cloud.ErrFileDoesNotExist) {
		return errors.Wrapf(err,
			"returned an unexpected error when checking for the existence of %s file", backupEncryptionInfoFile)
	}
