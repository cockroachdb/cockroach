...
// unflushed files of the FileSSTSink under different write scenarios.
func TestFileSSTSinkWrite(t *testing.T) {
	...
	for _, tt := range []testCase{
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			progCh := make(chan execinfrapb.RemoteProducerMetadata_BulkProcessorProgress, 100) // Buffer all progress.
			defer close(progCh)
		...
		Loop:
			for {
				...
				case p := <-progCh:
					...
					for _, f := range progDetails.Files {
	...
	for _, tt := range []testCase{
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, input := range tt.inputs {
				kvs := input.input
				// Add a range key in the input as well.s
...
func checkFiles(
	...
	for f, spans := range filePathToSpans {
		iter, err := storageccl.ExternalSSTReader(ctx, []storageccl.StoreFile{{store, f}}, nil, iterOpts)
		if err != nil {
			return err
		}
