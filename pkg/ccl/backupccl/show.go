...
func showBackupPlanHook(
	...
	fn := func(ctx context.Context, _ []sql.PlanNode, resultsCh chan<- tree.Datums) error {
		...
		if err := checkShowBackupURIPrivileges(ctx, p, dest[0]); err != nil {
			return err
		}
...
you must pass the 'encryption_info_dir' parameter that points to the directory of your full backup`
		...
		// TODO (msbutler, benbardin): this is an extremely crude check that the user is passing at
		// least as many URIS as there are localities in the backup.
		// This check is only meant for the 22.1 backport. Ben is working on a much more robust check
		for _, locMap := range info.localityInfo {
			if len(locMap.URIsByOriginalLocalityKV) > len(dest) && subdir != "" {
				p.BufferClientNotice(ctx, pgnotice.Newf("Your backup contains %d localities; however, "+
					"you passed only %d URIs to SHOW BACKUP. To capture all locality aware data, "+
		...
		if err != nil {
			if errors.Is(err, errLocalityDescriptor) && subdir == "" {
				p.BufferClientNotice(ctx,
					pgnotice.Newf("You ran `SHOW BACKUP` using the old syntax on a locality aware backup."+
						" The old SHOW BACKUP syntax does not display or validate data specific to locality"+
						" aware backups. Consider using the new `BACKUP INTO` syntax and `SHOW BACKUP"+
...
func getManifestDirs(fullSubdir string, defaultUris []string) ([]string, error) {
	...
	if len(strings.Split(defaultUris[0], fullSubdir)) != 2 {
		return nil, errors.Newf("the full backup URI %s does not contain 1 instance of the subdir %s"+
			"", defaultUris[0], fullSubdir)
	}
	fullCollectionRoot := strings.Split(defaultUris[0], fullSubdir)[0]

	if len(defaultUris) == 1 {
		return manifestDirs, nil
	}
	// unless the incremental backup is stored in the default incremental
	// directory (i.e. fullCollection/incrementals/fullSubdir), the incremental
	// chain is stored in collection/fullSubdir. Note: for incremental backups
	// created with incremental_location, the collection path will be different
	// than the full backup's.
	incSubdir := fullSubdir
	if len(strings.Split(defaultUris[1], fullSubdir)) != 2 {
		return nil, errors.Newf("the incremental backup URI %s does not contain 1 instance of the"+
			" subdir %s", defaultUris[1], fullSubdir)
	}
	incLocNoSubdir := strings.Split(defaultUris[1], fullSubdir)[0]

	if path.Join(fullCollectionRoot, DefaultIncrementalsSubdir) == incLocNoSubdir {
		// incremental backups stored in default incremental directory
		incSubdir = path.Join(DefaultIncrementalsSubdir, fullSubdir)
	}

	for i, incURI := range defaultUris[1:] {
		incDir := strings.Split(incURI, incSubdir)[1]
		manifestDirs[i+1] = path.Join(incSubdir, incDir)
	}
	return manifestDirs, nil
}
...
func showBackupsInCollectionPlanHook(
	...
	fn := func(ctx context.Context, _ []sql.PlanNode, resultsCh chan<- tree.Datums) error {
		...
		if err := checkShowBackupURIPrivileges(ctx, p, collection[0]); err != nil {
			return err
		}
