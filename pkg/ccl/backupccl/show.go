...
func showBackupTypeCheck(
	...
	if err := evalexpr.TypeCheck(
		...
		evalexpr.Strings{backup.Path},
		evalexpr.StringArrays{tree.Exprs(backup.InCollection)},
		evalexpr.KVOptions{
...
var showBackupOptions = evalexpr.KVOptionValidationMap{
	backupencryption.BackupOptEncPassphrase: evalexpr.KVStringOptRequireValue,
	backupencryption.BackupOptEncKMS:        evalexpr.KVStringOptRequireValue,
	backupOptWithPrivileges:                 evalexpr.KVStringOptRequireNoValue,
	backupOptAsJSON:                         evalexpr.KVStringOptRequireNoValue,
	backupOptWithDebugIDs:                   evalexpr.KVStringOptRequireNoValue,
	backupOptIncStorage:                     evalexpr.KVStringOptRequireValue,
	backupOptDebugMetadataSST:               evalexpr.KVStringOptRequireNoValue,
	backupOptEncDir:                         evalexpr.KVStringOptRequireValue,
	backupOptCheckFiles:                     evalexpr.KVStringOptRequireNoValue,
}
...
func showBackupPlanHook(
	...
	to, err := p.EvalAsString(ctx, backup.Path, "SHOW BACKUP")
	if err != nil {
	...
	if backup.InCollection != nil {
		inCol, err = p.EvalAsStringArray(ctx, tree.Exprs(backup.InCollection), "SHOW BACKUP")
		if err != nil {
			return nil, nil, nil, false, err
		}
	}
	...
	infoReader := getBackupInfoReader(p, backup)
	opts, err := p.EvalAsStringOpts(ctx, backup.Options, showBackupOptions)
	if err != nil {
		return nil, nil, nil, false, err
	}
...
func showBackupsInCollectionTypeCheck(
	...
	if err := evalexpr.TypeCheck(ctx, "SHOW BACKUPS", p.SemaCtx(),
		evalexpr.StringArrays{tree.Exprs(backup.InCollection)},
	); err != nil {
		return false, nil, err
	}
...
func showBackupsInCollectionPlanHook(
	...
	collection, err := p.EvalAsStringArray(ctx, tree.Exprs(backup.InCollection), "SHOW BACKUPS")
	if err != nil {
		return nil, nil, nil, false, err
	}
