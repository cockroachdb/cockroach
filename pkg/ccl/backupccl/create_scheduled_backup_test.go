...
func newTestHelper(t *testing.T) (*testHelper, func()) {
	...
	// Make it easy to delete all system.scheduled_jobs records.
	th.sqlDB.Exec(t, "SET sql_safe_updates = false")
...
func (h *testHelper) clearSchedules(t *testing.T) {
	t.Helper()
	h.sqlDB.Exec(t, "DELETE FROM system.scheduled_jobs")
}
...
`)
	...
	testCases := []struct {
		...
		schedule     string
		restore      string
		verifyTables map[string]string
	}{
		{
			name:         "cluster-backup",
			schedule:     "CREATE SCHEDULE FOR BACKUP TO $1 RECURRING '@hourly'",
			restore:      "RESTORE db.t1 FROM $1 WITH into_db=restore_db",
			verifyTables: map[string]string{"t1": "db.t1"},
		},
		{
			name:         "tables-backup-with-encryption",
			schedule:     "CREATE SCHEDULE FOR BACKUP db.t2, db.t3 TO $1 WITH encryption_passphrase='secret' RECURRING '@hourly'",
			restore:      "RESTORE TABLE db.t2, db.t3 FROM $1 WITH encryption_passphrase='secret', into_db=restore_db",
			verifyTables: map[string]string{"t2": "db.t2", "t3": "db.t3"},
		},
		{
			name:         "table-backup-in-different-dbs",
			schedule:     "CREATE SCHEDULE FOR BACKUP db.t1, other_db.t1, db.t3 TO $1 RECURRING '@hourly'",
			restore:      "RESTORE TABLE db.t3, other_db.t1 FROM $1 WITH into_db=restore_db",
			verifyTables: map[string]string{"t1": "other_db.t1", "t3": "db.t3"},
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Verify backup.
			th.sqlDB.Exec(t, "CREATE DATABASE restore_db; USE restore_db")
			defer th.sqlDB.Exec(t, "DROP DATABASE restore_db")
			th.sqlDB.Exec(t, tc.restore, destination)
			for restoreName, sourceName := range tc.verifyTables {
				th.sqlDB.CheckQueryResults(t,
					"SELECT * FROM restore_db."+restoreName,
					th.sqlDB.QueryStr(t, "SELECT * FROM "+sourceName))
			}
		})
	}
}
