...
func TestCreateStatement(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			scheduleCreateStmts := th.sqlDB.QueryStr(t, "SHOW CREATE ALL SCHEDULES")
			require.Equal(t, len(schedules), len(scheduleCreateStmts))

			for _, sj := range schedules {
				sjID := sj.ScheduleID()
				var createStmt string
				for _, schedule := range scheduleCreateStmts {
					if schedule[0] == strconv.Itoa(int(sjID)) {
						createStmt = schedule[1]
					}
				}
