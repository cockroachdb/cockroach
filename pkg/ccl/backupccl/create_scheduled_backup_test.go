	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if len(schedules) == 1 {
				...
				// The full should list incremental in it's schedule group, and vise versa.
				require.EqualValues(t, []int64{inc.ScheduleID()}, full.ScheduleMetaInfo().ScheduleGroup)
				require.EqualValues(t, []int64{full.ScheduleID()}, inc.ScheduleMetaInfo().ScheduleGroup)
			}
