...
func writeBackupMetadataSST(
	...
	if err := constructMetadataSST(ctx, dest, w, manifest, stats); err != nil {
		return err
	}
...
func (b *BackupMetadata) FileIter(ctx context.Context) FileIterator {
	backing := makeBytesIter(ctx, b.store, b.filename, []byte(sstFilesPrefix), b.enc, false)
	return FileIterator{
		backing: backing,
	}
}
...
func (fi *FileIterator) Next(file *BackupManifest_File) bool {
	wrapper := resultWrapper{}
	ok := fi.backing.next(&wrapper)
	if !ok {
		return false
	}
	err := protoutil.Unmarshal(wrapper.value, file)
	if err != nil {
		fi.err = err
		return false
	}
...
func (bi *bytesIter) next(result interface{}) bool {
	if bi.iterError != nil {
		return false
	}
	...
	resWrapper, ok := result.(*resultWrapper)
	if !ok {
		bi.iterError = errors.Errorf("cannot store result in type %T", result)
		return false
	}
