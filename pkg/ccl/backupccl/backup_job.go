...
func clusterNodeCount(gw gossip.OptionalGossip) (int, error) {
	...
	// If we somehow got 0 and return it, a caller may panic if they devide by
	// such a nonsenseical nodecount.
	if nodes == 0 {
...
func (b *backupResumer) Resume(
	...
	if err != nil {
		...
		log.Warningf(ctx, "unable to determine cluster node count")
		numClusterNodes = 1
	}
