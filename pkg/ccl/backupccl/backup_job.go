...
func backup(
	...
	if err := backupinfo.WriteBackupManifest(ctx, settings, defaultStore, backupinfo.BackupManifestName,
	...
	if err := backupinfo.WriteTableStatistics(ctx, defaultStore, backupStatisticsFileName, encryption, &statsTable); err != nil {
		return roachpb.RowCount{}, err
	}
...
func (b *backupResumer) Resume(ctx context.Context, execCtx interface{}) error {
	...
	redactedURI := backupinfo.RedactURIForErrorMessage(details.URI)
...
func (b *backupResumer) readManifestOnResume(
	...
	if err != nil {
		...
		desc, memSize, err = backupinfo.ReadBackupCheckpointManifest(ctx, mem, defaultStore, tmpCheckpoint, details.EncryptionOptions)
		if err != nil {
			return nil, 0, err
		}
