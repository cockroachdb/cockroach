...
func (b *backupResumer) Resume(
	...
	if err := writeBackupManifest(
		ctx, p.ExecCfg().Settings, defaultStore, BackupManifestCheckpointName, details.Encryption, &BackupManifest{},
	); err != nil {
		return errors.Wrapf(err, "writing checkpoint to %s", details.URI)
	}
