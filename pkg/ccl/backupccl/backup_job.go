...
func backup(
	...
	if backupUseFollowerReads.Get(evalCtx.ExecCfg.SV()) {
		planningTS, err = kv.MakeMockNoSenderTxnWithTimestamp(
			ctx, execCtx.ExecCfg().AmbientCtx, execCtx.ExecCfg().Clock,
			roachpb.NodeID(dsp.GatewayID()), execCtx.ExecCfg().DB.Context(),
