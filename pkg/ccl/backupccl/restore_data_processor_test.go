...
func runTestIngest(t *testing.T, init func(*cluster.Settings)) {
	...
	for i, testCase := range [][][]int{
		...
		t.Run(fmt.Sprintf("%d-%v", i, testCase), func(t *testing.T) {
			...
			if err := kvDB.AdminSplit(ctx, reqMidKey1, hlc.MaxTimestamp, roachpb.AdminSplitRequest_Ingestion); err != nil {
				t.Fatal(err)
			}
			if err := kvDB.AdminSplit(ctx, reqMidKey2, hlc.MaxTimestamp, roachpb.AdminSplitRequest_Ingestion); err != nil {
				t.Fatal(err)
			}
