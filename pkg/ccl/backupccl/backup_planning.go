...
func getURIsByLocalityKV(to []string, appendPath string) (string, map[string]string, error) {
	...
	for _, uri := range to {
		...
		if localityKV == defaultLocalityValue {
			// MB NOTE: I don't understand this logic. We will error out if defaultURI
			// isn't already specified. Doesn't this imply that the order of the user
			// submitted partitioned URI's would matter?
			if defaultURI != "" {
...
func backupJobDescription(
	...
	b, err := GetRedactedBackupNode(backup, to, incrementalFrom, kmsURIs, resolvedSubdir, incrementalStorage, true /* hasBeenPlanned */)
	if err != nil {
		return "", err
	}
