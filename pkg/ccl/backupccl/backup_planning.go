...
func privilegesDeprecationNotice(
	...
	preamble := "The existing privileges required for backup are being deprecated " +
		"in favour of a fine-grained privilege model explained here <link>. In 22.2 we will support " +
		"both the old and new privilege requirements, but in 23.1 to run"
...
func checkPrivilegesForBackup(
	...
	if requiresBackupSystemPrivilege && hasBackupSystemPrivilege {
		...
		return errors.New("only users with admin role or the BACKUP " +
			"system privilege are allowed to perform the backup")
	}
	...
	// descriptors.
	var hasBackupDatabasePrivileges bool
	if backupStmt.Targets != nil && backupStmt.Targets.Databases != nil {
		hasBackupDatabasePrivileges = true
		for _, desc := range targetDescs {
			switch desc := desc.(type) {
			case catalog.DatabaseDescriptor:
				hasBackupDatabasePrivileges = hasBackupDatabasePrivileges &&
					p.CheckPrivilegeForUser(ctx, desc, privilege.BACKUP, p.User()) == nil
			}
		}
	}

	// If a user has the BACKUP privilege on the target databases we can move on
	// to checking the destination URIs.
	if hasBackupDatabasePrivileges {
		return checkBackupDestinationPrivileges(ctx, p, to)
	}
	...
	// error. In 22.2 we continue to check for old style privileges on the
	// descriptors.
	var hasBackupTablePrivileges bool
	if backupStmt.Targets != nil && backupStmt.Targets.Tables.TablePatterns != nil {
		hasBackupTablePrivileges = true
		for _, desc := range targetDescs {
			switch desc := desc.(type) {
			case catalog.TableDescriptor:
				err := p.CheckPrivilegeForUser(ctx, desc, privilege.BACKUP, p.User())
				hasBackupTablePrivileges = hasBackupTablePrivileges && err == nil
			}
		}
	}

	if hasBackupTablePrivileges {
		return checkBackupDestinationPrivileges(ctx, p, to)
	}
