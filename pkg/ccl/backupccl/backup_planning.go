...
func getLogicallyMergedSpansAcrossTables(
	...
	if len(tableIDs) == 1 {
		return spans[tableIDs[0]].mergedSpans
	}
	...
	for curIndex := 0; curIndex < len(tableIDs)-1; curIndex++ {
		...
		// If found, we do not want to merge as the backup will include unexpected data.
		// These could be keys corresponding to tables that have been dropped, but not gc'ed.
		...
		// If all three checks passed, a merge is possible and we add every span up until the last one on the LHS table,
		// then we update the start key of the first span on the RHS table to be the start key of the last span
		// on the LHS table. The merged span will be added to the result slice in future iterations.
...
func spansForAllTableIndexes(
	...
	sort.Sort(revIDs)
	mergedRevTableSpans := getLogicallyMergedSpansAcrossTables(
		revIDs,
		revTblSpans,
		endTime,
		added,
