...
// This struct is a WIP. I like the concept, but I think it needs a rename.
...
func backupPlanHook(
	...
	encryptionCtx.encryptMode = noEncryption
	if backupStmt.Options.EncryptionPassphrase != nil {
		...
		pwFn = fn
		encryptionCtx.encryptMode = passphrase
	}
	...
	fn := func(ctx context.Context, _ []sql.PlanNode, resultsCh chan<- tree.Datums) error {
		...
		case passphrase:
			...
			encryptionCtx.encryptionPassphrase = []byte(pw)
		case kms:
			encryptionCtx.kmsURIs, encryptionCtx.kmsEnv, err = kmsFn()
			if err != nil {
				return err
			}
			if err := requireEnterprise("encryption"); err != nil {
		...
		collectionURI, defaultURI, urisByLocalityKV, prevBackups, encryption, err := resolveDest(ctx, p, backupStmt, defaultURI, urisByLocalityKV, makeCloudStorage, endTime, to, incrementalFrom, encryptionCtx)
		if err != nil {
			return err
		}
		...
		description, err := backupJobDescription(p, backupStmt.Backup, to, incrementalFrom, encryptionCtx.kmsURIs)
		if err != nil {
			return err
		}
		...
		if encryption == nil {
			encryption, err = makeNewEncryptionOptions(ctx, p, encryptionCtx, makeCloudStorage, defaultURI)
			if err != nil {
				return err
			}
		}
...
func makeNewEncryptionOptions(
	...
	case passphrase:
		...
		encryption = &jobspb.BackupEncryptionOptions{
			...
			Key:  storageccl.GenerateKey(encryptionCtx.encryptionPassphrase, salt)}
	case kms:
		...
		encryptedDataKeyByKMSMasterKeyID, defaultKMSInfo, err :=
			getEncryptedDataKeyByKMSMasterKeyID(ctx, encryptionCtx.kmsURIs, plaintextDataKey, encryptionCtx.kmsEnv)
		if err != nil {
			return nil, err
		}
