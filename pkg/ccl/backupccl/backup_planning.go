...
func backupPlanHook(
	...
	fn := func(ctx context.Context, _ []sql.PlanNode, resultsCh chan<- tree.Datums) error {
		...
		initialDetails := jobspb.BackupDetails{
			...
			EndTime:     endTime,
			Arguments: &jobspb.BackupDetails_Arguments{
		...
		if backupStmt.Targets != nil && backupStmt.Targets.Tenant != (roachpb.TenantID{}) {
			...
			initialDetails.Arguments.SpecificTenantIds = []uint64{backupStmt.Targets.Tenant.ToUint64()}
		}
		...
		description, err := backupJobDescription(p, backupStmt.Backup, to, incrementalFrom, encryptionParams.kmsURIs, backupDetails.Destination.Subdir)
		if err != nil {
			return err
		}
...
func collectTelemetry(backupManifest BackupManifest, initialDetails, backupDetails jobspb.BackupDetails, licensed bool) {
	...
	if backupDetails.StartTime.IsEmpty() {
		...
		if len(initialDetails.Arguments.IncrementalFrom) == 0 {
...
func getBackupDetailAndManifest(
	...
	initialDetails jobspb.BackupDetails, user security.SQLUsername,
	encryptionParams backupEncryptionParams,
	...
	if initialDetails.Arguments.RevisionHistory {
	...
	if initialDetails.Arguments.FullCluster {
		...
		descriptorProtos = initialDetails.Arguments.ResolvedTargets
	...
	collectionURI, defaultURI, resolvedSubdir, urisByLocalityKV, prevs, err :=
		resolveDest(ctx, user, initialDetails.Destination, makeCloudStorage, endTime, initialDetails.Arguments.IncrementalFrom)
	if err != nil {
		return jobspb.BackupDetails{}, BackupManifest{}, err
	}
	...
	if len(prevBackups) > 0 {
		...
		if baseManifest.DescriptorCoverage == tree.AllDescriptors &&
			!initialDetails.Arguments.FullCluster {
	...
	if mvccFilter == MVCCFilter_All {
		...
		revs, err = getRelevantDescChanges(ctx, execCfg, startTime, endTime, targetDescs,
			initialDetails.Arguments.ResolvedCompleteDbs, priorIDs, initialDetails.Arguments.FullCluster)
		if err != nil {
			return jobspb.BackupDetails{}, BackupManifest{}, err
		}
	}
	...
	if initialDetails.Arguments.FullCluster && execCfg.Codec.ForSystemTenant() {
		// Include all tenants.
	...
	} else if len(initialDetails.Arguments.SpecificTenantIds) > 0 {
		for _, id := range initialDetails.Arguments.SpecificTenantIds {
	...
	if len(prevBackups) > 0 {
		...
		if !initialDetails.Arguments.FullCluster {
			...
			if err := checkForNewCompleteDatabases(targetDescs, initialDetails.Arguments.ResolvedCompleteDbs, dbsInPrev); err != nil {
				return jobspb.BackupDetails{}, BackupManifest{}, err
			}
		}
	...
	backupManifest := BackupManifest{
		...
		CompleteDbs:         initialDetails.Arguments.ResolvedCompleteDbs,
		Spans:               spans,
