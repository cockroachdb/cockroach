...
func writeBackupManifestCheckpoint(
	...
	// We timestamp the checkpoint files in order to enforce write once
	// backups. By timestamping with monotonically increasing timestamps
	// we avoid ever overwriting a checkpoint file.
	filename := newTimestampedCheckpointFileName()

	// HTTP storage does not support listing, and so the very first time
	// we write a checkpoint we just write a non-versioned checkpoint file
	// that we can always find without listing.
	...
	if !execCfg.Settings.Version.IsActive(ctx, clusterversion.BackupDoesNotOverwriteLatestAndCheckpoint) {
		err = cloud.WriteFile(ctx, defaultStore, backupManifestCheckpointName+backupManifestChecksumSuffix, bytes.NewReader(checksum))
	} else {
