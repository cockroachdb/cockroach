...
func backupPlanHook(
	...
	fn := func(ctx context.Context, _ []sql.PlanNode, resultsCh chan<- tree.Datums) error {
		...
		if err := setupProtectedTimestampChaining(ctx, p, &backupDetails, backupStmt); err != nil {
			return err
		}
...
func setupProtectedTimestampChaining(
	ctx context.Context,
	p sql.PlanHookState,
	backupDetails *jobspb.BackupDetails,
	backupStmt *annotatedBackupStatement,
	...
	sj, args, err := getScheduledBackupExecutionArgsFromSchedule(ctx, env,
	...
	if args.BackupType == ScheduledBackupExecutionArgs_FULL {
		...
		incSj, _, err := getScheduledBackupExecutionArgsFromSchedule(ctx, env,
		...
		backupDetails.ReleaseOnSuccessPTSRecord = incSj.ExecutionArgs().ProtectedTimestampRecord
	} else {
		...
		backupDetails.UpdateOnSuccessPTSRecord = sj.ExecutionArgs().ProtectedTimestampRecord
	}
	return nil
}
