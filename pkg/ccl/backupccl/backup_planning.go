...
// kmsEnv captures the cluster properties that  concrete KMS implementations
// need to setup and operate.
var kmsEnv *backupKMSEnv

func getBackupKMSEnv() (*backupKMSEnv, error) {
	if kmsEnv == nil {
		return nil, errors.New("BACKUP KMS env has not been initialized in BACKUP planning")
	}
...
func validateKMSURIsAgainstFullBackup(
	kmsURIs []string, kmsMasterKeyIDToDataKey map[string][]byte, kmsEnv cloud.KMSEnv,
...
) (map[string][]byte, *jobspb.BackupEncryptionOptions_KMSInfo, error) {
