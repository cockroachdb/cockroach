...
func (e *scheduledBackupExecutor) CreateString(sj *jobs.ScheduledJob) (string, error) {
	...
	fullBackup := &tree.FullBackupClause{AlwaysFull: false}
	if !backupNode.AppendToLatest && args.UnpauseOnSuccess == jobs.InvalidScheduleID {
		fullBackup.AlwaysFull = true
	} else {
		fullBackup.Recurrence = tree.NewDString(args.DependentScheduleRecurrence)
	}
	...
	if sj.ScheduleDetails().IgnoreExistingBackups {
		scheduleOptions = append(scheduleOptions, tree.KVOption{
			Key: optIgnoreExistingBackups,
			Value: nil,
		})
	}
	...
	node := &tree.ScheduledBackup{
		...
		Recurrence: tree.NewDString(sj.ScheduleExpr()),
