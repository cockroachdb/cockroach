...
// tenantMetadataQuery returs a query used to retrieve the tenant metadata.
// A ` WHERE id = $1` clause can be appended.
func tenantMetadataQuery(ctx context.Context, settings *cluster.Settings) string {
	if !settings.Version.IsActive(ctx, clusterversion.TenantUsageTable) {
		...
		return tenantMetadataQueryNoTenantUsageTable
	}
	return tenantMetadataQueryWithTenantUsageTable
}
...
func retrieveAllTenantsMetadata(
	...
	rows, err := ie.QueryBuffered(
		...
		tenantMetadataQuery(ctx, settings),
	)
	if err != nil {
		return nil, err
	}
