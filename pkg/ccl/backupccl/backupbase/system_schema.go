...
func GetSystemTablesToRestoreBeforeData() map[string]struct{} {
	backupSystemTables := GetSystemTablesToIncludeInClusterBackup()
	for systemTable := range backupSystemTables {
		if !SystemTableBackupConfiguration[systemTable].RestoreBeforeData {
			delete(backupSystemTables, systemTable)
		}
	}

	return backupSystemTables
}
