...
func alterBackupPlanHook(
	...
	fromFn, err := p.TypeAsStringArray(ctx, tree.Exprs(alterBackupStmt.Backup), "ALTER BACKUP")
	if err != nil {
		return nil, nil, nil, false, err
	}
	...
	fn := func(ctx context.Context, _ []sql.PlanNode, resultsCh chan<- tree.Datums) error {
		...
		if subdir != "" {
			if strings.EqualFold(subdir, "LATEST") {
				...
				latest, err := readLatestFile(ctx, backup[0], p.ExecCfg().DistSQLSrv.ExternalStorageFromURI, p.User())
				if err != nil {
					return err
				}
				subdir = latest
			...
			if err = appendPaths(backup, subdir); err != nil {
				return err
			}
		}
...
func doAlterBackupPlan(
	...
	baseStores, err := p.ExecCfg().DistSQLSrv.ExternalStorageFromURI(ctx, backup[0], p.User())
	if err != nil {
	...
	err = writeAlteredEncryptionInfo(ctx, encryptionInfo, baseStores)
	if err != nil {
		return err
	}
	return nil
...
func getEncryptionInfoFiles(ctx context.Context, dest cloud.ExternalStorage) ([]string, error) {
	...
	if len(files) < 1 {
		return nil, errors.Errorf("no encrypted files found")
	}
...
func writeAlteredEncryptionInfo(
	...
	files, err := getEncryptionInfoFiles(ctx, dest)
	if err != nil {
		return err
	}

	// New encryption-info file name is in the format "ENCRYPTION-INFO-<version number>"
	newEncryptionInfoFile := fmt.Sprintf("%s-%d", backupEncryptionInfoFile, len(files)+1)
