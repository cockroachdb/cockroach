...
func alterBackupTypeCheck(
	...
	for _, expr := range []tree.Expr{
		...
		if _, err := tree.TypeCheckAndRequire(
			ctx, expr, p.SemaCtx(), types.String, "ALTER BACKUP",
		); err != nil {
...
func alterBackupPlanHook(
	...
	backup, err := p.EvalAsString(ctx, alterBackupStmt.Backup, "ALTER BACKUP")
	if err != nil {
		return nil, nil, nil, false, err
	}
	...
	if alterBackupStmt.Subdir != nil {
		subdir, err = p.EvalAsString(ctx, alterBackupStmt.Subdir, "ALTER BACKUP")
		if err != nil {
			return nil, nil, nil, false, err
		}
	}
	...
	for _, cmd := range alterBackupStmt.Cmds {
		...
		case *tree.AlterBackupKMS:
			newKms, err = p.EvalAsStringArray(ctx, tree.Exprs(v.KMSInfo.NewKMSURI), "ALTER BACKUP")
			if err != nil {
				return nil, nil, nil, false, err
			}
			oldKms, err = p.EvalAsStringArray(ctx, tree.Exprs(v.KMSInfo.OldKMSURI), "ALTER BACKUP")
			if err != nil {
				return nil, nil, nil, false, err
			}
		}
