	...
	fn := func(ctx context.Context, _ []sql.PlanNode, resultsCh chan<- tree.Datums) error {
		...
		if subdir != "" {
			if strings.EqualFold(subdir, "LATEST") {
				...
				latest, err := backupdestination.ReadLatestFile(ctx, backup, p.ExecCfg().DistSQLSrv.ExternalStorageFromURI, p.User())
				if err != nil {
					return err
				}
				subdir = latest
...
func doAlterBackupPlan(
	...
	for _, old := range oldKms {
		for _, encFile := range opts {
			defaultKMSInfo, err = backupencryption.ValidateKMSURIsAgainstFullBackup([]string{old},
