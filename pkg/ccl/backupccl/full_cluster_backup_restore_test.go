...
func TestFullClusterBackup(t *testing.T) {
	...
	_, _, sqlDB2, cleanupEmptyCluster := backupRestoreTestSetupEmpty(t, singleNode, tempDir, initNone)
	defer cleanupFn()
	...
	t.Run("ensure user table data restored", func(t *testing.T) {
		...
		for _, table := range expectedUserTables {
			...
			sqlDB2.CheckQueryResults(t, query, sqlDB.QueryStr(t, query))
		}
	})

	t.Run("ensure that grants are restored", func(t *testing.T) {
		grantCheck := "use system; SHOW grants"
		sqlDB2.CheckQueryResults(t, grantCheck, sqlDB.QueryStr(t, grantCheck))
	...
	t.Run("ensure that jobs are restored", func(t *testing.T) {
		...
		// its own.
		newJobs := sqlDB2.QueryStr(t, "SELECT * FROM system.jobs")
