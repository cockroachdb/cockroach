...
func TestReintroduceOfflineSpans(t *testing.T) {
	...
	t.Run("restore-canceled", func(t *testing.T) {
		...
		WHERE job_type = 'RESTORE' AND status NOT IN ('succeeded', 'failed')`,
			[][]string{},
		)
		...
		destDB.CheckQueryResults(t,
			...
			[][]string{{"failed", "1"}, {"succeeded", "1"}})

		destDB.ExpectErr(t, `relation "restoredb.bank" does not exist`,
			`SELECT count(*) FROM restoredb.bank`, `relation restoredb.bank does not exist`)
	})
}
