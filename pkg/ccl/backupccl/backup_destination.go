...
func getBackupManifests(
	...
	for i := range backupURIs {
		i := i
		// boundAccount isn't threadsafe so we'll make a new one for each goroutine
		// to use while reading. When it is done, we'll shink and close that local
		// one,
		// local one but
		// before reserving the size of from the passed one to account for the size
		...
		g.GoCtx(func(ctx context.Context) error {
			...
			// We'll lock the main one and reserve the indicicated size which is still
			// currently reserved in subMem, so we'll briefly be double reserved but
			// that's safer than releasing first.
			memMu.Lock()
