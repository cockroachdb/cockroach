		...
		g2.GoCtx(func(ctx context.Context) error {
			...
			for importSpanChunk := range restoreEntryChunksCh {
				...
				if _, ok := flowCtx.NodeID.OptionalNodeID(); !ok {
					// If a sperate process tenant is running restore, the nodeID returned
					// by scatter does not map identically to a sqlInstanceID; thus, the
					// processor must randomly choose a sqlInstanceID to route the chunk
					// to.
					//
					// TODO(msbutler): it is unfortunate that all logic after scatter
					// operates on a NodeID object. The logic should use
					// SQLInstanceID's as these chunks are routed to dist sql processors
					// running on sql servers.
