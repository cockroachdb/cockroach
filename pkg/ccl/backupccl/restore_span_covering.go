...
func makeSpanCoveringFilter(
	requiredSpans roachpb.Spans,
	persistedFrontier []jobspb.RestoreProgress_FrontierEntry,
	...
	checkpointFrontier, err := spanUtils.MakeFrontier(requiredSpans...)
	if err != nil {
		return sh, err
	}
	for _, sp := range persistedFrontier {
		_, err = checkpointFrontier.Forward(sp.Span, sp.Timestamp)
		if err != nil {
			return sh, err
		}
	}
	sh.checkpointFrontier = checkpointFrontier
	return sh, nil
}
...
func generateAndSendImportSpans(
	...
	for _, requiredSpan := range requiredSpans {
		spans := filter.filterCompleted(requiredSpan)
		if len(spans) == 0 {
			spanStr := filter.checkpointFrontier.String()
			fmt.Println(spanStr)
		}
