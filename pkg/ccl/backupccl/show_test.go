...
func TestShowBackup(t *testing.T) {
	...
	// WITH SCHEMAS: Test the creation statement.
	...
	// Test that tables, views and sequences are all supported.
	{
		...
		showBackupRows = sqlDB.QueryStr(t, fmt.Sprintf(`SHOW BACKUP WITH SCHEMAS '%s'`, viewTableSeq))
		expected = []string{
			expectedCreateTable,
			expectedCreateView,
			expectedCreateSeq,
	...
	// are included.
	{
		...
		showBackupRows = sqlDB.QueryStr(t, fmt.Sprintf(`SHOW BACKUP WITH SCHEMAS '%s'`, includedFK))
	...
	// the create statement.
	{
		...
		showBackupRows = sqlDB.QueryStr(t, fmt.Sprintf(`SHOW BACKUP WITH SCHEMAS '%s'`, missingFK))
