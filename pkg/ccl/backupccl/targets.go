...
func getRelevantDescChanges(
	...
	systemTableIDsToBackup, err := GetSystemTableIDsToIncludeInClusterBackup(ctx, execCfg)
	if err != nil {
		return nil, err
	}
	isExcludedDescriptor := func(id descpb.ID) bool {
		if _, isOptInSystemTable := systemTableIDsToBackup[id]; id == keys.SystemDatabaseID || !isOptInSystemTable {
			return true
		}
		return false
	}
