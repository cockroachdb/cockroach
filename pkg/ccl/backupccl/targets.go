...
// manifest.IntroducedSpans field. This check only applies to backups with a
// start time that is less than the restore AOST.
			...
			if table, _, _, _ := descpb.FromDescriptor(&desc); table != nil {
				...
				if !inPrevBackup && !introduced {
					...
					return errors.WithIssueLink(tableError, errors.IssueLink{
						...
						Detail:   `A backup chain with revision history can lose data on a table with an import`,
					})
				}
			}
		}
