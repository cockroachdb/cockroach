			...
			if b.StartTime.Less(endTime) && endTime.LessEq(b.EndTime) {
				if endTime != b.EndTime && b.MVCCFilter != MVCCFilter_All {
					return BackupTableEntry{}, errors.Newf(
						"reading data for requested time requires that BACKUP was created with "+backupOptRevisionHistory+
							" or should specify the time to be an exact backup time, nearest backup time is %s", timeutil.Unix(0, b.EndTime.WallTime).UTC())
				}
				ind = i
				break
			}
