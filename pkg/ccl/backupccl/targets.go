...
type descriptorsMatched struct {
	...
	//
	// TODO(ajwerner): Replace this with ExtendedDescriptorInterface.
	descs []sqlbase.DescriptorInterface
...
type descriptorResolver struct {
	descByID map[descpb.ID]sqlbase.DescriptorInterface
	// Map: db name -> dbID
...
func (r *descriptorResolver) LookupSchema(
	...
	if dbID, ok := r.dbsByName[dbName]; ok {
		if dbDesc, ok := r.descByID[dbID].(sqlbase.DatabaseDescriptorInterface); ok {
...
func newDescriptorResolver(descs []sqlbase.DescriptorInterface) (*descriptorResolver, error) {
	r := &descriptorResolver{
		descByID:   make(map[descpb.ID]sqlbase.DescriptorInterface),
	...
	for _, desc := range descs {
		if _, isDB := desc.(sqlbase.DatabaseDescriptorInterface); isDB {
	...
	// object registry.
	registerDesc := func(parentID descpb.ID, desc sqlbase.DescriptorInterface, kind string) error {
...
func descriptorsMatchingTargets(
	...
	descriptors []sqlbase.DescriptorInterface,
	...
	getTypeByID := func(id descpb.ID) (sqlbase.TypeDescriptorInterface, error) {
		desc, ok := resolver.descByID[id]
		if !ok {
			return nil, errors.Newf("type with ID %d not found", id)
		}
		typeDesc, ok := desc.(sqlbase.TypeDescriptorInterface)
		if !ok {
	...
	for _, pattern := range targets.Tables {
		...
		case *tree.TableName:
			...
			tableDesc := descI.(sqlbase.TableDescriptorInterface)
		...
		case *tree.AllTablesSelector:
			...
			desc := descI.(sqlbase.DatabaseDescriptorInterface)
...
func getRelevantDescChanges(
	...
	for _, i := range descs {
		...
		if table, isTable := i.(sqlbase.TableDescriptorInterface); isTable {
...
) ([]sqlbase.DescriptorInterface, []descpb.ID, error) {
...
func fullClusterTargetsBackup(
	allDescs []sqlbase.DescriptorInterface,
) ([]sqlbase.DescriptorInterface, []descpb.ID, error) {
...
func fullClusterTargets(
	allDescs []sqlbase.DescriptorInterface,
) ([]sqlbase.DescriptorInterface, []*sqlbase.ImmutableDatabaseDescriptor, error) {
	fullClusterDescs := make([]sqlbase.DescriptorInterface, 0, len(allDescs))
...
func fullClusterTargetsRestore(
	...
	filteredDescs := make([]sqlbase.DescriptorInterface, 0, len(fullClusterDescs))
