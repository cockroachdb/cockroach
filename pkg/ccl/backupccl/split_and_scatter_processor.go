...
func (s dbSplitAndScatterer) findDestination(
	...
	if len(br.Responses) != 1 {
		log.Warningf(ctx, "expected 1 response from scatter request, got %d", len(br.Responses))
		return 0
	}

	// RangeInfos was moved to the batch request header in 20.2. If the batch
	// request does not populate RangeInfos, the span must have been scattered to
	// a 20.1 node. Since we won't be able to plan a RestoreData processor on this
	// node, we'll just send it to the default stream.
	if len(br.RangeInfos) == 0 {
		log.Infof(ctx, "scattered span on a 20.1 node")
		return 0
	}

	if len(br.RangeInfos) != 1 {
		log.Warningf(ctx, "expected only 1 range to be scattered, scattered %d",
			len(br.RangeInfos))
	}

	return br.RangeInfos[0].Lease.Replica.NodeID
}
