...
func TestTenantBackupMultiRegionDatabases(t *testing.T) {
	...
	tc, db, cleanup := multiregionccltestutils.TestingCreateMultiRegionCluster(t, 3 /*numServers*/, base.TestingKnobs{})
	defer cleanup()
	...
	tenSQLDB.Exec(t, `BACKUP DATABASE mrdb INTO $1`, tenDst)

	sqlDB.Exec(t, `CREATE DATABASE mrdb PRIMARY REGION "us-east1"`)
	sqlDB.Exec(t, `BACKUP DATABASE mrdb INTO $1`, hostDst)

	{
		...
		tenSQLDB.ExpectErr(t, "setting .* disallows secondary tenant to restore a multi-region database",
			"RESTORE DATABASE mrdb FROM LATEST IN $1",
			tenDst,
		)

		// The system tenant should remain unaffected.
		sqlDB.Exec(t, "DROP DATABASE mrdb CASCADE")
		sqlDB.Exec(t, "RESTORE DATABASE mrdb FROM LATEST IN $1", hostDst)
	}

	{
		...
		tenSQLDB.Exec(t, "RESTORE DATABASE mrdb FROM LATEST IN $1", tenDst)
	}

	{
		...
		tenSQLDB.Exec(t, `BACKUP DATABASE nonMrDB INTO $1`, tenDst)

		tenSQLDB.Exec(t, "DROP DATABASE nonMrDB CASCADE")
		tenSQLDB.Exec(t, "RESTORE DATABASE nonMrDB FROM LATEST IN $1", tenDst)
	}
}
