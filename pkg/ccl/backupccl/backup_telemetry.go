...
// invoked backup or a scheduled backup. It is necessary to populate information
// from both initialDetails and backupDetails as initialDetails contain
// information prior to resolution, such as whether the user specified the
// LATEST subdir, while backupDetails contain information about all of the
// resolved targets.
func logBackupTelemetry(
	...
	revisionHistory := backupManifest.MVCCFilter == backuppb.MVCCFilter_All
	passphrase, kms, kmsCount := getPassphraseAndKMS(ctx, backupDetails.EncryptionOptions)
	...
	event := &eventpb.RecoveryEvent{
		...
		AsOfInterval:            backupDetails.AsOfInterval,
		Options:                 options,
	}

	event.AuthTypes = make([]string, 0, len(authTypes))
	for typ := range authTypes {
		event.AuthTypes = append(event.AuthTypes, typ)
	}
	sort.Strings(event.AuthTypes)
...
func logCreateScheduleTelemetry(
	...
	event := &eventpb.RecoveryEvent{
		...
		CustomFirstRunTime:    firstRunNanos,
		OnPreviousRunning:     jobspb.ScheduleDetails_WaitBehavior_name[int32(details.Wait)],
		OnExecutionFailure:    jobspb.ScheduleDetails_ErrorHandlingBehavior_name[int32(details.OnError)],
