...
func (r *standbyReadTSPollerResumer) poll(ctx context.Context) error {
	...
	for {
		...
		case <-ticker.C:
			...
			if err := replication.SetupOrAdvanceStandbyReaderCatalog(ctx, tenantID, replicatedTime, r.db, r.settings); err != nil {
				return err
			}
		}
	}
