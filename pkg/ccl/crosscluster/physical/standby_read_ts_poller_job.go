...
func (r *standbyReadTSPollerResumer) poll(ctx context.Context, execCfg *sql.ExecutorConfig) error {
	pollInterval := standbyReadTSPollInterval

	if execCfg.StreamingTestingKnobs != nil &&
		execCfg.StreamingTestingKnobs.StandbyPollInterval.Seconds() > 0 {
		pollInterval = execCfg.StreamingTestingKnobs.StandbyPollInterval
	}
	...
	for {
		...
		case <-ctx.Done():
			return ctx.Err()
		case <-ticker.C:
			readerTenant := execCfg.UpgradeJobDeps.SystemDeps().ReaderTenant
			if readerTenant == nil {
				log.Info(ctx, "reader tenant is nil")
				continue
			}
			tenantID, replicatedTime, err := readerTenant.ReadFromTenantInfo(ctx)
			if err != nil {
