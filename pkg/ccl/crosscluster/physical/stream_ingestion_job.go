...
import (
	...
	"github.com/cockroachdb/cockroach/pkg/ccl/crosscluster/producer"
...
func maybeRevertToCutoverTimestamp(
	...
	if !cutoverTimestamp.Equal(replicatedTimeAtCutover) && readerTenantID.IsSet() {
		err := stopTenant(ctx, p.ExecCfg(), readerTenantID)
		if err != nil {
...
func stopTenant(ctx context.Context, execCfg *sql.ExecutorConfig, tenantID roachpb.TenantID) error {
	...
	_, err = ie.Exec(ctx, "stop tenant", nil, `ALTER VIRTUAL CLUSTER $1 STOP SERVICE`, tenantInfo.Name)
	if err != nil {
		return err
	}
	...
	if err = execCfg.InternalDB.Txn(ctx, func(ctx context.Context, txn isql.Txn) error {
		err = sql.UpdateTenantRecord(ctx, execCfg.Settings, txn, tenantInfo)
		if err != nil {
			return err
		}
		return nil
	...
	_, err = ie.Exec(ctx, "drop tenant", nil, `DROP VIRTUAL CLUSTER IF EXISTS $1 IMMEDIATE`, tenantInfo.Name)
	if err != nil {
		return err
	}
	return nil
