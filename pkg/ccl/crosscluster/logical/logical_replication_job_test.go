...
func TestLogicalJobResiliency(t *testing.T) {
	...
	// DB B should be on a different node
	...
	// This test will follow the following steps:
	// Create a table A with ranges on multiple nodes
	// Create another table B with ranges on another node
	// Set up the initial logical replication job so we get addresses to both partition
	...
	getStreamAddresses(t, dbA, jobAID)

	t.Logf("Stopping node 1")
	// Stop the first node that's coordinating the stream
	server.StopServer(0)
	...
	WaitUntilReplicatedTime(t, now, dbB, jobAID)
}
...
func getStreamAddresses(t *testing.T, db *sqlutils.SQLRunner, ingestionJobID jobspb.JobID) {
	testutils.SucceedsSoon(t, func() error {
		progress := jobutils.GetJobProgress(t, db, ingestionJobID)
		addresses := progress.Details.(*jobspb.Progress_LogicalReplication).LogicalReplication.StreamAddresses
		if len(addresses) == 0 {
			return errors.Newf("streaming has no addresses")
		}
		return nil
	})
}
