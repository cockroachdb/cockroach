...
func TestShowLogicalReplicationJobs(t *testing.T) {
	...
	for showRows.Next() {
		...
		progress := jobutils.GetJobProgress(t, dbA, expectedJobID)
		expectedReplicatedTime := progress.GetLogicalReplication().ReplicatedTime.GoTime().Round(time.Microsecond)
		// `SHOW LOGICAL REPLICATION JOBS` query runs before the job query in `jobutils.GetJobProgress()`,
		// `LogicalReplicationProgress.ReplicatedTime` could have advanced by the time we call `jobutils.GetJobProgress()`,
		// therefore replicatedTime should be less than or equal to expectedReplicatedTime.
		require.LessOrEqual(t, replicatedTime, expectedReplicatedTime)
