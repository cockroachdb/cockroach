...
func TestPreviouslyInterestingTables(t *testing.T) {
	...
	testCases := []testCase{
		...
		{
			// Collated string store data in the value even when
			// they are part of the primary key. As a result, the
			// KeyValue in the rangefeed event generated by a DELETE
			// does not have enough data to construct a valid value
			// for lookup into the destination table.
			name:   "collated string in primary key",
			schema: `CREATE TABLE rand_table (col STRING COLLATE en_US PRIMARY KEY)`,
			delete: true,
		},
		{
			// Collated string store data in the value even when
			...
			// does not have enough data to construct a valid value
			// for lookup into the destination table.
			name:   "single key table with udf",
