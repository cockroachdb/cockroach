...
func TestUserDefinedTypes(t *testing.T) {
	...
	dbA.Exec(t, "CREATE TABLE data (pk INT PRIMARY KEY, val my_enum)")
	dbB.Exec(t, "CREATE TABLE data (pk INT PRIMARY KEY, val my_enum)")
	...
	dbB.CheckQueryResults(t, "SELECT * FROM data", [][]string{{"1", "one"}})
	dbA.CheckQueryResults(t, "SELECT * FROM data", [][]string{{"1", "one"}})
}
