...
func TestLWWConflictResolution(t *testing.T) {
	...
	testutils.RunTrueAndFalse(t, "useKVProc", func(t *testing.T, useKVProc bool) {
		...
		testutils.RunTrueAndFalse(t, "optimistic_insert", func(t *testing.T, optimisticInsert bool) {
			...
			// Write to table on both the src and dest cluster and see how conflicts are handled
			// The local write will not set crdb_replication_origin_timestamp
			...
			// Receive multiple updates remotely and handle conflicts between them
			...
			t.Run("outdated-write-conflict", func(t *testing.T) {
				...
				// The src cluster sends another write, but the local write wins the conflict
				...
				// This time the src write should win the conflict
