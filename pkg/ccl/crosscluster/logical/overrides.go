...
// destination table. LDR sessions run as the job owner (the user who
...
// else. Attach the result to a SessionData (for paths that hand the
// session to the applier directly) or to an InternalExecutorOverride
// (for paths that use per-op overrides).
func destTableOverrides(tableIDs []descpb.ID) map[uint32]sessiondata.DescriptorOverride {
	...
	for _, id := range tableIDs {
		grants[uint32(id)] = sessiondata.DescriptorOverride{Privileges: bits}
	}
	return grants
}
