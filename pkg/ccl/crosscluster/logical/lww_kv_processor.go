...
func writeableColunms(ctx context.Context, td catalog.TableDescriptor) ([]catalog.Column, error) {
	...
	storedColumnIDs := td.GetPrimaryIndex().CollectPrimaryStoredColumnIDs()

	pubCols := td.PublicColumns()
	ret := make([]catalog.Column, 0, len(pubCols))
	for _, c := range pubCols {
		if c.IsComputed() && !(keyColumnIDs.Contains(c.GetID()) || storedColumnIDs.Contains(c.GetID())) {
			continue
		}
		ret = append(ret, c)
	}
...
func (p *kvTableWriter) deleteRow(
	...
	return p.rd.DeleteRow(ctx, b, p.oldVals, ph, oth.DelWithCPut, false)
}
