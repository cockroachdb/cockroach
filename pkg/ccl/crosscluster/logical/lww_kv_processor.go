...
func (p *kvRowProcessor) getWriter(ctx context.Context, id descpb.ID, ts hlc.Timestamp) (*kvTableWriter, error) {
	w, ok := p.writers[id]
	if ok {
...
func (p *kvTableWriter) insertRow(
	ctx context.Context, b *kv.Batch, after cdcevent.Row,
) error {
