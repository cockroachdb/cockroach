...
func (p *kvRowProcessor) HandleBatch(
	...
	if len(batch) == 1 {
		...
		err = p.cfg.DB.Txn(ctx, func(ctx context.Context, txn isql.Txn) error {
			b := makeKVBatch(txn.KV())

			for _, kv := range batch {
				if p.spec.Discard == jobspb.LogicalReplicationDetails_DiscardAllDeletes && len(kv.KeyValue.Value.RawBytes) == 0 {
					continue
				}
				s, err := p.processRow(ctx, txn, b, kv.KeyValue, kv.PrevValue)
				if err != nil {
					return err
				}
				stats.Add(s)
		...
		}, isql.WithSessionData(p.sd))
	}
	return stats, err
}

func (p *kvRowProcessor) processRow(
	ctx context.Context, txn isql.Txn, b *kv.Batch, keyValue roachpb.KeyValue, prevValue roachpb.Value,
) (batchStats, error) {
	var err error
