...
var flushBatchSize = settings.RegisterIntSetting(
	...
	"the number of row updates to attempt in a single KV transaction in validated mode",
	32,
...
var immediateFlushBatchSize = settings.RegisterIntSetting(
	...
	"the number of row updates to attempt in a single KV transaction in immediate mode",
	1,
	settings.NonNegativeInt,
)
...
func (lrw *logicalReplicationWriterProcessor) flushChunk(
	ctx context.Context,
	bh BatchHandler,
	chunk []streampb.StreamEvent_KV,
	canRetry retryEligibility,
	batchSize int,
) (flushStats, error) {
	...
	for len(chunk) > 0 {
		...
		lrw.metrics.ApplyBatchNanosHist.RecordValue(batchTime.Nanoseconds() / int64(len(batch)))
	}
	return stats, nil
}
