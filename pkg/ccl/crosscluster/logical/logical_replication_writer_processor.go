...
func newLogicalReplicationWriterProcessor(
	...
	for _, md := range spec.TableMetadata {
		desc := md.SourceDescriptor
		tableDescs[md.TableId] = tabledesc.NewBuilder(&desc).BuildImmutableTable()
		tableIDToDb[md.TableId] = md.DatabasePrefix
	}
	...
	lrw := &logicalReplicationWriterProcessor{
		...
		dlqClient: InitDeadLetterQueueClient(dlqDbExec, tableIDToDb),
