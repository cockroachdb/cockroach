...
func NewPoller(
	...
	poller.g.GoCtx(func(ctx context.Context) error {
		...
		for {
			...
			laggingInterval := clock.Now().AddDuration(-(time.Hour * 24))
			stats, err := computeRangeStats(ctx, laggingInterval, spans, frontier, ranges)
			if err != nil {
			...
			// TODO remove the info log
			log.Infof(ctx, "publishing range stats: %+v", stats)
