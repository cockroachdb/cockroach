			...
			resultString, err := func() (string, error) {
				...
				case "jwt_cluster_setting":
					for _, a := range td.CmdArgs {
						...
						case "jwks_auto_fetch_enabled":
							if len(a.Vals) != 1 {
								t.Fatalf("wrong number of argumenets to jwt_cluster_setting jwks_auto_fetch_enabled: %d", len(a.Vals))
							}
							v, err := strconv.ParseBool(a.Vals[0])
							if err != nil {
								t.Fatalf("unknown value for jwt_cluster_setting jwks_auto_fetch_enabled: %s", a.Vals[0])
							}
