...
func TestAuthenticateWithSessionRevivalToken(t *testing.T) {
	...
	t.Run("authenticate with token", func(t *testing.T) {
		...
		q := pgURL.Query()
		q.Add("crdb:session_revival_token", token)
	...
	t.Run("use an invalid token", func(t *testing.T) {
		pgURL, cleanup := sqlutils.PGUrl(
			t,
			tenant.SQLAddr(),
			"TestToken",
		...
		q := pgURL.Query()
		q.Add("crdb:session_revival_token", base64.StdEncoding.EncodeToString(badTokenBytes))
		pgURL.RawQuery = q.Encode()
		_, err = pgx.Connect(ctx, pgURL.String())
		require.Contains(t, err.Error(), "session revival token is invalid: signature was not verified")
	})
}
