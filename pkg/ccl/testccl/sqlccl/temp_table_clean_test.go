...
func setupTestSQLLiveness(
	...
	t.Helper()
	slinstance.DefaultTTL.Override(ctx, &settings.SV, 5*sessionDuration)
	slinstance.DefaultHeartBeat.Override(ctx, &settings.SV, sessionDuration)
...
func TestTenantTempTableCleanup(t *testing.T) {
	...
	// Set up a callback so, sessions expire within 10 seconds of a
	// nodes death.
	liveness, livenessStopper := setupTestSQLLiveness(t, ctx, clock, time.Second, settings)

	defer livenessStopper.Stop(ctx)
		...
		base.TestTenantArgs{
			...
			TestingKnobs: base.TestingKnobs{
				SQLLivenessKnobs: &sqlliveness.TestingKnobs{
					SessionOverride: liveness.Session,
				},
			},
		})
	...
	// we will wait for 10 seconds.
	{
		tEnd := timeutil.Now().Add(time.Second * 10)
		found := false
		lastResult := [][]string{}
