...
func TestSQLReconcilerDataDriven(t *testing.T) {
	...
	datadriven.Walk(t, "testdata/", func(t *testing.T, path string) {
		...
		datadriven.RunTest(t, path, func(t *testing.T, d *datadriven.TestData) string {
			...
			case "reconcile":
				// TODO(arul): should this accept an ID instead?
				mustHaveArgOrFatal(t, d, id)

				var objID int
				d.ScanArgs(t, id, &objID)
				spanConfigUpdates, err := ds.sqlReconciler.Reconcile(ctx, descpb.IDs{descpb.ID(objID)})
				if err != nil {
					return err.Error()
				}
				...
				return output.String()
			case "full-reconcile":
				updates, _, err := ds.sqlReconciler.FullReconcile(ctx)
				require.NoError(t, err)
				var output strings.Builder
...
type dataDrivenTestState struct {
	...
	sqlDB         *gosql.DB
	sqlReconciler *spanconfigsqlreconciler.SQLReconciler
}
...
func newDataDrivenTestState(tc serverutils.TestClusterInterface) *dataDrivenTestState {
	...
	sqlReconciler := spanconfigsqlreconciler.New(&execCfg, keys.SystemSQLCodec)
