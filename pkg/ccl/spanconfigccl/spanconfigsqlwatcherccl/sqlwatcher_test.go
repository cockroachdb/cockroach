...
func TestSQLWatcherReactsToUpdates(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			expectedTargets: []ptpb.Target{*ptpb.MakeClusterTarget()},
		},
		{
			stmt: `
...
BACKUP TENANT 2 INTO 'nodelocal://1/foo'`,
			expectedTargets: []ptpb.Target{
				*ptpb.MakeTenantsTarget([]roachpb.TenantID{roachpb.MakeTenantID(2)})},
		},
	}
	...
	var mu struct {
		...
		receivedTargets map[string]int
		lastCheckpoint  hlc.Timestamp
	}
	mu.receivedIDs = make(map[descpb.ID]struct{})
	mu.receivedTargets = make(map[string]int)
			...
			func(ctx context.Context, updates []spanconfig.SQLUpdate, checkpointTS hlc.Timestamp) error {
				...
				for _, update := range updates {
					if update.IsDescriptorUpdate() {
						...
						mu.receivedTargets[ptsUpdate.ClusterOrTenantsTarget.String()]++
					}
				}
				return nil
			})
	...
	for _, tc := range testCases {
		...
		// different keys.
		mu.Lock()
		require.Equal(t, len(tc.expectedTargets), len(mu.receivedTargets))
		...
		for _, target := range tc.expectedTargets {
			count, seen := mu.receivedTargets[target.String()]
			require.True(t, seen)
			// We expect to see every target twice, once on Protect, and again on
			// Release.
			require.Equal(t, 2, count)
			delete(mu.receivedTargets, target.String())
		}
		mu.Unlock()
	}
