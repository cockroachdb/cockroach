...
func prepareNewTableDescsForIngestion(
	...
	// as sqlbase.TableDescriptorInterface.
	tableDescs := make([]sqlbase.TableDescriptorInterface, len(newMutableTableDescriptors))
	for i := range tableDescs {
	...
	err := execCfg.DB.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
		...
		for _, tbl := range details.Tables {
			...
			if !tbl.IsNew {
				...
				for i, c := range newTableDesc.AllActiveAndInactiveChecks() {
					ck := *c
