...
func (r *importResumer) OnFailOrCancel(ctx context.Context, txn *client.Txn) error {
	...
	for _, tbl := range details.Tables {
		...
		tableDesc.Version++
		if tbl.IsNew {
			log.Infof(ctx, "dropping %q via cput  %v \n%v\n -> \n%v\n", tbl.Desc.Name, sqlbase.MakeDescMetadataKey(tableDesc.ID), sqlbase.WrapDescriptor(tbl.Desc), sqlbase.WrapDescriptor(&tableDesc))
			...
			log.Infof(ctx, "putting %q back to public via cput %v \n%v\n -> \n%v\n", tbl.Name, sqlbase.MakeDescMetadataKey(tableDesc.ID), sqlbase.WrapDescriptor(tbl.Desc), sqlbase.WrapDescriptor(&tableDesc))
			tableDesc.Version++
...
func (r *importResumer) OnSuccess(ctx context.Context, txn *client.Txn) error {
	...
	for _, tbl := range details.Tables {
		...
		tableDesc.Version++
		tableDesc.State = sqlbase.TableDescriptor_PUBLIC
		log.Infof(ctx, "publish tbl via cput  %v \n%v\n -> \n%v\n", sqlbase.MakeDescMetadataKey(tableDesc.ID), sqlbase.WrapDescriptor(tbl.Desc), sqlbase.WrapDescriptor(&tableDesc))
		b.CPut(sqlbase.MakeDescMetadataKey(tableDesc.ID), sqlbase.WrapDescriptor(&tableDesc), sqlbase.WrapDescriptor(tbl.Desc))
	}
