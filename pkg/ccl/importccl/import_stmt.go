...
func importPlanHook(
	...
	fn := func(ctx context.Context, _ []sql.PlanNode, resultsCh chan<- tree.Datums) error {
		...
		tableDetails := make([]jobs.ImportDetails_Table, 0, len(tableDescs))
		for _, i := range tableDescs {
			tableDetails = append(tableDetails, jobs.ImportDetails_Table{Desc: i})
		}
		for _, i := range names {
			tableDetails = append(tableDetails, jobs.ImportDetails_Table{Name: i})
		}
