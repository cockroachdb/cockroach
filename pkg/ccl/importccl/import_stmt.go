...
func prepareExistingTableDescForIngestion(
	...
	importing, err := descsCol.GetMutableTableByID(ctx, desc.ID, txn)
	if err != nil {
		return nil, err
	}
...
func (r *importResumer) publishTables(ctx context.Context, execCfg *sql.ExecutorConfig) error {
	...
	err := descs.Txn(ctx, execCfg.Settings, lm, ie, db, func(
		...
		for _, tbl := range details.Tables {
			newTableDesc, err := descsCol.GetMutableTableByID(ctx, tbl.Desc.ID, txn)
			if err != nil {
				return err
			}
...
func (r *importResumer) dropTables(
	...
	for _, tbl := range details.Tables {
		if !tbl.IsNew {
			desc, err := descsCol.GetMutableTableByID(ctx, tbl.Desc.ID, txn)
			if err != nil {
				return err
			}
