...
func importPlanHook(
	...
	fn := func(ctx context.Context, _ []sql.PlanNode, resultsCh chan<- tree.Datums) error {
		...
		if transform == "" {
			var found bool
			var descI tree.SchemaMeta
			if table == nil {
				found, _, descI, err = tree.ResolveCatalog(ctx,
					p, p.SessionData().SearchPath, p.SessionData().Database)
			} else {
		...
		if importStmt.Bundle {
			...
			case roachpb.IOFileFormat_Mysqldump:
				tableDescs, err = readMysqlCreateTable(reader, &p.ExtendedEvalContext().EvalContext, match)
			default:
