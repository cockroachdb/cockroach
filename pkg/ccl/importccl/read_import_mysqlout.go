...
func (d *mysqloutfileReader) readFile(
	...
	addField := func() error {
		...
		if gotNull {
			...
			// This uses ParseStringAsWithRawBytes instead of ParseDatumStringAs since mysql emits
			...
			datum, err := tree.ParseStringAsWithRawBytes(d.conv.VisibleColTypes[len(row)], field, d.conv.EvalCtx)
			if err != nil {
