...
func TestImportCSVStmt(t *testing.T) {
	...
	t.Run("failed-import-gc", func(t *testing.T) {
		...
		if err := kvDB.Txn(ctx, func(ctx context.Context, txn *kv.Txn) (err error) {
			descs, err := catalogkv.GetAllDescriptors(ctx, txn, keys.SystemSQLCodec, true)
			if err != nil {
				return err
			}

			for _, desc := range descs {
				if desc.GetName() == "t" && desc.DescriptorType() == catalog.Table {
					td = desc.(catalog.TableDescriptor)
				}
			}

			if td == nil {
				return errors.Wrapf(catalog.ErrDescriptorNotFound, "could not find descriptor for table %s", "t")
			}
		...
		if err := kvDB.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
			_, err := catalogkv.MustGetTableDescByID(ctx, txn, keys.SystemSQLCodec, td.GetID())
...
func TestImportPgDumpSchemas(t *testing.T) {
	...
	t.Run("inject-error-ensure-cleanup", func(t *testing.T) {
		...
		schemaIDs := []descpb.ID{descpb.ID(dbID + 2), descpb.ID(dbID + 3), descpb.ID(dbID + 4)}
		// The table IDs are allocated after the schemas are created. There is one
		// extra table in the "public" schema.
		tableIDs := []descpb.ID{descpb.ID(dbID + 5), descpb.ID(dbID + 6), descpb.ID(dbID + 7),
			descpb.ID(dbID + 8)}
...
`,
				...
				errString: `type descriptor "typ" \(76\) has been ` +
					`modified, potentially incompatibly, since import planning; ` +
					`aborting to avoid possible corruption`,
				args: []interface{}{srv.URL},
