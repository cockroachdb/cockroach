...
func TestImportIntoCSV(t *testing.T) {
	...
	t.Run("import-into-default", func(t *testing.T) {
		...
		t.Run("sequence-impure", func(t *testing.T) {
			...
			sqlDB.ExpectErr(t,
				fmt.Sprintf("non-literal default expression for non-targeted column .* is not supported by IMPORT INTO"),
	...
	t.Run("import-not-targeted-not-null", func(t *testing.T) {
		...
		defer srv.Close()
		defer sqlDB.Exec(t, `DROP TABLE t`)
		sqlDB.ExpectErr(t, "all non-target columns in IMPORT INTO must be nullable or have default expressions but violated by column b",
			fmt.Sprintf(`IMPORT INTO t (a) CSV DATA ("%s")`, srv.URL),
		)
	})
