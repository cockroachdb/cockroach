...
b STRING) CSV DATA (%s)`, testFiles.files[0])); err != nil {
	...
	t.Run("userfile-simple", func(t *testing.T) {
		...
		data := []byte("1,2")
		require.NoError(t, cloud.WriteFile(ctx, "", bytes.NewReader(data), userfileStorage))
...
func TestImportObjectLevelRBAC(t *testing.T) {
	...
	writeToUserfile := func(filename string) {
		...
		require.NoError(t, err)
		require.NoError(t, cloud.WriteFile(ctx, filename, bytes.NewReader([]byte("1,aaa")), fileTableSystem1))
	}
...
func benchUserUpload(b *testing.B, uploadBaseURI string) {
	...
	if uri.Scheme == "nodelocal" {
		...
		require.NoError(b, err)
		err = cloud.WriteFile(ctx, "", bytes.NewReader(content), userfileStorage)
		require.NoError(b, err)
