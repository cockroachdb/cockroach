...
func TestImportDefaultWithResume(t *testing.T) {
	...
	for _, test := range testCases {
		t.Run(test.name, func(t *testing.T) {
			...
			emptySequenceDetails := make([]*jobspb.SequenceDetails, 1)

			// Job resumption resets the progress.
			emptySequenceDetails[0] = &jobspb.SequenceDetails{}
			require.Equal(t, emptySequenceDetails, seqDetailsOnSuccess)
