...
const (
	SourceAddress tree.AnnotationIdx = iota + 1
	RowAddress
	ColumnAddress
)

func getAddressAnnotations(ann tree.Annotations) (int, int, int) {
	return ann.Get(SourceAddress).(int), ann.Get(RowAddress).(int), ann.Get(ColumnAddress).(int)
}
	...
	"random": makeBuiltinOverride(
		...
		tree.Overload{
			...
			Fn: func(evalCtx *tree.EvalContext, args tree.Datums) (tree.Datum, error) {
				source, rowIdx, colIdx := getAddressAnnotations(*evalCtx.Annotations)
				return tree.NewDFloat(tree.DFloat(float32(source+rowIdx+colIdx) / 1000)), nil
...
func TestFunWithVolatility(t *testing.T) {
	...
	testCases := []struct {
		...
		// 	expr: "(100*random())::int + current_timestamp()::int",
		// 	typ:  expectErrorType,
		// },
	}

	ann := tree.MakeAnnotations(3)
	evalCtx.Annotations = &ann
	ctx := context.Background()

	for idx, tc := range testCases {
		ann.Set(SourceAddress, idx+10)
		ann.Set(RowAddress, idx+20)
		ann.Set(ColumnAddress, idx+30)
		t.Run(tc.name, func(t *testing.T) {
