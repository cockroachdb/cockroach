...
func (sp *csvWriter) Run(ctx context.Context) {
	...
	err := func() error {
		...
		for {
			...
			// we need to close the writer here before we store content written so far
			// to the external storage, cause it also appends archive footer to the content
			// in case compression was enabled
			writer.Close()
