...
func readInputFiles(
	...
	getResumePos := func(idx int32) int64 {
		if resumePos == nil {
			return 0
		}
		return resumePos[idx]
	}
		...
		if err := func() error {
			...
			if rejected != nil {
				...
				grp.GoCtx(func(ctx context.Context) error {
					...
					if err := fileFunc(ctx, src, dataFileIndex, dataFile, getResumePos(dataFileIndex), rejected); err != nil {
				...
				if err := fileFunc(ctx, src, dataFileIndex, dataFile, getResumePos(dataFileIndex), nil /* rejected */); err != nil {
					return errors.Wrap(err, dataFile)
				}
			}
			return nil
