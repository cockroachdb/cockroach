...
func getSimplePostgresDumpTestdata(t *testing.T) ([]simpleTestRow, string) {
	dest := filepath.Join(testutils.TestDataPath(), `pgdump`, `simple.sql`)
...
func getSecondPostgresDumpTestdata(t *testing.T) (int, string) {
	dest := filepath.Join(testutils.TestDataPath(), `pgdump`, `second.sql`)
	if rewritePostgresTestData {
...
func getMultiTablePostgresDumpTestdata(t *testing.T) string {
	dest := filepath.Join(testutils.TestDataPath(), `pgdump`, `db.sql`)
	if rewritePostgresTestData {
...
func getPgCopyTestdata(t *testing.T) ([]simpleTestRow, []pgCopyDumpCfg) {
	...
	for i := range configs {
		configs[i].filename = filepath.Join(testutils.TestDataPath(), `pgcopy`, configs[i].name, `test.txt`)
	}

	if rewritePostgresTestData {
		genSimplePostgresTestdata(t, func() {
			if err := os.RemoveAll(filepath.Join(testutils.TestDataPath(), `pgcopy`)); err != nil {
				t.Fatal(err)
			}
