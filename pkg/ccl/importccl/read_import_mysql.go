...
func mysqlTableToCockroach(
	...
	if seqDesc != nil {
		return []*sqlbase.TableDescriptor{seqDesc, desc.TableDescriptor}, fkDefs, nil
	}
	return []*sqlbase.TableDescriptor{desc.TableDescriptor}, fkDefs, nil
}
...
func addDelayedFKs(ctx context.Context, defs []delayedFK, resolver fkResolver) error {
	for _, def := range defs {
		...
		if err := fixDescriptorFKState(def.tbl.TableDescriptor); err != nil {
			return err
		}
