...
func makeSSTs(
	...
	if sst.DataSize > 0 {
		// Although we don't need to avoid row splitting here because there aren't
		// any more keys to read, we do still want to produce the same kind of
		// lastKey argument for the span as in the case above.
		lastKey, err = keys.EnsureSafeSplitKey(kv.Key.Key)
		if err != nil {
			return err
		}
