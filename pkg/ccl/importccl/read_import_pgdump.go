...
func readPostgresCreateTable(
	input io.Reader, evalCtx *tree.EvalContext, settings *cluster.Settings, match string, parentID sqlbase.ID, walltime int64,
	...
	for {
		...
		if err == io.EOF {
			...
			for name, create := range createTbl {
				if match == "" || match == name {
...
func (m *pgDumpReader) readFile(
	...
	for {
		...
		case *tree.Insert:
			...
			startingCount := count
			_ = startingCount
			switch i := i.Rows.Select.(type) {
			case *tree.ValuesClause:
				for _, tuple := range i.Tuples {
					count++
			...
			default:
				return errors.Errorf("unsupported: %s", i)
			}
		default:
			if log.V(3) {
