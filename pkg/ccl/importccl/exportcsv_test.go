...
import (
	...
	//goparquet "github.com/fraugster/parquet-go"
	_ "github.com/fraugster/parquet-go/parquet"
	_ "github.com/fraugster/parquet-go/parquetschema"
...
func validateParquetFile(t *testing.T, file string, truthRows [][]interface{}) error {
	...
	fmt.Printf("Schema: %s", fr.GetSchemaDefinition())
	...
	for {
		...
		if int64(count) > fr.NumRows(){
			return fmt.Errorf("more row than accorrding to metadata")
		}
