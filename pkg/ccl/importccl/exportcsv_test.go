...
type parquetTest struct {
	...
	// nullableCols provides the expected nullability of each column in the parquet file.
	nullableCols []parquet.FieldRepetitionType
...
func validateParquetFile(t *testing.T, file string, test parquetTest) error {
	...
	if test.nullableCols != nil {
		for i, col := range cols {
			require.Equal(t, *col.SchemaElement.RepetitionType, test.nullableCols[i])
		}
	}
