...
// syncBarrier allows 2 threads (a controller and a worker) to synchronize between themselves.
// A controller portion of the barrier waits until worker starts running, and then notifies worker to proceed.
// The worker is the opposite: notifies controller that it started running, and waits for the proceed signal.
type syncBarrier interface {
	// Enter blocks the barrier, and returns a function that, when executed, unblocks the other thread.
	Enter() func()
}
...
// A special jobs.Resumer that, instead of finishing the job successfully, forces the job to be paused.
...
func (r *cancellableImportResumer) OnFailOrCancel(ctx context.Context, txn *client.Txn) error {
	// This callback is invoked when an error or cancellation occurs during the import.
	// Since our OnSuccess handler returned an error (after pausing the job), we need to short-circuits jobs machinery
	// so that this job is not marked as failed.
...
func TestCSVImportMarksFilesFullyProcessed(t *testing.T) {
	...
	// All files should have been processed, and the resume position set to maxInt64.
	js := queryJobUntil(t, sqlDB.DB, jobID, func(js jobState) bool { return jobs.StatusPaused == js.status })
