...
func (a *ensureMinKeyAdder) SetOnFlush(f func()) {
	a.onFlush = f
}
		...
		for _, resumePos := range resumes {
			...
			t.Run(fmt.Sprintf("resume-%v-%v", spec.Format.Format, resumePos), func(t *testing.T) {
				...
				go func() {
					...
					// then the progress resport must be emitted every batchSize rows (possibly out of order),
					// starting from our initial resumePos
					for prog := range progCh {
						if !t.Failed() && prog.ResumePos[0] < (resumePos+int64(batchSize)) {
...
func newTestSpec(
	...
	case roachpb.IOFileFormat_Mysqldump, roachpb.IOFileFormat_PgDump, roachpb.IOFileFormat_PgCopy:
		descr = descForTable(t,
