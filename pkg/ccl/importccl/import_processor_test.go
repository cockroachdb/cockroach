...
func TestImportIgnoresProcessedFiles(t *testing.T) {
	...
	setInputOffsets := func(
		t *testing.T, spec *execinfrapb.ReadImportDataSpec, offsets []uint64) *execinfrapb.ReadImportDataSpec {
...
func newTestSpec(
	...
	// Add input files to convert.
	basePath, err := os.Getwd()
	if err != nil {
		t.Fatalf("Failed to get working dir: %v", err)
	}
	for id, path := range inputs {
		if spec.inputs[int32(id)], err = cloud.MakeLocalStorageURI(filepath.Join(basePath, path)); err != nil {
