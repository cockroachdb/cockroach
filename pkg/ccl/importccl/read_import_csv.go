...
func (c *csvInputReader) startConverterWorkers(group ctxgroup.Group) {
	workerID := struct {
		syncutil.Mutex
		id int
	}{}

	nextWorkerID := func() int {
		workerID.Lock()
		defer workerID.Unlock()
		id := workerID.id
		workerID.id++
		return id
	}
	...
	group.GoCtx(func(ctx context.Context) error {
		...
		return ctxgroup.GroupWorkers(ctx, c.parallelism, func(ctx context.Context) error {
			return c.convertRecordWorker(ctx, nextWorkerID())
		})
	})
}
...
// Updates emitted row for the specified worker and returns low watermark for the emitted rows across all workers.
