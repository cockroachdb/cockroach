...
func (w *workloadReader) readFiles(
	...
	var finishedBatchesAtomic, lastReportedBatchesAtomic int64
	finishedBatchFn := func() {
		if finishedBatches := atomic.AddInt64(&finishedBatchesAtomic, 1); finishedBatches%1000 == 0 {
			lastReportedBatches := atomic.SwapInt64(&lastReportedBatchesAtomic, finishedBatches)
			progressDelta := float32(finishedBatches-lastReportedBatches) / float32(numTotalBatches)
