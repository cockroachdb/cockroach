...
func (c *parquetExporter) Bytes() []byte { // IDENTICAL TO CSV
	return c.buf.Bytes()
}
...
// parquetColumn contains the relevant data to map crdb table column to a parquet table column
type parquetColumn struct {
	...
	//encodeFn converts crbd table column value to a native go type. It's called by the processor.
	encodeFn func(datum tree.Datum) (interface{}, error)
}

// newParquetExporter allocates a buffer, creates a new parquet file writer,
// defines the parquet file schema, and packs all this info into a new parquetExporter
func newParquetExporter(
	...
	for i := 0; i < len(typs); i++ {
		name := fmt.Sprintf("Col%d", i) // TODO(mbutler): figure out how to pass column names to export processor
...
func newParquetColumn(typ *types.T, name string) (parquetColumn, error) {
	...
	default:
		return col, errors.Errorf("parquet export does not support type yet")
	}
...
func (sp *parquetWriterProcessor) Run(ctx context.Context) {
	...
	err := func() error {
		...
		// TODO: Deal with NULL VALUES pt 1!!!
		/*var nullsAs string
		...
		for {
			var rows int64
			// MB: a big difference from csv: for parquet, it's easiest to create a
			// new ParquetExporter each time the processor creates a new parquet file,
			// as the ParquetExporter contains metadata specific to that file
			...
			for {
				...
				for i, ed := range row {
					// TODO: Deal with NULL VALUES pt 2!!!
					/*if ed.IsNull() {
						if sp.spec.Options.NullEncoding != nil {
							parquetRow[colname] = &nullsAs // slightly diff from csv
							continue
						} else {
