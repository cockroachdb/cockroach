	...
	*/
	...
	case types.ArrayFamily:
		...
		*/
		...
		// Next define the child definition, required by the parquet vendor.
		child := new(parquetschema.ColumnDefinition)
		...
		col.encodeFn = func(d tree.Datum) (interface{}, error) {
			...
			for _, elt := range datumArr.Array {
				var el interface{}
				if elt.ResolvedType().Family() == types.UnknownFamily {
					el = nil
					// skip populating the encoded array value if the datum is dNULL.
				} else {
				...
				els = append(els, map[string]interface{}{"element": el})
			}
