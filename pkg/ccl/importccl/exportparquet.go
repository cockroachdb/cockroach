...
type ParquetColumn struct {
	...
	// DecodeFn converts a native go type read from a parquet file into a crdb column value
	...
	*/
	...
	case types.FloatFamily:
		/*	if typ.Oid() == oid.T_float4 {
				...
				col.DecodeFn = func(x interface{}) (tree.Datum, error) {
					h := float64(x.(float32))
					// converting float32 to 64 is really quite annoying.
					return tree.NewDFloat(tree.DFloat(h)), nil
				}
			} else {
	...
	case types.DateFamily:
		...
		col.DecodeFn = func(x interface{}) (tree.Datum, error) {
			...
			if dependCtx {
				return nil, fmt.Errorf("decoding date %s failed. depends on context", string(x.([]byte)))
			}
			return d, err
		}
	...
	case types.TimeTZFamily:
		...
		col.DecodeFn = func(x interface{}) (tree.Datum, error) {
			...
			if dependsOnCtx {
				return nil, fmt.Errorf("parsed ime depends on context")
			}
			return d, err
		}
	case types.IntervalFamily:
		// The parquet vendor actually supports intervals as a parquet converted type,
	...
	case types.TimestampFamily:
		// Didn't encode this as Microseconds ( original attempt commented out)
		// since the unix epoch because of threat of overflow. See comment
		// associated with time.Time.UnixMicro().
		populateLogicalStringCol(schemaEl)

		/*schemaEl.Type = parquet.TypePtr(parquet.Type_INT64)
		  schemaEl.LogicalType = parquet.NewLogicalType()
		  schemaEl.LogicalType.TIMESTAMP = parquet.NewTimestampType()
		  t := parquet.NewTimeUnit()
		  t.MICROS = parquet.NewMicroSeconds()
		  schemaEl.LogicalType.TIMESTAMP.Unit = t
		  schemaEl.LogicalType.TIMESTAMP.IsAdjustedToUTC = true*/
		...
		*/
		...
		col.DecodeFn = func(x interface{}) (tree.Datum, error) {
			...
			if _, nonEmpty := vals[0]["element"]; !nonEmpty {
				if len(vals) > 1 {
					return nil, fmt.Errorf("array is empty, it shouldn't have a length greater than 1")
				}
			} else {
