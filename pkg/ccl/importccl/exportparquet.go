...
func newParquetColumns(typs []*types.T, sp execinfrapb.ParquetWriterSpec) ([]parquetColumn, error) {
	...
	for i := 0; i < len(typs); i++ {
		parquetCol, err := newParquetColumn(typs[i], sp.LogicalColNames[i], sp.NullableCols[i])
		if err != nil {
			return nil, err
		}
...
// MustBeStreaming currently never gets called by the parquetWriterProcessor as the function only
