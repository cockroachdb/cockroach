...
func (sp *parquetWriterProcessor) Run(ctx context.Context) {
	...
	err := func() error {
		...
		// allocate memory for a bunch of objects that will be recycled every time we create a new
		// parquetExporter in the loop below
		...
		for {
			...
			buf.Reset()
			exporter, err := newParquetExporter(sp.spec, buf, schema, parquetColumns)
			if err != nil {
				return err
			}
