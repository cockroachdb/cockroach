...
type (
	// A breakpoint handler runs some breakpoint specific logic.
	// A handler returns a boolean indicating if this breakpoint should remain active.
	// An error result will cause csv generation to stop and the csv server to return that error.
	csvBpHandler  = func() (bool, error)
	csvBreakpoint struct {
		row     csvRow
		enabled bool
	...
	csvGenerator struct {
		...
		rowOffset int // Offset withing the current row
	}
)
...
// Note: we read one row at a time because reading as much as the buffer space allows might cause problems
// for breakpoints (e.g. a breakpoint may block until job progress is updated, but that update would never
// happen because we're still reading the data).
...
// Creates a new instance of csvGenerator, generating 'numRows', starting form the specified 'startRow'.
// The 'columns' list specifies data generators for each column.
// The optional 'breakpoints' specifies the list of csv breakpoints which allow caller to execute some code
// while generating csv data.
...
// generatorExternalStorage is an external storage implementation that returns its data from the underlying generator.
...
// generatedStorage is a factory (cloud.ExternalStorageFactory) that returns the underlying csvGenerators
// The file name of the generated file doesn't matter: the first time we attempt to get a generator for a file,
// we return the next generator on the list.
