...
func (s *kafkaSink) buildKafkaConfig(u sinkURL, opts map[string]string) (*sarama.Config, error) {
	dialConfig := struct {
		tlsEnabled    bool
		tlsSkipVerify bool
		caCert        []byte
	...
	if tlsBool := u.consumeParam(changefeedbase.SinkParamTLSEnabled); tlsBool != `` {
		var err error
		if dialConfig.tlsEnabled, err = strconv.ParseBool(tlsBool); err != nil {
			return nil, errors.Errorf(`param %s must be a bool: %s`, changefeedbase.SinkParamTLSEnabled, err)
		}
	}
	if tlsVerifyBool := u.consumeParam(changefeedbase.SinkParamSkipTLSVerify); tlsVerifyBool != `` {
		var err error
		if dialConfig.tlsSkipVerify, err = strconv.ParseBool(tlsVerifyBool); err != nil {
	...
	saslParam := u.consumeParam(changefeedbase.SinkParamSASLEnabled)
	if saslParam != `` {
		b, err := strconv.ParseBool(saslParam)
		if err != nil {
			return nil, errors.Wrapf(err, `param %s must be a bool:`, changefeedbase.SinkParamSASLEnabled)
		}
		dialConfig.saslEnabled = b
	}

	handshakeParam := u.consumeParam(changefeedbase.SinkParamSASLHandshake)
	...
	if dialConfig.tlsEnabled {
		...
		config.Net.TLS.Config.InsecureSkipVerify = dialConfig.tlsSkipVerify
	}
	...
	} else if dialConfig.clientKey != nil {
...
func makeKafkaSink(
	...
	config, err := sink.buildKafkaConfig(u, opts)
	if err != nil {
		return nil, err
	}

	if unknownParams := u.remainingQueryParams(); len(unknownParams) > 0 {
		return nil, errors.Errorf(`unknown kafka sink query parameters: %s`, unknownParams)
	}
