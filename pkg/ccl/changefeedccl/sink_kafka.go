...
type kafkaSink struct {
	...
	// require access to the lock however if an internal retry process is
	// triggered it will remain locked until retries are completed
	clientMu struct {
		syncutil.Mutex
		client    kafkaClient
...
func (s *kafkaSink) workerLoop() {
	...
	startInternalRetry := func(err error) {
		s.mu.AssertHeld()
		log.Infof(s.ctx, "kafka sink with flush config (%+v) beginning internal retry due to error: %s", s.kafkaCfg.Producer.Flush, err.Error())
		retryErr = err
	...
	shouldLogConfig := log.Every(2 * time.Hour)

	for {
		var ackMsg *sarama.ProducerMessage
		var ackError error

		// Log the active config every so often as it may be different from the
		// configured value if internal retries had ever been applied.
		if shouldLogConfig.ShouldLog() {
			log.Infof(s.ctx, "kafka sink worker processing with flush config: (%+v)", s.kafkaCfg.Producer.Flush)
		}
		...
		if retryBuf != nil && ackMsg != nil && ackMsg.Key != nil && ackMsg.Value != nil {
...
retryLoop:
	for {
		...
		if !isInternalRetryable(lastSendErr) || wasReduced {
			s.mu.sendErr = lastSendErr
			break
		}
		s.metrics.recordBatchReduction()
...
func reduceBatchingConfig(c *sarama.Config) (*sarama.Config, bool) {
	...
	// MaxMessages of 0 means unlimited, so treat "halving" it as reducing to 1000
	if c.Producer.Flush.MaxMessages == 0 {
		newConfig.Producer.Flush.MaxMessages = 1000
	} else {
