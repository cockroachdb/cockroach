...
func buildConfluentKafkaConfig(u sinkURL) (kafkaDialConfig, error) {
	...
	newRequiredValueError := func(param string, value string) error {
		return errors.Newf("scheme %s requires parameter %s=%s", changefeedbase.SinkSchemeConfluentKafka, param,
			value)
	}
	...
	if wasSet, err := u.consumeBool(changefeedbase.SinkParamSASLEnabled, &dialConfig.saslEnabled); err != nil {
		...
		return kafkaDialConfig{}, newRequiredValueError(changefeedbase.SinkParamSASLEnabled, "true")
	}
	...
	if dialConfig.saslMechanism = u.consumeParam(changefeedbase.SinkParamSASLMechanism); dialConfig.saslMechanism != `` &&
		dialConfig.saslMechanism != sarama.SASLTypePlaintext {
		return kafkaDialConfig{}, newRequiredValueError(changefeedbase.SinkParamSASLMechanism, sarama.SASLTypePlaintext)
	}
	...
	if wasSet, err := u.consumeBool(changefeedbase.SinkParamTLSEnabled, &dialConfig.tlsEnabled); err != nil {
		...
		return kafkaDialConfig{}, newRequiredValueError(changefeedbase.SinkParamTLSEnabled, "true")
	}
	...
	if wasSet, err := u.consumeBool(changefeedbase.SinkParamSASLHandshake, &dialConfig.saslHandshake); err != nil {
		...
		return kafkaDialConfig{}, newRequiredValueError(changefeedbase.SinkParamSASLHandshake, "true")
	}
