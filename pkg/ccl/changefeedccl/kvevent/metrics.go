...
type whichBuffer string

const (
	rangefeedBuffer whichBuffer = "rangefeed"
	kvFeedBuffer    whichBuffer = "kvfeed"
)
...
func MakeMetrics(histogramWindow time.Duration) Metrics {
	// HACK: we don't want to rely on labels for this, since tsdb doesn't support them. TODO: do we?
	metaForBuf := func(which whichBuffer, meta metric.Metadata) metric.Metadata {
		meta.Name = fmt.Sprintf("%s.%s", meta.Name, which)
		return meta
	}
