...
// sinkSupportsExactlyOnceExport returns true if the sink supports exactly once
// export semantics.  Exactly once is available for a subset of cloud storage sink.
func sinkSupportsExactlyOnceExport(uriScheme string) bool {
	switch uriScheme {
	case changefeedbase.SinkSchemeCloudStorageS3, changefeedbase.SinkSchemeCloudStorageGCS,
		changefeedbase.SinkSchemeCloudStorageNodelocal, changefeedbase.SinkSchemeCloudStorageAzure:
		return true
	default:
		return false
	}
...
type cloudStorageSink struct {
	...
	// exactlyOnce configured via configureExactlyOnceSemantics() enables
	// this sink functionality to support exactly once semantics during export.
	exactlyOnce struct {
		enabled     bool
		partitionID int32
		flushGen    flushGenerationOracle
	}
...
func (s *cloudStorageSink) Flush(ctx context.Context) error {
	...
	if s.exactlyOnce.enabled {
		s.exactlyOnce.flushGen.inc()
	}
	return nil
}
...
func (s *cloudStorageSink) flushFile(ctx context.Context, file *cloudStorageSinkFile) error {
	...
	if s.exactlyOnce.enabled {
		filename = s.exactlyOncePath(filename)
	}
...
// configureExactlyOnceSemantics configures this sink to ensure that keys are
// written exactly once, and that, in case of a restart, previously written
// files are removed prior to being rewritten. This configuration only makes
// sense in case of export mode changefeed where we wish to provide exactly once
// semantics.
//
// The way that this works is that it is required that the export employs fixed
// partition scheme -- decided when changefeed export created. Subsequently,
// each restart will use the same partition ID. Furthermore, the sink keeps
// track of flush generation sequence. This generation number is increment every
// time explicit Flush is triggered by the caller. The caller may persist
// checkpoints, and it must persist written flush generation sequence.
// When exactly once semantics enabled, each file generated by this sink
// will be placed in a "subdirectory" <partitionID>~<flushGen>/
// If the export is restarted, the all files within this partition with flush
// generation greater or equal to the specified flushGeneration will be removed.
func (s *cloudStorageSink) configureExactlyOnceSemantics(
	ctx context.Context, partitionID int32, flushGen flushGenerationOracle,
) error {
	s.exactlyOnce.enabled = true
	s.exactlyOnce.partitionID = partitionID
	s.exactlyOnce.flushGen = flushGen

	// Delete files generated prior to this restart.
	flushGenStr := flushGenerationStr(flushGen.get())
	prefix := fmt.Sprintf("%s%s~", s.dataFilePartition, partitionIDStr(partitionID))

	numFilesRemoved := 0
	if log.V(1) {
		log.Infof(ctx, "removing files from partition %d with %d or higher generation (%s)",
			partitionID, flushGen.get(), prefix)
		defer func() {
			log.Infof(ctx, "removed %d files from partition %d with %d or higher generation (%s)",
				numFilesRemoved, partitionID, flushGen.get(), prefix)
		}()
	}

	return s.es.List(ctx, prefix, "", func(p string) error {
		// The first part of the path, prior to "/" will be the generation number
		// for this partition.
		dir, _ := path.Split(p)
		// Hex values can be compared alphanumerically.
		if strings.Compare(dir, flushGenStr) >= 0 {
			p = prefix + p
			if log.V(2) {
				log.Infof(ctx, "removing file %s", p)
			}
			if err := s.es.Delete(ctx, p); err != nil && !errors.Is(err, cloud.ErrFileDoesNotExist) {
				return err
			}
			numFilesRemoved++
		}
...
func (s *cloudStorageSink) exactlyOncePath(p string) string {
	prefix := fmt.Sprintf("%s~%s",
		partitionIDStr(s.exactlyOnce.partitionID),
		flushGenerationStr(s.exactlyOnce.flushGen.get()))
...
func partitionIDStr(p int32) string {
	return fmt.Sprintf("%08x", p)
}

func flushGenerationStr(g int64) string {
	return fmt.Sprintf("%08x", g)
}
