...
func (s *cloudStorageSink) flushTopicVersions(
	...
	s.files.AscendRange(gte, lt, func(i btree.Item) (wantMore bool) {
		...
		err = s.flushFile(ctx, f)
		return err == nil
	})
	if err != nil {
		return err
	}
	// The btree is not robust to mutations during iteration so the below logic
	// deletes all entries from files which were just flushed.
	key := gte
	for called := true; called && key.schemaID <= maxVersionToFlush; {
		called = false
		s.files.AscendRange(key, lt, func(i btree.Item) (wantMore bool) {
			f := i.(*cloudStorageSinkFile)
			key, called = f.key(), true
			return false
		})
		s.files.Delete(key)
	}
...
func (s *cloudStorageSink) Flush(ctx context.Context) error {
	...
	s.files.Clear(false /* addNodesToFreeList */)
...
func (k cloudStorageSinkKey) key() cloudStorageSinkKey {
	return k
}

func (k cloudStorageSinkKey) Less(other btree.Item) bool {
	return keyerLess(k, other.(keyer))
}
...
func keyerLess(ak, bk keyer) bool {
	a, b := ak.key(), bk.key()
	if a.topic == b.topic {
