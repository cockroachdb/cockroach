...
type cloudStorageSinkFile struct {
	// `minTs` is the local spanFrontier's Frontier().Next() timestamp as of
...
// format). This mapping is stable for a given distsql flow of a changefeed
// (meaning any given row is always emitted by the same processor), but it's not
// stable across restarts (pause/unpause). Each of these processors report partial
// progress information to a central coordinator (changeFrontier), which is
// responsible for writing the resolved timestamp files.
//
...
// for comparison with resolved timestamp filenames as part of guarantee.
// b) For each topic, the row ordering guarantees must be preserved.
// One intuitive way of solving (b) is to ensure that filenames are lexically
// ordered the way they're emitted. This guarantees correctness as long as
// the underlying system is correct.
//
// Before the local progress is sent to the coordinator (called the "local
// frontier" as opposed to the resolved timestamp which is exactly a "global
// frontier" or "changefeed-level frontier"), all buffered data which preceded
// that update is flushed. To accomplish (a), we need two invariants. First (a1)
// is that once Flush is called, we can never write a file with a timestamp that
// is less than or equal to the local frontier. This is because the local
// progress update could indeed cause a resolved timestamp file to be written
// with that timestamp. If this happens, the client is free to ignore any files
// with a lesser filename. We picked the resolve timestamp filename to sort
// after a data file with the same timestamp, so it can't even be at the same
// timestamp, it has to be strictly greater. (Note that the local frontier is a
// guarantee that the sink will never get an EmitRow with that timestamp or
// lower). Second, (a2) is that whenever Flush is called, all files written by
// the sink must be named using timestamps less than or equal to the one for the
// local frontier at the time Flush is called. This is again because our local
// progress update could cause the global progress to be updated and we need
// everything written so far to lexically compare as less than the new resolved
// timestamp.
//
...
// `changeAggregator`, as of the time the cloudStorageSinkFile is created in
// memory. Intuitively, this can be thought of as an inclusive lower bound on
// the timestamps of updates that can be seen in a given file.
//
...
// `<node_id>-<sink_id>-<file_id>-<session_id>` where `<sink_id>` is a unique id
// for each cloudStorageSink in a running process, `<file_id>` is a unique id
// for each file written by a given `<sink_id>` and <session_id> is a unique
// identifying string for the job session running the `changeAggregator` that
// owns this sink.
//
...
// 2. Ai, for i in integers, refers to `the filename of the i'th data file
...
// integers, for all job sessions X. The naming convention described above guarantees
// this.
// 3. Data files are named 1 logical timestamp higher than the "local frontier"
// timestamp as of the time the file starts buffering in memory. Since all
// EmitRow calls are guaranteed to be for rows that equal or succeed this
// timestamp, ts(Xi) is an inclusive lower bound for the rows contained inside
// Xi.
...
//
// Lemma 1: Given two totally ordered sets of files X and Y that preserve CDC's
// ordering guarantee along with invariants 3 and 4, their union produces a
// totally ordered set of files that preserves this guarantee. Proof of lemma:
// Lets refer to the data filenames emitted by these sessions as X1,X2.... and
// similarly for session Y. Additionally, lets refer to the last file ever
// emitted by session X as Xn, for some n in integers. Now lexically speaking
// there are 2 cases here: 1. Y1 < Xn: For the sake of contradiction, let's
// assume there is a violation here. Since there is a total lexical ordering
// among files in each set individually, we must have read Y(e-1) before Ye, for
// all e in integers. Similarly for X. Without loss of generality, lets say
// there are 2 files Ye and Xf such that (1.1) Ye < Xf such that Xf contains an
// unseen timestamp that is lower than at least one timestamp seen in Ye. Let's
// call this timestamp t. More explicitly, it must be the case that this
// timestamp t does not exist in any files Y1...Ye. This must mean that
// timestamp t lies before the starting point of session Y's catch-up scan
// (again from invariant 4). Thus it must be the case that (1.2) ts(Y1) > t.
// Now, due to invariant 3, we know that we won't see any rows in a file Xi with
// a timestamp that is lower than ts(Xi). This must mean that (1.3) ts(Xf) <= t.
// Statements 1.1, 1.2 and 1.3 together give us a contradiction. 2. Y1 > Xn.
// This case means that all data files of session Y lexically succeed all the
// data files of session X. This means that all data files are ordered
// monotonically relative to when they were emitted, this gives us invariant 2
// (but for 2 sessions). Correctness follows from this and invariant 1. Note
// that Y1 == Xn is not possible because sessions are assigned unique session
// IDs. QED.
//
// Proof of correctness: It is impossible to see a previously unseen timestamp
// that is lower than any timestamp seen thus far, across n job sessions for all
// n, n in integers. We do this by induction, let k be the number of job
// sessions a changefeed job goes through: Case k = 1: Correctness for this case
// follows from corollary 1. Case k = 2: This follows from lemma 1 stated above.
// Case k > 2 (induction case): Assume that the statement of the proof is true
// for the output of a changefeed job with k sessions. We will show that it must
// also be true for the output of a changefeed job with k+1 sessions. Let's
// refer to the first k jobs as P1,P2,..PK, and the k+1st job as Q. Now, since
// we assumed the statement is true for P1,P2...PK, it must produce a totally
// ordered (lexical ordering) set of files that satisfies requirements of lemma
// 1. So we can consider these k jobs conceptually as one job (call it P). Now,
...
func (s *cloudStorageSink) EmitRow(
	...
	if file == nil {
		...
		minTs := s.sf.Frontier().Next()
		file = &cloudStorageSinkFile{minTs: minTs}
		s.files[key] = file
	}
...
func (s *cloudStorageSink) flushFile(
	...
	part := file.minTs.GoTime().Format(s.partitionFormat)
	ts := cloudStorageFormatTime(file.minTs)
	...
	if s.prevFileTs.Less(file.minTs) {
		// Reset fileID if the local frontier has advanced.
		s.fileID = 0
	} else if s.prevFileTs.Equal(file.minTs) {
		s.fileID++
	} else {
		// This should never happen since the local span frontier should never
		// regress.
		return errors.AssertionFailedf("error: detected a file timestamp %s"+
			"that is lower than the timestamp of a previously emitted file: %s.",
			file.minTs.String(), s.prevFileTs.String())
	}
	fileID := s.fileID
