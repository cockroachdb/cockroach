...
// `$<version_id>-<session_id>-<node_id>-<sink_id>-<file_id>` where `<sink_id>` is a unique id for each
// cloudStorageSink in a running process, `<file_id>` is a unique id for each file written
// by a given `<sink_id>`,  <session_id> is a unique identifying string for the job
// session running the `changeAggregator` that owns this sink, and <version_id> is a unique
// incrementing identifier for the version of the filename scheme, to allow for schema evolution
...
func makeCloudStorageSink(
	...
	s := &cloudStorageSink{
		...
		includeMinMVCCInFilename: includeMinMVCCInFilename.Get(&settings.SV),
		testingKnobs:             testingKnobs,
	}
...
var includeMinMVCCInFilename = settings.RegisterBoolSetting(
	settings.ApplicationLevel,
	"changefeed.cloudstorage.include_min_mvcc_in_filename.enabled",
	"include minimum MVCC timestamp in cloudstorage data filenames",
	true,
)
...
func (s *cloudStorageSink) flushFile(ctx context.Context, file *cloudStorageSinkFile) error {
	...
	var uniquer string
	if s.includeMinMVCCInFilename {
		uniquer = fmt.Sprintf(`%03d-%s-%s-%d-%d-%08x`, s.nameSchemeVersion, cloudStorageFormatTime(file.oldestMVCC), s.jobSessionID, s.srcID, s.sinkID, fileID)
	} else {
