...
// `<now_timestamp>` is the current timestamp in nanoseconds,
// `<sink_id>`is a unique id for each cloudStorageSink in a running process,
...
// `changeAggregator` that owns this sink, and `<version_id>` is a unique
// incrementing identifier for the version of the filename scheme, to allow for
// schema evolution without breaking ordering.
//
