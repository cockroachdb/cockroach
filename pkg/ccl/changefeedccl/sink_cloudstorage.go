...
func (s *cloudStorageSink) configureExactlyOnceSemantics(
	ctx context.Context, partitionID int32, flushGen int64,
) error {
	...
	flushGenStr := flushGenerationStr(flushGen)
...
func (s *cloudStorageSink) exactlyOncePath(p string) string {
	prefix := fmt.Sprintf("%s~%s", partitionIDStr(s.exactlyOnce.partitionID), flushGenerationStr(s.exactlyOnce.flushGen))
