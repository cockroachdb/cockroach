...
// that update is flushed. To accomplish (a), we need two invariants.
// (a1) is that once Flush is called, we can never write a file with a timestamp that
...
// with that timestamp. We cannot break this invariant because the client is free
// to ignore any files with a lexically lesser filename. Additionally, because
// we picked the resolved timestamp filename to sort after a data file with the
// same timestamp, a data file can't even be emitted at the same timestamp, it must
// be emitted at a timestamp that is strictly greater than the last globally resolved
// timestamp. Note that the local frontier is a guarantee that the sink will never
// get an EmitRow with that timestamp or lower.
// (a2) is that whenever Flush is called, all files written by the sink must
// be named using timestamps less than or equal to the one for the local frontier
// at the time Flush is called. This is again because our local progress update
// could cause the global progress to be updated and we need everything written
// so far to lexically compare as less than the new resolved timestamp.
//
...
// this.
// 3. Data files are named using the successor of the "local frontier"
...
// emitted by session X as Xn, for some n in Nat. Now lexically speaking
...
// all e in Nat. Similarly for X. Without loss of generality, lets say
// there are 2 files Ye and Xf such that (1.1) Ye < Xf and Xf contains an
...
// n, n in Nat. We do this by induction, let k be the number of job
