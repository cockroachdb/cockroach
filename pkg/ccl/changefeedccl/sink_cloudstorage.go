...
func (s *cloudStorageSink) flushFile(ctx context.Context, file *cloudStorageSinkFile) error {
	...
	if s.asyncFlushActive && !asyncFlushEnabled {
		// Async flush behavior was turned off -- ensure we drain any active
		// flush requests before flushing this file.
		if err := s.waitAsyncFlush(); err != nil {
			return err
		}
	}
	...
	go func() {
		...
		if err := file.flushToStorage(ctx, s.es, dest, s.metrics); err != nil {
			s.flushErr.CompareAndSwap(nil, err)
		}
	}()
	return nil
}
