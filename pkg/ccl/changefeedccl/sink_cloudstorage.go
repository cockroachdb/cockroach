...
// files that preserves this guarantee. Proof of lemma: Lets refer to the data filenames
// emitted by these sessions as X1,X2.... and similarly for session Y. Additionally, lets
// refer to the last file ever emitted by session X as Xn, for some n in Nat. Now
// lexically speaking there are 2 cases here: 1. Y1 < Xn: For the sake of contradiction,
// let's assume there is a violation here. Since there is a total lexical ordering among
// files in each set individually, we must have read Y(e-1) before Ye, for all e in Nat.
// Similarly for X. Without loss of generality, lets say there are 2 files Ye and Xf such
// that (1.1) Ye < Xf and Xf contains an unseen timestamp that is lower than at least one
// timestamp seen in Ye. call this timestamp t. More explicitly, it must be the case that
// this timestamp t does not exist in any files Y1...Ye. This must mean that timestamp t
// lies before the starting point of session Y's catch-up scan (again from invariant 4).
// Thus it must be the case that (1.2) ts(Y1) > t. Now, due to invariant 3, we know that
// we won't see any rows in a file Xi with a timestamp that is lower than ts(Xi). This
// must mean that (1.3) ts(Xf) <= t. Statements 1.1, 1.2 and 1.3 together give us a
// contradiction. 2. Y1 > Xn. This case means that all data files of session Y lexically
// succeed all the data files of session X. This means that all data files are ordered
// monotonically relative to when they were emitted, this gives us invariant 2 (but for 2
// sessions). Correctness follows from this and invariant 1. Note that Y1 == Xn is not
// possible because sessions are assigned unique session IDs. QED.
//
// Proof of correctness: It is impossible to see a previously unseen timestamp that is
// lower than any timestamp seen thus far, across n job sessions for all n, n in Nat.
// We do this by induction, let k be the number of job sessions a changefeed job goes
// through: Case k = 1: Correctness for this case follows from corollary 1. Case k = 2:
// This follows from lemma 1 stated above. Case k > 2 (induction case): Assume that the
// statement of the proof is true for the output of a changefeed job with k sessions. We
// will show that it must also be true for the output of a changefeed job with k+1
// sessions. Let's refer to the first k jobs as P1,P2,..PK, and the k+1st job as Q. Now,
// since we assumed the statement is true for P1,P2...PK, it must produce a totally
// ordered (lexical ordering) set of files that satisfies requirements of lemma 1. So we
// can consider these k jobs conceptually as one job (call it P). Now, we're back to the
// case where k = 2 with jobs P and Q. Thus, by induction we have the required proof.
//
...
func makeCloudStorageSink(
	...
	if timestampOracle != nil {
		s.inclusiveLowerBoundTs = timestampOracle.inclusiveLowerBoundTS()
	}
...
func (s *cloudStorageSink) flushFile(
	...
	part := file.inclusiveLowerBoundTs.GoTime().Format(s.partitionFormat)
	ts := cloudStorageFormatTime(file.inclusiveLowerBoundTs)
	...
	filename := fmt.Sprintf(`%s-%s-%d-%d-%08x-%s-%x%s`, ts,
