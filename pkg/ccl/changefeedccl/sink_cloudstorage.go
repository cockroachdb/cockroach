...
// filenameVersion is the current version of the filename scheme. This must be
// incremented when the filename scheme is changed.
const filenameVersion = 1
...
// `~<version_id>-<now_timestamp>-<session_id>-<node_id>-<sink_id>-<file_id>` where
// - `<version_id>` is a unique incrementing identifier for the version of the filename scheme,
// to allow for schema evolution without breaking ordering.
// - `<now_timestamp>` is the wall clock time when the file was created in nanoseconds. This
// provides a tiebreaker for ordering files with the same `<timestamp>` (which can occur
// when multiple files are created between resolved timestamp advances).
// - `<session_id>` is a unique identifying string for the job session running the
// `changeAggregator` that owns this sink,
// - `<node_id>` is the id of the node that is running the `changeAggregator` that owns this sink,
// - `<sink_id>`is a unique id for each cloudStorageSink in a running process, and
// - `<file_id>` is a unique id for each file written by a given `<sink_id>`
//
...
// 5. nowts(Xi) refers to the <now_timestamp> part of Xi (the wall clock time
// when file Xi was created).
//
// Invariants
...
// this, primarily through the `<now_timestamp>` component of the uniquer which
// monotonically increases with wall clock time.
...
// inside Xi.
// 4. When a job restarts, the new job session starts with a catch-up scan from either
// (a) the last globally resolved timestamp of the changefeed, or (b) from checkpointed
// span frontiers which may be ahead of the globally resolved timestamp (when some spans
// are lagging behind others). In either case, the catch-up scan replays all rows from
// the resume point onward, preserving invariant 1 for the rows emitted by this session.
// IMPORTANT: Due to (b), a restarted session may NOT re-emit all rows that were emitted
// by a previous session, even if those rows had timestamps greater than the globally
// resolved timestamp. For example, if session X emits file F1 with rows K@1 and K@2 at
// ts(F1)=1 (the HWM), then the changefeed checkpoints K's span at 1.5, and session Y
// restarts resuming from that checkpoint, Y will only emit K@2 in file F2 (skipping K@1).
// Both F1 and F2 will have ts=1, but correctness is preserved by the `<now_timestamp>`
// ordering mechanism described in invariant 5, which ensures F2 > F1 lexically.
// 5. Files created at different wall clock times are ordered by their `<now_timestamp>`
// component. Since time.Now() is monotonic within a node and strictly increases across
// restarts (wall clock time does not go backwards), files created by a later session
// will always have a later `<now_timestamp>` than files created by an earlier session,
// regardless of their `<timestamp>` or `<session_id>` components.
//
...
//
// Lemma 1: Given two totally ordered sets of files X and Y emitted by consecutive sessions
// (where session Y starts after session X completes) that individually preserve CDC's ordering
// guarantee along with invariants 1-5, their union produces a totally ordered set of files
// that preserves this guarantee.
// Proof of lemma: Let the data filenames emitted by these sessions be X1,X2,... and Y1,Y2,...
// respectively. Let Xn be the last file emitted by session X, for some n in Nat.
// By invariant 5, since session Y starts after session X completes (Y's files are created
// after X's files), we have nowts(Y1) > nowts(Xn). Given the filename format
// `<timestamp>-~<version>-<now_timestamp>-...`, files are lexically ordered first by
// <timestamp>, then by the uniquer starting with ~<version> and <now_timestamp>.
//
// Consider the lexical ordering of Y1 and Xn. We examine two cases:
// Case 1: ts(Y1) > ts(Xn). Then Y1 > Xn lexically (since timestamp is the first component).
// Since Xi < Xn for all i < n (invariant 2 for session X), and Y1 > Xn, we have Y1 > Xi
// for all i. Similarly, Yj > Y1 for all j > 1 (invariant 2 for session Y). Thus, all Y
// files lexically succeed all X files, and the union maintains lexical ordering consistent
// with emission order. Correctness follows from invariants 1 and 2.
//
// Case 2: ts(Y1) <= ts(Xn). By invariant 5, nowts(Y1) > nowts(Xn) since Y1 was created
// after Xn. The uniquer component is `~<version>-<now_timestamp>-...`, and since version
// is the same for both, the nowts comparison determines ordering: Y1 > Xn lexically.
// The same argument as Case 1 then applies.
//
// Now we prove no ordering violation occurs. Suppose for contradiction there exist Ye and Xf
// such that Ye < Xf lexically, but Xf contains a previously unseen row with timestamp t that
// is lower than some timestamp seen in Ye.
//
// Since Ye is from session Y which started after X completed, by invariant 5 we have
// nowts(Ye) > nowts(Xf). For Ye < Xf to hold lexically with nowts(Ye) > nowts(Xf), we must
// have ts(Ye) < ts(Xf) (timestamp component dominates). By invariant 3, ts(Ye) is a lower
// bound for all rows in Ye, so all timestamps in Ye are >= ts(Ye). Similarly, ts(Xf) <= t
// (since Xf contains row at timestamp t, and ts(Xf) is a lower bound). But this gives us
// t <= ts(Xf) and the timestamps in Ye >= ts(Ye), and since ts(Ye) < ts(Xf), we have
// timestamps in Ye < ts(Xf). This contradicts the assumption that Ye contains a timestamp
// greater than t. Therefore no such violation can exist. QED.
//
...
type cloudStorageSink struct {
	...
	// Keep track of the previous filename for each topic.
	// This is used to detect ordering violations.
	prevFilename map[string]string
	metrics      metricsRecorder

	nameSchemeVersion         int
	includeUniqueTSInFilename bool
...
func makeCloudStorageSink(
	...
	s := &cloudStorageSink{
		...
		nameSchemeVersion:         filenameVersion,
		includeUniqueTSInFilename: includeUniqueTSInFilename.Get(&settings.SV),
		testingKnobs:              testingKnobs,
		prevFilename:              make(map[string]string),
	}
...
var includeUniqueTSInFilename = settings.RegisterBoolSetting(
	settings.ApplicationLevel,
	"changefeed.cloudstorage.include_unique_ts_in_filename.enabled",
	"include unique timestamp in cloudstorage data filenames",
	true,
)
...
func (s *cloudStorageSink) flushFile(ctx context.Context, file *cloudStorageSinkFile) error {
	...
	var uniquer string
	if s.includeUniqueTSInFilename {
		now := timeutil.Now().UnixNano()
		uniquer = fmt.Sprintf(`~%03d-%d-%s-%d-%d-%08x`, s.nameSchemeVersion, now, s.jobSessionID, s.srcID, s.sinkID, fileID)
	} else {
		uniquer = fmt.Sprintf(`~%03d-%s-%d-%d-%08x`, s.nameSchemeVersion, s.jobSessionID, s.srcID, s.sinkID, fileID)
	}
	filename := fmt.Sprintf(`%s-%s-%s-%x%s`, s.dataFileTs, uniquer, file.topic, file.schemaID, s.ext)

	if s.prevFilename[file.topic] != "" && filename < s.prevFilename[file.topic] {
		err := errors.AssertionFailedf("error: detected a filename %s that lexically "+
			"precedes a file emitted before for topic %s: %s", filename, s.prevFilename[file.topic], file.topic)
		logcrash.ReportOrPanic(ctx, &s.settings.SV, "incorrect filename order: %v", err)
		return err
	}
	s.prevFilename[file.topic] = filename
