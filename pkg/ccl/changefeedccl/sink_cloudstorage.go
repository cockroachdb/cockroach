...
type cloudStorageSink struct {
	...
	jobSessionID string
	// We keep track of the successor timestamp of the last `Flush()` call made on this
	// sink. If `Flush()` hasn't been called, these fields are based on the statement time
	// of the changefeed.
	dataFileTs        string
