...
type cloudStorageSinkFile struct {
	// `frontier` is the least resolved timestamp being tracked by the
	// changeAggregator as of the time of creation of this object.
	// cloudStorageSink uses it to name its output files such that
	// CDC's external ordering guarantees are preserved. See comment
	// on cloudStorageSink for more details.
...
// Intuitively, this can be thought of as a lower bound on the timestamps of updates
// that can be seen in a given file.
//
...
// `<node_id>_<sink_id>_<file_id>_<session_id>` where `<sink_id>` is a unique id for each
...
//
// The choice of `_` to delimit parts of the data filenames is also deliberate.
// This ensures that the resolved files are ordered before the data files since
// the ascii value of `.` is lower than the ascii value of `_`.
...
func (s *cloudStorageSink) EmitRow(
	...
	if file == nil {
		...
		frontier := s.sf.Frontier()
		if frontier.Less(s.initialHighWater) {
			// This condition being true indicates that this is a changefeed
			// job session that hasn't yet seen any resolved timestamps. So
			// we start with the initialHighWater.
			frontier = s.initialHighWater
		}
		file = &cloudStorageSinkFile{frontier: frontier}
		s.files[key] = file
	}
...
func (s *cloudStorageSink) flushFile(
	...
	part := file.frontier.GoTime().Format(s.partitionFormat)
	ts := cloudStorageFormatTime(file.frontier)
	fileID := s.fileID
	s.fileID++
