...
func startDistChangefeed(
	...
	var replanNoProgress *jobspb.ChangefeedProgress
	...
	replanner, stopReplanner := sql.PhysicalPlanChangeChecker(ctx,
		...
		makePlan(execCtx, jobID, details, initialHighWater,
			trackedSpans, replanNoProgress, replanNoDrainingNodes),
		execCtx,
		replanOracle,
...
func partitionSpans(
	...
	if len(details.StaticPartition) == 0 {
	...
	if enableBalancedRangeDistribution.Get(sv) && scanType == changefeedbase.OnlyInitialScan && len(details.StaticPartition) == 0 {
