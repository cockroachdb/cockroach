...
// However, if "selectClause" is specified, the set of spans returned may be
// restricted to satisfy predicate in the select clause.  In that case,
// possibly updated select clause returned representing the "remaining filter"
...
func fetchSpansForTables(
	...
	return cdceval.ConstrainPrimaryIndexSpanByFilter(ctx, execCtx, selectClause, tableDescs[0])
}
...
func startDistChangefeed(
	...
	trackedSpans, selectClause, err := fetchSpansForTables(ctx, execCtx, tableDescs, details.Select)
	if err != nil {
		return err
	}
