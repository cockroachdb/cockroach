...
func (c *kvEventToRowConsumer) makeRowHeaders(
	...
	if objIt == nil {
		if jsonHeaderWrongTypeLogLim.ShouldLog() || log.V(2) {
			log.Warningf(ctx, "headers column %s must be a JSON object, got %s - %s", c.encodingOpts.HeadersJSONColName, headersJSON.Type(), headersJSON)
		}
		return nil, nil
	}
	// Format the object's kv pairs. The intended use case is to have string
	// values, but permit other types, formatted as JSON. I'd rather have
	// confusing headers than the feed choke on specific values.
	for objIt.Next() {
		var val string
		...
		default:
			if jsonHeaderWrongValTypeLogLim.ShouldLog() || log.V(2) {
				log.Warningf(ctx, "headers column %s must be a JSON object with string values, got %s - %s in %s",
					c.encodingOpts.HeadersJSONColName, objIt.Value().Type(), objIt.Value(), headersJSON)
			}
			val = objIt.Value().String()
		}
		headers[objIt.Key()] = []byte(val)
	}
	return headers, nil
}
