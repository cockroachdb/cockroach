...
func (c *kvEventToRowConsumer) ConsumeEvent(ctx context.Context, ev kvevent.Event) error {
	...
	if schemaTimestamp.LessEq(c.frontier.Frontier()) && !schemaTimestamp.Equal(c.cursor) {
		...
		return errors.New("cdc ux violation")
	}
...
type parallelEventConsumer struct {
	...
	numWorkers int64
	// workerCh stores the event buffers for each worker.
...
// getBucketForEvent returns a value between [0, parallelEventConsumer.numWorkers).
// If the event is a KV event, the return value is deterministic. Otherwise,
// it is random.
//
...
func (c *parallelEventConsumer) Flush(ctx context.Context) error {
	...
	// Wait for the flushCh signal. This signal will occur if there is a
	// terminal error or if there are no more events in flight.
	select {
	case <-ctx.Done():
		return ctx.Err()
	case <-c.termCh:
...
func (c *parallelEventConsumer) Close() error {
	...
	// If there is a terminal error, the parallel consumer is shutting down.
	if c.mu.termErr != nil {
		c.mu.Unlock()
		return c.mu.termErr
	}
