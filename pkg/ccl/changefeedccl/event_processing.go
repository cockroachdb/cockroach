...
func (p *pacerWrapper) Close() {
	if p.inner != nil {
		p.inner.Close()
	}
}

func (p *pacerWrapper) Pace(ctx context.Context) error {
	if p.inner != nil {
...
func (c *parallelEventConsumer) workerLoop(
	...
	// If a tenant server was used to create the changefeed, the admission controller
	// will be uninitialized.
	var pacer pacerWrapper
	if c.admissionController != nil {
		pacer.inner = c.admissionController.NewPacer(tenantID, c.admissionPacerRequestUnit, admissionpb.BulkNormalPri)
		defer pacer.Close()
	}
	...
	for {
		...
		case e := <-c.workerCh[id]:
			// Request c.admissionPacerRequestUnit CPU time to use for event
			// consumption, block if this time is unavailable. If there is unused CPU
			// time left from the last call to Pace, then this will utilize not block
