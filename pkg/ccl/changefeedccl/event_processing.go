...
func (c *kvEventToRowConsumer) topicForEvent(src cdcevent.EventSource) (TopicDescriptor, error) {
	if topic, ok := c.topicDescriptorCache[TopicIdentifier{TableID: src.TableID(), FamilyID: src.FamilyID()}]; ok {
		if topic.GetVersion() == src.Version() {
			return topic, nil
		}
	}
	for _, s := range c.details.TargetSpecifications {
		if s.TableID == src.TableID() && (s.FamilyName == "" || s.FamilyName == src.FamilyName()) {
			topic, err := makeTopicDescriptorFromSpec(s, src)
			if err != nil {
				return noTopic{}, err
			}
	...
	return noTopic{}, errors.AssertionFailedf("no TargetSpecification for row %s", cdcevent.EventSourceString(src))
}
...
func (c *kvEventToRowConsumer) ConsumeEvent(ctx context.Context, ev kvevent.Event) error {
	...
	prevRow, err := func() (cdcevent.EventRow, error) {
		if _, withDiff := c.details.Opts[changefeedbase.OptDiff]; !withDiff {
			return cdcevent.EventRow{}, nil
		}
	...
	topic, err := c.topicForEvent(updatedRow)
	if err != nil {
		return err
	}
	...
	if err != nil {
		return errors.Wrapf(err, "encoding key %s", roachpb.PrettyPrintKey(nil, ev.KV().Key))
	}
