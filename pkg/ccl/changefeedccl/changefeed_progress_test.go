...
func TestChangefeedFrontierPersistence(t *testing.T) {
	...
	testFn := func(t *testing.T, s TestServer, f cdctest.TestFeedFactory) {
		...
		fpMetrics := registry.MetricsStruct().Changefeed.(*Metrics).AggMetrics.Timers.FrontierPersistence

		// Verify metric starts at zero
		initialCount, _ := fpMetrics.CumulativeSnapshot().Total()
		...
		// Verify frontier persistence metrics
		count, _ := fpMetrics.CumulativeSnapshot().Total()
		mean := fpMetrics.CumulativeSnapshot().Mean()
