...
func TestChangefeedFrontierPersistence(t *testing.T) {
	...
	testFn := func(t *testing.T, s TestServer, f cdctest.TestFeedFactory) {
		...
		avgLatency := time.Duration(metric.CumulativeSnapshot().Mean())
		...
		require.Greater(t, avgLatency, time.Duration(0), "frontier persistence avg latency should be greater than 0")
	}
