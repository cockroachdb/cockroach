...
func TestTablesetBasic(t *testing.T) {
	...
	mm := mon.NewMonitor(mon.Options{
		...
		// The limit value doesn't matter so long as it's at least the cache
		// size (1KiB) plus enough for some data.
		Limit:     1024 * 1024 * 10,
		Increment: 128,
...
func TestTablesetNonIdempotentProtection(t *testing.T) {
	...
	setup := func(ctx context.Context, eg *errgroup.Group) (*tableset.Watcher, context.CancelFunc) {
	...
	t.Run("regressing timestamps", func(t *testing.T) {
		eg, ctx := errgroup.WithContext(ctx)
		watcher, cancel := setup(ctx, eg)
		defer cancel()
		...
		_, err = watcher.PopUpTo(ctx, ts.AddDuration(-1*time.Second))
		require.Error(t, err)
		require.Contains(t, err.Error(), "PopUpTo called with non-advancing timestamp")
	...
	t.Run("same timestamp in allowed repeated calls", func(t *testing.T) {
		eg, ctx := errgroup.WithContext(ctx)
		watcher, cancel := setup(ctx, eg)
		defer cancel()
		...
		_, err = watcher.PopUpToAllowRepeatedTimes(ctx, ts)
		require.NoError(t, err)
	...
	t.Run("regressing timestamps in allowed repeated calls (still errors)", func(t *testing.T) {
		eg, ctx := errgroup.WithContext(ctx)
		watcher, cancel := setup(ctx, eg)
		defer cancel()
		...
		_, err = watcher.PopUpToAllowRepeatedTimes(ctx, ts.AddDuration(-1*time.Second))
		require.Error(t, err)
		require.Contains(t, err.Error(), "PopUpTo called with non-advancing timestamp")
