...
func New(histogramWindow time.Duration) *Timers {
	...
	stageOpts := func(labelName, desc string) metric.HistogramOptions {
		labeledName := fmt.Sprintf("%s.%s", stagePrefix, latencySuffix)
		name := fmt.Sprintf("%s.%s.%s", stagePrefix, labelName, latencySuffix)
		return histogramOptsFor(name, labeledName, labelName, desc)
	}

	ptsStageOpts := func(labelName, desc string) metric.HistogramOptions {
		name := fmt.Sprintf("%s.%s.%s.%s", stagePrefix, ptsSubCategory, labelName, latencySuffix)
