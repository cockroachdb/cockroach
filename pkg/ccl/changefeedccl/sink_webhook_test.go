...
// statusCodeRepeatedFromRetries returns an array of status codes that the mock
...
func statusCodeRepeatedFromRetries(code int) []int {
	defaultAttempts := defaultRetryConfig().MaxRetries + 1
	arr := make([]int, defaultAttempts)
...
func TestWebhookSinkShutsDownOnError(t *testing.T) {
	...
	webhookSinkTestfn := func(parallelism int) {
		...
		sinkDest.SetStatusCodes(statusCodeRepeatedFromRetries(http.StatusInternalServerError))
		sinkDestHost, err := url.Parse(sinkDest.URL())
		require.NoError(t, err)
