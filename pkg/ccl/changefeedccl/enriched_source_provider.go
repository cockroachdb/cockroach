...
func GetTableSchemaInfo(
	...
	if err != nil {
		return make(map[descpb.ID]tableSchemaInfo), err
	}
...
func getDescriptors(
	...
	f := func(ctx context.Context, txn descs.Txn) error {
		...
		tableDesc, err := byIDGetter.Table(ctx, tableID)
		if err != nil {
			return err
		}
		...
		schemaDesc, err := byIDGetter.Schema(ctx, tableDesc.GetParentSchemaID())
		if err != nil {
			return err
		}
