...
func doCreateChangefeedSchedule(
	...
	if !createChangefeedopts.IsInitialScanSpecified() {
		...
		eval.Options = append(eval.Options, tree.KVOption{
			Key: changefeedbase.OptInitialScanOnly,
		})
		eval.createChangefeedOptions[changefeedbase.OptInitialScanOnly] = ""
	} else {
	...
	if err = dryRunCreateChangefeed(
		...
		wrappedErr := errors.Wrapf(err, "Failed to dry run create changefeed")
		if err = es.Delete(ctx, p.ExecCfg().InternalExecutor, p.Txn()); err != nil {
			return errors.Wrapf(wrappedErr, "Failed to delete created schedule before dry run")
		}
		return wrappedErr
	}

	if err := emitSchedule(
		ctx, es, createChangefeedNode, *eval.evaluatedSinkURI,
		eval.createChangefeedOptions, resultsCh,
	); err != nil {
		return err
	}
