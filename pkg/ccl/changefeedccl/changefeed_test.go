...
func TestChangefeedProgressMetrics(t *testing.T) {
	...
	cdcTest(t, func(t *testing.T, s TestServer, f cdctest.TestFeedFactory) {
		...
		sqlDB.Exec(t, `CREATE TABLE foo (a INT PRIMARY KEY)`)
		foo := feed(t, f, `CREATE CHANGEFEED FOR foo WITH metrics_label='label_a', resolved='100ms'`)
		defer closeFeed(t, foo)

		registry := s.Server.JobRegistry().(*jobs.Registry)
		sli, err := registry.MetricsStruct().Changefeed.(*Metrics).getSLIMetrics("label_a")
		require.NoError(t, err)
		...
		for i := 0; i < 3; i++ {
			testutils.SucceedsSoon(t, func() error {
				progress := sli.AggregatorProgress.Value()
		...
		for i := 0; i < 3; i++ {
			testutils.SucceedsSoon(t, func() error {
				progress := sli.CheckpointProgress.Value()
				if progress > lastTimestamp {
					lastTimestamp = progress
					return nil
				}
