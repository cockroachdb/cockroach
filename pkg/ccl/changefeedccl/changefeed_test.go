...
func TestChangefeedFailOnRBRChange(t *testing.T) {
	...
	t.Run(`http`, httpTestWithServerArgs(withTestServerRegion, testFn))
}
...
func TestChangefeedErrors(t *testing.T) {
	...
	sqlDB.ExpectErr(
		t, `option http_client_timeout must be a positive integer`,
		`CREATE CHANGEFEED FOR foo INTO $1 WITH http_client_timeout='not_an_integer'`, `webhook-https://fake-host`,
	)
	sqlDB.ExpectErr(
		t, `option http_client_timeout must be a positive integer`,
		`CREATE CHANGEFEED FOR foo INTO $1 WITH http_client_timeout='0'`, `webhook-https://fake-host`,
	)
	sqlDB.ExpectErr(
		t, `option http_client_timeout must be a positive integer`,
		`CREATE CHANGEFEED FOR foo INTO $1 WITH http_client_timeout='-500'`, `webhook-https://fake-host`,
	)
	sqlDB.ExpectErr(
		t, `option http_client_timeout must be a positive integer`,
		`CREATE CHANGEFEED FOR foo INTO $1 WITH http_client_timeout='0.5'`, `webhook-https://fake-host`,
	)
	sqlDB.ExpectErr(
		t, `this sink is incompatible with envelope=key_only`,
		`CREATE CHANGEFEED FOR foo INTO $1 WITH envelope='key_only'`,
...
func TestChangefeedProtectedTimestampOnPause(t *testing.T) {
	...
	testutils.RunTrueAndFalse(t, "protect_on_pause", func(t *testing.T, shouldPause bool) {
		...
		t.Run(`http`, httpTest(testFn(shouldPause)))
	})
...
func TestChangefeedProtectedTimestampsVerificationFails(t *testing.T) {
	...
	t.Run(`http`, httpTestWithServerArgs(setStoreKnobs, testFn))
}
...
func TestChangefeedPrimaryKeyChangeMixedVersion(t *testing.T) {
	...
	t.Run("http", httpTestWithServerArgs(setMixedVersion, testFn))
}
