...
func TestSinkBackpressureMetric(t *testing.T) {
	...
	// This test relies on messing with timings to create backpressure,
	// so skip it when the system is loaded.
	skip.UnderDuress(t)

	// Add delay so quota limits are hit, which results in backpressure.
	defer testingEnableQueuingDelay()()
	...
	testFn := func(t *testing.T, s TestServer, f cdctest.TestFeedFactory) {
		...
		// Keep writing data to the same key to ensure contention and quota pressure.
		ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
		defer cancel()
		...
		// Create changefeed with fast flush to increase quota pressure
		foo, err := f.Feed("CREATE CHANGEFEED FOR TABLE foo WITH pubsub_sink_config=" +
			"'{\"Flush\": {\"Frequency\": \"100ms\"}}'")
		require.NoError(t, err)
