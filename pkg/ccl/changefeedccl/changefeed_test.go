...
func TestChangefeedEnriched(t *testing.T) {
	...
	keySchema := map[string]any{
		...
		// TODO: We're just setting optional to true on everything but I can see
		// an argument for not doing that, and being more realistic about it.
		"optional": true,
		"type":     "struct",
	}
	...
	tsNsSchema := map[string]any{"field": "ts_ns", "optional": true, "type": "int64"}
	opSchema := map[string]any{"field": "op", "optional": true, "type": "string"}

	cases := []struct {
		name                 string
		enrichedProperties   []string
		makeExpectedJSONBody func(source map[string]any) map[string]any
	...
	}{
		{
			...
			makeExpectedJSONBody: func(source map[string]any) map[string]any {
				return map[string]any{
					"after": afterVal,
					"op":    "c",
					// NOTE: here and in all these cases, ts_ns is stripped by the test framework.
				}
			},
			expectedKey: key,
		},
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			testFn := func(t *testing.T, s TestServer, f cdctest.TestFeedFactory) {
				...
				msgBody := toJSON(t, tc.makeExpectedJSONBody(source))
