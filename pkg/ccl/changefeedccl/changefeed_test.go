...
func TestChangefeedLaggingSpanCheckpointing(t *testing.T) {
  ...
  `)
	...
	knobs.FeedKnobs.ShouldSkipCheckpoint = func(checkpoint *roachpb.RangeFeedCheckpoint) bool {
		...
		if laggingSpans.Encloses(checkpoint.Span) || (numLagging < 5 && rnd.Int()%3 == 0) {
	...
	// We expect checkpoint to have non-zero spans that are ahead of progress highwatermark.
	// The highwatermark should be exactly the value of the cursor since we have lagging spans.
	progress := loadProgress()
	require.True(t, progress.GetHighWater().Equal(cursor))
	...
	require.True(t, cursor.Less(checkpointTS))
