...
func TestChangefeedMemBufferCapacity(t *testing.T) {
	...
	testFn := func(t *testing.T, db *gosql.DB, f cdctest.TestFeedFactory) {
		...
		//
		// TODO(dan): I think this is racy as written. We could happen to get
		// everything from RangeFeed spaced out just enough that they stay perfectly
		// under the budget. I haven't seen any flakes when stressing it locally,
		// but I'd still like to do something smarter with blocking the
		// BeforeEmitRow knob, but there are some subtleties with that approach to
		// work out.
		const rows = 1000
		sqlDB.Exec(t, `INSERT INTO foo SELECT i, 'foofoofoo' FROM generate_series(1, $1) AS g(i)`, rows)
		var err error
		for i := 0; i < rows; i++ {
			if _, err = foo.Next(); err != nil {
				break
			}
		}
		if !testutils.IsError(err, `memory budget exceeded`) {
