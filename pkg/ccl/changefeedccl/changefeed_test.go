		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			if testCase.name == "json" {
				sourceAssertion = func(actualSource map[string]interface{}) {
					...
					assertion := fmt.Sprintf(`{"cluster_id": "%s", "cluster_name": "%s", "db_version": "%s", "job_id": "%d", "node_id": "%s", "node_name": "%s", "source_node_locality": "%s"}`,
				...
				sourceAssertion = func(actualSource map[string]interface{}) {
					...
					sourceNodeLocality := fmt.Sprintf(`region=us-east%s`, nodeID)
					assertion := fmt.Sprintf(`{"source": {"cluster_id": {"string": "%s"}, "cluster_name": {"string": "%s"}, "db_version": {"string": "%s"}, "job_id": {"string": "%d"}, "node_id": {"string": "%s"}, "node_name": {"string": "%s"}, "source_node_locality": {"string": "%s"}}}`,
						clusterID, clusterName, dbVersion, jobID, nodeID, nodeName, sourceNodeLocality)
