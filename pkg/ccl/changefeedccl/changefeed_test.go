...
func TestChangefeedSchemaChangeBackfillCheckpoint(t *testing.T) {
	...
	testFn := func(t *testing.T, db *gosql.DB, f cdctest.TestFeedFactory) {
		...
		knobs.ShouldSkipResolved = func(r *jobspb.ResolvedSpan) bool {
			// Stop resolving anything after checkpoint set to avoid eventualy resolving the full span
	...
	t.Run(fmt.Sprintf("enterprise"), enterpriseTest(testFn, feedTestNoTenants))
	t.Run(fmt.Sprintf("cloudstorage"), cloudStorageTest(testFn, feedTestNoTenants))
	t.Run(fmt.Sprintf("kafka"), kafkaTest(testFn, feedTestNoTenants))
