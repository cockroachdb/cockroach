						...
						sourceAssertion = func(actualSource map[string]any) {
							...
							if testCase.format == "avro" {
								...
								if withUpdated {
									...
									actualTSNS := tsnsMap["long"].(float64)
									tsnsFloat := strconv.FormatFloat(actualTSNS, 'f', -1, 64)
									assertReasonableMVCCTimestamp(t, tsnsFloat)
									...
									assertEqualNSHLCWalltime(t, actualTSNS, tshlcMap["string"].(string))
								...
								if withUpdated {
									actualTSNS := actualSource["ts_ns"].(float64)
									tsnsFloat := strconv.FormatFloat(actualTSNS, 'f', -1, 64)
									assertReasonableMVCCTimestamp(t, tsnsFloat)
									actualSource["ts_ns"] = dummyUpdatedTSNS
									assertionMap["ts_ns"] = dummyUpdatedTSNS
									assertEqualNSHLCWalltime(t, actualTSNS, actualSource["ts_hlc"].(string))
...
// tsns is float64 for some reason when reading from enriched metadata
// source body, keeping the same type here to not lose precision
func assertEqualNSHLCWalltime(t *testing.T, tsns float64, tshlc string) {
