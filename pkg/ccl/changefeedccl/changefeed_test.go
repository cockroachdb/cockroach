...
func TestChangefeedCreateTelemetryLogs(t *testing.T) {
	...
	t.Run(`gcpubsub_sink_type`, func(t *testing.T) {
		pubsubFeedFactory := makePubsubFeedFactory(s, db)
		beforeCreatePubsub := timeutil.Now()
		pubsubFeed := feed(t, pubsubFeedFactory, `CREATE CHANGEFEED FOR foo, bar`)
...
func TestChangefeedFailedTelemetryLogs(t *testing.T) {
	...
	t.Run(`unknown_error`, enterpriseTest(func(t *testing.T, db *gosql.DB, f cdctest.TestFeedFactory) {
		sqlDB := sqlutils.MakeSQLRunner(db)
		sqlDB.Exec(t, `CREATE TABLE foo (a INT PRIMARY KEY, b STRING)`)
