...
func TestChangefeedDuplicateNotices(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)
	...
	t.Run("resume existing changefeed", func(t *testing.T) {
		...
		jobID = createChangefeedAndCheckDuplicateNotices(t, s.Server, `CREATE CHANGEFEED FOR d.t0 INTO 'null://'`, false, jobspb.InvalidJobID)
		sqlDB.Exec(t, `PAUSE JOB $1`, jobID)
		...
		jobID1 := jobID
		jobID = min(jobID, createChangefeedAndCheckDuplicateNotices(t, s.Server, `CREATE CHANGEFEED FOR d.t0 INTO 'null://'`, false, jobspb.InvalidJobID))
		...
		createChangefeedAndCheckDuplicateNotices(t, s.Server, `CREATE CHANGEFEED FOR d.t0 INTO 'null://'`, true, jobID)
	})

	t.Run("create new changefeed", func(t *testing.T) {
		sqlDB.Exec(t, `CREATE TABLE t1 (a INT PRIMARY KEY, b STRING)`)
		jobID = createChangefeedAndCheckDuplicateNotices(t, s.Server, `CREATE CHANGEFEED FOR d.t1 INTO 'null://'`, false, jobspb.InvalidJobID)
		...
		createChangefeedAndCheckDuplicateNotices(t, s.Server, `CREATE CHANGEFEED FOR d.t1 INTO 'null://'`, true, jobID)
	})
	...
	t.Run("notifying lowest matching jobID from duplicate jobs", func(t *testing.T) {
		...
		jobID = createChangefeedAndCheckDuplicateNotices(t, s.Server, `CREATE CHANGEFEED FOR d.t1 INTO 'null://'`, false, jobspb.InvalidJobID)
		for i := 0; i < 10; i++ {
			tempJobID := createChangefeedAndCheckDuplicateNotices(t, s.Server, `CREATE CHANGEFEED FOR d.t1 INTO 'null://'`, true, jobID)
	...
	t.Run("multiple tables and ordering", func(t *testing.T) {
		...
		jobID = createChangefeedAndCheckDuplicateNotices(t, s.Server, "CREATE CHANGEFEED FOR d.t1, d.t2 INTO 'null://'", false, jobspb.InvalidJobID)
		jobID = min(jobID, createChangefeedAndCheckDuplicateNotices(t, s.Server, "CREATE CHANGEFEED FOR d.t1, d.t2 INTO 'null://'", true, jobID))
		createChangefeedAndCheckDuplicateNotices(t, s.Server, "CREATE CHANGEFEED FOR d.t2, d.t1 INTO 'null://'", true, jobID)
	})

	t.Run("changing table name", func(t *testing.T) {
		sqlDB.Exec(t, `ALTER TABLE t2 RENAME TO t100`)
		createChangefeedAndCheckDuplicateNotices(t, s.Server, "CREATE CHANGEFEED FOR d.t100 INTO 'null://'", false, jobspb.InvalidJobID)
	...
	t.Run("alter changefeed", func(t *testing.T) {
		...
		jobID = createChangefeedAndCheckDuplicateNotices(t, s.Server, "CREATE CHANGEFEED FOR d.t3, d.t4 INTO 'null://'", false, jobspb.InvalidJobID)
		sqlDB.Exec(t, `PAUSE JOB $1`, jobID)
		sqlDB.CheckQueryResultsRetry(t, fmt.Sprintf("SELECT count(*) FROM [SHOW JOBS] WHERE job_id = %d and job_type='CHANGEFEED' and status='paused'", jobID), [][]string{{"1"}})
		...
		time.Sleep(3 * time.Second) // Resuming job - status is set to running before the code for resuming the job runs. So, takes a few seconds before it can be tracked for dups.
		sqlDB.CheckQueryResultsRetry(t, fmt.Sprintf("SELECT count(*) FROM [SHOW JOBS] WHERE job_id = %d and job_type='CHANGEFEED' and status='running'", jobID), [][]string{{"1"}})
		createChangefeedAndCheckDuplicateNotices(t, s.Server, "CREATE CHANGEFEED FOR d.t3 INTO 'null://'", true, jobID)
	})
	...
	t.Run("column family", func(t *testing.T) {
		...
		jobIDMost := createChangefeedAndCheckDuplicateNotices(t, s.Server, "CREATE CHANGEFEED FOR d.t5 FAMILY most INTO 'null://'", false, jobspb.InvalidJobID)
		jobIDRest := createChangefeedAndCheckDuplicateNotices(t, s.Server, "CREATE CHANGEFEED FOR d.t5 FAMILY rest INTO 'null://'", false, jobspb.InvalidJobID)
		createChangefeedAndCheckDuplicateNotices(t, s.Server, "CREATE CHANGEFEED FOR d.t5 FAMILY most INTO 'null://'", true, jobIDMost)
		createChangefeedAndCheckDuplicateNotices(t, s.Server, "CREATE CHANGEFEED FOR d.t5 FAMILY rest INTO 'null://'", true, jobIDRest)
	})
}
