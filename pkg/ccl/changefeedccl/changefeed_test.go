...
func TestChangefeedRetryableSinkError(t *testing.T) {
	...
	var failSink int64 = 0
	failSinkHook := func() bool {
		return atomic.LoadInt64(&failSink) != 0
	}
	s, sqlDBRaw, _ := serverutils.StartServer(t, base.TestServerArgs{
		Knobs: base.TestingKnobs{
			DistSQL: &distsqlrun.TestingKnobs{
				FailChangefeedSqlSinkHook: failSinkHook,
			},
		},
		UseDatabase: "d",
	})
	...
	sqlDB.Exec(t, fmt.Sprintf(`CREATE CHANGEFEED FOR foo INTO 'experimental-sql://sinkuser:sink@%s/d'`, s.ServingAddr()))
	...
	testutils.SucceedsSoon(t, func() error {
		if retryCounter.Counter.Count() < 10 {
