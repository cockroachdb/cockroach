						...
						sourceAssertion = func(actualSource map[string]any) {
							...
							if testCase.format == "avro" {
								...
								if withUpdated {
									...
									actualTSNS, err := tsnsMap["long"].(gojson.Number).Int64()
									require.NoError(t, err)
									tsnsString := strconv.FormatInt(actualTSNS, 10)
								...
								if withUpdated {
									actualTSNS, err := actualSource["ts_ns"].(gojson.Number).Int64()
									require.NoError(t, err)
									tsnsString := strconv.FormatInt(actualTSNS, 10)
									assertReasonableMVCCTimestamp(t, tsnsString)
...
func assertEqualNSHLCWalltime(t *testing.T, tsns int64, tshlc string) {
