...
func TestChangefeedJobRetryOnNoInboundStream(t *testing.T) {
	...
	sqlDB.Exec(t, "SET CLUSTER SETTING sql.distsql.max_running_flows=0")
	sqlDB.Exec(t, "SET CLUSTER SETTING sql.distsql.flow_stream_timeout='1s'")
	testutils.SucceedsSoon(t, func() error {
		for nodeID := 0; nodeID < cluster.NumServers(); nodeID++ {
			var flows int
			conn := cluster.ServerConn(nodeID)
