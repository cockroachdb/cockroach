		...
		t.Run(fmt.Sprintf("%s/errAfter=%d", testName, triggerErrorAfter),
			func(t *testing.T) {
				...
				defer func() {
					...
					for i := 0; i < numNodes; i++ {
						testDir := path.Join(testTmpDir, str(i), testName)
						_ = os.RemoveAll(testDir) // err ignored since not all directories should exist (due to locality filter).
					}
				}()

				const fileSize = 64 // Tiny file size to cause lots of flushes.
				dest := fmt.Sprintf("nodelocal://0/%s?file_size=%d", testName, fileSize)
				...
				for i := range expect {
					where := path.Join(testTmpDir, str(i), testName)
				...
				require.NoError(t,
					filepath.WalkDir(testTmpDir, func(p string, d fs.DirEntry, err error) error {
						if err != nil {
							return err
						}
						if d.IsDir() {
