...
func TestChangefeedBackfillCheckpoint(t *testing.T) {
	...
	testFn := func(t *testing.T, db *gosql.DB, f cdctest.TestFeedFactory) {
		...
		haveGaps := false
		knobs.ShouldEmitResolved = func(r *jobspb.ResolvedSpan) bool {
		...
		checkpoint := progress.GetChangefeed().Checkpoint
		require.Less(t, 0, len(checkpoint.Spans))
		checkpointFrontier, err := span.MakeFrontier(checkpoint.Spans...)
		...
		// Wait for high water mark to be non-zero
		testutils.SucceedsSoon(t, func() error {
			progress := loadProgress()
		...
		for _, sp := range resolved {
			checkpointFrontier.SpanEntries(
				sp, func(s roachpb.Span, ts hlc.Timestamp) (done span.OpResult) {
					t.Fatalf("span should not have been resolved: %s", sp)
