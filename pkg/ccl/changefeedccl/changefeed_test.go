...
func fetchDescVersionModificationTime(
	...
	tblKey := keys.SystemTenantKeyGen.TablePrefix(keys.DescriptorTableID)
	...
	for _, file := range res.(*roachpb.ExportResponse).Files {
		...
		for it.SeekGE(storage.NilKey); ; it.Next() {
			...
			k := it.UnsafeKey()
			remaining, _, _, err := keys.SystemTenantKeyGen.DecodeIndexPrefix(k.Key)
			if err != nil {
				t.Fatal(err)
			}
	...
	var (
		...
		mkCheckRecord = func(t *testing.T, tableID int) func(r *ptpb.Record) error {
			expectedKeys := map[string]struct{}{
				string(keys.SystemTenantKeyGen.TablePrefix(uint32(tableID))):        {},
				string(keys.SystemTenantKeyGen.TablePrefix(keys.DescriptorTableID)): {},
			}
