...
func TestParallelIOMetrics(t *testing.T) {
	...
	testFn := func(t *testing.T, s TestServer, f cdctest.TestFeedFactory) {
		...
		db.Exec(t, `INSERT INTO foo (a) SELECT * FROM generate_series(1, 2);`)
		...
		g.Go(func() error {
			for {
				...
				case <-done:
					return nil
				default:
					_, err := s.DB.Exec(`UPDATE foo SET b = foo.b + 1`)
					if err != nil {
						return err
					}
		...
		pendingRowsWaiter := pmr.pendingRows.(*testMetric).SetWaiter(func(val int64) bool { return val > 0 })
		knobs.ParallelIOOnEmit = func() {
			// Block emit until we get some rows in pending.
			require.NoError(t, pendingRowsWaiter.Wait(30*time.Second))
		}

		goWaitMultiple(30*time.Second, g.Go,
			pendingRowsWaiter,
