...
// same transacton and thus at the same moment but in later code changes, it
// will.
...
ALTER TABLE bar ADD COLUMN c INT NOT NULL DEFAULT 7;
...
`)
		...
		assertPayloads(t, cf, []string{
			`bar: [1]->{"after": {"a": 1, "b": "a", "c": 7}}`,
			`bar: [2]->{"after": {"a": 2, "b": "b", "c": 7}}`,
			`bar: [3]->{"after": {"a": 3, "b": "c", "c": 7}}`,
			`foo: [3]->{"after": {"a": 3, "b": "c"}}`,
		})
		defer maybeHandleRestart(t)()
		assertPayloads(t, cf, []string{
