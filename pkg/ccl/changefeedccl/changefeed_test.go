...
func TestDuplicateChangefeed(t *testing.T) {
	...
	t.Run("column family", func(t *testing.T) {
		...
		var jobIDMost jobspb.JobID
		var jobIDRest jobspb.JobID
		sqlDB.QueryRow(t, `CREATE CHANGEFEED FOR d.t5 FAMILY most INTO 'null://'`).Scan(&jobIDMost)
		sqlDB.QueryRow(t, `CREATE CHANGEFEED FOR d.t5 FAMILY rest INTO 'null://'`).Scan(&jobIDRest)
		expectNoticeGetJobID(t, s.Server, "CREATE CHANGEFEED FOR d.t5 FAMILY most INTO 'null://'", true, jobIDMost)
		expectNoticeGetJobID(t, s.Server, "CREATE CHANGEFEED FOR d.t5 FAMILY rest INTO 'null://'", true, jobIDRest)
	})
}
