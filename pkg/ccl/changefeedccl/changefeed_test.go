...
func TestChangefeedLaggingSpanCheckpointing(t *testing.T) {
  ...
  `)
	...
	var nonLaggingSpans roachpb.Spans
	numLagging := 0
	knobs.FeedKnobs.ShouldSkipCheckpoint = func(checkpoint *kvpb.RangeFeedCheckpoint) bool {
		...
		if i := slices.IndexFunc(nonLaggingSpans, func(s roachpb.Span) bool {
			return s.Equal(checkpoint.Span)
		}); i != -1 {
			return i%3 == 0
		}
		...
		nonLaggingSpans = append(nonLaggingSpans, checkpoint.Span)
		return false
	}
	...
	knobs.FeedKnobs.OnRangeFeedStart = func(spans []kvcoord.SpanTimePair) {
		...
		for _, sp := range spans {
			// NB: It would be nice if we could create a reverse index from
			// spans to timestamps, but unfortunately roachpb.Span does not
			// implement the equal operator and so cannot be a map key type.
			var checkpointTS hlc.Timestamp
			for ts, spans := range spanLevelCheckpoint.All() {
				if slices.ContainsFunc(spans, func(s roachpb.Span) bool {
					return s.Equal(sp.Span)
				}) {
					checkpointTS = ts
					break
				}
			}
			if checkpointTS.IsSet() {
