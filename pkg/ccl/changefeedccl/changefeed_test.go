...
func TestChangefeedAuthorization(t *testing.T) {
	...
	testFn := func(t *testing.T, s TestServer, f cdctest.TestFeedFactory) {
		...
		expectSuccess := func(stmt string) {
			successfulFeed := feed(t, f, `CREATE CHANGEFEED FOR table_a`)
		...
		// GRANT CHANGEFEED ON DATABASE grants CHANGEFEED on all tables in that database.
		rootDB.Exec(t, `GRANT CHANGEFEED ON DATABASE d TO guest`)
		asUser(t, f, `guest`, func() {
			expectSuccess(`CREATE CHANGEFEED FOR table_a`)
		})

		// GRANT CHANGEFED ON prefix.* grants CHANGEFEED on all current tables with that prefix.
		rootDB.Exec(t, `CREATE TABLE table_c (id int primary key)`)
		rootDB.Exec(t, `INSERT INTO table_c values (0)`)
...
func TestChangefeedAuthorizationSinkless(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)
	...
	testFn := func(t *testing.T, s TestServer, f cdctest.TestFeedFactory) {
		rootDB := sqlutils.MakeSQLRunner(s.DB)
		rootDB.Exec(t, `create user guest`)
		rootDB.Exec(t, `create table table_a (id int)`)
		rootDB.Exec(t, `create table table_b (id int)`)
		rootDB.Exec(t, `insert into table_a(id) values (0)`)
		rootDB.Exec(t, `insert into table_b(id) values (0)`)
		rootDB.Exec(t, `GRANT CHANGEFEED, SELECT ON table_a TO guest`)
		rootDB.Exec(t, `GRANT CHANGEFEED ON table_b TO guest`)

		// SELECT privileges are needed for core changefeeds.
		asUser(t, f, `guest`, func() {
			expectErrCreatingFeed(t, f, `CREATE CHANGEFEED FOR table_a, table_b`,
				`user guest does not have SELECT privilege on relation table_b`)
		})

		rootDB.Exec(t, `GRANT SELECT ON table_b TO guest`)

		asUser(t, f, `guest`, func() {
			successfulFeed := feed(t, f, `CREATE CHANGEFEED FOR table_a, table_b`)
			defer closeFeed(t, successfulFeed)
			_, err := successfulFeed.Next()
			require.NoError(t, err)
		})
