...
func TestChangefeedReplanning(t *testing.T) {
	...
	assertReplanCounter := func(t *testing.T, m *Metrics, exp int64) {
		...
		testutils.SucceedsSoon(t, func() error {
			if got := m.ReplanMetrics.Count(); got != exp {
	...
	testFn := func(t *testing.T, s TestServer, f cdctest.TestFeedFactory) {
		...
		sqlDB.Exec(t, `INSERT INTO foo (a) SELECT * FROM generate_series(1, 1000); ALTER TABLE foo SPLIT AT (SELECT * FROM generate_series(1, 1000, 50)); ALTER TABLE foo SCATTER`)
