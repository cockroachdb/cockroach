...
func TestChangefeedOutdatedCursor(t *testing.T) {
	...
	testFn := func(t *testing.T, s TestServerWithSystem, f cdctest.TestFeedFactory) {
		...
		createChangefeed := `CREATE CHANGEFEED FOR TABLE f with cursor = '` + outdatedTS + `'`
		expectedErrorPattern := ".*could not create changefeed: cursor " + outdatedTS +
			" is older than the GC threshold .*"
		expectErrCreatingFeed(t, f, createChangefeed, expectedErrorPattern)
	}
