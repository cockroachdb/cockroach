...
func TestDatabaseLevelChangefeedChangingTableset(t *testing.T) {
	...
	startMakingTables := make(chan struct{}, 1)
	stopMakingTables := make(chan struct{})
	...
	knobsFn := func(knobs *base.TestingKnobs) {
		...
		cfKnobs.TablesetChangingCallback = func() {
			select {
			case startMakingTables <- struct{}{}:
				// signal the test once; non-blocking
			case <-time.After(5 * time.Second):
	...
	testFnAdd := func(t *testing.T, s TestServer, f cdctest.TestFeedFactory) {
		...
		close(stopMakingTables)
		wg.Wait()
	}
	cdcTest(t, testFnAdd, withKnobsFn(knobsFn), feedTestForceSink("enterprise"))
