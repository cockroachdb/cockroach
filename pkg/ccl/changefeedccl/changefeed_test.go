...
func TestChangefeedAuthorization(t *testing.T) {
	...
	testFn := func(t *testing.T, s TestServer, f cdctest.TestFeedFactory) {
		...
		// Users can never create a changefeed on a table they lack SELECT on
		asUser(t, f, `guest`, func() {
			expectErrCreatingFeed(t, f, `CREATE CHANGEFEED FOR table_a`,
				`privilege on relation table_a`)
		})
		...
		// Users without the controlchangefeed role option need both the CHANGEFEED
		// and SELECT privilege on every table.
		asUser(t, f, `guest`, func() {
		...
		t.Log("about to grant on database")
		rootDB.Exec(t, `GRANT CHANGEFEED ON DATABASE d TO guest`)
		t.Log("granted on database")
		asUser(t, f, `guest`, func() {
			successfulFeed := feed(t, f, `CREATE CHANGEFEED FOR table_a`)
			defer closeFeed(t, successfulFeed)
			_, err := successfulFeed.Next()
		...
		asUser(t, f, `guest`, func() {
			successfulFeed := feed(t, f, `CREATE CHANGEFEED FOR table_c`)
			defer closeFeed(t, successfulFeed)
			_, err := successfulFeed.Next()
			require.NoError(t, err)
		})
