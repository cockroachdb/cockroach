...
func TestKafkaSink(t *testing.T) {
	...
	require.NoError(t, sink.EmitRow(ctx, topic(`t`), []byte(`2`), nil, zeroTS, zeroTS, pool.alloc(), ""))
	m2 := <-p.inputCh
	require.NoError(t, sink.EmitRow(ctx, topic(`t`), []byte(`3`), nil, zeroTS, zeroTS, pool.alloc(), ""))

	m3 := <-p.inputCh
	require.NoError(t, sink.EmitRow(ctx, topic(`t`), []byte(`4`), nil, zeroTS, zeroTS, pool.alloc(), ""))
	...
	require.NoError(t, sink.EmitRow(ctx, topic(`t`), []byte(`5`), nil, zeroTS, zeroTS, pool.alloc(), ""))
