...
func TestKafkaSink(t *testing.T) {
	...
	// Timeout
	if err := sink.EmitRow(ctx, topic(`t`), []byte(`1`), nil, makeResource(), zeroTS); err != nil {
		t.Fatal(err)
	}
	m1 := <-p.inputCh
	for i := 0; i < 2; i++ {
	...
	r := makeResource(0)
	if err := sink.EmitRow(ctx, topic(`t`), []byte(`2`), nil, r.acquireResource(), zeroTS); err != nil {
		t.Fatal(err)
	}
	m2 := <-p.inputCh
	if err := sink.EmitRow(ctx, topic(`t`), []byte(`3`), nil, r.acquireResource(), zeroTS); err != nil {
		t.Fatal(err)
	}
	m3 := <-p.inputCh
	if err := sink.EmitRow(ctx, topic(`t`), []byte(`4`), nil, r.acquireResource(), zeroTS); err != nil {
		t.Fatal(err)
	}
	m4 := <-p.inputCh
	...
	if err := sink.EmitRow(ctx, topic(`t`), []byte(`5`), nil, r.acquireResource(), zeroTS); err != nil {
		t.Fatal(err)
	}
	m5 := <-p.inputCh
...
func TestKafkaSinkEscaping(t *testing.T) {
	...
	if err := sink.EmitRow(ctx, topic(`☃`), []byte(`k☃`), []byte(`v☃`), makeResource(), zeroTS); err != nil {
		t.Fatal(err)
	}
	m := <-p.inputCh
...
func TestKafkaTopicNameProvided(t *testing.T) {
	...
	require.NoError(t, sink.EmitRow(ctx, topic("particular0"), []byte(`k☃`), []byte(`v☃`), makeResource(), zeroTS))
	m := <-p.inputCh
...
func BenchmarkEmitRow(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		require.NoError(b, sink.EmitRow(ctx, topic, []byte(`k☃`), []byte(`v☃`), makeResource(), hlc.Timestamp{}))
	}
...
func TestSQLSink(t *testing.T) {
	...
	require.EqualError(t,
		sink.EmitRow(ctx, overrideTopic(`nope`), nil, nil, makeResource(), zeroTS),
		`cannot emit to undeclared topic: `)

	// With one row, nothing flushes until Flush is called.
	require.NoError(t, sink.EmitRow(ctx, fooTopic, []byte(`k1`), []byte(`v0`), makeResource(), zeroTS))
	...
	for i := 0; i < sqlSinkRowBatchSize+1; i++ {
		require.NoError(t,
			sink.EmitRow(ctx, fooTopic, []byte(`k1`), []byte(`v`+strconv.Itoa(i)), makeResource(), zeroTS))
	}
	...
	r := makeResource(0)
	require.NoError(t, sink.EmitRow(ctx, fooTopic, []byte(`kfoo`), []byte(`v0`), r.acquireResource(), zeroTS))
	require.NoError(t, sink.EmitRow(ctx, barTopic, []byte(`kbar`), []byte(`v0`), r.acquireResource(), zeroTS))
	require.NoError(t, sink.EmitRow(ctx, fooTopic, []byte(`kfoo`), []byte(`v1`), r.acquireResource(), zeroTS))
	require.NoError(t, sink.Flush(ctx))
	require.EqualValues(t, 0, r.used())
	...
	for i := 0; i < sqlSinkNumPartitions+1; i++ {
		require.NoError(t,
			sink.EmitRow(ctx, fooTopic, []byte(`v`+strconv.Itoa(i)), []byte(`v0`), makeResource(), zeroTS))
	}
	for i := 0; i < sqlSinkNumPartitions+1; i++ {
		require.NoError(t,
			sink.EmitRow(ctx, fooTopic, []byte(`v`+strconv.Itoa(i)), []byte(`v1`), makeResource(), zeroTS))
	}
	require.NoError(t, sink.Flush(ctx))
	...
	require.NoError(t, sink.EmitRow(ctx, fooTopic, []byte(`foo0`), []byte(`v0`), makeResource(), zeroTS))
...
func TestKafkaSinkTracksMemory(t *testing.T) {
	...
	r := makeResource(0)
	for i := 0; i < 10; i++ {
		require.NoError(t, sink.EmitRow(ctx, testTopic, key, val, r.acquireResource(), zeroTS))
	}
