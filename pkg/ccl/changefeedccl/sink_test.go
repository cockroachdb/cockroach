...
func (t *testResource) acquireResource() kvevent.Resource {
	atomic.AddInt32(t.n, 1)
	return t
}

func (t *testResource) used() int32 {
	return atomic.LoadInt32(t.n)
}

func (t *testResource) Release() {
	if t.moved {
		return
	}
	if t.used() == 0 {
...
func (t *testResource) Move() kvevent.Resource {
	t.moved = true
	return &testResource{n: t.n}
}

func makeResource(alloc ...int32) *testResource {
	var n int32 = 1
	if len(alloc) > 0 {
		n = alloc[0]
	}
	return &testResource{n: &n}
}
...
func TestKafkaSink(t *testing.T) {
	...
	// Timeout
	if err := sink.EmitRow(ctx, topic(`t`), []byte(`1`), nil, zeroTS, makeResource()); err != nil {
		t.Fatal(err)
	}
	m1 := <-p.inputCh
	for i := 0; i < 2; i++ {
	...
	r := makeResource(0)
	if err := sink.EmitRow(ctx, topic(`t`), []byte(`2`), nil, zeroTS, r.acquireResource()); err != nil {
		t.Fatal(err)
	}
	m2 := <-p.inputCh
	if err := sink.EmitRow(ctx, topic(`t`), []byte(`3`), nil, zeroTS, r.acquireResource()); err != nil {
		t.Fatal(err)
	}
	m3 := <-p.inputCh
	if err := sink.EmitRow(ctx, topic(`t`), []byte(`4`), nil, zeroTS, r.acquireResource()); err != nil {
		t.Fatal(err)
	}
	m4 := <-p.inputCh
	...
	if err := sink.EmitRow(ctx, topic(`t`), []byte(`5`), nil, zeroTS, r.acquireResource()); err != nil {
		t.Fatal(err)
	}
	m5 := <-p.inputCh
...
func TestKafkaSinkEscaping(t *testing.T) {
	...
	if err := sink.EmitRow(ctx, topic(`☃`), []byte(`k☃`), []byte(`v☃`), zeroTS, makeResource()); err != nil {
		t.Fatal(err)
	}
	m := <-p.inputCh
...
func TestKafkaTopicNameProvided(t *testing.T) {
	...
	require.NoError(t, sink.EmitRow(ctx, topic("particular0"), []byte(`k☃`), []byte(`v☃`), zeroTS, makeResource()))
	m := <-p.inputCh
...
func BenchmarkEmitRow(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		require.NoError(b, sink.EmitRow(ctx, topic, []byte(`k☃`), []byte(`v☃`), hlc.Timestamp{}, makeResource()))
	}
...
func TestSQLSink(t *testing.T) {
	...
	require.EqualError(t,
		sink.EmitRow(ctx, overrideTopic(`nope`), nil, nil, zeroTS, makeResource()),
		`cannot emit to undeclared topic: `)

	// With one row, nothing flushes until Flush is called.
	require.NoError(t, sink.EmitRow(ctx, fooTopic, []byte(`k1`), []byte(`v0`), zeroTS, makeResource()))
	...
	for i := 0; i < sqlSinkRowBatchSize+1; i++ {
		require.NoError(t,
			sink.EmitRow(ctx, fooTopic, []byte(`k1`), []byte(`v`+strconv.Itoa(i)), zeroTS, makeResource()))
	}
	...
	r := makeResource(0)
	require.NoError(t, sink.EmitRow(ctx, fooTopic, []byte(`kfoo`), []byte(`v0`), zeroTS, r.acquireResource()))
	require.NoError(t, sink.EmitRow(ctx, barTopic, []byte(`kbar`), []byte(`v0`), zeroTS, r.acquireResource()))
	require.NoError(t, sink.EmitRow(ctx, fooTopic, []byte(`kfoo`), []byte(`v1`), zeroTS, r.acquireResource()))
	require.NoError(t, sink.Flush(ctx))
	require.EqualValues(t, 0, r.used())
	...
	for i := 0; i < sqlSinkNumPartitions+1; i++ {
		require.NoError(t,
			sink.EmitRow(ctx, fooTopic, []byte(`v`+strconv.Itoa(i)), []byte(`v0`), zeroTS, makeResource()))
	}
	for i := 0; i < sqlSinkNumPartitions+1; i++ {
		require.NoError(t,
			sink.EmitRow(ctx, fooTopic, []byte(`v`+strconv.Itoa(i)), []byte(`v1`), zeroTS, makeResource()))
	}
	require.NoError(t, sink.Flush(ctx))
	...
	require.NoError(t, sink.EmitRow(ctx, fooTopic, []byte(`foo0`), []byte(`v0`), zeroTS, makeResource()))
...
func TestKafkaSinkTracksMemory(t *testing.T) {
	...
	r := makeResource(0)
	for i := 0; i < 10; i++ {
		require.NoError(t, sink.EmitRow(ctx, testTopic, key, val, zeroTS, r.acquireResource()))
	}
