...
func (e *versionEncoder) encodeKeyRaw(ctx context.Context, it cdcevent.Iterator) (json.JSON, error) {
...
func (e *versionEncoder) datumToJSON(ctx context.Context, d tree.Datum) (json.JSON, error) {
	...
	if e.encodeJSONValueNullAsObject {
		...
		} else if j.Type() == json.ObjectJSONType && j.Len() == 1 {
			// Check to see if this value would collide with our null object format, and if so, warn.
			v, err := j.FetchValKey(jsonNullAsObjectKey)
			if err != nil {
				return nil, err
			}
			if v != nil {
				if v, ok := v.AsBool(); ok && v {
					log.Warningf(ctx, "encountered value that collides with null object format: %q", j)
				}
			}
...
// builtin.
func EncodeAsJSONChangefeedWithFlags(ctx context.Context, r cdcevent.Row, flags ...string) ([]byte, error) {
