...
func (e *versionEncoder) rowAsGoNative(row cdcevent.Row, extras []keyValPair) (json.JSON, error) {
	if !row.HasValues() || row.IsDeleted() {
		return json.NullJSONValue, nil
	}

	if e.valueBuilder == nil {
		keys := make([]string, 0, len(row.ResultColumns())+len(extras))
	...
	for _, kv := range extras {
		if err := e.valueBuilder.Set(kv.k, kv.v); err != nil {
			return nil, err
		}
	}
...
func (e *jsonEncoder) initRawEnvelope() error {
	addMeta := e.updatedField || e.mvccTimestampField
	var metaBuilder *json.FixedKeysObjectBuilder
	var extraArgs []keyValPair
	e.envelopeEncoder = func(evCtx eventContext, updated, _ cdcevent.Row) (_ json.JSON, err error) {
		...
		if metaBuilder == nil {
			metaBuilder, err = json.NewFixedKeysObjectBuilder([]string{"mvcc_timestamp", "updated"})
			if err != nil {
				return nil, err
			}
			extraArgs = []keyValPair{{k: jsonMetaSentinel}}
		}
		...
		extraArgs[0].v, err = metaBuilder.Build()
		if err != nil {
			return nil, err
		}
		return ve.rowAsGoNative(updated, extraArgs)
	}
	return nil
}
...
func (e *jsonEncoder) initWrappedEnvelope() error {
	...
	e.envelopeEncoder = func(evCtx eventContext, updated, prev cdcevent.Row) (json.JSON, error) {
		after, err := e.versionEncoder(updated.EventDescriptor).rowAsGoNative(updated, nil)
		if err != nil {
			return nil, err
		}
		...
		if e.keyInValue {
			keyEntries, err := e.versionEncoder(updated.EventDescriptor).encodeKeyRaw(updated)
			if err != nil {
				return nil, err
			}
			if err := b.Set("key", keyEntries); err != nil {
				return nil, err
			}
		}
