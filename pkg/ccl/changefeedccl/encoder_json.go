...
func makeJSONEncoder(
	ctx context.Context, opts jsonEncoderOptions, sourceOpts enrichedEnvelopeSourceProviderOpts,
) (*jsonEncoder, error) {
	...
	e := &jsonEncoder{
		...
		enrichedEnvelopeSourceProvider: newEnrichedEnvelopeSourceProvider(sourceOpts),
	}
...
func (e *jsonEncoder) initEnrichedEnvelope(ctx context.Context) error {
	...
	e.envelopeEncoder = func(evCtx eventContext, updated, prev cdcevent.Row) (json.JSON, error) {
		...
		if err := payloadBuilder.Set("source", e.enrichedEnvelopeSourceProvider.GetJSON(updated)); err != nil {
			return nil, err
		}
