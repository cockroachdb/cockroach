...
func makeWebhookSinkClient(
	...
	if opts.Compression != "" {
		algo, _, err := compressionFromString(strings.ToLower(opts.Compression))
		if err != nil {
			return nil, errors.Wrap(err, "unsupported compression type")
		}
		compression = algo
	}
...
func (sc *webhookSinkClient) makePayloadForBytes(body []byte) (SinkPayload, error) {
	...
	if sc.compression.enabled() {
		...
		codec, done, err := newCompressionCodec(sc.compression, &sc.settings.SV, &buf)
		if err != nil {
...
func (sc *webhookSinkClient) readResponseBody(res *http.Response) ([]byte, error) {
	...
	reader, done, err := newDecompressionReader(algo, res.Body)
	if err != nil {
		return nil, errors.Wrapf(err,
