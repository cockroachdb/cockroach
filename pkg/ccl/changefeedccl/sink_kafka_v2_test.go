...
func TestProducerBatchMaxBytes(t *testing.T) {
	...
	tests := []struct {
		...
		// as ProducerBatchMaxBytes in the json.
		perChangefeed int
		...
		{
			name:      "default",
			wantValue: int32(changefeedbase.KafkaProducerBatchMaxBytesLimit),
		},
		{
		...
		},
		{
			...
			perChangefeed: changefeedbase.KafkaProducerBatchMinBytes,
			wantValue:     int32(changefeedbase.KafkaProducerBatchMinBytes),
		},
		{
			name:          "boundary_maximum_accepted",
			perChangefeed: changefeedbase.KafkaProducerBatchMaxBytesLimit,
			wantValue:     int32(changefeedbase.KafkaProducerBatchMaxBytesLimit),
		},
		{
		...
		},
		{
			...
			wantErrContains: fmt.Sprintf("at most %d", changefeedbase.KafkaProducerBatchMaxBytesLimit),
		},
	}
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.perChangefeed != 0 {
				jsonBs, err := json.Marshal(map[string]any{
					"ProducerBatchMaxBytes": tc.perChangefeed,
				})
				require.NoError(t, err)
