...
func TestJsonRountrip(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			dab := rowenc.EncDatumRow{rowenc.EncDatum{Datum: tree.NewDInt(1)}, rowenc.EncDatum{Datum: test.datum}}

			row := cdcevent.TestingMakeEventRow(tableDesc, 0, dab, false)
			...
			// datum as JSON.
			bytes, err := encoder.EncodeValue(context.Background(), eventContext{}, row, cdcevent.Row{})
			require.NoError(t, err)
