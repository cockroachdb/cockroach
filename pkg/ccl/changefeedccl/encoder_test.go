...
func TestEncoders(t *testing.T) {
	...
	for _, o := range opts {
		...
		t.Run(name, func(t *testing.T) {
			...
			e, err := getEncoder(context.Background(), o, targets, false, nil, nil, enrichedEnvelopeSourceDataSource{
				jobId: "testJobId",
			})
			require.NoError(t, err)
...
func TestAvroEncoderWithTLS(t *testing.T) {
	...
	for _, setClientCert := range []bool{true, false} {
		t.Run(fmt.Sprintf("setClientCert=%t", setClientCert), func(t *testing.T) {
			...
			e, err := getEncoder(context.Background(), opts, targets, false, nil, nil, enrichedEnvelopeSourceDataSource{
				jobId: "testJobId",
			})
			require.NoError(t, err)
			...
			enc, err := getEncoder(context.Background(), opts, targets, false, nil, nil, enrichedEnvelopeSourceDataSource{
				jobId: "testJobId",
			})
			require.NoError(t, err)
...
func BenchmarkEncoders(b *testing.B) {
	...
	bench := func(b *testing.B, fn encodeFn, opts changefeedbase.EncodingOptions, updatedRows, prevRows []cdcevent.Row) {
		...
		encoder, err := getEncoder(context.Background(), opts, targets, false, nil, nil, enrichedEnvelopeSourceDataSource{
			jobId: "testJobId",
		})
		if err != nil {
			b.Fatal(err)
...
func TestJsonRountrip(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			encoder, err := makeJSONEncoder(context.Background(), opts, enrichedEnvelopeSourceDataSource{
				jobId: "testJobId",
			})
			require.NoError(t, err)
