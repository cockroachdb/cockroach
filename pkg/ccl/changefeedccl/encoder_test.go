...
func TestAvroEncoderWithTLS(t *testing.T) {
	...
	t.Run("format=experimental_avro,envelope=key_only", func(t *testing.T) {
		...
		require.NoError(t, err)
		defer reg.Close()
		opts[changefeedbase.OptConfluentSchemaRegistry] = reg.URL()
		rowStringFn = func(k, v []byte) string {
			key, value := avroToJSON(t, reg, k), avroToJSON(t, reg, v)
			return fmt.Sprintf(`%s->%s`, key, value)
		}
		...
		params := url.Values{}
		params.Add("registry_tls_enabled", "true")
		params.Add("registry_ca_cert", certBase64)

		u, err := url.Parse(fmt.Sprintf("kafka://test?%s", params.Encode()))
		require.NoError(t, err)

		e, err := getEncoder(opts, targets, &sinkURL{URL: u})
		require.NoError(t, err)
		...
		tlsEncoderNoCert, err := getEncoder(opts, targets, &sinkURL{URL: u})
		require.NoError(t, err)
		...
		wrongTLSEncoder, err := getEncoder(opts, targets, &sinkURL{URL: u})
		require.NoError(t, err)
