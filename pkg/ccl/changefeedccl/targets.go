...
type nameQualification bool

const (
	unqualifiedName    nameQualification = false
	fullyQualifiedName nameQualification = true
)
...
func resolveChangefeedTargets(
	...
	forEachResolved func(name tree.TablePattern, desc catalog.TableDescriptor) error,
) error {
	...
	for _, desc := range targetDescs {
		if table, isTable := desc.(catalog.TableDescriptor); isTable {
			...
			name, err := getChangefeedTargetName(
				ctx, table, *p.ExecCfg(), p.ExtendedEvalContext().Txn, bool(qualification))
			if err != nil {
				return err
			}
			...
			if err := forEachResolved(name, table); err != nil {
				return err
			}
		}
	}
...
func getChangefeedTargetName(
	...
	return tree.NewUnqualifiedTableName(tree.Name(desc.GetName())), nil
}
