...
var KafkaValidOptions = makeStringSet(OptAvroSchemaPrefix, OptConfluentSchemaRegistry, OptKafkaSinkConfig, OptHeadersJSONColumnName, OptHeaders)
...
var WebhookValidOptions = makeStringSet(OptWebhookAuthHeader, OptWebhookClientTimeout, OptWebhookSinkConfig, OptCompression, OptHeaders)
...
func (s StatementOptions) GetWebhookSinkOptions() (WebhookSinkOptions, error) {
	...
	headersMap, err := parseHeaders(s.m[OptHeaders])
	if err != nil {
		return o, err
	}
	o.AdditionalHeaders = headersMap
	return o, nil
}
...
func parseHeaders(headers string) (map[string]string, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "failed to parse headers")
	}
	it, err := headersJ.ObjectIter()
	if err != nil {
		return nil, errors.Wrap(err, "failed to parse headers as object")
	}
	headersMap := make(map[string]string, headersJ.Len())
	for it.Next() {
		k := it.Key()
		v := it.Value()
		s, err := v.AsText()
		if err != nil {
			return nil, errors.Wrap(err, "failed to parse header value as text")
		}
		if s == nil {
			continue
		}
...
func (s StatementOptions) GetKafkaSinkOptions() (KafkaSinkOptions, error) {
	...
	for k, v := range headersMap {
		if len(v) > 1 {
			return KafkaSinkOptions{}, errors.Newf(`multiple values for header %s`, k)
		}
	}
...
// // GetKafkaSinkOptions returns arbitrary json to be interpreted
// // by the kafka sink.
// func (s StatementOptions) GetKafkaSinkOptions() SinkSpecificJSONConfig {
