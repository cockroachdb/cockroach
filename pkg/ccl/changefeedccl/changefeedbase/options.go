...
func (s StatementOptions) GetRangeDistributionStrategy() (RangeDistributionStrategy, bool, error) {
	v, err := s.getEnumValue(OptRangeDistributionStrategy)
	if err != nil {
		return "", false, err
	}
	if v == `` {
		return RangeDistributionStrategyDefault, false, nil
	}
	return RangeDistributionStrategy(v), true, nil
}
