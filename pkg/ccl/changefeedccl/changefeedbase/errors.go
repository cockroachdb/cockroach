...
func AsTerminalError(ctx context.Context, lm *lease.Manager, cause error) (termErr error) {
	defer func() {
		log.Infof(ctx, "AsTerminalError for %s returning %s", cause, termErr)
	}()
	if cause == nil {
		return nil
	}
...
// CategorizeError tries to determine if the passed in error should
// be treated as a terminal error.
