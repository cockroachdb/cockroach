...
type parquetCloudStorageSink struct {
	...
	writers     map[*cloudStorageSinkFile]parquet.Writer
}
...
func makeParquetCloudStorageSink(
	...
	parquetSink := &parquetCloudStorageSink{
		...
		writers:     make(map[*cloudStorageSinkFile]parquet.Writer),
	}
...
func (parquetSink *parquetCloudStorageSink) EncodeAndEmitRow(
	...
	if file.parquetCodec == nil {
		var err error
		file.parquetCodec, err = newParquetWriterFromRow(updatedRow, &file.buf,
...
func makeParquetWriter(
	row cdcevent.Row, buf *bytes.Buffer, compression parquet.CompressionCodec,
) (*parquet.Writer, error) {
	schemaDef, err := newParquetSchemaDefinitionFromRow(row)
	if err != nil {
		return nil, err
	}

	return parquet.NewWriter(schemaDef, buf, parquet.WithCompressionCodec(compression))
...
func newParquetSchemaDefinitionFromRow(row cdcevent.Row) (*parquet.SchemaDefinition, error) {
	typs := make([]*types.T, 0)
	names := make([]string, 0)
	...
	names = append(names, parquetCrdbEventTypeColName)
	return parquet.NewSchema(names, typs)
}
