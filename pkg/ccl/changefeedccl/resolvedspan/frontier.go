...
// TimestampAtBoundary returns whether a timestamp is at the latest resolved
// span boundary, and if so, what the boundary type is.
func (f *resolvedSpanFrontier) TimestampAtBoundary(
	ts hlc.Timestamp,
) (bool, jobspb.ResolvedSpan_BoundaryType) {
	return f.boundary.At(ts)
}

// ReachedBoundary returns true at the single moment when all watched spans
// have reached a boundary and no spans after the boundary have been received.
func (f *resolvedSpanFrontier) ReachedBoundary() (
	bool,
