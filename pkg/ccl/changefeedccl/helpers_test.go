...
func checkStructuredLogs(t *testing.T, eventType string, startTime int64) []string {
	...
	testutils.SucceedsSoon(t, func() error {
		...
		for _, e := range entries {
			...
			err = gojson.Unmarshal(jsonPayload, &baseStruct)
...
func checkCreateChangefeedLogs(t *testing.T, startTime int64) []eventpb.CreateChangefeedQuery {
	var matchingEntries []eventpb.CreateChangefeedQuery

	for _, m := range checkStructuredLogs(t, "create_changefeed_query", startTime) {
		jsonPayload := []byte(m)
		var event eventpb.CreateChangefeedQuery
