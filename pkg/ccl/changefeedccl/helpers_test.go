...
func sinklessTest(testFn func(*testing.T, *gosql.DB, testfeedFactory)) func(*testing.T) {
	return func(t *testing.T) {
		...
		sqlDB.Exec(t, `SET CLUSTER SETTING sql.defaults.conn_results_buffer_size = '0'`)
		sqlDB.Exec(t, `CREATE DATABASE d`)

		// Now that we've updated sql.defaults.conn_results_buffer_size, open a new
		// conn pool so that connection use the new setting.
