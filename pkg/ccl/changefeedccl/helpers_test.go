...
func withAllowChangefeedErr(reason string /*for documentation only*/) feedTestOption {
	return func(opts *feedTestOptions) {
...
func requireNoFeedsFail(t *testing.T) (fn updateKnobsFn) {
	...
	fn = func(knobs *base.TestingKnobs) {
		...
		handleErr := func(err error) error {
			...
			t.Logf("handleErr: %v", err) // DBG
			if shouldIgnoreErr(err) {
				t.Logf("requireNoFeedsFail: ignoring error: %v", err) // DBG
				return nil
			}
