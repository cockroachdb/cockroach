...
func watchdog(timeout time.Duration) (cancel func(), wait func()) {
	...
	go func() {
		...
		case <-time.After(timeout):
			panic(fmt.Sprintf("test timeout\n%s", debug.Stack()))
		case <-cancelCh:
			return
		}
	}()
	return func() { close(cancelCh) }, wg.Wait
}
	...
	return func(t *testing.T) {
		...
		setupFeed := func() (feedCh chan *sarama.ProducerMessage, cleanup func()) {
			...
			return fs.feedCh, func() {
				cancel, wait := watchdog(30 * time.Second)
				defer wait()
