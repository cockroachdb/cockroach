...
func readNextMessages(
	t testing.TB, f cdctest.TestFeed, numMessages int, withLeaveTs bool,
) []string {
	t.Helper()
	...
	for len(actual) < numMessages {
		...
		if err != nil {
			...
			if withLeaveTs {
				...
				msg = fmt.Sprintf(`%s: %s->%s`, m.Topic, m.Key, m.Value)
			}
...
func assertPayloadsBase(t testing.TB, f cdctest.TestFeed, expected []string, withLeaveTs bool) {
	t.Helper()
	actual := readNextMessages(t, f, len(expected), withLeaveTs)
