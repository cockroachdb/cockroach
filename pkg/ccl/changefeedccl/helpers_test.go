...
var changefeedDuplicateNoticeRegex = regexp.MustCompile(`One or more changefeed jobs are running with the same table\(s\) and sink URI: job ID (\d+),?`)

// createChangefeedCheckDuplicateNotices is used to 1. create a changefeed job, then 2. check for duplicate changefeed notices that occured in that creation.
// If multiple notices returned, it checks all to see that the expected notice is present
// exactly once.
func createChangefeedAndCheckDuplicateNotices(
	t *testing.T,
	...
	if noticeExpected {
		if expectedJobID != jobspb.InvalidJobID {
			checkNotice = func(notice string) bool {
				matches := changefeedDuplicateNoticeRegex.FindStringSubmatch(notice)
				if matches == nil {
					return false
				}
