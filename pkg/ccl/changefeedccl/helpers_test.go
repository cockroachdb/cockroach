...
func enterpriseTest(testFn func(*testing.T, *gosql.DB, testfeedFactory)) func(*testing.T) {
	return func(t *testing.T) {
		...
		// value because otherwise schema changes become flaky (they don't
		// commit their txn in time, get pushed by closed timestamps, and retry
		// forever). This is more likely when the tests run slower (race builds
		// or inside docker). The conservative value makes our tests take a long
		// longer, though. Figure out some way to speed this up.
