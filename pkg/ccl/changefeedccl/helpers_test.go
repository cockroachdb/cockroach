...
func checkContinuousChangefeedLogs(t *testing.T, startTime int64) []eventpb.ChangefeedEmittedBytes {
	...
	for i, m := range checkStructuredLogs(t, "changefeed_emitted_bytes", startTime) {
		jsonPayload := []byte(m)
	...
	// Assert entries are sorted. If this assertion fails, logs are being read out of order and
	// we must implement a sort here.
