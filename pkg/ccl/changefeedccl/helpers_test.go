...
type feedTestOptions struct {
	...
	includeSinkless   bool // separate flag since some tests assume existence of a job
}
...
var feedTestForceTenant = func(opts *feedTestOptions) { opts.useTenant = true }
var _ = feedTestForceTenant // silence unused warning
...
var feedTestRestrictSinks = func(sinkTypes ...string) feedTestOption {
	return func(opts *feedTestOptions) { opts.allowedSinkTypes = sinkTypes }
}

var feedTestOmitSinks = func(sinkTypes ...string) feedTestOption {
	return func(opts *feedTestOptions) { opts.disabledSinkTypes = sinkTypes }
}
var _ = feedTestOmitSinks // silence unused warning
...
func expectNotice(t *testing.T, s serverutils.TestTenantInterface, sql string, expected string) {
	url, cleanup := sqlutils.PGUrl(t, s.SQLAddr(), t.Name(), url.User(security.RootUser))
	defer cleanup()
...
// of a test running as the system tenant or a secondary tenant, while still
// having access to system interfaces for operations that require them such as
...
) (testServer TestServer, cleanup func()) {
...
func makeServer(t *testing.T, opts ...feedTestOption) (testServer TestServer, cleanup func()) {
	options := makeOptions(opts...)
...
func cdcTestDesc(t *testing.T, desc string, testFn cdcTestFn, testOpts ...feedTestOption) {
	t.Helper()
	options := makeOptions(testOpts...)
