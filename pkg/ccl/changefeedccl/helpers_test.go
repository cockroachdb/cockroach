...
func (c *tableFeed) Next(
	...
	// by repeatedly generating a new message id high-water and selecting every
	// row between the previous high-water and the new one. Then it pages
	...
	// returning the error. The last poll's highwater is guaranteed to be
	// greater than any thing written by the sink before the error was set
	// because everything uses the same machine's clock.
	for {
		...
		newMessageID := int64(builtins.GenerateUniqueInt(0))
		var err error
		c.rows, err = c.db.Query(
			`SELECT * FROM sqlsink WHERE message_id > $1 AND message_id <= $2 `+
				`ORDER BY PRIMARY KEY sqlsink`,
			c.lastMessageID, newMessageID)
		if err != nil {
			t.Fatal(err)
		}
		c.lastMessageID = newMessageID
