...
func TestBuildAzureKafkaConfig(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			uri:                     "azure-event-hub://wenyieventhubs.servicebus.windows.net:9093?shared_access_key_name=saspolicyhistory&shared_access_key=q%2BSecretRedacted%3D",
			expectedKafkaDialConfig: makeKafkaDialConfig("wenyieventhubs.servicebus.windows.net", "saspolicyhistory", "q+SecretRedacted="),
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
