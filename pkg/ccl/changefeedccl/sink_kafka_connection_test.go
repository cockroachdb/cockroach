...
func TestChangefeedExternalConnections(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			name:          "sasl_mechanism must be provided when AWS IAM authentication is enabled",
			uri:           "kafka://nope/?sasl_enabled=true&sasl_aws_iam_role_enabled=true&sasl_aws_iam_session_name=foo&sasl_aws_iam_session_name=foo",
			expectedError: "sasl_mechanism must be OAUTHBEARER when AWS IAM role authentication is enabled",
		},
