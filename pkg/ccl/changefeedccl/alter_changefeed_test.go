...
func TestAlterChangefeedAddTargetsDuringSchemaChangeError(t *testing.T) {
	...
	testFn := func(t *testing.T, db *gosql.DB, f cdctest.TestFeedFactory) {
		...
		sqlDB.ExpectErr(t,
			`cannot perform initial scan on newly added targets during a non-initial backfill, please wait for the backfill to complete before adding these targets.`,
			fmt.Sprintf(`ALTER CHANGEFEED %d ADD bar WITH initial_scan`, jobFeed.JobID()),
		)
	}
