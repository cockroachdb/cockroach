...
func TestAlterChangefeedRandomizedTargetChanges(t *testing.T) {
	...
	testFn := func(t *testing.T, s TestServer, f cdctest.TestFeedFactory) {
		...
		createStmt := fmt.Sprintf(`CREATE CHANGEFEED FOR %s WITH updated`, strings.Join(initialTables, ", "))
		t.Log(createStmt)
		...
		for i := range numAlters {
			...
			numAdds := rand.Intn(len(nonWatchedTables) + 1)
			numDrops := rand.Intn(len(watchedTables))
			if numAdds == 0 && numDrops == 0 {
				t.Logf("skipping alter #%d", i+1)
				continue
			}
			adds := getNFromSet(nonWatchedTables, numAdds)
			drops := getNFromSet(watchedTables, numDrops)

			var expectedRows []string
			for len(adds) > 0 || len(drops) > 0 {
				// Randomize the order of adds and drops.
				switch add := len(adds) > 0 && (len(drops) == 0 || rnd.Intn(2) == 0); add {
				case true: // Add a target.
					addTarget := getOneFromSet(adds)
					delete(adds, addTarget)
				...
				case false: // Drop a target.
					dropTarget := getOneFromSet(drops)
					delete(drops, dropTarget)
...
func setDifference[K comparable](s map[K]struct{}, t map[K]struct{}) map[K]struct{} {
...
func getOneFromSet[K comparable](s map[K]struct{}) K {
	for e := range s {
		return e
	}
	panic("set empty")
...
// getNFromSet returns a new set with n random elements from s.
func getNFromSet[K comparable](s map[K]struct{}, n int) map[K]struct{} {
	if n == 0 {
		return nil
	}
	...
	panic(fmt.Sprintf("not enough elements in set, wanted %d, found %d", n, len(m)))
}
