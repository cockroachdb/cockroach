...
func TestAlterChangefeedInitialScanBothSyntax(t *testing.T) {
	...
	testutils.RunTrueAndFalse(t, "initial_scan", func(t *testing.T, val bool) {
		initialScanOpt := "initial_scan"
		if val {
			initialScanOpt += " = 'yes'"
		}
...
func TestAlterChangefeedNoInitialScanBothSyntax(t *testing.T) {
	...
	testutils.RunTrueAndFalse(t, "initial_scan", func(t *testing.T, val bool) {
		var initialScanOpt string
		if val {
			initialScanOpt = "no_initial_scan"
		} else {
			initialScanOpt = "initial_scan = 'no'"
		}
		t.Run(`kafka`, kafkaTest(testFn(initialScanOpt)))
	})
}