...
func TestAlterChangefeedAddTargetPrivileges(t *testing.T) {
	...
	t.Run("using-changefeed-grant", func(t *testing.T) {
		...
		// With enforce_external_connections enabled, the user requires USAGE on the external connection.
		rootDB.Exec(t, "SET CLUSTER SETTING changefeed.permissions.enforce_external_connections = true")
		withUser(t, "user1", func(userDB *sqlutils.SQLRunner) {
			userDB.ExpectErr(t,
		...
		rootDB.Exec(t, "SET CLUSTER SETTING changefeed.permissions.enforce_external_connections = false")
	})
...
func TestAlterChangefeedAccessControl(t *testing.T) {
	...
	asUser(t, f, `jobController`, func(userDB *sqlutils.SQLRunner) {
		userDB.ExpectErr(t, "pq: user jobcontroller does not have CHANGEFEED privilege on relation table_a", fmt.Sprintf(`ALTER CHANGEFEED %d ADD table_b`, currentFeed.JobID()))
	})
	...
	asUser(t, f, `regularUser`, func(userDB *sqlutils.SQLRunner) {
		userDB.ExpectErr(t, "pq: user regularuser does not have CHANGEFEED privilege on relation table_a", fmt.Sprintf(`ALTER CHANGEFEED %d ADD table_b`, currentFeed.JobID()))
	})
	closeCf()
