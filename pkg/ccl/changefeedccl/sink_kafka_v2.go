...
func newKafkaSinkClientV2(
	...
	baseOpts := []kgo.Opt{
		...
		// Allow multiple concurrent produce requests per broker. This is safe
		// because ParallelIO handles ordering independently of franz-go.
...
func makeKafkaSinkV2(
	...
	client, err := newKafkaSinkClientV2(ctx, clientOpts, batchCfg,
		u.Host, settings, knobs, mb, topicsForConnectionCheck, partitionAlg)
	if err != nil {
		return nil, err
	}
