...
func buildKgoConfig(
	...
	if sinkCfg.Flush.MaxRequestSize != 0 {
		if sinkCfg.Flush.MaxRequestSize < changefeedbase.KafkaMaxRequestSizeMin {
			return nil, errors.Errorf(
				"MaxRequestSize must be at least %d bytes, got %d",
				changefeedbase.KafkaMaxRequestSizeMin, sinkCfg.Flush.MaxRequestSize)
		}
		if sinkCfg.Flush.MaxRequestSize > changefeedbase.KafkaMaxRequestSizeLimit {
			return nil, errors.Errorf(
				"MaxRequestSize must be at most %d bytes, got %d",
				changefeedbase.KafkaMaxRequestSizeLimit, sinkCfg.Flush.MaxRequestSize)
		}
		// Override the cluster setting default set in baseOpts; kgo uses last-writer-wins.
		opts = append(opts, kgo.ProducerBatchMaxBytes(sinkCfg.Flush.MaxRequestSize))
	}
