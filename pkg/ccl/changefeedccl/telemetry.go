...
// A sinkTelemetryDataStore is used to track metrics destined to be
// logged as telemetry.
type sinkTelemetryDataStore interface {
	// incEmittedBytes atomically adds numBytes to the running count of emitted bytes.
	incEmittedBytes(numBytes int)
	// resetEmittedBytes atomically resets running count of emitted bytes and
	// returns the old count.
	resetEmittedBytes() int
}

var _ sinkTelemetryDataStore = (*sinkTelemetryData)(nil)

type sinkTelemetryData struct {
	emittedBytes *atomic.Int64
}

var noopTelemetryDataStore = (*sinkTelemetryData)(nil)
...
// resetEmittedBytes implements the sinkTelemetryDataStore interface.
...
func (ptl *periodicTelemetryLogger) maybeLogTelemetry(
	...
	loggingInterval := changefeedbase.ContinuousTelemetryInterval.Get(&s.SV).Nanoseconds()
	...
	if newResolved.WallTime-ptl.lastResolved.WallTime >= loggingInterval {
		...
		if knobs.ContinuousTelemetryLogCallback != nil {
			knobs.ContinuousTelemetryLogCallback(ptl.lastResolved, newResolved)
		}
