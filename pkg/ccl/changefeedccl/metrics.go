...
func (m *sliMetrics) clearId(id int) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *sliMetrics) setResolved(id int, ts hlc.Timestamp) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.mu.resolved[id] = ts
}

func (m *sliMetrics) setCheckpoint(id int, ts hlc.Timestamp) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.mu.checkpoint[id] = ts
}

func (m *sliMetrics) claimId() int {
	m.mu.Lock()
	defer m.mu.Unlock()
	id := m.mu.id
	m.mu.id++
