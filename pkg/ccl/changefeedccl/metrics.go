...
var maxSLIScopes = settings.RegisterIntSetting(
	"changefeed.monitoring.max_sli_scopes",
	"maximum number of custom SLI scopes; 0 disables SLI scopes;"+
		"server.child_metrics.enabled setting must be enabled to export scoped metrics to prometheus",
	0,
)
...
// keeping track of all cahngefeeds which did not have explicit sli scope specified.
...
type sliMetrics struct {
	...
	// backfillTS is an atomic.Value storing the hlc.Timestamp of the backfill event.
	// it is set when backfill completes to the timestamp of backfill.
	backfillTS atomic.Value
}
...
func (m *sliMetrics) recordEmittedBatch(
	...
	if backfillTS, ok := m.backfillTS.Load().(hlc.Timestamp); ok && backfillTS.Less(mvcc) {
...
func (m *sliMetrics) getBackfillCallback() func() func(t hlc.Timestamp) {
	return func() func(t hlc.Timestamp) {
		m.BackfillCount.Inc(1)
		return func(t hlc.Timestamp) {
			m.backfillTS.Store(t)
...
func newAggregateMetrics(histogramWindow time.Duration) *AggMetrics {
	...
	_, err := a.getOrCreateScope(defaultSLIScope, nil)
	if err != nil {
...
func (a *AggMetrics) getOrCreateScope(scope string, sv *settings.Values) (*sliMetrics, error) {
	a.mu.Lock()
	defer a.mu.Unlock()
	...
	if scope != defaultSLIScope {
		if maxScopes := maxSLIScopes.Get(sv); int64(len(a.mu.sliMetrics)) > maxScopes {
			return nil, errors.WithHint(
				pgerror.Newf(pgcode.ConfigurationLimitExceeded,
					"cannot create scope %q; maximum number of scopes is %d", scope, maxScopes),
				"try increasing changefeed.monitoring.max_sli_scopes setting",
			)
		}
	}
...
func (m *Metrics) getSLIMetrics(scope string, sv *settings.Values) (*sliMetrics, error) {
	return m.AggMetrics.getOrCreateScope(scope, sv)
}
