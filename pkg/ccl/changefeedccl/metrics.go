...
func newJobScopedUsageMetrics(histogramWindow time.Duration) *JobScopedUsageMetrics {
	...
	m.m.metrics = make(map[catpb.JobID]*innerJobScopedUsageMetrics)
...
func (m *JobScopedUsageMetrics) UpdateTableBytes(
	...
	if _, ok := m.m.metrics[jobID]; !ok {
		m.m.metrics[jobID] = &innerJobScopedUsageMetrics{}
	}
...
var (
	...
	metaChangefeedUsageErrorCount = metric.Metadata{
		...
		Help:        "Count of errors generating usage metrics for changefeeds",
		Measurement: "Errors",
		Unit:        metric.Unit_COUNT,
	}
	metaChangefeedUsageQueryDuration = metric.Metadata{
		Name:        "changefeed.usage.query_duration",
		Help:        "Time taken by the queries to generate usage metrics for changefeeds",
		Measurement: "Nanoseconds",
		Unit:        metric.Unit_NANOSECONDS,
	}
)
