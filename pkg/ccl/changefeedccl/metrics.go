...
type AggMetrics struct {
	...
	BackfillRangeCount *aggmetric.AggGauge
...
type sliMetrics struct {
	...
	BackfillRangeCount *aggmetric.Gauge
...
// and returns a second callback to decrement the value until all ranges have
// been emitted
func (m *sliMetrics) getBackfillRangeCallback() func(int64) func() {
	return func(initial int64) func() {
		remaining := initial
		m.BackfillRangeCount.Inc(initial)
		return func() {
			if atomic.LoadInt64(&remaining) > 0 {
				atomic.AddInt64(&remaining, -1)
				m.BackfillRangeCount.Dec(1)
			}
		}
	}
}
...
func newAggregateMetrics(histogramWindow time.Duration) *AggMetrics {
	...
	metaChangefeedBackfillRangeCount := metric.Metadata{
		Name:        "changefeed.backfill_range_count",
		Help:        "Number of ranges that are pending backfill",
		Measurement: "Count",
		Unit:        metric.Unit_COUNT,
	}
	...
	a := &AggMetrics{
		...
		BackfillRangeCount: b.Gauge(metaChangefeedBackfillRangeCount),
...
func (a *AggMetrics) getOrCreateScope(scope string) (*sliMetrics, error) {
	...
	sm := &sliMetrics{
		...
		BackfillRangeCount: a.BackfillRangeCount.AddChild(scope),
