...
func decimalToRat(dec apd.Decimal, scale int32) (big.Rat, error) {
	if dec.Form != apd.Finite {
		return big.Rat{}, errors.Errorf(`cannot convert %s form decimal: %s`, dec.Form, &dec)
	}
	if e := -dec.Exponent; e > scale {
		return big.Rat{}, errors.Errorf(`%s has more than %d digits after the decimal`, &dec, scale)
	}
