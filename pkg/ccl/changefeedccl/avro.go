...
func (r *avroEnvelopeRecord) BinaryFromRow(
	...
	if r.opts.updatedField {
		...
		if u, ok := meta[`updated`]; ok {
			...
			native[`updated`] = goavro.Union(avroUnionKey(avroSchemaString), nativeFromHLC(ts))
		}
	}
	if r.opts.resolvedField {
		...
		if u, ok := meta[`resolved`]; ok {
			...
			native[`resolved`] = goavro.Union(avroUnionKey(avroSchemaString), nativeFromHLC(ts))
		}
	}
...
func nativeFromHLC(ts hlc.Timestamp) interface{} {
	return tree.TimestampToDecimal(ts).Decimal.String()
}
