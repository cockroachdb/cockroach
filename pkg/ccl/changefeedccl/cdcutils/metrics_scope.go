...
// SLIScope represents a set of SLI related metrics for a particular "scope".
type SLIScope struct {
	Scopes [maxSLIScopes]*SLIMetrics // Exported so that we can register w/ metrics registry.
	names  map[string]*SLIMetrics
}

// MetricStruct implements metric.Struct interface
func (*SLIScope) MetricStruct() {}

// NewSLIScope creates changefeed specific SLI scope: a metric.Struct containing
...
func (s *SLIScope) GetSLIMetrics(scopeName string) *SLIMetrics {
