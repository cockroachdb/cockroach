...
func withSchedulerHelper(sh *testHelper) func(opts *feedTestOptions) {
	return func(opts *feedTestOptions) {
		opts.knobsFn = func(knobs *base.TestingKnobs) {
			...
			// These knobs are needed so that SQL stats compaction schedule job doesn't produce
			// surprising stack traces when executed by this test.
...
`,
			...
			es: expectedSchedule{
				nameRe: "foo-export",
				// AS OF TIME clause ignored when creating export schedules.
...
`)
	...
	getFeed := func() (string, cdctest.TestFeed, func()) {
		...
		// NB: This is partially initialized feed.  It doesn't have feed
	...
	t.Run("one-table", func(t *testing.T) {
		...
		// Wait for the backup complete and verify export.
