...
func TestProtoEncoder_BareEnvelope_WithMetadata(t *testing.T) {
	...
	require.Equal(t, evCtx.updated.AsOfSystemTime(), bare.Metadata.Updated)
	require.Equal(t, evCtx.mvcc.AsOfSystemTime(), bare.Metadata.MvccTimestamp)
	require.NotNil(t, bare.Metadata.Key)
	assert.Equal(t, "test-topic", bare.Metadata.Topic)
}
...
func TestProtoEncoder_ResolvedEnvelope(t *testing.T) {
	...
	res := msg.GetResolved()
	log.Infof(context.Background(), "Decoded resolved message:\n%s", proto.MarshalTextString(res))
