...
func Test_ProtoEncoderAllTypes(t *testing.T) {
	...
	overrideRandGen = func(typ *types.T) (tree.Datum, *types.T) {
		...
		case types.TimestampFamily, types.TimestampTZFamily, types.DateFamily:
			// clamp into Protobuf-supported range:
		...
		case types.ArrayFamily:
			...
			for i := 0; i < numElems; i++ {
				// recurse into overrideRandGen for each element
				elemDatum, _ := overrideRandGen(elemTyp)
				if elemDatum == nil {
					// fallback to default random for anything else
	...
	for _, typ := range typesToTest {
		// generating the random datums
		// generate a datum, clamping timestamps (and arrays thereof) via overrideRandGen
		datum, usedTyp := overrideRandGen(typ)
		if datum == nil {
			datum = randgen.RandDatum(rng, typ, false)
		}
		if datum == tree.DNull { //datum can still be null if type is unknown
			continue // skip nulls
		}
...
func generateClampedTemporal(rng *rand.Rand, family types.Family) (tree.Datum, error) {
	...
	switch family {
	case types.DateFamily:
		pgd, _ := pgdate.MakeDateFromTime(t) // time component is ignored
...
func skipType(typ *types.T) bool {
	// 1) Exclude all Cockroach vector types by family or OID.
	switch typ.Family() {
	...
	switch typ.Family() {
	case types.AnyFamily, types.OidFamily, types.TriggerFamily, types.TupleFamily:
		return true
	}
	...
	// 4) Arrays: skip if element is JSON, already skipped, or disallowed.
	...
	// 5) Everything else: rely on randgen’s notion of legal columns.
...
func convertProtobufValueToDatum(
	...
	case types.GeographyFamily:
		geoObj, err := geo.ParseGeography(val.GetStringValue())
		if err != nil {
			return nil, err
		}
		return tree.NewDGeography(geoObj), nil

	case types.GeometryFamily:
		geomObj, err := geo.ParseGeometry(val.GetStringValue())
		if err != nil {
			return nil, err
		}
		return tree.NewDGeometry(geomObj), nil
