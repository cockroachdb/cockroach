...
type schemaFeed struct {
	...
	mu struct {
		...
		// staleLeases indicates if the count is table multiple times.
		staleLeases bool
	}
}
...
func (tf *schemaFeed) pauseOrResumePolling(ctx context.Context, atOrBefore hlc.Timestamp) error {
	...
	if tf.mu.staleLeases {
		...
		if err := tf.targets.EachTableID(func(id descpb.ID) error {
			...
			// have changed.
			heldProcessed++
			canPausePolling = canPausePolling && heldLease.leasedDescriptor.Underlying().(catalog.TableDescriptor).IsSchemaLocked()
			return nil
