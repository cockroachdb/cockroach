...
func New(
	...
	m.mu.timeMonitor = timemonitor.New(&m.mu)
...
type schemaFeed struct {
	...
	mu struct {
		...
		// timeMonitor keeps track of the schema feed frontier and error timestamp.
...
func (tf *schemaFeed) updateTableHistory(ctx context.Context, endTS hlc.Timestamp) error {
	...
	func() {
		tf.mu.Lock()
		defer tf.mu.Unlock()
		startTS = tf.mu.timeMonitor.Frontier()
	}()
...
func (tf *schemaFeed) pauseOrResumePolling(ctx context.Context, atOrBefore hlc.Timestamp) error {
	...
	frontier := tf.mu.timeMonitor.Frontier()
	...
	if err := tf.targets.EachTableID(func(id descpb.ID) error {
		...
		if !desc1.IsSchemaLocked() {
			return errors.Newf("desc %d not schema-locked at frontier", desc1.GetID())
		}
		...
		if desc1.GetVersion() != desc2.GetVersion() {
			return errors.Newf("desc %d version changed from version %d to %d between frontier and atOrBefore",
	...
	return tf.mu.timeMonitor.AdvanceFrontier(atOrBefore)
}

// waitForTS blocks until the given timestamp is less than or equal to the
// frontier or error timestamp. In the latter case, the error is returned.
//
...
func (tf *schemaFeed) waitForTS(ctx context.Context, ts hlc.Timestamp) error {
	...
	duration, err := tf.mu.timeMonitor.WaitForFrontier(ctx, ts)
	if duration == 0 {
		if log.V(1) {
			log.Infof(ctx, "fast path taken while waiting for frontier to reach %s", ts)
		}
	} else {
		if log.V(1) {
			log.Infof(ctx, "waited %s for frontier to reach %s: err=%v", duration, ts, err)
		}
		if tf.metrics != nil {
			tf.metrics.TableMetadataNanos.Inc(duration.Nanoseconds())
		}
	}
	return err
}
...
func (tf *schemaFeed) adjustTimestamps(startTS, endTS hlc.Timestamp, validateErr error) error {
	...
	if validateErr != nil {
		if err := tf.mu.timeMonitor.SetError(endTS, validateErr); err != nil {
			return errors.Wrapf(err, "failed to set time monitor error with timestamp %s", endTS)
		}
		return validateErr
	}

	if frontier := tf.mu.timeMonitor.Frontier(); frontier.Less(startTS) {
		return errors.Errorf(`gap between %s and %s`, frontier, startTS)
	}
	return tf.mu.timeMonitor.AdvanceFrontier(endTS)
}
