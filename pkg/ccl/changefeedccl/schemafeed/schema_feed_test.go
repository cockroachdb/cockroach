...
func TestPauseOrResumePolling(t *testing.T) {
	...
	// We expect a non-terminal error to be swallowed at time 10.
	...
	// We expect polling not to be paused at time 30.
	require.NoError(t, sf.pauseOrResumePolling(ctx, hlc.Timestamp{WallTime: 30}))
	require.False(t, sf.pollingPaused())
	require.Equal(t, hlc.Timestamp{WallTime: 20}, sf.highWater())

	// We expect polling not to be paused at time 40 since the highwater
	...
	// We expect polling to be paused at time 40 now that the highwater has
	// caught up to the schema-locked version.
	require.NoError(t, sf.pauseOrResumePolling(ctx, hlc.Timestamp{WallTime: 40}))
	require.True(t, sf.pollingPaused())
	require.Equal(t, hlc.Timestamp{WallTime: 40}, sf.highWater())

	// We expect polling continue to be paused at time 50 and to see the highwater
	// bumped up.
	...
	// We expect polling to be resumed at time 60 and to not see the highwater
	// bumped up.
