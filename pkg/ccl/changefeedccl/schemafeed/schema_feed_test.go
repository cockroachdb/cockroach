...
func TestTableHistoryIngestionTracking(t *testing.T) {
	...
	m := schemaFeed{}
	m.mu.timeMonitor = timemonitor.New(&m.mu, timemonitor.WithInitialFrontier(ts(0)))
	frontier := func() hlc.Timestamp {
		m.mu.Lock()
		defer m.mu.Unlock()
		return m.mu.timeMonitor.Frontier()
	}
...
func TestPauseOrResumePolling(t *testing.T) {
	...
	sf.mu.timeMonitor = timemonitor.New(&sf.mu)

	getFrontier := func() hlc.Timestamp {
		sf.mu.Lock()
		defer sf.mu.Unlock()
		return sf.mu.timeMonitor.Frontier()
	}
	setFrontier := func(ts hlc.Timestamp) error {
		sf.mu.Lock()
		defer sf.mu.Unlock()
		return sf.mu.timeMonitor.AdvanceFrontier(ts)
	}
