...
func noteFeedMessage(a fsm.Args) error {
	...
	// one. It doesn't push any unresolved intents (is this right?), so if we have
	// an open transaction, it's possible that the poller is stuck waiting on it
	// to resolve, which would cause the below call to `Next` to deadlock. This
