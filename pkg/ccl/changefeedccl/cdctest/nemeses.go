...
func RunNemesis(f TestFeedFactory, db *gosql.DB, isSinkless bool) (Validator, error) {
	...
	for i := 0; i < ns.maxTestColumnCount; i++ {
		fmt.Fprintf(&createFprintStmtBuf, `, test%d STRING DEFAULT NULL`, i)
	}
	...
	m := fsm.MakeMachine(compiledStateTransitions,
		stateRunning{FeedPaused: fsm.False, TxnOpen: fsm.False,
			CanAddColumn: fsm.True, CanRemoveColumn: fsm.False}, ns)
	for {
		state := m.CurState()
...
var stateTransitions = fsm.Pattern{
	stateRunning{FeedPaused: fsm.Var("w"), TxnOpen: fsm.Var("x"),
		CanAddColumn: fsm.Var("y"), CanRemoveColumn: fsm.Var("z")}: {
		eventSplit{}: {
			Next: stateRunning{FeedPaused: fsm.Var("w"), TxnOpen: fsm.Var("x"),
				CanAddColumn: fsm.Var("y"), CanRemoveColumn: fsm.Var("z")},
	...
	stateRunning{FeedPaused: fsm.False, TxnOpen: fsm.False,
		CanAddColumn: fsm.True, CanRemoveColumn: fsm.Any}: {
		eventAddColumn{
			CanAddColumnAfter: fsm.Var("x"),
		}: {
			Next: stateRunning{FeedPaused: fsm.False, TxnOpen: fsm.False,
				CanAddColumn: fsm.Var("x"), CanRemoveColumn: fsm.True},
			Action: logEvent(addColumn),
		},
	},
	stateRunning{FeedPaused: fsm.False, TxnOpen: fsm.False,
		CanAddColumn: fsm.Any, CanRemoveColumn: fsm.True}: {
		eventRemoveColumn{
			CanRemoveColumnAfter: fsm.Var("x"),
		}: {
			Next: stateRunning{FeedPaused: fsm.False, TxnOpen: fsm.False,
				CanAddColumn: fsm.True, CanRemoveColumn: fsm.Var("x")},
			Action: logEvent(removeColumn),
		},
	},
	stateRunning{FeedPaused: fsm.False, TxnOpen: fsm.False,
		CanAddColumn: fsm.Var("x"), CanRemoveColumn: fsm.Var("y")}: {
		eventFeedMessage{}: {
			Next: stateRunning{FeedPaused: fsm.False, TxnOpen: fsm.False,
				CanAddColumn: fsm.Var("x"), CanRemoveColumn: fsm.Var("y")},
	...
	stateRunning{FeedPaused: fsm.Var("x"), TxnOpen: fsm.False,
		CanAddColumn: fsm.Var("y"), CanRemoveColumn: fsm.Var("z")}: {
		eventOpenTxn{}: {
			Next: stateRunning{FeedPaused: fsm.Var("x"), TxnOpen: fsm.True,
				CanAddColumn: fsm.Var("y"), CanRemoveColumn: fsm.Var("z")},
			Action: logEvent(openTxn),
		},
	},
	stateRunning{FeedPaused: fsm.Var("x"), TxnOpen: fsm.True,
		CanAddColumn: fsm.Var("y"), CanRemoveColumn: fsm.Var("z")}: {
		eventCommit{}: {
			Next: stateRunning{FeedPaused: fsm.Var("x"), TxnOpen: fsm.False,
				CanAddColumn: fsm.Var("y"), CanRemoveColumn: fsm.Var("z")},
	...
	stateRunning{FeedPaused: fsm.False, TxnOpen: fsm.Var("x"),
		CanAddColumn: fsm.Var("y"), CanRemoveColumn: fsm.Var("z")}: {
		eventPause{}: {
			Next: stateRunning{FeedPaused: fsm.True, TxnOpen: fsm.Var("x"),
				CanAddColumn: fsm.Var("y"), CanRemoveColumn: fsm.Var("z")},
			Action: logEvent(pause),
		},
	},
	stateRunning{FeedPaused: fsm.True, TxnOpen: fsm.Var("x"),
		CanAddColumn: fsm.Var("y"), CanRemoveColumn: fsm.Var("z")}: {
		eventResume{}: {
			Next: stateRunning{FeedPaused: fsm.False, TxnOpen: fsm.Var("x"),
				CanAddColumn: fsm.Var("y"), CanRemoveColumn: fsm.Var("z")},
...
func addColumn(a fsm.Args) error {
	...
	// TODO(aayush, dan): Revisit after the de-duplication logic in testFeed is made more
	// precise.
