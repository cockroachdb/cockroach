...
func RunNemesis(f TestFeedFactory, db *gosql.DB, isSinkless bool) (Validator, error) {
	...
	m := fsm.MakeMachine(txnStateTransitions, stateRunning{Paused: fsm.False, TxnOpen: fsm.False}, ns)
	for {
		state := m.CurState()
...
func (ns *nemeses) nextEvent(rng *rand.Rand, state fsm.State, f TestFeed) (fsm.Event, error) {
	...
	possibleEvents, ok := compatibleStateTransitions[state]
	if !ok {
...
var compatibleStateTransitions = fsm.Pattern{
	...
	stateRunning{Paused: fsm.False, TxnOpen: fsm.True}: {
		...
		eventPause{}: {
			Next:   stateRunning{Paused: fsm.True, TxnOpen: fsm.True},
			Action: logEvent(pause),
		},
		eventPush{}: {
			Next:   stateRunning{Paused: fsm.False, TxnOpen: fsm.True},
	...
	stateRunning{Paused: fsm.True, TxnOpen: fsm.True}: {
		eventResume{}: {
			Next:   stateRunning{Paused: fsm.False, TxnOpen: fsm.True},
			Action: logEvent(resume),
		},
	},
	stateRunning{Paused: fsm.True, TxnOpen: fsm.False}: {
		eventResume{}: {
			Next:   stateRunning{Paused: fsm.False, TxnOpen: fsm.False},
			Action: logEvent(resume),
		},
