...
func TestFingerprintValidator(t *testing.T) {
	...
	createTableStmt := func(tableName string) string {
		return fmt.Sprintf(`CREATE TABLE %s`+
			`(k INT PRIMARY KEY, v INT, test0 STRING, test1 STRING,`+
			`test2 STRING, test3 STRING, test4 STRING, test5 STRING,`+
			`test6 STRING, test7 STRING, test8 STRING, test9 STRING)`, tableName)
	}
