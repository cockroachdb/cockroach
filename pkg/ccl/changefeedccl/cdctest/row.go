...
func MakeRangeFeedValueReader(
...
) func(t *testing.T) *roachpb.RangeFeedValue {
	t.Helper()
	execCfg := execCfgI.(sql.ExecutorConfig)
	rows := make(chan *roachpb.RangeFeedValue)
	_, err := execCfg.RangeFeedFactory.RangeFeed(context.Background(), "feed-"+desc.GetName(),
	...
	return func(t *testing.T) *roachpb.RangeFeedValue {
		...
		select {
		case r := <-rows:
			log.Infof(context.Background(), "Got Row: %s", roachpb.PrettyPrintKey(nil, r.Key))
			return r
