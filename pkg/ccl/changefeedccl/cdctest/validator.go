...
func (v *FingerprintValidator) fingerprint(ts hlc.Timestamp) error {
	...
	query := fmt.Sprintf(`SELECT * FROM crdb_internal.fingerprint((select crdb_internal.index_span(%d, 1)), true) as of system time '%s'`,
		v.origTableID, ts.AsOfSystemTime())
	...
	query = fmt.Sprintf(`SELECT * FROM crdb_internal.fingerprint((select crdb_internal.index_span(%d, 1)), true)`,
		v.fprintTableID)
