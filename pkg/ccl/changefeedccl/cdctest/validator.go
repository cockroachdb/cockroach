...
func (v *beforeAfterValidator) NoteRow(
	...
	beforeDatums, err := parseJSON(beforeJson)
	if err != nil {
		return err
	}
	...
	// updated timestamp.
	if err := v.checkRowAt("after", primaryKeyDatums, afterDatums, updated); err != nil {
		return err
	}

	if v.resolved[partition].IsEmpty() && beforeDatums == nil {
	...
	return v.checkRowAt("before", primaryKeyDatums, beforeDatums, updated.Prev())
}
