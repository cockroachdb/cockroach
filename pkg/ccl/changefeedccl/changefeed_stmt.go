...
func createChangefeedJobRecord(
	...
	if changefeedStmt.Select != nil {
		...
		normalized, err := validateAndNormalizeChangefeedExpression(
			ctx, p, changefeedStmt.Select, targetDescs, targets,
			opts.IncludeVirtual(), opts.KeyOnly(), opts.IsSet(changefeedbase.OptSplitColumnFamilies), statementTime,
		)
		if err != nil {
			return nil, err
		}
...
func validateAndNormalizeChangefeedExpression(
	...
	norm, _, err := cdceval.NormalizeAndPlan(
		ctx, execCtx.ExecCfg(), execCtx.User(), execCtx.SessionData().SessionData,
		tableDescr, statementTime, targets[0], sc, splitColFams, cdceval.MustNormalize)
	if err != nil {
		return nil, err
	}
	return norm, nil
