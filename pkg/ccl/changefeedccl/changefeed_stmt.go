...
func changefeedPlanHook(
	...
	fn := func(ctx context.Context, _ []sql.PlanNode, resultsCh chan<- tree.Datums) error {
		...
		if scope, ok := opts[changefeedbase.OptSLIScope]; ok {
			...
			if scope == defaultSLIScope {
				return pgerror.Newf(pgcode.InvalidParameterValue,
					"sli_scope=%q is the default scope which keeps track of aggregate statistics "+
						"across all changefeeds.  If this is an intended behavior, please re-run the statement "+
						"without specifying scli_scope parameter.  "+
						"Otherwise, please re-run with a different sli_scope value.", defaultSLIScope)
			}
		}
		...
		sli, err := metrics.getSLIMetrics(opts[changefeedbase.OptSLIScope], &p.ExecCfg().Settings.SV)
		if err != nil {
			return err
		}
...
func (b *changefeedResumer) resumeWithRetries(
	...
	for r := retry.StartWithCtx(ctx, opts); r.Next(); {
		...
		if metrics, ok := execCfg.JobRegistry.MetricsStruct().Changefeed.(*Metrics); ok {
			sli, err := metrics.getSLIMetrics(details.Opts[changefeedbase.OptSLIScope], execCfg.SV())
			if err != nil {
				return err
			}
