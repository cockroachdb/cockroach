...
func maybeShowCreateChangefeedWarning(
	...
	if opts.HasStartCursor() && st != changefeedbase.OnlyInitialScan {
		...
		warningAge := func() int64 {
			...
			if knobs != nil && knobs.OverrideCursorWarningAge != nil {
				return knobs.OverrideCursorWarningAge()
			}
			return int64(5 * time.Hour)
		}()
...
func changefeedPlanHook(
	...
	rowFn := func(ctx context.Context, resultsCh chan<- tree.Datums) error {
		...
		if details.SinkURI == `` {
			...
			if err := maybeShowCreateChangefeedWarning(ctx, p, opts); err != nil {
				return err
			}
