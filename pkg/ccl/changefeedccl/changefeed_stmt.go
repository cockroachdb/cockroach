...
func changefeedPlanHook(
	...
	rowFn := func(ctx context.Context, resultsCh chan<- tree.Datums) error {
		...
		jr.JobID = jobID
		{
			...
			var ptr *ptpb.Record
			var cdcProgressPTSRecords *cdcprogresspb.ProtectedTimestampRecords
			...
			if writingMultiplePTS {
				...
				cdcProgressPTSRecords = &cdcprogresspb.ProtectedTimestampRecords{
			...
			if changefeedStmt.CreatedByInfo != nil {
				...
				if cdcProgressPTSRecords != nil {
					if err := writeChangefeedJobInfo(ctx, perTableProtectedTimestampsFilename, cdcProgressPTSRecords, p.InternalSQLTxn(), jobID); err != nil {
						return err
					}
				}
			...
			if err := p.ExecCfg().InternalDB.Txn(ctx, func(ctx context.Context, txn isql.Txn) error {
				...
				if cdcProgressPTSRecords != nil {
					if err := writeChangefeedJobInfo(
						ctx, perTableProtectedTimestampsFilename, cdcProgressPTSRecords, txn, jobID,
					); err != nil {
						return err
					}
				}
