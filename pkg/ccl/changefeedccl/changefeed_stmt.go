...
func getTargetsAndTables(
	...
	for i, ct := range rawTargets {
		...
		if spec, ok := originalSpecs[ct]; ok {
			...
			name, err := getChangefeedTargetName(ctx, td, p.ExecCfg(), p.ExtendedEvalContext().TxnToDelete, qualified)
			if err != nil {
				return nil, nil, err
			}
...
func (b *changefeedResumer) handleChangefeedError(
	...
	case changefeedbase.OptOnErrorPause:
		...
		return b.job.PauseRequested(ctx, jobExec.ExtendedEvalContext().TxnToDelete, func(ctx context.Context,
