...
func changefeedPlanHook(
	...
	fn := func(ctx context.Context, _ []sql.PlanNode, resultsCh chan<- tree.Datums) error {
		...
		//   storage sink or http sink. Kafka etc have a key and value field in each
		//   message but cloud storage sinks and http sinks don't have anywhere to
		//   put the key. So if the key is not in the value, then for DELETEs there
		//   is no way to recover which key was deleted. We could make the user
		//   explicitly pass this option for every cloud storage sink/http sink and
		//   error if they don't, but that seems user-hostile for insufficient
		//   reason. We can't do this any earlier, because we might return errors
		//   about `key_in_value` being incompatible which is confusing when the
		//   user didn't type that option.
		//   This is the same for the topic and http sink, which uses
		...
		if isCloudStorageSink(parsedSink) || isHTTPSink(parsedSink) {
			details.Opts[changefeedbase.OptKeyInValue] = ``
		}
		if isHTTPSink(parsedSink) {
...
func changefeedJobDescription(
	...
	for k, v := range opts {
		if k == changefeedbase.OptHTTPAuthHeader {
			v = redactHTTPAuthHeader(v)
		}
