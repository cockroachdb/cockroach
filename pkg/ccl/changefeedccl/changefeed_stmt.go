...
func (b *changefeedResumer) resumeWithRetries(
	...
	resolvedDest := resolveDest(ctx, execCfg, details.SinkURI)
			...
			g.GoCtx(func(ctx context.Context) error {
				...
				for {
					...
					case <-confPoller:
						return nil
					case <-t.C:
						newDest, err := reloadDetails(ctx, jobID, details, execCfg)
						if err != nil {
...
func resolveDest(ctx context.Context, execCfg *sql.ExecutorConfig, sinkURI string) string {
	u, err := url.Parse(sinkURI)
	if err != nil || u.Scheme != changefeedbase.SinkSchemeExternalConnection {
		return sinkURI
	}
	resolved := ""
	if err := execCfg.InternalDB.Txn(ctx, func(ctx context.Context, txn isql.Txn) error {
		...
		log.Warningf(ctx, "failed to resolve external connection %s: %v", u.Host, err)
	}
	return resolved
}
...
func reloadDetails(
	...
	j, err := execCfg.JobRegistry.LoadJob(ctx, id)
	if err != nil {
		return "", err
	}
	newDetails := j.Details().(jobspb.ChangefeedDetails)
	newDest := resolveDest(ctx, execCfg, newDetails.SinkURI)
	return newDest, nil
}
