...
func (b *changefeedResumer) handleChangefeedError(
	...
	case changefeedbase.OptOnErrorPause:
		...
		err := b.job.PauseRequested(ctx, jobExec.ExtendedEvalContext().Txn, b.OnPauseRequest)
		if err != nil {
			return err
		}

		// directly update running status to avoid the running/reverted job status check
		err = b.job.Update(ctx, jobExec.ExtendedEvalContext().Txn, func(
			txn *kv.Txn, md jobs.JobMetadata, ju *jobs.JobUpdater,
		) error {
			md.Progress.RunningStatus = fmt.Sprintf("paused on error: %v", changefeedErr)
