...
func createChangefeedJobRecord(
	...
	if opts.ExactlyOnceExport() {
		if !p.ExecCfg().Settings.Version.IsActive(ctx, clusterversion.V24_1_ExactlyOnceCDCExport) {
			return nil, pgerror.Newf(
				pgcode.FeatureNotSupported,
				"cannot create new changefeed export with exactly once option until upgrade to version %s is complete",
				changefeedbase.OptExecutionLocality, clusterversion.V24_1_ExactlyOnceCDCExport.String(),
			)
		}
