...
func changefeedPlanHook(
	...
	rowFn := func(ctx context.Context, resultsCh chan<- tree.Datums) error {
		...
		jr.JobID = jobID
		{
			...
			if perTablePTEnabled := changefeedbase.PerTableProtectedTimestamps.Get(&p.ExecCfg().Settings.SV); perTablePTEnabled {
				...
				for _, target := range targets {
					...
					err := target.EachTarget(
						func(t changefeedbase.Target) error {
							if tableID != 0 {
								return errors.AssertionFailedf("only one table descriptor is supported for per-table protected timestamps")
							}
							tableID = t.DescID
							return nil
						},
					...
					if tableID == 0 {
						return errors.AssertionFailedf("table descriptor ID is not set for per-table protected timestamp")
					}
				...
				err = jobs.WriteChunkedFileToJobInfo(ctx, perTablePTSInfoKey, ptsBytes, p.InternalSQLTxn(), jobID)
				if err != nil {
					return err
				}
			} else {
