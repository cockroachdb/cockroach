...
func createChangefeedJobRecord(
	...
	isUserSuppliedCursor := opts.HasStartCursor()
	...
	targetDescs, err := getTableDescriptors(ctx, p, &tableOnlyTargetList, statementTime, initialHighWater, isUserSuppliedCursor)
	if err != nil {
		return nil, err
	}
...
func getTableDescriptors(
	...
	if err != nil {
		...
		if !initialHighWater.IsEmpty() {
			...
			if isUserSuppliedCursor && errors.As(err, &e) {
				err = errors.Newf("could not create changefeed: cursor %s is older than the GC threshold %s", initialHighWater, e.Threshold)
