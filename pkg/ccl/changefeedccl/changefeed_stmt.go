...
func createChangefeedJobRecord(
	...
	if parsedSink.Query().Get(changefeedbase.SinkParamSASLProprietaryClientAssertionLocation) != "" {
		if err := checkProprietaryOauthFileBasedCredentialVersion(ctx, p.ExecCfg().Settings); err != nil {
			return nil, changefeedbase.Targets{}, err
		}
		if err := checkFileBasedCredentialPrivilege(func() (bool, error) {
			return p.HasPrivilege(ctx, syntheticprivilege.GlobalPrivilegeObject,
...
// checkFileBasedCredentialPrivilege returns InsufficientPrivilege if the
// caller lacks EXTERNALIOIMPLICITACCESS. The privilege lookup is passed in
// as a closure so this helper can be shared between the planner path (which
// uses sql.PlanHookState.HasPrivilege) and the external-connection
// validation path (which has no planner and goes through the
// has_system_privilege SQL builtin via isql).
func checkFileBasedCredentialPrivilege(hasPrivilege func() (bool, error)) error {
	ok, err := hasPrivilege()
	if err != nil {
		return err
	}
...
// checkProprietaryOauthFileBasedCredentialVersion guards against mixed-version
...
// fail the job. Callers gate on the URI referencing a credential file before
// calling this.
func checkProprietaryOauthFileBasedCredentialVersion(
