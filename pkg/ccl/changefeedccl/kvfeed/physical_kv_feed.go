	...
	for {
		...
		case e := <-p.eventCh:
			...
			case *kvpb.RangeFeedValue:
				...
				// TODO: or i could put this in the memBuf.Add method
				stop := p.st.For(timers.StageRangefeedBufferValue).StartTimer()
				if err := p.memBuf.Add(
			...
			case *kvpb.RangeFeedCheckpoint:
				...
				stop := p.st.For(timers.StageRangefeedBufferCheckpoint).StartTimer()
				if err := p.memBuf.Add(
