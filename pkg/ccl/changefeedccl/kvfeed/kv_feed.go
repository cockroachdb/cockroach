...
func Run(ctx context.Context, cfg Config) error {
	...
	if !errors.As(err, &scErr) {
		...
		return errors.CombineErrors(err, f.writer.Close(ctx, err))
	}
	...
	err = errors.CombineErrors(f.writer.Drain(ctx), f.writer.Close(ctx, nil))
...
func (f *kvFeed) runUntilTableEvent(
	...
	defer func() {
		err = errors.CombineErrors(err, memBuf.Close(ctx, err))
	}()
