	...
	bf := func() kvevent.Buffer {
		return kvevent.NewErrorWrapperEventBuffer(
			kvevent.NewMemBuffer(
				cfg.MM.MakeBoundAccount(),
				changefeedbase.EventMemoryMultiplier.Get(&cfg.Settings.SV),
				cfg.Metrics,
			))
	}
