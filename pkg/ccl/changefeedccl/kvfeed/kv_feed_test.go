...
func TestKVFeed(t *testing.T) {
	...
	for _, tc := range []testCase{
		...
		},
		{
			...
			events: []kvpb.RangeFeedEvent{
				checkpointEvent(tableSpan(codec, 42), ts(2)),
				checkpointEvent(tableSpan(codec, 42), hlc.Timestamp{WallTime: ts(2).WallTime, Logical: 1}),
				checkpointEvent(tableSpan(codec, 42), hlc.Timestamp{WallTime: ts(3).WallTime + 5, Logical: 1}),
				checkpointEvent(tableSpan(codec, 42), hlc.Timestamp{WallTime: ts(4).WallTime, Logical: 3}),
			},
			expEvents: []kvpb.RangeFeedEvent{
				checkpointEvent(tableSpan(codec, 42), hlc.Timestamp{WallTime: ts(2).WallTime, Logical: 0}),
				checkpointEvent(tableSpan(codec, 42), hlc.Timestamp{WallTime: ts(2).WallTime, Logical: 0}),
				checkpointEvent(tableSpan(codec, 42), hlc.Timestamp{WallTime: ts(3).WallTime, Logical: 0}),
				checkpointEvent(tableSpan(codec, 42), hlc.Timestamp{WallTime: ts(4).WallTime, Logical: 0}),
			},
			expEventsCount: 4,
		},
		{
...
func TestFrontierQuantization(t *testing.T) {
	...
	makeCheckpointEvent := func(key []byte, endKey []byte, ts int) *kvpb.RangeFeedEvent {
		return &kvpb.RangeFeedEvent{
			Checkpoint: &kvpb.RangeFeedCheckpoint{
				Span:       roachpb.Span{Key: key, EndKey: endKey},
				ResolvedTS: hlc.Timestamp{WallTime: int64(ts)},
			},
		}
	}
	...
	for name, tc := range map[string]struct {
		...
		"overlapping spans with close ts with quantization": {
			...
			events: []*kvpb.RangeFeedEvent{
				makeCheckpointEvent([]byte("a"), []byte("b"), 12),
				makeCheckpointEvent([]byte("b"), []byte("c"), 13),
				makeCheckpointEvent([]byte("c"), []byte("d"), 14),
			},
		...
		"non-overlapping spans with close ts with quantization": {
			...
			events: []*kvpb.RangeFeedEvent{
				...
				makeCheckpointEvent([]byte("b"), []byte("c"), 13),
				makeCheckpointEvent([]byte("c"), []byte("d"), 14),
				makeCheckpointEvent([]byte("k"), []byte("m"), 12),
			},
...
func TestFrontierQuantizationRand(t *testing.T) {
	...
	if quantizedHW.WallTime == hw.WallTime {
		require.LessOrEqual(t, quantizedHW.Logical, hw.Logical)
	} else {
		require.LessOrEqual(t, quantizedHW.WallTime, hw.WallTime)
	}
}
