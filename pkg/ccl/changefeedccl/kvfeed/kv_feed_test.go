...
func TestKVFeed(t *testing.T) {
	...
	runTest := func(t *testing.T, tc testCase) {
		tc.Settings = cluster.MakeTestingClusterSettings()
		tc.MM = mon.NewUnlimitedMonitor(
			context.Background(), "test", mon.MemoryResource,
			nil /* curCount */, nil /* maxHist */, math.MaxInt64, tc.Settings,
		)
		defer tc.MM.Stop(context.Background())

		metrics := kvevent.MakeMetrics(time.Minute)
		tc.Metrics = &metrics
		tc.Codec = keys.SystemSQLCodec
		tc.Knobs = TestingKnobs{}
		...
		tc.SchemaFeed = newRawTableFeed(tc.descs, tc.InitialHighWater)

		ctx, cancel := context.WithCancel(context.Background())
		f, _, reader, err := setupFeed(ctx, tc.Config, sf, rangefeedFactory(ref.run))
		require.NoError(t, err)
		defer func() {
			require.NoError(t, f.writer.Close(context.Background()))
		}()
		...
		spansToScan := filterCheckpointSpans(tc.Spans, tc.BackfillCheckpoint)
	...
	for _, tc := range []testCase{
		...
		{
			name: "no events -  full BackfillCheckpoint",
			Config: Config{
				SchemaChangeEvents: changefeedbase.OptSchemaChangeEventClassDefault,
				SchemaChangePolicy: changefeedbase.OptSchemaChangePolicyBackfill,
				NeedsInitialScan:   true,
