	...
	runTest := func(t *testing.T, tc testCase) {
		...
		testG.GoCtx(func(ctx context.Context) error {
			for expScans := tc.expScans; len(expScans) > 0; expScans = expScans[1:] {
				scan := <-scans
				require.Equal(t, expScans[0], scan.Timestamp)
				require.Equal(t, tc.withDiff, scan.WithDiff)
			}
			return nil
		})
		testG.GoCtx(func(ctx context.Context) error {
...
func newRawTableFeed(
	...
	for i, d := range descs {
		...
		f.events = append(f.events, tablefeed.Event{
			Before: descs[i-1],
			After:  d,
		})
	}
...
) (events []tablefeed.Event, err error) {
...
var _ tableFeed = (*rawTableFeed)(nil)
