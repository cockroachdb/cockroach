...
// Run implements the physicalFeedFactory interface for testing by replaying a
// pre-built event log into the sink. The same event log is used regardless of
// where the test resumes from, so Run does the post-processing a real
// rangefeed would otherwise hide:
//
...
func TestRangefeedClientFactoryErrorPropagation(t *testing.T) {
	...
	t.Run("sink error propagates", func(t *testing.T) {
		...
		pff := &rangefeedClientFactory{factory: factory}
		err := pff.Run(ctx, sink, newCfg())
