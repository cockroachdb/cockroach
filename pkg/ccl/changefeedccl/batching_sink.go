...
func makeBatchKey(topic, tableName string) batchKey {
	return batchKey{
		topic:     topic,
		tableName: tableName,
	}
	...
	for {
		...
		case req := <-s.eventCh:
			...
			case *rowEvent:
				...
				bKey := makeBatchKey(topic, r.tableName)
				...
				batchBuffer, ok := batches[bKey]
				if !ok {
					batchBuffer = s.newBatchBuffer(bKey)
					batches[bKey] = batchBuffer
				}
				...
				if batchBuffer.buffer.ShouldFlush() {
					...
					if err := tryFlushBatch(bKey); err != nil {
						s.handleError(err)
					}
				}
			case flushReq:
