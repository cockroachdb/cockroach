...
type confluentAvroEncoder struct {
	...
	keyCache   map[unsafe.Pointer]confluentRegisteredSchema
	valueCache map[unsafe.Pointer]confluentRegisteredSchema
...
func newConfluentAvroEncoder(
	...
	e := &confluentAvroEncoder{
		...
		keyCache:    make(map[unsafe.Pointer]confluentRegisteredSchema),
		valueCache:  make(map[unsafe.Pointer]confluentRegisteredSchema),
	}
...
func (e *confluentAvroEncoder) EncodeKey(
	...
	// TODO(dan): Is unsafe.Pointer as a map key okay?
	registered, ok := e.keyCache[unsafe.Pointer(tableDesc)]
	if !ok {
		var err error
		...
		e.keyCache[unsafe.Pointer(tableDesc)] = registered
	}
...
func (e *confluentAvroEncoder) EncodeValue(
	...
	// TODO(dan): Is unsafe.Pointer as a map key okay?
	registered, ok := e.valueCache[unsafe.Pointer(tableDesc)]
	if !ok {
		var err error
		...
		// TODO(dan): Bound the size of this cache.
		e.valueCache[unsafe.Pointer(tableDesc)] = registered
	}
	if e.json {
		return registered.schema.TextualFromRow(row)
	}
