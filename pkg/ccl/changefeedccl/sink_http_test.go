...
func testSendAndReceiveRows(t *testing.T, sinkSrc Sink, sinkDest *cdctest.MockHTTPSink) {
	...
	encoder, err := makeJSONEncoder(opts, jobspb.ChangefeedTargets{})
	if err != nil {
		t.Fatal(err)
	}

	// test a resolved timestamp entry
	err = sinkSrc.EmitResolvedTimestamp(ctx, Encoder(encoder), hlc.Timestamp{WallTime: 1})
	if err != nil {
		t.Fatal(err)
	}
	err = sinkSrc.Flush(ctx)
	...
	require.Equal(t,
		"{\"resolved\":\"1.0000000000\"}", sinkDest.Latest(),
		"sink %s expected to receive message %s", sinkDest.URL(),
