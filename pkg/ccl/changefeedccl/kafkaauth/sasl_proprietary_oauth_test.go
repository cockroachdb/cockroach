...
func TestProprietaryOAuthValidateParams(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:        "proprietary param under wrong mechanism rejected",
			uri:         `kafka://b?sasl_enabled=true&sasl_mechanism=PLAIN&sasl_user=u&sasl_password=p&sasl_proprietary_client_assertion_location=/p/oidc.jwt`,
			expectedErr: "sasl_proprietary_client_assertion_location is only a valid parameter for sasl_mechanism=PROPRIETARY_OAUTH",
		},
	}

	// Use a reader rooted at /p so the file-mode happy path's location
	// (/p/oidc.jwt) survives path validation in the builder.
...
// TestProprietaryOAuthFileMode verifies the file-mode closure re-reads the
// JWT on each bearer mint instead of capturing it at startup, so an external
