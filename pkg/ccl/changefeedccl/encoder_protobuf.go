...
type protobufEncoder struct {
	updatedField       bool                        // Whether to include logical update timestamps.
	mvccTimestampField bool                        // Whether to include MVCC commit timestamps.
	beforeField        bool                        // Whether to include the "before" row in the wrapped envelope.
	keyInValue         bool                        // Whether to embed the key in the value envelope.
	topicInValue       bool                        // Whether to embed the topic in the value envelope.
	envelopeType       changefeedbase.EnvelopeType // The envelope type: wrapped or bare.
	targets            changefeedbase.Targets      // Targets tracked by the encoder, used for table naming.
}
...
func buildKeyMessage(row cdcevent.Row) (*changefeedpb.Key, error) {
	keyValues := make([]*changefeedpb.Value, 0, row.NumKeyColumns())
	...
	return &changefeedpb.Key{Key: keyValues}, nil
}
...
func datumToProtoValue(d tree.Datum) (*changefeedpb.Value, error) {
	...
	case *tree.DJSON:
		//TODO: implement this as ProtoJSON Format?
	...
	case *tree.DGeography:
		...
		coords := shape.FlatCoords()
		typ := reflect.TypeOf(shape).Elem().Name()
		var latlon []float64
		if typ == "Point" && len(coords) >= 2 { //TODO: Handling differnt types
			latlon = []float64{coords[0], coords[1]}
		}
		return &changefeedpb.Value{
			Value: &changefeedpb.Value_GeojsonValue{
				GeojsonValue: &changefeedpb.GeoJSON{
					Type:        typ,
					Coordinates: latlon,
				},
			},
	...
	case *tree.DGeometry:
		...
		coords := shape.FlatCoords()
		var xy []float64
		if typ == "Point" && len(coords) >= 2 {
			xy = []float64{coords[0], coords[1]}
		}
		return &changefeedpb.Value{Value: &changefeedpb.Value_GeojsonValue{GeojsonValue: &changefeedpb.GeoJSON{Type: typ, Coordinates: xy}}}, nil
	case *tree.DVoid:
		return nil, nil
	case *tree.DDate:
		date, err := v.ToTime()
	...
	case *tree.DTime, *tree.DTimeTZ:
		return &changefeedpb.Value{Value: &changefeedpb.Value_TimeValue{TimeValue: v.String()}}, nil
	case *tree.DOid, *tree.DIPAddr, *tree.DBitArray, *tree.DBox2D,
		*tree.DTSVector, *tree.DTSQuery, *tree.DPGLSN, *tree.DPGVector:
		// These are currently serialized as strings.
