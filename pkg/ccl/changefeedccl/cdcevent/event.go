...
func NewEventDescriptor(
	...
	addColumn := func(col catalog.Column, nameOverrideString string, ord int) int {
		...
		if nameOverrideString != "" {
			resultColumn.Name = nameOverrideString
		}
		colIdx := len(sd.cols)
	...
	// For declarative schema changes, we intentionally select a previous descriptor
	// where a dropped column is in the WRITE_ONLY state (instead of PUBLIC).
	// This is necessary because we intentionally shifted the schema change stages
	// for dropped columns to help support automatic schema_locked support.
	// Consequently, we treat columns in that state as visible here so that their
	// previous values are available to the change feed.
