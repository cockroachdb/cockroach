...
func TestMakeRowFromTuple(t *testing.T) {
	...
	rowFromUnlabeledTuple := MakeRowFromTuple(unlabeledTuple)
	expectedCols := []struct {
		name        string
		typ         *types.T
		valAsString string
	...
	rowFromUnlabeledTuple.ForEachColumn().Datum(func(d tree.Datum, col ResultColumn) error {
	...
	rowFromLabeledTuple := MakeRowFromTuple(labeledTuple)

	rowFromLabeledTuple.ForEachColumn().Datum(func(d tree.Datum, col ResultColumn) error {
		current := remainingCols[0]
		remainingCols = remainingCols[1:]
		require.Equal(t, current.name, col.Name)
		require.Equal(t, current.typ, col.Typ)
