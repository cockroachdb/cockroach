...
// setKeysFromRow sets this projection keys from keys in the specified row.
func (p *Projection) setKeysFromRow(r Row) error {
	// Copy key datums.
	idx := 0
	return r.ForEachKeyColumn().Datum(func(d tree.Datum, col ResultColumn) error {
...
func (p *Projection) Project(r Row) (Row, error) {
	...
	if err := p.setKeysFromRow(r); err != nil {
		return Row{}, err
	}
	return Row(*p), nil
}
