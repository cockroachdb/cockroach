...
func TestTableHistory(t *testing.T) {
	...
	require.NoError(t, m.IngestDescriptors(context.TODO(), ts(1), ts(2), nil))
	...
	// overlap
	require.NoError(t, m.IngestDescriptors(context.TODO(), ts(1), ts(3), nil))
	require.Equal(t, ts(3), m.HighWater())

	// gap
	require.EqualError(t, m.IngestDescriptors(context.TODO(), ts(4), ts(5), nil),
		`gap between 0.000000003,0 and 0.000000004,0`)
	require.Equal(t, ts(3), m.HighWater())

	// validates
	require.NoError(t, m.IngestDescriptors(context.TODO(), ts(3), ts(4), []*sqlbase.TableDescriptor{
		{ID: 0},
	}))
	...
	require.NoError(t, m.IngestDescriptors(context.TODO(), ts(4), ts(5), nil))
	requireChannelEmpty(t, errCh6)
	requireChannelEmpty(t, errCh7)

	// high-water advances, unblocks only errCh6
	require.NoError(t, m.IngestDescriptors(context.TODO(), ts(5), ts(6), nil))
	require.NoError(t, <-errCh6)
	requireChannelEmpty(t, errCh7)

	// high-water advances again, unblocks errCh7
	require.NoError(t, m.IngestDescriptors(context.TODO(), ts(6), ts(7), nil))
	...
	require.EqualError(t, m.IngestDescriptors(context.TODO(), ts(7), ts(10), []*sqlbase.TableDescriptor{
		{ID: 0, Name: `whoops!`},
	}), `whoops!`)
	...
	require.EqualError(t, m.IngestDescriptors(context.TODO(), ts(7), ts(9), []*sqlbase.TableDescriptor{
		{ID: 0, Name: `oh no!`},
	}), `oh no!`)
	require.Equal(t, ts(7), m.HighWater())
	...
	require.NoError(t, m.IngestDescriptors(context.TODO(), ts(7), ts(8), nil))
