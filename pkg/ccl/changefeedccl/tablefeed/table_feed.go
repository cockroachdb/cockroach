...
// Event represents a change to a table descriptor.
type Event struct {
...
// EventFilterFunc allows a client to control which events are added to the
// TableFeed.
type EventFilterFunc func(context.Context, Event) (shouldFilter bool, err error)

// Config configures a TableFeed.
type Config struct {
	DB       *client.DB
	Clock    *hlc.Clock
	...
	//
	// NB: When clients want to create a changefeed which has a resolve timestamp
	...
	//
	// TODO(ajwerner): Should this live underneath the EventFilterFunc?
	// Should there be another function to decide whether to update the
	// lease manager?
	LeaseManager *sql.LeaseManager
}

// TableFeed tracks changes to a set of tables and exports them as a queue of
...
// New creates TableFeed with the given Config.
func New(cfg Config) *TableFeed {
...
func (tf *TableFeed) markStarted() error {
	tf.mu.Lock()
	defer tf.mu.Unlock()
	if tf.mu.started {
		return errors.AssertionFailedf("TableFeed started more than once")
	}
	tf.mu.started = true
	return nil
}

// Start will start the tablefeed. It will run asynchronously in the passed
// ctxgroup.Group. An error will be returned if it fails to perform its
// initial setup. It is an error to start a feed more than once.
func (tf *TableFeed) Start(ctx context.Context, g ctxgroup.Group) error {
...
func (tf *TableFeed) primeInitialTableDescs(ctx context.Context) error {
	tf.mu.Lock()
...
func (tf *TableFeed) updateTableHistory(ctx context.Context, endTS hlc.Timestamp) error {
...
func (tf *TableFeed) Pop(
	ctx context.Context, atOrBefore hlc.Timestamp,
) (events []Event, err error) {
	return tf.peekOrPop(ctx, atOrBefore, true /* pop */)
}

func (tf *TableFeed) peekOrPop(
...
func (tf *TableFeed) highWater() hlc.Timestamp {
	tf.mu.Lock()
...
// transactionally).
func (tf *TableFeed) waitForTS(ctx context.Context, ts hlc.Timestamp) error {
...
func (tf *TableFeed) adjustTimestamps(startTS, endTS hlc.Timestamp, validateErr error) error {
	tf.mu.Lock()
	defer tf.mu.Unlock()
