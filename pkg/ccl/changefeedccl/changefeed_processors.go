...
// changefeedJobProgressTracker tracks changefeed progress using
// an underying job whose state is tracked in the db.
type changefeedJobProgressTracker struct {
	job *jobs.Job
}

// coreChangefeedProgressTracker stores the changefeed progress in memory.
// This is instantiated and stored in the EvalCtx when the core changefeed
...
func (cpt *coreChangefeedProgressTracker) markIdle(isIdle bool) {
	panic("cannot mark a core changefeed as idle")
}
...
// SetHighwater implements the eval.CoreChangefeedState interface.
...
// SetCheckpoint implements the eval.CoreChangefeedState interface.
...
func (cf *changeFrontier) checkpointJobProgress(
	...
	res, err := cf.js.progressTracker.updateProgress(cf.Ctx, frontier, checkpoint, updateRunStatus, timestampManager)
	if err != nil {
		return false, err
	}
