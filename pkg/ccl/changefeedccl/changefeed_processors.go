...
func (ca *changeAggregator) Start(ctx context.Context) {
	...
	feed, err := makeChangefeedConfigFromJobDetails(ca.spec.Feed, ctx, ca.FlowCtx.Cfg.ExecutorConfig.(*sql.ExecutorConfig))
	if err != nil {
		if log.V(2) {
...
func (ca *changeAggregator) makeKVFeedCfg(
	...
	if schemaChange.Policy == changefeedbase.OptSchemaChangePolicyIgnore || initialScanOnly {
		...
		targets, err := AllTargets(ca.spec.Feed, ctx, ca.FlowCtx.Cfg.ExecutorConfig.(*sql.ExecutorConfig))
		if err != nil {
			return kvfeed.Config{}, err
		}
		sf = schemafeed.New(ctx, cfg, schemaChange.EventClass, targets,
...
func newChangeFrontierProcessor(
	...
	targets, err := AllTargets(spec.Feed, ctx, flowCtx.Cfg.ExecutorConfig.(*sql.ExecutorConfig))
	if err != nil {
		return nil, err
	}
...
func (cf *changeFrontier) manageProtectedTimestamps(
	...
	targets, err := AllTargets(cf.spec.Feed, ctx, cf.FlowCtx.Cfg.ExecutorConfig.(*sql.ExecutorConfig))
	if err != nil {
		return false, err
	}
	if progress.ProtectedTimestampRecord == uuid.Nil {
		ptr := createProtectedTimestampRecord(
			ctx, cf.FlowCtx.Codec(), cf.spec.JobID, targets, highWater,
		)
	...
	if !makeTargetToProtect(targets).Equal(rec.Target) {
...
func (cf *changeFrontier) remakePTSRecord(
	...
	ptr := createProtectedTimestampRecord(
		ctx, cf.FlowCtx.Codec(), cf.spec.JobID, targets, resolved,
	)
