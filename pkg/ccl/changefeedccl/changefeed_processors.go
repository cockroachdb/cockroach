...
func (ca *changeAggregator) Start(ctx context.Context) {
	...
	ca.sliMetrics.mu.Lock()
	ca.sliMetricsID = ca.sliMetrics.mu.id
	ca.sliMetrics.mu.id++
	ca.sliMetrics.mu.Unlock()
...
func (ca *changeAggregator) noteResolvedSpan(resolved jobspb.ResolvedSpan) error {
	...
	if advanced && ca.sliMetricsID != -1 {
		ca.sliMetrics.mu.resolved[ca.sliMetricsID] = ca.frontier.Frontier()
	}
...
func (ca *changeAggregator) closeMetrics() {
	ca.sliMetrics.mu.Lock()
	delete(ca.sliMetrics.mu.resolved, ca.sliMetricsID)
	ca.sliMetricsID = -1
	ca.sliMetrics.mu.Unlock()
}
...
func (cf *changeFrontier) Start(ctx context.Context) {
	...
	cf.sliMetrics.mu.Lock()
	cf.sliMetricsID = cf.sliMetrics.mu.id
	cf.sliMetrics.mu.id++
	cf.sliMetrics.mu.Unlock()
...
func (cf *changeFrontier) closeMetrics() {
	...
	cf.sliMetrics.mu.Lock()
	delete(cf.sliMetrics.mu.checkpoint, cf.sliMetricsID)
	cf.sliMetricsID = -1
	cf.sliMetrics.mu.Unlock()
}
...
func (cf *changeFrontier) forwardFrontier(resolved jobspb.ResolvedSpan) error {
	...
	if emitResolved {
		...
		cf.sliMetrics.mu.Lock()
		if cf.sliMetricsID != -1 {
			cf.sliMetrics.mu.checkpoint[cf.sliMetricsID] = newResolved
		}
		cf.sliMetrics.mu.Unlock()
