...
// heartbeatMeta is a record sent by aggregator to the coordinator
// to signal being alive.
var heartbeatMeta = &execinfrapb.ProducerMetadata{
...
func (ca *changeAggregator) Next() (rowenc.EncDatumRow, *execinfrapb.ProducerMetadata) {
	...
	for ca.State == execinfra.StateRunning {
		if !ca.changedRowBuf.IsEmpty() {
			...
			ca.lastPush = timeutil.Now()
			return nil, heartbeatMeta
		}

		if err := ca.tick(); err != nil {
			var e kvevent.ErrBufferClosed
			if errors.Is(err, changefeedbase.ErrNodeDraining) {
				err = changefeedbase.ErrNodeDraining
...
type changeFrontier struct {
	...
	// Use by core style changefeeds as well as regular changefeeds to make
...
func (cp *cachedState) SetHighwater(frontier hlc.Timestamp) {
...
func (cp *cachedState) SetCheckpoint(spans []roachpb.Span, timestamp hlc.Timestamp) {
...
func (cf *changeFrontier) maybeCheckpointJob(
	...
	if updateCheckpoint || updateHighWater {
		if cf.knobs.ShouldCheckpoint != nil && !cf.knobs.ShouldCheckpoint(cf.frontier.Frontier()) {
			return false, nil
		}
