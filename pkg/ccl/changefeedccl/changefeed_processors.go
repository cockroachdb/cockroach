...
func (cf *changeFrontier) Start(ctx context.Context) {
	...
	if cf.spec.Checkpoint == nil {
		err = checkpoint.Restore(cf.frontier, nil, hlc.Timestamp{WallTime: 0}, cf.spec.SpanLevelCheckpoint)
	} else {
		err = checkpoint.Restore(cf.frontier, cf.spec.Checkpoint.Spans, cf.spec.Checkpoint.Timestamp, cf.spec.SpanLevelCheckpoint)
	}
	if err != nil {
		if log.V(2) {
