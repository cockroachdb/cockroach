...
func (cf *changeFrontier) checkpointResolvedTimestamp(
	...
	if timeutil.Now().Sub(cf.js.lastRunStatusUpdate) > runStatusUpdateFrequency {
		defer func() {
			setJobRunningStatus(context.Background(), cf.js.job, "running: resolved=%s", resolved)
			cf.js.lastRunStatusUpdate = timeutil.Now()
		}()
	}

	return cf.js.job.HighWaterProgressed(cf.Ctx, nil, func(
