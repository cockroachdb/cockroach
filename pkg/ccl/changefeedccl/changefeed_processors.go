...
func (ca *changeAggregator) Start(ctx context.Context) context.Context {
	...
	for _, watch := range ca.spec.Watches {
		if watch.InitialResolved.IsEmpty() {
			// This case means this is the first session of this changefeed job, and not a
			// restart. We forward `sf` to StatementTime because rows emitted in the
			// initial scan will be emitted at this timestamp (see `makePoller()`) and we
			// want to avoid naming the first cloudStorageSink file to the zero epoch
			// timestamp. We call Prev() on it because `sf` is Next()ed in the
			// cloudStorageSink when it's used to name a file.
			sf.Forward(watch.Span, ca.spec.Feed.StatementTime.Prev())
		} else {
	...
	if ca.sink, err = getSink(
		ca.spec.Feed.SinkURI, nodeID, ca.spec.Feed.SessionID, ca.spec.Feed.Opts, ca.spec.Feed.Targets, ca.flowCtx.Cfg.Settings, sf,
	); err != nil {
...
func (cf *changeFrontier) Start(ctx context.Context) context.Context {
	...
	var err error
	// Pass a nil spanFrontier because this sink is only used to emit resolved timestamps
	// but the spanFrontier is only used when emitting row updates.
	var sf *spanFrontier
	if cf.sink, err = getSink(
		cf.spec.Feed.SinkURI, nodeID, cf.spec.Feed.SessionID, cf.spec.Feed.Opts, cf.spec.Feed.Targets, cf.flowCtx.Cfg.Settings, sf,
	); err != nil {
		err = MarkRetryableError(err)
