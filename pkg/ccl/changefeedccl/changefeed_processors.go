...
func (cf *changeFrontier) maybeReleaseProtectedTimestamp(
	...
	if progress.ProtectedTimestampRecord == nil {
		return nil
	}
	if log.V(2) {
		log.Infof(ctx, "releasing protected timestamp %v", *progress.ProtectedTimestampRecord)
	}
	if err := pts.Release(ctx, txn, *progress.ProtectedTimestampRecord); err != nil {
		return err
	}
	progress.ProtectedTimestampRecord = nil
	return nil
}
...
func (cf *changeFrontier) maybeProtectTimestamp(
	...
	progress.ProtectedTimestampRecord = &protectedTimestampID
	if log.V(2) {
		log.Infof(ctx, "creating protected timestamp %v at %v", protectedTimestampID, resolved)
	}
	spansToProtect := makeSpansToProtect(cf.spec.Feed.Targets)
	rec := jobsprotectedts.MakeRecord(
		protectedTimestampID, cf.spec.JobID, resolved, spansToProtect)
