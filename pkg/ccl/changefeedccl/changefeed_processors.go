...
func (ca *changeAggregator) Start(ctx context.Context) context.Context {
	...
	for _, watch := range ca.spec.Watches {
		if watch.InitialResolved.IsEmpty() {
			// This case means this is the first session of this changefeed job, and
			// not a restart. We forward `sf` to StatementTime because rows emitted
			// in the initial scan will be emitted at this timestamp (see `makePoller()`)
