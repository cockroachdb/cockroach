...
func (ca *changeAggregator) Start(ctx context.Context) {
	...
	acc := kvFeedMemMon.MakeBoundAccount()
	ca.sink, err = getSink(
		ctx, ca.spec.Feed.SinkURI, ca.flowCtx.EvalCtx.NodeID.SQLInstanceID(),
		ca.spec.Feed.Opts, ca.spec.Feed.Targets, ca.flowCtx.Cfg.Settings, timestampOracle,
		ca.flowCtx.Cfg.ExternalStorageFromURI, ca.spec.User(), &acc,
	)
	if err != nil {
...
func (cf *changeFrontier) Start(ctx context.Context) {
	...
	var err error
	// TODO(yevgeniy): Evaluate if we shoudl add memory monitoring for change frontier.
	acc := mon.BoundAccount{}
	cf.sink, err = getSink(
		ctx, cf.spec.Feed.SinkURI, cf.flowCtx.EvalCtx.NodeID.SQLInstanceID(), cf.spec.Feed.Opts,
		cf.spec.Feed.Targets, cf.flowCtx.Cfg.Settings, nilOracle,
		cf.flowCtx.Cfg.ExternalStorageFromURI, cf.spec.User(), &acc,
	)
	if err != nil {
		err = MarkRetryableError(err)
