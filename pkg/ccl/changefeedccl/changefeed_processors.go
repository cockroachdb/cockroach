...
func (ca *changeAggregator) Start(ctx context.Context) {
	...
	ca.sliMetrics, err = ca.metrics.getSLIMetrics(
		ca.spec.Feed.Opts[changefeedbase.OptSLIScope], &ca.flowCtx.Cfg.Settings.SV)
	if err != nil {
		ca.MoveToDraining(err)
		ca.cancel()
		return
...
func (ca *changeAggregator) makeKVFeedCfg(
	...
	onBackfillCb := func() func() {
		...
		ca.sliMetrics.BackfillCount.Inc(1)
		return func() {
			sm.backfilling.Set(false)
			ca.sliMetrics.BackfillCount.Dec(1)
		}
	}
...
func (cf *changeFrontier) Start(ctx context.Context) {
	...
	sli, err := cf.metrics.getSLIMetrics(
		cf.spec.Feed.Opts[changefeedbase.OptSLIScope], &cf.flowCtx.Cfg.Settings.SV)
	if err != nil {
