...
func (ca *changeAggregator) noteResolvedSpan(resolved *jobspb.ResolvedSpan) error {
	...
	checkpointSpans := ca.spec.JobID != 0 && /* enterprise changefeed */
		(resolved.Timestamp.Equal(ca.frontier.BackfillTS()) || ca.frontier.laggingHighwater(&ca.flowCtx.Cfg.Settings.SV)) &&
...
func (cf *changeFrontier) maybeCheckpointJob(
	...
	updateCheckpoint :=
		(inBackfill || cf.frontier.laggingHighwater(&cf.js.settings.SV)) && cf.js.canCheckpointSpans()
...
// timestamp of the checkpointed spans.
func (f *schemaChangeFrontier) getCheckpointSpans(
	...
	return spans, checkpointFrontier
}
...
// laggingHighwater returns true when the time between the earliest and latest
// resolved spans has exceeded the configured HighwaterLagCheckpointThreshold
func (f *schemaChangeFrontier) laggingHighwater(sv *settings.Values) bool {
