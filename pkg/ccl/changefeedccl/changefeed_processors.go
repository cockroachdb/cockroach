...
func (cf *changeFrontier) runBillingMetricReporting(ctx context.Context) {
	...
	defer cf.metrics.BillingMetrics.DeregisterJobMetrics(cf.spec.JobID)
	...
	for ctx.Err() == nil {
		...
		if err != nil {
			...
			if shouldCountBillingError(err) {
				...
				cf.metrics.BillingMetrics.RecordError()
			}
			continue
		}
		cf.metrics.BillingMetrics.UpdateTableBytes(cf.spec.JobID, res, timeutil.Since(start))
