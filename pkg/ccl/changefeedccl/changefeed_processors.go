...
func (cf *changeFrontier) runUsageMetricReporting(ctx context.Context) {
	...
	skipNextWait := cf.knobs.SkipFirstUsageMetricsReportingWait

	var t timeutil.Timer
	defer t.Stop()
	for ctx.Err() == nil {
		ivl := jitter(changefeedbase.UsageMetricsReportingInterval.Get(&cf.flowCtx.Cfg.Settings.SV))
		var start time.Time

		// Skip the first wait if requested, in tests.
		if !skipNextWait {
			t.Reset(ivl)
			select {
			case start = <-t.C:
		...
		res, err := FetchChangefeedUsageBytes(reqCtx, cf.flowCtx.Cfg.ExecutorConfig.(*sql.ExecutorConfig), cf.js.job.Payload())
		cancel()
		if err != nil {
		...
		cf.metrics.UsageMetrics.UpdateTableBytes(cf.spec.JobID, res, timeutil.Since(start))
	}
}
