...
// tick is the workhorse behind Next(). It pulls kv changes from the
...
func (ca *changeAggregator) maybeFlush(resolvedSpan *jobspb.ResolvedSpan) error {
	if resolvedSpan != nil {
		_ = ca.spanFrontier.Forward(resolvedSpan.Span, resolvedSpan.Timestamp)
	}

	if resolvedSpan == nil ||
		(timeutil.Since(ca.lastFlush) < ca.flushFrequency && !resolvedSpan.BoundaryReached) {
		return nil
	}
