...
func (ca *changeAggregator) tick() error {
	event, resource, err := ca.eventProducer.GetEvent(ca.Ctx)
	if err != nil {
		return err
	}
	...
	switch event.Type() {
	case kvevent.TypeKV:
		if err := ca.eventConsumer.ConsumeEvent(ca.Ctx, event, resource.Move()); err != nil {
			return err
		}
...
func (c *kvEventToRowConsumer) ConsumeEvent(
	...
	// Release resources if we encounter an error.
	defer resource.Release()
	...
	if err := c.sink.EmitRow(
		...
		keyCopy, valueCopy, resource.Move(), r.updated,
	); err != nil {
		return err
	}
	if log.V(3) {
...
func (c *nativeKVConsumer) ConsumeEvent(
	...
	return c.sink.EmitRow(ctx, &noTopic{}, keyBytes, valBytes, resource.Move(), val.Timestamp)
}
