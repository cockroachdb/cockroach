...
func (cf *changeFrontier) checkpointJobProgress(
	...
	if cf.tsWatcher != nil {
		...
		if len(tableAdds) > 0 {
			// If there are changes, we'll only advance the frontier to the
			// creation time of the first added table, so we don't get a
			// situation where the hwm is less than a table in the frontier,
