...
func (ca *changeAggregator) tick() error {
	...
	case kvfeed.ResolvedEvent:
		...
		if ca.knobs.ShouldEmitResolved == nil || ca.knobs.ShouldEmitResolved(resolved) {
...
func (cf *changeFrontier) maybeCheckpointJob(frontierChanged, isBehind bool) error {
	...
	if updateCheckpoint {
		cf.frontier.checkpoint(&checkpoint)
	}
...
// In addition, f 'manageProtected' is true, which only happens when frontier advanced,
...
func (f *schemaChangeFrontier) checkpoint(c *jobspb.ChangefeedProgress_Checkpoint) {
