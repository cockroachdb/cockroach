...
func (ca *changeAggregator) maybeLogLaggingRanges(resolved jobspb.ResolvedSpan) {
	...
	// backfill_pending_ranges metric.  Lagging ranges during a backfill aren't
	// necessarily problematic, so the lagging ranges metric is reserved for just
	// the non-backfill case.
