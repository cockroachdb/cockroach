...
func makeSchemaChangeFrontier(
	...
	for _, span := range spans {
		sf.Forward(span, initialHighWater)
	}
...
func (f *schemaChangeFrontier) BackfillTS() hlc.Timestamp {
	...
	// If the backfill is occuring after any initial scan (non-empty frontier),
	// then we can only be in a schema change backfill, where the scan is
