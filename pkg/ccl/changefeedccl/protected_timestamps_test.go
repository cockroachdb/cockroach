	...
	testFn := func(t *testing.T, s TestServerWithSystem, f cdctest.TestFeedFactory) {
		...
		getSystemTablesRecordID := func() uuid.UUID {
			...
			require.NoError(t, execCfg.InternalDB.Txn(context.Background(), func(ctx context.Context, txn isql.Txn) error {
				...
				systemTablesRecordID = ptsEntries.SystemTablesRecord
				return nil
			}))
...
WITH resolved='10ms', min_checkpoint_frequency='100ms', initial_scan='no'`
		...
		err = execCfg.InternalDB.Txn(context.Background(), func(ctx context.Context, txn isql.Txn) error {
			...
			require.Equal(t, 0, len(ptsEntries.TableRecords))
	...
	testFn := func(t *testing.T, s TestServer, f cdctest.TestFeedFactory) {
		...
		testutils.SucceedsSoon(t, func() error {
			return execCfg.InternalDB.Txn(context.Background(), func(ctx context.Context, txn isql.Txn) error {
				...
				if len(ptsEntries.TableRecords) != len(expectedTables) {
					return errors.Newf(
						"expected %d per-table PTS records, got %d",
						len(expectedTables), len(ptsEntries.TableRecords),
					)
				}
				...
				for tableID := range expectedTables {
					...
					ptsRecordID := ptsEntries.TableRecords[tableID]
					...
					tablePTS[tableID] =
						ptutil.GetPTSTimestamp(t, sqlDB, ptsEntries.TableRecords[tableID])
				}
				...
				systemTablesTarget := ptutil.GetPTSTarget(t, sqlDB, &ptsEntries.SystemTablesRecord)
				...
				systemTablesPTS = ptutil.GetPTSTimestamp(t, sqlDB, ptsEntries.SystemTablesRecord)
		...
		testutils.SucceedsSoon(t, func() error {
			return execCfg.InternalDB.Txn(context.Background(), func(ctx context.Context, txn isql.Txn) error {
				...
				newSystemTablesPTS :=
					ptutil.GetPTSTimestamp(t, sqlDB, ptsEntries.SystemTablesRecord)
