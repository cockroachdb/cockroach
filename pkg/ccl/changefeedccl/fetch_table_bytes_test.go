...
func TestFetchChangefeedUsageBytesE2EIndexes(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)
	...
	fx.db.Exec(t, "CREATE TABLE test (id STRING PRIMARY KEY, value STRING)")
	fx.db.Exec(t, "INSERT INTO test (id, value) SELECT i::string, i::string FROM generate_series(1, 10000) as g(i)")
	// fx.db.Exec(t, "ALTER TABLE test ADD COLUMN name STRING")

	row := fx.db.QueryRow(t, `CREATE CHANGEFEED FOR TABLE test INTO 'null://' WITH initial_scan='no';`)
	var feedJobId int64
	row.Scan(&feedJobId)

	// Wait for the first run to complete.
	fx.tableBytesTracker.WaitForIncrease() // TODO: this wont work because we wait for the next loop which is 10 hours

	// Add another index to the table and see the bytes increase.
	fx.db.Exec(t, `CREATE INDEX ON test (value)`)
	fx.tableBytesTracker.WaitForIncrease()
	require.Zero(t, fx.metrics.UsageErrorCount.Count())
}
...
func newUsageFixtureWithMockTSS(
	...
	return &UsageFixture{
		...
		queryDurationTracker: &metricValueTracker{t: t, name: "query_duration", observeValue: func() int64 { c, _ := metrics.UsageQueryDuration.CumulativeSnapshot().Total(); return c }},
		close:                func() { s.Stopper().Stop(ctx) },
	}
}

func (fx *UsageFixture) getTableSpans(ctx context.Context, name string) (spans roachpb.Spans) {
