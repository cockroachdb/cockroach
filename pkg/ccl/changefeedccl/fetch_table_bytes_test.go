...
func TestFetchChangefeedBillingBytesBasic(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)

	ctx := context.Background()

	fx := newBillingFx(ctx, t)
	defer fx.close()
...
func TestFetchChangefeedBillingBytesE2EErrorCount(t *testing.T) {
	...
	fx := newBillingFxWithMockTss(ctx, t, tss)
	defer fx.close()
...
func TestFetchChangefeedBillingBytesE2EDisabledByDefault(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)
...
func newBillingFx(ctx context.Context, t *testing.T) *billingFx {
	return newBillingFxWithMockTss(ctx, t, nil)
}

func newBillingFxWithMockTss(ctx context.Context, t *testing.T, mockTss *mocks.MockTenantStatusServer) *billingFx {
	...
	if mockTss != nil { // TODO: this is a data race. how can we do it properly?
		s.DistSQLServer().(*distsql.ServerImpl).ExecutorConfig.(*sql.ExecutorConfig).TenantStatusServer = mockTss
	}

	execCfg := s.ExecutorConfig().(sql.ExecutorConfig)
	metrics := execCfg.JobRegistry.MetricsStruct().Changefeed.(*changefeedccl.Metrics).BillingMetrics
	...
	return &billingFx{
		...
		query:             query,
		metrics:           metrics,
		tableBytesTracker: &metricValueTracker{t: t, name: "table_bytes", observeValue: metrics.BillingTableBytes.Value},
		errorCountTracker: &metricValueTracker{t: t, name: "error_count", observeValue: metrics.BillingErrorCount.Count},
		close:             func() { s.Stopper().Stop(ctx) },
	}
}

func (fx *billingFx) getTableSpans(ctx context.Context, name string) (spans roachpb.Spans) {
...
func (fx *billingFx) getChangefeedPayload(ctx context.Context, jobID catpb.JobID) (jobspb.Payload, error) {
...
func (fx *billingFx) waitForPausedJob(jobID int64) {
