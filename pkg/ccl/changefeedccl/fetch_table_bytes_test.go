...
func TestFetchChangefeedUsageBytesE2E(t *testing.T) {
	...
	// TODO: make trackers curl the other process' prom endpoint and grep etc
...
func newUsageFixtureWithMockTSS(
	...
	// ServingSQLAddr refers to the tenant if there is one, else the system
	db := serverutils.OpenDBConn(t, s.ServingSQLAddr(), "testdb", false, s.Stopper())
	...
	return &UsageFixture{
		...
		queryDurationTracker: &metricValueTracker{t: t, ts: ts, name: "query_duration_count", observeValue: func() int64 { c, _ := metrics.UsageQueryDuration.CumulativeSnapshot().Total(); return c }},
...
func (fx *UsageFixture) getChangefeedPayload(
	...
	f := func(ctx context.Context, txn descs.Txn) error {
		row, err := txn.QueryRowEx(ctx, "test", txn.KV(), sessiondata.RootUserSessionDataOverride, changefeedDetailsQuery, jobID)
		if err != nil {
			return err
		}
