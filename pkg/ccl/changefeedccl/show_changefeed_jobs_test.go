...
func TestShowChangefeedJobs(t *testing.T) {
	...
	query = `CREATE CHANGEFEED FOR TABLE foo INTO 
		'webhook-https://fake-http-sink:8081' WITH http_auth_header='Basic Zm9v'`
	...
	require.Equal(t, "CREATE CHANGEFEED FOR TABLE foo INTO 'webhook-https://fake-http-sink:8081' WITH http_auth_header = 'Basic redacted'", out.description, "Expected description:%s but found description:%s", "CREATE CHANGEFEED FOR TABLE foo INTO 'webhook-https://fake-http-sink:8081' WITH http_auth_header = 'Basic redacted'", out.description)
