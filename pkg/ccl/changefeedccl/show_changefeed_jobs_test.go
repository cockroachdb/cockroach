...
func TestShowChangefeedJobs(t *testing.T) {
	...
	query = `SELECT job_id FROM crdb_internal.jobs WHERE description LIKE '%CREATE CHANGEFEED FOR TABLE foo, bar INTO%'`
	sqlDB.QueryRow(t, query).Scan(&changefeedID)

	var out row

	query = `SELECT job_id, job_type, sink_uri, full_table_names, format FROM [SHOW CHANGEFEED JOB $1]`
	sqlDB.QueryRow(t, query, changefeedID).Scan(&out.id, &out.typ, &out.SinkURI, &out.FulLTableNames, &out.format)

	require.Equal(t, changefeedID, out.id, "Expected id:%d but found id:%d", changefeedID, out.id)
	require.Equal(t, "CHANGEFEED", out.typ, "Expected type:%s but found type:%s", "CHANGEFEED", out.typ)
	...
	query = `SELECT job_id, job_type, sink_uri, full_table_names, format FROM [SHOW CHANGEFEED JOBS] ORDER BY job_id`
...
func TestShowChangefeedJobsStatusChange(t *testing.T) {
	...
	query = `SELECT job_id FROM crdb_internal.jobs WHERE description LIKE '%CREATE CHANGEFEED FOR TABLE foo INTO%'`
	sqlDB.QueryRow(t, query).Scan(&changefeedID)
	...
	waitForJobOperation(sqlDB, t, changefeedID, "canceled")
}
...
func TestShowChangefeedJobsNoResults(t *testing.T) {
	...
	query := `SELECT job_id, job_type, sink_uri, full_table_names, format FROM [SHOW CHANGEFEED JOB 999999999999]`
	rowResults := sqlDB.Query(t, query)
