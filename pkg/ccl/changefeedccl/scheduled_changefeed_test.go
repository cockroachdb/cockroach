...
func TestScheduledChangefeedErrors(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:  "explicit-conflict-with-initial-scan-only",
			stmt:  "CREATE SCHEDULE test_schedule FOR CHANGEFEED t1 INTO 'null://sink' WITH resolved, initial_scan='no', initial_scan_only RECURRING '@daily';",
			errRE: "cannot specify both initial_scan and initial_scan_onl",
		},
	}
