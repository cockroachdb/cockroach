...
type changedKVs struct {
	...
	// ExportRequest.
	sst []byte
	// closed, if non-zero, is a guarantee that all key values in subsequent
	// changedKVs will have a higher timestamp.
...
func runChangefeedFlow(ctx context.Context, execCfg *sql.ExecutorConfig, job *jobs.Job) error {
	...
	changedKVsFn := exportRequestPoll(execCfg, details)
...
// are returned, the timestamp used for the fetch is returned as closed.
//
...
func exportRequestPoll(
	execCfg *sql.ExecutorConfig, details jobs.ChangefeedDetails,
	...
	return func(ctx context.Context) (changedKVs, error) {
		...
		// always append the closed timestamp.
		highwater = nextHighwater
		buffer.append(changedKVs{closed: highwater})
		ret, _ := buffer.get()
		return ret, nil
	}
}
...
func kvsToRows(
	...
	return func(ctx context.Context) ([]emitRow, error) {
		...
		if input.closed != (hlc.Timestamp{}) {
			output = append(output, emitRow{closed: input.closed})
		}
		return output, nil
	}
}
		...
		for _, input := range inputs {
			if input.row != nil {
				...
				for i := range input.row {
					jsonValueRaw[input.tableDesc.Columns[i].Name], err = builtins.AsJSON(input.row[i])
					if err != nil {
						return err
					}
				}
			...
			if input.closed != (hlc.Timestamp{}) {
				if err := jobProgressedFn(ctx, input.closed); err != nil {
					return err
				}
