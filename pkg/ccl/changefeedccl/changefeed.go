...
func changefeedPlanHook(
	...
	fn := func(ctx context.Context, resultsCh chan<- tree.Datums) error {
		...
		if changefeedStmt.AsOf.Expr != nil {
			var err error
			if highwater, err = sql.EvalAsOfTimestamp(
				nil, changefeedStmt.AsOf, highwater,
			); err != nil {
				return err
			}
		}
		...
		descriptorTime := p.ExecCfg().Clock.Now()
...
func (cf *changefeed) poll(ctx context.Context, startTime, endTime hlc.Timestamp) error {
	...
	for _, span := range cf.spans {
		...
		for _, file := range res.(*roachpb.ExportResponse).Files {
			...
			if err := sst.Iterate(start, end, emitFunc); err != nil {
				return err
			}
		}
	}
	...
	for {
		...
		row, tableDesc, _, err := cf.rf.NextRow(ctx)
		if err != nil {
			return err
		}
		if row == nil {
		...
		for i := range row {
			if err := row[i].EnsureDecoded(&tableDesc.Columns[i].Type, &cf.a); err != nil {
				return err
			}
			jsonEntries[tableDesc.Columns[i].Name], err = builtins.AsJSON(row[i].Datum)
			if err != nil {
				return err
			}
		}
		...
		if err := cf.kafka.Produce(message, nil /* deliveryChan */); err != nil {
...
func (b *changefeedResumer) Resume(
	...
	for {
		...
		// TODO(dan): This Flush is here so the mock knows when an entire poll
		// has completed. It's possible that it's useful for real kafka as well,
		// but if it isn't, figure something else out.
		...
		pollDuration = pollDuration - timeutil.Since(time.Unix(0, highwater.WallTime))
		select {
		case <-ctx.Done():
			return ctx.Err()
