...
// GetCheckpointSpans returns the spans that should be checkpointed based on the
// current state of the frontier.
func (f *resolvedSpanFrontier) GetCheckpointSpans(
	maxBytes int64,
) (spans []roachpb.Span, timestamp hlc.Timestamp) {
	return getCheckpointSpans(f.Frontier(), f.Entries, maxBytes)
}

// aggregatorResolvedSpanFrontier wraps a resolvedSpanFrontier with additional
// checks specific to how change aggregators process boundaries.
type aggregatorResolvedSpanFrontier struct {
	resolvedSpanFrontier
}

// newAggregatorResolvedSpanFrontier returns a new aggregatorResolvedSpanFrontier.
func newAggregatorResolvedSpanFrontier(
	statementTime hlc.Timestamp, initialHighWater hlc.Timestamp, spans ...roachpb.Span,
) (*aggregatorResolvedSpanFrontier, error) {
	rsf, err := newResolvedSpanFrontier(statementTime, initialHighWater, spans...)
	if err != nil {
		return nil, err
	}
	return &aggregatorResolvedSpanFrontier{
...
func (f *aggregatorResolvedSpanFrontier) ForwardResolvedSpan(
	ctx context.Context, r jobspb.ResolvedSpan,
) (bool, error) {
...
// frontierResolvedSpanFrontier wraps a resolvedSpanFrontier with additional
// checks specific to how change frontiers process boundaries.
