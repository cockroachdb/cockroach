...
func makeSQLSink(uri *url.URL, tableName string, targets jobspb.ChangefeedTargets) (Sink, error) {
	...
	if unknownParams := u.remainingQueryParams(); len(unknownParams) > 0 {
		return nil, errors.Errorf(`unknown SQL sink query parameters: %s`, unknownParams)
	}
