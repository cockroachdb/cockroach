...
func (p *poller) rangefeedImpl(ctx context.Context) error {
	for {
		...
		// rest of the changefeed (which latter is implemented with an unbuffered
		// channel, and so doesn't actually buffer). Ideally, we'd have one, but the
		// structure of the poller code right now makes this hard. Specifically,
		// when a schema change happens, we need a barrier where we flush out every
		// change before the schema change timestamp before we start emitting any
		// changes from after the schema change. The poller's `tableHist` is
		// responsible for detecting and enforcing these (they queue up in
		// `p.scanBoundaries`), but the after-poller buffer doesn't have access to
		// any of this state. A cleanup is in order.
