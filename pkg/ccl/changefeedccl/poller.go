...
func hasNewColumnDropBackfillMutation(old, new *sqlbase.TableDescriptor) (res bool) {
	...
	return !dropMutationExists(old) && dropMutationExists(new)
}

func newColumnBackfillComplete(old, new *sqlbase.TableDescriptor) (res bool) {
	return len(old.Columns) < len(new.Columns) &&
		old.HasColumnBackfillMutation() && !new.HasColumnBackfillMutation()
}
