	...
	slurpKVs := func() error {
		...
		for _, kv := range kvs {
			...
			if withDiff {
				...
				// change.
				prev := kv.Value.ShallowClone()
				prevSchemaTimetamp := schemaTimestamp.Prev()
				err = p.buf.AddKV(ctx, kv, prev, schemaTimestamp, prevSchemaTimetamp)
			} else {
				err = p.buf.AddKV(ctx, kv, nil /* prevVal */, schemaTimestamp, hlc.Timestamp{})
			}
			if err != nil {
				return err
			}
