...
// sinkDialer is an interface that Sink implementations may optionally implement
// if the sink needs to establish connection to the remote location.
...
func getSink(
	...
	newSink := func() (Sink, error) {
		...
		case u.Scheme == changefeedbase.SinkSchemeKafka:
			return makeKafkaSink(ctx, u, feedCfg.Targets, feedCfg.Opts, acc)
		case isCloudStorageSink(u):
			return makeCloudStorageSink(
				ctx, u, serverCfg.NodeID.SQLInstanceID(), serverCfg.Settings,
		...
		case u.Scheme == changefeedbase.SinkSchemeExperimentalSQL:
			return makeSQLSink(u, sqlSinkTableName, feedCfg.Targets)
		}
	...
	if sinkDial, wantDial := sink.(sinkDialer); wantDial {
		if err := sinkDial.Dial(); err != nil {
			return nil, err
		}
	}
	return sink, nil
