...
func (s *bufferSink) getTopicDatum(t TopicDescriptor) *tree.DString {
	if len(t.GetNameComponents()) == 1 {
		return s.alloc.NewDString(tree.DString(t.GetNameComponents()[0]))
	}
	var b strings.Builder
	for i, c := range t.GetNameComponents() {
		if i > 0 {
			b.WriteByte('.')
		}
		b.WriteString(c)
	}
	return s.alloc.NewDString(tree.DString(b.String()))
}
