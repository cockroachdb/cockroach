...
func getSink(
	...
	case `experimental-s3`, `experimental-gs`, `experimental-nodelocal`:
...
func (s *cloudStorageSink) Flush(ctx context.Context, ts hlc.Timestamp) error {
	...
	for key, file := range s.files {
		...
		// If the bucket end is `<= ts`, we'll never see another row for this
		// bucket and it can be cleaned up.
