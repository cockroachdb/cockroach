...
func TestPrivileges(t *testing.T) {
	...
	privilegeNotNeeded := func(oldErrs []error, notNeeded privilege.Kind) (filtered []error) {
		for _, e := range oldErrs {
			var mp missingPrivilege
			if !errors.As(e, &mp) || mp.k != notNeeded {
	...
	// CONTROLCHANGEFEED role option should not change the result of any sinkless
	...
	for _, at := range tests {
		...
		newTest.acceptableErrors = privilegeNotNeeded(at.acceptableErrors, privilege.CHANGEFEED)
		testsWithControlChangefeed = append(testsWithControlChangefeed, newTest)
	}

	tests = append(tests, testsWithControlChangefeed...)

	// Specifying a sink removes the requirement for the SELECT privilege, unless
	// (for legacy reasons) the user has the CONTROLCHANGEFEED option.
	var testsWithSink []authTest
	for _, at := range tests {
		newTest := at
		newTest.name = "kafka/" + at.name
		newTest.sinkURI = "kafka://sink"
		if !newTest.hasControlChangefeed {
			newTest.acceptableErrors = privilegeNotNeeded(at.acceptableErrors, privilege.SELECT)
		}
		testsWithSink = append(testsWithSink, newTest)
	}

	tests = append(tests, testsWithSink...)

	t.Logf("Running %d generated test cases", len(tests))

	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			uri, parseError := url.Parse(test.sinkURI)
			require.NoError(t, parseError)
			err := checkPrivileges(context.Background(), test, append(dbDescs, test.tables...), uri)
