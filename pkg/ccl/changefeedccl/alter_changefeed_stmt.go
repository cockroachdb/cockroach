	...
	for _, cmd := range alterCmds {
		...
		case *tree.AlterChangefeedAddTarget:
			...
			for _, target := range v.Targets {
				...
				tblName, err := getQualifiedTableNameObj(ctx, p.ExecCfg(), p.Txn(), desc.(catalog.TableDescriptor))
				if err != nil {
					return nil, nil, hlc.Timestamp{}, nil, err
				}
				tablePattern, err := tblName.NormalizeTablePattern()
				if err != nil {
					return nil, nil, hlc.Timestamp{}, nil, err
				}
