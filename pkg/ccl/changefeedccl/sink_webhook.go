...
func (s *webhookSink) setupWorkers() {
	...
	s.workerGroup.GoCtx("run webhook sink batch worker", func(ctx context.Context) error {
		s.batchWorker()
		return nil
	})
	for i := 0; i < s.parallelism; i++ {
		...
		j := i
		s.workerGroup.GoCtx("run webhook sink worker loop", func(ctx context.Context) error {
			s.workerLoop(j)
			return nil
		})
	}
