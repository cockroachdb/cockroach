...
func (b *batchTracker) addToBuffer(payload webhookMessage) {
	b.bufferMsgCount++
	b.bufferBytes += len(payload.m)
	b.buffer = append(b.buffer, payload)
}

func (b *batchTracker) emptyBufferAndReset() []webhookMessage {
	buffer := b.buffer
	b.buffer = []webhookMessage{}
	b.bufferMsgCount = 0
	b.bufferBytes = 0
	return buffer
}

func (b *batchTracker) close() {
	close(b.bufferChan)
}
...
type webhookSink struct {
	...
	// batch messages are aggregated here and split into different workers
	// to be sent separately
	batchTr batchTracker
}
...
func (s *webhookSink) setupWorkers() {
	...
	s.batchTr.bufferChan = make(chan webhookMessage)
...
func (s *webhookSink) shouldSendBatch() bool {
	...
	case s.batchCfg.Messages > 0 && s.batchTr.bufferMsgCount >= s.batchCfg.Messages:
		return true
	// bytes threshold has been reached
	case s.batchCfg.Bytes > 0 && s.batchTr.bufferBytes >= s.batchCfg.Bytes:
		return true
	default:
		return false
	}
...
func (s *webhookSink) batchWorker() {
	...
	for {
		...
		case <-s.workerCtx.Done():
			return
		case msg := <-s.batchTr.bufferChan:
			s.batchTr.addToBuffer(msg)
			if s.shouldSendBatch() {
				batch := s.batchTr.emptyBufferAndReset()
				ticker.Stop()
				err := s.splitAndSendBatch(batch)
				s.inflight.maybeSetError(err)
				timerCh = nil
			} else {
				if time.Duration(s.batchCfg.Frequency) > 0 {
					// only start timer when first message appears
					timerCh = ticker.C
					ticker.Reset(time.Duration(s.batchCfg.Frequency))
				}
			}
		// timerCh will be nil if first message hasn't appeared, so won't trigger
		case <-timerCh:
			batch := s.batchTr.emptyBufferAndReset()
			ticker.Stop()
			if len(batch) > 0 {
...
func (s *webhookSink) EmitRow(
	...
	case <-ctx.Done():
		return ctx.Err()
	case s.batchTr.bufferChan <- webhookMessage{k: key, m: value, a: alloc}:
	}
	return nil
}
