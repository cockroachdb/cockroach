...
func (s *webhookSink) workerLoop(workerCh chan webhookMessage) {
	for {
		...
		case msg := <-workerCh:
			...
			s.inflight.FinishRequest(msg.r)
...
func (i *inflightTracker) FinishRequest(resource kvevent.Resource) {
	resource.Release()
...
func (s *webhookSink) EmitRow(
	ctx context.Context, _ TopicDescriptor, key, value []byte, _ hlc.Timestamp, r kvevent.Resource,
) error {
	...
	case <-ctx.Done():
		return ctx.Err()
	case s.eventsChans[s.workerIndex(key)] <- webhookMessage{m: payload, r: sr.Move()}:
	}
	return nil
}
...
func (s *webhookSink) EmitResolvedTimestamp(
	...
	s.inflight.maybeSetError(err)
	s.inflight.FinishRequest(kvevent.NoResource)
	return err
}
