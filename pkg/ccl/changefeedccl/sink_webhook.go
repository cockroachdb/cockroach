...
func (b *batch) emptyBufferAndReset() []webhookMessage {
	buffer := b.buffer
...
func (s *webhookSink) setupWorkers() {
	...
	s.batchTracker.bufferChan = make(chan webhookMessage)
...
func (s *webhookSink) shouldSendBatch() bool {
	...
	case s.batchCfg.Messages > 0 && s.batchTracker.bufferMsgCount >= s.batchCfg.Messages:
		return true
	// bytes threshold has been reached
	case s.batchCfg.Bytes > 0 && s.batchTracker.bufferBytes >= s.batchCfg.Bytes:
		return true
	default:
		return false
	}
...
func (s *webhookSink) batchWorker() {
	...
	for {
		...
		case <-s.workerCtx.Done():
			return
		case msg := <-s.batchTracker.bufferChan:
			s.batchTracker.addToBuffer(msg)
			if s.shouldSendBatch() {
				batch := s.batchTracker.emptyBufferAndReset()
				ticker.Stop()
...
func (s *webhookSink) EmitRow(
	...
	case <-ctx.Done():
		return ctx.Err()
	case s.batchTracker.bufferChan <- webhookMessage{k: key, m: value, a: alloc}:
	}
	return nil
}
