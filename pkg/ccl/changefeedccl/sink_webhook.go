...
func makeWebhookSink(ctx context.Context, u sinkURL, opts map[string]string) (Sink, error) {
	...
	if timeout, ok := opts[changefeedbase.OptWebhookClientTimeout]; ok {
		...
		if err != nil || connTimeout <= time.Duration(0) {
	...
	sink.flushCh = make(chan struct{})
	sink.wg = new(sync.WaitGroup)
...
func (s *webhookSink) Flush(ctx context.Context) error {
	...
	for s.mu.inflight > 0 {
		log.VInfof(ctx, 2, "flush waiting for %d inflight messages", s.mu.inflight)
		select {
		case <-ctx.Done():
			return ctx.Err()
		case <-s.flushCh:
...
func (s *webhookSink) Close() error {
	// Flush to make sure we're not sending to closed channels. If error,
	// ignore since we're shutting down (most likely context canceled)
	_ = s.Flush(s.ctx)
	s.cancelFunc()
	s.wg.Wait()
