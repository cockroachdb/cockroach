...
func adjustNative(native map[string]interface{}, writerSchema, readerSchema *avroDataRecord) {
	for _, writerField := range writerSchema.Fields {
		if _, inReader := readerSchema.fieldIdxByName[string(writerField.Name)]; !inReader {
			...
			// ignored."
			delete(native, string(writerField.Name))
		}
	}
	for _, readerField := range readerSchema.Fields {
		if _, inWriter := writerSchema.fieldIdxByName[string(readerField.Name)]; !inWriter {
			...
			if readerFieldDefault, ok := readerField.defaultValueNative(); ok {
				native[string(readerField.Name)] = readerFieldDefault
			}
		}
	}
}
