...
// Run runs a datadriven test using the file in the relative path of the caller.
...
func Run(ctx context.Context, t *testing.T, path string) {
...
func handleKVRequest(
	...
	if request == "DeleteRange" {
		...
		bankSpan := roachpb.Span{Key: k, EndKey: k.PrefixEnd()}
...
func (d *datadrivenTestState) addServer(t *testing.T, cfg serverCfg) error {
	...
	if cfg.iodir == "" {
		tc, _, cfg.iodir, cleanup = BulkTestSetupWithParams(t, clusterSize, cfg.splits,
			InitManualReplication, params, true)
	} else {
		tc, _, cleanup = BulkTestSetupEmptyWithParams(t, clusterSize, cfg.iodir,
