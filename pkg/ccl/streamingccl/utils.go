...
func ScanSST(
	...
	for rangeIter.SeekGE(storage.MVCCKey{Key: scanWithin.Key}); ; rangeIter.Next() {
		...
		if _, hasRange := rangeIter.HasPointAndRange(); hasRange {
			for _, rangeKeyVal := range rangeIter.RangeKeys() {
				...
				intersectedSpan := scanWithin.Intersect(
