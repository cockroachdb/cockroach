...
func NewSpanConfigStreamClient(
	ctx context.Context, remote *url.URL, db isql.DB,
	...
	if remote.Scheme == "external" {
		...
		return NewSpanConfigStreamClient(ctx, url, db)
	}
...
func GetFirstActiveSpanConfigClient(
	ctx context.Context, streamAddresses []string, db isql.DB,
) (SpanConfigClient, error) {
	...
	newClient := func(ctx context.Context, address streamingccl.StreamAddress) (Client, error) {
		...
		return NewSpanConfigStreamClient(ctx, streamURL, db)
	}
	client, err := getFirstClient(ctx, streamAddresses, newClient)
	if err != nil {
		return nil, err
	}
	return client.(SpanConfigClient), err
}
