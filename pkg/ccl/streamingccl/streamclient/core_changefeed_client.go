...
type coreChangefeedClient struct{}

var _ Client = &coreChangefeedClient{}

// GetTopology implements the Client interface.
func (m *coreChangefeedClient) GetTopology(
...
func (m *coreChangefeedClient) ConsumePartition(
	...
	go func() {
		...
		for rows.Next() {
			...
			case eventCh <- event:
			case <-ctx.Done():
				// Put ctx.Err() on the err channcel.
				return
			}
		}
	}()
