...
type interceptFn func(event streamingccl.Event)
...
type randomStreamClient struct {
	tableIDToDesc map[descpb.ID]*tabledesc.Mutable
		...
		for {
			...
			case <-kvTimer.C:
				...
				if len(systemKVs) > 0 {
					...
					event = streamingccl.MakeKVEvent(systemKV, partitionAddress)
					...
					if rng.Float64() < m.config.dupProbability && dupKVEvent != nil {
						...
						dupKV.Value.Timestamp = hlc.Timestamp{WallTime: timeutil.Now().UnixNano()}
						event = streamingccl.MakeKVEvent(*dupKV, partitionAddress)
					} else {
						event = streamingccl.MakeKVEvent(m.makeRandomKey(r, tableDesc), partitionAddress)
						dupKVEvent = event
					}
				}
			...
			case <-resolvedTimer.C:
				...
				event = streamingccl.MakeCheckpointEvent(hlcResolvedTime, partitionAddress)
