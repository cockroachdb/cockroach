...
func (m *randomStreamClient) Plan(
	...
	for i := 0; i < m.config.numPartitions; i++ {
		...
		topology = append(topology,
			PartitionInfo{
				ID:                "1",
				SubscriptionToken: []byte(partitionURI.String()),
			})
	}
...
func (m *randomStreamClient) Create(ctx context.Context, target roachpb.TenantID) (StreamID, error) {
...
func (m *randomStreamClient) Subscribe(ctx context.Context,
	stream StreamID,
