...
func (sc testStreamClient) Subscribe(
	_ context.Context, _ streampb.StreamID, _ SubscriptionToken, _ hlc.Timestamp, _ bool,
) (Subscription, error) {
...
func ExampleClient() {
	...
	replicationProducerSpec, err := client.Create(ctx, "system")
	if err != nil {
		panic(err)
	}
	...
	id := replicationProducerSpec.StreamID
	...
	grp.GoCtx(func(ctx context.Context) error {
		...
		for _, partition := range topology.Partitions {
			...
			sub, err := client.Subscribe(ctx, id, partition.SubscriptionToken, ts, true /* withInitialScan */)
			if err != nil {
				panic(err)
			}
