...
func BenchmarkLastWriteWinsInsert(b *testing.B) {
		...
		Knobs: base.TestingKnobs{
			Store: &kvserver.StoreTestingKnobs{
				TestingRequestFilter: func(_ context.Context, ba *kvpb.BatchRequest) *kvpb.Error {
					...
					req, ok := ba.GetArg(kvpb.Put)
					if !ok {
						return nil
					}
					if !req.(*kvpb.PutRequest).Key.Equal(key) {
						return nil
					}
	...
	// Insert a single row into the table which we'll attempt to overwrite via
	// the row processor.
	// TODO(yuzefovich): I don't quite understand why this is needed - it
	// appears as if the BatchRequest produced for the rp's INSERT query changes
	// without it.
	runner.Exec(b, "INSERT INTO tab VALUES (1, 'hello')")
