...
func TestCutoverJob(t *testing.T) {
	...
	group.GoCtx(func(ctx context.Context) error {
		...
		if err := db.QueryRowContext(
			...
			*job.ID()).Scan(&jobID); err != nil {
			return err
		}
		require.Equal(t, *job.ID(), int64(jobID))

		// Wait for the job to move into a terminal state.
		_, err := db.ExecContext(ctx, `SHOW JOB WHEN COMPLETE $1`, jobID)
		require.NoError(t, err)
	...
	// Give a chance for the above group to schedule in order to test that SHOW
	// JOBS WHEN COMPLETE does block until the job is canceled.
