...
func DoGenerationSwitchover(
	...
	if !sp.StreamIngest.NextGeneration.IsEmpty() {
		return errors.Newf("switchover to generation started at %s has not"+
			" completed yet", sp.StreamIngest.NextGeneration.String())
	}
	sp.StreamIngest.NextGeneration = cutoverTimestamp
	return setJobProgress(evalCtx, txn, jobID, progress)
}
