...
func (c *TenantStreamingClusters) setupSrcTenant(ctx context.Context) {
...
func CreateMultiTenantStreamingCluster(
	...
	tsc := &TenantStreamingClusters{
		...
		SrcCleanup:    cleanup,
		DestCluster:   cluster,
		DestSysSQL:    sqlutils.MakeSQLRunner(cluster.ServerConn(0)),
		DestSysServer: cluster.Server(0),
	}
	tsc.setupSrcTenant(ctx)
	tsc.init()
	return tsc, func() {
