...
type FeedSource interface {
	// Next returns the next event, a flag indicating if there are more events
	// to consume.
...
func (rf *ReplciationFeed) ObserveKey(key roachpb.Key) roachpb.KeyValue {
...
func (rf *ReplciationFeed) ObserveResolved(lo hlc.Timestamp) hlc.Timestamp {
	require.NoError(rf.t, rf.consumeUntil(ResolvedAtLeast(lo)))
	return *rf.msg.GetResolved()
}

func (rf *ReplciationFeed) Close() {
	rf.f.Close()
}

func (rf *ReplciationFeed) consumeUntil(pred FeedPredicate) error {
