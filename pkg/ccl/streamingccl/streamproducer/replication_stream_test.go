...
`)
	...
	observed := feed.ObserveKey(expected.Key)

	require.True(t, expected.Value.EqualTagAndData(observed.kv.Value))
	...
	feed.ObserveResolved(observed.kv.Value.Timestamp)

	// Update our row.
	h.tenant.sql.Exec(t, `UPDATE d.t1 SET b = 'blah' WHERE i = 42`)
	expected = encodeKV(t, h.tenant.codec, descr, 42, nil, "blah")

	// Observe its changes.
	updated := feed.ObserveKey(expected.Key)
	require.True(t, expected.Value.EqualTagAndData(updated.kv.Value))
	require.True(t, observed.kv.Value.Timestamp.Less(updated.kv.Value.Timestamp))
}
