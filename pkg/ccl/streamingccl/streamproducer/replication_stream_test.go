...
`)
	...
	source, feed := startReplication(t, h, makePartitionStreamDecoder,
		streamPartitionQuery, streamID, encodeSpec(t, h, srcTenant, hlc.Timestamp{}, "t1", "t2"))
	defer feed.Close(ctx)
	...
	for {
		require.True(t, source.rows.Next())
		source.codec.decode()
		if codec.e.Batch != nil && len(codec.e.Batch.DelRanges) > 0 {
	...
	data, start, end := storageutils.MakeSST(t, h.SysServer.ClusterSettings(), []interface{}{
		storageutils.PointKV(string(t2Span.Key), 5, "5"),
		// Delete range from t1e - t2s, emitting t1s - t1e.
		storageutils.RangeKV(string(t1Span.Key), string(t2Span.Key), 10, ""),
		// Delete range from t1e - t2enn, emitting t2s - t2e.
		storageutils.RangeKV(string(t1Span.EndKey), string(t2Span.EndKey.Next().Next()), 15, ""),
		// Delete range for t2sn - t2en, emitting t2sn - t2e.
		storageutils.RangeKV(string(t2Span.Key.Next()), string(t2Span.EndKey.Next()), 20, ""),
		// Delete range for t3s - t3e, emitting nothing.
		storageutils.RangeKV(string(t3Span.Key), string(t3Span.EndKey), 25, ""),
	})
	expectedDelRange1 := roachpb.RangeFeedDeleteRange{Span: t1Span, Timestamp: hlc.Timestamp{WallTime: 10}}
	expectedDelRange2 := roachpb.RangeFeedDeleteRange{Span: t2Span, Timestamp: hlc.Timestamp{WallTime: 15}}
	expectedDelRange3 := roachpb.RangeFeedDeleteRange{
		Span:      roachpb.Span{Key: t2Span.Key.Next(), EndKey: t2Span.EndKey},
	...
	_, _, _, err := h.SysServer.DB().AddSSTableAtBatchTimestamp(ctx, start, end, data, false,
		false, hlc.Timestamp{}, nil, false, h.SysServer.Clock().Now())
	require.NoError(t, err)
	...
	for {
		...
		if len(codec.e.Batch.KeyValues) > 0 {
			receivedKVs = append(receivedKVs, codec.e.Batch.KeyValues...)
		}

		if len(codec.e.Batch.DelRanges) > 0 {
			receivedDelRanges = append(receivedDelRanges, codec.e.Batch.DelRanges...)
		}
		if len(receivedDelRanges) == 3 && len(receivedKVs) == 1 {
			break
		}
	}
	...
	require.Equal(t, int64(5), receivedKVs[0].Value.Timestamp.WallTime)
	require.Equal(t, expectedDelRange1.String(), receivedDelRanges[0].String())
	require.Equal(t, expectedDelRange2.String(), receivedDelRanges[1].String())
	require.Equal(t, expectedDelRange3.String(), receivedDelRanges[2].String())
}
