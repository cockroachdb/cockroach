...
`)
	...
	t.Run("stream-tenant", func(t *testing.T) {
		feed := startReplication(t, h, makeCoreChangefeedDecoder, streamTenantQuery)
	...
	t.Run("stream-tenant-with-cursor", func(t *testing.T) {
		...
		feed := startReplication(t, h, makeCoreChangefeedDecoder,
...
`)
	...
	t1Desc := catalogkv.TestingGetTableDescriptor(
		h.SysServer.DB(), h.Tenant.Codec, "d", "t1")
	t1Span := t1Desc.PrimaryIndexSpan(h.Tenant.Codec)
	log.Infof(context.Background(), "t1Span: %s", t1Span)

	makePartitionSpec := func(startFrom hlc.Timestamp) []byte {
		spec := streampb.StreamPartitionSpec{
			StartFrom: startFrom,
			Spans:     []roachpb.Span{t1Span},
			Config: streampb.StreamPartitionSpec_ExecutionConfig{
				MinCheckpointFrequency: 100 * time.Millisecond,

				// TODO(yevgeniy): Support reasonable batching.  For now, force each event to be flushed.
				BatchByteSize: 1,
			},
		}
	...
	t.Run("stream-table", func(t *testing.T) {
		feed := startReplication(t, h, makePartitionStreamDecoder,
			streamPartitionQuery, streamID, makePartitionSpec(hlc.Timestamp{}))
		defer feed.Close(ctx)
	...
	t.Run("stream-table-with-cursor", func(t *testing.T) {
		...
		feed := startReplication(t, h, makePartitionStreamDecoder,
			streamPartitionQuery, streamID, makePartitionSpec(beforeUpdateTS))
		defer feed.Close(ctx)
