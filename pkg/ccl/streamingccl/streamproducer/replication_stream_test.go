...
`)
	...
	localTS := int(batchHLCTime.Logical)
	data, start, end := storageutils.MakeSST(t, h.SysServer.ClusterSettings(), []interface{}{
		storageutils.PointKVWithLocalTS(string(t2Span.Key), ts, localTS, "5"),
		// Delete range from t1s - t2s, emitting t1s - t1e.
		storageutils.RangeKVWithLocalTS(string(t1Span.Key), string(t2Span.Key), ts, localTS, ""),
		// Delete range from t1e - t2enn, emitting t2s - t2e.
		storageutils.RangeKVWithLocalTS(string(t1Span.EndKey), string(t2Span.EndKey.Next().Next()), ts, localTS, ""),
		// Delete range for t2sn - t2en, which overlaps the range above on t2s - t2e, emitting nothing.
		storageutils.RangeKVWithLocalTS(string(t2Span.Key.Next()), string(t2Span.EndKey.Next()), ts, localTS, ""),
		// Delete range for t3s - t3e, emitting nothing.
		storageutils.RangeKVWithLocalTS(string(t3Span.Key), string(t3Span.EndKey), ts, localTS, ""),
	})
