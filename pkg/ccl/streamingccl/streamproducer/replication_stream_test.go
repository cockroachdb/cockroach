...
func makePartitionSpec(h *streamingtest.ReplicationHelper,
	startFrom hlc.Timestamp, tables ...string) *streampb.StreamPartitionSpec {
...
func encodeSpec(t *testing.T, h *streamingtest.ReplicationHelper, startFrom hlc.Timestamp, tables ...string) []byte {
	opaqueSpec, err := protoutil.Marshal(makePartitionSpec(h, startFrom, tables...))
	require.NoError(t, err)
	return opaqueSpec
}
...
// no initial scan and import before creating rangefeed
//    many identical keyvalues and no one from sstable

// initial scan and import before creating rangefeed; only this case consistently pass
//    one keyvalue not from addsstable

// no initial scan and import after creating rangefeed
//    many identical keyvalues and one from addsstable
// initial scan and import after creating rangefeed
//    many identical keyvalues and one from addsstable

// My guess1: if import before the rangefeed creation, then no sstable will be received
// My guess2: after we add sstable in the source cluster, the registered rangefeed emits both
//            the sstable and some rangefeedvalue events that have data in the sstable.
//            any misuse of setting when we addsstable?
...
`)
	...
	testAddSSTable := func(t *testing.T, initialScan bool, addSSTableBeforeRangefeed bool, table string) {
		...
		for {
			...
			source.codec.decode()
			if codec.e.Batch != nil && len(codec.e.Batch.KeyValues) > 0 {
				for _, keyVal := range codec.e.Batch.KeyValues {
					fmt.Println("received", keyVal.Key.String(), keyVal.Value.String())
				}
				require.Equal(t, 1, len(codec.e.Batch.KeyValues))
	...
	// no initial scan and import before creating rangefeed
	//    one event from addsstable
	// initial scan and import before creating rangefeed
	//    one event from addsstable

	// no initial scan and import after creating rangefeed
	//    two events and one from addsstable
	// initial scan and import after creating rangefeed
	//    five events and one from addsstable  sometimes one
	t.Run("initial-scan-import-before-rangefeed", func(t *testing.T) {
		testAddSSTable(t, true /* initialScan*/,
			true /* addSSTableBeforeRangefeed */, "x1" /* table */)
	})

	t.Run("no-initial-scan-import-before-rangefeed", func(t *testing.T) {
		testAddSSTable(t, false /* initialScan*/,
			true /* addSSTableBeforeRangefeed */, "x2" /* table */)
	})

	t.Run("initial-scan-import-after-rangefeed", func(t *testing.T) {
		testAddSSTable(t, true /* initialScan*/,
			false /* addSSTableBeforeRangefeed */, "x3" /* table */)
	})

	t.Run("no-initial-scan-import-after-rangefeed", func(t *testing.T) {
		testAddSSTable(t, false /* initialScan*/,
			false /* addSSTableBeforeRangefeed */, "x4" /* table */)
	})
}
