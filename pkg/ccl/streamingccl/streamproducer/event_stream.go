...
type eventStream struct {
	...
	// addMu is non-nil during initial scans anc serializes addKV's in onValue.
...
func (s *eventStream) maybeCheckpoint(ctx context.Context, advanced bool, frontier rangefeed.VisitableFrontier) {
	if s.addMu != nil {
		s.addMu.Lock()
		defer s.addMu.Unlock()
	}
...
func (s *eventStream) addSST(
	sst *kvpb.RangeFeedSSTable, registeredSpan roachpb.Span,
) error {
