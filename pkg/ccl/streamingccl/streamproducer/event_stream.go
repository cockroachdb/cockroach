...
func init() {
	streamingccl.StreamPartitionHook = streamPartition
}
	...
	if s.spec.StartFrom.IsEmpty() {
		...
		opts = append(opts,
			...
			rangefeed.WithOnInitialScanError(func(ctx context.Context, err error) (shouldFail bool) {
				// TODO(yevgeniy): When encountering an error during initial scan, resume from checkpoint.
			...
			rangefeed.WithOnSpanScanCompleted(s.onSpanCompleted),
		)
	}
...
func (s *eventStream) onSpanCompleted(ctx context.Context, sp roachpb.Span) {
...
func (p *checkpointPacer) shouldCheckpoint(frontierAdvanced bool) bool {
	now := timeutil.Now()
	...
	// TODO(yevgeniy): Support checkpoint during backfills
	return false
}
	...
	for {
		...
		case ev := <-s.eventsCh:
			...
			case ev.Checkpoint != nil:
				...
				if pacer.shouldCheckpoint(advanced) {
