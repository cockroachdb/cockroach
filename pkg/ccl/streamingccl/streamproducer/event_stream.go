...
type eventStream struct {
	...
	eventsCh    chan kvclient.RangeFeedEnvelope // Channel receiving rangefeed events.
...
func (s *eventStream) Start(ctx context.Context, txn *kv.Txn) error {
	...
	s.eventsCh = make(chan kvclient.RangeFeedEnvelope)
...
func (s *eventStream) onValue(ctx context.Context, value *roachpb.RangeFeedValue) {
	select {
	case <-ctx.Done():
	case s.eventsCh <- kvclient.RangeFeedEnvelope{RangeFeedEvent: &roachpb.RangeFeedEvent{Val: value}}:
...
func (s *eventStream) onCheckpoint(ctx context.Context, checkpoint *roachpb.RangeFeedCheckpoint) {
	select {
	case <-ctx.Done():
	case s.eventsCh <- kvclient.RangeFeedEnvelope{RangeFeedEvent: &roachpb.RangeFeedEvent{Checkpoint: checkpoint}}:
...
func (s *eventStream) onSpanCompleted(ctx context.Context, sp roachpb.Span) error {
	...
	case <-ctx.Done():
		return ctx.Err()
	case s.eventsCh <- kvclient.RangeFeedEnvelope{
...
func (s *eventStream) onDeleteRange(ctx context.Context, delRange *roachpb.RangeFeedDeleteRange) {
	select {
	case <-ctx.Done():
	case s.eventsCh <- kvclient.RangeFeedEnvelope{RangeFeedEvent: &roachpb.RangeFeedEvent{DeleteRange: delRange}}:
