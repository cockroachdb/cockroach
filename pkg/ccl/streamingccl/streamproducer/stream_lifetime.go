...
func buildReplicationStreamSpec(
	...
	jobExecCtx := eval.JobExecContext.(sql.JobExecContext)
...
func setupSpanConfigsStream(
	ctx context.Context, eval *eval.Context, txn isql.Txn, tenantName roachpb.TenantName,
) (*streampb.ReplicationStreamSpec, error) {

	tenantRecord, err := sql.GetTenantRecordByName(ctx, eval.Settings, txn, tenantName)
	if err != nil {
		return nil, err
	}
	...
	// TODO(msbutler): shorten this span to the keyspan within the span config table relevant to this specific tenant.
	span := roachpb.Span{Key: spanConfigKey, EndKey: spanConfigKey.PrefixEnd()}
	return buildReplicationStreamSpec(ctx, eval, tenantID, true, roachpb.Spans{span})
}
