...
func startReplicationProducerJob(
	...
	span, targetToProtect, err := getTargetSpans(ctx, tenantID, forSpanConfigs, execConfig)
	if err != nil {
		return streampb.ReplicationProducerSpec{}, err
	}
	...
	pts := jobsprotectedts.MakeRecord(ptsID, int64(jr.JobID), statementTime,
		roachpb.Spans{span}, jobsprotectedts.Jobs, targetToProtect)
...
func getTargetSpans(
	ctx context.Context, tenantID uint64, forSpanConfigs bool, execConfig *sql.ExecutorConfig,
) (roachpb.Span, *ptpb.Target, error) {
	if !forSpanConfigs {
		span := makeTenantSpan(tenantID)
		targetToProtect := ptpb.MakeTenantsTarget([]roachpb.TenantID{roachpb.MustMakeTenantID(tenantID)})
		return span, targetToProtect, nil
	}
	...
	targetToProtect := ptpb.MakeSchemaObjectsTarget(descpb.IDs{spanConfigID})
	return span, targetToProtect, nil
}
