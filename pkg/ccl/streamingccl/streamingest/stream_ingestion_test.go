...
func TestStreamIngestionJobWithRandomClient(t *testing.T) {
	...
	knobs := base.TestingKnobs{
		DistSQL: &execinfra.TestingKnobs{StreamIngestionTestingKnobs: &sql.StreamIngestionTestingKnobs{
			Interceptors: []func(event streamingccl.Event){cancelJobAfterCheckpoints, registerValidator},
		},
		},
	}
	...
	_, conn10 := serverutils.StartTenant(t, tc.Server(0),
		base.TestTenantArgs{TenantID: roachpb.MakeTenantID(uint64(tenantID))})
	defer conn10.Close()
	...
	testutils.SucceedsSoon(t, func() error {
		...
		if jobs.Status(status) != jobs.StatusCanceled {
			return errors.New("could not cancel job")
		}
		return nil
	})
	...
	// Lastly, check for two tables in the tenant, since we ingested 2 partitions.
	tenant10.CheckQueryResults(t, `SELECT table_name FROM [SHOW TABLES] ORDER BY table_name`,
		[][]string{{"foo52"}, {"foo53"}, {"foo54"}})
}
