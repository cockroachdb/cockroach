...
func TestHeartbeatSender(t *testing.T) {
	...
	done, _, err := hbs.maybeHeartbeat(ctx, hlc.Timestamp{WallTime: timeutil.Now().UnixNano()}, time.Second)
...
func TestHeartbeatLoop(t *testing.T) {
	...
	for _, tc := range []testCase{
		...
		hbs.startHeartbeatLoop(ctx)
		// Heartbeat should be called at least once.
		<-heartbeatCh
		if tc.expectedRetry {
			// We're expecting a retry, therefore Heartbeat should be called again.
			<-heartbeatCh
			// Make sure the test doesn't have an expectedErr because we expect the loop to retry forever.
