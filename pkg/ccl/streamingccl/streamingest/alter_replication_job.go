...
func alterReplicationJobTypeCheck(
	...
	if cutoverTime := alterStmt.Cutover; cutoverTime != nil {
		if cutoverTime.Timestamp != nil {
			if _, err := typeCheckCutoverTime(ctx, &p.ExtendedEvalContext().Context, p.SemaCtx(), cutoverTime.Timestamp); err != nil {
				return false, nil, err
			}
		}
	}
		...
		if alterTenantStmt.Cutover != nil {
			if alterTenantStmt.Cutover.Latest {
				...
				if ts == nil || ts.IsEmpty() {
					return errors.Newf("replication job for tenant %q has not yet recorded a safe replication time", tenantName)
				}
				cutoverTime = *ts
			}
			...
			if stats.IngestionDetails.ProtectedTimestampRecordID == nil {
				return errors.New("replication job has not yet recorded a retained timestamp")
			} else {
				...
				if cutoverTime.Less(record.Timestamp) {
					return errors.Newf("%s is before earliest safe completion time %s", cutoverTime, record.Timestamp)
				}
			}
