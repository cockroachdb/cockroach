...
func checkLaggingNodes(
	...
	laggingNode, maxLagDifference := computeMeanLagDifference(ctx, executionDetails)
	log.VEventf(ctx, 2, "computed mean lag diff: %d lagging node, difference %.2f", laggingNode, maxLagDifference.Minutes())
	if maxAllowableLag < maxLagDifference {
		return errors.Wrapf(ErrNodeLagging, "node %d is %.2f minutes behind the average frontier. Try replanning", laggingNode, maxLagDifference.Minutes())
	}
	return nil
}
