...
func TestRevertTenantToTimestampPTS(t *testing.T) {
	...
	gcTenantTableRange := func(t *testing.T, rangeID int64, tableID int64) {
		...
		testutils.SucceedsSoon(t, func() error {
			repl = store.LookupReplica(roachpb.RKey(tableStartKey))
			if repl == nil {
	...
	// Wait until these updates are definitely past our new
	// gc.ttl.
	//
	// TODO(ssd): We need to wait before we stop service, to
	// ensure that our gc.ttlseconds config is reconcilled before
	// we shut down.
	//
	// TODO(ssd): We also need to wait until the gc.ttlseconds
	// zone configuration is translated to the host tenant.
	...
	// TODO(ssd):This sleep is the worst. PTS records aren't
	// verified. Since we are forcing the GC to run, we want the
	// PTS record to be in place before that happens.
	time.Sleep(2 * time.Second)
	gcTenantTableRange(t, rangeID, tableID)
