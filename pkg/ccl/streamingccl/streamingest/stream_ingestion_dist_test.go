...
func TestSourceDestMatching(t *testing.T) {
	...
	validate := func(sipSpecs []*execinfrapb.StreamIngestionDataSpec, dstNodes []sql.InstanceLocality,
		expected map[pair]struct{}) {
		for i, spec := range sipSpecs {

			//sip's are created in the order of the destination node ids
	...
	// For a variety of src dest topologies, ensure the right matching occurs
	for _, tc := range []struct {
		name          string
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			validate(sipSpecs, tc.dstNodes, tc.expectedPairs)
		})
	}
}
