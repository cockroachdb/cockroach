...
func testingSetCutoverSignalFrequency(frequency time.Duration) (cleanup func()) {
	prevFrequency := checkForCutoverSignalFrequency
	checkForCutoverSignalFrequency = frequency
	return func() {
		checkForCutoverSignalFrequency = prevFrequency
	}
}
...
func (s *streamIngestionResumer) checkForCutoverSignal(
	ctx context.Context, registry *jobs.Registry, stopPoller chan struct{}, cancelIngestionCtx func(),
) error {
	tick := time.NewTicker(checkForCutoverSignalFrequency)
	defer tick.Stop()
	for {
		select {
		case <-stopPoller:
...
func (s *streamIngestionResumer) Resume(resumeCtx context.Context, execCtx interface{}) error {
	...
	g.GoCtx(func(ctx context.Context) error {
		return s.checkForCutoverSignal(ctx, p.ExecCfg().JobRegistry, stopPoller, cancelIngest)
	})
