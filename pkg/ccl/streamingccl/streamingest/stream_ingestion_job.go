...
func ingestWithRetries(
	...
	for r := retry.Start(ro); r.Next(); {
		...
		latestReplicatedTime := loadRetainedTime(ctx, execCtx.ExecCfg().InternalDB, ingestionJob)
		if lastReplicatedTime.Less(latestReplicatedTime) {
			r.Reset()
			lastReplicatedTime = latestReplicatedTime
		}
	}
	if err != nil {
		return err
...
func loadRetainedTime(ctx context.Context, db isql.DB, ingestionJob *jobs.Job) hlc.Timestamp {
