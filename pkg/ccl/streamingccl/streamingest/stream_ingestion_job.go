...
func (s *streamIngestionResumer) Resume(resumeCtx context.Context, execCtx interface{}) error {
	...
	handleResumeError := func(err error) error {
	...
	// Cutover should be the first thing checked upon resumption as it is the most
	// critical task in disaster recovery
	reverted, err := maybeRevertToCutoverTimestamp(resumeCtx, execCtx, s.job.ID())
	if err != nil {
		return handleResumeError(err)
	}
	if reverted {
...
// revertToCutoverTimestamp attempts a cutover and errors out if one was not executed
...
func maybeRevertToCutoverTimestamp(
	...
	if progress.GetHighWater().Less(cutoverTime) {
