...
func (sip *streamIngestionProcessor) maybeSizeFlush() (*jobspb.ResolvedSpans, error) {
	...
	if kvBufMax > 0 && sip.curKVBatchSize >= kvBufMax {
		log.Infof(sip.Ctx(), "flushing because current KV batch based on size %d >= %d", sip.curKVBatchSize, kvBufMax)
		return sip.flush()
	} else if rkBufMax > 0 && sip.rangeBatcher.bufferSize() >= rkBufMax {
		log.Infof(sip.Ctx(), "flushing beacuse current range key batch based on size %d >= %d", sip.rangeBatcher.bufferSize(), rkBufMax)
		return sip.flush()
	}
	return nil, nil
}
...
func (r *rangeKeyBatcher) flush(ctx context.Context) error {
	...
	log.Infof(ctx, "flushing %d range keys", len(r.curRangeKVBatch))
...
func splitRangeKeySSTAtKey(
	...
	var (
		...
		// we'll swap in a new writer to write into the right
		// side.
		writer = storage.MakeIngestionSSTWriter(ctx, st, left)
	)
...
func (sip *streamIngestionProcessor) flush() (*jobspb.ResolvedSpans, error) {
	...
	if len(sip.rangeBatcher.curRangeKVBatch) > 0 {
		...
		if err := sip.rangeBatcher.flush(ctx); err != nil {
			log.Infof(ctx, "flush error: %v", err)
