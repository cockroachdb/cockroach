...
func newStreamIngestionDataProcessor(
	...
	if err != nil {
		return nil, errors.Wrap(err, "creating stream client")
	}
...
func (sip *streamIngestionProcessor) consumeEvents() (*jobspb.ResolvedSpan, error) {
	for event := range sip.eventCh {
		...
		case streamingccl.CheckpointEvent:
			...
			// partition address.
			//
			// TODO: Fix the issue where partition spans may overlap if one is a
			// prefix of another (e.g. partitions ["test://pABC", "test://pABCD"]).
