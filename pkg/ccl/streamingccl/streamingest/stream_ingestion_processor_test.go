...
func TestStreamIngestionProcessor(t *testing.T) {
	...
	out, err := runStreamIngestionProcessor(t, ctx, kvDB, "some://stream", startTime, mockClient)
...
func TestRandomClientGeneration(t *testing.T) {
	...
	valueRange := 100
	kvsPerResolved := 1_000_000
	kvFrequency := 50 * time.Nanosecond
	streamAddr := makeTestStreamURI(tableID, valueRange, kvsPerResolved, kvFrequency)

	startTime := hlc.Timestamp{WallTime: timeutil.Now().UnixNano()}

	out, err := runStreamIngestionProcessor(t, ctx, kvDB, streamAddr, startTime, nil /* mockClient */)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.Greater(t, numRows, 0)

	require.Greater(t, numResolvedEvents, 0)
}
