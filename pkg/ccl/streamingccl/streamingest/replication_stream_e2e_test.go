...
func TestTenantStreamingProducerJobTimedOut(t *testing.T) {
	...
	c.SrcSysSQL.Exec(t, fmt.Sprintf(`ALTER TENANT '%s' SET REPLICATION EXPIRATION ='1m'`, c.Args.SrcTenantName))
	...
	c.SrcSysSQL.Exec(t, fmt.Sprintf(`ALTER TENANT '%s' SET REPLICATION EXPIRATION ='100ms'`, c.Args.SrcTenantName))
