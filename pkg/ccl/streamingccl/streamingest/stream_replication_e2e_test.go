...
func (c *tenantStreamingClusters) cutover(producerJobID, ingestionJobID int, cutoverTime time.Time) {
...
func createtenantStreamingClusters(
	t *testing.T,
	ctx context.Context,
	args tenantStreamingClustersArgs) (*tenantStreamingClusters, func()) {
...
func getClockTimestamp(t *testing.T, sql *sqlutils.SQLRunner) time.Time {
	var currTime time.Time
	sql.QueryRow(t, "SELECT clock_timestamp()").
		Scan(&currTime)
	return currTime
}
...
func TestPartitionedTenantStreamingEndToEnd(t *testing.T) {
	...
	testTenantStreaming := func(t *testing.T, withInitialScan bool) {
		...
		c, cleanup := createtenantStreamingClusters(t, ctx, tenantStreamingClustersArgs{
