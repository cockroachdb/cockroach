...
var invalid_username = "invalid_user"

var keyId1 = "test_kid1"
var keyId2 = "test_kid2"
var invalid_key_id = "invalid_key_id"
...
func createJWT(t *testing.T, subject string, audience string, issuer string, expiredAt time.Time, key jwk.Key, algorithm jwa.SignatureAlgorithm) []byte {
	token := jwt.New()
...
func TestJWTEnabledCheck(t *testing.T) {
	...
	token := createJWT(t, username1, audience, issuer1, time.Now().Add(time.Hour), key, jwa.RS256)
...
func TestJWTSingleKey(t *testing.T) {
	...
	err = verifier.ValidateJWTLogin(s.ClusterSettings(), username.MakeSQLUsernameFromPreNormalizedString(invalid_username), token)
	require.ErrorContains(t, err, "JWT authentication: invalid token")
...
func TestJWTMultiKey(t *testing.T) {
	...
	token := createJWT(t, username1, audience, issuer1, time.Now().Add(time.Hour), key2, jwa.ES384)
	publicKey, err := key.PublicKey()
	require.NoError(t, err)
...
func TestExpiredToken(t *testing.T) {
	...
	token := createJWT(t, username1, audience, issuer1, time.Now().Add(-1*time.Second), key, jwa.RS256)
	JWTAuthJWKS.Override(ctx, &s.ClusterSettings().SV, serializePublicKeySet(t, keySet))
	verifier := ConfigureJWTAuth(ctx, s.AmbientCtx(), s.ClusterSettings(), s.StorageClusterID())
...
func TestKeyIdMismatch(t *testing.T) {
	...
	key.Set(jwk.KeyIDKey, invalid_key_id)
	token := createJWT(t, username1, audience, issuer1, time.Now().Add(time.Hour), key, jwa.RS256)
	// Make sure jwt auth is enabled and accepts valid signing keys.
	JWTAuthEnabled.Override(ctx, &s.ClusterSettings().SV, true)
	JWTAuthJWKS.Override(ctx, &s.ClusterSettings().SV, serializePublicKeySet(t, keySet))
...
func TestSubjectCheck(t *testing.T) {
	...
	token := createJWT(t, username1, audience, issuer2, time.Now().Add(time.Hour), key2, jwa.ES384)
...
func TestAudienceCheck(t *testing.T) {
	...
	defer s.Stopper().Stop(ctx)
	keySet, key, _ := createJWKS(t)
	token := createJWT(t, username1, audience, issuer2, time.Now().Add(time.Hour), key, jwa.RS256)
...
func TestIssuerCheck(t *testing.T) {
	...
	defer s.Stopper().Stop(ctx)
	keySet, key, _ := createJWKS(t)
	token1 := createJWT(t, username1, audience, issuer1, time.Now().Add(time.Hour), key, jwa.RS256)
	token2 := createJWT(t, username1, audience, issuer2, time.Now().Add(time.Hour), key, jwa.RS256)
