...
func TestJWTAuthWithCustomCACert(t *testing.T) {
	...
	certPEMBlock, err := securitytest.EmbeddedAssets.ReadFile(
		filepath.Join(certnames.EmbeddedCertsDir, certnames.EmbeddedNodeCert))
	require.NoError(t, err)
	keyPEMBlock, err := securitytest.EmbeddedAssets.ReadFile(
	...
	// provided in the cluster settings.
	publicKeyPEM, err := securitytest.EmbeddedAssets.ReadFile(
