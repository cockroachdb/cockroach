...
func TestJWTSingleKey(t *testing.T) {
	...
	detailedError, err := verifier.ValidateJWTLogin(ctx, s.ClusterSettings(), username.MakeSQLUsernameFromPreNormalizedString(invalidUsername), token, identMap)
	require.ErrorContains(t, err, "JWT authentication: unable to validate token")
	require.EqualValues(t, "unable to fetch jwks: Get \"issuer1/.well-known/openid-configuration\": unsupported protocol scheme \"\"", string(detailedError))
...
func NewLocalHTTPSTestServerCustomCA(
	handler http.Handler, customCACert tls.Certificate,
) *httptest.Server {
	ts := httptest.NewUnstartedServer(handler)
	ts.TLS = &tls.Config{Certificates: []tls.Certificate{customCACert}}
	ts.StartTLS()
	return ts
}

func TestJWTAuthCanUseCustomCACert(t *testing.T) {
	defer leaktest.AfterTest(t)()
	ctx := context.Background()

	tlsCert, err := tls.LoadX509KeyPair("testdata/node.crt", "testdata/node.key")
	require.NoError(t, err)

	// start proxy server with tls using customCA cert
	proxy := NewLocalHTTPSTestServerCustomCA(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		_, _ = w.Write([]byte(fmt.Sprintf("proxied-%s", r.URL)))
	}), tlsCert)
	defer proxy.Close()
	...
	authenticator := jwtAuthenticator{}
	_, err = getHttpResponse(ctx, "http://my-server/.well-known/openid-configuration", &authenticator)
	require.ErrorContains(t, err, "proxyconnect tcp: tls: failed to verify "+
		"certificate: x509: ")

	// http library throws 2 different error failures locally and on teamcity.
	// This is to handle both cases.
	errString := err.Error()
	failureReason1 := "certificate signed by unknown authority"
	failureReason2 := "“node” certificate is not standards compliant"
	if !(strings.Contains(errString, failureReason1) ||
		strings.Contains(errString, failureReason2)) {
		t.Fatalf("expected error %q to contain %q or %q", err, failureReason1, failureReason2)
	}

	publicKeyPem, err := os.ReadFile("testdata/ca.crt")
	require.NoError(t, err)

	authenticator.mu.conf.customCA = string(publicKeyPem)
	res, err := getHttpResponse(ctx, "http://my-server/.well-known/openid-configuration", &authenticator)
	require.NoError(t, err)
	require.EqualValues(t, "proxied-http://my-server/.well-known/openid-configuration", string(res))
}
