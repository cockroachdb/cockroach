...
//
// The clusterUUID field is used to check the validity of the enterprise license. It is set once at initialization.
// The conf field contains all the values that come from cluster settings.
// The enabled field represents the present state of if this feature is enabled. When combined with the
// enabled value of conf, it allows us to detect when this feature becomes enabled.
...
func (authenticator *jwtAuthenticator) reloadConfig(
	...
	authenticator.mu.mutex.Lock()
	defer authenticator.mu.mutex.Unlock()
...
func (authenticator *jwtAuthenticator) ValidateJWTLogin(
	...
	if parsedToken.Subject() != user.Normalized() {
		return errors.WithDetailf(errors.Newf("JWT authentication: invalid subject"), "Token issued for %s but login was for %s", parsedToken.Subject(), user.Normalized())
	}
	audienceMatch := false
	...
	if !audienceMatch {
		return errors.WithDetailf(errors.Newf("JWT authentication: invalid audience"), "Token issued with an audience of %s", parsedToken.Audience())
	}
	...
	if !issuerMatch {
		return errors.WithDetailf(errors.Newf("JWT authentication: invalid issuer"), "Token issued by %s", parsedToken.Issuer())
	}
...
var ConfigureJWTAuth = func(
	...
	jwtAuthenticator.clusterUUID = clusterUUID
	jwtAuthenticator.reloadConfig(serverCtx, st)
	JWTAuthAudience.SetOnChange(&st.SV, func(ctx context.Context) {
		jwtAuthenticator.reloadConfig(ambientCtx.AnnotateCtx(ctx), st)
	})
