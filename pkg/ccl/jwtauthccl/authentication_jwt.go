	...
	if authenticator.mu.conf.claim == "" || authenticator.mu.conf.claim == "sub" {
		...
		case []interface{}:
			...
			for _, maybePrincipal := range castClaimValue {
				switch principal := maybePrincipal.(type) {
				case string:
					tokenPrincipals = append(tokenPrincipals, principal)
				default:
					return errors.WithDetailf(
						errors.Newf("JWT authentication: invalid claim value"),
						"values for claim %s must be strings or string arrays", authenticator.mu.conf.claim)
				}
			}
		case []string:
