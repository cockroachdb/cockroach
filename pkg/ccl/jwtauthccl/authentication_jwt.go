...
type jwtAuthenticatorConf struct {
	...
	issuers             []string
	jwks                jwk.Set
	issuerToIdentityMap map[string]string
}
...
func (authenticator *jwtAuthenticator) mapUsername(tokenUsername string, issuer string, identMap *identmap.Conf) ([]username.SQLUsername, error) {
	if !identMap.HasMap(issuer) {
	...
	return identMap.Map(issuer, tokenUsername)
}
...
func (authenticator *jwtAuthenticator) ValidateJWTLogin(
	...
	if user.IsRootUser() || user.IsReserved() {
		return errors.WithDetailf(
			...
			"cannot use JWT auth to login to a reserved user")
		user.Normalized()
	}
	audienceMatch := false
