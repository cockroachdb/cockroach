...
func TestRegionAddDropFailureEnclosingRegionalByRowAlters(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:           "region-drop-alter-from-global",
			setup:          `CREATE TABLE db.t () LOCALITY GLOBAL`,
			regionAlterCmd: `ALTER DATABASE db DROP REGION "us-east3"`,
		},
		{
			name:           "region-drop-alter-from-regional",
			setup:          `CREATE TABLE db.t () LOCALITY REGIONAL`,
			regionAlterCmd: `ALTER DATABASE db DROP REGION "us-east3"`,
		},
		{
			name:           "region-add-alter-from-global",
			setup:          `CREATE TABLE db.t () LOCALITY GLOBAL`,
			regionAlterCmd: `ALTER DATABASE db ADD REGION "us-east4"`,
		},
		{
			name:           "region-add-alter-from-regional",
			setup:          `CREATE TABLE db.t () LOCALITY REGIONAL`,
			regionAlterCmd: `ALTER DATABASE db ADD REGION "us-east4"`,
		},
	}
