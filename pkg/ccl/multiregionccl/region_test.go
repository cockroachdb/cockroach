...
`)
				...
				// Wait for the type change to finish now that we've completed
				// our backup with the type change in progress. This wait
				// prevents the DROP DATABASE command below from dropping
				// the type descriptor while the alter cmd is in progress.
				<-typeChangeFinished

				// FIXME: We shouldn't need this drop here, as we're using a
				//  different cluster.
				_, err = sqlDBBackup.Exec(`DROP DATABASE db`)
				require.NoError(t, err)
				...
				// Need SucceedsSoon here because the schema change job
				// occurring as part of the restore may not be done yet.
				testutils.SucceedsSoon(t, func() error {
					...
					// Now validate the partitions on the REGIONAL BY ROW
					// tables.
					rowsPartitions, err := sqlDBRestore.Query(`
SELECT partition_name FROM [SHOW PARTITIONS FROM TABLE db.rbr] ORDER BY partition_name`)
					if err != nil {
						return err
					}
					defer rowsPartitions.Close()

					var partitionNames []string
					for rowsPartitions.Next() {
						var partitionName string
						if err := rowsPartitions.Scan(&partitionName); err != nil {
							return err
						}
					...
					if len(partitionNames) != len(regionAlterCmd.expectedPartitions) {
						return errors.Newf(
					...
					for i, expectedPartition := range regionAlterCmd.expectedPartitions {
						if expectedPartition != partitionNames[i] {
							return errors.Newf(
