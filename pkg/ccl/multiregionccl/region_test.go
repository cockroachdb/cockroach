...
func TestMultiRegionDatabaseStats(t *testing.T) {
    ...
    with x as (show ranges from table a) select replicas from x;`)
		...
		var respNodes []string
		for _, n := range resp.Stats.NodeIDs {
			respNodes = append(respNodes, fmt.Sprint(n))
		}

		assert.Equal(t, nodes, respNodes, "The nodes returned from endpoint do not match nodes listed in the show range from table.")
