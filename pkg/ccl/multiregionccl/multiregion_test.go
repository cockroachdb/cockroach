...
// policy refresher. With the refresher enabled, we expect latency-based
// policies on global tables. Without it, no latency-based policies should
// exist.
func TestReplicaClosedTSPolicyWithPolicyRefresher(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = sqlDB.Exec(`SET CLUSTER SETTING kv.closed_timestamp.policy_refresh_interval = '1s'`)
	require.NoError(t, err)
	// TODO(wenyihu): get this sorted out
