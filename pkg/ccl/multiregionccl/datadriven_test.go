...
// server.
//
// "trace-sql idx=server_number [trace-analysis=trace_analysis_type]":
// Similar to exec-sql, but also traces the input statement and analyzes the
// trace. trace-analysis defaults to simple.
//
...
//     locally.
//   rbr:
//     - served local region search on current node: prints true iff the
//     locality-optimized search was executed on local data (as opposed to
//     reaching out to the leaseholder)
//     - served local region search via follower read: prints true iff the
//     first search was local and served by a follower read. This is omitted
//     completely if the first search was not local.
//     - found in local region search: prints true iff there was only one
//     search executed.
//     - served remote region read via follower read: prints true iff the second
//     read executed was executed via a follower read. This is omitted
//     completely if the query was served via locality-optimized search.
//
...
func analyzeRegionalByRowQuery(rec tracing.Recording) (string, error) {
	...
	var output strings.Builder
	output.WriteString(
		fmt.Sprintf("served local region search on current node: %v\n", attemptedLocalSearch),
	)
	if attemptedLocalSearch {
		output.WriteString(
			fmt.Sprintf(
				"served local region search via follower read: %v\n",
				servedLocalUsingFollowerReads,
			))
	}
	output.WriteString(
		fmt.Sprintf("found in local region search: %v\n", !foundRemoteRead),
	)
	// Only print follower read information if the query was served locally.
	if foundRemoteRead {
		output.WriteString(
			fmt.Sprintf(
				"served remote region read via follower read: %v\n",
