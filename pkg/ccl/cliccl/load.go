...
func runLoadShow(cmd *cobra.Command, args []string) error {
	...
	externalStorageFromURI := func(ctx context.Context, uri string) (cloud.ExternalStorage, error) {
		return cloud.ExternalStorageFromURI(ctx, uri, base.EnableAllExternalStorage(),
	...
	// upgraded.
	desc, err := backupccl.ReadBackupDescriptorFromURI(ctx, basepath, externalStorageFromURI, nil)
	if err != nil {
		return err
	}
