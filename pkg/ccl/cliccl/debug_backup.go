...
func runListIncrementalCmd(cmd *cobra.Command, args []string) error {
	...
	oldIncPaths, err := backupdestination.FindPriorBackups(ctx, baseStore, backupbase.OmitManifest)
	if err != nil {
		return err
	}
	...
	newIncPaths, err := backupdestination.FindPriorBackups(ctx, incStore, backupbase.OmitManifest)
	if err != nil {
		return err
	}
