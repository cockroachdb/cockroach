...
func TestLoadShowBackups(t *testing.T) {
	...
	ts := generateContinuousTimeStamps(3)
...
// Generate n continuous TimeStamps with minimal interval of 10 milliseconds to be used in incremental backup tests.
func generateContinuousTimeStamps(n int) []hlc.Timestamp {
	timeStamps := make([]hlc.Timestamp, 0, n)
	if n == 0 {
		return timeStamps
	}
	lastTS := hlc.Timestamp{WallTime: timeutil.Now().UnixNano()}
	timeStamps = append(timeStamps, lastTS)
	for {
		newTS := hlc.Timestamp{WallTime: timeutil.Now().UnixNano()}
		if lastTS.Add(time.Millisecond.Nanoseconds()*10, 0).Less(newTS) {
