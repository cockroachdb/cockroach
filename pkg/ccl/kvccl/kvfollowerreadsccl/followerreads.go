...
func (r bulkOracle) ChoosePreferredReplica(
	...
	if r.locFilter.NonEmpty() {
		...
		if len(matches) > 0 {
			return replicas[matches[randutil.FastUint32()%uint32(len(matches))]].ReplicaDescriptor, false, nil
		}
	}
	return replicas[randutil.FastUint32()%uint32(len(replicas))].ReplicaDescriptor, false, nil
}
