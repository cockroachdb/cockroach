...
func TestFollowerReadsWithStaleDescriptor(t *testing.T) {
	...
	// n1.Exec(t, `SET CLUSTER SETTING kv.follower_read.target_multiple = 5`)
	...
	// doesn't have a replica any more and then it'll try n1 which will return an
	// updated descriptor.
	n4.Exec(t, historicalQuery)
	// As a sanity check, verify that this was not a follower read.
	rec := <-recCh
	require.False(t, tracing.OnlyFollowerReads(rec), "query was not served through follower reads: %s", rec)
	...
	require.True(t, tracing.OnlyFollowerReads(rec), "query was not served through follower reads: %s", rec)
