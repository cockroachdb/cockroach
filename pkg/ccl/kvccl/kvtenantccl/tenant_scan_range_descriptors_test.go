...
func TestScanRangeDescriptors(t *testing.T) {
	...
	scanner := tenant2.RangeDescriptorScanner().(rangedesc.Scanner)
	rangeDescs, err := scanner.Scan(ctx, roachpb.Span{
	...
	scanner = tc.Server(0).RangeDescriptorScanner().(rangedesc.Scanner)
	rangeDescs, err = scanner.Scan(ctx, keys.EverythingSpan)
	require.NoError(t, err)
	require.Len(t, rangeDescs, 57)
	// Last range we created above.
	require.Equal(
		t,
		keys.MustAddr(ten2Codec.TenantPrefix().PrefixEnd()),
		rangeDescs[56].StartKey,
	)
}
