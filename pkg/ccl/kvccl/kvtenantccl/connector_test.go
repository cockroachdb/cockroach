...
func waitForStoreDesc(t *testing.T, c *Connector, storeID roachpb.StoreID) {
	t.Helper()
	testutils.SucceedsSoon(t, func() error {
		_, err := c.GetNodeDescriptorByStoreID(storeID)
		return err
	})
}
...
func TestConnectorGossipSubscription(t *testing.T) {
	...
	// Test GetNodeDescriptorByStoreID.
	...
	desc, err = c.GetNodeDescriptorByStoreID(storeID1)
	require.Equal(t, node1, desc)
	waitForStoreDesc(t, c, storeID2)
	desc, err = c.GetNodeDescriptorByStoreID(storeID2)
	require.Equal(t, node2, desc)
	desc, err = c.GetNodeDescriptorByStoreID(3)
	require.Nil(t, desc)
	require.Error(t, err)
	require.Contains(t, err.Error(), "unable to look up node ID for store ID 3")
