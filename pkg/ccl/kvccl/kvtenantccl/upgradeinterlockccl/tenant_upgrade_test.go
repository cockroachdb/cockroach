...
func TestTenantUpgradeInterlock(t *testing.T) {
	...
	runTest := func(t *testing.T, variant interlockTestVariant, test interlockTestConfig) {
		...
		t.Logf("creating an initial tenant server")
		...
		t.Logf("verifying the version of the storage cluster")
		...
		t.Logf("upgrading the storage cluster")
		hostClusterRunner.Exec(t, "SET CLUSTER SETTING version = $1", bv.String())

		t.Logf("checking the tenant after the storage cluster upgrade")
		// Ensure that the tenant still works.
		initialTenantRunner.CheckQueryResults(t, "SELECT * FROM t", [][]string{{"1"}, {"2"}})

		t.Logf("start upgrading the tenant")
		...
		// starting.
		<-reachedChannel
		t.Logf("upgrader is ready")

		t.Logf("starting another tenant server")
		...
		if expectingStartupToFail {
			t.Logf("shutting down the other tenant server")
		...
		t.Logf("waiting for the instance table to get in the right state")
		...
		// complete.
		t.Logf("resuming upgrade")
		resumeChannel <- struct{}{}
		t.Logf("waiting for upgrade to complete")
		<-completedChannel
		t.Logf("upgrade completed")
		...
		if test.expStartupErr[variant] == "" {
			...
			t.Logf("check the second server still works")
			otherTenantRunner.CheckQueryResults(t, "SELECT * FROM t", [][]string{{"1"}, {"2"}})
			t.Logf("waiting for second server to reach target final version")
	...
	for variant := range variants {
		...
		t.Run(variantName, func(t *testing.T) {
			for i := range tests {
				test := tests[i]
				t.Run(test.name, func(t *testing.T) { runTest(t, variant, test) })
			}
		})
	}
}
