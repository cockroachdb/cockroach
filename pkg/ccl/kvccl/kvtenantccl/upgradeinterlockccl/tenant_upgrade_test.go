...
func TestTenantUpgradeInterlock(t *testing.T) {
	...
	t.Logf("to troubleshoot this test effectively, consider using -test.parallel=1")
	...
	for v := range variants {
		...
		t.Run(variantName, func(t *testing.T) {
			for i := range tests {
				// Copy the loop variables so that the closure below gets a
				// different value in every sub-test.
				test := tests[i]
				variant := v
				t.Run(test.name, func(t *testing.T) {
					// We can run these tests in parallel: each of them only runs 3 servers.
					t.Parallel()
