...
// GetSpanConfigEntries implements the config.SpanConfigAccessor interface.
func (c *Connector) GetSpanConfigsFor(ctx context.Context, span roachpb.Span) ([]roachpb.SpanConfigEntry, error) {
	for ctx.Err() == nil {
		client, err := c.getClient(ctx)
		if err != nil {
			continue
...
func (c *Connector) UpdateSpanConfigEntries(ctx context.Context, upsert, delete []roachpb.SpanConfigEntry) error {
