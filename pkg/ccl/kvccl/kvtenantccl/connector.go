...
func (c *Connector) GetNodeDescriptorByStoreID(
	...
	if !ok {
		return nil, errors.Errorf("unable to look up node ID for store ID %d", storeID)
	}
	desc, ok := c.mu.nodeDescs[nodeID]
	if !ok {
		return nil, errors.Errorf("unable to look up descriptor for node ID %d store ID %d", nodeID, storeID)
	}
	return desc, nil
}
