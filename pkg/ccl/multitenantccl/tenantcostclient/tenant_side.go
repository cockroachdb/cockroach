...
func newTenantSideCostController(
	...
	c.limiter.Reconfigure(timeSource.Now(), tokenBucketReconfigureArgs{
...
func (c *tenantSideCostController) onTick(ctx context.Context, newTime time.Time) {
	...
	if !c.run.fallbackRateStart.IsZero() && !newTime.Before(c.run.fallbackRateStart) {
		...
		c.limiter.Reconfigure(c.timeSource.Now(), tokenBucketReconfigureArgs{
...
func (c *tenantSideCostController) OnRequestWait(ctx context.Context) error {
	...
	// initial rate.
	// Send needed=0 for requests, since the exact cost won't be known until a
	// successful response is received (i.e. b/c read responses can be of any
	// size and b/c both read and write requests might result in an transient
	// error, which shouldn't be charged).
