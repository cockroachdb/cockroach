...
const anticipation = 1 * time.Second
...
func (c *tenantSideCostController) onTick(ctx context.Context, newTime time.Time) {
	...
	sqlRU := c.costCfg.PodCPUCost(deltaCPU)
	...
	if newExternalUsage.PGWireEgressBytes > c.run.externalUsage.PGWireEgressBytes {
		...
		sqlRU += c.costCfg.PGWireEgressCost(int64(deltaPGWireEgressBytes))
	}
	...
	c.mu.consumption.RU += float64(sqlRU)
	newConsumption := c.mu.consumption
	kvRU := tenantcostmodel.RU(c.mu.consumption.KVRU - c.run.consumption.KVRU)
	c.mu.Unlock()
	...
	avg := float64(sqlRU+kvRU) * float64(time.Second) / float64(deltaTime)
	c.run.avgRUPerSec = movingAvgRUPerSecFactor*avg + (1-movingAvgRUPerSecFactor)*c.run.avgRUPerSec

	// Notify the limiter that another tick has elapsed, passing it the SQL and
	// KV RUs consumed during the tick.
	c.limiter.OnTick(newTime, sqlRU, kvRU)
...
func (c *tenantSideCostController) OnRequestWait(ctx context.Context) error {
	...
	// initial rate.
	// Don't amortize SQL RU with this request; wait until a successful response
	// to do so.
	return c.limiter.Wait(ctx, 0, false /* amortize */)
}
...
func (c *tenantSideCostController) OnResponseWait(
	...
	// Amortize any waiting SQL RU with this response.
	if err := c.limiter.Wait(ctx, totalRU, true /* amortize */); err != nil {
		return err
	}
...
func (c *tenantSideCostController) onExternalIO(
	...
	if wait {
		// Don't amortize waiting SQL RU with external IO RU.
		if err := c.limiter.Wait(ctx, totalRU, false /* amortize */); err != nil {
			return err
		}
	} else {
