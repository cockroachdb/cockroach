...
// OnRequestWait is part of the multitenant.TenantSideBatchInterceptor
// interface.
...
func (c *tenantSideCostController) OnResponse(ctx context.Context, br *roachpb.BatchResponse) {
	info := tenantcostmodel.MakeResponseInfo(br)
	readBytes := tenantcostmodel.MakeResponseInfo(br).ReadBytes()
	if readBytes == 0 {
		return
	}
	c.mu.Lock()
