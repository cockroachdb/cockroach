...
func (c *tenantSideCostController) onTick(ctx context.Context, newTime time.Time) {
	...
	// Notify the limiter that another tick has elapsed, passing it the CPU and
	// egress usage accumulated during the tick.
...
func (c *tenantSideCostController) OnRequestWait(ctx context.Context) error {
	...
	// initial rate.
	return c.limiter.OnRequestWait(ctx)
}
...
func (c *tenantSideCostController) OnResponseWait(
	...
	if err := c.limiter.OnResponseWait(ctx, totalRU); err != nil {
		return err
	}
