...
func (c *tenantSideCostController) updateAvgRUPerSec() {
	c.run.avgRUPerSec = 0.5 * (c.run.avgRUPerSec + (c.run.consumption.RU - c.run.avgRUPerSecLastRU))
...
func (c *tenantSideCostController) sendTokenBucketRequest(ctx context.Context) {
	...
	c.stopper.RunAsyncTask(ctx, "token-bucket-request", func(ctx context.Context) {
		if log.V(1) {
