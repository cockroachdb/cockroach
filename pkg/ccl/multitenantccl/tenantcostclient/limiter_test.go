		...
		if req != nil {
			if tenantcostmodel.RU(req.needed) <= lim.AvailableRU(ts.Now()) {
				...
				fmt.Fprintf(out, "%8.3f,%14.2f,%19d,%18.1f,%14.2f,%10.2f\n",
					...
					req.needed, latency, d, lim.AvailableRU(ts.Now()), lim.extraUsage())

				req = nil
			} else {
				fmt.Fprintf(out, "%8.3f,              ,                   ,%18.1f,%14.2f,%10.2f\n",
					(time.Duration(tick) * tickDuration).Seconds(),
					d, lim.AvailableRU(ts.Now()), lim.extraUsage())
			}
		}

		if req == nil {
			// Create a request now.
			req = &waitRequest{needed: quotapool.Tokens(randRu() + lim.amortizeExtraRU())}
			reqTick = tick
		}
