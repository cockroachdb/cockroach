...
func newTickWaiter() *tickWaiter {
	return &tickWaiter{ch: make(chan struct{}, 1)}
}

func (tw *tickWaiter) Event(now time.Time, typ tenantcostclient.TestEventType) {
	if typ == tenantcostclient.TickProcessed {
		// Non-blocking send.
		select {
		case tw.ch <- struct{}{}:
		default:
...
func (tw *tickWaiter) HasTick() bool {
	return len(tw.ch) > 0
}

func (tw *tickWaiter) Wait() {
	// Wait to receive tick processed notification. Time out after 1 millisecond
	// to handle race condition where the controller is still starting up and has
	// not yet created its ticker.
	select {
	case <-tw.ch:
...
func TestWaitingRU(t *testing.T) {
	...
	// Refill the token bucket at a fixed 100 RU/s so that we can control it.
	testProvider := newTestProvider()
	testProvider.configure(testProviderConfig{ProviderError: true, FallbackRate: 100})

	tenantID := serverutils.TestTenantID()
	timeSource := timeutil.NewManualTime(t0)
	tickWait := newTickWaiter()
	ctrl, err := tenantcostclient.TestingTenantSideCostController(
		st, tenantID, testProvider, timeSource, tickWait)
	...
	testutils.SucceedsSoon(t, func() error {
		...
		return errors.New("RUs did not drop below 1K")
	})
}
