...
func (ts *testState) readRequest(t *testing.T, d *datadriven.TestData) string {
	ba := tenantcostmodel.MockBatchRequest(false /* isWrite */, 0 /* writeBytes */)
	if err := ts.controller.OnRequestWait(context.Background(), ba); err != nil {
		d.Fatalf(t, "%v", err)
	}
	return ""
}
...
func (ts *testState) readResponse(t *testing.T, d *datadriven.TestData) string {
	br := tenantcostmodel.MockBatchResponse(bytesArg(t, d))
	ts.controller.OnResponse(context.Background(), br)
	return ""
}

func (ts *testState) writeRequest(t *testing.T, d *datadriven.TestData) string {
	ba := tenantcostmodel.MockBatchRequest(true /* isWrite */, bytesArg(t, d))
	if err := ts.controller.OnRequestWait(context.Background(), ba); err != nil {
		d.Fatalf(t, "%v", err)
	}
	return ""
