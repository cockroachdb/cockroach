...
// are updated every second and passed to the limiter's OnTick method.
//
...
// notify sends a non-blocking notification on lowRUNotify (unless there is
// already a notification pending in the channel), and disables further
...
func (req *waitRequest) Acquire(
	...
	if !fulfilled {
		...
		// it looks like we have enough RUs. Don't wait until OnWaitStart, since
		// there is a small unlocked window between now and then where waitingRU
		// will be incorrect.
		l.maybeAddWaitingRULocked(req)
	} else {
		// If this request was part of waitingRU, eagerly subtract it now that it's
		// complete. Don't wait until OnWaitFinish, since there is a small unlocked
		// window between now and then where waitingRU will be incorrect.
