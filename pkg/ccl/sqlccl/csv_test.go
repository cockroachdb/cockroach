...
`
	...
	if _, err := db.Exec(`RESTORE csv.* FROM $1`, fmt.Sprintf("nfs://%s", tmp)); err != nil {
		t.Fatal(err)
	}
...
func makeCSVData(
	...
	for fn := 0; fn < numFiles; fn++ {
		...
		files = append(files, fmt.Sprintf(`'nfs://%s'`, path))
		filesWithOpts = append(filesWithOpts, fmt.Sprintf(`'nfs://%s'`, pathWithOpts))
		filesWithDups = append(filesWithDups, fmt.Sprintf(`'nfs://%s'`, pathDup))
	}
...
func TestImportStmt(t *testing.T) {
	...
	for i, tc := range []struct {
	...
	}{
		{
			...
			[]interface{}{fmt.Sprintf("nfs://%s", tablePath)},
			files,
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			backupPath := fmt.Sprintf("nfs://%s", filepath.Join(dir, t.Name()))
...
func BenchmarkImport(b *testing.B) {
	...
	tmp := fmt.Sprintf("nfs://%s", filepath.Join(dir, b.Name()))
