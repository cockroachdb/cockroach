...
func TestImportStmt(t *testing.T) {
	...
	for i, tc := range []struct {
		...
		},
		{
			...
			`IMPORT TABLE t (a int primary key, b string, index (b), index (a, b)) CSV DATA (%s) WITH temp = $1, comma = '|', comment = '#'`,
			nil,
			filesWithOpts,
			"",
		},
		...
		},
		{
			...
			`IMPORT TABLE t (a int primary key, b string, index (b), index (a, b)) CSV DATA (%s) WITH temp = $1, distributed, comma = '|', comment = '#'`,
			nil,
			filesWithOpts,
			"",
		},
