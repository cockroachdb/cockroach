...
func TestBackupRestoreCrossTableReferences(t *testing.T) {
	...
	// Generate some testdata and back it up.
	{
		...
		origDB.Exec(`CREATE VIEW store.ordercounts AS
	...
	origOrderCounts := origDB.QueryStr(`SELECT * from store.ordercounts ORDER BY id`)
	...
	t.Run("restore simple view", func(t *testing.T) {
		...
		if _, err := db.DB.Exec(`RESTORE store.early_customers FROM $1`, dir); !testutils.IsError(err,
			`cannot restore "early_customers" without referenced table`,
		) {
			t.Fatal(err)
		}
	...
	t.Run("restore multi-table view", func(t *testing.T) {
		...
		if _, err := db.DB.Exec(
			`RESTORE store.early_customers, store.ordercounts, store.customers FROM $1`, dir,
		); !testutils.IsError(err, `cannot restore "ordercounts" without referenced table`) {
			t.Fatal(err)
		}

		db.Exec(`RESTORE store.customers, store.ordercounts, store.orders
			FROM $1`, dir)
		...
		// customers is aware of the view that depends on it.
		if _, err := db.DB.Exec(`DROP TABLE store.customers`); !testutils.IsError(err,
			`cannot drop table "customers" because view "ordercounts" depends on it`,
		) {
			t.Fatal(err)
		}
		...
		if _, err := db.DB.Exec(`DROP TABLE store.orders`); !testutils.IsError(err,
			`cannot drop table "orders" because view "ordercounts" depends on it`,
		) {
			t.Fatal(err)
		}

		db.CheckQueryResults(`SELECT * FROM store.ordercounts ORDER BY id`, origOrderCounts)
	})
}
