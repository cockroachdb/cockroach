...
func backupRestoreTestSetupWithParams(
	...
	cleanupFn := func() {
		...
		dirCleanupFn()                    // cleans up dir, which is the nfs:// storage
	...
	return ctx, "nfs://" + dir, tc, sqlDB, cleanupFn
}
...
func TestBackupRestoreLocalPathChecks(t *testing.T) {
	...
	t.Run("multi-node", func(t *testing.T) {
		...
		sqlDB.Exec("BACKUP DATABASE data TO $1", "nfs:/"+dir)
	})
}
	...
	do := func(query string, check inProgressChecker) {
		...
		err := util.RetryForDuration(testutils.DefaultSucceedsSoonDuration, func() error {
			return check(ctx, inProgressState{
				...
				backupDir:     strings.TrimPrefix(dir, "nfs:"),
			})
		})
...
func TestBackupRestoreResume(t *testing.T) {
	...
	t.Run("backup", func(t *testing.T) {
		...
		backupDirRaw := strings.TrimPrefix(backupDir, "nfs:")
...
func TestAsOfSystemTimeOnRestoredData(t *testing.T) {
	...
	// backupRestoreTestSetup sets nfs:// on dir, so strip it off.
	...
	var beforeTs string
	sqlDB.QueryRow(`SELECT cluster_logical_timestamp()`).Scan(&beforeTs)
	sqlDB.Exec(`RESTORE data.* FROM $1`, fmt.Sprintf("nfs://%s", backup.BaseDir))
	var afterTs string
...
func TestBackupRestoreChecksum(t *testing.T) {
	...
	rawDir := strings.TrimPrefix(dir, "nfs://")
