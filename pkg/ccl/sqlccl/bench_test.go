...
func bankBuf(numAccounts int) *bytes.Buffer {
	bankData := dataccl.Bank(numAccounts, dataccl.CfgDefault, dataccl.CfgDefault)
...
func BenchmarkClusterBackup(b *testing.B) {
	...
	bankData := dataccl.Bank(b.N, dataccl.CfgDefault, dataccl.CfgDefault)
	loadDir := filepath.Join(dir, "load")
	if _, err := dataccl.ToBackup(b, bankData, loadDir); err != nil {
		b.Fatalf("%+v", err)
	}
...
func BenchmarkClusterRestore(b *testing.B) {
	...
	bankData := dataccl.Bank(b.N, dataccl.CfgDefault, dataccl.CfgDefault)
	backup, err := dataccl.ToBackup(b, bankData, dir)
	if err != nil {
		b.Fatalf("%+v", err)
	}
...
func runEmptyIncrementalBackup(b *testing.B) {
	...
	bankData := dataccl.Bank(numStatements, dataccl.CfgDefault, dataccl.CfgDefault)
	_, err := dataccl.ToBackup(b, bankData, restoreDir)
	if err != nil {
		b.Fatalf("%+v", err)
	}
	sqlDB.Exec(`DROP TABLE data.bank`)
