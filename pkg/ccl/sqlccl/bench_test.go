...
func bankBuf(numAccounts int) *bytes.Buffer {
	bankData := testdataccl.BankRows(numAccounts)
...
func BenchmarkClusterBackup(b *testing.B) {
	...
	bankData := testdataccl.BankRows(b.N)
	loadDir := filepath.Join(dir, "load")
	if _, err := testdataccl.ToBackup(b, bankData, loadDir); err != nil {
		b.Fatalf("%+v", err)
	}
...
func BenchmarkClusterRestore(b *testing.B) {
	...
	bankData := testdataccl.BankRows(b.N)
	backup, err := testdataccl.ToBackup(b, bankData, dir)
	if err != nil {
		b.Fatalf("%+v", err)
	}
...
func runEmptyIncrementalBackup(b *testing.B) {
	...
	bankData := testdataccl.BankRows(numStatements)
	_, err := testdataccl.ToBackup(b, bankData, restoreDir)
	if err != nil {
		b.Fatalf("%+v", err)
	}
	sqlDB.Exec(`DROP TABLE data.bank`)
