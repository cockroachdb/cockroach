...
func GenerateSubzoneSpans(
	...
	if err := tableDesc.ForeachNonDropIndex(func(idxDesc *sqlbase.IndexDescriptor) error {
		_, subzoneIncludesIndex := subzoneIndexByIndexID[idxDesc.ID]
		if subzoneIncludesIndex {
		...
		indexUsed := subzoneIncludesIndex || len(indexPartitionCoverings) > 0
...
func indexCoveringsForPartitioning(
	...
	if len(partDesc.List) > 0 {
		// The returned spans are required to be ordered with hightest
		// precedence first. The span for (1, DEFAULT) overlaps with (1, 2) and
		// needs to be returned at a lower precedence. Luckily, because of the
		// partitioning validation, we're guaranteed that all entries in a list
		// partitioning with the same number of DEFAULTs are non-overlapping.
		// So, bucket the `intervalccl.Range`s by the number of non-DEFAULT
		// columns and return them ordered from least # of DEFAULTs to most.
	...
	if len(partDesc.Range) > 0 {
		...
		if len(prefixDatums) > 0 {
			...
			colTypes := make([]parser.Type, len(prefixDatums))
			for i := 0; i < len(prefixDatums); i++ {
				id := idxDesc.ColumnIDs[i]
				colMap[id] = i
				column, err := tableDesc.FindColumnByID(id)
				if err != nil {
					return nil, err
				}
