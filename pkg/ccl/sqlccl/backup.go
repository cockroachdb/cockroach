...
func reassignParentIDs(
	...
	for _, table := range tables {
		...
		{
			targetDB, override, err := opt.Get(backupOptIntoDB)
			log.Infof(context.TODO(), "%#v", opt)
			if err != nil {
				return err
			}
			if !override {
...
func backupPlanHook(
	...
	fn := func() ([]parser.Datums, error) {
		...
		if err := backup.Options.CheckAllRead(); err != nil {
			return nil, err
		}
...
func restorePlanHook(
	...
	fn := func() ([]parser.Datums, error) {
		...
		if err := restore.Options.CheckAllRead(); err != nil {
			return nil, err
		}
		return nil, nil
	}
