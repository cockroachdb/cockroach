...
func getRelevantDescChanges(
	...
	for _, i := range descs {
		...
		if t := i.GetTable(); t != nil {
			for j := t.ReplacementOf.ID; j != 0; j = priorIDs[j] {
	...
	for _, revs := range allRevs {
		...
		for _, rev := range revs.Values {
			...
			if len(rev.RawBytes) != 0 {
				...
				r.Desc = &desc
				if t := desc.GetTable(); t != nil && t.ReplacementOf.ID != 0 {
			...
			for _, d := range targetDescs {
				if t := d.GetTable(); t != nil {
					...
					if t.ReplacementOf.ID != 0 {
						...
						found := false
						for was := t.ReplacementOf.ID; was != 0 && !found; was = priorIDs[was] {
