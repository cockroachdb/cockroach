...
func ReadBackupDescriptor(
	ctx context.Context, base storageccl.ExportStorage,
) (BackupDescriptor, error) {
	r, err := base.ReadFile(ctx, BackupDescriptorName)
	if err != nil {
...
func ValidatePreviousBackups(ctx context.Context, paths []string) (hlc.Timestamp, error) {
	...
	for _, path := range paths {
		base, err := storageccl.ExportStorageFromURI(ctx, path)
		if err != nil {
			return hlc.Timestamp{}, err
		}
		backupDesc, err := ReadBackupDescriptor(ctx, base)
		if err != nil {
			return hlc.Timestamp{}, err
		}
		...
		if backupDesc.StartTime != endTime {
			return hlc.Timestamp{}, errors.Errorf("missing backup between %s and %s in %s",
				endTime, backupDesc.StartTime, path)
		}
...
func (r backupFileDescriptors) Less(i, j int) bool {
	return r[i].StartKey.Compare(r[j].EndKey) == -1
}
...
func Backup(
	...
	// Backup the descriptors and the entire keyspace for user data.
	...
	desc := BackupDescriptor{StartTime: startTime, EndTime: endTime, SQL: sqlDescs}
	for _, req := range requests {
		...
		if file.Path == "" {
			if log.V(1) {
				log.Infof(ctx, "skipped backup of empty range %s-%s", file.StartKey, file.EndKey)
			}
			continue
		}
...
// export dumps the requested kv entries into files of non-overlapping key ranges
// in a format suitable for bulk import.
func export(
	ctx context.Context,
	db client.DB,
...
) (res BackupDescriptor_File, dataSize int64, retErr error) {
...
func Import(
	...
	for _, file := range files {
		...
		base, err := storageccl.MakeExportStorage(ctx, file.Base)
		if err != nil {
			return err
		}

		localPath, err := base.FetchFile(ctx, file.Path)
		if err != nil {
			return err
		}
...
var _ interval.Interface = importEntry{}

// ID is part of `interval.Interface` but unused in makeImportRequests.
func (ie importEntry) ID() uintptr { return 0 }

// Range is part of `interval.Interface`.
func (ie importEntry) Range() interval.Range {
...
func makeImportRequests(
	...
	for _, table := range tables {
		for _, index := range table.AllNonDropIndexes() {
			...
			ie := importEntry{Span: roachpb.Span{
	...
	for _, b := range backups {
		...
		for _, f := range b.Files {
			backupFileCovering = append(backupFileCovering, intervalccl.Range{
				...
				End:   f.EndKey,
				Payload: importEntry{
					Span:      roachpb.Span{Key: f.StartKey, EndKey: f.EndKey},
...
// files.
func Restore(
	ctx context.Context, db client.DB, backupBases []string, table parser.TableName,
) ([]sqlbase.TableDescriptor, error) {
	backupDescs := make([]BackupDescriptor, len(backupBases))
	for i, basePath := range backupBases {
		base, err := storageccl.ExportStorageFromURI(ctx, basePath)
		if err != nil {
			return nil, errors.Wrapf(err, "failed to create export stoage handler from %q", basePath)
		}
		backupDescs[i], err = ReadBackupDescriptor(ctx, base)
		if err != nil {
			return nil, errors.Wrap(err, "failed to read backup descriptor")
		}
		backupDescs[i].Base = base.Conf()
	}
	...
	databasesByID, tables, err := userTablesAndDBsMatchingName(lastBackupDesc.SQL, table)
	if err != nil {
		return nil, err
	}
	if len(tables) == 0 {
