...
func (jpl *jobProgressLogger) chunkFinished(ctx context.Context) error {
	...
	defer jpl.Unlock()
	jpl.chunkCount++
	fraction := float32(jpl.chunkCount) / float32(jpl.totalChunks)
	if jpl.shouldLogProgress(fraction) {
...
func (jpl *jobProgressLogger) shouldLogProgress(fraction float32) bool {
	return fraction-jpl.lastReportedFraction > progressFractionThreshold ||
