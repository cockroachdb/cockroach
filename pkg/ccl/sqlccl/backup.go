...
func backupJobDescription(backup *parser.Backup) (string, error) {
	...
	var err error
	if b.To, err = storageccl.SanitizeExportStorageURI(backup.To); err != nil {
		return "", err
	}

	for i, incFrom := range backup.IncrementalFrom {
		if b.IncrementalFrom[i], err = storageccl.SanitizeExportStorageURI(incFrom); err != nil {
			return "", err
		}
	}

	var buf bytes.Buffer
	b.Format(&buf, parser.FmtSimple)
