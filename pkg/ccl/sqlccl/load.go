...
// is the storage location. ts is the time at which the MVCC data will be set.
func Load(
	ctx context.Context, db *gosql.DB, r io.Reader, database, uri string, ts hlc.Timestamp,
) (BackupDescriptor, error) {
	importChunkBytes := config.DefaultZoneConfig().RangeMaxBytes / 2
	...
	base, err := storageccl.ExportStorageFromURI(ctx, uri)
	if err != nil {
	...
	for {
		...
		case *parser.CreateTable:
			if tableDesc != nil {
				if err := writeSST(ctx, &backup, base, kvs, ts); err != nil {
		...
		default:
			return BackupDescriptor{}, errors.Errorf("unsupported import statement: %q", stmt)
		}
	}
	...
	descFile, err := base.PutFile(ctx, BackupDescriptorName)
	if err != nil {
...
func insertStmtToKVs(
	...
	if stmt.OnConflict != nil {
		return errors.Errorf("import insert: ON CONFLICT not supported: %q", stmt)
	}
	if len(stmt.Returning) > 0 {
		return errors.Errorf("import insert: RETURNING not supported: %q", stmt)
	}
	if len(stmt.Columns) > 0 {
		if len(stmt.Columns) != len(cols) {
			return errors.Errorf("import insert: wrong number of columns: %q", stmt)
		}
		for i, col := range tableDesc.Columns {
			if stmt.Columns[i].String() != col.Name {
				return errors.Errorf("import insert: unexpected column order: %q", stmt)
			}
		}
	}
	if stmt.Rows.Limit != nil {
		return errors.Errorf("import insert: LIMIT not supported: %q", stmt)
	}
	if stmt.Rows.OrderBy != nil {
		return errors.Errorf("import insert: ORDER BY not supported: %q", stmt)
	}
	values, ok := stmt.Rows.Select.(*parser.ValuesClause)
	if !ok {
		return errors.Errorf("import insert: expected VALUES clause: %q", stmt)
	}
...
func writeSST(
	...
	filename := fmt.Sprintf("import-%d.sst", rand.Int63())
