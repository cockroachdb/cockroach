...
func TestImportChunking(t *testing.T) {
	...
	numAccounts := int(config.DefaultZoneConfig().RangeMaxBytes / backupRestoreRowPayloadSize)

	ctx, dir, _, _, sqlDB, cleanupFn := backupRestoreTestSetup(t, singleNode, 0)
	defer cleanupFn()

	ts := hlc.Timestamp{WallTime: hlc.UnixNano()}
	desc, err := Load(ctx, sqlDB.DB, bankStatementBuf(numAccounts), "bench", dir, ts)
	if err != nil {
		t.Fatalf("%+v", err)
	}
	if len(desc.Files) < 2 {
...
func BenchmarkImport(b *testing.B) {
	...
	b.ResetTimer()
	if _, err := Load(ctx, sqlDB.DB, buf, "bench", dir, ts); err != nil {
		b.Fatalf("%+v", err)
	}
}
...
func BenchmarkRestore(b *testing.B) {
	...
	ts := hlc.Timestamp{WallTime: hlc.UnixNano()}
	backup, err := Load(ctx, sqlDB.DB, bankStatementBuf(b.N), "bench", dir, ts)
	if err != nil {
		b.Fatalf("%+v", err)
	}
