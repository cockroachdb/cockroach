...
func descriptorsMatchingTargets(
	...
	tablesInDB := make(map[sqlbase.ID]map[sqlbase.ID]bool)
	...
	for _, desc := range descriptors {
		if tableDesc := desc.GetTable(); tableDesc != nil {
			...
			if _, ok := tablesInDB[dbDesc.ID]; !ok {
				tablesInDB[dbDesc.ID] = make(map[sqlbase.ID]bool)
			}
			tablesInDB[dbDesc.ID][tableDesc.ID] = false
			normalizedDBName := dbDesc.Name
			if tables, ok := tablesByDatabase[normalizedDBName]; ok {
				for i := range tables {
					if tables[i].name == tableDesc.Name {
						tables[i].validity = valid
						tablesInDB[dbDesc.ID][tableDesc.ID] = true
						ret.descs = append(ret.descs, desc)
						break
					}
				}
