...
func doLocalCSVTransform(
	...
	es, err := exportStorageFromURI(ctx, dest, st, execCfg.Gossip)
	if err != nil {
		return 0, 0, 0, err
	}
	defer es.Close()
	...
	group.Go(func() error {
		...
		var err error
		csvCount, err = readCSV(gCtx, comma, comment, len(tableDesc.VisibleColumns()), dataFiles, recordCh, readProgressFn, st, execCfg.Gossip)
		return err
	})
	group.Go(func() error {
		defer close(kvCh)
