...
func newRowToSSTProcessor(
	flowCtx *distsqlrun.FlowCtx, spec distsqlrun.RowToSSTSpec, input distsqlrun.RowSource, output distsqlrun.RowReceiver,
) (distsqlrun.Processor, error) {
	if flowCtx.TempStorage == nil {
		return nil, errors.New("SST writer requires a temp storage")
	}
	sp := &sstProcessor{
		uri:         spec.SSTDestination,
		name:        spec.Name,
...
var _ distsqlrun.Processor = &sstProcessor{}

func (sp *sstProcessor) OutputTypes() []sqlbase.ColumnType {
	return sstOutputTypes
}

func (sp *sstProcessor) Run(ctx context.Context, wg *sync.WaitGroup) {
	if wg != nil {
		defer wg.Done()
	}
...
func init() {
	...
	distsqlrun.NewRowToSSTProcessor = newRowToSSTProcessor
}
