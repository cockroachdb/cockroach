	...
	err := func() error {
		...
		for i, span := range sp.spec.Spans {
			...
			if sp.spec.Destination == "" {
				if err := sp.db.AdminSplit(ctx, span.End, span.End); err != nil {
					return err
				}

				log.VEventf(ctx, 1, "scattering key %s", roachpb.PrettyPrintKey(nil, span.End))
				scatterReq := &roachpb.AdminScatterRequest{
					Span: roachpb.Span{
						Key:    span.End,
						EndKey: roachpb.Key(span.End).Next(),
					},
				}
				...
				if err := sp.db.AddSSTable(ctx, firstKey, lastKey, data); err != nil {
					return errors.Wrapf(err, "ADDSSTable [%s, %s]", roachpb.PrettyPrintKey(nil, firstKey), roachpb.PrettyPrintKey(nil, lastKey))
				}
			} else {
