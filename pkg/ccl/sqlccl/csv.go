...
// sstWriter is a dist SQL processor that consumes rows, sorts them in an
// on-disk RocksDB instance at tempStorage, then writes them all to a single
// SST file at uri/name. walltime is used as the MVCC timestamp. It returns
// a single row containing the file name, size, checksum, observed start
// and end keys.
