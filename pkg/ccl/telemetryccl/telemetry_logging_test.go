...
func TestTelemetryLogRegions(t *testing.T) {
	...
	// The relocation might fail when the node decides to move the newly created
	// ranges before / during execution of the relocation query, so we are ok
	// with some retries.
	sqlDB.ExecSucceedsSoon(t, "ALTER TABLE one_region EXPERIMENTAL_RELOCATE VALUES (ARRAY[1], 1)")
	sqlDB.ExecSucceedsSoon(t, "ALTER TABLE two_regions EXPERIMENTAL_RELOCATE VALUES (ARRAY[1], 1), (ARRAY[2], 2)")
	sqlDB.ExecSucceedsSoon(t, "ALTER TABLE three_regions EXPERIMENTAL_RELOCATE VALUES (ARRAY[1], 1), (ARRAY[2], 2), (ARRAY[3], 3)")

	// Populate the range cache on the gateway node so that it plans the scans
	// according to the leaseholder placement.
	sqlDB.Exec(t, "SELECT count(*) FROM one_region")
	sqlDB.Exec(t, "SELECT count(*) FROM two_regions")
	sqlDB.Exec(t, "SELECT count(*) FROM three_regions")
