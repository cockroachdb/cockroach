...
func evalImport(ctx context.Context, cArgs storage.CommandArgs) (*roachpb.ImportResponse, error) {
	...
	var importStart, importEnd roachpb.Key
	{
		...
		if !ok {
			return &roachpb.ImportResponse{}, errors.Errorf("could not rewrite key: %s", importStart)
		}
		importEnd, ok = kr.RewriteKey(append([]byte(nil), args.DataSpan.EndKey...))
		if !ok {
			return &roachpb.ImportResponse{}, errors.Errorf("could not rewrite key: %s", importEnd)
		}
	}
	...
	for _, file := range args.Files {
		...
		if err != nil {
			return &roachpb.ImportResponse{}, err
		}
		defer func() {
		...
		if len(file.Sha512) > 0 {
			...
			if !bytes.Equal(checksum, file.Sha512) {
				return &roachpb.ImportResponse{}, errors.Errorf("checksum mismatch for %s", file.Path)
			}
		}
