...
func (e *encryptionStatsHandler) GetActiveDataKeyID() ([]byte, error) {
	k, err := e.dataKM.ActiveKey(context.TODO())
	if err != nil {
		return nil, err
	}
	if k != nil {
		return []byte(k.Info.KeyId), nil
	}
...
func (e *encryptionStatsHandler) GetKeyIDFromSettings(settings []byte) ([]byte, error) {
	var s enginepbccl.EncryptionSettings
	if err := protoutil.Unmarshal(settings, &s); err != nil {
		return nil, err
	}
	return []byte(s.KeyId), nil
}
