...
func TestEncryptedFS(t *testing.T) {
	...
	fileRegistry := &engine.FileRegistry{FS: memfs, DBDir: "/bar"}
	require.NoError(t, fileRegistry.Load())

	// Write a key for the StoreKeyManager.
	var b []byte
	for i := 0; i < kKeyIdLength+16; i++ {
		b = append(b, 'a')
	}
	...
	require.NoError(t, keyManager.Load())
	...
	// This test code is copied from Pebble's mem_fs_test.go.
