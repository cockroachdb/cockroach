...
// in the test below have names prefixed by TEST and spared this error
...
func (op *renameOp) run(t *fsTest) {
	...
	// registry. And the underlying FS.Rename will return an error since from
	// does not exist, so the semantics of encrypted FS and plain FS are not
