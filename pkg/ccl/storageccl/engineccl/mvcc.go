...
var timeBoundIterators = settings.RegisterBoolSetting(
	"engineccl.time.bound.iterators.enabled",
	"speed up incremental backups by efficiently skipping over old SSTs",
	true,
)

func newEngineIter(e engine.Reader, startTime, endTime hlc.Timestamp) engine.Iterator {
	if timeBoundIterators.Get() {
