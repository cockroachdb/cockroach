...
DBStatus DBBatchReprVerify(
  ...
  const rocksdb::Comparator* kComparator = internal::CockroachComparator();
  ...
  rocksdb::WriteBatch b(internal::ToString(repr));
  std::unique_ptr<rocksdb::WriteBatch::Handler> inserter(internal::GetDBBatchInserter(&batch));
  b.Iterate(inserter.get());
  std::unique_ptr<rocksdb::Iterator> iter;
  iter.reset(batch.NewIteratorWithBase(rocksdb::NewEmptyIterator()));

  iter->SeekToFirst();
  if (iter->Valid() && kComparator->Compare(iter->key(), internal::EncodeKey(start)) < 0) {
    return internal::FmtStatus("key not in request range");
  }
  iter->SeekToLast();
  if (iter->Valid() && kComparator->Compare(iter->key(), internal::EncodeKey(end)) >= 0) {
    return internal::FmtStatus("key not in request range");
  }

  *stats = internal::MVCCComputeStatsInternal(iter.get(), start, end, now_nanos);
