...
func (f *MultiIterator) advance() {
	...
	// in proposedNextIdx as we go. If it's still -1 at the end, then all the
	// underlying iterators are exhausted and so is this one.
	proposedNextIdx := -1
	for iterIdx := range f.iters {
		// If this iterator is exhausted skip it (or error if it's errored).
		if !f.iters[iterIdx].Valid() {
			if err := f.iters[iterIdx].Error(); err != nil {
				f.valid = false
				f.err = err
				return
			}
		...
		if proposedNextIdx > -1 {
			proposedMVCCKey = f.iters[proposedNextIdx].UnsafeKey()
		}

		iterMVCCKey := f.iters[iterIdx].UnsafeKey()
	...
	// If proposedNextIdx is still -1, all the underlying iterators are invalid.
	if proposedNextIdx == -1 {
		f.valid = false
		return
	}

	// Otherwise proposedNextIdx is our next value.
