...
func iterateExpectErr(
	...
	return func(t *testing.T) {
		...
		_, _, _, exportErr := ExportSST(context.TODO(), e, startKey, endKey, startTime, endTime, allRevisions)
...
func assertEqualKVs(
	...
	return func(t *testing.T) {
		...
		t.Run("mvccExport", func(t *testing.T) {
			sst, entries, _, err := ExportSST(context.TODO(), e, startKey, endKey, startTime, endTime, allRevisions)
			if err != nil {
				t.Fatal(err)
			}
			...
			if len(kvs) != int(entries) {
				t.Fatalf("claimed different number of entries (%d) than in kvs (%d): %+v", entries, len(kvs), kvs)
			}
			if len(kvs) != len(expected) {
