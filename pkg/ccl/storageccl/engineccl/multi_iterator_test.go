...
func TestMultiIterator(t *testing.T) {
	...
	for _, test := range tests {
		...
		t.Run(name, func(t *testing.T) {
			...
			check := func(expected string, fn func(*MultiIterator)) func(*testing.T) {
				return func(t *testing.T) {
					var output bytes.Buffer
					it := MakeMultiIterator(iters)
					for it.Seek(engine.MVCCKey{Key: keys.MinKey}); it.Valid(); fn(it) {
			...
			t.Run("NextKey", check(test.expectedNextKey, func(it *MultiIterator) {
				it.NextKey()
			}))
			t.Run("Next", check(test.expectedNext, func(it *MultiIterator) {
				it.Next()
			}))
		})
	}
