...
// Encryption is done using AES-GCM with a key derived from the provided
...
// DecryptSaltedFile decrypts files created by EncryptSaltedFile which include a
// salt in their header prefix, and thus can be passed to DeriveKeyFromFile to
// dervice the key to decrypt them with this function.
func DecryptSaltedFile(ciphertext, key, expectedSalt []byte) ([]byte, error) {
	ciphertext, salt, err := readSaltFromFile(ciphertext)
	if err != nil {
		return nil, err
	}

	// We already have a key so we just check that it was indeed derived from the
...
// readSaltFromFile checks the header on a salt-prefixed file and extracts the
// salt prefix, returning the remaining body (iv+cipheretext) along with the
// extracted salt.
func readSaltFromFile(ciphertext []byte) (ivAndCiphertext, salt []byte, _ error) {
	if !AppearsEncrypted(ciphertext) {
		return nil, nil, errors.New("file does not appear to be encrypted")
	}
	ciphertext = ciphertext[len(encryptionPreamble):]

	if len(ciphertext) < 1 {
		return nil, nil, errors.New("invalid encryption header")
	}
	version := ciphertext[0]
	ciphertext = ciphertext[1:]

	if version != encryptionVersionAESGCMWithSalt {
		return nil, nil, errors.Errorf("unexpected encryption scheme/config version %d", version)
	}
...
// DeriveKeyFromSaltedFile reads the salt from a salt-prefixed file to derive
// the key that corresponds to the supplied passphrase.
func DeriveKeyFromSaltedFile(ciphertext, passphrase []byte) (key, salt []byte, _ error) {
	_, salt, err := readSaltFromFile(ciphertext)
	if err != nil {
		return nil, nil, err
	}
	return kdf(passphrase, salt), salt, nil
