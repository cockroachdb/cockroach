...
func EncryptFile(plaintext, key []byte) ([]byte, error) {
	...
	// Pick a unique IV for this file.
	...
	headerSize := len(encryptionPreamble) + 1 + len(iv)
	ciphertext := make([]byte, headerSize, headerSize+len(plaintext))
	copy(ciphertext, encryptionPreamble)

	// Write our header (version + chosen IV) to the ciphertext buffer.
	ciphertext[len(encryptionPreamble)] = encryptionVersionAESGCM
	// Write the picked salt as a prefix of the ciphertext, for use by decrypt.
	_ = copy(ciphertext[len(encryptionPreamble)+1:], iv)
