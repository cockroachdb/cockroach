...
func ExternalSSTReader(
	...
	if len(e) != len(filenames) {
		return nil, errors.New("must supply an external store for each file")
	}
	if !remoteSSTs.Get(&e[0].Settings().SV) {
		return newMemPebbleSSTReader(ctx, e, filenames, encryption, iterOps)
	}
	remoteCacheSize := remoteSSTSuffixCacheSize.Get(&e[0].Settings().SV)
	readers := make([]sstable.ReadableFile, 0, len(filenames))
