...
const (
	...
	cloudstorageGSJsonkey        = ".jsonkey"
	cloudstorageGSDefaultJsonkey = cloudstorageGSDefault + cloudstorageGSJsonkey
)
...
func makeGCSStorage(ctx context.Context, conf *roachpb.ExportStorage_GCS, settings *cluster.Settings) (ExportStorage, error) {
	...
	case "", authParamDefault:
		...
		if conf.Auth == authParamDefault && key == "" {
			return nil, errors.Errorf("expected settings value for %s", cloudstorageGSDefaultJsonkey)
		}
		if key != "" {
