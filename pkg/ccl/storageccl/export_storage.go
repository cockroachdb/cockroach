...
func ExportStorageConfFromURI(path string) (roachpb.ExportStorage, error) {
	...
	case "network-file", "file", "nodelocal":
		if uri.Host != "" {
		...
		conf.LocalFile.IsSharedFS = uri.Scheme == "network-file"
	case "":
		conf.Provider = roachpb.ExportStorageProvider_LocalFile
...
func MakeExportStorage(
	...
	case roachpb.ExportStorageProvider_LocalFile:
		return makeLocalStorage(dest.LocalFile.Path, dest.LocalFile.IsSharedFS)
...
func makeLocalStorage(base string, sharedFS bool) (ExportStorage, error) {
	if base == "" {
	...
	return &localFileStorage{base: base, isSharedFS: sharedFS}, nil
}
