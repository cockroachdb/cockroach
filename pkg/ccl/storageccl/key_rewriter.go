...
func (kr *KeyRewriter) rewriteKey(skipCols int, key []byte) ([]byte, bool, error) {
	...
	// TODO(mjibson): We should be able to use
	// idx.Interleave.Ancestors[].SharedPrefixLen, but for an unknown reason it
	// seems to be not populated, so we have to do this instead.
