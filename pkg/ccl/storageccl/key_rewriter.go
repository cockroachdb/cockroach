...
func makeKeyRewriter(
	codec keys.SQLCodec, descs map[descpb.ID]*tabledesc.Immutable,
) (*KeyRewriter, error) {
...
func (kr *KeyRewriter) RewriteKey(key []byte, isFromSpan bool) ([]byte, bool, error) {
	// First strip the old tenant prefix.
	...
	// Prepend the new tenant prefix.
	tenantPrefix := kr.codec.TenantPrefix()
	rekeyed = append(tenantPrefix, rekeyed...)
