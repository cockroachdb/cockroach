...
func init() {
	storage.SetWriteBatchCmd(storage.Command{Eval: WriteBatch})
}
...
func WriteBatch(
	...
	ctx, span := tracing.ChildSpan(ctx, fmt.Sprintf("WriteBatch %s-%s", args.Key, args.EndKey))
	...
	// request header.
	msBatch, err := engine.VerifyBatchRepr(args.Data, mvccStartKey, mvccEndKey, h.Timestamp.WallTime)
	if err != nil {
