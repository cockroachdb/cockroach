...
func TestNewExternalSSTReader(t *testing.T) {
	...
	for i, subdir := range subdirs {
		...
		store, err := cloud.ExternalStorageFromURI(ctx, localFoo+subdir+"/", base.ExternalIODirConfig{}, clusterSettings, blobs.TestBlobServiceClient(tempDir), username.RootUserName(), tc.Servers[0].InternalExecutor().(*sql.InternalExecutor), tc.Servers[0].CollectionFactory().(*descs.CollectionFactory), tc.Servers[0].DB(), nil)
