...
func TestNewExternalSSTReader(t *testing.T) {
	...
	for i, subdir := range subdirs {
		...
		// Create an SST and write it to external storage. Ensure that the SSTs do
		// not span the same keyspace.
		numKeys := 10000
		kvs := make(storageutils.KVs, numKeys)
		for j := 0; j < numKeys; j++ {
			// Ensure each key is unique
			kvs[j] = storageutils.PointKV(subdir+"a"+strconv.Itoa(i), numKeys-j, "1")
		}
