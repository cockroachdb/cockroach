...
2,0,initial-6d54
	...
	_, err = ExportStorageFromURI(ctx, `experimental-workload:///fmt/bank/bank`, settings)
	require.EqualError(t, err, `unsupported format: fmt`)
	_, err = ExportStorageFromURI(ctx, `experimental-workload:///csv/nope/nope`, settings)
	...
	require.EqualError(t, err, `experimental-workload requires all nodes to be upgraded to 2.1-3`)
}
