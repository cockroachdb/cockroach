...
func TestPutHttp(t *testing.T) {
	...
	t.Run("multiHost", func(t *testing.T) {
		s1, f1, cleanup1 := makeServer()
		defer cleanup1()
		s2, f2, cleanup2 := makeServer()
		defer cleanup2()
		s3, f3, cleanup3 := makeServer()
		defer cleanup3()

		combined := *s1
		combined.Host = strings.Join([]string{s1.Host, s2.Host, s3.Host}, ",")

		testExportToTarget(t, roachpb.ExportStorage{
			Provider: roachpb.ExportStorageProvider_Http,
			HttpPath: roachpb.ExportStorage_Http{BaseUri: combined.String() + "/"},
		})
		// testExportToTarget writes 10 simple files and one big one, so expect 11.
		w1, w2, w3 := f1(), f2(), f3()
		if expected, actual := 11, w1+w2+w3; expected != actual {
			t.Fatalf("expected %d files written across all hosts, got %d", expected, actual)
		}
		if expected, actual := 3, w1; expected != actual {
			t.Fatalf("expected %d files written to http host 1, got %d", expected, actual)
		}
		if expected, actual := 4, w3; expected != actual {
