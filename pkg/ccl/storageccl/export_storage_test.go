...
func TestPutS3(t *testing.T) {
	...
	testExportStore(t, fmt.Sprintf(
		"s3://%s/%s?S3AccessKeyParam=%s&S3SecretParam=%s",
		bucket, "backup-test", s3Keys.AccessKey, s3Keys.SecretKey,
	), false)
}
...
func TestPutAzure(t *testing.T) {
	...
	testExportStore(t, fmt.Sprintf("azure://%s/%s?%s=%s&%s=%s",
		bucket, "backup-test", AzureAccountNameParam, accountName, AzureAccountKeyParam, accountKey),
		false,
	)
}
