...
func assertEqualKVs(
	...
	return func(t *testing.T) {
		...
		sst, _, err := engine.ExportToSst(context.TODO(), e, start, end, exportAllRevisions, io)
		if err != nil {
			t.Fatal(err)
		}

		// Compare new C++ implementation against the oracle.
		expectedKVS := loadSST(t, expected)
		kvs := loadSST(t, sst)
...
func TestRandomKeyAndTimestampExport(t *testing.T) {
	...
	t.Run("ts (0-∞], latest, nontimebound", assertEqualKVs(rocksdb, keyMin, keyMax, tsMin, tsMax, false, false))
	t.Run("ts (0-∞], all, nontimebound", assertEqualKVs(rocksdb, keyMin, keyMax, tsMin, tsMax, true, false))
	t.Run("ts (0-∞], latest, timebound", assertEqualKVs(rocksdb, keyMin, keyMax, tsMin, tsMax, false, true))
	t.Run("ts (0-∞], all, timebound", assertEqualKVs(rocksdb, keyMin, keyMax, tsMin, tsMax, true, true))

	upperBound := randutil.RandIntInRange(rnd, 1, numKeys)
	lowerBound := rnd.Intn(upperBound)

	// Exercise random key ranges.
	t.Run("kv [randLower, randUpper), latest, nontimebound", assertEqualKVs(rocksdb, keys[lowerBound], keys[upperBound], tsMin, tsMax, false, false))
	t.Run("kv [randLower, randUpper), all, nontimebound", assertEqualKVs(rocksdb, keys[lowerBound], keys[upperBound], tsMin, tsMax, true, false))
	t.Run("kv [randLower, randUpper), latest, timebound", assertEqualKVs(rocksdb, keys[lowerBound], keys[upperBound], tsMin, tsMax, false, true))
	t.Run("kv [randLower, randUpper), all, timebound", assertEqualKVs(rocksdb, keys[lowerBound], keys[upperBound], tsMin, tsMax, true, true))

	upperBound = randutil.RandIntInRange(rnd, 1, numKeys)
	lowerBound = rnd.Intn(upperBound)

	// Exercise random timestamps.
	t.Run("kv [randLowerTime, randUpperTime), latest, nontimebound", assertEqualKVs(rocksdb, keyMin, keyMax, timestamps[lowerBound], timestamps[upperBound], false, false))
	t.Run("kv [randLowerTime, randUpperTime), all, nontimebound", assertEqualKVs(rocksdb, keyMin, keyMax, timestamps[lowerBound], timestamps[upperBound], true, false))
	t.Run("kv [randLowerTime, randUpperTime), latest, timebound", assertEqualKVs(rocksdb, keyMin, keyMax, timestamps[lowerBound], timestamps[upperBound], false, true))
	t.Run("kv [randLowerTime, randUpperTime), all, timebound", assertEqualKVs(rocksdb, keyMin, keyMax, timestamps[lowerBound], timestamps[upperBound], true, true))
}
