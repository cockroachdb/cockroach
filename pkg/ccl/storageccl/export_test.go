...
func TestExportCancelledCtx(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	// Check that no work is even started if the context is cancelled.
	_, err := evalExportKeys(ctx, nil, storage.CommandArgs{}, nil)
