...
(SELECT id, value FROM t);`)
		...
		_, pErr := export(t, res5.end, roachpb.MVCCFilter_Latest, 0)
		require.Regexp(t, expectedError, pErr)
		_, pErr = export(t, res5.end, roachpb.MVCCFilter_All, 0)
		require.Regexp(t, expectedError, pErr)
	...
	t.Run("ts7", func(t *testing.T) {
		...
		// Target Size to one KV and TargetBytes to greated than all KVs. Checks if
