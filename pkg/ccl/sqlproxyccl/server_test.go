...
func TestShouldLogError(t *testing.T) {
	...
	t.Run("no error", func(t *testing.T) {
		require.True(t, s.shouldLogError(nil, nil, nil))
	})

	t.Run("normal error", func(t *testing.T) {
		require.True(t, s.shouldLogError(errors.New("foo"), nil, nil))
	})
	...
	t.Run("no tenant ID", func(t *testing.T) {
		require.True(t, s.shouldLogError(highFreqErr, nil, reqTags))
	})
	...
	t.Run("invalid connection", func(t *testing.T) {
		require.True(t, s.shouldLogError(highFreqErr, &fakeTCPConn{}, reqTags))
	})

	conn := &fakeTCPConn{remoteAddr: &net.TCPAddr{IP: net.IP{1, 2, 3, 4}}}

	t.Run("successful - tenant 42", func(t *testing.T) {
		require.True(t, s.shouldLogError(highFreqErr, conn, reqTags))
		require.False(t, s.shouldLogError(highFreqErr, conn, reqTags))
		require.False(t, s.shouldLogError(highFreqErr, conn, reqTags))
	})
