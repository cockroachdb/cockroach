...
func countGuesses(
	...
	for i := 0; step*time.Duration(i) < period; i++ {
		...
		err = throttle.ReportAttempt(connection, throttleTime, AttemptInvalidCredentials)
...
func TestReportSuccessDisablesLimiter(t *testing.T) {
	...
	require.NoError(t, throttle.ReportAttempt(tenant1, throttleTime, AttemptOK))
...
func TestRacingRequests(t *testing.T) {
	...
	require.NoError(t, throttle.ReportAttempt(connection, throttleTime, AttemptInvalidCredentials))

	l := throttle.lockedGetThrottle(connection)
	nextTime := l.nextTime

	for _, status := range []AttemptStatus{AttemptOK, AttemptInvalidCredentials} {
		require.Error(t, throttle.ReportAttempt(connection, throttleTime, status))
