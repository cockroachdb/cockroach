...
func DestroyTenant(tc serverutils.TestClusterInterface, id ID) error {
	srv := tc.Server(0)
	conn := srv.InternalExecutor().(*sql.InternalExecutor)
	if _, err := conn.Exec(
		context.Background(),
...
func StartTenant(
	...
	tenant, err := srv.StartTenantWithCtx(
		ctx,
		base.TestTenantArgs{
			Existing:      true,
...
func TestDirectoryErrors(t *testing.T) {
	...
	_, err := dir.LookupTenantIPs(ctx, ID(1000))
	assert.Regexp(t, "not found", err)
	_, err = dir.LookupTenantIPs(ctx, ID(1001))
	assert.Regexp(t, "not found", err)
	_, err = dir.LookupTenantIPs(ctx, ID(1002))
	assert.Regexp(t, "not found", err)

	// Fail to find tenant that does not exist.
	_, err = dir.EnsureTenantIP(ctx, ID(1000), "")
	assert.Regexp(t, "not found", err)

	// Fail to find tenant when cluster name doesn't match.
	_, err = dir.EnsureTenantIP(ctx, ID(tenantID), "unknown")
	assert.Regexp(t, "not found", err)

	// No-op when reporting failure for tenant that doesn't exit.
	require.NoError(t, dir.ReportFailure(ctx, ID(1000), ""))
}
...
func TestEndpointWatcher(t *testing.T) {
	...
	// Resume tenant by a direct call to the directory server
	_, err := tds.EnsureEndpoint(ctx, &EnsureEndpointRequest{uint64(tenantID)})
	require.NoError(t, err)
