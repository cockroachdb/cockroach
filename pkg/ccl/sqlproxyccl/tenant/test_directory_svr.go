...
func (s *TestDirectoryServer) Get(id ID) (result map[net.Addr]*Process) {
	result = make(map[net.Addr]*Process)
	s.proc.RLock()
	defer s.proc.RUnlock()
	processes, ok := s.proc.processByAddrByTenantID[uint64(id)]
	if ok {
...
func (s *TestDirectoryServer) WatchEndpoints(
	...
	c := make(chan *WatchEndpointsResponse)
	s.listen.Lock()
...
func (s *TestDirectoryServer) notifyEventListenersLocked(req *WatchEndpointsResponse) {
	for e := s.listen.eventListeners.Front(); e != nil; e = e.Next() {
		e.Value.(chan *WatchEndpointsResponse) <- req
	}
}
