...
func (e *tenantEntry) Initialize(ctx context.Context, ctl DirectoryClient) error {
	...
	tenantResp, err := ctl.GetTenant(ctx, &GetTenantRequest{TenantId: uint64(e.TenantID)})
	if err != nil {
		e.initialized = true
		e.initError = err
		return err
...
func (e *tenantEntry) RefreshEndpoints(ctx context.Context, ctl DirectoryClient) error {
	if !e.initialized {
	...
	_, err := e.fetchEndpointsLocked(ctx, ctl)
	return err
}
...
func (e *tenantEntry) ChooseEndpointIP(
	...
	if len(ips) == 0 {
		...
		var err error
		if ips, err = e.ensureTenantEndpoint(ctx, ctl, errorIfNoEndpoints); err != nil {
			return "", err
		}
	}
	return ips[0], nil
...
func (e *tenantEntry) ensureTenantEndpoint(
	...
	resp, err := ctl.ListEndpoints(ctx, &ListEndpointsRequest{TenantId: uint64(e.TenantID)})
	if err != nil {
		return nil, err
	}
	...
	for {
		...
		if len(resp.Endpoints) == 0 {
			...
			if _, err := ctl.EnsureEndpoint(
				ctx, &EnsureEndpointRequest{uint64(e.TenantID)},
			); err != nil {
				return nil, err
			}
		}
		...
		ips, err = e.fetchEndpointsLocked(ctx, ctl)
		if err != nil {
			return nil, err
		}
		if len(ips) != 0 {
...
func (e *tenantEntry) fetchEndpointsLocked(
	...
	list, err := ctl.ListEndpoints(ctx, &ListEndpointsRequest{uint64(e.TenantID)})
	if err != nil {
		return nil, err
	}
