...
type IdleDisconnectConnection struct {
	...
	lastDeadlineSet time.Time
}

var notSupportedError = errors.Errorf(
	"Not supported for IdleDisconnectConnection",
)

func (c *IdleDisconnectConnection) updateDeadline() error {
	now := timeutil.Now()
	// If it has been more than 5% of the timeout duration - advance the deadline.
	if now.Sub(c.lastDeadlineSet) > c.timeout/20 {
		c.lastDeadlineSet = now.Add(c.timeout)
		if err := c.Conn.SetReadDeadline(c.lastDeadlineSet); err != nil {
			return err
		}
	}
	return nil
...
// idle for more than timemout duration.
