...
func (c IdleDisconnectConnection) Write(b []byte) (n int, err error) {
	// The Write for the connection is not blocking (or can block only temporary in case of flow control).
	// For idle connections, the Read will be the call that will block and stay blocked until the backend
	// doesn't send something. However, it is theoretically possible, that the traffic is only
	// going in one direction - from the proxy to the backend, in which case we will call repeatedly
	// Write but stay blocked on the Read. For that specific case - the write pushes further out
	// the read deadline so the read doesn't timeout.
	if err := c.Conn.SetReadDeadline(timeutil.Now().Add(c.timeout)); err != nil {
		return 0, err
	}
