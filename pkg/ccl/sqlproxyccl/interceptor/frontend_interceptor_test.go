...
func TestFrontendInterceptor(t *testing.T) {
	...
	t.Run("small bufSize", func(t *testing.T) {
		fi := interceptor.NewFrontendInterceptor(nil /* src */, 1 /* bufSize */)
		require.NotNil(t, fi)
	})
	...
	t.Run("PeekMsg returns the right message type", func(t *testing.T) {
		...
		fi := interceptor.NewFrontendInterceptor(src, 16 /* bufSize */)
		require.NotNil(t, fi)
	...
	t.Run("WriteMsg writes data to dst", func(t *testing.T) {
		...
		fi := interceptor.NewFrontendInterceptor(nil /* src */, 10 /* bufSize */)
		require.NotNil(t, fi)
