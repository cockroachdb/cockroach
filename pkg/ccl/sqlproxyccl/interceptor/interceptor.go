...
func (p *pgInterceptor) PeekMsg() (typ byte, size int, err error) {
	...
	typ = p.buf[p.readPos]
	size = int(binary.BigEndian.Uint32(p.buf[p.readPos+1:])) - 4
...
// also advances the interceptor to the next message. On return, the type and
// body fields are valid if and only if err == nil. If err != nil and a Read
// was attempted because the buffer did not have enough bytes, the interceptor
// will be closed.
//
...
func (p *pgInterceptor) ReadMsg() (typ byte, body []byte, err error) {
	...
	if p.closed {
		return 0, nil, ErrInterceptorClosed
	}

	// Read header of the current message for body size.
	typ, size, err := p.PeekMsg()
	if err != nil {
		return 0, nil, err
	}
	p.readPos += pgHeaderSizeBytes
	...
	// Read more bytes.
	if _, err := io.ReadFull(p.src, body[n:]); err != nil {
		return 0, nil, err
	}
...
func newChunkReader(typ byte, body []byte) pgproto3.ChunkReader {
	cr := &chunkReader{body: body}
	cr.header[0] = typ
	binary.BigEndian.PutUint32(cr.header[1:], uint32(len(body)+4))
	return cr
}

// Next implements the pgproto3.ChunkReader interface. This implements a tiny
// state machine where Next has to be called in the following order: n=5 to
// read headers, followed by n=len(body) to read the message body. An io.EOF
// will be returned once the entire message has been read. If the state machine
// protocol isn't obeyed, an errInvalidRead will be returned.
func (cr *chunkReader) Next(n int) (buf []byte, err error) {
	switch cr.pos {
	case 0:
		// Only the headers can be requested.
		if n != 5 {
	...
	case 5:
		// After header is read, only the body can be requested.
		if n != len(cr.body) {
			return nil, errInvalidRead
		}
		cr.pos += n
...
func (fi *FrontendInterceptor) ReadMsg() (msg pgproto3.BackendMessage, err error) {
	typ, body, err := fi.p.ReadMsg()
	if err != nil {
		return nil, err
	}
	// errPanicWriter is used here because Receive must not Write.
	return pgproto3.NewFrontend(newChunkReader(typ, body), &errPanicWriter{}).Receive()
}
...
func (bi *BackendInterceptor) ReadMsg() (msg pgproto3.FrontendMessage, err error) {
	typ, body, err := bi.p.ReadMsg()
	if err != nil {
		return nil, err
	}
	// errPanicWriter is used here because Receive must not Write.
	return pgproto3.NewBackend(newChunkReader(typ, body), &errPanicWriter{}).Receive()
}
