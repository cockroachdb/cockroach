...
// defaultClusterName corresponds to the cluster name which will be returned
// through GetTenant.
const defaultClusterName = "tenant-cluster"

// TestSimpleDirectoryServer is a directory server that returns a single
// pre-defined address and cluster name for all tenants. It is expected that
// a SQL pod is listening on that address.
type TestSimpleDirectoryServer struct {
	...
	podAddr string
	// clusterName corresponds to the cluster's name which will be returned
	// through GetTenant. For now, this will always be defaultClusterName.
	clusterName string
...
// tenant ID. If the address of the pod cannot be resolved, a GRPC NotFound
...
// GetTenant returns the cluster name associated with the simple directory
// regardless of tenants. If the tenant has been deleted, a GRPC NotFound error
// will be returned.
//
...
func (d *TestSimpleDirectoryServer) GetTenant(
	...
	return &tenant.GetTenantResponse{ClusterName: d.clusterName}, nil
}
