...
type TestDirectoryServer struct {
	...
	mu struct {
		...
		processByAddrByTenantID map[uint64][]string
...
func New(stopper *stop.Stopper, createTenant TenantStarterFunc) (*TestDirectoryServer, error) {
	...
	dir.mu.processByAddrByTenantID = map[uint64][]string{}
...
func (s *TestDirectoryServer) StartTenant(ctx context.Context, id roachpb.TenantID) error {
	...
	func() {
		s.mu.Lock()
		defer s.mu.Unlock()
		s.registerInstanceLocked(id.ToUint64(), sqlAddr)
	}()
...
func (s *TestDirectoryServer) Drain() {
	...
	for tenantID, processByAddr := range s.mu.processByAddrByTenantID {
		for _, addr := range processByAddr {
...
func (s *TestDirectoryServer) EnsurePod(
	...
	tenantStopper, sqlAddr, err := func() (*stop.Stopper, string, error) {
		...
		s.registerInstanceLocked(req.TenantID, sqlAddr)
...
func (s *TestDirectoryServer) registerInstanceLocked(tenantID uint64, sqlAddr string) {
	s.mu.processByAddrByTenantID[tenantID] = append(s.mu.processByAddrByTenantID[tenantID], sqlAddr)
	s.notifyEventListenersLocked(&tenant.WatchPodsResponse{
		Pod: &tenant.Pod{
			TenantID:       tenantID,
			Addr:           sqlAddr,
...
func (s *TestDirectoryServer) deregisterInstance(tenantID uint64, sqlAddr string) {
	...
	for _, foundAddr := range processByAddr {
		if sqlAddr == foundAddr {
			s.notifyEventListenersLocked(&tenant.WatchPodsResponse{
				Pod: &tenant.Pod{
					TenantID:       tenantID,
					Addr:           sqlAddr,
			...
			remaining = append(remaining, foundAddr)
		}
	}
