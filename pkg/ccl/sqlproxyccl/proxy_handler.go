...
func (handler *proxyHandler) handle(ctx context.Context, proxyConn *conn) error {
	...
	for r := retry.StartWithCtx(ctx, retryOpts); r.Next(); {
		...
		if err != nil &&
			...
			if shouldLog {
				log.Ops.Errorf(ctx,
					"backend dial: %v (%d errors in the past minute)",
					err,
					codeBackendDownErrs,
				)
				codeBackendDownErrs = 0
			}
			err = handler.directory.ReportFailure(ctx, roachpb.MakeTenantID(tenID), outgoingAddress)
			if err != nil {
				reportFailureErrs++
				if shouldLog {
					log.Ops.Errorf(ctx,
						"report failure: %v (%d errors in the past minute)",
						err,
						reportFailureErrs,
					)
					reportFailureErrs = 0
	...
	handler.connCache.Insert(
		&cache.ConnKey{IPAddress: ipAddr, TenantID: roachpb.MakeTenantID(tenID)},
	)
...
func (handler *proxyHandler) outgoingAddress(
	ctx context.Context, name string, tenID uint64,
) (string, error) {
	...
	addr, err := handler.directory.EnsureTenantIP(ctx, roachpb.MakeTenantID(tenID), "")
	if err != nil {
		return "", err
	}
	return addr, nil
...
func (handler *proxyHandler) validateAccessAndThrottle(
	...
	connKey := cache.ConnKey{IPAddress: ipAddr, TenantID: roachpb.MakeTenantID(tenID)}
...
func (handler *proxyHandler) validateAccess(
	...
	if entry, err := list.Denied(denylist.DenyEntity{Item: fmt.Sprint(tenID), Type: denylist.ClusterType}); err != nil {
		// Log error but don't return since this could be transient.
		log.Errorf(ctx, "could not consult denied list for tenant: %d", tenID)
	} else if entry != nil {
		log.Errorf(ctx, "access denied for tenant: %d, reason: %s", tenID, entry.Reason)
		return newErrorf(codeProxyRefusedConnection, "tenant %d %s", tenID, entry.Reason)
	}
