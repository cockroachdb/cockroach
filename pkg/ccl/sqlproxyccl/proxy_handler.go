...
type ProxyHandler struct {
	...
	// IncomingTLSCert is the managed cert of the proxy endpoint to
...
func NewProxyHandler(
	ctx context.Context, stopper *stop.Stopper, options ProxyOptions,
) (*ProxyHandler, error) {
...
func (handler *ProxyHandler) Handle(ctx context.Context, metrics *Metrics, proxyConn *Conn) error {
	...
	for i := 1; i < 10; i++ {
		...
		if err != nil &&
			...
			errReport := handler.Directory.ReportFailure(ctx, roachpb.MakeTenantID(tenID), outgoingAddress)
			if errReport != nil {
				return errReport
			}
			continue
