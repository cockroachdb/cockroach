...
type ProxyOptions struct {
	...
	// Token bucket policy used to throttle IP addresses that have a history
	// of successful authentication.
	AuthenticatedBucketPolicy throttler.BucketPolicy

	// Token bucket policy used to throttle IP addresses that have never
	// authenticated successfully.
	UnauthenticatedBucketPolicy throttler.BucketPolicy
}
...
func newProxyHandler(
	...
	handler.throttleService = throttler.NewLocalService(
		throttler.WithAuthenticatedPolicy(handler.AuthenticatedBucketPolicy),
		throttler.WithUnauthenticatedPolicy(handler.UnauthenticatedBucketPolicy),
	)
...
func (handler *proxyHandler) handle(ctx context.Context, incomingConn *proxyConn) error {
	...
	if err := handler.throttleService.LoginCheck(ipAddr); err != nil {
	...
	handler.throttleService.ReportSuccess(ipAddr)
