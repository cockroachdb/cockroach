...
func (handler *ProxyHandler) Handle(ctx context.Context, proxyConn *Conn) error {
	...
	for r := retry.StartWithCtx(ctx, retryOpts); r.Next(); {
		...
		if err != nil &&
			...
			_ = handler.directory.ReportFailure(ctx, roachpb.MakeTenantID(tenID), outgoingAddress)
			continue
		}
		break
	}
	...
	// go routine that closes connections that are blacklisted.
	go func() {
...
func (handler *ProxyHandler) validateAccess(
	...
	if entry, err := list.Denied(denylist.DenyEntity{fmt.Sprint(tenID), denylist.ClusterType}); err != nil {
	...
	if entry, err := list.Denied(denylist.DenyEntity{ipAddr, denylist.IPAddrType}); err != nil {
		// Log error but don't return since this could be transient.
