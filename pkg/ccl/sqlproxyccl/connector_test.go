...
func TestConnector_openClusterConnInternal(t *testing.T) {
	...
	t.Run("successful", func(t *testing.T) {
		...
		mock := NewMockTenantResolver(ctrl)
		c := &connector{Directory: mock, TenantID: roachpb.MakeTenantID(42)}

		mock.EXPECT().
			ReportFailure(gomock.Eq(ctx), gomock.Eq(c.TenantID), "127.0.0.10:42").
			Times(1).
			Return(nil)

		// We will exercise the following events:
		// 1. retriable error on lookupAddr.
		// 2. retriable error on dialOutgoingAddr.
		...
		c.testingKnobs.dialOutgoingAddr = func(outgoingAddr string) (net.Conn, error) {
			require.Equal(t, outgoingAddr, "127.0.0.10:42")
...
func TestConnector_lookupAddr(t *testing.T) {
	...
	t.Run("directory", func(t *testing.T) {
		...
		mock.EXPECT().
			EnsureTenantAddr(gomock.Eq(ctx), gomock.Eq(c.TenantID), gomock.Eq(c.ClusterName)).
