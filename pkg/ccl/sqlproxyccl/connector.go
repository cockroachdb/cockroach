...
//
// This is only used during connection migration.
func (c *connector) OpenTenantConnWithToken(
	...
	// When we use token-based authentication, we would still get the initial
	...
	// is connecting to the SQL pod), we'll slurp/discard all of the messages,
	// and only return once we've seen a ReadyForQuery message.
	//
	// NOTE: This will need to be updated when we implement query cancellation.
	if err := slurpInitialConnData(serverConn); err != nil {
		return nil, err
	}
