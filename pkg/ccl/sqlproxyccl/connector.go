...
//go:generate mockgen -package=sqlproxyccl -destination=mocks_generated_test.go . TenantResolver
...
func (c *connector) OpenClusterConnWithAuth(
	...
	if err := authenticate(clientConn, crdbConn, throttleHook); err != nil {
		return nil, true, err
	}
		...
		if err != nil {
			if isRetriableConnectorError(err) {
				...
				if backendDialErr.ShouldLog() {
					log.Ops.Errorf(ctx, "dial outgoing address (%d errors skipped): %v",
						codeBackendDownErrs, err)
				...
				if c.Directory != nil {
					if err = reportFailureToDirectory(
						ctx, c.TenantID, outgoingAddr, c.Directory,
					); err != nil {
						reportFailureErrs++
