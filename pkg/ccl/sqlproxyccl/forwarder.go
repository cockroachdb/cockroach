	...
	for f.ctx.Err() == nil {
		...
		case stateTransferRequested:
			...
			if !f.isSafeTransferPoint() {
				f.metrics.ConnMigrationRejectedCount.Inc(1)
			...
			f.metrics.ConnMigrationAcceptedCount.Inc(1)
		...
		case stateTransferInProgress:
			if err := f.processTransfer(); err != nil {
				...
				if !isConnRecoverableError(err) {
					f.metrics.ConnMigrationErrorCount.Inc(1)
				...
				f.metrics.ConnMigrationFailCount.Inc(1)
			} else {
...
func (f *forwarder) processTransfer() (retErr error) {
	f.mu.Lock()
	transferKey, transferCtx := f.mu.transferKey, f.mu.transferCtx
	f.mu.Unlock()
	...
	state, revivalToken, err := waitForShowTransferState(transferCtx,
		f.serverInterceptor, f.clientConn, transferKey)
	if err != nil {
	...
	newServerConn, err := f.connector.OpenTenantConnWithToken(transferCtx, revivalToken)
	if err != nil {
...
func (f *forwarder) runTransferTimeoutHandler(waitTimeout time.Duration) {
	...
	go func() {
		...
		case <-transferCtx.Done():
			f.mu.Lock()
			// If transferCtx is nil, that means the connection is recoverable
			// because we called finishTransfer.
			recoverable := f.mu.transferConnRecoverable || f.mu.transferCtx == nil
			f.mu.Unlock()

			// Connection is recoverable, don't close the connection. Context
			// cancellation will be propagated up accordingly.
			if recoverable {
				f.metrics.ConnMigrationTimeoutRecoverableCount.Inc(1)
				break
			}

			f.metrics.ConnMigrationTimeoutClosedCount.Inc(1)
