...
func TestClusterNameAndTenantFromParams(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			expectedParams:      map[string]string{},
			expectedMetrics: func(t *testing.T, m *metrics) {
				require.Equal(t, int64(1), m.ClusterIdentifierReadsTotal.Count())
				require.Equal(t, int64(1), m.ClusterIdentifierSNIReads.Value())
			},
		},
		{
			...
			expectedMetrics: func(t *testing.T, m *metrics) {
				require.Equal(t, int64(2), m.ClusterIdentifierReadsTotal.Count())
				require.Equal(t, int64(1), m.ClusterIdentifierDatabaseReads.Value())
				require.Equal(t, int64(1), m.ClusterIdentifierClusterOptionReads.Value())
			},
		},
		{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expectedMetrics != nil {
				...
				require.Zero(t, m.ClusterIdentifierReadsTotal.Count())
				require.Zero(t, m.ClusterIdentifierSNIReads.Value())
				require.Zero(t, m.ClusterIdentifierDatabaseReads.Value())
				require.Zero(t, m.ClusterIdentifierClusterOptionReads.Value())
			}
		})
	}
}
