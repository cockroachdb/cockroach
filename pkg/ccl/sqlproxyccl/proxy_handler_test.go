...
func newProxyServer(
	...
	handler, err := NewProxyHandler(ctx, stopper, *opts)
...
func TestLongDBName(t *testing.T) {
	...
	s, addr, done := newSecureProxyServer(ctx, t, stopper, &ProxyOptions{})
	defer done()
...
func TestFailedConnection(t *testing.T) {
	...
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	s, addr, done := newSecureProxyServer(ctx, t, stopper, &ProxyOptions{RoutingRule: "undialable%$!@$"})
	defer done()
...
func TestProxyModifyRequestParams(t *testing.T) {
	...
	s, proxyAddr, done := newSecureProxyServer(ctx, t, sql.Stopper(), &ProxyOptions{})
	defer done()
...
func TestInsecureDoubleProxy(t *testing.T) {
	...
	_, proxyA, cleanupA := newProxyServer(ctx, t, sql.Stopper(),
		&ProxyOptions{RoutingRule: sql.ServingSQLAddr(), Insecure: true},
	)
	defer cleanupA()
	_, proxyB, cleanupB := newProxyServer(ctx, t, sql.Stopper(),
...
func TestErroneousFrontend(t *testing.T) {
	...
	_, addr, cleanup := newProxyServer(ctx, t, stopper, &ProxyOptions{})
	defer cleanup()
...
func TestProxyKeepAlive(t *testing.T) {
	...
	s, addr, done := newSecureProxyServer(ctx, t, sql.Stopper(), &ProxyOptions{Denylist: denyList.Name()})
	defer done()
...
func TestProxyAgainstSecureCRDBWithIdleTimeout(t *testing.T) {
	...
	s, addr, done := newSecureProxyServer(ctx, t, sql.Stopper(), &ProxyOptions{IdleTimeout: idleTimeout})
	defer done()
