...
func newProxyServer(
	...
	handler, err := NewProxyHandler(ctx, stopper, &proxyMetrics, *opts)
	...
	require.NoError(t, err)
	require.NoError(t,
		stopper.RunAsyncTask(ctx, "proxy-ln-close", func(ctx context.Context) {
	...
	_ = server.Stopper.RunAsyncTask(ctx, "proxy-server-serve", func(ctx context.Context) {
...
func newDirectoryServer(
	...
	require.NoError(t, err)
	tds := tenant.NewTestDirectoryServer(tdsStopper)
...
func TestDirectoryReconnect(t *testing.T) {
	...
	url := fmt.Sprintf("postgres://root:admin@%s/?sslmode=disable&options=--cluster=dim-dog-28", addr)
	_, err = pgx.Connect(ctx, url)
	require.NoError(t, err)
	...
	var succeeded syncutil.AtomicBool
