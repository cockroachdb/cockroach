...
func TestConnectionMigration(t *testing.T) {
	...
	t.Run("successful", func(t *testing.T) {
		...
		t.Run("normal_transfer", func(t *testing.T) {
			...
			// at least half in total.
			require.Eventually(t, func() bool {
				return f.metrics.ConnMigrationSuccessCount.Count() >= 5
			}, 10*time.Second, 25*time.Millisecond)
			require.True(t, tenant1Addr > 2)
			require.True(t, tenant2Addr > 2)
			require.Equal(t, int64(0), f.metrics.ConnMigrationFailCount.Count())
			require.Equal(t, int64(0), f.metrics.ConnMigrationErrorCount.Count())
		...
		t.Run("failed_transfers_with_tx", func(t *testing.T) {
			...
			require.True(t, f.metrics.ConnMigrationFailCount.Count() >= 5)
			require.Equal(t, int64(0), f.metrics.ConnMigrationErrorCount.Count())
			require.Equal(t, initSuccessCount, f.metrics.ConnMigrationSuccessCount.Count())
			prevFailCount := f.metrics.ConnMigrationFailCount.Count()
			...
			require.Equal(t, prevFailCount, f.metrics.ConnMigrationFailCount.Count())
			require.Equal(t, int64(0), f.metrics.ConnMigrationErrorCount.Count())
		...
		t.Run("failed_transfers_with_dial_issues", func(t *testing.T) {
			initSuccessCount := f.metrics.ConnMigrationSuccessCount.Count()
			initFailCount := f.metrics.ConnMigrationFailCount.Count()
			initAddr := queryAddr(t, tCtx, db)
			...
			f.RequestTransfer()
			require.Eventually(t, func() bool {
				return f.metrics.ConnMigrationFailCount.Count() == initFailCount+1
			}, 10*time.Second, 25*time.Millisecond)
	...
	t.Run("transfer_timeout_in_response", func(t *testing.T) {
		...
		require.Equal(t, int64(0), f.metrics.ConnMigrationTimeoutClosedCount.Count())
		...
		require.Eventually(t, func() bool {
			return f.metrics.ConnMigrationTimeoutClosedCount.Count() == 1
