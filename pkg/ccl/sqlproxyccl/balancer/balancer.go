...
func collectRunningPodAssignments(
	...
	for addr, d := range podAssignments {
		...
		if excess > 0 {
			mustMove = append(mustMove, chooseNRandom(d, excess)...)
			d = d[:len(d)-excess]
			missingCount -= excess
		}
		...
		if excess > 0 {
			eligibleToMove = append(eligibleToMove, chooseNRandom(d, excess)...)
			d = d[:len(d)-excess]
		}
	...
	if missingCount > 0 {
		mustMove = append(mustMove, chooseNRandom(eligibleToMove, missingCount)...)
	}
...
// chooseNRandom picks n elements from source randomly and returns those.
// If n <= len(src), everything will be chosen. If n <= 0, this returns nil.
//
// NOTE: Elements in src may be shuffled around when this function returns. It
// is guaranteed that elements that are chosen will be the last n items of src.
var chooseNRandom = func(src []*ServerAssignment, n int) []*ServerAssignment {
	if n <= 0 {
		return nil
	}
	if n >= len(src) {
	...
	dst := make([]*ServerAssignment, 0, n)
	for i := 0; i < n; i++ {
		idx := rand.Intn(len(src))
		dst = append(dst, src[idx])
		src[idx], src[len(src)-1] = src[len(src)-1], src[idx]
		src = src[:len(src)-1]
	}
	return dst
}
