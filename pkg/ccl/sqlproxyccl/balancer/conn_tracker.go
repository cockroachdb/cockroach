...
func (e *tenantEntry) refreshPartitions() {
	// moveTo moves the given server assignment sa from the `from` partition to
	// the `to` partition. If the assignment is no longer present in the `from`
	// partition, this will be a no-op.
	moveTo := func(
		from map[*ServerAssignment]struct{},
		to map[*ServerAssignment]struct{},
		sa *ServerAssignment,
	) {
		e.mu.Lock()
		defer e.mu.Unlock()
	...
	for _, sa := range activeList {
		if sa.Owner() != nil && sa.Owner().IsIdle() {
			moveTo(e.assignments.active, e.assignments.idle, sa)
		}
	}
	for _, sa := range idleList {
		if sa.Owner() != nil && !sa.Owner().IsIdle() {
			moveTo(e.assignments.idle, e.assignments.active, sa)
		}
	}
}
