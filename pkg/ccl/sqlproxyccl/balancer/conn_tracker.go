...
func (e *tenantEntry) addHandle(handle ConnectionHandle) (success bool) {
	// Update the cache without holding onto the lock. This is because
	// handle.ServerRemoteAddr() obtains the forwarder lock, and we would like
	// to avoid any potential deadlocks in the future.
	defer func() {
		if success {
...
func (e *tenantEntry) removeHandle(handle ConnectionHandle) (success bool) {
	// Update the cache without holding onto the lock. This is because both
	// handle.ServerRemoteAddr() and handle.IsIdle obtain the forwarder lock,
	// and we would like to avoid any potential deadlocks in the future.
	defer func() {
		if success {
			// We'll update the cache based on the data that we have now. It is
			// possible that the cache was computed using a different state
			// (e.g. cache does not include this connection, or cache counted
			// this connection as active, but is idle now), resulting in us
			// decrementing from a different map here. Regardless, since this is
			// a cache, accuracy does not matter.
			if handle.IsIdle() {
