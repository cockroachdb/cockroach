...
func TestCollectRunningPodAssignments(t *testing.T) {
	...
	defer func(oldFn func(src []*ServerAssignment, n int) []*ServerAssignment) {
		chooseNRandom = oldFn
	}(chooseNRandom)
	chooseNRandom = func(src []*ServerAssignment, n int) []*ServerAssignment {
		if n <= 0 {
			return nil
		}
		if n >= len(src) {
...
func TestChooseNRandom(t *testing.T) {
	defer leaktest.AfterTest(t)()
