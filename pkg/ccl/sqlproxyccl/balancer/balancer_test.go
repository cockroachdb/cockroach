...
func TestRebalancer_rebalanceLoop(t *testing.T) {
	...
	// is draining. At the same time, 5 connections should be moved away from
	// runningPods[1] because 9 > 5 (average across 2 running pods).
	...
	testutils.SucceedsSoon(t, func() error {
		...
		if reflect.DeepEqual(map[string]int{runningPods[0].Addr: 5, runningPods[1].Addr: 5}, distribution) {
			return nil
		}
		return errors.Newf("expected balanced, but got %v", distribution)
	})
}
...
func TestEnqueueRebalanceRequests(t *testing.T) {
	...
	// Since rebalanceRate is 0.5, only the first ceil(7.5) = 8 will be
	// transferred.
	testutils.SucceedsSoon(t, func() error {
		for i := 0; i < 8; i++ {
			count := list[i].Owner().(*testConnHandle).transferConnectionCount()
			if count != 1 {
				return errors.Newf("pending count 1, but got %d", count)
			}
		}
		for i := 0; i < 7; i++ {
			count := list[i+8].Owner().(*testConnHandle).transferConnectionCount()
			if count != 0 {
				return errors.Newf("pending count 0, but got %d", count)
			}
		}
		return nil
	})
...
func TestCollectRunningPodAssignments(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.True(t, matched, "could not match expected set distribution")
		})
	}
}
