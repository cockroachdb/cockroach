	...
	for {
		...
		err = s.Stopper.RunAsyncTask(ctx, "proxy-con-serve", func(ctx context.Context) {
			...
			if err != nil && !errors.Is(err, context.Canceled) {
				...
				if s.shouldLogError(err, conn, reqTags) {
...
// pair.
func (s *Server) shouldLogError(err error, conn net.Conn, reqTags map[string]interface{}) bool {
	...
	tenID, hasTenantID := reqTags["tenant"]
	if !hasTenantID {
		return true
	}
