...
type Server struct {
	connHandler     func(metrics *Metrics, proxyConn *Conn) error
	mux             *http.ServeMux
	Metrics         *Metrics
...
// checks as well.
func NewServer(connHandler func(metrics *Metrics, proxyConn *Conn) error) *Server {
...
func (s *Server) Serve(ln net.Listener) error {
	for {
		...
		go func() {
			...
			err := s.connHandler(s.Metrics, conn)
