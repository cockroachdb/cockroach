...
type Options struct {
	...
	// Deprecated: pass a BackendDialer instead.
	...
	BackendDialer func(*pgproto3.StartupMessage) (net.Conn, error)
}
...
func (s *Server) Proxy(proxyConn *Conn) error {
	...
	if backendDialer == nil {
		...
		backendDialer = func(msg *pgproto3.StartupMessage) (net.Conn, error) {
			...
			crdbConn = KeepAliveOverlay(ctx, crdbConn, backendConfig.KeepAliveLoop)
			crdbConn = IdleDisconnectOverlay(crdbConn, backendConfig.IdleTimeout)
			return crdbConn, nil
		}
	}
	...
	if err != nil {
		...
		sendErrToClient(conn, codeErr.code, "unable to reach backend SQL server")
		return codeErr
	}
	...
	errOutgoing := make(chan error, 1) // outgoing is frontend->backend
	errIncoming := make(chan error, 1) // incoming is backend->frontend
