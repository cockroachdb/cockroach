...
type Options struct {
	...
	ProxyInterceptor func(s *Server, proxyConn *Conn) error
...
func (s *Server) Proxy(proxyConn *Conn) error {
	...
	go func() {
		err := s.ConnectionCopy(crdbConn, conn)
...
func (s *Server) ConnectionCopy(crdbConn, conn net.Conn) error {
	...
	case err := <-errIncoming:
		if err == nil {
			...
			s.SendErrToClient(conn, CodeIdleDisconnect, "terminating connection due to idle timeout")
