...
func TestInsecureDoubleProxy(t *testing.T) {
	...
	sa, proxyA, cleanupA := newInsecureProxyServer(t, tc.Server(0).ServingSQLAddr(),
		nil /* tls config */)
	defer cleanupA()
	sb, proxyB, cleanupB := newInsecureProxyServer(t, proxyA, nil /* tls config */)
	defer cleanupB()
	...
	defer func() {
		require.NoError(t, conn.Close(ctx))
		require.Equal(t, int64(1), sa.metrics.SuccessfulConnCount.Count())
		require.Equal(t, int64(1), sa.metrics.AuthFailedCount.Count())
		require.Equal(t, int64(1), sb.metrics.SuccessfulConnCount.Count())
		require.Equal(t, int64(1), sb.metrics.AuthFailedCount.Count())
	}()
	require.NoError(t, runTestQuery(conn))

	u = fmt.Sprintf("postgres://bob:wrong@%s?sslmode=disable", proxyB)
	_, err = pgx.Connect(context.Background(), u)
	require.Error(t, err)
	require.True(t, testutils.IsError(err, "ERROR: password authentication failed for user bob"))
}
