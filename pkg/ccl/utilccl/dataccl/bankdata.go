...
// Split creates the configured number of ranges in a created version of the
...
func Bank(rows int, payloadBytes int, ranges int) Data {
	// TODO(dan): BEFORE MERGE this interface is a little wonky, but it's a bit
	// annoying to replace it with a struct because that would make most of the
	// callsites wrap. Complicating things is that there needs to be a
	// distinction between the default value and an explicit 0 for ranges and
	// payloadBytes.
	if rows == CfgDefault {
		rows = 1000
	}
