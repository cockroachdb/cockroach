...
// cfgDefault will trigger the default for any of the parameters for `Bank`.
const cfgDefault = -1
...
func BankRows(rows int) Data {
	return Bank(rows, cfgDefault, cfgDefault)
}
...
func Bank(rows int, payloadBytes int, ranges int) Data {
	...
	if payloadBytes == cfgDefault {
		payloadBytes = 100
	}
	if ranges == cfgDefault {
		ranges = 10
	}
	if ranges > rows {
		ranges = rows
