...
// without introducing a dependency cycle. This function is useful for packages
// that can't import `ccl`. However, it panics if package `ccl` (which, in turn,
// imports all the other ccl/... packages) wasn't linked into the binary. In
// tests, generally the right thing to do is to import `ccl` in a file in the
// `foo_test` pkg (for example, main_test.go). Frequently, calling
// ccl.TestingEnableEnterprise in TestMain is enough, so this
// utilccl.TestingEnableEnterprise is not frequently used. Even if this function
// didn't panic, CheckEnterpriseEnabled() requires all the CCL code to be linked
// in, so callers of utilccl.TestingEnableEnterprise would not get what they
// want when `ccl` is not linked it.
