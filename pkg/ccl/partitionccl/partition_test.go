		...
		if strings.HasPrefix(subzoneShort, "@") {
			...
			if len(constraints) > 0 {
				fmt.Fprintf(&zoneConfigStmts,
					`ALTER INDEX %s@%s CONFIGURE ZONE 'constraints: [%s]';`,
			...
			if len(constraints) > 0 {
				fmt.Fprintf(&zoneConfigStmts,
					`ALTER PARTITION %s OF TABLE %s CONFIGURE ZONE 'constraints: [%s]';`,
...
func TestRemovePartitioningExpiredLicense(t *testing.T) {
	...
	sqlDB.Exec(t, `ALTER PARTITION p1 OF TABLE t CONFIGURE ZONE ''`)
	sqlDB.Exec(t, `ALTER PARTITION p34 OF TABLE t CONFIGURE ZONE ''`)
	sqlDB.Exec(t, `ALTER INDEX t@primary CONFIGURE ZONE ''`)
	sqlDB.Exec(t, `ALTER INDEX t@i CONFIGURE ZONE ''`)
	...
	expectLicenseErr(`ALTER PARTITION p1 OF TABLE t CONFIGURE ZONE ''`)
	expectLicenseErr(`ALTER PARTITION p34 OF TABLE t CONFIGURE ZONE ''`)
	expectLicenseErr(`ALTER INDEX t@primary CONFIGURE ZONE ''`)
	expectLicenseErr(`ALTER INDEX t@i CONFIGURE ZONE ''`)

	// But they can be removed.
	sqlDB.Exec(t, `ALTER TABLE t PARTITION BY NOTHING`)
	sqlDB.Exec(t, `ALTER INDEX t@i PARTITION BY NOTHING`)
	sqlDB.Exec(t, `ALTER INDEX t@primary CONFIGURE ZONE NULL`)
	sqlDB.Exec(t, `ALTER INDEX t@i CONFIGURE ZONE NULL`)
