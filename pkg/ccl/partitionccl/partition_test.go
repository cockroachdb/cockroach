...
`, jobID).Scan(&isDropJob); err != nil {
	...
	ctx := context.Background()
	defer s.Stopper().Stop(ctx)
	defer closeOnce.Do(func() { close(dropCh) })
	...
	testutils.SucceedsSoon(t, func() error {
		var done bool
		tdb.QueryRow(t, `SELECT count(*) = 0
  FROM [SHOW JOBS]
 WHERE job_type = 'TYPEDESC SCHEMA CHANGE' AND status IN`+
			jobs.NonTerminalStatusTupleString).Scan(&done)
		if done {
			return nil
		}
	...
	closeOnce.Do(func() { close(dropCh) })
