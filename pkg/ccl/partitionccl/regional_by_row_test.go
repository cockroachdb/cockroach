...
// REGIONAL BY ROW which gets interrupted.
func TestAlterTableLocalityRegionalByRowError(t *testing.T) {
	...
	// alterState is a struct that contains an action for a base test case to alter towards.
	...
	nonRegionalByRowAlterStates := []alterState{
		...
		{
			desc:                  "REGIONAL BY ROW, cancel during add column",
	...
	for _, testCase := range []struct {
		...
		},
		{
			...
			expectedPKCols: []string{"crdb_region", "rowid"},
		...
		},
		{
			...
			expectedPKCols: []string{"cr2", "rowid"},
			alterStates:    regionalByRowAlterStates,
		},
	} {
...
`, testCase.setupQuery)); err != nil {
	...
	`); err != nil {
							...
							testutils.SucceedsSoon(t, func() error {
								...
								if tableDesc.GetPrimaryIndex().NumColumns() != len(testCase.expectedPKCols) {
									return errors.Errorf("expected primary key change to not succeed after cancellation")
								}
								for i, name := range testCase.expectedPKCols {
