...
CREATE TABLE db.t (
	...
	id2 INT,
	id3 INT UNIQUE,
	UNIQUE (id2) PARTITION BY LIST (id2) (
    PARTITION id2_one VALUES IN (1),
    PARTITION id2_two VALUES IN (2)
	    )
...
INSERT INTO db.t VALUES (1, 1, 3), (2, 2, 4);
`); err != nil {
		t.Fatalf("unexpected error: %s", err)
	}

	// Overwrite the value on partition one with a duplicate unique value for id3.
	values := []tree.Datum{tree.NewDInt(1), tree.NewDInt(1), tree.NewDInt(4)}
	tableDesc := desctestutils.TestingGetPublicTableDescriptor(kvDB, keys.SystemSQLCodec, "db", "t")
	primaryIndex := tableDesc.GetPrimaryIndex()
	var colIDtoRowIndex catalog.TableColMap
	colIDtoRowIndex.Set(tableDesc.PublicColumns()[0].GetID(), 0)
