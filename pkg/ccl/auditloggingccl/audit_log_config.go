...
func validateAuditLogConfig(_ *settings.Values, input string) error {
	...
	if auditlogging.UserAuditEnterpriseParamsHook == nil {
		return errors.New("Cannot validate log config, enterprise parameters not initialized yet")
	}
	st, clusterID := auditlogging.UserAuditEnterpriseParamsHook()
