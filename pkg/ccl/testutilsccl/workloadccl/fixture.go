...
//
// Files are chunked into ~chunkSizeBytes or smaller. Concurrency is limited by
// sem. The GCS object paths to the written files are returned on pathsCh.
func (c *groupCSVWriter) groupWriteCSVs(
	...
	if err := func() error {
		...
		path := filepath.Join(c.folder, table.Name, fmt.Sprintf(`%09d.csv`, rowStart))
		const maxAttempts = 3
		err := retry.WithMaxAttempts(ctx, base.DefaultRetryOptions(), maxAttempts, func() error {
			...
			for rowIdx = rowStart; rowIdx < rowEnd; rowIdx++ {
				if c.chunkSizeBytes > 0 && bytesWrittenW.BytesWritten() > c.chunkSizeBytes {
					break
				}
			...
			pathsCh <- path
			newBytesWritten := atomic.AddInt64(&c.csvBytesWritten, bytesWrittenW.BytesWritten())
			d := timeutil.Since(c.start)
			throughput := float64(newBytesWritten*int64(time.Second)) / float64(d.Nanoseconds()*2<<20)
...
func (w *bytesWrittenWriter) BytesWritten() int64 {
	return w.written
}
