...
func addFakeJob(t *testing.T, h *testHelper, id int64, status Status, txn *kv.Txn) {
	...
	n, err := h.ex.ExecEx(context.Background(), "fake-job", txn,
		...
		fmt.Sprintf("INSERT INTO %s (sched_id, status, payload) VALUES ($1, $2, $3)",
...
func TestJobSchedulerReschedulesRunning(t *testing.T) {
	...
	for _, wait := range []jobspb.ScheduleDetails_WaitBehavior{
		...
		t.Run(wait.String(), func(t *testing.T) {
			...
			j := h.newScheduledJob("j", "j sql")
			j.execSpec.Mutable().Wait = wait
			...
			require.NoError(t,
				h.kvDB.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
					...
					for _, status := range []Status{
						...
						addFakeJob(t, h, j.schedID.Value(), status, txn)
					}
					return nil
				}))
			...
			require.Equal(t, expectedRunTime, loaded.nextRun.Value())
			...
			// future.
			c := newCronDaemon(h.env, h.kvDB, h.ex)
			c.findJobs(ctx)
...
func TestJobSchedulerExecutesAndSchedulesNextRun(t *testing.T) {
	...
	j := h.newScheduledJob("j", "SELECT 42 AS meaning_of_life;")
	require.NoError(t, j.SetSchedule("@hourly"))
	...
	loaded = h.loadJob(t, j.schedID.Value())
	require.Equal(t, expectedRunTime, loaded.nextRun.Value())
	require.Equal(t, "executed: rows effected 0", loaded.changeInfo.Changes[0].Reason)
}
