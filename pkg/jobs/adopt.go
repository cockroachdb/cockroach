...
, $3  >= COALESCE(last_run, created) + least(
	...
	if err := r.stopper.RunAsyncTask(ctx, job.taskName(), func(ctx context.Context) {
		...
		//TODO(sajjad): To discuss: Didn't we remove the job from adoptedJobs already
		// before returning from runJob in the above line? In any case, attempting to
		// remove again doesn't harm the system.
		...
		for _, row := range rows {
			...
			case StatusReverting:
				if err := job.Update(ctx, txn, func(txn *kv.Txn, md JobMetadata, ju *JobUpdater) error {
					//TODO (sajjad): To discuss: Should we unregister the job here?
					// If the job is not already cancelled and has not stopped its execution,
					// we should not unregister the job. We should only cancel the job's
					// context. Even if we don't unregister the job here, it will eventually
					// be unregistered when the runJob() function completes with an error.
					// If we unregister the job before it stops executing, we can end up
					// in a dangerous state due to concurrent execution. It may happen that
					// this cancel loop unregisters the job while a concurrent adopt loop
					// immediately registers the job. In this way, the job may be running
					// in two go-routines for a short duration: one is reverting the job
					// whereas the other may moving towards success or returning an error.
					r.unregister(id)
