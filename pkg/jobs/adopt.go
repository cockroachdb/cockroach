...
// available.
func (r *Registry) claimJobs(ctx context.Context, sid sqlliveness.SessionID) error {
...
func (r *Registry) processClaimedJobs(ctx context.Context, sid sqlliveness.SessionID) error {
...
WHERE (status = $1 OR status = $2) AND (claim_session_id = $3 AND claim_instance_id = $4)`,
		StatusRunning, StatusReverting, sid, r.ID(),
	)
	if err != nil {
	...
	for id := range claimedToResume {
		if err := r.resumeJob(ctx, id, sid); err != nil {
...
func (r *Registry) resumeJob(ctx context.Context, jobID int64, sid sqlliveness.SessionID) error {
...
SELECT status, payload, progress FROM system.jobs WHERE id = $1 AND claim_session_id = $2`,
	...
	r.mu.adoptedJobs[jobID] = &adoptedJob{sid: sid, cancel: cancel}
...
func (r *Registry) servePauseAndCancelRequests(
...
RETURNING id, status`,
		...
		for _, row := range rows {
			...
			case StatusPaused:
				r.unregister(id)
				log.Infof(ctx, "job %d, session %s: paused", id, hex.EncodeToString(sid))
