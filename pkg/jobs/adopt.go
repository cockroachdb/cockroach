...
func (r *Registry) maybeDumpTrace(
	...
	if jobErr != nil && !strings.Contains(jobErr.Error(), "job canceled") && resumerCtx.Err() == nil {
		r.td.MaybeDump(dumpCtx, strconv.Itoa(int(jobID)), traceID, r.ex)
		return
	}

	// If the dump mode is set to `dumpAlways` then we should dump the trace when
	// the job is any of paused, canceled, succeeded or failed state.
	if dumpMode == int64(alwaysDump) {
		r.td.MaybeDump(dumpCtx, strconv.Itoa(int(jobID)), traceID, r.ex)
	}
}
