...
//
// setterIdx is the beginning index in the place holders.
func canRunClause(setterIdx int) string {
	// The placeholder numbers in the following statement should be equivalent to:
	// $1 ...
	//            $2 ...,
	//            $2 ...,
	//            $3
	...
	//        ) ...
	stmt := `
$%d  >= COALESCE(last_run, created) + least(
          IF(
            $%d * (power(2, least(62, COALESCE(num_runs, 0))) - 1)::FLOAT >= 0.0,
            $%d * (power(2, least(62, COALESCE(num_runs, 0))) - 1)::FLOAT,
            $%d
          ),
          $%d
        )::INTERVAL`
	return fmt.Sprintf(stmt, setterIdx, setterIdx+1, setterIdx+1, setterIdx+2, setterIdx+2)
}
...
func (r *Registry) resumeJob(ctx context.Context, jobID jobspb.JobID, s sqlliveness.Session) error {
	...
	cols := "status, payload, progress, crdb_internal.sql_liveness_is_alive(claim_session_id)"
	fromTable := " FROM system.jobs WHERE id = $1 AND claim_session_id = $2 "
	...
	if backoffIsActive {
		cols += ", " + canRunClause(3) + " as can_run"
		initDelay := retryInitialDelaySetting.Get(&r.settings.SV).Seconds()
		maxDelay := retryMaxDelaySetting.Get(&r.settings.SV).Seconds()
		args = append(args, r.clock.Now().GoTime(), initDelay, maxDelay)
	}
	stmt := "SELECT " + cols + fromTable
