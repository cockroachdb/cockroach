...
var statusIsOperating = fmt.Sprintf("status IN ('%s', '%s', '%s', '%s')",
...
func (r *Registry) claimJobs(
	...
	for _, c := range liveClaims {
		...
		whereClaimExpired = fmt.Sprintf("%s OR (sqlliveness_name = '%s' AND sqlliveness_epoch < %d)",
...
WHERE (%s) AND (%s) ORDER BY created DESC LIMIT $3 RETURNING id`, statusIsOperating, whereClaimExpired),
...
func (r *Registry) processClaimedJobs(ctx context.Context, epoch int64) error {
	if err := r.ServePauseAndCancelRequests(ctx, epoch); err != nil {
		// TODO should we fail or just log the error and try to do smething atmore at least???
		return errors.Wrapf(err, "could not cancel/pause claimed jobs")
	}
...
WHERE (status = $1 OR status = $2) AND (sqlliveness_name = $3 AND sqlliveness_epoch = $4)`,
	...
	for id := range claimedToResume {
		if err := r.runJob(ctx, id, epoch); err != nil {
...
func (r *Registry) runJob(ctx context.Context, jobID, epoch int64) error {
	log.Infof(ctx, "job %d: attempting to (re-)start", jobID)
	...
	if row == nil {
		return errors.Errorf("job %d: changed, no longer eligible for restarting", jobID)
	}
	status := Status(*row[0].(*tree.DString))
	if status != StatusRunning && status != StatusReverting {
		// A concurrent process can request the job to be paused or cancelled.
		return errors.Errorf("job %d: status changed to %s which is not restartable", jobID, status)
	}
