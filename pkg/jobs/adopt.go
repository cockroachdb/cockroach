...
`
	...
	nextRunClause = `
AND ($5 > (COALESCE(last_run, created) + least(($6::INTERVAL * ((1 << least(62, COALESCE(num_runs, 0))) - 1)), $7::INTERVAL)))`
)
...
func getProcessQuery(
	...
	if r.settings.Version.IsActive(ctx, clusterversion.RetryJobsWithExponentialBackoff) {
		...
		query += nextRunClause
		retryBase := retryBaseSetting.Get(&r.settings.SV).Seconds()
		retryMax := retryMaxDelaySetting.Get(&r.settings.SV).Seconds()
		args = append(args, []interface{}{r.clock.Now().GoTime(), retryBase, retryMax}...)
	}
	return query, args
}
