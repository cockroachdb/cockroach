...
// ExecutionEvent in executionLog. An error is returned if executionLog or an
// ExecutionEvent in the log is nil. Default values are omitted from the resulting
// JSON object.
func ExecutionLogToJSON(executionLog []*ExecutionEvent) (json.JSON, error) {
	if executionLog == nil {
		return nil, errors.New("unable to convert a nil slice")
	}
