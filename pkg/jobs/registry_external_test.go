...
func TestRegistrySettingUpdate(t *testing.T) {
	...
	for _, test := range [...]struct {
		...
		short      interface{} // Duration when expecting a large number of job runs.
		...
		t.Run(test.name, func(t *testing.T) {
			...
			// seconds.
			tdb.Exec(t, fmt.Sprintf("SET CLUSTER SETTING %s = '%v'", test.setting, test.short))
			atomic.StoreInt32(&seen, 0)
			testutils.SucceedsSoon(t, func() error {
				counted = int(atomic.LoadInt32(&seen))
				if counted >= moreThan {
