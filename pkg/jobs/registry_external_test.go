 ...
 WHERE job_id = $1;`, id),
	...
	t.Run("truncation", func(t *testing.T) {
		...
		var firstExecErr parsedError
		{
			...
			// in the logs.
			expTruncatedError := jobs.ExecutionErrorCauseTruncatedPrefix +
				err1[:maxSize-len(jobs.ExecutionErrorCauseTruncatedPrefix)]
		...
		{
			execErrs := getExecErrors(t, id)
			require.Len(t, execErrs, 2) // one ResumeError, one CleanupError
			executionErrorEqual(t, secondExecErr, execErrs[0])
			// Ensure we see the truncated error in the table but the full error
			// in the logs.
			expTruncatedError := jobs.ExecutionErrorCauseTruncatedPrefix +
				err4[:maxSize-len(jobs.ExecutionErrorCauseTruncatedPrefix)]
			checkExecutionError(t, execErrs[1], jobs.StatusReverting, fourthStart, fifthStart, expTruncatedError)
		...
		sixthRun, sixthStart := waitForEvent(t, id)
		{
			...
			require.Len(t, execErrs, 2) // one ResumeError, one CleanupError
			executionErrorEqual(t, secondExecErr, execErrs[0])
			checkExecutionError(t, execErrs[1], jobs.StatusReverting, fifthStart, sixthStart, err5)
			checkLogEntry(t, id, jobs.StatusReverting, fifthStart, sixthStart, err5)
		}
		close(sixthRun.resume)
