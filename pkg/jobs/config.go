...
type loopController struct {
	...
	lastRun time.Time
	updated chan struct{}
	// getInterval returns shortInterval of the associated cluster setting.
...
func (lc *loopController) onUpdate() {
	lc.onUpdateWithBound(lc.getInterval())
}

// onUpdateWithBound sets the associated timer's reset shortInterval to the minimum
// of associated cluster setting and maxDur.
func (lc *loopController) onUpdateWithBound(maxDur time.Duration) {
	interval := lc.getInterval()
	if maxDur < interval {
		interval = maxDur
	}
	lc.timer.Reset(timeutil.Until(lc.lastRun.Add(jitter(interval))))
}

// onExecute is called after the associated job is executed.
func (lc *loopController) onExecute() {
	lc.onExecuteWithBound(lc.getInterval())
}

// onExecuteWithBound is called after the associated jobs is executed. It sets
// the upper limit of the timer's reset value to maxDur.
func (lc *loopController) onExecuteWithBound(maxDur time.Duration) {
	lc.timer.Read = true
	...
	lc.timer.Reset(jitter(interval))
}
