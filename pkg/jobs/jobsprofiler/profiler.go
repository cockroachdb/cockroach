...
func StorePlanDiagram(
	...
	if err := stopper.RunAsyncTask(ctx, "jobs-store-plan-diagram", func(ctx context.Context) {
		...
		err := db.Txn(ctx, func(ctx context.Context, txn isql.Txn) error {
			...
			key, err := dspKey.MakeKey(timeutil.Now().UnixNano())
			if err != nil {
				return errors.Wrap(err, "failed to construct DSP diagram info key")
			}
			infoStorage := jobs.InfoStorageForJob(txn, jobID)
			return infoStorage.Write(ctx, key, []byte(diagURL.String()))
		})
		if err != nil {
...
// node and processor executing as part of the job's DistSQL flow.
func StorePerNodeProcessorProgressFraction(
	...
	if err := db.Txn(ctx, func(ctx context.Context, txn isql.Txn) error {
		...
		for componentID, fraction := range perComponentProgress {
			progressKey := profilerconstants.MakeNodeProcessorProgressInfoKey()
			key, err := progressKey.MakeKey(componentID.FlowID.String(),
