...
// NotifyJobTermination is a callback invoked by the system job before it completes.
...
func NotifyJobTermination(
	...
	schedule, executor, err := lookupScheduleAndExecutor(ctx, env, md, ex, txn)
	if err != nil {
		return err
	}
...
func lookupScheduleAndExecutor(
	...
	rows, cols, err := ex.QueryWithCols(ctx, "lookup-schedule", txn,
		...
		fmt.Sprintf(
			...
			env.ScheduledJobsTableName(), md.Payload.ScheduleID))
