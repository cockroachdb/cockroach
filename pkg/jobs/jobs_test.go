...
func TestRegistryLifecycle(t *testing.T) {
	...
	t.Run("normal success", func(t *testing.T) {
		...
		j, err := rts.registry.CreateAndStartJobTesting(rts.ctx, nil, rts.mockJob)
		if err != nil {
			t.Fatal(err)
		}
		rts.job = j
	...
	t.Run("rollback", func(t *testing.T) {
		...
		rts.setUp(t)
		defer rts.tearDown()
		job, err := rts.registry.CreateAndStartJobTesting(rts.ctx, nil, rts.mockJob)
		if err != nil {
			t.Fatal(err)
		}
		rts.job = job
...
func TestShowJobWhenComplete(t *testing.T) {
	...
	t.Run("show job", func(t *testing.T) {
		...
		job, err := registry.CreateAndStartJobTesting(ctx, nil, mockJob)
		if err != nil {
			t.Fatal(err)
		}
...
func TestStartableJobMixedVersion(t *testing.T) {
	...
	require.NoError(t, db.Txn(ctx, func(ctx context.Context, txn *kv.Txn) (err error) {
		j, err = jr.CreateStartableJobWithTxn(ctx, j, jobID, txn, jobs.Record{
			Details:  jobspb.ImportDetails{},
			Progress: jobspb.ImportProgress{},
		})
		return err
...
func TestStartableJob(t *testing.T) {
	...
	createStartableJob := func(t *testing.T) (sj *jobs.StartableJob) {
		jobID := jr.MakeJobID()
		require.NoError(t, db.Txn(ctx, func(ctx context.Context, txn *kv.Txn) (err error) {
			sj, err = jr.CreateStartableJobWithTxn(ctx, sj, jobID, txn, rec)
			return err
		}))
		return sj
	}
	...
	t.Run("Start called with active txn", func(t *testing.T) {
		...
		sj, err := jr.CreateStartableJobWithTxn(ctx, nil, jr.MakeJobID(), txn, rec)
		require.NoError(t, err)
		err = sj.Start(ctx)
