...
func TestRejectInvalidJobID(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)
	...
	t.Run("Get", func(t *testing.T) {
		require.NoError(t, s.InternalDB().(isql.DB).
			Txn(ctx, func(ctx context.Context, txn isql.Txn) error {
				_, _, err := Get(ctx, txn, jobspb.InvalidJobID, name)
				require.Error(t, err)
	...
	t.Run("GetResolvedSpans", func(t *testing.T) {
		require.NoError(t, s.InternalDB().(isql.DB).
			Txn(ctx, func(ctx context.Context, txn isql.Txn) error {
				_, _, err := GetResolvedSpans(ctx, txn, jobspb.InvalidJobID, name)
				require.Error(t, err)
				require.Contains(t, err.Error(), "invalid job ID")
				return nil
			}))
	...
	t.Run("GetAllResolvedSpans", func(t *testing.T) {
		require.NoError(t, s.InternalDB().(isql.DB).
			Txn(ctx, func(ctx context.Context, txn isql.Txn) error {
				_, _, err := GetAllResolvedSpans(ctx, txn, jobspb.InvalidJobID)
				require.Error(t, err)
				require.Contains(t, err.Error(), "invalid job ID")
				return nil
			}))
	...
	t.Run("Store", func(t *testing.T) {
		require.NoError(t, s.InternalDB().(isql.DB).
			Txn(ctx, func(ctx context.Context, txn isql.Txn) error {
				...
				err = Store(ctx, txn, jobspb.InvalidJobID, name, frontier)
				require.Error(t, err)
				require.Contains(t, err.Error(), "invalid job ID")
				return nil
			}))
	...
	t.Run("Delete", func(t *testing.T) {
		require.NoError(t, s.InternalDB().(isql.DB).
			Txn(ctx, func(ctx context.Context, txn isql.Txn) error {
				err := Delete(ctx, txn, jobspb.InvalidJobID, name)
				require.Error(t, err)
				require.Contains(t, err.Error(), "invalid job ID")
				return nil
			}))
