...
func TestJobProgressAndStatusAccessors(t *testing.T) {
	...
	t.Run("progress", func(t *testing.T) {
		...
		require.NoError(t, idb.Txn(ctx, func(ct context.Context, txn isql.Txn) error {
			if err := job1.ProgressStorage().Write(ctx, txn, 0.4, hlc.Timestamp{}); err != nil {
				return err
			}
	...
	t.Run("status-and-message", func(t *testing.T) {
		...
		require.NoError(t, idb.Txn(ctx, func(ct context.Context, txn isql.Txn) error {
			return job1.StatusStorage().Record(ctx, txn, "k1", "a")
		}))
		...
		require.NoError(t, idb.Txn(ctx, func(ct context.Context, txn isql.Txn) error {
			var err error
			j1Messages, err = job1.StatusStorage().AllMessages(ctx, txn)
			if err != nil {
				return err
			}
			j2Messages, err = job2.StatusStorage().AllMessages(ctx, txn)
			return err
		}))
