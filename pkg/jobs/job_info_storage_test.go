...
func TestJobInfoAccessors(t *testing.T) {
	...
	require.NoError(t, idb.Txn(ctx, func(ctx context.Context, txn isql.Txn) error {
		infoStorage := job1.InfoStorageWithTxn(txn)
	...
	require.NoError(t, idb.Txn(ctx, func(ctx context.Context, txn isql.Txn) error {
		infoStorage := job2.InfoStorageWithTxn(txn)
	...
	require.NoError(t, idb.Txn(ctx, func(ctx context.Context, txn isql.Txn) error {
		infoStorage := job3.InfoStorageWithTxn(txn)
		return infoStorage.Iterate(ctx, kPrefix, func(key, value []byte) error {
...
func TestAccessorsWithWrongSQLLivenessSession(t *testing.T) {
	...
	require.NoError(t, err)
	infoStorage := job.InfoStorageNoTxn()
	require.NoError(t, infoStorage.Write(ctx, []byte("foo"), []byte("baz")))
	...
	require.True(t, testutils.IsError(infoStorage.Write(ctx, []byte("foo"), []byte("bar")),
	...
	require.NoError(t, infoStorage.Iterate(ctx, []byte("foo"), func(infoKey, value []byte) error {
