...
func TestPrivileges(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		accessLevel jobsprivilege.AccessLevel
	...
	}{
		{
			...
			accessLevel: jobsprivilege.ControlAccess,
		...
		},
		{
			...
			payload:     makeBackupPayload("user2"),
			accessLevel: jobsprivilege.ViewAccess,
			canAccess:   false,
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			err := jobsprivilege.Authorize(ctx, testAuth, 0, tc.payload, tc.accessLevel)
