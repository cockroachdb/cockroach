...
func TestPrivileges(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			flag:        jobsprivilege.UserCanAccessOwnJob,
			canAccess:   true,
		},
		{
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			err := jobsprivilege.Authorize(ctx, testAuth, 0, tc.payload, tc.flag)
