...
func TestCreateOneOffJob(t *testing.T) {
	...
	loaded := h.loadJob(t, j.ScheduleID())
	require.True(t, loaded.NextRun().Equal(j.NextRun()))
...
func TestFailedJobsHandling(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.onerror.String(), func(t *testing.T) {
			...
			require.NoError(t, NotifyJobCompletion(ctx, h.env, md, h.ex, nil))

			// Verify nextRun updated
			loaded := h.loadJob(t, j.ScheduleID())
			require.True(t, test.nextRun.Equal(loaded.NextRun()))
		})
	}
}
