...
func (j *Job) Update(ctx context.Context, txn *kv.Txn, updateFn UpdateFn) error {
	...
	if err := j.runInTxn(ctx, txn, func(ctx context.Context, txn *kv.Txn) error {
		sessionIDClause := ""
		if j.sessionID != "" {
			sessionIDClause = ", claim_session_id"
		}
		...
		if j.registry.settings.Version.IsActive(ctx, clusterversion.RetryJobsWithExponentialBackoff) {
			statsClause = ", COALESCE(last_run, created), COALESCE(num_runs, 0)"
		}
		forUpdateClause := " FOR UPDATE"
		// We added this testing knob to evade deadlocks/livelocks caused due to
		// "FOR UPDATE" locking.
		if j.registry.knobs.DisableForUpdate {
			forUpdateClause = ""
		}
		stmt := fmt.Sprintf("SELECT status, payload, progress%s%s FROM system.jobs WHERE id = $1%s",
			sessionIDClause, statsClause, forUpdateClause)

		var err error
		var row tree.Datums
		if row, err = j.registry.ex.QueryRowEx(
			ctx,
			"log-job",
			txn,
