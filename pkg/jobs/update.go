...
// WithReadLock will ensure that the Updater acquires a SELCT FOR UPDATE lock
// when reading the job row.
func (u Updater) WithReadLock() Updater {
	u.useReadLock = true
	return u
}

// Updater binds a job to a transaction and can be used to update the job.
type Updater struct {
	j           *Job
	txn         isql.Txn
	useReadLock bool
...
// NoTxn creates a non-transactional Updater.
func (j *Job) NoTxn() Updater {
	return Updater{j: j}
}

// WithTxn creates a transactional Updater.
func (j *Job) WithTxn(txn isql.Txn) Updater {
	return Updater{j: j, txn: txn}
}

// maybeWithTxn is an internal convenience to construct an updater with
...
func (u Updater) update(ctx context.Context, updateFn UpdateFn) (retErr error) {
	...
	row, err := u.txn.QueryRowEx(
		...
		getSelectStmtForJobUpdate(j.session != nil, u.useReadLock), j.ID(),
	)
	if err != nil {
		return err
	}
