...
func (j *Job) Update(ctx context.Context, txn *kv.Txn, updateFn UpdateFn) error {
	...
	if err := j.runInTxn(ctx, txn, func(ctx context.Context, txn *kv.Txn) error {
		cols := "status, payload, progress"
		if j.sessionID != "" {
			cols += ", claim_session_id"
		}
		...
		if backoffIsActive {
			cols += ", COALESCE(last_run, created), COALESCE(num_runs, 0)"
		}
		...
		stmt := "SELECT " + cols + " FROM system.jobs WHERE id = $1 " + forUpdate
