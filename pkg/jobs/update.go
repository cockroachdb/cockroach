...
`
	...
	if v.AtLeast(clusterversion.V25_1_AddJobsTables.Version()) {
		if ju.md.Status != "" && ju.md.Status != status {
			if err := j.StatusStorage().Record(ctx, u.txn, "state", string(ju.md.Status)); err != nil {
				return err
			}
		}
		...
		if progress != nil {
			...
			} else if ju.md.Status != "" && ju.md.Status != status {
			...
			if progress.TraceID != beforeProgress.TraceID {
				if err := j.StatusStorage().Record(ctx, u.txn, "trace-id", fmt.Sprintf("%d", progress.TraceID)); err != nil {
					return err
				}
			}
		}
