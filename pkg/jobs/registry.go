...
// jobMetadataTables all all of the tables that have rows for a given job that
// are idenfitied by the job_id column in that table; when a given job ID is
// deleted from the jobs system, all rows with its ID should be deleted from
// each of these tables as well.
