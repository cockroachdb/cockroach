...
func (r *Registry) FindMatchingChangefeedJobs(
	...
	it, err := txn.QueryIteratorEx(
		...
		txn.KV(),
		sessiondata.NodeUserSessionDataOverride,
		"select ji.value,j.status from system.jobs j inner join system.job_info ji on j.id = ji.job_id where j.job_type = 'CHANGEFEED' and ji.info_key = 'legacy_payload' and j.id != $1",
		record.JobID,
	)
	if err != nil {
		return false, err
