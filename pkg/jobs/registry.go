...
// CreateAndStartJobTesting creates and asynchronously starts a job from record.
...
func (r *Registry) CreateAndStartJobTesting(
	...
	if err := r.db.Txn(ctx, func(ctx context.Context, txn *kv.Txn) (err error) {
		rj, err = r.CreateStartableJobWithTxn(ctx, rj, jobID, txn, record)
		return err
	}); err != nil {
		if rj != nil {
...
// *StartableJob outside the closure. The returned *StartableJob will be the
// same as the one passed in if the method is called with the same job ID and
// has already been initialized with a tracing span and registered. The point is
// to ensure that the tracing span is created and the job registered exactly
// once, if and only if the transaction commits. This is a fragile API.
