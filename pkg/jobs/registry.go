...
func (r *Registry) FindDuplicateChangefeedJob(
	...
	if txn == nil {
		return jobspb.InvalidJobID, errors.New("txn is required")
	}
	...
	row, err := txn.QueryRowEx(ctx, "changefeed-duplicate-check", txn.KV(), sessiondata.NodeUserSessionDataOverride,
		...
		where j.job_type = 'CHANGEFEED' 
			...
			and (j.created_by_type is NULL or j.created_by_type != 'crdb_schedule') 
			and ji.job_id != $1 
			and ji.info_key = $2 
			and ji.value = $3 
	...
	matchingJobID := jobspb.JobID(*row[0].(*tree.DInt))

	return matchingJobID, nil
}
