...
func (r *Registry) stepThroughStateMachine(
	...
	case StatusRunning:
		...
		if nonCancelableRetry := job.Payload().Noncancelable && !IsPermanentJobError(err); nonCancelableRetry ||
			...
			jm.ResumeRetryError.Inc(1)
			err = errors.Wrapf(err, "job %d: restarting in background", job.ID())
	...
	case StatusReverting:
		...
		// TODO(sajjad): Remove version-gate below and delete rest of the code in this condition.
		// It will be in the next commit in this PR.
		if true {
			// All reverting jobs are retried.
			jm.FailOrCancelRetryError.Inc(1)
			err = errors.Wrapf(err, "job %d: failed to revert, restarting in background", job.ID())
			return onExecutionFailed(err)
		}
