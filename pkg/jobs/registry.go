...
func batchJobInsertStmt(
	...
	if schemaVersion.AtLeast(clusterversion.V25_1_AddJobsColumns.Version()) {
		...
		if len(jobs[0].Payload().Statement) > 0 {
			columns = append(columns, `statement`)
			valueFns[`statement`] = func(job *Job) (interface{}, error) { return strings.Join(job.Payload().Statement, ";"), nil }
		}
	}
...
func (r *Registry) CreateJobWithTxn(
	...
	do := func(ctx context.Context, txn isql.Txn) error {
		...
		if v.AtLeast(clusterversion.V25_1_AddJobsColumns.Version()) {
			...
			if len(j.mu.payload.Statement) > 0 {
				cols = append(cols, "statement")
				vals = append(vals, strings.Join(j.mu.payload.Statement, ";"))
			}
		}
