	...
	stopper.RunWorker(context.Background(), func(ctx context.Context) {
		for {
			...
			case <-time.After(gcInterval):
				if err := r.cleanupOldJobs(ctx); err != nil {
...
func (r *Registry) cleanupOldJobs(ctx context.Context) error {
	old := timeutil.Now().Add(-1 * gcSetting.Get(&r.settings.SV))

	const stmt = `SELECT id, payload FROM system.jobs WHERE status IN ($1, $2, $3) AND created < $4 ORDER BY created LIMIT 100`
	rows, _ /* cols */, err := r.ex.Query(
		ctx, "gc-jobs", nil /* txn */, stmt, StatusFailed, StatusSucceeded, StatusCanceled, old,
	)
	if err != nil {
		return err
	}
	...
	oldMicros := timeutil.ToUnixMicros(old)
