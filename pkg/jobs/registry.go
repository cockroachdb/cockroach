...
func (r *Registry) CreateJobWithTxn(
	...
	numCols := 7
	cols := [8]string{"id", "status", "payload", "progress", "claim_session_id", "claim_instance_id", "job_type"}
	vals := [8]interface{}{jobID, StatusRunning, payloadBytes, progressBytes, s.ID().UnsafeBytes(), r.ID(), jobType.String()}
