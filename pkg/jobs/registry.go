...
func (r *Registry) createJobsStatsPollerIfNotExists(ctx context.Context) error {
	if r.knobs.DisableJobsMetricsPolling {
		return nil
	}

	for {
		const TxnName = "Jobs Stats Poller Txn"
		txn := r.db.NewTxn(ctx, TxnName)

		exists, err := JobExists(ctx, r.ex, txn, func(payload *jobspb.Payload) bool {
...
func (r *Registry) PollMetricsTask(ctx context.Context) error {
	...
	var err error
	updateMetrics := func(ctx2 context.Context, s sqlliveness.Session) {
		...
		for {
			...
			case <-lc.timer.C:
				lc.timer.Read = true
				if err = r.updatePausedMetrics(ctx2, s); err != nil {
					log.Errorf(ctx2, "failed to update paused metrics: %v", err)
					return
				}
				lc.onExecute()
			}
