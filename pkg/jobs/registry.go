...
var CancellationsUpdateLimitSetting = settings.RegisterIntSetting(
 ...
 WHERE claim_session_id <> $1
   ...
   AND NOT crdb_internal.sql_liveness_is_alive(claim_session_id) FETCH 
	...
	if err := stopper.RunAsyncTask(context.Background(), "jobs/cancel", func(ctx context.Context) {
		...
		cancelLoopTask(ctx)
		controller, cleanup := makeLoopController(r.settings, cancelIntervalSetting, r.knobs.CancelIntervalOverride)
		defer cleanup()
		for {
			select {
	...
	if err := stopper.RunAsyncTask(context.Background(), "jobs/gc", func(ctx context.Context) {
		...
		gcController, gcCleanup := makeLoopController(r.settings, gcIntervalSetting, r.knobs.GcIntervalOverride)
		defer gcCleanup()

		// When the retention time cluster setting is updated
		retentionTimeSetting.SetOnChange(&r.settings.SV, func(ctx context.Context) {
			select {
			case gcController.updated <- struct{}{}:
			default:
			}
		})

		// Retention duration of terminal job records
		retentionTime := func() time.Duration {
			if r.knobs.RetentionTimeOverride != nil {
				return *r.knobs.RetentionTimeOverride
			}
		...
		for {
			select {
			case <-gcController.updated:
				gcController.updateTimerWithBound(retentionTime())
			case <-stopper.ShouldQuiesce():
				return
			case <-gcController.timer.C:
				old := timeutil.Now().Add(-1 * retentionTime())
				if err := r.cleanupOldJobs(ctx, old); err != nil {
					log.Warningf(ctx, "error cleaning up old job records: %v", err)
				}
				gcController.onExecuteWithBound(retentionTime())
			}
		}
	}); err != nil {
		return err
	...
	return stopper.RunAsyncTask(context.Background(), "jobs/adopt", func(ctx context.Context) {
		ctx, cancel := stopper.WithCancelOnQuiesce(ctx)
		defer cancel()
		controller, cleanup := makeLoopController(r.settings, adoptIntervalSetting, r.knobs.AdoptIntervalOverride)
		defer cleanup()
		for {
			select {
			case <-controller.updated:
...
const jobsQuery = "SELECT id, payload, status, created FROM system.jobs " +
...
func (r *Registry) cleanupOldJobsPage(
	...
	it, err := r.ex.QueryIterator(ctx, "gc-jobs", nil /* txn */, jobsQuery, olderThan, minID, pageSize)
	if err != nil {
		return false, 0, err
	}
