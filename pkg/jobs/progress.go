...
// progress on (ie. when a custom func with other side-effects is not needed).
...
func (p *ProgressUpdateBatcher) Add(ctx context.Context, delta float32) error {
	...
	meaningfulProgress := p.completed-p.reported > progressFractionThreshold
	shouldReport := meaningfulProgress && p.lastReported.Add(progressTimeThreshold).Before(timeutil.Now())
