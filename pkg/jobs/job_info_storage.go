...
// InfoStorageWithTxn returns a new InfoStorage with the passed in job and txn.
func (j *Job) InfoStorageWithTxn(txn isql.Txn) InfoStorage {
	return InfoStorage{j: j, txn: txn}
}

// InfoStorageNoTxn returns a new InfoStorage with the passed in job but no txn.
func (j *Job) InfoStorageNoTxn() InfoStorage {
	return InfoStorage{j: j}
}
...
func (i InfoStorage) get(ctx context.Context, infoKey []byte) ([]byte, bool, error) {
	if i.txn == nil {
		...
		var exists bool
		var err error
		err = i.j.registry.internalDB.Txn(ctx, func(ctx context.Context, txn isql.Txn) error {
			i.txn = txn
			val, exists, err = i.get(ctx, infoKey)
			return err
		})
...
func (i InfoStorage) write(ctx context.Context, infoKey, value []byte) error {
	if i.txn == nil {
		return i.j.registry.internalDB.Txn(ctx, func(ctx context.Context, txn isql.Txn) error {
			i.txn = txn
	...
	if j.Session() != nil {
		...
		// TODO(during review): Do we want to allow this? CreateAdoptableJobWithTxn
		// writes to the jobs info table before the job is adopted and has an
		// associated sqlliveness session.
