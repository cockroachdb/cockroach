...
var (
	retainedProgressHistory = settings.RegisterIntSetting(
		settings.ApplicationLevel, "jobs.retained_progress_entries", "number of historical progress entreis to retain", 1000,
	)
	retainedMessageHistory = settings.RegisterIntSetting(
		settings.ApplicationLevel, "jobs.retained_messages", "number of historical messages of each kind to retain", 100,
	)
)
...
// StatusStorage reads and writes status rows and messages, i.e. short strings
// that are for *human* consumption. These are distinct from info rows, which
// are byte strings used by the code of the job itself to persist state that it
// will consume. Statuses and messages are stored in system.jobs_status and
// system.jobs_message, with status being a single row that is replaced by each
// change while message is a log to which one appends with historical entries
// retained up to confgiured limits.
...
// Sets writes a status message row, replacing the current one if it exists.
func (i StatusStorage) Set(ctx context.Context, txn isql.Txn, message string) error {
	...
	if err := i.Record(ctx, txn, "status", message); err != nil {
		return err
	}
...
func (i StatusStorage) Get(ctx context.Context, txn isql.Txn) (string, time.Time, error) {
	...
	row, err := txn.QueryRowEx(
		...
		"SELECT written, status FROM system.job_status WHERE job_id = $1 LIMIT 1",
		i,
	)
...
func (i StatusStorage) Record(ctx context.Context, txn isql.Txn, kind, message string) error {
	...
	// Prune old messages of the same kind to bound histoical data.
	if _, err := txn.ExecEx(
...
func (i StatusStorage) AllMessages(
	ctx context.Context, txn isql.Txn,
) (_ []JobMessage, retErr error) {
