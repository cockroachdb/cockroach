...
func (sj *StartableJob) Start(ctx context.Context) (err error) {
	...
	if sj.span != nil {
		defer sj.span.Finish()
	}

	return sj.registry.stopper.RunAsyncTask(ctx, sj.taskName(), func(ctx context.Context) {
...
// implies the job failed -- it just means that this registry encountered an error
...
// Calling this method prior to copmletion may return partial results.
func (sj *StartableJob) ReportExecutionResults(
	...
	// If the resumer does not implement reporter interface, return nothing.
