...
// makeCancelWhenDisabled arranges for scheduler enabled setting callback to cancel
// currently executing context.
func makeCancelWhenDisabled(sv *settings.Values) *syncCancelFunc {
	sf := &syncCancelFunc{sv: sv}
...
func (sf *syncCancelFunc) withCancelOnDisabled(
	...
	ctx, cancel := func() (context.Context, context.CancelFunc) {
		...
		if !schedulerEnabledSetting.Get(sf.sv) {
			cancel()
		}
...
func (s *jobScheduler) runDaemon(ctx context.Context, stopper *stop.Stopper) {
	_ = stopper.RunAsyncTask(ctx, "job-scheduler", func(ctx context.Context) {
		...
		whenDisabled := makeCancelWhenDisabled(&s.Settings.SV)
		...
		for timer := time.NewTimer(initialDelay); ; timer.Reset(
			...
			case <-timer.C:
				...
				if err := whenDisabled.withCancelOnDisabled(ctx, func(ctx context.Context) error {
