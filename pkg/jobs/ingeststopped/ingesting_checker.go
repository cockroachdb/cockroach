...
func WaitForNoIngestingNodes(
	...
	ctx, sp := tracing.ChildSpan(ctx, "ingeststopped")
	defer sp.Finish()
	...
	const maxIngestingCheckDelay = time.Minute * 5
	...
	for retries.Next() {
		...
		if timeutil.Since(lastStatusUpdate) > statusUpdateFrequency {
			if statusErr := job.NoTxn().RunningStatus(ctx, func(_ context.Context, _ jobspb.Details) (jobs.RunningStatus, error) {
				return jobs.RunningStatus(fmt.Sprintf("waiting for all nodes to finish writing before reverting: %s", err)), nil
			}); statusErr != nil {
				log.Warningf(ctx, "failed to update running status of job %d: %s", errors.Safe(job.ID()), statusErr)
			} else {
