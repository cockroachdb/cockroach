...
// TestStatementStorePrewarm verifies that prewarm populates the cache
// with previously persisted fingerprints, preventing duplicate inserts.
func TestStatementStorePrewarm(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)
	...
	// Phase 1: insert a fingerprint via a first store instance.
	store1 := statementstore.NewStatementStore(settings, nil)
	m1 := mon.NewUnlimitedMonitor(ctx, mon.Options{
		Name: mon.MakeName("test-statement-store-1"),
	})
	store1.SetInternalExecutor(db, m1)
	store1.Start(ctx, srv.Stopper())

	info := statementstore.StatementInfo{
		FingerprintID: appstatspb.StmtFingerprintID(777),
		Fingerprint:   "INSERT INTO _ VALUES (_)",
		Database:      "defaultdb",
		Summary:       "INSERT",
		ImplicitTxn:   true,
	}
	store1.PutStatement(ctx, info)
	store1.TestingFlushPending(ctx)
	...
	testutils.SucceedsSoon(t, func() error {
		row, err := db.Executor().QueryRowEx(ctx,
			...
			"SELECT count(*) FROM system.statements WHERE fingerprint = $1",
			info.Fingerprint,
		)
		if err != nil {
			return err
		...
		if count == 0 {
			return fmt.Errorf("fingerprint not yet persisted")
		}
		return nil
	})

	// Phase 2: create a new store which should prewarm from the table.
	store2 := statementstore.NewStatementStore(settings, nil)
	m2 := mon.NewUnlimitedMonitor(ctx, mon.Options{
		Name: mon.MakeName("test-statement-store-2"),
	})
	store2.SetInternalExecutor(db, m2)
	store2.Start(ctx, srv.Stopper())

	// Wait for the prewarm goroutine to finish.
	testutils.SucceedsSoon(t, func() error {
		if !store2.TestingIsCached(info.FingerprintID) {
			return fmt.Errorf("fingerprint not yet prewarmed in cache")
		}
		return nil
	})

	// PutStatement should be a no-op since the cache already has it.
	store2.PutStatement(ctx, info)
	require.Equal(t, 0, store2.TestingPendingCount(),
		"prewarmed fingerprint should not be re-enqueued")
}
