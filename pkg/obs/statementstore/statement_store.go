...
func (ss *StatementStore) addToCacheIfAbsent(
	...
	if ss.memAcc != nil {
		_ = ss.memAcc.Grow(ctx, cacheEntrySize)
	}
	return true
}
...
func (ss *StatementStore) Start(ctx context.Context, stopper *stop.Stopper) {
	...
	if err := stopper.RunAsyncTask(ctx,
		...
		log.Ops.Warningf(ctx,
			"failed to start statement store cache prewarm: %s", err)
	}
}
...
func (ss *StatementStore) prewarmCache(ctx context.Context) {
	...
	if err != nil {
		log.Ops.Warningf(ctx,
			"failed to prewarm statement cache: %s", err)
		return
	}
	defer func() {
		if err := it.Close(); err != nil {
			log.Ops.Warningf(ctx,
				"failed to close prewarm statement cache iterator: %s", err)
		}
	}()
	...
	for ok, err = it.Next(ctx); ok; ok, err = it.Next(ctx) {
		...
		if decErr != nil {
			log.Ops.Warningf(ctx,
				"failed to decode fingerprint ID during cache prewarm: %s",
				decErr)
			continue
		}
		ss.addToCacheIfAbsent(ctx, appstatspb.StmtFingerprintID(id))
		count++
	}
	if err != nil {
		log.Ops.Warningf(ctx,
