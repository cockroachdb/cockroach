...
func (s *SQLStore) Write(ctx context.Context, metrics []WritableMetric) error {
...
func (s *SQLStore) Get(ctx context.Context) ([]WritableMetric, error) {
	if !s.st.Version.IsActive(ctx, clusterversion.V26_2_AddSystemClusterMetricsTable) {
		return nil, nil
	}

	var result []WritableMetric
