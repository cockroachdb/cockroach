...
func TestWriter_Delete(t *testing.T) {
	...
	t.Run("scalar delete then inc before flush", func(t *testing.T) {
		...
		// Inc after Delete: cancels the deletion, value should be written.
		stored, ok := env.store.get("c")
		require.True(t, ok, "metric should be written after delete-then-inc")
		require.Equal(t, int64(5), stored.StoredValue)
		require.Equal(t, int64(1), env.writerMetrics.MetricsWritten.Count())
		require.Equal(t, int64(0), env.writerMetrics.MetricsDeleted.Count())
	})
	...
	t.Run("scalar delete then update gauge", func(t *testing.T) {
		...
		// Update after Delete cancels the deletion.
		stored, ok := env.store.get("g")
		require.True(t, ok, "gauge should be written after delete-then-update")
		require.Equal(t, int64(42), stored.StoredValue)
		require.Equal(t, int64(0), env.writerMetrics.MetricsDeleted.Count())
	})
	...
	t.Run("scalar inc after flushed delete", func(t *testing.T) {
		env := newTestEnv()
		c := cmmetrics.NewCounter(metric.Metadata{Name: "c"})
		env.writer.AddMetric(c)
		...
		// Delete and flush — row removed from store.
		c.Delete()
		env.writer.Flush(env.ctx)
		_, ok = env.store.get("c")
		require.False(t, ok)

		// Inc after the delete has been flushed — metric is still
		// tracked, so the new value should be written.
		c.Inc(3)
		env.writer.Flush(env.ctx)
		stored, ok := env.store.get("c")
		require.True(t, ok, "inc after flushed delete should be written")
		require.Equal(t, int64(3), stored.StoredValue)
	})
