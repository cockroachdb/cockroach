...
func findMetric(metrics []WritableMetric, name string) WritableMetric {
...
func TestSQLStore_Write(t *testing.T) {
	...
	t.Run("counter", func(t *testing.T) {
		...
		require.NoError(t, env.store.Write(env.ctx, []WritableMetric{c}))
	...
	t.Run("gauge", func(t *testing.T) {
		...
		require.NoError(t, env.store.Write(env.ctx, []WritableMetric{g}))
	...
	t.Run("counter accumulates on upsert", func(t *testing.T) {
		env := newSQLStoreTestEnv(t)
		c := NewCounter(metric.Metadata{Name: "test.counter.accum"})
		c.Inc(10)
	...
	t.Run("gauge replaces on upsert", func(t *testing.T) {
		env := newSQLStoreTestEnv(t)
		g := NewGauge(metric.Metadata{Name: "test.gauge.replace"})
		g.Update(100)
	...
	t.Run("multiple metrics", func(t *testing.T) {
		...
		require.NoError(t, env.store.Write(env.ctx, []WritableMetric{c, g}))
