...
func (cmr ClusterMetricRow) ToMetric() (metric.Iterable, error) {
	if len(cmr.Labels) == 0 {
		md, ok := clustermetrics.GetClusterMetricMetadata(cmr.Name)
		if !ok {
			return nil, errors.Newf("no metadata found for metric %s", cmr.Name)
		}
		return cmr.toMetric(md)
	}
	md, ok := clustermetrics.GetLabeledClusterMetricMetadata(cmr.Name)
	if !ok {
		return nil, errors.Newf("no metadata found for labeled metric %s", cmr.Name)
	}
	return cmr.toExportedMetric(md.Metadata, md.Labels)
}

func (cmr ClusterMetricRow) toExportedMetric(
...
func (d *RowDecoder) DecodeRow(kv roachpb.KeyValue) (_ ClusterMetricRow, tombstone bool, _ error) {
	...
	// Map datums at indices 1-9 to struct fields.
	// Index 10 is the virtual shard column (always DNull) — skip it.
