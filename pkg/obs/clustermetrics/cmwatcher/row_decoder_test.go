...
func TestRowDecoder(t *testing.T) {
	...
	r.Exec(t, `INSERT INTO system.cluster_metrics (id, name, labels, type, value, node_id, unit, help_text, measurement)
		VALUES (1, 'test.metric', '{"store": "1"}', 'gauge', 42, 1, 0, 'A test metric', 'count')`)
	r.Exec(t, `INSERT INTO system.cluster_metrics (id, name, labels, type, value, node_id, unit, help_text, measurement)
		VALUES (2, 'test.null_value', '{}', 'counter', NULL, 2, 1, 'Metric with null value', 'bytes')`)
	...
	var foundMetric, foundNullValue bool
...
func TestToMetric(t *testing.T) {
	...
	cleanupLabeledGauge := clustermetrics.TestingRegisterLabeledClusterMetric(
		"test.labeled.gauge",
		clustermetrics.LabeledMetricMetadata{Metadata: labeledGaugeMeta, Labels: []string{"store"}},
	)
	defer cleanupLabeledGauge()
	cleanupLabeledCounter := clustermetrics.TestingRegisterLabeledClusterMetric(
		"test.labeled.counter",
		clustermetrics.LabeledMetricMetadata{Metadata: labeledCounterMeta, Labels: []string{"node"}},
	)
