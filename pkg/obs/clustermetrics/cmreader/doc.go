...
//     directly; only the updater may add or remove metrics.
//
//   - updater: subscribes to the system.cluster_metrics table via a
...
//     inserted or updated, the updater creates or updates the
...
// clustermetrics.RegisterClusterMetric, which populates the
// registeredClusterMetrics map. Labeled metrics must be registered
// via clustermetrics.RegisterLabeledClusterMetric, which populates
// the registeredLabeledClusterMetrics map. Both registration
// functions should be called in init() so that metadata is available
// before the updater starts consuming rangefeed events. If a row
// arrives whose name is not found in the appropriate registry, the
// updater logs an error and skips the row.
package cmreader
