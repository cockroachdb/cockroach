...
// TestUpdaterIntegration starts a real test server, wires the updater into the
...
func TestUpdaterIntegration(t *testing.T) {
	...
	defer clustermetrics.TestingRegisterLabeledClusterMetric(
		"test.gauge_labeled", clustermetrics.LabeledMetricMetadata{
	...
	// Insert rows before starting the updater so the initial scan picks
	// them up via OnRefresh.
	r.Exec(t, `INSERT INTO system.cluster_metrics
		(id, name, labels, type, value, node_id, unit, help_text, measurement)
		VALUES (100, 'test.gauge_labeled', '{"store": "1"}',
			'gauge', 42, 1, 0, 'A test gauge', 'count')`)
	r.Exec(t, `INSERT INTO system.cluster_metrics
		(id, name, labels, type, value, node_id, unit, help_text, measurement)
		VALUES (200, 'test.counter', '{}', 'counter', 10, 1, 0, 'A test counter', 'ops')`)
	r.Exec(t, `INSERT INTO system.cluster_metrics
		(id, name, labels, type, value, node_id, unit, help_text, measurement)
		VALUES (300, 'test.scalar', '{}', 'gauge', 50, 1, 0,
			'A scalar gauge for value verification', 'count')`)

	// Start the updater via the production path, which wires metrics into
	...
	r.Exec(t, `UPSERT INTO system.cluster_metrics
		(id, name, labels, type, value, node_id, unit, help_text, measurement)
		VALUES (300, 'test.scalar', '{}', 'gauge', 123, 1, 0,
			'A scalar gauge for value verification', 'count')`)
	...
	r.Exec(t, `INSERT INTO system.cluster_metrics
		(id, name, labels, type, value, node_id, unit, help_text, measurement)
		VALUES (101, 'test.gauge_labeled', '{"store": "2"}',
			'gauge', 77, 1, 0, 'A test gauge', 'count')`)
	...
	r.Exec(t, `UPSERT INTO system.cluster_metrics
		(id, name, labels, type, value, node_id, unit, help_text, measurement)
		VALUES (100, 'test.gauge_labeled', '{"store": "1"}',
			'gauge', 99, 1, 0, 'A test gauge', 'count')`)
	...
	r.Exec(t, `INSERT INTO system.cluster_metrics
		(id, name, labels, type, value, node_id, unit, help_text, measurement)
		VALUES (400, 'test.newgauge', '{}', 'gauge', 55, 1, 0,
			'A new gauge added after initial scan', 'count')`)
