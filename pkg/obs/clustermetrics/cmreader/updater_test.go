...
// makeTestUpdater creates an updater with a fresh registry for unit testing.
// It does not create a watcher or connect to any rangefeed.
func makeTestUpdater() (*updater, *registry) {
	reg := &registry{metric.NewRegistry()}
	u := &updater{registry: reg}
...
func TestUpdateMetricLocked(t *testing.T) {
	...
	tests := []struct {
		...
		verify func(t *testing.T, u *updater, reg *registry)
	}{{
		name: "new gauge",
		setup: func() func() {
		...
		verify: func(t *testing.T, u *updater, _ *registry) {
			g := u.mu.trackedMetrics["test.gauge"].(*metric.Gauge)
		...
		setup: func() func() {
			return clustermetrics.TestingRegisterLabeledClusterMetric(
				"test.gaugevec", clustermetrics.LabeledMetricMetadata{
		...
		setup: func() func() {
			return clustermetrics.TestingRegisterLabeledClusterMetric(
				"test.countervec", clustermetrics.LabeledMetricMetadata{
					Metadata: metric.Metadata{
...
func TestDeleteMetricLocked(t *testing.T) {
	...
	tests := []struct {
		...
		setup: func() func() {
			return clustermetrics.TestingRegisterLabeledClusterMetric(
				"test.vec", clustermetrics.LabeledMetricMetadata{
					Metadata: metric.Metadata{
						Name: "test.vec",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			u.deleteMetricLocked(ctx, tt.deleteRow)
			u.mu.Unlock()
