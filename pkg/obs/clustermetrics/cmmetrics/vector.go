...
// given metadata and label names.
func NewWriteStopwatchVec(metadata metric.Metadata, labelNames ...string) *WriteStopwatchVec {
	ensureLabeledClusterMetricRegistered(metadata.Name, metadata, labelNames)
...
func (v *WriteStopwatchVec) SetStartTime(labels map[string]string) {
	v.GaugeVec.Update(labels, timeutil.Now().Unix())
}
