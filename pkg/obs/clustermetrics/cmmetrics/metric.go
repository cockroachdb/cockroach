...
// metadata. The metadata is automatically registered so that the
// cmreader can materialize this metric from the rangefeed.
func NewWriteStopwatch(metadata metric.Metadata) *WriteStopwatch {
	ensureClusterMetricRegistered(metadata.Name, metadata)
	return &WriteStopwatch{Gauge: metric.NewGauge(metadata)}
}

// SetStartTime records the current time as the start time and marks
// dirty.
func (s *WriteStopwatch) SetStartTime() {
	s.Gauge.Update(timeutil.Now().Unix())
	s.dirty.Store(true)
}
...
// "STOPWATCH" type string (via MetricTypeString), the metric itself
// behaves as a gauge for prometheus and TSDB purposes.
