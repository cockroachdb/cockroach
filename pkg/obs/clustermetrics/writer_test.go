...
func TestWriter_Flush(t *testing.T) {
	...
	t.Run("single flush", func(t *testing.T) {
		tests := []struct {
			...
			name: "counter resets after flush",
			setup: func(env *testEnv) {
				c := NewCounter(metric.Metadata{Name: "c"})
				env.registry.AddMetric(c)
				c.Inc(100)
			},
			wantStored: map[string]int64{"c": 100},
			postCheck: func(t *testing.T, env *testEnv) {
				env.registry.Each(func(_ string, v interface{}) {
					if c, ok := v.(*Counter); ok {
						require.Equal(t, int64(0), c.Count(), "counter should reset after flush")
					}
				})
			},
		}, {
			...
			name: "gauge retains value after flush",
			setup: func(env *testEnv) {
				g := NewGauge(metric.Metadata{Name: "g"})
				env.registry.AddMetric(g)
				g.Update(42)
			},
			wantStored: map[string]int64{"g": 42},
			postCheck: func(t *testing.T, env *testEnv) {
				env.registry.Each(func(_ string, v interface{}) {
					if g, ok := v.(*Gauge); ok {
			...
			setup: func(env *testEnv) {
				...
				env.registry.AddMetric(c1)
				env.registry.AddMetric(c2)
				c1.Inc(10)
				c2.Inc(20)
			},
			...
			setup: func(env *testEnv) {
				...
				env.registry.AddMetric(g1)
				env.registry.AddMetric(g2)
				g1.Update(100)
				g2.Update(200)
			},
			...
			setup: func(env *testEnv) {
				...
				env.registry.AddMetricStruct(&m)
				m.Count.Inc(15)
				m.Status.Update(200)
			},
