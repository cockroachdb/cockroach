...
// It accepts and buffers a stream of TypedEvents and "flushes" to
// the underlying flush goroutine based on various flush triggers, which
// in turn invokes the Processor(s) for each event based on event type.
//
// It's designed to never block callers.
...
func (bs *asyncProcessorRouter) register(eventType log.EventType, processor Processor) {
	bs.rwmu.Lock()
	...
	bs.rwmu.routes[eventType] = append(container, processor)
}
...
// underlying Processor for processing based on its event type.
...
func (bs *asyncProcessorRouter) flushAsyncLocked(ctx context.Context) {
