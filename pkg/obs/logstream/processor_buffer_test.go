...
func TestTriggerLen(t *testing.T) {
	...
	buffer, mock := getMockProcessorBuffer(ctx, t, stopper, noMaxStaleness, 1 /* triggerLen */, noMaxLen)
...
func TestMaxStaleness(t *testing.T) {
	...
	stopper := stop.NewStopper()
	defer stopper.Stop(ctx)
	buffer, mock := getMockProcessorBuffer(ctx, t, stopper, time.Second, noTriggerLen, noMaxLen)
...
func TestMaxLen(t *testing.T) {
	...
	ctx := context.Background()
	stopper := stop.NewStopper()
	buffer, mock := getMockProcessorBuffer(ctx, t, stopper, noMaxStaleness, noTriggerLen, 5)
...
func TestMultipleFlushes(t *testing.T) {
	...
	stopper := stop.NewStopper()
	defer stopper.Stop(ctx)
	buffer, mock := getMockProcessorBuffer(ctx, t, stopper, noMaxStaleness, 2 /*triggerLen*/, noMaxLen)
...
func TestCtxDoneFlushesBeforeExit(t *testing.T) {
	...
	stopper := stop.NewStopper()
	defer stopper.Stop(ctx)
	buffer, mock := getMockProcessorBuffer(ctx, t, stopper, noMaxStaleness, noTriggerLen, noMaxLen)
...
func TestBlockedFlush(t *testing.T) {
	...
	stopper := stop.NewStopper()
	defer stopper.Stop(ctx)
	buffer, mock := getMockProcessorBuffer(ctx, t, stopper, noMaxStaleness, 1, noMaxLen)
	...
	flush1C := make(chan struct{})
	firstEvent := "element1"
	mock.EXPECT().Process(gomock.Any(), firstEvent).Do(addArgs(func() {
