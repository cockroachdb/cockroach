...
func MakeRelease(platform Platform, opts BuildOptions, pkgDir string) error {
	...
	if platform == PlatformLinux || platform == PlatformLinuxFIPS {
		...
		if platfform == PlatformLinuxFIPS && !strings.Contains(string(stdoutBytes), "FIPS") {
			log.Fatalf("`go nm tool` does not contain `FIPS` in its output")
		}
		if platfform == PlatformLinux && strings.Contains(string(stdoutBytes), "FIPS") {
			log.Fatalf("`go nm tool` contains `FIPS` in its output")
		}
	}
	return nil
}
