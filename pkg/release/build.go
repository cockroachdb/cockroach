...
func MakeRelease(platform Platform, opts BuildOptions, pkgDir string) error {
	...
	if platform == PlatformLinux || platform == PlatformLinuxFIPS {
		...
		if out := string(stdoutBytes); out != "" {
			if platform == PlatformLinuxFIPS && !strings.Contains(string(stdoutBytes), "goland-fips") {
				log.Print("`go nm tool` does not contain `goland-fips` in its output")
				log.Fatalf("%s %s: out=%s", cmd.Env, cmd.Args, out)
			}
			if platform == PlatformLinux && strings.Contains(string(stdoutBytes), "goland-fips") {
				log.Print("`go nm tool` contains `goland-fips` in its output")
				log.Fatalf("%s %s: out=%s", cmd.Env, cmd.Args, out)
			}
		}
	}
