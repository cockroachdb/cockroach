...
func (aw *accountingWriter) Close() error {
	// NB: We only record bytes actually written (according to the
	// underlying writer) in aw.count. We wait after recording the
	// bytes since these bytes have already been consumed.
...
func (aw *accountingWriter) maybeWaitForRUs() error {
	if aw.limit > 0 && aw.count >= aw.limit {
		old := aw.count
		aw.count = 0
		// We record the success first since these bytes have
		// already been consumed.
		aw.recorder.ExternalIOWriteSuccess(aw.ctx, old)
		if err := aw.recorder.ExternalIOWriteWait(aw.ctx, old); err != nil {
			return err
		}
	}
	return nil
...
func WriteFileAccounted(
	...
	if err := recorder.ExternalIOWriteWait(ctx, bytesToWrite); err != nil {
		return err
	}
