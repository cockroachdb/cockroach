...
type TenantSideExternalIORecorder interface {
	...
	// ExternalIOWriteFailure returns RUs to the pool in the case
	// of a Write that failed and wrote fewer bytes than
	// expected. The RUs for the used number of bytes is recorded,
	// the RUs for the unused bytes are returned to the pool.
	//
	// If the context (or a parent context) was created using
	// WithTenantCostControlExemption, the method is a no-op.
	ExternalIOWriteFailure(ctx context.Context, used int64, returned int64)

	// ExternalIOWriteSuccess records the usage of RUs associated
	// with a write of the given size. This function does not
	// wait.
	//
	// If the context (or a parent context) was created using
	// WithTenantCostControlExemption, the method is a no-op.
	...
	// ExternalIOReadWait records the number of bytes read from an
	// external service, waiting for RUs if necessary.
	//
	// If the context (or a parent context) was created using
	// WithTenantCostControlExemption, the method is a no-op.
