...
type EstimatedCPUModel struct {
	...
	// write request in a batch, beyond the second. As the batch size increases,
...
var DefaultEstimatedCPUModel = EstimatedCPUModel{
	...
	WriteBatchCost: struct {
		...
		RatePerNode: []float64{200, 400, 800, 1600, 3200, 6400, 12800},
		CPUPerBatch: []EstimatedCPU{
			1.0 / 950, 1.0 / 1100, 1.0 / 1300, 1.0 / 1700, 1.0 / 2200, 1.0 / 2700, 1.0 / 3150,
		},
	},
	WriteRequestCost: struct {
		BatchSize     []float64
...
func (m *EstimatedCPUModel) BatchCost(
	...
	if bi.WriteCount > 0 {
		...
		// Add cost for additional requests in the batch, beyond the second.
		if bi.WriteCount > 2 {
			ecpuPerRequest := m.lookupCost(
