...
func (u Entry) String() string {
	...
	for i, capID := range CapabilityIDs {
		...
		builder.WriteByte(':')
		builder.WriteString(u.TenantCapabilities.Cap(capID).Get().String())
	}
	builder.WriteByte('}')
	return fmt.Sprintf("ten=%v cap=%v", u.TenantID, builder.String())
}
...
var stringToCapabilityIDMap = stringerutil.StringToEnumValueMap[CapabilityID](
...
var CapabilityIDs = stringerutil.EnumValues[CapabilityID](
	_CapabilityID_index[:LastCapabilityID],
	1, /* offset */
)
