...
func (a *Authorizer) HasBlobStorageCapability(ctx context.Context, tenID roachpb.TenantID) error {
	if a.elideCapabilityChecks(ctx, tenID) {
		return nil
	}
	cp, found := a.capabilitiesReader.GetCapabilities(tenID)
	...
	if !found || !cp.GetBool(tenantcapabilities.CanUseBlobStorage) {
		return errors.Newf("client tenant does not have capability to use blob storage")
	}
	return nil
}
