...
// IsNotRateLimited returns true if the tenant is not subject to rate limiting.
func (a *Authorizer) IsNotRateLimited(ctx context.Context, tenID roachpb.TenantID) bool {
	if tenID.IsSystem() {
		return true
	}
	if a.capabilitiesReader == nil {
