...
func (w *Watcher) GetInfo(
	id roachpb.TenantID,
) (mtinfopb.SQLInfo, *tenantcapabilitiespb.TenantCapabilities, <-chan struct{}, bool) {
	cp := w.getInternal(id)

	if cp.Entry != nil {
		return mtinfopb.SQLInfo{
			ID:          id.ToUint64(),
			Name:        cp.Name,
			ServiceMode: cp.ServiceMode,
			DataState:   cp.DataState,
		}, cp.TenantCapabilities, cp.changeCh, true
	}
	return mtinfopb.SQLInfo{}, nil, cp.changeCh, false
}
...
func (w *Watcher) handleUpdate(ctx context.Context, u rangefeedcache.Update) {
	...
	default:
		logcrash.ReportOrPanic(ctx, &w.st.SV, "unknown update type: %v", u.Type)
		log.Warningf(ctx, "unknown update type: %v", u.Type)
	}
}
