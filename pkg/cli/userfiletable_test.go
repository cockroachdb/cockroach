...
func Example_userfile_upload_recursive() {
	if err := runExampleUserfileUploadRecursive(); err != nil {
		fmt.Println(err)
	}
}

func runExampleUserfileUploadRecursive() (err error) {
	c := NewCLITest(TestCLIParams{})
	defer c.Cleanup()
	...
	defer func() {
		err = errors.CombineErrors(err, os.RemoveAll(tmpDir))
	}()
	testDir := filepath.Join(tmpDir, "testdir/")
	if err = os.MkdirAll(filepath.Join(testDir, "d1/d11/d111/d1111"), 0755); err != nil {
		return err
	}
	if err = os.MkdirAll(filepath.Join(testDir, "d2/d21"), 0755); err != nil {
		return err
	}
	...
	for _, relPath := range filesRelativePaths {
		if err := ioutil.WriteFile(filepath.Join(testDir, relPath), []byte("content"), 0666); err != nil {
			return err
		}
	}

	c.RunWithArgs([]string{"userfile", "upload", "-r", testDir, "foo"})
	c.RunWithArgs([]string{"userfile", "upload", "-r", testDir, "/foo/bar/"})
	c.RunWithArgs([]string{"userfile", "upload", "-r", testDir, "foo/baz/"})
	// No destination specified, so we use the filepath.Base() of the source,
	// i.e. "testdir", as the "directory" name.
	c.RunWithArgs([]string{"userfile", "upload", "-r", testDir})
	c.RunWithArgs([]string{"userfile", "upload", "-r", testDir, "foo/../bar"})
	c.RunWithArgs([]string{"userfile", "upload", "-r", testDir, "foo/./bar"})
	c.RunWithArgs([]string{"userfile", "upload", "-r", "/dir/does/not/exist", "foo/foo"})
	c.RunWithArgs([]string{"userfile", "upload", "-r", testDir, "/foo/À"})
	// Test fully qualified URI specifying db.schema.tablename_prefix.
	c.RunWithArgs([]string{"userfile", "upload", "-r", testDir, "userfile://defaultdb.public.foo/someDir"})
	// Test URI with no db.schema.tablename_prefix.
	c.RunWithArgs([]string{"userfile", "upload", "-r", testDir, "userfile:///someOtherDir"})
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, f := range files {
				...
				destinationFileURI := dstDir + "/" + f.relativePath
				...
				if tc.name == "destination-not-full-URI" {
					destinationFileURI = constructUserfileDestinationURI("",
						filepath.Join(dstDir, f.relativePath), security.RootUserName())
				}
