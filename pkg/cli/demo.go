...
func runDemo(cmd *cobra.Command, gen workload.Generator) error {
	...
	// If geo-partition-replicas is requested, make sure the workload is MovR and has a Partitioning step.
	if demoCtx.geoPartitionedReplicas {
		if gen.Meta().Name != "movr" {
			return errors.New("--geo-partitioned-replicas is only defined on the MovR dataset.")
		}
