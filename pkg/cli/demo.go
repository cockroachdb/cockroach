...
// getServerURL generates a connection URL for the given server.
func (c *transientCluster) getServerURL(
	nodeID roachpb.NodeID, gen workload.Generator, includeAppName bool,
) (url.URL, error) {
	return url.URL{}, nil
}
...
func (c *transientCluster) listDemoNodes(w io.Writer) {
	numNodesLive := 0
	hasMultipleNodes := len(c.servers) > 1
