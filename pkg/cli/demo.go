...
func setupTransientCluster(
	...
	// allServsReadyCh is used to account for all servers successfully called ReadyFn.
	allServsReadyCh := make(chan struct{}, demoCtx.nodes)
	// errCh is used to catch all errors when initializing servers in parallel.
	...
	for i := 0; i < demoCtx.nodes; i++ {
		...
		servReadyFnCh := make(chan struct{}, 1)
		serv.Cfg.ReadyFn = func(_ bool) {
			servReadyFnCh <- struct{}{}
		}
