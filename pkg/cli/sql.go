...
type cliState struct {
	...
	// partialLines is the array of lines accumulated so far in a multi-line entry.
	// This may be arbitrarily rewritten if syntax checking is enabled.
	partialLines []string

	// partialStmtsLen grows larger than zero whenever 1) syntax
	// checking is enabled and 2) multi-statement (as opposed to
	...
	// concatLines is the concatenation of partialLines, computed just before
	// doRunStatement().
	concatLines string
...
func (c *cliState) prepareStartLine(nextState cliStateEnum) cliStateEnum {
...
func (c *cliState) prepareContinueLine(nextState cliStateEnum) cliStateEnum {
	c.atEOF = false
...
func (c *cliState) doProcessCliCmd(loopState, nextState cliStateEnum) cliStateEnum {
...
func runInteractive(conn *sqlConn, config *readline.Config) (exitErr error) {
	...
	for {
		...
		case cliStartLine:
			state = c.prepareStartLine(cliReadLine)

		case cliContinueLine:
			state = c.prepareContinueLine(cliReadLine)
		...
		case cliDecidePath:
			if len(c.partialLines) == 0 {
				state = cliProcessFirstLine
			} else if isInteractive {
				// In interactive mode, we allow client-side commands to be
				// issued on intermediate lines.
				state = cliProcessCliCmd
			} else {
				// Neither interactive nor at start, continue with processing.
				state = cliPrepareStatementLine
			}

		case cliProcessFirstLine:
			state = c.doProcessFirstLine(cliReadLine, cliProcessCliCmd)

		case cliProcessCliCmd:
			state = c.doProcessCliCmd(cliReadLine, cliPrepareStatementLine)
