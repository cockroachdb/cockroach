...
func (u urlParser) Set(v string) error {
	...
	// makeClientConnURL().
	// If a client command does not use makeClientConnURL() it also
	// will not accept --user and so any attempt to set either user
	// or password here will produce an error.
	if parsedURL.User != nil {
		f := u.cmd.Flags().Lookup(cliflags.User.Name)
		if f == nil {
			return fmt.Errorf("command %q does not accept user/password details", u.cmd.Name())
		}
	...
	if parsedURL.Path != "" {
		...
		if f == nil {
			return fmt.Errorf("command %q does not accept a database name", u.cmd.Name())
		}
	...
	if parsedURL.RawQuery != "" {
		...
		fl := u.cmd.Flags()
		hasInsecure := fl.Lookup(cliflags.ClientInsecure.Name) != nil

		switch sslMode := options.Get("sslmode"); sslMode {
		case "disable":
			if !hasInsecure {
				return fmt.Errorf("command %q does not support insecure connections", u.cmd.Name())
			}
