...
func (u urlParser) setInternal(v string, warn bool) error {
	...
	// --host / --port.
	if net, host, port := parsedURL.GetNetworking(); net == pgurl.ProtoTCP {
		if host != "" {
	...
	// If a database is specified, forward it to --database.
	...
	if tlsUsed && tlsMode == pgurl.TLSUnspecified {
		// The sslmode argument was not specified.
		...
		if flInsecure.Changed {
			...
			// No --insecure specified. For strict (non-SQL) commands,
			// we go for maximum security.
			tlsMode = pgurl.TLSVerifyFull
