...
func runStartSQL(cmd *cobra.Command, args []string) error {
	...
	return runStartInternal(cmd, serverType, initConfig, newTenantServerController, nil /* maybeRunInitialSQL */)
}

func newTenantServerController(
	ctx context.Context, serverCfg server.Config, stopper *stop.Stopper,
) (serverStartupInterface, error) {
	return server.StartTenant(
		ctx,
		stopper,
