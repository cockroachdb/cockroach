	...
	if strings.Contains(schema, "REGIONS = ") {
		...
		for _, line := range strings.Split(schema, "\n") {
			...
			if strings.HasPrefix(line, "CREATE DATABASE") &&
				...
				//
				// Note that at the time of this writing, the CREATE DATABASE
				// stmt in the bundle comes from the 'create_statement' column
				// of the 'crdb_internal.databases` vtable, and there all
				// multi-region enabled DBs will have the survival goal set, so
				// we always expect SURVIVE word right after REGIONS.
