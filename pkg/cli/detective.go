...
var debugDetectiveCmd = &cobra.Command{
	Use:   "detective [command]",
	Short: "run a cockroach debug detective tool command",
	Long: `
The debug detective is a suite of tools for debugging and manipulating statement
bundles created with EXPLAIN ANALYZE (BUNDLE).
`,
}

var detectiveRecreateCmd = &cobra.Command{
...
func init() {
	detectiveRecreateCmd.RunE = MaybeDecorateGRPCError(func(cmd *cobra.Command, args []string) error {
		return runDetectiveRecreate(cmd, args)
	})
}
...
func loadStatementBundle(zipdir string) (*statementBundle, error) {
	...
	return ret, filepath.WalkDir(zipdir, func(path string, d fs.DirEntry, err error) error {
		if d.IsDir() {
			return nil
		}
...
func runDetectiveRecreate(cmd *cobra.Command, args []string) error {
	zipdir := args[0]
