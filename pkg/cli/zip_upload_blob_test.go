...
func (m *mockBlobUploader) Upload(_ context.Context, srcPath string) (string, int64, error) {
	m.uploadCount++
	if m.uploadErr != nil {
	...
	m.content = data
	return "gs://test-bucket/test-prefix/" + filepath.Base(srcPath), int64(len(data)), nil
}
...
// funcBlobUploader varies behaviour per call (e.g. fail-then-succeed).
type funcBlobUploader struct {
	uploadFn func(ctx context.Context, srcPath string) (string, int64, error)
	closeFn  func() error
}

func (f *funcBlobUploader) Upload(ctx context.Context, srcPath string) (string, int64, error) {
