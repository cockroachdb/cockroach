...
func runGetZone(cmd *cobra.Command, args []string) error {
	...
	conn, err := getPasswordAndMakeSQLClient("cockroach zone get")
	if err != nil {
		return err
	}
	defer conn.Close()
...
func runLsZones(cmd *cobra.Command, args []string) error {
	...
	conn, err := getPasswordAndMakeSQLClient("cockroach zone ls")
	if err != nil {
		return err
	}
	defer conn.Close()
...
func runRmZone(cmd *cobra.Command, args []string) error {
	...
	conn, err := getPasswordAndMakeSQLClient("cockroach zone rm")
	if err != nil {
		return err
	}
	defer conn.Close()
...
func runSetZone(cmd *cobra.Command, args []string) error {
	...
	conn, err := getPasswordAndMakeSQLClient("cockroach zone set")
	if err != nil {
		return err
	}
	defer conn.Close()
