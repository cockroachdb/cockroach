...
func (c *transientCluster) Start(
	...
	// 8. Start multitenant SQL servers.
	...
	if c.demoCtx.Multitenant {
		...
		for i := 0; i < c.demoCtx.NumNodes; i++ {
			...
			if err != nil {
				c.warnLog(phaseCtx, "failed starting tenant %d", i, err)
			}
			...
			if !c.demoCtx.Insecure {
				...
				_, err = ie.Exec(ctx, "tenant-password", nil, "CREATE USER $1 WITH PASSWORD $2", demoUsername, demoPassword)
				if err != nil {
					return err
				}
...
func (demoCtx *Context) generateCerts(certsDir string) (err error) {
	...
	if demoCtx.Multitenant {
		tenantCAKeyPath := filepath.Join(certsDir, security.EmbeddedTenantClientCAKey)
		...
		for i := 0; i < demoCtx.NumNodes; i++ {
			...
			pair, err := security.CreateTenantClientPair(
				certsDir,
				tenantCAKeyPath,
				demoCtx.DefaultKeySize,
				demoCtx.DefaultCertLifetime,
			...
			if err := security.WriteTenantClientPair(certsDir, pair, false /* overwrite */); err != nil {
				return err
			}
		}
	}
	...
	if gen != nil {
		...
		if c.demoCtx.RunWorkload {
			...
			for i := range c.servers {
				sqlURL, err := c.getNetworkURLForServer(ctx, i, true, false)
				if err != nil {
					return err
				}
...
func (c *transientCluster) ListDemoNodes(w, ew io.Writer, justOne bool) {
	...
	for i, s := range c.servers {
		...
		netURL, err := c.getNetworkURLForServer(context.Background(), i, false, false)
		if err != nil {
	...
	if c.demoCtx.Multitenant {
		for i := range c.servers {
			...
			tenantURL, err := c.getNetworkURLForServer(context.Background(), i, false, true)
			if err != nil {
