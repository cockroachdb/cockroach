...
func TestExpandEnvironmentVariables(t *testing.T) {
	...
	require.NoError(t, os.Unsetenv("EXPAND_ABSENT_VAR"))
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			vars:           []string{"EXPAND_ABSENT_VAR"},
			expectedErrMsg: `variable "EXPAND_ABSENT_VAR" is not defined in environment`,
		},
		{
			in:             "${EXPAND_ABSENT_VAR}",
			vars:           []string{"EXPAND_ABSENT_VAR"},
		...
		{
			in:             "$HOST_IP",
			expectedErrMsg: `unknown variable "HOST_IP" used in configuration`,
		},
		{
			in:             "${HOST_IP}",
			expectedErrMsg: `unknown variable "HOST_IP" used in configuration`,
