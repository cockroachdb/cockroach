...
var doctorExamineCmd = &cobra.Command{
	...
	Long: `
Run doctor tool to examine system tables and perform validation checks.
System tables are queried either from a live cluster or from a directory created by unzipping debug.zip.
`,
	Args: cobra.MinimumNArgs(1),
...
var doctorRecreateCmd = &cobra.Command{
	...
	Long: `
Run doctor tool to examine system tables and generate SQL statements that,
when run on an empty cluster, recreate that state as closely as possible.
System tables are queried either from a live cluster or from a directory created by unzipping debug.zip.
`,
	Args: cobra.MinimumNArgs(1),
	PersistentPreRun: func(cmd *cobra.Command, args []string) {
