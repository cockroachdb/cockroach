...
func fromZipDir(
	...
	if err := tableMap(descFile, func(row string) error {
		...
		if err != nil {
			return errors.Wrapf(err, "failed to parse descriptor id %s: %v", fields[0])
		}
	...
	if err := tableMap(jobsFile, func(row string) error {
		...
		if err != nil {
			return errors.Wrapf(err, "failed to parse job id %s: %v", fields[0])
		}
		...
		if err != nil {
			return errors.Wrapf(err, "job %d: failed to decode hex progress: %v", id)
		}
		md.Progress = &jobspb.Progress{}
		if err := protoutil.Unmarshal(progressBytes, md.Progress); err != nil {
