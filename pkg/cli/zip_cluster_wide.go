...
func makeClusterWideZipRequests(
	...
	if zipCtx.files.shouldIncludeFile(zipPathJoin(prefix, eventsFile)) {
	...
	if zipCtx.files.shouldIncludeFile(zipPathJoin(prefix, rangeLogFile)) {
		zipRequests = append(zipRequests, zipRequest{
			fn: func(ctx context.Context) (interface{}, error) {
	...
	if zipCtx.files.shouldIncludeFile(zipPathJoin(prefix, settingsFile)) {
		zipRequests = append(zipRequests, zipRequest{
			fn: func(ctx context.Context) (interface{}, error) {
	...
	if zipCtx.includeRangeInfo {
		if zipCtx.files.shouldIncludeFile(zipPathJoin(prefix, problemRangesName+".json")) {
			zipRequests = append(zipRequests, zipRequest{
				fn: func(ctx context.Context) (interface{}, error) {
...
func (zc *debugZipContext) collectClusterData(
...

	{
		...
		if zipCtx.files.shouldIncludeFile(zipPathJoin(debugBase, nodesFile)) {
		...
		if zipCtx.files.shouldIncludeFile(zipPathJoin(zc.prefix, livenessFile)) {
