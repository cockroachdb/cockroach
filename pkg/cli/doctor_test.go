...
func TestDoctorZipDir(t *testing.T) {
	...
	t.Run("examine", func(t *testing.T) {
		out, err := c.RunWithCapture("debug doctor examine zipdir testdata/doctor/debugzip 22.1-52")
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("deprecated doctor zipdir with verbose", func(t *testing.T) {
		out, err := c.RunWithCapture("debug doctor zipdir testdata/doctor/debugzip 22.11-52 --verbose")
		if err != nil {
			t.Fatal(err)
		}
