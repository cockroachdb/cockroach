...
func TestSensitiveSettingScrubbedFromZipDumps(t *testing.T) {
	...
	insertEvent("server.oidc_authentication.client_secret", secret)
	...
	// Populate system.settings with a sensitive value.
	...
	sqlDB.Exec(t, fmt.Sprintf(
		"ALTER TENANT ALL SET CLUSTER SETTING server.oidc_authentication.client_secret = '%s'", secret))
