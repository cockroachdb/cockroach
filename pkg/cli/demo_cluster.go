...
func (c *transientCluster) start(
	...
	//
	// So we proceed phases, as follows:
	//
	...
	//    then let the servers continue initializing.
	// 6. wait for readiness OR error from all the nodes.
	// 7. initialize SQL and telemetry.
	//
	...
	{
		ctx := logtags.AddTag(ctx, "phase", 1)
		log.Infof(ctx, "creating the first node")

		firstRPCAddrReadyCh, err := c.createAndAddNode(ctx, 0, latencyMapWaitCh, timeoutCh)
		if err != nil {
			return err
		}
	...
	{
		ctx := logtags.AddTag(ctx, "phase", 2)

		log.Infof(ctx, "starting first node")
		if err := c.startNodeAsync(ctx, 0, errCh, timeoutCh); err != nil {
			return err
		}
		log.Infof(ctx, "waiting for first node RPC address")
	...
	{
		ctx := logtags.AddTag(ctx, "phase", 3)
		log.Infof(ctx, "starting other nodes")

		for i := 1; i < demoCtx.nodes; i++ {
			rpcAddrReady, err := c.createAndAddNode(ctx, i, latencyMapWaitCh, timeoutCh)
			if err != nil {
				return err
			}
	...
	{
		ctx := logtags.AddTag(ctx, "phase", 4)
		log.Infof(ctx, "waiting for remaining nodes to get their RPC address")
...

	{
		...
		log.Infof(ctx, "letting servers initialize")
	...
	{
		ctx := logtags.AddTag(ctx, "phase", 6)

		// Step 6: wait for readiness or an error from all the nodes.
		...
		for r := retry.StartWithCtx(ctx, retryOpts); r.Next(); {
			...
			for i := 0; i < demoCtx.nodes; i++ {
				...
				case <-timeoutCh:
					return errors.Newf("initialization timeout while waiting for server %d readiness", i)
				case err := <-errCh:
					return errors.Wrapf(err, "server %d failed to start", i)
				case <-ctx.Done():
					return errors.CombineErrors(errors.Newf("context cancellation while waiting for server %d to become ready", i), ctx.Err())
				case <-c.servers[i].Stopper().ShouldQuiesce():
					return errors.Newf("server %s shut down prematurely", i)
				case <-c.stopper.ShouldQuiesce():
					return errors.Newf("demo cluster shut down prematurely while waiting for server %d to become ready", i)
				default:
					if err := c.servers[i].Readiness(ctx); err != nil {
						allReady = false
						log.Infof(ctx, "server %d not yet ready: %v", i, err)
					} else {
						log.Infof(ctx, "server %d reports ready", i)
					}
				}
			}
			if allReady {
				// Done waiting; exit the retry loop.
				break
			}
		}
		log.Infof(ctx, "all nodes ready")
	}

	{
		ctx := logtags.AddTag(ctx, "phase", 7)
		...
		// admin user.
		log.Infof(ctx, "running initial SQL for demo cluster")

		const demoUsername = "demo"
		demoPassword := genDemoPassword(demoUsername)
		if err := runInitialSQL(ctx, c.firstServer.Server, demoCtx.nodes < 3, demoUsername, demoPassword); err != nil {
			return err
		}
		if demoCtx.insecure {
		...
		if !demoCtx.disableTelemetry {
			log.Infof(ctx, "starting telemetry")
...
func (c *transientCluster) createAndAddNode(
	...
	// after it has signalled SignalAfterGettingRPCAddress, and before
