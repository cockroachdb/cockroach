	...
	for e, ok := stream.peek(); ok; e, ok = stream.peek() {
		...
		if prevTargetPath != "" && prevTargetPath != currTargetPath {
			...
			uploadFn(logUploadSig{
				logUploadType: getUploadType(prevTime),
				key:           prevTargetPath,
				nodeID:        nodeID,
				logLines:      logLines,
			})
...
func uploadZipLogs(ctx context.Context, uploadID string, debugDirPath string) error {
	...
	// start the GCP writer pool
	startWriterPool(gcsWriteFunc, debugZipUploadOpts.maxConcurrentUploads, gcsWorkChan, doneChan)

	// start the datadog writer pool
	startWriterPool(datadogWriteFunc, debugZipUploadOpts.maxConcurrentUploads, ddWorkChan, doneChan)
	...
	if recentLogSize != 0 {
		fmt.Fprintf(os.Stderr, "Logs from within the last 72 hours uploaded to datadog! (%s)\n", humanReadableSize(recentLogSize))
	}
...
// average size of 1000 lines is well withing the limit (5MB). So, we are only
...
type writeFunc func(context.Context, logUploadSig) (int, error)
...
func startWriterPool(
	fn writeFunc, size int, workChan <-chan logUploadSig, doneChan chan<- logUploadStatus,
) {
	for i := 0; i < size; i++ {
		go func() {
...
// gcsWriteFunc is a function that writes the logs to GCS.
...
var gcsWriteFunc = func(ctx context.Context, sig logUploadSig) (int, error) {
...
// datadogWriteFunc wraps the uploadLogsToDatadog function and adds the required formatting
// required for uplaoding multiple logs at once.
func datadogWriteFunc(ctx context.Context, sig logUploadSig) (int, error) {
...
// logs to datadog. This exists because artifact aother than logs might also
...
func uploadLogsToDatadog(ctx context.Context, payload []byte) (int, error) {
	...
	if _, err := doUploadReq(req); err != nil {
		return 0, fmt.Errorf("failed to send logs response: %w", err)
	}
