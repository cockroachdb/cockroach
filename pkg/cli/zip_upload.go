...
// here is much simpler than the logs upload. We spawn a pool of workers that
// listen to a channel for work. File names of the table dumps are queued in
// this channel. The workers read the file, parse the columns and uploads the
// data to datadog.
...
func processTableDump(
	...
	for i := 0; i < len(lines); i += datadogMaxLogLinesPerReq {
		...
		buf.WriteByte('[')
		buf.Write(bytes.Join(lines[i:end], []byte(",")))
		buf.WriteByte(']')

		_, err = uploadLogsToDatadog(ctx, buf.Bytes())
		if err != nil {
			return err
		}
	}
