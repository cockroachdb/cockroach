...
// by examining the debug_zip_command_flags.txt file in the system tenant.
...
func validateRedactionStatus(debugDirPath string) error {
	flagsFilePath := path.Join(debugDirPath, "debug_zip_command_flags.txt")
	...
	if err != nil {
		if oserror.IsNotExist(err) {
			...
			warningMsg := buildRedactionWarning(
				"⚠️  WARNING: The debug_zip_command_flags.txt file does not exist.\n" +
					"This means we cannot determine if redaction was enabled when this debug zip was generated.\n")

			return promptUserForConfirmation(warningMsg)
		}
		return fmt.Errorf("failed to read debug_zip_command_flags.txt file at %s: %w. "+
			"Please ensure the file is readable and the path is correct", flagsFilePath, err)
	}
	...
	if strings.Contains(flagsStr, "--redact=false") {
		warningMsg = buildRedactionWarning(
			"⚠️  WARNING: Your debug zip was created WITHOUT redaction (--redact=false).\n",
		)
	} else {
		warningMsg = buildRedactionWarning(
			"⚠️  WARNING: Your debug zip redaction status is unclear (no --redact flag found).\n",
		)
	}
