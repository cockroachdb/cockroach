...
func init() {
	...
	for _, cmd := range []*cobra.Command{demoCmd, statementBundleRecreateCmd} {
		...
		cliflagcfg.BoolFlag(f, &demoCtx.SimulateSeparateTenantProcess, cliflags.DemoSimulateSeparateTenantProcess)
		...
		_ = f.MarkHidden(cliflags.DemoSimulateSeparateTenantProcess.Name)
