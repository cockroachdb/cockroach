...
func setDefaultStderrVerbosity(cmd *cobra.Command, defaultSeverity log.Severity) error {
	...
	if vf := pf.Lookup(logflags.AlsoLogToStderrName); !vf.Changed {
		...
		// The Set() calls above also change the `Changed` attribute.
		// We don't want that, because the code for `start` wants to
		// know the original value. So restore it before returning.
		vf.Changed = false
	}
