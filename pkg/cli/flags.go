...
func init() {
	...
	pf.Lookup(logflags.LogToStderrName).NoOptDefVal = "unknown"
...
func setDefaultStderrVerbosity(cmd *cobra.Command, defaultSeverity log.Severity) error {
	...
	// set, then set stderr logging to the default level.
	if !vf.Changed && !log.DirSet() {
		vf.Value.Set(defaultSeverity.String())
	}

	// If `--logtostderr` was specified without explicit verbosity,
	// set to the default level.
	if vf.Value.String() == log.Severity_UNKNOWN.String() {
