...
func TestSendKVBatchTrace(t *testing.T) {
	...
	require.NoError(t, err)
	require.Contains(t, output, "=== operation:/cockroach.server.serverpb.Admin/SendKVBatch")

	// jaeger mode, output to stderr.
	output, err = c.RunWithCapture("debug send-kv-batch --trace=jaeger " + path)
	require.NoError(t, err)
	require.Contains(t, output, `"operationName": "/cockroach.server.serverpb.Admin/SendKVBatch",`)

	traceOut := filepath.Join(t.TempDir(), "trace.out")
	// text mode, output to file.
	output, err = c.RunWithCapture("debug send-kv-batch --trace=text --trace-output=" + traceOut + " " + path)
	require.NoError(t, err)
	b, err := ioutil.ReadFile(traceOut)
	require.NoError(t, err)
	require.Contains(t, string(b), "=== operation:/cockroach.server.serverpb.Admin/SendKVBatch")

	// jaeger mode, output to file.
	output, err = c.RunWithCapture("debug send-kv-batch --trace=jaeger --trace-output=" + traceOut + " " + path)
	require.NoError(t, err)
	b, err = ioutil.ReadFile(traceOut)
	require.NoError(t, err)
	require.Contains(t, string(b), `"operationName": "/cockroach.server.serverpb.Admin/SendKVBatch",`)
}
