...
set unwritable_dir [exec bash -c "d=\$(mktemp -d) && mkdir -p \$d/logs && chmod a-w \$d/logs && echo \$d"]
send "$argv start -s=path=$unwritable_dir --insecure --logtostderr\r"
eexpect "log: exiting because of error: log: cannot create log: open"
eexpect "permission denied"
eexpect ":/# "
end_test
