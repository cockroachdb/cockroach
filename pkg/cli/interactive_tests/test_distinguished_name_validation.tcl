...
proc start_secure_server {argv certs_dir extra} {
    ...
    send "$argv sql --certs-dir=$certs_dir -e 'SET CLUSTER SETTING enterprise.license = \"$::env(COCKROACH_DEV_LICENSE)\"'\r";
    eexpect $::prompt
    send "$argv sql --certs-dir=$certs_dir -e 'SET CLUSTER SETTING cluster.organization = \"Cockroach Labs - Production Testing\"';\r"
    eexpect $::prompt
...
send "$argv sql --certs-dir=$certs_dir --user=goofus -e 'select 1'\r"
eexpect "certificate authentication failed for user \"goofus\" (DN: o=NotCockroach,cn=invalid)"
end_test
