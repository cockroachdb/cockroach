...
system "n=`grep 'SELECT 660' $logfile | sed -e 's/.*SELECT 660//g' | awk '{print \$8}' | uniq | wc -l`; if test \$n -ne 2; then echo unexpected \$n; exit 1; fi"
# Same txns.
system "n=`grep 'SELECT 770' $logfile | sed -e 's/.*SELECT 770//g' | awk '{print \$8}' | uniq | wc -l`; if test \$n -ne 1; then echo unexpected \$n; exit 1; fi"
system "n=`grep 'SELECT 880' $logfile | sed -e 's/.*SELECT 880//g' | awk '{print \$8}' | uniq | wc -l`; if test \$n -ne 1; then echo unexpected \$n; exit 1; fi"
system "n=`grep 'SELECT 990' $logfile | sed -e 's/.*SELECT 990//g' | awk '{print \$8}' | uniq | wc -l`; if test \$n -ne 1; then echo unexpected \$n; exit 1; fi"
