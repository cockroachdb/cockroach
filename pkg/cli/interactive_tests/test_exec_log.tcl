...
eexpect root@
send "SET CLUSTER SETTING sql.trace.log_statement_execute.split = TRUE;\r"
eexpect root@
...
send "SET CLUSTER SETTING sql.trace.log_statement_execute.split = FALSE;\r"
eexpect root@
send "SELECT 'lovely';\r"
eexpect root@

# We can't flush the log externally, so shut down the server to force a flush
interrupt
eexpect eof
stop_server $argv
