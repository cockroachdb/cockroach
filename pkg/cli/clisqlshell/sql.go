...
func (c *cliState) doHandleCliCmd(loopState, nextState cliStateEnum) cliStateEnum {
	...
	case `\password`:
		...
		if password1 != password2 {
			...
			hashedPassword, err := security.HashPassword(context.Background(), &sv, passwordHashMethod, password1)
...
var passwordHashMethod security.HashMethod
...
func (c *cliState) RunInteractive(cmdIn, cmdOut, cmdErr *os.File) (exitErr error) {
	...
	passwordHashMethod = c.getPasswordHashMethod()
...
func (c *cliState) getPasswordHashMethod() security.HashMethod {
	passwordEncrptMethod, err := c.getSessionVarValue("password_encryption")
	if err != nil {
	...
	switch passwordEncrptMethod {
	case "<invalid>":
		return security.HashInvalidMethod
	case "<missing password>":
		return security.HashMissingPassword
	case "crdb-bcrypt":
		return security.HashBCrypt
	case "scram-sha-256":
		return security.HashSCRAMSHA256
	default:
		panic(errors.AssertionFailedf("unknown hash method %s", passwordEncrptMethod))
	}
}
