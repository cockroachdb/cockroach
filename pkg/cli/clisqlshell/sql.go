...
Informational
  \d[S+]  [PATTERN] list tables and other stored objects.
  ...
  \du[S+] [PATTERN] list users and roles.
...
func (c *cliState) handleDescribe(cmd []string, loopState, errState cliStateEnum) cliStateEnum {
	var sql string
	var qargs []interface{}
	var foreach func([]string) (string, string, []interface{})
	sql, qargs, foreach, c.exitErr = pgInspect(cmd)
	if c.exitErr != nil {
		clierror.OutputError(c.iCtx.stderr, c.exitErr, true /*showSeverity*/, false /*verbose*/)
		return errState
	}

	// Each of the output tables is a "stage", resulting from executing
	// a SQL statement with a number of positional arguments.
	type stage struct {
		title string
		sql   string
		qargs []interface{}
	...
	if foreach == nil {
		// A single stage.
		toRun = []stage{{sql: sql, qargs: qargs}}
	} else {
		...
		for _, row := range rows {
			title, sql, qargs := foreach(row)
			if title == "" {
				continue
			}
			toRun = append(toRun, stage{title: title, sql: sql, qargs: qargs})
		}
	}
