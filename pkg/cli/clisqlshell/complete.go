...
func (b *bubblineReader) getCompletions(
	...
	for _, row := range rows {
		...
		if len(row) >= 5 {
			...
			start := byteOffsetToRuneOffset(sql, roffset, offset /* cursor */, i)
			end := byteOffsetToRuneOffset(sql, roffset, offset /* cursor */, j)
...
// to a rune offset suitable for the bubble rune-based addressing.  We
// use the cursor as a starting point to avoid scanning the sql string
// from the start.
