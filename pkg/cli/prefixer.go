...
// collection of log files. The prefixes exclude common paths that don't
// include ",", ".", or "-" delimiters (by default).
//
// File paths are split into tokens using "/" or "\" (by default). If no
...
func (f filePrefixer) Prefix(logFiles []fileInfo) {
	...
	for i, lf := range logFiles {
		...
		tokens := f.splitter.Split(filepath.Dir(lf.path), -1)
