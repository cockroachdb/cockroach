...
func runInit(cmd *cobra.Command, args []string) error {
	...
	conn, finish, err := waitForClientReadinessAndGeClientGRPCConn(ctx)
	if err != nil {
		return err
	}
	defer finish()
...
// waitForClientReadinessAndGeClientGRPCConn waits for the node to
...
func waitForClientReadinessAndGeClientGRPCConn(
	ctx context.Context,
	...
	for r := retry.StartWithCtx(ctx, retryOpts); r.Next(); {
		if err = func() error {
			timeoutCtx, stop := context.WithTimeout(ctx, 5*time.Second)
			defer stop()
			...
			_, err := ac.Details(timeoutCtx, &serverpb.DetailsRequest{})
			return err
		}(); err != nil {
