...
func (d *datadogWriter) upload(fileName string) error {
	...
	// 730 hours per month.
	// For a single node uploade that has 6500 unique series, that's about $.40
	// per upload.
	...
	msgJson, _ := json.Marshal(struct {
		...
		DurationMS:     int(timeutil.Now().UnixMilli() - d.uploadTime.UnixMilli()),
