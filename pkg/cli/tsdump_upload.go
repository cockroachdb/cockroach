...
func (d *datadogWriter) upload(fileName string) error {
	var recvDatadogSeries func() ([]DatadogSeries, error)
	...
	recvDatadogSeries = func() ([]DatadogSeries, error) {
		r := <-ch
		return r.data, r.err
	}

	for {
		data, err := recvDatadogSeries()
		if err == io.EOF {
			if len(data) != 0 {
				return d.emitDataDogMetrics(data)
			}
			return nil
		}
		...
		go func() {
			// requestChannel is closed when the process exits, so closing channel here is
			// more for extra protection.
			err := d.emitDataDogMetrics(data)
			if err != nil {
				fmt.Printf("rertries exhausted for datadog upload containing series: %s", data[0].Metric)
				return
			}
		}()
