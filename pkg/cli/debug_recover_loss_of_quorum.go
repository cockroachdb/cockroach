	...
	for _, store := range debugRecoverCollectInfoOpts.Stores.Specs {
		...
		if err != nil {
			return errors.Wrapf(err, "failed to open store at path %q. ensure that store path is "+
				"correct and that it is not used by another process", store.Path)
		}
...
func runDebugPlanReplicaRemoval(cmd *cobra.Command, args []string) error {
	...
	fmt.Fprintf(stderr, `Total replicas analyzed: %d
Healthy replicas found:  %d
Ranges without quorum:   %d
Discarded live replicas: %d

`, report.TotalReplicas, report.HealthyReplicas, len(report.PlannedUpdates), report.LiveNonSurvivors)
	for _, r := range report.PlannedUpdates {
		fmt.Fprintf(stderr, "Recovering range r%d:%s promoting replica r%d at store s%d to r%d. "+
	...
	if len(deadStoreIDs) == 0 {
		...
		if len(plan.Updates) == 0 {
			fmt.Fprintf(stderr, "No recoverable ranges found.\n")
			return nil
		}

		switch debugRecoverPlanOpts.confirmAction {
		case prompt:
			fmt.Fprintf(stderr, "Proceed with plan creation [y/N] ")
			...
			if len(line) < 1 || (line[0] != 'y' && line[0] != 'Y') {
				fmt.Fprint(stderr, "Aborted at user request\n")
				return nil
			}
		case allYes:
	...
	fmt.Fprintf(stderr, "Plan created\n")
	for node, stores := range report.UpdatedNodes {
		fmt.Fprintf(stderr, "Run apply-plan on node n%d, store(s) %s\n", node, joinStoreIDs(stores))
	}
...
func runDebugExecuteRecoverPlan(cmd *cobra.Command, args []string) error {
	...
	stores := make(map[roachpb.StoreID]*loqrecovery.UpdatableStore)
	...
	if len(prepReport.UpdatedReplicas) == 0 {
		...
		fmt.Fprintf(stderr, "No updates planned on this node.\n")
		return nil
	}

	for _, r := range prepReport.UpdatedReplicas {
		message := fmt.Sprintf(
			"Replica %d for range %d:%s will be promoted to %d with peer replica(s) removed: %s",
			r.OldReplicaID, r.RangeID, r.StartKey, r.ReplicaID, r.RemovedReplicas)
	...
	applyReport, err := loqrecovery.CommitReplicaChanges(stores)
	fmt.Fprintf(stderr, "Updated store(s): %s\n", joinStoreIDs(applyReport.UpdatedStores))
	return err
}
