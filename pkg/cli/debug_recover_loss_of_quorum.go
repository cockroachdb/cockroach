...
func runDebugDeadReplicaCollect(cmd *cobra.Command, args []string) error {
	...
	fmt.Fprintf(stderr, "Collected info about %d replicas.\n", len(replicaInfo.Replicas))
	return nil
}
...
func runDebugPlanReplicaRemoval(cmd *cobra.Command, args []string) error {
	...
	console := getConfirmAction(debugRecoverPlanOpts.confirmAction)
	...
	console.printf("Total replicas analyzed: %d\nHealthy replicas found:  %d\nDiscarded live replicas: %d\n\n",
		report.TotalReplicas, report.HealthyReplicas, report.LiveNonSurvivors)
	for _, r := range report.PlannedUpdates {
		console.printf("Recovering range r%d:%s promoting replica r%d at store s%d to r%d. Discarding replicas: %s\n",
			r.RangeID, r.StartKey, r.OldReplicaID, r.StoreID, r.ReplicaID, r.DiscardedReplicas)
	}

	// If we are autodetecting dead stores, confirm user if we want to proceed with the plan.
	...
	for node, stores := range report.UpdatedNodes {
		console.printf("Run update on node n%d, store(s) %s\n", node, joinStoreIDs(stores))
	}
...
func readReplicaInfoData(fileNames []string) ([]loqrecoverypb.NodeReplicaInfo, error) {
	...
	for _, filename := range fileNames {
		...
		if err != nil {
			return nil, errors.Wrapf(err, "failed to read replica info file %s", filename)
		}

		var nodeReplicas loqrecoverypb.NodeReplicaInfo
		jsonpb := protoutil.JSONPb{Indent: "  "}
		if err = jsonpb.Unmarshal(data, &nodeReplicas); err != nil {
			return nil, errors.Wrapf(err, "failed to unmarshal replica info from file %s", filename)
		}
...
func runDebugExecuteRecoverPlan(cmd *cobra.Command, args []string) error {
	...
	console := getConfirmAction(debugRecoverExecuteOpts.confirmAction)
	...
	updates, prepReport, err := loqrecovery.PrepareUpdateReplicas(cmd.Context(), nodeUpdates, localNodeID, stores)
	if err != nil {
		return err
	}

	for _, r := range prepReport.SkippedReplicas {
		console.printf("Replica %d for range r%d is already updated.\n", r.ReplicaID, r.RangeID)
	}

	if len(updates) == 0 {
		if len(prepReport.MissingStores) > 0 {
			return errors.Newf("stores %s expected on the node but no paths were provided", joinStoreIDs(prepReport.MissingStores))
		}
		console.printf("No updates planned on this node.\n")
		return nil
	}

	for _, r := range prepReport.UpdatedReplicas {
		message := fmt.Sprintf("Replica %d for range %d:%s will be promoted with peer replica(s) removed: %s", r.ReplicaID, r.RangeID, r.StartKey, r.RemovedReplicas)
		if r.AbortedTransaction {
			message += fmt.Sprintf(", and range update transaction %s aborted.", r.AbortedTransactionID.Short())
		}
		console.printf("%s\n", message)
	}

	if proceed, err := console.confirm("\nProceed with above changes"); !proceed || err != nil {
		if !proceed {
	...
	applyReport, err := loqrecovery.ApplyRecoveryChanges(cmd.Context(), updates)
	console.printf("Updated store(s): %s\n", joinStoreIDs(applyReport.UpdatedStores))
	return err
}

func getConfirmAction(opt confirmActionFlag) dialog {
	switch opt {
	case prompt:
		return interactiveDialog{reader: bufio.NewReader(os.Stdin), out: stderr}
	case allYes:
		return defaultedDialog{result: true, out: stderr}
	default:
		// Default to reject in case some input validation bug slips.
		return defaultedDialog{result: false, out: stderr}
	}
}
