...
to be removed replicas coming back online and conflicting with recovered
state. Note that if system ranges suffer loss of quorum, it may be impossible
to decommission nodes. In that case, recovery can proceed, but those nodes
must be decommissioned once cluster is back online after recovery.
