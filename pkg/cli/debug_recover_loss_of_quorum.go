...
func runDebugPlanReplicaRemoval(_ *cobra.Command, args []string) error {
	...
	for _, id := range debugDeadReplicaPlanOpts.deadStoreIDs {
		deadStoreIDs = append(deadStoreIDs, roachpb.StoreID(id))
	}

	// Run planner
	plan, err := planReplicas(descriptors, deadStoreIDs, confirm)
	if err != nil {
		return err
	}
...
func planReplicas(
	...
	if len(deadStores) == 0 {
		if ok, err := d.confirm(fmt.Sprintf("Found dead stores %s. Proceed with deletion", joinStoreIDs(missingStoreIDs))); err != nil {
			if !ok {
				return nil, errors.Errorf("Aborting at user request")
			}
			if err != nil {
				return nil, err
			}
	...
	for _, rangeDesc := range descriptors {
		...
		if rangeDesc.StoreID != maxLiveVoter {
			d.printf("Not designated survivor, skipping: %s\n", &desc)
			continue
		}
		...
		if desc.Replicas().CanMakeProgress(func(rep roachpb.ReplicaDescriptor) bool {
			...
			d.printf("Replica has not lost quorum, skipping: %s\n", &desc)
			continue
		}
		...
		d.printf("Replica has lost quorum, recovering: %s -> %s\n", &desc, &update)
...
// liveStores       - all storeIDs for which info was collected
...
func validateReplicaSets(
	...
	liveStoreIDs = make(storeIDSet)
	// Find all stores where replicas are present.
	...
	return liveStoreIDs, missingStoreIDs, nil
}

// marshalAndWriteResults serializes plan and write into file or stdout.
func marshalAndWriteResults(results interface{}, filename string, d dialog) error {
	var out []byte
	var err error
	...
	if filename != "" {
		...
		if _, err = os.Stat(filename); err == nil {
			var proceed bool
			proceed, err = d.confirm(fmt.Sprintf("File '%s' exists, overwrite", filename))
			if err != nil {
				return err
			}
			if !proceed {
...
func removeReplicas(
	...
	if proceed, err := d.confirm("Proceed with above changes"); !proceed || err != nil {
		if !proceed {
	...
	for id, batch := range batches {
		...
		if !batch.Empty() {
			if err := batch.Commit(true); err != nil {
				...
				d.printf("Failed to update store s%d\n", id)
				return err
			}
			d.printf("Updated store s%d\n", id)
		}
	}
	return nil
}
...
func getConfirmAction(out io.Writer) dialog {
