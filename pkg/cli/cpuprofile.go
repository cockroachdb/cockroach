...
func (s cpuProfiler) CheckOwnsFile(_ context.Context, fi os.FileInfo) bool {
	return strings.HasPrefix(fi.Name(), heapprofiler.CpuProfFileNamePrefix)
}
...
func initCPUProfile(ctx context.Context, dir string, st *cluster.Settings) {
	...
	profilestore := dumpstore.NewStore(dir, heapprofiler.MaxCombinedCPUProfFileSize, st)
	...
	go func() {
		...
		for {
			...
			if err := debug.CPUProfileDo(st, cluster.CPUProfileDefault, func() error {
				...
				now := timeutil.Now()
				name := heapprofiler.CpuProfFileNamePrefix + now.Format(heapprofiler.CpuProfTimeFormat)
