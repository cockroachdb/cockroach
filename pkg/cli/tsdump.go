	...
	RunE: clierrorplus.MaybeDecorateError(func(cmd *cobra.Command, args []string) error {
		...
		if convertFile == "" {
			...
			if debugTimeSeriesDumpOpts.metricsListFile != "" {
				...
				names, err = getTimeseriesNamesFromMetricsList(ctx, adminClient, debugTimeSeriesDumpOpts.metricsListFile)
				if err != nil {
					return err
				}
...
// getTimeseriesNamesFromMetricsList reads metric names and regex patterns from a file
...
func getTimeseriesNamesFromMetricsList(
	ctx context.Context, adminClient serverpb.RPCAdminClient, filePath string,
) ([]string, error) {
	...
	resp, err := adminClient.AllMetricMetadata(ctx, &serverpb.MetricMetadataRequest{})
	if err != nil {
