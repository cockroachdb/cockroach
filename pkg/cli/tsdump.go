...
`,
	...
	RunE: clierrorplus.MaybeDecorateError(func(cmd *cobra.Command, args []string) error {
		...
		if convertFile == "" {
			...
			var regexMatchCounts map[string]int
			names, regexMatchCounts, err = serverpb.GetInternalTimeseriesNamesFromServer(ctx, adminClient, filter)
			if err != nil {
				return err
			}
			if debugTimeSeriesDumpOpts.metricsListFile != "" {
				// Print regex match counts for user feedback
				for pattern, count := range regexMatchCounts {
					if count > 0 {
