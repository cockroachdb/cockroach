...
// bytesOrPercentageValue is a flag that accepts an integer value, a integer
...
// known once other flags are parsed (e.g. --max-dist-temp-storage=10% depends
// on --store).
...
// distPercentResolverFactory takes in a path and produces a percentResolverFunc
...
func runStart(cmd *cobra.Command, args []string) error {
	...
	if serverCfg.Stores.Specs[0].InMemory && !diskTempStorageSizeValue.IsSet() {
		// The default temp store size is 100MB when the first store is in memory.
		serverCfg.TempStoreMaxSizeBytes = 100 << 20 // 100MB
	}
