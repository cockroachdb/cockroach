...
func runStart(cmd *cobra.Command, args []string, startSingleNode bool) (returnErr error) {
	...
	serverCfg.ReadyFn = func(waitForInit bool) {
		...
		if startCtx.listeningURLFile != "" {
			...
			pgURL, err := clientconnurl.PGURL(clientConnOptions, serverParams, url.User(username.RootUser))
			if err != nil {
...
) (clientconnurl.ClientSecurityOptions, clientconnurl.ServerParameters) {
	clientConnOptions := clientconnurl.ClientSecurityOptions{
		Insecure: serverCfg.Config.Insecure,
		CertsDir: serverCfg.Config.SSLCertsDir,
	}
	serverParams := clientconnurl.ServerParameters{
