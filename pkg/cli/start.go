...
func setupAndInitializeLoggingAndProfiling(
	...
	if !startCtx.logDir.IsSet() && !lf.Changed {
		...
		if newDir == "" {
			// If there were no stores (as is the case during `start-sql`),
			// use the location that would result from the default store.
			newDir = filepath.Join(server.DefaultStorePath, "logs")
		}
