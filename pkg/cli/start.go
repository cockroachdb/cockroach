...
func runStart(_ *cobra.Command, args []string) error {
	...
	case sig := <-signalCh:
		log.Infof(startCtx, "received signal '%s'", sig)
		if sig == os.Interrupt {
	...
	go func() {
		...
		for {
			...
			case <-ticker.C:
				if log.V(1) {
					log.Infof(startCtx, "running tasks:\n%s", stopper.RunningTasks())
				}
				log.Infof(startCtx, "%d running tasks", stopper.NumTasks())
			case <-stopper.ShouldStop():
				return
			}
		}
	...
	case sig := <-signalCh:
		...
		log.Errorf(startCtx, "%v", returnErr)
