...
func runStart(_ *cobra.Command, args []string) error {
	...
	var serverDraining bool
	var serverStatusMu syncutil.Mutex // protects serverCreated and serverDraining
	var s *server.Server
	errChan := make(chan error, 1)
	go func() {
		if err := serverCfg.InitNode(); err != nil {
			errChan <- fmt.Errorf("failed to initialize node: %s", err)
			return
		}
		...
		if err != nil {
			errChan <- fmt.Errorf("failed to start Cockroach server: %s", err)
			return
		}
		...
		if err := s.Start(startCtx); err != nil {
			errChan <- fmt.Errorf("cockroach server exited with error: %s", err)
			return
		}
		sp.Finish()
