	...
	go func() {
		...
		if err := func() error {
			...
			if shutdownInProgress := serverStatusMu.setStarted(); shutdownInProgress {
				// The shutdown code is taking a shortcut. Help it along.
				stopper.Stop(ctx)
				return nil
			}
...
// up (e.g. in runStart) and the async goroutine that stops the
// server (in waitForShutdown).
//
// This is needed because it isn't safe to try to
// drain a server that doesn't exist or is in the middle of starting up,
// or to start a server after shutdown has begun.
type serverStatus struct {
	syncutil.Mutex
	// started indicates that the server has started already. After
	// started has become true, a graceful shutdown should use a soft drain.
	started bool
	// shutdownRequested indicates that shutdown has started already. After
	// draining has become true, server startup should stop.
	shutdownRequested bool
}

// setStarted marks the server as started. It returns whether
// shutdown has been requested already. In that case, we know that
// the shutdown goroutine did not use the stopper, so the
// server startup can do that.
...
// shutdownInProgress returns whether a shutdown
...
// startShutdown registers the shutdown request and
// returns whether the server was started already.
...
func waitForShutdown(
	...
	case err := <-errChan:
		...
		// therefore we are OK with an expedited shutdown.
		startShutdownAsync(getS, stopper, serverStatusMu, stopWithoutDrain, false /* gracefulDrain */)
		// Don't return: we're shutting down gracefully.
		break

	case <-stopper.ShouldQuiesce():
		// Receiving a signal on ShouldQuiesce means some internal system
		// inside the server triggered a shutdown.
		//
		...
		// Let the final steps of the shutdown below as usual.
		break
