...
func runStartInternal(
	...
	if err = func() error {
		if startCtx.goMemLimitValue.IsSet() {
			...
			} else if goMemLimit > 0 && goMemLimit < minDefaultGoMemLimit {
				log.Ops.Shoutf(
					ctx, severity.WARNING, "--max-go-memory (%s) is smaller "+
						"than the recommended minimum (%s), consider increasing it",
					humanizeutil.IBytes(goMemLimit), humanizeutil.IBytes(minDefaultGoMemLimit),
				)
			}
		} else {
			if _, envVarSet := os.LookupEnv("GOMEMLIMIT"); envVarSet {
				...
				if envVarLimit := envutil.EnvOrDefaultInt64("GOMEMLIMIT", -1); envVarLimit < minDefaultGoMemLimit {
					log.Ops.Shoutf(
						ctx, severity.WARNING, "GOMEMLIMIT (%s) is smaller "+
							"than the recommended minimum (%s), consider increasing it",
						humanizeutil.IBytes(envVarLimit), humanizeutil.IBytes(minDefaultGoMemLimit),
					)
				}
				return nil
			}
...
// minDefaultGoMemLimit determines the lower bound on the default value of the
// goMemLimit.
const minDefaultGoMemLimit = 256 << 20 /* 256MiB */
...
func getDefaultGoMemLimit(ctx context.Context) int64 {
	...
	if maxGoMemLimit < minDefaultGoMemLimit {
		...
		// don't shout here.
		maxGoMemLimit = minDefaultGoMemLimit
	}
