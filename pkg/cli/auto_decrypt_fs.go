...
//
// Assumes that path is absolute; the returned FS also assumes that any paths
// used are absolute. This is needed when using encryptedFS, since the
// PebbleFileRegistry used in that context attempts to convert function input
// paths to relative paths using the DBDir. Both the DBDir and function input
// paths in a CockroachDB node are absolute paths, but when using the Pebble
// tool, the function input paths are based on what the cli user passed to the
// pebble command. We do not wish for a user using the cli to remember to pass
// an absolute path to the various pebble tool commands that accept paths. Note
// that the pebble tool commands taking a path parameter are quite varied:
// ranging from "pebble db" to "pebble lsm", so it is simplest to intercept the
// function input paths here.
