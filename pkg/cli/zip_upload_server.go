...
func uploadDebugZipToCRLSupport(ctx context.Context, zipFilePath string) error {
	...
	// Initialise to a sentinel so a never-entered loop (e.g. ctx already
	// cancelled) doesn't leave err == nil and falsely report success.
	err = errors.New("upload not attempted")
	...
	for r := retry.Start(retryOpts); r.Next(); {
		...
		// A dead session URI cannot be revived by retrying.
...
// disables the per-request timeout (the blob client uses this, since
// upload duration scales with file size and link speed).
func newUploadHTTPClient(timeout time.Duration) (*http.Client, error) {
	transport := http.DefaultTransport.(*http.Transport).Clone()

	proxy := debugZipUploadOpts.proxy
	if proxy == "" {
		// depth=0 keeps the recorded "consumer" location pinned to this
		// call site so envutil's per-var consumer check doesn't panic
		// when newUploadHTTPClient is invoked from multiple call sites
		// (orchestrator + tests).
		proxy, _ = envutil.ExternalEnvString(httpsProxyEnvVar, 0)
	}
	if proxy != "" {
