...
func (u *gcsSessionURIUploader) Upload(ctx context.Context, srcPath string) (string, int64, error) {
	...
	var offset int64
	if u.resuming {
		off, complete, err := u.queryUploadOffset(ctx, totalSize)
		if err != nil {
			return "", 0, err
		}
		if complete {
...
// object. Returns (totalSize, true, nil) if the object is already
// complete, (offset, false, nil) to resume at byte `offset`, or
// errSessionURIDead if the URI is gone (404/410).
//
...
func (u *gcsSessionURIUploader) queryUploadOffset(
	...
	if err != nil {
		return 0, false, errors.Wrap(err, "building probe request")
	}
	req.ContentLength = 0
	...
	resp, err := doCRLHTTPRequest(u.httpClient, req)
	if err != nil {
		return 0, false, errors.Wrap(err, "probing upload offset")
	}
	defer resp.Body.Close()
	...
	case http.StatusNotFound, http.StatusGone:
		return 0, false, errSessionURIDead
...
func (u *gcsSessionURIUploader) destURI() string {
	return fmt.Sprintf("gs://%s/%s", u.bucket, u.objectPath)
}
