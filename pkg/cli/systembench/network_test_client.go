...
func RunClient(clientOptions ClientOptions) error {
	...
	return runTest(ctx, test{
		...
		tick: func(elapsedTotal time.Duration, i int) {
			...
			if i%20 == 0 {
				fmt.Println("_elapsed____address_____________ops/sec___mb/sec__p50(ms)__p95(ms)__p99(ms)_pMax(ms)")
			}
			reg.Tick(func(tick histogramTick) {
				h := tick.Hist
				fmt.Printf("%8s %16s %10.1f %8.1f %8.1f %8.1f %8.1f %8.1f\n",
			...
			const totalHeader = "\n_elapsed____address_____________ops(total)__mb(total)__avg(ms)__p50(ms)__p95(ms)__p99(ms)_pMax(ms)"
			fmt.Println(totalHeader + `__total`)
			reg.Tick(func(tick histogramTick) {
				h := tick.Cumulative
				fmt.Printf("%8s %16s %13d %10.1f %8.1f %8.1f %8.1f %8.1f %8.1f\n",
