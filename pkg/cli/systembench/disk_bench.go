...
func newWorkerSeqWrite(ctx context.Context, diskOptions *DiskOptions, registry *histogramRegistry) worker {
...
func (w *workerSeqWrite) run(ctx context.Context) error {
	...
	for {
		...
		w.latency.Lock()
		w.latency.namedHistogram.Record(timeutil.Since(start))
		w.latency.Unlock()
	}
}
