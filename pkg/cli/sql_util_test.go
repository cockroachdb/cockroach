...
func TestUtfName(t *testing.T) {
	...
	if err := runQueryAndFormatResults(conn, &b,
		makeQuery(`create database test_utf; create table test_utf.żółw (id int primary key, value int); alter table test_utf.żółw add constraint żó unique (value)`)); err != nil {
		t.Fatal(err)
	}
