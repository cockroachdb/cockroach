		...
		if !anyActive && replicaCount == 0 {
			...
			for _, targetNode := range nodeIDs {
				if targetNode == localNodeID {
					...
					log.Warning(ctx,
						"node specified with --host should not be a target node; skipped the drain step for this node.",
					)
					continue
				}
...
func runRecommissionNode(cmd *cobra.Command, args []string) error {
	...
	r, err := s.Node(ctx, &serverpb.NodeRequest{NodeId: "local"})
	if err != nil {
		return err
	}
	localNodeID := r.Desc.NodeID
