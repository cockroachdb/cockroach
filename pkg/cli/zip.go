...

	{
		...
		if zipCtx.cpuProfDuration > 0 {
			...
			for i := range nodeList {
				if livenessByNodeID[nodeList[i].Desc.NodeID] == nodelivenesspb.NodeLivenessStatus_DECOMMISSIONED {
					continue
				}
				wg.Add(1)
		...
		for _, node := range nodeList {
			...
			if liveness == nodelivenesspb.NodeLivenessStatus_DECOMMISSIONED {
