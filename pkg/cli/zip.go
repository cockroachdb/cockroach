...
var customQuery = map[string]string{
	"crdb.internal.node_inflight_trace_spans": "WITH spans AS (" +
		"SELECT span_id FROM crdb.internal.node_inflight_trace_spans" +
		") SELECT * FROM spans, LATERAL crdb_internal.payloads_for_span(span_id) " +
		"WHERE duration > 10*time.Second ORDER BY trace_id ASC, duration DESC",
