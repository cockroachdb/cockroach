...

	{
		...
		if err := contextutil.RunWithTimeout(baseCtx, "request nodes", timeout, func(ctx context.Context) error {
			...
			for _, node := range nodes.Nodes {
				...
				// At this point sqlConn is the node `debug zip` was pointed at,
				// but we also want to connect to nodes individually to grab node-
				// local SQL tables. Try to guess by replacing the host in the
				// connection string; this may or may not work and if it doesn't,
				// we let the invalid sqlConn get used anyway so that anything
				// that does *not* need it will still happen.
				sqlConn := guessNodeURL(sqlConn.url, node.Desc.Address.AddressField)
