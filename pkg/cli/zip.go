...
func runDebugZip(_ *cobra.Command, args []string) (retErr error) {
	...
	if sqlAddr.IsEmpty() {
		if zipCtx.serverless {
			// No way to access a serverless cluster without a SQL address,
			// return error. Technically, this should never happen.
			return s.fail(errors.New("no SQL address for serverless"))
		}
