...
// id.
func (zc *debugZipContext) getRedactedNodeDetails(redactedNodesList *serverpb.NodesListResponse, nodeId int32) serverpb.NodeDetails {
...
func runDebugZip(cmd *cobra.Command, args []string) (retErr error) {
	...
	for _, tenant := range tenants {
		if err := func() error {
			...
			if err := zc.collectCPUProfiles(ctx, redactedNodesList, livenessByNodeID); err != nil {
				return err
			}
