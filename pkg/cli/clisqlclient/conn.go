...
func (c *sqlConn) getLastQueryStatisticsInternal() (
	...
	parsedExecLatency, e1 := tree.ParseDInterval(execLatencyRaw)
	parsedServiceLatency, e2 := tree.ParseDInterval(serviceLatencyRaw)
	parsedPlanLatency, e3 := tree.ParseDInterval(planLatencyRaw)
	parsedParseLatency, e4 := tree.ParseDInterval(parseLatencyRaw)
	var e5 error
	var parsedJobsLatency *tree.DInterval
	if containsJobLat {
		parsedJobsLatency, e5 = tree.ParseDInterval(jobsLatencyRaw)
	}
