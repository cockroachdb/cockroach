...
func (c *sqlConn) GetServerMetadata(
	...
	if c.conn.IsClosed() {
		return 0, "", "", errorutil.CombineErrorsWithMark(err, ErrConnectionClosed)
	}
	if err != nil {
		return 0, "", "", err
	}
...
func (c *sqlConn) checkServerMetadata(ctx context.Context) error {
	...
	if c.conn.IsClosed() {
		return errorutil.CombineErrorsWithMark(err, ErrConnectionClosed)
	}
	if err != nil {
...
func (c *sqlConn) Query(ctx context.Context, query string, args ...interface{}) (Rows, error) {
	...
	if len(args) > 0 || c.alwaysInferResultTypes {
		...
		if c.conn.IsClosed() {
			c.reconnecting = true
			c.silentClose()
			return nil, errorutil.CombineErrorsWithMark(err, ErrConnectionClosed)
		}
		if err != nil {
			return nil, err
		}
	...
	if c.conn.IsClosed() {
		c.reconnecting = true
		c.silentClose()
		return nil, errorutil.CombineErrorsWithMark(multiResultReader.Close(), ErrConnectionClosed)
	}
