...
func isAtLeast22dot1ClusterVersion(conn Conn) bool {
	// Check whether the migration to add the conditional diagnostics columns to
	// the statement_diagnostics_requests system table has already been run - if
	// this query returns an error, then it hasn't.
	err := conn.Exec("SELECT count(min_execution_latency) FROM system.statement_diagnostics_requests", nil /* args */)
	return err == nil
}

func stmtDiagListOutstandingRequestsInternal(conn Conn) ([]StmtDiagActivationRequest, error) {
	var extraColumns string
	atLeast22dot1 := isAtLeast22dot1ClusterVersion(conn)
	if atLeast22dot1 {
