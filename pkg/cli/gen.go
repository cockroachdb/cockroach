...
`,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		sArgs := base.TestServerArgs{
			Insecure: true,
			// TODO(knz): set after #108762 is merged.
		...
		srv := s.(serverutils.TestServerInterface)
		...
		case <-time.After(5 * time.Second):
			return errors.Newf("could not initialize server in time")
		}
		...
		if err := retrieve(srv.SystemLayer(), func(layer catalog.MetricLayer) bool {
			return layer != catalog.MetricLayer_APPLICATION
		}); err != nil {
			return err
		}
		if err := retrieve(srv.ApplicationLayer(), func(layer catalog.MetricLayer) bool {
			return layer == catalog.MetricLayer_APPLICATION
		}); err != nil {
			return err
		}
