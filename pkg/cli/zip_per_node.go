...
// Tables collected from each serverless SQL node in a debug zip.
var debugZipTablesPerServerlessNode = []string{
...
func (zc *debugZipContext) collectCPUProfiles(
	...
	// TODO(rima): Implement profiles endpoint for serverless clusters.
	if zipCtx.cpuProfDuration <= 0 || zipCtx.serverless {
...
func (zc *debugZipContext) collectPerNodeData(
	...
	if zipCtx.serverless {
		debugZipTables = debugZipTablesPerServerlessNode
	}
	...
	if zipCtx.serverless {
		// Return early for serverless since subsequent endpoints (stacks, profiles etc.)
		// are not yet supported for serverless.
