...
func makePerNodeZipRequests(
	...
	if zipCtx.files.shouldIncludeFile(zipPathJoin(prefix, detailsFileName)) {
	...
	if zipCtx.files.shouldIncludeFile(zipPathJoin(prefix, gossipFileName)) {
		zipRequests = append(zipRequests, zipRequest{
			fn: func(ctx context.Context) (interface{}, error) {
...
func (zc *debugZipContext) collectCPUProfiles(
	...
	for i := range nodeList {
		...
		if !zipCtx.files.shouldIncludeFile(zipPathJoin(nodePrefix, cpuProfileFileName)) {
	...
	for i, pd := range resps {
		...
		if err := zc.z.createRawOrError(s, zipPathJoin(prefix, cpuProfileFileName), pd.data, pd.err); err != nil {
			return err
		}
	}
	return nil
...
func (zc *debugZipContext) collectFileList(
	...
	if requestErr := zc.runZipFn(ctx, s,
		...
		for _, file := range files.Files {
			...
			if !zipCtx.files.isIncluded(zipPathJoin(prefix, file.Name), ctime, ctime) {
...
func (zc *debugZipContext) getRangeInformation(
	...
	if zipCtx.includeRangeInfo {
		if !zipCtx.files.shouldIncludeFile(zipPathJoin(prefix, rangesInfoFileName)) {
...
func (zc *debugZipContext) getLogFiles(
	...
	if requestErr := zc.runZipFn(ctx, s,
		...
		for _, file := range logs.Files {
			...
			if !zipCtx.files.isIncluded(zipPathJoin(prefix, "logs", file.Name), ctime, mtime) {
...
func (zc *debugZipContext) getEngineStats(
	...
	if !zipCtx.files.shouldIncludeFile(zipPathJoin(prefix, lsmFileName)) {
...
func (zc *debugZipContext) getCurrentHeapProfile(
	...
	if !zipCtx.files.shouldIncludeFile(zipPathJoin(prefix, heapPprofFileName)) {
...
func (zc *debugZipContext) getStackInformation(
	...
	if zipCtx.includeStacks {
		if zipCtx.files.shouldIncludeFile(zipPathJoin(prefix, stacksFileName)) {
			var stacksData []byte
	...
	if zipCtx.files.shouldIncludeFile(zipPathJoin(prefix, stacksWithLabelFileName)) {
	...
	if zipCtx.files.shouldIncludeFile(zipPathJoin(prefix, stacksPprofFileName)) {
...
func (zc *debugZipContext) getNodeStatus(
	...
	if !zipCtx.files.shouldIncludeFile(zipPathJoin(prefix, statusFileName)) {
