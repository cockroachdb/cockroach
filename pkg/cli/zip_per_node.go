...
func (zc *debugZipContext) collectPerNodeData(
	...
	for _, table := range debugZipTablesPerNode {
		...
		if err := zc.dumpTableDataForZip(nodePrinter, curSQLConn, prefix, table, selectClause); err != nil {
	...
	if requestErr := zc.runZipFn(ctx, s,
		...
		for _, file := range logs.Files {
			...
			s := logPrinter.start("requesting file")
			if requestErr := zc.runZipFn(ctx, s,
				func(ctx context.Context) error {
					var err error
				...
				if err := zc.z.createError(s, name, requestErr); err != nil {
					return err
				}
				continue
			}
