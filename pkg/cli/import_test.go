...
func TestImportCLI(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			"PGDUMP",
			"testdata/import/db.sql",
			"--max-row-size=1000 --skip-foreign-keys=true --row-limit=10 --ignore-unsupported=true" +
				" --ignore-unsupported-log='foo://bar'",
			"IMPORT PGDUMP 'userfile://defaultdb.public.userfiles_root/db." +
				"sql' WITH max_row_size='1000', skip_foreign_keys, row_limit='10', ignore_unsupported, " +
				"ignore_unsupported_log='foo://bar'",
			"IMPORT TABLE foo FROM PGDUMP " +
				"'userfile://defaultdb.public.userfiles_root/db.sql' WITH max_row_size='1000', " +
				"skip_foreign_keys, row_limit='10', ignore_unsupported, ignore_unsupported_log='foo://bar'",
		},
		{
			"mysql",
			"MYSQLDUMP",
