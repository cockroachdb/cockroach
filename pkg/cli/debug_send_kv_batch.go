...
func runSendKVBatch(cmd *cobra.Command, args []string) error {
	verboseTrace := false
	fmtJaeger := false
	switch debugSendKVBatchOpts.traceFormat {
	case "on", "text":
	...
	if verboseTrace {
		fileName := debugSendKVBatchOpts.traceOutput
		if fileName == "" {
	...
	br, rec, err := setupTraceThenSendKVBatchRequest(ctx, tracer, verboseTrace, admin, &ba)
	if err != nil {
		return err
	}
...
func setupTraceThenSendKVBatchRequest(
	...
	if verboseTrace {
		opts = append(opts, tracing.WithRecording(tracing.RecordingVerbose))
	}
	ctx, sp := tracer.StartSpanCtx(ctx, "debug-send-kv-batch", opts...)
	defer sp.Finish()
