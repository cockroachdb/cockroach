...
func runSendKVBatch(cmd *cobra.Command, args []string) error {
	...
	br, rec, err := sendKVBatchRequestWithTracingOption(ctx, serverCfg.Tracer, enableTracing, admin, &ba)
	if err != nil {
		return err
	}
...
func sendKVBatchRequestWithTracingOption(
	...
	if verboseTrace {
		...
		_, sp = tracer.StartSpanCtx(ctx, "debug-send-kv-batch", tracing.WithRecording(tracing.RecordingVerbose))
		defer sp.Finish()
