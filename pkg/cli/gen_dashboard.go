...
in the format specified (datadog or grafana) in the current directory.
...
func runGenDashboardCmd(_ *cobra.Command, args []string) error {
	...
	case "datadog":
		return generateDatadogDashboard(dashboardType, dashConfig, metricsNameMap, metricTypeMap)
	case "grafana":
...
func extractPercentileValue(metricName string) string {
	for _, suffix := range percentileSuffixes {
		if strings.HasSuffix(metricName, suffix) {
			// Extract the percentile value by removing the "-p" prefix
...
func buildDatadogQuery(
	...
	if metricType == "HISTOGRAM" && percentileValue != "" {
		aggregation = fmt.Sprintf("p%s", percentileValue)
	}
