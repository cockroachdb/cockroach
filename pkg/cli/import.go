...
func runImport(
	...
	case "pgdump":
		...
		if isTableImport {
			importQuery = fmt.Sprintf(`IMPORT TABLE %s FROM PGDUMP '%s' %s`,
				tableName, unescapedUserfileURL, optionsClause)
		} else {
	...
	case "mysqldump":
		...
		if isTableImport {
			importQuery = fmt.Sprintf(`IMPORT TABLE %s FROM MYSQLDUMP '%s'%s`,
				tableName, unescapedUserfileURL, optionsClause)
		} else {
