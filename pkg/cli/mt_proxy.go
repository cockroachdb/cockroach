...
func runStartSQLProxy(cmd *cobra.Command, args []string) (returnErr error) {
	...
	proxyMetrics := sqlproxyccl.MakeProxyMetrics()
	handler, err := sqlproxyccl.NewProxyHandler(ctx, stopper, &proxyMetrics, proxyContext)
	if err != nil {
		return err
	}

	server := sqlproxyccl.NewServer(stopper, &proxyMetrics, handler.Handle)
