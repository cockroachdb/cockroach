...
func runStmtDiagList(cmd *cobra.Command, args []string) (resErr error) {
	...
	if len(reqs) == 0 {
		...
		w := tabwriter.NewWriter(&buf, 4, 0, 2, ' ', 0)
		fmt.Fprint(w, "  ID\tActivation time\tStatement\tPlan gist\tSampling probability\tMin execution latency\tExpires at\n")
		for _, r := range reqs {
			minExecLatency := "N/A"
			...
			fmt.Fprintf(
				w, "  %d\t%s\t%s\t%s\t%s\t%s\t%s\n",
				r.ID, r.RequestedAt.UTC().Format(timeFmt), r.Statement, r.PlanGist, samplingProbability, minExecLatency, expiresAt,
			)
		}
		_ = w.Flush()
