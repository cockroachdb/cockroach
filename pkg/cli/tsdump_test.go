...
`
	...
	t.Run("debug tsdump with --metrics-list flag and invalid regex", func(t *testing.T) {
		// Create a metrics list file with an invalid regex pattern
		...
		// Write an invalid regex pattern (unclosed bracket is detected as regex due to '[')
		...
		// The command output should contain the error message about invalid regex
		require.Contains(t, out, "invalid regex pattern")
	})
}
...
// TestGetTimeseriesNamesFromFile tests the full pipeline of reading metrics
// from a file and expanding them to timeseries names with histogram suffixes.
func TestGetTimeseriesNamesFromFile(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)
...
distsender\.errors\..*`,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Call getTimeseriesNamesFromFile
			ctx := context.Background()
			names, err := getTimeseriesNamesFromFile(ctx, mockClient, tmpFile.Name())
			require.NoError(t, err)
