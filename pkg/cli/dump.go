...
func getDumpMetadata(
	...
	if len(tableNames) == 0 {
		...
		for _, table := range tableNames {
			// Try to parse the input name to find qualifications. We do a sneaky
			// trick here where we attempt to parse the input table name as a column
			// name with a fake prefix rather than just a table name to get around
			// limitations in our parser. In particular, the parser cannot parse
			// qualified names individually -- it must parse them as part of a larger
			// statement. This causes qualified names with keywords to require quotes,
			// which pg_dump does not require. Parsing as a column name with a fake
			// prefix allows us to circumvent this limitation.
			stmt, err := parser.ParseOne(fmt.Sprintf("SELECT fakeprefix.%s", table))
			if err != nil {
				return nil, err
			}
			unresolvedName := stmt.AST.(*tree.Select).Select.(*tree.SelectClause).Exprs[0].Expr.(*tree.UnresolvedName)
			var nameParts [3]string
			numParts := unresolvedName.NumParts - 1
			for i := 0; i < numParts; i++ {
				nameParts[i] = unresolvedName.Parts[i]
			}
			tableName, err := tree.NewUnresolvedObjectName(numParts, nameParts, 0 /* annotationIdx */)
			if err != nil {
				return nil, err
			}

			dumpTable := dumpTable{table: tableName.Object()}
			switch tableName.NumParts {
			case 1:
				// If there is no qualification, then the table is assumed to be in
				// the public schema.
				dumpTable.schema = tree.PublicSchema
			case 2:
				dumpTable.schema = tableName.Schema()
			default:
				return nil, errors.Newf("cannot qualify name with database: %s", tableName)
			}
			dumpTables = append(dumpTables, dumpTable)
		}
	}
