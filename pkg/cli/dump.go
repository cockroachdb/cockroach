...
func runDump(cmd *cobra.Command, args []string) error {
	...
	for _, dbName := range dbNames {
		...
		// Dump schema create statements, if any.
...
`
	...
	if err != nil {
		// On versions before 20.1, the cluster won't have the crdb_is_user_defined
...
func getDumpMetadata(
	...
	if len(tableNames) == 0 {
		...
		for _, table := range tableNames {
			// Try to parse the input as a table name.
			tableName, err := parser.ParseTableName(table)
			if err != nil {
				// If we get an error parsing, then just try to treat the input string
				// as the base name. We have to do this because of limitations in parsing.
				// Our parser isn't able to parse names individually, but must parse them
				// as part of a larger statement. This makes it unable to parse input names
				// like `unique` that are reserved keywords without quotes. Postgres does
				// not require these table names to be quoted when input to pg_dump, so
				// attempt to work around that behavior here.
				tableName, err = tree.NewUnresolvedObjectName(1, [3]string{table}, 0)
				if err != nil {
					return nil, err
				}
			}
