...
func TestDebugJobTrace(t *testing.T) {
	...
	targetFile := "tracetest"
	args := []string{strconv.Itoa(int(id)), targetFile}
	pgURL, _ := sqlutils.PGUrl(t, tc.Server(0).ServingSQLAddr(),
	...
	targetFilePath := filepath.Join(jobTraceDir, targetFile)
	...
	exp := [][]string{
		jobTraceHeader,
		{`\d+`, `\d+`, operationName, ".*", ".*", "sql.distsqlrun.ComponentStats", ".*"},
		{`\d+`, `\d+`, operationName, ".*", ".*", "sql.distsqlrun.ComponentStats", ".*"},
	...
	require.NoError(t, os.RemoveAll(targetFilePath))
}
