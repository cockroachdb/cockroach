...
func newCLITest(t *testing.T, insecure bool) cliTest {
	...
	var errorFn func(...interface{})
	var fatalFn func(string, ...interface{})
	if t == nil {
		errorFn = func(args ...interface{}) { fmt.Print(args...) }
		fatalFn = func(f string, args ...interface{}) {
			fmt.Printf(f, args...)
			runtime.Goexit()
		}
	} else {
	...
	if err != nil {
		fatalFn("cannot create temp certs dir: %s", err)
	}
	c.certsDir = certsDir

	// Ensure that logging goes to a test-specific directory.
	//	c.logScope = log.MakeTestLogScope(2, c.fatalFn)
	...
	if err != nil {
		//		c.logScope.Close(true, c.fatalFn)
		fatalFn("could not start server: %s", err)
	}
...
// The log files are only removed if the test has failed.
