...
func Example_sql_table() {
	...
	c.RunWithArgs([]string{"sql", "--escape-special-chars", "--format=pretty", "-e", "select * from t.t"})
	c.RunWithArgs([]string{"sql", "--escape-special-chars", "--format=tsv", "-e", "select * from t.t"})
	c.RunWithArgs([]string{"sql", "--escape-special-chars", "--format=csv", "-e", "select * from t.t"})
	c.RunWithArgs([]string{"sql", "--escape-special-chars", "--format=sql", "-e", "select * from t.t"})
	c.RunWithArgs([]string{"sql", "--escape-special-chars", "--format=html", "-e", "select * from t.t"})
	c.RunWithArgs([]string{"sql", "--escape-special-chars", "--format=raw", "-e", "select * from t.t"})
	c.RunWithArgs([]string{"sql", "--escape-special-chars", "--format=records", "-e", "select * from t.t"})
	...
	//   | 12	123123213	12313
	// d | tabs
	// sql --escape-special-chars --format=pretty -e select * from t.t
	// +--------------------------------+--------------------------------+
	// |               s                |               d                |
	// +--------------------------------+--------------------------------+
	// | foo                            | printable ASCII                |
	...
	// +--------------------------------+--------------------------------+
	// (9 rows)
	// sql --escape-special-chars --format=tsv -e select * from t.t
	// s	d
	// foo	printable ASCII
	// "\""foo"	printable ASCII with quotes
	// \\foo	printable ASCII with backslash
	...
	// a\tb\tc\n12\t123123213\t12313	tabs
	// # 9 rows
	// sql --escape-special-chars --format=csv -e select * from t.t
	// s,d
	// foo,printable ASCII
	// "\""foo",printable ASCII with quotes
	// \\foo,printable ASCII with backslash
	...
	// a\tb\tc\n12\t123123213\t12313,tabs
	// # 9 rows
	// sql --escape-special-chars --format=sql -e select * from t.t
	// CREATE TABLE results (
	//   s STRING,
	//   d STRING
	// );
	...
	// INSERT INTO results VALUES (e'\\u070885', 'UTF8 string with RTL char');
	// INSERT INTO results VALUES (e'a\\tb\\tc\\n12\\t123123213\\t12313', 'tabs');
	// sql --escape-special-chars --format=html -e select * from t.t
	// <table>
	// <thead><tr><th>row</th><th>s</th><th>d</th></tr></head>
	// <tbody>
	// <tr><td>1</td><td>foo</td><td>printable ASCII</td></tr>
	...
	// </tbody>
	// <tfoot><tr><td colspan=3>9 rows</td></tr></tfoot></table>
	// sql --escape-special-chars --format=raw -e select * from t.t
	// # 2 columns
	// # row 1
	// ## 3
	// foo
	...
	// tabs
	// # 9 rows
	// sql --escape-special-chars --format=records -e select * from t.t
	// -[ RECORD 1 ]
	// s | foo
	// d | printable ASCII
	// -[ RECORD 2 ]
