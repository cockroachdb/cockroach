...
func TestHttpHostFlagValue(t *testing.T) {
	...
	testData := []struct {
		...
		{[]string{"start", "--http-addr=127.0.0.1", "--unencrypted-localhost-http"}, "localhost:" + base.DefaultHTTPPort, false},
		{[]string{"start", "--http-addr=:1234", "--unencrypted-localhost-http"}, "localhost:1234", false},
		{[]string{"start", "--listen-addr=blah:1111", "--unencrypted-localhost-http"}, "localhost:" + base.DefaultHTTPPort, false},
	}
