...
func TestSQLPodStorageDefaults(t *testing.T) {
	...
	expectedDefaultDir, err := base.GetAbsoluteStorePath("", "cockroach-data-tenant")
	if err != nil {
		t.Fatal(err)
	}
...
func Test_tenantIDFromFileWrapper_setTenantIDFromFile(t *testing.T) {
	...
	t.Run("non-existent file", func(t *testing.T) {
		w := &tenantIDFromFileWrapper{tenantIDFromFile: "non-existent-file"}
		require.ErrorContains(t, w.setTenantIDFromFile(&cfgTenantID), "no such file or directory")
	})
	...
	t.Run("file exists, has complete first row, but the value is an invalid tenant id",
		func(t *testing.T) {
			...
			defer file.Close()
			w := &tenantIDFromFileWrapper{tenantIDFromFile: file.Name()}
			require.ErrorContains(t, w.setTenantIDFromFile(&cfgTenantID), "invalid tenant ID: strconv.ParseUint")
		})
	...
	t.Run("file exists, has incomplete first row, waits for it to complete and after completion with invalid tenant id fails",
		func(t *testing.T) {
			...
			defer file.Close()
			w := &tenantIDFromFileWrapper{tenantIDFromFile: file.Name()}
			require.EqualValues(t, 0, w.watcherWaitCount.Load())
			generatedError := atomic.Bool{}
			go func() {
				require.ErrorContains(t, w.setTenantIDFromFile(&cfgTenantID), "invalid tenant ID: strconv.ParseUint")
				generatedError.Store(true)
			}()
			require.Eventually(t, func() bool { return w.watcherWaitCount.Load() == 1 }, time.Second, time.Millisecond)
			require.EqualValues(t, 0, w.watcherEventCount.Load())
			require.Equal(t, false, generatedError.Load())
			require.NoError(t, os.WriteFile(file.Name(), []byte("abc\n"), 0777))
			require.Eventually(t, func() bool { return w.watcherEventCount.Load() == 1 }, time.Second, time.Millisecond)
	...
	t.Run("file exists, has complete first row, it has tenant id and it is set to valid value",
		func(t *testing.T) {
			...
			defer file.Close()
			w := &tenantIDFromFileWrapper{tenantIDFromFile: file.Name()}
			require.NoError(t, w.setTenantIDFromFile(&cfgTenantID))
...
func Test_tenantIDFromFileWrapper_Set(t *testing.T) {
	defer leaktest.AfterTest(t)()
	defer log.Scope(t).Close(t)

	t.Run("handle tenant-id-from-file",
		func(t *testing.T) {
			var delayedSetFunc func(*roachpb.TenantID) error
			w := &tenantIDFromFileWrapper{delayedSetFunc: &delayedSetFunc}
			require.NoError(t, w.Set("@/some/file"))
			require.NotNil(t, delayedSetFunc)
		})
