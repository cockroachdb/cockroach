...
func TestClientURLFlagEquivalence(t *testing.T) {
	...
	testData := []struct {
		...
		{anyNonSQLShell, []string{"--url=postgresql:///foo"}, nil, "command .* does not accept a database name"},

		{anySQL, []string{"--url=postgresql://foo@"}, []string{"--user=foo"}, ""},
		{anyNonSQL, []string{"--url=postgresql://foo@"}, nil, "command .* does not accept user/password details"},
