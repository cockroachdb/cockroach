...
func maybeShowTimes(
	...
	backgroundLat := serviceLat - parseLat - planLat - execLat
	if sqlCtx.verboseTimings {
		fmt.Fprintf(w, " (parse %s / plan %s / exec %s / background %s / network %s)\n",
			parseLat, planLat, execLat, backgroundLat, networkLat)
		...
		sep := " ("
		maybeReport := func(label string, lat time.Duration) {
			percentage := lat.Seconds() * 100 / totalSeconds
			if percentage < 1 /*%*/ {
				return
			}
			fmt.Fprintf(w, "%s%s %.1f%%", sep, label, percentage)
			sep = " / "
		}
		maybeReport("parse", parseLat)
		maybeReport("plan", planLat)
		maybeReport("exec", execLat)
		maybeReport("background", backgroundLat)
		maybeReport("network", networkLat)
		if sep != "( " {
			fmt.Fprint(w, ")")
		}
		fmt.Fprintln(w)
