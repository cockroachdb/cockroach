...
type TableRegistryConfig struct {
	...
	// NB: the ordering of the columns should match the unredacted column ordering
	// as much as possible, to provide a consistent experience across
...
func (r DebugZipTableRegistry) QueryForTable(tableName string, redact bool) (string, error) {
	...
	for i, colName := range tableConfig.nonSensitiveCols {
		...
		colsString.WriteString(tree.NameString(colName))
...
var zipInternalTablesPerCluster = DebugZipTableRegistry{
	"crdb_internal.cluster_contention_events": {
		// `key` column contains the contended key, which may contain sensitive row-level data.
		nonSensitiveCols: NonSensitiveColumns{
			"table_id",
			"index_id",
	...
	"crdb_internal.cluster_locks": {
		// `lock_key` column contains the txn lock key, which may contain sensitive row-level data.
		// `lock_key_pretty` column contains the pretty-printed txn lock key, which may contain sensitive
		//  row-level data.
		nonSensitiveCols: NonSensitiveColumns{
			"range_id",
			"table_id",
	...
	"crdb_internal.cluster_settings": {
		customQueryRedacted: `SELECT * FROM (
		    ...
		    SELECT ` + tree.NameString("variable") +
			`, '<redacted>' as value, ` +
			tree.NameString("type") +
			`, ` + tree.NameString("public") +
			`, ` + tree.NameString("description") +
			` FROM crdb_internal.cluster_settings 
			WHERE ` + tree.NameString("type") + ` = 's'
		)`,
	},
	"crdb_internal.cluster_transactions": {
		// `last_auto_retry_reason` contains error text that may contain sensitive data.
		// `txn_string` contains the transaction key, which may contain sensitive row-level data.
		nonSensitiveCols: NonSensitiveColumns{
			"id",
			"node_id",
			"session_id",
	...
	"crdb_internal.kv_node_status": {
		// `env` column can contain sensitive node environment variable values, such as AWS_ACCESS_KEY.
		nonSensitiveCols: NonSensitiveColumns{
			"node_id",
			"network",
			"address",
	...
	"crdb_internal.transaction_contention_events": {
		// `contending_key` column contains the contended key, which may contain sensitive row-level data.
		nonSensitiveCols: NonSensitiveColumns{
			"collection_ts",
			"blocking_txn_id",
...
var zipInternalTablesPerNode = DebugZipTableRegistry{
	...
	"crdb_internal.gossip_nodes": {
		// `cluster_name` is hashed as we only care to see whether values are identical across nodes.
		customQueryRedacted: `SELECT ` +
			tree.NameString("node_id") +
			`, ` + tree.NameString("network") +
			`, ` + tree.NameString("address") +
			`, ` + tree.NameString("advertise_address") +
			`, ` + tree.NameString("sql_network") +
			`, ` + tree.NameString("sql_address") +
			`, ` + tree.NameString("advertise_sql_address") +
			`, ` + tree.NameString("attrs") +
			`, ` + tree.NameString("locality") +
			`, sha1(cluster_name) as cluster_name` +
			`, ` + tree.NameString("server_version") +
			`, ` + tree.NameString("build_tag") +
			`, ` + tree.NameString("started_at") +
			`, ` + tree.NameString("is_live") +
			`, ` + tree.NameString("ranges") +
			`, ` + tree.NameString("leases") +
...
var zipSystemTables = DebugZipTableRegistry{
	...
	"system.settings": {
		...
		customQueryRedacted: `SELECT * FROM (
    		...
    		WHERE ` + tree.NameString("valueType") + ` <> 's'
    	) UNION (
    	    SELECT ` + tree.NameString("name") +
			`, '<redacted>' as value, ` +
			tree.NameString("lastUpdated") +
			`, ` + tree.NameString("valueType") +
	...
	"system.tenant_settings": {
		customQueryRedacted: `SELECT * FROM (
    		...
    		WHERE ` + tree.NameString("value_type") + ` <> 's'
    	) UNION (
    	    SELECT ` + tree.NameString("tenant_id") +
			`, ` + tree.NameString("name") +
			`, '<redacted>' as value, ` +
			tree.NameString("last_updated") +
			`, ` + tree.NameString("value_type") +
			`, ` + tree.NameString("reason") +
			` FROM system.tenant_settings
			WHERE ` + tree.NameString("value_type") + ` = 's'
    	)`,
	},
