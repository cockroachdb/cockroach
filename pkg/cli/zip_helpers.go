...
// archive (e.g. "debug/nodes/1/details.json"). Patterns containing
// '/' are matched against zipPath; patterns without '/' are matched
// against filename (the base name).
func (fs *fileSelection) isIncluded(filename, zipPath string, ctime, mtime time.Time) bool {
	// To be included, a file must be included in at least one of the retrieval patterns.
	included := fs.shouldIncludeFile(filename, zipPath)
	if !included {
		return false
	}
...
// matchPattern matches a single pattern against the appropriate
// target. If the pattern contains '/', it is matched against the full
// zip path; if the zip path is not available, it falls back to
// matching the last path component of the pattern against the base
// file name. Patterns without '/' are always matched against the base
// file name.
func matchPattern(pattern, filename, zipPath string) bool {
	if strings.Contains(pattern, "/") {
		if zipPath != "" {
			matched, _ := filepath.Match(pattern, zipPath)
			return matched
		}
		// When zipPath is not available, fall back to matching the
		// base component of the path pattern against the filename.
		matched, _ := filepath.Match(filepath.Base(pattern), filename)
		return matched
	}
...
// the full path of the file within the zip archive. Patterns
// containing '/' are matched against zipPath; patterns without '/'
// are matched against filename (the base name).
func (fs *fileSelection) shouldIncludeFile(filename, zipPath string) bool {
	...
	for _, p := range patterns {
		if matchPattern(p, filename, zipPath) {
			included = true
			break
		}
	}
