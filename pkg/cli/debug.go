...
var filterParams = map[filter]struct {
...
func runDebugKeys(cmd *cobra.Command, args []string) error {
	...
	filterOpts := filterParams[debugCtx.filterKeys]
	filter := filterOpts.predicate
	if debugCtx.startKey.Equal(storage.NilKey) {
		debugCtx.startKey = filterOpts.minKey
	}
	if debugCtx.endKey.Equal(storage.NilKey) {
		debugCtx.endKey = filterOpts.maxKey
	}
