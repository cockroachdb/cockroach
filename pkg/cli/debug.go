...
func init() {
	...
	f.StringVar(&debugTimeSeriesDumpOpts.metricsListFile, "metrics-list", "", "text file containing metric names or regex patterns to dump (one per line). Prefixes cr.node., cr.store., and cockroachdb. are automatically stripped if present. When specified, only matching metrics are dumped instead of all metrics.")
