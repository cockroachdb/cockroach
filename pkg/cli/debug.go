...
func runDebugRangeData(cmd *cobra.Command, args []string) error {
	...
	for ; ; iter.Next() {
		...
		if err := debug.PrintKeyValue(engine.MVCCKeyValue{
			Key:   iter.Key(),
			Value: iter.Value(),
		}, debugCtx.sizes); err != nil {
			return err
		}
	}
	return nil
...
func runDebugRangeDescriptors(cmd *cobra.Command, args []string) error {
	...
	return db.Iterate(start, end, func(kv engine.MVCCKeyValue) (bool, error) {
		...
		return false, debug.PrintKeyValue(kv, debugCtx.sizes)
	})
}
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if err != nil {
			...
			fmt.Printf("unable to decode key: %v, assuming it's a roachpb.Key with fake timestamp;\n"+
				"if the result below like garbage, then it likely is:\n\n", err)
			k = engine.MVCCKey{
				Key:       bs[0],
		...
		return debug.PrintKeyValue(engine.MVCCKeyValue{
			Key:   k,
			Value: bs[1],
		}, debugCtx.sizes)
	},
