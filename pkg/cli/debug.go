...
var debugGCCmd = &cobra.Command{
	Use:   "estimate-gc <directory> [range id] [ttl-in-seconds] [intent-age-in-seconds]",
...
func runDebugGCCmd(cmd *cobra.Command, args []string) error {
	...
	intentAgeThresholdSeconds := int64((2 * time.Hour).Seconds())

	if len(args) > 3 {
		var err error
		if intentAgeThresholdSeconds, err = parsePositiveInt(args[2]); err != nil {
			return errors.Wrapf(err, "unable to parse %v as intent age threshold", args[2])
		}
	}
	if len(args) > 2 {
		var err error
	...
	for _, desc := range descs {
		...
		info, err := gc.Run(
			context.Background(),
			&desc, snap,
			now, thresh, time.Second*time.Duration(intentAgeThresholdSeconds), policy,
			gc.NoopGCer{},
