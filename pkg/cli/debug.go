...
var debugMergeLogsOpts = struct {
	...
	keepRedactable: true,
	prefixer:       newFilePrefixer(),
	redactInput:    false,
}

type prefixer interface {
	// Prefix generates prefixes for each file path given a collection of files.
	// Prefixes are generated by removing commmon tokens identified across
	// all files that don't contain specific delimiters.
	Prefix(fileInfos []fileInfo)
}
...
func runDebugMergeLogs(cmd *cobra.Command, args []string) error {
	...
	s, err := newMergedStreamFromPatterns(context.Background(),
		args, o.file, o.program, o.from, o.to, inputEditMode, o.format, o.prefixer)
	if err != nil {
		return err
	}
