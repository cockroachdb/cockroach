...
func runDebugGCCmd(cmd *cobra.Command, args []string) error {
	...
	intentBatchSize := gc.IntentCleanupBatchSize.Default()
	...
	for _, desc := range descs {
		...
		info, err := gc.Run(
			context.Background(),
			&desc, snap,
			now, thresh, gc.RunOptions{IntentAgeThreshold: intentAgeThreshold, MaxIntentCleanupBatch: intentBatchSize}, policy,
			gc.NoopGCer{},
