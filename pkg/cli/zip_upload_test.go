...
func TestUploadZipProfiles(t *testing.T) {
	...
	datadriven.RunTest(t, "testdata/upload/profiles", func(t *testing.T, d *datadriven.TestData) string {
		...
		customerNameArg := "--customer-name=ABC"
		if d.HasArg("skip-customer-name") {
			debugZipUploadOpts.customerName = ""
			customerNameArg = ""
		}
		...
		stdout, err := c.RunWithCapture(
			fmt.Sprintf("debug zip upload %s --dd-api-key=dd-api-key %s %s", debugDir, tags, customerNameArg),
		)
		require.NoError(t, err)
