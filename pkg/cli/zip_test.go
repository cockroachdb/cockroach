...
func TestUnavailableZip(t *testing.T) {
	...
	// Three nodes. We move all the data to nodes 2 and 3 which we
	// subsequently stop to make the cluster fully unavailable.
	tc := testcluster.StartTestClusterWithoutReplication(t,
		3 /*nodes*/, 2 /* dataNodeID */, 3, /*systemNodeID*/
		base.TestClusterArgs{ServerArgs: base.TestServerArgs{Insecure: true}})
	defer tc.Stopper().Stop(ctx)

	// Switch most nodes to make the cluster unavailable.
	tc.StopServer(1)
	tc.StopServer(2)
