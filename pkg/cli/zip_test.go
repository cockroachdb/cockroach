...
func TestUnavailableZip(t *testing.T) {
	...
	t.Skip("this test hangs because 19.2 servers don't shut down cleanly when stores are unavailable")

	// unavailableCh is used by the replica command filter
	// to conditionally block requests and simulate unavailability.
	var unavailableCh, doneCh atomic.Value
	closedCh := make(chan struct{})
	close(closedCh)
	unavailableCh.Store(closedCh)
	closedReaderCh := (<-chan struct{})(closedCh)
	doneCh.Store(closedReaderCh)

	knobs := &storage.StoreTestingKnobs{
		TestingRequestFilter: func(_ roachpb.BatchRequest) *roachpb.Error {
			select {
			case <-unavailableCh.Load().(chan struct{}):
			case <-doneCh.Load().(<-chan struct{}):
			}
			return nil
		},
	}
	...
	stopperCtx, cancel := tc.Stopper().WithCancelOnStop(context.Background())
	defer cancel()
	doneCh.Store(stopperCtx.Done())
...
requesting data for debug/nodes/1/details... writing: debug/nodes/1/details.json
requesting data for debug/nodes/1/gossip... writing: debug/nodes/1/gossip.json
requesting data for debug/nodes/1/enginestats... writing: debug/nodes/1/enginestats.json.err.txt
  ^- resulted in ...
requesting stacks for node 1... writing: debug/nodes/1/stacks.txt
requesting heap profile for node 1... writing: debug/nodes/1/heap.pprof
requesting heap files for node 1... writing: debug/nodes/1/heapprof.err.txt
...
requesting data for debug/nodes/3/enginestats... writing: debug/nodes/3/enginestats.json.err.txt
  ^- resulted in ...
