...
func TestZipJobTrace(t *testing.T) {
	...
	func() {
		...
		importJobID = triggerJobAndWaitForRun(sqlConn, `IMPORT INTO x CSV DATA ('workload:///csv/bank/bank?rows=100&version=1.0.0') WITH detached`)
		importJobID2 = triggerJobAndWaitForRun(sqlConn, `IMPORT INTO x CSV DATA ('workload:///csv/bank/bank?rows=100&version=1.0.0') WITH detached`)
