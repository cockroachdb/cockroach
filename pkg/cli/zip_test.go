...
func TestZipIncludeGoroutineStacks(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			// We use datadriven simply to read the golden output file; we don't actually
			// run any commands. Using datadriven allows TESTFLAGS=-rewrite.
			datadriven.RunTest(t, testutils.TestDataPath(t, "zip", tc.outputFileName),
...
func TestZipIncludeRangeInfo(t *testing.T) {
	...
	// We use datadriven simply to read the golden output file; we don't actually
	// run any commands. Using datadriven allows TESTFLAGS=-rewrite.
	datadriven.RunTest(t, testutils.TestDataPath(t, "zip", "testzip_include_range_info"),
		func(t *testing.T, td *datadriven.TestData) string {
			return out
		},
	)
...
func TestUnavailableZip(t *testing.T) {
	...
	debugZipCommand :=
		"debug zip --concurrency=1 --cpu-profile-duration=0 --include-range-info=true " + os.
			DevNull + " --timeout=.25s"
