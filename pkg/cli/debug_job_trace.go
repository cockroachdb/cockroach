...
func stitchJaegerJSON(
	...
	if cumulativeTraceCollection.Data[0].TraceID != curTraceCollection.Data[0].TraceID {
		return cumulativeTraceCollection, errors.AssertionFailedf("expected traceID of nodeTrace: %d and curTrace: %d to be equal",
	...
	for _, span := range curTraceCollection.Data[0].Spans {
		cumulativeTraceCollection.Data[0].Spans = append(cumulativeTraceCollection.Data[0].Spans, span)
	}
...
func populateInflightTraceRow(vals []driver.Value) (inflightTraceRow, error) {
	...
	if traceJSON, ok := vals[2].(string); ok {
		row.traceJSON = traceJSON
	} else {
		return row, errors.Errorf("unexpected value: %T of %v", vals[1], vals[1])
	}

	if traceStr, ok := vals[3].(string); ok {
		row.traceStr = traceStr
	} else {
		return row, errors.Errorf("unexpected value: %T of %v", vals[2], vals[2])
	}

	if jaegerJSON, ok := vals[4].(string); ok {
...
SELECT node_id, root_op_name, trace_json, trace_str, jaeger_json FROM crdb_internal.cluster_inflight_traces WHERE trace_id=$1 ORDER BY node_id
`
	...
	var traceStrBuf, traceJSONBuf bytes.Buffer
	var nodeTraceCollection *tracing.TraceCollection
	flushAndReset := func(ctx context.Context, nodeID int64) {
		z.AddFile(constructFilename(nodeID, "trace.json"), traceJSONBuf.String())
	...
	for {
		...
		_, err = traceJSONBuf.WriteString(fmt.Sprintf("\n\n-- Root Operation: %s --\n\n", row.rootOpName))
		if err != nil {
			return err
		}
		_, err = traceJSONBuf.WriteString(row.jaegerJSON)
		if err != nil {
			return err
		}
		prevNodeID = row.nodeID
