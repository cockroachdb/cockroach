...
func runDebugJobTrace(_ *cobra.Command, args []string) error {
	...
	return writeTracePayloadJSON(sqlConn, jobID, traceJSONFile)
}

func getJobTraceID(sqlConn *sqlConn, jobID int) (int64, error) {
	var traceID int64
	rows, err := sqlConn.Query(`SELECT trace_id FROM crdb_internal.jobs WHERE job_id=$1`, []driver.Value{int64(jobID)})
	if err != nil {
		return traceID, err
	}
...
func writeTracePayloadJSON(sqlConn *sqlConn, jobID int, traceFile string) error {
