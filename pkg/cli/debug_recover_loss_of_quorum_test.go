...
func (e *quorumRecoveryEnv) handleMakePlan(t *testing.T, _ datadriven.TestData) (string, error) {
	var rangeInfo []quorum.LocalRangeInfo
	for _, nodeRanges := range e.replicas {
		rangeInfo = append(rangeInfo, nodeRanges...)
	}
	var err error
	e.plan, err = planReplicas(rangeInfo)
	if err != nil {
		return "", err
	}
...
func (e *quorumRecoveryEnv) parseStoresArg(t *testing.T, d datadriven.TestData) []roachpb.StoreID {
...
func (e *quorumRecoveryEnv) handleApplyPlan(t *testing.T, d datadriven.TestData) (string, error) {
	...
	for nodeID, stores := range nodes {
		if err := removeReplicas(ctx, e.plan, nodeID, stores, func(s string) (bool, error) { return true, nil }); err != nil {
			return "", err
		}
	}
	return "ok", nil
