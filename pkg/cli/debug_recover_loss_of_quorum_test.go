...
func TestCollectInfoFromMultipleStores(t *testing.T) {
	...
	tc := testcluster.NewTestCluster(t, 3, base.TestClusterArgs{
		...
		ServerArgsPerNode: map[int]base.TestServerArgs{
			0: {StoreConfig: storagepb.NodeConfig{Stores: []storagepb.StoreSpec{{Path: dir + "/store-1"}}}},
			1: {StoreConfig: storagepb.NodeConfig{Stores: []storagepb.StoreSpec{{Path: dir + "/store-2"}}}},
			2: {StoreConfig: storagepb.NodeConfig{Stores: []storagepb.StoreSpec{{Path: dir + "/store-3"}}}},
		},
	})
	tc.Start(t)
