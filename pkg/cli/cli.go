...
func init() {
	// If we're not logging to stderr, the logging system will redirect stderr to
	// a file. We want error messages generated by the sql shell to always go to
	// the original stderr.
	newFd, err := syscall.Dup(int(os.Stderr.Fd()))
	if err != nil {
		panic(err)
	}
	stderr = os.NewFile(uintptr(newFd), "stderr")
}
