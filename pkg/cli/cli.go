		...
		go func() {
			...
			for {
				...
				if !started {
					...
					if err != nil {
						log.Shoutf(context.Background(), severity.WARNING, "debug server could not start listening on %s: %v", listenAddr, err)
						continue
					}

					server := http.Server{Handler: mux}
					go server.Serve(listener)
					log.Shoutf(context.Background(), severity.INFO, "debug server listinging on %s", listener.Addr())
					started = true
				}
			}
		}()
