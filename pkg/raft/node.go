...
func (n *node) run() {
	...
	for {
		...
		case m := <-n.recvc:
			if IsResponseMsg(m.Type) && !IsLocalMsgTarget(m.From) && r.progressTracker.Progress()[m.From] == nil {
				// Filter out response message from unknown From.
				break
			}
			r.Step(m)
		case cc := <-n.confc:
			_, okBefore := r.progressTracker.Progress()[r.id]
			...
			if _, okAfter := r.progressTracker.Progress()[r.id]; okBefore && !okAfter {
				var found bool
