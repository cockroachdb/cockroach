...
type Node interface {
	...
	// the Ready() channel, handle Ready, and call Advance (if required), before
	// reading from AcceptReady() again.
...
func (n *node) run() {
	...
	for {
		...
		case acceptReadyC <- struct{}{}:
			...
			rd = n.rn.Ready()
			n.readyW.resp <- rd // never blocks because of contract on Node.Ready()
			if n.rn.asyncStorageWrites {
				rd = Ready{} // don't reference any memory after Ready is handed over
			} else {
