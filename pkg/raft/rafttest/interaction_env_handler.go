...
func (env *InteractionEnv) Handle(t *testing.T, d datadriven.TestData) string {
	...
	case "maybe-send-de-fortify":
		// Testing hook into (*raft).maybeSendDeFortify. Takes 2 nodes -- the
		// leader, which will be de-fortified, and the follower that's going to
		// de-fortify.
		//
		// maybe-send-de-fortify lead_id peer_id
		// Arguments are:
