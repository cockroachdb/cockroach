...
func (l *raftLog) commitTo(mark logMark) {
	...
	// mark.term entry as committed, all future leaders have it in the log. It is
	// thus safe to bump the commit index if accTerm >= mark.term.
...
func (l *raftLog) maybeCommit(at logMark) bool {
	...
	// term match.
	if at.term != 0 && at.index > l.committed && l.matchTerm(entryID(at)) {
		l.commitTo(at)
		return true
	}
	return false
