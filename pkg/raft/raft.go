...
func (r *raft) bcastFortify() {
	Assert(r.state == StateLeader, "only leaders can fortify")
...
func (r *raft) handleFortifyResp(m pb.Message) {
	Assert(r.state == StateLeader, "only leaders should be handling fortification responses")
	Assert(m.Reject, "TODO(arul): implement")
}
