...
func (r *raft) handleHeartbeat(m pb.Message) {
	r.checkMatch(m.Match)
	// The m.Term leader indicates to us that indices <= m.Commit in its log are
	// committed. If our log matches the leader's up to index M, then we can
	// update our commit index to min(m.Commit, M).
	//
