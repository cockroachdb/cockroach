...
// by the code are exposed (e.g. Uint64n).
type lockedRand struct {
	mu sync.Mutex
}

func (r *lockedRand) Uint64n(n uint64) uint64 {
	r.mu.Lock()
...
func (r *raft) resetRandomizedElectionTimeout() {
	r.randomizedElectionTimeout = r.electionTimeout + globalRand.Uint64n(r.electionTimeout)
}
