...
// store liveness problem with the follower.
func (r *raft) maybeSendFortify(id pb.PeerID) {
	...
	if !r.supportTracker.IsSuppotedBy(id, curEpoch) {
...
// peers (including the leader itself) whose stores are currently providing
// store liveness support to the leader's store but who have not provided
