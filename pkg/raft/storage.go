...
func (ms *MemoryStorage) Entries(lo, hi, maxSize uint64) ([]pb.Entry, error) {
	...
	if hi > ms.lastIndex()+1 {
		logger.GetLogger().Panicf("entries' hi(%d) is out of bound lastindex(%d)", hi, ms.lastIndex())
	}
...
func (ms *MemoryStorage) CreateSnapshot(
	...
	offset := ms.ents[0].Index
	if i > ms.lastIndex() {
		logger.GetLogger().Panicf("snapshot %d is out of bound lastindex(%d)", i, ms.lastIndex())
	}
...
func (ms *MemoryStorage) Compact(compactIndex uint64) error {
	...
	if compactIndex > ms.lastIndex() {
		logger.GetLogger().Panicf("compact %d is out of bound lastindex(%d)", compactIndex, ms.lastIndex())
	}
