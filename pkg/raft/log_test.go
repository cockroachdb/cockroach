...
func TestIsUpToDate(t *testing.T) {
	...
	raftLog := newLog(NewMemoryStorage(), logger.DiscardLogger)
...
func TestAppend(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		t.Run("", func(t *testing.T) {
			...
			raftLog := newLog(storage, logger.DiscardLogger)
...
func TestAcceptApplying(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		t.Run("", func(t *testing.T) {
			...
			raftLog := newLogWithSize(storage, logger.DiscardLogger, maxSize)
			require.True(t, raftLog.append(init))
			require.NoError(t, storage.Append(init.entries[:1]))
...
func TestStableToWithSnap(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		t.Run("", func(t *testing.T) {
			...
			raftLog := newLog(s, logger.DiscardLogger)
