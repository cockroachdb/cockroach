...
// clone creates a new complete copy of the term cache.
func (tc *termCache) clone() termCache {
	tcCopy := termCache{
		cache:   make([]entryID, len(tc.cache), tc.maxSize),
		maxSize: tc.maxSize,
	}
	copy(tcCopy.cache, tc.cache)
	return tcCopy
}
