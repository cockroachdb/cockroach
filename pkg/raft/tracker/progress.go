...
type Progress struct {
	...
	// receivedCommit is the highest commit index received from the follower.
...
func (pr *Progress) IsFollowerCommitStale(index uint64) bool {
	return index > pr.receivedCommit
}
...
// MaybeUpdateReceivedCommit updates the received commit from a follower if it's
// larger than the previous received commit.
func (pr *Progress) MaybeUpdateReceivedCommit(commit uint64) {
	if commit > pr.receivedCommit {
		pr.receivedCommit = commit
	}
}
...
// making sure to advance the followers' commit index. If it's false, it means
// that the responsibility is primarily on MsgHeartbeat.
func (pr *Progress) ShouldSendMsgApp(last, commit uint64, advanceCommit bool) bool {
	...
	case StateReplicate:
		...
		// sending empty MsgApps until we have sent the latest commit index to the
		// follower.
