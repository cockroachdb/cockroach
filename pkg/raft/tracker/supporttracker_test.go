...
func TestLeadSupportUntil(t *testing.T) {
	...
	testCases := []struct {
		...
		run           func(t *testing.T, supportTracker SupportTracker)
	}{
		{
			ids:           []pb.PeerID{1, 2, 3},
			storeLiveness: mockLiveness3Peers,
			run: func(t *testing.T, supportTracker SupportTracker) {
				// No support's been recorded yet; empty timestamp.
				require.Equal(t, hlc.Timestamp{}, supportTracker.LeadSupportUntil())
			},
		},
		{
			ids:           []pb.PeerID{1, 2, 3},
			...
			run: func(t *testing.T, supportTracker SupportTracker) {
				...
				// Haven't received support from a quorum; empty timestamp.
				require.Equal(t, hlc.Timestamp{}, supportTracker.LeadSupportUntil())
			},
		},
		{
			...
			run: func(t *testing.T, supportTracker SupportTracker) {
				...
				require.Equal(t, ts(10), supportTracker.LeadSupportUntil())
				// Record support from peer 2; the support until should advance.
				supportTracker.RecordSupport(2, 20)
				require.Equal(t, ts(15), supportTracker.LeadSupportUntil())
			},
		},
		{
			ids:           []pb.PeerID{1, 2, 3},
