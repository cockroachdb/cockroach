...
type FortificationTracker struct {
	...
	// meaningful only when steppingDown is true. It's useful in some cases where
	// the leader needs to know the term for which it's stepping down. For
	// example, if there is a stranded follower at a higher term, the leader can
	// step down to the stranded follower's term to allow the follower to join
	// the quorum.
