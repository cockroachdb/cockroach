...
// InformCommittedTerm informs the fortification tracker that the supplied term
// has been commited.
func (ft *FortificationTracker) InformCommittedTerm(committedTerm uint64) {
	if committedTerm > ft.term {
		...
		// elected at term T' > T, and it has committed a log entry with term T'.
		// We're in this case -- save some state so we can safely say no the next
		// time we're asked whether we need to de-fortify or not.
