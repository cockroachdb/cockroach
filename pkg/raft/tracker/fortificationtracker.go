...
func (ft *FortificationTracker) ComputeLeadSupportUntil(state pb.StateType) {
	...
	//
	// We try to avoid unnecessary copying by creating the slice here, and making
	// PopulateMajorityConfigSupport() to populate it in place.
...
// expiration.
//
// This function expects the slice to be of the same size as the majority
// config.
func (ft *FortificationTracker) PopulateMajorityConfigSupport(
	majorityConfig quorum.MajorityConfig,
	support []hlc.Timestamp,
) {
	...
	for id := range majorityConfig {
		if supportEpoch, ok := ft.fortification[id]; ok {
			...
			if curEpoch == supportEpoch {
				...
				// We fill from the right (since the zeros will end up on the left after
				// sorting anyway).
				support[i] = curExp
				i--
			}
