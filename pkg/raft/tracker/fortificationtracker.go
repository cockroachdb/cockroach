...
func (ft *FortificationTracker) ComputeLeadSupportUntil(state pb.StateType) {
	...
	//
	// We try to prevent the slice from escaping to the heap by allocating here
	// and having PopulateMajorityConfigSupport() to populate it in place.
...
// expiration.
//
// If a peer has not fortified the leader, if we have no entry for it in
