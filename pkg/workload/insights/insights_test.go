...
func TestInsights(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(fmt.Sprintf("rows=%d/ranges=%d", test.rows, test.ranges), func(t *testing.T) {
			...
			sqlDB.Exec(t, `DROP TABLE IF EXISTS insights`)

			insights := FromConfig(test.rows, test.rows, defaultPayloadBytes, test.ranges)
			insightsTable := insights.Tables()[0]
			sqlDB.Exec(t, fmt.Sprintf(`CREATE TABLE %s %s`, insightsTable.Name, insightsTable.Schema))

			if err := workloadsql.Split(ctx, db, insightsTable, 1 /* concurrency */); err != nil {
				t.Fatalf("%+v", err)
			}

			var rangeCount int
			sqlDB.QueryRow(t,
				fmt.Sprintf(`SELECT count(*) FROM [SHOW RANGES FROM TABLE %s]`, insightsTable.Name),
			).Scan(&rangeCount)
