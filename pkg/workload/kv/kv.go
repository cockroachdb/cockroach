...
func (w *kv) Ops(urls []string, reg *histogram.Registry) (workload.QueryLoad, error) {
	...
	if w.shards == 0 {
		...
		// Manually plumb down the computed shard column since the optimizer doesn't yet
		// support deriving values of computed columns when all the columns they reference
		// are available. See
