...
var kvMeta = workload.Meta{
	...
	New: func() workload.Generator {
		...
		g.flags.Meta = map[string]workload.FlagMeta{
			...
			`contention`:            {RuntimeOnly: true},
			`contention-num-writes`: {RuntimeOnly: true},
		...
		g.flags.BoolVar(&g.contention, `contention`, false,
		...
		g.flags.IntVar(&g.contentionNumWrites, `contention-num-writes`, 10,
			`Number of writes in the long-running write transaction when using --contention.`)
...
func (o *kvOp) run(ctx context.Context) (retErr error) {
	...
	start := timeutil.Now()
	var err error
	if o.config.writesUseSelect1 || o.config.writesUseSelectForUpdate || o.config.contention {
		...
		iterations := 1
		if o.config.contention {
			iterations = o.config.contentionNumWrites
		}
