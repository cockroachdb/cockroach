...
func (w *tpcc) tpccHistoryInitialRowBatch(rowIdx int, cb coldata.Batch, a *bufalloc.ByteAllocator) {
	...
	//
	// So how do we accomplish (2)? Well we already have a unique integer per row,
	// so we could just map that to the uuids, say by setting the first 8 bytes.
	// However, since V4 uuids are generated by simply filling 16 bytes with
	// random data (then setting the version and variant), they're randomly
	// distributed through the space of possible values. This also means they're
	// roughly evenly distributed. We'd like our values to be similarly
	// distributed.
	//
	// So, instead, we space the row indexes out to fill the space of the integers
	...
	spacedRowIndex := uint64(float64(rowIdx) * float64(math.MaxUint64) / float64(historyRowCount))
	binary.BigEndian.PutUint64(l.uuidAlloc[0:8], spacedRowIndex)
	binary.BigEndian.PutUint64(l.uuidAlloc[8:16], uint64(rowIdx))
	l.uuidAlloc.SetVersion(uuid.V4)
	l.uuidAlloc.SetVariant(uuid.VariantRFC4122)
	var rowID []byte
