...
const (
	...
	// replicas of each range in the partition are stored in the zone.
	partitionedReplication partitionStrategy = iota
	// The partitionedReplication strategy collocates read leases for a given
	// partition to within a single zone. It does so by configuring lease
	// preferences on each range in the partition to prefer the same zone.
