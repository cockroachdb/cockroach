...
func replicateItem(db *gosql.DB, cfg zoneConfig, partitions int) error {
	var constraints []string
	if len(cfg.zones) == 0 {
		// Assume we have partitions number of racks which are zero indexed.
...
func partitionTables(db *gosql.DB, cfg zoneConfig, wPart *partitioner) error {
	...
	return replicateItem(db, cfg, wPart.parts)
}
