...
func partitionObject(
	...
	for i := 0; i < p.parts; i++ {
		if err := configureZone(ctx, db, cfg, table, fmt.Sprintf("p%d_%d", idx, i), i, p.parts); err != nil {
			return err
		}
	}
	return nil
...
func indexExists(ctx context.Context, db *gosql.Conn, table, index string) (bool, error) {
