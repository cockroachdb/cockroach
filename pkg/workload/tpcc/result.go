...
// It should only be used if the number of ActiveWarehouses matches. Elapsed
// is computed as the max of all elapsed value and makes the assumption that
// the distibutions roughly overlap in time. This should be used when merging
// from multiple load generators during the same test run.
	...
	for _, r := range results {
		...
		for q, h := range r.Cumulative {
			if cur, exists := base.Cumulative[q]; exists {
				...
				base.Cumulative[q] = histogram.Clone(h)
			}
		}
	}
	return base
...
// NewResultFromSnapshots creates a new result from a deserialized set of
// histogram snapshots.
...
// NewResultFromRegistry creates a Result from a registry.
func NewResultFromRegistry(
	activeWarehouses int, warehouseFactor float64, reg *histogram.Registry,
) *Result {
	ret := make(map[string]*hdrhistogram.Histogram)
	var elapsed time.Duration
	reg.Tick(func(t histogram.Tick) {
		if elapsed == 0 {
			elapsed = t.Now.Sub(reg.Start())
		}
		// Clone the histogram as the registry retains ownership of the passed
		// histogram values.
		ret[t.Name] = histogram.Clone(t.Cumulative)
	})
	return NewResult(activeWarehouses, warehouseFactor, elapsed, ret)
}

// Passes returns true if r is a passing Result.
