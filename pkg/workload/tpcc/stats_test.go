...
func TestTPCCStats(t *testing.T) {
	...
	testStats := func(table, expectedStr string, actual []stats.JSONStatistic) {
		var expected []stats.JSONStatistic
		...
		for i := 0; i < len(actual); i++ {
			...
			if actual[i].Name != stats.AutoStatsName {
				t.Fatalf("for table %s, expected name %s but found %s",
					table, stats.AutoStatsName, actual[i].Name)
			}
		}
	}
