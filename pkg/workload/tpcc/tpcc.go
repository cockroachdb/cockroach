...
func (w *tpcc) executeTx(
...
) (onTxnStartTime time.Duration, err error) {
	...
	txnFuncWithStartFuncs := func(tx pgx.Tx) (err error) {
		...
		if w.onTxnStartFns != nil {
			...
			onTxnStartTime += timeutil.Since(startTime)
		}
		return fn(tx)
	}

	if w.txnRetries {
		return onTxnStartTime, crdbpgx.ExecuteTx(ctx, conn, txOpts, txnFuncWithStartFuncs)
	}

	tx, err := conn.BeginTx(ctx, txOpts)
	if err != nil {
		return onTxnStartTime, err
	}
	err = txnFuncWithStartFuncs(tx)
	if err != nil {
		return onTxnStartTime, err
	}
	return onTxnStartTime, tx.Commit(ctx)
}
