...
type tpccTx interface {
	...
	run(ctx context.Context, wID int) (txnData interface{}, onTxnStartTime time.Duration, err error)
}
...
func (w *worker) run(ctx context.Context) error {
	...
	start := timeutil.Now()
	_, onTxnStartTime, err := tx.run(context.Background(), warehouseID)
	if err != nil {
	...
	if ctx.Err() == nil {
		...
		// change them now.
		w.hists.Get(txInfo.name).Record(elapsed - onTxnStartTime)
	}
