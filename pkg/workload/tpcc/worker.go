...
type txInfo struct {
	...
	// durationHistory measures the duration of the given txn.
...
func init() {
	for i, tx := range allTxs {
		allTxs[i].successCounter = prometheusMetrics.NewCounter(
			prometheus.CounterOpts{
				Name: fmt.Sprintf("op_%s_success", tx.name),
				Help: fmt.Sprintf("The total number of successful %s events.", tx.name),
			},
		)
		allTxs[i].errorCounter = prometheusMetrics.NewCounter(
			prometheus.CounterOpts{
				Name: fmt.Sprintf("op_%s_error", tx.name),
				Help: fmt.Sprintf("The total number of error %s events.", tx.name),
			},
		)
		allTxs[i].durationHistogram = prometheusMetrics.NewHistogram(
			prometheus.HistogramOpts{
				Name: fmt.Sprintf("op_%s_durationMS", tx.name),
				Help: fmt.Sprintf("Histogram of %s durations.", tx.name),
			},
		)
	}
}
...
func (w *worker) run(ctx context.Context) error {
	...
	if ctx.Err() == nil {
		...
		txInfo.durationHistogram.Observe(float64(elapsed.Nanoseconds()) / float64(time.Millisecond))
