...
func (w *conflictWorker) RunOp(ctx context.Context) (err error) {
	...
	for _, writer := range w.writers {
		group.Go(func() error {
			return w.runOpOnCluster(ctx, writer, row)
		})
	}
...
func (w *conflictWorker) runOpOnCluster(ctx context.Context, writer *writer, row []any) error {
	var err error
	...
	for range w.rng.Int31n(3) {
		if rand.Float64() < 0.8 {
			if rand.Float64() < 0.5 {
				row, err = w.table.MutateRow(w.rng, w.nullPct, row)
				if err != nil {
					return err
				}
			}
