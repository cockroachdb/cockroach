...
func printJobState(ctx context.Context, db *gosql.DB) error {
	rows, err := db.Query(`
...
func waitForDistribution(ctx context.Context, db *gosql.DB) error {
	...
	var prevRangeIDToLeaseholder map[string]string
	for {
		pollAt := timeutil.Now()
		rangeIDToLeaseholder := make(map[string]string)
		rows, err := db.QueryContext(ctx, fmt.Sprintf("SELECT range_id, lease_holder FROM [SHOW RANGES FROM TABLE %s];", ttlTableName))
		if err != nil {
			return err
		}
		for rows.Next() {
			...
			rangeIDToLeaseholder[rangeID] = leaseholder
		}

		log.Infof(ctx, "pollAt=%s pollCount=%d rangeIDToLeaseholder=%s", pollAt.Format(strfmt.RFC3339Millis), pollCount, rangeIDToLeaseholder)

		if !reflect.DeepEqual(prevRangeIDToLeaseholder, rangeIDToLeaseholder) {
			prevRangeIDToLeaseholder = rangeIDToLeaseholder
			prevPollAt = pollAt
...
func (t *ttlBench) Hooks() workload.Hooks {
	return workload.Hooks{
		...
		PostLoad: func(ctx context.Context, db *gosql.DB) error {
			...
			log.Infof(ctx, "TTL set initialExpiredRowCount=%d now=%s\n%s", initialExpiredRowCount, ttlResetTime.Format(strfmt.RFC3339Millis), ttlStatement)
			...
			log.Infof(ctx, "TTL complete pollCount=%d duration=%dms now=%s", pollCount, ttlEndTime.Sub(ttlStartTime).Milliseconds(), ttlEndTime.Format(strfmt.RFC3339Millis))
