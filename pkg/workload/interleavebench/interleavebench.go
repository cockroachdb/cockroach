...
var interleaveBenchMeta = workload.Meta{
	...
	New: func() workload.Generator {
		...
		b.flags.BoolVar(&b.noFKs, `no-fks`, false, `Specifies whether foreign keys are omitted`)
		...
		b.flags.Float64Var(&b.ratio, `ratio`, 100, `Specifies the ratio between the first tables in consequent levels (e.g. # rows in child = 100 x # rows in parent)`)
...
func (b *interleaveBench) Hooks() workload.Hooks {
	return workload.Hooks{
		Validate: func() error {
			...
			// option - we only support concurrency of 1 (this makes it easy
			// to track which rows have already been deleted).
			b.connFlags.Concurrency = 1
			if b.levels < 2 {
		...
		PostLoad: func(db *gosql.DB) error {
			...
			getInsert := func(level, ordinal int, start, end int) string {
				...
				for l := 0; l < level; l++ {
					...
					s += fmt.Sprintf("floor((i-1)*%.2f)::INT+1, ", ancestorRowCount/rowCount)
				}
...
func (b *interleaveBench) getTableRowCount(level, ordinal int) float64 {
	...
	rowCount := float64(b.getTableRowCount(level-1, 0)) * b.ratio
	for i := 0; i < ordinal; i++ {
...
const maxSingleInsertRowCount = 10000
...
func (b *interleaveBench) splitAndScatter(db *gosql.DB, level int) error {
	...
	for ordinal := 0; ordinal < b.tablesPerLevel[level]; ordinal++ {
		splitQuery := fmt.Sprintf(
			...
			b.getTableName(level, ordinal), b.getTableRowCount(0, 0), int(b.getTableRowCount(0, 0)/float64(b.rangesToSplit)),
		)
