...
// in this file that uses scratch space (randPartName).
const maxBytesLength = 55
...
func randVString(rng *rand.Rand, scratch []byte, minLen, maxLen int) []byte {
	buf := scratch[:randInt(rng, minLen, maxLen)]
	for i := range buf {
...
func randPhone(rng *rand.Rand, scratch []byte, nationKey int16) []byte {
	buf := scratch[:0]
...
// by spaces.
func randPartName(rng *rand.Rand, namePerm []int, scratch []byte) []byte {
...
func randMfgr(rng *rand.Rand, scratch []byte) (byte, []byte) {
	buf := append(scratch[:0], manufacturerString...)
	m := byte(rng.Intn(5) + '1')
	buf = append(buf, m)
	return m, buf
}
...
func randBrand(rng *rand.Rand, scratch []byte, m byte) []byte {
	buf := append(scratch[:0], brandString...)
	n := byte(rng.Intn(5) + '1')
	buf = append(buf, m)
	buf = append(buf, n)
	return buf
...
func randClerk(rng *rand.Rand, scratch []byte, scaleFactor int) []byte {
	buf := scratch[:len(clerkString)+9]
...
func supplierName(scratch []byte, suppKey int64) []byte {
	buf := scratch[:len(supplierString)+9]
...
func customerName(scratch []byte, custKey int64) []byte {
	buf := scratch[:len(customerString)+9]
...
func randSyllables(rng *rand.Rand, scratch []byte, syllables [][]string) []byte {
	buf := scratch[:0]
...
func randType(rng *rand.Rand, scratch []byte) []byte {
	return randSyllables(rng, scratch, typeSyllables)
}
...
func randContainer(rng *rand.Rand, scratch []byte) []byte {
	return randSyllables(rng, scratch, containerSyllables)
}
