...
func (w *tpch) tpchRegionInitialRowBatch(batchIdx int, cb coldata.Batch) {
...
func (w *tpch) tpchNationInitialRowBatch(batchIdx int, cb coldata.Batch) {
	l := w.localsPool.Get().(*generateLocals)
	defer w.localsPool.Put(l)
	rng := l.rng
	rng.Seed(w.seed + uint64(batchIdx))
...
func (w *tpch) tpchSupplierInitialRowBatch(batchIdx int, cb coldata.Batch) {
	l := w.localsPool.Get().(*generateLocals)
	defer w.localsPool.Put(l)
	rng := l.rng
	rng.Seed(w.seed + uint64(batchIdx))
	...
	cb.ColVec(1).Bytes().Set(0, supplierName(w.scratch.supplier, suppKey))     // s_name
	cb.ColVec(2).Bytes().Set(0, randVString(rng, w.scratch.supplier, 10, 40))  // s_address
	cb.ColVec(3).Int16()[0] = nationKey                                        // s_nationkey
	cb.ColVec(4).Bytes().Set(0, randPhone(rng, w.scratch.supplier, nationKey)) // s_phone
...
func (w *tpch) tpchPartInitialRowBatch(batchIdx int, cb coldata.Batch) {
	l := w.localsPool.Get().(*generateLocals)
	defer w.localsPool.Put(l)
	rng := l.rng
	rng.Seed(w.seed + uint64(batchIdx))
	...
	cb.ColVec(1).Bytes().Set(0, randPartName(rng, l.namePerm, w.scratch.part))
	m, mfgr := randMfgr(rng, w.scratch.part)
	...
	cb.ColVec(3).Bytes().Set(0, randBrand(rng, w.scratch.part, m))
	// P_TYPE random string [Types].
	cb.ColVec(4).Bytes().Set(0, randType(rng, w.scratch.part))
	// P_SIZE random value [1 .. 50].
	cb.ColVec(5).Int16()[0] = int16(randInt(rng, 1, 50))
	// P_CONTAINER random string [Containers].
	cb.ColVec(6).Bytes().Set(0, randContainer(rng, w.scratch.part))
...
func (w *tpch) tpchPartSuppInitialRowBatch(batchIdx int, cb coldata.Batch) {
	l := w.localsPool.Get().(*generateLocals)
	defer w.localsPool.Put(l)
	rng := l.rng
	rng.Seed(w.seed + uint64(batchIdx))
...
func (w *tpch) tpchCustomerInitialRowBatch(batchIdx int, cb coldata.Batch) {
	l := w.localsPool.Get().(*generateLocals)
	defer w.localsPool.Put(l)
	rng := l.rng
	rng.Seed(w.seed + uint64(batchIdx))
	...
	cb.ColVec(1).Bytes().Set(0, customerName(w.scratch.customer, custKey))
	// C_ADDRESS random v-string [10,40].
	cb.ColVec(2).Bytes().Set(0, randVString(rng, w.scratch.customer, 10, 40))
	nationKey := int16(randInt(rng, 0, 24))
	// C_NATIONKEY random value [0 .. 24].
	cb.ColVec(3).Int16()[0] = nationKey
	// C_PHONE generated according to Clause 4.2.2.9.
	cb.ColVec(4).Bytes().Set(0, randPhone(rng, w.scratch.customer, nationKey))
...
func (w *tpch) tpchOrdersInitialRowBatch(batchIdx int, cb coldata.Batch) {
	l := w.localsPool.Get().(*generateLocals)
	defer w.localsPool.Put(l)
	rng := l.rng
	...
	for i := 0; i < numOrderPerCustomer; i++ {
		...
		clerkCol.Set(i, randClerk(rng, w.scratch.orders, w.scaleFactor))
...
func (w *tpch) tpchLineItemInitialRowBatch(batchIdx int, cb coldata.Batch) {
	l := w.localsPool.Get().(*generateLocals)
	defer w.localsPool.Put(l)
	rng := l.rng
