...
func (w *tpch) tpchOrdersInitialRowBatch(
	...
	for i := 0; i < numOrderPerCustomer; i++ {
		...
		for j := 0; j < l.orderData.nOrders; j++ {
			...
			// identical behaivor on all platforms. See https://golang.org/ref/spec#Floating_point_operators
			totalPrice += float32(ep * (1 + l.orderData.tax[j]) * (1 - l.orderData.discount[j])) //nolint:roachvet
		}
