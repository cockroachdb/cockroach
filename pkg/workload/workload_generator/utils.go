...
// GenType is an enum for all the data generator types.
type GenType string

const (
	GenTypeSequence  GenType = "sequence"
	GenTypeInteger   GenType = "integer"
	GenTypeUUID      GenType = "uuid"
	GenTypeBit       GenType = "bit"
	GenTypeBytes     GenType = "bytes"
	GenTypeString    GenType = "string"
	GenTypeFloat     GenType = "float"
	GenTypeDate      GenType = "date"
	GenTypeTimestamp GenType = "timestamp"
	GenTypeBool      GenType = "bool"
)
...
// decanonical undoes canonical() for at most two components. IT is used later I think, so I am commenting out for this pr.
...
// data generators.
func mapSQLType(sql string, col *Column, rng *rand.Rand) (GenType, map[string]any) {
...
func mapIntegerType(sql string, col *Column, args map[string]any) (GenType, map[string]any) {
...
func mapBitType(sql string, col *Column, args map[string]any) (GenType, map[string]any) {
...
func mapByteType(sql string, col *Column, args map[string]any) (GenType, map[string]any) {
	args["size"] = 1
	return GenTypeBytes, args
}

func mapVarcharType(sql string, col *Column, args map[string]any) (GenType, map[string]any) {
...
func mapCharType(sql string, col *Column, args map[string]any) (GenType, map[string]any) {
	n := atoi(charRe.FindStringSubmatch(sql)[1])
	setArgsRange(args, n, n)
	return GenTypeString, args
}

func mapDecimalType(sql string, col *Column, args map[string]any) (GenType, map[string]any) {
...
func mapPlainStringType(sql string, col *Column, args map[string]any) (GenType, map[string]any) {
	setArgsRange(args, 5, 30)
	return GenTypeString, args
}

func mapFloatType(sql string, col *Column, args map[string]any) (GenType, map[string]any) {
	setArgsRange(args, 0, 1)
	args["round"] = 2
	return GenTypeFloat, args
}

func mapDateType(sql string, col *Column, args map[string]any) (GenType, map[string]any) {
...
func mapTimestampType(sql string, col *Column, args map[string]any) (GenType, map[string]any) {
	args["start"] = "2000-01-01"
	args["end"] = "2025-01-01"
