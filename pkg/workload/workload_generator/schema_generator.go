...
func processColumnDefs(table *TableSchema, columnDefs []string) {
	...
	for _, columnDef := range columnDefs {
		...
		// While adding columns to teh order, surrounding quotes are stripped if any.
		// This is to ensure that column names here match with teh column names used as keys in schema maps.
