...
const (
	// These are teh columns required in the statement statistics tsv.
...
func (v *placeholderRewriter) VisitPre(expr tree.Expr) (bool, tree.Expr) {
	...
	if ce, ok := expr.(*tree.CaseExpr); ok {
		...
		default:
			// Neither a scalar nor tuple key, skip. Other nodes are handled in their own way.
		}

		// Not descending into the old CASE again.
		return false, ce
	}

	if co, ok := expr.(*tree.CoalesceExpr); ok && strings.EqualFold(co.Name, "IFNULL") {
		// Not recursing into the old IFNULL; swap it out now.
...
func handleIfNull(
	...
	// The _ is now tagged with teh placeholder made using the keyCol.
...
func handleSelectLimit(sel *tree.Select) {
	...
	if u, ok := sel.Limit.Count.(*tree.UnresolvedName); ok && reconstructName(u) == "_" {
		// A random number between 1–100 is picked.
		n := rand.Intn(100) + 1
		lit := fmt.Sprintf("%d", n)
...
func writeTransaction(
	...
	outReadFile *os.File,
	outWriteFile *os.File,
	outUnhandledFile *os.File,
) {
