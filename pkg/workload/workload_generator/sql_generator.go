...
func generateWorkload(
	...
	for node := 1; ; node++ {
		...
		for scanner.Scan() {
			...
			// 5d) Process the sql query to replace _ and __more__ with new placeholders which contain information about the column they refer to.
...
func replacePlaceholders(rawSQL string, allSchemas map[string]*TableSchema) (string, error) {
	...
	for _, stmt := range stmts {
		// INSERT…VALUES (<placeholders>) rewrite.
