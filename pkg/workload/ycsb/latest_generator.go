...
// LatestGenerator is a random number generator that generates numbers in the
// range [iMin, iMax], but skews it towards iMax using a zipfian distribution.
...
// IMaxHead returns the current value of iMax, without incrementing.
func (z *LatestGenerator) IMaxHead() uint64 {
	z.mu.Lock()
	defer z.mu.Unlock()
	return z.mu.iMax
}
...
func (z *LatestGenerator) IncrementIMax() error {
	z.mu.Lock()
	defer z.mu.Unlock()
	z.mu.iMax++
...
func (z *LatestGenerator) Uint64() uint64 {
	z.mu.Lock()
	defer z.mu.Unlock()
