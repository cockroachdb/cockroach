...
var ycsbMeta = workload.Meta{
	...
	New: func() workload.Generator {
		...
		g.flags.IntVar(&g.recordCount, `record-count`, 0, `Key is start workload insertions from. Must be >= insert-start + insert-count. (Default: insert-start + insert-count)`)
...
func (yw *ycsbWorker) nextReadKey() string {
	...
	// expected keys at the end of the workload, and hashes it again to obtain
	// the key. If the generated key is greater than the number of acknowledged
	// rows, then a key is regenerated. Unfortunately, we cannot match this
	// implementation since we run YCSB for a set amount of time rather than
	// number of operations, so we don't know the expected number of keys at the
	// end of the workload. Instead we use a zipfian generator whose key space
	// grows as more rows are acknowledged, but we don't hash so the hotspot is
	// consistent. This causes the hotspot to be randomly distributed in the key
	// space, but it is not random w.r.t the insertion order of the keys. The
	// distribution is also slightly different than the official YCSB's
