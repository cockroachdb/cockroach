...
var ycsbMeta = workload.Meta{
	...
	New: func() workload.Generator {
		...
		g.flags.IntVar(&g.zeroPadding, `zero-padding`, 1, `Key using "insert-random=false" has zeros padded to left to make this length of digits.`)
	...
	usertable := workload.Table{
		...
		Splits: workload.Tuples(
			...
			func(splitIdx int) []interface{} {
				w := ycsbWorker{
					config:   g,
					prngPerm: crrand.MakePerm64(uint64(RandomSeed.Seed())),
				}
...
func (yw *ycsbWorker) buildKeyName(keynum uint64) string {
	if yw.config.insertRandom {
		...
		return fmt.Sprintf("user%d", yw.prngPerm.At(keynum))
	}
