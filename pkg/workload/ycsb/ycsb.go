	...
	return workload.Hooks{
		Validate: func() error {
			...
			if !g.flags.Lookup(`families`).Changed {
				...
				// These determinations were computed on 80da27b (04/04/2020)
				// while running the ycsb roachtests.
				//
				...
				//
				g.families = map[string]bool{
					// Workload A is highly contended. It performs 50% single-row
					// lookups and 50% single-column updates. Using column families
					// breaks the contention between all updates to different columns of
					// the same row, so we use them by default.
					"A": true,
					// Workload B is less contended than Workload A, but still
					// bottlenecks on contention as concurrency grows. It performs 95%
					// single-row lookups and 5% single-column updates. Using column
					// families slows down the single-row lookups but speeds up the
					// updates (see above). This trade-off favors column families for
					// higher concurrency levels but does not at lower concurrency
					// levels. We prefer larger YCSB deployments, so we use column
					// families by default.
					"B": true,
					// Workload C has no contention. It consistent entirely of
					// single-row lookups. Using column families slows down single-row
					// lookups, so we do not use them by default.
					"C": false,
					// Workload D has no contention. It performs 95% single-row lookups
					// and 5% single-row insertion. Using column families slows down
					// single-row lookups and single-row insertion, so we do not use
					// them by default.
					"D": false,
					// Workload E has moderate contention. It performs 95% multi-row
					// scans and 5% single-row insertion. Using column families slows
					// down multi-row scans and single-row insertion, so we do not use
					// them by default.
					"E": false,
					// Workload F is highly contended. It performs 50% single-row
					// lookups and 50% single-column updates expressed as
					// multi-statement read-modify-write transactions. Using column
					// families breaks the contention between all updates to different
					// columns of the same row, so we use them by default.
					"F": true,
				}[g.workload]
			}
