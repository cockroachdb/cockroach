...
var ycsbMeta = workload.Meta{
	...
	New: func() workload.Generator {
		...
		g.flags.StringVar(&g.distribution, `request-distribution`, `uniform`, `Distribution for random number generator [zipfian, uniform].`)
...
func (g *ycsb) Ops(urls []string, reg *workload.HistogramRegistry) (workload.QueryLoad, error) {
	...
	if strings.ToLower(g.distribution) == "zipfian" {
		randGen, err = NewZipfGenerator(
			zipfRng, zipfIMin, defaultIMax, defaultTheta, false) /* verbose */
	} else {
		randGen, err = NewUniformGenerator(zipfRng, zipfIMin)
	}
...
func (yw *ycsbWorker) nextReadKey() string {
	...
	rownum := yw.hashKey(yw.randGen.Uint64()) % uint64(yw.config.initialRows)
