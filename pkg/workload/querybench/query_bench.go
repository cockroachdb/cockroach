...
var queryBenchMeta = workload.Meta{
	...
	New: func() workload.Generator {
		...
		g.flags.IntVar(&g.numRunsPerQuery, `num-runs`, 0, `Specifies the number of times each query in the query file to be run `+
			`(note that --duration and --max-ops take precendence, so if duration or max-ops is reached, querybench will exit without honoring --num-runs)`)
		g.flags.BoolVar(&g.useOpt, `use-opt`, true, `Use cost-based optimizer`)
		g.flags.BoolVar(&g.useVectorized, `use-vec`, false, `Turn experimental vectorized execution on`)
