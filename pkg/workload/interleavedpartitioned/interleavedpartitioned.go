...
func (w *interleavedPartitioned) Hooks() workload.Hooks {
	return workload.Hooks{
		PreLoad: func(db *gosql.DB) error {
			...
			if _, err := db.Exec(
				fmt.Sprintf("ALTER PARTITION west OF TABLE sessions CONFIGURE ZONE 'experimental_lease_preferences: [[+zone=%s]]'", w.westZoneName),
			); err != nil {
				return errors.Wrapf(err, "could not set zone for partition west")
			}
			if _, err := db.Exec(
				fmt.Sprintf("ALTER PARTITION east OF TABLE sessions CONFIGURE ZONE 'experimental_lease_preferences: [[+zone=%s]]'", w.eastZoneName),
			); err != nil {
