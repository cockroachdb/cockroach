...
`,
				...
				for _, rbrTable := range rbrTables {
					...
					if _, err := db.Exec(fmt.Sprintf("ALTER TABLE %s SET (schema_locked = true)", rbrTable)); err != nil {
						return err
					}
				}
				for _, globalTable := range globalTables {
					...
					if _, err := db.Exec(fmt.Sprintf("ALTER TABLE %s SET (schema_locked = true)", globalTable)); err != nil {
						return err
					}
				}
