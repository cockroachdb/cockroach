...
// {col1: 'val1', col2: 'val2', ...}. Returns "<nil>" if the row is nil.
...
// canHashType returns true if the given type can be hashed using
// crdb_internal.datums_to_bytes. Certain type families do not support keyside
// encoding and must be excluded from the hash computation.
func canHashType(t *types.T) bool {
	switch t.Family() {
...
//	FROM my_table
func buildHashQuery(schema *Table, hashableCols []int, pkHashExpr string, tableName string) string {
	var b strings.Builder
	b.WriteString("SELECT ")
	b.WriteString(pkHashExpr)
	b.WriteString(" AS pk_hash, fnv64(crdb_internal.datums_to_bytes(")
	for i, colIdx := range hashableCols {
		if i > 0 {
			b.WriteString(", ")
		}
	...
	b.WriteString(tree.NameString(tableName))
	b.WriteString(buildOrderByClause(schema))
	return b.String()
}

// loadHashes executes the hash query and returns a map from pk_hash to
// row_hash. Both are int64 values produced by fnv64.
func loadHashes(conn *gosql.DB, query string) (map[int64]int64, error) {
...
func fetchDiffRows(
	...
	for rows.Next() {
		// +3: crdb_internal_mvcc_timestamp, crdb_internal_origin_timestamp, pk_hash
...
func Diff(
	...
	var hashableCols []int
	for i, col := range schema.Cols {
		if canHashType(col.DataType) {
			hashableCols = append(hashableCols, i)
		}
	}
	...
	hashQueryA := buildHashQuery(&schema, hashableCols, pkHashExpr, tableA)
	hashesA, err := loadHashes(connA, hashQueryA)
	if err != nil {
		return nil, err
	}
	hashQueryB := buildHashQuery(&schema, hashableCols, pkHashExpr, tableB)
	hashesB, err := loadHashes(connB, hashQueryB)
	if err != nil {
		return nil, err
	}
