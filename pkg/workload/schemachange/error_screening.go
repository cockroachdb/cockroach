...
`, tableName.String())
	...
	for _, row := range rows {
		...
		for constraintIdx, constraint := range constraints {
			...
			// Row is not comparable to others, has a NULL value
...
// isValidGenerationError these codes can be observed when genearting an expressions.
...
func (og *operationGenerator) violatesFkConstraints(
	...
	/*activeConstraintsInTxn := ""
	  if len(og.newConstraintsInTxn) > 0 {
	  	activeConstraintsInTxnBuilder := strings.Builder{}
	  	activeConstraintsInTxnBuilder.WriteString(" OR ( ")
	  	for i, constraintName := range og.newConstraintsInTxn {
	  		if i != 0 {
	  			activeConstraintsInTxnBuilder.WriteString(" AND ")
	  		}
	  		activeConstraintsInTxnBuilder.WriteString("CONSTRAINT_NAME='")
	  		activeConstraintsInTxnBuilder.WriteString(constraintName)
	  		activeConstraintsInTxnBuilder.WriteString("'")
	  	}
	  	activeConstraintsInTxnBuilder.WriteString(")")
	  	activeConstraintsInTxn = activeConstraintsInTxnBuilder.String()
	  }*/
