...
func (s *schemaChange) Ops(
	...
	for i := 0; i < s.concurrency; i++ {
		...
		opBuilderParams := operationGeneratorParams{
			seqNum:    seqNum,
		...
		w := &schemaChangeWorker{
			...
			pool:            pool,
			hists:           reg.GetHandle(),
			opBuilderParams: &opBuilderParams,
		}
...
func (w *schemaChangeWorker) runInTxn(tx *pgx.Tx, opGen operationGenerator) (string, error) {
	var log strings.Builder
	opsNum := 1 + opGen.params.rng.Intn(w.maxOpsPerWorker)

	for i := 0; i < opsNum; i++ {
		op, noops, expectedErrorCode, err := opGen.randOp(tx)
		if err != nil {
