...
CREATE TABLE IF NOT EXISTS trigger_log (
	...
	// Check if the trigger function log_change_timestamp exists.
	...
	// Build the SQL statement
	sqlStatement := fmt.Sprintf("CREATE TRIGGER %s %s %s ON %s FOR EACH ROW EXECUTE FUNCTION log_change_timestamp()",
...
func (og *operationGenerator) dropTrigger(ctx context.Context, tx pgx.Tx) (*opStmt, error) {
	...
	triggerWithInfo, triggerExists, err := findExistingTrigger(ctx, tx)
	if err != nil {
		return nil, err
	}
	if !triggerExists {
...
func findExistingTrigger(ctx context.Context, tx pgx.Tx) (*triggerInfo, bool, error) {
