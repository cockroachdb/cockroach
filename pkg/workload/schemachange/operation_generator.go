...
func (og *operationGenerator) createIndex(ctx context.Context, tx pgx.Tx) (*opStmt, error) {
	...
	jsonInvertedIndexesAllowed, err := isClusterVersionLessThan(ctx, tx, clusterversion.ByKey(clusterversion.V23_2))
	if err != nil {
		return nil, err
	}
	for i := range def.Columns {
		...
		if def.Inverted {
			...
			if !jsonInvertedIndexesAllowed && columnNames[i].typ.Family() == types.JsonFamily {
				nonIndexableType = true
			}
		} else {
