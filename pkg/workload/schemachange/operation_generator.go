...
func makeOperationBuilder(params *operationGeneratorParams) operationGenerator {
...
func (og operationGenerator) randOp(tx *pgx.Tx) (string, string, *pgcode.Code, error) {
	...
	for {
		...
		switch op {
		case addColumn:
			stmt, expectedErrorCode, err = og.addColumn(tx)

		case addConstraint:
			stmt, err = og.addConstraint(tx)

		case createIndex:
			stmt, expectedErrorCode, err = og.createIndex(tx)
...
func (og operationGenerator) addColumn(tx *pgx.Tx) (string, *pgcode.Code, error) {
...
func (og operationGenerator) createIndex(tx *pgx.Tx) (string, *pgcode.Code, error) {
	tableName, err := og.randTable(tx, og.pctExisting(true), "")
	if err != nil {
		return "", nil, err
	}
	...
	if !tableExists {
		return tree.Serialize(&tree.CreateIndex{
			...
			Table:       *tableName,
			Unique:      og.randIntn(2) == 0,
			Inverted:    og.randIntn(2) == 0,
...
func (og operationGenerator) createSchema(tx *pgx.Tx) (string, error) {
	...
	stmt := rowenc.MakeSchemaName(og.randIntn(2) == 0, schemaName, "")
	return tree.Serialize(stmt), nil
}
