...
func (og *operationGenerator) alterPolicy(ctx context.Context, tx pgx.Tx) (*opStmt, error) {
	...
	opStmt.potentialExecErrors.addAll(codesWithConditions{
		// Currently we will not see this error as our createPolicy method
		// does not create policies that are specific to a command.
		// Example: CREATE POLICY p ON t FOR INSERT
		{code: pgcode.Syntax, condition: includeWithCheck || includeUsing},
	})
