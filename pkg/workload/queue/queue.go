...
var queueMeta = workload.Meta{
	Name: `queue`,
	Description: `Batched writes and deletes from a queue-like table.` +
		`sequence, deletions from beginning of sequence.`,
	Version: `1.0.0`,
...
func (w *queue) Ops(urls []string, reg *workload.HistogramRegistry) (workload.QueryLoad, error) {
	...
	// Generate queue deletion statement.
	deleteStmt, err := db.Prepare(`DELETE FROM queue WHERE ts >= $1 AND ts < $2`)
	if err != nil {
		return workload.QueryLoad{}, err
	}
...
func (o *queueOp) run(ctx context.Context) error {
	...
	startTime = timeutil.Now()
	_, err = o.deleteStmt.Exec(start, end)
