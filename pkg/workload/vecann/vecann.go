...
const (
	defaultDataset = "dbpedia-openai-100k-angular"
)
...
var vectorMeta = workload.Meta{
	...
	are similar. Vectors are grouped together separate K-means trees in the
	index according to a Zipfian distribution - if the first group has N
	vectors, then the second group has N / 2 vectors, the third N / 3, and
	so on.
	...
	New: func() workload.Generator {
		...
		g.flags.Meta = map[string]workload.FlagMeta{
			...
			"query-percent":    {RuntimeOnly: true},
		}
...
func (v *vectorWorkload) ConnFlags() *workload.ConnFlags { return v.connFlags }

// Tables implements the Generator interface.
func (v *vectorWorkload) Tables() []workload.Table {
	tables := make([]workload.Table, 1)

	// Table for storing vectors (no initial rows).
	tables[0] = workload.Table{
		Name:   v.tableName,
		Schema: fmt.Sprintf(vectorTableSchema, v.searchData.Test.Dims),
	}
...
func (v *vectorWorkload) Hooks() workload.Hooks {
	return workload.Hooks{
		Validate: func() error {
			...
			// Load dataset containing vectors to insert and query.
			...
			v.insertData = loader.Data
			v.searchData = loader.SearchData

			datasetName := strings.ReplaceAll(v.datasetName, "-", "_")
			v.tableName = fmt.Sprintf("vectors_%s", datasetName)
			return nil
		},
	}
}
...
func (vw *vectorWorkload) Ops(
	...
	cfg.AfterConnect = func(ctx context.Context, conn *pgx.Conn) error {
		_, err := conn.Exec(ctx, fmt.Sprintf("SET vector_search_beam_size = %d", vw.searchBeamSize))
		if err != nil {
			return err
		}
		return nil
	...
	for i := range vw.connFlags.Concurrency {
		...
		worker.zipf = rand.NewZipf(worker.rng, 1.1, 1, uint64(vw.numGroups)-1)
