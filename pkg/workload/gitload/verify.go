...
// runAllChecks runs oracle verification in two phases. Data checks
// (repo stats, commit graph, metadata, replay, blobs) run in an AOST
// transaction to avoid contention with concurrent writers. Schema checks
// (referential integrity, secondary indexes) run in a separate
// current-time transaction because they use FORCE_INDEX hints and
// schema introspection that require the current schema.
func runAllChecks(ctx context.Context, db *gosql.DB, baseRepoPath string, verifyEveryN int) error {
	if err := runDataChecks(ctx, db, baseRepoPath, verifyEveryN); err != nil {
		return err
	}
	return runSchemaChecks(ctx, db)
}

func runDataChecks(ctx context.Context, db *gosql.DB, baseRepoPath string, verifyEveryN int) error {
	tx, err := db.BeginTx(ctx, &gosql.TxOptions{ReadOnly: true})
	if err != nil {
		return errors.Wrap(err, "starting verification transaction")
	}
...
func runSchemaChecks(ctx context.Context, db *gosql.DB) error {
	tx, err := db.BeginTx(ctx, &gosql.TxOptions{ReadOnly: true})
	if err != nil {
		return errors.Wrap(err, "starting schema verification transaction")
	}
	defer func() { _ = tx.Rollback() }()
