...
// need to use TypedTuples instead and specify the phystypes.
...
// made to infer them.
func TypedTuples(count int, colTypes []phystypes.T, fn func(int) []interface{}) BatchedTuples {
	...
	if fn != nil {
		t.FillBatch = func(batchIdx int, cb coldata.Batch, _ *bufalloc.ByteAllocator) {
			...
			colTypesOnce.Do(func() {
				if colTypes == nil {
					colTypes = make([]phystypes.T, len(row))
					for i, datum := range row {
						if datum == nil {
							...
							switch datum.(type) {
							case time.Time:
								colTypes[i] = phystypes.Bytes
							default:
								colTypes[i] = phystypes.FromGoType(datum)
							}
						}
					}
				}
...
func ColBatchToRows(cb coldata.Batch) [][]interface{} {
	...
	for colIdx, col := range cb.ColVecs() {
		...
		case phystypes.Bytes:
			...
			// returned as a coldata.Batch, which has a more limited set of phystypes.
