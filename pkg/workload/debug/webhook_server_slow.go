...
func webhookServerSlow(cmd *cobra.Command, args []string) error {
	...
	lastTransientErrorTime.Store(time.Now())
	...
	if len(args) > 0 {
		if interval, err := strconv.Atoi(args[0]); err == nil {
			...
			return fmt.Errorf("Invalid transient error frequency, using default: %d ms", defaultTransientErrorInterval)
		}
	}
	...
	if len(args) > 1 {
		...
		for i, delay := range args[1:] {
			if d, err := strconv.Atoi(delay); err == nil {
				...
				return fmt.Errorf("Invalid range delay at index %d, using default: 0 ms", i)
			}
		}
	}
