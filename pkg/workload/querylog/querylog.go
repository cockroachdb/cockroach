...
func (w *querylog) Ops(urls []string, reg *workload.HistogramRegistry) (workload.QueryLoad, error) {
...
func (w *worker) run(ctx context.Context) error {
	if w.config.stmtTimeoutSeconds != 0 {
		if _, err := w.db.Exec(fmt.Sprintf("SET statement_timeout='%ds'", w.config.stmtTimeoutSeconds)); err != nil {
			return err
		}
	}
	...
	for {
		...
		if err != nil {
			if w.config.verbose {
				log.Infof(ctx, "Encountered an error %s while generating values for the placeholders\n", err.Error())
				printQueryShortened(ctx, chosenQuery)
			}
			continue
		}
		...
		start = timeutil.Now()
		rows, err := w.db.Query(chosenQuery, placeholders...)
		if err != nil {
			if w.config.verbose {
				log.Infof(ctx, "Encountered an error %s while executing the query\n", err.Error())
				printQueryShortened(ctx, chosenQuery)
			}
			continue
		}
		...
		if err != nil {
			// This is an unexpected error, so we do not "swallow" it.
			return err
		}
	...
	for j := 0; j < numRepeats; j++ {
		for i, column := range pholdersColumnNames {
			...
			if !columnMatched {
				d := w.rng.Int31n(10) + 1
				if w.config.verbose {
					log.Infof(ctx, "Couldn't deduce the corresponding to $%d column, so generated %d (a small int)\n", i+1, d)
...
func (w *querylog) parseFile(ctx context.Context, fileInfo os.FileInfo, re *regexp.Regexp) error {
	...
	for {
		...
		if err != nil {
			...
			log.Infof(ctx, "Processing of %s is done in %fs\n", fileInfo.Name(), elapsed.Seconds())
			break
		}
		...
		if !isPrefix {
			...
			if lineCount >= queryLogHeaderLines {
				...
				if !skipQuery {
					...
					if !tableFound {
						return errors.Errorf("No table matched query %s while processing %s\n", query, fileInfo.Name())
					}
				}
			}
			buffer = buffer[:0]
...
func (w *querylog) processQueryLog(ctx context.Context) error {
	if w.zipPath != "" {
		log.Infof(ctx, "About to start unzipping %s\n", w.zipPath)
		w.dirPath = w.zipPath[:len(w.zipPath)-4]
		if err := unzip(w.zipPath, w.dirPath); err != nil {
			return err
		}
		log.Infof(ctx, "Unzipping to %s is complete\n", w.dirPath)
	}
	...
	for fileNum, fileInfo := range files {
		...
		if fileInfo.IsDir() {
			if w.verbose {
				log.Infof(ctx, "Unexpected: a directory %s is encountered with the query log, skipping it.\n", fileInfo.Name())
			}
			continue
		}
		log.Infof(ctx, "Processing %d out of %d\n", fileNum, numFiles)
