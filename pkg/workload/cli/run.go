	...
	if *histograms != "" {
		...
		case "openmetrics":
			...
			for _, label := range labelValues {
				...
				if len(parts) != 2 {
					return nil, nil, errors.Errorf(`invalid histogram label "%s"`, label)
				}
				labels[strings.TrimSpace(parts[0])] = strings.TrimSpace(parts[1])
			}
			metricsExporter = &exporter.OpenmetricsExporter{
				Labels: &labels,
			}
		default:
		...
		if err = metricsExporter.Init(&writer); err != nil {
			return nil, nil, errors.Wrap(err, "failed to initialize histograms metricsExporter")
		}
	}
