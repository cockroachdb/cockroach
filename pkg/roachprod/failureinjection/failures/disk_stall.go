...
func getStallType(diskStallArgs DiskStallArgs) ([]bandwidthType, error) {
	var stallType []bandwidthType
	if diskStallArgs.StallWrites {
		stallType = []bandwidthType{writeBandwidth}
	}
	if diskStallArgs.StallReads {
		stallType = append(stallType, readBandwidth)
	}
	if len(stallType) == 0 {
		return nil, errors.New("at least one of reads or writes must be stalled")
	}
	return stallType, nil
}
...
func (s *CGroupDiskStaller) Inject(ctx context.Context, l *logger.Logger, args FailureArgs) error {
	...
	rand.Shuffle(len(stallType), func(i, j int) {
		stallType[i], stallType[j] = stallType[j], stallType[i]
	})
	...
	if err := s.setThroughput(ctx, l, stallType, throughput{limited: true, bytesPerSecond: fmt.Sprintf("%d", bytesPerSecond)}, nodes, cockroachIOController); err != nil {
		return err
	}
...
func (s *CGroupDiskStaller) Restore(ctx context.Context, l *logger.Logger, args FailureArgs) error {
	...
	if err != nil && diskStallArgs.RestartNodes {
		l.Printf("failed to log cgroup bandwidth limits, assuming node exited and restarting: %v", err)
		return s.StartNodes(ctx, l, nodes)
	}
	return nil
}
...
func (s *CGroupDiskStaller) WaitForFailureToPropagate(
	...
	if diskStallArgs.StallWrites {
		...
		// and kill the node.
		return s.WaitForSQLNodeDeath(ctx, l, diskStallArgs.Nodes, time.Minute)
	}
	return nil
}
...
func (s *CGroupDiskStaller) WaitForFailureToRestore(
	...
	nodes := args.(DiskStallArgs).Nodes
	return s.WaitForSQLLiveness(ctx, l, nodes, time.Minute)
}
...
func (s *DmsetupDiskStaller) Restore(
	...
	if err := s.PingNode(ctx, l, nodes); err != nil && diskStallArgs.RestartNodes {
		l.Printf("failed to connect, assuming node exited and restarting: %v", err)
		return s.StartNodes(ctx, l, nodes)
	}
	return nil
}
...
func (s *DmsetupDiskStaller) WaitForFailureToPropagate(
	...
	nodes := args.(DiskStallArgs).Nodes
	return s.WaitForSQLNodeDeath(ctx, l, nodes, time.Minute)
}
