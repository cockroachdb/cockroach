...
type Failer struct {
	...
	// _injectedError is a test only field that can be used to inject an error.
...
// maybeInjectError checks if the test only _injectedError is
// set and returns it if so, also marking the Failer as invalid.
func (f *Failer) maybeInjectError() error {
	if f._injectedError != nil {
		f.state = invalid
	}
	return f._injectedError
}
