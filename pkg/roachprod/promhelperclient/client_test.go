...
func TestUpdatePrometheusTargets(t *testing.T) {
	...
	t.Run("UpdatePrometheusTargets fails with 400", func(t *testing.T) {
		...
		err := UpdatePrometheusTargets(ctx, promUrl, "c1", []string{"n1"}, l)
...
`, ir.Config)
		...
		err := UpdatePrometheusTargets(ctx, promUrl, "c1", []string{"n1", "n2"}, l)
		require.Nil(t, err)
	})
}
...
func TestDeleteClusterConfig(t *testing.T) {
	...
	t.Run("DeleteClusterConfig fails with 400", func(t *testing.T) {
		...
		err := DeleteClusterConfig(ctx, promUrl, "c1", l)
		require.NotNil(t, err)
		require.Equal(t, "request failed with status 400 and error failed", err.Error())
	})
...
func Test_getToken(t *testing.T) {
	...
	t.Run("insecure url", func(t *testing.T) {
		token, err := getToken(ctx, "http://test.com")
		require.Nil(t, err)
		require.Empty(t, token)
	})
	t.Run(ServiceAccountJson+" is missing", func(t *testing.T) {
		token, err := getToken(ctx, "https://test.com")
		require.NotNil(t, err)
		require.Empty(t, token)
		require.Equal(t, ServiceAccountJson+" env variable was not found", err.Error())
	})
	t.Run(ServiceAccountAudience+" is missing", func(t *testing.T) {
		os.Setenv(ServiceAccountJson, "{}")
		token, err := getToken(ctx, "https://test.com")
		require.NotNil(t, err)
		require.Empty(t, token)
		require.Equal(t, ServiceAccountAudience+" env variable was not found", err.Error())
	})
