...
func UpdatePrometheusTargets(
	ctx context.Context, promUrl, clusterName string, nodes []string, l *logger.Logger,
) error {
...
func DeleteClusterConfig(ctx context.Context, promUrl, clusterName string, l *logger.Logger) error {
	token, err := getToken(ctx, promUrl)
	if err != nil {
		return err
	}
