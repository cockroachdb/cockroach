...
func (te TransientError) Error() string {
	return fmt.Sprintf("TRANSIENT_ERROR(%s): %s", te.Label, te.Err)
}
...
func NewSSHError(err error) TransientError {
	return TransientFailure(err, sshProblemLabel)
}

func IsSSHError(err error) bool {
	var transientErr TransientError
	if errors.As(err, &transientErr) {
		return transientErr.Label == sshProblemLabel
	}
