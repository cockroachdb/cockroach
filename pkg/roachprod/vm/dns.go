...
func FanOutDNS(list List, action func(DNSProvider, List) error) error {
	...
	for name, vms := range m {
		...
		g.Go(func() error {
			...
			if !ok {
				// This fan out is lenient, so we don't return an error here.
				return nil
			}
...
// action. If lenient is true, then the action is not executed if the provider
// is not specified or not a DNSProvider, and no error is returned.
func ForDNSProvider(named string, action func(DNSProvider) error, lenient bool) error {
	if named == "" {
		if lenient {
			return nil
		}
