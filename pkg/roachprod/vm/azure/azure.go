...
func (p *Provider) createNIC(
	...
	sg := p.getSecurityGroupByName(p.getVnetNetworkSecurityGroupName(*group.Location))
...
func (p *Provider) createVNets(
	...
	for _, location := range providerOpts.Locations {
		group := p.getResourcesGroupByName(vnetResourceGroupName(location))
	...
	for location, prefix := range prefixesByLocation {
		resourceGroup := p.getResourcesGroupByName(vnetResourceGroupName(location))
		networkSecurityGroup := p.getSecurityGroupByName(p.getVnetNetworkSecurityGroupName(location))
...
func (p *Provider) getResourcesGroupByName(name string) resources.Group {
	p.mu.Lock()
	defer p.mu.Unlock()
	return p.mu.resourceGroups[name]
}

// getSecurityGroupByName receives a string name and returns
// tje network.SecurityGroup associated with it.
func (p *Provider) getSecurityGroupByName(name string) network.SecurityGroup {
	p.mu.Lock()
	defer p.mu.Unlock()
	return p.mu.securityGroups[name]
}
