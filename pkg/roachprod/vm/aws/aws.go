...
func (p *Provider) Create(
	...
	for i := range names {
		...
		g.Go(func() error {
			...
			vm, err := p.runInstance(l, capName, index, placement, machineType, opts, providerOpts)
			if err != nil {
				return err
			}
	...
	ipAddresses, err := p.waitForIPs(l, names, regions, providerOpts)
	if err != nil {
		return nil, err
	}

	for i, vm := range vmList {
		vmList[i].PublicIP = ipAddresses[vm.Name].PublicIP
		vmList[i].PrivateIP = ipAddresses[vm.Name].PrivateIP
	}
...
func (in *DescribeInstancesOutputInstance) toVM(volumes map[string]vm.Volume, remoteUserName string) *vm.VM {
