...
func TestIOPSCalculation(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			// Simulate the IOPS calculation logic from createEBSVolumes
			volumeSize := tc.volumeSize
			volumeType := tc.volumeType
			iops := 0

			// This is the logic from createEBSVolumes (lines 1655-1686)
			if iops == 0 && (volumeType == "io1" || volumeType == "io2") {
				iops = volumeSize * 10
