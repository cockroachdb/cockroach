...
// The disks argument is only used to enrich the volumes with some additional
// information (namely type and labels) than the one returned in the jsonVM.
func (jsonVM *jsonVM) toVM(
	project string, disks []describeVolumeCommandResponse, dnsDomain string,
) (ret *vm.VM) {
	var vmErrors []error
	var err error
	...
	for _, jsonVMDisk := range jsonVM.Disks {
		...
		vol, volType, err := attachedDiskToVolume(jsonVMDisk)
		if err != nil {
		...
		for _, detailedDisk := range disks {
			if detailedDisk.SelfLink == jsonVMDisk.Source {
				err = enrichVolumeWithDiskDetails(vol, detailedDisk)
				if err != nil {
					vmErrors = append(vmErrors, err)
				}
			}
...
func (j *managedInstanceGroupInstance) toVM(
	...
	for _, disk := range instanceTemplate.Properties.Disks {
		...
		diskSize, err := strconv.Atoi(disk.InitializeParams.DiskSizeGb)
		if err != nil {
			vmErrors = append(vmErrors, errors.Wrapf(
				err,
				"failed to parse disk size %s", disk.InitializeParams.DiskSizeGb,
			))
		}

		if disk.InitializeParams.SourceImage == "" && disk.InitializeParams.DiskType == "SCRATCH" {
		...
		vol := vm.Volume{
			...
			Zone:               zone,
			Size:               diskSize,
			ProviderResourceID: lastComponent(j.PreservedStateFromPolicy.Disks[disk.DeviceName].Source),
			ProviderVolumeType: disk.InitializeParams.DiskType,
...
func (p *Provider) List(l *logger.Logger, opts vm.ListOptions) (vm.List, error) {
	...
	for _, prj := range p.GetProjects() {
		...
		for _, jsonVM := range jsonVMS {
			vms = append(vms, *jsonVM.toVM(
				prj,
				userVMToDetailedDisk[jsonVM.SelfLink],
				p.publicDomain,
			))
		}
	}
...
// attachedDiskToVolume converts an attachedDisk struct (e.g. disks returned
// by `instances list`) to a vm.Volume.
func attachedDiskToVolume(d attachDiskCmdDisk) (*vm.Volume, VolumeType, error) {
	...
	diskType := func(s string) string {
		if s == "PERSISTENT" {
			// Unfortunately, we don't know if it's a pd-ssd or pd-standard.
			// We assume pd_ssd since those are common.
			return "pd-ssd"
		}
		return "unknown"
	}
	...
	vol := &vm.Volume{
		...
		Size:               diskSize,
		ProviderResourceID: d.Source,
		ProviderVolumeType: diskType(d.Type),
	}

	return vol, volType, nil
}

// enrichVolumeWithDiskDetails enriches the given volume with details from the
// disks list command. It sets the labels and the actual type.
func enrichVolumeWithDiskDetails(vol *vm.Volume, diskDetails describeVolumeCommandResponse) error {
	vol.Labels = diskDetails.Labels
	vol.ProviderVolumeType = diskDetails.Type
	return nil
}
