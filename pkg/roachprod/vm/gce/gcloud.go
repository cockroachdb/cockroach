...
func createInstanceTemplates(
	...
	var instanceTemplatesMu sync.Mutex
			...
			g.Go(func() error {
				...
				for _, i := range instances {
					v := i.toVM(project,
						...
						p.CreateProviderOpts().(*ProviderOpts),
						p.publicDomain,
					)
...
// TODO(ludo): return new vms
func (p *Provider) Grow(l *logger.Logger, vms vm.List, clusterName string, names []string) (vm.List, error) {
	if !isManaged(vms) {
...
func (j *managedInstanceGroupInstance) toVM(
	...
	// TODO(ludo): get CPU arch and platform from the machine type.
	var arch vm.CPUArch
...
func getManagedInstanceGroupVMs(
	...
	instanceTemplates map[string]jsonInstanceTemplate, dnsDomain string,
) (vm.List, error) {
