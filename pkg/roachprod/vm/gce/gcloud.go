...
func NewProvider(options ...Option) (*Provider, error) {
	...
	if p.withSDKSupport {
		...
		if err != nil {
			return nil, errors.Wrapf(err, "unable to init compute instances client")
		}
		...
		if err != nil {
			return nil, errors.Wrapf(err, "unable to init compute instance templates client")
		}
		...
		if dnsProviderInitializedInInit {
			...
			if err != nil {
				return nil, errors.Wrapf(err, "unable to init dns client")
			}
		}
	}
...
// ListWithContext queries gcloud to produce a list of VM info objects.
