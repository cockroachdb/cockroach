...
func (p *Provider) Create(
	...
	default:
		...
		for zone, zoneHosts := range zoneToHostNames {
			chunkSize := MaxConcurrentHosts / 4
			for i := 0; i < len(zoneHosts); i += chunkSize {
				hostChunk := zoneHosts[i:min(i+chunkSize, len(zoneHosts))]
				argsWithZone := append(createArgs, "--zone", zone)
				argsWithZone = append(argsWithZone, hostChunk...)

				g.Go(func() error {
					err := sem.Acquire(context.Background(), len(hostChunk))
					if err != nil {
						return errors.Wrapf(err, "Failed to acquire semaphore")
					}
					defer sem.Release(len(hostChunk))
	...
	for project, zoneMap := range projectZoneMap {
		for zone, names := range zoneMap {
			...
			for i := 0; i < len(names); i += chunkSize {
				nameChunk := names[i:min(i+chunkSize, len(names))]
				...
				args = append(args, nameChunk...)

				g.Go(func() error {
					err := sem.Acquire(ctx, len(nameChunk))
					if err != nil {
						return errors.Wrapf(err, "Failed to acquire semaphore")
					}
					defer sem.Release(len(nameChunk))
