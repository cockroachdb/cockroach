...
func (n dnsProvider) CreateRecords(records ...vm.DNSRecord) error {
	...
	// short (less than 30 seconds), but we set it to 2 minutes to be safe.
...
func (n dnsProvider) waitForRecordsAvailable(ctx context.Context, records ...vm.DNSRecord) error {
	...
	for ctx.Err() == nil {
		...
		for name := range names {
			...
			if err != nil {
				// Retain the last error in case we time out.
				lastErr = err
			}
			for _, foundRecord := range foundRecords {
				delete(notAvailable, keyFn(foundRecord))
			}
		}
	...
	return errors.Wrapf(
		errors.CombineErrors(ctx.Err(), lastErr),
		"timed out waiting for DNS records to become available: %d out of %d records not available",
