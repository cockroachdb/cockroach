...
func NewDNSProvider(configDir, zone string) vm.DNSProvider {
	return &dnsProvider{configDir: configDir, lockDir: path.Join(configDir, "DNS_LOCK"), zone: zone}
}
...
func (n *dnsProvider) CreateRecords(records ...vm.DNSRecord) error {
	unlock, err := lock.AcquireFilesystemLock(n.lockDir)
	if err != nil {
		return err
	}
	defer unlock()
...
func (n *dnsProvider) DeleteRecordsBySubdomain(subdomain string) error {
	unlock, err := lock.AcquireFilesystemLock(n.configDir)
	if err != nil {
		return err
	}
	defer unlock()
