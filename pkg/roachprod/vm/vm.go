...
func (p *CLIProvider) runCommand(ctx context.Context, l *logger.Logger, depth int, args []string, modeOverrides ...bool) ([]byte, error) {
	if p.CLICommand == "" {
	...
	if err != nil {
		...
		if matched, _ := regexp.Match(`.*Unknown zone`, stderr); !matched {
			return nil, errors.Wrapf(err, "failed to run: gcloud %s\nstdout: %s\nstderr: %s\n",
				strings.Join(args, " "), bytes.TrimSpace(output), bytes.TrimSpace(stderr))
		}
	}
	return output, nil
}
...
func (p *CLIProvider) RunCommand(ctx context.Context, l *logger.Logger, args []string, modeOverrides ...bool) ([]byte, error) {
...
func (p *CLIProvider) RunJSONCommand(l *logger.Logger, args []string, parsed interface{}, modeOverrides ...bool) error {
	if p.CLICommand == "aws" {
