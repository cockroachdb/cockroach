...
func (p *Provider) createInstance(l *logger.Logger, opts instanceOptions) (*vm.VM, error) {
	...
	attachedVolumes := make(volumeAttachments, 0, len(opts.providerOpts.AttachedVolumes))
	for i, attachedVolume := range opts.providerOpts.AttachedVolumes {
		attachedVolumes = append(attachedVolumes, &volumeAttachment{
			Name:                   fmt.Sprintf("%s-%s-%04d", opts.vmName, attachedVolume.VolumeType, i+1),
			ResourceGroupID:        p.config.roachprodResourceGroupID,
		...
		vpcService.NewCreateInstanceOptions(
			&vpcv1.InstancePrototype{
				AvailabilityPolicy: &vpcv1.InstanceAvailabilityPolicyPrototype{
					HostFailure: core.StringPtr("stop"),
				},
	...
	if err != nil {
		return nil, errors.Wrapf(
			err,
			"failed to create floating IP for instance %s in region %s",
			opts.vmName,
			opts.region,
		)
	}
	...
	for waitForRunningState.Next() {
		...
		if core.StringNilMapper(instanceResp.Status) != "running" {
			continue
		}
	...
	for _, i := range instances {
		...
		if opts.IncludeVolumes {
			...
			for _, attachedVolume := range i.instance.VolumeAttachments {
				...
				if *attachedVolume.ID == i.bootVolume.ID {
					continue
				}
...
func (p *Provider) checkCreateOpts(
	opts *vm.CreateOpts, providerOpts *ProviderOpts, zones []string,
) error {
	...
	if opts.SSDOpts.UseLocalSSD {
		return fmt.Errorf("IBM does not support local SSDs")
	}
	...
	for i, attachedVolume := range providerOpts.AttachedVolumes {
		if err := p.checkAttachedVolumeTypeSizeIops(
			...
			return errors.Wrapf(err, "attached volume %d is invalid", i)
		}
	}
