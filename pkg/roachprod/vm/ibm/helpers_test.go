...
func TestValInRange(t *testing.T) {
	...
	t.Run("specific-errors", func(t *testing.T) {
		...
		require.Error(t, err)
		require.Contains(t, err.Error(), "invalid range format")
	})
}
...
func TestCheckCreateOpts(t *testing.T) {
	...
	t.Run("valid-options", func(t *testing.T) {
		...
		err := p.checkCreateOpts(opts, providerOpts, zones)
		require.NoError(t, err)
	})
	...
	t.Run("local-ssd-not-supported", func(t *testing.T) {
		...
		err := p.checkCreateOpts(opts, providerOpts, zones)
		require.Error(t, err)
		assert.Contains(t, err.Error(), "IBM does not support local SSDs")
	})
	...
	t.Run("invalid-attached-volume", func(t *testing.T) {
		...
		err := p.checkCreateOpts(opts, providerOpts, zones)
		require.Error(t, err)
		assert.Contains(t, err.Error(), "attached volume 0 is invalid")
	})
}
