...
const (
	...
	defaultImageAMI    = "ibm-ubuntu-22-04-4-minimal-s390x-4"
...
// a random zone is selected from a random region.
...
func (p *Provider) Delete(l *logger.Logger, vms vm.List) error {
	...
	for _, vm := range vms {
		...
		g.Go(func() error {
			...
			instance, err := newInstanceFromCRN(p, vm.ProviderID, deleteFloatingIPs)
			if err != nil {
