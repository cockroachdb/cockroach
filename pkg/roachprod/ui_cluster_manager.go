...
func (m *uiClusterManager) RefreshAWSSSO(ctx context.Context) (string, error) {
	...
	// Give it a moment to write the initial output with the code
	...
	m.logger.Printf("AWS SSO login initiated")
	return output, nil
}
...
func (m *uiClusterManager) GetClusterInfo(ctx context.Context, clusterName string) (*ui.ClusterDetails, error) {
	...
	for i, vm := range cluster.VMs {
		...
		// PostgreSQL URL format: postgresql://root@<ip>:26257?sslmode=disable
		pgURL := fmt.Sprintf("postgresql://root@%s:26257?sslmode=disable", vm.PublicIP)
		details.PGURLs = append(details.PGURLs, pgURL)

		// Admin UI URL format: http://<ip>:26258 (26258 is the default admin UI port)
		adminURL := fmt.Sprintf("http://%s:26258", vm.PublicIP)
		details.AdminURLs = append(details.AdminURLs, adminURL)

		m.logger.Printf("Node %d: IP=%s, PG=%s, Admin=%s", i+1, vm.PublicIP, pgURL, adminURL)
	}
