...
func CreateCluster(
	nodes int, opts vm.CreateOpts, providersOptsContainer vm.ProvidersOptionsContainer,
) error {
	...
	return vm.ProvidersParallel(opts.VMProviders, func(p vm.Provider) error {
		return p.Create(vmLocations[p.Name()], opts, providersOptsContainer.Providers[p.Name()])
	})
}
