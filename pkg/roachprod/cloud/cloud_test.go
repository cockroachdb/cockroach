...
func (*fakeDNSProvider) LookupRecords(context.Context, vm.DNSType, string) ([]vm.DNSRecord, error) {
	return nil, nil
}
...
func TestGCDNSUsesConfiguredGCEProvider(t *testing.T) {
	withoutSlack(t)
	oldProvider, hadProvider := vm.DNSProviders[gce.ProviderName]
	...
	vm.DNSProviders[gce.ProviderName] = provider
	t.Cleanup(func() {
		if hadProvider {
			vm.DNSProviders[gce.ProviderName] = oldProvider
		} else {
			delete(vm.DNSProviders, gce.ProviderName)
		}
	})

	cld := NewCloud()
	cld.Clusters["live"] = &cloudcluster.Cluster{Name: "live"}
