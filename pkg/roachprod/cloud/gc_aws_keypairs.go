...
// getTemporaryCredentialsByAssumingRole uses AWS STS to get temporary AWS credentials by assuming an IAM role
func getTemporaryCredentialsByAssumingRole(
	roleArn, roleSessionName, region string,
) (*types.Credentials, error) {
	cfg, err := config.LoadDefaultConfig(context.TODO(), config.WithRegion(region))
	if err != nil {
		return nil, errors.Wrap(err, "GetCredentialsByAssumingRole: failed to load config")
	}
	...
	if err != nil {
		return nil, errors.Wrap(err, "GetCredentialsByAssumingRole: failed to assume role")
	}

	return tmpCredentials.Credentials, nil
}

// setTemporaryCredentialsByAssumingRole fetches temporary aws credentials by assuming an IAM role and sets
// amazon.AWSAccessKeyParam, amazon.AWSSecretParam, amazon.AWSTempTokenParam in environment variables
func setTemporaryCredentialsByAssumingRole(roleArn, roleSessionName, region string) error {
	tmpCredentials, err := getTemporaryCredentialsByAssumingRole(roleArn, roleSessionName, region)
	if err != nil {
		return err
	}
...
func GCAWS(l *logger.Logger, dryrun bool) error {
	...
	for _, accountId := range awsAccountIds {
		roleArn := fmt.Sprintf("arn:aws:iam::%s:role/shailendra.patel-test-role", accountId)
		roleSessionName := fmt.Sprintf("gc-role-session-%s", accountId)

		if err := setTemporaryCredentialsByAssumingRole(roleArn, roleSessionName, ""); err != nil {
