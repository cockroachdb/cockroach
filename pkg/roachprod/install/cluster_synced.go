...
func (c *SyncedCluster) Put(src, dest string) error {
	...
	if haveErr {
		errMsg := fmt.Sprintf("put %s failed", src)
...
func (c *SyncedCluster) Get(src, dest string) error {
	...
	if haveErr {
		errMsg := fmt.Sprintf("get %s failed", src)
		return errors.New(errMsg)
	}
	return nil
}
...
func (c *SyncedCluster) Init() error {
	...
	if !vers.AtLeast(version.MustParse("v20.1.0")) {
		return errors.WithDetail(err, "install.Init() failed: `roachprod init` only supported for v20.1 and beyond.")
	}
