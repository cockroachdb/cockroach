...
func (c *SyncedCluster) Put(src, dest string) {
	...
	if haveErr {
		clog.Fatalf(context.Background(), "put %s failed", src)
	}
}
...
func (c *SyncedCluster) Get(src, dest string) {
	...
	if haveErr {
		clog.Fatalf(context.Background(), "get %s failed", src)
	}
}
...
func (c *SyncedCluster) Parallel(
	...
	if err != nil {
		...
		clog.Fatal(context.Background(), "command failed")
	}
}
...
func (c *SyncedCluster) Init() {
	...
	if err != nil {
		clog.Fatalf(context.Background(), "unable to retrieve cockroach version: %v", err)
	}

	if !vers.AtLeast(version.MustParse("v20.1.0")) {
		clog.Fatal(context.Background(), "`roachprod init` only supported for v20.1 and beyond")
	}

	fmt.Printf("%s: initializing cluster\n", h.c.Name)
	initOut, err := h.initializeCluster(firstNodeIdx)
	if err != nil {
		clog.Fatalf(context.Background(), "unable to initialize cluster: %v", err)
	}
	if initOut != "" {
		fmt.Println(initOut)
	}
	...
	if err != nil {
		clog.Fatalf(context.Background(), "unable to set cluster settings: %v", err)
	}
