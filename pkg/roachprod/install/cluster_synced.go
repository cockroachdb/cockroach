...
func runWithMaybeRetry(
	...
	for r := retry.Start(runRetryOpts.Options); r.Next(); {
		res, err = f()
		...
		if err == nil && res.Err != nil {
			...
			if runRetryOpts.shouldRetryFn == nil || runRetryOpts.shouldRetryFn(res) {
				l.Printf("Encountered [%v] on attempt %v of %v", res.Err, r.CurrentAttempt()+1, runRetryOpts.MaxRetries+1)
				continue
			}
		}
		break
...
func (c *SyncedCluster) GetInternalIP(
	...
	sess, err := c.newSession(l, n, `hostname --all-ip-addresses`)
	if err != nil {
...
func (c *SyncedCluster) newSession(l *logger.Logger, node Node, cmd string) (session, error) {
	if c.IsLocal() {
		return newLocalSession(cmd), nil
	}
...
`
			...
			sess, err := c.newSession(l, node, buf.String())
			if err != nil {
				ch <- NodeMonitorInfo{Node: node, Err: err}
				wg.Done()
				return
...
func (c *SyncedCluster) runCmdOnSingleNode(
	...
	sess, err := c.newSession(l, node, nodeCmd)
	if err != nil {
		return newRunResultDetails(node, err), err
	}
	defer sess.Close()
...
func (c *SyncedCluster) fileExistsOnFirstNode(
	...
	if err := c.Parallel(l, display, 1, 0, func(i int) (*RunResultDetails, error) {
		node := c.Nodes[i]
		sess, err := c.newSession(l, node, `test -e `+path)
		if err != nil {
			return newRunResultDetails(node, err), err
		}
		defer sess.Close()
