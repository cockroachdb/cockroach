...
func (m *monitorNode) processMonitorOutput(lines []string) {
	...
	// Update process IDs from the monitor script output frame.
	for i := 0; i < len(lines); i += 2 {
		...
		processID, err := strconv.Atoi(vcLine[1])
		if err != nil {
			parseErr := errors.Wrap(err, "failed to parse process ID")
	...
	// Check for dead processes by comparing previous process IDs.
...
func (m *monitorNode) monitorNode(ctx context.Context, l *logger.Logger) {
	...
	if err := t.Execute(&buf, params); err != nil {
		templateErr := errors.Wrap(err, "failed to execute template")
		m.sendEvent(NodeMonitorInfo{Node: m.node, Event: MonitorError{templateErr}})
		return
	}

	// This is the exception to funneling all SSH traffic through `c.runCmdOnSingleNode`
	sess := m.cluster.newSession(l, m.node, buf.String(), withDebugDisabled())
	defer sess.Close()
	...
	go func(p io.Reader) {
		...
		for {
			...
			// An empty line indicates the end of a frame.
			if len(line) == 0 {
