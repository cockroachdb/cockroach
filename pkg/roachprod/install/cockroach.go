...
func (c *SyncedCluster) TenantStoreDir(node Node, tenantName string, instance int) string {
	suffix := tenantDirSuffix(tenantName, instance)
	if c.IsLocal() {
		return filepath.Join(c.localVMDir(node), "data"+suffix)
	}
	return fmt.Sprintf("data%s", suffix)
}
...
func tenantDirSuffix(tenantName string, instance int) string {
	suffix := ""
	if tenantName != SystemTenantName && tenantName != "" {
		suffix = fmt.Sprintf("-%s-%d", tenantName, instance)
	}
	return suffix
}
