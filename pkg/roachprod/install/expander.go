...
// instance is assumed.
func extractTenantInfo(m []string) (string, int, error) {
	...
	if m[1] != "" {
		var err error
		// Trim off the leading ':' in the capture group.
		tenantInstance, err = strconv.Atoi(trim(m[1]))
		if err != nil {
			return "", 0, err
		}
	}
...
func (e *expander) maybeExpandPgURL(
	...
	tenant, tenantInstance, err := extractTenantInfo(m[2:])
	if err != nil {
		return "", false, err
	}
	if e.pgURLs[tenant] == nil {
		e.pgURLs[tenant], err = c.pgurls(ctx, l, allNodes(len(c.VMs)), tenant, tenantInstance)
		if err != nil {
			return "", false, err
		}
	}
...
func (e *expander) maybeExpandPgPort(
	...
	if e.pgPorts == nil {
		...
		for _, node := range allNodes(len(c.VMs)) {
			desc, err := c.DiscoverService(node, tenant, ServiceTypeSQL, tenantInstance)
			if err != nil {
				return s, false, err
			}
