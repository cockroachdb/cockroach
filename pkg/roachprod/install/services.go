...
func (c *SyncedCluster) DiscoverService(
	...
	// If there are multiple services, prefer the one with the highest priority.
...
// the given nodes.
func (d ServiceDescriptors) Filter(nodes Nodes) []ServiceDesc {
	filteredDescriptors := make([]ServiceDesc, 0)
...
func (c *SyncedCluster) FindOpenPorts(
	...
	if c.IsLocal() {
		...
		defer func() {
			for _, p := range ports {
				if p >= localClusterPortCache.offset {
					localClusterPortCache.offset = p + 1
				}
			}
			localClusterPortCache.mu.Unlock()
		}()
		if localClusterPortCache.offset > startPort {
			startPort = localClusterPortCache.offset
		}
	}
	...
	res, err := c.runCmdOnSingleNode(ctx, l, node, buf.String(), false, l.Stdout, l.Stderr)
	if err != nil || res.Err != nil {
		return nil, errors.CombineErrors(err, res.Err)
	}
	ports, err = stringToIntegers(strings.TrimPrefix(res.Stdout, "open ports:"))
	if err != nil {
		return nil, err
	}
	return ports, nil
