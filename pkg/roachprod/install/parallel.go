...
// ParallelE runs the given function in parallel on the specified elements,
// which will typically be nodes.
//
...
func ParallelE[E any](
	ctx context.Context,
	l *logger.Logger,
	elements []E,
	fn func(ctx context.Context, e E) (*RunResultDetails, error),
	opts ...ParallelOption,
	...
	startNext := func() {
		...
		go func(i int) {
			...
			res, err := runWithMaybeRetry(
				groupCtx, l, options.retryOpts, func(ctx context.Context) (*RunResultDetails, error) { return fn(ctx, elements[i]) },
			)
			if err != nil {
				errorChannel <- err
				return
...
// Parallel runs a user-defined function across the elements provided, which
// will typically be the nodes in the cluster. If any of the commands fail,
// Parallel will log each failure and return an error.
//
...
func Parallel[E any](
	...
	results, hasError, err := ParallelE[E](ctx, l, elements, fn, opts...)
