...
func StartJaeger(
	ctx context.Context, l *logger.Logger, clusterName string, virtualClusterName string, secure bool,
) error {
	if err := LoadClusters(); err != nil {
		return err
	}
	...
	startCmd := fmt.Sprintf("docker run -d --name %s -p %[2]d:%[2]d -p %[3]d:%[3]d %s",
		...
		jaegerUIPort,
		jaegerImageName)
	err = c.Run(ctx, l, l.Stdout, l.Stderr, jaegerNode, "start jaegertracing/all-in-one using docker", startCmd)
	if err != nil {
		return err
	}
	...
	_, err = c.ExecSQL(ctx, l, jaegerNode, virtualClusterName, 0, []string{"-e", fmt.Sprintf(
		"SET CLUSTER SETTING trace.opentelemetry.collector='%s'", otelCollectionHostPort)})
	if err != nil {
		return err
	}
...
func StopJaeger(ctx context.Context, l *logger.Logger, clusterName string) error {
	...
	err = c.Run(ctx, l, l.Stdout, l.Stderr, jaegerNode, stopCmd, stopCmd)
	if err != nil {
		return err
	}
	rmCmd := fmt.Sprintf("docker rm %s", jaegerContainerName)
	return c.Run(ctx, l, l.Stdout, l.Stderr, jaegerNode, rmCmd, rmCmd)
}
