...
// PGURLOptions controls the kind of URL generated by PgURL.
type PGURLOptions struct {
	Secure   bool
	External bool
	// TenantName is the name of the tenant to connect to. If
	// non-empty, the -ccluser option is added to the PG URL,
	// which is supported by Cockroach 23.1+ to connect to a
	// specific tenant.
	//
	// If empty, no tenant option is added and the URL will result
	// in a connection to the server's configured default tenant.
	TenantName string
}
