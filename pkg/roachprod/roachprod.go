...
func updatePrometheusTargets(
	...
	for _, node := range c.Nodes {
		...
		go func(index int, v vm.VM) {
			...
			nodeIP := v.PrivateIP
			if reachability == promhelperclient.NodeProviderReachabilityPublic {
				nodeIP = v.PublicIP
			}
			nodeInfo := fmt.Sprintf("%s:%d", nodeIP, desc.Port)
			nodeIPPortsMutex.Lock()
			// ensure atomicity in map update
			if _, ok := nodeIPPorts[index]; !ok {
				nodeIPPorts[index] = []*promhelperclient.NodeInfo{
					{
						Target:       fmt.Sprintf("%s:%s", nodeIP, vm.NodeExporterPort),
						CustomLabels: createLabels(v, "node_exporter", !c.Secure),
					},
				}
			}
			nodeIPPorts[index] = append(
				nodeIPPorts[index],
