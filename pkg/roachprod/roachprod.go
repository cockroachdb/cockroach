...
func updatePrometheusTargets(ctx context.Context, l *logger.Logger, c *install.SyncedCluster) {
	// nodeIPPortInfo is used for sending the node IP information into a channel
	type nodeIPPortInfo struct {
		nodeIPPort string
		nodeIndex  int
	}
	// noIpInfoChannel channel is used so that DiscoverService can be executed in parallel
	noIpInfoChannel := make(chan *nodeIPPortInfo)
	...
	for i, node := range c.VMs {
		if node.Provider == gce.ProviderName {
			...
			go func(index int, v vm.VM) {
				...
				noIpInfoChannel <- &nodeIPPortInfo{nodeInfo, index}
			}(i, node)
		}
	}
	go func() {
		wg.Wait()
		close(noIpInfoChannel)
	}()
	nodeIPPorts := make([]string, len(c.Nodes))
	for {
		info, ok := <-noIpInfoChannel
		if !ok {
			break
		}
		nodeIPPorts[info.nodeIndex] = info.nodeIPPort
	}
