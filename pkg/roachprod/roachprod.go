...
func Deploy(
	ctx context.Context,
	l *logger.Logger,
	clusterName, applicationName, version, binaryPath string,
	...
	if applicationName == "local" {
		if binaryPath == "" {
			return errors.Errorf("%s application requires a binary path", applicationName)
		}
		err = c.Put(ctx, l, c.TargetNodes(), binaryPath, filepath.Join(stageDir, "cockroach"))
	} else {
