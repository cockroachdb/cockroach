...
const (
	...
	// bucket and testsFileLocation are for uploading the csv for each cloud in GCS
	bucket            = "roachtest-data"
	testsFileLocation = "tests/selected"
	testsCsvExtension = "csv"
	// project and secret are for fetching the sf creds for running query
...
func getSFCreds(ctx context.Context) (string, string, error) {
	...
	if err != nil {
		return "", "", fmt.Errorf("failed to setup secrets manager client: %w", err)
	}
	defer func() { _ = client.Close() }()
	// fetch the secret from the secrets manager
	...
	creds := string(result.GetPayload().GetData())
