...
func (s *ScratchHandle) SetFingerprint(
	...
	s.metadata.AsOfTime = asOf
	if err := s.registry.upsertMetadata(s.metadata); err != nil {
		return err
	}
	s.logger.Printf("fixture '%s' fingerprint set to '%s' with asOf time %s ", s.metadata.DataPath, s.metadata.Fingerprint, s.metadata.AsOfTime)
	return nil
}
