...
func Init(
	...
	if len(cfg.NodeExporter) > 0 {
		...
		if err := c.Run(ctx, l, l.Stdout, l.Stderr, install.OnNodes(cfg.NodeExporter).WithRetryFn(install.AlwaysRetry),
...
sudo systemd-run --unit node_exporter --same-dir ./node_exporter`,
	...
	if err := c.Run(
		...
		l.Stdout,
		l.Stderr,
		install.OnNodes(cfg.PrometheusNode).WithRetryFn(install.AlwaysRetry),
		"reset prometheus",
...
sudo systemd-run --unit prometheus --same-dir \
	...
	if cfg.Grafana.Enabled {
		...
		if err := c.Run(ctx, l,
			l.Stdout,
			l.Stderr, install.OnNodes(cfg.PrometheusNode).WithRetryFn(install.AlwaysRetry), "install grafana",
			fmt.Sprintf(`
...
sudo mkdir -p /var/lib/grafana/dashboards`,
		...
		if err := c.Run(ctx, l,
			l.Stdout,
			l.Stderr, install.OnNodes(cfg.PrometheusNode).WithRetryFn(install.AlwaysRetry), "permissions",
...
func Shutdown(
	...
	if err := c.Run(
		...
		l.Stdout,
		l.Stderr,
		install.OnNodes(promNode).WithRetryFn(install.AlwaysRetry),
		"stop prometheus",
