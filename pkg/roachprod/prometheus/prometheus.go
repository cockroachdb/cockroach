...
// WithPrometheusNode specifies the nodes to set up prometheus on.
...
sudo apt-get update -qqy && sudo apt-get install -qqy grafana-enterprise && sudo mkdir -p /var/lib/grafana/dashboards`,
		...
		if err := c.PutString(ctx, l, `apiVersion: 1
...
func StopPrometheus(
	...
	if err := c.Run(ctx, l, os.Stdout, os.Stderr, nodes, "stop grafana",
