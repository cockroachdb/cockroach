...
// Package owner deciphers OWNERS overrides in the file tree to determine
// which team owns a given file. The OWNERS file is in YAML format.
//
// An OWNERS file is structured as the following:
//  owners: # all owners by default in this directory and subdirectories
//    [sequence of teams found in TEAMS]
//  overrides: # override specific files with different owners.
...
// FileExpression is either a filename, or a wildcard for a set of overrides.
...
type OwnersYAML struct {
	...
	Overrides map[FileExpression][]team.Name `yaml:"overrides,omitempty"`
}

// Owners contains metadata about ownership for a given directory.
type Owners struct {
	// Owners is a list of owners for the team directory.
	// This can be superceded by "Overrides".
	Owners []team.Team
	// Overrides is a map from a file name wildcard expression
	// to a list of owners.
	Overrides map[FileExpression][]team.Team
}
...
func LoadOwners(r io.Reader, teams map[team.Name]team.Team) (*Owners, error) {
	...
	ret := &Owners{
		...
		Overrides: make(map[FileExpression][]team.Team, len(ownersIn.Overrides)),
	}
	...
	for fileExpr, owners := range ownersIn.Overrides {
		fileExprTeams := make([]team.Team, len(owners))
		...
		ret.Overrides[fileExpr] = fileExprTeams
	}
	return ret, nil
}
...
func FindOwners(fileOrDir string, teams map[team.Name]team.Team) ([]team.Team, error) {
	...
	for currentDir != prevDirOrFile {
		...
		for expr, owners := range loadedOwners.Overrides {
