...
// DefaultCodeOwnersLocation is the default location for the TEAMS.yaml file.https://github.com/mattn/go-zglob
...
// NewCodeOwners returns a new CodeOwners struct.
func NewCodeOwners(rules []Rule, teams map[team.Alias]team.Team) *CodeOwners {
	return &CodeOwners{
		rules: rules,
		teams: teams,
	}
...
func (co *CodeOwners) Match(inPath string) ([]team.Team, error) {
	...
	if err != nil {
		log.Fatalf("failed to stat %q: %v", inPath, err)
	}
	filePath := strings.TrimPrefix(
		fullPath,
		filepath.Join(os.Getenv("GOPATH"), "src/github.com/cockroachdb/cockroach"),
	)
