...
func getConnect(schema string) (*gosql.DB, context.Context, context.CancelFunc) {
	...
	if err != nil {
		log.Fatalf(ctx, "Could not  contruct connection string %v", err)
	}
	db, err := gosql.Open("snowflake", dsn)
	if err != nil {
		log.Fatalf(ctx, "Could not open sql connection %v", err)
	}
...
func writeRowsGCS(ctx context.Context, filename string, rows *gosql.Rows) string {
	credKey := envutil.EnvOrDefaultString(keyTag, "")
	if credKey == "" {
		log.Fatalf(ctx, "%s not set\n", keyTag)
		return ""
	}
	...
	for rows.Next() {
		...
		if _, err := wc.Write([]byte(rowData + "\n")); err != nil {
			log.Fatalf(ctx, "Failed to write to file: %v", err)
			return ""
		}

	}
	if err := wc.Close(); err != nil {
		log.Fatalf(ctx, "createFile: unable to close bucket %q, file %q: %v", bucketName, filename, err)
		return ""
	}
