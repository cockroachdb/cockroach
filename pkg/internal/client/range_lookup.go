...
// distinct range in the cluster. They map the logical keyspace in cockroach the
...
//   at "/meta2/p". So, we first determine the meta key for the lookup key using
...
//   key. We then issue a forward ScanRequest over this range. Since we already
//   know the range that contains this span of keys, we send the request
//   directly to that range's replica. We then find that the first KV pair we
//   see during the scan is at "/meta2/p". This is our desired range descriptor.
//  1.b: LookupRange(key=m)
...
//   let DistSender do it's job when scanning over the meta2 range.
//
...
func lookupRangeRevScan(
	...
	// descriptors if they aren't needed. This occurs in case 2 from above.
...
// IsRangeLookup returns if the provided BatchRequest is a single RangeLookup scan.
...
// IsRangeLookupRequest returns if the provided Request is a single RangeLookup scan.
