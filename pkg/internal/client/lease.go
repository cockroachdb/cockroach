...
func (m *LeaseManager) AcquireLease(ctx context.Context, key roachpb.Key) (*Lease, error) {
	...
	if err := m.db.Txn(ctx, func(txn *Txn) error {
		...
		if !m.leaseAvailable(&val) {
			return errors.Errorf("lease %q is not available until at least %s", key, val.Expiration)
		}
		lease.val = &LeaseVal{
			Owner:      m.clientID,
	...
	if err := m.db.Txn(ctx, func(txn *Txn) error {
		...
		if !l.val.Expiration.Equal(val.Expiration) {
			log.Warningf(ctx,
				...
				l.val.Expiration, val.Expiration, l.key)
			l.val.Expiration = val.Expiration
		}
		if val.Owner != m.clientID {
			// Something is wrong - immediately expire the local lease state.
			l.val.Expiration = hlc.Timestamp{}
			return errors.Errorf("can't extend lease whose owner %q is not this process (%q)",
				val.Owner, m.clientID)
		} else if m.timeRemaining(&val) < 0 {
			return errors.Errorf("can't extend lease that expired at time %s", val.Expiration)
		}
		newVal = &LeaseVal{
			Owner:      m.clientID,
			Expiration: m.clock.Now().Add(m.leaseDuration.Nanoseconds(), 0),
		}
		if err := txn.Put(l.key, newVal); err != nil {
			return err
		}
		l.val = newVal
		return nil
...
func (m *LeaseManager) ReleaseLease(ctx context.Context, l *Lease) error {
	return m.db.Txn(ctx, func(txn *Txn) error {
		var val LeaseVal
		err := txn.GetProto(l.key, &val)
		if err != nil {
			return err
		...
		if val.Owner != m.clientID {
			return errors.Errorf("can't release lease whose owner %q is not this process (%q)",
				val.Owner, m.clientID)
		}
