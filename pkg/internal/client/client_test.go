...
func TestClientRetryNonTxn(t *testing.T) {
	...
	for i, test := range testCases {
		log.Infof(context.TODO(), "running test case %d", i)
		...
		err := db.Txn(context.TODO(), func(ctx context.Context, txn *client.Txn) error {
			...
			if count == 1 {
				...
				log.Infof(context.TODO(), "on loop %d (%p)", count, notify)
				...
				testutils.SucceedsSoon(t, func() error {
					...
					if _, ok := mu.m[string(key)]; ok {
						log.Infof(context.TODO(), "txn push attempted")
						return nil
					}
				...
				if test.canPush {
					...
					// attempts.
					log.Infof(context.TODO(), "can push, waiting for notification")
					<-notify
					log.Infof(context.TODO(), "can push, received")
				}
			}
			return nil
		})
