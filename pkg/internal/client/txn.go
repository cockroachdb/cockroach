...
func (txn *Txn) Send(
	...
	if retryErr, ok := pErr.GetDetail().(*roachpb.HandledRetryableTxnError); ok {
		...
		txn.replaceSenderMaybeLocked(ctx, retryErr, requestTxnID)
		txn.mu.Unlock()
	}
	return br, pErr
}
...
func (txn *Txn) UpdateStateOnRemoteRetryableErr(ctx context.Context, pErr *roachpb.Error) error {
	...
	txn.replaceSenderMaybeLocked(ctx, pErr.GetDetail().(*roachpb.HandledRetryableTxnError), origTxnID)

	return pErr.GoError()
}

// BumpEpochAfterConcurrentRetryError bumps the transaction epoch manually and
// resets the transaction state away from txnError. This is meant to be used
...
func (txn *Txn) BumpEpochAfterConcurrentRetryError(ctx context.Context) {
	txn.mu.sender.BumpEpochAfterConcurrentRetryError(ctx)
}

// replaceSenderMaybeLocked handles TransactionAbortedErrors, on which a new
...
func (txn *Txn) replaceSenderMaybeLocked(
...
// ManualRestart bumps the transactions epoch, and can upgrade the priority and
// timestamp.
