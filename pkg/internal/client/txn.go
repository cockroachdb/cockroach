...
func (txn *Txn) Send(
	...
	if pErr != nil {
		...
		if retryErr, ok := pErr.GetDetail().(*roachpb.HandledRetryableTxnError); ok {
			newPErr := txn.updateStateOnRetryableErrLocked(ctx, *retryErr, requestTxnID)
			if newPErr != nil {
				return nil, newPErr
			}
		}
	...
	if br != nil {
		...
		if br.Txn != nil && br.Txn.ID != txn.mu.Proto.ID {
			txn.forgetTxnOnAbort(ctx, br.Txn)
			return nil, roachpb.NewError(&roachpb.RespForPrevTxnIDError{})
		}
...
func (txn *Txn) updateStateOnRetryableErrLocked(
	...
	if abortErr {
		...
		//
		// TODO(nvanbenschoten): See comment on forgetTxnOnAbort. We only need
		// this because locking isn't shared between the TxnCoordSender and
		// Txn.Send.
		txn.forgetTxnOnAbort(ctx, &txn.mu.Proto)
...
// a single lock.
func (txn *Txn) forgetTxnOnAbort(ctx context.Context, txnProto *roachpb.Transaction) {
	if tc, ok := txn.db.sender.(SenderWithTxnCoordBackdoor); ok {
		tc.ForgetTxn(ctx, *txnProto)
	}
}
