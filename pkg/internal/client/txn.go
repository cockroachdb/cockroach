...
func (txn *Txn) IsFinalized() bool {
	txn.mu.Lock()
	defer txn.mu.Unlock()
	return txn.isFinalizedLocked()
}

func (txn *Txn) isFinalizedLocked() bool {
...
//
// Not Thread-Safe Accessors
//
// These methods access data that should not be changed during the execution of
// a running transaction, and therefore do not need to be synchronized.
//

// Proto not thread-safe, only use if you know that no requests are executing concurrently.
//
// A thread-safe alternative would be to clone the Proto under lock and return this
// clone, but we currently have no situations where this is needed.
...
type TxnExecOptions struct {
	...
	// ordering.
	//
	// REVIEW: is there a better name for this?
	AssignOrigTimestamp bool
}
...
func (txn *Txn) Exec(opt TxnExecOptions, fn func(txn *Txn, opt *TxnExecOptions) error) (err error) {
	...
	for {
		if txn != nil {
			...
			if opt.AssignOrigTimestamp && !txn.mu.Proto.IsInitialized() {
		...
		if err == nil && opt.AutoCommit {
			...
			case roachpb.PENDING:
				// fn succeeded, but didn't commit. This may race with another
				// EndTxnRequest.
				//
...
func (txn *Txn) send(ba roachpb.BatchRequest) (*roachpb.BatchResponse, *roachpb.Error) {
	...
	lockedPrelude := func() *roachpb.Error {
		...
		if txn.mu.Proto.Status != roachpb.PENDING || txn.isFinalizedLocked() {
...
func InitTxnProtoFromRequest(
	...
	newTxn := roachpb.MakeTransaction(
		proto.Name,
		nil, /* baseKey */
		priority,
		proto.Isolation,
