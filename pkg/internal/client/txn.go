...
func (txn *Txn) IsRetryableErrMeantForTxn(retryErr roachpb.RetryableTxnError) bool {
	txn.mu.Lock()
	defer txn.mu.Unlock()
	return txn.isRetryableErrMeantForTxnLocked(retryErr)
}

func (txn *Txn) isRetryableErrMeantForTxnLocked(retryErr roachpb.RetryableTxnError) bool {
...
func (txn *Txn) send(
	...
	if pErr != nil {
		...
		if pErr.TransactionRestart != roachpb.TransactionRestart_NONE {
			retryErr, ok := pErr.GetDetail().(*roachpb.HandledRetryableError)
			if !ok {
				log.Fatalf(ctx, "unexpected retryable error at the client.Txn level: (%T) %s", pErr.GetDetail(), pErr)
			}
			txn.updateStateOnRetryableErrLocked(ctx, *retryErr, ba.UserPriority, requestTxnID, requestEpoch)
		}
		return nil, pErr
	}
...
func (txn *Txn) updateStateOnRetryableErrLocked(
	ctx context.Context,
	retryErr roachpb.HandledRetryableError,
