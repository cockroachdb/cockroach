...
func (txn *Txn) Send(
	...
	if br != nil {
		...
		if br.Txn != nil && br.Txn.ID != txn.mu.Proto.ID {
			txn.abandonTxnOnAbort(ctx, br.Txn)
			return nil, roachpb.NewError(&roachpb.TxnAbortedAsyncErr{})
		}
...
func (txn *Txn) UpdateStateOnRemoteRetryableErr(ctx context.Context, pErr roachpb.Error) {
	...
	if _, ok := txn.db.GetSender().(SenderWithTxnCoordBackdoor).GetTxnState(txnID); ok {
...
func (txn *Txn) updateStateOnRetryableErrLocked(
	...
	if abortErr {
		...
		// Make sure that the TxnCoordSender forgets about the previous
		...
		// be being heartbeated again. This is because the TxnCoordSender has
		// no memory so it will consider it a new transaction.
		//
		// TODO(nvanbenschoten): See comment on abandonTxnOnAbort. We only need
		// this because locking isn't shared between the TxnCoordSender and
		// Txn.Send.
		txn.abandonTxnOnAbort(ctx, &txn.mu.Proto)
...
// abandonTxnOnAbort assures that if the Txn's wrapped sender is a TxnCoordSender,
// then it will not continue to heartbeat the specified Transaction. This is needed
// in cases where we have sent multiple requests concurrently and one resulted in
// a transaction abort. Because the abort will change the Txn ID, the TxnCoordSender
// should forget about the old Txn incarnation. However, the timing of responses for
// requests sent by the old Txn incarnation could result in the TxnCoordSender thinking
// responses to the old Txn ID are responses for a new Transaction. We therefore need
// to make sure that when we detect a concurrent txn abort while handling responses
// (successful or otherwise) in Txn.Send, that we communicate back to the TxnCoordSender
// to drop the old Txn incarnation.
//
// TODO(nvanbenschoten): The need for this in so many places here is due to a failure
// in the Txn/TxnCoordSender API. When we first handle transaction aborts, we're under
// lock in TxnCoordSender. However, we release this lock until we grab a different
// lock to handle the abort in Txn. Because we unlock while passing over the API border,
// response handling can be ordered differently for a pair of concurrent requests.
// Ripping out TxnCoordSender and merging its responsibility with Txn could prevent
// this and allow concurrent abort handling logic to be much simpler. For instance,
// we could ignore any responses for requests in the previousIDs set, all while under
// a single lock.
func (txn *Txn) abandonTxnOnAbort(ctx context.Context, txnProto *roachpb.Transaction) {
	if tc, ok := txn.db.sender.(SenderWithTxnCoordBackdoor); ok {
		tc.AbandonTxnIfExists(ctx, txnProto)
	}
}
