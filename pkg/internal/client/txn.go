...
type Txn struct {
	...
	// leafOrRoot indicates the type of transaction.
	leafOrRoot TxnType
...
// NewTxn returns a new txn. The leafOrRoot parameter specifies whether this
...
func NewTxn(db *DB, gatewayNodeID roachpb.NodeID, leafOrRoot TxnType) *Txn {
	return NewTxnWithProto(db, gatewayNodeID, leafOrRoot, roachpb.MakeTransaction(
		"unnamed",
		nil, // baseKey
...
func NewTxnWithProto(
	db *DB, gatewayNodeID roachpb.NodeID, leafOrRoot TxnType, proto roachpb.Transaction,
) *Txn {
	if db == nil {
		log.Fatalf(context.TODO(), "attempting to create txn with nil db for Transaction: %s", proto)
	}
	proto.AssertInitialized(context.TODO())
	txn := &Txn{db: db, leafOrRoot: leafOrRoot, gatewayNodeID: gatewayNodeID}
	txn.mu.Proto = proto
	txn.mu.sender = db.factory.New(leafOrRoot)
	return txn
}
...
// when running a request through the txn.
func (txn *Txn) UpdateStateOnRemoteRetryableErr(
	...
	// ignore the error.
	if retryErr.TxnID != txn.mu.Proto.ID {
		log.Eventf(ctx, "retryable error for an older version of txn: current txn: %s, retryErr: %s",
			txn.mu.Proto, retryErr)
		return
	}
	txn.updateStateOnRetryableErrLocked(ctx, retryErr)
}
...
func (txn *Txn) updateStateOnRetryableErrLocked(
	...
	if abortErr {
		...
		txn.mu.sender = txn.db.factory.New(txn.leafOrRoot)
	} else {
