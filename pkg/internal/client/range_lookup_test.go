...
func TestRangeLookupRaceSplits(t *testing.T) {
	...
	t.Run("MissingDescriptor", func(t *testing.T) {
		...
		sender := SenderFunc(func(_ context.Context, ba roachpb.BatchRequest) (*roachpb.BatchResponse, *roachpb.Error) {
			...
			assertReadConsistency := func(rc roachpb.ReadConsistencyType) {
				if ba.Txn != nil {
					t.Fatalf("unexpected batch transaction, found %v", ba)
				}
				if a := ba.ReadConsistency; a != rc {
					t.Fatalf("expected read consistency %v, found %v", rc, a)
				}
			}
			switch attempt {
			case 0:
				assertReadConsistency(roachpb.INCONSISTENT)
				// Return an inconsistent result.
				return badRes, nil
			case 1:
				assertReadConsistency(roachpb.CONSISTENT)
	...
	// This scenerio is similar to the previous one, but assumes that a scan
