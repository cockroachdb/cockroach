...
func TestTxnRequestTxnTimestamp(t *testing.T) {
	...
	testCases := []struct {
		...
		{hlc.Timestamp{WallTime: 0, Logical: 0}, hlc.Timestamp{WallTime: 10, Logical: 0}},
	...
	for testIdx = range testCases {
		if _, pErr := txn.sendInternal(context.Background(), ba); pErr != nil {
			t.Fatal(pErr)
		}
	}
}
...
func TestTxnResetTxnOnAbort(t *testing.T) {
	...
	txn := NewTxn(db)
	_, pErr := txn.sendInternal(context.Background(), testPut())
...
func TestSetPriority(t *testing.T) {
	...
	if _, pErr := txn.sendInternal(context.Background(), roachpb.BatchRequest{}); pErr != nil {
		t.Fatal(pErr)
	}
