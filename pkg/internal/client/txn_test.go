	...
	for _, success := range []bool{true, false} {
		...
		if err := db.Txn(context.TODO(), func(ctx context.Context, txn *Txn) error {
			if !ok {
				...
				intRetErr := roachpb.NewErrorWithTxn(
					roachpb.NewTransactionRetryError(), txn.Proto()).GoError().(*roachpb.InternalRetryableTxnError)
				return roachpb.NewError(
					roachpb.NewHandledRetryableError(
						intRetErr.Error(), intRetErr.TxnID, *intRetErr.Transaction)).GoError()
			}
			if !success {
...
func TestTransactionKeyNotChangedInRestart(t *testing.T) {
	...
	db := NewDB(newTestSender(func(ba roachpb.BatchRequest) (*roachpb.BatchResponse, *roachpb.Error) {
		...
		if attempt == 0 {
			...
			// get the retryable error expected by db.Txn().
			intRetErr := roachpb.NewErrorWithTxn(
				roachpb.NewTransactionRetryError(), ba.Txn).GoError().(*roachpb.InternalRetryableTxnError)
			return nil, roachpb.NewError(roachpb.NewHandledRetryableError(intRetErr.Error(), intRetErr.TxnID, *intRetErr.Transaction))
		}
		return ba.CreateReply(), nil
	}), clock)
					...
					if _, ok := ba.GetArg(roachpb.Put); ok {
						...
						if count == 1 {
							...
							if pErr.TransactionRestart != roachpb.TransactionRestart_NONE {
								// HACK ALERT: to do without a TxnCoordSender, we jump through hoops to
								// get the retryable error expected by db.Txn().
								intRetErr := pErr.GoError().(*roachpb.InternalRetryableTxnError)
								intRetErr.Transaction = ba.Txn
								return nil, roachpb.NewError(roachpb.NewHandledRetryableError(
									intRetErr.Error(), intRetErr.TxnID, *intRetErr.Transaction))
							}
							return nil, pErr
						}
					}
...
func TestWrongTxnRetry(t *testing.T) {
	...
	txnClosure := func(ctx context.Context, outerTxn *Txn) error {
		...
		innerClosure := func(ctx context.Context, innerTxn *Txn, opt *TxnExecOptions) error {
			...
			intRetErr := roachpb.NewErrorWithTxn(&roachpb.TransactionPushError{
				PusheeTxn: *outerTxn.Proto()}, innerTxn.Proto()).GoError().(*roachpb.InternalRetryableTxnError)
