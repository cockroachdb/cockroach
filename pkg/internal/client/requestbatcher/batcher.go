...
type Config struct {
	...
	// MaxBytesPerBatchReq is the maximum number of bytes that each batch is
	// allowed to touch during one of its requests. If the limit is exceeded,
	// the batch is paginated over a series of individual requests. This limit
	// corresponds to the TargetBytes assigned to the Header of each request. If
	// MaxBytesPerBatchReq <= 0 then no limit is enforced.
...
func (b *RequestBatcher) sendBatch(ctx context.Context, ba *batch) {
	if err := b.cfg.Stopper.RunAsyncTask(ctx, "send-batch", func(ctx context.Context) {
		...
		// if MaxKeysPerBatchReq or MaxBytesPerBatchReq is set. If so, partial
...
func (b *batch) batchRequest(cfg *Config) *roachpb.BatchRequest {
	...
	if cfg.MaxBytesPerBatchReq > 0 {
		req.TargetBytes = int64(cfg.MaxBytesPerBatchReq)
	}
	return req
}
