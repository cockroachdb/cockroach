...
// TODO(ajwerner): Consider a more sophisticated machanism to limit on maximum
...
type Config struct {
	...
	// number of in flight batches is again below this threshold. Note that values
	// smaller than 10 will result in a limit of 10. Note that this value does not
	// limit the number of batches which may ultimately be in flight as batches
	// which are queued to send but not yet in flight will still send.
