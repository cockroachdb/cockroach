...
func (s *Smither) generateWhereTLP() (unpartitioned, partitioned string) {
	...
	part1 := fmt.Sprintf("SELECT * FROM %s WHERE %s",
		tableName, predicate)
	part2 := fmt.Sprintf("SELECT * FROM %s WHERE NOT (%s)",
		tableName, predicate)
	part3 := fmt.Sprintf("SELECT * FROM %s WHERE (%s) IS NULL",
		tableName, predicate)
...
func (s *Smither) generateJoinTLP() (unpartitioned, partitioned string) {
	...
	leftJoinTrue := fmt.Sprintf("SELECT * FROM %s LEFT JOIN %s ON TRUE",
		tableName1, tableName2)
	leftJoinFalse := fmt.Sprintf("SELECT * FROM %s LEFT JOIN %s ON FALSE",
		tableName1, tableName2)

	unpartitioned = fmt.Sprintf("SELECT count(*) FROM (%s UNION ALL %s UNION ALL %s)",
	...
	part1 := fmt.Sprintf("SELECT * FROM %s LEFT JOIN %s ON %s",
		tableName1, tableName2, predicate)
	part2 := fmt.Sprintf("SELECT * FROM %s LEFT JOIN %s ON NOT (%s)",
		tableName1, tableName2, predicate)
	part3 := fmt.Sprintf("SELECT * FROM %s LEFT JOIN %s ON (%s) IS NULL",
		tableName1, tableName2, predicate)
