...
// tested. We currently only test basic WHERE and JOIN query filters. It is possible to
// use TLP to test aggregations, GROUP BY, and HAVING, which have all
...
// generateWhereTLP returns two SQL queries as strings that can be used by the GenerateTLP function.
// These queries make use of the WHERE clause to partition the original query into three.
...
// generateJoinTLP returns two SQL queries as strings that can be used by the GenerateTLP function.
// These queries make use of LEFT JOIN to partition the original query in two ways. The latter query
// is partitioned by a predicate p, while the former is not.
...
// and then all rows concatenated with NULL values for the second and third parts
// (JOIN ON FALSE). Recall our TLP logical guarantee that a given predicate p
// satisfies either (p), NOT (p), or (p) is NULL. It follows that for any row
// in table1, exactly one of the expressions (p), NOT (p), or (p) is NULL will
// resolve to TRUE. For a given row, when the expression resolves to TRUE in table1,
// it matches with every row in table2. Otherwise, it is concatenated with null values.
// So each row in table1 is matched with every row in table2 exactly once (CROSS JOIN) and
// also matched with NULL values exactly twice, as expected by the unpartitioned query.
//
// If the resulting counts of the two queries are not equal, there is a logical
// bug.
