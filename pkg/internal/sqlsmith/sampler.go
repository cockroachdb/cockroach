...
// reading 4 values from the chan would return `0` one time and `1` three
// times in some random order. All weights must be >= 1.
...
func (w *WeightedSampler) Next() int {
	w.mu.Lock()
	if w.n == 0 {
		// Generate a new order each time through the samples so we don't repeat
		// after the first loop.
		w.rnd.Shuffle(len(w.samples), func(i, j int) {
			w.samples[i], w.samples[j] = w.samples[j], w.samples[i]
		})
	}
	v := w.samples[w.n]
	w.n++
