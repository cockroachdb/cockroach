...
func TestInitNewCluster(t *testing.T) {
	db := &fakeDB{}
	mgr := Manager{
	...
	testCases := [][]migrationDescriptor{
		{},
		{noopMigration1},
		{noopMigration1, noopMigration2},
		{errorMigration},
		{panicMigration},
		{noopMigration1, errorMigration, noopMigration2, panicMigration},
	}
	for _, migrations := range testCases {
		t.Run("", func(t *testing.T) {
			db.kvs = make(map[string][]byte)
			backwardCompatibleMigrations = migrations
			if err := mgr.InitNewCluster(context.Background()); err != nil {
				t.Error(err)
			}
			for _, migration := range migrations {
				key := string(migrationKey(migration))
				if _, ok := db.kvs[key]; !ok {
					t.Errorf("expected key %s to be written, but it wasn't", key)
				}
			}
			if len(db.kvs) != len(migrations) {
				t.Errorf("expected %d key to be written, but %d were",
					len(migrations), len(db.kvs))
			}
		})
	}
}
...
// cause the work function's context to be cancelled.
func TestLeaseExpiration(t *testing.T) {
	...
	waitForExitMigration := migrationDescriptor{
		name: "wait for context to be cancelled",
		workFn: func(ctx context.Context, _ runner) error {
			select {
			case <-ctx.Done():
				return nil
