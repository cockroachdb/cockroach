...
// GetOriginalTypeName extracts the original (non-normalized to a key)
// type name of an error. This can be used to print out error details in reports.
func GetOriginalTypeName(err error) string {
	switch t := err.(type) {
	case *opaqueLeaf:
		return t.details.OriginalTypeName
	case *opaqueWrapper:
		return t.details.OriginalTypeName
	}

	return getFullTypeName(err)
}
