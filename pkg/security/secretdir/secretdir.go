...
func (r *Reader) Validate(absPath string) error {
	if r == nil {
		return errors.WithHint(
			errors.New("--secret-directory is not configured"),
			"Set --secret-directory at node startup on every node, "+
	...
	if !fileutil.IsContained(r.absBase, filepath.Clean(absPath)) {
		return errors.WithHintf(
			errors.Errorf("path %q escapes --secret-directory %q", absPath, r.absBase),
			"Place the credential file under %q or adjust --secret-directory.", r.absBase)
	}
	return nil
}
