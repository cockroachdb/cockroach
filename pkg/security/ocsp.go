...
func makeOCSPVerifier(settings TLSSettings) func([][]byte, [][]*x509.Certificate) error {
	return func(rawCerts [][]byte, verifiedChains [][]*x509.Certificate) error {
		...
		ctx, cancel := context.WithTimeout(context.Background(), settings.ocspTimeout())
		defer cancel()
...
func queryOCSP(ctx context.Context, url string, cert, issuer *x509.Certificate) (bool, error) {
	...
	if httpResp.StatusCode != http.StatusOK {
		return false, errors.Newf("OCSP server returned status code %v", httpResp.StatusCode)
	}
	if ct := httpResp.Header.Get("Content-Type"); ct != "application/ocsp-response" {
		return false, errors.Newf("OCSP server returned unexpected content-type %q", ct)
	}
	...
	case ocsp.Revoked:
		return false, nil
	default:
		return false, errors.Newf("OCSP returned status %v", ocspResp.Status)
	}
}
