...
var allowedCiphers = struct {
	sync.Once
	ciphers allowedTLSCiphers
}{}

// init initializes the allowedTLSCiphers with all the ciphers which golang
// implements and have been allowed for cockroach in RecommendedCipherSuites,
// OldCipherSuites or as part of TLS 1.3 ciphers in crypto/tls.
func (ciphers *allowedTLSCiphers) init() {
...
func getCipherNameFromID(cid uint16) (cipher string, ok bool) {
	allowedCiphers.Do(func() {
		if len(allowedCiphers.ciphers.ciphersMapByName) == 0 {
			allowedCiphers.ciphers.init()
		}
	})
	allowedCiphersMap := allowedCiphers.ciphers.getAllowedCiphersMapByID()
...
func SetTLSCipherSuitesConfigured(ciphers []string) error {
	allowedCiphers.Do(func() {
		if allowedCiphers.ciphers.ciphersMapByName == nil {
			allowedCiphers.ciphers.init()
		}
	})
	allowedCiphersMap := allowedCiphers.ciphers.getAllowedCiphersMapByName()
...
// GetTLSCipherSuitesConfigured retrieves the tls ciphers set for all incoming
// connections(sql/rpc/http, etc.) of a node. These are set through the
// SetTLSCipherSuitesConfigured function defined in this package.
...
var TLSCipherRestrict = func(conn net.Conn) (err net.Error) {
	if configuredCiphers := GetTLSCipherSuitesConfigured(); len(configuredCiphers) != 0 {
		if tlsConn, ok := conn.(*tls.Conn); ok {
			...
			return &cipherRestrictError{errors.New("connection provided is not TLS, cannot obtain TLS connection details")}
		}
	}
	return
}
