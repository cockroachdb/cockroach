...
func TLSCipherRestrict(conn net.Conn) (err error) {
	if configuredCiphers := GetTLSCipherSuitesConfigured(); len(configuredCiphers) != 0 {
		...
		if tlsConn, ok := conn.(*tls.Conn); ok {
			...
			if !ok {
				return errors.Errorf(cipherRestrictErr+"cipher id %v does match implemented tls ciphers", selectedCipherID)
			}
			if !slices.Contains(configuredCiphers, cName) {
				return errors.Newf(cipherRestrictErr+"presented cipher %s not in allowed cipher suite list", cName)
			}
		} else {
			return errors.New(cipherRestrictErr + "connection provided is not TLS, cannot obtain TLS connection details")
		}
	}
	return
}
