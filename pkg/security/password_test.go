...
func TestBCryptToSCRAMConversion(t *testing.T) {
	...
	for i := bcrypt.MinCost; i < ourDefault+3; i++ {
		t.Run(fmt.Sprintf("bcrypt=%d", i), func(t *testing.T) {
			...
			converted, newHash, err := MaybeUpgradePasswordHash(ctx, &s.SV, cleartext, bh)
			require.NoError(t, err)
			require.True(t, converted)
			require.Equal(t, HashSCRAMSHA256, newHash.Method())
			...
			ec, _, err := MaybeUpgradePasswordHash(ctx, &s.SV, cleartext, newHash)
			require.NoError(t, err)
			require.False(t, ec)
		})
	}
