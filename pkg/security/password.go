...
var SCRAMCost = settings.RegisterIntSetting(
	...
	//
	// The default value 130000 incurs ~60ms latency on the sane hw.
...
const scramMaxCost = 10 * 1024 * 1024 // arbitrary value to prevent unreasonably long logins
...
func MaybeUpgradePasswordHash(
...
) (converted bool, newHash PasswordHash, err error) {
	...
	// (The last two conditions are checked by GetConfiguredHashMethod).
	if !isBcrypt || !autoUpgradePasswordHashes.Get(sv) || GetConfiguredHashMethod(ctx, sv) != HashSCRAMSHA256 {
		// Nothing to do.
		return false, hashed, nil
	}
	...
	if err != nil {
		...
		// already.
		return false, hashed, errors.NewAssertionErrorWithWrappedErrf(err, "programming error: authn succeeded but invalid bcrypt hash")
	}
	if bcryptCost < 0 || bcryptCost >= len(bcryptCostToSCRAMIterCount) || bcryptCostToSCRAMIterCount[bcryptCost] == 0 {
		...
		return false, hashed, errors.AssertionFailedf("unexpected: bcrypt cost %d is out of bounds or has no mapping", bcryptCost)
	}
	...
	if newHash.Method() != HashSCRAMSHA256 {
		...
		return false, hashed, errors.AssertionFailedf("programming error: re-hash failed to produce SCRAM hash, produced %T instead", newHash)
	}
