...
var AutoDetectPasswordHashes = settings.RegisterBoolSetting(
	...
	"whether the server recognizes pre-hashed user passwords",
	true,
).WithPublic()
...
var bcryptHashRe = regexp.MustCompile(`^BCRYPT\$\d[a-z]?\$\d\d\$[0-9A-Za-z\./]{22}[0-9A-Za-z\./]+$`)
