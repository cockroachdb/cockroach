...
const (
	...
	beginLDAPProvisionCounterName          = ldapCounterPrefix + "begin_provisioning"
	provisionLDAPSuccessCounterName        = ldapCounterPrefix + "provisioning_success"
...
func ClusterProvisioningConfig(settings *cluster.Settings) UserProvisioningConfig {
	ldapProvisioningEnabled.SetOnChange(&settings.SV, func(_ context.Context) {
		if ldapProvisioningEnabled.Get(&settings.SV) == true {
			telemetry.Inc(enableLDAPProvisionCounter)
		}
	})
	return clusterProvisioningConfig{}
}
