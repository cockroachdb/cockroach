...
func TestIsTenantNameSANURI(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err != nil {
				t.Fatalf("invalid SAN URI: %q", tc.sanURL)
			}

			if actual := security.IsTenantNameSANURI(url); actual != tc.expected {
				t.Errorf("invalid tenant name SAN URI: %q", tc.sanURL)
			}
		})
	}
}
...
func TestIsCRDBSANURI(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if actual := security.IsCRDBSANURI(url); actual != tc.expected {
				t.Errorf("invalid crdb SAN URI: %q", tc.sanURL)
			}
		})
	}
}
