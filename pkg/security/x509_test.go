...
func TestGenerateCertLifetime(t *testing.T) {
	...
	defTenantIDs := []roachpb.TenantID{roachpb.SystemTenantID}
	_, err = security.GenerateClientCert(caCert, testKey, testKey.Public(), clientDuration, security.TestUserName(), defTenantIDs)
	if !testutils.IsError(err, "CA lifetime is .*, shorter than the requested .*") {
		t.Fatal(err)
	}
	...
	clientBytes, err := security.GenerateClientCert(caCert, testKey, testKey.Public(), clientDuration, security.TestUserName(), defTenantIDs)
	if err != nil {
		t.Fatal(err)
	}
