...
func (s *hmacCancelAuth) Authenticate(
	...
	if validNonce {
		// Compute the hmac.
		expH := computeCancelHMAC(sessionID, s.sharedSecret, curNonce)
...
func computeCancelHMAC(sessionID, sharedSecret string, nonce uint32) []byte {
	mac := hmac.New(sha256.New, []byte(sharedSecret))
	fmt.Fprintf(mac, "%s:%d", sessionID, nonce)
	return mac.Sum(nil)
}
...
func (s *hmacCancelRequestClient) MakeRequest() (string, CancelRequest) {
	h := computeCancelHMAC(s.sessionID, s.clientKey, s.nonce)
