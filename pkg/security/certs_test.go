...
func TestGenerateClientCerts(t *testing.T) {
	...
	tenantID := roachpb.MakeTenantID(123)
	...
	expectedSAN, err := security.MakeTenantURISAN(user, tenantID)
	require.NoError(t, err)
	for _, info := range infos {
		...
		require.Equal(t, expectedSAN, info.ParsedCertificates[0].URIs[0])
	}
}
...
func generateSplitCACerts(certsDir string) error {
	...
	if err := security.CreateClientPair(
		certsDir, filepath.Join(certsDir, security.EmbeddedClientCAKey),
		testKeySize, time.Hour*48, true, username.NodeUserName(), roachpb.SystemTenantID, false,
	); err != nil {
	...
	if err := security.CreateClientPair(
		certsDir, filepath.Join(certsDir, security.EmbeddedClientCAKey),
		testKeySize, time.Hour*48, true, username.RootUserName(), roachpb.SystemTenantID, false,
	); err != nil {
		return errors.Wrap(err, "could not generate Client pair")
	}
