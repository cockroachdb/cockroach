...
func TestGenerateTenantCerts(t *testing.T) {
	...
	cp, err := security.CreateTenantClientPair(
		...
		time.Hour,
		"tenant999",
		[]string{"1.2.3.4", "apples.and.banan.as"},
	)
	...
	require.Equal(t, []string{"apples.and.banan.as"}, infos[1].ParsedCertificates[0].DNSNames)
	require.Len(t, infos[1].ParsedCertificates[0].IPAddresses, 1)
	require.Equal(t,
		"1.2.3.4",
		infos[1].ParsedCertificates[0].IPAddresses[0].String(),
	)
}
