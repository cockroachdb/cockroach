	...
	testData := []struct {
		...
		},
		{
			...
			files: []testFile{
				...
				{"node.key", 0700, []byte{}},
				{"client.root.crt", 0777, append(goodRootCert, []byte("-----BEGIN CERTIFICATE-----abcdef\n-----END CERTIFICATE-----")...)},
			...
			certs: []security.CertInfo{
				...
				{FileUsage: security.ClientPem, Filename: "client.root.crt", Name: "root",
					Error: errors.New("failed to parse certificate file client.root.crt as PEM: block #1 could not be parsed")},
				{FileUsage: security.NodePem, Filename: "node.crt",
					Error: errors.New("failed to parse certificate file node.crt as PEM: block #0 could not be parsed")},
			},
		},
		{
		...
		for i, actual := range cl.Certificates() {
			...
			if actual.Filename != "" {
				...
				if expected.ParsedCertificates != nil {
					...
					for certIndex, _ := range actual.ParsedCertificates {
