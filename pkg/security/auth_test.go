...
func TestGetCertificateUserScope(t *testing.T) {
	...
	if userScopes, err := security.GetCertificateUserScope(makeFakeTLSState(t, "foo")); err != nil {
		...
		userScope, ok := userScopes["foo"]
		require.True(t, ok)
		require.Equal(t, "foo", userScope.Username)
		require.Equal(t, 1, len(userScope.TenantIDs))
		_, ok = userScope.TenantIDs[roachpb.SystemTenantID]
		require.True(t, ok)
	}
	...
	if userScopes, err := security.GetCertificateUserScope(makeFakeTLSState(t, "foo,dns:bar,dns:blah;CA")); err != nil {
		...
		for _, username := range []string{"foo", "bar", "blah"} {
			userScope, ok := userScopes[username]
			require.True(t, ok)
			require.Equal(t, username, userScope.Username)
			require.Equal(t, 1, len(userScope.TenantIDs))
			_, ok = userScope.TenantIDs[roachpb.SystemTenantID]
			require.True(t, ok)
		}
	}
	// Extract all of the principals with tenant scopes from the certificate.
	if userScopes, err := security.GetCertificateUserScope(makeFakeTLSState(t, "foo,dns:bar,dns:blah,uri:crdb://tenant/123/user/foo,uri:crdb://tenant/456/user/foo;CA")); err != nil {
		t.Error(err)
	} else {
		require.Equal(t, 3, len(userScopes))
		for _, username := range []string{"foo", "bar", "blah"} {
			...
			require.True(t, ok)
			require.Equal(t, username, userScope.Username)
			require.Equal(t, 2, len(userScope.TenantIDs))
			_, ok = userScope.TenantIDs[roachpb.MakeTenantID(123)]
			require.True(t, ok)
			_, ok = userScope.TenantIDs[roachpb.MakeTenantID(456)]
			require.True(t, ok)
		}
	}
}
