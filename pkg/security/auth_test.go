...
func TestGetCertificateUserScope(t *testing.T) {
	...
	if userScopes, err := security.GetCertificateUserScope(makeFakeTLSState(t, "foo,dns:bar,dns:blah,uri:crdb://tenant/123,uri:crdb://tenant/456;CA")); err != nil {
		t.Error(err)
	} else {
		require.Equal(t, 3, len(userScopes))
		for _, username := range []string{"foo", "bar", "blah"} {
...
func TestGetCertificateUsersMapped(t *testing.T) {
	...
	for _, c := range testCases {
		t.Run("", func(t *testing.T) {
			...
			var names string
			for username, _ := range userScopes {
				if len(names) == 0 {
					names = username
				} else {
					names += "," + username
