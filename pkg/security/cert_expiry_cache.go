...
func NewClientCertExpirationCache(
	...
	var ptr interface{}
	var ok bool
	if ptr, ok = timeSrc.(*timeutil.DefaultTimeSource); ok {
		c.timeSrcPtr = ptr
	} else if ptr, ok = timeSrc.(*timeutil.ManualTime); ok {
		c.timeSrcPtr = ptr
	}
...
func (c *ClientCertExpirationCache) MaybeUpsert(
	...
	if !ok {
		...
		if err == nil && newExpiry > c.timeNow() {
...
func (c *ClientCertExpirationCache) timeNow() int64 {
	if defaultTime, ok := c.timeSrcPtr.(*timeutil.DefaultTimeSource); ok {
		return defaultTime.Now().Unix()
	}
	if manualTime, ok := c.timeSrcPtr.(*timeutil.ManualTime); ok {
		return manualTime.Now().Unix()
	}
...
// with expiration values in the future.
func (c *ClientCertExpirationCache) PurgePastExpirations() {
	...
	c.mu.cache.Do(func(entry *cache.Entry) {
		gauge := entry.Value.(*aggmetric.Gauge)
		if gauge.Value() <= c.timeNow() || gauge.Value() == 0 {
