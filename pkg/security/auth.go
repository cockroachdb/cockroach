...
func UserAuthCertHook(
	...
	return func(ctx context.Context, systemIdentity username.SQLUsername, clientConnection bool) error {
		...
		if AuthenticateUserScope(certUserScope, systemIdentity.Normalized(), tenantID) {
			return nil
		}
...
// AuthenticateUserScope returns true if the user is a valid user for the tenant based on the certificate
