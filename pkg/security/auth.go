...
func GetCertificateUsers(tlsState *tls.ConnectionState) ([]string, error) {
	...
	users := make([]string, 0, 1+len(peerCert.DNSNames))
	users = append(users, transformPrincipal(tlsState.PeerCertificates[0].Subject.CommonName))
	for _, name := range peerCert.DNSNames {
		users = append(users, transformPrincipal(name))
	}
	return users, nil
}
