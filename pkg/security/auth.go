...
// certificate CN, while databaseUsername reflects any username mappings
...
func UserAuthCertHook(insecureMode bool, tlsState *tls.ConnectionState) (UserAuthHook, error) {
	...
	return func(ctx context.Context, systemIdentity, dbUser SQLUsername, clientConnection bool) (func(), error) {
		// TODO(marc): we may eventually need stricter user syntax rules.
		if systemIdentity.Undefined() || dbUser.Undefined() {
			return nil, errors.New("user is missing")
		}

		if !clientConnection && !dbUser.IsNodeUser() {
			return nil, errors.Errorf("user %s is not allowed", dbUser)
		}
...
func UserAuthPasswordHook(insecureMode bool, password string, hashedPassword []byte) UserAuthHook {
	return func(ctx context.Context, systemIdentity, dbUsername SQLUsername, clientConnection bool) (func(), error) {
		if systemIdentity.Undefined() || dbUsername.Undefined() {
			return nil, errors.New("user is missing")
		}
