...
// evaluateRootOrNodeDN returns distinguished name set for root or node user via
// root-cert-distinguished-name and node-cert-distinguished flags respectively
// if systemIdentity conforms to one of these 2 users. It may also return
// original subject option if it is set for the role. Root and Node roles cannot
// have subject role option set for them.
func evaluateRootOrNodeDN(
	setRoleSubject *ldap.DN, systemIdentity username.SQLUsername,
) (dn *ldap.DN) {
	dn = setRoleSubject
	switch {
...
func UserAuthCertHook(
	...
	return func(ctx context.Context, systemIdentity username.SQLUsername, clientConnection bool) error {
		...
		roleSubject = evaluateRootOrNodeDN(roleSubject, systemIdentity)
