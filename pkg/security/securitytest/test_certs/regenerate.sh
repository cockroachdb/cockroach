...
openssl genrsa -out client.testuser_san_only.key
openssl req -new  -key client.testuser_san_only.key -out client.testuser_san_only.csr -batch -addext "subjectAltName = DNS:testuser"
openssl ca -config ca.cnf -keyfile ca.key -cert ca.crt -policy signing_policy -extensions signing_client_req -out client.testuser_san_only.crt -outdir . -in client.testuser_san_only.csr -batch

# CN only client certs
openssl genrsa -out client.testuser_cn_only.key
openssl req -new  -key client.testuser_cn_only.key -out client.testuser_cn_only.csr -batch -subj /CN=testuser
openssl ca -config ca.cnf -keyfile ca.key -cert ca.crt -policy signing_policy -extensions signing_client_req -out client.testuser_cn_only.crt -outdir . -in client.testuser_cn_only.csr -batch

# CN and san dns: client certs
openssl genrsa -out client.testuser_cn_and_san.key
openssl req -new  -key client.testuser_cn_and_san.key -out client.testuser_cn_and_san.csr -batch -subj /CN=testuser -addext "subjectAltName = DNS:testuser"
openssl ca -config ca.cnf -keyfile ca.key -cert ca.crt -policy signing_policy -extensions signing_client_req -out client.testuser_cn_and_san.crt -outdir . -in client.testuser_cn_and_san.csr -batch
