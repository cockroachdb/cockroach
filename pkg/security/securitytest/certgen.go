...
// periodic manual regeneration. All cert generation uses only Go's standard
// crypto libraries to avoid importing pkg/security (which would create a
// dependency cycle through pkg/sql/protoreflect).
	...
	template := &x509.Certificate{
		...
		Subject: pkix.Name{
			CommonName: fmt.Sprintf(
				"Tenant %d Token Signing Certificate", tenantID),
		},
