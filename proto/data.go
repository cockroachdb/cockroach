...
func (v *Value) GetBytesChecked() ([]byte, error) {
	if v.GetTag() != ValueType_BYTES {
		return nil, fmt.Errorf("value type is not BYTES: %s", v.GetTag())
	}
	return v.Bytes, nil
}
...
func (v *Value) GetFloat() (float64, error) {
	if v.GetTag() != ValueType_FLOAT {
		return 0, fmt.Errorf("value type is not FLOAT: %s", v.GetTag())
	}
	if len(v.Bytes) != 8 {
...
func (v *Value) GetInt() (int64, error) {
	if v.GetTag() != ValueType_INT {
		return 0, fmt.Errorf("value type is not INT: %s", v.GetTag())
	}
	if len(v.Bytes) != 8 {
