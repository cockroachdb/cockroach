...
TEST=tpccbench/nodes=9/cpu=4/chaos/partition
time caffeinate -- ./roachtest.$sha run "${TEST}" --port 8080 --parallelism 50 --debug --count 800 --cpu-quota 1600 --roachprod roachprod.${sha} --workload workload.${sha} --cockroach ./cockroach.$sha --artifacts artifacts.$sha
