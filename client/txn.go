...
func (txn *Txn) send(ba roachpb.BatchRequest) (*roachpb.BatchResponse, *roachpb.Error) {
	...
	if elideEndTxn && pErr == nil {
		...
		// happen since happen since the txn deadline is normally updated
		// when it is about to expire or expired. We will just keep the code for safety.
		// (see TestReacquireLeaseOnRestart).
