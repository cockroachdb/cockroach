...
func setupClientBenchData(useSSL bool, numVersions, numKeys int, b *testing.B) (
	*server.TestServer, *client.DB) {
	...
	for t := 1; t <= numVersions; t++ {
		...
		for i := 0; i < numKeys; i++ {
			if t == 1 {
				keys[i] = roachpb.Key(encoding.EncodeUvarint([]byte("key-"), uint64(i), encoding.Ascending))
...
func runClientScan(useSSL bool, numRows, numVersions int, b *testing.B) {
	...
	b.RunParallel(func(pb *testing.PB) {
		...
		for pb.Next() {
			...
			startKey := roachpb.Key(encoding.EncodeUvarint(
				startKeyBuf, uint64(keyIdx), encoding.Ascending))
			endKey := roachpb.Key(encoding.EncodeUvarint(
				endKeyBuf, uint64(keyIdx)+uint64(numRows), encoding.Ascending))
