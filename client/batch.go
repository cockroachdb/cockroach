	...
	for i := range b.Results {
		...
		for k := 0; k < result.calls; k++ {
			...
			case *roachpb.AdminSetFrozenRequest:
				...
				row.Key = []byte(req.Key)
				if result.PErr == nil {
					t := reply.(*roachpb.AdminSetFrozenResponse)
					row.Value = &roachpb.Value{}
					row.Value.SetInt(int64(t.Affected))
				}
...
// AdminSetFrozen .
func (b *Batch) AdminSetFrozen(s, e interface{}, frozen bool) {
	begin, err := marshalKey(s)
	if err != nil {
		b.initResult(0, 0, err)
		return
	...
	b.reqs = append(b.reqs,
		roachpb.NewAdminSetFrozen(roachpb.Key(begin), roachpb.Key(end), frozen))
	b.initResult(1, 1, nil)
}
