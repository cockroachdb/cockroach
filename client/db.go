...
// StartKey.
func (db *DB) ChangeFrozen(begin, end interface{}, frozen bool) (int64, roachpb.RKey, *roachpb.Error) {
	b := db.NewBatch()
	b.ChangeFrozen(begin, end, frozen)
	r, pErr := runOneResult(db, b)
	if pErr != nil {
		return 0, nil, pErr
	}
	...
	return resp.Affected, resp.MinStartKey, nil
}
