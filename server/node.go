...
const (
	...
	maybeNotifyTableLeaderInterval = 50 * time.Millisecond
)
...
// startTableLeaderElections loops on a periodic ticker to attempt to
...
func (n *Node) periodicallyCheckTableLeaders(stopper *stop.Stopper) {
	stopper.RunWorker(func() {
		ticker := time.NewTicker(maybeNotifyTableLeaderInterval)
		defer ticker.Stop()
		for {
			select {
			case <-ticker.C:
				if n.maybeNotifyTableLeaders(stopper) {
...
// maybeNotifyTableLeaders checks to see if any of the tables have
// changed node leadership, and sends notifications for all leader
// changes.
func (n *Node) maybeNotifyTableLeaders(stopper *stop.Stopper) bool {
	n.mu.Lock()
	defer n.mu.Unlock()
	for i, ks := range n.notifyKeys {
		if ok := n.hasTableLeaderChanged(ks); ok {
			ks.isLeader = !ks.isLeader
			n.notifyKeys[i] = ks
			notifyTableLeader(stopper, ks)
		}
	}
...
func (n *Node) hasTableLeaderChanged(ks keyState) bool {
	var isLeader bool
	if err := n.lSender.VisitStores(func(s *storage.Store) error {
...
func notifyTableLeader(s *stop.Stopper, ks keyState) {
	s.RunAsyncTask(func() {
		if log.V(2) {
			log.Infof("notified node of leader(%v) for key: %v", ks.isLeader, ks.key)
		}
		ks.report <- sql.TableLeaderNotification{Key: ks.key, IsLeader: ks.isLeader}
	})
}
...
// RegisterNotifyTableLeader registers a key for table leader notifications. A node is a
// tabe leader if the node is a replica leader for the range containing the prefix key for
// the table.
