...
// jsonpbUnmarshal unmarshals data using protoutil.JSONPb.
func jsonpbUnmarshal(data []byte, v interface{}) error {
...
func TestNodeStatusResponse(t *testing.T) {
	...
	if err := jsonpbUnmarshal(body, &wrapper); err != nil {
		t.Fatal(err)
	}
	...
	for _, oldNodeStatus := range nodeStatuses {
		...
		if err := jsonpbUnmarshal(requestBody, &nodeStatus); err != nil {
			t.Fatal(err)
		}
...
func TestRangesResponse(t *testing.T) {
	...
	if err := jsonpbUnmarshal(body, &response); err != nil {
		t.Fatal(err)
	}
