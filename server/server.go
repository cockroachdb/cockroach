...
func (s *Server) Start() error {
	...
	if err := s.stopper.RunAsyncTask(func() {
		...
		log.Errorf("failed to invoke upgrade of system tables: %s", err)
	}
	return nil
...
func upgradeSystemTablesSchema(exec *sql.Executor) {
	// System tables should be created with the highest privileges that a user
	...
	for _, table := range sqlbase.NewSystemTablesSchema {
		...
		if err := checkQueryResults(r.ResultList, 1); err != nil {
			log.Warningf("err: %s, Unable to create system table: %s", err, table)
		}
	}

	// Run any schema change commands on system tables.
}
