...
func NewServer(srvCtx Context, stopper *stop.Stopper) (*Server, error) {
	...
	// Add a dynamic log tag value for the node ID (which we won't know until we
	// Start).
...
// The passed context can be used to trace the server startup. It should be
// based off context.Background (not s.Ctx()).
