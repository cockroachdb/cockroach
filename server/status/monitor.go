...
type NodeStatusMonitor struct {
	...
	sync.RWMutex // Guards the store monitor collection and node descriptor.
...
func (nsm *NodeStatusMonitor) OnReplicationStatus(event *storage.ReplicationStatusEvent) {
	...
	ssm.Lock()
	defer ssm.Unlock()
	ssm.leaderRangeCount.Update(int64(event.LeaderRangeCount))
	ssm.replicatedRangeCount.Update(int64(event.ReplicatedRangeCount))
	ssm.availableRangeCount.Update(int64(event.AvailableRangeCount))
}
