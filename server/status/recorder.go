...
func (mr *MetricsRecorder) GetStatusSummary() *NodeStatus {
	...
	nodeStat := &NodeStatus{
		...
		Metrics:       make([]*MetricValue, 0, mr.mu.lastNodeMetricCount),
	}

	eachRecordableValue(mr.nodeRegistry, func(name string, val float64) {
		nodeStat.Metrics = append(nodeStat.Metrics, &MetricValue{
			Name:  name,
			Value: val,
		})
	})
	...
	for storeID, r := range mr.mu.storeRegistries {
		storeMetrics := make([]*MetricValue, 0, mr.mu.lastStoreMetricCount)
		eachRecordableValue(r, func(name string, val float64) {
			storeMetrics = append(storeMetrics, &MetricValue{
				Name:  name,
				Value: val,
			})
		})
