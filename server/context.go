...
func GetTotalMemory() (int64, error) {
	...
	if mem.Total > math.MaxInt64 {
		return 0, fmt.Errorf("inferred memory size %s exceeds maximum support memory size %s",
	...
	if runtime.GOOS == "linux" {
		...
		if cgAvlMem, err = strconv.ParseUint(strings.TrimSpace(string(buf)), 10, 64); err != nil {
			if log.V(1) {
				log.Infof("can't parse available memory from cgroups (%s), using system memory %s instaed", err,
					util.IBytes(totalMem))
			}
			return totalMem, nil
		}
