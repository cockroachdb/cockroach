...
func setOpenFileLimit(physicalStoreCount int) (int, error) {
	...
	if rLimit.Max < minimumOpenFileLimit {
		return 0, fmt.Errorf("max limit for open file descriptors is too low (%d), minimum required %d\n%s",
			rLimit.Max,
			minimumOpenFileLimit,
			productionSettingsWebpage)
	}

	// The current open file descriptor limit is higher than the recommended
	...
	if rLimit.Cur < newCurrent {
		if log.V(1) {
			log.Infof("setting the current limit for open file descriptors (%d) to %d as it greatly increase performance",
		...
		if log.V(1) {
			log.Infof("the current limit for open file descriptors is now set to (%d)",
				rLimit.Cur)
		}
	}
	...
	if rLimit.Cur < minimumOpenFileLimit {
		return 0, fmt.Errorf("current limit for open file descriptors is too low (%d) and cannot be increased, minimum required %d\n%s",
			rLimit.Cur,
			minimumOpenFileLimit,
			productionSettingsWebpage)
	}
	...
	// warning.
	log.Warningf("the open file descriptors limit of %d is lower than the recommended amount of %d, this may decrease performance\n%s",
		rLimit.Cur,
