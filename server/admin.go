...
func (s *adminServer) TableDetails(ctx context.Context, req *serverpb.TableDetailsRequest) (
	*serverpb.TableDetailsResponse, error) {
	...
	// Query the zone configuration for this table.
	{
		...
		id, zone, err := s.queryZonePath(ctx, session, path)
		if err != nil {
			return nil, s.serverError(err)
		}

		if zone == nil {
			defaultConfig := config.DefaultZoneConfig()
			zone = &defaultConfig
		}
...
// TODO(mrtracy): The following methods, used to look up the zone configuration for a
// database or table, use the same algorithm as a set of methods in cli/zone.go for the same purpose.
// However, as that code connects to the server with a SQL connections, while this code uses a
// sql.Executor directly, the code cannot be commonized.
//
// Github issue #4869 is the most likely candidate for addressing this incompatibility; when that issue
// has been resolved, this code from cli/zone.go should be moved to a common location and shared
// with this system.

// queryZone retrieves the specifics ZoneConfig associated with the supplied ID, if it exists.
func (s *adminServer) queryZone(ctx context.Context, session *sql.Session, id sqlbase.ID) (
...
// queryZonePath queries a path of sql object IDs, as generated by queryDescriptorIDPath(),
// for a ZoneConfig. It returns the most specific ZoneConfig specified for the object IDs in the path.
func (s *adminServer) queryZonePath(ctx context.Context, session *sql.Session, path []sqlbase.ID) (
	sqlbase.ID, *config.ZoneConfig, error) {
	for i := len(path) - 1; i >= 0; i-- {
		zone, err := s.queryZone(ctx, session, path[i])
		if err != nil || zone != nil {
...
// queryNamespaceID queries for the ID of the namespace with the given name and parent ID.
func (s *adminServer) queryNamespaceID(
	...
	result := r.ResultList[0]
	if len(result.Rows) == 0 {
		return 0, fmt.Errorf("namespace %s with ParentID %d not found", name, parentID)
	}
...
// queryDescriptorIDPath converts a path of namespaces into a path of namespace IDs. For example,
// if this function is called with a database/table name pair, it will return a list of IDs consisting
// of the root namespace ID, the databases ID, and the table ID (in that order).
func (s *adminServer) queryDescriptorIDPath(ctx context.Context, session *sql.Session, names []string) (
	[]sqlbase.ID, error) {
