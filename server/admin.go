...
func (s *adminServer) SetUIData(ctx context.Context, req *SetUIDataRequest) (*SetUIDataResponse, error) {
	...
	for key, val := range req.KeyValues {
		...
		if alreadyExists {
			...
			_ = qargs.SetValue(`1`, parser.NewDString(string(val)))
			_ = qargs.SetValue(`2`, parser.NewDString(key))
			...
			qargs := parser.MakePlaceholderInfo()
			_ = qargs.SetValue(`1`, parser.NewDString(key))
			_ = qargs.SetValue(`2`, parser.NewDBytes(parser.DBytes(val)))
			r := s.server.sqlExecutor.ExecuteStatements(ctx, session, query, qargs)
			if err := s.checkQueryResults(r.ResultList, 2); err != nil {
				return nil, s.serverError(err)
			}
...
func makeSQLQuery() *sqlQuery {
	return &sqlQuery{qargs: parser.MakePlaceholderInfo()}
}
...
func (q *sqlQuery) Append(s string, params ...parser.Datum) {
	...
	for i := 0; i < placeholders; i++ {
		_ = q.qargs.SetValue(fmt.Sprint(firstpidx+i+1), params[i])
	}
}
