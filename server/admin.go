...
// unfrozen Replicas (or an error or timeout occurs). The map will be mutated.
func (s *adminServer) waitForStoreFrozen(
	required map[roachpb.StoreID]struct{},
) error {
	mu := struct {
		sync.Mutex
	...
	for r := retry.Start(opts); r.Next(); {
		...
		for _, storeDesc := range storeList.Stores {
			...
			if _, ok := required[storeID]; !ok {
				// We don't care about this Store. It may have launched so
				// recently that it doesn't hold any data.
				continue
			}
		...
		numWaiting = 0
		for storeID, ok := range mu.oks {
			if ok {
				// Store has reported that it is frozen.
				delete(required, storeID)
				continue
			}
			numWaiting++
		}
	...
	default:
		...
		} else if remReq := len(required); remReq > 0 {
			err = fmt.Errorf("unable to reach %d stores", remReq)
		}
	}
	return err
}
...
func (s *adminServer) ClusterFreeze(
	...
	touchedStores := make(map[roachpb.StoreID]struct{})
	process := func(from, to roachpb.Key) (roachpb.Key, error) {
		...
		for _, storeID := range fr.Stores {
			touchedStores[storeID] = struct{}{}
		}
	...
	var err error
	if req.Freeze {
		err = s.waitForStoreFrozen(touchedStores)
	}
	return &resp, err
}
