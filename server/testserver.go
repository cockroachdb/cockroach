...
func StartTestServerWithContext(t util.Tester, ctx *Context, nodes int) TestServer {
	s := TestServer{Ctx: ctx}
	var err error
	err = s.StartWithStopper(nil, startParams{nodes: nodes})
	if err != nil {
		if t != nil {
...
func StartTestServer(t util.Tester) TestServer {
	return StartTestServerWithContext(t, nil, 1)
}
...
func (ts *TestServer) StartWithStopper(stopper *stop.Stopper, startParams startParams) error {
	...
	for i := range cfg.ReplicaAttrs {
		cfg.ReplicaAttrs[i] = roachpb.Attributes{}
	}
