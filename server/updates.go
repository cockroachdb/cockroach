...
// These are only vars to allow testing.
var updateCheckFrequency = time.Hour * 24
var updateCheckJitterSeconds = 120
var updateCheckRetryFrequency = time.Hour
...
// These are usually set by `init` but are nil'ed or reset in tests.
var parsedUpdatesURL *url.URL
var parsedReportingURL *url.URL
...
func init() {
	u, err := url.Parse(baseUpdatesURL)
	if err != nil {
		// Should never happen (unless we break the URL), so fail fast.
		panic(err)
	}
	parsedUpdatesURL = u
}
...
func init() {
	u, err := url.Parse(baseReportingURL)
	if err != nil {
		// Should never happen (unless we break the URL), so fail fast.
		panic(err)
	}
	parsedReportingURL = u
...
func (s *Server) checkForUpdates() {
	...
	if parsedUpdatesURL == nil {
		return
	}

	q := parsedUpdatesURL.Query()
	q.Set("version", util.GetBuildInfo().Tag)
	q.Set("uuid", s.node.ClusterID.String())
	parsedUpdatesURL.RawQuery = q.Encode()

	res, err := http.Get(parsedUpdatesURL.String())
	if err != nil {
...
func (s *Server) reportUsage() {
	...
	q := parsedReportingURL.Query()
	q.Set("version", util.GetBuildInfo().Tag)
	q.Set("uuid", s.node.ClusterID.String())
	parsedReportingURL.RawQuery = q.Encode()

	_, err := http.Post(parsedReportingURL.String(), "application/json", b)
	if err != nil && log.V(2) {
		// This is probably going to be relatively common in production
		// environments where network access is usually curtailed.
