...
func (s *statusServer) RegisterGRPCGateway(serverCtx *Context) error {
	...
	if err := RegisterStatusHandlerFromEndpoint(s.gwCtx, s.gwMux, serverCtx.Addr, opts); err != nil {
	...
	s.router.NotFound = slashStripper{server: s}
	return nil
}

// slashStripper is a HTTP middleware that removes trailing slashes and
// duplicates. This makes incoming requests to `foo/` go to the `foo` grpc
// handler instead of `foo/{id}` and maintains previous compatibility with `foo//{id}`.
type slashStripper struct {
	server *statusServer
}

func (s slashStripper) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	r.URL.Path = strings.TrimRight(strings.Replace(r.URL.Path, "//", "/", -1), "/")
	s.server.gwMux.ServeHTTP(w, r)
}
...
func (s *statusServer) handleDetailsLocal() *DetailsResponse {
...
func (s *statusServer) Details(ctx context.Context, req *DetailsRequest) (*DetailsResponse, error) {
	...
	if local {
		return s.handleDetailsLocal(), nil
	}
	status, err := s.dialNode(nodeID)
	if err != nil {
		return nil, err
...
func (s *statusServer) Nodes(_ context.Context, req *NodesRequest) (*NodesResponse, error) {
	...
	for _, row := range rows {
		...
		resp.D = append(resp.D, nodeStatus)
	}
	return resp, nil
}
