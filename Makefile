...
#
# XXX: Ideally we'd scope the dependency on $(UI_PROTOS*) to the protos DLLs,
# but Make v3.81 has a bug that causes the dependency to be ignored [1]. We're
# stuck with this workaround until Apple decides to update the version of Make
# they ship with macOS or we require a newer version of Make. Such a requirement
...
pkg/ui/distoss/bindata.go: $(UI_OSS_DLLS) $(UI_OSS_MANIFESTS) $(UI_JS_OSS) $(shell find pkg/ui/src -type f)
pkg/ui/dist%/bindata.go: pkg/ui/webpack.app.js $(shell find pkg/ui/styl -type f)
...
ui-watch ui-watch-secure: $(UI_DLLS) pkg/ui/yarn.opt.installed
	cd pkg/ui && $(WEBPACK_DASHBOARD) -- $(WEBPACK_DEV_SERVER) --config webpack.ccl.js --port $(PORT) $(WEBPACK_DEV_SERVER_FLAGS)
