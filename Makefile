...
pkg/sql/lex/keywords.go: pkg/sql/parser/sql.y pkg/sql/lex/allkeywords/allkeywords.go | bin/.bootstrap
	go run -tags all-keywords pkg/sql/lex/allkeywords/allkeywords.go < $< > $@.tmp || rm $@.tmp
	mv -f $@.tmp $@
	gofmt -s -w $@
