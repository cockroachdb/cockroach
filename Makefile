...
logic-test-selector := $(if $(TESTCONFIG),$(subst _,,$(subst -,,$(subst @,,$(subst .,,$(subst /,,$(TESTCONFIG)))))),.*)_$(if $(FILES),$(subst _,,$(subst -,,$(subst @,,$(subst .,,$(subst /,,$(FILES)))))),.*)/$(SUBTESTS)
...
testbaselogic testccllogic testoptlogic testsqlitelogic testsqliteccllogic: TESTFLAGS := -test.v $(if $(FILES),-show-sql) $(if $(TESTCONFIG),-config $(TESTCONFIG))
testbaselogic testccllogic testoptlogic testsqlitelogic testsqliteccllogic:
	$(xgo) test $($(<F)-package)/tests/... -test.run "$(TESTS)" -test.timeout $(TESTTIMEOUT) $(TESTFLAGS)
...
sqlitelogictest-package = ./pkg/sql/sqlitelogictest
logictestccl-package = ./pkg/ccl/logictestccl
sqlitelogictestccl-package = ./pkg/ccl/sqlitelogictestccl
