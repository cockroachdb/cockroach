...
OPTGEN_TARGETS = \
	...
	pkg/sql/opt/xform/factory.og.go \
	pkg/sql/opt/xform/rule_name.og.go
...
ifndef XHOST_TRIPLE
	 @$(COCKROACH) gen settings-list --format=html > docs/generated/settings/settings.html.tmp
	 @mv -f docs/generated/settings/settings.html{.tmp,}
endif
...
generate: protobuf $(DOCGEN_TARGETS) $(OPTGEN_TARGETS) $(SQLPARSER_TARGETS)
...
optgen-rules := pkg/sql/opt/xform/rules/*.opt
...
pkg/sql/opt/xform/factory.og.go: $(optgen-defs) $(optgen-rules) bin/optgen
	optgen -out $@ factory $(optgen-defs) $(optgen-rules)

pkg/sql/opt/xform/rule_name.og.go: $(optgen-defs) $(optgen-rules) bin/optgen
	optgen -out $(@D)/rule_name.og.go rulenames $(optgen-defs) $(optgen-rules)
