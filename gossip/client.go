...
func newClient(addr net.Addr, globalMetrics gossipMetrics) *client {
	return &client{
...
func (c *client) sendGossip(g *Gossip, addr util.UnresolvedAddr, stream Gossip_GossipClient) error {
	...
	if delta := g.is.delta(c.remoteHighWaterStamps); len(delta) > 0 {
		...
		c.globalMetrics.infosSent.Add(infosSent)
		c.globalMetrics.bytesSent.Add(bytesSent)
...
func (c *client) handleResponse(g *Gossip, reply *Response) error {
	...
	c.globalMetrics.bytesReceived.Add(bytesReceived)
	c.metrics.infosReceived.Add(infosReceived)
	c.globalMetrics.infosReceived.Add(infosReceived)
