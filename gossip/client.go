...
func (c *client) start(g *Gossip, disconnected chan *client, rpcCtx *rpc.Context, stopper *stop.Stopper) {
	g.mu.Lock()
	defer g.mu.Unlock()
	c.startLocked(g, disconnected, rpcCtx, stopper)
}

// startLocked requires that the gossip server lock is held.
func (c *client) startLocked(g *Gossip, disconnected chan *client, rpcCtx *rpc.Context, stopper *stop.Stopper) {
	ctx := context.TODO()
