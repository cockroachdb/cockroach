...
func newClient(addr net.Addr, nodeMetrics metrics, breaker *circuit.Breaker) *client {
	return &client{
...
func (c *client) start(g *Gossip, disconnected chan *client, rpcCtx *rpc.Context, stopper *stop.Stopper) {
	...
	stopper.RunWorker(func() {
		...
		consecFailures := c.breaker.ConsecFailures()
		var stream Gossip_GossipClient
		if err := c.breaker.Call(func() error {
