...
type Persistence interface {
	// Read fetches the bootstrap data from the persistent store into
	// the provided bootstrap protobuf. Returns nil or an error on failure.
	Read(*BootstrapInfo) error
	// Write stores the provided bootstrap data to the persistent
	// store. Returns nil or an error on failure.
	Write(*BootstrapInfo) error
}
...
func (g *Gossip) SetPersistence(persist Persistence) error {
	...
	if len(storedBI.Addresses) == 0 {
		...
		if numAddrs := len(g.bootstrapInfo.Addresses); numAddrs > 0 {
			if err := g.persist.Write(&g.bootstrapInfo); err != nil {
				log.Error(err)
			}
