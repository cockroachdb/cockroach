			...
			if g.server.stopper.RunTask(func() {
				...
				if !haveClients || !haveSentinel {
					...
					if addr := g.getNextBootstrapAddress(); addr != nil {
						g.startClient(ctx, addr, g.mu.is.NodeID)
					} else {
...
func (g *Gossip) tightenNetwork(distantNodeID roachpb.NodeID) {
	...
	if g.outgoing.hasSpace() {
		if nodeAddr, err := g.getNodeIDAddressLocked(distantNodeID); err != nil {
			...
			g.startClient(g.ctx, nodeAddr, g.mu.is.NodeID)
		}
	}
}
...
func (g *Gossip) doDisconnected(c *client) {
	...
	if c.forwardAddr != nil {
		g.startClient(g.ctx, c.forwardAddr, g.mu.is.NodeID)
	}
	g.maybeSignalStatusChangeLocked()
}
...
// a goroutine.
func (g *Gossip) startClient(ctx context.Context, addr net.Addr, nodeID roachpb.NodeID) {
	g.clientsMu.Lock()
	defer g.clientsMu.Unlock()
	...
	c := newClient(ctx, addr, g.serverMetrics)
