...
type server struct {
	...
	nodeMap  map[util.UnresolvedAddr]serverInfo // Incoming client's local address -> serverConn
...
func (s *server) status() string {
	...
	for addr, info := range s.nodeMap {
		...
		fmt.Fprintf(&buf, "  %d: %s (%s)\n",
			info.peerID, addr.AddressField, roundSecs(time.Since(info.createdAt)))
	}
	return buf.String()
}
