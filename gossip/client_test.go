...
func startFakeServerGossips(t *testing.T) (local *Gossip, remote *fakeGossipServer, stopper *stop.Stopper) {
	...
	local = New(lRPCContext, nil, stopper, nil)
...
func TestClientGossip(t *testing.T) {
	...
	client := newClient(&remote.is.NodeAddr, newGossipMetrics())
...
func TestClientGossipMetrics(t *testing.T) {
	...
	disconnected <- newClient(&local.is.NodeAddr, newGossipMetrics())
	disconnected <- newClient(&remote.is.NodeAddr, newGossipMetrics())
	...
	util.SucceedsSoon(t, func() error {
		...
		for i, reg := range []*metric.Registry{localRegistry, remoteRegistry} {
			for _, counterName := range []string{
				ConnectionsIncomingCounterName,
				ConnectionsOutgoingCounterName,
			} {
				counter := reg.GetCounter(counterName)
				if counter == nil {
					return errors.Errorf("%d. missing counter %q", i, counterName)
				}
				stats[counterName] += counter.Count()
			}
		}
...
func TestClientNodeID(t *testing.T) {
	...
	// Use an insecure context. We're talking to tcp socket which are not in the certs.
	rpcContext := rpc.NewContext(&base.Context{Insecure: true}, nil, stopper)
	c := newClient(&remote.nodeAddr, newGossipMetrics())
	disconnected := make(chan *client, 1)
	disconnected <- c
...
func TestClientRegisterWithInitNodeID(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		gnode := New(RPCContext, resolvers, stopper, nil)
...
func TestClientForwardUnresolved(t *testing.T) {
	...
	client := newClient(&addr, newGossipMetrics()) // never started
