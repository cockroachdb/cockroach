...
512MB if the memory size cannot be determined.`),
	...
	"database_host": wrapText(`
Database server host to connect to.`),

	"database_port": wrapText(`
Database server port to connect to.`),

	"database_http_port": wrapText(`
...
func initFlags(ctx *Context) {
...

	{
		...
		// Server flags.
		f.StringVar(&connHost, "host", envutil.EnvOrDefaultString("server_host", ""), usage("server_host"))
		f.StringVarP(&connPort, "port", "p", envutil.EnvOrDefaultString("server_port", base.DefaultPort), usage("server_port"))
		f.StringVar(&httpPort, "http-port", envutil.EnvOrDefaultString("server_http_port", base.DefaultHTTPPort), usage("server_http_port"))
		f.StringVar(&ctx.Attrs, "attrs", envutil.EnvOrDefaultString("attrs", ctx.Attrs), usage("attrs"))
		...
		f.StringVar(&ctx.JoinUsing, "join", envutil.EnvOrDefaultString("join", ctx.JoinUsing), usage("join"))

		// Engine flags.
		setDefaultCacheSize(&ctx.Context)
		ctx.CacheSize = envutil.EnvOrDefaultBytes("cache", ctx.CacheSize)
	...
	for _, cmd := range certCmds {
		...
		// Certificate flags.
		f.StringVar(&ctx.SSLCA, "ca-cert", envutil.EnvOrDefaultString("ca_cert", ctx.SSLCA), usage("ca_cert"))
		f.StringVar(&ctx.SSLCAKey, "ca-key", envutil.EnvOrDefaultString("ca_key", ctx.SSLCAKey), usage("ca_key"))
		f.StringVar(&ctx.SSLCert, "cert", envutil.EnvOrDefaultString("cert", ctx.SSLCert), usage("cert"))
		f.StringVar(&ctx.SSLCertKey, "key", envutil.EnvOrDefaultString("key", ctx.SSLCertKey), usage("key"))
		f.IntVar(&keySize, "key-size", envutil.EnvOrDefaultInt("key_size", defaultKeySize), usage("key_size"))
	}
	...
	for _, cmd := range clientCmds {
		...
		f.BoolVar(&ctx.Insecure, "insecure", envutil.EnvOrDefaultBool("insecure", ctx.Insecure), usage("insecure"))
		f.StringVar(&connHost, "host", envutil.EnvOrDefaultString("database_host", ""), usage("database_host"))
	...
	for _, cmd := range simpleCmds {
		f := cmd.PersistentFlags()
		f.StringVarP(&connPort, "port", "p", envutil.EnvOrDefaultString("database_port", base.DefaultPort), usage("database_port"))
	}
	...
	for _, cmd := range httpCmds {
		f := cmd.PersistentFlags()
		f.StringVar(&httpPort, "http-port", envutil.EnvOrDefaultString("database_http_port", base.DefaultHTTPPort), usage("database_http_port"))
	}
	...
	for _, cmd := range []*cobra.Command{scanCmd, reverseScanCmd, lsRangesCmd} {
		f := cmd.Flags()
		f.Int64Var(&maxResults, "max-results", envutil.EnvOrDefaultInt64("max_results", 1000), usage("max_results"))
	}
}
