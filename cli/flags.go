...
`,
	...
	cliflags.BackgroundName: wrapText(` Start the server in the
background. This is similar to appending "&" to the command line, but
when the server is started with --background control is not returned
to the shell until the server is ready to accept requests.`),
