...
`,
	...
	"host": wrapText(`
Database server host. The node will advertise itself using this hostname; it
must resolve from other nodes in the cluster.`),
...
func initFlags(ctx *Context) {
...

	{
		...
		// Server flags.
		f.StringVarP(&connHost, "host", "h", "", usage("host"))
		f.StringVarP(&connPort, "port", "p", base.DefaultPort, usage("port"))
	...
	for _, cmd := range clientCmds {
		...
		f.BoolVar(&ctx.Insecure, "insecure", ctx.Insecure, usage("insecure"))
		f.StringVar(&ctx.Certs, "certs", ctx.Certs, usage("certs"))
		f.StringVarP(&connHost, "host", "h", "", usage("client_host"))
	}
