...
func initFlags(ctx *Context) {
	...
	simpleCmds := []*cobra.Command{kvCmd, nodeCmd, rangeCmd, exterminateCmd}
	for _, cmd := range simpleCmds {
		f := cmd.PersistentFlags()
		f.StringVarP(&connPort, "port", "p", base.DefaultPort, usage("client_port"))
	}

	quitCmd.PersistentFlags().StringVar(&httpPort, "http-port", base.DefaultHTTPPort, usage("http_port"))
