...
func runInteractive(conn *sqlConn) (exitErr error) {
	...
	for isFinished := false; !isFinished; {
		...
		if hasSet {
			// TODO(mjibson): since the syntax is only updated once the commands are run,
			// it is possible that multiple statements are specified in a single line
			// of text. If the first statement changes the syntax mode and subsequent
			// statements on the same line could cause problems because the lexer is
			// still in the previous mode.
