...
func runInteractive(db *sql.DB, dbURL string) (exitErr error) {
	...
	for {
		...
		if err != nil {
			// Linenoise only returns err != nil, saying "prompt was quited
			// with a killsignal", when the input was terminated with Ctrl+C
			// or Ctrl+D. This is not an error, and merely indicates end of
			// input.  io.EOF is never returned.
			break
		}
		...
		if keepHistory {
			...
			if histFile != "" {
				...
				// current shell.
				err = linenoise.SaveHistory(histFile)
				if err != nil {
					log.Warning("cannot save command-line history: %s", err)
