...
func runInteractive(db *sql.DB, dbURL string) (exitErr error) {
	...
	for {
		...
		if len(stmt) == 0 && tl == "help" {
			// We only recognize "help" if at the start of a multi-line
			// statement, because "select", newline, "help", newline, "from
			// ..." is valid SQL.
			handleShortCommand(`\?`)
			continue
		} else {
			r := handleShortCommand(tl)
			if r == shortCmdContinue {
				continue
			} else if r == shortCmdExit {
				break
			}
		}
