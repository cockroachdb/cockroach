...
func dumpTable(w io.Writer, conn *sqlConn, origDBName, origTableName string) error {
	...
	{
		rows, err := conn.Query("SELECT cluster_logical_timestamp()::int", nil)
		if err != nil {
			return err
		}
	...
	var desc sqlbase.Descriptor
	{
		...
		// rows.Close must be after proto.Unmarshal because b is mutated during Close.
		if err := rows.Close(); err != nil {
			return err
		}
	}
