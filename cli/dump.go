...
func dumpTable(w io.Writer, conn *sqlConn, origDBName, origTableName string) error {
	...
	table := desc.GetTable()
	if table == nil {
		return fmt.Errorf("expected table descriptor")
	}
	...
	for {
		...
		fmt.Fprintln(os.Stderr, "NPK", nextPK)
		pk := make([]driver.Value, len(nextPK))
		// TODO(mjibson): verify that the npk copy indeed needs to happen before rows.Close, not just copying []byte
		for si, sv := range nextPK {
			b, ok := sv.([]byte)
			if !ok {
				pk[si] = sv
				continue
			...
			if strings.HasPrefix(coltypes[pkcols[si]], "STRING") {
				...
				// Byte slices need to be copied because rows.Close() below mutates the
				// underlying data.
				pk[si] = append([]byte{}, b...)
			}
		}
