...
func (c *sqlConn) Query(query string, args []driver.Value) (*sqlRows, error) {
	...
	return &sqlRows{Rows: rows, conn: c}, err
}
...
func runQueryWithFormat(db *sqlConn, format fmtMap, query string, parameters ...driver.Value) (
	...
	// set of types when being passed to driver.Queryer.Query. We use
	// driver.DefaultParameterConverter to perform the necessary conversion. This
	// is usually taken care of by the sql package, but we have to do so manually
	// because we're talking directly to the driver.
