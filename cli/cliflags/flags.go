...
type FlagInfo struct {
	...
	// Description of the flag. This text will be automatically re-wrapped.
	//
	// The wrapping can be stopped by embedding the tag "<STOPWRAP>". This tag is
	// removed from the text text but it signals that everything that follows
	// should not be re-wrapped. To start wrapping again, use "<STARTWRAP>".
	Description string
}
...
each storage device, for example:<STOPWRAP>
...
strings separated by colons, for example: :<STOPWRAP>
...
memory that the store may consume, for example:<STOPWRAP>
