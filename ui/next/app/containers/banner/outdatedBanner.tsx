...
class OutdatedBanner extends React.Component<OutdatedBannerProps, OutdatedBannerState> {
  ...
  bannerText() {
    if (!this.props.versionsMatch) {
      return "The node versions are mismatched.";
  ...
  visible() {
    ...
    return !this.state.dismissed &&
      ...
      (this.props.versionCheckDismissedFetched && !this.props.versionCheckDismissed || (Date.now() - this.props.versionCheckDismissed > DAY)) &&
...
// TODO: change the way uiData stores data so we don't need an added check to see if the value was fetched
// TODO: determine whether they dismissed a node version mismatch or an upstream version check
let versionCheckDismissedFetched = (state: any): boolean => state && state.uiData && state.uiData.data && _.has(state.uiData.data, VERSION_DISMISSED_KEY);
let versionCheckDismissed = (state: any): number => state && state.uiData && state.uiData.data && state.uiData.data[VERSION_DISMISSED_KEY];

let versions = createSelector(
  nodeStatuses,
  (statuses: NodeStatus[]): string[] => statuses && _.keys(_.countBy<NodeStatus>(statuses, (s: NodeStatus) => s.build_info.tag))
);
