...
class ClusterMain extends React.Component<ClusterMainProps, {}> {
  ...
  render() {
    let { totalNodes, bytesUsed, availableCapacity } = this.props;
...
let totalNodes = createSelector(
  nodeStatuses,
  (ns) => (ns && ns.length) || 0
);

let totalAvailableCapacity = createSelector(
  nodeStatuses,
  (nss) => _.sumBy(nss, (ns) => ns.metrics.get(MetricConstants.availableCapacity))
);

let totalBytesUsed = createSelector(
  nodeStatuses,
  (nss) => _.sumBy(nss, BytesUsed)
);

let clusterMainConnected = connect(
  (state, ownProps) => {
    return {
      totalNodes: totalNodes(state),
      availableCapacity: totalAvailableCapacity(state),
      bytesUsed: totalBytesUsed(state),
    };
  },
  {
