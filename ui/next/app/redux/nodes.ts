  ...
  return (dispatch: Dispatch, getState: () => any): void => {
    ...
    fetch("/_status/nodes/").then((response) => {
      ...
      let result = _.map(jsonResult, (nsObj: any) => {
        // HACK: Proto2TypeScript doesn't quite generate the right code, so
        // force it find the constructor we know exists.
        let ns = new (protos as any).cockroach.server.status.NodeStatus(nsObj);
