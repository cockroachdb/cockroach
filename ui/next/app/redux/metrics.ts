...
function metricsQueryReducer(state: MetricsQuery, action: Action) {
  switch (action.type) {
    ...
    case RECEIVE:
      let { payload: response } = action as PayloadAction<WithID<TSResponse>>;
      state = _.clone(state);
 ...
 */
export function receiveMetrics(id: string, response: TSResponse): PayloadAction<WithID<TSResponse>> {
  return {
    type: RECEIVE,
    payload: {
      id: id,
 ...
 */
export function queryMetrics(id: string, query: TSRequest) {
      ...
      setTimeout(() => {
        ...
        let promises = _.map(batches, (batch) => {
          ...
          return fetch("/ts/query", {
            method: "POST",
            body: new protos.cockroach.ts.TimeSeriesQueryRequest(unifiedRequest).encodeJSON(),
          }).then((response) => {
...
function timespanKey(query: TSRequest): string {
