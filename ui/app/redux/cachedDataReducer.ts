 ...
 * a number of redux actions. The reducer method is the reducer and the refresh
...
export class CachedDataReducer<TRequest, TResponseMessage> {
   ...
   *
   * req - the request associated with this call to refresh. It includes any parameters passed to the API call.
   * stateAccessor (optional) - a helper function that accesses this reducer's state given the global state object
   */
...
/**
 * KeyedCachedDataReducer is a wrapper object that contains a redux reducer and an instance of CachedDataReducer.
 * The reducer method is the reducer and the refresh method is the main action creator that refreshes
 * the data when dispatched. All action creators and the basic reducer are from the CachedDataReducer instance.
 *
 * Each instance of this class is instantiated with an api endpoint with request type
 * TRequest and response type Promise<TResponseMessage>.
 */
export class KeyedCachedDataReducer<TRequest, TResponseMessage> {
   ...
   *  apiEndpoint - The API endpoint the "refresh" function will use to refresh data.
   *  actionNamespace - A unique namespace for the redux actions.
   *  requestToID - A function that takes a TRequest and returns a string. Used as a key to store data returned from that request
   *  invalidationPeriodMillis (optional) - The number of milliseconds after data is received after which it will be invalidated.
   */
  ...
  /**
   * refresh calls the internal CachedDataReducer's refresh function using a default stateAccessor
   * that indexes in to the state based on a key generated from the request.
   */
  ...
  /**
   * Keyed redux reducer which pulls out the id from the action payload
   * and then runs the CachedDataReducer reducer on the action.
   */
