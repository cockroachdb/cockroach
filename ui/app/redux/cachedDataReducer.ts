...
export class CachedDataReducer<TRequest, TResponseMessage> {
  ...
  public static namespaces: { [namespace: string]: boolean } = {};
 ...
 *  namespace - A unique namespace for the redux actions.
 *  invalidationPeriodMillis - The number of milliseconds after data is received after which it will be invalidated.
 */
  constructor(private apiEndpoint: (req: TRequest) => Promise<TResponseMessage>, public namespace: string, private invalidationPeriodMillis?: number) {
    // check namespace
    assert.notProperty(CachedDataReducer.namespaces, namespace, "Expect namespace to be unique.");
    CachedDataReducer.namespaces[namespace] = true;

    this.REQUEST = `cockroachui/CachedDataReducer/${namespace}/REQUEST`;
    this.RECEIVE = `cockroachui/CachedDataReducer/${namespace}/RECEIVE`;
    this.ERROR = `cockroachui/CachedDataReducer/${namespace}/ERROR`;
    this.INVALIDATE = `cockroachui/CachedDataReducer/${namespace}/INVALIDATE`;
  }
   ...
   * Dispatching it will attempt to asynchronously refresh the cached data as long as:
  ...
  refresh = (req?: TRequest) => {
    return (dispatch: Dispatch, getState: () => any) => {
      let state: CachedDataReducerState<TResponseMessage> = getState().cachedData[this.namespace];
      ...
      // Fetch data from the servers. The promise is returned for testing.
