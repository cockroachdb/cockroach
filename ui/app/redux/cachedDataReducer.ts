 ...
 *
 * If an idGenerator is specified in the constructor, the data will be cached under a key
 */
export class CachedDataReducer<TRequest, TResponseMessage> {
  ...
  requestData = (req: TRequest): PayloadAction<void|WithID<void>> => {
    return {
      type: this.REQUEST,
      payload: this.idGenerator && { id: this.idGenerator(req) },
    };
  }
  ...
  receiveData = (data: TResponseMessage, req: TRequest): PayloadAction<TResponseMessage|WithID<TResponseMessage>> => {
    return {
      type: this.RECEIVE,
      payload: this.idGenerator ? { id: this.idGenerator(req), data } : data,
    };
  }
  ...
  errorData = (error: Error, req: TRequest): PayloadAction<Error|WithID<Error>> => {
    return {
      type: this.ERROR,
      payload: this.idGenerator ? { id: this.idGenerator(req), data: error } : error,
    };
  }

  // invalidateData is the INVALIDATE action creator.
  invalidateData = (req: TRequest): PayloadAction<void|WithID<void>> => {
    return {
