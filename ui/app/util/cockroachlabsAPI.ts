...
export function versionCheck(request: VersionCheckRequest, timeout?: moment.Duration): Promise<VersionList> {
  return timeoutFetch(`${COCKROACHLABS_ADDR}/api/clusters/updates?uuid=${request.clusterID}&version=${request.buildtag}`, undefined, { timeout });
}
...
export function unregisterCluster(request: UnregistrationRequest, timeout?: moment.Duration): Promise<{}> {
  return timeoutFetch(`${COCKROACHLABS_ADDR}/api/clusters/unregister?uuid=${request.clusterID}`, undefined, { method: "DELETE", timeout });
}
