...
class Constraints extends React.Component<ConstraintsProps, ConstraintsState> {
  ...
  nodeDatum(stores: cockroach.roachpb.StoreDescriptor[], candidates: _.Dictionary<cockroach.server.serverpb.Candidate>): Datum {
  ...
  tiersDatum(tiers: cockroach.roachpb.Tier[], base: Datum): Datum {
    if (tiers.length === 0) {
      return base;
    }
  ...
  mergeDatum(base: Datum, addition: Datum) {
    for (let d of base.children) {
      if (d.name === addition.name) {
        for (let a of addition.children) {
          this.mergeDatum(d, a);
        }
        return;
      }
