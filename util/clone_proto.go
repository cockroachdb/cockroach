...
func typeContainsArray(pb proto.Message) bool {
	t := reflect.TypeOf(pb)

	types.Lock()
	defer types.Unlock()

	typeContainsArray, ok := types.known[t]
	if !ok {
		typeContainsArray = isOrContainsArray(t)
		types.known[t] = typeContainsArray
	}
	return typeContainsArray
}
