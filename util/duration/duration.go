...
func (d Duration) String() string {
	return fmt.Sprintf("%dm%dd%dn", d.Months, d.Days, d.Nanos)
}

// Encode returns a three-tuple such that the original Duration is recoverable
// and the first int will approximately sort a collection of encoded Durations.
// If there would be an overflow, and error is returned and EncodeBigInt should
// be used instead.
func (d Duration) Encode() (sortNanos int64, months int64, days int64, err error) {
	sortNanos = d.Months*nanosInMonth + d.Days*nanosInDay + d.Nanos
	// TODO(dan): Handle overflow.
	return sortNanos, d.Months, d.Days, nil
}
...
func (d Duration) EncodeBigInt() (sortNanos *big.Int, months int64, days int64) {
	...
	sortNanos = big.NewInt(d.Nanos)
	sortNanos.Add(sortNanos, bigMonths).Add(sortNanos, bigDays)
	return sortNanos, d.Months, d.Days
}
...
// AddTime returns the time t+d.
func (d Duration) AddTime(t time.Time) time.Time {
	// TODO(dan): Overflow handling.
	return t.AddDate(0, int(d.Months), int(d.Days)).Add(time.Duration(d.Nanos))
}
