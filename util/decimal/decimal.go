...
func Pow(z, x, y *inf.Dec, s inf.Scale) *inf.Dec {
	...
	digitsToBitsRatio := inf.NewDec(3, 0)
	digitsToBitsRatio.Mul(decimalOne, digitsToBitsRatio)

	n := inf.NewDec(int64(x.UnscaledBig().BitLen()), 0)
	n.QuoRound(n, digitsToBitsRatio, 0, inf.RoundDown)
	numDigits := n.UnscaledBig().Int64() - int64(x.Scale())

	// Round up y which should provide us with a threshold in calulcating the new scale.
	yu := new(inf.Dec).Round(y, 0, inf.RoundUp).UnscaledBig().Int64()
