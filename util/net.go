...
func (rc *replayableConn) Read(p []byte) (int, error) {
	if limitReader, ok := rc.reader.(*io.LimitedReader); ok {
		...
		if !bytes.HasPrefix(http2ClientPrefaceFirstLineSlice[off:], p[:n]) {
			...
			// We've exhausted our LimitedReader.
			if rc.reader != rc.Conn {
				...
				// http2ClientPrefaceFirstLineSlice. However, Go 1.6 introduced strict
				// Host header checking for HTTP >= 1.1 requests (see
				...
				// the next line, we're going to help the caller out by providing a
				// bogus HTTP 1.x-style Host header. This will get us past Host header
				// verification.
				//
			...
			// We've replaced our LimitedReader with rc.Conn or a new MultiReader.
			// Either way, we can keep reading now.
			err = nil
		} else {
