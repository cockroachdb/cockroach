...
func (bc *replayableConn) replay() *replayableConn {
	if bc.isReplaying {
		log.Fatalf("%T is already replaying", bc)
	}
	bc.isReplaying = true
...
func (bc *replayableConn) Read(p []byte) (int, error) {
	...
	if err == nil {
		...
		if bytes.HasPrefix(p, http2ClientPrefaceFirstLine) {
			...
			// longer required; send all reads directly to the underlying connection.
			bc.reader = bc.Conn
			// Set isReplaying so that an errant call to replay will panic.
			bc.isReplaying = true
		}
		var m int
