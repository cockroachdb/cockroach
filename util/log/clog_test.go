...
`
	...
	timeParse := func(s string) time.Time {
		v, err := time.ParseInLocation("060102 15:04:05.999999", s, time.Local)
		if err != nil {
			t.Fatal(err)
		}
		return v
	...
	expected := []Entry{
		{
			Severity: 0,
			Time:     timeParse(`160224 10:55:08.758542`).UnixNano(),
			File:     `clog_test.go`,
			Line:     136,
			Message:  `info`,
		},
		{
			Severity: 1,
			Time:     timeParse(`160224 10:55:08.758653`).UnixNano(),
			File:     `clog_test.go`,
			Line:     137,
			Message:  `warning`,
		},
		{
			Severity: 2,
			Time:     timeParse(`160224 10:55:08.758658`).UnixNano(),
			File:     `clog_test.go`,
			Line:     138,
			Message:  `error`,
		},
		{
			Severity: 3,
			Time:     timeParse(`160224 10:55:08.758661`).UnixNano(),
			File:     `clog_test.go`,
			Line:     139,
			Message:  `fatal`,
		},
