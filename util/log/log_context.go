...
// context.Value) and from that we can traverse the entire chain. Ancestor
// contexts are associated to parts of that chain, so once a logTag is
// associated to a context, it is immutable.
type logTag struct {
	name string
