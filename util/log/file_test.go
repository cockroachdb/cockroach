...
func TestSelectFiles(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		// 0 is UNKNOWN, add 1 to avoid it.
		// NONE is not used, subtract 1 from the modulus to avoid it.
		// FATAL is not used in this test, subtract 1 from the modulus to avoid it.
		sev := Severity(i%(int(Severity_NONE)-1-1) + 1)
