...
// withEventLogInt embeds a trace.EventLog in the context, causing future logging
// and event calls to go to the EventLog. The current context must not have an
// existing open span.
func withEventLogInt(ctx context.Context, eventLog trace.EventLog) context.Context {
...
func WithEventLog(ctx context.Context, family, title string) context.Context {
	return withEventLogInt(ctx, trace.NewEventLog(family, title))
}
...
// false.
func getSpanOrEventLog(ctx context.Context) (opentracing.Span, *ctxEventLog,
	bool) {
...
func Event(ctx context.Context, msg string) {
	eventInternal(ctx, false, true, "%s", msg)
}
...
func ErrEvent(ctx context.Context, msg string) {
	eventInternal(ctx, true, true, "%s", msg)
}
