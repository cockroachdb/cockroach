...
func (n *node) split(i int, fast bool) (Interface, *node) {
	...
	copy(second.interfaces, n.interfaces[i+1:])
	n.interfaces = n.interfaces[:i]
	if len(n.children) > 0 {
		second.children = make(children, n.t.Degree)
...
// Delete deletes the Interface e if it exists in the B-tree. The deleted Interface is equal to the
// Interface e.
