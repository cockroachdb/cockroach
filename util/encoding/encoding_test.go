...
func TestEncodeDecodeVarint(t *testing.T) {
	...
	testCases := []testCaseInt64{
		{math.MinInt64, []byte{0x05, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}},
		{math.MinInt64 + 1, []byte{0x05, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01}},
		{-1 << 8, []byte{0x0b, 0xff, 0x00}},
		{-1, []byte{0x0c, 0xff}},
		{0, []byte{0x0d}},
		{1, []byte{0x0e}},
		{63, []byte{0x4c}},
		{64, []byte{0x4d, 0x40}},
		{1 << 8, []byte{0x4e, 0x01, 0x00}},
		{math.MaxInt64, []byte{0x54, 0x7f, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff}},
	}
...
func TestEncodeDecodeVarintDecreasing(t *testing.T) {
	...
	testCases := []testCaseInt64{
		{math.MinInt64, []byte{0x54, 0x7f, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff}},
		{math.MinInt64 + 1, []byte{0x54, 0x7f, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xfe}},
		{-1 << 8, []byte{0x4d, 0xff}},
		{-64, []byte{0x4c}},
		{-63, []byte{0x4b}},
		{-1, []byte{0x0d}},
		{0, []byte{0x0c, 0xff}},
		{1, []byte{0x0c, 0xfe}},
		{1 << 8, []byte{0x0b, 0xfe, 0xff}},
		{math.MaxInt64, []byte{0x05, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}},
	}
...
func TestEncodeDecodeUvarint(t *testing.T) {
	...
	testCases := []testCaseUint64{
		...
		{64, []byte{0x4d, 0x40}},
		{1 << 8, []byte{0x4e, 0x01, 0x00}},
		{math.MaxUint64, []byte{0x54, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff}},
	}
...
func TestDecodeInvalid(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			buf:     []byte{intMax + 1, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01},
			pattern: "invalid uvarint length of [0-9]+",
			decode:  func(b []byte) error { _, _, err := DecodeUvarint(b); return err },
		},
		{
...
func TestEncodeDecodeUvarintDecreasing(t *testing.T) {
	...
	testCases := []testCaseUint64{
		...
		{1, []byte{0x0c, 0xfe}},
		{1 << 8, []byte{0x0b, 0xfe, 0xff}},
		{math.MaxUint64 - 1, []byte{0x05, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01}},
		{math.MaxUint64, []byte{0x05, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}},
	}
...
func TestEncodeDecodeBytes(t *testing.T) {
	testCases := []struct {
		...
		{[]byte{0, 1, 'a'}, []byte{0x73, 0x00, 0xff, 1, 'a', 0x00, 0x01}},
		{[]byte{0, 'a'}, []byte{0x73, 0x00, 0xff, 'a', 0x00, 0x01}},
		{[]byte{0, 0xff, 'a'}, []byte{0x73, 0x00, 0xff, 0xff, 'a', 0x00, 0x01}},
		{[]byte{'a'}, []byte{0x73, 'a', 0x00, 0x01}},
		{[]byte{'b'}, []byte{0x73, 'b', 0x00, 0x01}},
		{[]byte{'b', 0}, []byte{0x73, 'b', 0x00, 0xff, 0x00, 0x01}},
		{[]byte{'b', 0, 0}, []byte{0x73, 'b', 0x00, 0xff, 0x00, 0xff, 0x00, 0x01}},
		{[]byte{'b', 0, 0, 'a'}, []byte{0x73, 'b', 0x00, 0xff, 0x00, 0xff, 'a', 0x00, 0x01}},
		{[]byte{'b', 0xff}, []byte{0x73, 'b', 0xff, 0x00, 0x01}},
		{[]byte("hello"), []byte{0x73, 'h', 'e', 'l', 'l', 'o', 0x00, 0x01}},
	}
...
func TestEncodeDecodeBytesDecreasing(t *testing.T) {
	testCases := []struct {
		...
		{[]byte("hello"), []byte{^byte(0x73), ^byte('h'), ^byte('e'), ^byte('l'), ^byte('l'), ^byte('o'), 0xff, 0xfe}},
		{[]byte{'b', 0xff}, []byte{^byte(0x73), ^byte('b'), 0x00, 0xff, 0xfe}},
		{[]byte{'b', 0, 0, 'a'}, []byte{^byte(0x73), ^byte('b'), 0xff, 0x00, 0xff, 0x00, ^byte('a'), 0xff, 0xfe}},
		{[]byte{'b', 0, 0}, []byte{^byte(0x73), ^byte('b'), 0xff, 0x00, 0xff, 0x00, 0xff, 0xfe}},
		{[]byte{'b', 0}, []byte{^byte(0x73), ^byte('b'), 0xff, 0x00, 0xff, 0xfe}},
		{[]byte{'b'}, []byte{^byte(0x73), ^byte('b'), 0xff, 0xfe}},
		{[]byte{'a'}, []byte{^byte(0x73), ^byte('a'), 0xff, 0xfe}},
		{[]byte{0, 0xff, 'a'}, []byte{^byte(0x73), 0xff, 0x00, 0x00, ^byte('a'), 0xff, 0xfe}},
		{[]byte{0, 'a'}, []byte{^byte(0x73), 0xff, 0x00, ^byte('a'), 0xff, 0xfe}},
		{[]byte{0, 1, 'a'}, []byte{^byte(0x73), 0xff, 0x00, 0xfe, ^byte('a'), 0xff, 0xfe}},
	}
	for i, c := range testCases {
...
func TestEncodeDecodeString(t *testing.T) {
	testCases := []struct {
		...
		{"\x00\x01a", []byte{0x73, 0x00, 0xff, 1, 'a', 0x00, 0x01}},
		{"\x00a", []byte{0x73, 0x00, 0xff, 'a', 0x00, 0x01}},
		{"\x00\xffa", []byte{0x73, 0x00, 0xff, 0xff, 'a', 0x00, 0x01}},
		{"a", []byte{0x73, 'a', 0x00, 0x01}},
		{"b", []byte{0x73, 'b', 0x00, 0x01}},
		{"b\x00", []byte{0x73, 'b', 0x00, 0xff, 0x00, 0x01}},
		{"b\x00\x00", []byte{0x73, 'b', 0x00, 0xff, 0x00, 0xff, 0x00, 0x01}},
		{"b\x00\x00a", []byte{0x73, 'b', 0x00, 0xff, 0x00, 0xff, 'a', 0x00, 0x01}},
		{"b\xff", []byte{0x73, 'b', 0xff, 0x00, 0x01}},
		{"hello", []byte{0x73, 'h', 'e', 'l', 'l', 'o', 0x00, 0x01}},
	}
	for i, c := range testCases {
...
func TestEncodeDecodeStringDecreasing(t *testing.T) {
	testCases := []struct {
		...
		{"hello", []byte{^byte(0x73), ^byte('h'), ^byte('e'), ^byte('l'), ^byte('l'), ^byte('o'), 0xff, 0xfe}},
		{"b\xff", []byte{^byte(0x73), ^byte('b'), 0x00, 0xff, 0xfe}},
		{"b\x00\x00a", []byte{^byte(0x73), ^byte('b'), 0xff, 0x00, 0xff, 0x00, ^byte('a'), 0xff, 0xfe}},
		{"b\x00\x00", []byte{^byte(0x73), ^byte('b'), 0xff, 0x00, 0xff, 0x00, 0xff, 0xfe}},
		{"b\x00", []byte{^byte(0x73), ^byte('b'), 0xff, 0x00, 0xff, 0xfe}},
		{"b", []byte{^byte(0x73), ^byte('b'), 0xff, 0xfe}},
		{"a", []byte{^byte(0x73), ^byte('a'), 0xff, 0xfe}},
		{"\x00\xffa", []byte{^byte(0x73), 0xff, 0x00, 0x00, ^byte('a'), 0xff, 0xfe}},
		{"\x00a", []byte{^byte(0x73), 0xff, 0x00, ^byte('a'), 0xff, 0xfe}},
		{"\x00\x01a", []byte{^byte(0x73), 0xff, 0x00, 0xfe, ^byte('a'), 0xff, 0xfe}},
	}
	for i, c := range testCases {
...
func TestEncodeDecodeTime(t *testing.T) {
	...
	if a, e := lastEncoded, []byte("\x74\x51\x01 \xbc\x0e\xae\x50\r\xf2\x8e\x80"); !bytes.Equal(a, e) {
