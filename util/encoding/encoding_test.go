...
func TestEncodeDecodeBytes(t *testing.T) {
	testCases := []struct {
		...
		{[]byte{0, 1, 'a'}, []byte{0x2c, 0x00, 0xff, 1, 'a', 0x00, 0x01}},
		{[]byte{0, 'a'}, []byte{0x2c, 0x00, 0xff, 'a', 0x00, 0x01}},
		{[]byte{0, 0xff, 'a'}, []byte{0x2c, 0x00, 0xff, 0xff, 'a', 0x00, 0x01}},
		{[]byte{'a'}, []byte{0x2c, 'a', 0x00, 0x01}},
		{[]byte{'b'}, []byte{0x2c, 'b', 0x00, 0x01}},
		{[]byte{'b', 0}, []byte{0x2c, 'b', 0x00, 0xff, 0x00, 0x01}},
		{[]byte{'b', 0, 0}, []byte{0x2c, 'b', 0x00, 0xff, 0x00, 0xff, 0x00, 0x01}},
		{[]byte{'b', 0, 0, 'a'}, []byte{0x2c, 'b', 0x00, 0xff, 0x00, 0xff, 'a', 0x00, 0x01}},
		{[]byte{'b', 0xff}, []byte{0x2c, 'b', 0xff, 0x00, 0x01}},
		{[]byte("hello"), []byte{0x2c, 'h', 'e', 'l', 'l', 'o', 0x00, 0x01}},
	}
...
func TestEncodeDecodeBytesDescending(t *testing.T) {
	testCases := []struct {
		...
		{[]byte("hello"), []byte{0x2e, ^byte('h'), ^byte('e'), ^byte('l'), ^byte('l'), ^byte('o'), 0xff, 0xfe}},
		{[]byte{'b', 0xff}, []byte{0x2e, ^byte('b'), 0x00, 0xff, 0xfe}},
		{[]byte{'b', 0, 0, 'a'}, []byte{0x2e, ^byte('b'), 0xff, 0x00, 0xff, 0x00, ^byte('a'), 0xff, 0xfe}},
		{[]byte{'b', 0, 0}, []byte{0x2e, ^byte('b'), 0xff, 0x00, 0xff, 0x00, 0xff, 0xfe}},
		{[]byte{'b', 0}, []byte{0x2e, ^byte('b'), 0xff, 0x00, 0xff, 0xfe}},
		{[]byte{'b'}, []byte{0x2e, ^byte('b'), 0xff, 0xfe}},
		{[]byte{'a'}, []byte{0x2e, ^byte('a'), 0xff, 0xfe}},
		{[]byte{0, 0xff, 'a'}, []byte{0x2e, 0xff, 0x00, 0x00, ^byte('a'), 0xff, 0xfe}},
		{[]byte{0, 'a'}, []byte{0x2e, 0xff, 0x00, ^byte('a'), 0xff, 0xfe}},
		{[]byte{0, 1, 'a'}, []byte{0x2e, 0xff, 0x00, 0xfe, ^byte('a'), 0xff, 0xfe}},
	}
	for i, c := range testCases {
...
func TestEncodeDecodeString(t *testing.T) {
	testCases := []struct {
		...
		{"\x00\x01a", []byte{0x2c, 0x00, 0xff, 1, 'a', 0x00, 0x01}},
		{"\x00a", []byte{0x2c, 0x00, 0xff, 'a', 0x00, 0x01}},
		{"\x00\xffa", []byte{0x2c, 0x00, 0xff, 0xff, 'a', 0x00, 0x01}},
		{"a", []byte{0x2c, 'a', 0x00, 0x01}},
		{"b", []byte{0x2c, 'b', 0x00, 0x01}},
		{"b\x00", []byte{0x2c, 'b', 0x00, 0xff, 0x00, 0x01}},
		{"b\x00\x00", []byte{0x2c, 'b', 0x00, 0xff, 0x00, 0xff, 0x00, 0x01}},
		{"b\x00\x00a", []byte{0x2c, 'b', 0x00, 0xff, 0x00, 0xff, 'a', 0x00, 0x01}},
		{"b\xff", []byte{0x2c, 'b', 0xff, 0x00, 0x01}},
		{"hello", []byte{0x2c, 'h', 'e', 'l', 'l', 'o', 0x00, 0x01}},
	}
	for i, c := range testCases {
...
func TestEncodeDecodeStringDecreasing(t *testing.T) {
	testCases := []struct {
		...
		{"hello", []byte{0x2e, ^byte('h'), ^byte('e'), ^byte('l'), ^byte('l'), ^byte('o'), 0xff, 0xfe}},
		{"b\xff", []byte{0x2e, ^byte('b'), 0x00, 0xff, 0xfe}},
		{"b\x00\x00a", []byte{0x2e, ^byte('b'), 0xff, 0x00, 0xff, 0x00, ^byte('a'), 0xff, 0xfe}},
		{"b\x00\x00", []byte{0x2e, ^byte('b'), 0xff, 0x00, 0xff, 0x00, 0xff, 0xfe}},
		{"b\x00", []byte{0x2e, ^byte('b'), 0xff, 0x00, 0xff, 0xfe}},
		{"b", []byte{0x2e, ^byte('b'), 0xff, 0xfe}},
		{"a", []byte{0x2e, ^byte('a'), 0xff, 0xfe}},
		{"\x00\xffa", []byte{0x2e, 0xff, 0x00, 0x00, ^byte('a'), 0xff, 0xfe}},
		{"\x00a", []byte{0x2e, 0xff, 0x00, ^byte('a'), 0xff, 0xfe}},
		{"\x00\x01a", []byte{0x2e, 0xff, 0x00, 0xfe, ^byte('a'), 0xff, 0xfe}},
	}
	for i, c := range testCases {
...
func TestEncodeDecodeTime(t *testing.T) {
	...
	for _, dir := range []Direction{Ascending, Descending} {
		...
		if dir == Ascending {
			a, e := lastEncoded, []byte("\x2d\xfa\x01 \xbc\x0e\xae\xf9\r\xf2\x8e\x80")
			if !bytes.Equal(a, e) {
...
func TestPeekType(t *testing.T) {
	testCases := []struct {
		...
		{EncodeDecimalDescending(nil, decimal.Decimal{}), Decimal},
		{EncodeDecimalDescending(nil, decimal.Decimal{}), Decimal},
