...
func testBasicEncodeDecode32(encFunc func([]byte, uint32, Direction) []byte,
	decFunc func([]byte, Direction) ([]byte, uint32, error), decreasing bool, t *testing.T) {
	testCases := []uint32{
		0, 1,
		1<<8 - 1, 1 << 8,
		1<<16 - 1, 1 << 16,
...
func testCustomEncodeUint32(testCases []testCaseUint32,
	encFunc func([]byte, uint32, Direction) []byte, dir Direction, t *testing.T) {
	for _, test := range testCases {
		enc := encFunc(nil, test.value, dir)
...
func TestEncodeDecodeUint32(t *testing.T) {
	...
	testCustomEncodeUint32(testCases, EncodeUint32, Ascending, t)
}

func TestEncodeDecodeUint32Decreasing(t *testing.T) {
	testBasicEncodeDecode32(EncodeUint32, DecodeUint32, true, t)
	testCases := []testCaseUint32{
	...
	testCustomEncodeUint32(testCases, EncodeUint32, Descending, t)
}

func testBasicEncodeDecodeUint64(encFunc func([]byte, uint64, Direction) []byte,
	decFunc func([]byte, Direction) ([]byte, uint64, error), decreasing bool, t *testing.T) {
	testCases := []uint64{
		0, 1,
		1<<8 - 1, 1 << 8,
		1<<16 - 1, 1 << 16,
...
func testBasicEncodeDecodeInt64(encFunc func([]byte, int64, Direction) []byte,
	decFunc func([]byte, Direction) ([]byte, int64, error), decreasing bool, t *testing.T) {
	testCases := []int64{
...
func testCustomEncodeInt64(testCases []testCaseInt64,
	encFunc func([]byte, int64, Direction) []byte, dir Direction, t *testing.T) {
	for _, test := range testCases {
		enc := encFunc(nil, test.value, dir)
		if bytes.Compare(enc, test.expEnc) != 0 {
...
func testCustomEncodeUint64(testCases []testCaseUint64,
	encFunc func([]byte, uint64, Direction) []byte, dir Direction, t *testing.T) {
	for _, test := range testCases {
		enc := encFunc(nil, test.value, dir)
		if bytes.Compare(enc, test.expEnc) != 0 {
			t.Errorf("expected [% x]; got [% x] (value: %d)", test.expEnc, enc, test.value)
...
func TestEncodeDecodeUint64(t *testing.T) {
	...
	testCustomEncodeUint64(testCases, EncodeUint64, Ascending, t)
}

func TestEncodeDecodeUint64Decreasing(t *testing.T) {
	testBasicEncodeDecodeUint64(EncodeUint64, DecodeUint64, true, t)
	testCases := []testCaseUint64{
	...
	testCustomEncodeUint64(testCases, EncodeUint64, Descending, t)
}
...
func TestEncodeDecodeVarint(t *testing.T) {
	...
	testCustomEncodeInt64(testCases, EncodeVarint, Ascending, t)
}

func TestEncodeDecodeVarintDecreasing(t *testing.T) {
	testBasicEncodeDecodeInt64(EncodeVarint, DecodeVarint, true, t)
	testCases := []testCaseInt64{
	...
	testCustomEncodeInt64(testCases, EncodeVarint, Descending, t)
}
...
func TestEncodeDecodeUvarint(t *testing.T) {
	...
	testCustomEncodeUint64(testCases, EncodeUvarint, Ascending, t)
}
...
func TestDecodeInvalid(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			decode:  func(b []byte) error { _, _, err := DecodeVarint(b, Ascending); return err },
		},
		{
			name:    "Bytes, no marker",
			buf:     []byte{'a'},
			pattern: "did not find marker",
			decode:  func(b []byte) error { _, _, err := DecodeBytes(b, nil, Ascending); return err },
		},
		{
		...
		},
		{
			...
			pattern: "did not find terminator",
			decode:  func(b []byte) error { _, _, err := DecodeBytes(b, nil, Descending); return err },
		},
		{
...
func TestEncodeDecodeUvarintDecreasing(t *testing.T) {
	testBasicEncodeDecodeUint64(EncodeUvarint, DecodeUvarint, true, t)
	testCases := []testCaseUint64{
		{0, []byte{0x88}},
		{1, []byte{0x87, 0xfe}},
		{1 << 8, []byte{0x86, 0xfe, 0xff}},
	...
	testCustomEncodeUint64(testCases, EncodeUvarint, Descending, t)
}
...
func TestEncodeDecodeBytes(t *testing.T) {
	...
	for i, c := range testCases {
		enc := EncodeBytes(nil, c.value, Ascending)
		...
		remainder, dec, err := DecodeBytes(enc, nil, Ascending)
		if err != nil {
			t.Error(err)
			continue
		}
		...
		remainder, _, err = DecodeBytes(enc, nil, Ascending)
		if err != nil {
			t.Error(err)
			continue
		}
...
func TestEncodeDecodeBytesDecreasing(t *testing.T) {
	testCases := []struct {
		...
		{[]byte("hello"), []byte{0x21, ^byte('h'), ^byte('e'), ^byte('l'), ^byte('l'), ^byte('o'), 0xff, 0xfe}},
		{[]byte{'b', 0xff}, []byte{0x21, ^byte('b'), 0x00, 0xff, 0xfe}},
		{[]byte{'b', 0, 0, 'a'}, []byte{0x21, ^byte('b'), 0xff, 0x00, 0xff, 0x00, ^byte('a'), 0xff, 0xfe}},
		{[]byte{'b', 0, 0}, []byte{0x21, ^byte('b'), 0xff, 0x00, 0xff, 0x00, 0xff, 0xfe}},
		{[]byte{'b', 0}, []byte{0x21, ^byte('b'), 0xff, 0x00, 0xff, 0xfe}},
		{[]byte{'b'}, []byte{0x21, ^byte('b'), 0xff, 0xfe}},
		{[]byte{'a'}, []byte{0x21, ^byte('a'), 0xff, 0xfe}},
		{[]byte{0, 0xff, 'a'}, []byte{0x21, 0xff, 0x00, 0x00, ^byte('a'), 0xff, 0xfe}},
		{[]byte{0, 'a'}, []byte{0x21, 0xff, 0x00, ^byte('a'), 0xff, 0xfe}},
		{[]byte{0, 1, 'a'}, []byte{0x21, 0xff, 0x00, 0xfe, ^byte('a'), 0xff, 0xfe}},
	}
	for i, c := range testCases {
		enc := EncodeBytes(nil, c.value, Descending)
		if !bytes.Equal(enc, c.encoded) {
			t.Errorf("unexpected encoding mismatch for %v. expected [% x], got [% x]",
				c.value, c.encoded, enc)
		}
		...
		remainder, dec, err := DecodeBytes(enc, nil, Descending)
		if err != nil {
			t.Error(err)
			continue
		}
		...
		enc = append(enc, []byte("remainder")...)
		remainder, _, err = DecodeBytes(enc, nil, Descending)
		if err != nil {
			t.Error(err)
			continue
		}
...
func TestEncodeDecodeString(t *testing.T) {
	...
	for i, c := range testCases {
		enc := EncodeString(nil, c.value, Ascending)
		if !bytes.Equal(enc, c.encoded) {
			t.Errorf("unexpected encoding mismatch for %v. expected [% x], got [% x]",
				c.value, c.encoded, enc)
		}
		...
		remainder, dec, err := DecodeString(enc, nil, Ascending)
		if err != nil {
			t.Error(err)
			continue
		}
		...
		remainder, _, err = DecodeString(enc, nil, Ascending)
		if err != nil {
			t.Error(err)
			continue
		}
...
func TestEncodeDecodeStringDecreasing(t *testing.T) {
	testCases := []struct {
		...
		{"hello", []byte{0x21, ^byte('h'), ^byte('e'), ^byte('l'), ^byte('l'), ^byte('o'), 0xff, 0xfe}},
		{"b\xff", []byte{0x21, ^byte('b'), 0x00, 0xff, 0xfe}},
		{"b\x00\x00a", []byte{0x21, ^byte('b'), 0xff, 0x00, 0xff, 0x00, ^byte('a'), 0xff, 0xfe}},
		{"b\x00\x00", []byte{0x21, ^byte('b'), 0xff, 0x00, 0xff, 0x00, 0xff, 0xfe}},
		{"b\x00", []byte{0x21, ^byte('b'), 0xff, 0x00, 0xff, 0xfe}},
		{"b", []byte{0x21, ^byte('b'), 0xff, 0xfe}},
		{"a", []byte{0x21, ^byte('a'), 0xff, 0xfe}},
		{"\x00\xffa", []byte{0x21, 0xff, 0x00, 0x00, ^byte('a'), 0xff, 0xfe}},
		{"\x00a", []byte{0x21, 0xff, 0x00, ^byte('a'), 0xff, 0xfe}},
		{"\x00\x01a", []byte{0x21, 0xff, 0x00, 0xfe, ^byte('a'), 0xff, 0xfe}},
	}
	for i, c := range testCases {
		enc := EncodeString(nil, c.value, Descending)
		if !bytes.Equal(enc, c.encoded) {
			t.Errorf("unexpected encoding mismatch for %v. expected [% x], got [% x]",
				c.value, c.encoded, enc)
		}
		...
		remainder, dec, err := DecodeString(enc, nil, Descending)
		if err != nil {
			t.Error(err)
			continue
		}
		...
		enc = append(enc, "remainder"...)
		remainder, _, err = DecodeString(enc, nil, Descending)
		if err != nil {
			t.Error(err)
			continue
		}
...
func TestEncodeDecodeNull(t *testing.T) {
	...
	buf := EncodeNull([]byte(hello), Ascending)
...
func TestEncodeDecodeTime(t *testing.T) {
	...
	for _, dir := range []Direction{Ascending, Descending} {
		for i := range testCases {
			...
			var b []byte
			if !last.IsZero() {
				b = EncodeTime(b, current, dir)
				_, decodedCurrent, err := DecodeTime(b, dir)
				if err != nil {
					t.Error(err)
					continue
				}
...
func TestPeekType(t *testing.T) {
	testCases := []struct {
		...
		{EncodeNull(nil, Ascending), Null},
		{EncodeNotNull(nil, Ascending), NotNull},
		{EncodeNull(nil, Descending), Null},
		{EncodeNotNull(nil, Descending), NotNull},
		{EncodeVarint(nil, 0, Ascending), Int},
		{EncodeUvarint(nil, 0, Ascending), Int},
		{EncodeFloat(nil, 0, Ascending), Float},
		{EncodeBytes(nil, []byte(""), Ascending), Bytes},
		{EncodeTime(nil, time.Now(), Ascending), Time},
	}
	for i, c := range testCases {
		typ := PeekType(c.enc)
		if c.typ != typ {
...
func BenchmarkEncodeUint32(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = EncodeUint32(buf, vals[i%len(vals)], Ascending)
	}
}
...
func BenchmarkDecodeUint32(b *testing.B) {
	...
	for i := range vals {
		vals[i] = EncodeUint32(nil, uint32(rng.Int31()), Ascending)
	}

	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _, _ = DecodeUint32(vals[i%len(vals)], Ascending)
	}
}
...
func BenchmarkEncodeUint64(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = EncodeUint64(buf, vals[i%len(vals)], Ascending)
	}
}
...
func BenchmarkDecodeUint64(b *testing.B) {
	...
	vals := make([][]byte, 10000)
	for i := range vals {
		vals[i] = EncodeUint64(nil, uint64(rng.Int63()), Ascending)
	}

	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _, _ = DecodeUint64(vals[i%len(vals)], Ascending)
	}
}
...
func BenchmarkEncodeVarint(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = EncodeVarint(buf, vals[i%len(vals)], Ascending)
	}
}
...
func BenchmarkDecodeVarint(b *testing.B) {
	...
	vals := make([][]byte, 10000)
	for i := range vals {
		vals[i] = EncodeVarint(nil, rng.Int63(), Ascending)
	}

	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _, _ = DecodeVarint(vals[i%len(vals)], Ascending)
	}
}
...
func BenchmarkEncodeUvarint(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = EncodeUvarint(buf, vals[i%len(vals)], Ascending)
	}
}
...
func BenchmarkDecodeUvarint(b *testing.B) {
	...
	vals := make([][]byte, 10000)
	for i := range vals {
		vals[i] = EncodeUvarint(nil, uint64(rng.Int63()), Ascending)
	}

	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _, _ = DecodeUvarint(vals[i%len(vals)], Ascending)
	}
}
...
func BenchmarkEncodeBytes(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = EncodeBytes(buf, vals[i%len(vals)], Ascending)
	}
}
...
func BenchmarkEncodeBytesDecreasing(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = EncodeBytes(buf, vals[i%len(vals)], Descending)
	}
}
...
func BenchmarkDecodeBytes(b *testing.B) {
	...
	vals := make([][]byte, 10000)
	for i := range vals {
		vals[i] = EncodeBytes(nil, randutil.RandBytes(rng, 100), Ascending)
	}
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _, _ = DecodeBytes(vals[i%len(vals)], buf, Ascending)
	}
}
...
func BenchmarkDecodeBytesDecreasing(b *testing.B) {
	...
	vals := make([][]byte, 10000)
	for i := range vals {
		vals[i] = EncodeBytes(nil, randutil.RandBytes(rng, 100), Descending)
	}
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _, _ = DecodeBytes(vals[i%len(vals)], buf, Descending)
	}
}
...
func BenchmarkEncodeString(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = EncodeString(buf, vals[i%len(vals)], Ascending)
	}
}
...
func BenchmarkEncodeStringDecreasing(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = EncodeString(buf, vals[i%len(vals)], Descending)
	}
}
...
func BenchmarkDecodeString(b *testing.B) {
	...
	vals := make([][]byte, 10000)
	for i := range vals {
		vals[i] = EncodeString(nil, string(randutil.RandBytes(rng, 100)), Ascending)
	}
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _, _ = DecodeString(vals[i%len(vals)], buf, Ascending)
	}
}
...
func BenchmarkDecodeStringDecreasing(b *testing.B) {
	...
	vals := make([][]byte, 10000)
	for i := range vals {
		vals[i] = EncodeString(nil, string(randutil.RandBytes(rng, 100)), Descending)
	}
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _, _ = DecodeString(vals[i%len(vals)], buf, Descending)
	}
}
