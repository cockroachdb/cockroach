...
func randValueEncode(rd randData, buf []byte, colID uint32, typ Type) ([]byte, bool) {
	switch typ {
	case Null:
		return EncodeNullValue(buf, colID), true
	case True:
		return EncodeBoolValue(buf, colID, true), true
	case False:
		return EncodeBoolValue(buf, colID, false), true
	case Int:
		return EncodeIntValue(buf, colID, rd.Int63()), true
	case Float:
		return EncodeFloatValue(buf, colID, rd.NormFloat64()), true
	case Decimal:
		return EncodeDecimalValue(buf, colID, rd.decimal()), true
	case Bytes:
		return EncodeBytesValue(buf, colID, randutil.RandBytes(rd.Rand, 100)), true
	case Time:
		return EncodeTimeValue(buf, colID, rd.time()), true
	case Duration:
		return EncodeDurationValue(buf, colID, rd.duration()), true
	default:
		return buf, false
	}
}
...
func TestValueEncodingPeekLength(t *testing.T) {
	...
	for i := 0; i < 1000; {
		lastLen := len(buf)
		var ok bool
		buf, ok = randValueEncode(rd, buf, uint32(rng.Int63()), Type(rng.Intn(int(SentinelType))))
		if ok {
