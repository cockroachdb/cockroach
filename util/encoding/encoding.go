...
func decodeBytesInternal(b []byte, r []byte, e escapes) ([]byte, []byte, error) {
	if len(b) == 0 || b[0] != e.marker {
		return nil, nil, util.Errorf("did not find marker: %#x", b)
	}
	b = b[1:]
	...
	for {
		...
		if i == -1 {
			return nil, nil, util.Errorf("did not find terminator: %#x", b)
		}
		if i+1 >= len(b) {
			return nil, nil, util.Errorf("malformed escape: %#x", b)
		}
