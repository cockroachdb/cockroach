...
// nat into buf using big-endian encoding. len(buf) must be >= len(nat)*_S.
...
var digitsLookupTable map[int]tableVal
...
func init() {
	const tableSize = 128
	digitsLookupTable = make(map[int]tableVal, tableSize)
	...
	for i := 1; i <= tableSize; i++ {
		...
		digitsLookupTable[i] = tableVal{
			digits: digits,
			border: bigIntArr[i-1].Exp(ten, digitBi, nil),
		}
	}
}
...
func numDigits(bi *big.Int, tmp []byte) (int, []byte) {
	if val, ok := digitsLookupTable[bi.BitLen()]; ok {
