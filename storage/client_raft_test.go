...
func TestReplicateAddAndRemove(t *testing.T) {
	...
	testFunc := func(addFirst bool) {
		...
		mtc.manualClock.Increment(int64(storage.ReplicaGCQueueInactivityThreshold+storage.DefaultLeaderLeaseDuration(mtc.clock.MaxOffset())) + 1)
...
func TestReplicateRogueRemovedNode(t *testing.T) {
	...
	util.SucceedsSoon(t, func() error {
		mtc.manualClock.Increment(int64(storage.DefaultLeaderLeaseDuration(mtc.clock.MaxOffset())+
...
func TestLeaderRemoveSelf(t *testing.T) {
	...
	header.Timestamp = clock.Update(clock.Now().Add(int64(storage.DefaultLeaderLeaseDuration(clock.MaxOffset())), 0))
