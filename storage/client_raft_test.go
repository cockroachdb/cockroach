...
func TestStoreRangeDownReplicate(t *testing.T) {
	...
	{
		ref := store0.LookupReplica(roachpb.RKeyMin, nil)
...
func TestReplicaRemovalCampaign(t *testing.T) {
	...
	for i, td := range testData {
		func() {
			...
			replica2, release, err := ref.Acquire()
			...
			if td.remove {
				...
				if err := store0.RemoveReplica(ref, *ref.Desc(), true, false); err != nil {
					t.Fatal(err)
				}
			}
...
func TestTransferRaftLeadership(t *testing.T) {
	...
	ref := mtc.stores[0].LookupReplica(keys.MustAddr(key), nil)
	if ref == nil {
