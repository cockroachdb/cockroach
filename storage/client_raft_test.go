	...
	for i := 0; i < extraStores; i++ {
		util.SucceedsSoon(t, func() error {
			...
			if len(replicas) < numReplicas {
				return errors.Errorf("initial replication: len(replicas) = %d < %d, %+v", len(replicas), numReplicas, replicas)
			}
		...
		util.SucceedsSoon(t, func() error {
			...
			// to be delivered the first time. Gossiping stores doesn't have the same
			// issue since the StoreGossiper gossips the store descriptors from all
			// stores.
			corrupt.Lock()
			...
			if len(replicas) < numReplicas {
				return errors.Errorf("len(replicas) = %d < %d, %+v", len(replicas), numReplicas, replicas)
			}
			return nil
		})
	}
