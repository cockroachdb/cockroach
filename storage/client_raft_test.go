...
func TestCheckInconsistent(t *testing.T) {
	...
	case <-time.After(5 * time.Second):
		t.Fatal("didn't receive notification from CollectChecksum() that should have panicked")
	}
	select {
	case <-notifyReportDiff:
	case <-time.After(5 * time.Second):
		t.Fatal("didn't receive notification from CheckConsistency() that should have reported a diff")
	}
}
