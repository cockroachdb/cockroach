...
func TestStoreRangeRebalance(t *testing.T) {
	...
	*mtc.storeContext = storage.TestStoreContext()
	...
	mtc.initGossipNetwork()
	util.SucceedsSoon(t, func() error {
		// The lease for the first replica tends to congregate on store 2,
		// which stops this test from rebalancing. So force the lease for the
		...
		// there may be a slight delay after gossiping has occurred and the
		// store pools updating their internal state as this is an async task.
		mtc.gossipStores()
