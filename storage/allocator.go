...
const (
	...
	// minSecsBetweenRebalances is the minimum number of seconds that must elapse
	// between replica rebalances. This should be slightly larger than
	// server.gossipStoresInterval.
	minRebalanceInterval = 65 * time.Second
)
...
func (a *Allocator) ShouldRebalance(storeID roachpb.StoreID) bool {
	...
	if now.Before(a.nextRebalance) {
		if log.V(2) {
			log.Infof("ineligible for rebalance for %.f seconds", a.nextRebalance.Sub(now).Seconds())
		}
		return false
	}
...
func (a *Allocator) UpdateNextRebalance() {
	...
	if log.V(2) {
		log.Infof("next rebalance opportunity in %.f seconds",
			a.nextRebalance.Sub(timeutil.Now()).Seconds())
	}
}
