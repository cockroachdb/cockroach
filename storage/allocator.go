...
func (a *Allocator) AllocateTarget(
	...
	log.Infof(context.TODO(), "AllocateTarget condidates %+v", candidates)
...
func (a Allocator) RebalanceTarget(
	...
	// TODO(d4l3k): Determine if a.options.Deterministic is needed.
	...
	for _, candidate := range candidates {
		found := false
		for _, repl := range existing {
			if repl.StoreID == candidate.StoreID {
				found = true
				break
			}
		}
