...
func TestRangeTransferLease(t *testing.T) {
	...
	{
		// Transfering the lease to ourself should be a no-op.
		origLease, _ := replica0.GetLease()
		if pErr := replica0.AdminTransferLease(replica0Desc.StoreID); pErr != nil {
			t.Fatal(pErr)
		}
		newLease, _ := replica0.GetLease()
		if !reflect.DeepEqual(origLease, newLease) {
...

	{
		...
		if pErr := replica0.AdminTransferLease(1000); !testutils.IsError(pErr, expected) {
			t.Fatalf("expected %s, but found %v", expected, pErr)
		}
	}
	...
	go func() {
		...
		if pErr := replica1.AdminTransferLease(replica0Desc.StoreID); pErr != nil {
			panic(pErr)
		}
	}()
