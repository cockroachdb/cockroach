...
func TestReplicaGCShouldQueue(t *testing.T) {
	...
	_, _, _, _ = cTS, cTSnext, iTS, iTSnext
	...
	for i, test := range []struct {
		...
		// No activity for candidate timeout; queue with priority.
		{now: cTSnext, lastCheck: z, lastActivity: bTS, isCandidate: true, shouldQ: true, priority: 1},
		// Last processed recently: shortened candidate timeout takes no effect.
		{now: cTSnext, lastCheck: bTS, lastActivity: z, isCandidate: true, shouldQ: false, priority: 0},
		// Still no effect until iTS reached.
		{now: iTS.Add(-1, 0), lastCheck: bTS, lastActivity: z, isCandidate: true, shouldQ: false, priority: 0},
