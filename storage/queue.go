...
// as the replica with the lowest priority will be dropped. Returns an
// error if the replica was not added. Returns whether the replica was
// added (true), or was already added (false), and an error if the
// replica was not added.
		...
		for {
			...
			case <-bq.impl.purgatoryChan():
				...
				if len(bq.mu.purgatory) == 0 {
					bq.eventLog.VInfof(log.V(0), "purgatory is now empty")
					bq.mu.purgatory = nil
					bq.mu.Unlock()
					return
				}
