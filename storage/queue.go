...
type queueImpl interface {
	...
	shouldQueue(hlc.Timestamp, ReplicaPromise, config.SystemConfig) (shouldQueue bool, priority float64)
	...
	// to be initialized.
	process(context.Context, hlc.Timestamp, ReplicaPromise, config.SystemConfig) error
...
func (bq *baseQueue) Add(repl ReplicaPromise, priority float64) (bool, error) {
	bq.mu.Lock()
	defer bq.mu.Unlock()
...
// dropped.
func (bq *baseQueue) MaybeAdd(rp ReplicaPromise, now hlc.Timestamp) {
	// Load the system config.
	cfg, cfgOk := bq.gossip.GetSystemConfig()
	requiresSplit := cfgOk && bq.requiresSplit(cfg, rp)
	...
	if requiresSplit {
		...
		log.VEventf(1, bq.ctx, "%s: split needed; not adding", rp)
		return
	}

	should, priority := bq.impl.shouldQueue(now, rp, cfg)
	if _, err := bq.addInternal(rp, should, priority); !isExpectedQueueError(err) {
		log.Errorf(bq.ctx, "unable to add %s: %s", rp, err)
	}
}

func (bq *baseQueue) requiresSplit(cfg config.SystemConfig, rp ReplicaPromise) bool {
...
func (bq *baseQueue) addInternal(rp ReplicaPromise, should bool, priority float64) (bool, error) {
	repl, release, err := rp.Acquire()
	defer release()
	if err != nil {
		return false, err
...
func (bq *baseQueue) MaybeRemove(rp ReplicaPromise) {
	bq.mu.Lock()
	defer bq.mu.Unlock()

	// Note that no attempt at unboxing `rp` is made. Especially if the replica
	// has been deleted, we still want to remove it from the queue.
	desc := rp.Desc()
	if bq.mu.stopped || desc == nil {
		return
	}

	if item, ok := bq.mu.replicas[desc.RangeID]; ok {
			...
			case <-nextTime:
				...
				if rp != nil {
					if stopper.RunTask(func() {
						if err := bq.processReplica(rp, clock); err != nil {
							// Maybe add failing replica to purgatory if the queue supports it.
							bq.maybeAddToPurgatory(rp, err, clock, stopper)
						}
					}) != nil {
						return
					}
...
func (bq *baseQueue) processReplica(rp ReplicaPromise, clock *hlc.Clock) error {
	bq.processMu.Lock()
	...
	if bq.requiresSplit(cfg, rp) {
		// Range needs to be split due to zone configs, but queue does
		// not accept unsplit ranges.
		log.VEventf(3, bq.ctx, "%s: split needed; skipping", rp)
		return nil
	}
	...
	if pErr := func() *roachpb.Error {
		...
		defer release()
		if err != nil {
			return roachpb.NewError(roachpb.NewRangeNotFoundError(rp.Desc().RangeID))
		}
		...
		default:
			return errors.Wrapf(pErr.GoError(), "%s: could not obtain lease", rp)
		}
	}

	log.VEventf(3, bq.ctx, "processing")
	start := timeutil.Now()
	err := bq.impl.process(ctx, clock.Now(), rp, cfg)
...
// purgatory.
func (bq *baseQueue) maybeAddToPurgatory(rp ReplicaPromise, triggeringErr error, clock *hlc.Clock, stopper *stop.Stopper) {
	repl, release, err := rp.Acquire()
	defer release()
	if err != nil {
		return
...
// locked.
func (bq *baseQueue) pop() ReplicaPromise {
	bq.mu.Lock()
