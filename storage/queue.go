...
type queueImpl interface {
	...
	// purgatoryChan returns a channel to signals when it's time to retry
...
// processing. The failing error must be retryable and the queue
// implementation must have its own mechanism for signaling
// re-processing of replicas held in purgatory.
func (bq *baseQueue) maybeAddToPurgatory(repl *Replica, err error, clock *hlc.Clock, stopper *stop.Stopper) {
	// Verify the replica failure and the queue require purgatory.
	if r, ok := err.(retry.Retryable); !ok || !r.CanRetry() || bq.impl.purgatoryChan() == nil {
		return
	}
	bq.Lock()
	defer bq.Unlock()
...
func (bq *baseQueue) DrainQueue(clock *hlc.Clock) {
	...
	for repl != nil {
		if err := bq.processReplica(repl, clock); err != nil {
			log.Errorf("[%s] failed processing replica %s: %s", bq.name, repl, err)
		}
		bq.Lock()
		repl = bq.pop()
		bq.Unlock()
