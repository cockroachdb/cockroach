...
func (p *pendingLeaderLeaseRequest) InitOrJoinRequest(
	...
	if !replica.store.Stopper().RunAsyncTask(func() {
		// Proposed a LeaderLease command and wait for it to apply.
		...
		ch, _, err := replica.proposeRaftCommand(replica.context(context.Background()), ba)
		if err != nil {
...
// getOrExtendLeaderLeaseLocked executes a request to obtain or extend a lease
// asynchronously and returns a promise for the result. If there's already a
// request in progress, we join in waiting the results of that request.
// Unless an error is returned, the obtained lease will be valid for a time
// interval containing the requested timestamp.
func (r *Replica) getOrExtendLeaderLeaseLocked(timestamp hlc.Timestamp) <-chan *roachpb.Error {
