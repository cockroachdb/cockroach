...
func (r *Replica) maybeCoalesceHeartbeats(msg raftpb.Message,
	...
	if enableCoalescedHeartbeats && (msg.Type == raftpb.MsgHeartbeat || msg.Type == raftpb.MsgHeartbeatResp) {
		...
		if msg.Type == raftpb.MsgHeartbeat {
			r.store.coalesced.heartbeats[toStore] = append(r.store.coalesced.heartbeats[toStore], beat)
		} else if msg.Type == raftpb.MsgHeartbeatResp {
			r.store.coalesced.heartbeatResponses[toStore] = append(r.store.coalesced.heartbeatResponses[toStore], beat)
		}
...
func (r *Replica) sendRaftMessage(msg raftpb.Message) {
	...
	if r.maybeCoalesceHeartbeats(msg, toReplica, fromReplica) {
		return
	}
