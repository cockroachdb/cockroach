...
func (r *Replica) endCmds(cmdKeys []interface{}, ba *roachpb.BatchRequest, br *roachpb.BatchResponse, pErr *roachpb.Error) {
	r.mu.Lock()
	if r.mu.frozen {
		if arg, ok := ba.GetArg(roachpb.ChangeFrozen); ok &&
			arg.(*roachpb.ChangeFrozenRequest).Frozen {
			...
			// might be in order to move this to a "better" location a few stack
			// frames up. In particular, we should be able to avoid the panic
			// but simply return an error to the client (or we risk wedging
			// ourselves in a bad state while a Replica is offline).
			waitFor := r.mu.appliedIndex
			// The defer will run outside of the lock.
			defer func() {
				if err := util.RetryForDuration(10*time.Second, func() error {
					...
					// here stalls since oldestInUse is always >= raftStatus.Applied.
					if waitFor >= oldestInUse {
						r.store.stopper.RunAsyncTask(func() {
							...
							// and (at most once) mildly annoying otherwise.
							_ = r.requestLeaderLease(r.store.Clock().Now())
						})
						return util.Errorf("waiting for index %d to be unused "+
							"(still using %d)", waitFor, oldestInUse)
					}
					return nil
				}); err != nil {
					panic(err)
