...
func (r *Replica) beginCmds(ctx context.Context, ba *roachpb.BatchRequest) (func(*roachpb.BatchResponse, *roachpb.Error) *roachpb.Error, error) {
	...
	if ba.ReadConsistency != roachpb.INCONSISTENT {
		...
		for i, ch := range chans {
			...
			case <-ch:
			case <-ctx.Done():
				// Continue waiting in the background. The command queue
				// assumes that commands don't drop out before their
				// prerequisites, but the caller is free to go.
				go func(idx int) {
					for _, ch := range chans[idx:] {
						<-ch
					}
