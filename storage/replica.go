...
type Replica struct {
	...
	mu struct {
		...
		// there is another event that will repopulate the replicas map.
		// When it is temporarily dropped and recreated, the newly recreated
		// replica will have a complete range descriptor.
...
func (r *Replica) getReplicaDescriptorByIDLocked(
	...
	if repDesc, ok := r.mu.state.Desc.GetReplicaDescriptorByID(roachpb.ReplicaID(replicaID)); ok {
		return repDesc
	}
	return fallback
}
...
func (r *Replica) sendRaftMessage(msg raftpb.Message) {
	...
	r.mu.Lock()
	fromReplica := r.getReplicaDescriptorByIDLocked(roachpb.ReplicaID(msg.From), r.mu.lastToReplica)
	toReplica := r.getReplicaDescriptorByIDLocked(roachpb.ReplicaID(msg.To), r.mu.lastFromReplica)
	r.mu.Unlock()

	if fromReplica.ReplicaID != roachpb.ReplicaID(msg.From) {
		log.Warningf(context.TODO(), "failed to look up sender replica %d in range %d while sending %s", msg.From, rangeID, msg.Type)
		return
	}
	if toReplica.ReplicaID != roachpb.ReplicaID(msg.To) {
		log.Warningf(context.TODO(), "failed to look up recipient replica %d in range %d while sending %s", msg.To, rangeID, msg.Type)
		return
	}
