...
func (r *Replica) getLeaderLease() (*roachpb.Lease, *roachpb.Lease) {
	r.mu.Lock()
	defer r.mu.Unlock()
	return r.mu.state.lease, r.mu.pendingLeaderLeaseRequest.RequestPending()
}
...
// checkLeaseAndMaybeExtendLocked checks that the replica owns the lease at the
// specified timestamp.
...
func (r *Replica) checkLeaseAndMaybeExtendLocked(
...
func (r *Replica) redirectOnOrAcquireLeaderLease(ctx context.Context) *roachpb.Error {
	...
	for attempt := 1; ; attempt++ {
		...
		llChan, pErr := func() (<-chan *roachpb.Error, *roachpb.Error) {
			r.mu.Lock()
			defer r.mu.Unlock()
			covering, pErr := r.checkLeaseAndMaybeExtendLocked(timestamp)
			if pErr != nil {
				return nil, pErr
			}
			if covering {
...
func (r *Replica) GetReplica() (*roachpb.ReplicaDescriptor, error) {
	...
	if replica == nil {
		return nil, &errReplicaNotInRange{
			StoreID: r.store.StoreID(), RangeDesc: *r.Desc()}
	}
	return replica, nil
}
