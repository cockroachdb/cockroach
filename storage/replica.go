...
func frozenWaitTrigger(r *Replica, ba roachpb.BatchRequest, waitForIndex uint64) {
	arg, ok := ba.GetArg(roachpb.ChangeFrozen)
	if !ok || !arg.(*roachpb.ChangeFrozenRequest).Frozen {
		return
	}

	// We're frozen and the batch contains a freeze request, so it's
	// fair to assume this is the request that froze us. We must make
	// sure that *all* Replicas have applied this (not only a majority).
	...
	if err := util.RetryForDuration(10*time.Second, func() error {
		...
		_, oldestInUse, err := getTruncatableIndexes(r)
		if err != nil {
			return err
		}
		// TODO(tschottdorf): is this off-by-one? Failing on equality
		// here stalls without the leader lease requests since
		// usually oldestInUse == raftStatus.Applied.
		if waitForIndex >= oldestInUse {
			r.store.stopper.RunAsyncTask(func() {
				// Give Raft something to do so we commit more log
				// entries. These are no-ops if we're still frozen,
				// or (at most once) mildly annoying otherwise.
				log.Warningf("LL: %v", <-r.requestLeaderLease(r.store.Clock().Now()))
			})
			return util.Errorf("waiting for index %d to be unused "+
				"(still using %d)", waitForIndex, oldestInUse)
		}
		return nil
	}); err != nil {
		panic(err)
...
func (r *Replica) endCmds(cmdKeys []interface{}, ba *roachpb.BatchRequest, br *roachpb.BatchResponse, pErr *roachpb.Error) {
	...
	if r.mu.frozen {
		...
		// we get the "convenient" lock to avoid overhead. Some refactoring
		// should move this to a "better" location a few stack frames up.
		// In particular, we should be able to return an error from this
		// trigger.
		// Deferred to run outside of the lock and after leaving the command
		// queue.
		defer frozenWaitTrigger(r, *ba, r.mu.appliedIndex)
	}
	defer r.mu.Unlock()
...
func (r *Replica) proposeRaftCommand(ctx context.Context, ba roachpb.BatchRequest) (*pendingCmd, error) {
	...
	if len(ba.Requests) == 1 {
		fReq, ok := ba.Requests[0].GetInner().(*roachpb.ChangeFrozenRequest)
		if ok {
			info := build.GetInfo()
			if info.Tag != fReq.MustVersion && fReq.Frozen {
				return nil, util.Errorf("cannot freeze running version %s from "+
					"version %s", info.Tag, fReq.MustVersion)
			}
		}
	}
