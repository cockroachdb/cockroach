...
func (r *Replica) requestLeaderLease(timestamp roachpb.Timestamp) *roachpb.Error {
	...
	if replica == nil {
		return roachpb.NewError(roachpb.NewRangeNotFoundError(desc.RangeID))
	}
...
func (r *Replica) Send(ctx context.Context, ba roachpb.BatchRequest) (*roachpb.BatchResponse, *roachpb.Error) {
	...
	if err != nil {
		if _, ok := err.GoError().(*roachpb.RaftGroupDeletedError); ok {
			...
			// clients will retry.
			err = roachpb.NewError(roachpb.NewRangeNotFoundError(r.Desc().RangeID))
		}
	}
...
func (r *Replica) handleSkippedIntents(intents []intentsWithArg) {
	...
	for _, item := range intents {
		...
		stopper.RunAsyncTask(func() {
			...
			if wiErr, ok := err.GoError().(*roachpb.WriteIntentError); !ok || wiErr == nil || !wiErr.Resolved {
...
func (r *Replica) maybeSetCorrupt(err *roachpb.Error) *roachpb.Error {
	if err != nil {
		if _, ok := err.GoError().(*roachpb.ReplicaCorruptionError); ok {
			cErr := &roachpb.ReplicaCorruptionError{}
