...
// withRaftGroupLocked ensures that the Raft Group is initialized. It assumes
...
// withRaftGroup ensures that the Raft Group is initialized, and then calls
// the given function with the initialized Raft Group. If the Raft Group fails
// to initialize, or the function returns an error, that error is returned.
// withRaftGroup acquires and releases the r.mu lock.
...
func (r *Replica) setReplicaIDLocked(replicaID roachpb.ReplicaID) error {
	...
	// When raftGroupLocked() is called, it will initialize the raftGroup if
	// r.mu.internalRaftGroup is nil
	r.mu.internalRaftGroup = nil

	// If there was a previous replica, repropose any of its pending commands
	// under this new incarnation.
...
// if the Raft group has not been brought up yet.
