...
// raftGroupLocked returns the RaftGroup owned by this replica. It ensures that
// the raft group is initialized. It assumes that the replica lock has already
// been acquired.
...
func (r *Replica) setReplicaIDLocked(replicaID roachpb.ReplicaID) error {
	...
	// raftGroup will be lazily initialized later.
	r.mu.raftGroup = nil

	// If there was a previous replica, we need to initialize the new raft
	// group in order to repropose commands that were pending from the previous
	// raft group.
