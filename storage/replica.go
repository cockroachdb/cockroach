...
type Replica struct {
	...
	mu struct {
		...
		pendingCmds    map[storage_util.CmdIDKey]*pendingCmd
...
func (r *Replica) newReplicaInner(desc *roachpb.RangeDescriptor, clock *hlc.Clock, replicaID roachpb.ReplicaID) error {
	...
	r.mu.pendingCmds = map[storage_util.CmdIDKey]*pendingCmd{}
...
func (r *Replica) addReadOnlyCmd(ctx context.Context, ba roachpb.BatchRequest) (br *roachpb.BatchResponse, pErr *roachpb.Error) {
	...
	br, intents, pErr = r.executeBatch(ctx, storage_util.CmdIDKey(""), r.store.Engine(), nil, ba)
...
func (r *Replica) tryAbandon(idKey storage_util.CmdIDKey) bool {
	r.mu.Lock()
...
func (r *Replica) proposeRaftCommand(ctx context.Context, ba roachpb.BatchRequest) (*pendingCmd, error) {
	...
	idKey := storage_util.CmdIDKey(idKeyBuf)
...
func (r *Replica) proposePendingCmdLocked(idKey storage_util.CmdIDKey, p *pendingCmd) error {
...
func (r *Replica) handleRaftReady() error {
	...
	for _, e := range rd.CommittedEntries {
		...
		case raftpb.EntryNormal:
			...
			_ = r.processRaftCommand(storage_util.CmdIDKey(commandID), e.Index, command)
		...
		case raftpb.EntryConfChange:
			...
			if err := r.processRaftCommand(storage_util.CmdIDKey(ctx.CommandID), e.Index, command); err != nil {
...
func (r *Replica) processRaftCommand(idKey storage_util.CmdIDKey, index uint64, raftCmd roachpb.RaftCommand) *roachpb.Error {
	if index == 0 {
...
func (r *Replica) applyRaftCommand(idKey storage_util.CmdIDKey, ctx context.Context, index uint64,
...
func (r *Replica) applyRaftCommandInBatch(
	ctx context.Context, idKey storage_util.CmdIDKey, originReplica roachpb.ReplicaDescriptor,
...
func (r *Replica) executeBatch(
	ctx context.Context, idKey storage_util.CmdIDKey,
	...
	*roachpb.BatchResponse, []intentsWithArg, *roachpb.Error) {
	...
	for index, union := range ba.Requests {
		...
		reply, curIntents, pErr := r.executeCmd(
			ctx, storage_util.RaftCmdIDAndIndex{IDKey: idKey, Index: index},
			batch, ms, header, remScanResults, args)
...
func (r *Replica) loadSystemConfigSpan() ([]roachpb.KeyValue, []byte, error) {
	...
	br, intents, pErr :=
		r.executeBatch(r.context(), storage_util.CmdIDKey(""), r.store.Engine(), nil, ba)
	if pErr != nil {
