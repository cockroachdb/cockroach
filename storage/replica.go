		...
		stopper.RunAsyncTask(func() {
			...
			if args.Method() == roachpb.EndTransaction {
				...
				}
				{
					...
					for _, kv := range kvs {
						gcKeys = append(gcKeys, roachpb.GCRequest_GCKey{Key: kv.Key})
					}
				}
...
// intents are resolved asynchronously in a batch. If sync is true, all
...
func (r *Replica) resolveIntents(ctx context.Context, intents []roachpb.Intent, wait bool) error {
	...
	if len(baLocal.Requests) > 0 {
		...
		wg.Add(1)
		if wait || !r.store.Stopper().RunAsyncTask(action) {
		...
		action := func() {
			...
			if err := r.store.DB().Run(b); err != nil {
				if wait {
					...
					log.Warningf("unable to resolve external intent: %s", err)
				}
			}
		}
		if wait || !r.store.Stopper().RunAsyncTask(action) {
