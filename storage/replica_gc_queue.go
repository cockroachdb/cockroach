...
// in the past.
func (*replicaGCQueue) shouldQueue(now hlc.Timestamp, rp ReplicaPromise, _ config.SystemConfig) (bool, float64) {
...
func (q *replicaGCQueue) process(
	...
	if _, currentMember := replyDesc.GetReplicaDescriptor(q.store.StoreID()); !currentMember {
		...
		if err := q.store.RemoveReplica(
			rp, replyDesc, true /* destroy */, false, /* !raftMuHeld */
		); err != nil {
			return err
		}
		...
		if err := q.store.RemoveReplica(
			rp, replyDesc, false /* !destroy */, false, /* !raftMuHeld */
		); err != nil {
			return err
		}
