...
func replicaGCShouldQueueImpl(
	...
	// check interval in the past. Note that we don't do this is the replica
	// is in candidate state, in which case we want to be more aggressive -
	// a failed rebalance attempt could have checked this range, and candidate
	// state suggests that a retry succeeded. See #7489.
	if !isCandidate && now.Less(lastCheck.Add(ReplicaGCQueueInactivityThreshold.Nanoseconds(), 0)) {
		return false, 0
	}
