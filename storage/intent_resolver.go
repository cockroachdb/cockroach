	...
	for _, item := range intents {
		if item.args.Method() != roachpb.EndTransaction {
			stopper.RunLimitedAsyncTask(ir.sem, func() {
				...
				// the primary way our Push can only lead to aborting intents
				// is that of the transaction having timed out (and thus
				// presumably no client being around any more, though at the
				// time of writing we don't guarantee that). But there's
				// another path in which the Push comes back successful,
				// namely that of the transaction already having been aborted
				// by someone else, in which case the client may still be
				// running. Thus, we must poison.
			...
			stopper.RunLimitedAsyncTask(ir.sem, func() {
				...
				// the client makes no assumptions about the true result. For
