	...
	for _, item := range intents {
		if item.args.Method() != roachpb.EndTransaction {
			stopper.RunLimitedAsyncTask(ir.sem, func() {
				...
				if err := ir.resolveIntents(ctxWithTimeout, r, resolverID, resolveIntents,
			...
			stopper.RunLimitedAsyncTask(ir.sem, func() {
				...
				if err := ir.resolveIntents(ctxWithTimeout, r, resolverID, item.intents,
					true /* wait */, false /* TODO(tschottdorf): #5088 */); err != nil {
					log.Warningc(ctxWithTimeout, "failed to resolve intents: %s", err)
					return
				}
...
func (ir *intentResolver) resolveIntents(ctx context.Context, r *Replica,
	resolverID *uuid.UUID, intents []roachpb.Intent, wait bool, poison bool) error {
