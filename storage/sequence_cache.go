...
func (sc *SequenceCache) Put(e engine.Engine, id []byte, epoch, seq uint32, txnKey roachpb.Key, txnTS roachpb.Timestamp, err *roachpb.Error) error {
	if seq <= 0 || len(id) == 0 {
...
func (sc *SequenceCache) shouldCacheError(err *roachpb.Error) bool {
