...
func (rc *SequenceCache) CopyInto(e engine.Engine, destRangeID roachpb.RangeID) error {
	...
	return e.Iterate(start, end, func(kv engine.MVCCKeyValue) (bool, error) {
		...
		// write it to the corresponding key in the new span.
...
// span entries return an error. The copy is done directly using the
...
// span in the hopes of retrying to a successful outcome.
