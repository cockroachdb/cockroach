...
func decodeSequenceCacheKey(key roachpb.Key, dest []byte) ([]byte, uint32, uint32, error) {
	...
	b, _, err := encoding.DecodeUvarint(b, encoding.Ascending)
	if err != nil {
		return nil, 0, 0, err
	}
	...
	// Decode the id.
	b, id, err := encoding.DecodeBytes(b, dest, encoding.Ascending)
	if err != nil {
		return nil, 0, 0, err
	}
	// Decode the epoch.
	...
	b, seq, err := encoding.DecodeUint32(b, encoding.Descending)
	if err != nil {
		return nil, 0, 0, err
	}
	if len(b) > 0 {
