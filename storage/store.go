...
func NewStore(ctx StoreContext, eng engine.Engine, nodeDesc *roachpb.NodeDescriptor) *Store {
	...
	s.scheduler = newScheduler(ctx.Ctx, s, storeReplicaRaftReadyConcurrency)
...
func (s *Store) HandleRaftRequest(
	_ context.Context, // TODO(peter): remove?
...
func (s *Store) handleRaftRequest(req *RaftMessageRequest) (pErr *roachpb.Error) {
	ctx := s.Ctx()
...
func (s *Store) processRaftRequest(rangeID roachpb.RangeID) {
	s.mu.Lock()
	...
	for _, req := range q {
		if pErr := s.handleRaftRequest(req); pErr != nil {
