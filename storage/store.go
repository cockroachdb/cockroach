...
type raftRequestInfo struct {
	...
	errStream MultiRaft_RaftMessageServer
}
...
func (s *Store) processRaftRequestQueue(rangeID roachpb.RangeID) {
	...
	for _, info := range q {
		if pErr := s.processRaftRequest(info.errStream.Context(), info.req); pErr != nil {
			if err := info.errStream.Send(newRaftMessageResponse(info.req, pErr)); err != nil {
				// Seems excessive to log this on every occurrence as the other side
				// might have closed.
				if log.V(1) {
					log.Infof(s.Ctx(), "%s", errors.Wrapf(err, "error sending error"))
				}
			}
		}
	}
