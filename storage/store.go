...
func (e rangeAlreadyExists) Error() string {
	return fmt.Sprintf("KeyRange %s already exists on store", e.rng)
...
func (s *Store) removePlaceholderLocked(rngID roachpb.RangeID) error {
	...
	if !ok {
		log.Infof(context.Background(), "store %s: Placeholder contains: %v", s, s.mu.replicaPlaceholders)
...
func (s *Store) handleRaftMessage(req *RaftMessageRequest) error {
	...
	case raftpb.MsgSnap:
		earlyTerm, maybeError := func() (bool, error) {
			s.mu.Lock()
			defer s.mu.Unlock()
			canApplySnapshot, placeholder := s.canApplySnapshotLocked(req.RangeID, req.Message.Snapshot)
			...
			if placeholder != nil {
				s.mu.replicaPlaceholders[req.RangeID] = placeholder
				if exRng := s.mu.replicasByKey.ReplaceOrInsert(placeholder); exRng != nil {
					delete(s.mu.replicaPlaceholders, placeholder.RangeID)
					return true, errors.Errorf("range for RangeID %v already exists in replicasByKey btree, when canApplySnapshotLocked returned true",
						(exRng.(KeyRange)).endKey())
				}
			}
			return false, nil
		}()
	...
	if req.ToReplica.ReplicaID == 0 {
		if req.Message.Type == raftpb.MsgSnap {
			...
			//s.mu.Lock()
			// TODO(arjun): Is this unlocked application safe?
...
// canApplySnapshotLocked returns true if the snapshot can be applied to
// this store's replica (i.e. the snapshot is not from an older incarnation of
// the replica) and the store could successfully add a placeholder to its
// replicasByKey map (if necessary).
func (s *Store) canApplySnapshotLocked(rangeID roachpb.RangeID, snap raftpb.Snapshot) (bool, *ReplicaPlaceholder) {
	...
	if err := parsedSnap.Unmarshal(snap.Data); err != nil {
		log.Infof(context.TODO(), "canApplySnapshotLocked: could not unmarshal snapshot: %s", err)
		return false, nil
	}

	if exRng, ok := s.mu.replicaPlaceholders[rangeID]; ok {
		log.Infof(context.Background(), "%s: canApplySnapshotLocked: could not add ReplicaPlaceholder: %s", s, rangeAlreadyExists{exRng})
	}
	...
	if s.hasOverlappingReplicaLocked(&parsedSnap.RangeDescriptor) {
		...
		log.Infof(context.Background(), "%s: canApplySnapshotLocked: cannot apply snapshot, received snapshot for an overlapping range", s)
		return false, nil
	}
