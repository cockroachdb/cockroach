...
func (s *Store) HandleRaftRequest(
	...
	if len(req.Heartbeats) > 0 {
		for _, beat := range req.Heartbeats {
			...
			// (req.Heartbeats) > 0, and this recursive call
			// back into HandleRaftRequest has len(...) == 0.
			if err := s.HandleRaftRequest(ctx, beatReq, respStream); err != nil {
				log.Errorf(s.Ctx(), "could not send coalescedHeartbeat %s", err)
			}
		}
		return nil
	}
	...
	if len(req.HeartbeatResps) > 0 {
		for _, resp := range req.HeartbeatResps {
			msg := raftpb.Message{
				Type:   raftpb.MsgHeartbeatResp,
				From:   uint64(resp.FromReplicaID),
				To:     uint64(resp.ToReplicaID),
				Term:   resp.Term,
				Commit: resp.Commit,
			}
			respReq := &RaftMessageRequest{
				RangeID: resp.RangeID,
				FromReplica: roachpb.ReplicaDescriptor{
					NodeID:    req.FromReplica.NodeID,
					StoreID:   req.FromReplica.StoreID,
					ReplicaID: resp.FromReplicaID,
				},
				ToReplica: roachpb.ReplicaDescriptor{
					NodeID:    req.ToReplica.NodeID,
					StoreID:   req.ToReplica.StoreID,
					ReplicaID: resp.ToReplicaID,
				},
				Message: msg,
			}
			// This recursion is safe as long as it is called only on len
			// (req.HeartbeatResps) > 0, and this
			// recursive call back into HandleRaftRequest has len(...) == 0.
			if err := s.HandleRaftRequest(ctx, respReq, respStream); err != nil {
				log.Errorf(ctx, "could not send coalescedHeartbeat %s", err)
			}
		}
		return nil
	}
...
// sendQueuedHeartbeatsToNode requires that the s.coalescedMu lock is held.
func (s *Store) sendQueuedHeartbeatsToNode(
	...
	return accum + len(beats) + len(resps)
}
...
func (s *Store) sendQueuedHeartbeatsLocked() {
	...
	for to, beats := range s.coalescedMu.heartbeats {
		beatsSent = s.sendQueuedHeartbeatsToNode(beats, nil, to, beatsSent)
	}
	for to, resps := range s.coalescedMu.heartbeatResponses {
		beatsSent = s.sendQueuedHeartbeatsToNode(nil, resps, to, beatsSent)
	}
	s.coalescedMu.heartbeats = map[roachpb.StoreIdent][]RaftHeartbeat{}
	s.coalescedMu.heartbeatResponses = map[roachpb.StoreIdent][]RaftHeartbeat{}
