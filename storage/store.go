...
func splitTriggerPostCommit(
	...
	// After a split, the newly create right-hand side range needs to be enqueued
	// for a Raft update check which will cause the creation of the Replica's
	// Raft group.
	...
	s.stopper.RunWorker(func() {
		...
		for {
			...
			case <-ticker.C:
				...
				s.processRaftMu.Lock()
				s.mu.Lock()
				pending := make([]roachpb.RangeID, 0, len(s.mu.replicas))
