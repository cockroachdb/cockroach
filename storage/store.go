...
func newStoreShuffledRangeSet(store *Store) *storeRangeSet {
	return &storeRangeSet{
		store:   store,
		visited: -1,
		shuffle: true,
...
func NewStore(ctx StoreContext, eng engine.Engine, nodeDesc *roachpb.NodeDescriptor) *Store {
	...
	if s.ctx.Gossip != nil {
		...
		s.scanner = newReplicaScanner(
			ctx.Ctx, ctx.ScanInterval, ctx.ScanMaxIdleTime, newStoreShuffledRangeSet(s),
		)
		...
		s.consistencyScanner = newReplicaScanner(
			ctx.Ctx, ctx.ConsistencyCheckInterval, 0, newStoreShuffledRangeSet(s),
		)
