...
// TestingChecksumMismatch turns on a panic on VerifyChecksum() seeing
// checksums that match. The default value is false, when VerifyChecksum
// panics when the checksums don't match.
func (s *Store) TestingChecksumMismatch(enable bool) {
	visitor := newStoreRangeSet(s)
	visitor.Visit(func(r *Replica) bool {
		r.testingChecksumMismatch = enable
		return true
	})
}
