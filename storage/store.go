	...
	err = IterateRangeDescriptors(ctx, s.engine,
		func(desc roachpb.RangeDescriptor) (bool, error) {
			...
			if _, ok := desc.GetReplicaDescriptor(s.StoreID()); !ok {
				rp, err := s.getReplicaLocked(desc.RangeID)
				if err != nil {
					log.Fatalf(
						ctx, "cannot retrieve inserted Replica %v: %s", desc, err,
					)
				}
				...
				if added, err := s.replicaGCQueue.Add(
					rp, replicaGCPriorityRemoved,
				); err != nil {
					log.Errorf(ctx, "%s: unable to add replica to GC queue: %s", rp, err)
				} else if added {
					log.Infof(ctx, "%s: added to replica GC queue", rp)
				}
			}
...
func splitTriggerPostCommit(
	...
	if err := r.store.SplitRange(r.AsRef(), rightRng.AsRef()); err != nil {
...
// of a Raft command.
func (s *Store) SplitRange(origRP, newRef ReplicaRef) error {
...
func (s *Store) removeReplicaImpl(rep *Replica, origDesc roachpb.RangeDescriptor, destroyData bool) error {
	...
	// read lock.
	_ = s.stopper.RunAsyncTask(context.TODO(), func(ctx context.Context) {
