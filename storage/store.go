...
type Store struct {
	...
	// processing. Reads are processed solely through Store.Send; writes
	// start out on Store.Send until they propose their raft command and
	// then they finish on the raft goroutines.
	//
	...
	// that everything is finished before releasing it.
	//
	...
	//   and released briefly as needed. This lock should be held when
	//   calling any methods on the raft group. Raft may call back into
	//   the Replica via the methods of the raft.Storage interface,
	//   which assume the lock is held even though they do not follow
	//   our convention of the "Locked" suffix.
	//
