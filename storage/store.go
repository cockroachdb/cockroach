...
type storeMetrics struct {
	...
	raftSelectDuration  *metric.Counter
	raftWorkingDuration *metric.Counter
	raftTickingDuration *metric.Counter
...
func newStoreMetrics() *storeMetrics {
	...
	return &storeMetrics{
		...
		// Raft processing metrics.
		raftSelectDuration:  storeRegistry.Counter("process-raft.waiting"),
		raftWorkingDuration: storeRegistry.Counter("process-raft.working"),
		raftTickingDuration: storeRegistry.Counter("process-raft.ticking"),
	}
}
...
func (s *Store) processRaft() {
	...
	s.stopper.RunWorker(func() {
		...
		for {
			...
			s.metrics.raftWorkingDuration.Inc(timeutil.Since(workingStart).Nanoseconds())

			selectStart := timeutil.Now()

			select {
			case <-s.wakeRaftLoop:
				s.metrics.raftSelectDuration.Inc(timeutil.Since(selectStart).Nanoseconds())
			...
			case <-ticker.C:
				...
				s.mu.Unlock()
				s.processRaftMu.Unlock()
				s.metrics.raftTickingDuration.Inc(timeutil.Since(tickerStart).Nanoseconds())
