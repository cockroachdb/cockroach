...
type Store struct {
	...
	mu struct {
		...
		// Replicas.raftMu to be held while a replica is being inserted into
		// Store.mu.replicas.
		replicaQueues map[roachpb.RangeID][]*RaftMessageRequest
	}
...
func NewStore(ctx StoreContext, eng engine.Engine, nodeDesc *roachpb.NodeDescriptor) *Store {
	...
	s.scheduler = newScheduler(ctx.Ctx, s, storeSchedulerConcurrency)

	s.mu.Lock()
	s.mu.replicas = map[roachpb.RangeID]*Replica{}
	s.mu.replicaPlaceholders = map[roachpb.RangeID]*ReplicaPlaceholder{}
	s.mu.replicaQueues = map[roachpb.RangeID][]*RaftMessageRequest{}
...
func (s *Store) HandleRaftRequest(
	...
	if len(q) >= replicaRequestQueueSize {
		...
		//
		// TODO(peter): Add a metric for how often we drop messages here.
		return nil
	}
	s.mu.replicaQueues[req.RangeID] = append(q, req)
...
func (s *Store) processRaftRequestQueue(rangeID roachpb.RangeID) {
	s.mu.Lock()
	...
	for _, req := range q {
		if pErr := s.processRaftRequest(s.Ctx(), req); pErr != nil {
			// TODO(peter): Do something with this error? Logging it could be
			// excessive. log.V(1)?
			_ = s.ctx.Transport.SendResponse(req, pErr)
		}
	}
}

func (s *Store) processRaftReady(rangeID roachpb.RangeID) {
	start := timeutil.Now()
	...
	s.metrics.RaftWorkingDurationNanos.Inc(timeutil.Since(start).Nanoseconds())
}

func (s *Store) processRaftTick(rangeID roachpb.RangeID) bool {
	start := timeutil.Now()
...
func (s *Store) processRaft() {
	...
	s.stopper.RunWorker(s.raftTickLoop)
	s.stopper.RunWorker(s.raftSnapshotStatusLoop)
}
