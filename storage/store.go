...
func splitTriggerPostCommit(
	...
	if rightRef != nil {
		...
		//
		// Need AcquireExclusive() but that's prone to livelock as there may
		// be clients already sitting on this. Must investigate this better,
		// but essentially it's always tricky to assume that you can get an
		// exclusive lock quickly.
		rightRng, release, err = rightRef.Acquire()
		if err == nil {
		...
		if err == nil {
			rightRng, release, err = rightRng.AsRef().AcquireExclusive()
		}
	}
	defer release()
	if err != nil {
...
func (s *Store) maybeSetCorrupt(
	...
	if err := setReplicaDestroyedError(ctx, r.store.Engine(), r.RangeID, roachpb.NewError(cErr)); err != nil {
		...
		// underlying storage.
		log.Fatal(ctx, "unable to persist corruption: %s", err)
	}
}
...
func (s *Store) processRaftRequest(
	...
	ref, uninitRaftLocked, err := s.getOrCreateReplica(
