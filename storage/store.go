...
func (s *Store) Send(ctx context.Context, ba roachpb.BatchRequest) (br *roachpb.BatchResponse, pErr *roachpb.Error) {
	...
	if ba.Txn != nil {
		// Reduce uncertainty restarts by attaching the above timestamp to the
		// returned response or error (if in a txn). The caller can use it to
		// shorten its uncertainty interval when it comes back to this node.
		defer func() {
			if pErr != nil {
		...
		// If transactional, we make the transaction aware that no other
		// operation that causally precedes it could have started after
		// `now`. This is important: important: If we wind up pushing a value,
		// it will be in our immediate future, and not updating the observed
		// timestamp would lead to an uncertainty restart (at least in the
		// absence of a prior observed timestamp from this node, in which case
