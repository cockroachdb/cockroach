...
func (s *Store) migrate7310And6991(desc roachpb.RangeDescriptor) {
	...
	if err != nil {
		log.Fatal(errors.Wrap(err, "could not migrate TruncatedState: %s"))
	}
	if (*state.TruncatedState == roachpb.RaftTruncatedState{}) {
		...
		if _, err := saveState(batch, state); err != nil {
			log.Fatal(errors.Wrapf(err, "could not migrate TruncatedState to %+v", &state.TruncatedState))
		}
	...
	// a snapshot here, so we could wind up lowering the commit index (which
	// would error out and fatal us).
	if hs.Commit == 0 {
		if err := synthesizeHardState(batch, state); err != nil {
			log.Fatal(errors.Wrap(err, "could not migrate HardState"))
		}
	}
