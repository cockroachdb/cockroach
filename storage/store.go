...
func sendSnapshot(
	...
	// continue
	default:
		return errors.Errorf("range=%s: server sent an invalid status during negotiation: %d",
	...
	case SnapshotResponse_APPLIED:
		return nil
	default:
		return errors.Errorf("range=%s: server sent an invalid status during finalization: %d",
			header.RangeDescriptor.RangeID, resp.Status)
	}
}
