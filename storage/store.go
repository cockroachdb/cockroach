...
func (s *Store) handleStreamingSnapshotMessage(
	ctx context.Context, req *SnapshotRequest, batches [][]byte, logEntries [][]byte) *roachpb.Error {
	s.processRaftMu.Lock()
	defer s.processRaftMu.Unlock()
	...
	placeholder, err := s.canApplySnapshotLocked(&req.Header.RangeDescriptor)
	if err != nil {
		return roachpb.NewErrorf("%s: could not apply snapshot: %s", s, err)
	}
	if placeholder != nil {
	...
	r, err = s.getOrCreateReplicaLocked(req.Header.RangeDescriptor.RangeID,
		req.Header.ToReplica.ReplicaID, req.Header.FromReplica)
	...
	r.setLastReplicaDescriptors(req.Header.FromReplica, req.Header.ToReplica)
	snapUUID, err := uuid.FromBytes(req.Header.RaftMessage.Snapshot.Data)
	...
	r.mu.inSnap = inSnap
	r.mu.Unlock()
	if req.Header.ToReplica.ReplicaID == 0 {
		...
		if req.Header.RaftMessage.Term == 0 {
			return roachpb.NewErrorf(
				"preemptive snapshot from term %d received with zero term",
				req.Header.RaftMessage.Snapshot.Metadata.Term,
			)
		}
		...
		if err := raftGroup.Step(*req.Header.RaftMessage); err != nil {
	...
	if err := r.withRaftGroup(func(raftGroup *raft.RawNode) error {
		return raftGroup.Step(*req.Header.RaftMessage)
	}); err != nil {
		return roachpb.NewError(err)
	}
