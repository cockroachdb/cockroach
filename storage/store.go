...
func (s *Store) HandleSnapshot(header *SnapshotRequest_Header, stream MultiRaft_RaftSnapshotServer,
	...
	addedPlaceholder, cleanup, err := s.prepareForSnapshotIfPossible(
		stream.Context(), &header.RangeDescriptor)
	if err != nil {
...
// prepareForSnapshotIfPossible checks to see if it's possible to apply a
// snapshot on the range. A non-nil error indicates the snapshot is
// un-applyable. If the snapshot is applyable, this method might add a
// placeholder replica to the store, indicated by addedPlaceholder.
// The cleanup function must be called if a snapshot does not end up getting
// successfully applied.
func (s *Store) prepareForSnapshotIfPossible(
	ctx context.Context, rangeDescriptor *roachpb.RangeDescriptor,
) (addedPlaceholder bool, cleanup func(), err error) {
	s.mu.Lock()
	defer s.mu.Unlock()
