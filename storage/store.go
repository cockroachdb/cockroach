		...
		if err := func() error {
			...
			if placeholder != nil {
				if exRng := s.mu.replicasByKey.ReplaceOrInsert(placeholder); exRng != nil {
					panic(errors.Errorf("range for RangeID %v already exists in replicasByKey btree, when canApplySnapshotLocked returned true",
						(exRng.(KeyRange)).endKey()))
				}
				if rng, exRng := s.mu.replicaPlaceholders[req.RangeID]; exRng {
					panic(errors.Errorf("range for RangeID %v already exists in replicaPlacholders map, when canApplySnapshotLocked returned true",
						rng.endKey()))
				}
