...
func (s *Store) Start(ctx context.Context, stopper *stop.Stopper) error {
	...
	s.ctx.Transport.Listen(s.StoreID(), s, s)
	s.processRaft()
...
// HandleSnapshot implements the RaftMessageHandler interface.
func (s *Store) HandleSnapshot(
	ctx context.Context, req *SnapshotRequest, batches [][]byte, logEntries [][]byte, addedPlaceholder bool,
) *roachpb.Error {
	s.processRaftMu.Lock()
	defer s.processRaftMu.Unlock()

	s.mu.Lock()
	r, ok := s.mu.replicas[req.Header.RangeDescriptor.RangeID]
	s.mu.Unlock()
	...
	r, err := s.getOrCreateReplicaLocked(req.Header.RangeDescriptor.RangeID,
	...
	r.setLastReplicaDescriptors(rmr.FromReplica, rmr.ToReplica)
	...
	inSnap := &IncomingSnapshot{
		SnapUUID:        *snapUUID,
		RangeDescriptor: req.Header.RangeDescriptor,
		Batches:         batches,
	...
	if rmr.ToReplica.ReplicaID == 0 {
		...
		if err := raftGroup.Step(req.Header.RaftMessageRequest.Message); err != nil {
		...
		if addedPlaceholder {
			...
			if err := s.removePlaceholderLocked(req.Header.RangeDescriptor.RangeID); err != nil {
				return roachpb.NewError(err)
			}
		}
	...
	if err := r.withRaftGroup(func(raftGroup *raft.RawNode) error {
		return raftGroup.Step(req.Header.RaftMessageRequest.Message)
	}); err != nil {
		return roachpb.NewError(err)
	}
...
func (s *Store) HandleRaftRequest(ctx context.Context, req *RaftMessageRequest) *roachpb.Error {
	...
	case raftpb.MsgSnap:
		...
		panic("Received a snapshot via the Raft endpoint: Programming Error")
	...
	r.setLastReplicaDescriptors(req.FromReplica, req.ToReplica)
...
func (s *Store) canApplySnapshotLocked(rangeDescriptor *roachpb.RangeDescriptor) (*ReplicaPlaceholder, error) {
	...
	if s.hasOverlappingKeyRangeLocked(rangeDescriptor) {
...
// PrepareForSnapshotIfPossible implements the RaftMessageHandler interface.
func (s *Store) PrepareForSnapshotIfPossible(
	ctx context.Context, rangeDescriptor *roachpb.RangeDescriptor,
) (bool, func(), error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	placeholder, err := s.canApplySnapshotLocked(rangeDescriptor)
	if err != nil {
	...
	if placeholder != nil {
		if err := s.addPlaceholderLocked(placeholder); err != nil {
			log.Fatalf(ctx, "%s: could not add placeholder %s although canApplySnapshotLocked returned true",
				s, placeholder)
		}
		cleanup = func() {
			s.mu.Lock()
			if err := s.removePlaceholderLocked(placeholder.rangeDesc.RangeID); err != nil {
				log.Warningf(ctx, "failed to cleanup placeholder: %+v", err)
			}
			s.mu.Unlock()
		}
	}
	return placeholder != nil, cleanup, nil
}
