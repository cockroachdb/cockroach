...
type storeMetrics struct {
	...
	available       *metric.Gauge // Does not include reserved bytes.
...
// DescriptorWithoutReservations returns a StoreDescriptor including current
// store capacity information without including reservations.
func (s *Store) DescriptorWithoutReservations() (*roachpb.StoreDescriptor, error) {
...
// capacity information that includes reservations.
func (s *Store) Descriptor() (*roachpb.StoreDescriptor, error) {
	desc, err := s.DescriptorWithoutReservations()
	if err != nil {
		return &roachpb.StoreDescriptor{}, err
	}
	desc.Capacity = s.bookie.AdjustCapacity(desc.Capacity)
	return desc, nil
}
