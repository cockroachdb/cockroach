...
func (s *Store) Start(stopper *stop.Stopper) error {
	...
	s.stopper.RunAsyncTask(func() {
		...
		var unfrozen int32    // updated atomically
		newStoreRangeSet(s).Visit(func(r *Replica) bool {
			...
			wg.Add(1)
			s.stopper.RunLimitedAsyncTask(sem, func() {
				defer wg.Done()
				desc := r.Desc()
				var ba roachpb.BatchRequest
				fReq := roachpb.AdminSetFrozenRequest{
					Span: roachpb.Span{
				...
				ba.Add(&fReq)
				if _, pErr := r.Send(context.Background(), ba); pErr != nil {
					log.Errorf("could not unfreeze Range %s on startup: %s", r, pErr)
				} else {
					atomic.AddInt32(&unfrozen, 1)
				}
			})
			return true
		})
