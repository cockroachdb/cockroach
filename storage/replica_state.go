...
func loadState(
	...
	if s.TxnSpanThreshold, err = loadTxnSpanThreshold(ctx, reader, desc.RangeID); err != nil {
		return storagebase.ReplicaState{}, err
	}
...
func saveState(
	...
	if err := setTxnSpanThreshold(ctx, eng, ms, rangeID, &state.TxnSpanThreshold); err != nil {
		return enginepb.MVCCStats{}, err
	}
...
func loadTxnSpanThreshold(
	ctx context.Context, reader engine.Reader, rangeID roachpb.RangeID,
) (hlc.Timestamp, error) {
	var t hlc.Timestamp
	_, err := engine.MVCCGetProto(ctx, reader, keys.RangeTxnSpanThresholdKey(rangeID),
		hlc.ZeroTimestamp, true, nil, &t)
	return t, err
}

func setTxnSpanThreshold(
	ctx context.Context,
	eng engine.ReadWriter,
	ms *enginepb.MVCCStats,
	rangeID roachpb.RangeID,
	...
	return engine.MVCCPutProto(ctx, eng, ms,
		keys.RangeTxnSpanThresholdKey(rangeID), hlc.ZeroTimestamp, nil, threshold)
}
