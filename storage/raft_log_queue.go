...
// matched. The returned value is adjusted by padding to allow retaining
...
func getQuorumMatchedIndex(raftStatus *raft.Status, padding uint64) uint64 {
	matchedIndexes := make([]uint64, 0, len(raftStatus.Progress))
	for _, progress := range raftStatus.Progress {
		matchedIndexes = append(matchedIndexes, progress.Match)
	}
	sort.Sort(uint64Slice(matchedIndexes))

	n := len(matchedIndexes)
	q := computeQuorum(n)
	index := matchedIndexes[n-q]
	if index >= padding {
		index -= padding
	} else {
		index = 0
	}
	if index < matchedIndexes[0] {
