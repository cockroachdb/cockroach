...
func getTruncatableIndexes(r *Replica) (uint64, uint64, error) {
	...
	// big. If the oldest committed index is before the first raft log entry we
	// have, the raft log is unable to catch that node up and should be truncated
	// since a snapshot will have to be sent instead.
