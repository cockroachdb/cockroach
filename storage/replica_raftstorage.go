...
const (
	...
	raftCommandHasSplitBit          = 1 << 7
	raftCommandHasSplitMask         = raftCommandHasSplitBit - 1
)
...
func raftCommandHasSplit(data []byte) bool {
	return len(data) > 0 && (data[0]&raftCommandHasSplitBit) != 0
}
...
func DecodeRaftCommand(data []byte) (commandID string, command []byte) {
	if data[0]&raftCommandHasSplitMask != raftCommandEncodingVersion {
