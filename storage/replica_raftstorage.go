...
func (r *Replica) applySnapshot(
	...
	// Clear the range using a distinct batch. Note that we clear keys here
	// that are potentially overwritten below, which violates the spirit of
	// the distinct batch. This is safe because we don't do any reads until
	// after the distinct batch is closed.
