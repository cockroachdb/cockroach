...
// encode a commandId, an encoded roachpb.RaftCommand and whether the
// command. The hasSplit parameter indicates whether the command contains an
// EndTransaction containing a split trigger. We store this info in the first
// byte of the encoded data so that we can quickly determine whether a raft
// command contains a split before decoding the command. Splits require
