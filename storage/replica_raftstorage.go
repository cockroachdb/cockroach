...
func (r *Replica) GetSnapshot(ctx context.Context) (*OutgoingSnapshot, error) {
	for i := 0; ; i++ {
		log.Tracef(ctx, "snapshot retry loop pass %d", i)
...
// created, it needs to be closed via CloseOutSnap() to prevent resource leakage.
...
type IncomingSnapshot struct {
	...
	// If true, indicates that this snapshot has had a placeholder added.
	addedPlaceholder bool

	// If true, indicates that the placeholder for this snapshot must still
	// be removed.
