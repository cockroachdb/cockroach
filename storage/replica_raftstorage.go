...
func snapshot(
	...
	var ret OutgoingSnapshot
	// Intentionally let this iterator and snapshot escape so that the streamer can send
	// chunks from it bit by bit.
	ret.Snap = snap
	ret.Iter = NewReplicaDataIterator(&desc, snap, true /* replicatedOnly */)
	ret.LogEntries = snapData.LogEntries
	ret.SnapUUID = *uuid.NewV4()

	ret.Snapshot = &raftpb.Snapshot{
		Data: ret.SnapUUID.GetBytes(),
	...
	log.Infof(ctx, "generated snapshot %s for range %s at index %d in %s. %d log entries",
		ret.SnapUUID, rangeID, appliedIndex, timeutil.Since(start), len(snapData.LogEntries))
	return ret, nil
}
...
func (r *Replica) applySnapshot(
	...
	if !raft.IsEmptyHardState(hs) {
		if err := setHardState(ctx, batch, r.RangeID, hs); err != nil {
