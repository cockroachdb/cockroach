...
func (r *Replica) applySnapshot(snap raftpb.Snapshot) (uint64, error) {
	...
	if replicaID == 0 {
		...
		if err != nil {
			return 0, errors.Wrap(err, "unable to write initial state")
		}
		if err := setMVCCStats(batch, r.RangeID, ms); err != nil {
			return 0, errors.Wrap(err, "unable to write MVCC stats")
		}
		if s, err = loadState(batch, &desc); err != nil {
			return 0, err
		}
	}
