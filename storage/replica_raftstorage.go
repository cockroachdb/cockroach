...
func (r *Replica) append(batch engine.ReadWriter, prevLastIndex uint64, prevRaftLogSize int64, entries []raftpb.Entry) (uint64, int64, error) {
	...
	for i := range entries {
		...
		raftLogSize += int64((&roachpb.KeyValue{Key: key, Value: value}).Size())

		if err := engine.MVCCPut(ctx, batch, nil /* ms */, key, hlc.ZeroTimestamp, value, nil /* txn */); err != nil {
			return 0, 0, err
		}
	}
	...
	for i := lastIndex + 1; i <= prevLastIndex; i++ {
		key := keys.RaftLogKey(r.RangeID, i)
		value, _, err := engine.MVCCGet(ctx, batch, key, hlc.ZeroTimestamp, true /* consistent */, nil /* txn */)
		if err != nil {
			return 0, 0, err
		}
		raftLogSize -= int64((&roachpb.KeyValue{Key: key, Value: *value}).Size())
