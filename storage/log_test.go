...
func TestLogSplits(t *testing.T) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&rangeID, &otherRangeID, &info); err != nil {
			t.Fatal(err)
		}
		...
		if err := json.Unmarshal([]byte(info.String), &infoStruct); err != nil {
			t.Errorf("error unmarshalling info string for split of range %d", rangeID)
			continue
		}
		if int64(infoStruct.UpdatedDesc.RangeID) != rangeID {
			t.Errorf("recorded wrong updated descriptor %s for split of range %d", infoStruct.UpdatedDesc.String(), rangeID)
		}
		if int64(infoStruct.NewDesc.RangeID) != otherRangeID.Int64 {
			t.Errorf("recorded wrong new descriptor %s for split of range %d", infoStruct.NewDesc.String(), rangeID)
		}
	}
	if rows.Err() != nil {
		t.Fatal(rows.Err())
