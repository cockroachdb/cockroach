...
func TestLogRebalances(t *testing.T) {
	...
	for rows.Next() {
		...
		if a, e := info.AddReplica, desc.Replicas[0]; a != e {
			t.Errorf("recorded wrong updated replica %v for add replica of range %d, expected %v",
				a, rangeID, e)
		}
	}
	if rows.Err() != nil {
	...
	for rows.Next() {
		...
		if a, e := info.RemovedReplica, desc.Replicas[0]; a != e {
			t.Errorf("recorded wrong updated replica %v for remove replica of range %d, expected %v",
				a, rangeID, e)
		}
	}
	if rows.Err() != nil {
