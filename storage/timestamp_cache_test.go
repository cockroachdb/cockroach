...
func TestTimestampCacheEviction(t *testing.T) {
	...
	if !tc.GetMax(engine.Key("c"), nil, "").Equal(aTS) {
		t.Errorf("expected low water mark %+v, got %+v", aTS, tc.GetMax(engine.Key("c"), nil, ""))
	}
}
...
func TestTimestampCacheWithTxnMD5(t *testing.T) {
	...
	ts1 := clock.Now()
	tc.Add(engine.Key("a"), nil, ts1, "")

	// Add two successive txn entries.
	ts2 := clock.Now()
	tc.Add(engine.Key("a"), nil, ts2, "1")
	ts3 := clock.Now()
	tc.Add(engine.Key("a"), nil, ts3, "2")

	// Fetching with no transaction gets latest value.
	if ts := tc.GetMax(engine.Key("a"), nil, ""); !ts.Equal(ts3) {
		t.Errorf("expected %s; got %s", ts3, ts)
	}
	// Fetching with txn MD5 "1" gets most recent.
	if ts := tc.GetMax(engine.Key("a"), nil, "1"); !ts.Equal(ts3) {
		t.Errorf("expected %s; got %s", ts3, ts)
	}
	// Fetching with txn MD5 "2" skips most recent.
	if ts := tc.GetMax(engine.Key("a"), nil, "2"); !ts.Equal(ts2) {
