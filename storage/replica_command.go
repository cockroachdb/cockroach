...
func (r *Replica) AdminSplit(
	...
	if err := r.store.DB().Txn(context.TODO(), func(txn *client.Txn) error {
		...
		// side of the split first to locate the txn record there.
		{
			...
			// This prevents a race where GC encounters intents without a txn
			// record and deletes them, and then allows the begin txn to
			// recreate the txn and continue (see #9265).
		...
		// TODO(spencer): event logging API should accept a vanilla DB
...
func (r *Replica) ChangeReplicas(
	...
	if err := r.store.DB().Txn(context.TODO(), func(txn *client.Txn) error {
...

		{
			...
			// Run transaction up to this point (see #9265).
			if err := txn.Run(b); err != nil {
				return err
			}
		}
