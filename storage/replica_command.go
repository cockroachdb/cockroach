...
// remScanResults stores the number of scan results remaining for this batch (MaxInt64 for no
// limit).
...
func scanMaxResultsValue(remScanResults int64, scanMaxResults int64) int64 {
	if remScanResults != math.MaxInt64 && (scanMaxResults == 0 || scanMaxResults < remScanResults) {
		return remScanResults
	}
	return scanMaxResults
}

// updateRemScanResults updates the remScanResults value after a scan returned results.
func updateRemScanResults(remScanResults *int64, returnedResults int) {
	if *remScanResults != math.MaxInt64 {
		if *remScanResults < int64(returnedResults) {
			panic(fmt.Sprintf("scan returned more results than requested (%d vs %d)",
				returnedResults, *remScanResults))
		}
		*remScanResults -= int64(returnedResults)
	}
}
...
func (r *Replica) Scan(batch engine.Engine, h roachpb.Header, remScanResults *int64,
	...
	updateRemScanResults(remScanResults, len(rows))
