...
func (r *Replica) EndTransaction(batch engine.Engine, ms *engine.MVCCStats, h roachpb.Header, args roachpb.EndTransactionRequest) (roachpb.EndTransactionResponse, []roachpb.Intent, error) {
	...
	for _, span := range args.IntentSpans {
		if err := func() error {
			intent := roachpb.Intent{Span: span, Txn: reply.Txn.Meta, Status: reply.Txn.Status}
...
func (r *Replica) PushTxn(batch engine.Engine, ms *engine.MVCCStats, h roachpb.Header, args roachpb.PushTxnRequest) (roachpb.PushTxnResponse, error) {
	...
	if !ok {
		...
		reply.PusheeTxn.Meta = args.PusheeTxn
...
func (r *Replica) maybePoison(batch engine.Engine, shouldPoison bool, txn roachpb.Meta, status roachpb.TransactionStatus) error {
	if !shouldPoison {
		return nil
	}
	var poison uint32
	...
	case roachpb.PENDING:
		...
		// Previous code here poisoned unless the transaction was either
		// SERIALIZABLE and with it's original timestamp or SNAPSHOT.
		// This hasn't been possible since we factored out the transaction
		// metadata from the transaction proto, since now we only have the
		// **metadata** available at the callsite.
		// With a bit more elbow grease, it should be possible to thread
		// the necessary information through ResolveIntent{,Range} to here,
		// at least in cases where the performance matters (short-circuiting
		// a Transaction helps avoid redundant work).
		return nil
	default:
		return nil
	}
...
func (r *Replica) ResolveIntent(batch engine.Engine, ms *engine.MVCCStats, h roachpb.Header, args roachpb.ResolveIntentRequest) (roachpb.ResolveIntentResponse, error) {
	...
	intent.Span.Key = args.Key
	intent.Txn = args.IntentTxn
	intent.Status = args.Status
