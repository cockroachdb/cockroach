...
func (r *Replica) LeaseInfo(
	...
	if nextLease != nil {
		...
		// lease.
		reply.Replica = nextLease.Replica
		return reply, nil
	}
	if lease.Covers(r.store.Clock().Now()) {
		reply.Replica = lease.Replica
		return reply, nil
	}
	if pErr := r.redirectOnOrAcquireLease(ctx); pErr != nil {
		if nlhe, ok := pErr.GetDetail().(*roachpb.NotLeaseHolderError); ok {
			// If we got information about the lease holder, return it.
			// Otherwise, a NotLeaseHolderError with an unknown LeaseHolder will be
			// propagated to the client.
			if nlhe.LeaseHolder != nil {
				reply.Replica = *nlhe.LeaseHolder
				return reply, nil
			}
		}
