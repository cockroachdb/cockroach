			...
			if diff != nil {
				for _, d := range diff {
					...
					log.Errorf("inconsistent k:v = (%s (%x), %s, %x) not present on %s",
						keys.PrettyPrint(roachpb.Key(d.Key)), d.Key, d.Timestamp, d.Value, l)
				}
			}
	...
	for {
		...
		if e.Key == nil {
			...
			if v.Key == nil {
				...
				log.Infof("comp returns %d", comp)
			}
		}
		switch comp {
		case -1:
			log.Info("key less than curr")
			addLeader()

		case 0:
			if !e.Timestamp.Equal(v.Timestamp) {
				if e.Timestamp.Less(v.Timestamp) {
					log.Info("timestamp less than curr")
					addLeader()
				} else {
					addReplica()
				}
			} else if !bytes.Equal(e.Value, v.Value) {
				log.Info("values unequal")
				addLeader()
				addReplica()
			} else {
				// No diff; skip.
