...
// ComputeChecksum request. The command is marked as IsWrite so that
// it executes on every replica, but it actually doesn't modify
// the replica. Therefore, all errors seen here can be handled here and
// not propagated back, because an erroneous command has the same effect
// as a successful one.
...
// range.
func (r *Replica) splitTrigger(ctx context.Context, batch engine.Engine, ms *engine.MVCCStats, split *roachpb.SplitTrigger, ts roachpb.Timestamp) error {
