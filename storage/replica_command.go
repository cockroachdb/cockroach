...
func (r *Replica) executeCmd(ctx context.Context, raftCmdID storagebase.CmdIDKey,
	...
	if filter := r.store.ctx.TestingKnobs.TestingCommandFilter; filter != nil {
		...
		if pErr := filter(filterArgs); pErr != nil {
			log.Infof("range %s: test injecting error: %s", r, pErr)
			return nil, pErr
		}
	}
	...
	if log.V(2) {
		log.Infof("range %s: executed %s command %+v: %+v, err=%s", r, args.Method(), args, reply, err)
	}
...
func (r *Replica) EndTransaction(
	...
	if reply.Txn.Status == roachpb.COMMITTED {
		if err := r.runCommitTrigger(ctx, batch.(engine.Batch), ms, args, reply.Txn); err != nil {
			...
			log.Error(errors.Wrapf(err, "range %s: commit trigger", r))
			return reply, nil, err
		}
	}
...
func (r *Replica) PushTxn(
	...
	if log.V(1) && reason != "" {
		...
		log.Infof("range %s: %s "+s+" %s: %s", r, args.PusherTxn.ID.Short(), reply.PusheeTxn.ID.Short(), reason)
	}

	if !pusherWins {
		err := roachpb.NewTransactionPushError(reply.PusheeTxn)
		if log.V(1) {
			log.Infof("range %s: %v", r, err)
		}
		return reply, err
	}
...
func (r *Replica) TruncateLog(
	...
	if r.RangeID != args.RangeID {
		log.Infof("range %s: attempting to truncate raft logs for another range %d. Normally this is due to a merge and can be ignored.",
			r, args.RangeID)
		return reply, nil
	}
	...
	if firstIndex >= args.Index {
		if log.V(3) {
			log.Infof("range %s: attempting to truncate previously truncated raft log. FirstIndex:%d, TruncateFrom:%d",
...
func (r *Replica) applyNewLeaseLocked(
	...
	if prevLease.Replica.StoreID != r.mu.state.Lease.Replica.StoreID {
		...
		if r.mu.state.Lease.Replica.ReplicaID == r.mu.replicaID {
			...
			log.Infof("range %s: new range lease %s following %s [physicalTime=%s]",
			...
			if raftGroup.Status().RaftState == raft.StateLeader {
				...
				// lease.
				log.Infof("range %s: replicaID %v transfer raft leadership to replicaID %v",
...
func (r *Replica) getChecksum(id uuid.UUID) (replicaChecksum, bool) {
	...
	<-c.notify
	if log.V(1) {
		log.Infof("range %s: waited for compute checksum for %s", r, timeutil.Since(now))
	}
	r.mu.Lock()
	c, ok = r.mu.checksums[id]
	r.mu.Unlock()
...
func (r *Replica) computeChecksumDone(id uuid.UUID, sha []byte, snapshot *roachpb.RaftSnapshotData) {
	...
	if c, ok := r.mu.checksums[id]; ok {
		...
		// really bad happened.
		log.Errorf("range %s: no checksum for id = %v", r, id)
	}
}
...
func (r *Replica) ComputeChecksum(
	...
	if args.Version != replicaChecksumVersion {
		log.Errorf("range %s: Incompatible versions: e=%d, v=%d", r, replicaChecksumVersion, args.Version)
...
func (r *Replica) VerifyChecksum(
	...
	if args.Version != replicaChecksumVersion {
		log.Errorf("range %s: consistency check skipped: incompatible versions: e=%d, v=%d",
	...
	c, ok := r.getChecksum(id)
	if !ok {
		log.Errorf("range %s: consistency check skipped: checksum for id = %v doesn't exist", r, id)
	...
	if c.checksum != nil && !bytes.Equal(c.checksum, args.Checksum) {
		...
		if args.Snapshot == nil {
			...
			if err := r.store.stopper.RunAsyncTask(func() {
				log.Errorf("range %s: consistency check failed; fetching details", r)
				desc := r.Desc()
			...
			if diff != nil {
				for _, d := range diff {
					...
					log.Errorf("range %s: consistency check failed: k:v = (%s (%x), %s, %x) not present on %s",
...
func (r *Replica) ChangeFrozen(
	...
	if args.MustVersion == "" {
		...
		log.Warningf("range %s: freeze %s issued from %s is applied by %s",
...
func (r *Replica) AdminSplit(
	...
	log.Infof("range %s: initiating a split of this range at key %s", r, splitKey)
...
func (r *Replica) splitTrigger(
	...
	batch.Defer(func() {
		...
		if err := r.store.SplitRange(r, newRng); err != nil {
			...
			log.Fatalf("range %s: failed to update Store after split: %s", r, err)
		}
			...
			if err := r.store.stopper.RunAsyncTask(func() {
				...
				if err != nil {
					if _, ok := err.(*roachpb.RangeNotFoundError); ok {
						log.Infof("range %s: new replica %d removed before campaigning",
							r, r.mu.replicaID)
					} else {
						log.Infof("range %s: new replica %d unable to campaign: %s",
				...
				if err := replica.withRaftGroup(func(raftGroup *raft.RawNode) error {
					if err := raftGroup.Campaign(); err != nil {
						log.Warningf("range %s: error %v", r, err)
					}
					return nil
				}); err != nil {
					panic(err)
...
func (r *Replica) AdminMerge(
	...
	// repeat the lookup inside the transaction.
	{
		...
		log.Infof("range %s: initiating a merge of %s into this range", r, rightRng)
	}
...
func (r *Replica) mergeTrigger(
	...
	batch.Defer(func() {
		if err := r.store.MergeRange(r, merge.UpdatedDesc.EndKey, subsumedRangeID); err != nil {
			// Our in-memory state has diverged from the on-disk state.
			log.Fatalf("range %s: failed to update store after merging range: %s", r, err)
		}
	})
	return nil
}
	...
	if change.ChangeType == roachpb.REMOVE_REPLICA && r.store.StoreID() == change.Replica.StoreID {
		...
		batch.Defer(func() {
			if err := r.store.replicaGCQueue.Add(r, 1.0); err != nil {
				...
				log.Errorf("range %s: unable to add to GC queue: %s", r, err)
			}
		})
	} else {
...
func (r *Replica) ChangeReplicas(
	...
	if err := r.store.DB().Txn(func(txn *client.Txn) error {
		...
		log.Infof("range %s: change replicas of %d: read existing descriptor %+v", r, rangeID, oldDesc)
