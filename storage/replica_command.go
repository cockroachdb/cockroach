...
func (r *Replica) DeleteRange(batch engine.Engine, ms *engine.MVCCStats, h roachpb.Header, args roachpb.DeleteRangeRequest) (roachpb.DeleteRangeResponse, error) {
	...
	deleted, err := engine.MVCCDeleteRangeReturning(batch, ms, args.Key, args.EndKey, args.MaxEntriesToDelete, h.Timestamp, h.Txn)
	reply.Keys = deleted
	return reply, err
}
...
func (r *Replica) mergeTrigger(batch engine.Engine, merge *roachpb.MergeTrigger) error {
	...
	if err := engine.MVCCDeleteRange(batch, nil, localRangeKeyPrefix, localRangeKeyPrefix.PrefixEnd(), 0, roachpb.ZeroTimestamp, nil); err != nil {
