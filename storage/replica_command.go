...
func (r *Replica) EndTransaction(
	...
	// call), we can purge the local abort cache state. Why? We are
	...
	// replayed over its own resolved intents, the write will succeed but
	...
	// call because the txn timestamp will be too old. Replays which
	// include a BeginTransaction never succeed because EndTransaction
	// inserts in the write timestamp cache, forcing the BeginTransaction
	// to fail with a transaction retry error. If the replay didn't include
	// a BeginTransaction, any push will immediately succeed as a missing
	// txn record on push sets the transaction to aborted. In both
	// cases, the txn will be GCd on the slow path.
	err := r.clearAbortCache(batch, ms, reply.Txn.TxnMeta)
