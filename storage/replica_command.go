...
// performance it only computes the stats for the new range (the
...
// differ from normal requests in that it doesn't go through Raft but instead
// allows the leader Replica to act as the orchestrator for the distributed
// transaction that performs the split. As such, this request is only executed
// on the replica leader-lease and the request is redirected to the leader if
// the recipient is a follower.
//
...
// and the meta2 range addressing information. That transaction includes a
// special SplitTrigger flag on the EndTransaction request. Like all
// transactions, the requests within the transaction are replicated via Raft,
// including the end-transaction request.
//
