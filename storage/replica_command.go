...
func (r *Replica) DeleteRange(
	...
	deleted, err := engine.MVCCDeleteRange(ctx, batch, ms, args.Key, args.EndKey, int64(math.MaxInt64), h.Timestamp, h.Txn, args.ReturnKeys)
	if err == nil {
		reply.Keys = deleted
...
func (r *Replica) resolveLocalIntents(
	...
	for _, span := range args.IntentSpans {
		if err := func() error {
			...
			if inSpan != nil {
				intent.Span = *inSpan
				_, err := engine.MVCCResolveWriteIntentRangeUsingIter(ctx, batch, iterAndBuf, ms, intent, int64(math.MaxInt64))
				return err
			}
			return nil
		}(); err != nil {
...
func (r *Replica) ResolveIntentRange(
	...
	if _, err := engine.MVCCResolveWriteIntentRange(ctx, batch, ms, intent, int64(math.MaxInt64)); err != nil {
		return reply, err
	}
	if intent.Status == roachpb.ABORTED {
		return reply, r.setAbortCache(ctx, batch, ms, args.IntentTxn, args.Poison)
...
func (r *Replica) TruncateLog(
	...
	if _, err := engine.MVCCDeleteRange(ctx, batch, &diff, start, end, int64(math.MaxInt64), /* max */
...
func (r *Replica) splitTrigger(
	...
	*PostCommitTrigger,
	...
	// - node two can illegally propose a write to 'd' at a lower timestamp.
	{
		...
		if err := setLease(
			batch, &bothDeltaMS, split.RightDesc.RangeID, rightLease,
		); err != nil {
...
func (r *Replica) mergeTrigger(
	...
	*ms = enginepb.MVCCStats{}
	...
	if _, err := engine.MVCCDeleteRange(ctx, batch, nil, localRangeIDKeyPrefix, localRangeIDKeyPrefix.PrefixEnd(), int64(math.MaxInt64), hlc.ZeroTimestamp, nil, false); err != nil {
