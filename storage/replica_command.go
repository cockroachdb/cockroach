...
func (r *Replica) CheckConsistency(
	...
	if inconsistencyCount == 0 {
		...
		logFunc(r.store.Ctx(), "%s: consistency check failed with %d inconsistent replicas",
			r, inconsistencyCount)
	} else {
		if err := r.store.stopper.RunAsyncTask(func() {
			log.Errorf(r.store.Ctx(),
				"%s: consistency check failed with %d inconsistent replicas; fetching details",
				r, inconsistencyCount)
