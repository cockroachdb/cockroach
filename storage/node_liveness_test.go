...
func TestNodeLivenessRestart(t *testing.T) {
	...
	util.SucceedsSoon(t, func() error {
		...
		if !reflect.DeepEqual(keys, expKeys) {
			return errors.Errorf("expected keys %+v != keys %+v", expKeys, keys)
		}
		return nil
	})
}
...
func TestNodeLivenessSelf(t *testing.T) {
	...
	if lGet.Epoch == 2 || lSelf.NodeID == 2 {
		t.Errorf("expected GetLiveness() and Self() not to return artificially gossiped liveness", lGet, lSelf)
	}
}
