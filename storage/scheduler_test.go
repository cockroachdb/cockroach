...
func (p *testProcessor) processRaftReady(rangeID roachpb.RangeID) {
	p.mu.Lock()
	p.mu.raftReady[rangeID]++
	p.mu.Unlock()
}

func (p *testProcessor) processRaftRequestQueue(rangeID roachpb.RangeID) {
	p.mu.Lock()
	p.mu.raftRequest[rangeID]++
	p.mu.Unlock()
}

func (p *testProcessor) processRaftTick(rangeID roachpb.RangeID) bool {
	p.mu.Lock()
...
func TestSchedulerLoop(t *testing.T) {
	...
	p := newTestProcessor()
	s := newScheduler(context.Background(), p, 1)
