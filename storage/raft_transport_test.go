...
func TestSendAndReceive(t *testing.T) {
	...
	for nodeIndex := 0; nodeIndex < numNodes; nodeIndex++ {
		...
		// sending order.
		transports[nodeID].SnapshotStatusChan = make(chan storage.RaftSnapshotStatus, numNodes*storesPerNode)
		...
		for storeIndex := 0; storeIndex < storesPerNode; storeIndex++ {
			...
			channels[storeID] = newChannelServer(numNodes*storesPerNode*len(messageTypes), 0)
