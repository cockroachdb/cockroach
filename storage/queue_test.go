...
type testQueueImpl struct {
	shouldQueueFn func(hlc.Timestamp, *ReplicaRef) (bool, float64)
	processed     int32
...
func (tq *testQueueImpl) shouldQueue(now hlc.Timestamp, ref *ReplicaRef, _ config.SystemConfig) (bool, float64) {
	return tq.shouldQueueFn(now, ref)
}
...
func TestBaseQueueAddUpdateAndRemove(t *testing.T) {
	...
	r1 := createReplica(tc.store, 1001, roachpb.RKey("1001"), roachpb.RKey("1001/end")).AsRef()
	if err := tc.store.AddReplicaTest(r1); err != nil {
		t.Fatal(err)
	}
	r2 := createReplica(tc.store, 1002, roachpb.RKey("1002"), roachpb.RKey("1002/end")).AsRef()
	...
	testQueue := &testQueueImpl{
		shouldQueueFn: func(now hlc.Timestamp, ref *ReplicaRef) (shouldQueue bool, priority float64) {
			return shouldAddMap[ref], priorityMap[ref]
		},
	}
...
func TestBaseQueueAdd(t *testing.T) {
	...
	testQueue := &testQueueImpl{
		shouldQueueFn: func(now hlc.Timestamp, r *ReplicaRef) (shouldQueue bool, priority float64) {
			return false, 0.0
		},
	}
...
func TestBaseQueueProcess(t *testing.T) {
	...
	testQueue := &testQueueImpl{
		...
		shouldQueueFn: func(now hlc.Timestamp, ref *ReplicaRef) (shouldQueue bool, priority float64) {
			shouldQueue = true
			priority = float64(ref.Desc().RangeID)
			return
		},
	}
	bq := makeTestBaseQueue("test", testQueue, tc.store, tc.gossip, queueConfig{maxSize: 2})
...
func TestAcceptsUnsplitRanges(t *testing.T) {
	...
	neverSplits := createReplica(s, 2, roachpb.RKeyMin, dataMaxAddr).AsRef()
	if err := s.AddReplicaTest(neverSplits); err != nil {
		t.Fatal(err)
	}

	// This range will need to be split after user db/table entries are created.
	willSplit := createReplica(s, 3, dataMaxAddr, roachpb.RKeyMax).AsRef()
...
func TestBaseQueuePurgatory(t *testing.T) {
	...
	testQueue := &testQueueImpl{
		...
		shouldQueueFn: func(_ hlc.Timestamp, ref *ReplicaRef) (shouldQueue bool, priority float64) {
			shouldQueue = true
			priority = float64(ref.Desc().RangeID)
			return
		},
	...
	for i := 1; i <= replicaCount; i++ {
		r := createReplica(tc.store, roachpb.RangeID(i+1000),
			roachpb.RKey(fmt.Sprintf("%d", i)), roachpb.RKey(fmt.Sprintf("%d/end", i))).AsRef()
...
func TestBaseQueueProcessTimeout(t *testing.T) {
	...
	ptQueue := &processTimeoutQueueImpl{
		testQueueImpl: testQueueImpl{
			blocker: make(chan struct{}, 1),
			shouldQueueFn: func(now hlc.Timestamp, _ *ReplicaRef) (shouldQueue bool, priority float64) {
				return true, 1.0
			},
		},
	}
