...
func TestBaseQueuePurgatory(t *testing.T) {
	...
	purgatorySize := 10
	bq := makeBaseQueue("test", testQueue, g, purgatorySize)
	mc := hlc.NewManualClock(0)
	clock := hlc.NewClock(mc.UnixNano)
	bq.Start(clock, stopper)

	for i := 1; i <= purgatorySize; i++ {
	...
	util.SucceedsWithin(t, queueItemProcessTimeout, func() error {
		if pc := atomic.LoadInt32(&testQueue.processed); pc != int32(purgatorySize) {
			return fmt.Errorf("expected %d processed replicas; got %d", purgatorySize, pc)
		}
		return nil
	})
	...
	if l := len(bq.mu.purgatory); l != purgatorySize {
		t.Errorf("expected purgatory size of %d; got %d", purgatorySize, l)
	}
	...
	util.SucceedsWithin(t, queueItemProcessTimeout, func() error {
		if pc := atomic.LoadInt32(&testQueue.processed); pc != int32(purgatorySize*2) {
			return fmt.Errorf("expected %d processed replicas; got %d", purgatorySize*2, pc)
		}
		return nil
	})
	...
	util.SucceedsWithin(t, queueItemProcessTimeout, func() error {
		if pc := atomic.LoadInt32(&testQueue.processed); pc != int32(purgatorySize*3) {
			return fmt.Errorf("expected %d processed replicas; got %d", purgatorySize*3, pc)
		}
		return nil
	})
