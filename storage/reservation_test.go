...
// TestBookieReserveMaxRanges ensures that there is no over-booking on the
// number of ranges declines further reservations.
func TestBookieReserveMaxRanges(t *testing.T) {
	...
	for i := 1; i <= previousReserved; i++ {
		...
		if !b.Reserve(req).Reserved {
			t.Errorf("%d: could add reservation", i)
		}
...
// TestBookieReserveMaxBytes ensures that there is no over-booking on the total
// reserved bytes.
