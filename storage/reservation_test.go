...
func TestReserve(t *testing.T) {
	...
	if b.Reserve(overfilledReq).Reserved {
		t.Errorf("expected reservation to fail due disk space constraints, but it succeeded")
	}
	verifyBookie(t, b, 1, 13, 100) // The same numbers from the last call to verifyBookie.

	// Test rejecting a reservation due to a lack of available reservation space.
	b.mu.Lock()
	...
	if b.Reserve(overfilledReq2).Reserved {
		t.Errorf("expected reservation to fail due to a lack of available reservation space, but it succeeded")
	}
	verifyBookie(t, b, 1, 13, 100) // The same numbers from the last call to verifyBookie.
}
