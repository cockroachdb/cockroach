...
func TestReserve(t *testing.T) {
	...
	if b.Reserve(overfilledReq).Reserved {
		t.Errorf("expected reservation to fail due to too lack of available disk space, but it succeeded")
	}
	verifyBookie(t, b, 1, 13, 100) // The same numbers from the last call to verifyBookie.
	...
	if b.Reserve(overfilledReq2).Reserved {
		t.Errorf("expected reservation to fail due to too lack of available reservation space, but it succeeded")
	}
	verifyBookie(t, b, 1, 13, 100) // The same numbers from the last call to verifyBookie.
}
