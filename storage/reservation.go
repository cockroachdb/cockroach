...
// AdjustCapacity adds the reserved replica count and subtracts the reserved
// bytes from a StoreCapacity.
func (b *bookie) AdjustCapacity(cap roachpb.StoreCapacity) roachpb.StoreCapacity {
	b.mu.Lock()
	defer b.mu.Unlock()
	cap.RangeCount += int32(len(b.mu.resByRangeID))
	cap.Available -= b.mu.size
	return cap
}
