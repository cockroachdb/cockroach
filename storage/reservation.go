...
func (b *bookie) Reserve(res reservation) bool {
	...
	if oldRes, ok := b.mu.resByRangeID[res.rangeID]; ok {
		...
		if oldRes.nodeID == res.nodeID &&
			oldRes.storeID == res.storeID {
			...
			if log.V(2) {
				log.Infof("there is pre-existing reservation %v, can't updated with %v", oldRes, res)
			}
			return false
		}
	}
...
func (b *bookie) fillBookingLocked(res *booking) bool {
	if b.mu.resByRangeID[res.rangeID] != res {
		if log.V(2) {
