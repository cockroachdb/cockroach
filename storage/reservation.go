...
// bookingPQ implements the heap.Interface (which includes sort.Interface) and
// holds bookings. bookingPQ is not threadsafe.
...
// enqueue either adds the booking to the queue or updates its location in the
// priority queue.
func (pq *bookingPQ) enqueue(resTimeout *booking) {
	if resTimeout.index < 0 {
		heap.Push(pq, resTimeout)
	} else {
		heap.Fix(pq, resTimeout.index)
	}
}
...
// len returns the total number of outstanding reservations.
func (b *bookie) len() int {
	b.mu.RLock()
	defer b.mu.RUnlock()
	return len(b.mu.resByRangeID)
}

// size returns the total bytes reserved.
func (b *bookie) size() int64 {
	b.mu.RLock()
	defer b.mu.RUnlock()
	return b.mu.size
}

// reserve resreves a new replica.
func (b *bookie) reserve(res reservation) bool {
	...
	newBooking := &booking{
		...
		index:       -1, // This is required to add it to the queue.
	}
