...
func (rq *replicateQueue) shouldQueue(
	...
	if action != AllocatorNoop {
		if log.V(2) {
			log.Infof(context.TODO(), "enqueuing %s: repair needed", repl)
		}
	...
	if log.V(2) {
		if target != nil {
			log.Infof(context.TODO(), "enqueuing %s: rebalance target found", repl)
		} else {
			log.Infof(context.TODO(), "not enqueuing %s: no rebalance target found", repl)
		}
	}
	return target != nil, 0
}
