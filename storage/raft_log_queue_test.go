...
func TestGetTruncatableIndexes(t *testing.T) {
	...
	if truncatableIndexes, oldestIndex, err := getTruncatableIndexes(rngNew); err != nil || truncatableIndexes != 0 ||
		oldestIndex != 0 {
		if err != nil {
	...
	if truncatableIndexes, oldestIndex, err := getTruncatableIndexes(r); err != nil || truncatableIndexes == 0 ||
	...
	// This might be flaky, if so, we can remove this final part of the test, the functionality is tested in
	// client_raft_log_queue as well.
