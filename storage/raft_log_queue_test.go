...
func TestGetClusterCommittedIndex(t *testing.T) {
	...
	for i, c := range testCases {
		...
		clusterCommittedIndex := getClusterCommittedIndex(status)
		if c.expected != clusterCommittedIndex {
			t.Fatalf("%d: expected %d, but got %d", i, c.expected, clusterCommittedIndex)
		}
	}
}
