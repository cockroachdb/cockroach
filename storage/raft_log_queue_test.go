...
 
	...
	// it with succeedsSoon, we can't do that here. So this check may be fragile
	// in that the truncation my erroneously occur but getTruncatableIndexes
	// might not pick up that truncation right away. If this turns out to be
	// fragile, more tooling into the raft_log_queue will be required to
	// directly measure if a truncation command was sent or not.
