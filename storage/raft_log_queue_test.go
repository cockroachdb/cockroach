...
func TestGetTruncatableIndexes(t *testing.T) {
	...
	// It's not possible to use a SucceedsSoon in this case as we don't expect
	// to see a change. Failures in the previous section only occurred while
	// stressing once out of every 10,000 runs without the succeedsSoon. So in
	// almost all cases, the normal force should be sufficient for testing.
