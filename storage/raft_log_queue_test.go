...
func TestGetTruncatableIndexes(t *testing.T) {
	...
	getIndexes := func(r *Replica) (uint64, uint64, uint64, error) {
		r.mu.Lock()
	...
	aFirst, aTruncatable, aOldest, err := getIndexes(r)
	if err != nil {
		t.Fatal(err)
	}
	...
	bFirst, bTruncatable, bOldest, err := getIndexes(r)
	if err != nil {
		t.Fatal(err)
	}

	if aFirst != bFirst {
		t.Errorf("expected firstIndex to not change, instead it went from %d -> %d", aFirst, bFirst)
	}
	if aTruncatable >= bTruncatable {
		t.Errorf("expected truncatableIndexes to increase, instead it went from %d -> %d", aTruncatable, bTruncatable)
	}
	if aOldest >= bOldest {
		t.Errorf("expected oldestIndex to increase, instead it went from %d -> %d", aOldest, bOldest)
	}
	...
	util.SucceedsSoon(t, func() error {
		var err error
		cFirst, cTruncatable, cOldest, err = getIndexes(r)
		if err != nil {
			t.Fatal(err)
		}
		if bFirst == cFirst {
	...
	if bTruncatable < cTruncatable {
		t.Errorf("expected truncatableIndexes to decrease, instead it went from %d -> %d", bTruncatable, cTruncatable)
	}
	if bOldest >= cOldest {
		t.Errorf("expected oldestIndex to increase, instead it went from %d -> %d", bOldest, cOldest)
	}
	...
	dFirst, dTruncatable, dOldest, err := getIndexes(r)
	if err != nil {
		t.Fatal(err)
	}
