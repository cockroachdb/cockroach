...
func (rcb rangeCountBalancer) shouldRebalance(
	...
	target := sl.candidateCount.mean * (1 + rebalanceThreshold)
	shouldRebalance := store.Capacity.FractionUsed() >= maxFractionUsedThreshold ||
		float64(store.Capacity.RangeCount) > target
	if shouldRebalance {
	...
	if log.V(2) {
		log.Infof(context.TODO(),
			"%d: should-rebalance=%t: fraction-used=%.2f range-count=%d (mean=%.1f, target=%.1f)",
