...
func (rcb rangeCountBalancer) shouldRebalance(
	...
	var underfullTargetExists bool
	for _, desc := range sl.stores {
		if desc.Capacity.RangeCount < underfullThreshold {
			underfullTargetExists = true
			break
		}
	}
	rebalanceToUnderfullStore :=
		(float64(store.Capacity.RangeCount) > sl.candidateCount.mean && underfullTargetExists)
	...
	shouldRebalance :=
		(maxCapacityUsed || rangeCountAboveTarget || underfullTargetExists) && rebalanceConvergesOnMean
	if log.V(2) {
