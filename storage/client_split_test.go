...
func TestStoreRangeSplitStats(t *testing.T) {
	...
	rp := store.LookupReplica(keyPrefix, nil)
...
func TestStoreZoneUpdateAndRangeSplit(t *testing.T) {
...

	{
		...
		util.SucceedsSoon(t, func() error {
			rp = store.LookupReplica(tableBoundary, nil)
			if actualRSpan := rp.Desc().RSpan(); !actualRSpan.Equal(expectedRSpan) {
				return errors.Errorf("expected range %s to span %s", rp, expectedRSpan)
			}
			return nil
		})
...
func TestStoreRangeSplitWithMaxBytesUpdate(t *testing.T) {
	...
	origRP := store.LookupReplica(roachpb.RKeyMin, nil)
	...
	util.SucceedsSoon(t, func() error {
		newRP := store.LookupReplica(keys.MakeTablePrefix(descID), nil)
