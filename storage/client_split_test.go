...
func TestStoreRangeSplit(t *testing.T) {
	...
	if reply, err := client.SendWrapped(store, &lIncArgs); err != nil {
		t.Fatal(err)
	...
	if reply, err := client.SendWrapped(store, &rIncArgs); err != nil {
		t.Fatal(err)
