...
func TestStoreRangeSplitConcurrent(t *testing.T) {
	...
	for i := 0; i < concurrentCount; i++ {
		pErr := <-errCh
		if pErr != nil {
			// There are only two expected errors from concurrent splits: conflicting
			// range descriptors if the splits are initiated concurrently or that the
			// range is already split at the specified key.
			s := pErr.String()
			if strings.Index(s, "conflict updating range descriptors") == -1 &&
				strings.Index(s, "range is already split at key") == -1 {
