...
func (gcq *gcQueue) process(
	...
	gcArgs.TxnSpanThreshold = info.TxnSpanThreshold
...
type GCInfo struct {
	...
	// TxnSpanThreshold is the timestamp below the last activity of which
	// transaction span entries were considered for GC.
...
func RunGC(
	...
	abortSpanThreshold := now.Add(-int64(abortCacheAgeThreshold), 0)

	gc := engine.MakeGarbageCollector(now, policy)
	infoMu.Threshold = gc.Threshold
	infoMu.TxnSpanThreshold = txnExp
	...
	gcKeys = append(gcKeys, processAbortCache(
		ctx, snap, desc.RangeID, abortSpanThreshold, &infoMu, pushTxnFn)...)
