...
func NewTimestampCache(clock *hlc.Clock) *TimestampCache {
	tc := &TimestampCache{
		...
		requests:              btree.New(64), // fanout of 64 is arbitrary and not yet tuned
...
func (tc *TimestampCache) ExpandRequests(timestamp roachpb.Timestamp) {
	...
	tc.requests.AscendGreaterOrEqual(&tc.tmpReq, func(i btree.Item) bool {
		// TODO(peter): We could more intelligent about not expanding a request if
		// there is no possibility of overlap. For example, in workloads where
