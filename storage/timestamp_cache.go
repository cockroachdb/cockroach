...
func (tc *TimestampCache) Add(start, end roachpb.Key, timestamp roachpb.Timestamp, txnID *uuid.UUID, readOnly bool) {
	...
	if tc.lowWater.Less(timestamp) {
		var rg interval.RangeGroup
		rStart := interval.Comparable(start)
		rEnd := interval.Comparable(end)
		r := interval.Range{Start: rStart, End: rEnd}
		...
		for _, o := range cache.GetOverlaps(start, end) {
			...
			if !timestamp.Less(cv.timestamp) {
				...
				sCmp := rStart.Compare(o.Key.Start)
				eCmp := rEnd.Compare(o.Key.End)
				switch {
				case sCmp == 0 && eCmp == 0:
				...
				case eCmp >= 0:
					...
					o.Key.End = rStart
				case sCmp <= 0:
					// Partial overlap; truncate old start.
					//
					// New: --------
					// Old:     --------
					//
				...
				if rg == nil {
					// Only use the RangeGroup when subranges start getting removed to avoid
					// allocations for the common path.
					rg = tc.rg
					tc.rg.Add(r)
					defer rg.Clear()
				}
		...
		if rg == nil {
			...
			if err := rg.ForEach(func(r interval.Range) error {
				key := cache.MakeKey(r.Start, r.End)
				entry := makeCacheEntry(key, value)
				cache.AddEntry(entry)
				return nil
