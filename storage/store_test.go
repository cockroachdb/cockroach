...
func TestStoreResolveWriteIntent(t *testing.T) {
	...
	for i, resolvable := range []bool{true, false} {
		...
		pArgs.Txn = pusher
		_, err := client.SendWrapped(store.testSender(), nil, &pArgs)
		if resolvable {
			if err != nil {
...
func TestStoreResolveWriteIntentRollback(t *testing.T) {
	...
	args.Txn = pusher
	args.Increment = 2
	if resp, err := client.SendWrapped(store.testSender(), nil, &args); err != nil {
...
func TestStoreBadRequests(t *testing.T) {
	...
	for i, test := range testCases {
		if _, err := client.SendWrapped(store.testSender(), nil, test); err == nil {
