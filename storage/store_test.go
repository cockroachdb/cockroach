...
func TestStoreChangeFrozen(t *testing.T) {
	...
	assertFrozen := func(b bool) {
		...
		if pFrozen != b {
			t.Fatal(util.ErrorfSkipFrames(1, "expected status %t, got %t", pFrozen, b))
		}
	}

	fReqInvalid := roachpb.NewChangeFrozen(keys.LocalMax, keys.LocalMax.Next(),
		true /* frozen */, "notvalidversion").(*roachpb.ChangeFrozenRequest)

	{
		_, pErr := client.SendWrapped(store.testSender(), nil, fReqInvalid)
		if !testutils.IsPError(pErr, "cannot freeze .* from version notvalidversion") {
			t.Fatal(pErr)
		}
		assertFrozen(false)
	}

	// When processing a freeze from a different version, indicate corruption.
	{
		b := tc.store.Engine().NewBatch()
		defer b.Close()
		var h roachpb.Header
		_, err := tc.rng.ChangeFrozen(context.Background(), b, nil, h, *fReqInvalid)
		if !testutils.IsError(err, "attempt to freeze .* issued from .* was applied by") {
			t.Fatal(err)
		}
		assertFrozen(false)
	}
	...
	// The freeze (albeit failing) still makes it through, returning an error
	// from the actual freeze logic.
	{
		_, pErr := client.SendWrapped(store.testSender(), nil, fReqInvalid)
		if !testutils.IsPError(pErr, "cannot freeze .* from version notvalidversion") {
			t.Fatal(pErr)
