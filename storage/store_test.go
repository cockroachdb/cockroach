...
func (db *testSender) Send(ctx context.Context, ba roachpb.BatchRequest) (*roachpb.BatchResponse, *roachpb.Error) {
	...
	rp := db.store.LookupReplica(rs.Key, rs.EndKey)
	if rp == nil {
...
func TestStoreAddRemoveRanges(t *testing.T) {
	...
	rng2 := createReplica(store, 2, roachpb.RKey("a"), roachpb.RKey("b")).AsPromise()
	...
	rng2Dup := createReplica(store, 1, roachpb.RKey("a"), roachpb.RKey("b")).AsPromise()
	...
	rng3 := createReplica(store, 3, roachpb.RKey("c"), roachpb.RKey("d")).AsPromise()
...
func TestReplicasByKey(t *testing.T) {
	...
	for i, desc := range reps {
		desc.replica = createReplica(store, roachpb.RangeID(desc.id), desc.start, desc.end).AsPromise()
...
func TestStoreRangeSet(t *testing.T) {
	...
	for i := 0; i < newCount; i++ {
		...
		if err := store.AddReplicaTest(rng.AsPromise()); err != nil {
			t.Fatal(err)
		}
	}
	...
	for pass := 0; pass < 2; pass++ {
		...
		ranges.Visit(func(rp ReplicaPromise) bool {
			if rp.Desc().RangeID != roachpb.RangeID(i) {
				t.Errorf("expected range with Range ID %d; got %v", i, rp)
			}
	...
	go func() {
		...
		ranges.Visit(func(rp ReplicaPromise) bool {
			if i == 1 {
				...
				if rp.Desc().RangeID != roachpb.RangeID(i+1) {
					t.Errorf("expected range with Range ID %d; got %v", i+1, rp)
				}
			}
			i++
			return true
	...
	if err = store.SplitRange(store.LookupReplica(roachpb.RKey("a00"), nil), rng.AsPromise()); err != nil {
		t.Fatal(err)
	}
	...
	rp := store.LookupReplica(roachpb.RKey("a01"), nil)
	desc := rp.Desc()
	if desc.RangeID != 2 {
		t.Errorf("expected fetch of rangeID=2; got %d", desc.RangeID)
	}
	if err := store.RemoveReplica(rp, *desc, true, false); err != nil {
		t.Error(err)
	}
	if ec := ranges.EstimatedCount(); ec != 9 {
		t.Errorf("expected 9 remaining; got %d", ec)
...
func splitTestRange(store *Store, key, splitKey roachpb.RKey, t *testing.T) *Replica {
	...
	if err = store.SplitRange(rng, newRng.AsPromise()); err != nil {
		t.Fatal(err)
	}
	return newRng
}
...
func TestStoreReplicasByKey(t *testing.T) {
	...
	r1 := splitTestRange(store, roachpb.RKeyMin, roachpb.RKey("A"), t).AsPromise()
	r2 := splitTestRange(store, roachpb.RKey("A"), roachpb.RKey("C"), t).AsPromise()
	r3 := splitTestRange(store, roachpb.RKey("C"), roachpb.RKey("X"), t).AsPromise()
	r4 := splitTestRange(store, roachpb.RKey("X"), roachpb.RKey("ZZ"), t).AsPromise()
...
func TestStoreSetRangesMaxBytes(t *testing.T) {
	...
	rp := store.LookupReplica(roachpb.RKeyMin, nil)
...
type fakeRangeQueue struct {
	maybeRemovedRngs chan ReplicaPromise
}
...
func (fq *fakeRangeQueue) MaybeAdd(_ ReplicaPromise, _ hlc.Timestamp) {
	// Do nothing
}

func (fq *fakeRangeQueue) MaybeRemove(rp ReplicaPromise) {
	fq.maybeRemovedRngs <- rp
}
...
func TestMaybeRemove(t *testing.T) {
	...
	fq := &fakeRangeQueue{
		maybeRemovedRngs: make(chan ReplicaPromise),
	}
...
func TestStoreRangePlaceholders(t *testing.T) {
	...
	if err := s.AddReplicaTest(rep.AsPromise()); err != nil {
		t.Fatal(err)
	}
