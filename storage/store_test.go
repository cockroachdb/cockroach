...
func TestStoreRangePlaceholders(t *testing.T) {
	...
	if err := s.addPlaceholderLocked(placeholder2); err != nil {
		t.Fatalf("could not add non-overlapping placeholder, got %v", err)
	}
	...
	if err := s.addPlaceholderLocked(placeholder1); !testutils.IsError(err, "range for RangeID \\d+ already exists in replicasByKey btree, when canApplySnapshotLocked returned true") {
