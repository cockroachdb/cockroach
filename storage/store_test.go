...
func TestStoreRangePlaceholders(t *testing.T) {
	...
	if err := s.addPlaceholderLocked(placeholder); !testutils.IsError(err, "range for RangeID /Max already exists in replicasByKey btree, when canApplySnapshotLocked returned true") {
	...
	if err := s.addPlaceholderLocked(placeholder); !testutils.IsError(err, "range for RangeID \"c\" already exists in replicasByKey btree, when canApplySnapshotLocked returned true") {
	...
	if err := s.removePlaceholderLocked(repID); !testutils.IsError(err, "cannot remove placeholder for RangeID 2; Placeholder doesn't exist") {
