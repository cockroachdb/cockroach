...
func createTestStoreWithoutStart(t testing.TB, cfg *StoreConfig) (*Store, *hlc.ManualClock, *stop.Stopper) {
	stopper := stop.NewStopper()
	// Setup fake zone config handler.
	config.TestingSetupZoneConfigHook(stopper)
	rpcContext := rpc.NewContext(&base.Context{Insecure: true}, nil, stopper)
	server := rpc.NewServer(rpcContext) // never started
	cfg.Gossip = gossip.New(rpcContext, server, nil, stopper, metric.NewRegistry())
...
// upon completion.
func createTestStoreWithContext(t testing.TB, cfg *StoreConfig) (
...
func TestProcessRangeDescriptorUpdate(t *testing.T) {
	...
	r := &Replica{
		...
		raftSender: store.cfg.Transport.MakeSender(func(err error, toReplica roachpb.ReplicaDescriptor) {}),
	}
