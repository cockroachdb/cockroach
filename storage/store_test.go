...
func createTestStore(t *testing.T) (*Store, *hlc.ManualClock) {
	...
	store.db = &client.KV{Sender: &testSender{store: store}}
...
func TestStoreResolveWriteIntent(t *testing.T) {
	...
	for i, resolvable := range []bool{true, false} {
		...
		if resolvable {
			...
			if !ok {
				t.Errorf("resolvable? %t, expected txn push error; got %s", resolvable, err)
			}
			if !bytes.Equal(rErr.PusheeTxn.ID, pushee.ID) {
				t.Errorf("resolvable? %t, expected txn to match pushee %q; got %s", resolvable, pushee.ID, rErr)
			}
...
func TestStoreResolveWriteIntentRollback(t *testing.T) {
	...
	args.Txn = pusher
	args.Increment = 2
	err := store.ExecuteCmd(proto.Increment, args, reply)
	if err == nil {
...
func TestStoreResolveWriteIntentNoTxn(t *testing.T) {
	...
	err := store.ExecuteCmd(proto.Put, args, reply)
	if err == nil {
...
func fillRange(store *Store, rangeID int64, prefix proto.Key, bytes int64, t *testing.T) {
	src := rand.New(rand.NewSource(0))
	for {
		keyBytes, err := engine.GetRangeStat(store.engine, rangeID, engine.StatKeyBytes)
		if err != nil {
