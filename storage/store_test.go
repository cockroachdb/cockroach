	...
	testCases := []struct {
		...
		{badKey,
			func(_ roachpb.Response, pErr *roachpb.Error) {
				...
				if txn := pErr.GetTxn(); txn == nil || txn.GetObservedTimestamp(desc.NodeID).WallTime != wallTime {
		...
		{goodKey,
			func(pReply roachpb.Response, pErr *roachpb.Error) {
				...
				if act, exp := txn.GetObservedTimestamp(desc.NodeID).WallTime, int64(wallTime); exp != act {
					t.Fatalf("unexpected observed wall time: %d", act)
				}
			}},
	}
