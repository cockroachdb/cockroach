...
func TestAllocatorRelaxConstraints(t *testing.T) {
	...
	for i, test := range testCases {
		...
		result, err := a.AllocateTarget(test.required, existing, test.relaxConstraints)
...
func TestAllocatorRebalance(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		result := a.RebalanceTarget(nil, []roachpb.ReplicaDescriptor{{StoreID: 3}}, 0)
		if result == nil {
			t.Fatal("nil result")
		}
...
func TestAllocatorRebalanceByCount(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		result := a.RebalanceTarget(nil, []roachpb.ReplicaDescriptor{{StoreID: 1}}, 0)
...
func TestAllocatorComputeAction(t *testing.T) {
	...
	testCases := []struct {
		...
		// Needs three replicas, two are on dead stores.
		{
			zone: config.ZoneConfig{
				NumReplicas:   3,
				Constraints:   []config.Constraint{{Value: "us-east"}},
				RangeMinBytes: 0,
				RangeMaxBytes: 64000,
			},
			desc: roachpb.RangeDescriptor{
