...
type raftEntryCache struct {
	sync.Mutex                              // protects Cache for concurrent access.
	bytes        uint64                     // total size of the cache in bytes
	cache        *cache.UnorderedCache      // LRU cache of log entries, keyed by rangeID / log index
	firstIndexes map[roachpb.RangeID]uint64 // map from rangeID -> minimum cached index
}
...
func newRaftEntryCache(maxBytes uint64) *raftEntryCache {
	rec := &raftEntryCache{
		cache:        cache.NewUnorderedCache(cache.Config{Policy: cache.CacheLRU}),
		firstIndexes: map[roachpb.RangeID]uint64{},
	}
	// We expect the raft entry cache mutex to be held on ShouldEvict
	// and OnEvicted callbacks.
	// On ShouldEvice, compare the total size of the cache in bytes to the
...
func (rec *raftEntryCache) addEntries(rangeID roachpb.RangeID, ents []raftpb.Entry) {
	...
	if fi, ok := rec.firstIndexes[rangeID]; !ok || ents[0].Index < fi {
		rec.firstIndexes[rangeID] = ents[0].Index
	}
}
...
func (rec *raftEntryCache) getEntries(rangeID roachpb.RangeID, lo, hi, maxBytes uint64) (
	[]raftpb.Entry, uint64 /* size in bytes */, uint64 /* max Index */) {
	rec.Lock()
	defer rec.Unlock()
	...
	for i := lo; i < hi; i++ {
		v, ok := rec.cache.Get(entryCacheKey{RangeID: rangeID, Index: i})
		if ok {
			ent = v.(raftpb.Entry)
			ents = append(ents, ent)
			bytes += uint64(ent.Size())
			if maxBytes > 0 && bytes > maxBytes {
				return ents, bytes, i + 1
			}
		} else {
			return ents, bytes, i
		}
	}
	return ents, bytes, hi
}
...
func (rec *raftEntryCache) delEntries(rangeID roachpb.RangeID, lo, hi uint64) {
	rec.Lock()
	defer rec.Unlock()
	if lo > hi || rec.cache.Len() == 0 || (rec.cache.Len() > 0 && lo == 0) {
		return
	}

	if fi := rec.firstIndexes[rangeID]; fi >= lo && fi < hi {
		for i := fi; i < hi; i++ {
			rec.cache.Del(entryCacheKey{RangeID: rangeID, Index: i})
		}
		rec.firstIndexes[rangeID] = hi
		return
	}
...
func (rec *raftEntryCache) clearTo(rangeID roachpb.RangeID, index uint64) {
	...
	for i, ok := rec.firstIndexes[rangeID]; ok && i < index; i++ {
		rec.cache.Del(entryCacheKey{RangeID: rangeID, Index: i})
	}
	delete(rec.firstIndexes, rangeID)
}
