...
type raftEntryCache struct {
	...
	entryCache *cache.UnorderedCache
...
func newRaftEntryCache(size int) *raftEntryCache {
	return &raftEntryCache{
		entryCache: cache.NewUnorderedCache(cache.Config{
...
func (rec *raftEntryCache) evictEntry() {
	if len := rec.entryCache.Len(); len <= rec.size {
		...
		rec.firstIndex = rec.firstIndex + uint64(evictLen)
	}
	return
}
