...
func (a entryCacheKey) Compare(b llrb.Comparable) int {
	bk := b.(entryCacheKey)
	if a.RangeID < bk.RangeID {
		return -1
	} else if a.RangeID > bk.RangeID {
		return 1
	}
	return int(a.Index - bk.Index)
}
...
func newRaftEntryCache(maxBytes uint64) *raftEntryCache {
	...
	rec.cache.Config.OnEvicted = func(k, v interface{}) {
		log.Infof(context.TODO(), "evicting log entry %+v", k)
		ent := v.(raftpb.Entry)
...
func (rec *raftEntryCache) clearTo(rangeID roachpb.RangeID, index uint64) {
	...
	rec.cache.DoRange(func(k, v interface{}) bool {
		...
		return false
	}, entryCacheKey{RangeID: rangeID, Index: 0}, entryCacheKey{RangeID: rangeID, Index: index})
