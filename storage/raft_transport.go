...
var (
	// InitialResolverBackoff is the starting retry backoff for
	// resolving from node ID -> Address. Resolving may fail in the
	// event that a node is down long enough for its node descriptor
	// to expire in gossip.
	InitialResolverBackoff = time.Second
)
...
func (t *RaftTransport) resolveNodeID(nodeID roachpb.NodeID) (net.Addr, error) {
	...
	opts := retry.Options{
		InitialBackoff: InitialResolverBackoff,
