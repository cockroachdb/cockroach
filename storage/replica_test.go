...
// TestTxnSpanThreshold verifies that aborting transactions which haven't
...
// transaction's subsequent attempts to create its initial record fail.
//
// See #9265 for context.
func TestEndTransactionTxnSpanThreshold(t *testing.T) {
	defer leaktest.AfterTest(t)()
	tc := testContext{}
	tc.Start(t)
	defer tc.Stop()
	...
	if abortedPushee.Status != roachpb.ABORTED {
		t.Fatalf("expected push to abort pushee, got %+v", resp)
	}
	...
	// last activity against the persisted TxnSpanThreshold.
	{
