...
func TestRangeLeaderLease(t *testing.T) {
	...
	pErr := tc.rng.redirectOnOrAcquireLeaderLease(tracer.NilTrace())
	...
	if _, ok := rng.redirectOnOrAcquireLeaderLease(tracer.NilTrace()).GetDetail().(*roachpb.NotLeaderError); !ok {
