...
func testReplicaCommandQueueCancellation(t *testing.T, instructions []commandInstruction) {
	...
	for i := range cancellations {
		j := rand.Intn(i + 1)
		cancellations[i], cancellations[j] = cancellations[j], cancellations[i]
	}
...
func TestReplicaCommandQueueCancelRandom(t *testing.T) {
	...
	const trials = 25
	for i := 0; i < trials; i++ {
		commandCount := randutil.RandIntInRange(rng, 5, 25)
		...
		for j := range instructions {
			...
			fmt.Println(instructions[j].span, instructions[j].cancel)
		}
