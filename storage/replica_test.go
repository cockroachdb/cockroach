...
func TestRangeLeaderLease(t *testing.T) {
	...
	pErr := tc.rng.redirectOnOrAcquireLeaderLease(tracing.NilTrace())
	...
	if _, ok := rng.redirectOnOrAcquireLeaderLease(tracing.NilTrace()).GetDetail().(*roachpb.NotLeaderError); !ok {
