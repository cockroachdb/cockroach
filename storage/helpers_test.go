...
func (s *Store) ComputeMVCCStats() (enginepb.MVCCStats, error) {
	...
	visitor.Visit(func(rp ReplicaPromise) bool {
...
func (s *Store) ForceReplicationScanAndProcess() {
	newStoreRangeSet(s).Visit(func(rp ReplicaPromise) bool {
		s.replicateQueue.MaybeAdd(rp, s.ctx.Clock.Now())
		return true
	})
...
func (s *Store) ForceReplicaGCScanAndProcess() {
	newStoreRangeSet(s).Visit(func(rp ReplicaPromise) bool {
		s.replicaGCQueue.MaybeAdd(rp, s.ctx.Clock.Now())
		return true
	})
...
func (s *Store) ForceRaftLogScanAndProcess() {
	...
	for _, r := range replicas {
		s.raftLogQueue.MaybeAdd(r.AsPromise(), s.ctx.Clock.Now())
	}
...
func (s *Store) ManualReplicaGC(rp ReplicaPromise) error {
	cfg, ok := s.Gossip().GetSystemConfig()
	if !ok {
		return fmt.Errorf("%s: system config not yet available", s)
	}
	return s.gcQueue.process(s.Ctx(), s.Clock().Now(), rp, cfg)
}
