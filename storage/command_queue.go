...
// determining its prerequisite commands, the command is added to the queue via add().
...
// queue, so in a concurrent setting, their execution must be synchronized under the
// same lock.
//
...
// specified key ranges. If an end key is empty, the method only affects the start
// key. readOnly is true if the requester is a read-only command; false for read-write.
func (cq *CommandQueue) getPrereqs(readOnly bool, spans ...roachpb.Span) (prereqs []*cmd) {
	...
	for i := 0; i < len(spans); i++ {
		...
		// the new commands overlapped range.
		//
		...
		// without the need for the majority of dependencies to be help explicitly, which
...
//
// add should be invoked before waiting on prereqs' pending channels.
