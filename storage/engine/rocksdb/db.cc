...
struct EngineGetter : public Getter {
  EngineGetter(DBEngine* db, DBSnapshot* snap, DBSlice key)
      : db_(db),
        snap_(snap),
        key_(key) {
  }
  ...
  virtual DBStatus Get(DBString* value) {
    std::string tmp;
    rocksdb::Status s = db_->rep->Get(MakeReadOptions(snap_), ToSlice(key_), &tmp);
    if (!s.ok()) {
      if (s.IsNotFound()) {
...
DBStatus DBEngine::Put(DBSlice key, DBSlice value) {
...
DBStatus DBEngine::Merge(DBSlice key, DBSlice value) {
  rocksdb::WriteOptions options;
...
DBStatus DBEngine::Get(DBSlice key, DBString* value) {
  EngineGetter base(this, NULL, key);
  return base.Get(value);
}

DBStatus DBBatch::Get(DBSlice key, DBString* value) {
  if (updates == 0) {
    return DBEngine::Get(key, value);
  }
...
DBStatus DBSnapshot::Get(DBSlice key, DBString* value) {
  EngineGetter base(this, this, key);
  return base.Get(value);
}
...
DBStatus DBEngine::Delete(DBSlice key) {
  rocksdb::WriteOptions options;
...
DBIterator* DBEngine::NewIter() {
  DBIterator* iter = new DBIterator;
  iter->rep = rep->NewIterator(MakeReadOptions(NULL));
  return iter;
}

DBIterator* DBBatch::NewIter() {
  if (updates == 0) {
    return DBEngine::NewIter();
  }

  DBIterator* iter = new DBIterator;
  rocksdb::Iterator* base = rep->NewIterator(MakeReadOptions(NULL));
...
DBIterator* DBSnapshot::NewIter() {
  DBIterator* iter = new DBIterator;
  iter->rep = rep->NewIterator(MakeReadOptions(this));
  return iter;
}
