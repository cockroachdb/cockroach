...
func (r *RocksDB) WriteBatch(repr []byte) error {
	return dbWriteBatch(r.rdb, repr)
}
...
func (r *rocksDBSnapshot) WriteBatch(repr []byte) error {
...
// WriteBatch is illegal for a batch and returns an error.
func (r *rocksDBBatch) WriteBatch(repr []byte) error {
	return util.Errorf("cannot WriteBatch to a batch")
}
...
func dbWriteBatch(rdb *C.DBEngine, repr []byte) error {
	return statusToError(C.DBWriteBatch(rdb, goToCSlice(repr)))
}
