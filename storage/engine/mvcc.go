...
func MVCCResolveWriteIntentRange(engine Engine, ms *MVCCStats, intent roachpb.Intent, max int64) (int64, error) {
	...
	for {
		...
		// nextKey is already a metadata key. We copy unsafeKey.Key because we
		// can't append to it directly. And then we append a 0 byte which is that
		// same as roacphb.Key.Next but without creating a new copy.
		keyBuf = append(keyBuf, 0)
		nextKey.Key = append(keyBuf)
	}
