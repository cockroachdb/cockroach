...
var (
	...
	// all other keys.
	MVCCKeyMax = MakeMVCCKey(roachpb.KeyMax)
...
// MakeMVCCKey creates an MVCCKey from a roachpb.Key.
func MakeMVCCKey(key roachpb.Key) MVCCKey {
...
func init() {
	for _, r := range keys.NoSplitSpans {
		illegalSplitKeySpans = append(illegalSplitKeySpans,
			encodedSpan{
				start: MakeMVCCKey(r.Key),
				end:   MakeMVCCKey(r.EndKey),
			})
	}
}
...
func getReverseScanMeta(iter Iterator, encEndKey MVCCKey, meta *MVCCMetadata) (MVCCKey, error) {
	...
	if metaKey.IsValue() {
		...
		iter.Seek(MakeMVCCKey(metaKey.Key))
		if !iter.Valid() {
			return NilKey, iter.Error()
		}
...
func MVCCIterate(engine Engine, startKey, endKey roachpb.Key, timestamp roachpb.Timestamp,
	...
	if reverse {
		encEndKey = MakeMVCCKey(startKey)
		encKey = MakeMVCCKey(endKey)
		getMeta = getReverseScanMeta
	} else {
		encEndKey = MakeMVCCKey(endKey)
		encKey = MakeMVCCKey(startKey)
		getMeta = getScanMeta
	}
	...
	for {
		...
		if reverse {
			...
			iter.Seek(MakeMVCCKey(metaKey.Key.Next()))
			if !iter.Valid() {
				if err := iter.Error(); err != nil {
					return nil, err
				}
...
func MVCCResolveWriteIntentRange(engine Engine, ms *MVCCStats, key, endKey roachpb.Key, max int64, timestamp roachpb.Timestamp, txn *roachpb.Transaction) (int64, error) {
	...
	for {
		...
		nextKey = MakeMVCCKey(key0.Key.Next())
	}
...
func IsValidSplitKey(key roachpb.Key) bool {
	...
	return isValidEncodedSplitKey(MakeMVCCKey(key))
}
...
func MVCCFindSplitKey(engine Engine, rangeID roachpb.RangeID, key, endKey roachpb.RKey) (roachpb.Key, error) {
	...
	encStartKey := MakeMVCCKey(key.AsRawKey())
	encEndKey := MakeMVCCKey(endKey.AsRawKey())
