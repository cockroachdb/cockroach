...
func MVCCGetAsTxn(engine Engine, key roachpb.Key, timestamp roachpb.Timestamp, consistent bool, txnMeta roachpb.Meta) (*roachpb.Value, []roachpb.Intent, error) {
...
func MVCCResolveWriteIntent(engine Engine, ms *MVCCStats, intent roachpb.Intent) error {
	key := intent.Key // TODO(tschottdorf)
	if len(key) == 0 {
		return emptyKeyError()
	}
	...
	commit := intent.Status == roachpb.COMMITTED
	pushed := intent.Status == roachpb.PENDING && meta.Txn.Timestamp.Less(intent.Txn.Timestamp)
	if (commit || pushed) && meta.Txn.Epoch == intent.Txn.Epoch {
		newMeta := *meta
		...
		if !meta.Timestamp.Equal(intent.Txn.Timestamp) {
			...
			newKey := MVCCKey{Key: key, Timestamp: intent.Txn.Timestamp}
	...
	if !iter.Valid() || !iter.unsafeKey().Key.Equal(key) {
		...
		if ms != nil {
			ms.Add(updateStatsOnAbort(key, origMetaKeySize, origMetaValSize, 0, 0, meta, nil, 0, intent.Txn.Timestamp.WallTime))
		}
		return nil
	}
	...
	if ms != nil {
		ms.Add(updateStatsOnAbort(key, origMetaKeySize, origMetaValSize, metaKeySize, metaValSize, meta, newMeta, iterKey.Timestamp.WallTime, intent.Txn.Timestamp.WallTime))
	}
...
func MVCCResolveWriteIntentRange(engine Engine, ms *MVCCStats, intent roachpb.Intent, max int64) (int64, error) {
	key, endKey := intent.Key, intent.EndKey
