...
func putUvarint(buf []byte, x uint32) int {
	i := 0
	for x >= 0x80 {
		buf[i] = byte(x) | 0x80
		x >>= 7
...
func (b *rocksDBBatchBuilder) encodeKey(key MVCCKey, extra int) {
	...
	n := lenUvarint(uint32(length))
	pos := 1 + len(b.repr)
	b.grow(1 + n + length + extra)
	putUvarint(b.repr[pos:pos+n], uint32(length))
	pos += n
	...
	if timestampLength > 0 {
		...
		b.repr[pos] = 0
		pos++
		encoding.EncodeUint64Ascending(b.repr[pos:pos], uint64(key.Timestamp.WallTime))
		if key.Timestamp.Logical != 0 {
			pos += 8
			encoding.EncodeUint32Ascending(b.repr[pos:pos], uint32(key.Timestamp.Logical))
		}
	}
