...
func TestMVCCInlineWithTxn(t *testing.T) {
	...
	// Put an inline value.
	err := MVCCPut(engine, nil, testKey1, roachpb.ZeroTimestamp, value1, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	_, _, err = MVCCGet(engine, testKey1, roachpb.ZeroTimestamp, true, txn1)
	if err != nil {
		t.Error(err)
	}

	// Verify inline put with txn is an error.
	err = MVCCPut(engine, nil, testKey2, roachpb.ZeroTimestamp, value2, txn2)
	if err == nil {
