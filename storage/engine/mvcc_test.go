...
func TestMVCCDeleteMissingKey(t *testing.T) {
	...
	if val, err := engine.Get(MakeMVCCKey(testKey1)); err != nil || val != nil {
...
func TestMVCCAbortTxn(t *testing.T) {
	...
	meta, err := engine.Get(MakeMVCCKey(testKey1))
	if err != nil {
		t.Fatal(err)
	}
	if len(meta) != 0 {
...
func TestValidSplitKeys(t *testing.T) {
	...
	testCases := []struct {
		...
		{roachpb.Key("\x02\x01"), false},
		{roachpb.Key("\x02\x01m1"), false},
		{roachpb.Key("\x02\x01m1\x00"), false},
		{roachpb.Key("\x02\x01m1\xff"), false},
		{roachpb.Key("\x02\x01m2"), true},
		{roachpb.Key("\x02\x01m2\x00"), true},
		{roachpb.Key("\x02\x01m2\xff"), true},
		{roachpb.Key("\x02\x01m2\xff\xff"), false},
		{roachpb.Key("\x02\x01meta3"), true},
		{roachpb.Key("\x02\x02"), true},
		// Deprecated accounting config key. Valid split.
		{roachpb.Key("\x02accs\xff"), true},
		{roachpb.Key("\x02acct"), true},
		{roachpb.Key("\x02acct\x00"), true},
		{roachpb.Key("\x02acct\xff"), true},
		{roachpb.Key("\x02accu"), true},
		{roachpb.Key("\x02perl"), true},
		{roachpb.Key("\x02perm"), true},
		// Deprecated permission config key. Valid split.
		{roachpb.Key("\x02perm\x00"), true},
		{roachpb.Key("\x02perm\xff"), true},
		{roachpb.Key("\x02pern"), true},
		{roachpb.Key("\x02zond"), true},
		{roachpb.Key("\x02zone"), true},
		// Deprecated zone config key. Valid split.
		{roachpb.Key("\x02zone\x00"), true},
		{roachpb.Key("\x02zone\xff"), true},
		{roachpb.Key("\x02zonf"), true},
...
func TestFindValidSplitKeys(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			keys: []roachpb.Key{
				roachpb.Key("\x02\x01m1"),
				roachpb.Key("\x02\x01m1\x00"),
				roachpb.Key("\x02\x01m1\xff"),
			},
			expSplit: nil,
			expError: true,
		},
		...
		// Between meta1 and meta2, splits at meta2.
		{
			keys: []roachpb.Key{
				...
				roachpb.Key("\x02\x01m1\x00"),
				roachpb.Key("\x02\x01m1\xff"),
				roachpb.Key("\x02\x01m2"),
				roachpb.Key("\x02\x01m2\x00"),
				roachpb.Key("\x02\x01m2\xff"),
			},
			expSplit: roachpb.Key("\x02\x01m2"),
			expError: false,
		},
		...
		// Lopsided, truncate non-zone prefix.
		{
			keys: []roachpb.Key{
				roachpb.Key("\x02zond"),
				roachpb.Key("\x02zone"),
				roachpb.Key("\x02zone\x00"),
				roachpb.Key("\x02zone\xff"),
			},
			expSplit: roachpb.Key("\x02zone\x00"),
			expError: false,
		},
		// Lopsided, truncate non-zone suffix.
		{
			...
			expSplit: roachpb.Key("\x02zone\xff"),
			expError: false,
		},
	}
...
func TestMVCCStatsBasic(t *testing.T) {
	...
	ts := makeTS(1*1E9, 0)
	key := roachpb.Key("a")
	keySize := int64(mvccVersionKey(key, ts).EncodedSize() - MakeMVCCKey(key).EncodedSize())
	...
	mKeySize := int64(MakeMVCCKey(key).EncodedSize())
	...
	mKey2Size := int64(MakeMVCCKey(key2).EncodedSize())
	...
	txnKeySize := int64(MakeMVCCKey(txnKey).EncodedSize())
...
func TestMVCCStatsWithRandomRuns(t *testing.T) {
	...
	for i := int32(0); i < int32(1000); i++ {
		...
		if i%10 == 0 {
			...
			var expMS MVCCStats
			err := iter.ComputeStats(&expMS, MakeMVCCKey(roachpb.KeyMin),
				MakeMVCCKey(roachpb.KeyMax), int64(i+1)*1E9)
			iter.Close()
			if err != nil {
				t.Fatal(err)
			}
...
func TestMVCCGarbageCollect(t *testing.T) {
	...
	kvsn, err := Scan(engine, MakeMVCCKey(keyMin), MakeMVCCKey(keyMax), 0)
	if err != nil {
		t.Fatal(err)
	}
	for i, kv := range kvsn {
	...
	kvs, err := Scan(engine, MakeMVCCKey(keyMin), MakeMVCCKey(keyMax), 0)
	if err != nil {
		t.Fatal(err)
	}
	...
	var expMS MVCCStats
	err = iter.ComputeStats(&expMS, MakeMVCCKey(roachpb.KeyMin),
		MakeMVCCKey(roachpb.KeyMax), ts3.WallTime)
	iter.Close()
	if err != nil {
		t.Fatal(err)
	}
...
func TestMVCCComputeStatsError(t *testing.T) {
	...
	// protobuf.
	if err := engine.Put(MakeMVCCKey(roachpb.Key("garbage")), []byte("garbage")); err != nil {
		t.Fatal(err)
	}

	var ms MVCCStats
	iter := engine.NewIterator(false)
	err := iter.ComputeStats(&ms, MakeMVCCKey(roachpb.KeyMin),
		MakeMVCCKey(roachpb.KeyMax), 100)
	iter.Close()
