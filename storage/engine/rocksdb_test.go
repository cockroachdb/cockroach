...
func setupMVCCData(numVersions, numKeys, valueBytes int, b *testing.B) (*RocksDB, *stop.Stopper) {
	...
	for i, idx := range order {
		...
		if err := MVCCPut(NoSpan, batch, nil, key, ts, value, nil); err != nil {
			b.Fatal(err)
		}
	}
	if err := batch.Commit(); err != nil {
...
func runMVCCScan(numRows, numVersions, valueSize int, b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		kvs, _, err := MVCCScan(NoSpan, rocksdb, startKey, keyMax, int64(numRows), ts, true, nil)
		if err != nil {
...
func runMVCCGet(numVersions, valueSize int, b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		walltime := int64(5 * (rand.Int31n(int32(numVersions)) + 1))
		ts := makeTS(walltime, 0)
		if v, _, err := MVCCGet(NoSpan, rocksdb, key, ts, true, nil); err != nil {
...
func runMVCCPut(valueSize int, b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		if err := MVCCPut(NoSpan, rocksdb, nil, key, ts, value, nil); err != nil {
...
func runMVCCConditionalPut(valueSize int, createFirst bool, b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		key := roachpb.Key(encoding.EncodeUvarintAscending(keyBuf[:4], uint64(i)))
		ts := makeTS(timeutil.Now().UnixNano(), 0)
		if err := MVCCConditionalPut(NoSpan, rocksdb, nil, key, ts, value, expected, nil); err != nil {
			b.Fatalf("failed put: %s", err)
		}
	}
...
func runMVCCMerge(value *roachpb.Value, numKeys int, b *testing.B) {
	...
	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
			ms := MVCCStats{}
			ts.Logical++
			err := MVCCMerge(NoSpan, rocksdb, &ms, keys[rand.Intn(numKeys)], ts, *value)
			if err != nil {
				b.Fatal(err)
			}
		}
	...
	for _, key := range keys {
		val, _, err := MVCCGet(NoSpan, rocksdb, key, roachpb.ZeroTimestamp, true, nil)
		if err != nil {
			b.Fatal(err)
		} else if val == nil {
			continue
...
func runMVCCDeleteRange(valueBytes int, b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		b.StartTimer()
		_, err := MVCCDeleteRange(NoSpan, dupRocksdb, &MVCCStats{}, roachpb.KeyMin, roachpb.KeyMax, 0, roachpb.MaxTimestamp, nil, false)
		if err != nil {
			b.Fatal(err)
		}
		b.StopTimer()
...
func BenchmarkMVCCPutDelete(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		if err := MVCCPut(NoSpan, rocksdb, nil, key, zeroTS, value, nil /* txn */); err != nil {
			b.Fatal(err)
		}
		if err := MVCCDelete(NoSpan, rocksdb, nil, key, zeroTS, nil /* txn */); err != nil {
			b.Fatal(err)
		}
	}
}
