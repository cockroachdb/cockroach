...
of the timestamp (8 bytes for the nanosecond wall time, followed by
four bytes for the logical time). The MVCC version value is a message
of type roachpb.Value. A deletion is is indicated by an empty value.
Note that an empty roachpb.Value will encode to a non-empty byte
slice. The decreasing encoding on the timestamp sorts the most recent
version directly after the metadata key, which is treated specially by
the RocksDB comparator (by making the zero timestamp sort first). This
...
key/value and then using it to directly get the MVCC version using the
metadata's most recent version timestamp. This avoids using an
