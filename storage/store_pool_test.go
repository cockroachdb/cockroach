...
// node server RPCs, an rpc context used for the server and a listener. The
// listener should be closed when it is no longer needed.
func newFakeNodeServer(t *testing.T, stopper *stop.Stopper) (*fakeNodeServer, *rpc.Context, net.Listener) {
...
func TestStorePoolReserve(t *testing.T) {
	...
	f, ctx, ln := newFakeNodeServer(t, stopper)
	defer ln.Close()
