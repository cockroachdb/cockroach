...
func (p *ProposalData) Absorb(q ProposalData) error {
	...
	coalesce := func(lhs *bool, rhs bool) {
		*lhs = *lhs || rhs
	}
	coalesce(&p.gossipFirstRange, q.gossipFirstRange)
	coalesce(&p.maybeGossipSystemConfig, q.maybeGossipSystemConfig)
	coalesce(&p.maybeAddToSplitQueue, q.maybeAddToSplitQueue)
	coalesce(&p.addToReplicaGCQueue, q.addToReplicaGCQueue)
	return nil
}
...
func (r *Replica) handleProposalData(
	...
	// TODO(tschottdorf): check this without reflect.
	...
	if newTruncState := pd.State.TruncatedState; newTruncState != nil {
		...
		r.store.raftEntryCache.clearTo(
			r.RangeID, newTruncState.Index+1,
		)
	}
	...
	if !reflect.DeepEqual(pd, ProposalData{}) {
