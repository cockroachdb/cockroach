...
// errLeaseVersionChanged indicates that the lease version has changed.
type errLeaseVersionChanged struct {
}

func (*errLeaseVersionChanged) Error() string {
	...
	for r := retry.Start(retryOpts); r.Next(); {
		...
		pErr = s.db.Txn(func(txn *client.Txn) *roachpb.Error {
			...
			if expectedVersion != tableDesc.Version {
				...
				return roachpb.NewError(&errLeaseVersionChanged{})
			}
		...
		if _, ok := pErr.GoError().(*errLeaseVersionChanged); !ok {
			return pErr
		}
	}
