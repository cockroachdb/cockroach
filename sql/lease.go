...
func (s LeaseStore) Acquire(txn *client.Txn, tableID ID, minVersion Version) (*LeaseState, error) {
	lease := &LeaseState{}
...
// of a descriptor.
func (s LeaseStore) countLeases(descID ID, version Version, expiration time.Time) (int, error) {
	var count int
	err := s.db.Txn(func(txn *client.Txn) error {
		p := planner{txn: txn, user: security.RootUser}
...
func (l *leaseSet) find(version Version, expiration parser.DTimestamp) *LeaseState {
...
func (l *leaseSet) findIndex(version Version, expiration parser.DTimestamp) (int, bool) {
...
func (l *leaseSet) findNewest(version Version) *LeaseState {
	if len(l.data) == 0 {
		return nil
	}
	if version == 0 {
...
func (t *tableState) acquire(txn *client.Txn, version Version, store LeaseStore) (*LeaseState, error) {
	t.mu.Lock()
	defer t.mu.Unlock()
...
func (t *tableState) acquireNodeLease(txn *client.Txn, minVersion Version, store LeaseStore) (*LeaseState, error) {
...
// knows about.
func (m *LeaseManager) Acquire(txn *client.Txn, tableID ID, version Version) (*LeaseState, error) {
...
func (m *LeaseManager) refreshLease(db *client.DB, id ID, minVersion Version) error {
