...
// the table desc.
// The update closure is called after the wait, and it's supposed to be the one
// providing the new version of the descriptor to be written. In a multi-step
// schema operation, this update should perform a single step.
		...
		if t.acquiring != nil {
			...
			if pErr != nil {
				if _, ok := pErr.GetDetail().(*roachpb.DescriptorDeletedError); ok {
					// If we're trying to acquire and the descriptor is being deleted, we
					// need to release all existing leases since they're now stale.
