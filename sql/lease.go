...
func (t *tableNameCache) get(dbID sqlbase.ID, tableName string) (bool, sqlbase.ID) {
	t.mu.Lock()
	defer t.mu.Unlock()
	if id, ok := t.tables[t.makeCacheKey(dbID, tableName)]; ok {
		return true, id
	}
	return false, 0
}
...
func (t *tableNameCache) makeCacheKey(dbID sqlbase.ID, tableName string) string {
	// TODO(andrei): Figure out an escaping scheme instead of this prefix.
	return fmt.Sprintf("___cachekey___%d:%s", dbID, sqlbase.NormalizeName(tableName))
}
...
func NewLeaseManager(
	...
	return &LeaseManager{
		...
		tableNames: tableNameCache{
			tables: make(map[string]sqlbase.ID),
		},
...
func (m *LeaseManager) AcquireByName(
	...
	ok, tableID := m.tableNames.get(dbID, tableName)
...
// hasUsableLease returns true if we have a lease on the specified descriptor
