...
func (t *tableState) release(lease *LeaseState, store LeaseStore) error {
	...
	// Decrements the refcount and returns true if refcount drops to 0.
	decrementAndMaybeMarkUnused := func(wouldRemoveFromStore bool) bool {
		s.mu.Lock()
		defer s.mu.Unlock()
		s.refcount--
		if log.V(3) {
	...
	notInUse := decrementAndMaybeMarkUnused(wouldRemoveFromStore)
	// Remove the lease from the store if the refcount is 0 and we don't want to
	// keep the lease around.
	if notInUse && wouldRemoveFromStore {
