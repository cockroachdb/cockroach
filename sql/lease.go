...
func (s LeaseStore) Acquire(txn *client.Txn, tableID ID, minVersion DescriptorVersion) (*LeaseState, error) {
	...
	values, err := p.queryRow(getDescriptor, golangParameters{int(tableID)})
	if err != nil {
		return nil, err
	}
	if values == nil {
	...
	err = s.db.Txn(func(txn *client.Txn) error {
		...
		const insertLease = `INSERT INTO system.lease (descID, version, nodeID, expiration) ` +
			`VALUES ($1, $2, $3, $4)`
		count, err := p.exec(insertLease, golangParameters{lease.ID, int(lease.Version), s.nodeID, lease.expiration})
		if err != nil {
			return err
		}
		if count != 1 {
...
func (s LeaseStore) Release(lease *LeaseState) error {
	return s.db.Txn(func(txn *client.Txn) error {
		...
		count, err := p.exec(deleteLease, golangParameters{lease.ID, int(lease.Version), s.nodeID, lease.expiration})
		if err != nil {
			return err
		}
		if count != 1 {
...
func (s LeaseStore) countLeases(descID ID, version DescriptorVersion, expiration time.Time) (int, error) {
	...
	err := s.db.Txn(func(txn *client.Txn) error {
		...
		values, err := p.queryRow(countLeases, golangParameters{descID, int(version), expiration})
		if err != nil {
			return err
		}
