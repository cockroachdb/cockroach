...
func (n *alterTableNode) Start() *roachpb.Error {
	...
	for _, cmd := range n.n.Cmds {
		...
		case *parser.AlterTableAddConstraint:
			...
			default:
				return roachpb.NewErrorf("unsupported constraint: %T", t.ConstraintDef)
			}
		...
		default:
			return roachpb.NewErrorf("unsupported alter cmd: %T", cmd)
		}
	}
	...
	if pErr := n.p.writeTableDesc(n.tableDesc); pErr != nil {
		return pErr
	}
...
func (n *alterTableNode) ExplainTypes(fn func(string, string)) {}
...
func applyColumnMutation(col *ColumnDescriptor, mut parser.ColumnMutationCmd) error {
	...
	case *parser.AlterTableSetDefault:
		if t.Default == nil {
			...
			typedExpr, err := parser.TypeCheck(t.Default, nil, colDatumType)
			if err != nil {
				return err
			}
			...
			if parser.ContainsVars(typedExpr) {
				return fmt.Errorf("default expression contains placeholders")
			}
			s := t.Default.String()
			col.DefaultExpr = &s
		}
