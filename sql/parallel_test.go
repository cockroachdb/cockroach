...
//
// Each test lives in a separate subdir under testdata/paralleltest. Each test dir
// contains a "main" file along with a set of files in logic test format.
//
// The format of the main file is very simple (for now). Each line is of the form
// 	run <file1> <file2> ..
// This will run the given logic test files in parallel and wait for them to complete.
...
var (
	paralleltestdata = flag.String("partestdata", "testdata/paralleltest/[^.]*", "test data glob")
)
...
func (t *parallelTest) processTestFile(path string, db *sql.DB, ch chan bool) {
	...
	l := &logicTest{T: t.T, db: db, user: security.RootUser}
