...
func (t *parallelTest) setup(spec *parTestSpec) {
	...
	if spec.RangeSplitSize != 0 {
		...
		cfg.RangeMaxBytes = int64(spec.RangeSplitSize)
		cfg.RangeMinBytes = cfg.RangeMaxBytes / 2
		defer config.TestingSetDefaultZoneConfig(cfg)()
		if testing.Verbose() || log.V(1) {
			fmt.Printf("Range split size: %d\n", spec.RangeSplitSize)
		}
	}
	...
	sqlutils.MakeSQLRunner(t, t.clients[0][0]).Exec("CREATE DATABASE test")
	for i := range t.clients {
