...
func (td *tableDeleter) deleteIndex(
	ctx context.Context, idx *sqlbase.IndexDescriptor, resume roachpb.Span, bound int64,
) (roachpb.Span, error) {
	if len(idx.Interleave.Ancestors) > 0 || len(idx.InterleavedBy) > 0 {
		if log.V(2) {
			log.Info(ctx, "delete forced to scan: table is interleaved")
		}
		return td.deleteIndexScan(ctx, idx, resume, bound)
	}
	return td.deleteIndexFast(ctx, idx, resume, bound)
}
...
func (td *tableDeleter) deleteIndexFast(
	...
	td.b.Header.MaxSpanRequestKeys = bound
