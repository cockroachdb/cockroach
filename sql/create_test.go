...
func verifyTables(t *testing.T,
	...
	for id := range completed {
		...
		insertTableSQL := fmt.Sprintf(`
			...
			VALUES ($1, $1)
		`, tableName)
		db := tc.ServerConn(count % tc.NumServers())
		if _, err := db.Exec(insertTableSQL, count); err != nil {
			t.Fatal(err)
		}
		...
		if otherTableName, ok := usedTableIDs[tableDesc.ID]; ok && !duplicateIDsAllowed {
	...
	numTablesCreated := len(usedTableIDs)
