...
func (p *planner) RenameDatabase(n *parser.RenameDatabase) (planNode, error) {
	...
	p.verifyMetadata = func(systemConfig *config.SystemConfig) error {
		if v := systemConfig.GetValue(newKey); v != nil {
			id, err := v.GetInt()
			if err != nil {
				return err
			}
		...
		if v := systemConfig.GetValue(descKey); v != nil {
			var desc Descriptor
		...
		if systemConfig.GetValue(oldKey) != nil {
			return errStaleMetadata
		}
		return nil
	}
...
func (p *planner) RenameTable(n *parser.RenameTable) (planNode, error) {
	...
	p.verifyMetadata = func(systemConfig *config.SystemConfig) error {
		if v := systemConfig.GetValue(newTbKey); v != nil {
			id, err := v.GetInt()
			if err != nil {
				return err
			}
		...
		if systemConfig.GetValue(tbKey) != nil {
			return errStaleMetadata
		}
		return nil
	}
