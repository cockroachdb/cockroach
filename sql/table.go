...
func makeColumnDefDescs(d *parser.ColumnTableDef) (*ColumnDescriptor, *IndexDescriptor, error) {
	...
	// Check_constraint says that if the constraint refers to a single column
	// only, it is possible to specify the constraint as part of the column
	// definition. Postgres allows specifying them anywhere about any columns,
	// but it moves all constraints to the table level (i.e., columns never have
	// a check constraint themselves). We will adhere to the stricter definition.
	if d.CheckExpr != nil {
