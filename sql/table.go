...
func getKeysForTableDescriptor(tableDesc *TableDescriptor) (
...
// found.
func (p *planner) getTableDescEx(qname *parser.QualifiedName) (
...
func getTableDescFromID(txn *client.Txn, id ID) (*TableDescriptor, *roachpb.Error) {
	...
	// is some other type of descriptor other than a TableDescriptor.
	...
	if !found {
		...
		if pErr != nil {
			if _, ok := pErr.GetDetail().(*roachpb.DescriptorNotFoundError); ok {
				...
				// table's name.
				return TableDescriptor{}, tableDoesntExistError(*qname)
			}
			return TableDescriptor{}, pErr
		}
...
func tableDoesntExistError(qname parser.QualifiedName) *roachpb.Error {
	return roachpb.NewError(fmt.Errorf("table %q does not exist", qname.TableName()))
}
...
func (p *planner) getTableID(qname *parser.QualifiedName) (ID, *roachpb.Error) {
	...
	if !gr.Exists() {
		return 0, tableDoesntExistError(*qname)
	}
