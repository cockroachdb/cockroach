...
type SchemaAccessor interface {
	...
	//    table
	//    *
	expandTableGlob(expr *parser.QualifiedName) (parser.TableNames, error)
...
func (p *planner) getTableLease(tn *parser.TableName) (*sqlbase.TableDescriptor, error) {
	if log.V(2) {
		log.Infof(context.TODO(), "planner acquiring lease on table %s", tn)
	}

	if tn.Database() == sqlbase.SystemDB.Name || testDisableTableLeases {
	...
	for _, l := range p.leases {
		if sqlbase.EqualName(l.Name, tn.Table()) &&
			l.ParentID == dbID {
			lease = l
			if log.V(2) {
				log.Infof(context.TODO(), "found lease in planner cache for table %s", tn)
			}
			break
		}
	}
...
func (p *planner) getAliasedTableName(n parser.TableExpr) (*parser.TableName, error) {
	...
	return table.NormalizeTableNameWithDatabaseName(p.session.Database)
}
...
func (p *planner) expandTableGlob(expr *parser.QualifiedName) (
	...
	if err := expr.NormalizeTablePattern(); err != nil {
		return nil, err
	}

	if t, ok := expr.Target.(*parser.TableName); ok {
	...
	glob := expr.Target.(*parser.AllTablesSelector)
