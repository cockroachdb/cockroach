...
func (e virtualTableEntry) getPlanNode(p *planner) (planNode, error) {
	...
	err := e.populate(p, func(datum ...parser.Datum) {
...
// pair. The function will return the table's entry if the qname matches a specific
// table. It will return an error if the qname references a virtual database but the table
// is non-existent.
func getVirtualTable(qname *parser.QualifiedName) (virtualTableEntry, error) {
