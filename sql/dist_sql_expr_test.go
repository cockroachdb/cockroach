...
func (d testVarContainer) IndexedVarTypeCheck(idx int, args parser.MapArgs) (parser.Datum, error) {
	return nil, nil
}
...
func TestProcessSQLExpression(t *testing.T) {
	defer leaktest.AfterTest(t)()

	e := SQLExpression{Expr: "$0 * ($1 + $2) + $0"}
	h := parser.MakeIndexedVarHelper(testVarContainer{}, 4)
	expr, err := processSQLExpression(e, &h)
	if err != nil {
		t.Fatal(err)
	}
