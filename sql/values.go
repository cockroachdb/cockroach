...
func (p *planner) Values(n parser.Values) (planNode, *roachpb.Error) {
	...
	for num, tuple := range n {
		if num == 0 {
			v.columns = make([]outputColumn, 0, len(tuple))
		...
		for i := range tuple {
			...
			if num == 0 {
				v.columns = append(v.columns, outputColumn{name: "column" + strconv.Itoa(i+1), typ: typ})
...
func (n *valuesNode) Columns() []outputColumn {
	return n.columns
}
