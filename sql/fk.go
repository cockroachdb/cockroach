...
func (p *planner) fillFKTableMap(m TablesByID) error {
	...
	for k := range m {
		...
		if m[k], err = getTableDescFromID(p.txn, k); err != nil {
			return err
		}
	}
	return nil
...
func makeFKInsertHelper(
	txn *client.Txn, table *sqlbase.TableDescriptor, others TablesByID, colMap map[sqlbase.ColumnID]int,
) (fkInsertHelper, error) {
	var fks fkInsertHelper
	for _, idx := range table.AllNonDropIndexes() {
		if idx.ForeignKey != nil {
			fk, err := makeBaseFKHelper(txn, others, idx, idx.ForeignKey, colMap)
			if err != nil {
				return fks, err
			}
			if fks == nil {
...
func makeFKDeleteHelper(
	...
	for _, idx := range table.AllNonDropIndexes() {
		for _, ref := range idx.ReferencedBy {
			fk, err := makeBaseFKHelper(txn, others, idx, ref, colMap)
			if err != nil {
				return fks, err
			}
			if fks == nil {
...
func makeFKUpdateHelper(
	...
	ret := fkUpdateHelper{}
	var err error
	if ret.inbound, err = makeFKDeleteHelper(txn, table, others, colMap); err != nil {
		return ret, err
	}
	ret.outbound, err = makeFKInsertHelper(txn, table, others, colMap)
	return ret, err
}
...
func makeBaseFKHelper(
	...
	searchTable, ok := others[ref.Table]
	if !ok {
		return b, util.Errorf("referenced table %d not in provided table map %+v", ref.Table, others)
	}
