...
type baseFKHelper struct {
	...
	prefix      []byte                   // prefix of keys in searchIdx
...
func makeBaseFKHelepr(
	...
	b := baseFKHelper{txn: txn, writeIdx: writeIdx, prefix: ref.IndexKeyPrefix()}
...
func (f baseFKHelper) check(values parser.DTuple) (parser.DTuple, error) {
	keyBytes, _, err := sqlbase.EncodeIndexKey(f.searchIdx, f.ids, values, f.prefix)
	if err != nil {
		return nil, err
	}
