	...
	if len(dropped) > 0 || nonNullableColumn != "" || len(defaultExprs) > 0 {
		...
		for done := false; !done; {
			...
			*lease = l
			...
			sp.start, done, pErr = sc.truncateAndBackfillColumnsChunk(
...
func (sc *SchemaChanger) truncateAndBackfillColumnsChunk(
	...
	return curSentinel.PrefixEnd(), done, pErr
}
...
func (sc *SchemaChanger) backfillIndexes(
	...
	for done := false; !done; {
		...
		*lease = l
		...
		sp.start, done, pErr = sc.backfillIndexesChunk(added, sp)
		if pErr != nil {
			return pErr
		}
	}
