...
func (p *planner) backfillBatch(b *client.Batch, tableDesc *TableDescriptor) *roachpb.Error {
	...
	var defaultExprs map[ColumnDescriptor]parser.Expr

	// Add new columns and Create default expressions.
	// TODO(seif): this is duplicating code from insert.go and that we should find a way not do that.
