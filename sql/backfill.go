...
var schemaChangeNotPendingError = roachpb.NewErrorf(
	"the schema change is not at the front of the mutation queue",
)

// getTableDescriptor return the table descriptor for the schema change. It
// returns schemaChangeNoPendingWorkError if the schema change mutation is not
// at the front of the mutation queue.
func (sc *SchemaChanger) getTableDesc(txn *client.Txn) (*TableDescriptor, *roachpb.Error) {
	table, err := getTableDescFromID(txn, sc.tableID)
	if err != nil {
		return nil, err
	}
	if len(table.Mutations) == 0 || sc.mutationID != table.Mutations[0].MutationID {
		return nil, schemaChangeNotPendingError
	}
	return table, nil
}

// runBackfill runs the backfill for the schema changer. It short-circuits and
// returns immediately if the schema change is not at the front of the table
// descriptor mutation queue.
func (sc *SchemaChanger) runBackfill(lease *TableDescriptor_SchemaChangeLease) *roachpb.Error {
	pErr := sc.runBackfillIfPending(lease)
	if pErr == schemaChangeNotPendingError {
		// Swallow this error. It can happen when another node has executed
		// the schema change or when the table gets deleted.
		pErr = nil
	}
	return pErr
}

func (sc *SchemaChanger) runBackfillIfPending(lease *TableDescriptor_SchemaChangeLease) *roachpb.Error {
	...
	*lease = l
	...
	if pErr := sc.db.Txn(func(txn *client.Txn) *roachpb.Error {
		tableDesc, pErr := sc.getTableDesc(txn)
		if pErr != nil {
			return pErr
		}
