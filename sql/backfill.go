...
func convertBackfillError(tableDesc *TableDescriptor, b *client.Batch, pErr *roachpb.Error) *roachpb.Error {
...
// getTableDescAtVersion attempts to read a descriptor from the database
// at a specific version.
func getTableDescAtVersion(txn *client.Txn, id ID, version DescriptorVersion) (*TableDescriptor, *roachpb.Error) {
...
// It runs the entire backfill at a specific version of the
// table descriptor, and re-attempts to run the schema change
...
// Run the backfill at the latest table descriptor version.
// It returns descriptorChangedVersionError when the table
// descriptor version changes while it is running the backfill.
func (sc *SchemaChanger) runBackfillAtLatestVersion(lease *TableDescriptor_SchemaChangeLease) *roachpb.Error {
	...
	*lease = l
	...
	// of mutations. Collect the elements that are part of the mutation.
	...
	parse := parser.Parser{}
	defaultExprs, err := makeDefaultExprs(newColumnDescs, parse, evalCtx)
	if err != nil {
