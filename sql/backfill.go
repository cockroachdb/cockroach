...
var _ sort.Interface = indexDescriptorSlice{}
...
type indexDescriptorSlice []IndexDescriptor

func (ids indexDescriptorSlice) Len() int {
	return len(ids)
}
func (ids indexDescriptorSlice) Less(i, j int) bool {
	return ids[i].ID < ids[j].ID
}
func (ids indexDescriptorSlice) Swap(i, j int) {
...
func (p *planner) backfillBatch(b *client.Batch, tableName *parser.QualifiedName, oldTableDesc, newTableDesc *TableDescriptor) error {
	...
	sort.Sort(indexDescriptorSlice(oldTableDesc.Indexes))
	sort.Sort(indexDescriptorSlice(newTableDesc.Indexes))
