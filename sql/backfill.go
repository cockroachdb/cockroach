...
// specific version. It returns errDescriptorChangedVersion if the current
...
func getTableDescAtVersion(
	...
	if version != tableDesc.Version {
		return nil, errDescriptorChangedVersion
	}
...
func (sc *SchemaChanger) runBackfill(lease *TableDescriptor_SchemaChangeLease) error {
	for {
		...
		if err == errDescriptorChangedVersion {
			continue
		}
		return err
	}
...
// errDescriptorChangedVersion when the table descriptor version changes
