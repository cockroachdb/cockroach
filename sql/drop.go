...
import (
	...
	"github.com/cockroachdb/cockroach-back/util"
...
func (n *dropTableNode) Start() error {
	for _, droppedDesc := range n.td {
		...
		for _, idx := range droppedDesc.AllNonDropIndexes() {
			if idx.ForeignKey != nil {
				if err := n.p.removeFKBackReference(droppedDesc, idx); err != nil {
					return err
				}
			}
			for _, ref := range idx.ReferencedBy {
				// Nil forces re-fetching tables, since they may have been modified.
				if err := n.p.removeFK(ref, nil); err != nil {
					return err
				}
			}
		}
...
func (p *planner) removeFKBackReference(
	...
	if err != nil {
		return util.Errorf("error resolving referenced table ID %d: %v", idx.ForeignKey.Table, err)
	}
