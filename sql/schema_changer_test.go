...
func TestSchemaChangeRetry(t *testing.T) {
	...
	defer csql.TestSpeedupAsyncSchemaChanges()()
	// Disable async schema change execution to allow predictable processing
	// of schema changes through synchronous path. Enable async schema change
	// processing right at the end.
...
`); err != nil {
	...
	defer func(backfillChunkSize int) {
		csql.IndexBackfillChunkSize = backfillChunkSize
	}(csql.IndexBackfillChunkSize)
