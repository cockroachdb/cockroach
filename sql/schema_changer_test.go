...
func TestSchemaChangePurgeFailure(t *testing.T) {
	...
	// mu protects disableAsyncSchemaChanges.
	var mu sync.Mutex
	// Disable the async schema changer.
	disableAsyncSchemaChanges := true
	...
	ctx.TestingKnobs.SQLExecutor = &csql.ExecutorTestingKnobs{
		...
		AsyncSchemaChangerExecNotification: func() error {
			mu.Lock()
			defer mu.Unlock()
			if disableAsyncSchemaChanges {
				return errors.New("don't process schema change")
			}
			return nil
		},
	}
...
`); err != nil {
	...
	mu.Lock()
	disableAsyncSchemaChanges = false
	mu.Unlock()
