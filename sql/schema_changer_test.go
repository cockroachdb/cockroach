...
`); err != nil {
	...
	if err := db.Txn(func(txn *client.Txn) error {
		t := sql.NewSchemaChangerForTesting(id, node, txn)
		var err error
		lease, err = t.AcquireLease()
		return err
	}); err != nil {
		t.Fatal(err)
	}
	...
	if err := db.Txn(func(txn *client.Txn) error {
		t := sql.NewSchemaChangerForTesting(id, node, txn)
		var err error
		newLease, err = t.ExtendLease(lease)
		return err
	}); err != nil {
		t.Fatal(err)
	}
	...
	if err := db.Txn(func(txn *client.Txn) error {
		t := sql.NewSchemaChangerForTesting(id, node, txn)
		return t.ReleaseLease(newLease)
	}); err != nil {
		t.Fatal(err)
	}
