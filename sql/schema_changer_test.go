...
func TestTableLeaderLease(t *testing.T) {
...
`); err != nil {
	...
	var lease sql.TableDescriptor_LeaderLease
	var id = sql.ID(keys.MaxReservedDescID + 2)

	// Acquire a lease.
	if err := kvDB.Txn(func(txn *client.Txn) error {
		t := sql.NewTableLeaderLeaserForTesting(txn)
		var err error
	...
	var newLease sql.TableDescriptor_LeaderLease
	if err := kvDB.Txn(func(txn *client.Txn) error {
		t := sql.NewTableLeaderLeaserForTesting(txn)
		var err error
		_, err = t.Acquire(id)
