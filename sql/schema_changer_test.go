...
`); err != nil {
	...
	// Run two schema changes and ensure they both execute serially.
	if pErr := kvDB.GetProto(descKey, desc); pErr != nil {
		t.Fatal(pErr)
	}
	...
	// Wait until the backfill for the schema change starts.
	util.SucceedsSoon(t, func() error {
		if pErr := kvDB.GetProto(descKey, desc); pErr != nil {
			t.Fatal(pErr)
		}
	...
	// Renaming the index in the middle of the backfill will wait for the
	// backfill because the backfill has the schema change lease on the table.
	if _, err := sqlDB.Exec("ALTER INDEX t.test@bar RENAME TO vidx"); err != nil {
		t.Fatal(err)
	}
	...
	case <-notify:
	default:
		t.Fatal("index creation schema change is not yet complete")
	}
}
