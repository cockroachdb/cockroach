...
func (p *planner) Show(n *parser.Show) (planNode, *roachpb.Error) {
	...
	v := &valuesNode{columns: []outputColumn{{name: name, typ: parser.DummyString}}}
...
func (p *planner) ShowDatabases(n *parser.ShowDatabases) (planNode, *roachpb.Error) {
	...
	v := &valuesNode{columns: []outputColumn{{name: "Database", typ: parser.DummyString}}}
	for _, row := range sr {
...
func (p *planner) ShowTables(n *parser.ShowTables) (planNode, *roachpb.Error) {
	...
	v := &valuesNode{columns: []outputColumn{{name: "Table", typ: parser.DummyString}}}
