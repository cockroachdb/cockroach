...
// during development cycles, and regular (nightly) larger-scale
...
//
// -verbosity N   Logging verbosity level.
//
// The output generated by the following flags is suppressed unless
// either -v is given, -verbosity N with N>=1, or a test fails.
//
...
type logicQuery struct {
	...
	expectedHash string
	// expectedValues indicate the number of rows expected when
	// expectedHash is set.
	expectedValues int
}
...
type logicTest struct {
	*testing.T
	...
	traceFile *os.File
	// verbose indicate whether -v or -verbosity N with N>=1 was passed.
	verbose bool
...
func (t *logicTest) processTestFile(path string) error {
	...
	for s.Scan() {
		if *maxErrs > 0 && t.failures >= *maxErrs {
			t.Fatalf("too many errors encountered")
		}
