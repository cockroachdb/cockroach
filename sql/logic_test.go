...
//
// TestLogic is used both for direct unit testing of CockroachDB
// during development cycles, and regular (nightly) stress testing of
// the product.
//
...
// for Sqllogictest and Test-Script is
...
// directives.
//
// The Test-Script is line-oriented. It supports both statements which
...
//
// - each test is instantiated either as a logicStatement or
//   logicQuery object. These are then processed by either
//   logicTest.execStatement() or logicTest.execQuery().
//
// TestLogic has three main parameter groups:
//
// - Selection of the input files.
// - How and how early to stop when tests fail.
// - What gets reported.
//
...
// How and how early to stop when tests fail:
//
// -try-harder    continue testing after the first error encountered.
//                This can help find more errors to troubleshoot during
//                one development iteration.
//
// -max-errors N  when -try-harder is specified, this stops testing
//                after N errors have been encountered. Default 10.
//
// -soft          tolerate / ignore errors that occur during query
//                preparation. With -soft you can indicate that it is
//                OK as long as the database reports early to the
//                client that it does not support a given query.
//                Errors are still reported if queries fail during
//                execution only or if a statement fails.
//
// -flex-types    tolerate when a result column is produced
//                with another numeric type than the one expected by
//                the test. This enables reusing tests designed for
//                database with sligtly different typing semantics.
//
// What gets reported:
//
// -v             be more verbose. This causes database log messages
//                to be output alongside the test results. It also
//                enables progress messages during long tests.
//
...
//                failure.
//                Works only in combination with -v.
//
// -error-summary produces a report organized by error message
//                of all queries that have caused that error.
//                Useful with -soft and/or -flex-types.
//                Works only in combination with -v.
//
...
// - For compatibility testing: add -soft -flex-types.
...
var (
	...
	// Testing mode
	tryHarder = flag.Bool("try-harder", false,
		"do not stop the test at the first query error encountered")
	maxErrsPerInput = flag.Int("max-errors", 10,
		"stop processing each input file after this number of errors (set to 0 for no limit)")
	...
	errorSummary = flag.Bool("error-summary", false,
		"print a per-error summary of failing queries at the end of testing, when -soft is set")
...
type logicStatement struct {
	...
	// succeed.
	expectErr string
	// expected pgcode for the error, if any. "" indicates no particular
	// pgwire error code is expected.
	expectErrCode string
}

// logicSorter is the common type for result sorter functions.
// Sorting occurs (or not) depending on Test-Script's sorting option
// for a "query" test. See the implementation of the "query" directive
// below for details.
...
type logicQuery struct {
	...
	pos string
	sql string
	// colTypes indicate the expected result column types.
	colTypes string
	// colNames tells to check whether the result columns have the same
	// names as indicated in the test input.
	colNames bool
	// label annotates queries so as to check that queries referring to
	// the same label produce the same output.
	label string
	...
	expectErr     string
	expectErrCode string
	// expectedResults indicate the expected result values, indicated as
	// the sequence of output text words that would be produced by
	// printing all rows in plain text as in a SQL shell. Note, this
	// does not specify row separators.
	expectedResults []string
	// expectedResultsRaw indicate the same as expectedResults, but
	...
	expectedHash string
	// expectedValues indicate how many rows are expected when
	// expectedHash is set.
	expectedValues int
}
...
type logicTest struct {
	*testing.T
	...
	// to prepare errors, when -soft is set.
	unsupported int
	...
	verbose bool
	// softSummary retains the per-error list of failing queries when
	// -error-summary is set.
...
// when -soft is specified. The argument "sql" is "" to indicate the
...
func (t *logicTest) unexpectedError(sql string, pos string, err error) bool {
	if *softTest && sql != "" {
		// This is a query and -soft is set.  Try to prepare the query. If
		// prepare fails, this means we (probably) do not support the
		// input syntax, and -soft instructs us to ignore the unexpected
		// error.
				...
				case 'I':
					if valT != reflect.Int64 {
						if *flexTypes && (valT == reflect.Float64 || valT == reflect.Slice) {
							t.signalSoftError(fmt.Errorf("result type mismatch: expected I, got %T", val), query.pos, query.sql)
						} else {
				...
				case 'R':
					if valT != reflect.Float64 && valT != reflect.Slice {
						if *flexTypes && (valT == reflect.Int64) {
							t.signalSoftError(fmt.Errorf("result type mismatch: expected R, got %T", val), query.pos, query.sql)
						} else {
...
// -soft or -flex-types are specified.
...
// signalSoftError registers a failing but ignored query.
func (t *logicTest) signalSoftError(err error, pos string, sql string) {
	if *errorSummary {
		errmsg, expected := splitLastMessagePart(shortenString(fmt.Sprintf("%s", err)))
		var buf bytes.Buffer
		fmt.Fprintf(&buf, "-- %s (%s)\n%s", pos, expected, shortenString(sql+";"))
		t.softSummary[errmsg] = append(t.softSummary[errmsg], buf.String())
	}
	t.unsupported++
}

// Errorf overloads testing.T.Errorf so that the test can fail early
// if -try-harder was not specified. It also handles printing the
// per-query "FAIL" marker when -show-sql is set.
