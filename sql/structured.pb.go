...
const (
	...
	// (column or index data) can only be deleted once all the
	// nodes have transitioned to the DELETE_ONLY state.
...
type ColumnDescriptor struct {
	...
	// The mutation state for this descriptor. A null value implies no
	// outstanding mutation; the presence of a mutation disables column
	// reads.
	Mutation *DescriptorMutation `protobuf:"bytes,6,opt,name=mutation" json:"mutation,omitempty"`
}
...
func (m *TableDescriptor) MarshalTo(data []byte) (int, error) {
	...
	n3, err := m.ModificationTime.MarshalTo(data[i:])
	if err != nil {
		return 0, err
	}
	i += n3
	...
	n4, err := m.PrimaryIndex.MarshalTo(data[i:])
	if err != nil {
		return 0, err
	}
	i += n4
...
func (m *Descriptor) MarshalTo(data []byte) (int, error) {
	...
	_ = l
	if m.Union != nil {
		nn7, err := m.Union.MarshalTo(data[i:])
		if err != nil {
			return 0, err
		}
		i += nn7
...
func (m *Descriptor_Database) MarshalTo(data []byte) (int, error) {
	...
	if m.Database != nil {
		...
		n9, err := m.Database.MarshalTo(data[i:])
		if err != nil {
			return 0, err
		}
		i += n9
