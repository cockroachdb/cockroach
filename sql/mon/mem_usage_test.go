...
func TestMemoryUsageMonitor(t *testing.T) {
	...
	if err := m.reserveMemory(10, ctx); err == nil {
	...
	if err := m.reserveMemory(91, ctx); err == nil {
		t.Fatalf("monitor accepted excessive allocation: %v", err)
	}
	if err := m.reserveMemory(90, ctx); err != nil {
	...
	m.releaseMemory(90, ctx) // Should succeed without panic.
	...
	m.releaseMemory(10, ctx) // Should succeed without panic.
	if m.curAllocated != 0 {
...
func TestAllocSpan(t *testing.T) {
	...
	var a1, a2 AllocationSpan

	m.StartMonitor()
	m.maxAllocatedBudget = 100

	if err := m.ExtendSpan(&a1, 10, ctx); err != nil {
		t.Fatalf("monitor refused allocation: %v", err)
	}

	if err := m.ExtendSpan(&a2, 30, ctx); err != nil {
		t.Fatalf("monitor refused allocation: %v", err)
	}

	if err := m.ExtendSpan(&a1, 61, ctx); err == nil {
		t.Fatalf("monitor accepted excessive allocation")
	}

	if err := m.ExtendSpan(&a2, 61, ctx); err == nil {
		t.Fatalf("monitor accepted excessive allocation")
	}
	...
	if err := m.ResizeItem(&a2, 50, 60, ctx); err == nil {
		t.Fatalf("monitor accepted excessive allocation")
	}

	if err := m.ResizeItem(&a1, 0, 5, ctx); err != nil {
		t.Fatalf("monitor refused allocation: %v", err)
	}

	if err := m.ResetSpanAndAlloc(&a2, 40, ctx); err != nil {
