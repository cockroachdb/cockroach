...
func TestMemoryAllocations(t *testing.T) {
	...
	checkInvariants := func() {
		...
		if pool.mu.cur > pool.max {
			t.Errorf("pool cur %d exceeds max %d", pool.mu.cur, pool.max)
			fail = true
		}
		if m.curBudget.curAllocated != pool.mu.cur {
			t.Errorf("monitor budget %d different from pool cur %d", m.curBudget.curAllocated, pool.mu.cur)
			fail = true
		}
	...
	if log.V(2) {
		...
		reportAndCheck = func(extraFmt string, extras ...interface{}) {
			fmt.Printf("%5d %5d %5d %5d ", m.curAllocated, m.curBudget.curAllocated, m.preBudget.curAllocated, pool.mu.cur)
			for accI := range accs {
		...
		for _, hf := range hysteresisFactors {
			...
			for _, pb := range preBudgets {
				...
				for _, pa := range poolAllocSizes {
					...
					m.StopMonitor(ctx)
					if pool.mu.cur != 0 {
						t.Fatalf("pool not empty after monitor close: %d", pool.mu.cur)
					}
				}
			}
		}
