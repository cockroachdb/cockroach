...
func TestMemoryAllocations(t *testing.T) {
	...
	checkInvariants := func() {
		...
		if m.curAllocated < 0 {
			t.Errorf("monitor current count went negative: %d", m.curAllocated)
			fail = true
		}
		if sum != m.curAllocated {
			t.Errorf("total account sum %d different from monitor count %d", sum, m.curAllocated)
			fail = true
		}
		if m.curBudget.curAllocated < 0 {
			t.Errorf("monitor current budget went negative: %d", m.curBudget.curAllocated)
			fail = true
		}
		avail := m.curBudget.curAllocated + m.preBudget.curAllocated
		if sum > avail {
			t.Errorf("total account sum %d greater than total monitor budget %d", sum, avail)
			fail = true
		}
		if pool.mu.curBytes > pool.maxBytes {
			t.Errorf("pool cur %d exceeds max %d", pool.mu.curBytes, pool.maxBytes)
			fail = true
		}
		if m.curBudget.curAllocated != pool.mu.curBytes {
			t.Errorf("monitor budget %d different from pool cur %d", m.curBudget.curAllocated, pool.mu.curBytes)
			fail = true
		}
	...
	if log.V(2) {
		...
		reportAndCheck = func(extraFmt string, extras ...interface{}) {
			fmt.Printf("%5d %5d %5d %5d ", m.curAllocated, m.curBudget.curAllocated, m.preBudget.curAllocated, pool.mu.curBytes)
			for accI := range accs {
		...
		for _, hf := range hysteresisFactors {
			...
			for _, pb := range preBudgets {
				...
				for _, pa := range poolAllocSizes {
					...
					m.StartMonitor(&pool, MemoryAccount{curAllocated: pb})
					m.poolAllocationSize = pa
					...
					m.StopMonitor(ctx)
					if pool.mu.curBytes != 0 {
						t.Fatalf("pool not empty after monitor close: %d", pool.mu.curBytes)
					}
				}
			}
		}
...
func TestMemoryAccount(t *testing.T) {
	...
	m := MemoryUsageMonitor{}
	pool := MakeMemoryPool(100)
	m.StartMonitor(&pool, MemoryAccount{})
