...
// in a span. The reservation is not modified if the new allocation is
// refused, so that the caller can keep using the original item
// without an accounting error. This is better than calling ResetSpan
// then ExtendSpan because if the Reset succeeds and the Extend fails
