...
func checkTableData(t *testing.T, kvDB *client.DB, sqlDB *sql.DB, descKey roachpb.Key, desc *csql.Descriptor, state csql.DescriptorMutation_State, e [][]string, returnWithMutation bool) {
	...
	if tableDesc.ColumnMutations != nil && len(tableDesc.ColumnMutations) > 0 {
		...
		tableDesc.ColumnMutations = tableDesc.ColumnMutations[1:]
	...
	if returnWithMutation {
		tableDesc.ColumnMutations = append(tableDesc.ColumnMutations, csql.ColumnMutation{Column: tableDesc.Columns[len(tableDesc.Columns)-1], Mutation: csql.DescriptorMutation{State: state}})
