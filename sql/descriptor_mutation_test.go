...
func checkTableData(t *testing.T, kvDB *client.DB, sqlDB *csql.DB, descKey roachpb.Key, desc *sql.Descriptor, state sql.DescriptorMutation_State, e [][]string, revert bool) {
	// Remove mutation to check real values in DB using SQL
	tableDesc := desc.GetTable()
	id, err := tableDesc.FindColumnByName("i")
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(cols) != len(e[0]) {
		t.Fatal(fmt.Sprintf("wrong number of columns %d", len(cols)))
	}
	...
	for ; rows.Next(); i++ {
		if i >= len(e) {
			t.Fatal(fmt.Sprintf("more rows than expected:%d, %v", len(e), e))
		}
		...
		for j, v := range vals {
			if val := *v.(*interface{}); val != nil {
				s := fmt.Sprint(val)
				if e[i][j] != s {
					t.Fatal(fmt.Sprintf("e:%v, v:%v", e[i][j], s))
				}
			} else if e[i][j] != "NULL" {
				t.Fatal(fmt.Sprintf("e:%v, v:%v", e[i][j], "NULL"))
			}
		}
	}
	if i != len(e) {
		t.Fatal(fmt.Sprintf("fewer rows read than expected: %d, e=%v", i, e))
	}
...
`); err != nil {
	...
	tableDesc.Columns[id].Mutation = &sql.DescriptorMutation{}
	...
	for _, test := range testData {
		...
		if rows, err := sqlDB.Query(`SELECT * FROM t.kv`); err != nil {
			...
			if cols, err := rows.Columns(); err != nil {
				...
				if len(cols) != 2 || cols[0] != "k" || cols[1] != "v" {
					t.Fatal(fmt.Sprintf("wrong set of columns; len=%d, columns=%v", len(cols), cols))
				}
			}
			i := 0
			for ; rows.Next(); i++ {
				...
				for j := range row {
					if row[j] != test.start[i][j] {
						t.Fatal(fmt.Sprintf("e:%v, v:%s", test.start[i][j], row[j]))
					}
				}
			}
			if i < len(test.start) {
				t.Fatal(fmt.Sprintf("fewer rows read than expected: %d, e=%v", i, test.start))
			}
		}
		...
		checkTableData(t, kvDB, sqlDB, descKey, desc, test.state, test.afterInsertWithout, true)
		...
		checkTableData(t, kvDB, sqlDB, descKey, desc, test.state, test.afterUpdateWithout, true)
