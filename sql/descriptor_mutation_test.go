...
// checkColumnIsUnreadable checks that column i is unreadable.
func (mt mutationTest) checkColumnIsUnreadable(t *testing.T, e [][]string) {
	// Column i is hidden.
...
`); err != nil {
	...
	for _, state := range []csql.DescriptorMutation_State{csql.DescriptorMutation_DELETE_ONLY, csql.DescriptorMutation_WRITE_ONLY} {
		...
		mTest.checkTableData(t, initRows)
		...
		mTest.checkColumnIsUnreadable(t, initRows)
		...
		mTest.dropMutations(t)
		mTest.checkTableData(t, afterInsert)
		...
		mTest.dropMutations(t)
		mTest.checkTableData(t, afterUpdate)
		...
		mTest.dropMutations(t)
		mTest.checkTableData(t, afterDelete)
	}
