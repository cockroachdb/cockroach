...
func (p *planner) groupBy(n *parser.Select, s *scanNode) (*groupNode, error) {
	...
	// A single copy is made up-front, rather than making several on-the-fly.
	visitor.groupedCopy = &extractAggregatesVisitor{n: group, groupStrs: visitor.groupStrs}
...
type extractAggregatesVisitor struct {
	...
	// nil indicates the visitor is in a Expr subtree that appears in the GROUP BY clause.
