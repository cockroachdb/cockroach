...
func (p *planner) groupBy(n *parser.Select, s *scanNode) (*groupNode, error) {
	...
	if aggregation, invalidAgg := checkAggregateExprs(n.GroupBy, s.render); !aggregation {
		return nil, nil
	} else if invalidAgg != nil {
		return nil, invalidAgg
	}
...
type groupNode struct {
	...
	// This will be nil until the first call to Next calls computeAggregates.
	values *valuesNode
...
func (v *checkAggregateVisitor) Visit(expr parser.Expr, pre bool) (parser.Visitor, parser.Expr) {
	...
	if t, ok := expr.(*qvalue); ok {
		...
		v.invalid = fmt.Errorf("column \"%s\" must appear in the GROUP BY clause or be used in an aggregate function", t.col.Name)
		return v, expr
	}
...
func checkAggregateExprs(group parser.GroupBy, exprs []parser.Expr) (bool, error) {
	...
	// TODO(davidt): consider other ways of comparing expression trees.
	...
	for _, expr := range exprs {
		...
		if v.invalid != nil && aggregated {
			return aggregated, v.invalid
		}
	}
