...
func (n *groupNode) Next() (bool, error) {
	...
	for !n.populated {
		...
		for i, value := range aggregatedValues {
			if err := n.funcs[i].add(&n.planner.mon, encoded, value); err != nil {
				return false, err
			}
		}
		scratch = encoded[:0]
...
func (n *groupNode) computeAggregates() error {
	...
	// Render the results.
	n.values.rows = parser.NewRowContainer(&n.planner.mon, n.values.Columns(), len(n.buckets))
...
func (a *aggregateFuncHolder) close(mon *mon.MemoryUsageMonitor) {
	a.buckets = nil
	a.seen = nil
	a.group = nil
...
func (a *aggregateFuncHolder) add(mon *mon.MemoryUsageMonitor, bucket []byte, d parser.Datum) error {
