...
func (d *deleteNode) FastPathResults() (bool, int) {
	if d.fastPath {
		return true, d.rh.results.rowCount
	}
	return false, 0
}
...
func (d *deleteNode) Next() bool {
	...
	if next {
		...
		resultRow, err := d.rh.append(rowVals)
		if err != nil {
			d.pErr = roachpb.NewError(err)
			return false
		}
		...
		// FIXME(knz): insert had the following conversion, but delete didn't.
		// I am adding it here for symmetry. Is it really needed?
		if d.pErr != nil {
			d.pErr = convertBatchError(d.tableDesc, *d.b, d.pErr)
		}
		d.done = true
		d.b = nil
	}
