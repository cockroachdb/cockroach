...
func TestMakeConstraints(t *testing.T) {
	...
	testData := []struct {
		...
		{`a != 1 AND b = 1`, []string{"a", "b"}, `[a IS NOT NULL, b = 1]`},
		{`a != 1 AND b != 1`, []string{"a", "b"}, `[a IS NOT NULL, b IS NOT NULL]`},
		{`a != 1 AND b > 1`, []string{"a", "b"}, `[a IS NOT NULL, b >= 2]`},
		{`a != 1 AND b >= 1`, []string{"a", "b"}, `[a IS NOT NULL, b >= 1]`},
...
func TestMakeSpans(t *testing.T) {
	...
	testData := []struct {
		...
		// !!! TODO(andrei): figure out how to add a test for MaxInt
		//{fmt.Sprintf(`a = %d`, math.MaxInt64), []string{"a"}, `/1-/2`},
		...
		{`a < 1 AND b = 1`, []string{"a", "b"}, `/#-/0/2`},
		{`a < 1 AND b != 1`, []string{"a", "b"}, `/#-/1`},
		{`a < 1 AND b > 1`, []string{"a", "b"}, `/#-/1`},
		{`a < 1 AND b >= 1`, []string{"a", "b"}, `/#-/1`},
		{`a < 1 AND b < 1`, []string{"a", "b"}, `/#-/0/1`},
		{`a < 1 AND b <= 1`, []string{"a", "b"}, `/#-/0/2`},
		{`a < 1 AND b IS NULL`, []string{"a", "b"}, `/#-/0/#`},
		{`a < 1 AND b IS NOT NULL`, []string{"a", "b"}, `/#-/1`},
