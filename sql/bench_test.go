...
// runBenchmarkUpdateTwoRowsTxn benchmarks updating two random rows in a single transaction.
func runBenchmarkUpdateTwoRowsTxn(b *testing.B, db *sql.DB) {
	rows := 10000
	prepareUpdateBenchData(b, db, rows)
	defer cleanupUpdateBenchData(b, db)
...
func BenchmarkUpdateTwoRowsTxn_Cockroach(b *testing.B) {
	benchmarkCockroach(b, runBenchmarkUpdateTwoRowsTxn)
}

func BenchmarkUpdateTwoRowsTxn_Postgres(b *testing.B) {
	benchmarkPostgres(b, runBenchmarkUpdateTwoRowsTxn)
}
