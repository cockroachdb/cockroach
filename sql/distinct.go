...
func (p *planner) Distinct(n *parser.SelectClause) *distinctNode {
	...
	d := &distinctNode{p: p}
	p.session.OpenAccount(&d.prefixMemAcc)
	p.session.OpenAccount(&d.suffixMemAcc)
	return d
}
...
func (n *distinctNode) addSuffixSeen(sKey string) error {
	sz := int64(len(sKey))
	if err := n.p.session.GrowAccount(&n.suffixMemAcc, sz); err != nil {
		return err
	}
	...
	for {
		...
		if !bytes.Equal(prefix, n.prefixSeen) {
			...
			if len(n.suffixSeen) > 0 {
				n.p.session.ClearAccount(&n.suffixMemAcc)
				n.suffixSeen = make(map[string]struct{})
			}
			if err := n.p.session.ClearAccountAndAlloc(&n.prefixMemAcc, int64(len(prefix))); err != nil {
				return false, err
			}
			n.prefixSeen = prefix
			if suffix != nil {
...
func (n *distinctNode) Close() {
	...
	n.prefixSeen = nil
	n.suffixSeen = nil
	n.p.session.CloseAccount(&n.suffixMemAcc)
	n.p.session.CloseAccount(&n.prefixMemAcc)
}
