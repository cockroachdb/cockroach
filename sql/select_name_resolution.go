...
func (s *selectNode) resolveNames(expr parser.Expr) (parser.Expr, error) {
	...
	return resolveNames(expr, s.sourceInfo, s.ivarHelper, v)
}

// resolveNames walks the provided expression and resolves all names
// using the tableInfo and ivarHelper. The function takes an optional
// nameResolutionVisitor to provide the caller the option of avoiding an
// allocation.
func resolveNames(
	expr parser.Expr, sources multiSourceInfo, ivarHelper parser.IndexedVarHelper, v *nameResolutionVisitor,
) (parser.Expr, error) {
	if expr == nil {
		return expr, nil
	}
