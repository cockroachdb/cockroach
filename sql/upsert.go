...
func (p *planner) makeUpsertHelper(
	...
	excludedAliasTable := tableInfo{
		...
		columnOffset: len(table.columns),
	}
	aliasedTables := []*tableInfo{&excludedAliasTable}
	...
	for _, updateExpr := range updateExprs {
		...
		resolvedExpr, err := resolveQNames(expandedExpr, &table, aliasedTables, qvals, &p.qnameVisitor)
		if err != nil {
			return nil, err
		}
	...
	helper := &upsertHelper{evalCtx: p.evalCtx, qvals: qvals, evalExprs: normExprs}
	return helper, nil
}
...
func (uh *upsertHelper) eval(
	...
	row := existingRow[:len(existingRow):len(existingRow)]
	row = append(row, insertRow...)
	uh.qvals.populateQVals(row)
...
func upsertExprsAndIndex(
	...
	return onConflict.Exprs, conflictIndex, nil
}
