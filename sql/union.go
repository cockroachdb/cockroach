...
// distinct, which always returns unique results, and all, which does no uniqueing.
//
// Ordering of rows is expected to be handled externally to unionNode.
// TODO(dan): In the long run, this is insufficient. If we know both left and right are ordered the
// same way, we can do the set logic without the map state. Additionally, if the unionNode has an
// ordering then we can hint it down to left and right and force the condition for this first
// optimization.
//
// All six of the operations can be completed without cacheing rows by iterating one side then the
// other and keeping counts of unique rows in a map. Because EXCEPT needs to iterate the right side
// first, and the other two don't care, we always read right before left.
//
// The emit logic for each op is represented by implementors of the unionNodeEmit interface. The
// emitRight method is called for each row output by the right side and passed a hashable
// representation of the row. If it returns true, the row is emitted. After all right rows are
// examined, then each left row is passed to emitLeft in the same way.
//
...
// 1
// 1
// There are three 1s on the left and two 1s on the right, so we emit 1, 1. Nothing else is in both.
//  emitRight: For each row, increment the map entry.
//  emitLeft: For each row, if the row is not present in the map, it was not in both, don't emit.
//    Otherwise, if the count for the row was > 0, emit and decrement the entry. Otherwise, the
//    row was on the right, but we've already emitted as many as were on the right, don't emit.
type unionNode struct {
	...
	rightDone   bool
	leftDone    bool
	emitAll     bool // emitAll is a performance optimization to avoid computing hashes for UNION ALL.
	emit        unionNodeEmit
	scratch     []byte
}
...
func (n *unionNode) readRight() bool {
	for {
		...
		// TODO(dan): Sending the entire encodeDTuple to be stored in the map would use a lot of
		// memory for big rows or big resultsets. Consider using a hash of the bytes instead.
...
// NB: the compiler optimizes out the string allocation in `myMap[string(myBytes)]`. See:
