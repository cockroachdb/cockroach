...
func (s *SchemaChangeManager) Start(stopper *stop.Stopper) {
	...
	stopper.RunWorker(func() {
		...
		// Creates a timer that fires and attempts to run a schema changer.
		newTimer := func() *time.Timer {
			waitDuration := time.Duration(math.MaxInt64)
			now := time.Now()
			for _, sc := range schemaChangers {
				d := sc.creationTime.Add(asyncSchemaChangeExecDelay).Sub(now)
				if d < waitDuration {
					waitDuration = d
				}
			}
			// Create a timer if there is an existing schema changer.
			if len(schemaChangers) > 0 {
		...
		for {
			...
			case <-gossipUpdateC:
				...
				oldSchemaChangers := make(map[ID]struct{}, len(schemaChangers))
				for k := range schemaChangers {
			...
			case <-timer.C:
				for _, sc := range schemaChangers {
					if time.Since(sc.creationTime) > asyncSchemaChangeExecDelay {
						...
						// Advance the creation time so that this schema changer
						// doesn't get called again for a while.
						sc.creationTime = time.Now()
					}
