...
// notifications from the node. This interfaces is only
// needed to break a circular dependency between the sql
// and server package.
...
func (sc *SchemaChanger) Start(s *stop.Stopper, gossip *gossip.Gossip, node TableLeaderInterface, db *client.DB, leaseMgr *LeaseManager) {
	s.RunWorker(func() {
		...
		for {
			select {
			case <-sc.newConfig:
				sc.readOutstandingSchemaChanges(s, tlNotify, node)
				// Check if we need to apply them.
...
func (sc *SchemaChanger) readOutstandingSchemaChanges(s *stop.Stopper, tlNotify chan TableLeaderNotification, node TableLeaderInterface) {
	...
	for _, kv := range cfg.Values {
		...
		default:
			panic(fmt.Sprintf("bad descriptor: %T, %v", union, descriptor.Union))
		}
	}
