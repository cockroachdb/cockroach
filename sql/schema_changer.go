...
func (sc SchemaChanger) exec() error {
	...
	if table.Deleted() {
		...
		if err := sc.truncateAndDropTable(sc.ctx, &lease, table); err != nil {
			return err
		}
		needRelease = false
		return nil
	...
	if sqlbase.IsIntegrityConstraintError(err) {
		log.Warningf(sc.ctx, "reversing schema change due to irrecoverable error: %s", err)
		...
		if errPurge := sc.runStateMachineAndBackfill(&lease); errPurge != nil {
			...
			log.Warningf(sc.ctx, "error purging mutation: %s, after error: %s", errPurge, err)
		}
	}
...
func (sc *SchemaChanger) waitToUpdateLeases(tableID sqlbase.ID) error {
	...
	if log.V(2) {
		log.Infof(sc.ctx, "waiting for a single version of table %d...", tableID)
	}
	_, err := sc.leaseMgr.waitForOneVersion(tableID, retryOpts)
	if log.V(2) {
		log.Infof(sc.ctx, "waiting for a single version of table %d... done", tableID)
	}
	return err
}
...
func (sc *SchemaChanger) reverseMutations(causingError error) error {
	...
	_, err := sc.leaseMgr.Publish(sc.tableID, func(desc *sqlbase.TableDescriptor) error {
		...
		for i, mutation := range desc.Mutations {
			...
			log.Warningf(sc.ctx, "reverse schema change mutation: %+v", mutation)
			switch mutation.Direction {
			case sqlbase.DescriptorMutation_ADD:
			...
			if idx := mutation.GetIndex(); idx != nil {
				...
				for _, name := range idx.ColumnNames {
					if _, ok := columns[name]; ok {
						...
						log.Warningf(sc.ctx, "delete schema change mutation: %+v", mutation)
						deleteMutation = true
						break
					}
				}
	...
	stopper.RunWorker(func() {
		...
		for {
			...
			case <-gossipUpdateC:
				...
				if log.V(2) {
					log.Info(s.ctx, "received a new config")
				}
				...
				for _, kv := range cfg.Values {
					...
					if err := kv.Value.GetProto(&descriptor); err != nil {
						log.Warningf(s.ctx, "%s: unable to unmarshal descriptor %v", kv.Key, kv.Value)
						continue
					}
					...
					case *sqlbase.Descriptor_Table:
						...
						if err := table.ValidateTable(); err != nil {
							log.Errorf(s.ctx, "%s: received invalid table descriptor: %v", kv.Key, table)
							continue
						}
						...
						if table.UpVersion || table.Deleted() || table.Adding() ||
							...
							if log.V(2) {
								log.Infof(s.ctx, "%s: queue up pending schema change; table: %d, version: %d",
					...
					if timeutil.Since(sc.execAfter) > 0 {
						...
						if err != nil {
							if err == errExistingSchemaChangeLease {
								...
								// constraints.
								log.Warningf(s.ctx, "Error executing schema change: %s", err)
							}
						}
