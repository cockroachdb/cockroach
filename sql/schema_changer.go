...
func (sc *SchemaChanger) applyMutations(holder *schemaChangeLeaseHolder) error {
	var err error
	holder.lease, err = sc.ExtendLease(holder.lease)
	if err != nil {
		return err
	}
	err = sc.db.Txn(func(txn *client.Txn) error {
		...
		b := client.Batch{}
		if err := p.backfillBatch(&b, tableDesc); err != nil {
			return err
		}
...
// This holder allows exec() to keep extending the lease
// and holding the updated lease here.
type schemaChangeLeaseHolder struct {
...
func (sc SchemaChanger) exec() error {
	...
	holder := &schemaChangeLeaseHolder{lease: lease}
	// Always try to release lease.
	defer func() {
		if err := sc.ReleaseLease(holder.lease); err != nil {
			log.Warning(err)
		}
	}()
	...
	// apply backfill.
	if err := sc.applyMutations(holder); err != nil {
		// Purge the mutations if the application of the mutations fail.
		if err := sc.purgeMutations(holder); err != nil {
			return err
		}
		return err
	}
...
func (sc *SchemaChanger) purgeMutations(holder *schemaChangeLeaseHolder) error {
	...
	stopper.RunWorker(func() {
		...
		for {
			...
			case <-gossipUpdateC:
				...
				schemaChanger := SchemaChanger{nodeID: roachpb.NodeID(s.leaseMgr.nodeID), db: s.db, leaseMgr: s.leaseMgr}
				schemaChangers = nil
