...
func (n *createTableNode) Start() error {
	...
	tableKey := tableKey{parentID: n.dbDesc.ID, name: n.n.Table.Table()}
	key := tableKey.Key()
	...
	for _, def := range n.n.Defs {
		...
		case *parser.ColumnTableDef:
			if d.References.Table != nil {
				fromCols := parser.NameList{string(d.Name)}
				var targetCol parser.NameList
				if d.References.Col != "" {
					targetCol = append(targetCol, string(d.References.Col))
				}
				if err := n.resolveFK(&desc, fromCols, d.References.Table, targetCol, d.References.ConstraintName, affected); err != nil {
					return err
				}
			}
		case *parser.ForeignKeyConstraintTableDef:
			if err := n.resolveFK(&desc, d.FromCols, d.Table, d.ToCols, d.Name, affected); err != nil {
				return err
			}
		}
	}
...
func (n *createTableNode) resolveFK(
	...
	if target == nil {
		...
		// a non-public "ADD" state and made public only after the other table is
		// updated to include the backref.
