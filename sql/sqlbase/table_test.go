...
func makeTableDescForTest(test indexKeyTest) (TableDescriptor, map[ColumnID]int) {
	...
	interleaves := make([]InterleaveDescriptor, 0, 2)
	makeInterleave := func(indexID IndexID, componentParentIDs []ID) {
		if len(componentParentIDs) > 0 {
			components := make([]InterleaveDescriptor_Component, len(componentParentIDs))
			for j, componentParentID := range componentParentIDs {
				components[j] = InterleaveDescriptor_Component{ParentID: componentParentID, IndexID: IndexID(componentParentID + 1), SharedPrefixLen: 1}
			}
			interleaves = append(interleaves, InterleaveDescriptor{IndexID: indexID, Components: components})
		}
	}
	makeInterleave(1, test.primaryInterleaves)
	makeInterleave(2, test.secondaryInterleaves)
