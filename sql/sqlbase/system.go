...
func addSystemDatabaseToSchema(target *MetadataSchema) {
	...
	// Do not add new system tables here because the tables added here are
	// only added during the bootstrapping process. All new tables should be
	// added to NewSystemTablesSchema, which contains schema for system tables
	// that are added post bootstrapping and are also added on legacy
	// clusters.
...
func systemTableID(name string) ID {
	...
	log.Warningf("system table %s not found", name)
	return 0
}
