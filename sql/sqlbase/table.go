...
func MakeIndexKeyPrefix(desc *TableDescriptor, indexID IndexID) []byte {
	var key []byte
	if i, ok := desc.FindInterleaveByIndexID(indexID); ok && len(i.Components) > 0 {
		key = encoding.EncodeUvarintAscending(key, uint64(i.Components[0].ParentID))
		key = encoding.EncodeUvarintAscending(key, uint64(i.Components[0].IndexID))
		return key
	}
...
func EncodeIndexKey(
	...
	if interleave, ok := tableDesc.FindInterleaveByIndexID(index.ID); ok {
		for i, component := range interleave.Components {
			// The first component is assumed to already be encoded in keyPrefix.
			if i != 0 {
				key = encoding.EncodeUvarintAscending(key, uint64(component.ParentID))
				key = encoding.EncodeUvarintAscending(key, uint64(component.IndexID))
			}

			l := int(component.SharedPrefixLen)
			var n bool
...
func DecodeIndexKeyPrefix(a *DatumAlloc, desc *TableDescriptor, key []byte) (
	...
	interleaves := append([]InterleaveDescriptor(nil), desc.Interleaves...)
	...
	for component := 0; ; component++ {
		...
		for i := len(interleaves) - 1; i >= 0; i-- {
			if len(interleaves[i].Components) <= component ||
				interleaves[i].Components[component].ParentID != ID(tableID) ||
				interleaves[i].Components[component].IndexID != indexID {
		...
		// use the first one.
		for i := uint32(0); i < interleaves[0].Components[component].SharedPrefixLen; i++ {
...
func DecodeIndexKey(
	...
	var err error
	var offset int
	if primaryInterleave, ok := desc.FindInterleaveByIndexID(indexID); ok {
		for _, component := range primaryInterleave.Components {
			var parentTableID uint64
			key, parentTableID, err = encoding.DecodeUvarintAscending(key)
			if err != nil {
				return nil, err
			}
			if ID(parentTableID) != component.ParentID {
				return nil, errors.Errorf("%s: unexpected table ID: %d != %d", desc.Name, component.ParentID, parentTableID)
			}
			...
			if IndexID(parentIndexID) != component.IndexID {
				return nil, errors.Errorf("%s: unexpected index ID: %d != %d", desc.Name, component.IndexID, parentIndexID)
			}
...
func ExtractIndexKey(
	...
	if _, ok := tableDesc.FindInterleaveByIndexID(indexID); ok {
