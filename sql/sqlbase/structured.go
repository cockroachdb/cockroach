...
// FamilyID is a custom type for FamilyDescriptor IDs.
...
func (desc *TableDescriptor) MaybeUpgradeFormatVersion() bool {
	...
	desc.Families = []FamilyDescriptor{
		{ID: 0, Name: "primary"},
	}
...
func (desc *TableDescriptor) AllocateIDs() error {
	...
	ensureColumnInFamily := func(col *ColumnDescriptor) {
		...
		if _, ok := primaryIndexColIDs[col.ID]; ok || desc.ParentID != keys.SystemDatabaseID {
			desc.Families[0].ColumnNames = append(desc.Families[0].ColumnNames, col.Name)
			desc.Families[0].ColumnIDs = append(desc.Families[0].ColumnIDs, col.ID)
			return
		}
		...
		desc.Families = append(desc.Families, FamilyDescriptor{
			ID:          familyID,
	...
	for i, family := range desc.Families {
		...
		if found != -1 {
			desc.Families[i].ColumnIDs = append(
				desc.Families[i].ColumnIDs[:found], desc.Families[i].ColumnIDs[found+1:]...)
			desc.Families[i].ColumnNames = append(
				desc.Families[i].ColumnNames[:found], desc.Families[i].ColumnNames[found+1:]...)
			break
		}
	}
}
