	...
	for !n.exhausted && len(n.rows) <= 1 {
		...
		for _, sp := range n.txn.CollectedSpans {
			for _, entry := range sp.Logs {
				if entry.Timestamp.Before(earliest) || earliest.IsZero() {
		...
		for _, sp := range n.txn.CollectedSpans {
			for i, entry := range sp.Logs {
				var timeVal string
				if i > 0 {
					timeVal = fmt.Sprintf("%.3fms", time.Duration(entry.Timestamp.Sub(n.lastTS)).Seconds()*1000)
				}
				sinceStart := fmt.Sprintf("%.3fms", time.Duration(entry.Timestamp.Sub(earliest)).Seconds()*1000)

				cols := append(parser.DTuple{
					parser.DString(sinceStart),
