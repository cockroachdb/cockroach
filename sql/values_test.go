...
func TestGolangParams(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			int(1),
			reflect.TypeOf(parser.DummyInt),
		},
		{
			int8(1),
			reflect.TypeOf(parser.DummyInt),
		...
		{
			float32(1.0),
			reflect.TypeOf(parser.DummyFloat),
		},
		{
			float64(1.0),
			reflect.TypeOf(parser.DummyFloat),
		...
		{
			"test",
			reflect.TypeOf(parser.DummyString),
		},
		// Bytes type.
		{
			[]byte("abc"),
			reflect.TypeOf(parser.DummyBytes),
		},

		// Interval and timestamp.
		{
			time.Duration(1),
			reflect.TypeOf(parser.DummyInterval),
		},
		{
			time.Now(),
			reflect.TypeOf(parser.DummyTimestamp),
		},
