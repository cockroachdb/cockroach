...
func databaseDoesNotExistUErr(name string) *roachpb.Error {
	return roachpb.NewUErrorf("database %q does not exist", name)
}
...
func (p *planner) getDatabaseDesc(name string) (*DatabaseDescriptor, *roachpb.Error) {
	...
	if desc == nil {
		return nil, databaseDoesNotExistUErr(name)
	}
	return desc, nil
}
...
func getKeysForDatabaseDescriptor(dbDesc *DatabaseDescriptor) (
