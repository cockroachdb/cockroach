...
func TestPGPreparedQuery(t *testing.T) {
	...
	queryTests := map[string][]preparedQueryTest{
		...
		"SELECT DISTINCT 1 FROM d.ts GROUP BY a HAVING MAX(b) <= CURRENT_DATE()": {
			base.Params().Results(1),
		},
		/* TODO(knz) fix #5355
	...
	initStmt := `CREATE DATABASE d; CREATE TABLE d.t (a INT); INSERT INTO d.t VALUES (10),(11); CREATE TABLE d.ts (a TIMESTAMP, b DATE); CREATE TABLE d.tsd (a TIMESTAMP DEFAULT NOW(), b INT);`
