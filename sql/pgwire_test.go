...
func TestPGPreparedExec(t *testing.T) {
	...
	queryTests := []struct {
		...
		{
			"CREATE TABLE d.t (i INT, s STRING)",
			nil,
		},
		{
			...
			[]preparedTest{
				base.Params(1, "one"),
				base.Params("two", 2).Error(`pq: param $1: strconv.ParseInt: parsing "two": invalid syntax`),
			},
		},
	}
	...
	for _, queryTest := range queryTests {
		if queryTest.tests == nil {
			_, err := db.Exec(queryTest.query)
			if err != nil {
				t.Fatal(err)
			}
			continue
		...
		if err != nil {
			t.Errorf("prepare error: %s: %s", queryTest.query, err)
			continue
		}
		...
		for _, test := range queryTest.tests {
			...
			if err != nil {
				if test.error == "" {
					t.Errorf("%s: %#v: unexpected error: %s", queryTest.query, test.params, err)
				} else if test.error != err.Error() {
					t.Errorf("%s: %#v: expected error: %s, got %s", queryTest.query, test.params, test.error, err)
				}
				continue
			}
			if test.error != "" && err == nil {
				t.Errorf("expected error: %s: %#v", queryTest.query, test.params)
			}
		}
		if err := stmt.Close(); err != nil {
			t.Error(err)
