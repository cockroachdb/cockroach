...
func TestPGPrepareFail(t *testing.T) {
	...
	testFailures := map[string]string{
		...
		"SELECT CASE WHEN TRUE THEN $1 END":             "pq: unsupported case expression value: parameter",
		"SELECT CASE WHEN TRUE THEN $1 ELSE $2 END":     "pq: unsupported case else type: parameter",
		"SELECT CASE WHEN TRUE THEN 1 ELSE $1 END":      "pq: unsupported case else type: parameter",
		"UPDATE d.t SET d = CASE WHEN TRUE THEN $1 END": "pq: unsupported case expression value: parameter",
