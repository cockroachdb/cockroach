...
func findColumn(table *tableInfo, qname *parser.QualifiedName) (columnRef, error) {
	...
	if qname.Base == "" {
		qname.Base = parser.Name(table.alias)
	}
	if equalName(table.alias, string(qname.Base)) {
...
func (v *qnameVisitor) Visit(expr parser.Expr, pre bool) (parser.Visitor, parser.Expr) {
	...
	case *qvalue:
		...
		colRef.table = v.table
		return v, v.qvals.getQVal(colRef)

	case *parser.QualifiedName:
		var colRef columnRef

		colRef, v.err = findColumn(v.table, t)
		if v.err != nil {
			return nil, expr
		}
		return v, v.qvals.getQVal(colRef)
