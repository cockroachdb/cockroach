...
func (ss *sortChunksStrategy) Execute(s *sorter) error {
	// pivoted is a helper function to determine if row is pivoted under a given pivot, i.e. it
	// shares the same values for the first s.matchLen ordering columns.
	pivoted := func(row, pivot sqlbase.EncDatumRow) bool {
	...
	for {
		...
		for {
			...
			if nextRow == nil || !pivoted(nextRow, pivot) {
				break
			}
		}
