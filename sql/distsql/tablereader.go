...
func (tr *tableReader) Run(wg *sync.WaitGroup) {
	...
	if log.V(2) {
		...
		log.Infoc(tr.ctx, "TableReader starting (filter: %s)", tr.filter)
		defer log.Infoc(tr.ctx, "TableReader exiting")
	}

	if err := tr.fetcher.StartScan(tr.ctx.txn, tr.spans, tr.getLimitHint()); err != nil {
		if log.V(1) {
			log.Errorc(tr.ctx, "TableReader scan error: %s", err)
		}
		tr.output.Close(err)
		return
	}
	...
	for {
		...
		if log.V(3) {
			log.Infoc(tr.ctx, "TableReader pushing row %s\n", outRow)
		}
		// Push the row to the output RowReceiver; stop if they don't need more
		// rows.
		if !tr.output.PushRow(outRow) {
			if log.V(2) {
				log.Infoc(tr.ctx, "TableReader: no more rows required")
			}
			tr.output.Close(nil)
			return
		}
