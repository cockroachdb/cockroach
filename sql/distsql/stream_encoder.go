...
// FormMessage populates a message. The StreamMessage should be treated as
// immutable.  If final is true, a message trailer is populated with the given
// error.
//
// If the returned message is used (sent), NextMessage must be called before the
// next call to FormMessage. If the returned message is not used, FormMessage
// can be called again without NextMessage (it will return the same message,
// possibly with more rows if AddRow was called in the meantime).
...
// NextMessage resets the row data for the next message. Any result previously
// returned by FormMessage is no longer valid.
func (se *StreamEncoder) NextMessage() {
