...
func newOutboxSimpleFlowStream(logCtx context.Context, stream DistSQL_RunSimpleFlowServer) *outbox {
	return &outbox{flowCtx: &FlowCtx{Context: logCtx}, simpleFlowStream: stream}
}
...
func (m *outbox) flush(last bool, err error) error {
	...
	if log.V(3) {
		log.Infoc(m.flowCtx, "flushing outbox")
	}
	var sendErr error
	if m.stream != nil {
	...
	if sendErr != nil {
		if log.V(1) {
			log.Errorc(m.flowCtx, "outbox flush error: %s", sendErr)
		}
	} else if log.V(3) {
		log.Infoc(m.flowCtx, "outbox flushed")
	}
	if sendErr != nil {
		return sendErr
	}
...
func (m *outbox) mainLoop() error {
	if m.simpleFlowStream == nil {
		...
		if log.V(2) {
			log.Infoc(m.flowCtx, "outbox: calling FlowStream")
		}
		m.stream, err = client.FlowStream(context.TODO())
		if err != nil {
			if log.V(1) {
				log.Infoc(m.flowCtx, "FlowStream error: %s", err)
			}
			return err
		}
		if log.V(2) {
			log.Infoc(m.flowCtx, "outbox: FlowStream returned")
		}
	}
