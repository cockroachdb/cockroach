...
func assertHeapPos(src *srcInfo, heapPos int) {
	if src.heapPos != heapPos {
		panic(fmt.Sprintf("source has heapPos=%d, should be %d", src.heapPos, heapPos))
	}
}
...
func (s *orderedSynchronizer) initHeap() error {
	for srcIdx := range s.sources {
		...
		if src.row != nil {
			...
			src.heapPos = len(s.heap)
			s.heap = append(s.heap, srcIdx)
		}
	}
	heap.Init(s)
