...
func newJoinReader(
	...
	if spec.IndexIdx != 0 {
		...
		panic("join with index not implemented")
	}
...
func (jr *joinReader) generateKey(
	...
	// We will append to the prefix, so preallocate a larger size.
	key := make(roachpb.Key, len(primaryKeyPrefix), len(primaryKeyPrefix)*2)
	...
	for i, val := range row {
		...
		if index.ColumnDirections[i] == sqlbase.IndexDescriptor_DESC {
			encoding = sqlbase.DatumEncoding_DESCENDING_KEY
		}
		var err error
...
// minLoop runs the mainLoop and returns any error.
