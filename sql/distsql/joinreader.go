...
func (jr *joinReader) mainLoop() error {
	...
	if log.V(2) {
		...
		log.Infoc(jr.ctx, "JoinReader starting (filter: %s)", jr.filter)
		defer log.Infoc(jr.ctx, "JoinReader exiting")
	}
	...
	for {
		...
		for {
			...
			if log.V(3) {
				log.Infoc(jr.ctx, "JoinReader pushing row %s\n", outRow)
			}
			// Push the row to the output RowReceiver; stop if they don't need more
			// rows.
			if !jr.output.PushRow(outRow) {
				if log.V(2) {
					log.Infoc(jr.ctx, "JoinReader: no more rows required")
				}
				return nil
			}
		}
...
func (jr *joinReader) Run(wg *sync.WaitGroup) {
	err := jr.mainLoop()
	if err != nil && log.V(1) {
		log.Errorc(jr.ctx, "JoinReader error: %s", err)
	}
	jr.output.Close(err)
	if wg != nil {
		wg.Done()
