...
// IsPrepared returns whether a PreparedStatement with the provided name exists.
func (ps PreparedStatements) IsPrepared(name string) bool {
	_, ok := ps.Get(name)
	return ok
}
...
// IsBound returns whether a PreparedPortal with the provided name exists.
func (pp PreparedPortals) IsBound(name string) bool {
	_, ok := pp.Get(name)
	return ok
}
