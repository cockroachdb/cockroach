...
func (v *qnameVisitor) Visit(expr parser.Expr, pre bool) (parser.Visitor, parser.Expr) {
	...
	case *parser.FuncExpr:
		...
		t.Exprs[0] = &starDatum{datum: parser.DummyInt}
		return v, expr
...
func (q *starDatum) String() string {
	return "*"
}

func (q *starDatum) Walk(v parser.Visitor) {
	q.datum = parser.WalkExpr(v, q.datum).(parser.Datum)
}

func (q *starDatum) TypeCheck(args parser.MapArgs) (parser.Datum, error) {
	return q.datum.TypeCheck(args)
}

func (q *starDatum) Eval(ctx parser.EvalContext) (parser.Datum, error) {
	return q.datum.Eval(ctx)
}
