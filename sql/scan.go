...
type scanNode struct {
	...
	originalCols     []outputColumn // copy of `columns` before additions (e.g. by sort or group)
	columnIDs        []ColumnID
...
func (n *scanNode) Columns() []outputColumn {
	return n.columns
}
...
func (n *scanNode) addRender(target parser.SelectExpr) *roachpb.Error {
		...
		if qname.IsStar() {
			...
			if n.isSecondaryIndex {
				for _, id := range n.index.ColumnIDs {
					...
					qval := n.getQVal(*col)
					n.columns = append(n.columns, outputColumn{name: col.Name, typ: qval.datum})
	...
	n.columns = append(n.columns, outputColumn{name: outputName, typ: typ})
	return nil
}
