...
func (u *updateNode) Start() *roachpb.Error {
	...
	for _, expr := range exprs {
		if expr.Tuple {
			if t, ok := expr.Expr.(parser.DTuple); ok {
				for _, e := range t {
					targets = append(targets, parser.SelectExpr{Expr: e})
					i++
				}
...
func (u *updateNode) Next() bool {
	...
	if next {
		...
		resultRow, err := u.rh.append(rowVals[:len(u.tableDesc.Columns)])
		if err != nil {
			u.pErr = roachpb.NewError(err)
			return false
		}
...
func (u *updateNode) MarkDebug(_ explainMode) {}
