...
func (p *planner) Update(n *parser.Update, autoCommit bool) (planNode, *roachpb.Error) {
	...
	// Secondary indexes needing updating. If the primary key changed, we need to
	// update all of them.
	needsUpdate := func(index IndexDescriptor) bool {
		for _, id := range index.ColumnIDs {
			if _, ok := colIDSet[id]; ok || primaryKeyColChange {
				return true
			}
		}
		return false
	...
	for rows.Next() {
		...
		for i, newSecondaryIndexEntry := range newSecondaryIndexEntries {
			...
			if rowPrimaryKeyChanged || !bytes.Equal(newSecondaryIndexEntry.key, secondaryIndexEntry.key) {
				if log.V(2) {
					log.Infof("Del %s", secondaryIndexEntry.key)
				}
				b.Del(secondaryIndexEntry.key)
