...
type copyNode struct {
	...
	buf           bytes.Buffer
	rows          []*parser.Tuple
	rowsAllocSpan mon.AllocationSpan
}
...
func (n *copyNode) Close() {
	n.p.session.CloseSpan(&n.rowsAllocSpan)
}
...
func (n *copyNode) addRow(line []byte) error {
	...
	for i, part := range parts {
		...
		sz, _ := d.Size()
		if err := n.p.session.ExtendSpan(&n.rowsAllocSpan, int64(sz)); err != nil {
			return err
		}
	...
	if err := n.p.session.ExtendSpan(&n.rowsAllocSpan, int64(unsafe.Sizeof(tuple))); err != nil {
		return err
	}
...
func (p *planner) CopyData(n CopyDataBlock, autoCommit bool) (planNode, error) {
	...
	cf.rows = cf.rows[:0]
	p.session.ResetSpan(&cf.rowsAllocSpan)
