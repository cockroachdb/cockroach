...
// Similar to TestSystemTableLiterals except for the virtual tables -- see the
// comment on TestSystemTableLiterals for motivation, usage, etc.
func TestVirtualTableLiterals(t *testing.T) {
	...
	for _, schema := range virtualSchemas {
		for _, table := range schema.tables {
			...
			// Literals don't have families (which are only meaningful in the context
			// of storage, and these tables aren't stored), so we blank the ones
			// created by AllocateIDs so that `gen` will match.
			gen.Families = nil
			gen.NextFamilyID = 0
