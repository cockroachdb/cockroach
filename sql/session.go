...
type txnState struct {
	...
	// Ctx is used to init the contexts for every SQL txn, as this txnState is
	// reset for every txn.
	Ctx context.Context
...
func (ts *txnState) resetForNewSQLTxn(e *Executor, s *Session) {
	...
	*ts = txnState{}
	...
	if traceSQL {
		...
		ctx = opentracing.ContextWithSpan(ctx, ts.sp)
	}
	ts.Ctx = ctx
...
func (ts *txnState) finishSQLTxn() {
	if ts.sp == nil {
		panic("no ts.sp? Was resetForNewSQLTxn() called previously?")
	}
