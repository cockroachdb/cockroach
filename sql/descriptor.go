...
// If `plainKey` doesn't exist, found is set to false and pErr is nil.
...
func (p *planner) getDescriptor(plainKey descriptorKey, descriptor descriptorProto) (
	...
	case *TableDescriptor:
		table := desc.GetTable()
		if table == nil {
			return true, roachpb.NewUErrorf("%q is not a table", plainKey.Name())
		}
		*t = *table
	case *DatabaseDescriptor:
		database := desc.GetDatabase()
		if database == nil {
			return true, roachpb.NewUErrorf("%q is not a database", plainKey.Name())
		}
		*t = *database
	}

	return true, roachpb.NewError(descriptor.Validate())
}
