...
func (e *Executor) execStmtInOpenTxn(
	...
	if txnState.events != nil {
		txnState.events.Printf("%s", stmt)
	}
	...
	if pErr != nil {
		if txnState.events != nil {
			txnState.events.Errorf("%v", pErr)
		}
		result = Result{PErr: pErr}
		txnState.aborted = true
	} else if txnState.events != nil {
		switch result.Type {
		case parser.RowsAffected:
			txnState.events.Printf("%s %d", result.PGTag, result.RowsAffected)
		case parser.Rows:
			txnState.events.Printf("%s %d", result.PGTag, len(result.Rows))
		default:
			txnState.events.Printf("%s", result.PGTag)
		}
	}
	if txnDone {
		txnState.aborted = false
