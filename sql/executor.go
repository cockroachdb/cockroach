...
func (e *Executor) Prepare(user string, query string, session *Session, args parser.MapArgs) (
	...
	*planMaker = planner{
	...
	planMaker.setTxn(txn, time.Time{})
	planMaker.evalCtx.StmtTimestamp = parser.DTimestamp{Time: time.Time{}}
...
func (e *Executor) ExecuteStatements(
	...
	*planMaker = planner{
	...
	if txnProto := session.Txn.Txn; txnProto != nil {
		...
		e.initializeTxn(curTxnState.txn, session)
	}
...
func runTxnAttempt(
	...
	*results = nil
	...
	planMaker.setTxn(txn, txn.Proto.OrigTimestamp.GoTime())
