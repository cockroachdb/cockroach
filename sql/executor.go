...
func (schemaChangers *schemaChangerMap) queueSchemaChanger(
	schemaChanger SchemaChanger) {
	schemaChangers.schemaChangers[schemaChangers.curStatementIdx] = append(
		schemaChangers.schemaChangers[schemaChangers.curStatementIdx], schemaChanger)
}
...
func (schemaChangers *schemaChangerMap) execSchemaChanges(
	...
	if len(schemaChangers.schemaChangers) > 0 &&
		...
		var stmtIdxs []int
		for k := range schemaChangers.schemaChangers {
			stmtIdxs = append(stmtIdxs, k)
		}
		sort.Ints(stmtIdxs)
		for _, idx := range stmtIdxs {
			for _, sc := range schemaChangers.schemaChangers[idx] {
				sc.db = e.db
		...
		schemaChangers.schemaChangers = make(map[int][]SchemaChanger)
	}
}
...
// It may resume a SQL transaction, if one was previously open for this client.
//
...
func (e *Executor) execRequest(txnState *txnState, sql string, planMaker *planner) Response {
	...
	for len(stmts) > 0 {
		...
		inTxn := (txnState.txn != nil) || txnState.aborted
		...
		if pErr != nil {
			...
			// done if the error was encountered when executing the COMMIT/ROLLBACL.
			// There's nothing we can use it for any more.
			// TODO(andrei): once db.Resume() doesn't abort retryable txns any more,
		...
		// Trim the stmts that have just been executed.
		if execOpt.AutoCommit {
			stmts = stmts[1:]
			if len(remainingStmts) > 0 {
				panic("got some remaining statements after an implicit txn?!?")
			}
		} else {
...
// runTxnAttempt is the closure we pass to db.Resume. It will be called
...
func runTxnAttempt(
	...
	*results = nil
	...
	var firstErr *roachpb.Error
	*results, *remainingStmts, firstErr = e.execStmtsInCurrentTxn(
	...
	// Make sure db.Resume() doesn't retry this txn if the txn hasn't ended.
	// We do want to retry for implicit transactions though (opt.AutoCommit).
	if (firstErr != nil) && (planMaker.txn != nil) && !opt.AutoCommit {
		opt.AutoRetry = false
	}
	planMaker.resetTxn()
	return firstErr
...
//  - the first error encountered while executing statements. If an error
//    occurred, all subsequent statements have been refused (got an txn aborted
//    err result).
func (e *Executor) execStmtsInCurrentTxn(
	...
	var results []Result
	if (planMaker.txn == nil) && !txnState.aborted {
		panic("execStmtsInCurrentTransaction called outside of a txn")
	}
	var firstError *roachpb.Error
	for i, stmt := range stmts {
		...
		if txnDone {
			...
			txnState.aborted = false
			txnState.txn = nil
			remaining := parser.StatementList{}
			if i < len(stmts)-1 {
				remaining = stmts[i+1:]
			}
			return results, remaining, firstError
		}
	}
	return results, parser.StatementList{}, firstError
}
