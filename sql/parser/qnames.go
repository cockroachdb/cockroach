...
func (node *QualifiedName) NormalizeDatabaseName() (DatabaseName, error) {
	...
	if len(n) != 1 {
		return DatabaseName{}, fmt.Errorf("invalid database name: %s", n)
	}
	...
	if len(name) == 0 {
		return DatabaseName{}, fmt.Errorf("empty database name: %s", n)
	}
...
func (node *QualifiedName) NormalizeFunctionName() (*FunctionName, error) {
	...
	if len(name) == 0 {
		return nil, fmt.Errorf("empty function name: %s", n)
	}
...
func (node *QualifiedName) NormalizeTableName() (*TableName, error) {
	...
	if len(n) == 0 || len(n) > 2 {
		return nil, fmt.Errorf("invalid table name: %s", n)
	}
	...
	if len(name) == 0 {
		return nil, fmt.Errorf("empty table name: %s", n)
	}
	...
	if len(n) > 1 {
		db, ok = n[0].(Name)
		if !ok {
			return nil, fmt.Errorf("invalid database name: %s", n[0])
		}

		if len(db) == 0 {
			return nil, fmt.Errorf("empty database name: %s", n)
		}
	}
...
func (t *TableName) QualifyWithDatabase(database string) error {
	...
	if database == "" {
		return fmt.Errorf("no database specified: %s", t)
	}
...
func (node *QualifiedName) NormalizeTablePattern() error {
	...
	if len(n) > 1 {
		db, ok = n[0].(Name)
		if !ok {
			return fmt.Errorf("invalid database name: %s", n[0])
		}
	}
...
func (at *AllTablesSelector) QualifyWithDatabase(database string) error {
	...
	if database == "" {
		return fmt.Errorf("no database specified: %s", at)
	}
...
func (node *QualifiedName) NormalizeNameInExpr() error {
	...
	if !ok {
		return fmt.Errorf("invalid column name: %s", n[:i])
	}
	if len(colName) == 0 {
...
func (node *QualifiedName) NormalizeUnqualifiedColumnItem() (*ColumnItem, error) {
	if c, ok := node.Target.(*ColumnItem); ok {
		if c.TableName.TableName != "" {
			return nil, fmt.Errorf("cannot use a table name prefix here: %s", c)
		}
		return c, nil
	}
	...
	if len(n) == 0 {
		return nil, fmt.Errorf("invalid column name: %s", n)
	}
	...
	if len(colName) == 0 {
		return nil, fmt.Errorf("empty column name: %s", n)
	}
