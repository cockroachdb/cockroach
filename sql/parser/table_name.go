...
type NormalizableTableName struct{ TableNameReference }
...
// NormalizeTableName transforms an UnresolvedName to a TableName.
...
func (n UnresolvedName) NormalizeTableName() (*TableName, error) {
	...
	// FIXME(knz) return by value?
	return &res, nil
}

// NormalizeTableNameWithDatabaseName normalizes the UnresolvedName to
// a table name if not done already, then returns the resulting
// TableName with the given database name set if not already.
func (n UnresolvedName) NormalizeTableNameWithDatabaseName(database string) (*TableName, error) {
	t, err := n.NormalizeTableName()
	if err != nil {
		return nil, err
	}
