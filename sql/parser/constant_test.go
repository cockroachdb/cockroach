		...
		for _, availType := range avail {
			if res, err := c.ResolveAsType(availType); err != nil {
				...
				resErr := func(parsed, resolved interface{}) {
					t.Errorf("%d: expected resolving %v as available type %s would produce a Datum with the value %v, found %v", i, c, availType.Type(), parsed, resolved)
				}
				switch typ := res.(type) {
				case *DInt:
					var i int64
		...
		for _, availType := range avail {
			if res, err := test.c.ResolveAsType(availType); err != nil {
				...
				if resString != test.c.s {
					t.Errorf("%d: expected resolving %v as available type %s would produce a Datum with the same value, found %s", i, test.c, availType.Type(), resString)
				}
			}
		}
	}
