...
func (s *scanner) scanNumber(lval *sqlSymType, ch int) {
	...
	if hasDecimal || hasExponent {
		...
		if floatConst.Kind() == constant.Unknown {
			lval.id = ERROR
			lval.str = fmt.Sprintf("could not make constant float from literal string %q", lval.str)
			return
		}
		...
		if intConst.Kind() == constant.Unknown {
			lval.id = ERROR
			lval.str = fmt.Sprintf("could not make constant int from literal string %q", lval.str)
			return
		}
