...
func (p *PlaceholderInfo) SetValue(name string, val Datum) error {
	if _, ok := p.Values[name]; ok {
		return fmt.Errorf("placeholder %s already has a value", name)
	}
	p.Values[name] = val
...
// SetTypes resets the type and values in the map. If src is nil, the
// map is cleared.
...
// IsUnresolvedArgument returns whether expr is an unresolved placeholder. In
...
func (p *PlaceholderInfo) IsUnresolvedArgument(expr Expr) bool {
