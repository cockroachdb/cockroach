...
// returnTuple returns the Datum type that all arguments share, or an error
// if they do not share types.
func returnTuple(args DTuple) (Datum, error) {
	var typ string
	var i int
	arg := DNull
	// Find first non-null argument.
	for ; i < len(args); i++ {
		arg = args[i]
		if arg != DNull {
			typ = arg.Type()
	...
	for _, a := range args[i:] {
		if a.Type() != typ && a != DNull {
			return nil, errors.New("all arguments must be of the same type")
		}
	}
	return arg, nil
}
