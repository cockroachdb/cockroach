...
type rowHelper struct {
	...
	// that are changing, plus columns in each index that needs to be updated.
...
func newRowInserter(
	...
	rowHelper := newRowHelper(tableDesc, colIDtoRowIndex, indexes)
	if err := rowHelper.requirePrimaryIndexCols(); err != nil {
		return nil, err
	}
...
func newRowUpdater(
	...
	rowHelper.primaryIndexCols = primaryIndexCols

	// TODO(dan): Is it safe for these to share rowHelper instead of creating
	// two more?
	rd, err := newRowDeleter(tableDesc, colIDtoRowIndex)
	if err != nil {
		return nil, err
	}
	ri, err := newRowInserter(tableDesc, colIDtoRowIndex, tableDesc.Columns)
	if err != nil {
		return nil, err
	}
