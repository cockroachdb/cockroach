...
func TestInitialKeys(t *testing.T) {
	...
	ms := sql.NewMetadataSchema()
	kv := ms.GetInitialValues()
	// Only ID Generator by default.
	if actual, expected := len(kv), 1; actual != expected {
		t.Fatalf("Wrong number of initial sql kv pairs: %d, wanted %d", actual, expected)
	}

	// Add system database to the schema.
	sql.AddSystemDatabaseToSchema(ms)
	// IDGenerator + 2 for each system object.
	kv = ms.GetInitialValues()
	...
	db := ms.AddDatabase("testdb", sql.NewPrivilegeDescriptor(security.RootUser, privilege.List{privilege.ALL}))
