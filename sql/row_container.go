...
type RowContainer struct {
	...
	// allocSpan tracks the current memory consumption of this
	// RowContainer.
...
func (p *planner) NewRowContainer(h ResultColumns, rowCapacity int) *RowContainer {
	...
	res.fixedColsSize += int64(unsafe.Sizeof(make(parser.DTuple, 0, nCols)))
...
func (c *RowContainer) Close() {
	...
	c.p.session.CloseSpan(&c.allocSpan)
}
...
func (c *RowContainer) AddRow(row parser.DTuple) error {
	if err := c.p.session.ExtendSpan(&c.allocSpan, c.rowSize(row)); err != nil {
		return err
	}
...
func (c *RowContainer) Replace(i int, newRow parser.DTuple) error {
	...
	if newSz != oldSz {
		if err := c.p.session.ResizeItem(&c.allocSpan, oldSz, newSz); err != nil {
			return err
		}
	}
	c.rows[i] = newRow
