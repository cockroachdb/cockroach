...
func (p *planner) NewRowContainer(h ResultColumns, rowCapacity int) *RowContainer {
	...
	p.session.OpenAccount(&res.memAcc)
...
func (c *RowContainer) Close() {
	...
	c.p.session.CloseAccount(&c.memAcc)
}
...
func (c *RowContainer) AddRow(row parser.DTuple) error {
	if err := c.p.session.GrowAccount(&c.memAcc, c.rowSize(row)); err != nil {
		return err
	}
...
func (c *RowContainer) Replace(i int, newRow parser.DTuple) error {
	...
	if newSz != oldSz {
		if err := c.p.session.ResizeItem(&c.memAcc, oldSz, newSz); err != nil {
			return err
		}
	}
	c.rows[i] = newRow
