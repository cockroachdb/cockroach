...
type selectTopNode struct {
	...
	// together (connectPlans).
	plan planNode
}

func (n *selectTopNode) ExplainTypes(f func(string, string)) {
	if n.plan == nil {
		// No common plan constructed yet; ask the sub-nodes individually.
		plans := []planNode{n.limit, n.distinct, n.sort, n.group, n.source}
		for _, p := range plans {
			if p != nil {
				p.ExplainTypes(f)
			}
...
func (n *selectTopNode) connectPlans() {
	plan := n.source
	plan = wrapGroup(&n.group, plan)
	plan = wrapSort(&n.sort, plan)
	plan = wrapDistinct(&n.distinct, plan)
	plan = wrapLimit(&n.limit, plan)
	n.plan = plan
}
...
func (n *selectTopNode) ExplainPlan(v bool) (name, description string, children []planNode) {
	if !v {
		return n.plan.ExplainPlan(v)
	}

	return "select", "", []planNode{n.plan}
}

func (n *selectTopNode) Columns() []ResultColumn {
	// FIXME: probably proper behavior is to call sel.Columns() until
	// expandPlan(), then plan.Columns() afterwards. Not sure.
...
func (s *selectNode) ExplainPlan(v bool) (name, description string, children []planNode) {
	...
	return "render/filter", buf.String(), []planNode{s.source.plan}
}
...
func (p *planner) Select(n *parser.Select, desiredTypes []parser.Datum, autoCommit bool) (planNode, error) {
	...
	default:
		...
		return &selectTopNode{source: plan, sort: sort, limit: limit}, nil
	}
}
...
func (s *selectNode) expandPlan() error {
	...
	// FIXME(knz) -- what to do here?
	//limitCount, limitOffset, err := p.evalLimit(limit)
	//if err != nil {
	//		return nil, err
	//	}
	limitOffset := 0
	limitCount := math.MaxInt64
...
func (s *selectNode) addRender(target parser.SelectExpr, desiredType parser.Datum) error {
	...
	if resolved, err = s.planner.replaceSubqueries(resolved, 1); err != nil {
		return err
	}
