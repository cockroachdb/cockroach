...
func encodeStartConstraintAscending(spans []span, c *parser.ComparisonExpr) {
	...
	case parser.IsNot:
		...
		if c.Right != parser.DNull {
			panic(util.Errorf("expected NULL operand for IS NOT operator, found %v", c.Right))
		}
		for i := range spans {
	...
	default:
		panic(util.Errorf("unexpected operator: %s", c))
	}
}
...
func encodeStartConstraintDescending(
	...
	case parser.Is:
		...
		// to begin at NULL.
		if c.Right != parser.DNull {
			panic(util.Errorf("expected NULL operand for IS operator, found %v", c.Right))
		}
		for i := range spans {
			spans[i].start = encoding.EncodeNullDescending(spans[i].start)
		}
	case parser.NE:
		panic("'!=' operators should have been transformed to 'IS NULL'")
...
func encodeEndConstraintAscending(spans []span, c *parser.ComparisonExpr,
	...
	default:
		...
		if !isLastEndConstraint {
			panic(util.Errorf("can't have other end constraints after a '<' constraint, found %v", c.Operator))
		}
		key, err := encodeTableKey(nil, datum, encoding.Ascending)
		if err != nil {
			panic(err)
