...
func (p *planner) SelectClause(
	...
	if err := s.initFrom(p, parsed, allowNonPublicCols); err != nil {
		return nil, err
	}
...
func (s *selectNode) initFrom(
	p *planner, parsed *parser.SelectClause, allowNonPublicCols bool,
) error {
	from := parsed.From
	...
	case 1:
		...
		case *parser.QualifiedName:
			// Usual case: a table.
			scan := p.Scan()
			s.source.info.alias, err = scan.initTable(p, expr, ate.Hints, allowNonPublicCols)
			if err != nil {
				return err
			}
			s.source.plan = scan
