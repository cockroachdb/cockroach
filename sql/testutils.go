...
type LeaseReleaseWaiter struct {
	mu sync.Mutex
	// map from lease we're waiting to a channel on which the result of the
...
func (w *LeaseReleaseWaiter) PrepareToWait(lease *LeaseState) {
	...
	if _, ok := w.waitingFor[lease]; ok {
		// Already prepared to wait this lease.
		return
	}
	w.waitingFor[lease] = make(chan error, 1)
}

// WaitForRelease blockes until a lease previously registered with
