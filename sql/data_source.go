...
func (p *planner) getDataSource(
	...
	default:
		return planDataSource{}, util.UnimplementedWithIssueErrorf(2970, "unsupported FROM type %T", src)
	}
}
...
func (src *dataSourceInfo) expandStar(
	...
	if len(src.sourceColumns) == 0 {
		return nil, nil, fmt.Errorf("cannot use \"%s\" without a data source", qname)
	}
	...
	if len(exprs) == 0 {
		// If the table contained no columns, or only hidden columns, we
		// may have selected nothing at this point.
...
// into account). That is, given a table blah containing
// column "y", findUnaliasedColumn("y") returns 0 even in the context of
...
// concatDataSourceInfos concatenates the column metadata for two data
//  sources. If a table name appears on both sides, an ambiguity is
//  reported.
...
// findColumnByName returns the index of the column in the data source.
// Returns -1 if the column is not found.
func (src *dataSourceInfo) findColumnByName(name string, context string) (int, error) {
	ret := -1
	for i, col := range src.sourceColumns {
		if col.Name == name {
			if ret != -1 {
				return -1, fmt.Errorf("column name \"%s\" is ambiguous in %s", name, context)
			}
			ret = i
		}
	}
	if ret == -1 {
		return -1, fmt.Errorf("column \"%s\" not found in %s", name, context)
	}
	return ret, nil
}
