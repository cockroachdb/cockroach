...
func (sources multiSourceInfo) findColumn(
	...
	for iSrc, src := range sources {
		findCol := func(src *dataSourceInfo, iSrc, srcIdx, colIdx int, idx int) (int, int, error) {
			col := src.sourceColumns[idx]
			if sqlbase.ReNormalizeName(col.Name) == colName {
				if colIdx != invalidColIdx {
		...
		if tableName.Table() == "" {
			for idx := 0; idx < len(src.sourceColumns); idx++ {
				sIdx, colIdx, err = findCol(src, iSrc, sIdx, colIdx, idx)
				if err != nil {
					return sIdx, colIdx, err
				}
			}
		} else {
