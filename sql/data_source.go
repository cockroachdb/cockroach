...
func (sources multiSourceInfo) checkDatabaseName(tn *parser.TableName) error {
	...
	if tn.DatabaseName == "" {
		...
		for _, src := range sources {
			for name := range src.sourceAliases {
				if name.TableName == tn.TableName {
					if found {
						return fmt.Errorf("ambiguous source name: %q", tn.String())
					}
		...
		if !found {
			return fmt.Errorf("source name %q not found in FROM clause", tn.TableName.String())
		}
		return nil
	}
	...
	if !found {
		return fmt.Errorf("table %q not selected in FROM clause", tn.String())
	}
	return nil
}
...
func (sources multiSourceInfo) findColumn(
	...
	case *parser.ColumnItem:
		c = t
	default:
		return nil, invalidColIdx, fmt.Errorf("cannot use \"%s\" in this context", qname)
	}

	if len(c.Selector) > 0 {
		return nil, invalidColIdx, fmt.Errorf("syntax not supported: %s", qname)
	}
	...
	if colIdx == invalidColIdx {
		return nil, invalidColIdx, fmt.Errorf("column name \"%s\" not found or not selected", qname)
	}
