	...
	default:
		...
		if *failureRec.txn.TxnMeta.ID != *txn.TxnMeta.ID {
			...
			panic(fmt.Sprintf("new transaction for value \"%s\" (%s) "+
				...
				txn.TxnMeta.ID, failureRec.txn.TxnMeta.ID))
		}
	}
...
`); err != nil {
	...
	for _, tc := range testCases {
		...
		cleanupFilter := cmdFilters.AppendFilter(
			func(sid roachpb.StoreID, req roachpb.Request, hdr roachpb.Header) error {
				return injectErrors(sid, req, hdr, tc.magicVals)
			})
...
// !!! write test for correct errors in COMMIT_WAIT

// !!! write test for handling of errors on COMMIT when RELEASE wasn't issued
