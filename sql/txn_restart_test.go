...
// SQL statements and aborting them. The TxnAborter's StatementFilter needs to
// be hooked up to a Server's ExecutorTestingKnobs' StatementFilter, so that the
// Aborter sees what statements are being executed. You register a statement for
// abortion (meaning, the statement's transaction will be TransactionAborted)
// with QueueStmtForAbortion(). When the Aborter sees that statement, it will
// run a higher priority transaction that tramples the data, so the original
// transaction will get a TransactionAborted error when it tries to commit.
//
// Note that aborting transaction cannot be done as an injected error, since we
// want the pusher to clean up the intents of the pushee.
//
// The aborter only works with statements operating on the tabke t.test defined
// below:
...
//		FixTxnPriority: true,
//		// We're going to abort txns using the StatementFilter, and that's
//		// incompatible with AutoCommit.
//		DisableAutoCommit: true,
//		StatementFilter:   aborter.StatementFilter,
//	}
...
func (ta *TxnAborter) InitConn(
	t testing.TB, s serverutils.TestServerInterface) {
...
// QueueStmtForAbortion registers a statemnt whose transaction will be aborted.
//
...
func runTestTxn(
	t *testing.T, magicVals *filterVals,
	expectedErr string, sqlDB *gosql.DB, tx *gosql.Tx, sentinelInsert string,
) bool {
	retriesNeeded :=
