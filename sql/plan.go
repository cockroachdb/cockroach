...
func (p *planner) query(sql string, args parser.Args) (planNode, error) {
	...
	if err := parser.FillArgs(stmts[0], args); err != nil {
		return nil, err
	}
	return p.makePlan(stmts[0])
}

func (p *planner) queryRow(sql string, args parser.Args) (parser.DTuple, error) {
...
func (p *planner) exec(sql string, args parser.Args) (int, error) {
	plan, err := p.query(sql, args)
	if err != nil {
		return 0, err
	}
