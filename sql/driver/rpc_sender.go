...
//
// TODO(pmattis): Duplicated from rpc/server.go:connected.
const rpcConnected = "200 Connected to Go RPC"

// TODO(pmattis): Duplicated from rpc/tls.go.
...
//
// TODO(pmattis): Duplicated from rpc/tls.go:tlsDial.
...
//
// TODO(pmattis): Duplicated from rpc/tls.go:TLSDialHTTP.
...
func newRPCSender(server string, context *base.Context, retryOpts retry.Options) (*rpcSender, error) {
	...
	conn, err := tlsDialHTTP(addr.Network(), addr.String(), tlsConfig)
	if err != nil {
		return nil, err
	}
