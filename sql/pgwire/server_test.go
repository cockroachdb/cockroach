...
func TestSimpleQuery(t *testing.T) {
	...
	rows, err := s.db.Query("SELECT true, 42, 3.14, 'data'::blob, 'hello', '2015-09-12 15:30:00'::timestamp, '2015-09-12'::date")
	if err != nil {
		t.Fatal(err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		var tm time.Time
		var date time.Time
		if err := rows.Scan(&b, &i, &f, &d, &s, &tm, &date); err != nil {
			t.Fatal(err)
		}
		if !b {
		...
		if e := []byte("data"); bytes.Compare(d, e) != 0 {
			t.Errorf("expected %q but got %s", e, d)
		}
		if e := "hello"; s != e {
			t.Errorf("expected %q but got %s", e, s)
		}
		if a, e := tm.Format(time.RFC3339), "2015-09-12T15:30:00Z"; a != e {
			t.Errorf("expected %s but got %s", e, a)
		}
		if a, e := date.Format(time.RFC3339), "2015-09-12T00:00:00Z"; a != e {
			t.Errorf("expected %s but got %s", e, a)
		}
	}
