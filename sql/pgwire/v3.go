...
func (c *v3Conn) handleParse(buf *readBuffer) error {
	...
	for k, v := range args {
		...
		if err != nil {
			return c.sendInternalError(fmt.Sprintf("non-integer parameter name: %s", k))
		}
		// ValArgs are 1-indexed, pq.inTypes are 0-indexed.
		i--
		if len(pq.inTypes) <= i {
			return c.sendInternalError("internal error: leftover valargs")
		}
...
func (c *v3Conn) sendPError(pErr *roachpb.Error) error {
	var errCode string
	if sqlErr, ok := pErr.GetDetail().(*roachpb.SQLErrorProto); ok {
