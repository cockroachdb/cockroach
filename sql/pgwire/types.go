...
const (
	PG_NUMERIC_POS pgSign = 0x0000
	PG_NUMERIC_NEG pgSign = 0x4000
	PG_NUMERIC_NAN pgSign = 0xC000
)
...
var (
	decShift = big.NewInt(int64(math.Pow10(DEC_DIGITS)))
)
...
func (b *writeBuffer) writeBinaryDatum(d parser.Datum) {
	...
	case *parser.DDecimal:
		...
		if v.Sign() >= 0 {
			alloc.pgNum.sign = PG_NUMERIC_POS
		} else {
			alloc.pgNum.sign = PG_NUMERIC_NEG
		}
		...
		for i := alloc.pgNum.weight + 1; i < alloc.pgNum.ndigits; i++ {
			// Push DEC_DIGITS ahead of the decimal point.
			alloc.fracDec.SetScale(alloc.fracDec.Scale() - DEC_DIGITS)
	...
	case oid.T_numeric:
		...
		case formatBinary:
			...
			for i := int16(0); i < alloc.pgNum.ndigits; i++ {
				...
				alloc.dec.SetScale(DEC_DIGITS * inf.Scale(i-alloc.pgNum.weight))
