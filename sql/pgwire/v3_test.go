...
func makeTestV3Conn(c net.Conn) v3Conn {
	return makeV3Conn(c,
		&sql.Executor{},
		makeServerMetrics(metric.NewRegistry()),
		sql.SessionArgs{},
	)
}
...
func TestMaliciousInputs(t *testing.T) {
	...
	for _, data := range [][]byte{
		...
		[]byte{byte(clientMsgClose), 0x00, 0x00, 0x00, 0x04},
		// This byte string exploited the same bug using a clientMsgDescribe
		// message type.
		[]byte{byte(clientMsgDescribe), 0x00, 0x00, 0x00, 0x04},
	} {
...
func testMaliciousInput(t *testing.T, data []byte) {
	...
	go func() {
		...
		w.Write([]byte{byte(clientMsgSync), 0x00, 0x00, 0x00, 0x04})
		w.Write([]byte{byte(clientMsgTerminate), 0x00, 0x00, 0x00, 0x04})
	}()
