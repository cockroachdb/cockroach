...
func transferMoney(client *sql.DB, numAccounts, maxTransfer int) error {
...
func transferMoneyLoop(idx int, state *testState, numAccounts, maxTransfer int) {
	...
	for !state.done() {
		if err := func() error {
			client.RLock()
			defer client.RUnlock()
			if err := transferMoney(client.db, numAccounts, maxTransfer); err != nil {
				// Ignore some errors.
...
func chaosMonkey(state *testState, c cluster.Cluster, stopClients bool, pickNodes func() []int) {
	...
	for atomic.StoreUint64(&state.monkeyIteration, 1); !state.done(); atomic.AddUint64(&state.monkeyIteration, 1) {
