...
func testDockerFail(t *testing.T, cmd []string) {
	if err := testDocker(t, cmd); err == nil {
...
func testDockerSuccess(t *testing.T, cmd []string) {
	if err := testDocker(t, cmd); err != nil {
		t.Error("expected success: %s", err)
	}
}

func testDocker(t *testing.T, cmd []string) error {
	SkipUnlessLocal(t)
	...
	return l.OneShot("cockroachdb/postgres-test", "20160203-140220", c, h)
}
