...
func (cli retryingDockerClient) ContainerCreate(
	...
	err := cli.retry(ctx, "ContainerCreate",
		func(timeoutCtx context.Context) error {
			var err error
			resp, err = cli.APIClient.ContainerCreate(timeoutCtx, config, hostConfig, networkingConfig, containerName)
			return err
		})
	return resp, err
}
...
func (cli retryingDockerClient) ContainerWait(ctx context.Context, containerID string) (int, error) {
	var ret int
	err := cli.retry(ctx, "ContainerWait",
		func(timeoutCtx context.Context) error {
			var err error
