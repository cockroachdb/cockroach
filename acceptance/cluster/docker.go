	...
	for i := 0; i < cli.attempts; i++ {
		...
		if err != nil {
			...
			if strings.Contains(err.Error(), context.DeadlineExceeded.Error()) {
				...
				if matched := regexp.MustCompile(retryErrorsRE).
					MatchString(err.Error()); matched {
...
func (cli retryingDockerClient) ContainerCreate(
...

	{
		...
		if len(ret) > 0 {
			return types.ContainerCreateResponse{}, fmt.Errorf("container %s already exists: %+v", ret)
		}
	}
