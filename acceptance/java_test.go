...
// TestDockerJava connects to a cluster with Java.
func TestDockerJava(t *testing.T) {
	testDockerSuccess(t, []string{"/bin/sh", "-c", strings.Replace(java, "%v", "3", 1)})
	testDockerFail(t, []string{"/bin/sh", "-c", strings.Replace(java, "%v", `"a"`, 1)})
}
