...
bin=/bidirectional-reference-version/cockroach
...
bin=/forward-reference-version/cockroach
# TODO(bdarnell): when --background is in forward-reference-version, use it here and below.
$bin start &
sleep 1
echo "Use the reference binary to write a couple rows, then render its output to a file and shut down."
$bin sql -e "CREATE DATABASE old"
