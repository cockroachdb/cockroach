...
$bin sql -d old -e "INSERT INTO testing values (2, false, 'world', decimal '0.14159', 0.14159, NOW(), interval '2h')"
...
$bin start &
sleep 1
# Compare the new binary's view of the data written by old.
$bin sql -d old -e "SELECT * FROM testing" > new.everything
diff new.everything old.everything

# Add a row with the new binary and render the updated data before shutting down.
$bin sql -d old -e "INSERT INTO testing values (3, false, '!', decimal '2.14159', 2.14159, NOW(), interval '3h')"
$bin sql -d old -e "SELECT * FROM testing" > new.everything
$bin quit
wait

# Start the old binary again on top of the modified data.
bin=/reference-version/cockroach
$bin start &
sleep 1
$bin sql -d old -e "SELECT * FROM testing" > old.everything
