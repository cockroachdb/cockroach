...
func mustGetHosts(t *testing.T, c cluster.Cluster) ([]iptables.IP, map[iptables.IP]int) {
	...
	for i := 0; i < c.NumNodes(); i++ {
		host, _, err := net.SplitHostPort(c.InternalAddr(i).String())
		if err != nil {
			t.Fatal(err)
		}
		addr := iptables.IP(host)
		addrsToNode[addr] = i
...
// between two random groups of nodes.
var BidirectionalPartitionNemesis NemesisFn = func(t *testing.T, stop <-chan struct{}, c cluster.Cluster) {
	randSec := func() time.Duration { return time.Duration(rand.Intn(15)) * time.Second }
	for {
		ch := make(chan struct{})
		go func() {
			select {
...
func cutNetwork(t *testing.T, c cluster.Cluster, closer <-chan struct{}, partitions ...[]int) <-chan struct{} {
