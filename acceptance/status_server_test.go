...
func checkNode(t *testing.T, c cluster.Cluster, i int, nodeID, otherNodeID, expectedNodeID roachpb.NodeID) {
	...
	if err := getJSON(c.URL(i), fmt.Sprintf("/_status/details/%d", otherNodeID), &details); err != nil {
		t.Fatal(util.ErrorfSkipFrames(1, "unable to parse details - %s", err))
	}
	if details.NodeID != expectedNodeID {
		t.Fatal(util.ErrorfSkipFrames(1, "%d calling %d: node ids don't match - expected %d, actual %d", nodeID, otherNodeID, expectedNodeID, details.NodeID))
	}

	get(t, c.URL(i), fmt.Sprintf("/_status/gossip/%d", otherNodeID))
	get(t, c.URL(i), fmt.Sprintf("/_status/nodes/%d", otherNodeID))

	// TODO(wiz): Once the rest of the status HTTP endpoints are ported to grpc
	// remove this if statement. See #5530.
	if nodeID == otherNodeID {
		get(t, c.URL(i), fmt.Sprintf("/_status/logfiles/%d", otherNodeID))
		get(t, c.URL(i), fmt.Sprintf("/_status/logs/%d", otherNodeID))
		get(t, c.URL(i), fmt.Sprintf("/_status/stacks/%d", otherNodeID))
	}
}
