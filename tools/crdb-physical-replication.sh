...
function workload {
    ...
    run $COCKROACH workload init bank "$workloadconn"
    run $COCKROACH workload run bank "$workloadconn"
}
...
function license {
    ...
    local org
    org=$(run $COCKROACH sql --execute "$sql" --url "$systemconn" --certs-dir "$certsdir" --format 'tsv' | tail -1 | xargs)
    ...
    local lic
    lic=$(run $COCKROACH sql --execute "$sql" --url "$systemconn" --certs-dir "$certsdir" --format 'tsv' | tail -1 | xargs)
    if [ -z $lic ];
    then
    ...
    local enabled
    enabled=$(run $COCKROACH sql --execute "$sql" --url "$systemconn" --certs-dir "$certsdir" --format 'tsv' | tail -1 | xargs)
    ...
    then
        ...
        local user
        user=$(run $COCKROACH sql --execute "$sql" --url "$systemconn" --certs-dir "$certsdir" --format 'tsv' | tail -1 | xargs)
...
then    
    ...
    do
        ...
        if run $COCKROACH sql --certs-dir "$certsdir" --url "$primaryconn" --execute "$trysql" 2>/dev/null;
        then
            ok='t'
