...
TEST(FileKeyManager, ReadKeyFiles) {
  ...
  for (int i = 0; i < ARRAY_LENGTH(keys, KeyFiles); i++) {
    ASSERT_OK(rocksdb::WriteStringToFile(env.get(), keys[i].contents, keys[i].filename));
  }
  ...
  for (int i = 0; i < ARRAY_LENGTH(test_cases, TestCase); i++) {
    auto test = test_cases[i];
    FileKeyManager fkm(env.get(), test.new_key, test.old_key);
    auto status = fkm.LoadKeys();
    EXPECT_ERR(status, test.error);
  }
}
