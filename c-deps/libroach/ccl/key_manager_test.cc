...
TEST(DataKeyManager, LoadKeys) {
  ...
  {
    ASSERT_OK(rocksdb::WriteStringToFile(env.get(), "blah blah", "/COCKROACHDB_DATA_KEYS"));
    DataKeyManager dkm(env.get(), "", 0);
    EXPECT_ERR(dkm.LoadKeys(), "failed to parse key registry /COCKROACHDB_DATA_KEYS");
    ASSERT_OK(env->DeleteFile("/COCKROACHDB_DATA_KEYS"));
  }

  // Empty file.
  {
    ASSERT_OK(rocksdb::WriteStringToFile(env.get(), "", "/COCKROACHDB_DATA_KEYS"));
    DataKeyManager dkm(env.get(), "", 0);
    EXPECT_OK(dkm.LoadKeys());
    ASSERT_OK(env->DeleteFile("/COCKROACHDB_DATA_KEYS"));
  }
  ...
  // Empty protobuf.
  {
    ...
    ASSERT_OK(rocksdb::WriteStringToFile(env.get(), contents, "/COCKROACHDB_DATA_KEYS"));
    DataKeyManager dkm(env.get(), "", 0);
    EXPECT_OK(dkm.LoadKeys());
    ASSERT_OK(env->DeleteFile("/COCKROACHDB_DATA_KEYS"));
  }
...
TEST(DataKeyManager, KeyFromKeyInfo) {
  ...
  for (auto t : test_cases) {
    ...
    if (t.new_key.type == enginepb::Plaintext) {
      ...
      EXPECT_EQ(new_key.info().key_id(), KeyHash(new_key.key()));
    }
  }
}
...
// TODO(mberhault): add key rotation test.
