...
rocksdb::Status AESCipher::Encrypt(char *data) {
	CryptoPP::ECB_Mode<CryptoPP::AES>::Encryption enc((byte *)key, CryptoPP::AES::DEFAULT_KEYLENGTH);
	std::string output;
...
rocksdb::Status AESCipher::Decrypt(char *data) {
	CryptoPP::ECB_Mode<CryptoPP::AES>::Decryption dec((byte *)key, CryptoPP::AES::DEFAULT_KEYLENGTH);
	std::string output;
	output.reserve(CryptoPP::AES::BLOCKSIZE);
