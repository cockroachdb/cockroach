...
template <bool reverse> class mvccScanner {
 ...
 private:
  ...
  bool getFromIntentHistory() {
    cockroach::kv::storage::engine::enginepb::MVCCMetadata_SequencedIntent readIntent;
    readIntent.set_sequence(txn_sequence_);

    auto end = meta_.intent_history().end();
    cockroach::kv::storage::engine::enginepb::MVCCMetadata_SequencedIntent intent;
    ...
    auto up = std::upper_bound(
        ...
        [](const cockroach::kv::storage::engine::enginepb::MVCCMetadata_SequencedIntent& a,
           const cockroach::kv::storage::engine::enginepb::MVCCMetadata_SequencedIntent& b) -> bool {
 ...
 public:
  ...
  bool peeked_;
  cockroach::kv::storage::engine::enginepb::MVCCMetadata meta_;
