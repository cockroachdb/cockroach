...
template <bool reverse> class mvccScanner {
 ...
 private:
  ...
  bool getAndAdvance() {
    ...
    if (is_value) {
      ...
      if (check_uncertainty_) {
        ...
        // Delegate to seekVersion to return a clock uncertainty
        // errors if there are any more versions above
        // txn_max_timestamp_.
  ...
  bool nextKey() {
    ...
    // advancing the iterator unnecessarily.
  ...
  // prevKey advances the iterator to point the prev MVCC key less
...
DBScanResults MVCCScan(DBIterator* iter, DBSlice start, DBSlice end, DBTimestamp timestamp,
  ...
  if (reverse) {
    mvccScanner<true> scanner(iter, end, start, timestamp, max_keys, txn, consistent);
    return scanner.scan();
  } else {
    mvccScanner<false> scanner(iter, start, end, timestamp, max_keys, txn, consistent);
    return scanner.scan();
  }
}
