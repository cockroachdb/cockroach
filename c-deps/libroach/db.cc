...
WARN_UNUSED_RESULT bool SplitKey(
    rocksdb::Slice buf, rocksdb::Slice* key, rocksdb::Slice* timestamp) {
  if (buf.empty()) {
    return false;
  }
...
WARN_UNUSED_RESULT bool DecodeTimestamp(
    rocksdb::Slice* timestamp, int64_t* wall_time, int32_t* logical) {
  uint64_t w;
...
WARN_UNUSED_RESULT bool DecodeHLCTimestamp(
    rocksdb::Slice buf, cockroach::util::hlc::Timestamp* timestamp) {
  int64_t wall_time;
  int32_t logical;
...
WARN_UNUSED_RESULT inline bool DecodeKey(
    rocksdb::Slice buf, rocksdb::Slice* key, int64_t* wall_time, int32_t* logical) {
  key->clear();
...
WARN_UNUSED_RESULT bool inline DecodeKey(
    rocksdb::Slice buf, rocksdb::Slice* key, DBTimestamp *ts) {
...
class mvccScanner {
 public:
  mvccScanner(DBIterator *iter, DBSlice start, DBSlice end,
              DBTimestamp timestamp, int64_t max_keys, DBTxn txn, bool consistent)
      : iter_(iter),
  ...
  // to the key) a key with a zero timestamp will appear. We arrange
  // for the keys to be sorted such that the intent sorts first. For
  // example:
  //
  //   A @ T3
  //   A @ T2
 ...
 private:
  ...
  bool getAndAdvance() {
    ...
    if (is_value) {
      if (timestamp_ >= cur_timestamp_) {
        // Fast path: emit if this is a versioned value which we can read.
    ...
    // Emit immediately if the value is inline.
    if (!meta_.has_txn()) {
    ...
    // The key contains an intent, but we're reading before the
    // intent. Seek to the desired version.
    ...
    if (!consistent_) {
      // The key contains an intent and we're doing an inconsistent
      // read at timestamp newer than the intent. We ignore the intent
      // by insisting that the timestamp we're reading at is a
    ...
    if (!own_intent) {
      // The key contains an intent which was not written by our
      // transaction.
      intents_->Put(key, value);
      return nextKey();
    }
    ...
    if (txn_epoch_ == meta_.txn().epoch()) {
      // We're reading our own txn's intent.
      return seekVersion(meta_timestamp, false);
    }

    if (txn_epoch_ < meta_.txn().epoch()) {
      // We're reading our own txn's intent but the current txn has an
      // earlier epoch than the intent. Return an error so that
      // earlier incarnation of our transaction aborts (presumably
      // this is some operation that was retried).
      results_.status = FmtStatus(
          "failed to read with epoch %u due to a write intent with epoch %u",
    ...
    // reading. In this case, we skip the intent.
    return seekVersion(PrevTimestamp(ToDBTimestamp(meta_.timestamp())), false);
  }

  // nextKey advances the iterator to point the next MVCC key greater
  // than old_key. Returns false if the iterator is exhausted or an
  // error occurs. Note that old_key may be mutated.
  bool nextKey() {
    // Check to see if the next key is the end key.
    if (cur_key_.size()+1 == end_key_.size() &&
        end_key_.starts_with(cur_key_) &&
  ...
  // seekVersion advances the iterator to point an MVCC version for the
  // specified key that is earlier than <ts_wall_time,
  // ts_logical>. Returns false if the iterator is exhausted or an error
  // occurs.
...
DBScanResults MVCCGet(DBIterator* iter, DBSlice key, DBTimestamp timestamp,
  ...
  const DBSlice end = {const_cast<char*>("\xff\xff"), 2};
