...
// to perform when looking for the next key or a particular version
// before calling iter->Seek(). Note that mvccScanner makes this
// number adaptive. It starts with a value of kMaxItersPerSeek/2 and
// increases the value every time a call to iter->{Next,Prev}()
