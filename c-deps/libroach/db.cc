...
DBStatus DBIngestExternalFiles(DBEngine* db, char** paths, size_t len, bool move_files,
  ...
  // does this check *even if it is not planning on using seqno* at all, so we
  // need to set allow_global_seqno to true for skip_seqno to have the desired
  // effect.
  ingest_options.write_global_seqno = !skip_seqno;
  ingest_options.allow_global_seqno = allow_file_modifications || skip_seqno;
