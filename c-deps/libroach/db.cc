...
uint64_t DBGetApproximateSize(DBEngine* db, DBKey start, DBKey end) {
  ...
  uint64_t size;
  db->rep->GetApproximateSizes(&range, 1, &size);
  return size;
}
...
DBStatus DBCompactRange(DBEngine* db, DBKey start, DBKey end) {
  ...
  if (max_level != db->rep->NumberLevels() - 1) {
    ...
    return ToDBStatus(db->rep->CompactRange(options, start_key.length() > 0 ? &start_slice : nullptr, end_key.length() > 0 ? &end_slice : nullptr));
  }
