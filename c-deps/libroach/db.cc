...
mvccScanner<reverse>::mvccScanner(DBIterator* iter, DBSlice start, DBSlice end,
                                  DBTimestamp timestamp, int64_t max_keys, DBTxn txn,
                                  bool consistent)
    : iter_(iter),
...
template <bool reverse> const DBScanResults& mvccScanner<reverse>::get() {
  is_get_ = true;
...
template <bool reverse> const DBScanResults& mvccScanner<reverse>::scan() {
...
template <bool reverse> const DBScanResults& mvccScanner<reverse>::fillResults() {
...
template <bool reverse> bool mvccScanner<reverse>::uncertaintyError(DBTimestamp ts) {
...
template <bool reverse> bool mvccScanner<reverse>::setStatus(const DBStatus& status) {
  results_.status = status;
  return false;
}

template <bool reverse> bool mvccScanner<reverse>::getAndAdvance() {
...
// or an error occurs.
template <bool reverse> bool mvccScanner<reverse>::nextKey() {
...
template <bool reverse>
bool mvccScanner<reverse>::backwardLatestVersion(const rocksdb::Slice& key, int i) {
...
template <bool reverse> bool mvccScanner<reverse>::prevKey(const rocksdb::Slice& key) {
...
// occurs.
template <bool reverse> bool mvccScanner<reverse>::advanceKey() {
  if (reverse) {
...
template <bool reverse> bool mvccScanner<reverse>::advanceKeyAtEnd() {
  if (reverse) {
...
template <bool reverse> bool mvccScanner<reverse>::advanceKeyAtNewKey(const rocksdb::Slice& key) {
  if (reverse) {
...
template <bool reverse> bool mvccScanner<reverse>::addAndAdvance(const rocksdb::Slice& value) {
  if (value.size() > 0) {
...
template <bool reverse>
bool mvccScanner<reverse>::seekVersion(DBTimestamp desired_timestamp, bool check_uncertainty) {
  key_buf_.assign(cur_key_.data(), cur_key_.size());
...
template <bool reverse> bool mvccScanner<reverse>::updateCurrent() {
  if (!iter_rep_->Valid()) {
    return false;
  }
...
template <bool reverse> bool mvccScanner<reverse>::iterSeek(const rocksdb::Slice& key) {
  clearPeeked();
  iter_rep_->Seek(key);
...
// less than key.
template <bool reverse> bool mvccScanner<reverse>::iterSeekReverse(const rocksdb::Slice& key) {
  clearPeeked();
...
template <bool reverse> bool mvccScanner<reverse>::iterNext() {
  if (reverse && peeked_) {
...
template <bool reverse> bool mvccScanner<reverse>::iterPrev() {
  if (peeked_) {
    peeked_ = false;
    return updateCurrent();
  }
...
// iterator position.
template <bool reverse> bool mvccScanner<reverse>::iterPeekPrev(rocksdb::Slice* peeked_key) {
  if (!peeked_) {
    peeked_ = true;
...
template <bool reverse> void mvccScanner<reverse>::clearPeeked() {
  if (reverse) {
    peeked_ = false;
  }
}
