...
std::string findStart(Iterator* iter, rocksdb::Slice start, rocksdb::Slice end) {
  iter->Seek(start);
  if (!iter->Valid()) {
    return end.ToString();
  }
...
template <typename Iterator>
std::string findEnd(Iterator* iter, rocksdb::Slice start, rocksdb::Slice end) {
...
DBStatus DBBatch::DeleteRange(DBKey start, DBKey end) {
  ...
  // present in a lower level sstable. This defeats are hack if we weren't
