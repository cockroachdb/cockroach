...
DBIterator::DBIterator(std::atomic<int64_t>* iters, DBIterOptions iter_options) : iters_count(iters) {
  ...
  if (!EmptyTimestamp(iter_options.min_timestamp) || !EmptyTimestamp(iter_options.max_timestamp)) {
    const std::string min = EncodeTimestamp(iter_options.min_timestamp);
    const std::string max = EncodeTimestamp(iter_options.max_timestamp);
