...
```
5. Create a new pair of ssh keys `ssh-keygen -t rsa -C "your-email"` for GCE with location `~/.ssh/google_compute_engine`, or use an existing pair and adjust the `key_name` variable described below. Do not use a passphrase while generating the keys or else your private key will be encrypted. You have to use an unencrypted private key with terraform.
