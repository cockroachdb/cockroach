...
if [ "${action}" != "start" ]; then
  echo "Usage: ${0} [start] [load balancer address:port]"
  exit 1
fi
...
sudo iptables -t nat -A POSTROUTING -p tcp -d ${LB_ADDRESS} --dport ${SQL_PORT} -j SNAT --to-source ${LOCAL_ADDRESS}
sudo iptables -t nat -A POSTROUTING -p tcp -d ${LB_ADDRESS} --dport ${HTTP_PORT} -j SNAT --to-source ${LOCAL_ADDRESS}
