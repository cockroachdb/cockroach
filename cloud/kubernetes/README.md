...
an older version of Kubernetes, use the files from the appropriate subdirectory
(e.g. the `v1.7` directory for Kubernetes 1.7 or the `v1.6` directory for
Kubernetes 1.6). Even older versions that don't have their own directory are no
longer supported.
