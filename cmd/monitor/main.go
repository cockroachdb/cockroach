...
const (
	...
	urlPath = "_status/nodes"
)

var interval = flag.Duration("interval", 10*time.Second, "Interval in which to poll each services's status.")
var addrs = flag.String("addrs", ":26257", "Comma-separated list of host:port status endpoints to monitor.")
...
func (m *statusMonitor) QueryStatus() ([]byte, error) {
	var queryErr error
...
func main() {
	...
	for _, addr := range parsedAddrs {
		...
		log.Infof("Monitoring Status URL: %s\n", client.url)
		stopper.RunWorker(func() {
			...
			for {
				...
				case <-timer:
					...
					if err != nil {
						log.Warningf("Could not get status from url %s. Time since monitor started %s.", client.url, time.Since(startTime))
						stopper.Stop()
						continue
					}
					log.Infof("Got status from url %s.", client.url)
				}
			}
		})
	}
	...
	select {
	case <-stopper.ShouldStop():
		log.Infof("Stopper stopped by error...")
	case <-signalCh:
