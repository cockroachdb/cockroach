...
func (c *cluster) transferLease(nodeIdx int, r *rand.Rand, key roachpb.Key) (bool, error) {
	...
	// TODO(peter): This is using too much knowledge about the internals of
	// leases.
	now := hlc.Timestamp{
		WallTime: timeutil.Now().UnixNano(),
	}
	duration := int64(4.5 * float64(time.Second))
	maxOffset := int64(250 * time.Millisecond)
	req := &roachpb.TransferLeaseRequest{
		Span: roachpb.Span{
			Key: key,
		},
		Lease: roachpb.Lease{
			Start:       now,
			StartStasis: now.Add(duration, 0),
			Expiration:  now.Add(duration+maxOffset, 0),
			Replica:     desc.Replicas[r.Intn(len(desc.Replicas))],
		},
	}
	sender := c.clients[nodeIdx].GetSender()
	if _, pErr := client.SendWrapped(sender, nil, req); pErr != nil {
		return false, errors.Errorf("%s: transfer lease: %s", key, pErr)
	}
	return true, nil
}
