...
Use the `engflow_artifacts.py` script at
`.claude/skills/engflow-artifacts/engflow_artifacts.py` to download
artifacts. Authentication is handled via environment variables set by
the workflow — no manual login needed. The workflow is:

1. **Discover failed targets:**
   ```bash
   python3 .claude/skills/engflow-artifacts/engflow_artifacts.py targets <invocation_id>
   ```

2. **List artifacts for a target:**
   ```bash
   python3 .claude/skills/engflow-artifacts/engflow_artifacts.py list <invocation_id> --target <target_label>
   ```

3. **Download artifacts for a shard:**
   ```bash
   DEST="/tmp/engflow-artifacts"
   python3 .claude/skills/engflow-artifacts/engflow_artifacts.py download <invocation_id> \
